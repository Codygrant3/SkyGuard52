...
$combined = Join-Path $wave "wave01_all_proposals.json"
$payload = [ordered]@{
  wave = $WaveId
  model = $Model
...
$summaryPath = Join-Path $wave "wave01_job_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $summaryPath -Encoding utf8

$listPath = Join-Path $wave "wave01_proposals_list.json"
...
$rankedCsv = Join-Path $wave "wave01_ranked.csv"
...
[void]$lines.Add("Base freeze: **L52** (11/11 usable). L53 rejected.")
