...
$combined = Join-Path $wave "wave02_all_proposals.json"
$payload = [ordered]@{
  wave = $WaveId
  model = $Model
...
$summaryPath = Join-Path $wave "wave02_job_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $summaryPath -Encoding utf8

$listPath = Join-Path $wave "wave02_proposals_list.json"
...
$rankedCsv = Join-Path $wave "wave02_ranked.csv"
...
[void]$lines.Add("Base freeze: **L54** densify+thin-art (11/11). L52 densify recipe; L53 rejected.")
