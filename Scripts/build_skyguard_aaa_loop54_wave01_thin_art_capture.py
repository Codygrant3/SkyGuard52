...
PREFIX = "AAA_L54_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L54"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L54"
...
def densify():
    ...
    log("loop54 mat palette size=%d" % len(mats))
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop54 wave01 thin art densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L54", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L54_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop54 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l52_freeze_wave01_selected_thin_art_7\n")
...
def main():
    log("loop54 L52 freeze + wave01 selected thin art deltas (7 proposals) start")
    ...
    log("Loop54 complete stills=%d" % (len(saved) if saved else 0))
