...
PREFIX = "AAA_L52_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L52"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L52"
...
def densify():
    ...
    log("loop52 mat palette size=%d" % len(mats))
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop52 cockpit-combat densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L52", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L52_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop52 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l51_freeze_cockpit_combat_hf_densify_keep_ads\n")
...
def main():
    log("loop52 L51 freeze + Cockpit/Combat HF densify (keep ADS) start")
    ...
    log("Loop52 complete stills=%d" % (len(saved) if saved else 0))
