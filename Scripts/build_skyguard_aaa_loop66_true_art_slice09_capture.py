...
PREFIX = "AAA_L66_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L66"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L66"
...
def create_textured_material(name, albedo_path, normal_path=None, rough_path=None, metallic_const=0.15, emissive_scale=0.0, brighten=1.12, uv_scale=1.0, rough_bias=0.0):
    ...
    try:
        ...
        try:
            ...
            pass
        log("slice09 mat ready " + name)
        return mat
    except Exception as e:
        log("slice09 mat fail %s %s" % (name, e))
        return None

def ensure_slice09_materials():
    global _SLICE09_MATS
    if _SLICE09_MATS is not None:
        return _SLICE09_MATS
    mats = {}
    ...
    # Slice09: stronger lighting-response variants (once-authored, load-existing preferred)
    light_specs = [
...
def densify():
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        if name in ("YakBeauty", "Prop", "PropNose", "Wide"):
            if sm_yak:
                spawn_sm(sm_yak, (bx + 3.55, cy - 0.2, cz - 0.4), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA9_MAT01_YakAnchor", mats=hero_air_slots)
            else:
                spawn_sm(cube, (bx + 3.4, cy - 0.2, cz + 0.1), (0.12, 1.8, 0.7), None, PREFIX + "TA9_MAT01_AirframePanel", m_air)
            for k in range(4):
                spawn_sm(cube, (bx + 3.46, cy - 0.65 + k * 0.4, cz + 0.1 + (k % 2) * 0.14), (0.05, 0.4, 0.24), None, PREFIX + "TA9_MAT01_Plate_%d" % k, m_plate)
            for k in range(5):
                spawn_sm(cube, (bx + 3.44, cy - 0.7 + k * 0.3, cz + 0.02), (0.04, 0.07, 0.32), None, PREFIX + "TA9_MAT01_AOSeam_%d" % k, m_air_ao)
            spawn_sm(cube, (bx + 3.45, cy + 0.35, cz + 0.35), (0.06, 0.95, 0.22), unreal.Rotator(0, 0, 8), PREFIX + "TA9_MAT01_MetalStrip", m_plate)
            spawn_sm(cube, (bx + 3.43, cy + 0.35, cz + 0.2), (0.04, 0.9, 0.05), None, PREFIX + "TA9_MAT01_AOStrip", m_plate_ao)
            spawn_sm(cube, (bx + 3.42, cy - 0.55, cz + 0.05), (0.05, 0.55, 0.12), None, PREFIX + "TA9_MAT01_RustBreak", m_rust_d)
            spawn_sm(cube, (bx + 3.43, cy + 0.6, cz - 0.05), (0.05, 0.35, 0.1), None, PREFIX + "TA9_MAT01_RustEdge", m_rust)
            for k in range(8):
                spawn_sm(sphere, (bx + 3.5, cy - 0.4 + k * 0.14, cz + 0.7), (0.028, 0.028, 0.028), None, PREFIX + "TA9_MAT01_Rivet_%d" % k, m_plate)
            for k in range(6):
                spawn_sm(sphere, (bx + 3.49, cy + 0.05 + k * 0.09, cz + 0.4), (0.022, 0.022, 0.022), None, PREFIX + "TA9_MAT01_Screw_%d" % k, m_plate_ao)
            spawn_sm(sphere, (bx + 3.25, cy - 0.9, cz + 0.15), (0.07, 0.07, 0.07), None, PREFIX + "TA9_MAT04_ExhaustPin", m_glow)
            spawn_sm(sphere, (bx + 3.28, cy - 0.95, cz + 0.12), (0.05, 0.05, 0.05), None, PREFIX + "TA9_MAT04_ExhaustCore", m_muzzle_hot)
            spawn_sm(sphere, (bx + 3.22, cy - 0.85, cz + 0.08), (0.04, 0.04, 0.04), None, PREFIX + "TA9_MAT04_ExhaustHalo", unlit_y or m_muzzle_hot)
            if name == "Prop" and sm_prop:
                spawn_sm(sm_prop, (bx + 3.7, cy + 0.0, cz + 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 0, 20), PREFIX + "TA9_MAT01_PropDisc", mats=[m_plate, m_air, m_plate_ao, m_glow])
                spawn_niagara(PREFIX + "TA9_VFX_PropWash", (bx + 3.65, cy + 0.0, cz + 0.05), "NS_PropWash", (0.5, 0.5, 0.5))
                spawn_niagara(PREFIX + "TA9_VFX_Contrail", (bx + 3.8, cy + 0.1, cz + 0.0), "NS_ContrailRibbon", (0.35, 0.35, 0.35))
            if name in ("YakBeauty", "Wide") and sm_cock:
                spawn_sm(sm_cock, (bx + 3.65, cy + 0.8, cz - 0.2), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA9_AIR_CockpitTub", mats=[m_air_ao, m_plate, m_glass, m_plaster])

        # City multi-slot heroes + denser facade language
        if name in ("City", "Wide", "Harbor"):
            if sm_tower:
                spawn_sm(sm_tower, (bx + 3.6, cy + 1.6, cz - 1.0), (0.45, 0.45, 0.55), unreal.Rotator(0, -20, 0), PREFIX + "TA9_MAT02_ConcreteFacade", mats=hero_city_slots)
            else:
                spawn_sm(cube, (bx + 3.35, cy + 1.6, cz + 0.2), (0.1, 1.4, 2.2), None, PREFIX + "TA9_MAT02_ConcreteSlab", m_conc)
            for k in range(4):
                spawn_sm(cube, (bx + 3.37, cy + 1.1 + k * 0.3, cz - 0.42 + k * 0.04), (0.04, 0.95, 0.04), None, PREFIX + "TA9_MAT02_AORecess_%d" % k, m_conc_ao)
                spawn_sm(cube, (bx + 3.38, cy + 1.1 + k * 0.3, cz - 0.35 + k * 0.04), (0.05, 0.9, 0.08), None, PREFIX + "TA9_MAT02_Sill_%d" % k, m_plaster)
            spawn_sm(cube, (bx + 3.4, cy + 1.6, cz + 0.85), (0.05, 1.15, 0.08), None, PREFIX + "TA9_MAT02_PlasterLintel", m_plaster)
            for iy in range(2):
                for ix in range(3):
                    spawn_sm(cube, (bx + 3.36, cy + 1.15 + ix * 0.4, cz + 0.0 + iy * 0.4), (0.04, 0.18, 0.26), None, PREFIX + "TA9_MAT02_Glass_%d_%d" % (ix, iy), m_glass)
                    spawn_sm(cube, (bx + 3.35, cy + 1.15 + ix * 0.4, cz - 0.05 + iy * 0.4), (0.03, 0.22, 0.04), None, PREFIX + "TA9_MAT02_WinAO_%d_%d" % (ix, iy), m_conc_ao)
            if sm_apt:
                spawn_sm(sm_apt, (bx + 3.7, cy - 1.8, cz - 0.8), (0.4, 0.4, 0.5), unreal.Rotator(0, 15, 0), PREFIX + "TA9_MAT03_BrickFacade", mats=hero_brick_slots)
            else:
                spawn_sm(cube, (bx + 3.4, cy - 1.8, cz + 0.1), (0.1, 1.1, 1.8), None, PREFIX + "TA9_MAT03_BrickSlab", m_brick)
            for k in range(5):
                spawn_sm(cube, (bx + 3.42, cy - 1.8, cz + 0.7 - k * 0.24), (0.05, 1.0, 0.055), None, PREFIX + "TA9_MAT03_BrickCourse_%d" % k, m_brick_d if k % 2 == 0 else m_brick)
            spawn_sm(cube, (bx + 3.44, cy - 1.5, cz + 0.2), (0.04, 0.2, 0.5), None, PREFIX + "TA9_MAT03_BrickQuoin", m_plaster)
            spawn_sm(cube, (bx + 3.35, cy + 2.2, cz - 0.7), (0.08, 0.22, 0.16), None, PREFIX + "TA9_MAT02_UtilBox", m_plate)
            spawn_sm(cube, (bx + 3.34, cy + 2.2, cz - 0.55), (0.05, 0.1, 0.05), None, PREFIX + "TA9_MAT02_UtilLatch", m_plate_ao)
            if name in ("City", "Wide"):
                spawn_niagara(PREFIX + "TA9_VFX_CityFire", (bx + 3.5, cy + 0.5, cz + 0.8), "NS_CityFire", (0.3, 0.3, 0.3))
                spawn_niagara(PREFIX + "TA9_VFX_CloudWisps", (bx + 3.8, cy - 0.5, cz + 1.4), "NS_CloudWisps", (0.4, 0.4, 0.4))
            if name == "Harbor":
                if sm_crane:
                    spawn_sm(sm_crane, (bx + 3.8, cy + 2.5, cz - 0.9), (0.4, 0.4, 0.4), unreal.Rotator(0, -30, 0), PREFIX + "TA9_HARBOR_Crane", mats=[m_plate, m_plate_ao, m_rust_d, m_air_ao])
                if sm_ship:
                    spawn_sm(sm_ship, (bx + 3.9, cy - 2.5, cz - 1.1), (0.45, 0.45, 0.45), unreal.Rotator(0, 15, 0), PREFIX + "TA9_HARBOR_Ship", mats=[m_plate, m_conc, m_rust, m_plate_ao])
                if sm_sub:
                    spawn_sm(sm_sub, (bx + 3.7, cy - 0.2, cz - 1.2), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA9_HARBOR_Sub", mats=[m_plate_ao, m_air_ao, m_rust_d, m_waterline])
        ...
        if name in ("Combat", "ADS", "Cockpit"):
            if sm_rifle and name in ("ADS", "Cockpit"):
                spawn_sm(sm_rifle, (bx + 3.2, cy + 0.15, cz - 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA9_WPN_Rifle", mats=hero_wpn_slots)
            if sm_glove and name in ("ADS", "Cockpit"):
                spawn_sm(sm_glove, (bx + 3.15, cy + 0.0, cz - 0.25), (0.3, 0.3, 0.3), unreal.Rotator(0, 90, 0), PREFIX + "TA9_WPN_Glove", mats=[load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_rust, m_rust_d, m_air_ao])
            if sm_arm and name in ("ADS", "Cockpit"):
                spawn_sm(sm_arm, (bx + 3.18, cy - 0.15, cz - 0.35), (0.28, 0.28, 0.28), unreal.Rotator(0, 90, 0), PREFIX + "TA9_WPN_Arm", mats=[load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_rust, m_rust_d])
            if sm_instr and name == "Cockpit":
                spawn_sm(sm_instr, (bx + 3.25, cy + 0.4, cz + 0.35), (0.25, 0.25, 0.25), None, PREFIX + "TA9_COCK_Instruments", mats=[m_plate, m_glass, m_glow, m_plate_ao])
            if sm_station and name in ("Cockpit", "ADS"):
                spawn_sm(sm_station, (bx + 3.35, cy - 0.4, cz - 0.15), (0.3, 0.3, 0.3), unreal.Rotator(0, 90, 0), PREFIX + "TA9_COCK_Station", mats=[m_air_ao, m_plate, m_rust_d, m_plaster])
            if sm_igla and name == "Combat":
                spawn_sm(sm_igla, (bx + 3.3, cy + 0.9, cz + 0.2), (0.3, 0.3, 0.3), unreal.Rotator(10, 90, 0), PREFIX + "TA9_WPN_Igla", mats=[m_plate, m_plate_ao, m_glow, m_rust])
            spawn_niagara(PREFIX + "TA9_VFX01_Muzzle", (bx + 2.45, cy + 0.35, cz + 0.2), "NS_MuzzleFlash", (0.65, 0.65, 0.65))
            spawn_niagara(PREFIX + "TA9_VFX01_GunSmoke", (bx + 2.5, cy + 0.25, cz + 0.15), "NS_GunSmoke", (0.5, 0.5, 0.5))
            spawn_niagara(PREFIX + "TA9_VFX01_Tracer", (bx + 2.55, cy + 0.4, cz + 0.25), "NS_TracerBurst", (0.45, 0.45, 0.45))
            spawn_niagara(PREFIX + "TA9_VFX01_Shells", (bx + 2.35, cy + 0.15, cz + 0.05), "NS_ShellCasings", (0.35, 0.35, 0.35))
            if name == "Combat":
                spawn_niagara(PREFIX + "TA9_VFX01_Missile", (bx + 2.7, cy - 0.2, cz + 0.4), "NS_MissileTrail", (0.35, 0.35, 0.35))
                spawn_niagara(PREFIX + "TA9_VFX01_Igla", (bx + 2.8, cy + 0.5, cz + 0.1), "NS_IglaLaunch", (0.3, 0.3, 0.3))
                spawn_niagara(PREFIX + "TA9_VFX01_DroneTrail", (bx + 3.2, cy - 1.1, cz + 0.55), "NS_DroneTrail", (0.3, 0.3, 0.3))
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.4, cy - 1.0, cz + 0.6), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 10), PREFIX + "TA9_ENEMY_Shahed", mats=[m_plate_ao, m_air_ao, m_rust_d, m_expl])
                spawn_niagara(PREFIX + "TA9_VFX01_DroneExpl", (bx + 3.35, cy - 1.0, cz + 0.7), "NS_DroneExplosion", (0.4, 0.4, 0.4))
                spawn_sm(sphere, (bx + 3.35, cy - 1.0, cz + 0.7), (0.13, 0.13, 0.13), None, PREFIX + "TA9_VFX01_ExplCore", m_expl)
            spawn_sm(sphere, (bx + 2.42, cy + 0.28, cz + 0.18), (0.12, 0.12, 0.12), None, PREFIX + "TA9_VFX01_CoreA", m_muzzle_hot)
            spawn_sm(sphere, (bx + 2.48, cy + 0.42, cz + 0.22), (0.08, 0.08, 0.08), None, PREFIX + "TA9_VFX01_CoreB", unlit_w or m_bright_white or mats[1])
            spawn_sm(sphere, (bx + 2.52, cy + 0.33, cz + 0.28), (0.05, 0.05, 0.05), None, PREFIX + "TA9_VFX01_CoreC", unlit_y or m_muzzle_hot)
            for k in range(5):
                spawn_sm(cube, (bx + 2.34 + k * 0.03, cy + 0.26 + k * 0.045, cz + 0.15 + k * 0.03), (0.18 - k * 0.02, 0.02, 0.02), unreal.Rotator(0, -15 + k * 6, 0), PREFIX + "TA9_VFX01_Filament_%d" % k, m_muzzle_hot)
            for k in range(5):
                spawn_sm(sphere, (bx + 2.55, cy + 0.06 + k * 0.06, cz + 0.0 + k * 0.025), (0.028, 0.028, 0.028), None, PREFIX + "TA9_VFX01_Shell_%d" % k, m_plate)

        # Ocean multi-Niagara + denser foam crown
        if name in ("Ocean", "Harbor"):
            spawn_niagara(PREFIX + "TA9_VFX02_OceanSpray", (bx + 3.3, cy - 0.5, cz - 0.7), "NS_OceanSpray", (0.8, 0.8, 0.8))
            spawn_niagara(PREFIX + "TA9_VFX02_WaterSplash", (bx + 3.35, cy - 0.2, cz - 0.75), "NS_WaterSplash", (0.6, 0.6, 0.6))
            spawn_sm(cube, (bx + 3.22, cy - 0.45, cz - 0.85), (0.08, 0.9, 0.06), None, PREFIX + "TA9_VFX02_Waterline", m_waterline)
            for k in range(7):
                spawn_sm(sphere, (bx + 3.24, cy - 0.85 + k * 0.15, cz - 0.55 + (k % 2) * 0.08), (0.045 + (k % 2) * 0.015, 0.045 + (k % 2) * 0.015, 0.045 + (k % 2) * 0.015), None, PREFIX + "TA9_VFX02_Foam_%d" % k, m_foam)
            for k in range(6):
                spawn_sm(sphere, (bx + 3.3, cy - 0.45 + k * 0.09, cz - 0.4), (0.028, 0.028, 0.028), None, PREFIX + "TA9_VFX02_Mist_%d" % k, m_foam)

        # Impact multi-Niagara + denser spark/debris language
        if name in ("City", "Combat", "Harbor"):
            spawn_niagara(PREFIX + "TA9_VFX03_HitSparks", (bx + 3.35, cy + 1.2, cz + 0.4), "NS_HitSparks", (0.7, 0.7, 0.7))
            spawn_niagara(PREFIX + "TA9_VFX03_Flak", (bx + 3.4, cy + 1.0, cz + 0.55), "NS_FlakBurst", (0.45, 0.45, 0.45))
            spawn_sm(sphere, (bx + 3.3, cy + 1.2, cz + 0.4), (0.11, 0.11, 0.11), None, PREFIX + "TA9_VFX03_SparkCore", m_muzzle_hot)
            for k in range(8):
                spawn_sm(sphere, (bx + 3.28, cy + 0.88 + k * 0.07, cz + 0.45 + k * 0.028), (0.028, 0.028, 0.028), None, PREFIX + "TA9_VFX03_Spark_%d" % k, m_muzzle_hot)
            for k in range(5):
                spawn_sm(cube, (bx + 3.32, cy + 0.98 + k * 0.06, cz + 0.2), (0.03, 0.05, 0.03), unreal.Rotator(0, k * 16, 0), PREFIX + "TA9_VFX03_Debris_%d" % k, m_plate_ao)
    ...
    log("loop66 true-art slice09 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L66", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L65_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop66 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l64_freeze_true_art_slice09_multislot_hero_multiniagara\n")
...
def main():
    log("loop66 L64 freeze + true-art slice09 multi-slot hero materials + multi-niagara start")
    ...
    log("Loop66 complete stills=%d" % (len(saved) if saved else 0))
