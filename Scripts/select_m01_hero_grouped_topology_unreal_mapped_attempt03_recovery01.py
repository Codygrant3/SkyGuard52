"""Select one global physical-light rig from the recovery_01 sweep."""
...
BASE = load_module("skyguard_attempt03_selector_helpers", BASE_SELECTOR_PATH)
...
def main() -> int:
    ...
    if output.exists():
        raise RuntimeError("immutable recovery_01 selector output already exists")
    ...
    if (
        ...
        raise RuntimeError("recovery sweep manifest gate is not ready")
    if manifest.get("capture_count") != contract["capture"]["capture_count"]:
        raise RuntimeError("recovery sweep count differs from contract")
    policy = contract["selector"]
    evaluations: dict[str, list] = {}
    for record in manifest["captures"]:
        path = Path(record["path"])
        if not path.is_file() or sha256_file(path) != record["sha256"]:
            raise RuntimeError("recovery capture hash changed: " + str(path))
        metrics = BASE.image_metrics(
            path, int(policy["active_pixel_threshold_luma"])
        )
        if metrics["dimensions"] != contract["capture"]["resolution"]:
            raise RuntimeError("recovery capture dimensions changed")
    ...
    expected_rigs = [record["rig_id"] for record in contract["capture"]["rig_candidates"]]
    if sorted(evaluations) != sorted(expected_rigs):
        raise RuntimeError("recovery rig set differs from contract")
    candidates = []
    for rig in contract["capture"]["rig_candidates"]:
        records = evaluations[rig["rig_id"]]
        if len(records) != 9:
            raise RuntimeError("each recovery rig must contain exactly nine views")
        candidates.append(
            {
                "rig_id": rig["rig_id"],
    ...
    if not eligible:
        report = {
            "schema": "skyguard.m01.hero-grouped-topology-unreal-mapped-attempt03-recovery01-selection.v1",
            "gate": "FAIL_CLOSED_NO_GLOBAL_PHYSICAL_RIG_PASSED_ALL_NINE_HARD_BOUNDS",
    ...
    if len(canonical) != policy["canonical_capture_count"]:
        raise RuntimeError("canonical recovery count differs from contract")
    report = {
        "schema": "skyguard.m01.hero-grouped-topology-unreal-mapped-attempt03-recovery01-selection.v1",
        "gate": "PASS_RECOVERY01_GLOBAL_RIG_SELECTED_AWAITING_ORIGINAL_RESOLUTION_REVIEW",
