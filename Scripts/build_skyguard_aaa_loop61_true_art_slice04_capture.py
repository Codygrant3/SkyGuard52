...
PREFIX = "AAA_L61_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L61"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L61"
...
def create_slice04_material(name, base_color, roughness=0.55, metallic=0.0, specular=0.5, emissive=None):
    """Author a bright-readable material graph under Generated for Slice04 capture."""
    ...
    try:
        ...
        base.set_editor_property("constant", unreal.LinearColor(float(base_color[0]), float(base_color[1]), float(base_color[2]), 1.0))
        # brighten path so capture does not collapse to dark PBR mass
        mult = mel.create_material_expression(mat, unreal.MaterialExpressionMultiply, -250, -120)
        gain = mel.create_material_expression(mat, unreal.MaterialExpressionConstant, -500, -20)
        gain.set_editor_property("r", 1.15)
        ...
        if emissive is not None:
            ...
            em.set_editor_property("constant", unreal.LinearColor(float(emissive[0]), float(emissive[1]), float(emissive[2]), 1.0))
        ...
        try:
            ...
            pass
        log("slice04 mat ready " + name)
        return mat
    except Exception as e:
        log("slice04 mat fail %s %s" % (name, e))
        return None

def ensure_slice04_materials():
    mats = {}
    mats["air_bright"] = create_slice04_material("M_L61_AirframeBright", (0.72, 0.74, 0.70), roughness=0.38, metallic=0.55, specular=0.7)
    mats["plate_bright"] = create_slice04_material("M_L61_PlateBright", (0.78, 0.79, 0.82), roughness=0.32, metallic=0.7, specular=0.75)
    mats["rust_warm"] = create_slice04_material("M_L61_RustWarm", (0.48, 0.24, 0.12), roughness=0.72, metallic=0.35, specular=0.4)
    mats["concrete_lit"] = create_slice04_material("M_L61_ConcreteLit", (0.62, 0.60, 0.57), roughness=0.78, metallic=0.02, specular=0.35)
    mats["brick_warm"] = create_slice04_material("M_L61_BrickWarm", (0.62, 0.34, 0.24), roughness=0.8, metallic=0.0, specular=0.3)
    mats["plaster_lit"] = create_slice04_material("M_L61_PlasterLit", (0.86, 0.84, 0.78), roughness=0.7, metallic=0.0, specular=0.25)
    mats["glass_lit"] = create_slice04_material("M_L61_GlassLit", (0.35, 0.55, 0.65), roughness=0.08, metallic=0.05, specular=0.95, emissive=(0.03, 0.05, 0.07))
    mats["muzzle_hot"] = create_slice04_material("M_L61_MuzzleHot", (1.0, 0.72, 0.22), roughness=0.25, metallic=0.0, specular=0.5, emissive=(2.2, 1.1, 0.2))
    mats["foam_lit"] = create_slice04_material("M_L61_FoamLit", (0.92, 0.95, 0.98), roughness=0.45, metallic=0.0, specular=0.6, emissive=(0.08, 0.1, 0.12))
    mats["waterline"] = create_slice04_material("M_L61_Waterline", (0.12, 0.38, 0.48), roughness=0.2, metallic=0.05, specular=0.8, emissive=(0.02, 0.05, 0.07))
    return mats
...
def densify():
    ...
    log("loop61 mat palette size=%d" % len(mats))
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # ---- TRUE ART SLICE04 (L61 / L60 freeze) ----
        # Authored Slice04 materials + multi-mat language + multi-Niagara clusters.
        # Capture-safe: L52 FOV densify retained; opaque x>=bx+3; no L53 multi-hero stacks.
        s4 = ensure_slice04_materials()
        m_air = s4.get("air_bright") or load_mat("/Game/Skyguard/Materials/M_Tex_airframe_metal") or load_mat("/Game/Skyguard/Materials/Generated/M_AirframeMetal") or load_mat("/Game/Skyguard/Materials/M_Metal")
        m_plate = s4.get("plate_bright") or load_mat("/Game/Skyguard/Materials/M_Tex_L3_plate") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Panel") or m_air
        m_rust = s4.get("rust_warm") or load_mat("/Game/Skyguard/Materials/M_MetalRust") or load_mat("/Game/Skyguard/Materials/M_Tex_L4_rust")
        m_conc = s4.get("concrete_lit") or load_mat("/Game/Skyguard/Materials/M_Tex_concrete") or load_mat("/Game/Skyguard/Materials/M_CityConcrete")
        m_brick = s4.get("brick_warm") or load_mat("/Game/Skyguard/Materials/M_Tex_brick") or load_mat("/Game/Skyguard/Materials/Generated/M_BrickFacade")
        m_plaster = s4.get("plaster_lit") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Plaster") or unlit_w
        m_glass = s4.get("glass_lit") or load_mat("/Game/Skyguard/Materials/M_CityGlass") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan")
        m_muzzle_hot = s4.get("muzzle_hot") or load_mat("/Game/Skyguard/Materials/Generated/MI_MuzzleFlash_Hot") or unlit_y
        m_foam = s4.get("foam_lit") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Foam") or unlit_w
        m_waterline = s4.get("waterline") or load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightOcean") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan")
        m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or m_muzzle_hot or unlit_y
        ...
        # Aircraft authored multi-mat readability
        if name in ("YakBeauty", "Prop", "PropNose", "Wide"):
            if sm_yak:
                spawn_sm(sm_yak, (bx + 3.55, cy - 0.2, cz - 0.4), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA4_MAT01_YakAnchor", m_air or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 0.2, cz + 0.1), (0.12, 1.8, 0.7), None, PREFIX + "TA4_MAT01_AirframePanel", m_air or mats[0])
            spawn_sm(cube, (bx + 3.45, cy + 0.35, cz + 0.35), (0.06, 0.95, 0.22), unreal.Rotator(0, 0, 8), PREFIX + "TA4_MAT01_MetalStrip", m_plate or mats[1])
            spawn_sm(cube, (bx + 3.48, cy + 0.1, cz + 0.55), (0.05, 0.55, 0.35), None, PREFIX + "TA4_MAT01_PlateA", m_plate or mats[0])
            spawn_sm(cube, (bx + 3.48, cy - 0.35, cz + 0.15), (0.05, 0.45, 0.28), None, PREFIX + "TA4_MAT01_PlateB", m_plate or mats[1])
            spawn_sm(cube, (bx + 3.42, cy - 0.55, cz + 0.05), (0.05, 0.55, 0.12), None, PREFIX + "TA4_MAT01_RustBreak", m_rust or mats[0])
            for k in range(5):
                spawn_sm(sphere, (bx + 3.5, cy - 0.25 + k * 0.2, cz + 0.7), (0.04, 0.04, 0.04), None, PREFIX + "TA4_MAT01_Rivet_%d" % k, m_plate or unlit_w or mats[k % nmat])
            spawn_sm(sphere, (bx + 3.25, cy - 0.9, cz + 0.15), (0.07, 0.07, 0.07), None, PREFIX + "TA4_MAT04_ExhaustPin", m_glow or unlit_y or mats[0])
            spawn_sm(sphere, (bx + 3.28, cy - 0.95, cz + 0.12), (0.05, 0.05, 0.05), None, PREFIX + "TA4_MAT04_ExhaustCore", m_muzzle_hot or unlit_y or mats[1])
            if name == "Prop" and sm_prop:
                spawn_sm(sm_prop, (bx + 3.7, cy + 0.0, cz + 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 0, 20), PREFIX + "TA4_MAT01_PropDisc", m_plate or mats[0])
                spawn_niagara(PREFIX + "TA4_VFX_PropWash", (bx + 3.65, cy + 0.0, cz + 0.05), "NS_PropWash", (0.35, 0.35, 0.35))

        # City authored multi-mat facade language
        if name in ("City", "Wide", "Harbor"):
            if sm_tower:
                spawn_sm(sm_tower, (bx + 3.6, cy + 1.6, cz - 1.0), (0.45, 0.45, 0.55), unreal.Rotator(0, -20, 0), PREFIX + "TA4_MAT02_ConcreteFacade", m_conc or mats[1])
            else:
                spawn_sm(cube, (bx + 3.35, cy + 1.6, cz + 0.2), (0.1, 1.4, 2.2), None, PREFIX + "TA4_MAT02_ConcreteSlab", m_conc or mats[1])
            spawn_sm(cube, (bx + 3.38, cy + 1.6, cz - 0.35), (0.06, 1.25, 0.1), None, PREFIX + "TA4_MAT02_ConcreteSill", m_plaster or mats[1])
            spawn_sm(cube, (bx + 3.4, cy + 1.6, cz + 0.85), (0.05, 1.15, 0.08), None, PREFIX + "TA4_MAT02_PlasterLintel", m_plaster or unlit_w or mats[0])
            spawn_sm(cube, (bx + 3.36, cy + 1.35, cz + 0.25), (0.04, 0.28, 0.35), None, PREFIX + "TA4_MAT02_GlassA", m_glass or mats[2 % nmat])
            spawn_sm(cube, (bx + 3.36, cy + 1.85, cz + 0.25), (0.04, 0.28, 0.35), None, PREFIX + "TA4_MAT02_GlassB", m_glass or mats[3 % nmat])
            if sm_apt:
                spawn_sm(sm_apt, (bx + 3.7, cy - 1.8, cz - 0.8), (0.4, 0.4, 0.5), unreal.Rotator(0, 15, 0), PREFIX + "TA4_MAT03_BrickFacade", m_brick or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 1.8, cz + 0.1), (0.1, 1.1, 1.8), None, PREFIX + "TA4_MAT03_BrickSlab", m_brick or mats[0])
            spawn_sm(cube, (bx + 3.42, cy - 1.8, cz + 0.75), (0.05, 1.05, 0.08), None, PREFIX + "TA4_MAT03_BrickHeader", m_brick or unlit_y or mats[0])
            spawn_sm(cube, (bx + 3.43, cy - 1.8, cz - 0.2), (0.05, 0.9, 0.07), None, PREFIX + "TA4_MAT03_BrickCourse", m_brick or mats[1])
            spawn_sm(cube, (bx + 3.44, cy - 1.5, cz + 0.2), (0.04, 0.2, 0.5), None, PREFIX + "TA4_MAT03_BrickQuoin", m_plaster or mats[1])

        # Combat/ADS multi-Niagara + weapon readability
        if name in ("Combat", "ADS", "Cockpit"):
            if sm_rifle and name in ("ADS", "Cockpit"):
                spawn_sm(sm_rifle, (bx + 3.2, cy + 0.15, cz - 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA4_WPN_Rifle", load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate or mats[0])
            if sm_glove and name in ("ADS", "Cockpit"):
                spawn_sm(sm_glove, (bx + 3.15, cy + 0.0, cz - 0.25), (0.3, 0.3, 0.3), unreal.Rotator(0, 90, 0), PREFIX + "TA4_WPN_Glove", load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_rust or mats[1])
            spawn_niagara(PREFIX + "TA4_VFX01_Muzzle", (bx + 2.45, cy + 0.35, cz + 0.2), "NS_MuzzleFlash", (0.5, 0.5, 0.5))
            spawn_niagara(PREFIX + "TA4_VFX01_GunSmoke", (bx + 2.5, cy + 0.25, cz + 0.15), "NS_GunSmoke", (0.35, 0.35, 0.35))
            spawn_niagara(PREFIX + "TA4_VFX01_Tracer", (bx + 2.55, cy + 0.4, cz + 0.25), "NS_TracerBurst", (0.3, 0.3, 0.3))
            spawn_sm(sphere, (bx + 2.42, cy + 0.28, cz + 0.18), (0.12, 0.12, 0.12), None, PREFIX + "TA4_VFX01_CoreA", m_muzzle_hot or unlit_y or mats[0])
            spawn_sm(sphere, (bx + 2.48, cy + 0.42, cz + 0.22), (0.08, 0.08, 0.08), None, PREFIX + "TA4_VFX01_CoreB", unlit_w or m_bright_white or mats[1])
            spawn_sm(cube, (bx + 2.4, cy + 0.35, cz + 0.2), (0.2, 0.03, 0.03), unreal.Rotator(0, -12, 0), PREFIX + "TA4_VFX01_FilamentA", m_muzzle_hot or unlit_y or mats[0])
            spawn_sm(cube, (bx + 2.43, cy + 0.3, cz + 0.25), (0.14, 0.025, 0.025), unreal.Rotator(0, 10, 0), PREFIX + "TA4_VFX01_FilamentB", unlit_y or mats[1])
            spawn_sm(sphere, (bx + 2.55, cy + 0.2, cz + 0.1), (0.05, 0.05, 0.05), None, PREFIX + "TA4_VFX01_ShellHint", m_plate or mats[2 % nmat])

        # Ocean multi-Niagara + authored foam/waterline
        if name in ("Ocean", "Harbor"):
            spawn_niagara(PREFIX + "TA4_VFX02_OceanSpray", (bx + 3.3, cy - 0.5, cz - 0.7), "NS_OceanSpray", (0.65, 0.65, 0.65))
            spawn_niagara(PREFIX + "TA4_VFX02_WaterSplash", (bx + 3.35, cy - 0.2, cz - 0.75), "NS_WaterSplash", (0.45, 0.45, 0.45))
            spawn_sm(sphere, (bx + 3.25, cy - 0.55, cz - 0.62), (0.1, 0.1, 0.1), None, PREFIX + "TA4_VFX02_FoamA", m_foam or unlit_w or mats[1])
            spawn_sm(sphere, (bx + 3.32, cy - 0.35, cz - 0.55), (0.07, 0.07, 0.07), None, PREFIX + "TA4_VFX02_FoamB", m_foam or unlit_w or mats[0])
            spawn_sm(sphere, (bx + 3.28, cy - 0.65, cz - 0.5), (0.055, 0.055, 0.055), None, PREFIX + "TA4_VFX02_Droplet", unlit_y or mats[1])
            spawn_sm(cube, (bx + 3.22, cy - 0.45, cz - 0.85), (0.08, 0.75, 0.06), None, PREFIX + "TA4_VFX02_Waterline", m_waterline or mats[2 % nmat])
            for k in range(3):
                spawn_sm(sphere, (bx + 3.27, cy - 0.7 + k * 0.2, cz - 0.45), (0.04, 0.04, 0.04), None, PREFIX + "TA4_VFX02_Mist_%d" % k, m_foam or unlit_w or mats[k % nmat])

        # Impact multi-Niagara + spark chain
        if name in ("City", "Combat", "Harbor"):
            spawn_niagara(PREFIX + "TA4_VFX03_HitSparks", (bx + 3.35, cy + 1.2, cz + 0.4), "NS_HitSparks", (0.55, 0.55, 0.55))
            spawn_niagara(PREFIX + "TA4_VFX03_Flak", (bx + 3.4, cy + 1.0, cz + 0.55), "NS_FlakBurst", (0.3, 0.3, 0.3))
            spawn_sm(sphere, (bx + 3.3, cy + 1.2, cz + 0.4), (0.1, 0.1, 0.1), None, PREFIX + "TA4_VFX03_SparkCore", m_muzzle_hot or unlit_y or mats[0])
            for k in range(5):
                spawn_sm(sphere, (bx + 3.28, cy + 1.0 + k * 0.1, cz + 0.45 + k * 0.04), (0.04, 0.04, 0.04), None, PREFIX + "TA4_VFX03_Spark_%d" % k, unlit_y or m_muzzle_hot or mats[k % nmat])
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop61 true-art slice04 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L61", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L61_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop61 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l60_freeze_true_art_slice04_authored_mats_multiniagara\n")
...
def main():
    log("loop61 L60 freeze + true-art slice04 authored materials + multi-niagara start")
    ...
    log("Loop61 complete stills=%d" % (len(saved) if saved else 0))
