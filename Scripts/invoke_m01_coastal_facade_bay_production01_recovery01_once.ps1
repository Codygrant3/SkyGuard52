...
$Root = 'D:\Skyguard52'
$AssetId = 'm01-coastal-facade-bay-production01-recovery01'
$ControllerPath = Join-Path $Root 'Scripts\skyguard_production.py'
...
if (-not (Test-Path -LiteralPath $ControllerPath -PathType Leaf)) {
    [Console]::Error.WriteLine('Production controller is missing.')
