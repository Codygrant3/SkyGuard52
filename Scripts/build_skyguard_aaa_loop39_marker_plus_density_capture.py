...
PREFIX = "AAA_L39_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L39"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L39"
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L39", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L39_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop39 stills\n")
...
def main():
    log("loop39 marker recipe + denser stage content start")
    ...
    log("Loop39 complete stills=%d" % (len(saved) if saved else 0))
