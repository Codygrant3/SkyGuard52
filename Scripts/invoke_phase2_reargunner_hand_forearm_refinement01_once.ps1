...
$AuthorityPath = Join-Path $ProjectRoot 'Saved\Reports\PHASE2_REARGUNNER_HAND_FOREARM_REFINEMENT01_EXECUTION_AUTHORITY.json'
...
$AssetId = 'core-reargunner-hand-forearm-refinement01'
$FutureAttemptRoot = Join-Path $ProjectRoot 'Production\Attempts\core-reargunner-hand-forearm-refinement01'
$ExpectedAuthorityHash = 'fa3b9a6ca9de5d95067f9a8c9b2d06947e8233687781b5011a2bbcd0d7c25262'
...
if ($authority.classification -ne 'PASSED_READY_FOR_EXPLICIT_SINGLE_REARGUNNER_HAND_FOREARM_REFINEMENT01_BLENDER_AUTHORIZATION') {
...
if ($asset.Count -ne 1) {
    throw 'Expected exactly one Refinement01 registry entry.'
}
if ($asset[0].worker.script -ne 'Scripts\Workers\worker_core_reargunner_hand_forearm_refinement01.py') {
    throw 'The Refinement01 worker binding is incorrect.'
}
if ([int]$asset[0].worker.minimum_renders -ne 12) {
    throw 'The governed render count is not twelve.'
}
if ($asset[0].status -ne 'ready') {
    throw "Refinement01 registry state is $($asset[0].status), not ready."
}
