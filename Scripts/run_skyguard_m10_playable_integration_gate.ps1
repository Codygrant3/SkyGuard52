...
$builder = Join-Path $ProjectRoot "Scripts\build_skyguard_m10_playable_integration.py"
$verifier = Join-Path $ProjectRoot "Scripts\verify_skyguard_m10_playable_integration.py"
$stamp = [DateTime]::UtcNow.ToString("yyyyMMddTHHmmssfffZ")
$attempt = Join-Path $ProjectRoot "Saved\Reports\M10_Playable\attempt_$stamp"
...
Invoke-Stage "automation" $cmd @(
    $project,
    "-ExecCmds=Automation RunTests Skyguard52.Mission10",
...
$audit = Get-Content (
    Join-Path $ProjectRoot `
        "Saved\Reports\M10_PLAYABLE_INTEGRATION_AUDIT.json"
...
$receipt = [ordered]@{
    schema = "skyguard.m10-playable-supervisor.v1"
    gate = $gate
    attempt = $attempt
    automation = [ordered]@{
        success = $success
...
$receipt | ConvertTo-Json -Depth 12 |
    Set-Content (
        Join-Path $ProjectRoot `
            "Saved\Reports\M10_PLAYABLE_INTEGRATION_GATE_LATEST.json"
    ) -Encoding utf8
if ($gate -ne "PASS") {
    throw "M10 playable gate failed"
}
Write-Output "M10_PLAYABLE_INTEGRATION_GATE=PASS"
