...
PREFIX = "AAA_L49_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L49"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L49"
...
def densify():
    ...
    log("loop49 mat palette size=%d" % len(mats))
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop49 pure densify + light boost done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L49", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L49_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop49 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=pure_l45_moderate_light_boost_no_hero\n")
...
def main():
    log("loop49 pure L45 densify + moderate sun/sky/board light boost (no hero overlays) start")
    ...
    log("Loop49 complete stills=%d" % (len(saved) if saved else 0))
