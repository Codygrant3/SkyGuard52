...
PREFIX = "AAA_L50_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L50"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L50"
...
def densify():
    ...
    log("loop50 mat palette size=%d" % len(mats))
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # Capture recovery densify for weak stages (Cockpit/ADS/City) - wall plane only, multi-mat unlit/bright
        if name in ("Cockpit", "ADS", "City"):
            denser = 2.2 if name == "Cockpit" else (2.4 if name == "ADS" else 2.8)
            yspan = 16 if name != "City" else 14
            zspan = 12 if name != "City" else 11
            nmat = max(len(mats), 1)
            for iy in range(-yspan, yspan + 1):
                for iz in range(-zspan, zspan + 1):
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L50", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L50_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop50 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l45_plus_cockpit_ads_city_wall_densify_fill_lights\n")
...
def main():
    log("loop50 pure L45 + Cockpit/ADS/City wall densify + stage fill lights (no hero) start")
    ...
    log("Loop50 complete stills=%d" % (len(saved) if saved else 0))
