...
PRIOR_FREEZE = ROOT / r"Docs\AAA_Review\M01_STORM_DRAIN_GROK_MCP_RECOVERY01_ATTEMPT01_TERMINAL_FREEZE.json"
PRIOR_FREEZE_BYTES = 2_005
PRIOR_FREEZE_SHA256 = "fe4f73dacfee55d422d2b3f8b5aac7a1047ffbdda7dd7b755a3c407977a3edae"
SCRIPT = ROOT / r"Scripts\GrokProduction\finalize_m01_storm_drain_deterministic_recovery02_scene.py"
SCRIPT_BYTES = 3_119
SCRIPT_SHA256 = "97bfd0c9fc60f4f96410db72c8d2dd58bfde7db781dea01b599a7e1f1f7a35df"
ATTEMPT = ROOT / r"Production\Attempts\m01-storm-drain-deterministic-recovery02\attempt_20260811T103000000000Z"
...
EXPECTED_BLEND = OUTPUT / "M01_Promenade_StormDrain_Recovery02.blend"
EXPECTED_GLB = OUTPUT / "exports" / "M01_Promenade_StormDrain_Recovery02.glb"
...
def sample_tree(pid: int) -> None:
    ...
    for process in candidates:
        try:
            rows.append({"pid": process.pid, "parent_pid": process.ppid(), "name": process.name(), "working_set": process.memory_info().rss})
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue
...
def main() -> int:
    ...
    try:
        authorities.extend([
            ...
            require_authority(PRIOR_FREEZE, PRIOR_FREEZE_BYTES, PRIOR_FREEZE_SHA256),
        ...
        if report.get("classification") != "PASSED_AWAITING_DIRECT_VISUAL_REVIEW" or report.get("slot_count") != 12:
            raise RuntimeError(f"Unexpected implementation report: {report.get('classification')} slots={report.get('slot_count')}")
        ...
        if ATTEMPT.exists():
            ...
            write_json(TERMINAL, {
                "schema": "skyguard.m01-storm-drain.deterministic-recovery02.terminal.v1",
                ...
                "source_authority": {"expected_bytes": SOURCE_BYTES, "expected_sha256": SOURCE_SHA256, "after": source_after},
                "execution": {"blender_launches": 1 if process else 0, "grok_launches": 0, "unreal_launches": 0, "retries": 0, "timed_out": timed_out, "pid": process.pid if process else None, "exit_code": exit_code, "exit_code_type": "System.Int32" if exit_code is not None else None},
                "exact_arguments": arguments,
                "artifacts": inventory,
                "runtime_promotion_performed": False,
                "next_gate": "Direct full-resolution review of all eight original renders" if classification.startswith("PASSED_") else "Preserve failure; never retry this namespace",
            })
