...
$Author = Join-Path $Root 'Scripts\GrokProduction\author_m01_window_recovery06_unrealready01_import01.py'
$Verifier = Join-Path $Root 'Scripts\GrokProduction\verify_m01_window_recovery06_unrealready01_import01_offline.py'
$Contract = Join-Path $Root 'Docs\GrokProduction\Wave02\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01_CONTRACT.json'
...
$Destination = 'D:\SG52T08_ENV01\Content\ToolchainWave08\GrokAccepted\M01WindowBayRecovery06UnrealReady01'
$Attempt = Join-Path $Root 'Saved\BuildAttempts\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01\attempt_01'
$Receipt = Join-Path $Attempt 'import_receipt.json'
$Terminal = Join-Path $Root 'Saved\Reports\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01_TERMINAL_SUPERVISOR.json'
$Emergency = Join-Path $Root 'Saved\Reports\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 2700

$Expected = [ordered]@{
    $Author = '2dcf14f805c8877ff8369414aacb0a24339169a63b3cff58759e54dc1a773611'
    $Verifier = '3fa34b12626ea67748a244555593e0982de52cc7f8548b394bbfe8e157f5b0b6'
    $Contract = 'b61bf3b2ac3649231a90f09872e531991b9b568723c6974f4278e176525b4d5a'
...
$State = [ordered]@{
    schema = 'skyguard.m01-window-recovery06-unrealready01.reversible-unreal-import01.supervisor.v1'
    ...
    process_samples = @()
    receipt = $null
    imported_asset_inventory = @()
    source_unchanged = $false
    acceptance_freeze_unchanged = $false
    project_descriptor_unchanged = $false
    map_mutations = 0
...
try {
    ...
    if (Test-Path -LiteralPath $Destination) { throw "Fresh import namespace exists: $Destination" }

    $verifyOutput = & python $Verifier 2>&1
    if ($LASTEXITCODE -ne 0 -or ($verifyOutput -join "`n") -notmatch 'PASS_M01_WINDOW_RECOVERY06') {
        throw "Offline verifier failed: $($verifyOutput -join ' ')"
    }
    $authorTest = & python $Author --offline-contract-test 2>&1
    if ($LASTEXITCODE -ne 0 -or ($authorTest -join "`n") -notmatch 'PASS_M01_WINDOW_RECOVERY06') {
        throw "Author offline contract test failed: $($authorTest -join ' ')"
    }
    ...
    while (-not $process.HasExited) {
        ...
        if ([DateTime]::UtcNow -ge $deadline) {
            ...
            throw "Unreal reversible import exceeded $TimeoutSeconds seconds."
        }
    ...
    if (-not (Test-Path -LiteralPath $Receipt -PathType Leaf)) { throw 'Import receipt missing.' }
    $payload = Get-Content -LiteralPath $Receipt -Raw | ConvertFrom-Json
    $State.receipt = Get-Record $Receipt
    if ($payload.classification -ne 'PASSED_M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_IMPORT_READY_FOR_MAPPED_PREVIEW') {
        throw "Unexpected import receipt classification: $($payload.classification)"
    }
    if (@($payload.production_static_meshes.PSObject.Properties).Count -ne 3) { throw 'Production StaticMesh count changed.' }
    if ([int]$payload.map_mutations -ne 0 -or [bool]$payload.runtime_promotion_performed) {
        throw 'Reversible import exceeded its no-map/no-promotion boundary.'
    }
    if (-not (Test-Path -LiteralPath $Destination -PathType Container)) { throw 'Imported asset namespace missing.' }
    $State.imported_asset_inventory = @(
        Get-ChildItem -LiteralPath $Destination -Recurse -File |
            Sort-Object FullName |
            ForEach-Object { Get-Record $_.FullName }
    )
    if ($State.imported_asset_inventory.Count -lt 3) { throw 'Imported asset inventory is unexpectedly small.' }
    $State.source_unchanged = ((Get-Sha256 $Source) -eq $Expected[$Source])
    $State.acceptance_freeze_unchanged = ((Get-Sha256 $AcceptanceFreeze) -eq $Expected[$AcceptanceFreeze])
    $State.project_descriptor_unchanged = ((Get-Sha256 $Project) -eq $Expected[$Project])
    if (-not $State.source_unchanged -or -not $State.acceptance_freeze_unchanged -or -not $State.project_descriptor_unchanged) {
        throw 'An immutable authority changed during import.'
    }
    $State.classification = 'PASSED_M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_IMPORT_READY_FOR_MAPPED_PREVIEW'
    $State.failure_stage = $null
    $Exit = 0
}
catch {
