...
$Executor=Join-Path $Root 'Scripts\GrokProduction\author_and_capture_m01_window_recovery06_unrealready01_mapped_preview01_recovery01.py'
$BaseExecutor=Join-Path $Root 'Scripts\GrokProduction\author_and_capture_m01_window_recovery06_unrealready01_mapped_preview01.py'
...
$Attempt=Join-Path $Root 'Saved\BuildAttempts\M01_WINDOW_RECOVERY06_UNREALREADY01_MAPPED_PREVIEW01_RECOVERY01\attempt_01'
$Receipt=Join-Path $Attempt 'mapped_preview_receipt.json'
$MapFile='D:\SG52T08_ENV01\Content\T08\GW02PreviewR01\Lvl_GW02_WindowPreview01_Recovery01.umap'
$Terminal=Join-Path $Root 'Saved\Reports\M01_WINDOW_RECOVERY06_UNREALREADY01_MAPPED_PREVIEW01_RECOVERY01_TERMINAL_SUPERVISOR.json'
$Emergency=Join-Path $Root 'Saved\Reports\M01_WINDOW_RECOVERY06_UNREALREADY01_MAPPED_PREVIEW01_RECOVERY01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds=600
$Expected=[ordered]@{
    $Executor='2b4d8c5ecdfd017cea50edff4c7e4bf44e5f33a5e95a6378890103b5a17c4f29'
    $BaseExecutor='bf114b348475ff29bee80c7ec7c15e1c0d73a567422a38690639cb9df25ea893'
...
try{
    ...
    foreach($future in @($Attempt,$Terminal,$MapFile)){if(Test-Path -LiteralPath $future){throw "Fresh Recovery01 namespace exists: $future"}}
    $syntax=& python -c "from pathlib import Path;p=Path(r'$Executor');compile(p.read_text(encoding='utf-8'),str(p),'exec');print('PASS')" 2>&1
    if($LASTEXITCODE-ne0-or($syntax-join"`n")-notmatch'PASS'){throw "Executor syntax failed: $($syntax-join' ')"}
    ...
    if(-not(Test-Path -LiteralPath $Receipt -PathType Leaf)){throw 'Recovery01 preview receipt missing'}
    $payload=Get-Content -LiteralPath $Receipt -Raw|ConvertFrom-Json;$State.receipt=Get-Record $Receipt
    if($payload.classification-ne'PASSED_RECOVERY01_AUTOMATIC_AWAITING_DIRECT_VISUAL_REVIEW'){throw "Unexpected receipt classification: $($payload.classification)"}
    if([int]$payload.capture_count-ne6){throw 'Capture count changed'}
    if(-not(Test-Path -LiteralPath $MapFile -PathType Leaf)){throw 'Review map missing'}
    $State.map=Get-Record $MapFile;$State.capture_inventory=@($payload.captures|ForEach-Object{Get-Record $_.path})
    if($State.capture_inventory.Count-ne6){throw 'PNG inventory changed'}
    if(-not[bool]$payload.accepted_source_tree_unchanged-or[bool]$payload.runtime_promotion_performed){throw 'Recovery01 preview exceeded its boundary'}
    $State.classification='PASSED_RECOVERY01_AUTOMATIC_AWAITING_DIRECT_VISUAL_REVIEW';$State.failure_stage=$null;$Exit=0
}catch{$State.classification='FAILED_WITH_EVIDENCE';$State.failure_message=$_.Exception.Message;$Exit=1}finally{$State.ended_utc=[DateTime]::UtcNow.ToString('o');if(-not$OfflineContractTest){try{Write-JsonAtomic $Terminal $State}catch{[IO.Directory]::CreateDirectory((Split-Path -Parent $Emergency))|Out-Null;[IO.File]::AppendAllText($Emergency,(([ordered]@{utc=[DateTime]::UtcNow.ToString('o');classification=$State.classification;message=$_.Exception.Message}|ConvertTo-Json -Compress)+[Environment]::NewLine),[Text.UTF8Encoding]::new($false));$Exit=1}}}
