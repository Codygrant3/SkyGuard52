...
$promptPath = 'D:\Skyguard52\Production\Prompts\M01_LIGHTHOUSE_GROK_MCP_PRODUCTION_ATTEMPT01.md'
$attemptRoot = 'D:\Skyguard52\Production\Attempts\m01-lighthouse-grok-mcp\attempt_20260811T034000000000Z'
...
$expectedBlend = Join-Path $outputRoot 'M01_Lighthouse_GrokMCP_Production_A.blend'
$expectedGlb = Join-Path $outputRoot 'exports\M01_Lighthouse_GrokMCP_Production_A.glb'
...
try {
    ...
    foreach ($requiredPath in @($promptPath, $blenderPath, $nodePath, $codexPath, $routerSecretPath, $mcpClient, $shutdownScript)) {
    ...
    $passed = (
        ...
        $sourceHashAfter -eq $sourceBlendHash -and
        ((Get-Content -LiteralPath $grokFinalPath -Raw) -match 'PASSED_AWAITING_DIRECT_VISUAL_REVIEW')
    )
    if ($passed) {
    ...
    if (Test-Path -LiteralPath $attemptRoot) {
        ...
        $terminal = [ordered]@{
            schema = 'skyguard.m01-lighthouse.grok-mcp.production-attempt01.terminal.v1'
