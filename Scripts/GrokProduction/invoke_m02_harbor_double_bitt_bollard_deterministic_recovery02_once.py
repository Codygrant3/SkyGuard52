...
ATTEMPT = PROJECT / r"Production\Attempts\m02-harbor-double-bitt-bollard-deterministic-recovery02\attempt_20260811T1310000000000Z"
...
CREATE_SRC_SHA = "d037c9f76549bb1194892d94ecc36a72af73f130a1afce63d56e779196399c53"
...
def rewrite_paths(text: str) -> str:
    old_root = r"D:\Skyguard52\Production\Attempts\m02-harbor-double-bitt-bollard-grok-mcp-production01"
    new_root = r"D:\Skyguard52\Production\Attempts\m02-harbor-double-bitt-bollard-deterministic-recovery02"
    old_attempt = r"attempt_20260811T1245000000000Z"
    new_attempt = r"attempt_20260811T1310000000000Z"
    rewritten = text.replace(old_root, new_root).replace(old_attempt, new_attempt)
    if old_root in rewritten or old_attempt in rewritten:
        raise RuntimeError("Path rewrite incomplete")
    if new_root not in rewritten and "OUTPUT" in rewritten:
        # Finalizer uses single absolute path; creation script uses concatenated Path.
        pass
    return rewritten
...
def main() -> int:
    ...
    CREATE_SCRIPT.write_text(rewrite_paths(CREATE_SCRIPT_SRC.read_text(encoding="utf-8")), encoding="utf-8")
    FINALIZER.write_text(rewrite_paths(FINALIZER_SRC.read_text(encoding="utf-8")), encoding="utf-8")
    ...
    terminal = {
        "schema": "skyguard.m02-harbor-double-bitt-bollard.deterministic-recovery02.terminal.v1",
