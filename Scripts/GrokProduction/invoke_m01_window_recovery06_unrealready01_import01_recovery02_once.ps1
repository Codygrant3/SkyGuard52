...
$Author = Join-Path $Root 'Scripts\GrokProduction\author_m01_window_recovery06_unrealready01_import01_recovery02.py'
$Verifier = Join-Path $Root 'Scripts\GrokProduction\verify_m01_window_recovery06_unrealready01_import01_recovery02_offline.py'
$Contract = Join-Path $Root 'Docs\GrokProduction\Wave02\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01_RECOVERY02_CONTRACT.json'
$BaseAuthor = Join-Path $Root 'Scripts\GrokProduction\author_m01_window_recovery06_unrealready01_import01_recovery01.py'
...
$PriorFreeze = Join-Path $Root 'Docs\AAA_Review\M01_WINDOW_REVERSIBLE_UNREAL_IMPORT01_RECOVERY01_ATTEMPT01_TERMINAL_FREEZE.json'
$Probe02Freeze = Join-Path $Root 'Docs\AAA_Review\M01_WINDOW_INTERCHANGE_PIPELINE_PROBE02_ATTEMPT01_TERMINAL_FREEZE.json'
$Authorization = Join-Path $Root 'Production\standing_heavy_process_authorization.json'
$Destination = 'D:\SG52T08_ENV01\Content\T08\GW02'
$Attempt = Join-Path $Root 'Saved\BuildAttempts\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01_RECOVERY02\attempt_01'
$Receipt = Join-Path $Attempt 'import_receipt.json'
$Terminal = Join-Path $Root 'Saved\Reports\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01_RECOVERY02_TERMINAL_SUPERVISOR.json'
$Emergency = Join-Path $Root 'Saved\Reports\M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_UNREAL_IMPORT01_RECOVERY02_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 2700
$Expected = [ordered]@{
    $Author = 'fc08c6a6d7aa14bc8e5aa8d6309603fa4d9e57d56e8eddc7dbe38a4dd9e6f356'
    $Verifier = 'd47ccbf65ed22b5053d5bfe3d9014f0b1af2c695d766466adab3be434a54171a'
    $Contract = '7fababfccd368a4e49d47b71dd7ae6ac1b1dad47784f7cbc19cc6371712d160c'
    $BaseAuthor = '1bbc40d6c0063a60e26be1921698ec43aa18d522a520fb6e5e75532d602e6d79'
    ...
    $PriorFreeze = 'e4c8baf00d3d580482c44b45104e9f1a68b95ddb0a665b65555ec7fd999be02b'
    $Probe02Freeze = '7a413ba78c6e3a37922e3f0c4ff6f84599312dda5214bfc000008cbba4b55d36'
...
try{
    ...
    if(Test-Path -LiteralPath $Attempt){throw "Fresh Recovery02 attempt exists: $Attempt"}
    if(Test-Path -LiteralPath $Terminal){throw "Fresh Recovery02 terminal exists: $Terminal"}
    if(Test-Path -LiteralPath $Destination){throw "Fresh Recovery02 destination exists: $Destination"}
    $verifyOutput=& python $Verifier 2>&1
    if($LASTEXITCODE-ne0-or($verifyOutput-join"`n")-notmatch'PASS_M01_WINDOW_REVERSIBLE_UNREAL_IMPORT01_RECOVERY02_OFFLINE'){throw "Recovery02 offline verifier failed: $($verifyOutput-join' ')"}
    $authorOutput=& python $Author --offline-contract-test 2>&1
    if($LASTEXITCODE-ne0-or($authorOutput-join"`n")-notmatch'PASS_M01_WINDOW_REVERSIBLE_UNREAL_IMPORT01_RECOVERY02_CONTRACT'){throw "Recovery02 author contract failed: $($authorOutput-join' ')"}
    ...
    if(-not(Test-Path -LiteralPath $Receipt -PathType Leaf)){throw 'Recovery02 import receipt missing.'}
    $payload=Get-Content -LiteralPath $Receipt -Raw|ConvertFrom-Json;$State.receipt=Get-Record $Receipt
    if($payload.classification-ne'PASSED_M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_IMPORT_RECOVERY02_READY_FOR_MAPPED_PREVIEW'){throw "Unexpected receipt classification: $($payload.classification)"}
    ...
    if([int]$payload.map_mutations-ne0-or[bool]$payload.runtime_promotion_performed){throw 'Recovery02 exceeded its reversible-import boundary.'}
    if(-not(Test-Path -LiteralPath $Destination -PathType Container)){throw 'Recovery02 destination missing.'}
    ...
    if($State.imported_asset_inventory.Count-lt3){throw 'Recovery02 imported inventory is unexpectedly small.'}
    ...
    $State.classification='PASSED_M01_WINDOW_RECOVERY06_UNREALREADY01_REVERSIBLE_IMPORT_RECOVERY02_READY_FOR_MAPPED_PREVIEW';$State.failure_stage=$null;$Exit=0
