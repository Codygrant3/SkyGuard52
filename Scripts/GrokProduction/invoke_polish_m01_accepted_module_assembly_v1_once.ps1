...
$Author = Join-Path $Root 'Scripts\GrokProduction\polish_m01_accepted_module_assembly_v1.py'
...
$TargetMap = Join-Path $Root 'Content\Skyguard\Maps\Assembly\Lvl_M01_AcceptedModuleAssembly_v1.umap'
$Attempt = Join-Path $Root 'Saved\BuildAttempts\M01_ACCEPTED_MODULE_ASSEMBLY_v1_POLISH01_RECOVERY01\attempt_01'
$Receipt = Join-Path $Attempt 'polish_receipt.json'
$Terminal = Join-Path $Root 'Saved\Reports\M01_ACCEPTED_MODULE_ASSEMBLY_V1_POLISH01_RECOVERY01_TERMINAL_SUPERVISOR.json'
$Emergency = Join-Path $Root 'Saved\Reports\M01_ACCEPTED_MODULE_ASSEMBLY_V1_POLISH01_RECOVERY01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1800
...
$State = [ordered]@{
    schema = 'skyguard.m01-accepted-module-assembly.polish01.supervisor.v1'
...
try {
    ...
    foreach ($path in @($Project, $Editor, $Author, $Authorization, $Playable, $TargetMap)) {
    ...
    if ($State.playable_before.bytes -ne $ExpectedPlayableBytes -or $State.playable_before.sha256 -ne $ExpectedPlayableSha256) {
        throw 'Playable M01 authority does not match the frozen author-script constants.'
    }
    ...
    if ($LASTEXITCODE -ne 0 -or ($offlineOutput -join "`n") -notmatch 'PASS_M01_ACCEPTED_MODULE_ASSEMBLY_V1_POLISH01_OFFLINE_CONTRACT') {
    ...
    if ($OfflineContractTest) {
        ...
        $Exit = 0
        $State | ConvertTo-Json -Depth 40 | Write-Output
    ...
    while (-not $process.HasExited) {
        ...
        if ([DateTime]::UtcNow -ge $deadline) {
            ...
            throw "Unreal assembly polish exceeded $TimeoutSeconds seconds."
        }
    ...
    if (-not (Test-Path -LiteralPath $Receipt -PathType Leaf)) { throw "Polish receipt missing: $Receipt" }
    ...
    $State.target_map = Get-Record $TargetMap
    if ($State.playable_after.sha256 -ne $ExpectedPlayableSha256 -or $State.playable_after.bytes -ne $ExpectedPlayableBytes) {
        throw 'Playable M01 map changed during assembly polish.'
    }
    if ($payload.classification -ne 'PASSED_M01_ACCEPTED_MODULE_ASSEMBLY_V1_POLISH01_AWAITING_REVIEW') {
        throw "Unexpected polish classification: $($payload.classification) error=$($payload.error)"
    }
