...
OUTPUT = ROOT / r"Production\Attempts\m02-harbor-double-bitt-bollard-deterministic-recovery06\attempt_20260811T1325000000000Z\output"
CHECKPOINT = OUTPUT / "checkpoint"
RENDERS = OUTPUT / "renders"
EXPORTS = OUTPUT / "exports"
RECEIPTS = OUTPUT / "receipts"
FINAL_BLEND = OUTPUT / "M02_Harbor_DoubleBittBollard_Production01.blend"
FINAL_GLB = EXPORTS / "M02_Harbor_DoubleBittBollard_Production01.glb"

MESH_NAME = "SM_M02_Harbor_DoubleBittBollard_A"
SOCKET_NAME = "SOCKET_MooringBollard_Origin"
COLLISION_NAME = "UCX_SM_M02_Harbor_DoubleBittBollard_A_00"
SOURCE_NAMES = {
    "SRC_MooringBollard_Base",
    "SRC_MooringBollard_Posts",
    "SRC_MooringBollard_Heads",
    "SRC_MooringBollard_Gussets",
    "SRC_MooringBollard_Fasteners",
}
MATERIAL_NAMES = {
    "M_M02_MooringBollard_PaintedCastSteel",
    "M_M02_MooringBollard_ExposedMetal",
}
CHECK_KEYS = {
    "complete_double_bitt_visible",
    "heads_flared_and_connected",
    "posts_connected_to_base",
    "gussets_integrated",
    "fasteners_grounded",
...
base = bpy.data.objects["SRC_MooringBollard_Base"]
posts = bpy.data.objects["SRC_MooringBollard_Posts"]
heads = bpy.data.objects["SRC_MooringBollard_Heads"]
gussets = bpy.data.objects["SRC_MooringBollard_Gussets"]
fasteners = bpy.data.objects["SRC_MooringBollard_Fasteners"]
...
require(vertices >= 1800, f"Vertex count too low: {vertices}")
require(polygons >= 1400, f"Polygon count too low: {polygons}")

minimum, maximum = object_bounds(mesh)
dimensions = maximum - minimum
require(1.15 <= dimensions.x <= 1.35, f"Length outside contract: {dimensions.x}")
require(0.62 <= dimensions.y <= 0.82, f"Width outside contract: {dimensions.y}")
require(0.78 <= dimensions.z <= 0.98, f"Height outside contract: {dimensions.z}")
...
require(abs((minimum.y + maximum.y) * 0.5) <= 0.040, "Asset is not centered on Y")

base_min, base_max = object_bounds(base)
posts_min, posts_max = object_bounds(posts)
heads_min, heads_max = object_bounds(heads)
gusset_min, gusset_max = object_bounds(gussets)
fast_min, fast_max = object_bounds(fasteners)
base_thickness = base_max.z - base_min.z
require(0.08 <= base_thickness <= 0.16, f"Base plate thickness outside contract: {base_thickness}")
require(base_min.z <= 0.03, "Base plate does not ground")
require(posts_min.z <= base_max.z + 0.03, "Posts do not connect into the base casting")
require(posts_max.z >= 0.55, "Posts are too short")
require(heads_max.z >= posts_max.z - 0.03, "Heads are not seated on the posts")
require(heads_max.x - heads_min.x >= 0.48, "Paired heads do not span the governed post spacing")
require(heads_max.x - heads_min.x <= 1.25, "Head span exceeds the governed bollard envelope")
require(gusset_min.z <= base_max.z + 0.08, "Gussets are not integrated into the base")
require(gusset_max.z >= posts_min.z + 0.10, "Gussets do not rise into the posts")
require(fast_min.z <= base_max.z + 0.04, "Fasteners are not grounded on the base")
require(fast_max.z >= base_min.z, "Fasteners float below the base")
...
normalize("M_M02_MooringBollard_PaintedCastSteel", (0.045, 0.060, 0.085), 0.78, 0.48)
normalize("M_M02_MooringBollard_ExposedMetal", (0.20, 0.22, 0.24), 0.92, 0.28)
...
views = [
    ("01_daylight_front_three_quarter.png", (2.20, -2.55, 1.55), (0.0, 0.0, 0.45), 58.0, "daylight"),
    ("02_daylight_opposite_three_quarter.png", (-2.20, 2.55, 1.52), (0.0, 0.0, 0.45), 58.0, "daylight"),
    ("03_low_post_base_gusset_detail.png", (1.55, -1.70, 0.42), (0.0, 0.0, 0.28), 64.0, "overcast"),
    ("04_elevated_heads_and_base.png", (0.0, -2.70, 2.35), (0.0, 0.0, 0.55), 56.0, "overcast"),
    ("05_grounded_fastener_detail.png", (1.70, -1.40, 0.35), (0.45, 0.0, 0.10), 68.0, "wet"),
    ("06_overcast_whole_asset.png", (-2.30, -2.70, 1.55), (0.0, 0.0, 0.45), 56.0, "overcast"),
    ("07_night_whole_asset.png", (2.30, -2.70, 1.55), (0.0, 0.0, 0.45), 56.0, "night"),
    ("08_cockpit_worklight_whole_asset.png", (-2.15, -2.55, 1.48), (0.0, 0.0, 0.44), 56.0, "cockpit"),
]
...
report = {
    "schema": "skyguard.m02-harbor-double-bitt-bollard.grok-mcp.production01.implementation.v1",
    "classification": "PASSED_AWAITING_DIRECT_VISUAL_REVIEW",
    "asset": "M02 generic non-branded cast-steel double-bitt mooring bollard",
    "identity_boundary": "Project-provisional harbor fitting; no manufacturer, load rating, port authority, country, or certification claimed",
    "mesh": MESH_NAME,
    "socket": SOCKET_NAME,
