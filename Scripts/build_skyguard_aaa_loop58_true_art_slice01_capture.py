...
PREFIX = "AAA_L58_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L58"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L58"
...
def densify():
    ...
    log("loop58 mat palette size=%d" % len(mats))
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # ---- TRUE ART SLICE01 (L58 / Terra) ----
        # Capture-safe: L52 HF densify remains in FOV; opaque authored content x>=bx+3
        ...
        # MAT-01 aircraft metal anchor (behind wall) + MAT-04 exhaust pin
        if name in ("YakBeauty", "Prop", "PropNose", "Wide"):
            if sm_yak:
                spawn_sm(sm_yak, (bx + 3.55, cy - 0.2, cz - 0.4), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA_MAT01_YakAnchor", m_air or m_panel or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 0.2, cz + 0.1), (0.12, 1.8, 0.7), None, PREFIX + "TA_MAT01_AirframePanel", m_air or m_panel or mats[0])
            # tiny exhaust emissive pin (additive scale, still slightly behind wall edge)
            spawn_sm(sphere, (bx + 3.25, cy - 0.9, cz + 0.15), (0.08, 0.08, 0.08), None, PREFIX + "TA_MAT04_ExhaustPin", m_glow or unlit_y or mats[0])

        # MAT-02 concrete + MAT-03 brick breakup (behind wall city anchors)
        if name in ("City", "Wide", "Harbor"):
            if sm_tower:
                spawn_sm(sm_tower, (bx + 3.6, cy + 1.6, cz - 1.0), (0.45, 0.45, 0.55), unreal.Rotator(0, -20, 0), PREFIX + "TA_MAT02_ConcreteFacade", m_conc or m_panel or mats[1])
            else:
                spawn_sm(cube, (bx + 3.35, cy + 1.6, cz + 0.2), (0.1, 1.4, 2.2), None, PREFIX + "TA_MAT02_ConcreteSlab", m_conc or m_panel or mats[1])
            if sm_apt:
                spawn_sm(sm_apt, (bx + 3.7, cy - 1.8, cz - 0.8), (0.4, 0.4, 0.5), unreal.Rotator(0, 15, 0), PREFIX + "TA_MAT03_BrickFacade", m_brick or m_bright_brick or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 1.8, cz + 0.1), (0.1, 1.1, 1.8), None, PREFIX + "TA_MAT03_BrickSlab", m_brick or m_bright_brick or mats[0])

        # VFX-01 muzzle readability (tiny, combat/ADS/cockpit)
        if name in ("Combat", "ADS", "Cockpit"):
            spawn_niagara(PREFIX + "TA_VFX01_Muzzle", (bx + 2.45, cy + 0.35, cz + 0.2), "NS_MuzzleFlash", (0.35, 0.35, 0.35))
            # keep tiny additive proxy as fallback readability if NS is empty/stub
            spawn_sm(sphere, (bx + 2.42, cy + 0.35, cz + 0.2), (0.12, 0.12, 0.12), None, PREFIX + "TA_VFX01_MuzzleCore", m_muzzle or unlit_y or mats[0])

        # VFX-02 ocean contact accent
        if name in ("Ocean", "Harbor"):
            spawn_niagara(PREFIX + "TA_VFX02_OceanSpray", (bx + 3.3, cy - 0.5, cz - 0.7), "NS_OceanSpray", (0.5, 0.5, 0.5))
            spawn_sm(sphere, (bx + 3.25, cy - 0.5, cz - 0.65), (0.1, 0.1, 0.1), None, PREFIX + "TA_VFX02_FoamPin", m_bright_white or unlit_w or mats[1])

        # VFX-03 impact event on rear concrete
        if name in ("City", "Combat", "Harbor"):
            spawn_niagara(PREFIX + "TA_VFX03_HitSparks", (bx + 3.35, cy + 1.2, cz + 0.4), "NS_HitSparks", (0.4, 0.4, 0.4))
            spawn_sm(sphere, (bx + 3.3, cy + 1.2, cz + 0.4), (0.09, 0.09, 0.09), None, PREFIX + "TA_VFX03_SparkCore", m_muzzle or unlit_y or mats[0])
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop58 true-art slice01 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L58", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L58_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop58 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l57_freeze_true_art_slice01_materials_niagara\n")
...
def main():
    log("loop58 L57 freeze + true-art slice01 materials+niagara start")
    ...
    log("Loop58 complete stills=%d" % (len(saved) if saved else 0))
