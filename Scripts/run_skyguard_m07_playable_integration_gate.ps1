...
$builder = Join-Path $ProjectRoot "Scripts\build_skyguard_m07_playable_integration.py"
$verifier = Join-Path $ProjectRoot "Scripts\verify_skyguard_m07_playable_integration.py"
$stamp = [DateTime]::UtcNow.ToString("yyyyMMddTHHmmssfffZ")
$attempt = Join-Path $ProjectRoot "Saved\Reports\M07_Playable\attempt_$stamp"
...
function Invoke-Stage(
    ...
    if (-not $process.WaitForExit($StageTimeoutSeconds * 1000)) {
        throw (
            "$Name timed out with authoritative PID " +
            "$($process.Id); wait, never duplicate"
        )
    }
    $process.WaitForExit()
    $process.Refresh()
    ...
    if ($null -eq $exitCode -and
        $SuccessPattern -and
        $text -match $SuccessPattern) {
        $exitCode = 0
    }
    if ($null -eq $exitCode) {
        throw (
            "$Name completed without a readable exit code " +
            "or accepted success marker"
        )
    }
...
Invoke-Stage "automation" $cmd @(
    $project,
    "-ExecCmds=Automation RunTests Skyguard52.Mission07",
...
$log = Get-Content (
    Join-Path $attempt "automation.stdout.log"
) -Raw
$required = @(
...
$audit = Get-Content (
    Join-Path $ProjectRoot `
        "Saved\Reports\M07_PLAYABLE_INTEGRATION_AUDIT.json"
...
$receipt = [ordered]@{
    schema = "skyguard.m07-playable-supervisor.v1"
    gate = $gate
    attempt = $attempt
    automation = [ordered]@{
        success = $success
...
$receipt | ConvertTo-Json -Depth 12 |
    Set-Content (
        Join-Path $ProjectRoot `
            "Saved\Reports\M07_PLAYABLE_INTEGRATION_GATE_LATEST.json"
    ) -Encoding utf8
if ($gate -ne "PASS") {
    throw "M07 playable gate failed"
}
Write-Output "M07_PLAYABLE_INTEGRATION_GATE=PASS"
