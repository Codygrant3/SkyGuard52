...
PREFIX = "AAA_L82_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L82"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L82"
...
def densify():
    ...
    log("loop82 mat palette size=%d" % len(mats))
    ...
    log("loop82 true-art slice25 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L82", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L82_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop82 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l81_freeze_true_art_slice25_niagara_material_push")
...
def main():
    log("loop82 L81 freeze + true-art slice25 niagara + material push start")
    ...
    log("Loop82 complete stills=%d" % (len(saved) if saved else 0))
