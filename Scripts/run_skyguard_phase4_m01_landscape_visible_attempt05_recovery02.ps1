...
$ContractPath = Join-Path $ProjectRoot "Docs\AAA_Review\PHASE4_M01_LANDSCAPE_VISIBLE_ATTEMPT05_RECOVERY02_CONTRACT.json"
...
$LatestGate = Join-Path $ProjectRoot "Saved\Reports\PHASE4_M01_LANDSCAPE_VISIBLE_GPU_REVIEW_ATTEMPT05_RECOVERY02_LATEST.json"
...
function Invoke-ProfileStage {
    ...
    $arguments = @(
        ...
        "-SkyguardP45ProfileRunId=recovery02_$Mode",
...
foreach ($required in @(
    ...
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required recovery02 file missing: $required"
    }
}
if ($Contract.status -ne "OFFLINE_RECOVERY02_DESIGN_READY_PENDING_CPP_RECOMPILE_AND_AUTHORIZATION") {
    throw "Recovery02 contract is not in the reviewed offline-ready state"
}
if (Test-Path -LiteralPath $RecoveryRoot) {
    throw "Recovery02 root already exists; refuse duplicate or overwrite"
}
...
try {
    foreach ($profile in @(
        ...
        if (
            ...
            throw "$($profile.mode) recovery02 profile receipt failed"
        }
    ...
    $stdout = Join-Path $LogsRoot "verify_recovery02_visible_gpu_gate.stdout.log"
    $stderr = Join-Path $LogsRoot "verify_recovery02_visible_gpu_gate.stderr.log"
    $gateArgs = @(
        $GateVerifier,
        "--manifest",
        $ManifestPath,
    ...
    $gateStage = [ordered]@{
        name = "verify_recovery02_visible_gpu_gate"
    ...
    if ($gateProcess.ExitCode -notin @(0, 2)) {
        throw "Recovery02 visible/GPU verifier process failed"
    }
    if ($null -ne $gateLock.error) {
        throw "Recovery02 final package lock failed"
    }
    $gateReport = Get-Content -LiteralPath $GatePath -Raw | ConvertFrom-Json
    if ($gateReport.technical_gate -ne "PASS") {
        throw "Recovery02 technical visible/GPU gate failed"
    }
    ...
    elseif ($gateReport.gate -eq "INCOMPLETE_HUMAN_REVIEW") {
        ...
        throw "Unexpected recovery02 gate state: $($gateReport.gate)"
    }
}
catch {
    ...
    if (@(Wait-ForZeroHeavyProcesses -TimeoutSeconds 30).Count -ne 0) {
        throw "Heavy processes remain after recovery02 supervisor"
    }
}
