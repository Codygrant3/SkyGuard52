"""Fresh-process persistence audit for M08 playable integration."""
...
REPORT = ROOT / "Saved/Reports/M08_PLAYABLE_INTEGRATION_AUDIT.json"
...
def main():
    if not unreal.EditorLevelLibrary.load_level(TARGET_MAP):
        raise RuntimeError("M08 playable map failed fresh load")
    ...
    bosses = [
        actor
        for actor in actors
        if actor.get_class().get_name() == "SkyguardLifelineHunterBoss"
    ]
    governed = [
        actor
        for actor in actors
    ...
    checks = {
        ...
        "assembly_identity": mission_id == "M08_RescueCover",
        "assembly_valid": valid,
        "distinct_rescue_orbit": len(route_points) >= 4
        and route_x_span >= 60000.0
        and route_y_span >= 15000.0,
        "rescue_landmarks": all(
        ...
        "single_lifeline_hunter": len(bosses) == 1,
    ...
    report = {
        "schema": "skyguard.m08-playable-integration-audit.v1",
        ...
        "rescue_component_names": component_names,
        "weakpoints": weakpoints,
        "limitations": [
            "Persistence is not PIE, rendering, input, performance, or packaging proof.",
            "Rescue actors, maritime elements and boss remain proxy art.",
        ],
    }
    REPORT.parent.mkdir(parents=True, exist_ok=True)
    REPORT.write_text(json.dumps(report, indent=2), encoding="utf-8")
    unreal.log("[SkyguardM08PlayableAudit] " + json.dumps(report))
    if report["gate"] != "PASS":
        raise RuntimeError("M08 playable persistence audit failed")
