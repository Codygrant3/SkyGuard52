...
PREFIX = "AAA_L80_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L80"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L80"
...
def densify():
    ...
    log("loop80 mat palette size=%d" % len(mats))
    ...
    log("loop80 true-art slice23 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L80", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L80_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop80 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l79_freeze_true_art_slice23_niagara_material_push")
...
def main():
    log("loop80 L79 freeze + true-art slice23 niagara + material push start")
    ...
    log("Loop80 complete stills=%d" % (len(saved) if saved else 0))
