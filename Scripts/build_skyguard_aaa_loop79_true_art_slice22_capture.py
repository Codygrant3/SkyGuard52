...
PREFIX = "AAA_L79_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L79"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L79"
...
def densify():
    ...
    log("loop79 mat palette size=%d" % len(mats))
    ...
    log("loop79 true-art slice22 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L79", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L79_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop79 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l78_freeze_true_art_slice22_niagara_material_push")
...
def main():
    log("loop79 L78 freeze + true-art slice22 niagara + material push start")
    ...
    log("Loop79 complete stills=%d" % (len(saved) if saved else 0))
