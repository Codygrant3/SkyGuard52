"""Source-only repair generator for BLD-M01-YAK-UPLIFT-003-R2.

R2 preserves the 240-entry runtime ledger while treating the eight
...
This module writes only the isolated Uplift_003_R2 namespace and refuses to
overwrite any prior R1/R2 evidence.
"""
...
BLEND_PATH = OUTPUT_DIR / "BLD_M01_YAK_UPLIFT_003_R2_MASTER.blend"
GLB_PATH = OUTPUT_DIR / "bld_m01_yak_uplift_003_r2.glb"
MANIFEST_PATH = ROOT / "Saved" / "Reports" / "BLD_M01_YAK_UPLIFT_003_R2_MANIFEST.json"
COMPARISON_DIR = ROOT / "Saved" / "Screenshots" / "BLD_M01_YAK_UPLIFT_003_R2"
DONOR_COLLECTION_NAME = "BLD_M01_YAK_UPLIFT_003_R2_DONORS"
GOVERNANCE_COLLECTION_NAME = "BLD_M01_YAK_UPLIFT_003_R2_GOVERNANCE"
...
def resolve_ledger(overlay: dict, base: dict, runtime: dict) -> list[dict]:
    ...
    if len(entries) != overlay["governed_component_count"] or len(set(names)) != len(
        ...
        raise RuntimeError("R2 runtime component inventory is not exactly 240 unique names")
    exceptions = overlay["classification_overrides"]
    if set(exceptions) - set(names):
        raise RuntimeError("R2 source-absent exception references unknown runtime component")
    if set(exceptions.values()) != {"source_absent_hold"}:
        raise RuntimeError("R2 exceptions must only use source_absent_hold")
    resolved = []
    for entry in entries:
        name = entry["name"]
        bundle = entry["bundle"]
        if bundle not in base["bundle_defaults"]:
            raise RuntimeError(f"R2 unknown source bundle: {bundle}")
        ...
        if classification not in overlay["allowed_classifications"]:
            raise RuntimeError(f"R2 forbidden classification: {classification}")
    ...
    if counts["source_absent_hold"] != overlay["source_absent_hold_count"]:
        raise RuntimeError("R2 source_absent_hold accounting is not exactly eight")
    if len(resolved) - counts["source_absent_hold"] != overlay["exact_object_requirement_count"]:
        raise RuntimeError("R2 exact-object accounting is not exactly 232")
    return resolved
...
def load_and_verify_governance() -> tuple[dict, dict, dict, dict, list[dict]]:
    ...
    if contract["build_id"] != BUILD_ID or overlay["build_id"] != BUILD_ID:
        raise RuntimeError("R2 governance build id mismatch")
    if contract["status"] != "source_only_not_run":
        raise RuntimeError("R2 source contract must remain source_only_not_run")
    for key, record in contract["immutable_sources"].items():
        verify_record(ROOT / record["path"], record, key)
    if audit["reconciliation"]["accounting_equation"] != (
        "232 exact required objects + 8 source_absent_hold exceptions = 240 governed components"
    ):
        raise RuntimeError("R2 source-audit accounting drifted")
    resolved = resolve_ledger(overlay, base, runtime)
    actual_names = parse_glb_mesh_node_names(L88_GLB_PATH)
    normalized = ("\n".join(actual_names) + "\n").encode("utf-8")
    inventory = audit["evidence"]["l88_glb_mesh_inventory"]
    if len(actual_names) != inventory["mesh_node_count"]:
        raise RuntimeError("R2 actual L88 mesh-node count drifted")
    if len(normalized) != inventory["normalized_sorted_names_bytes"]:
        raise RuntimeError("R2 normalized actual L88 source inventory size drifted")
    if hashlib.sha256(normalized).hexdigest() != inventory["normalized_sorted_names_sha256"]:
        raise RuntimeError("R2 normalized actual L88 source inventory hash drifted")
...
def refuse_overwrite() -> None:
    ...
    if existing:
        raise RuntimeError("R2 immutable output already exists: " + ", ".join(existing))
    if COMPARISON_DIR.exists() and any(COMPARISON_DIR.iterdir()):
        raise RuntimeError("R2 comparison directory is not empty")
...
def isolated_copy_and_open(contract: dict) -> None:
    ...
    if sha256_file(BLEND_PATH) != contract["immutable_sources"]["l88_blend"]["sha256"]:
        raise RuntimeError("R2 isolated L88 copy failed hash verification")
    bpy.ops.wm.open_mainfile(filepath=str(BLEND_PATH))
    if sha256_file(L88_BLEND_PATH) != contract["immutable_sources"]["l88_blend"]["sha256"]:
        raise RuntimeError("R2 immutable L88 source changed")
...
def stage_source_inventory_exception_gate(
    ...
    for entry in resolved:
        ...
        if bpy.data.objects.get(governed_name) is not None:
            raise RuntimeError(
                f"R2 source-absent contract changed; create a new version: {governed_name}"
            )
    ...
    if len(records) != 8:
        raise RuntimeError("R2 did not account for exactly eight source-absent entries")
    return records
...
def stage_component_ledger_tags(resolved: list[dict]) -> list[str]:
    ...
    for entry in resolved:
        ...
        if obj is None:
            raise RuntimeError(f"R2 required exact L88 object missing: {entry['name']}")
        ...
        obj["SKG_R2ExactObjectRequirement"] = True
        tagged.append(entry["name"])
    if len(tagged) != 232:
        raise RuntimeError("R2 must tag exactly 232 exact source objects")
    return tagged
...
def stage_selective_002_donors(contract: dict, base_ledger: dict) -> list[str]:
    ...
    rename = {
        "GEO_PROD002_CowlingShell": "GEO_UPLIFT003R2_DONOR_CowlingShell",
        "GEO_PROD002_CowlingFrontRing": "GEO_UPLIFT003R2_DONOR_CowlingFrontRing",
        "GEO_PROD002_CowlingShutters": "GEO_UPLIFT003R2_DONOR_CowlingShutters",
        "GEO_PROD002_CowlingInletCone": "GEO_UPLIFT003R2_DONOR_CowlingInletCone",
        "GEO_PROD002_Spinner": "GEO_UPLIFT003R2_DONOR_Spinner",
        "GEO_PROD002_PropBlade_A": "GEO_UPLIFT003R2_DONOR_PropBlade_A",
        "GEO_PROD002_PropBlade_B": "GEO_UPLIFT003R2_DONOR_PropBlade_B",
    }
    for source_name, target_name in rename.items():
        obj = bpy.data.objects.get(source_name)
        if obj is None or source_name in before:
            raise RuntimeError(f"R2 donor helper did not create {source_name}")
        obj.name = target_name
        r1.link_exclusively(obj, collection)
        tag_donor(obj, source_name)
    for side, suffix in ((-1.0, "L"), (1.0, "R")):
        obj = donor.base.add_cylinder(
            f"GEO_UPLIFT003R2_DONOR_MainWheelWell_{suffix}",
            0.33,
            0.08,
            (0.75, side * 1.05, 0.04),
    ...
    nose = donor.base.add_cylinder(
        "GEO_UPLIFT003R2_DONOR_NoseWheelWell",
        0.23,
        0.07,
        (3.05, 0.0, -0.18),
        (0.0, 1.5707963267948966, 0.0),
    ...
    for name, location, role in (
        (
            "DATUM_UPLIFT003R2_DONOR_CanopyTravel",
            (-1.25, 0.0, 1.33),
            "canopy_travel_only_l88_canopy_preserved",
        ),
        (
            "DATUM_UPLIFT003R2_DONOR_MainGearPivot_L",
            (0.78, -0.92, 0.15),
            "main_gear_pivot_L_only_l88_gear_preserved",
        ),
        (
            "DATUM_UPLIFT003R2_DONOR_MainGearPivot_R",
            (0.78, 0.92, 0.15),
            "main_gear_pivot_R_only_l88_gear_preserved",
        ),
        (
            "DATUM_UPLIFT003R2_DONOR_NoseGearPivot",
            (3.02, 0.0, 0.04),
    ...
    for target_name in base_ledger["donor_replacement_map"]:
        ...
        if target is None:
            raise RuntimeError(f"R2 donor target missing: {target_name}")
    ...
    if missing:
        raise RuntimeError(f"R2 selective donor output missing: {missing}")
...
def configure_render() -> None:
    ...
    light_name = "LIGHT_UPLIFT003R2_Key"
...
def main() -> int:
    ...
    if stages != contract["required_stage_order"]:
        raise RuntimeError("R2 governed stage order drifted")
    write_manifest(
        contract,
        resolved,
        exception_records,
    ...
    print(f"[{BUILD_ID}] provisional R2 comparison candidate emitted")
    return 0
