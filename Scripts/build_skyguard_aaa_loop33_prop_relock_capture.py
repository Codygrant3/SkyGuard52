...
PREFIX = "AAA_L33_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L33"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L33"
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L33", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = []
    for name, cam, dist, ny, nz in stages:
        cams.append(("AAA_Cam_L33_%s" % name, cam, (0.0, 0.0, 0.0)))
    # context cams
    cams.extend([
        ("AAA_Cam_L33_City", (-1000.0, -300.0, 300.0), (-5.0, 50.0, 0.0)),
        ("AAA_Cam_L33_Combat", (700.0, -10.0, 445.0), (-4.0, 185.0, 0.0)),
        ("AAA_Cam_L33_Harbor", (-300.0, -200.0, 160.0), (-4.0, -10.0, 0.0)),
        ("AAA_Cam_L33_Ocean", (700.0, -40.0, 130.0), (-8.0, 30.0, 0.0)),
        ("AAA_Cam_L33_Wide", (200.0, -500.0, 420.0), (-10.0, 140.0, 0.0)),
    ])
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop33 stills\n")
...
def main():
    log("loop33 HF frustum boards + dual/triple capture start")
    ...
    log("Loop33 complete stills=%d" % (len(saved) if saved else 0))
