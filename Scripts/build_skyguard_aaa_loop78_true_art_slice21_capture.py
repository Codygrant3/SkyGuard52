...
PREFIX = "AAA_L78_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L78"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L78"
...
def densify():
    ...
    log("loop78 mat palette size=%d" % len(mats))
    ...
    log("loop78 true-art slice21 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L78", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L78_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop78 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l77_freeze_true_art_slice21_niagara_material_push")
...
def main():
    log("loop78 L77 freeze + true-art slice21 niagara + material push start")
    ...
    log("Loop78 complete stills=%d" % (len(saved) if saved else 0))
