...
# `= 0` + `.f` as Harbor. Harbor is 40 + `.f` / 80 + `.f`
# only. `= 0` is not Harbor.
...
# then this isolated TotalUnderruns field decl.
...
# THIS IS NOT leftover isolated receipt PeakActiveVoices.
# THIS IS NOT leftover isolated receipt SampleCount #1436.
# THIS IS NOT leftover isolated receipt EvidenceSha256 #1437.
# THIS IS NOT leftover isolated receipt BuildSha256 #1435.
...
# SampleCount / PeakActiveVoices / MaximumAudioThreadMs /
# MaximumTruePeakDbTP / bPackagedDevelopmentBuild /
# bAudibleDeviceObserved / bCalibratedMetering /
# bProductionBankReady / bAccepted.
# Identifier bounds: match EXACT `TotalUnderruns` as a
# whole field name after `int32`. Do NOT accept `SampleCount`
# or `PeakActiveVoices`.
...
# FSkyguardAudibleAcceptanceReceipt only, type is int32,
# required initializer `= 0`, UPROPERTY is
...
TARGET = "int32 TotalUnderruns = 0;"
TARGET_WRONG_BARE = "int32 TotalUnderruns;"
TARGET_WRONG_EQ_ONE = "int32 TotalUnderruns = 1;"
TARGET_WRONG_EQ_NONE = "int32 TotalUnderruns = NAME_None;"
TARGET_WRONG_TRUE = "int32 TotalUnderruns = true;"
TARGET_WRONG_FALSE = "int32 TotalUnderruns = false;"
TARGET_WRONG_EQ_FLOAT = "int32 TotalUnderruns = 0.f;"
TARGET_WRONG_FSTRING = "FString TotalUnderruns;"
TARGET_WRONG_FLOAT = "float TotalUnderruns = 0;"
TARGET_WRONG_FNAME = "FName TotalUnderruns;"
TARGET_WRONG_FTEXT = "FText TotalUnderruns;"
TARGET_WRONG_BOOL = "bool TotalUnderruns = false;"
TARGET_WRONG_TARRAY = "TArray<int32> TotalUnderruns;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> TotalUnderruns;"
...
def leftover_sibling_receipt_field_scripts() -> tuple[str, ...]:
    ...
    # MissionDebrief / ObjectiveProgress / RoutePoint / RadioLine /
    # TheaterKit / harness class / bank class / AudioTelemetry /
    # SampleCount #1436 / PeakActiveVoices / EvidenceSha256 #1437 /
    # BuildSha256 #1435 stay locked; this file is TotalUnderruns only.
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 TotalUnderruns = 0;`.
    # Match the EXACT identifier TotalUnderruns as a whole
    # field name after int32. Do NOT accept SampleCount or
    # PeakActiveVoices. Reject dropped/changed initializer,
    # Harbor 40/80, wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"int32\s+TotalUnderruns\s*=\s*0\s*;",
        compact,
    )
    ...
    for assigned in re.findall(
        r"int32\s+TotalUnderruns\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "0":
            return False
    if re.search(
        r"float\s+TotalUnderruns\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+TotalUnderruns\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+TotalUnderruns\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+TotalUnderruns\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+TotalUnderruns\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+TotalUnderruns\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+TotalUnderruns\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+TotalUnderruns\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 TotalUnderruns is missing from "
...
class AudibleAcceptanceReceiptTotalUnderrunsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_total_underruns_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sample_count_does_not_satisfy_total_underruns(self) -> None:
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 TotalUnderruns = 0;", compact_origin)
        self.assertNotIn("TotalUnderruns = 1", compact_origin)
        self.assertNotIn("TotalUnderruns = true", compact_origin)
        self.assertNotIn("FString TotalUnderruns", compact_origin)
        self.assertNotIn("float TotalUnderruns", compact_origin)

    def test_total_underruns_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 TotalUnderruns"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_PEAK_ACTIVE_VOICES} = 0;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tint32 TotalUnderrunss = 0;\n",
            "\tint32 TotalUnderruns = 1;\n",
            "\tbool TotalUnderruns;\n",
            "\tfloat TotalUnderruns = " + forty + ";\n",
            "\tfloat TotalUnderruns = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt TotalUnderruns field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "total_underruns_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_total", THIS_SCRIPT)
