# THIS IS leftover-safe FSkyguardMissionDebrief::Narrative.
# origin/main form: split-line UPROPERTY(VisibleAnywhere,
# BlueprintReadOnly, meta = (MultiLine = "true")) then
# `FText Narrative;` with no Category= and no initializer.
# Origin/main HAS spaces around `=` in the meta
# (`meta = (MultiLine = "true")`). If a clone asserts no
# MultiLine, keep MultiLine. If a clone asserts Category=
# and origin/main has none, drop that assertion. If a clone
# asserts `= 0` / `= false` / an initializer, drop that and
# accept the bare decl.
...
# MissionDisplayName, bNewBestScore, bNewBestMedal,
# bProgressSaved, SaveSlotName, NextMissionId,
# NextMissionDisplayName, NextMissionMap,
...
# meta = (MultiLine = "true"), no Category=. Accept bare
# `FText Narrative;`. Fail-closed if type/decl missing, renamed,
# Category= added, or MultiLine meta is dropped.
...
TARGET = "FText Narrative;"
TARGET_WRONG_ZERO = "FText Narrative = 0;"
TARGET_WRONG_FALSE = "FText Narrative = false;"
TARGET_WRONG_ONE = "FText Narrative = 1;"
TARGET_WRONG_FLOAT = "FText Narrative = 0.f;"
TARGET_WRONG_TRUE = "FText Narrative = true;"
TARGET_WRONG_INIT = "FText Narrative = FText();"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # = false / = 1 / = 0.f / = true / FText() for
    # FSkyguardMissionDebrief::Narrative.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    field = re.search(r"FText\s+Narrative\b", compact)
    if field is None:
        raise AssertionError(
            "UPROPERTY for FText Narrative is missing from "
...
class MissionDebriefNarrativeFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Narrative", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_narrative_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
    ...
    def test_narrative_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText Narrative"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "FText Narrative;")
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('meta = (MultiLine = "true")', UPROPERTY_VISIBLE)
        self.assertNotIn('meta=(MultiLine="true")', UPROPERTY_VISIBLE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief Narrative field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_narrative"
            "_field_decl_contract.py"
        ))
