# THIS IS leftover-safe FSkyguardCampaignMissionSpec Brief.
# origin/main form: plain C++ field `const TCHAR* Brief = TEXT("");`
...
# Fail-closed if the initializer is missing or not TEXT("").
...
# Do NOT parse leftover StormRainBeatKit Title as this
# locked decl or parse window.
...
# Do NOT contract sibling fields MissionId / Title / Success /
...
# invent Harbor 40/80.
# Do NOT claim leftover StormRainBeatKit Title.
# Do NOT claim leftover CampaignMissionSpec MissionId.
# Do NOT claim leftover CampaignMissionSpec Title.
...
# CampaignMissionSpec parse window comes from leftover-safe
# MissionId, BUT this slot locks Brief, not MissionId.
# Do NOT claim leftover MissionId.
# Plain-struct TCHAR* TEXT("") locking comes from leftover
# StormRain Title, BUT the parse window is
# FSkyguardCampaignMissionSpec in SkyguardCampaignRoster.h
# (stop before namespace SkyguardCampaignRoster).
# Do NOT claim leftover StormRain Title.
# UPROPERTY rejection is copied from leftover-safe
# CampaignMissionSpec MissionId. Do not keep MissionId
# as the locked decl.
...
TARGET = 'const TCHAR* Brief = TEXT("");'
TARGET_WRONG_BARE = "const TCHAR* Brief;"
TARGET_WRONG_EQ_NONE = "const TCHAR* Brief = NAME_None;"
TARGET_WRONG_FALSE = "const TCHAR* Brief = false;"
TARGET_WRONG_TRUE = "const TCHAR* Brief = true;"
TARGET_WRONG_ZERO = "const TCHAR* Brief = 0.f;"
TARGET_WRONG_FLOAT = "float Brief;"
TARGET_WRONG_HEALTH = "float Brief = 160.f;"
TARGET_WRONG_BOOL = "bool Brief;"
TARGET_WRONG_INT = "int32 Brief;"
TARGET_WRONG_FNAME = "FName Brief;"
TARGET_WRONG_TEXT = 'const TCHAR* Brief = TEXT("Storm");'
...
LOCKED_DECL = TARGET
LEFTOVER_STORM_TITLE_DECL = TARGET_WRONG_TITLE
...
BRIEF_DECL_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*Brief\s*=\s*TEXT\s*\(\s*\"\"\s*\)\s*;"
)
BRIEF_INIT_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*Brief\s*=\s*([^;]+);"
)
INVENTED_FIELD_MSG = (
    "INDEX_NONE",
    "NAME_None",
...
def brief_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*const\s+TCHAR\s*\*\s*Brief\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `const TCHAR* Brief = TEXT("");`.
    # Do not accept missing TEXT(""), leftover Title, leftover
    # StormRain Title, leftover MissionId, FName Brief,
    # `= NAME_None` / `= false` / `= true` / `= 0.f` /
    # `= 160.f` when origin/main locks TEXT(""). Do not
    # accept sibling Title / Success / Failure / WeatherLabel
    # as this Brief decl. Do not accept UPROPERTY / Category
    # clones from leftover TheaterKit or leftover MissionId.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if BRIEF_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+Brief\b", compact):
        return False
    if re.search(r"FName\s+Brief\b", compact):
        return False
    if re.search(r"bool\s+Brief\b", compact):
        return False
    if re.search(r"int32\s+Brief\b", compact):
        return False
    if re.search(
        r"const\s+TCHAR\s*\*\s*Title\b",
        compact,
    ) and not has_identifier(compact, "Brief"):
        return False
    for match in BRIEF_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != 'TEXT("")':
            return False
    if brief_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*const TCHAR\* Brief\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for const TCHAR* Brief is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*const TCHAR\* Brief\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on const TCHAR* Brief is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "Brief"):
        raise AssertionError(
            "UPROPERTY clone landed on Brief; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "Brief"):
            raise AssertionError(
                f"{token} clone landed on Brief; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CampaignMissionSpecBriefFieldDeclContractTests(unittest.TestCase):
    def test_campaign_mission_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Brief"), section)
    ...
    def test_locked_decl_is_plain_tchar_brief_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, 'const TCHAR* Brief = TEXT("");')
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("const TCHAR* Brief"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn('TEXT("")', LOCKED_DECL)
        self.assertIn("Brief", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_STORM_TITLE_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("Brief", str(raised.exception))
    ...
    def test_does_not_claim_leftover_storm_rain_title(self) -> None:
        self.assertNotEqual(HEADER_PATH, LEFTOVER_STORM_HEADER)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_STORM_KIT)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_STORM_TITLE_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TITLE)
        ...
        self.assertIn(LEFTOVER_STORM_TITLE_DECL, storm_header)
    ...
    def test_does_not_claim_leftover_campaign_mission_id_or_title(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MISSION_ID)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TITLE)
        ...
        self.assertNotIn("mission_id", Path(__file__).name)
        self.assertTrue(Path(__file__).name.endswith(
            "brief_field_decl_contract.py"
        ))
    ...
    def test_does_not_claim_leftover_storm_day_night_or_result(self) -> None:
        ...
        self.assertIn("FName MissionId;", day_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Brief"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_missing_brief_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_title_does_not_satisfy_brief(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_TITLE}\n"
    ...
    def test_initializer_must_be_empty_text(self) -> None:
        ...
        self.assertIn('const TCHAR* Brief = TEXT("");', compact_origin)
        self.assertNotIn("Brief = NAME_None", compact_origin)
        self.assertNotIn("Brief = false", compact_origin)
        self.assertNotIn("Brief = true", compact_origin)
        self.assertNotIn("Brief = 0.f", compact_origin)
        self.assertNotIn("Brief = 160.f", compact_origin)
        self.assertNotIn('Brief = TEXT("Storm")', compact_origin)

    def test_brief_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("const TCHAR* Brief"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SUCCESS}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    brief_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(brief_has_uproperty_wrap(origin), origin)
    ...
    def test_harbor_tokens_fail_closed_if_present(self) -> None:
        locked_only = f"{LOCKED_DECL}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec Brief field decl contract "
