...
# or lock the CanLaunch body in the .cpp.
...
# This lane only adds an isolated Python CanLaunch
# declaration contract. Stay off ConfigureFromMission
# (sibling draft #366), SetAssetsReady (sibling draft),
# AdvanceBriefing (sibling draft), AcknowledgeAndLaunch
# (in-flight), GetBriefingState, GetElapsedSeconds,
...
# leftover AcknowledgeAndLaunch, leftover
# FindObjective #364, and leftover
...
# .cpp CanLaunch body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionBriefingComponent::CanLaunch",
    "SkyguardMissionBriefingComponent.cpp",
)
SIBLING_TYPES = (
...
class BriefingCanLaunchDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, CAN_LAUNCH), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CAN_LAUNCH)
        self.assertIn("CanLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CAN_LAUNCH))

    def test_missing_can_launch_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CAN_LAUNCH)
        self.assertIn("CanLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CAN_LAUNCH)
        self.assertIn("CanLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, CAN_LAUNCH), section)
        self.assertNotIn("BlueprintPure", CAN_LAUNCH)
        self.assertNotIn("UFUNCTION", CAN_LAUNCH)
        self.assertNotIn("Category", CAN_LAUNCH)
        self.assertNotIn("BlueprintCallable", CAN_LAUNCH)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CAN_LAUNCH)
        self.assertIn("CanLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        wrong_return = "\tvoid CanLaunch() const;\n"
        missing_const = "\tbool CanLaunch();\n"
        added_arg = "\tbool CanLaunch(bool bForce) const;\n"
        int_return = "\tint32 CanLaunch() const;\n"
        mission_arg = (
            "\tbool CanLaunch("
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CAN_LAUNCH)
            self.assertIn("CanLaunch", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_can_launch_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CAN_LAUNCH),
            CAN_LAUNCH,
        )
        self.assertTrue(has_declaration(section, CAN_LAUNCH))
        self.assertEqual(declaration_count(section, CAN_LAUNCH), 1)
        self.assertTrue(CAN_LAUNCH.startswith("bool "), CAN_LAUNCH)
        self.assertTrue(CAN_LAUNCH.endswith(";"), CAN_LAUNCH)
        self.assertIn("CanLaunch()", CAN_LAUNCH)
        self.assertIn(" const", CAN_LAUNCH)
        self.assertTrue(CAN_LAUNCH.endswith(" const;"), CAN_LAUNCH)
        self.assertNotIn("INDEX_NONE", CAN_LAUNCH)
        self.assertNotIn("{", CAN_LAUNCH)
        self.assertNotIn("}", CAN_LAUNCH)
        self.assertNotIn("return ", CAN_LAUNCH)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, CAN_LAUNCH), 1)
        one_line = f"{{\npublic:\n\t{CAN_LAUNCH}\n}}\n"
        self.assertTrue(has_declaration(one_line, CAN_LAUNCH))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CAN_LAUNCH), section)
        self.assertEqual(
            require_declaration(section, CAN_LAUNCH),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", CAN_LAUNCH)
        self.assertNotIn("return ", CAN_LAUNCH)
        self.assertNotIn("return false", CAN_LAUNCH)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{CAN_LAUNCH}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", CAN_LAUNCH)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", CAN_LAUNCH)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_can_launch_cpp_body(self) -> None:
        locked_only = f"{CAN_LAUNCH}\n"
        self.assertNotIn("{", CAN_LAUNCH)
        self.assertNotIn("}", CAN_LAUNCH)
        self.assertNotIn("return ", CAN_LAUNCH)
        ...
        self.assertNotIn("return false", CAN_LAUNCH)
        self.assertNotIn("return true", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("ConfigureFromMission", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_set_assets_ready(self) -> None:
        ...
        for neighbor in SET_ASSETS_READY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("SetAssetsReady", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_advance_briefing(self) -> None:
        ...
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("AdvanceBriefing", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_acknowledge_and_launch(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_AND_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("AcknowledgeAndLaunch", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_get_briefing_state(self) -> None:
        ...
        for neighbor in GET_BRIEFING_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("GetBriefingState", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_SECONDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("GetElapsedSeconds", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_get_minimum_warmup_seconds(self) -> None:
        ...
        for neighbor in GET_MINIMUM_WARMUP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("GetMinimumWarmupSeconds", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("GetBriefingText", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_get_radio_chatter(self) -> None:
        ...
        for neighbor in GET_RADIO_CHATTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("GetRadioChatter", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, CAN_LAUNCH)
        self.assertNotIn("Unconfigured", CAN_LAUNCH)
        self.assertNotIn("Warming", CAN_LAUNCH)
        self.assertNotIn("Launched", CAN_LAUNCH)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            CAN_LAUNCH,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CAN_LAUNCH)
        self.assertNotIn("FSkyguardBriefingCard", CAN_LAUNCH)
        self.assertNotIn("FSkyguardBriefingRadioRow", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAN_LAUNCH)
        self.assertNotIn("FillResultCombatStats", CAN_LAUNCH)
        self.assertNotIn("ASkyguardGunner", CAN_LAUNCH)
        self.assertNotIn("FillAndFinalize", CAN_LAUNCH)
        self.assertNotIn("FillAndFail", CAN_LAUNCH)
        self.assertNotIn("ApplyHydraForClusters", CAN_LAUNCH)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CAN_LAUNCH}\n"
        self.assertEqual(
            require_declaration(locked_only, CAN_LAUNCH),
            CAN_LAUNCH,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing CanLaunch contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, CAN_LAUNCH.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing CanLaunch contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CAN_LAUNCH)

    def test_contract_is_can_launch_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(CAN_LAUNCH.startswith("bool "))
        self.assertTrue(CAN_LAUNCH.endswith(" const;"))
        self.assertIn(UFUNCTION_BRIEFING, section)
