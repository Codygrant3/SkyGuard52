...
# bPackagedDevelopmentBuild.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(BlueprintReadOnly)
# then `bool bPackagedDevelopmentBuild = false;`.
...
# / 80 + `.f` only. Sibling floats are not Harbor.
# Analog leftover audible-acceptance-receipt-defaults #193 /
# #1cd7 then leftover-safe isolated BuildSha256 #1435 through
# PeakActiveVoices #1438 then this isolated
# bPackagedDevelopmentBuild field decl.
...
# Identifier bounds: match EXACT `bPackagedDevelopmentBuild`
# as a whole field name after `bool`. Do NOT accept sibling
# bools bAudibleDeviceObserved / bCalibratedMetering /
# bProductionBankReady / bAccepted.
...
TARGET = "bool bPackagedDevelopmentBuild = false;"
TARGET_WRONG_BARE = "bool bPackagedDevelopmentBuild;"
TARGET_WRONG_EQ_ONE = "bool bPackagedDevelopmentBuild = 1;"
TARGET_WRONG_EQ_NONE = "bool bPackagedDevelopmentBuild = NAME_None;"
TARGET_WRONG_TRUE = "bool bPackagedDevelopmentBuild = true;"
TARGET_WRONG_EQ_ZERO = "bool bPackagedDevelopmentBuild = 0;"
TARGET_WRONG_EQ_FLOAT = "bool bPackagedDevelopmentBuild = 0.f;"
TARGET_WRONG_FSTRING = "FString bPackagedDevelopmentBuild;"
TARGET_WRONG_FLOAT = "float bPackagedDevelopmentBuild = 0.f;"
TARGET_WRONG_FNAME = "FName bPackagedDevelopmentBuild;"
TARGET_WRONG_FTEXT = "FText bPackagedDevelopmentBuild;"
TARGET_WRONG_INT32 = "int32 bPackagedDevelopmentBuild = 0;"
TARGET_WRONG_TARRAY = "TArray<bool> bPackagedDevelopmentBuild;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> bPackagedDevelopmentBuild;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_AUDIBLE = "bool bAudibleDeviceObserved = false;"
TARGET_WRONG_CALIBRATED = "bool bCalibratedMetering = false;"
TARGET_WRONG_BANK_READY = "bool bProductionBankReady = false;"
TARGET_WRONG_ACCEPTED = "bool bAccepted = false;"
...
FIELD_NAME = "bPackagedDevelopmentBuild"
...
def leftover_sibling_receipt_field_scripts() -> tuple[str, ...]:
    ...
    # stay in LOCKED_SCRIPTS via leftover_save_record_scripts,
    # leftover_production_audio_scripts, and audio-telemetry
    # leftovers. Isolated receipt field decls BuildSha256 #1435
    # through PeakActiveVoices #1438 stay leftover here.
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Match the EXACT identifier bPackagedDevelopmentBuild as
    # a whole field name after bool. Do NOT accept sibling
    # bools. Reject dropped/changed initializer, Harbor 40/80,
    # wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"bool\s+bPackagedDevelopmentBuild\s*=\s*false\s*;",
        compact,
    )
    ...
    if re.search(r"\bbAccepted\b", compact) and exact is None:
        return False
    if re.search(
        r"bool\s+bPackagedDevelopmentBuild\s*;",
        compact,
    ) and re.search(
        r"bool\s+bPackagedDevelopmentBuild\s*=",
        compact,
    ...
    for assigned in re.findall(
        r"bool\s+bPackagedDevelopmentBuild\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "false":
            return False
    if re.search(
        r"float\s+bPackagedDevelopmentBuild\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bPackagedDevelopmentBuild\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+bPackagedDevelopmentBuild\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+bPackagedDevelopmentBuild\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bPackagedDevelopmentBuild\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+bPackagedDevelopmentBuild\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+bPackagedDevelopmentBuild\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bPackagedDevelopmentBuild\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bPackagedDevelopmentBuild is missing from "
...
class AudibleAcceptanceReceiptPackagedDevelopmentBuildFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_packaged_development_build_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_bools_do_not_satisfy_packaged_development_build(self) -> None:
        audible_only = f"\t{TARGET_WRONG_AUDIBLE}\n"
        self.assertFalse(has_declaration(audible_only, LOCKED_DECL))
        self.assertFalse(has_identifier(audible_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(audible_only, SIBLING_AUDIBLE_DEVICE_OBSERVED)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(audible_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        accepted_only = f"\t{TARGET_WRONG_ACCEPTED}\n"
        self.assertFalse(has_declaration(accepted_only, LOCKED_DECL))
        self.assertFalse(has_identifier(accepted_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(accepted_only, SIBLING_ACCEPTED)
        )
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{TARGET_WRONG_AUDIBLE}\n"
            f"\t{TARGET_WRONG_ACCEPTED}\n"
            f"\t{TARGET_WRONG_CALIBRATED}\n"
            f"\t{TARGET_WRONG_BANK_READY}\n"
            "};\n"
        )
        section = spec_section(mixed)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bPackagedDevelopmentBuild = false;", compact_origin)
        self.assertNotIn("bPackagedDevelopmentBuild = true", compact_origin)
        self.assertNotIn("bPackagedDevelopmentBuild = 1", compact_origin)
        self.assertNotIn("FString bPackagedDevelopmentBuild", compact_origin)
        self.assertNotIn("int32 bPackagedDevelopmentBuild", compact_origin)

    def test_packaged_development_build_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bPackagedDevelopmentBuild"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn(SIBLING_AUDIBLE_DEVICE_OBSERVED, LOCKED_DECL)
        self.assertNotIn(SIBLING_ACCEPTED, LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        wrongs = (
            ...
            TARGET_WRONG_SOFT,
            TARGET_WRONG_HEALTH,
            TARGET_WRONG_AUDIBLE,
            TARGET_WRONG_CALIBRATED,
            TARGET_WRONG_BANK_READY,
            TARGET_WRONG_ACCEPTED,
        )
        for wrong in wrongs:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_TOTAL_UNDERRUNS} = 0;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tbool bPackagedDevelopmentBuilds = false;\n",
            "\tbool bPackagedDevelopmentBuild = true;\n",
            "\tbool bPackagedDevelopmentBuild;\n",
            "\tfloat bPackagedDevelopmentBuild = " + forty + ";\n",
            "\tfloat bPackagedDevelopmentBuild = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt packaged-development-build field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "packaged_development_build_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_packaged", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_AUDIBLE_DEVICE_OBSERVED, locked_only)
        self.assertNotIn(SIBLING_ACCEPTED, locked_only)
        for helper in namespace_helper_tokens():
