...
# RadarGhostSpawnLocation are sibling lanes (do
...
# This contract is RadarGhostSpawnRotation
...
# Skyguard|Mission07 / FRotator(0.f, 205.f, 0.f),
...
# Lock RadarGhostSpawnRotation only. Do not lock
...
LOCKED_DECL = RADAR_GHOST_SPAWN_ROTATION_FIELD
...
# an isolated Python RadarGhostSpawnRotation
...
class Mission07RadarGhostSpawnRotationFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RadarGhostSpawnRotation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_radar_ghost_spawn_rotation_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_radar_ghost_spawn_rotation_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FRotator RadarGhostSpawnRotation"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("RadarGhostSpawnRotation", LOCKED_DECL)
        self.assertIn("= FRotator(0.f, 205.f, 0.f)", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("RadarGhostSpawnRotation", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 RadarGhostSpawnRotation field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission07 RadarGhostSpawnRotation field contains "
    ...
    def test_contract_is_radar_ghost_spawn_rotation_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        for token in leftover_short_roster_values():
            ...
            self.assertNotEqual(token, "RadarGhostSpawnRotation")
    ...
    def test_sibling_director_fields_do_not_satisfy_radar_ghost_spawn_rotation(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        assigned = "\tFRotator RadarGhostSpawnRotation = FRotator(0.f, 0.f, 0.f);\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_airfield_target_runtime_does_not_satisfy_radar_ghost_spawn_rotation(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
    ...
    def test_payload_window_runtime_does_not_satisfy_radar_ghost_spawn_rotation(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
