...
# BlueprintReadOnly float MeanHeightCentimeters = 0.f; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# MeanHeightCentimeters FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float MeanHeightCentimeters = 0.f; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# MeanHeightCentimeters FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# float MeanHeightCentimeters = 0.f; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeFootprintSampleResult MeanHeightCentimeters
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float MeanHeightCentimeters = 0.f;
...
# MeanHeightCentimeters FIELD (declaration-only). It is
...
TARGET = "float MeanHeightCentimeters = 0.f;"
TARGET_WRONG = "int32 MeanHeightCentimeters = 0;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*float\s+MeanHeightCentimeters\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for float MeanHeightCentimeters is missing from "
...
class LandscapeFootprintSampleMeanHeightCentimetersFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("MeanHeightCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_mean_height_centimeters_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_mean_height_centimeters_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MeanHeightCentimeters"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("MeanHeightCentimeters", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("MeanHeightCentimeters", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float MeanHeightCentimeters"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("MeanHeightCentimeters", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_mean_height_centimeters(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
