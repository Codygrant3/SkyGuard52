...
# or lock the GetMissionTitle body.
# origin/main is the one-line prototype
# (`FText GetMissionTitle() const;`);
# accept that exact one-line form, other
...
# This lane only adds an isolated Python GetMissionTitle
# declaration contract. Stay off Configure (sibling this
# wave), GetPresentation, GetBriefingText,
...
# here. Configure / GetPresentation / GetBriefingText /
# GetBriefingCards / GetRadioRows / GetHowToFlyRows /
...
# .cpp GetMissionTitle body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. An inline body is accepted as presence,
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardBriefingWidget::GetMissionTitle",
    "SkyguardSortiePresentationWidgets.cpp",
)
SIBLING_TYPES = (
...
class BriefingWidgetGetMissionTitleDeclContractTests(unittest.TestCase):
    def test_briefing_widget_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_MISSION_TITLE),
            section,
        )
    ...
    def test_debrief_widget_does_not_satisfy_class(self) -> None:
        debrief_only = (
            ...
            "{\n"
            "public:\n"
            "\tFText GetMissionTitle() const { return Title; }\n"
            "\tFSkyguardMissionDebrief GetDebrief() const;\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_MISSION_TITLE)
        self.assertIn("GetMissionTitle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_MISSION_TITLE))

    def test_missing_get_mission_title_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tvoid Configure("
            "USkyguardSortiePresentationComponent* InPresentation);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_MISSION_TITLE)
        self.assertIn("GetMissionTitle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_MISSION_TITLE)
        self.assertIn("GetMissionTitle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_MISSION_TITLE)
        self.assertNotIn("UFUNCTION", GET_MISSION_TITLE)
        self.assertNotIn("Category", GET_MISSION_TITLE)
        self.assertNotIn("BlueprintCallable", GET_MISSION_TITLE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_MISSION_TITLE)
        self.assertIn("GetMissionTitle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tFText GetMissionTitle();\n"
        wrong_return_void = "\tvoid GetMissionTitle() const;\n"
        wrong_return_bool = "\tbool GetMissionTitle() const;\n"
        wrong_return_string = "\tFString GetMissionTitle() const;\n"
        added_arg = "\tFText GetMissionTitle(bool bReady) const;\n"
        wrong_name = "\tFText GetBriefingText() const;\n"
        ref_return = "\tconst FText& GetMissionTitle() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_MISSION_TITLE)
            self.assertIn("GetMissionTitle", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_mission_title_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_MISSION_TITLE),
            GET_MISSION_TITLE,
        )
        self.assertTrue(has_declaration(section, GET_MISSION_TITLE))
        self.assertEqual(
            declaration_count(section, GET_MISSION_TITLE),
            1,
        )
        self.assertTrue(
            GET_MISSION_TITLE.startswith("FText "),
            GET_MISSION_TITLE,
        )
        self.assertTrue(
            GET_MISSION_TITLE.endswith(";"),
            GET_MISSION_TITLE,
        )
        self.assertIn("GetMissionTitle()", GET_MISSION_TITLE)
        self.assertIn(" const", GET_MISSION_TITLE)
        self.assertIn("FText", GET_MISSION_TITLE)
        self.assertNotIn("INDEX_NONE", GET_MISSION_TITLE)
        self.assertNotIn("{", GET_MISSION_TITLE)
        self.assertNotIn("}", GET_MISSION_TITLE)
        self.assertNotIn("return ", GET_MISSION_TITLE)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_MISSION_TITLE}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_MISSION_TITLE))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_MISSION_TITLE),
            GET_MISSION_TITLE,
        )
        exact_inline = (
            "FText GetMissionTitle() const { return Title; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_MISSION_TITLE))
        self.assertEqual(
            require_declaration(exact_inline, GET_MISSION_TITLE),
            GET_MISSION_TITLE,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_MISSION_TITLE),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
        ...
        self.assertNotIn("{", GET_MISSION_TITLE)
        self.assertNotIn("return Title", GET_MISSION_TITLE)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tFText\n"
            "\tGetMissionTitle() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tFText GetMissionTitle(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_MISSION_TITLE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_MISSION_TITLE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_MISSION_TITLE),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", GET_MISSION_TITLE)
        self.assertNotIn("return ", GET_MISSION_TITLE)
        self.assertNotIn("return Other", GET_MISSION_TITLE)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_MISSION_TITLE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_MISSION_TITLE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_MISSION_TITLE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_mission_title_body(self) -> None:
        locked_only = f"{GET_MISSION_TITLE}\n"
        self.assertNotIn("{", GET_MISSION_TITLE)
        self.assertNotIn("}", GET_MISSION_TITLE)
        self.assertNotIn("return ", GET_MISSION_TITLE)
        self.assertNotIn("return Title", GET_MISSION_TITLE)
        self.assertNotIn("return Title", locked_only)
        self.assertNotIn(
            "USkyguardBriefingWidget::GetMissionTitle",
            GET_MISSION_TITLE,
        )
        ...
        self.assertNotIn("return false", GET_MISSION_TITLE)
        self.assertNotIn("return true", GET_MISSION_TITLE)

    def test_contract_does_not_relock_configure(self) -> None:
        locked_only = f"{GET_MISSION_TITLE}\n"
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("Configure(", GET_MISSION_TITLE)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("GetPresentation", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("GetBriefingText", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_get_briefing_cards(self) -> None:
        ...
        for neighbor in GET_BRIEFING_CARDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("GetBriefingCards", GET_MISSION_TITLE)
        self.assertNotIn("GetBriefingCards", locked_only)
        self.assertNotIn("FSkyguardBriefingCard", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_get_radio_rows(self) -> None:
        ...
        for neighbor in GET_RADIO_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("GetRadioRows", GET_MISSION_TITLE)
        self.assertNotIn("GetRadioRows", locked_only)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_get_how_to_fly_rows(self) -> None:
        ...
        for neighbor in GET_HOW_TO_FLY_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("GetHowToFlyRows", GET_MISSION_TITLE)
        self.assertNotIn("GetHowToFlyRows", locked_only)
        self.assertNotIn("FSkyguardHowToFlyRow", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_acknowledge_briefing(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("AcknowledgeBriefing", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_launch_sortie(self) -> None:
        ...
        for neighbor in LAUNCH_SORTIE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("LaunchSortie", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_MISSION_TITLE)
        self.assertNotIn("Unconfigured", GET_MISSION_TITLE)
        self.assertNotIn("Warming", GET_MISSION_TITLE)
        self.assertNotIn("Launched", GET_MISSION_TITLE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_MISSION_TITLE,
        )
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation(
        ...
        self.assertNotIn("GetPresentationState", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MISSION_TITLE)
        self.assertNotIn("FillResultCombatStats", GET_MISSION_TITLE)
        self.assertNotIn("ASkyguardGunner", GET_MISSION_TITLE)
        self.assertNotIn("FillAndFinalize", GET_MISSION_TITLE)
        self.assertNotIn("FillAndFail", GET_MISSION_TITLE)
        self.assertNotIn("ApplyHydraForClusters", GET_MISSION_TITLE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_MISSION_TITLE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_MISSION_TITLE),
            GET_MISSION_TITLE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing widget GetMissionTitle contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_MISSION_TITLE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing widget GetMissionTitle contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_MISSION_TITLE)

    def test_contract_is_get_mission_title_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_MISSION_TITLE.startswith("FText "))
        self.assertTrue(GET_MISSION_TITLE.endswith(";"))
        self.assertIn(" const", GET_MISSION_TITLE)
        self.assertIn(UFUNCTION_BRIEFING, section)
