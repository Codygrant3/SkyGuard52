# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot GuidedReady.
...
# Exact identifier vs siblings CannonReady, RocketReady,
# Score, Medal, ShotsFired, and Hits (same type+initializer).
...
# SelectedLoadout, CannonReady, RocketReady.
...
# leftover Harbor clock directors,
...
TARGET = "int32 GuidedReady = 0;"
TARGET_WRONG_MISSING = "int32 GuidedReady;"
TARGET_WRONG_FALSE = "int32 GuidedReady = false;"
TARGET_WRONG_TRUE = "int32 GuidedReady = true;"
TARGET_WRONG_ZERO_F = "int32 GuidedReady = 0.f;"
TARGET_WRONG_HUNDRED = "int32 GuidedReady = 100;"
TARGET_WRONG_FLOAT = "float GuidedReady = 0;"
TARGET_WRONG_BOOL = "bool GuidedReady = 0;"
TARGET_WRONG_FNAME = "FName GuidedReady;"
...
STOP_BEFORE_MISSION_RESULT = "FSkyguardMissionResult"
...
STOP_BEFORE_AUDIBLE = "FSkyguardAudibleAcceptanceReceipt"
STOP_BEFORE_MESH_BIND = "FSkyguardMeshBindSlot"
...
SAME_TYPE_INITIALIZER_SIBLINGS = (
...
def leftover_cpg_debrief_isolated_scripts() -> tuple[str, ...]:
    ...
    return (
        LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS,
        prefix + "test_cpg_debrief_snapshot_defaults.py",
        prefix + "test_cpg_debrief_snapshot_defaults_tests.py",
        LEFTOVER_CPG_DEBRIEF_FAIL_CLOSED,
        LEFTOVER_CPG_DEBRIEF_FAIL_CLOSED_TESTS,
        LEFTOVER_CPG_DEBRIEF_FAIL_CLOSED_CONTRACT,
        prefix + "test_cpg_debrief_copy_decl_contract.py",
        prefix + "test_cpg_copy_has_banned_term_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_valid_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_won_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_mission_title"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_outcome_narrative"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_score_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_medal_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_shots_fired"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_hits_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_cargo_percent"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_radar_dead"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_destroyed_systems"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_selected_loadout"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_cannon_ready"
        "_field_decl_contract.py",
        prefix + "test_cpg_debrief_snapshot_rocket_ready"
        "_field_decl_contract.py",
        prefix + "test_mesh_bind_slot_notes_field_decl_contract.py",
        prefix + "test_mesh_bind_slot_proxy_fallback"
        "_field_decl_contract.py",
        prefix + "test_mesh_bind_slot_slot_id_field_decl_contract.py",
        prefix + "test_mesh_bind_slot_preferred_field_decl_contract.py",
        prefix + "test_audible_acceptance_receipt_defaults_contract.py",
        LEFTOVER_MESH_BIND_SLOT,
        LEFTOVER_THEATER_KIT_BULK,
    )
...
LOCKED_SCRIPTS = (
    ...
    LEFTOVER_RADAR_NODE_RESET_GAMEPLAY_TESTS,
    LEFTOVER_RADAR_NODE_RESET_GAMEPLAY_CONTRACT,
) + leftover_live_copy_boss_scripts() + leftover_cpg_debrief_isolated_scripts()
...
GUIDED_READY_DECL_RE = re.compile(r"int32\s+GuidedReady\s*=\s*0\s*;")
WRONG_GUIDED_READY_INIT_RE = re.compile(
    r"int32\s+GuidedReady\s*=\s*(?:0\.f|0\.F|false|true|100|0\.0|NAME_None)\b"
)
MISSING_GUIDED_READY_INIT_RE = re.compile(r"int32\s+GuidedReady\s*;")
WRONG_GUIDED_READY_TYPE_RE = re.compile(
    r"(?:float|bool|FName|FString|uint32|int64)\s+GuidedReady\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 GuidedReady = 0;`.
    # Do not accept missing initializer, `= 0.f`, `= false`,
    # `= 100`, leftover float / bool / FName, or sibling
    # CannonReady / RocketReady / Score / Medal / ShotsFired /
    # Hits (same type+initializer). `= 0` on int32 is not Harbor.
    compact = collapsed(region)
    if WRONG_GUIDED_READY_INIT_RE.search(compact):
        return False
    if WRONG_GUIDED_READY_TYPE_RE.search(compact):
        return False
    if MISSING_GUIDED_READY_INIT_RE.search(compact) and (
        GUIDED_READY_DECL_RE.search(compact) is None
    ):
        return False
    if GUIDED_READY_DECL_RE.search(compact) is None:
        return False
...
def count_one_declaration(region: str, declaration: str) -> int:
    compact_region = collapsed(region)
    return len(GUIDED_READY_DECL_RE.findall(compact_region))
...
def field_has_attached_uproperty(section: str) -> bool:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GuidedReady\b", compact[index:]
        ):
            return True
...
class CpgDebriefSnapshotGuidedReadyFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GuidedReady"), section)
    ...
    def test_capture_or_patrol_body_does_not_satisfy(self) -> None:
        mixed = (
        ...
        self.assertFalse(has_identifier(section, "GuidedReady"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        self.assertNotIn(STOP_BEFORE_PATROL, section)
        self.assertNotIn(STOP_BEFORE_BUILD_COPY, section)
        self.assertNotIn(STOP_BEFORE_BANNED_TERM, section)
        ...
        self.assertIn("GuidedReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_guided_ready_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tint32 {SIBLING_CANNON_READY} = 0;\n"
            f"\tint32 {SIBLING_ROCKET_READY} = 0;\n"
            f"\tint32 {SIBLING_SCORE} = 0;\n"
            "};\n"
        )
    ...
    def test_clone_theater_uproperty_metadata_fails_closed(self) -> None:
        ...
        self.assertNotIn("GENERATED_BODY", section)
        self.assertNotIn("USTRUCT", section)
        self.assertFalse(field_has_attached_uproperty(section))
        ...
        leftover_section = spec_section(leftover_clone)
        self.assertTrue(field_has_attached_uproperty(leftover_section))
        origin = spec_section(origin_main_header())
        self.assertFalse(field_has_attached_uproperty(origin))
        self.assertNotEqual(leftover_section, origin)

    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        missing = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_MISSING}\n"
            "};\n"
        )
        ...
        self.assertIn("int32 GuidedReady = 0;", compact_origin)
        self.assertNotIn("GuidedReady = 0.f", compact_origin)
        self.assertNotIn("GuidedReady = false", compact_origin)
        self.assertNotIn("GuidedReady = 100", compact_origin)
        self.assertNotIn("GuidedReady = true", compact_origin)

    def test_guided_ready_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GuidedReady"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_MISSING}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_MISSING}\n", LOCKED_DECL)
        self.assertIn("GuidedReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_same_type_initializer_siblings_do_not_satisfy(self) -> None:
        for sibling in SAME_TYPE_INITIALIZER_SIBLINGS:
            region = f"\tint32 {sibling} = 0;\n"
            self.assertFalse(has_declaration(region, LOCKED_DECL), sibling)
            with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_ROCKET_READY} = 0;\n",
            f"\tint32 {SIBLING_SCORE} = 0;\n",
            f"\tint32 {SIBLING_MEDAL} = 0;\n",
            f"\tint32 {SIBLING_SHOTS_FIRED} = 0;\n",
            f"\tint32 {SIBLING_HITS} = 0;\n",
            f"\tint32 {SIBLING_CARGO_PERCENT} = 100;\n",
            "\tint32 GuidedReadies = 0;\n",
            "\tbool GuidedReady = false;\n",
            "\tfloat GuidedReady = " + forty + ";\n",
            "\tfloat GuidedReady = " + eighty + ";\n",
        )
    ...
    def test_leftover_category_or_visible_anywhere_is_not_required(self) -> None:
        origin = spec_section(origin_main_header())
        ...
        self.assertIn("VisibleAnywhere", leftover_section)
        self.assertIn("BlueprintReadOnly", leftover_section)
        self.assertIn('Category="Skyguard|Theater"', leftover_section)
        self.assertTrue(field_has_attached_uproperty(leftover_section))
        self.assertFalse(field_has_attached_uproperty(origin))
        write_only = (
            f"struct {STRUCT_NAME}\n"
        ...
        self.assertIn("EditAnywhere", write_section)
        self.assertIn("BlueprintReadWrite", write_section)
        self.assertTrue(field_has_attached_uproperty(write_section))
        self.assertNotIn("EditAnywhere", origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for region in leftover_wraps:
            self.assertTrue(has_declaration(region, LOCKED_DECL), region)
            self.assertTrue(field_has_attached_uproperty(region))
        origin = spec_section(origin_main_header())
        self.assertFalse(field_has_attached_uproperty(origin))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 GuidedReady = 0;")
        self.assertNotIn("UPROPERTY", LOCKED_DECL)
        self.assertNotIn("VisibleAnywhere", LOCKED_DECL)
        self.assertNotIn("BlueprintReadOnly", LOCKED_DECL)
        self.assertNotIn("EditAnywhere", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot GuidedReady field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "guided_ready_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
