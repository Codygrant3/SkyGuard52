# THIS IS leftover-safe FSkyguardWeatherProfile Weather.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then `ESkyguardMissionWeather Weather =
...
# The initializer IS ESkyguardMissionWeather::Clear.
...
# ESkyguardMissionWeather, if the initializer is not
# ESkyguardMissionWeather::Clear, or if Harbor tokens
...
# TimeOfDayHours / WindSpeedMetersPerSecond /
# Precipitation / CloudCoverage.
# Those sibling decls remain unclaimed.
# Do NOT reopen leftover ProfileId field-decl draft.
...
# isolated Weather field decl, same as leftover
...
# ESkyguardMissionWeather::Clear, specifiers are
...
TARGET_WRONG_BARE = "ESkyguardMissionWeather Weather;"
TARGET_WRONG_STORM = (
    "ESkyguardMissionWeather Weather = "
    "ESkyguardMissionWeather::Storm;"
)
TARGET_WRONG_OVERCAST = (
    "ESkyguardMissionWeather Weather = "
...
TARGET_WRONG_NIGHT_CLEAR = (
    "ESkyguardMissionWeather Weather = "
    "ESkyguardMissionWeather::NightClear;"
)
TARGET_WRONG_NIGHT_OVERCAST = (
    "ESkyguardMissionWeather Weather = "
    "ESkyguardMissionWeather::NightOvercast;"
)
TARGET_WRONG_EQ_NONE = (
    "ESkyguardMissionWeather Weather = NAME_None;"
)
TARGET_WRONG_FALSE = (
    "ESkyguardMissionWeather Weather = false;"
)
TARGET_WRONG_TRUE = (
    "ESkyguardMissionWeather Weather = true;"
)
TARGET_WRONG_ZERO = (
    "ESkyguardMissionWeather Weather = 0.f;"
)
TARGET_WRONG_FNAME = "FName Weather;"
TARGET_WRONG_FLOAT = "float Weather;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardMissionWeather Weather =
    # ESkyguardMissionWeather::Clear;`.
    # Do not accept a bare Weather, a non-Clear
    # initializer, FName / float / leftover Health, or
    # sibling ProfileId / TimeOfDayHours fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"ESkyguardMissionWeather\s+Weather\s*;",
        compact,
    ):
        return False
    if re.search(
    ...
    if re.search(
        ...
        return False
    if re.search(
        r"ESkyguardMissionWeather\s+Weather\s*=\s*"
        r"ESkyguardMissionWeather::(?!Clear\b)",
        compact,
    ):
        return False
    if re.search(
        r"ESkyguardMissionWeather\s+Weather\s*=\s*"
        r"ESkyguardMissionWeather::Clear\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardMissionWeather\s+Weather\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardMissionWeather Weather is "
...
def require_weather_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} Weather UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} Weather"
        )
...
class WeatherProfileWeatherFieldDeclContractTests(unittest.TestCase):
    def test_weather_profile_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Weather"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Weather"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_weather_specifiers(section)
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        storm = (
            ...
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
            f"\t{TARGET_WRONG_STORM}\n"
            "};\n"
        )
        section = spec_section(storm)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Weather", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertNotIn("Weather = NAME_None", compact_origin)
        self.assertNotIn("Weather = false", compact_origin)
        self.assertNotIn("Weather = true", compact_origin)
        self.assertNotIn("Weather = 0.f", compact_origin)
        self.assertNotIn("Weather = 160.f", compact_origin)
        self.assertNotIn(
            "ESkyguardMissionWeather Weather = "
            "ESkyguardMissionWeather::Storm;",
            compact_origin,
    ...
    def test_weather_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardMissionWeather Weather"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("ESkyguardMissionWeather ", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_STORM}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_STORM}\n",
            f"\t{TARGET_WRONG_OVERCAST}\n",
            f"\t{TARGET_WRONG_RAIN}\n",
            f"\t{TARGET_WRONG_NIGHT_CLEAR}\n",
            f"\t{TARGET_WRONG_NIGHT_OVERCAST}\n",
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_weather_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weather_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weather_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weather_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weather_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weather-profile Weather field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weather_profile_weather_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
