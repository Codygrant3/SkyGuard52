# THIS IS leftover-safe FSkyguardStormRainBeatKit MissionId.
...
# leftover `FSkyguardNightSortieBeatKit`.
...
# Shore labels as this kit.
# Do NOT parse leftover `ASkyguardGunshipSortieDirector`.
# Do NOT contract sibling fields BeatCount / Title /
...
# Do NOT claim leftover NightSortieBeatKit MissionId #1484.
...
# bulk. Isolated field decl does not relock the bulk.
# THIS IS NOT leftover DaySortieBeatKit isolated MissionId.
# THIS IS NOT leftover NightSortieBeatKit isolated MissionId.
# THIS IS NOT leftover-safe TheaterKitSpec WeatherIdentity
# #1300 UPROPERTY clone.
...
# locked decl is the bare field on the StormRain kit.
...
KIND_ENUM = "ESkyguardStormRainBeatKind"
...
TARGET_WRONG_KIND = (
    "ESkyguardStormRainBeatKind MissionId = "
...
STOP_BEFORE_KIND = "enum class ESkyguardStormRainBeatKind"
...
SIBLING_HYDRA_FOR_CLUSTERS = "bHydraForClusters"
SIBLING_KINDS = "Kinds"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # BeatCount / Title / WeatherIdentity fields. Do not
    # invent a keep-thermal field on the StormRain kit.
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_LOADOUT,
        STOP_BEFORE_DAY_BEAT,
        STOP_BEFORE_NIGHT_BEAT,
...
def spec_section(header: str) -> str:
    ...
    leftover_day_beat = LEFTOVER_DAY_BEAT_STRUCT_RE.search(section)
    if leftover_day_beat is not None:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes leftover "
            f"{STOP_BEFORE_DAY_BEAT}"
        )
    leftover_night_beat = LEFTOVER_NIGHT_BEAT_STRUCT_RE.search(section)
    if leftover_night_beat is not None:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes leftover "
            f"{STOP_BEFORE_NIGHT_BEAT}"
        )
    return section
...
class StormRainBeatKitMissionIdFieldDeclContractTests(unittest.TestCase):
    def test_storm_rain_beat_kit_struct_exists(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_KIND, header)
        self.assertNotIn(STOP_BEFORE_KIND, section)
        self.assertNotIn(STOP_BEFORE_KIND, body)
        leftover_day_beat = LEFTOVER_DAY_BEAT_STRUCT_RE.search(section)
        self.assertIsNone(leftover_day_beat, section)
        leftover_night_beat = LEFTOVER_NIGHT_BEAT_STRUCT_RE.search(section)
        self.assertIsNone(leftover_night_beat, section)
        leftover_day_beat_header = LEFTOVER_DAY_BEAT_STRUCT_RE.search(header)
        self.assertIsNone(leftover_day_beat_header, header)
        leftover_night_beat_header = LEFTOVER_NIGHT_BEAT_STRUCT_RE.search(
            header
        )
        self.assertIsNone(leftover_night_beat_header, header)
    ...
    def test_leftover_day_and_night_beat_structs_do_not_satisfy(self) -> None:
        leftovers = (
            LEFTOVER_DAY_BEAT_STRUCT,
            LEFTOVER_NIGHT_BEAT_STRUCT,
        )
    ...
    def test_leftover_day_night_and_loadout_do_not_satisfy(self) -> None:
    ...
    def test_does_not_claim_leftover_day_night_or_loadout(self) -> None:
        self.assertNotEqual(HEADER_PATH, LEFTOVER_DAY_HEADER)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_NIGHT_HEADER)
    ...
    def test_missing_mission_id_declaration_fails_closed(self) -> None:
        empty = (
            ...
            f"\tbool {SIBLING_HYDRA_FOR_CLUSTERS} = true;\n"
            "};\n"
        )
    ...
    def test_does_not_parse_leftover_kits_or_neighbors(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_KIND, header)
        self.assertNotIn(STOP_BEFORE_KIND, section)
        self.assertNotIn(STOP_BEFORE_KIND, leaked)
        leftover_day_beat = LEFTOVER_DAY_BEAT_STRUCT_RE.search(header)
        self.assertIsNone(leftover_day_beat, header)
        leftover_night_beat = LEFTOVER_NIGHT_BEAT_STRUCT_RE.search(header)
        self.assertIsNone(leftover_night_beat, header)
        leftover_day_beat_section = LEFTOVER_DAY_BEAT_STRUCT_RE.search(section)
        self.assertIsNone(leftover_day_beat_section, section)
        leftover_night_beat_section = LEFTOVER_NIGHT_BEAT_STRUCT_RE.search(
            section
        )
        self.assertIsNone(leftover_night_beat_section, section)
        leftover_day_beat_body = LEFTOVER_DAY_BEAT_STRUCT_RE.search(leaked)
        self.assertIsNone(leftover_day_beat_body, leaked)
        leftover_night_beat_body = LEFTOVER_NIGHT_BEAT_STRUCT_RE.search(leaked)
        self.assertIsNone(leftover_night_beat_body, leaked)
    ...
    def test_harbor_breaker_labels_are_not_this_kit(self) -> None:
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "storm-rain-beat-kit MissionId field decl contract "
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_DAY_BEAT_STRUCT, locked_only)
        self.assertNotIn(LEFTOVER_NIGHT_BEAT_STRUCT, locked_only)
        self.assertNotIn(KIND_ENUM, locked_only)
