...
# then `float FlightClearanceRadiusCentimeters = 3000.f;`
...
# meta ClampMin 500.0 IS present. Default IS `= 3000.f`.
# This is a mission-map flight-clearance radius float, not
...
# FlightClearanceVerticalCentimeters = 2500.f, not leftover
...
# Stop BEFORE `private:`.
# Stop BEFORE sibling FlightClearanceVerticalCentimeters
# as a claimed slot.
...
# LandmarkAnchors, FlightClearanceVerticalCentimeters,
# or Readiness.
...
# RouteLengthCentimeters / sibling 2500.f
# AS THE LOCKED DECL.
...
TARGET = "float FlightClearanceRadiusCentimeters = 3000.f;"
TARGET_WRONG_BARE = "float FlightClearanceRadiusCentimeters;"
TARGET_WRONG_INIT = "float FlightClearanceRadiusCentimeters = nullptr;"
TARGET_WRONG_FALSE = "float FlightClearanceRadiusCentimeters = false;"
TARGET_WRONG_TRUE = "float FlightClearanceRadiusCentimeters = true;"
TARGET_WRONG_ZERO = "float FlightClearanceRadiusCentimeters = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> FlightClearanceRadiusCentimeters;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> FlightClearanceRadiusCentimeters;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> FlightClearanceRadiusCentimeters;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "FlightClearanceVerticalCentimeters"
...
RADIUS_DECL_RE = re.compile(
    r"float\s+FlightClearanceRadiusCentimeters\s*=\s*3000\.f\s*;"
)
RADIUS_BARE_RE = re.compile(
    r"float\s+FlightClearanceRadiusCentimeters\s*;"
)
LOOK_YAW_LIMIT_DECL_RE = re.compile(
    r"float\s+LookYawLimit\s*=\s*95\.f\s*;"
)
VERTICAL_DECL_RE = re.compile(
    r"float\s+FlightClearanceVerticalCentimeters\s*=\s*2500\.f\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover FlightClearanceVerticalCentimeters = 2500.f,
    # leftover RouteLengthCentimeters = 0.f,
    # leftover FlareCount = 6, leftover
    # IsPointInsideFlightClearance, or a bare radius
    # without the 3000.f initializer as the slot.
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_LOOK_PITCH_MIN:
            break
        if ident != "FlightClearanceRadiusCentimeters":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+FlightClearanceRadiusCentimeters\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float FlightClearanceRadiusCentimeters "
...
class MissionMapFlightClearanceRadiusCentimetersFieldDeclContractTests(unittest.TestCase):
    def test_mission_map_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "FlightClearanceRadiusCentimeters"),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertIn("FlightClearanceRadiusCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_look_yaw_limit_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float FlightClearanceRadiusCentimeters"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("3000.f", LOCKED_DECL)
        self.assertNotIn("95.f", LOCKED_DECL)
        self.assertNotIn("2500.f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tfloat FlightClearanceRadiusCentimeterss = 3000.f;\n",
            "\tint32 FlightClearanceRadiusCentimeters = 3000;\n",
            "\tbool FlightClearanceRadiusCentimeters = true;\n",
            "\tfloat FlightClearanceRadiusCentimeters = " + forty + ";\n",
            "\tfloat FlightClearanceRadiusCentimeters = " + eighty + ";\n",
        )
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "FlightClearanceRadiusCentimeters")
        )
        self.assertTrue(
            has_identifier(section, "FlightClearanceRadiusCentimeters")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map FlightClearanceRadiusCentimeters "
                "field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_map_flight_clearance_radius_centimeters"
            "_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
