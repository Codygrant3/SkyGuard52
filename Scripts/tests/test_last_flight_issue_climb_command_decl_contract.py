...
# or lock the IssueClimbCommand body in the
...
# BeginTerminalStrikeCycle / DivertWreckFromCivilians /
...
# Python IssueClimbCommand declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::IssueClimbCommand",
    "SkyguardLastFlightBoss.cpp",
    "SetExposed",
...
class LastFlightIssueClimbCommandDeclContractTests(unittest.TestCase):
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        added_arg = "\tbool IssueClimbCommand(int32 BayIndex);\n"
        leftover_arg = (
            "\tbool IssueClimbCommand(int32 BayIndex, float Extra);\n"
        )
    ...
    def test_issue_climb_command_declaration_matches_origin_main(
        ...
        self.assertNotIn("OpenGuidanceArrayExposure", ISSUE_CLIMB_COMMAND)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            ISSUE_CLIMB_COMMAND,
        )
        ...
        self.assertNotIn("PortGuidanceArray", ISSUE_CLIMB_COMMAND)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        self.assertTrue(
            has_declaration(one_line, ISSUE_CLIMB_COMMAND)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ISSUE_CLIMB_COMMAND),
    ...
    def test_contract_does_not_lock_issue_climb_command_cpp_body(
        ...
        self.assertNotIn("SetBossPhase", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetFinaleStage", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("IsWreckDiverted", ISSUE_CLIMB_COMMAND)
        self.assertNotIn(
            "GetCivilianSeparationMeters",
            ISSUE_CLIMB_COMMAND,
        )
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("PortGuidanceArray", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("StarboardGuidanceArray", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("CommandCore", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("PortEngine", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", ISSUE_CLIMB_COMMAND)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            ISSUE_CLIMB_COMMAND,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight IssueClimbCommand contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                ...
                "last flight IssueClimbCommand contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_is_issue_climb_command_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
