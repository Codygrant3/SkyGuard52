...
# then `float HeavyCruiseSpeed = 1150.f;`
# (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Spawning` with NO spaces
# around `=`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS `= 1150.f`.
# KEEP EditAnywhere BlueprintReadWrite Category Skyguard|Spawning.
# KEEP default `1150.f`. KEEP specifiers matching origin/main.
# Word-boundary: `HeavyCruiseSpeed` is NOT leftover analog
# `LightSpeedMax` (1750.f), NOT leftover analog
# `LightSpeedMin` (1300.f), NOT leftover analog
...
# `void SetSpawningEnabled(bool bEnabled);`. LOCKED_DECL is NOT
# leftover `FName WeatherIdentity`. LOCKED_DECL is NOT leftover
# `FireRate = 12.0f` / `RecoilPitch = 0.92f` /
# `RouteLengthCm = 45000.f` / `160.f`. Harbor leftover is only the
# split 40 / 80 float tokens. 1150.f is the origin/main default
# for this unique identifier, not Harbor.
# Do NOT wrap Category=`Skyguard` fields `SpawnInterval` or
# `MaxActiveDrones`. Do NOT wrap LightSpeedMax /
# LightSpeedMin / HeavyLaneModulo / bMixedThreatRoster /
# bSpawningEnabled / MaxActiveDrones / SpawnInterval in this PR.
# leftover analog drone-spawner-empty-fail-closed bulk
# #161/#df16 is behavioral.
# Parse CLASS `ASkyguardDroneSpawner` public UPROPERTY window only.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `protected:`.
# Stop BEFORE `protected:` / SpawnTimer as a claimed slot.
...
# 40592bc71aaf17722aacf8a3a611189e25fbc822 RETARGET.
...
# Clone leftover unique analog bSpawningEnabled SHA
# a0ce9ee9d3d01c95bc91948dd508698204428c71 RETARGET (bool — this
# slot is float HeavyCruiseSpeed).
# Clone leftover unique analog SetSpawningEnabled #1719 SHA
# ebbc1fd01dd17368d06692c56ad4750481f607be RETARGET (UFUNCTION —
# this slot is UPROPERTY).
# Clone leftover unique analog FIELD LightSpeedMin SHA
# 2fa898e1963cbd2325a3b626aaa926ac276d1c7d RETARGET (sibling float
# 1300.f — this slot is 1150.f).
# Clone leftover unique analog FIELD HeavyLaneModulo SHA
# c62dff4d6ee6789e64648d237c70a9560c4f925a RETARGET (int32 — this
# slot is float).
# Clone leftover unique analog FIELD bMixedThreatRoster #1723 SHA
# 12143ae2d878a0aeeecc8b232d0307462eb7324e RETARGET (bool — this
# slot is float with 1150.f).
# Clone leftover unique analog FIELD bSpawningEnabled #1722 SHA
# a0ce9ee9d3d01c95bc91948dd508698204428c71 RETARGET (bool — this
# slot is float).
# Clone leftover unique analog Mission01 actor bools #1637-#1639
# RETARGET KEEP initializer form.
# Clone #1300 SHA c4679feb43f4b97d3a814c088e3b6c344e15831f is
# Category Theater wrapping `FName WeatherIdentity` — RETARGET hard.
# LOCKED_DECL is `float HeavyCruiseSpeed = 1150.f;`
# NOT `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# NOT `FName WeatherIdentity;` (no initializer)
# NOT `void SetSpawningEnabled(bool bEnabled);`
# NOT `int32 MaxActiveDrones = 12;`
# NOT `int32 PoolCapacity = 12;`
# NOT `float SpawnInterval = 1.85f;`
# NOT `bool bSpawningEnabled = true;`
# NOT `bool bMixedThreatRoster = true;`
# NOT `bool bAllowBoundedActorSpawning = false;`
# NOT leftover analog `float LightSpeedMax = 1750.f;`
# NOT leftover analog `float LightSpeedMin = 1300.f;`
# NOT leftover analog `int32 HeavyLaneModulo = 4;`
# NOT leftover analog `int32 MaxReleasesPerBay = 6;`
# NOT leftover analog `float MinimumCivilianSeparationMeters = 550.f;`
# NOT leftover analog `float FireRate = 12.0f` /
...
# TArray RoutePoints / bool bSpawningEnabled = true;
...
# Skyguard|Pause or Skyguard|Radio or Skyguard|Briefing or
# Skyguard|Apache without |Damage or
# Category="Skyguard" without leftover-safe unique `|Spawning`
# suffix.
# Fail-closed if wrap is VisibleAnywhere or BlueprintReadOnly
# or EditInstanceOnly (must be EditAnywhere + BlueprintReadWrite).
# Public float fail-closed if LOCKED_DECL is leftover
# combat look float or leftover CPG TObjectPtr mesh/text.
# Harbor leftover is only the split 40 / 80 float tokens.
# 1150.f is not Harbor.
...
TARGET = "float HeavyCruiseSpeed = 1150.f;"
TARGET_WRONG_BARE = "float HeavyCruiseSpeed;"
TARGET_WRONG_INIT = "float HeavyCruiseSpeed = nullptr;"
TARGET_WRONG_FALSE = "bool bSpawningEnabled = true;"
TARGET_WRONG_TRUE = "float HeavyCruiseSpeed = true;"
TARGET_WRONG_ZERO = "float HeavyCruiseSpeed = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> HeavyCruiseSpeed;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> HeavyCruiseSpeed;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> HeavyCruiseSpeed;"
...
TARGET_WRONG_ALLOW_BOUNDED = "bool bAllowBoundedActorSpawning = false;"
TARGET_WRONG_MAX_RELEASES = "int32 MaxReleasesPerBay = 6;"
TARGET_WRONG_MIN_CIVILIAN = "float MinimumCivilianSeparationMeters = 550.f;"
TARGET_WRONG_LIGHT_MAX = "float LightSpeedMax = 1750.f;"
TARGET_WRONG_LIGHT_MIN = "float LightSpeedMin = 1300.f;"
TARGET_WRONG_HEAVY_LANE = "int32 HeavyLaneModulo = 4;"
TARGET_WRONG_SPAWNING_BOOL = "bool bSpawningEnabled = true;"
SIBLING_SET_SPAWNING = "SetSpawningEnabled"
SIBLING_SPAWN_INTERVAL = "SpawnInterval"
SIBLING_MAX_ACTIVE = "MaxActiveDrones"
SIBLING_HEAVY_LANE = "HeavyLaneModulo"
SIBLING_MIXED_ROSTER = "bMixedThreatRoster"
SIBLING_SPAWNING_ENABLED = "bSpawningEnabled"
SIBLING_LIGHT_MAX = "LightSpeedMax"
SIBLING_LIGHT_MIN = "LightSpeedMin"
SIBLING_COUNT_ACTIVE = "CountActiveDrones"
SIBLING_ALLOW_BOUNDED = "bAllowBoundedActorSpawning"
SIBLING_POOL_CAPACITY = "PoolCapacity"
UPROPERTY_LEFTOVER_SKYGUARD_ONLY = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard")'
)
LEFTOVER_SET_SPAWNING = (
    "Scripts/tests/test_drone_spawner_set_spawning_enabled"
    "_decl_contract.py"
)
LEFTOVER_SPAWNING_ENABLED = (
    "Scripts/tests/test_drone_spawner_spawning_enabled"
    "_field_decl_contract.py"
)
LEFTOVER_LIGHT_SPEED_MIN = (
    "Scripts/tests/test_drone_spawner_light_speed_min"
    "_field_decl_contract.py"
)
LEFTOVER_MIXED_THREAT = (
...
STOP_BEFORE_ROUTE_LENGTH_CM = "SpawnTimer"
...
SIBLING_LICENSED_VEG = "bLicensedVegetationLibraryApproved"
...
THIS_SCRIPT = (
    "Scripts/tests/test_drone_spawner_heavy_cruise_speed"
    "_field_decl_contract.py"
)
...
HEAVY_CRUISE_SPEED_DECL_RE = re.compile(
    r"float\s+HeavyCruiseSpeed\s*=\s*1150\.f\s*;"
)
HEAVY_CRUISE_SPEED_BARE_RE = re.compile(
    r"float\s+HeavyCruiseSpeed\s*;"
)
LIGHT_SPEED_MAX_DECL_RE = re.compile(
    r"float\s+LightSpeedMax\s*=\s*1750\.f\b"
)
LIGHT_SPEED_MIN_LEFTOVER_RE = re.compile(
    r"float\s+LightSpeedMin\s*=\s*1300\.f\b"
)
HEAVY_LANE_DECL_RE = re.compile(
    r"int32\s+HeavyLaneModulo\b"
)
SPAWNING_ENABLED_LEFTOVER_RE = re.compile(
    r"bool\s+bSpawningEnabled\b"
)
...
SET_SPAWNING_DECL_RE = re.compile(
    r"void\s+SetSpawningEnabled\s*\("
)
MAX_ACTIVE_DECL_RE = re.compile(
    r"int32\s+MaxActiveDrones\s*=\s*12\b"
)
SPAWN_INTERVAL_DECL_RE = re.compile(
    r"float\s+SpawnInterval\s*=\s*1\.85f\b"
)
POOL_CAPACITY_DECL_RE = re.compile(
    r"int32\s+PoolCapacity\s*=\s*12\b"
)
MIXED_ROSTER_DECL_RE = re.compile(
    r"bool\s+bMixedThreatRoster\s*=\s*true\s*;"
)
ALLOW_BOUNDED_DECL_RE = re.compile(
    r"bool\s+bAllowBoundedActorSpawning\b"
)
MAX_RELEASES_DECL_RE = re.compile(
    r"int32\s+MaxReleasesPerBay\s*=\s*6\b"
)
MIN_CIVILIAN_DECL_RE = re.compile(
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_ROUTE_POINTS,
        SIBLING_SET_SPAWNING,
        SIBLING_SPAWN_INTERVAL,
        SIBLING_MAX_ACTIVE,
        SIBLING_HEAVY_LANE,
        SIBLING_MIXED_ROSTER,
        SIBLING_SPAWNING_ENABLED,
        SIBLING_LIGHT_MAX,
        SIBLING_LIGHT_MIN,
        SIBLING_COUNT_ACTIVE,
        SIBLING_ALLOW_BOUNDED,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept leftover analog SetSpawningEnabled,
    # leftover analog bSpawningEnabled, leftover analog
    # LightSpeedMax / LightSpeedMin / HeavyLaneModulo,
    # leftover analog MaxActiveDrones / SpawnInterval /
    # PoolCapacity / bMixedThreatRoster /
    # bAllowBoundedActorSpawning, leftover FName WeatherIdentity,
    # leftover FName AssemblyRevision, leftover FName MissionId,
    # leftover float LookYawLimit = 95.f, leftover TArray
    # RoutePoints, leftover readiness
    # bPCGGenerationAuthorized / bReadyForAuthoredPCGGeneration,
    ...
    # Harbor 40 / 80, leftover FlightRouteSpline /
    # MissionDefinition / SkylineStyle / Readiness, or a
    # bare HeavyCruiseSpeed without `= 1150.f`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if HEAVY_CRUISE_SPEED_DECL_RE.search(compact) is None:
        return False
    if HEAVY_CRUISE_SPEED_BARE_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if SET_SPAWNING_DECL_RE.search(compact):
        return False
    if MAX_ACTIVE_DECL_RE.search(compact):
        return False
    if SPAWN_INTERVAL_DECL_RE.search(compact):
        return False
    if POOL_CAPACITY_DECL_RE.search(compact):
        return False
    if MIXED_ROSTER_DECL_RE.search(compact):
        return False
    if ALLOW_BOUNDED_DECL_RE.search(compact):
        return False
    if MAX_RELEASES_DECL_RE.search(compact):
        return False
    if MIN_CIVILIAN_DECL_RE.search(compact):
        return False
    if LIGHT_SPEED_MAX_DECL_RE.search(compact):
        return False
    if LIGHT_SPEED_MIN_LEFTOVER_RE.search(compact):
        return False
    if HEAVY_LANE_DECL_RE.search(compact):
        return False
    if SPAWNING_ENABLED_LEFTOVER_RE.search(compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+HeavyCruiseSpeed\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float HeavyCruiseSpeed "
...
class DroneSpawnerHeavyCruiseSpeedFieldDeclContractTests(unittest.TestCase):
    def test_drone_spawner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HeavyCruiseSpeed"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_SET_SPAWNING), body)
        self.assertFalse(has_identifier(section, SIBLING_SET_SPAWNING), section)
        self.assertTrue(has_identifier(body, SIBLING_MAX_ACTIVE), body)
        self.assertFalse(has_identifier(section, SIBLING_MAX_ACTIVE), section)
        self.assertTrue(has_identifier(body, SIBLING_MIXED_ROSTER), body)
        self.assertFalse(
            has_identifier(section, SIBLING_MIXED_ROSTER),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_HEAVY_LANE), body)
        self.assertFalse(has_identifier(section, SIBLING_HEAVY_LANE), section)
        self.assertTrue(has_identifier(body, SIBLING_SPAWNING_ENABLED), body)
        self.assertFalse(
            has_identifier(section, SIBLING_SPAWNING_ENABLED),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_LIGHT_MAX), body)
        self.assertFalse(has_identifier(section, SIBLING_LIGHT_MAX), section)
        self.assertTrue(has_identifier(body, SIBLING_LIGHT_MIN), body)
        self.assertFalse(has_identifier(section, SIBLING_LIGHT_MIN), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertIn("HeavyCruiseSpeed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_heavy_cruise_speed_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('Skyguard|Briefing', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("float LightSpeedMax = 1750.f;", compact_origin)
        self.assertNotIn("float LightSpeedMin = 1300.f;", compact_origin)
        self.assertNotIn("bool bSpawningEnabled = true;", compact_origin)
        self.assertNotIn("float SpawnInterval = 1.85f;", compact_origin)

    def test_heavy_cruise_speed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float HeavyCruiseSpeed"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1150.f", LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "float HeavyCruiseSpeed = 1150.f;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MIN_CIVILIAN)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LIGHT_MAX)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LIGHT_MIN)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HEAVY_LANE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPAWNING_BOOL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FALSE)
        self.assertNotIn("SetSpawningEnabled", LOCKED_DECL)
        self.assertNotIn("MaxActiveDrones", LOCKED_DECL)
        self.assertNotIn("SpawnInterval", LOCKED_DECL)
        self.assertNotIn("PoolCapacity", LOCKED_DECL)
        self.assertNotIn("bMixedThreatRoster", LOCKED_DECL)
        self.assertNotIn("bAllowBoundedActorSpawning", LOCKED_DECL)
        self.assertNotIn("bSpawningEnabled", LOCKED_DECL)
        self.assertNotIn("LightSpeedMax", LOCKED_DECL)
        self.assertNotIn("LightSpeedMin", LOCKED_DECL)
        self.assertNotIn("HeavyLaneModulo", LOCKED_DECL)
        self.assertNotIn("1750.f", LOCKED_DECL)
        self.assertNotIn("1300.f", LOCKED_DECL)
        self.assertNotIn("1.85f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat HeavyCruiseSpeeds = 1150.f;\n",
            "\tint32 HeavyCruiseSpeed = 1150;\n",
            "\tfloat HeavyCruiseSpeed = 1750.f;\n",
            "\tfloat HeavyCruiseSpeed = " + forty + ";\n",
            "\tfloat HeavyCruiseSpeed = " + eighty + ";\n",
            f"\t{TARGET_WRONG_LIGHT_MAX}\n",
            f"\t{TARGET_WRONG_LIGHT_MIN}\n",
            f"\t{TARGET_WRONG_HEAVY_LANE}\n",
            f"\t{TARGET_WRONG_SPAWNING_BOOL}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Skyguard|Briefing', origin)
        ...
        self.assertNotIn('Skyguard|Theater', origin)
        self.assertNotIn('Skyguard|MissionMap', origin)
        leftover_skyguard_only = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_LEFTOVER_SKYGUARD_ONLY}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        skyguard_only_specs = attached_uproperty_specifiers(
            spec_section(leftover_skyguard_only)
        )
        self.assertIn('Category="Skyguard"', skyguard_only_specs)
        self.assertNotIn("|Spawning", skyguard_only_specs)
        ...
        self.assertIn("EditAnywhere", origin)
        self.assertNotIn('Category="Skyguard|Spawning"', map_specs)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tfloat\n\tHeavyCruiseSpeed = 1150.f;\n",
            "\tfloat   HeavyCruiseSpeed = 1150.f;\n",
            "\tfloat\tHeavyCruiseSpeed = 1150.f;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_spawn_interval_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "HeavyCruiseSpeed"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_SET_SPAWNING))
        self.assertFalse(has_identifier(section, SIBLING_SET_SPAWNING))
        self.assertTrue(has_identifier(leaked, SIBLING_MAX_ACTIVE))
        self.assertFalse(has_identifier(section, SIBLING_MAX_ACTIVE))
        self.assertFalse(has_identifier(section, SIBLING_MIXED_ROSTER))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "HeavyCruiseSpeed"))
        self.assertTrue(has_identifier(section, "HeavyCruiseSpeed"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_SET_SPAWNING))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_MAX_ACTIVE))
        self.assertFalse(has_identifier(section, SIBLING_MAX_ACTIVE))
        self.assertTrue(has_identifier(leaked, SIBLING_MIXED_ROSTER))
        self.assertFalse(has_identifier(section, SIBLING_MIXED_ROSTER))
        self.assertTrue(has_identifier(leaked, SIBLING_HEAVY_LANE))
        self.assertFalse(has_identifier(section, SIBLING_HEAVY_LANE))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn(
            'Category="Skyguard|Radio"',
            UPROPERTY_EDIT_READWRITE,
        )
        self.assertNotIn(
            'Category="Skyguard|Briefing"',
            UPROPERTY_EDIT_READWRITE,
        )
        self.assertNotIn(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "drone spawner heavy cruise speed field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "drone_spawner_heavy_cruise_speed_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_SET_SPAWNING, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SPAWNING_ENABLED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LIGHT_SPEED_MIN, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MIXED_THREAT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_DRONE_SPAWNER_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_DRONE_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MAX_RELEASES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MIN_CIVILIAN, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ALLOW_AUTHORED_PCG, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOCK_WINDOW_ATTACK, LOCKED_SCRIPTS)
