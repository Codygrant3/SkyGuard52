...
# GetActivationCount / ActivatePooledEffect /
...
# Word-boundary match GetAllocatedPoolSize vs GetActivationCount.
# Identifier GetAllocatedPoolSize is NOT leftover analog
# GetActivationCount and NOT leftover analog GetSortieHits.
# THIS identifier is GetAllocatedPoolSize.
...
# `int32 GetAllocatedPoolSize() const;`
...
# LOCKED_DECL is `int32 GetAllocatedPoolSize() const;`
# NOT `int32 GetSortieHits() const;` and NOT
# `int32 GetActivationCount() const;` and NOT
...
# GetSortieHits / GetActivationCount / DeactivateAllEffects /
...
TARGET = "int32 GetAllocatedPoolSize() const;"
TARGET_WRONG_BARE = "int32 GetAllocatedPoolSize();"
TARGET_WRONG_INIT = "int32 GetAllocatedPoolSize() = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "int32 GetAllocatedPoolSize() = true;"
TARGET_WRONG_ZERO = "int32 GetAllocatedPoolSize() = 0.f;"
TARGET_WRONG_NO_CONST = "int32 GetAllocatedPoolSize();"
TARGET_WRONG_FLOAT = "float GetAllocatedPoolSize() const;"
TARGET_WRONG_FNAME_RET = "FName GetAllocatedPoolSize() const;"
...
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GetAllocatedPoolSize;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GetAllocatedPoolSize;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> GetAllocatedPoolSize;"
...
SIBLING_GET_SORTIE_SHOTS = "GetActivationCount"
...
GET_SORTIE_HITS_RE = re.compile(
    r"int32\s+GetAllocatedPoolSize\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+Components\.Num\(\)\s*;\s*\})"
)
GET_SORTIE_HITS_WRONG_TYPE_RE = re.compile(
    r"(?:float|FName|bool|FLinearColor|void)\s+"
    r"GetAllocatedPoolSize\b"
)
...
GET_SORTIE_HITS_NO_CONST_RE = re.compile(
    r"int32\s+GetAllocatedPoolSize\s*\(\s*\)\s*;"
)
...
def has_one_declaration(region: str, declaration: str) -> bool:
    ...
    pattern = re.compile(
        re.escape(stem)
        + r"\s*(?:;|\{[^}]*return\s+Components\.Num\(\)\s*;\s*\})"
    )
    return pattern.search(compact_region) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 GetAllocatedPoolSize() const;`
    # OR origin/main inline `{ return Components.Num(); }`.
    ...
    # leftover GetActivationCount / DeactivateAllEffects /
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetAllocatedPoolSize\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for int32 GetAllocatedPoolSize "
...
class EnvironmentVFXGetAllocatedPoolSizeDeclContractTests(unittest.TestCase):
    def test_environment_vfx_pool_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetAllocatedPoolSize"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GetAllocatedPoolSize"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GetAllocatedPoolSize", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_allocated_pool_size_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            collapsed("int32 GetAllocatedPoolSize();"),
            compact_origin,
        )
    ...
    def test_get_allocated_pool_size_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GetAllocatedPoolSize"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GetAllocatedPoolSizes() const;\n",
            "\tfloat GetAllocatedPoolSize() const;\n",
            "\tFName GetAllocatedPoolSize() const;\n",
            "\tint32 GetAllocatedPoolSize() const { return " + forty + "; }\n",
            "\tint32 GetAllocatedPoolSize() const { return " + eighty + "; }\n",
            f"\t{TARGET_WRONG_LAMP}\n",
            f"\t{TARGET_WRONG_PEAK}\n",
    ...
    def test_does_not_claim_sibling_ufunctions_or_neighbors(self) -> None:
        self.assertFalse(
            has_identifier(TARGET_WRONG_ACTIVATION, "GetAllocatedPoolSize")
        )
        self.assertTrue(
            has_identifier(LOCKED_DECL, "GetAllocatedPoolSize")
        )
        ...
        self.assertTrue(has_identifier(section, "GetAllocatedPoolSize"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GetAllocatedPoolSize"))
        self.assertTrue(has_identifier(section, "GetAllocatedPoolSize"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_SORTIE_SHOTS))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "environment VFX GetAllocatedPoolSize decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "environment_vfx_get_allocated_pool_size_decl_contract.py"
        ))
        self.assertNotIn("SkyguardEnvironmentVFXPoolComponent.cpp", THIS_SCRIPT)
