...
# INDEX_NONE or lock BeachTiles construction in
# the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector, not leftover
...
# bAutoLaunchAfterBriefing. Keep BeachTiles
# distinct from sibling environment fields Root
# (environment director Root — distinct from
# leftover BossDrone Root #471 and leftover
# Mission01 Integration Root #574), OceanTiles
# (in-flight sibling), LandTiles, RouteExclusion,
...
# BeachTiles;`); accept that form and other
...
# isolated-test drafts #107–#583, leftover Apache
...
# LandTiles / RouteExclusion environment field
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::BeachTiles",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01BeachTilesFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, BEACH_TILES_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, BEACH_TILES_FIELD)
        self.assertIn("BeachTiles", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, BEACH_TILES_FIELD))

    def test_missing_beach_tiles_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, BEACH_TILES_FIELD)
        self.assertIn("BeachTiles", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_MISSION01_ENVIRONMENT}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, BEACH_TILES_FIELD)
        self.assertIn("BeachTiles", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT, section)
        ...
        self.assertNotIn("UPROPERTY", BEACH_TILES_FIELD)
        self.assertNotIn("VisibleAnywhere", BEACH_TILES_FIELD)
        self.assertNotIn("BlueprintReadOnly", BEACH_TILES_FIELD)
        self.assertNotIn("Category", BEACH_TILES_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("BlueprintCallable", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertIn("Skyguard|Mission01|Environment", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertIn("VisibleAnywhere", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertIn("BlueprintReadOnly", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("EditAnywhere", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Mission02", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Boss", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Destruction", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Apache", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Mission07", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Mission10", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Encounter", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn("Safety", UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotEqual(
            UPROPERTY_MISSION01_ENVIRONMENT,
            UPROPERTY_INTEGRATION,
        )
        self.assertNotIn(UPROPERTY_INTEGRATION, UPROPERTY_MISSION01_ENVIRONMENT)
        self.assertNotIn(
            'Category="Skyguard|Mission01")',
            UPROPERTY_MISSION01_ENVIRONMENT,
        )
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_MISSION01_ENVIRONMENT)
            self.assertNotIn(invented, BEACH_TILES_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, BEACH_TILES_FIELD)
        self.assertIn("BeachTiles", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUHierarchicalInstancedStaticMeshComponent* BeachTiles;\n"
        )
        as_array = (
            "\tTArray<TObjectPtr<"
        ...
        scene = "\tTObjectPtr<USceneComponent> BeachTiles;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> BeachTiles;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> BeachTiles;\n"
        )
        weak = (
        ...
        soft = (
            "\tTSoftObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            "BeachTiles;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, BEACH_TILES_FIELD)
            self.assertIn("BeachTiles", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_beach_tiles_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, BEACH_TILES_FIELD),
            BEACH_TILES_FIELD,
        )
        self.assertTrue(has_declaration(section, BEACH_TILES_FIELD))
        self.assertEqual(
            declaration_count(section, BEACH_TILES_FIELD),
            1,
        )
        self.assertTrue(
            BEACH_TILES_FIELD.startswith(
                "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            ),
            BEACH_TILES_FIELD,
        )
        self.assertTrue(BEACH_TILES_FIELD.endswith(";"), BEACH_TILES_FIELD)
        self.assertIn("BeachTiles", BEACH_TILES_FIELD)
        self.assertIn(
            "UHierarchicalInstancedStaticMeshComponent",
            BEACH_TILES_FIELD,
        )
        self.assertIn("TObjectPtr<", BEACH_TILES_FIELD)
        self.assertNotIn("TArray<", BEACH_TILES_FIELD)
        self.assertNotIn("TSoftObjectPtr", BEACH_TILES_FIELD)
        self.assertNotIn("=", BEACH_TILES_FIELD)
        self.assertNotIn("nullptr", BEACH_TILES_FIELD)
        self.assertNotIn("INDEX_NONE", BEACH_TILES_FIELD)
        self.assertNotIn("UFUNCTION", BEACH_TILES_FIELD)
        self.assertNotIn("{", BEACH_TILES_FIELD)
        self.assertNotIn("}", BEACH_TILES_FIELD)
        self.assertNotIn("return ", BEACH_TILES_FIELD)
        self.assertNotIn("OceanTiles", BEACH_TILES_FIELD)
        self.assertNotIn("LandTiles", BEACH_TILES_FIELD)
        self.assertNotIn("RouteExclusion", BEACH_TILES_FIELD)
        self.assertNotIn("LandScatterBounds", BEACH_TILES_FIELD)
        self.assertNotIn("InlandVegetationPCG", BEACH_TILES_FIELD)
        self.assertNotIn("ProductionLandscape", BEACH_TILES_FIELD)
        self.assertNotIn("AuthoredPCGGraph", BEACH_TILES_FIELD)
        self.assertNotIn("Root", BEACH_TILES_FIELD)
        self.assertNotIn("GetReadiness", BEACH_TILES_FIELD)
        self.assertNotIn("Briefing", BEACH_TILES_FIELD)
        self.assertNotIn("AudioDirector", BEACH_TILES_FIELD)
        self.assertNotIn("RadioChatter", BEACH_TILES_FIELD)
        self.assertNotIn("SortiePresentation", BEACH_TILES_FIELD)
        self.assertNotIn("CampaignDefinition", BEACH_TILES_FIELD)
        self.assertNotIn("MissionDefinition", BEACH_TILES_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", BEACH_TILES_FIELD)
        self.assertNotIn("bAutoInitialize", BEACH_TILES_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", BEACH_TILES_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", BEACH_TILES_FIELD)
        self.assertNotIn("GetAircraft", BEACH_TILES_FIELD)
        self.assertNotIn("ConfigureFromMission", BEACH_TILES_FIELD)
        self.assertNotIn("AdvanceBriefing", BEACH_TILES_FIELD)
        self.assertNotIn("SetAssetsReady", BEACH_TILES_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", BEACH_TILES_FIELD)
        self.assertNotIn("CanLaunch", BEACH_TILES_FIELD)
        self.assertNotIn("GetElapsedSeconds", BEACH_TILES_FIELD)
        self.assertNotIn("GetBriefingState", BEACH_TILES_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", BEACH_TILES_FIELD)
        self.assertNotIn("GetBriefingText", BEACH_TILES_FIELD)
        self.assertNotIn("GetRadioChatter", BEACH_TILES_FIELD)
        self.assertNotIn("GetPresentation", BEACH_TILES_FIELD)
        self.assertNotIn("GetMissionTitle", BEACH_TILES_FIELD)
        self.assertNotIn("AcknowledgeBriefing", BEACH_TILES_FIELD)
        self.assertNotIn("LaunchSortie", BEACH_TILES_FIELD)
        self.assertNotIn("HullCollider", BEACH_TILES_FIELD)
        self.assertNotIn("OpticalTracker", BEACH_TILES_FIELD)
        self.assertNotIn("WeaponServo", BEACH_TILES_FIELD)
        self.assertNotIn("CountermeasurePod", BEACH_TILES_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", BEACH_TILES_FIELD)
        self.assertNotIn("RadarNode", BEACH_TILES_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", BEACH_TILES_FIELD)
        self.assertNotIn("HarborIndustrial", BEACH_TILES_FIELD)
        self.assertNotIn("MaxIntegrity", BEACH_TILES_FIELD)
        self.assertNotIn("CurrentIntegrity", BEACH_TILES_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, BEACH_TILES_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT}\n"
            f"\t{BEACH_TILES_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT} {BEACH_TILES_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{BEACH_TILES_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, BEACH_TILES_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, BEACH_TILES_FIELD),
            section,
    ...
    def test_assigned_beach_tiles_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            "BeachTiles = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, BEACH_TILES_FIELD)
        self.assertIn("BeachTiles", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, BEACH_TILES_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("BeachTiles", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, BEACH_TILES_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{BEACH_TILES_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, BEACH_TILES_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            BEACH_TILES_FIELD.startswith("UFUNCTION"),
            BEACH_TILES_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", BEACH_TILES_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_beach_tiles_cpp_body(self) -> None:
        locked_only = f"{BEACH_TILES_FIELD}\n"
        self.assertNotIn("{", BEACH_TILES_FIELD)
        self.assertNotIn("}", BEACH_TILES_FIELD)
        self.assertNotIn("return ", BEACH_TILES_FIELD)
        ...
        self.assertNotIn("return false", BEACH_TILES_FIELD)
        self.assertNotIn("return true", BEACH_TILES_FIELD)
        self.assertNotIn("CreateDefaultSubobject", BEACH_TILES_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEACH_TILES_FIELD)
        self.assertIn("BeachTiles", BEACH_TILES_FIELD)
        self.assertTrue(
            BEACH_TILES_FIELD.startswith(
                "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, BEACH_TILES_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, BEACH_TILES_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, BEACH_TILES_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", BEACH_TILES_FIELD)
        self.assertNotIn("CurrentIntegrity", BEACH_TILES_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", BEACH_TILES_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, BEACH_TILES_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", BEACH_TILES_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{BEACH_TILES_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, BEACH_TILES_FIELD),
            BEACH_TILES_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BEACH_TILES_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, BEACH_TILES_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 BeachTiles field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{BEACH_TILES_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, BEACH_TILES_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                BEACH_TILES_FIELD.lower(),
                "mission01 BeachTiles contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, BEACH_TILES_FIELD)

    def test_contract_is_beach_tiles_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("BeachTiles", BEACH_TILES_FIELD)
        self.assertTrue(BEACH_TILES_FIELD.endswith(";"))
        self.assertNotIn("=", BEACH_TILES_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT, section)

    def test_sibling_environment_fields_do_not_satisfy_beach_tiles(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, BEACH_TILES_FIELD))
            self.assertNotEqual(BEACH_TILES_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_beach_tiles(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
