# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot OutcomeNarrative.
...
# Bare FString with NO initializer.
...
# not FString, or an initializer is added
# (`= TEXT("")` / `= FString()` / `= false`).
# Exact identifier vs sibling MissionTitle (same type,
# also bare). Do not claim sibling fields: bValid, bWon,
# MissionTitle, Score, Medal, ShotsFired, Hits,
# CargoPercent, bRadarDead, DestroyedSystems,
# SelectedLoadout, CannonReady, RocketReady, GuidedReady.
# Static-verify origin/main via git show
# origin/main:Source/Skyguard52/SkyguardCpgDebrief.h.
...
# (keep test_cpg_debrief_snapshot_defaults_contract.py in
# LOCKED_SCRIPTS).
...
# directors, leftover retired live-copy types
# (split tokens).
...
# If a clone still asserts UPROPERTY /
# Category="Skyguard|Theater" / VisibleAnywhere /
# EditAnywhere / BlueprintReadOnly, fail-closed.
...
TARGET = "FString OutcomeNarrative;"
TARGET_WRONG_TEXT = 'FString OutcomeNarrative = TEXT("");'
TARGET_WRONG_FSTRING = "FString OutcomeNarrative = FString();"
TARGET_WRONG_FALSE = "FString OutcomeNarrative = false;"
TARGET_WRONG_TRUE = "FString OutcomeNarrative = true;"
TARGET_WRONG_NONE = "FString OutcomeNarrative = NAME_None;"
TARGET_WRONG_ZERO = "FString OutcomeNarrative = 0;"
TARGET_WRONG_FNAME = "FName OutcomeNarrative;"
TARGET_WRONG_BOOL = "bool OutcomeNarrative;"
TARGET_WRONG_INT = "int32 OutcomeNarrative;"
TARGET_WRONG_FLOAT = "float OutcomeNarrative;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
SIBLING_TITLE = "FString MissionTitle;"
LOCKED_DECL = TARGET
CLONE_THEATER_UPROPERTY = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
    'Category="Skyguard|Theater")'
)
CLONE_EDIT_ANYWHERE = "UPROPERTY(EditAnywhere, BlueprintReadOnly)"
STOP_BEFORE_CAPTURE = "SkyguardCaptureCpgDebrief"
STOP_BEFORE_COPY = "SkyguardBuildCpgDebriefCopy"
...
STOP_BEFORE_THEATER = "FSkyguardTheaterKitSpec"
...
LEFTOVER_CPG_DEBRIEF_VALID = (
    "Scripts/tests/test_cpg_debrief_snapshot_valid"
    "_field_decl_contract.py"
)
LEFTOVER_CPG_DEBRIEF_WON = (
...
LEFTOVER_AUDIO_PRODUCTION_BANK_FAIL_CLOSED = (
    "Scripts/tests/test_audio_production_bank_empty_fail_closed.py"
)
LEFTOVER_SORTIE_DIRECTOR_FAIL_CLOSED = (
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_briefing_widget_get_outcome_narrative_decl_contract.py",
...
def leftover_neighbor_type_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_CAPTURE,
        STOP_BEFORE_COPY,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString OutcomeNarrative;`.
    # Do not accept `= TEXT("")` / `= FString()` / `= false` /
    # `= true` / leftover FName / sibling MissionTitle.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FString\s+OutcomeNarrative\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:FName|bool|int32|float|uint32)\s+OutcomeNarrative\b",
        compact,
    ):
        return False
    if re.search(
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return leftover_neighbor_type_tokens() + (
        ...
        "class SKYGUARD52_API ASkyguardPatrolShipBoss",
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString\s+OutcomeNarrative\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString OutcomeNarrative is missing from "
        f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} body"
    )


def reflection_prefix(header: str) -> str:
    match = STRUCT_RE.search(header)
    if match is None:
        raise AssertionError(
            f"{STRUCT_NAME} is missing from origin/main:{HEADER_PATH}"
        )
    return header[max(0, match.start() - 80) : match.start()]
...
def require_plain_field(header: str) -> str:
    section = spec_section(header)
    require_declaration(section, LOCKED_DECL)
    prefix = reflection_prefix(header)
    if "USTRUCT" in prefix or "USTRUCT" in section:
        raise AssertionError(
            f"{STRUCT_NAME} is a plain C++ struct; USTRUCT is forbidden"
        )
    if "GENERATED_BODY" in section:
        raise AssertionError(
            f"{STRUCT_NAME} is a plain C++ struct; GENERATED_BODY is forbidden"
        )
    if "UPROPERTY" in section:
        raise AssertionError(
            f"{STRUCT_NAME} OutcomeNarrative must stay a bare FString "
            "with no UPROPERTY"
        )
    for banned in (
        "VisibleAnywhere",
        ...
        if banned in section:
            raise AssertionError(
                f"{STRUCT_NAME} OutcomeNarrative must not carry {banned}"
            )
    return LOCKED_DECL


class CpgDebriefSnapshotOutcomeNarrativeFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_THEATER)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_MISSION_RESULT)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_AUDIBLE)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_MESH_BIND)
        body = struct_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "OutcomeNarrative"), section)
        ...
        self.assertIn(STOP_BEFORE_COPY, header)
        self.assertNotIn(STOP_BEFORE_COPY, section)
        self.assertNotIn(STOP_BEFORE_COPY, body)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        result = (
            f"struct {STOP_BEFORE_MISSION_RESULT}\n"
            "{\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
    ...
    def test_capture_or_patrol_declaration_does_not_satisfy(self) -> None:
        mixed = (
        ...
        self.assertFalse(has_identifier(section, "OutcomeNarrative"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        self.assertNotIn(STOP_BEFORE_PATROL, section)
        self.assertNotIn(STOP_BEFORE_COPY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("OutcomeNarrative", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_outcome_narrative_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{SIBLING_TITLE}\n"
    ...
    def test_origin_main_has_no_uproperty_or_reflection(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        self.assertEqual(require_plain_field(header), LOCKED_DECL)
        self.assertNotIn("UPROPERTY", section)
        self.assertNotIn("USTRUCT", section)
        self.assertNotIn("GENERATED_BODY", section)
        ...
        self.assertNotIn("USTRUCT", reflection_prefix(header))
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_clone_theater_uproperty_fails_closed(self) -> None:
        wrapped = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{CLONE_THEATER_UPROPERTY}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_plain_field(wrapped)
        self.assertIn("UPROPERTY", str(raised.exception))
        edit = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{CLONE_EDIT_ANYWHERE}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_plain_field(edit)
        self.assertIn("UPROPERTY", str(raised.exception))
        ustruct = (
            "USTRUCT()\n"
            f"struct {STRUCT_NAME}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_plain_field(ustruct)
        message = str(raised.exception)
        self.assertTrue(
            "USTRUCT" in message or "GENERATED_BODY" in message,
            message,
        )
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString OutcomeNarrative;", compact_origin)
        self.assertNotIn("OutcomeNarrative = TEXT(\"\")", compact_origin)
        self.assertNotIn("OutcomeNarrative = FString()", compact_origin)
        self.assertNotIn("OutcomeNarrative = false", compact_origin)
        self.assertNotIn("OutcomeNarrative = true", compact_origin)
        self.assertNotIn("OutcomeNarrative = NAME_None", compact_origin)

    def test_outcome_narrative_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString OutcomeNarrative"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{SIBLING_TITLE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{SIBLING_TITLE}\n", LOCKED_DECL)
        self.assertIn("OutcomeNarrative", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\t{SIBLING_TITLE}\n",
            "\tFString OutcomeNarratives;\n",
            "\tFString Title;\n",
            "\tint32 OutcomeNarrative;\n",
            "\tbool OutcomeNarrative;\n",
            "\tfloat OutcomeNarrative = " + forty + ";\n",
            "\tfloat OutcomeNarrative = " + eighty + ";\n",
        )
    ...
    def test_clone_category_or_edit_anywhere_fails_closed(self) -> None:
        no_plain = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(VisibleAnywhere, BlueprintReadOnly)\n"
    ...
    def test_does_not_contract_sibling_snapshot_fields(self) -> None:
        ...
        self.assertTrue(has_one_declaration(section, SIBLING_TITLE), section)
        self.assertNotEqual(LOCKED_DECL, SIBLING_TITLE)
        for field in leftover_neighbor_hit_fields():
    ...
    def test_does_not_parse_capture_copy_or_neighbors(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_COPY, header)
        self.assertNotIn(STOP_BEFORE_COPY, section)
        self.assertNotIn(STOP_BEFORE_COPY, leaked)
        self.assertIn(STOP_BEFORE_BANNED_TERM, header)
        self.assertNotIn(STOP_BEFORE_BANNED_TERM, section)
        ...
        self.assertNotIn("class ASkyguardPatrolShipBoss\n{", header)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString OutcomeNarrative;")
        self.assertNotEqual(LOCKED_DECL, CLONE_THEATER_UPROPERTY)
        self.assertIn("VisibleAnywhere", CLONE_THEATER_UPROPERTY)
        self.assertIn("BlueprintReadOnly", CLONE_THEATER_UPROPERTY)
        self.assertIn('Category="Skyguard|Theater"', CLONE_THEATER_UPROPERTY)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardRuntimeMeshCatalog.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot OutcomeNarrative field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "outcome_narrative_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_CPG_DEBRIEF_VALID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_WON, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_COPY, locked_only)
