# THIS IS leftover-safe FSkyguardMeshBindSlot Notes.
...
# then BARE `FString Notes;` (no initializer).
...
# FString, or if an initializer is added.
...
# Do NOT contract sibling fields SlotId / Preferred /
# ProxyFallback.
# Analog leftover mesh-bind-slot-fields #194 / #6829 then
# leftover-safe isolated SlotId / Preferred in-flight then
# this isolated Notes field decl.
...
# Category="Skyguard|MeshBind", bare FString Notes.
...
# Harbor 40/80 fail-closed. Bare FString has no Harbor
# initializer.
...
TARGET = "FString Notes;"
TARGET_WRONG_EQ_NONE = "FString Notes = NAME_None;"
TARGET_WRONG_FALSE = "FString Notes = false;"
TARGET_WRONG_TRUE = "FString Notes = true;"
TARGET_WRONG_ZERO = "FString Notes = 0.f;"
TARGET_WRONG_FLOAT = "float Notes;"
TARGET_WRONG_FNAME = "FName Notes;"
...
LOCKED_SCRIPTS = (
    ...
    LEFTOVER_MESH_BIND_SLOT,
    # leftover-safe isolated SlotId / Preferred in-flight
    LEFTOVER_MESH_BIND_SLOT_SLOT_ID,
    LEFTOVER_MESH_BIND_SLOT_PREFERRED,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString Notes;`.
    ...
    # SlotId / Preferred / ProxyFallback fields.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString\s+Notes\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString Notes is missing from "
...
class MeshBindSlotNotesFieldDeclContractTests(unittest.TestCase):
    def test_mesh_bind_slot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Notes"), section)
    ...
    def test_catalog_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Notes"), section)
        self.assertNotIn(STOP_BEFORE_CATALOG, section)
        self.assertNotIn(CATALOG_NAME, section)
        self.assertNotIn(ENSURE_DEFAULT_SLOTS, section)
        self.assertNotIn(RESOLVE_MESH, section)
    ...
    def test_missing_notes_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString Notes;", compact_origin)
        self.assertNotIn("Notes = NAME_None", compact_origin)
        self.assertNotIn("Notes = false", compact_origin)
        self.assertNotIn("Notes = true", compact_origin)
        self.assertNotIn("Notes = 0.f", compact_origin)
        self.assertNotIn("Notes = 160.f", compact_origin)

    def test_notes_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString Notes"),
            LOCKED_DECL,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mesh-bind-slot Notes field decl contract "
