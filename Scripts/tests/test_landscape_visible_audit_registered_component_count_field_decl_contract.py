...
# RegisteredComponentCount FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# int32 RegisteredComponentCount = 0; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# RegisteredComponentCount FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 RegisteredComponentCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# LandscapeComponentCount later; this ticket is RegisteredComponentCount only).
# LOCK leftover landscape-visible-audit-defaults #0bce.
# LOCK leftover Footprint bSuccess #1189.
...
# BlueprintReadOnly int32 RegisteredComponentCount = 0;
...
# RegisteredComponentCount FIELD (declaration-only). It is
...
TARGET = "int32 RegisteredComponentCount = 0;"
TARGET_WRONG = "float RegisteredComponentCount = 0;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+RegisteredComponentCount\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 RegisteredComponentCount is missing from "
...
class LandscapeVisibleAuditRegisteredComponentCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RegisteredComponentCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_registered_component_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_registered_component_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RegisteredComponentCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("RegisteredComponentCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("RegisteredComponentCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 RegisteredComponentCount"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("RegisteredComponentCount", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_registered_component_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
