...
# bAuthoredStructureReady. THIS IS NOT leftover Mission01
...
# bool bAuthoredStructureReady = false. Fail closed if type/decl
# missing, renamed, Category= is added, or default is not false.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bAuthoredStructureReady. THIS IS NOT leftover
# mission01-environment-authoring-result-defaults #df1a.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bAuthoredStructureReady. THIS IS NOT leftover MaterialCompilation bSuccess #1223.
...
# mission01-environment-authoring-result-defaults #df1a.
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bAuthoredStructureReady FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bAuthoredStructureReady = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS `= false` NOT `= true`. Fail closed if the type
...
# Do NOT parse FSkyguardLandscapeCaptureConfigurationResult (leftover bSuccess #1214).
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bAuthoredStructureReady. THIS IS NOT leftover VisibleAudit bSuccess #1199.
# THIS IS NOT leftover FootprintSampleResult bSuccess #1189.
# THIS IS NOT leftover landscape-capture-config-defaults #d6fd.
# THIS IS NOT leftover landscape-visible-audit-defaults #0bce.
# THIS IS NOT leftover landscape-material-compilation-defaults #01d4.
...
# bool bAuthoredStructureReady = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS `= false` NOT `= true` (NOT leftover
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bAuthoredStructureReady FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bAuthoredStructureReady = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly bool bAuthoredStructureReady = false;
...
TARGET = "bool bAuthoredStructureReady = false;"
TARGET_WRONG = "bool bAuthoredStructureReady = true;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bAuthoredStructureReady (must be = false, not = true).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bAuthoredStructureReady\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bAuthoredStructureReady is missing from "
...
class Mission01EnvironmentAuthoringResultAuthoredStructureReadyFieldDeclContractTests(unittest.TestCase):
    def test_mission01_environment_authoring_result_struct_exists(self) -> None:
        ...
        self.assertIn("bAuthoredStructureReady", section)
        ...
        self.assertNotIn("bValid", section.replace("bAuthoredStructureReady", ""))
        ...
        self.assertNotIn("bSuccess", LOCKED_DECL.replace("bAuthoredStructureReady", ""))
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bAuthoredStructureReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_authored_structure_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_authored_structure_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bAuthoredStructureReady"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(
                "\tbool bAuthoredStructureReady = true;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("bAuthoredStructureReady", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bAuthoredStructureReady", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bAuthoredStructureReady"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bAuthoredStructureReady", locked_only)
