...
# values, or beat-kind tables. origin/main may split KindAt across
# lines as KindAt( / const FSkyguardNightSortieBeatKit& Kit, /
# int32 Index);
KIND_AT = (
...
KIND_AT_HEAD = "KindAt("
KIND_AT_KIT = "const FSkyguardNightSortieBeatKit& Kit,"
KIND_AT_INDEX = "int32 Index);"
# Leftover #56–#64 plus NightSortieBeatKit production sources/tests.
# This lane only adds an isolated Python KindAt declaration
# contract. Stay off leftover Harbor #6/#8/#9.
LOCKED = {
...
# on-main NightEyes()/DownedBird() sequences, Beats[7] (#252),
# bKeepThermal (#250), beat-kind enum (#246), beat defaults (#247),
# and kit FName fields (#254) stay sibling-only.
LOCKED_SCRIPTS = (
...
# defaults, #254 FName fields stay unlocked. Parse the namespace,
# not those types.
...
# .cpp bodies / invented kind tables stay unlocked.
CPP_AND_INVENTED = (
    "return ",
    "INDEX_NONE",
    "NAME_None",
...
class NightSortieKindAtContractTests(unittest.TestCase):
    def test_night_sortie_namespace_exists(self) -> None:
        ...
        self.assertEqual(require_declaration(body, KIND_AT), KIND_AT)
    ...
    def test_missing_kind_at_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "{\n"
            "\tconst FSkyguardNightSortieBeatKit& NightEyes();\n"
            "\tconst FSkyguardNightSortieBeatKit& DownedBird();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, KIND_AT)
        self.assertIn("KindAt", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(NAMESPACE_NAME, str(raised.exception))

    def test_kind_at_declaration_matches_origin_main(self) -> None:
        body = namespace_body(origin_main_header())
        self.assertEqual(require_declaration(body, KIND_AT), KIND_AT)
        self.assertIn(KIND_AT_HEAD, body)
        self.assertIn(KIND_AT_KIT, body)
        self.assertIn(KIND_AT_INDEX, body)
        self.assertEqual(body.count(KIND_AT_HEAD), 1)
        self.assertTrue(KIND_AT.endswith(";"), KIND_AT)
        self.assertNotIn("INDEX_NONE", KIND_AT)
        self.assertNotIn("return ", KIND_AT)

    def test_kind_at_declaration_may_be_split_across_lines(self) -> None:
        split = (
            "{\n"
            "\tESkyguardNightSortieBeatKind KindAt(\n"
            "\t\tconst FSkyguardNightSortieBeatKit& Kit,\n"
        ...
        one_line = "{\n\t" + KIND_AT + "\n}\n"
        self.assertEqual(require_declaration(split, KIND_AT), KIND_AT)
        self.assertEqual(require_declaration(one_line, KIND_AT), KIND_AT)
        self.assertIn(KIND_AT_HEAD, split)
        self.assertIn(KIND_AT_KIT, split)
        self.assertIn(KIND_AT_INDEX, split)
        body = namespace_body(origin_main_header())
        self.assertEqual(require_declaration(body, KIND_AT), KIND_AT)
        self.assertIn(KIND_AT_HEAD, body)
        self.assertIn(KIND_AT_KIT, body)
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", KIND_AT)
        self.assertNotIn("INDEX_NONE", KIND_AT)
        self.assertNotIn("NAME_None", KIND_AT)
        self.assertNotIn("{", KIND_AT)
        self.assertNotIn("}", KIND_AT)
        ...
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, KIND_AT)
            if token != "return ":
                self.assertNotIn(token, body)

    def test_contract_does_not_relock_kit_sequences(self) -> None:
        locked_only = f"{KIND_AT}\n"
        self.assertEqual(require_declaration(locked_only, KIND_AT), KIND_AT)
        for name in KIT_SEQUENCES_NOT_LOCKED:
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, KIND_AT)
    ...
    def test_contract_does_not_relock_sequences_differ(self) -> None:
        ...
        self.assertNotIn("SequencesDiffer", KIND_AT)
    ...
    def test_contract_does_not_relock_beat_index(self) -> None:
        ...
        self.assertNotIn("BeatIndexForElapsed", KIND_AT)
    ...
    def test_contract_does_not_relock_beats_enum_defaults_or_fields(
        ...
        self.assertNotIn(BEATS_NOT_LOCKED, KIND_AT)
        self.assertNotIn(KEEP_THERMAL_NOT_LOCKED, body)
        self.assertNotIn(KEEP_THERMAL_NOT_LOCKED, KIND_AT)
    ...
    def test_contract_does_not_read_cpp_or_kind_tables(self) -> None:
        body = namespace_body(origin_main_header())
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, KIND_AT)
        self.assertNotIn("SkyguardNightSortieBeatKit.cpp", body)
        self.assertNotIn("SkyguardNightSortieBeatKit::KindAt", body)
        self.assertNotIn("Beats[Index]", body)
        self.assertNotIn("Beats[7]", body)
        self.assertNotIn("Kit.Beats", body)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(KIND_AT, "Rifle")
        self.assertNotEqual(KIND_AT, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"night-sortie KindAt contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not Yak",
            )
            self.assertNotIn(banned, KIND_AT.lower())

    def test_contract_is_kind_at_declaration_only(self) -> None:
        header = origin_main_header()
        body = namespace_body(header)
        self.assertIn(f"namespace {NAMESPACE_NAME}", header)
        self.assertEqual(require_declaration(body, KIND_AT), KIND_AT)
        ...
        for neighbor in UNLOCKED_NEIGHBORS:
            ...
            self.assertNotIn(neighbor, KIND_AT)
        for name in KIT_SEQUENCES_NOT_LOCKED:
            self.assertNotIn(name, locked_only)
        self.assertNotIn("SequencesDiffer", locked_only)
        self.assertNotIn("BeatIndexForElapsed", locked_only)
