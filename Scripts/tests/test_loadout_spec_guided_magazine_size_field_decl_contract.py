# THIS IS leftover-safe FSkyguardLoadoutSpec GuidedMagazineSize.
...
# GuidedReserve (same int32 type and same
...
# RocketMagazineSize / GuidedReserve / etc.).
...
TARGET_WRONG_BARE = "int32 GuidedMagazineSize;"
TARGET_WRONG_ZERO = "int32 GuidedMagazineSize = 0;"
TARGET_WRONG_ZERO_F = "int32 GuidedMagazineSize = 0.f;"
TARGET_WRONG_FALSE = "int32 GuidedMagazineSize = false;"
TARGET_WRONG_HUNDRED = "int32 GuidedMagazineSize = 100;"
TARGET_WRONG_SIX = "int32 GuidedMagazineSize = 6;"
TARGET_WRONG_CPG_FEEL = (
    "int32 GuidedMagazineSize = SkyguardApacheCpgFeel::CannonMagazineSize;"
)
TARGET_WRONG_ROCKET_MAG = (
    "int32 GuidedMagazineSize = "
    "SkyguardApacheCpgFeel::RocketMagazineSize;"
)
TARGET_WRONG_GUIDED_RESERVE = (
    "int32 GuidedMagazineSize = "
    "SkyguardApacheCpgFeel::GuidedReserve;"
)
TARGET_WRONG_FLOAT = (
    "float GuidedMagazineSize = "
    "SkyguardApacheCpgFeel::GuidedMagazineSize;"
)
TARGET_WRONG_BOOL = (
    "bool GuidedMagazineSize = "
    "SkyguardApacheCpgFeel::GuidedMagazineSize;"
)
...
SIBLING_GUIDED_RESERVE = "GuidedReserve"
...
def locked_cpg_feel_initializer() -> str:
    return "SkyguardApacheCpgFeel::GuidedMagazineSize"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # RocketMagazineSize / GuidedReserve / HullIntegrity /
    # PlaystyleLine.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+GuidedMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::GuidedMagazineSize\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"int32\s+GuidedMagazineSize\s*=\s*0\b", compact):
        return False
    if re.search(r"int32\s+GuidedMagazineSize\s*=\s*0\.f", compact):
        return False
    if re.search(r"int32\s+GuidedMagazineSize\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+GuidedMagazineSize\s*=\s*100\b", compact):
        return False
    if re.search(r"int32\s+GuidedMagazineSize\s*=\s*6\b", compact):
        return False
    if re.search(
        r"int32\s+GuidedMagazineSize\s*=\s*"
    ...
    if re.search(
        r"int32\s+GuidedMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::GuidedReserve\b",
        compact,
    ):
        return False
    if re.search(r"float\s+GuidedMagazineSize\b", compact):
        return False
    if re.search(r"bool\s+GuidedMagazineSize\b", compact):
        return False
    if re.search(r"int32\s+GuidedMagazineSize\s*;", compact) and not re.search(
        r"int32\s+GuidedMagazineSize\s*=",
        compact,
    ):
        return False
    return True
...
class LoadoutSpecGuidedMagazineSizeFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GuidedMagazineSize"), section)
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GuidedMagazineSize"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GuidedMagazineSize", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_guided_magazine_size_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 GuidedMagazineSize"))
    ...
    def test_literal_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("int32 GuidedMagazineSize = 0;", compact_origin)
        self.assertNotIn("int32 GuidedMagazineSize = 6;", compact_origin)
        self.assertNotIn("int32 GuidedMagazineSize = 100;", compact_origin)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            "SkyguardApacheCpgFeel::RocketReserve;\n",
            f"\tint32 {SIBLING_GUIDED_RESERVE} = "
            "SkyguardApacheCpgFeel::GuidedReserve;\n",
            f"\tint32 {SIBLING_FLARE} = 6;\n",
            "\tint32 GuidedMagazineSizes = "
            "SkyguardApacheCpgFeel::GuidedMagazineSize;\n",
            "\tint32 GuidedMagazineSize = " + forty + ";\n",
            "\tint32 GuidedMagazineSize = " + eighty + ";\n",
        )
    ...
    def test_guided_magazine_size_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GuidedMagazineSize"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("SkyguardApacheCpgFeel::GuidedMagazineSize", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec GuidedMagazineSize field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "guided_magazine_size_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
