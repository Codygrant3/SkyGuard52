...
# or lock the GetEffectiveSpeedMultiplier body in the .cpp.
...
# AdvanceEncounter, sibling ResetEncounterState,
# sibling IsRouteStateSafe, and in-flight
# GetRouteProgress stay sibling-only.
...
# Python GetEffectiveSpeedMultiplier declaration
# contract. Stay off AdvanceEncounter (draft #376),
# ResetEncounterState (sibling just pushed),
# IsRouteStateSafe (sibling just pushed),
# GetRouteProgress (in-flight),
# IsAttackTelegraphActive (sibling this wave),
# and GetTelegraphsTriggered on this class. Stay off
# UPROPERTY fields, delegates, leftover boss-phase
# enum #60cb, leftover boss-definition defaults,
# leftover boss-weak-point defaults, leftover
...
# section stay unlocked. The signed min-height
# leftover is not a Harbor clock pair.
...
# .cpp GetEffectiveSpeedMultiplier body / invented
# INDEX_NONE stay unlocked. Do not invent INDEX_NONE
# or lock the cpp body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "USkyguardPathfinderEncounterController::GetEffectiveSpeedMultiplier",
    "SkyguardPathfinderEncounterController.cpp",
)
SIBLING_TYPES = (
...
class PathfinderGetEffectiveSpeedMultiplierDeclContractTests(unittest.TestCase):
    def test_pathfinder_controller_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_EFFECTIVE_SPEED_MULTIPLIER),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{GET_EFFECTIVE_SPEED_MULTIPLIER}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertIn("GetEffectiveSpeedMultiplier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_missing_get_effective_speed_multiplier_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tUSkyguardPathfinderEncounterController();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertIn("GetEffectiveSpeedMultiplier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertIn("GetEffectiveSpeedMultiplier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("UFUNCTION", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("Category", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("BlueprintCallable", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertIn("GetEffectiveSpeedMultiplier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tfloat GetEffectiveSpeedMultiplier();\n"
        wrong_return_void = "\tvoid GetEffectiveSpeedMultiplier() const;\n"
        wrong_return_bool = "\tbool GetEffectiveSpeedMultiplier() const;\n"
        wrong_return_int = "\tint32 GetEffectiveSpeedMultiplier() const;\n"
        added_arg = (
            "\tfloat GetEffectiveSpeedMultiplier(float DeltaSeconds) const;\n"
        )
        for region in (
            missing_const,
            wrong_return_void,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_EFFECTIVE_SPEED_MULTIPLIER)
            self.assertIn("GetEffectiveSpeedMultiplier", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_effective_speed_multiplier_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, GET_EFFECTIVE_SPEED_MULTIPLIER),
            1,
        )
        self.assertTrue(
            GET_EFFECTIVE_SPEED_MULTIPLIER.startswith("float "),
            GET_EFFECTIVE_SPEED_MULTIPLIER,
        )
        self.assertTrue(
            GET_EFFECTIVE_SPEED_MULTIPLIER.endswith(";"),
            GET_EFFECTIVE_SPEED_MULTIPLIER,
        )
        self.assertTrue(
            GET_EFFECTIVE_SPEED_MULTIPLIER.endswith(" const;"),
            GET_EFFECTIVE_SPEED_MULTIPLIER,
        )
        self.assertIn(
        ...
        self.assertIn(" const", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("INDEX_NONE", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("{", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("}", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("return ", GET_EFFECTIVE_SPEED_MULTIPLIER)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tfloat\n"
            "\tGetEffectiveSpeedMultiplier() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tfloat GetEffectiveSpeedMultiplier(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_EFFECTIVE_SPEED_MULTIPLIER}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, GET_EFFECTIVE_SPEED_MULTIPLIER)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_EFFECTIVE_SPEED_MULTIPLIER),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("return ", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("return Other", GET_EFFECTIVE_SPEED_MULTIPLIER)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_EFFECTIVE_SPEED_MULTIPLIER}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_EFFECTIVE_SPEED_MULTIPLIER)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_effective_speed_multiplier_cpp_body(
        self,
    ) -> None:
        locked_only = f"{GET_EFFECTIVE_SPEED_MULTIPLIER}\n"
        self.assertNotIn("{", GET_EFFECTIVE_SPEED_MULTIPLIER)
        ...
        self.assertNotIn("return false", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("return true", GET_EFFECTIVE_SPEED_MULTIPLIER)

    def test_contract_does_not_relock_advance_encounter(self) -> None:
        locked_only = f"{GET_EFFECTIVE_SPEED_MULTIPLIER}\n"
        for neighbor in ADVANCE_ENCOUNTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("AdvanceEncounter", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_reset_encounter_state(self) -> None:
        ...
        for neighbor in RESET_ENCOUNTER_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("ResetEncounterState", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_is_route_state_safe(self) -> None:
        ...
        for neighbor in IS_ROUTE_STATE_SAFE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("IsRouteStateSafe", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_get_route_progress(self) -> None:
        ...
        for neighbor in GET_ROUTE_PROGRESS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("GetRouteProgress", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_get_telegraphs_triggered(self) -> None:
        ...
        for neighbor in GET_TELEGRAPHS_TRIGGERED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("GetTelegraphsTriggered", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_uprop_fields_or_delegates(self) -> None:
        ...
        for token in LEFTOVER_UPROPERTY_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("UPROPERTY", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("UPROPERTY", locked_only)
        self.assertNotIn("bAutoAdvance", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("ObservedPhase", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("OnAttackTelegraphChanged", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("OnAttackCommitted", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("MinHeightFromOriginCm", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_leftover_boss_phase_enum(self) -> None:
        ...
        for token in LEFTOVER_BOSS_PHASE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("ESkyguardBossPhase", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("ESkyguardBossPhase", locked_only)
        self.assertNotIn("SkyguardBossTypes.h", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn(
            "test_boss_phase_enum_contract.py",
            GET_EFFECTIVE_SPEED_MULTIPLIER,
        )
    ...
    def test_contract_does_not_relock_leftover_boss_defaults(self) -> None:
        ...
        for token in leftover_weak_point_tokens():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FSkyguardBossDefinition", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FSkyguardBossWeakPoint", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn(
            "test_boss_definition_defaults_contract.py",
            GET_EFFECTIVE_SPEED_MULTIPLIER,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("Unconfigured", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("Warming", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("Launched", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_EFFECTIVE_SPEED_MULTIPLIER,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FSkyguardBriefingCard", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FSkyguardBriefingCard", locked_only)
        self.assertNotIn("FSkyguardBriefingRadioRow", locked_only)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("GetBriefingText", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("GetRadioChatter", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FillResultCombatStats", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("ASkyguardGunner", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FillAndFinalize", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("FillAndFail", GET_EFFECTIVE_SPEED_MULTIPLIER)
        self.assertNotIn("ApplyHydraForClusters", GET_EFFECTIVE_SPEED_MULTIPLIER)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_EFFECTIVE_SPEED_MULTIPLIER}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_EFFECTIVE_SPEED_MULTIPLIER),
            GET_EFFECTIVE_SPEED_MULTIPLIER,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder GetEffectiveSpeedMultiplier contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_EFFECTIVE_SPEED_MULTIPLIER.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "pathfinder GetEffectiveSpeedMultiplier contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, GET_EFFECTIVE_SPEED_MULTIPLIER.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_EFFECTIVE_SPEED_MULTIPLIER)

    def test_contract_is_get_effective_speed_multiplier_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(GET_EFFECTIVE_SPEED_MULTIPLIER.startswith("float "))
        self.assertTrue(GET_EFFECTIVE_SPEED_MULTIPLIER.endswith(";"))
        self.assertTrue(GET_EFFECTIVE_SPEED_MULTIPLIER.endswith(" const;"))
        self.assertIn(UFUNCTION_ENCOUNTER, section)
