...
# INDEX_NONE or lock AdvanceLightningWindow
# construction in the .cpp. This is leftover-safe
# Mission05 Storm AdvanceLightningWindow METHOD on
# ASkyguardMission05IntegrationDirector. It is
# NOT leftover storm-runtime-defaults #89c9
# (struct defaults, not this method), NOT leftover
...
# #ec8b, and NOT leftover GetStormRuntime.
# Distinct from leftover Mission05 StartNextWave
# #668, leftover Mission03 StartNextWave, leftover
# Mission04 StartNextWave #649, leftover Mission05
# InitializePlayableMission #666, leftover
# Mission05 ConfigureMissionDefinition #665, and
# leftover Mission04 StartSearchlightWindow #652
# (similar window method, different class). Not
# leftover Briefing / AudioDirector / Root /
...
# NotifyThreatDestroyed / StartNextWave /
# TriggerLightningWindow / AdvanceTurbulence /
...
# Mission05 StartNextWave #668, leftover
...
# leftover Mission03 Waves StartNextWave, not
# leftover Mission04 Waves StartNextWave #649,
# not leftover Mission05 Waves StartNextWave
# #668, not leftover GetStormRuntime, not leftover
# storm-runtime-defaults #89c9.
# Stay off leftover drafts #56–#64, leftover Harbor
# #6/#8/#9, leftover theater-kit #59, leftover
# isolated-test drafts #107–#668, leftover Apache
...
# leftover theater-kit #59, leftover #107–#668, plus
# leftover Mission03 StartNextWave, leftover
# Mission04 StartNextWave #649, leftover Mission05
# StartNextWave #668, leftover Mission05
# InitializePlayableMission #666, leftover
...
# AdvanceLightningWindow method declaration
...
# Mission05 StartNextWave #668, leftover Mission05
...
SIBLING_TRIGGER_LIGHTNING_NOT_LOCKED = (
    "TriggerLightningWindow",
    TRIGGER_LIGHTNING_WINDOW,
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission05IntegrationDirector::AdvanceLightningWindow",
    "SkyguardMission05IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission05AdvanceLightningWindowDeclContractTests(unittest.TestCase):
    def test_mission05_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ADVANCE_LIGHTNING_WINDOW),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ADVANCE_LIGHTNING_WINDOW)
        self.assertIn("AdvanceLightningWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_missing_advance_lightning_window_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission05IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ADVANCE_LIGHTNING_WINDOW)
        self.assertIn("AdvanceLightningWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ADVANCE_LIGHTNING_WINDOW)
        self.assertIn("AdvanceLightningWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("BlueprintCallable", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("Category", ADVANCE_LIGHTNING_WINDOW)
        ...
        self.assertNotIn("Mission04", UFUNCTION_STORM)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ADVANCE_LIGHTNING_WINDOW)
        self.assertIn("AdvanceLightningWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid AdvanceLightningWindow(float DeltaSeconds);\n"
        as_const = "\tbool AdvanceLightningWindow(float DeltaSeconds) const;\n"
        with_arg = "\tbool AdvanceLightningWindow();\n"
        as_int = "\tbool AdvanceLightningWindow(int32 Amount = 1);\n"
        renamed = "\tbool AdvanceLightning();\n"
        short_name = "\tbool LightningWindow(float DeltaSeconds);\n"
        leftover_start_next = f"\t{START_NEXT_WAVE}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ADVANCE_LIGHTNING_WINDOW)
            self.assertIn("AdvanceLightningWindow", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_advance_lightning_window_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ADVANCE_LIGHTNING_WINDOW),
            ADVANCE_LIGHTNING_WINDOW,
        )
        self.assertTrue(has_declaration(section, ADVANCE_LIGHTNING_WINDOW))
        self.assertEqual(
            declaration_count(section, ADVANCE_LIGHTNING_WINDOW),
            1,
        )
        self.assertTrue(
            ADVANCE_LIGHTNING_WINDOW.startswith("bool "),
            ADVANCE_LIGHTNING_WINDOW,
        )
        self.assertTrue(
            ADVANCE_LIGHTNING_WINDOW.endswith("(float DeltaSeconds);"),
            ADVANCE_LIGHTNING_WINDOW,
        )
        self.assertIn("AdvanceLightningWindow", ADVANCE_LIGHTNING_WINDOW)
        self.assertIn("DeltaSeconds", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("StartNextWave", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("TriggerLightningWindow", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("TArray<", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("TObjectPtr", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("TSoftObjectPtr", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("INDEX_NONE", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("UFUNCTION", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("{", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("}", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("return ", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("const;", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("Root", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("Briefing", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("AudioDirector", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("RadioChatter", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("SortiePresentation", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("CampaignDefinition", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("MissionDefinition", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("Readiness", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("bAutoInitialize", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("bAllowBoundedActorSpawning", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("bAutoLaunchAfterBriefing", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("PathfinderSpawnLocation", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetAircraft", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("BindRuntimeActors", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("HandleDroneCityImpact", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("InitializePlayableMission", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("NotifyThreatDestroyed", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("AdvanceConvoyByDistance", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("NotifyConvoyDamage", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetWaveState", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetConvoyRouteState", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetDayBeatKit", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetNightBeatKit", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("StartSearchlightWindow", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("AdvanceSearchlightTrack", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("NotifySubstationDamage", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetRemainingThreatsInWave", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetSearchlightRuntime", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetSubstationIntegrity", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ConvoyRuntimeAnchor", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("RoadHunterSpawnLocation", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ConfigureMissionDefinition", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("NotifyObjectiveProgress", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("NotifyProtectedAssetFailed", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("SynchronizeRuntimeState", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("IsCorePlayableReady", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetReadiness", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetObjectiveRuntime", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetGunner", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetPathfinder", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetMissionId", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ValidateMissionContract", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ConfigureFromMission", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("AdvanceBriefing", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("SetAssetsReady", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("AcknowledgeAndLaunch", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("CanLaunch", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetElapsedSeconds", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetBriefingState", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetMinimumWarmupSeconds", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetBriefingText", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetRadioChatter", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetPresentation", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("GetMissionTitle", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("AcknowledgeBriefing", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("LaunchSortie", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("HullCollider", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("OpticalTracker", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("WeaponServo", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("CountermeasurePod", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("MinHeightFromOriginCm", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("RadarNode", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ESkyguardMission02WaveState", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("HarborIndustrial", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("MaxIntegrity", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("CurrentIntegrity", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("FillAndFinalize", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("FillAndFail", ADVANCE_LIGHTNING_WINDOW)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tAdvanceLightningWindow(float DeltaSeconds);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_STORM} {ADVANCE_LIGHTNING_WINDOW}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ADVANCE_LIGHTNING_WINDOW}\n}}\n"
        self.assertTrue(has_declaration(one_line, ADVANCE_LIGHTNING_WINDOW))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADVANCE_LIGHTNING_WINDOW),
            section,
    ...
    def test_environment_category_does_not_satisfy_storm(self) -> None:
        ...
        self.assertNotIn(leftover_waves, UFUNCTION_STORM)
        self.assertNotIn(leftover_protection, UFUNCTION_STORM)
        self.assertNotIn(leftover_integration, UFUNCTION_STORM)
        self.assertNotIn("Mission01", UFUNCTION_STORM)
        self.assertNotIn("Mission03", UFUNCTION_STORM)
        self.assertNotIn("Waves", UFUNCTION_STORM)
        self.assertNotIn("Protection", UFUNCTION_STORM)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{ADVANCE_LIGHTNING_WINDOW}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ADVANCE_LIGHTNING_WINDOW)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_advance_lightning_window_cpp_body(self) -> None:
        locked_only = f"{ADVANCE_LIGHTNING_WINDOW}\n"
        self.assertNotIn("{", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("}", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("return ", ADVANCE_LIGHTNING_WINDOW)
        ...
        self.assertNotIn("return false", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("return true", ADVANCE_LIGHTNING_WINDOW)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{ADVANCE_LIGHTNING_WINDOW}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_LIGHTNING_WINDOW)
        self.assertIn("AdvanceLightningWindow", ADVANCE_LIGHTNING_WINDOW)
        self.assertTrue(ADVANCE_LIGHTNING_WINDOW.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, ADVANCE_LIGHTNING_WINDOW)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("CurrentIntegrity", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("SkyguardApacheAircraft.h", ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ADVANCE_LIGHTNING_WINDOW}\n"
        self.assertEqual(
            require_declaration(locked_only, ADVANCE_LIGHTNING_WINDOW),
            ADVANCE_LIGHTNING_WINDOW,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", section)
        self.assertNotIn("ASkyguardTempestBoss", ADVANCE_LIGHTNING_WINDOW)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 AdvanceLightningWindow method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ADVANCE_LIGHTNING_WINDOW}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ADVANCE_LIGHTNING_WINDOW.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ADVANCE_LIGHTNING_WINDOW.lower(),
                "mission05 AdvanceLightningWindow contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ADVANCE_LIGHTNING_WINDOW)

    def test_contract_is_advance_lightning_window_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(ADVANCE_LIGHTNING_WINDOW.startswith("bool "))
        self.assertIn("AdvanceLightningWindow", ADVANCE_LIGHTNING_WINDOW)
        self.assertTrue(ADVANCE_LIGHTNING_WINDOW.endswith(";"))
        self.assertIn(UFUNCTION_STORM, section)

    def test_sibling_director_fields_do_not_satisfy_advance_lightning_window(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, ADVANCE_LIGHTNING_WINDOW)
            )
            self.assertNotEqual(ADVANCE_LIGHTNING_WINDOW, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("AdvanceLightningWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, ADVANCE_LIGHTNING_WINDOW))
        locked_only = f"{ADVANCE_LIGHTNING_WINDOW}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool AdvanceLightningWindow(float DeltaSeconds) { return false; }\n"
            "};\n"
        )
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", ADVANCE_LIGHTNING_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_STORM)
    ...
    def test_sibling_trigger_lightning_window_does_not_satisfy(self) -> None:
        ...
        locked_only = f"{ADVANCE_LIGHTNING_WINDOW}\n"
        for token in SIBLING_TRIGGER_LIGHTNING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_LIGHTNING_WINDOW)
        self.assertIn(
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission05_advance_lightning_window_decl_contract.py"
            )
        )
