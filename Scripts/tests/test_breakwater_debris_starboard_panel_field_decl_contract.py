...
HEADER_PATH = "Source/Skyguard52/SkyguardBreakwaterBoss.h"
CLASS_NAME = "ASkyguardBreakwaterBoss"
# Field-declaration presence only. Do not invent
# INDEX_NONE or lock DebrisStarboardPanel construction
# in the .cpp. This is a FIELD contract on
# ASkyguardBreakwaterBoss, not leftover Breakwater
# sibling fields PortLatch / StarboardLatch / DecoyPods /
# Engine / ElevatorLinkage / DebrisEngine /
# DebrisPortPanel and not
# leftover emergency-finish named surfaces
# (split tokens only). Do not lock leftover
# ESkyguardMission02WaveState (Harbor leftover
# #6/#8/#9). Stay off the constructor and
# HandleWeakPointDestroyed. Keep Breakwater
# Engine distinct from leftover RadarGhost Engine
# and leftover Pathfinder Engine. Keep Breakwater
# DebrisStarboardPanel distinct from leftover Tempest
# DebrisStarboardPanel (PR leftover, different class
# ASkyguardTempestBoss). Weak-point PortLatch must
# not satisfy this mesh field. This is not a leftover
# USkyguardBossWeakPointComponent member
# contract. origin/main is a one-line field
# (`TObjectPtr<UStaticMeshComponent>
# DebrisStarboardPanel;`); accept that form and other
...
# section of ASkyguardBreakwaterBoss only.
# Do not lock leftover sibling Iron Rain
# fields DispenserCenter,
...
# isolated-test drafts #107–#544 including
...
# Harbor-adjacent.
# Breakwater sibling weak-point field. Not the
# primary lock. PortLatch / DecoyPods must not
# satisfy this destruction-mesh field.
DECOY_PODS_FIELD = (
...
# Breakwater sibling fields. Not the primary lock.
# Breakwater Engine stays distinct from leftover
# RadarGhost Engine and leftover Pathfinder Engine
# even when the declaration text matches.
# Primary lock is Breakwater DebrisStarboardPanel.
# Leftover Tempest DebrisStarboardPanel uses the same
# declaration text on ASkyguardTempestBoss.
PORT_LATCH_FIELD = (
...
# Primary lock. Same declaration text as leftover
# Tempest DebrisStarboardPanel; class ASkyguardBreakwaterBoss
# keeps this isolated.
...
# leftover #107–#544, plus
# SkyguardBreakwaterBoss production files.
# This lane only adds an isolated Python
# DebrisStarboardPanel field declaration contract on
# ASkyguardBreakwaterBoss. Stay off leftover
# sibling Breakwater fields including leftover
# DecoyPods, leftover drafted
# emergency-finish methods, leftover RadarGhost
# fields including leftover SignatureModulator,
# leftover drafted RadarGhost methods, leftover Iron Rain
# sibling fields, leftover Iron Rain
# public methods, leftover Tempest ControlServo
...
# leftover isolated-test drafts #107–#544,
# leftover searchlight-track-runtime-defaults
...
LOCKED = {
    "SkyguardBreakwaterBoss.h",
    "SkyguardBreakwaterBoss.cpp",
...
# isolated Breakwater field contracts, leftover
# RadarGhost field contracts including leftover
# SignatureModulator, leftover RadarGhost method
# contracts, leftover Iron Rain / Tempest /
# Pathfinder / BlackKite / LastFlight /
# LifelineHunter / BossDrone isolated contracts
# stay unlocked for THIS test's primary lock. Leftover
...
# Primary Breakwater lock. Leftover Tempest uses
# the same text on ASkyguardTempestBoss only.
...
LEFTOVER_BREAKWATER_DEBRIS_PORT_PANEL_NOT_LOCKED = (
    "test_breakwater_debris_port_panel_field_decl_contract.py",
    "DebrisPortPanel",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBreakwaterBoss::DebrisStarboardPanel",
    "SkyguardBreakwaterBoss.cpp",
    "SetExposed",
...
class BreakwaterDebrisStarboardPanelFieldDeclContractTests(unittest.TestCase):
    def test_breakwater_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            "class SKYGUARD52_API AOtherBreakwaterBoss "
            ": public AActor\n"
            "{\n"
            "public:\n"
            f"\t{DEBRIS_STARBOARD_PANEL_FIELD}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_missing_debris_starboard_panel_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardBreakwaterBoss();\n"
            f"\t{leftover_arm_emergency_finish()}\n"
            f"\t{leftover_is_emergency_finish_armed()}\n"
            f"\t{PORT_LATCH_FIELD}\n"
            f"\t{STARBOARD_LATCH_FIELD}\n"
            f"\t{BREAKWATER_ENGINE_FIELD}\n"
            f"\t{ELEVATOR_LINKAGE_FIELD}\n"
            f"\t{DECOY_PODS_FIELD}\n"
            f"\t{BREAKWATER_DEBRIS_ENGINE_FIELD}\n"
            f"\t{BREAKWATER_DEBRIS_PORT_PANEL_FIELD}\n"
            f"\t{SIGNATURE_MODULATOR_FIELD}\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("Category", DEBRIS_STARBOARD_PANEL_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisStarboardPanel;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisStarboardPanel;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DebrisStarboardPanel;\n"
        ...
        leftover_debris_port = f"\t{DECOY_PODS_FIELD}\n"
        leftover_debris_starboard = f"\t{DEBRIS_PORT_PANEL_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_STARBOARD_PANEL_FIELD)
            self.assertIn("DebrisStarboardPanel", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_starboard_panel_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD),
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_STARBOARD_PANEL_FIELD), 1)
        self.assertTrue(
            DEBRIS_STARBOARD_PANEL_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
        self.assertTrue(DEBRIS_STARBOARD_PANEL_FIELD.endswith(";"), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("TArray<", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("=", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("{", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("}", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("return ", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisCenter", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("RadarNode", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_STARBOARD_PANEL_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_DESTRUCTION} {DEBRIS_STARBOARD_PANEL_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DEBRIS_STARBOARD_PANEL_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DEBRIS_STARBOARD_PANEL_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_STARBOARD_PANEL_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD),
            section,
    ...
    def test_assigned_debris_starboard_panel_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisStarboardPanel = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_STARBOARD_PANEL_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DebrisStarboardPanel", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_STARBOARD_PANEL_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_STARBOARD_PANEL_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_DESTRUCTION, section)
        self.assertTrue(has_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_STARBOARD_PANEL_FIELD.startswith("UFUNCTION"),
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_STARBOARD_PANEL_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_STARBOARD_PANEL_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_starboard_panel_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        self.assertNotIn("{", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("}", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("return ", DEBRIS_STARBOARD_PANEL_FIELD)
        ...
        self.assertNotIn(
            "ASkyguardBreakwaterBoss::DecoyPods",
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("return true", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_STARBOARD_PANEL_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisIntakePanel", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_starboard_panel(self) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        for token in LEFTOVER_DEBRIS_STARBOARD_PANEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", locked_only)
        self.assertNotIn("ASkyguardTempestBoss", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardBreakwaterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardTempestBoss")
        self.assertIn("DebrisStarboardPanel", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn(
            "Scripts/tests/test_tempest_debris_starboard_panel_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_contract_does_not_relock_leftover_debris_port_panel(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DispenserCenter", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DispenserStarboard", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetManeuver", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("StarboardLatch", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DecoyPods", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ElevatorLinkage", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", DEBRIS_STARBOARD_PANEL_FIELD)

    def test_breakwater_engine_is_not_radar_ghost_or_pathfinder_engine(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        self.assertNotEqual(BREAKWATER_ENGINE_FIELD, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertEqual(BREAKWATER_ENGINE_FIELD, ENGINE_FIELD)
        ...
        self.assertNotIn("Engine;", DEBRIS_STARBOARD_PANEL_FIELD)

    def test_breakwater_debris_starboard_panel_is_not_tempest_debris_starboard_panel(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        self.assertEqual(
        ...
        self.assertNotIn("ASkyguardTempestBoss", locked_only)
        self.assertNotIn("ASkyguardTempestBoss", DEBRIS_STARBOARD_PANEL_FIELD)
        for token in LEFTOVER_DECOY_PODS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        for token in LEFTOVER_DEBRIS_STARBOARD_PANEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), DEBRIS_STARBOARD_PANEL_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_sibling_radar_ghost_fields(self) -> None:
        ...
        for token in SIBLING_RADAR_GHOST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("RadarReceiver", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("CoolingDoor", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisPortEWPanel", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisStarboardEWPanel", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("DebrisCoolingDoor", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn("DebrisStarboardPanel", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("IsLightningExposed", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_STARBOARD_PANEL_FIELD)
        ...
        self.assertEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardBreakwaterBoss.h",
        )
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_STARBOARD_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_STARBOARD_PANEL_FIELD),
            DEBRIS_STARBOARD_PANEL_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, DEBRIS_STARBOARD_PANEL_FIELD), 1)
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", section)
        self.assertNotIn("ASkyguardBreakwaterBoss::DebrisStarboardPanel", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "breakwater DebrisStarboardPanel field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_STARBOARD_PANEL_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_STARBOARD_PANEL_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_STARBOARD_PANEL_FIELD.lower(),
                "breakwater DebrisStarboardPanel contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_STARBOARD_PANEL_FIELD)

    def test_contract_is_debris_starboard_panel_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DEBRIS_STARBOARD_PANEL_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_STARBOARD_PANEL_FIELD)
        self.assertIn(UPROPERTY_DESTRUCTION, section)
