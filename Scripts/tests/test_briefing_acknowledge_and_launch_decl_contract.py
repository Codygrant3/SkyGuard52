...
# or lock the AcknowledgeAndLaunch body in the .cpp.
...
# This lane only adds an isolated Python AcknowledgeAndLaunch
# declaration contract. Stay off ConfigureFromMission
# (sibling draft), SetAssetsReady (in-flight),
# AdvanceBriefing (in-flight), CanLaunch,
# GetBriefingState, GetElapsedSeconds,
# GetMinimumWarmupSeconds, GetBriefingText, and
# GetRadioChatter on this class. Stay off leftover
...
# leftover ConfigureFromMission, leftover
# SetAssetsReady, leftover AdvanceBriefing,
# leftover FindObjective #364, and leftover
# ValidateDefinition #365 stay sibling-only.
LOCKED_SCRIPTS = (
...
# AdvanceBriefing / CanLaunch / GetBriefingState /
...
# .cpp AcknowledgeAndLaunch body / invented INDEX_NONE
...
class BriefingAcknowledgeAndLaunchDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_AND_LAUNCH), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ACKNOWLEDGE_AND_LAUNCH)
        self.assertIn("AcknowledgeAndLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ACKNOWLEDGE_AND_LAUNCH))

    def test_missing_acknowledge_and_launch_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
            "\tbool ConfigureFromMission("
            "USkyguardMissionDefinition* Mission);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ACKNOWLEDGE_AND_LAUNCH)
        self.assertIn("AcknowledgeAndLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ACKNOWLEDGE_AND_LAUNCH)
        self.assertIn("AcknowledgeAndLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_AND_LAUNCH), section)
        self.assertNotIn("BlueprintPure", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("UFUNCTION", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("Category", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("BlueprintCallable", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ACKNOWLEDGE_AND_LAUNCH)
        self.assertIn("AcknowledgeAndLaunch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        wrong_return = "\tvoid AcknowledgeAndLaunch();\n"
        added_const = "\tbool AcknowledgeAndLaunch() const;\n"
        added_arg = "\tbool AcknowledgeAndLaunch(bool bForce);\n"
        int_return = "\tint32 AcknowledgeAndLaunch();\n"
        mission_arg = (
            "\tbool AcknowledgeAndLaunch("
            "USkyguardMissionDefinition* Mission);\n"
        )
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ACKNOWLEDGE_AND_LAUNCH)
            self.assertIn("AcknowledgeAndLaunch", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_acknowledge_and_launch_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_AND_LAUNCH))
        self.assertEqual(declaration_count(section, ACKNOWLEDGE_AND_LAUNCH), 1)
        self.assertTrue(
            ACKNOWLEDGE_AND_LAUNCH.startswith("bool "),
            ACKNOWLEDGE_AND_LAUNCH,
        )
        self.assertTrue(
            ACKNOWLEDGE_AND_LAUNCH.endswith(";"),
            ACKNOWLEDGE_AND_LAUNCH,
        )
        self.assertIn("AcknowledgeAndLaunch()", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("INDEX_NONE", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("{", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("}", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("return ", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn(" const", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, ACKNOWLEDGE_AND_LAUNCH),
                section,
            )
            self.assertEqual(
                require_declaration(section, ACKNOWLEDGE_AND_LAUNCH),
            ...
            self.assertEqual(
                declaration_count(section, ACKNOWLEDGE_AND_LAUNCH),
                1,
            )
        one_line = f"{{\npublic:\n\t{ACKNOWLEDGE_AND_LAUNCH}\n}}\n"
        self.assertTrue(has_declaration(one_line, ACKNOWLEDGE_AND_LAUNCH))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_AND_LAUNCH), section)
        self.assertEqual(
            require_declaration(section, ACKNOWLEDGE_AND_LAUNCH),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("return ", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("return false", ACKNOWLEDGE_AND_LAUNCH)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{ACKNOWLEDGE_AND_LAUNCH}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", ACKNOWLEDGE_AND_LAUNCH)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_acknowledge_and_launch_cpp_body(self) -> None:
        locked_only = f"{ACKNOWLEDGE_AND_LAUNCH}\n"
        self.assertNotIn("{", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("}", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("return ", ACKNOWLEDGE_AND_LAUNCH)
        ...
        self.assertNotIn("return false", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("return true", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("ConfigureFromMission", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_advance_briefing(self) -> None:
        ...
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("AdvanceBriefing", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_set_assets_ready(self) -> None:
        ...
        for neighbor in SET_ASSETS_READY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("SetAssetsReady", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_can_launch(self) -> None:
        ...
        for neighbor in CAN_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("CanLaunch", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_get_briefing_state(self) -> None:
        ...
        for neighbor in GET_BRIEFING_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("GetBriefingState", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_SECONDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("GetElapsedSeconds", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_get_minimum_warmup_seconds(self) -> None:
        ...
        for neighbor in GET_MINIMUM_WARMUP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("GetMinimumWarmupSeconds", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("GetBriefingText", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_get_radio_chatter(self) -> None:
        ...
        for neighbor in GET_RADIO_CHATTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("GetRadioChatter", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("Unconfigured", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("Warming", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("Launched", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            ACKNOWLEDGE_AND_LAUNCH,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("FSkyguardBriefingCard", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("FSkyguardBriefingRadioRow", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("FillResultCombatStats", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("ASkyguardGunner", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("FillAndFinalize", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("FillAndFail", ACKNOWLEDGE_AND_LAUNCH)
        self.assertNotIn("ApplyHydraForClusters", ACKNOWLEDGE_AND_LAUNCH)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ACKNOWLEDGE_AND_LAUNCH}\n"
        self.assertEqual(
            require_declaration(locked_only, ACKNOWLEDGE_AND_LAUNCH),
            ACKNOWLEDGE_AND_LAUNCH,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing AcknowledgeAndLaunch contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, ACKNOWLEDGE_AND_LAUNCH.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing AcknowledgeAndLaunch contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ACKNOWLEDGE_AND_LAUNCH)

    def test_contract_is_acknowledge_and_launch_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(ACKNOWLEDGE_AND_LAUNCH.startswith("bool "))
        self.assertTrue(ACKNOWLEDGE_AND_LAUNCH.endswith(";"))
        self.assertIn(UFUNCTION_BRIEFING, section)
