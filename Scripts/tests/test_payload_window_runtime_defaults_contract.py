...
# Apache CPG airfield-sortie payload timing. Not Yak. Target default
# is enough; the full ESkyguardAirfieldTarget set stays unlocked.
PUBLIC_FIELDS = (
    "bool bActive = false;",
...
# Readiness (bYakRuntimeReady), airfield-target runtime, wave state,
...
def payload_window_body(header: str) -> str:
...
class PayloadWindowRuntimeDefaultsContractTests(unittest.TestCase):
    ...
    def test_in_class_defaults_match_origin_main(self) -> None:
        ...
        self.assertEqual(defaults.get("bJammed"), "false")
        ...
        self.assertIn("bool bJammed = false;", body)
        self.assertNotIn("bActive = true", body)
        self.assertNotIn("bJammed = true", body)
    ...
    def test_contract_does_not_lock_readiness_target_runtime_or_wave(self) -> None:
        body = payload_window_body(origin_main_header())
        for name in TYPES_NOT_LOCKED:
            self.assertNotIn(name, body)
        self.assertNotIn("FSkyguardMission06IntegrationReadiness", body)
        self.assertNotIn("FSkyguardAirfieldTargetRuntime", body)
    ...
    def test_struct_bans_igla_yak_rifle(self) -> None:
        lowered = payload_window_body(origin_main_header()).lower()
