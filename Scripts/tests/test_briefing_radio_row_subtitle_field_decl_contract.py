# THIS IS leftover-safe FSkyguardBriefingRadioRow::Subtitle.
...
# THIS IS NOT leftover BriefingCard Body.
...
# meta=(MultiLine="true"), no Category=. Accept bare `FText Subtitle;`.
...
# BriefingCard Priority #1251 / CardId #1252 / Title #1253.
...
TARGET_WRONG_INIT = "FText Subtitle = FText();"
...
STOP_AFTER = "struct FSkyguardHowToFlyRow"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer when origin/main has `FText Subtitle;`.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    for match in re.finditer(r"UPROPERTY\(", compact):
        ...
        if depth != 0:
            continue
        after = compact[index:]
        if re.match(r"\s*FText\s+Subtitle\b", after):
...
class BriefingRadioRowSubtitleFieldDeclContractTests(unittest.TestCase):
    def test_briefing_radio_row_struct_exists(self) -> None:
        ...
        self.assertNotIn("FText Subtitle)", section)
    ...
    def test_subtitle_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("FText Subtitle"), LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing-radio-row Subtitle field contract contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_briefing_radio_row_subtitle_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertIn("Subtitle;", locked_only)
