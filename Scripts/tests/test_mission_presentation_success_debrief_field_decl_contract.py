# THIS IS leftover-safe FSkyguardMissionPresentation SuccessDebrief.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (MultiLine = "true"))
# then BARE `FText SuccessDebrief;` (no initializer).
...
# `FSkyguardBossWeakPointDefinition`, `FSkyguardWeatherProfile`,
# `FSkyguardObjectiveDefinition`, `FSkyguardObjectiveProgress`,
# `FSkyguardRouteDefinition`, `USkyguardMissionDefinition`
# (leftover presentation field decls #350-#364 including
# Presentation #359).
...
# fields.
# Sibling decls stay unclaimed: Briefing / RadioChatter /
# FailureDebrief / MinimumBriefingWarmupSeconds.
# THIS IS NOT leftover mission-presentation-defaults
# #186 / #157e. Analog leftover mission-presentation-defaults
# then isolated SuccessDebrief field decl.
# THIS IS NOT leftover USkyguardMissionDefinition::Presentation
# #359.
...
LEFTOVER_MISSION_DEF_CLASS = "USkyguardMissionDefinition"
LEFTOVER_BOSS_DEF = "FSkyguardBossDefinition"
LEFTOVER_WEAK_POINT_DEF = "FSkyguardBossWeakPointDefinition"
LEFTOVER_WEATHER_PROFILE = "FSkyguardWeatherProfile"
LEFTOVER_OBJECTIVE_DEF = "FSkyguardObjectiveDefinition"
LEFTOVER_OBJECTIVE_PROGRESS = "FSkyguardObjectiveProgress"
LEFTOVER_ROUTE_DEF = "FSkyguardRouteDefinition"
LEFTOVER_MISSION_DEBRIEF = "FSkyguardMissionDebrief"
TARGET = "FText SuccessDebrief;"
TARGET_WRONG_EQ_NONE = "FText SuccessDebrief = FText::GetEmpty();"
TARGET_WRONG_FALSE = "FText SuccessDebrief = false;"
TARGET_WRONG_TRUE = "FText SuccessDebrief = true;"
TARGET_WRONG_ZERO = "FText SuccessDebrief = 0.f;"
TARGET_WRONG_FLOAT = "float SuccessDebrief;"
TARGET_WRONG_NAME = "FName SuccessDebrief;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_CATEGORY = (
    'UPROPERTY(EditAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Campaign", meta = (MultiLine = "true"))'
)
LOCKED_DECL = TARGET
...
SIBLING_MINIMUM_WARMUP = "MinimumBriefingWarmupSeconds"
...
LEFTOVER_MISSION_DEFINITION_MISSION_ID = (
    "Scripts/tests/test_mission_definition_mission_id"
    "_decl_contract.py"
)
LEFTOVER_MISSION_DEFINITION_DISPLAY_NAME = (
    "Scripts/tests/test_mission_definition_display_name"
    "_decl_contract.py"
)
LEFTOVER_MISSION_DEFINITION_CAMPAIGN_ORDER = (
    "Scripts/tests/test_mission_definition_campaign_order"
    "_decl_contract.py"
)
LEFTOVER_MISSION_DEFINITION_MISSION_MAP = (
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_MINIMUM_WARMUP,
    )


def leftover_mission_definition_tokens() -> tuple[str, ...]:
    return (
        LEFTOVER_MISSION_DEF_CLASS,
        LEFTOVER_BOSS_DEF,
        LEFTOVER_WEAK_POINT_DEF,
        LEFTOVER_WEATHER_PROFILE,
        LEFTOVER_OBJECTIVE_DEF,
        LEFTOVER_OBJECTIVE_PROGRESS,
        LEFTOVER_ROUTE_DEF,
        LEFTOVER_MISSION_DEBRIEF,
        STOP_BEFORE_SCORE_RULES,
    )


def leftover_presentation_field_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_MISSION_PRESENTATION_DEFAULTS,
        LEFTOVER_MISSION_PRESENTATION_DEFAULTS_PY,
        LEFTOVER_MISSION_PRESENTATION_DEFAULTS_TESTS,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FText SuccessDebrief;`.
    # Do not accept `= FText::GetEmpty()` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` / leftover float Health / FName when
    # origin/main has a bare FText. Do not accept sibling
    # Briefing / RadioChatter / FailureDebrief fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FText\s+SuccessDebrief\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SuccessDebrief\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+SuccessDebrief\b",
        compact,
    ):
        return False
    if re.search(
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        "class SKYGUARD52_API ASkyguardCampaignTheaterKit",
    )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FText\s+SuccessDebrief\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FText SuccessDebrief is missing from "
...
class MissionPresentationSuccessDebriefFieldDeclContractTests(unittest.TestCase):
    def test_mission_presentation_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_MISSION_DEF_CLASS)
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_MISSION_DEBRIEF)
        body = struct_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "SuccessDebrief"), section)
        ...
        self.assertNotIn(LEFTOVER_MISSION_DEF_CLASS, section)
        self.assertNotIn(LEFTOVER_MISSION_DEF_CLASS, body)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        leftover_def = (
            f"class {LEFTOVER_MISSION_DEF_CLASS}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_EDIT_READONLY_MULTILINE}\n"
            f"\t{LOCKED_DECL}\n"
        ...
        self.assertIn("missing", str(raised.exception).lower())
        debrief = (
            f"struct {LEFTOVER_MISSION_DEBRIEF}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_EDIT_READONLY_MULTILINE}\n"
            f"\t{LOCKED_DECL}\n"
    ...
    def test_leftover_definition_or_score_rules_do_not_satisfy(self) -> None:
        mixed = (
        ...
        self.assertFalse(has_identifier(section, "SuccessDebrief"), section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(LEFTOVER_MISSION_DEF_CLASS, section)
        self.assertNotIn("Presentation", section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SuccessDebrief", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_success_debrief_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('MultiLine = "true"', UPROPERTY_EDIT_READONLY_MULTILINE)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FText SuccessDebrief;", compact_origin)
        self.assertNotIn("SuccessDebrief = FText::GetEmpty()", compact_origin)
        self.assertNotIn("SuccessDebrief = false", compact_origin)
        self.assertNotIn("SuccessDebrief = true", compact_origin)
        self.assertNotIn("SuccessDebrief = 0.f", compact_origin)
        self.assertNotIn("SuccessDebrief = 160.f", compact_origin)

    def test_success_debrief_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText SuccessDebrief"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFText {SIBLING_BRIEFING};\n",
            f"\tFText {SIBLING_FAILURE_DEBRIEF};\n",
            "\tFText SuccessDebriefs;\n",
            "\tint32 SuccessDebrief;\n",
            "\tbool SuccessDebrief;\n",
            "\tfloat SuccessDebrief = " + forty + ";\n",
            "\tfloat SuccessDebrief = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_multiline_or_added_category_fails_closed(
        self,
    ) -> None:
        no_multiline = (
            f"struct {STRUCT_NAME}\n"
        ...
        no_edit = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(BlueprintReadOnly, meta = (MultiLine = "
            '"true"))\n'
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        ...
        self.assertIn('Category="Skyguard|Campaign"', category_specs)
        self.assertNotIn("Category", origin)
        visible = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_does_not_contract_sibling_presentation_fields(self) -> None:
        ...
        self.assertNotIn("Presentation", LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_leftover_structs_or_mission_definition(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(f"struct {LEFTOVER_BOSS_DEF}", header)
        self.assertNotIn(f"struct {LEFTOVER_BOSS_DEF}", section)
        self.assertNotIn(f"struct {LEFTOVER_BOSS_DEF}", leaked)
        self.assertIn(f"struct {LEFTOVER_WEAK_POINT_DEF}", header)
        self.assertNotIn(f"struct {LEFTOVER_WEAK_POINT_DEF}", section)
        self.assertIn(f"struct {LEFTOVER_WEATHER_PROFILE}", header)
        self.assertNotIn(f"struct {LEFTOVER_WEATHER_PROFILE}", section)
        self.assertIn(f"struct {LEFTOVER_OBJECTIVE_DEF}", header)
        self.assertNotIn(f"struct {LEFTOVER_OBJECTIVE_DEF}", section)
        self.assertIn(f"struct {LEFTOVER_ROUTE_DEF}", header)
        self.assertNotIn(f"struct {LEFTOVER_ROUTE_DEF}", section)
        self.assertIn(f"struct {LEFTOVER_OBJECTIVE_PROGRESS}", header)
        self.assertNotIn(f"struct {LEFTOVER_OBJECTIVE_PROGRESS}", section)
        self.assertIn(f"struct {LEFTOVER_MISSION_DEBRIEF}", header)
        self.assertNotIn(f"struct {LEFTOVER_MISSION_DEBRIEF}", section)
        self.assertNotIn(LEFTOVER_MISSION_DEF_CLASS, header)
        self.assertNotIn(LEFTOVER_MISSION_DEF_CLASS, section)
        self.assertNotIn(LEFTOVER_MISSION_DEF_CLASS, leaked)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, header)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn("ASkyguardCampaignTheaterKit", header)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Campaign"', UPROPERTY_EDIT_READONLY_MULTILINE)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READONLY_MULTILINE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-presentation SuccessDebrief field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "success_debrief_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            "Scripts/tests/test_briefing_fail_closed.py",
            "Scripts/tests/test_sortie_presentation_fail_closed.py",
        ) + leftover_presentation_field_scripts()
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_MISSION_DEF_CLASS, locked_only)
        ...
        self.assertNotIn("Presentation", locked_only)
        for helper in namespace_helper_tokens():
