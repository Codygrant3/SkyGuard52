# THIS IS leftover-safe FSkyguardCpgContactMark LockAlpha.
...
# `float LockAlpha = 0.f;` with that in-struct initializer.
# Fail-closed if LockAlpha is missing or renamed, if the
...
# (split-token), or is not `= 0.f`.
# LockAlpha here is NOT leftover HUD LockProgress #1512
# (`float LockProgress = 0.f`), leftover HUD LockLine
# #1505, leftover HUD LockPhase #1508, leftover HUD
# HeadingDegrees #1513 (`float HeadingDegrees = 0.f`),
# leftover ContactMark WorldLocation #1516, leftover
# ContactMark Label #1519, leftover ContactMark bLocked
# #1518, or leftover ContactMark bSeeking #1517.
...
# #1501-#1515. Keep leftover-safe WeaponLine #1502,
# LockLine #1505, LockPhase #1508, LockProgress #1512,
# HeadingDegrees #1513, and siblings in LOCKED_SCRIPTS.
# THIS IS NOT leftover ContactMark WorldLocation #1516.
# Keep leftover-safe WorldLocation, bSeeking #1517,
# bLocked #1518, and Label #1519 in LOCKED_SCRIPTS.
...
# struct. ContactMark parse-window retarget is
# leftover-safe #1516 WorldLocation (FVector,
# `= FVector::ZeroVector`, parses ContactMark only,
# stops before SkyguardCpgWeaponLabel). RETARGET:
# type is float, identifier is LockAlpha, initializer
# is `= 0.f`, parse STRUCT FSkyguardCpgContactMark,
# stop before SkyguardCpgWeaponLabel.
# Do NOT copy leftover HUD snapshot parse window
# (#1501-#1515). Do NOT copy leftover RoutePoint /
# MissionObjectiveAnchor / MissionLandmarkAnchor
# WorldLocation parse windows.
...
TARGET = "float LockAlpha = 0.f;"
TARGET_WRONG_BARE = "float LockAlpha;"
TARGET_WRONG_EQ_EMPTY = 'float LockAlpha = TEXT("");'
TARGET_WRONG_EQ_NONE = "float LockAlpha = NAME_None;"
TARGET_WRONG_TRUE = "float LockAlpha = true;"
TARGET_WRONG_ZERO = "float LockAlpha = 0;"
TARGET_WRONG_ONE = "float LockAlpha = 1.f;"
TARGET_WRONG_ZERO_F = "bool LockAlpha = 0.f;"
TARGET_WRONG_FLOAT = "float LockAlpha = 1.0f;"
TARGET_WRONG_HEALTH = "float LockAlpha = 160.f;"
TARGET_WRONG_INT = "int32 LockAlpha;"
TARGET_WRONG_INT_FALSE = "int32 LockAlpha = 0;"
TARGET_WRONG_FNAME = "FName LockAlpha;"
TARGET_WRONG_FSTRING = "FString LockAlpha;"
TARGET_WRONG_TCHAR = 'const TCHAR* LockAlpha = TEXT("");'
TARGET_WRONG_FVECTOR = "FVector LockAlpha = FVector::ZeroVector;"
TARGET_WRONG_WORLD = "FVector WorldLocation = FVector::ZeroVector;"
TARGET_WRONG_LABEL = "FString Label;"
TARGET_WRONG_LOCKED = "bool bLocked = false;"
...
LEFTOVER_HUD_LOCK_LINE_DECL = "FString LockLine;"
LEFTOVER_HUD_LOCK_PHASE_DECL = (
    "ESkyguardGuidedLockPhase LockPhase = "
    "ESkyguardGuidedLockPhase::Search;"
)
LEFTOVER_HUD_LOCK_PROGRESS_DECL = "float LockProgress = 0.f;"
...
LEFTOVER_CONTACT_WORLD_LOCATION_SCRIPT = (
    "Scripts/tests/test_cpg_contact_mark_world_location"
    "_field_decl_contract.py"
)
LEFTOVER_CONTACT_SEEKING_SCRIPT = (
    "Scripts/tests/test_cpg_contact_mark_seeking"
    "_field_decl_contract.py"
)
LEFTOVER_CONTACT_LOCKED_SCRIPT = (
    "Scripts/tests/test_cpg_contact_mark_locked"
    "_field_decl_contract.py"
)
LEFTOVER_CONTACT_LABEL_SCRIPT = (
    "Scripts/tests/test_cpg_contact_mark_label"
    "_field_decl_contract.py"
)
...
def leftover_contact_mark_isolated_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_CONTACT_WORLD_LOCATION_SCRIPT,
        LEFTOVER_CONTACT_SEEKING_SCRIPT,
        LEFTOVER_CONTACT_LOCKED_SCRIPT,
...
LOCKED_SCRIPTS = (
    ...
    + leftover_contact_mark_isolated_scripts()
    + (
        LEFTOVER_ANALOG_DEBRIEF_DEFAULTS,
        LEFTOVER_LOADOUT_FLARE,
        CLONE_THEATER_WEATHER_IDENTITY,
...
LOCK_ALPHA_LOCKED_RE = re.compile(
    r"float\s+LockAlpha\s*=\s*0\.f\s*;"
)
LOCK_ALPHA_BARE_RE = re.compile(r"float\s+LockAlpha\s*;")
LOCK_ALPHA_INIT_RE = re.compile(
    r"float\s+LockAlpha\s*=\s*([^;]+);"
)
...
def leftover_hud_snapshot_decls() -> tuple[str, ...]:
    return (
        ...
        LEFTOVER_HUD_LOCK_LINE_DECL,
        LEFTOVER_HUD_LOCK_PHASE_DECL,
        LEFTOVER_HUD_LOCK_PROGRESS_DECL,
    )
...
def leftover_hud_lock_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_HUD_LOCK_LINE,
        LEFTOVER_HUD_LOCK_PHASE,
        LEFTOVER_HUD_LOCK_PROGRESS,
...
def lock_alpha_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*float\s+LockAlpha\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float LockAlpha = 0.f;`.
    # Do not accept a missing initializer, Harbor 40/80,
    # leftover sibling WorldLocation / Label / bLocked /
    # bSeeking, leftover HUD snapshot LockLine /
    # LockPhase / LockProgress / WeaponLine /
    # HeadingDegrees, leftover debrief MissionTitle, or
    ...
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"bool\s+LockAlpha\b", compact):
        return False
    if re.search(r"FString\s+LockAlpha\b", compact):
        return False
    if re.search(r"FVector\s+LockAlpha\b", compact):
        return False
    if re.search(r"int32\s+LockAlpha\b", compact):
        return False
    if re.search(r"FName\s+LockAlpha\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*LockAlpha\b", compact):
        return False
    if LOCK_ALPHA_BARE_RE.search(compact) and (
        LOCK_ALPHA_LOCKED_RE.search(compact) is None
    ):
        return False
    init_match = LOCK_ALPHA_INIT_RE.search(compact)
    if init_match is None:
        return False
    if init_match.group(1).strip() != LOCKED_INITIALIZER:
        return False
    if LOCK_ALPHA_LOCKED_RE.search(compact) is None:
        return False
    if lock_alpha_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float LockAlpha\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LockAlpha is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*float LockAlpha\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on float LockAlpha is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "LockAlpha"):
        raise AssertionError(
            "UPROPERTY clone landed on LockAlpha; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "LockAlpha"):
            raise AssertionError(
                f"{token} clone landed on LockAlpha; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgContactMarkLockAlphaFieldDeclContractTests(unittest.TestCase):
    def test_cpg_contact_mark_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockAlpha"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "float LockAlpha = 0.f;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("float LockAlpha"))
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCKED)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("LockAlpha", str(raised.exception))
    ...
    def test_leftover_world_location_structs_do_not_satisfy(self) -> None:
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CONTACT_WORLD_LOCATION_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CONTACT_SEEKING_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CONTACT_LOCKED_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CONTACT_LABEL_SCRIPT)
    ...
    def test_hud_snapshot_before_contact_mark_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LockAlpha"), section)
        self.assertNotIn(STOP_BEFORE_HUD_SNAPSHOT, section)
        self.assertNotIn(LEFTOVER_HUD_SNAPSHOT, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_lock_alpha_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WORLD}\n"
            f"\t{TARGET_WRONG_LABEL}\n"
            f"\t{TARGET_WRONG_LOCKED}\n"
    ...
    def test_sibling_contact_mark_fields_do_not_satisfy_lock_alpha(self) -> None:
        leftovers = leftover_sibling_contact_mark_decls()
    ...
    def test_leftover_hud_lock_fields_do_not_satisfy(self) -> None:
        ...
        scripts = leftover_hud_lock_scripts()
        for leftover_decl, script in zip(leftovers, scripts):
            leftover = (
                f"struct {STRUCT_NAME}\n"
                "{\n"
                f"\t{leftover_decl}\n"
            ...
            self.assertNotEqual(THIS_SCRIPT, script)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_LINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_PHASE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_PROGRESS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HEADING)
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float LockAlpha = 0.f;", compact_origin)
        self.assertNotIn("LockAlpha = TEXT", compact_origin)
        self.assertNotIn("LockAlpha = NAME_None", compact_origin)
        self.assertNotIn("LockAlpha = true", compact_origin)
        self.assertNotIn("LockAlpha = false", compact_origin)
        self.assertNotIn("LockAlpha = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("LockAlpha = " + forty, compact_origin)
        self.assertNotIn("LockAlpha = " + eighty, compact_origin)

    def test_lock_alpha_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(LOCKED_DECL.startswith("float LockAlpha"), LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_LOCKED}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SEEKING}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tfloat LockAlpha = " + forty + ";\n",
            "\tfloat LockAlpha = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    lock_alpha_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(lock_alpha_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-contact-mark LockAlpha field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        for script in leftover_world_location_isolated_scripts():
            self.assertIn(script, LOCKED_SCRIPTS)
        for script in leftover_contact_mark_isolated_scripts():
            self.assertIn(script, LOCKED_SCRIPTS)
        for script in leftover_hud_lock_scripts():
            self.assertIn(script, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CONTACT_WORLD_LOCATION_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CONTACT_SEEKING_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CONTACT_LOCKED_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CONTACT_LABEL_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_LOCKED, locked_only)
