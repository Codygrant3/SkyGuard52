...
# BoundsMaximum FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# FVector BoundsMaximum = FVector::ZeroVector; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS `= FVector::ZeroVector` NOT `= false` / `= true` /
# `= 0` and NOT BoundsMinimum / bBoundsFiniteAndNonzero.
# Fail closed if the type or decl is missing, renamed,
# Category= is added, or default is not FVector::ZeroVector.
# Parse STRUCT public section only AFTER
# struct FSkyguardLandscapeVisibleAudit and BEFORE
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# BoundsMaximum. THIS IS NOT leftover BoundsMinimum
# (in-flight sibling; LOCK that script). THIS IS NOT leftover
# bBoundsFiniteAndNonzero (in-flight sibling; LOCK that
# script). THIS IS NOT leftover VisibleAudit
# BoundsMaximum wake sibling for bActorTemporarilyHiddenInEditor
# (LOCK that script). THIS IS NOT leftover VisibleAudit
# bActorHiddenInGame #1208. THIS IS NOT leftover
# ContractCameraFrustumIntersectionCount #1207. THIS IS NOT
# leftover VisibleAudit HiddenInGameComponentCount #1204.
# THIS IS NOT leftover landscape-visible-audit-defaults #0bce.
# THIS IS NOT leftover VisibleAudit bSuccess #1199. THIS IS
# NOT leftover Footprint bSuccess #1189 / Error #1198. THIS
# IS NOT leftover Mission01 EnvironmentReadiness
# LandscapeComponentCount #1176. LOCK leftover drafts
# #56-#64 plus leftover enum/defaults PRs including leftover
# landscape-visible-audit-defaults #0bce. LOCK leftover
# VisibleAudit bSuccess #1199 through
# GovernedMaterialParentMatchComponentCount #1206. LOCK
# leftover ContractCameraFrustumIntersectionCount #1207 /
# bActorHiddenInGame #1208 / leftover-safe
# actor-temporarily-hidden-in-editor field contract
# (this wake). Harbor 40/80 fail-closed in this Mission01-
# related test file. Ban retired live-copy tokens via split
...
# FVector BoundsMaximum = FVector::ZeroVector; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS `= FVector::ZeroVector` NOT `= false` / `= true` / `= 0`. Fail closed if the type
# or decl is missing, renamed, Category= is added, or default
# is not FVector::ZeroVector. Parse STRUCT public section only AFTER
# struct FSkyguardLandscapeVisibleAudit and BEFORE
# struct FSkyguardLandscapeCaptureConfigurationResult.
# Do NOT parse USkyguardMission01EnvironmentAuthoringLibrary.
# Do NOT parse ASkyguardMission01EnvironmentDirector.
...
# Do NOT parse FSkyguardLandscapeFootprintSampleResult.
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# BoundsMaximum. THIS IS NOT leftover
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# BoundsMaximum FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FVector BoundsMaximum = FVector::ZeroVector; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly FVector BoundsMaximum = FVector::ZeroVector;
...
# BoundsMaximum FIELD (declaration-only). It is
...
TARGET = "FVector BoundsMaximum = FVector::ZeroVector;"
TARGET_WRONG = "FVector BoundsMinimum = FVector::ZeroVector;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bool literal for FSkyguardLandscapeVisibleAudit::
    # BoundsMaximum (must be = FVector::ZeroVector, not BoundsMinimum or a bool default).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FVector\s+BoundsMaximum\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FVector BoundsMaximum is missing from "
...
class LandscapeVisibleAuditBoundsMaximumFieldDeclContractTests(unittest.TestCase):
    def test_landscape_visible_audit_struct_exists(self) -> None:
        ...
        self.assertIn("BoundsMaximum", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("BoundsMaximum", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_bounds_maximum_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_bounds_maximum_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FVector BoundsMaximum"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(
                "\tFVector BoundsMaximum = FVector::OneVector;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("BoundsMaximum", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("BoundsMaximum", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FVector BoundsMaximum"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("BoundsMaximum", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_bounds_maximum(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
