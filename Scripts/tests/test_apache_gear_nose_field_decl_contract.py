# THIS IS leftover-safe ASkyguardApacheAircraft GearNose.
...
# then BARE `TObjectPtr<UStaticMeshComponent> GearNose;`
# (no initializer).
...
# Stop BEFORE sibling GearLeft as a claimed slot.
# Do NOT claim GearLeft / GearRight / Nose /
...
# GearLeft / GearRight / Nose / HorizontalTail are sibling
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> GearNose;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> GearNose;` and NOT
# leftover `Nose` / leftover `GearLeft` / leftover
# `GearRight` / leftover `HorizontalTail` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> GearNose;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> GearNose = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> GearNose = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> GearNose = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GearNose;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GearNose;"
...
GEAR_NOSE_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+GearNose\s*;"
)
GEAR_NOSE_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+GearNose\s*="
)
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_GEAR_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_GEAR_LEFT):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+GearNose\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> GearNose "
...
class ApacheGearNoseFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GearNose"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GearNose"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GearNose", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_gear_nose_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> GearNose;", compact_origin)
        self.assertNotIn("GearNose = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_gear_nose_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> GearNose"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> GearNose;")
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> GearNose;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> GearNoses;\n",
            "\tint32 GearNose = 140;\n",
            "\tbool GearNose = true;\n",
            "\tfloat GearNose = " + forty + ";\n",
            "\tfloat GearNose = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_gear_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_GEAR_LEFT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GearNose field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gear_nose_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
