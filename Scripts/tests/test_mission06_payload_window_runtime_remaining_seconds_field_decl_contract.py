...
STRUCT_NAME = "FSkyguardPayloadWindowRuntime"
CLASS_NAME = STRUCT_NAME
# THIS IS leftover-safe Mission06 PayloadWindowRuntime
# RemainingSeconds FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly float = 0.f;
# one-line / split-line UPROPERTY wraps; origin/main
# has NO Category). PARSE
# FSkyguardPayloadWindowRuntime STRUCT
# public section only (after struct
# FSkyguardPayloadWindowRuntime, before
# struct FSkyguardMission06IntegrationReadiness).
# Do NOT parse ASkyguardMission06IntegrationDirector,
# FSkyguardMission06IntegrationReadiness,
# FSkyguardAirfieldTargetRuntime, leftover enum
# ESkyguardAirfieldTarget type body, leftover
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission06 PayloadWindowRuntime
# RemainingSeconds FIELD (declaration-only). THIS IS NOT leftover
# payload-window-runtime-defaults #215/#f114. THIS IS
# NOT leftover StartPayloadWindow / AdvancePayloadWindow
# / TryJamActivePayload / GetPayloadWindow. THIS IS NOT
# leftover Mission08 HoistWindowRuntime RemainingSeconds #1132
# / leftover Mission08 HoistWindowRuntime bActive #1130.
# THIS IS NOT leftover Mission04 searchlight RemainingSeconds
# #943. THIS IS NOT leftover airfield-target enum #222/#14d2,
# leftover airfield-target-runtime-defaults #217/#6ad8,
# leftover AirfieldTargetRuntime Target #1141 /
# Integrity #1139 / bDestroyed #1142. THIS IS NOT leftover
# PayloadWindowRuntime bActive #1143 or in-flight
# PayloadWindow Target. THIS IS NOT leftover
...
# merge #665 #666 #668-#673, or open drafts #674-#1143.
# LOCK leftover Mission01/02/03/04/05/06/08/09/10
...
# Mission06 PayloadWindowRuntime RemainingSeconds
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float = 0.f,
# one-line / split-line UPROPERTY wraps) on
# FSkyguardPayloadWindowRuntime. THIS IS NOT leftover
# AirfieldTargetRuntime Integrity #1139. It is
# leftover-safe Mission06 PayloadWindowRuntime
# RemainingSeconds FIELD (declaration-only). It is
...
# struct section of
# FSkyguardPayloadWindowRuntime only
# between struct FSkyguardPayloadWindowRuntime
# and struct FSkyguardMission06IntegrationReadiness / the following UCLASS /
...
# not leftover FSkyguardAirfieldTargetRuntime, not leftover
...
# This PayloadWindowRuntime field sits after Target and before
# bJammed in the public section.
# Skip leftover b + Ya + kRuntimeReady forever (it sits
# after bMapAssemblyReady and before bGunnerReady). This contract
# is RemainingSeconds after Target / before bJammed. THIS IS NOT
# leftover PayloadWindowRuntime bActive #1143 or in-flight
# PayloadWindow Target. THIS IS NOT leftover Mission08
# HoistWindowRuntime RemainingSeconds #1132. THIS IS NOT leftover
# Mission04 searchlight RemainingSeconds #943. THIS IS NOT
...
# drafts #674-#1143 including leftover Mission02
...
# the locked FSkyguardPayloadWindowRuntime
# public section. Do not parse
# ASkyguardMission07IntegrationDirector director-class
# fields. Do not parse FSkyguardMission06IntegrationReadiness.
...
# Do not parse FSkyguardAirfieldTargetRuntime. Do not
...
# drafts #674-#1143. Do not reopen leftover
...
REMAINING_SECONDS_ZERO = (
    "float RemainingSeconds = 0.0f;"
)
LOCKED_DECL = REMAINING_SECONDS
...
# LOCK leftover Mission01/02/03/04/05/06/08/09/10 same-name
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission07_search_track_runtime_classified_false_field_decl_contract.py",
...
def alternate_bool_defaults(declaration: str) -> tuple[str, ...]:
    ...
    if "= 0.0f;" in declaration:
        forms.append(declaration.replace("= 0.0f;", "= 0.f;"))
    unique: list[str] = []
    for form in forms:
...
class Mission06PayloadWindowRuntimeRemainingSecondsFieldDeclContractTests(unittest.TestCase):
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 RemainingSeconds field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission06 RemainingSeconds contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_payload_window_runtime_remaining"
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        self.assertNotIn("FSkyguardMission06IntegrationReadiness", section)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", section)
        self.assertNotIn("MaximumTargetIntegrity", section)
        ...
        with self.assertRaises(AssertionError) as raised:
            public_section(mission07_ptr_only)
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        mission06_readiness_only = (
            "struct FSkyguardMission06IntegrationReadiness\n"
            "{\n"
            "public:\n"
            f"\t{LOCKED_DECL}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            public_section(mission06_readiness_only)
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        mission06_director_only = (
            "class SKYGUARD52_API ASkyguardMission06IntegrationDirector "
            ": public AActor\n"
            "{\n"
            "public:\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            public_section(mission06_director_only)
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        airfield_enum_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            public_section(airfield_enum_only)
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        payload_only = (
        ...
        payload_section = public_section(payload_only)
        self.assertTrue(
            has_declaration(payload_section, LOCKED_DECL),
            payload_section,
        )
        locked_only = f"{LOCKED_DECL}\n"
    ...
    def test_leftover_same_name_wave_count_contracts_stay_locked(
        ...
        self.assertIn(
            "Scripts/tests/test_mission07_search_track_runtime"
            "_classified_false_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission08_hoist_window_runtime"
            "_completed_windows_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission06_payload_window_runtime"
