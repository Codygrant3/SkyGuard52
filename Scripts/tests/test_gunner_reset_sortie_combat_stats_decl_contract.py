# THIS IS leftover-safe ASkyguardGunner
...
# then BARE `void ResetSortieCombatStats();`
# (declaration-only, no inline body required).
# THIS IS leftover-safe isolated UFUNCTION.
# Category IS `Skyguard|Combat|Sortie`. BlueprintCallable IS present.
# const IS absent. Return type IS void. Declaration-only.
# Unique leftover-safe Category path: Skyguard|Combat|Sortie
# (NEW unused 3-segment suffix Sortie). This is leftover-safe
# unique analog, NOT leftover analog Combat.
...
# return type is not void, or if Category is
# Skyguard|Combat WITHOUT the unique leftover-safe suffix
# |Sortie / Skyguard|Combat|Feedback / Skyguard|Combat|Safety /
...
# and BEFORE `private:`.
# Stop BEFORE first public UPROPERTY (`LookYawLimit` / CPG kit).
# Combat|Sortie UFUNCTIONs sit early in public, before UPROPERTY.
# Do NOT claim sibling UFUNCTION
# GetSortieShotsFired / GetSortieHits as this slot
# (leave them uncontracted).
# Do NOT claim GetSortieAircraftDamageFraction
# (retired live-copy comment in source; skip).
# Do NOT claim FillSortieCombatStats (NO UFUNCTION macro).
# Do NOT claim Record + primary / guided shot-hit helpers
# (banned; not this slot).
# Do NOT claim leftover unique Gunner FIELD #1593-#1629
# including FireCameraShakeClass #1625 Combat|Feedback,
# MinimumSafeSideFireYaw #1626 Combat|Safety, Boom #1629,
# LookYawLimit #1618, BaseDamage #1624.
# Do NOT claim leftover unique Combat without |Sortie FIELD
# LookYawLimit / LookPitchMin / LookPitchMax / MouseSensitivity
# / ADSFov / HipFov / TraceRange / BaseDamage.
...
# Do NOT claim leftover unique fill-result-combat-stats #567
# CLOSED USkyguardCampaignSubsystem::FillResultCombatStats
# (different class; do not reopen).
# Do NOT claim leftover unique IronRain MaxReleasesPerBay
# #1683 / LastFlight MinimumCivilianSeparationMeters #1684.
...
# (NO spaces around =).
# Better leftover unique int32 BlueprintPure clone was
# BlueprintPure int32 const — RETARGET wrap BlueprintPure
# -> BlueprintCallable, return int32 -> void, drop const.
# Leftover analog theater-kit-get-lamp-instance-count is
# int32 not void and Category Skyguard|Theater — RETARGET
# hard, do not claim as this slot.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is `void ResetSortieCombatStats();`
# NOT `FName WeatherIdentity;` and NOT GetLampInstanceCount
# and NOT EnsureDefaultEntries and NOT
# GetResolvedProductionLoopRouteCount and NOT
# GetSortieShotsFired / GetSortieHits and NOT
# FillSortieCombatStats and NOT FillResultCombatStats and NOT
# GlobalVoiceLimit = 24 and NOT OpenCockpitWindLoop and NOT
# EngineIdleLoop and NOT IsDestroyed leftover analog and NOT
# FireRate = 12.0f / RecoilPitch = 0.92f and NOT leftover Boom
# and NOT leftover LookYawLimit and NOT leftover
# VegetationStartCullDistanceCm
# and NOT TObjectPtr ProductionBank.
# Fail-closed if this test still asserts
# EnsureDefaultEntries / WeatherIdentity /
# GetLampInstanceCount /
# GetResolvedProductionLoopRouteCount /
# FireRate = 12.0f / RecoilPitch = 0.92f / leftover Boom /
# leftover LookYawLimit
# AS THE LOCKED DECL.
...
TARGET = "void ResetSortieCombatStats();"
TARGET_INLINE = (
    "void ResetSortieCombatStats() { }"
)
TARGET_WRONG_BARE = "void ResetSortieCombatStats() const;"
TARGET_WRONG_NO_CONST = (
    "bool ResetSortieCombatStats();"
)
TARGET_WRONG_VOID = "bool AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_FLOAT = "float ResetSortieCombatStats();"
TARGET_WRONG_INT = "int32 ResetSortieCombatStats();"
TARGET_WRONG_RENAME = "void ResetSortieCombatStat();"
...
TARGET_WRONG_TRUE = "bool ResetSortieCombatStats = true;"
TARGET_WRONG_ZERO = "bool ResetSortieCombatStats = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bvoid\s+ResetSortieCombatStats\s*\(\s*\)\s*(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+ResetSortieCombatStats\s*\(\s*\)\s*const\b", compact):
        return False
    if re.search(r"\bbool\s+ResetSortieCombatStats\b", compact):
        return False
    if re.search(r"\bfloat\s+ResetSortieCombatStats\b", compact):
        return False
    if re.search(r"\bint32\s+ResetSortieCombatStats\b", compact):
        return False
    if re.search(
        r"\bvoid\s+ResetSortieCombatStats\s*\(\s*[^)]+\s*\)",
        compact,
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+ResetSortieCombatStats\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void ResetSortieCombatStats "
...
class GunnerResetSortieCombatStatsDeclContractTests(
    unittest.TestCase
):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "ResetSortieCombatStats"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_CPG_STATION), body)
        self.assertFalse(
        ...
        self.assertTrue(has_identifier(body, "FireCameraShakeClass"), body)
    ...
    def test_missing_reset_sortie_combat_stats_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertFalse(
            leftover_combat_category_without_sortie(section),
            section,
        )
        self.assertIn('Skyguard|Combat|Sortie', section)
        ...
        self.assertFalse(
            leftover_combat_category_without_sortie(specifiers),
            specifiers,
        )
        self.assertIn('Skyguard|Combat|Sortie', specifiers)
        ...
        self.assertTrue("|Sortie" in specifiers)
        self.assertNotEqual(
            specifiers,
            'BlueprintCallable, Category="Skyguard|Combat"',
        )
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "void ResetSortieCombatStats()const;",
    ...
    def test_reset_sortie_combat_stats_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertFalse(
            leftover_combat_category_without_sortie(LOCKED_UFUNCTION_WRAP),
            LOCKED_UFUNCTION_WRAP,
        )
        self.assertIn('Skyguard|Combat|Sortie', LOCKED_UFUNCTION_WRAP)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid ResetSortieCombatStatss();\n",
            "\tint32 ResetSortieCombatStats = 1;\n",
            "\tFName ResetSortieCombatStats = NAME_None;\n",
            "\tvoid ResetSortieCombatStats() = "
            + forty
            + ";\n",
            "\tvoid ResetSortieCombatStats() = "
    ...
    def test_missing_category_or_blueprint_callable_fails_closed(self) -> None:
        ...
        self.assertFalse(
            leftover_combat_category_without_sortie(origin),
            origin,
        )
        self.assertIn('Skyguard|Combat|Sortie', origin)
    ...
    def test_does_not_claim_sibling_sortie_counters_or_neighbors(
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_STATION))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "ResetSortieCombatStats")
        )
        self.assertTrue(
            has_identifier(section, "ResetSortieCombatStats")
        )
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_FIRE_RATE))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, "FireCameraShakeClass"))
        self.assertFalse(has_identifier(section, "FireCameraShakeClass"))
        self.assertTrue(has_identifier(leaked, "MinimumSafeSideFireYaw"))
        self.assertFalse(has_identifier(section, "MinimumSafeSideFireYaw"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner reset sortie combat stats "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_reset_sortie_combat_stats"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioProductionBank.cpp", THIS_SCRIPT)
