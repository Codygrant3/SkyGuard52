# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot Score.
...
# Exact identifier vs siblings Medal, ShotsFired, and Hits
# (same type+initializer). Do not claim sibling fields:
# bValid, bWon, MissionTitle, OutcomeNarrative, Medal,
...
TARGET = "int32 Score = 0;"
TARGET_WRONG_NONE = "int32 Score;"
TARGET_WRONG_FALSE = "int32 Score = false;"
TARGET_WRONG_TRUE = "int32 Score = true;"
TARGET_WRONG_FLOAT = "int32 Score = 0.f;"
TARGET_WRONG_HUNDRED = "int32 Score = 100;"
TARGET_WRONG_ONE = "int32 Score = 1;"
TARGET_WRONG_MEDAL = "int32 Medal = 0;"
TARGET_WRONG_SHOTS = "int32 ShotsFired = 0;"
TARGET_WRONG_HITS = "int32 Hits = 0;"
TARGET_WRONG_FNAME = "FName Score;"
TARGET_WRONG_BOOL = "bool Score = 0;"
TARGET_WRONG_FLOAT_TYPE = "float Score = 0;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 Score = 0;`.
    # Do not accept missing initializer, `= 0.f`, `= false`,
    # `= 100`, sibling Medal / ShotsFired / Hits, or a
    # leftover theater UPROPERTY wrap as the locked decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"int32\s+Score\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"int32\s+Score\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+Score\s*=\s*true\b", compact):
        return False
    if re.search(r"int32\s+Score\s*=\s*100\b", compact):
        return False
    if re.search(r"(?:float|bool|FName|FString)\s+Score\b", compact):
        return False
    if re.search(r"int32\s+Score\s*=\s*0\s*;", compact) is None:
        return False
    return True
...
class CpgDebriefSnapshotScoreFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Score"), section)
    ...
    def test_capture_function_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Score"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
    ...
    def test_origin_main_score_is_plain_int32_zero(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
        ...
        self.assertEqual(LOCKED_DECL, "int32 Score = 0;")
        self.assertTrue(LOCKED_DECL.startswith("int32 Score"), LOCKED_DECL)
        ...
        self.assertIn("int32 Score = 0;", compact)
        self.assertNotIn("Score = 0.f", compact)
        self.assertNotIn("Score = false", compact)
        self.assertNotIn("Score = 100", compact)
    ...
    def test_exact_identifier_vs_same_type_siblings(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Score"), section)
        self.assertIn("Score", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot Score field decl contract "
