...
# then `float LockWindowClimbCm = 650.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Route`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"0.0"`. Default IS `= 650.f`.
# Clone wrap ClampMin WAS `"100.0"` for LockWindowSpeed. THIS slot
# ClampMin IS `"0.0"` (NO spaces around =). Fail-closed if wrap still
# has ClampMin="100.0". 650.f is NOT Harbor 40 or 80.
...
# ClampMin="100.0" from clone LockWindowSpeed.
# LOCKED_DECL is `float LockWindowClimbCm = 650.f;`
...
TARGET = "float LockWindowClimbCm = 650.f;"
TARGET_WRONG_BARE = "float LockWindowClimbCm;"
TARGET_WRONG_INIT = "float LockWindowClimbCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float LockWindowClimbCm = true;"
TARGET_WRONG_ZERO = "float LockWindowClimbCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LockWindowClimbCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LockWindowClimbCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LockWindowClimbCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "LockWindowClimbSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_lock_window_climb_cm"
...
LOCK_WINDOW_CLIMB_CM_DECL_RE = re.compile(
    r"float\s+LockWindowClimbCm\s*=\s*650\.f\s*;"
)
LOCK_WINDOW_CLIMB_CM_BARE_RE = re.compile(
    r"float\s+LockWindowClimbCm\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float LockWindowClimbCm = 650.f;`.
    ...
    # LockWindowClimbCm without the 650.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LOCK_WINDOW_CLIMB_CM_DECL_RE.search(compact) is None:
        return False
    if LOCK_WINDOW_CLIMB_CM_BARE_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LockWindowClimbCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LockWindowClimbCm "
...
class PathfinderEncounterLockWindowClimbCmFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockWindowClimbCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LockWindowClimbCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LockWindowClimbCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lock_window_climb_cm_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float LockWindowClimbCm;",
            compact_origin.split("LockWindowClimbCm = 650.f;")[0]
            + compact_origin.split("LockWindowClimbCm = 650.f;")[-1]
            if "LockWindowClimbCm = 650.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_lock_window_climb_cm_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LockWindowClimbCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("650.f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat LockWindowClimbCms = 650.f;\n",
            "\tint32 LockWindowClimbCm = 320;\n",
            "\tFName LockWindowClimbCm = NAME_None;\n",
            "\tfloat LockWindowClimbCm = " + forty + ";\n",
            "\tfloat LockWindowClimbCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockWindowClimbCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LockWindowClimbCm"))
        self.assertTrue(has_identifier(section, "LockWindowClimbCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_lock_window_climb_cm_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
