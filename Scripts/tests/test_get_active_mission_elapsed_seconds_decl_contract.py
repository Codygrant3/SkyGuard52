...
# Declaration presence only. Do not invent INDEX_NONE, an
# elapsed-seconds return value, or lock the
# GetActiveMissionElapsedSeconds body in the .cpp.
...
# GetActiveMissionElapsedSeconds declaration contract. Stay
# off GetActiveMission (newly drafted sibling),
# GetObjectiveRuntime / TravelToNextMission (in-flight
# siblings), leftover objective-runtime fail-closed /
# leftover route-runtime fail-closed, leftover CPG debrief
# #284/#195/#130/#8ccd, FillResultCombatStats (takes leftover
# ASkyguardGunner*), leftover campaign-save
# empty-fail-closed drafts, leftover campaign-roster lookup
# #111, LoadCampaignProgressAfterConfigure (#290), leftover
# Harbor #6/#8/#9, leftover theater-kit #59, leftover
# flare/HUD #57/#61/#62, leftover drafts #56–#64, leftover
# #147 ApacheSystem, leftover #149 weapon stations, leftover
# #152 pilot commands, leftover #154 loadout / lock-phase,
# Harbor IncomingRadar 40/80, Yak/Igla/rifle live copy,
...
# flare/HUD, and newly drafted campaign-subsystem siblings
# stay sibling-only.
LOCKED_SCRIPTS = (
...
# GetActiveMission is a newly drafted sibling.
# GetObjectiveRuntime / TravelToNextMission are in-flight
# siblings. FillResultCombatStats takes leftover
# ASkyguardGunner*. Leftover CPG debrief #284/#195/#130/
# #8ccd stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# .cpp GetActiveMissionElapsedSeconds body / invented
...
CPP_AND_INVENTED = (
    ...
    "FMath::Max(0.f",
    "USkyguardCampaignSubsystem::GetActiveMissionElapsedSeconds",
    "SkyguardCampaignSubsystem.cpp",
)
SIBLING_TYPES = (
...
class GetActiveMissionElapsedSecondsDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_ELAPSED), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_ELAPSED)
        self.assertIn("GetActiveMissionElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_ELAPSED))

    def test_missing_elapsed_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_ELAPSED)
        self.assertIn("GetActiveMissionElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_ELAPSED)
        self.assertIn("GetActiveMissionElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_neighbor_getters_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_getters, GET_ELAPSED)
        self.assertIn("GetActiveMissionElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = (
            "\tfloat GetActiveMissionElapsedSeconds() const;\n"
        )
        non_const = (
            "\tfloat GetActiveMissionElapsedSeconds("
        ...
        wrong_return = (
            "\tint32 GetActiveMissionElapsedSeconds("
            "const UObject* WorldContextObject) const;\n"
        )
        mutable_world = (
            "\tfloat GetActiveMissionElapsedSeconds("
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_ELAPSED)
            self.assertIn(
                "GetActiveMissionElapsedSeconds",
                str(raised.exception),
            )
            self.assertIn("missing", str(raised.exception).lower())

    def test_elapsed_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_ELAPSED),
            GET_ELAPSED,
        )
        self.assertTrue(has_declaration(section, GET_ELAPSED))
        self.assertEqual(declaration_count(section, GET_ELAPSED), 1)
        self.assertTrue(GET_ELAPSED.endswith("const;"), GET_ELAPSED)
        self.assertTrue(GET_ELAPSED.startswith("float "), GET_ELAPSED)
        self.assertIn("const UObject* WorldContextObject", GET_ELAPSED)
        self.assertNotIn("INDEX_NONE", GET_ELAPSED)
        self.assertNotIn("{", GET_ELAPSED)
        self.assertNotIn("}", GET_ELAPSED)
        self.assertNotIn("return ", GET_ELAPSED)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_arg = (
            "public:\n"
            "\tfloat GetActiveMissionElapsedSeconds(const UObject*\n"
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, GET_ELAPSED), 1)
        one_line = f"{{\npublic:\n\t{GET_ELAPSED}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_ELAPSED))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_ELAPSED), section)
        self.assertEqual(
            require_declaration(section, GET_ELAPSED),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_ELAPSED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_ELAPSED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_ELAPSED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_declaration_does_not_invent_elapsed_seconds_return_value(
        self,
    ) -> None:
        locked_only = f"{GET_ELAPSED}\n"
        self.assertNotIn("return ", GET_ELAPSED)
        self.assertNotIn("return 0.f", GET_ELAPSED)
        self.assertNotIn("return 0.0f", GET_ELAPSED)
        self.assertNotIn("return -1.f", GET_ELAPSED)
        self.assertNotIn("GetTimeSeconds", GET_ELAPSED)
        self.assertNotIn("MissionStartWorldTimeSeconds", GET_ELAPSED)
        self.assertNotIn("FMath::Max", GET_ELAPSED)
        self.assertNotIn("return 0.f", locked_only)
        self.assertNotIn("return 0.0f", locked_only)
        self.assertNotIn("GetTimeSeconds", locked_only)
        self.assertNotIn("MissionStartWorldTimeSeconds", locked_only)
        section = public_section(origin_main_header())
        self.assertNotIn("return 0.f", section)
    ...
    def test_contract_does_not_lock_elapsed_seconds_cpp_body(self) -> None:
        locked_only = f"{GET_ELAPSED}\n"
        self.assertNotIn("{", GET_ELAPSED)
        self.assertNotIn("}", GET_ELAPSED)
        self.assertNotIn("return ", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_configure_can_start_or_start(self) -> None:
        ...
        for neighbor in CONFIGURE_CAN_START_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("ConfigureCampaign", GET_ELAPSED)
        self.assertNotIn("CanStartMission", GET_ELAPSED)
        self.assertNotIn("StartMission", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_is_mission_unlocked(self) -> None:
        ...
        for neighbor in IS_MISSION_UNLOCKED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("IsMissionUnlocked", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("AddObjectiveProgress", GET_ELAPSED)
        self.assertNotIn("FailObjective", GET_ELAPSED)
        self.assertNotIn("CompleteSurviveObjectiveIfIntact", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("CompleteActiveMission", GET_ELAPSED)
        self.assertNotIn("FinalizeActiveMission", GET_ELAPSED)
        self.assertNotIn("FailActiveMission", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("FillResultCombatStats", GET_ELAPSED)
        self.assertNotIn("ASkyguardGunner", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("AcknowledgeDebrief", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_debrief_neighbors(self) -> None:
        ...
        for neighbor in DEBRIEF_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("RetrySaveLastDebrief", GET_ELAPSED)
        self.assertNotIn("GetLastDebrief", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_leftover_cpg_debrief_siblings(self) -> None:
        ...
        for token in LEFTOVER_CPG_DEBRIEF_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_ELAPSED)
    ...
    def test_contract_does_not_relock_travel_helpers(self) -> None:
        ...
        for neighbor in TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("CanTravelToNextMission", GET_ELAPSED)
        self.assertNotIn("GetNextMissionMapPackageName", GET_ELAPSED)
        self.assertNotIn("TravelToNextMission", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_runtime_getters(self) -> None:
        ...
        for neighbor in RUNTIME_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("GetObjectiveRuntime", GET_ELAPSED)
        self.assertNotIn("GetRouteRuntime", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_save_game_helpers(self) -> None:
        ...
        for neighbor in SAVE_GAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED)
        self.assertNotIn("ApplySaveGame", GET_ELAPSED)
        self.assertNotIn("BuildSaveGame", GET_ELAPSED)
        self.assertNotIn("SaveCampaignToSlot", GET_ELAPSED)
        self.assertNotIn("LoadCampaignFromSlot", GET_ELAPSED)
        self.assertNotIn("DeleteCampaignSlot", GET_ELAPSED)
        self.assertNotIn("IsValidCampaignSlotName", GET_ELAPSED)
        self.assertNotIn("GetEarnedCampaignMedals", GET_ELAPSED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_ELAPSED}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_ELAPSED),
            GET_ELAPSED,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(GET_ELAPSED, "Rifle")
        self.assertNotEqual(GET_ELAPSED, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign GetActiveMissionElapsedSeconds contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, GET_ELAPSED.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", GET_ELAPSED)

    def test_contract_is_elapsed_seconds_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
