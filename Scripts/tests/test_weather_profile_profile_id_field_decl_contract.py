# THIS IS leftover-safe FSkyguardWeatherProfile ProfileId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName ProfileId;` (no initializer).
...
# Do NOT contract sibling fields Weather /
...
# isolated ProfileId field decl, same as leftover
...
# `FName LineId`, retarget: type is FName ProfileId,
# no `=`, specifiers are EditAnywhere + BlueprintReadOnly,
# no Category.
...
TARGET = "FName ProfileId;"
TARGET_WRONG_EQ_NONE = "FName ProfileId = NAME_None;"
TARGET_WRONG_FALSE = "FName ProfileId = false;"
TARGET_WRONG_TRUE = "FName ProfileId = true;"
TARGET_WRONG_ZERO = "FName ProfileId = 0.f;"
TARGET_WRONG_FLOAT = "float ProfileId;"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+ProfileId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName ProfileId is missing from "
...
def require_profile_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} ProfileId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} ProfileId"
        )
...
class WeatherProfileProfileIdFieldDeclContractTests(unittest.TestCase):
    def test_weather_profile_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ProfileId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ProfileId"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("ProfileId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_profile_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_profile_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName ProfileId;", compact_origin)
        self.assertNotIn("ProfileId = NAME_None", compact_origin)
        self.assertNotIn("ProfileId = false", compact_origin)
        self.assertNotIn("ProfileId = true", compact_origin)
        self.assertNotIn("ProfileId = 0.f", compact_origin)
        self.assertNotIn("ProfileId = 160.f", compact_origin)

    def test_profile_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName ProfileId"),
            LOCKED_DECL,
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_profile_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_profile_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_profile_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_profile_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_profile_id_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weather-profile ProfileId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weather_profile_profile_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
