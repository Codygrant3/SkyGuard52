# THIS IS leftover-safe FSkyguardHowToFlyRow::Instruction.
# origin/main form: split-line
# `UPROPERTY(VisibleAnywhere, BlueprintReadOnly, meta=(MultiLine="true"))`
# then BARE `FText Instruction;` (no initializer, no Category=).
# Accept origin/main meta MultiLine if present. If a clone asserts an
# initializer and origin/main has a bare decl, accept the bare decl.
# If a clone asserts Category and origin/main has none, drop that
# assertion.
# THIS IS NOT leftover how-to-fly-row-defaults #191 / #9ab2.
# THIS IS NOT leftover BriefingRadioRow Subtitle.
# THIS IS NOT leftover BriefingCard Body #1254.
...
# meta=(MultiLine="true"), no Category=. Accept bare `FText Instruction;`.
# Fail-closed if type/decl missing, renamed, Category= added, or
# MultiLine meta is dropped when origin/main has it.
...
# leftover how-to-fly-row-defaults #191/#9ab2, leftover briefing-widget
# #388-#391, leftover AuthoringResult through #1250, leftover
# BriefingCard #1251-#1254, leftover RadioRow Speaker #1255 / LineId #1256.
...
TARGET = "FText Instruction;"
TARGET_WRONG_INIT = "FText Instruction = FText();"
TARGET_WRONG_EMPTY = "FText Instruction = FText::GetEmpty();"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, meta=(MultiLine="true"))'
)
UPROPERTY_NO_MULTILINE = "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
STOP_AFTER = "DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer when origin/main has `FText Instruction;`.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    for match in re.finditer(r"UPROPERTY\(", compact):
        ...
        if depth != 0:
            continue
        after = compact[index:]
        if re.match(r"\s*FText\s+Instruction\b", after):
            return compact[start : index - 1]
    raise AssertionError(
        "UPROPERTY for FText Instruction is missing from "
...
class HowToFlyRowInstructionFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Instruction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_instruction_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_instruction_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_EMPTY}\n", LOCKED_DECL)
        self.assertIn("Instruction", str(raised.exception))

    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            "\tFText Instruction = FText();\n",
            "\tFText Instruction = FText::GetEmpty();\n",
            "\tFString Instruction;\n",
            "\tFText InputHint;\n",
            "\tFText Body;\n",
            "\tFText Title;\n",
            "\tFText Subtitle;\n",
            "\tFText CardInstruction;\n",
    ...
    def test_multiline_meta_dropped_fails_closed(self) -> None:
        dropped = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_NO_MULTILINE}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
    ...
    def test_does_not_contract_pictogram_or_neighbors(self) -> None:
        self.assertNotIn("Pictogram", LOCKED_DECL)
        self.assertNotIn("Priority", LOCKED_DECL)
        self.assertNotIn("Title", LOCKED_DECL)
        self.assertNotIn("CardId", LOCKED_DECL)
    ...
    def test_does_not_parse_neighbors_or_component_methods(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIn(STOP_AFTER, header)
        self.assertIn(NEIGHBOR_CARD, header)
        self.assertIn(NEIGHBOR_RADIO, header)
        self.assertIn(COMPONENT_CLASS, header)
        self.assertIn(ENUM_HEAD, header)
        self.assertIn(ADD_CARD, header)
        self.assertNotIn(STOP_AFTER, section)
        self.assertNotIn(NEIGHBOR_CARD, section)
        self.assertNotIn(NEIGHBOR_RADIO, section)
        self.assertNotIn(COMPONENT_CLASS, section)
        ...
        self.assertNotIn(NEIGHBOR_CARD, leaked)
        self.assertNotIn(NEIGHBOR_RADIO, leaked)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            "\tvoid AddHowToFlyRow(\n"
            "\t\tFName StepId,\n"
            "\t\tconst FText& InputHint,\n"
            "\t\tconst FText& Instruction,\n"
            "\t\tESkyguardBriefingPictogram Pictogram);\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn("Category", UPROPERTY_VISIBLE)
        self.assertIn("MultiLine", UPROPERTY_VISIBLE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "how-to-fly-row Instruction field contract contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_how_to_fly_row_instruction_field_decl_contract.py"
        ))
    ...
    def test_leftover_briefing_defaults_stay_locked(self) -> None:
        leftovers = (
            "Scripts/tests/test_briefing_card_defaults_contract.py",
            "Scripts/tests/test_briefing_radio_row_defaults_contract.py",
            "Scripts/tests/test_how_to_fly_row_defaults_contract.py",
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertIn("Instruction;", locked_only)
        self.assertNotIn("Pictogram", locked_only)
        self.assertNotIn("Priority", locked_only)
        self.assertNotIn("GetBriefingCards", locked_only)
        self.assertNotIn("AddBriefingCard", locked_only)
