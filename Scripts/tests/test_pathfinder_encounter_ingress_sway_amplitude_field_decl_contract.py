...
# IngressSwayWavelengthCm = 9000.f / LockWindowClimbCm = 650.f /
...
# LOCKED_DECL is `float IngressSwayAmplitudeCm = 320.f;`
...
TARGET = "float IngressSwayAmplitudeCm = 320.f;"
TARGET_WRONG_BARE = "float IngressSwayAmplitudeCm;"
TARGET_WRONG_INIT = "float IngressSwayAmplitudeCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float IngressSwayAmplitudeCm = true;"
TARGET_WRONG_ZERO = "float IngressSwayAmplitudeCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> IngressSwayAmplitudeCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> IngressSwayAmplitudeCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> IngressSwayAmplitudeCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "IngressSwayWavelengthCm"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_ingress_sway_amplitude"
...
INGRESS_SWAY_AMPLITUDE_DECL_RE = re.compile(
    r"float\s+IngressSwayAmplitudeCm\s*=\s*320\.f\s*;"
)
INGRESS_SWAY_AMPLITUDE_BARE_RE = re.compile(
    r"float\s+IngressSwayAmplitudeCm\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float IngressSwayAmplitudeCm = 320.f;`.
    ...
    # IngressSwayAmplitudeCm without the 320.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if INGRESS_SWAY_AMPLITUDE_DECL_RE.search(compact) is None:
        return False
    if INGRESS_SWAY_AMPLITUDE_BARE_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "IngressSwayAmplitudeCm":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+IngressSwayAmplitudeCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float IngressSwayAmplitudeCm "
...
class PathfinderEncounterIngressSwayAmplitudeFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "IngressSwayAmplitudeCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "IngressSwayAmplitudeCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("IngressSwayAmplitudeCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_ingress_sway_amplitude_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float IngressSwayAmplitudeCm;",
            compact_origin.split("IngressSwayAmplitudeCm = 320.f;")[0]
            + compact_origin.split("IngressSwayAmplitudeCm = 320.f;")[-1]
            if "IngressSwayAmplitudeCm = 320.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_ingress_sway_amplitude_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float IngressSwayAmplitudeCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("320.f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat IngressSwayAmplitudeCms = 320.f;\n",
            "\tint32 IngressSwayAmplitudeCm = 320;\n",
            "\tFName IngressSwayAmplitudeCm = NAME_None;\n",
            "\tfloat IngressSwayAmplitudeCm = " + forty + ";\n",
            "\tfloat IngressSwayAmplitudeCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "IngressSwayAmplitudeCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "IngressSwayAmplitudeCm"))
        self.assertTrue(has_identifier(section, "IngressSwayAmplitudeCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_ingress_sway_amplitude_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
