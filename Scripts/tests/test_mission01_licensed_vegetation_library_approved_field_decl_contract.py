...
# then `bool bLicensedVegetationLibraryApproved = false;`
# (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Mission01|Environment|PCG`.
# EditInstanceOnly IS present. BlueprintReadWrite IS present.
# Default IS `= false`.
...
# (VisibleAnywhere, BlueprintReadOnly, no Category).
...
# / Category="Skyguard|MissionMap" / Category="Skyguard"
# without |Mission01|Environment|PCG.
...
# Stop BEFORE sibling bAllowAuthoredPCGGeneration as a claimed slot.
...
# Category="Skyguard|Mission01|Environment|PCG" wrapping FName
...
# (NO spaces around =).
# LOCKED_DECL is `bool bLicensedVegetationLibraryApproved = false;`
# NOT `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# and NOT `FName WeatherIdentity;` (no initializer) and NOT
# leftover `bool bLicensedVegetationApproved = false;`
# (readiness struct) and NOT leftover
# `bool bAllowAuthoredPCGGeneration = false;` and NOT leftover
# `bool bEnableCoastalHazeTransition = true;` and NOT leftover
# `bool bUseAuthoredLandscapeSurface = false;` and NOT leftover
# combat look float LookYawLimit = 95.f and NOT leftover analog
...
# / TArray RoutePoints / bool bLicensedVegetationApproved = false;
# AS THE LOCKED DECL.
...
# or EditInstanceOnly (must be EditInstanceOnly + BlueprintReadWrite).
# Fail-closed if LOCKED_DECL is leftover readiness
# bPCGGenerationAuthorized / bAuthoredLandscapeSurfaceExposed /
...
TARGET = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_BARE = "bool bLicensedVegetationLibraryApproved;"
TARGET_WRONG_INIT = "bool bLicensedVegetationLibraryApproved = true;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationApproved = false;"
TARGET_WRONG_TRUE = "bool bLicensedVegetationLibraryApproved = true;"
TARGET_WRONG_ZERO = "bool bLicensedVegetationLibraryApproved = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> bLicensedVegetationLibraryApproved;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> bLicensedVegetationLibraryApproved;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> bLicensedVegetationLibraryApproved;"
...
STOP_BEFORE_SKYLINE_STYLE = "bAllowAuthoredPCGGeneration"
...
LICENSED_VEGETATION_LIBRARY_DECL_RE = re.compile(
    r"bool\s+bLicensedVegetationLibraryApproved\s*=\s*false\s*;"
)
LICENSED_VEGETATION_LIBRARY_BARE_RE = re.compile(
    r"bool\s+bLicensedVegetationLibraryApproved\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover readiness `bool bLicensedVegetationApproved
    # = false;`, leftover `= 160.f`, leftover analog
    # apache-cpg-feel FireRate / RecoilPitch, leftover CPG
    # mesh/text, leftover LoadoutSpec magazine fields,
    # leftover Harbor 40 / 80, leftover
    ...
    # bLicensedVegetationLibraryApproved without false.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LICENSED_VEGETATION_LIBRARY_DECL_RE.search(compact) is None:
        return False
    if LICENSED_VEGETATION_LIBRARY_BARE_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "bLicensedVegetationLibraryApproved":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bLicensedVegetationLibraryApproved\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bLicensedVegetationLibraryApproved "
...
class Mission01LicensedVegetationLibraryApprovedFieldDeclContractTests(unittest.TestCase):
    def test_mission01_environment_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bLicensedVegetationLibraryApproved"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_OCEAN_TILES), body)
        self.assertFalse(
            has_identifier(section, SIBLING_OCEAN_TILES),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_PRODUCTION_LANDSCAPE), body)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ALLOW_AUTHORED_PCG), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bLicensedVegetationLibraryApproved"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("bLicensedVegetationLibraryApproved", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_licensed_vegetation_library_approved_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_licensed_vegetation_library_approved_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bLicensedVegetationLibraryApproved"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool bLicensedVegetationLibraryApproveds = false;\n",
            "\tint32 bLicensedVegetationLibraryApproved = 1;\n",
            "\tFName bLicensedVegetationLibraryApproved = NAME_None;\n",
            "\tbool bLicensedVegetationLibraryApproved = " + forty + ";\n",
            "\tbool bLicensedVegetationLibraryApproved = " + eighty + ";\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Mission01|Environment|PCG"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission01|Environment|PCG"', theater_specs)
        self.assertIn('Category="Skyguard|Mission01|Environment|PCG"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
    ...
    def test_does_not_claim_sibling_skyline_style_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bLicensedVegetationLibraryApproved"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "bLicensedVegetationLibraryApproved"))
        self.assertTrue(has_identifier(section, "bLicensedVegetationLibraryApproved"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_ROOT))
        self.assertFalse(has_identifier(section, SIBLING_ROOT))
        self.assertTrue(has_identifier(leaked, SIBLING_OCEAN_TILES))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_ALLOW_AUTHORED_PCG))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 licensed vegetation library approved field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_licensed_vegetation_library_approved_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
