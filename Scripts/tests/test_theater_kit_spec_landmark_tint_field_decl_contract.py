# THIS IS leftover-safe FSkyguardTheaterKitSpec LandmarkTint.
...
# NamedLandmark / SilhouetteKit / SilhouetteTint /
...
# the bulk).
# THIS IS NOT leftover sibling drafts #1299-#1308.
...
TARGET = "FLinearColor LandmarkTint = FLinearColor::White;"
TARGET_WRONG_EQ_NONE = "FLinearColor LandmarkTint = NAME_None;"
TARGET_WRONG_FALSE = "FLinearColor LandmarkTint = false;"
TARGET_WRONG_TRUE = "FLinearColor LandmarkTint = true;"
TARGET_WRONG_ZERO = "FLinearColor LandmarkTint = 0.f;"
TARGET_WRONG_FLOAT = "float LandmarkTint;"
TARGET_WRONG_BARE = "FLinearColor LandmarkTint;"
TARGET_WRONG_FNAME = "FName LandmarkTint;"
TARGET_WRONG_BLACK = "FLinearColor LandmarkTint = FLinearColor::Black;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if re.search(
        r"FName\s+LandmarkTint\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LandmarkTint\b",
        compact,
    ):
        return False
    if re.search(
        r"FLinearColor\s+LandmarkTint\s*;",
        compact,
    ):
        return False
    assigned = re.findall(
        r"FLinearColor\s+LandmarkTint\s*=\s*(\S[^;]*);",
        compact,
    )
    if not assigned:
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FLinearColor\s+LandmarkTint\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FLinearColor LandmarkTint is missing from "
...
class TheaterKitSpecLandmarkTintFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LandmarkTint"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LandmarkTint"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LandmarkTint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_landmark_tint_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_decl_fails_closed_when_origin_has_white_default(self) -> None:
        ...
        self.assertNotIn("LandmarkTint = NAME_None", compact_origin)
        self.assertNotIn("LandmarkTint = false", compact_origin)
        self.assertNotIn("LandmarkTint = true", compact_origin)
        self.assertNotIn("LandmarkTint = 0.f", compact_origin)
        self.assertNotIn("LandmarkTint = 160.f", compact_origin)
        self.assertNotIn("LandmarkTint = FLinearColor::Black", compact_origin)

    def test_landmark_tint_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FLinearColor LandmarkTint"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFLinearColor LandmarkTints = FLinearColor::White;\n",
            "\tint32 LandmarkTint;\n",
            "\tbool LandmarkTint;\n",
            "\tfloat LandmarkTint = " + forty + ";\n",
            "\tfloat LandmarkTint = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tFLinearColor\n\tLandmarkTint = FLinearColor::White;\n",
            "\tFLinearColor   LandmarkTint = FLinearColor::White;\n",
            "\tFLinearColor\tLandmarkTint = FLinearColor::White;\n",
            "\tFLinearColor LandmarkTint =\n\tFLinearColor::White;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_VISIBLE_READONLY}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec LandmarkTint field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "landmark_tint_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
