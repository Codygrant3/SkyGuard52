...
# INDEX_NONE or lock AdvanceReinforcementTimer
# construction in the .cpp. This is leftover-safe
# Mission07 Boss AdvanceReinforcementTimer
...
# Mission07 NotifyThreatDestroyed, NOT leftover
# Mission06 NotifyThreatDestroyed #689, NOT leftover
# Mission05 NotifyThreatDestroyed #669, NOT leftover
# Mission03 / Mission04 NotifyThreatDestroyed, NOT
# leftover Mission07 InitializePlayableMission, NOT
# leftover Mission07 StartNextWave, NOT leftover
...
# leftover Mission07 StartNextWave, leftover
# Mission07 NotifyThreatDestroyed, leftover
...
# Waves / Destruction / Briefing, not Environment,
...
# Waves #669, not leftover Harbor Mission02.
...
# #668/#669. Do not reopen leftover
# #670–#673. Do not reopen leftover
# #674–#713.
...
# an isolated Python AdvanceReinforcementTimer
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission07IntegrationDirector::AdvanceReinforcementTimer",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission07AdvanceReinforcementTimerDeclContractTests(unittest.TestCase):
    def test_mission07_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ADVANCE_REINFORCEMENT_TIMER),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ADVANCE_REINFORCEMENT_TIMER)
        self.assertIn("AdvanceReinforcementTimer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_missing_advance_reinforcement_timer_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission07IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ADVANCE_REINFORCEMENT_TIMER)
        self.assertIn("AdvanceReinforcementTimer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ADVANCE_REINFORCEMENT_TIMER)
        self.assertIn("AdvanceReinforcementTimer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("BlueprintCallable", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("Category", ADVANCE_REINFORCEMENT_TIMER)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ADVANCE_REINFORCEMENT_TIMER)
        self.assertIn("AdvanceReinforcementTimer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid AdvanceReinforcementTimer(float DeltaSeconds);\n"
        as_const = "\tbool AdvanceReinforcementTimer(float DeltaSeconds) const;\n"
        with_arg = "\tbool AdvanceReinforcementTimer(bool bForce);\n"
        renamed = "\tbool AdvanceReinforcement();\n"
        short_name = "\tbool ReinforcementTimer();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ADVANCE_REINFORCEMENT_TIMER)
            self.assertIn("AdvanceReinforcementTimer", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_advance_reinforcement_timer_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ADVANCE_REINFORCEMENT_TIMER),
            ADVANCE_REINFORCEMENT_TIMER,
        )
        self.assertTrue(has_declaration(section, ADVANCE_REINFORCEMENT_TIMER))
        self.assertEqual(
            declaration_count(section, ADVANCE_REINFORCEMENT_TIMER),
            1,
        )
        self.assertTrue(
            ADVANCE_REINFORCEMENT_TIMER.startswith("bool "),
            ADVANCE_REINFORCEMENT_TIMER,
        )
        self.assertTrue(
            ADVANCE_REINFORCEMENT_TIMER.endswith(";"),
            ADVANCE_REINFORCEMENT_TIMER,
        )
        self.assertIn("AdvanceReinforcementTimer", ADVANCE_REINFORCEMENT_TIMER)
        self.assertIn("(float DeltaSeconds)", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("TArray<", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("TObjectPtr", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("TSoftObjectPtr", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("INDEX_NONE", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("UFUNCTION", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("{", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("}", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("return ", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("const;", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("Root", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("Briefing", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("AudioDirector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("RadioChatter", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("SortiePresentation", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("CampaignDefinition", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("MissionDefinition", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("Readiness", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("bAutoInitialize", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("bAllowBoundedActorSpawning", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("bAutoLaunchAfterBriefing", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("PathfinderSpawnLocation", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetAircraft", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("BindRuntimeActors", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("HandleDroneCityImpact", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("InitializePlayableMission", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("StartNextWave", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("AdvanceConvoyByDistance", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("NotifyConvoyDamage", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetWaveState", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetConvoyRouteState", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetDayBeatKit", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetNightBeatKit", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("StartSearchlightWindow", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("AdvanceSearchlightTrack", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("NotifySubstationDamage", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetRemainingThreatsInWave", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetSearchlightRuntime", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetSubstationIntegrity", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("StartPayloadWindow", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("AdvancePayloadWindow", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("TryJamActivePayload", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("NotifyAirfieldTargetDamage", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetPayloadWindow", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetTargetRuntime", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetSurvivingTargetCount", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetDayBeatKind", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetDayBeatIndex", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("TickDayBeatKit", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("HandleBossPhaseChanged", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ESkyguardMission06WaveState", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ESkyguardMission07WaveState", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ESkyguardSearchSector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("FSkyguardSearchTrackRuntime", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ClassifyFalseTrack", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ConfirmRadarGhostIdentification", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("NotifyProtectedTargetDamage", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("NotifyThreatDestroyed", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetSearchSector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetClassifiedFalseTrackCount", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("IsHostileContactConfirmed", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetProtectedTarget", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetReinforcementTimeRemaining", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetNightBeatKind", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetNightBeatIndex", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("TickNightBeatKit", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ESkyguardAirfieldTarget", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ConvoyRuntimeAnchor", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("RoadHunterSpawnLocation", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ConfigureMissionDefinition", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("NotifyObjectiveProgress", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("NotifyProtectedAssetFailed", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("SynchronizeRuntimeState", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("IsCorePlayableReady", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetReadiness", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetObjectiveRuntime", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetGunner", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetPathfinder", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetMissionId", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ValidateMissionContract", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ConfigureFromMission", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("AdvanceBriefing", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("SetAssetsReady", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("AcknowledgeAndLaunch", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("CanLaunch", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetElapsedSeconds", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetBriefingState", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetMinimumWarmupSeconds", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetBriefingText", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetRadioChatter", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetPresentation", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("GetMissionTitle", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("AcknowledgeBriefing", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("LaunchSortie", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("HullCollider", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("OpticalTracker", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("WeaponServo", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("CountermeasurePod", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("MinHeightFromOriginCm", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("RadarNode", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ESkyguardMission02WaveState", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("HarborIndustrial", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("MaxIntegrity", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("CurrentIntegrity", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("FillAndFinalize", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("FillAndFail", ADVANCE_REINFORCEMENT_TIMER)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tAdvanceReinforcementTimer(float DeltaSeconds);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_name = (
            "public:\n"
            "\tbool AdvanceReinforcementTimer(\n"
            "\t\tfloat DeltaSeconds);\n"
            "};\n"
        )
        wrap_ufunction = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_BOSS} {ADVANCE_REINFORCEMENT_TIMER}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ADVANCE_REINFORCEMENT_TIMER}\n}}\n"
        self.assertTrue(has_declaration(one_line, ADVANCE_REINFORCEMENT_TIMER))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADVANCE_REINFORCEMENT_TIMER),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{ADVANCE_REINFORCEMENT_TIMER}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ADVANCE_REINFORCEMENT_TIMER)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_advance_reinforcement_timer_cpp_body(self) -> None:
        locked_only = f"{ADVANCE_REINFORCEMENT_TIMER}\n"
        self.assertNotIn("{", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("}", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("return ", ADVANCE_REINFORCEMENT_TIMER)
        ...
        self.assertNotIn("return false", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("return true", ADVANCE_REINFORCEMENT_TIMER)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{ADVANCE_REINFORCEMENT_TIMER}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_REINFORCEMENT_TIMER)
        self.assertIn("AdvanceReinforcementTimer", ADVANCE_REINFORCEMENT_TIMER)
        self.assertTrue(ADVANCE_REINFORCEMENT_TIMER.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, ADVANCE_REINFORCEMENT_TIMER)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("CurrentIntegrity", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("SkyguardApacheAircraft.h", ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ADVANCE_REINFORCEMENT_TIMER}\n"
        self.assertEqual(
            require_declaration(locked_only, ADVANCE_REINFORCEMENT_TIMER),
            ADVANCE_REINFORCEMENT_TIMER,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", ADVANCE_REINFORCEMENT_TIMER)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", ADVANCE_REINFORCEMENT_TIMER)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 AdvanceReinforcementTimer method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ADVANCE_REINFORCEMENT_TIMER}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ADVANCE_REINFORCEMENT_TIMER.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ADVANCE_REINFORCEMENT_TIMER.lower(),
                "mission07 AdvanceReinforcementTimer contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ADVANCE_REINFORCEMENT_TIMER)

    def test_contract_is_advance_reinforcement_timer_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(ADVANCE_REINFORCEMENT_TIMER.startswith("bool "))
        self.assertIn("AdvanceReinforcementTimer", ADVANCE_REINFORCEMENT_TIMER)
        self.assertTrue(ADVANCE_REINFORCEMENT_TIMER.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)

    def test_sibling_director_fields_do_not_satisfy_advance_reinforcement_timer(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, ADVANCE_REINFORCEMENT_TIMER)
            )
            self.assertNotEqual(ADVANCE_REINFORCEMENT_TIMER, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("AdvanceReinforcementTimer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, ADVANCE_REINFORCEMENT_TIMER))
        locked_only = f"{ADVANCE_REINFORCEMENT_TIMER}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool AdvanceReinforcementTimer(float DeltaSeconds) { return false; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_advance_reinforcement_timer"
                "_decl_contract.py"
            )
        )
        self.assertIn(
