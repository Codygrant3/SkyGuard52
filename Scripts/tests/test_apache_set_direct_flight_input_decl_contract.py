...
# or lock the SetDirectFlightInput body in the .cpp.
# origin/main public form is split across lines
# (`void SetDirectFlightInput(` then `float Collective,`
# `float Yaw,` `float CyclicPitch,` `float CyclicRoll);`);
# accept that split-line form, a one-line prototype, other
# wraps, and an inline
...
# SetOrbitFocus, FaceWorldLocation, SetSensorView, or
# SetFirstPersonInterior siblings. Stay off mount getters,
...
# SetFirstPersonInterior, GetForwardSpeed, ApplyDamage,
# GetDamageFraction, ApplySystemHit, ApplyHit,
# IsSensorLive, IsThermalAvailable,
...
# HandleDebriefKey, leftover IssuePilotCommand sibling
# in this wave, leftover GetPilotCommand sibling in
# this wave, leftover GetPilotConfirmationsIssued
# sibling in this wave, leftover SetOrbitFocus
# sibling in this wave, leftover FaceWorldLocation
# sibling in this wave, leftover SetSensorView sibling
# in this wave, leftover SetFirstPersonInterior sibling
# in this wave, leftover GetForwardSpeed sibling in
# this wave, leftover ApplyDamage sibling in this wave,
# and leftover SortiePresentationWidgets
# stay sibling-only.
SET_DIRECT_FLIGHT_INPUT = (
...
# SetDirectFlightInput declaration contract on
...
# IssuePilotCommand sibling, leftover GetPilotCommand
# sibling, leftover GetPilotConfirmationsIssued
# sibling, leftover SetOrbitFocus sibling, leftover
# FaceWorldLocation sibling, leftover SetSensorView
# sibling, leftover SetFirstPersonInterior sibling,
# leftover GetForwardSpeed sibling, leftover
# ApplyDamage sibling, and dirty workspace paths.
LOCKED = {
...
# wave, leftover GetForwardSpeed sibling in this
# wave, leftover ApplyDamage sibling in this wave,
...
SET_ORBIT_FOCUS_NOT_LOCKED = (
    "void SetOrbitFocus(const FVector& WorldLocation);",
)
FACE_WORLD_NOT_LOCKED = (
    "void FaceWorldLocation(const FVector& WorldLocation);",
)
SENSOR_VIEW_NOT_LOCKED = (
    "void SetSensorView(bool bInSensor);",
)
FIRST_PERSON_INTERIOR_NOT_LOCKED = (
    "void SetFirstPersonInterior(bool bInterior);",
)
...
# unlocked. Do not create or edit that file.
LEFTOVER_FACE_WORLD_NOT_LOCKED = (
...
# Leftover SetFirstPersonInterior sibling in this wave stays
# unlocked. Do not create or edit that file.
...
# .cpp SetDirectFlightInput body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::SetDirectFlightInput",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheSetDirectFlightInputDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SET_DIRECT_FLIGHT_INPUT),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("SetDirectFlightInput", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_missing_set_direct_flight_input_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("SetDirectFlightInput", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("SetDirectFlightInput", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("UFUNCTION", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("Category", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("BlueprintCallable", SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("SetDirectFlightInput", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetDirectFlightInput;\n"
        wrong_return_bool = (
            "\tbool SetDirectFlightInput("
            "float Collective, float Yaw, float CyclicPitch, "
            "float CyclicRoll);\n"
        )
        wrong_return_int = (
            "\tint32 SetDirectFlightInput("
            "float Collective, float Yaw, float CyclicPitch, "
            "float CyclicRoll);\n"
        )
        added_const = (
            ...
            "float Collective, float Yaw, float CyclicPitch, "
            "float CyclicRoll) const;\n"
        )
        wrong_type = (
            "\tvoid SetDirectFlightInput("
            "int Collective, float Yaw, float CyclicPitch, "
            "float CyclicRoll);\n"
        )
        missing_cyclic = (
            "\tvoid SetDirectFlightInput("
        ...
        wrong_value = (
            "\tvoid SetDirectFlightInput("
            "FVector Collective, float Yaw, float CyclicPitch, "
            "float CyclicRoll);\n"
        )
        added_arg = (
            "\tvoid SetDirectFlightInput("
            "float Collective, float Yaw, float CyclicPitch, "
            "float CyclicRoll, float Extra);\n"
        )
        no_args = "\tvoid SetDirectFlightInput();\n"
        leftover_orbit = (
            "\tvoid SetOrbitFocus(const FVector& WorldLocation);\n"
        )
        ...
        leftover_sensor = "\tvoid SetSensorView(bool bInSensor);\n"
        leftover_interior = (
            "\tvoid SetFirstPersonInterior(bool bInterior);\n"
        )
        leftover_speed = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_DIRECT_FLIGHT_INPUT)
            self.assertIn("SetDirectFlightInput", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_direct_flight_input_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, SET_DIRECT_FLIGHT_INPUT))
        self.assertEqual(
            declaration_count(section, SET_DIRECT_FLIGHT_INPUT),
            1,
        )
        self.assertTrue(
            SET_DIRECT_FLIGHT_INPUT.startswith("void "),
            SET_DIRECT_FLIGHT_INPUT,
        )
        self.assertTrue(
            SET_DIRECT_FLIGHT_INPUT.endswith(";"),
            SET_DIRECT_FLIGHT_INPUT,
        )
        self.assertIn("SetDirectFlightInput(", SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("float Collective", SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("float Yaw", SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("float CyclicPitch", SET_DIRECT_FLIGHT_INPUT)
        self.assertIn("float CyclicRoll", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("INDEX_NONE", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("{", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("}", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("return ", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn(" const", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("SetOrbitFocus", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("SetRotorPower", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("AimChinTurret", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetChinMuzzleLocation", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("IssuePilotCommand", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetPilotCommand", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn(
            "GetPilotConfirmationsIssued",
            SET_DIRECT_FLIGHT_INPUT,
        )
        self.assertNotIn("FaceWorldLocation", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("SetSensorView", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("SetFirstPersonInterior", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetForwardSpeed", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetChinTurret", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetRotorPowerScale", SET_DIRECT_FLIGHT_INPUT)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetDirectFlightInput(float Collective, float Yaw, "
        ...
        one_line = f"{{\npublic:\n\t{SET_DIRECT_FLIGHT_INPUT}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, SET_DIRECT_FLIGHT_INPUT)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SET_DIRECT_FLIGHT_INPUT),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_DIRECT_FLIGHT_INPUT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_direct_flight_input_cpp_body(
        self,
    ) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        self.assertNotIn("{", SET_DIRECT_FLIGHT_INPUT)
        ...
        self.assertNotIn("return false", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("return true", SET_DIRECT_FLIGHT_INPUT)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetGunnerMount", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetEyeMount", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetWeaponMount", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetChinTurret", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetPilotMount", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetSensorTurret", SET_DIRECT_FLIGHT_INPUT)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetChinMuzzleLocation", SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_contract_does_not_relock_set_orbit_focus(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        for neighbor in SET_ORBIT_FOCUS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("SetOrbitFocus", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("SetOrbitFocus", locked_only)

    def test_contract_does_not_relock_face_world_location(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        for neighbor in FACE_WORLD_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("FaceWorldLocation", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("FaceWorldLocation", locked_only)

    def test_contract_does_not_relock_sensor_view(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
    ...
    def test_contract_does_not_relock_first_person_interior(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        for neighbor in FIRST_PERSON_INTERIOR_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("SetFirstPersonInterior", SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("ApplyDamage", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("GetDamageFraction", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("ApplySystemHit", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("ApplyHit", SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("IsSensorLive", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("IsThermalAvailable", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("IsSensorViewActive", SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("ESkyguardApacheSystem", SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_contract_does_not_relock_leftover_face_world_sibling(
        self,
    ) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        for token in LEFTOVER_FACE_WORLD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn(
            "test_apache_face_world_location_decl_contract.py",
    ...
    def test_contract_does_not_relock_leftover_set_first_person_interior_sibling(
        self,
    ) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("Pursuit", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("OrbitLeft", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("AttackRun", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("FaceTarget", SET_DIRECT_FLIGHT_INPUT)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_DIRECT_FLIGHT_INPUT)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("USkyguardBriefingWidget", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            SET_DIRECT_FLIGHT_INPUT,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_DIRECT_FLIGHT_INPUT)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            SET_DIRECT_FLIGHT_INPUT,
        )
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_DIRECT_FLIGHT_INPUT}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_DIRECT_FLIGHT_INPUT),
            SET_DIRECT_FLIGHT_INPUT,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SetDirectFlightInput contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, SET_DIRECT_FLIGHT_INPUT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache SetDirectFlightInput contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_DIRECT_FLIGHT_INPUT)

    def test_contract_is_set_direct_flight_input_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", SET_DIRECT_FLIGHT_INPUT)
        self.assertNotIn("{", SET_DIRECT_FLIGHT_INPUT)
        self.assertTrue(SET_DIRECT_FLIGHT_INPUT.startswith("void "))
        self.assertTrue(SET_DIRECT_FLIGHT_INPUT.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
