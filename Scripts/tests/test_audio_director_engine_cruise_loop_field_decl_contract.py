# THIS IS leftover-safe USkyguardAudioDirectorComponent EngineCruiseLoop.
...
# then BARE `TSoftObjectPtr<USoundBase> EngineCruiseLoop;`
# (no initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Audio|Engine`. EditAnywhere IS present.
# BlueprintReadWrite IS present. There is NO default.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if Category is missing, or if specifiers
# drop to VisibleAnywhere / BlueprintReadOnly or stay
# Category="Skyguard" without |Audio|Engine.
...
# Stop BEFORE sibling EnginePowerLoop as a claimed slot.
# Do NOT claim EngineIdleLoop, EnginePowerLoop, PropellerLoop,
# OpenCockpitWindLoop, GlobalVoiceLimit, EventDefinitions,
# CockpitExteriorAttenuation, CockpitLowPassHz, ProductionBank,
# ProductionBankAsset.
# Skip leftover analog PropellerLoop (PropSpinner leftover).
...
# (different class). Skip private members.
# Keep leftover analog bulk in LOCKED_SCRIPTS including
# audio-procedural-bank-empty-fail-closed,
# audio-harness-fail-closed, procedural-bank unique
# development UPROPERTY, harness unique thresholds,
# coastal unique actor UPROPERTY.
# Clone wrap was VisibleAnywhere BlueprintReadOnly
# Category Skyguard wrapping TObjectPtr Boom — RETARGET
# hard: EditAnywhere BlueprintReadWrite
# Category Skyguard|Audio|Engine wrapping
# `TSoftObjectPtr<USoundBase> EngineCruiseLoop;`.
# Parse start protected: RETARGET hard to public: after
# GENERATED_BODY.
# LOCKED_DECL is `TSoftObjectPtr<USoundBase> EngineCruiseLoop;`
# NOT `FName WeatherIdentity;` and NOT `160.f` and NOT
# leftover Boom and NOT MinimumMeasuredSamples / SampleRate /
# GeneratedByteBudget / VegetationStartCullDistanceCm /
# EngineIdleLoop / EnginePowerLoop / PropellerLoop /
# OpenCockpitWindLoop / GlobalVoiceLimit AS THE LOCKED DECL.
...
# or still Category="Skyguard" without |Audio|Engine.
# This slot has NO initializer. Fail-closed if decl gains
# `= nullptr` / `= false` / `= true`.
...
TARGET = "TSoftObjectPtr<USoundBase> EngineCruiseLoop;"
TARGET_WRONG_BARE = "TSoftObjectPtr<USoundBase> EngineIdleLoop;"
TARGET_WRONG_INIT = "TSoftObjectPtr<USoundBase> EngineCruiseLoop = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundBase> EngineCruiseLoop = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundBase> EngineCruiseLoop = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundBase> EngineCruiseLoop = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> EngineCruiseLoop;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> EngineCruiseLoop;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> EngineCruiseLoop;"
...
TARGET_WRONG_PROPELLER = "TSoftObjectPtr<USoundBase> PropellerLoop;"
TARGET_WRONG_WIND = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;"
TARGET_WRONG_VOICE = "int32 GlobalVoiceLimit = 24;"
TARGET_WRONG_EVENTS = "TArray<FSkyguardAudioEventDefinition> EventDefinitions;"
TARGET_WRONG_PROD_BANK = "TObjectPtr<USkyguardAudioProductionBank> ProductionBank;"
TARGET_WRONG_PROD_ASSET = (
    "TSoftObjectPtr<USkyguardAudioProductionBank> ProductionBankAsset;"
)
TARGET_WRONG_ATTENUATION = "float CockpitExteriorAttenuation = 0.72f;"
TARGET_WRONG_LOWPASS = "float CockpitLowPassHz = 7200.f;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "EnginePowerLoop"
STOP_BEFORE_EPIC_SHRUB = "EnginePowerLoop"
...
SIBLING_LOOK_PITCH_MIN = "GeneratedByteBudget"
...
SIBLING_ENGINE_POWER = "EnginePowerLoop"
SIBLING_PROPELLER = "PropellerLoop"
SIBLING_WIND_LOOP = "OpenCockpitWindLoop"
SIBLING_VOICE_LIMIT = "GlobalVoiceLimit"
SIBLING_EVENT_DEFS = "EventDefinitions"
SIBLING_PROD_BANK = "ProductionBank"
SIBLING_PROD_ASSET = "ProductionBankAsset"
SIBLING_ATTENUATION = "CockpitExteriorAttenuation"
SIBLING_LOWPASS = "CockpitLowPassHz"
...
LEFTOVER_AUDIO_DIRECTOR_IDLE = (
    "Scripts/tests/test_audio_director_engine_idle_loop"
    "_field_decl_contract.py"
)
LEFTOVER_AUDIO_DIRECTOR_POWER = (
...
LEFTOVER_AUDIO_DIRECTOR_VOICE = (
    "Scripts/tests/test_audio_director_global_voice_limit"
    "_field_decl_contract.py"
)
LEFTOVER_AUDIO_PROCEDURAL_SAMPLE_RATE = (
...
LEFTOVER_PRODUCTION_ROUTING_ATTENUATION = (
    "Scripts/tests/test_production_audio_routing_cockpit_exterior_attenuation"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_ROUTING_LOWPASS = (
...
ENGINE_CRUISE_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EngineCruiseLoop\s*;"
)
ENGINE_CRUISE_INIT_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EngineCruiseLoop\s*="
)
SAMPLE_RATE_DECL_RE = re.compile(
    r"int32\s+SampleRate\s*=\s*48000\s*;"
)
SAMPLE_RATE_BARE_RE = re.compile(
    r"int32\s+SampleRate\s*;"
)
BOOM_DECL_RE = re.compile(
    r"TObjectPtr<USpringArmComponent>\s+Boom\b"
)
ENGINE_IDLE_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EngineIdleLoop\b"
)
ENGINE_POWER_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EnginePowerLoop\b"
)
PROPELLER_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+PropellerLoop\b"
)
WIND_LOOP_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+OpenCockpitWindLoop\b"
)
VOICE_LIMIT_DECL_RE = re.compile(
    r"int32\s+GlobalVoiceLimit\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover `= 160.f`, leftover analog Boom,
    # leftover SampleRate / MinimumMeasuredSamples /
    # GeneratedByteBudget, leftover EngineIdleLoop /
    # EnginePowerLoop / PropellerLoop /
    # OpenCockpitWindLoop / GlobalVoiceLimit, or an
    # initializer on EngineCruiseLoop as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ENGINE_CRUISE_DECL_RE.search(compact) is None:
        return False
    if ENGINE_CRUISE_INIT_RE.search(compact):
        return False
    if BOOM_DECL_RE.search(compact):
        return False
    if ENGINE_IDLE_DECL_RE.search(compact):
        return False
    if ENGINE_POWER_DECL_RE.search(compact):
        return False
    if PROPELLER_DECL_RE.search(compact):
        return False
    if WIND_LOOP_DECL_RE.search(compact):
        return False
    if VOICE_LIMIT_DECL_RE.search(compact):
        return False
    if SAMPLE_RATE_DECL_RE.search(compact):
        return False
    if SAMPLE_RATE_BARE_RE.search(compact):
        return False
...
def field_identifier(field_text: str) -> str:
    ...
    match = re.search(
        r"(?:TSoftObjectPtr<[^>]+>|TObjectPtr<[^>]+>|TSubclassOf<[^>]+>|float|int32|bool|"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr<USoundBase>\s+EngineCruiseLoop\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundBase> EngineCruiseLoop "
...
class AudioDirectorEngineCruiseLoopFieldDeclContractTests(unittest.TestCase):
    def test_audio_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EngineCruiseLoop"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENGINE_IDLE), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT), section)
        self.assertTrue(has_identifier(body, SIBLING_ENGINE_POWER), body)
        self.assertFalse(
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_PROCEDURAL_BANK_CLASS)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EngineCruiseLoop"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("EngineCruiseLoop", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_engine_cruise_loop_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Audio|Engine"', section)
        ...
        self.assertIn('Category="Skyguard|Audio|Engine"', specifiers)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TSoftObjectPtr<USoundBase> EngineCruiseLoop;", compact_origin)
        self.assertNotIn("EngineCruiseLoop = nullptr", compact_origin)
    ...
    def test_engine_cruise_loop_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSoftObjectPtr<USoundBase> EngineCruiseLoop"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
        self.assertNotIn("48000", LOCKED_DECL)
        ...
        self.assertIn("TSoftObjectPtr", LOCKED_DECL)
        self.assertIn("USoundBase", LOCKED_DECL)
        self.assertNotIn("TObjectPtr<USpringArmComponent>", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TSoftObjectPtr<USoundBase> EngineCruiseLoop;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PROPELLER)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 SampleRates = 48000;\n",
            "\tfloat SampleRate = 48000.f;\n",
            "\tbool SampleRate = true;\n",
            "\tint32 SampleRate = " + forty + ";\n",
            "\tint32 SampleRate = " + eighty + ";\n",
            f"\t{TARGET_WRONG_SEED}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Audio|Engine"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Engine"', apache_specs)
        self.assertNotIn('Category="Skyguard|Audio|Acceptance"', origin)
        self.assertNotIn('Category="Skyguard|Audio|Development"', origin)
        self.assertIn('Category="Skyguard|Apache"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
    ...
    def test_does_not_claim_sibling_engine_power_loop_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "EngineCruiseLoop"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "EngineCruiseLoop"))
        self.assertTrue(has_identifier(section, "EngineCruiseLoop"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_IDLE))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_IDLE))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, SIBLING_PROD_BANK))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio director EngineCruiseLoop field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_director_engine_cruise_loop_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioProceduralBankComponent.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AUDIO_DIRECTOR_IDLE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_PRODUCTION_BANK_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_ROUTING_ATTENUATION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION01_AUDIO_DIRECTOR, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_PROCEDURAL_SAMPLE_RATE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_BANK_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_HARNESS_EMPTY, LOCKED_SCRIPTS)
