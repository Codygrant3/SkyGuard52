...
# INDEX_NONE or lock StarboardGuidanceArray
# construction in the .cpp. This is a FIELD
# contract on ASkyguardLastFlightBoss, not a
...
# Stay off leftover LastFlight sibling
# methods already drafted
...
# GetObjectiveMilestonesReached. Stay off
# leftover emergency-finish surfaces. Stay
# off leftover BossDrone ApplyWeaponHit /
# IssuePilotCommand / GetBossPhase /
# GetDefeatDebrisPieceCount /
# GetMaxDefeatDebrisPieces /
# NotifyWeakPointDestroyed / GetTelemetry.
# Stay off leftover Apache HullCollider
# field #425. Stay off leftover BossDrone
# Root / WeakPoints / BodyMesh field
...
# Apache IssuePilotCommand. Leftover
# briefing / debrief widget isolated
...
# flare / HUD, leftover ApacheSystem /
# weapon stations / leftover roster /
# loadout / lock-phase, leftover drafts
# #56–#64, leftover isolated-test drafts
# #107–#479 including leftover
...
# style HarborIndustrial (leftover enum,
# not a Harbor 40/80 retune), leftover
...
# SortiePresentationWidgets stay
...
# Harbor clock names may be scanned in
# this relevant public section and must
# be absent. Do not open
# SkyguardBossWeakPointComponent.h as the
# locked header. Do not lock leftover
# accept flags on that component.
STARBOARD_GUIDANCE_ARRAY_FIELD = (
...
# Leftover GetFinaleStage method contract
# stays sibling-only. Do not lock that
# method here.
GET_FINALE_STAGE = (
...
# Leftover #56–#64 plus
# SkyguardLastFlightBoss production files.
# This lane only adds an isolated Python
# StarboardGuidanceArray field declaration
# contract on ASkyguardLastFlightBoss. Stay
# off leftover sibling
# OpenGuidanceArrayExposure /
# BeginTerminalStrikeCycle /
# IssueClimbCommand /
# DivertWreckFromCivilians /
...
# GetFinaleStage / ESkyguardLastFlightStage,
# leftover sibling PortGuidanceArray field,
# leftover emergency-finish surfaces,
...
# fields, leftover apache aircraft
# isolated contracts, leftover settings /
# input-capture contracts, leftover CPG
# HUD / sight HUD, leftover drafts
# #56–#64, leftover isolated-test drafts
# #107–#479, leftover
# searchlight-track-runtime-defaults
# #7347, leftover BlackKite siblings,
# leftover RadarGhost siblings, leftover
# LifelineHunter siblings, leftover
# Tempest siblings, leftover IronRain
# siblings, leftover BossDrone fields /
# debris getters, leftover patrol-ship
# empty fail-closed #5382, leftover
# ApacheSystem enum values, leftover
# roster enum values, leftover Harbor
# clocks, leftover skyline
# HarborIndustrial, leftover
# DebriefWidget isolated contracts,
# leftover BriefingWidget isolated
# contracts, leftover gun-fire camera
# shake, leftover sortie-hud-host
# fail-closed, leftover Apache
# HullCollider field, and dirty
# workspace paths.
LOCKED = {
...
# Isolated-test drafts stay off this lane.
# Leftover LastFlight methods / leftover
# sibling PortGuidanceArray field /
# leftover IronRain / leftover debris
# getters / leftover BossDrone fields /
# leftover Apache HullCollider field stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# Neighbors in the same public section.
# Presence is not locked here. Sibling
# PortGuidanceArray, leftover LastFlight
# methods, leftover GetFinaleStage,
# leftover emergency-finish surfaces,
# leftover sibling LastFlight fields,
# leftover USkyguardBossWeakPointComponent
# member fields, leftover mount getters,
# leftover GetChinMuzzleLocation, leftover
# BossDrone fields / debris getters,
# leftover IronRain, leftover Apache
# HullCollider, and leftover apache /
# Gunner / settings helpers stay
# sibling-only.
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::StarboardGuidanceArray",
    "SkyguardLastFlightBoss.cpp",
...
class LastFlightStarboardGuidanceArrayFieldDeclContractTests(
    unittest.TestCase
):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, STARBOARD_GUIDANCE_ARRAY_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertIn("StarboardGuidanceArray", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_missing_starboard_guidance_array_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLastFlightBoss();\n"
    ...
    def test_missing_type_fails_closed(self) -> None:
        name_only = "\tStarboardGuidanceArray;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(name_only, STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertIn("StarboardGuidanceArray", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(name_only, STARBOARD_GUIDANCE_ARRAY_FIELD)
        )

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_STARBOARD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertIn("StarboardGuidanceArray", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission10|Boss"', section)
        self.assertTrue(
            has_declaration(section, STARBOARD_GUIDANCE_ARRAY_FIELD),
            section,
        )
        self.assertNotIn("UPROPERTY", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("VisibleAnywhere", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("BlueprintReadOnly", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("Category", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_STARBOARD)
        self.assertNotIn("BlueprintCallable", UPROPERTY_STARBOARD)
        self.assertIn("Skyguard|Mission10|Boss", UPROPERTY_STARBOARD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* StarboardGuidanceArray;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> StarboardGuidanceArray;\n"
        )
        weak = (
        ...
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardGuidanceArray = nullptr;\n"
        )
        wrong_name = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "PortGuidanceArray;\n"
        )
        leftover_port = f"\t{PORT_GUIDANCE_ARRAY_FIELD}\n"
        leftover_min = f"\t{MINIMUM_CIVILIAN_SEPARATION}\n"
    ...
    def test_starboard_guidance_array_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, STARBOARD_GUIDANCE_ARRAY_FIELD),
            1,
        )
        self.assertTrue(
            STARBOARD_GUIDANCE_ARRAY_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
        self.assertTrue(
            STARBOARD_GUIDANCE_ARRAY_FIELD.endswith(";"),
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
        self.assertIn("StarboardGuidanceArray", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
        self.assertIn("TObjectPtr<", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("TArray<", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("=", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("INDEX_NONE", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("UFUNCTION", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("GetFinaleStage", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("{", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("}", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("return ", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("PortGuidanceArray", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn(
            "MinimumCivilianSeparationMeters",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
        ...
        self.assertNotIn("HullCollider", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("WeakPoints", STARBOARD_GUIDANCE_ARRAY_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, STARBOARD_GUIDANCE_ARRAY_FIELD)
        for flag in leftover_weak_point_accept_flags():
            self.assertNotIn(flag, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_STARBOARD} {STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{STARBOARD_GUIDANCE_ARRAY_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, STARBOARD_GUIDANCE_ARRAY_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, STARBOARD_GUIDANCE_ARRAY_FIELD),
    ...
    def test_assigned_starboard_guidance_array_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardGuidanceArray = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertIn("StarboardGuidanceArray", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(assigned, STARBOARD_GUIDANCE_ARRAY_FIELD)
        )

    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        port = f"\t{PORT_GUIDANCE_ARRAY_FIELD}\n"
        bay = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "PortStrikeBayMechanism;\n"
        )
        ...
        minimum = f"\t{MINIMUM_CIVILIAN_SEPARATION}\n"
        for region in (port, bay, core, minimum):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, STARBOARD_GUIDANCE_ARRAY_FIELD)
            self.assertIn("StarboardGuidanceArray", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, STARBOARD_GUIDANCE_ARRAY_FIELD)
            )

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            STARBOARD_GUIDANCE_ARRAY_FIELD.startswith("UFUNCTION"),
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", STARBOARD_GUIDANCE_ARRAY_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_starboard_guidance_array_cpp_body(
        self,
    ) -> None:
        locked_only = f"{STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
        self.assertNotIn("{", STARBOARD_GUIDANCE_ARRAY_FIELD)
        ...
        self.assertNotIn("CreateDefaultSubobject", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )

    def _assert_tokens_stay_off(self, tokens: tuple[str, ...]) -> None:
        locked_only = f"{STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
        for token in tokens:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_sibling_port_guidance_array(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_PORT_GUIDANCE_ARRAY_NOT_LOCKED)
        self.assertNotIn("PortGuidanceArray", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn(
            "test_last_flight_port_guidance_array_field_decl_contract.py",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
    ...
    def test_contract_does_not_relock_sibling_last_flight_fields(self) -> None:
        self._assert_tokens_stay_off(SIBLING_FIELDS_NOT_LOCKED)
        self.assertNotIn("CommandCore", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("PortEngine", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("Jammer", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_minimum_civilian_separation(self) -> None:
        locked_only = f"{STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
    ...
    def test_contract_does_not_relock_get_finale_stage_sibling(self) -> None:
        self._assert_tokens_stay_off(GET_FINALE_STAGE_NOT_LOCKED)
        self.assertNotIn("GetFinaleStage", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn(
            "ESkyguardLastFlightStage",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
    ...
    def test_contract_does_not_relock_leftover_last_flight_methods(
        self,
    ) -> None:
        self._assert_tokens_stay_off(BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED)
        self._assert_tokens_stay_off(OPEN_GUIDANCE_ARRAY_EXPOSURE_NOT_LOCKED)
        self._assert_tokens_stay_off(ISSUE_CLIMB_COMMAND_NOT_LOCKED)
        self._assert_tokens_stay_off(DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED)
        self._assert_tokens_stay_off(SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED)
        self._assert_tokens_stay_off(GET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED)
        self._assert_tokens_stay_off(IS_CLIMB_COMMAND_ISSUED_NOT_LOCKED)
        self._assert_tokens_stay_off(IS_WRECK_DIVERTED_NOT_LOCKED)
        self._assert_tokens_stay_off(GET_OBJECTIVE_MILESTONES_REACHED_NOT_LOCKED)
        self._assert_tokens_stay_off(GETTERS_NOT_LOCKED)
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_boss_drone_fields(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_BOSS_DRONE_FIELDS_NOT_LOCKED)
        self.assertNotIn(
            "test_boss_drone_root_field_decl_contract.py",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_hull_collider(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_HULL_COLLIDER_NOT_LOCKED)
        self.assertNotIn("HullCollider", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_searchlight_runtime(
        self,
    ) -> None:
        self._assert_tokens_stay_off(LEFTOVER_SEARCHLIGHT_RUNTIME_NOT_LOCKED)

    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_BLACK_KITE_NOT_LOCKED)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_RADAR_GHOST_NOT_LOCKED)
        self.assertNotIn("SetContactIdentified", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_TEMPEST_NOT_LOCKED)
        self.assertNotIn("SetLightningExposed", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_IRON_RAIN_NOT_LOCKED)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
        self.assertNotIn("ASkyguardIronRainBoss", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_PATROL_SHIP_NOT_LOCKED)
        self.assertNotIn("ASkyguardPatrolShip", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        self._assert_tokens_stay_off(MOUNT_GETTERS_NOT_LOCKED)
        self.assertNotIn("GetGunnerMount", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        self._assert_tokens_stay_off(GET_CHIN_MUZZLE_NOT_LOCKED)
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_empty_fail_closed(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_EMPTY_FAIL_CLOSED_NOT_LOCKED)

    def test_contract_does_not_relock_leftover_chin_muzzle(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_CHIN_MUZZLE_NOT_LOCKED)

    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_OWN_SHIP_NOT_LOCKED)
        self.assertNotIn("ESkyguardApacheSystem", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_cpg_feel(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_CPG_FEEL_NOT_LOCKED)

    def test_contract_does_not_relock_leftover_apache_decl_siblings(
        self,
    ) -> None:
        self._assert_tokens_stay_off(LEFTOVER_APACHE_DECL_NOT_LOCKED)
        self.assertNotIn("FaceWorldLocation", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("IssuePilotCommand", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_settings(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_SETTINGS_NOT_LOCKED)

    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_INPUT_CAPTURE_NOT_LOCKED)

    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_GUNNER_NOT_LOCKED)
        self.assertNotIn("ASkyguardGunner", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_RADAR_NODE_NOT_LOCKED)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_relock_leftover_siblings(self) -> None:
        self._assert_tokens_stay_off(LEFTOVER_NOT_LOCKED)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_GUIDANCE_ARRAY_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        self._assert_tokens_stay_off(WRONG_HARBOR_HEADERS_NOT_SCANNED)
        self.assertNotIn("MinHeightFromOriginCm", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("MaxIntegrity", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("CurrentIntegrity", STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, STARBOARD_GUIDANCE_ARRAY_FIELD),
            STARBOARD_GUIDANCE_ARRAY_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("{", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("}", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("return false", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("return true", STARBOARD_GUIDANCE_ARRAY_FIELD)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight StarboardGuidanceArray field contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{STARBOARD_GUIDANCE_ARRAY_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, STARBOARD_GUIDANCE_ARRAY_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                STARBOARD_GUIDANCE_ARRAY_FIELD.lower(),
                "last flight StarboardGuidanceArray contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, STARBOARD_GUIDANCE_ARRAY_FIELD)

    def test_contract_is_starboard_guidance_array_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(STARBOARD_GUIDANCE_ARRAY_FIELD.endswith(";"))
        self.assertNotIn("=", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertNotIn("PortGuidanceArray", STARBOARD_GUIDANCE_ARRAY_FIELD)
        self.assertIn(UPROPERTY_STARBOARD, section)
