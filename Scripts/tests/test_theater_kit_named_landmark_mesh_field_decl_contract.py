# THIS IS leftover-safe ASkyguardCampaignTheaterKit NamedLandmarkMesh.
...
# then BARE `TObjectPtr<UStaticMeshComponent>
# NamedLandmarkMesh;` (no initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Theater`. VisibleAnywhere IS present.
...
# ONLY after `class ASkyguardCampaignTheaterKit`. Stop at
# `private:`.
# Do NOT parse `struct FSkyguardTheaterKitSpec`.
# Do NOT parse the namespace helpers.
# Do NOT contract leftover theater-kit-weather-apply #59
# (`ApplyTheaterKitToWorld` gameplay).
# Do NOT contract sibling fields Root / RoadInstances /
# BuildingInstances / LampInstances / SilhouetteInstances /
# AppliedSpec.
# Do NOT contract UFUNCTION methods.
...
# (ApplyTheaterKitToWorld gameplay).
# THIS IS NOT leftover isolated spec fields #1299-#1314
# (keep those files in LOCKED_SCRIPTS).
# THIS IS NOT leftover actor fields #1315-#1318
# (Root / RoadInstances / LampInstances / BuildingInstances;
# keep those files in LOCKED_SCRIPTS).
...
# NamedLandmarkMesh does not block this isolated UPROPERTY
# decl, same as leftover briefing-card-defaults then
# isolated CardId).
...
# retarget: type is TObjectPtr UStaticMeshComponent, no `=`,
# Category is Skyguard|Theater, parse the actor public section.
# If a clone asserts FName WeatherIdentity / parses the spec
# struct / Category="Skyguard|Campaign", retarget to this lane.
...
TARGET = f"{MESH_TYPE} NamedLandmarkMesh;"
TARGET_WRONG_EQ_NONE = f"{MESH_TYPE} NamedLandmarkMesh = nullptr;"
TARGET_WRONG_FALSE = f"{MESH_TYPE} NamedLandmarkMesh = false;"
TARGET_WRONG_TRUE = f"{MESH_TYPE} NamedLandmarkMesh = true;"
TARGET_WRONG_ZERO = f"{MESH_TYPE} NamedLandmarkMesh = 0.f;"
TARGET_WRONG_FLOAT = "float NamedLandmarkMesh;"
TARGET_WRONG_FNAME = "FName NamedLandmarkMesh;"
TARGET_WRONG_HISM = (
    "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
    "NamedLandmarkMesh;"
)
TARGET_WRONG_ROOT_TYPE = "TObjectPtr<USceneComponent> NamedLandmarkMesh;"
TARGET_WRONG_SPEC_NAMED_LANDMARK = "FName NamedLandmark;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `TObjectPtr<UStaticMeshComponent>
    # NamedLandmarkMesh;`.
    # Do not accept `= nullptr` / `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` / leftover float Health
    # when origin/main has a bare TObjectPtr. Do not accept
    # sibling Root / RoadInstances / leftover spec
    # NamedLandmark fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TObjectPtr<UStaticMeshComponent>"
        r"\s+NamedLandmarkMesh\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+NamedLandmarkMesh\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+NamedLandmarkMesh\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr<USceneComponent>\s+NamedLandmarkMesh\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr<UHierarchicalInstancedStaticMeshComponent>"
        r"\s+NamedLandmarkMesh\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>"
            r"\s+NamedLandmarkMesh\b",
            compact[index:],
        ):
    ...
    raise AssertionError(
        "UPROPERTY for TObjectPtr"
        "<UStaticMeshComponent> "
        "NamedLandmarkMesh is missing from "
...
class TheaterKitNamedLandmarkMeshFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "NamedLandmarkMesh"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("NamedLandmarkMesh", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_named_landmark_mesh_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Theater"', UPROPERTY_VISIBLE_READONLY)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("NamedLandmarkMesh = nullptr", compact_origin)
        self.assertNotIn("NamedLandmarkMesh = NAME_None", compact_origin)
        self.assertNotIn("NamedLandmarkMesh = false", compact_origin)
        self.assertNotIn("NamedLandmarkMesh = true", compact_origin)
        self.assertNotIn("NamedLandmarkMesh = 0.f", compact_origin)
        self.assertNotIn("NamedLandmarkMesh = 160.f", compact_origin)

    def test_named_landmark_mesh_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tTObjectPtr<USceneComponent> {SIBLING_ROOT};\n",
            f"\t{TARGET_WRONG_ROOT_TYPE}\n",
            f"\t{TARGET_WRONG_HISM}\n",
            f"\t{TARGET_WRONG_FNAME}\n",
            f"\t{TARGET_WRONG_SPEC_NAMED_LANDMARK}\n",
            "\tTObjectPtr<UStaticMeshComponent> NamedLandmarkMeshs;\n",
            "\tint32 NamedLandmarkMesh;\n",
            "\tbool NamedLandmarkMesh;\n",
            "\tfloat NamedLandmarkMesh = " + forty + ";\n",
            "\tfloat NamedLandmarkMesh = " + eighty + ";\n",
        )
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("Category", UPROPERTY_VISIBLE_READONLY)
        self.assertIn('Category="Skyguard|Theater"', UPROPERTY_VISIBLE_READONLY)
        self.assertNotIn('Category="Skyguard|Campaign"', UPROPERTY_VISIBLE_READONLY)
        self.assertNotIn("EditAnywhere", UPROPERTY_VISIBLE_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_VISIBLE_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit NamedLandmarkMesh field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "named_landmark_mesh_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_APPLIED_SPEC, locked_only)
        for leftover_fn in leftover_ufunction_tokens():
