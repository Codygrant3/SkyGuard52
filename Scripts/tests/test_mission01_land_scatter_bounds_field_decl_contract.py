...
# INDEX_NONE or lock LandScatterBounds construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# LandScatterBounds distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep LandScatterBounds
...
# (#585), BeachTiles, LandTiles (in-flight),
# RouteExclusion (in-flight), InlandVegetationPCG,
...
# Environment alone, not Mission02 / Boss /
...
# isolated-test drafts #107–#585, leftover Apache
...
# leftover theater-kit #59, leftover #107–#585, plus
# SkyguardMission01EnvironmentDirector production
# files. This lane only adds an isolated Python
# LandScatterBounds field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::LandScatterBounds",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01LandScatterBoundsFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, LAND_SCATTER_BOUNDS_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn("LandScatterBounds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, LAND_SCATTER_BOUNDS_FIELD))

    def test_missing_land_scatter_bounds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn("LandScatterBounds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn("LandScatterBounds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("VisibleAnywhere", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("BlueprintReadOnly", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("Category", LAND_SCATTER_BOUNDS_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn("LandScatterBounds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUBoxComponent* LandScatterBounds;\n"
        )
        as_array = (
            "\tTArray<TObjectPtr<"
            "UBoxComponent>> "
        ...
        scene = "\tTObjectPtr<USceneComponent> LandScatterBounds;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> LandScatterBounds;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> LandScatterBounds;\n"
        )
        weak = (
            "\tTWeakObjectPtr<UBoxComponent> "
            "LandScatterBounds;\n"
        )
        assigned = (
            "\tTObjectPtr<UBoxComponent> "
            "LandScatterBounds = nullptr;\n"
        )
        soft = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, LAND_SCATTER_BOUNDS_FIELD)
            self.assertIn("LandScatterBounds", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_land_scatter_bounds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LAND_SCATTER_BOUNDS_FIELD),
            LAND_SCATTER_BOUNDS_FIELD,
        )
        self.assertTrue(has_declaration(section, LAND_SCATTER_BOUNDS_FIELD))
        self.assertEqual(
            declaration_count(section, LAND_SCATTER_BOUNDS_FIELD),
            1,
        )
        self.assertTrue(
            LAND_SCATTER_BOUNDS_FIELD.startswith(
                "TObjectPtr<UBoxComponent> "
            ),
            LAND_SCATTER_BOUNDS_FIELD,
        )
        self.assertTrue(LAND_SCATTER_BOUNDS_FIELD.endswith(";"), LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn("LandScatterBounds", LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn(
            "UBoxComponent",
            LAND_SCATTER_BOUNDS_FIELD,
        )
        self.assertIn("TObjectPtr<", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("TArray<", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("TSoftObjectPtr", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("=", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("nullptr", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("INDEX_NONE", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("UFUNCTION", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("{", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("}", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("return ", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("OceanTiles", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("LandTiles", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("RouteExclusion", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("bRouteExclusionValid", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("BeachTiles", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("InlandVegetationPCG", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("ProductionLandscape", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("AuthoredPCGGraph", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("Root", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetReadiness", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("Briefing", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("AudioDirector", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("RadioChatter", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("SortiePresentation", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("CampaignDefinition", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("MissionDefinition", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("bAutoInitialize", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetAircraft", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("ConfigureFromMission", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("AdvanceBriefing", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("SetAssetsReady", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("CanLaunch", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetElapsedSeconds", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetBriefingState", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetBriefingText", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetRadioChatter", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetPresentation", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("GetMissionTitle", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("AcknowledgeBriefing", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("LaunchSortie", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("HullCollider", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("OpticalTracker", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("WeaponServo", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("CountermeasurePod", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("RadarNode", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("HarborIndustrial", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("MaxIntegrity", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("CurrentIntegrity", LAND_SCATTER_BOUNDS_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_PCG} {LAND_SCATTER_BOUNDS_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{LAND_SCATTER_BOUNDS_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, LAND_SCATTER_BOUNDS_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, LAND_SCATTER_BOUNDS_FIELD),
            section,
    ...
    def test_assigned_land_scatter_bounds_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UBoxComponent> "
            "LandScatterBounds = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn("LandScatterBounds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, LAND_SCATTER_BOUNDS_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("LandScatterBounds", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, LAND_SCATTER_BOUNDS_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{LAND_SCATTER_BOUNDS_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            LAND_SCATTER_BOUNDS_FIELD.startswith("UFUNCTION"),
            LAND_SCATTER_BOUNDS_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", LAND_SCATTER_BOUNDS_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_land_scatter_bounds_cpp_body(self) -> None:
        locked_only = f"{LAND_SCATTER_BOUNDS_FIELD}\n"
        self.assertNotIn("{", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("}", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("return ", LAND_SCATTER_BOUNDS_FIELD)
        ...
        self.assertNotIn("return false", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("return true", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("CreateDefaultSubobject", LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn("LandScatterBounds", LAND_SCATTER_BOUNDS_FIELD)
        self.assertTrue(
            LAND_SCATTER_BOUNDS_FIELD.startswith(
                "TObjectPtr<UBoxComponent> "
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("CurrentIntegrity", LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, LAND_SCATTER_BOUNDS_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{LAND_SCATTER_BOUNDS_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, LAND_SCATTER_BOUNDS_FIELD),
            LAND_SCATTER_BOUNDS_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAND_SCATTER_BOUNDS_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, LAND_SCATTER_BOUNDS_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 LandScatterBounds field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{LAND_SCATTER_BOUNDS_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, LAND_SCATTER_BOUNDS_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LAND_SCATTER_BOUNDS_FIELD.lower(),
                "mission01 LandScatterBounds contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, LAND_SCATTER_BOUNDS_FIELD)

    def test_contract_is_land_scatter_bounds_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("LandScatterBounds", LAND_SCATTER_BOUNDS_FIELD)
        self.assertTrue(LAND_SCATTER_BOUNDS_FIELD.endswith(";"))
        self.assertNotIn("=", LAND_SCATTER_BOUNDS_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_PCG, section)

    def test_sibling_environment_fields_do_not_satisfy_land_scatter_bounds(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, LAND_SCATTER_BOUNDS_FIELD))
            self.assertNotEqual(LAND_SCATTER_BOUNDS_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_land_scatter_bounds(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
