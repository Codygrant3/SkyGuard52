# THIS IS leftover-safe ASkyguardGunner CpgKneeLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgKneeLeft;`
# (no initializer).
...
# Stop BEFORE sibling CpgKneeRight as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeRight, CpgCanopyBow, Boom,
# GunnerCamera, CpgDash.
# Do NOT claim leftover CpgSeatBack #1595, leftover
# CpgCockpit #1594, leftover CpgStation #1593,
# leftover CpgSeatPan (in-flight sibling).
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgKneeLeft;`
...
# CpgKneeLeft fail-closed if LOCKED_DECL is leftover
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgKneeLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgKneeLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgKneeLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgKneeLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgKneeLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgKneeLeft;"
...
STOP_BEFORE_KNEE_RIGHT = "CpgKneeRight"
...
CPG_SEAT_BACK_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgKneeLeft\s*;"
)
CPG_SEAT_BACK_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgKneeLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgSeatBack / CpgSeatPan / CpgCockpit /
    # CpgStation / CpgKneeRight, or an initializer on
    # CpgKneeLeft as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_SEAT_BACK_DECL_RE.search(compact) is None:
        return False
    if CPG_SEAT_BACK_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_KNEE_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_KNEE_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_KNEE_RIGHT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgKneeLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgKneeLeft "
...
class GunnerCpgKneeLeftFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgKneeLeft"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_KNEE_RIGHT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_KNEE_RIGHT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgKneeLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgKneeLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_knee_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgKneeLeft;", compact_origin)
        self.assertNotIn("CpgKneeLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_knee_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgKneeLeft"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_KNEE_RIGHT}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgKneeLeft;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgKneeLefts;\n",
            "\tint32 CpgKneeLeft = 140;\n",
            "\tbool CpgKneeLeft = true;\n",
            "\tfloat CpgKneeLeft = " + forty + ";\n",
            "\tfloat CpgKneeLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_knee_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(TARGET_WRONG_KNEE_RIGHT, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_KNEE_RIGHT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_KNEE_RIGHT))
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_CANOPY_BOW))
        self.assertFalse(has_identifier(section, SIBLING_CPG_CANOPY_BOW))
        self.assertTrue(has_identifier(section, SIBLING_CPG_SEAT_PAN))
        self.assertTrue(has_identifier(section, SIBLING_CPG_SEAT_BACK))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgKneeLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_knee_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
