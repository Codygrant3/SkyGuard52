# THIS IS leftover-safe FSkyguardMissionLandmarkAnchor LandmarkId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName LandmarkId;` (no initializer).
...
# if Category= is added, if the type is not FName, or
# if an initializer is added.
...
# Do NOT contract sibling fields Role / WorldLocation /
# bMissionExclusive.
...
# retarget: type is FName LandmarkId, no `=`, specifiers
...
TARGET = "FName LandmarkId;"
TARGET_WRONG_EQ_NONE = "FName LandmarkId = NAME_None;"
TARGET_WRONG_FALSE = "FName LandmarkId = false;"
TARGET_WRONG_TRUE = "FName LandmarkId = true;"
TARGET_WRONG_ZERO = "FName LandmarkId = 0.f;"
TARGET_WRONG_FLOAT = "float LandmarkId;"
...
TARGET_WRONG_TEXT = "FText LandmarkId;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName LandmarkId;`.
    # Do not accept `= NAME_None` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` / leftover float Health when
    # origin/main has a bare FName. Do not accept sibling
    # Role / WorldLocation / bMissionExclusive. Do not accept
    # leftover FSkyguardMissionObjectiveAnchor ObjectiveId
    # or leftover FSkyguardObjectiveDefinition /
    # FSkyguardObjectiveProgress ObjectiveId decls outside
    # this struct window.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FName\s+LandmarkId\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LandmarkId\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+LandmarkId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName LandmarkId is missing from "
...
def require_landmark_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} LandmarkId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} LandmarkId"
        )
...
class MissionLandmarkAnchorLandmarkIdFieldDeclContractTests(unittest.TestCase):
    def test_landmark_anchor_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LandmarkId"), section)
    ...
    def test_objective_or_readiness_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LandmarkId"), section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_ANCHOR, section)
        self.assertNotIn(STOP_BEFORE_MAP_READINESS, section)
        self.assertNotIn(ACTOR_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LandmarkId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_landmark_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_landmark_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName LandmarkId;", compact_origin)
        self.assertNotIn("LandmarkId = NAME_None", compact_origin)
        self.assertNotIn("LandmarkId = false", compact_origin)
        self.assertNotIn("LandmarkId = true", compact_origin)
        self.assertNotIn("LandmarkId = 0.f", compact_origin)
        self.assertNotIn("LandmarkId = 160.f", compact_origin)

    def test_landmark_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName LandmarkId"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tbool LandmarkId;\n",
            "\tFText LandmarkId;\n",
            "\tfloat LandmarkId = " + forty + ";\n",
            "\tfloat LandmarkId = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_landmark_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_landmark_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_landmark_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_landmark_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_landmark_id_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-landmark-anchor LandmarkId field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_landmark_anchor_landmark_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionMapAssemblyDirector.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
