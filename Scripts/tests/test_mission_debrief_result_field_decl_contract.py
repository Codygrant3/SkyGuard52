# THIS IS leftover-safe FSkyguardMissionDebrief::Result.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `FSkyguardMissionResult Result;` with no Category=.
# Bare type+name with NO initializer.
# If a clone asserts `= 0` / `= false` / an initializer, drop that
# and accept the bare decl. If a clone asserts Category= and
# origin/main has none, drop that assertion.
# Fail-closed if the type/decl is missing or renamed, if Category=
# is added, or if an initializer appears.
...
# (State, MissionDisplayName, Narrative, bNewBestScore,
# bNewBestMedal, bProgressSaved, SaveSlotName, NextMissionId,
# NextMissionDisplayName, NextMissionMap, bNextMissionUnlocked,
# bCampaignComplete).
...
TARGET = "FSkyguardMissionResult Result;"
TARGET_WRONG_FALSE = "FSkyguardMissionResult Result = false;"
TARGET_WRONG_ZERO = "FSkyguardMissionResult Result = 0;"
TARGET_WRONG_TRUE = "FSkyguardMissionResult Result = true;"
TARGET_WRONG_BRACE = "FSkyguardMissionResult Result{};"
TARGET_WRONG_DEFAULT = "FSkyguardMissionResult Result = {};"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on an initializer. Do not accept = 0 / = false /
    # = true / brace-init / a defaulted decl for
    # FSkyguardMissionDebrief::Result (must be the bare
    # `FSkyguardMissionResult Result;`).
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"FSkyguardMissionResult\s+Result\s*=", compact):
        return False
    if re.search(r"FSkyguardMissionResult\s+Result\s*\{", compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FSkyguardMissionResult\s+Result\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FSkyguardMissionResult Result is missing from "
...
class MissionDebriefResultFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_result_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_initializer_added_fails_closed(self) -> None:
        ...
        self.assertNotIn("Result =", origin)
        self.assertNotIn("Result = 0", origin)
        self.assertNotIn("Result = false", origin)

    def test_result_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FSkyguardMissionResult Result"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("FSkyguardMissionResult", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BRACE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            "\tFSkyguardMissionResult Result = 0;\n",
            "\tFSkyguardMissionResult Result = false;\n",
            "\tFSkyguardMissionResult Result = true;\n",
            "\tFSkyguardMissionResult Result{};\n",
            "\tFSkyguardMissionResult Result = {};\n",
            "\tbool Result = false;\n",
            "\tint32 Result;\n",
            "\tFSkyguardMissionResult Results;\n",
            "\tFText Result;\n",
            "\tFSkyguardMissionResult MedalTier;\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief Result field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_result_field_decl_contract.py"
        ))
