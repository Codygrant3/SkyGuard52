# THIS IS leftover-safe ASkyguardGunner CpgConsoleRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgConsoleRight;`
# (no initializer).
...
# Stop BEFORE sibling CpgRailLeft as a claimed slot.
...
# CpgGripRight, CpgRailLeft, CpgRailRight, CpgTedacText,
# Boom, GunnerCamera, HandMesh.
# Do NOT claim leftover CpgEufd #1604, leftover
# CpgMpdLeft #1603, leftover CpgMpdRight #1602,
# leftover CpgTedac #1601, leftover CpgDash #1600,
# leftover CpgCanopyBow #1599, leftover CpgSeatPan
# #1598, leftover CpgKneeRight #1597, leftover
# CpgKneeLeft #1596, leftover CpgSeatBack #1595,
# leftover CpgCockpit #1594, leftover CpgStation
# #1593. Do NOT claim leftover CpgGripRight #1607,
# leftover CpgGripLeft #1608, leftover CpgReticleV
# #1609, leftover CpgReticleH #1605, leftover
# CpgTedacBezel #1606. CpgConsoleRight is NOT leftover
# HandMesh. Do NOT claim CpgGripRight, CpgGripLeft,
# CpgReticleV, CpgReticleH, CpgTedacBezel, CpgRailLeft,
# CpgRailRight, CpgTedacText, or HandMesh as this slot.
# CpgConsoleRight fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgConsoleRight;`
...
# CpgConsoleRight fail-closed if LOCKED_DECL is leftover
...
# CpgGripLeft / CpgGripRight / CpgRailLeft / leftover
# Apache Canopy / leftover Apache PilotCanopy /
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgConsoleRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgConsoleRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgConsoleRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgConsoleRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgConsoleRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgConsoleRight;"
...
TARGET_WRONG_TEXT = "TObjectPtr<UTextRenderComponent> CpgConsoleRight;"
...
CPG_CONSOLE_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgConsoleRight\s*;"
)
CPG_CONSOLE_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgConsoleRight\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # CpgSeatBack / CpgSeatPan / CpgCockpit / CpgStation /
    # CpgTedac / CpgMpdLeft / CpgMpdRight / CpgEufd /
    # CpgTedacBezel / CpgReticleH / CpgReticleV /
    # CpgGripLeft / CpgGripRight / leftover Apache
    # Canopy, or an initializer on CpgConsoleRight
    # as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_CONSOLE_RIGHT_DECL_RE.search(compact) is None:
        return False
    if CPG_CONSOLE_RIGHT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_RAIL_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_RAIL_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_RAIL_LEFT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgConsoleRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgConsoleRight "
...
class GunnerCpgConsoleRightFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgConsoleRight"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgConsoleRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgConsoleRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_console_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgConsoleRight;", compact_origin)
        self.assertNotIn("CpgConsoleRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_console_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgConsoleRight"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgConsoleRight;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgConsoleRights;\n",
            "\tint32 CpgConsoleRight = 140;\n",
            "\tbool CpgConsoleRight = true;\n",
            "\tfloat CpgConsoleRight = " + forty + ";\n",
            "\tfloat CpgConsoleRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_rail_left_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgConsoleRight"))
        ...
        self.assertIn(TARGET_WRONG_MPD_RIGHT, section)
        self.assertIn(TARGET_WRONG_EUFD, section)
        self.assertIn(TARGET_WRONG_TEDAC_BEZEL, section)
        self.assertIn(TARGET_WRONG_RETICLE_H, section)
        self.assertIn(TARGET_WRONG_RETICLE_V, section)
        self.assertIn(TARGET_WRONG_GRIP_LEFT, section)
        self.assertIn(TARGET_WRONG_GRIP_RIGHT, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgConsoleRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_console_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
