# THIS IS leftover-safe FSkyguardEnemyFormationDefinition SpacingCentimeters.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "100.0"))
# then `float SpacingCentimeters = 1200.f;` (initializer IS `1200.f`,
# not `1200.0f` / `1200.0` / `1200` / Harbor 40/80 / leftover `0.f` /
# leftover look-ahead `2.f` / leftover Health `160.f`).
# THIS IS leftover-safe isolated UPROPERTY with no Category.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# ClampMin IS present. There is NO Category=.
...
# if Category= is added, if the type is not float,
# if meta = (ClampMin = "100.0") is missing or changed,
# or if the initializer is missing or not `= 1200.f`.
...
# (leftover isolated fields #1339+, leftover defaults #180).
...
# Do NOT contract sibling fields FormationId /
# Formation / UnitCount.
...
# the bulk).
# THIS IS NOT leftover #1332-#1349 isolated
# RadioLine / RoutePoint / EnemyWave field drafts.
...
# isolated SpacingCentimeters field decl, same as leftover
# enemy-wave-defaults then isolated SpacingCentimeters.
# Clone LOCKED_SCRIPTS / parse / fail-closed style from
# leftover-safe draft #1300 TheaterKit WeatherIdentity.
...
# no ClampMin / no `=` / ClampMin = "100.0" / `= 1200.f`,
# retarget: type is float SpacingCentimeters,
# initializer is `= 1200.f`, specifiers are EditAnywhere +
# BlueprintReadOnly + meta = (ClampMin = "100.0"), no Category.
...
TARGET = "float SpacingCentimeters = 1200.f;"
TARGET_WRONG_EQ_NONE = "float SpacingCentimeters = 1200.0f;"
TARGET_WRONG_FALSE = "float SpacingCentimeters = false;"
TARGET_WRONG_TRUE = "float SpacingCentimeters = true;"
TARGET_WRONG_ZERO = "float SpacingCentimeters = 0.f;"
TARGET_WRONG_FLOAT = "FName SpacingCentimeters;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "float SpacingCentimeters;"
TARGET_WRONG_LOOKAHEAD = "float SpacingCentimeters = 2.f;"
TARGET_WRONG_POINT_ZERO = "float SpacingCentimeters = 1200.0;"
TARGET_WRONG_INT = "int32 SpacingCentimeters = 1200;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_enemy_formation_spacing_centimeters"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float SpacingCentimeters = 1200.f`.
    # Reject missing initializer, `1200.0f` / `1200.0` / `1200` /
    # leftover start-time `0.f` / leftover look-ahead `2.f` /
    # Harbor 40/80 / leftover Health.
    # Do not accept sibling FormationId / Formation / UnitCount.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+SpacingCentimeters\s*=\s*1200\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+SpacingCentimeters\s*=\s*1200\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SpacingCentimeters\s*=\s*1200\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SpacingCentimeters\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SpacingCentimeters\s*=\s*2\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+SpacingCentimeters\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+SpacingCentimeters\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+SpacingCentimeters\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+SpacingCentimeters\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float SpacingCentimeters is missing from "
...
def require_spacing_centimeters_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} SpacingCentimeters "
            "UPROPERTY"
        )
    ...
    if 'ClampMin = "100.0"' not in specifiers:
        raise AssertionError(
            'meta = (ClampMin = "100.0") is missing or changed on '
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} "
            "SpacingCentimeters UPROPERTY"
        )
    return specifiers


class EnemyFormationSpacingCentimetersFieldDeclContractTests(unittest.TestCase):
    def test_enemy_formation_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SpacingCentimeters"), section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedEnemyWaveDefinition\n{\n};\n"
            )
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SpacingCentimeters"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_WAVE, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        ...
        self.assertIn("SpacingCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_spacing_centimeters_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_spacing_centimeters_specifiers(section)
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float SpacingCentimeters = 1200.f;", compact_origin)
        self.assertNotIn("SpacingCentimeters = 0.0f", compact_origin)
        self.assertNotIn("SpacingCentimeters = 0.0;", compact_origin)
        self.assertNotIn("SpacingCentimeters = 0;", compact_origin)
        self.assertNotIn("SpacingCentimeters = 2.f", compact_origin)
        self.assertNotIn("SpacingCentimeters = 160.f", compact_origin)
        self.assertNotIn("SpacingCentimeters = false", compact_origin)

    def test_spacing_centimeters_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float SpacingCentimeters"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 1200.f", LOCKED_DECL)
        self.assertNotIn("= 0.0f", LOCKED_DECL)
        self.assertNotIn("= 0.0", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_LOOKAHEAD}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_POINT_ZERO}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_LOOKAHEAD}\n",
            f"\t{TARGET_WRONG_POINT_ZERO}\n",
            leftover_primary,
            leftover_guided,
            f"\tFText {SIBLING_SPEAKER};\n",
            "\tfloat StartTimeSecond = 1200.f;\n",
            "\tint32 SpacingCentimeters = 0;\n",
            "\tbool SpacingCentimeters;\n",
            f"\t{TARGET_WRONG_EMPTY}\n",
            f"\t{TARGET_WRONG_LOOKAHEAD}\n",
            f"\t{TARGET_WRONG_POINT_ZERO}\n",
            "\tfloat SpacingCentimeters = " + forty + ";\n",
            "\tfloat SpacingCentimeters = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_spacing_centimeters_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_spacing_centimeters_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_spacing_centimeters_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_spacing_centimeters_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_spacing_centimeters_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_spacing_centimeters_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_spacing_centimeters_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('meta = (ClampMin = "100.0")', UPROPERTY_EDIT_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("Category", UPROPERTY_EDIT_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "enemy-formation SpacingCentimeters field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enemy_formation_spacing_centimeters_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(SIBLING_FORMATIONS_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ENEMY_WAVE_START_TIME, LOCKED_SCRIPTS)
        self.assertIn(SIBLING_FORMATION_ID_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(SIBLING_FORMATION_FIELD_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(REMAINING_UNIT_COUNT_SCRIPT, LOCKED_SCRIPTS)
