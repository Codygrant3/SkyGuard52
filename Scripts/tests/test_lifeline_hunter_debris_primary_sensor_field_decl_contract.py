...
# INDEX_NONE or lock DebrisPrimarySensor construction
# in the .cpp. This is a FIELD contract on
# ASkyguardLifelineHunterBoss, not leftover
# LifelineHunter sibling fields OpticalTracker /
# WeaponServo / CountermeasurePod / Engine /
# DebrisSecondarySensor / DebrisControlSurface
# and not leftover emergency-finish named
# surfaces (split tokens only). Keep
# DebrisPrimarySensor distinct from sibling
# DebrisSecondarySensor and DebrisControlSurface.
# Weak-point fields OpticalTracker, WeaponServo,
# CountermeasurePod, Engine must not satisfy this
# mesh field. Do not lock leftover Harbor-adjacent
# MinimumWeaponSeparationMeters. Sibling
# RunwayBreaker debris field contracts stay
# leftover. Sibling fields DebrisSecondarySensor
# and DebrisControlSurface must not satisfy the
# primary lock.
...
# DebrisPrimarySensor;`); accept that form and other
...
# declaration text as leftover RunwayBreaker,
# different class. Not the primary lock.
...
RUNWAY_BREAKER_DEBRIS_PORT_WING_FIELD = (
    "TObjectPtr<UStaticMeshComponent> DebrisPortWing;"
)
...
# DebrisPrimarySensor field declaration contract on
...
# unlocked for THIS test's primary DebrisPrimarySensor lock. Leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLifelineHunterBoss::DebrisPrimarySensor",
    "SkyguardLifelineHunterBoss.cpp",
    "SetExposed",
...
class LifelineHunterDebrisPrimarySensorFieldDeclContractTests(unittest.TestCase):
    def test_lifeline_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_missing_debris_primary_sensor_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardLifelineHunterBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("Category", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisPrimarySensor;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisPrimarySensor;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> DebrisPrimarySensor;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_PRIMARY_SENSOR_FIELD)
            self.assertIn("DebrisPrimarySensor", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_primary_sensor_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD),
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_PRIMARY_SENSOR_FIELD), 1)
        self.assertTrue(
            DEBRIS_PRIMARY_SENSOR_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
        self.assertTrue(DEBRIS_PRIMARY_SENSOR_FIELD.endswith(";"), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("OpticalTracker", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("WeaponServo", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("CountermeasurePod", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisSecondarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisControlSurface", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("MinimumWeaponSeparationMeters", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPortWing", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("HeatManifold", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("RunwayRack", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPayloadBay", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("LeftActuator", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("RightActuator", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisLeftWing", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisRightWing", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisCoolingDoor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("RadarReceiver", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("CoolingDoor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("TArray<", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("=", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("{", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("}", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("return ", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisCenter", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("RadarNode", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_PRIMARY_SENSOR_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {DEBRIS_PRIMARY_SENSOR_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DEBRIS_PRIMARY_SENSOR_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DEBRIS_PRIMARY_SENSOR_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_PRIMARY_SENSOR_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD),
            section,
    ...
    def test_assigned_debris_primary_sensor_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisPrimarySensor = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_PRIMARY_SENSOR_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            f"\t{DEBRIS_PAYLOAD_BAY_FIELD}\n",
            f"\t{RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD}\n",
            f"\t{RUNWAY_BREAKER_DEBRIS_PORT_WING_FIELD}\n",
            f"\t{TARGETING_CAMERA_FIELD}\n",
            ...
            self.assertIn("DebrisPrimarySensor", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_PRIMARY_SENSOR_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_PRIMARY_SENSOR_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_PRIMARY_SENSOR_FIELD.startswith("UFUNCTION"),
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_PRIMARY_SENSOR_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_PRIMARY_SENSOR_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_primary_sensor_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotIn("{", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("}", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("return ", DEBRIS_PRIMARY_SENSOR_FIELD)
        ...
        self.assertNotIn("SkyguardLifelineHunterBoss.cpp", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SkyguardLifelineHunterBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("return true", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_PRIMARY_SENSOR_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisIntakePanel", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DispenserCenter", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("IsContactIdentified", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), DEBRIS_PRIMARY_SENSOR_FIELD)

    def test_debris_cooling_door_does_not_satisfy_debris_primary_sensor(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DispenserStarboard", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetManeuver", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), DEBRIS_PRIMARY_SENSOR_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        self.assertNotIn("CoolingDoor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("OpenSensorExposure", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("MinimumWeaponSeparationMeters", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_open_safe_window(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_safe_engine_fallback(), DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("IsLightningExposed", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_PRIMARY_SENSOR_FIELD),
            DEBRIS_PRIMARY_SENSOR_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardLifelineHunterBoss::DebrisPrimarySensor", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "lifeline hunter DebrisPrimarySensor field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_PRIMARY_SENSOR_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_PRIMARY_SENSOR_FIELD.lower(),
                "lifeline hunter DebrisPrimarySensor contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_PRIMARY_SENSOR_FIELD)

    def test_contract_is_debris_primary_sensor_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DEBRIS_PRIMARY_SENSOR_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        self.assertNotIn("DebrisEngine", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisRightWing", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardRoadHunterBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("StarboardLatch", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DecoyPods", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ElevatorLinkage", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_lifeline_hunter_is_not_breakwater_engine(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(collapsed(DEBRIS_PRIMARY_SENSOR_FIELD), collapsed(ENGINE_FIELD))
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardLastFlightBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_PRIMARY_SENSOR_FIELD)

    def test_lifeline_hunter_debris_primary_sensor_is_not_leftover_debris_engine(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotEqual(
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        self.assertNotIn("DebrisSecondarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisControlSurface", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("Engine;", DEBRIS_PRIMARY_SENSOR_FIELD)


    def test_lifeline_hunter_is_not_radar_ghost_engine(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardRadarGhostBoss")
    ...
    def test_lifeline_hunter_is_not_pathfinder_engine(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotEqual(
            collapsed(DEBRIS_PRIMARY_SENSOR_FIELD),
    ...
    def test_lifeline_hunter_is_not_road_hunter_engine(self) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardRoadHunterBoss")
    ...
    def test_hangar_rack_does_not_satisfy_debris_primary_sensor(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD))
        self.assertNotEqual(HANGAR_RACK_FIELD, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_PRIMARY_SENSOR_FIELD)

    def test_heat_manifold_and_runway_rack_do_not_satisfy_debris_primary_sensor(
        self,
    ) -> None:
        for region, name in (
            (f"\t{HEAT_MANIFOLD_FIELD}\n", "HeatManifold"),
    ...
    def test_runway_breaker_sibling_fields_do_not_satisfy_debris_primary_sensor(
        self,
    ) -> None:
        for region, name in (
            (f"\t{PORT_ENGINE_FIELD}\n", "PortEngine"),
    ...
    def test_debris_engine_does_not_satisfy_debris_primary_sensor(self) -> None:
    ...
    def test_targeting_camera_does_not_satisfy_debris_primary_sensor(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD))
        self.assertNotEqual(TARGETING_CAMERA_FIELD, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)

    def test_left_and_right_actuators_do_not_satisfy_debris_primary_sensor(self) -> None:
        for region, name in (
            (f"\t{LEFT_ACTUATOR_FIELD}\n", "LeftActuator"),
            (f"\t{RIGHT_ACTUATOR_FIELD}\n", "RightActuator"),
        ):
    ...
    def test_lifeline_hunter_debris_primary_sensor_is_not_runway_breaker_debris_port_wing(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotEqual(
        ...
        self.assertNotEqual(CLASS_NAME, "ASkyguardRunwayBreakerBoss")
        self.assertNotEqual(
            HEADER_PATH,
        ...
        self.assertNotIn("ASkyguardIronRainBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", locked_only)
        self.assertNotIn("ASkyguardRunwayBreakerBoss", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPortWing", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_PRIMARY_SENSOR_FIELD)
    ...
    def test_port_engine_does_not_satisfy_debris_primary_sensor(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD))
        self.assertNotEqual(PORT_ENGINE_FIELD, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_ENGINE_FIELD,
        )
    ...
    def test_lifeline_hunter_is_not_last_flight_port_engine(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, LAST_FLIGHT_PORT_ENGINE_FIELD)
    ...
    def test_debris_secondary_sensor_does_not_satisfy_debris_primary_sensor(
        self,
    ) -> None:
        leftover = f"\t{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_debris_control_surface_does_not_satisfy_debris_primary_sensor(
        self,
    ) -> None:
        leftover = f"\t{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("DebrisPrimarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_leftover_weak_point_fields_do_not_satisfy_debris_primary_sensor(
        self,
    ) -> None:
        for region, name in (
            (f"\t{OPTICAL_TRACKER_FIELD}\n", "OpticalTracker"),
    ...
    def test_minimum_weapon_separation_does_not_satisfy_debris_primary_sensor(
        self,
    ) -> None:
        ...
        self.assertFalse(has_declaration(leftover, DEBRIS_PRIMARY_SENSOR_FIELD))
        self.assertNotIn("MinimumWeaponSeparationMeters", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertNotIn("450.f", DEBRIS_PRIMARY_SENSOR_FIELD)
        for token in leftover_harbor_tokens():
            self.assertNotEqual(token, MINIMUM_WEAPON_SEPARATION)

    def test_leftover_named_surfaces_do_not_satisfy_debris_primary_sensor(
        self,
    ) -> None:
        for region in (
            f"\t{leftover_open_safe_window()}\n",
