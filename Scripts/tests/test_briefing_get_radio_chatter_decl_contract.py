...
# or lock the GetRadioChatter body.
...
GET_RADIO_CHATTER = "TArray<FText> GetRadioChatter() const;"
UFUNCTION_BRIEFING = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Briefing")'
)
# Leftover #56–#64 plus BriefingComponent production files.
# This lane only adds an isolated Python GetRadioChatter
...
# GetMinimumWarmupSeconds, and GetBriefingText on this class.
...
# leftover GetElapsedSeconds, leftover
# GetMinimumWarmupSeconds, and leftover GetBriefingText
# stay sibling-only.
LOCKED_SCRIPTS = (
...
# GetMinimumWarmupSeconds / GetBriefingText stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# .cpp GetRadioChatter body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline `return RadioChatter` is
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionBriefingComponent::GetRadioChatter",
    "SkyguardMissionBriefingComponent.cpp",
)
SIBLING_TYPES = (
...
class BriefingGetRadioChatterDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_RADIO_CHATTER),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_RADIO_CHATTER)
        self.assertIn("GetRadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_RADIO_CHATTER))

    def test_missing_get_radio_chatter_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_RADIO_CHATTER)
        self.assertIn("GetRadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_RADIO_CHATTER)
        self.assertIn("GetRadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_RADIO_CHATTER)
        self.assertNotIn("UFUNCTION", GET_RADIO_CHATTER)
        self.assertNotIn("Category", GET_RADIO_CHATTER)
        self.assertNotIn("BlueprintCallable", GET_RADIO_CHATTER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_RADIO_CHATTER)
        self.assertIn("GetRadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tTArray<FText> GetRadioChatter();\n"
        wrong_return_void = "\tvoid GetRadioChatter() const;\n"
        wrong_return_bool = "\tbool GetRadioChatter() const;\n"
        added_arg = "\tTArray<FText> GetRadioChatter(bool bReady) const;\n"
        wrong_name = "\tFText GetBriefingText() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_RADIO_CHATTER)
            self.assertIn("GetRadioChatter", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_radio_chatter_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_RADIO_CHATTER),
            GET_RADIO_CHATTER,
        )
        self.assertTrue(has_declaration(section, GET_RADIO_CHATTER))
        self.assertEqual(
            declaration_count(section, GET_RADIO_CHATTER),
            1,
        )
        self.assertTrue(
            GET_RADIO_CHATTER.startswith("TArray<FText> "),
            GET_RADIO_CHATTER,
        )
        self.assertTrue(
            GET_RADIO_CHATTER.endswith(";"),
            GET_RADIO_CHATTER,
        )
        self.assertIn("GetRadioChatter()", GET_RADIO_CHATTER)
        self.assertIn(" const", GET_RADIO_CHATTER)
        self.assertNotIn("INDEX_NONE", GET_RADIO_CHATTER)
        self.assertNotIn("{", GET_RADIO_CHATTER)
        self.assertNotIn("}", GET_RADIO_CHATTER)
        self.assertNotIn("return ", GET_RADIO_CHATTER)
        self.assertNotIn("return RadioChatter", GET_RADIO_CHATTER)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_RADIO_CHATTER}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_RADIO_CHATTER))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_RADIO_CHATTER),
            GET_RADIO_CHATTER,
        )
        exact_inline = (
            "TArray<FText> GetRadioChatter() const { return RadioChatter; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_RADIO_CHATTER))
        self.assertEqual(
            require_declaration(exact_inline, GET_RADIO_CHATTER),
            GET_RADIO_CHATTER,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_RADIO_CHATTER),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tTArray<FText>\n"
            "\tGetRadioChatter() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tTArray<FText> GetRadioChatter(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_RADIO_CHATTER}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_RADIO_CHATTER))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_RADIO_CHATTER),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_RADIO_CHATTER)
        self.assertNotIn("return RadioChatter", GET_RADIO_CHATTER)
        self.assertNotIn("return Other", GET_RADIO_CHATTER)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_RADIO_CHATTER}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_RADIO_CHATTER)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_RADIO_CHATTER)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_radio_chatter_body(self) -> None:
        locked_only = f"{GET_RADIO_CHATTER}\n"
        self.assertNotIn("{", GET_RADIO_CHATTER)
        self.assertNotIn("}", GET_RADIO_CHATTER)
        self.assertNotIn("return ", GET_RADIO_CHATTER)
        self.assertNotIn("return RadioChatter", GET_RADIO_CHATTER)
        self.assertNotIn("return RadioChatter", locked_only)
        self.assertNotIn(
            "USkyguardMissionBriefingComponent::GetRadioChatter",
            GET_RADIO_CHATTER,
        )
        ...
        self.assertNotIn("return false", GET_RADIO_CHATTER)
        self.assertNotIn("return true", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("ConfigureFromMission", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_set_assets_ready(self) -> None:
        ...
        for neighbor in SET_ASSETS_READY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("SetAssetsReady", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_advance_briefing(self) -> None:
        ...
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("AdvanceBriefing", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_acknowledge_and_launch(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_AND_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("AcknowledgeAndLaunch", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_can_launch(self) -> None:
        ...
        for neighbor in CAN_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("CanLaunch", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_get_briefing_state(self) -> None:
        ...
        for neighbor in GET_BRIEFING_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("GetBriefingState", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_get_minimum_warmup_seconds(self) -> None:
        ...
        for neighbor in GET_MINIMUM_WARMUP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("GetBriefingText", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_SECONDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("GetElapsedSeconds", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_RADIO_CHATTER)
        self.assertNotIn("Unconfigured", GET_RADIO_CHATTER)
        self.assertNotIn("Warming", GET_RADIO_CHATTER)
        self.assertNotIn("Launched", GET_RADIO_CHATTER)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_RADIO_CHATTER,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_RADIO_CHATTER)
        self.assertNotIn("FSkyguardBriefingCard", GET_RADIO_CHATTER)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_RADIO_CHATTER)
        self.assertNotIn("FillResultCombatStats", GET_RADIO_CHATTER)
        self.assertNotIn("ASkyguardGunner", GET_RADIO_CHATTER)
        self.assertNotIn("FillAndFinalize", GET_RADIO_CHATTER)
        self.assertNotIn("FillAndFail", GET_RADIO_CHATTER)
        self.assertNotIn("ApplyHydraForClusters", GET_RADIO_CHATTER)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_RADIO_CHATTER}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_RADIO_CHATTER),
            GET_RADIO_CHATTER,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing GetRadioChatter contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_RADIO_CHATTER.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing GetRadioChatter contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_RADIO_CHATTER)

    def test_contract_is_get_radio_chatter_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_RADIO_CHATTER.startswith("TArray<FText> "))
        self.assertTrue(GET_RADIO_CHATTER.endswith(";"))
        self.assertIn(" const", GET_RADIO_CHATTER)
        self.assertIn(UFUNCTION_BRIEFING, section)
