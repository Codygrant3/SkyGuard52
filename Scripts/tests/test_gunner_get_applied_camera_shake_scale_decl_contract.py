# THIS IS leftover-safe ASkyguardGunner GetAppliedCameraShakeScale.
# origin/main form: one-line and split-line UFUNCTION wraps
# UFUNCTION(BlueprintPure, Category="Skyguard|Settings")
# wrapping `float GetAppliedCameraShakeScale() const { return AppliedCameraShakeScale; }`.
# LOCKED_DECL may be declaration-only
# `float GetAppliedCameraShakeScale() const;` if the matcher strips
# inline bodies, OR the full origin/main inline form.
# Accept declaration-only AND the origin/main inline body.
# Do NOT fail-closed on `{ return AppliedCameraShakeScale; }`.
# THIS IS leftover-safe unique analog Category Skyguard|Settings
# (unique unused Category path; leftover unique GameUserSettings
# wraps Category="Settings" / "Settings|Accessibility" WITHOUT the
# Skyguard| prefix). This is leftover-safe unique analog, NOT leftover
# analog Settings and NOT leftover analog Combat|Sortie.
# BlueprintPure IS present. const IS present. Return type IS float.
# Fail-closed if the UFUNCTION or decl is missing or renamed, if
# Category is missing, if specifiers drop to BlueprintCallable, if
# const is missing, if the return type is not float, or if Category
# is "Settings" WITHOUT Skyguard| (leftover analog GameUserSettings),
# or Settings|Input / Settings|Audio / Settings|Accessibility
# (leftover unique GameUserSettings), or Skyguard|Combat WITHOUT a
# unique leftover-safe suffix, or Skyguard|Combat|Sortie (leftover
# unique analog #1685-#1687), or Skyguard|Combat|Feedback /
# Skyguard|Combat|Safety (leftover unique FIELD #1625/#1626), or
# Skyguard|Apache / Skyguard|Theater / Skyguard|MissionMap /
# Skyguard|Audio|Acceptance / Skyguard|Audio|Development / still
# Category="Skyguard" without a leftover-safe unique Category path /
...
# Parse class `ASkyguardGunner` public section ONLY after
# `class ASkyguardGunner` / `GENERATED_BODY`. Start at `public:`.
# Stop BEFORE `protected:` and BEFORE `private:`. Stop BEFORE first
# public UPROPERTY (`LookYawLimit` / CPG kit / Boom).
# Settings UFUNCTIONs sit early in public, before Combat|Sortie
# and before UPROPERTY.
# Do NOT claim sibling Skyguard|Settings UFUNCTION
# GetAppliedLookSensitivity / IsVerticalLookInverted
# (other workers; leave uncontracted).
# Do NOT claim leftover unique Combat|Sortie GetSortieHits #1685 /
# GetSortieShotsFired #1686 / ResetSortieCombatStats #1687.
# Do NOT claim GetSortieAircraftDamageFraction (skip).
# Do NOT claim FillSortieCombatStats (NO UFUNCTION macro).
# Do NOT claim leftover unique Gunner FIELD Boom / FireCameraShakeClass
# #1625 / LookYawLimit / CPG kit.
# Do NOT claim leftover analog apache-cpg-feel FireRate 12.0f /
# RecoilPitch 0.92f.
# Do NOT claim leftover analog Gunner Category Skyguard|Apache
# UFUNCTION/UPROPERTY.
# Do NOT claim leftover unique USkyguardGameUserSettings
# GetCameraShakeScale / SetCameraShakeScale (different class;
# different identifier GetCameraShakeScale vs
# GetAppliedCameraShakeScale; Category Settings|Accessibility
# without Skyguard|).
# Do NOT claim leftover unique IronRain MaxReleasesPerBay #1683 /
# LastFlight MinimumCivilianSeparationMeters #1684.
# Skip leftover analog theater-kit-get-lamp-instance-count
# Category Skyguard|Theater.
# Skip leftover analog GetResolvedProductionLoopRouteCount
# Category Skyguard|Audio|Production.
# Clone leftover unique BlueprintPure int32 UFUNCTION GetSortieHits
# #1685 wrapping `int32 GetSortieHits() const;` — RETARGET hard:
# Category Combat|Sortie → Skyguard|Settings, method GetSortieHits
# → GetAppliedCameraShakeScale, return int32 → float.
# KEEP BlueprintPure. KEEP const. Accept declaration-only
# AND origin/main inline body.
# Clone leftover unique AudioDirector Production UFUNCTION #1658
# is BlueprintPure int32 declaration-only — RETARGET return type
# and Category.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity
# — RETARGET hard.
# LOCKED_DECL is `float GetAppliedCameraShakeScale() const;`
# NOT `int32 GetSortieHits() const;` and NOT
# `FName WeatherIdentity;` and NOT
# `float GetAppliedLookSensitivity() const;`
# and NOT `bool IsVerticalLookInverted() const;` and NOT
# `float GetCameraShakeScale() const;` and NOT
# `void ResetSortieCombatStats();`.
# Fail-closed if LOCKED_DECL is FName WeatherIdentity / 160.f /
# FireRate = 12.0f / RecoilPitch = 0.92f / RouteLengthCm = 45000.f /
# TSubclassOf FireCameraShakeClass leftover unique FIELD.
# Harbor leftover is only the split 40 / 80 float tokens.
# Ban retired live-copy tokens via split tokens.
# Stay Apache CPG 30 mm / Hydra / Hellfire. Fail-closed on live
# Ig+la / Ri+fle / Ya+k appearing as contiguous tokens in THIS
# test file.
...
TARGET = "float GetAppliedCameraShakeScale() const;"
TARGET_WRONG_BARE = "float GetAppliedCameraShakeScale();"
TARGET_WRONG_INIT = "float GetAppliedCameraShakeScale() = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float GetAppliedCameraShakeScale() = true;"
TARGET_WRONG_ZERO = "float GetAppliedCameraShakeScale() = 0.f;"
TARGET_WRONG_NO_CONST = "float GetAppliedCameraShakeScale();"
TARGET_WRONG_INT32 = "int32 GetAppliedCameraShakeScale() const;"
TARGET_WRONG_FNAME_RET = "FName GetAppliedCameraShakeScale() const;"
TARGET_WRONG_CLONE_DECL = "int32 GetResolvedProductionLoopRouteCount() const;"
TARGET_WRONG_BODY = (
...
TARGET_WRONG_RESET = "void ResetSortieCombatStats();"
TARGET_WRONG_FILL = "void FillSortieCombatStats(FSkyguardMissionResult& OutResult) const;"
TARGET_WRONG_DAMAGE = "float GetSortieAircraftDamageFraction() const;"
TARGET_WRONG_HITS_EQ = "int32 Hits = 0;"
TARGET_WRONG_SNAPSHOT_HITS = "int32 Hits;"
...
TARGET_WRONG_SETTINGS = (
    'UFUNCTION(BlueprintPure, '
    'Category="Settings")'
)
TARGET_WRONG_SETTINGS_INPUT = (
    'UFUNCTION(BlueprintPure, '
    'Category="Settings|Input")'
)
TARGET_WRONG_SETTINGS_AUDIO = (
    'UFUNCTION(BlueprintPure, '
    'Category="Settings|Audio")'
)
TARGET_WRONG_SETTINGS_ACCESSIBILITY = (
    'UFUNCTION(BlueprintPure, '
...
TARGET_WRONG_LOOK_SENSITIVITY = "float GetAppliedLookSensitivity() const;"
TARGET_WRONG_LOOK_SENSITIVITY_INLINE = (
    "float GetAppliedLookSensitivity() const "
    "{ return AppliedLookSensitivity; }"
)
TARGET_WRONG_INVERT_LOOK = "bool IsVerticalLookInverted() const;"
TARGET_WRONG_INVERT_LOOK_INLINE = (
    "bool IsVerticalLookInverted() const "
    "{ return bInvertVerticalLookApplied; }"
)
TARGET_WRONG_GUS_SHAKE = "float GetCameraShakeScale() const;"
TARGET_WRONG_HITS = "int32 GetSortieHits() const;"
TARGET_WRONG_HITS_INLINE = (
    "int32 GetSortieHits() const { return SortieHits; }"
)
TARGET_WRONG_ROUTE_LENGTH = "float RouteLengthCm = 45000.f;"
TARGET_WRONG_FIRE_CAMERA_CLASS = (
...
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GetAppliedCameraShakeScale;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GetAppliedCameraShakeScale;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> GetAppliedCameraShakeScale;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_gunner_get_applied_camera_shake_scale"
    "_decl_contract.py"
)
LEFTOVER_AUDIO_DIRECTOR_GET_RESOLVED = (
    "Scripts/tests/test_audio_director_get_resolved_production"
    "_loop_route_count_decl_contract.py"
)
LEFTOVER_MISSION_RESULT_HITS = (
    "Scripts/tests/test_mission_result_hits"
    "_field_decl_contract.py"
)
LEFTOVER_CPG_DEBRIEF_HITS = (
    "Scripts/tests/test_cpg_debrief_snapshot_hits"
    "_field_decl_contract.py"
)
LEFTOVER_FILL_RESULT_COMBAT_STATS = (
    "Scripts/tests/test_fill_result_combat_stats"
    "_decl_contract.py"
)
LEFTOVER_GUNNER_FIRE_CAMERA_SHAKE = (
    "Scripts/tests/test_gunner_fire_camera_shake_class"
    "_field_decl_contract.py"
)
LEFTOVER_GUNNER_MIN_SAFE_YAW = (
    "Scripts/tests/test_gunner_minimum_safe_side_fire_yaw"
    "_field_decl_contract.py"
)
LEFTOVER_GUNNER_GET_SORTIE_HITS = (
    "Scripts/tests/test_gunner_get_sortie_hits"
    "_decl_contract.py"
)
LEFTOVER_GUNNER_GET_SORTIE_SHOTS = (
    "Scripts/tests/test_gunner_get_sortie_shots_fired"
    "_decl_contract.py"
)
LEFTOVER_GUNNER_RESET_SORTIE = (
    "Scripts/tests/test_gunner_reset_sortie_combat_stats"
    "_decl_contract.py"
)
LEFTOVER_GUNNER_GET_LOOK_SENSITIVITY = (
    "Scripts/tests/test_gunner_get_applied_look_sensitivity"
    "_decl_contract.py"
)
LEFTOVER_GUNNER_IS_VERTICAL_INVERT = (
    "Scripts/tests/test_gunner_is_vertical_look_inverted"
    "_decl_contract.py"
)
LEFTOVER_GUS_GET_CAMERA_SHAKE_SCALE = (
    "Scripts/tests/test_game_user_settings_get_camera_shake_scale"
    "_decl_contract.py"
)
LEFTOVER_GUS_SET_CAMERA_SHAKE_SCALE = (
    "Scripts/tests/test_game_user_settings_set_camera_shake_scale"
    "_decl_contract.py"
)
LEFTOVER_IRON_RAIN_MAX_RELEASES = (
    "Scripts/tests/test_iron_rain_max_releases_per_bay"
    "_field_decl_contract.py"
)
LEFTOVER_LAST_FLIGHT_CIVILIAN = (
    "Scripts/tests/test_last_flight_minimum_civilian_separation"
    "_field_decl_contract.py"
)
SIBLING_GET_SORTIE_SHOTS = "GetSortieShotsFired"
SIBLING_RESET_SORTIE = "ResetSortieCombatStats"
SIBLING_FILL_SORTIE = "FillSortieCombatStats"
SIBLING_GET_SORTIE_DAMAGE = "GetSortieAircraftDamageFraction"
SIBLING_GET_SORTIE_HITS = "GetSortieHits"
SIBLING_GET_LOOK_SENSITIVITY = "GetAppliedLookSensitivity"
SIBLING_IS_VERTICAL_INVERT = "IsVerticalLookInverted"
SIBLING_GET_CAMERA_SHAKE_SCALE = "GetCameraShakeScale"
SIBLING_SET_CAMERA_SHAKE_SCALE = "SetCameraShakeScale"
SIBLING_FIRE_CAMERA = "FireCameraShakeClass"
SIBLING_MIN_SAFE_YAW = "MinimumSafeSideFireYaw"
SIBLING_ROUTE_LENGTH_LOCKED = "RouteLengthCm"
...
GET_APPLIED_CAMERA_SHAKE_SCALE_RE = re.compile(
    r"float\s+GetAppliedCameraShakeScale\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+AppliedCameraShakeScale\s*;\s*\})"
)
GET_APPLIED_CAMERA_SHAKE_SCALE_WRONG_TYPE_RE = re.compile(
    r"(?:int32|FName|bool|FLinearColor|void)\s+"
    r"GetAppliedCameraShakeScale\b"
)
GET_APPLIED_CAMERA_SHAKE_SCALE_WRONG_BODY_RE = re.compile(
    r"int32\s+GetResolvedProductionLoopRouteCount\s*\(\s*\)\s*const\s*\{"
)
GET_APPLIED_CAMERA_SHAKE_SCALE_NO_CONST_RE = re.compile(
    r"float\s+GetAppliedCameraShakeScale\s*\(\s*\)\s*;"
)
OPEN_COCKPIT_WIND_LOOP_DECL_RE = GET_APPLIED_CAMERA_SHAKE_SCALE_RE
OPEN_COCKPIT_WIND_LOOP_INIT_RE = GET_APPLIED_CAMERA_SHAKE_SCALE_WRONG_BODY_RE
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_GET_SORTIE_SHOTS,
        SIBLING_RESET_SORTIE,
        SIBLING_FILL_SORTIE,
        SIBLING_GET_SORTIE_DAMAGE,
        SIBLING_GET_SORTIE_HITS,
        SIBLING_GET_LOOK_SENSITIVITY,
        SIBLING_IS_VERTICAL_INVERT,
        SIBLING_GET_CAMERA_SHAKE_SCALE,
        SIBLING_SET_CAMERA_SHAKE_SCALE,
        SIBLING_FIRE_CAMERA,
        SIBLING_MIN_SAFE_YAW,
        SIBLING_CPG_STATION,
        SIBLING_CPG_COCKPIT,
...
def has_one_declaration(region: str, declaration: str) -> bool:
    ...
    if compact_decl in compact_region:
        return True
    if collapsed(TARGET_INLINE) in compact_region and declaration == LOCKED_DECL:
        return True
    ...
    pattern = re.compile(
        re.escape(stem)
        + r"\s*(?:;|\{[^}]*return\s+AppliedCameraShakeScale\s*;\s*\})"
    )
    return pattern.search(compact_region) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float GetAppliedCameraShakeScale() const;`
    # OR origin/main inline `{ return AppliedCameraShakeScale; }`.
    # Do not accept missing const, FName / int32 /
    # FLinearColor / bool return types, leftover clone
    # GetResolvedProductionLoopRouteCount inline body,
    # BlueprintCallable wraps, leftover analog
    # GetLampInstanceCount, leftover FName WeatherIdentity,
    # leftover GetSortieShotsFired / ResetSortieCombatStats,
    # leftover Hits =, leftover FireRate / RecoilPitch,
    ...
    if GET_APPLIED_CAMERA_SHAKE_SCALE_WRONG_TYPE_RE.search(compact):
        return False
    if GET_APPLIED_CAMERA_SHAKE_SCALE_WRONG_BODY_RE.search(compact):
        return False
    if GET_APPLIED_CAMERA_SHAKE_SCALE_RE.search(compact) is None:
        return False
    if GET_APPLIED_CAMERA_SHAKE_SCALE_NO_CONST_RE.search(compact) and (
        GET_APPLIED_CAMERA_SHAKE_SCALE_RE.search(compact) is None
    ):
        return False
...
def count_one_declaration(region: str, declaration: str) -> int:
    ...
    if collapsed(TARGET_INLINE) in compact_region and declaration == LOCKED_DECL:
        return compact_region.count(collapsed(TARGET_INLINE))
    stem = declaration_stem(declaration)
    if not stem:
        return 0
    pattern = re.compile(
...
def public_section(header: str) -> str:
    ...
    event = re.search(r"\bLookYawLimit\b", section)
...
def _declaration_end(text: str, start: int) -> int:
    ...
    while index < len(text):
        ...
        index += 1
    raise AssertionError(
        f"{CLASS_NAME} public UFUNCTION field is missing a "
        f"semicolon or inline body in origin/main:{HEADER_PATH}"
    )
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        end = _declaration_end(section, after_paren)
        ...
        if ident in (
            ...
            break
        if ident != "GetAppliedCameraShakeScale":
            cursor = end
            continue
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+GetAppliedCameraShakeScale\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for float GetAppliedCameraShakeScale "
...
class GunnerGetAppliedCameraShakeScaleDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetAppliedCameraShakeScale"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_GET_SORTIE_SHOTS), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_SORTIE_SHOTS), section)
        self.assertTrue(has_identifier(body, SIBLING_GET_SORTIE_HITS), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_SORTIE_HITS), section)
        self.assertTrue(has_identifier(body, SIBLING_GET_LOOK_SENSITIVITY), body)
        self.assertFalse(
            has_identifier(section, SIBLING_GET_LOOK_SENSITIVITY),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_IS_VERTICAL_INVERT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_IS_VERTICAL_INVERT),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_RESET_SORTIE), body)
        self.assertFalse(has_identifier(section, SIBLING_RESET_SORTIE), section)
        self.assertTrue(has_identifier(body, SIBLING_FILL_SORTIE), body)
        self.assertFalse(
            has_identifier(section, SIBLING_FILL_SORTIE),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_GET_SORTIE_DAMAGE), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_SORTIE_DAMAGE), section)
        self.assertTrue(has_identifier(body, SIBLING_BOOM), body)
        self.assertFalse(has_identifier(section, SIBLING_BOOM), section)
        self.assertTrue(has_identifier(body, SIBLING_FIRE_CAMERA), body)
        self.assertFalse(has_identifier(section, SIBLING_FIRE_CAMERA), section)
        self.assertTrue(has_identifier(body, SIBLING_MIN_SAFE_YAW), body)
        self.assertFalse(has_identifier(section, SIBLING_MIN_SAFE_YAW), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GetAppliedCameraShakeScale"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GetAppliedCameraShakeScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_applied_camera_shake_scale_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Settings"', section)
        ...
        self.assertIn('Skyguard|Settings', section)
        self.assertNotIn('Category="Settings"', section)
        self.assertNotIn('Settings|Input', section)
        self.assertNotIn('Settings|Audio', section)
        self.assertNotIn('Settings|Accessibility', section)
        self.assertNotIn('Skyguard|Combat|Sortie', section)
        self.assertNotIn('Skyguard|Combat|Feedback', section)
        self.assertNotIn('Skyguard|Combat|Safety', section)
        self.assertIsNone(
            re.search(r'Category="Skyguard\|Combat"(?!\|)', section)
        )
        self.assertIsNone(
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            collapsed("float GetAppliedCameraShakeScale();"),
            compact_origin,
        )
    ...
    def test_get_applied_camera_shake_scale_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float GetAppliedCameraShakeScale"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(
            LOCKED_DECL,
            "float GetAppliedCameraShakeScale() const;",
        )
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BODY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SHOTS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RESET)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FILL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_DAMAGE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HITS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HITS_INLINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_SENSITIVITY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_SENSITIVITY_INLINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_INVERT_LOOK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_INVERT_LOOK_INLINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GUS_SHAKE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_INT32)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_LENGTH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FIRE_CAMERA_CLASS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HITS_EQ)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SNAPSHOT_HITS)
        ...
        self.assertNotIn("GetSortieShotsFired", LOCKED_DECL)
        self.assertNotIn("ResetSortieCombatStats", LOCKED_DECL)
        self.assertNotIn("GetAppliedLookSensitivity", LOCKED_DECL)
        self.assertNotIn("IsVerticalLookInverted", LOCKED_DECL)
        self.assertNotIn("GetCameraShakeScale", LOCKED_DECL)
        self.assertNotIn("SetCameraShakeScale", LOCKED_DECL)
        self.assertNotIn("45000.f", LOCKED_DECL)
        self.assertNotIn("RouteLengthCm", LOCKED_DECL)
        self.assertNotIn("TSubclassOf", LOCKED_DECL)
        self.assertNotIn("FireCameraShakeClass", LOCKED_DECL)
        self.assertNotIn("Hits =", LOCKED_DECL)
        self.assertNotIn("FireRate", LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_YAW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MISSION_ID)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat GetAppliedCameraShakeScales() const;\n",
            "\tint32 GetAppliedCameraShakeScale() const;\n",
            "\tFName GetAppliedCameraShakeScale() const;\n",
            "\tfloat GetAppliedCameraShakeScale() const { return " + forty + "; }\n",
            "\tfloat GetAppliedCameraShakeScale() const { return " + eighty + "; }\n",
            f"\t{TARGET_WRONG_LAMP}\n",
            f"\t{TARGET_WRONG_PEAK}\n",
            ...
            f"\t{TARGET_WRONG_BODY}\n",
            f"\t{TARGET_WRONG_CLONE_DECL}\n",
            f"\t{TARGET_WRONG_SHOTS}\n",
            f"\t{TARGET_WRONG_RESET}\n",
            f"\t{TARGET_WRONG_FILL}\n",
            f"\t{TARGET_WRONG_HITS}\n",
            f"\t{TARGET_WRONG_HITS_INLINE}\n",
            f"\t{TARGET_WRONG_LOOK_SENSITIVITY}\n",
            f"\t{TARGET_WRONG_LOOK_SENSITIVITY_INLINE}\n",
            f"\t{TARGET_WRONG_INVERT_LOOK}\n",
            f"\t{TARGET_WRONG_INVERT_LOOK_INLINE}\n",
            f"\t{TARGET_WRONG_GUS_SHAKE}\n",
            f"\t{TARGET_WRONG_INT32}\n",
            f"\t{TARGET_WRONG_ROUTE_LENGTH}\n",
            f"\t{TARGET_WRONG_FIRE_CAMERA_CLASS}\n",
            f"\t{TARGET_WRONG_HITS_EQ}\n",
            f"\t{TARGET_WRONG_SNAPSHOT_HITS}\n",
            f"\t{TARGET_WRONG_BOOM}\n",
    ...
    def test_missing_category_or_blueprint_pure_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Settings"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        self.assertNotIn('Skyguard|MissionMap', origin)
        self.assertIn('Skyguard|Settings', origin)
        self.assertNotIn('Category="Settings"', origin)
        self.assertNotIn('Settings|Input', origin)
        self.assertNotIn('Settings|Audio', origin)
        self.assertNotIn('Settings|Accessibility', origin)
        self.assertNotIn('Skyguard|Combat|Sortie', origin)
        self.assertNotIn('Skyguard|Combat|Feedback', origin)
        self.assertNotIn('Skyguard|Combat|Safety', origin)
        self.assertIsNone(
            re.search(r'Category="Skyguard\|Combat"(?!\|)', origin)
        )
        ...
        self.assertNotIn('Category="Skyguard|Settings"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Settings"', theater_specs)
        self.assertIn('Category="Skyguard|Settings"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Settings"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Settings"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Settings"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Settings"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Settings"', audio_only_specs)
        self.assertIn('Category="Skyguard|Settings"', origin)
        leftover_callable = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
        ...
        self.assertNotIn('Category="Skyguard|Settings"', combat_bare_specs)
        leftover_feedback = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Settings"', feedback_specs)
        leftover_safety = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Settings"', safety_specs)
        leftover_boss = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Settings"', boss_specs)
        leftover_settings_bare = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_SETTINGS}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        settings_bare_specs = attached_uproperty_specifiers(
            spec_section(leftover_settings_bare)
        )
        self.assertIn('Category="Settings"', settings_bare_specs)
        self.assertNotIn('Category="Skyguard|Settings"', settings_bare_specs)
        leftover_settings_access = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_SETTINGS_ACCESSIBILITY}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        settings_access_specs = attached_uproperty_specifiers(
            spec_section(leftover_settings_access)
        )
        self.assertIn(
            'Category="Settings|Accessibility"',
            settings_access_specs,
        )
        self.assertNotIn(
            'Category="Skyguard|Settings"',
            settings_access_specs,
        )
        leftover_settings_input = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_SETTINGS_INPUT}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        settings_input_specs = attached_uproperty_specifiers(
            spec_section(leftover_settings_input)
        )
        self.assertIn('Category="Settings|Input"', settings_input_specs)
        self.assertNotIn('Category="Skyguard|Settings"', settings_input_specs)
        leftover_settings_audio = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_SETTINGS_AUDIO}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        settings_audio_specs = attached_uproperty_specifiers(
            spec_section(leftover_settings_audio)
        )
        self.assertIn('Category="Settings|Audio"', settings_audio_specs)
        self.assertNotIn('Category="Skyguard|Settings"', settings_audio_specs)
        leftover_sortie = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_SORTIE}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        sortie_specs = attached_uproperty_specifiers(
            spec_section(leftover_sortie)
        )
        self.assertIn('Category="Skyguard|Combat|Sortie"', sortie_specs)
        self.assertNotIn('Category="Skyguard|Settings"', sortie_specs)
        wraps = (
            "\tfloat\n\tGetAppliedCameraShakeScale() const;\n",
            "\tfloat   GetAppliedCameraShakeScale() const;\n",
            "\tfloat\tGetAppliedCameraShakeScale() const;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_ufunctions_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetAppliedCameraShakeScale"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_GET_SORTIE_SHOTS))
        self.assertFalse(has_identifier(section, SIBLING_GET_SORTIE_SHOTS))
        self.assertTrue(has_identifier(leaked, SIBLING_RESET_SORTIE))
        self.assertFalse(has_identifier(section, SIBLING_RESET_SORTIE))
        self.assertTrue(has_identifier(leaked, SIBLING_FILL_SORTIE))
        self.assertFalse(has_identifier(section, SIBLING_FILL_SORTIE))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GetAppliedCameraShakeScale"))
        self.assertTrue(has_identifier(section, "GetAppliedCameraShakeScale"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_SORTIE_SHOTS))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_FILL_SORTIE))
        self.assertFalse(has_identifier(section, SIBLING_FILL_SORTIE))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_SORTIE_DAMAGE))
        self.assertFalse(has_identifier(section, SIBLING_GET_SORTIE_DAMAGE))
        self.assertTrue(has_identifier(leaked, SIBLING_BOOM))
        self.assertFalse(has_identifier(section, SIBLING_BOOM))
        self.assertTrue(has_identifier(leaked, SIBLING_FIRE_CAMERA))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_CAMERA))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner GetAppliedCameraShakeScale decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_get_applied_camera_shake_scale_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AUDIO_DIRECTOR_GET_RESOLVED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_RESULT_HITS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_HITS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_FILL_RESULT_COMBAT_STATS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_FIRE_CAMERA_SHAKE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_MIN_SAFE_YAW, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_GET_SORTIE_HITS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_GET_SORTIE_SHOTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_RESET_SORTIE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_GET_LOOK_SENSITIVITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_IS_VERTICAL_INVERT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUS_GET_CAMERA_SHAKE_SCALE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUS_SET_CAMERA_SHAKE_SCALE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_IRON_RAIN_MAX_RELEASES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LAST_FLIGHT_CIVILIAN, LOCKED_SCRIPTS)
        leftovers = (
            LEFTOVER_MISSION_RESULT_DEFAULTS,
            LEFTOVER_MISSION_DEBRIEF_DEFAULTS,
            LEFTOVER_OBJECTIVE_PROGRESS_DEFAULTS,
