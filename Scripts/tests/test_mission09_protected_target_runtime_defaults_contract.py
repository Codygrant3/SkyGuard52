...
# Isolated-test drafts #107–#223 and newer stay off this lane.
# Mission 07/10 protected-target drafts and Harbor clocks stay sibling-only.
LOCKED_SCRIPTS = (
...
# Apache CPG escort integrity. Lock the Target default only — not the
# full ESkyguardMission09ProtectedTarget enumerator set.
PUBLIC_FIELDS = (
...
# Readiness (bYakRuntimeReady), wave state, and the full protected-target
# enumerator set stay unlocked. MetropolitanSkyline is an escort-category
# default, not the Harbor IncomingRadar 40/80 inbound clock.
TYPES_NOT_LOCKED = (
...
def in_class_defaults(body: str) -> dict[str, str]:
    return {
        ...
        for name, value in re.findall(
            r"(?:ESkyguardMission09ProtectedTarget|int32|bool)\s+(\w+)\s*=\s*([^;]+);",
            body,
        )
    }


class Mission09ProtectedTargetRuntimeDefaultsContractTests(unittest.TestCase):
    ...
    def test_contract_does_not_lock_readiness_wave_or_full_enum(self) -> None:
        body = protected_target_body(origin_main_header())
        defaults = in_class_defaults(body)
    ...
    def test_struct_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                ...
                "protected-target runtime is Apache CPG escort integrity, "
                "not Yak",
            )
