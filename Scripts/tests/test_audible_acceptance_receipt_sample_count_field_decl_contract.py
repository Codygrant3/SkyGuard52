...
# then `int32 SampleCount = 0;`.
# BlueprintReadOnly IS present. There is NO EditAnywhere.
# There is NO Category=. Required initializer `= 0`
# (not Harbor 40/80; Harbor is only forty-dot-f / eighty-dot-f).
# Fail-closed if field missing/renamed, EditAnywhere added,
# BlueprintReadWrite replaces BlueprintReadOnly, type is not
# int32, initializer is not `= 0`, or Category= is added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardAudibleAcceptanceReceipt` body ONLY
# after `struct FSkyguardAudibleAcceptanceReceipt`. Stop
# BEFORE `class SKYGUARD52_API USkyguardAudioAcceptanceHarness`.
...
# (keep that enum OUT of the parse window).
# Do NOT claim sibling fields: BuildSha256 / EvidenceSha256 /
...
# #1cd7 then leftover-safe isolated BuildSha256 in-flight then
# this isolated SampleCount field decl.
# THIS IS NOT leftover #193 / #1cd7 (keep that bulk file in
# LOCKED_SCRIPTS).
...
# (has MinimumMeasuredSamples = 600).
# THIS IS NOT leftover enum class ESkyguardProductionAudioCategory
# (Ri + fleMuzzle / Ig + laSearch).
# THIS IS NOT leftover MissionResult #1264/#1265/#1426-#1432.
# THIS IS NOT leftover RouteDefinition RouteId/Points.
# THIS IS NOT leftover MeshBindSlot #194 / #6829.
# Clone LOCKED_SCRIPTS + parse-window style from leftover-safe
# draft #1300 (theater-kit-spec WeatherIdentity). If a clone
# asserts VisibleAnywhere / Category="Skyguard|Theater" /
# BlueprintReadWrite / FName WeatherIdentity / no initializer /
# EditAnywhere, RETARGET: UPROPERTY is BlueprintReadOnly with
# NO EditAnywhere, type is int32, REQUIRED initializer `= 0`,
# no Category=.
# Harbor 40/80 fail-closed. `= 0` is not Harbor.
...
TARGET = "int32 SampleCount = 0;"
TARGET_WRONG_BARE = "int32 SampleCount;"
TARGET_WRONG_FALSE = "int32 SampleCount = false;"
TARGET_WRONG_TRUE = "int32 SampleCount = true;"
TARGET_WRONG_ZERO = "int32 SampleCount = 0.f;"
TARGET_WRONG_FORTY = "int32 SampleCount = 40;"
TARGET_WRONG_EIGHTY = "int32 SampleCount = 80;"
TARGET_WRONG_FLOAT = "float SampleCount = 0;"
TARGET_WRONG_FNAME = "FName SampleCount;"
TARGET_WRONG_BOOL = "bool SampleCount = false;"
TARGET_WRONG_FSTRING = "FString SampleCount;"
TARGET_WRONG_TARRAY = "TArray<FName> SampleCount;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> SampleCount;"
...
LEFTOVER_HARNESS_CATEGORY = 'Category="Skyguard|Audio|Acceptance"'
LEFTOVER_HARNESS_MIN_SAMPLES = "MinimumMeasuredSamples"
LEFTOVER_HARNESS_MAX_VOICES = "MaximumAllowedVoices"
LEFTOVER_HARNESS_MIN_SAMPLES_INIT = "MinimumMeasuredSamples = 600"
LEFTOVER_HARNESS_MAX_VOICES_INIT = "MaximumAllowedVoices = 48"
...
SIBLING_MAXIMUM_TRUE_PEAK = "MaximumTruePeakDbTP"
SIBLING_PACKAGED_DEV_BUILD = "bPackagedDevelopmentBuild"
SIBLING_AUDIBLE_DEVICE = "bAudibleDeviceObserved"
...
# Leftover ProductionAudio Audit count siblings stay named so
# LOCKED_SCRIPTS comments can keep #1412 out of this contract.
LEFTOVER_AUDIT_REQUIRED_CATEGORY_COUNT = "RequiredCategoryCount"
LEFTOVER_AUDIT_EXPLICIT_MISSING_SOURCE_COUNT = "ExplicitMissingSourceCount"
LEFTOVER_AUDIT_QA_TEST_ONLY_COUNT = "QATestOnlyCount"
LEFTOVER_AUDIT_BOUND_PRODUCTION_SOURCE_COUNT = "BoundProductionSourceCount"
...
THIS_SCRIPT = (
    "Scripts/tests/test_audible_acceptance_receipt"
    "_sample_count_field_decl_contract.py"
)
...
LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS_TESTS = (
    "Scripts/tests/test_audible_acceptance_receipt_defaults_tests.py"
)
LEFTOVER_BOUND_PRODUCTION_SOURCE_COUNT = (
    "Scripts/tests/test_production_audio_audit_bound"
    "_production_source_count_field_decl_contract.py"
)
LEFTOVER_BUILD_SHA256 = (
    "Scripts/tests/test_audible_acceptance_receipt_build_sha256"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION_RESULT_HITS = (
    "Scripts/tests/test_mission_result_hits_field_decl_contract.py"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_MAXIMUM_TRUE_PEAK,
        SIBLING_PACKAGED_DEV_BUILD,
        SIBLING_AUDIBLE_DEVICE,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 SampleCount = 0;`.
    # Reject dropped initializer, Harbor 40/80, false, float, bool,
    # FName, FString, TArray, TSoftObjectPtr.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+SampleCount\s*;",
        compact,
    ) and re.search(
        r"int32\s+SampleCount\s*=",
        compact,
        ...
        return False
    if re.search(
        r"SampleCount\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"SampleCount\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"SampleCount\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"SampleCount\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"SampleCount\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SampleCount\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+SampleCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+SampleCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+SampleCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+SampleCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+SampleCount\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+SampleCount\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def parse_window(header: str) -> str:
    ...
    if header.find(STOP_BEFORE_HARNESS_NAIVE) >= 0:
        raise AssertionError(
            ...
            "leftover harness class fields would leak"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+SampleCount\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 SampleCount is missing from "
...
class ProductionAudioAuditSampleCountFieldDeclContractTests(
    ...
    def test_audible_acceptance_receipt_struct_exists(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_UCLASS, header)
        ...
        self.assertNotIn(LEFTOVER_HARNESS_CATEGORY, section)
        self.assertNotIn(LEFTOVER_HARNESS_MIN_SAMPLES, section)
        self.assertNotIn(LEFTOVER_HARNESS_MAX_VOICES, section)
        self.assertIn(LEFTOVER_HARNESS_CATEGORY, header)
        self.assertIn(LEFTOVER_HARNESS_MIN_SAMPLES_INIT, header)
        self.assertIn(LEFTOVER_HARNESS_MAX_VOICES_INIT, header)
    ...
    def test_missing_bound_count_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            ...
            f"\tint32 {LEFTOVER_AUDIT_BOUND_PRODUCTION_SOURCE_COUNT} = 0;\n"
            "};\n"
        )
    ...
    def test_initializer_drop_or_harbor_change_fails_closed(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
        ...
        self.assertIn("int32 SampleCount = 0;", compact_origin)
        self.assertNotIn("SampleCount = 40", compact_origin)
        self.assertNotIn("SampleCount = 80", compact_origin)
        self.assertNotIn("SampleCount = false", compact_origin)
        self.assertNotIn("SampleCount;", compact_origin.replace(
            "int32 SampleCount = 0;", ""
        ))

    def test_bound_count_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 SampleCount"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            f"\tint32 {LEFTOVER_AUDIT_BOUND_PRODUCTION_SOURCE_COUNT} = 0;\n",
            "\tint32 SampleCounts = 0;\n",
            "\tint32 SampleCount = 1;\n",
            "\tbool SampleCount;\n",
            "\tfloat SampleCount = " + forty + ";\n",
            "\tfloat SampleCount = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_receipt_fields(self) -> None:
        ...
        self.assertFalse(
            has_identifier(section, LEFTOVER_AUDIT_REQUIRED_CATEGORY_COUNT),
            section,
        )
        self.assertFalse(
            has_identifier(section, LEFTOVER_AUDIT_BOUND_PRODUCTION_SOURCE_COUNT),
            section,
        )
        self.assertNotIn(LEFTOVER_HARNESS_MIN_SAMPLES, section)
        self.assertNotIn(LEFTOVER_HARNESS_MAX_VOICES, section)
    ...
    def test_does_not_parse_neighbors_or_leftover_banks(self) -> None:
        ...
        self.assertIn(LEFTOVER_HARNESS_CATEGORY, header)
        self.assertNotIn(LEFTOVER_HARNESS_CATEGORY, section)
        self.assertNotIn(LEFTOVER_HARNESS_CATEGORY, leaked)
        self.assertNotIn(LEFTOVER_HARNESS_CATEGORY, window)
        self.assertIn(LEFTOVER_HARNESS_MIN_SAMPLES, header)
        self.assertNotIn(LEFTOVER_HARNESS_MIN_SAMPLES, section)
        self.assertNotIn(LEFTOVER_HARNESS_MIN_SAMPLES, leaked)
        self.assertNotIn(LEFTOVER_HARNESS_MIN_SAMPLES, window)
        self.assertIn(LEFTOVER_HARNESS_MAX_VOICES, header)
        self.assertNotIn(LEFTOVER_HARNESS_MAX_VOICES, section)
        self.assertNotIn(LEFTOVER_HARNESS_MAX_VOICES, leaked)
        self.assertNotIn(LEFTOVER_HARNESS_MAX_VOICES, window)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, section)
    ...
    def test_naive_harness_class_stop_is_not_present(self) -> None:
        ...
        self.assertEqual(header.find(STOP_BEFORE_BANK_NAIVE), -1)
        self.assertIn(STOP_BEFORE_HARNESS, header)
        self.assertTrue(STOP_BEFORE_HARNESS.startswith("class SKYGUARD52_API "))
        self.assertNotIn(STOP_BEFORE_BANK, header)
        window = parse_window(header)
        self.assertNotIn(STOP_BEFORE_HARNESS, window)
        self.assertNotIn(STOP_BEFORE_HARNESS_NAIVE, window)
        ...
        self.assertNotIn(LEFTOVER_HARNESS_MIN_SAMPLES, window)
        self.assertNotIn(LEFTOVER_HARNESS_MAX_VOICES, window)
        self.assertNotIn(LEFTOVER_HARNESS_MIN_SAMPLES_INIT, window)
        self.assertNotIn(LEFTOVER_HARNESS_MAX_VOICES_INIT, window)
        self.assertNotIn("BeginEvidenceRun", window)
        self.assertNotIn("RecordMeasuredSample", window)
        self.assertNotIn("CompleteEvidenceRun", window)
        self.assertNotIn("GetReceipt", window)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit SampleCount field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "sample_count_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_BOUND_PRODUCTION_SOURCE_COUNT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_BUILD_SHA256, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_RESULT_HITS, LOCKED_SCRIPTS)
