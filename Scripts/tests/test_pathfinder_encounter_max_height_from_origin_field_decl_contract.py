...
# then `float MaxHeightFromOriginCm = 1200.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Safety`. EditAnywhere IS present.
# BlueprintReadWrite IS present. There is NO meta= / NO ClampMin /
# NO ClampMax. Default IS `= 1200.f`.
...
# It is NOT leftover analog ApproachSpeedCmPerSecond (leave sibling
# uncontracted).
...
# if wrap still has ClampMin / ClampMax / meta=, if the default is not
# `= 1200.f`, if Category is Skyguard|Apache / Skyguard|Combat /
# Skyguard|Theater / Skyguard|MissionMap / Skyguard|Audio|Acceptance /
...
# Do NOT claim leftover Safety sibling MaxLateralOffsetCm
# (already leftover #1667) / MaxSimulationStepSeconds /
# MaxSimulationSubsteps.
# Do NOT claim Commands CommandLateralBiasCm / BreakClimbCm.
# Do NOT claim Telegraph interval fields.
# SKIP leftover analog MinHeightFromOriginCm = -
# plus Harbor 80 float token. Never contract it.
...
# (leftover UFUNCTION #376-#382).
# Do NOT claim leftover analog Pathfinder EncounterController
# TObjectPtr field (pathfinder-encounter-controller-field #512).
...
# Clone wrap was leftover Pathfinder Safety draft MaxLateralOffset
...
# Category="Skyguard|Boss|Encounter|Safety" with NO meta= /
# NO ClampMin / NO ClampMax (NO spaces around =).
# LOCKED_DECL is `float MaxHeightFromOriginCm = 1200.f;`
# NOT leftover analog `float MaxLateralOffsetCm = 3600.f;`
# NOT leftover analog MinHeightFromOriginCm Harbor 80 token
...
# / ApproachSpeedCmPerSecond / MaxLateralOffsetCm = 3600.f
# / ClampMin / leftover analog MinHeightFromOriginCm
# AS THE LOCKED DECL.
...
# Fail-closed if wrap still has ClampMin / ClampMax / meta=
# or default is not 1200.f.
# Default is `= 1200.f` not `= 3600.f` / `= 850.f` / `= 700.f` /
# `= 1250.f` / `= 9000.f`.
...
TARGET = "float MaxHeightFromOriginCm = 1200.f;"
TARGET_WRONG_BARE = "float MaxHeightFromOriginCm;"
TARGET_WRONG_INIT = "float MaxHeightFromOriginCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float MaxHeightFromOriginCm = true;"
TARGET_WRONG_ZERO = "float MaxHeightFromOriginCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MaxHeightFromOriginCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MaxHeightFromOriginCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MaxHeightFromOriginCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "MaxSimulationStepSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_max_height_from_origin"
...
MAX_HEIGHT_DECL_RE = re.compile(
    r"float\s+MaxHeightFromOriginCm\s*=\s*1200\.f\s*;"
)
MAX_HEIGHT_BARE_RE = re.compile(
    r"float\s+MaxHeightFromOriginCm\s*;"
)
MAX_LATERAL_OFFSET_DECL_RE = re.compile(
    r"float\s+MaxLateralOffsetCm\s*=\s*3600\.f\s*;"
)
MIN_HEIGHT_DECL_RE = re.compile(
    r"float\s+MinHeightFromOriginCm\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float MaxHeightFromOriginCm = 1200.f;`.
    ...
    # leftover analog MaxLateralOffsetCm = 3600.f,
    # leftover analog MinHeightFromOriginCm Harbor 80 token,
    ...
    # MaxHeightFromOriginCm without the 1200.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MAX_HEIGHT_DECL_RE.search(compact) is None:
        return False
    if MAX_HEIGHT_BARE_RE.search(compact):
        return False
    if MAX_LATERAL_OFFSET_DECL_RE.search(compact):
        return False
    if MIN_HEIGHT_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "MaxHeightFromOriginCm":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MaxHeightFromOriginCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaxHeightFromOriginCm "
...
class PathfinderEncounterMaxHeightFromOriginFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxHeightFromOriginCm"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_MAX_LATERAL), body)
        self.assertFalse(has_identifier(section, SIBLING_MAX_LATERAL), section)
        self.assertTrue(has_identifier(body, SIBLING_MIN_HEIGHT), body)
        self.assertFalse(has_identifier(section, SIBLING_MIN_HEIGHT), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MaxHeightFromOriginCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaxHeightFromOriginCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_max_height_from_origin_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float MaxHeightFromOriginCm;",
            compact_origin.split("MaxHeightFromOriginCm = 1200.f;")[0]
            + compact_origin.split("MaxHeightFromOriginCm = 1200.f;")[-1]
            if "MaxHeightFromOriginCm = 1200.f;" in compact_origin
            else compact_origin,
        )
        self.assertNotIn("float MaxLateralOffsetCm = 3600.f;", compact_origin)
        self.assertNotIn("MinHeightFromOriginCm", compact_origin)
    ...
    def test_max_height_from_origin_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MaxHeightFromOriginCm"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat MaxHeightFromOriginCms = 1200.f;\n",
            "\tint32 MaxHeightFromOriginCm = 850;\n",
            "\tFName MaxHeightFromOriginCm = NAME_None;\n",
            "\tfloat MaxHeightFromOriginCm = " + forty + ";\n",
            "\tfloat MaxHeightFromOriginCm = " + eighty + ";\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxHeightFromOriginCm"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_MAX_LATERAL))
        self.assertFalse(has_identifier(section, SIBLING_MAX_LATERAL))
        self.assertTrue(has_identifier(leaked, SIBLING_MIN_HEIGHT))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "MaxHeightFromOriginCm"))
        self.assertTrue(has_identifier(section, "MaxHeightFromOriginCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_MAX_LATERAL))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn("meta=", UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter max height from origin field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_max_height_from_origin_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
