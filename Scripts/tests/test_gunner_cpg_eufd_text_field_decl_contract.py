# THIS IS leftover-safe ASkyguardGunner CpgEufdText.
...
# then BARE `TObjectPtr<UTextRenderComponent> CpgEufdText;`
# (no initializer).
...
# Stop BEFORE leftover guided muzzle or private: as a claimed slot.
...
# This is a TEXT slot. Do not lock leftover mesh CpgEufd
# #1604 as this slot. Do not lock leftover CpgConsoleRight
# #1612 / CpgRailLeft #1611 / CpgRailRight #1610 /
# CpgReticleV #1609. Fail-closed leftover Apache Canopy
# #1559 / PilotCanopy #1572.
# CpgEufdText fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UTextRenderComponent> CpgEufdText;`
...
# Text slot fail-closed if LOCKED_DECL is
# `TObjectPtr<UStaticMeshComponent>` or
# `TObjectPtr<USceneComponent>`.
# CpgEufdText fail-closed if LOCKED_DECL is leftover
# mesh CpgMpdLeft / CpgRailRight / CpgRailLeft /
# CpgConsoleRight / CpgReticleV / CpgGripRight /
# CpgGripLeft / CpgReticleH / CpgTedacBezel / CpgTedacText /
# CpgMpdRightText / CpgMpdLeftText / CpgEufd / CpgTedac /
# CpgDash / CpgKneeRight / CpgKneeLeft / CpgSeatBack /
# CpgCockpit / CpgStation / CpgSeatPan / CpgCanopyBow /
...
TARGET = "TObjectPtr<UTextRenderComponent> CpgEufdText;"
TARGET_WRONG_INIT = "TObjectPtr<UTextRenderComponent> CpgEufdText = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UTextRenderComponent> CpgEufdText = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UTextRenderComponent> CpgEufdText = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgEufdText;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgEufdText;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CpgEufdText;"
...
CPG_EUFD_TEXT_DECL_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgEufdText\s*;"
)
CPG_EUFD_TEXT_INIT_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgEufdText\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover mesh CpgMpdLeft / CpgRailRight /
    # CpgRailLeft / CpgConsoleRight / CpgReticleV /
    # leftover Apache Canopy, leftover Apache
    # PilotCanopy, or an initializer on CpgEufdText
    # as the slot. Text slot fail-closed if LOCKED_DECL
    # is still UStaticMeshComponent or USceneComponent.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_EUFD_TEXT_DECL_RE.search(compact) is None:
        return False
    if CPG_EUFD_TEXT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, leftover_banned_guided_muzzle()):
            break
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UTextRenderComponent>\s+CpgEufdText\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UTextRenderComponent> CpgEufdText "
...
class GunnerCpgEufdTextFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgEufdText"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgEufdText"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgEufdText", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_eufd_text_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UTextRenderComponent> CpgEufdText;", compact_origin)
        self.assertNotIn("CpgEufdText = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_eufd_text_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UTextRenderComponent> CpgEufdText"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("UTextRenderComponent", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UTextRenderComponent> CpgEufdText;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UTextRenderComponent> CpgEufdTexts;\n",
            "\tint32 CpgEufdText = 140;\n",
            "\tbool CpgEufdText = true;\n",
            "\tfloat CpgEufdText = " + forty + ";\n",
            "\tfloat CpgEufdText = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_mpd_right_text_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "CpgEufdText"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgEufdText"))
        self.assertTrue(has_identifier(section, "CpgEufdText"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgEufdText field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_eufd_text_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
