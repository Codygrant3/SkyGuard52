...
# INDEX_NONE or lock HeatManifold construction
# in the .cpp. This is a FIELD contract on
# ASkyguardRunwayBreakerBoss, not leftover
# USkyguardBossWeakPointComponent member
# contract. Keep HeatManifold distinct from
# sibling RunwayBreaker fields RunwayRack,
# HangarRack, PortEngine, DebrisPortWing,
# DebrisPayloadBay, and DebrisEngine. Those
...
# HeatManifold;`); accept that form and other
...
# HeatManifold, Engine, DebrisPortEWPanel,
...
# HeatManifold field declaration contract on
...
# unlocked for THIS test's primary HeatManifold lock. Leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRunwayBreakerBoss::HeatManifold",
    "SkyguardRunwayBreakerBoss.cpp",
    "SetExposed",
...
class RunwayBreakerHeatManifoldFieldDeclContractTests(unittest.TestCase):
    def test_runway_breaker_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, HEAT_MANIFOLD_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, HEAT_MANIFOLD_FIELD)
    ...
    def test_missing_heat_manifold_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRunwayBreakerBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("VisibleAnywhere", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("BlueprintReadOnly", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("Category", HEAT_MANIFOLD_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, HEAT_MANIFOLD_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* HeatManifold;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> HeatManifold;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> HeatManifold;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, HEAT_MANIFOLD_FIELD)
            self.assertIn("HeatManifold", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_heat_manifold_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, HEAT_MANIFOLD_FIELD),
            HEAT_MANIFOLD_FIELD,
        )
        self.assertTrue(has_declaration(section, HEAT_MANIFOLD_FIELD))
        self.assertEqual(declaration_count(section, HEAT_MANIFOLD_FIELD), 1)
        self.assertTrue(
            HEAT_MANIFOLD_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            HEAT_MANIFOLD_FIELD,
        )
        self.assertTrue(HEAT_MANIFOLD_FIELD.endswith(";"), HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisCoolingDoor", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SignatureModulator", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("RadarReceiver", HEAT_MANIFOLD_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            HEAT_MANIFOLD_FIELD,
        )
        self.assertIn("TObjectPtr<", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("TArray<", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("=", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("INDEX_NONE", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("UFUNCTION", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("{", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("}", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("return ", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("CommandAntenna", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ControlLinkage", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("NoseCamera", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisCenter", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisTail", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisNose", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisSpine", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("AdvanceEncounter", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ResetEncounterState", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("IsRouteStateSafe", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetRouteProgress", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("RadarNode", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("IssueClimbCommand", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetFinaleStage", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("IsClimbCommandIssued", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("IsWreckDiverted", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetGunnerMount", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("FaceWorldLocation", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("PortGuidanceArray", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisArmorPort", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetSearchlightTracked", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetContactIdentified", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetLightningExposed", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("HullCollider", HEAT_MANIFOLD_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, HEAT_MANIFOLD_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {HEAT_MANIFOLD_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, HEAT_MANIFOLD_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{HEAT_MANIFOLD_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, HEAT_MANIFOLD_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, HEAT_MANIFOLD_FIELD),
            section,
    ...
    def test_assigned_heat_manifold_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "HeatManifold = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, HEAT_MANIFOLD_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("HeatManifold", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, HEAT_MANIFOLD_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{HEAT_MANIFOLD_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, HEAT_MANIFOLD_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, HEAT_MANIFOLD_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            HEAT_MANIFOLD_FIELD.startswith("UFUNCTION"),
            HEAT_MANIFOLD_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, HEAT_MANIFOLD_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", HEAT_MANIFOLD_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_heat_manifold_cpp_body(self) -> None:
        locked_only = f"{HEAT_MANIFOLD_FIELD}\n"
        self.assertNotIn("{", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("}", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("return ", HEAT_MANIFOLD_FIELD)
        ...
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", locked_only)
        self.assertNotIn("return false", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("return true", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("CreateDefaultSubobject", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetExposed", HEAT_MANIFOLD_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            HEAT_MANIFOLD_FIELD,
        )
        self.assertNotIn("SetBossPhase", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", HEAT_MANIFOLD_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{HEAT_MANIFOLD_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("PortDischargeBoom", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("StarboardDischargeBoom", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("EngineIntake", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisPortPanel", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisStarboardPanel", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisIntakePanel", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", HEAT_MANIFOLD_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            HEAT_MANIFOLD_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("EncounterController", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ControlServo", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DispenserCenter", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("MaxReleasesPerBay", HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetContactIdentified", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("OpenOrbitExposure", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("IsContactIdentified", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), HEAT_MANIFOLD_FIELD)

    def test_debris_cooling_door_does_not_satisfy_heat_manifold(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HEAT_MANIFOLD_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("UStaticMeshComponent", HEAT_MANIFOLD_FIELD)
        self.assertNotIn(
            "USkyguardBossWeakPointComponent",
            DEBRIS_COOLING_DOOR_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DispenserStarboard", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetManeuver", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), HEAT_MANIFOLD_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetSearchlightTracked", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SetLightningExposed", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("IsLightningExposed", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), HEAT_MANIFOLD_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardGunner", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SkyguardRadarNode", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("MaxIntegrity", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("CurrentIntegrity", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", HEAT_MANIFOLD_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            HEAT_MANIFOLD_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{HEAT_MANIFOLD_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, HEAT_MANIFOLD_FIELD),
            HEAT_MANIFOLD_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRunwayBreakerBoss::HeatManifold", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "runway breaker HeatManifold field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{HEAT_MANIFOLD_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, HEAT_MANIFOLD_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                HEAT_MANIFOLD_FIELD.lower(),
                "runway breaker HeatManifold contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, HEAT_MANIFOLD_FIELD)

    def test_contract_is_heat_manifold_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(HEAT_MANIFOLD_FIELD.endswith(";"))
        self.assertNotIn("=", HEAT_MANIFOLD_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_runway_breaker_fields(
        ...
        for token in SIBLING_RUNWAY_BREAKER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("RunwayRack", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("HangarRack", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("PortEngine", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisPortWing", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisPayloadBay", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisEngine", HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", HEAT_MANIFOLD_FIELD)
        self.assertIn(
            "Scripts/tests/test_runway_breaker_runway_rack_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        for token in SIBLING_ROAD_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("TargetingCamera", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("LeftActuator", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("RightActuator", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardRoadHunterBoss", HEAT_MANIFOLD_FIELD)
        self.assertIn(
            "Scripts/tests/test_road_hunter_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("PortLatch", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("StarboardLatch", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DecoyPods", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ElevatorLinkage", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", HEAT_MANIFOLD_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_heat_manifold_is_not_road_hunter_or_breakwater_engine(self) -> None:
        locked_only = f"{HEAT_MANIFOLD_FIELD}\n"
        self.assertNotEqual(HEAT_MANIFOLD_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotEqual(HEAT_MANIFOLD_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(HEAT_MANIFOLD_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(collapsed(HEAT_MANIFOLD_FIELD), collapsed(ENGINE_FIELD))
        ...
        for token in LEFTOVER_BREAKWATER_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("Engine;", HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", HEAT_MANIFOLD_FIELD)
    ...
    def test_runway_breaker_port_engine_is_not_road_hunter_engine(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HEAT_MANIFOLD_FIELD))
        self.assertNotEqual(PORT_ENGINE_FIELD, HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", HEAT_MANIFOLD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("OpticalTracker", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("WeaponServo", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("CountermeasurePod", HEAT_MANIFOLD_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", HEAT_MANIFOLD_FIELD)

    def test_debris_engine_does_not_satisfy_heat_manifold(self) -> None:
        leftover = f"\t{RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
    ...
    def test_port_engine_does_not_satisfy_heat_manifold(self) -> None:
        leftover = f"\t{PORT_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HEAT_MANIFOLD_FIELD)
        self.assertIn("HeatManifold", str(raised.exception))
    ...
    def test_runway_rack_and_hangar_rack_do_not_satisfy_heat_manifold(
        self,
    ) -> None:
        for region, name in (
            (f"\t{RUNWAY_RACK_FIELD}\n", "RunwayRack"),
    ...
    def test_debris_payload_bay_does_not_satisfy_heat_manifold(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HEAT_MANIFOLD_FIELD))
        self.assertNotEqual(DEBRIS_PAYLOAD_BAY_FIELD, HEAT_MANIFOLD_FIELD)
        self.assertNotIn("DebrisPayloadBay", HEAT_MANIFOLD_FIELD)
