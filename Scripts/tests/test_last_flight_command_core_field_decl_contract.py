...
# INDEX_NONE or lock CommandCore
...
# CommandCore;`); accept that form and
...
# Python CommandCore field declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::CommandCore",
    "SkyguardLastFlightBoss.cpp",
    "SetExposed",
...
class LastFlightCommandCoreFieldDeclContractTests(unittest.TestCase):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, COMMAND_CORE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, COMMAND_CORE_FIELD)
        self.assertIn("CommandCore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, COMMAND_CORE_FIELD)
    ...
    def test_missing_command_core_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLastFlightBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, COMMAND_CORE_FIELD)
        self.assertIn("CommandCore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, COMMAND_CORE_FIELD)
        self.assertIn("CommandCore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", COMMAND_CORE_FIELD)
        self.assertNotIn("VisibleAnywhere", COMMAND_CORE_FIELD)
        self.assertNotIn("BlueprintReadOnly", COMMAND_CORE_FIELD)
        self.assertNotIn("Category", COMMAND_CORE_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, COMMAND_CORE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, COMMAND_CORE_FIELD)
        self.assertIn("CommandCore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* CommandCore;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> CommandCore;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, COMMAND_CORE_FIELD)
            self.assertIn("CommandCore", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_command_core_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, COMMAND_CORE_FIELD),
            1,
        )
        self.assertTrue(
            COMMAND_CORE_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            COMMAND_CORE_FIELD,
        )
        self.assertTrue(
            COMMAND_CORE_FIELD.endswith(";"),
            COMMAND_CORE_FIELD,
        )
        self.assertIn("CommandCore", COMMAND_CORE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            COMMAND_CORE_FIELD,
        )
        self.assertIn("TObjectPtr<", COMMAND_CORE_FIELD)
        self.assertNotIn("TArray<", COMMAND_CORE_FIELD)
        self.assertNotIn("=", COMMAND_CORE_FIELD)
        self.assertNotIn("INDEX_NONE", COMMAND_CORE_FIELD)
        self.assertNotIn("UFUNCTION", COMMAND_CORE_FIELD)
        self.assertNotIn("{", COMMAND_CORE_FIELD)
        self.assertNotIn("}", COMMAND_CORE_FIELD)
        self.assertNotIn("return ", COMMAND_CORE_FIELD)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            COMMAND_CORE_FIELD,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", COMMAND_CORE_FIELD)
        self.assertNotIn("IssueClimbCommand", COMMAND_CORE_FIELD)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            COMMAND_CORE_FIELD,
        )
        ...
        self.assertNotIn("GetCivilianSeparationMeters", COMMAND_CORE_FIELD)
        self.assertNotIn("GetFinaleStage", COMMAND_CORE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", COMMAND_CORE_FIELD)
        self.assertNotIn("IsWreckDiverted", COMMAND_CORE_FIELD)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            COMMAND_CORE_FIELD,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", COMMAND_CORE_FIELD)
        self.assertNotIn("GetGunnerMount", COMMAND_CORE_FIELD)
        self.assertNotIn("FaceWorldLocation", COMMAND_CORE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", COMMAND_CORE_FIELD)
        self.assertNotIn("PortCoolingSystem", COMMAND_CORE_FIELD)
        self.assertNotIn("PortEngine", COMMAND_CORE_FIELD)
        self.assertNotIn("SetSearchlightTracked", COMMAND_CORE_FIELD)
        self.assertNotIn("SetContactIdentified", COMMAND_CORE_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            COMMAND_CORE_FIELD,
        )
        self.assertNotIn("SetLightningExposed", COMMAND_CORE_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            COMMAND_CORE_FIELD,
        )
        self.assertNotIn("HullCollider", COMMAND_CORE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, COMMAND_CORE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {COMMAND_CORE_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{COMMAND_CORE_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, COMMAND_CORE_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, COMMAND_CORE_FIELD),
    ...
    def test_assigned_command_core_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "CommandCore = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, COMMAND_CORE_FIELD)
        self.assertIn("CommandCore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, COMMAND_CORE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            self.assertIn("CommandCore", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, COMMAND_CORE_FIELD))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, COMMAND_CORE_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            COMMAND_CORE_FIELD.startswith("UFUNCTION"),
            COMMAND_CORE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, COMMAND_CORE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", COMMAND_CORE_FIELD)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_lock_port_engine_cpp_body(
        ...
        self.assertNotIn("return false", COMMAND_CORE_FIELD)
        self.assertNotIn("return true", COMMAND_CORE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", COMMAND_CORE_FIELD)
        self.assertNotIn("SetExposed", COMMAND_CORE_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            COMMAND_CORE_FIELD,
        )
        self.assertNotIn("SetBossPhase", COMMAND_CORE_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_apply_strike(), COMMAND_CORE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), COMMAND_CORE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, COMMAND_CORE_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_last_flight_fields(self) -> None:
        ...
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("PortGuidanceArray", COMMAND_CORE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", COMMAND_CORE_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", COMMAND_CORE_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", COMMAND_CORE_FIELD)
        self.assertNotIn("StarboardCoolingSystem", COMMAND_CORE_FIELD)
        self.assertNotIn("Jammer", COMMAND_CORE_FIELD)
        self.assertNotIn("PortEngine", COMMAND_CORE_FIELD)
        self.assertNotIn("StarboardEngine", COMMAND_CORE_FIELD)
        self.assertNotIn("PortCoolingSystem", COMMAND_CORE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            COMMAND_CORE_FIELD,
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", COMMAND_CORE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("SetSearchlightTracked", COMMAND_CORE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("SetContactIdentified", COMMAND_CORE_FIELD)
        self.assertNotIn("OpenOrbitExposure", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("SetLightningExposed", COMMAND_CORE_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", COMMAND_CORE_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            COMMAND_CORE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, COMMAND_CORE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("ASkyguardGunner", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("SkyguardRadarNode", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, COMMAND_CORE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMMAND_CORE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", COMMAND_CORE_FIELD)
        self.assertNotIn("MaxIntegrity", COMMAND_CORE_FIELD)
        self.assertNotIn("CurrentIntegrity", COMMAND_CORE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, COMMAND_CORE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{COMMAND_CORE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, COMMAND_CORE_FIELD),
            COMMAND_CORE_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight CommandCore field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{COMMAND_CORE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, COMMAND_CORE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                COMMAND_CORE_FIELD.lower(),
                "last flight CommandCore contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, COMMAND_CORE_FIELD)

    def test_contract_is_command_core_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(COMMAND_CORE_FIELD.endswith(";"))
        self.assertNotIn("=", COMMAND_CORE_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
