...
# then `int32 BoundProductionSourceCount = 0;`.
...
TARGET = "int32 BoundProductionSourceCount = 0;"
TARGET_WRONG_BARE = "int32 BoundProductionSourceCount;"
TARGET_WRONG_FALSE = "int32 BoundProductionSourceCount = false;"
TARGET_WRONG_TRUE = "int32 BoundProductionSourceCount = true;"
TARGET_WRONG_ZERO = "int32 BoundProductionSourceCount = 0.f;"
TARGET_WRONG_FORTY = "int32 BoundProductionSourceCount = 40;"
TARGET_WRONG_EIGHTY = "int32 BoundProductionSourceCount = 80;"
TARGET_WRONG_FLOAT = "float BoundProductionSourceCount = 0;"
TARGET_WRONG_FNAME = "FName BoundProductionSourceCount;"
TARGET_WRONG_BOOL = "bool BoundProductionSourceCount = false;"
TARGET_WRONG_FSTRING = "FString BoundProductionSourceCount;"
TARGET_WRONG_TARRAY = "TArray<FName> BoundProductionSourceCount;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> BoundProductionSourceCount;"
...
FIELD_NAME = "BoundProductionSourceCount"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 BoundProductionSourceCount = 0;`.
    ...
    if re.search(
        r"int32\s+BoundProductionSourceCount\s*;",
        compact,
    ) and re.search(
        r"int32\s+BoundProductionSourceCount\s*=",
        compact,
        ...
        return False
    if re.search(
        r"BoundProductionSourceCount\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"BoundProductionSourceCount\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"BoundProductionSourceCount\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"BoundProductionSourceCount\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"BoundProductionSourceCount\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BoundProductionSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+BoundProductionSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+BoundProductionSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+BoundProductionSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+BoundProductionSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+BoundProductionSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BoundProductionSourceCount\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+BoundProductionSourceCount\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 BoundProductionSourceCount is missing from "
...
class ProductionAudioAuditBoundProductionSourceCountFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_initializer_drop_or_harbor_change_fails_closed(self) -> None:
        ...
        self.assertIn("int32 BoundProductionSourceCount = 0;", compact_origin)
        self.assertNotIn("BoundProductionSourceCount = 40", compact_origin)
        self.assertNotIn("BoundProductionSourceCount = 80", compact_origin)
        self.assertNotIn("BoundProductionSourceCount = false", compact_origin)
        self.assertNotIn("BoundProductionSourceCount;", compact_origin.replace(
    ...
    def test_bound_count_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 BoundProductionSourceCount"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tint32 BoundProductionSourceCounts = 0;\n",
            "\tint32 BoundProductionSourceCount = 1;\n",
            "\tbool BoundProductionSourceCount;\n",
            "\tfloat BoundProductionSourceCount = " + forty + ";\n",
            "\tfloat BoundProductionSourceCount = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit BoundProductionSourceCount field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "bound_production_source_count_field_decl_contract.py"
        ))
