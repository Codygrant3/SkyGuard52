# THIS IS leftover-safe ASkyguardApacheAircraft ChinHousing.
...
# then BARE `TObjectPtr<UStaticMeshComponent> ChinHousing;`
# (no initializer).
...
# Stop BEFORE sibling ChinBarrel as a claimed slot.
# Do NOT claim ChinTurret / ChinBarrel / SensorTurret /
# SensorBall / NightVisionTurret / TailRotor / MainRotor /
# RotorMast / MainRotorCross / AircraftRoot / Fuselage /
# Nose / Canopy / SilhouetteMesh / StubWingLeft /
# StubWingRight / MaxIntegrity / CurrentIntegrity /
# HullCollider / PilotCanopy / GunnerMount / PilotMount /
# EyeMount / WeaponMount as this slot.
# ChinHousing is NOT leftover analog apache-aim-chin-turret
# UFUNCTION, NOT leftover analog apache-get-chin-slew-scale /
# apache-get-chin-fire-scale / apache-is-chin-turret-down
# UFUNCTION, NOT leftover analog apache-mount-fail-closed,
# NOT leftover sibling ChinTurret (USceneComponent) /
# ChinBarrel. Type is TObjectPtr<UStaticMeshComponent>.
...
# (#411-#426), leftover analog apache-hull-collider,
# leftover analog apache-is-canopy-glass-cracked UFUNCTION,
# leftover analog apache-cpg-feel, leftover analog
# apache-own-ship-systems.
# Do NOT parse leftover analog apache-rotor-mast-field
# (#1565), leftover analog apache-stub-wing-right-field
# (#1564), leftover analog apache-vertical-tail-field
# (#1563), leftover analog apache-stub-wing-left-field,
...
# leftover analog apache-silhouette-mesh-field,
# leftover analog apache-hover-bob-centimeters-field,
# leftover analog apache-rotor-power-field,
# leftover analog apache-max-integrity-field,
# leftover analog apache-current-integrity-field.
# MainRotor is an opening sibling leftover.
# TailRotor and ChinTurret are in-flight siblings,
# not this slot.
# Do NOT parse leftover ProtectAsset MaxIntegrity /
# CurrentIntegrity (#1296 / #1297), leftover LoadoutSpec
# HullIntegrity (#1475), leftover TheaterKit
# WeatherIdentity (#1300), leftover ArcadeLook UPROPERTY
# (#1542/#1544-#1551), leftover GuidedLockRules methods,
# leftover CampaignMissionSpec, leftover Harbor Breaker
# Approach/Contact/Shore, leftover campaign-roster
# function decls / analog #85ab.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> ChinHousing;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent>` as the locked decl
# and NOT `float CurrentIntegrity = 140.f;`.
...
TARGET = "TObjectPtr<UStaticMeshComponent> ChinHousing;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> ChinHousing = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> ChinHousing = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> ChinHousing = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> ChinHousing;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> ChinHousing;"
...
STOP_BEFORE_CHIN_BARREL = "ChinBarrel"
...
SIBLING_NIGHT_VISION_TURRET = "NightVisionTurret"
...
LEFTOVER_APACHE_AIM_CHIN_TURRET = (
    "Scripts/tests/test_apache_aim_chin_turret"
    "_decl_contract.py"
)
...
CHIN_HOUSING_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+ChinHousing\s*;"
)
CHIN_HOUSING_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+ChinHousing\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover ChinTurret / ChinBarrel / RotorMast, or an
    # initializer on ChinHousing as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CHIN_HOUSING_DECL_RE.search(compact) is None:
        return False
    if CHIN_HOUSING_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CHIN_BARREL):
            break
    ...
    if has_identifier(window, SIBLING_CHIN_BARREL):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CHIN_BARREL}"
        )
    for later in (
        SIBLING_CHIN_BARREL,
        SIBLING_SENSOR_TURRET,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+ChinHousing\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> ChinHousing "
...
class ApacheChinHousingFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ChinHousing"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_CHIN_BARREL), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CHIN_BARREL),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_SENSOR_TURRET), section)
        self.assertFalse(has_identifier(section, SIBLING_SENSOR_BALL), section)
        self.assertFalse(has_identifier(section, SIBLING_NIGHT_VISION_TURRET), section)
        ...
        self.assertFalse(has_identifier(section, SIBLING_CHIN_BARREL), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ChinHousing"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ChinHousing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_chin_housing_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> ChinHousing;", compact_origin)
        self.assertNotIn("ChinHousing = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_chin_housing_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> ChinHousing"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn(SIBLING_NIGHT_VISION_TURRET, LOCKED_DECL)
        ...
        self.assertNotIn(TARGET_WRONG_LOCKED_SCENE, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CHIN_BARREL}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> ChinHousing;")
        self.assertNotEqual(LOCKED_DECL, "FName WeatherIdentity;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<USceneComponent> ChinHousing;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> ChinHousings;\n",
            "\tint32 ChinHousing = 140;\n",
            "\tbool ChinHousing = true;\n",
            "\tfloat ChinHousing = " + forty + ";\n",
            "\tfloat ChinHousing = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_chin_barrel_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_CHIN_BARREL))
        ...
        self.assertFalse(has_identifier(section, SIBLING_SENSOR_TURRET))
        ...
        self.assertNotIn(TARGET_WRONG_CHIN_BARREL, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_CHIN_BARREL))
        self.assertFalse(has_identifier(section, SIBLING_CHIN_BARREL))
        self.assertTrue(has_identifier(leaked, SIBLING_MAIN_ROTOR))
        self.assertTrue(has_identifier(section, SIBLING_MAIN_ROTOR))
        self.assertTrue(has_identifier(leaked, SIBLING_TAIL_ROTOR))
        self.assertTrue(has_identifier(section, SIBLING_TAIL_ROTOR))
        self.assertTrue(has_identifier(leaked, SIBLING_CHIN_TURRET))
        self.assertTrue(has_identifier(section, SIBLING_CHIN_TURRET))
        self.assertTrue(has_identifier(leaked, SIBLING_ROTOR_MAST))
        self.assertTrue(has_identifier(section, SIBLING_ROTOR_MAST))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache ChinHousing field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "chin_housing_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_APACHE_AIM_CHIN_TURRET, LOCKED_SCRIPTS)
