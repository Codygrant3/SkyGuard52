# THIS IS leftover-safe ASkyguardApacheAircraft ChinTurret.
...
# then BARE `TObjectPtr<USceneComponent> ChinTurret;` (no initializer).
...
# Stop BEFORE sibling ChinHousing as a claimed slot.
# Do NOT claim ChinHousing / ChinBarrel / SensorTurret /
...
# ChinTurret is NOT leftover analog apache-aim-chin-turret
...
# NOT leftover sibling ChinHousing / ChinBarrel (those are
...
# `TObjectPtr<USceneComponent> ChinTurret;` NOT
...
LOCKED_DECL = "TObjectPtr<USceneComponent> ChinTurret;"
...
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> ChinTurret = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> ChinTurret = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> ChinTurret = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> ChinTurret = 0.f;"
TARGET_WRONG_FLOAT = "float ChinTurret;"
TARGET_WRONG_FNAME = "FName ChinTurret;"
TARGET_WRONG_INT = "int32 ChinTurret = 0;"
TARGET_WRONG_BOOL = "bool ChinTurret = true;"
...
STOP_BEFORE_CHIN_HOUSING = "ChinHousing"
...
TARGET_WRONG_STATIC_MESH = "TObjectPtr<UStaticMeshComponent> ChinTurret;"
...
CHIN_TURRET_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*ChinTurret\s*;"
)
CHIN_TURRET_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*ChinTurret\s*="
)
CHIN_TURRET_FLOAT_RE = re.compile(r"float\s+ChinTurret\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if CHIN_TURRET_INIT_RE.search(compact):
        return False
    if CHIN_TURRET_FLOAT_RE.search(compact):
        return False
    if CHIN_TURRET_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+ChinTurret\b", compact):
        return False
    if re.search(r"int32\s+ChinTurret\b", compact):
        return False
    if re.search(r"bool\s+ChinTurret\b", compact):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*UStaticMeshComponent\s*>\s*ChinTurret\b",
        compact,
    ):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*TObjectPtr<USceneComponent>\s*ChinTurret\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if STOP_BEFORE_CHIN_HOUSING in window:
                raise AssertionError(
                    "claimed window includes sibling ChinHousing"
                )
            if "UStaticMeshComponent" in window:
                raise AssertionError(
                    "claimed window uses leftover UStaticMeshComponent "
                    "instead of USceneComponent ChinTurret"
                )
    ...
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> ChinTurret; is "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*ChinTurret\b",
            compact[index:],
        ):
...
class ApacheChinTurretFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ChinTurret"), section)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("ChinTurret", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_chin_turret_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        ...
        self.assertIn("ChinTurret", LOCKED_DECL)
    ...
    def test_locked_decl_requires_bare_pointer(self) -> None:
        ...
        self.assertNotIn("ChinTurret = nullptr", compact_origin)
        self.assertNotIn("ChinTurret = NAME_None", compact_origin)
        self.assertNotIn("ChinTurret = false", compact_origin)
        self.assertNotIn("ChinTurret = true", compact_origin)
        self.assertNotIn("ChinTurret = 0.f", compact_origin)
        self.assertNotIn("ChinTurret = 160.f", compact_origin)

    def test_chin_turret_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> ChinTurret"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USceneComponent> ChinTurrets;\n",
            ...
            "\tTObjectPtr<UStaticMeshComponent> SilhouetteMesh;\n",
            "\tfloat ChinTurret = " + forty + ";\n",
            "\tfloat ChinTurret = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_fields(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_CHIN_HOUSING, section)
        self.assertNotIn(STOP_BEFORE_CHIN_HOUSING, claimed)
    ...
    def test_claimed_window_stops_before_chin_housing(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("ChinTurret", claimed)
        ...
        self.assertNotIn(SIBLING_CHIN_HOUSING, window)
    ...
    def test_parse_window_starts_at_protected_stops_before_private(self) -> None:
        ...
        self.assertIn("ChinTurret", class_body(header))
        self.assertIn("ChinTurret", protected)
        self.assertIn("ChinTurret", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache ChinTurret field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "chin_turret_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_header_path_is_apache_aircraft_only(self) -> None:
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<UStaticMeshComponent> ChinTurret;",
        )
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STATIC_MESH)
        self.assertNotIn("UStaticMeshComponent", LOCKED_DECL)
        self.assertNotEqual(
