# THIS IS leftover-safe FSkyguardLoadoutSpec RocketMagazineSize.
...
# Do NOT parse leftover `SkyguardApacheCpgFeel` namespace
# as the parse window (the initializer references it; do
# not lock the feel namespace itself).
...
# StartingStation #1466 / FlareCount #1467.
...
# CannonReserve / RocketReserve / GuidedMagazineSize /
...
# CpgFeel constant (CannonMagazineSize / RocketReserve /
# etc.).
...
TARGET_WRONG_BARE = "int32 RocketMagazineSize;"
TARGET_WRONG_ZERO = "int32 RocketMagazineSize = 0;"
TARGET_WRONG_SIX = "int32 RocketMagazineSize = 6;"
TARGET_WRONG_ZERO_F = "int32 RocketMagazineSize = 0.f;"
TARGET_WRONG_FALSE = "int32 RocketMagazineSize = false;"
TARGET_WRONG_HUNDRED = "int32 RocketMagazineSize = 100;"
TARGET_WRONG_CPG_FEEL = (
    "int32 RocketMagazineSize = "
    "SkyguardApacheCpgFeel::CannonMagazineSize;"
)
TARGET_WRONG_ROCKET_RESERVE = (
    "int32 RocketMagazineSize = "
...
TARGET_WRONG_FLOAT = (
    "float RocketMagazineSize = "
    "SkyguardApacheCpgFeel::RocketMagazineSize;"
)
TARGET_WRONG_BOOL = (
    "bool RocketMagazineSize = "
    "SkyguardApacheCpgFeel::RocketMagazineSize;"
)
...
TARGET_FIELD = "RocketMagazineSize"
THIS_SCRIPT = (
...
def leftover_wrong_cpg_feel_initializers() -> tuple[str, ...]:
    return (
        "SkyguardApacheCpgFeel::CannonMagazineSize",
...
def leftover_cpg_feel_initializers() -> tuple[str, ...]:
    return leftover_wrong_cpg_feel_initializers()


def target_cpg_feel_initializer() -> str:
    return "SkyguardApacheCpgFeel::RocketMagazineSize"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `int32 RocketMagazineSize = SkyguardApacheCpgFeel::RocketMagazineSize;`.
    # Do not accept leftover literals `= 0`, `= 6`, `= 100`,
    # `= 0.f`, `= false`, missing initializer, or a
    # different CpgFeel constant (CannonMagazineSize /
    # RocketReserve / GuidedMagazineSize / etc.).
    # Do not accept sibling CannonMagazineSize /
    # FlareCount / HullIntegrity / PlaystyleLine.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+RocketMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::RocketMagazineSize\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"int32\s+RocketMagazineSize\s*=\s*0\b", compact):
        return False
    if re.search(r"int32\s+RocketMagazineSize\s*=\s*6\b", compact):
        return False
    if re.search(r"int32\s+RocketMagazineSize\s*=\s*0\.f", compact):
        return False
    if re.search(r"int32\s+RocketMagazineSize\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+RocketMagazineSize\s*=\s*100\b", compact):
        return False
    if re.search(
        r"int32\s+RocketMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::CannonMagazineSize",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RocketMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::RocketReserve",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RocketMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::GuidedMagazineSize",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RocketMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::CannonReserve",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RocketMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::GuidedReserve",
        compact,
    ):
        return False
    if re.search(r"float\s+RocketMagazineSize\b", compact):
        return False
    if re.search(r"bool\s+RocketMagazineSize\b", compact):
        return False
    if re.search(r"int32\s+RocketMagazineSize\s*;", compact) and not re.search(
        r"int32\s+RocketMagazineSize\s*=",
        compact,
    ):
        return False
    return True
...
class LoadoutSpecRocketMagazineSizeFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, TARGET_FIELD), section)
        ...
        self.assertIn(target_cpg_feel_initializer(), section)
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, TARGET_FIELD), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn(TARGET_FIELD, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_rocket_magazine_size_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 RocketMagazineSize"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn(target_cpg_feel_initializer(), LOCKED_DECL)
    ...
    def test_hud_snapshot_zero_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("int32 RocketMagazineSize = 0;", compact_origin)
        self.assertNotIn("int32 RocketMagazineSize = 6;", compact_origin)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            f"\tint32 {SIBLING_CANNON_MAG} = 6;\n",
            f"\tint32 {SIBLING_CANNON_RESERVE} = 6;\n",
            f"\tint32 {SIBLING_FLARE_COUNT} = 6;\n",
            f"\tint32 {SIBLING_ROCKET_RESERVE} = 6;\n",
            f"\tint32 {SIBLING_GUIDED_MAG} = 6;\n",
            f"\tint32 {SIBLING_GUIDED_RESERVE} = 6;\n",
            "\tint32 RocketMagazineSizes = "
            "SkyguardApacheCpgFeel::RocketMagazineSize;\n",
            "\tint32 RocketMagazineSize = " + forty + ";\n",
            "\tint32 RocketMagazineSize = " + eighty + ";\n",
        )
    ...
    def test_rocket_magazine_size_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RocketMagazineSize"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("SkyguardApacheCpgFeel::RocketMagazineSize", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        for init in leftover_wrong_cpg_feel_initializers():
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec RocketMagazineSize field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "rocket_magazine_size_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_FLARE_COUNT_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOADOUT_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_STARTING_STATION_FIELD, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertIn(target_cpg_feel_initializer(), locked_only)
