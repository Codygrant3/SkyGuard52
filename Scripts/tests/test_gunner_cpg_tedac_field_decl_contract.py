# THIS IS leftover-safe ASkyguardGunner CpgTedac.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgTedac;`
# (no initializer).
...
# Stop BEFORE sibling CpgMpdLeft as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeLeft, CpgKneeRight, CpgCanopyBow,
# CpgDash, CpgMpdLeft, CpgMpdRight, CpgEufd,
# CpgTedacBezel, CpgTedacText, Boom, GunnerCamera.
# Do NOT claim leftover CpgDash / CpgCanopyBow /
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgCockpit /
# CpgStation (in-flight siblings). CpgSeatPan and
# CpgCanopyBow are sibling in-flight slots.
# Do NOT claim CpgTedacBezel or CpgTedacText as this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgTedac;`
...
# CpgTedac fail-closed if LOCKED_DECL is leftover
# CpgDash / CpgKneeRight / CpgKneeLeft / CpgSeatBack /
# CpgCockpit / CpgStation / CpgSeatPan / CpgCanopyBow /
# CpgMpdLeft / leftover Apache Canopy / leftover
# Apache PilotCanopy.
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgTedac;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgTedac = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgTedac = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgTedac = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgTedac;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgTedac;"
...
CPG_TEDAC_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgTedac\s*;"
)
CPG_TEDAC_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgTedac\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if CPG_TEDAC_DECL_RE.search(compact) is None:
        return False
    if CPG_TEDAC_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_MPD_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_MPD_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_MPD_LEFT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgTedac\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgTedac "
...
class GunnerCpgTedacFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgTedac"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_MPD_LEFT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_MPD_LEFT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgTedac"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgTedac", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_tedac_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgTedac;", compact_origin)
        self.assertNotIn("CpgTedac = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_tedac_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgTedac"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgTedac;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgTedacs;\n",
            "\tint32 CpgTedac = 140;\n",
            "\tbool CpgTedac = true;\n",
            "\tfloat CpgTedac = " + forty + ";\n",
            "\tfloat CpgTedac = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_mpd_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(TARGET_WRONG_MPD_LEFT, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_MPD_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_MPD_LEFT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgTedac field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_tedac_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
