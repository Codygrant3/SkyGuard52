...
# or lock the IsProgressSaved body in the .cpp.
# origin/main is one line
# (`bool IsProgressSaved() const;`);
# accept that exact prototype, other split-line wraps,
...
# leftover GetDebrief (leftover mission-debrief
# defaults). This is not leftover DebriefWidget
# GetPresentation. This is not leftover
# BriefingWidget. Leftover briefing-fail-closed,
# leftover briefing-card defaults, leftover
...
IS_PROGRESS_SAVED = ("bool IsProgressSaved() const;")
UFUNCTION_DEBRIEF = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Presentation|Debrief")'
)
# Leftover #56–#64 plus this class's production files.
# This lane only adds an isolated Python IsProgressSaved
# declaration contract. Stay off Configure, GetPresentation
# (sibling this wave), GetDebrief, GetDebriefNarrative
# (sibling this wave), GetFinalScore (sibling this wave),
# GetPresentationState, AcknowledgeDebrief, RetrySave,
# TravelNext, and HandleDebriefKey on this class. Stay
# off leftover BriefingWidget Configure / GetPresentation
...
# fail-closed #7600, leftover CPG debrief fail-closed
...
# USkyguardBriefingWidget, leftover DebriefWidget
# Configure / GetPresentation, leftover
# sortie-presentation state enum, and dirty
# workspace paths.
LOCKED = {
...
# GetMissionTitle / GetBriefingText /
# AcknowledgeBriefing / LaunchSortie, leftover
# DebriefWidget Configure, leftover
# GetPresentation, leftover GetDebrief, leftover
# GetDebriefNarrative, leftover GetFinalScore,
# leftover GetPresentationState, leftover
# AcknowledgeDebrief, leftover RetrySave, leftover
# TravelNext, leftover HandleDebriefKey, leftover
# sortie-presentation state enum, and leftover
# BriefingWidget stay sibling-only.
LOCKED_SCRIPTS = (
...
# GetDebriefNarrative / GetFinalScore / GetPresentationState /
...
# unlocked. Do not re-lock leftover fail-closed.
...
# .cpp IsProgressSaved body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardDebriefWidget::IsProgressSaved",
    "SkyguardSortiePresentationWidgets.cpp",
)
SIBLING_TYPES = (
...
class DebriefWidgetIsProgressSavedDeclContractTests(unittest.TestCase):
    def test_debrief_widget_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, IS_PROGRESS_SAVED),
            section,
        )
    ...
    def test_briefing_widget_does_not_satisfy_class(self) -> None:
        briefing_only = (
            ...
            "{\n"
            "public:\n"
            "\tbool IsProgressSaved() const { return false; }\n"
            "\tFText GetMissionTitle() const;\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_PROGRESS_SAVED)
        self.assertIn("IsProgressSaved", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, IS_PROGRESS_SAVED))

    def test_missing_is_progress_saved_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tvoid Configure("
            "USkyguardSortiePresentationComponent* InPresentation);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_PROGRESS_SAVED)
        self.assertIn("IsProgressSaved", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_PROGRESS_SAVED)
        self.assertIn("IsProgressSaved", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", IS_PROGRESS_SAVED)
        self.assertNotIn("UFUNCTION", IS_PROGRESS_SAVED)
        self.assertNotIn("Category", IS_PROGRESS_SAVED)
        self.assertNotIn("BlueprintCallable", IS_PROGRESS_SAVED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_PROGRESS_SAVED)
        self.assertIn("IsProgressSaved", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tbool IsProgressSaved();\n"
        wrong_return_void = "\tvoid IsProgressSaved() const;\n"
        wrong_return_int = "\tint32 IsProgressSaved() const;\n"
        added_arg = "\tbool IsProgressSaved(bool bReady) const;\n"
        wrong_name = (
            "\tESkyguardSortiePresentationState "
            "GetPresentationState() const;\n"
        )
        progress_ready = "\tbool IsProgressReady() const;\n"
        get_progress = "\tbool GetProgressSaved() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_int,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_PROGRESS_SAVED)
            self.assertIn("IsProgressSaved", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_progress_saved_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, IS_PROGRESS_SAVED),
            IS_PROGRESS_SAVED,
        )
        self.assertTrue(has_declaration(section, IS_PROGRESS_SAVED))
        self.assertEqual(
            declaration_count(section, IS_PROGRESS_SAVED),
            1,
        )
        self.assertTrue(
            IS_PROGRESS_SAVED.startswith("bool "),
            IS_PROGRESS_SAVED,
        )
        self.assertTrue(
            IS_PROGRESS_SAVED.endswith(";"),
            IS_PROGRESS_SAVED,
        )
        self.assertIn("IsProgressSaved()", IS_PROGRESS_SAVED)
        self.assertIn(" const", IS_PROGRESS_SAVED)
        self.assertNotIn("INDEX_NONE", IS_PROGRESS_SAVED)
        self.assertNotIn("{", IS_PROGRESS_SAVED)
        self.assertNotIn("}", IS_PROGRESS_SAVED)
        self.assertNotIn("return ", IS_PROGRESS_SAVED)
        self.assertNotIn("return false", IS_PROGRESS_SAVED)
        self.assertNotIn("return true", IS_PROGRESS_SAVED)
    ...
    def test_declaration_accepts_origin_main_inline_and_prototype(
        ...
        self.assertTrue(
            has_declaration(one_line_prototype, IS_PROGRESS_SAVED)
        )
        self.assertEqual(
            require_declaration(one_line_prototype, IS_PROGRESS_SAVED),
            IS_PROGRESS_SAVED,
        )
        exact_inline = "bool IsProgressSaved() const { return false; }"
        self.assertTrue(has_declaration(exact_inline, IS_PROGRESS_SAVED))
        self.assertEqual(
            require_declaration(exact_inline, IS_PROGRESS_SAVED),
            IS_PROGRESS_SAVED,
        )
        self.assertEqual(
            declaration_count(exact_inline, IS_PROGRESS_SAVED),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
        ...
        self.assertNotIn("{", IS_PROGRESS_SAVED)
        self.assertNotIn("return false", IS_PROGRESS_SAVED)
        self.assertIn(IS_PROGRESS_SAVED, section)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tIsProgressSaved() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        one_line = f"{{\npublic:\n\t{IS_PROGRESS_SAVED}\n}}\n"
        self.assertTrue(has_declaration(one_line, IS_PROGRESS_SAVED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, IS_PROGRESS_SAVED),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return false", IS_PROGRESS_SAVED)
        self.assertNotIn("return true", IS_PROGRESS_SAVED)
        self.assertNotIn("return Other", IS_PROGRESS_SAVED)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{IS_PROGRESS_SAVED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", IS_PROGRESS_SAVED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", IS_PROGRESS_SAVED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_progress_saved_body(self) -> None:
        locked_only = f"{IS_PROGRESS_SAVED}\n"
        self.assertNotIn("{", IS_PROGRESS_SAVED)
        self.assertNotIn("}", IS_PROGRESS_SAVED)
        self.assertNotIn("return ", IS_PROGRESS_SAVED)
        self.assertNotIn("return false", IS_PROGRESS_SAVED)
        self.assertNotIn("return true", IS_PROGRESS_SAVED)
        self.assertNotIn("return false", locked_only)
        self.assertNotIn(
            "USkyguardDebriefWidget::IsProgressSaved",
            IS_PROGRESS_SAVED,
        )
    ...
    def test_contract_does_not_relock_constructor(self) -> None:
        ...
        for neighbor in CONSTRUCTOR_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("ObjectInitializer", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_configure(self) -> None:
        ...
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("Configure(", IS_PROGRESS_SAVED)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("GetPresentation", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_get_debrief(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("GetDebrief()", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_get_debrief_narrative(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NARRATIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("GetDebriefNarrative", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_get_final_score(self) -> None:
        ...
        for neighbor in GET_FINAL_SCORE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("GetFinalScore", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_get_presentation_state(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("GetPresentationState", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("AcknowledgeDebrief", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_retry_save(self) -> None:
        ...
        for neighbor in RETRY_SAVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("RetrySave", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_travel_next(self) -> None:
        ...
        for neighbor in TRAVEL_NEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("TravelNext", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_handle_debrief_key(self) -> None:
        ...
        for neighbor in HANDLE_DEBRIEF_KEY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("HandleDebriefKey", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_PROGRESS_SAVED)
        self.assertNotIn("Unconfigured", IS_PROGRESS_SAVED)
        self.assertNotIn("Warming", IS_PROGRESS_SAVED)
        self.assertNotIn("Launched", IS_PROGRESS_SAVED)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            IS_PROGRESS_SAVED,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_PROGRESS_SAVED)
        self.assertNotIn("FSkyguardBriefingCard", IS_PROGRESS_SAVED)
        self.assertNotIn("FSkyguardBriefingRadioRow", IS_PROGRESS_SAVED)
        self.assertNotIn("FSkyguardHowToFlyRow", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_PROGRESS_SAVED)
        self.assertNotIn("FillResultCombatStats", IS_PROGRESS_SAVED)
        self.assertNotIn("ASkyguardGunner", IS_PROGRESS_SAVED)
        self.assertNotIn("FillAndFinalize", IS_PROGRESS_SAVED)
        self.assertNotIn("FillAndFail", IS_PROGRESS_SAVED)
        self.assertNotIn("ApplyHydraForClusters", IS_PROGRESS_SAVED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_PROGRESS_SAVED}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_PROGRESS_SAVED),
            IS_PROGRESS_SAVED,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "debrief widget IsProgressSaved contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, IS_PROGRESS_SAVED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"debrief widget IsProgressSaved contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_PROGRESS_SAVED)

    def test_contract_is_is_progress_saved_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(IS_PROGRESS_SAVED.startswith("bool "))
        self.assertTrue(IS_PROGRESS_SAVED.endswith(";"))
        self.assertIn(" const", IS_PROGRESS_SAVED)
        self.assertIn(UFUNCTION_DEBRIEF, section)
