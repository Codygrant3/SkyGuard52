# THIS IS leftover-safe USkyguardRadioChatterComponent
# GetDroppedLineCount.
# origin/main form: one-line BlueprintPure wrap
# UFUNCTION(BlueprintPure, Category="Skyguard|Radio")
# wrapping `int32 GetDroppedLineCount() const { return DroppedLines; }`.
# LOCKED_DECL may be declaration-only
# `int32 GetDroppedLineCount() const;` if the matcher strips
# inline bodies, OR the full origin/main inline form.
# Accept declaration-only AND the origin/main inline body.
# Do NOT fail-closed on `{ return DroppedLines; }`.
# Do not require a separate .cpp definition.
# KEEP BlueprintPure. KEEP specifiers matching origin/main.
# KEEP const. Return type IS int32. No args.
# Do NOT retarget to PrimeLines / EnqueueLine /
# AdvanceRadioState / ClearQueue / GetQueuedLineCount /
# GetCurrentLineId / GetPlayedLineCount.
# Word-boundary match GetDroppedLineCount vs
# GetQueuedLineCount vs GetPlayedLineCount vs
# GetCurrentLineId vs ClearQueue vs EnqueueLine vs
# AdvanceRadioState vs PrimeLines.
# THIS IS leftover-safe unique analog Category
# Skyguard|Radio (unique unused Radio path).
# leftover analog radio-chatter-empty-fail-closed bulk
# #177/#52ae, leftover analog
# radio-chatter-empty-queue-fail-closed #150/#5137,
# leftover analog radio-chatter-empty-line-tests
# #121/#fb2b are behavioral; wrap Category Radio
# UFUNCTION declaration only.
# leftover unique FSkyguardRadioLine FIELD #1332-#1338
# wrap NO Category and do not parse the component —
# do NOT claim LineId/Speaker/Subtitle/Sound/Priority/
# EstimatedDurationSeconds/CooldownSeconds.
# Identifier GetDroppedLineCount is NOT leftover analog
# GetQueuedLineCount (in-flight), NOT leftover analog
# GetPlayedLineCount, NOT leftover analog GetCurrentLineId
# (in-flight), NOT leftover analog ClearQueue #1716,
# NOT leftover analog AdvanceRadioState #1715,
# NOT leftover analog PrimeLines #1713, NOT leftover analog
# EnqueueLine #1712, NOT leftover analog RadioLine fields
# #1332-#1338, NOT leftover analog radio-chatter-empty-fail-closed
# bulk #177/#52ae (behavioral). leftover analog
# radio-chatter-empty-queue-fail-closed #150/#5137 is behavioral.
# BlueprintPure IS present. BlueprintCallable IS absent.
# const method IS present. Return type IS int32. No args.
...
# args are present, if the return type is not int32,
# or if Category is
...
# Parse CLASS `USkyguardRadioChatterComponent` public
# section ONLY after `class USkyguardRadioChatterComponent`
# / GENERATED_BODY. Start at that class `public:`.
# Do NOT parse the USTRUCT `FSkyguardRadioLine` body as
# this slot. Stop BEFORE `private:` if present.
# Stop BEFORE first public UPROPERTY (`MaxQueuedLines`).
# Isolate THIS UFUNCTION window so sibling
# PrimeLines / EnqueueLine / AdvanceRadioState /
# ClearQueue / GetQueuedLineCount / GetCurrentLineId /
# GetPlayedLineCount are uncontracted.
# Do NOT claim those siblings. Do NOT claim OnLineStarted.
# Clone leftover unique BlueprintPure GetSortieHits #1685
# RETARGET method/class/Category. KEEP BlueprintPure KEEP
# const int32. FAIL-CLOSED if LOCKED_DECL is
# int32 GetSortieHits() const;.
# Clone leftover unique Validation BlueprintPure #1695
# RETARGET method. KEEP BlueprintPure.
# Clone leftover unique EnqueueLine #1712 RETARGET
# (that one is BlueprintCallable — keep BlueprintPure here).
# FAIL-CLOSED if LOCKED_DECL is
# bool EnqueueLine(const FSkyguardRadioLine& Line);.
# Clone leftover unique BindPresentation #1707 RETARGET.
# KEEP BlueprintPure not BlueprintCallable.
# Clone leftover unique ClearQueue RETARGET
# (BlueprintCallable no-args — keep BlueprintPure here).
# FAIL-CLOSED if LOCKED_DECL is void ClearQueue();.
# Clone leftover unique TogglePause #1711 RETARGET
# Category Pause -> Radio. KEEP BlueprintPure not
# BlueprintCallable. FAIL-CLOSED if LOCKED_DECL is
# void TogglePause();.
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# Category Skyguard|Audio|Production — RETARGET Category
# Production -> Skyguard|Radio.
# KEEP BlueprintPure (NOT BlueprintCallable).
# FAIL-CLOSED if LOCKED_DECL is void EnsureDefaultEntries();.
# Clone leftover unique Validation BlueprintCallable #1694
# RETARGET; this slot is BlueprintPure not BlueprintCallable.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is int32 GetDroppedLineCount() const;
...
# and NOT void PrimeLines(const TArray<FSkyguardRadioLine>& Lines);
# and NOT void AdvanceRadioState(float DeltaSeconds); and NOT
# void ClearQueue(); and NOT leftover analog
# int32 GetQueuedLineCount() const; and NOT leftover analog
# int32 GetPlayedLineCount() const; and NOT leftover analog
# FName GetCurrentLineId() const; and NOT leftover analog
# bool EnqueueLine(const FSkyguardRadioLine& Line);
...
# void BindPresentation( / void PrimeLines( /
...
HEADER_PATH = "Source/Skyguard52/SkyguardRadioChatterComponent.h"
CLASS_NAME = "USkyguardRadioChatterComponent"
...
TARGET = "int32 GetDroppedLineCount() const;"
TARGET_SPLIT = (
    "int32\n"
    "\tGetDroppedLineCount() const;"
)
TARGET_INLINE = (
    "int32 GetDroppedLineCount() const { return DroppedLines; }"
)
TARGET_WRONG_BARE = "int32 GetDroppedLineCount();"
TARGET_WRONG_NO_CONST = "int32 GetDroppedLineCount();"
...
TARGET_WRONG_GET_BOUND = "bool EnqueueLine(const FSkyguardRadioLine& Line);"
...
TARGET_WRONG_CLEAR = "void ClearQueue();"
...
TARGET_WRONG_FLOAT = "float GetDroppedLineCount() const;"
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "int32 GetDroppedLineCounts() const;"
...
TARGET_WRONG_TRUE = "int32 GetDroppedLineCount = true;"
TARGET_WRONG_ZERO = "int32 GetDroppedLineCount = 0.f;"
...
STOP_BEFORE_EVENT_DEFINITIONS = "MaxQueuedLines"
SIBLING_INITIALIZE_REQUIRED = "PrimeLines"
SIBLING_EVALUATE_READINESS = "AdvanceRadioState"
SIBLING_CONFIGURE_ROUTING = "ClearQueue"
SIBLING_UNBOUND_CATEGORIES = "GetQueuedLineCount"
SIBLING_FIND_ENTRY = "GetCurrentLineId"
SIBLING_GET_REQUIRED = "EnqueueLine"
SIBLING_GET_DISPLAY = "GetPlayedLineCount"
SIBLING_ENTRIES = "PrimeLines"
SIBLING_ROUTING = "AdvanceRadioState"
...
STOP_BEFORE_SKYLINE_STYLE = "OnLineStarted"
...
THIS_SCRIPT = (
    "Scripts/tests/test_radio_chatter_get_dropped_line_count"
    "_decl_contract.py"
)
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        "_decl_contract.py",
        f"{prefix}test_radio_chatter_advance_radio_state"
        "_decl_contract.py",
    ) + leftover_apache_ufunction_scripts() + leftover_apache_mesh_field_scripts()


LOCKED_SCRIPTS = (
    "Scripts/tests/test_radio_chatter_enqueue_line"
    "_decl_contract.py",
    "Scripts/tests/test_radio_chatter_clear_queue"
    "_decl_contract.py",
    "Scripts/tests/test_radio_chatter_prime_lines"
    "_decl_contract.py",
    "Scripts/tests/test_radio_chatter_advance_radio_state"
    "_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bint32\s+GetDroppedLineCount\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+DroppedLines\s*;\s*\})"
)
ENQUEUE_LINE_IDENT_RE = re.compile(r"\bGetDroppedLineCount\b")
GET_DROPPED_LINE_COUNT_RE = METHOD_SIGNATURE_RE
GET_DROPPED_LINE_COUNT_WRONG_TYPE_RE = re.compile(
    r"(?:float|FName|bool|FLinearColor|void)\s+"
    r"GetDroppedLineCount\b"
)
GET_DROPPED_LINE_COUNT_NO_CONST_RE = re.compile(
    r"int32\s+GetDroppedLineCount\s*\(\s*\)\s*;"
)
GET_QUEUED_LINE_COUNT_RE = re.compile(r"\bGetQueuedLineCount\b")
GET_PLAYED_LINE_COUNT_RE = re.compile(r"\bGetPlayedLineCount\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # int32 GetDroppedLineCount() const;
    # OR origin/main inline `{ return DroppedLines; }`.
    # Accept origin/main split-line trailing-semicolon form.
    # Do not fail-closed on that inline body.
    # Do not accept missing const, a void/float/bool
    # rename, leftover unique EnsureDefaultEntries /
    # TogglePause / BindPresentation / GetSortieHits,
    # leftover sibling PrimeLines / EnqueueLine /
    # AdvanceRadioState / ClearQueue / GetQueuedLineCount /
    # GetPlayedLineCount / GetCurrentLineId,
    # leftover analog GetSortieHits /
    # bSuccess = false, leftover FName WeatherIdentity,
    # leftover TObjectPtr ProductionBank, leftover
    # FireRate / RecoilPitch, leftover Boom, leftover
    # VegetationStartCullDistanceCm, leftover analog
    # PoolCapacity = 12, or Harbor 40 / 80.
    del declaration
    compact = collapsed(region)
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if GET_DROPPED_LINE_COUNT_WRONG_TYPE_RE.search(compact):
        return False
    if GET_DROPPED_LINE_COUNT_NO_CONST_RE.search(compact) and (
        METHOD_SIGNATURE_RE.search(compact) is None
    ):
        return False
    if re.search(r"\bvoid\s+GetDroppedLineCount\b", compact):
        return False
    if re.search(r"\bfloat\s+GetDroppedLineCount\b", compact):
        return False
    if re.search(r"\bbool\s+GetDroppedLineCount\b", compact):
        return False
    if re.search(r"\bbool\s+EnqueueLine\s*\(", compact):
        return False
    if re.search(r"\bvoid\s+ClearQueue\s*\(", compact):
        return False
    if re.search(r"\bint32\s+GetQueuedLineCount\s*\(", compact):
        return False
    if re.search(r"\bint32\s+GetPlayedLineCount\s*\(", compact):
        return False
    ...
    if re.search(r"\bvoid\s+BindPresentation\s*\(", compact):
        return False
    if re.search(r"\bvoid\s+PrimeLines\s*\(", compact):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetDroppedLineCount\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for GetDroppedLineCount "
...
class RadioChatterGetDroppedLineCountDeclContractTests(
    unittest.TestCase
):
    def test_radio_chatter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "GetDroppedLineCount"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, "AdvanceRadioState"), body)
        self.assertFalse(has_identifier(section, "AdvanceRadioState"), section)
    ...
    def test_missing_dropped_line_count_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Radio"', section)
        ...
        self.assertIn('Category="Skyguard|Radio"', specifiers)
        ...
        self.assertTrue("|Radio" in specifiers)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertTrue(
            ENQUEUE_LINE_IDENT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
            GET_QUEUED_LINE_COUNT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
            GET_PLAYED_LINE_COUNT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
        ...
        self.assertFalse(
            has_identifier(
                "int32 GetQueuedLineCount() const;",
                "GetDroppedLineCount",
            )
        )
        self.assertFalse(
            has_identifier(
                "int32 GetPlayedLineCount() const;",
                "GetDroppedLineCount",
            )
        )
        self.assertFalse(
            has_identifier(
                "FName GetCurrentLineId() const;",
                "GetDroppedLineCount",
            )
        )
        self.assertFalse(
    ...
    def test_dropped_line_count_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "Line"))
        self.assertNotIn(
            "FSkyguardRadioLine",
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GetDroppedLineCountX() const;\n",
            "\tint32 GetDroppedLineCount = 1;\n",
            "\tFName GetDroppedLineCount = NAME_None;\n",
            "\tint32 GetDroppedLineCount() = "
            + forty
            + ";\n",
            "\tint32 GetDroppedLineCount() = "
    ...
    def test_missing_category_or_blueprint_pure_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Radio"', origin)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "EnqueueLine")
        )
        self.assertTrue(
            has_identifier(section, "GetDroppedLineCount")
        )
        ...
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, "AdvanceRadioState"))
        self.assertFalse(has_identifier(section, "AdvanceRadioState"))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertIn("SkyguardRadioChatterComponent.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio chatter GetDroppedLineCount "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_chatter_get_dropped_line_count"
            "_decl_contract.py"
        ))
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "Line"))
        self.assertIn("GetDroppedLineCount", LOCKED_DECL)
        for leftover in leftover_apache_ufunction_tokens():
            self.assertNotIn(leftover, locked_only)

    def test_word_boundary_dropped_line_count_vs_queued_played_and_enqueue(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier(
        ...
        self.assertFalse(
            has_identifier(LOCKED_DECL, "PrimeLines")
        )
        self.assertFalse(
            has_identifier(LOCKED_DECL, "AdvanceRadioState")
        )
        self.assertFalse(has_identifier(LOCKED_DECL, "ClearQueue"))
        self.assertFalse(has_identifier(LOCKED_DECL, "EnqueueLine"))
        self.assertFalse(has_identifier(LOCKED_DECL, "GetQueuedLineCount"))
        self.assertFalse(has_identifier(LOCKED_DECL, "GetPlayedLineCount"))
        self.assertFalse(has_identifier(LOCKED_DECL, "GetCurrentLineId"))
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_REFRESH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_REBIND)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GET_BOUND)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GET_BOUND_INLINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ACTIVATION)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PLAYED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLEAR)
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "GetDroppedLineCount"), section)
        self.assertFalse(has_identifier(section, "PrimeLines"), section)
        self.assertFalse(
            has_identifier(section, "AdvanceRadioState"),
            section,
        )
        self.assertFalse(has_identifier(section, "EnqueueLine"), section)
        self.assertFalse(has_identifier(section, "GetQueuedLineCount"), section)
        self.assertFalse(has_identifier(section, "GetPlayedLineCount"), section)
        self.assertFalse(has_identifier(section, "GetCurrentLineId"), section)
        self.assertFalse(has_identifier(section, "ClearQueue"), section)
        self.assertEqual(declaration_count(section, LOCKED_DECL), 1)

    def test_does_not_parse_radio_line_struct_as_this_slot(self) -> None:
        header = origin_main_header()
        struct_token = "struct FSkyguardRadioLine"
        class_token = f"class SKYGUARD52_API {CLASS_NAME}"
        self.assertIn(struct_token, header)
        self.assertIn(class_token, header)
        self.assertLess(header.index(struct_token), header.index(class_token))
        body = class_body(header)
        section = spec_section(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertNotIn(struct_token, body)
        self.assertTrue(has_identifier(header, "LineId"))
        self.assertTrue(has_identifier(header, "Speaker"))
        self.assertTrue(has_identifier(header, "Subtitle"))
        self.assertTrue(has_identifier(header, "EstimatedDurationSeconds"))
        self.assertTrue(has_identifier(header, "CooldownSeconds"))
        for field in (
            "Speaker",
            "Subtitle",
            ...
            self.assertFalse(has_identifier(section, field), field)
            self.assertFalse(has_identifier(body, field), field)
        self.assertFalse(has_identifier(section, "LineId"), section)
        self.assertFalse(has_identifier(section, "Priority"), section)
        self.assertNotEqual(CLASS_NAME, LEFTOVER_RADIO_LINE_STRUCT)
        self.assertNotIn("LineId", LOCKED_DECL)
        self.assertNotIn("Speaker", LOCKED_DECL)
        self.assertNotIn("Subtitle", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_clone_ensure_toggle_hits_or_bind(
        ...
        self.assertNotEqual(LOCKED_DECL, "int32 GetQueuedLineCount() const;")
        self.assertNotEqual(LOCKED_DECL, "int32 GetPlayedLineCount() const;")
        self.assertNotEqual(LOCKED_DECL, "FName GetCurrentLineId() const;")
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 GetDroppedLineCount("))
        self.assertNotIn("const FSkyguardRadioLine& Line", LOCKED_DECL)
        self.assertIsNotNone(re.search(r"\)\s*const\b", LOCKED_DECL))
        self.assertIn(
            'Category="Skyguard|Radio"',
            LOCKED_UFUNCTION_WRAP,
        ...
        self.assertNotIn('Category="Skyguard|Radio"', pause_specs)
        leftover_hud = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Radio"', hud_specs)
