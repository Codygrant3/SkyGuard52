...
# INDEX_NONE or lock SortiePresentation construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01IntegrationDirector, not leftover
# USkyguardSortiePresentationComponent method /
# fail-closed contracts (already drafted) and not
# leftover briefing / debrief widget getters. Keep
# SortiePresentation distinct from sibling director
# fields Briefing / AudioDirector / RadioChatter /
# Root / CampaignDefinition / MissionDefinition /
# Readiness. Do not lock leftover spawn-location
...
# leftover debrief-widget getters. Do not open
# SkyguardSortiePresentationComponent.h or
# SkyguardSortiePresentationWidgets.h. Stay off
# leftover sortie-presentation fail-closed / enum
# contracts and leftover audio-director listener /
# telemetry / suppression / engine-state / bank-null /
# world-event fail-closed contracts. origin/main is a
# one-line field (`TObjectPtr<
# USkyguardSortiePresentationComponent>
# SortiePresentation;`); accept that form and other
...
# isolated-test drafts #107–#573, leftover Apache
...
# leftover theater-kit #59, leftover #107–#573, plus
# SkyguardMission01IntegrationDirector production
# files. This lane only adds an isolated Python
# SortiePresentation field declaration contract on
...
# contracts, leftover MissionBriefingComponent
# method decl contracts, leftover briefing-card /
# briefing-radio-row defaults, leftover briefing
# fail-closed tests, leftover sortie-presentation
# fail-closed / enum contracts, leftover
# audio-director listener / telemetry / suppression
# / engine-state / bank-null / world-event
# fail-closed contracts, leftover Harbor #6/#8/#9,
# leftover theater-kit #59, leftover
# environment-readiness defaults #6b9d / #b931,
# sibling Briefing / AudioDirector / RadioChatter /
# Root field contracts stay sibling-only.
LOCKED_SCRIPTS = (
...
LEFTOVER_DEBRIEF_WIDGET_NOT_LOCKED = (
    "test_debrief_widget_configure_decl_contract.py",
    "test_debrief_widget_get_presentation_decl_contract.py",
    "test_debrief_widget_handle_debrief_key_decl_contract.py",
    "test_debrief_widget_travel_next_decl_contract.py",
    "test_debrief_widget_get_presentation_state_decl_contract.py",
    "test_debrief_widget_acknowledge_debrief_decl_contract.py",
    "test_debrief_widget_retry_save_decl_contract.py",
    "test_debrief_widget_is_progress_saved_decl_contract.py",
    "test_debrief_widget_get_debrief_narrative_decl_contract.py",
    "test_debrief_widget_get_final_score_decl_contract.py",
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01IntegrationDirector::SortiePresentation",
    "SkyguardMission01IntegrationDirector.cpp",
...
class Mission01SortiePresentationFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SORTIE_PRESENTATION_FIELD), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SORTIE_PRESENTATION_FIELD)
        self.assertIn("SortiePresentation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SORTIE_PRESENTATION_FIELD))
    ...
    def test_missing_sortie_presentation_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SORTIE_PRESENTATION_FIELD)
        self.assertIn("SortiePresentation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SORTIE_PRESENTATION_FIELD)
        self.assertIn("SortiePresentation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SORTIE_PRESENTATION_FIELD), section)
        self.assertNotIn("UPROPERTY", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("VisibleAnywhere", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("BlueprintReadOnly", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("Category", SORTIE_PRESENTATION_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, SORTIE_PRESENTATION_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SORTIE_PRESENTATION_FIELD)
        self.assertIn("SortiePresentation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        scene = "\tTObjectPtr<USceneComponent> SortiePresentation;\n"
        briefing = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SORTIE_PRESENTATION_FIELD)
            self.assertIn("SortiePresentation", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_sortie_presentation_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SORTIE_PRESENTATION_FIELD))
        self.assertEqual(declaration_count(section, SORTIE_PRESENTATION_FIELD), 1)
        self.assertTrue(
            SORTIE_PRESENTATION_FIELD.startswith(
                "TObjectPtr<USkyguardSortiePresentationComponent> "
            ),
            SORTIE_PRESENTATION_FIELD,
        )
        self.assertTrue(SORTIE_PRESENTATION_FIELD.endswith(";"), SORTIE_PRESENTATION_FIELD)
        self.assertIn("SortiePresentation", SORTIE_PRESENTATION_FIELD)
        self.assertIn(
            "USkyguardSortiePresentationComponent",
            SORTIE_PRESENTATION_FIELD,
        )
        self.assertIn("TObjectPtr<", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("TArray<", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("TSoftObjectPtr", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("=", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("INDEX_NONE", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("UFUNCTION", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("{", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("}", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("return ", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("Briefing", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("RadioChatter", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("AudioDirector", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("CampaignDefinition", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("MissionDefinition", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("Readiness", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetAircraft", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("ConfigureFromMission", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("AdvanceBriefing", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("SetAssetsReady", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("CanLaunch", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetElapsedSeconds", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetBriefingState", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetBriefingText", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetRadioChatter", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetPresentation", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("GetMissionTitle", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("AcknowledgeBriefing", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("LaunchSortie", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("HullCollider", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("OpticalTracker", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("WeaponServo", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("CountermeasurePod", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("RadarNode", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("HarborIndustrial", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("MaxIntegrity", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("CurrentIntegrity", SORTIE_PRESENTATION_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, SORTIE_PRESENTATION_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01} {SORTIE_PRESENTATION_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, SORTIE_PRESENTATION_FIELD), 1)
        one_line = f"{{\npublic:\n\t{SORTIE_PRESENTATION_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, SORTIE_PRESENTATION_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SORTIE_PRESENTATION_FIELD), section)
        self.assertEqual(
            require_declaration(section, SORTIE_PRESENTATION_FIELD),
    ...
    def test_assigned_sortie_presentation_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardSortiePresentationComponent> "
            "SortiePresentation = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, SORTIE_PRESENTATION_FIELD)
        self.assertIn("SortiePresentation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, SORTIE_PRESENTATION_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("SortiePresentation", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, SORTIE_PRESENTATION_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{SORTIE_PRESENTATION_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, SORTIE_PRESENTATION_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            SORTIE_PRESENTATION_FIELD.startswith("UFUNCTION"),
            SORTIE_PRESENTATION_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SORTIE_PRESENTATION_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", SORTIE_PRESENTATION_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_sortie_presentation_cpp_body(self) -> None:
        locked_only = f"{SORTIE_PRESENTATION_FIELD}\n"
        self.assertNotIn("{", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("}", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("return ", SORTIE_PRESENTATION_FIELD)
        ...
        self.assertNotIn("return false", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("return true", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("CreateDefaultSubobject", SORTIE_PRESENTATION_FIELD)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{SORTIE_PRESENTATION_FIELD}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SORTIE_PRESENTATION_FIELD)
        self.assertIn("SortiePresentation", SORTIE_PRESENTATION_FIELD)
        self.assertIn(
            "USkyguardSortiePresentationComponent",
            SORTIE_PRESENTATION_FIELD,
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, SORTIE_PRESENTATION_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_audio_director_fail_closed(
        ...
        self.assertNotIn("SkyguardSortiePresentationWidgets.h", locked_only)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation_fail_closed(
        ...
        self.assertNotIn("SetSortieLaunched", locked_only)
        self.assertNotIn("RefreshDebrief", locked_only)

    def test_contract_does_not_relock_leftover_debrief_widget(self) -> None:
        locked_only = f"{SORTIE_PRESENTATION_FIELD}\n"
        for token in LEFTOVER_DEBRIEF_WIDGET_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SORTIE_PRESENTATION_FIELD)
        self.assertIn(
            "Scripts/tests/test_debrief_widget_configure"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_debrief_widget_get_presentation"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_debrief_widget_acknowledge_debrief"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("CurrentIntegrity", SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", SORTIE_PRESENTATION_FIELD)

    def test_contract_does_not_open_leftover_sortie_presentation_headers(
        self,
    ) -> None:
        locked_only = f"{SORTIE_PRESENTATION_FIELD}\n"
        ...
        for leftover_header in leftover_headers:
            self.assertNotIn(leftover_header, locked_only)
            self.assertNotIn(leftover_header, SORTIE_PRESENTATION_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SORTIE_PRESENTATION_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", SORTIE_PRESENTATION_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SORTIE_PRESENTATION_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, SORTIE_PRESENTATION_FIELD),
            SORTIE_PRESENTATION_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SORTIE_PRESENTATION_FIELD)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 SortiePresentation field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{SORTIE_PRESENTATION_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SORTIE_PRESENTATION_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SORTIE_PRESENTATION_FIELD.lower(),
                "mission01 SortiePresentation contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SORTIE_PRESENTATION_FIELD)

    def test_contract_is_sortie_presentation_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(SORTIE_PRESENTATION_FIELD.endswith(";"))
        self.assertNotIn("=", SORTIE_PRESENTATION_FIELD)
        self.assertIn(UPROPERTY_MISSION01, section)

    def test_sibling_director_fields_do_not_satisfy_sortie_presentation(self) -> None:
        for leftover in (
            ROOT_FIELD,
            BRIEFING_FIELD,
            AUDIO_DIRECTOR_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, SORTIE_PRESENTATION_FIELD))
            self.assertNotEqual(SORTIE_PRESENTATION_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_root_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_debrief_widget_scripts_stay_sibling_only(self) -> None:
        self.assertIn(
            "Scripts/tests/test_debrief_widget_configure"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
