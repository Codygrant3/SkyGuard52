# THIS IS leftover-safe FSkyguardTheaterKitSpec SilhouetteKit.
...
# then BARE `FName SilhouetteKit;` (no initializer).
...
# NamedLandmark / LandmarkTint / SilhouetteTint /
...
TARGET = "FName SilhouetteKit;"
TARGET_WRONG_EQ_NONE = "FName SilhouetteKit = NAME_None;"
TARGET_WRONG_FALSE = "FName SilhouetteKit = false;"
TARGET_WRONG_TRUE = "FName SilhouetteKit = true;"
TARGET_WRONG_ZERO = "FName SilhouetteKit = 0.f;"
TARGET_WRONG_FLOAT = "float SilhouetteKit;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName SilhouetteKit;`.
    ...
    if re.search(
        r"FName\s+SilhouetteKit\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SilhouetteKit\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+SilhouetteKit\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName SilhouetteKit is missing from "
...
class TheaterKitSpecSilhouetteKitFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SilhouetteKit"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SilhouetteKit"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SilhouetteKit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_silhouette_kit_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName SilhouetteKit;", compact_origin)
        self.assertNotIn("SilhouetteKit = NAME_None", compact_origin)
        self.assertNotIn("SilhouetteKit = false", compact_origin)
        self.assertNotIn("SilhouetteKit = true", compact_origin)
        self.assertNotIn("SilhouetteKit = 0.f", compact_origin)
        self.assertNotIn("SilhouetteKit = 160.f", compact_origin)

    def test_silhouette_kit_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName SilhouetteKit"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            "\tFName SilhouetteKits;\n",
            "\tint32 SilhouetteKit;\n",
            "\tbool SilhouetteKit;\n",
            "\tfloat SilhouetteKit = " + forty + ";\n",
            "\tfloat SilhouetteKit = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec SilhouetteKit field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "silhouette_kit_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
