# THIS IS leftover-safe FSkyguardRouteDefinition Points.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `TArray<FSkyguardRoutePoint> Points;`
# (no initializer).
...
# TArray<FSkyguardRoutePoint>,
# or if an initializer is added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardRouteDefinition` body ONLY after
# `struct FSkyguardRouteDefinition`. Stop at
# `struct FSkyguardObjectiveDefinition` and later structs
# (`USkyguardMissionDefinition` if present).
# Do NOT parse the body of `FSkyguardRoutePoint`
# (leftover isolated fields #1339-#1343, leftover
# defaults #180 / #0ac5) except as the TArray element
# type on this field.
...
# leftover `FSkyguardMissionDebrief`.
# Do NOT parse leftover ProductionAudio Audit/Entry/
# Routing or leftover `USkyguardAudioProductionBank`.
...
# Do NOT parse leftover route-definition-fields
# #192 / #dbff except this isolated Points field.
# Do NOT parse leftover mission-types-defaults-tests
# #153 / #f47d (keep that bulk file in LOCKED_SCRIPTS).
# Do NOT contract sibling field RouteId.
# Analog leftover route-definition-fields #192 / #dbff
# then leftover isolated RoutePoint fields #1339-#1343,
# then this isolated Points field decl.
# THIS IS NOT leftover route-definition-fields #192 / #dbff.
# THIS IS NOT leftover RoutePoint isolated PointId #1339,
# WorldLocation #1340, TargetAirspeedKph #1341,
# LookAheadSeconds #1342, or bAllowCombatOrbit #1343.
# THIS IS NOT leftover route-point-defaults #180 / #0ac5.
# THIS IS NOT leftover EnemyWave Formations #1346.
# THIS IS NOT leftover ObjectiveDefinition public
# UPROPERTY leftover-safe exhausted #1364-#1370.
...
# the bulk).
# THIS IS NOT leftover mission-definition Route /
# Objectives / Waves / Weather / ScoreRules field decls.
# Analog: leftover route-definition-fields then this
# isolated Points field decl, same as leftover
# route-point-defaults then isolated RoutePoint fields,
# same as leftover enemy-wave-defaults then isolated
# Formations, same as leftover radio-line-defaults then
# isolated RadioLine fields.
# If a clone asserts VisibleAnywhere /
# Category="Skyguard|Theater" / BlueprintReadWrite /
# `FName WeatherIdentity` / `FName LineId` /
# `FName PointId` / `FName RouteId` / required
# initializer, retarget: type is
# TArray<FSkyguardRoutePoint> Points,
# no `=`, specifiers are EditAnywhere +
# BlueprintReadOnly, no Category.
# Harbor 40/80 fail-closed on the accepted decl
...
TARGET = "TArray<FSkyguardRoutePoint> Points;"
TARGET_WRONG_EQ_NONE = (
    "TArray<FSkyguardRoutePoint> Points = {};"
)
TARGET_WRONG_FALSE = (
    "TArray<FSkyguardRoutePoint> Points = false;"
)
TARGET_WRONG_TRUE = (
    "TArray<FSkyguardRoutePoint> Points = true;"
)
TARGET_WRONG_ZERO = (
    "TArray<FSkyguardRoutePoint> Points = 0.f;"
)
TARGET_WRONG_FLOAT = "float Points;"
TARGET_WRONG_FNAME = "FName Points;"
TARGET_WRONG_TARRAY_FNAME = "TArray<FName> Points;"
...
STOP_BEFORE_ENEMY_FORMATION = "struct FSkyguardRoutePoint"
...
LEFTOVER_OBJECTIVE_DEFINITION_FAILURE_ENDS = (
...
LEFTOVER_AUDIO_PRODUCTION_BANK = (
...
def leftover_route_point_body_tokens() -> tuple[str, ...]:
    return (
        "PointId",
        "WorldLocation",
        "TargetAirspeedKph",
...
def leftover_objective_definition_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_OBJECTIVE_DEFINITION_OBJECTIVE_ID,
        LEFTOVER_OBJECTIVE_DEFINITION_DISPLAY_NAME,
        LEFTOVER_OBJECTIVE_DEFINITION_TYPE,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # RouteId field. Type must be TArray of
    # FSkyguardRoutePoint. Harbor 40/80
    # fail-closed on the accepted decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FSkyguardRoutePoint\s*>"
        r"\s+Points\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:FName|float|bool|int32)\s+Points\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+Points\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FSkyguardRoutePoint"
            r"\s*>\s+Points\b",
            compact[index:],
        ):
    ...
    raise AssertionError(
        "UPROPERTY for TArray<FSkyguardRoutePoint> "
...
def require_points_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} Points UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} Points"
        )
...
class RouteDefinitionPointsFieldDeclContractTests(unittest.TestCase):
    def test_route_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Points"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Points"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_DEFINITION, section)
        ...
        self.assertIn("Points", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_points_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_ROUTE_ID};\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_points_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("Points = NAME_None", compact_origin)
        self.assertNotIn("Points = false", compact_origin)
        self.assertNotIn("Points = true", compact_origin)
        self.assertNotIn("Points = 0.f", compact_origin)
        self.assertNotIn("Points = 160.f", compact_origin)
        self.assertNotIn("Points = {}", compact_origin)

    def test_points_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("FSkyguardRoutePoint", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTArray<FSkyguardRoutePoint> Point;\n",
            "\tint32 Points;\n",
            "\tbool Points;\n",
            "\tfloat Points = " + forty + ";\n",
            "\tfloat Points = " + eighty + ";\n",
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_points_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_points_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_points_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_points_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_points_specifiers(spec_section(with_category))
    ...
    def test_does_not_contract_sibling_route_definition_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        for leftover in leftover_route_point_body_tokens():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, LOCKED_DECL)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for leftover in leftover_route_point_body_tokens():
            ...
            self.assertNotIn(leftover, LOCKED_DECL)
        self.assertIn(ROUTE_POINT_NAME, section)
        self.assertIn("TArray<" + ROUTE_POINT_NAME + ">", collapsed(section))
        self.assertNotIn("struct " + ROUTE_POINT_NAME, section)
        self.assertNotIn("struct " + ROUTE_POINT_NAME, leaked)
        for leftover in leftover_pictogram_values():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, leaked)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "route-definition Points field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "route_definition_points_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(SIBLING_ROUTE_ID_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            *leftover_radio_defaults_scripts(),
            ...
            REMAINING_ALLOW_COMBAT_ORBIT_SCRIPT,
            LEFTOVER_ENEMY_WAVE_FORMATIONS,
            *leftover_objective_definition_scripts(),
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_ROUTE_ID, locked_only)
