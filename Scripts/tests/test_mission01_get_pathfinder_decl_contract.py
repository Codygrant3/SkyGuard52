...
# INDEX_NONE or lock GetPathfinder construction
...
# GetMissionId / ValidateMissionContract. Do not lock leftover
...
# GetPathfinder method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01IntegrationDirector::GetPathfinder",
    "SkyguardMission01IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission01GetPathfinderDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_PATHFINDER),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_PATHFINDER)
        self.assertIn("GetPathfinder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_PATHFINDER)
    ...
    def test_missing_get_pathfinder_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_PATHFINDER)
        self.assertIn("GetPathfinder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_PATHFINDER)
        self.assertIn("GetPathfinder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_PATHFINDER)
        self.assertNotIn("BlueprintPure", GET_PATHFINDER)
        self.assertNotIn("Category", GET_PATHFINDER)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_PATHFINDER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_PATHFINDER)
        self.assertIn("GetPathfinder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid GetPathfinder() const;\n"
        missing_const = (
        ...
        extra_arg = (
            "\tASkyguardPathfinderBoss* "
            "GetPathfinder(int32 Amount) const;\n"
        )
        extra_args = (
            "\tASkyguardPathfinderBoss* "
            "GetPathfinder("
        ...
        as_bool = "\tbool GetPathfinder() const;\n"
        missing_star = (
            "\tASkyguardPathfinderBoss "
            "GetPathfinder() const;\n"
        )
        ...
        short_name = "\tASkyguardPathfinderBoss* Pathfinder;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_PATHFINDER)
            self.assertIn("GetPathfinder", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_pathfinder_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_PATHFINDER),
            GET_PATHFINDER,
        )
        self.assertTrue(has_declaration(section, GET_PATHFINDER))
        self.assertEqual(
            declaration_count(section, GET_PATHFINDER),
            1,
        )
        self.assertTrue(
            GET_PATHFINDER.startswith("ASkyguardPathfinderBoss* "),
            GET_PATHFINDER,
        )
        self.assertTrue(
            GET_PATHFINDER.endswith(" const"),
            GET_PATHFINDER,
        )
        self.assertFalse(
            GET_PATHFINDER.endswith(";"),
            GET_PATHFINDER,
        )
        self.assertIn("GetPathfinder", GET_PATHFINDER)
        self.assertIn("GetPathfinder()", GET_PATHFINDER)
        self.assertIn(
            "ASkyguardPathfinderBoss*",
            GET_PATHFINDER,
        ...
        self.assertNotIn("EnvironmentReadiness", GET_PATHFINDER)
        self.assertNotIn("MissionMapReadiness", GET_PATHFINDER)
        self.assertNotIn("TArray<", GET_PATHFINDER)
        self.assertNotIn("TObjectPtr", GET_PATHFINDER)
        self.assertNotIn("TSoftObjectPtr", GET_PATHFINDER)
        self.assertNotIn("INDEX_NONE", GET_PATHFINDER)
        self.assertNotIn("UFUNCTION", GET_PATHFINDER)
        self.assertNotIn("{", GET_PATHFINDER)
        self.assertNotIn("}", GET_PATHFINDER)
        self.assertNotIn("return ", GET_PATHFINDER)
        self.assertIn(" const", GET_PATHFINDER)
        self.assertNotIn(" const;", GET_PATHFINDER)
        self.assertNotIn("Root", GET_PATHFINDER)
        self.assertNotIn("Briefing", GET_PATHFINDER)
        self.assertNotIn("AudioDirector", GET_PATHFINDER)
        self.assertNotIn("RadioChatter", GET_PATHFINDER)
        self.assertNotIn("SortiePresentation", GET_PATHFINDER)
        self.assertNotIn("CampaignDefinition", GET_PATHFINDER)
        self.assertNotIn("MissionDefinition", GET_PATHFINDER)
        self.assertNotIn("Readiness;", GET_PATHFINDER)
        self.assertNotIn("bAutoInitialize", GET_PATHFINDER)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_PATHFINDER)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_PATHFINDER)
        self.assertNotIn("PathfinderSpawnLocation", GET_PATHFINDER)
        self.assertNotIn("GetAircraft", GET_PATHFINDER)
        self.assertNotIn("BindRuntimeActors", GET_PATHFINDER)
        self.assertNotIn("HandleDroneCityImpact", GET_PATHFINDER)
        self.assertNotIn("ConfigureMissionDefinition", GET_PATHFINDER)
        self.assertNotIn("InitializePlayableMission", GET_PATHFINDER)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_PATHFINDER)
        self.assertNotIn("SynchronizeRuntimeState", GET_PATHFINDER)
        self.assertNotIn("NotifyObjectiveProgress", GET_PATHFINDER)
        self.assertNotIn("IsCorePlayableReady", GET_PATHFINDER)
        self.assertNotIn("GetObjectiveRuntime", GET_PATHFINDER)
        self.assertNotIn("GetGunner", GET_PATHFINDER)
        self.assertNotIn("GetReadiness", GET_PATHFINDER)
        self.assertNotIn("GetMissionId", GET_PATHFINDER)
        self.assertNotIn("ValidateMissionContract", GET_PATHFINDER)
        self.assertNotIn("ConfigureFromMission", GET_PATHFINDER)
        self.assertNotIn("AdvanceBriefing", GET_PATHFINDER)
        self.assertNotIn("SetAssetsReady", GET_PATHFINDER)
        self.assertNotIn("AcknowledgeAndLaunch", GET_PATHFINDER)
        self.assertNotIn("CanLaunch", GET_PATHFINDER)
        self.assertNotIn("GetElapsedSeconds", GET_PATHFINDER)
        self.assertNotIn("GetBriefingState", GET_PATHFINDER)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_PATHFINDER)
        self.assertNotIn("GetBriefingText", GET_PATHFINDER)
        self.assertNotIn("GetRadioChatter", GET_PATHFINDER)
        self.assertNotIn("GetPresentation", GET_PATHFINDER)
        self.assertNotIn("GetMissionTitle", GET_PATHFINDER)
        self.assertNotIn("AcknowledgeBriefing", GET_PATHFINDER)
        self.assertNotIn("LaunchSortie", GET_PATHFINDER)
        self.assertNotIn("HullCollider", GET_PATHFINDER)
        self.assertNotIn("OpticalTracker", GET_PATHFINDER)
        self.assertNotIn("WeaponServo", GET_PATHFINDER)
        self.assertNotIn("CountermeasurePod", GET_PATHFINDER)
        self.assertNotIn("MinHeightFromOriginCm", GET_PATHFINDER)
        self.assertNotIn("RadarNode", GET_PATHFINDER)
        self.assertNotIn("ESkyguardMission02WaveState", GET_PATHFINDER)
        self.assertNotIn("HarborIndustrial", GET_PATHFINDER)
        self.assertNotIn("MaxIntegrity", GET_PATHFINDER)
        self.assertNotIn("CurrentIntegrity", GET_PATHFINDER)
        self.assertNotIn("FillAndFinalize", GET_PATHFINDER)
        self.assertNotIn("FillAndFail", GET_PATHFINDER)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_PATHFINDER)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_INTEGRATION} {GET_PATHFINDER}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_PATHFINDER}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_PATHFINDER))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_PATHFINDER),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_PATHFINDER)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_pathfinder_cpp_body(self) -> None:
        locked_only = f"{GET_PATHFINDER}\n"
        self.assertNotIn("{", GET_PATHFINDER)
        self.assertNotIn("}", GET_PATHFINDER)
        self.assertNotIn("return ", GET_PATHFINDER)
        ...
        self.assertNotIn("return false", GET_PATHFINDER)
        self.assertNotIn("return true", GET_PATHFINDER)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_PATHFINDER}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PATHFINDER)
        self.assertIn("GetPathfinder", GET_PATHFINDER)
        self.assertTrue(GET_PATHFINDER.startswith("ASkyguardPathfinderBoss* "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_PATHFINDER)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_PATHFINDER)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_PATHFINDER)
        self.assertNotIn("CurrentIntegrity", GET_PATHFINDER)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_PATHFINDER)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_PATHFINDER)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_PATHFINDER)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_PATHFINDER}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_PATHFINDER),
            GET_PATHFINDER,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PATHFINDER)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 GetPathfinder method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_PATHFINDER}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_PATHFINDER.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_PATHFINDER.lower(),
                "mission01 GetPathfinder contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_PATHFINDER)

    def test_contract_is_get_pathfinder_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(GET_PATHFINDER.startswith("ASkyguardPathfinderBoss* "))
        self.assertIn("GetPathfinder", GET_PATHFINDER)
        self.assertIn("GetPathfinder() const", GET_PATHFINDER)
        self.assertTrue(
            GET_PATHFINDER.endswith(" const"),
            GET_PATHFINDER,
        )
        self.assertFalse(GET_PATHFINDER.endswith(";"))
        self.assertNotIn("EnvironmentReadiness", GET_PATHFINDER)
        self.assertNotIn("MissionMapReadiness", GET_PATHFINDER)
        self.assertIn(UFUNCTION_INTEGRATION, section)

    def test_sibling_director_fields_do_not_satisfy_get_pathfinder(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_PATHFINDER)
            )
            self.assertNotEqual(GET_PATHFINDER, leftover)
        self.assertIn(
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", GET_PATHFINDER)
        self.assertNotIn("return ", GET_PATHFINDER)
        self.assertNotIn("return Other", GET_PATHFINDER)
        self.assertNotIn("return Pathfinder", GET_PATHFINDER)
