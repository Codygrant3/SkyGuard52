...
# INDEX_NONE or lock Readiness construction in the
# .cpp. This is a FIELD contract on
# ASkyguardMission01IntegrationDirector, not leftover
# FSkyguardMission01IntegrationReadiness struct-defaults
# and not leftover environment-readiness defaults
# #6b9d / #b931. Do not lock leftover GetReadiness;
# this is the field. Keep Readiness distinct from
# sibling director fields Briefing (#568) /
# AudioDirector (#572) / Root (#574) / RadioChatter /
# SortiePresentation / CampaignDefinition (in flight) /
# MissionDefinition (sibling this wave). Do not lock
# leftover spawn-location fields on this class. Do
# not lock leftover GetAircraft. Do not lock leftover
...
# Mission02 / Boss / Destruction / Apache.
...
# defaults #6b9d / #b931, leftover audio-director
# fail-closed contracts, leftover radio-chatter
# fail-closed contracts, leftover
# campaign-definition method contracts, leftover
# skyline style HarborIndustrial (leftover enum, not
# a Harbor 40/80 retune). Harbor interval retune
...
# Readiness field declaration contract on
...
# contracts, leftover briefing-card /
# briefing-radio-row defaults, leftover briefing
# fail-closed tests, leftover Harbor #6/#8/#9,
# leftover theater-kit #59, leftover
# environment-readiness defaults #6b9d / #b931,
# leftover Mission01 Briefing field contract #568,
# leftover Mission01 AudioDirector field contract
# #572, leftover Mission01 Root field contract #574,
# leftover RadioChatter / SortiePresentation /
# CampaignDefinition / MissionDefinition field
# contracts, leftover GetReadiness method, leftover
# audio-director fail-closed, leftover radio-chatter
# fail-closed, leftover campaign-definition method
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission01_integration_readiness_defaults_contract.py",
...
LEFTOVER_INTEGRATION_READINESS_STRUCT_DEFAULTS_NOT_LOCKED = (
    ...
    "ObjectiveCount",
    "RadioLineCount",
    "test_mission01_integration_readiness_defaults_contract.py",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01IntegrationDirector::Readiness",
    "SkyguardMission01IntegrationDirector.cpp",
...
class Mission01ReadinessFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, READINESS_FIELD), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, READINESS_FIELD)
        self.assertIn("Readiness", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, READINESS_FIELD))
    ...
    def test_missing_readiness_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, READINESS_FIELD)
        self.assertIn("Readiness", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, READINESS_FIELD)
        self.assertIn("Readiness", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, READINESS_FIELD), section)
        self.assertNotIn("UPROPERTY", READINESS_FIELD)
        self.assertNotIn("VisibleAnywhere", READINESS_FIELD)
        self.assertNotIn("BlueprintReadOnly", READINESS_FIELD)
        self.assertNotIn("Category", READINESS_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, READINESS_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, READINESS_FIELD)
        self.assertIn("Readiness", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        leftover_get = f"\t{GET_READINESS}\n"
        leftover_env = (
            "\tFSkyguardMission01EnvironmentReadiness Readiness;\n"
        )
        leftover_env_generic = (
            "\tFSkyguardEnvironmentReadiness Readiness;\n"
        )
        leftover_ptr = (
            "\tTObjectPtr<FSkyguardMission01IntegrationReadiness> "
            "Readiness;\n"
        )
        leftover_array = (
            "\tTArray<FSkyguardMission01IntegrationReadiness> "
            "Readiness;\n"
        )
        leftover_soft = (
            "\tTSoftObjectPtr<FSkyguardMission01IntegrationReadiness> "
            "Readiness;\n"
        )
        leftover_assigned = (
            "\tFSkyguardMission01IntegrationReadiness "
        ...
        leftover_struct_member = "\tbool bMissionDefinitionValid = false;\n"
        leftover_runtime = (
            f"\tbool {leftover_readiness_tokens()[0]} = false;\n"
        )
        for region in (
            leftover_get,
            leftover_env,
            ...
            leftover_wave,
            leftover_skyline,
            leftover_struct_member,
            leftover_runtime,
        ):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, READINESS_FIELD)
            self.assertIn("Readiness", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_readiness_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, READINESS_FIELD))
        self.assertEqual(declaration_count(section, READINESS_FIELD), 1)
        self.assertTrue(
            READINESS_FIELD.startswith(
                "FSkyguardMission01IntegrationReadiness "
            ),
            READINESS_FIELD,
        )
        self.assertTrue(READINESS_FIELD.endswith(";"), READINESS_FIELD)
        self.assertIn("Readiness", READINESS_FIELD)
        self.assertIn(
            "FSkyguardMission01IntegrationReadiness",
            READINESS_FIELD,
        )
        self.assertNotIn("TObjectPtr", READINESS_FIELD)
        self.assertNotIn("TArray<", READINESS_FIELD)
        self.assertNotIn("TSoftObjectPtr", READINESS_FIELD)
        self.assertNotIn("=", READINESS_FIELD)
        self.assertNotIn("INDEX_NONE", READINESS_FIELD)
        self.assertNotIn("UFUNCTION", READINESS_FIELD)
        self.assertNotIn("{", READINESS_FIELD)
        self.assertNotIn("}", READINESS_FIELD)
        self.assertNotIn("return ", READINESS_FIELD)
        self.assertNotIn("GetReadiness", READINESS_FIELD)
        self.assertNotIn("Briefing", READINESS_FIELD)
        self.assertNotIn("AudioDirector", READINESS_FIELD)
        self.assertNotIn("Root;", READINESS_FIELD)
        self.assertNotIn("RadioChatter", READINESS_FIELD)
        self.assertNotIn("SortiePresentation", READINESS_FIELD)
        self.assertNotIn("CampaignDefinition", READINESS_FIELD)
        self.assertNotIn("MissionDefinition", READINESS_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", READINESS_FIELD)
        self.assertNotIn("GetAircraft", READINESS_FIELD)
        self.assertNotIn("ConfigureFromMission", READINESS_FIELD)
        self.assertNotIn("AdvanceBriefing", READINESS_FIELD)
        self.assertNotIn("SetAssetsReady", READINESS_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", READINESS_FIELD)
        self.assertNotIn("CanLaunch", READINESS_FIELD)
        self.assertNotIn("GetElapsedSeconds", READINESS_FIELD)
        self.assertNotIn("GetBriefingState", READINESS_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", READINESS_FIELD)
        self.assertNotIn("GetBriefingText", READINESS_FIELD)
        self.assertNotIn("GetRadioChatter", READINESS_FIELD)
        self.assertNotIn("GetPresentation", READINESS_FIELD)
        self.assertNotIn("GetMissionTitle", READINESS_FIELD)
        self.assertNotIn("AcknowledgeBriefing", READINESS_FIELD)
        self.assertNotIn("LaunchSortie", READINESS_FIELD)
        self.assertNotIn("HullCollider", READINESS_FIELD)
        self.assertNotIn("OpticalTracker", READINESS_FIELD)
        self.assertNotIn("WeaponServo", READINESS_FIELD)
        self.assertNotIn("CountermeasurePod", READINESS_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", READINESS_FIELD)
        self.assertNotIn("RadarNode", READINESS_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", READINESS_FIELD)
        self.assertNotIn("HarborIndustrial", READINESS_FIELD)
        self.assertNotIn("MaxIntegrity", READINESS_FIELD)
        self.assertNotIn("CurrentIntegrity", READINESS_FIELD)
        self.assertNotIn(
            "FSkyguardMission01EnvironmentReadiness",
            READINESS_FIELD,
        )
        ...
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, READINESS_FIELD)
        for name in leftover_spawn_name_tokens():
            self.assertNotIn(name, READINESS_FIELD)
        for name in leftover_readiness_tokens():
            self.assertNotIn(name, READINESS_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01} {READINESS_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, READINESS_FIELD), 1)
        one_line = f"{{\npublic:\n\t{READINESS_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, READINESS_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, READINESS_FIELD), section)
        self.assertEqual(
            require_declaration(section, READINESS_FIELD),
    ...
    def test_assigned_readiness_does_not_satisfy(self) -> None:
        assigned = (
            "\tFSkyguardMission01IntegrationReadiness "
            "Readiness = {};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, READINESS_FIELD)
        self.assertIn("Readiness", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, READINESS_FIELD))

    def test_leftover_get_readiness_does_not_satisfy(self) -> None:
        leftover = f"\t{GET_READINESS}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, READINESS_FIELD)
        self.assertIn("Readiness", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, READINESS_FIELD))
        self.assertNotIn("GetReadiness", READINESS_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_readiness_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_leftover_struct_defaults_do_not_satisfy(self) -> None:
        leftover_member = "\tbool bMissionDefinitionValid = false;\n"
        leftover_runtime = (
            f"\tbool {leftover_readiness_tokens()[0]} = false;\n"
        )
        leftover_env = (
        ...
        for region in (leftover_member, leftover_runtime, leftover_env):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, READINESS_FIELD)
            self.assertIn("Readiness", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, READINESS_FIELD))
        for token in leftover_readiness_tokens():
            self.assertNotIn(token, READINESS_FIELD)
        self.assertNotIn("bMissionDefinitionValid", READINESS_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            READINESS_FIELD.startswith("UFUNCTION"),
            READINESS_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, READINESS_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", READINESS_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_readiness_cpp_body(self) -> None:
        locked_only = f"{READINESS_FIELD}\n"
        self.assertNotIn("{", READINESS_FIELD)
        self.assertNotIn("}", READINESS_FIELD)
        self.assertNotIn("return ", READINESS_FIELD)
        ...
        self.assertNotIn("return false", READINESS_FIELD)
        self.assertNotIn("return true", READINESS_FIELD)
        self.assertNotIn("CreateDefaultSubobject", READINESS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_struct_defaults(self) -> None:
        locked_only = f"{READINESS_FIELD}\n"
        for token in LEFTOVER_INTEGRATION_READINESS_STRUCT_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, READINESS_FIELD)
        for token in leftover_readiness_tokens():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, READINESS_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_integration_readiness"
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, READINESS_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", READINESS_FIELD)
        self.assertNotIn("CurrentIntegrity", READINESS_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", READINESS_FIELD)

    def test_contract_does_not_open_leftover_briefing_component_header(
        self,
    ) -> None:
        locked_only = f"{READINESS_FIELD}\n"
        file_text = this_file_text()
        leftover_header = "SkyguardMissionBriefingComponent.h"
        self.assertNotIn(leftover_header, locked_only)
        self.assertNotIn(leftover_header, READINESS_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, READINESS_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", READINESS_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{READINESS_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, READINESS_FIELD),
            READINESS_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, READINESS_FIELD)
        ...
        self.assertNotIn("SkyguardMissionBriefingComponent.h", locked_only)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            locked_only,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 Readiness field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{READINESS_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, READINESS_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                READINESS_FIELD.lower(),
                "mission01 Readiness contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, READINESS_FIELD)

    def test_contract_is_readiness_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(READINESS_FIELD.endswith(";"))
        self.assertNotIn("=", READINESS_FIELD)
        self.assertNotIn("GetReadiness", READINESS_FIELD)
        self.assertIn(UPROPERTY_MISSION01, section)

    def test_sibling_director_fields_do_not_satisfy_readiness(self) -> None:
        for leftover in (
            ROOT_FIELD,
            BRIEFING_FIELD,
            AUDIO_DIRECTOR_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, READINESS_FIELD))
            self.assertNotEqual(READINESS_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_briefing_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
