...
# Recycled FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission09 PoolRuntime
# Recycled. THIS IS NOT leftover Mission09
# PoolRuntime Available #1119 / Active #1121 /
# PeakActive leftover / leftover Mission08
...
# INDEX_NONE or lock Recycled
# construction in the .cpp. This is leftover-safe
# Mission09 PoolRuntime Recycled
...
# Recycled FIELD (declaration-only). It is
...
class Mission09PoolRuntimeRecycledFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Recycled", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_recycled_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_recycled_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 Recycled"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("Recycled", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("Recycled", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 Recycled"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("Recycled", locked_only)
    ...
    def test_contract_is_recycled_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_recycled(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission09_pool_runtime_recycled"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
