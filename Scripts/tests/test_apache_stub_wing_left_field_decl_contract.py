# THIS IS leftover-safe ASkyguardApacheAircraft StubWingLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> StubWingLeft;`
# (no initializer).
...
# Stop BEFORE sibling StubWingRight as a claimed slot.
# Do NOT claim VerticalTail / TailBoom / PylonLeft /
# HydraLeft / HellfireLeft.
# Do NOT claim AircraftRoot, SilhouetteMesh, Fuselage,
# RotorPower, HoverBobCentimeters, MaxIntegrity,
# CurrentIntegrity, HullCollider.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> StubWingLeft;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> StubWingLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> StubWingLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> StubWingLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> StubWingLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> StubWingLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> StubWingLeft;"
...
STOP_BEFORE_STUB_WING_RIGHT = "StubWingRight"
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        LEFTOVER_APACHE_ROTOR_POWER,
        LEFTOVER_APACHE_SILHOUETTE_MESH,
        f"{prefix}test_apache_fuselage_field_decl_contract.py",
        f"{prefix}test_apache_canopy_field_decl_contract.py",
        f"{prefix}test_apache_nose_field_decl_contract.py",
        f"{prefix}test_apache_aircraft_root_field_decl_contract.py",
        f"{prefix}test_apache_tail_boom_field_decl_contract.py",
        f"{prefix}test_apache_vertical_tail_field_decl_contract.py",
        f"{prefix}test_apache_stub_wing_right_field_decl_contract.py",
        f"{prefix}test_apache_pylon_left_field_decl_contract.py",
        f"{prefix}test_apache_hydra_left_field_decl_contract.py",
        f"{prefix}test_apache_hellfire_left_field_decl_contract.py",
        LEFTOVER_APACHE_EMPTY,
        LEFTOVER_APACHE_EMPTY_TESTS,
        LEFTOVER_APACHE_EMPTY_CONTRACT,
        LEFTOVER_APACHE_OWN_SHIP,
...
STUB_WING_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+StubWingLeft\s*;"
)
STUB_WING_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+StubWingLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover StubWingRight / VerticalTail / TailBoom /
    # PylonLeft / HydraLeft / HellfireLeft / AircraftRoot,
    # leftover SilhouetteMesh, or an initializer
    # on StubWingLeft as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if STUB_WING_LEFT_DECL_RE.search(compact) is None:
        return False
    if STUB_WING_LEFT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_STUB_WING_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_STUB_WING_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_STUB_WING_RIGHT}"
        )
    for sibling in (
        SIBLING_PYLON_LEFT,
        SIBLING_HYDRA_LEFT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+StubWingLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> StubWingLeft "
...
class ApacheStubWingLeftFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "StubWingLeft"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_STUB_WING_RIGHT),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_PYLON_LEFT), section)
        self.assertFalse(has_identifier(section, SIBLING_HYDRA_LEFT), section)
        self.assertFalse(has_identifier(section, SIBLING_HELLFIRE_LEFT), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "StubWingLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("StubWingLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_stub_wing_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> StubWingLeft;", compact_origin)
        self.assertNotIn("StubWingLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_stub_wing_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> StubWingLeft"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_PYLON_LEFT}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> StubWingLeft;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> StubWingLefts;\n",
            "\tint32 StubWingLeft = 140;\n",
            "\tbool StubWingLeft = true;\n",
            "\tfloat StubWingLeft = " + forty + ";\n",
            "\tfloat StubWingLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_stub_wing_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_STUB_WING_RIGHT))
        ...
        self.assertNotIn(TARGET_WRONG_STUB_WING_RIGHT, section)
        self.assertNotIn(TARGET_WRONG_PYLON_LEFT, section)
        self.assertNotIn(TARGET_WRONG_HYDRA_LEFT, section)
        self.assertNotIn(TARGET_WRONG_HELLFIRE_LEFT, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_STUB_WING_RIGHT))
        self.assertFalse(has_identifier(section, SIBLING_STUB_WING_RIGHT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache StubWingLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "stub_wing_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
