...
# then `TArray<FVector> RoutePoints;`
# (NO initializer).
...
# This is a public TArray<FVector> route field, not a combat look float,
# not leftover Theater WeatherIdentity, not leftover
# FSkyguardRouteDefinition.Points, not leftover RoutePoint
# WorldLocation, and not leftover analog
...
# Stop BEFORE sibling ObjectiveAnchors as a claimed slot.
...
# SkylineStyle / WeatherProfileId / ObjectiveAnchors /
...
# Do NOT claim leftover FSkyguardRouteDefinition.Points #1433
# (TArray of FSkyguardRoutePoint, different class/type).
# Do NOT claim leftover RoutePoint WorldLocation #1340.
...
# LOCKED_DECL is `TArray<FVector> RoutePoints;`
# NOT `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# and NOT `TArray<FSkyguardRoutePoint> Points;` and NOT
# `TObjectPtr<USplineComponent> FlightRouteSpline;` and NOT
# `FName WeatherIdentity;` and NOT leftover analog
...
# / FName AssemblyRevision = TEXT("CampaignMapAssembly_v1")
# / TArray<FSkyguardRoutePoint> Points AS THE LOCKED DECL.
...
# or leftover FName AssemblyRevision with initializer.
...
TARGET = "TArray<FVector> RoutePoints;"
TARGET_WRONG_BARE = "TArray<FVector> RoutePoints = {};"
TARGET_WRONG_INIT = "TArray<FVector> RoutePoints = nullptr;"
TARGET_WRONG_FALSE = "TArray<FVector> RoutePoints = false;"
TARGET_WRONG_TRUE = "TArray<FVector> RoutePoints = true;"
TARGET_WRONG_ZERO = "TArray<FVector> RoutePoints = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> RoutePoints;"
TARGET_WRONG_ASSEMBLY_REVISION = (
    'FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");'
)
TARGET_WRONG_ROUTE_DEF_POINTS = "TArray<FSkyguardRoutePoint> Points;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> RoutePoints;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> RoutePoints;"
...
STOP_BEFORE_OBJECTIVE_ANCHORS = "ObjectiveAnchors"
...
LEFTOVER_MISSION_MAP_ASSEMBLY_REVISION = (
...
ROUTE_POINTS_DECL_RE = re.compile(
    r"TArray<FVector>\s+RoutePoints\s*;"
)
ROUTE_POINTS_INIT_RE = re.compile(
    r"TArray<FVector>\s+RoutePoints\s*="
)
ASSEMBLY_REVISION_DECL_RE = re.compile(
    r"FName\s+AssemblyRevision\s*=\s*TEXT\(\s*\"CampaignMapAssembly_v1\"\s*\)\s*;"
)
ROUTE_DEF_POINTS_DECL_RE = re.compile(
    r"TArray<FSkyguardRoutePoint>\s+Points\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover FName AssemblyRevision with TEXT initializer,
    # leftover TArray<FSkyguardRoutePoint> Points,
    ...
    # RoutePoints field that still has an initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ROUTE_POINTS_DECL_RE.search(compact) is None:
        return False
    if ROUTE_POINTS_INIT_RE.search(compact):
        return False
    if ASSEMBLY_REVISION_DECL_RE.search(compact):
        return False
    if ROUTE_DEF_POINTS_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_OBJECTIVE_ANCHORS:
            break
        if ident != "RoutePoints":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_OBJECTIVE_ANCHORS):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_OBJECTIVE_ANCHORS}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray<FVector>\s+RoutePoints\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FVector> RoutePoints "
...
class MissionMapRoutePointsFieldDeclContractTests(unittest.TestCase):
    def test_mission_map_assembly_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RoutePoints"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_OBJECTIVE_ANCHORS), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_OBJECTIVE_ANCHORS),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RoutePoints"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("RoutePoints", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_route_points_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_route_points_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FVector> RoutePoints"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("TArray<FVector>", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_DEF_POINTS)
        ...
        self.assertNotIn("=", LOCKED_DECL)
        self.assertNotIn('TEXT("CampaignMapAssembly_v1")', LOCKED_DECL)
        self.assertNotIn("FSkyguardRoutePoint", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTArray<FVector> RoutePointses = {};\n",
            "\tint32 RoutePoints = 1;\n",
            "\tbool RoutePoints = true;\n",
            "\tTArray<FVector> RoutePoints = " + forty + ";\n",
            "\tTArray<FVector> RoutePoints = " + eighty + ";\n",
            f"\t{TARGET_WRONG_ASSEMBLY_REVISION}\n",
            f"\t{TARGET_WRONG_ROUTE_DEF_POINTS}\n",
    ...
    def test_does_not_claim_sibling_objective_anchors_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "RoutePoints"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_OBJECTIVE_ANCHORS))
        self.assertFalse(has_identifier(section, STOP_BEFORE_OBJECTIVE_ANCHORS))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "RoutePoints"))
        self.assertTrue(has_identifier(section, "RoutePoints"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_OBJECTIVE_ANCHORS))
        self.assertFalse(has_identifier(section, STOP_BEFORE_OBJECTIVE_ANCHORS))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, SIBLING_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_WEATHER_PROFILE_ID))
        self.assertFalse(has_identifier(section, SIBLING_WEATHER_PROFILE_ID))
        self.assertTrue(has_identifier(leaked, SIBLING_LANDMARK_ANCHORS))
        self.assertFalse(has_identifier(section, SIBLING_LANDMARK_ANCHORS))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map RoutePoints field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_map_route_points_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_MISSION_MAP_ASSEMBLY_REVISION, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_MISSION_MAP_FLIGHT_ROUTE_SPLINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_MAP_FLIGHT_CLEARANCE_RADIUS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_MAP_FLIGHT_CLEARANCE_VERTICAL, LOCKED_SCRIPTS)
