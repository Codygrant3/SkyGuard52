# THIS IS leftover-safe FSkyguardBriefingCard CardId.
# origin/main form: split-line UPROPERTY(VisibleAnywhere,
# BlueprintReadOnly) then BARE `FName CardId;` (no initializer,
...
# bare FName CardId;. Fail closed if type/decl missing, renamed,
# Category= is added, or an initializer appears when origin/main
# is bare.
# Parse STRUCT public section ONLY after struct FSkyguardBriefingCard
# and BEFORE struct FSkyguardBriefingRadioRow.
...
# THIS IS leftover-safe FSkyguardBriefingCard
...
# struct FSkyguardBriefingCard and BEFORE
...
# Error #1232.
# Declaration-only. Accept origin/main split-line UPROPERTY and
# FName CardId;. Fail closed if type/decl
...
STRUCT_NAME = "FSkyguardBriefingCard"
...
# BlueprintReadOnly FName CardId;
...
TARGET = "FName CardId;"
TARGET_WRONG = "FName CardId = NAME_None;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer for FSkyguardBriefingCard::
    # Error (must be bare FName CardId;).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FName\s+CardId\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FName CardId is missing from "
...
class BriefingCardCardIdFieldDeclContractTests(unittest.TestCase):
    def test_briefing_card_struct_exists(self) -> None:
        ...
        self.assertIn("Pictogram", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("CardId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_card_id_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_card_id_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName CardId"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("CardId", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("CardId", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FName CardId"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("CardId", locked_only)
    ...
    def test_this_file_bans_live_retired_tokens_case_sensitive(self) -> None:
        ...
        for banned in live:
            self.assertNotIn(
                banned,
                file_text,
                "briefing-card CardId field contract contains live "
