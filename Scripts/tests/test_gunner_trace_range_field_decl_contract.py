# THIS IS leftover-safe ASkyguardGunner TraceRange.
...
# then `float TraceRange = 32000.f;` (HAS initializer).
# Do NOT claim leftover ADSFov / MouseSensitivity /
# LookPitchMax #1619 / LookYawLimit #1618 / LookPitchMin
# #1617 / leftover in-flight HipFov as this slot.
# SKIP leftover analog apache-cpg-feel FireRate = 12.0f
# forever. Parse past it; do not lock it.
# Stop BEFORE sibling BaseDamage. Do not claim BaseDamage,
# RecoilPitch, FireRate, HipFov, ADSFov, MouseSensitivity.
# LOCKED_DECL initializer is 32000.f, NOT leftover Harbor 40 / 80.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Combat`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS `= 32000.f`.
# This is a combat trace-range float, not a CPG mesh/text slot
# and not leftover analog apache-cpg-feel FireRate /
# RecoilPitch. Fail-closed if the UPROPERTY or decl is
# missing or renamed, if Category is missing, or if
# specifiers drop to VisibleAnywhere / BlueprintReadOnly
# or stay Category="Skyguard" without |Combat.
...
# Stop BEFORE sibling BaseDamage as a claimed slot.
# Skip previous sibling FireRate = 12.0f forever.
...
# Category Skyguard — RETARGET hard: wrap is
# EditAnywhere BlueprintReadWrite Category Skyguard|Combat.
# LOCKED_DECL is `float TraceRange = 32000.f;` NOT
...
# or still Category="Skyguard" without |Combat.
# Combat trace-range float fail-closed if LOCKED_DECL is
...
TARGET = "float TraceRange = 32000.f;"
TARGET_WRONG_BARE = "float TraceRange;"
TARGET_WRONG_INIT = "float TraceRange = nullptr;"
TARGET_WRONG_FALSE = "float TraceRange = false;"
TARGET_WRONG_TRUE = "float TraceRange = true;"
TARGET_WRONG_ZERO = "float TraceRange = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> TraceRange;"
...
TARGET_WRONG_HARBOR_FORTY = (
    "float TraceRange = " + "40" + ".f;"
)
TARGET_WRONG_HARBOR_EIGHTY = (
    "float TraceRange = " + "80" + ".f;"
)
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> TraceRange;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> TraceRange;"
...
STOP_BEFORE_BASE_DAMAGE = "BaseDamage"
...
TRACE_RANGE_WRONG_INIT_RE = re.compile(
    r"float\s+TraceRange\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float TraceRange = 32000.f;`.
    ...
    # leftover ADSFov / HipFov / MouseSensitivity,
    # leftover BaseDamage, leftover LoadoutSpec magazine
    # fields, leftover Harbor 40 / 80, or a bare TraceRange
    # without the 32000.f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if TRACE_RANGE_DECL_RE.search(compact) is None:
        return False
    if TRACE_RANGE_BARE_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"float\s+LookYawLimit\b", compact):
        return False
    if re.search(r"float\s+LookPitchMin\b", compact):
        return False
    if re.search(r"float\s+LookPitchMax\b", compact):
        return False
    if re.search(r"float\s+MouseSensitivity\b", compact):
        return False
    if re.search(r"float\s+ADSFov\b", compact):
        return False
    if re.search(r"float\s+HipFov\b", compact):
        return False
    if re.search(r"float\s+BaseDamage\b", compact):
        return False
    if re.search(r"float\s+TraceRange\s*=\s*" + "40" + r"\.f\b", compact):
        return False
    if re.search(r"float\s+TraceRange\s*=\s*" + "80" + r"\.f\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == SIBLING_FIRE_RATE:
            cursor = semi + 1
            continue
        if ident == STOP_BEFORE_BASE_DAMAGE:
            break
        if ident != "TraceRange":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_BASE_DAMAGE):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_BASE_DAMAGE}"
        )
...
class GunnerTraceRangeFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "TraceRange"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_BASE_DAMAGE), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_BASE_DAMAGE),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_LOOK_PITCH_MIN), body)
        self.assertFalse(has_identifier(section, SIBLING_LOOK_PITCH_MIN), section)
        self.assertTrue(has_identifier(body, SIBLING_LOOK_PITCH_MAX), body)
        self.assertFalse(has_identifier(section, SIBLING_LOOK_PITCH_MAX), section)
        self.assertTrue(has_identifier(body, SIBLING_MOUSE_SENSITIVITY), body)
        self.assertFalse(
            has_identifier(section, SIBLING_MOUSE_SENSITIVITY),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_HIP_FOV), body)
        self.assertFalse(has_identifier(section, SIBLING_HIP_FOV), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "TraceRange"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("TraceRange", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_trace_range_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float TraceRange = 32000.f;", compact_origin)
        self.assertNotIn("TraceRange = 160.f", compact_origin)
    ...
    def test_trace_range_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float TraceRange = 32000.f;")
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_YAW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_PITCH_MIN)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_PITCH_MAX)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MOUSE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ADS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HIP)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat TraceRanges = 32000.f;\n",
            "\tint32 TraceRange = 32000;\n",
            "\tbool TraceRange = true;\n",
            "\tfloat TraceRange = " + forty + ";\n",
            "\tfloat TraceRange = " + eighty + ";\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Combat"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_does_not_claim_sibling_base_damage_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "TraceRange"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_BASE_DAMAGE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_BASE_DAMAGE))
        self.assertTrue(has_identifier(leaked, SIBLING_FIRE_RATE))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, SIBLING_ADS_FOV))
        self.assertFalse(has_identifier(section, SIBLING_ADS_FOV))
        self.assertTrue(has_identifier(leaked, SIBLING_HIP_FOV))
        self.assertFalse(has_identifier(section, SIBLING_HIP_FOV))
        self.assertTrue(has_identifier(leaked, SIBLING_MOUSE_SENSITIVITY))
        self.assertFalse(has_identifier(section, SIBLING_MOUSE_SENSITIVITY))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "TraceRange"))
        self.assertTrue(has_identifier(section, "TraceRange"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_BASE_DAMAGE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_BASE_DAMAGE))
        self.assertTrue(has_identifier(leaked, SIBLING_FIRE_RATE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner TraceRange field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_trace_range_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
