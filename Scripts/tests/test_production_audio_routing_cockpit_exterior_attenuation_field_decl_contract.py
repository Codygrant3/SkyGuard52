...
# then `float CockpitExteriorAttenuation = 0.72f;`.
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Required float initializer `= 0.72f`.
# Required meta ClampMin="0.0" ClampMax="1.0".
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# float, initializer dropped or changed from 0.72f,
...
# locked initializer 0.72f is allowed and is NOT Harbor.
# Sibling CockpitLowPassHz lives in this struct and stays
# unclaimed (its 7200.f Clamp meta is different).
...
# RadioSubmix / CockpitSoundMix / CockpitLowPassHz.
# Use identifier bounds so leftover CockpitSubmix /
# ExteriorSubmix / leftover Entry Attenuation do not
# false-positive on the Cockpit / Exterior / Attenuation
# substrings.
# Analog leftover production-audio-routing-defaults then
# isolated CockpitExteriorAttenuation field decl.
# THIS IS NOT leftover CockpitSubmix.
# THIS IS NOT leftover ExteriorSubmix.
...
# FSkyguardProductionAudioEntry, retarget:
# parse FSkyguardProductionAudioRouting only, type is
# float, REQUIRED initializer `= 0.72f`, REQUIRED meta
# ClampMin="0.0" ClampMax="1.0", BlueprintReadWrite,
# no Category.
...
TARGET = "float CockpitExteriorAttenuation = 0.72f;"
TARGET_WRONG_BARE = "float CockpitExteriorAttenuation;"
TARGET_WRONG_EQ_NONE = "float CockpitExteriorAttenuation = nullptr;"
TARGET_WRONG_FALSE = "float CockpitExteriorAttenuation = false;"
TARGET_WRONG_TRUE = "float CockpitExteriorAttenuation = true;"
TARGET_WRONG_ZERO = "float CockpitExteriorAttenuation = 0.f;"
TARGET_WRONG_SOFT = (
    "TSoftObjectPtr<USoundSubmixBase> CockpitExteriorAttenuation;"
)
TARGET_WRONG_FNAME = "FName CockpitExteriorAttenuation;"
TARGET_WRONG_BOOL = "bool CockpitExteriorAttenuation;"
TARGET_WRONG_INT = "int32 CockpitExteriorAttenuation;"
TARGET_WRONG_FTEXT = "FText CockpitExteriorAttenuation;"
TARGET_WRONG_PTR = "USoundSubmixBase* CockpitExteriorAttenuation;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> CockpitExteriorAttenuation;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundMix> CockpitExteriorAttenuation;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> CockpitExteriorAttenuation;"
TARGET_WRONG_FSTRING = "FString CockpitExteriorAttenuation;"
TARGET_WRONG_LOW_PASS = "float CockpitExteriorAttenuation = 7200.f;"
...
SIBLING_COCKPIT_LOW_PASS_HZ = "CockpitLowPassHz"
...
FIELD_NAME = "CockpitExteriorAttenuation"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `= 0.f` / leftover 7200.f / leftover float Health /
    # Harbor forty / eighty / TSoftObjectPtr when
    # origin/main requires float + 0.72f. Do not accept
    ...
    if re.search(
        r"TSoftObjectPtr\s*<[^>]+>\s+CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<[^>]+>\s+CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*CockpitExteriorAttenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CockpitExteriorAttenuation\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CockpitExteriorAttenuation\s*=\s*0\.72f\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CockpitExteriorAttenuation\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CockpitExteriorAttenuation is missing from "
...
class ProductionAudioRoutingCockpitExteriorAttenuationFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_cockpit_exterior_attenuation_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            ...
            f"\tfloat {SIBLING_COCKPIT_LOW_PASS_HZ} = 7200.f;\n"
            "};\n"
        )
    ...
    def test_initializer_fails_closed_when_dropped_or_changed(self) -> None:
        ...
        self.assertNotIn("CockpitExteriorAttenuation = nullptr", compact_origin)
        self.assertNotIn("CockpitExteriorAttenuation = false", compact_origin)
        self.assertNotIn("CockpitExteriorAttenuation = true", compact_origin)
        self.assertNotIn("CockpitExteriorAttenuation = 0.f", compact_origin)
        self.assertNotIn("CockpitExteriorAttenuation = 160.f", compact_origin)
        self.assertNotIn("CockpitExteriorAttenuation = 7200.f", compact_origin)

    def test_cockpit_exterior_attenuation_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith(
                "float CockpitExteriorAttenuation"
            ),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 0.72f", LOCKED_DECL)
        ...
        self.assertNotIn("= 7200.f", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_LOW_PASS}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 CockpitExteriorAttenuation = 1;\n",
            "\tbool CockpitExteriorAttenuation;\n",
            "\tfloat CockpitExteriorAttenuation = " + forty + ";\n",
            "\tfloat CockpitExteriorAttenuation = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_dropped_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn("BlueprintReadWrite", origin)
        self.assertIn('ClampMin="0.0"', origin)
        self.assertIn('ClampMax="1.0"', origin)
        dropped_meta = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_does_not_contract_sibling_routing_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_COCKPIT_LOW_PASS_HZ),
            section,
        )
        self.assertTrue(has_identifier(section, FIELD_NAME), section)
        ...
        self.assertFalse(has_identifier(SIBLING_COCKPIT_LOW_PASS_HZ, FIELD_NAME))
        ...
        self.assertNotIn(SIBLING_COCKPIT_LOW_PASS_HZ, LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="0.0"', UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMax="1.0"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing CockpitExteriorAttenuation field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cockpit_exterior_attenuation_field_decl_contract.py"
        ))
