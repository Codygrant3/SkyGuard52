...
# INDEX_NONE or lock Engine construction
...
# RightActuator / DebrisLeftWing / DebrisEngine /
# DebrisRightWing and not leftover
# emergency-finish named surfaces
# (split tokens only). Keep RoadHunter Engine
# distinct from leftover Breakwater Engine
# (PR #545), leftover RadarGhost Engine
# (PR #537), leftover Pathfinder Engine, and
# sibling RoadHunter DebrisEngine
# (UStaticMeshComponent). Sibling fields
# TargetingCamera, LeftActuator, RightActuator,
# DebrisLeftWing, DebrisEngine, DebrisRightWing
# must not satisfy the primary lock. This is not a
...
# Engine;`); accept that form and other
...
# leftover isolated-test drafts #107–#548,
# leftover searchlight-track-runtime-defaults
...
# unlocked for THIS test's primary Engine lock. Leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRoadHunterBoss::Engine",
    "SkyguardRoadHunterBoss.cpp",
    "SetExposed",
...
class RoadHunterEngineFieldDeclContractTests(unittest.TestCase):
    def test_road_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ROAD_HUNTER_ENGINE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_missing_engine_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRoadHunterBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("VisibleAnywhere", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("BlueprintReadOnly", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("Category", ROAD_HUNTER_ENGINE_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* Engine;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> Engine;\n"
        )
        mesh = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ROAD_HUNTER_ENGINE_FIELD)
            self.assertIn("Engine", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_engine_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ROAD_HUNTER_ENGINE_FIELD),
            ROAD_HUNTER_ENGINE_FIELD,
        )
        self.assertTrue(has_declaration(section, ROAD_HUNTER_ENGINE_FIELD))
        self.assertEqual(declaration_count(section, ROAD_HUNTER_ENGINE_FIELD), 1)
        self.assertTrue(
            ROAD_HUNTER_ENGINE_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            ROAD_HUNTER_ENGINE_FIELD,
        )
        self.assertTrue(ROAD_HUNTER_ENGINE_FIELD.endswith(";"), ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("TargetingCamera", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("LeftActuator", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("RightActuator", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisLeftWing", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisEngine", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisRightWing", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisCoolingDoor", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SignatureModulator", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("RadarReceiver", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("CoolingDoor", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("PortLatch", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("NoseCamera", ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            ROAD_HUNTER_ENGINE_FIELD,
        )
        self.assertIn("TObjectPtr<", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("TArray<", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("=", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("INDEX_NONE", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("UFUNCTION", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("{", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("}", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("return ", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("CommandAntenna", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ControlLinkage", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("NoseCamera", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisCenter", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisTail", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisNose", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisSpine", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("AdvanceEncounter", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ResetEncounterState", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("IsRouteStateSafe", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetRouteProgress", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("RadarNode", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("IssueClimbCommand", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetFinaleStage", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("IsWreckDiverted", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetGunnerMount", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("FaceWorldLocation", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("PortGuidanceArray", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisArmorPort", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetSearchlightTracked", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetContactIdentified", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetLightningExposed", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("HullCollider", ROAD_HUNTER_ENGINE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {ROAD_HUNTER_ENGINE_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, ROAD_HUNTER_ENGINE_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{ROAD_HUNTER_ENGINE_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ROAD_HUNTER_ENGINE_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ROAD_HUNTER_ENGINE_FIELD),
            section,
    ...
    def test_assigned_engine_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "Engine = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ROAD_HUNTER_ENGINE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("Engine", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ROAD_HUNTER_ENGINE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ROAD_HUNTER_ENGINE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ROAD_HUNTER_ENGINE_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, ROAD_HUNTER_ENGINE_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ROAD_HUNTER_ENGINE_FIELD.startswith("UFUNCTION"),
            ROAD_HUNTER_ENGINE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ROAD_HUNTER_ENGINE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ROAD_HUNTER_ENGINE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_engine_cpp_body(self) -> None:
        locked_only = f"{ROAD_HUNTER_ENGINE_FIELD}\n"
        self.assertNotIn("{", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("}", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("return ", ROAD_HUNTER_ENGINE_FIELD)
        ...
        self.assertNotIn("SkyguardRoadHunterBoss.cpp", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SkyguardRoadHunterBoss.cpp", locked_only)
        self.assertNotIn("return false", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("return true", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetExposed", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            ROAD_HUNTER_ENGINE_FIELD,
        )
        self.assertNotIn("SetBossPhase", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", ROAD_HUNTER_ENGINE_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{ROAD_HUNTER_ENGINE_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("PortDischargeBoom", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("StarboardDischargeBoom", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("EngineIntake", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisPortPanel", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisStarboardPanel", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisIntakePanel", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            ROAD_HUNTER_ENGINE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("EncounterController", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ControlServo", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DispenserCenter", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("MaxReleasesPerBay", ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetContactIdentified", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("OpenOrbitExposure", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("IsContactIdentified", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), ROAD_HUNTER_ENGINE_FIELD)

    def test_debris_cooling_door_does_not_satisfy_engine(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, ROAD_HUNTER_ENGINE_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("UStaticMeshComponent", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(
            "USkyguardBossWeakPointComponent",
            DEBRIS_COOLING_DOOR_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DispenserStarboard", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetManeuver", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), ROAD_HUNTER_ENGINE_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetSearchlightTracked", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        self.assertNotIn("CoolingDoor", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SignatureModulator", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SetLightningExposed", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("IsLightningExposed", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardGunner", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SkyguardRadarNode", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("MaxIntegrity", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("CurrentIntegrity", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            ROAD_HUNTER_ENGINE_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ROAD_HUNTER_ENGINE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ROAD_HUNTER_ENGINE_FIELD),
            ROAD_HUNTER_ENGINE_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss::Engine", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "road hunter Engine field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{ROAD_HUNTER_ENGINE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ROAD_HUNTER_ENGINE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ROAD_HUNTER_ENGINE_FIELD.lower(),
                "road hunter Engine contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ROAD_HUNTER_ENGINE_FIELD)

    def test_contract_is_engine_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(ROAD_HUNTER_ENGINE_FIELD.endswith(";"))
        self.assertNotIn("=", ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("PortLatch", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("StarboardLatch", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DecoyPods", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ElevatorLinkage", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_road_hunter_engine_is_not_breakwater_engine(self) -> None:
        ...
        self.assertNotIn("ASkyguardBreakwaterBoss", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss", ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("DebrisEngine", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("TargetingCamera", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("OpticalTracker", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("WeaponServo", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("CountermeasurePod", ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", ROAD_HUNTER_ENGINE_FIELD)


    def test_road_hunter_engine_is_not_radar_ghost_engine(self) -> None:
        locked_only = f"{ROAD_HUNTER_ENGINE_FIELD}\n"
        self.assertEqual(ROAD_HUNTER_ENGINE_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardRoadHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardRadarGhostBoss")
    ...
    def test_road_hunter_engine_is_not_pathfinder_engine(self) -> None:
        locked_only = f"{ROAD_HUNTER_ENGINE_FIELD}\n"
        self.assertEqual(
            collapsed(ROAD_HUNTER_ENGINE_FIELD),
    ...
    def test_debris_engine_does_not_satisfy_engine(self) -> None:
    ...
    def test_targeting_camera_does_not_satisfy_engine(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, ROAD_HUNTER_ENGINE_FIELD))
        self.assertNotEqual(TARGETING_CAMERA_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotIn("TargetingCamera", ROAD_HUNTER_ENGINE_FIELD)
        self.assertIn("Engine", ROAD_HUNTER_ENGINE_FIELD)

    def test_left_and_right_actuators_do_not_satisfy_engine(self) -> None:
        for region, name in (
            (f"\t{LEFT_ACTUATOR_FIELD}\n", "LeftActuator"),
            (f"\t{RIGHT_ACTUATOR_FIELD}\n", "RightActuator"),
        ):
