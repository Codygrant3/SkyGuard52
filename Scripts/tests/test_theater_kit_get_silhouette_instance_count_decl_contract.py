...
# then declaration-only `int32 GetSilhouetteInstanceCount() const;`
...
# drop to BlueprintCallable, if const is missing, or
# if the return type is not int32.
...
# Do NOT contract leftover SilhouetteInstances UPROPERTY #1320.
...
# THIS IS NOT leftover SilhouetteInstances field #1320.
...
# retarget: this is UFUNCTION BlueprintPure
...
TARGET = "int32 GetSilhouetteInstanceCount() const;"
TARGET_WRONG_EQ_NONE = "int32 GetSilhouetteInstanceCount() = NAME_None;"
TARGET_WRONG_FALSE = "int32 GetSilhouetteInstanceCount() = false;"
TARGET_WRONG_TRUE = "int32 GetSilhouetteInstanceCount() = true;"
TARGET_WRONG_ZERO = "int32 GetSilhouetteInstanceCount() = 0.f;"
TARGET_WRONG_FLOAT = "float GetSilhouetteInstanceCount() const;"
TARGET_WRONG_FNAME = "FName GetSilhouetteInstanceCount() const;"
TARGET_WRONG_FLINEAR = "FLinearColor GetSilhouetteInstanceCount() const;"
TARGET_WRONG_NO_CONST = "int32 GetSilhouetteInstanceCount();"
TARGET_WRONG_BODY = (
    "int32 GetSilhouetteInstanceCount() const "
...
TARGET_WRONG_CALLABLE = (
    'UFUNCTION(BlueprintCallable, '
    'Category="Skyguard|Theater")'
)
LOCKED_DECL = TARGET
UFUNCTION_PURE_THEATER = (
    'UFUNCTION(BlueprintPure, '
    'Category="Skyguard|Theater")'
)
UPROPERTY_VISIBLE_READONLY = UFUNCTION_PURE_THEATER
...
GET_SILHOUETTE_INSTANCE_COUNT_RE = re.compile(
    r"int32\s+GetSilhouetteInstanceCount\s*\(\s*\)\s*const\s*;"
)
GET_SILHOUETTE_INSTANCE_COUNT_NO_CONST_RE = re.compile(
    r"int32\s+GetSilhouetteInstanceCount\s*\(\s*\)\s*(?!const)"
)
GET_SILHOUETTE_INSTANCE_COUNT_WRONG_TYPE_RE = re.compile(
    r"(?:FName|float|FLinearColor|bool|void)\s+GetSilhouetteInstanceCount\b"
)
GET_SILHOUETTE_INSTANCE_COUNT_BODY_RE = re.compile(
    r"int32\s+GetSilhouetteInstanceCount\s*\(\s*\)\s*const"
    r"\s*\{"
)
INVENTED_UPROPERTY = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # BlueprintCallable wraps, or leftover float Health.
    # Do not accept sibling GetBuildingTint /
    # GetRoadInstanceCount UFUNCTIONs.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GET_SILHOUETTE_INSTANCE_COUNT_WRONG_TYPE_RE.search(compact):
        return False
    if GET_SILHOUETTE_INSTANCE_COUNT_BODY_RE.search(compact):
        return False
    if GET_SILHOUETTE_INSTANCE_COUNT_RE.search(compact) is None:
        return False
    return True
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetSilhouetteInstanceCount\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for int32 GetSilhouetteInstanceCount is missing "
...
class TheaterKitGetSilhouetteInstanceCountDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetSilhouetteInstanceCount"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetSilhouetteInstanceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_silhouette_instance_count_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
            f"\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            f"{SIBLING_SILHOUETTE_INSTANCES};\n"
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_PURE_THEATER}\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn(UFUNCTION_PURE_THEATER, section)
        self.assertIn("BlueprintPure", section)
        self.assertIn('Category="Skyguard|Theater"', section)
        self.assertIn("BlueprintPure", UFUNCTION_PURE_THEATER)
        self.assertIn("Category", UFUNCTION_PURE_THEATER)
        self.assertIn('Category="Skyguard|Theater"', UFUNCTION_PURE_THEATER)
        self.assertNotIn("BlueprintCallable", UFUNCTION_PURE_THEATER)
        self.assertNotIn("EditAnywhere", UFUNCTION_PURE_THEATER)
        self.assertNotIn("BlueprintReadWrite", UFUNCTION_PURE_THEATER)
        ...
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertIsNone(GET_SILHOUETTE_INSTANCE_COUNT_BODY_RE.search(collapsed(section)))
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertIsNotNone(GET_SILHOUETTE_INSTANCE_COUNT_RE.search(compact_origin))
        self.assertIsNone(GET_SILHOUETTE_INSTANCE_COUNT_BODY_RE.search(compact_origin))
        self.assertNotIn("FName GetSilhouetteInstanceCount", compact_origin)
        self.assertNotIn("float GetSilhouetteInstanceCount", compact_origin)
        self.assertNotIn("FLinearColor GetSilhouetteInstanceCount", compact_origin)

    def test_get_silhouette_instance_count_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GetSilhouetteInstanceCount"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            f"{SIBLING_SILHOUETTE_INSTANCES};\n",
            "\tint32 GetSilhouetteInstanceCounts() const;\n",
            ...
            "\tbool GetSilhouetteInstanceCount() const;\n",
            "\tFName GetSilhouetteInstanceCount() const;\n",
            f"\t{TARGET_WRONG_BODY}\n",
            f"\t{TARGET_WRONG_SPEC_FIELD}\n",
            "\tfloat GetSilhouetteInstanceCount() const { return "
            + forty
            + "; }\n",
            "\tfloat GetSilhouetteInstanceCount() const { return "
            + eighty
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tint32\n\tGetSilhouetteInstanceCount() const;\n",
            "\tint32   GetSilhouetteInstanceCount() const;\n",
            "\tint32\tGetSilhouetteInstanceCount() const;\n",
            "\tint32 GetSilhouetteInstanceCount(\n\t) const;\n",
            "\tint32 GetSilhouetteInstanceCount()  const;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UFUNCTION_PURE_THEATER}\n\t{LOCKED_DECL}\n",
            f"\t{UFUNCTION_PURE_THEATER} {LOCKED_DECL}\n",
            "\tUFUNCTION(BlueprintPure, "
            'Category="Skyguard|Theater")\n'
            f"\t{LOCKED_DECL}\n",
            "\tUFUNCTION(\n\t\tBlueprintPure, "
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        ...
        self.assertNotIn("EditAnywhere", UFUNCTION_PURE_THEATER)
        self.assertNotIn("BlueprintReadWrite", UFUNCTION_PURE_THEATER)
        self.assertNotIn("VisibleAnywhere", UFUNCTION_PURE_THEATER)
        self.assertNotIn("MultiLine", UFUNCTION_PURE_THEATER)
        self.assertNotIn("ClampMin", UFUNCTION_PURE_THEATER)
        self.assertNotIn("ClampMax", UFUNCTION_PURE_THEATER)
        for invented in INVENTED_FIELD_META:
            self.assertNotIn(invented, LOCKED_DECL)
            self.assertNotIn(invented, UFUNCTION_PURE_THEATER)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetSilhouetteInstanceCount decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "get_silhouette_instance_count_decl_contract.py"
        ))
