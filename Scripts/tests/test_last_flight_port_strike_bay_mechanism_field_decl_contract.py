...
# INDEX_NONE or lock PortStrikeBayMechanism
...
# PortStrikeBayMechanism;`); accept that form and
...
# Harbor-adjacent.
PORT_STRIKE_BAY_MECHANISM_FIELD = (
...
# Python PortStrikeBayMechanism field declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::PortStrikeBayMechanism",
    "SkyguardLastFlightBoss.cpp",
    "SetExposed",
...
class LastFlightPortStrikeBayMechanismFieldDeclContractTests(unittest.TestCase):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, PORT_STRIKE_BAY_MECHANISM_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{PORT_STRIKE_BAY_MECHANISM_FIELD}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("PortStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_missing_port_strike_bay_mechanism_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLastFlightBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("PortStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("PortStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("VisibleAnywhere", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("BlueprintReadOnly", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("Category", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_PORT)
        self.assertNotIn("BlueprintCallable", UPROPERTY_PORT)
        self.assertIn("Skyguard|Mission10|Boss", UPROPERTY_PORT)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("PortStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* PortStrikeBayMechanism;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> PortStrikeBayMechanism;\n"
        )
        weak = (
        ...
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "PortStrikeBayMechanism = nullptr;\n"
        )
        wrong_name = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardGuidanceArray;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, PORT_STRIKE_BAY_MECHANISM_FIELD)
            self.assertIn("PortStrikeBayMechanism", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_port_strike_bay_mechanism_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, PORT_STRIKE_BAY_MECHANISM_FIELD),
            1,
        )
        self.assertTrue(
            PORT_STRIKE_BAY_MECHANISM_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertTrue(
            PORT_STRIKE_BAY_MECHANISM_FIELD.endswith(";"),
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertIn("PortStrikeBayMechanism", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertIn("TObjectPtr<", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("TArray<", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("=", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("INDEX_NONE", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("UFUNCTION", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("{", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("}", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("return ", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("IssueClimbCommand", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        ...
        self.assertNotIn("GetCivilianSeparationMeters", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetFinaleStage", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("IsClimbCommandIssued", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("IsWreckDiverted", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetGunnerMount", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("FaceWorldLocation", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("StarboardGuidanceArray", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CommandCore", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("SetLightningExposed", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("HullCollider", PORT_STRIKE_BAY_MECHANISM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {PORT_STRIKE_BAY_MECHANISM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{PORT_STRIKE_BAY_MECHANISM_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, PORT_STRIKE_BAY_MECHANISM_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, PORT_STRIKE_BAY_MECHANISM_FIELD),
    ...
    def test_assigned_port_strike_bay_mechanism_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "PortStrikeBayMechanism = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("PortStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, PORT_STRIKE_BAY_MECHANISM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            self.assertIn("PortStrikeBayMechanism", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, PORT_STRIKE_BAY_MECHANISM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{PORT_STRIKE_BAY_MECHANISM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, PORT_STRIKE_BAY_MECHANISM_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, PORT_STRIKE_BAY_MECHANISM_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            PORT_STRIKE_BAY_MECHANISM_FIELD.startswith("UFUNCTION"),
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, PORT_STRIKE_BAY_MECHANISM_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", PORT_STRIKE_BAY_MECHANISM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_port_strike_bay_mechanism_cpp_body(
        self,
    ) -> None:
        locked_only = f"{PORT_STRIKE_BAY_MECHANISM_FIELD}\n"
        self.assertNotIn("{", PORT_STRIKE_BAY_MECHANISM_FIELD)
        ...
        self.assertNotIn("return false", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("return true", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetExposed", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("SetBossPhase", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_apply_strike(), PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), PORT_STRIKE_BAY_MECHANISM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, PORT_STRIKE_BAY_MECHANISM_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_last_flight_fields(self) -> None:
        ...
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("PortGuidanceArray", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("StarboardGuidanceArray", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CommandCore", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("PortEngine", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("OpenOrbitExposure", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetLightningExposed", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardGunner", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SkyguardRadarNode", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("MaxIntegrity", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CurrentIntegrity", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, PORT_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{PORT_STRIKE_BAY_MECHANISM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, PORT_STRIKE_BAY_MECHANISM_FIELD),
            PORT_STRIKE_BAY_MECHANISM_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight PortStrikeBayMechanism field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{PORT_STRIKE_BAY_MECHANISM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, PORT_STRIKE_BAY_MECHANISM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                PORT_STRIKE_BAY_MECHANISM_FIELD.lower(),
                "last flight PortStrikeBayMechanism contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, PORT_STRIKE_BAY_MECHANISM_FIELD)

    def test_contract_is_port_strike_bay_mechanism_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(PORT_STRIKE_BAY_MECHANISM_FIELD.endswith(";"))
        self.assertNotIn("=", PORT_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
