...
# INDEX_NONE or lock GetSearchSector
# construction in the .cpp. This is leftover-safe
# Mission07 Search GetSearchSector METHOD on
# ASkyguardMission07IntegrationDirector. It is
# NOT leftover Mission07 GetReadiness, NOT leftover
...
# an isolated Python GetSearchSector
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission07IntegrationDirector::GetSearchSector",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission07GetSearchSectorDeclContractTests(unittest.TestCase):
    def test_mission07_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_SEARCH_SECTOR),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_SEARCH_SECTOR)
        self.assertIn("GetSearchSector", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_SEARCH_SECTOR)
    ...
    def test_missing_get_search_sector_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission07IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_SEARCH_SECTOR)
        self.assertIn("GetSearchSector", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_SEARCH_SECTOR)
        self.assertIn("GetSearchSector", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_SEARCH_SECTOR)
        self.assertNotIn("BlueprintPure", GET_SEARCH_SECTOR)
        self.assertNotIn("Category", GET_SEARCH_SECTOR)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_SEARCH_SECTOR)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_SEARCH_SECTOR)
        self.assertIn("GetSearchSector", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetSearchSector() const;\n"
        )
        as_const = (
            "\tESkyguardSearchSector GetSearchSector();\n"
        )
        with_arg = (
            "\tESkyguardSearchSector GetSearchSector(int32 Amount) const;\n"
        )
        renamed = (
            "\tESkyguardSearchSector GetSearchState() const;\n"
        )
        short_name = "\tbool GetSearchSector() const;\n"
        leftover_m06_type = (
            "\tESkyguardMission06SearchSector GetSearchSector() const;\n"
        )
        leftover_m05_type = (
            "\tESkyguardMission05SearchSector GetSearchSector() const;\n"
        )
        leftover_m04_type = (
            "\tESkyguardMission04SearchSector GetSearchSector() const;\n"
        )
        leftover_m01_type = (
            "\tESkyguardMission01SearchSector GetSearchSector() const;\n"
        )
        leftover_env_type = (
            "\tESkyguardMission01EnvironmentSearchSector GetSearchSector() const;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_SEARCH_SECTOR)
            self.assertIn("GetSearchSector", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_search_sector_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_SEARCH_SECTOR),
            GET_SEARCH_SECTOR,
        )
        self.assertTrue(has_declaration(section, GET_SEARCH_SECTOR))
        self.assertEqual(
            declaration_count(section, GET_SEARCH_SECTOR),
            1,
        )
        self.assertTrue(
            GET_SEARCH_SECTOR.startswith("ESkyguardSearchSector"),
            GET_SEARCH_SECTOR,
        )
        self.assertTrue(
            GET_SEARCH_SECTOR.startswith("ESkyguardSearchSector"),
        ...
        self.assertIn("ESkyguardSearchSector", GET_SEARCH_SECTOR)
        self.assertIn("GetSearchSector", GET_SEARCH_SECTOR)
        self.assertNotIn(";", GET_SEARCH_SECTOR)
        self.assertIn(" const", GET_SEARCH_SECTOR)
        self.assertIn("()", GET_SEARCH_SECTOR)
        self.assertNotIn("TArray<", GET_SEARCH_SECTOR)
        self.assertNotIn("TObjectPtr", GET_SEARCH_SECTOR)
        self.assertNotIn("TSoftObjectPtr", GET_SEARCH_SECTOR)
        self.assertNotIn("INDEX_NONE", GET_SEARCH_SECTOR)
        self.assertNotIn("UFUNCTION", GET_SEARCH_SECTOR)
        self.assertNotIn("{", GET_SEARCH_SECTOR)
        self.assertNotIn("}", GET_SEARCH_SECTOR)
        self.assertNotIn("return ", GET_SEARCH_SECTOR)
        self.assertIn(" const", GET_SEARCH_SECTOR)
        self.assertNotIn("Root", GET_SEARCH_SECTOR)
        self.assertNotIn("Briefing", GET_SEARCH_SECTOR)
        self.assertNotIn("AudioDirector", GET_SEARCH_SECTOR)
        self.assertNotIn("RadioChatter", GET_SEARCH_SECTOR)
        self.assertNotIn("SortiePresentation", GET_SEARCH_SECTOR)
        self.assertNotIn("CampaignDefinition", GET_SEARCH_SECTOR)
        self.assertNotIn("MissionDefinition", GET_SEARCH_SECTOR)
        self.assertNotIn("Readiness;", GET_SEARCH_SECTOR)
        self.assertNotIn("bAutoInitialize", GET_SEARCH_SECTOR)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_SEARCH_SECTOR)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_SEARCH_SECTOR)
        self.assertNotIn("PathfinderSpawnLocation", GET_SEARCH_SECTOR)
        self.assertNotIn("GetAircraft", GET_SEARCH_SECTOR)
        self.assertNotIn("BindRuntimeActors", GET_SEARCH_SECTOR)
        self.assertNotIn("HandleDroneCityImpact", GET_SEARCH_SECTOR)
        self.assertNotIn("InitializePlayableMission", GET_SEARCH_SECTOR)
        self.assertNotIn("StartNextWave", GET_SEARCH_SECTOR)
        self.assertNotIn("AdvanceConvoyByDistance", GET_SEARCH_SECTOR)
        self.assertNotIn("NotifyConvoyDamage", GET_SEARCH_SECTOR)
        self.assertNotIn("GetWaveState", GET_SEARCH_SECTOR)
        self.assertNotIn("GetConvoyRouteState", GET_SEARCH_SECTOR)
        self.assertNotIn("GetDayBeatKit", GET_SEARCH_SECTOR)
        self.assertNotIn("GetNightBeatKit", GET_SEARCH_SECTOR)
        self.assertNotIn("StartSearchlightWindow", GET_SEARCH_SECTOR)
        self.assertNotIn("AdvanceSearchlightTrack", GET_SEARCH_SECTOR)
        self.assertNotIn("NotifySubstationDamage", GET_SEARCH_SECTOR)
        self.assertNotIn("GetRemainingThreatsInWave", GET_SEARCH_SECTOR)
        self.assertNotIn("GetSearchlightRuntime", GET_SEARCH_SECTOR)
        self.assertNotIn("GetSubstationIntegrity", GET_SEARCH_SECTOR)
        self.assertNotIn("StartPayloadWindow", GET_SEARCH_SECTOR)
        self.assertNotIn("AdvancePayloadWindow", GET_SEARCH_SECTOR)
        self.assertNotIn("TryJamActivePayload", GET_SEARCH_SECTOR)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_SEARCH_SECTOR)
        self.assertNotIn("GetPayloadWindow", GET_SEARCH_SECTOR)
        self.assertNotIn("GetTargetRuntime", GET_SEARCH_SECTOR)
        self.assertNotIn("GetSurvivingTargetCount", GET_SEARCH_SECTOR)
        self.assertNotIn("GetDayBeatKind", GET_SEARCH_SECTOR)
        self.assertNotIn("GetDayBeatIndex", GET_SEARCH_SECTOR)
        self.assertNotIn("TickDayBeatKit", GET_SEARCH_SECTOR)
        self.assertNotIn("HandleBossPhaseChanged", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", GET_SEARCH_SECTOR)
        self.assertNotIn("ESkyguardMission06WaveState", GET_SEARCH_SECTOR)
        self.assertNotIn("ESkyguardMission07WaveState", GET_SEARCH_SECTOR)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", GET_SEARCH_SECTOR)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", GET_SEARCH_SECTOR)
        self.assertNotIn("FSkyguardMission07IntegrationReadiness", GET_SEARCH_SECTOR)
        self.assertNotIn("FSkyguardSearchTrackRuntime", GET_SEARCH_SECTOR)
        self.assertNotIn("ClassifyFalseTrack", GET_SEARCH_SECTOR)
        self.assertNotIn("ConfirmRadarGhostIdentification", GET_SEARCH_SECTOR)
        self.assertNotIn("NotifyProtectedTargetDamage", GET_SEARCH_SECTOR)
        self.assertNotIn("AdvanceReinforcementTimer", GET_SEARCH_SECTOR)
        self.assertNotIn("GetReadiness", GET_SEARCH_SECTOR)
        self.assertNotIn("GetClassifiedFalseTrackCount", GET_SEARCH_SECTOR)
        self.assertNotIn("IsHostileContactConfirmed", GET_SEARCH_SECTOR)
        self.assertNotIn("GetProtectedTarget", GET_SEARCH_SECTOR)
        self.assertNotIn("GetReinforcementTimeRemaining", GET_SEARCH_SECTOR)
        self.assertNotIn("GetNightBeatKind", GET_SEARCH_SECTOR)
        self.assertNotIn("GetNightBeatIndex", GET_SEARCH_SECTOR)
        self.assertNotIn("TickNightBeatKit", GET_SEARCH_SECTOR)
        self.assertNotIn("ESkyguardAirfieldTarget", GET_SEARCH_SECTOR)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_SEARCH_SECTOR)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_SEARCH_SECTOR)
        self.assertNotIn("RoadHunterSpawnLocation", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_SEARCH_SECTOR)
        self.assertNotIn("ConfigureMissionDefinition", GET_SEARCH_SECTOR)
        self.assertNotIn("NotifyObjectiveProgress", GET_SEARCH_SECTOR)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_SEARCH_SECTOR)
        self.assertNotIn("SynchronizeRuntimeState", GET_SEARCH_SECTOR)
        self.assertNotIn("NotifyThreatDestroyed", GET_SEARCH_SECTOR)
        self.assertNotIn("IsCorePlayableReady", GET_SEARCH_SECTOR)
        self.assertNotIn("GetObjectiveRuntime", GET_SEARCH_SECTOR)
        self.assertNotIn("GetGunner", GET_SEARCH_SECTOR)
        self.assertNotIn("GetPathfinder", GET_SEARCH_SECTOR)
        self.assertNotIn("GetMissionId", GET_SEARCH_SECTOR)
        self.assertNotIn("ValidateMissionContract", GET_SEARCH_SECTOR)
        self.assertNotIn("ConfigureFromMission", GET_SEARCH_SECTOR)
        self.assertNotIn("AdvanceBriefing", GET_SEARCH_SECTOR)
        self.assertNotIn("SetAssetsReady", GET_SEARCH_SECTOR)
        self.assertNotIn("AcknowledgeAndLaunch", GET_SEARCH_SECTOR)
        self.assertNotIn("CanLaunch", GET_SEARCH_SECTOR)
        self.assertNotIn("GetElapsedSeconds", GET_SEARCH_SECTOR)
        self.assertNotIn("GetBriefingState", GET_SEARCH_SECTOR)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_SEARCH_SECTOR)
        self.assertNotIn("GetBriefingText", GET_SEARCH_SECTOR)
        self.assertNotIn("GetRadioChatter", GET_SEARCH_SECTOR)
        self.assertNotIn("GetPresentation", GET_SEARCH_SECTOR)
        self.assertNotIn("GetMissionTitle", GET_SEARCH_SECTOR)
        self.assertNotIn("AcknowledgeBriefing", GET_SEARCH_SECTOR)
        self.assertNotIn("LaunchSortie", GET_SEARCH_SECTOR)
        self.assertNotIn("HullCollider", GET_SEARCH_SECTOR)
        self.assertNotIn("OpticalTracker", GET_SEARCH_SECTOR)
        self.assertNotIn("WeaponServo", GET_SEARCH_SECTOR)
        self.assertNotIn("CountermeasurePod", GET_SEARCH_SECTOR)
        self.assertNotIn("MinHeightFromOriginCm", GET_SEARCH_SECTOR)
        self.assertNotIn("RadarNode", GET_SEARCH_SECTOR)
        self.assertNotIn("ESkyguardMission02WaveState", GET_SEARCH_SECTOR)
        self.assertNotIn("HarborIndustrial", GET_SEARCH_SECTOR)
        self.assertNotIn("MaxIntegrity", GET_SEARCH_SECTOR)
        self.assertNotIn("CurrentIntegrity", GET_SEARCH_SECTOR)
        self.assertNotIn("FillAndFinalize", GET_SEARCH_SECTOR)
        self.assertNotIn("FillAndFail", GET_SEARCH_SECTOR)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_SEARCH_SECTOR)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SEARCH} {GET_SEARCH_SECTOR}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_SEARCH_SECTOR}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_SEARCH_SECTOR))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_SEARCH_SECTOR),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_SEARCH_SECTOR}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_SEARCH_SECTOR)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_SEARCH_SECTOR)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_search_sector_cpp_body(self) -> None:
        locked_only = f"{GET_SEARCH_SECTOR}\n"
        self.assertNotIn("{", GET_SEARCH_SECTOR)
        self.assertNotIn("}", GET_SEARCH_SECTOR)
        self.assertNotIn("return ", GET_SEARCH_SECTOR)
        ...
        self.assertNotIn("return false", GET_SEARCH_SECTOR)
        self.assertNotIn("return true", GET_SEARCH_SECTOR)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_SEARCH_SECTOR}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_SEARCH_SECTOR)
        self.assertIn("GetSearchSector", GET_SEARCH_SECTOR)
        self.assertTrue(GET_SEARCH_SECTOR.startswith("ESkyguardSearchSector"))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_SEARCH_SECTOR)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_SEARCH_SECTOR)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_SEARCH_SECTOR)
        self.assertNotIn("CurrentIntegrity", GET_SEARCH_SECTOR)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_SEARCH_SECTOR)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_SEARCH_SECTOR)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_SEARCH_SECTOR}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_SEARCH_SECTOR),
            GET_SEARCH_SECTOR,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_SEARCH_SECTOR)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_SEARCH_SECTOR)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", GET_SEARCH_SECTOR)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 GetSearchSector method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_SEARCH_SECTOR}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_SEARCH_SECTOR.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_SEARCH_SECTOR.lower(),
                "mission07 GetSearchSector contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_SEARCH_SECTOR)

    def test_contract_is_get_search_sector_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_search_sector(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_SEARCH_SECTOR)
            )
            self.assertNotEqual(GET_SEARCH_SECTOR, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetSearchSector", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_SEARCH_SECTOR))
        locked_only = f"{GET_SEARCH_SECTOR}\n"
    ...
    def test_leftover_environment_get_search_sector_does_not_satisfy(self) -> None:
        leftover = (
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission07_get_search_sector"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_get_search_sector"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_mission01_get_search_sector_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission01IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission03_get_search_sector_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission03IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission04_get_search_sector_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission04IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission05_get_search_sector_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission05IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission06_get_search_sector_does_not_satisfy(self) -> None:
        leftover = (
