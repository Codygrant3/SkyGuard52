...
# then `float CriticalTurnSeconds = 5.5f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Route`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"1.0"`. Default IS `= 5.5f`.
# Clone wrap ClampMin WAS `"100.0"` or `"0.0"`. THIS slot
# ClampMin IS `"1.0"` (NO spaces around =). Fail-closed if wrap still
# has ClampMin="100.0" or ClampMin="0.0". 5.5f is NOT Harbor 40 or 80.
...
# if ClampMin is missing, if ClampMin is still `"100.0"` or `"0.0"`, if the default
# is not `= 5.5f`, if Category is
...
# LockWindowClimbCm = 650.f / LockWindowClimbSeconds = 2.4f /
# CriticalTurnRadiusCm = 1500.f as this slot.
...
# Category="Skyguard|Boss|Encounter|Route", meta=(ClampMin="1.0")
# (NO spaces around =). Fail-closed if wrap still has
# ClampMin="100.0" or ClampMin="0.0" from clone LockWindowSpeed / Climb.
# LOCKED_DECL is `float CriticalTurnSeconds = 5.5f;`
...
TARGET = "float CriticalTurnSeconds = 5.5f;"
TARGET_WRONG_BARE = "float CriticalTurnSeconds;"
TARGET_WRONG_INIT = "float CriticalTurnSeconds = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float CriticalTurnSeconds = true;"
TARGET_WRONG_ZERO = "float CriticalTurnSeconds = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CriticalTurnSeconds;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CriticalTurnSeconds;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CriticalTurnSeconds;"
...
STOP_BEFORE_SKYLINE_STYLE = "MaxLateralOffsetCm"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_critical_turn_seconds"
...
CRITICAL_TURN_SECONDS_DECL_RE = re.compile(
    r"float\s+CriticalTurnSeconds\s*=\s*5\.5f\s*;"
)
CRITICAL_TURN_SECONDS_BARE_RE = re.compile(
    r"float\s+CriticalTurnSeconds\s*;"
)
...
LOCK_WINDOW_CLIMB_CM_LEFTOVER_DECL_RE = re.compile(
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float CriticalTurnSeconds = 5.5f;`.
    ...
    # leftover analog ClampMin="100.0" / ClampMin="0.0",
    ...
    # CriticalTurnSeconds without the 5.5f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CRITICAL_TURN_SECONDS_DECL_RE.search(compact) is None:
        return False
    if CRITICAL_TURN_SECONDS_BARE_RE.search(compact):
        return False
    ...
    if TURN_RADIUS_LEFTOVER_DECL_RE.search(compact):
        return False
    if LOCK_WINDOW_CLIMB_CM_LEFTOVER_DECL_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CriticalTurnSeconds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CriticalTurnSeconds "
...
class PathfinderEncounterCriticalTurnSecondsFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalTurnSeconds"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CriticalTurnSeconds"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CriticalTurnSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_critical_turn_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float CriticalTurnSeconds;",
            compact_origin.split("CriticalTurnSeconds = 5.5f;")[0]
            + compact_origin.split("CriticalTurnSeconds = 5.5f;")[-1]
            if "CriticalTurnSeconds = 5.5f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_critical_turn_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CriticalTurnSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("5.5f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat CriticalTurnSecondss = 5.5f;\n",
            "\tint32 CriticalTurnSeconds = 320;\n",
            "\tFName CriticalTurnSeconds = NAME_None;\n",
            "\tfloat CriticalTurnSeconds = " + forty + ";\n",
            "\tfloat CriticalTurnSeconds = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalTurnSeconds"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CriticalTurnSeconds"))
        self.assertTrue(has_identifier(section, "CriticalTurnSeconds"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter critical turn seconds field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_critical_turn_seconds_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
