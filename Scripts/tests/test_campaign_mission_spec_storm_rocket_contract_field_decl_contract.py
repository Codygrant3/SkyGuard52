# THIS IS leftover-safe FSkyguardCampaignMissionSpec bStormRocketContract.
# origin/main form: BARE plain C++ field `bool bStormRocketContract = false;`
# with initializer false. NOT a UPROPERTY wrap.
# FSkyguardCampaignMissionSpec is a plain C++ roster struct.
# Comment on origin/main: Storm missions start on Hydra for
# barge/armor clusters. No new station. Lock the FIELD, not
# leftover StormRainBeatKit bHydraForClusters.
...
# Fail-closed if identifier is leftover Storm kit
# bHydraForClusters.
...
# invent Harbor 40/80.
# THIS SLOT IS NOT leftover StormRainBeatKit
# bHydraForClusters leftover-safe #1494 SHA
# 8d6c16a977e3430784ff982fe7f63659410ef170 (bool on leftover
# StormRain kit). Fail-closed if parse window is
# FSkyguardStormRainBeatKit or identifier is
# bHydraForClusters.
# THIS IS NOT leftover analog storm-rain-keeps-hydra.
# THIS IS NOT leftover analog campaign-roster-lookup-tests
# #85ab.
# THIS IS NOT leftover CampaignMissionSpec bStormRocketContract
# (sibling bool, leftover-safe in-flight/draft).
...
# is bStormRocketContract and the initializer is false.
# Do NOT claim leftover StormRain / Day / Night kit
# MissionId or leftover Storm kit bHydraForClusters.
...
# WeatherIdentity / `= 160.f` / leftover StormRain
# bHydraForClusters as the locked decl.
...
TARGET = "bool bStormRocketContract = false;"
TARGET_WRONG_TRUE = "bool bStormRocketContract = true;"
TARGET_WRONG_BARE = "bool bStormRocketContract;"
TARGET_WRONG_ZERO = "bool bStormRocketContract = 0;"
TARGET_WRONG_EQ_NONE = "bool bStormRocketContract = NAME_None;"
TARGET_WRONG_FLOAT = "float bStormRocketContract = false;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_FNAME = "FName bStormRocketContract;"
TARGET_WRONG_INT = "int32 bStormRocketContract = false;"
TARGET_WRONG_HYDRA_TRUE = "bool bHydraForClusters = true;"
TARGET_WRONG_HYDRA_FALSE = "bool bHydraForClusters = false;"
...
LEFTOVER_HYDRA_FOR_CLUSTERS = "bHydraForClusters"
...
THIS_SCRIPT = (
    "Scripts/tests/test_campaign_mission_spec_storm_rocket_contract"
...
LEFTOVER_ANALOG_STORM_KEEPS_HYDRA = (
    "Scripts/tests/test_storm_rain_keeps_hydra"
    "_contract.py"
)
LEFTOVER_ANALOG_STORM_KIT_DEFAULTS = (
    "Scripts/tests/test_storm_rain_beat_kit_defaults_contract.py"
)
LEFTOVER_SPEC_NIGHT_IDENTITY = (
...
def leftover_storm_kit_field_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_STORM_KIT_HYDRA,
        LEFTOVER_ANALOG_STORM_KEEPS_HYDRA,
        LEFTOVER_STORM_KIT_MISSION_ID,
...
LOCKED_SCRIPTS = (
    ...
    + leftover_storm_kit_field_scripts()
...
def storm_rocket_contract_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*bool\s+bStormRocketContract\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare / `= true` / leftover Storm kit
    # bHydraForClusters / leftover Theater Category /
    # FName WeatherIdentity / `= 160.f` when origin/main
    # has bool = false. Do not accept sibling MissionId /
    # WeatherIdentity / bNightIdentity fields.
    ...
    compact = collapsed(region)
    if re.search(
        r"bool\s+bStormRocketContract\s*=\s*true",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bStormRocketContract\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bStormRocketContract\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bStormRocketContract\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+bStormRocketContract\s*=\s*false",
        compact,
    ) is None:
        return False
    if re.search(
        r"\bbHydraForClusters\b",
        compact,
    ) and not has_identifier(declaration, "bStormRocketContract"):
        return False
    if storm_rocket_contract_has_uproperty_wrap(region):
        return False
...
def spec_section(header: str) -> str:
    ...
    if STRUCT_NAME == LEFTOVER_STORM_KIT:
        raise AssertionError(
            f"parse window is leftover {LEFTOVER_STORM_KIT}"
        )
    if has_identifier(LOCKED_DECL, LEFTOVER_HYDRA_FOR_CLUSTERS):
        raise AssertionError(
            f"identifier is leftover {LEFTOVER_HYDRA_FOR_CLUSTERS}"
        )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bStormRocketContract\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bStormRocketContract is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    ...
    if re.search(
        ...
        raise AssertionError(
            "UPROPERTY wrap on bool bStormRocketContract is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "bStormRocketContract"):
        raise AssertionError(
            "UPROPERTY clone landed on bStormRocketContract; locked decl is "
            f"bare {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "bStormRocketContract"):
            raise AssertionError(
                f"{token} clone landed on bStormRocketContract; locked decl is "
                f"bare {LOCKED_DECL}"
            )


class CampaignMissionSpecStormRocketContractFieldDeclContractTests(
    unittest.TestCase
):
    def test_campaign_mission_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bStormRocketContract"), section)
        self.assertFalse(has_identifier(section, LEFTOVER_HYDRA_FOR_CLUSTERS))
    ...
    def test_locked_decl_is_bare_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "bool bStormRocketContract = false;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("bool bStormRocketContract"))
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HYDRA_TRUE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HYDRA_FALSE)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("bStormRocketContract", str(raised.exception))
    ...
    def test_leftover_storm_kit_parse_window_fails_closed(self) -> None:
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_STORM_KIT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HYDRA_TRUE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HYDRA_FALSE)
        self.assertNotIn(LEFTOVER_HYDRA_FOR_CLUSTERS, LOCKED_DECL)
        leftover = (
            f"struct {LEFTOVER_STORM_KIT}\n"
            "{\n"
            f"\t{TARGET_WRONG_HYDRA_TRUE}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_does_not_claim_leftover_storm_day_night_or_debrief(self) -> None:
        ...
        self.assertIn(LEFTOVER_HYDRA_FOR_CLUSTERS, storm_header)
        self.assertIn(TARGET_WRONG_HYDRA_TRUE, storm_header)
        self.assertNotIn("bStormRocketContract", storm_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn("bStormRocketContract", day_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn(LEFTOVER_HYDRA_FOR_CLUSTERS, night_header)
        self.assertNotIn("bStormRocketContract", night_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bStormRocketContract"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_missing_storm_rocket_contract_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_night_identity_does_not_satisfy(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_true_or_bare_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bStormRocketContract = false;", compact_origin)
        self.assertNotIn("bStormRocketContract = true", compact_origin)
        self.assertNotIn("bStormRocketContract = NAME_None", compact_origin)
        self.assertNotIn("bStormRocketContract = 0.f", compact_origin)
        self.assertNotIn("bStormRocketContract = 160.f", compact_origin)
        self.assertNotIn(LEFTOVER_HYDRA_FOR_CLUSTERS, compact_origin)

    def test_storm_rocket_contract_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bStormRocketContract"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tbool bStormRocketContracts = false;\n",
            "\tint32 bStormRocketContract = 0;\n",
            "\tFName bStormRocketContract;\n",
            "\tfloat bStormRocketContract = " + forty + ";\n",
            "\tfloat bStormRocketContract = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    storm_rocket_contract_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(storm_rocket_contract_has_uproperty_wrap(origin), origin)
    ...
    def test_does_not_parse_leftover_kits_or_neighbors(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_HYDRA_FOR_CLUSTERS, header)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_HUD_SNAPSHOT, section)
        self.assertNotIn(LEFTOVER_LOADOUT_SPEC, section)
        self.assertNotIn(LEFTOVER_HYDRA_FOR_CLUSTERS, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec storm-rocket-contract field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "storm_rocket_contract_field_decl_contract.py"
        ))
        ...
        self.assertNotIn("night_identity", Path(__file__).name)
        self.assertNotIn("keeps_hydra", Path(__file__).name)
        self.assertNotIn("SkyguardCampaignRoster.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardNightSortieBeatKit.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_SPEC_MISSION_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_STORM_KIT_HYDRA, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ANALOG_STORM_KEEPS_HYDRA, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SPEC_NIGHT_IDENTITY, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_HYDRA_FOR_CLUSTERS, locked_only)
        for field in leftover_neighbor_hit_fields():
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_STORM_KIT_HYDRA)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_ANALOG_STORM_KEEPS_HYDRA)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_SPEC_NIGHT_IDENTITY)
