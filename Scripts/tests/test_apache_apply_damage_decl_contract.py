...
# or lock the ApplyDamage body in the .cpp.
...
# apache-own-ship-systems #96c5 (stay off ApplySystemHit /
# ApplyHit / ESkyguardApacheSystem). This is not leftover
...
# GetDamageFraction. This is not leftover
...
# SetFirstPersonInterior, SetDirectFlightInput, or
# GetForwardSpeed siblings. Stay off mount getters,
...
# GetForwardSpeed, GetDamageFraction, ApplySystemHit,
...
# HandleDebriefKey, leftover SetOrbitFocus sibling
# in this wave, leftover FaceWorldLocation sibling
# in this wave, leftover SetSensorView sibling in
# this wave, leftover SetFirstPersonInterior sibling
# in this wave, leftover SetDirectFlightInput sibling
# in this wave, leftover GetForwardSpeed sibling in
# this wave, and leftover SortiePresentationWidgets
# stay sibling-only.
APPLY_DAMAGE = (
    "void ApplyDamage(float Amount);"
)
...
# ApplyDamage declaration contract on
...
# GetForwardSpeed sibling, and dirty workspace paths.
LOCKED = {
...
GET_DAMAGE_FRACTION_NOT_LOCKED = (
    "float GetDamageFraction() const;",
)
...
# .cpp ApplyDamage body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::ApplyDamage",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheApplyDamageDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, APPLY_DAMAGE), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, APPLY_DAMAGE)
        self.assertIn("ApplyDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, APPLY_DAMAGE))
    ...
    def test_missing_apply_damage_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, APPLY_DAMAGE)
        self.assertIn("ApplyDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, APPLY_DAMAGE)
        self.assertIn("ApplyDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, APPLY_DAMAGE), section)
        self.assertNotIn("BlueprintPure", APPLY_DAMAGE)
        self.assertNotIn("UFUNCTION", APPLY_DAMAGE)
        self.assertNotIn("Category", APPLY_DAMAGE)
        self.assertNotIn("BlueprintCallable", APPLY_DAMAGE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, APPLY_DAMAGE)
        self.assertIn("ApplyDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid ApplyDamage;\n"
        wrong_return_bool = "\tbool ApplyDamage(float Amount);\n"
        wrong_return_int = "\tint32 ApplyDamage(float Amount);\n"
        added_const = "\tvoid ApplyDamage(float Amount) const;\n"
        wrong_type = "\tvoid ApplyDamage(int Amount);\n"
        wrong_rotator = (
            "\tvoid ApplyDamage(const FRotator& Amount);\n"
        )
        wrong_value = "\tvoid ApplyDamage(FVector Amount);\n"
        added_arg = "\tvoid ApplyDamage(float Amount, float Extra);\n"
        no_args = "\tvoid ApplyDamage();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, APPLY_DAMAGE)
            self.assertIn("ApplyDamage", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_apply_damage_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, APPLY_DAMAGE))
        self.assertEqual(declaration_count(section, APPLY_DAMAGE), 1)
        self.assertTrue(
            APPLY_DAMAGE.startswith("void "),
            APPLY_DAMAGE,
        )
        self.assertTrue(APPLY_DAMAGE.endswith(";"), APPLY_DAMAGE)
        self.assertIn("ApplyDamage(", APPLY_DAMAGE)
        self.assertIn("float Amount", APPLY_DAMAGE)
        self.assertNotIn("INDEX_NONE", APPLY_DAMAGE)
        self.assertNotIn("{", APPLY_DAMAGE)
        self.assertNotIn("}", APPLY_DAMAGE)
        self.assertNotIn("return ", APPLY_DAMAGE)
        self.assertNotIn(" const", APPLY_DAMAGE)
        self.assertNotIn("SetRotorPower", APPLY_DAMAGE)
        self.assertNotIn("AimChinTurret", APPLY_DAMAGE)
        self.assertNotIn("GetChinMuzzleLocation", APPLY_DAMAGE)
        self.assertNotIn("IssuePilotCommand", APPLY_DAMAGE)
        self.assertNotIn("GetPilotCommand", APPLY_DAMAGE)
        self.assertNotIn("GetPilotConfirmationsIssued", APPLY_DAMAGE)
        self.assertNotIn("SetOrbitFocus", APPLY_DAMAGE)
        self.assertNotIn("FaceWorldLocation", APPLY_DAMAGE)
        self.assertNotIn("SetSensorView", APPLY_DAMAGE)
        self.assertNotIn("SetFirstPersonInterior", APPLY_DAMAGE)
        self.assertNotIn("SetDirectFlightInput", APPLY_DAMAGE)
        self.assertNotIn("GetForwardSpeed", APPLY_DAMAGE)
        self.assertNotIn("GetDamageFraction", APPLY_DAMAGE)
        self.assertNotIn("ApplySystemHit", APPLY_DAMAGE)
        self.assertNotIn("GetChinTurret", APPLY_DAMAGE)
        self.assertNotIn("GetRotorPowerScale", APPLY_DAMAGE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, APPLY_DAMAGE),
                section,
            )
            self.assertEqual(
                require_declaration(section, APPLY_DAMAGE),
            ...
            self.assertEqual(
                declaration_count(section, APPLY_DAMAGE),
                1,
            )
        one_line = f"{{\npublic:\n\t{APPLY_DAMAGE}\n}}\n"
        self.assertTrue(has_declaration(one_line, APPLY_DAMAGE))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, APPLY_DAMAGE), section)
        self.assertEqual(
            require_declaration(section, APPLY_DAMAGE),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{APPLY_DAMAGE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", APPLY_DAMAGE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", APPLY_DAMAGE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_apply_damage_cpp_body(self) -> None:
        locked_only = f"{APPLY_DAMAGE}\n"
        self.assertNotIn("{", APPLY_DAMAGE)
        self.assertNotIn("}", APPLY_DAMAGE)
        self.assertNotIn("return ", APPLY_DAMAGE)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", APPLY_DAMAGE)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", APPLY_DAMAGE)
        self.assertNotIn("return true", APPLY_DAMAGE)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{APPLY_DAMAGE}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_DAMAGE)
        self.assertNotIn("GetGunnerMount", APPLY_DAMAGE)
        self.assertNotIn("GetEyeMount", APPLY_DAMAGE)
        self.assertNotIn("GetWeaponMount", APPLY_DAMAGE)
        self.assertNotIn("GetChinTurret", APPLY_DAMAGE)
        self.assertNotIn("GetPilotMount", APPLY_DAMAGE)
        self.assertNotIn("GetSensorTurret", APPLY_DAMAGE)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{APPLY_DAMAGE}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_DAMAGE)
        self.assertNotIn("GetChinMuzzleLocation", APPLY_DAMAGE)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_DAMAGE)
        for neighbor in GET_DAMAGE_FRACTION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_DAMAGE)
        self.assertNotIn("GetDamageFraction", APPLY_DAMAGE)
        self.assertNotIn("ApplySystemHit", APPLY_DAMAGE)
        self.assertNotIn("ApplyHit", APPLY_DAMAGE)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_DAMAGE)
        self.assertNotIn("IsSensorLive", APPLY_DAMAGE)
        self.assertNotIn("IsThermalAvailable", APPLY_DAMAGE)
        self.assertNotIn("IsSensorViewActive", APPLY_DAMAGE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_DAMAGE)
        self.assertNotIn("ESkyguardApacheSystem", APPLY_DAMAGE)
    ...
    def test_contract_does_not_relock_leftover_set_first_person_sibling(
        self,
    ) -> None:
        locked_only = f"{APPLY_DAMAGE}\n"
    ...
    def test_contract_does_not_relock_leftover_set_direct_flight_sibling(
        self,
    ) -> None:
        locked_only = f"{APPLY_DAMAGE}\n"
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_DAMAGE)
        self.assertNotIn("Pursuit", APPLY_DAMAGE)
        self.assertNotIn("OrbitLeft", APPLY_DAMAGE)
        self.assertNotIn("AttackRun", APPLY_DAMAGE)
        self.assertNotIn("FaceTarget", APPLY_DAMAGE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_DAMAGE)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", APPLY_DAMAGE)
        self.assertNotIn("USkyguardBriefingWidget", APPLY_DAMAGE)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            APPLY_DAMAGE,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_DAMAGE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", APPLY_DAMAGE)
        self.assertNotIn("ESkyguardMissionSkylineStyle", APPLY_DAMAGE)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{APPLY_DAMAGE}\n"
        self.assertEqual(
            require_declaration(locked_only, APPLY_DAMAGE),
            APPLY_DAMAGE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache ApplyDamage contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, APPLY_DAMAGE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache ApplyDamage contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, APPLY_DAMAGE)

    def test_contract_is_apply_damage_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        for token in LEFTOVER_GET_FORWARD_SPEED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_DAMAGE)
        for token in GET_DAMAGE_FRACTION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_DAMAGE)
        for token in LEFTOVER_PILOT_COMMAND_ROSTER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
        ...
        self.assertNotIn("return ", APPLY_DAMAGE)
        self.assertNotIn("{", APPLY_DAMAGE)
        self.assertTrue(APPLY_DAMAGE.startswith("void "))
        self.assertTrue(APPLY_DAMAGE.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
