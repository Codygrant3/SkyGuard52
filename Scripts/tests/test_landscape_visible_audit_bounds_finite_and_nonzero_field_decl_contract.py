...
# bBoundsFiniteAndNonzero FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bBoundsFiniteAndNonzero = false; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# bBoundsFiniteAndNonzero. THIS IS NOT leftover
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# bBoundsFiniteAndNonzero FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bBoundsFiniteAndNonzero = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly bool bBoundsFiniteAndNonzero = false;
...
# bBoundsFiniteAndNonzero FIELD (declaration-only). It is
...
TARGET = "bool bBoundsFiniteAndNonzero = false;"
TARGET_WRONG = "bool bBoundsFiniteAndNonzero = true;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bBoundsFiniteAndNonzero (must be = false, not = true).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bBoundsFiniteAndNonzero\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bBoundsFiniteAndNonzero is missing from "
...
class LandscapeVisibleAuditBoundsFiniteAndNonzeroFieldDeclContractTests(unittest.TestCase):
    def test_landscape_visible_audit_struct_exists(self) -> None:
        ...
        self.assertIn("bBoundsFiniteAndNonzero", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bBoundsFiniteAndNonzero", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_bounds_finite_and_nonzero_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_bounds_finite_and_nonzero_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bBoundsFiniteAndNonzero"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(
                "\tbool bBoundsFiniteAndNonzero = true;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("bBoundsFiniteAndNonzero", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bBoundsFiniteAndNonzero", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bBoundsFiniteAndNonzero"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bBoundsFiniteAndNonzero", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_bounds_finite_and_nonzero(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
