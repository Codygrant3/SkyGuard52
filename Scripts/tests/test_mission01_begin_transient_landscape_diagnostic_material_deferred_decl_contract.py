...
# static FSkyguardLandscapeVisibleAudit BeginTransientLandscapeDiagnosticMaterialDeferred(ALandscape* Landscape, UMaterialInterface* Material);
...
# Identifier BeginTransientLandscapeDiagnosticMaterialDeferred is NOT leftover
# analog VisibleAudit field bSuccess and NOT leftover unique
# PrepareGovernedLandscapeForVisibleValidation #1694 /
# AuditLandscapeVisibleReadiness #1696 /
...
# BeginTransientLandscapeDiagnosticMaterialDeferred.
# RETARGET second arg GovernedMaterial -> Material.
...
# LOCKED_DECL is BeginTransientLandscapeDiagnosticMaterialDeferred
...
# void BeginTransientLandscapeDiagnosticMaterialDeferred
# AS THE LOCKED DECL.
...
TARGET_WRONG_FLOAT = (
    "float BeginTransientLandscapeDiagnosticMaterialDeferred();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void BeginTransientLandscapeDiagnosticMaterialDeferreds();"
...
TARGET_WRONG_TRUE = "bool BeginTransientLandscapeDiagnosticMaterialDeferred = true;"
TARGET_WRONG_ZERO = "bool BeginTransientLandscapeDiagnosticMaterialDeferred = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    ...
    r"BeginTransientLandscapeDiagnosticMaterialDeferred\("
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bBeginTransientLandscapeDiagnosticMaterialDeferred\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+BeginTransientLandscapeDiagnosticMaterialDeferred\b", compact):
        return False
    if re.search(r"\bbool\s+BeginTransientLandscapeDiagnosticMaterialDeferred\b", compact):
        return False
    if re.search(r"\bfloat\s+BeginTransientLandscapeDiagnosticMaterialDeferred\b", compact):
        return False
    if re.search(r"\bint32\s+BeginTransientLandscapeDiagnosticMaterialDeferred\b", compact):
        return False
...
def spec_section(header: str) -> str:
    ...
    method = "BeginTransientLandscapeDiagnosticMaterialDeferred"
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            ...
            r"BeginTransientLandscapeDiagnosticMaterialDeferred\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for BeginTransientLandscapeDiagnosticMaterialDeferred "
...
class Mission01BeginTransientLandscapeDiagnosticMaterialDeferredDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "BeginTransientLandscapeDiagnosticMaterialDeferred"),
            section,
        )
    ...
    def test_missing_begin_transient_landscape_diagnostic_material_deferred_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "BeginTransientLandscapeDiagnosticMaterialDeferred()const",
    ...
    def test_begin_transient_landscape_diagnostic_material_deferred_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid BeginTransientLandscapeDiagnosticMaterialDeferredX();\n",
            "\tint32 BeginTransientLandscapeDiagnosticMaterialDeferred = 1;\n",
            "\tFName BeginTransientLandscapeDiagnosticMaterialDeferred = NAME_None;\n",
            "\tvoid BeginTransientLandscapeDiagnosticMaterialDeferred() = "
            + forty
            + ";\n",
            "\tvoid BeginTransientLandscapeDiagnosticMaterialDeferred() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "BeginTransientLandscapeDiagnosticMaterialDeferred")
        )
        self.assertTrue(
            has_identifier(section, "BeginTransientLandscapeDiagnosticMaterialDeferred")
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_begin_transient_landscape_diagnostic_material_deferred"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
