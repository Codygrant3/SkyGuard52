...
# Mission10 bCampaignDefinitionValid FIELD on
# FSkyguardMission10IntegrationReadiness
# (declaration-only, one-line / split-line
# UPROPERTY wraps, bool default false). Note
# VisibleAnywhere / BlueprintReadOnly /
# bool = false, not EditAnywhere /
# BlueprintReadWrite / Category /
# ClampMin / TObjectPtr / int32 = 100. It is
# NOT leftover Mission10 CampaignDefinition field
# #798 / branch cursor/mission10-campaign-definition-
# field-decl-contract-c332, NOT leftover
# Mission10 Readiness field on the director, NOT
# leftover Mission01 Readiness #579, NOT leftover
...
# origin/main is a one-line / split-line UPROPERTY
# field (`bool bCampaignDefinitionValid = false;`);
# accept that form, other one-line / split-line wraps,
# the `= false` default, and nearby split-line
...
# No Category on this leftover-safe field, not
...
UPROPERTY_MISSION10_CLAMP = 'meta=(ClampMin="1")'
...
CAMPAIGN_DEFINITION_VALID_FIELD = (
    "bool bCampaignDefinitionValid = false;"
)
LOCKED_DECL = CAMPAIGN_DEFINITION_VALID_FIELD
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission10_maximum_protected_integrity_field"
    "_decl_contract.py",
...
SIBLING_DIRECTOR_FIELDS_NOT_LOCKED = (
    ...
    "test_mission10_minimum_weapon_separation_meters_field"
    "_decl_contract.py",
)
...
CLASS_RE = re.compile(
    rf"(?:struct|class)\s+(?:SKYGUARD52_API\s+)?{re.escape(CLASS_NAME)}\b"
)
...
LEFTOVER_MISSION10_CAMPAIGN_DEFINITION_FIELD_NOT_LOCKED = (
    "TSoftObjectPtr<USkyguardCampaignDefinition>",
    "CampaignDefinition;",
    "test_mission10_campaign_definition_field_decl_contract.py",
    "cursor/mission10-campaign-definition-field-decl-contract-c332",
    "#798",
)
LEFTOVER_MISSION01_CAMPAIGN_DEFINITION_FIELD_NOT_LOCKED = (
    "test_mission01_campaign_definition_field_decl_contract.py",
    "cursor/mission01-campaign-definition-field-decl-contract-c332",
    "#577",
)
...
SIBLING_READINESS_FIELDS_NOT_LOCKED = (
...
class Mission10CampaignDefinitionValidFieldDeclContractTests(unittest.TestCase):
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Mission10"', section)
        ...
        self.assertNotIn("Skyguard|Mission10|Campaign", UPROPERTY_MISSION10)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission10 bCampaignDefinitionValid field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission10 bCampaignDefinitionValid contains "
    ...
    def test_contract_is_campaign_definition_valid_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_campaign_definition_valid(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        assigned = "\tbool bCampaignDefinitionValid = true;\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission10_campaign_definition_valid_field"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_campaign_definition_valid_field"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_mission10_campaign_definition_field(
        self,
    ) -> None:
        locked_only = f"{LOCKED_DECL}\n"
        for token in LEFTOVER_MISSION10_CAMPAIGN_DEFINITION_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn(
            "Scripts/tests/test_mission10_campaign_definition"
        ...
        self.assertNotIn("CampaignDefinition;", LOCKED_DECL)
        self.assertNotIn("#798", LOCKED_DECL)

    def test_contract_does_not_relock_leftover_mission01_campaign_definition_field(
        self,
    ) -> None:
        locked_only = f"{LOCKED_DECL}\n"
        for token in LEFTOVER_MISSION01_CAMPAIGN_DEFINITION_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn(
            "Scripts/tests/test_mission01_campaign_definition"
        ...
        self.assertNotIn("#577", LOCKED_DECL)

    def test_contract_does_not_relock_leftover_mission10_readiness_field(
        self,
    ) -> None:
        locked_only = f"{LOCKED_DECL}\n"
        for token in LEFTOVER_MISSION10_READINESS_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_mission01_readiness_field(
        self,
    ) -> None:
        locked_only = f"{LOCKED_DECL}\n"
        for token in LEFTOVER_MISSION01_READINESS_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn(
        ...
        self.assertNotIn("#579", LOCKED_DECL)

    def test_contract_does_not_relock_leftover_mission10_get_readiness(
        self,
    ) -> None:
        locked_only = f"{LOCKED_DECL}\n"
        ...
        self.assertNotIn("GetReadiness", LOCKED_DECL)
        self.assertNotIn("#776", LOCKED_DECL)

    def test_contract_does_not_relock_sibling_readiness_struct_fields(
        self,
    ) -> None:
        locked_only = f"{LOCKED_DECL}\n"
        for token in SIBLING_READINESS_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in leftover_readiness_tokens():
            self.assertNotIn(token, locked_only)
