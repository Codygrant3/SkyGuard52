# THIS IS leftover-safe FSkyguardAudioTelemetry::PriorityEvictions.
# origin/main form: split-line
# UPROPERTY(BlueprintReadOnly)
# then `int32 PriorityEvictions = 0;` with no Category=.
...
# do not copy those; this field is `int32 PriorityEvictions = 0;`.
...
# RejectedMissingAsset, PeakActiveVoices).
...
TARGET = "int32 PriorityEvictions = 0;"
TARGET_WRONG_BARE = "int32 PriorityEvictions;"
TARGET_WRONG_FALSE = "int32 PriorityEvictions = false;"
TARGET_WRONG_TRUE = "int32 PriorityEvictions = true;"
TARGET_WRONG_FLOAT = "int32 PriorityEvictions = 0.f;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # when origin/main has `int32 PriorityEvictions = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+PriorityEvictions\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PriorityEvictions\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PriorityEvictions\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PriorityEvictions\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+PriorityEvictions\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 PriorityEvictions is missing from "
...
class AudioTelemetryPriorityEvictionsFieldDeclContractTests(unittest.TestCase):
    def test_audio_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("PriorityEvictions", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("PriorityEvictions", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_priority_evictions_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("PriorityEvictions = 0", collapsed(origin))
        self.assertNotIn("PriorityEvictions = false", collapsed(origin))
        self.assertNotIn("PriorityEvictions = true", collapsed(origin))
        self.assertNotIn("PriorityEvictions = 0.f", collapsed(origin))

    def test_priority_evictions_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 PriorityEvictions"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool PriorityEvictions = 0;\n",
            "\tint32 PriorityEviction = 0;\n",
            "\tint32 PriorityEvictionss = 0;\n",
            "\tFText PriorityEvictions = 0;\n",
            "\tint32 PriorityEvictions{};\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio-telemetry PriorityEvictions field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_audio_telemetry_priority_evictions"
            "_field_decl_contract.py"
        ))
