...
# a campaign-id body, or lock CampaignId in the .cpp.
...
# FindMission / GetPrimaryAssetId (sibling drafts this
# wave), DisplayName, Missions array, leftover
...
# broadcast, ValidateDefinition #331, FindMission /
# GetPrimaryAssetId (sibling drafts this wave), and newly
...
# here. ValidateDefinition #331 / FindMission /
# GetPrimaryAssetId / DisplayName / Missions stay
# sibling-only.
UNLOCKED_NEIGHBORS = (
    "FText DisplayName",
...
# is CampaignId on USkyguardCampaignDefinition only.
...
class CampaignDefinitionCampaignIdDeclContractTests(unittest.TestCase):
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintReadOnly", CAMPAIGN_ID)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CAMPAIGN_ID)
        self.assertIn("CampaignId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_default = "\tFName CampaignId;\n"
        name_none = "\tFName CampaignId = NAME_None;\n"
        wrong_default = (
            '\tFName CampaignId = TEXT("OtherCampaign");\n'
        )
        wrong_type = (
            '\tFString CampaignId = TEXT("Skyguard52MainCampaign");\n'
        )
        text_type = (
            '\tFText CampaignId = TEXT("Skyguard52MainCampaign");\n'
        )
        wrong_name = (
            '\tFName MissionId = TEXT("Skyguard52MainCampaign");\n'
        )
        for region in (
            missing_default,
            name_none,
    ...
    def test_missing_default_does_not_satisfy(self) -> None:
        bare_field = "\tFName CampaignId;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(bare_field, CAMPAIGN_ID)
        self.assertIn("CampaignId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(bare_field, CAMPAIGN_ID))

    def test_wrong_default_does_not_satisfy(self) -> None:
        wrong = '\tFName CampaignId = TEXT("Skyguard52");\n'
        with self.assertRaises(AssertionError) as raised:
            require_declaration(wrong, CAMPAIGN_ID)
        self.assertIn("CampaignId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(wrong, CAMPAIGN_ID))
    ...
    def test_declaration_locks_origin_main_default(self) -> None:
        ...
        self.assertIn("Skyguard52MainCampaign", locked_only)
        self.assertIn("CampaignId =", CAMPAIGN_ID)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CAMPAIGN_ID), section)
        self.assertIn(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_name = (
            "public:\n"
            "\tFName\n"
            '\tCampaignId = TEXT("Skyguard52MainCampaign");\n'
            "private:\n"
            "};\n"
        )
        wrap_assign = (
    ...
    def test_contract_does_not_lock_campaign_id_cpp_body(self) -> None:
        ...
        self.assertNotIn("return false", CAMPAIGN_ID)
        self.assertNotIn("AddError", CAMPAIGN_ID)
    ...
    def test_contract_does_not_relock_validate_definition(self) -> None:
        ...
        self.assertNotIn("ValidateDefinition", CAMPAIGN_ID)
        self.assertNotIn("ValidateDefinition", locked_only)
        self.assertNotIn("OutErrors", CAMPAIGN_ID)
        self.assertNotIn("BlueprintCallable", CAMPAIGN_ID)
    ...
    def test_contract_does_not_relock_find_mission(self) -> None:
        ...
        for neighbor in FIND_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ID)
        self.assertNotIn("FindMission", CAMPAIGN_ID)
        self.assertNotIn("FindMission", locked_only)
        self.assertNotIn("BlueprintPure", CAMPAIGN_ID)
    ...
    def test_contract_does_not_relock_primary_asset_id(self) -> None:
        ...
        for neighbor in PRIMARY_ASSET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ID)
        self.assertNotIn("GetPrimaryAssetId", CAMPAIGN_ID)
        self.assertNotIn("GetPrimaryAssetId", locked_only)
        self.assertNotIn("FPrimaryAssetId", CAMPAIGN_ID)

    def test_contract_does_not_relock_display_name_or_missions(self) -> None:
        locked_only = f"{CAMPAIGN_ID}\n"
        for neighbor in CAMPAIGN_FIELDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ID)
        self.assertNotIn("DisplayName", CAMPAIGN_ID)
        self.assertNotIn("DisplayName", locked_only)
        self.assertNotIn("Missions", CAMPAIGN_ID)
        self.assertNotIn("Missions", locked_only)
        self.assertNotIn("TArray", CAMPAIGN_ID)
        self.assertNotIn("FText", CAMPAIGN_ID)
    ...
    def test_contract_does_not_relock_campaign_subsystem_methods(self) -> None:
        ...
        self.assertNotIn("ConfigureCampaign", CAMPAIGN_ID)
        self.assertNotIn("CanStartMission", CAMPAIGN_ID)
        self.assertNotIn("StartMission", CAMPAIGN_ID)
        self.assertNotIn("GetActiveMission", CAMPAIGN_ID)
        self.assertNotIn("USkyguardCampaignSubsystem", CAMPAIGN_ID)
    ...
    def test_contract_does_not_relock_save_game_helpers(self) -> None:
        ...
        for neighbor in SAVE_GAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ID)
        self.assertNotIn("ApplySaveGame", CAMPAIGN_ID)
        self.assertNotIn("BuildSaveGame", CAMPAIGN_ID)
        self.assertNotIn("RetrySaveLastDebrief", CAMPAIGN_ID)
        self.assertNotIn("SaveCampaignToSlot", CAMPAIGN_ID)
        self.assertNotIn("LoadCampaignFromSlot", CAMPAIGN_ID)
        self.assertNotIn("DeleteCampaignSlot", CAMPAIGN_ID)
        self.assertNotIn("GetMissionRecords", CAMPAIGN_ID)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ID)
        self.assertNotIn("FillResultCombatStats", CAMPAIGN_ID)
        self.assertNotIn("ASkyguardGunner", CAMPAIGN_ID)
        self.assertNotIn("FillAndFinalize", CAMPAIGN_ID)
        self.assertNotIn("FillAndFail", CAMPAIGN_ID)
        self.assertNotIn("ApplyHydraForClusters", CAMPAIGN_ID)
