...
# then `float ApproachSpeedCmPerSecond = 1250.f;` (HAS initializer).
...
# Default IS `= 1250.f`.
# THIS IS leftover-safe isolated UPROPERTY. Category IS
# `Skyguard|Boss|Encounter|Route`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"100.0"`.
# Default IS `= 1250.f`. It is NOT leftover analog coastal/Mission01
# RouteLengthCm = 45000.f. It is NOT leftover analog Pathfinder
# EncounterController UFUNCTION #376-#382. It is NOT leftover analog
# pathfinder-encounter-controller-field #512.
...
# or stay Category="Skyguard|Combat" / Category="Skyguard|Theater"
# / Category="Skyguard|MissionMap" / Category="Skyguard|Apache"
...
# Stop BEFORE sibling LockWindowSpeedCmPerSecond as a claimed slot.
# Do NOT claim sibling RouteLengthCm (leftover analog coastal/Mission01
# same-name 45000.f).
# Do NOT claim sibling LockWindowSpeedCmPerSecond /
# CriticalSpeedCmPerSecond / IngressSwayAmplitudeCm /
...
# TObjectPtr field #512.
# Do NOT claim leftover analog Pathfinder debris / nose camera /
# control linkage.
...
# PropellerLoop.
# Skip leftover GetResolvedProductionLoopRouteCount #1658 /
# AreResolvedProductionLoopRoutesComplete #1659 /
# OpenCockpitWindLoop #1656 / GlobalVoiceLimit #1657.
# Skip leftover EngineIdleLoop #1653 / EnginePowerLoop #1654 /
# EngineCruiseLoop #1655.
# Do NOT claim leftover sibling EngineCruiseLoop / EnginePowerLoop /
# PropellerLoop / EngineIdleLoop / GlobalVoiceLimit /
...
# CockpitExteriorAttenuation and CockpitLowPassHz.
...
# Skip leftover analog VegetationStartCullDistanceCm.
# Skip leftover analog Boom.
# Do NOT parse leftover analog audio-procedural-bank-empty-fail-closed
# bulk, leftover analog audio-harness-fail-closed bulk, leftover
# analog audible-acceptance-receipt fields, leftover analog
# procedural-bank unique development UPROPERTY, leftover analog
# coastal unique actor UPROPERTY, leftover analog Mission01 unique
# actor bools.
# Do NOT parse leftover struct
# FSkyguardMission01EnvironmentReadiness.
# Skip banned Ig+laMuzzle, all Ig+la* fields, Ri+fleMesh /
# HandMesh. Do not claim those.
# Do NOT mention enum variant names that contain Ig+la or
# Ri+fle as contiguous tokens in THIS test file.
# Do NOT claim leftover Mission01 Root / OceanTiles /
# BeachTiles / LandTiles / RouteExclusion / LandScatterBounds /
# InlandVegetationPCG / ProductionLandscape / AuthoredPCGGraph /
# bAllowAuthoredPCGGeneration / RouteLengthCm / DistrictLengthCm /
# Readiness / CoastalHaze* / bUseAuthoredLandscapeSurface /
# LandMaterial / OceanMaterial / BeachMaterial as this slot.
# Do NOT claim leftover FName WeatherIdentity TheaterKit
# (different class, Category Theater, no initializer).
# Do NOT claim leftover Boom (different class ASkyguardGunner,
# Category Skyguard, TObjectPtr spring arm).
...
# Category="Skyguard|Boss|Encounter|Route" wrapping
# leftover `TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;`
# — RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Route"
# plus meta ClampMin `"100.0"` (NO spaces around =).
# Clone LookYawLimit #1618 is float Clamp
# `float LookYawLimit = 95.f;` Category Skyguard|Combat
# — RETARGET hard: class -> USkyguardPathfinderEncounterController,
# Category -> Skyguard|Boss|Encounter|Route,
# decl -> `float ApproachSpeedCmPerSecond = 1250.f;`.
# Clone #1300 SHA c4679feb43f4b97d3a814c088e3b6c344e15831f is
# Category Theater wrapping FName WeatherIdentity — RETARGET hard.
# LOCKED_DECL is `float ApproachSpeedCmPerSecond = 1250.f;`
...
# OpenCockpitWindLoop and NOT leftover `float RouteLengthCm = 45000.f`
# and NOT leftover analog `float FireRate = 12.0f` /
# `float RecoilPitch = 0.92f` and NOT leftover
# LockWindowClimbSeconds = 2.4f and NOT Harbor 4 + 0 / 8 + 0
# float tokens and NOT leftover combat look float
# LookYawLimit = 95.f.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# / Boom / MinimumMeasuredSamples / SampleRate /
# GeneratedByteBudget / VegetationStartCullDistanceCm
# / EngineCruiseLoop / EnginePowerLoop / PropellerLoop /
# EngineIdleLoop / GlobalVoiceLimit
# AS THE LOCKED DECL.
...
# or Skyguard|Audio|Engine
# or Category="Skyguard" without |Boss|Encounter|Route.
# Fail-closed if wrap is VisibleAnywhere or BlueprintReadOnly
# or EditInstanceOnly (must be EditAnywhere + BlueprintReadWrite).
# Fail-closed if ClampMin is missing.
# Fail-closed if the default is not `= 1250.f`.
# Fail-closed if LOCKED_DECL is RouteLengthCm = 45000.f /
# FireRate = 12.0f / RecoilPitch = 0.92f /
# MinHeightFromOriginCm Harbor analog /
# LockWindowClimbSeconds = 2.4f / Harbor 4 + 0 or 8 + 0.
...
TARGET = "float ApproachSpeedCmPerSecond = 1250.f;"
TARGET_WRONG_BARE = "float ApproachSpeedCmPerSecond;"
TARGET_WRONG_INIT = "float ApproachSpeedCmPerSecond = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float ApproachSpeedCmPerSecond = true;"
TARGET_WRONG_ZERO = "float ApproachSpeedCmPerSecond = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> ApproachSpeedCmPerSecond;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> ApproachSpeedCmPerSecond;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> ApproachSpeedCmPerSecond;"
...
STOP_BEFORE_SKYLINE_STYLE = "LockWindowSpeedCmPerSecond"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_approach_speed"
...
TARGET_WRONG_INGRESS_SWAY_AMP = "float IngressSwayAmplitudeCm = 320.f;"
TARGET_WRONG_INGRESS_SWAY_WAVE = "float IngressSwayWavelengthCm = 9000.f;"
TARGET_WRONG_LOCK_WINDOW_CLIMB = "float LockWindowClimbCm = 650.f;"
TARGET_WRONG_LOCK_WINDOW_CLIMB_S = "float LockWindowClimbSeconds = 2.4f;"
TARGET_WRONG_TURN_RADIUS = "float CriticalTurnRadiusCm = 1500.f;"
TARGET_WRONG_TURN_SECONDS = "float CriticalTurnSeconds = 5.5f;"
TARGET_WRONG_MIN_HEIGHT = "float MinHeightFromOriginCm;"
...
TARGET_WRONG_NOSE_CAMERA = "TObjectPtr<UStaticMeshComponent> NoseCamera;"
TARGET_WRONG_CONTROL_LINKAGE = (
    "TObjectPtr<UStaticMeshComponent> ControlLinkage;"
)
TARGET_WRONG_ROUTE_COUNT = (
    "int32 GetResolvedProductionLoopRouteCount() const;"
)
TARGET_WRONG_ROUTES_COMPLETE = (
...
SIBLING_LOCK_WINDOW_CLIMB_S = "LockWindowClimbSeconds"
SIBLING_TURN_RADIUS = "CriticalTurnRadiusCm"
SIBLING_TURN_SECONDS = "CriticalTurnSeconds"
...
SIBLING_GET_SPEED_MULT = "GetEffectiveSpeedMultiplier"
SIBLING_IS_TELEGRAPH = "IsAttackTelegraphActive"
SIBLING_GET_TELEGRAPHS = "GetTelegraphsTriggered"
SIBLING_ENCOUNTER_CONTROLLER = "EncounterController"
SIBLING_NOSE_CAMERA = "NoseCamera"
SIBLING_CONTROL_LINKAGE = "ControlLinkage"
SIBLING_DEBRIS_NOSE = "DebrisNose"
SIBLING_DEBRIS_CENTER = "DebrisCenter"
SIBLING_DEBRIS_SPINE = "DebrisSpine"
SIBLING_DEBRIS_TAIL = "DebrisTail"
LEFTOVER_PATHFINDER_ENCOUNTER_CONTROLLER_FIELD = (
    "Scripts/tests/test_pathfinder_encounter_controller"
    "_field_decl_contract.py"
)
LEFTOVER_PATHFINDER_ADVANCE_ENCOUNTER = (
    "Scripts/tests/test_pathfinder_advance_encounter"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_RESET_ENCOUNTER = (
    "Scripts/tests/test_pathfinder_reset_encounter_state"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_IS_ROUTE_SAFE = (
    "Scripts/tests/test_pathfinder_is_route_state_safe"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_GET_ROUTE_PROGRESS = (
    "Scripts/tests/test_pathfinder_get_route_progress"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_GET_SPEED_MULT = (
    "Scripts/tests/test_pathfinder_get_effective_speed_multiplier"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_IS_TELEGRAPH = (
    "Scripts/tests/test_pathfinder_is_attack_telegraph_active"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_GET_TELEGRAPHS = (
    "Scripts/tests/test_pathfinder_get_telegraphs_triggered"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_NOSE_CAMERA = (
    "Scripts/tests/test_pathfinder_nose_camera"
    "_field_decl_contract.py"
)
LEFTOVER_PATHFINDER_CONTROL_LINKAGE = (
    "Scripts/tests/test_pathfinder_control_linkage"
    "_field_decl_contract.py"
)
...
def leftover_pathfinder_ufunction_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_PATHFINDER_ADVANCE_ENCOUNTER,
        LEFTOVER_PATHFINDER_RESET_ENCOUNTER,
        LEFTOVER_PATHFINDER_IS_ROUTE_SAFE,
...
def leftover_pathfinder_ufunction_tokens() -> tuple[str, ...]:
    return (
        SIBLING_ADVANCE_ENCOUNTER,
        SIBLING_RESET_ENCOUNTER,
        SIBLING_IS_ROUTE_SAFE,
        SIBLING_GET_ROUTE_PROGRESS,
        SIBLING_GET_SPEED_MULT,
        SIBLING_IS_TELEGRAPH,
        SIBLING_GET_TELEGRAPHS,
        SIBLING_ENCOUNTER_CONTROLLER,
        SIBLING_NOSE_CAMERA,
        SIBLING_CONTROL_LINKAGE,
        SIBLING_DEBRIS_NOSE,
        SIBLING_DEBRIS_CENTER,
        SIBLING_DEBRIS_SPINE,
        SIBLING_DEBRIS_TAIL,
        "GetResolvedProductionLoopRouteCount",
...
APPROACH_SPEED_DECL_RE = re.compile(
    r"float\s+ApproachSpeedCmPerSecond\s*=\s*1250\.f\s*;"
)
APPROACH_SPEED_BARE_RE = re.compile(
    r"float\s+ApproachSpeedCmPerSecond\s*;"
)
ROUTE_LENGTH_CM_DECL_RE = re.compile(
    r"float\s+RouteLengthCm\s*=\s*45000\.f\b"
)
LOCK_WINDOW_CLIMB_S_DECL_RE = re.compile(
...
VEGETATION_CULL_DECL_RE = re.compile(
    r"VegetationStartCullDistanceCm\b"
)
BOOM_DECL_RE = re.compile(
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_LOCK_WINDOW_CLIMB_S,
        SIBLING_TURN_RADIUS,
        SIBLING_TURN_SECONDS,
        SIBLING_MIN_HEIGHT,
        SIBLING_ADVANCE_ENCOUNTER,
        SIBLING_RESET_ENCOUNTER,
        SIBLING_IS_ROUTE_SAFE,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float ApproachSpeedCmPerSecond = 1250.f;`.
    ...
    # GlobalVoiceLimit, leftover analog SampleRate /
    ...
    # RouteLengthCm = 45000.f, leftover
    # LockWindowClimbSeconds = 2.4f, leftover
    # OpenCockpitWindLoop, leftover GlobalVoiceLimit = 24,
    # leftover bAllowAuthoredPCGGeneration /
    ...
    # ApproachSpeedCmPerSecond without the 1250.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if APPROACH_SPEED_DECL_RE.search(compact) is None:
        return False
    if APPROACH_SPEED_BARE_RE.search(compact):
        return False
    if ROUTE_LENGTH_CM_DECL_RE.search(compact):
        return False
    if LOCK_WINDOW_CLIMB_S_DECL_RE.search(compact):
        return False
    if GLOBAL_VOICE_LIMIT_DECL_RE.search(compact):
        return False
    if OPEN_COCKPIT_WIND_LOOP_DECL_RE.search(compact):
        return False
    if VEGETATION_CULL_DECL_RE.search(compact):
        return False
    if BOOM_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "ApproachSpeedCmPerSecond":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+ApproachSpeedCmPerSecond\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float ApproachSpeedCmPerSecond "
...
class PathfinderEncounterApproachSpeedFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ApproachSpeedCmPerSecond"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_LOCK_WINDOW_SPEED), body)
        self.assertFalse(has_identifier(section, SIBLING_LOCK_WINDOW_SPEED), section)
        self.assertTrue(has_identifier(body, SIBLING_ADVANCE_ENCOUNTER), body)
        self.assertFalse(
            has_identifier(section, SIBLING_ADVANCE_ENCOUNTER),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_LOCK_WINDOW_CLIMB_S), body)
        self.assertFalse(has_identifier(section, SIBLING_LOCK_WINDOW_CLIMB_S), section)
        ...
        for leftover in leftover_pathfinder_ufunction_tokens():
            self.assertFalse(has_identifier(LOCKED_DECL, leftover), leftover)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ApproachSpeedCmPerSecond"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ApproachSpeedCmPerSecond", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_approach_speed_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_approach_speed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float ApproachSpeedCmPerSecond"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1250.f", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_WINDOW_CLIMB_S)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, "float RouteLengthCm = 45000.f;")
        self.assertNotEqual(LOCKED_DECL, "int32 GlobalVoiceLimit = 24;")
        self.assertNotEqual(
            LOCKED_DECL,
            "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;",
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat ApproachSpeedCmPerSeconds = 1250.f;\n",
            "\tint32 ApproachSpeedCmPerSecond = 1250;\n",
            "\tbool ApproachSpeedCmPerSecond = true;\n",
            "\tfloat ApproachSpeedCmPerSecond = " + forty + ";\n",
            "\tfloat ApproachSpeedCmPerSecond = " + eighty + ";\n",
            f"\t{TARGET_WRONG_BOOM}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Skyguard|Audio|Engine', origin)
        self.assertIsNone(
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', theater_specs)
        self.assertIn('Category="Skyguard|Boss|Encounter|Route"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_route_length_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "ApproachSpeedCmPerSecond"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, SIBLING_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_CRITICAL_SPEED))
        self.assertFalse(has_identifier(section, SIBLING_CRITICAL_SPEED))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "ApproachSpeedCmPerSecond"))
        self.assertTrue(has_identifier(section, "ApproachSpeedCmPerSecond"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_ROUTE_LENGTH_CM))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_ADVANCE_ENCOUNTER))
        self.assertFalse(has_identifier(section, SIBLING_ADVANCE_ENCOUNTER))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_TELEGRAPHS))
        self.assertFalse(has_identifier(section, SIBLING_GET_TELEGRAPHS))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardPathfinderBoss.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter approach speed field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_approach_speed_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_PATHFINDER_ENCOUNTER_CONTROLLER_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_OPEN_COCKPIT_WIND_LOOP, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PATHFINDER_ADVANCE_ENCOUNTER, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_ROUTE_LENGTH_CM, locked_only)
        ...
        self.assertNotIn(SIBLING_NOSE_CAMERA, locked_only)
        self.assertNotIn("OpenCockpitWindLoop", locked_only)
        self.assertNotIn(SIBLING_GLOBAL_VOICE_LIMIT, locked_only)
