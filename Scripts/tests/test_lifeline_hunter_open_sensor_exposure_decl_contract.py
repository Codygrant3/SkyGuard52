...
# or lock the OpenSensorExposure body in the .cpp.
# origin/main is one line (`bool OpenSensorExposure();`);
...
# mount getters, leftover #4e39 GetChinMuzzleLocation.
# Stay off leftover USkyguardBossWeakPointComponent
...
# leftover isolated-test drafts #107–#434 including
# leftover searchlight-track-runtime-defaults #7347
# and leftover BlackKite / RadarGhost
# SetContactIdentified / IsContactIdentified. Harbor
# interval retune tokens fail closed in this file
# and the locked declaration only. Do not scan the
# Apache public section for those tokens. Harbor
# clock names may be scanned in this relevant public
# section and must be absent. Stay off leftover
...
# Python OpenSensorExposure declaration contract on
...
# leftover isolated-test drafts #107–#434, leftover
...
# IsContactIdentified, leftover gun-fire camera
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_lifeline_hunter_set_friendly_separation_decl_contract.py",
...
# not locked here. Sibling SetFriendlySeparation /
# RedirectDisabledDrone / IsDisabledDescent /
# IsCrashRedirected / GetFriendlySeparationMeters,
...
MISSION08_NEIGHBORS_NOT_LOCKED = (
    ...
    IS_CRASH_REDIRECTED,
    GET_FRIENDLY_SEPARATION,
    "test_lifeline_hunter_set_friendly_separation_decl_contract.py",
...
# .cpp OpenSensorExposure body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLifelineHunterBoss::OpenSensorExposure",
    "SkyguardLifelineHunterBoss.cpp",
    "SetExposed",
)
SIBLING_TYPES = (
...
class LifelineHunterOpenSensorExposureDeclContractTests(unittest.TestCase):
    def test_lifeline_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, OPEN_SENSOR_EXPOSURE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, OPEN_SENSOR_EXPOSURE)
        self.assertIn("OpenSensorExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, OPEN_SENSOR_EXPOSURE))

    def test_missing_open_sensor_exposure_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLifelineHunterBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, OPEN_SENSOR_EXPOSURE)
        self.assertIn("OpenSensorExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, OPEN_SENSOR_EXPOSURE)
        self.assertIn("OpenSensorExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("UFUNCTION", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("Category", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("BlueprintCallable", OPEN_SENSOR_EXPOSURE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, OPEN_SENSOR_EXPOSURE)
        self.assertIn("OpenSensorExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool OpenSensorExposure;\n"
        wrong_return_void = "\tvoid OpenSensorExposure();\n"
        wrong_return_int = "\tint32 OpenSensorExposure();\n"
        added_const = "\tbool OpenSensorExposure() const;\n"
        added_arg = "\tbool OpenSensorExposure(bool bExposed);\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, OPEN_SENSOR_EXPOSURE)
            self.assertIn("OpenSensorExposure", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_open_sensor_exposure_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, OPEN_SENSOR_EXPOSURE))
        self.assertEqual(
            declaration_count(section, OPEN_SENSOR_EXPOSURE),
            1,
        )
        self.assertTrue(
            OPEN_SENSOR_EXPOSURE.startswith("bool "),
            OPEN_SENSOR_EXPOSURE,
        )
        self.assertTrue(
            OPEN_SENSOR_EXPOSURE.endswith(";"),
            OPEN_SENSOR_EXPOSURE,
        )
        self.assertIn("OpenSensorExposure(", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("INDEX_NONE", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("{", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("}", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("return ", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn(" const", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SetFriendlySeparationMeters", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("RedirectDisabledDrone", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("GetChinMuzzleLocation", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("GetGunnerMount", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("FaceWorldLocation", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("OpticalTracker", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SetContactIdentified", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SetSearchlightTracked", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn(
            "FSkyguardSearchlightTrackRuntime",
            OPEN_SENSOR_EXPOSURE,
        )
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, OPEN_SENSOR_EXPOSURE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{OPEN_SENSOR_EXPOSURE}\n}}\n"
        self.assertTrue(has_declaration(one_line, OPEN_SENSOR_EXPOSURE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, OPEN_SENSOR_EXPOSURE),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{OPEN_SENSOR_EXPOSURE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", OPEN_SENSOR_EXPOSURE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_open_sensor_exposure_cpp_body(
        self,
    ) -> None:
        locked_only = f"{OPEN_SENSOR_EXPOSURE}\n"
        self.assertNotIn("{", OPEN_SENSOR_EXPOSURE)
        ...
        self.assertNotIn("return false", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("return true", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SetExposed", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("OpticalTracker->", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_mission08_neighbors(self) -> None:
        ...
        for token in MISSION08_NEIGHBORS_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SetFriendlySeparationMeters", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("OpticalTracker", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("WeaponServo", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("CountermeasurePod", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("GetGunnerMount", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("GetChinTurret", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("GetSensorTurret", OPEN_SENSOR_EXPOSURE)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{OPEN_SENSOR_EXPOSURE}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("GetChinMuzzleLocation", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("ESkyguardApacheSystem", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("ASkyguardGunner", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("FillAndFinalize", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SkyguardRadarNode", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("USkyguardBriefingWidget", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            OPEN_SENSOR_EXPOSURE,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_SENSOR_EXPOSURE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("ESkyguardMissionSkylineStyle", OPEN_SENSOR_EXPOSURE)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("MinHeightFromOriginCm", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("MaxIntegrity", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SkyguardApacheAircraft.h", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("SkyguardPathfinder", OPEN_SENSOR_EXPOSURE)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{OPEN_SENSOR_EXPOSURE}\n"
        self.assertEqual(
            require_declaration(locked_only, OPEN_SENSOR_EXPOSURE),
            OPEN_SENSOR_EXPOSURE,
        )
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        # header. Do not scan the Apache public section or
        # other headers for Harbor interval retune tokens.
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "lifeline hunter OpenSensorExposure contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, OPEN_SENSOR_EXPOSURE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                OPEN_SENSOR_EXPOSURE.lower(),
                f"lifeline hunter OpenSensorExposure contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, OPEN_SENSOR_EXPOSURE)

    def test_contract_is_open_sensor_exposure_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", OPEN_SENSOR_EXPOSURE)
        self.assertNotIn("{", OPEN_SENSOR_EXPOSURE)
        self.assertTrue(OPEN_SENSOR_EXPOSURE.startswith("bool "))
        self.assertTrue(OPEN_SENSOR_EXPOSURE.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)
