# THIS IS leftover-safe ASkyguardApacheAircraft EngineRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> EngineRight;`
# (no initializer).
...
# Stop BEFORE sibling RadarDome as a claimed slot.
# Do NOT claim EngineLeft / RadarDome / MainRotorCross /
# PilotCanopy / NightVisionTurret / EngineRight / Canopy /
...
# EngineLeft (in-flight), leftover analog
# apache-aircraft-empty-fail-closed (#851b).
# Do NOT parse leftover analog apache-set-sensor-view
# UFUNCTION or leftover analog apache-get-sensor-quality
# UFUNCTION (those are not this TObjectPtr field).
# Type is TObjectPtr<UStaticMeshComponent>, NOT
# TObjectPtr<USceneComponent>.
# Do NOT parse leftover Apache UFUNCTION decls
# (#411-#426), including GetRotorRPM / SetRotorPower /
# AimChinTurret / SetSensorView / IsCanopyGlassCracked.
# Do NOT parse leftover analog apache-aircraft-empty-fail-closed
# (#851b; keep bulk in LOCKED_SCRIPTS).
...
# leftover analog apache-sensor-ball-field,
# leftover analog apache-night-vision-turret-field,
# leftover analog apache-pilot-canopy-field,
# leftover analog apache-engine-left-field,
# leftover analog apache-rotor-mast-field,
# leftover analog apache-main-rotor-field,
# leftover analog apache-tail-rotor-field,
# leftover analog apache-chin-turret-field,
# leftover analog apache-chin-housing-field,
# leftover analog apache-chin-barrel-field,
# leftover analog apache-sensor-turret-field,
# leftover analog apache-vertical-tail-field,
# leftover analog apache-tail-boom-field,
# leftover analog apache-fuselage-field,
...
# EngineLeft / RadarDome / EngineRight / Canopy are sibling
...
# Do NOT parse leftover ArcadeLook UPROPERTY exhausted
# (#1542/#1544-#1551), leftover GuidedLockRules constexprs,
# leftover CampaignMissionSpec, leftover Harbor Breaker
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> EngineRight;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> EngineRight;` and NOT
# leftover `Canopy` / leftover `EngineLeft` / leftover
# `RadarDome` and NOT `float CurrentIntegrity = 140.f;`.
...
TARGET = "TObjectPtr<UStaticMeshComponent> EngineRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> EngineRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> EngineRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> EngineRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> EngineRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> EngineRight;"
...
ENGINE_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+EngineRight\s*;"
)
ENGINE_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+EngineRight\s*="
)
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_RADAR_DOME):
            break
    ...
    if has_identifier(window, SIBLING_RADAR_DOME):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_RADAR_DOME}"
        )
    for later in (
        SIBLING_MAIN_ROTOR_CROSS,
        SIBLING_HORIZONTAL_TAIL,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+EngineRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> EngineRight "
...
class ApacheEngineRightFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EngineRight"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_RADAR_DOME), body)
        self.assertFalse(
            has_identifier(section, SIBLING_RADAR_DOME),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_MAIN_ROTOR_CROSS), section)
        self.assertTrue(has_identifier(section, SIBLING_PILOT_CANOPY), section)
        self.assertFalse(has_identifier(section, SIBLING_RADAR_DOME), section)
        ...
        self.assertFalse(has_identifier(section, SIBLING_HORIZONTAL_TAIL), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EngineRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("EngineRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_engine_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> EngineRight;", compact_origin)
        self.assertNotIn("EngineRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_engine_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> EngineRight"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn(SIBLING_NOSE, LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> EngineRight;")
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> EngineRight;",
        )
        self.assertNotEqual(
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> EngineRights;\n",
            "\tint32 EngineRight = 140;\n",
            "\tbool EngineRight = true;\n",
            "\tfloat EngineRight = " + forty + ";\n",
            "\tfloat EngineRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_radar_dome_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_MAIN_ROTOR_CROSS))
        self.assertFalse(has_identifier(section, SIBLING_HORIZONTAL_TAIL))
        self.assertFalse(has_identifier(section, SIBLING_RADAR_DOME))
        ...
        self.assertNotIn(TARGET_WRONG_RADAR_DOME, section)
        ...
        self.assertNotIn(TARGET_WRONG_MAIN_ROTOR_CROSS, section)
        self.assertNotIn(SIBLING_NIGHT_VISION, LOCKED_DECL)
        self.assertNotIn(SIBLING_STUB_WING_RIGHT, LOCKED_DECL)
        self.assertNotIn(SIBLING_VERTICAL_TAIL, LOCKED_DECL)
        self.assertNotIn(SIBLING_PILOT_CANOPY, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache EngineRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "engine_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_NOSE, locked_only)
