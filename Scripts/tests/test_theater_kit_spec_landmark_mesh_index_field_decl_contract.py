# THIS IS leftover-safe FSkyguardTheaterKitSpec LandmarkMeshIndex.
...
# Category="Skyguard|Campaign", retarget: type is int32,
# default is `= 0`, Category is Skyguard|Theater.
...
TARGET = "int32 LandmarkMeshIndex = 0;"
TARGET_WRONG_EQ_NONE = "int32 LandmarkMeshIndex = NAME_None;"
TARGET_WRONG_FALSE = "int32 LandmarkMeshIndex = false;"
TARGET_WRONG_TRUE = "int32 LandmarkMeshIndex = true;"
TARGET_WRONG_ZERO = "int32 LandmarkMeshIndex = 0.f;"
TARGET_WRONG_FLOAT = "float LandmarkMeshIndex;"
TARGET_WRONG_BARE = "int32 LandmarkMeshIndex;"
TARGET_WRONG_ONE = "int32 LandmarkMeshIndex = 1;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 LandmarkMeshIndex = 0;`.
    # Do not accept bare `int32 LandmarkMeshIndex;`, `= 0.f`,
    # `= 160.f`, leftover float Health, FName, or harbor 40/80
    # when origin/main has int32 default 0. Do not accept sibling
    # WeatherIdentity / KitId / LandmarkSet / BuildingKit fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+LandmarkMeshIndex\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+LandmarkMeshIndex\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LandmarkMeshIndex\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+LandmarkMeshIndex\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+LandmarkMeshIndex\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+LandmarkMeshIndex\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 LandmarkMeshIndex is missing from "
...
class TheaterKitSpecLandmarkMeshIndexFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LandmarkMeshIndex"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LandmarkMeshIndex"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LandmarkMeshIndex", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_landmark_mesh_index_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_missing_default_fails_closed_when_origin_is_int32_zero(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
        ...
        self.assertIn("int32 LandmarkMeshIndex = 0;", compact_origin)
        self.assertNotIn("LandmarkMeshIndex = NAME_None", compact_origin)
        self.assertNotIn("LandmarkMeshIndex = false", compact_origin)
        self.assertNotIn("LandmarkMeshIndex = true", compact_origin)
        self.assertNotIn("LandmarkMeshIndex = 0.f", compact_origin)
        self.assertNotIn("LandmarkMeshIndex = 160.f", compact_origin)

    def test_landmark_mesh_index_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 LandmarkMeshIndex"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            "\tint32 LandmarkMeshIndexes;\n",
            "\tFName LandmarkMeshIndex;\n",
            "\tbool LandmarkMeshIndex;\n",
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_ONE}\n",
            "\tfloat LandmarkMeshIndex = " + forty + ";\n",
            "\tfloat LandmarkMeshIndex = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec LandmarkMeshIndex field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "landmark_mesh_index_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
