...
HEADER_PATH = "Source/Skyguard52/SkyguardRunwayBreakerBoss.h"
CLASS_NAME = "ASkyguardRunwayBreakerBoss"
# Field-declaration presence only. Do not invent
# INDEX_NONE or lock DebrisPortWing construction
# in the .cpp. This is a FIELD contract on
# ASkyguardRunwayBreakerBoss, not leftover
# RunwayBreaker sibling fields RunwayRack /
# HangarRack / HeatManifold / PortEngine /
# DebrisPayloadBay / DebrisEngine and not leftover
# emergency-finish named surfaces
# (split tokens only). Keep RunwayBreaker
# DebrisPortWing distinct from leftover IronRain
# DebrisPortWing (PR #531, different class).
# Keep RunwayBreaker DebrisEngine / PortEngine
# distinct from leftover RoadHunter Engine /
# DebrisEngine and leftover Breakwater Engine /
# DebrisEngine (different classes; do not lock
# those here). Sibling fields RunwayRack,
# HangarRack, HeatManifold, PortEngine,
# DebrisPayloadBay, DebrisEngine must not
# satisfy the primary lock.
...
# DebrisPortWing;`); accept that form and other
...
# section of ASkyguardRunwayBreakerBoss only.
...
# isolated-test drafts #107–#555 including
...
# scanned in the RunwayBreaker public section and
...
# leftover #107–#556, plus
# SkyguardRunwayBreakerBoss production files.
# This lane only adds an isolated Python
# DebrisPortWing field declaration contract on
# ASkyguardRunwayBreakerBoss. Stay off leftover
...
# leftover isolated-test drafts #107–#556,
# leftover searchlight-track-runtime-defaults
...
LOCKED = {
    "SkyguardRunwayBreakerBoss.h",
    "SkyguardRunwayBreakerBoss.cpp",
...
# unlocked for THIS test's primary DebrisPortWing lock. Leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRunwayBreakerBoss::DebrisPortWing",
    "SkyguardRunwayBreakerBoss.cpp",
    "SetExposed",
...
class RunwayBreakerDebrisPortWingFieldDeclContractTests(unittest.TestCase):
    def test_runway_breaker_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_PORT_WING_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            "class SKYGUARD52_API AOtherRunwayBreakerBoss "
            ": public AActor\n"
            "{\n"
            "public:\n"
            f"\t{DEBRIS_PORT_WING_FIELD}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_PORT_WING_FIELD)
    ...
    def test_missing_debris_port_wing_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRunwayBreakerBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("Category", DEBRIS_PORT_WING_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DEBRIS_PORT_WING_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisPortWing;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisPortWing;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> DebrisPortWing;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_PORT_WING_FIELD)
            self.assertIn("DebrisPortWing", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_port_wing_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_PORT_WING_FIELD),
            DEBRIS_PORT_WING_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_PORT_WING_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_PORT_WING_FIELD), 1)
        self.assertTrue(
            DEBRIS_PORT_WING_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_PORT_WING_FIELD,
        )
        self.assertTrue(DEBRIS_PORT_WING_FIELD.endswith(";"), DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("HeatManifold", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("RunwayRack", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisPayloadBay", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("LeftActuator", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("RightActuator", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisLeftWing", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisRightWing", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisCoolingDoor", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("RadarReceiver", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("CoolingDoor", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_PORT_WING_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PORT_WING_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("TArray<", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("=", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("{", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("}", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("return ", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisCenter", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("RadarNode", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_PORT_WING_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_PORT_WING_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {DEBRIS_PORT_WING_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DEBRIS_PORT_WING_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DEBRIS_PORT_WING_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_PORT_WING_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_PORT_WING_FIELD),
            section,
    ...
    def test_assigned_debris_port_wing_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisPortWing = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_PORT_WING_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DebrisPortWing", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_PORT_WING_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_PORT_WING_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, DEBRIS_PORT_WING_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_PORT_WING_FIELD.startswith("UFUNCTION"),
            DEBRIS_PORT_WING_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_PORT_WING_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_PORT_WING_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_port_wing_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        self.assertNotIn("{", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("}", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("return ", DEBRIS_PORT_WING_FIELD)
        ...
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("return true", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_PORT_WING_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_PORT_WING_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisIntakePanel", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_PORT_WING_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PORT_WING_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DispenserCenter", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("IsContactIdentified", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), DEBRIS_PORT_WING_FIELD)

    def test_debris_cooling_door_does_not_satisfy_debris_port_wing(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PORT_WING_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, DEBRIS_PORT_WING_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DispenserStarboard", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetManeuver", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), DEBRIS_PORT_WING_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        self.assertNotIn("CoolingDoor", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("IsLightningExposed", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DEBRIS_PORT_WING_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_PORT_WING_FIELD),
            DEBRIS_PORT_WING_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, DEBRIS_PORT_WING_FIELD), 1)
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", section)
        self.assertNotIn("ASkyguardRunwayBreakerBoss::DebrisPortWing", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "runway breaker DebrisPortWing field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_PORT_WING_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_PORT_WING_FIELD.lower(),
                "runway breaker DebrisPortWing contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_PORT_WING_FIELD)

    def test_contract_is_debris_port_wing_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DEBRIS_PORT_WING_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_PORT_WING_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        self.assertNotIn("DebrisEngine", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisRightWing", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardRoadHunterBoss", DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", DEBRIS_PORT_WING_FIELD)
        self.assertNotEqual(DEBRIS_PORT_WING_FIELD, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("StarboardLatch", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DecoyPods", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ElevatorLinkage", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", DEBRIS_PORT_WING_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_runway_breaker_port_engine_is_not_breakwater_engine(self) -> None:
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        self.assertNotEqual(DEBRIS_PORT_WING_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_PORT_WING_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(collapsed(DEBRIS_PORT_WING_FIELD), collapsed(ENGINE_FIELD))
        self.assertNotEqual(DEBRIS_PORT_WING_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertEqual(BREAKWATER_ENGINE_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardRunwayBreakerBoss")
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardLastFlightBoss", DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DEBRIS_PORT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("OpticalTracker", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("WeaponServo", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("CountermeasurePod", DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_PORT_WING_FIELD)
    ...
    def test_runway_breaker_port_engine_is_not_pathfinder_engine(self) -> None:
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        self.assertNotEqual(
            collapsed(DEBRIS_PORT_WING_FIELD),
    ...
    def test_hangar_rack_does_not_satisfy_debris_port_wing(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PORT_WING_FIELD))
        self.assertNotEqual(HANGAR_RACK_FIELD, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_PORT_WING_FIELD)

    def test_heat_manifold_and_runway_rack_do_not_satisfy_debris_port_wing(
        self,
    ) -> None:
        for region, name in (
            (f"\t{HEAT_MANIFOLD_FIELD}\n", "HeatManifold"),
    ...
    def test_runway_breaker_sibling_fields_do_not_satisfy_debris_port_wing(
        self,
    ) -> None:
        for region, name in (
            (f"\t{PORT_ENGINE_FIELD}\n", "PortEngine"),
    ...
    def test_debris_engine_does_not_satisfy_debris_port_wing(self) -> None:
    ...
    def test_targeting_camera_does_not_satisfy_debris_port_wing(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PORT_WING_FIELD))
        self.assertNotEqual(TARGETING_CAMERA_FIELD, DEBRIS_PORT_WING_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_PORT_WING_FIELD)
        self.assertIn("DebrisPortWing", DEBRIS_PORT_WING_FIELD)

    def test_left_and_right_actuators_do_not_satisfy_debris_port_wing(self) -> None:
        for region, name in (
            (f"\t{LEFT_ACTUATOR_FIELD}\n", "LeftActuator"),
            (f"\t{RIGHT_ACTUATOR_FIELD}\n", "RightActuator"),
        ):
    ...
    def test_runway_breaker_debris_port_wing_is_not_iron_rain_debris_port_wing(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        self.assertEqual(DEBRIS_PORT_WING_FIELD, IRON_RAIN_DEBRIS_PORT_WING_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardRunwayBreakerBoss")
        ...
        self.assertNotIn("ASkyguardIronRainBoss", DEBRIS_PORT_WING_FIELD)
    ...
    def test_port_engine_does_not_satisfy_debris_port_wing(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PORT_WING_FIELD))
        self.assertNotEqual(PORT_ENGINE_FIELD, DEBRIS_PORT_WING_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_ENGINE_FIELD,
        )
    ...
    def test_runway_breaker_port_engine_is_not_last_flight_port_engine(
        ...
        locked_only = f"{DEBRIS_PORT_WING_FIELD}\n"
        self.assertNotEqual(DEBRIS_PORT_WING_FIELD, LAST_FLIGHT_PORT_ENGINE_FIELD)
