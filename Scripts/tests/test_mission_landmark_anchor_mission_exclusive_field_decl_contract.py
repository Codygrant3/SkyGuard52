# THIS IS leftover-safe FSkyguardMissionLandmarkAnchor
# bMissionExclusive.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then `bool bMissionExclusive = false;`.
...
# (sibling struct; leftover ObjectiveId isolated decl is
# a different lane).
...
# FName LandmarkId with no initializer,
# retarget: type is bool bMissionExclusive = false,
# specifiers are EditAnywhere + BlueprintReadOnly,
# no Category. The initializer `= false` is required.
...
TARGET = "bool bMissionExclusive = false;"
TARGET_WRONG_BARE = "bool bMissionExclusive;"
TARGET_WRONG_EQ_NONE = "bool bMissionExclusive = NAME_None;"
TARGET_WRONG_FALSE = "bool bMissionExclusive = False;"
TARGET_WRONG_TRUE = "bool bMissionExclusive = true;"
TARGET_WRONG_ONE = "bool bMissionExclusive = 1;"
TARGET_WRONG_ZERO = "bool bMissionExclusive = 0;"
TARGET_WRONG_FLOAT = "float bMissionExclusive;"
TARGET_WRONG_FNAME = "FName bMissionExclusive;"
TARGET_WRONG_INT = "int32 bMissionExclusive = false;"
...
TARGET_WRONG_TEXT = "FText bMissionExclusive;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # LandmarkId / Role / WorldLocation fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"bool\s+bMissionExclusive\s*=\s*false\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"bool\s+bMissionExclusive\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bMissionExclusive\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bMissionExclusive\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bMissionExclusive\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+bMissionExclusive\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bMissionExclusive\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bMissionExclusive is missing from "
...
def require_mission_exclusive_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} bMissionExclusive UPROPERTY"
        )
...
class MissionLandmarkAnchorMissionExclusiveFieldDeclContractTests(unittest.TestCase):
    def test_landmark_anchor_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bMissionExclusive"), section)
    ...
    def test_objective_or_readiness_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bMissionExclusive"), section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_ANCHOR, section)
        self.assertNotIn(STOP_BEFORE_MAP_READINESS, section)
        self.assertNotIn(ACTOR_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("bMissionExclusive", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_mission_exclusive_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_LANDMARK_ID};\n"
            f"\tFName {SIBLING_ROLE};\n"
            f"\tFVector {SIBLING_WORLD_LOCATION} = FVector::ZeroVector;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_mission_exclusive_specifiers(section)
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(spec_section(flipped), LOCKED_DECL)
        self.assertIn("bMissionExclusive", str(raised.exception))
        ...
        self.assertIn("bool bMissionExclusive = false;", compact_origin)
        self.assertNotIn("bMissionExclusive = NAME_None", compact_origin)
        self.assertNotIn("bMissionExclusive = true", compact_origin)
        self.assertNotIn("bMissionExclusive = 0", compact_origin)
        self.assertNotIn("bMissionExclusive = 1", compact_origin)
        self.assertNotIn("bMissionExclusive = 160.f", compact_origin)

    def test_mission_exclusive_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bMissionExclusive"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFName {SIBLING_ROLE};\n",
            f"\tFVector {SIBLING_WORLD_LOCATION} = FVector::ZeroVector;\n",
            "\tbool bMissionExclusiv = false;\n",
            "\tbool bMissionExclusives = false;\n",
            "\tint32 bMissionExclusive = false;\n",
            "\tFName bMissionExclusive;\n",
            "\tFText bMissionExclusive;\n",
            "\tfloat bMissionExclusive = " + forty + ";\n",
            "\tfloat bMissionExclusive = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_mission_exclusive_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_mission_exclusive_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_mission_exclusive_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_mission_exclusive_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_mission_exclusive_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-landmark-anchor bMissionExclusive field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_landmark_anchor_mission_exclusive_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionMapAssemblyDirector.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
