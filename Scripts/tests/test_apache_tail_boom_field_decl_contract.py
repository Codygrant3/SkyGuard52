# THIS IS leftover-safe ASkyguardApacheAircraft TailBoom.
...
# then BARE `TObjectPtr<UStaticMeshComponent> TailBoom;`
# (no initializer).
...
# Stop BEFORE sibling VerticalTail as a claimed slot.
# Do NOT claim Canopy / Nose / Fuselage / SilhouetteMesh /
# AircraftRoot / RotorPower / HoverBobCentimeters /
# MaxIntegrity / CurrentIntegrity / HullCollider / TailRotor.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> TailBoom;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> TailBoom;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> TailBoom = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> TailBoom = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> TailBoom = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> TailBoom;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> TailBoom;"
...
STOP_BEFORE_VERTICAL_TAIL = "VerticalTail"
...
TAIL_BOOM_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+TailBoom\s*;"
)
TAIL_BOOM_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+TailBoom\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer on TailBoom as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if TAIL_BOOM_DECL_RE.search(compact) is None:
        return False
    if TAIL_BOOM_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_VERTICAL_TAIL):
            break
    ...
    if has_identifier(window, SIBLING_VERTICAL_TAIL):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_VERTICAL_TAIL}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+TailBoom\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> TailBoom "
...
class ApacheTailBoomFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "TailBoom"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_VERTICAL_TAIL),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "TailBoom"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("TailBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_tail_boom_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> TailBoom;", compact_origin)
        self.assertNotIn("TailBoom = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_tail_boom_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> TailBoom"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> TailBoom;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> TailBooms;\n",
            "\tint32 TailBoom = 140;\n",
            "\tbool TailBoom = true;\n",
            "\tfloat TailBoom = " + forty + ";\n",
            "\tfloat TailBoom = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_vertical_tail_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_VERTICAL_TAIL))
        ...
        self.assertNotIn(TARGET_WRONG_VERTICAL_TAIL, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_VERTICAL_TAIL))
        self.assertFalse(has_identifier(section, SIBLING_VERTICAL_TAIL))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache TailBoom field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "tail_boom_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
