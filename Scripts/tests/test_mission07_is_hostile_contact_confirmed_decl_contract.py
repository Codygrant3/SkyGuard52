...
# INDEX_NONE or lock IsHostileContactConfirmed
# construction in the .cpp. This is leftover-safe
# Mission07 Search IsHostileContactConfirmed METHOD on
# ASkyguardMission07IntegrationDirector. It is
# NOT leftover ClassifyFalseTrack #710, NOT leftover
# search-sector enum #b4d4, NOT leftover
# search-track-runtime-defaults #8266, NOT leftover
# Mission07 GetSearchSector, NOT leftover
# Mission07 GetReadiness, NOT leftover
# Mission06 GetReadiness #697, NOT leftover Mission05
# GetReadiness, NOT leftover Mission01 / Mission03 /
# Mission04 GetReadiness,
# NOT leftover env-readiness
# #6b9d/#b931, NOT leftover Mission07
# NotifyThreatDestroyed, NOT leftover Mission07
# InitializePlayableMission, NOT leftover
# Mission07 StartNextWave, NOT leftover
...
# IsCorePlayableReady / GetObjectiveRuntime /
# GetReadiness / GetWaveState / GetRemainingThreatsInWave /
# GetSearchSector / GetPayloadWindow / GetTargetRuntime /
...
# leftover Mission07 GetReadiness,
...
# Category is Skyguard|Mission07|Search, not
# Mission01 / Mission02 / Mission03 / Mission04 /
# Mission05 / Mission06 / Waves / Integration /
...
# Waves #669, not leftover Harbor Mission02, not leftover
# Mission07 Integration.
...
# an isolated Python IsHostileContactConfirmed
# method declaration contract on
# ASkyguardMission07IntegrationDirector.
# Do not lock leftover GetSearchSector.
LOCKED = {
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission07IntegrationDirector::IsHostileContactConfirmed",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission07IsHostileContactConfirmedDeclContractTests(unittest.TestCase):
    def test_mission07_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, IS_HOSTILE_CONTACT_CONFIRMED),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{IS_HOSTILE_CONTACT_CONFIRMED}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn("IsHostileContactConfirmed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_missing_is_hostile_contact_confirmed_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission07IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn("IsHostileContactConfirmed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_SEARCH}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn("IsHostileContactConfirmed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_SEARCH, section)
        ...
        self.assertNotIn("UFUNCTION", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("BlueprintPure", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("Category", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("BlueprintCallable", UFUNCTION_SEARCH)
        self.assertIn("Skyguard|Mission07|Search", UFUNCTION_SEARCH)
        self.assertNotIn("Mission01", UFUNCTION_SEARCH)
        self.assertNotIn("Mission03", UFUNCTION_SEARCH)
        self.assertNotIn("Mission04", UFUNCTION_SEARCH)
        self.assertNotIn("Mission05", UFUNCTION_SEARCH)
        self.assertNotIn("Mission06", UFUNCTION_SEARCH)
        self.assertNotIn("Waves", UFUNCTION_SEARCH)
        self.assertNotIn("Integration", UFUNCTION_SEARCH)
        self.assertNotIn("Protection", UFUNCTION_SEARCH)
        self.assertNotIn("Payload", UFUNCTION_SEARCH)
        self.assertNotIn("Targets", UFUNCTION_SEARCH)
        self.assertNotIn("Objectives", UFUNCTION_SEARCH)
        self.assertIn("BlueprintPure", UFUNCTION_SEARCH)
        self.assertNotIn("Environment", UFUNCTION_SEARCH)
        self.assertNotIn("Briefing", UFUNCTION_SEARCH)
        self.assertNotIn("Mission02", UFUNCTION_SEARCH)
        self.assertNotIn("Boss", UFUNCTION_SEARCH)
        self.assertNotIn("Destruction", UFUNCTION_SEARCH)
        self.assertNotIn("Apache", UFUNCTION_SEARCH)
        self.assertNotIn("Mission06", UFUNCTION_SEARCH)
        self.assertNotIn("Mission08", UFUNCTION_SEARCH)
        self.assertNotIn("Mission10", UFUNCTION_SEARCH)
        self.assertNotIn("Encounter", UFUNCTION_SEARCH)
        self.assertNotIn("Safety", UFUNCTION_SEARCH)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_SEARCH)
            self.assertNotIn(invented, IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn("IsHostileContactConfirmed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid IsHostileContactConfirmed() const;\n"
        )
        as_const = (
            "\tESkyguardSearchSector IsHostileContactConfirmed();\n"
        )
        with_arg = (
            "\tESkyguardSearchSector IsHostileContactConfirmed(int32 Amount) const;\n"
        )
        renamed = (
            "\tESkyguardSearchSector GetSearchState() const;\n"
        )
        short_name = "\tESkyguardSearchSector IsHostileContactConfirmed() const;\n"
        leftover_m06_type = (
            "\tESkyguardMission06SearchSector IsHostileContactConfirmed() const;\n"
        )
        leftover_m05_type = (
            "\tESkyguardMission05SearchSector IsHostileContactConfirmed() const;\n"
        )
        leftover_m04_type = (
            "\tESkyguardMission04SearchSector IsHostileContactConfirmed() const;\n"
        )
        leftover_m01_type = (
            "\tESkyguardMission01SearchSector IsHostileContactConfirmed() const;\n"
        )
        leftover_env_type = (
            "\tESkyguardMission01EnvironmentSearchSector IsHostileContactConfirmed() const;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_HOSTILE_CONTACT_CONFIRMED)
            self.assertIn("IsHostileContactConfirmed", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_hostile_contact_confirmed_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, IS_HOSTILE_CONTACT_CONFIRMED),
            IS_HOSTILE_CONTACT_CONFIRMED,
        )
        self.assertTrue(has_declaration(section, IS_HOSTILE_CONTACT_CONFIRMED))
        self.assertEqual(
            declaration_count(section, IS_HOSTILE_CONTACT_CONFIRMED),
            1,
        )
        self.assertTrue(
            IS_HOSTILE_CONTACT_CONFIRMED.startswith("bool"),
            IS_HOSTILE_CONTACT_CONFIRMED,
        )
        self.assertTrue(
            IS_HOSTILE_CONTACT_CONFIRMED.startswith("bool"),
        ...
        self.assertIn("bool", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn("IsHostileContactConfirmed", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn(";", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn(" const", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn("()", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("TArray<", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("TObjectPtr", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("TSoftObjectPtr", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("INDEX_NONE", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("UFUNCTION", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("{", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("}", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("return ", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn(" const", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("Root", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("Briefing", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AudioDirector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("RadioChatter", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("SortiePresentation", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("CampaignDefinition", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("MissionDefinition", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("Readiness;", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("bAutoInitialize", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("bAllowBoundedActorSpawning", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("bAutoLaunchAfterBriefing", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("PathfinderSpawnLocation", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetAircraft", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("BindRuntimeActors", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("HandleDroneCityImpact", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("InitializePlayableMission", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("StartNextWave", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AdvanceConvoyByDistance", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("NotifyConvoyDamage", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetWaveState", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetConvoyRouteState", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetDayBeatKit", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetNightBeatKit", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("StartSearchlightWindow", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AdvanceSearchlightTrack", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("NotifySubstationDamage", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetRemainingThreatsInWave", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetSearchlightRuntime", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetSubstationIntegrity", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("StartPayloadWindow", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AdvancePayloadWindow", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("TryJamActivePayload", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("NotifyAirfieldTargetDamage", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetPayloadWindow", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetTargetRuntime", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetSurvivingTargetCount", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetDayBeatKind", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetDayBeatIndex", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("TickDayBeatKit", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("HandleBossPhaseChanged", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ESkyguardMission06WaveState", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ESkyguardMission07WaveState", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("FSkyguardMission07IntegrationReadiness", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("FSkyguardSearchTrackRuntime", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ClassifyFalseTrack", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ConfirmRadarGhostIdentification", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("NotifyProtectedTargetDamage", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AdvanceReinforcementTimer", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetReadiness", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetClassifiedFalseTrackCount", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetSearchSector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetProtectedTarget", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetReinforcementTimeRemaining", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetNightBeatKind", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetNightBeatIndex", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("TickNightBeatKit", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ESkyguardAirfieldTarget", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ConvoyRuntimeAnchor", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("RoadHunterSpawnLocation", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ConfigureMissionDefinition", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("NotifyObjectiveProgress", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("NotifyProtectedAssetFailed", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("SynchronizeRuntimeState", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("NotifyThreatDestroyed", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("IsCorePlayableReady", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetObjectiveRuntime", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetGunner", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetPathfinder", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetMissionId", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ValidateMissionContract", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ConfigureFromMission", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AdvanceBriefing", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("SetAssetsReady", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AcknowledgeAndLaunch", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("CanLaunch", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetElapsedSeconds", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetBriefingState", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetMinimumWarmupSeconds", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetBriefingText", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetRadioChatter", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetPresentation", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("GetMissionTitle", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("AcknowledgeBriefing", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("LaunchSortie", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("HullCollider", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("OpticalTracker", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("WeaponServo", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("CountermeasurePod", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("MinHeightFromOriginCm", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("RadarNode", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ESkyguardMission02WaveState", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("HarborIndustrial", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("MaxIntegrity", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("CurrentIntegrity", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("FillAndFinalize", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("FillAndFail", IS_HOSTILE_CONTACT_CONFIRMED)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tIsHostileContactConfirmed() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_name = (
            "public:\n"
            "\tbool IsHostileContactConfirmed(\n"
            "\t\t) const;\n"
            "};\n"
        )
        wrap_origin_body = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SEARCH} {IS_HOSTILE_CONTACT_CONFIRMED}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{IS_HOSTILE_CONTACT_CONFIRMED}\n}}\n"
        self.assertTrue(has_declaration(one_line, IS_HOSTILE_CONTACT_CONFIRMED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, IS_HOSTILE_CONTACT_CONFIRMED),
            section,
    ...
    def test_environment_category_does_not_satisfy_search(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_SEARCH)
        self.assertNotIn("Briefing", UFUNCTION_SEARCH)
        environment = (
        ...
        self.assertNotEqual(environment, UFUNCTION_SEARCH)
        self.assertNotEqual(briefing, UFUNCTION_SEARCH)
        self.assertNotIn(environment, UFUNCTION_SEARCH)
        self.assertNotIn(briefing, UFUNCTION_SEARCH)
        leftover_m01 = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m02, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_harbor, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m03, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m04, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m05, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m06, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_waves, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m05_waves, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_integration, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_payload, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_targets, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_objectives, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_m01, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_m02, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_harbor, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_m03, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_m04, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_m05, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_m06, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_m05_waves, UFUNCTION_SEARCH)
        self.assertNotIn(leftover_integration, UFUNCTION_SEARCH)
        self.assertNotIn("Mission01", UFUNCTION_SEARCH)
        self.assertNotIn("Mission03", UFUNCTION_SEARCH)
        self.assertNotIn("Mission04", UFUNCTION_SEARCH)
        self.assertNotIn("Mission05", UFUNCTION_SEARCH)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{IS_HOSTILE_CONTACT_CONFIRMED}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", IS_HOSTILE_CONTACT_CONFIRMED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_hostile_contact_confirmed_cpp_body(self) -> None:
        locked_only = f"{IS_HOSTILE_CONTACT_CONFIRMED}\n"
        self.assertNotIn("{", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("}", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("return ", IS_HOSTILE_CONTACT_CONFIRMED)
        ...
        self.assertNotIn("return false", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("return true", IS_HOSTILE_CONTACT_CONFIRMED)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{IS_HOSTILE_CONTACT_CONFIRMED}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn("IsHostileContactConfirmed", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertTrue(IS_HOSTILE_CONTACT_CONFIRMED.startswith("bool"))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("CurrentIntegrity", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("SkyguardApacheAircraft.h", IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_HOSTILE_CONTACT_CONFIRMED}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_HOSTILE_CONTACT_CONFIRMED),
            IS_HOSTILE_CONTACT_CONFIRMED,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", IS_HOSTILE_CONTACT_CONFIRMED)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 IsHostileContactConfirmed method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{IS_HOSTILE_CONTACT_CONFIRMED}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, IS_HOSTILE_CONTACT_CONFIRMED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                IS_HOSTILE_CONTACT_CONFIRMED.lower(),
                "mission07 IsHostileContactConfirmed contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_HOSTILE_CONTACT_CONFIRMED)

    def test_contract_is_is_hostile_contact_confirmed_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_is_hostile_contact_confirmed(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, IS_HOSTILE_CONTACT_CONFIRMED)
            )
            self.assertNotEqual(IS_HOSTILE_CONTACT_CONFIRMED, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("IsHostileContactConfirmed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, IS_HOSTILE_CONTACT_CONFIRMED))
        locked_only = f"{IS_HOSTILE_CONTACT_CONFIRMED}\n"
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("HandleBossPhaseChanged", IS_HOSTILE_CONTACT_CONFIRMED)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_SEARCH)
    ...
    def test_leftover_environment_is_hostile_contact_confirmed_does_not_satisfy(self) -> None:
        leftover = (
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission07_is_hostile_contact_confirmed"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_is_hostile_contact_confirmed"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_mission01_is_hostile_contact_confirmed_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission01IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission03_is_hostile_contact_confirmed_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission03IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission04_is_hostile_contact_confirmed_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission04IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission05_is_hostile_contact_confirmed_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission05IntegrationDirector "
            ": public AActor\n"
            "{\n"
    ...
    def test_leftover_mission06_is_hostile_contact_confirmed_does_not_satisfy(self) -> None:
        leftover = (
    ...
    def test_contract_does_not_relock_leftover_search_sector(self) -> None:
        ...
        self.assertNotIn("ESkyguardSearchSector", UFUNCTION_SEARCH)
        self.assertIn("Search", UFUNCTION_SEARCH)
        self.assertNotIn("Integration", UFUNCTION_SEARCH)
