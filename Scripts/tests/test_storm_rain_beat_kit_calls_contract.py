...
LOCKED_CALLS = "const TCHAR* Calls[BeatCount] = {};"
# Leftover #56–#64 plus StormRainBeatKit production sources. This lane
# only adds an isolated Python Calls[BeatCount] contract.
LOCKED = {
...
# Storm-rain-beat-kind enum (#245), kit defaults (#248), Kinds (#opening),
# Threats (in-flight), Stations (sibling spawn), day/night Beats[7]
...
# Apache CPG storm-rain kit Calls[BeatCount] only. BeatCount is the
# array bound identifier. Do not re-lock BeatCount=7 / Weather=Storm /
# bHydraForClusters=true (#248). Do not invent call-string contents.
DEFAULTS_NOT_LOCKED = (
...
def calls_declaration(body: str) -> str:
    match = re.search(
        r"const\s+TCHAR\*\s+Calls\[BeatCount\]\s*=\s*\{\};",
        body,
    )
    if match is None:
        raise AssertionError(
            f"const TCHAR* Calls[BeatCount] is missing from "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME}"
        )
    return match.group(0)


def calls_array_bound(body: str) -> str:
    match = re.search(
        r"const\s+TCHAR\*\s+Calls\[([A-Za-z_][A-Za-z0-9_]*)\]\s*=\s*\{\};",
        body,
    )
    if match is None:
        raise AssertionError(
...
class StormRainBeatKitCallsContractTests(unittest.TestCase):
    ...
    def test_missing_calls_declaration_fails_closed(self) -> None:
        ...
        self.assertIn("Calls[BeatCount]", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(STRUCT_NAME, str(raised.exception))

    def test_calls_is_declared_as_beatcount_array(self) -> None:
        body = kit_body(origin_main_header())
        self.assertEqual(calls_declaration(body), LOCKED_CALLS)
        self.assertIn(LOCKED_CALLS, body)
        self.assertEqual(calls_array_bound(body), "BeatCount")
        self.assertNotEqual(calls_array_bound(body), "7")
        self.assertNotEqual(calls_array_bound(body), "INDEX_NONE")
        self.assertNotIn("Calls[7]", body)
        self.assertNotIn("Calls[6]", body)
        self.assertNotIn("Calls[8]", body)
        self.assertNotIn("Calls[INDEX_NONE]", body)
    ...
    def test_calls_declaration_does_not_invent_call_strings(self) -> None:
        declaration = calls_declaration(kit_body(origin_main_header()))
    ...
    def test_contract_does_not_lock_unlocked_members(self) -> None:
        ...
        for line in UNLOCKED_ARRAY_LINES:
            self.assertNotEqual(declaration, line)
            self.assertNotIn(line, declaration)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        body = kit_body(origin_main_header())
        declaration = calls_declaration(body)
        self.assertEqual(declaration, LOCKED_CALLS)
    ...
    def test_struct_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                ...
                "storm-rain beat-kit Calls[BeatCount] is Apache CPG "
                "Hydra/Storm identity, not Yak",
            )

    def test_contract_is_storm_rain_beat_kit_calls_only(self) -> None:
        header = origin_main_header()
        body = kit_body(header)
        declaration = calls_declaration(body)
        self.assertIn(f"struct {STRUCT_NAME}", header)
        ...
        self.assertNotIn("enum class", body)
        self.assertNotIn("namespace SkyguardStormRainBeatKits", body)
        self.assertNotEqual(declaration, "const TCHAR* Rifle[BeatCount] = {};")
