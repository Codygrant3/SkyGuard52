...
# then BARE `void EnsureDefaultEntries();`
# (declaration-only, no inline body required).
# THIS IS leftover-safe isolated data-asset UFUNCTION.
...
# Stop BEFORE first public UPROPERTY (`Entries`).
# Do NOT claim sibling UFUNCTION
# InitializeRequiredEntries as this slot
# (leave it uncontracted).
# Do NOT claim sibling EvaluateReadiness /
...
# UFUNCTION AreResolvedProductionLoopRoutesComplete /
...
# (NO spaces around =).
# Better Production UFUNCTION leftover unique
# AreResolvedProductionLoopRoutesComplete was BlueprintPure
# bool const — RETARGET class -> USkyguardAudioProductionBank,
# wrap BlueprintPure -> BlueprintCallable, method ->
# void EnsureDefaultEntries(); (no const).
...
# LOCKED_DECL is `void EnsureDefaultEntries();`
...
TARGET = "void EnsureDefaultEntries();"
TARGET_INLINE = (
    "void EnsureDefaultEntries() { }"
)
TARGET_WRONG_BARE = "void EnsureDefaultEntries() const;"
TARGET_WRONG_NO_CONST = (
    "bool EnsureDefaultEntries();"
)
TARGET_WRONG_VOID = "bool AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_FLOAT = "float EnsureDefaultEntries();"
TARGET_WRONG_INT = "int32 EnsureDefaultEntries();"
...
TARGET_WRONG_TRUE = "bool EnsureDefaultEntries = true;"
TARGET_WRONG_ZERO = "bool EnsureDefaultEntries = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bvoid\s+EnsureDefaultEntries\s*\(\s*\)\s*(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+EnsureDefaultEntries\s*\(\s*\)\s*const\b", compact):
        return False
    if re.search(r"\bbool\s+EnsureDefaultEntries\b", compact):
        return False
    if re.search(r"\bfloat\s+EnsureDefaultEntries\b", compact):
        return False
    if re.search(r"\bint32\s+EnsureDefaultEntries\b", compact):
        return False
    if re.search(
        r"\bvoid\s+EnsureDefaultEntries\s*\(\s*[^)]+\s*\)",
        compact,
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+EnsureDefaultEntries\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void EnsureDefaultEntries "
...
class AudioProductionBankEnsureDefaultEntriesDeclContractTests(
    unittest.TestCase
):
    def test_audio_production_bank_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "EnsureDefaultEntries"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, "HasValidSha256"), body)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "void EnsureDefaultEntries()const;",
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid EnsureDefaultEntriess();\n",
            "\tint32 EnsureDefaultEntries = 1;\n",
            "\tFName EnsureDefaultEntries = NAME_None;\n",
            "\tvoid EnsureDefaultEntries() = "
            + forty
            + ";\n",
            "\tvoid EnsureDefaultEntries() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "EnsureDefaultEntries")
        )
        self.assertTrue(
            has_identifier(section, "EnsureDefaultEntries")
        )
        ...
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, "HasValidSha256"))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_production_bank_ensure_default_entries"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioProductionBank.cpp", THIS_SCRIPT)
