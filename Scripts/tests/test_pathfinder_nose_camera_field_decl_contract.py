...
# INDEX_NONE or lock NoseCamera construction
...
# NoseCamera;`); accept that form and other
...
# ControlLinkage, DebrisNose, DebrisCenter,
...
# isolated-test drafts #107–#500 including
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardPathfinderBoss::NoseCamera",
    "SkyguardPathfinderBoss.cpp",
    "SetExposed",
...
class PathfinderNoseCameraFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, NOSE_CAMERA_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, NOSE_CAMERA_FIELD)
        self.assertIn("NoseCamera", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, NOSE_CAMERA_FIELD)
    ...
    def test_missing_nose_camera_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardPathfinderBoss();\n"
            f"\t{COMMAND_ANTENNA_FIELD}\n"
            f"\t{ENGINE_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, NOSE_CAMERA_FIELD)
        self.assertIn("NoseCamera", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_NOSE}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, NOSE_CAMERA_FIELD)
        self.assertIn("NoseCamera", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_NOSE, section)
        ...
        self.assertNotIn("UPROPERTY", NOSE_CAMERA_FIELD)
        self.assertNotIn("VisibleAnywhere", NOSE_CAMERA_FIELD)
        self.assertNotIn("BlueprintReadOnly", NOSE_CAMERA_FIELD)
        self.assertNotIn("Category", NOSE_CAMERA_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_NOSE)
        self.assertNotIn("BlueprintCallable", UPROPERTY_NOSE)
        self.assertIn("Skyguard|Boss", UPROPERTY_NOSE)
        self.assertNotIn("Destruction", UPROPERTY_NOSE)
        self.assertNotIn("Encounter", UPROPERTY_NOSE)
        self.assertNotIn("Mission10", UPROPERTY_NOSE)
        self.assertNotIn("Safety", UPROPERTY_NOSE)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_NOSE)
            self.assertNotIn(invented, NOSE_CAMERA_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, NOSE_CAMERA_FIELD)
        self.assertIn("NoseCamera", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* NoseCamera;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> NoseCamera;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> NoseCamera;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, NOSE_CAMERA_FIELD)
            self.assertIn("NoseCamera", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_nose_camera_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, NOSE_CAMERA_FIELD),
            NOSE_CAMERA_FIELD,
        )
        self.assertTrue(has_declaration(section, NOSE_CAMERA_FIELD))
        self.assertEqual(declaration_count(section, NOSE_CAMERA_FIELD), 1)
        self.assertTrue(
            NOSE_CAMERA_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            NOSE_CAMERA_FIELD,
        )
        self.assertTrue(NOSE_CAMERA_FIELD.endswith(";"), NOSE_CAMERA_FIELD)
        self.assertIn("NoseCamera", NOSE_CAMERA_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            NOSE_CAMERA_FIELD,
        )
        self.assertIn("TObjectPtr<", NOSE_CAMERA_FIELD)
        self.assertNotIn("TArray<", NOSE_CAMERA_FIELD)
        self.assertNotIn("=", NOSE_CAMERA_FIELD)
        self.assertNotIn("INDEX_NONE", NOSE_CAMERA_FIELD)
        self.assertNotIn("UFUNCTION", NOSE_CAMERA_FIELD)
        self.assertNotIn("{", NOSE_CAMERA_FIELD)
        self.assertNotIn("}", NOSE_CAMERA_FIELD)
        self.assertNotIn("return ", NOSE_CAMERA_FIELD)
        self.assertNotIn("CommandAntenna", NOSE_CAMERA_FIELD)
        self.assertNotIn("ControlLinkage", NOSE_CAMERA_FIELD)
        self.assertNotIn("DebrisNose", NOSE_CAMERA_FIELD)
        self.assertNotIn("DebrisCenter", NOSE_CAMERA_FIELD)
        self.assertNotIn("DebrisTail", NOSE_CAMERA_FIELD)
        self.assertNotIn("DebrisSpine", NOSE_CAMERA_FIELD)
        self.assertNotIn("EncounterController", NOSE_CAMERA_FIELD)
        self.assertNotIn("AdvanceEncounter", NOSE_CAMERA_FIELD)
        self.assertNotIn("ResetEncounterState", NOSE_CAMERA_FIELD)
        self.assertNotIn("IsRouteStateSafe", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetRouteProgress", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", NOSE_CAMERA_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", NOSE_CAMERA_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", NOSE_CAMERA_FIELD)
        self.assertNotIn("RadarNode", NOSE_CAMERA_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", NOSE_CAMERA_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", NOSE_CAMERA_FIELD)
        self.assertNotIn("IssueClimbCommand", NOSE_CAMERA_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", NOSE_CAMERA_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetFinaleStage", NOSE_CAMERA_FIELD)
        self.assertNotIn("IsClimbCommandIssued", NOSE_CAMERA_FIELD)
        self.assertNotIn("IsWreckDiverted", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", NOSE_CAMERA_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetGunnerMount", NOSE_CAMERA_FIELD)
        self.assertNotIn("FaceWorldLocation", NOSE_CAMERA_FIELD)
        self.assertNotIn("PortGuidanceArray", NOSE_CAMERA_FIELD)
        self.assertNotIn("DebrisArmorPort", NOSE_CAMERA_FIELD)
        self.assertNotIn("SetSearchlightTracked", NOSE_CAMERA_FIELD)
        self.assertNotIn("SetContactIdentified", NOSE_CAMERA_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", NOSE_CAMERA_FIELD)
        self.assertNotIn("SetLightningExposed", NOSE_CAMERA_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", NOSE_CAMERA_FIELD)
        self.assertNotIn("HullCollider", NOSE_CAMERA_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, NOSE_CAMERA_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_NOSE} {NOSE_CAMERA_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, NOSE_CAMERA_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{NOSE_CAMERA_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, NOSE_CAMERA_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, NOSE_CAMERA_FIELD),
            section,
    ...
    def test_assigned_nose_camera_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "NoseCamera = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, NOSE_CAMERA_FIELD)
        self.assertIn("NoseCamera", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, NOSE_CAMERA_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("NoseCamera", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, NOSE_CAMERA_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{NOSE_CAMERA_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, NOSE_CAMERA_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_NOSE, section)
        self.assertTrue(has_declaration(section, NOSE_CAMERA_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            NOSE_CAMERA_FIELD.startswith("UFUNCTION"),
            NOSE_CAMERA_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, NOSE_CAMERA_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", NOSE_CAMERA_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_nose_camera_cpp_body(self) -> None:
        locked_only = f"{NOSE_CAMERA_FIELD}\n"
        self.assertNotIn("{", NOSE_CAMERA_FIELD)
        self.assertNotIn("}", NOSE_CAMERA_FIELD)
        self.assertNotIn("return ", NOSE_CAMERA_FIELD)
        ...
        self.assertNotIn("SkyguardPathfinderBoss.cpp", NOSE_CAMERA_FIELD)
        self.assertNotIn("SkyguardPathfinderBoss.cpp", locked_only)
        self.assertNotIn("return false", NOSE_CAMERA_FIELD)
        self.assertNotIn("return true", NOSE_CAMERA_FIELD)
        self.assertNotIn("CreateDefaultSubobject", NOSE_CAMERA_FIELD)
        self.assertNotIn("SetExposed", NOSE_CAMERA_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            NOSE_CAMERA_FIELD,
        )
        self.assertNotIn("SetBossPhase", NOSE_CAMERA_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_sibling_pathfinder_fields(self) -> None:
        ...
        for token in SIBLING_PATHFINDER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("CommandAntenna", NOSE_CAMERA_FIELD)
        self.assertNotIn("ControlLinkage", NOSE_CAMERA_FIELD)
        self.assertNotIn("DebrisNose", NOSE_CAMERA_FIELD)
        self.assertNotIn("DebrisCenter", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), NOSE_CAMERA_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), NOSE_CAMERA_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), NOSE_CAMERA_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), NOSE_CAMERA_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), NOSE_CAMERA_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, NOSE_CAMERA_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", NOSE_CAMERA_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("SetSearchlightTracked", NOSE_CAMERA_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("SetContactIdentified", NOSE_CAMERA_FIELD)
        self.assertNotIn("OpenOrbitExposure", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", NOSE_CAMERA_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("SetLightningExposed", NOSE_CAMERA_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", NOSE_CAMERA_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, NOSE_CAMERA_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("ASkyguardGunner", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("SkyguardRadarNode", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, NOSE_CAMERA_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", NOSE_CAMERA_FIELD)
        self.assertNotIn("MaxIntegrity", NOSE_CAMERA_FIELD)
        self.assertNotIn("CurrentIntegrity", NOSE_CAMERA_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", NOSE_CAMERA_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            NOSE_CAMERA_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, NOSE_CAMERA_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", NOSE_CAMERA_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{NOSE_CAMERA_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, NOSE_CAMERA_FIELD),
            NOSE_CAMERA_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardPathfinderBoss::NoseCamera", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder NoseCamera field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{NOSE_CAMERA_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, NOSE_CAMERA_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                NOSE_CAMERA_FIELD.lower(),
                "pathfinder NoseCamera contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, NOSE_CAMERA_FIELD)

    def test_contract_is_nose_camera_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(NOSE_CAMERA_FIELD.endswith(";"))
        self.assertNotIn("=", NOSE_CAMERA_FIELD)
        self.assertIn(UPROPERTY_NOSE, section)
