...
# or lock the GetTelegraphsTriggered body.
...
# ESkyguardBossPhase. Leftover briefing-fail-closed #9fe9,
# leftover briefing declaration contracts through
# GetRadioChatter, leftover mission-briefing-state enum,
# leftover radio-chatter empty-fail-closed, leftover
# campaign-roster #111, leftover campaign-save
# empty-fail-closed, leftover boss-phase enum #60cb,
# leftover boss-definition defaults, leftover
# boss-weak-point defaults, leftover Gunner helpers,
# leftover ApacheSystem / weapon stations / pilot
# commands / loadout / lock-phase, leftover Harbor
# clocks, leftover theater-kit / flare / HUD, leftover
# settings invert-look / ApplySettings, leftover
# bind-hud-host, leftover objective-runtime /
# route-runtime fail-closed, leftover gun-fire camera
# shake, leftover mission-weather enum, sibling
# AdvanceEncounter / ResetEncounterState /
# IsRouteStateSafe / GetRouteProgress, and in-flight
# GetEffectiveSpeedMultiplier /
# IsAttackTelegraphActive stay sibling-only.
GET_TELEGRAPHS_TRIGGERED = "int32 GetTelegraphsTriggered() const;"
UFUNCTION_ENCOUNTER = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Boss|Encounter")'
)
# Leftover #56–#64 plus PathfinderEncounterController
# production files. This lane only adds an isolated
# Python GetTelegraphsTriggered declaration contract. Stay
# off AdvanceEncounter (draft #376),
# ResetEncounterState (draft #378),
# IsRouteStateSafe (draft #377),
# GetRouteProgress (sibling just pushed),
# GetEffectiveSpeedMultiplier (in-flight),
# and IsAttackTelegraphActive (in-flight)
# on this class. Stay off UPROPERTY fields, delegates,
# leftover boss-phase enum #60cb, leftover
# boss-definition defaults, leftover boss-weak-point
# defaults, leftover briefing-fail-closed, leftover
# briefing declaration contracts through
# GetRadioChatter, leftover
...
# empty-fail-closed, leftover boss-phase enum #60cb,
# leftover boss-definition defaults, leftover
# boss-weak-point defaults, leftover CPG debrief,
...
# enum, leftover AdvanceEncounter, leftover
# ResetEncounterState, leftover IsRouteStateSafe,
...
# GetEffectiveSpeedMultiplier / IsAttackTelegraphActive /
# UPROPERTY fields / delegates stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
IS_ROUTE_STATE_SAFE_NOT_LOCKED = ("bool IsRouteStateSafe() const;",)
GET_ROUTE_PROGRESS_NOT_LOCKED = ("float GetRouteProgress() const;",)
...
# Leftover boss-phase enum stays unlocked. This lane
# parses the public class section only and does not
# parse ESkyguardBossPhase.
LEFTOVER_BOSS_PHASE_NOT_LOCKED = (
    "test_boss_phase_enum_contract.py",
    "SkyguardBossTypes.h",
    "enum class ESkyguardBossPhase",
)
# Leftover boss-definition defaults / leftover
# boss-weak-point defaults stay unlocked.
...
# .cpp GetTelegraphsTriggered body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline `return TelegraphsTriggered`
# is accepted as presence, not a locked implementation
# contract.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    ...
    "return false",
    "return true",
    "USkyguardPathfinderEncounterController::GetTelegraphsTriggered",
    "SkyguardPathfinderEncounterController.cpp",
)
SIBLING_TYPES = (
...
class PathfinderGetTelegraphsTriggeredDeclContractTests(unittest.TestCase):
    def test_pathfinder_controller_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_TELEGRAPHS_TRIGGERED), section)

    def test_missing_class_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            class_body(
                "class SKYGUARD52_API USkyguardUnrelatedEncounter "
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_TELEGRAPHS_TRIGGERED)
        self.assertIn("GetTelegraphsTriggered", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_TELEGRAPHS_TRIGGERED))

    def test_missing_get_telegraphs_triggered_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardPathfinderEncounterController();\n"
            "\tvoid AdvanceEncounter(float DeltaSeconds);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_TELEGRAPHS_TRIGGERED)
        self.assertIn("GetTelegraphsTriggered", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_TELEGRAPHS_TRIGGERED)
        self.assertIn("GetTelegraphsTriggered", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_TELEGRAPHS_TRIGGERED),
            section,
        )
        self.assertNotIn("BlueprintPure", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("UFUNCTION", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("Category", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("BlueprintCallable", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_TELEGRAPHS_TRIGGERED)
        self.assertIn("GetTelegraphsTriggered", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tint32 GetTelegraphsTriggered();\n"
        wrong_return_void = "\tvoid GetTelegraphsTriggered() const;\n"
        wrong_return_bool = "\tbool GetTelegraphsTriggered() const;\n"
        wrong_return_float = "\tfloat GetTelegraphsTriggered() const;\n"
        added_arg = "\tint32 GetTelegraphsTriggered(float DeltaSeconds) const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_TELEGRAPHS_TRIGGERED)
            self.assertIn("GetTelegraphsTriggered", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_telegraphs_triggered_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_TELEGRAPHS_TRIGGERED),
            GET_TELEGRAPHS_TRIGGERED,
        )
        self.assertTrue(has_declaration(section, GET_TELEGRAPHS_TRIGGERED))
        self.assertEqual(declaration_count(section, GET_TELEGRAPHS_TRIGGERED), 1)
        self.assertTrue(
            GET_TELEGRAPHS_TRIGGERED.startswith("int32 "),
            GET_TELEGRAPHS_TRIGGERED,
        )
        self.assertTrue(GET_TELEGRAPHS_TRIGGERED.endswith(";"), GET_TELEGRAPHS_TRIGGERED)
        self.assertTrue(
            GET_TELEGRAPHS_TRIGGERED.endswith(" const;"),
            GET_TELEGRAPHS_TRIGGERED,
        )
        self.assertIn("GetTelegraphsTriggered()", GET_TELEGRAPHS_TRIGGERED)
        self.assertIn(" const", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("INDEX_NONE", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("{", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("}", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return ", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return TelegraphsTriggered", GET_TELEGRAPHS_TRIGGERED)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_TELEGRAPHS_TRIGGERED}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_TELEGRAPHS_TRIGGERED))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_TELEGRAPHS_TRIGGERED),
            GET_TELEGRAPHS_TRIGGERED,
        )
        exact_inline = (
            "int32 GetTelegraphsTriggered() const { return TelegraphsTriggered; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_TELEGRAPHS_TRIGGERED))
        self.assertEqual(
            require_declaration(exact_inline, GET_TELEGRAPHS_TRIGGERED),
            GET_TELEGRAPHS_TRIGGERED,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_TELEGRAPHS_TRIGGERED),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tint32\n"
            "\tGetTelegraphsTriggered() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tint32 GetTelegraphsTriggered(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        wrap_inline = (
            "public:\n"
            "\tint32 GetTelegraphsTriggered()\n"
            "\tconst { return TelegraphsTriggered; }\n"
            "};\n"
        )
        header_wrap_type = (
            f"class SKYGUARD52_API {CLASS_NAME} "
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, GET_TELEGRAPHS_TRIGGERED),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_TELEGRAPHS_TRIGGERED}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_TELEGRAPHS_TRIGGERED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_TELEGRAPHS_TRIGGERED),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return TelegraphsTriggered", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return Other", GET_TELEGRAPHS_TRIGGERED)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_TELEGRAPHS_TRIGGERED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_TELEGRAPHS_TRIGGERED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_telegraphs_triggered_body(self) -> None:
        locked_only = f"{GET_TELEGRAPHS_TRIGGERED}\n"
        self.assertNotIn("{", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("}", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return ", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return TelegraphsTriggered", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return TelegraphsTriggered", locked_only)
        self.assertNotIn(
            "USkyguardPathfinderEncounterController::GetTelegraphsTriggered",
            GET_TELEGRAPHS_TRIGGERED,
        )
        ...
        self.assertNotIn("return false", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("return true", GET_TELEGRAPHS_TRIGGERED)

    def test_contract_does_not_relock_advance_encounter(self) -> None:
        locked_only = f"{GET_TELEGRAPHS_TRIGGERED}\n"
        for neighbor in ADVANCE_ENCOUNTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("AdvanceEncounter", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_contract_does_not_relock_reset_encounter_state(self) -> None:
        ...
        for neighbor in RESET_ENCOUNTER_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("ResetEncounterState", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("ResetEncounterState", locked_only)

    def test_contract_does_not_relock_is_route_state_safe(self) -> None:
        locked_only = f"{GET_TELEGRAPHS_TRIGGERED}\n"
        for neighbor in IS_ROUTE_STATE_SAFE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("IsRouteStateSafe", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_contract_does_not_relock_get_effective_speed_multiplier(
        ...
        for neighbor in GET_EFFECTIVE_SPEED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("GetEffectiveSpeedMultiplier", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_contract_does_not_relock_is_attack_telegraph_active(self) -> None:
        ...
        for neighbor in IS_ATTACK_TELEGRAPH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("IsAttackTelegraphActive", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_contract_does_not_relock_get_route_progress(self) -> None:
        ...
        for neighbor in GET_ROUTE_PROGRESS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("GetRouteProgress", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("GetRouteProgress", locked_only)

    def test_contract_does_not_relock_uprop_fields_or_delegates(self) -> None:
        locked_only = f"{GET_TELEGRAPHS_TRIGGERED}\n"
        self.assertNotIn("UPROPERTY", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("UPROPERTY", locked_only)
        self.assertNotIn("bAutoAdvance", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("ObservedPhase", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("OnAttackTelegraphChanged", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("OnAttackCommitted", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn(
        ...
        self.assertNotIn("DECLARE_DYNAMIC_MULTICAST_DELEGATE", locked_only)

    def test_contract_does_not_relock_leftover_boss_phase_enum(self) -> None:
        locked_only = f"{GET_TELEGRAPHS_TRIGGERED}\n"
        for token in LEFTOVER_BOSS_PHASE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("ESkyguardBossPhase", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("ESkyguardBossPhase", locked_only)
        self.assertNotIn("SkyguardBossTypes.h", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn(
            "test_boss_phase_enum_contract.py",
            GET_TELEGRAPHS_TRIGGERED,
        )
    ...
    def test_contract_does_not_relock_leftover_boss_defaults(self) -> None:
        ...
        for token in LEFTOVER_BOSS_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FSkyguardBossDefinition", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FSkyguardBossWeakPoint", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn(
            "test_boss_definition_defaults_contract.py",
            GET_TELEGRAPHS_TRIGGERED,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("Unconfigured", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("Warming", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("Launched", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_TELEGRAPHS_TRIGGERED,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FSkyguardBriefingCard", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FSkyguardBriefingCard", locked_only)
        self.assertNotIn("FSkyguardBriefingRadioRow", locked_only)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("GetBriefingText", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("GetRadioChatter", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FillResultCombatStats", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("ASkyguardGunner", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FillAndFinalize", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("FillAndFail", GET_TELEGRAPHS_TRIGGERED)
        self.assertNotIn("ApplyHydraForClusters", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_TELEGRAPHS_TRIGGERED}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_TELEGRAPHS_TRIGGERED),
            GET_TELEGRAPHS_TRIGGERED,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("SkyguardPathfinderEncounterController.cpp", section)
        self.assertNotIn(
            "USkyguardPathfinderEncounterController::GetTelegraphsTriggered",
            section,
        )
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardBossTypes.h")
        self.assertNotIn("SkyguardBossTypes.h", GET_TELEGRAPHS_TRIGGERED)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder GetTelegraphsTriggered contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_TELEGRAPHS_TRIGGERED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"pathfinder GetTelegraphsTriggered contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_TELEGRAPHS_TRIGGERED)

    def test_contract_is_get_telegraphs_triggered_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("FSkyguardBossWeakPoint", locked_only)
        ...
        self.assertTrue(GET_TELEGRAPHS_TRIGGERED.startswith("int32 "))
        self.assertTrue(GET_TELEGRAPHS_TRIGGERED.endswith(";"))
        self.assertTrue(GET_TELEGRAPHS_TRIGGERED.endswith(" const;"))
        self.assertIn(UFUNCTION_ENCOUNTER, section)
