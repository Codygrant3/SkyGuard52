# THIS IS leftover-safe ASkyguardGunner FireCameraShakeClass.
...
# then `TSubclassOf<UCameraShakeBase> FireCameraShakeClass;` (NO initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Combat|Feedback`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS absent (NO initializer).
# This is a combat feedback TSubclassOf, not a CPG mesh/text slot,
# not leftover analog apache-cpg-feel FireRate / RecoilPitch,
# not leftover look floats, and not leftover analog
# gun-fire-camera-shake-tests bulk (keep that in LOCKED_SCRIPTS).
# Fail-closed if the UPROPERTY or decl is missing or renamed,
# if Category is missing, or if specifiers drop to VisibleAnywhere /
# BlueprintReadOnly or stay Category="Skyguard" without |Combat,
# or stay Category="Skyguard|Combat" without |Feedback,
# or become Skyguard|Combat|Safety / Skyguard|Apache /
# Skyguard|Theater.
...
# Stop BEFORE sibling MinimumSafeSideFireYaw as a claimed slot.
# Skip leftover analog apache-cpg-feel RecoilPitch = 0.92f
# (previous sibling) forever. Parse past it; do not lock it.
...
# Category Skyguard|Combat (look floats) — RETARGET hard:
# wrap is EditAnywhere BlueprintReadWrite Category
# Skyguard|Combat|Feedback.
# LOCKED_DECL is `TSubclassOf<UCameraShakeBase> FireCameraShakeClass;`
...
# `TSubclassOf<AActor>` plus Ig+laMissileClass.
...
# without |Feedback, Skyguard|Apache, Skyguard|Theater,
# or Skyguard|Combat|Safety.
# Combat feedback TSubclassOf fail-closed if LOCKED_DECL is
...
TARGET = "TSubclassOf<UCameraShakeBase> FireCameraShakeClass;"
TARGET_WRONG_BARE = "float FireCameraShakeClass;"
TARGET_WRONG_INIT = "TSubclassOf<UCameraShakeBase> FireCameraShakeClass = nullptr;"
TARGET_WRONG_FALSE = "TSubclassOf<UCameraShakeBase> FireCameraShakeClass = false;"
TARGET_WRONG_TRUE = "TSubclassOf<UCameraShakeBase> FireCameraShakeClass = true;"
TARGET_WRONG_ZERO = "TSubclassOf<UCameraShakeBase> FireCameraShakeClass = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> FireCameraShakeClass;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> FireCameraShakeClass;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> FireCameraShakeClass;"
...
FIRE_CAMERA_SHAKE_CLASS_INIT_RE = re.compile(
    r"TSubclassOf\s*<\s*UCameraShakeBase\s*>\s+"
    r"FireCameraShakeClass\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored TSubclassOf UCameraShakeBase
    # FireCameraShakeClass with NO initializer.
    ...
    # Harbor 40 / 80, leftover Ig+laMissileClass, or an
    # initialized FireCameraShakeClass as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if FIRE_CAMERA_SHAKE_CLASS_DECL_RE.search(compact) is None:
        return False
    if FIRE_CAMERA_SHAKE_CLASS_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW:
            break
        if ident in (
            SIBLING_RECOIL_PITCH,
            SIBLING_FIRE_RATE,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSubclassOf<UCameraShakeBase>\s+"
            r"FireCameraShakeClass\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSubclassOf<UCameraShakeBase> "
...
class GunnerFireCameraShakeClassFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "FireCameraShakeClass"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertIn("FireCameraShakeClass", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_fire_camera_shake_class_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Combat|Feedback"', section)
        ...
        self.assertIsNone(
            re.search(
                r'Category="Skyguard\|Combat"(?!\|Feedback)',
                UPROPERTY_EDIT_READWRITE,
            )
        )
        ...
        self.assertIn('Category="Skyguard|Combat|Feedback"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TSubclassOf<UCameraShakeBase> FireCameraShakeClass;", compact_origin)
        self.assertNotIn("FireCameraShakeClass = 160.f", compact_origin)
    ...
    def test_fire_camera_shake_class_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSubclassOf<UCameraShakeBase> FireCameraShakeClass"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("TSubclassOf", LOCKED_DECL)
        self.assertIn("UCameraShakeBase", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTSubclassOf<UCameraShakeBase> FireCameraShakeClasss;\n",
            "\tint32 FireCameraShakeClass = 95;\n",
            "\tbool FireCameraShakeClass = true;\n",
            "\tTSubclassOf<UCameraShakeBase> FireCameraShakeClass = " + forty + ";\n",
            "\tTSubclassOf<UCameraShakeBase> FireCameraShakeClass = " + eighty + ";\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Combat|Feedback"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Combat|Feedback"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
        ...
        self.assertNotIn('Category="Skyguard|Combat|Feedback"', theater_specs)
        self.assertIn('Category="Skyguard|Combat|Feedback"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
    ...
    def test_does_not_claim_sibling_minimum_safe_side_fire_yaw_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "FireCameraShakeClass"))
        self.assertTrue(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner FireCameraShakeClass field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_fire_camera_shake_class_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
