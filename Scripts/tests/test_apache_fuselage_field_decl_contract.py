# THIS IS leftover-safe ASkyguardApacheAircraft Fuselage.
...
# then BARE `TObjectPtr<UStaticMeshComponent> Fuselage;`
# (no initializer).
...
# Stop BEFORE sibling Nose as a claimed slot.
# Do NOT claim AircraftRoot / SilhouetteMesh / RotorPower /
# HoverBobCentimeters / MaxIntegrity / CurrentIntegrity /
# HullCollider.
# Do NOT parse leftover analog apache-aircraft-empty-fail-closed
# #851b (keep bulk in LOCKED_SCRIPTS).
# Do NOT parse leftover Apache UFUNCTION #411-#426,
# leftover analog apache-hull-collider, leftover analog
# apache-mount-fail-closed, leftover analog apache-cpg-feel.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> Fuselage;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> Fuselage;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> Fuselage = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> Fuselage = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> Fuselage = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> Fuselage;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> Fuselage;"
TARGET_WRONG_NOSE = "TObjectPtr<UStaticMeshComponent> Nose;"
TARGET_WRONG_AIRCRAFT_ROOT = "TObjectPtr<USceneComponent> AircraftRoot;"
TARGET_WRONG_SILHOUETTE_MESH = "TObjectPtr<UStaticMeshComponent> SilhouetteMesh;"
...
FUSELAGE_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+Fuselage\s*;"
)
FUSELAGE_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+Fuselage\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover Nose / SilhouetteMesh / AircraftRoot, or an initializer
    # on Fuselage as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if FUSELAGE_DECL_RE.search(compact) is None:
        return False
    if FUSELAGE_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_NOSE):
            break
    ...
    if has_identifier(window, SIBLING_NOSE):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+Fuselage\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> Fuselage "
...
class ApacheFuselageFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Fuselage"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_NOSE),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Fuselage"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Fuselage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_fuselage_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> Fuselage;", compact_origin)
        self.assertNotIn("Fuselage = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_fuselage_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> Fuselage"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SILHOUETTE_MESH}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> Fuselage;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> Fuselages;\n",
            "\tint32 Fuselage = 140;\n",
            "\tbool Fuselage = true;\n",
            "\tfloat Fuselage = " + forty + ";\n",
            "\tfloat Fuselage = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_nose_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_NOSE))
        ...
        self.assertNotIn(TARGET_WRONG_NOSE, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_NOSE))
        self.assertFalse(has_identifier(section, SIBLING_NOSE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache Fuselage field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "fuselage_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
