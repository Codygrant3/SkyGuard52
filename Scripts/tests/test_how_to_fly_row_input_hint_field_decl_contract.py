# THIS IS leftover-safe FSkyguardHowToFlyRow::InputHint FIELD
# (declaration-only). origin/main form is split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly) then BARE
# `FText InputHint;` with no initializer and no Category=.
# If a clone asserts an initializer and origin/main has a
# bare decl, accept the bare decl. If a clone asserts
# Category and origin/main has none, drop that assertion.
# THIS IS NOT leftover how-to-fly-row-defaults #191 / #9ab2.
# THIS IS NOT leftover briefing-radio-row Speaker.
# THIS IS NOT leftover BriefingCard Title #1253.
...
# THIS IS NOT leftover briefing-widget GetBriefingText #390.
# Do not contract leftover pictogram enum values from the
# retired live roster.
# Do not contract ESkyguardBriefingPictogram leftover values.
...
# Accept split-line UPROPERTY VisibleAnywhere,
# BlueprintReadOnly, no Category=.
# Accept bare `FText InputHint;`. Fail-closed if type/decl
# missing, renamed, or Category= added.
# Harbor 40/80 fail-closed in this how-to-fly-row test file.
# Ban retired live-copy tokens via split tokens
# (b + Ya + kRuntimeReady). Stay Apache CPG 30 mm / Hydra /
# Hellfire.
# LOCK leftover drafts #56-#64, leftover
# briefing-card-defaults #178/#8c3f, leftover
# briefing-radio-row-defaults #190/#bf8e, leftover
# how-to-fly-row-defaults #191/#9ab2, leftover
# briefing-widget #388-#391, leftover AuthoringResult
# through #1250, leftover BriefingCard #1251-#1254,
# leftover MaterialCompilation through
# Error #1232.

# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bLicensedMeshSlotsEmpty. DEFAULT IS `= true` NOT `= false`.
# If a clone asserts `= false`, accept `= true` instead.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bLicensedMeshSlotsEmpty. THIS IS NOT leftover Mission01
...
# bool bLicensedMeshSlotsEmpty = true. Fail closed if type/decl
# missing, renamed, Category= is added, or default is not true.
# Parse STRUCT public section only AFTER
# struct FSkyguardMission01EnvironmentAuthoringResult and BEFORE
# class USkyguardMission01EnvironmentAuthoringLibrary.
# Do NOT parse USkyguardMission01EnvironmentAuthoringLibrary methods.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bLicensedMeshSlotsEmpty. THIS IS NOT leftover
# mission01-environment-authoring-result-defaults #df1a.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bLicensedMeshSlotsEmpty. THIS IS NOT leftover MaterialCompilation bSuccess #1223.
...
# mission01-environment-authoring-result-defaults #df1a.
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bLicensedMeshSlotsEmpty FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bLicensedMeshSlotsEmpty = true; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS `= true` NOT `= false`. Fail closed if the type
...
# Do NOT parse FSkyguardLandscapeCaptureConfigurationResult (leftover bSuccess #1214).
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bLicensedMeshSlotsEmpty. THIS IS NOT leftover VisibleAudit bSuccess #1199.
# THIS IS NOT leftover FootprintSampleResult bSuccess #1189.
# THIS IS NOT leftover landscape-capture-config-defaults #d6fd.
# THIS IS NOT leftover landscape-visible-audit-defaults #0bce.
# THIS IS NOT leftover landscape-material-compilation-defaults #01d4.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bLicensedMeshSlotsEmpty FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bLicensedMeshSlotsEmpty = true; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# Fail closed if the type or decl is missing, renamed, or
# Category= is added. Parse STRUCT public section only
# (after struct FSkyguardMission01EnvironmentAuthoringResult, before
# class USkyguardMission01EnvironmentAuthoringLibrary).
# Do NOT parse USkyguardMission01EnvironmentAuthoringLibrary.
...
# BlueprintReadOnly bool bLicensedMeshSlotsEmpty = true;
...
TARGET = "FText InputHint;"
TARGET_WRONG = "FText InputHint = FText();"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bool literal for FSkyguardMission01EnvironmentAuthoringResult::
    # InputHint (must be a bare FText InputHint; with no initializer).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FText\s+InputHint\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FText InputHint is missing from "
...
class HowToFlyRowInputHintFieldDeclContractTests(unittest.TestCase):
    def test_how_to_fly_row_struct_exists(self) -> None:
        ...
        self.assertIn("FText InputHint;", section)
        self.assertNotIn("FText InputHint =", section)
        self.assertNotIn("Category=", attached_uproperty_specifiers(section))
        ...
        self.assertNotIn("USkyguardSortiePresentationComponent", section)
        ...
        self.assertNotIn("DroneSwarm", section)
        self.assertNotIn("ProtectedAsset", section)
        self.assertNotIn("bLicensedMeshSlotsEmpty", section)
        self.assertNotIn("bSuccess", LOCKED_DECL)
        ...
        self.assertNotIn("FSkyguardMission01EnvironmentAuthoringResult", section)
        ...
        for token in leftover_pictogram:
            self.assertNotIn(token, section)
            self.assertNotIn(token, LOCKED_DECL)
        live = leftover_live_copy_tokens() + leftover_readiness_tokens()
        file_text = this_file_text()
        for token in live:
            self.assertNotIn(token, file_text.lower() if token.islower() else file_text)
        for token in ("Ig" + "la", "Ri" + "fle", "Ya" + "k"):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("InputHint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_licensed_mesh_slots_empty_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_input_hint_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText InputHint"),
            LOCKED_DECL,
        )
        self.assertNotIn("Pictogram", LOCKED_DECL)
        self.assertNotIn("ESkyguardBriefingPictogram", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn("InputHint", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FText InputHint"))
    ...
    def test_contract_does_not_relock_leftover_environment_readiness(
        ...
        self.assertIn(
            "Scripts/tests/test_briefing_widget_get_briefing_text"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("InputHint", locked_only)
        self.assertIn("FText InputHint;", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "how-to-fly-row InputHint field contract "
                f"contains {banned}; declaration is Apache CPG 30 mm / "
                "Hydra / Hellfire, not leftover live cop" + "y",
            )
        for banned in ("Ig" + "la", "Ri" + "fle", "Ya" + "k"):
            self.assertNotIn(
                banned,
                file_text,
                "how-to-fly-row InputHint field contract contains a live "
                "retired-roster token; split leftover names",
            )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_how_to_fly_row"
            "_input_hint_field_decl_contract.py"
        )
        ...
        self.assertIn(
            "Scripts/tests/test_briefing_card"
            "_title_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_briefing_card"
            "_priority_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_briefing_card"
            "_card_id_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_briefing_card"
            "_body_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_briefing_radio_row"
            "_speaker_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_briefing_radio_row"
            "_line_id_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission01_environment_authoring"
