# THIS IS leftover-safe FSkyguardTheaterKitSpec LandmarkScale.
...
# Fail-closed if the type is not FVector or the default
# is not `= FVector(4.f, 4.f, 12.f)`.
...
# Do NOT contract sibling fields KitId / LandmarkSet /
# BuildingKit / BuildingTint / LampTreatment / LampColor /
# LampIntensity / RoadTreatment / RoadTint / NamedLandmark /
# LandmarkTint / SilhouetteKit / SilhouetteTint /
# LandmarkMeshIndex / WeatherIdentity.
...
# Category="Skyguard|Campaign", retarget: type is FVector,
# default is `= FVector(4.f, 4.f, 12.f)`, Category is
# Skyguard|Theater. Harbor 40/80 fail-closed; the 4.f
# in this vector is the live LandmarkScale default,
# not Harbor.
...
TARGET = "FVector LandmarkScale = FVector(4.f, 4.f, 12.f);"
TARGET_WRONG_BARE = "FVector LandmarkScale;"
TARGET_WRONG_EQ_NONE = "FVector LandmarkScale = FVector::ZeroVector;"
TARGET_WRONG_FALSE = "FVector LandmarkScale = false;"
TARGET_WRONG_TRUE = "FVector LandmarkScale = true;"
TARGET_WRONG_ZERO = "FVector LandmarkScale = 0.f;"
TARGET_WRONG_FLOAT = "float LandmarkScale;"
TARGET_WRONG_FNAME = "FName LandmarkScale;"
TARGET_WRONG_UNIT = "FVector LandmarkScale = FVector(1.f, 1.f, 1.f);"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare `FVector LandmarkScale;`,
    # `= FVector::ZeroVector` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` / leftover float Health when
    # origin/main has FVector with that exact default.
    # Do not accept sibling WeatherIdentity / LandmarkTint /
    # LandmarkMeshIndex / SilhouetteTint fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+LandmarkScale\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+LandmarkScale\b",
        compact,
    ):
        return False
    if re.search(
        r"FVector\s+LandmarkScale\s*;",
        compact,
    ) and re.search(
        r"FVector\s+LandmarkScale\s*=",
        compact,
        ...
        return False
    if re.search(
        r"FVector\s+LandmarkScale\s*=\s*FVector\s*"
        r"\(\s*4\.f\s*,\s*4\.f\s*,\s*12\.f\s*\)",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FVector\s+LandmarkScale\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FVector LandmarkScale is missing from "
...
class TheaterKitSpecLandmarkScaleFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LandmarkScale"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LandmarkScale"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LandmarkScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_landmark_scale_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_or_wrong_default_fails_closed(self) -> None:
        ...
        self.assertNotIn("LandmarkScale = FVector::ZeroVector", compact_origin)
        self.assertNotIn("LandmarkScale = false", compact_origin)
        self.assertNotIn("LandmarkScale = true", compact_origin)
        self.assertNotIn("LandmarkScale = 0.f", compact_origin)
        self.assertNotIn("LandmarkScale = 160.f", compact_origin)

    def test_landmark_scale_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FVector LandmarkScale"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= FVector(4.f, 4.f, 12.f)", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFName LandmarkScales;\n",
            "\tint32 LandmarkScale;\n",
            "\tbool LandmarkScale;\n",
            "\tfloat LandmarkScale = " + forty + ";\n",
            "\tfloat LandmarkScale = " + eighty + ";\n",
            "\tFVector LandmarkScale = FVector("
            + forty + ", " + eighty + ", 12.f);\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tFVector\n\tLandmarkScale = FVector(4.f, 4.f, 12.f);\n",
            "\tFVector   LandmarkScale = FVector(4.f, 4.f, 12.f);\n",
            "\tFVector\tLandmarkScale = FVector(4.f, 4.f, 12.f);\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_VISIBLE_READONLY}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec LandmarkScale field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "landmark_scale_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
