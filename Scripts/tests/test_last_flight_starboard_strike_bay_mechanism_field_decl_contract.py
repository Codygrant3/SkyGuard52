...
# INDEX_NONE or lock StarboardStrikeBayMechanism
...
# StarboardStrikeBayMechanism;`); accept that form and
...
# isolated-test drafts #107–#481 including leftover
...
# Python StarboardStrikeBayMechanism field declaration
...
# leftover isolated-test drafts #107–#481,
...
LEFTOVER_SIBLING_FIELD_CONTRACTS_NOT_LOCKED = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::StarboardStrikeBayMechanism",
    "SkyguardLastFlightBoss.cpp",
    "SetExposed",
...
class LastFlightStarboardStrikeBayMechanismFieldDeclContractTests(unittest.TestCase):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, STARBOARD_STRIKE_BAY_MECHANISM_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("StarboardStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_missing_starboard_strike_bay_mechanism_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLastFlightBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("StarboardStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_STARBOARD_STRIKE}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("StarboardStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_STARBOARD_STRIKE, section)
        ...
        self.assertNotIn("UPROPERTY", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("VisibleAnywhere", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("BlueprintReadOnly", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("Category", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_STARBOARD_STRIKE)
        self.assertNotIn("BlueprintCallable", UPROPERTY_STARBOARD_STRIKE)
        self.assertIn("Skyguard|Mission10|Boss", UPROPERTY_STARBOARD_STRIKE)
        self.assertNotIn("Destruction", UPROPERTY_STARBOARD_STRIKE)
        self.assertNotIn("Safety", UPROPERTY_STARBOARD_STRIKE)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_STARBOARD_STRIKE)
            self.assertNotIn(invented, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("StarboardStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* StarboardStrikeBayMechanism;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> StarboardStrikeBayMechanism;\n"
        )
        weak = (
        ...
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardStrikeBayMechanism = nullptr;\n"
        )
        wrong_name = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardGuidanceArray;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
            self.assertIn("StarboardStrikeBayMechanism", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_starboard_strike_bay_mechanism_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, STARBOARD_STRIKE_BAY_MECHANISM_FIELD),
            1,
        )
        self.assertTrue(
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertTrue(
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD.endswith(";"),
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertIn("StarboardStrikeBayMechanism", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertIn("TObjectPtr<", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("TArray<", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("=", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("INDEX_NONE", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("UFUNCTION", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("{", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("}", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("return ", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("IssueClimbCommand", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        ...
        self.assertNotIn("GetCivilianSeparationMeters", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetFinaleStage", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("IsClimbCommandIssued", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("IsWreckDiverted", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetGunnerMount", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("FaceWorldLocation", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("PortGuidanceArray", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("StarboardGuidanceArray", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CommandCore", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("SetLightningExposed", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("HullCollider", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_STARBOARD_STRIKE}\n"
            f"\t{STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_STARBOARD_STRIKE} {STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, STARBOARD_STRIKE_BAY_MECHANISM_FIELD),
    ...
    def test_assigned_starboard_strike_bay_mechanism_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardStrikeBayMechanism = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn("StarboardStrikeBayMechanism", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, STARBOARD_STRIKE_BAY_MECHANISM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        debris = "\tTObjectPtr<UStaticMeshComponent> DebrisArmorPort;\n"
        civilian = f"\t{MINIMUM_CIVILIAN_SEPARATION}\n"
        for region in (leftover_port, starboard, leftover_port_strike, command, debris, civilian):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
            self.assertIn("StarboardStrikeBayMechanism", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, STARBOARD_STRIKE_BAY_MECHANISM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_STARBOARD_STRIKE, section)
        self.assertTrue(has_declaration(section, STARBOARD_STRIKE_BAY_MECHANISM_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD.startswith("UFUNCTION"),
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, STARBOARD_STRIKE_BAY_MECHANISM_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_starboard_strike_bay_mechanism_cpp_body(
        self,
    ) -> None:
        locked_only = f"{STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n"
        self.assertNotIn("{", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        ...
        self.assertNotIn("return false", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("return true", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetExposed", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
        self.assertNotIn("SetBossPhase", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_apply_strike(), STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_last_flight_fields(self) -> None:
        ...
        self.assertNotIn("PortStrikeBayMechanism", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CommandCore", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("PortEngine", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("OpenOrbitExposure", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SetLightningExposed", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("ASkyguardGunner", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SkyguardRadarNode", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("MaxIntegrity", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("CurrentIntegrity", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, STARBOARD_STRIKE_BAY_MECHANISM_FIELD),
            STARBOARD_STRIKE_BAY_MECHANISM_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight StarboardStrikeBayMechanism field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{STARBOARD_STRIKE_BAY_MECHANISM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, STARBOARD_STRIKE_BAY_MECHANISM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                STARBOARD_STRIKE_BAY_MECHANISM_FIELD.lower(),
                "last flight StarboardStrikeBayMechanism contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, STARBOARD_STRIKE_BAY_MECHANISM_FIELD)

    def test_contract_is_starboard_strike_bay_mechanism_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(STARBOARD_STRIKE_BAY_MECHANISM_FIELD.endswith(";"))
        self.assertNotIn("=", STARBOARD_STRIKE_BAY_MECHANISM_FIELD)
        self.assertIn(UPROPERTY_STARBOARD_STRIKE, section)
