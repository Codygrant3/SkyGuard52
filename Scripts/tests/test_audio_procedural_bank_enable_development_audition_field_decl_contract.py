# THIS IS leftover-safe USkyguardAudioProceduralBankComponent
...
# then `bool bEnableDevelopmentAudition = true;`
# (HAS initializer, HAS spaces around =).
# Locked wrap has NO spaces around = inside the wrap.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Audio|Development`.
# EditAnywhere IS present. BlueprintReadWrite IS present.
# Default IS `= true`.
# THIS IS leftover-safe unique development-audition bool, not leftover
# ArcadeLook bEnabled, not leftover CampaignMissionSpec
# bNightIdentity, not leftover analog
# audio-procedural-bank-empty-fail-closed C++ bulk
# (keep bulk in LOCKED_SCRIPTS).
# It is NOT leftover sibling SampleRate / GeneratedByteBudget /
# private Waves as a claimed slot.
# Fail-closed if the UPROPERTY or decl is missing or renamed,
# if Category is missing, or if specifiers drop to
# VisibleAnywhere / BlueprintReadOnly / EditInstanceOnly
...
# or Category="Skyguard" without |Audio|Development.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `USkyguardAudioProceduralBankComponent` public
# UPROPERTY fields ONLY after `class USkyguardAudioProceduralBankComponent`.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `private:`.
# Stop BEFORE sibling SampleRate as a claimed slot.
# Do NOT claim leftover sibling SampleRate / GeneratedByteBudget
# as this slot.
...
# AudioDirector, leftover analog coastal unique actor UPROPERTY,
# leftover analog Mission01 unique actor bools.
...
# Clone wrap was EditInstanceOnly BlueprintReadWrite
# Category="Skyguard|Mission01|Environment|PCG" wrapping
...
# (NO spaces around =).
# LOCKED_DECL is `bool bEnableDevelopmentAudition = true;`
# NOT `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# and NOT `FName WeatherIdentity;` (no initializer) and NOT
# leftover `bool bLicensedVegetationLibraryApproved = false;`
# and NOT leftover `bool bAllowAuthoredPCGGeneration = false;`
# and NOT leftover `bool bEnableCoastalHazeTransition = true;`
# and NOT leftover `bool bUseAuthoredLandscapeSurface = false;`
# and NOT leftover ArcadeLook `bool bEnabled = true;`
# and NOT leftover CampaignMissionSpec bNightIdentity
# and NOT leftover coastal PlacementSeed
# and NOT leftover `int32 MinimumMeasuredSamples = 600;`
# and NOT leftover `int32 MaximumAllowedVoices = 48;`
# and NOT leftover sibling `int32 SampleRate = 48000;`
# and NOT leftover sibling `int32 GeneratedByteBudget = 1048576;`
# and NOT leftover private Waves
# and NOT leftover combat look float LookYawLimit = 95.f
...
# / FName AssemblyRevision / float LookYawLimit = 95.f
# / TArray RoutePoints / Mission01 bools
# / coastal PlacementSeed
...
# or Skyguard|Environment|Performance
# or Category="Skyguard" without |Audio|Development.
...
TARGET = "bool bEnableDevelopmentAudition = true;"
TARGET_WRONG_BARE = "bool bEnableDevelopmentAudition;"
TARGET_WRONG_INIT = "bool bEnableDevelopmentAudition = false;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "bool bEnabled = true;"
TARGET_WRONG_ZERO = "bool bEnableDevelopmentAudition = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> bEnableDevelopmentAudition;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> bEnableDevelopmentAudition;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> bEnableDevelopmentAudition;"
...
STOP_BEFORE_SKYLINE_STYLE = "SampleRate"
...
ENABLE_DEVELOPMENT_AUDITION_DECL_RE = re.compile(
    r"bool\s+bEnableDevelopmentAudition\s*=\s*true\s*;"
)
ENABLE_DEVELOPMENT_AUDITION_BARE_RE = re.compile(
    r"bool\s+bEnableDevelopmentAudition\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bEnableDevelopmentAudition without true.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ENABLE_DEVELOPMENT_AUDITION_DECL_RE.search(compact) is None:
        return False
    if ENABLE_DEVELOPMENT_AUDITION_BARE_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "bEnableDevelopmentAudition":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bEnableDevelopmentAudition\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bEnableDevelopmentAudition "
...
class AudioProceduralBankEnableDevelopmentAuditionFieldDeclContractTests(unittest.TestCase):
    def test_audio_procedural_bank_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bEnableDevelopmentAudition"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_SAMPLE_RATE), body)
        self.assertFalse(has_identifier(section, SIBLING_SAMPLE_RATE), section)
        self.assertTrue(has_identifier(body, SIBLING_GENERATED_BYTE_BUDGET), body)
        self.assertFalse(
            has_identifier(section, SIBLING_GENERATED_BYTE_BUDGET),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bEnableDevelopmentAudition"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("bEnableDevelopmentAudition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_enable_development_audition_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_enable_development_audition_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bEnableDevelopmentAudition"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool bEnableDevelopmentAuditions = false;\n",
            "\tint32 bEnableDevelopmentAudition = 1;\n",
            "\tFName bEnableDevelopmentAudition = NAME_None;\n",
            "\tbool bEnableDevelopmentAudition = " + forty + ";\n",
            "\tbool bEnableDevelopmentAudition = " + eighty + ";\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Audio|Development"', theater_specs)
        self.assertIn('Category="Skyguard|Audio|Development"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Development"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Audio|Development"', perf_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tbool\n\tbEnableDevelopmentAudition = true;\n",
            "\tbool   bEnableDevelopmentAudition = true;\n",
            "\tbool\tbEnableDevelopmentAudition = true;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_skyline_style_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bEnableDevelopmentAudition"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_GENERATED_BYTE_BUDGET))
        self.assertFalse(has_identifier(section, SIBLING_GENERATED_BYTE_BUDGET))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_WAVES))
        self.assertFalse(has_identifier(section, SIBLING_WAVES))
        self.assertTrue(has_identifier(leaked, SIBLING_SAMPLE_RATE))
        self.assertFalse(has_identifier(section, SIBLING_SAMPLE_RATE))
        self.assertTrue(has_identifier(leaked, SIBLING_GENERATED_BYTE_BUDGET))
        self.assertFalse(has_identifier(section, SIBLING_GENERATED_BYTE_BUDGET))
        self.assertTrue(has_identifier(leaked, SIBLING_WAVES))
        self.assertFalse(has_identifier(section, SIBLING_WAVES))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio procedural bank enable development audition field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_procedural_bank_enable_development_audition_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
