...
# then `float IngressSwayWavelengthCm = 9000.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Route`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"100.0"`. Default IS `= 9000.f`.
...
# if ClampMin is missing, if the default is not `= 9000.f`, if Category is
...
# LockWindowSpeedCmPerSecond / CriticalSpeedCmPerSecond /
# IngressSwayAmplitudeCm / LockWindowClimbCm /
...
# (NO spaces around =).
# LOCKED_DECL is `float IngressSwayWavelengthCm = 9000.f;`
...
# Fail-closed if ClampMin is missing or default is not 9000.f.
...
TARGET = "float IngressSwayWavelengthCm = 9000.f;"
TARGET_WRONG_BARE = "float IngressSwayWavelengthCm;"
TARGET_WRONG_INIT = "float IngressSwayWavelengthCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float IngressSwayWavelengthCm = true;"
TARGET_WRONG_ZERO = "float IngressSwayWavelengthCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> IngressSwayWavelengthCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> IngressSwayWavelengthCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> IngressSwayWavelengthCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "LockWindowClimbCm"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_ingress_sway_wavelength"
...
INGRESS_SWAY_WAVELENGTH_DECL_RE = re.compile(
    r"float\s+IngressSwayWavelengthCm\s*=\s*9000\.f\s*;"
)
INGRESS_SWAY_WAVELENGTH_BARE_RE = re.compile(
    r"float\s+IngressSwayWavelengthCm\s*;"
)
...
CRITICAL_TURN_RADIUS_LEFTOVER_RE = re.compile(
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float IngressSwayWavelengthCm = 9000.f;`.
    ...
    # IngressSwayWavelengthCm without the 9000.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if INGRESS_SWAY_WAVELENGTH_DECL_RE.search(compact) is None:
        return False
    if INGRESS_SWAY_WAVELENGTH_BARE_RE.search(compact):
        return False
    ...
    if LOCK_WINDOW_CLIMB_CM_LEFTOVER_RE.search(compact):
        return False
    if CRITICAL_TURN_RADIUS_LEFTOVER_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "IngressSwayWavelengthCm":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+IngressSwayWavelengthCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float IngressSwayWavelengthCm "
...
class PathfinderEncounterIngressSwayWavelengthFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "IngressSwayWavelengthCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "IngressSwayWavelengthCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("IngressSwayWavelengthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_ingress_sway_wavelength_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float IngressSwayWavelengthCm;",
            compact_origin.split("IngressSwayWavelengthCm = 9000.f;")[0]
            + compact_origin.split("IngressSwayWavelengthCm = 9000.f;")[-1]
            if "IngressSwayWavelengthCm = 9000.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_ingress_sway_wavelength_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float IngressSwayWavelengthCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("9000.f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat IngressSwayWavelengthCms = 9000.f;\n",
            "\tint32 IngressSwayWavelengthCm = 9000;\n",
            "\tFName IngressSwayWavelengthCm = NAME_None;\n",
            "\tfloat IngressSwayWavelengthCm = " + forty + ";\n",
            "\tfloat IngressSwayWavelengthCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "IngressSwayWavelengthCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "IngressSwayWavelengthCm"))
        self.assertTrue(has_identifier(section, "IngressSwayWavelengthCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter ingress sway wavelength field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_ingress_sway_wavelength_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
