# THIS IS leftover-safe FSkyguardRoutePoint TargetAirspeedKph.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then `float TargetAirspeedKph = 220.f;` (initializer IS `220.f`,
# not `220.0f` / `220.0` / `220` / Harbor 40/80 / `2.f` / `0.f`).
...
# the type is not float, if Category= is added, if
# ClampMin is added, or if the initializer is missing
# or not `= 220.f`.
...
# `struct FSkyguardRouteDefinition` and before later
# structs. Do NOT parse `FSkyguardRouteDefinition`.
# Do NOT parse `USkyguardMissionDefinition` (leftover
# mission-definition field decls).
# Do NOT parse leftover route-point-defaults #180 / #0ac5.
# Do NOT parse leftover route-definition-fields #192 / #dbff.
...
# THIS IS NOT leftover RadioLine isolated fields #1332-#1337
# or the CooldownSeconds draft being opened.
# THIS IS NOT leftover radio-line-defaults #172 / #1dec.
# THIS IS NOT leftover radio-chatter empty fail-closed.
# THIS IS NOT leftover TheaterKit #1299-#1331.
...
# in LOCKED_SCRIPTS).
# Analog: leftover route-point-defaults #180 then isolated
# field decl, same as leftover radio-line-defaults #172
# then isolated RadioLine fields, same as leftover
# briefing-card-defaults then isolated CardId.
# If a clone asserts FName / no `=` /
# Category="Skyguard|Theater" / VisibleAnywhere /
# BlueprintReadWrite / ClampMin present, retarget: type is
# float, initializer is `= 220.f`, EditAnywhere +
# BlueprintReadOnly, no Category=, no ClampMin.
...
ROUTE_DEF_NAME = "FSkyguardRouteDefinition"
MISSION_DEF_NAME = "USkyguardMissionDefinition"
COMPONENT_NAME = MISSION_DEF_NAME
ACTOR_NAME = "ASkyguardCampaignTheaterKit"
NAMESPACE_NAME = "SkyguardCampaignTheaterKit"
TARGET = "float TargetAirspeedKph = 220.f;"
TARGET_WRONG_EQ_NONE = "float TargetAirspeedKph = 220.0f;"
TARGET_WRONG_FALSE = "float TargetAirspeedKph = false;"
TARGET_WRONG_TRUE = "float TargetAirspeedKph = true;"
TARGET_WRONG_ZERO = "float TargetAirspeedKph = 0.f;"
TARGET_WRONG_FLOAT = "int32 TargetAirspeedKph = 220;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "float TargetAirspeedKph;"
TARGET_WRONG_TWO = "float TargetAirspeedKph = 220;"
TARGET_WRONG_TWO_POINT_ZERO = (
    "float TargetAirspeedKph = 220.0;"
)
TARGET_WRONG_LOOKAHEAD = "float TargetAirspeedKph = 2.f;"
...
STOP_BEFORE_MISSION_DEF = "class USkyguardMissionDefinition"
...
SIBLING_COMBAT_ORBIT = "bAllowCombatOrbit"
SIBLING_LINE_ID = SIBLING_POINT_ID
SIBLING_SPEAKER = SIBLING_WORLD_LOCATION
SIBLING_SUBTITLE = SIBLING_LOOK_AHEAD
SIBLING_SOUND = SIBLING_COMBAT_ORBIT
SIBLING_PRIORITY = "RouteId"
SIBLING_COOLDOWN = "Points"
SIBLING_KIT_ID = SIBLING_POINT_ID
SIBLING_LANDMARK_SET = SIBLING_WORLD_LOCATION
SIBLING_BUILDING_KIT = SIBLING_LOOK_AHEAD
SIBLING_BUILDING_TINT = SIBLING_COMBAT_ORBIT
SIBLING_LAMP_TREATMENT = "RouteId"
SIBLING_LAMP_COLOR = "Points"
SIBLING_LAMP_INTENSITY = "ObjectiveId"
SIBLING_ROAD_TREATMENT = "RequiredProgress"
SIBLING_ROAD_TINT = "ScoreReward"
SIBLING_NAMED_LANDMARK = "FormationId"
SIBLING_LANDMARK_TINT = "WaveId"
SIBLING_SILHOUETTE_KIT = "WeatherId"
SIBLING_SILHOUETTE_TINT = "ScoreRules"
SIBLING_LANDMARK_MESH_INDEX = "MissionId"
SIBLING_LANDMARK_SCALE = "CampaignOrder"
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_LOOK_AHEAD,
        SIBLING_COMBAT_ORBIT,
    )


def leftover_later_struct_tokens() -> tuple[str, ...]:
    return (
        "RouteId",
        "ObjectiveId",
        "RequiredProgress",
...
def leftover_component_tokens() -> tuple[str, ...]:
    return leftover_later_struct_tokens()
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float TargetAirspeedKph = 220.f`.
    # Reject missing initializer, `220.0f` / `220.0` / `220` /
    # Harbor 40/80 / sibling `2.f` / `0.f` / leftover Health.
    ...
    if re.search(
        r"float\s+TargetAirspeedKph\s*=\s*220\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+TargetAirspeedKph\s*=\s*220\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+TargetAirspeedKph\s*=\s*220\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+TargetAirspeedKph\s*=\s*220\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+TargetAirspeedKph\s*=\s*2\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+TargetAirspeedKph\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+TargetAirspeedKph\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+TargetAirspeedKph\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+TargetAirspeedKph\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float TargetAirspeedKph is missing from "
...
class RoutePointTargetAirspeedKphFieldDeclContractTests(unittest.TestCase):
    def test_route_point_struct_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "TargetAirspeedKph"),
            section,
        )
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        mixed = (
            ...
            "{{\n"
            f"\tconst FSkyguardRadioLine& {RESOLVE}"
            "(float TargetAirspeedKph);\n"
            "}}\n"
            f"class {ACTOR_NAME}\n"
            "{{\n"
            "public:\n"
        ...
        self.assertIn("TargetAirspeedKph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_target_airspeed_kph_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn("BlueprintReadOnly", UPROPERTY_EDIT_WRITE)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_WRITE)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_WRITE)
        self.assertNotIn("Category", UPROPERTY_EDIT_WRITE)
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "TargetAirspeedKph = 220.0f",
            compact_origin,
        )
        self.assertNotIn(
            "TargetAirspeedKph = 220.0;",
            compact_origin,
        )
        self.assertNotIn(
            "TargetAirspeedKph = 220;",
            compact_origin,
        )
        self.assertNotIn(
            "TargetAirspeedKph = 0.f",
            compact_origin,
        )
        self.assertNotIn(
            "TargetAirspeedKph = 160.f",
            compact_origin,
        )
        self.assertNotIn(
            "TargetAirspeedKph = false",
            compact_origin,
        )
        self.assertNotIn(
            "TargetAirspeedKph = 2.f",
            compact_origin,
        )

    def test_target_airspeed_kph_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float TargetAirspeedKph"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 220.f", LOCKED_DECL)
        self.assertNotIn("= 220.0f", LOCKED_DECL)
        self.assertNotIn("= 220.0", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            "\tfloat TargetAirspeedKp;\n",
            "\tint32 TargetAirspeedKph = 220;\n",
            "\tbool TargetAirspeedKph;\n",
            "\tFText TargetAirspeedKph;\n",
            f"\t{TARGET_WRONG_TWO}\n",
            f"\t{TARGET_WRONG_TWO_POINT_ZERO}\n",
            f"\t{TARGET_WRONG_EMPTY}\n",
            f"\t{TARGET_WRONG_LOOKAHEAD}\n",
            "\tfloat TargetAirspeedKph = " + forty + ";\n",
            "\tfloat TargetAirspeedKph = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_only_fails_closed(self) -> None:
        ...
        cooldown_specs = attached_uproperty_specifiers(
            spec_section(cooldown_clamp)
        )
        self.assertIn('ClampMin="0.0"', cooldown_specs)
        self.assertNotIn("ClampMin", origin)
        priority_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        priority_specs = attached_uproperty_specifiers(
            spec_section(priority_clamp)
        )
        self.assertIn("ClampMax", priority_specs)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "route-point TargetAirspeedKph field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "target_airspeed_kph_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(ROUTE_DEF_NAME, locked_only)
        self.assertNotIn(MISSION_DEF_NAME, locked_only)
        self.assertNotIn("FSkyguardRadioLine", locked_only)
        for helper in namespace_helper_tokens():
