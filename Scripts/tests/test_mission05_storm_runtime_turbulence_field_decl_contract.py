...
# Turbulence FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission05 StormRuntime
# Turbulence. THIS IS NOT leftover storm-runtime-
# defaults #89c9. THIS IS NOT leftover GetStormRuntime
# / AdvanceTurbulence / TriggerLightningWindow /
# AdvanceLightningWindow. THIS IS NOT leftover Mission05
...
# INDEX_NONE or lock Turbulence
# construction in the .cpp. This is leftover-safe
# Mission05 StormRuntime Turbulence
...
# Turbulence FIELD (declaration-only). It is
...
# drafts #674-#1144 including leftover Mission02
...
# drafts #674-#1144. Do not reopen leftover
...
class Mission05StormRuntimeTurbulenceFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Turbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_turbulence_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_turbulence_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float Turbulence"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("Turbulence", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("Turbulence", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float Turbulence"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("Turbulence", locked_only)
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission05 StormRuntime Turbulence contains "
    ...
    def test_contract_is_turbulence_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_turbulence(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
