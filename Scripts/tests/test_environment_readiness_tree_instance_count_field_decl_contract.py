...
# TreeInstanceCount FIELD (declaration-only;
...
# FSkyguardEnvironmentReadiness TreeInstanceCount
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 TreeInstanceCount = 0;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardEnvironmentReadiness
# TreeInstanceCount. THIS IS NOT leftover mission-map-
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardEnvironmentReadiness
# TreeInstanceCount. THIS IS NOT leftover Mission05
...
# MissionMapReadiness TreeInstanceCount
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 TreeInstanceCount = 0;
# one-line / split-line UPROPERTY wraps;
...
# TreeInstanceCount FIELD (declaration-only). It is
...
TARGET = "int32 TreeInstanceCount = 0;"
TARGET_WRONG = "int32 TreeInstanceCount = 1;"
LOCKED_DECL = TARGET
...
class EnvironmentReadinessTreeInstanceCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("TreeInstanceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_tree_instance_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_tree_instance_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 TreeInstanceCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("TreeInstanceCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("TreeInstanceCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 TreeInstanceCount"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("TreeInstanceCount", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_tree_instance_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_environment_readiness_tree"
                "_instance_count_field_decl_contract.py"
            )
        )
        self.assertIn(
