# THIS IS leftover-safe FSkyguardBossWeakPointDefinition RequiredWeapon.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName RequiredWeapon;` (no initializer).
...
# `struct FSkyguardBossDefinition` and later structs
...
# (leftover WeakPointId #566 / bDestroyed #569 /
# bExposed #570 / Integrity #571 / MaxIntegrity #573).
...
# Do NOT contract sibling fields WeakPointId /
...
# #153 / #f47d.
# THIS IS NOT leftover mission-definition Weather #357.
# THIS IS NOT leftover RadioLine isolated fields
# #1332-#1337 or leftover #1332-#1349.
...
# the bulk).
# Analog: leftover boss-weak-point-defaults then this
# isolated RequiredWeapon field decl, same as leftover
...
# `FName PointId` / `FName LineId`, retarget: type is
# FName RequiredWeapon, no `=`, specifiers are
# EditAnywhere + BlueprintReadOnly, no Category.
...
TARGET = "FName RequiredWeapon;"
TARGET_WRONG_EQ_NONE = "FName RequiredWeapon = NAME_None;"
TARGET_WRONG_FALSE = "FName RequiredWeapon = false;"
TARGET_WRONG_TRUE = "FName RequiredWeapon = true;"
TARGET_WRONG_ZERO = "FName RequiredWeapon = 0.f;"
TARGET_WRONG_FLOAT = "float RequiredWeapon;"
...
SIBLING_WEAK_POINT_ID = "WeakPointId"
SIBLING_EXPOSES_WEAK_POINT_ID = "ExposesWeakPointId"
SIBLING_INTEGRITY = "Integrity"
LEFTOVER_COMPONENT_DESTROYED = "bDestroyed"
LEFTOVER_COMPONENT_EXPOSED = "bExposed"
LEFTOVER_COMPONENT_MAX_INTEGRITY = "MaxIntegrity"
...
REMAINING_WEAK_POINT_ID_SCRIPT = (
...
def leftover_component_fields() -> tuple[str, ...]:
    return (
...
def leftover_boss_weak_point_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_BOSS_WEAK_POINT_DEFAULTS,
        LEFTOVER_BOSS_WEAK_POINT_DEFAULTS_PY,
        LEFTOVER_BOSS_WEAK_POINT_DEFAULTS_TESTS,
...
def leftover_route_point_scripts() -> tuple[str, ...]:
    return (
        ...
        LEFTOVER_ENEMY_FORMATION_DEFAULTS,
        LEFTOVER_ENEMY_WAVE_DEFAULTS,
    ) + leftover_boss_weak_point_scripts()
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+RequiredWeapon\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName RequiredWeapon is missing from "
...
def require_required_weapon_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} RequiredWeapon UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} RequiredWeapon"
        )
...
class BossWeakPointDefinitionRequiredWeaponFieldDeclContractTests(unittest.TestCase):
    def test_boss_weak_point_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RequiredWeapon"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RequiredWeapon"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_DEFINITION, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        self.assertNotIn(BOSS_DEFINITION_NAME, section)
        ...
        self.assertIn("RequiredWeapon", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_required_weapon_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_WEAK_POINT_ID};\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_required_weapon_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName RequiredWeapon;", compact_origin)
        self.assertNotIn("RequiredWeapon = NAME_None", compact_origin)
        self.assertNotIn("RequiredWeapon = false", compact_origin)
        self.assertNotIn("RequiredWeapon = true", compact_origin)
        self.assertNotIn("RequiredWeapon = 0.f", compact_origin)
        self.assertNotIn("RequiredWeapon = 160.f", compact_origin)

    def test_required_weapon_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName RequiredWeapon"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFName RequiredWeapons;\n",
            "\tint32 RequiredWeapon;\n",
            "\tbool RequiredWeapon;\n",
            "\tfloat RequiredWeapon = " + forty + ";\n",
            "\tfloat RequiredWeapon = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_required_weapon_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_weapon_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_weapon_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_weapon_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_weapon_specifiers(spec_section(with_category))
    ...
    def test_does_not_contract_sibling_boss_weak_point_fields(self) -> None:
        ...
        for leftover in leftover_weather_profile_siblings():
            ...
            self.assertNotIn(leftover, LOCKED_DECL)
        for leftover in leftover_component_fields():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, LOCKED_DECL)
        for leftover in leftover_theater_siblings():
            self.assertNotIn(leftover, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-weak-point-definition RequiredWeapon field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "required_weapon_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_COMPONENT_DESTROYED_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_EXPOSED_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_INTEGRITY_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_MAX_INTEGRITY_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(BOSS_DEFINITION_NAME, locked_only)
        self.assertNotIn(WEATHER_PROFILE_NAME, locked_only)
        self.assertNotIn(OBJECTIVE_PROGRESS_NAME, locked_only)
        self.assertNotIn(SIBLING_WEAK_POINT_ID, locked_only)
        self.assertNotIn(SIBLING_EXPOSES_WEAK_POINT_ID, locked_only)
        self.assertNotIn(SIBLING_INTEGRITY, locked_only)
        self.assertNotIn(LEFTOVER_COMPONENT_DESTROYED, locked_only)
        self.assertNotIn(LEFTOVER_COMPONENT_EXPOSED, locked_only)
        self.assertNotIn(LEFTOVER_COMPONENT_MAX_INTEGRITY, locked_only)
