# THIS IS leftover-safe FSkyguardBriefingCard::Priority.
# origin/main form: split-line UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `int32 Priority = 0;` with no Category=.
...
# Do not contract Body (has meta MultiLine) or Pictogram.
...
# no Category=. Accept `int32 Priority = 0`. Fail-closed if type/decl
# missing, renamed, Category= added, or default is not 0.
...
TARGET = "int32 Priority = 0;"
TARGET_WRONG_FALSE = "int32 Priority = false;"
TARGET_WRONG_ONE = "int32 Priority = 1;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # for FSkyguardBriefingCard::Priority (must be = 0).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+Priority\b",
        compact,
    )
    if match is None:
        raise AssertionError(
...
class BriefingCardPriorityFieldDeclContractTests(unittest.TestCase):
    def test_briefing_card_struct_exists(self) -> None:
        ...
        self.assertNotIn("int32 Priority)", section)
    ...
    def test_priority_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 Priority"), LOCKED_DECL)
    ...
    def test_does_not_contract_body_or_pictogram(self) -> None:
        self.assertNotIn("Body", LOCKED_DECL)
        self.assertNotIn("Pictogram", LOCKED_DECL)
        self.assertNotIn("MultiLine", LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        self.assertEqual(UPROPERTY_VISIBLE, "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)")
        self.assertNotIn("Category", UPROPERTY_VISIBLE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing-card Priority field contract contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_briefing_card_priority_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertNotIn("Body;", locked_only)
        self.assertNotIn("Pictogram", locked_only)
        self.assertNotIn("MultiLine", locked_only)
