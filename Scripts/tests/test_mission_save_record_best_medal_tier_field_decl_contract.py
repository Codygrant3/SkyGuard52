# THIS IS leftover-safe FSkyguardMissionSaveRecord BestMedalTier.
...
# then `int32 BestMedalTier = 0;`.
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Initializer `= 0` is REQUIRED.
# Spaces around `=` in meta are required. ClampMax is "3".
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# int32, initializer dropped or changed, ClampMin/ClampMax
# missing or wrong, or Category= added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardMissionSaveRecord` body ONLY after
# `struct FSkyguardMissionSaveRecord`. Stop at
# `class SKYGUARD52_API USkyguardCampaignSaveGame` or at
# the `UCLASS(BlueprintType)` immediately after the struct.
# CRITICAL: do NOT search for the token
# `class` + leftover save-game class without the API macro
# (origin/main uses `class SKYGUARD52_API` + leftover
# save-game class, so a naive class find returns -1 and
# would leak leftover USkyguardCampaignSaveGame fields
# SaveVersion / CampaignId / MissionRecords / SavedAtUtc /
# MinSupportedSaveVersion / CurrentSaveVersion /
# MigrateCampaignSave).
# Sibling decls stay unclaimed: bCompleted / BestScore /
# BestCompletionTimeSeconds.
# Analog leftover mission-save-record-defaults #206 / #ac38
# then isolated BestMedalTier field decl.
# THIS IS NOT leftover bCompleted isolated decl.
# THIS IS NOT leftover BestScore isolated decl.
# THIS IS NOT leftover campaign-save-empty-fail-closed.
# THIS IS NOT leftover USkyguardCampaignSaveGame
# MissionRecords #347.
...
# ObjectiveAnchor / LandmarkAnchor defaults.
...
# `= true` / bare bool / `= false`, retarget: type is int32,
# initializer is `= 0`, BlueprintReadWrite, ClampMin = "0",
# ClampMax = "3", spaces around `=` in meta, no Category.
...
STRUCT_NAME = "FSkyguardMissionSaveRecord"
ACTOR_NAME = "ASkyguardCampaignTheaterKit"
NAMESPACE_NAME = "SkyguardCampaignTheaterKit"
TARGET = "int32 BestMedalTier = 0;"
TARGET_WRONG_BARE = "int32 BestMedalTier;"
TARGET_WRONG_TRUE = "int32 BestMedalTier = true;"
TARGET_WRONG_ONE = "int32 BestMedalTier = 1;"
TARGET_WRONG_FALSE = "int32 BestMedalTier = false;"
TARGET_WRONG_FLOAT = "float BestMedalTier = 0;"
TARGET_WRONG_FNAME = "FName BestMedalTier;"
TARGET_WRONG_FTEXT = "FText BestMedalTier;"
TARGET_WRONG_BOOL = "bool BestMedalTier = 0;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_CATEGORY = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
    'Category="Skyguard|Campaign")'
)
TARGET_WRONG_NO_META = "UPROPERTY(EditAnywhere, BlueprintReadWrite)"
TARGET_WRONG_CLAMP_MAX = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
    'meta = (ClampMin = "0", ClampMax = "4"))'
)
TARGET_WRONG_CLAMP_MIN = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
    'meta = (ClampMin = "1", ClampMax = "3"))'
)
TARGET_WRONG_NO_SPACES = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
...
SIBLING_COMPLETED = "bCompleted"
SIBLING_BEST_SCORE = "BestScore"
SIBLING_BEST_TIME = "BestCompletionTimeSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_mission_save_record_best_medal_tier"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare / `= true` / `= 1` / `= false` /
    # `= 0.f` / leftover float Health when origin/main
    # has int32 = 0. Do not accept sibling bCompleted /
    # BestScore / BestCompletionTimeSeconds fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+BestMedalTier\s*;",
        compact,
    ) and re.search(
        r"int32\s+BestMedalTier\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+BestMedalTier\s*=\s*true",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BestMedalTier\s*=\s*false",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BestMedalTier\s*=\s*1\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BestMedalTier\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+BestMedalTier\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+BestMedalTier\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+BestMedalTier\b",
        compact,
    ):
        return False
    if re.search(
...
def parse_window(header: str) -> str:
    ...
    # Stop at the UCLASS immediately after the struct, or at
    # class SKYGUARD52_API USkyguardCampaignSaveGame.
    # Never use naive `class` + leftover save-game class
    # without the API macro: origin/main find returns -1
    # and would leak leftover CampaignSave fields.
    stop = header.find(STOP_BEFORE_UCLASS, start)
    stop_token = STOP_BEFORE_UCLASS
    if stop < 0:
        stop = header.find(STOP_BEFORE_SAVE_GAME_API, start)
        stop_token = STOP_BEFORE_SAVE_GAME_API
    if stop < 0:
        raise AssertionError(
            f"{STRUCT_NAME} parse window is missing {STOP_BEFORE_UCLASS}"
        )
    window = header[start:stop]
    if STOP_BEFORE_SAVE_GAME_API in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_SAVE_GAME_API}"
        )
    ...
    naive = "class " + LEFTOVER_SAVE_GAME_CLASS
    if stop_token == naive:
        raise AssertionError(
            f"{STRUCT_NAME} parse window used naive {naive}"
        )
    return window
...
def struct_body(header: str) -> str:
    window = header
    if STRUCT_RE.search(header) is not None and (
        STOP_BEFORE_UCLASS in header
        or STOP_BEFORE_SAVE_GAME_API in header
    ):
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+BestMedalTier\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 BestMedalTier is missing from "
...
class MissionSaveRecordBestMedalTierFieldDeclContractTests(
    unittest.TestCase
):
    def test_mission_save_record_struct_exists(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_SAVE_GAME_API, header)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedMissionSaveRecord\n{\n};\n"
            )
    ...
    def test_missing_best_medal_tier_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tbool {SIBLING_COMPLETED} = false;\n"
            f"\tint32 {SIBLING_BEST_SCORE} = 0;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMax = "3"', specifiers)
        self.assertNotIn("ClampMin=", specifiers.replace("ClampMin = ", ""))
        self.assertNotIn("ClampMax=", specifiers.replace("ClampMax = ", ""))
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 BestMedalTier = 0;", compact_origin)
        self.assertNotIn("BestMedalTier = true", compact_origin)
        self.assertNotIn("BestMedalTier = 0.f", compact_origin)
        self.assertNotIn("BestMedalTier = 160.f", compact_origin)

    def test_best_medal_tier_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 BestMedalTier"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tbool " + leftover_retired_primary_hits_field() + " = true;\n"
        )
        leftover_guided = (
            "\tbool " + leftover_retired_guided_hits_field() + " = true;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_BEST_SCORE};\n",
            f"\tFText {SIBLING_COMPLETED};\n",
            "\tint32 BestMedalTiers = 0;\n",
            "\tbool BestMedalTier = 0;\n",
            "\tFText BestMedalTier;\n",
            "\tfloat BestMedalTier = " + forty + ";\n",
            "\tfloat BestMedalTier = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_dropped_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('ClampMax = "3"', origin)
        no_meta = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_NO_META}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        dropped_meta = attached_uproperty_specifiers(spec_section(no_meta))
        self.assertNotIn("ClampMin", dropped_meta)
        self.assertNotIn("ClampMax", dropped_meta)
        ...
        max_specs = attached_uproperty_specifiers(spec_section(wrong_max))
        self.assertIn('ClampMax = "4"', max_specs)
        self.assertNotIn('ClampMax = "4"', origin)
        self.assertIn('ClampMax = "3"', origin)
        wrong_min = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_CLAMP_MIN}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        min_specs = attached_uproperty_specifiers(spec_section(wrong_min))
        self.assertIn('ClampMin = "1"', min_specs)
        self.assertNotIn('ClampMin = "1"', origin)
        no_spaces = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_NO_SPACES}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        tight = attached_uproperty_specifiers(spec_section(no_spaces))
        self.assertIn("ClampMin=", tight)
        self.assertNotIn("ClampMin = ", tight)
        self.assertIn("ClampMin = ", origin)
    ...
    def test_does_not_contract_sibling_save_record_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, SIBLING_COMPLETED), section)
        self.assertTrue(has_identifier(section, SIBLING_BEST_SCORE), section)
        self.assertTrue(has_identifier(section, SIBLING_BEST_TIME), section)
    ...
    def test_does_not_parse_leftover_save_game_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        naive = "class " + LEFTOVER_SAVE_GAME_CLASS
        self.assertEqual(header.find(naive), -1)
        self.assertIn(STOP_BEFORE_SAVE_GAME_API, header)
        for leftover in leftover_save_game_field_tokens():
            self.assertIn(leftover, header)
            self.assertNotIn(leftover, section)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("EditAnywhere", UPROPERTY_EDIT_READWRITE)
        self.assertIn("BlueprintReadWrite", UPROPERTY_EDIT_READWRITE)
        self.assertIn('meta = (ClampMin = "0", ClampMax = "3")', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("Category", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertIn('ClampMax = "3"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMax = "4"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardCampaignSaveGame.h",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-save-record BestMedalTier field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "best_medal_tier_field_decl_contract.py"
        ))
        self.assertIn(LEFTOVER_SAVE_RECORD_COMPLETED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SAVE_RECORD_BEST_SCORE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LANDMARK_ID_DECL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LANDMARK_ROLE_DECL, LOCKED_SCRIPTS)
