...
# ShrubInstanceCount FIELD (declaration-only;
...
# FSkyguardEnvironmentReadiness ShrubInstanceCount
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 ShrubInstanceCount = 0;
...
# ShrubInstances HISM. THIS IS NOT leftover
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardEnvironmentReadiness
# ShrubInstanceCount. THIS IS NOT leftover mission-map-
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardEnvironmentReadiness
# ShrubInstanceCount. THIS IS NOT leftover Mission05
...
# merge #665 #666 #668-#673, or open drafts #674-#1167.
...
# MissionMapReadiness ShrubInstanceCount
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 ShrubInstanceCount = 0;
# one-line / split-line UPROPERTY wraps;
...
# ShrubInstanceCount FIELD (declaration-only). It is
...
# drafts #674-#1167 including leftover Mission02
...
# drafts #674-#1167. Do not reopen leftover
...
TARGET = "int32 ShrubInstanceCount = 0;"
TARGET_WRONG = "int32 ShrubInstanceCount = 1;"
LOCKED_DECL = TARGET
...
class EnvironmentReadinessShrubInstanceCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("ShrubInstanceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_shrub_instance_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_shrub_instance_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 ShrubInstanceCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("ShrubInstanceCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("ShrubInstanceCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 ShrubInstanceCount"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("ShrubInstanceCount", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_shrub_instance_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_environment_readiness_shrub"
                "_instance_count_field_decl_contract.py"
            )
        )
        self.assertIn(
