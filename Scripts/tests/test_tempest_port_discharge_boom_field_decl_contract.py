...
# Category="Skyguard|Mission05|Boss") is required as
...
# leftover isolated-test drafts #107–#508
# including leftover
...
FIELD_NAME = "PortDischargeBoom"
UPROPERTY_MISSION05 = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Mission05|Boss")'
)
# Leftover #56–#64 plus leftover Harbor #6/#8/#9,
# leftover theater-kit #59, leftover isolated-test
# drafts #107–#508, plus TempestBoss
# production files. This lane only adds an isolated
# Python PortDischargeBoom field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardTempestBoss::PortDischargeBoom",
    "SkyguardTempestBoss.cpp",
    "SetExposed",
...
class TempestPortDischargeBoomFieldDeclContractTests(unittest.TestCase):
    def test_tempest_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, PORT_DISCHARGE_BOOM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("PortDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, PORT_DISCHARGE_BOOM_FIELD))

    def test_missing_port_discharge_boom_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardTempestBoss();\n"
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardDischargeBoom;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("PortDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("PortDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission05|Boss"', section)
        self.assertTrue(has_declaration(section, PORT_DISCHARGE_BOOM_FIELD), section)
        self.assertNotIn("UPROPERTY", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("VisibleAnywhere", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("BlueprintReadOnly", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("Category", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_MISSION05)
        self.assertNotIn("BlueprintCallable", UPROPERTY_MISSION05)
        self.assertIn("Skyguard|Mission05|Boss", UPROPERTY_MISSION05)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("PortDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tUSkyguardBossWeakPointComponent* PortDischargeBoom;\n"
        as_array = (
            "\tTArray<TObjectPtr<USkyguardBossWeakPointComponent>> "
            "PortDischargeBoom;\n"
        )
        scene = "\tTObjectPtr<USceneComponent> PortDischargeBoom;\n"
        static_mesh = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, PORT_DISCHARGE_BOOM_FIELD)
            self.assertIn("PortDischargeBoom", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_port_discharge_boom_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, PORT_DISCHARGE_BOOM_FIELD),
            PORT_DISCHARGE_BOOM_FIELD,
        )
        self.assertTrue(has_declaration(section, PORT_DISCHARGE_BOOM_FIELD))
        self.assertEqual(declaration_count(section, PORT_DISCHARGE_BOOM_FIELD), 1)
        self.assertTrue(
            PORT_DISCHARGE_BOOM_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            PORT_DISCHARGE_BOOM_FIELD,
        )
        self.assertTrue(PORT_DISCHARGE_BOOM_FIELD.endswith(";"), PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("PortDischargeBoom", PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_DISCHARGE_BOOM_FIELD,
        )
        self.assertIn("TObjectPtr<", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("TArray<", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("=", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("INDEX_NONE", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("UFUNCTION", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("{", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("}", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("return ", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardDischargeBoom", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ControlServo", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("EngineIntake", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisPortPanel", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisStarboardPanel", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisIntakePanel", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("CommandAntenna", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("NoseCamera", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ControlLinkage", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisNose", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("Engine", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisTail", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisSpine", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("EncounterController", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("AdvanceEncounter", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ResetEncounterState", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IsRouteStateSafe", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetRouteProgress", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IssueClimbCommand", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetGunnerMount", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("FaceWorldLocation", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortGuidanceArray", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("CommandCore", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortEngine", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardEngine", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetLightningExposed", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("HullCollider", PORT_DISCHARGE_BOOM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION05} {PORT_DISCHARGE_BOOM_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, PORT_DISCHARGE_BOOM_FIELD), 1)
        one_line = f"{{\npublic:\n\t{PORT_DISCHARGE_BOOM_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, PORT_DISCHARGE_BOOM_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, PORT_DISCHARGE_BOOM_FIELD), section)
        self.assertEqual(
            require_declaration(section, PORT_DISCHARGE_BOOM_FIELD),
    ...
    def test_assigned_port_discharge_boom_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "PortDischargeBoom = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("PortDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, PORT_DISCHARGE_BOOM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        debris_port = "\tTObjectPtr<UStaticMeshComponent> DebrisPortPanel;\n"
        debris_starboard = (
            "\tTObjectPtr<UStaticMeshComponent> DebrisStarboardPanel;\n"
        )
        debris_intake = (
        ...
        for region in (
            ...
            self.assertIn("PortDischargeBoom", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, PORT_DISCHARGE_BOOM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{PORT_DISCHARGE_BOOM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            PORT_DISCHARGE_BOOM_FIELD.startswith("UFUNCTION"),
            PORT_DISCHARGE_BOOM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, PORT_DISCHARGE_BOOM_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", PORT_DISCHARGE_BOOM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_port_discharge_boom_cpp_body(self) -> None:
        locked_only = f"{PORT_DISCHARGE_BOOM_FIELD}\n"
        self.assertNotIn("{", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("}", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("return ", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardTempestBoss::PortDischargeBoom", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SkyguardTempestBoss.cpp", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SkyguardTempestBoss.cpp", locked_only)
        self.assertNotIn("return false", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("return true", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetExposed", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("RefreshAuthoredWeakPointRegistry", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetBossPhase", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", PORT_DISCHARGE_BOOM_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{PORT_DISCHARGE_BOOM_FIELD}\n"
        for token in TEMPEST_SIBLING_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardDischargeBoom", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ControlServo", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("EngineIntake", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisPortPanel", PORT_DISCHARGE_BOOM_FIELD)
        ...
        self.assertNotIn("DebrisIntakePanel", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("USkyguardBossWeakPointComponent", PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn("PortDischargeBoom", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_get_civilian_separation_meters_sibling(
        ...
        for token in GET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetFinaleStage", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IssueClimbCommand", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IsWreckDiverted", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_finale_stage(self) -> None:
        ...
        for token in GET_FINALE_STAGE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetFinaleStage", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_is_climb_command_issued_sibling(
        ...
        for token in IS_CLIMB_COMMAND_ISSUED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IsClimbCommandIssued", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), PORT_DISCHARGE_BOOM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, PORT_DISCHARGE_BOOM_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_weak_point_component_fields(
        ...
        self.assertNotIn("UStaticMeshComponent", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_guidance_array(
        ...
        for token in LEFTOVER_STARBOARD_GUIDANCE_ARRAY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardGuidanceArray", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_strike_bay_mechanism(
        ...
        for token in LEFTOVER_PORT_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_strike_bay_mechanism(
        ...
        for token in LEFTOVER_STARBOARD_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_cooling_system(self) -> None:
        ...
        for token in LEFTOVER_PORT_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortCoolingSystem", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_cooling_system(
        ...
        for token in LEFTOVER_STARBOARD_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardCoolingSystem", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_jammer(self) -> None:
        ...
        for token in LEFTOVER_JAMMER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("Jammer", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_last_flight_debris_fields(
        ...
        for token in LEFTOVER_LAST_FLIGHT_DEBRIS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisArmorPort", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisArmorStarboard", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisStrikeBayPort", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisStrikeBayStarboard", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisEnginePort", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DebrisEngineStarboard", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("OpenOrbitExposure", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            PORT_DISCHARGE_BOOM_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardGunner", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SkyguardRadarNode", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(self) -> None:
        ...
        self.assertNotIn("USkyguardDebriefWidget", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("MaxIntegrity", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("CurrentIntegrity", PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, PORT_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", PORT_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{PORT_DISCHARGE_BOOM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, PORT_DISCHARGE_BOOM_FIELD),
            PORT_DISCHARGE_BOOM_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss::PortDischargeBoom", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "tempest PortDischargeBoom field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{PORT_DISCHARGE_BOOM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, PORT_DISCHARGE_BOOM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                PORT_DISCHARGE_BOOM_FIELD.lower(),
                "tempest PortDischargeBoom contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, PORT_DISCHARGE_BOOM_FIELD)

    def test_contract_is_port_discharge_boom_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("USkyguardBossWeakPointComponent", PORT_DISCHARGE_BOOM_FIELD)
        self.assertTrue(PORT_DISCHARGE_BOOM_FIELD.endswith(";"))
        self.assertNotIn("=", PORT_DISCHARGE_BOOM_FIELD)
        self.assertIn(UPROPERTY_MISSION05, section)
