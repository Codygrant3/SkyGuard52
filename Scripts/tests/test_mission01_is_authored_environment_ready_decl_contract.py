...
# INDEX_NONE or lock IsAuthoredEnvironmentReady
...
# #6b9d / #b931 or leftover Environment
# GetReadiness. Distinct from leftover
# Integration methods
...
# not Environment without PCG, not
# Environment|Landscape, not Environment|Layout,
# not Environment|Materials, not
# Environment|Visibility, not Integration, not
...
# IsAuthoredEnvironmentReady method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::IsAuthoredEnvironmentReady",
    "SkyguardMission01EnvironmentDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission01IsAuthoredEnvironmentReadyDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, IS_AUTHORED_ENVIRONMENT_READY),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{IS_AUTHORED_ENVIRONMENT_READY}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_AUTHORED_ENVIRONMENT_READY)
        self.assertIn("IsAuthoredEnvironmentReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, IS_AUTHORED_ENVIRONMENT_READY)
    ...
    def test_missing_is_authored_environment_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
            ...
            f"\t{SET_USE_AUTHORED_LANDSCAPE}\n"
            f"\t{REFRESH_AUTHORED_BINDINGS}\n"
            f"\t{IS_PCG_GENERATION_AUTHORIZED}\n"
            f"\t{ENVIRONMENT_GET_READINESS}\n"
            f"\t{LICENSED_VEGETATION_LIBRARY_FIELD}\n"
            f"\t{USE_AUTHORED_LANDSCAPE_SURFACE_FIELD}\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_AUTHORED_ENVIRONMENT_READY)
        self.assertIn("IsAuthoredEnvironmentReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_PCG}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_AUTHORED_ENVIRONMENT_READY)
        self.assertIn("IsAuthoredEnvironmentReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_PCG, section)
        ...
        self.assertNotIn("UFUNCTION", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("BlueprintCallable", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("BlueprintPure", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("Category", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("BlueprintCallable", UFUNCTION_PCG)
        self.assertIn("Skyguard|Mission01|Environment|PCG", UFUNCTION_PCG)
        self.assertIn("BlueprintPure", UFUNCTION_PCG)
        self.assertNotIn("Integration", UFUNCTION_PCG)
        self.assertNotIn("Briefing", UFUNCTION_PCG)
        self.assertNotIn("Mission02", UFUNCTION_PCG)
        self.assertNotIn("Boss", UFUNCTION_PCG)
        self.assertNotIn("Destruction", UFUNCTION_PCG)
        self.assertNotIn("Apache", UFUNCTION_PCG)
        self.assertNotIn("Mission07", UFUNCTION_PCG)
        self.assertNotIn("Mission10", UFUNCTION_PCG)
        self.assertNotIn("Encounter", UFUNCTION_PCG)
        self.assertNotIn("Safety", UFUNCTION_PCG)
        self.assertIn("|PCG", UFUNCTION_PCG)
        self.assertNotIn("|Landscape", UFUNCTION_PCG)
        self.assertNotIn("|Layout", UFUNCTION_PCG)
        self.assertNotIn("|Materials", UFUNCTION_PCG)
        self.assertNotIn("|Visibility", UFUNCTION_PCG)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_PCG)
            self.assertNotIn(invented, IS_AUTHORED_ENVIRONMENT_READY)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_AUTHORED_ENVIRONMENT_READY)
        self.assertIn("IsAuthoredEnvironmentReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid IsAuthoredEnvironmentReady() const;\n"
        )
        missing_const = (
            "\tbool IsAuthoredEnvironmentReady();\n"
        )
        extra_arg = (
            "\tbool IsAuthoredEnvironmentReady("
        ...
        as_int = (
            "\tint32 IsAuthoredEnvironmentReady() const;\n"
        )
        renamed = "\tbool IsAuthoredReady() const;\n"
        short_name = "\tbool IsEnvironmentReady() const;\n"
        getter = "\tbool GetAuthoredEnvironmentReady() const;\n"
        leftover_point = (
            "\tbool IsPointAllowedForPCG(const FVector& WorldPoint) const;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_AUTHORED_ENVIRONMENT_READY)
            self.assertIn("IsAuthoredEnvironmentReady", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_authored_environment_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, IS_AUTHORED_ENVIRONMENT_READY),
            IS_AUTHORED_ENVIRONMENT_READY,
        )
        self.assertTrue(has_declaration(section, IS_AUTHORED_ENVIRONMENT_READY))
        self.assertEqual(
            declaration_count(section, IS_AUTHORED_ENVIRONMENT_READY),
            1,
        )
        self.assertTrue(
            IS_AUTHORED_ENVIRONMENT_READY.startswith("bool "),
            IS_AUTHORED_ENVIRONMENT_READY,
        )
        self.assertFalse(
            IS_AUTHORED_ENVIRONMENT_READY.endswith(";"),
            IS_AUTHORED_ENVIRONMENT_READY,
        )
        ...
        self.assertTrue(
            IS_AUTHORED_ENVIRONMENT_READY.endswith(" const"),
            IS_AUTHORED_ENVIRONMENT_READY,
        )
        self.assertNotIn("TArray<", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("TObjectPtr", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("TSoftObjectPtr", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("INDEX_NONE", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("UFUNCTION", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("{", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("}", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("return ", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn(" const;", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("void ", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("Root", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("Briefing", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("AudioDirector", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("RadioChatter", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("SortiePresentation", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("CampaignDefinition", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("MissionDefinition", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("Readiness", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("bAutoInitialize", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("bAllowBoundedActorSpawning", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("bAutoLaunchAfterBriefing", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("PathfinderSpawnLocation", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetAircraft", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("BindRuntimeActors", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("HandleDroneCityImpact", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("ConfigureMissionDefinition", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("InitializePlayableMission", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("NotifyProtectedAssetFailed", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("SynchronizeRuntimeState", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("NotifyObjectiveProgress", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetReadiness", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetObjectiveRuntime", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetGunner", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetPathfinder", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetMissionId", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("ValidateMissionContract", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("ConfigureFromMission", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("AdvanceBriefing", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("SetAssetsReady", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("AcknowledgeAndLaunch", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("CanLaunch", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetElapsedSeconds", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetBriefingState", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetMinimumWarmupSeconds", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetBriefingText", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetRadioChatter", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetPresentation", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("GetMissionTitle", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("AcknowledgeBriefing", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("LaunchSortie", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("HullCollider", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("OpticalTracker", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("WeaponServo", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("CountermeasurePod", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("MinHeightFromOriginCm", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("RadarNode", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("ESkyguardMission02WaveState", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("HarborIndustrial", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("MaxIntegrity", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("CurrentIntegrity", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("FillAndFinalize", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("FillAndFail", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("IsRouteExclusionSafe", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("bRouteExclusionValid", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            IS_AUTHORED_ENVIRONMENT_READY,
        )
        self.assertNotIn("bAllowAuthoredPCGGeneration", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("bProductionLandscapeBound", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("bUseAuthoredLandscapeSurface", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("RebuildProductionLayout", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("HasContinuousCoastline", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn(
            "RefreshAuthoredEnvironmentBindings",
            IS_AUTHORED_ENVIRONMENT_READY,
        )
        self.assertNotIn("IsPointAllowedForPCG", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("IsPCGGenerationAuthorized", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("IsCorePlayableReady", IS_AUTHORED_ENVIRONMENT_READY)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, IS_AUTHORED_ENVIRONMENT_READY)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tIsAuthoredEnvironmentReady() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_PCG} {IS_AUTHORED_ENVIRONMENT_READY}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{IS_AUTHORED_ENVIRONMENT_READY}\n}}\n"
        self.assertTrue(has_declaration(one_line, IS_AUTHORED_ENVIRONMENT_READY))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, IS_AUTHORED_ENVIRONMENT_READY),
            section,
    ...
    def test_integration_category_does_not_satisfy_environment(self) -> None:
        ...
        self.assertNotEqual(integration, UFUNCTION_PCG)
        self.assertNotEqual(briefing, UFUNCTION_PCG)
        self.assertNotEqual(leftover_environment, UFUNCTION_PCG)
        self.assertNotEqual(leftover_callable, UFUNCTION_PCG)
        self.assertNotEqual(leftover_landscape, UFUNCTION_PCG)
        self.assertNotIn(integration, UFUNCTION_PCG)
        self.assertNotIn(briefing, UFUNCTION_PCG)
        self.assertNotIn(leftover_environment, UFUNCTION_PCG)
        self.assertNotIn(leftover_callable, UFUNCTION_PCG)
        self.assertNotIn(leftover_landscape, UFUNCTION_PCG)

    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{IS_AUTHORED_ENVIRONMENT_READY}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, IS_AUTHORED_ENVIRONMENT_READY)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", IS_AUTHORED_ENVIRONMENT_READY)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_authored_environment_ready_cpp_body(self) -> None:
        locked_only = f"{IS_AUTHORED_ENVIRONMENT_READY}\n"
        self.assertNotIn("{", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("}", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("return ", IS_AUTHORED_ENVIRONMENT_READY)
        ...
        self.assertNotIn("return false", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("return true", IS_AUTHORED_ENVIRONMENT_READY)
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, IS_AUTHORED_ENVIRONMENT_READY)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("CurrentIntegrity", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("SkyguardApacheAircraft.h", IS_AUTHORED_ENVIRONMENT_READY)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", IS_AUTHORED_ENVIRONMENT_READY)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_AUTHORED_ENVIRONMENT_READY}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_AUTHORED_ENVIRONMENT_READY),
            IS_AUTHORED_ENVIRONMENT_READY,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_AUTHORED_ENVIRONMENT_READY)
        ...
        self.assertNotIn("IsPCGGenerationAuthorized", locked_only)
        self.assertNotIn("SetUseAuthoredLandscapeSurfaceForValidation", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 IsAuthoredEnvironmentReady method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{IS_AUTHORED_ENVIRONMENT_READY}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, IS_AUTHORED_ENVIRONMENT_READY.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                IS_AUTHORED_ENVIRONMENT_READY.lower(),
                "mission01 IsAuthoredEnvironmentReady contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_AUTHORED_ENVIRONMENT_READY)

    def test_contract_is_is_authored_environment_ready_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn(" const;", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertFalse(IS_AUTHORED_ENVIRONMENT_READY.endswith(";"))
        self.assertIn(UFUNCTION_PCG, section)

    def test_sibling_director_fields_do_not_satisfy_is_authored_environment_ready(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, IS_AUTHORED_ENVIRONMENT_READY)
            )
            self.assertNotEqual(IS_AUTHORED_ENVIRONMENT_READY, leftover)
        self.assertIn(
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("return ", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn("return Other", IS_AUTHORED_ENVIRONMENT_READY)
        self.assertNotIn(
            "return Readiness.bAuthoredPCGStructureReady",
            IS_AUTHORED_ENVIRONMENT_READY,
        )
    ...
    def test_declaration_accepts_split_parameter_wrap(self) -> None:
        ...
        for wrap in (wrap_const, wrap_type_and_const, wrap_ufunction_split, wrap_origin_inline):
            header = (
                f"class SKYGUARD52_API {CLASS_NAME} "
                f": public AActor\n{{\n{wrap}"
            )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission01_is_authored_environment_ready"
            "_decl_contract.py"
        )
