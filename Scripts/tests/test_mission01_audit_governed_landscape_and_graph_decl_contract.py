...
# Category path Skyguard|Mission01|Environment|Authoring.
# Leftover unique AuthoringResult FIELD drafts wrap struct members with
# NO Category and explicitly do NOT parse this library.
# Category IS `Skyguard|Mission01|Environment|Authoring`.
# BlueprintCallable IS present. const IS absent. static IS present.
# Return type IS FSkyguardMission01EnvironmentAuthoringResult.
# One Director argument (NOT HeightmapSourcePath / GraphPackagePath).
...
# return type is not FSkyguardMission01EnvironmentAuthoringResult,
...
# Do NOT parse struct `FSkyguardMission01EnvironmentAuthoringResult`
# (leftover unique FIELD). Do NOT parse VisibleAudit /
# CaptureConfiguration / MaterialCompilation structs.
# Do NOT parse Validation UFUNCTIONs
# (AuditLandscapeVisibleReadiness / AuditLandscapeMaterialCompilation /
# PrepareGovernedLandscapeForVisibleValidation /
# FinishLandscapeMaterialCompilation) as this slot.
...
# Identifier AuditGovernedLandscapeAndGraph is NOT leftover analog
# AuditLandscapeVisibleReadiness / AuditLandscapeMaterialCompilation
# (Validation) and NOT leftover unique SampleLandscapeHeight /
# SampleLandscapeFootprint and NOT sibling
# AuthorGovernedLandscapeAndGraph /
# AuthorGovernedLandscapeWithExistingGraph and NOT struct field
# bSuccess / Landscape / Graph.
# Clone wrap was BlueprintCallable
# Category="Skyguard|Audio|Production" wrapping
# `void EnsureDefaultEntries();` on USkyguardAudioProductionBank
# — RETARGET hard: class -> USkyguardMission01EnvironmentAuthoringLibrary,
# wrap Category Production -> Skyguard|Mission01|Environment|Authoring,
# decl -> AuditGovernedLandscapeAndGraph static struct-return.
# Clone leftover unique BlueprintPure GetSortieHits Category
# Skyguard|Combat|Sortie — RETARGET Category Combat|Sortie ->
# Skyguard|Mission01|Environment|Authoring. KEEP BlueprintCallable
# (NOT BlueprintPure). NOT leftover int32 GetSortieHits() const;.
# Clone leftover unique BlueprintCallable void ResetSortieCombatStats
# — RETARGET hard; LOCKED_DECL is NOT void ResetSortieCombatStats();.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is static struct-return AuditGovernedLandscapeAndGraph
# with one Director argument
...
# and NOT AuditLandscapeVisibleReadiness
...
# GetActiveRHIAndFeatureLevel / AuditLandscapeVisibleReadiness /
...
TARGET = "static FSkyguardMission01EnvironmentAuthoringResult AuditGovernedLandscapeAndGraph(ASkyguardMission01EnvironmentDirector* Director);"
TARGET_INLINE = (
    "static FSkyguardMission01EnvironmentAuthoringResult "
    "AuditGovernedLandscapeAndGraph("
    "ASkyguardMission01EnvironmentDirector* Director) { }"
...
TARGET_WRONG_BARE = (
    "static FSkyguardMission01EnvironmentAuthoringResult "
    "AuditGovernedLandscapeAndGraph("
    "ASkyguardMission01EnvironmentDirector* Director) const;"
)
TARGET_WRONG_NO_CONST = (
    "bool AuditGovernedLandscapeAndGraph();"
)
TARGET_WRONG_VOID = "bool AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_FLOAT = "float AuditGovernedLandscapeAndGraph();"
TARGET_WRONG_INT = "int32 AuditGovernedLandscapeAndGraph();"
...
TARGET_WRONG_TRUE = "bool AuditGovernedLandscapeAndGraph = true;"
TARGET_WRONG_ZERO = "bool AuditGovernedLandscapeAndGraph = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    ...
    r"AuditGovernedLandscapeAndGraph\s*\(\s*"
    r"ASkyguardMission01EnvironmentDirector\s*\*\s*Director\s*\)\s*"
    r"(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # static FSkyguardMission01EnvironmentAuthoringResult
    # AuditGovernedLandscapeAndGraph(Director).
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bAuditGovernedLandscapeAndGraph\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+AuditGovernedLandscapeAndGraph\b", compact):
        return False
    if re.search(r"\bbool\s+AuditGovernedLandscapeAndGraph\b", compact):
        return False
    if re.search(r"\bfloat\s+AuditGovernedLandscapeAndGraph\b", compact):
        return False
    if re.search(r"\bint32\s+AuditGovernedLandscapeAndGraph\b", compact):
        return False
    if re.search(
        r"\bAuditGovernedLandscapeAndGraph\s*\(\s*\)",
        compact,
    ):
        return False
    if re.search(
        r"\bAuditGovernedLandscapeAndGraph\s*\((?:(?!\)).)*HeightmapSourcePath",
        compact,
    ):
        return False
    if re.search(
        r"\bAuditGovernedLandscapeAndGraph\s*\((?:(?!\)).)*GraphPackagePath",
        compact,
    ):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            ...
            r"AuditGovernedLandscapeAndGraph\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for static FSkyguardMission01EnvironmentAuthoringResult "
...
class Mission01AuditGovernedLandscapeAndGraphDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "AuditGovernedLandscapeAndGraph"),
            section,
        )
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn(
            "static FSkyguardMission01EnvironmentAuthoringResult AuditGovernedLandscapeAndGraph(ASkyguardMission01EnvironmentDirector* Director);",
            compact_origin,
        )
        self.assertNotIn(
            "AuditGovernedLandscapeAndGraph()const;",
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertIn("ASkyguardMission01EnvironmentDirector", LOCKED_DECL)
        self.assertIn("Director", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid AuditGovernedLandscapeAndGraphs();\n",
            "\tint32 AuditGovernedLandscapeAndGraph = 1;\n",
            "\tFName AuditGovernedLandscapeAndGraph = NAME_None;\n",
            "\tvoid AuditGovernedLandscapeAndGraph() = "
            + forty
            + ";\n",
            "\tvoid AuditGovernedLandscapeAndGraph() = "
            + eighty
    ...
    def test_missing_category_or_blueprint_callable_fails_closed(self) -> None:
        ...
        self.assertNotIn('Skyguard|Combat', origin)
        self.assertNotIn('|Validation', origin)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "AuditGovernedLandscapeAndGraph")
        )
        self.assertTrue(
            has_identifier(section, "AuditGovernedLandscapeAndGraph")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 audit governed landscape and graph "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_audit_governed_landscape_and_graph"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertNotIn("AuditLandscapeVisibleReadiness", locked_only)
        ...
        self.assertIn("ASkyguardMission01EnvironmentDirector", locked_only)
        self.assertIn("Director", locked_only)
