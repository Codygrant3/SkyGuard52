...
# INDEX_NONE or lock Root construction in the
# .cpp. This is a FIELD contract on
# ASkyguardMission01IntegrationDirector, not leftover
# BossDrone Root #471, not leftover Pathfinder Root,
# and not leftover Apache AircraftRoot (protected).
# Keep Root distinct from sibling director fields
# Briefing / AudioDirector / RadioChatter /
# SortiePresentation / CampaignDefinition /
# MissionDefinition / Readiness. Do not lock leftover
...
# (`TObjectPtr<USceneComponent> Root;`); accept that
# form and other one-line / split-line wraps. Nearby
# origin/main UPROPERTY(VisibleAnywhere,
# BlueprintReadOnly, Category="Skyguard|Mission01")
...
# isolated-test drafts #107–#571, leftover Apache
...
AIRCRAFT_ROOT_FIELD = "TObjectPtr<USceneComponent> AircraftRoot;"
...
# leftover BossDrone Root #471 stay sibling-only.
LOCKED_SCRIPTS = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01IntegrationDirector::Root",
    "SkyguardMission01IntegrationDirector.cpp",
...
class Mission01RootFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ROOT_FIELD), section)
    ...
    def test_leftover_boss_drone_root_class_does_not_satisfy(self) -> None:
        leftover_boss = (
            "class SKYGUARD52_API ASkyguardBossDrone "
            ": public AActor\n"
            "{\n"
            "public:\n"
            f"\t{ROOT_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            public_section(leftover_boss)
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_leftover_pathfinder_root_class_does_not_satisfy(self) -> None:
        leftover_path = (
            "class SKYGUARD52_API ASkyguardPathfinderBoss "
            ": public AActor\n"
            "{\n"
            "public:\n"
            f"\t{ROOT_FIELD}\n"
    ...
    def test_protected_aircraft_root_does_not_satisfy_public_lock(self) -> None:
        protected_only = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            ": public AActor\n"
            "{\n"
            ...
            f"\t{BRIEFING_FIELD}\n"
            "protected:\n"
            f"\t{AIRCRAFT_ROOT_FIELD}\n"
            "};\n"
        )
        section = public_section(protected_only)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ROOT_FIELD)
        self.assertIn("Root", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ROOT_FIELD))
        self.assertFalse(has_declaration(section, AIRCRAFT_ROOT_FIELD))
    ...
    def test_missing_root_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ROOT_FIELD)
        self.assertIn("Root", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ROOT_FIELD)
        self.assertIn("Root", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ROOT_FIELD), section)
        self.assertNotIn("UPROPERTY", ROOT_FIELD)
        self.assertNotIn("VisibleAnywhere", ROOT_FIELD)
        self.assertNotIn("BlueprintReadOnly", ROOT_FIELD)
        self.assertNotIn("Category", ROOT_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ROOT_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ROOT_FIELD)
        self.assertIn("Root", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tUSceneComponent* Root;\n"
        as_array = "\tTArray<TObjectPtr<USceneComponent>> Root;\n"
        briefing = (
            "\tTObjectPtr<USkyguardMissionBriefingComponent> Root;\n"
        )
        audio = (
            "\tTObjectPtr<USkyguardAudioDirectorComponent> Root;\n"
        )
        weak = "\tTWeakObjectPtr<USceneComponent> Root;\n"
        assigned = "\tTObjectPtr<USceneComponent> Root = nullptr;\n"
        soft = "\tTSoftObjectPtr<USceneComponent> Root;\n"
        ...
        leftover_aircraft_root = f"\t{AIRCRAFT_ROOT_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ROOT_FIELD)
            self.assertIn("Root", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_root_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ROOT_FIELD))
        self.assertEqual(declaration_count(section, ROOT_FIELD), 1)
        self.assertTrue(
            ROOT_FIELD.startswith("TObjectPtr<USceneComponent> "),
            ROOT_FIELD,
        )
        self.assertTrue(ROOT_FIELD.endswith(";"), ROOT_FIELD)
        self.assertIn("Root", ROOT_FIELD)
        self.assertIn("USceneComponent", ROOT_FIELD)
        self.assertIn("TObjectPtr<", ROOT_FIELD)
        self.assertNotIn("TArray<", ROOT_FIELD)
        self.assertNotIn("TSoftObjectPtr", ROOT_FIELD)
        self.assertNotIn("=", ROOT_FIELD)
        self.assertNotIn("INDEX_NONE", ROOT_FIELD)
        self.assertNotIn("UFUNCTION", ROOT_FIELD)
        self.assertNotIn("{", ROOT_FIELD)
        self.assertNotIn("}", ROOT_FIELD)
        self.assertNotIn("return ", ROOT_FIELD)
        self.assertNotIn("Briefing", ROOT_FIELD)
        self.assertNotIn("AudioDirector", ROOT_FIELD)
        self.assertNotIn("RadioChatter", ROOT_FIELD)
        self.assertNotIn("SortiePresentation", ROOT_FIELD)
        self.assertNotIn("CampaignDefinition", ROOT_FIELD)
        self.assertNotIn("MissionDefinition", ROOT_FIELD)
        self.assertNotIn("Readiness", ROOT_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", ROOT_FIELD)
        self.assertNotIn("AircraftRoot", ROOT_FIELD)
        self.assertNotIn("GetAircraft", ROOT_FIELD)
        self.assertNotIn("ConfigureFromMission", ROOT_FIELD)
        self.assertNotIn("AdvanceBriefing", ROOT_FIELD)
        self.assertNotIn("SetAssetsReady", ROOT_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", ROOT_FIELD)
        self.assertNotIn("CanLaunch", ROOT_FIELD)
        self.assertNotIn("GetElapsedSeconds", ROOT_FIELD)
        self.assertNotIn("GetBriefingState", ROOT_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", ROOT_FIELD)
        self.assertNotIn("GetBriefingText", ROOT_FIELD)
        self.assertNotIn("GetRadioChatter", ROOT_FIELD)
        self.assertNotIn("GetPresentation", ROOT_FIELD)
        self.assertNotIn("GetMissionTitle", ROOT_FIELD)
        self.assertNotIn("AcknowledgeBriefing", ROOT_FIELD)
        self.assertNotIn("LaunchSortie", ROOT_FIELD)
        self.assertNotIn("HullCollider", ROOT_FIELD)
        self.assertNotIn("OpticalTracker", ROOT_FIELD)
        self.assertNotIn("WeaponServo", ROOT_FIELD)
        self.assertNotIn("CountermeasurePod", ROOT_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ROOT_FIELD)
        self.assertNotIn("RadarNode", ROOT_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", ROOT_FIELD)
        self.assertNotIn("HarborIndustrial", ROOT_FIELD)
        self.assertNotIn("MaxIntegrity", ROOT_FIELD)
        self.assertNotIn("CurrentIntegrity", ROOT_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ROOT_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01} {ROOT_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, ROOT_FIELD), 1)
        one_line = f"{{\npublic:\n\t{ROOT_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ROOT_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ROOT_FIELD), section)
        self.assertEqual(
            require_declaration(section, ROOT_FIELD),
    ...
    def test_assigned_root_does_not_satisfy(self) -> None:
        assigned = "\tTObjectPtr<USceneComponent> Root = nullptr;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ROOT_FIELD)
        self.assertIn("Root", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ROOT_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("Root", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ROOT_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ROOT_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ROOT_FIELD.startswith("UFUNCTION"),
            ROOT_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ROOT_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ROOT_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_root_cpp_body(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        self.assertNotIn("{", ROOT_FIELD)
        self.assertNotIn("}", ROOT_FIELD)
        self.assertNotIn("return ", ROOT_FIELD)
        ...
        self.assertNotIn("return false", ROOT_FIELD)
        self.assertNotIn("return true", ROOT_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ROOT_FIELD)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROOT_FIELD)
        self.assertIn("Root", ROOT_FIELD)
        self.assertIn("USceneComponent", ROOT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ROOT_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_boss_drone_root(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        for token in LEFTOVER_BOSS_DRONE_ROOT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROOT_FIELD)
        self.assertIn(
            "Scripts/tests/test_boss_drone_root_field_decl_contract.py",
    ...
    def test_contract_does_not_relock_leftover_pathfinder_root(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        for token in LEFTOVER_PATHFINDER_ROOT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROOT_FIELD)
        self.assertIn(
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ROOT_FIELD)
        self.assertNotIn("CurrentIntegrity", ROOT_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ROOT_FIELD)
    ...
    def test_contract_does_not_open_leftover_briefing_component_header(
        ...
        self.assertNotIn(leftover_header, ROOT_FIELD)
    ...
    def test_contract_does_not_open_leftover_boss_drone_header(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        file_text = this_file_text()
        leftover_header = "SkyguardBossDroneBase.h"
        self.assertNotIn(leftover_header, locked_only)
        self.assertNotIn(leftover_header, ROOT_FIELD)
        self.assertNotIn(f"origin/main:{leftover_header}", file_text)
        self.assertNotIn(
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ROOT_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ROOT_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ROOT_FIELD),
            ROOT_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ROOT_FIELD)
        ...
        self.assertNotIn("AircraftRoot", locked_only)
        ...
        self.assertNotIn("SkyguardBossDroneBase.h", locked_only)
        for name in leftover_live_copy_method_names():
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 Root field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ROOT_FIELD.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ROOT_FIELD)

    def test_contract_is_root_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("USceneComponent", ROOT_FIELD)
        self.assertTrue(ROOT_FIELD.endswith(";"))
        self.assertNotIn("=", ROOT_FIELD)
        self.assertIn(UPROPERTY_MISSION01, section)

    def test_sibling_director_fields_do_not_satisfy_root(self) -> None:
        for leftover in (
            BRIEFING_FIELD,
            AUDIO_DIRECTOR_FIELD,
            RADIO_CHATTER_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ROOT_FIELD))
            self.assertNotEqual(ROOT_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_briefing_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_briefing_field_contract_stays_sibling_only(self) -> None:
        self.assertIn(
            "Scripts/tests/test_mission01_briefing_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
