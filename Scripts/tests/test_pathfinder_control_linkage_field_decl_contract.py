...
# INDEX_NONE or lock ControlLinkage construction in the
# .cpp. This is a FIELD contract on
# ASkyguardPathfinderBoss, not leftover
# Pathfinder encounter-controller methods and
# not leftover RadarNode. origin/main is a
# one-line field
# (`TObjectPtr<USkyguardBossWeakPointComponent>
# ControlLinkage;`); accept that form and other
...
# Stay off leftover sibling Pathfinder fields
# CommandAntenna, NoseCamera, Engine,
# DebrisNose, DebrisCenter, DebrisTail,
# DebrisSpine, EncounterController as the
# primary lock. Stay off leftover Pathfinder
...
# leftover isolated-test drafts #107–#503
# including leftover
...
# drafts #107–#503, plus PathfinderBoss
# production files. This lane only adds an isolated
# Python ControlLinkage field declaration contract on
# ASkyguardPathfinderBoss. Stay off leftover
# sibling Pathfinder fields, leftover Pathfinder
...
# NoseCamera, Engine, DebrisNose,
# DebrisCenter, DebrisTail, DebrisSpine,
...
# .cpp ControlLinkage body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardPathfinderBoss::ControlLinkage",
    "SkyguardPathfinderBoss.cpp",
    "SetExposed",
...
class PathfinderControlLinkageFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CONTROL_LINKAGE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CONTROL_LINKAGE_FIELD)
        self.assertIn("ControlLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CONTROL_LINKAGE_FIELD))

    def test_missing_control_linkage_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardPathfinderBoss();\n"
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "CommandAntenna;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CONTROL_LINKAGE_FIELD)
        self.assertIn("ControlLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CONTROL_LINKAGE_FIELD)
        self.assertIn("ControlLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, CONTROL_LINKAGE_FIELD), section)
        self.assertNotIn("UPROPERTY", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("VisibleAnywhere", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("BlueprintReadOnly", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("Category", CONTROL_LINKAGE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CONTROL_LINKAGE_FIELD)
        self.assertIn("ControlLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tUSkyguardBossWeakPointComponent* ControlLinkage;\n"
        as_array = (
            "\tTArray<TObjectPtr<USkyguardBossWeakPointComponent>> "
            "ControlLinkage;\n"
        )
        scene = "\tTObjectPtr<USceneComponent> ControlLinkage;\n"
        mesh = "\tTObjectPtr<UStaticMeshComponent> ControlLinkage;\n"
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CONTROL_LINKAGE_FIELD)
            self.assertIn("ControlLinkage", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_control_linkage_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CONTROL_LINKAGE_FIELD),
            CONTROL_LINKAGE_FIELD,
        )
        self.assertTrue(has_declaration(section, CONTROL_LINKAGE_FIELD))
        self.assertEqual(declaration_count(section, CONTROL_LINKAGE_FIELD), 1)
        self.assertTrue(
            CONTROL_LINKAGE_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            CONTROL_LINKAGE_FIELD,
        )
        self.assertTrue(CONTROL_LINKAGE_FIELD.endswith(";"), CONTROL_LINKAGE_FIELD)
        self.assertIn("ControlLinkage", CONTROL_LINKAGE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            CONTROL_LINKAGE_FIELD,
        )
        self.assertIn("TObjectPtr<", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("TArray<", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("=", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("INDEX_NONE", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("UFUNCTION", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("{", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("}", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("return ", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("CommandAntenna", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("NoseCamera", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("Engine", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisNose", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisCenter", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisTail", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisSpine", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("EncounterController", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("AdvanceEncounter", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ResetEncounterState", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("IsRouteStateSafe", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetRouteProgress", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("IssueClimbCommand", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetGunnerMount", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("FaceWorldLocation", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("PortGuidanceArray", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("CommandCore", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("PortEngine", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("StarboardEngine", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetSearchlightTracked", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetContactIdentified", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetLightningExposed", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("HullCollider", CONTROL_LINKAGE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, CONTROL_LINKAGE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_BOSS} {CONTROL_LINKAGE_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, CONTROL_LINKAGE_FIELD), 1)
        one_line = f"{{\npublic:\n\t{CONTROL_LINKAGE_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, CONTROL_LINKAGE_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CONTROL_LINKAGE_FIELD), section)
        self.assertEqual(
            require_declaration(section, CONTROL_LINKAGE_FIELD),
    ...
    def test_assigned_control_linkage_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "ControlLinkage = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, CONTROL_LINKAGE_FIELD)
        self.assertIn("ControlLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, CONTROL_LINKAGE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        debris_center = "\tTObjectPtr<UStaticMeshComponent> DebrisCenter;\n"
        ...
        for region in (
            ...
            self.assertIn("ControlLinkage", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, CONTROL_LINKAGE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{CONTROL_LINKAGE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, CONTROL_LINKAGE_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            CONTROL_LINKAGE_FIELD.startswith("UFUNCTION"),
            CONTROL_LINKAGE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CONTROL_LINKAGE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CONTROL_LINKAGE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_control_linkage_cpp_body(self) -> None:
        locked_only = f"{CONTROL_LINKAGE_FIELD}\n"
        self.assertNotIn("{", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("}", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("return ", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss::ControlLinkage", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SkyguardPathfinderBoss.cpp", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SkyguardPathfinderBoss.cpp", locked_only)
        self.assertNotIn("return false", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("return true", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetExposed", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("RefreshAuthoredWeakPointRegistry", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetBossPhase", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", CONTROL_LINKAGE_FIELD)

    def test_contract_does_not_relock_sibling_pathfinder_fields(self) -> None:
        locked_only = f"{CONTROL_LINKAGE_FIELD}\n"
        for token in PATHFINDER_SIBLING_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("CommandAntenna", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("NoseCamera", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("Engine", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisNose", CONTROL_LINKAGE_FIELD)
        ...
        self.assertNotIn("DebrisSpine", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("EncounterController", CONTROL_LINKAGE_FIELD)
        self.assertIn("USkyguardBossWeakPointComponent", CONTROL_LINKAGE_FIELD)
        self.assertIn("ControlLinkage", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_get_civilian_separation_meters_sibling(
        ...
        for token in GET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetFinaleStage", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("IssueClimbCommand", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("IsWreckDiverted", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_finale_stage(self) -> None:
        ...
        for token in GET_FINALE_STAGE_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetFinaleStage", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_is_climb_command_issued_sibling(
        ...
        for token in IS_CLIMB_COMMAND_ISSUED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), CONTROL_LINKAGE_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), CONTROL_LINKAGE_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), CONTROL_LINKAGE_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), CONTROL_LINKAGE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), CONTROL_LINKAGE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, CONTROL_LINKAGE_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_guidance_array(
        ...
        for token in LEFTOVER_STARBOARD_GUIDANCE_ARRAY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_strike_bay_mechanism(
        ...
        for token in LEFTOVER_PORT_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_strike_bay_mechanism(
        ...
        for token in LEFTOVER_STARBOARD_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_cooling_system(self) -> None:
        ...
        for token in LEFTOVER_PORT_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("PortCoolingSystem", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_cooling_system(
        ...
        for token in LEFTOVER_STARBOARD_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("StarboardCoolingSystem", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_jammer(self) -> None:
        ...
        for token in LEFTOVER_JAMMER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("Jammer", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_last_flight_debris_fields(
        ...
        for token in LEFTOVER_LAST_FLIGHT_DEBRIS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisArmorPort", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisArmorStarboard", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisStrikeBayPort", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisStrikeBayStarboard", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisEnginePort", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("DebrisEngineStarboard", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetSearchlightTracked", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetContactIdentified", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("OpenOrbitExposure", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SetLightningExposed", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", CONTROL_LINKAGE_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            CONTROL_LINKAGE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardGunner", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SkyguardRadarNode", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(self) -> None:
        ...
        self.assertNotIn("USkyguardDebriefWidget", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("MaxIntegrity", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("CurrentIntegrity", CONTROL_LINKAGE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, CONTROL_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", CONTROL_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CONTROL_LINKAGE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, CONTROL_LINKAGE_FIELD),
            CONTROL_LINKAGE_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("ASkyguardPathfinderBoss::ControlLinkage", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder ControlLinkage field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{CONTROL_LINKAGE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, CONTROL_LINKAGE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                CONTROL_LINKAGE_FIELD.lower(),
                "pathfinder ControlLinkage contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CONTROL_LINKAGE_FIELD)

    def test_contract_is_control_linkage_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("USkyguardBossWeakPointComponent", CONTROL_LINKAGE_FIELD)
        self.assertTrue(CONTROL_LINKAGE_FIELD.endswith(";"))
        self.assertNotIn("=", CONTROL_LINKAGE_FIELD)
        self.assertIn(UPROPERTY_BOSS, section)
