# THIS IS leftover-safe FSkyguardMissionResult::MedalTier.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly, meta = (ClampMin = "0", ClampMax = "3"))
# then `int32 MedalTier = 0;` with no Category=.
# Origin/main HAS spaces around `=` in the meta. Initializer IS present: `= 0`.
# If a clone asserts no initializer, drop that and accept origin/main.
# If a clone asserts Category= and origin/main has none, drop that assertion.
# If a clone asserts `= false` / `= 0.f` / `= true`, do not copy those;
# this field is `int32 ... = 0`. Keep ClampMin/ClampMax; fail-closed if those
# meta values are dropped.
# Fail-closed if the type/decl is missing or renamed, if Category= is added,
# if ClampMin/ClampMax meta is dropped, or if the default is not `= 0`.
# THIS IS NOT leftover mission-result-defaults #c4db / test_mission_result_defaults*.
...
# THIS IS NOT leftover HowToFlyRow #1258-#1260 and defaults #191 / #9ab2.
...
# Do NOT contract EditAnywhere MissionResult gameplay fields
# (MissionId, bMissionSucceeded, ShotsFired, Hits,
# AircraftDamageFraction, CompletionTimeSeconds, CompletedObjectiveIds).
# Do NOT contract FinalScore (sibling lane).
# Parse STRUCT public section ONLY after struct FSkyguardMissionResult
# and BEFORE enum class ESkyguardMissionDebriefState /
...
# USkyguardSortiePresentationComponent methods, briefing/how-to-fly rows,
# or leftover AddObjectiveProgress.
...
TARGET = "int32 MedalTier = 0;"
TARGET_WRONG_FALSE = "int32 MedalTier = false;"
TARGET_WRONG_ONE = "int32 MedalTier = 1;"
TARGET_WRONG_FLOAT = "int32 MedalTier = 0.f;"
TARGET_WRONG_TRUE = "int32 MedalTier = true;"
TARGET_BARE = "int32 MedalTier;"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored default. Do not accept = false / = 1 /
    # = 0.f / = true / a bare decl for FSkyguardMissionResult::MedalTier
    # (must be = 0).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    # Nested meta=(ClampMin=..., ClampMax=...) needs a balanced scan.
    # A [^)]* matcher would stop at the inner meta close-paren.
    cursor = 0
    while True:
        ...
        while index < len(compact) and depth:
            if compact[index] == "(":
                ...
                depth -= 1
            index += 1
        if depth == 0 and re.match(r"\s*int32\s+MedalTier\b", compact[index:]):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 MedalTier is missing from "
        f"origin/main:{HEADER_PATH} struct {CLASS_NAME} public section"
    )


def require_clamp_meta(specifiers: str) -> str:
    compact = collapsed(specifiers)
    if "ClampMin" not in compact or "ClampMax" not in compact:
        raise AssertionError(
            "ClampMin/ClampMax meta is missing from "
            f"origin/main:{HEADER_PATH} struct {CLASS_NAME} MedalTier"
        )
    if '"0"' not in compact or '"3"' not in compact:
        raise AssertionError(
            "ClampMin/ClampMax values are not 0/3 on "
            f"origin/main:{HEADER_PATH} struct {CLASS_NAME} MedalTier"
        )
    return specifiers
...
class MissionResultMedalTierFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertIn("MedalTier", section)
        self.assertIn(FINAL_SCORE, section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("MedalTier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_clamp_meta_dropped_fails_closed(self) -> None:
        dropped = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "\tUPROPERTY(VisibleAnywhere, BlueprintReadOnly)\n"
        ...
        origin = require_clamp_meta(
            attached_uproperty_specifiers(public_section(origin_main_header()))
        )
    ...
    def test_medal_tier_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 MedalTier"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn(FINAL_SCORE, LOCKED_DECL)
        for field in GAMEPLAY_FIELDS:
        ...
        self.assertFalse(has_declaration(f"\t{TARGET_BARE}\n", LOCKED_DECL))
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            "\tint32 MedalTier = 1;\n",
            "\tint32 MedalTier = false;\n",
            "\tint32 MedalTier = true;\n",
            "\tint32 MedalTier = 0.f;\n",
            "\tbool MedalTier = false;\n",
            "\tbool MedalTier = 0;\n",
            "\tint32 Medal = 0;\n",
            "\tint32 FinalScore = 0;\n",
            "\tint32 MedalTier;\n",
    ...
    def test_does_not_contract_gameplay_or_final_score(self) -> None:
        self.assertNotIn(FINAL_SCORE, LOCKED_DECL)
        for field in GAMEPLAY_FIELDS:
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("ClampMin", UPROPERTY_VISIBLE)
        self.assertIn("ClampMax", UPROPERTY_VISIBLE)
        self.assertIn('ClampMin = "0"', UPROPERTY_VISIBLE)
        self.assertIn('ClampMax = "3"', UPROPERTY_VISIBLE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result MedalTier field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_medal_tier_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertNotIn(FINAL_SCORE, locked_only)
        for field in GAMEPLAY_FIELDS:
