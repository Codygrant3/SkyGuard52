...
# leftover harness MaximumAudioThreadMs = 2.f).
...
# `= 0.f` is not Harbor. Sibling MaximumTruePeakDbTP
# = -120.f is not Harbor. Harbor is 40 + `.f` / 80 + `.f`
# only.
# Analog leftover audible-acceptance-receipt-defaults #193 /
# #1cd7 then leftover-safe isolated BuildSha256 #1435 /
# SampleCount #1436 / EvidenceSha256 #1437 / PeakActiveVoices
# then this isolated MaximumAudioThreadMs field decl.
...
# THIS IS NOT leftover AudioTelemetry fields #1281-#1287.
# THIS IS NOT leftover USkyguardAudioProductionBank class
# (do not parse).
# THIS IS NOT leftover USkyguardAudioAcceptanceHarness class
# (do not parse; has MaximumAllowedVoices = 48 and a
# leftover MaximumAudioThreadMs = 2.f).
# THIS IS NOT leftover enum class
# ESkyguardProductionAudioCategory (Ri+fleMuzzle / Ig+laSearch).
...
# LOCKED_SCRIPTS leftover comments also cover MissionResult /
# RouteDefinition / ProductionAudio / MissionDebrief /
# ObjectiveProgress / RoutePoint / RadioLine / TheaterKit /
# harness class / bank class / AudioTelemetry /
# SampleCount #1436 / PeakActiveVoices / EvidenceSha256
# #1437 / BuildSha256 #1435.
# Sibling decls stay unclaimed: BuildSha256 / EvidenceSha256 /
# SampleCount / PeakActiveVoices / TotalUnderruns /
# MaximumTruePeakDbTP / bPackagedDevelopmentBuild /
# bAudibleDeviceObserved / bCalibratedMetering /
# bProductionBankReady / bAccepted.
# Identifier bounds: match EXACT `MaximumAudioThreadMs` as a
# whole field name after `float`. Do NOT accept sibling
# MaximumTruePeakDbTP.
...
# FSkyguardAudibleAcceptanceReceipt only, type is float,
# required initializer `= 0.f`, UPROPERTY is
...
TARGET = "float MaximumAudioThreadMs = 0.f;"
TARGET_WRONG_BARE = "float MaximumAudioThreadMs;"
TARGET_WRONG_EQ_ONE = "float MaximumAudioThreadMs = 1.f;"
TARGET_WRONG_EQ_NONE = "float MaximumAudioThreadMs = NAME_None;"
TARGET_WRONG_TRUE = "float MaximumAudioThreadMs = true;"
TARGET_WRONG_FALSE = "float MaximumAudioThreadMs = false;"
TARGET_WRONG_EQ_ZERO_NO_F = "float MaximumAudioThreadMs = 0;"
TARGET_WRONG_EQ_HARNESS = "float MaximumAudioThreadMs = 2.f;"
TARGET_WRONG_FSTRING = "FString MaximumAudioThreadMs;"
TARGET_WRONG_INT32 = "int32 MaximumAudioThreadMs = 0;"
TARGET_WRONG_FNAME = "FName MaximumAudioThreadMs;"
TARGET_WRONG_FTEXT = "FText MaximumAudioThreadMs;"
TARGET_WRONG_BOOL = "bool MaximumAudioThreadMs = false;"
TARGET_WRONG_TARRAY = "TArray<float> MaximumAudioThreadMs;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MaximumAudioThreadMs;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SAMPLE = "int32 SampleCount = 0;"
TARGET_WRONG_UNDERRUNS = "int32 TotalUnderruns = 0;"
TARGET_WRONG_PEAK = "int32 PeakActiveVoices = 0;"
TARGET_WRONG_TRUE_PEAK = "float MaximumTruePeakDbTP = -120.f;"
...
FIELD_NAME = "MaximumAudioThreadMs"
...
HARNESS_WRONG_THREAD_MS = "MaximumAudioThreadMs = 2.f"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float MaximumAudioThreadMs = 0.f;`.
    # Match the EXACT identifier MaximumAudioThreadMs as a whole
    # field name after float. Do NOT accept MaximumTruePeakDbTP
    # or PeakActiveVoices. Reject dropped/changed initializer,
    # `= 0` without `.f`, leftover harness `= 2.f`, Harbor 40/80,
    # wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"float\s+MaximumAudioThreadMs\s*=\s*0\.f\s*;",
        compact,
    )
    if re.search(r"\bMaximumTruePeakDbTP\b", compact) and exact is None:
        return False
    if re.search(r"\bPeakActiveVoices\b", compact) and exact is None:
        return False
    if re.search(r"\bSampleCount\b", compact) and exact is None:
        return False
    if re.search(r"\bTotalUnderruns\b", compact) and exact is None:
        return False
    if re.search(
        r"float\s+MaximumAudioThreadMs\s*;",
        compact,
    ) and re.search(
        r"float\s+MaximumAudioThreadMs\s*=",
        compact,
    ...
    for assigned in re.findall(
        r"float\s+MaximumAudioThreadMs\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "0.f":
            return False
    if re.search(
        r"int32\s+MaximumAudioThreadMs\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MaximumAudioThreadMs\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+MaximumAudioThreadMs\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MaximumAudioThreadMs\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MaximumAudioThreadMs\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+MaximumAudioThreadMs\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MaximumAudioThreadMs\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MaximumAudioThreadMs\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaximumAudioThreadMs is missing from "
...
class AudibleAcceptanceReceiptMaximumAudioThreadMsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_maximum_audio_thread_ms_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            ...
            f"\tint32 {SIBLING_PEAK_ACTIVE_VOICES} = 0;\n"
    ...
    def test_true_peak_does_not_satisfy_maximum_audio_thread_ms(self) -> None:
        ...
        self.assertTrue(
            has_identifier(sample_only, SIBLING_SAMPLE_COUNT)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertTrue(
            has_identifier(underruns_only, SIBLING_TOTAL_UNDERRUNS)
        )
        peak_only = f"\t{TARGET_WRONG_PEAK}\n"
        self.assertFalse(has_declaration(peak_only, LOCKED_DECL))
        self.assertFalse(has_identifier(peak_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(peak_only, SIBLING_PEAK_ACTIVE_VOICES)
        )
        true_peak_only = f"\t{TARGET_WRONG_TRUE_PEAK}\n"
        self.assertFalse(has_declaration(true_peak_only, LOCKED_DECL))
        self.assertFalse(has_identifier(true_peak_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(true_peak_only, SIBLING_MAXIMUM_TRUE_PEAK_DB_TP)
        )
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float MaximumAudioThreadMs = 0.f;", compact_origin)
        self.assertNotIn("MaximumAudioThreadMs = 1", compact_origin)
        self.assertNotIn("MaximumAudioThreadMs = true", compact_origin)
        self.assertNotIn("FString MaximumAudioThreadMs", compact_origin)
        self.assertNotIn("int32 MaximumAudioThreadMs", compact_origin)
        self.assertNotIn("MaximumAudioThreadMs = 0;", compact_origin)
        self.assertNotIn(HARNESS_WRONG_THREAD_MS, compact_origin)

    def test_maximum_audio_thread_ms_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MaximumAudioThreadMs"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("= 0.f;"), LOCKED_DECL)
        self.assertIn("= 0.f", LOCKED_DECL)
        self.assertNotIn("= 0;", LOCKED_DECL)
        self.assertNotIn("= 2.f", LOCKED_DECL)
        ...
        wrongs = (
            ...
            TARGET_WRONG_TRUE,
            TARGET_WRONG_FALSE,
            TARGET_WRONG_EQ_ZERO_NO_F,
            TARGET_WRONG_EQ_HARNESS,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_EQ_ZERO_NO_F}\n",
            f"\t{TARGET_WRONG_EQ_HARNESS}\n",
            ...
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            f"\t{TARGET_WRONG_TRUE_PEAK}\n",
            "\tfloat MaximumAudioThreadMss = 0.f;\n",
            "\tfloat MaximumAudioThreadMs = 1.f;\n",
            "\tbool MaximumAudioThreadMs;\n",
            "\tfloat MaximumAudioThreadMs = " + forty + ";\n",
            "\tfloat MaximumAudioThreadMs = " + eighty + ";\n",
        )
    ...
    def test_does_not_parse_harness_or_leftover_neighbors(self) -> None:
        ...
        self.assertIn(HARNESS_WRONG_THREAD_MS, header)
        self.assertNotIn(HARNESS_WRONG_THREAD_MS, section)
        self.assertNotIn(HARNESS_WRONG_THREAD_MS, leaked)
        self.assertNotIn(HARNESS_WRONG_THREAD_MS, window)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt MaximumAudioThreadMs field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "maximum_audio_thread_ms_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_maximum", THIS_SCRIPT)
        ...
        self.assertIn(CLONE_EVIDENCE_SHA256, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_EVIDENCE_SHA256, THIS_SCRIPT)
        self.assertIn(CLONE_SAMPLE_COUNT, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_SAMPLE_COUNT, THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_PEAK_ACTIVE_VOICES, locked_only)
        self.assertNotIn(SIBLING_TOTAL_UNDERRUNS, locked_only)
        self.assertNotIn(SIBLING_MAXIMUM_TRUE_PEAK_DB_TP, locked_only)
        for helper in namespace_helper_tokens():
