...
# THIS IS NOT leftover LandscapeHeightSample SupportedFraction.
...
# BlueprintReadOnly float SupportedFraction = 0.f; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# SupportedFraction FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float SupportedFraction = 0.f; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# SupportedFraction FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# float SupportedFraction = 0.f; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeHeightSample SupportedFraction
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float SupportedFraction = 0.f;
...
# FSkyguardLandscapeHeightSample STRUCT
...
# SupportedFraction FIELD (declaration-only). It is
...
TARGET = "float SupportedFraction = 0.f;"
TARGET_WRONG = "int32 SupportedFraction = 0;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*float\s+SupportedFraction\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for float SupportedFraction is missing from "
...
class LandscapeFootprintSampleSupportedFractionFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("SupportedFraction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_supported_fraction_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_supported_fraction_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float SupportedFraction"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("SupportedFraction", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("SupportedFraction", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float SupportedFraction"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("SupportedFraction", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_supported_fraction(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
