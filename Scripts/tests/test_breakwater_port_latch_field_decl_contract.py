...
# PortLatch;`); accept that form and other
...
# PortLatch field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBreakwaterBoss::PortLatch",
    "SkyguardBreakwaterBoss.cpp",
    "SetExposed",
...
class BreakwaterPortLatchFieldDeclContractTests(unittest.TestCase):
    def test_breakwater_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, PORT_LATCH_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, PORT_LATCH_FIELD)
        self.assertIn("PortLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, PORT_LATCH_FIELD)
    ...
    def test_missing_port_latch_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardBreakwaterBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, PORT_LATCH_FIELD)
        self.assertIn("PortLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, PORT_LATCH_FIELD)
        self.assertIn("PortLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", PORT_LATCH_FIELD)
        self.assertNotIn("VisibleAnywhere", PORT_LATCH_FIELD)
        self.assertNotIn("BlueprintReadOnly", PORT_LATCH_FIELD)
        self.assertNotIn("Category", PORT_LATCH_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, PORT_LATCH_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, PORT_LATCH_FIELD)
        self.assertIn("PortLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* PortLatch;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> PortLatch;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> PortLatch;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, PORT_LATCH_FIELD)
            self.assertIn("PortLatch", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_port_latch_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, PORT_LATCH_FIELD),
            PORT_LATCH_FIELD,
        )
        self.assertTrue(has_declaration(section, PORT_LATCH_FIELD))
        self.assertEqual(declaration_count(section, PORT_LATCH_FIELD), 1)
        self.assertTrue(
            PORT_LATCH_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            PORT_LATCH_FIELD,
        )
        self.assertTrue(PORT_LATCH_FIELD.endswith(";"), PORT_LATCH_FIELD)
        self.assertIn("PortLatch", PORT_LATCH_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_LATCH_FIELD,
        )
        self.assertIn("TObjectPtr<", PORT_LATCH_FIELD)
        self.assertNotIn("TArray<", PORT_LATCH_FIELD)
        self.assertNotIn("=", PORT_LATCH_FIELD)
        self.assertNotIn("INDEX_NONE", PORT_LATCH_FIELD)
        self.assertNotIn("UFUNCTION", PORT_LATCH_FIELD)
        self.assertNotIn("{", PORT_LATCH_FIELD)
        self.assertNotIn("}", PORT_LATCH_FIELD)
        self.assertNotIn("return ", PORT_LATCH_FIELD)
        self.assertNotIn("CommandAntenna", PORT_LATCH_FIELD)
        self.assertNotIn("ControlLinkage", PORT_LATCH_FIELD)
        self.assertNotIn("NoseCamera", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisCenter", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisTail", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisNose", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisSpine", PORT_LATCH_FIELD)
        self.assertNotIn("AdvanceEncounter", PORT_LATCH_FIELD)
        self.assertNotIn("ResetEncounterState", PORT_LATCH_FIELD)
        self.assertNotIn("IsRouteStateSafe", PORT_LATCH_FIELD)
        self.assertNotIn("GetRouteProgress", PORT_LATCH_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", PORT_LATCH_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", PORT_LATCH_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", PORT_LATCH_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PORT_LATCH_FIELD)
        self.assertNotIn("RadarNode", PORT_LATCH_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", PORT_LATCH_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", PORT_LATCH_FIELD)
        self.assertNotIn("IssueClimbCommand", PORT_LATCH_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", PORT_LATCH_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", PORT_LATCH_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", PORT_LATCH_FIELD)
        self.assertNotIn("GetFinaleStage", PORT_LATCH_FIELD)
        self.assertNotIn("IsClimbCommandIssued", PORT_LATCH_FIELD)
        self.assertNotIn("IsWreckDiverted", PORT_LATCH_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", PORT_LATCH_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", PORT_LATCH_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PORT_LATCH_FIELD)
        self.assertNotIn("GetGunnerMount", PORT_LATCH_FIELD)
        self.assertNotIn("FaceWorldLocation", PORT_LATCH_FIELD)
        self.assertNotIn("PortGuidanceArray", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisArmorPort", PORT_LATCH_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_LATCH_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_LATCH_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", PORT_LATCH_FIELD)
        self.assertNotIn("SetLightningExposed", PORT_LATCH_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", PORT_LATCH_FIELD)
        self.assertNotIn("HullCollider", PORT_LATCH_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, PORT_LATCH_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {PORT_LATCH_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, PORT_LATCH_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{PORT_LATCH_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, PORT_LATCH_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, PORT_LATCH_FIELD),
            section,
    ...
    def test_assigned_port_latch_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "PortLatch = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, PORT_LATCH_FIELD)
        self.assertIn("PortLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, PORT_LATCH_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("PortLatch", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, PORT_LATCH_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{PORT_LATCH_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, PORT_LATCH_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, PORT_LATCH_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            PORT_LATCH_FIELD.startswith("UFUNCTION"),
            PORT_LATCH_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, PORT_LATCH_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", PORT_LATCH_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_port_latch_cpp_body(self) -> None:
        locked_only = f"{PORT_LATCH_FIELD}\n"
        self.assertNotIn("{", PORT_LATCH_FIELD)
        self.assertNotIn("}", PORT_LATCH_FIELD)
        self.assertNotIn("return ", PORT_LATCH_FIELD)
        ...
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", PORT_LATCH_FIELD)
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", locked_only)
        self.assertNotIn("return false", PORT_LATCH_FIELD)
        self.assertNotIn("return true", PORT_LATCH_FIELD)
        self.assertNotIn("CreateDefaultSubobject", PORT_LATCH_FIELD)
        self.assertNotIn("SetExposed", PORT_LATCH_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            PORT_LATCH_FIELD,
        )
        self.assertNotIn("SetBossPhase", PORT_LATCH_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", PORT_LATCH_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{PORT_LATCH_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("PortDischargeBoom", PORT_LATCH_FIELD)
        self.assertNotIn("StarboardDischargeBoom", PORT_LATCH_FIELD)
        self.assertNotIn("EngineIntake", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisPortPanel", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisStarboardPanel", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisIntakePanel", PORT_LATCH_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", PORT_LATCH_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_LATCH_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("EncounterController", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("ControlServo", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("DispenserCenter", PORT_LATCH_FIELD)
        self.assertNotIn("MaxReleasesPerBay", PORT_LATCH_FIELD)
        self.assertIn("PortLatch", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("DispenserStarboard", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("GetManeuver", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), PORT_LATCH_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), PORT_LATCH_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), PORT_LATCH_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), PORT_LATCH_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), PORT_LATCH_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, PORT_LATCH_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", PORT_LATCH_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_LATCH_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_LATCH_FIELD)
        self.assertNotIn("OpenOrbitExposure", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_sibling_radar_ghost_fields(self) -> None:
        ...
        for token in SIBLING_RADAR_GHOST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("RadarReceiver", PORT_LATCH_FIELD)
        self.assertNotIn("CoolingDoor", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisPortEWPanel", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisStarboardEWPanel", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisCoolingDoor", PORT_LATCH_FIELD)
        self.assertIn("PortLatch", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("StarboardLatch", PORT_LATCH_FIELD)
        self.assertNotIn("DecoyPods", PORT_LATCH_FIELD)
        self.assertNotIn("ElevatorLinkage", PORT_LATCH_FIELD)
        self.assertNotIn("DebrisEngine", PORT_LATCH_FIELD)
        self.assertIn("PortLatch", PORT_LATCH_FIELD)
    ...
    def test_breakwater_engine_is_not_radar_ghost_engine(self) -> None:
        ...
        for token in LEFTOVER_BREAKWATER_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("Engine", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_harbor_wave_state(self) -> None:
        ...
        for token in LEFTOVER_HARBOR_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", PORT_LATCH_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("SetLightningExposed", PORT_LATCH_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", PORT_LATCH_FIELD)
        self.assertNotIn("IsLightningExposed", PORT_LATCH_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", PORT_LATCH_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), PORT_LATCH_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), PORT_LATCH_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, PORT_LATCH_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("ASkyguardGunner", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("SkyguardRadarNode", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_LATCH_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, PORT_LATCH_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PORT_LATCH_FIELD)
        self.assertNotIn("MaxIntegrity", PORT_LATCH_FIELD)
        self.assertNotIn("CurrentIntegrity", PORT_LATCH_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", PORT_LATCH_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            PORT_LATCH_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, PORT_LATCH_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", PORT_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{PORT_LATCH_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, PORT_LATCH_FIELD),
            PORT_LATCH_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardBreakwaterBoss::PortLatch", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "breakwater PortLatch field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{PORT_LATCH_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, PORT_LATCH_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                PORT_LATCH_FIELD.lower(),
                "breakwater PortLatch contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, PORT_LATCH_FIELD)

    def test_contract_is_port_latch_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(PORT_LATCH_FIELD.endswith(";"))
        self.assertNotIn("=", PORT_LATCH_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
