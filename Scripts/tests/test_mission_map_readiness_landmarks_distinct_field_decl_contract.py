...
# bLandmarksDistinct FIELD (declaration-only;
...
# the Native integrity boundary comment).
...
# UPROPERTY(Transient).
# THIS IS leftover-safe MissionMapReadiness
# bLandmarksDistinct. THIS IS NOT leftover mission-map-
# readiness-defaults #3a2a. THIS IS NOT leftover
# mission-map GetReadiness / ValidateAssembly /
# RebuildRouteSpline / IsPointInsideFlightClearance.
# THIS IS NOT leftover MissionMapReadiness
# bDefinitionValid #1155 / leftover
# bRequiredObjectivesAnchored #1157 / in-flight
# bRouteMatchesDefinition. LOCK those sibling
# scripts. Do not parse ObjectiveAnchor /
# LandmarkAnchor (EditAnywhere).
# THIS IS NOT leftover Mission10 Group #1156.
...
TARGET = "bool bLandmarksDistinct = false;"
TARGET_WRONG = "bool bLandmarksDistinct = true;"
LOCKED_DECL = TARGET
...
class MissionMapReadinessLandmarksDistinctFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bLandmarksDistinct", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_landmarks_distinct_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_landmarks_distinct_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bLandmarksDistinct"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bLandmarksDistinct", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bLandmarksDistinct", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bLandmarksDistinct"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bLandmarksDistinct", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map-readiness landmarks-distinct field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission-map-readiness landmarks-distinct contains "
    ...
    def test_sibling_director_fields_do_not_satisfy_landmarks_distinct(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission_map_readiness_landmarks"
                "_distinct_field_decl_contract.py"
            )
        )
        self.assertIn(
