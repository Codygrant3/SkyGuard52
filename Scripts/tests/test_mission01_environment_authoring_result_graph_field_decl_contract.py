...
# TObjectPtr<UPCGGraph> Graph; bare decl, NO initializer; one-line /
...
# TObjectPtr<UPCGGraph> Graph; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly TObjectPtr<UPCGGraph> Graph; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly TObjectPtr<UPCGGraph> Graph;
...
TARGET = "TObjectPtr<UPCGGraph> Graph;"
TARGET_WRONG = "TObjectPtr<UPCGGraph> Graph = nullptr;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare TObjectPtr. Do not accept an
    # initializer for FSkyguardMission01EnvironmentAuthoringResult::
    # Graph (must be bare TObjectPtr<UPCGGraph> Graph;).
    # THIS IS NOT leftover VisibleAudit bSuccess #1199.
    # THIS IS NOT leftover FootprintSampleResult bSuccess #1189.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*TObjectPtr<UPCGGraph>\s+Graph\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for TObjectPtr<UPCGGraph> Graph is missing from "
...
class Mission01EnvironmentAuthoringResultGraphFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Graph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_graph_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_graph_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UPCGGraph> Graph"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("Graph;"), LOCKED_DECL)
        ...
        self.assertNotIn("ESkyguardMission05ProtectedTarget", LOCKED_DECL)
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UPCGGraph> "),
            LOCKED_DECL,
        )
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("Graph", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("TObjectPtr<UPCGGraph> Graph"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("Graph", locked_only)
    ...
    def test_contract_is_target_field_declaration_only(
        ...
        self.assertTrue(LOCKED_DECL.startswith("TObjectPtr<UPCGGraph> Graph"))
        self.assertIn("Graph", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.endswith("Graph;"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("GetProtectedGroup", LOCKED_DECL)
        self.assertNotIn("FSkyguardMission10ProtectedRuntime", LOCKED_DECL)
        self.assertIn(UPROPERTY_MISSION10, section)

    def test_sibling_director_fields_do_not_satisfy_graph(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
