...
# INDEX_NONE or lock GetPoolRuntime
# construction in the .cpp. This is leftover-safe
# Mission09 Performance GetPoolRuntime METHOD on
# ASkyguardMission09IntegrationDirector. It is
# the METHOD, not leftover Mission09 pool-runtime-
# defaults #5426 / #243, not leftover Mission09
# pool-budget-defaults #4537 / #242. It is NOT
# leftover Mission09 GetReadiness, NOT leftover
# Mission08 GetReadiness #739 /
# branch cursor/mission08-get-readiness-decl-
...
# leftover Mission09 GetReadiness,
...
# leftover Mission09 GetReadiness,
# leftover Mission09 pool-runtime-defaults
# #5426 / #243 (THIS IS THE METHOD, NOT THE
# DEFAULTS STRUCT), leftover Mission09
# pool-budget-defaults #4537 / #242,
...
# GetPoolRuntime() const { return PoolRuntime; }`);
...
# Waves / Search / Protection / Payload / Targets /
# Objectives / Boss / Destruction / Briefing / Hoist /
# Rescue / Safety / Integration, not Environment,
...
# Protection / Integration / Objectives.
...
# an isolated Python GetPoolRuntime
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission07IntegrationDirector::GetPoolRuntime",
    "SkyguardMission07IntegrationDirector.cpp",
    "ASkyguardMission08IntegrationDirector::GetPoolRuntime",
    "SkyguardMission08IntegrationDirector.cpp",
    "ASkyguardMission09IntegrationDirector::GetPoolRuntime",
    "SkyguardMission09IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission09GetPoolRuntimeDeclContractTests(unittest.TestCase):
    def test_mission09_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_POOL_RUNTIME),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_POOL_RUNTIME)
        self.assertIn("GetPoolRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_POOL_RUNTIME)
    ...
    def test_missing_get_pool_runtime_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_POOL_RUNTIME)
        self.assertIn("GetPoolRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_PERFORMANCE}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_POOL_RUNTIME)
        self.assertIn("GetPoolRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_PERFORMANCE, section)
        ...
        self.assertNotIn("UFUNCTION", GET_POOL_RUNTIME)
        self.assertNotIn("BlueprintPure", GET_POOL_RUNTIME)
        self.assertNotIn("Category", GET_POOL_RUNTIME)
        self.assertNotIn("BlueprintCallable", UFUNCTION_PERFORMANCE)
        self.assertIn("Skyguard|Mission09|Performance", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission01", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission03", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission04", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission05", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission06", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Waves", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Search", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Protection", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Payload", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Targets", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Objectives", UFUNCTION_PERFORMANCE)
        self.assertIn("BlueprintPure", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Environment", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Briefing", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission02", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Boss", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Destruction", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Apache", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission06", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission07", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission08", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission10", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Encounter", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Safety", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Integration", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Hoist", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Rescue", UFUNCTION_PERFORMANCE)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_PERFORMANCE)
            self.assertNotIn(invented, GET_POOL_RUNTIME)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_POOL_RUNTIME)
        self.assertIn("GetPoolRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetPoolRuntime() const;\n"
        )
        as_const = (
            "\tconst FSkyguardMission09PoolRuntime& "
            "GetPoolRuntime();\n"
        ...
        with_arg = (
            "\tconst FSkyguardMission09PoolRuntime& "
            "GetPoolRuntime(int32 Amount) const;\n"
        )
        renamed = (
            "\tconst FSkyguardMission09PoolRuntime& "
            "GetReadyState() const;\n"
        )
        short_name = "\tbool GetPoolRuntime() const;\n"
        leftover_m06_type = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_POOL_RUNTIME)
            self.assertIn("GetPoolRuntime", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_pool_runtime_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_POOL_RUNTIME),
            GET_POOL_RUNTIME,
        )
        self.assertTrue(has_declaration(section, GET_POOL_RUNTIME))
        self.assertEqual(
            declaration_count(section, GET_POOL_RUNTIME),
            1,
        )
        self.assertTrue(
            GET_POOL_RUNTIME.startswith("const "),
            GET_POOL_RUNTIME,
        )
        self.assertTrue(
            GET_POOL_RUNTIME.startswith("const "),
        ...
        self.assertIn("FSkyguardMission09PoolRuntime", GET_POOL_RUNTIME)
        self.assertIn("GetPoolRuntime", GET_POOL_RUNTIME)
        self.assertNotIn(";", GET_POOL_RUNTIME)
        self.assertIn("return PoolRuntime;", section)
        self.assertIn(UFUNCTION_PERFORMANCE, section)
        self.assertIn(" const", GET_POOL_RUNTIME)
        self.assertIn("()", GET_POOL_RUNTIME)
        self.assertNotIn("TArray<", GET_POOL_RUNTIME)
        self.assertNotIn("TObjectPtr", GET_POOL_RUNTIME)
        self.assertNotIn("TSoftObjectPtr", GET_POOL_RUNTIME)
        self.assertNotIn("INDEX_NONE", GET_POOL_RUNTIME)
        self.assertNotIn("UFUNCTION", GET_POOL_RUNTIME)
        self.assertNotIn("{", GET_POOL_RUNTIME)
        self.assertNotIn("}", GET_POOL_RUNTIME)
        self.assertNotIn("return ", GET_POOL_RUNTIME)
        self.assertIn(" const", GET_POOL_RUNTIME)
        self.assertNotIn("Root", GET_POOL_RUNTIME)
        self.assertNotIn("Briefing", GET_POOL_RUNTIME)
        self.assertNotIn("AudioDirector", GET_POOL_RUNTIME)
        self.assertNotIn("RadioChatter", GET_POOL_RUNTIME)
        self.assertNotIn("SortiePresentation", GET_POOL_RUNTIME)
        self.assertNotIn("CampaignDefinition", GET_POOL_RUNTIME)
        self.assertNotIn("MissionDefinition", GET_POOL_RUNTIME)
        self.assertNotIn("Readiness;", GET_POOL_RUNTIME)
        self.assertNotIn("bAutoInitialize", GET_POOL_RUNTIME)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_POOL_RUNTIME)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_POOL_RUNTIME)
        self.assertNotIn("PathfinderSpawnLocation", GET_POOL_RUNTIME)
        self.assertNotIn("GetAircraft", GET_POOL_RUNTIME)
        self.assertNotIn("BindRuntimeActors", GET_POOL_RUNTIME)
        self.assertNotIn("HandleDroneCityImpact", GET_POOL_RUNTIME)
        self.assertNotIn("InitializePlayableMission", GET_POOL_RUNTIME)
        self.assertNotIn("StartNextWave", GET_POOL_RUNTIME)
        self.assertNotIn("AdvanceConvoyByDistance", GET_POOL_RUNTIME)
        self.assertNotIn("NotifyConvoyDamage", GET_POOL_RUNTIME)
        self.assertNotIn("GetWaveState", GET_POOL_RUNTIME)
        self.assertNotIn("GetConvoyRouteState", GET_POOL_RUNTIME)
        self.assertNotIn("GetDayBeatKit", GET_POOL_RUNTIME)
        self.assertNotIn("GetNightBeatKit", GET_POOL_RUNTIME)
        self.assertNotIn("StartSearchlightWindow", GET_POOL_RUNTIME)
        self.assertNotIn("AdvanceSearchlightTrack", GET_POOL_RUNTIME)
        self.assertNotIn("NotifySubstationDamage", GET_POOL_RUNTIME)
        self.assertNotIn("GetRemainingThreatsInWave", GET_POOL_RUNTIME)
        self.assertNotIn("GetSearchlightRuntime", GET_POOL_RUNTIME)
        self.assertNotIn("GetSubstationIntegrity", GET_POOL_RUNTIME)
        self.assertNotIn("StartPayloadWindow", GET_POOL_RUNTIME)
        self.assertNotIn("AdvancePayloadWindow", GET_POOL_RUNTIME)
        self.assertNotIn("TryJamActivePayload", GET_POOL_RUNTIME)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_POOL_RUNTIME)
        self.assertNotIn("GetPayloadWindow", GET_POOL_RUNTIME)
        self.assertNotIn("GetTargetRuntime", GET_POOL_RUNTIME)
        self.assertNotIn("GetSurvivingTargetCount", GET_POOL_RUNTIME)
        self.assertNotIn("GetDayBeatKind", GET_POOL_RUNTIME)
        self.assertNotIn("GetDayBeatIndex", GET_POOL_RUNTIME)
        self.assertNotIn("TickDayBeatKit", GET_POOL_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardMission07IntegrationDirector", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission06WaveState", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission07WaveState", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission08WaveState", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission09WaveState", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission08ProtectedTarget", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission09ProtectedTarget", GET_POOL_RUNTIME)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", GET_POOL_RUNTIME)
        ...
        self.assertNotIn("GetReadiness", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardSearchSector", GET_POOL_RUNTIME)
        self.assertNotIn("FSkyguardSearchTrackRuntime", GET_POOL_RUNTIME)
        self.assertNotIn("ClassifyFalseTrack", GET_POOL_RUNTIME)
        self.assertNotIn("ConfirmRadarGhostIdentification", GET_POOL_RUNTIME)
        self.assertNotIn("NotifyProtectedTargetDamage", GET_POOL_RUNTIME)
        self.assertNotIn("AdvanceReinforcementTimer", GET_POOL_RUNTIME)
        self.assertNotIn("GetSearchSector", GET_POOL_RUNTIME)
        self.assertNotIn("GetClassifiedFalseTrackCount", GET_POOL_RUNTIME)
        self.assertNotIn("IsHostileContactConfirmed", GET_POOL_RUNTIME)
        self.assertNotIn("GetProtectedTarget", GET_POOL_RUNTIME)
        self.assertNotIn("GetReinforcementTimeRemaining", GET_POOL_RUNTIME)
        self.assertNotIn("GetNightBeatKind", GET_POOL_RUNTIME)
        self.assertNotIn("GetNightBeatIndex", GET_POOL_RUNTIME)
        self.assertNotIn("TickNightBeatKit", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardAirfieldTarget", GET_POOL_RUNTIME)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_POOL_RUNTIME)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_POOL_RUNTIME)
        self.assertNotIn("RoadHunterSpawnLocation", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_POOL_RUNTIME)
        self.assertNotIn("ConfigureMissionDefinition", GET_POOL_RUNTIME)
        self.assertNotIn("NotifyObjectiveProgress", GET_POOL_RUNTIME)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_POOL_RUNTIME)
        self.assertNotIn("SynchronizeRuntimeState", GET_POOL_RUNTIME)
        self.assertNotIn("NotifyThreatDestroyed", GET_POOL_RUNTIME)
        self.assertNotIn("IsCorePlayableReady", GET_POOL_RUNTIME)
        self.assertNotIn("GetObjectiveRuntime", GET_POOL_RUNTIME)
        self.assertNotIn("GetGunner", GET_POOL_RUNTIME)
        self.assertNotIn("GetPathfinder", GET_POOL_RUNTIME)
        self.assertNotIn("GetMissionId", GET_POOL_RUNTIME)
        self.assertNotIn("ValidateMissionContract", GET_POOL_RUNTIME)
        self.assertNotIn("ConfigureFromMission", GET_POOL_RUNTIME)
        self.assertNotIn("AdvanceBriefing", GET_POOL_RUNTIME)
        self.assertNotIn("SetAssetsReady", GET_POOL_RUNTIME)
        self.assertNotIn("AcknowledgeAndLaunch", GET_POOL_RUNTIME)
        self.assertNotIn("CanLaunch", GET_POOL_RUNTIME)
        self.assertNotIn("GetElapsedSeconds", GET_POOL_RUNTIME)
        self.assertNotIn("GetBriefingState", GET_POOL_RUNTIME)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_POOL_RUNTIME)
        self.assertNotIn("GetBriefingText", GET_POOL_RUNTIME)
        self.assertNotIn("GetRadioChatter", GET_POOL_RUNTIME)
        self.assertNotIn("GetPresentation", GET_POOL_RUNTIME)
        self.assertNotIn("GetMissionTitle", GET_POOL_RUNTIME)
        self.assertNotIn("AcknowledgeBriefing", GET_POOL_RUNTIME)
        self.assertNotIn("LaunchSortie", GET_POOL_RUNTIME)
        self.assertNotIn("HullCollider", GET_POOL_RUNTIME)
        self.assertNotIn("OpticalTracker", GET_POOL_RUNTIME)
        self.assertNotIn("WeaponServo", GET_POOL_RUNTIME)
        self.assertNotIn("CountermeasurePod", GET_POOL_RUNTIME)
        self.assertNotIn("MinHeightFromOriginCm", GET_POOL_RUNTIME)
        self.assertNotIn("RadarNode", GET_POOL_RUNTIME)
        self.assertNotIn("ESkyguardMission02WaveState", GET_POOL_RUNTIME)
        self.assertNotIn("HarborIndustrial", GET_POOL_RUNTIME)
        self.assertNotIn("MaxIntegrity", GET_POOL_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_POOL_RUNTIME)
        self.assertNotIn("FillAndFinalize", GET_POOL_RUNTIME)
        self.assertNotIn("FillAndFail", GET_POOL_RUNTIME)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_POOL_RUNTIME)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction = (
            "public:\n"
            f"\t{UFUNCTION_PERFORMANCE}\n"
            f"\t{GET_POOL_RUNTIME}\n"
            "};\n"
        )
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_PERFORMANCE} {GET_POOL_RUNTIME}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_POOL_RUNTIME}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_POOL_RUNTIME))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_POOL_RUNTIME),
            section,
    ...
    def test_environment_category_does_not_satisfy_performance(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Briefing", UFUNCTION_PERFORMANCE)
        environment = (
        ...
        self.assertNotEqual(environment, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(briefing, UFUNCTION_PERFORMANCE)
        self.assertNotIn(environment, UFUNCTION_PERFORMANCE)
        self.assertNotIn(briefing, UFUNCTION_PERFORMANCE)
        leftover_m01 = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m02, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_harbor, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m03, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m04, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m05, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m06, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_waves, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m05_waves, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_integration, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m07_ready, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m08_waves, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m08_hoist, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m08_safety, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m08_protection, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m08_ready, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m09_waves, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m09_protection, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m09_integration, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_m09_objectives, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_payload, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_targets, UFUNCTION_PERFORMANCE)
        self.assertNotEqual(leftover_objectives, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m01, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m02, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_harbor, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m03, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m04, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m05, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m06, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m05_waves, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_integration, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m07_ready, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m08_waves, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m08_hoist, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m08_safety, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m08_protection, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m08_ready, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m09_waves, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m09_protection, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m09_integration, UFUNCTION_PERFORMANCE)
        self.assertNotIn(leftover_m09_objectives, UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission01", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission03", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission04", UFUNCTION_PERFORMANCE)
        self.assertNotIn("Mission05", UFUNCTION_PERFORMANCE)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_POOL_RUNTIME)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_pool_runtime_cpp_body(self) -> None:
        locked_only = f"{GET_POOL_RUNTIME}\n"
        self.assertNotIn("{", GET_POOL_RUNTIME)
        self.assertNotIn("}", GET_POOL_RUNTIME)
        self.assertNotIn("return ", GET_POOL_RUNTIME)
        ...
        self.assertNotIn("return false", GET_POOL_RUNTIME)
        self.assertNotIn("return true", GET_POOL_RUNTIME)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("GetPoolRuntime", GET_POOL_RUNTIME)
        self.assertTrue(GET_POOL_RUNTIME.startswith("const "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_POOL_RUNTIME)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_POOL_RUNTIME)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_POOL_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_POOL_RUNTIME)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_POOL_RUNTIME)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_POOL_RUNTIME)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_POOL_RUNTIME}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_POOL_RUNTIME),
            GET_POOL_RUNTIME,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_POOL_RUNTIME)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardIronRainBoss", GET_POOL_RUNTIME)
        self.assertNotIn("ASkyguardRadarGhostBoss", GET_POOL_RUNTIME)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission09 GetPoolRuntime method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_POOL_RUNTIME}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_POOL_RUNTIME.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_POOL_RUNTIME.lower(),
                "mission09 GetPoolRuntime contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_POOL_RUNTIME)

    def test_contract_is_get_pool_runtime_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_pool_runtime(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_POOL_RUNTIME)
            )
            self.assertNotEqual(GET_POOL_RUNTIME, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetPoolRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_POOL_RUNTIME))
        locked_only = f"{GET_POOL_RUNTIME}\n"
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_POOL_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", GET_POOL_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_PERFORMANCE)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission09_get_pool_runtime"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_mission07_get_readiness_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_m07_type, GET_POOL_RUNTIME)
        self.assertIn("GetPoolRuntime", str(raised.exception))
    ...
    def test_sibling_mission08_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("NotifyThreatDestroyed", GET_POOL_RUNTIME)
        self.assertNotIn("StartNextWave", GET_POOL_RUNTIME)
        self.assertNotIn("StartHoistWindow", GET_POOL_RUNTIME)
        self.assertNotIn("AdvanceHoistWindow", GET_POOL_RUNTIME)
        self.assertNotIn("ValidateWeaponRelease", GET_POOL_RUNTIME)
        self.assertNotIn("IsCorePlayableReady", GET_POOL_RUNTIME)
        self.assertNotIn("SynchronizeRuntimeState", GET_POOL_RUNTIME)
        self.assertNotIn("GetObjectiveRuntime", GET_POOL_RUNTIME)
        self.assertNotIn("GetWaveState", GET_POOL_RUNTIME)
    ...
    def test_leftover_mission08_get_readiness_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_m08_type, GET_POOL_RUNTIME)
        self.assertIn("GetPoolRuntime", str(raised.exception))
        self.assertNotIn(
            "cursor/mission08-get-readiness-decl-contract-c332",
            GET_POOL_RUNTIME,
    ...
    def test_contract_does_not_relock_leftover_mission09_pool(self) -> None:
        ...
        self.assertIn("Performance", UFUNCTION_PERFORMANCE)
        self.assertNotIn("GetReadiness", GET_POOL_RUNTIME)
        self.assertNotIn("Available = 48", GET_POOL_RUNTIME)
        self.assertNotIn("PeakActive", GET_POOL_RUNTIME)
        self.assertNotIn("Recycled", GET_POOL_RUNTIME)
        self.assertNotIn("PoolBudget", GET_POOL_RUNTIME)
