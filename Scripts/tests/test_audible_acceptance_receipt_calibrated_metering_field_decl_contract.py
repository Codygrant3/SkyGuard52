...
# then `bool bCalibratedMetering = false;`.
...
# bCalibratedMetering field decl.
...
# PeakActiveVoices #1438 (keep those sibling decls in
# LOCKED_SCRIPTS).
...
# Identifier bounds: match EXACT `bCalibratedMetering` as a
...
TARGET = "bool bCalibratedMetering = false;"
TARGET_WRONG_BARE = "bool bCalibratedMetering;"
TARGET_WRONG_EQ_ONE = "bool bCalibratedMetering = true;"
TARGET_WRONG_EQ_NONE = "bool bCalibratedMetering = NAME_None;"
TARGET_WRONG_TRUE = "bool bCalibratedMetering = true;"
TARGET_WRONG_FALSE = "int32 bCalibratedMetering = false;"
TARGET_WRONG_EQ_FLOAT = "bool bCalibratedMetering = 0.f;"
TARGET_WRONG_FSTRING = "FString bCalibratedMetering;"
TARGET_WRONG_FLOAT = "float bCalibratedMetering = false;"
TARGET_WRONG_FNAME = "FName bCalibratedMetering;"
TARGET_WRONG_FTEXT = "FText bCalibratedMetering;"
TARGET_WRONG_BOOL = "int32 bCalibratedMetering = 0;"
TARGET_WRONG_TARRAY = "TArray<bool> bCalibratedMetering;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> bCalibratedMetering;"
...
FIELD_NAME = "bCalibratedMetering"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bCalibratedMetering = false;`.
    # Match the EXACT identifier bCalibratedMetering as a whole
    ...
    exact = re.search(
        r"bool\s+bCalibratedMetering\s*=\s*false\s*;",
        compact,
    )
    ...
    if re.search(r"\bbAccepted\b", compact) and exact is None:
        return False
    if re.search(
        r"bool\s+bCalibratedMetering\s*;",
        compact,
    ) and re.search(
        r"bool\s+bCalibratedMetering\s*=",
        compact,
    ...
    for assigned in re.findall(
        r"bool\s+bCalibratedMetering\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "false":
            return False
    if re.search(
        r"float\s+bCalibratedMetering\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bCalibratedMetering\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bCalibratedMetering\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+bCalibratedMetering\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+bCalibratedMetering\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+bCalibratedMetering\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+bCalibratedMetering\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bCalibratedMetering\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bCalibratedMetering is missing from "
...
class AudibleAcceptanceReceiptCalibratedMeteringFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_calibrated_metering_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_bools_do_not_satisfy_calibrated_metering(self) -> None:
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bCalibratedMetering = false;", compact_origin)
        self.assertNotIn("bCalibratedMetering = true", compact_origin)
        self.assertNotIn("int32 bCalibratedMetering", compact_origin)
        self.assertNotIn("FString bCalibratedMetering", compact_origin)
        self.assertNotIn("float bCalibratedMetering", compact_origin)

    def test_calibrated_metering_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bCalibratedMetering"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_TOTAL_UNDERRUNS} = 0;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tbool bCalibratedMeterings = false;\n",
            "\tbool bCalibratedMetering = true;\n",
            "\tint32 bCalibratedMetering;\n",
            "\tfloat bCalibratedMetering = " + forty + ";\n",
            "\tfloat bCalibratedMetering = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt bCalibratedMetering field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "calibrated_metering_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_calibrated", THIS_SCRIPT)
