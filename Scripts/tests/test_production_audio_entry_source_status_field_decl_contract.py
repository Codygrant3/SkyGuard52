# THIS IS leftover-safe FSkyguardProductionAudioEntry SourceStatus.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `ESkyguardAudioSourceStatus SourceStatus =`
# `ESkyguardAudioSourceStatus::MISSING_SOURCE;`.
# THIS FIELD HAS an initializer.
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Required initializer
# `= ESkyguardAudioSourceStatus::MISSING_SOURCE`.
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# ESkyguardAudioSourceStatus, initializer dropped or changed
# from MISSING_SOURCE, Category= added.
...
# itself (leftover #241 / #65ba). Isolated FIELD decl only.
...
# Sound / Attenuation / Concurrency / OutputSubmix /
# ProvenanceId / SourceSha256.
# Analog leftover production-audio-entry-defaults #198 /
# #314b then isolated SourceStatus field decl.
# THIS IS NOT leftover audio-source-status-enum #241 / #65ba.
...
# THIS IS NOT leftover Category #1400.
# THIS IS NOT leftover OutputSubmix #1401.
# THIS IS NOT leftover Concurrency #1402.
...
# ESkyguardAudioEvent.
# If a clone asserts no initializer / bool / `= false` /
# VisibleAnywhere / BlueprintReadOnly /
# Category="Skyguard|Theater", retarget:
# type is ESkyguardAudioSourceStatus, REQUIRED initializer
# `= ESkyguardAudioSourceStatus::MISSING_SOURCE`,
# BlueprintReadWrite, no Category.
...
REQUIRED_INIT = "ESkyguardAudioSourceStatus::MISSING_SOURCE"
TARGET_WRONG_BARE = "ESkyguardAudioSourceStatus SourceStatus;"
...
TARGET_WRONG_LICENSED = (
    "ESkyguardAudioSourceStatus SourceStatus = "
    "ESkyguardAudioSourceStatus::LICENSED_THIRD_PARTY;"
)
TARGET_WRONG_QA = (
    "ESkyguardAudioSourceStatus SourceStatus = "
    "ESkyguardAudioSourceStatus::PROCEDURAL_QA_TEST_ONLY;"
)
TARGET_WRONG_FALSE = "ESkyguardAudioSourceStatus SourceStatus = false;"
TARGET_WRONG_TRUE = "ESkyguardAudioSourceStatus SourceStatus = true;"
TARGET_WRONG_ZERO = "ESkyguardAudioSourceStatus SourceStatus = 0.f;"
TARGET_WRONG_FLOAT = "float SourceStatus;"
TARGET_WRONG_FNAME = "FName SourceStatus;"
TARGET_WRONG_BOOL = "bool SourceStatus;"
TARGET_WRONG_INT = "int32 SourceStatus;"
TARGET_WRONG_FTEXT = "FText SourceStatus;"
TARGET_WRONG_FSTRING = "FString SourceStatus;"
TARGET_WRONG_PTR = "USoundBase* SourceStatus;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundBase> SourceStatus;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundWave> SourceStatus;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> SourceStatus;"
TARGET_WRONG_CATEGORY_TYPE = (
    "ESkyguardProductionAudioCategory SourceStatus = "
...
SIBLING_OUTPUT_SUBMIX = "OutputSubmix"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored initialized
    # `ESkyguardAudioSourceStatus SourceStatus =`
    # `ESkyguardAudioSourceStatus::MISSING_SOURCE;`.
    # Reject bare `SourceStatus;` without initializer.
    # Reject changed initializer / wrong types
    # (FName, FString, FText, bool, int32, float,
    # ESkyguardProductionAudioCategory, TSoftObjectPtr).
    # Do not accept sibling Category / DisplayName / Sound
    # fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"ESkyguardAudioSourceStatus\s+SourceStatus\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundBase\s*\*\s*SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundBase\s*>\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USound(?:Base|Wave)\s*>\s+SourceStatus\b",
        compact,
    ):
        return False
    if re.search(
        r"ESkyguardProductionAudioCategory\s+SourceStatus\b",
        compact,
    ):
        return False
    required = re.search(
        r"ESkyguardAudioSourceStatus\s+SourceStatus\s*=\s*"
        r"ESkyguardAudioSourceStatus\s*::\s*MISSING_SOURCE\s*;",
        compact,
    )
    if required is None:
        return False
    if re.search(
        ...
        for value in assigned:
            if collapsed(value) != "ESkyguardAudioSourceStatus::MISSING_SOURCE":
                return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardAudioSourceStatus\s+SourceStatus\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardAudioSourceStatus SourceStatus is missing from "
...
class ProductionAudioEntrySourceStatusFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_source_status_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_missing_or_changed_initializer_fails_closed(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READWRITE}\n"
        ...
        self.assertIn(REQUIRED_INIT, compact_origin)
        self.assertNotIn(
            "ESkyguardAudioSourceStatus SourceStatus;",
        ...
        self.assertNotIn("SourceStatus = false", compact_origin)
        self.assertNotIn("SourceStatus = true", compact_origin)
        self.assertNotIn("SourceStatus = 0.f", compact_origin)
        self.assertNotIn("SourceStatus = 160.f", compact_origin)

    def test_source_status_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardAudioSourceStatus SourceStatus"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn(REQUIRED_INIT, LOCKED_DECL)
        self.assertIn("= ESkyguardAudioSourceStatus::MISSING_SOURCE", LOCKED_DECL)
        ...
        self.assertIn("ESkyguardAudioSourceStatus", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CATEGORY_TYPE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_LICENSED}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_QA}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTSoftObjectPtr<USoundBase> {SIBLING_SOUND};\n",
            "\tint32 SourceStatus = 1;\n",
            "\tbool SourceStatus;\n",
            "\tfloat SourceStatus = " + forty + ";\n",
            "\tfloat SourceStatus = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_entry_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_OUTPUT_SUBMIX), section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry SourceStatus field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertNotIn(DRAFTED_DISPLAY_NAME, LOCKED_SCRIPTS)
        self.assertNotIn(DRAFTED_PROVENANCE_ID, LOCKED_SCRIPTS)
        self.assertNotIn(DRAFTED_SOUND, LOCKED_SCRIPTS)
        self.assertNotIn(DRAFTED_SOURCE_SHA256, LOCKED_SCRIPTS)
        self.assertNotIn(DRAFTED_ATTENUATION, LOCKED_SCRIPTS)
        self.assertNotIn(DRAFTED_CATEGORY, LOCKED_SCRIPTS)
        self.assertNotIn(DRAFTED_OUTPUT_SUBMIX, LOCKED_SCRIPTS)
        self.assertNotIn(DRAFTED_CONCURRENCY, LOCKED_SCRIPTS)
        self.assertTrue(Path(__file__).name.endswith(
            "source_status_field_decl_contract.py"
        ))
