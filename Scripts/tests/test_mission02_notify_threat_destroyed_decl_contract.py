...
# InitializePlayableMission / StartPayloadWindow /
...
# leftover Mission01 NotifyThreatDestroyed
# #608, leftover Mission03 NotifyThreatDestroyed
# #628, leftover Mission04 NotifyThreatDestroyed,
# leftover Mission05 NotifyThreatDestroyed #666,
...
# (`bool NotifyThreatDestroyed(int32 Amount = 1);`); accept that
...
# drafts #674–#828+, leftover Apache
...
# NotifyThreatDestroyed #729. Do not reopen
# leftover Mission09 NotifyThreatDestroyed
...
# leftover Mission05 NotifyThreatDestroyed #666,
# leftover Mission04 NotifyThreatDestroyed,
# leftover Mission06 NotifyThreatDestroyed #685,
# leftover Mission07 NotifyThreatDestroyed,
...
# an isolated Python NotifyThreatDestroyed
...
# contracts, leftover Mission01 NotifyThreatDestroyed
# #608, leftover Mission03 NotifyThreatDestroyed
...
LEFTOVER_MISSION10_NOTIFY_THREAT_DESTROYED_NOT_LOCKED = (
    "test_mission10_notify_threat_destroyed_decl_contract.py",
    "ASkyguardMission10IntegrationDirector",
    "SkyguardMission10IntegrationDirector.h",
    'Category="Skyguard|Mission10|Waves"',
    "cursor/mission10-notify-threat-destroyed-decl-contract-c332",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission02IntegrationDirector::NotifyThreatDestroyed",
    "SkyguardMission02IntegrationDirector.cpp",
    "ASkyguardMission10IntegrationDirector::NotifyThreatDestroyed",
    "SkyguardMission10IntegrationDirector.cpp",
    "SkyguardMission09IntegrationDirector.cpp",
    "SkyguardMission08IntegrationDirector.cpp",
    "SkyguardMission06IntegrationDirector.cpp",
...
class Mission02NotifyThreatDestroyedDeclContractTests(unittest.TestCase):
    def test_mission02_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, NOTIFY_THREAT_DESTROYED),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, NOTIFY_THREAT_DESTROYED)
        self.assertIn("NotifyThreatDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, NOTIFY_THREAT_DESTROYED)
    ...
    def test_missing_notify_threat_destroyed_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission02IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, NOTIFY_THREAT_DESTROYED)
        self.assertIn("NotifyThreatDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, NOTIFY_THREAT_DESTROYED)
        self.assertIn("NotifyThreatDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("BlueprintCallable", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("Category", NOTIFY_THREAT_DESTROYED)
        ...
        self.assertNotIn("Rescue", UFUNCTION_WAVES)
        self.assertIn("BlueprintCallable", UFUNCTION_WAVES)
        ...
        self.assertNotIn("Evacuation", UFUNCTION_WAVES)
        self.assertNotIn("Campaign", UFUNCTION_WAVES)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, NOTIFY_THREAT_DESTROYED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, NOTIFY_THREAT_DESTROYED)
        self.assertIn("NotifyThreatDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid NotifyThreatDestroyed(int32 Amount = 1);\n"
        as_const = "\tbool NotifyThreatDestroyed(int32 Amount = 1) const;\n"
        with_arg = "\tbool NotifyThreatDestroyed(bool bForce);\n"
        renamed = "\tbool NotifyThreat();\n"
        short_name = "\tbool NotifyDestroyed();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, NOTIFY_THREAT_DESTROYED)
            self.assertIn("NotifyThreatDestroyed", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_notify_threat_destroyed_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, NOTIFY_THREAT_DESTROYED),
            NOTIFY_THREAT_DESTROYED,
        )
        self.assertTrue(has_declaration(section, NOTIFY_THREAT_DESTROYED))
        self.assertEqual(
            declaration_count(section, NOTIFY_THREAT_DESTROYED),
            1,
        )
        self.assertTrue(
            NOTIFY_THREAT_DESTROYED.startswith("bool "),
            NOTIFY_THREAT_DESTROYED,
        )
        self.assertTrue(
            NOTIFY_THREAT_DESTROYED.endswith(";"),
            NOTIFY_THREAT_DESTROYED,
        )
        self.assertIn("NotifyThreatDestroyed", NOTIFY_THREAT_DESTROYED)
        self.assertIn("(int32 Amount = 1)", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("TArray<", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("TObjectPtr", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("TSoftObjectPtr", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("INDEX_NONE", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("UFUNCTION", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("{", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("}", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("return ", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("const;", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("Root", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("Briefing", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AudioDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("RadioChatter", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("SortiePresentation", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("CampaignDefinition", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("MissionDefinition", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("Readiness", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("bAutoInitialize", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("bAllowBoundedActorSpawning", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("bAutoLaunchAfterBriefing", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("PathfinderSpawnLocation", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetAircraft", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("BindRuntimeActors", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("HandleDroneCityImpact", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("StartNextWave", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("InitializePlayableMission", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AdvanceConvoyByDistance", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("NotifyConvoyDamage", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetWaveState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetConvoyRouteState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetDayBeatKit", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetNightBeatKit", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("StartSearchlightWindow", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AdvanceSearchlightTrack", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("NotifySubstationDamage", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetRemainingThreatsInWave", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetSearchlightRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetSubstationIntegrity", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("StartPayloadWindow", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AdvancePayloadWindow", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("TryJamActivePayload", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("NotifyAirfieldTargetDamage", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetPayloadWindow", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetTargetRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetSurvivingTargetCount", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetDayBeatKind", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetDayBeatIndex", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("TickDayBeatKit", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("HandleBossPhaseChanged", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission06WaveState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission07WaveState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission07IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission08IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission08WaveState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission09WaveState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission09ProtectedTarget", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardMission09ProtectedTargetRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardMission09PoolRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardMission09PoolBudget", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("BindCampaignRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetPoolRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("IronRainSpawnLocation", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission08ProtectedTarget", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardMission08ProtectedTargetRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardHoistWindowRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("StartHoistWindow", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AdvanceHoistWindow", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ValidateWeaponRelease", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetHoistRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetRejectedWeaponReleases", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetStormRainBeatKit", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ApplyStormRainPlayContract", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetStormRainBeatKind", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("TickStormRainBeatKit", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("RescueHelicopterAnchor", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("LifelineHunterSpawnLocation", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardSearchSector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardSearchTrackRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ClassifyFalseTrack", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ConfirmRadarGhostIdentification", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("NotifyProtectedTargetDamage", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AdvanceReinforcementTimer", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetSearchSector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetClassifiedFalseTrackCount", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("IsHostileContactConfirmed", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetProtectedTarget", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetReinforcementTimeRemaining", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetNightBeatKind", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetNightBeatIndex", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("TickNightBeatKit", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardAirfieldTarget", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ConvoyRuntimeAnchor", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("RoadHunterSpawnLocation", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ConfigureMissionDefinition", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("NotifyObjectiveProgress", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("NotifyProtectedAssetFailed", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("SynchronizeRuntimeState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("IsCorePlayableReady", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetReadiness", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetObjectiveRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetGunner", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetPathfinder", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetMissionId", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ValidateMissionContract", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ConfigureFromMission", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AdvanceBriefing", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("SetAssetsReady", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AcknowledgeAndLaunch", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("CanLaunch", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetElapsedSeconds", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetBriefingState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetMinimumWarmupSeconds", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetBriefingText", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetRadioChatter", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetPresentation", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetMissionTitle", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("AcknowledgeBriefing", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("LaunchSortie", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("HullCollider", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("OpticalTracker", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("WeaponServo", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("CountermeasurePod", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("MinHeightFromOriginCm", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("RadarNode", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ESkyguardMission02WaveState", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("HarborIndustrial", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("MaxIntegrity", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("CurrentIntegrity", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FillAndFinalize", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("FillAndFail", NOTIFY_THREAT_DESTROYED)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, NOTIFY_THREAT_DESTROYED)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tNotifyThreatDestroyed(int32 Amount = 1);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_WAVES} {NOTIFY_THREAT_DESTROYED}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{NOTIFY_THREAT_DESTROYED}\n}}\n"
        self.assertTrue(has_declaration(one_line, NOTIFY_THREAT_DESTROYED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, NOTIFY_THREAT_DESTROYED),
            section,
    ...
    def test_environment_category_does_not_satisfy_waves(self) -> None:
        ...
        self.assertNotEqual(leftover_m10, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_harbor, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m02_integration, UFUNCTION_WAVES)
        ...
        self.assertNotEqual(leftover_m07, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m08, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09_waves, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m10_waves, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m10_safety, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m10_protection, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09_pool, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_hoist, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_safety, UFUNCTION_WAVES)
        ...
        self.assertNotIn(leftover_m10, UFUNCTION_WAVES)
        self.assertNotIn(leftover_harbor, UFUNCTION_WAVES)
        self.assertNotIn(leftover_m02_integration, UFUNCTION_WAVES)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{NOTIFY_THREAT_DESTROYED}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, NOTIFY_THREAT_DESTROYED)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", NOTIFY_THREAT_DESTROYED)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_lock_initialize_cpp_body(self) -> None:
        ...
        self.assertNotIn("return false", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("return true", NOTIFY_THREAT_DESTROYED)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{NOTIFY_THREAT_DESTROYED}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_THREAT_DESTROYED)
        self.assertIn("NotifyThreatDestroyed", NOTIFY_THREAT_DESTROYED)
        self.assertTrue(NOTIFY_THREAT_DESTROYED.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, NOTIFY_THREAT_DESTROYED)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, NOTIFY_THREAT_DESTROYED)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("CurrentIntegrity", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("SkyguardApacheAircraft.h", NOTIFY_THREAT_DESTROYED)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", NOTIFY_THREAT_DESTROYED)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn("550.f", NOTIFY_THREAT_DESTROYED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{NOTIFY_THREAT_DESTROYED}\n"
        self.assertEqual(
            require_declaration(locked_only, NOTIFY_THREAT_DESTROYED),
            NOTIFY_THREAT_DESTROYED,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, NOTIFY_THREAT_DESTROYED)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardIronRainBoss", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardRadarGhostBoss", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("ASkyguardTempestBoss", section)
        self.assertNotIn("ASkyguardLastFlightBoss", NOTIFY_THREAT_DESTROYED)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn(
            "ASkyguardMission08IntegrationDirector::NotifyThreatDestroyed",
            section,
        )
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("{", NOTIFY_THREAT_DESTROYED)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission02 NotifyThreatDestroyed method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{NOTIFY_THREAT_DESTROYED}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, NOTIFY_THREAT_DESTROYED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                NOTIFY_THREAT_DESTROYED.lower(),
                "mission02 NotifyThreatDestroyed contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, NOTIFY_THREAT_DESTROYED)

    def test_contract_is_initialize_playable_mission_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(NOTIFY_THREAT_DESTROYED.startswith("bool "))
        self.assertIn("NotifyThreatDestroyed", NOTIFY_THREAT_DESTROYED)
        self.assertTrue(NOTIFY_THREAT_DESTROYED.endswith(";"))
        self.assertIn(UFUNCTION_WAVES, section)
    ...
    def test_sibling_director_fields_do_not_satisfy_initialize(
        ...
        for leftover in (
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, NOTIFY_THREAT_DESTROYED)
            )
            self.assertNotEqual(NOTIFY_THREAT_DESTROYED, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("NotifyThreatDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, NOTIFY_THREAT_DESTROYED))
        locked_only = f"{NOTIFY_THREAT_DESTROYED}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool NotifyThreatDestroyed(int32 Amount = 1) { return false; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission02_notify_threat_destroyed"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_mission10_route_phase_enum(
        ...
        self.assertNotIn("GetRoutePhase", NOTIFY_THREAT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_mission10_protected_group(
        ...
        self.assertNotIn("Protection", UFUNCTION_WAVES)
        self.assertNotIn("GetProtectedGroup", NOTIFY_THREAT_DESTROYED)
    ...
    def test_leftover_mission10_sibling_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("HandleDroneCityImpact", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("UFUNCTION", GET_DAY_BEAT_KIT)
        self.assertNotIn("StartPhaseWave", NOTIFY_THREAT_DESTROYED)
        self.assertNotIn("GetRoutePhase", NOTIFY_THREAT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_mission10_notify_threat_destroyed(
        self,
    ) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission10IntegrationDirector "
        ...
        self.assertIn("missing", str(raised.exception).lower())
        locked_only = f"{NOTIFY_THREAT_DESTROYED}\n"
        for token in LEFTOVER_MISSION10_NOTIFY_THREAT_DESTROYED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_THREAT_DESTROYED)
        self.assertIn(
