...
# or lock the RecordPlayerEvent body in the .cpp.
...
# observer records an event reached exclusively through
# a bound player-input wrapper; it never drives input or
# combat. Stay off sibling RecordGameplayEvent (this
# wave) and leftover IsCaptureActive (#427). Stay off
# leftover Initialize / Deinitialize / Tick /
# GetStatId / IsTickable overrides. Stay off leftover
# settings-apply-broadcast #1268. Stay off leftover
# Gunner, leftover apache-aircraft empty-fail-closed
# #851b, leftover apache-chin-muzzle #4e39, leftover
# apache-own-ship-systems #96c5. Stay off leftover
# apache-cpg-feel #8951. Stay off leftover
# SetInvertVerticalLook (#428). Stay off leftover
...
# isolated-test drafts #107–#428, leftover skyline
...
# RecordPlayerEvent declaration contract on
# USkyguardInputCombatPerformanceCapture. Stay off
# RecordGameplayEvent (this wave), leftover
# IsCaptureActive (#427), leftover Initialize /
# Deinitialize / Tick / GetStatId / IsTickable
# overrides, leftover apache-aircraft
...
# SetInvertVerticalLook (#428), leftover CPG HUD /
# sight HUD, leftover drafts #56–#64, leftover
# isolated-test drafts #107–#428, leftover
...
# leftover settings contracts including leftover
# SetInvertVerticalLook (#428), leftover
...
# settings-apply-broadcast #1268, leftover
# IsCaptureActive (#427), leftover RecordGameplayEvent
# sibling, leftover Initialize / Deinitialize / Tick /
...
# #107–#428, leftover gun-fire camera shake, leftover
...
# Tick / GetStatId / IsTickable overrides, leftover
# IsCaptureActive (#427), and sibling
# RecordGameplayEvent stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# SetInvertVerticalLook (#428) stay unlocked. Do not
...
# Leftover IsCaptureActive (#427) and sibling
# RecordGameplayEvent stay sibling-only.
...
# .cpp RecordPlayerEvent body / invented INDEX_NONE stay
...
class InputCaptureRecordPlayerEventDeclContractTests(unittest.TestCase):
    def test_input_capture_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, RECORD_PLAYER_EVENT), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, RECORD_PLAYER_EVENT)
        self.assertIn("RecordPlayerEvent", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, RECORD_PLAYER_EVENT))

    def test_missing_record_player_event_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, RECORD_PLAYER_EVENT)
        self.assertIn("RecordPlayerEvent", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, RECORD_PLAYER_EVENT)
        self.assertIn("RecordPlayerEvent", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tstatic void RecordPlayerEvent;\n"
        missing_static = (
            "\tvoid RecordPlayerEvent("
            "const UObject* WorldContext, FName EventName);\n"
        )
        wrong_return_bool = (
            "\tstatic bool RecordPlayerEvent("
            "const UObject* WorldContext, FName EventName);\n"
        )
        wrong_return_int = (
            "\tstatic int32 RecordPlayerEvent("
            "const UObject* WorldContext, FName EventName);\n"
        )
        added_const = (
            "\tstatic void RecordPlayerEvent("
            "const UObject* WorldContext, FName EventName) const;\n"
        )
        missing_const = (
            "\tstatic void RecordPlayerEvent("
            "UObject* WorldContext, FName EventName);\n"
        )
        wrong_type = (
            "\tstatic void RecordPlayerEvent("
            "const UWorld* WorldContext, FName EventName);\n"
        )
        missing_ptr = (
            "\tstatic void RecordPlayerEvent("
            "const UObject WorldContext, FName EventName);\n"
        ...
        missing_event = (
            "\tstatic void RecordPlayerEvent(const UObject* WorldContext);\n"
        )
        added_arg = (
            "\tstatic void RecordPlayerEvent("
            "const UObject* WorldContext, FName EventName, int32 Extra);\n"
        )
        no_args = "\tstatic void RecordPlayerEvent();\n"
        leftover_gameplay = (
            "\tstatic void RecordGameplayEvent("
            "const UObject* WorldContext, FName EventName);\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, RECORD_PLAYER_EVENT)
            self.assertIn("RecordPlayerEvent", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_record_player_event_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, RECORD_PLAYER_EVENT),
            RECORD_PLAYER_EVENT,
        )
        self.assertTrue(has_declaration(section, RECORD_PLAYER_EVENT))
        self.assertEqual(declaration_count(section, RECORD_PLAYER_EVENT), 1)
        self.assertTrue(
            RECORD_PLAYER_EVENT.startswith("static void "),
            RECORD_PLAYER_EVENT,
        )
        self.assertTrue(RECORD_PLAYER_EVENT.endswith(";"), RECORD_PLAYER_EVENT)
        self.assertIn("RecordPlayerEvent(", RECORD_PLAYER_EVENT)
        self.assertIn("const UObject* WorldContext", RECORD_PLAYER_EVENT)
        self.assertIn("FName EventName", RECORD_PLAYER_EVENT)
        self.assertNotIn("INDEX_NONE", RECORD_PLAYER_EVENT)
        self.assertNotIn("{", RECORD_PLAYER_EVENT)
        self.assertNotIn("}", RECORD_PLAYER_EVENT)
        self.assertNotIn("return ", RECORD_PLAYER_EVENT)
        self.assertNotIn(" const;", RECORD_PLAYER_EVENT)
        self.assertNotIn("UFUNCTION", RECORD_PLAYER_EVENT)
        self.assertNotIn("IsCaptureActive", RECORD_PLAYER_EVENT)
        self.assertNotIn("RecordGameplayEvent", RECORD_PLAYER_EVENT)
        self.assertNotIn("Initialize", RECORD_PLAYER_EVENT)
        self.assertNotIn("Deinitialize", RECORD_PLAYER_EVENT)
        self.assertNotIn("GetStatId", RECORD_PLAYER_EVENT)
        self.assertNotIn("IsTickable", RECORD_PLAYER_EVENT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, RECORD_PLAYER_EVENT),
                section,
            )
            self.assertEqual(
                require_declaration(section, RECORD_PLAYER_EVENT),
            ...
            self.assertEqual(
                declaration_count(section, RECORD_PLAYER_EVENT),
                1,
            )
        one_line = f"{{\npublic:\n\t{RECORD_PLAYER_EVENT}\n}}\n"
        self.assertTrue(has_declaration(one_line, RECORD_PLAYER_EVENT))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, RECORD_PLAYER_EVENT), section)
        self.assertEqual(
            require_declaration(section, RECORD_PLAYER_EVENT),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{RECORD_PLAYER_EVENT}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", RECORD_PLAYER_EVENT)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", RECORD_PLAYER_EVENT)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_invent_ufunction(self) -> None:
        ...
        self.assertNotIn("BlueprintCallable", RECORD_PLAYER_EVENT)
        self.assertNotIn("BlueprintPure", RECORD_PLAYER_EVENT)
        self.assertNotIn("UFUNCTION", section)

    def test_contract_does_not_lock_record_player_event_cpp_body(self) -> None:
        locked_only = f"{RECORD_PLAYER_EVENT}\n"
        self.assertNotIn("{", RECORD_PLAYER_EVENT)
        self.assertNotIn("}", RECORD_PLAYER_EVENT)
        self.assertNotIn("return ", RECORD_PLAYER_EVENT)
        ...
        self.assertNotIn("return false", RECORD_PLAYER_EVENT)
        self.assertNotIn("return true", RECORD_PLAYER_EVENT)

    def test_contract_does_not_relock_leftover_overrides(self) -> None:
        locked_only = f"{RECORD_PLAYER_EVENT}\n"
        for neighbor in LEFTOVER_OVERRIDES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RECORD_PLAYER_EVENT)
        self.assertNotIn("Initialize", RECORD_PLAYER_EVENT)
        self.assertNotIn("Deinitialize", RECORD_PLAYER_EVENT)
        self.assertNotIn("GetStatId", RECORD_PLAYER_EVENT)
        self.assertNotIn("IsTickable", RECORD_PLAYER_EVENT)
        self.assertNotIn("DeltaTime", RECORD_PLAYER_EVENT)

    def test_contract_does_not_relock_is_capture_active(self) -> None:
        locked_only = f"{RECORD_PLAYER_EVENT}\n"
        for neighbor in IS_CAPTURE_ACTIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RECORD_PLAYER_EVENT)
        self.assertNotIn("IsCaptureActive", RECORD_PLAYER_EVENT)
    ...
    def test_contract_does_not_relock_leftover_empty_fail_closed(self) -> None:
        ...
        for token in LEFTOVER_EMPTY_FAIL_CLOSED_NOT_LOCKED:
            ...
            self.assertNotIn(token, RECORD_PLAYER_EVENT)
        self.assertNotIn(
            "test_apache_aircraft_empty_fail_closed.py",
            RECORD_PLAYER_EVENT,
        )
    ...
    def test_contract_does_not_relock_leftover_chin_muzzle(self) -> None:
        ...
        for token in LEFTOVER_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_PLAYER_EVENT)
        self.assertNotIn("GetChinMuzzleLocation", RECORD_PLAYER_EVENT)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_PLAYER_EVENT)
        self.assertNotIn("ESkyguardApacheSystem", RECORD_PLAYER_EVENT)
    ...
    def test_contract_does_not_relock_leftover_settings(self) -> None:
        ...
        self.assertNotIn("ApplySettings", RECORD_PLAYER_EVENT)
        self.assertNotIn("bInvertLook", RECORD_PLAYER_EVENT)
        self.assertNotIn("SetInvertVerticalLook", RECORD_PLAYER_EVENT)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_PLAYER_EVENT)
        self.assertNotIn("FillAndFinalize", RECORD_PLAYER_EVENT)
        self.assertNotIn("FillAndFail", RECORD_PLAYER_EVENT)
        self.assertNotIn("ASkyguardGunner", RECORD_PLAYER_EVENT)
    ...
    def test_contract_does_not_relock_leftover_apache_decl_siblings(
        ...
        for token in LEFTOVER_APACHE_DECL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_PLAYER_EVENT)
        self.assertNotIn("FaceWorldLocation", RECORD_PLAYER_EVENT)
        self.assertNotIn("AimChinTurret", RECORD_PLAYER_EVENT)
        self.assertNotIn("GetRotorRPM", RECORD_PLAYER_EVENT)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", RECORD_PLAYER_EVENT)
        self.assertNotIn("USkyguardBriefingWidget", RECORD_PLAYER_EVENT)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, RECORD_PLAYER_EVENT)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", RECORD_PLAYER_EVENT)
        self.assertNotIn("ESkyguardMissionSkylineStyle", RECORD_PLAYER_EVENT)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{RECORD_PLAYER_EVENT}\n"
        self.assertEqual(
            require_declaration(locked_only, RECORD_PLAYER_EVENT),
            RECORD_PLAYER_EVENT,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "input capture RecordPlayerEvent contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, RECORD_PLAYER_EVENT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"input capture RecordPlayerEvent contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, RECORD_PLAYER_EVENT)

    def test_contract_is_record_player_event_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", RECORD_PLAYER_EVENT)
        self.assertNotIn("{", RECORD_PLAYER_EVENT)
        self.assertTrue(RECORD_PLAYER_EVENT.startswith("static void "))
        self.assertTrue(RECORD_PLAYER_EVENT.endswith(";"))
