# THIS IS leftover-safe FSkyguardMeshBindSlot ProxyFallback.
...
# then BARE `TSoftObjectPtr<UStaticMesh> ProxyFallback;` (no initializer).
...
# if Category= is dropped or changed, if an initializer
# is added, or if the type is not TSoftObjectPtr<UStaticMesh>.
...
# Do NOT parse the leftover catalog class body.
# Do NOT parse leftover runtime-mesh-catalog-fail-closed
# #140 / #c9be except keep that file in LOCKED_SCRIPTS.
# Do NOT contract sibling fields SlotId / Preferred / Notes.
# Sibling Preferred is also TSoftObjectPtr<UStaticMesh>;
# lock exact identifier ProxyFallback only.
# Analog leftover mesh-bind-slot-fields #194 / #6829
# then this isolated ProxyFallback field decl.
# THIS IS NOT leftover #194 / #6829.
...
# THIS IS NOT leftover MissionResult.
# THIS IS NOT leftover TheaterKit Category API #1299-#1331.
# THIS IS NOT leftover protect-asset-cargo-proxy #60
# or leftover patrol-ship-proxy-fallback #67
# (those are presentation binds, not this isolated UPROPERTY).
...
# leftover clone FName field / no Category / required
# initializer, RETARGET: type is TSoftObjectPtr<UStaticMesh>
# ProxyFallback, no `=`, specifiers are EditAnywhere +
# BlueprintReadOnly, KEEP Category="Skyguard|MeshBind".
# Do NOT parse leftover FSkyguardMissionResult /
# leftover FSkyguardMissionDebrief /
# leftover FSkyguardRouteDefinition /
# leftover FSkyguardAudibleAcceptanceReceipt /
# leftover ProductionAudio Audit/Entry/Routing /
# leftover USkyguardAudioProductionBank /
# leftover FSkyguardTheaterKitSpec /
# leftover ASkyguardCampaignTheaterKit.
...
TARGET = "TSoftObjectPtr<UStaticMesh> ProxyFallback;"
TARGET_WRONG_EQ_NONE = (
    "TSoftObjectPtr<UStaticMesh> ProxyFallback = nullptr;"
)
TARGET_WRONG_FALSE = (
    "TSoftObjectPtr<UStaticMesh> ProxyFallback = false;"
)
TARGET_WRONG_TRUE = (
    "TSoftObjectPtr<UStaticMesh> ProxyFallback = true;"
)
TARGET_WRONG_ZERO = (
    "TSoftObjectPtr<UStaticMesh> ProxyFallback = 0.f;"
)
TARGET_WRONG_FLOAT = "float ProxyFallback;"
TARGET_WRONG_FNAME = "FName ProxyFallback;"
TARGET_WRONG_PREFERRED = (
    "TSoftObjectPtr<UStaticMesh> Preferred;"
)
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<UStaticMesh> ProxyFallback;"
TARGET_WRONG_SKELETAL = (
    "TSoftObjectPtr<USkeletalMesh> ProxyFallback;"
)
...
STOP_BEFORE_CATALOG_BARE = "class USkyguardRuntimeMeshCatalog"
...
LEFTOVER_MESH_BIND_SLOT_FIELDS = LEFTOVER_MESH_BIND_SLOT
LEFTOVER_RUNTIME_MESH_CATALOG_FAIL_CLOSED = (
    "Scripts/tests/test_runtime_mesh_catalog_fail_closed.py"
)
LEFTOVER_RUNTIME_MESH_CATALOG_FAIL_CLOSED_TESTS = (
    "Scripts/tests/test_runtime_mesh_catalog_fail_closed_tests.py"
)
LEFTOVER_RUNTIME_MESH_CATALOG_FAIL_CLOSED_CONTRACT = (
    "Scripts/tests/test_runtime_mesh_catalog_fail_closed"
    "_contract.py"
)
LEFTOVER_PATROL_SHIP_PROXY_FALLBACK = (
    "Scripts/tests/test_patrol_ship_proxy_fallback.py"
)
LEFTOVER_PATROL_SHIP_PROXY_FALLBACK_TESTS = (
    "Scripts/tests/test_patrol_ship_proxy_fallback_tests.py"
)
LEFTOVER_PATROL_SHIP_PROXY_FALLBACK_CONTRACT = (
    "Scripts/tests/test_patrol_ship_proxy_fallback_contract.py"
)
...
LEFTOVER_MISSION_RESULT_SHOTS_FIRED = (
...
LEFTOVER_MISSION_RESULT_COMPLETION_TIME = (
    "Scripts/tests/test_mission_result_completion_time_seconds"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION_RESULT_AIRCRAFT_DAMAGE = (
    "Scripts/tests/test_mission_result_aircraft_damage_fraction"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION_RESULT_COMPLETED_OBJECTIVE_IDS = (
...
LOCKED_SCRIPTS = (
    ...
    LEFTOVER_MISSION_RESULT_COMPLETED_OBJECTIVE_IDS,
    CLONE_WEATHER_IDENTITY,
    # leftover MissionResult / RouteDefinition /
    # AudibleAcceptanceReceipt / ProductionAudio /
    # MissionDebrief / TheaterKit / catalog fail-closed
    # #140 / leftover proxy PRs #60/#67
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept initializer / leftover float Health when
    # origin/main has a bare TSoftObjectPtr. Do not accept
    # sibling SlotId / Preferred / Notes. Preferred is the
    # same TSoftObjectPtr type; require exact ProxyFallback.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*UStaticMesh\s*>\s+ProxyFallback\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:FName|float|bool|int32|FString|TObjectPtr)\s+"
        r"ProxyFallback\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USkeletalMesh\s*>\s+ProxyFallback\b",
        compact,
    ):
        return False
    if re.search(
...
def leftover_catalog_class_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_CATALOG,
        STOP_BEFORE_CATALOG_BARE,
        "EnsureDefaultSlots",
        "ResolveMesh",
        "FindSlot",
        "ResolveSlot",
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return leftover_catalog_class_tokens() + (
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*UStaticMesh\s*>\s+"
            r"ProxyFallback\b",
            compact[index:],
        ):
    ...
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<UStaticMesh> ProxyFallback "
...
class MeshBindSlotProxyFallbackFieldDeclContractTests(unittest.TestCase):
    def test_mesh_bind_slot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ProxyFallback"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ProxyFallback"), section)
        self.assertNotIn(STOP_BEFORE_CATALOG, section)
        self.assertNotIn(CATALOG_NAME, section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        ...
        self.assertIn("ProxyFallback", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_proxy_fallback_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_SLOT_ID};\n"
            f"\t{TARGET_WRONG_PREFERRED}\n"
            "};\n"
        )
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("ProxyFallback = NAME_None", compact_origin)
        self.assertNotIn("ProxyFallback = false", compact_origin)
        self.assertNotIn("ProxyFallback = true", compact_origin)
        self.assertNotIn("ProxyFallback = 0.f", compact_origin)
        self.assertNotIn("ProxyFallback = 160.f", compact_origin)

    def test_proxy_fallback_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("TSoftObjectPtr<UStaticMesh>", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<UStaticMesh> ProxyFallbacks;\n",
            "\tint32 ProxyFallback;\n",
            "\tbool ProxyFallback;\n",
            "\tfloat ProxyFallback = " + forty + ";\n",
            "\tfloat ProxyFallback = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        self.assertFalse(
            has_declaration("\tFName\n\tProxyFallback;\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_PREFERRED}\n", LOCKED_DECL)
        )
    ...
    def test_does_not_parse_actor_namespace_or_neighbors(self) -> None:
        ...
        for token in leftover_catalog_class_tokens():
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mesh-bind-slot ProxyFallback field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "proxy_fallback_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_PATROL_SHIP_PROXY_FALLBACK, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PATROL_SHIP_PROXY_FALLBACK_TESTS, LOCKED_SCRIPTS)
        self.assertIn(
            LEFTOVER_PATROL_SHIP_PROXY_FALLBACK_CONTRACT,
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_PREFERRED, locked_only)
        for helper in namespace_helper_tokens():
