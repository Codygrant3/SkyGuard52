...
# or lock the OpenOrbitExposure body in the .cpp.
...
# SetContactIdentified / IsContactIdentified. Stay off
...
# #56–#64, leftover isolated-test drafts #107–#434,
...
OPEN_ORBIT_EXPOSURE = "bool OpenOrbitExposure();"
...
# isolated Python OpenOrbitExposure declaration
...
# drafts #107–#434, leftover Pathfinder height
...
# contracts, leftover BlackKite SetSearchlightTracked /
# IsSearchlightTracked, leftover
# searchlight-track-runtime-defaults #7347, leftover
# RadarGhost SetContactIdentified /
# IsContactIdentified siblings, leftover
# apache-aircraft empty-fail-closed #851b, leftover
# apache-chin-muzzle #4e39, leftover
# apache-own-ship-systems #96c5, leftover
...
# .cpp OpenOrbitExposure body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRadarGhostBoss::OpenOrbitExposure",
    "SkyguardRadarGhostBoss.cpp",
)
SIBLING_TYPES = (
...
class RadarGhostOpenOrbitExposureDeclContractTests(unittest.TestCase):
    def test_radar_ghost_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, OPEN_ORBIT_EXPOSURE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, OPEN_ORBIT_EXPOSURE)
        self.assertIn("OpenOrbitExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, OPEN_ORBIT_EXPOSURE))

    def test_missing_open_orbit_exposure_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tvoid SetContactIdentified(bool bIdentified);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, OPEN_ORBIT_EXPOSURE)
        self.assertIn("OpenOrbitExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, OPEN_ORBIT_EXPOSURE)
        self.assertIn("OpenOrbitExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("UFUNCTION", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("Category", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("BlueprintCallable", OPEN_ORBIT_EXPOSURE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, OPEN_ORBIT_EXPOSURE)
        self.assertIn("OpenOrbitExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool OpenOrbitExposure;\n"
        wrong_return_void = "\tvoid OpenOrbitExposure();\n"
        wrong_return_int = "\tint32 OpenOrbitExposure();\n"
        added_const = "\tbool OpenOrbitExposure() const;\n"
        added_arg = "\tbool OpenOrbitExposure(bool Extra);\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, OPEN_ORBIT_EXPOSURE)
            self.assertIn("OpenOrbitExposure", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_open_orbit_exposure_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, OPEN_ORBIT_EXPOSURE))
        self.assertEqual(declaration_count(section, OPEN_ORBIT_EXPOSURE), 1)
        self.assertTrue(
            OPEN_ORBIT_EXPOSURE.startswith("bool "),
            OPEN_ORBIT_EXPOSURE,
        )
        self.assertTrue(
            OPEN_ORBIT_EXPOSURE.endswith(";"),
            OPEN_ORBIT_EXPOSURE,
        )
        self.assertIn("OpenOrbitExposure(", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("INDEX_NONE", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("{", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("}", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("return ", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn(" const", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SetContactIdentified", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("IsContactIdentified", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SetSearchlightTracked", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("GetChinMuzzleLocation", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("GetGunnerMount", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("FaceWorldLocation", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SignatureModulator", OPEN_ORBIT_EXPOSURE)
        for token in leftover_named_surfaces():
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, OPEN_ORBIT_EXPOSURE),
                1,
            )
        one_line = f"{{\npublic:\n\t{OPEN_ORBIT_EXPOSURE}\n}}\n"
        self.assertTrue(has_declaration(one_line, OPEN_ORBIT_EXPOSURE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, OPEN_ORBIT_EXPOSURE),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("return ", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("return false", OPEN_ORBIT_EXPOSURE)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{OPEN_ORBIT_EXPOSURE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", OPEN_ORBIT_EXPOSURE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_open_orbit_exposure_cpp_body(
        self,
    ) -> None:
        locked_only = f"{OPEN_ORBIT_EXPOSURE}\n"
        self.assertNotIn("{", OPEN_ORBIT_EXPOSURE)
        ...
        self.assertNotIn("SkyguardRadarGhostBoss.cpp", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SkyguardRadarGhostBoss.cpp", locked_only)
        self.assertNotIn("return false", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("return true", OPEN_ORBIT_EXPOSURE)

    def test_contract_does_not_relock_set_or_is_contact_siblings(
        self,
    ) -> None:
        locked_only = f"{OPEN_ORBIT_EXPOSURE}\n"
    ...
    def test_contract_does_not_relock_leftover_named_surfaces(self) -> None:
        ...
        for sibling in leftover_boss_script_names():
            self.assertNotIn(sibling, OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_weak_point_fields(self) -> None:
        ...
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SignatureModulator", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("RadarReceiver", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("CoolingDoor", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_black_kite_siblings(
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("ASkyguardBlackKiteBoss", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SetSearchlightTracked", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("IsSearchlightTracked", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_empty_fail_closed(self) -> None:
        ...
        for token in LEFTOVER_EMPTY_FAIL_CLOSED_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("GetGunnerMount", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("GetChinTurret", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("ESkyguardApacheSystem", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_apache_isolated(self) -> None:
        ...
        for token in LEFTOVER_APACHE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("FaceWorldLocation", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("MaxIntegrity", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("ASkyguardGunner", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_settings_input(self) -> None:
        ...
        for token in LEFTOVER_SETTINGS_INPUT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SetInvertVerticalLook", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("IsCaptureActive", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("USkyguardBriefingWidget", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("ESkyguardMissionSkylineStyle", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_scan_wrong_headers(self) -> None:
        ...
        for token in LEFTOVER_WRONG_HEADER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("MinHeightFromOriginCm", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("MaxIntegrity", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SkyguardPathfinder", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SkyguardApacheAircraft.h", OPEN_ORBIT_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("ASkyguardRadarNode", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SkyguardRadarNode.h", locked_only)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{OPEN_ORBIT_EXPOSURE}\n"
        self.assertEqual(
            require_declaration(locked_only, OPEN_ORBIT_EXPOSURE),
            OPEN_ORBIT_EXPOSURE,
        )
        for neighbor in UNLOCKED_NEIGHBORS:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("SetContactIdentified", locked_only)
        self.assertNotIn("IsContactIdentified", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar ghost OpenOrbitExposure contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        file_text = this_file_text()
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, OPEN_ORBIT_EXPOSURE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                OPEN_ORBIT_EXPOSURE.lower(),
                f"radar ghost OpenOrbitExposure contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, OPEN_ORBIT_EXPOSURE)

    def test_contract_is_open_orbit_exposure_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", OPEN_ORBIT_EXPOSURE)
        self.assertNotIn("{", OPEN_ORBIT_EXPOSURE)
        self.assertTrue(OPEN_ORBIT_EXPOSURE.startswith("bool "))
        self.assertTrue(OPEN_ORBIT_EXPOSURE.endswith(";"))
        self.assertIn(UFUNCTION_MISSION07_BOSS, section)
