# THIS IS leftover-safe FSkyguardStormRainBeatKit Weather.
# origin/main form: plain C++ field
# `ESkyguardMissionWeather Weather =
# ESkyguardMissionWeather::Storm;`
# with in-struct initializer Storm. NOT a UPROPERTY wrap.
...
# Approach / Con+tact / Sho+re, leftover LoadoutSpec, leftover
# ASkyguardGunshipSortieDirector, leftover FSkyguardTheaterKitSpec.
# Do NOT parse leftover FSkyguardWeatherProfile Weather #1353
# (`ESkyguardMissionWeather::Clear` plus UPROPERTY wrap).
# Do NOT contract sibling fields BeatCount / MissionId / Title /
# WeatherIdentity / WeatherLabel / bHydraForClusters / Kinds /
# Threats / Stations / Calls.
# Do NOT claim leftover WeatherIdentity or leftover WeatherLabel.
# THIS IS the Weather enum field, not leftover WeatherIdentity,
# not leftover WeatherLabel, not leftover
# FSkyguardWeatherProfile.Weather #1353.
# THIS IS NOT leftover-safe StormRain WeatherIdentity isolated.
# THIS IS NOT leftover-safe Night kit WeatherIdentity #1483.
# THIS IS NOT leftover-safe Day kit WeatherIdentity.
# THIS IS NOT leftover-safe TheaterKitSpec WeatherIdentity #1300
# UPROPERTY clone. If a clone asserts UPROPERTY /
...
# ESkyguardMissionWeather, identifier is Weather, initializer
# is ESkyguardMissionWeather::Storm, locked decl is the plain
...
TARGET_WRONG_EQ_NONE = "ESkyguardMissionWeather Weather = NAME_None;"
TARGET_WRONG_FALSE = "ESkyguardMissionWeather Weather = false;"
TARGET_WRONG_TRUE = "ESkyguardMissionWeather Weather = true;"
TARGET_WRONG_ZERO = "ESkyguardMissionWeather Weather = 0.f;"
TARGET_WRONG_FLOAT = "float Weather;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_BOOL = "bool Weather;"
TARGET_WRONG_INT = "int32 Weather;"
...
WEATHER_DECL_RE = re.compile(
    r"ESkyguardMissionWeather\s+Weather\s*=\s*"
    r"ESkyguardMissionWeather::Storm\s*;"
)
WEATHER_INIT_RE = re.compile(
    r"ESkyguardMissionWeather\s+Weather\s*=\s*([^;]+);"
)
INVENTED_FIELD_META = (
    "INDEX_NONE",
...
def leftover_wrong_weather_inits() -> tuple[str, ...]:
    return (
        TARGET_WRONG_BARE,
        TARGET_WRONG_CLEAR,
        TARGET_WRONG_OVERCAST,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardMissionWeather Weather =
    # ESkyguardMissionWeather::Storm;`.
    # Do not accept bare Weather, leftover Clear / Rain /
    # Overcast / Night*, leftover FName WeatherIdentity,
    # leftover WeatherLabel, leftover profile #1353
    # UPROPERTY wrap, `= NAME_None` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` when origin/main locks Storm.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if WEATHER_DECL_RE.search(compact) is None:
        return False
    if re.search(
        ...
        return False
    if re.search(r"float\s+Weather\b", compact):
        return False
    if re.search(r"FName\s+Weather\b", compact):
        return False
    if re.search(
    ...
    if re.search(
        ...
        return False
    for match in WEATHER_INIT_RE.finditer(compact):
        init = collapsed(match.group(1))
        if init != "ESkyguardMissionWeather::Storm":
            return False
    return True
...
def leftover_weather_profile_struct_def(region: str):
    return re.search(
        r"struct\s+FSkyguardWeatherProfile\b",
        region,
    )
...
def spec_section(header: str) -> str:
    ...
    leftover_profile = leftover_weather_profile_struct_def(section)
    if leftover_profile is not None:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes leftover "
            f"{LEFTOVER_WEATHER_PROFILE}"
        )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardMissionWeather\s+Weather\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardMissionWeather Weather is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*ESkyguardMissionWeather\s+Weather\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on ESkyguardMissionWeather Weather is not "
            f"the locked decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "Weather"):
        raise AssertionError(
            "UPROPERTY clone landed on Weather; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "Weather"):
            raise AssertionError(
                f"{token} clone landed on Weather; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class StormRainBeatKitWeatherFieldDeclContractTests(unittest.TestCase):
    def test_storm_rain_beat_kit_struct_exists(self) -> None:
        ...
        self.assertIsNone(
            leftover_weather_profile_struct_def(section),
            section,
        )
        self.assertIsNone(leftover_weather_profile_struct_def(body), body)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertIn("= ESkyguardMissionWeather::Storm", LOCKED_DECL)
        self.assertNotIn("= ESkyguardMissionWeather::Clear", LOCKED_DECL)
        self.assertNotIn("WeatherIdentity", LOCKED_DECL)
        self.assertNotIn("WeatherLabel", LOCKED_DECL)
    ...
    def test_does_not_claim_leftover_day_or_night_weather_identity(
        self,
    ) -> None:
    ...
    def test_does_not_claim_leftover_weather_identity_or_label(
        self,
    ) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_IDENTITY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LABEL)
        ...
        self.assertTrue(has_identifier(LOCKED_DECL, "Weather"))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_WEATHER_IDENTITY),
            section,
        )
        self.assertTrue(
    ...
    def test_does_not_claim_leftover_weather_profile_weather(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLEAR)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_PROFILE_HEADER)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_WEATHER_PROFILE)
        self.assertNotIn("UPROPERTY", LOCKED_DECL)
        ...
        self.assertIn(TARGET_WRONG_CLEAR, profile_header)
    ...
    def test_missing_weather_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tstatic constexpr int32 {SIBLING_BEAT_COUNT} = 7;\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        section = spec_section(leftover_clear)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Weather", str(raised.exception))
        origin = spec_section(origin_main_header())
    ...
    def test_weather_declaration_matches_origin_main(self) -> None:
        ...
        for wrong in leftover_wrong_weather_inits():
            self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_CLEAR}\n", LOCKED_DECL)
        self.assertIn("Weather", str(raised.exception))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_IDENTITY}\n", LOCKED_DECL)
        self.assertIn("Weather", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {LEFTOVER_NIGHT_KEEP_THERMAL} = true;\n",
            f"\t{LEFTOVER_DAY_BEAT} {LEFTOVER_DAY_BEATS}[7];\n",
            "\tESkyguardMissionWeather Weathers = "
            "ESkyguardMissionWeather::Storm;\n",
            "\tint32 Weather;\n",
            "\tbool Weather;\n",
            "\tFName Approach;\n",
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        self.assertFalse(
            has_declaration("\tFName WeatherIdentity;\n", LOCKED_DECL)
        )
    ...
    def test_does_not_parse_leftover_kind_enum_or_neighbors(self) -> None:
        ...
        self.assertIsNone(leftover_harbor_kit_struct_def(section), section)
        self.assertIsNone(
            leftover_weather_profile_struct_def(header),
            header,
        )
        self.assertIsNone(
            leftover_weather_profile_struct_def(section),
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "storm-rain-beat-kit Weather field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weather_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(TARGET_WRONG_IDENTITY, locked_only)
        self.assertNotIn(TARGET_WRONG_LABEL, locked_only)
        self.assertNotIn(TARGET_WRONG_CLEAR, locked_only)
