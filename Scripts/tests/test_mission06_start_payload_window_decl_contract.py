...
# INDEX_NONE or lock StartPayloadWindow
# construction in the .cpp. This is leftover-safe
# Mission06 Payload StartPayloadWindow on
# ASkyguardMission06IntegrationDirector. It is
# NOT leftover payload-window-runtime-defaults
# #f114, NOT leftover airfield-target enum #14d2,
# NOT leftover airfield-target-runtime-defaults
# #6ad8, NOT leftover Mission06 wave-state enum
# #fa65, NOT leftover Mission05 StartNextWave
# #668, NOT leftover Mission03 / Mission04
# StartNextWave, NOT leftover Mission01 /
...
# `float WindowSeconds);`); accept that
...
# StartPayloadWindow method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission06IntegrationDirector::StartPayloadWindow",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission06StartPayloadWindowDeclContractTests(unittest.TestCase):
    def test_mission06_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, START_PAYLOAD_WINDOW),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, START_PAYLOAD_WINDOW)
        self.assertIn("StartPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, START_PAYLOAD_WINDOW)
    ...
    def test_missing_initialize_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, START_PAYLOAD_WINDOW)
        self.assertIn("StartPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, START_PAYLOAD_WINDOW)
        self.assertIn("StartPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", START_PAYLOAD_WINDOW)
        self.assertNotIn("BlueprintCallable", START_PAYLOAD_WINDOW)
        self.assertNotIn("Category", START_PAYLOAD_WINDOW)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, START_PAYLOAD_WINDOW)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, START_PAYLOAD_WINDOW)
        self.assertIn("StartPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid StartPayloadWindow("
            "ESkyguardAirfieldTarget Target, "
            "float WindowSeconds);\n"
        )
        as_const = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, START_PAYLOAD_WINDOW)
            self.assertIn("StartPayloadWindow", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_initialize_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, START_PAYLOAD_WINDOW))
        self.assertEqual(
            declaration_count(section, START_PAYLOAD_WINDOW),
            1,
        )
        self.assertTrue(
            START_PAYLOAD_WINDOW.startswith("bool "),
            START_PAYLOAD_WINDOW,
        )
        self.assertTrue(
            START_PAYLOAD_WINDOW.endswith(";"),
            START_PAYLOAD_WINDOW,
        )
        self.assertIn("StartPayloadWindow", START_PAYLOAD_WINDOW)
        self.assertIn("ESkyguardAirfieldTarget Target", START_PAYLOAD_WINDOW)
        self.assertIn("float WindowSeconds", START_PAYLOAD_WINDOW)
        self.assertNotIn("TArray<", START_PAYLOAD_WINDOW)
        self.assertNotIn("TObjectPtr", START_PAYLOAD_WINDOW)
        self.assertNotIn("TSoftObjectPtr", START_PAYLOAD_WINDOW)
        self.assertNotIn("INDEX_NONE", START_PAYLOAD_WINDOW)
        self.assertNotIn("UFUNCTION", START_PAYLOAD_WINDOW)
        self.assertNotIn("{", START_PAYLOAD_WINDOW)
        self.assertNotIn("}", START_PAYLOAD_WINDOW)
        self.assertNotIn("return ", START_PAYLOAD_WINDOW)
        self.assertNotIn("const;", START_PAYLOAD_WINDOW)
        self.assertNotIn("Root", START_PAYLOAD_WINDOW)
        self.assertNotIn("Briefing", START_PAYLOAD_WINDOW)
        self.assertNotIn("AudioDirector", START_PAYLOAD_WINDOW)
        self.assertNotIn("RadioChatter", START_PAYLOAD_WINDOW)
        self.assertNotIn("SortiePresentation", START_PAYLOAD_WINDOW)
        self.assertNotIn("CampaignDefinition", START_PAYLOAD_WINDOW)
        self.assertNotIn("MissionDefinition", START_PAYLOAD_WINDOW)
        self.assertNotIn("Readiness", START_PAYLOAD_WINDOW)
        self.assertNotIn("bAutoInitialize", START_PAYLOAD_WINDOW)
        self.assertNotIn("bAllowBoundedActorSpawning", START_PAYLOAD_WINDOW)
        self.assertNotIn("bAutoLaunchAfterBriefing", START_PAYLOAD_WINDOW)
        self.assertNotIn("PathfinderSpawnLocation", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetAircraft", START_PAYLOAD_WINDOW)
        self.assertNotIn("BindRuntimeActors", START_PAYLOAD_WINDOW)
        self.assertNotIn("HandleDroneCityImpact", START_PAYLOAD_WINDOW)
        self.assertNotIn("InitializePlayableMission", START_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyThreatDestroyed", START_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceConvoyByDistance", START_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyConvoyDamage", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetWaveState", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetConvoyRouteState", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatKit", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetNightBeatKit", START_PAYLOAD_WINDOW)
        self.assertNotIn("StartSearchlightWindow", START_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceSearchlightTrack", START_PAYLOAD_WINDOW)
        self.assertNotIn("NotifySubstationDamage", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetRemainingThreatsInWave", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetSearchlightRuntime", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetSubstationIntegrity", START_PAYLOAD_WINDOW)
        self.assertNotIn("StartNextWave", START_PAYLOAD_WINDOW)
        self.assertNotIn("AdvancePayloadWindow", START_PAYLOAD_WINDOW)
        self.assertNotIn("TryJamActivePayload", START_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyAirfieldTargetDamage", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetPayloadWindow", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetTargetRuntime", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetSurvivingTargetCount", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatKind", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatIndex", START_PAYLOAD_WINDOW)
        self.assertNotIn("TickDayBeatKit", START_PAYLOAD_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", START_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", START_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", START_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardMission06WaveState", START_PAYLOAD_WINDOW)
        self.assertIn("ESkyguardAirfieldTarget", START_PAYLOAD_WINDOW)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", START_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", START_PAYLOAD_WINDOW)
        self.assertNotIn("ConvoyRuntimeAnchor", START_PAYLOAD_WINDOW)
        self.assertNotIn("RoadHunterSpawnLocation", START_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", START_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", START_PAYLOAD_WINDOW)
        self.assertNotIn("ConfigureMissionDefinition", START_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyObjectiveProgress", START_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyProtectedAssetFailed", START_PAYLOAD_WINDOW)
        self.assertNotIn("SynchronizeRuntimeState", START_PAYLOAD_WINDOW)
        self.assertNotIn("IsCorePlayableReady", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetReadiness", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetObjectiveRuntime", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetGunner", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetPathfinder", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetMissionId", START_PAYLOAD_WINDOW)
        self.assertNotIn("ValidateMissionContract", START_PAYLOAD_WINDOW)
        self.assertNotIn("ConfigureFromMission", START_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceBriefing", START_PAYLOAD_WINDOW)
        self.assertNotIn("SetAssetsReady", START_PAYLOAD_WINDOW)
        self.assertNotIn("AcknowledgeAndLaunch", START_PAYLOAD_WINDOW)
        self.assertNotIn("CanLaunch", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetElapsedSeconds", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetBriefingState", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetMinimumWarmupSeconds", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetBriefingText", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetRadioChatter", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetPresentation", START_PAYLOAD_WINDOW)
        self.assertNotIn("GetMissionTitle", START_PAYLOAD_WINDOW)
        self.assertNotIn("AcknowledgeBriefing", START_PAYLOAD_WINDOW)
        self.assertNotIn("LaunchSortie", START_PAYLOAD_WINDOW)
        self.assertNotIn("HullCollider", START_PAYLOAD_WINDOW)
        self.assertNotIn("OpticalTracker", START_PAYLOAD_WINDOW)
        self.assertNotIn("WeaponServo", START_PAYLOAD_WINDOW)
        self.assertNotIn("CountermeasurePod", START_PAYLOAD_WINDOW)
        self.assertNotIn("MinHeightFromOriginCm", START_PAYLOAD_WINDOW)
        self.assertNotIn("RadarNode", START_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardMission02WaveState", START_PAYLOAD_WINDOW)
        self.assertNotIn("HarborIndustrial", START_PAYLOAD_WINDOW)
        self.assertNotIn("MaxIntegrity", START_PAYLOAD_WINDOW)
        self.assertNotIn("CurrentIntegrity", START_PAYLOAD_WINDOW)
        self.assertNotIn("FillAndFinalize", START_PAYLOAD_WINDOW)
        self.assertNotIn("FillAndFail", START_PAYLOAD_WINDOW)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, START_PAYLOAD_WINDOW)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_PAYLOAD} {START_PAYLOAD_WINDOW}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{START_PAYLOAD_WINDOW}\n}}\n"
        self.assertTrue(has_declaration(one_line, START_PAYLOAD_WINDOW))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, START_PAYLOAD_WINDOW),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{START_PAYLOAD_WINDOW}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, START_PAYLOAD_WINDOW)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", START_PAYLOAD_WINDOW)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_lock_initialize_cpp_body(self) -> None:
        ...
        self.assertNotIn("return false", START_PAYLOAD_WINDOW)
        self.assertNotIn("return true", START_PAYLOAD_WINDOW)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{START_PAYLOAD_WINDOW}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, START_PAYLOAD_WINDOW)
        self.assertIn("StartPayloadWindow", START_PAYLOAD_WINDOW)
        self.assertTrue(START_PAYLOAD_WINDOW.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, START_PAYLOAD_WINDOW)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, START_PAYLOAD_WINDOW)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", START_PAYLOAD_WINDOW)
        self.assertNotIn("CurrentIntegrity", START_PAYLOAD_WINDOW)
        self.assertNotIn("SkyguardApacheAircraft.h", START_PAYLOAD_WINDOW)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, START_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", START_PAYLOAD_WINDOW)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{START_PAYLOAD_WINDOW}\n"
        self.assertEqual(
            require_declaration(locked_only, START_PAYLOAD_WINDOW),
            START_PAYLOAD_WINDOW,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, START_PAYLOAD_WINDOW)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", START_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", START_PAYLOAD_WINDOW)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 StartPayloadWindow method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{START_PAYLOAD_WINDOW}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, START_PAYLOAD_WINDOW.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                START_PAYLOAD_WINDOW.lower(),
                "mission06 StartPayloadWindow contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, START_PAYLOAD_WINDOW)

    def test_contract_is_start_payload_window_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(START_PAYLOAD_WINDOW.startswith("bool "))
        self.assertIn("StartPayloadWindow", START_PAYLOAD_WINDOW)
        self.assertTrue(START_PAYLOAD_WINDOW.endswith(";"))
        self.assertIn(UFUNCTION_PAYLOAD, section)
    ...
    def test_sibling_director_fields_do_not_satisfy_initialize(
        ...
        for leftover in (
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, START_PAYLOAD_WINDOW)
            )
            self.assertNotEqual(START_PAYLOAD_WINDOW, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("StartPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, START_PAYLOAD_WINDOW))
        locked_only = f"{START_PAYLOAD_WINDOW}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            ...
            "\tbool StartPayloadWindow("
            "ESkyguardAirfieldTarget Target, "
            "float WindowSeconds) { return false; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_start_payload_window"
                "_decl_contract.py"
            )
        )
        self.assertIn(
