# THIS IS leftover-safe FSkyguardObjectiveDefinition ObjectiveId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName ObjectiveId;` (no initializer).
...
# Do NOT contract sibling fields DisplayName / Type /
...
# then this isolated ObjectiveId field decl, same as leftover
...
# `FName LineId` / `FName PointId`, retarget: type is
# FName ObjectiveId, no `=`, specifiers are EditAnywhere
...
TARGET_WRONG_EQ_NONE = "FName ObjectiveId = NAME_None;"
TARGET_WRONG_FALSE = "FName ObjectiveId = false;"
TARGET_WRONG_TRUE = "FName ObjectiveId = true;"
TARGET_WRONG_ZERO = "FName ObjectiveId = 0.f;"
TARGET_WRONG_FLOAT = "float ObjectiveId;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if re.search(
        r"FName\s+ObjectiveId\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+ObjectiveId\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+ObjectiveId\b", compact[index:]
        ):
...
def require_objective_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} ObjectiveId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} ObjectiveId"
        )
...
class ObjectiveDefinitionObjectiveIdFieldDeclContractTests(unittest.TestCase):
    def test_objective_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ObjectiveId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ObjectiveId"), section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_objective_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName ObjectiveId;", compact_origin)
        self.assertNotIn("ObjectiveId = NAME_None", compact_origin)
        self.assertNotIn("ObjectiveId = false", compact_origin)
        self.assertNotIn("ObjectiveId = true", compact_origin)
        self.assertNotIn("ObjectiveId = 0.f", compact_origin)
        self.assertNotIn("ObjectiveId = 160.f", compact_origin)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 ObjectiveId;\n",
            "\tbool ObjectiveId;\n",
            "\tfloat ObjectiveId = " + forty + ";\n",
            "\tfloat ObjectiveId = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_objective_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_objective_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_objective_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_objective_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_objective_id_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "objective-definition ObjectiveId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "objective_definition_objective_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
