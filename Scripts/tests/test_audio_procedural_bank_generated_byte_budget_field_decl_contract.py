# THIS IS leftover-safe unique development GeneratedByteBudget int32.
...
# then `int32 GeneratedByteBudget = 1048576;` (HAS spaces around =).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Audio|Development`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"65536"`.
# ClampMax IS `"2097152"`. Default IS `= 1048576`.
# This is a unique development GeneratedByteBudget int32.
# It is NOT leftover sibling SampleRate / bEnableDevelopmentAudition /
# private TotalGeneratedBytes, NOT leftover analog
# audio-procedural-bank-empty-fail-closed C++ bulk
# (keep bulk in LOCKED_SCRIPTS), NOT leftover analog
# audio-harness-fail-closed bulk, NOT leftover analog coastal
# unique actor UPROPERTY start-cull / end-cull / PlacementSeed /
# EpicTreeBudget / EpicShrubBudget / TreeInstances /
# ShrubInstances, NOT leftover analog Root / VFXPool /
# Readiness / RouteLengthCm / WindStrength / WindSpeed /
# Quality, NOT leftover Gunner TraceRange = 32000.f,
# NOT leftover analog audio-acceptance MinimumMeasuredSamples.
...
# Stop BEFORE private TotalGeneratedBytes as a claimed slot.
...
# (keep bulk in LOCKED_SCRIPTS).
# Do NOT parse leftover analog audio-harness-fail-closed
# (keep bulk in LOCKED_SCRIPTS).
# Do NOT parse leftover analog coastal unique actor UPROPERTY
# (keep those isolated sibling field decls in LOCKED_SCRIPTS).
...
# — RETARGET hard: Category Skyguard|Audio|Development
# wrapping `int32 GeneratedByteBudget = 1048576;` plus
# ClampMin `"65536"` ClampMax `"2097152"`.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity
# with NO initializer — RETARGET hard.
# LOCKED_DECL is `int32 GeneratedByteBudget = 1048576;` NOT
...
# `float TraceRange = 32000.f;` AS THE LOCKED DECL.
...
# `int32 VegetationStartCullDistanceCm = 8000;` /
# `int32 SampleRate = 48000;` /
# `int32 MinimumMeasuredSamples = 600;` AS THE LOCKED DECL.
...
# GeneratedByteBudget is 1048576 not Harbor 40 / 80 float tokens.
...
TARGET = "int32 GeneratedByteBudget = 1048576;"
TARGET_WRONG_BARE = "int32 GeneratedByteBudget;"
TARGET_WRONG_INIT = "int32 GeneratedByteBudget = nullptr;"
TARGET_WRONG_FALSE = "int32 GeneratedByteBudget = false;"
TARGET_WRONG_TRUE = "int32 GeneratedByteBudget = true;"
TARGET_WRONG_ZERO = "int32 GeneratedByteBudget = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GeneratedByteBudget;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GeneratedByteBudget;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> GeneratedByteBudget;"
...
UPROPERTY_EDIT_READWRITE = (
    ...
    'meta=(ClampMin="65536", ClampMax="2097152"))'
)
STOP_BEFORE_PRIVATE = "private:"
STOP_BEFORE_PUBLIC = "public:"
STOP_BEFORE_PROTECTED = "protected:"
STOP_BEFORE_LOOK_PITCH_MIN = "TotalGeneratedBytes"
STOP_BEFORE_EPIC_SHRUB = "TotalGeneratedBytes"
...
SIBLING_LOOK_PITCH_MIN = "TotalGeneratedBytes"
...
GENERATED_BYTE_BUDGET_DECL_RE = re.compile(
    r"int32\s+GeneratedByteBudget\s*=\s*1048576\s*;"
)
GENERATED_BYTE_BUDGET_BARE_RE = re.compile(
    r"int32\s+GeneratedByteBudget\s*;"
)
GENERATED_BYTE_BUDGET_WRONG_INIT_RE = re.compile(
    r"int32\s+GeneratedByteBudget\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 GeneratedByteBudget = 1048576;`.
    ...
    # TObjectPtr, leftover SampleRate = 48000,
    # leftover GeneratedByteBudget = 1048576,
    # leftover MinimumMeasuredSamples = 600, or a bare
    # GeneratedByteBudget without the 1048576 initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GENERATED_BYTE_BUDGET_DECL_RE.search(compact) is None:
        return False
    if GENERATED_BYTE_BUDGET_BARE_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GeneratedByteBudget\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 GeneratedByteBudget "
...
class AudioProceduralBankGeneratedByteBudgetFieldDeclContractTests(unittest.TestCase):
    def test_audio_procedural_bank_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GeneratedByteBudget"), section)
        ...
        self.assertTrue(has_identifier(body, "Waves"), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT), section)
        self.assertTrue(has_identifier(body, "bEnableDevelopmentAudition"), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_MPD_RIGHT_TEXT),
            section,
        )
        self.assertTrue(has_identifier(body, "SampleRate"), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_TEDAC_TEXT), section)
        self.assertTrue(has_identifier(body, "PCMData"), body)
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE), section)
        self.assertTrue(has_identifier(body, "Checksums"), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GeneratedByteBudget"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GeneratedByteBudget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_generated_byte_budget_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('Skyguard|Environment|Performance', section)
        self.assertIsNone(
        ...
        self.assertNotIn('Skyguard|Environment|Performance', specifiers)
        ...
        self.assertIn('ClampMin="65536"', specifiers)
        self.assertIn('ClampMax="2097152"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 GeneratedByteBudget = 1048576;", compact_origin)
        self.assertNotIn("GeneratedByteBudget = 160", compact_origin)
    ...
    def test_generated_byte_budget_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GeneratedByteBudget"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1048576", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 GeneratedByteBudget = 1048576;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GeneratedByteBudgets = 1048576;\n",
            "\tfloat GeneratedByteBudget = 1048576.f;\n",
            "\tbool GeneratedByteBudget = true;\n",
            "\tint32 GeneratedByteBudget = " + forty + ";\n",
            "\tint32 GeneratedByteBudget = " + eighty + ";\n",
            f"\t{TARGET_WRONG_SEED}\n",
    ...
    def test_does_not_claim_sibling_sample_rate_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "GeneratedByteBudget"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GeneratedByteBudget"))
        self.assertTrue(has_identifier(section, "GeneratedByteBudget"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertTrue(has_identifier(leaked, "Waves"))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(leaked, "SampleRate"))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, "bEnableDevelopmentAudition"))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="65536"', UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMax="2097152"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio procedural bank generated-byte-budget field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_procedural_bank_generated_byte_budget_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioProceduralBankComponent.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AUDIO_HARNESS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MIN_SAMPLES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MAX_VOICES, LOCKED_SCRIPTS)
