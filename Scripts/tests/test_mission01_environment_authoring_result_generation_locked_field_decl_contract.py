...
# bGenerationLocked. DEFAULT IS `= true` NOT `= false`.
# If a clone asserts `= false`, accept `= true` instead.
# THIS IS NOT leftover bSuccess #1235.
# THIS IS NOT leftover bLicensedMeshSlotsEmpty (sibling, also = true).
# THIS IS NOT leftover mission01-environment-authoring-result-defaults #df1a.
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bGenerationLocked FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bGenerationLocked = true; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# Fail closed if the type or decl is missing, renamed,
# Category= is added, or default is not true.
...
# leftover AuthoringResult through #1241, leftover
# MaterialCompilation through Error #1232.
# Harbor 40/80 fail-closed. Ban retired live-copy tokens via
# split tokens (b + Ya + kRuntimeReady). Stay Apache CPG
# 30 mm / Hydra / Hellfire.

# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bGenerationLocked. THIS IS NOT leftover Mission01
...
# bool bGenerationLocked = true. Fail closed if type/decl
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bGenerationLocked. THIS IS NOT leftover
# mission01-environment-authoring-result-defaults #df1a.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bGenerationLocked. THIS IS NOT leftover MaterialCompilation bSuccess #1223.
...
# Do NOT parse FSkyguardLandscapeCaptureConfigurationResult (leftover bSuccess #1214).
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bGenerationLocked. THIS IS NOT leftover VisibleAudit bSuccess #1199.
# THIS IS NOT leftover FootprintSampleResult bSuccess #1189.
# THIS IS NOT leftover landscape-capture-config-defaults #d6fd.
# THIS IS NOT leftover landscape-visible-audit-defaults #0bce.
# THIS IS NOT leftover landscape-material-compilation-defaults #01d4.
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# bGenerationLocked FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bGenerationLocked = true; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly bool bGenerationLocked = true;
...
TARGET = "bool bGenerationLocked = true;"
TARGET_WRONG = "bool bGenerationLocked = false;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bGenerationLocked (must be = true, not = false).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bGenerationLocked\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bGenerationLocked is missing from "
...
class Mission01EnvironmentAuthoringResultGenerationLockedFieldDeclContractTests(unittest.TestCase):
    def test_mission01_environment_authoring_result_struct_exists(self) -> None:
        ...
        self.assertIn("bGenerationLocked", section)
        ...
        self.assertNotIn("bValid", section.replace("bGenerationLocked", ""))
        ...
        self.assertNotIn("bSuccess", LOCKED_DECL.replace("bGenerationLocked", ""))
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bGenerationLocked", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_generation_locked_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_generation_locked_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bGenerationLocked"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bGenerationLocked", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bGenerationLocked", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bGenerationLocked"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bGenerationLocked", locked_only)
