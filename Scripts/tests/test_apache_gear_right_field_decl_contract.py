# THIS IS leftover-safe ASkyguardApacheAircraft GearRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> GearRight;`
# (no initializer).
...
# Stop BEFORE sibling PylonLeft as a claimed slot.
# Do NOT claim sibling GearNose / GearLeft / StubWingRight /
# PylonRight / HydraRight / HellfireRight / Nose /
# HorizontalTail / Canopy / Fuselage / AircraftRoot /
# SilhouetteMesh.
# THIS IS leftover-safe ASkyguardApacheAircraft.GearRight.
# It is NOT leftover GearNose (opening/in-flight), NOT leftover
# analog apache-hull-collider, NOT leftover analog
...
# GearNose / GearLeft / PylonLeft / StubWingRight / Nose /
# HorizontalTail are sibling slots being opened, not this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> GearRight;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> GearRight;` and NOT
# leftover `GearNose` / leftover `GearLeft` / leftover
# `Nose` / leftover `StubWingRight` and NOT
# `float CurrentIntegrity = 140.f;`.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# AS THE LOCKED DECL.
# FAIL-CLOSED if LOCKED_DECL is TObjectPtr<USceneComponent>.
# FAIL-CLOSED if LOCKED_DECL is leftover GearNose or leftover
# GearLeft or leftover Nose or leftover StubWingRight.
...
TARGET = "TObjectPtr<UStaticMeshComponent> GearRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> GearRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> GearRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> GearRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GearRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GearRight;"
...
STOP_BEFORE_PYLON_LEFT = "PylonLeft"
...
GEAR_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+GearRight\s*;"
)
GEAR_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+GearRight\s*="
)
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_PYLON_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_PYLON_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_PYLON_LEFT}"
        )
    for later in (
        SIBLING_PYLON_RIGHT,
        SIBLING_HYDRA_RIGHT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+GearRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> GearRight "
...
class ApacheGearRightFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GearRight"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_GEAR_NOSE), body)
        ...
        self.assertFalse(
            has_identifier(LOCKED_DECL, SIBLING_GEAR_NOSE),
            LOCKED_DECL,
        )
        self.assertFalse(
            has_identifier(LOCKED_DECL, SIBLING_GEAR_LEFT),
            LOCKED_DECL,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GearRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GearRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_gear_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> GearRight;", compact_origin)
        self.assertNotIn("GearRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_gear_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> GearRight"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> GearRight;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STUB_WING_RIGHT)
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> GearRight;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> GearRights;\n",
            "\tint32 GearRight = 140;\n",
            "\tbool GearRight = true;\n",
            "\tfloat GearRight = " + forty + ";\n",
            "\tfloat GearRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_pylon_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_PYLON_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_PYLON_RIGHT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_GEAR_NOSE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_GEAR_LEFT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_STUB_WING_RIGHT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_NOSE))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertFalse(has_identifier(section, SIBLING_PYLON_LEFT))
        self.assertTrue(has_identifier(section, "GearRight"))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_GEAR_NOSE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GearRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gear_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
