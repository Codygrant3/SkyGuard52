# THIS IS leftover-safe FSkyguardMissionDebrief::bProgressSaved.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `bool bProgressSaved = false;` with no Category=.
...
TARGET = "bool bProgressSaved = false;"
TARGET_WRONG_BARE = "bool bProgressSaved;"
TARGET_WRONG_TRUE = "bool bProgressSaved = true;"
TARGET_WRONG_ZERO = "bool bProgressSaved = 0;"
TARGET_WRONG_FLOAT = "bool bProgressSaved = 0.f;"
TARGET_WRONG_MEDAL = "bool bNewBestMedal = false;"
TARGET_WRONG_SCORE = "bool bNewBestScore = false;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # origin/main has `bool bProgressSaved = false;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"bool\s+bProgressSaved\s*=\s*true\b", compact):
        return False
    if re.search(r"bool\s+bProgressSaved\s*=\s*0(?:\.f)?\b", compact):
        return False
    if re.search(r"bool\s+bProgressSaved\s*=\s*false\b", compact) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bProgressSaved\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bProgressSaved is missing from "
...
class MissionDebriefProgressSavedFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bProgressSaved", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_progress_saved_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bProgressSaved = false", collapsed(origin))
        self.assertNotIn("bProgressSaved = true", collapsed(origin))
        self.assertNotIn("bProgressSaved = 0", collapsed(origin))

    def test_progress_saved_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bProgressSaved"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            "\tint32 bProgressSaved = false;\n",
            "\tbool bProgressSaveds = false;\n",
            "\tbool ProgressSaved = false;\n",
            "\tFText bProgressSaved = false;\n",
            "\tbool bProgressSaved{};\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief bProgressSaved field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_progress_saved_field_decl_contract.py"
        ))
