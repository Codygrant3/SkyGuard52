# THIS IS leftover-safe FSkyguardStormRainBeatKit Threats.
...
# threats bulk. NOT leftover Night/Day Beat.Threat
# isolated.
...
# bHydraForClusters / Kinds / Stations / Calls.
# Fail-closed if this lane claims leftover singular
# Night/Day Beat.Threat instead of Threats[BeatCount].
# THIS IS NOT leftover analog storm-rain-beat-kit-threats
# #257 / #0132.
...
# THIS IS NOT leftover Night Beat Threat isolated #1477.
# THIS IS NOT leftover Day Beat Threat isolated #1481.
# THIS IS NOT leftover Night kit Beats array #c332.
# THIS IS NOT leftover Day kit Beats array.
# THIS IS NOT leftover-safe StormRain Hydra isolated.
...
# leftover Night `Beats[7]` / leftover singular Threat /
# invented `Threats[7]`, retarget: type is
# ESkyguardThreatKind, identifier is Threats, extent is
...
TARGET = "ESkyguardThreatKind Threats[BeatCount] = {};"
TARGET_WRONG_BARE = "ESkyguardThreatKind Threats[BeatCount];"
TARGET_WRONG_BOUND_SEVEN = "ESkyguardThreatKind Threats[7] = {};"
TARGET_WRONG_BOUND_SIX = "ESkyguardThreatKind Threats[6] = {};"
TARGET_WRONG_BOUND_EIGHT = "ESkyguardThreatKind Threats[8] = {};"
TARGET_WRONG_ZERO = "ESkyguardThreatKind Threats[BeatCount] = 0;"
TARGET_WRONG_ZERO_F = "ESkyguardThreatKind Threats[BeatCount] = 0.f;"
TARGET_WRONG_BRACES_ZERO = "ESkyguardThreatKind Threats[BeatCount] = {0};"
TARGET_WRONG_FALSE = "ESkyguardThreatKind Threats[BeatCount] = false;"
TARGET_WRONG_TRUE = "ESkyguardThreatKind Threats[BeatCount] = true;"
TARGET_WRONG_KINDS_TYPE = (
    "ESkyguardStormRainBeatKind Threats[BeatCount] = {};"
)
TARGET_WRONG_STATIONS_TYPE = (
    "ESkyguardGunshipWeapon Threats[BeatCount] = {};"
)
TARGET_WRONG_CALLS_TYPE = "const TCHAR* Threats[BeatCount] = {};"
TARGET_WRONG_KINDS_ID = "ESkyguardThreatKind Kinds[BeatCount] = {};"
TARGET_WRONG_STATIONS_ID = "ESkyguardThreatKind Stations[BeatCount] = {};"
TARGET_WRONG_CALLS_ID = "ESkyguardThreatKind Calls[BeatCount] = {};"
TARGET_WRONG_BOOL = "bool Threats[BeatCount] = {};"
TARGET_WRONG_INT = "int32 Threats[BeatCount] = {};"
TARGET_WRONG_FLOAT = "float Threats[BeatCount] = {};"
TARGET_WRONG_FNAME = "FName Threats[BeatCount] = {};"
TARGET_WRONG_TARRAY = "TArray<ESkyguardThreatKind> Threats;"
TARGET_WRONG_SCALAR = "ESkyguardThreatKind Threats;"
...
THREATS_DECL_RE = re.compile(
    r"ESkyguardThreatKind\s+Threats\s*\[\s*BeatCount\s*\]\s*=\s*\{\s*\}\s*;"
)
THREATS_INIT_RE = re.compile(
    r"ESkyguardThreatKind\s+Threats\s*\[\s*BeatCount\s*\]\s*=\s*([^;]+);"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover Kinds / Stations / Calls types, leftover
    # Night/Day Beat.Threat, leftover Night/Day Beats[7],
    # or leftover sibling identifiers as the field name.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if THREATS_DECL_RE.search(compact) is None:
        return False
    if re.search(r"ESkyguardThreatKind\s+Threats\s*\[\s*7\s*\]", compact):
        return False
    if re.search(r"ESkyguardThreatKind\s+Threats\s*\[\s*6\s*\]", compact):
        return False
    if re.search(r"ESkyguardThreatKind\s+Threats\s*\[\s*8\s*\]", compact):
        return False
    if re.search(
        r"ESkyguardStormRainBeatKind\s+Threats\b",
        compact,
    ):
        return False
    if re.search(r"ESkyguardGunshipWeapon\s+Threats\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*Threats\b", compact):
        return False
    if re.search(r"TArray\s*<\s*ESkyguardThreatKind\s*>\s+Threats\b", compact):
        return False
    if re.search(
        r"ESkyguardThreatKind\s+Threats\s*\[\s*BeatCount\s*\]\s*;",
        compact,
    ) and not re.search(
        r"ESkyguardThreatKind\s+Threats\s*\[\s*BeatCount\s*\]\s*=",
        compact,
        ...
        return False
    for match in THREATS_INIT_RE.finditer(compact):
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardThreatKind Threats\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardThreatKind Threats is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*ESkyguardThreatKind Threats\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on ESkyguardThreatKind Threats is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "Threats"):
        raise AssertionError(
            "UPROPERTY clone landed on Threats; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "Threats"):
            raise AssertionError(
                f"{token} clone landed on Threats; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class StormRainBeatKitThreatsFieldDeclContractTests(unittest.TestCase):
    def test_storm_rain_beat_kit_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Threats"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("ESkyguardThreatKind Threats"))
        ...
        self.assertIn("= {}", LOCKED_DECL)
        self.assertIn("Threats", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_NIGHT_BEAT_THREAT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_DAY_BEAT_THREAT)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("Threats", str(raised.exception))
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Threats"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_threats_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_title_does_not_satisfy_threats(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_TITLE}\n"
    ...
    def test_leftover_kinds_stations_calls_do_not_satisfy(self) -> None:
        leftovers = (
            TARGET_WRONG_KINDS_ID,
            TARGET_WRONG_STATIONS_ID,
            TARGET_WRONG_CALLS_ID,
            TARGET_WRONG_KINDS_TYPE,
            TARGET_WRONG_STATIONS_TYPE,
            TARGET_WRONG_CALLS_TYPE,
        )
    ...
    def test_initializer_must_be_empty_braces(self) -> None:
        ...
        self.assertNotIn("Threats[7] = {}", compact_origin)
        self.assertNotIn("Threats[BeatCount] = false", compact_origin)
        self.assertNotIn("Threats[BeatCount] = true", compact_origin)
        self.assertNotIn("Threats[BeatCount] = 0.f", compact_origin)
        self.assertNotIn("Threats[BeatCount] = {0}", compact_origin)
        self.assertNotIn(
            "ESkyguardThreatKind Threats[BeatCount];",
    ...
    def test_threats_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardThreatKind Threats"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tESkyguardThreatKind Threat[BeatCount] = {};\n",
            "\tESkyguardThreatKind Threatss[BeatCount] = {};\n",
            "\tfloat Threats[BeatCount] = " + forty + ";\n",
            "\tfloat Threats[BeatCount] = " + eighty + ";\n",
            "\tESkyguardThreatKind Threats[BeatCount] = " + forty + ";\n",
            "\tESkyguardThreatKind Threats[BeatCount] = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "storm-rain-beat-kit Threats field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "threats_field_decl_contract.py"
        ))
        self.assertIn("storm_rain_beat_kit", Path(__file__).name)
        self.assertNotIn("keep_thermal", Path(__file__).name)
        self.assertNotIn("night_sortie_beat_threat", Path(__file__).name)
        self.assertNotIn("day_sortie_beat_threat", Path(__file__).name)
        self.assertNotIn("SkyguardStormRainBeatKit.cpp", THIS_SCRIPT)
