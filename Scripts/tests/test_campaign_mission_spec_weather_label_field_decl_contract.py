# THIS IS leftover-safe FSkyguardCampaignMissionSpec WeatherLabel.
# origin/main form: plain C++ field `const TCHAR* WeatherLabel = TEXT("");`
...
# Do NOT parse leftover `FSkyguardStormRainBeatKit` WeatherLabel
# leftover-safe #1493. Same TCHAR* TEXT("") must not steal
# that leftover parse window.
# Do NOT parse leftover analog storm-rain-beat-kit-labels
# #261 / #b557 as this slot.
...
# Fail-closed if identifier is Title / Brief / Success /
# Failure instead of WeatherLabel.
# Fail-closed if leftover CampaignMissionSpec Title #1522 /
# Brief #1524 / Success #1523 / Failure are treated as
# this slot.
...
# leftover StormRainBeatKit WeatherLabel, retarget: type is
# const TCHAR*, identifier is WeatherLabel, initializer is
...
TARGET = 'const TCHAR* WeatherLabel = TEXT("");'
TARGET_WRONG_BARE = "const TCHAR* WeatherLabel;"
TARGET_WRONG_EQ_NONE = "const TCHAR* WeatherLabel = NAME_None;"
TARGET_WRONG_FALSE = "const TCHAR* WeatherLabel = false;"
TARGET_WRONG_TRUE = "const TCHAR* WeatherLabel = true;"
TARGET_WRONG_ZERO = "const TCHAR* WeatherLabel = 0.f;"
TARGET_WRONG_FLOAT = "float WeatherLabel;"
TARGET_WRONG_HEALTH = "float WeatherLabel = 160.f;"
TARGET_WRONG_BOOL = "bool WeatherLabel;"
TARGET_WRONG_INT = "int32 WeatherLabel;"
TARGET_WRONG_FNAME = "FName WeatherLabel;"
TARGET_WRONG_TEXT = 'const TCHAR* WeatherLabel = TEXT("Storm");'
...
CLONE_FNAME_WEATHER_IDENTITY = "FName WeatherIdentity;"
...
LEFTOVER_STORM_RAIN_WEATHER_LABEL = (
...
LEFTOVER_CAMPAIGN_SPEC_SUCCESS = (
...
WEATHER_LABEL_DECL_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*WeatherLabel\s*=\s*TEXT\s*\(\s*\"\"\s*\)\s*;"
)
WEATHER_LABEL_INIT_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*WeatherLabel\s*=\s*([^;]+);"
)
SIBLING_TCHAR_IDENTIFIERS = (
    SIBLING_TITLE,
    SIBLING_BRIEF,
    SIBLING_SUCCESS,
    SIBLING_FAILURE,
...
def leftover_storm_rain_weather_label_tokens() -> tuple[str, ...]:
    return (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `const TCHAR* WeatherLabel = TEXT("");`.
    # Do not accept missing TEXT(""), Title, Brief, Success,
    # Failure, FName WeatherLabel, `= NAME_None` /
    # `= false` / `= true` / `= 0.f` / `= 160.f` when
    # origin/main locks TEXT(""). Do not accept leftover
    # StormRainBeatKit WeatherLabel as this CampaignMissionSpec
    # parse window.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if WEATHER_LABEL_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+WeatherLabel\b", compact):
        return False
    if re.search(r"FName\s+WeatherLabel\b", compact):
        return False
    for sibling in SIBLING_TCHAR_IDENTIFIERS:
        if (
            re.search(
                rf"const\s+TCHAR\s*\*\s*{re.escape(sibling)}\b",
                compact,
            )
            and not has_identifier(compact, "WeatherLabel")
        ):
            return False
    for match in WEATHER_LABEL_INIT_RE.finditer(compact):
...
def spec_section(header: str) -> str:
    ...
    if LEFTOVER_STORM_RAIN_KIT in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window is leftover "
            f"{LEFTOVER_STORM_RAIN_KIT}, not this WeatherLabel slot"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*const TCHAR\* WeatherLabel\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for const TCHAR* WeatherLabel is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*const TCHAR\* WeatherLabel\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on const TCHAR* WeatherLabel is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "WeatherLabel"):
        raise AssertionError(
            "UPROPERTY clone landed on WeatherLabel; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "WeatherLabel"):
            raise AssertionError(
                f"{token} clone landed on WeatherLabel; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CampaignMissionSpecWeatherLabelFieldDeclContractTests(unittest.TestCase):
    def test_campaign_mission_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "WeatherLabel"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("const TCHAR* WeatherLabel"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn('TEXT("")', LOCKED_DECL)
        self.assertTrue(has_identifier(LOCKED_DECL, "WeatherLabel"))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_TITLE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_BRIEF))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SUCCESS))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_FAILURE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_WEATHER))
        self.assertFalse(
            has_identifier(LOCKED_DECL, SIBLING_WEATHER_IDENTITY)
        )
        ...
        self.assertNotEqual(LOCKED_DECL, CLONE_FNAME_WEATHER_IDENTITY)
    ...
    def test_leftover_storm_rain_weather_label_does_not_satisfy(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_STORM_RAIN_KIT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_STORM_RAIN_WEATHER_LABEL)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_ANALOG_STORM_KIT_LABELS)
        self.assertNotEqual(STOP_BEFORE_NAMESPACE, STOP_BEFORE_STORM_RAIN_NS)
        self.assertNotIn("StormRainBeatKit", HEADER_PATH)
        self.assertNotIn("storm_rain_beat_kit_weather_label", THIS_SCRIPT)
        self.assertNotIn("storm_rain_beat_kit_labels", THIS_SCRIPT)
    ...
    def test_does_not_claim_leftover_storm_rain_or_neighbors(self) -> None:
        ...
        self.assertIn(CLONE_FNAME_WEATHER_IDENTITY, night_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "WeatherLabel"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_missing_weather_label_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_tchar_fields_do_not_satisfy_weather_label(self) -> None:
        leftovers = (
            TARGET_WRONG_TITLE,
            TARGET_WRONG_BRIEF,
            TARGET_WRONG_SUCCESS,
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SUCCESS))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_FAILURE))
        for script in leftover_campaign_spec_sibling_scripts():
    ...
    def test_initializer_must_be_empty_text(self) -> None:
        ...
        self.assertNotIn("WeatherLabel = NAME_None", compact_origin)
        self.assertNotIn("WeatherLabel = false", compact_origin)
        self.assertNotIn("WeatherLabel = true", compact_origin)
        self.assertNotIn("WeatherLabel = 0.f", compact_origin)
        self.assertNotIn("WeatherLabel = 160.f", compact_origin)
        self.assertNotIn('WeatherLabel = TEXT("Storm")', compact_origin)

    def test_weather_label_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("const TCHAR* WeatherLabel"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tconst TCHAR* WeatherLabels = TEXT(\"\");\n",
            "\tint32 WeatherLabel;\n",
            "\tbool WeatherLabel;\n",
            "\tfloat WeatherLabel = " + forty + ";\n",
            "\tfloat WeatherLabel = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        self.assertFalse(
            has_declaration(
                f"\t{CLONE_FNAME_WEATHER_IDENTITY}\n",
                LOCKED_DECL,
            )
        )
    ...
    def test_does_not_contract_sibling_spec_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_SUCCESS))
        self.assertTrue(has_identifier(section, SIBLING_FAILURE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec WeatherLabel field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weather_label_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_STORM_RAIN_WEATHER_LABEL, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_CAMPAIGN_SPEC_SUCCESS, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_BRIEF))
        self.assertFalse(has_identifier(locked_only, SIBLING_SUCCESS))
        self.assertFalse(has_identifier(locked_only, SIBLING_FAILURE))
        for harbor in leftover_harbor_beat_tokens():
            self.assertNotIn(harbor, locked_only)
        for helper in namespace_helper_tokens():
            self.assertNotIn(helper, locked_only)
        for leftover in leftover_storm_rain_weather_label_tokens():
