# THIS IS leftover-safe FSkyguardBriefingCard::Title FIELD
# (declaration-only). origin/main form is split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly) then BARE
# `FText Title;` with no initializer and no Category=.
...
# Accept bare `FText Title;`. Fail-closed if type/decl
# missing, renamed, or Category= added.
# Harbor 40/80 fail-closed in this briefing-card test file.
...
# through #1250, leftover MaterialCompilation through
# Error #1232.
...
TARGET = "FText Title;"
TARGET_WRONG = "FText Title = FText();"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Title (must be a bare FText Title; with no initializer).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FText\s+Title\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FText Title is missing from "
...
class BriefingCardTitleFieldDeclContractTests(unittest.TestCase):
    def test_briefing_card_struct_exists(self) -> None:
        ...
        self.assertIn("FText Title;", section)
        self.assertNotIn("FText Title =", section)
    ...
    def test_title_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText Title"),
            LOCKED_DECL,
        )
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn("Title", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FText Title"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("Title", locked_only)
        self.assertIn("FText Title;", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "briefing-card Title field contract "
        ...
        for banned in ("Ig" + "la", "Ri" + "fle", "Ya" + "k"):
            self.assertNotIn(
                banned,
                file_text,
                "briefing-card Title field contract contains a live "
