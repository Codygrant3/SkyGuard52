...
# then BARE `FSkyguardProductionAudioAudit EvaluateReadiness() const;`
...
# const IS present. Return type IS FSkyguardProductionAudioAudit.
# Declaration-only.
...
# return type is not FSkyguardProductionAudioAudit, or if
...
# GetUnboundRequiredCategories as this slot
# (leave them uncontracted).
# Do NOT claim leftover unique
# EvaluateReadiness /
...
# InitializeRequiredEntries / ConfigureRoutingTopology.
# Skip leftover analog GetUnboundRequiredCategories.
...
# EvaluateReadiness, return FSkyguardProductionAudioAudit.
...
TARGET = "FSkyguardProductionAudioAudit EvaluateReadiness() const;"
TARGET_INLINE = (
    "FSkyguardProductionAudioAudit EvaluateReadiness() const { return false; }"
)
TARGET_WRONG_BARE = "FSkyguardProductionAudioAudit EvaluateReadiness();"
TARGET_WRONG_NO_CONST = (
    "FSkyguardProductionAudioAudit EvaluateReadiness() { return false; }"
)
TARGET_WRONG_VOID = "void EvaluateReadiness() const;"
TARGET_WRONG_FLOAT = "float EvaluateReadiness() const;"
TARGET_WRONG_INT = "int32 EvaluateReadiness() const;"
...
TARGET_WRONG_BOOL_RETURN = "bool EvaluateReadiness() const;"
...
TARGET_WRONG_TRUE = "FSkyguardProductionAudioAudit EvaluateReadiness = true;"
TARGET_WRONG_ZERO = "FSkyguardProductionAudioAudit EvaluateReadiness = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bFSkyguardProductionAudioAudit\s+EvaluateReadiness\s*\(\s*\)\s*const\s*(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+EvaluateReadiness\b", compact):
        return False
    if re.search(r"\bfloat\s+EvaluateReadiness\b", compact):
        return False
    if re.search(r"\bint32\s+EvaluateReadiness\b", compact):
        return False
    if re.search(r"\bbool\s+EvaluateReadiness\b", compact):
        return False
    if re.search(r"\bAreResolvedProductionLoopRoutesComplete\b", compact):
        return False
    if re.search(
        r"\bFSkyguardProductionAudioAudit\s+EvaluateReadiness\s*\(\s*[^)]+\s*\)",
        compact,
    ):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FSkyguardProductionAudioAudit\s+EvaluateReadiness\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for FSkyguardProductionAudioAudit EvaluateReadiness "
...
class AudioProductionBankEvaluateReadinessDeclContractTests(
    unittest.TestCase
):
    def test_audio_production_bank_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "EvaluateReadiness"),
            section,
        )
    ...
    def test_missing_evaluate_readiness_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "FSkyguardProductionAudioAudit EvaluateReadiness();",
    ...
    def test_evaluate_readiness_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertIn("FSkyguardProductionAudioAudit ", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tFSkyguardProductionAudioAudit EvaluateReadinesss() const;\n",
            "\tint32 EvaluateReadiness = 1;\n",
            "\tFName EvaluateReadiness = NAME_None;\n",
            "\tFSkyguardProductionAudioAudit EvaluateReadiness() const = "
            + forty
            + ";\n",
            "\tFSkyguardProductionAudioAudit EvaluateReadiness() const = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "EvaluateReadiness")
        )
        self.assertTrue(
            has_identifier(section, "EvaluateReadiness")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio production bank evaluate readiness "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_production_bank_evaluate_readiness"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
