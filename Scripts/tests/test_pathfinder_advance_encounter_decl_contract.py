...
# or lock the AdvanceEncounter body in the .cpp.
...
# shake, and leftover mission-weather enum stay
# sibling-only.
ADVANCE_ENCOUNTER = "void AdvanceEncounter(float DeltaSeconds);"
UFUNCTION_ENCOUNTER = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Boss|Encounter")'
)
# Leftover #56–#64 plus PathfinderEncounterController
# production files. This lane only adds an isolated
# Python AdvanceEncounter declaration contract. Stay
# off ResetEncounterState, IsRouteStateSafe,
...
# briefing declaration contracts, leftover
...
# declaration contracts through
# GetMinimumWarmupSeconds / GetBriefingText /
# GetRadioChatter, leftover mission-briefing-state
# enum, leftover radio-chatter empty-fail-closed,
# leftover campaign-roster #111, leftover
...
# here. ResetEncounterState / IsRouteStateSafe /
# GetRouteProgress / GetEffectiveSpeedMultiplier /
...
# .cpp AdvanceEncounter body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardPathfinderEncounterController::AdvanceEncounter",
    "SkyguardPathfinderEncounterController.cpp",
)
SIBLING_TYPES = (
...
class PathfinderAdvanceEncounterDeclContractTests(unittest.TestCase):
    def test_pathfinder_controller_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ADVANCE_ENCOUNTER), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ADVANCE_ENCOUNTER)
        self.assertIn("AdvanceEncounter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ADVANCE_ENCOUNTER))

    def test_missing_advance_encounter_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardPathfinderEncounterController();\n"
            "\tvoid ResetEncounterState("
            "const FTransform& NewRouteOrigin);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ADVANCE_ENCOUNTER)
        self.assertIn("AdvanceEncounter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ADVANCE_ENCOUNTER)
        self.assertIn("AdvanceEncounter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ADVANCE_ENCOUNTER),
            section,
        )
        self.assertNotIn("BlueprintPure", ADVANCE_ENCOUNTER)
        self.assertNotIn("UFUNCTION", ADVANCE_ENCOUNTER)
        self.assertNotIn("Category", ADVANCE_ENCOUNTER)
        self.assertNotIn("BlueprintCallable", ADVANCE_ENCOUNTER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ADVANCE_ENCOUNTER)
        self.assertIn("AdvanceEncounter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = "\tvoid AdvanceEncounter();\n"
        wrong_return = "\tbool AdvanceEncounter(float DeltaSeconds);\n"
        added_const = "\tvoid AdvanceEncounter(float DeltaSeconds) const;\n"
        wrong_type = "\tvoid AdvanceEncounter(double DeltaSeconds);\n"
        int_type = "\tvoid AdvanceEncounter(int32 DeltaSeconds);\n"
        for region in (
            missing_arg,
            wrong_return,
            added_const,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ADVANCE_ENCOUNTER)
            self.assertIn("AdvanceEncounter", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_advance_encounter_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ADVANCE_ENCOUNTER),
            ADVANCE_ENCOUNTER,
        )
        self.assertTrue(has_declaration(section, ADVANCE_ENCOUNTER))
        self.assertEqual(declaration_count(section, ADVANCE_ENCOUNTER), 1)
        self.assertTrue(
            ADVANCE_ENCOUNTER.startswith("void "),
            ADVANCE_ENCOUNTER,
        )
        self.assertTrue(ADVANCE_ENCOUNTER.endswith(";"), ADVANCE_ENCOUNTER)
        self.assertIn("float DeltaSeconds", ADVANCE_ENCOUNTER)
        self.assertNotIn("INDEX_NONE", ADVANCE_ENCOUNTER)
        self.assertNotIn("{", ADVANCE_ENCOUNTER)
        self.assertNotIn("}", ADVANCE_ENCOUNTER)
        self.assertNotIn("return ", ADVANCE_ENCOUNTER)
        self.assertNotIn(" const", ADVANCE_ENCOUNTER)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tAdvanceEncounter(float DeltaSeconds);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, ADVANCE_ENCOUNTER),
                1,
            )
        one_line = f"{{\npublic:\n\t{ADVANCE_ENCOUNTER}\n}}\n"
        self.assertTrue(has_declaration(one_line, ADVANCE_ENCOUNTER))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADVANCE_ENCOUNTER),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", ADVANCE_ENCOUNTER)
        self.assertNotIn("return ", ADVANCE_ENCOUNTER)
        self.assertNotIn("RouteProgressCm +=", ADVANCE_ENCOUNTER)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{ADVANCE_ENCOUNTER}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", ADVANCE_ENCOUNTER)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", ADVANCE_ENCOUNTER)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_advance_encounter_cpp_body(self) -> None:
        locked_only = f"{ADVANCE_ENCOUNTER}\n"
        self.assertNotIn("{", ADVANCE_ENCOUNTER)
        self.assertNotIn("}", ADVANCE_ENCOUNTER)
        self.assertNotIn("return ", ADVANCE_ENCOUNTER)
        ...
        self.assertNotIn("return false", ADVANCE_ENCOUNTER)
        self.assertNotIn("return true", ADVANCE_ENCOUNTER)

    def test_contract_does_not_relock_reset_encounter_state(self) -> None:
        locked_only = f"{ADVANCE_ENCOUNTER}\n"
        for neighbor in RESET_ENCOUNTER_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_ENCOUNTER)
        self.assertNotIn("ResetEncounterState", ADVANCE_ENCOUNTER)
    ...
    def test_contract_does_not_relock_is_route_state_safe(self) -> None:
        ...
        for neighbor in IS_ROUTE_STATE_SAFE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_ENCOUNTER)
        self.assertNotIn("IsRouteStateSafe", ADVANCE_ENCOUNTER)
    ...
    def test_contract_does_not_relock_get_route_progress(self) -> None:
        ...
        for neighbor in GET_ROUTE_PROGRESS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_ENCOUNTER)
        self.assertNotIn("GetRouteProgress", ADVANCE_ENCOUNTER)
    ...
    def test_contract_does_not_relock_get_effective_speed_multiplier(
        ...
        for neighbor in GET_EFFECTIVE_SPEED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_ENCOUNTER)
        self.assertNotIn("GetEffectiveSpeedMultiplier", ADVANCE_ENCOUNTER)
    ...
    def test_contract_does_not_relock_is_attack_telegraph_active(self) -> None:
        ...
        for neighbor in IS_ATTACK_TELEGRAPH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_ENCOUNTER)
        self.assertNotIn("IsAttackTelegraphActive", ADVANCE_ENCOUNTER)
    ...
    def test_contract_does_not_relock_get_telegraphs_triggered(self) -> None:
        ...
        for neighbor in GET_TELEGRAPHS_TRIGGERED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_ENCOUNTER)
        self.assertNotIn("GetTelegraphsTriggered", ADVANCE_ENCOUNTER)
        self.assertNotIn("GetTelegraphsTriggered", locked_only)

    def test_contract_does_not_relock_uprop_fields_or_delegates(self) -> None:
        locked_only = f"{ADVANCE_ENCOUNTER}\n"
        self.assertNotIn("UPROPERTY", ADVANCE_ENCOUNTER)
        self.assertNotIn("UPROPERTY", locked_only)
        self.assertNotIn("bAutoAdvance", ADVANCE_ENCOUNTER)
        self.assertNotIn("ObservedPhase", ADVANCE_ENCOUNTER)
        self.assertNotIn("OnAttackTelegraphChanged", ADVANCE_ENCOUNTER)
        self.assertNotIn("OnAttackCommitted", ADVANCE_ENCOUNTER)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_leftover_boss_phase_enum(self) -> None:
        ...
        for token in LEFTOVER_BOSS_PHASE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ADVANCE_ENCOUNTER)
        self.assertNotIn("ESkyguardBossPhase", ADVANCE_ENCOUNTER)
        self.assertNotIn("ESkyguardBossPhase", locked_only)
        self.assertNotIn("SkyguardBossTypes.h", ADVANCE_ENCOUNTER)
        self.assertNotIn(
            "test_boss_phase_enum_contract.py",
            ADVANCE_ENCOUNTER,
        )
    ...
    def test_contract_does_not_relock_leftover_boss_defaults(self) -> None:
        ...
        for token in LEFTOVER_BOSS_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_ENCOUNTER)
        self.assertNotIn("FSkyguardBossDefinition", ADVANCE_ENCOUNTER)
        self.assertNotIn("FSkyguardBossWeakPoint", ADVANCE_ENCOUNTER)
        self.assertNotIn(
            "test_boss_definition_defaults_contract.py",
            ADVANCE_ENCOUNTER,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_ENCOUNTER)
        self.assertNotIn("Unconfigured", ADVANCE_ENCOUNTER)
        self.assertNotIn("Warming", ADVANCE_ENCOUNTER)
        self.assertNotIn("Launched", ADVANCE_ENCOUNTER)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            ADVANCE_ENCOUNTER,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_ENCOUNTER)
        self.assertNotIn("FSkyguardBriefingCard", ADVANCE_ENCOUNTER)
        self.assertNotIn("FSkyguardBriefingRadioRow", ADVANCE_ENCOUNTER)
        self.assertNotIn("FSkyguardBriefingCard", locked_only)
        self.assertNotIn("FSkyguardBriefingRadioRow", locked_only)
        self.assertNotIn("GetMinimumWarmupSeconds", ADVANCE_ENCOUNTER)
        self.assertNotIn("GetBriefingText", ADVANCE_ENCOUNTER)
        self.assertNotIn("GetRadioChatter", ADVANCE_ENCOUNTER)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_ENCOUNTER)
        self.assertNotIn("FillResultCombatStats", ADVANCE_ENCOUNTER)
        self.assertNotIn("ASkyguardGunner", ADVANCE_ENCOUNTER)
        self.assertNotIn("FillAndFinalize", ADVANCE_ENCOUNTER)
        self.assertNotIn("FillAndFail", ADVANCE_ENCOUNTER)
        self.assertNotIn("ApplyHydraForClusters", ADVANCE_ENCOUNTER)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ADVANCE_ENCOUNTER}\n"
        self.assertEqual(
            require_declaration(locked_only, ADVANCE_ENCOUNTER),
            ADVANCE_ENCOUNTER,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder AdvanceEncounter contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, ADVANCE_ENCOUNTER.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"pathfinder AdvanceEncounter contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ADVANCE_ENCOUNTER)

    def test_contract_is_advance_encounter_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(ADVANCE_ENCOUNTER.startswith("void "))
        self.assertTrue(ADVANCE_ENCOUNTER.endswith(";"))
        self.assertIn(UFUNCTION_ENCOUNTER, section)
