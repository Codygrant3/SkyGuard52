...
def readiness_body(header: str) -> str:
    start = header.index("struct FSkyguardMissionMapReadiness")
...
class MissionMapReadinessDefaultsContractTests(unittest.TestCase):
    def test_readiness_struct_exists(self) -> None:
        header = origin_main(HEADER_NAME)
        self.assertIn("struct FSkyguardMissionMapReadiness", header)
    ...
    def test_in_class_defaults_match_origin_main(self) -> None:
        ...
        self.assertIn("bool bDefinitionValid = false;", body)
        self.assertIn("bool bRouteMatchesDefinition = false;", body)
        self.assertIn("bool bRequiredObjectivesAnchored = false;", body)
        self.assertIn("bool bLandmarksDistinct = false;", body)
        self.assertIn("bool bWeatherMatchesDefinition = false;", body)
        self.assertIn("int32 RoutePointCount = 0;", body)
        self.assertIn("int32 ObjectiveAnchorCount = 0;", body)
        self.assertIn("int32 LandmarkCount = 0;", body)
        self.assertIn("int32 MissionExclusiveLandmarkCount = 0;", body)
        self.assertIn("float RouteLengthCentimeters = 0.f;", body)
        self.assertEqual(len(defaults), 10, defaults)
    ...
    def test_public_get_readiness_returns_the_struct(self) -> None:
        ...
        self.assertIn("FSkyguardMissionMapReadiness Readiness;", header)
    ...
    def test_struct_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"FSkyguardMissionMapReadiness contains {banned}",
            )
