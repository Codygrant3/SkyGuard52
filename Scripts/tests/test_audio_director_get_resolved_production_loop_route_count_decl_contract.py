...
# (`EventDefinitions`).
# Do NOT claim sibling UFUNCTION GetWindBlend /
# GetPowerBlend / GetCruiseBlend / GetIdleBlend /
# GetActiveVoiceCount / GetTelemetry /
# GetSuppressionAmount / AdvanceAudioState /
...
# PrimeConfiguredAssets (those use
# Category="Skyguard|Audio" leftover analog, NOT
# |Production).
# Do NOT claim sibling AreResolvedProductionLoopRoutesComplete
# as this slot (leave it uncontracted).
...
# AND leftover analog GlobalVoiceLimit = 24 — RETARGET hard:
# wrap is BlueprintPure Category="Skyguard|Audio|Production"
# (NO spaces around =).
# Better leftover analog UFUNCTION BlueprintPure int32
# shape: theater-kit-get-lamp-instance-count Category
# Skyguard|Theater class ASkyguardCampaignTheaterKit
# `int32 GetLampInstanceCount() const;` — RETARGET
# Category Skyguard|Theater → Skyguard|Audio|Production,
# class → USkyguardAudioDirectorComponent, method →
# GetResolvedProductionLoopRouteCount.
...
# `int32 GlobalVoiceLimit = 24;` and NOT
# leftover OpenCockpitWindLoop and NOT leftover
...
# EngineIdleLoop / MaximumAllowedVoices = 48 /
...
TARGET = "int32 GetResolvedProductionLoopRouteCount() const;"
TARGET_WRONG_BARE = "int32 GetResolvedProductionLoopRouteCount();"
TARGET_WRONG_INIT = "int32 GetResolvedProductionLoopRouteCount() = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "int32 GetResolvedProductionLoopRouteCount() = true;"
TARGET_WRONG_ZERO = "int32 GetResolvedProductionLoopRouteCount() = 0.f;"
TARGET_WRONG_NO_CONST = "int32 GetResolvedProductionLoopRouteCount();"
TARGET_WRONG_FLOAT = "float GetResolvedProductionLoopRouteCount() const;"
TARGET_WRONG_FNAME_RET = "FName GetResolvedProductionLoopRouteCount() const;"
TARGET_WRONG_BODY = (
    "int32 GetResolvedProductionLoopRouteCount() const "
...
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GetResolvedProductionLoopRouteCount;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GetResolvedProductionLoopRouteCount;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> GetResolvedProductionLoopRouteCount;"
...
def public_section(header: str) -> str:
    ...
    if has_identifier(section, SIBLING_EVENT_DEFINITIONS):
        raise AssertionError(
            f"{CLASS_NAME} parse window includes {SIBLING_EVENT_DEFINITIONS}"
        )
    return section
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident in (
            ...
            break
        if ident != "GetResolvedProductionLoopRouteCount":
            cursor = semi + 1
            continue
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetResolvedProductionLoopRouteCount\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for int32 GetResolvedProductionLoopRouteCount "
...
class AudioDirectorGetResolvedProductionLoopRouteCountDeclContractTests(unittest.TestCase):
    def test_audio_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetResolvedProductionLoopRouteCount"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_OPEN_COCKPIT_WIND_LOOP), body)
        self.assertFalse(has_identifier(section, SIBLING_OPEN_COCKPIT_WIND_LOOP), section)
        self.assertTrue(has_identifier(body, SIBLING_ARE_ROUTES_COMPLETE), body)
        self.assertFalse(has_identifier(section, SIBLING_ARE_ROUTES_COMPLETE), section)
        self.assertTrue(has_identifier(body, SIBLING_GET_WIND_BLEND), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_WIND_BLEND), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GetResolvedProductionLoopRouteCount"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GetResolvedProductionLoopRouteCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_resolved_production_loop_route_count_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertIn(
            collapsed("int32 GetResolvedProductionLoopRouteCount() const;"),
            compact_origin,
        )
        self.assertNotIn(
            collapsed("int32 GetResolvedProductionLoopRouteCount();"),
            compact_origin,
        )
    ...
    def test_get_resolved_production_loop_route_count_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GetResolvedProductionLoopRouteCount"),
            LOCKED_DECL,
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tint32\n\tGetResolvedProductionLoopRouteCount() const;\n",
            "\tint32   GetResolvedProductionLoopRouteCount() const;\n",
            "\tint32\tGetResolvedProductionLoopRouteCount() const;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_ufunctions_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetResolvedProductionLoopRouteCount"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GetResolvedProductionLoopRouteCount"))
        self.assertTrue(has_identifier(section, "GetResolvedProductionLoopRouteCount"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_IDLE_LOOP))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio director GetResolvedProductionLoopRouteCount decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_director_get_resolved_production_loop_route_count_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
