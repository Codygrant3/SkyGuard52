# THIS IS leftover-safe FSkyguardRoutePoint LookAheadSeconds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then `float LookAheadSeconds = 2.f;` (initializer IS `2.f`,
# not `2.0f` / `2.0` / `2` / Harbor 40/80 / `220.f` / `0.f`).
...
# TargetAirspeedKph / bAllowCombatOrbit.
# THIS IS NOT leftover RadioLine isolated fields #1332-#1338.
...
# float, initializer is `= 2.f`, EditAnywhere +
...
TARGET = "float LookAheadSeconds = 2.f;"
TARGET_WRONG_EQ_NONE = "float LookAheadSeconds = 2.0f;"
TARGET_WRONG_FALSE = "float LookAheadSeconds = false;"
TARGET_WRONG_TRUE = "float LookAheadSeconds = true;"
TARGET_WRONG_ZERO = "float LookAheadSeconds = 0.f;"
TARGET_WRONG_FLOAT = "int32 LookAheadSeconds = 2;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "float LookAheadSeconds;"
TARGET_WRONG_TWO = "float LookAheadSeconds = 2;"
TARGET_WRONG_TWO_POINT_ZERO = (
    "float LookAheadSeconds = 2.0;"
)
TARGET_WRONG_AIRSPEED = "float LookAheadSeconds = 220.f;"
...
SIBLING_SUBTITLE = SIBLING_TARGET_AIRSPEED
...
SIBLING_BUILDING_KIT = SIBLING_TARGET_AIRSPEED
...
LEFTOVER_ROUTE_POINT_TARGET_AIRSPEED = (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float LookAheadSeconds = 2.f`.
    # Reject missing initializer, `2.0f` / `2.0` / `2` /
    # Harbor 40/80 / sibling `220.f` / `0.f` / leftover Health.
    # Do not accept sibling PointId / WorldLocation /
    # TargetAirspeedKph / bAllowCombatOrbit fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+LookAheadSeconds\s*=\s*2\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+LookAheadSeconds\s*=\s*2\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LookAheadSeconds\s*=\s*2\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LookAheadSeconds\s*=\s*2\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LookAheadSeconds\s*=\s*220\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+LookAheadSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+LookAheadSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+LookAheadSeconds\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LookAheadSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LookAheadSeconds is missing from "
...
class RoutePointLookAheadSecondsFieldDeclContractTests(unittest.TestCase):
    def test_route_point_struct_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "LookAheadSeconds"),
            section,
        )
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("LookAheadSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_look_ahead_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "LookAheadSeconds = 0.f",
            compact_origin,
        )
        self.assertNotIn(
            "LookAheadSeconds = 160.f",
            compact_origin,
        )
        self.assertNotIn(
            "LookAheadSeconds = false",
            compact_origin,
        )
        self.assertNotIn(
            "LookAheadSeconds = 220.f",
            compact_origin,
        )

    def test_look_ahead_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LookAheadSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 2.f", LOCKED_DECL)
        self.assertNotIn("= 2.0f", LOCKED_DECL)
        self.assertNotIn("= 2.0", LOCKED_DECL)
        ...
        self.assertNotIn("= 220.f", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_AIRSPEED}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            "\tfloat LookAheadSecond;\n",
            "\tint32 LookAheadSeconds = 2;\n",
            "\tbool LookAheadSeconds;\n",
            "\tFText LookAheadSeconds;\n",
            f"\t{TARGET_WRONG_TWO}\n",
            f"\t{TARGET_WRONG_TWO_POINT_ZERO}\n",
            f"\t{TARGET_WRONG_EMPTY}\n",
            f"\t{TARGET_WRONG_AIRSPEED}\n",
            "\tfloat LookAheadSeconds = " + forty + ";\n",
            "\tfloat LookAheadSeconds = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "route-point LookAheadSeconds field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_ROUTE_POINT_TARGET_AIRSPEED, LOCKED_SCRIPTS)
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "look_ahead_seconds_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
