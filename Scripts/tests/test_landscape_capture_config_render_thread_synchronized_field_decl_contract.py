...
# bRenderThreadSynchronized FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bRenderThreadSynchronized = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS FSkyguardLandscapeCaptureConfigurationResult::
# bRenderThreadSynchronized, NOT leftover
# landscape-capture-config-defaults #d6fd.
# THIS IS NOT leftover landscape-visible-audit-defaults #0bce.
# THIS IS NOT leftover bUsesShowOnlyLandscape sibling.
# THIS IS NOT leftover bSuccess / Mode / ShowOnlyLandscapeComponentCount /
# GeneratedMaterialInstanceReadyComponentCount siblings.
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# bRenderThreadSynchronized. THIS IS NOT leftover VisibleAudit bSuccess #1199.
...
# Do NOT parse FSkyguardLandscapeVisibleAudit (leftover bSuccess #1199).
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# bRenderThreadSynchronized. THIS IS NOT leftover
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# bRenderThreadSynchronized FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bRenderThreadSynchronized = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly bool bRenderThreadSynchronized = false;
...
# bRenderThreadSynchronized FIELD (declaration-only). It is
...
TARGET = "bool bRenderThreadSynchronized = false;"
TARGET_WRONG = "bool bRenderThreadSynchronized = true;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bool literal for FSkyguardLandscapeCaptureConfigurationResult::
    # bRenderThreadSynchronized (must be = false, not = true).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bRenderThreadSynchronized\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bRenderThreadSynchronized is missing from "
...
class LandscapeCaptureConfigRenderThreadSynchronizedFieldDeclContractTests(unittest.TestCase):
    def test_landscape_capture_config_struct_exists(self) -> None:
        ...
        self.assertIn("bRenderThreadSynchronized", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bRenderThreadSynchronized", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_render_thread_synchronized_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_render_thread_synchronized_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bRenderThreadSynchronized"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(
                "\tbool bRenderThreadSynchronized = true;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("bRenderThreadSynchronized", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bRenderThreadSynchronized", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bRenderThreadSynchronized"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bRenderThreadSynchronized", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_render_thread_synchronized(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_landscape_capture_config"
            "_render_thread_synchronized_field_decl_contract.py"
        )
