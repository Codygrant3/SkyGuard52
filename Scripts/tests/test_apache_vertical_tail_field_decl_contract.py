# THIS IS leftover-safe ASkyguardApacheAircraft VerticalTail.
...
# then BARE `TObjectPtr<UStaticMeshComponent> VerticalTail;`
# (no initializer).
...
# Stop BEFORE sibling StubWingLeft as a claimed slot.
# Do NOT claim TailBoom / HorizontalTail / TailRotor /
# Canopy / Nose / Fuselage.
# Do NOT claim AircraftRoot (in-flight), SilhouetteMesh,
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> VerticalTail;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> VerticalTail;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> VerticalTail = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> VerticalTail = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> VerticalTail = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> VerticalTail;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> VerticalTail;"
...
STOP_BEFORE_STUB_WING_LEFT = "StubWingLeft"
...
VERTICAL_TAIL_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+VerticalTail\s*;"
)
VERTICAL_TAIL_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+VerticalTail\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer on VerticalTail as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if VERTICAL_TAIL_DECL_RE.search(compact) is None:
        return False
    if VERTICAL_TAIL_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_STUB_WING_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_STUB_WING_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_STUB_WING_LEFT}"
        )
    for later in (
        SIBLING_HORIZONTAL_TAIL,
        SIBLING_TAIL_ROTOR,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+VerticalTail\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> VerticalTail "
...
class ApacheVerticalTailFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "VerticalTail"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_STUB_WING_LEFT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "VerticalTail"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("VerticalTail", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_vertical_tail_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> VerticalTail;", compact_origin)
        self.assertNotIn("VerticalTail = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_vertical_tail_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> VerticalTail"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> VerticalTail;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> VerticalTails;\n",
            "\tint32 VerticalTail = 140;\n",
            "\tbool VerticalTail = true;\n",
            "\tfloat VerticalTail = " + forty + ";\n",
            "\tfloat VerticalTail = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_stub_wing_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_STUB_WING_LEFT))
        ...
        self.assertNotIn(TARGET_WRONG_STUB_WING_LEFT, section)
        self.assertNotIn(SIBLING_FUSELAGE, LOCKED_DECL)
        self.assertNotIn(SIBLING_TAIL_BOOM, LOCKED_DECL)
        self.assertNotIn(SIBLING_HORIZONTAL_TAIL, LOCKED_DECL)
        self.assertNotIn(SIBLING_TAIL_ROTOR, LOCKED_DECL)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_STUB_WING_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_STUB_WING_LEFT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache VerticalTail field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "vertical_tail_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
