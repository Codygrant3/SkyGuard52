...
# ActivatePooledEffect.
# origin/main form: split-line BlueprintCallable wrap
# UFUNCTION(BlueprintCallable, Category="Skyguard|Environment|VFX")
# then bool ActivatePooledEffect(
#   ESkyguardEnvironmentVFXType Type,
#   const FTransform& WorldTransform);
# (declaration-only, no inline body required). KEEP Type and
# WorldTransform args. Do NOT drop args.
# THIS IS leftover-safe unique analog component UFUNCTION
# wrapping unused Category path Skyguard|Environment|VFX.
# Identifier ActivatePooledEffect is NOT leftover analog
# Mission09 PoolCapacity and NOT leftover analog
# environment-vfx-pool-fail-closed bulk (behavioral; wrap
# Category VFX UFUNCTION declaration only) and NOT leftover
# analog EnvironmentReadiness VFXPoolSize.
# Word-boundary match ActivatePooledEffect vs
# DeactivateAllEffects.
# Category IS `Skyguard|Environment|VFX`.
# BlueprintCallable IS present. BlueprintPure IS absent. const IS absent.
# static IS absent. Return type IS bool.
...
# return type is not bool, if static is present, if Type /
# WorldTransform args are dropped, or if Category is
...
# Parse CLASS `USkyguardEnvironmentVFXPoolComponent`
...
# Do NOT parse leftover unique Validation UFUNCTION
# PrepareGovernedLandscapeForVisibleValidation.
# Do NOT parse leftover unique Authoring UFUNCTIONs.
# Do NOT parse ASkyguardMission01EnvironmentDirector.
# Do NOT claim siblings DeactivateAllEffects /
# GetAllocatedPoolSize / GetActivationCount
# as this slot (leave them uncontracted).
# Do NOT claim leftover analog PoolCapacity = 12 /
# Mission09 PoolCapacity / EnvironmentReadiness VFXPoolSize.
# Clone wrap was BlueprintCallable
# Category="Skyguard|Mission01|Environment|Validation"
# wrapping leftover unique PrepareGovernedLandscapeForVisibleValidation
# on USkyguardMission01EnvironmentAuthoringLibrary — RETARGET hard:
# Category Validation -> Skyguard|Environment|VFX,
# decl -> bool ActivatePooledEffect Type WorldTransform
# (not PrepareGovernedLandscapeForVisibleValidation).
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# Category Skyguard|Audio|Production — RETARGET Category
# Production -> Skyguard|Environment|VFX.
...
# Combat|Sortie -> Skyguard|Environment|VFX.
# KEEP BlueprintCallable (NOT BlueprintPure).
...
# LOCKED_DECL is ActivatePooledEffect bool with Type and
# WorldTransform args. NOT FName WeatherIdentity;
...
# bSuccess = false / EnsureDefaultEntries /
# DeactivateAllEffects / GetAllocatedPoolSize /
# GetActivationCount / PoolCapacity = 12 /
# PrepareGovernedLandscapeForVisibleValidation /
...
LEFTOVER_COASTAL_DIRECTOR_CLASS = "ASkyguardCoastalEnvironmentDirector"
...
TARGET_WRONG_ALLOCATED = "int32 GetAllocatedPoolSize() const;"
...
TARGET_WRONG_FLOAT = "float ActivatePooledEffect();"
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void ActivatePooledEffects();"
...
UFUNCTION_MISSION01_VALIDATION = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Mission01|Environment|Validation")'
)
UFUNCTION_PERFORMANCE = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Environment|Performance")'
)
UFUNCTION_GROUNDING = (
...
TARGET_WRONG_TRUE = "bool ActivatePooledEffect = true;"
TARGET_WRONG_ZERO = "bool ActivatePooledEffect = 0.f;"
...
STOP_BEFORE_EVENT_DEFINITIONS = "GetAllocatedPoolSize"
SIBLING_INITIALIZE_REQUIRED = "DeactivateAllEffects"
SIBLING_EVALUATE_READINESS = "GetAllocatedPoolSize"
SIBLING_CONFIGURE_ROUTING = "GetActivationCount"
SIBLING_UNBOUND_CATEGORIES = "PoolCapacity"
SIBLING_FIND_ENTRY = "SmokeSystem"
SIBLING_GET_REQUIRED = "FireSystem"
SIBLING_GET_DISPLAY = "SparksSystem"
SIBLING_ENTRIES = "GetActivationCount"
SIBLING_ROUTING = "PoolCapacity"
...
STOP_BEFORE_SKYLINE_STYLE = "DeactivateAllEffects"
...
THIS_SCRIPT = (
    "Scripts/tests/test_environment_vfx_activate_pooled_effect"
    "_decl_contract.py"
)
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bbool\s+ActivatePooledEffect\("
    r"ESkyguardEnvironmentVFXType Type,"
    r"const FTransform& WorldTransform\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bool ActivatePooledEffect(Type, WorldTransform).
    # Accept origin/main split-line trailing-semicolon form.
    # Do not accept a method const qualifier, a void/float/int32
    # rename, dropped Type / WorldTransform args, leftover unique
    # PrepareGovernedLandscapeForVisibleValidation, leftover
    # siblings DeactivateAllEffects / GetAllocatedPoolSize /
    # GetActivationCount / PoolCapacity = 12, leftover analog
    # GetSortieHits / bSuccess = false / EnsureDefaultEntries,
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bActivatePooledEffect\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+ActivatePooledEffect\b", compact):
        return False
    if re.search(r"\bstatic\s+bool\s+ActivatePooledEffect\b", compact):
        return False
    if re.search(r"\bfloat\s+ActivatePooledEffect\b", compact):
        return False
    if re.search(r"\bint32\s+ActivatePooledEffect\b", compact):
        return False
    if re.search(r"\bDeactivateAllEffects\b", compact):
        return False
    if re.search(r"\bGetAllocatedPoolSize\b", compact):
        return False
    if re.search(r"\bGetActivationCount\b", compact):
        return False
    if re.search(r"\bint32\s+PoolCapacity\s*=\s*12\b", compact):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+ActivatePooledEffect\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for ActivatePooledEffect "
...
class EnvironmentVFXActivatePooledEffectDeclContractTests(
    unittest.TestCase
):
    def test_environment_vfx_pool_component_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_COASTAL_DIRECTOR_CLASS)
        body = class_body(header)
        ...
        self.assertTrue(
            has_identifier(section, "ActivatePooledEffect"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, "DeactivateAllEffects"), body)
        self.assertFalse(has_identifier(section, "DeactivateAllEffects"), section)
    ...
    def test_missing_activate_pooled_effect_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotEqual(
            specifiers,
            'BlueprintCallable, Category="Skyguard|Environment"',
        )
        self.assertNotEqual(
            specifiers,
        ...
        self.assertNotEqual(
            specifiers,
            'BlueprintCallable, Category="Skyguard|Environment|Performance"',
        )
        self.assertNotEqual(
            specifiers,
            'BlueprintCallable, '
        ...
        self.assertNotIn("|Performance", specifiers)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn("static bool ActivatePooledEffect", compact_origin)
        self.assertNotIn("void DeactivateAllEffects", compact_origin)
        self.assertNotIn("int32 GetAllocatedPoolSize", compact_origin)
        self.assertNotIn("int32 GetActivationCount", compact_origin)
        self.assertNotIn("int32 PoolCapacity = 12", compact_origin)
        self.assertNotIn(
            "ActivatePooledEffect()const",
    ...
    def test_activate_pooled_effect_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertIn("ESkyguardEnvironmentVFXType Type", LOCKED_DECL)
        self.assertIn("const FTransform& WorldTransform", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ALLOCATED)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid ActivatePooledEffectX();\n",
            "\tint32 ActivatePooledEffect = 1;\n",
            "\tFName ActivatePooledEffect = NAME_None;\n",
            "\tvoid ActivatePooledEffect() = "
            + forty
            + ";\n",
            "\tvoid ActivatePooledEffect() = "
            + eighty
            ...
            f"\t{TARGET_WRONG_DEACTIVATE}\n",
            f"\t{TARGET_WRONG_ALLOCATED}\n",
    ...
    def test_missing_category_or_blueprint_callable_fails_closed(self) -> None:
        ...
        leftover_validation = (
            ...
            "{\n"
            "public:\n"
            f"\t{UFUNCTION_MISSION01_VALIDATION}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
    ...
    def test_does_not_claim_sibling_deactivate_or_neighbors(
        self,
    ) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        ...
        self.assertFalse(
            has_identifier("void DeactivateAllEffects();", "ActivatePooledEffect")
        )
        self.assertTrue(
            has_identifier("bool ActivatePooledEffect(", "ActivatePooledEffect")
        )
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_DEACTIVATE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ALLOCATED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ACTIVATION_COUNT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_POOL_CAPACITY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VFX_POOL_SIZE)
        self.assertFalse(has_declaration(f"\t{TARGET_WRONG_DEACTIVATE}\n", LOCKED_DECL))
        self.assertFalse(has_declaration(f"\t{TARGET_WRONG_ALLOCATED}\n", LOCKED_DECL))
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ACTIVATION_COUNT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_POOL_CAPACITY}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_VFX_POOL_SIZE}\n", LOCKED_DECL)
        )
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "ActivatePooledEffect")
        )
        self.assertTrue(
            has_identifier(section, "ActivatePooledEffect")
        )
        ...
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, "DeactivateAllEffects"))
        self.assertFalse(has_identifier(section, "DeactivateAllEffects"))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardMission09IntegrationDirector.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "environment vfx activate pooled effect "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "environment_vfx_activate_pooled_effect"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardEnvironmentVFXPoolComponent.cpp", THIS_SCRIPT)
