...
# or lock the IssueClimbCommand body in the .cpp.
...
# Category="Skyguard|Mission09|Boss") is expected as
...
# isolated-test drafts #107–#445, leftover
...
# Python IssueClimbCommand declaration contract
...
# leftover isolated-test drafts #107–#445,
...
# ReleasePooledEscort, IssueCrossCommand,
...
def leftover_short_roster_values() -> tuple[str, ...]:
    ...
    # enum value. IssueClimbCommand is the locked
...
class IronRainIssueClimbCommandDeclContractTests(unittest.TestCase):
    def test_iron_rain_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ISSUE_CLIMB_COMMAND),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ISSUE_CLIMB_COMMAND)
        self.assertIn("IssueClimbCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ISSUE_CLIMB_COMMAND))

    def test_missing_issue_climb_command_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardIronRainBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ISSUE_CLIMB_COMMAND)
        self.assertIn("IssueClimbCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ISSUE_CLIMB_COMMAND)
        self.assertIn("IssueClimbCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("UFUNCTION", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("Category", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("BlueprintCallable", ISSUE_CLIMB_COMMAND)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ISSUE_CLIMB_COMMAND)
        self.assertIn("IssueClimbCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool IssueClimbCommand;\n"
        wrong_return_void = "\tvoid IssueClimbCommand();\n"
        wrong_return_int = "\tint32 IssueClimbCommand();\n"
        added_const = "\tbool IssueClimbCommand() const;\n"
        wrong_type = "\tbool IssueClimbCommand(int32 BayIndex);\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ISSUE_CLIMB_COMMAND)
            self.assertIn("IssueClimbCommand", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_issue_climb_command_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, ISSUE_CLIMB_COMMAND))
        self.assertEqual(
            declaration_count(section, ISSUE_CLIMB_COMMAND),
            1,
        )
        self.assertTrue(
            ISSUE_CLIMB_COMMAND.startswith("bool "),
            ISSUE_CLIMB_COMMAND,
        )
        self.assertTrue(
            ISSUE_CLIMB_COMMAND.endswith(";"),
            ISSUE_CLIMB_COMMAND,
        )
        self.assertIn("IssueClimbCommand()", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("int32 BayIndex", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("INDEX_NONE", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("{", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("}", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("return ", ISSUE_CLIMB_COMMAND)
        self.assertNotIn(" const", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ReleasePooledEscort", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("OpenDispenserBay", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("IssueCrossCommand", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("OpenUpperEngineExposure", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetChinMuzzleLocation", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetGunnerMount", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("FaceWorldLocation", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("DispenserPort", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SetSearchlightTracked", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SetContactIdentified", ISSUE_CLIMB_COMMAND)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            ISSUE_CLIMB_COMMAND,
        )
        self.assertNotIn("SetLightningExposed", ISSUE_CLIMB_COMMAND)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, ISSUE_CLIMB_COMMAND)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{ISSUE_CLIMB_COMMAND}\n}}\n"
        self.assertTrue(has_declaration(one_line, ISSUE_CLIMB_COMMAND))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ISSUE_CLIMB_COMMAND),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{ISSUE_CLIMB_COMMAND}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", ISSUE_CLIMB_COMMAND)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_issue_climb_command_cpp_body(
        self,
    ) -> None:
        locked_only = f"{ISSUE_CLIMB_COMMAND}\n"
        self.assertNotIn("{", ISSUE_CLIMB_COMMAND)
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SkyguardIronRainBoss.cpp", locked_only)
        self.assertNotIn("return false", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("return true", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SetExposed", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("IsValidIndex", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("BayReleaseCounts", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_release_pooled_escort_sibling(
        ...
        for token in RELEASE_POOLED_ESCORT_NOT_LOCKED:
            ...
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ReleasePooledEscort", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetDestroyedDispenserCount", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetManeuver", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("MaxReleasesPerBay", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("DispenserPort", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("DispenserCenter", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("EnginePodPort", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("FuelControlPort", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SetSearchlightTracked", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("IsSearchlightTracked", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ASkyguardBlackKiteBoss", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SetContactIdentified", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("IsContactIdentified", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("OpenOrbitExposure", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ASkyguardLifelineHunterBoss", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SetLightningExposed", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ApplyCorrectiveBankGust", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ASkyguardTempestBoss", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetGunnerMount", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetChinTurret", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetSensorTurret", ISSUE_CLIMB_COMMAND)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{ISSUE_CLIMB_COMMAND}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("GetChinMuzzleLocation", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ESkyguardApacheSystem", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ASkyguardGunner", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("FillAndFinalize", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SkyguardRadarNode", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("USkyguardBriefingWidget", ISSUE_CLIMB_COMMAND)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            ISSUE_CLIMB_COMMAND,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ISSUE_CLIMB_COMMAND)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            ISSUE_CLIMB_COMMAND,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("MinHeightFromOriginCm", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("MaxIntegrity", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SkyguardApacheAircraft.h", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("SkyguardPathfinder", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ISSUE_CLIMB_COMMAND)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ISSUE_CLIMB_COMMAND)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ISSUE_CLIMB_COMMAND}\n"
        self.assertEqual(
            require_declaration(locked_only, ISSUE_CLIMB_COMMAND),
            ISSUE_CLIMB_COMMAND,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain IssueClimbCommand contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ISSUE_CLIMB_COMMAND.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ISSUE_CLIMB_COMMAND.lower(),
                "iron rain IssueClimbCommand contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ISSUE_CLIMB_COMMAND)

    def test_contract_is_issue_climb_command_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", ISSUE_CLIMB_COMMAND)
        self.assertNotIn("{", ISSUE_CLIMB_COMMAND)
        self.assertTrue(ISSUE_CLIMB_COMMAND.startswith("bool "))
        self.assertTrue(ISSUE_CLIMB_COMMAND.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)
