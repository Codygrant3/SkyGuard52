# THIS IS leftover-safe FSkyguardCpgHudSnapshot HeadingDegrees.
...
# `float HeadingDegrees = 0.f;` with that in-struct initializer.
# Fail-closed if HeadingDegrees is missing or renamed, if the
...
# locked decl.
# Fail-closed if the initializer is missing, is `-1.f`,
# is Harbor 40/80 (split-token), or is not `0.f`.
# HeadingDegrees is NOT leftover sibling RangeMeters
# (`float RangeMeters = -1.f`) and NOT leftover LockProgress
...
# ThreatCount / LockProgress / FlareCount /
# bMissileInbound.
...
# type is float, identifier is HeadingDegrees, initializer
# is `= 0.f`, locked decl is the plain field, not a
# UPROPERTY wrap. RangeMeters leftover SHA is a sibling
# float with `= -1.f` — do not contract RangeMeters here.
...
TARGET = "float HeadingDegrees = 0.f;"
TARGET_WRONG_BARE = "float HeadingDegrees;"
TARGET_WRONG_EQ_EMPTY = 'float HeadingDegrees = TEXT("");'
TARGET_WRONG_EQ_NONE = "float HeadingDegrees = NAME_None;"
TARGET_WRONG_FALSE = "float HeadingDegrees = false;"
TARGET_WRONG_TRUE = "float HeadingDegrees = true;"
TARGET_WRONG_NEG = "float HeadingDegrees = -1.f;"
TARGET_WRONG_FSTRING = "FString HeadingDegrees;"
TARGET_WRONG_HEALTH = "float HeadingDegrees = 160.f;"
TARGET_WRONG_BOOL = "bool HeadingDegrees;"
TARGET_WRONG_INT = "int32 HeadingDegrees;"
TARGET_WRONG_FNAME = "FName HeadingDegrees;"
TARGET_WRONG_TCHAR = 'const TCHAR* HeadingDegrees = TEXT("");'
...
HEADING_LOCKED_RE = re.compile(
    r"float\s+HeadingDegrees\s*=\s*0\.f\s*;"
)
HEADING_BARE_RE = re.compile(r"float\s+HeadingDegrees\s*;")
HEADING_INIT_RE = re.compile(
    r"float\s+HeadingDegrees\s*=\s*([^;]+);"
)
HEADING_WRONG_INIT_RE = re.compile(
    r"float\s+HeadingDegrees\s*=(?!\s*0\.f\b)"
)
RANGE_METERS_LEFTOVER_RE = re.compile(
    r"float\s+RangeMeters\s*="
)
LOCK_PROGRESS_LEFTOVER_RE = re.compile(
    r"float\s+LockProgress\s*="
)
...
def heading_degrees_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*float\s+HeadingDegrees\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float HeadingDegrees = 0.f;`.
    # Do not accept a missing initializer, Harbor 40/80,
    # leftover sibling `= -1.f` RangeMeters, leftover
    # LockProgress, `= TEXT("")` / `= NAME_None` /
    # `= false` / `= true` / `= 160.f`. Do not accept
    # leftover sibling RangeMeters / LockProgress or
    # leftover ContactMark LockAlpha / Label / leftover
    # debrief MissionTitle. Do not accept leftover
    # LoadoutSpec FlareCount. Do not accept UPROPERTY /
    # Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if RANGE_METERS_LEFTOVER_RE.search(compact) and (
        HEADING_LOCKED_RE.search(compact) is None
    ):
        return False
    if LOCK_PROGRESS_LEFTOVER_RE.search(compact) and (
        HEADING_LOCKED_RE.search(compact) is None
    ):
        return False
    if re.search(r"FString\s+HeadingDegrees\b", compact):
        return False
    if re.search(r"bool\s+HeadingDegrees\b", compact):
        return False
    if re.search(r"int32\s+HeadingDegrees\b", compact):
        return False
    if re.search(r"FName\s+HeadingDegrees\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*HeadingDegrees\b", compact):
        return False
    if HEADING_BARE_RE.search(compact) and (
        HEADING_LOCKED_RE.search(compact) is None
    ):
        return False
    init_match = HEADING_INIT_RE.search(compact)
    if init_match is None:
        return False
    if init_match.group(1).strip() != LOCKED_INITIALIZER:
        return False
    if HEADING_WRONG_INIT_RE.search(compact):
        return False
    if HEADING_LOCKED_RE.search(compact) is None:
        return False
    if heading_degrees_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float HeadingDegrees\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float HeadingDegrees is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*float HeadingDegrees\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on float HeadingDegrees is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "HeadingDegrees"):
        raise AssertionError(
            "UPROPERTY clone landed on HeadingDegrees; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "HeadingDegrees"):
            raise AssertionError(
                f"{token} clone landed on HeadingDegrees; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotHeadingDegreesFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HeadingDegrees"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "float HeadingDegrees = 0.f;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("float HeadingDegrees"))
        ...
        self.assertIn("HeadingDegrees", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("HeadingDegrees", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HeadingDegrees"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_heading_degrees_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_range_meters_does_not_satisfy_heading_degrees(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_lock_progress_does_not_satisfy_heading_degrees(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float HeadingDegrees = 0.f;", compact_origin)
        self.assertNotIn("HeadingDegrees = TEXT", compact_origin)
        self.assertNotIn("HeadingDegrees = NAME_None", compact_origin)
        self.assertNotIn("HeadingDegrees = false", compact_origin)
        self.assertNotIn("HeadingDegrees = true", compact_origin)
        self.assertNotIn("HeadingDegrees = -1.f", compact_origin)
        self.assertNotIn("HeadingDegrees = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("HeadingDegrees = " + forty, compact_origin)
        self.assertNotIn("HeadingDegrees = " + eighty, compact_origin)

    def test_heading_degrees_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float HeadingDegrees"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_NEG}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{LEFTOVER_CONTACT_LABEL}\n",
            "\tFString HeadingDegrees;\n",
            "\tint32 HeadingDegrees;\n",
            "\tbool HeadingDegrees;\n",
            "\tfloat HeadingDegrees = " + forty + ";\n",
            "\tfloat HeadingDegrees = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    heading_degrees_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(heading_degrees_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot HeadingDegrees field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "heading_degrees_field_decl_contract.py"
        ))
