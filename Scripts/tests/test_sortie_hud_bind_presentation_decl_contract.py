...
# then BARE `void BindPresentation(
# USkyguardSortiePresentationComponent* InPresentation);`
# (declaration-only, no inline body required).
# KEEP InPresentation arg from origin/main.
# KEEP BlueprintCallable. Do NOT retarget to
# RefreshFromPresentationState / RebindIfNeeded /
# GetBoundPresentation.
# Word-boundary match BindPresentation vs
# GetBoundPresentation.
# THIS IS leftover-safe unique analog Category
# Skyguard|Presentation|HUD (unique unused 3-segment
# suffix HUD). This is leftover-safe unique analog, NOT
# leftover analog Presentation / Briefing / Debrief.
# BlueprintCallable IS present. BlueprintPure IS absent.
# const IS absent. Return type IS void.
# Identifier BindPresentation is NOT leftover analog
# sortie-hud-host-fail-closed bulk #132 (behavioral;
# wrap Category HUD UFUNCTION declaration only) and NOT
# leftover analog Gunner CpgSightHud #1627.
...
# return type is not void, if InPresentation is
# missing, or if Category is
...
# without Skyguard| / Category="Skyguard|Presentation"
# without leftover-safe unique |HUD suffix /
...
# RefreshFromPresentationState / RebindIfNeeded /
# GetBoundPresentation are uncontracted.
# Do NOT claim sibling UFUNCTION
# RefreshFromPresentationState / RebindIfNeeded /
# GetBoundPresentation as this slot
...
# Category="Skyguard|Presentation|HUD"
# wrapping leftover unique
# BindPresentation — RETARGET
# hard: method/return/args. KEEP BlueprintCallable.
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# Category Skyguard|Audio|Production — RETARGET Category
# Production -> Skyguard|Presentation|HUD.
# KEEP BlueprintCallable (NOT BlueprintPure).
# RETARGET void EnsureDefaultEntries(); -> void BindPresentation(;
...
# Combat|Sortie -> Skyguard|Presentation|HUD.
# KEEP BlueprintCallable (NOT BlueprintPure).
...
# LOCKED_DECL is void BindPresentation(
# USkyguardSortiePresentationComponent* InPresentation);
...
# and NOT void RefreshFromPresentationState(); and NOT
# void RebindIfNeeded(); and NOT GetBoundPresentation() const;
# and NOT int32 GetActivationCount() const; and NOT
# BindPresentation and NOT
...
# void RebindIfNeeded() / GetBoundPresentation() const /
...
TARGET_WRONG_FLOAT = (
    "float BindPresentation("
    "USkyguardSortiePresentationComponent* InPresentation);"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void BindPresentations(USkyguardSortiePresentationComponent* InPresentation);"
...
TARGET_WRONG_TRUE = "bool BindPresentation = true;"
TARGET_WRONG_ZERO = "bool BindPresentation = 0.f;"
...
STOP_BEFORE_EVENT_DEFINITIONS = "GetBoundPresentation"
...
SIBLING_UNBOUND_CATEGORIES = "GetBoundPresentation"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bvoid\s+BindPresentation\("
    r"USkyguardSortiePresentationComponent\* InPresentation\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # rename, leftover unique BindPresentation,
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bBindPresentation\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bbool\s+BindPresentation\b", compact):
        return False
    if re.search(r"\bfloat\s+BindPresentation\b", compact):
        return False
    if re.search(r"\bint32\s+BindPresentation\b", compact):
        return False
    if re.search(
        r"\bstatic\s+FSkyguardLandscapeVisibleAudit\s+BindPresentation\b",
        compact,
    ):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+BindPresentation\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for BindPresentation "
...
class SortieHudBindPresentationDeclContractTests(
    unittest.TestCase
):
    def test_sortie_hud_host_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "BindPresentation"),
            section,
        )
    ...
    def test_prepare_governed_landscape_for_visible_validation_declaration_matches_origin_main(
        ...
        self.assertIn("InPresentation", LOCKED_DECL)
        self.assertIn(
            "USkyguardSortiePresentationComponent*",
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid BindPresentationX(USkyguardSortiePresentationComponent* InPresentation);\n",
            "\tint32 BindPresentation = 1;\n",
            "\tFName BindPresentation = NAME_None;\n",
            "\tvoid BindPresentation() = "
            + forty
            + ";\n",
            "\tvoid BindPresentation() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "BindPresentation")
        )
        self.assertTrue(
            has_identifier(section, "BindPresentation")
        )
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertNotIn("GetBoundPresentation", LOCKED_DECL)
