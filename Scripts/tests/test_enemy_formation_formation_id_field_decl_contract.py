# THIS IS leftover-safe FSkyguardEnemyFormationDefinition FormationId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName FormationId;` (no initializer).
...
# Do NOT parse leftover ESkyguardFormationType enum contract
# if any (keep that file in LOCKED_SCRIPTS).
# Do NOT contract sibling fields Formation remaining /
# UnitCount remaining / SpacingCentimeters remaining.
...
# isolated FormationId field decl, same as leftover
...
# `FName PointId` / `FName WaveId`, retarget: type is
# FName FormationId, no `=`, specifiers are EditAnywhere +
...
TARGET = "FName FormationId;"
TARGET_WRONG_EQ_NONE = "FName FormationId = NAME_None;"
TARGET_WRONG_FALSE = "FName FormationId = false;"
TARGET_WRONG_TRUE = "FName FormationId = true;"
TARGET_WRONG_ZERO = "FName FormationId = 0.f;"
TARGET_WRONG_FLOAT = "float FormationId;"
...
SIBLING_FORMATION = "ESkyguardFormationType Formation"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+FormationId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName FormationId is missing from "
...
def require_formation_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} FormationId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} FormationId"
        )
...
class EnemyFormationFormationIdFieldDeclContractTests(unittest.TestCase):
    def test_enemy_formation_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "FormationId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "FormationId"), section)
        self.assertNotIn(STOP_BEFORE_ENEMY_WAVE, section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        ...
        self.assertIn("FormationId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_formation_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tint32 {SIBLING_UNIT_COUNT} = 3;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_formation_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName FormationId;", compact_origin)
        self.assertNotIn("FormationId = NAME_None", compact_origin)
        self.assertNotIn("FormationId = false", compact_origin)
        self.assertNotIn("FormationId = true", compact_origin)
        self.assertNotIn("FormationId = 0.f", compact_origin)
        self.assertNotIn("FormationId = 160.f", compact_origin)

    def test_formation_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName FormationId"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 FormationId;\n",
            "\tbool FormationId;\n",
            "\tfloat FormationId = " + forty + ";\n",
            "\tfloat FormationId = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_formation_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_id_specifiers(spec_section(with_category))
    ...
    def test_does_not_contract_sibling_enemy_formation_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Formation"), "Formation")
        self.assertTrue(has_identifier(section, SIBLING_UNIT_COUNT), SIBLING_UNIT_COUNT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "enemy-formation FormationId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enemy_formation_formation_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_UNIT_COUNT, locked_only)
