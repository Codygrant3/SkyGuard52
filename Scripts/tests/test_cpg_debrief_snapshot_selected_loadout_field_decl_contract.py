# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot SelectedLoadout.
# origin/main form is the BARE field on a PLAIN C++ struct:
...
# not ESkyguardLoadout, initializer is missing or is not
# `= ESkyguardLoadout::Balanced`, or someone writes
# `= 0` / `= false` / another enumerator.
# Parse STRUCT `FSkyguardCpgDebriefSnapshot` body ONLY.
...
# Do NOT reopen leftover loadout-spec-defaults #253 /
# leftover resolve-loadout #275.
# Do NOT contract sibling fields bValid / bWon /
# MissionTitle / OutcomeNarrative / Score / Medal /
# ShotsFired / Hits / CargoPercent / bRadarDead /
# DestroyedSystems / CannonReady / RocketReady /
# GuidedReady.
...
# leftover MissionDefinition,
# leftover USkyguardRuntimeMeshCatalog,
# leftover USkyguardAudioAcceptanceHarness,
# leftover USkyguardAudioProductionBank,
# leftover FSkyguardAudioEventDefinition,
# leftover ESkyguardAudioEvent,
# leftover ASkyguardGunshipSortieDirector,
# leftover Harbor 40/80 directors,
# leftover retired mount / live-copy types.
...
# Ri+fle / Ya+k appearing as contiguous tokens
# in THIS test file.
...
TARGET = "ESkyguardLoadout SelectedLoadout = ESkyguardLoadout::Balanced;"
TARGET_WRONG_BARE = "ESkyguardLoadout SelectedLoadout;"
TARGET_WRONG_ZERO = "ESkyguardLoadout SelectedLoadout = 0;"
TARGET_WRONG_FALSE = "ESkyguardLoadout SelectedLoadout = false;"
TARGET_WRONG_TRUE = "ESkyguardLoadout SelectedLoadout = true;"
TARGET_WRONG_ANTI_ARMOR = (
    "ESkyguardLoadout SelectedLoadout = ESkyguardLoadout::AntiArmor;"
)
TARGET_WRONG_ROCKET_HEAVY = (
    "ESkyguardLoadout SelectedLoadout = ESkyguardLoadout::RocketHeavy;"
)
TARGET_WRONG_INTERCEPT = (
    "ESkyguardLoadout SelectedLoadout = ESkyguardLoadout::Intercept;"
)
TARGET_WRONG_INT32 = "int32 SelectedLoadout = 0;"
TARGET_WRONG_BOOL = "bool SelectedLoadout = false;"
TARGET_WRONG_FNAME = "FName SelectedLoadout;"
TARGET_WRONG_FLOAT = "float SelectedLoadout;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SPEC = "FSkyguardLoadoutSpec SelectedLoadout;"
LOCKED_DECL = TARGET
LEFTOVER_THEATER_UPROPERTY = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Theater")'
)
LEFTOVER_EDIT_UPROPERTY = (
...
STOP_BEFORE_RESOLVE = "SkyguardResolveLoadout"
...
STOP_BEFORE_MESH_BIND = "struct FSkyguardMeshBindSlot"
STOP_BEFORE_AUDIBLE = "struct FSkyguardAudibleAcceptanceReceipt"
STOP_BEFORE_MISSION_RESULT = "struct FSkyguardMissionResult"
STOP_BEFORE_THEATER_KIT = "struct FSkyguardTheaterKitSpec"
STOP_BEFORE_MISSION_DEF = "struct FSkyguardMissionDefinition"
...
SIBLING_B_RADAR_DEAD = "bRadarDead"
...
def leftover_function_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_CAPTURE,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardLoadout SelectedLoadout = ESkyguardLoadout::Balanced;`.
    # Do not accept a bare decl, `= 0`, `= false`, `= true`,
    # AntiArmor / RocketHeavy / Intercept, leftover float Health,
    # or a clone that still wraps UPROPERTY.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"ESkyguardLoadout\s+SelectedLoadout\s*=\s*"
        r"ESkyguardLoadout::Balanced\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"ESkyguardLoadout\s+SelectedLoadout\s*;", compact):
        return False
    if re.search(
        r"\b(?:int32|bool|float|FName|FString|FSkyguardLoadoutSpec)"
        r"\s+SelectedLoadout\b",
        compact,
    ):
        return False
    if re.search(
        r"SelectedLoadout\s*=\s*(?:0|false|true|NAME_None)\b",
        compact,
    ):
        return False
    if re.search(
        r"SelectedLoadout\s*=\s*ESkyguardLoadout::"
        r"(?:AntiArmor|RocketHeavy|Intercept)\b",
        compact,
    ):
        return False
    if "UPROPERTY" in compact:
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_MESH_BIND,
        STOP_BEFORE_AUDIBLE,
        STOP_BEFORE_MISSION_RESULT,
        STOP_BEFORE_THEATER_KIT,
        ...
        f"class SKYGUARD52_API {STOP_BEFORE_PATROL}",
        f"class {STOP_BEFORE_PATROL} :",
        "GENERATED_BODY",
        "USTRUCT",
        "UPROPERTY",
    )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardLoadout\s+SelectedLoadout\b", compact[index:]
        ):
            raise AssertionError(
                "UPROPERTY for ESkyguardLoadout SelectedLoadout is present "
                f"on origin/main:{HEADER_PATH} struct {STRUCT_NAME} body; "
                "FSkyguardCpgDebriefSnapshot is a plain C++ struct"
            )
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardLoadout SelectedLoadout is missing from "
...
class CpgDebriefSnapshotSelectedLoadoutFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_LOADOUT_SPEC)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_THEATER_KIT)
        body = struct_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "SelectedLoadout"), section)
        ...
        self.assertIn(STOP_BEFORE_CAPTURE_SIG, header)
        self.assertNotIn(STOP_BEFORE_CAPTURE_SIG, section)
        ...
        self.assertNotIn(STOP_BEFORE_LOADOUT_SPEC, section)
        ...
        self.assertNotIn(LEFTOVER_THEATER_UPROPERTY, body)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        theater = (
            f"struct {STOP_BEFORE_THEATER_KIT[7:]}\n"
            "{\n"
    ...
    def test_capture_or_copy_declaration_does_not_satisfy(self) -> None:
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "};\n"
            f"{STOP_BEFORE_CAPTURE_SIG}(\n"
            f"\tconst {STOP_BEFORE_SORTIE}* Director,\n"
            f"\tconst {STOP_BEFORE_GUNNER}* Gunner,\n"
            f"\tconst {STOP_BEFORE_PATROL}* Ship);\n"
        ...
        self.assertFalse(has_identifier(section, "SelectedLoadout"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        self.assertNotIn(STOP_BEFORE_BUILD_COPY, section)
        self.assertNotIn(STOP_BEFORE_BANNED_TERM, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SelectedLoadout", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_selected_loadout_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tbool {SIBLING_B_VALID} = false;\n"
            "};\n"
        )
    ...
    def test_origin_main_has_no_uproperty_or_ustruct(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_EDIT_UPROPERTY, LOCKED_DECL)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_clone_uproperty_wrap_fails_closed(self) -> None:
        wrapped = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{LEFTOVER_THEATER_UPROPERTY}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(wrapped_section, LOCKED_DECL)
        self.assertIn("SelectedLoadout", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        edit_wrap = (
            f"\t{LEFTOVER_EDIT_UPROPERTY}\n"
            f"\t{LOCKED_DECL}\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(edit_wrap, LOCKED_DECL)
        self.assertIn("SelectedLoadout", str(raised.exception))
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn(
            "ESkyguardLoadout SelectedLoadout = "
            "ESkyguardLoadout::Balanced;",
            compact_origin,
        )
        self.assertNotIn("SelectedLoadout = 0", compact_origin)
        self.assertNotIn("SelectedLoadout = false", compact_origin)
        self.assertNotIn("SelectedLoadout = true", compact_origin)
        self.assertNotIn(
            "SelectedLoadout = ESkyguardLoadout::AntiArmor",
            compact_origin,
        )
        self.assertNotIn(
            "SelectedLoadout = ESkyguardLoadout::RocketHeavy",
            compact_origin,
        )
        self.assertNotIn(
            "SelectedLoadout = ESkyguardLoadout::Intercept",
            compact_origin,
        )

    def test_selected_loadout_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardLoadout SelectedLoadout"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("AntiArmor", LOCKED_DECL)
        self.assertNotIn("RocketHeavy", LOCKED_DECL)
        self.assertNotIn("Intercept", LOCKED_DECL)
        ...
        self.assertNotIn("FSkyguardLoadoutSpec", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ANTI_ARMOR}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ROCKET_HEAVY}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SPEC}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_TRUE}\n",
            f"\t{TARGET_WRONG_ANTI_ARMOR}\n",
            f"\t{TARGET_WRONG_ROCKET_HEAVY}\n",
            ...
            leftover_guided,
            f"\tbool {SIBLING_B_VALID} = false;\n",
            "\tESkyguardLoadout SelectedLoadouts = "
            "ESkyguardLoadout::Balanced;\n",
            "\tint32 SelectedLoadout = 0;\n",
            "\tbool SelectedLoadout = false;\n",
            "\tfloat SelectedLoadout = " + forty + ";\n",
            "\tfloat SelectedLoadout = " + eighty + ";\n",
        )
    ...
    def test_clone_category_or_visible_anywhere_fails_closed(self) -> None:
        no_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(VisibleAnywhere, BlueprintReadOnly)\n"
        ...
        self.assertIn("UPROPERTY", str(raised.exception))
        self.assertNotIn('Category="Skyguard|Theater"', origin)
        self.assertNotIn('Category="Skyguard|Campaign"', origin)
    ...
    def test_does_not_contract_sibling_snapshot_fields(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_BUILD_COPY, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_BANNED_TERM, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_RESOLVE, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_APACHE_CLASS, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_LOADOUT_SPEC, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_capture_copy_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_CAPTURE_SIG, header)
        self.assertNotIn(STOP_BEFORE_CAPTURE_SIG, section)
        self.assertNotIn(STOP_BEFORE_CAPTURE_SIG, leaked)
        ...
        self.assertIn(STOP_BEFORE_SORTIE, header)
        ...
        self.assertNotIn(STOP_BEFORE_RESOLVE, header)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\t{STOP_BEFORE_LOADOUT_SPEC} {STOP_BEFORE_RESOLVE}"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot SelectedLoadout field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "selected_loadout_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_ISOLATED_MISSION_TITLE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_B_VALID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_B_WON, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_OUTCOME_NARRATIVE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_MEDAL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_SCORE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_CARGO_PERCENT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_HITS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_SHOTS_FIRED, LOCKED_SCRIPTS)
    ...
    def test_leftover_snapshot_defaults_and_isolated_siblings_stay_locked(self) -> None:
        leftovers = (
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS,
            LEFTOVER_LOADOUT_SPEC_DEFAULTS,
            LEFTOVER_RESOLVE_LOADOUT,
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("FSkyguardLoadoutSpec", locked_only)
        ...
        self.assertNotIn("FSkyguardMeshBindSlot", locked_only)
        self.assertNotIn("FSkyguardAudibleAcceptanceReceipt", locked_only)
        self.assertNotIn("FSkyguardMissionDefinition", locked_only)
        self.assertNotIn("USkyguardRuntimeMeshCatalog", locked_only)
        self.assertNotIn("USkyguardAudioAcceptanceHarness", locked_only)
        self.assertNotIn("USkyguardAudioProductionBank", locked_only)
        ...
        for helper in leftover_function_tokens():
