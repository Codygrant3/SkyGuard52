...
# then BARE `TArray<FName> MissingAttenuationBindings;`.
...
# MissingSoundBindings / MissingCategoryEntries /
# MissingConcurrencyBindings / MissingOutputSubmixBindings /
# MissingRoutingAssets / bCategoryContractComplete /
# bProductionReady.
# Match the EXACT identifier MissingAttenuationBindings as a
# whole field name after TArray<FName>. Do NOT accept a
# substring match on Missing or Bindings. Do NOT claim
# sibling Missing* / *Bindings arrays.
...
# If a clone asserts int32 / required `= 0` (BoundProductionSourceCount
# leftover #1412), retarget: parse FSkyguardProductionAudioAudit
# only, UPROPERTY is BlueprintReadOnly with NO EditAnywhere,
# type is TArray<FName>, BARE decl with NO initializer, no Category=.
...
TARGET = "TArray<FName> MissingAttenuationBindings;"
TARGET_WRONG_EQ_EMPTY = "TArray<FName> MissingAttenuationBindings = {};"
TARGET_WRONG_EQ_ZERO = "TArray<FName> MissingAttenuationBindings = 0;"
TARGET_WRONG_FALSE = "TArray<FName> MissingAttenuationBindings = false;"
TARGET_WRONG_TRUE = "TArray<FName> MissingAttenuationBindings = true;"
TARGET_WRONG_ZERO = "TArray<FName> MissingAttenuationBindings = 0.f;"
TARGET_WRONG_FORTY = "TArray<FName> MissingAttenuationBindings = 40;"
TARGET_WRONG_EIGHTY = "TArray<FName> MissingAttenuationBindings = 80;"
TARGET_WRONG_FLOAT = "float MissingAttenuationBindings = 0;"
TARGET_WRONG_INT32 = "int32 MissingAttenuationBindings = 0;"
TARGET_WRONG_FNAME = "FName MissingAttenuationBindings;"
TARGET_WRONG_BOOL = "bool MissingAttenuationBindings = false;"
TARGET_WRONG_FSTRING = "FString MissingAttenuationBindings;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MissingAttenuationBindings;"
...
FIELD_NAME = "MissingAttenuationBindings"
...
THIS_SCRIPT = (
    ...
    "_attenuation_bindings_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored bare `TArray<FName> MissingAttenuationBindings;`.
    # Reject added initializer (`= {}`, `= 0`, Harbor 40/80),
    # int32, float, bool, FName, FString, TSoftObjectPtr.
    # Match the EXACT identifier MissingAttenuationBindings after
    # TArray<FName>. Do not accept a substring match on Missing
    # or Bindings. Do not claim sibling Missing* / *Bindings
    # arrays.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+MissingAttenuationBindings\s*=",
        compact,
    ):
        return False
    if re.search(
        r"MissingAttenuationBindings\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingAttenuationBindings\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingAttenuationBindings\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingAttenuationBindings\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingAttenuationBindings\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MissingAttenuationBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MissingAttenuationBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MissingAttenuationBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MissingAttenuationBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MissingAttenuationBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MissingAttenuationBindings\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+MissingAttenuationBindings\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> MissingAttenuationBindings is missing from "
...
class ProductionAudioAuditMissingAttenuationBindingsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_attenuation_bindings_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_added_fails_closed(self) -> None:
        ...
        self.assertIn("TArray<FName> MissingAttenuationBindings;", compact_origin)
        self.assertNotIn("MissingAttenuationBindings = 40", compact_origin)
        self.assertNotIn("MissingAttenuationBindings = 80", compact_origin)
        self.assertNotIn("MissingAttenuationBindings = false", compact_origin)
        self.assertNotIn("MissingAttenuationBindings = {}", compact_origin)
        self.assertNotIn("MissingAttenuationBindings = 0", compact_origin)

    def test_missing_attenuation_bindings_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> MissingAttenuationBindings"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tTArray<FName> MissingAttenuationBindingss;\n",
            "\tTArray<FName> MissingAttenuationBindings = 1;\n",
            "\tbool MissingAttenuationBindings;\n",
            "\tfloat MissingAttenuationBindings = " + forty + ";\n",
            "\tfloat MissingAttenuationBindings = " + eighty + ";\n",
    ...
    def test_exact_identifier_rejects_substring_missing_siblings(self) -> None:
        ...
        self.assertFalse(has_identifier(exact, "AttenuationBindings"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit MissingAttenuationBindings field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "missing_attenuation_bindings_field_decl_contract.py"
        ))
