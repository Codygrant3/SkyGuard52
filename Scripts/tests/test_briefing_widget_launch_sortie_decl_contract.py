...
# or lock the LaunchSortie body in the .cpp.
...
# (draft #369) and not leftover CanLaunch
# (draft #370). Leftover sortie-presentation fail-closed
...
# stay sibling-only.
LAUNCH_SORTIE = "bool LaunchSortie();"
UFUNCTION_BRIEFING = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Presentation|Briefing")'
)
...
# LaunchSortie declaration contract on
...
# and AcknowledgeBriefing on this class. Stay off leftover
...
# AcknowledgeAndLaunch, leftover CanLaunch,
...
# sibling test_briefing_widget_get_mission_title_decl_contract.py,
# sibling test_briefing_widget_get_briefing_text_decl_contract.py,
# sibling test_briefing_widget_acknowledge_briefing_decl_contract.py,
...
# leftover GetHowToFlyRows, leftover AcknowledgeBriefing,
...
# GetBriefingCards / GetRadioRows / GetHowToFlyRows /
# AcknowledgeBriefing stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# stay unlocked. AcknowledgeAndLaunch (#369) and
# leftover CanLaunch (#370) are not this widget
# declaration.
...
# .cpp LaunchSortie body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardBriefingWidget::LaunchSortie",
    "SkyguardSortiePresentationWidgets.cpp",
)
...
class BriefingWidgetLaunchSortieDeclContractTests(unittest.TestCase):
    def test_briefing_widget_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, LAUNCH_SORTIE), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LAUNCH_SORTIE)
        self.assertIn("LaunchSortie", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, LAUNCH_SORTIE))

    def test_missing_launch_sortie_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tvoid Configure("
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, LAUNCH_SORTIE)
        self.assertIn("LaunchSortie", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, LAUNCH_SORTIE)
        self.assertIn("LaunchSortie", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, LAUNCH_SORTIE), section)
        self.assertNotIn("BlueprintPure", LAUNCH_SORTIE)
        self.assertNotIn("UFUNCTION", LAUNCH_SORTIE)
        self.assertNotIn("Category", LAUNCH_SORTIE)
        self.assertNotIn("BlueprintCallable", LAUNCH_SORTIE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, LAUNCH_SORTIE)
        self.assertIn("LaunchSortie", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        wrong_return = "\tvoid LaunchSortie();\n"
        added_const = "\tbool LaunchSortie() const;\n"
        added_arg = "\tbool LaunchSortie(bool bForce);\n"
        int_return = "\tint32 LaunchSortie();\n"
        sibling_name = "\tbool AcknowledgeAndLaunch();\n"
        acknowledge = "\tbool AcknowledgeBriefing();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, LAUNCH_SORTIE)
            self.assertIn("LaunchSortie", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_acknowledge_and_launch_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, LAUNCH_SORTIE)
        self.assertIn("LaunchSortie", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, LAUNCH_SORTIE))
        self.assertNotIn("AcknowledgeAndLaunch", LAUNCH_SORTIE)

    def test_can_launch_does_not_satisfy(self) -> None:
        leftover = "\tbool CanLaunch() const;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, LAUNCH_SORTIE)
        self.assertIn("LaunchSortie", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, LAUNCH_SORTIE))
        self.assertNotIn("CanLaunch", LAUNCH_SORTIE)

    def test_launch_sortie_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, LAUNCH_SORTIE))
        self.assertEqual(declaration_count(section, LAUNCH_SORTIE), 1)
        self.assertTrue(
            LAUNCH_SORTIE.startswith("bool "),
            LAUNCH_SORTIE,
        )
        self.assertTrue(
            LAUNCH_SORTIE.endswith(";"),
            LAUNCH_SORTIE,
        )
        self.assertIn("LaunchSortie()", LAUNCH_SORTIE)
        self.assertNotIn("INDEX_NONE", LAUNCH_SORTIE)
        self.assertNotIn("{", LAUNCH_SORTIE)
        self.assertNotIn("}", LAUNCH_SORTIE)
        self.assertNotIn("return ", LAUNCH_SORTIE)
        self.assertNotIn(" const", LAUNCH_SORTIE)
        self.assertNotIn("AcknowledgeAndLaunch", LAUNCH_SORTIE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, LAUNCH_SORTIE),
                section,
            )
            self.assertEqual(
                require_declaration(section, LAUNCH_SORTIE),
            ...
            self.assertEqual(
                declaration_count(section, LAUNCH_SORTIE),
                1,
            )
        one_line = f"{{\npublic:\n\t{LAUNCH_SORTIE}\n}}\n"
        self.assertTrue(has_declaration(one_line, LAUNCH_SORTIE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, LAUNCH_SORTIE),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", LAUNCH_SORTIE)
        self.assertNotIn("return ", LAUNCH_SORTIE)
        self.assertNotIn("return false", LAUNCH_SORTIE)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{LAUNCH_SORTIE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", LAUNCH_SORTIE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", LAUNCH_SORTIE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_launch_sortie_cpp_body(self) -> None:
        locked_only = f"{LAUNCH_SORTIE}\n"
        self.assertNotIn("{", LAUNCH_SORTIE)
        self.assertNotIn("}", LAUNCH_SORTIE)
        self.assertNotIn("return ", LAUNCH_SORTIE)
        ...
        self.assertNotIn("return false", LAUNCH_SORTIE)
        self.assertNotIn("return true", LAUNCH_SORTIE)

    def test_contract_does_not_relock_configure(self) -> None:
        locked_only = f"{LAUNCH_SORTIE}\n"
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("Configure(", LAUNCH_SORTIE)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("GetPresentation", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_get_mission_title(self) -> None:
        ...
        for neighbor in GET_MISSION_TITLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("GetMissionTitle", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("GetBriefingText", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_get_briefing_cards(self) -> None:
        ...
        for neighbor in GET_BRIEFING_CARDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("GetBriefingCards", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_get_radio_rows(self) -> None:
        ...
        for neighbor in GET_RADIO_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("GetRadioRows", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_get_how_to_fly_rows(self) -> None:
        ...
        for neighbor in GET_HOW_TO_FLY_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("GetHowToFlyRows", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_acknowledge_briefing(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("AcknowledgeBriefing", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("ConfigureFromMission", LAUNCH_SORTIE)
        self.assertNotIn("ConfigureFromMission", locked_only)
        self.assertNotIn("USkyguardMissionBriefingComponent", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_briefing_component_neighbors(
        ...
        for neighbor in (
            ...
            *GET_RADIO_CHATTER_NOT_LOCKED,
            ...
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("SetAssetsReady", LAUNCH_SORTIE)
        self.assertNotIn("AdvanceBriefing", LAUNCH_SORTIE)
        self.assertNotIn("AcknowledgeAndLaunch", LAUNCH_SORTIE)
        self.assertNotIn("CanLaunch", LAUNCH_SORTIE)
        self.assertNotIn("GetBriefingState", LAUNCH_SORTIE)
        self.assertNotIn("GetElapsedSeconds", LAUNCH_SORTIE)
        self.assertNotIn("GetMinimumWarmupSeconds", LAUNCH_SORTIE)
        self.assertNotIn("GetRadioChatter", LAUNCH_SORTIE)
        self.assertNotIn("ESkyguardMissionBriefingState", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, LAUNCH_SORTIE)
        self.assertNotIn("Unconfigured", LAUNCH_SORTIE)
        self.assertNotIn("Warming", LAUNCH_SORTIE)
        self.assertNotIn("Launched", LAUNCH_SORTIE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            LAUNCH_SORTIE,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LAUNCH_SORTIE)
        self.assertNotIn("FSkyguardBriefingCard", LAUNCH_SORTIE)
        self.assertNotIn("FSkyguardBriefingRadioRow", LAUNCH_SORTIE)
        self.assertNotIn("FSkyguardHowToFlyRow", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation(
        ...
        for token in LEFTOVER_SORTIE_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LAUNCH_SORTIE)
        self.assertNotIn("SkyguardSortiePresentationFailClosed", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAUNCH_SORTIE)
        self.assertNotIn("FillResultCombatStats", LAUNCH_SORTIE)
        self.assertNotIn("ASkyguardGunner", LAUNCH_SORTIE)
        self.assertNotIn("FillAndFinalize", LAUNCH_SORTIE)
        self.assertNotIn("FillAndFail", LAUNCH_SORTIE)
        self.assertNotIn("ApplyHydraForClusters", LAUNCH_SORTIE)
    ...
    def test_contract_does_not_relock_bind_hud_or_theater(self) -> None:
        ...
        for token in LEFTOVER_BIND_HUD_AND_THEATER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LAUNCH_SORTIE)
        self.assertNotIn("test_bind_hud_host_presentation_tests.py", LAUNCH_SORTIE)
        self.assertNotIn("test_campaign_theater_kit_contract.py", LAUNCH_SORTIE)
        self.assertNotIn(
            "test_briefing_widget_configure_decl_contract.py",
            LAUNCH_SORTIE,
        )
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{LAUNCH_SORTIE}\n"
        self.assertEqual(
            require_declaration(locked_only, LAUNCH_SORTIE),
            LAUNCH_SORTIE,
        )
    ...
    def test_contract_does_not_parse_debrief_widget(self) -> None:
        ...
        for token in DEBRIEF_WIDGET_NOT_LOCKED:
            ...
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", section)
        self.assertNotIn("GetDebrief", LAUNCH_SORTIE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing widget LaunchSortie contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, LAUNCH_SORTIE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "briefing widget LaunchSortie contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, LAUNCH_SORTIE.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, LAUNCH_SORTIE)

    def test_contract_is_launch_sortie_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(LAUNCH_SORTIE.startswith("bool "))
        self.assertTrue(LAUNCH_SORTIE.endswith(";"))
        self.assertIn(UFUNCTION_BRIEFING, section)
