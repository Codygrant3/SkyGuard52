...
# INDEX_NONE or lock BindCampaignRuntime
# construction in the .cpp. This is leftover-safe
# Mission09 Integration BindCampaignRuntime
# on ASkyguardMission09IntegrationDirector. THIS
# is BindCampaignRuntime, NOT leftover
...
# (`bool BindCampaignRuntime(USkyguardCampaignSubsystem*
# InCampaignRuntime);`); accept that
...
REQUIRED_DECL = BIND_CAMPAIGN_RUNTIME
GET_POOL_RUNTIME = (
...
# an isolated Python BindCampaignRuntime
# method declaration contract on
# ASkyguardMission09IntegrationDirector.
# THIS is BindCampaignRuntime, NOT leftover
# BindRuntimeActors.
LOCKED = {
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission09IntegrationDirector::BindCampaignRuntime",
    "SkyguardMission08IntegrationDirector.cpp",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission09BindCampaignRuntimeDeclContractTests(unittest.TestCase):
    ...
    def test_missing_bind_campaign_runtime_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = "\tbool BindCampaignRuntime();\n"
        as_void = (
            "\tvoid BindCampaignRuntime("
            "USkyguardCampaignSubsystem* InCampaignRuntime);\n"
        )
        as_const = (
            "\tbool BindCampaignRuntime("
            "USkyguardCampaignSubsystem* InCampaignRuntime) const;\n"
        )
        const_ptr = (
            "\tbool BindCampaignRuntime("
            "const USkyguardCampaignSubsystem* InCampaignRuntime);\n"
        )
        by_ref = (
            "\tbool BindCampaignRuntime("
            "USkyguardCampaignSubsystem& InCampaignRuntime);\n"
        )
        renamed = "\tbool BindCampaign("
        "USkyguardCampaignSubsystem* InCampaignRuntime);\n"
    ...
    def test_bind_campaign_runtime_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, REQUIRED_DECL),
            REQUIRED_DECL,
        ...
        self.assertIn("BindCampaignRuntime", REQUIRED_DECL)
        self.assertIn("InCampaignRuntime", REQUIRED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tBindCampaignRuntime(USkyguardCampaignSubsystem* InCampaignRuntime);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission09 BindCampaignRuntime method contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                REQUIRED_DECL.lower(),
                "mission09 BindCampaignRuntime contains "
    ...
    def test_contract_is_bind_campaign_runtime_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_bind_campaign_runtime(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission09_bind_campaign_runtime"
                "_decl_contract.py"
            )
        )
        self.assertIn(
