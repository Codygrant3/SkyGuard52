# THIS IS leftover-safe ASkyguardApacheAircraft HydraLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> HydraLeft;`
# (no initializer).
...
# Stop BEFORE sibling HydraRight as a claimed slot.
# Do NOT claim HydraRight / HellfireLeft / HellfireRight /
# PylonLeft / PylonRight / GearLeft / GearRight /
# StubWingLeft / GunnerMount / PilotMount / EyeMount /
# WeaponMount / HullCollider / RotorPower.
# THIS IS leftover-safe ASkyguardApacheAircraft.HydraLeft.
# It is NOT leftover Nose field, NOT leftover analog
...
# leftover analog apache-cpg-feel,
# leftover analog gunship-weapon-stations.
# HydraRight / HellfireLeft / HellfireRight / PylonLeft
# are sibling slots being opened, not this slot.
# HydraLeft is a protected TObjectPtr rocket-pod mesh,
# NOT leftover LoadoutSpec RocketReserve /
# RocketMagazineSize, NOT leftover StormRainBeatKit
# bHydraForClusters, NOT leftover analog apache-cpg-feel,
# NOT leftover gunship-weapon-stations.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> HydraLeft;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> HydraLeft;` and NOT
# leftover `PylonLeft` / leftover `HydraRight` / leftover
# `RocketReserve` / leftover `bHydraForClusters` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> HydraLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> HydraLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> HydraLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> HydraLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> HydraLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> HydraLeft;"
...
STOP_BEFORE_HYDRA_RIGHT = "HydraRight"
...
HYDRA_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HydraLeft\s*;"
)
HYDRA_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HydraLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover PylonLeft / HydraRight / GearNose,
    # leftover LoadoutSpec RocketReserve / RocketMagazineSize,
    # leftover StormRainBeatKit bHydraForClusters, leftover
    # TObjectPtr<USceneComponent>, or an initializer on
    # HydraLeft as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if HYDRA_LEFT_DECL_RE.search(compact) is None:
        return False
    if HYDRA_LEFT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_HYDRA_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_HYDRA_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_HYDRA_RIGHT}"
        )
    for later in (
        SIBLING_HELLFIRE_LEFT,
        SIBLING_HELLFIRE_RIGHT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+HydraLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> HydraLeft "
...
class ApacheHydraLeftFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HydraLeft"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_HYDRA_RIGHT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_HYDRA_RIGHT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HydraLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("HydraLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_hydra_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> HydraLeft;", compact_origin)
        self.assertNotIn("HydraLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_hydra_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> HydraLeft"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> HydraLeft;")
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> HydraLeft;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> HydraLefts;\n",
            "\tint32 HydraLeft = 140;\n",
            "\tbool HydraLeft = true;\n",
            "\tfloat HydraLeft = " + forty + ";\n",
            "\tfloat HydraLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_hydra_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_HYDRA_RIGHT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache HydraLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "hydra_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
