# THIS IS leftover-safe ASkyguardApacheAircraft ChinBarrel.
...
# then BARE `TObjectPtr<UStaticMeshComponent> ChinBarrel;`
# (no initializer).
...
# Stop BEFORE sibling SensorTurret as a claimed slot.
# Do NOT claim ChinTurret / ChinHousing / SensorTurret /
...
# Do NOT parse leftover analog apache-aim-chin-turret
# UFUNCTION (that is not this TObjectPtr field).
# Do NOT parse leftover analog apache-get-chin-slew-scale /
# apache-get-chin-fire-scale / apache-is-chin-turret-down
# UFUNCTION decls.
# Do NOT parse leftover Apache UFUNCTION decls
# (#411-#426), including AimChinTurret / GetChinTurret.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> ChinBarrel;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent>` as the locked decl and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> ChinBarrel;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> ChinBarrel = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> ChinBarrel = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> ChinBarrel = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> ChinBarrel;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> ChinBarrel;"
...
STOP_BEFORE_SENSOR_TURRET = "SensorTurret"
...
CHIN_BARREL_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+ChinBarrel\s*;"
)
CHIN_BARREL_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+ChinBarrel\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer on ChinBarrel as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CHIN_BARREL_DECL_RE.search(compact) is None:
        return False
    if CHIN_BARREL_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_SENSOR_TURRET):
            break
    ...
    if has_identifier(window, SIBLING_SENSOR_TURRET):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_SENSOR_TURRET}"
        )
    for later in (
        SIBLING_SENSOR_BALL,
        SIBLING_NIGHT_VISION,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+ChinBarrel\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> ChinBarrel "
...
class ApacheChinBarrelFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ChinBarrel"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_SENSOR_TURRET), body)
        self.assertFalse(
            has_identifier(section, SIBLING_SENSOR_TURRET),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_NIGHT_VISION), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ChinBarrel"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ChinBarrel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_chin_barrel_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> ChinBarrel;", compact_origin)
        self.assertNotIn("ChinBarrel = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_chin_barrel_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> ChinBarrel"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> ChinBarrel;")
        self.assertNotEqual(LOCKED_DECL, "FName WeatherIdentity;")
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SCENE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CHIN_TURRET)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CHIN_HOUSING)
        self.assertNotIn("WeatherIdentity", LOCKED_DECL)
        self.assertNotIn("FName ", LOCKED_DECL)
        self.assertNotIn("USceneComponent", LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> ChinBarrels;\n",
            "\tint32 ChinBarrel = 140;\n",
            "\tbool ChinBarrel = true;\n",
            "\tfloat ChinBarrel = " + forty + ";\n",
            "\tfloat ChinBarrel = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_sensor_turret_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, SIBLING_CHIN_HOUSING))
        self.assertTrue(has_identifier(section, SIBLING_ROTOR_MAST))
        self.assertFalse(has_identifier(section, SIBLING_SENSOR_TURRET))
        self.assertFalse(has_identifier(section, SIBLING_SENSOR_BALL))
        ...
        self.assertNotIn(TARGET_WRONG_SENSOR_TURRET, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertFalse(has_identifier(section, SIBLING_HORIZONTAL_TAIL))
        self.assertTrue(has_identifier(leaked, SIBLING_SENSOR_BALL))
        self.assertFalse(has_identifier(section, SIBLING_SENSOR_BALL))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache ChinBarrel field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "chin_barrel_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
