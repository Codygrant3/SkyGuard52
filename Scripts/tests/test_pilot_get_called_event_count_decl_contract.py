...
# values, or the actual call count.
GET_CALLED_EVENT_COUNT = "int32 GetCalledEventCount();"
LOCKED_DECLARATION = GET_CALLED_EVENT_COUNT
LOCKED_DECLARATIONS = (GET_CALLED_EVENT_COUNT,)
# Leftover #56–#64 plus PilotVoice production sources/tests.
# This lane only adds an isolated Python GetCalledEventCount
...
# Isolated-test drafts stay off this lane. GetLastCalledLine /
# GetLastCalledText are in-flight sibling getters. MakeRadioLine
# (#285), LineTextForEvent (#283), ConfirmLineForCommand strings
...
# invent the actual line strings or the actual call count.
...
# Do not invent the actual call count or probe increment.
CALL_COUNT_VALUES_NOT_LOCKED = (
    "return CalledEventCount",
    "CalledEventCount =",
    "GCalledEventCount",
    "CalledEventCount++",
    "++CalledEventCount",
    "return GCalledEventCount",
    "int32 CalledEventCount",
    "CalledCount++",
    "GCalledEventCount++",
    "return 0;",
    "CalledEventCount = 0",
...
# .cpp bodies / invented return values / call-count tables stay
# unlocked.
CPP_AND_INVENTED = (
    "return ",
    "INDEX_NONE",
...
class PilotGetCalledEventCountDeclContractTests(unittest.TestCase):
    def test_pilot_voice_namespace_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(body, GET_CALLED_EVENT_COUNT), body)
    ...
    def test_missing_get_called_event_count_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_CALLED_EVENT_COUNT)
        self.assertIn("GetCalledEventCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(NAMESPACE_NAME, str(raised.exception))

    def test_neighbors_do_not_satisfy_get_called_event_count(self) -> None:
        neighbors_only = (
            f"namespace {NAMESPACE_NAME}\n"
            "{\n"
            "\tFString ConfirmLineForCommand(ESkyguardPilotCommand Command);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(body, GET_CALLED_EVENT_COUNT)
        self.assertIn("GetCalledEventCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(body, GET_CALLED_EVENT_COUNT))
    ...
    def test_get_called_event_count_declaration_matches_origin_main(
        self,
    ) -> None:
        body = namespace_body(origin_main_header())
        self.assertEqual(
            require_declaration(body, GET_CALLED_EVENT_COUNT),
            GET_CALLED_EVENT_COUNT,
        )
        self.assertEqual(LOCKED_DECLARATIONS, (GET_CALLED_EVENT_COUNT,))
        self.assertEqual(LOCKED_DECLARATION, GET_CALLED_EVENT_COUNT)
        self.assertTrue(has_declaration(body, GET_CALLED_EVENT_COUNT))
        self.assertEqual(declaration_count(body, GET_CALLED_EVENT_COUNT), 1)
        self.assertTrue(
            GET_CALLED_EVENT_COUNT.endswith(";"),
            GET_CALLED_EVENT_COUNT,
        )
        self.assertNotIn("INDEX_NONE", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("return ", GET_CALLED_EVENT_COUNT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        self.assertTrue(
            has_declaration(wrap_type, GET_CALLED_EVENT_COUNT),
            wrap_type,
        )
        self.assertTrue(
            has_declaration(wrap_args, GET_CALLED_EVENT_COUNT),
            wrap_args,
        )
        self.assertEqual(
            require_declaration(wrap_type, GET_CALLED_EVENT_COUNT),
            GET_CALLED_EVENT_COUNT,
        )
        self.assertEqual(
            require_declaration(wrap_args, GET_CALLED_EVENT_COUNT),
            GET_CALLED_EVENT_COUNT,
        )
        self.assertEqual(
            declaration_count(wrap_type, GET_CALLED_EVENT_COUNT),
            1,
        )
        self.assertEqual(
            declaration_count(wrap_args, GET_CALLED_EVENT_COUNT),
            1,
        )
        one_line = f"{{\n\t{GET_CALLED_EVENT_COUNT}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_CALLED_EVENT_COUNT))
        body = namespace_body(origin_main_header())
        self.assertTrue(has_declaration(body, GET_CALLED_EVENT_COUNT), body)
        self.assertEqual(
            require_declaration(body, GET_CALLED_EVENT_COUNT),
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("INDEX_NONE", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("NAME_None", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("{", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("}", GET_CALLED_EVENT_COUNT)
        ...
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, GET_CALLED_EVENT_COUNT)
            if token != "return ":
                self.assertNotIn(token, body)

    def test_declaration_does_not_invent_the_actual_call_count(self) -> None:
        body = namespace_body(origin_main_header())
        locked_only = f"{GET_CALLED_EVENT_COUNT}\n"
        for token in CALL_COUNT_VALUES_NOT_LOCKED:
            self.assertNotIn(token, GET_CALLED_EVENT_COUNT)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, body)
        self.assertNotIn("return CalledEventCount", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("CalledEventCount =", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("GCalledEventCount", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("CalledEventCount++", GET_CALLED_EVENT_COUNT)
    ...
    def test_declaration_does_not_invent_line_strings(self) -> None:
        ...
        for token in LINE_STRINGS_NOT_LOCKED:
            ...
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, body)
        self.assertNotIn("TEXT(", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("TEXT(", body)
        self.assertNotIn('return TEXT("', body)
        self.assertNotEqual(GET_CALLED_EVENT_COUNT, "Radar just lit us")
        self.assertNotEqual(GET_CALLED_EVENT_COUNT, "Missile inbound")

    def test_contract_does_not_relock_sibling_getters(self) -> None:
        locked_only = f"{GET_CALLED_EVENT_COUNT}\n"
        for neighbor in SIBLING_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CALLED_EVENT_COUNT)
            self.assertNotIn(neighbor, LOCKED_DECLARATIONS)
        self.assertNotIn("GetLastCalledLine", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("GetLastCalledText", GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_line_text_for_event(self) -> None:
        ...
        self.assertNotIn(LINE_TEXT_NOT_LOCKED, GET_CALLED_EVENT_COUNT)
        self.assertNotIn("LineTextForEvent", GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_make_radio_line(self) -> None:
        ...
        self.assertNotIn(MAKE_RADIO_LINE_NOT_LOCKED, GET_CALLED_EVENT_COUNT)
        self.assertNotIn("MakeRadioLine", GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_confirm_line_for_command(self) -> None:
        locked_only = f"{GET_CALLED_EVENT_COUNT}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_CALLED_EVENT_COUNT),
            GET_CALLED_EVENT_COUNT,
        )
        self.assertNotIn(CONFIRM_LINE_NOT_LOCKED, locked_only)
        self.assertNotIn(CONFIRM_LINE_NOT_LOCKED, GET_CALLED_EVENT_COUNT)
        self.assertNotIn("ConfirmLineForCommand", GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_line_duration(self) -> None:
        ...
        self.assertNotIn("LineDurationForEvent", GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_warn_lock_reload(self) -> None:
        ...
        for neighbor in WARN_LOCK_RELOAD_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CALLED_EVENT_COUNT)
        self.assertNotIn("WarnOffAxis", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("CallLock", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("CallReload", GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_call_probe(self) -> None:
        ...
        for neighbor in CALL_PROBE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CALLED_EVENT_COUNT)
        self.assertNotIn("ResetCallProbe", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("CallEvent", GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_pilot_line_enum(self) -> None:
        ...
        self.assertNotIn("enum class ESkyguardPilotLine", GET_CALLED_EVENT_COUNT)
        ...
        for name in PILOT_LINE_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(name, GET_CALLED_EVENT_COUNT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        for neighbor in UNLOCKED_NEIGHBORS:
            ...
            self.assertNotIn(neighbor, GET_CALLED_EVENT_COUNT)
        self.assertNotIn("MakeRadioLine", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("ConfirmCommand", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("LineTextForEvent", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("GetLastCalledLine", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("GetLastCalledText", GET_CALLED_EVENT_COUNT)
        self.assertNotIn("MakeRadioLine", locked_only)
    ...
    def test_contract_does_not_read_cpp_or_line_tables(self) -> None:
        ...
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, GET_CALLED_EVENT_COUNT)
        self.assertNotIn("SkyguardPilotVoice.cpp", body)
        self.assertNotIn("SkyguardPilotVoice::GetCalledEventCount", body)
        self.assertNotIn("switch (Line)", body)
        self.assertNotIn("return TEXT(", body)
        self.assertNotIn("return FString()", body)
        self.assertNotIn("return CalledEventCount", body)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(GET_CALLED_EVENT_COUNT, "Rifle")
        self.assertNotEqual(GET_CALLED_EVENT_COUNT, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"pilot GetCalledEventCount contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not Yak",
            )
            self.assertNotIn(banned, GET_CALLED_EVENT_COUNT.lower())
    ...
    def test_contract_is_get_called_event_count_declaration_only(self) -> None:
        header = origin_main_header()
        body = namespace_body(header)
        self.assertIn(f"namespace {NAMESPACE_NAME}", header)
        self.assertEqual(LOCKED_DECLARATIONS, (GET_CALLED_EVENT_COUNT,))
        ...
        self.assertNotIn("GetLastCalledText", LOCKED_DECLARATIONS)
