# THIS IS leftover-safe ASkyguardCampaignTheaterKit SilhouetteInstances.
...
# SilhouetteInstances;` (no initializer).
...
# BuildingInstances / LampInstances / NamedLandmarkMesh /
# AppliedSpec.
...
# the bulk). Analog: leftover briefing-card-defaults then
# isolated CardId field decls. Bulk string-contains
# SilhouetteInstances does not block this isolated UPROPERTY.
...
TARGET = (
    f"TObjectPtr<{HISM}> SilhouetteInstances;"
)
TARGET_WRONG_EQ_NONE = (
    f"TObjectPtr<{HISM}> SilhouetteInstances = nullptr;"
)
TARGET_WRONG_FALSE = (
    f"TObjectPtr<{HISM}> SilhouetteInstances = false;"
)
TARGET_WRONG_TRUE = (
    f"TObjectPtr<{HISM}> SilhouetteInstances = true;"
)
TARGET_WRONG_ZERO = (
    f"TObjectPtr<{HISM}> SilhouetteInstances = 0.f;"
)
TARGET_WRONG_FLOAT = "float SilhouetteInstances;"
TARGET_WRONG_SCENE = (
    "TObjectPtr<USceneComponent> SilhouetteInstances;"
)
TARGET_WRONG_STATIC = (
    "TObjectPtr<UStaticMeshComponent> SilhouetteInstances;"
)
...
PRIVATE_LAMPS = "Lamps"
GET_BUILDING_INSTANCE_COUNT = "GetBuildingInstanceCount"
GET_ROAD_INSTANCE_COUNT = "GetRoadInstanceCount"
GET_LAMP_INSTANCE_COUNT = "GetLampInstanceCount"
GET_SILHOUETTE_INSTANCE_COUNT = "GetSilhouetteInstanceCount"
APPLY_THEATER_KIT = "ApplyTheaterKit"
GET_APPLIED_SPEC = "GetAppliedSpec"
...
def leftover_spec_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_KIT_ID,
...
def leftover_theater_kit_spec_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_THEATER_KIT_SPEC_LANDMARK_SET,
        LEFTOVER_THEATER_KIT_SPEC_WEATHER_IDENTITY,
        LEFTOVER_THEATER_KIT_SPEC_KIT_ID,
...
def leftover_theater_kit_actor_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_THEATER_KIT_ROOT,
        LEFTOVER_THEATER_KIT_ROAD_INSTANCES,
        LEFTOVER_THEATER_KIT_BUILDING_INSTANCES,
...
def private_transient_fields() -> tuple[str, ...]:
    return (
        PRIVATE_CUBE_MESH,
...
def leftover_ufunction_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_APPLY,
        APPLY_THEATER_KIT,
        GET_BUILDING_INSTANCE_COUNT,
        GET_ROAD_INSTANCE_COUNT,
        GET_LAMP_INSTANCE_COUNT,
        GET_SILHOUETTE_INSTANCE_COUNT,
        GET_APPLIED_SPEC,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # SilhouetteInstances;`. Do not accept `= nullptr` /
    # `= false` / `= true` / `= 0.f` / `= 160.f` /
    # leftover float Health when origin/main has a bare
    # TObjectPtr. Do not accept sibling Root /
    # RoadInstances / BuildingInstances fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    type_name = re.escape(HISM)
    if re.search(
        rf"TObjectPtr\s*<\s*{type_name}\s*>\s*SilhouetteInstances\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SilhouetteInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USceneComponent\s*>\s*SilhouetteInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*UStaticMeshComponent\s*>\s*SilhouetteInstances\b",
        compact,
    ):
        return False
    if re.search(
...
def spec_section(header: str) -> str:
    raise AssertionError(
        f"do not parse {STRUCT_NAME}; isolated leftover spec "
        "fields #1299-#1314 stay locked"
    )
...
def actor_section(header: str) -> str:
    ...
    for token in private_transient_fields():
        if has_identifier(section, token):
            raise AssertionError(
                f"{ACTOR_NAME} parse window includes private {token}"
            )
    if STOP_BEFORE_NAMESPACE in section:
        raise AssertionError(
            f"{ACTOR_NAME} parse window includes {STOP_BEFORE_NAMESPACE}"
        )
    if "struct FSkyguardTheaterKitSpec" in section:
        raise AssertionError(
            f"{ACTOR_NAME} parse window includes class {ACTOR_NAME}"
        )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            rf"\s*TObjectPtr\s*<\s*{re.escape(HISM)}\s*>"
            r"\s*SilhouetteInstances\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr SilhouetteInstances is missing "
        f"from origin/main:{HEADER_PATH} class {ACTOR_NAME} "
        "public section"
    )
...
class TheaterKitSilhouetteInstancesFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SilhouetteInstances"), section)
        ...
        for leftover in leftover_spec_uncontracted_decls():
        ...
        for token in private_transient_fields():
            self.assertFalse(has_identifier(section, token), token)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        empty_actor = (
            ...
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
            f"\tTObjectPtr<{HISM}> {SIBLING_ROAD_INSTANCES};\n"
            "private:\n"
            "};\n"
        )
        section = actor_section(empty_actor)
        ...
        self.assertIn("SilhouetteInstances", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_silhouette_instances_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("SilhouetteInstances = nullptr", compact_origin)
        self.assertNotIn("SilhouetteInstances = false", compact_origin)
        self.assertNotIn("SilhouetteInstances = true", compact_origin)
        self.assertNotIn("SilhouetteInstances = 0.f", compact_origin)
        self.assertNotIn("SilhouetteInstances = 160.f", compact_origin)

    def test_silhouette_instances_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<"),
            LOCKED_DECL,
        )
        ...
        self.assertIn(HISM, LOCKED_DECL)
        self.assertIn("SilhouetteInstances", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tTObjectPtr<{HISM}> {SIBLING_ROAD_INSTANCES};\n",
            f"\tTObjectPtr<{HISM}> BuildingIdentities;\n",
            "\tint32 SilhouetteInstances;\n",
            "\tbool SilhouetteInstances;\n",
            "\tfloat SilhouetteInstances = " + forty + ";\n",
            "\tfloat SilhouetteInstances = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_theater_fields(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_APPLY, LOCKED_DECL)
        self.assertNotIn(GET_BUILDING_INSTANCE_COUNT, LOCKED_DECL)
        section = actor_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_spec_namespace_or_neighbors(self) -> None:
        header = origin_main_header()
        section = actor_section(header)
        ...
        for token in private_transient_fields():
            self.assertTrue(has_identifier(leaked, token), token)
            self.assertFalse(has_identifier(section, token), token)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit SilhouetteInstances field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "silhouette_instances_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_THEATER_KIT_WEATHER_APPLY, LOCKED)
        self.assertIn(LEFTOVER_THEATER_KIT_BUILDING_INSTANCES, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_THEATER_KIT_BUILDING_INSTANCES, THIS_SCRIPT)
        for leftover in leftover_theater_kit_spec_scripts():
            self.assertIn(leftover, LOCKED_SCRIPTS)
            self.assertNotEqual(leftover, THIS_SCRIPT)
        for leftover in leftover_theater_kit_actor_scripts():
            self.assertIn(leftover, LOCKED_SCRIPTS)
            self.assertNotEqual(leftover, THIS_SCRIPT)
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            LEFTOVER_THEATER_KIT_BULK,
            LEFTOVER_THEATER_KIT_BUILDING_INSTANCES,
            *leftover_theater_kit_spec_scripts(),
            *leftover_theater_kit_actor_scripts(),
            "Scripts/tests/test_briefing_card_defaults_contract.py",
            "Scripts/tests/test_briefing_radio_row_defaults_contract.py",
            "Scripts/tests/test_how_to_fly_row_defaults_contract.py",
            "Scripts/tests/test_briefing_card_priority_field_decl_contract.py",
