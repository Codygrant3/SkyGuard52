# THIS IS leftover-safe FSkyguardAudioTelemetry::PeakActiveVoices.
# origin/main form: split-line
# UPROPERTY(BlueprintReadOnly)
# then `int32 PeakActiveVoices = 0;` with no Category=.
# THIS IS UPROPERTY(BlueprintReadOnly) ONLY. It is NOT
# VisibleAnywhere. NO Category=. Initializer IS `= 0`
# (int32). If a clone asserts VisibleAnywhere or Category=
# and origin/main has none, drop those assertions. If a
# clone asserts `= false` / `= true` / `= 0.f` / bare decl,
# do not copy those; this field is `int32 PeakActiveVoices = 0;`.
# Fail-closed if the type/decl is missing or renamed, if
# Category= is added, if VisibleAnywhere is added, or if
...
# THIS IS NOT leftover audio-telemetry-requested-events
# #1281 / test_audio_telemetry_requested_events*.
# THIS IS NOT leftover audio-telemetry-played-events
# #1282 / test_audio_telemetry_played_events*.
# THIS IS NOT leftover audio-telemetry-rejected-by-cooldown
# #1283 / test_audio_telemetry_rejected_by_cooldown*.
...
# FSkyguardAudioTelemetry, stop at that struct's closing };
...
HEADER_PATH = "Source/Skyguard52/SkyguardAudioTypes.h"
STRUCT_NAME = "FSkyguardAudioTelemetry"
...
TARGET_WRONG_FLOAT = "int32 PeakActiveVoices = 0.f;"
TARGET_WRONG_SIBLING = "int32 RequestedEvents = 0;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `= 0` default. Do not
    # accept a bare decl or `= false` / `= true` / `= 0.f`
    # when origin/main has `int32 PeakActiveVoices = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+PeakActiveVoices\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PeakActiveVoices\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PeakActiveVoices\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
...
class AudioTelemetryPeakActiveVoicesFieldDeclContractTests(unittest.TestCase):
    def test_audio_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("PeakActiveVoices", section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            class_body(
                "struct FSkyguardUnrelatedTelemetry "
                ": public AActor\n{\n};\n"
            )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("PeakActiveVoices", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("PeakActiveVoices = 0", collapsed(origin))
        self.assertNotIn("PeakActiveVoices = false", collapsed(origin))
        self.assertNotIn("PeakActiveVoices = true", collapsed(origin))
        self.assertNotIn("PeakActiveVoices = 0.f", collapsed(origin))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool PeakActiveVoices = 0;\n",
            "\tint32 PeakActiveVoice = 0;\n",
            "\tint32 PeakActiveVoicess = 0;\n",
            "\tFText PeakActiveVoices = 0;\n",
            "\tint32 PeakActiveVoices{};\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            f"public:\n\t{LOCKED_DECL}\n",
            f"public:\n\t{UPROPERTY_READONLY}\n\t{LOCKED_DECL}\n",
            f"public:\n\t{UPROPERTY_READONLY} {LOCKED_DECL}\n",
            "public:\n\tUPROPERTY(BlueprintReadOnly)\n"
            f"\t{LOCKED_DECL}\n",
            "public:\n\tUPROPERTY(\n\t\tBlueprintReadOnly)\n"
            f"\t{LOCKED_DECL}\n",
    ...
    def test_does_not_contract_other_telemetry_or_result_fields(self) -> None:
        for field in OTHER_TELEMETRY_FIELDS:
            self.assertNotIn(field, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_does_not_parse_neighbors_or_director_methods(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIn(STOP_BEFORE_AUDIO_EVENT, header)
        self.assertIn(STOP_BEFORE_LISTENER, header)
        self.assertIn(STOP_BEFORE_EVENT_DEF, header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio-telemetry PeakActiveVoices field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_audio_telemetry_peak_active_voices"
            "_field_decl_contract.py"
        ))
