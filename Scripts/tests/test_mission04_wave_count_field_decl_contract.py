...
# INDEX_NONE or lock WaveCount
# construction in the .cpp. This is leftover-safe
# Mission04 IntegrationReadiness WaveCount
...
# an int32 readiness FIELD named WaveCount, NOT
# leftover Mission03 WaveCount FIELD #906 /
# branch cursor/mission03-wave-count-field-decl-
# contract-c332, NOT leftover Mission02 WaveCount
# FIELD #871 /
# branch cursor/mission02-wave-count-field-decl-
# contract-c332, NOT leftover Mission02
# bWavesReady FIELD #865, NOT leftover Mission03
# bWavesReady sibling, NOT leftover Mission03
# wave-state enum #0242 / leftover GetWaveState(),
# NOT leftover Mission02 wave-state enum / leftover
# Mission02 GetWaveState / leftover Mission02
# GetCurrentWaveIndex / leftover Mission02
# bWavesReady. It is NOT leftover Mission10
# WaveCount FIELD / branch cursor/mission10-
# wave-count-field-decl-contract-c332. It is
# NOT leftover Mission10 bBriefingReady FIELD #819 /
# branch cursor/mission10-briefing-ready-field-decl-
# contract-c332, NOT leftover Mission01 Briefing
...
# (not ASkyguardMission04IntegrationDirector public
# fields, not leftover GetWaveState(), not leftover
# Mission04 wave-state enum #bb22, not leftover
# Mission04 bWavesReady #933, not leftover
# Mission03 WaveCount FIELD #906, not leftover
# Mission02 WaveCount FIELD #871, not leftover
# Mission10 WaveCount FIELD #824, not leftover
...
# drafts #674-#864 including leftover Mission10
...
# drafts #674-#867 including leftover Mission10
...
# drafts #536-#673 or open drafts #674-#867.
...
# an isolated Python WaveCount
# field declaration contract on
# FSkyguardMission04IntegrationReadiness.
# THIS IS NOT leftover Mission02 WaveCount #871,
# leftover Mission03 WaveCount #906, leftover
# Mission10 WaveCount #824, leftover Mission04
# bWavesReady #933, leftover Mission04 GetWaveState.
LOCKED = {
...
LEFTOVER_MISSION03_WAVE_COUNT_FIELD_NOT_LOCKED = (
...
LEFTOVER_MISSION10_OBJECTIVE_COUNT_FIELD_NOT_LOCKED = (
...
class Mission04WaveCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("WaveCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_wave_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_wave_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 WaveCount"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("= 0;"), LOCKED_DECL)
        self.assertIn("WaveCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("WaveCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 WaveCount"))
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        self.assertNotIn("ESkyguardMission02WaveState", LOCKED_DECL)
        self.assertNotIn("ESkyguardMission03WaveState", LOCKED_DECL)
        self.assertIn(
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("WaveCount", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission04 WaveCount field contract "
    ...
    def test_contract_is_root_field_declaration_only(
        ...
        self.assertNotIn("GetCurrentWaveIndex", LOCKED_DECL)
        self.assertNotIn("ESkyguardMission02WaveState", LOCKED_DECL)
    ...
    def test_leftover_mission03_wave_count_does_not_satisfy(self) -> None:
        leftover = (
        ...
        with self.assertRaises(AssertionError) as raised:
            public_section(leftover_director)
        self.assertIn(CLASS_NAME, str(raised.exception))
        ...
        with self.assertRaises(AssertionError) as raised:
            public_section(leftover_searchlight)
        self.assertIn(CLASS_NAME, str(raised.exception))
        leftover_waves = "\tbool bWavesReady = false;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_waves, LOCKED_DECL)
        self.assertIn("WaveCount", str(raised.exception))
        self.assertFalse(has_declaration(leftover_waves, LOCKED_DECL))
        for token in LEFTOVER_MISSION03_WAVE_COUNT_FIELD_NOT_LOCKED:
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION04_WAVES_READY_NOT_LOCKED:
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION04_SEARCHLIGHTS_READY_NOT_LOCKED:
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn(
            "Scripts/tests/test_mission03_wave_count"
            "_field_decl_contract.py",
    ...
    def test_leftover_mission10_wave_count_does_not_satisfy(self) -> None:
        leftover = (
        ...
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        leftover_field = "\tint32 ObjectiveCount = 0;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_field, LOCKED_DECL)
        self.assertIn("WaveCount", str(raised.exception))
        self.assertFalse(has_declaration(leftover_field, LOCKED_DECL))
        leftover_enum = (
            "\tESkyguardMission02WaveState GetWaveState() const;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_enum, LOCKED_DECL)
        self.assertIn("WaveCount", str(raised.exception))
        self.assertNotIn("UENUM", LOCKED_DECL)
        self.assertNotIn("enum class", LOCKED_DECL)
        for token in LEFTOVER_MISSION10_WAVE_COUNT_FIELD_NOT_LOCKED:
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION10_OBJECTIVE_COUNT_FIELD_NOT_LOCKED:
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION02_BRIEFING_READY_NOT_LOCKED:
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn(
            "Scripts/tests/test_mission10_wave_count"
            "_field_decl_contract.py",
