# THIS IS leftover-safe FSkyguardEnemyWaveDefinition CompletionObjectiveId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName CompletionObjectiveId;` (no initializer).
...
# StartTimeSeconds / Formations.
# THIS IS NOT leftover isolated WaveId field decl
# (leftover draft being opened).
...
# isolated CompletionObjectiveId field decl, same as leftover
...
# `FName PointId` / `FName WaveId`, retarget:
# type is FName CompletionObjectiveId,
...
TARGET = "FName CompletionObjectiveId;"
TARGET_WRONG_EQ_NONE = "FName CompletionObjectiveId = NAME_None;"
TARGET_WRONG_FALSE = "FName CompletionObjectiveId = false;"
TARGET_WRONG_TRUE = "FName CompletionObjectiveId = true;"
TARGET_WRONG_ZERO = "FName CompletionObjectiveId = 0.f;"
TARGET_WRONG_FLOAT = "float CompletionObjectiveId;"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+CompletionObjectiveId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName CompletionObjectiveId is missing from "
...
def require_completion_objective_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} CompletionObjectiveId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} CompletionObjectiveId"
        )
...
class EnemyWaveCompletionObjectiveIdFieldDeclContractTests(unittest.TestCase):
    def test_enemy_wave_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CompletionObjectiveId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CompletionObjectiveId"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_DEFINITION, section)
        ...
        self.assertIn("CompletionObjectiveId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_completion_objective_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tfloat {SIBLING_START_TIME} = 0.f;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_completion_objective_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName CompletionObjectiveId;", compact_origin)
        self.assertNotIn("CompletionObjectiveId = NAME_None", compact_origin)
        self.assertNotIn("CompletionObjectiveId = false", compact_origin)
        self.assertNotIn("CompletionObjectiveId = true", compact_origin)
        self.assertNotIn("CompletionObjectiveId = 0.f", compact_origin)
        self.assertNotIn("CompletionObjectiveId = 160.f", compact_origin)

    def test_completion_objective_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName CompletionObjectiveId"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFName CompletionObjectiveIds;\n",
            "\tint32 CompletionObjectiveId;\n",
            "\tbool CompletionObjectiveId;\n",
            "\tfloat CompletionObjectiveId = " + forty + ";\n",
            "\tfloat CompletionObjectiveId = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_completion_objective_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_objective_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_objective_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_objective_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_objective_id_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "enemy-wave CompletionObjectiveId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enemy_wave_completion_objective_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
