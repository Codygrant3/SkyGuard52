...
# INDEX_NONE or lock Phase construction in the .cpp.
# Lock field-declaration presence of
# `ESkyguardBossPhase Phase;`. origin/main is
# `ESkyguardBossPhase Phase =
# ESkyguardBossPhase::Approach;`. Accept that
# live initializer without adding leftover
# enum contract #60cb. Accept the no-initializer
# presence form and other one-line /
...
# Harbor 40/80.
PHASE_FIELD = "ESkyguardBossPhase Phase;"
PHASE_FIELD_LIVE = (
    "ESkyguardBossPhase Phase = ESkyguardBossPhase::Approach;"
)
PHASE_FIELD_RE = re.compile(
    r"ESkyguardBossPhase\s+Phase"
    r"(?:\s*=\s*ESkyguardBossPhase\s*::\s*Approach)?"
    r"\s*;"
)
UPROPERTY_PHASE = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
...
# Leftover Root field contract stays sibling-only.
GET_BOSS_PHASE = (
...
# Python Phase field declaration
...
# #48e1, leftover Root field, leftover
...
# specifiers. Neighbor WeakPoints / Root may
...
# .cpp Phase body / invented INDEX_NONE stay
...
def _phase_field_forms() -> tuple[str, ...]:
    return (PHASE_FIELD, PHASE_FIELD_LIVE)


def has_declaration(region: str, declaration: str) -> bool:
    if declaration in _phase_field_forms():
        return PHASE_FIELD_RE.search(region) is not None
...
def declaration_count(region: str, declaration: str) -> int:
    if declaration in _phase_field_forms():
        return len(PHASE_FIELD_RE.findall(region))
    if declaration in region:
...
class BossDronePhaseFieldDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, PHASE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, PHASE_FIELD)
    ...
    def test_missing_phase_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, PHASE_FIELD)
        self.assertIn("Phase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_PHASE}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, PHASE_FIELD)
        self.assertIn("Phase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PHASE, section)
        ...
        self.assertNotIn("UPROPERTY", PHASE_FIELD)
        self.assertNotIn("VisibleAnywhere", PHASE_FIELD)
        self.assertNotIn("BlueprintReadOnly", PHASE_FIELD)
        self.assertNotIn("Category", PHASE_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_PHASE)
        self.assertNotIn("BlueprintCallable", UPROPERTY_PHASE)
        self.assertIn("Skyguard|Boss", UPROPERTY_PHASE)
        self.assertNotIn("Destruction", UPROPERTY_PHASE)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_PHASE)
            self.assertNotIn(invented, PHASE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, PHASE_FIELD)
        self.assertIn("Phase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        leftover_wrong_type = "\tESkyguardPilotCommand Phase;\n"
        leftover_int = "\tint32 Phase;\n"
        leftover_zero = "\tESkyguardBossPhase Phase = 0;\n"
        leftover_invented = (
            "\tESkyguardBossPhase Phase = "
            "ESkyguardBossPhase::Invented;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, PHASE_FIELD)
            self.assertIn("Phase", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_phase_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, PHASE_FIELD),
            1,
        )
        self.assertTrue(
            PHASE_FIELD.startswith("ESkyguardBossPhase "),
            PHASE_FIELD,
        )
        self.assertTrue(
            PHASE_FIELD.endswith(";"),
            PHASE_FIELD,
        )
        self.assertIn("ESkyguardBossPhase", PHASE_FIELD)
        self.assertIn("Phase", PHASE_FIELD)
        self.assertNotIn("GetBossPhase", PHASE_FIELD)
        self.assertNotIn("=", PHASE_FIELD)
        self.assertTrue(
            has_declaration(PHASE_FIELD_LIVE, PHASE_FIELD),
            PHASE_FIELD_LIVE,
        )
        self.assertTrue(
            has_declaration(f"\t{PHASE_FIELD_LIVE}\n", PHASE_FIELD),
        )
        self.assertNotIn("INDEX_NONE", PHASE_FIELD)
        self.assertNotIn("UFUNCTION", PHASE_FIELD)
        self.assertNotIn("GetBossPhase", PHASE_FIELD)
        self.assertNotIn("{", PHASE_FIELD)
        self.assertNotIn("}", PHASE_FIELD)
        self.assertNotIn("return ", PHASE_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            PHASE_FIELD,
        )
        self.assertNotIn("ApplyWeaponHit", PHASE_FIELD)
        self.assertNotIn("IssuePilotCommand", PHASE_FIELD)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            PHASE_FIELD,
        )
        ...
        self.assertNotIn("GetDefeatDebrisPieceCount", PHASE_FIELD)
        self.assertNotIn("GetTelemetry", PHASE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PHASE_FIELD)
        self.assertNotIn("GetGunnerMount", PHASE_FIELD)
        self.assertNotIn("FaceWorldLocation", PHASE_FIELD)
        self.assertNotIn("WeakPoints", PHASE_FIELD)
        self.assertNotIn("SetSearchlightTracked", PHASE_FIELD)
        self.assertNotIn("SetContactIdentified", PHASE_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            PHASE_FIELD,
        )
        self.assertNotIn("SetLightningExposed", PHASE_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            PHASE_FIELD,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, PHASE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_live_split = (
            "public:\n"
            "\tESkyguardBossPhase\n"
            "\tPhase = ESkyguardBossPhase::Approach;\n"
            "};\n"
        )
        wrap_live_assign = (
            "public:\n"
            "\tESkyguardBossPhase Phase =\n"
            "\t\tESkyguardBossPhase::Approach;\n"
            "};\n"
        )
        wrap_uproperty = (
            "public:\n"
        ...
        wrap_uproperty_live_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PHASE} {PHASE_FIELD_LIVE}\n"
            "};\n"
        )
        header_wrap_type = (
            f"class SKYGUARD52_API {CLASS_NAME} "
        ...
        header_wrap_live_split = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public AActor\n{{\n{wrap_live_split}"
        )
        header_wrap_live_assign = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public AActor\n{{\n{wrap_live_assign}"
        )
        header_wrap_uproperty_live = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public AActor\n{{\n{wrap_uproperty_live}"
        )
        header_wrap_uproperty_live_one_line = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public AActor\n{{\n{wrap_uproperty_live_one_line}"
        )
        for header in (
        ...
        one_line = f"{{\npublic:\n\t{PHASE_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, PHASE_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, PHASE_FIELD),
    ...
    def test_live_initializer_is_accepted(self) -> None:
        live = f"\t{PHASE_FIELD_LIVE}\n"
        self.assertTrue(has_declaration(live, PHASE_FIELD), live)
        self.assertEqual(
            require_declaration(live, PHASE_FIELD),
            PHASE_FIELD,
        )
        self.assertEqual(declaration_count(live, PHASE_FIELD), 1)
        presence = f"\t{PHASE_FIELD}\n"
        self.assertTrue(has_declaration(presence, PHASE_FIELD), presence)
        wrong_zero = "\tESkyguardBossPhase Phase = 0;\n"
        wrong_invented = (
            "\tESkyguardBossPhase Phase = "
            "ESkyguardBossPhase::Invented;\n"
        )
        for region in (wrong_zero, wrong_invented):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, PHASE_FIELD)
            self.assertIn("Phase", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, PHASE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (body, weak_points, root, telemetry, command):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, PHASE_FIELD)
            self.assertIn("Phase", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PHASE, section)
        self.assertTrue(has_declaration(section, PHASE_FIELD), section)

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{PHASE_FIELD}\n"
        self.assertNotIn("UFUNCTION", PHASE_FIELD)
        self.assertNotIn("UFUNCTION", locked_only)
        self.assertFalse(PHASE_FIELD.startswith("UFUNCTION"), PHASE_FIELD)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, PHASE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", PHASE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_phase_cpp_body(
        self,
    ) -> None:
        locked_only = f"{PHASE_FIELD}\n"
        self.assertNotIn("{", PHASE_FIELD)
        ...
        self.assertNotIn("return false", PHASE_FIELD)
        self.assertNotIn("return true", PHASE_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            PHASE_FIELD,
        )
        self.assertNotIn("SetBossPhase", PHASE_FIELD)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            PHASE_FIELD,
        )
        self.assertNotIn("RegisterDefeatDebris", PHASE_FIELD)
        self.assertNotIn("CleanupDefeatDebris", PHASE_FIELD)
        self.assertNotIn("HandleDefeated", PHASE_FIELD)

    def test_contract_does_not_relock_get_boss_phase_sibling(self) -> None:
        locked_only = f"{PHASE_FIELD}\n"
        for token in GET_BOSS_PHASE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("GetBossPhase", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_root_field_sibling(self) -> None:
        ...
        for token in ROOT_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("Root", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_public_fields(self) -> None:
        ...
        for token in PUBLIC_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("Root", PHASE_FIELD)
        self.assertNotIn("BodyMesh", PHASE_FIELD)
        self.assertNotIn("WeakPoints", PHASE_FIELD)
        self.assertNotIn("CurrentPilotCommand", PHASE_FIELD)
        self.assertNotIn("Telemetry", PHASE_FIELD)
        self.assertNotIn("OnBossPhaseChanged", PHASE_FIELD)
        self.assertNotIn("OnPilotCommandNative", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_hull_collider(self) -> None:
        ...
        for token in LEFTOVER_HULL_COLLIDER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("HullCollider", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, PHASE_FIELD)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", PHASE_FIELD)
        self.assertNotIn("PortGuidanceArray", PHASE_FIELD)
        self.assertNotIn("CommandCore", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("SetSearchlightTracked", PHASE_FIELD)
        self.assertNotIn("IsSearchlightTracked", PHASE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("SetContactIdentified", PHASE_FIELD)
        self.assertNotIn("IsContactIdentified", PHASE_FIELD)
        self.assertNotIn("OpenOrbitExposure", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", PHASE_FIELD)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            PHASE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", PHASE_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", PHASE_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            PHASE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("GetGunnerMount", PHASE_FIELD)
        self.assertNotIn("GetChinTurret", PHASE_FIELD)
        self.assertNotIn("GetSensorTurret", PHASE_FIELD)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{PHASE_FIELD}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, PHASE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("ASkyguardGunner", PHASE_FIELD)
        self.assertNotIn("FillAndFinalize", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("SkyguardRadarNode", PHASE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", PHASE_FIELD)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            PHASE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PHASE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", PHASE_FIELD)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            PHASE_FIELD,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PHASE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PHASE_FIELD)
        self.assertNotIn("MaxIntegrity", PHASE_FIELD)
        self.assertNotIn("CurrentIntegrity", PHASE_FIELD)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            PHASE_FIELD,
        )
        self.assertNotIn("SkyguardPathfinder", PHASE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, PHASE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{PHASE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, PHASE_FIELD),
            PHASE_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone Phase field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, PHASE_FIELD.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, PHASE_FIELD)

    def test_contract_is_phase_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", PHASE_FIELD)
        self.assertNotIn("{", PHASE_FIELD)
        self.assertTrue(PHASE_FIELD.startswith("ESkyguardBossPhase "))
        self.assertTrue(PHASE_FIELD.endswith(";"))
        self.assertIn("ESkyguardBossPhase", PHASE_FIELD)
        self.assertIn("Phase", PHASE_FIELD)
        self.assertNotIn("=", PHASE_FIELD)
        self.assertIn(UPROPERTY_PHASE, section)
        self.assertTrue(has_declaration(PHASE_FIELD_LIVE, PHASE_FIELD))
