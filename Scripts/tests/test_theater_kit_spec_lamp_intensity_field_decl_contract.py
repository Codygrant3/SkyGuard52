# THIS IS leftover-safe FSkyguardTheaterKitSpec LampIntensity.
...
# BlueprintReadOnly IS present. Default MUST be `= 0.f`.
...
# LampColor / RoadTreatment / RoadTint / NamedLandmark /
...
# the bulk).
# THIS IS NOT leftover sibling drafts #1299-#1302.
...
# / Category="Skyguard|Campaign", retarget: type is float,
# default is `= 0.f`, Category is Skyguard|Theater.
# Harbor `40` + `.f` / `80` + `.f` still fail-closed
# (this default is `0.f`, not those).
...
TARGET = "float LampIntensity = 0.f;"
TARGET_WRONG_EQ_NONE = "float LampIntensity = NAME_None;"
TARGET_WRONG_FALSE = "float LampIntensity = false;"
TARGET_WRONG_TRUE = "float LampIntensity = true;"
TARGET_WRONG_BARE = "float LampIntensity;"
TARGET_WRONG_FNAME = "FName LampIntensity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_ONE_SIXTY = "float LampIntensity = 160.f;"
TARGET_WRONG_WHITE = "float LampIntensity = FLinearColor::White;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float LampIntensity = 0.f;`.
    # Do not accept bare float / FName / no default /
    # `= NAME_None` / `= false` / `= true` / `= 160.f` /
    # harbor forty/eighty / leftover float Health when
    # origin/main default is `= 0.f`.
    ...
    if re.search(
        r"FName\s+LampIntensity\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LampIntensity\s*;",
        compact,
    ):
        return False
    assigned = re.findall(
        r"float\s+LampIntensity\s*=\s*(\S[^;]*);",
        compact,
    )
    if not assigned:
        return False
    if any(value.strip() != "0.f" for value in assigned):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LampIntensity\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LampIntensity is missing from "
...
class TheaterKitSpecLampIntensityFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LampIntensity"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LampIntensity"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LampIntensity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lamp_intensity_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_decl_fails_closed_when_origin_has_zero_default(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
        ...
        self.assertIn("float LampIntensity = 0.f;", compact_origin)
        self.assertNotIn("LampIntensity = NAME_None", compact_origin)
        self.assertNotIn("LampIntensity = false", compact_origin)
        self.assertNotIn("LampIntensity = true", compact_origin)
        self.assertNotIn("LampIntensity = 160.f", compact_origin)
        self.assertNotIn("LampIntensity = FLinearColor::White", compact_origin)

    def test_lamp_intensity_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LampIntensity"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("= FLinearColor::White", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_WHITE}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_WHITE}\n",
            f"\t{TARGET_WRONG_HEALTH}\n",
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            f"\tFName {SIBLING_WEATHER_IDENTITY};\n",
            "\tfloat LampIntensities = 0.f;\n",
            "\tint32 LampIntensity;\n",
            "\tbool LampIntensity;\n",
            "\tFName LampIntensity;\n",
            "\tfloat LampIntensity = " + forty + ";\n",
            "\tfloat LampIntensity = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec LampIntensity field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "lamp_intensity_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
