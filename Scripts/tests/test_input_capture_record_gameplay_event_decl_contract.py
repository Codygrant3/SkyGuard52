...
# or lock the RecordGameplayEvent body in the .cpp.
...
# observer records an accepted gameplay transition; the
# method never causes it. Stay off sibling
# RecordPlayerEvent (this wave) and leftover
# IsCaptureActive (#427). Stay off leftover Initialize /
# Deinitialize / Tick / GetStatId / IsTickable
# overrides. Stay off leftover settings-apply-broadcast
# #1268. Stay off leftover Gunner, leftover
...
# Apache MaxIntegrity, leftover settings
# SetInvertVerticalLook (#428), leftover
# IsCaptureActive (#427), and leftover
...
# RecordGameplayEvent declaration contract on
# USkyguardInputCombatPerformanceCapture. Stay off
# RecordPlayerEvent, leftover IsCaptureActive (#427),
...
# settings contracts including leftover
# SetInvertVerticalLook (#428), leftover apache
# aircraft isolated contracts, leftover gun-fire
# camera shake, leftover sortie-hud-host fail-closed,
# leftover input/combat performance capture siblings,
# leftover IsCaptureActive (#427), and dirty
# workspace paths.
LOCKED = {
...
# RecordPlayerEvent sibling, leftover IsCaptureActive
# (#427), leftover Initialize / Deinitialize / Tick /
# GetStatId / IsTickable overrides, leftover
...
# Tick / GetStatId / IsTickable overrides, sibling
# RecordPlayerEvent, and leftover IsCaptureActive
# (#427) stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# Leftover IsCaptureActive (#427) stays sibling-only.
...
# .cpp RecordGameplayEvent body / invented INDEX_NONE
...
class InputCaptureRecordGameplayEventDeclContractTests(unittest.TestCase):
    def test_input_capture_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, RECORD_GAMEPLAY_EVENT), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, RECORD_GAMEPLAY_EVENT)
        self.assertIn("RecordGameplayEvent", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, RECORD_GAMEPLAY_EVENT))

    def test_missing_record_gameplay_event_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, RECORD_GAMEPLAY_EVENT)
        self.assertIn("RecordGameplayEvent", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, RECORD_GAMEPLAY_EVENT)
        self.assertIn("RecordGameplayEvent", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tstatic void RecordGameplayEvent;\n"
        missing_static = (
            "\tvoid RecordGameplayEvent("
            "const UObject* WorldContext, FName EventName);\n"
        )
        wrong_return_bool = (
            "\tstatic bool RecordGameplayEvent("
            "const UObject* WorldContext, FName EventName);\n"
        )
        wrong_return_int = (
            "\tstatic int32 RecordGameplayEvent("
            "const UObject* WorldContext, FName EventName);\n"
        )
        added_const = (
        ...
        added_arg = (
            "\tstatic void RecordGameplayEvent("
            "const UObject* WorldContext, FName EventName, bool Extra);\n"
        )
        no_args = "\tstatic void RecordGameplayEvent();\n"
        missing_event = (
            "\tstatic void RecordGameplayEvent("
            "const UObject* WorldContext);\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, RECORD_GAMEPLAY_EVENT)
            self.assertIn("RecordGameplayEvent", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_record_gameplay_event_declaration_matches_origin_main(
        self,
    ) -> None:
        ...
        self.assertTrue(has_declaration(section, RECORD_GAMEPLAY_EVENT))
        self.assertEqual(declaration_count(section, RECORD_GAMEPLAY_EVENT), 1)
        self.assertTrue(
            RECORD_GAMEPLAY_EVENT.startswith("static void "),
            RECORD_GAMEPLAY_EVENT,
        )
        self.assertTrue(
            RECORD_GAMEPLAY_EVENT.endswith(";"),
            RECORD_GAMEPLAY_EVENT,
        )
        self.assertIn("RecordGameplayEvent(", RECORD_GAMEPLAY_EVENT)
        self.assertIn("const UObject* WorldContext", RECORD_GAMEPLAY_EVENT)
        self.assertIn("FName EventName", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("INDEX_NONE", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("{", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("}", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("return ", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn(" const;", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("UFUNCTION", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("RecordPlayerEvent", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("IsCaptureActive", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("Initialize", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("Deinitialize", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("GetStatId", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("IsTickable", RECORD_GAMEPLAY_EVENT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, RECORD_GAMEPLAY_EVENT),
                section,
            )
            self.assertEqual(
                require_declaration(section, RECORD_GAMEPLAY_EVENT),
            ...
            self.assertEqual(
                declaration_count(section, RECORD_GAMEPLAY_EVENT),
                1,
            )
        one_line = f"{{\npublic:\n\t{RECORD_GAMEPLAY_EVENT}\n}}\n"
        self.assertTrue(has_declaration(one_line, RECORD_GAMEPLAY_EVENT))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, RECORD_GAMEPLAY_EVENT), section)
        self.assertEqual(
            require_declaration(section, RECORD_GAMEPLAY_EVENT),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{RECORD_GAMEPLAY_EVENT}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", RECORD_GAMEPLAY_EVENT)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_invent_ufunction(self) -> None:
        ...
        self.assertNotIn("BlueprintCallable", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("BlueprintPure", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("UFUNCTION", section)

    def test_contract_does_not_lock_record_gameplay_event_cpp_body(self) -> None:
        locked_only = f"{RECORD_GAMEPLAY_EVENT}\n"
        self.assertNotIn("{", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("}", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("return ", RECORD_GAMEPLAY_EVENT)
        ...
        self.assertNotIn("return false", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("return true", RECORD_GAMEPLAY_EVENT)

    def test_contract_does_not_relock_leftover_overrides(self) -> None:
        locked_only = f"{RECORD_GAMEPLAY_EVENT}\n"
        for neighbor in LEFTOVER_OVERRIDES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("Initialize", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("Deinitialize", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("GetStatId", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("IsTickable", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("DeltaTime", RECORD_GAMEPLAY_EVENT)
    ...
    def test_contract_does_not_relock_leftover_empty_fail_closed(self) -> None:
        ...
        for token in LEFTOVER_EMPTY_FAIL_CLOSED_NOT_LOCKED:
            ...
            self.assertNotIn(token, RECORD_GAMEPLAY_EVENT)
        self.assertNotIn(
            "test_apache_aircraft_empty_fail_closed.py",
            RECORD_GAMEPLAY_EVENT,
        )
    ...
    def test_contract_does_not_relock_leftover_chin_muzzle(self) -> None:
        ...
        for token in LEFTOVER_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("GetChinMuzzleLocation", RECORD_GAMEPLAY_EVENT)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("ESkyguardApacheSystem", RECORD_GAMEPLAY_EVENT)
    ...
    def test_contract_does_not_relock_leftover_settings(self) -> None:
        ...
        self.assertNotIn("ApplySettings", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("bInvertLook", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("SetInvertVerticalLook", RECORD_GAMEPLAY_EVENT)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("FillAndFinalize", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("FillAndFail", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("ASkyguardGunner", RECORD_GAMEPLAY_EVENT)
    ...
    def test_contract_does_not_relock_leftover_apache_decl_siblings(
        ...
        for token in LEFTOVER_APACHE_DECL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("FaceWorldLocation", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("AimChinTurret", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("GetRotorRPM", RECORD_GAMEPLAY_EVENT)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("USkyguardBriefingWidget", RECORD_GAMEPLAY_EVENT)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, RECORD_GAMEPLAY_EVENT)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("ESkyguardMissionSkylineStyle", RECORD_GAMEPLAY_EVENT)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{RECORD_GAMEPLAY_EVENT}\n"
        self.assertEqual(
            require_declaration(locked_only, RECORD_GAMEPLAY_EVENT),
            RECORD_GAMEPLAY_EVENT,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "input capture RecordGameplayEvent contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, RECORD_GAMEPLAY_EVENT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"input capture RecordGameplayEvent contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, RECORD_GAMEPLAY_EVENT)

    def test_contract_is_record_gameplay_event_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", RECORD_GAMEPLAY_EVENT)
        self.assertNotIn("{", RECORD_GAMEPLAY_EVENT)
        self.assertTrue(RECORD_GAMEPLAY_EVENT.startswith("static void "))
        self.assertTrue(RECORD_GAMEPLAY_EVENT.endswith(";"))
