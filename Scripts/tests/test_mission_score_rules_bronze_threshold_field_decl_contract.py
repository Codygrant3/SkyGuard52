...
# if the initializer is not `= 5000`, if meta ClampMin
...
# Parse STRUCT `FSkyguardMissionScoreRules` body ONLY after
# `struct FSkyguardMissionScoreRules`. Stop at
# `struct FSkyguardObjectiveProgress` and later structs.
# Do NOT parse `FSkyguardWeatherProfile`
# (leftover weather-profile-defaults #184 / #7dd0).
# Do NOT parse `FSkyguardMissionPresentation`
# (leftover mission-presentation-defaults #157e).
...
# (leftover mission-definition ScoreRules field decls).
# Do NOT parse leftover mission-score-rules-defaults
# #185 / #deae.
...
# Do NOT parse leftover #1332-#1356 RadioLine / RoutePoint /
# EnemyWave / EnemyFormation / WeatherProfile isolated
# field drafts.
# Do NOT parse leftover TheaterKit #1299-#1331.
# Do NOT contract sibling fields CompletionScore /
# PerfectAccuracyBonus / NoDamageBonus / SilverThreshold /
# GoldThreshold.
# Those sibling decls remain unclaimed.
# THIS IS NOT leftover mission-score-rules-defaults
# #185 / #deae.
# THIS IS NOT leftover weather-profile-defaults #184 / #7dd0.
...
# #153 / #f47d.
# THIS IS NOT leftover mission-definition ScoreRules.
...
# the bulk).
# Analog: leftover mission-score-rules-defaults then this
# isolated BronzeThreshold field decl, same as leftover
# weather-profile-defaults then isolated WindSpeed,
...
# `FName WeatherIdentity` / `float BronzeThreshold`
# / `int32 UnitCount` / no `=` / ClampMin missing,
# retarget: type is int32 BronzeThreshold,
# initializer is `= 5000`, specifiers are EditAnywhere +
...
TARGET = "int32 BronzeThreshold = 5000;"
TARGET_WRONG_EQ_NONE = "int32 BronzeThreshold = NAME_None;"
TARGET_WRONG_FALSE = "int32 BronzeThreshold = false;"
TARGET_WRONG_TRUE = "int32 BronzeThreshold = true;"
TARGET_WRONG_ZERO = "int32 BronzeThreshold = 0;"
TARGET_WRONG_FLOAT = "float BronzeThreshold = 5000;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "int32 BronzeThreshold;"
TARGET_WRONG_BONUS = "int32 BronzeThreshold = 2500;"
TARGET_WRONG_FNAME = "FName BronzeThreshold;"
TARGET_WRONG_COMPLETION = "int32 CompletionScore = 5000;"
...
SIBLING_PERFECT_ACCURACY = "PerfectAccuracyBonus"
SIBLING_NO_DAMAGE = "NoDamageBonus"
SIBLING_COMPLETION = "CompletionScore"
SIBLING_SILVER = "SilverThreshold"
SIBLING_GOLD = "GoldThreshold"
SIBLING_PROFILE_ID = "ProfileId"
...
LEFTOVER_MISSION_SCORE_RULES_DEFAULTS_PY = (
    "Scripts/tests/test_mission_score_rules_defaults.py"
)
LEFTOVER_MISSION_SCORE_RULES_DEFAULTS_TESTS = (
    "Scripts/tests/test_mission_score_rules_defaults_tests.py"
)
LEFTOVER_ENEMY_FORMATION_UNIT_COUNT = (
    "Scripts/tests/test_enemy_formation_unit_count"
    "_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Reject missing initializer, `= 0` / `= 2500` /
    # Harbor 40/80 / leftover Health / float / FName /
    # sibling CompletionScore = 5000. Do not accept
    # sibling CompletionScore / PerfectAccuracyBonus /
    # NoDamageBonus / SilverThreshold / GoldThreshold.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+BronzeThreshold\s*=\s*5000\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+BronzeThreshold\s*=\s*0\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BronzeThreshold\s*=\s*2500\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BronzeThreshold\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+BronzeThreshold\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BronzeThreshold\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+BronzeThreshold\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 BronzeThreshold is missing from "
...
def require_bronze_threshold_specifiers(section: str) -> str:
...
class MissionScoreRulesBronzeThresholdFieldDeclContractTests(unittest.TestCase):
    def test_mission_score_rules_struct_exists(self) -> None:
        ...
        self.assertEqual(STRUCT_NAME, SCORE_RULES_NAME)
        self.assertNotEqual(STRUCT_NAME, WEATHER_PROFILE_NAME)
        self.assertNotEqual(STRUCT_NAME, OBJECTIVE_PROGRESS_NAME)
        ...
        self.assertTrue(has_identifier(section, "BronzeThreshold"), section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedMissionScoreRules\n{\n};\n"
            )
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BronzeThreshold"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        self.assertNotIn(PRESENTATION_NAME, section)
        self.assertNotIn(OBJECTIVE_PROGRESS_NAME, section)
        self.assertNotIn(WEATHER_PROFILE_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("BronzeThreshold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_bronze_threshold_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tint32 {SIBLING_COMPLETION} = 5000;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_bronze_threshold_specifiers(section)
    ...
    def test_initializer_fails_closed_when_not_5000(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
        ...
        self.assertIn("int32 BronzeThreshold = 5000;", compact_origin)
        self.assertNotIn("BronzeThreshold = NAME_None", compact_origin)
        self.assertNotIn("BronzeThreshold = false", compact_origin)
        self.assertNotIn("BronzeThreshold = true", compact_origin)
        self.assertNotIn("BronzeThreshold = 0.f", compact_origin)
        self.assertNotIn("BronzeThreshold = 160.f", compact_origin)

    def test_bronze_threshold_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 BronzeThreshold"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 5000", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BONUS}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FNAME}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_COMPLETION}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_BONUS}\n",
            f"\t{TARGET_WRONG_FNAME}\n",
            f"\t{TARGET_WRONG_COMPLETION}\n",
            leftover_primary,
            leftover_guided,
            f"\tFText {SIBLING_SPEAKER};\n",
            "\tFName BronzeThresholds;\n",
            "\tbool BronzeThreshold;\n",
            "\tfloat BronzeThreshold = " + forty + ";\n",
            "\tfloat BronzeThreshold = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_bronze_threshold_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(compact_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        with_max = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_bronze_threshold_specifiers(spec_section(with_max))
    ...
    def test_does_not_contract_sibling_score_rules_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_does_not_parse_presentation_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-score-rules BronzeThreshold field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_score_rules_bronze_threshold_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_MISSION_SCORE_RULES_DEFAULTS_PY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_SCORE_RULES_DEFAULTS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ENEMY_FORMATION_UNIT_COUNT, LOCKED_SCRIPTS)
        self.assertIn(REMAINING_WIND_SPEED_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(REMAINING_WEATHER_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(REMAINING_TIME_OF_DAY_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_sibling_score_rules_field_scripts_stay_unclaimed(self) -> None:
        remaining = (
        ...
        self.assertIn(LEFTOVER_MISSION_SCORE_RULES_DEFAULTS, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_MISSION_SCORE_RULES_DEFAULTS, THIS_SCRIPT)
        self.assertIn(REMAINING_WIND_SPEED_SCRIPT, LOCKED_SCRIPTS)
        self.assertNotEqual(REMAINING_WIND_SPEED_SCRIPT, THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_PROFILE_ID, locked_only)
