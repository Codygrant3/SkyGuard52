# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot bRadarDead.
...
# not `bool`, initializer is missing or is not `= false`,
# or someone writes `= true` / `= 0` / `= 0` + `.f`.
# Exact identifier vs siblings `bValid` and `bWon`
# (same type+initializer).
# Do not claim sibling fields: bValid, bWon, MissionTitle,
# OutcomeNarrative, Score, Medal, ShotsFired, Hits,
# CargoPercent, DestroyedSystems, SelectedLoadout,
# CannonReady, RocketReady, GuidedReady.
...
# (keep test_cpg_debrief_snapshot_defaults_contract.py
# in LOCKED_SCRIPTS).
# Do not parse leftover MeshBindSlot #194/#6829 /
# leftover isolated #1447-#1450.
# Do not reopen leftover isolated MissionTitle #1451 /
# bValid #1452 / bWon #1453 / OutcomeNarrative #1454 /
# Medal #1455 / Score #1456 / CargoPercent #1457 /
# Hits #1458 / ShotsFired #1459.
# Do not parse leftover AudibleAcceptanceReceipt,
# leftover MissionResult, leftover TheaterKit,
# leftover MissionDefinition, leftover
# USkyguardRuntimeMeshCatalog, leftover
# USkyguardAudioAcceptanceHarness, leftover
# USkyguardAudioProductionBank, leftover
# FSkyguardAudioEventDefinition, leftover
# ESkyguardAudioEvent, leftover
# ASkyguardGunshipSortieDirector, leftover Harbor
# 40 + `.f` / 80 + `.f` directors, leftover retired
# live-copy types.
# Clone LOCKED_SCRIPTS / fail-closed / git-show /
# unittest skeleton from leftover-safe draft #1300.
# If a clone asserts UPROPERTY / Category="Skyguard|Theater"
# / VisibleAnywhere / EditAnywhere / BlueprintReadOnly /
# `FName WeatherIdentity` / bare no-`=` FName / `bool bWon`,
# RETARGET: parse FSkyguardCpgDebriefSnapshot only, type is
# bool, required initializer `= false`, identifier is
# bRadarDead, no UPROPERTY wrap.
...
FIELD_NAME = "bRadarDead"
SIBLING_VALID = "bValid"
SIBLING_WON = "bWon"
TARGET = "bool bRadarDead = false;"
TARGET_WRONG_BARE = "bool bRadarDead;"
TARGET_WRONG_TRUE = "bool bRadarDead = true;"
TARGET_WRONG_ZERO = "bool bRadarDead = 0;"
TARGET_WRONG_FLOAT = "bool bRadarDead = 0.f;"
TARGET_WRONG_EQ_NONE = "bool bRadarDead = NAME_None;"
TARGET_WRONG_INT32 = "int32 bRadarDead = false;"
TARGET_WRONG_INT_ZERO = "int32 bRadarDead = 0;"
TARGET_WRONG_FNAME = "FName bRadarDead;"
TARGET_WRONG_FSTRING = "FString bRadarDead;"
TARGET_WRONG_FTEXT = "FText bRadarDead;"
TARGET_WRONG_TARRAY = "TArray<bool> bRadarDead;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SIBLING_VALID = "bool bValid = false;"
TARGET_WRONG_SIBLING_WON = "bool bWon = false;"
TARGET_WRONG_THEATER = "FName WeatherIdentity;"
TARGET_WRONG_CAMPAIGN = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Theater")'
)
LOCKED_DECL = TARGET
CLONE_THEATER_UPROPERTY = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Theater")'
)
...
STOP_BEFORE_PATROL_CLASS_BODY = "class ASkyguardPatrolShipBoss :"
...
STOP_BEFORE_AUDIBLE = "struct FSkyguardAudibleAcceptanceReceipt"
...
SIBLING_MISSION_TITLE = "MissionTitle"
SIBLING_OUTCOME_NARRATIVE = "OutcomeNarrative"
SIBLING_SCORE = "Score"
SIBLING_MEDAL = "Medal"
SIBLING_SHOTS_FIRED = "ShotsFired"
SIBLING_HITS = "Hits"
SIBLING_CARGO_PERCENT = "CargoPercent"
SIBLING_DESTROYED_SYSTEMS = "DestroyedSystems"
SIBLING_SELECTED_LOADOUT = "SelectedLoadout"
SIBLING_CANNON_READY = "CannonReady"
SIBLING_ROCKET_READY = "RocketReady"
SIBLING_GUIDED_READY = "GuidedReady"
...
LEFTOVER_BANK_CLASS = "USkyguardAudioProductionBank"
LEFTOVER_MESH_CATALOG_CLASS = "USkyguardRuntimeMeshCatalog"
THIS_SCRIPT = (
    "Scripts/tests/test_cpg_debrief_snapshot_radar_dead"
    "_field_decl_contract.py"
)
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS = (
    "Scripts/tests/test_cpg_debrief_snapshot_defaults"
    "_contract.py"
)
...
def leftover_mesh_bind_slot_isolated_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/test_mesh_bind_slot_"
    suffix = "_field_decl_contract.py"
    return (
        prefix + "notes" + suffix,
        prefix + "proxy_fallback" + suffix,
        prefix + "slot_id" + suffix,
        prefix + "preferred" + suffix,
        LEFTOVER_MESH_BIND_SLOT,
    )


def leftover_cpg_debrief_analog_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS,
        LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_PY,
        LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_TESTS,
...
def leftover_cpg_debrief_sibling_field_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/test_cpg_debrief_snapshot_"
    suffix = "_field_decl_contract.py"
    return (
        prefix + "valid" + suffix,
        prefix + "won" + suffix,
        prefix + "mission_title" + suffix,
        prefix + "outcome_narrative" + suffix,
        prefix + "score" + suffix,
        prefix + "medal" + suffix,
        prefix + "shots_fired" + suffix,
        prefix + "hits" + suffix,
        prefix + "cargo_percent" + suffix,
        prefix + "destroyed_systems" + suffix,
        prefix + "selected_loadout" + suffix,
        prefix + "cannon_ready" + suffix,
        prefix + "rocket_ready" + suffix,
        prefix + "guided_ready" + suffix,
    )


def leftover_audible_receipt_scripts() -> tuple[str, ...]:
...
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_mesh_bind_slot_isolated_scripts()
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_cpg_debrief_analog_scripts()
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_cpg_debrief_sibling_field_scripts()
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_audible_receipt_scripts()
LOCKED = set(LOCKED)
LOCKED.update(
    {
...
CLONE_THEATER_UPROPERTY_TOKENS = (
    "UPROPERTY",
    "Category",
    "VisibleAnywhere",
    "EditAnywhere",
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_VALID,
        SIBLING_WON,
        SIBLING_MISSION_TITLE,
        SIBLING_OUTCOME_NARRATIVE,
        SIBLING_SCORE,
        SIBLING_MEDAL,
        SIBLING_SHOTS_FIRED,
        SIBLING_HITS,
        SIBLING_CARGO_PERCENT,
        SIBLING_DESTROYED_SYSTEMS,
        SIBLING_SELECTED_LOADOUT,
        SIBLING_CANNON_READY,
        SIBLING_ROCKET_READY,
        SIBLING_GUIDED_READY,
    )


def sibling_same_type_initializer_decls() -> tuple[str, ...]:
    return (
        SIBLING_VALID,
        SIBLING_WON,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bRadarDead = false;`.
    # Match EXACT identifier bRadarDead as a whole field name
    # after bool. Do NOT accept sibling `bool bValid = false;`
    # or `bool bWon = false;` (same type+initializer).
    # Do NOT accept missing `=` / `= true` / `= 0` / `= 0` + `.f`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"bool\s+bRadarDead\s*=\s*false\s*;",
        compact,
    )
    if exact is None:
        return False
    if re.search(r"\bbValid\b", compact) and exact is None:
        return False
    if re.search(r"\bbWon\b", compact) and exact is None:
        return False
    if re.search(r"bool\s+bRadarDead\s*;", compact) and re.search(
        r"bool\s+bRadarDead\s*=",
        compact,
    ) is None:
        return False
    assign = re.search(r"bool\s+bRadarDead\s*=\s*([^;]+);", compact)
    if assign is None:
        return False
    if assign.group(1).strip() != "false":
        return False
    if re.search(r"float\s+bRadarDead\b", compact):
        return False
    if re.search(r"int32\s+bRadarDead\b", compact):
        return False
    if re.search(r"FName\s+bRadarDead\b", compact):
        return False
    if re.search(r"FString\s+bRadarDead\b", compact):
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        f"class {ACTOR_NAME}",
        f"class SKYGUARD52_API {LEFTOVER_HARNESS_CLASS}",
        f"class {LEFTOVER_BANK_CLASS}",
        f"class {LEFTOVER_MESH_CATALOG_CLASS}",
    )
...
def attached_uproperty_specifiers(section: str) -> str | None:
    # origin/main is a BARE bool field. A leftover theater-kit
    # clone that still requires UPROPERTY must fail-closed.
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bRadarDead\b", compact[index:]
        ):
...
def require_no_attached_uproperty(section: str) -> None:
    attached = attached_uproperty_specifiers(section)
    if attached is not None:
        raise AssertionError(
            f"{STRUCT_NAME} {FIELD_NAME} must stay a bare bool field; "
            f"found leftover UPROPERTY({attached})"
        )
    compact = collapsed(section)
    if re.search(r"UPROPERTY\s*\([^)]*\)\s*bool\s+bRadarDead\b", compact):
        raise AssertionError(
            f"{STRUCT_NAME} {FIELD_NAME} must not wrap UPROPERTY"
        )


class CpgDebriefSnapshotRadarDeadFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertNotIn("USTRUCT", header.split(STRUCT_NAME, 1)[0][-80:])
        ...
        self.assertNotIn(STOP_BEFORE_BUILD_COPY, body)
        self.assertIn(STOP_BEFORE_BANNED_TERM, header)
        self.assertNotIn(STOP_BEFORE_BANNED_TERM, section)
        self.assertNotIn(STOP_BEFORE_BANNED_TERM, body)
        self.assertIn(f"class {STOP_BEFORE_PATROL};", header)
        self.assertNotIn(STOP_BEFORE_PATROL_CLASS_BODY, header)
        self.assertNotIn(STOP_BEFORE_PATROL, section)
        self.assertNotIn(STOP_BEFORE_PATROL, body)
    ...
    def test_capture_helper_declaration_does_not_satisfy(self) -> None:
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "};\n"
            f"{STOP_BEFORE_CAPTURE_SIG}();\n"
    ...
    def test_missing_radar_dead_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SIBLING_VALID}\n"
            "};\n"
        )
    ...
    def test_sibling_valid_same_type_initializer_does_not_satisfy(self) -> None:
        sibling_only = f"\t{TARGET_WRONG_SIBLING_VALID}\n"
        self.assertFalse(has_declaration(sibling_only, LOCKED_DECL))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(sibling_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        won_only = f"\t{TARGET_WRONG_SIBLING_WON}\n"
        self.assertFalse(has_declaration(won_only, LOCKED_DECL))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(won_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))

    def test_clone_theater_uproperty_macro_does_not_satisfy(self) -> None:
        macro_only = f"\t{CLONE_THEATER_UPROPERTY}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_field_has_no_uproperty(self) -> None:
        section = spec_section(origin_main_header())
        require_no_attached_uproperty(section)
        self.assertIsNone(attached_uproperty_specifiers(section))
        self.assertNotIn("UPROPERTY", section)
        self.assertNotIn("VisibleAnywhere", section)
        self.assertNotIn("BlueprintReadOnly", section)
        self.assertNotIn("EditAnywhere", section)
    ...
    def test_clone_attached_theater_uproperty_fails_closed(self) -> None:
        wrapped = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{CLONE_THEATER_UPROPERTY}\n"
        ...
        self.assertIn("VisibleAnywhere", attached)
        self.assertIn("BlueprintReadOnly", attached)
        self.assertIn('Category="Skyguard|Theater"', attached)
        origin = spec_section(origin_main_header())
        self.assertIsNone(attached_uproperty_specifiers(origin))
    ...
    def test_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bRadarDead = false;", compact_origin)
        self.assertNotIn("bool bRadarDead = true;", compact_origin)
        self.assertNotIn("bool bRadarDead = 0;", compact_origin)
        self.assertNotIn("bool bRadarDead = 0.f;", compact_origin)
        self.assertNotIn("FName WeatherIdentity;", compact_origin)

    def test_radar_dead_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bRadarDead"),
            LOCKED_DECL,
        )
        ...
        wrongs = (
            ...
            TARGET_WRONG_INT32,
            TARGET_WRONG_INT_ZERO,
            TARGET_WRONG_FNAME,
            TARGET_WRONG_FSTRING,
            TARGET_WRONG_FTEXT,
            TARGET_WRONG_TARRAY,
            TARGET_WRONG_HEALTH,
            TARGET_WRONG_SIBLING_VALID,
            TARGET_WRONG_SIBLING_WON,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\t{TARGET_WRONG_SIBLING_VALID}\n",
            f"\t{TARGET_WRONG_SIBLING_WON}\n",
            "\tbool bRadarDeads = false;\n",
            "\tint32 bRadarDead = 0;\n",
            "\tFName bRadarDead;\n",
            "\tbool WeatherIdentity = false;\n",
            "\tfloat bRadarDead = " + forty + ";\n",
            "\tfloat bRadarDead = " + eighty + ";\n",
        )
    ...
    def test_clone_theater_category_is_not_required(self) -> None:
        origin = spec_section(origin_main_header())
        ...
        self.assertIsNotNone(campaign_specs)
        assert campaign_specs is not None
        self.assertIn('Category="Skyguard|Theater"', campaign_specs)
        self.assertIsNone(attached_uproperty_specifiers(origin))
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        leftover_wraps = (
            f"\t{CLONE_THEATER_UPROPERTY}\n\t{LOCKED_DECL}\n",
            f"\t{CLONE_THEATER_UPROPERTY} {LOCKED_DECL}\n",
            "\tUPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
            'Category="Skyguard|Theater")\n'
            f"\t{LOCKED_DECL}\n",
        )
        for region in leftover_wraps:
            self.assertTrue(has_declaration(region, LOCKED_DECL), region)
            with self.assertRaises(AssertionError):
                require_no_attached_uproperty(region)
    ...
    def test_does_not_contract_sibling_snapshot_fields(self) -> None:
        ...
        for same in sibling_same_type_initializer_decls():
            self.assertNotEqual(same, FIELD_NAME)
            self.assertTrue(has_identifier(section, same), same)

    def test_does_not_parse_capture_helpers_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(f"class {STOP_BEFORE_SORTIE};", header)
        self.assertNotIn(STOP_BEFORE_SORTIE, section)
        self.assertNotIn(STOP_BEFORE_SORTIE, leaked)
        self.assertIn(f"class {STOP_BEFORE_GUNNER};", header)
        ...
        self.assertNotIn(LEFTOVER_HARNESS_CLASS, header)
        self.assertNotIn(LEFTOVER_BANK_CLASS, header)
        self.assertNotIn(LEFTOVER_MESH_CATALOG_CLASS, header)
        self.assertNotIn(STOP_BEFORE_AUDIBLE, header)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\tFString {STOP_BEFORE_BUILD_COPY}"
            f"(const {STRUCT_NAME}& Snap);\n"
            f"\tbool {STOP_BEFORE_BANNED_TERM}(const FString& Text);\n"
            f"\t{TARGET_WRONG_SIBLING_VALID}\n"
            f"\t{TARGET_WRONG_HEALTH}\n"
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn(CLONE_THEATER_UPROPERTY, LOCKED_DECL)
        self.assertNotIn("UPROPERTY", LOCKED_DECL)
        self.assertNotIn("VisibleAnywhere", LOCKED_DECL)
        self.assertNotIn("BlueprintReadOnly", LOCKED_DECL)
        self.assertNotIn("EditAnywhere", LOCKED_DECL)
        ...
        for invented in INVENTED_FIELD_META:
            if invented == "= 0":
                self.assertNotIn("= 0;", LOCKED_DECL)
                self.assertNotIn("= 0", LOCKED_DECL.replace("= false", ""))
                continue
            self.assertNotIn(invented, LOCKED_DECL)
            self.assertNotIn(invented, CLONE_THEATER_UPROPERTY)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardAudioAcceptanceHarness.h", HEADER_PATH)
        self.assertEqual(HEADER_PATH, "Source/Skyguard52/SkyguardCpgDebrief.h")
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot bRadarDead field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radar_dead_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_CPG_DEBRIEF_FAIL_CLOSED_CONTRACT, LOCKED_SCRIPTS)
    ...
    def test_leftover_analog_bulk_and_neighbors_stay_locked(self) -> None:
        leftovers = (
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS,
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_PY,
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_TESTS,
            ...
            "Scripts/tests/test_briefing_radio_row_speaker_field_decl_contract.py",
            "Scripts/tests/test_briefing_radio_row_subtitle_field_decl_contract.py",
        ) + leftover_mesh_bind_slot_isolated_scripts() + leftover_cpg_debrief_sibling_field_scripts() + leftover_audible_receipt_scripts()
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_AUDIBLE, locked_only)
        self.assertNotIn(STOP_BEFORE_THEATER_SPEC, locked_only)
        self.assertNotIn(STOP_BEFORE_MESH_CATALOG, locked_only)
