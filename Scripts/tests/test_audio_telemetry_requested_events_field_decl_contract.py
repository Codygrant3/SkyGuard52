# THIS IS leftover-safe FSkyguardAudioTelemetry::RequestedEvents.
# origin/main form: split-line
# UPROPERTY(BlueprintReadOnly)
# then `int32 RequestedEvents = 0;` with no Category=.
# THIS IS UPROPERTY(BlueprintReadOnly) ONLY.
# It is NOT VisibleAnywhere. NO Category=.
# Initializer IS `= 0` (int32).
# If a clone asserts VisibleAnywhere or Category= and
# origin/main has none, drop those assertions.
# If a clone asserts `= false` / `= true` / `= 0.f` /
# bare decl, do not copy those; this field is
...
# (PlayedEvents, RejectedByCooldown, RejectedByConcurrency,
# RejectedMissingAsset, PriorityEvictions, PeakActiveVoices)
# — sibling workers own those.
...
# THIS IS NOT leftover BossTelemetry WeakPointsDestroyed
# #1279 / PilotCommandsIssued #1280.
...
TARGET = "int32 RequestedEvents = 0;"
TARGET_WRONG_BARE = "int32 RequestedEvents;"
TARGET_WRONG_FALSE = "int32 RequestedEvents = false;"
TARGET_WRONG_TRUE = "int32 RequestedEvents = true;"
TARGET_WRONG_FLOAT = "int32 RequestedEvents = 0.f;"
TARGET_WRONG_PLAYED = "int32 PlayedEvents = 0;"
LOCKED_DECL = TARGET
UPROPERTY_READONLY = "UPROPERTY(BlueprintReadOnly)"
STOP_BEFORE_EVENT = "enum class ESkyguardAudioEvent"
STOP_BEFORE_PERSPECTIVE = "enum class ESkyguardListenerPerspective"
STOP_BEFORE_DEFINITION = "struct FSkyguardAudioEventDefinition"
...
LEFTOVER_AUDIO_TELEMETRY_DEFAULTS_PLAIN = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # when origin/main has `int32 RequestedEvents = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+RequestedEvents\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequestedEvents\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequestedEvents\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequestedEvents\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_EVENT,
        STOP_BEFORE_PERSPECTIVE,
        STOP_BEFORE_DEFINITION,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+RequestedEvents\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 RequestedEvents is missing from "
...
class AudioTelemetryRequestedEventsFieldDeclContractTests(unittest.TestCase):
    def test_audio_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("RequestedEvents", section)
        ...
        self.assertNotIn(STOP_BEFORE_EVENT, section)
        self.assertNotIn(STOP_BEFORE_PERSPECTIVE, section)
        self.assertNotIn(STOP_BEFORE_DEFINITION, section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RequestedEvents", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_requested_events_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("RequestedEvents = 0", collapsed(origin))
        self.assertNotIn("RequestedEvents = false", collapsed(origin))
        self.assertNotIn("RequestedEvents = true", collapsed(origin))
        self.assertNotIn("RequestedEvents = 0.f", collapsed(origin))

    def test_requested_events_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RequestedEvents"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool RequestedEvents = 0;\n",
            "\tint32 RequestedEvent = 0;\n",
            "\tint32 RequestedEventss = 0;\n",
            "\tFText RequestedEvents = 0;\n",
            "\tint32 RequestedEvents{};\n",
        )
    ...
    def test_visible_anywhere_added_fails_closed(self) -> None:
        decorated = (
            f"struct {CLASS_NAME}\n"
            "{\n"
        ...
        section = public_section(decorated)
        specifiers = attached_uproperty_specifiers(section)
    ...
    def test_does_not_parse_neighbors_or_director_methods(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIn(STOP_BEFORE_EVENT, header)
        self.assertIn(STOP_BEFORE_PERSPECTIVE, header)
        self.assertIn(STOP_BEFORE_DEFINITION, header)
        ...
        self.assertNotIn(STOP_BEFORE_EVENT, leaked)
        self.assertNotIn(STOP_BEFORE_PERSPECTIVE, leaked)
        self.assertNotIn(STOP_BEFORE_DEFINITION, leaked)
    ...
    def test_parse_window_excludes_leftover_audio_event_enum_body(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        leaked = class_body(header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio-telemetry RequestedEvents field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_audio_telemetry_requested_events"
            "_field_decl_contract.py"
        ))
        self.assertNotIn(THIS_SCRIPT, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_AUDIO_TELEMETRY_DEFAULTS_PLAIN, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("FSkyguardBossTelemetry", locked_only)
