...
# bCalibratedMetering / bProductionBankReady.
# Identifier bounds: match EXACT `bAccepted` as a
...
TARGET = "bool bAccepted = false;"
TARGET_WRONG_BARE = "bool bAccepted;"
TARGET_WRONG_EQ_ONE = "bool bAccepted = true;"
TARGET_WRONG_EQ_NONE = "bool bAccepted = NAME_None;"
TARGET_WRONG_TRUE = "bool bAccepted = true;"
TARGET_WRONG_FALSE = "int32 bAccepted = false;"
TARGET_WRONG_EQ_FLOAT = "bool bAccepted = 0.f;"
TARGET_WRONG_FSTRING = "FString bAccepted;"
TARGET_WRONG_FLOAT = "float bAccepted = false;"
TARGET_WRONG_FNAME = "FName bAccepted;"
TARGET_WRONG_FTEXT = "FText bAccepted;"
TARGET_WRONG_BOOL = "int32 bAccepted = 0;"
TARGET_WRONG_TARRAY = "TArray<bool> bAccepted;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> bAccepted;"
...
SIBLING_PRODUCTION_BANK_READY = "bProductionBankReady"
...
THIS_SCRIPT = (
    "Scripts/tests/test_audible_acceptance_receipt_accepted"
...
CLONE_RECEIPT_MAXIMUM_TRUE_PEAK_DB_TP = (
    "Scripts/tests/test_audible_acceptance_receipt_maximum"
...
def leftover_sibling_receipt_field_scripts() -> tuple[str, ...]:
    # Leftover MissionResult / RouteDefinition / ProductionAudio /
    # MissionDebrief / harness class / bank class / AudioTelemetry
    # stay in LOCKED_SCRIPTS via leftover_* helpers. Isolated
    # BuildSha256 #1435 through MaximumTruePeakDbTP #1441 plus
    # later sibling bools stay locked here. Do not lock THIS
    # bAccepted file.
    ...
    return (
        ...
        prefix + "production_bank_ready" + suffix,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bAccepted = false;`.
    # Match the EXACT identifier bAccepted as a whole
    # field name after bool. Do NOT accept sibling bools
    # bPackagedDevelopmentBuild / bAudibleDeviceObserved /
    # bCalibratedMetering / bProductionBankReady. Reject
    # dropped/changed initializer (`= true` or bare),
    # Harbor 40/80, wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"bool\s+bAccepted\s*=\s*false\s*;",
        compact,
    )
    ...
    if re.search(r"\bbCalibratedMetering\b", compact) and exact is None:
        return False
    if re.search(r"\bbProductionBankReady\b", compact) and exact is None:
        return False
    if re.search(
        r"bool\s+bAccepted\s*;",
        compact,
    ) and re.search(
        r"bool\s+bAccepted\s*=",
        compact,
    ...
    for assigned in re.findall(
        r"bool\s+bAccepted\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "false":
            return False
    if re.search(
        r"float\s+bAccepted\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bAccepted\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bAccepted\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+bAccepted\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+bAccepted\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+bAccepted\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+bAccepted\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bAccepted\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bAccepted is missing from "
...
class AudibleAcceptanceReceiptAcceptedFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_accepted_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            ...
            f"\tbool {SIBLING_CALIBRATED_METERING} = false;\n"
            "};\n"
        )
    ...
    def test_sibling_bools_do_not_satisfy_accepted(self) -> None:
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bAccepted = false;", compact_origin)
        self.assertNotIn("bAccepted = true", compact_origin)
        self.assertNotIn("int32 bAccepted", compact_origin)
        self.assertNotIn("FString bAccepted", compact_origin)
        self.assertNotIn("float bAccepted", compact_origin)

    def test_accepted_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bAccepted"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_TOTAL_UNDERRUNS} = 0;\n",
            f"\tbool {SIBLING_CALIBRATED_METERING} = false;\n",
            "\tbool bAccepteds = false;\n",
            "\tbool bAccepted = true;\n",
            "\tint32 bAccepted;\n",
            "\tfloat bAccepted = " + forty + ";\n",
            "\tfloat bAccepted = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_receipt_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_PRODUCTION_BANK_READY), section
        )
        self.assertTrue(has_identifier(section, SIBLING_CALIBRATED_METERING), section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt bAccepted field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "accepted_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_accepted", THIS_SCRIPT)
        ...
        self.assertNotEqual(CLONE_RECEIPT_MAXIMUM_AUDIO_THREAD_MS, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_MAXIMUM_TRUE_PEAK_DB_TP, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_MAXIMUM_TRUE_PEAK_DB_TP, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_PACKAGED_DEVELOPMENT_BUILD, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_PACKAGED_DEVELOPMENT_BUILD, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_AUDIBLE_DEVICE_OBSERVED, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_AUDIBLE_DEVICE_OBSERVED, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_CALIBRATED_METERING, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_CALIBRATED_METERING, THIS_SCRIPT)
        self.assertIn(
