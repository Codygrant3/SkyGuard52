...
# INDEX_NONE or lock Telemetry construction
# in the .cpp. This is a FIELD contract for
# the Telemetry UPROPERTY, not leftover
# FSkyguardBossTelemetry defaults #48e1
# and not leftover GetTelemetry method.
...
# origin/main and must be part of the
# locked field declaration. Nearby
# origin/main
...
# defaults #48e1 (do not add a leftover
# struct-defaults contract). Stay off leftover
# Apache HullCollider field #425.
# Stay off leftover Root / BodyMesh /
# WeakPoints / Phase field contracts.
...
# Harbor 40/80.
TELEMETRY_FIELD = "FSkyguardBossTelemetry Telemetry;"
UPROPERTY_TELEMETRY = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
...
# Python Telemetry field declaration
...
LEFTOVER_TELEMETRY_DEFAULTS_NOT_LOCKED = (
...
# .cpp Telemetry body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBossDroneBase::Telemetry",
    "SkyguardBossDroneBase.cpp",
...
class BossDroneTelemetryFieldDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, TELEMETRY_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, TELEMETRY_FIELD)
        self.assertIn("Telemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, TELEMETRY_FIELD)
    ...
    def test_missing_telemetry_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, TELEMETRY_FIELD)
        self.assertIn("Telemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_TELEMETRY}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, TELEMETRY_FIELD)
        self.assertIn("Telemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_TELEMETRY, section)
        ...
        self.assertNotIn("UPROPERTY", TELEMETRY_FIELD)
        self.assertNotIn("VisibleAnywhere", TELEMETRY_FIELD)
        self.assertNotIn("BlueprintReadOnly", TELEMETRY_FIELD)
        self.assertNotIn("Category", TELEMETRY_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_TELEMETRY)
        self.assertNotIn("BlueprintCallable", UPROPERTY_TELEMETRY)
        self.assertIn("Skyguard|Boss", UPROPERTY_TELEMETRY)
        self.assertNotIn("Destruction", UPROPERTY_TELEMETRY)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_TELEMETRY)
            self.assertNotIn(invented, TELEMETRY_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, TELEMETRY_FIELD)
        self.assertIn("Telemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tFVector Telemetry;\n"
        box = "\tFSkyguardCpgDebriefSnapshot Telemetry;\n"
        static_mesh = "\tESkyguardBossPhase Telemetry;\n"
        weak = "\tTObjectPtr<USceneComponent> Telemetry;\n"
        assigned = "\tFSkyguardBossTelemetry Telemetry = {};\n"
        wrong_name = "\tFSkyguardBossTelemetry Phase;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, TELEMETRY_FIELD)
            self.assertIn("Telemetry", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_telemetry_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, TELEMETRY_FIELD),
            1,
        )
        self.assertTrue(
            TELEMETRY_FIELD.startswith("FSkyguardBossTelemetry "),
            TELEMETRY_FIELD,
        )
        self.assertTrue(
            TELEMETRY_FIELD.endswith(";"),
            TELEMETRY_FIELD,
        )
        self.assertIn("Telemetry", TELEMETRY_FIELD)
        self.assertNotIn("=", TELEMETRY_FIELD)
        self.assertNotIn("INDEX_NONE", TELEMETRY_FIELD)
        self.assertNotIn("UFUNCTION", TELEMETRY_FIELD)
        self.assertNotIn("GetBossPhase", TELEMETRY_FIELD)
        self.assertNotIn("{", TELEMETRY_FIELD)
        self.assertNotIn("}", TELEMETRY_FIELD)
        self.assertNotIn("return ", TELEMETRY_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            TELEMETRY_FIELD,
        )
        self.assertNotIn("ApplyWeaponHit", TELEMETRY_FIELD)
        self.assertNotIn("IssuePilotCommand", TELEMETRY_FIELD)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            TELEMETRY_FIELD,
        )
        ...
        self.assertNotIn("GetDefeatDebrisPieceCount", TELEMETRY_FIELD)
        self.assertNotIn("GetTelemetry", TELEMETRY_FIELD)

    def test_contract_does_not_relock_leftover_telemetry_defaults(self) -> None:
        locked_only = f"{TELEMETRY_FIELD}\n"
        for token in LEFTOVER_TELEMETRY_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn(
            "test_cpg_debrief_snapshot_defaults_contract.py",
            TELEMETRY_FIELD,
        )

    def test_contract_does_not_relock_sibling_field_contracts(self) -> None:
        locked_only = f"{TELEMETRY_FIELD}\n"
        ...
        for token in PHASE_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("Root", TELEMETRY_FIELD)
        self.assertNotIn("BodyMesh", TELEMETRY_FIELD)
        self.assertNotIn("WeakPoints", TELEMETRY_FIELD)
        self.assertNotIn("Phase", TELEMETRY_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", TELEMETRY_FIELD)
        self.assertNotIn("GetGunnerMount", TELEMETRY_FIELD)
        self.assertNotIn("FaceWorldLocation", TELEMETRY_FIELD)
        self.assertNotIn("WeakPoints", TELEMETRY_FIELD)
        self.assertNotIn("SetSearchlightTracked", TELEMETRY_FIELD)
        self.assertNotIn("SetContactIdentified", TELEMETRY_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            TELEMETRY_FIELD,
        )
        self.assertNotIn("SetLightningExposed", TELEMETRY_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            TELEMETRY_FIELD,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, TELEMETRY_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_TELEMETRY}\n"
            f"\t{TELEMETRY_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_TELEMETRY} {TELEMETRY_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{TELEMETRY_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, TELEMETRY_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, TELEMETRY_FIELD),
    ...
    def test_assigned_telemetry_does_not_satisfy(self) -> None:
        assigned = "\tFSkyguardBossTelemetry Telemetry = {};\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, TELEMETRY_FIELD)
        self.assertIn("Telemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, TELEMETRY_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (root, body, weak_points, phase):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, TELEMETRY_FIELD)
            self.assertIn("Telemetry", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, TELEMETRY_FIELD))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_TELEMETRY, section)
        self.assertTrue(has_declaration(section, TELEMETRY_FIELD), section)

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{TELEMETRY_FIELD}\n"
        self.assertNotIn("UFUNCTION", TELEMETRY_FIELD)
        self.assertNotIn("UFUNCTION", locked_only)
        self.assertFalse(TELEMETRY_FIELD.startswith("UFUNCTION"), TELEMETRY_FIELD)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, TELEMETRY_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", TELEMETRY_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_telemetry_cpp_body(
        self,
    ) -> None:
        locked_only = f"{TELEMETRY_FIELD}\n"
        self.assertNotIn("{", TELEMETRY_FIELD)
        ...
        self.assertNotIn("return false", TELEMETRY_FIELD)
        self.assertNotIn("return true", TELEMETRY_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            TELEMETRY_FIELD,
        )
        self.assertNotIn("SetBossPhase", TELEMETRY_FIELD)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            TELEMETRY_FIELD,
        )
        self.assertNotIn("RegisterDefeatDebris", TELEMETRY_FIELD)
        self.assertNotIn("CleanupDefeatDebris", TELEMETRY_FIELD)
        self.assertNotIn("HandleDefeated", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_sibling_public_fields(self) -> None:
        ...
        self.assertNotIn("BodyMesh", TELEMETRY_FIELD)
        self.assertNotIn("WeakPoints", TELEMETRY_FIELD)
        self.assertNotIn("CurrentPilotCommand", TELEMETRY_FIELD)
        self.assertNotIn("OnBossPhaseChanged", TELEMETRY_FIELD)
        self.assertNotIn("OnPilotCommandNative", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_hull_collider(self) -> None:
        ...
        for token in LEFTOVER_HULL_COLLIDER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("HullCollider", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", TELEMETRY_FIELD)
        self.assertNotIn("PortGuidanceArray", TELEMETRY_FIELD)
        self.assertNotIn("CommandCore", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("SetSearchlightTracked", TELEMETRY_FIELD)
        self.assertNotIn("IsSearchlightTracked", TELEMETRY_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("SetContactIdentified", TELEMETRY_FIELD)
        self.assertNotIn("IsContactIdentified", TELEMETRY_FIELD)
        self.assertNotIn("OpenOrbitExposure", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", TELEMETRY_FIELD)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            TELEMETRY_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", TELEMETRY_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", TELEMETRY_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            TELEMETRY_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("GetGunnerMount", TELEMETRY_FIELD)
        self.assertNotIn("GetChinTurret", TELEMETRY_FIELD)
        self.assertNotIn("GetSensorTurret", TELEMETRY_FIELD)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{TELEMETRY_FIELD}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, TELEMETRY_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("ASkyguardGunner", TELEMETRY_FIELD)
        self.assertNotIn("FillAndFinalize", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("SkyguardRadarNode", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", TELEMETRY_FIELD)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            TELEMETRY_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, TELEMETRY_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", TELEMETRY_FIELD)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            TELEMETRY_FIELD,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TELEMETRY_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", TELEMETRY_FIELD)
        self.assertNotIn("MaxIntegrity", TELEMETRY_FIELD)
        self.assertNotIn("CurrentIntegrity", TELEMETRY_FIELD)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            TELEMETRY_FIELD,
        )
        self.assertNotIn("SkyguardPathfinder", TELEMETRY_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, TELEMETRY_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{TELEMETRY_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, TELEMETRY_FIELD),
            TELEMETRY_FIELD,
        )
        ...
        self.assertNotIn("Phase", locked_only)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone Telemetry field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, TELEMETRY_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                TELEMETRY_FIELD.lower(),
                "boss drone Telemetry contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, TELEMETRY_FIELD)

    def test_contract_is_telemetry_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", TELEMETRY_FIELD)
        self.assertNotIn("{", TELEMETRY_FIELD)
        self.assertTrue(TELEMETRY_FIELD.startswith("FSkyguardBossTelemetry "))
        self.assertTrue(TELEMETRY_FIELD.endswith(";"))
        self.assertIn("FSkyguardBossTelemetry", TELEMETRY_FIELD)
        self.assertIn("Telemetry", TELEMETRY_FIELD)
        self.assertNotIn("=", TELEMETRY_FIELD)
        self.assertIn(UPROPERTY_TELEMETRY, section)
