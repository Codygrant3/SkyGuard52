...
# Mission07 IntegrationReadiness bAudioReady
...
# bBriefingReady #1037 / leftover
# bSortiePresentationReady / leftover
# bCampaignRuntimeStarted siblings spawned
# this wave,
...
# briefing-ready-field-decl-contract-c332,
# NOT leftover Mission05 bBriefingReady
# FIELD #972 / branch cursor/mission05-
# briefing-ready-field-decl-contract-c332,
...
# drafts #674-#1001 including leftover Mission02
...
# drafts #674-#1001. Do not reopen leftover
...
class Mission07AudioReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 bAudioReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission07 bAudioReady contains "
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        for token in LEFTOVER_SAME_NAME_BRIEFING_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION06_BRIEFING_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_BRIEFING_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_SAME_NAME_AUDIO_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION06_AUDIO_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_SORTIE_PRESENTATION_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_CAMPAIGN_RUNTIME_STARTED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
