# THIS IS leftover-safe FSkyguardWeatherProfile Precipitation.
...
# if type is not float, if initializer is not `= 0.f`,
...
# TimeOfDayHours / WindSpeedMetersPerSecond / CloudCoverage.
...
# THIS IS NOT leftover #1332-#1352.
...
# initializer is `= 0.f`, no Category.
...
TARGET = "float Precipitation = 0.f;"
TARGET_WRONG_BARE = "float Precipitation;"
TARGET_WRONG_FALSE = "float Precipitation = false;"
TARGET_WRONG_TRUE = "float Precipitation = true;"
TARGET_WRONG_ZERO_D = "float Precipitation = 0.0f;"
TARGET_WRONG_ONE = "float Precipitation = 1.f;"
TARGET_WRONG_CLOUD = "float Precipitation = 0.25f;"
TARGET_WRONG_FNAME = "FName Precipitation;"
TARGET_WRONG_INT = "int32 Precipitation = 0;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float Precipitation = 0.f;`.
    # Do not accept bare `float Precipitation;` / `= 0.0f` /
    # `= 1.f` / `= 0.25f` / leftover CloudCoverage when
    # origin/main has `= 0.f`. Do not accept sibling
    # ProfileId / Weather / TimeOfDayHours /
    # WindSpeedMetersPerSecond / CloudCoverage fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+Precipitation\s*=\s*0\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"FName\s+Precipitation\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Precipitation\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Precipitation\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+Precipitation\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float Precipitation is missing from "
...
class WeatherProfilePrecipitationFieldDeclContractTests(unittest.TestCase):
    def test_weather_profile_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Precipitation"), section)
    ...
    def test_neighbor_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Precipitation"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(MISSION_DEF_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Precipitation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_precipitation_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float Precipitation = 0.f;", compact_origin)
        self.assertNotIn("Precipitation = NAME_None", compact_origin)
        self.assertNotIn("Precipitation = false", compact_origin)
        self.assertNotIn("Precipitation = true", compact_origin)
        self.assertNotIn("Precipitation = 160.f", compact_origin)
        self.assertNotIn("Precipitation = 0.0f", compact_origin)
        self.assertNotIn("Precipitation = 1.f", compact_origin)

    def test_precipitation_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float Precipitation"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_PROFILE_ID};\n",
            f"\tfloat {SIBLING_CLOUD_COVERAGE} = 0.25f;\n",
            "\tfloat Precipitations = 0.f;\n",
            "\tint32 Precipitation = 0;\n",
            "\tbool Precipitation;\n",
            "\tfloat Precipitation = " + forty + ";\n",
            "\tfloat Precipitation = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weather-profile Precipitation field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "precipitation_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
