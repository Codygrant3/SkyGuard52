...
# then `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|MissionMap`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS
# `= TEXT("CampaignMapAssembly_v1")`.
# This is a public FName revision field, not a combat look float,
# not leftover Theater WeatherIdentity, and not leftover analog
...
# Stop BEFORE sibling SkylineStyle as a claimed slot.
...
# MissionDefinition / MissionId / SkylineStyle /
...
# LOCKED_DECL is `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# NOT `float LookYawLimit = 95.f;` and NOT
# `FName WeatherIdentity;` (no initializer) and NOT
# leftover `FName MissionId;` and NOT leftover analog
...
# / float LookYawLimit = 95.f AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is Skyguard|Combat
# or still Skyguard|Theater.
# Public FName fail-closed if LOCKED_DECL is leftover
...
TARGET = 'FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");'
TARGET_WRONG_BARE = "FName AssemblyRevision;"
TARGET_WRONG_INIT = "FName AssemblyRevision = nullptr;"
TARGET_WRONG_FALSE = "FName AssemblyRevision = false;"
TARGET_WRONG_TRUE = "FName AssemblyRevision = true;"
TARGET_WRONG_ZERO = "FName AssemblyRevision = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> AssemblyRevision;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> AssemblyRevision;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> AssemblyRevision;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bare AssemblyRevision without the TEXT initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ASSEMBLY_REVISION_DECL_RE.search(compact) is None:
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+AssemblyRevision\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName AssemblyRevision "
...
class MissionMapAssemblyRevisionFieldDeclContractTests(unittest.TestCase):
    def test_mission_map_assembly_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "AssemblyRevision"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "AssemblyRevision"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("AssemblyRevision", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_assembly_revision_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_assembly_revision_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName AssemblyRevision"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn('TEXT("CampaignMapAssembly_v1")', LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 AssemblyRevision = 1;\n",
            "\tbool AssemblyRevision = true;\n",
            "\tFName AssemblyRevision = " + forty + ";\n",
            "\tFName AssemblyRevision = " + eighty + ";\n",
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            '\tFName\n\tAssemblyRevision = TEXT("CampaignMapAssembly_v1");\n',
            '\tFName   AssemblyRevision = TEXT("CampaignMapAssembly_v1");\n',
            '\tFName\tAssemblyRevision = TEXT("CampaignMapAssembly_v1");\n',
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_skyline_style_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "AssemblyRevision"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map AssemblyRevision field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_map_assembly_revision_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
