...
# a mission-map body, or lock MissionMap in the .cpp.
# origin/main is one line (`TSoftObjectPtr<UWorld> MissionMap;`);
...
# This is USkyguardMissionDefinition::MissionMap, not
# USkyguardMissionDefinition::MissionId, not
# USkyguardCampaignDefinition::CampaignId, and not
# USkyguardCampaignSaveGame::CampaignId.
MISSION_MAP = "TSoftObjectPtr<UWorld> MissionMap;"
UPROPERTY_WORLD = (
    'UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "World")'
)
# Leftover #56–#64 plus MissionDefinition production files.
# This lane only adds an isolated Python MissionMap field
# declaration contract. Stay off MissionId #350, DisplayName
# (in-flight sibling), CampaignOrder (in-flight sibling),
...
# leftover DisplayName / CampaignOrder in-flight
# siblings, leftover CPG debrief, leftover
...
# here. MissionId / DisplayName / CampaignOrder / Route /
...
# identity fields stay unlocked. This lane is MissionMap on
...
# EditAnywhere, BlueprintReadOnly, Category = "World".
...
# .cpp MissionMap body / invented INDEX_NONE stay unlocked.
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionDefinition::MissionMap",
    "SkyguardMissionDefinition.cpp",
)
SIBLING_TYPES = (
...
class MissionDefinitionMissionMapDeclContractTests(unittest.TestCase):
    def test_mission_definition_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, MISSION_MAP), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, MISSION_MAP))

    def test_missing_mission_map_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tFName MissionId;\n"
            "\tFText DisplayName;\n"
            "\tint32 CampaignOrder = 1;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_WORLD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_WORLD, section)
        self.assertIn("EditAnywhere", section)
        self.assertIn("BlueprintReadOnly", section)
        self.assertIn('Category = "World"', section)
        self.assertTrue(has_declaration(section, MISSION_MAP), section)
        self.assertNotIn("UPROPERTY", MISSION_MAP)
        self.assertNotIn("EditAnywhere", MISSION_MAP)
        self.assertNotIn("BlueprintReadOnly", MISSION_MAP)
        self.assertNotIn("Category", MISSION_MAP)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_WORLD)
            self.assertNotIn(invented, MISSION_MAP)
    ...
    def test_neighbor_fields_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_fields, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        assigned = "\tTSoftObjectPtr<UWorld> MissionMap = nullptr;\n"
        world_ptr = "\tUWorld* MissionMap;\n"
        class_ptr = "\tTSoftClassPtr<UWorld> MissionMap;\n"
        soft_path = "\tFSoftObjectPath MissionMap;\n"
        name_type = "\tFName MissionMap;\n"
        level_type = "\tTSoftObjectPtr<ULevel> MissionMap;\n"
        object_ptr = "\tTObjectPtr<UWorld> MissionMap;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, MISSION_MAP)
            self.assertIn("MissionMap", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_campaign_definition_campaign_id_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(definition_field, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(definition_field, MISSION_MAP))

    def test_save_game_campaign_id_does_not_satisfy(self) -> None:
        save_field = "\tFName CampaignId;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(save_field, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(save_field, MISSION_MAP))

    def test_mission_id_does_not_satisfy(self) -> None:
        mission_id = "\tFName MissionId;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(mission_id, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(mission_id, MISSION_MAP))

    def test_assigned_mission_map_does_not_satisfy(self) -> None:
        assigned = "\tTSoftObjectPtr<UWorld> MissionMap = nullptr;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, MISSION_MAP)
        self.assertIn("MissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, MISSION_MAP))

    def test_mission_map_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, MISSION_MAP),
            MISSION_MAP,
        )
        self.assertTrue(has_declaration(section, MISSION_MAP))
        self.assertEqual(
            declaration_count(section, MISSION_MAP),
            1,
        )
        self.assertTrue(
            MISSION_MAP.endswith(";"),
            MISSION_MAP,
        )
        self.assertTrue(
            MISSION_MAP.startswith("TSoftObjectPtr<UWorld> "),
            MISSION_MAP,
        )
        self.assertIn("MissionMap", MISSION_MAP)
        self.assertIn("UWorld", MISSION_MAP)
        self.assertNotIn("=", MISSION_MAP)
        self.assertNotIn("TEXT(", MISSION_MAP)
        self.assertNotIn("INDEX_NONE", MISSION_MAP)
        self.assertNotIn("NAME_None", MISSION_MAP)
        self.assertNotIn("UFUNCTION", MISSION_MAP)
        self.assertNotIn("{", MISSION_MAP)
        self.assertNotIn("}", MISSION_MAP)
        self.assertNotIn("return ", MISSION_MAP)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, MISSION_MAP),
                section,
            )
            self.assertEqual(
                require_declaration(section, MISSION_MAP),
        ...
        one_line = f"{{\npublic:\n\t{MISSION_MAP}\n}}\n"
        self.assertTrue(has_declaration(one_line, MISSION_MAP))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, MISSION_MAP), section)
        self.assertEqual(
            require_declaration(section, MISSION_MAP),
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(MISSION_MAP.startswith("UFUNCTION"), MISSION_MAP)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, MISSION_MAP), section)
        self.assertEqual(
            require_declaration(section, MISSION_MAP),
    ...
    def test_contract_does_not_lock_mission_map_cpp_body(self) -> None:
        locked_only = f"{MISSION_MAP}\n"
        self.assertNotIn("{", MISSION_MAP)
        self.assertNotIn("}", MISSION_MAP)
        self.assertNotIn("return ", MISSION_MAP)
        ...
        self.assertNotIn("return false", MISSION_MAP)
        self.assertNotIn("AddError", MISSION_MAP)

    def test_contract_does_not_relock_mission_id(self) -> None:
        locked_only = f"{MISSION_MAP}\n"
        for neighbor in MISSION_ID_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FName MissionId", MISSION_MAP)
    ...
    def test_contract_does_not_relock_display_name(self) -> None:
        ...
        for neighbor in DISPLAY_NAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("DisplayName", MISSION_MAP)
        self.assertNotIn("DisplayName", locked_only)
        self.assertNotIn("FText", MISSION_MAP)
    ...
    def test_contract_does_not_relock_campaign_order(self) -> None:
        ...
        for neighbor in CAMPAIGN_ORDER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("CampaignOrder", MISSION_MAP)
    ...
    def test_contract_does_not_relock_route(self) -> None:
        ...
        for neighbor in ROUTE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FSkyguardRouteDefinition", MISSION_MAP)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("Objectives", MISSION_MAP)
        self.assertNotIn("Objectives", locked_only)
        self.assertNotIn("FSkyguardObjectiveDefinition", MISSION_MAP)
    ...
    def test_contract_does_not_relock_waves(self) -> None:
        ...
        for neighbor in WAVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("Waves", MISSION_MAP)
        self.assertNotIn("Waves", locked_only)
        self.assertNotIn("FSkyguardEnemyWaveDefinition", MISSION_MAP)
    ...
    def test_contract_does_not_relock_boss(self) -> None:
        ...
        for neighbor in BOSS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FSkyguardBossDefinition", MISSION_MAP)
    ...
    def test_contract_does_not_relock_weather(self) -> None:
        ...
        for neighbor in WEATHER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FSkyguardWeatherProfile", MISSION_MAP)
    ...
    def test_contract_does_not_relock_presentation(self) -> None:
        ...
        for neighbor in PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FSkyguardMissionPresentation", MISSION_MAP)
    ...
    def test_contract_does_not_relock_score_rules(self) -> None:
        ...
        for neighbor in SCORE_RULES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FSkyguardMissionScoreRules", MISSION_MAP)
    ...
    def test_contract_does_not_relock_prerequisite_mission_ids(self) -> None:
        ...
        for neighbor in PREREQUISITE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("PrerequisiteMissionIds", MISSION_MAP)
    ...
    def test_contract_does_not_relock_required_campaign_medals(self) -> None:
        ...
        for neighbor in MEDALS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("RequiredCampaignMedals", MISSION_MAP)
    ...
    def test_contract_does_not_relock_primary_asset_id(self) -> None:
        ...
        for neighbor in PRIMARY_ASSET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("GetPrimaryAssetId", MISSION_MAP)
        self.assertNotIn("GetPrimaryAssetId", locked_only)
        self.assertNotIn("FPrimaryAssetId", MISSION_MAP)
    ...
    def test_contract_does_not_relock_validate_definition(self) -> None:
        ...
        for neighbor in VALIDATE_DEFINITION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("ValidateDefinition", MISSION_MAP)
        self.assertNotIn("ValidateDefinition", locked_only)
        self.assertNotIn("OutErrors", MISSION_MAP)
        self.assertNotIn("BlueprintCallable", MISSION_MAP)
    ...
    def test_contract_does_not_relock_find_objective(self) -> None:
        ...
        for neighbor in FIND_OBJECTIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FindObjective", MISSION_MAP)
        self.assertNotIn("FindObjective", locked_only)
        self.assertNotIn("ObjectiveId", MISSION_MAP)
    ...
    def test_contract_does_not_relock_campaign_definition_fields(self) -> None:
        ...
        for token in DEFINITION_NOT_LOCKED:
            ...
            self.assertNotIn(token, MISSION_MAP)
            self.assertNotIn(token, section)
        self.assertNotIn("Skyguard52MainCampaign", MISSION_MAP)
        self.assertNotIn("Skyguard52MainCampaign", locked_only)
        self.assertNotIn("USkyguardCampaignDefinition", MISSION_MAP)
    ...
    def test_contract_does_not_relock_leftover_struct_default_drafts(
        ...
        for token in LEFTOVER_STRUCT_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, MISSION_MAP)
        self.assertNotIn("defaults_contract", MISSION_MAP)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MISSION_MAP)
        self.assertNotIn("FillResultCombatStats", MISSION_MAP)
        self.assertNotIn("ASkyguardGunner", MISSION_MAP)
        self.assertNotIn("FillAndFinalize", MISSION_MAP)
        self.assertNotIn("FillAndFail", MISSION_MAP)
        self.assertNotIn("ApplyHydraForClusters", MISSION_MAP)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{MISSION_MAP}\n"
        self.assertEqual(
            require_declaration(locked_only, MISSION_MAP),
            MISSION_MAP,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission MissionMap contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, MISSION_MAP.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"mission MissionMap contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, MISSION_MAP)

    def test_contract_is_mission_map_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertEqual(MISSION_MAP, "TSoftObjectPtr<UWorld> MissionMap;")
