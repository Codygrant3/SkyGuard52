...
# mission-record payload, or lock the CurrentSaveVersion
# body in the .cpp.
...
# origin/main initializer `= 2`, only.
CURRENT = "static constexpr int32 CurrentSaveVersion = 2;"
# Leftover #56–#64 plus CampaignSaveGame production files.
# This lane only adds an isolated Python CurrentSaveVersion
# declaration contract. Stay off MinSupportedSaveVersion
# (unused sibling for a later worker), leftover
# campaign-save empty-fail-closed, leftover mission-save-
# record defaults #ac38 (FSkyguardMissionSaveRecord
# fields), MigrateCampaignSave (sibling draft this wave),
...
# settings invert-look, and MigrateCampaignSave stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# MinSupportedSaveVersion is an unused sibling for a later
# worker. MigrateCampaignSave is a sibling draft this wave.
UNLOCKED_NEIGHBORS = (
...
# .cpp CurrentSaveVersion body / invented save-version
...
class CurrentSaveVersionDeclContractTests(unittest.TestCase):
    def test_campaign_save_game_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, CURRENT), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CURRENT)
        self.assertIn("CurrentSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CURRENT))

    def test_missing_current_save_version_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tstatic constexpr int32 MinSupportedSaveVersion = 1;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CURRENT)
        self.assertIn("CurrentSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CURRENT)
        self.assertIn("CurrentSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{CURRENT}\n"
        self.assertNotIn("UFUNCTION", CURRENT)
        self.assertNotIn("UFUNCTION", locked_only)
        self.assertFalse(CURRENT.startswith("UFUNCTION"), CURRENT)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CURRENT), section)
        self.assertEqual(require_declaration(section, CURRENT), CURRENT)
    ...
    def test_neighbor_fields_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_fields, CURRENT)
        self.assertIn("CurrentSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_static = "\tconstexpr int32 CurrentSaveVersion = 2;\n"
        missing_constexpr = "\tstatic int32 CurrentSaveVersion = 2;\n"
        wrong_type = "\tstatic constexpr int CurrentSaveVersion = 2;\n"
        wrong_initializer = (
            "\tstatic constexpr int32 CurrentSaveVersion = 1;\n"
        )
        zero_initializer = (
            "\tstatic constexpr int32 CurrentSaveVersion = 0;\n"
        )
        no_initializer = "\tstatic constexpr int32 CurrentSaveVersion;\n"
        min_supported = (
            "\tstatic constexpr int32 MinSupportedSaveVersion = 1;\n"
        )
        min_supported_as_two = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CURRENT)
            self.assertIn("CurrentSaveVersion", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_current_save_version_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CURRENT),
            CURRENT,
        )
        self.assertTrue(has_declaration(section, CURRENT))
        self.assertEqual(declaration_count(section, CURRENT), 1)
        self.assertTrue(CURRENT.startswith("static constexpr int32 "), CURRENT)
        self.assertIn("CurrentSaveVersion", CURRENT)
        self.assertTrue(CURRENT.endswith("= 2;"), CURRENT)
        self.assertIn("= 2", CURRENT)
        self.assertNotIn("INDEX_NONE", CURRENT)
        self.assertNotIn("UFUNCTION", CURRENT)
        self.assertNotIn("{", CURRENT)
        self.assertNotIn("}", CURRENT)
        self.assertNotIn("return ", CURRENT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_equals = (
            "public:\n"
            "\tstatic constexpr int32 CurrentSaveVersion =\n"
            "\t\t2;\n"
            "};\n"
        )
        header_wrap_name = (
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, CURRENT), 1)
        one_line = f"{{\npublic:\n\t{CURRENT}\n}}\n"
        self.assertTrue(has_declaration(one_line, CURRENT))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CURRENT), section)
        self.assertEqual(
            require_declaration(section, CURRENT),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CURRENT)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_migration_result(self) -> None:
        locked_only = f"{CURRENT}\n"
        self.assertNotIn("FSkyguardMigrationResult", CURRENT)
        self.assertNotIn("ESkyguardMigrationResult", CURRENT)
    ...
    def test_declaration_does_not_invent_save_version_runtime_return(
        ...
        locked_only = f"{CURRENT}\n"
        self.assertNotIn("return CurrentSaveVersion", CURRENT)
        self.assertNotIn("return MinSupportedSaveVersion", CURRENT)
        self.assertNotIn("return SaveVersion", CURRENT)
        ...
        self.assertNotIn("return ", CURRENT)
        self.assertFalse(CURRENT.startswith("static int32 "), CURRENT)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_mission_record_payload(self) -> None:
        ...
        self.assertNotIn("BestScore", CURRENT)
        self.assertNotIn("BestMedalTier", CURRENT)
        self.assertNotIn("BestCompletionTimeSeconds", CURRENT)
        self.assertNotIn("bCompleted", CURRENT)
    ...
    def test_contract_does_not_lock_current_save_version_cpp_body(self) -> None:
        locked_only = f"{CURRENT}\n"
        self.assertNotIn("{", CURRENT)
        self.assertNotIn("}", CURRENT)
        self.assertNotIn("return ", CURRENT)
        ...
        self.assertNotIn("SkyguardCampaignSaveGame.cpp", CURRENT)
        self.assertNotIn("SkyguardCampaignSaveGame.cpp", locked_only)
        self.assertNotIn("already-v2", CURRENT)
        self.assertNotIn("Identity migrate", CURRENT)
        self.assertNotIn("NewObject", CURRENT)

    def test_contract_does_not_relock_min_supported_save_version(self) -> None:
        locked_only = f"{CURRENT}\n"
        for neighbor in MIN_SUPPORTED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CURRENT)
        self.assertNotIn("MinSupportedSaveVersion", CURRENT)
    ...
    def test_contract_does_not_relock_migrate_campaign_save(self) -> None:
        ...
        for neighbor in MIGRATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CURRENT)
        self.assertNotIn("MigrateCampaignSave", CURRENT)
    ...
    def test_contract_does_not_relock_save_fields(self) -> None:
        ...
        for neighbor in SAVE_FIELDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CURRENT)
        self.assertNotIn("CampaignId", CURRENT)
        self.assertNotIn("MissionRecords", CURRENT)
        self.assertNotIn("SavedAtUtc", CURRENT)
        ...
        self.assertNotIn("int32 SaveVersion = CurrentSaveVersion;", CURRENT)
        self.assertNotIn(
            "int32 SaveVersion = CurrentSaveVersion;",
            locked_only,
        )
    ...
    def test_contract_does_not_relock_leftover_mission_save_record(
        ...
        for token in MISSION_SAVE_RECORD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT)
        self.assertNotIn("FSkyguardMissionSaveRecord", CURRENT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CURRENT}\n"
        self.assertEqual(
            require_declaration(locked_only, CURRENT),
            CURRENT,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(CURRENT, "Rifle")
        self.assertNotEqual(CURRENT, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign CurrentSaveVersion contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", CURRENT)

    def test_contract_is_current_save_version_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
