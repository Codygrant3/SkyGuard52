...
# then `TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors;`
# (NO initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|MissionMap`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS absent.
# This is a public TArray landmark-anchor field, not a combat look float,
# not leftover Theater WeatherIdentity, not leftover FName
# AssemblyRevision, and not leftover analog apache-cpg-feel
# FireRate / RecoilPitch. Fail-closed if the
# UPROPERTY or decl is missing or renamed, if Category is missing,
# or if specifiers drop to VisibleAnywhere / BlueprintReadOnly
...
# UPROPERTY fields ONLY after `class ASkyguardMissionMapAssemblyDirector`.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `private:`.
# Stop BEFORE sibling FlightClearanceRadiusCentimeters as a claimed slot.
# Skip banned Ig+laMuzzle, all Ig+la* fields, Ri+fleMesh /
# HandMesh. Do not claim those.
# Do NOT claim leftover Root / FlightRouteSpline /
# MissionDefinition / MissionId / AssemblyRevision / SkylineStyle /
# WeatherProfileId / RoutePoints / ObjectiveAnchors /
# FlightClearanceRadiusCentimeters /
# FlightClearanceVerticalCentimeters / Readiness as this slot.
# Do NOT claim leftover FSkyguardMissionLandmarkAnchor.LandmarkId
# #1387 / Role #1388 / WorldLocation #1390 / bMissionExclusive
# #1391 (struct member fields, not this director array).
# Do NOT claim leftover mission-landmark-anchor-defaults #4230
# (struct defaults bulk).
# Do NOT claim leftover FName WeatherIdentity TheaterKit #1300
# (different class, Category Theater, no initializer).
# Do NOT claim leftover drafted AssemblyRevision #1630 /
# FlightRouteSpline #1631 / FlightClearanceRadiusCentimeters
# #1632 / FlightClearanceVerticalCentimeters #1633.
# Do NOT claim leftover FName MissionId on this class or
# CampaignMissionSpec / StormRain.
# Do NOT claim leftover ESkyguardMissionSkylineStyle enum
# or leftover FSkyguardMissionMapReadiness struct fields.
...
# Category Skyguard|MissionMap wrapping FName AssemblyRevision
# = TEXT("CampaignMapAssembly_v1") — RETARGET hard:
# wrap stays EditAnywhere BlueprintReadWrite
# Category = "Skyguard|MissionMap" (spaces around =).
# LOCKED_DECL is `TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors;`
# NOT `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# and NOT `FName WeatherIdentity;` and NOT `FName LandmarkId;`
# and NOT `TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors;`
# and NOT `TArray<FVector> RoutePoints;` and NOT
...
# / Harbor 40 / 80 / FireRate = 12.0f / RecoilPitch = 0.92f
# / FlareCount = 6 AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is Skyguard|Combat
# or still Skyguard|Theater or still Category="Skyguard"
# without |MissionMap.
# Public TArray fail-closed if LOCKED_DECL is leftover
# combat look float or leftover CPG TObjectPtr mesh/text
# or leftover FName AssemblyRevision.
...
TARGET = "TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors;"
TARGET_WRONG_BARE = "FName AssemblyRevision;"
TARGET_WRONG_INIT = "TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors = {};"
TARGET_WRONG_FALSE = "TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors = false;"
TARGET_WRONG_TRUE = "TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors = true;"
TARGET_WRONG_ZERO = "TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LandmarkAnchors;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LandmarkAnchors;"
TARGET_WRONG_ASSEMBLY_REVISION = (
    'FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");'
)
TARGET_WRONG_LANDMARK_ID = "FName LandmarkId;"
TARGET_WRONG_OBJECTIVE_ANCHORS = (
    "TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors;"
)
TARGET_WRONG_ROUTE_POINTS = "TArray<FVector> RoutePoints;"
TARGET_WRONG_FLIGHT_CLEARANCE = (
    "float FlightClearanceRadiusCentimeters = 3000.f;"
)
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LandmarkAnchors;"
...
STOP_BEFORE_SKYLINE_STYLE = "SkylineStyle"
STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS = "FlightClearanceRadiusCentimeters"
...
LANDMARK_ANCHORS_DECL_RE = re.compile(
    r"TArray\s*<\s*FSkyguardMissionLandmarkAnchor\s*>\s+LandmarkAnchors\s*;"
)
LANDMARK_ANCHORS_INIT_RE = re.compile(
    r"TArray\s*<\s*FSkyguardMissionLandmarkAnchor\s*>\s+LandmarkAnchors\s*="
)
ASSEMBLY_REVISION_DECL_RE = re.compile(
    r"FName\s+AssemblyRevision\b"
)
LANDMARK_ID_DECL_RE = re.compile(
    r"FName\s+LandmarkId\b"
)
OBJECTIVE_ANCHORS_DECL_RE = re.compile(
    r"TArray\s*<\s*FSkyguardMissionObjectiveAnchor\s*>\s+ObjectiveAnchors\b"
)
ROUTE_POINTS_DECL_RE = re.compile(
    r"TArray\s*<\s*FVector\s*>\s+RoutePoints\b"
)
FLIGHT_CLEARANCE_DECL_RE = re.compile(
    r"float\s+FlightClearanceRadiusCentimeters\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover FName AssemblyRevision, leftover FName LandmarkId,
    # leftover ObjectiveAnchors / RoutePoints,
    # leftover FlightClearanceRadiusCentimeters,
    # leftover FName MissionId, leftover float LookYawLimit = 95.f,
    ...
    # MissionDefinition / SkylineStyle / Readiness, or an
    # initializer on LandmarkAnchors.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LANDMARK_ANCHORS_DECL_RE.search(compact) is None:
        return False
    if LANDMARK_ANCHORS_INIT_RE.search(compact):
        return False
    if ASSEMBLY_REVISION_DECL_RE.search(compact):
        return False
    if LANDMARK_ID_DECL_RE.search(compact):
        return False
    if OBJECTIVE_ANCHORS_DECL_RE.search(compact):
        return False
    if ROUTE_POINTS_DECL_RE.search(compact):
        return False
    if FLIGHT_CLEARANCE_DECL_RE.search(compact):
        return False
...
def field_identifier(field_text: str) -> str:
    ...
    match = re.search(
        r"(?:TObjectPtr<[^>]+>|TSubclassOf<[^>]+>|TArray<[^>]+>|float|int32|bool|"
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS:
            break
        if ident != "LandmarkAnchors":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS}"
        )
    ...
    for sibling in (
        STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray<FSkyguardMissionLandmarkAnchor>\s+LandmarkAnchors\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors "
...
class MissionMapLandmarkAnchorsFieldDeclContractTests(unittest.TestCase):
    def test_mission_map_assembly_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LandmarkAnchors"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_ASSEMBLY_REVISION), body)
        self.assertFalse(has_identifier(section, SIBLING_ASSEMBLY_REVISION), section)
        self.assertTrue(has_identifier(body, SIBLING_OBJECTIVE_ANCHORS), body)
        self.assertFalse(has_identifier(section, SIBLING_OBJECTIVE_ANCHORS), section)
        self.assertTrue(has_identifier(body, SIBLING_ROUTE_POINTS), body)
        self.assertFalse(has_identifier(section, SIBLING_ROUTE_POINTS), section)
        self.assertTrue(has_identifier(body, SIBLING_ROOT), body)
        self.assertFalse(has_identifier(section, SIBLING_ROOT), section)
        self.assertTrue(has_identifier(body, SIBLING_MISSION_ID), body)
        self.assertFalse(has_identifier(section, SIBLING_MISSION_ID), section)
        self.assertTrue(has_identifier(body, SIBLING_FLIGHT_ROUTE_SPLINE), body)
        self.assertFalse(
            has_identifier(section, SIBLING_FLIGHT_ROUTE_SPLINE),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_MISSION_DEFINITION), body)
        self.assertFalse(has_identifier(section, SIBLING_MISSION_DEFINITION), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LandmarkAnchors"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LandmarkAnchors", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_landmark_anchors_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("FName LandmarkId;", compact_origin)
        self.assertNotIn(
            "TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors;",
            compact_origin,
        )
        self.assertNotIn("TArray<FVector> RoutePoints;", compact_origin)
        self.assertNotIn(
            "float FlightClearanceRadiusCentimeters = 3000.f;",
            compact_origin,
        )
    ...
    def test_landmark_anchors_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("FSkyguardMissionLandmarkAnchor", LOCKED_DECL)
        self.assertIn("LandmarkAnchors", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ASSEMBLY_REVISION)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LANDMARK_ID)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_OBJECTIVE_ANCHORS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_POINTS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FLIGHT_CLEARANCE)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotIn("AssemblyRevision", LOCKED_DECL)
        self.assertNotIn("ObjectiveAnchors", LOCKED_DECL)
        self.assertNotIn("RoutePoints", LOCKED_DECL)
        self.assertNotIn("FlightClearanceRadiusCentimeters", LOCKED_DECL)
        self.assertNotIn("LandmarkId", LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FIRE_RATE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RECOIL_PITCH)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tFName AssemblyRevisions = TEXT(\"CampaignMapAssembly_v1\");\n",
            "\tint32 LandmarkAnchors = 1;\n",
            "\tbool LandmarkAnchors = true;\n",
            "\tTArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors = " + forty + ";\n",
            "\tTArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors = " + eighty + ";\n",
            f"\t{TARGET_WRONG_ASSEMBLY_REVISION}\n",
            f"\t{TARGET_WRONG_LANDMARK_ID}\n",
            f"\t{TARGET_WRONG_OBJECTIVE_ANCHORS}\n",
            f"\t{TARGET_WRONG_ROUTE_POINTS}\n",
            f"\t{TARGET_WRONG_FLIGHT_CLEARANCE}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category = "Skyguard|MissionMap"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category = "Skyguard|MissionMap"', theater_specs)
        self.assertIn('Category = "Skyguard|MissionMap"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            '\tTArray<FSkyguardMissionLandmarkAnchor>\n\tLandmarkAnchors;\n',
            '\tTArray<FSkyguardMissionLandmarkAnchor>   LandmarkAnchors;\n',
            '\tTArray<FSkyguardMissionLandmarkAnchor>\tLandmarkAnchors;\n',
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_flight_clearance_radius_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "LandmarkAnchors"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS))
        self.assertFalse(has_identifier(section, STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS))
        self.assertFalse(has_identifier(section, SIBLING_ASSEMBLY_REVISION))
        self.assertFalse(has_identifier(section, SIBLING_OBJECTIVE_ANCHORS))
        self.assertFalse(has_identifier(section, SIBLING_ROUTE_POINTS))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LandmarkAnchors"))
        self.assertTrue(has_identifier(section, "LandmarkAnchors"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS))
        self.assertFalse(has_identifier(section, STOP_BEFORE_FLIGHT_CLEARANCE_RADIUS))
        self.assertTrue(has_identifier(leaked, SIBLING_ASSEMBLY_REVISION))
        self.assertFalse(has_identifier(section, SIBLING_ASSEMBLY_REVISION))
        self.assertTrue(has_identifier(leaked, SIBLING_OBJECTIVE_ANCHORS))
        self.assertFalse(has_identifier(section, SIBLING_OBJECTIVE_ANCHORS))
        self.assertTrue(has_identifier(leaked, SIBLING_ROUTE_POINTS))
        self.assertFalse(has_identifier(section, SIBLING_ROUTE_POINTS))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_FLIGHT_ROUTE_SPLINE))
        self.assertFalse(has_identifier(section, SIBLING_FLIGHT_ROUTE_SPLINE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map LandmarkAnchors field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_map_landmark_anchors_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        self.assertIn(LEFTOVER_GUNNER_LOOK_YAW_LIMIT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ASSEMBLY_REVISION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LANDMARK_ID_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LANDMARK_DEFAULTS, LOCKED_SCRIPTS)
