# THIS IS leftover-safe FSkyguardRadioLine Sound.
...
# drop to BlueprintReadOnly / VisibleAnywhere-only, or
# if the type is not TSoftObjectPtr<USoundBase>. Fail-closed
# if Category= is added when origin/main has none.
...
# Do NOT parse leftover radio-line-subtitle isolated decl.
...
# or UPROPERTY fields.
# THIS IS NOT leftover briefing-radio-row-subtitle #c332
# (FSkyguardBriefingRadioRow::Subtitle).
# THIS IS NOT leftover briefing-card-defaults then isolated
# CardId. Analog: leftover radio-line-defaults #172 then
# this isolated Sound decl.
# If a clone asserts FName / FText / float / `= 160.f` /
# Category="Skyguard|Theater" / VisibleAnywhere /
# BlueprintReadOnly, retarget: type is
# TSoftObjectPtr<USoundBase>, no `=`,
# EditAnywhere + BlueprintReadWrite, no Category=.
...
TARGET_WRONG_EMPTY = "TSoftObjectPtr<USoundBase> Sound = TSoftObjectPtr<USoundBase>();"
...
SIBLING_BUILDING_TINT = SIBLING_PRIORITY
SIBLING_LAMP_TREATMENT = SIBLING_ESTIMATED_DURATION
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundBase\s*>\s+Sound\b",
            compact[index:],
        ):
...
class RadioLineSoundFieldDeclContractTests(unittest.TestCase):
    def test_radio_line_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Sound"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Sound"), section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio-line Sound field decl contract "
