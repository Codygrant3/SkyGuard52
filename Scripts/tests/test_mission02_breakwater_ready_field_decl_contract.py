...
# INDEX_NONE or lock bBreakwaterReady
# construction in the .cpp. This is leftover-safe
# Mission02 IntegrationReadiness bBreakwaterReady
...
# NOT leftover Mission10 bGunnerReady FIELD #814 /
# branch cursor/mission10-gunner-ready-field-decl-
# contract-c332, NOT leftover Mission02
# bGunnerReady FIELD / branch cursor/mission02-
# gunner-ready-field-decl-contract-c332, NOT leftover
# GetBreakwater method #843 / branch
# cursor/mission02-get-breakwater-decl-contract-c332,
# NOT leftover Breakwater boss field contracts
# #540-#545 (debris / engine / elevator / decoy /
# latch fields), NOT leftover director Readiness
...
# an isolated Python bBreakwaterReady
# field declaration contract on
# FSkyguardMission02IntegrationReadiness.
# Leftover #56-#64 plus leftover Mission10
# route-phase enum / protected-group enum /
# protected-runtime-defaults stay locked as
# sibling isolated contracts. This is not leftover
# GetBreakwater #843 and not leftover Breakwater
# boss fields #540-#545.
LOCKED = {
...
LEFTOVER_MISSION02_GET_BREAKWATER_NOT_LOCKED = (
    "test_mission02_get_breakwater_decl_contract.py",
    "cursor/mission02-get-breakwater-decl-contract-c332",
    "GetBreakwater",
)
LEFTOVER_BREAKWATER_BOSS_FIELDS_NOT_LOCKED = (
...
LEFTOVER_SAME_NAME_BREAKWATER_READY_NOT_LOCKED = (
    "test_mission01_breakwater_ready_field_decl_contract.py",
    "test_mission03_breakwater_ready_field_decl_contract.py",
    "test_mission04_breakwater_ready_field_decl_contract.py",
    "test_mission05_breakwater_ready_field_decl_contract.py",
    "test_mission06_breakwater_ready_field_decl_contract.py",
    "test_mission07_breakwater_ready_field_decl_contract.py",
    "test_mission08_breakwater_ready_field_decl_contract.py",
    "test_mission09_breakwater_ready_field_decl_contract.py",
    "test_mission10_breakwater_ready_field_decl_contract.py",
)
...
class Mission02BreakwaterReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bBreakwaterReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_breakwater_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_breakwater_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bBreakwaterReady"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("= false;"), LOCKED_DECL)
        self.assertIn("bBreakwaterReady", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bBreakwaterReady", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bBreakwaterReady"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bBreakwaterReady", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission02 bBreakwaterReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission02 bBreakwaterReady contains "
    ...
    def test_contract_is_root_field_declaration_only(
        ...
        for group in leftover_groups:
            for token in group:
                self.assertNotIn(token, locked_only)
                self.assertNotIn(token, LOCKED_DECL)
        # Leftover pilot-command roster tokens (Break / Hold /
        # Climb / Descend) must stay unlocked. Use a word-boundary
        # check so leftover Break does not collide with this
        # IntegrationReadiness field name.
        for token in leftover_short_roster_values():
            self.assertIsNone(
                re.search(rf"\b{re.escape(token)}\b", locked_only),
                token,
            )
            self.assertIsNone(
                re.search(rf"\b{re.escape(token)}\b", LOCKED_DECL),
                token,
            )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission02_breakwater_ready"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
