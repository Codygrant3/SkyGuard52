# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot::bValid.
...
# Exact identifier vs sibling `bWon` (same type+initializer).
# Do NOT claim sibling fields: bWon, MissionTitle,
...
TARGET_WRONG_TRUE = "bool bValid = true;"
TARGET_WRONG_ZERO = "bool bValid = 0;"
TARGET_WRONG_ONE = "bool bValid = 1;"
TARGET_WRONG_FLOAT = "bool bValid = 0.f;"
TARGET_WRONG_BARE = "bool bValid;"
TARGET_WRONG_INT = "int32 bValid = false;"
TARGET_WRONG_INT_ZERO = "int32 bValid = 0;"
TARGET_WRONG_WON = "bool bWon = false;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `bool bValid = false;`.
    # Do not accept `= true` / `= 0` / `= 0.f` / bare `bool bValid;`
    # / leftover sibling `bool bWon = false;` (same type+initializer,
    # different identifier). Do not accept int32 / float retargets.
    compact_decl = collapsed(declaration)
    if re.search(r"\bbool\s+bValid\s*=\s*false\s*;", compact_decl) is None:
        return False
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"\bbool\s+bValid\s*=\s*false\s*;", compact) is None:
        return False
    if re.search(r"\bbool\s+bValid\s*=\s*true\b", compact):
        return False
    if re.search(r"\bbool\s+bValid\s*=\s*0(?:\.f)?\b", compact):
        return False
    if re.search(r"\bbool\s+bValid\s*;", compact):
        return False
    if re.search(r"\bint32\s+bValid\b", compact):
        return False
    if re.search(r"\bfloat\s+bValid\b", compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str | None:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bValid\b", compact[index:]
        ):
...
class CpgDebriefSnapshotValidFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bValid"), section)
    ...
    def test_actor_or_helper_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bValid"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        self.assertNotIn(STOP_BEFORE_PATROL_CLASS, section)
    ...
    def test_missing_valid_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WON}\n"
    ...
    def test_valid_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_WON}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_WON}\n", LOCKED_DECL)
        self.assertIn("bValid", str(raised.exception))

    def test_sibling_bwon_same_type_does_not_satisfy(self) -> None:
        self.assertEqual(TARGET_WRONG_WON, "bool bWon = false;")
        self.assertEqual(LOCKED_DECL, "bool bValid = false;")
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WON)
        self.assertTrue(has_identifier(TARGET_WRONG_WON, SIBLING_WON))
        self.assertFalse(has_identifier(TARGET_WRONG_WON, "bValid"))
        self.assertTrue(has_identifier(LOCKED_DECL, "bValid"))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_WON))
        origin = spec_section(origin_main_header())
        self.assertTrue(has_identifier(origin, "bValid"))
        self.assertTrue(has_identifier(origin, SIBLING_WON))
        self.assertTrue(has_declaration(origin, LOCKED_DECL))
        self.assertFalse(has_declaration(origin, TARGET_WRONG_WON))
        won_only = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WON}\n"
        ...
        section = spec_section(won_only)
        self.assertTrue(has_identifier(section, SIBLING_WON), section)
        self.assertFalse(has_identifier(section, "bValid"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("bValid", str(raised.exception))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot bValid field decl contract "
