...
# INDEX_NONE or lock DebrisSecondarySensor construction
# in the .cpp. This is a FIELD contract on
# ASkyguardLifelineHunterBoss, not leftover
# LifelineHunter sibling debris fields
# DebrisPrimarySensor / DebrisControlSurface
# and not leftover weak-point fields
# OpticalTracker / WeaponServo /
# CountermeasurePod / Engine. This is not a
# leftover USkyguardBossWeakPointComponent
# member contract. Do not lock leftover
# emergency-finish named surfaces
# (split tokens only). Sibling debris fields
# DebrisPrimarySensor and DebrisControlSurface
# must not satisfy the primary lock. Weak-point
# fields OpticalTracker, WeaponServo,
# CountermeasurePod, Engine must not satisfy
# this mesh field. Do not lock
# MinimumWeaponSeparationMeters
# (Harbor-adjacent 450.f, not Harbor 40/80).
# Stay off leftover RunwayBreaker sibling
# fields RunwayRack / HangarRack /
# HeatManifold / PortEngine /
# DebrisPayloadBay / DebrisEngine /
# DebrisPortWing. Keep leftover IronRain
# DebrisPortWing (PR #531, different class)
# unlocked. Keep leftover RoadHunter Engine /
# DebrisEngine and leftover Breakwater Engine /
# DebrisEngine unlocked.
...
# DebrisSecondarySensor;`); accept that form and other
...
# section of ASkyguardLifelineHunterBoss only.
...
# DebrisPortPanel / DebrisStarboardPanel.
# Do not lock leftover sibling RoadHunter
# fields TargetingCamera / LeftActuator /
# RightActuator / Engine / DebrisLeftWing /
# DebrisEngine / DebrisRightWing. Stay off leftover
...
# isolated-test drafts #107–#559 including
...
# scanned in the LifelineHunter public section and
...
# Leftover LastFlight field. Same PortEngine
# declaration text as sibling RunwayBreaker
# PortEngine, different class. Not the
# primary lock.
LAST_FLIGHT_PORT_ENGINE_FIELD = (
    "TObjectPtr<USkyguardBossWeakPointComponent> PortEngine;"
)
# Leftover IronRain field. Same DebrisPortWing
# declaration text, different class. Not the
# primary lock.
IRON_RAIN_DEBRIS_PORT_WING_FIELD = (
    "TObjectPtr<UStaticMeshComponent> DebrisPortWing;"
)
DEBRIS_PAYLOAD_BAY_FIELD = (
    "TObjectPtr<UStaticMeshComponent> DebrisPayloadBay;"
)
RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD = (
    "TObjectPtr<UStaticMeshComponent> DebrisEngine;"
)
# Leftover RoadHunter fields. Same Engine / DebrisEngine
# declaration text as leftover Breakwater, different class.
...
# leftover #107–#559, plus
# SkyguardLifelineHunterBoss production files.
# This lane only adds an isolated Python
# DebrisSecondarySensor field declaration contract on
# ASkyguardLifelineHunterBoss. Stay off leftover
...
# leftover isolated-test drafts #107–#559,
# leftover searchlight-track-runtime-defaults
...
def leftover_live_copy_boss_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        "_finish_armed_decl_contract.py",
        f"{prefix}test_runway_breaker_arm_emergency_{banned}"
        "_finish_decl_contract.py",
        f"{prefix}test_runway_breaker_is_emergency_{banned}"
        "_finish_armed_decl_contract.py",
    )
...
# unlocked for THIS test's primary DebrisSecondarySensor lock. Leftover
...
LEFTOVER_RADAR_GHOST_ENGINE_NOT_LOCKED = (
    "test_radar_ghost_engine_field_decl_contract.py",
    "ASkyguardRadarGhostBoss",
)
LEFTOVER_LAST_FLIGHT_PORT_ENGINE_NOT_LOCKED = (
...
SIBLING_RUNWAY_BREAKER_FIELDS_NOT_LOCKED = (
    "RunwayRack",
    "HangarRack",
    "HeatManifold",
    "PortEngine",
...
SIBLING_LIFELINE_HUNTER_FIELDS_NOT_LOCKED = (
    "OpticalTracker",
    "WeaponServo",
    "CountermeasurePod",
    "DebrisPrimarySensor",
...
LEFTOVER_ROAD_HUNTER_ENGINE_NOT_LOCKED = (
    "test_road_hunter_engine_field_decl_contract.py",
    "ASkyguardRoadHunterBoss",
)
LEFTOVER_ROAD_HUNTER_DEBRIS_ENGINE_NOT_LOCKED = (
    "test_road_hunter_debris_engine_field_decl_contract.py",
    "DebrisEngine",
)
...
LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED = (
    ...
    "test_lifeline_hunter_debris_primary_sensor_field_decl_contract.py",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLifelineHunterBoss::DebrisSecondarySensor",
    "SkyguardLifelineHunterBoss.cpp",
    "SetExposed",
...
class LifelineHunterDebrisSecondarySensorFieldDeclContractTests(unittest.TestCase):
    def test_lifeline_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_missing_debris_secondary_sensor_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardLifelineHunterBoss();\n"
            ...
            f"\t{WEAPON_SERVO_FIELD}\n"
            f"\t{COUNTERMEASURE_POD_FIELD}\n"
            f"\t{LIFELINE_HUNTER_ENGINE_FIELD}\n"
            f"\t{DEBRIS_PRIMARY_SENSOR_FIELD}\n"
            f"\t{DEBRIS_CONTROL_SURFACE_FIELD}\n"
            f"\t{MINIMUM_WEAPON_SEPARATION}\n"
            f"\t{RUNWAY_RACK_FIELD}\n"
            f"\t{HANGAR_RACK_FIELD}\n"
            f"\t{HEAT_MANIFOLD_FIELD}\n"
            f"\t{PORT_ENGINE_FIELD}\n"
            f"\t{DEBRIS_PAYLOAD_BAY_FIELD}\n"
            f"\t{RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("Category", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_PORT)
        self.assertNotIn("BlueprintCallable", UPROPERTY_PORT)
        self.assertIn("Skyguard|Boss|Destruction", UPROPERTY_PORT)
        self.assertNotIn("Mission06", UPROPERTY_PORT)
        self.assertIn("Destruction", UPROPERTY_PORT)
        self.assertNotIn("Encounter", UPROPERTY_PORT)
        self.assertNotIn("Mission03", UPROPERTY_PORT)
        self.assertNotIn("Mission02", UPROPERTY_PORT)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisSecondarySensor;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisSecondarySensor;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> DebrisSecondarySensor;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_SECONDARY_SENSOR_FIELD)
            self.assertIn("DebrisSecondarySensor", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_secondary_sensor_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD),
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_SECONDARY_SENSOR_FIELD), 1)
        self.assertTrue(
            DEBRIS_SECONDARY_SENSOR_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
        self.assertTrue(DEBRIS_SECONDARY_SENSOR_FIELD.endswith(";"), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HeatManifold", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("RunwayRack", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPayloadBay", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPrimarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisControlSurface", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("OpticalTracker", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("WeaponServo", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("CountermeasurePod", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("MinimumWeaponSeparationMeters", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("LeftActuator", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("RightActuator", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisLeftWing", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisRightWing", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisCoolingDoor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("RadarReceiver", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("CoolingDoor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("TArray<", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("=", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("{", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("}", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("return ", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisCenter", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("RadarNode", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_SECONDARY_SENSOR_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {DEBRIS_SECONDARY_SENSOR_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DEBRIS_SECONDARY_SENSOR_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DEBRIS_SECONDARY_SENSOR_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_SECONDARY_SENSOR_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD),
            section,
    ...
    def test_assigned_debris_secondary_sensor_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisSecondarySensor = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_SECONDARY_SENSOR_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DebrisSecondarySensor", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_SECONDARY_SENSOR_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_SECONDARY_SENSOR_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_SECONDARY_SENSOR_FIELD.startswith("UFUNCTION"),
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_SECONDARY_SENSOR_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_SECONDARY_SENSOR_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_secondary_sensor_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotIn("{", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("}", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("return ", DEBRIS_SECONDARY_SENSOR_FIELD)
        ...
        self.assertNotIn("SkyguardLifelineHunterBoss.cpp", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SkyguardLifelineHunterBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("return true", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisIntakePanel", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DispenserCenter", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("IsContactIdentified", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_debris_cooling_door_does_not_satisfy_debris_secondary_sensor(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_COOLING_DOOR_FIELD)
        self.assertNotIn("DebrisCoolingDoor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DispenserStarboard", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetManeuver", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), DEBRIS_SECONDARY_SENSOR_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        self.assertNotIn("CoolingDoor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("OpenSensorExposure", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("MinimumWeaponSeparationMeters", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_open_safe_window(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_safe_engine_fallback(), DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("IsLightningExposed", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_SECONDARY_SENSOR_FIELD),
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
        ...
        self.assertNotIn("HangarRack", locked_only)
        self.assertNotIn("HeatManifold", locked_only)
        self.assertNotIn("RunwayRack", locked_only)
        self.assertNotIn("DebrisPayloadBay", locked_only)
        self.assertNotIn("TargetingCamera", locked_only)
        ...
        self.assertNotIn("DebrisPrimarySensor", locked_only)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss", section)
        ...
        self.assertNotIn("ASkyguardLifelineHunterBoss::DebrisSecondarySensor", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "lifeline hunter DebrisSecondarySensor field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_SECONDARY_SENSOR_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_SECONDARY_SENSOR_FIELD.lower(),
                "lifeline hunter DebrisSecondarySensor contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_contract_is_debris_secondary_sensor_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DEBRIS_SECONDARY_SENSOR_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(UPROPERTY_PORT, section)

    def test_contract_does_not_relock_sibling_runway_breaker_fields(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        for token in SIBLING_RUNWAY_BREAKER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HeatManifold", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        self.assertNotIn("DebrisEngine", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisRightWing", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardRoadHunterBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("StarboardLatch", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DecoyPods", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ElevatorLinkage", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_runway_breaker_port_engine_is_not_breakwater_engine(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(collapsed(DEBRIS_SECONDARY_SENSOR_FIELD), collapsed(ENGINE_FIELD))
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertEqual(BREAKWATER_ENGINE_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardBreakwaterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardRoadHunterBoss")
        ...
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardBreakwaterBoss.h")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardRoadHunterBoss.h")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardRadarGhostBoss.h")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardPathfinderBoss.h")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardLastFlightBoss.h")
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardLastFlightBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_runway_breaker_debris_engine_is_not_leftover_debris_engine(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotEqual(
        ...
        for token in LEFTOVER_BREAKWATER_DEBRIS_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        for token in LEFTOVER_ROAD_HUNTER_DEBRIS_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DEBRIS_SECONDARY_SENSOR_FIELD)
    ...
    def test_runway_breaker_port_engine_is_not_radar_ghost_engine(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardRadarGhostBoss")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardRadarGhostBoss.h")
        for token in LEFTOVER_RADAR_GHOST_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "Scripts/tests/test_radar_ghost_engine_field_decl_contract.py",
    ...
    def test_runway_breaker_port_engine_is_not_pathfinder_engine(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotEqual(
            collapsed(DEBRIS_SECONDARY_SENSOR_FIELD),
            collapsed(ENGINE_FIELD),
        )
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardPathfinderBoss")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardPathfinderBoss.h")
    ...
    def test_runway_breaker_port_engine_is_not_road_hunter_engine(self) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardRoadHunterBoss")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardRoadHunterBoss.h")
        for token in LEFTOVER_ROAD_HUNTER_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "Scripts/tests/test_road_hunter_engine_field_decl_contract.py",
    ...
    def test_hangar_rack_does_not_satisfy_debris_secondary_sensor(self) -> None:
        leftover = f"\t{HANGAR_RACK_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD))
        self.assertNotEqual(HANGAR_RACK_FIELD, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_heat_manifold_and_runway_rack_do_not_satisfy_debris_secondary_sensor(
        self,
    ) -> None:
        for region, name in (
            (f"\t{HEAT_MANIFOLD_FIELD}\n", "HeatManifold"),
    ...
    def test_runway_breaker_sibling_fields_do_not_satisfy_debris_secondary_sensor(
        self,
    ) -> None:
        for region, name in (
            (f"\t{PORT_ENGINE_FIELD}\n", "PortEngine"),
    ...
    def test_debris_engine_does_not_satisfy_debris_secondary_sensor(self) -> None:
        leftover = f"\t{RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("UStaticMeshComponent", RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
    ...
    def test_targeting_camera_does_not_satisfy_debris_secondary_sensor(self) -> None:
        leftover = f"\t{TARGETING_CAMERA_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD))
        self.assertNotEqual(TARGETING_CAMERA_FIELD, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_left_and_right_actuators_do_not_satisfy_debris_secondary_sensor(self) -> None:
        for region, name in (
            (f"\t{LEFT_ACTUATOR_FIELD}\n", "LeftActuator"),
            (f"\t{RIGHT_ACTUATOR_FIELD}\n", "RightActuator"),
        ):
    ...
    def test_lifeline_hunter_debris_secondary_sensor_is_not_iron_rain_debris_port_wing(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, IRON_RAIN_DEBRIS_PORT_WING_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardIronRainBoss")
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardIronRainBoss.h",
        )
        for token in LEFTOVER_DEBRIS_PORT_WING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "Scripts/tests/test_iron_rain_debris_port_wing_field_decl_contract.py",
        ...
        self.assertNotIn("ASkyguardIronRainBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", locked_only)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPortWing", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_port_engine_does_not_satisfy_debris_secondary_sensor(self) -> None:
        leftover = f"\t{PORT_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD))
        self.assertNotEqual(PORT_ENGINE_FIELD, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_ENGINE_FIELD,
        )
    ...
    def test_runway_breaker_port_engine_is_not_last_flight_port_engine(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        self.assertNotEqual(DEBRIS_SECONDARY_SENSOR_FIELD, LAST_FLIGHT_PORT_ENGINE_FIELD)
        self.assertEqual(PORT_ENGINE_FIELD, LAST_FLIGHT_PORT_ENGINE_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardLifelineHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardLastFlightBoss")
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardLastFlightBoss.h",
        )
        for token in LEFTOVER_LAST_FLIGHT_PORT_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "Scripts/tests/test_last_flight_port_engine_field_decl_contract.py",
        ...
        self.assertNotIn("ASkyguardLastFlightBoss", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("ASkyguardLastFlightBoss", locked_only)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_SECONDARY_SENSOR_FIELD)


    def test_lifeline_hunter_sibling_debris_fields_do_not_satisfy(self) -> None:
        for region, name in (
            (f"\t{DEBRIS_PRIMARY_SENSOR_FIELD}\n", "DebrisPrimarySensor"),
            (f"\t{DEBRIS_CONTROL_SURFACE_FIELD}\n", "DebrisControlSurface"),
        ):
            ...
            self.assertFalse(has_declaration(region, DEBRIS_SECONDARY_SENSOR_FIELD))
            self.assertNotIn(name, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotEqual(DEBRIS_PRIMARY_SENSOR_FIELD, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotEqual(DEBRIS_CONTROL_SURFACE_FIELD, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_PRIMARY_SENSOR_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_leftover_weak_point_fields_do_not_satisfy_mesh_field(self) -> None:
        for region, name in (
            (f"\t{OPTICAL_TRACKER_FIELD}\n", "OpticalTracker"),
            (f"\t{WEAPON_SERVO_FIELD}\n", "WeaponServo"),
            (f"\t{COUNTERMEASURE_POD_FIELD}\n", "CountermeasurePod"),
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_SECONDARY_SENSOR_FIELD))
            self.assertNotIn(name.rstrip(";"), DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            OPTICAL_TRACKER_FIELD,
        )
    ...
    def test_minimum_weapon_separation_is_not_the_primary_lock(self) -> None:
        leftover = f"\t{MINIMUM_WEAPON_SEPARATION}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_SECONDARY_SENSOR_FIELD))
        self.assertNotIn("MinimumWeaponSeparationMeters", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("450.f", DEBRIS_SECONDARY_SENSOR_FIELD)
        for token in leftover_harbor_tokens():
            self.assertNotEqual(token, "450.f")
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)

    def test_leftover_safe_window_methods_do_not_satisfy(self) -> None:
        for region in (
            f"\t{leftover_open_safe_window()}\n",
            f"\t{leftover_arm_safe_engine_fallback()}\n",
    ...
    def test_contract_does_not_relock_sibling_lifeline_hunter_fields(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_SECONDARY_SENSOR_FIELD}\n"
        for token in SIBLING_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertIn("DebrisSecondarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        self.assertNotIn("DebrisPrimarySensor", DEBRIS_SECONDARY_SENSOR_FIELD)
        ...
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardBossWeakPointComponent.h")
        self.assertNotIn(
            "SkyguardBossWeakPointComponent.h",
            DEBRIS_SECONDARY_SENSOR_FIELD,
        )
