# THIS IS leftover-safe ASkyguardCampaignTheaterKit::GetNamedLandmark.
# origin/main form: split-line
# UFUNCTION(BlueprintPure, Category="Skyguard|Theater")
# then `FName GetNamedLandmark() const { return AppliedSpec.NamedLandmark; }`.
...
# replaces BlueprintPure, if const is dropped, or if the
# return type is not FName.
...
# GetAppliedKitId / GetBuildingTint / GetAppliedSpec
# stay uncontracted.
...
# GetNamedLandmark does not block this isolated
...
# UPROPERTY then leftover GetAppliedKitId
# then this isolated GetNamedLandmark UFUNCTION.
# If a clone asserts UPROPERTY / spec struct /
# Category="Skyguard|Campaign" / FName WeatherIdentity /
# float / `= 160.f`, retarget: this is
# UFUNCTION(BlueprintPure, Category="Skyguard|Theater")
# FName GetNamedLandmark() const, parse window is the
# actor public section.
# leftover actor UFUNCTION GetAppliedWeatherIdentity /
# GetAppliedKitId / GetBuildingTint stay locked.
...
TARGET = "FName GetNamedLandmark() const;"
TARGET_INLINE = (
    "FName GetNamedLandmark() const { return AppliedSpec.NamedLandmark; }"
)
TARGET_WRONG_BARE = "FName GetNamedLandmark();"
TARGET_WRONG_NO_CONST = (
    "FName GetNamedLandmark() { return AppliedSpec.NamedLandmark; }"
)
TARGET_WRONG_VOID = "void GetNamedLandmark() const;"
TARGET_WRONG_FLOAT = "float GetNamedLandmark() const;"
TARGET_WRONG_BOOL = "bool GetNamedLandmark() const;"
TARGET_WRONG_RENAME = "FName GetNamed() const;"
TARGET_WRONG_SIBLING_WEATHER = (
    "FName GetAppliedWeatherIdentity() const;"
)
TARGET_WRONG_SIBLING_KIT_ID = "FName GetAppliedKitId() const;"
TARGET_WRONG_SIBLING_TINT = "FLinearColor GetBuildingTint() const;"
TARGET_WRONG_SIBLING_SPEC = (
...
GET_NAMED_LANDMARK_SIGNATURE_RE = re.compile(
    r"\bFName\s+GetNamedLandmark\s*\(\s*\)\s*const\s*(?:;|\{)"
)
INVENTED_UFUNCTION = (
    "BlueprintCallable",
    "VisibleAnywhere",
...
def leftover_spec_names_not_in_locked_decl() -> tuple[str, ...]:
    # GetNamedLandmark contains the NamedLandmark stem; do not treat
    # that substring as contracting leftover FName NamedLandmark.
    return tuple(
        name
        for name in leftover_spec_field_names()
        if name != SIBLING_NAMED_LANDMARK
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `FName GetNamedLandmark() const`. Accept origin/main
    # inline body without locking the body, plus a trailing
    # semicolon form. Do not accept a missing const, a
    # void/float/bool rename, extra params, sibling
    # GetAppliedWeatherIdentity / GetAppliedKitId /
    # GetBuildingTint / GetAppliedSpec, or leftover
    # ApplyTheaterKitToWorld when origin/main has
    # `FName GetNamedLandmark() const`.
    del declaration
    compact = collapsed(region)
    if GET_NAMED_LANDMARK_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+GetNamedLandmark\b", compact):
        return False
    if re.search(r"\bfloat\s+GetNamedLandmark\b", compact):
        return False
    if re.search(r"\bbool\s+GetNamedLandmark\b", compact):
        return False
    if re.search(r"\bFName\s+GetNamedLandmark\s*\(\s*[^)]+\s*\)", compact):
        return False
    return True
...
def count_one_declaration(region: str, declaration: str) -> int:
    del declaration
    compact = collapsed(region)
    return len(GET_NAMED_LANDMARK_SIGNATURE_RE.findall(compact))
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+GetNamedLandmark\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for FName GetNamedLandmark is missing from "
...
class TheaterKitGetNamedLandmarkDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetNamedLandmark"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetNamedLandmark", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_named_landmark_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn(
            "GetNamedLandmark()const { return AppliedSpec.NamedLandmark; }",
            compact_origin,
        )
        self.assertNotIn("void GetNamedLandmark", compact_origin)
        self.assertNotIn("float GetNamedLandmark", compact_origin)
        self.assertNotIn("bool GetNamedLandmark", compact_origin)

    def test_get_named_landmark_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName GetNamedLandmark()"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SIBLING_KIT_ID}\n",
            f"\t{TARGET_WRONG_SIBLING_TINT}\n",
            f"\t{TARGET_WRONG_SIBLING_SPEC}\n",
            leftover_primary,
            leftover_guided,
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetNamedLandmark decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_THEATER_KIT_GET_APPLIED_KIT_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_GET_BUILDING_TINT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_ufunction_declaration_only(self) -> None:
        ...
        self.assertIn("GetNamedLandmark", locked_only)
        self.assertIn("FName ", locked_only)
