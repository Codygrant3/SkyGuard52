...
# bAudibleDeviceObserved.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(BlueprintReadOnly)
# then `bool bAudibleDeviceObserved = false;`.
# BlueprintReadOnly IS present. There is NO EditAnywhere.
# There is NO Category=. Type is `bool`. Initializer is
# exactly `= false`. Fail-closed if initializer dropped or
# changed to `= true`.
# Fail-closed if field missing/renamed, EditAnywhere added,
# BlueprintReadWrite replaces BlueprintReadOnly, type is not
# bool, initializer is not `= false`, or Category= is added.
...
# The API macro is REQUIRED in the stop token. Naive
...
# Do NOT parse the leftover harness class body.
...
# Do NOT parse leftover enum class ESkyguardProductionAudioCategory
...
# Sibling receipt floats MaximumAudioThreadMs and
# MaximumTruePeakDbTP live in this struct — do not treat
# `= 0` + `.f` or `= -120` + `.f` as Harbor. Harbor is
# 40 + `.f` / 80 + `.f` only.
# Analog leftover audible-acceptance-receipt-defaults #193 /
# #1cd7 then leftover-safe isolated BuildSha256 #1435 through
# PeakActiveVoices #1438 then this isolated decl.
...
# bPackagedDevelopmentBuild / bCalibratedMetering /
# bProductionBankReady / bAccepted.
# Identifier bounds: match EXACT `bAudibleDeviceObserved` as a
...
TARGET = "bool bAudibleDeviceObserved = false;"
TARGET_WRONG_BARE = "bool bAudibleDeviceObserved;"
TARGET_WRONG_TRUE = "bool bAudibleDeviceObserved = true;"
TARGET_WRONG_ZERO = "bool bAudibleDeviceObserved = 0;"
TARGET_WRONG_EQ_NONE = "bool bAudibleDeviceObserved = NAME_None;"
TARGET_WRONG_EQ_FLOAT = "bool bAudibleDeviceObserved = 0.f;"
TARGET_WRONG_INT = "int32 bAudibleDeviceObserved = 0;"
TARGET_WRONG_FSTRING = "FString bAudibleDeviceObserved;"
TARGET_WRONG_FLOAT = "float bAudibleDeviceObserved = false;"
TARGET_WRONG_FNAME = "FName bAudibleDeviceObserved;"
TARGET_WRONG_FTEXT = "FText bAudibleDeviceObserved;"
TARGET_WRONG_TARRAY = "TArray<bool> bAudibleDeviceObserved;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> bAudibleDeviceObserved;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_PACKAGED = "bool bPackagedDevelopmentBuild = false;"
...
FIELD_NAME = "bAudibleDeviceObserved"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bAudibleDeviceObserved = false;`.
    # Match the EXACT identifier bAudibleDeviceObserved as a whole
    # field name after bool. Do NOT accept sibling bools
    # bPackagedDevelopmentBuild / bCalibratedMetering /
    # bProductionBankReady / bAccepted. Reject dropped/changed
    # initializer (`= true` or bare), Harbor 40/80, wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"bool\s+bAudibleDeviceObserved\s*=\s*false\s*;",
        compact,
    )
    ...
    if re.search(r"\bbAccepted\b", compact) and exact is None:
        return False
    if re.search(
        r"bool\s+bAudibleDeviceObserved\s*;",
        compact,
    ) and re.search(
        r"bool\s+bAudibleDeviceObserved\s*=",
        compact,
    ...
    for assigned in re.findall(
        r"bool\s+bAudibleDeviceObserved\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "false":
            return False
    if re.search(
        r"int32\s+bAudibleDeviceObserved\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bAudibleDeviceObserved\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bAudibleDeviceObserved\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+bAudibleDeviceObserved\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+bAudibleDeviceObserved\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+bAudibleDeviceObserved\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+bAudibleDeviceObserved\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bAudibleDeviceObserved\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bAudibleDeviceObserved is missing from "
...
class AudibleAcceptanceReceiptAudibleDeviceObservedFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_audible_device_observed_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            ...
            f"\tbool {SIBLING_PACKAGED_DEVELOPMENT_BUILD} = false;\n"
    ...
    def test_sibling_bools_do_not_satisfy_audible_device_observed(self) -> None:
        packaged_only = f"\t{TARGET_WRONG_PACKAGED}\n"
        self.assertFalse(has_declaration(packaged_only, LOCKED_DECL))
        self.assertFalse(has_identifier(packaged_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(packaged_only, SIBLING_PACKAGED_DEVELOPMENT_BUILD)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(packaged_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bAudibleDeviceObserved = false;", compact_origin)
        self.assertNotIn("bAudibleDeviceObserved = true", compact_origin)
        self.assertNotIn("int32 bAudibleDeviceObserved", compact_origin)
        self.assertNotIn("FString bAudibleDeviceObserved", compact_origin)
        self.assertNotIn("float bAudibleDeviceObserved", compact_origin)

    def test_audible_device_observed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bAudibleDeviceObserved"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn(SIBLING_PEAK_ACTIVE_VOICES, LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_PEAK_ACTIVE_VOICES} = 0;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tbool bAudibleDeviceObserveds = false;\n",
            "\tbool bAudibleDeviceObserved = true;\n",
            "\tbool bAudibleDeviceObserved;\n",
            "\tfloat bAudibleDeviceObserved = " + forty + ";\n",
            "\tfloat bAudibleDeviceObserved = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_receipt_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_PEAK_ACTIVE_VOICES),
            section,
        )
        self.assertTrue(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt bAudibleDeviceObserved field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audible_device_observed_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_audible", THIS_SCRIPT)
