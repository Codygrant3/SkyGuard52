# THIS IS leftover-safe FSkyguardMissionDebrief::NextMissionDisplayName.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `FText NextMissionDisplayName;` with no Category=, no
# initializer, and no MultiLine. Narrative has MultiLine; this
# field does not. If a clone asserts MultiLine, drop that. If a
# clone asserts `= false` / `= 0` / `= FText()` / an initializer,
# drop that and accept the bare decl. If a clone asserts Category=
# and origin/main has none, drop that assertion.
# Fail-closed if the type/decl is missing or renamed, if
# Category= is added, or if an initializer appears.
...
# bNewBestMedal / bProgressSaved #1271.
# THIS IS NOT leftover FSkyguardMissionDebrief::MissionDisplayName
# (different field).
...
# bProgressSaved, SaveSlotName, NextMissionId, NextMissionMap,
# bNextMissionUnlocked, bCampaignComplete).
...
# no Category=. Accept bare `FText NextMissionDisplayName;`.
...
TARGET = "FText NextMissionDisplayName;"
TARGET_WRONG_ZERO = "FText NextMissionDisplayName = 0;"
TARGET_WRONG_FALSE = "FText NextMissionDisplayName = false;"
TARGET_WRONG_ONE = "FText NextMissionDisplayName = 1;"
TARGET_WRONG_FLOAT = "FText NextMissionDisplayName = 0.f;"
TARGET_WRONG_TRUE = "FText NextMissionDisplayName = true;"
TARGET_WRONG_INIT = "FText NextMissionDisplayName = FText();"
TARGET_WRONG_EMPTY = "FText NextMissionDisplayName = FText::GetEmpty();"
TARGET_WRONG_NAME = "FName NextMissionDisplayName;"
TARGET_WRONG_STRING = "FString NextMissionDisplayName;"
TARGET_WRONG_SIBLING = "FText MissionDisplayName;"
TARGET_WRONG_ID = "FName NextMissionId;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # = false / = 1 / = 0.f / = true / FText() / FText::GetEmpty()
    # for FSkyguardMissionDebrief::NextMissionDisplayName.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"FText\s+NextMissionDisplayName\s*=", compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    field = re.search(r"FText\s+NextMissionDisplayName\b", compact)
    if field is None:
        raise AssertionError(
            "UPROPERTY for FText NextMissionDisplayName is missing from "
...
class MissionDebriefNextMissionDisplayNameFieldDeclContractTests(
    unittest.TestCase
):
    def test_mission_debrief_struct_exists(self) -> None:
        ...
        self.assertIn("NextMissionId", section)
        ...
        self.assertNotIn("int32 MedalTier", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("NextMissionDisplayName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_next_mission_display_name_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
    ...
    def test_next_mission_display_name_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText NextMissionDisplayName"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "FText NextMissionDisplayName;")
        ...
        self.assertIn("NextMissionDisplayName", LOCKED_DECL)
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIBLING)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ID)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_STRING}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SIBLING}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ID}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            "\tFText NextMissionDisplayName = 0;\n",
            "\tFText NextMissionDisplayName = false;\n",
            "\tbool NextMissionDisplayName = false;\n",
            "\tFString NextMissionDisplayName;\n",
            "\tFName NextMissionDisplayName;\n",
            "\tFName NextMissionId;\n",
            "\tFText MissionDisplayName;\n",
            "\tFText NextMissionDisplayName = FText();\n",
            "\tFText NextMissionDisplayName = FText::GetEmpty();\n",
    ...
    def test_does_not_contract_neighbors_or_director_methods(self) -> None:
        ...
        self.assertIn("Narrative", section)
        self.assertIn("MissionDisplayName", section)
        self.assertIn("NextMissionDisplayName", section)
        self.assertNotIn("FinalScore", section)
        self.assertNotIn("MedalTier", section)
        self.assertNotIn(GET_OBJECTIVE_RUNTIME, section)
        self.assertNotIn(ADD_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(NEIGHBOR_PROGRESS, section)
        self.assertNotIn(NEIGHBOR_RESULT, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief NextMissionDisplayName field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_next_mission_display_name"
            "_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIBLING)
        self.assertNotIn("Narrative", locked_only)
        self.assertIn("NextMissionDisplayName", locked_only)
        ...
        self.assertNotIn("SaveSlotName", locked_only)
        self.assertNotIn("NextMissionId", locked_only)
        self.assertNotIn("NextMissionMap", locked_only)
        self.assertNotIn("bNextMissionUnlocked", locked_only)
        self.assertNotIn("bCampaignComplete", locked_only)
