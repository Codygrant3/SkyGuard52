# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot bWon.
...
# Exact identifier vs sibling `bValid` (same type+initializer).
# Do not claim sibling fields: bValid, MissionTitle,
# OutcomeNarrative, Score, Medal, ShotsFired, Hits,
# CargoPercent, bRadarDead, DestroyedSystems,
# SelectedLoadout, CannonReady, RocketReady, GuidedReady.
...
# `FName WeatherIdentity` / bare no-`=` FName, RETARGET:
# parse FSkyguardCpgDebriefSnapshot only, type is bool,
# required initializer `= false`, no UPROPERTY wrap.
...
TARGET = "bool bWon = false;"
TARGET_WRONG_BARE = "bool bWon;"
TARGET_WRONG_TRUE = "bool bWon = true;"
TARGET_WRONG_ZERO = "bool bWon = 0;"
TARGET_WRONG_FLOAT = "bool bWon = 0.f;"
TARGET_WRONG_EQ_NONE = "bool bWon = NAME_None;"
TARGET_WRONG_INT32 = "int32 bWon = false;"
TARGET_WRONG_INT_ZERO = "int32 bWon = 0;"
TARGET_WRONG_FNAME = "FName bWon;"
TARGET_WRONG_FSTRING = "FString bWon;"
TARGET_WRONG_FTEXT = "FText bWon;"
TARGET_WRONG_TARRAY = "TArray<bool> bWon;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SIBLING_VALID = "bool bValid = false;"
TARGET_WRONG_SIBLING_RADAR = "bool bRadarDead = false;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bWon = false;`.
    # Match EXACT identifier bWon as a whole field name
    # after bool. Do NOT accept sibling `bool bValid = false;`
    # or `bool bRadarDead = false;` (same type+initializer).
    # Do NOT accept missing `=` / `= true` / `= 0` / `= 0` + `.f`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"bool\s+bWon\s*=\s*false\s*;",
        compact,
    )
    if exact is None:
        return False
    if re.search(r"\bbValid\b", compact) and exact is None:
        return False
    if re.search(r"\bbRadarDead\b", compact) and exact is None:
        return False
    if re.search(r"bool\s+bWon\s*;", compact) and re.search(
        r"bool\s+bWon\s*=",
        compact,
    ) is None:
        return False
    assign = re.search(r"bool\s+bWon\s*=\s*([^;]+);", compact)
    if assign is None:
        return False
    if assign.group(1).strip() != "false":
        return False
    if re.search(r"float\s+bWon\b", compact):
        return False
    if re.search(r"int32\s+bWon\b", compact):
        return False
    if re.search(r"FName\s+bWon\b", compact):
        return False
    if re.search(r"FString\s+bWon\b", compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str | None:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bWon\b", compact[index:]
        ):
...
def require_no_attached_uproperty(section: str) -> None:
    ...
    compact = collapsed(section)
    if re.search(r"UPROPERTY\s*\([^)]*\)\s*bool\s+bWon\b", compact):
        raise AssertionError(
            f"{STRUCT_NAME} {FIELD_NAME} must not wrap UPROPERTY"
        )


class CpgDebriefSnapshotWonFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_won_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_valid_same_type_initializer_does_not_satisfy(self) -> None:
        ...
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        radar_only = f"\t{TARGET_WRONG_SIBLING_RADAR}\n"
        self.assertFalse(has_declaration(radar_only, LOCKED_DECL))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(radar_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
    ...
    def test_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bWon = false;", compact_origin)
        self.assertNotIn("bool bWon = true;", compact_origin)
        self.assertNotIn("bool bWon = 0;", compact_origin)
        self.assertNotIn("bool bWon = 0.f;", compact_origin)
        self.assertNotIn("FName WeatherIdentity;", compact_origin)

    def test_won_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bWon"),
            LOCKED_DECL,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot bWon field decl contract "
