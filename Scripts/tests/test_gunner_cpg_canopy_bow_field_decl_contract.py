# THIS IS leftover-safe ASkyguardGunner CpgCanopyBow.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgCanopyBow;`
# (no initializer).
...
# Stop BEFORE sibling CpgDash as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeLeft, CpgKneeRight, CpgDash, Boom,
# GunnerCamera, CpgTedac.
# Do NOT claim leftover CpgSeatBack / CpgCockpit /
# CpgStation (in-flight siblings). CpgKneeRight,
# CpgKneeLeft, and CpgSeatPan are sibling in-flight slots.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgCanopyBow;`
...
# CpgCanopyBow fail-closed if LOCKED_DECL is leftover
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgCanopyBow;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgCanopyBow = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgCanopyBow = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgCanopyBow = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgCanopyBow;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgCanopyBow;"
...
CPG_CANOPY_BOW_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgCanopyBow\s*;"
)
CPG_CANOPY_BOW_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgCanopyBow\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgKneeRight / CpgSeatPan / CpgCockpit /
    # CpgStation / leftover Apache Canopy, or an
    # initializer on CpgCanopyBow as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_CANOPY_BOW_DECL_RE.search(compact) is None:
        return False
    if CPG_CANOPY_BOW_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_DASH):
            break
    ...
    if has_identifier(window, SIBLING_CPG_DASH):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgCanopyBow\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgCanopyBow "
...
class GunnerCpgCanopyBowFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgCanopyBow"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_DASH), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_DASH),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgCanopyBow"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgCanopyBow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_canopy_bow_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgCanopyBow;", compact_origin)
        self.assertNotIn("CpgCanopyBow = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_canopy_bow_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgCanopyBow"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgCanopyBow;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgCanopyBows;\n",
            "\tint32 CpgCanopyBow = 140;\n",
            "\tbool CpgCanopyBow = true;\n",
            "\tfloat CpgCanopyBow = " + forty + ";\n",
            "\tfloat CpgCanopyBow = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_dash_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertIn(TARGET_WRONG_KNEE_RIGHT, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgCanopyBow field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_canopy_bow_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
