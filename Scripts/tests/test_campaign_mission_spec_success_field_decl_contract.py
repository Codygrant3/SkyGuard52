# THIS IS leftover-safe FSkyguardCampaignMissionSpec Success.
# origin/main form: plain C++ field `const TCHAR* Success = TEXT("");`
...
# Fail-closed if initializer is missing or not `= TEXT("")`.
...
# Do NOT parse leftover `FSkyguardMissionPresentation`
# SuccessDebrief as this slot.
...
# invent Harbor 40/80.
# Do NOT claim leftover StormRainBeatKit Title as the
# locked decl / parse window.
...
# a plain-struct const TCHAR* field with TEXT("").
# Parse window comes from leftover-safe CampaignMissionSpec
# MissionId (stop before namespace SkyguardCampaignRoster).
# TCHAR* / TEXT("") locking comes from leftover-safe
# StormRain Title, BUT this slot is CampaignMissionSpec
# Success, not leftover StormRain Title.
# Do NOT claim leftover StormRain Title.
# UPROPERTY rejection is copied from leftover-safe
# CampaignMissionSpec MissionId. Do not keep that
# MissionId lock.
...
LEFTOVER_MISSION_PRESENTATION = "FSkyguardMissionPresentation"
...
TARGET = 'const TCHAR* Success = TEXT("");'
TARGET_WRONG_BARE = "const TCHAR* Success;"
TARGET_WRONG_EQ_NONE = "const TCHAR* Success = NAME_None;"
TARGET_WRONG_FALSE = "const TCHAR* Success = false;"
TARGET_WRONG_TRUE = "const TCHAR* Success = true;"
TARGET_WRONG_ZERO = "const TCHAR* Success = 0.f;"
TARGET_WRONG_FLOAT = "float Success;"
TARGET_WRONG_HEALTH = "float Success = 160.f;"
TARGET_WRONG_BOOL = "bool Success;"
TARGET_WRONG_INT = "int32 Success;"
TARGET_WRONG_FNAME = "FName Success;"
TARGET_WRONG_TEXT = 'const TCHAR* Success = TEXT("Clear");'
...
TARGET_WRONG_SUCCESS_DEBRIEF = "FText SuccessDebrief;"
LOCKED_DECL = TARGET
...
STOP_BEFORE_MISSION_PRESENTATION = "struct FSkyguardMissionPresentation"
...
LEFTOVER_SUCCESS_DEBRIEF = "SuccessDebrief"
...
def leftover_campaign_spec_sibling_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_CAMPAIGN_SPEC_MISSION_ID,
        LEFTOVER_CAMPAIGN_SPEC_TITLE,
        LEFTOVER_CAMPAIGN_SPEC_BRIEF,
...
LOCKED_SCRIPTS = (
    leftover_campaign_spec_sibling_scripts()
...
SUCCESS_DECL_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*Success\s*=\s*TEXT\s*\(\s*\"\"\s*\)\s*;"
)
SUCCESS_INIT_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*Success\s*=\s*([^;]+);"
)
INVENTED_FIELD_MSG = (
    "INDEX_NONE",
    "NAME_None",
...
def leftover_sibling_decls() -> tuple[str, ...]:
    return (
        TARGET_WRONG_MISSION_ID,
        TARGET_WRONG_TITLE,
        TARGET_WRONG_BRIEF,
        TARGET_WRONG_FAILURE,
        TARGET_WRONG_WEATHER_LABEL,
        TARGET_WRONG_WEATHER_IDENTITY,
        TARGET_WRONG_SUCCESS_DEBRIEF,
    )
...
def success_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*const TCHAR\s*\*\s*Success\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `const TCHAR* Success = TEXT("");`.
    # Do not accept missing TEXT(""), leftover Title / Brief /
    # Failure / WeatherLabel, leftover SuccessDebrief, FName
    # Success, `= NAME_None` / `= false` / `= true` / `= 0.f` /
    # `= 160.f` when origin/main locks TEXT(""). Do not accept
    # leftover StormRain Title or leftover MissionPresentation
    # SuccessDebrief as this slot. Do not invent an initializer.
    # Do not accept UPROPERTY / Category clones from leftover
    # TheaterKit or leftover SuccessDebrief.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SUCCESS_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+Success\b", compact):
        return False
    if re.search(r"FName\s+Success\b", compact):
        return False
    if re.search(r"FText\s+SuccessDebrief\b", compact):
        return False
    if re.search(r"bool\s+Success\b", compact):
        return False
    if re.search(r"int32\s+Success\b", compact):
        return False
    if re.search(
        r"const\s+TCHAR\s*\*\s*Title\b",
        compact,
    ) and not has_identifier(compact, "Success"):
        return False
    for match in SUCCESS_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != 'TEXT("")':
            return False
    if success_has_uproperty_wrap(region):
        return False
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_MISSION_PRESENTATION,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*const TCHAR\* Success\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for const TCHAR* Success is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*const TCHAR\* Success\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on const TCHAR* Success is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "Success"):
        raise AssertionError(
            "UPROPERTY clone landed on Success; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "Success"):
            raise AssertionError(
                f"{token} clone landed on Success; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CampaignMissionSpecSuccessFieldDeclContractTests(unittest.TestCase):
    def test_campaign_mission_spec_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_MISSION_PRESENTATION)
        ...
        self.assertTrue(has_identifier(section, "Success"), section)
        self.assertFalse(has_identifier(section, LEFTOVER_SUCCESS_DEBRIEF))
        ...
        self.assertNotIn(LEFTOVER_MISSION_PRESENTATION, section)
    ...
    def test_locked_decl_is_plain_tchar_text_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, 'const TCHAR* Success = TEXT("");')
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("const TCHAR* Success"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn('TEXT("")', LOCKED_DECL)
        self.assertIn("Success", LOCKED_DECL)
        ...
        self.assertNotIn(LEFTOVER_SUCCESS_DEBRIEF, LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("Success", str(raised.exception))
    ...
    def test_does_not_claim_leftover_storm_day_night_or_result(self) -> None:
        ...
        self.assertIn(TARGET_WRONG_TITLE, storm_header)
        ...
        self.assertIn(TARGET_WRONG_MISSION_ID, day_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn(TARGET_WRONG_MISSION_ID, night_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn(TARGET_WRONG_MISSION_ID, result_header)
        self.assertIn(LEFTOVER_MISSION_PRESENTATION, result_header)
        self.assertIn(LEFTOVER_SUCCESS_DEBRIEF, result_header)
        self.assertNotIn(LOCKED_DECL, result_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_leftover_storm_rain_title_is_not_this_slot(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TITLE)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_STORM_KIT)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_STORM_HEADER)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_STORM_KIT_TITLE)
    ...
    def test_leftover_success_debrief_is_not_this_slot(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SUCCESS_DEBRIEF)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_MISSION_PRESENTATION)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_MISSION_TYPES_HEADER)
        self.assertNotEqual(
        ...
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SUCCESS_DEBRIEF}\n"
            "};\n"
        )
        section = spec_section(leftover)
        self.assertFalse(has_declaration(section, LOCKED_DECL), section)
        ...
        self.assertFalse(
            has_declaration(wrapped_section, LOCKED_DECL),
            wrapped_section,
        )
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Success"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_sibling_title_does_not_satisfy_success(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_TITLE}\n"
    ...
    def test_initializer_must_be_empty_text(self) -> None:
        ...
        filled_section = spec_section(filled)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(filled_section, LOCKED_DECL)
        self.assertIn("Success", str(raised.exception))
        ...
        self.assertIn('const TCHAR* Success = TEXT("");', compact_origin)
        self.assertNotIn("Success = NAME_None", compact_origin)
        self.assertNotIn("Success = false", compact_origin)
        self.assertNotIn("Success = true", compact_origin)
        self.assertNotIn("Success = 0.f", compact_origin)
        self.assertNotIn("Success = 160.f", compact_origin)
        self.assertNotIn('Success = TEXT("Clear")', compact_origin)
    ...
    def test_success_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("const TCHAR* Success"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FAILURE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_MISSION_ID}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(
                f"\t{TARGET_WRONG_WEATHER_IDENTITY}\n",
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_TITLE}\n", LOCKED_DECL)
        self.assertIn("Success", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tconst TCHAR* Successes = TEXT(\"\");\n",
            "\tint32 Success;\n",
            "\tbool Success;\n",
            "\tfloat Success = " + forty + ";\n",
            "\tfloat Success = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    success_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(success_has_uproperty_wrap(origin), origin)
    ...
    def test_does_not_contract_sibling_spec_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Success"))
    ...
    def test_does_not_parse_leftover_kits_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_MISSION_PRESENTATION, header)
        self.assertNotIn(LEFTOVER_MISSION_PRESENTATION, header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec Success field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "success_field_decl_contract.py"
        ))
        ...
        self.assertNotIn("mission_presentation", Path(__file__).name)
        self.assertNotIn("SkyguardCampaignRoster.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardStormRainBeatKit.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_CAMPAIGN_SPEC_MISSION_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CAMPAIGN_SPEC_TITLE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CAMPAIGN_SPEC_BRIEF, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CAMPAIGN_SPEC_FAILURE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_STORM_KIT_TITLE, LOCKED_SCRIPTS)
        self.assertIn(
            LEFTOVER_MISSION_PRESENTATION_SUCCESS_DEBRIEF,
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_MISSION_PRESENTATION, locked_only)
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CAMPAIGN_SPEC_MISSION_ID)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CAMPAIGN_SPEC_TITLE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CAMPAIGN_SPEC_BRIEF)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_CAMPAIGN_SPEC_FAILURE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_STORM_KIT_TITLE)
        self.assertNotEqual(
            THIS_SCRIPT,
            LEFTOVER_MISSION_PRESENTATION_SUCCESS_DEBRIEF,
