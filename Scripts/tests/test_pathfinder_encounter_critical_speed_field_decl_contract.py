...
# then `float CriticalSpeedCmPerSecond = 700.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Route`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"100.0"`.
# Default IS `= 700.f`.
...
# LockWindowSpeedCmPerSecond (leave siblings uncontracted).
...
# if ClampMin is missing, if the default is not `= 700.f`,
# if Category is Skyguard|Apache / Skyguard|Combat /
# Skyguard|Theater / Skyguard|MissionMap /
# Skyguard|Audio|Acceptance / Skyguard|Audio|Development /
# or still Category="Skyguard" without leftover-safe unique
# Category path `Skyguard|Boss|Encounter|Route`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `USkyguardPathfinderEncounterController` public
# UPROPERTY fields ONLY after `class USkyguardPathfinderEncounterController`.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `private:`.
# Do NOT claim sibling RouteLengthCm (leftover analog
# coastal/Mission01 same-name 45000.f).
# Do NOT claim sibling ApproachSpeedCmPerSecond /
# LockWindowSpeedCmPerSecond / IngressSwayAmplitudeCm /
# IngressSwayWavelengthCm / LockWindowClimbCm /
# LockWindowClimbSeconds / CriticalTurnRadiusCm /
# CriticalTurnSeconds as this slot.
...
# TObjectPtr field. Do NOT claim leftover analog Pathfinder debris.
# Do NOT claim leftover analog apache-cpg-feel FireRate = 12.0f /
# RecoilPitch = 0.92f.
# Skip leftover analog audio-director / ProductionBank / PropellerLoop.
# Skip leftover GetResolvedProductionLoopRouteCount /
# AreResolvedProductionLoopRoutesComplete / OpenCockpitWindLoop /
# GlobalVoiceLimit.
# Skip private members.
# Skip banned Ig+la / Ri+fle / Ya+k (split live-copy tokens).
# Clone wrap was VisibleAnywhere BlueprintReadOnly
# Category="Skyguard" wrapping leftover Boom, leftover
# OpenCockpitWindLoop TSoftObjectPtr, leftover FName
# WeatherIdentity Theater, leftover VegetationStartCullDistanceCm
# — RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Route"
# meta ClampMin `"100.0"` (NO spaces around =).
# LOCKED_DECL is `float CriticalSpeedCmPerSecond = 700.f;`
...
# and NOT leftover ApproachSpeedCmPerSecond and NOT leftover
# LockWindowSpeedCmPerSecond and NOT leftover analog
# `float FireRate = 12.0f` / `float RecoilPitch = 0.92f`
# and NOT leftover LookYawLimit = 95.f and NOT leftover
# RouteLengthCm = 45000.f.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# / Boom / VegetationStartCullDistanceCm / GlobalVoiceLimit
# / OpenCockpitWindLoop / ApproachSpeedCmPerSecond /
# LockWindowSpeedCmPerSecond AS THE LOCKED DECL.
...
TARGET = "float CriticalSpeedCmPerSecond = 700.f;"
TARGET_WRONG_BARE = "float CriticalSpeedCmPerSecond;"
TARGET_WRONG_INIT = "float CriticalSpeedCmPerSecond = nullptr;"
TARGET_WRONG_FALSE = "float CriticalSpeedCmPerSecond = false;"
TARGET_WRONG_TRUE = "float CriticalSpeedCmPerSecond = true;"
TARGET_WRONG_ZERO = "float CriticalSpeedCmPerSecond = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CriticalSpeedCmPerSecond;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CriticalSpeedCmPerSecond;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CriticalSpeedCmPerSecond;"
...
TARGET_WRONG_SEED = "int32 PlacementSeed = 5201;"
TARGET_WRONG_EPIC_TREE = "int32 EpicTreeBudget = 240;"
TARGET_WRONG_EPIC_SHRUB = "int32 EpicShrubBudget = 480;"
TARGET_WRONG_END_CULL = "int32 VegetationEndCullDistanceCm = 32000;"
TARGET_WRONG_TRACE = "float TraceRange = 32000.f;"
TARGET_WRONG_VEGETATION = (
...
TARGET_WRONG_LOCK_WINDOW_SPEED = "float LockWindowSpeedCmPerSecond = 850.f;"
TARGET_WRONG_OPEN_COCKPIT = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;"
TARGET_WRONG_GLOBAL_VOICE = "int32 GlobalVoiceLimit = 24;"
TARGET_WRONG_CULL_START = "int32 VegetationStartCullDistanceCm = 8000;"
TARGET_WRONG_BOOM_ARM = "TObjectPtr<USpringArmComponent> Boom;"
TARGET_WRONG_CLIMB_SECONDS = "float LockWindowClimbSeconds = 2.4f;"
TARGET_WRONG_ENCOUNTER_PTR = (
    "TObjectPtr<USkyguardPathfinderEncounterController> "
    "EncounterController;"
)
TARGET_WRONG_AUDIO_ACCEPT = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Audio|Acceptance")'
)
TARGET_WRONG_AUDIO_DEV = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
...
STOP_BEFORE_LOOK_PITCH_MIN = "IngressSwayAmplitudeCm"
STOP_BEFORE_EPIC_SHRUB = "IngressSwayAmplitudeCm"
...
SIBLING_LOOK_PITCH_MIN = "IngressSwayAmplitudeCm"
SIBLING_SEED = "ApproachSpeedCmPerSecond"
SIBLING_TREE_INSTANCES = "LockWindowSpeedCmPerSecond"
SIBLING_SHRUB_INSTANCES = "bAutoAdvance"
SIBLING_WIND_SOURCE = "WindSource"
SIBLING_VFX_POOL = "VFXPool"
SIBLING_QUALITY = "LockWindowClimbCm"
SIBLING_ROUTE_LENGTH = "RouteLengthCm"
SIBLING_WIND_STRENGTH = "WindStrength"
SIBLING_WIND_SPEED = "WindSpeed"
SIBLING_READINESS = "Readiness"
SIBLING_APPLIED_WEATHER = "AppliedWeather"
SIBLING_TREE_COUNT = "TreeInstanceCount"
SIBLING_ROOT = "RouteLengthCm"
...
SIBLING_INGRESS_SWAY_AMP = "IngressSwayAmplitudeCm"
SIBLING_INGRESS_SWAY_WAVE = "IngressSwayWavelengthCm"
SIBLING_LOCK_WINDOW_CLIMB = "LockWindowClimbCm"
...
SIBLING_IS_ROUTE_SAFE = "IsRouteStateSafe"
SIBLING_GET_ROUTE_PROGRESS = "GetRouteProgress"
SIBLING_GET_SPEED_MULTIPLIER = "GetEffectiveSpeedMultiplier"
SIBLING_IS_TELEGRAPH_ACTIVE = "IsAttackTelegraphActive"
SIBLING_GET_TELEGRAPHS = "GetTelegraphsTriggered"
SIBLING_OPEN_COCKPIT = "OpenCockpitWindLoop"
SIBLING_GLOBAL_VOICE = "GlobalVoiceLimit"
SIBLING_PRODUCTION_BANK = "ProductionBank"
SIBLING_PROPELLER_LOOP = "PropellerLoop"
SIBLING_ENGINE_IDLE = "EngineIdleLoop"
SIBLING_CULL_START = "VegetationStartCullDistanceCm"
SIBLING_ENCOUNTER_CONTROLLER = "EncounterController"
LEFTOVER_AUDIO_DIRECTOR_CLASS = "USkyguardAudioDirectorComponent"
LEFTOVER_COASTAL_CLASS = "ASkyguardCoastalEnvironmentDirector"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_critical_speed"
    "_field_decl_contract.py"
)
LEFTOVER_OPEN_COCKPIT_WIND = (
    "Scripts/tests/test_audio_director_open_cockpit_wind_loop"
    "_field_decl_contract.py"
)
LEFTOVER_GLOBAL_VOICE_LIMIT = (
    "Scripts/tests/test_audio_director_global_voice_limit"
    "_field_decl_contract.py"
)
LEFTOVER_ENGINE_IDLE_LOOP = (
    "Scripts/tests/test_audio_director_engine_idle_loop"
    "_field_decl_contract.py"
)
LEFTOVER_ENGINE_POWER_LOOP = (
    "Scripts/tests/test_audio_director_engine_power_loop"
    "_field_decl_contract.py"
)
LEFTOVER_ENGINE_CRUISE_LOOP = (
    "Scripts/tests/test_audio_director_engine_cruise_loop"
    "_field_decl_contract.py"
)
LEFTOVER_RESOLVED_LOOP_COUNT = (
    "Scripts/tests/test_audio_director_get_resolved_production_loop"
    "_route_count_decl_contract.py"
)
LEFTOVER_RESOLVED_LOOP_COMPLETE = (
    "Scripts/tests/test_audio_director_are_resolved_production_loop"
    "_routes_complete_decl_contract.py"
)
LEFTOVER_PATHFINDER_CONTROLLER_FIELD = (
    "Scripts/tests/test_pathfinder_encounter_controller"
    "_field_decl_contract.py"
)
LEFTOVER_PATHFINDER_ADVANCE = (
    "Scripts/tests/test_pathfinder_advance_encounter"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_RESET = (
    "Scripts/tests/test_pathfinder_reset_encounter_state"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_ROUTE_SAFE = (
    "Scripts/tests/test_pathfinder_is_route_state_safe"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_ROUTE_PROGRESS = (
    "Scripts/tests/test_pathfinder_get_route_progress"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_SPEED_MULT = (
    "Scripts/tests/test_pathfinder_get_effective_speed_multiplier"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_TELEGRAPH_ACTIVE = (
    "Scripts/tests/test_pathfinder_is_attack_telegraph_active"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_TELEGRAPHS = (
    "Scripts/tests/test_pathfinder_get_telegraphs_triggered"
    "_decl_contract.py"
)
LEFTOVER_PATHFINDER_DEBRIS_NOSE = (
    "Scripts/tests/test_pathfinder_debris_nose"
    "_field_decl_contract.py"
)
LEFTOVER_PATHFINDER_DEBRIS_CENTER = (
    "Scripts/tests/test_pathfinder_debris_center"
    "_field_decl_contract.py"
)
LEFTOVER_PATHFINDER_DEBRIS_SPINE = (
    "Scripts/tests/test_pathfinder_debris_spine"
    "_field_decl_contract.py"
)
LEFTOVER_PATHFINDER_DEBRIS_TAIL = (
    "Scripts/tests/test_pathfinder_debris_tail"
    "_field_decl_contract.py"
)
...
LEFTOVER_COASTAL_START_CULL = (
    "Scripts/tests/test_coastal_vegetation_start_cull_distance_cm"
    "_field_decl_contract.py"
)
LEFTOVER_LOOK_YAW_LIMIT = (
...
LEFTOVER_COASTAL_EPIC_TREE_BUDGET = (
    "Scripts/tests/test_coastal_epic_tree_budget"
    "_field_decl_contract.py"
)
...
LEFTOVER_COASTAL_TREE_INSTANCES = (
    "Scripts/tests/test_coastal_tree_instances"
    "_field_decl_contract.py"
)
LEFTOVER_COASTAL_SHRUB_INSTANCES = (
    "Scripts/tests/test_coastal_shrub_instances"
    "_field_decl_contract.py"
)
LEFTOVER_COASTAL_WIND_SOURCE = (
    "Scripts/tests/test_coastal_wind_source"
    "_field_decl_contract.py"
)
LEFTOVER_ENV_TREE_COUNT = (
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        "_field_decl_contract.py",
        f"{prefix}test_audio_director_global_voice_limit"
        "_field_decl_contract.py",
        f"{prefix}test_audio_director_engine_idle_loop"
        "_field_decl_contract.py",
        f"{prefix}test_audio_director_engine_power_loop"
        "_field_decl_contract.py",
        f"{prefix}test_audio_director_engine_cruise_loop"
        "_field_decl_contract.py",
        f"{prefix}test_audio_director_get_resolved_production_loop"
        "_route_count_decl_contract.py",
        f"{prefix}test_audio_director_are_resolved_production_loop"
        "_routes_complete_decl_contract.py",
        f"{prefix}test_pathfinder_encounter_controller"
        "_field_decl_contract.py",
        f"{prefix}test_pathfinder_advance_encounter"
        "_decl_contract.py",
        f"{prefix}test_pathfinder_reset_encounter_state"
        "_decl_contract.py",
        f"{prefix}test_pathfinder_is_route_state_safe"
        "_decl_contract.py",
        f"{prefix}test_pathfinder_get_route_progress"
        "_decl_contract.py",
        ...
        "_decl_contract.py",
        f"{prefix}test_pathfinder_debris_nose"
        "_field_decl_contract.py",
        f"{prefix}test_pathfinder_debris_center"
        "_field_decl_contract.py",
        f"{prefix}test_pathfinder_debris_spine"
        "_field_decl_contract.py",
        f"{prefix}test_pathfinder_debris_tail"
        "_field_decl_contract.py",
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_coastal_tree_instances_field_decl_contract.py",
    "Scripts/tests/test_coastal_placement_seed_field_decl_contract.py",
    "Scripts/tests/test_coastal_epic_tree_budget_field_decl_contract.py",
    "Scripts/tests/test_gunner_trace_range_field_decl_contract.py",
    "Scripts/tests/test_coastal_shrub_instances_field_decl_contract.py",
    "Scripts/tests/test_coastal_wind_source_field_decl_contract.py",
    "Scripts/tests/test_mission01_environment_readiness_ocean_tile_count_field_decl_contract.py",
    "Scripts/tests/test_mission01_environment_readiness_beach_tile_count_field_decl_contract.py",
    "Scripts/tests/test_mission01_environment_readiness_land_tile_count_field_decl_contract.py",
    "Scripts/tests/test_mission01_environment_readiness_authored_landscape_surface_exposed_field_decl_contract.py",
...
CRITICAL_SPEED_DECL_RE = re.compile(
    r"float\s+CriticalSpeedCmPerSecond\s*=\s*700\.f\s*;"
)
CRITICAL_SPEED_BARE_RE = re.compile(
    r"float\s+CriticalSpeedCmPerSecond\s*;"
)
CRITICAL_SPEED_WRONG_INIT_RE = re.compile(
    r"float\s+CriticalSpeedCmPerSecond\s*="
)
PLACEMENT_SEED_DECL_RE = re.compile(
    r"int32\s+PlacementSeed\s*=\s*5201\s*;"
)
EPIC_TREE_LEFTOVER_DECL_RE = re.compile(
    r"int32\s+EpicTreeBudget\s*=\s*240\s*;"
)
TRACE_RANGE_DECL_RE = re.compile(
    r"float\s+TraceRange\s*=\s*32000\.f\b"
)
END_CULL_DECL_RE = re.compile(
    r"int32\s+VegetationEndCullDistanceCm\s*=\s*32000\s*;"
)
TREE_COUNT_DECL_RE = re.compile(
    r"int32\s+TreeInstanceCount\b"
)
TREE_INSTANCES_DECL_RE = re.compile(
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        "EpicShrubBudget",
        "TraceRange",
        SIBLING_TREE_INSTANCES,
        SIBLING_SHRUB_INSTANCES,
        SIBLING_WIND_SOURCE,
        SIBLING_VFX_POOL,
        SIBLING_QUALITY,
        SIBLING_ROUTE_LENGTH,
        SIBLING_WIND_STRENGTH,
        SIBLING_WIND_SPEED,
        SIBLING_READINESS,
        SIBLING_APPLIED_WEATHER,
        SIBLING_TREE_COUNT,
        SIBLING_ROOT,
        SIBLING_FIRE_RATE,
        ...
        SIBLING_INGRESS_SWAY_AMP,
        SIBLING_INGRESS_SWAY_WAVE,
        SIBLING_LOCK_WINDOW_CLIMB,
        ...
        SIBLING_IS_ROUTE_SAFE,
        SIBLING_GET_ROUTE_PROGRESS,
        SIBLING_GET_SPEED_MULTIPLIER,
        SIBLING_IS_TELEGRAPH_ACTIVE,
        SIBLING_GET_TELEGRAPHS,
        SIBLING_OPEN_COCKPIT,
        SIBLING_GLOBAL_VOICE,
        SIBLING_PRODUCTION_BANK,
        SIBLING_PROPELLER_LOOP,
        SIBLING_ENGINE_IDLE,
        SIBLING_CULL_START,
        SIBLING_ENCOUNTER_CONTROLLER,
        SIBLING_ROUTE_LENGTH,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float CriticalSpeedCmPerSecond = 700.f;`.
    # Do not accept leftover FName WeatherIdentity,
    # leftover `= 160.f`, leftover analog apache-cpg-feel
    # FireRate / RecoilPitch, leftover CPG mesh/text,
    # leftover LoadoutSpec magazine fields, leftover
    # Harbor 40 / 80, leftover PlacementSeed = 5201,
    # leftover TreeInstanceCount, leftover TreeInstances
    # TObjectPtr, or a bare CriticalSpeedCmPerSecond without
    # the 700.f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CRITICAL_SPEED_DECL_RE.search(compact) is None:
        return False
    if CRITICAL_SPEED_BARE_RE.search(compact):
        return False
    ...
    if PLACEMENT_SEED_DECL_RE.search(compact):
        return False
    if EPIC_TREE_LEFTOVER_DECL_RE.search(compact):
        return False
    if TRACE_RANGE_DECL_RE.search(compact):
        return False
    if END_CULL_DECL_RE.search(compact):
        return False
    if TREE_COUNT_DECL_RE.search(compact):
        return False
    if TREE_INSTANCES_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"float\s+LookYawLimit\s*=\s*95\.f\b", compact):
        return False
    if re.search(r"float\s+RouteLengthCm\s*=\s*45000\.f\b", compact):
        return False
    if re.search(r"float\s+ApproachSpeedCmPerSecond\s*=\s*1250\.f\b", compact):
        return False
    if re.search(
        ...
        return False
    if re.search(r"OpenCockpitWindLoop\b", compact):
        return False
    if re.search(r"GlobalVoiceLimit\b", compact):
        return False
    if re.search(r"VegetationStartCullDistanceCm\b", compact):
        return False
    if re.search(r"TObjectPtr<USpringArmComponent>\s+Boom\b", compact):
        return False
    if re.search(r"LockWindowClimbSeconds\s*=\s*2\.4f\b", compact):
        return False
    if re.search(r"MinHeightFromOriginCm\b", compact):
        return False
    return True
...
def field_identifier(field_text: str) -> str:
    ...
    match = re.search(
        r"(?:TObjectPtr<[^>]+>|TSubclassOf<[^>]+>|float|int32|bool|"
        r"uint8|FName|FString|FVector|FRotator|FText|ESkyguard\w+)"
        r"\s+(\w+)\b",
        compact,
    )
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_LOOK_PITCH_MIN:
            break
        if ident != "CriticalSpeedCmPerSecond":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_LOOK_PITCH_MIN):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_LOOK_PITCH_MIN}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CriticalSpeedCmPerSecond\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CriticalSpeedCmPerSecond "
...
class PathfinderEncounterCriticalSpeedFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalSpeedCmPerSecond"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_LOOK_PITCH_MIN), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_ROOT), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT), section)
        self.assertTrue(has_identifier(body, SIBLING_TREE_INSTANCES), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_MPD_RIGHT_TEXT),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_SEED), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_TEDAC_TEXT), section)
        self.assertTrue(has_identifier(body, SIBLING_SHRUB_INSTANCES), body)
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE), section)
        self.assertTrue(has_identifier(body, SIBLING_QUALITY), body)
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_COASTAL_CLASS)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CriticalSpeedCmPerSecond"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CriticalSpeedCmPerSecond", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_critical_speed_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Route"', section)
        self.assertNotIn('Category="Skyguard|Environment|Vegetation"', section)
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Route"', specifiers)
        self.assertNotIn('Category="Skyguard|Environment|Vegetation"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float CriticalSpeedCmPerSecond = 700.f;", compact_origin)
        self.assertNotIn("CriticalSpeedCmPerSecond = 160", compact_origin)
        self.assertNotIn("float RouteLengthCm = 45000.f;", compact_origin)
        self.assertNotIn("float ApproachSpeedCmPerSecond = 1250.f;", compact_origin)
        self.assertNotIn("float LockWindowSpeedCmPerSecond = 850.f;", compact_origin)
        self.assertNotIn("OpenCockpitWindLoop", compact_origin)
        self.assertNotIn("GlobalVoiceLimit", compact_origin)
        self.assertNotIn("VegetationStartCullDistanceCm", compact_origin)
        self.assertNotIn("int32 PlacementSeed = 5201;", compact_origin)
        self.assertNotIn("int32 EpicTreeBudget = 240;", compact_origin)
        self.assertNotIn("int32 VegetationEndCullDistanceCm = 32000;", compact_origin)
        self.assertNotIn("float TraceRange = 32000.f;", compact_origin)
        self.assertNotIn("int32 TreeInstanceCount = 0;", compact_origin)
    ...
    def test_critical_speed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CriticalSpeedCmPerSecond"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("700.f", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float CriticalSpeedCmPerSecond = 700.f;")
        self.assertNotEqual(LOCKED_DECL, "float LookYawLimit = 95.f;")
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_LENGTH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_APPROACH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_WINDOW_SPEED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_OPEN_COCKPIT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GLOBAL_VOICE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CULL_START)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BOOM_ARM)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLIMB_SECONDS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENCOUNTER_PTR)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_YAW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SEED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_EPIC_TREE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_EPIC_SHRUB)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_END_CULL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TRACE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_COUNT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TREES)
        ...
        self.assertIsNone(
            re.search(r'Category="Skyguard"(?!\|)', UPROPERTY_EDIT_READWRITE)
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat CriticalSpeedCmPerSeconds = 700.f;\n",
            "\tfloat CriticalSpeedCmPerSecond = 700;\n",
            "\tbool CriticalSpeedCmPerSecond = true;\n",
            "\tfloat CriticalSpeedCmPerSecond = " + forty + ";\n",
            "\tfloat CriticalSpeedCmPerSecond = " + eighty + ";\n",
            f"\t{TARGET_WRONG_SEED}\n",
            f"\t{TARGET_WRONG_EPIC_TREE}\n",
            f"\t{TARGET_WRONG_END_CULL}\n",
            f"\t{TARGET_WRONG_TRACE}\n",
            f"\t{TARGET_WRONG_COUNT}\n",
            f"\t{TARGET_WRONG_TREES}\n",
            f"\t{TARGET_WRONG_YAW}\n",
            f"\t{TARGET_WRONG_ROUTE_LENGTH}\n",
            f"\t{TARGET_WRONG_APPROACH}\n",
            f"\t{TARGET_WRONG_LOCK_WINDOW_SPEED}\n",
            f"\t{TARGET_WRONG_OPEN_COCKPIT}\n",
            f"\t{TARGET_WRONG_GLOBAL_VOICE}\n",
            f"\t{TARGET_WRONG_CULL_START}\n",
            f"\t{TARGET_WRONG_BOOM_ARM}\n",
            f"\t{TARGET_WRONG_CLIMB_SECONDS}\n",
            f"\t{TARGET_WRONG_ENCOUNTER_PTR}\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Route"', origin)
        self.assertNotIn('Category="Skyguard|Environment|Vegetation"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', apache_specs)
        self.assertIn('Category="Skyguard|Apache"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
        ...
        self.assertNotIn('Category="Skyguard|Combat"', theater_specs)
        self.assertIn('Category="Skyguard|Boss|Encounter|Route"', origin)
        self.assertNotIn('Category="Skyguard|Environment|Vegetation"', origin)
        self.assertNotIn('Skyguard|Combat', origin)
        self.assertNotIn('Skyguard|MissionMap', origin)
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalSpeedCmPerSecond"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CriticalSpeedCmPerSecond"))
        self.assertTrue(has_identifier(section, "CriticalSpeedCmPerSecond"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertTrue(has_identifier(leaked, SIBLING_ROOT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(leaked, SIBLING_SEED))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, SIBLING_TREE_INSTANCES))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Combat"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('Category="Skyguard|MissionMap"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('Category="Skyguard|Audio|Acceptance"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('Category="Skyguard|Audio|Development"', UPROPERTY_EDIT_READWRITE)
        self.assertIn('Category="Skyguard|Boss|Encounter|Route"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardCoastalEnvironmentDirector.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter critical-speed field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_critical_speed_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardPathfinderEncounterController.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_LOOK_YAW_LIMIT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_PLACEMENT_SEED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_EPIC_TREE_BUDGET, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_TRACE_RANGE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_TREE_INSTANCES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_SHRUB_INSTANCES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_WIND_SOURCE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ENV_TREE_COUNT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_APPROACH_SPEED, locked_only)
        self.assertNotIn(SIBLING_LOCK_WINDOW_SPEED, locked_only)
        self.assertNotIn(SIBLING_ROUTE_LENGTH, locked_only)
        self.assertNotIn(SIBLING_OPEN_COCKPIT, locked_only)
        self.assertNotIn(SIBLING_GLOBAL_VOICE, locked_only)
        self.assertNotIn(SIBLING_CULL_START, locked_only)
        self.assertNotIn(SIBLING_ADVANCE_ENCOUNTER, locked_only)
        self.assertNotIn(SIBLING_ENCOUNTER_CONTROLLER, locked_only)
        self.assertNotIn(SIBLING_PRODUCTION_BANK, locked_only)
        self.assertNotIn(SIBLING_PROPELLER_LOOP, locked_only)
        ...
        self.assertNotIn("EpicShrubBudget", locked_only)
        self.assertNotIn("VegetationEndCullDistanceCm", locked_only)
        self.assertNotIn("TraceRange", locked_only)
        self.assertNotIn("TreeInstanceCount", locked_only)
        self.assertNotIn("TreeInstances", locked_only)
