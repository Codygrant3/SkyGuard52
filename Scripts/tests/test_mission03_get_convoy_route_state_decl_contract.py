...
# INDEX_NONE or lock GetConvoyRouteState
# construction in the .cpp. This is leftover-safe
# Mission03 Convoy getter
# GetConvoyRouteState on
...
# Mission03 wave-state enum #0242. Distinct from
...
# GetWaveState / StartNextWave /
# GetRemainingThreatsInWave /
# GetCurrentWaveIndex / GetConvoyRouteAlpha /
# GetConvoyWorldLocation / GetConvoyIntegrity /
# GetMissionId / ValidateMissionContract /
# GetDayBeatKit. Do not lock leftover
# convoy-route-state enum #d34e (enum
# values). Do not lock leftover
...
# origin/main is a split-line UFUNCTION with an
...
# Do not lock leftover convoy-route-state
# enum #d34e (enum values).
...
GET_CURRENT_WAVE_INDEX = "int32 GetCurrentWaveIndex() const"
...
# GetConvoyRouteState Convoy getter declaration
...
class Mission03GetConvoyRouteStateDeclContractTests(unittest.TestCase):
    def test_mission03_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_CONVOY_ROUTE_STATE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_CONVOY_ROUTE_STATE)
        self.assertIn("GetConvoyRouteState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_CONVOY_ROUTE_STATE)
    ...
    def test_missing_get_convoy_route_state_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission03IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_CONVOY_ROUTE_STATE)
        self.assertIn("GetConvoyRouteState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_CONVOY_ROUTE_STATE)
        self.assertIn("GetConvoyRouteState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("BlueprintPure", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("Category", GET_CONVOY_ROUTE_STATE)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_CONVOY_ROUTE_STATE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_CONVOY_ROUTE_STATE)
        self.assertIn("GetConvoyRouteState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid GetConvoyRouteState();\n"
        as_const = "\tbool GetConvoyRouteState() const;\n"
        as_int = "\tESkyguardConvoyRouteState GetConvoyRouteState();\n"
        with_arg = "\tbool GetConvoyRouteState(int32 Amount = 1);\n"
        ...
        leftover_wave_index = f"\t{START_NEXT_WAVE}\n"
        leftover_remaining = f"\t{GET_REMAINING_THREATS}\n"
        leftover_wave_idx = f"\t{GET_CURRENT_WAVE_INDEX}\n"
        leftover_convoy = f"\t{ADVANCE_CONVOY_BY_DISTANCE}\n"
        leftover_convoy_alpha = f"\t{GET_CONVOY_ROUTE_ALPHA}\n"
        leftover_convoy_world = f"\t{GET_CONVOY_WORLD_LOCATION}\n"
        ...
        as_void_name = "\tvoid GetConvoyRouteState() const;\n"
        ...
        for region in (
            ...
            leftover_wave_idx,
            leftover_convoy,
            leftover_convoy_alpha,
            leftover_convoy_world,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_CONVOY_ROUTE_STATE)
            self.assertIn("GetConvoyRouteState", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_convoy_route_state_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_CONVOY_ROUTE_STATE),
            GET_CONVOY_ROUTE_STATE,
        )
        self.assertTrue(has_declaration(section, GET_CONVOY_ROUTE_STATE))
        self.assertEqual(
            declaration_count(section, GET_CONVOY_ROUTE_STATE),
            1,
        )
        self.assertTrue(
            GET_CONVOY_ROUTE_STATE.startswith("ESkyguardConvoyRouteState"),
            GET_CONVOY_ROUTE_STATE,
        )
        self.assertTrue(
            GET_CONVOY_ROUTE_STATE.endswith(" const"),
            GET_CONVOY_ROUTE_STATE,
        )
        self.assertIn("GetConvoyRouteState", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("static ", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn(
            "const USkyguardMissionDefinition* Mission",
            GET_CONVOY_ROUTE_STATE,
        )
        ...
        self.assertNotIn("EnvironmentReadiness", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("MissionMapReadiness", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("TObjectPtr", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("TSoftObjectPtr", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("INDEX_NONE", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("UFUNCTION", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("{", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("}", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("return ", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn(" const;", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("Root", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("Briefing", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("AudioDirector", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("RadioChatter", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("SortiePresentation", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("CampaignDefinition", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("TSoftObjectPtr<USkyguardMissionDefinition>", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("Readiness;", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("bAutoInitialize", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("PathfinderSpawnLocation", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetAircraft", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("BindRuntimeActors", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("HandleDroneCityImpact", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("ConfigureMissionDefinition", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("InitializePlayableMission", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("SynchronizeRuntimeState", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("NotifyObjectiveProgress", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("IsCorePlayableReady", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetObjectiveRuntime", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetGunner", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetPathfinder", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetReadiness", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetMissionId", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("ValidateDefinition", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("ConfigureFromMission", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("AdvanceBriefing", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("SetAssetsReady", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("AcknowledgeAndLaunch", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("CanLaunch", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetElapsedSeconds", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetBriefingState", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetBriefingText", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetRadioChatter", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetPresentation", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetMissionTitle", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("AcknowledgeBriefing", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("LaunchSortie", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("HullCollider", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("OpticalTracker", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("WeaponServo", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("CountermeasurePod", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("MinHeightFromOriginCm", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("RadarNode", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("ESkyguardMission02WaveState", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("ESkyguardMission03WaveState", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("NotifyThreatDestroyed", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetWaveState", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("StartNextWave", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetCurrentWaveIndex", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetRemainingThreatsInWave", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetConvoyRouteAlpha", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetConvoyWorldLocation", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetConvoyIntegrity", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("GetDayBeatKit", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("AdvanceConvoyByDistance", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("NotifyConvoyDamage", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("ValidateMissionContract", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("HarborIndustrial", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("MaxIntegrity", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("CurrentIntegrity", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("FillAndFinalize", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("FillAndFail", GET_CONVOY_ROUTE_STATE)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_CONVOY_ROUTE_STATE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_indent = (
            "public:\n"
            "\tESkyguardConvoyRouteState GetConvoyRouteState(\n"
            "\t) const;\n"
            "};\n"
        )
        wrap_origin = (
        ...
        wrap_ufunction = (
            "public:\n"
            f"\t{UFUNCTION_CONVOY}\n"
            f"\t{GET_CONVOY_ROUTE_STATE}\n"
            "};\n"
        )
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_CONVOY} {GET_CONVOY_ROUTE_STATE}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_CONVOY_ROUTE_STATE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_CONVOY_ROUTE_STATE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_CONVOY_ROUTE_STATE),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_CONVOY_ROUTE_STATE)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_lock_get_current_wave_index_cpp_body(self) -> None:
        ...
        self.assertNotIn("return false", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("return true", GET_CONVOY_ROUTE_STATE)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_CONVOY_ROUTE_STATE}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONVOY_ROUTE_STATE)
        self.assertIn("GetConvoyRouteState", GET_CONVOY_ROUTE_STATE)
        self.assertTrue(GET_CONVOY_ROUTE_STATE.startswith("ESkyguardConvoyRouteState"))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_CONVOY_ROUTE_STATE)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_CONVOY_ROUTE_STATE)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("CurrentIntegrity", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_CONVOY_ROUTE_STATE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_CONVOY_ROUTE_STATE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_CONVOY_ROUTE_STATE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_CONVOY_ROUTE_STATE),
            GET_CONVOY_ROUTE_STATE,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CONVOY_ROUTE_STATE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission03 GetConvoyRouteState method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_CONVOY_ROUTE_STATE}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_CONVOY_ROUTE_STATE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_CONVOY_ROUTE_STATE.lower(),
                "mission03 GetConvoyRouteState contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_CONVOY_ROUTE_STATE)

    def test_contract_is_get_convoy_route_state_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("TArray<FText>& OutErrors", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("static ", GET_CONVOY_ROUTE_STATE)
        self.assertTrue(
            GET_CONVOY_ROUTE_STATE.endswith(" const"),
            GET_CONVOY_ROUTE_STATE,
        )
        self.assertFalse(GET_CONVOY_ROUTE_STATE.endswith(";"))
        self.assertNotIn("EnvironmentReadiness", GET_CONVOY_ROUTE_STATE)
        self.assertNotIn("MissionMapReadiness", GET_CONVOY_ROUTE_STATE)
        self.assertIn(UFUNCTION_CONVOY, section)

    def test_sibling_director_fields_do_not_satisfy_get_convoy_route_state(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_CONVOY_ROUTE_STATE)
            )
            self.assertNotEqual(GET_CONVOY_ROUTE_STATE, leftover)
        self.assertIn(
    ...
    def test_leftover_mission_definition_validate_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn(MISSION_DEFINITION_VALIDATE, GET_CONVOY_ROUTE_STATE)
        self.assertIn(
            "Scripts/tests/test_mission_definition_validate_definition"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission03_get_convoy_route_state"
            "_decl_contract.py"
        )
