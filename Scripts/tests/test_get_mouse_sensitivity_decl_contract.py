...
# values, or the MouseSensitivity config default. origin/main
# is inline; accept that body and split-line forms without
# locking a specific function body.
GETTER_DECL = "float GetMouseSensitivity() const"
GETTER_INLINE_BODY = (
    "float GetMouseSensitivity() const { return MouseSensitivity; }"
)
# Leftover #56–#64 plus GameUserSettings production files.
# This lane only adds an isolated Python GetMouseSensitivity
# declaration contract. Stay off SetMouseSensitivity (in-flight
# sibling), GetMasterVolume (in-flight sibling),
...
# leftover Apache CPG feel constants (#118), and leftover
# theater-kit #59 stay sibling-only.
LOCKED_SCRIPTS = (
...
SET_MOUSE_NOT_LOCKED = "void SetMouseSensitivity(float Value);"
...
# invent INDEX_NONE or require a specific getter body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return nullptr",
...
def has_mouse_sensitivity_declaration(region: str) -> bool:
    if has_declaration(region, GETTER_DECL):
        return True
    return has_declaration(region, GETTER_INLINE_BODY)
...
def require_declaration(region: str, declaration: str) -> str:
    if not has_mouse_sensitivity_declaration(region):
        raise AssertionError(
...
class GetMouseSensitivityDeclContractTests(unittest.TestCase):
    def test_game_user_settings_class_exists(self) -> None:
        ...
        self.assertTrue(has_mouse_sensitivity_declaration(section), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("GetMouseSensitivity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_mouse_sensitivity_declaration(section))
    ...
    def test_set_mouse_sensitivity_does_not_satisfy_getter(self) -> None:
        setter_only = "\tvoid SetMouseSensitivity(float Value);\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(setter_only, GETTER_DECL)
        self.assertIn("GetMouseSensitivity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_mouse_sensitivity_declaration(setter_only))
    ...
    def test_getter_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_mouse_sensitivity_declaration(section))
        ...
        self.assertNotIn("0.07f", GETTER_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, GETTER_DECL), 1)
        one_line = f"{{\npublic:\n\t{GETTER_DECL};\n}}\n"
        self.assertTrue(has_mouse_sensitivity_declaration(one_line))
        section = public_section(origin_main_header())
        self.assertTrue(has_mouse_sensitivity_declaration(section), section)
        self.assertEqual(
            require_declaration(section, GETTER_DECL),
    ...
    def test_declaration_accepts_origin_main_inline_body_form(self) -> None:
        inline = (
            "public:\n"
            f"\t{GETTER_INLINE_BODY}\n"
            "private:\n"
        ...
        for header in (header_inline, header_split):
            section = public_section(header)
            self.assertTrue(has_mouse_sensitivity_declaration(section), section)
            self.assertEqual(
                require_declaration(section, GETTER_DECL),
        ...
        self.assertTrue(has_declaration(GETTER_INLINE_BODY, GETTER_DECL))
        self.assertNotEqual(GETTER_DECL, GETTER_INLINE_BODY)
        self.assertNotIn("{", GETTER_DECL)
        self.assertNotIn("return MouseSensitivity", GETTER_DECL)
        self.assertTrue(
            has_mouse_sensitivity_declaration(GETTER_INLINE_BODY),
            GETTER_INLINE_BODY,
        )
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return INDEX_NONE", GETTER_DECL)
        self.assertNotIn("return 0", GETTER_DECL)
        self.assertNotIn("return -1", GETTER_DECL)
        self.assertNotIn("{", GETTER_DECL)
        self.assertNotIn("}", GETTER_DECL)
    ...
    def test_contract_does_not_lock_mouse_sensitivity_default(self) -> None:
        locked_only = f"{GETTER_DECL}\n"
        section = public_section(origin_main_header())
        for token in MOUSE_DEFAULT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GETTER_DECL)
            self.assertNotIn(token, section)
        self.assertNotIn("0.07f", GETTER_DECL)
        self.assertNotIn("0.07f", locked_only)
        self.assertNotIn("float MouseSensitivity = 0.07f;", section)

    def test_contract_does_not_require_specific_function_body(self) -> None:
        locked_only = f"{GETTER_DECL}\n"
        self.assertEqual(
        ...
        self.assertNotIn("return ", GETTER_DECL)
        self.assertNotIn("return MouseSensitivity", GETTER_DECL)
        self.assertNotIn(GETTER_INLINE_BODY, GETTER_DECL)
        bare = "float GetMouseSensitivity() const;"
        self.assertTrue(has_mouse_sensitivity_declaration(bare))
        self.assertTrue(has_mouse_sensitivity_declaration(GETTER_INLINE_BODY))

    def test_contract_does_not_relock_set_mouse_sensitivity(self) -> None:
        locked_only = f"{GETTER_DECL}\n"
        self.assertNotIn(SET_MOUSE_NOT_LOCKED, locked_only)
        self.assertNotIn(SET_MOUSE_NOT_LOCKED, GETTER_DECL)
    ...
    def test_contract_does_not_relock_get_master_volume(self) -> None:
        ...
        self.assertNotIn(GET_MASTER_VOLUME_NOT_LOCKED, GETTER_DECL)
        self.assertNotIn("GetMasterVolume", GETTER_DECL)
        self.assertNotIn("GetMasterVolume", locked_only)
        self.assertNotIn("return MasterVolume", GETTER_DECL)
    ...
    def test_contract_does_not_relock_get_skyguard_game_user_settings(
        ...
        self.assertNotIn(GETTER_NOT_LOCKED, GETTER_DECL)
        self.assertNotIn("GetSkyguardGameUserSettings", GETTER_DECL)
    ...
    def test_contract_does_not_relock_apply_and_save_settings(self) -> None:
        ...
        self.assertNotIn(APPLY_AND_SAVE_NOT_LOCKED, GETTER_DECL)
        self.assertNotIn("ApplyAndSaveSettings", GETTER_DECL)
        self.assertNotIn("ApplyAndSaveSettings", locked_only)
        self.assertNotIn("bCheckForCommandLineOverrides", GETTER_DECL)

    def test_contract_does_not_relock_set_master_volume(self) -> None:
        locked_only = f"{GETTER_DECL}\n"
        self.assertNotIn(SET_MASTER_VOLUME_NOT_LOCKED, locked_only)
        self.assertNotIn(SET_MASTER_VOLUME_NOT_LOCKED, GETTER_DECL)
    ...
    def test_contract_does_not_relock_leftover_lanes(self) -> None:
        ...
        for token in LEFTOVER_LANES_NOT_LOCKED:
            ...
            self.assertNotIn(token, GETTER_DECL)
            self.assertNotIn(token, section)
        self.assertNotIn("ApplyHydraForClusters", GETTER_DECL)
        self.assertNotIn("ASkyguardGunner*", GETTER_DECL)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardGameUserSettings.cpp", section)
        self.assertNotIn(
            "USkyguardGameUserSettings::GetMouseSensitivity",
            section,
        )
        self.assertNotIn("return GEngine", section)
        self.assertNotIn("return nullptr", section)
    ...
    def test_contract_does_not_require_live_copy_leftovers(self) -> None:
        ...
        self.assertNotIn("FireIgla", GETTER_DECL)
        self.assertNotIn("FireRifle", GETTER_DECL)
        self.assertNotIn("bYakRuntimeReady", GETTER_DECL)
        self.assertNotIn("FSkyguardMission0NIntegrationReadiness", GETTER_DECL)

    def test_declaration_bans_live_copy_tokens(self) -> None:
        section = public_section(origin_main_header())
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"GetMouseSensitivity public section contains {banned}; "
    ...
    def test_contract_is_mouse_sensitivity_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
