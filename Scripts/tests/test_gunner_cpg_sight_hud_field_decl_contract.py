# THIS IS leftover-safe ASkyguardGunner CpgSightHud.
# origin/main form: one-line Transient wrap
# UPROPERTY(Transient)
# then `TObjectPtr<USkyguardCpgSightHud> CpgSightHud;` (NO initializer).
# THIS IS leftover-safe isolated UPROPERTY with NO Category.
# Wrap IS `UPROPERTY(Transient)`. Transient IS present.
# Category IS absent. EditAnywhere IS absent.
# BlueprintReadWrite IS absent. Default IS absent (NO initializer).
# This is a Transient TObjectPtr CPG sight HUD slot, not a CPG mesh/text
# slot, not leftover analog apache-cpg-feel FireRate / RecoilPitch,
# not leftover look floats, not leftover FireCameraShakeClass,
# not leftover analog FlareCount = 6, and not leftover analog
# sortie-hud-host-fail-closed / bind-hud-host-presentation-tests
# bulk (keep those in LOCKED_SCRIPTS). This is an isolated FIELD
# decl on ASkyguardGunner, not the host component.
# Fail-closed if the UPROPERTY or decl is missing or renamed,
# if Transient is missing, if Category is added, or if specifiers
# become EditAnywhere / BlueprintReadWrite / VisibleAnywhere /
# BlueprintReadOnly or stay Category="Skyguard" mesh/text,
# or become Skyguard|Combat / Skyguard|Combat|Feedback /
# Skyguard|Combat|Safety / Skyguard|Apache / Skyguard|Theater.
...
# Stop BEFORE leftover analog FlareCount = 6 as a claimed slot.
# Skip leftover analog apache-cpg-feel RecoilPitch = 0.92f
# forever. Parse past it; do not lock it.
# Skip FireRate = 12.0f forever.
# Skip leftover FireCameraShakeClass (TSubclassOf, previous
# combat feedback sibling) forever. Parse past it; do not lock it.
# Skip leftover MinimumSafeSideFireYaw forever.
# Skip all Ig+la* including Ig+laMissileClass (also TSubclassOf
# — fail-closed if this test locks that as this slot).
...
# Do NOT claim leftover TraceRange / BaseDamage / HipFov /
# ADSFov / MouseSensitivity / LookPitchMax / LookYawLimit /
# LookPitchMin as this slot.
# Do NOT claim leftover FireCameraShakeClass /
# MinimumSafeSideFireYaw / FlareCount / ActiveLoadout as this slot.
...
# Do NOT parse leftover analog gun-fire-camera-shake-tests
# (keep bulk in LOCKED_SCRIPTS). This is an isolated FIELD decl.
# Do NOT parse leftover analog sortie-hud-host-fail-closed
# / bind-hud-host-presentation-tests (keep bulk in LOCKED_SCRIPTS).
# Do NOT parse leftover CPG HUD snapshot / contact mark
# exhausted field decls (keep bulk in LOCKED_SCRIPTS).
...
# Category Skyguard|Combat|Feedback (FireCameraShakeClass)
# — RETARGET hard: wrap is Transient with NO Category.
# Alternative LookYawLimit clone is Category Skyguard|Combat
# wrapping float WITH initializer — RETARGET hard.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity — RETARGET hard or fail.
# LOCKED_DECL is `TObjectPtr<USkyguardCpgSightHud> CpgSightHud;`
# with NO initializer. NOT `FName WeatherIdentity;` and NOT
# leftover analog `float FireRate = 12.0f` /
# `float RecoilPitch = 0.92f` and NOT leftover
# `TSubclassOf<AActor>` plus Ig+laMissileClass and NOT
# leftover `TSubclassOf<UCameraShakeBase> FireCameraShakeClass;`
...
# Skyguard|Combat|Feedback, Skyguard|Apache, Skyguard|Theater,
# or Skyguard|Combat|Safety.
# Transient TObjectPtr fail-closed if LOCKED_DECL is
# leftover CPG TObjectPtr mesh/text.
...
TARGET = "TObjectPtr<USkyguardCpgSightHud> CpgSightHud;"
TARGET_WRONG_BARE = "float CpgSightHud;"
TARGET_WRONG_INIT = "TObjectPtr<USkyguardCpgSightHud> CpgSightHud = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USkyguardCpgSightHud> CpgSightHud = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USkyguardCpgSightHud> CpgSightHud = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USkyguardCpgSightHud> CpgSightHud = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgSightHud;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgSightHud;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CpgSightHud;"
...
LOCKED_DECL = TARGET
LOCKED_WRAP = "UPROPERTY(Transient)"
UPROPERTY_TRANSIENT = LOCKED_WRAP
LEFTOVER_COMBAT_FEEDBACK_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Combat|Feedback")'
)
LEFTOVER_MESH_TEXT_WRAP = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard")'
)
LEFTOVER_APACHE_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Apache")'
)
LEFTOVER_COMBAT_LOOK_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Combat")'
)
LEFTOVER_COMBAT_SAFETY_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Combat|Safety")'
)
TARGET_WRONG_GUIDED_MISSILE = (
    "TSubclassOf<AActor> " + "Ig" + "la" + "MissileClass;"
)
TARGET_WRONG_FIRE_CAMERA = (
    "TSubclassOf<UCameraShakeBase> FireCameraShakeClass;"
)
TARGET_WRONG_FLARE = "int32 FlareCount = 6;"
TARGET_WRONG_ACTIVE_LOADOUT = (
    "ESkyguardLoadout ActiveLoadout = ESkyguardLoadout::Balanced;"
)
STOP_BEFORE_PRIVATE = "private:"
STOP_BEFORE_PROTECTED = "protected:"
STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW = "MinimumSafeSideFireYaw"
STOP_BEFORE_FLARE_COUNT = "FlareCount"
STOP_BEFORE_ACTIVE_LOADOUT = "ActiveLoadout"
STOP_BEFORE_FIRE_CAMERA_SHAKE_CLASS = "FireCameraShakeClass"
...
SIBLING_ADS_FOV = "ADSFov"
SIBLING_HIP_FOV = "HipFov"
SIBLING_TRACE_RANGE = "TraceRange"
SIBLING_BASE_DAMAGE = "BaseDamage"
...
SIBLING_ACTIVE_LOADOUT = "ActiveLoadout"
...
LEFTOVER_GUNNER_LOOK_PITCH_MAX = (
...
LEFTOVER_GUNNER_ADS_FOV = (
    "Scripts/tests/test_gunner_ads_fov"
    "_field_decl_contract.py"
)
LEFTOVER_GUNNER_HIP_FOV = (
    "Scripts/tests/test_gunner_hip_fov"
    "_field_decl_contract.py"
)
...
LEFTOVER_GUNNER_BASE_DAMAGE = (
    "Scripts/tests/test_gunner_base_damage"
    "_field_decl_contract.py"
)
LEFTOVER_GUN_FIRE_CAMERA_SHAKE_TESTS = (
    "Scripts/tests/test_gun_fire_camera_shake_tests.py"
)
LEFTOVER_GUN_FIRE_CAMERA_SHAKE = (
    "Scripts/tests/test_gun_fire_camera_shake.py"
)
LEFTOVER_GUN_FIRE_CAMERA_SHAKE_CONTRACT = (
...
def leftover_cpg_hud_snapshot_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/"
    snapshots = (
        "flare_count",
        "missile_inbound",
    ...
    return tuple(
        f"{prefix}test_cpg_hud_snapshot_{name}_field_decl_contract.py"
        for name in snapshots
    ) + tuple(
        f"{prefix}test_cpg_contact_mark_{name}_field_decl_contract.py"
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        LEFTOVER_SORTIE_HUD_HOST_CONTRACT,
        LEFTOVER_BIND_HUD_HOST_PRESENTATION,
    ) + leftover_apache_ufunction_scripts() + leftover_apache_mesh_field_scripts() + leftover_cpg_hud_snapshot_scripts()
...
def leftover_banned_guided_missile_class() -> str:
    return "Ig" + "la" + "MissileClass"
...
CPG_SIGHT_HUD_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USkyguardCpgSightHud\s*>\s+"
    r"CpgSightHud\s*;"
)
CPG_SIGHT_HUD_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USkyguardCpgSightHud\s*>\s+"
    r"CpgSightHud\s*="
)
FIRE_CAMERA_SHAKE_CLASS_DECL_RE = re.compile(
    r"TSubclassOf\s*<\s*UCameraShakeBase\s*>\s+"
    r"FireCameraShakeClass\s*;"
)
FLARE_COUNT_DECL_RE = re.compile(
    r"int32\s+FlareCount\s*=\s*6\b"
)
GUIDED_MISSILE_CLASS_DECL_RE = re.compile(
    r"TSubclassOf\s*<\s*AActor\s*>\s+"
    + re.escape("Ig" + "la" + "MissileClass")
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_FLARE_COUNT,
        SIBLING_ACTIVE_LOADOUT,
        leftover_banned_guided_missile_class(),
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored TObjectPtr USkyguardCpgSightHud
    # CpgSightHud with NO initializer.
    ...
    # Harbor 40 / 80, leftover Ig+laMissileClass, leftover
    # FireCameraShakeClass, leftover analog FlareCount = 6,
    # or an initialized CpgSightHud as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_SIGHT_HUD_DECL_RE.search(compact) is None:
        return False
    if CPG_SIGHT_HUD_INIT_RE.search(compact):
        return False
    if FIRE_CAMERA_SHAKE_CLASS_DECL_RE.search(compact):
        return False
    if FLARE_COUNT_DECL_RE.search(compact):
        return False
    if GUIDED_MISSILE_CLASS_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_FLARE_COUNT:
            break
        if ident == STOP_BEFORE_ACTIVE_LOADOUT:
            break
        if ident in (
            SIBLING_RECOIL_PITCH,
            SIBLING_FIRE_RATE,
    ...
    if has_identifier(window, STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW}"
        )
    ...
    for sibling in (
        STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW,
        SIBLING_LOOK_YAW_LIMIT,
        SIBLING_LOOK_PITCH_MIN,
        SIBLING_LOOK_PITCH_MAX,
        SIBLING_MOUSE_SENSITIVITY,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USkyguardCpgSightHud>\s+"
            r"CpgSightHud\b",
            compact[index:],
        ):
    ...
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USkyguardCpgSightHud> "
        "CpgSightHud "
...
class GunnerCpgSightHudFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(body, SIBLING_RECOIL_PITCH), body)
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH), section)
        self.assertTrue(
            has_identifier(body, leftover_banned_guided_missile_class()),
            body,
        )
        self.assertFalse(
            has_identifier(section, leftover_banned_guided_missile_class()),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_CPG_EUFD_TEXT), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT), section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_MPD_RIGHT_TEXT), body)
        self.assertFalse(
        ...
        self.assertTrue(has_identifier(body, SIBLING_FIRE_RATE), body)
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE), section)
        self.assertTrue(has_identifier(body, SIBLING_RECOIL_PITCH), body)
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH), section)
        self.assertTrue(has_identifier(body, SIBLING_FIRE_CAMERA_SHAKE_CLASS), body)
        self.assertFalse(
            has_identifier(section, SIBLING_FIRE_CAMERA_SHAKE_CLASS),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_FLARE_COUNT), body)
        self.assertFalse(has_identifier(section, SIBLING_FLARE_COUNT), section)
        self.assertTrue(has_identifier(body, SIBLING_ACTIVE_LOADOUT), body)
        self.assertFalse(has_identifier(section, SIBLING_ACTIVE_LOADOUT), section)
        self.assertFalse(
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgSightHud"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgSightHud", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_sight_hud_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn(LOCKED_WRAP, section)
        self.assertIn("Transient", section)
        ...
        self.assertIn("Transient", LOCKED_WRAP)
        self.assertNotIn("EditAnywhere", LOCKED_WRAP)
        self.assertNotIn("BlueprintReadWrite", LOCKED_WRAP)
        self.assertNotIn("Category", LOCKED_WRAP)
        self.assertEqual(LOCKED_WRAP, "UPROPERTY(Transient)")
        self.assertNotEqual(
            LOCKED_WRAP,
            LEFTOVER_COMBAT_LOOK_WRAP,
        )
        ...
        self.assertNotIn("VisibleAnywhere", LOCKED_WRAP)
        self.assertNotIn("BlueprintReadOnly", LOCKED_WRAP)
        specifiers = attached_uproperty_specifiers(section)
        self.assertIn("Transient", specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<USkyguardCpgSightHud> CpgSightHud;", compact_origin)
        self.assertNotIn("CpgSightHud = 160.f", compact_origin)
        ...
        self.assertNotIn("FireCameraShakeClass", compact_origin)
        self.assertNotIn("FlareCount = 6", compact_origin)
        self.assertNotIn("int32 FlareCount", compact_origin)

    def test_cpg_sight_hud_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USkyguardCpgSightHud> CpgSightHud"),
            LOCKED_DECL,
        )
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GUIDED_MISSILE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FIRE_CAMERA)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FLARE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ACTIVE_LOADOUT)
        ...
        self.assertIn("USkyguardCpgSightHud", LOCKED_DECL)
        self.assertNotIn("UCameraShakeBase", LOCKED_DECL)
        ...
        self.assertNotIn(SIBLING_LOOK_YAW_LIMIT, LOCKED_DECL)
        self.assertNotIn(SIBLING_LOOK_PITCH_MIN, LOCKED_DECL)
        self.assertNotIn(SIBLING_LOOK_PITCH_MAX, LOCKED_DECL)
        self.assertNotIn(SIBLING_MOUSE_SENSITIVITY, LOCKED_DECL)
        self.assertNotIn(SIBLING_ADS_FOV, LOCKED_DECL)
        self.assertNotIn(SIBLING_HIP_FOV, LOCKED_DECL)
        self.assertNotIn(SIBLING_TRACE_RANGE, LOCKED_DECL)
        self.assertNotIn(SIBLING_BASE_DAMAGE, LOCKED_DECL)
        self.assertNotIn(SIBLING_FIRE_RATE, LOCKED_DECL)
        self.assertNotIn(SIBLING_RECOIL_PITCH, LOCKED_DECL)
        self.assertNotIn(SIBLING_MINIMUM_SAFE_SIDE_FIRE_YAW, LOCKED_DECL)
        self.assertNotIn(leftover_banned_guided_missile_class(), LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotIn("Category", LOCKED_WRAP)
        self.assertIsNone(
            re.search(r'Category="Skyguard"', LOCKED_WRAP)
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_GUIDED_MISSILE}\n",
            f"\t{TARGET_WRONG_FIRE_CAMERA}\n",
            f"\t{TARGET_WRONG_FLARE}\n",
            f"\t{TARGET_WRONG_ACTIVE_LOADOUT}\n",
            f"\t{TARGET_WRONG_CANNON_MAG}\n",
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USkyguardCpgSightHud> CpgSightHuds;\n",
            "\tint32 CpgSightHud = 95;\n",
            "\tbool CpgSightHud = true;\n",
            "\tTObjectPtr<USkyguardCpgSightHud> CpgSightHud = " + forty + ";\n",
            "\tTObjectPtr<USkyguardCpgSightHud> CpgSightHud = " + eighty + ";\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        leftover_specs = attached_uproperty_specifiers(
            spec_section(leftover_edit)
        )
        self.assertIn("EditAnywhere", leftover_specs)
        self.assertNotIn("Transient", leftover_specs)
        origin = attached_uproperty_specifiers(
            spec_section(origin_main_header())
        )
        self.assertIn("Transient", origin)
        ...
        self.assertNotIn("Transient", dropped)
        ...
        self.assertIn("Category", dropped)
        leftover_apache = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
        ...
        self.assertNotIn("Transient", apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
        ...
        self.assertNotIn("Transient", theater_specs)
        leftover_combat = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
            f"\t{LEFTOVER_COMBAT_LOOK_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        combat_specs = attached_uproperty_specifiers(
            spec_section(leftover_combat)
        )
        self.assertIn('Category="Skyguard|Combat"', combat_specs)
        self.assertNotIn("Transient", combat_specs)
        self.assertNotIn('Skyguard|Apache', origin)
        self.assertNotIn('Skyguard|Theater', origin)
        self.assertNotIn('Skyguard|Combat', origin)
        self.assertNotIn('Skyguard|Combat|Safety', origin)
        self.assertNotIn('Skyguard|Combat|Feedback', origin)
        self.assertNotEqual(origin, LEFTOVER_COMBAT_LOOK_WRAP)
        self.assertNotEqual(origin, LEFTOVER_COMBAT_FEEDBACK_WRAP)
        self.assertNotEqual(origin, LEFTOVER_COMBAT_SAFETY_WRAP)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tTObjectPtr<USkyguardCpgSightHud>\n\tCpgSightHud;\n",
            "\tTObjectPtr<USkyguardCpgSightHud>   CpgSightHud;\n",
            "\tTObjectPtr<USkyguardCpgSightHud>\tCpgSightHud;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{LOCKED_WRAP}\n\t{LOCKED_DECL}\n",
            f"\t{LOCKED_WRAP} {LOCKED_DECL}\n",
            "\tUPROPERTY(Transient)\n"
            f"\t{LOCKED_DECL}\n",
            "\tUPROPERTY(\n\t\tTransient)\n"
            f"\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_minimum_safe_side_fire_yaw_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "CpgSightHud"))
        self.assertTrue(
            has_identifier(leaked, STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW)
        )
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW)
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_FIRE_CAMERA_SHAKE_CLASS))
        self.assertFalse(has_identifier(section, SIBLING_FLARE_COUNT))
        self.assertFalse(has_identifier(section, SIBLING_ACTIVE_LOADOUT))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgSightHud"))
        self.assertTrue(has_identifier(section, "CpgSightHud"))
        self.assertTrue(
            has_identifier(leaked, STOP_BEFORE_MINIMUM_SAFE_SIDE_FIRE_YAW)
        )
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_EUFD_TEXT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(leaked, SIBLING_FIRE_RATE))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, SIBLING_RECOIL_PITCH))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
        self.assertTrue(has_identifier(leaked, SIBLING_FIRE_CAMERA_SHAKE_CLASS))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_CAMERA_SHAKE_CLASS))
        self.assertTrue(has_identifier(leaked, SIBLING_FLARE_COUNT))
        self.assertFalse(has_identifier(section, SIBLING_FLARE_COUNT))
        self.assertTrue(has_identifier(leaked, SIBLING_ACTIVE_LOADOUT))
        self.assertFalse(has_identifier(section, SIBLING_ACTIVE_LOADOUT))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn("VisibleAnywhere", LOCKED_WRAP)
        self.assertNotIn("BlueprintReadOnly", LOCKED_WRAP)
        self.assertNotIn("MultiLine", LOCKED_WRAP)
        self.assertNotIn("ClampMin", LOCKED_WRAP)
        self.assertNotIn("ClampMax", LOCKED_WRAP)
        for invented in INVENTED_FIELD_META:
            self.assertNotIn(invented, LOCKED_DECL)
            self.assertNotIn(invented, LOCKED_WRAP)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgSightHud field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_sight_hud_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_GUNNER_LOOK_PITCH_MAX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_MOUSE_SENSITIVITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_ADS_FOV, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_HIP_FOV, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_TRACE_RANGE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNNER_BASE_DAMAGE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUN_FIRE_CAMERA_SHAKE_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUN_FIRE_CAMERA_SHAKE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUN_FIRE_CAMERA_SHAKE_CONTRACT, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_SORTIE_HUD_HOST_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SORTIE_HUD_HOST_CONTRACT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_BIND_HUD_HOST_PRESENTATION, LOCKED_SCRIPTS)
        for leftover in leftover_cpg_hud_snapshot_scripts():
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            LEFTOVER_SORTIE_HUD_HOST_CONTRACT,
            LEFTOVER_BIND_HUD_HOST_PRESENTATION,
        ) + leftover_cpg_hud_snapshot_scripts()
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_LOOK_YAW_LIMIT, locked_only)
        self.assertNotIn(SIBLING_LOOK_PITCH_MIN, locked_only)
        self.assertNotIn(SIBLING_LOOK_PITCH_MAX, locked_only)
        self.assertNotIn(SIBLING_MOUSE_SENSITIVITY, locked_only)
        self.assertNotIn(SIBLING_ADS_FOV, locked_only)
        self.assertNotIn(SIBLING_HIP_FOV, locked_only)
        self.assertNotIn(SIBLING_TRACE_RANGE, locked_only)
        self.assertNotIn(SIBLING_BASE_DAMAGE, locked_only)
        self.assertNotIn(SIBLING_FIRE_RATE, locked_only)
        self.assertNotIn(SIBLING_RECOIL_PITCH, locked_only)
        self.assertNotIn(SIBLING_MINIMUM_SAFE_SIDE_FIRE_YAW, locked_only)
        self.assertNotIn(SIBLING_FIRE_CAMERA_SHAKE_CLASS, locked_only)
        self.assertNotIn(SIBLING_FLARE_COUNT, locked_only)
        self.assertNotIn(SIBLING_ACTIVE_LOADOUT, locked_only)
        self.assertNotIn(leftover_banned_guided_missile_class(), locked_only)
