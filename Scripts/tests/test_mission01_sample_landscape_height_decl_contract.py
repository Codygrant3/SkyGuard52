...
# INDEX_NONE or lock SampleLandscapeHeight
# construction in the .cpp. This is a leftover-safe
# Grounding library METHOD contract on
# USkyguardMission01LandscapeGroundingLibrary. It is NOT
# leftover landscape-height-sample-defaults #46a3
# (do not lock the struct defaults), NOT leftover
# environment-readiness defaults #6b9d / #b931, NOT
# leftover Environment GetReadiness, NOT leftover
# IsRouteExclusionSafe, and NOT leftover
# bUseAuthoredLandscapeSurface. Distinct from
...
# bUseAuthoredLandscapeSurface, and NOT leftover
# environment-readiness defaults #6b9d / #b931 or
# leftover Environment GetReadiness. Distinct from
# leftover Integration methods
...
# Do not lock sibling Grounding method
# SampleLandscapeFootprint. Do not lock leftover
# landscape-height-sample-defaults #46a3 or
# leftover landscape-footprint-sample-defaults
# #af83. Do not lock sibling Environment methods
# RebuildProductionLayout / HasContinuousCoastline /
...
# const FVector& WorldLocation);`); accept that
...
# not Environment|PCG, not Environment|Landscape, not
# Environment|Layout, not Environment|Materials,
...
# Mission01LandscapeGroundingLibrary public section and
...
# leftover theater-kit #59, leftover #107–#618, plus
# SkyguardMission01LandscapeGroundingLibrary production
# files. This lane only adds an isolated Python
# SampleLandscapeHeight method declaration
...
SIBLING_GROUNDING_METHODS_NOT_LOCKED = (
    SAMPLE_LANDSCAPE_FOOTPRINT,
    "SampleLandscapeFootprint",
    "test_mission01_sample_landscape_footprint_decl_contract.py",
)
...
class Mission01SampleLandscapeHeightDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SAMPLE_LANDSCAPE_HEIGHT),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("SampleLandscapeHeight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_missing_sample_landscape_height_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMission01LandscapeGroundingLibrary();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("SampleLandscapeHeight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("SampleLandscapeHeight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("BlueprintCallable", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("BlueprintPure", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("Category", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("BlueprintCallable", UFUNCTION_GROUNDING)
        self.assertIn("Skyguard|Mission01|Environment", UFUNCTION_GROUNDING)
        self.assertIn("BlueprintPure", UFUNCTION_GROUNDING)
        self.assertIn("|Grounding", UFUNCTION_GROUNDING)
        self.assertIn("Environment|Grounding", UFUNCTION_GROUNDING)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("SampleLandscapeHeight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid SampleLandscapeHeight(const FVector& WorldPoint) const;\n"
        )
        missing_const = (
            "\tbool SampleLandscapeHeight(const FVector& WorldPoint);\n"
        )
        extra_arg = (
            "\tbool SampleLandscapeHeight("
            "const FVector& WorldPoint, int32 Amount) const;\n"
        )
        extra_args = (
            "\tbool SampleLandscapeHeight("
        ...
        as_int = (
            "\tint32 SampleLandscapeHeight(const FVector& WorldPoint) const;\n"
        )
        renamed = "\tbool IsPointAllowed() const;\n"
        short_name = "\tbool IsAllowedForPCG(const FVector& WorldPoint) const;\n"
        getter = "\tbool GetPointAllowedForPCG() const;\n"
        missing_arg = "\tbool SampleLandscapeHeight() const;\n"
        ...
        leftover_rebuild = f"\t{REBUILD_PRODUCTION_LAYOUT}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SAMPLE_LANDSCAPE_HEIGHT)
            self.assertIn("SampleLandscapeHeight", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_sample_landscape_height_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SAMPLE_LANDSCAPE_HEIGHT),
            SAMPLE_LANDSCAPE_HEIGHT,
        )
        self.assertTrue(has_declaration(section, SAMPLE_LANDSCAPE_HEIGHT))
        self.assertEqual(
            declaration_count(section, SAMPLE_LANDSCAPE_HEIGHT),
            1,
        )
        self.assertTrue(
            SAMPLE_LANDSCAPE_HEIGHT.startswith("static FSkyguardLandscapeHeightSample "),
            SAMPLE_LANDSCAPE_HEIGHT,
        )
        self.assertTrue(
            SAMPLE_LANDSCAPE_HEIGHT.endswith(";"),
            SAMPLE_LANDSCAPE_HEIGHT,
        )
        ...
        self.assertNotIn("TArray<", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("TObjectPtr", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("TSoftObjectPtr", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("INDEX_NONE", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("UFUNCTION", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("{", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("}", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("return ", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn(" const;", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("void ", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("Root", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("Briefing", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("AudioDirector", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("RadioChatter", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("SortiePresentation", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("CampaignDefinition", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("MissionDefinition", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("Readiness", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("bAutoInitialize", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("bAllowBoundedActorSpawning", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("bAutoLaunchAfterBriefing", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("PathfinderSpawnLocation", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetAircraft", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("BindRuntimeActors", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("HandleDroneCityImpact", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("ConfigureMissionDefinition", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("InitializePlayableMission", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("NotifyProtectedAssetFailed", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("SynchronizeRuntimeState", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("NotifyObjectiveProgress", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetReadiness", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetObjectiveRuntime", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetGunner", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetPathfinder", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetMissionId", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("ValidateMissionContract", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("ConfigureFromMission", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("AdvanceBriefing", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("SetAssetsReady", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("AcknowledgeAndLaunch", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("CanLaunch", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetElapsedSeconds", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetBriefingState", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetMinimumWarmupSeconds", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetBriefingText", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetRadioChatter", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetPresentation", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("GetMissionTitle", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("AcknowledgeBriefing", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("LaunchSortie", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("HullCollider", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("OpticalTracker", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("WeaponServo", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("CountermeasurePod", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("MinHeightFromOriginCm", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("RadarNode", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("ESkyguardMission02WaveState", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("HarborIndustrial", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("MaxIntegrity", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("CurrentIntegrity", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("FillAndFinalize", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("FillAndFail", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("IsRouteExclusionSafe", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("bRouteExclusionValid", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            SAMPLE_LANDSCAPE_HEIGHT,
        )
        self.assertNotIn("bAllowAuthoredPCGGeneration", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("bProductionLandscapeBound", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("bUseAuthoredLandscapeSurface", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("RebuildProductionLayout", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("HasContinuousCoastline", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("SampleLandscapeFootprint", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("IsPointAllowedForPCG", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("bValid = false", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("static ", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("ALandscapeProxy*", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("WorldLocation", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn(
            "RefreshAuthoredEnvironmentBindings",
            SAMPLE_LANDSCAPE_HEIGHT,
        )
        self.assertNotIn("IsAuthoredEnvironmentReady", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("IsPCGGenerationAuthorized", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("IsCorePlayableReady", SAMPLE_LANDSCAPE_HEIGHT)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_spaces = (
            "public:\n"
            "\tstatic   FSkyguardLandscapeHeightSample   SampleLandscapeHeight(ALandscapeProxy* Landscape, const FVector& WorldLocation);\n"
            "private:\n"
            "};\n"
        )
        wrap_tab = (
            "public:\n"
            "\tstatic FSkyguardLandscapeHeightSample\t"
            "SampleLandscapeHeight(ALandscapeProxy* Landscape, const FVector& WorldLocation);\n"
            "};\n"
        )
        wrap_indent = (
            "public:\n"
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_GROUNDING} {SAMPLE_LANDSCAPE_HEIGHT}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{SAMPLE_LANDSCAPE_HEIGHT}\n}}\n"
        self.assertTrue(has_declaration(one_line, SAMPLE_LANDSCAPE_HEIGHT))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SAMPLE_LANDSCAPE_HEIGHT),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_HEIGHT}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", SAMPLE_LANDSCAPE_HEIGHT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_sample_landscape_height_cpp_body(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_HEIGHT}\n"
        self.assertNotIn("{", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("}", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("return ", SAMPLE_LANDSCAPE_HEIGHT)
        ...
        self.assertNotIn("return false", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("return true", SAMPLE_LANDSCAPE_HEIGHT)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_HEIGHT}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("SampleLandscapeHeight", SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_contract_does_not_relock_sibling_environment_methods(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_METHODS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_HEIGHT)
        for token in SIBLING_GROUNDING_METHODS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("CurrentIntegrity", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("SkyguardApacheAircraft.h", SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_HEIGHT}\n"
        self.assertEqual(
            require_declaration(locked_only, SAMPLE_LANDSCAPE_HEIGHT),
            SAMPLE_LANDSCAPE_HEIGHT,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 SampleLandscapeHeight method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_HEIGHT}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SAMPLE_LANDSCAPE_HEIGHT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SAMPLE_LANDSCAPE_HEIGHT.lower(),
                "mission01 SampleLandscapeHeight contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SAMPLE_LANDSCAPE_HEIGHT)

    def test_contract_is_sample_landscape_height_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn(" const;", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertTrue(SAMPLE_LANDSCAPE_HEIGHT.endswith(";"))
        self.assertIn(UFUNCTION_GROUNDING, section)

    def test_sibling_director_fields_do_not_satisfy_sample_landscape_height(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, SAMPLE_LANDSCAPE_HEIGHT)
            )
            self.assertNotEqual(SAMPLE_LANDSCAPE_HEIGHT, leftover)
        self.assertIn(
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("return ", SAMPLE_LANDSCAPE_HEIGHT)
        self.assertNotIn("return Other", SAMPLE_LANDSCAPE_HEIGHT)
    ...
    def test_declaration_accepts_split_parameter_wrap(self) -> None:
        ...
        wrap_type_and_const = (
            "public:\n"
            "\tstatic\n"
            "\tFSkyguardLandscapeHeightSample SampleLandscapeHeight(\n"
            "\t\tALandscapeProxy* Landscape,\n"
            "\t\tconst FVector& WorldLocation);\n"
            "};\n"
        )
    ...
    def test_leftover_height_sample_defaults_do_not_satisfy(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_HEIGHT}\n"
        for token in LEFTOVER_HEIGHT_SAMPLE_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_HEIGHT)
        region = "\tbool bValid = false;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("SampleLandscapeHeight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_sibling_grounding_footprint_does_not_satisfy(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_HEIGHT}\n"
        for token in SIBLING_GROUNDING_METHODS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_HEIGHT)
        region = f"\t{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, SAMPLE_LANDSCAPE_HEIGHT)
        self.assertIn("SampleLandscapeHeight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn(
            "Scripts/tests/test_landscape_height_sample_defaults"
            "_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
