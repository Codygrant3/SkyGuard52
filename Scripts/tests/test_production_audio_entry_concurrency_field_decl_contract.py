# THIS IS leftover-safe FSkyguardProductionAudioEntry Concurrency.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundConcurrency> Concurrency;`
# (no initializer).
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Bare TSoftObjectPtr<USoundConcurrency>,
# no initializer.
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# TSoftObjectPtr<USoundConcurrency>, initializer added,
# Category= added.
...
# SourceStatus / Sound / Attenuation / OutputSubmix /
# ProvenanceId / SourceSha256.
# Analog leftover production-audio-entry-defaults #198 /
# #314b then isolated Concurrency field decl.
...
# retarget: type is TSoftObjectPtr<USoundConcurrency>,
...
TARGET = "TSoftObjectPtr<USoundConcurrency> Concurrency;"
TARGET_WRONG_EQ_NONE = (
    "TSoftObjectPtr<USoundConcurrency> Concurrency = nullptr;"
)
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundConcurrency> Concurrency = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundConcurrency> Concurrency = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundConcurrency> Concurrency = 0.f;"
TARGET_WRONG_FLOAT = "float Concurrency;"
TARGET_WRONG_FNAME = "FName Concurrency;"
TARGET_WRONG_BOOL = "bool Concurrency;"
TARGET_WRONG_INT = "int32 Concurrency;"
TARGET_WRONG_FTEXT = "FText Concurrency;"
TARGET_WRONG_SOUND = "TSoftObjectPtr<USoundBase> Concurrency;"
TARGET_WRONG_OBJECTPTR = "TObjectPtr<USoundConcurrency> Concurrency;"
TARGET_WRONG_PTR = "USoundConcurrency* Concurrency;"
TARGET_WRONG_ATTENUATION = "TSoftObjectPtr<USoundAttenuation> Concurrency;"
TARGET_WRONG_SUBMIX = "TSoftObjectPtr<USoundSubmixBase> Concurrency;"
TARGET_WRONG_FSTRING = "FString Concurrency;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if re.search(
        r"TSoftObjectPtr<USoundConcurrency>\s+Concurrency\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundConcurrency\s*\*\s*Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr<USoundBase>\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr<USoundAttenuation>\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr<USoundSubmixBase>\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr<USoundConcurrency>\s+Concurrency\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr<USoundConcurrency>\s+Concurrency\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundConcurrency> Concurrency "
        "is missing from "
...
class ProductionAudioEntryConcurrencyFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_concurrency_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("Concurrency = nullptr", compact_origin)
        self.assertNotIn("Concurrency = false", compact_origin)
        self.assertNotIn("Concurrency = true", compact_origin)
        self.assertNotIn("Concurrency = 0.f", compact_origin)
        self.assertNotIn("Concurrency = 160.f", compact_origin)

    def test_concurrency_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("TSoftObjectPtr<USoundConcurrency>", LOCKED_DECL)
        ...
        self.assertNotIn("TSoftObjectPtr<USoundSubmixBase>", LOCKED_DECL)
        self.assertNotIn("USoundConcurrency*", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_OBJECTPTR}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SUBMIX}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tTSoftObjectPtr<USoundConcurrency> "
            + leftover_retired_primary_hits_field()
            + ";\n"
        )
        leftover_guided = (
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_OBJECTPTR}\n",
            f"\t{TARGET_WRONG_PTR}\n",
            f"\t{TARGET_WRONG_ATTENUATION}\n",
            f"\t{TARGET_WRONG_SUBMIX}\n",
            f"\t{TARGET_WRONG_FSTRING}\n",
            "\tTSoftObjectPtr<USoundConcurrency> Concurrencys;\n",
            "\tint32 Concurrency = 1;\n",
            "\tbool Concurrency;\n",
            "\tfloat Concurrency = " + forty + ";\n",
            "\tfloat Concurrency = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry Concurrency field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "concurrency_field_decl_contract.py"
        ))
        self.assertIn(LEFTOVER_DISPLAY_NAME, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PROVENANCE_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SOUND, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SOURCE_SHA256, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ATTENUATION, LOCKED_SCRIPTS)
