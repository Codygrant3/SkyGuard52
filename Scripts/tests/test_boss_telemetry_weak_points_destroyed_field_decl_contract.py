# THIS IS leftover-safe FSkyguardBossTelemetry::WeakPointsDestroyed.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `int32 WeakPointsDestroyed = 0;` with no Category=.
# Initializer IS `= 0` (int32).
# If a clone asserts Category= and origin/main has
# none, drop that assertion. If a clone asserts
# `= false` / `= true` / `= 0.f` / bare decl, do not
# copy those; this field is `int32 WeakPointsDestroyed = 0;`.
# Fail-closed if the type/decl is missing or renamed, if
# Category= is added, or if the default is not `= 0`.
# Accept origin/main split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly).
# THIS IS NOT leftover BossTelemetry Ri+fleHits / Ig+laHits.
# Do NOT contract those two fields.
# Do NOT parse enum class ESkyguardBossWeapon (contains
# leftover Ri+fle / Ig+la).
# Do NOT parse ESkyguardBossPhase / ESkyguardPilotCommand
# bodies except as nested type names if needed.
# Do NOT contract PilotCommandsIssued (sibling worker).
...
# FSkyguardBossTelemetry, stop at that struct's closing };
...
HEADER_PATH = "Source/Skyguard52/SkyguardBossTypes.h"
STRUCT_NAME = "FSkyguardBossTelemetry"
CLASS_NAME = STRUCT_NAME
TARGET = "int32 WeakPointsDestroyed = 0;"
TARGET_WRONG_BARE = "int32 WeakPointsDestroyed;"
TARGET_WRONG_FALSE = "int32 WeakPointsDestroyed = false;"
TARGET_WRONG_TRUE = "int32 WeakPointsDestroyed = true;"
TARGET_WRONG_FLOAT = "int32 WeakPointsDestroyed = 0.f;"
TARGET_WRONG_PILOT = "int32 PilotCommandsIssued = 0;"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
STOP_BEFORE_WEAPON = "enum class ESkyguardBossWeapon"
STOP_BEFORE_PHASE = "enum class ESkyguardBossPhase"
STOP_BEFORE_COMMAND = "enum class ESkyguardPilotCommand"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # when origin/main has `int32 WeakPointsDestroyed = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+WeakPointsDestroyed\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+WeakPointsDestroyed\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+WeakPointsDestroyed\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+WeakPointsDestroyed\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+WeakPointsDestroyed\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 WeakPointsDestroyed is missing from "
...
class BossTelemetryWeakPointsDestroyedFieldDeclContractTests(unittest.TestCase):
    def test_boss_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("WeakPointsDestroyed", section)
        ...
        self.assertNotIn(STOP_BEFORE_WEAPON, section)
        self.assertNotIn(STOP_BEFORE_PHASE, section)
        self.assertNotIn(STOP_BEFORE_COMMAND, section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("WeakPointsDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_weak_points_destroyed_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("WeakPointsDestroyed = 0", collapsed(origin))
        self.assertNotIn("WeakPointsDestroyed = false", collapsed(origin))
        self.assertNotIn("WeakPointsDestroyed = true", collapsed(origin))
        self.assertNotIn("WeakPointsDestroyed = 0.f", collapsed(origin))

    def test_weak_points_destroyed_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 WeakPointsDestroyed"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_PILOT}\n",
            leftover_primary,
            leftover_guided,
            "\tbool WeakPointsDestroyed = 0;\n",
            "\tint32 WeakPointDestroyed = 0;\n",
            "\tint32 WeakPointsDestroyeds = 0;\n",
            "\tFText WeakPointsDestroyed = 0;\n",
            "\tint32 WeakPointsDestroyed{};\n",
        )
    ...
    def test_does_not_parse_neighbors_or_director_methods(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIn(STOP_BEFORE_WEAPON, header)
        self.assertIn(STOP_BEFORE_PHASE, header)
        self.assertIn(STOP_BEFORE_COMMAND, header)
        ...
        self.assertNotIn(STOP_BEFORE_WEAPON, leaked)
        self.assertNotIn(STOP_BEFORE_PHASE, leaked)
        self.assertNotIn(STOP_BEFORE_COMMAND, leaked)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn("ClampMin", UPROPERTY_VISIBLE)
        self.assertNotIn("ClampMax", UPROPERTY_VISIBLE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-telemetry WeakPointsDestroyed field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_boss_telemetry_weak_points_destroyed"
            "_field_decl_contract.py"
        ))
