# THIS IS leftover-safe FSkyguardLoadoutSpec CannonReserve.
...
TARGET_WRONG_BARE = "int32 CannonReserve;"
TARGET_WRONG_ZERO = "int32 CannonReserve = 0;"
TARGET_WRONG_ZERO_F = "int32 CannonReserve = 0.f;"
TARGET_WRONG_FALSE = "int32 CannonReserve = false;"
TARGET_WRONG_HUNDRED = "int32 CannonReserve = 100;"
TARGET_WRONG_SIX = "int32 CannonReserve = 6;"
TARGET_WRONG_CPG_FEEL = (
    "int32 CannonReserve = SkyguardApacheCpgFeel::CannonMagazineSize;"
)
TARGET_WRONG_ROCKET_RESERVE = (
    "int32 CannonReserve = SkyguardApacheCpgFeel::RocketReserve;"
)
TARGET_WRONG_GUIDED_RESERVE = (
    "int32 CannonReserve = SkyguardApacheCpgFeel::GuidedReserve;"
)
TARGET_WRONG_FLOAT = (
    "float CannonReserve = SkyguardApacheCpgFeel::CannonReserve;"
)
TARGET_WRONG_BOOL = (
    "bool CannonReserve = SkyguardApacheCpgFeel::CannonReserve;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # constant (CannonMagazineSize / RocketReserve / etc.).
    # Do not accept sibling FlareCount / HullIntegrity.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+CannonReserve\s*=\s*"
        r"SkyguardApacheCpgFeel::CannonReserve\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"int32\s+CannonReserve\s*=\s*0\b", compact):
        return False
    if re.search(r"int32\s+CannonReserve\s*=\s*6\b", compact):
        return False
    if re.search(r"int32\s+CannonReserve\s*=\s*100\b", compact):
        return False
    if re.search(r"int32\s+CannonReserve\s*=\s*0\.f", compact):
        return False
    if re.search(r"int32\s+CannonReserve\s*=\s*false\b", compact):
        return False
    if re.search(
        r"int32\s+CannonReserve\s*=\s*"
    ...
    if re.search(
        ...
        return False
    if re.search(r"float\s+CannonReserve\b", compact):
        return False
    if re.search(r"bool\s+CannonReserve\b", compact):
        return False
    if re.search(r"int32\s+CannonReserve\s*;", compact) and not re.search(
        r"int32\s+CannonReserve\s*=",
        compact,
    ):
        return False
    return True
...
class LoadoutSpecCannonReserveFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CannonReserve"), section)
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CannonReserve"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CannonReserve", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cannon_reserve_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 CannonReserve"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("SkyguardApacheCpgFeel::CannonReserve", LOCKED_DECL)
    ...
    def test_hud_snapshot_zero_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("int32 CannonReserve = 0;", compact_origin)
        self.assertNotIn("int32 CannonReserve = 6;", compact_origin)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 CannonReserves = "
            "SkyguardApacheCpgFeel::CannonReserve;\n",
            "\tint32 CannonReserve = " + forty + ";\n",
            "\tint32 CannonReserve = " + eighty + ";\n",
        )
    ...
    def test_cannon_reserve_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 CannonReserve"),
            LOCKED_DECL,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec CannonReserve field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cannon_reserve_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertIn(LEFTOVER_CPG_FEEL + "::CannonReserve", locked_only)
