# THIS IS leftover-safe FSkyguardMeshBindSlot Preferred.
...
# then BARE `TSoftObjectPtr<UStaticMesh> Preferred;` (no initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|MeshBind`. EditAnywhere IS present.
# BlueprintReadOnly IS present. There is NO default.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if Category is missing or changed, if
# EditAnywhere is dropped, if BlueprintReadWrite replaces
# BlueprintReadOnly, if the type is not
# TSoftObjectPtr<UStaticMesh>, or if an initializer is added.
...
# Do NOT parse leftover catalog class body.
# Do NOT contract sibling fields SlotId / ProxyFallback / Notes.
# Both Preferred and ProxyFallback are TSoftObjectPtr<UStaticMesh>;
# lock Preferred only.
# Analog leftover mesh-bind-slot-fields #194 / #6829 then
# this isolated Preferred field decl.
# THIS IS NOT leftover #194 / #6829
# (test_mesh_bind_slot_fields_contract.py stays in LOCKED_SCRIPTS).
...
# THIS IS NOT leftover ProductionAudio.
# THIS IS NOT leftover MissionDebrief.
...
# FName WeatherIdentity / no Category / required initializer,
# retarget: EditAnywhere, BlueprintReadOnly,
# Category="Skyguard|MeshBind",
# TSoftObjectPtr<UStaticMesh> Preferred, no `=`.
...
CATALOG_CLASS = "USkyguardRuntimeMeshCatalog"
...
TARGET = "TSoftObjectPtr<UStaticMesh> Preferred;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<UStaticMesh> Preferred = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<UStaticMesh> Preferred = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<UStaticMesh> Preferred = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<UStaticMesh> Preferred = 0.f;"
TARGET_WRONG_FLOAT = "float Preferred;"
TARGET_WRONG_FNAME = "FName Preferred;"
TARGET_WRONG_PROXY = "TSoftObjectPtr<UStaticMesh> ProxyFallback;"
...
def catalog_class_body_tokens() -> tuple[str, ...]:
    return (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling SlotId / ProxyFallback / Notes.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*UStaticMesh\s*>\s+Preferred\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Preferred\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+Preferred\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*UStaticMesh\s*>\s+Preferred\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<UStaticMesh> Preferred is "
...
class MeshBindSlotPreferredFieldDeclContractTests(unittest.TestCase):
    def test_mesh_bind_slot_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, CATALOG_CLASS)
        ...
        self.assertTrue(has_identifier(section, "Preferred"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_CATALOG, header)
        self.assertIn("SKYGUARD52_API", STOP_BEFORE_CATALOG)
        self.assertTrue(
        ...
        for token in catalog_class_body_tokens():
            self.assertTrue(has_identifier(header, token), token)
            self.assertFalse(has_identifier(section, token), token)
            self.assertFalse(has_identifier(body, token), token)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedMeshBindSlot\n{\n};\n"
            )
    ...
    def test_catalog_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Preferred"), section)
        self.assertNotIn(STOP_BEFORE_CATALOG, section)
        self.assertNotIn(CATALOG_CLASS, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Preferred", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_preferred_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_SLOT_ID};\n"
            f"\t{TARGET_WRONG_PROXY}\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("TSoftObjectPtr<UStaticMesh> Preferred;", compact_origin)
        self.assertNotIn("Preferred = nullptr", compact_origin)
        self.assertNotIn("Preferred = false", compact_origin)
        self.assertNotIn("Preferred = true", compact_origin)
        self.assertNotIn("Preferred = 0.f", compact_origin)
        self.assertNotIn("Preferred = 160.f", compact_origin)

    def test_preferred_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSoftObjectPtr<UStaticMesh> Preferred"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_PROXY}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_PROXY}\n", LOCKED_DECL)
        self.assertIn("Preferred", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFName {SIBLING_SLOT_ID};\n",
            f"\tFString {SIBLING_NOTES};\n",
            "\tTSoftObjectPtr<UStaticMesh> Preferreds;\n",
            "\tint32 Preferred;\n",
            "\tbool Preferred;\n",
            "\tfloat Preferred = " + forty + ";\n",
            "\tfloat Preferred = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_mesh_bind_fields(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_CATALOG, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_catalog_class_or_neighbors(self) -> None:
        ...
        self.assertIn(CATALOG_CLASS, header)
        self.assertNotIn(f"class SKYGUARD52_API {CATALOG_CLASS}", section)
        self.assertNotIn(f"class SKYGUARD52_API {CATALOG_CLASS}", leaked)
        for token in catalog_class_body_tokens():
            self.assertTrue(has_identifier(header, token), token)
            self.assertFalse(has_identifier(section, token), token)
            self.assertFalse(has_identifier(leaked, token), token)
        self.assertFalse(has_identifier(section, CATALOG_SLOTS_FIELD))
        self.assertFalse(has_identifier(leaked, CATALOG_SLOTS_FIELD))
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        ...
        for helper in namespace_helper_tokens():
            self.assertFalse(has_identifier(section, helper), helper)
            self.assertFalse(has_identifier(leaked, helper), helper)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tconst FSkyguardMeshBindSlot& {RESOLVE_SLOT}"
            "(FName SlotId);\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mesh-bind-slot Preferred field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "preferred_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardRuntimeMeshCatalog.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
