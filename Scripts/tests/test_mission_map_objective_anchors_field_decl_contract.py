...
# then BARE `TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors;`
# (NO initializer).
...
# This is a public TArray objective-anchor field, not a combat
# look float, not leftover Theater WeatherIdentity, not leftover
# FName AssemblyRevision, and not leftover analog
...
# or stay Category="Skyguard" without |MissionMap.
...
# Stop BEFORE sibling LandmarkAnchors as a claimed slot.
...
# MissionDefinition / MissionId / AssemblyRevision /
# SkylineStyle / WeatherProfileId / RoutePoints /
# LandmarkAnchors / FlightClearanceRadiusCentimeters /
# FlightClearanceVerticalCentimeters / Readiness as this slot.
# Do NOT claim leftover FSkyguardMissionObjectiveAnchor
# ObjectiveId / WorldLocation struct members as this slot.
# Do NOT claim leftover mission-objective-anchor-defaults
...
# Category = "Skyguard|MissionMap" wrapping
# `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# — RETARGET hard: same wrap Category = "Skyguard|MissionMap"
# (spaces around =). LOCKED_DECL is
# `TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors;`
# with NO initializer. NOT leftover
# `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# and NOT `FName WeatherIdentity;` and NOT
# `FName ObjectiveId;` and NOT
# `FVector WorldLocation = FVector::ZeroVector;` and NOT
# `TArray<FVector> RoutePoints;` and NOT
...
# / FName AssemblyRevision / FName ObjectiveId /
# FVector WorldLocation / TArray<FVector> RoutePoints /
# TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors
# AS THE LOCKED DECL.
...
TARGET = "TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors;"
TARGET_WRONG_BARE = "FName AssemblyRevision;"
TARGET_WRONG_INIT = (
    "TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors = {};"
)
TARGET_WRONG_FALSE = (
    "TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors = false;"
)
TARGET_WRONG_TRUE = (
    "TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors = true;"
)
TARGET_WRONG_ZERO = (
    "TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors = 0.f;"
)
TARGET_WRONG_SKELETAL = (
    "TObjectPtr<USkeletalMeshComponent> ObjectiveAnchors;"
)
TARGET_WRONG_ASSEMBLY_REVISION = (
    'FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");'
)
TARGET_WRONG_OBJECTIVE_ID = "FName ObjectiveId;"
TARGET_WRONG_WORLD_LOCATION = (
    "FVector WorldLocation = FVector::ZeroVector;"
)
TARGET_WRONG_ROUTE_POINTS = "TArray<FVector> RoutePoints;"
TARGET_WRONG_LANDMARK_ANCHORS = (
    "TArray<FSkyguardMissionLandmarkAnchor> LandmarkAnchors;"
)
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> ObjectiveAnchors;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> ObjectiveAnchors;"
...
STOP_BEFORE_LANDMARK_ANCHORS = "LandmarkAnchors"
...
LEFTOVER_FLIGHT_ROUTE_SPLINE = (
    "Scripts/tests/test_mission_map_flight_route_spline"
    "_field_decl_contract.py"
)
LEFTOVER_FLIGHT_CLEARANCE_RADIUS = (
    "Scripts/tests/test_mission_map_flight_clearance_radius"
    "_centimeters_field_decl_contract.py"
)
LEFTOVER_FLIGHT_CLEARANCE_VERTICAL = (
    "Scripts/tests/test_mission_map_flight_clearance_vertical"
    "_centimeters_field_decl_contract.py"
)
...
OBJECTIVE_ANCHORS_DECL_RE = re.compile(
    r"TArray\s*<\s*FSkyguardMissionObjectiveAnchor\s*>\s+"
    r"ObjectiveAnchors\s*;"
)
OBJECTIVE_ANCHORS_INIT_RE = re.compile(
    r"TArray\s*<\s*FSkyguardMissionObjectiveAnchor\s*>\s+"
    r"ObjectiveAnchors\s*="
)
ASSEMBLY_REVISION_DECL_RE = re.compile(
    r"FName\s+AssemblyRevision\s*=\s*TEXT\(\s*\"CampaignMapAssembly_v1\"\s*\)\s*;"
)
ASSEMBLY_REVISION_BARE_RE = re.compile(
    r"FName\s+AssemblyRevision\s*;"
)
OBJECTIVE_ID_DECL_RE = re.compile(
    r"FName\s+ObjectiveId\b"
)
WORLD_LOCATION_DECL_RE = re.compile(
    r"FVector\s+WorldLocation\b"
)
ROUTE_POINTS_DECL_RE = re.compile(
    r"TArray\s*<\s*FVector\s*>\s+RoutePoints\b"
)
LANDMARK_ANCHORS_DECL_RE = re.compile(
    r"TArray\s*<\s*FSkyguardMissionLandmarkAnchor\s*>\s+"
    r"LandmarkAnchors\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `TArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors;`
    # with NO initializer.
    # Do not accept leftover FName AssemblyRevision,
    # leftover FName WeatherIdentity, leftover FName ObjectiveId,
    # leftover FVector WorldLocation, leftover TArray RoutePoints,
    # leftover TArray LandmarkAnchors, leftover FName MissionId,
    ...
    # FlightRouteSpline / MissionDefinition / SkylineStyle /
    # Readiness, or an ObjectiveAnchors initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if OBJECTIVE_ANCHORS_DECL_RE.search(compact) is None:
        return False
    if OBJECTIVE_ANCHORS_INIT_RE.search(compact):
        return False
    if ASSEMBLY_REVISION_DECL_RE.search(compact):
        return False
    if ASSEMBLY_REVISION_BARE_RE.search(compact):
        return False
    if OBJECTIVE_ID_DECL_RE.search(compact):
        return False
    if WORLD_LOCATION_DECL_RE.search(compact):
        return False
    if ROUTE_POINTS_DECL_RE.search(compact):
        return False
    if LANDMARK_ANCHORS_DECL_RE.search(compact):
        return False
...
def field_identifier(field_text: str) -> str:
    ...
    match = re.search(
        r"(?:TObjectPtr<[^>]+>|TSubclassOf<[^>]+>|TArray<[^>]+>|"
        r"float|int32|bool|"
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_LANDMARK_ANCHORS:
            break
        if ident != "ObjectiveAnchors":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_LANDMARK_ANCHORS):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_LANDMARK_ANCHORS}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray<FSkyguardMissionObjectiveAnchor>\s+"
            r"ObjectiveAnchors\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FSkyguardMissionObjectiveAnchor> "
        "ObjectiveAnchors "
...
class MissionMapObjectiveAnchorsFieldDeclContractTests(unittest.TestCase):
    def test_mission_map_assembly_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ObjectiveAnchors"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(body, STOP_BEFORE_LANDMARK_ANCHORS), body
        )
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_LANDMARK_ANCHORS),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_ASSEMBLY_REVISION), body)
        self.assertFalse(
            has_identifier(section, SIBLING_ASSEMBLY_REVISION), section
        )
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_ROUTE_POINTS), section
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ObjectiveAnchors"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ObjectiveAnchors", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_objective_anchors_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("FName ObjectiveId;", compact_origin)
    ...
    def test_objective_anchors_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_OBJECTIVE_ID)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WORLD_LOCATION)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_POINTS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LANDMARK_ANCHORS)
        ...
        self.assertIn("FSkyguardMissionObjectiveAnchor", LOCKED_DECL)
        self.assertNotIn("FSkyguardMissionLandmarkAnchor", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 ObjectiveAnchors = 1;\n",
            "\tbool ObjectiveAnchors = true;\n",
            "\tTArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors = "
            + forty
            + ";\n",
            "\tTArray<FSkyguardMissionObjectiveAnchor> ObjectiveAnchors = "
            ...
            f"\t{TARGET_WRONG_OBJECTIVE_ID}\n",
            f"\t{TARGET_WRONG_WORLD_LOCATION}\n",
            f"\t{TARGET_WRONG_ROUTE_POINTS}\n",
            f"\t{TARGET_WRONG_LANDMARK_ANCHORS}\n",
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tTArray<FSkyguardMissionObjectiveAnchor>\n"
            "\tObjectiveAnchors;\n",
            "\tTArray<FSkyguardMissionObjectiveAnchor>   "
            "ObjectiveAnchors;\n",
            "\tTArray<FSkyguardMissionObjectiveAnchor>\t"
    ...
    def test_does_not_claim_sibling_landmark_anchors_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "ObjectiveAnchors"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LANDMARK_ANCHORS))
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_LANDMARK_ANCHORS)
        )
        self.assertTrue(has_identifier(leaked, SIBLING_ASSEMBLY_REVISION))
        self.assertFalse(
            has_identifier(section, SIBLING_ASSEMBLY_REVISION)
        )
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "AssemblyRevision"))
        self.assertTrue(has_identifier(section, "ObjectiveAnchors"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LANDMARK_ANCHORS))
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_LANDMARK_ANCHORS)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map ObjectiveAnchors field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_map_objective_anchors_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("LandmarkAnchors", locked_only)
