# THIS IS leftover-safe FSkyguardMissionResult::ShotsFired.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `int32 ShotsFired = 0;`.
...
# this isolated ShotsFired field decl.
...
# FName / no initializer, RETARGET: specifiers are
# EditAnywhere + BlueprintReadWrite, type is int32,
...
# not int32, the initializer is dropped or changed from 0,
...
# Match EXACT identifier ShotsFired as a whole field name
# after int32. Do NOT claim leftover FinalScore / MedalTier.
# Do NOT claim sibling Hits (also int32 = 0).
# Siblings stay unclaimed: MissionId / bMissionSucceeded /
# Hits / AircraftDamageFraction / CompletionTimeSeconds /
...
TARGET = "int32 ShotsFired = 0;"
TARGET_WRONG_FALSE = "int32 ShotsFired = false;"
TARGET_WRONG_ONE = "int32 ShotsFired = 1;"
TARGET_WRONG_FLOAT = "int32 ShotsFired = 0.f;"
TARGET_WRONG_TRUE = "int32 ShotsFired = true;"
TARGET_BARE = "int32 ShotsFired;"
TARGET_WRONG_HITS = "int32 Hits = 0;"
TARGET_WRONG_FINAL = "int32 FinalScore = 0;"
TARGET_WRONG_MEDAL = "int32 MedalTier = 0;"
TARGET_WRONG_FNAME = "FName ShotsFired;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # = false / = 1 / = 0.f / = true / a bare decl / sibling
    # Hits / leftover FinalScore for FSkyguardMissionResult
    # ::ShotsFired (must be int32 ShotsFired = 0).
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"\bHits\b", compact) and not re.search(
        r"\bShotsFired\b", compact
    ):
        return False
    if re.search(
        r"int32\s+ShotsFired\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+ShotsFired\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+ShotsFired\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+ShotsFired\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 ShotsFired is missing from "
...
class MissionResultShotsFiredFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ShotsFired"), section)
        ...
        self.assertNotIn("int32 ShotsFired)", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("ShotsFired", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_shots_fired_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_shots_fired_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 ShotsFired"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("0;"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "int32 ShotsFired = 0;")
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HITS}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_HITS}\n", LOCKED_DECL)
        self.assertIn("ShotsFired", str(raised.exception))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result ShotsFired field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_shots_fired_field_decl_contract.py"
        ))
