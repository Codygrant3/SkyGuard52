...
# INDEX_NONE or lock StarboardDischargeBoom
# construction in the .cpp. This is a Tempest
# FIELD contract, NOT leftover Tempest methods.
# Leftover Harbor #6/#8/#9, leftover theater-kit
# #59, leftover drafts #56-#64, leftover
# isolated-test drafts #107-#510 stay sibling-only.
# This is a FIELD
# contract on ASkyguardTempestBoss, not a
...
# StarboardDischargeBoom;`); accept that form and
...
# Do not lock those sibling fields as the
# primary lock.
# Jammer, PortEngine,
# StarboardEngine, DebrisArmorPort,
...
# Python StarboardDischargeBoom field declaration
# contract on ASkyguardTempestBoss. Stay off
...
# leftover isolated-test drafts #107–#510,
...
LEFTOVER_STARBOARD_ENGINE_NOT_LOCKED = (
    "test_last_flight_starboard_engine_field_decl_contract.py",
    "StarboardEngine",
)
...
LEFTOVER_REQUIRED_LOCK_STABILITY_SECONDS_NOT_LOCKED = (
    "test_tempest_required_lock_stability_seconds_field_decl_contract.py",
    "RequiredLockStabilitySeconds",
)
LEFTOVER_GET_LOCK_STABILITY_SECONDS_NOT_LOCKED = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardTempestBoss::StarboardDischargeBoom",
    "SkyguardTempestBoss.cpp",
    "SetExposed",
...
class TempestStarboardDischargeBoomFieldDeclContractTests(unittest.TestCase):
    def test_tempest_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, STARBOARD_DISCHARGE_BOOM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn("StarboardDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_missing_starboard_discharge_boom_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardTempestBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn("StarboardDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn("StarboardDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("VisibleAnywhere", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("BlueprintReadOnly", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("Category", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_PORT)
        self.assertNotIn("BlueprintCallable", UPROPERTY_PORT)
        self.assertIn("Skyguard|Mission05|Boss", UPROPERTY_PORT)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn("StarboardDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* StarboardDischargeBoom;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> StarboardDischargeBoom;\n"
        )
        weak = (
        ...
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardDischargeBoom = nullptr;\n"
        )
        wrong_name = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardGuidanceArray;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, STARBOARD_DISCHARGE_BOOM_FIELD)
            self.assertIn("StarboardDischargeBoom", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_starboard_discharge_boom_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, STARBOARD_DISCHARGE_BOOM_FIELD),
            1,
        )
        self.assertTrue(
            STARBOARD_DISCHARGE_BOOM_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        self.assertTrue(
            STARBOARD_DISCHARGE_BOOM_FIELD.endswith(";"),
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        self.assertIn("StarboardDischargeBoom", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        self.assertIn("TObjectPtr<", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("TArray<", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("=", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("INDEX_NONE", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("UFUNCTION", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("{", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("}", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("return ", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IssueClimbCommand", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        ...
        self.assertNotIn("GetCivilianSeparationMeters", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetFinaleStage", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IsClimbCommandIssued", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("IsWreckDiverted", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetGunnerMount", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("FaceWorldLocation", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardGuidanceArray", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortCoolingSystem", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortEngine", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        self.assertNotIn("SetLightningExposed", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        self.assertNotIn("HullCollider", STARBOARD_DISCHARGE_BOOM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {STARBOARD_DISCHARGE_BOOM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{STARBOARD_DISCHARGE_BOOM_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, STARBOARD_DISCHARGE_BOOM_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, STARBOARD_DISCHARGE_BOOM_FIELD),
    ...
    def test_assigned_starboard_discharge_boom_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardDischargeBoom = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn("StarboardDischargeBoom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, STARBOARD_DISCHARGE_BOOM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            self.assertIn("StarboardDischargeBoom", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, STARBOARD_DISCHARGE_BOOM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{STARBOARD_DISCHARGE_BOOM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, STARBOARD_DISCHARGE_BOOM_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, STARBOARD_DISCHARGE_BOOM_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            STARBOARD_DISCHARGE_BOOM_FIELD.startswith("UFUNCTION"),
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, STARBOARD_DISCHARGE_BOOM_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", STARBOARD_DISCHARGE_BOOM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_starboard_discharge_boom_cpp_body(
        self,
    ) -> None:
        locked_only = f"{STARBOARD_DISCHARGE_BOOM_FIELD}\n"
        self.assertNotIn("{", STARBOARD_DISCHARGE_BOOM_FIELD)
        ...
        self.assertNotIn("return false", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("return true", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetExposed", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
        self.assertNotIn("SetBossPhase", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_apply_strike(), STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), STARBOARD_DISCHARGE_BOOM_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, STARBOARD_DISCHARGE_BOOM_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_last_flight_fields(self) -> None:
        ...
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortGuidanceArray", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardGuidanceArray", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardCoolingSystem", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("Jammer", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortEngine", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("StarboardEngine", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("PortCoolingSystem", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_DISCHARGE_BOOM_FIELD,
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_engine(
        self,
    ) -> None:
        locked_only = f"{STARBOARD_DISCHARGE_BOOM_FIELD}\n"
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("OpenOrbitExposure", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SetLightningExposed", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("ASkyguardGunner", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SkyguardRadarNode", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("MaxIntegrity", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("CurrentIntegrity", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn("550.f", STARBOARD_DISCHARGE_BOOM_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{STARBOARD_DISCHARGE_BOOM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, STARBOARD_DISCHARGE_BOOM_FIELD),
            STARBOARD_DISCHARGE_BOOM_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "tempest StarboardDischargeBoom field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{STARBOARD_DISCHARGE_BOOM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, STARBOARD_DISCHARGE_BOOM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                STARBOARD_DISCHARGE_BOOM_FIELD.lower(),
                "tempest StarboardDischargeBoom contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, STARBOARD_DISCHARGE_BOOM_FIELD)

    def test_contract_is_starboard_discharge_boom_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(STARBOARD_DISCHARGE_BOOM_FIELD.endswith(";"))
        self.assertNotIn("=", STARBOARD_DISCHARGE_BOOM_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
