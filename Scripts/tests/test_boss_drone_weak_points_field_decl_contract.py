...
# INDEX_NONE or lock WeakPoints construction
# in the .cpp. This is a FIELD contract on
# ASkyguardBossDroneBase, not a leftover
...
# split-line wraps. The element type appears
# on origin/main and must be part of the
# locked field declaration. Nearby origin/main
...
# Apache HullCollider field #425. Stay
# off leftover Root field. Stay off leftover
# GetFinaleStage /
...
# and leftover accept flags on that
# component. Do not open
# SkyguardBossWeakPointComponent.h as the
# locked header. Leftover
...
# Python WeakPoints field declaration
...
# member fields stay unlocked. The locked
# field is the ASkyguardBossDroneBase
# WeakPoints array; its element type is
# required. Do not open
# SkyguardBossWeakPointComponent.h.
LEFTOVER_WEAK_POINT_COMPONENT_FIELDS_NOT_LOCKED = (
    "PortGuidanceArray",
...
# specifiers. Neighbor Root / Phase may
...
# .cpp WeakPoints body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBossDroneBase::WeakPoints",
    "SkyguardBossDroneBase.cpp",
...
class BossDroneWeakPointsFieldDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, WEAK_POINTS_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, WEAK_POINTS_FIELD)
    ...
    def test_missing_weak_points_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, WEAK_POINTS_FIELD)
        self.assertIn("WeakPoints", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_WEAK_POINTS}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, WEAK_POINTS_FIELD)
        self.assertIn("WeakPoints", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_WEAK_POINTS, section)
        ...
        self.assertNotIn("UPROPERTY", WEAK_POINTS_FIELD)
        self.assertNotIn("VisibleAnywhere", WEAK_POINTS_FIELD)
        self.assertNotIn("BlueprintReadOnly", WEAK_POINTS_FIELD)
        self.assertNotIn("Category", WEAK_POINTS_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_WEAK_POINTS)
        self.assertNotIn("BlueprintCallable", UPROPERTY_WEAK_POINTS)
        self.assertIn("Skyguard|Boss", UPROPERTY_WEAK_POINTS)
        self.assertNotIn("Destruction", UPROPERTY_WEAK_POINTS)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_WEAK_POINTS)
            self.assertNotIn(invented, WEAK_POINTS_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, WEAK_POINTS_FIELD)
        self.assertIn("WeakPoints", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tTArray<USkyguardBossWeakPointComponent*> WeakPoints;\n"
        )
        not_array = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> WeakPoints;\n"
        )
        scene = (
            "\tTArray<TObjectPtr<USceneComponent>> WeakPoints;\n"
        )
        weak = (
            "\tTArray<TWeakObjectPtr<USkyguardBossWeakPointComponent>> "
            "WeakPoints;\n"
        )
        assigned = (
            "\tTArray<TObjectPtr<USkyguardBossWeakPointComponent>> "
        ...
        wrong_name = (
            "\tTArray<TObjectPtr<USkyguardBossWeakPointComponent>> Root;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, WEAK_POINTS_FIELD)
            self.assertIn("WeakPoints", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_weak_points_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, WEAK_POINTS_FIELD),
            1,
        )
        self.assertTrue(
            WEAK_POINTS_FIELD.startswith(
                "TArray<TObjectPtr<USkyguardBossWeakPointComponent>> "
            ),
            WEAK_POINTS_FIELD,
        )
        self.assertTrue(
            WEAK_POINTS_FIELD.endswith(";"),
            WEAK_POINTS_FIELD,
        )
        self.assertIn("WeakPoints", WEAK_POINTS_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            WEAK_POINTS_FIELD,
        )
        self.assertIn("TArray<", WEAK_POINTS_FIELD)
        self.assertIn("TObjectPtr<", WEAK_POINTS_FIELD)
        self.assertNotIn("=", WEAK_POINTS_FIELD)
        self.assertNotIn("INDEX_NONE", WEAK_POINTS_FIELD)
        self.assertNotIn("UFUNCTION", WEAK_POINTS_FIELD)
        self.assertNotIn("GetBossPhase", WEAK_POINTS_FIELD)
        self.assertNotIn("{", WEAK_POINTS_FIELD)
        self.assertNotIn("}", WEAK_POINTS_FIELD)
        self.assertNotIn("return ", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            WEAK_POINTS_FIELD,
        )
        self.assertNotIn("ApplyWeaponHit", WEAK_POINTS_FIELD)
        self.assertNotIn("IssuePilotCommand", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            WEAK_POINTS_FIELD,
        )
        ...
        self.assertNotIn("GetDefeatDebrisPieceCount", WEAK_POINTS_FIELD)
        self.assertNotIn("GetTelemetry", WEAK_POINTS_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", WEAK_POINTS_FIELD)
        self.assertNotIn("GetGunnerMount", WEAK_POINTS_FIELD)
        self.assertNotIn("FaceWorldLocation", WEAK_POINTS_FIELD)
        self.assertNotIn("TObjectPtr<USceneComponent> Root;", WEAK_POINTS_FIELD)
        self.assertNotIn("SetSearchlightTracked", WEAK_POINTS_FIELD)
        self.assertNotIn("SetContactIdentified", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            WEAK_POINTS_FIELD,
        )
        self.assertNotIn("SetLightningExposed", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            WEAK_POINTS_FIELD,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, WEAK_POINTS_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_WEAK_POINTS}\n"
            f"\t{WEAK_POINTS_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_WEAK_POINTS} {WEAK_POINTS_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{WEAK_POINTS_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, WEAK_POINTS_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, WEAK_POINTS_FIELD),
    ...
    def test_assigned_weak_points_does_not_satisfy(self) -> None:
        assigned = (
            "\tTArray<TObjectPtr<USkyguardBossWeakPointComponent>> "
            "WeakPoints = {};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, WEAK_POINTS_FIELD)
        self.assertIn("WeakPoints", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, WEAK_POINTS_FIELD))

    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        root = "\tTObjectPtr<USceneComponent> Root;\n"
        body = "\tTObjectPtr<UStaticMeshComponent> BodyMesh;\n"
        phase = "\tESkyguardBossPhase Phase = ESkyguardBossPhase::Approach;\n"
        telemetry = "\tFSkyguardBossTelemetry Telemetry;\n"
        for region in (root, body, phase, telemetry):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, WEAK_POINTS_FIELD)
            self.assertIn("WeakPoints", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, WEAK_POINTS_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{WEAK_POINTS_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, WEAK_POINTS_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_WEAK_POINTS, section)
        self.assertTrue(has_declaration(section, WEAK_POINTS_FIELD), section)

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{WEAK_POINTS_FIELD}\n"
        self.assertNotIn("UFUNCTION", WEAK_POINTS_FIELD)
        self.assertNotIn("UFUNCTION", locked_only)
        self.assertFalse(WEAK_POINTS_FIELD.startswith("UFUNCTION"), WEAK_POINTS_FIELD)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, WEAK_POINTS_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", WEAK_POINTS_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_weak_points_cpp_body(
        self,
    ) -> None:
        locked_only = f"{WEAK_POINTS_FIELD}\n"
        self.assertNotIn("{", WEAK_POINTS_FIELD)
        ...
        self.assertNotIn("return false", WEAK_POINTS_FIELD)
        self.assertNotIn("return true", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            WEAK_POINTS_FIELD,
        )
        self.assertNotIn("SetBossPhase", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            WEAK_POINTS_FIELD,
        )
        self.assertNotIn("RegisterDefeatDebris", WEAK_POINTS_FIELD)
        self.assertNotIn("CleanupDefeatDebris", WEAK_POINTS_FIELD)
        self.assertNotIn("HandleDefeated", WEAK_POINTS_FIELD)

    def test_contract_does_not_relock_get_boss_phase_sibling(self) -> None:
        locked_only = f"{WEAK_POINTS_FIELD}\n"
        for token in GET_BOSS_PHASE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("GetBossPhase", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_sibling_public_fields(self) -> None:
        ...
        for token in PUBLIC_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("BodyMesh", WEAK_POINTS_FIELD)
        self.assertNotIn("Root", WEAK_POINTS_FIELD)
        self.assertNotIn("CurrentPilotCommand", WEAK_POINTS_FIELD)
        self.assertNotIn("Telemetry", WEAK_POINTS_FIELD)
        self.assertNotIn("OnBossPhaseChanged", WEAK_POINTS_FIELD)
        self.assertNotIn("OnPilotCommandNative", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_hull_collider(self) -> None:
        ...
        for token in LEFTOVER_HULL_COLLIDER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("HullCollider", WEAK_POINTS_FIELD)

    def test_contract_does_not_relock_leftover_root_field(self) -> None:
        locked_only = f"{WEAK_POINTS_FIELD}\n"
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_weak_point_component_fields(
        ...
        locked_only = f"{WEAK_POINTS_FIELD}\n"
        for token in LEFTOVER_WEAK_POINT_COMPONENT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        ...
        self.assertNotIn("PortGuidanceArray", WEAK_POINTS_FIELD)
        self.assertNotIn("CommandCore", WEAK_POINTS_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            WEAK_POINTS_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("SetSearchlightTracked", WEAK_POINTS_FIELD)
        self.assertNotIn("IsSearchlightTracked", WEAK_POINTS_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("SetContactIdentified", WEAK_POINTS_FIELD)
        self.assertNotIn("IsContactIdentified", WEAK_POINTS_FIELD)
        self.assertNotIn("OpenOrbitExposure", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            WEAK_POINTS_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", WEAK_POINTS_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            WEAK_POINTS_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("GetGunnerMount", WEAK_POINTS_FIELD)
        self.assertNotIn("GetChinTurret", WEAK_POINTS_FIELD)
        self.assertNotIn("GetSensorTurret", WEAK_POINTS_FIELD)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{WEAK_POINTS_FIELD}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, WEAK_POINTS_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("ASkyguardGunner", WEAK_POINTS_FIELD)
        self.assertNotIn("FillAndFinalize", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("SkyguardRadarNode", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            WEAK_POINTS_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, WEAK_POINTS_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            WEAK_POINTS_FIELD,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINTS_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", WEAK_POINTS_FIELD)
        self.assertNotIn("MaxIntegrity", WEAK_POINTS_FIELD)
        self.assertNotIn("CurrentIntegrity", WEAK_POINTS_FIELD)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            WEAK_POINTS_FIELD,
        )
        self.assertNotIn("SkyguardPathfinder", WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, WEAK_POINTS_FIELD)
        ...
        self.assertEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardBossDroneBase.h",
        )
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, WEAK_POINTS_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{WEAK_POINTS_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, WEAK_POINTS_FIELD),
            WEAK_POINTS_FIELD,
        )
        ...
        self.assertNotIn("TObjectPtr<USceneComponent> Root;", locked_only)
        self.assertNotIn("SkyguardBossWeakPointComponent.h", locked_only)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone WeakPoints field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, WEAK_POINTS_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                WEAK_POINTS_FIELD.lower(),
                "boss drone WeakPoints contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, WEAK_POINTS_FIELD)

    def test_contract_is_weak_points_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(WEAK_POINTS_FIELD.endswith(";"))
        self.assertNotIn("=", WEAK_POINTS_FIELD)
        self.assertIn(UPROPERTY_WEAK_POINTS, section)
