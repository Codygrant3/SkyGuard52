# THIS IS leftover-safe ASkyguardDroneSpawner
# HeavyLaneModulo.
# origin/main form: one-line EditAnywhere BlueprintReadWrite wrap
# UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Skyguard|Spawning")
# then `int32 HeavyLaneModulo = 4;` (HAS initializer).
# Type is **int32**, not float. Default IS `= 4`.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Spawning`. EditAnywhere IS present.
# BlueprintReadWrite IS present.
# Specifiers ARE EditAnywhere + BlueprintReadWrite (NOT EditDefaultsOnly,
# NOT BlueprintReadOnly, NOT VisibleAnywhere). ClampMin IS absent.
# ClampMax IS absent. KEEP specifiers matching origin/main.
# Parse CLASS `ASkyguardDroneSpawner` public UPROPERTY window only.
# Do NOT wrap Category=`Skyguard` fields SpawnInterval or MaxActiveDrones.
# leftover analog drone-spawner-empty-fail-closed bulk #161/#df16 is
# behavioral.
# Word-boundary: `HeavyLaneModulo` is NOT leftover analog
# `MaxActiveDrones = 12`, NOT leftover analog `int32 PoolCapacity = 12`,
# NOT leftover analog `bSpawningEnabled`, NOT leftover analog
# `bMixedThreatRoster`.
# LOCKED_DECL is NOT leftover `int32 MaxActiveDrones = 12` /
# `int32 PoolCapacity = 12`. LOCKED_DECL is NOT leftover
# `float SpawnInterval = 1.85f`. LOCKED_DECL is NOT leftover
# `void SetSpawningEnabled(bool bEnabled);`. LOCKED_DECL is NOT leftover
# `bool bSpawningEnabled = true;`. LOCKED_DECL is NOT leftover
# `FName WeatherIdentity`. LOCKED_DECL is NOT leftover
# `FireRate = 12.0f` / `RecoilPitch = 0.92f` / `RouteLengthCm = 45000.f`
# / `160.f`. Harbor leftover is only the split 40 / 80 float tokens.
# Category MUST be `Skyguard|Spawning`. Fail-closed if wrap is
# `Category="Skyguard"` without leftover-safe unique `|Spawning` suffix.
# Do NOT wrap bSpawningEnabled / bMixedThreatRoster / LightSpeedMin /
# LightSpeedMax / HeavyCruiseSpeed / MaxActiveDrones / SpawnInterval
# in this PR.
# Clone leftover unique analog FIELD MaxReleasesPerBay #1683 SHA
# 40592bc71aaf17722aacf8a3a611189e25fbc822 RETARGET int32 default.
# Clone leftover unique analog FIELD MinimumCivilianSeparationMeters
# #1684 SHA e3a50c50ac111d614f29a870948fbcf74326be63 RETARGET.
# Clone leftover unique analog Coastal PlacementSeed #1643 RETARGET
# EditAnywhere BlueprintReadWrite int32 with initializer, no clamp.
# Clone leftover unique analog SetSpawningEnabled SHA
# ebbc1fd01dd17368d06692c56ad4750481f607be RETARGET
# (UFUNCTION — this slot is UPROPERTY).
# Clone #1300 SHA c4679feb43f4b97d3a814c088e3b6c344e15831f is Category
# Theater wrapping `FName WeatherIdentity` — RETARGET hard.
# wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Spawning"
# (NO spaces around =).
# LOCKED_DECL is `int32 HeavyLaneModulo = 4;`
# NOT leftover analog `int32 MaxReleasesPerBay = 6;`
# and NOT leftover analog `float MinimumCivilianSeparationMeters = 550.f;`
# and NOT leftover analog `int32 PlacementSeed = 5201;`
# and NOT leftover analog `void SetSpawningEnabled(bool bEnabled);`
# and NOT leftover analog `int32 MaxActiveDrones = 12;`
# and NOT leftover analog `int32 PoolCapacity = 12;`
# and NOT leftover analog `float SpawnInterval = 1.85f;`
# and NOT leftover analog `bool bSpawningEnabled = true;`
# and NOT leftover analog `bool bMixedThreatRoster = true;`
# and NOT leftover analog LightSpeedMin / LightSpeedMax /
# HeavyCruiseSpeed / CountActiveDrones.
# Fail-closed if this test still asserts FName WeatherIdentity /
# Category="Skyguard|Theater" / 160.f / Boom /
# VegetationStartCullDistanceCm / EngineCruiseLoop / FireRate /
# RecoilPitch / MaxReleasesPerBay / PlacementSeed / SetSpawningEnabled
# AS THE LOCKED DECL.
# Harbor leftover is only the split 40 / 80 float tokens.
# Default 4 is not Harbor.
...
HEADER_PATH = "Source/Skyguard52/SkyguardDroneSpawner.h"
CLASS_NAME = "ASkyguardDroneSpawner"
TARGET = "int32 HeavyLaneModulo = 4;"
TARGET_WRONG_BARE = "int32 HeavyLaneModulo;"
TARGET_WRONG_INIT = "int32 HeavyLaneModulo = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "int32 HeavyLaneModulo = true;"
TARGET_WRONG_ZERO = "int32 HeavyLaneModulo = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> HeavyLaneModulo;"
...
TARGET_WRONG_CLONE_SUBSTEPS = "int32 MaxSimulationSubsteps = 8;"
TARGET_WRONG_MAX_ACTIVE_THREATS = "int32 MaxActiveThreats = 24;"
TARGET_WRONG_POOL_CAPACITY = "int32 PoolCapacity = 12;"
TARGET_WRONG_MAX_ACTIVE_DRONES = "int32 MaxActiveDrones = 12;"
TARGET_WRONG_SPAWN_INTERVAL = "float SpawnInterval = 1.85f;"
TARGET_WRONG_SPAWNING_ENABLED = "bool bSpawningEnabled = true;"
TARGET_WRONG_SET_SPAWNING = "void SetSpawningEnabled(bool bEnabled);"
TARGET_WRONG_MIXED_ROSTER = "bool bMixedThreatRoster = true;"
TARGET_WRONG_LIGHT_SPEED_MIN = "float LightSpeedMin = 1300.f;"
TARGET_WRONG_LIGHT_SPEED_MAX = "float LightSpeedMax = 1750.f;"
TARGET_WRONG_HEAVY_CRUISE = "float HeavyCruiseSpeed = 1150.f;"
TARGET_WRONG_COUNT_ACTIVE = "int32 CountActiveDrones() const;"
TARGET_WRONG_PLACEMENT_SEED_ORIGIN = "int32 PlacementSeed = 5201;"
TARGET_WRONG_CIVILIAN_SEP = "float MinimumCivilianSeparationMeters = 550.f;"
TARGET_WRONG_MAX_RELEASES_CLONE = "int32 MaxReleasesPerBay = 6;"
LEFTOVER_SPAWNING_BARE_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard")'
)
LEFTOVER_SET_SPAWNING_WRAP = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Spawning")'
)
TARGET_WRONG_MAX_ACTIVE_DECOYS = "int32 MaxActiveDecoys = 12;"
TARGET_WRONG_MAX_SIMULTANEOUS = "int32 MaxSimultaneousExplosions = 6;"
TARGET_WRONG_POOL_BUDGET = "FSkyguardMission09PoolBudget PoolBudget;"
TARGET_WRONG_MIN_SEP = "float MinimumWeaponSeparationMeters = 450.f;"
SIBLING_DISPENSER_PORT = "DispenserPort"
SIBLING_DEBRIS_PORT_WING = "DebrisPortWing"
SIBLING_DEBRIS_CENTER_RACK = "DebrisCenterRack"
SIBLING_DEBRIS_STARBOARD_WING = "DebrisStarboardWing"
SIBLING_OPEN_DISPENSER_BAY = "SetSpawningEnabled"
SIBLING_RELEASE_POOLED_ESCORT = "LightSpeedMin"
SIBLING_ISSUE_CLIMB = "bSpawningEnabled"
SIBLING_ISSUE_CROSS = "IssueCrossCommand"
SIBLING_OPEN_UPPER_ENGINE = "OpenUpperEngineExposure"
SIBLING_GET_DESTROYED_ENGINE = "HeavyCruiseSpeed"
SIBLING_GET_RELEASED_ESCORT = "GetReleasedEscortCount"
SIBLING_BAY_RELEASE_COUNTS = "CountActiveDrones"
SIBLING_HANDLE_WEAK = "MaxActiveDrones"
SIBLING_POOL_BUDGET = "PoolBudget"
SIBLING_MAX_ACTIVE_THREATS = "MaxActiveThreats"
SIBLING_MAX_ACTIVE_DECOYS = "MaxActiveDecoys"
SIBLING_MAX_SIMULTANEOUS = "MaxSimultaneousExplosions"
SIBLING_POOL_CAPACITY = "PoolCapacity"
SIBLING_FLARE_COUNT = "FlareCount"
SIBLING_CLONE_SUBSTEPS = "MaxSimulationSubsteps"
SIBLING_DECOY_CONTROLLER = "DecoyController"
SIBLING_ENGINE_POD_PORT = "EnginePodPort"
SIBLING_FUEL_CONTROL_PORT = "FuelControlPort"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> HeavyLaneModulo;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> HeavyLaneModulo;"
...
UPROPERTY_LEFTOVER_CLONE_SAFETY = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Safety", '
    'meta=(ClampMin="1", ClampMax="16"))'
)
UPROPERTY_LEFTOVER_MISSION09_BOSS = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Mission09|Boss")'
)
UPROPERTY_LEFTOVER_DESTRUCTION = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Boss|Destruction")'
)
UPROPERTY_LEFTOVER_ENCOUNTER = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
...
STOP_BEFORE_SKYLINE_STYLE = "bMixedThreatRoster"
...
THIS_SCRIPT = (
    "Scripts/tests/test_drone_spawner_heavy_lane_modulo"
...
TARGET_WRONG_LATERAL = "float MaxLateralOffsetCm = 3600.f;"
TARGET_WRONG_SIM_STEP = "float MaxSimulationStepSeconds = 0.05f;"
TARGET_WRONG_MAX_HEIGHT = "float MaxHeightFromOriginCm = 1200.f;"
TARGET_WRONG_FLOAT_SUBSTEPS = "float MaxSimulationSubsteps = 8;"
TARGET_WRONG_TELEGRAPHS = "int32 MaxTelegraphsPerEncounter = 16;"
TARGET_WRONG_EPIC_TREE = "int32 EpicTreeBudget = 240;"
TARGET_WRONG_WIND = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;"
TARGET_WRONG_APPROACH = "float ApproachSpeedCmPerSecond = 1250.f;"
TARGET_WRONG_LOCK_SPEED = "float LockWindowSpeedCmPerSecond = 850.f;"
TARGET_WRONG_CRITICAL_SPEED = "float CriticalSpeedCmPerSecond = 700.f;"
TARGET_WRONG_WAVELENGTH = "float IngressSwayWavelengthCm = 9000.f;"
TARGET_WRONG_ROUTE_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Route", '
    'meta=(ClampMin="100.0"))'
)
TARGET_WRONG_COMMANDS = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Commands")'
)
TARGET_WRONG_TELEGRAPH = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Telegraph")'
)
TARGET_WRONG_ROUTE_LEN = "float RouteLengthCm = 45000.f;"
TARGET_WRONG_CLIMB_SEC = "float LockWindowClimbSeconds = 2.4f;"
TARGET_WRONG_FIRE_HELM = "float HelmetLockSeconds = 2.4f;"
TARGET_WRONG_CULL = "float VegetationStartCullDistanceCm;"
TARGET_WRONG_DEVELOPMENT = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Audio|Development")'
)
SIBLING_APPROACH_SPEED = "ApproachSpeedCmPerSecond"
SIBLING_CRITICAL_SPEED = "CriticalSpeedCmPerSecond"
SIBLING_LOCK_WINDOW_CLIMB_CM = "LockWindowClimbCm"
SIBLING_LOCK_WINDOW_CLIMB_SECONDS = "LockWindowClimbSeconds"
SIBLING_INGRESS_SWAY_AMPLITUDE = "IngressSwayAmplitudeCm"
SIBLING_INGRESS_SWAY_WAVELENGTH = "IngressSwayWavelengthCm"
SIBLING_CRITICAL_TURN_RADIUS = "CriticalTurnRadiusCm"
SIBLING_CRITICAL_TURN_SECONDS = "CriticalTurnSeconds"
SIBLING_ADVANCE_ENCOUNTER = "AdvanceEncounter"
SIBLING_RESET_ENCOUNTER = "ResetEncounterState"
SIBLING_IS_ROUTE_STATE_SAFE = "IsRouteStateSafe"
SIBLING_GET_ROUTE_PROGRESS = "GetRouteProgress"
SIBLING_GET_EFFECTIVE_SPEED = "GetEffectiveSpeedMultiplier"
SIBLING_IS_ATTACK_TELEGRAPH = "IsAttackTelegraphActive"
SIBLING_GET_TELEGRAPHS = "GetTelegraphsTriggered"
SIBLING_PATHFINDER = "Pathfinder"
SIBLING_AUTO_ADVANCE = "bAutoAdvance"
SIBLING_MIN_HEIGHT = "MinHeightFromOriginCm"
SIBLING_LOCK_WINDOW_SPEED = "LockWindowSpeedCmPerSecond"
SIBLING_MAX_HEIGHT = "MaxHeightFromOriginCm"
SIBLING_MAX_SIM_STEP = "MaxSimulationStepSeconds"
SIBLING_MAX_LATERAL_OFFSET = "MaxLateralOffsetCm"
SIBLING_COMMAND_LATERAL = "CommandLateralBiasCm"
SIBLING_BREAK_CLIMB = "BreakClimbCm"
SIBLING_APPROACH_ATTACK = "ApproachAttackIntervalSeconds"
SIBLING_LOCK_WINDOW_ATTACK = "LockWindowAttackIntervalSeconds"
SIBLING_CRITICAL_ATTACK = "CriticalAttackIntervalSeconds"
SIBLING_TELEGRAPH_LEAD = "AttackTelegraphLeadSeconds"
SIBLING_MAX_TELEGRAPHS = "MaxTelegraphsPerEncounter"
...
LOCKED = {
    "SkyguardDroneSpawner.h",
    "SkyguardDroneSpawner.cpp",
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        LEFTOVER_PROCEDURAL_BANK_BYTE_BUDGET,
        LEFTOVER_PROCEDURAL_BANK_AUDITION,
        f"{prefix}test_audio_director_open_cockpit_wind_loop"
        "_field_decl_contract.py",
        f"{prefix}test_pathfinder_advance_encounter_decl_contract.py",
        f"{prefix}test_pathfinder_reset_encounter_state_decl_contract.py",
        f"{prefix}test_pathfinder_is_route_state_safe_decl_contract.py",
        f"{prefix}test_pathfinder_get_route_progress_decl_contract.py",
        f"{prefix}test_pathfinder_get_effective_speed_multiplier"
        "_decl_contract.py",
        f"{prefix}test_pathfinder_is_attack_telegraph_active"
        "_decl_contract.py",
        f"{prefix}test_pathfinder_get_telegraphs_triggered"
        "_decl_contract.py",
        f"{prefix}test_pathfinder_debris_center_field_decl_contract.py",
        f"{prefix}test_pathfinder_debris_nose_field_decl_contract.py",
        f"{prefix}test_pathfinder_debris_tail_field_decl_contract.py",
        f"{prefix}test_pathfinder_debris_spine_field_decl_contract.py",
        f"{prefix}test_pathfinder_command_antenna_field_decl_contract.py",
        f"{prefix}test_pathfinder_engine_field_decl_contract.py",
        f"{prefix}test_pathfinder_control_linkage_field_decl_contract.py",
        f"{prefix}test_pathfinder_nose_camera_field_decl_contract.py",
        f"{prefix}test_audio_director_get_resolved_production_loop"
        "_route_count_decl_contract.py",
        f"{prefix}test_audio_director_are_resolved_production_loop"
        "_routes_complete_decl_contract.py",
        f"{prefix}test_iron_rain_max_releases_per_bay"
        "_field_decl_contract.py",
        f"{prefix}test_last_flight_minimum_civilian_separation"
        "_field_decl_contract.py",
        f"{prefix}test_coastal_placement_seed"
        "_field_decl_contract.py",
        f"{prefix}test_drone_spawner_set_spawning_enabled"
        "_decl_contract.py",
        f"{prefix}test_drone_spawner_empty_fail_closed.py",
        f"{prefix}test_drone_empty_fail_closed.py",
        f"{prefix}test_theater_kit_spec_weather_identity"
        "_field_decl_contract.py",
...
HEAVY_LANE_MODULO_DECL_RE = re.compile(
    r"int32\s+HeavyLaneModulo\s*=\s*4\s*;"
)
HEAVY_LANE_MODULO_BARE_RE = re.compile(
    r"int32\s+HeavyLaneModulo\s*;"
)

LEFTOVER_MAX_ACTIVE_DRONES_DECL_RE = re.compile(
    r"int32\s+MaxActiveDrones\s*=\s*12\b"
)
LEFTOVER_SPAWN_INTERVAL_DECL_RE = re.compile(
    r"float\s+SpawnInterval\s*=\s*1\.85f\b"
)
LEFTOVER_SPAWNING_ENABLED_DECL_RE = re.compile(
    r"bool\s+bSpawningEnabled\s*=\s*true\b"
)
LEFTOVER_SET_SPAWNING_DECL_RE = re.compile(
    r"void\s+SetSpawningEnabled\s*\(\s*bool\s+bEnabled\s*\)\s*;"
)
LEFTOVER_MIXED_ROSTER_DECL_RE = re.compile(
    r"bool\s+bMixedThreatRoster\b"
)
LEFTOVER_LIGHT_SPEED_MIN_DECL_RE = re.compile(
    r"float\s+LightSpeedMin\b"
)
LEFTOVER_LIGHT_SPEED_MAX_DECL_RE = re.compile(
    r"float\s+LightSpeedMax\b"
)
LEFTOVER_HEAVY_CRUISE_DECL_RE = re.compile(
    r"float\s+HeavyCruiseSpeed\b"
)
LEFTOVER_COUNT_ACTIVE_DECL_RE = re.compile(
    r"CountActiveDrones\b"
)
LEFTOVER_MAX_RELEASES_CLONE_DECL_RE = re.compile(
    r"int32\s+MaxReleasesPerBay\s*=\s*6\b"
)
LEFTOVER_CIVILIAN_SEP_DECL_RE = re.compile(
    r"float\s+MinimumCivilianSeparationMeters\s*=\s*550\.f\b"
)
LEFTOVER_CLONE_SUBSTEPS_DECL_RE = re.compile(
    r"int32\s+MaxSimulationSubsteps\s*=\s*8\s*;"
)
LEFTOVER_MAX_SIMULTANEOUS_DECL_RE = re.compile(
    r"int32\s+MaxSimultaneousExplosions\s*=\s*6\b"
)
LEFTOVER_MAX_ACTIVE_THREATS_DECL_RE = re.compile(
    r"int32\s+MaxActiveThreats\s*=\s*24\b"
)
LEFTOVER_POOL_CAPACITY_DECL_RE = re.compile(
    r"int32\s+PoolCapacity\s*=\s*12\b"
)
LEFTOVER_MAX_ACTIVE_DECOYS_DECL_RE = re.compile(
    r"int32\s+MaxActiveDecoys\s*=\s*12\b"
)
LEFTOVER_MIN_SEP_DECL_RE = re.compile(
    r"float\s+MinimumWeaponSeparationMeters\s*=\s*450\.f\b"
)
LEFTOVER_MAX_LATERAL_OFFSET_DECL_RE = re.compile(
    r"float\s+MaxLateralOffsetCm\s*=\s*3600\.f\s*;"
)
LEFTOVER_SIM_STEP_DECL_RE = re.compile(
    r"float\s+MaxSimulationStepSeconds\s*=\s*0\.05f\b"
)
LEFTOVER_FLOAT_SUBSTEPS_DECL_RE = re.compile(
    r"float\s+MaxSimulationSubsteps\s*=\s*8\s*;"
)
LEFTOVER_EPIC_TREE_DECL_RE = re.compile(
    r"int32\s+EpicTreeBudget\b"
)
LEFTOVER_VOICE_LIMIT_DECL_RE = re.compile(
    r"int32\s+GlobalVoiceLimit\s*=\s*24\b"
)
LEFTOVER_TELEGRAPHS_DECL_RE = re.compile(
    r"int32\s+MaxTelegraphsPerEncounter\b"
)
LOCK_WINDOW_SPEED_DECL_RE = re.compile(
    r"float\s+LockWindowSpeedCmPerSecond\s*=\s*850\.f\b"
)
CRITICAL_SPEED_DECL_RE = re.compile(
    r"float\s+CriticalSpeedCmPerSecond\s*=\s*700\.f\b"
)
WAVELENGTH_DECL_RE = re.compile(
    r"float\s+IngressSwayWavelengthCm\s*=\s*9000\.f\b"
)
OPEN_COCKPIT_WIND_LOOP_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+OpenCockpitWindLoop\s*;"
)
APPROACH_SPEED_DECL_RE = re.compile(
    r"float\s+ApproachSpeedCmPerSecond\s*=\s*1250\.f\b"
)
ROUTE_LENGTH_DECL_RE = re.compile(
    r"float\s+RouteLengthCm\s*=\s*45000\.f\b"
)
CLIMB_SECONDS_DECL_RE = re.compile(
    r"float\s+LockWindowClimbSeconds\s*=\s*2\.4f\b"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_APPROACH_SPEED,
        SIBLING_LOCK_WINDOW_SPEED,
        SIBLING_CRITICAL_SPEED,
        SIBLING_MAX_HEIGHT,
        SIBLING_MAX_SIM_STEP,
        SIBLING_MAX_LATERAL_OFFSET,
        SIBLING_COMMAND_LATERAL,
        SIBLING_BREAK_CLIMB,
        SIBLING_APPROACH_ATTACK,
        SIBLING_LOCK_WINDOW_ATTACK,
        SIBLING_CRITICAL_ATTACK,
        SIBLING_TELEGRAPH_LEAD,
        SIBLING_MAX_TELEGRAPHS,
        SIBLING_LOCK_WINDOW_CLIMB_CM,
        SIBLING_LOCK_WINDOW_CLIMB_SECONDS,
        SIBLING_INGRESS_SWAY_AMPLITUDE,
        SIBLING_INGRESS_SWAY_WAVELENGTH,
        SIBLING_CRITICAL_TURN_RADIUS,
        SIBLING_CRITICAL_TURN_SECONDS,
        SIBLING_ADVANCE_ENCOUNTER,
        SIBLING_RESET_ENCOUNTER,
        SIBLING_IS_ROUTE_STATE_SAFE,
        SIBLING_GET_ROUTE_PROGRESS,
        SIBLING_GET_EFFECTIVE_SPEED,
        SIBLING_IS_ATTACK_TELEGRAPH,
        SIBLING_GET_TELEGRAPHS,
        SIBLING_PATHFINDER,
        SIBLING_AUTO_ADVANCE,
        SIBLING_MIN_HEIGHT,
        ...
        SIBLING_DISPENSER_PORT,
        SIBLING_DEBRIS_PORT_WING,
        SIBLING_DEBRIS_CENTER_RACK,
        SIBLING_DEBRIS_STARBOARD_WING,
        SIBLING_OPEN_DISPENSER_BAY,
        SIBLING_RELEASE_POOLED_ESCORT,
        SIBLING_ISSUE_CLIMB,
        SIBLING_ISSUE_CROSS,
        SIBLING_OPEN_UPPER_ENGINE,
        SIBLING_GET_DESTROYED_ENGINE,
        SIBLING_GET_RELEASED_ESCORT,
        SIBLING_POOL_BUDGET,
        SIBLING_MAX_ACTIVE_THREATS,
        SIBLING_MAX_ACTIVE_DECOYS,
        SIBLING_MAX_SIMULTANEOUS,
        SIBLING_POOL_CAPACITY,
        SIBLING_FLARE_COUNT,
        SIBLING_CLONE_SUBSTEPS,
        SIBLING_DECOY_CONTROLLER,
        SIBLING_ENGINE_POD_PORT,
        SIBLING_FUEL_CONTROL_PORT,
        "SetSpawningEnabled",
        "bSpawningEnabled",
        "SpawnInterval",
        "MaxActiveDrones",
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 HeavyLaneModulo = 4;`.
    # Do not accept leftover FName WeatherIdentity,
    # leftover FName AssemblyRevision, leftover TArray
    # RoutePoints, leftover float LookYawLimit = 95.f,
    # leftover Boom, leftover sibling EngineCruiseLoop /
    # EnginePowerLoop / PropellerLoop / EngineIdleLoop /
    # GlobalVoiceLimit, leftover analog OpenCockpitWindLoop,
    # leftover analog ApproachSpeedCmPerSecond = 1250.f,
    # leftover analog RouteLengthCm = 45000.f,
    # leftover analog LockWindowClimbSeconds = 2.4f,
    # leftover analog SampleRate /
    # GeneratedByteBudget / MinimumMeasuredSamples /
    # VegetationStartCullDistanceCm, leftover `= 160.f`,
    # leftover analog apache-cpg-feel FireRate / RecoilPitch,
    # leftover CPG mesh/text, leftover LoadoutSpec magazine
    # fields, leftover Harbor 40 / 80, leftover
    # bAllowAuthoredPCGGeneration /
    # bEnableCoastalHazeTransition /
    # bUseAuthoredLandscapeSurface, leftover readiness
    # bPCGGenerationAuthorized /
    # bAuthoredLandscapeSurfaceExposed /
    # bReadyForAuthoredPCGGeneration, or a bare
    # HeavyLaneModulo without the 4 initializer.
    # Fail-closed if LOCKED_DECL is still float or leftover analog
    # float MaxLateralOffsetCm = 3600.f / 0.05f / ClampMin "100.0".
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if HEAVY_LANE_MODULO_DECL_RE.search(compact) is None:
        return False
    if HEAVY_LANE_MODULO_BARE_RE.search(compact):
        return False
    if LEFTOVER_MAX_ACTIVE_DRONES_DECL_RE.search(compact):
        return False
    if LEFTOVER_SPAWN_INTERVAL_DECL_RE.search(compact):
        return False
    if LEFTOVER_SPAWNING_ENABLED_DECL_RE.search(compact):
        return False
    if LEFTOVER_SET_SPAWNING_DECL_RE.search(compact):
        return False
    if LEFTOVER_MIXED_ROSTER_DECL_RE.search(compact):
        return False
    if LEFTOVER_LIGHT_SPEED_MIN_DECL_RE.search(compact):
        return False
    if LEFTOVER_LIGHT_SPEED_MAX_DECL_RE.search(compact):
        return False
    if LEFTOVER_HEAVY_CRUISE_DECL_RE.search(compact):
        return False
    if LEFTOVER_COUNT_ACTIVE_DECL_RE.search(compact):
        return False
    if LEFTOVER_MAX_RELEASES_CLONE_DECL_RE.search(compact):
        return False
    if LEFTOVER_CIVILIAN_SEP_DECL_RE.search(compact):
        return False
    if LEFTOVER_CLONE_SUBSTEPS_DECL_RE.search(compact):
        return False
    if LEFTOVER_MAX_SIMULTANEOUS_DECL_RE.search(compact):
        return False
    if LEFTOVER_MAX_ACTIVE_THREATS_DECL_RE.search(compact):
        return False
    if LEFTOVER_POOL_CAPACITY_DECL_RE.search(compact):
        return False
    if LEFTOVER_MAX_ACTIVE_DECOYS_DECL_RE.search(compact):
        return False
    if LEFTOVER_MIN_SEP_DECL_RE.search(compact):
        return False
    if LEFTOVER_MAX_LATERAL_OFFSET_DECL_RE.search(compact):
        return False
    if LEFTOVER_SIM_STEP_DECL_RE.search(compact):
        return False
    if LEFTOVER_FLOAT_SUBSTEPS_DECL_RE.search(compact):
        return False
    if LEFTOVER_EPIC_TREE_DECL_RE.search(compact):
        return False
    if LEFTOVER_VOICE_LIMIT_DECL_RE.search(compact):
        return False
    if LEFTOVER_TELEGRAPHS_DECL_RE.search(compact):
        return False
    if OPEN_COCKPIT_WIND_LOOP_DECL_RE.search(compact):
        return False
    if APPROACH_SPEED_DECL_RE.search(compact):
        return False
    if LOCK_WINDOW_SPEED_DECL_RE.search(compact):
        return False
    if CRITICAL_SPEED_DECL_RE.search(compact):
        return False
    if WAVELENGTH_DECL_RE.search(compact):
        return False
    if ROUTE_LENGTH_DECL_RE.search(compact):
        return False
    if CLIMB_SECONDS_DECL_RE.search(compact):
        return False
...
def public_section(header: str) -> str:
    ...
    if next_access is not None:
        ...
        section = rest[:close]
    if STOP_BEFORE_PROTECTED in section:
        raise AssertionError(
            f"{CLASS_NAME} parse window includes {STOP_BEFORE_PROTECTED}"
        )
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "HeavyLaneModulo":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+HeavyLaneModulo\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 HeavyLaneModulo "
...
class DroneSpawnerHeavyLaneModuloFieldDeclContractTests(unittest.TestCase):
    def test_drone_spawner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HeavyLaneModulo"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertNotIn(STOP_BEFORE_PROTECTED, section)
        self.assertNotEqual(CLASS_NAME, "USkyguardPathfinderEncounterController")
        self.assertNotEqual(CLASS_NAME, "ASkyguardMission09IntegrationDirector")
        self.assertNotEqual(CLASS_NAME, "ASkyguardIronRainBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardLastFlightBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardCoastalEnvironmentDirector")
        self.assertNotEqual(CLASS_NAME, "USkyguardRadioChatterComponent")
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardPathfinderEncounterController.h",
        )
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardMission09IntegrationDirector.h",
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_OPEN_DISPENSER_BAY), body)
        self.assertFalse(has_identifier(section, SIBLING_APPROACH_SPEED), section)
        self.assertTrue(has_identifier(body, SIBLING_BAY_RELEASE_COUNTS), body)
        self.assertFalse(has_identifier(section, SIBLING_ROUTE_LENGTH_CM), section)
        self.assertTrue(has_identifier(body, SIBLING_HANDLE_WEAK), body)
        self.assertFalse(
            has_identifier(section, SIBLING_EVENT_DEFINITIONS),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_RELEASE_POOLED_ESCORT), body)
        self.assertFalse(has_identifier(section, SIBLING_ADVANCE_ENCOUNTER), section)
        self.assertTrue(has_identifier(body, SIBLING_ISSUE_CLIMB), body)
        self.assertFalse(has_identifier(section, SIBLING_RESET_ENCOUNTER), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HeavyLaneModulo"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("HeavyLaneModulo", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_heavy_lane_modulo_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("EditDefaultsOnly", section)
        self.assertNotIn("BlueprintReadOnly", section)
        self.assertIn('Category="Skyguard|Spawning"', section)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', section)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', section)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', section)
        ...
        self.assertNotIn("EditDefaultsOnly", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_ROUTE_WRAP)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_SPEED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CRITICAL_SPEED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WAVELENGTH)
        ...
        self.assertNotIn("EditDefaultsOnly", specifiers)
        self.assertNotIn("BlueprintReadOnly", specifiers)
        self.assertIn('Category="Skyguard|Spawning"', specifiers)
        self.assertNotIn("EditInstanceOnly", specifiers)
        ...
        self.assertNotIn('ClampMin="1"', specifiers)
        self.assertNotIn('ClampMin="100.0"', specifiers)
        self.assertNotIn('ClampMin="1.0"', specifiers)
        self.assertNotIn("ClampMax", specifiers)
        self.assertNotIn('ClampMax="24"', specifiers)
        self.assertNotIn('ClampMax="16"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        bare = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn("float MaxSimulationSubsteps = 8;", compact_origin)
        self.assertNotIn("int32 MaxSimulationSubsteps = 8;", compact_origin)
        self.assertNotIn(
            "int32 HeavyLaneModulo;",
            compact_origin.split("HeavyLaneModulo = 4;")[0]
            + compact_origin.split("HeavyLaneModulo = 4;")[-1]
            if "HeavyLaneModulo = 4;" in compact_origin
            else compact_origin,
        )
        self.assertNotIn("float ApproachSpeedCmPerSecond = 1250.f;", LOCKED_DECL)
    ...
    def test_heavy_lane_modulo_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 HeavyLaneModulo"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 4", LOCKED_DECL)
        self.assertNotIn("= 6", LOCKED_DECL)
        self.assertNotIn("= 8", LOCKED_DECL)
        self.assertNotIn("= 12", LOCKED_DECL)
        self.assertNotIn("3600.f", LOCKED_DECL)
        self.assertNotIn("0.05f", LOCKED_DECL)
        self.assertNotIn("850.f", LOCKED_DECL)
        self.assertNotIn("700.f", LOCKED_DECL)
        self.assertNotIn("1250.f", LOCKED_DECL)
        self.assertNotIn("9000.f", LOCKED_DECL)
        ...
        self.assertNotIn("1250.f", LOCKED_DECL)
        self.assertNotIn("45000.f", LOCKED_DECL)
        self.assertNotIn("2.4f", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIM_STEP)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FLOAT_SUBSTEPS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_SUBSTEPS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FLARE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_SIMULTANEOUS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_ACTIVE_THREATS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_POOL_CAPACITY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_ACTIVE_DRONES)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPAWN_INTERVAL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPAWNING_ENABLED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SET_SPAWNING)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MIXED_ROSTER)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_RELEASES_CLONE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CIVILIAN_SEP)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PLACEMENT_SEED_ORIGIN)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, LEFTOVER_SPAWNING_BARE_WRAP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, LEFTOVER_SET_SPAWNING_WRAP)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_ACTIVE_DECOYS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_POOL_BUDGET)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MIN_SEP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, UPROPERTY_LEFTOVER_CLONE_SAFETY)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, UPROPERTY_LEFTOVER_MISSION09_BOSS)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, UPROPERTY_LEFTOVER_DESTRUCTION)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, UPROPERTY_LEFTOVER_ENCOUNTER)
        self.assertNotIn('Category="Skyguard|Mission09|Boss"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('Category="Skyguard|Boss|Destruction"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("FlareCount", LOCKED_DECL)
        self.assertNotIn("MaxSimultaneousExplosions", LOCKED_DECL)
        self.assertNotIn("MaxActiveThreats", LOCKED_DECL)
        self.assertNotIn("PoolBudget", LOCKED_DECL)
        self.assertNotIn("MaxSimulationSubsteps", LOCKED_DECL)
        self.assertNotIn("OpenDispenserBay", LOCKED_DECL)
        self.assertNotIn("ReleasePooledEscort", LOCKED_DECL)
        self.assertNotIn("DispenserPort", LOCKED_DECL)
        self.assertNotIn("DebrisPortWing", LOCKED_DECL)

        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TELEGRAPHS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_EPIC_TREE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VOICE)
        self.assertNotIn("float ", LOCKED_DECL)
        self.assertNotIn("3600.f", LOCKED_DECL)
        self.assertNotIn("0.05f", LOCKED_DECL)
        self.assertNotIn('ClampMin="100.0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="1.0"', UPROPERTY_EDIT_READWRITE)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLIMB_SEC)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VOICE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_YAW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MISSION_ID)
        ...
        self.assertIsNone(
            re.search(
                r'Category="Skyguard\|Boss\|Encounter"(?!\|)',
                UPROPERTY_EDIT_READWRITE,
            )
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 HeavyLaneModulos = 8;\n",
            "\tfloat MaxSimulationSubsteps = 8;\n",
            "\tFName HeavyLaneModulo = NAME_None;\n",
            "\tint32 HeavyLaneModulo = " + forty + ";\n",
            "\tint32 HeavyLaneModulo = " + eighty + ";\n",
            f"\t{TARGET_WRONG_LATERAL}\n",
            f"\t{TARGET_WRONG_SIM_STEP}\n",
            f"\t{TARGET_WRONG_FLOAT_SUBSTEPS}\n",
            f"\t{TARGET_WRONG_TELEGRAPHS}\n",
            f"\t{TARGET_WRONG_EPIC_TREE}\n",
            f"\t{TARGET_WRONG_WIND}\n",
            f"\t{TARGET_WRONG_APPROACH}\n",
            f"\t{TARGET_WRONG_LOCK_SPEED}\n",
            f"\t{TARGET_WRONG_CRITICAL_SPEED}\n",
            f"\t{TARGET_WRONG_WAVELENGTH}\n",
            f"\t{TARGET_WRONG_ROUTE_LEN}\n",
            f"\t{TARGET_WRONG_CLIMB_SEC}\n",
            f"\t{TARGET_WRONG_BOOM}\n",
            ...
            f"\t{TARGET_WRONG_CLONE_SUBSTEPS}\n",
            f"\t{TARGET_WRONG_MAX_ACTIVE_THREATS}\n",
            f"\t{TARGET_WRONG_POOL_CAPACITY}\n",
            f"\t{TARGET_WRONG_MAX_ACTIVE_DECOYS}\n",
            f"\t{TARGET_WRONG_MAX_SIMULTANEOUS}\n",
            f"\t{TARGET_WRONG_POOL_BUDGET}\n",
            f"\t{TARGET_WRONG_MIN_SEP}\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Spawning"', origin)
        ...
        self.assertNotIn("EditDefaultsOnly", origin)
        ...
        self.assertNotIn("BlueprintReadWrite", dropped)
        self.assertNotIn("EditDefaultsOnly", dropped)
        self.assertIn("VisibleAnywhere", dropped)
        self.assertIn("BlueprintReadOnly", dropped)
        self.assertIn("EditAnywhere", origin)
        self.assertNotIn("VisibleAnywhere", origin)
        self.assertNotIn("EditDefaultsOnly", origin)
        self.assertNotIn("EditInstanceOnly", origin)
        ...
        self.assertNotIn('Category="Skyguard|Spawning"', apache_specs)
        self.assertIn('EditAnywhere', apache_specs)
        self.assertNotIn("EditDefaultsOnly", apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Spawning"', theater_specs)
        self.assertIn('Category="Skyguard|Spawning"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Spawning"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Spawning"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Spawning"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Spawning"', development_specs)

        leftover_safety = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_LEFTOVER_CLONE_SAFETY}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Safety"', safety_specs)
        self.assertNotIn('Category="Skyguard|Spawning"', safety_specs)
        self.assertIn("EditAnywhere", safety_specs)
        self.assertIn("BlueprintReadWrite", safety_specs)
        leftover_boss = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_LEFTOVER_MISSION09_BOSS}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        ...
        self.assertIn('Category="Skyguard|Mission09|Boss"', boss_specs)
        self.assertNotIn('Category="Skyguard|Spawning"', boss_specs)
        self.assertIn("VisibleAnywhere", boss_specs)
        leftover_destruction = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_LEFTOVER_DESTRUCTION}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        destruction_specs = attached_uproperty_specifiers(
            spec_section(leftover_destruction)
        )
        self.assertIn('Category="Skyguard|Boss|Destruction"', destruction_specs)
        self.assertNotIn('Category="Skyguard|Spawning"', destruction_specs)
        leftover_write = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        write_specs = attached_uproperty_specifiers(
            spec_section(leftover_write)
        )
        self.assertIn("EditAnywhere", write_specs)
        self.assertIn("BlueprintReadWrite", write_specs)
        self.assertIn('Category="Skyguard"', write_specs)
        self.assertNotIn('Category="Skyguard|Spawning"', write_specs)
        self.assertIn("EditAnywhere", origin)
        self.assertIn("BlueprintReadWrite", origin)
        self.assertNotIn("EditDefaultsOnly", origin)
        self.assertNotIn("BlueprintReadOnly", origin)
        ...
        clamp16_specs = attached_uproperty_specifiers(
            spec_section(leftover_clamp16)
        )
        self.assertIn('ClampMax="16"', clamp16_specs)
        self.assertNotIn('ClampMax="24"', clamp16_specs)
        self.assertNotIn('ClampMax="24"', origin)
        self.assertNotIn('ClampMax="16"', origin)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertIn("EditInstanceOnly", instance_specs)
        self.assertNotIn("EditAnywhere", instance_specs)
        self.assertNotIn("EditDefaultsOnly", instance_specs)
        self.assertIn("EditAnywhere", origin)
        self.assertNotIn("EditDefaultsOnly", origin)
        self.assertNotIn("EditInstanceOnly", origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', origin)
        self.assertNotIn('Category="Skyguard|Mission09|Boss"', origin)
        self.assertNotIn('Category="Skyguard|Boss|Destruction"', origin)
        self.assertNotIn('Category="Skyguard|Boss|Encounter"', origin)
        self.assertNotEqual(origin, UPROPERTY_LEFTOVER_CLONE_SAFETY)
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "HeavyLaneModulo"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_OPEN_DISPENSER_BAY))
        self.assertFalse(has_identifier(section, SIBLING_APPROACH_SPEED))
        self.assertTrue(has_identifier(leaked, SIBLING_ISSUE_CLIMB))
        self.assertFalse(has_identifier(section, SIBLING_RESET_ENCOUNTER))
        self.assertTrue(has_identifier(leaked, SIBLING_HANDLE_WEAK))
        self.assertFalse(has_identifier(section, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertNotIn(STOP_BEFORE_PROTECTED, section)
        self.assertTrue(has_identifier(leaked, "HeavyLaneModulo"))
        self.assertTrue(has_identifier(section, "HeavyLaneModulo"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_OPEN_DISPENSER_BAY))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_HANDLE_WEAK))
        self.assertFalse(has_identifier(section, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
        self.assertTrue(has_identifier(leaked, SIBLING_RELEASE_POOLED_ESCORT))
        self.assertFalse(has_identifier(section, SIBLING_ADVANCE_ENCOUNTER))
        self.assertTrue(has_identifier(leaked, SIBLING_HANDLE_WEAK))
        self.assertFalse(has_identifier(section, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_DESTROYED_ENGINE))
        self.assertFalse(has_identifier(section, SIBLING_PATHFINDER))
        self.assertFalse(has_identifier(section, SIBLING_MAX_LATERAL_OFFSET))
        self.assertFalse(has_identifier(section, SIBLING_MIN_HEIGHT))
        self.assertFalse(has_identifier(section, SIBLING_MAX_HEIGHT))
        self.assertFalse(has_identifier(section, SIBLING_MAX_SIM_STEP))
        self.assertFalse(has_identifier(section, SIBLING_MAX_TELEGRAPHS))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn('ClampMin="1"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="100.0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="1.0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("ClampMax", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMax="24"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMax="16"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMax="64"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMax="1024"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardDroneSpawner.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardDroneSpawner.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "drone spawner heavy lane modulo field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "drone_spawner_heavy_lane_modulo_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("MaxLateralOffsetCm", locked_only)
        self.assertNotIn("MaxHeightFromOriginCm", locked_only)
        self.assertNotIn("MaxSimulationStepSeconds", locked_only)
        self.assertNotIn("MinHeightFromOriginCm", locked_only)
        self.assertNotIn("EpicTreeBudget", locked_only)
        self.assertNotIn("MaxTelegraphsPerEncounter", locked_only)
        self.assertNotIn("MaxSimulationSubsteps", locked_only)
        self.assertNotIn("FlareCount", locked_only)
        self.assertNotIn("MaxSimultaneousExplosions", locked_only)
        self.assertNotIn("MaxActiveThreats", locked_only)
        self.assertNotIn("MaxActiveDecoys", locked_only)
        self.assertNotIn("PoolCapacity", locked_only)
        self.assertNotIn("PoolBudget", locked_only)
        ...
        self.assertNotIn("GetDestroyedEngineCount", locked_only)
        ...
        self.assertNotIn("DebrisCenterRack", locked_only)
        self.assertNotIn("DebrisStarboardWing", locked_only)
        self.assertNotIn("DecoyController", locked_only)
        self.assertNotIn("MinimumWeaponSeparationMeters", locked_only)
        self.assertNotIn("SetSpawningEnabled", locked_only)
        self.assertNotIn("bSpawningEnabled", locked_only)
        self.assertNotIn("SpawnInterval", locked_only)
        self.assertNotIn("MaxActiveDrones", locked_only)
        self.assertNotIn("bMixedThreatRoster", locked_only)
        self.assertNotIn("LightSpeedMin", locked_only)
        self.assertNotIn("LightSpeedMax", locked_only)
        self.assertNotIn("HeavyCruiseSpeed", locked_only)
        self.assertNotIn("CountActiveDrones", locked_only)
        self.assertNotIn("MaxReleasesPerBay", locked_only)
        self.assertNotIn("MinimumCivilianSeparationMeters", locked_only)
        self.assertNotIn("PlacementSeed", locked_only)
        self.assertNotIn("OpenCockpitWindLoop", locked_only)
        self.assertNotIn("ApproachSpeedCmPerSecond", locked_only)
        self.assertNotIn("RouteLengthCm", locked_only)
        self.assertNotIn("LockWindowClimbSeconds", locked_only)
