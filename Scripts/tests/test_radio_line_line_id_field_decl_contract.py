# THIS IS leftover-safe FSkyguardRadioLine LineId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `FName LineId;` (no initializer).
...
# Do NOT contract sibling fields Speaker / Subtitle /
...
TARGET = "FName LineId;"
TARGET_WRONG_EQ_NONE = "FName LineId = NAME_None;"
TARGET_WRONG_FALSE = "FName LineId = false;"
TARGET_WRONG_TRUE = "FName LineId = true;"
TARGET_WRONG_ZERO = "FName LineId = 0.f;"
TARGET_WRONG_FLOAT = "float LineId;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName LineId;`.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+LineId\b", compact[index:]
        ):
...
def require_line_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} LineId UPROPERTY"
        )
    if "BlueprintReadOnly" in specifiers:
        raise AssertionError(
            "BlueprintReadOnly instead of BlueprintReadWrite on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} LineId"
        )
...
class RadioLineLineIdFieldDeclContractTests(unittest.TestCase):
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_line_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName LineId;", compact_origin)
        self.assertNotIn("LineId = NAME_None", compact_origin)
        self.assertNotIn("LineId = false", compact_origin)
        self.assertNotIn("LineId = true", compact_origin)
        self.assertNotIn("LineId = 0.f", compact_origin)
        self.assertNotIn("LineId = 160.f", compact_origin)
    ...
    def test_dropped_edit_anywhere_or_read_only_fails_closed(self) -> None:
        origin = require_line_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_line_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_line_id_specifiers(spec_section(read_only))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_line_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio-line LineId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_line_line_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardRadioChatterComponent.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
