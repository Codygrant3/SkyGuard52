# THIS IS leftover-safe FSkyguardCpgHudSnapshot RangeLine.
...
# `FString RangeLine;` with NO in-struct initializer.
# Fail-closed if RangeLine is missing or renamed, if the
...
# RangeLine is NOT leftover sibling RangeMeters
# (`float RangeMeters = -1.f`).
# There is NO leftover analog Python bulk for this HUD
# struct. Isolated field decl does not invent one.
# Do NOT contract sibling fields WeaponLine / ThreatLine /
# EufdLine / LockLine / SightLine / StationStatus /
...
# retarget: type is FString, identifier is RangeLine,
...
TARGET = "FString RangeLine;"
TARGET_WRONG_EQ_EMPTY = 'FString RangeLine = TEXT("");'
TARGET_WRONG_EQ_NONE = "FString RangeLine = NAME_None;"
TARGET_WRONG_FALSE = "FString RangeLine = false;"
TARGET_WRONG_TRUE = "FString RangeLine = true;"
TARGET_WRONG_ZERO = "FString RangeLine = 0.f;"
TARGET_WRONG_FLOAT = "float RangeLine;"
TARGET_WRONG_HEALTH = "float RangeLine = 160.f;"
TARGET_WRONG_BOOL = "bool RangeLine;"
TARGET_WRONG_INT = "int32 RangeLine;"
TARGET_WRONG_FNAME = "FName RangeLine;"
TARGET_WRONG_TCHAR = 'const TCHAR* RangeLine = TEXT("");'
...
RANGE_LINE_BARE_RE = re.compile(r"FString\s+RangeLine\s*;")
RANGE_LINE_INIT_RE = re.compile(r"FString\s+RangeLine\s*=")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def range_line_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*FString\s+RangeLine\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString RangeLine;`.
    ...
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if RANGE_LINE_INIT_RE.search(compact):
        return False
    if re.search(r"float\s+RangeLine\b", compact):
        return False
    if re.search(r"bool\s+RangeLine\b", compact):
        return False
    if re.search(r"int32\s+RangeLine\b", compact):
        return False
    if re.search(r"FName\s+RangeLine\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*RangeLine\b", compact):
        return False
    if re.search(r"float\s+RangeMeters\b", compact) and (
        RANGE_LINE_BARE_RE.search(compact) is None
    ):
        return False
    if RANGE_LINE_BARE_RE.search(compact) is None:
        return False
    if range_line_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString RangeLine\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString RangeLine is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FString RangeLine\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FString RangeLine is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "RangeLine"):
        raise AssertionError(
            "UPROPERTY clone landed on RangeLine; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "RangeLine"):
            raise AssertionError(
                f"{token} clone landed on RangeLine; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotRangeLineFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RangeLine"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString RangeLine;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("FString RangeLine"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("=", LOCKED_DECL)
        self.assertIn("RangeLine", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("RangeLine", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RangeLine"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_range_line_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_weapon_line_does_not_satisfy_range_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEAPON}\n"
    ...
    def test_leftover_range_meters_does_not_satisfy_range_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString RangeLine;", compact_origin)
        self.assertNotIn("RangeLine = TEXT", compact_origin)
        self.assertNotIn("RangeLine = NAME_None", compact_origin)
        self.assertNotIn("RangeLine = false", compact_origin)
        self.assertNotIn("RangeLine = true", compact_origin)
        self.assertNotIn("RangeLine = 0.f", compact_origin)
        self.assertNotIn("RangeLine = 160.f", compact_origin)

    def test_range_line_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString RangeLine"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 RangeLine;\n",
            "\tbool RangeLine;\n",
            "\tfloat RangeLine = " + forty + ";\n",
            "\tfloat RangeLine = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    range_line_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(range_line_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot RangeLine field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "range_line_field_decl_contract.py"
        ))
