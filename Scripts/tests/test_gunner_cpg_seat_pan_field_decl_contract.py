# THIS IS leftover-safe ASkyguardGunner CpgSeatPan.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgSeatPan;`
# (no initializer).
...
# Stop BEFORE sibling CpgKneeLeft as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgKneeLeft, CpgKneeRight, CpgCanopyBow, Boom,
# GunnerCamera, CpgDash.
# Do NOT claim leftover CpgCockpit / CpgStation
# (in-flight siblings).
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgSeatPan;`
...
# CpgSeatPan fail-closed if LOCKED_DECL is leftover
# CpgSeatBack / CpgCockpit / CpgStation.
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgSeatPan;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgSeatPan = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgSeatPan = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgSeatPan = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgSeatPan;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgSeatPan;"
...
STOP_BEFORE_KNEE_LEFT = "CpgKneeLeft"
...
CPG_SEAT_PAN_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgSeatPan\s*;"
)
CPG_SEAT_PAN_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgSeatPan\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgSeatBack / CpgCockpit / CpgStation, or an
    # initializer on CpgSeatPan as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_SEAT_PAN_DECL_RE.search(compact) is None:
        return False
    if CPG_SEAT_PAN_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_KNEE_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_KNEE_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_KNEE_LEFT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgSeatPan\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgSeatPan "
...
class GunnerCpgSeatPanFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgSeatPan"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_KNEE_LEFT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_KNEE_LEFT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgSeatPan"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgSeatPan", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_seat_pan_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgSeatPan;", compact_origin)
        self.assertNotIn("CpgSeatPan = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_seat_pan_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgSeatPan"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgSeatPan;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgSeatPans;\n",
            "\tint32 CpgSeatPan = 140;\n",
            "\tbool CpgSeatPan = true;\n",
            "\tfloat CpgSeatPan = " + forty + ";\n",
            "\tfloat CpgSeatPan = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_knee_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(TARGET_WRONG_KNEE_LEFT, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_KNEE_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_KNEE_LEFT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgSeatPan field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_seat_pan_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
