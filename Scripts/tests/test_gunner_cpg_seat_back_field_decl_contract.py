# THIS IS leftover-safe ASkyguardGunner CpgSeatBack.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgSeatBack;`
# (no initializer).
...
# Stop BEFORE sibling CpgSeatPan as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatPan,
# CpgKneeLeft, Boom, GunnerCamera, CpgDash, CpgTedac.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgSeatBack;`
...
# CpgSeatBack fail-closed if LOCKED_DECL is leftover
# CpgCockpit / CpgStation / CpgSeatPan.
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgSeatBack;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgSeatBack = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgSeatBack = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgSeatBack = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgSeatBack;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgSeatBack;"
...
STOP_BEFORE_SEAT_PAN = "CpgSeatPan"
...
CPG_SEAT_BACK_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgSeatBack\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgSeatPan / CpgCockpit / CpgStation, or an
    # initializer on CpgSeatBack as the slot.
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_SEAT_PAN):
            break
    ...
    if has_identifier(window, SIBLING_CPG_SEAT_PAN):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_SEAT_PAN}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgSeatBack\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgSeatBack "
...
class GunnerCpgSeatBackFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgSeatBack"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_SEAT_PAN), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_SEAT_PAN),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgSeatBack"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgSeatBack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_seat_back_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgSeatBack;", compact_origin)
        self.assertNotIn("CpgSeatBack = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_seat_back_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgSeatBack"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgSeatBack;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgSeatBacks;\n",
            "\tint32 CpgSeatBack = 140;\n",
            "\tbool CpgSeatBack = true;\n",
            "\tfloat CpgSeatBack = " + forty + ";\n",
            "\tfloat CpgSeatBack = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_seat_pan_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(TARGET_WRONG_SEAT_PAN, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgSeatBack field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_seat_back_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
