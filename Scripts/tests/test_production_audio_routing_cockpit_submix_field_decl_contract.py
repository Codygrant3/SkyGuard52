# THIS IS leftover-safe FSkyguardProductionAudioRouting CockpitSubmix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundSubmixBase> CockpitSubmix;`
# (no initializer).
...
# Sibling decls stay unclaimed: MasterSubmix / ExteriorSubmix /
# WeaponsSubmix / ExplosionsSubmix / RadioSubmix /
# CockpitSoundMix / CockpitExteriorAttenuation /
# CockpitLowPassHz.
# Claim ONLY CockpitSubmix. Do not treat CockpitSoundMix as
# this field. Do not claim the sibling float fields.
# Analog leftover production-audio-routing-defaults #197 /
# #fc92 then isolated CockpitSubmix field decl.
# THIS IS NOT leftover ProductionAudioEntry OutputSubmix #1401.
# THIS IS NOT leftover production-audio-audit-defaults #196 /
# #5e51.
# THIS IS NOT leftover ProductionAudioEntry Sound #1397.
...
# ESkyguardAudioEvent.
# Do NOT parse leftover TheaterKit actor.
# Do NOT parse leftover ProductionAudioEntry isolated decls.
...
TARGET = "TSoftObjectPtr<USoundSubmixBase> CockpitSubmix;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundSubmixBase> CockpitSubmix = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundSubmixBase> CockpitSubmix = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundSubmixBase> CockpitSubmix = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundSubmixBase> CockpitSubmix = 0.f;"
TARGET_WRONG_FLOAT = "float CockpitSubmix;"
TARGET_WRONG_FNAME = "FName CockpitSubmix;"
TARGET_WRONG_BOOL = "bool CockpitSubmix;"
TARGET_WRONG_INT = "int32 CockpitSubmix;"
TARGET_WRONG_FTEXT = "FText CockpitSubmix;"
TARGET_WRONG_PTR = "USoundSubmixBase* CockpitSubmix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> CockpitSubmix;"
TARGET_WRONG_SOUND_MIX = "TSoftObjectPtr<USoundMix> CockpitSubmix;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> CockpitSubmix;"
TARGET_WRONG_FSTRING = "FString CockpitSubmix;"
...
LEFTOVER_PRODUCTION_AUDIO_ENTRY_OUTPUT_SUBMIX = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling CockpitSoundMix / CockpitExteriorAttenuation /
    # CockpitLowPassHz / MasterSubmix fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+CockpitSubmix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundSubmixBase\s*>\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundMix\s*>\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+CockpitSubmix\b",
        compact,
    ):
        return False
    if re.search(
...
def parse_window(header: str) -> str:
    ...
    # live-copy tokens, leftover source-status enum, and leftover
    # FSkyguardProductionAudioEntry stay OUT of the parse window.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+CockpitSubmix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundSubmixBase> CockpitSubmix is missing from "
...
class ProductionAudioRoutingCockpitSubmixFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedProductionAudioRouting\n{\n};\n"
            )
    ...
    def test_missing_cockpit_submix_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("CockpitSubmix = nullptr", compact_origin)
        self.assertNotIn("CockpitSubmix = false", compact_origin)
        self.assertNotIn("CockpitSubmix = true", compact_origin)
        self.assertNotIn("CockpitSubmix = 0.f", compact_origin)
        self.assertNotIn("CockpitSubmix = 160.f", compact_origin)

    def test_cockpit_submix_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertNotIn("TSoftObjectPtr<USoundMix>", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SOUND_MIX}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_MASTER_SUBMIX};\n",
            "\tTSoftObjectPtr<USoundSubmixBase> CockpitSubmixes;\n",
            "\tint32 CockpitSubmix = 1;\n",
            "\tbool CockpitSubmix;\n",
            "\tfloat CockpitSubmix = " + forty + ";\n",
            "\tfloat CockpitSubmix = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_routing_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_COCKPIT_SOUND_MIX), section
        )
        self.assertTrue(
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_COCKPIT_LOW_PASS_HZ), section
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing CockpitSubmix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cockpit_submix_field_decl_contract.py"
        ))
        ...
        self.assertIn(
            LEFTOVER_PRODUCTION_AUDIO_ENTRY_OUTPUT_SUBMIX, LOCKED_SCRIPTS
        )
