...
# ShowOnlyLandscapeComponentCount FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# int32 ShowOnlyLandscapeComponentCount = 0; one-line /
...
# struct FSkyguardLandscapeCaptureConfigurationResult and BEFORE
...
# Do NOT parse FSkyguardLandscapeVisibleAudit (leftover LandscapeComponentCount #1200).
# Do NOT parse FSkyguardLandscapeFootprintSampleResult.
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# ShowOnlyLandscapeComponentCount. THIS IS NOT leftover VisibleAudit
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# ShowOnlyLandscapeComponentCount FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 ShowOnlyLandscapeComponentCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# Fail closed if the type or decl is missing, renamed, or
...
# BlueprintReadOnly int32 ShowOnlyLandscapeComponentCount = 0;
...
TARGET = "int32 ShowOnlyLandscapeComponentCount = 0;"
TARGET_WRONG = "int32 ShowOnlyLandscapeComponentCount = 1;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # int32 literal for FSkyguardLandscapeCaptureConfigurationResult::
    # ShowOnlyLandscapeComponentCount (must be = 0, not = 1).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+ShowOnlyLandscapeComponentCount\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 ShowOnlyLandscapeComponentCount is missing from "
...
class LandscapeCaptureConfigShowOnlyLandscapeComponentCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("ShowOnlyLandscapeComponentCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_show_only_landscape_component_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_show_only_landscape_component_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 ShowOnlyLandscapeComponentCount"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(
                "\tint32 ShowOnlyLandscapeComponentCount = 1;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("ShowOnlyLandscapeComponentCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("ShowOnlyLandscapeComponentCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 ShowOnlyLandscapeComponentCount"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("ShowOnlyLandscapeComponentCount", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_show_only_landscape_component_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
