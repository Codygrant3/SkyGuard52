...
# origin/main form: one-line wrap then one-line
# UFUNCTION(BlueprintPure, Category="Skyguard|Mission01|Environment|Validation")
# then static FString GetActiveRHIAndFeatureLevel();
...
# Leftover unique LandscapeVisibleAudit / MaterialCompilation /
# CaptureConfiguration / AuthoringResult FIELD drafts
# wrap struct members with NO Category and explicitly
# do NOT parse this library.
# Identifier GetActiveRHIAndFeatureLevel is NOT leftover
# analog struct field bSuccess and NOT leftover unique
# SampleLandscapeHeight / SampleLandscapeFootprint
# (Grounding) and NOT leftover unique Authoring UFUNCTION
...
# Return type IS FString. no parameters.
...
# return type is not FString, if static is missing, or if
# Category is Authoring / Grounding /
...
# Do NOT claim sibling Validation UFUNCTIONs
...
# Clone leftover unique Authoring UFUNCTION wrap was
# BlueprintCallable
# Category="Skyguard|Mission01|Environment|Authoring"
# wrapping leftover unique AuthorGovernedLandscapeAndGraph
# static struct-return — RETARGET Category Authoring ->
# Validation. KEEP BlueprintPure (NOT BlueprintCallable).
# Authoring clone KEEP BlueprintCallable — RETARGET
# specifiers Callable -> Pure for THIS slot.
...
# for BlueprintPure RETARGET class/return/decl.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is `static FString GetActiveRHIAndFeatureLevel();`
...
# EnsureDefaultEntries / AuthorGovernedLandscapeAndGraph /
# PrepareGovernedLandscapeForVisibleValidation /
# AuditLandscapeVisibleReadiness / SampleLandscapeHeight
# AS THE LOCKED DECL.
...
TARGET = "static FString GetActiveRHIAndFeatureLevel();"
TARGET_SPLIT = (
    "static FString\n"
    "\tGetActiveRHIAndFeatureLevel();"
)
...
TARGET_WRONG_BARE = (
    "FString GetActiveRHIAndFeatureLevel();"
)
TARGET_WRONG_NO_CONST = (
    "static FString GetActiveRHIAndFeatureLevel() const;"
)
...
TARGET_WRONG_FLOAT = (
    "float GetActiveRHIAndFeatureLevel();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "static FString GetActiveRHIAndFeatureLevels();"
...
TARGET_WRONG_GET_RHI = TARGET_WRONG_AUTHORING
...
TARGET_WRONG_TRUE = "bool GetActiveRHIAndFeatureLevel = true;"
TARGET_WRONG_ZERO = "bool GetActiveRHIAndFeatureLevel = 0.f;"
...
STOP_BEFORE_EVENT_DEFINITIONS = "AuthorGovernedLandscapeAndGraph"
...
LEFTOVER_AUTHORING_UFUNCTION_CLONE = (
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bstatic\s+FString\s+"
    r"GetActiveRHIAndFeatureLevel\(\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # static FString GetActiveRHIAndFeatureLevel();.
    # Accept origin/main one-line and split-line trailing-semicolon form.
    ...
    # leftover EnsureDefaultEntries, leftover
    # PrepareGovernedLandscapeForVisibleValidation /
    # AuditLandscapeVisibleReadiness, or Harbor 40 / 80.
    del declaration
    compact = collapsed(region)
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bGetActiveRHIAndFeatureLevel\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+GetActiveRHIAndFeatureLevel\b", compact):
        return False
    if re.search(r"\bbool\s+GetActiveRHIAndFeatureLevel\b", compact):
        return False
    if re.search(r"\bfloat\s+GetActiveRHIAndFeatureLevel\b", compact):
        return False
    if re.search(r"\bint32\s+GetActiveRHIAndFeatureLevel\b", compact):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static\s+FString\s+"
            r"GetActiveRHIAndFeatureLevel\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for GetActiveRHIAndFeatureLevel "
...
class Mission01GetActiveRHIAndFeatureLevelDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "GetActiveRHIAndFeatureLevel"),
            section,
        )
    ...
    def test_missing_get_active_rhi_and_feature_level_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "GetActiveRHIAndFeatureLevel()const",
    ...
    def test_get_active_rhi_and_feature_level_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid GetActiveRHIAndFeatureLevelX();\n",
            "\tint32 GetActiveRHIAndFeatureLevel = 1;\n",
            "\tFName GetActiveRHIAndFeatureLevel = NAME_None;\n",
            "\tvoid GetActiveRHIAndFeatureLevel() = "
            + forty
            + ";\n",
            "\tvoid GetActiveRHIAndFeatureLevel() = "
            + eighty
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "GetActiveRHIAndFeatureLevel")
        )
        self.assertTrue(
            has_identifier(section, "GetActiveRHIAndFeatureLevel")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 get active rhi and feature level "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_get_active_rhi_and_feature_level"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
