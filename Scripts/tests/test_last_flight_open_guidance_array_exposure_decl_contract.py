...
# or lock the OpenGuidanceArrayExposure body in the
...
# isolated-test drafts #107–#451 including leftover
...
# Python OpenGuidanceArrayExposure declaration
...
# .cpp OpenGuidanceArrayExposure body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::OpenGuidanceArrayExposure",
    "SkyguardLastFlightBoss.cpp",
    "SetExposed",
...
class LastFlightOpenGuidanceArrayExposureDeclContractTests(unittest.TestCase):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, OPEN_GUIDANCE_ARRAY_EXPOSURE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertIn("OpenGuidanceArrayExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_missing_open_guidance_array_exposure_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLastFlightBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertIn("OpenGuidanceArrayExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertIn("OpenGuidanceArrayExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("UFUNCTION", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("Category", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("BlueprintCallable", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertIn("OpenGuidanceArrayExposure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool OpenGuidanceArrayExposure;\n"
        wrong_return_void = "\tvoid OpenGuidanceArrayExposure();\n"
        wrong_return_int = "\tint32 OpenGuidanceArrayExposure();\n"
        added_const = "\tbool OpenGuidanceArrayExposure() const;\n"
        added_arg = "\tbool OpenGuidanceArrayExposure(int32 BayIndex);\n"
        leftover_arg = (
            "\tbool OpenGuidanceArrayExposure(int32 BayIndex, float Extra);\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, OPEN_GUIDANCE_ARRAY_EXPOSURE)
            self.assertIn("OpenGuidanceArrayExposure", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_open_guidance_array_exposure_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, OPEN_GUIDANCE_ARRAY_EXPOSURE),
            1,
        )
        self.assertTrue(
            OPEN_GUIDANCE_ARRAY_EXPOSURE.startswith("bool "),
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        self.assertTrue(
            OPEN_GUIDANCE_ARRAY_EXPOSURE.endswith(";"),
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        self.assertIn(
        ...
        self.assertNotIn("int32 BayIndex", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("INDEX_NONE", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("{", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("}", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("return ", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(" const", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        self.assertNotIn("IssueClimbCommand", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("GetGunnerMount", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("FaceWorldLocation", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("PortGuidanceArray", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("SetSearchlightTracked", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("SetContactIdentified", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        self.assertNotIn("SetLightningExposed", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        self.assertTrue(
            has_declaration(one_line, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, OPEN_GUIDANCE_ARRAY_EXPOSURE),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{OPEN_GUIDANCE_ARRAY_EXPOSURE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_open_guidance_array_exposure_cpp_body(
        self,
    ) -> None:
        locked_only = f"{OPEN_GUIDANCE_ARRAY_EXPOSURE}\n"
        self.assertNotIn("{", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        ...
        self.assertNotIn("return false", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("return true", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("SetExposed", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        self.assertNotIn("SetBossPhase", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("GetFinaleStage", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("IsWreckDiverted", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "GetCivilianSeparationMeters",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("PortGuidanceArray", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("StarboardGuidanceArray", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("CommandCore", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("PortEngine", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("SetSearchlightTracked", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("IsSearchlightTracked", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("ASkyguardBlackKiteBoss", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("SetContactIdentified", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("IsContactIdentified", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("OpenOrbitExposure", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("ASkyguardIronRainBoss", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("GetGunnerMount", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("GetChinTurret", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("GetSensorTurret", OPEN_GUIDANCE_ARRAY_EXPOSURE)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{OPEN_GUIDANCE_ARRAY_EXPOSURE}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("GetChinMuzzleLocation", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("ESkyguardApacheSystem", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("ASkyguardGunner", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("FillAndFinalize", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("SkyguardRadarNode", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("MinHeightFromOriginCm", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("MaxIntegrity", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
        self.assertNotIn("SkyguardPathfinder", OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, OPEN_GUIDANCE_ARRAY_EXPOSURE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{OPEN_GUIDANCE_ARRAY_EXPOSURE}\n"
        self.assertEqual(
            require_declaration(locked_only, OPEN_GUIDANCE_ARRAY_EXPOSURE),
            OPEN_GUIDANCE_ARRAY_EXPOSURE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight OpenGuidanceArrayExposure contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, OPEN_GUIDANCE_ARRAY_EXPOSURE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                OPEN_GUIDANCE_ARRAY_EXPOSURE.lower(),
                "last flight OpenGuidanceArrayExposure contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, OPEN_GUIDANCE_ARRAY_EXPOSURE)

    def test_contract_is_open_guidance_array_exposure_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertNotIn("{", OPEN_GUIDANCE_ARRAY_EXPOSURE)
        self.assertTrue(OPEN_GUIDANCE_ARRAY_EXPOSURE.startswith("bool "))
        self.assertTrue(OPEN_GUIDANCE_ARRAY_EXPOSURE.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)
