...
# bUsesShowOnlyLandscape FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bUsesShowOnlyLandscape = false; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# bUsesShowOnlyLandscape. THIS IS NOT leftover VisibleAudit bSuccess #1199.
...
# Do NOT parse FSkyguardLandscapeVisibleAudit (leftover bSuccess #1199).
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# bUsesShowOnlyLandscape. THIS IS NOT leftover
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# bUsesShowOnlyLandscape FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bUsesShowOnlyLandscape = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly bool bUsesShowOnlyLandscape = false;
...
# bUsesShowOnlyLandscape FIELD (declaration-only). It is
...
TARGET = "bool bUsesShowOnlyLandscape = false;"
TARGET_WRONG = "bool bUsesShowOnlyLandscape = true;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bUsesShowOnlyLandscape (must be = false, not = true).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bUsesShowOnlyLandscape\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bUsesShowOnlyLandscape is missing from "
...
class LandscapeCaptureConfigUsesShowOnlyLandscapeFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bUsesShowOnlyLandscape", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_uses_show_only_landscape_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_uses_show_only_landscape_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bUsesShowOnlyLandscape"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(
                "\tbool bUsesShowOnlyLandscape = true;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("bUsesShowOnlyLandscape", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bUsesShowOnlyLandscape", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bUsesShowOnlyLandscape"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bUsesShowOnlyLandscape", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_uses_show_only_landscape(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
