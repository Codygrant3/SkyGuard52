# THIS IS leftover-safe ASkyguardCampaignTheaterKit Root.
...
# then BARE `TObjectPtr<USceneComponent> Root;` (no initializer).
...
# isolated CardId.
# Sibling fields RoadInstances / BuildingInstances /
# LampInstances / SilhouetteInstances /
# NamedLandmarkMesh / AppliedSpec stay uncontracted.
...
# retarget: type is TObjectPtr<USceneComponent>,
# name is Root, no `=`, Category is Skyguard|Theater,
...
TARGET = "TObjectPtr<USceneComponent> Root;"
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> Root = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> Root = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> Root = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> Root = 0.f;"
TARGET_WRONG_FLOAT = "float Root;"
...
ROOT_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*Root\s*;"
)
ROOT_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*Root\s*="
)
ROOT_FLOAT_RE = re.compile(r"float\s+Root\b")
INVENTED_UPROPERTY = (
    "EditAnywhere",
    "BlueprintReadWrite",
    "BlueprintCallable",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # origin/main has a bare TObjectPtr. Do not accept
    # sibling RoadInstances / BuildingInstances fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ROOT_INIT_RE.search(compact):
        return False
    if ROOT_FLOAT_RE.search(compact):
        return False
    if ROOT_DECL_RE.search(compact) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*Root\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> Root is missing "
...
class TheaterKitRootFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Root"), section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("TObjectPtr<USceneComponent> Root;", compact_origin)
        self.assertNotIn("Root = nullptr", compact_origin)
        self.assertNotIn("Root = false", compact_origin)
        self.assertNotIn("Root = true", compact_origin)
        self.assertNotIn("Root = 0.f", compact_origin)
        self.assertNotIn("Root = 160.f", compact_origin)
    ...
    def test_root_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> Root"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            f"{SIBLING_ROAD_INSTANCES};\n",
            "\tTObjectPtr<USceneComponent> Roots;\n",
            "\tint32 Root;\n",
            "\tbool Root;\n",
            "\tFName Root;\n",
            "\tfloat Root = " + forty + ";\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit Root field decl contract "
