# THIS IS leftover-safe FSkyguardDaySortieBeat Kind.
...
# plus initializer ESkyguardDaySortieBeatKind::RidgeIngress.
# Fail-closed if Kind is missing or renamed, if the type is
# not ESkyguardDaySortieBeatKind, if the initializer is
# missing or is not ESkyguardDaySortieBeatKind::RidgeIngress,
...
# Do NOT contract sibling fields Call / Threat.
...
# #249 / #ba98 (keep that file in LOCKED_SCRIPTS).
...
# #6893.
# THIS IS NOT leftover NightSortieBeat isolated Kind /
# Call / Threat drafts.
# THIS IS NOT leftover LoadoutSpec isolated #1466-#1476.
# THIS IS NOT leftover TheaterKitSpec WeatherIdentity #1300
# (UPROPERTY clone source; keep that file in LOCKED_SCRIPTS).
# If a clone asserts UPROPERTY / Category /
# VisibleAnywhere / FName WeatherIdentity / no `=`,
# retarget: type is ESkyguardDaySortieBeatKind,
# identifier is Kind, initializer IS
# ESkyguardDaySortieBeatKind::RidgeIngress, no UPROPERTY.
# Harbor 40/80 fail-closed via split forty/eighty tokens.
...
LEFTOVER_KIT_STRUCT = "FSkyguardDaySortieBeatKit"
LEFTOVER_ENUM = "ESkyguardDaySortieBeatKind"
LEFTOVER_NAMESPACE = "SkyguardDaySortieBeatKit"
LEFTOVER_NIGHT_STRUCT = "FSkyguardNightSortieBeat"
LEFTOVER_STORM_KIT = "FSkyguardStormRainBeatKit"
TARGET = (
    "ESkyguardDaySortieBeatKind Kind = "
    "ESkyguardDaySortieBeatKind::RidgeIngress;"
)
TARGET_WRONG_BARE = "ESkyguardDaySortieBeatKind Kind;"
TARGET_WRONG_TECHNICAL = (
    "ESkyguardDaySortieBeatKind Kind = "
...
TARGET_WRONG_FALSE = "ESkyguardDaySortieBeatKind Kind = false;"
TARGET_WRONG_TRUE = "ESkyguardDaySortieBeatKind Kind = true;"
TARGET_WRONG_ZERO = "ESkyguardDaySortieBeatKind Kind = 0.f;"
TARGET_WRONG_FLOAT = "float Kind = 160.f;"
TARGET_WRONG_FNAME = "FName Kind;"
TARGET_WRONG_THREAT_TYPE = (
    "ESkyguardThreatKind Kind = ESkyguardThreatKind::GroundArmor;"
)
CLONE_UPROPERTY_WRAP = (
...
LEFTOVER_KIT_MISSION_ID = "MissionId"
LEFTOVER_KIT_WEATHER_IDENTITY = "WeatherIdentity"
LEFTOVER_KIT_KEEP_THERMAL = "bKeepThermal"
LEFTOVER_KIT_BEATS = "Beats"
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_night_sortie_downed_bird_decl_contract.py",
    "Scripts/tests/test_day_sortie_beat_kind_enum_contract.py",
    "Scripts/tests/test_day_sortie_beat_defaults_contract.py",
    ...
    "Scripts/tests/test_night_sortie_beat_kit_kinds_contract.py",
    CLONE_WEATHER_IDENTITY,
    LEFTOVER_NIGHT_BEAT_DEFAULTS,
    LEFTOVER_DAY_BEAT_DEFAULTS,
    LEFTOVER_NIGHT_KIT_DEFAULTS,
...
def leftover_enum_values_outside_locked() -> tuple[str, ...]:
    return (
        "TechnicalScreen",
        "ClusterRidge",
        "TankAmbush",
...
def clone_uproperty_locked_tokens() -> tuple[str, ...]:
    return (
        "UPROPERTY",
        "Category",
        "VisibleAnywhere",
...
def kind_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardDaySortieBeatKind\s+Kind\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare plain C++ field
    # `ESkyguardDaySortieBeatKind Kind =
    # ESkyguardDaySortieBeatKind::RidgeIngress;`.
    # Do not accept missing initializer, TechnicalScreen,
    # leftover Night DarkIngress, leftover float Health,
    # sibling Call / Threat, or UPROPERTY / Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"ESkyguardDaySortieBeatKind\s+Kind\s*=\s*"
        r"ESkyguardDaySortieBeatKind::RidgeIngress\s*;",
        compact,
    ) is None:
        return False
    if "ESkyguardNightSortieBeatKind" in compact:
        return False
    if "DarkIngress" in compact:
        return False
    if kind_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    if re.search(
        r"ESkyguardDaySortieBeatKind\s+Kind\s*;",
        compact,
    ) and not re.search(
        r"ESkyguardDaySortieBeatKind\s+Kind\s*=",
        compact,
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    ...
    ) + leftover_enum_values_outside_locked()
...
class DaySortieBeatKindFieldDeclContractTests(unittest.TestCase):
    def test_day_sortie_beat_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_KIT_STRUCT)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_NIGHT_STRUCT)
        ...
        self.assertTrue(has_identifier(section, "Kind"), section)
        ...
        self.assertNotIn(SIBLING_CALL, LOCKED_DECL)
        self.assertNotIn(SIBLING_THREAT, LOCKED_DECL)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            spec_section(leftover_enum)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        night = (
            f"struct {LEFTOVER_NIGHT_STRUCT}\n"
            "{\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
    ...
    def test_leftover_namespace_or_kit_does_not_satisfy(self) -> None:
        mixed = (
            ...
            f"{STOP_BEFORE_KIT}\n"
            "{{\n"
            f"\tFName {LEFTOVER_KIT_MISSION_ID};\n"
            f"\t{LOCKED_DECL}\n"
            "}};\n"
        ...
        self.assertFalse(has_identifier(section, "Kind"), section)
        self.assertNotIn(STOP_BEFORE_KIT, section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(KIND_AT, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Kind", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_kind_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{SIBLING_CALL_DECL}\n"
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{CLONE_UPROPERTY_WRAP}\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_clone_uproperty_is_not_the_locked_decl(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, CLONE_UPROPERTY_WRAP)
        self.assertNotEqual(LOCKED_DECL, CLONE_UPROPERTY_CAMPAIGN)
        for token in clone_uproperty_locked_tokens():
            self.assertNotIn(token, LOCKED_DECL)
        self.assertNotIn("WeatherIdentity", LOCKED_DECL)
        self.assertIn("Kind", LOCKED_DECL)
        self.assertIn("ESkyguardDaySortieBeatKind", LOCKED_DECL)
        self.assertNotIn("ESkyguardNightSortieBeatKind", LOCKED_DECL)
        self.assertIn("RidgeIngress", LOCKED_DECL)
        self.assertNotIn("DarkIngress", LOCKED_DECL)
    ...
    def test_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("Kind = ESkyguardDaySortieBeatKind::TechnicalScreen", compact_origin)
        self.assertNotIn("DarkIngress", compact_origin)
        self.assertNotIn("Kind = false", compact_origin)
        self.assertNotIn("Kind = true", compact_origin)
        self.assertNotIn("Kind = 0.f", compact_origin)
        self.assertNotIn("Kind = 160.f", compact_origin)

    def test_kind_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardDaySortieBeatKind Kind"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("= TechnicalScreen", LOCKED_DECL)
        self.assertNotIn("= DarkIngress", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TECHNICAL}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_THREAT_TYPE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_TECHNICAL}\n", LOCKED_DECL)
        self.assertIn("Kind", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_THREAT_TYPE}\n",
            leftover_primary,
            leftover_guided,
            f"\t{SIBLING_CALL_DECL}\n",
            f"\t{SIBLING_THREAT_DECL}\n",
            "\tESkyguardDaySortieBeatKind Kinds = "
            "ESkyguardDaySortieBeatKind::RidgeIngress;\n",
            "\tint32 Kind = 0;\n",
            "\tbool Kind = false;\n",
            "\tfloat Kind = " + forty + ";\n",
    ...
    def test_uproperty_or_category_clones_fail_closed(self) -> None:
        wrapped = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{CLONE_UPROPERTY_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        section = spec_section(wrapped)
        self.assertTrue(kind_has_uproperty_wrap(section), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Kind", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("Kind", str(raised.exception))
        origin = spec_section(origin_main_header())
        self.assertFalse(kind_has_uproperty_wrap(origin), origin)
        for token in clone_uproperty_locked_tokens():
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        rejected = (
            f"\t{CLONE_UPROPERTY_WRAP}\n\t{LOCKED_DECL}\n",
            f"\t{CLONE_UPROPERTY_WRAP} {LOCKED_DECL}\n",
            "\tUPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
            'Category="Skyguard|Theater")\n'
            f"\t{LOCKED_DECL}\n",
        )
    ...
    def test_does_not_contract_sibling_call_or_threat(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        self.assertNotIn(SIBLING_CALL_DECL, LOCKED_DECL)
        self.assertNotIn(SIBLING_THREAT_DECL, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, SIBLING_THREAT), section)
        for field in leftover_neighbor_hit_fields():
    ...
    def test_does_not_parse_leftover_enum_kit_namespace_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(LEFTOVER_KIT_STRUCT, header)
        self.assertNotIn(LEFTOVER_KIT_STRUCT, section)
        self.assertNotIn(LEFTOVER_KIT_STRUCT, leaked)
        self.assertIn(STOP_BEFORE_NAMESPACE, header)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_parse_window_excludes_leftover_enum_values(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        leaked = struct_body(header)
        self.assertIn("RidgeIngress", section)
        self.assertNotIn("DarkIngress", section)
        for leftover in leftover_enum_values_outside_locked():
            self.assertNotIn(leftover, section)
        ...
        self.assertNotIn(STOP_BEFORE_ENUM, section)
        self.assertNotIn(STOP_BEFORE_ENUM, leaked)
        self.assertNotIn(LEFTOVER_KIT_KEEP_THERMAL, section)
        self.assertNotIn(LEFTOVER_KIT_BEATS, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "day-sortie-beat Kind field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "day_sortie_beat_kind_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardDaySortieBeatKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_NIGHT_KIND_ENUM, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_DAY_KIND_ENUM, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_KIND_DRAFT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_CALL_DRAFT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_THREAT_DRAFT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_KIT_STRUCT, locked_only)
