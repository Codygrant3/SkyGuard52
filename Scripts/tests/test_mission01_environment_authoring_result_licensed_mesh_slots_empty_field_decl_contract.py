...
TARGET = "bool bLicensedMeshSlotsEmpty = true;"
TARGET_WRONG = "bool bLicensedMeshSlotsEmpty = false;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bLicensedMeshSlotsEmpty (must be = true, not = false).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bLicensedMeshSlotsEmpty\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bLicensedMeshSlotsEmpty is missing from "
...
class Mission01EnvironmentAuthoringResultLicensedMeshSlotsEmptyFieldDeclContractTests(unittest.TestCase):
    def test_mission01_environment_authoring_result_struct_exists(self) -> None:
        ...
        self.assertNotIn("bValid", section.replace("bLicensedMeshSlotsEmpty", ""))
        ...
        self.assertNotIn("bSuccess", LOCKED_DECL.replace("bLicensedMeshSlotsEmpty", ""))
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bLicensedMeshSlotsEmpty", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_licensed_mesh_slots_empty_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bLicensedMeshSlotsEmpty"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(
                "\tbool bLicensedMeshSlotsEmpty = false;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("bLicensedMeshSlotsEmpty", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bLicensedMeshSlotsEmpty", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bLicensedMeshSlotsEmpty"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bLicensedMeshSlotsEmpty", locked_only)
