# THIS IS leftover-safe ASkyguardApacheAircraft HydraRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> HydraRight;`
# (no initializer).
...
# Stop BEFORE sibling HellfireLeft as a claimed slot.
# Do NOT claim HydraLeft / HellfireLeft / HellfireRight /
# HorizontalTail / MainRotorCross / RadarDome / Canopy /
# Fuselage / AircraftRoot / SilhouetteMesh.
# THIS IS leftover-safe ASkyguardApacheAircraft.HydraRight.
# It is NOT leftover HydraLeft, NOT leftover PylonRight, NOT leftover analog
# apache-hull-collider, NOT leftover analog
# apache-mount-fail-closed, NOT leftover analog
# apache-aircraft-empty-fail-closed.
...
# leftover analog apache-hydra-left-field,
# leftover analog apache-pylon-right-field,
# leftover analog apache-hover-bob-centimeters-field,
# leftover analog apache-rotor-power-field.
# Do NOT parse leftover LoadoutSpec RocketReserve /
# RocketMagazineSize / GuidedReserve / GuidedMagazineSize.
# Do NOT parse leftover StormRainBeatKit bHydraForClusters.
# Do NOT parse leftover analog apache-cpg-feel or leftover
# analog gunship-weapon-stations.
# HydraLeft / PylonRight / HellfireLeft / HellfireRight are sibling
# slots being opened, not this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> HydraRight;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> HydraRight;` and NOT
# leftover `HydraLeft` / leftover `PylonRight` / leftover
# `HellfireLeft` / leftover `HellfireRight` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> HydraRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> HydraRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> HydraRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> HydraRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> HydraRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> HydraRight;"
...
TARGET_WRONG_ROCKET_RESERVE = "int32 RocketReserve = 38;"
TARGET_WRONG_ROCKET_MAGAZINE = "int32 RocketMagazineSize = 14;"
TARGET_WRONG_GUIDED_RESERVE = "int32 GuidedReserve = 8;"
TARGET_WRONG_GUIDED_MAGAZINE = "int32 GuidedMagazineSize = 4;"
...
STOP_BEFORE_HELLFIRE_LEFT = "HellfireLeft"
...
HYDRA_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HydraRight\s*;"
)
HYDRA_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HydraRight\s*="
)
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_HELLFIRE_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_HELLFIRE_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_HELLFIRE_LEFT}"
        )
    for later in (
        SIBLING_HELLFIRE_RIGHT,
        SIBLING_GUNNER_MOUNT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+HydraRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> HydraRight "
...
class ApacheHydraRightFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HydraRight"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_HELLFIRE_LEFT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_HELLFIRE_LEFT),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_HELLFIRE_RIGHT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_HELLFIRE_RIGHT),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_HYDRA_LEFT), body)
        self.assertTrue(has_identifier(section, SIBLING_HYDRA_LEFT), section)
        self.assertTrue(has_identifier(body, SIBLING_PYLON_RIGHT), body)
        ...
        self.assertTrue(has_identifier(section, SIBLING_GEAR_LEFT), section)
        self.assertTrue(has_identifier(body, SIBLING_GEAR_RIGHT), body)
        self.assertTrue(has_identifier(section, SIBLING_GEAR_RIGHT), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HydraRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("HydraRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_hydra_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> HydraRight;", compact_origin)
        self.assertNotIn("HydraRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_hydra_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> HydraRight"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_HYDRA_LEFT))
        self.assertNotIn(SIBLING_HELLFIRE_LEFT, LOCKED_DECL)
        self.assertNotIn(SIBLING_HELLFIRE_RIGHT, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HYDRA_LEFT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_PYLON_RIGHT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HELLFIRE_LEFT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HELLFIRE_RIGHT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ROCKET_RESERVE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HYDRA_FOR_CLUSTERS}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> HydraRight;")
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> HydraRight;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> HydraRights;\n",
            "\tint32 HydraRight = 140;\n",
            "\tbool HydraRight = true;\n",
            "\tfloat HydraRight = " + forty + ";\n",
            "\tfloat HydraRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_hellfire_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_HELLFIRE_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_HELLFIRE_RIGHT))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertFalse(has_identifier(section, SIBLING_HELLFIRE_LEFT))
        self.assertTrue(has_identifier(leaked, SIBLING_HELLFIRE_RIGHT))
        self.assertFalse(has_identifier(section, SIBLING_HELLFIRE_RIGHT))
        self.assertTrue(has_identifier(leaked, SIBLING_HYDRA_LEFT))
        self.assertTrue(has_identifier(section, SIBLING_HYDRA_LEFT))
        self.assertTrue(has_identifier(leaked, SIBLING_PYLON_RIGHT))
        self.assertTrue(has_identifier(section, SIBLING_PYLON_RIGHT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache HydraRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "hydra_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_APACHE_GEAR_LEFT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_APACHE_GEAR_RIGHT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_HYDRA_LEFT))
        self.assertNotIn(SIBLING_HELLFIRE_LEFT, locked_only)
        self.assertNotIn(SIBLING_HELLFIRE_RIGHT, locked_only)
