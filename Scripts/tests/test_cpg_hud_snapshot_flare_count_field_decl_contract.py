# THIS IS leftover-safe FSkyguardCpgHudSnapshot FlareCount.
...
# `int32 FlareCount = 0;` with in-struct initializer `= 0`.
# Fail-closed if FlareCount is missing or renamed, if the
# type is not int32, if the initializer is missing, is
# leftover LoadoutSpec `= 6`, or is not `= 0`, or if
...
# field. HUD snapshot `int32 FlareCount = 0` is NOT
# leftover LoadoutSpec `int32 FlareCount = 6`.
# Fail-closed if the parse window is leftover
# FSkyguardLoadoutSpec instead of FSkyguardCpgHudSnapshot.
...
# Fail-closed if sibling leftover `int32 ThreatCount = 0;`
# is locked instead of FlareCount.
...
# THIS IS NOT leftover HUD ThreatCount sibling.
...
# WeaponLine (FString, no initializer) and leftover-safe
# ThreatCount sibling int32 with `= 0` in the same HUD
# struct. RETARGET: this field is int32 with `= 0`.
# Do not contract ThreatCount here.
...
# leftover `int32 ThreatCount = 0;` / leftover LoadoutSpec
# `int32 FlareCount = 6;`, retarget: type is int32,
# identifier is FlareCount, initializer is `= 0`, locked
# decl is the plain field, not a UPROPERTY wrap, not
# leftover LoadoutSpec `= 6`.
...
TARGET = "int32 FlareCount = 0;"
TARGET_WRONG_BARE = "int32 FlareCount;"
TARGET_WRONG_EQ_EMPTY = 'int32 FlareCount = TEXT("");'
TARGET_WRONG_EQ_NONE = "int32 FlareCount = NAME_None;"
TARGET_WRONG_FALSE = "int32 FlareCount = false;"
TARGET_WRONG_TRUE = "int32 FlareCount = true;"
TARGET_WRONG_ZERO_F = "int32 FlareCount = 0.f;"
TARGET_WRONG_SIX = "int32 FlareCount = 6;"
TARGET_WRONG_ONE = "int32 FlareCount = 1;"
TARGET_WRONG_FLOAT = "float FlareCount;"
TARGET_WRONG_HEALTH = "float FlareCount = 160.f;"
TARGET_WRONG_BOOL = "bool FlareCount;"
TARGET_WRONG_FSTRING = "FString FlareCount;"
TARGET_WRONG_FNAME = "FName FlareCount;"
TARGET_WRONG_TCHAR = 'const TCHAR* FlareCount = TEXT("");'
...
FLARE_COUNT_INIT_RE = re.compile(r"int32\s+FlareCount\s*=\s*0\s*;")
FLARE_COUNT_BARE_RE = re.compile(r"int32\s+FlareCount\s*;")
FLARE_COUNT_WRONG_INIT_RE = re.compile(
    r"int32\s+FlareCount\s*=\s*(?:0\.f|6|1|false|true|NAME_None)"
)
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
...
def flare_count_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*int32\s+FlareCount\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 FlareCount = 0;`.
    # Do not accept a missing initializer, `= 0.f`, leftover
    # LoadoutSpec `= 6`, `= false`, `= 1`, sibling
    # ThreatCount, leftover ThreatLine, leftover Beat
    ...
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if FLARE_COUNT_WRONG_INIT_RE.search(compact):
        return False
    if FLARE_COUNT_BARE_RE.search(compact) and not FLARE_COUNT_INIT_RE.search(
        compact
    ):
        return False
    ...
    if re.search(r"bool\s+FlareCount\b", compact):
        return False
    if re.search(r"FString\s+FlareCount\b", compact):
        return False
    if re.search(r"FName\s+FlareCount\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*FlareCount\b", compact):
        return False
    if FLARE_COUNT_INIT_RE.search(compact) is None:
        return False
    if flare_count_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    if "= 6" in compact and "FlareCount" in compact:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32 FlareCount\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 FlareCount is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*int32 FlareCount\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on int32 FlareCount is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "FlareCount"):
        raise AssertionError(
            "UPROPERTY clone landed on FlareCount; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "FlareCount"):
            raise AssertionError(
                f"{token} clone landed on FlareCount; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotFlareCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 FlareCount = 0;")
        ...
        self.assertIn("FlareCount", LOCKED_DECL)
    ...
    def test_sibling_threat_count_does_not_satisfy_flare_count(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{SIBLING_THREAT_COUNT_DECL}\n"
    ...
    def test_threat_line_does_not_satisfy_flare_count(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_THREAT_LINE}\n"
    ...
    def test_initializer_missing_or_wrong_fails_closed(self) -> None:
        ...
        self.assertIn("int32 FlareCount = 0;", compact_origin)
        self.assertNotIn("FlareCount = TEXT", compact_origin)
        self.assertNotIn("FlareCount = NAME_None", compact_origin)
        self.assertNotIn("FlareCount = false", compact_origin)
        self.assertNotIn("FlareCount = true", compact_origin)
        self.assertNotIn("FlareCount = 0.f", compact_origin)
        self.assertNotIn("FlareCount = 160.f", compact_origin)
    ...
    def test_flare_count_declaration_matches_origin_main(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_SIX}\n", LOCKED_DECL)
        self.assertIn("FlareCount", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFString FlareCount;\n",
            "\tbool FlareCount;\n",
            "\tfloat FlareCount = " + forty + ";\n",
            "\tfloat FlareCount = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    flare_count_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(flare_count_has_uproperty_wrap(origin), origin)
    ...
    def test_hud_flare_count_is_not_leftover_loadout_spec_flare_count(
        self,
    ) -> None:
        self.assertEqual(LOCKED_DECL, "int32 FlareCount = 0;")
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_LOADOUT_FLARE_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot FlareCount field decl contract "
