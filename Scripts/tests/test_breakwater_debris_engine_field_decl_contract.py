...
# Engine / ElevatorLinkage / DebrisPortPanel /
# DebrisStarboardPanel. Keep DebrisEngine distinct
# from leftover Breakwater Engine and leftover
# RoadHunter / RunwayBreaker DebrisEngine. Not
...
# Engine / DebrisPortPanel distinct from leftover
# RadarGhost Engine and leftover Tempest
# DebrisPortPanel. This is not a leftover
...
# isolated-test drafts #107–#543 including
...
# Breakwater DebrisPortPanel stays distinct from
# leftover Tempest DebrisPortPanel even when the
# declaration text matches.
PORT_LATCH_FIELD = (
...
# sibling Breakwater fields, leftover drafted
...
# leftover isolated-test drafts #107–#543,
# leftover searchlight-track-runtime-defaults
...
LEFTOVER_RUNWAY_BREAKER_DEBRIS_ENGINE_NOT_LOCKED = (
    "ASkyguardRunwayBreakerBoss",
    "SkyguardRunwayBreakerBoss.h",
)
LEFTOVER_BREAKWATER_DEBRIS_STARBOARD_PANEL_NOT_LOCKED = (
    "test_breakwater_debris_starboard_panel_field_decl_contract.py",
    "DebrisStarboardPanel",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBreakwaterBoss::DebrisEngine",
    "SkyguardBreakwaterBoss.cpp",
    "SetExposed",
...
class BreakwaterDebrisEngineFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_debris_engine_declaration_fails_closed(self) -> None:
        neighbors_only = (
            ...
            f"\t{DECOY_PODS_FIELD}\n"
            f"\t{BREAKWATER_DEBRIS_STARBOARD_PANEL_FIELD}\n"
    ...
    def test_contract_does_not_lock_debris_engine_cpp_body(self) -> None:
        ...
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", DEBRIS_ENGINE_FIELD)
    ...
    def test_breakwater_engine_is_not_radar_ghost_or_pathfinder_engine(
        ...
        locked_only = f"{DEBRIS_ENGINE_FIELD}\n"
        self.assertNotEqual(BREAKWATER_ENGINE_FIELD, DEBRIS_ENGINE_FIELD)
        ...
        self.assertNotIn(BREAKWATER_ENGINE_FIELD, DEBRIS_ENGINE_FIELD)

    def test_breakwater_engine_does_not_satisfy_debris_engine(self) -> None:
        leftover = f"\t{BREAKWATER_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn("UStaticMeshComponent", BREAKWATER_ENGINE_FIELD)
        self.assertTrue(
            DEBRIS_ENGINE_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
        ...
        self.assertIn("DebrisEngine", DEBRIS_ENGINE_FIELD)
        self.assertIn("Engine;", BREAKWATER_ENGINE_FIELD)

    def test_road_hunter_runway_breaker_debris_engine_do_not_satisfy(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_ENGINE_FIELD}\n"
        file_text = this_file_text()
        for leftover_class, leftover_header in (
            ("ASkyguardRoadHunterBoss", "SkyguardRoadHunterBoss.h"),
            ("ASkyguardRunwayBreakerBoss", "SkyguardRunwayBreakerBoss.h"),
        ):
            other = (
                f"class SKYGUARD52_API {leftover_class} "
                ": public AActor\n"
                "{\n"
                "public:\n"
                f"\t{DEBRIS_ENGINE_FIELD}\n"
            ...
            self.assertIn(CLASS_NAME, str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertNotIn(leftover_class, locked_only)
            self.assertNotIn(leftover_class, DEBRIS_ENGINE_FIELD)
        ...
        for token in LEFTOVER_ROAD_HUNTER_DEBRIS_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_ENGINE_FIELD)
        for token in LEFTOVER_RUNWAY_BREAKER_DEBRIS_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_ENGINE_FIELD)
        self.assertEqual(
            HEADER_PATH,
    ...
    def test_breakwater_debris_port_panel_is_not_tempest_debris_port_panel(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_ENGINE_FIELD}\n"
        self.assertNotEqual(
        ...
        for token in LEFTOVER_BREAKWATER_DEBRIS_PORT_PANEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_ENGINE_FIELD)
        for token in LEFTOVER_DEBRIS_PORT_PANEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
    ...
    def test_contract_does_not_relock_sibling_radar_ghost_fields(self) -> None:
        ...
        self.assertNotIn("DebrisPortEWPanel", DEBRIS_ENGINE_FIELD)
        self.assertNotIn("DebrisStarboardEWPanel", DEBRIS_ENGINE_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardBreakwaterBoss::DebrisEngine", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "breakwater DebrisEngine field contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                ...
                "breakwater DebrisEngine contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
