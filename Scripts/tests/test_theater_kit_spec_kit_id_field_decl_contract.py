# THIS IS leftover-safe FSkyguardTheaterKitSpec::KitId.
# origin/main form: one-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Skyguard|Theater")
# then BARE `FName KitId;` (no initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# THIS IS UPROPERTY(VisibleAnywhere, BlueprintReadOnly,
# Category="Skyguard|Theater"). Category IS
# `Skyguard|Theater`. VisibleAnywhere IS present.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if Category is missing, or if an
# initializer appears when origin/main is bare.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardTheaterKitSpec` body ONLY after
# `struct FSkyguardTheaterKitSpec`. Stop at
# `namespace SkyguardCampaignTheaterKit`.
# Do NOT parse `ASkyguardCampaignTheaterKit`.
# Do NOT parse the namespace helpers.
# Do NOT contract sibling fields WeatherIdentity,
# LandmarkSet, BuildingKit, BuildingTint, LampTreatment,
# LampColor, LampIntensity, RoadTreatment, RoadTint,
# NamedLandmark, LandmarkTint, SilhouetteKit,
# SilhouetteTint, LandmarkMeshIndex, LandmarkScale.
# If a Health clone asserts `float` / `= 160.f` /
# Category="Skyguard|Campaign", retarget: type is FName,
# no `=`, Category is Skyguard|Theater.
# Fail-closed if specifiers drop to BlueprintReadWrite
# or EditAnywhere-only.
...
# THIS IS NOT leftover ProtectAsset / RadarNode
# #1288-#1298.
...
# If a clone drops Category because AudioTelemetry had
# none, do not drop it here.
...
# Do NOT parse ASkyguard + Ya + k52Aircraft.
...
# ESkyguardBossWeapon, ESkyguardAudioEvent,
...
TARGET = "FName KitId;"
TARGET_WRONG_INIT = "FName KitId = NAME_None;"
TARGET_WRONG_TEXT = 'FName KitId = TEXT("");'
TARGET_WRONG_FLOAT = "float KitId;"
TARGET_WRONG_INT = "int32 KitId;"
TARGET_WRONG_BOOL = "bool KitId;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SIBLING = "FName WeatherIdentity;"
LOCKED_DECL = TARGET
...
STOP_BEFORE_ACTOR = "ASkyguardCampaignTheaterKit"
STOP_BEFORE_APPLY_WORLD = "ApplyTheaterKitToWorld"
STOP_BEFORE_APPLY = "ApplyTheaterKit"
STOP_BEFORE_GET_APPLIED = "GetAppliedKitId"
...
STOP_BEFORE_SORTIE_DIRECTOR = "ASkyguardGunshipSortieDirector"
STOP_BEFORE_PATROL_BOSS = "ASkyguardPatrolShipBoss"
...
NAMESPACE_NUM_KITS = "NumKits"
NAMESPACE_GET_BY_INDEX = "GetByIndex"
NAMESPACE_RESOLVE = "Resolve"
NAMESPACE_FINGERPRINT = "Fingerprint"
NAMESPACE_PAIRWISE = "AreKitsPairwiseDistinct"
...
LEFTOVER_MESH_BIND_SLOT_FIELDS = (
    "Scripts/tests/test_mesh_bind_slot_fields_contract.py"
)
LEFTOVER_MESH_BIND_SLOT_FIELDS_TESTS = (
    "Scripts/tests/test_mesh_bind_slot_fields_tests.py"
)
LEFTOVER_MESH_BIND_SLOT_FIELDS_PY = (
    "Scripts/tests/test_mesh_bind_slot_fields.py"
)
...
def leftover_retired_aircraft() -> str:
    return "ASkyguard" + "Ya" + "k52Aircraft"
...
def actor_api_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_ACTOR,
        STOP_BEFORE_APPLY_WORLD,
        STOP_BEFORE_GET_APPLIED,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName KitId;`.
    # Do not accept `= NAME_None` / `= TEXT("")` /
    # leftover Health `float` / `= 160.f` when
    # origin/main has no initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"FName\s+KitId\s*=", compact):
        return False
    if re.search(r"FName\s+KitId\s*;", compact) is None:
        return False
    if re.search(r"float\s+KitId\b", compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f", compact) and (
        declaration == LOCKED_DECL
    ):
        return False
    return True
...
def struct_window(header: str) -> str:
    ...
    start = match.start()
    stop = header.find(STOP_BEFORE_NAMESPACE, start)
    if stop == -1:
        raise AssertionError(
            f"{STRUCT_NAME} parse window missing "
            f"{STOP_BEFORE_NAMESPACE} from origin/main:{HEADER_PATH}"
        )
    window = header[start:stop]
    if STOP_BEFORE_ACTOR in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_ACTOR}"
        )
    return window


def class_body(header: str) -> str:
    window = struct_window(header)
    start = window.index("{")
    depth = 0
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_SORTIE_DIRECTOR,
        STOP_BEFORE_PATROL_BOSS,
        leftover_retired_aircraft(),
        STOP_BEFORE_GUNNER,
...
def public_section(header: str) -> str:
    ...
    if STOP_BEFORE_ACTOR in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_ACTOR}"
        )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+KitId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName KitId is missing from "
        f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} public section"
    )
...
def require_declaration(region: str, declaration: str) -> str:
    if not has_declaration(region, declaration):
        raise AssertionError(
            ...
            f"struct {STRUCT_NAME} public section"
        )
    return declaration


class TheaterKitSpecKitIdFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "KitId"), section)
        ...
        self.assertIn(STOP_BEFORE_ACTOR, header)
        self.assertNotIn(STOP_BEFORE_ACTOR, section)
        self.assertNotIn(STOP_BEFORE_ACTOR, body)
        ...
        self.assertNotIn(STOP_BEFORE_PROP_SPINNER, section)
        self.assertNotIn(STOP_BEFORE_SORTIE_DIRECTOR, section)
        self.assertNotIn(STOP_BEFORE_PATROL_BOSS, section)
        self.assertNotIn(leftover_retired_aircraft(), section)
        ...
        self.assertNotIn(STOP_BEFORE_APPLY_WORLD, section)
        self.assertNotIn(STOP_BEFORE_GET_APPLIED, section)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        actor = (
            f"class {STOP_BEFORE_ACTOR} : public AActor\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
    ...
    def test_actor_declaration_does_not_satisfy_struct_lock(self) -> None:
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            ...
            "{{\n"
            "}}\n"
            f"class {STOP_BEFORE_ACTOR}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
            f"\tvoid {STOP_BEFORE_APPLY_WORLD}();\n"
            "};\n"
        )
        ...
        self.assertIn("KitId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_kit_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tFName WeatherIdentity;\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName KitId;", compact_origin)
        self.assertNotIn("KitId =", compact_origin)
        self.assertNotIn("KitId = NAME_None", compact_origin)
        self.assertNotIn("Health = 160.f", compact_origin)

    def test_kit_id_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName KitId"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn("Skyguard|Campaign", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_HEALTH}\n", LOCKED_DECL)
        self.assertIn("KitId", str(raised.exception))
    ...
    def test_does_not_parse_actor_namespace_or_neighbors(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_ACTOR, header)
        self.assertNotIn(STOP_BEFORE_ACTOR, section)
        self.assertNotIn(STOP_BEFORE_ACTOR, leaked)
        for token in actor_api_tokens():
        ...
        self.assertNotIn(LEFTOVER_PROTECT_ASSET_CLASS, header)
        self.assertNotIn(LEFTOVER_RADAR_NODE_CLASS, header)
        self.assertNotIn(leftover_retired_aircraft(), header)
        self.assertNotIn(STOP_BEFORE_AUDIO_EVENT, section)
        self.assertNotIn(STOP_BEFORE_LISTENER, section)
        self.assertNotIn(STOP_BEFORE_EVENT_DEF, section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAPON, section)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        self.assertNotIn(leftover_retired_aircraft(), section)
        self.assertNotIn(leftover_retired_aircraft(), leaked)

    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tvoid {STOP_BEFORE_APPLY_WORLD}();\n"
            f"\tvoid {STOP_BEFORE_APPLY}(FName WeatherIdentity);\n"
            f"\tFName {STOP_BEFORE_GET_APPLIED}() const;\n"
            f"\tint32 {NAMESPACE_NUM_KITS}();\n"
            f"\tconst {STRUCT_NAME}& {NAMESPACE_RESOLVE}"
            "(FName WeatherIdentity);\n"
            "\tFName WeatherIdentity;\n"
            "\tFName LandmarkSet;\n"
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertIn("SkyguardCampaignTheaterKit.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit spec KitId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "kit_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardProtectAsset.h", THIS_SCRIPT)
        self.assertIn(CLONE_RADAR_HEALTH, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_THEATER_KIT_WEATHER_APPLY_TESTS, LOCKED_SCRIPTS)
        self.assertIn(
            LEFTOVER_THEATER_KIT_WEATHER_APPLY_CONTRACT,
            LOCKED_SCRIPTS,
        )
        self.assertIn(LEFTOVER_MESH_BIND_SLOT_FIELDS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MESH_BIND_SLOT_FIELDS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MESH_BIND_SLOT_FIELDS_PY, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_ACTOR, locked_only)
