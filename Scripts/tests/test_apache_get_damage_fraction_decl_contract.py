...
# or lock the GetDamageFraction body in the .cpp.
...
# ASkyguardApacheAircraft only. This is a hull-integrity
# fraction getter. It is not leftover
# apache-own-ship-systems #96c5 (that leftover locks
# ESkyguardApacheSystem enumerators, not this method).
# Stay off leftover #96c5 methods ApplySystemHit,
# ApplyHit, IsSensorLive, IsThermalAvailable,
# IsSensorViewActive. Stay off leftover
# apache-aircraft empty-fail-closed #851b mount getters
# GetGunnerMount / GetEyeMount / GetWeaponMount /
# GetChinTurret / GetPilotMount / GetSensorTurret.
# Stay off leftover apache-chin-muzzle #4e39
# GetChinMuzzleLocation. Stay off leftover
# apache-cpg-feel #8951 and leftover
...
# GetForwardSpeed, ApplyDamage, and remaining unused
# neighbors. Leftover briefing / debrief widget isolated
...
# #56–#64 plus isolated-test drafts #107–#414,
...
# HandleDebriefKey, leftover AimChinTurret sibling,
# leftover SetRotorPower sibling, leftover
...
# leftover SetDirectFlightInput sibling, leftover
# GetForwardSpeed sibling, leftover ApplyDamage
# sibling, and leftover SortiePresentationWidgets
# stay sibling-only.
GET_DAMAGE_FRACTION = (
    "float GetDamageFraction() const;"
)
...
# GetDamageFraction declaration contract on
...
# FaceWorldLocation, SetSensorView,
# SetFirstPersonInterior, SetDirectFlightInput,
# GetForwardSpeed, ApplyDamage, ApplySystemHit,
# ApplyHit, IsSensorLive, IsThermalAvailable,
...
# leftover SetDirectFlightInput sibling, leftover
# GetForwardSpeed sibling, leftover ApplyDamage
# sibling, and dirty workspace paths.
LOCKED = {
...
# leftover ApplyDamage sibling in this wave,
...
# .cpp GetDamageFraction body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::GetDamageFraction",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheGetDamageFractionDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_DAMAGE_FRACTION), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_DAMAGE_FRACTION)
        self.assertIn("GetDamageFraction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_DAMAGE_FRACTION))

    def test_missing_get_damage_fraction_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_DAMAGE_FRACTION)
        self.assertIn("GetDamageFraction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_DAMAGE_FRACTION)
        self.assertIn("GetDamageFraction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_DAMAGE_FRACTION), section)
        self.assertNotIn("BlueprintPure", GET_DAMAGE_FRACTION)
        self.assertNotIn("UFUNCTION", GET_DAMAGE_FRACTION)
        self.assertNotIn("Category", GET_DAMAGE_FRACTION)
        self.assertNotIn("BlueprintCallable", GET_DAMAGE_FRACTION)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_DAMAGE_FRACTION)
        self.assertIn("GetDamageFraction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tfloat GetDamageFraction();\n"
        wrong_return_void = "\tvoid GetDamageFraction() const;\n"
        wrong_return_bool = "\tbool GetDamageFraction() const;\n"
        added_arg = "\tfloat GetDamageFraction(bool bReady) const;\n"
        leftover_apply = "\tvoid ApplyDamage(float Amount);\n"
        leftover_forward = (
            "\tfloat GetForwardSpeed() const "
            "{ return ForwardSpeed; }\n"
        )
        ...
        leftover_quality = "\tfloat GetSensorQuality() const;\n"
        leftover_flight = (
            "\tvoid SetDirectFlightInput("
            "float Collective,float Yaw,"
            "float CyclicPitch,float CyclicRoll);\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_DAMAGE_FRACTION)
            self.assertIn("GetDamageFraction", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_damage_fraction_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_DAMAGE_FRACTION),
            GET_DAMAGE_FRACTION,
        )
        self.assertTrue(has_declaration(section, GET_DAMAGE_FRACTION))
        self.assertEqual(declaration_count(section, GET_DAMAGE_FRACTION), 1)
        self.assertTrue(
            GET_DAMAGE_FRACTION.startswith("float "),
            GET_DAMAGE_FRACTION,
        )
        self.assertTrue(GET_DAMAGE_FRACTION.endswith(";"), GET_DAMAGE_FRACTION)
        self.assertIn("GetDamageFraction()", GET_DAMAGE_FRACTION)
        self.assertIn(" const", GET_DAMAGE_FRACTION)
        self.assertNotIn("INDEX_NONE", GET_DAMAGE_FRACTION)
        self.assertNotIn("{", GET_DAMAGE_FRACTION)
        self.assertNotIn("}", GET_DAMAGE_FRACTION)
        self.assertNotIn("return ", GET_DAMAGE_FRACTION)
        self.assertNotIn("AimChinTurret", GET_DAMAGE_FRACTION)
        self.assertNotIn("SetRotorPower", GET_DAMAGE_FRACTION)
        self.assertNotIn("IssuePilotCommand", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetPilotCommand", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetPilotConfirmationsIssued", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetChinMuzzleLocation", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetChinTurret", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetRotorPowerScale", GET_DAMAGE_FRACTION)
        self.assertNotIn("SetDirectFlightInput", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetForwardSpeed", GET_DAMAGE_FRACTION)
        self.assertNotIn("ApplyDamage", GET_DAMAGE_FRACTION)
        self.assertNotIn("ApplySystemHit", GET_DAMAGE_FRACTION)
        self.assertNotIn("ApplyHit", GET_DAMAGE_FRACTION)
        self.assertNotIn("IsSensorLive", GET_DAMAGE_FRACTION)
        self.assertNotIn("IsThermalAvailable", GET_DAMAGE_FRACTION)
        self.assertNotIn("IsSensorViewActive", GET_DAMAGE_FRACTION)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tfloat\n"
            "\tGetDamageFraction() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, GET_DAMAGE_FRACTION),
                section,
            )
            self.assertEqual(
                require_declaration(section, GET_DAMAGE_FRACTION),
            ...
            self.assertEqual(
                declaration_count(section, GET_DAMAGE_FRACTION),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_DAMAGE_FRACTION}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_DAMAGE_FRACTION))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_DAMAGE_FRACTION), section)
        self.assertEqual(
            require_declaration(section, GET_DAMAGE_FRACTION),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_DAMAGE_FRACTION}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_DAMAGE_FRACTION)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_DAMAGE_FRACTION)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_damage_fraction_cpp_body(self) -> None:
        locked_only = f"{GET_DAMAGE_FRACTION}\n"
        self.assertNotIn("{", GET_DAMAGE_FRACTION)
        self.assertNotIn("}", GET_DAMAGE_FRACTION)
        self.assertNotIn("return ", GET_DAMAGE_FRACTION)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", GET_DAMAGE_FRACTION)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", GET_DAMAGE_FRACTION)
        self.assertNotIn("return true", GET_DAMAGE_FRACTION)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{GET_DAMAGE_FRACTION}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DAMAGE_FRACTION)
        self.assertNotIn("GetGunnerMount", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetEyeMount", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetWeaponMount", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetChinTurret", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetPilotMount", GET_DAMAGE_FRACTION)
        self.assertNotIn("GetSensorTurret", GET_DAMAGE_FRACTION)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_DAMAGE_FRACTION}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DAMAGE_FRACTION)
        self.assertNotIn("GetChinMuzzleLocation", GET_DAMAGE_FRACTION)
    ...
    def test_contract_does_not_relock_face_world_location(self) -> None:
        ...
        for neighbor in FACE_WORLD_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DAMAGE_FRACTION)
        self.assertNotIn("FaceWorldLocation", GET_DAMAGE_FRACTION)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DAMAGE_FRACTION)
        self.assertNotIn("SetSensorView", GET_DAMAGE_FRACTION)
        self.assertNotIn("SetFirstPersonInterior", GET_DAMAGE_FRACTION)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DAMAGE_FRACTION)
        self.assertNotIn("ESkyguardApacheSystem", GET_DAMAGE_FRACTION)
    ...
    def test_contract_does_not_relock_leftover_set_orbit_focus_sibling(
        ...
        self.assertNotIn("SetOrbitFocus", GET_DAMAGE_FRACTION)

    def test_contract_does_not_relock_leftover_first_person_sibling(
        self,
    ) -> None:
        locked_only = f"{GET_DAMAGE_FRACTION}\n"
        for token in LEFTOVER_SET_FIRST_PERSON_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DAMAGE_FRACTION)
        self.assertNotIn(
            "test_apache_set_first_person_interior_decl_contract.py",
    ...
    def test_contract_does_not_relock_leftover_direct_flight_sibling(
        self,
    ) -> None:
        locked_only = f"{GET_DAMAGE_FRACTION}\n"
        for token in LEFTOVER_SET_DIRECT_FLIGHT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DAMAGE_FRACTION)
        self.assertNotIn(
            "test_apache_set_direct_flight_input_decl_contract.py",
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DAMAGE_FRACTION)
        self.assertNotIn("Pursuit", GET_DAMAGE_FRACTION)
        self.assertNotIn("OrbitLeft", GET_DAMAGE_FRACTION)
        self.assertNotIn("AttackRun", GET_DAMAGE_FRACTION)
        self.assertNotIn("FaceTarget", GET_DAMAGE_FRACTION)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DAMAGE_FRACTION)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", GET_DAMAGE_FRACTION)
        self.assertNotIn("USkyguardBriefingWidget", GET_DAMAGE_FRACTION)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            GET_DAMAGE_FRACTION,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DAMAGE_FRACTION)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_DAMAGE_FRACTION)
        self.assertNotIn("ESkyguardMissionSkylineStyle", GET_DAMAGE_FRACTION)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_DAMAGE_FRACTION}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_DAMAGE_FRACTION),
            GET_DAMAGE_FRACTION,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GetDamageFraction contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_DAMAGE_FRACTION.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache GetDamageFraction contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_DAMAGE_FRACTION)

    def test_contract_is_get_damage_fraction_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", GET_DAMAGE_FRACTION)
        self.assertNotIn("{", GET_DAMAGE_FRACTION)
        self.assertTrue(GET_DAMAGE_FRACTION.startswith("float "))
        self.assertTrue(GET_DAMAGE_FRACTION.endswith(";"))
        self.assertIn(" const", GET_DAMAGE_FRACTION)
        self.assertIn(UFUNCTION_APACHE, section)
