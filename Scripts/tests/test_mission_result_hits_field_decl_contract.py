# THIS IS leftover-safe FSkyguardMissionResult::Hits.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `int32 Hits = 0;` with REQUIRED initializer `= 0`.
...
# leftover MissionId draft, then this isolated Hits field decl.
# Clone parse-window from leftover FinalScore #1264 and leftover
# MissionId. Clone LOCKED_SCRIPTS from leftover-safe draft #1300
...
# EditAnywhere + BlueprintReadWrite, type is int32, REQUIRED
# initializer `= 0`, no Category=.
...
# Match EXACT identifier Hits as a whole field name
# after int32. Sibling ShotsFired is also int32 = 0.
# Do NOT accept ShotsFired. Do NOT claim leftover
# FinalScore / MedalTier.
# Siblings stay unclaimed: MissionId / bMissionSucceeded /
# ShotsFired / AircraftDamageFraction / CompletionTimeSeconds /
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 Hits = 0;`.
    # Do not accept a missing initializer, Harbor 40/80,
    # leftover ShotsFired, leftover FinalScore / MedalTier,
    # leftover MissionId, FName, or a renamed identifier.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"int32\s+Hits\s*=\s*0\s*;", compact) is None:
        return False
    if re.search(r"int32\s+Hits\s*=\s*0\s*;", compact):
        after_match = re.search(r"int32\s+Hits\b", compact)
        if after_match is None:
            return False
        after = compact[after_match.end() :]
        if re.match(r"\s*=\s*0\s*;", after) is None:
            return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+Hits\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 Hits is missing from "
...
class MissionResultHitsFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertIn(SIBLING_SHOTS, section)
        ...
        self.assertNotIn("int32 Hits)", section)
    ...
    def test_hits_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 Hits"),
            LOCKED_DECL,
        )
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_NONE}\n", LOCKED_DECL)
        self.assertIn("Hits", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_SHOTS}\n", LOCKED_DECL)
        self.assertIn("Hits", str(raised.exception))
    ...
    def test_does_not_contract_neighbors_or_director_methods(self) -> None:
        ...
        for leftover in leftover_neighbor_hit_fields():
            self.assertNotIn(leftover, LOCKED_DECL)
            self.assertFalse(
                has_declaration(f"\tint32 {leftover} = 0;\n", LOCKED_DECL)
            )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result Hits field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_hits_field_decl_contract.py"
        ))
