...
# INDEX_NONE or lock ControlServo construction
# in the .cpp. This is a FIELD contract on
# ASkyguardTempestBoss, not leftover Tempest
# lightning / gust / lock-stability methods
# (already drafted) and not leftover Tempest
...
# ControlServo;`); accept that form and other
...
# Do not lock leftover sibling Tempest
# fields PortDischargeBoom,
# StarboardDischargeBoom, EngineIntake,
# DebrisPortPanel, DebrisStarboardPanel,
# DebrisIntakePanel, or
...
# isolated-test drafts #107–#511 including
...
# leftover #107–#511, plus
# SkyguardTempestBoss production files.
# This lane only adds an isolated Python
# ControlServo field declaration contract on
# ASkyguardTempestBoss. Stay off leftover
# sibling Tempest fields, leftover Tempest
# lightning / gust / lock-stability methods,
...
# leftover isolated-test drafts #107–#511,
# leftover searchlight-track-runtime-defaults
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardTempestBoss::ControlServo",
    "SkyguardTempestBoss.cpp",
    "SetExposed",
...
class TempestControlServoFieldDeclContractTests(unittest.TestCase):
    def test_tempest_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CONTROL_SERVO_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CONTROL_SERVO_FIELD)
        self.assertIn("ControlServo", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, CONTROL_SERVO_FIELD)
    ...
    def test_missing_control_servo_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardTempestBoss();\n"
            f"\t{PORT_DISCHARGE_BOOM_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CONTROL_SERVO_FIELD)
        self.assertIn("ControlServo", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_SERVO}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CONTROL_SERVO_FIELD)
        self.assertIn("ControlServo", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_SERVO, section)
        ...
        self.assertNotIn("UPROPERTY", CONTROL_SERVO_FIELD)
        self.assertNotIn("VisibleAnywhere", CONTROL_SERVO_FIELD)
        self.assertNotIn("BlueprintReadOnly", CONTROL_SERVO_FIELD)
        self.assertNotIn("Category", CONTROL_SERVO_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_SERVO)
        self.assertNotIn("BlueprintCallable", UPROPERTY_SERVO)
        self.assertIn("Skyguard|Mission05|Boss", UPROPERTY_SERVO)
        self.assertIn("Mission05", UPROPERTY_SERVO)
        self.assertNotIn("Destruction", UPROPERTY_SERVO)
        self.assertNotIn("Encounter", UPROPERTY_SERVO)
        self.assertNotIn("Mission10", UPROPERTY_SERVO)
        self.assertNotIn("Safety", UPROPERTY_SERVO)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_SERVO)
            self.assertNotIn(invented, CONTROL_SERVO_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CONTROL_SERVO_FIELD)
        self.assertIn("ControlServo", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* ControlServo;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> ControlServo;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> ControlServo;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CONTROL_SERVO_FIELD)
            self.assertIn("ControlServo", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_control_servo_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CONTROL_SERVO_FIELD),
            CONTROL_SERVO_FIELD,
        )
        self.assertTrue(has_declaration(section, CONTROL_SERVO_FIELD))
        self.assertEqual(declaration_count(section, CONTROL_SERVO_FIELD), 1)
        self.assertTrue(
            CONTROL_SERVO_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            CONTROL_SERVO_FIELD,
        )
        self.assertTrue(CONTROL_SERVO_FIELD.endswith(";"), CONTROL_SERVO_FIELD)
        self.assertIn("ControlServo", CONTROL_SERVO_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            CONTROL_SERVO_FIELD,
        )
        self.assertIn("TObjectPtr<", CONTROL_SERVO_FIELD)
        self.assertNotIn("TArray<", CONTROL_SERVO_FIELD)
        self.assertNotIn("=", CONTROL_SERVO_FIELD)
        self.assertNotIn("INDEX_NONE", CONTROL_SERVO_FIELD)
        self.assertNotIn("UFUNCTION", CONTROL_SERVO_FIELD)
        self.assertNotIn("{", CONTROL_SERVO_FIELD)
        self.assertNotIn("}", CONTROL_SERVO_FIELD)
        self.assertNotIn("return ", CONTROL_SERVO_FIELD)
        self.assertNotIn("CommandAntenna", CONTROL_SERVO_FIELD)
        self.assertNotIn("ControlLinkage", CONTROL_SERVO_FIELD)
        self.assertNotIn("NoseCamera", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisCenter", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisTail", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisNose", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisSpine", CONTROL_SERVO_FIELD)
        self.assertNotIn("AdvanceEncounter", CONTROL_SERVO_FIELD)
        self.assertNotIn("ResetEncounterState", CONTROL_SERVO_FIELD)
        self.assertNotIn("IsRouteStateSafe", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetRouteProgress", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", CONTROL_SERVO_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", CONTROL_SERVO_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", CONTROL_SERVO_FIELD)
        self.assertNotIn("RadarNode", CONTROL_SERVO_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", CONTROL_SERVO_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", CONTROL_SERVO_FIELD)
        self.assertNotIn("IssueClimbCommand", CONTROL_SERVO_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", CONTROL_SERVO_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetFinaleStage", CONTROL_SERVO_FIELD)
        self.assertNotIn("IsClimbCommandIssued", CONTROL_SERVO_FIELD)
        self.assertNotIn("IsWreckDiverted", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", CONTROL_SERVO_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetGunnerMount", CONTROL_SERVO_FIELD)
        self.assertNotIn("FaceWorldLocation", CONTROL_SERVO_FIELD)
        self.assertNotIn("PortGuidanceArray", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisArmorPort", CONTROL_SERVO_FIELD)
        self.assertNotIn("SetSearchlightTracked", CONTROL_SERVO_FIELD)
        self.assertNotIn("SetContactIdentified", CONTROL_SERVO_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", CONTROL_SERVO_FIELD)
        self.assertNotIn("SetLightningExposed", CONTROL_SERVO_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", CONTROL_SERVO_FIELD)
        self.assertNotIn("HullCollider", CONTROL_SERVO_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, CONTROL_SERVO_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_SERVO} {CONTROL_SERVO_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, CONTROL_SERVO_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{CONTROL_SERVO_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, CONTROL_SERVO_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CONTROL_SERVO_FIELD),
            section,
    ...
    def test_assigned_control_servo_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "ControlServo = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, CONTROL_SERVO_FIELD)
        self.assertIn("ControlServo", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, CONTROL_SERVO_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("ControlServo", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, CONTROL_SERVO_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{CONTROL_SERVO_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, CONTROL_SERVO_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_SERVO, section)
        self.assertTrue(has_declaration(section, CONTROL_SERVO_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            CONTROL_SERVO_FIELD.startswith("UFUNCTION"),
            CONTROL_SERVO_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CONTROL_SERVO_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CONTROL_SERVO_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_control_servo_cpp_body(self) -> None:
        locked_only = f"{CONTROL_SERVO_FIELD}\n"
        self.assertNotIn("{", CONTROL_SERVO_FIELD)
        self.assertNotIn("}", CONTROL_SERVO_FIELD)
        self.assertNotIn("return ", CONTROL_SERVO_FIELD)
        ...
        self.assertNotIn("SkyguardTempestBoss.cpp", CONTROL_SERVO_FIELD)
        self.assertNotIn("SkyguardTempestBoss.cpp", locked_only)
        self.assertNotIn("return false", CONTROL_SERVO_FIELD)
        self.assertNotIn("return true", CONTROL_SERVO_FIELD)
        self.assertNotIn("CreateDefaultSubobject", CONTROL_SERVO_FIELD)
        self.assertNotIn("SetExposed", CONTROL_SERVO_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            CONTROL_SERVO_FIELD,
        )
        self.assertNotIn("SetBossPhase", CONTROL_SERVO_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", CONTROL_SERVO_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{CONTROL_SERVO_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("PortDischargeBoom", CONTROL_SERVO_FIELD)
        self.assertNotIn("StarboardDischargeBoom", CONTROL_SERVO_FIELD)
        self.assertNotIn("EngineIntake", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisPortPanel", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisStarboardPanel", CONTROL_SERVO_FIELD)
        self.assertNotIn("DebrisIntakePanel", CONTROL_SERVO_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", CONTROL_SERVO_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            CONTROL_SERVO_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("EncounterController", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), CONTROL_SERVO_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), CONTROL_SERVO_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), CONTROL_SERVO_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), CONTROL_SERVO_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), CONTROL_SERVO_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, CONTROL_SERVO_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("SetSearchlightTracked", CONTROL_SERVO_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("SetContactIdentified", CONTROL_SERVO_FIELD)
        self.assertNotIn("OpenOrbitExposure", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", CONTROL_SERVO_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("SetLightningExposed", CONTROL_SERVO_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", CONTROL_SERVO_FIELD)
        self.assertNotIn("IsLightningExposed", CONTROL_SERVO_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", CONTROL_SERVO_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), CONTROL_SERVO_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), CONTROL_SERVO_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", CONTROL_SERVO_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, CONTROL_SERVO_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("ASkyguardGunner", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("SkyguardRadarNode", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, CONTROL_SERVO_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", CONTROL_SERVO_FIELD)
        self.assertNotIn("MaxIntegrity", CONTROL_SERVO_FIELD)
        self.assertNotIn("CurrentIntegrity", CONTROL_SERVO_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", CONTROL_SERVO_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            CONTROL_SERVO_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, CONTROL_SERVO_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", CONTROL_SERVO_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CONTROL_SERVO_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, CONTROL_SERVO_FIELD),
            CONTROL_SERVO_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardTempestBoss::ControlServo", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "tempest ControlServo field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{CONTROL_SERVO_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, CONTROL_SERVO_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                CONTROL_SERVO_FIELD.lower(),
                "tempest ControlServo contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CONTROL_SERVO_FIELD)

    def test_contract_is_control_servo_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(CONTROL_SERVO_FIELD.endswith(";"))
        self.assertNotIn("=", CONTROL_SERVO_FIELD)
        self.assertIn(UPROPERTY_SERVO, section)
