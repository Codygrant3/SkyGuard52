...
# CoveredSeconds FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission08 HoistWindowRuntime
# CoveredSeconds FIELD (declaration-only). THIS IS NOT leftover
...
# CoveredSeconds FIELD (declaration-only). It is
...
# This HoistWindowRuntime field sits after RemainingSeconds and before
# CompletedWindows in the public section.
# Skip leftover b + Ya + kRuntimeReady forever (it sits
# after bMapAssemblyReady and before bGunnerReady). This contract
# is CoveredSeconds after RemainingSeconds and before CompletedWindows.
# THIS IS NOT leftover RemainingSeconds. THIS IS NOT leftover
...
COVERED_SECONDS_WRONG = (
    "float CoveredSeconds = 1.f;"
)
LOCKED_DECL = COVERED_SECONDS
...
# an isolated Python CoveredSeconds
...
class Mission08HoistWindowRuntimeCoveredSecondsFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("CoveredSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_covered_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        leftover_bool = "\tbool CoveredSeconds = false;\n"
        leftover_enum = (
            "\tESkyguardMission08ProtectedTarget CoveredSeconds = "
            "ESkyguardMission08ProtectedTarget::RescueHelicopter;\n"
        )
        leftover_zero = f"\t{COVERED_SECONDS_WRONG}\n"
    ...
    def test_covered_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CoveredSeconds"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("CoveredSeconds", LOCKED_DECL)
        ...
        self.assertNotIn("CompletedWindows", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn("ClampMin", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("CoveredSeconds", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float CoveredSeconds"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("CoveredSeconds", locked_only)
    ...
    def test_contract_is_covered_seconds_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_covered_seconds(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission08_hoist_window_runtime_covered_seconds"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
