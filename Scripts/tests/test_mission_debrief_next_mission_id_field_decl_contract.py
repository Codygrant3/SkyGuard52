# THIS IS leftover-safe FSkyguardMissionDebrief::NextMissionId.
# origin/main form: split-line UPROPERTY(VisibleAnywhere,
# BlueprintReadOnly) then `FName NextMissionId;` with no
# Category= and no initializer.
# If a clone asserts `= false` / `= 0` / `= NAME_None` / an
...
# no Category=. Accept bare `FName NextMissionId;`. Fail-closed if
...
# MissionDisplayName / Narrative / bNewBestScore / bNewBestMedal /
# bProgressSaved / SaveSlotName siblings.
...
TARGET = "FName NextMissionId;"
TARGET_WRONG_ZERO = "FName NextMissionId = 0;"
TARGET_WRONG_FALSE = "FName NextMissionId = false;"
TARGET_WRONG_ONE = "FName NextMissionId = 1;"
TARGET_WRONG_FLOAT = "FName NextMissionId = 0.f;"
TARGET_WRONG_TRUE = "FName NextMissionId = true;"
TARGET_WRONG_INIT = "FName NextMissionId = FName();"
TARGET_WRONG_NAME_NONE = "FName NextMissionId = NAME_None;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # = false / = 1 / = 0.f / = true / FName() / NAME_None for
    # FSkyguardMissionDebrief::NextMissionId.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"FName\s+NextMissionId\s*=", compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    field = re.search(r"FName\s+NextMissionId\b", compact)
    if field is None:
        raise AssertionError(
            "UPROPERTY for FName NextMissionId is missing from "
...
class MissionDebriefNextMissionIdFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("NextMissionId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_next_mission_id_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
    ...
    def test_next_mission_id_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName NextMissionId"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "FName NextMissionId;")
        ...
        self.assertIn("NextMissionId", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_NAME_NONE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief NextMissionId field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_next_mission_id"
            "_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertIn("NextMissionId", locked_only)
