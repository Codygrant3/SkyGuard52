...
# ASkyguardMission01EnvironmentDirector Root, not
# leftover Mission01 Integration Root #574
# (ASkyguardMission01IntegrationDirector::Root,
# category Skyguard|Mission01) and not leftover
# BossDrone Root #471. Do not lock sibling
# environment fields OceanTiles / BeachTiles /
# LandTiles / RouteExclusion / LandScatterBounds /
# InlandVegetationPCG / ProductionLandscape /
# AuthoredPCGGraph. Do not lock leftover
# bLicensedVegetationLibraryApproved /
# bAllowAuthoredPCGGeneration /
# bProductionLandscapeBound. Do not lock leftover
# IsRouteExclusionSafe / bRouteExclusionValid,
# leftover environment-readiness defaults (#6b9d /
# #b931), leftover GetReadiness, leftover Mission01
# Integration director fields (#568/#572/#574-#589),
...
# (`TObjectPtr<USceneComponent> Root;`); accept
# that form and other one-line / split-line wraps.
# Nearby origin/main UPROPERTY(VisibleAnywhere,
# BlueprintReadOnly,
...
# Stay off leftover drafts #56–#64 / #107–#589,
...
ALLOW_AUTHORED_PCG_FIELD = (
    "bool bAllowAuthoredPCGGeneration = false;"
)
PRODUCTION_LANDSCAPE_BOUND = "bProductionLandscapeBound"
IS_ROUTE_EXCLUSION_SAFE = "bool IsRouteExclusionSafe() const;"
ROUTE_EXCLUSION_VALID = "bRouteExclusionValid"
...
# environment-director Root field declaration
# contract on ASkyguardMission01EnvironmentDirector.
# Keep distinct from leftover Integration Root
# #574 filename test_mission01_root_field_decl
# _contract.py and leftover BossDrone Root #471.
LOCKED = {
...
# RouteExclusion / LandScatterBounds /
# InlandVegetationPCG / ProductionLandscape /
# AuthoredPCGGraph field contracts stay unlocked by
# this file. Leftover Mission01 Integration Root
# #574 (test_mission01_root_field_decl_contract.py)
# and leftover Mission01 Integration field
# contracts (#568/#572/#574-#589), leftover
...
SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED = (
    ...
    "test_mission01_land_scatter_bounds_field_decl_contract.py",
    "test_mission01_inland_vegetation_pcg_field_decl_contract.py",
    "test_mission01_production_landscape_field_decl_contract.py",
    "test_mission01_authored_pcg_graph_field_decl_contract.py",
)
...
LEFTOVER_ENVIRONMENT_FLAGS_NOT_LOCKED = (
    "bLicensedVegetationLibraryApproved",
    "bAllowAuthoredPCGGeneration",
    "bProductionLandscapeBound",
    LICENSED_VEGETATION_FIELD,
    ALLOW_AUTHORED_PCG_FIELD,
    PRODUCTION_LANDSCAPE_BOUND,
    IS_ROUTE_EXCLUSION_SAFE,
    ROUTE_EXCLUSION_VALID,
    "IsRouteExclusionSafe",
)
LEFTOVER_INTEGRATION_ROOT_NOT_LOCKED = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::Root",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01EnvironmentRootFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ROOT_FIELD),
            section,
        )
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        as_array = (
            "\tTArray<TObjectPtr<USceneComponent>> Root;\n"
        )
        hism = (
            "\tTObjectPtr<"
        ...
        soft = (
            "\tTSoftObjectPtr<USceneComponent> Root;\n"
        )
        renamed = (
            "\tTObjectPtr<USceneComponent> EnvironmentRoot;\n"
        )
    ...
    def test_root_declaration_matches_origin_main(self) -> None:
        ...
        self.assertEqual(
            declaration_count(section, ROOT_FIELD),
            1,
        )
        self.assertTrue(
            ROOT_FIELD.startswith(
        ...
        self.assertNotIn("OceanTiles", ROOT_FIELD)
        self.assertNotIn("BeachTiles", ROOT_FIELD)
        self.assertNotIn("LandTiles", ROOT_FIELD)
        self.assertNotIn("RouteExclusion", ROOT_FIELD)
        self.assertNotIn("LandScatterBounds", ROOT_FIELD)
        self.assertNotIn("InlandVegetationPCG", ROOT_FIELD)
        self.assertNotIn("ProductionLandscape", ROOT_FIELD)
        self.assertNotIn("AuthoredPCGGraph", ROOT_FIELD)
        self.assertNotIn("GetReadiness", ROOT_FIELD)
        ...
        self.assertNotIn("bAllowBoundedActorSpawning", ROOT_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", ROOT_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_ENVIRONMENT} {ROOT_FIELD}\n"
            "};\n"
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardBossDrone", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 environment Root field contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ROOT_FIELD.lower(),
                "mission01 environment Root contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_is_environment_root_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
    ...
    def test_sibling_environment_fields_do_not_satisfy_root(
        self,
    ) -> None:
        for leftover in (
            OCEAN_TILES_FIELD,
    ...
    def test_contract_is_distinct_from_leftover_integration_root(
        self,
    ) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        self.assertEqual(
        ...
        self.assertEqual(CLASS_NAME, "ASkyguardMission01EnvironmentDirector")
        self.assertNotEqual(
            CLASS_NAME,
            "ASkyguardMission01IntegrationDirector",
        )
        ...
        self.assertEqual(
            Path(__file__).name,
            "test_mission01_environment_root_field_decl_contract.py",
        )
        for token in LEFTOVER_INTEGRATION_ROOT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROOT_FIELD)

    def test_contract_is_distinct_from_leftover_boss_drone_root(self) -> None:
        locked_only = f"{ROOT_FIELD}\n"
    ...
    def test_contract_does_not_relock_leftover_environment_flags(
        self,
    ) -> None:
        locked_only = f"{ROOT_FIELD}\n"
        for token in LEFTOVER_ENVIRONMENT_FLAGS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROOT_FIELD)
