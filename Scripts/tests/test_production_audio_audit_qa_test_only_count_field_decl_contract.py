...
# THIS IS NOT leftover BoundProductionSourceCount #1412 or
# leftover production-audio-audit-defaults #196 / #5e51 analog
# (this is the isolated field decl).
...
TARGET = "int32 QATestOnlyCount = 0;"
TARGET_WRONG_BARE = "int32 QATestOnlyCount;"
TARGET_WRONG_FALSE = "int32 QATestOnlyCount = false;"
TARGET_WRONG_TRUE = "int32 QATestOnlyCount = true;"
TARGET_WRONG_ZERO = "int32 QATestOnlyCount = 0.f;"
TARGET_WRONG_FORTY = "int32 QATestOnlyCount = 40;"
TARGET_WRONG_EIGHTY = "int32 QATestOnlyCount = 80;"
TARGET_WRONG_FLOAT = "float QATestOnlyCount = 0;"
TARGET_WRONG_FNAME = "FName QATestOnlyCount;"
TARGET_WRONG_BOOL = "bool QATestOnlyCount = false;"
TARGET_WRONG_FSTRING = "FString QATestOnlyCount;"
TARGET_WRONG_TARRAY = "TArray<FName> QATestOnlyCount;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> QATestOnlyCount;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_production_audio_audit_qa"
    "_test_only_count_field_decl_contract.py"
)
...
def leftover_entry_routing_isolated_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        prefix + "test_production_audio_routing_cockpit_low_pass"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 QATestOnlyCount = 0;`.
    ...
    if re.search(
        ...
        return False
    if re.search(
        r"QATestOnlyCount\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"QATestOnlyCount\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"QATestOnlyCount\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"QATestOnlyCount\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"QATestOnlyCount\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+QATestOnlyCount\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+QATestOnlyCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+QATestOnlyCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+QATestOnlyCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+QATestOnlyCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+QATestOnlyCount\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+QATestOnlyCount\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+QATestOnlyCount\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 QATestOnlyCount is missing from "
...
class ProductionAudioAuditQATestOnlyCountFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_qa_test_only_count_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_drop_or_harbor_change_fails_closed(self) -> None:
        ...
        self.assertIn("int32 QATestOnlyCount = 0;", compact_origin)
        self.assertNotIn("QATestOnlyCount = 40", compact_origin)
        self.assertNotIn("QATestOnlyCount = 80", compact_origin)
        self.assertNotIn("QATestOnlyCount = false", compact_origin)
        self.assertNotIn("QATestOnlyCount;", compact_origin.replace(
            "int32 QATestOnlyCount = 0;", ""
        ))

    def test_qa_test_only_count_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 QATestOnlyCount"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tint32 QATestOnlyCounts = 0;\n",
            "\tint32 QATestOnlyCount = 1;\n",
            "\tbool QATestOnlyCount;\n",
            "\tfloat QATestOnlyCount = " + forty + ";\n",
            "\tfloat QATestOnlyCount = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit QATestOnlyCount field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "qa_test_only_count_field_decl_contract.py"
        ))
