# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot RocketReady.
...
# Exact identifier vs siblings CannonReady, GuidedReady,
# Score, Medal, ShotsFired, and Hits
...
TARGET = "int32 RocketReady = 0;"
TARGET_WRONG_NONE = "int32 RocketReady;"
TARGET_WRONG_FALSE = "int32 RocketReady = false;"
TARGET_WRONG_TRUE = "int32 RocketReady = true;"
TARGET_WRONG_FLOAT = "int32 RocketReady = 0.f;"
TARGET_WRONG_HUNDRED = "int32 RocketReady = 100;"
TARGET_WRONG_ONE = "int32 RocketReady = 1;"
...
TARGET_WRONG_CANNON = "int32 CannonReady = 0;"
TARGET_WRONG_GUIDED = "int32 GuidedReady = 0;"
TARGET_WRONG_FNAME = "FName RocketReady;"
TARGET_WRONG_BOOL = "bool RocketReady = 0;"
TARGET_WRONG_FLOAT_TYPE = "float RocketReady = 0;"
LOCKED_DECL = TARGET
...
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_RADAR_DEAD = (
    "Scripts/tests/test_cpg_debrief_snapshot_radar_dead"
    "_field_decl_contract.py"
)
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DESTROYED_SYSTEMS = (
    "Scripts/tests/test_cpg_debrief_snapshot_destroyed_systems"
    "_field_decl_contract.py"
)
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_SELECTED_LOADOUT = (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 RocketReady = 0;`.
    # Do not accept missing initializer, `= 0.f`, `= false`,
    # `= 100`, sibling CannonReady / GuidedReady / Score /
    ...
    if re.search(r"int32\s+RocketReady\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"int32\s+RocketReady\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+RocketReady\s*=\s*true\b", compact):
        return False
    if re.search(r"int32\s+RocketReady\s*=\s*100\b", compact):
        return False
    if re.search(r"(?:float|bool|FName|FString)\s+RocketReady\b", compact):
        return False
    if re.search(r"int32\s+RocketReady\s*=\s*0\s*;", compact) is None:
        return False
    return True
...
class CpgDebriefSnapshotRocketReadyFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RocketReady"), section)
    ...
    def test_capture_function_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RocketReady"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        ...
        self.assertIn("RocketReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_rocket_ready_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_rocket_ready_is_plain_int32_zero(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
        ...
        self.assertEqual(LOCKED_DECL, "int32 RocketReady = 0;")
        self.assertTrue(LOCKED_DECL.startswith("int32 RocketReady"), LOCKED_DECL)
        ...
        self.assertIn("int32 RocketReady = 0;", compact)
        self.assertNotIn("RocketReady = 0.f", compact)
        self.assertNotIn("RocketReady = false", compact)
        self.assertNotIn("RocketReady = 100", compact)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 RocketReadys = 0;\n",
            "\tint32 RocketReadyValue = 0;\n",
            "\tint32 RocketReadyCount = 0;\n",
            "\tfloat RocketReady = " + forty + ";\n",
            "\tfloat RocketReady = " + eighty + ";\n",
        )
    ...
    def test_exact_identifier_vs_same_type_siblings(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RocketReady"), section)
        self.assertIn("RocketReady", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot RocketReady field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "rocket_ready_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_RADAR_DEAD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DESTROYED_SYSTEMS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_SELECTED_LOADOUT, LOCKED_SCRIPTS)
