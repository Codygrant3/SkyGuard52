...
# bValid FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bValid = false; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeHeightSample
# bValid FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bValid = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# BlueprintReadOnly bool bValid = false;
...
# bValid FIELD (declaration-only). It is
...
TARGET = "bool bValid = false;"
TARGET_WRONG = "bool bValid = true;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bValid is missing from "
...
class LandscapeHeightSampleValidFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bValid", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_pcg_generation_authorized_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_pcg_generation_authorized_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bValid"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bValid", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bValid", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bValid"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bValid", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_pcg_generation_authorized(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
