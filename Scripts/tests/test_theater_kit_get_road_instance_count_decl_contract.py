...
# then declaration-only `int32 GetRoadInstanceCount() const;`
...
# GetAppliedSpec / GetBuildingInstanceCount /
# GetLampInstanceCount / GetSilhouetteInstanceCount /
# ApplyTheaterKit stay uncontracted.
# Do NOT contract leftover RoadInstances UPROPERTY #1317.
...
# THIS IS NOT leftover RoadInstances field #1317.
...
TARGET = "int32 GetRoadInstanceCount() const;"
TARGET_WRONG_EQ_NONE = "int32 GetRoadInstanceCount() = NAME_None;"
TARGET_WRONG_FALSE = "int32 GetRoadInstanceCount() = false;"
TARGET_WRONG_TRUE = "int32 GetRoadInstanceCount() = true;"
TARGET_WRONG_ZERO = "int32 GetRoadInstanceCount() = 0.f;"
TARGET_WRONG_FLOAT = "float GetRoadInstanceCount() const;"
TARGET_WRONG_FNAME = "FName GetRoadInstanceCount() const;"
TARGET_WRONG_FLINEAR = "FLinearColor GetRoadInstanceCount() const;"
TARGET_WRONG_NO_CONST = "int32 GetRoadInstanceCount();"
TARGET_WRONG_BODY = (
    "int32 GetRoadInstanceCount() const "
...
GET_ROAD_INSTANCE_COUNT_RE = re.compile(
    r"int32\s+GetRoadInstanceCount\s*\(\s*\)\s*const\s*;"
)
GET_ROAD_INSTANCE_COUNT_NO_CONST_RE = re.compile(
    r"int32\s+GetRoadInstanceCount\s*\(\s*\)\s*(?!const)"
)
GET_ROAD_INSTANCE_COUNT_WRONG_TYPE_RE = re.compile(
    r"(?:FName|float|FLinearColor|bool|void)\s+GetRoadInstanceCount\b"
)
GET_ROAD_INSTANCE_COUNT_BODY_RE = re.compile(
    r"int32\s+GetRoadInstanceCount\s*\(\s*\)\s*const"
    r"\s*\{"
)
INVENTED_UPROPERTY = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # GetBuildingInstanceCount UFUNCTIONs.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GET_ROAD_INSTANCE_COUNT_WRONG_TYPE_RE.search(compact):
        return False
    if GET_ROAD_INSTANCE_COUNT_BODY_RE.search(compact):
        return False
    if GET_ROAD_INSTANCE_COUNT_RE.search(compact) is None:
        return False
    return True
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetRoadInstanceCount\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for int32 GetRoadInstanceCount is missing "
...
class TheaterKitGetRoadInstanceCountDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetRoadInstanceCount"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetRoadInstanceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_road_instance_count_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
            f"\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            f"{SIBLING_ROAD_INSTANCES};\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertIsNone(GET_ROAD_INSTANCE_COUNT_BODY_RE.search(collapsed(section)))
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertIsNotNone(GET_ROAD_INSTANCE_COUNT_RE.search(compact_origin))
        self.assertIsNone(GET_ROAD_INSTANCE_COUNT_BODY_RE.search(compact_origin))
        self.assertNotIn("FName GetRoadInstanceCount", compact_origin)
        self.assertNotIn("float GetRoadInstanceCount", compact_origin)
        self.assertNotIn("FLinearColor GetRoadInstanceCount", compact_origin)

    def test_get_road_instance_count_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GetRoadInstanceCount"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            f"{SIBLING_ROAD_INSTANCES};\n",
            "\tint32 GetRoadInstanceCounts() const;\n",
            ...
            "\tbool GetRoadInstanceCount() const;\n",
            "\tFName GetRoadInstanceCount() const;\n",
            f"\t{TARGET_WRONG_BODY}\n",
            f"\t{TARGET_WRONG_SPEC_FIELD}\n",
            "\tfloat GetRoadInstanceCount() const { return "
            + forty
            + "; }\n",
            "\tfloat GetRoadInstanceCount() const { return "
            + eighty
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tint32\n\tGetRoadInstanceCount() const;\n",
            "\tint32   GetRoadInstanceCount() const;\n",
            "\tint32\tGetRoadInstanceCount() const;\n",
            "\tint32 GetRoadInstanceCount(\n\t) const;\n",
            "\tint32 GetRoadInstanceCount()  const;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UFUNCTION_PURE_THEATER}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetRoadInstanceCount decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "get_road_instance_count_decl_contract.py"
        ))
