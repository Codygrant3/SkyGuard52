...
# then `bool bCategoryContractComplete = false;`.
...
# type is bool, REQUIRED initializer `= false`.
...
TARGET = "bool bCategoryContractComplete = false;"
TARGET_WRONG_BARE = "bool bCategoryContractComplete;"
TARGET_WRONG_TRUE = "bool bCategoryContractComplete = true;"
TARGET_WRONG_ZERO = "bool bCategoryContractComplete = 0;"
TARGET_WRONG_FORTY = "bool bCategoryContractComplete = 40;"
TARGET_WRONG_EIGHTY = "bool bCategoryContractComplete = 80;"
TARGET_WRONG_FLOAT = "float bCategoryContractComplete = 0;"
TARGET_WRONG_FNAME = "FName bCategoryContractComplete;"
TARGET_WRONG_INT32 = "int32 bCategoryContractComplete = 0;"
TARGET_WRONG_FSTRING = "FString bCategoryContractComplete;"
TARGET_WRONG_TARRAY = "TArray<FName> bCategoryContractComplete;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> bCategoryContractComplete;"
...
FIELD_NAME = "bCategoryContractComplete"
...
THIS_SCRIPT = (
    "Scripts/tests/test_production_audio_audit_category"
    "_contract_complete_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `bool bCategoryContractComplete = false;`.
    ...
    if re.search(
        r"bool\s+bCategoryContractComplete\s*;",
        compact,
    ) and re.search(
        r"bool\s+bCategoryContractComplete\s*=",
        compact,
        ...
        return False
    if re.search(
        r"bCategoryContractComplete\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"bCategoryContractComplete\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"bCategoryContractComplete\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"bCategoryContractComplete\s*=\s*0\b",
        compact,
    ):
        return False
    if re.search(
        r"bCategoryContractComplete\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bCategoryContractComplete\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bCategoryContractComplete\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bCategoryContractComplete\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+bCategoryContractComplete\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+bCategoryContractComplete\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+bCategoryContractComplete\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+bCategoryContractComplete\s*=\s*false\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bCategoryContractComplete\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bCategoryContractComplete is missing from "
...
class ProductionAudioAuditCategoryContractCompleteFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_category_contract_complete_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_drop_or_harbor_change_fails_closed(self) -> None:
        ...
        self.assertIn("bool bCategoryContractComplete = false;", compact_origin)
        self.assertNotIn("bCategoryContractComplete = 40", compact_origin)
        self.assertNotIn("bCategoryContractComplete = 80", compact_origin)
        self.assertNotIn("bCategoryContractComplete = true", compact_origin)
        self.assertNotIn("bCategoryContractComplete = 0;", compact_origin)
        self.assertNotIn("bCategoryContractComplete;", compact_origin.replace(
            "bool bCategoryContractComplete = false;", ""
        ))

    def test_category_contract_complete_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bCategoryContractComplete"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tbool bCategoryContractCompletes = false;\n",
            "\tbool bCategoryContractComplete = true;\n",
            "\tint32 bCategoryContractComplete = 0;\n",
            "\tfloat bCategoryContractComplete = " + forty + ";\n",
            "\tfloat bCategoryContractComplete = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit category contract complete field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "category_contract_complete_field_decl_contract.py"
        ))
