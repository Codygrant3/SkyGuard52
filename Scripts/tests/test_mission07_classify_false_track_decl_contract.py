...
# INDEX_NONE or lock ClassifyFalseTrack
# construction in the .cpp. This is leftover-safe
# Mission07 Search ClassifyFalseTrack METHOD
...
CPP_AND_INVENTED = (
    ...
    "ASkyguardMission07IntegrationDirector::ClassifyFalseTrack",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission07ClassifyFalseTrackDeclContractTests(unittest.TestCase):
    def test_mission07_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CLASSIFY_FALSE_TRACK),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CLASSIFY_FALSE_TRACK)
        self.assertIn("ClassifyFalseTrack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, CLASSIFY_FALSE_TRACK)
    ...
    def test_missing_classify_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission07IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CLASSIFY_FALSE_TRACK)
        self.assertIn("ClassifyFalseTrack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CLASSIFY_FALSE_TRACK)
        self.assertIn("ClassifyFalseTrack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("BlueprintCallable", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("Category", CLASSIFY_FALSE_TRACK)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, CLASSIFY_FALSE_TRACK)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CLASSIFY_FALSE_TRACK)
        self.assertIn("ClassifyFalseTrack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid ClassifyFalseTrack(FName TrackId);\n"
        as_const = "\tbool ClassifyFalseTrack(FName TrackId) const;\n"
        with_arg = "\tbool ClassifyFalseTrack(int32 TrackId);\n"
        renamed = "\tbool ClassifyTrueTrack(FName TrackId);\n"
        short_name = "\tbool ClassifyTrack(FName TrackId);\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CLASSIFY_FALSE_TRACK)
            self.assertIn("ClassifyFalseTrack", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_classify_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CLASSIFY_FALSE_TRACK),
            CLASSIFY_FALSE_TRACK,
        )
        self.assertTrue(has_declaration(section, CLASSIFY_FALSE_TRACK))
        self.assertEqual(
            declaration_count(section, CLASSIFY_FALSE_TRACK),
            1,
        )
        self.assertTrue(
            CLASSIFY_FALSE_TRACK.startswith("bool "),
            CLASSIFY_FALSE_TRACK,
        )
        self.assertTrue(
            CLASSIFY_FALSE_TRACK.endswith(";"),
            CLASSIFY_FALSE_TRACK,
        )
        self.assertIn("ClassifyFalseTrack", CLASSIFY_FALSE_TRACK)
        self.assertIn("FName TrackId", CLASSIFY_FALSE_TRACK)
        self.assertIn("(", CLASSIFY_FALSE_TRACK)
        self.assertIn(")", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("TArray<", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("TObjectPtr", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("TSoftObjectPtr", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("INDEX_NONE", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("UFUNCTION", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("{", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("}", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("return ", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("const;", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("Root", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("Briefing", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AudioDirector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("RadioChatter", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("SortiePresentation", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("CampaignDefinition", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("MissionDefinition", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("Readiness", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("bAutoInitialize", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("bAllowBoundedActorSpawning", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("bAutoLaunchAfterBriefing", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("PathfinderSpawnLocation", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetAircraft", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("BindRuntimeActors", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("HandleDroneCityImpact", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("StartNextWave", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("NotifyThreatDestroyed", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AdvanceConvoyByDistance", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("NotifyConvoyDamage", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetWaveState", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetConvoyRouteState", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetDayBeatKit", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetNightBeatKit", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("StartSearchlightWindow", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AdvanceSearchlightTrack", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("NotifySubstationDamage", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetRemainingThreatsInWave", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetSearchlightRuntime", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetSubstationIntegrity", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("StartPayloadWindow", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AdvancePayloadWindow", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("TryJamActivePayload", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("NotifyAirfieldTargetDamage", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetPayloadWindow", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetTargetRuntime", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetSurvivingTargetCount", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetDayBeatKind", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetDayBeatIndex", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("TickDayBeatKit", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("HandleBossPhaseChanged", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ESkyguardMission06WaveState", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ESkyguardMission07WaveState", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ESkyguardSearchSector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("FSkyguardSearchTrackRuntime", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("InitializePlayableMission", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ConfirmRadarGhostIdentification", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("NotifyProtectedTargetDamage", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AdvanceReinforcementTimer", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetSearchSector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetClassifiedFalseTrackCount", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("IsHostileContactConfirmed", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetProtectedTarget", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetReinforcementTimeRemaining", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetNightBeatKind", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetNightBeatIndex", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("TickNightBeatKit", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ESkyguardAirfieldTarget", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ConvoyRuntimeAnchor", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("RoadHunterSpawnLocation", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ConfigureMissionDefinition", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("NotifyObjectiveProgress", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("NotifyProtectedAssetFailed", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("SynchronizeRuntimeState", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("IsCorePlayableReady", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetReadiness", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetObjectiveRuntime", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetGunner", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetPathfinder", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetMissionId", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ValidateMissionContract", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ConfigureFromMission", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AdvanceBriefing", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("SetAssetsReady", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AcknowledgeAndLaunch", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("CanLaunch", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetElapsedSeconds", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetBriefingState", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetMinimumWarmupSeconds", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetBriefingText", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetRadioChatter", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetPresentation", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("GetMissionTitle", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("AcknowledgeBriefing", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("LaunchSortie", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("HullCollider", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("OpticalTracker", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("WeaponServo", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("CountermeasurePod", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("MinHeightFromOriginCm", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("RadarNode", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ESkyguardMission02WaveState", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("HarborIndustrial", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("MaxIntegrity", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("CurrentIntegrity", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("FillAndFinalize", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("FillAndFail", CLASSIFY_FALSE_TRACK)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, CLASSIFY_FALSE_TRACK)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tClassifyFalseTrack(FName TrackId);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SEARCH} {CLASSIFY_FALSE_TRACK}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{CLASSIFY_FALSE_TRACK}\n}}\n"
        self.assertTrue(has_declaration(one_line, CLASSIFY_FALSE_TRACK))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CLASSIFY_FALSE_TRACK),
            section,
    ...
    def test_environment_category_does_not_satisfy_search(self) -> None:
        ...
        self.assertNotEqual(leftover_m07_waves, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m07_protection, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m07_objectives, UFUNCTION_SEARCH)
        self.assertNotEqual(leftover_m07_boss, UFUNCTION_SEARCH)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{CLASSIFY_FALSE_TRACK}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, CLASSIFY_FALSE_TRACK)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CLASSIFY_FALSE_TRACK)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_classify_cpp_body(self) -> None:
        locked_only = f"{CLASSIFY_FALSE_TRACK}\n"
        self.assertNotIn("{", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("}", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("return ", CLASSIFY_FALSE_TRACK)
        ...
        self.assertNotIn("return false", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("return true", CLASSIFY_FALSE_TRACK)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{CLASSIFY_FALSE_TRACK}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CLASSIFY_FALSE_TRACK)
        self.assertIn("ClassifyFalseTrack", CLASSIFY_FALSE_TRACK)
        self.assertTrue(CLASSIFY_FALSE_TRACK.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, CLASSIFY_FALSE_TRACK)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, CLASSIFY_FALSE_TRACK)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("CurrentIntegrity", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("SkyguardApacheAircraft.h", CLASSIFY_FALSE_TRACK)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", CLASSIFY_FALSE_TRACK)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CLASSIFY_FALSE_TRACK}\n"
        self.assertEqual(
            require_declaration(locked_only, CLASSIFY_FALSE_TRACK),
            CLASSIFY_FALSE_TRACK,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CLASSIFY_FALSE_TRACK)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", CLASSIFY_FALSE_TRACK)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", CLASSIFY_FALSE_TRACK)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 ClassifyFalseTrack method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{CLASSIFY_FALSE_TRACK}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, CLASSIFY_FALSE_TRACK.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                CLASSIFY_FALSE_TRACK.lower(),
                "mission07 ClassifyFalseTrack contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CLASSIFY_FALSE_TRACK)

    def test_contract_is_classify_false_track_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(CLASSIFY_FALSE_TRACK.startswith("bool "))
        self.assertIn("ClassifyFalseTrack", CLASSIFY_FALSE_TRACK)
        self.assertTrue(CLASSIFY_FALSE_TRACK.endswith(";"))
        self.assertIn(UFUNCTION_SEARCH, section)

    def test_sibling_director_fields_do_not_satisfy_classify(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, CLASSIFY_FALSE_TRACK)
            )
            self.assertNotEqual(CLASSIFY_FALSE_TRACK, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("ClassifyFalseTrack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, CLASSIFY_FALSE_TRACK))
        locked_only = f"{CLASSIFY_FALSE_TRACK}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool ClassifyFalseTrack(FName TrackId) { return false; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_classify_false_track"
                "_decl_contract.py"
            )
        )
        self.assertIn(
