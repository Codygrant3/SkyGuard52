...
# GetBossPhase. Stay off leftover Telemetry
# UPROPERTY field. Stay off leftover
# FSkyguardBossTelemetry defaults #48e1 (return
# type stays on the locked declaration; do not
# add a separate struct-defaults contract). Stay
# off leftover ESkyguardBossPhase enum #60cb.
...
# isolated-test drafts #107–#466 including leftover
...
# tokens. Harbor clock names may be scanned in
# this relevant public section and must be
# absent. LastFlight
...
# Python GetTelemetry declaration
...
# GetBossPhase, leftover Telemetry UPROPERTY,
# leftover FSkyguardBossTelemetry defaults #48e1,
# leftover ESkyguardBossPhase enum #60cb, leftover
...
# leftover isolated-test drafts #107–#466,
...
# GetBossPhase, leftover FSkyguardBossTelemetry
# defaults #48e1, leftover ESkyguardBossPhase
# enum #60cb, leftover
# apache aircraft isolated contracts, leftover
...
# NotifyWeakPointDestroyed, leftover
# GetBossPhase, leftover Telemetry UPROPERTY,
# leftover strike / lock-eligible surfaces, leftover
# USkyguardBossWeakPointComponent fields, leftover
...
# Leftover FSkyguardBossTelemetry defaults #48e1
# stays sibling-only. Return type remains on the
# locked GetTelemetry declaration.
...
# stays sibling-only. Do not lock the enum.
...
# headers. Do not scan Apache public section for
# Harbor clocks.
...
# .cpp GetTelemetry body / invented
...
# return Telemetry body. Do not lock the
# Telemetry UPROPERTY field.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "ASkyguardBossDroneBase::GetTelemetry",
    "SkyguardBossDroneBase.cpp",
    "SetBossPhase",
...
class BossDroneGetTelemetryDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_TELEMETRY),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_TELEMETRY)
        self.assertIn("GetTelemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_TELEMETRY)
    ...
    def test_missing_get_telemetry_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_TELEMETRY)
        self.assertIn("GetTelemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_TELEMETRY)
        self.assertIn("GetTelemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_TELEMETRY)
        self.assertNotIn("UFUNCTION", GET_TELEMETRY)
        self.assertNotIn("Category", GET_TELEMETRY)
        self.assertNotIn("BlueprintCallable", GET_TELEMETRY)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_TELEMETRY)
        self.assertIn("GetTelemetry", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tconst FSkyguardBossTelemetry& GetTelemetry();\n"
        missing_parens = "\tconst FSkyguardBossTelemetry& GetTelemetry;\n"
        wrong_return_void = "\tvoid GetTelemetry() const;\n"
        wrong_return_bool = "\tbool GetTelemetry() const;\n"
        wrong_return_float = "\tfloat GetTelemetry() const;\n"
        added_arg = (
            "\tconst FSkyguardBossTelemetry& GetTelemetry(int32 Extra) const;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_TELEMETRY)
            self.assertIn("GetTelemetry", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_telemetry_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, GET_TELEMETRY),
            1,
        )
        self.assertTrue(
            GET_TELEMETRY.startswith("const FSkyguardBossTelemetry& "),
            GET_TELEMETRY,
        )
        self.assertTrue(
            GET_TELEMETRY.endswith(";"),
            GET_TELEMETRY,
        )
        self.assertIn(
            "GetTelemetry()",
        ...
        self.assertIn(" const", GET_TELEMETRY)
        self.assertNotIn("INDEX_NONE", GET_TELEMETRY)
        self.assertNotIn("{", GET_TELEMETRY)
        self.assertNotIn("}", GET_TELEMETRY)
        self.assertNotIn("return ", GET_TELEMETRY)
        self.assertNotIn(
            "return Telemetry",
            GET_TELEMETRY,
        )
        self.assertNotIn("ApplyWeaponHit", GET_TELEMETRY)
        self.assertNotIn("IssuePilotCommand", GET_TELEMETRY)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            GET_TELEMETRY,
        )
        ...
        self.assertNotIn("GetBossPhase", GET_TELEMETRY)
        self.assertNotIn("GetDefeatDebrisPieceCount", GET_TELEMETRY)
        self.assertNotIn("GetChinMuzzleLocation", GET_TELEMETRY)
        self.assertNotIn("GetGunnerMount", GET_TELEMETRY)
        self.assertNotIn("FaceWorldLocation", GET_TELEMETRY)
        self.assertNotIn("WeakPoints", GET_TELEMETRY)
        self.assertNotIn("SetSearchlightTracked", GET_TELEMETRY)
        self.assertNotIn("SetContactIdentified", GET_TELEMETRY)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            GET_TELEMETRY,
        )
        self.assertNotIn("SetLightningExposed", GET_TELEMETRY)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            GET_TELEMETRY,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, GET_TELEMETRY)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tconst FSkyguardBossTelemetry&\n"
            "\tGetTelemetry() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tconst FSkyguardBossTelemetry& GetTelemetry\n"
            "\t() const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_TELEMETRY}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, GET_TELEMETRY)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_TELEMETRY),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_TELEMETRY}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_TELEMETRY)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_TELEMETRY)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_telemetry_cpp_body(
        self,
    ) -> None:
        locked_only = f"{GET_TELEMETRY}\n"
        self.assertNotIn("{", GET_TELEMETRY)
        ...
        self.assertNotIn("return false", GET_TELEMETRY)
        self.assertNotIn("return true", GET_TELEMETRY)
        self.assertNotIn(
            "return Telemetry",
            GET_TELEMETRY,
        )
        self.assertNotIn("SetBossPhase", GET_TELEMETRY)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            GET_TELEMETRY,
        )
        self.assertNotIn("RegisterDefeatDebris", GET_TELEMETRY)
        self.assertNotIn("CleanupDefeatDebris", GET_TELEMETRY)
        self.assertNotIn("HandleDefeated", GET_TELEMETRY)

    def test_contract_does_not_relock_apply_weapon_hit_sibling(self) -> None:
        locked_only = f"{GET_TELEMETRY}\n"
        for token in APPLY_WEAPON_HIT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("ApplyWeaponHit", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", GET_TELEMETRY)
        self.assertNotIn("PortGuidanceArray", GET_TELEMETRY)
        self.assertNotIn("CommandCore", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("SetSearchlightTracked", GET_TELEMETRY)
        self.assertNotIn("IsSearchlightTracked", GET_TELEMETRY)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("SetContactIdentified", GET_TELEMETRY)
        self.assertNotIn("IsContactIdentified", GET_TELEMETRY)
        self.assertNotIn("OpenOrbitExposure", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", GET_TELEMETRY)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            GET_TELEMETRY,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", GET_TELEMETRY)
        self.assertNotIn("ASkyguardIronRainBoss", GET_TELEMETRY)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            GET_TELEMETRY,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("GetGunnerMount", GET_TELEMETRY)
        self.assertNotIn("GetChinTurret", GET_TELEMETRY)
        self.assertNotIn("GetSensorTurret", GET_TELEMETRY)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_TELEMETRY}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TELEMETRY)
        self.assertNotIn("GetChinMuzzleLocation", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("ESkyguardApacheSystem", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("ASkyguardGunner", GET_TELEMETRY)
        self.assertNotIn("FillAndFinalize", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("SkyguardRadarNode", GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", GET_TELEMETRY)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            GET_TELEMETRY,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_TELEMETRY)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_TELEMETRY)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            GET_TELEMETRY,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TELEMETRY)
        self.assertNotIn("MinHeightFromOriginCm", GET_TELEMETRY)
        self.assertNotIn("MaxIntegrity", GET_TELEMETRY)
        self.assertNotIn("CurrentIntegrity", GET_TELEMETRY)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            GET_TELEMETRY,
        )
        self.assertNotIn("SkyguardPathfinder", GET_TELEMETRY)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_TELEMETRY)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_TELEMETRY}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_TELEMETRY),
            GET_TELEMETRY,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone GetTelemetry contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_TELEMETRY.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_TELEMETRY.lower(),
                "boss drone GetTelemetry contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_TELEMETRY)

    def test_contract_is_get_telemetry_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", GET_TELEMETRY)
        self.assertNotIn("{", GET_TELEMETRY)
        self.assertTrue(GET_TELEMETRY.startswith("const FSkyguardBossTelemetry& "))
        self.assertTrue(GET_TELEMETRY.endswith(";"))
        self.assertIn(" const", GET_TELEMETRY)
        self.assertIn(UFUNCTION_BOSS, section)
