# THIS IS leftover-safe FSkyguardObjectiveProgress::CurrentProgress.
# origin/main form: split-line UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `int32 CurrentProgress = 0;` with no Category=.
...
# THIS IS NOT leftover HowToFlyRow Priority.
...
# no Category=. Accept `int32 CurrentProgress = 0`. Fail-closed if
# type/decl missing, renamed, Category= added, or default is not 0.
...
TARGET = "int32 CurrentProgress = 0;"
TARGET_WRONG_FALSE = "int32 CurrentProgress = false;"
TARGET_WRONG_ONE = "int32 CurrentProgress = 1;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # for FSkyguardObjectiveProgress::CurrentProgress (must be = 0).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+CurrentProgress\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 CurrentProgress is missing from "
...
class ObjectiveProgressCurrentProgressFieldDeclContractTests(unittest.TestCase):
    def test_objective_progress_struct_exists(self) -> None:
        ...
        self.assertNotIn("int32 CurrentProgress)", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("CurrentProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_current_progress_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_current_progress_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 CurrentProgress"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("RequiredProgress", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            "\tint32 CurrentProgress = false;\n",
            "\tbool CurrentProgress = false;\n",
            "\tbool CurrentProgress = 0;\n",
            "\tint32 Progress = 0;\n",
            "\tint32 RequiredProgress = 0;\n",
            "\tint32 CurrentProgress;\n",
            "\tint32 CurrentProgress = 0.0;\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "objective-progress CurrentProgress field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_objective_progress_current_progress_field_decl_contract.py"
        ))
