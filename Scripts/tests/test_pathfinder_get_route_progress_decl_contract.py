...
# or lock the GetRouteProgress body.
...
# AdvanceEncounter, and in-flight ResetEncounterState /
# IsRouteStateSafe stay sibling-only.
GET_ROUTE_PROGRESS = "float GetRouteProgress() const;"
UFUNCTION_ENCOUNTER = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Boss|Encounter")'
)
# Leftover #56–#64 plus PathfinderEncounterController
# production files. This lane only adds an isolated
# Python GetRouteProgress declaration contract. Stay
# off AdvanceEncounter (sibling just pushed),
# ResetEncounterState (in-flight),
# IsRouteStateSafe (in-flight),
...
# leftover GetEffectiveSpeedMultiplier, leftover
...
# IsRouteStateSafe / GetEffectiveSpeedMultiplier /
...
# .cpp GetRouteProgress body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline `return RouteProgressCm`
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardPathfinderEncounterController::GetRouteProgress",
    "SkyguardPathfinderEncounterController.cpp",
)
SIBLING_TYPES = (
...
class PathfinderGetRouteProgressDeclContractTests(unittest.TestCase):
    def test_pathfinder_controller_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_ROUTE_PROGRESS), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_ROUTE_PROGRESS)
        self.assertIn("GetRouteProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_ROUTE_PROGRESS))

    def test_missing_get_route_progress_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardPathfinderEncounterController();\n"
            "\tvoid AdvanceEncounter(float DeltaSeconds);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_ROUTE_PROGRESS)
        self.assertIn("GetRouteProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_ROUTE_PROGRESS)
        self.assertIn("GetRouteProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_ROUTE_PROGRESS),
            section,
        )
        self.assertNotIn("BlueprintPure", GET_ROUTE_PROGRESS)
        self.assertNotIn("UFUNCTION", GET_ROUTE_PROGRESS)
        self.assertNotIn("Category", GET_ROUTE_PROGRESS)
        self.assertNotIn("BlueprintCallable", GET_ROUTE_PROGRESS)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_ROUTE_PROGRESS)
        self.assertIn("GetRouteProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tfloat GetRouteProgress();\n"
        wrong_return_void = "\tvoid GetRouteProgress() const;\n"
        wrong_return_bool = "\tbool GetRouteProgress() const;\n"
        wrong_return_int = "\tint32 GetRouteProgress() const;\n"
        added_arg = "\tfloat GetRouteProgress(float DeltaSeconds) const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_ROUTE_PROGRESS)
            self.assertIn("GetRouteProgress", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_route_progress_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_ROUTE_PROGRESS),
            GET_ROUTE_PROGRESS,
        )
        self.assertTrue(has_declaration(section, GET_ROUTE_PROGRESS))
        self.assertEqual(declaration_count(section, GET_ROUTE_PROGRESS), 1)
        self.assertTrue(
            GET_ROUTE_PROGRESS.startswith("float "),
            GET_ROUTE_PROGRESS,
        )
        self.assertTrue(GET_ROUTE_PROGRESS.endswith(";"), GET_ROUTE_PROGRESS)
        self.assertTrue(
            GET_ROUTE_PROGRESS.endswith(" const;"),
            GET_ROUTE_PROGRESS,
        )
        self.assertIn("GetRouteProgress()", GET_ROUTE_PROGRESS)
        self.assertIn(" const", GET_ROUTE_PROGRESS)
        self.assertNotIn("INDEX_NONE", GET_ROUTE_PROGRESS)
        self.assertNotIn("{", GET_ROUTE_PROGRESS)
        self.assertNotIn("}", GET_ROUTE_PROGRESS)
        self.assertNotIn("return ", GET_ROUTE_PROGRESS)
        self.assertNotIn("return RouteProgressCm", GET_ROUTE_PROGRESS)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_ROUTE_PROGRESS}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_ROUTE_PROGRESS))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_ROUTE_PROGRESS),
            GET_ROUTE_PROGRESS,
        )
        exact_inline = (
            "float GetRouteProgress() const { return RouteProgressCm; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_ROUTE_PROGRESS))
        self.assertEqual(
            require_declaration(exact_inline, GET_ROUTE_PROGRESS),
            GET_ROUTE_PROGRESS,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_ROUTE_PROGRESS),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, GET_ROUTE_PROGRESS),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_ROUTE_PROGRESS}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_ROUTE_PROGRESS))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_ROUTE_PROGRESS),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_ROUTE_PROGRESS)
        self.assertNotIn("return RouteProgressCm", GET_ROUTE_PROGRESS)
        self.assertNotIn("return Other", GET_ROUTE_PROGRESS)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_ROUTE_PROGRESS}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_ROUTE_PROGRESS)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_ROUTE_PROGRESS)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_route_progress_body(self) -> None:
        locked_only = f"{GET_ROUTE_PROGRESS}\n"
        self.assertNotIn("{", GET_ROUTE_PROGRESS)
        self.assertNotIn("}", GET_ROUTE_PROGRESS)
        self.assertNotIn("return ", GET_ROUTE_PROGRESS)
        self.assertNotIn("return RouteProgressCm", GET_ROUTE_PROGRESS)
        self.assertNotIn("return RouteProgressCm", locked_only)
        self.assertNotIn(
            "USkyguardPathfinderEncounterController::GetRouteProgress",
            GET_ROUTE_PROGRESS,
        )
        ...
        self.assertNotIn("return false", GET_ROUTE_PROGRESS)
        self.assertNotIn("return true", GET_ROUTE_PROGRESS)

    def test_contract_does_not_relock_advance_encounter(self) -> None:
        locked_only = f"{GET_ROUTE_PROGRESS}\n"
        for neighbor in ADVANCE_ENCOUNTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PROGRESS)
        self.assertNotIn("AdvanceEncounter", GET_ROUTE_PROGRESS)
    ...
    def test_contract_does_not_relock_reset_encounter_state(self) -> None:
        ...
        for neighbor in RESET_ENCOUNTER_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PROGRESS)
        self.assertNotIn("ResetEncounterState", GET_ROUTE_PROGRESS)
    ...
    def test_contract_does_not_relock_is_route_state_safe(self) -> None:
        ...
        for neighbor in IS_ROUTE_STATE_SAFE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PROGRESS)
        self.assertNotIn("IsRouteStateSafe", GET_ROUTE_PROGRESS)
    ...
    def test_contract_does_not_relock_get_effective_speed_multiplier(
        ...
        for neighbor in GET_EFFECTIVE_SPEED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PROGRESS)
        self.assertNotIn("GetEffectiveSpeedMultiplier", GET_ROUTE_PROGRESS)
    ...
    def test_contract_does_not_relock_is_attack_telegraph_active(self) -> None:
        ...
        for neighbor in IS_ATTACK_TELEGRAPH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PROGRESS)
        self.assertNotIn("IsAttackTelegraphActive", GET_ROUTE_PROGRESS)
    ...
    def test_contract_does_not_relock_get_telegraphs_triggered(self) -> None:
        ...
        for neighbor in GET_TELEGRAPHS_TRIGGERED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PROGRESS)
        self.assertNotIn("GetTelegraphsTriggered", GET_ROUTE_PROGRESS)
        self.assertNotIn("GetTelegraphsTriggered", locked_only)

    def test_contract_does_not_relock_uprop_fields_or_delegates(self) -> None:
        locked_only = f"{GET_ROUTE_PROGRESS}\n"
        self.assertNotIn("UPROPERTY", GET_ROUTE_PROGRESS)
        self.assertNotIn("UPROPERTY", locked_only)
        self.assertNotIn("bAutoAdvance", GET_ROUTE_PROGRESS)
        self.assertNotIn("ObservedPhase", GET_ROUTE_PROGRESS)
        self.assertNotIn("OnAttackTelegraphChanged", GET_ROUTE_PROGRESS)
        self.assertNotIn("OnAttackCommitted", GET_ROUTE_PROGRESS)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_leftover_boss_phase_enum(self) -> None:
        ...
        for token in LEFTOVER_BOSS_PHASE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_ROUTE_PROGRESS)
        self.assertNotIn("ESkyguardBossPhase", GET_ROUTE_PROGRESS)
        self.assertNotIn("ESkyguardBossPhase", locked_only)
        self.assertNotIn("SkyguardBossTypes.h", GET_ROUTE_PROGRESS)
        self.assertNotIn(
            "test_boss_phase_enum_contract.py",
            GET_ROUTE_PROGRESS,
        )
    ...
    def test_contract_does_not_relock_leftover_boss_defaults(self) -> None:
        ...
        for token in LEFTOVER_BOSS_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ROUTE_PROGRESS)
        self.assertNotIn("FSkyguardBossDefinition", GET_ROUTE_PROGRESS)
        self.assertNotIn("FSkyguardBossWeakPoint", GET_ROUTE_PROGRESS)
        self.assertNotIn(
            "test_boss_definition_defaults_contract.py",
            GET_ROUTE_PROGRESS,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ROUTE_PROGRESS)
        self.assertNotIn("Unconfigured", GET_ROUTE_PROGRESS)
        self.assertNotIn("Warming", GET_ROUTE_PROGRESS)
        self.assertNotIn("Launched", GET_ROUTE_PROGRESS)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_ROUTE_PROGRESS,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ROUTE_PROGRESS)
        self.assertNotIn("FSkyguardBriefingCard", GET_ROUTE_PROGRESS)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_ROUTE_PROGRESS)
        self.assertNotIn("FSkyguardBriefingCard", locked_only)
        self.assertNotIn("FSkyguardBriefingRadioRow", locked_only)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_ROUTE_PROGRESS)
        self.assertNotIn("GetBriefingText", GET_ROUTE_PROGRESS)
        self.assertNotIn("GetRadioChatter", GET_ROUTE_PROGRESS)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PROGRESS)
        self.assertNotIn("FillResultCombatStats", GET_ROUTE_PROGRESS)
        self.assertNotIn("ASkyguardGunner", GET_ROUTE_PROGRESS)
        self.assertNotIn("FillAndFinalize", GET_ROUTE_PROGRESS)
        self.assertNotIn("FillAndFail", GET_ROUTE_PROGRESS)
        self.assertNotIn("ApplyHydraForClusters", GET_ROUTE_PROGRESS)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_ROUTE_PROGRESS}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_ROUTE_PROGRESS),
            GET_ROUTE_PROGRESS,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder GetRouteProgress contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_ROUTE_PROGRESS.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"pathfinder GetRouteProgress contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_ROUTE_PROGRESS)

    def test_contract_is_get_route_progress_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_ROUTE_PROGRESS.startswith("float "))
        self.assertTrue(GET_ROUTE_PROGRESS.endswith(";"))
        self.assertTrue(GET_ROUTE_PROGRESS.endswith(" const;"))
        self.assertIn(UFUNCTION_ENCOUNTER, section)
