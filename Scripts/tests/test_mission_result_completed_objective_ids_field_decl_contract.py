# THIS IS leftover-safe FSkyguardMissionResult::CompletedObjectiveIds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TArray<FName> CompletedObjectiveIds;` (no initializer).
...
# leftover MissionId #1426 / ShotsFired #1427 /
# bMissionSucceeded #1428 / Hits draft, then this isolated
# CompletedObjectiveIds field decl.
# Clone parse-window from leftover FinalScore #1264 and leftover
# MissionId #1426. Clone LOCKED_SCRIPTS from leftover-safe
# draft #1300 (theater-kit-spec WeatherIdentity).
# If a clone asserts VisibleAnywhere / BlueprintReadOnly /
# int32 / required `= 0` / FName, RETARGET: specifiers are
# EditAnywhere + BlueprintReadWrite, type is TArray<FName>,
# BARE decl with NO initializer, no Category=.
...
# not TArray<FName>, an initializer is added, or Category=
# is added. Fail-closed if an initializer is added
# (`= {}`, `= 0`, Harbor 40/80).
...
# Match EXACT identifier CompletedObjectiveIds as a whole
# field name after TArray<FName>. Do NOT claim leftover
# FinalScore / MedalTier / leftover ObjectiveProgress id.
# Siblings stay unclaimed: MissionId / bMissionSucceeded /
# ShotsFired / Hits / AircraftDamageFraction /
# CompletionTimeSeconds / FinalScore / MedalTier.
...
TARGET = "TArray<FName> CompletedObjectiveIds;"
TARGET_WRONG_EQ_EMPTY = "TArray<FName> CompletedObjectiveIds = {};"
TARGET_WRONG_EQ_NONE = "TArray<FName> CompletedObjectiveIds = NAME_None;"
TARGET_WRONG_FALSE = "TArray<FName> CompletedObjectiveIds = false;"
TARGET_WRONG_TRUE = "TArray<FName> CompletedObjectiveIds = true;"
TARGET_WRONG_ZERO = "TArray<FName> CompletedObjectiveIds = 0;"
TARGET_WRONG_INT = "int32 CompletedObjectiveIds;"
TARGET_WRONG_FNAME = "FName CompletedObjectiveIds;"
TARGET_WRONG_FINAL = "int32 FinalScore = 0;"
TARGET_WRONG_MEDAL = "int32 MedalTier = 0;"
TARGET_WRONG_MISSION = "FName MissionId;"
TARGET_WRONG_PROGRESS = "FName ObjectiveId;"
LOCKED_DECL = TARGET
...
SIBLING_TIME = "CompletionTimeSeconds"
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission_result_mission_id_field_decl_contract.py",
    "Scripts/tests/test_mission_result_hits_field_decl_contract.py",
    "Scripts/tests/test_mission_result_shots_fired_field_decl_contract.py",
    "Scripts/tests/test_mission_result_mission_succeeded_field_decl_contract.py",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept `= {}` / `= 0` / `= NAME_None` /
    # `= false` / leftover int32 FinalScore / leftover
    # FName MissionId when origin/main has a bare TArray.
    # Do not accept leftover ObjectiveProgress id.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+CompletedObjectiveIds\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:int32|FName|bool|float)\s+CompletedObjectiveIds\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+CompletedObjectiveIds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> CompletedObjectiveIds is missing "
        f"from origin/main:{HEADER_PATH} struct {CLASS_NAME} public section"
    )
...
class MissionResultCompletedObjectiveIdsFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CompletedObjectiveIds"), section)
        ...
        self.assertNotIn("TArray<FName> CompletedObjectiveIds)", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("CompletedObjectiveIds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_completed_objective_ids_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_completed_objective_ids_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> CompletedObjectiveIds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "TArray<FName> CompletedObjectiveIds;")
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_MISSION}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_PROGRESS}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_FINAL}\n", LOCKED_DECL)
        self.assertIn("CompletedObjectiveIds", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            f"\t{TARGET_WRONG_MISSION}\n",
            f"\t{TARGET_WRONG_PROGRESS}\n",
            "\tTArray<FName> CompletedObjectiveId;\n",
            "\tTArray<FName> CompletedObjectives;\n",
            "\tTArray<int32> CompletedObjectiveIds;\n",
            "\tFName CompletedObjectiveIds = NAME_None;\n",
            "\tint32 CompletedObjectiveIds = 0;\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result CompletedObjectiveIds field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_completed_objective_ids_field_decl_contract.py"
        ))
        ...
        self.assertIn(CLONE_HITS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_SHOTS_FIRED, LOCKED_SCRIPTS)
        self.assertIn(CLONE_MISSION_SUCCEEDED, LOCKED_SCRIPTS)
