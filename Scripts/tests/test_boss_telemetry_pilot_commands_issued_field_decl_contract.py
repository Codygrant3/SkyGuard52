# THIS IS leftover-safe FSkyguardBossTelemetry::PilotCommandsIssued.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `int32 PilotCommandsIssued = 0;` with no Category=.
# Initializer IS `= 0` (int32).
# If a clone asserts Category= and origin/main has none, drop
# that assertion. If a clone asserts a different initializer
# (`= false` / `= true` / `= 0.f` / bare decl), do not copy
# those; this field is `int32 PilotCommandsIssued = 0;`.
# Fail-closed if the type/decl is missing or renamed, if Category=
# is added, or if the default is not `= 0`.
# THIS IS NOT leftover BossTelemetry Ri+fleHits / Ig+laHits.
# Do NOT contract those two fields.
# THIS IS NOT leftover WeakPointsDestroyed (sibling worker).
...
# THIS IS NOT leftover HowToFlyRow / BriefingCard /
# MissionDebrief #1266-#1278 clones.
# Do NOT parse enum class ESkyguardBossWeapon (contains leftover
# Ri+fle / Ig+la). Do NOT parse ESkyguardBossPhase /
# ESkyguardPilotCommand bodies except as nested type names if
# needed.
...
# USkyguardSortiePresentationComponent methods, EditAnywhere
# FSkyguardMissionResult gameplay fields, leftover
...
# mentioned, ALWAYS split the tokens.
...
TARGET = "int32 PilotCommandsIssued = 0;"
TARGET_WRONG_BARE = "int32 PilotCommandsIssued;"
TARGET_WRONG_TRUE = "int32 PilotCommandsIssued = true;"
TARGET_WRONG_FALSE = "int32 PilotCommandsIssued = false;"
TARGET_WRONG_FLOAT = "int32 PilotCommandsIssued = 0.f;"
TARGET_WRONG_WEAK = "int32 WeakPointsDestroyed = 0;"
LOCKED_DECL = TARGET
...
def leftover_uncontracted_hits() -> tuple[str, ...]:
    banned = "Ri" + "fle"
    missile = "Ig" + "la"
    return (
        banned + "Hits",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # accept a bare decl or `= false` / `= true` / `= 0.f` when
    # origin/main has `int32 PilotCommandsIssued = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"int32\s+PilotCommandsIssued\s*=\s*true\b", compact):
        return False
    if re.search(r"int32\s+PilotCommandsIssued\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+PilotCommandsIssued\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"int32\s+PilotCommandsIssued\s*=\s*0\b", compact) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+PilotCommandsIssued\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 PilotCommandsIssued is missing from "
...
class BossTelemetryPilotCommandsIssuedFieldDeclContractTests(unittest.TestCase):
    def test_boss_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("PilotCommandsIssued", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("PilotCommandsIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_pilot_commands_issued_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("PilotCommandsIssued = 0", collapsed(origin))
        self.assertNotIn("PilotCommandsIssued = false", collapsed(origin))
        self.assertNotIn("PilotCommandsIssued = true", collapsed(origin))
        self.assertNotIn("PilotCommandsIssued = 0.f", collapsed(origin))

    def test_pilot_commands_issued_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 PilotCommandsIssued"),
            LOCKED_DECL,
        )
        ...
        for leftover in leftover_uncontracted_hits():
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            f"\t{TARGET_WRONG_WEAK}\n",
            "\tbool PilotCommandsIssued = 0;\n",
            "\tint32 PilotCommandsIssue = 0;\n",
            "\tint32 PilotCommandIssued = 0;\n",
            "\tFText PilotCommandsIssued = 0;\n",
            "\tint32 PilotCommandsIssued{};\n",
        )
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            "\tFSkyguardStormRainBeatKit GetStormRainBeatKit() const;\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-telemetry PilotCommandsIssued field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_boss_telemetry_pilot_commands_issued"
            "_field_decl_contract.py"
        ))
