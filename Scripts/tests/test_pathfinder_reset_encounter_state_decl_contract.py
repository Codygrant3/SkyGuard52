...
# or lock the ResetEncounterState body in the .cpp.
...
# ESkyguardBossPhase. Leftover briefing-fail-closed
# #9fe9, leftover briefing declaration contracts through
...
# shake, leftover mission-weather enum, and in-flight
# AdvanceEncounter stay sibling-only.
...
# Python ResetEncounterState declaration contract.
# Stay off AdvanceEncounter (in-flight),
# IsRouteStateSafe (sibling this wave),
# GetRouteProgress, GetEffectiveSpeedMultiplier,
# IsAttackTelegraphActive, and GetTelegraphsTriggered
# on this class. Stay off leftover briefing-fail-closed,
# leftover briefing declaration contracts through
# GetRadioChatter, leftover mission-briefing-state enum,
# leftover radio-chatter empty-fail-closed, leftover
# campaign-roster lookup, leftover campaign-save
# empty-fail-closed, leftover boss-phase enum, leftover
# boss-definition defaults, leftover boss-weak-point
# defaults, leftover Gunner helpers, leftover Harbor
# clocks, leftover theater-kit / flare / HUD, leftover
...
# leftover route-runtime fail-closed, leftover gun-fire
# camera shake, leftover mission-weather enum, leftover
# mission 0N integration readiness, and dirty workspace
# paths.
LOCKED = {
...
# leftover bind-hud-host, leftover gun-fire camera shake,
# leftover mission-weather enum, leftover Gunner helpers
# FillAndFinalize / FillAndFail / FillResultCombatStats /
# ApplyHydraForClusters, and in-flight AdvanceEncounter
# stay sibling-only.
LOCKED_SCRIPTS = (
...
# here. AdvanceEncounter / IsRouteStateSafe /
# GetRouteProgress / GetEffectiveSpeedMultiplier /
# IsAttackTelegraphActive / GetTelegraphsTriggered stay
# sibling-only. UPROPERTY fields and delegates stay
# unlocked. Leftover boss-phase enum / leftover
# boss-definition defaults / leftover boss-weak-point
# defaults stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# parse ESkyguardBossPhase. Approach / LockWindow /
# Critical appear on public UPROPERTY fields, so those
# labels are checked against the locked declaration
# only. Disarm / Defeated stay off the public section.
LEFTOVER_ENUM_NOT_LOCKED = (
    "Disarm",
    "Defeated",
    "test_boss_phase_enum_contract.py",
)
LEFTOVER_ENUM_DECL_ONLY = (
    "Approach",
    "LockWindow",
    "Critical",
...
# Leftover boss-definition / boss-weak-point defaults
# stay unlocked. Field names from those leftovers stay
# off this file.
...
# Public UPROPERTY fields and delegates stay unlocked.
UPROPERTY_NOT_LOCKED = (
    "bAutoAdvance",
    "RouteLengthCm",
...
# .cpp ResetEncounterState body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardPathfinderEncounterController::ResetEncounterState",
    "SkyguardPathfinderEncounterController.cpp",
)
SIBLING_TYPES = (
    "FSkyguardMission0NIntegrationReadiness",
)
IN_FLIGHT_ADVANCE_NOT_LOCKED = (
...
class PathfinderResetEncounterStateDeclContractTests(unittest.TestCase):
    def test_pathfinder_controller_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, RESET_ENCOUNTER_STATE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, RESET_ENCOUNTER_STATE)
        self.assertIn("ResetEncounterState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, RESET_ENCOUNTER_STATE))

    def test_missing_reset_encounter_state_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tUSkyguardPathfinderEncounterController();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, RESET_ENCOUNTER_STATE)
        self.assertIn("ResetEncounterState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, RESET_ENCOUNTER_STATE)
        self.assertIn("ResetEncounterState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", RESET_ENCOUNTER_STATE)
        self.assertNotIn("UFUNCTION", RESET_ENCOUNTER_STATE)
        self.assertNotIn("Category", RESET_ENCOUNTER_STATE)
        self.assertNotIn("BlueprintCallable", RESET_ENCOUNTER_STATE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, RESET_ENCOUNTER_STATE)
        self.assertIn("ResetEncounterState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = "\tvoid ResetEncounterState();\n"
        wrong_return = (
            "\tbool ResetEncounterState("
            "const FTransform& NewRouteOrigin);\n"
        )
        added_const = (
            "\tvoid ResetEncounterState("
            "const FTransform& NewRouteOrigin) const;\n"
        )
        by_value = "\tvoid ResetEncounterState(FTransform NewRouteOrigin);\n"
        mutable_ref = (
            "\tvoid ResetEncounterState(FTransform& NewRouteOrigin);\n"
        )
        wrong_type = (
            "\tvoid ResetEncounterState(const FVector& NewRouteOrigin);\n"
        )
        for region in (
            missing_arg,
            wrong_return,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, RESET_ENCOUNTER_STATE)
            self.assertIn("ResetEncounterState", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_reset_encounter_state_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, RESET_ENCOUNTER_STATE))
        self.assertEqual(
            declaration_count(section, RESET_ENCOUNTER_STATE),
            1,
        )
        self.assertTrue(
            RESET_ENCOUNTER_STATE.startswith("void "),
            RESET_ENCOUNTER_STATE,
        )
        self.assertTrue(
            RESET_ENCOUNTER_STATE.endswith(";"),
            RESET_ENCOUNTER_STATE,
        )
        self.assertIn("ResetEncounterState(", RESET_ENCOUNTER_STATE)
        self.assertIn(
            "const FTransform& NewRouteOrigin",
            RESET_ENCOUNTER_STATE,
        )
        self.assertNotIn("INDEX_NONE", RESET_ENCOUNTER_STATE)
        self.assertNotIn("{", RESET_ENCOUNTER_STATE)
        self.assertNotIn("}", RESET_ENCOUNTER_STATE)
        self.assertNotIn("return ", RESET_ENCOUNTER_STATE)
        self.assertNotIn(" const;", RESET_ENCOUNTER_STATE)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tResetEncounterState(const FTransform& NewRouteOrigin);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        wrap_ref = (
            "public:\n"
            "\tvoid ResetEncounterState(const FTransform&\n"
            "\t\tNewRouteOrigin);\n"
            "};\n"
        )
        wrap_space = (
        ...
        one_line = f"{{\npublic:\n\t{RESET_ENCOUNTER_STATE}\n}}\n"
        self.assertTrue(has_declaration(one_line, RESET_ENCOUNTER_STATE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, RESET_ENCOUNTER_STATE),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", RESET_ENCOUNTER_STATE)
        self.assertNotIn("return ", RESET_ENCOUNTER_STATE)
        self.assertNotIn("RouteProgressCm = 0.f", RESET_ENCOUNTER_STATE)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", RESET_ENCOUNTER_STATE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_reset_encounter_state_cpp_body(
        self,
    ) -> None:
        locked_only = f"{RESET_ENCOUNTER_STATE}\n"
        self.assertNotIn("{", RESET_ENCOUNTER_STATE)
        ...
        self.assertNotIn("return false", RESET_ENCOUNTER_STATE)
        self.assertNotIn("return true", RESET_ENCOUNTER_STATE)

    def test_contract_does_not_relock_advance_encounter(self) -> None:
        locked_only = f"{RESET_ENCOUNTER_STATE}\n"
        for neighbor in ADVANCE_ENCOUNTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RESET_ENCOUNTER_STATE)
        self.assertNotIn("AdvanceEncounter", RESET_ENCOUNTER_STATE)
        self.assertNotIn("AdvanceEncounter", locked_only)
        for token in IN_FLIGHT_ADVANCE_NOT_LOCKED:
    ...
    def test_contract_does_not_relock_is_route_state_safe(self) -> None:
        ...
        for neighbor in IS_ROUTE_STATE_SAFE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RESET_ENCOUNTER_STATE)
        self.assertNotIn("IsRouteStateSafe", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_get_route_progress(self) -> None:
        ...
        for neighbor in GET_ROUTE_PROGRESS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RESET_ENCOUNTER_STATE)
        self.assertNotIn("GetRouteProgress", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_get_effective_speed_multiplier(
        ...
        for neighbor in GET_EFFECTIVE_SPEED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RESET_ENCOUNTER_STATE)
        self.assertNotIn("GetEffectiveSpeedMultiplier", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_is_attack_telegraph_active(self) -> None:
        ...
        for neighbor in IS_ATTACK_TELEGRAPH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RESET_ENCOUNTER_STATE)
        self.assertNotIn("IsAttackTelegraphActive", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_get_telegraphs_triggered(self) -> None:
        ...
        for neighbor in GET_TELEGRAPHS_TRIGGERED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RESET_ENCOUNTER_STATE)
        self.assertNotIn("GetTelegraphsTriggered", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_leftover_boss_phase_enum(self) -> None:
        ...
        for token in LEFTOVER_ENUM_DECL_ONLY:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RESET_ENCOUNTER_STATE)
        self.assertNotIn("Disarm", RESET_ENCOUNTER_STATE)
        self.assertNotIn("Defeated", RESET_ENCOUNTER_STATE)
        self.assertNotIn(
            "test_boss_phase_enum_contract.py",
            RESET_ENCOUNTER_STATE,
        )
        self.assertNotIn("enum class ESkyguardBossPhase", section)
        self.assertNotIn("SkyguardBossTypes.h", HEADER_PATH)
    ...
    def test_contract_does_not_relock_uprop_fields_or_delegates(self) -> None:
        locked_only = f"{RESET_ENCOUNTER_STATE}\n"
        for token in UPROPERTY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RESET_ENCOUNTER_STATE)
        self.assertNotIn("UPROPERTY", RESET_ENCOUNTER_STATE)
        self.assertNotIn("BlueprintAssignable", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RESET_ENCOUNTER_STATE)
        self.assertNotIn("FSkyguardBriefingCard", RESET_ENCOUNTER_STATE)
        self.assertNotIn("FSkyguardBriefingRadioRow", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RESET_ENCOUNTER_STATE)
        self.assertNotIn("FillResultCombatStats", RESET_ENCOUNTER_STATE)
        self.assertNotIn("ASkyguardGunner", RESET_ENCOUNTER_STATE)
        self.assertNotIn("FillAndFinalize", RESET_ENCOUNTER_STATE)
        self.assertNotIn("FillAndFail", RESET_ENCOUNTER_STATE)
        self.assertNotIn("ApplyHydraForClusters", RESET_ENCOUNTER_STATE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{RESET_ENCOUNTER_STATE}\n"
        self.assertEqual(
            require_declaration(locked_only, RESET_ENCOUNTER_STATE),
            RESET_ENCOUNTER_STATE,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("Disarm", section)
        self.assertNotIn("Defeated", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder ResetEncounterState contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, RESET_ENCOUNTER_STATE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"pathfinder ResetEncounterState contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, RESET_ENCOUNTER_STATE)

    def test_contract_is_reset_encounter_state_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("bAutoAdvance", locked_only)
        ...
        self.assertTrue(RESET_ENCOUNTER_STATE.startswith("void "))
        self.assertTrue(RESET_ENCOUNTER_STATE.endswith(";"))
        self.assertIn(
            "const FTransform& NewRouteOrigin",
            RESET_ENCOUNTER_STATE,
        )
