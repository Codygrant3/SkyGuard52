...
# wrapping `int32 GetQueuedLineCount() const { return Queue.Num(); }`.
# LOCKED_DECL may be declaration-only
# `int32 GetQueuedLineCount() const;` if the matcher strips
# inline bodies, OR the full origin/main inline form.
# Accept declaration-only AND the origin/main inline body
# `{ return Queue.Num(); }`. Do NOT fail-closed on that
# inline body. Do not require a separate .cpp definition.
# KEEP BlueprintPure. KEEP specifiers matching origin/main.
# Do NOT retarget to PrimeLines / EnqueueLine /
# AdvanceRadioState / ClearQueue / GetCurrentLineId /
# GetDroppedLineCount / GetPlayedLineCount.
# Word-boundary match GetQueuedLineCount vs ClearQueue
# (in-flight) vs GetDroppedLineCount vs GetPlayedLineCount
# vs PrimeLines #1713 vs EnqueueLine #1712 vs
# AdvanceRadioState #1715 vs RadioLine fields #1332-#1338
# vs leftover analog radio-chatter-empty-fail-closed bulk
# #177/#52ae (behavioral) vs leftover analog
...
# Leftover unique drafts never wrapped this Category.
...
# Identifier GetQueuedLineCount is NOT leftover analog
# ClearQueue / GetDroppedLineCount / GetPlayedLineCount /
# PrimeLines / EnqueueLine / AdvanceRadioState and NOT
# leftover analog RadioLine fields.
# BlueprintPure IS present. BlueprintCallable IS absent.
# const method IS present. Return type IS int32.
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# drop to BlueprintCallable, if const method is missing, if
...
# GetCurrentLineId / GetDroppedLineCount / GetPlayedLineCount
# are uncontracted.
# Do NOT claim those siblings. Do NOT claim OnLineStarted.
# Clone leftover unique BlueprintPure GetSortieHits #1685
# RETARGET method/class/Category. KEEP BlueprintPure
# (NOT BlueprintCallable). RETARGET class Gunner ->
# USkyguardRadioChatterComponent. RETARGET Category
# Combat|Sortie -> Skyguard|Radio. RETARGET
...
# Category HUD -> Radio and method/return. KEEP BlueprintPure.
# FAIL-CLOSED if LOCKED_DECL is void BindPresentation(.
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# Category Skyguard|Audio|Production — RETARGET Category
# Production -> Skyguard|Radio AND BlueprintCallable ->
# BlueprintPure AND void EnsureDefaultEntries(); ->
# int32 GetQueuedLineCount() const;.
# FAIL-CLOSED if LOCKED_DECL is void EnsureDefaultEntries();.
# Clone leftover unique TogglePause #1711 RETARGET;
# KEEP BlueprintPure (TogglePause is void no-args Callable).
# FAIL-CLOSED if LOCKED_DECL is void TogglePause();.
# Clone leftover unique AdvanceRadioState #1715 RETARGET;
# FAIL-CLOSED if LOCKED_DECL is
...
# Clone leftover unique PrimeLines #1713 RETARGET;
# FAIL-CLOSED if LOCKED_DECL is
...
# LOCKED_DECL is int32 GetQueuedLineCount() const;
...
# void ClearQueue(); and NOT
# bool EnqueueLine(const FSkyguardRadioLine& Line); and NOT
# leftover analog int32 PoolCapacity = 12
...
TARGET = "int32 GetQueuedLineCount() const;"
TARGET_SPLIT = (
    "int32 GetQueuedLineCount(\n"
    "\t) const;"
)
TARGET_INLINE = (
    "int32 GetQueuedLineCount() const { return Queue.Num(); }"
)
TARGET_WRONG_BARE = "int32 GetQueuedLineCount();"
...
TARGET_WRONG_FLOAT = (
    "float GetQueuedLineCount() const;"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "int32 GetQueuedLineCounts() const;"
...
TARGET_WRONG_TRUE = "int32 GetQueuedLineCount = true;"
TARGET_WRONG_ZERO = "int32 GetQueuedLineCount = 0.f;"
...
SIBLING_UNBOUND_CATEGORIES = "EnqueueLine"
...
THIS_SCRIPT = (
    "Scripts/tests/test_radio_chatter_get_queued_line_count"
    "_decl_contract.py"
)
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        f"{prefix}test_radio_chatter_enqueue_line_decl_contract.py",
        f"{prefix}test_radio_chatter_prime_lines_decl_contract.py",
        f"{prefix}test_radio_chatter_advance_radio_state"
        "_decl_contract.py",
        f"{prefix}test_radio_chatter_clear_queue_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bint32\s+GetQueuedLineCount\(\)\s*const(?:;|\s*\{)"
)
GET_QUEUED_LINE_COUNT_IDENT_RE = re.compile(r"\bGetQueuedLineCount\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # AND origin/main inline `{ return Queue.Num(); }`.
    ...
    # AdvanceRadioState / ClearQueue / GetDroppedLineCount /
    # GetPlayedLineCount, leftover analog GetSortieHits /
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bint32\s+GetQueuedLineCount\(\)\s*;",
        compact,
    ) and METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+GetQueuedLineCount\b", compact):
        return False
    if re.search(r"\bfloat\s+GetQueuedLineCount\b", compact):
        return False
    if re.search(r"\bbool\s+GetQueuedLineCount\b", compact):
        return False
    if re.search(
        r"\bint32\s+GetQueuedLineCount\(\)(?!\s*const)",
        compact,
    ):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetQueuedLineCount\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for GetQueuedLineCount "
...
class RadioChatterGetQueuedLineCountDeclContractTests(
    unittest.TestCase
):
    def test_radio_chatter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "GetQueuedLineCount"),
            section,
        )
    ...
    def test_missing_get_queued_line_count_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertTrue(
            "{ return Queue.Num(); }" in compact_origin
            or collapsed(TARGET) in compact_origin
        )
        self.assertNotIn("GetBoundPresentation", collapsed(LOCKED_DECL))
        self.assertTrue(
            GET_QUEUED_LINE_COUNT_IDENT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
    ...
    def test_get_queued_line_count_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GetQueuedLineCountX() const;\n",
            "\tint32 GetQueuedLineCount = 1;\n",
            "\tFName GetQueuedLineCount = NAME_None;\n",
            "\tint32 GetQueuedLineCount() = "
            + forty
            + ";\n",
            "\tbool EnqueueLine() = "
            ...
            f"\t{TARGET_WRONG_DROPPED}\n",
            f"\t{TARGET_WRONG_PLAYED}\n",
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "GetQueuedLineCount")
        )
        self.assertTrue(
            has_identifier(section, "GetQueuedLineCount")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio chatter get queued line count "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_chatter_get_queued_line_count"
            "_decl_contract.py"
        ))
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "Line"))
        self.assertIn("GetQueuedLineCount", LOCKED_DECL)
        for leftover in leftover_apache_ufunction_tokens():
            self.assertNotIn(leftover, locked_only)

    def test_word_boundary_get_queued_line_count_vs_siblings(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier(
        ...
        self.assertFalse(
            has_identifier(LOCKED_DECL, "EnqueueLine")
        )
        self.assertFalse(
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "GetQueuedLineCount"), section)
    ...
    def test_locked_decl_is_not_clone_ensure_toggle_hits_or_bind(
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_INLINE)
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 GetQueuedLineCount("))
