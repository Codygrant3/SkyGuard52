...
# BlueprintReadOnly) then BARE `FString Error;` (no initializer).
...
# bare FString Error;. Fail closed if type/decl missing, renamed,
...
# Error #1232.
# Declaration-only. Accept origin/main split-line UPROPERTY and
# FString Error;. Fail closed if type/decl
...
# BlueprintReadOnly FString Error; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly FString Error;
...
TARGET = "FString Error;"
TARGET_WRONG = "FString Error = TEXT(\"\");"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer for FSkyguardMission01EnvironmentAuthoringResult::
    # Error (must be bare FString Error;).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FString\s+Error\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FString Error is missing from "
...
class Mission01EnvironmentAuthoringResultErrorFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_error_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_error_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString Error"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("Error", LOCKED_DECL)
        ...
        self.assertNotIn("ESkyguardMission05ProtectedTarget", LOCKED_DECL)
        self.assertTrue(
            LOCKED_DECL.startswith("FString "),
            LOCKED_DECL,
        )
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("Error", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FString Error"))
