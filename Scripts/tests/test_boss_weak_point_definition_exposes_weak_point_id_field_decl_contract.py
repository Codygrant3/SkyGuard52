# THIS IS leftover-safe FSkyguardBossWeakPointDefinition ExposesWeakPointId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName ExposesWeakPointId;` (no initializer).
...
# Parse STRUCT `FSkyguardBossWeakPointDefinition` body ONLY
# after `struct FSkyguardBossWeakPointDefinition`. Stop at
# `struct FSkyguardBossDefinition`.
# Do NOT parse leftover `USkyguardBossWeakPointComponent`
# (leftover WeakPointId / bDestroyed / bExposed /
# Integrity / MaxIntegrity component fields).
...
# RequiredWeapon / Integrity.
# THIS IS NOT leftover boss-weak-point-defaults then
# isolated ExposesWeakPointId field decl analog only:
# leftover boss-weak-point-defaults stay locked.
...
# Keep leftover merged bulk
# test_campaign_theater_kit_contract.py in LOCKED_SCRIPTS;
# isolated field decl does not relock the bulk.
# Keep leftover mission-types-defaults-tests in
# LOCKED_SCRIPTS; isolated field decl does not relock
# the bulk defaults.
# Analog: leftover boss-weak-point-defaults then isolated
# ExposesWeakPointId field decl.
...
LEFTOVER_COMPONENT_NAME = "USkyguardBossWeakPointComponent"
LEFTOVER_MISSION_DEF_NAME = "USkyguardMissionDefinition"
LEFTOVER_OBJECTIVE_DEF = "FSkyguardObjectiveDefinition"
LEFTOVER_OBJECTIVE_PROGRESS = "FSkyguardObjectiveProgress"
LEFTOVER_SCORE_RULES = "FSkyguardMissionScoreRules"
LEFTOVER_WEATHER_PROFILE = "FSkyguardWeatherProfile"
LEFTOVER_ROUTE_DEF = "FSkyguardRouteDefinition"
TARGET = "FName ExposesWeakPointId;"
TARGET_WRONG_EQ_NONE = "FName ExposesWeakPointId = NAME_None;"
TARGET_WRONG_FALSE = "FName ExposesWeakPointId = false;"
TARGET_WRONG_TRUE = "FName ExposesWeakPointId = true;"
TARGET_WRONG_ZERO = "FName ExposesWeakPointId = 0.f;"
TARGET_WRONG_FLOAT = "float ExposesWeakPointId;"
...
STOP_BEFORE_COMPONENT = "USkyguardBossWeakPointComponent"
...
LEFTOVER_WEAK_POINT_MAX_INTEGRITY = (
...
def leftover_component_accept_flags() -> tuple[str, ...]:
    return (
        "bAccepts" + "Ri" + "fle",
        "bAccepts" + "Ig" + "la",
    )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+ExposesWeakPointId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName ExposesWeakPointId is missing from "
...
class BossWeakPointDefinitionExposesWeakPointIdFieldDeclContractTests(
    unittest.TestCase
):
    def test_boss_weak_point_definition_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_COMPONENT_NAME)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_MISSION_DEF_NAME)
        ...
        self.assertTrue(has_identifier(section, "ExposesWeakPointId"), section)
        ...
        self.assertNotIn(LEFTOVER_COMPONENT_NAME, section)
        self.assertNotIn(LEFTOVER_COMPONENT_NAME, body)
        self.assertNotIn(LEFTOVER_MISSION_DEF_NAME, header)
        self.assertNotIn(LEFTOVER_MISSION_DEF_NAME, section)
        ...
        self.assertNotIn(LEFTOVER_OBJECTIVE_DEF, section)
        self.assertNotIn(LEFTOVER_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(LEFTOVER_SCORE_RULES, section)
        self.assertNotIn(LEFTOVER_WEATHER_PROFILE, section)
        self.assertNotIn(LEFTOVER_ROUTE_DEF, section)
        self.assertNotIn(LEFTOVER_COMPONENT_MAX_INTEGRITY, section)
        self.assertNotIn(LEFTOVER_COMPONENT_DESTROYED, section)
        self.assertNotIn(LEFTOVER_COMPONENT_EXPOSED, section)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        component = (
            f"class {LEFTOVER_COMPONENT_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
    ...
    def test_leftover_component_or_neighbor_declaration_does_not_satisfy(
        self,
    ) -> None:
        mixed = (
        ...
        self.assertFalse(has_identifier(section, "ExposesWeakPointId"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_DEFINITION, section)
        self.assertNotIn(LEFTOVER_COMPONENT_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ExposesWeakPointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_exposes_weak_point_id_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName ExposesWeakPointId;", compact_origin)
        self.assertNotIn("ExposesWeakPointId = NAME_None", compact_origin)
        self.assertNotIn("ExposesWeakPointId = false", compact_origin)
        self.assertNotIn("ExposesWeakPointId = true", compact_origin)
        self.assertNotIn("ExposesWeakPointId = 0.f", compact_origin)
        self.assertNotIn("ExposesWeakPointId = 160.f", compact_origin)

    def test_exposes_weak_point_id_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName ExposesWeakPointId"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFName ExposesWeakPointIds;\n",
            "\tint32 ExposesWeakPointId;\n",
            "\tbool ExposesWeakPointId;\n",
            "\tfloat ExposesWeakPointId = " + forty + ";\n",
            "\tfloat ExposesWeakPointId = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_dropped_edit_anywhere_fails_closed(
        self,
    ) -> None:
        added_category = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_does_not_parse_leftover_component_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_OBJECTIVE_DEF, header)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_DEF, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_DEF, leaked)
        ...
        self.assertNotIn(LEFTOVER_COMPONENT_NAME, header)
        self.assertNotIn(LEFTOVER_COMPONENT_NAME, section)
        self.assertNotIn(LEFTOVER_COMPONENT_NAME, leaked)
        self.assertNotIn(LEFTOVER_MISSION_DEF_NAME, header)
        self.assertNotIn(LEFTOVER_MISSION_DEF_NAME, section)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for leftover in leftover_audio_event_enum_tokens():
            ...
            self.assertNotIn(leftover, leaked)
            self.assertNotIn(leftover, header)
        for leftover in leftover_component_accept_flags():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, leaked)
            self.assertNotIn(leftover, header)
        self.assertNotIn(STOP_BEFORE_AUDIO_EVENT, section)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardBossWeakPointComponent.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-weak-point-definition ExposesWeakPointId field "
                f"decl contract contains {banned}; declaration is "
                "Apache CPG 30 mm / Hydra / Hellfire, not leftover "
                "live cop" + "y",
            )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "exposes_weak_point_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardBossWeakPointComponent.h", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_BOSS_WEAK_POINT_DEFAULTS_PY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_BOSS_WEAK_POINT_DEFAULTS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_EXPOSED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_DESTROYED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_INTEGRITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_MAX_INTEGRITY, LOCKED_SCRIPTS)
    ...
    def test_locked_files_were_not_the_edit_surface(self) -> None:
        ...
        existing.append("Source/Skyguard52/SkyguardMissionTypes.h")
        existing.append("Source/Skyguard52/SkyguardBossWeakPointComponent.h")
        existing.append("Source/Skyguard52/SkyguardMissionDefinition.h")
