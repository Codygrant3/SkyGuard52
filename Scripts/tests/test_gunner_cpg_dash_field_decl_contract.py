# THIS IS leftover-safe ASkyguardGunner CpgDash.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgDash;`
# (no initializer).
...
# Stop BEFORE sibling CpgTedac as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeLeft, CpgKneeRight, CpgCanopyBow,
# CpgTedac, Boom, GunnerCamera, CpgMpdLeft.
# Do NOT claim leftover CpgCanopyBow / CpgKneeRight /
# CpgKneeLeft / CpgSeatBack / CpgCockpit / CpgStation
# (in-flight siblings). CpgSeatPan and CpgCanopyBow
# are sibling in-flight slots.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgDash;`
...
# CpgDash fail-closed if LOCKED_DECL is leftover
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgCockpit /
# CpgStation / CpgSeatPan / CpgCanopyBow / leftover
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgDash;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgDash = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgDash = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgDash = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgDash;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgDash;"
...
CPG_DASH_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgDash\s*;"
)
CPG_DASH_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgDash\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Apache Canopy, or an initializer on CpgDash as
    # the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_DASH_DECL_RE.search(compact) is None:
        return False
    if CPG_DASH_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_TEDAC):
            break
    ...
    if has_identifier(window, SIBLING_CPG_TEDAC):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgDash\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgDash "
...
class GunnerCpgDashFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgDash"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_TEDAC), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_TEDAC),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgDash"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgDash", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_dash_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgDash;", compact_origin)
        self.assertNotIn("CpgDash = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_dash_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgDash"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgDash;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgDashs;\n",
            "\tint32 CpgDash = 140;\n",
            "\tbool CpgDash = true;\n",
            "\tfloat CpgDash = " + forty + ";\n",
            "\tfloat CpgDash = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_tedac_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(TARGET_WRONG_TEDAC, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgDash field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_dash_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
