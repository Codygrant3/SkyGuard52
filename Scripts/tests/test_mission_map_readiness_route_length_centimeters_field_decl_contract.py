...
# RouteLengthCentimeters FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe MissionMapReadiness
# RouteLengthCentimeters. THIS IS NOT leftover mission-map-
...
# bRouteMatchesDefinition #1158 / leftover
# bWeatherMatchesDefinition #1159 / leftover
# bLandmarksDistinct #1160 / leftover
# RoutePointCount #1161 / leftover
# ObjectiveAnchorCount #1162 / leftover
# LandmarkCount #1163 / leftover
# MissionExclusiveLandmarkCount #1164. LOCK those
# sibling scripts. LOCK leftover Mission01
# environment director RouteLengthCm #593
# (different director field). Do not parse
...
# UPROPERTY(Transient).
# THIS IS leftover-safe MissionMapReadiness
# RouteLengthCentimeters. THIS IS NOT leftover Mission05
...
# merge #665 #666 #668-#673, or open drafts #674-#1164.
...
# this map test file (Mission08/09-
# only values). Do not treat
# RouteLengthCentimeters = 0.f as a
# Harbor literal. Incoming clock names must be absent.
...
# MissionMapReadiness RouteLengthCentimeters
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float RouteLengthCentimeters = 0.f;
# one-line / split-line UPROPERTY wraps;
# origin/main has NO Category) on
# FSkyguardMissionMapReadiness. It is
# leftover-safe MissionMapReadiness
# RouteLengthCentimeters FIELD (declaration-only). It is
...
# drafts #674-#1164 including leftover Mission02
...
# drafts #674-#1164. Do not reopen leftover
...
TARGET = "float RouteLengthCentimeters = 0.f;"
TARGET_WRONG = "float RouteLengthCentimeters = 1.f;"
LOCKED_DECL = TARGET
...
class MissionMapReadinessRouteLengthCentimetersFieldDeclContractTests(unittest.TestCase):
    def test_mission_map_readiness_struct_exists(self) -> None:
        ...
        self.assertIn("bRequiredObjectivesAnchored", section)
        self.assertIn("bLandmarksDistinct", section)
        self.assertIn("bWeatherMatchesDefinition", section)
        self.assertIn("RouteLengthCentimeters", section)
        self.assertIn("RoutePointCount", section)
        self.assertIn("MissionExclusiveLandmarkCount", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RouteLengthCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_route_length_centimeters_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_route_length_centimeters_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float RouteLengthCentimeters"),
            LOCKED_DECL,
        )
        self.assertTrue(
            LOCKED_DECL.endswith("0.f;")
            or LOCKED_DECL.endswith("0.0f;"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("RouteLengthCentimeters", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("RouteLengthCentimeters", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float RouteLengthCentimeters"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("RouteLengthCentimeters", locked_only)
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            self.assertNotEqual(token, "550.f")
            self.assertNotEqual(token, "450.f")
            self.assertNotEqual(token, "0.f")
            self.assertNotEqual(token, "0.0f")
            self.assertNotEqual(
                token,
    ...
    def test_sibling_director_fields_do_not_satisfy_route_length_centimeters(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_leftover_mission_map_get_readiness_sibling_stays_locked(
        ...
        self.assertIn(
            "Scripts/tests/test_mission_map_readiness_objective"
            "_anchor_count_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission_map_readiness_landmark"
            "_count_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission_map_readiness_mission"
            "_exclusive_landmark_count_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission01_environment_readiness"
