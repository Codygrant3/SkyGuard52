# THIS IS leftover-safe FSkyguardMissionSaveRecord bCompleted.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `bool bCompleted = false;`.
...
# BlueprintReadWrite, type not bool, initializer dropped
...
# MigrateCampaignSave).
# Sibling decls stay unclaimed: BestScore / BestMedalTier /
# BestCompletionTimeSeconds.
# Analog leftover mission-save-record-defaults #206 / #ac38
# then isolated bCompleted field decl.
...
# `= true` / bare bool, retarget: type is bool,
# initializer is `= false`, BlueprintReadWrite, no Category.
...
TARGET = "bool bCompleted = false;"
TARGET_WRONG_BARE = "bool bCompleted;"
TARGET_WRONG_TRUE = "bool bCompleted = true;"
TARGET_WRONG_ZERO = "bool bCompleted = 0;"
TARGET_WRONG_FLOAT = "float bCompleted = false;"
TARGET_WRONG_FNAME = "FName bCompleted;"
TARGET_WRONG_FTEXT = "FText bCompleted;"
TARGET_WRONG_INT = "int32 bCompleted = false;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare / `= true` / `= 0` /
    # `= 0.f` / leftover float Health when origin/main
    # has bool = false. Do not accept sibling BestScore /
    # BestMedalTier / BestCompletionTimeSeconds fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"bool\s+bCompleted\s*=\s*true",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bCompleted\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bCompleted\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+bCompleted\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bCompleted\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+bCompleted\s*=\s*false",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bCompleted\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bCompleted is missing from "
...
class MissionSaveRecordCompletedFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_completed_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bCompleted = false;", compact_origin)
        self.assertNotIn("bCompleted = true", compact_origin)
        self.assertNotIn("bCompleted = 0.f", compact_origin)
        self.assertNotIn("bCompleted = 160.f", compact_origin)

    def test_completed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bCompleted"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tbool bCompletedes = true;\n",
            "\tint32 bCompleted = 1;\n",
            "\tFText bCompleted;\n",
            "\tfloat bCompleted = " + forty + ";\n",
            "\tfloat bCompleted = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-save-record bCompleted field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "completed_field_decl_contract.py"
        ))
