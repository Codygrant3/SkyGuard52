...
# then BARE `void AdvanceRadioState(float DeltaSeconds);`
# (declaration-only, no inline body required).
# KEEP DeltaSeconds arg from origin/main.
# KEEP BlueprintCallable. Do NOT retarget to
# PrimeLines / EnqueueLine / ClearQueue /
# GetQueuedLineCount / GetCurrentLineId /
# GetDroppedLineCount / GetPlayedLineCount.
# Word-boundary match AdvanceRadioState vs
# PrimeLines vs EnqueueLine vs ClearQueue.
...
# Identifier AdvanceRadioState is NOT leftover analog
# PrimeLines / EnqueueLine and NOT leftover analog
# RadioLine fields.
# BlueprintCallable IS present. BlueprintPure IS absent.
# const method IS absent. Return type IS void.
...
# DeltaSeconds arg is missing, if the return type is
...
# PrimeLines / EnqueueLine / ClearQueue /
# GetQueuedLineCount / GetCurrentLineId /
# GetDroppedLineCount / GetPlayedLineCount are uncontracted.
...
# KEEP BlueprintCallable (NOT BlueprintPure). KEEP arg.
...
# RETARGET method/return/args. KEEP BlueprintCallable KEEP arg.
# Clone leftover unique BindPresentation #1707
# RETARGET Category HUD -> Radio and method/args.
# KEEP BlueprintCallable KEEP arg. This slot returns void.
# FAIL-CLOSED if LOCKED_DECL is void BindPresentation(.
# Clone leftover unique EnqueueLine — RETARGET method/return;
# KEEP Category Radio KEEP BlueprintCallable KEEP arg.
...
# LOCKED_DECL is void AdvanceRadioState(float DeltaSeconds);
...
# void ClearQueue(); and NOT leftover analog int32 PoolCapacity = 12
...
TARGET = "void AdvanceRadioState(float DeltaSeconds);"
TARGET_SPLIT = (
    "void AdvanceRadioState(\n"
    "\tfloat DeltaSeconds);"
)
TARGET_INLINE = TARGET
TARGET_WRONG_BARE = "void AdvanceRadioState();"
TARGET_WRONG_NO_CONST = (
    "void AdvanceRadioState(float DeltaSeconds) const;"
)
TARGET_WRONG_REFRESH = "void PrimeLines(const TArray<FSkyguardRadioLine>& Lines);"
TARGET_WRONG_REBIND = "bool EnqueueLine(const FSkyguardRadioLine& Line);"
...
TARGET_WRONG_FLOAT = (
    "float AdvanceRadioState(float DeltaSeconds);"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void AdvanceRadioStates(float DeltaSeconds);"
...
TARGET_WRONG_TRUE = "bool AdvanceRadioState = true;"
TARGET_WRONG_ZERO = "bool AdvanceRadioState = 0.f;"
...
SIBLING_EVALUATE_READINESS = "EnqueueLine"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bvoid\s+AdvanceRadioState\("
    r"float DeltaSeconds\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept a method const qualifier, a bool/float/int32
    ...
    # ClearQueue, leftover analog GetSortieHits /
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bAdvanceRadioState\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bbool\s+AdvanceRadioState\b", compact):
        return False
    if re.search(r"\bfloat\s+AdvanceRadioState\b", compact):
        return False
    if re.search(r"\bint32\s+AdvanceRadioState\b", compact):
        return False
    if re.search(r"\bvoid\s+AdvanceRadioState\s*\(\s*\)", compact):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+AdvanceRadioState\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for AdvanceRadioState "
...
class RadioChatterAdvanceRadioStateDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_advance_radio_state_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertTrue(
            ADVANCE_RADIO_IDENT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
    ...
    def test_advance_radio_state_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_identifier(LOCKED_DECL, "DeltaSeconds"))
        self.assertNotIn(
            "FSkyguardRadioLine",
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid AdvanceRadioStateX(float DeltaSeconds);\n",
            "\tint32 AdvanceRadioState = 1;\n",
            "\tFName AdvanceRadioState = NAME_None;\n",
            "\tvoid AdvanceRadioState() = "
            + forty
            + ";\n",
            "\tvoid AdvanceRadioState() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "AdvanceRadioState")
        )
        self.assertTrue(
            has_identifier(section, "AdvanceRadioState")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio chatter advance radio state "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_chatter_advance_radio_state"
            "_decl_contract.py"
        ))
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertTrue(has_identifier(LOCKED_DECL, "DeltaSeconds"))
        self.assertIn("AdvanceRadioState", LOCKED_DECL)
        for leftover in leftover_apache_ufunction_tokens():
            self.assertNotIn(leftover, locked_only)

    def test_word_boundary_advance_radio_state_vs_prime_enqueue_clear(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier(
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "AdvanceRadioState"), section)
        self.assertFalse(has_identifier(section, "PrimeLines"), section)
        self.assertFalse(
            has_identifier(section, "EnqueueLine"),
            section,
        )
        self.assertEqual(declaration_count(section, LOCKED_DECL), 1)
    ...
    def test_locked_decl_is_not_clone_ensure_toggle_hits_or_bind(
        ...
        self.assertTrue(LOCKED_DECL.startswith("void AdvanceRadioState("))
        self.assertIn("float DeltaSeconds", LOCKED_DECL)
        self.assertIsNone(re.search(r"\)\s*const\b", LOCKED_DECL))
        self.assertIn(
            'Category="Skyguard|Radio"',
            LOCKED_UFUNCTION_WRAP,
