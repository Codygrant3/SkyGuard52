# THIS IS leftover-safe ASkyguardGunner CpgMpdLeftText.
...
# then BARE `TObjectPtr<UTextRenderComponent> CpgMpdLeftText;`
# (no initializer).
...
# Stop BEFORE sibling CpgMpdRightText as a claimed slot.
...
# LOCKED_DECL is `TObjectPtr<UTextRenderComponent> CpgMpdLeftText;`
...
# CpgMpdLeftText fail-closed if LOCKED_DECL is leftover
...
# CpgMpdRightText / CpgEufdText / CpgEufd / CpgTedac /
...
TARGET = "TObjectPtr<UTextRenderComponent> CpgMpdLeftText;"
TARGET_WRONG_INIT = "TObjectPtr<UTextRenderComponent> CpgMpdLeftText = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UTextRenderComponent> CpgMpdLeftText = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UTextRenderComponent> CpgMpdLeftText = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgMpdLeftText;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgMpdLeftText;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CpgMpdLeftText;"
...
CPG_MPD_LEFT_TEXT_DECL_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgMpdLeftText\s*;"
)
CPG_MPD_LEFT_TEXT_INIT_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgMpdLeftText\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # PilotCanopy, or an initializer on CpgMpdLeftText
    ...
    if CPG_MPD_LEFT_TEXT_DECL_RE.search(compact) is None:
        return False
    if CPG_MPD_LEFT_TEXT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_MPD_RIGHT_TEXT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_MPD_RIGHT_TEXT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_MPD_RIGHT_TEXT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UTextRenderComponent>\s+CpgMpdLeftText\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UTextRenderComponent> CpgMpdLeftText "
...
class GunnerCpgMpdLeftTextFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgMpdLeftText"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgMpdLeftText"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgMpdLeftText", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UTextRenderComponent> CpgMpdLeftText;", compact_origin)
        self.assertNotIn("CpgMpdLeftText = 160.f", compact_origin)
    ...
    def test_cpg_mpd_left_text_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UTextRenderComponent> CpgMpdLeftText"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UTextRenderComponent> CpgMpdLeftText;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UTextRenderComponent> CpgMpdLeftTexts;\n",
            "\tint32 CpgMpdLeftText = 140;\n",
            "\tbool CpgMpdLeftText = true;\n",
            "\tfloat CpgMpdLeftText = " + forty + ";\n",
            "\tfloat CpgMpdLeftText = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_mpd_right_text_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgMpdLeftText"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgMpdLeftText"))
        self.assertTrue(has_identifier(section, "CpgMpdLeftText"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgMpdLeftText field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_mpd_left_text_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
