...
# or lock the GetBriefingText body.
# origin/main is one line
# (`FText GetBriefingText() const;`);
# accept that exact one-line form, split-line wraps,
# and an inline body without locking the .cpp body.
# Nearby origin/main
...
# USkyguardDebriefWidget. This is not
# USkyguardMissionBriefingComponent::GetBriefingText
# (draft #374). Leftover sortie-presentation
# fail-closed #7600, leftover briefing-fail-closed
# #9fe9, leftover briefing-card defaults #8c3f,
# leftover briefing-radio-row defaults #bf8e,
# leftover how-to-fly-row defaults, leftover
# mission-briefing-state enum, leftover
# radio-chatter empty-fail-closed, leftover
# briefing-component GetBriefingText #374,
...
# stay sibling-only.
GET_BRIEFING_TEXT = "FText GetBriefingText() const;"
UFUNCTION_BRIEFING = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Presentation|Briefing")'
)
# Leftover #56–#64 plus this class's production files.
# This lane only adds an isolated Python GetBriefingText
# declaration contract on USkyguardBriefingWidget.
# Stay off Configure, GetPresentation, GetMissionTitle,
...
# GetBriefingText #374, leftover sortie-presentation
...
# USkyguardDebriefWidget, sibling Configure,
# sibling GetPresentation, and dirty workspace paths.
LOCKED = {
...
# contracts including GetBriefingText #374, leftover
# Configure, leftover GetPresentation, leftover
# GetMissionTitle, leftover GetBriefingCards,
...
# here. Configure / GetPresentation / GetMissionTitle /
# GetBriefingCards / GetRadioRows / GetHowToFlyRows /
...
# Leftover briefing-component GetBriefingText (draft #374)
# stays unlocked. Same signature, different class.
COMPONENT_GET_BRIEFING_TEXT_NOT_LOCKED = (
...
# .cpp GetBriefingText body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. An inline `{ return Other; }` is accepted
# as presence, not a locked implementation contract.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "USkyguardBriefingWidget::GetBriefingText",
    "SkyguardSortiePresentationWidgets.cpp",
)
SIBLING_TYPES = (
...
class BriefingWidgetGetBriefingTextDeclContractTests(unittest.TestCase):
    def test_briefing_widget_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_BRIEFING_TEXT),
            section,
        )
    ...
    def test_briefing_component_does_not_satisfy_class(self) -> None:
        component_only = (
            "class SKYGUARD52_API USkyguardMissionBriefingComponent "
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_BRIEFING_TEXT)
        self.assertIn("GetBriefingText", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_BRIEFING_TEXT))

    def test_missing_get_briefing_text_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tvoid Configure("
            "USkyguardSortiePresentationComponent* InPresentation);\n"
            "\tUSkyguardSortiePresentationComponent* "
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_BRIEFING_TEXT)
        self.assertIn("GetBriefingText", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_BRIEFING_TEXT)
        self.assertIn("GetBriefingText", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_BRIEFING_TEXT)
        self.assertNotIn("UFUNCTION", GET_BRIEFING_TEXT)
        self.assertNotIn("Category", GET_BRIEFING_TEXT)
        self.assertNotIn("BlueprintCallable", GET_BRIEFING_TEXT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_BRIEFING_TEXT)
        self.assertIn("GetBriefingText", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tFText GetBriefingText();\n"
        wrong_return_void = "\tvoid GetBriefingText() const;\n"
        wrong_return_bool = "\tbool GetBriefingText() const;\n"
        wrong_return_fstring = "\tFString GetBriefingText() const;\n"
        added_arg = "\tFText GetBriefingText(bool bReady) const;\n"
        wrong_name = "\tFText GetMissionTitle() const;\n"
        const_ref = "\tconst FText& GetBriefingText() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            wrong_return_fstring,
            added_arg,
            wrong_name,
            const_ref,
        ):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_BRIEFING_TEXT)
            self.assertIn("GetBriefingText", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_briefing_text_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_BRIEFING_TEXT),
            GET_BRIEFING_TEXT,
        )
        self.assertTrue(has_declaration(section, GET_BRIEFING_TEXT))
        self.assertEqual(
            declaration_count(section, GET_BRIEFING_TEXT),
            1,
        )
        self.assertTrue(
            GET_BRIEFING_TEXT.startswith("FText "),
            GET_BRIEFING_TEXT,
        )
        self.assertTrue(
            GET_BRIEFING_TEXT.endswith(";"),
            GET_BRIEFING_TEXT,
        )
        self.assertIn("GetBriefingText()", GET_BRIEFING_TEXT)
        self.assertIn(" const", GET_BRIEFING_TEXT)
        self.assertIn("FText", GET_BRIEFING_TEXT)
        self.assertNotIn("INDEX_NONE", GET_BRIEFING_TEXT)
        self.assertNotIn("{", GET_BRIEFING_TEXT)
        self.assertNotIn("}", GET_BRIEFING_TEXT)
        self.assertNotIn("return ", GET_BRIEFING_TEXT)
        self.assertNotIn("return BriefingText", GET_BRIEFING_TEXT)

    def test_declaration_accepts_origin_main_one_line_and_inline(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_BRIEFING_TEXT}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_BRIEFING_TEXT))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_BRIEFING_TEXT),
            GET_BRIEFING_TEXT,
        )
        exact_inline = "FText GetBriefingText() const { return Other; }"
        self.assertTrue(has_declaration(exact_inline, GET_BRIEFING_TEXT))
        self.assertEqual(
            require_declaration(exact_inline, GET_BRIEFING_TEXT),
            GET_BRIEFING_TEXT,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_BRIEFING_TEXT),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
        ...
        self.assertNotIn("{", GET_BRIEFING_TEXT)
        self.assertNotIn("return BriefingText", GET_BRIEFING_TEXT)
        self.assertNotIn("return Other", GET_BRIEFING_TEXT)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tFText\n"
            "\tGetBriefingText() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tFText GetBriefingText(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_BRIEFING_TEXT}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_BRIEFING_TEXT))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_BRIEFING_TEXT),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_BRIEFING_TEXT}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_BRIEFING_TEXT)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_BRIEFING_TEXT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_briefing_text_body(self) -> None:
        locked_only = f"{GET_BRIEFING_TEXT}\n"
        self.assertNotIn("{", GET_BRIEFING_TEXT)
        self.assertNotIn("}", GET_BRIEFING_TEXT)
        self.assertNotIn("return ", GET_BRIEFING_TEXT)
        self.assertNotIn("return BriefingText", GET_BRIEFING_TEXT)
        self.assertNotIn("return BriefingText", locked_only)
        self.assertNotIn(
            "USkyguardBriefingWidget::GetBriefingText",
            GET_BRIEFING_TEXT,
        )
        ...
        self.assertNotIn("return false", GET_BRIEFING_TEXT)
        self.assertNotIn("return true", GET_BRIEFING_TEXT)

    def test_contract_does_not_relock_configure(self) -> None:
        locked_only = f"{GET_BRIEFING_TEXT}\n"
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("Configure(", GET_BRIEFING_TEXT)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("GetPresentation", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_get_mission_title(self) -> None:
        ...
        for neighbor in GET_MISSION_TITLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("GetMissionTitle", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_get_briefing_cards(self) -> None:
        ...
        for neighbor in GET_BRIEFING_CARDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("GetBriefingCards", GET_BRIEFING_TEXT)
        self.assertNotIn("GetBriefingCards", locked_only)
        self.assertNotIn("FSkyguardBriefingCard", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_get_radio_rows(self) -> None:
        ...
        for neighbor in GET_RADIO_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("GetRadioRows", GET_BRIEFING_TEXT)
        self.assertNotIn("GetRadioRows", locked_only)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_get_how_to_fly_rows(self) -> None:
        ...
        for neighbor in GET_HOW_TO_FLY_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("GetHowToFlyRows", GET_BRIEFING_TEXT)
        self.assertNotIn("GetHowToFlyRows", locked_only)
        self.assertNotIn("FSkyguardHowToFlyRow", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_acknowledge_briefing(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("AcknowledgeBriefing", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_launch_sortie(self) -> None:
        ...
        for neighbor in LAUNCH_SORTIE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("LaunchSortie", GET_BRIEFING_TEXT)
        self.assertNotIn("LaunchSortie", locked_only)

    def test_contract_does_not_relock_briefing_component_get_briefing_text(
        self,
    ) -> None:
        locked_only = f"{GET_BRIEFING_TEXT}\n"
        for token in COMPONENT_GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BRIEFING_TEXT)
        self.assertNotIn("USkyguardMissionBriefingComponent", GET_BRIEFING_TEXT)
        self.assertNotIn("return BriefingText", GET_BRIEFING_TEXT)
        self.assertNotIn(
            "test_briefing_get_briefing_text_decl_contract.py",
            GET_BRIEFING_TEXT,
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BRIEFING_TEXT)
        self.assertNotIn("Unconfigured", GET_BRIEFING_TEXT)
        self.assertNotIn("Warming", GET_BRIEFING_TEXT)
        self.assertNotIn("Launched", GET_BRIEFING_TEXT)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_BRIEFING_TEXT,
        )
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation(
        ...
        self.assertNotIn("GetPresentationState", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_TEXT)
        self.assertNotIn("FillResultCombatStats", GET_BRIEFING_TEXT)
        self.assertNotIn("ASkyguardGunner", GET_BRIEFING_TEXT)
        self.assertNotIn("FillAndFinalize", GET_BRIEFING_TEXT)
        self.assertNotIn("FillAndFail", GET_BRIEFING_TEXT)
        self.assertNotIn("ApplyHydraForClusters", GET_BRIEFING_TEXT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_BRIEFING_TEXT}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_BRIEFING_TEXT),
            GET_BRIEFING_TEXT,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing widget GetBriefingText contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_BRIEFING_TEXT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing widget GetBriefingText contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_BRIEFING_TEXT)

    def test_contract_is_get_briefing_text_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_BRIEFING_TEXT.startswith("FText "))
        self.assertTrue(GET_BRIEFING_TEXT.endswith(";"))
        self.assertIn(" const", GET_BRIEFING_TEXT)
        self.assertIn(UFUNCTION_BRIEFING, section)
