...
# then BARE `TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() const;`
# (declaration-only, const IS present, return type IS
# TArray<ESkyguardProductionAudioCategory>).
# THIS IS leftover-safe isolated UFUNCTION.
# Category IS `Skyguard|Audio|Production`. BlueprintPure IS present.
# const IS present. Return type IS TArray<ESkyguardProductionAudioCategory>.
# Declaration-only.
# Clone leftover unique analog BlueprintPure EvaluateReadiness #1682
# SHA 55627e805928626c211809c69f1707b159fe4720 wrapping
# `FSkyguardProductionAudioAudit EvaluateReadiness() const;`
# RETARGET hard: method EvaluateReadiness -> GetUnboundRequiredCategories;
# return FSkyguardProductionAudioAudit ->
# TArray<ESkyguardProductionAudioCategory>.
# KEEP BlueprintPure. KEEP const. KEEP Category="Skyguard|Audio|Production".
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# drop to BlueprintCallable, if const is missing, if the
# return type is not TArray<ESkyguardProductionAudioCategory>, or if
# Category is Skyguard|Apache / Skyguard|Combat /
...
# Category path / Category="Skyguard|Audio" without |Production.
...
# Stop BEFORE first public UPROPERTY (`Entries` / `Routing`).
# Do NOT claim sibling UFUNCTION EnsureDefaultEntries /
# InitializeRequiredEntries / ConfigureRoutingTopology /
# EvaluateReadiness as this slot
# (leave them uncontracted).
# Do NOT claim leftover unique analog EvaluateReadiness /
# FindEntry / GetRequiredCategories / GetCategoryDisplayName /
# GetResolvedProductionLoopRouteCount as this slot.
# Do NOT claim leftover analog FSkyguardProductionAudioAudit
# struct fields (this slot is the UFUNCTION only).
# Do NOT claim leftover analog Production Audio Entry /
# Routing struct fields.
...
# Skip leftover analog bank UFUNCTION EnsureDefaultEntries /
# InitializeRequiredEntries / ConfigureRoutingTopology.
# Skip leftover analog EvaluateReadiness.
# Skip leftover analog Pathfinder Encounter UFUNCTION.
# Skip leftover analog pathfinder-encounter-controller-field.
# Skip leftover analog RouteLengthCm / MinHeightFromOriginCm
# Harbor leftover analog / LockWindowClimbSeconds /
# bAutoAdvance / ObservedPhase / multicast / PathfinderBoss.
# Skip leftover analog Mix same-name
# CockpitExteriorAttenuation / CockpitLowPassHz.
# Skip leftover analog audio-production-bank-empty-fail-closed
# bulk.
# Skip leftover analog audio-director fail-closed bulk.
# Skip leftover analog Category Skyguard|Audio without
# |Production UFUNCTION bulk.
# Skip leftover unique Pathfinder Encounter Route / Safety /
# Commands / Telegraph FIELD.
# Skip private members.
# Clone wrap was BlueprintPure
# Category="Skyguard|Audio|Production" wrapping
# `TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() const;`
# — RETARGET hard: class USkyguardAudioDirectorComponent
# -> USkyguardAudioProductionBank, method ->
# GetUnboundRequiredCategories, return TArray<ESkyguardProductionAudioCategory>.
# KEEP BlueprintPure. KEEP const.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is `TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() const;`
# NOT `FName WeatherIdentity;` and NOT GetLampInstanceCount
# and NOT GetResolvedProductionLoopRouteCount and NOT
# EvaluateReadiness / FindEntry / GetRequiredCategories and NOT
# EnsureDefaultEntries / InitializeRequiredEntries /
# ConfigureRoutingTopology / GetCategoryDisplayName
# and NOT GlobalVoiceLimit = 24 and NOT OpenCockpitWindLoop
# and NOT EngineIdleLoop and NOT IsDestroyed leftover analog
# and NOT FireRate = 12.0f / RecoilPitch = 0.92f and NOT
# leftover Boom and NOT leftover VegetationStartCullDistanceCm
# and NOT RouteLengthCm = 45000.f and NOT leftover analog
# MinHeightFromOriginCm Harbor leftover and NOT
# LockWindowSpeedCmPerSecond = 850.f and NOT
# ApproachAttackIntervalSeconds = 6.5f and NOT
...
# EvaluateReadiness / FindEntry / GetRequiredCategories /
# EnsureDefaultEntries / InitializeRequiredEntries /
# ConfigureRoutingTopology / GetCategoryDisplayName /
...
# leftover VegetationStartCullDistanceCm / 160.f /
# RouteLengthCm = 45000.f / leftover analog Harbor
# MinHeightFromOriginCm / LockWindowSpeedCmPerSecond = 850.f /
...
TARGET = "TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() const;"
TARGET_INLINE = (
    "TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() const { return false; }"
)
TARGET_WRONG_BARE = "TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories();"
TARGET_WRONG_NO_CONST = (
    "TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() { return false; }"
)
TARGET_WRONG_VOID = "void GetUnboundRequiredCategories() const;"
TARGET_WRONG_FLOAT = "float GetUnboundRequiredCategories() const;"
TARGET_WRONG_INT = "int32 GetUnboundRequiredCategories() const;"
TARGET_WRONG_RENAME = "bool AreResolvedProductionLoopsComplete() const;"
TARGET_WRONG_SIBLING_COUNT = (
    "int32 GetResolvedProductionLoopRouteCount() const;"
)
TARGET_WRONG_CLONE_ARE_RESOLVED = (
    "bool AreResolvedProductionLoopRoutesComplete() const;"
)
TARGET_WRONG_ENSURE = "void EnsureDefaultEntries();"
TARGET_WRONG_INITIALIZE = "void InitializeRequiredEntries();"
TARGET_WRONG_CONFIGURE = "bool ConfigureRoutingTopology();"
TARGET_WRONG_UNBOUND = (
    "FSkyguardProductionAudioAudit "
    "EvaluateReadiness() const;"
)
...
TARGET_WRONG_FIND_ENTRY = (
    "const FSkyguardProductionAudioEntry* FindEntry("
    "ESkyguardProductionAudioCategory Category) const;"
)
TARGET_WRONG_GET_REQUIRED_CATEGORIES = (
    "static const TArray<ESkyguardProductionAudioCategory>& "
    "GetRequiredCategories();"
)
TARGET_WRONG_GET_CATEGORY_DISPLAY_NAME = (
    "static FText GetCategoryDisplayName("
    "ESkyguardProductionAudioCategory Category);"
)
TARGET_WRONG_BOOL_RETURN = "bool GetUnboundRequiredCategories() const;"
TARGET_WRONG_ROUTE_LENGTH = "float RouteLengthCm = 45000.f;"
TARGET_WRONG_LOCK_WINDOW_SPEED = (
    "float LockWindowSpeedCmPerSecond = 850.f;"
)
TARGET_WRONG_APPROACH_INTERVAL = (
    "float ApproachAttackIntervalSeconds = 6.5f;"
)
TARGET_WRONG_TELEGRAPH_LEAD = (
...
TARGET_WRONG_TRUE = "TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories = true;"
TARGET_WRONG_ZERO = "TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories = 0.f;"
...
SIBLING_CLONE_ARE_RESOLVED = "AreResolvedProductionLoopRoutesComplete"
SIBLING_ENSURE_DEFAULT = "EnsureDefaultEntries"
SIBLING_INITIALIZE_REQUIRED = "InitializeRequiredEntries"
SIBLING_CONFIGURE_ROUTING = "ConfigureRoutingTopology"
SIBLING_UNBOUND_CATEGORIES = "EvaluateReadiness"
...
STOP_BEFORE_SKYLINE_STYLE = "Entries"
...
THIS_SCRIPT = (
    "Scripts/tests/test_audio_production_bank_get_unbound_required_categories"
    "_decl_contract.py"
)
...
LEFTOVER_AUDIO_PRODUCTION_BANK_EVALUATE_READINESS = (
    "Scripts/tests/test_audio_production_bank_evaluate_readiness"
    "_decl_contract.py"
)
LEFTOVER_AUDIO_PRODUCTION_BANK_ENSURE_DEFAULT = (
    "Scripts/tests/test_audio_production_bank_ensure_default_entries"
    "_decl_contract.py"
)
LEFTOVER_AUDIO_PRODUCTION_BANK_INITIALIZE_REQUIRED = (
    "Scripts/tests/test_audio_production_bank_initialize_required_entries"
    "_decl_contract.py"
)
LEFTOVER_AUDIO_PRODUCTION_BANK_CONFIGURE_ROUTING = (
    "Scripts/tests/test_audio_production_bank_configure_routing_topology"
    "_decl_contract.py"
)
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bTArray\s*<\s*ESkyguardProductionAudioCategory\s*>\s+GetUnboundRequiredCategories\s*\(\s*\)\s*const\s*(?:;|\{)"
)
...
def leftover_analog_field_in_region(compact: str) -> bool:
    ...
    if re.search(r"RouteLengthCm\s*=\s*45000\.f\b", compact):
        return True
    if re.search(r"LockWindowSpeedCmPerSecond\s*=\s*850\.f\b", compact):
        return True
    if re.search(r"ApproachAttackIntervalSeconds\s*=\s*6\.5f\b", compact):
        return True
    if re.search(r"AttackTelegraphLeadSeconds\s*=\s*1\.2f\b", compact):
        return True
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # missing const, a void/float/int32 rename, extra params,
    # leftover analog IsDestroyed / GetLampInstanceCount /
    # GetResolvedProductionLoopRouteCount, leftover FName
    # WeatherIdentity, leftover GlobalVoiceLimit = 24,
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+GetUnboundRequiredCategories\b", compact):
        return False
    if re.search(r"\bfloat\s+GetUnboundRequiredCategories\b", compact):
        return False
    if re.search(r"\bint32\s+GetUnboundRequiredCategories\b", compact):
        return False
    if re.search(r"\bbool\s+GetUnboundRequiredCategories\b", compact):
        return False
    if re.search(r"\bAreResolvedProductionLoopRoutesComplete\b", compact):
        return False
    if re.search(
        r"\bTArray\s*<\s*ESkyguardProductionAudioCategory\s*>\s+GetUnboundRequiredCategories\s*\(\s*[^)]+\s*\)",
        compact,
    ):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*ESkyguardProductionAudioCategory\s*>\s+GetUnboundRequiredCategories\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories "
...
class AudioProductionBankGetUnboundRequiredCategoriesDeclContractTests(
    unittest.TestCase
):
    def test_audio_production_bank_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "GetUnboundRequiredCategories"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENSURE_DEFAULT), body)
        self.assertTrue(has_identifier(section, SIBLING_ENSURE_DEFAULT), section)
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_ENSURE_DEFAULT))
        self.assertFalse(has_identifier(section, SIBLING_ENGINE_IDLE_LOOP), section)
        self.assertFalse(has_identifier(section, SIBLING_ENGINE_CRUISE_LOOP), section)
        self.assertFalse(
            has_identifier(section, SIBLING_EVENT_DEFINITIONS),
            section,
        )
        self.assertFalse(has_identifier(section, SIBLING_GLOBAL_VOICE_LIMIT), section)
        self.assertFalse(has_identifier(section, SIBLING_PROPELLER_LOOP), section)
    ...
    def test_missing_get_unbound_required_categories_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn(UFUNCTION_PURE, section)
        ...
        self.assertIsNone(
            re.search(r'Category="Skyguard"(?!\|)', UFUNCTION_PURE)
        )
        self.assertIn("BlueprintPure", UFUNCTION_PURE)
        self.assertIn("Category", UFUNCTION_PURE)
        self.assertIn(
            'Category="Skyguard|Audio|Production"',
            UFUNCTION_PURE,
        )
        self.assertNotIn("BlueprintCallable", UFUNCTION_PURE)
        self.assertNotIn("VisibleAnywhere", UFUNCTION_PURE)
        self.assertNotIn("BlueprintReadOnly", UFUNCTION_PURE)
        ...
        self.assertTrue("|Production" in specifiers)
        self.assertNotEqual(
            specifiers,
            'BlueprintPure, Category="Skyguard|Audio"',
        )
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "TArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories();",
    ...
    def test_get_unbound_required_categories_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertIn("TArray<ESkyguardProductionAudioCategory> ", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_ARE_RESOLVED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENSURE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_INITIALIZE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CONFIGURE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_UNBOUND)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_POOL_CAPACITY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPAWN_INTERVAL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LIGHT_SPEED_MIN)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLEAR_QUEUE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SUCCESS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FIND_ENTRY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GET_REQUIRED_CATEGORIES)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GET_CATEGORY_DISPLAY_NAME)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BOOL_RETURN)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_LENGTH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_WINDOW_SPEED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_APPROACH_INTERVAL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotIn("FindEntry", LOCKED_DECL)
        self.assertNotIn("GetRequiredCategories", LOCKED_DECL)
        self.assertNotIn("GetCategoryDisplayName", LOCKED_DECL)
        self.assertNotIn("RouteLengthCm", LOCKED_DECL)
        self.assertNotIn("LockWindowSpeedCmPerSecond", LOCKED_DECL)
        self.assertNotIn("ApproachAttackIntervalSeconds", LOCKED_DECL)
        self.assertNotIn("AttackTelegraphLeadSeconds", LOCKED_DECL)
        self.assertNotIn("IsDestroyed", LOCKED_DECL)
        self.assertNotIn(
            'Category="Skyguard|Audio|Development"',
            UFUNCTION_PURE,
        ...
        self.assertNotEqual(UFUNCTION_PURE, UFUNCTION_AUDIO_ONLY)
        self.assertIsNone(
            re.search(r'Category\s*=\s*"Skyguard"(?!\|)', UFUNCTION_PURE)
        )
        self.assertIsNone(
            re.search(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_CLONE_ARE_RESOLVED}\n",
            f"\t{TARGET_WRONG_ENSURE}\n",
            f"\t{TARGET_WRONG_INITIALIZE}\n",
            f"\t{TARGET_WRONG_CONFIGURE}\n",
            f"\t{TARGET_WRONG_UNBOUND}\n",
            f"\t{TARGET_WRONG_BOOL_RETURN}\n",
            f"\t{TARGET_WRONG_ROUTE_LENGTH}\n",
            f"\t{TARGET_WRONG_LOCK_WINDOW_SPEED}\n",
            f"\t{TARGET_WRONG_APPROACH_INTERVAL}\n",
            ...
            leftover_primary,
            leftover_guided,
            "\tTArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategoriess() const;\n",
            "\tint32 GetUnboundRequiredCategories = 1;\n",
            "\tFName GetUnboundRequiredCategories = NAME_None;\n",
            "\tTArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() const = "
            + forty
            + ";\n",
            "\tTArray<ESkyguardProductionAudioCategory> GetUnboundRequiredCategories() const = "
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tTArray<ESkyguardProductionAudioCategory>\n"
            "\tGetUnboundRequiredCategories() const;\n",
            "\tTArray<ESkyguardProductionAudioCategory>   "
            "GetUnboundRequiredCategories() const;\n",
            "\tTArray<ESkyguardProductionAudioCategory>\t"
            "GetUnboundRequiredCategories() const;\n",
    ...
    def test_does_not_claim_sibling_get_resolved_production_loop_route_count_or_neighbors(
        self,
    ) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_ENSURE_DEFAULT))
        self.assertTrue(has_identifier(section, SIBLING_ENSURE_DEFAULT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_ENSURE_DEFAULT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_GET_RESOLVED_COUNT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_CLONE_ARE_RESOLVED))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "GetUnboundRequiredCategories")
        )
        self.assertTrue(
            has_identifier(section, "GetUnboundRequiredCategories")
        )
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        ...
        self.assertNotIn("VisibleAnywhere", UFUNCTION_PURE)
        self.assertNotIn("BlueprintReadOnly", UFUNCTION_PURE)
        self.assertNotIn("MultiLine", UFUNCTION_PURE)
        self.assertNotIn("ClampMin", UFUNCTION_PURE)
        self.assertNotIn("ClampMax", UFUNCTION_PURE)
        for invented in INVENTED_DECL_META:
            self.assertNotIn(invented, LOCKED_DECL)
            self.assertNotIn(invented, UFUNCTION_PURE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio production bank get unbound required categories "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_production_bank_get_unbound_required_categories"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AUDIO_PRODUCTION_BANK_EVALUATE_READINESS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_PRODUCTION_BANK_ENSURE_DEFAULT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_PRODUCTION_BANK_INITIALIZE_REQUIRED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_PRODUCTION_BANK_CONFIGURE_ROUTING, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_THEATER_LAMP_COUNT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertNotIn("FindEntry", locked_only)
        self.assertNotIn("GetRequiredCategories", locked_only)
        self.assertNotIn("GetCategoryDisplayName", locked_only)
        ...
        self.assertNotIn("TogglePause", locked_only)
        self.assertNotIn("ClearQueue", locked_only)
