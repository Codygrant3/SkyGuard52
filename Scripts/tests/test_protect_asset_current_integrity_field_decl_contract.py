# THIS IS leftover-safe ASkyguardProtectAsset::CurrentIntegrity.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Skyguard|Campaign")
# then `float CurrentIntegrity = 100.f;`.
# THIS IS UPROPERTY(VisibleAnywhere, BlueprintReadOnly,
# Category="Skyguard|Campaign"). Category IS
# `Skyguard|Campaign`. VisibleAnywhere IS present.
...
# Do NOT contract sibling MaxIntegrity
# (sibling worker).
# Do NOT contract leftover-safe UFUNCTION siblings
# ApplyDamage / IsDestroyed / GetIntegrityFraction /
# ResetIntegrity (#1288-#1291).
# THIS IS NOT leftover Apache CurrentIntegrity
# closed #411-#426 (different class
...
TARGET = "float CurrentIntegrity = 100.f;"
TARGET_WRONG_BARE = "float CurrentIntegrity;"
TARGET_WRONG_FALSE = "float CurrentIntegrity = false;"
TARGET_WRONG_TRUE = "float CurrentIntegrity = true;"
TARGET_WRONG_INT = "float CurrentIntegrity = 100;"
TARGET_WRONG_ZERO = "float CurrentIntegrity = 0.f;"
TARGET_WRONG_SIBLING = "float MaxIntegrity = 100.f;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # / `= 100` when origin/main has
    # `float CurrentIntegrity = 100.f;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+CurrentIntegrity\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CurrentIntegrity\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CurrentIntegrity\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CurrentIntegrity\s*=\s*100\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CurrentIntegrity\s*=\s*100\.f\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CurrentIntegrity\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CurrentIntegrity is missing from "
...
class ProtectAssetCurrentIntegrityFieldDeclContractTests(unittest.TestCase):
    def test_protect_asset_class_exists(self) -> None:
        ...
        self.assertIn("CurrentIntegrity", section)
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("CurrentIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_current_integrity_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Campaign"', UPROPERTY_VISIBLE_READONLY)
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("CurrentIntegrity = 100.f", compact_origin)
        self.assertNotIn("CurrentIntegrity = false", compact_origin)
        self.assertNotIn("CurrentIntegrity = true", compact_origin)
        self.assertNotIn("CurrentIntegrity = 0.f", compact_origin)

    def test_current_integrity_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CurrentIntegrity"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 100.f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 100.f;\n"
        )
        wrongs = (
            f"\t{TARGET_WRONG_BARE}\n",
            ...
            "\tfloat CurrentIntegrities = 100.f;\n",
            "\tint32 CurrentIntegrity = 100.f;\n",
            "\tbool CurrentIntegrity = 100.f;\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "public:\n\tfloat\n\tCurrentIntegrity = 100.f;\n",
            "public:\n\tfloat   CurrentIntegrity = 100.f;\n",
            "public:\n\tfloat\tCurrentIntegrity = 100.f;\n",
            f"public:\n\t{LOCKED_DECL}\n",
            f"public:\n\t{UPROPERTY_VISIBLE_READONLY}\n\t{LOCKED_DECL}\n",
            f"public:\n\t{UPROPERTY_VISIBLE_READONLY} {LOCKED_DECL}\n",
            "public:\n\tUPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
            'Category="Skyguard|Campaign")\n'
            f"\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "protect-asset CurrentIntegrity field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "current_integrity_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardProtectAsset.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardApacheAircraft.h", THIS_SCRIPT)
