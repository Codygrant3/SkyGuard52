...
# INDEX_NONE or lock PortEngine construction
# in the .cpp. This is a FIELD contract on
# ASkyguardRunwayBreakerBoss, not leftover
# RunwayBreaker sibling fields RunwayRack /
# HangarRack / HeatManifold / DebrisPortWing /
# DebrisPayloadBay / DebrisEngine and not leftover
# emergency-finish named surfaces
# (split tokens only). Keep RunwayBreaker
# PortEngine distinct from leftover Breakwater
# Engine, leftover RoadHunter Engine, leftover
# RadarGhost Engine, leftover Pathfinder Engine,
# leftover LastFlight PortEngine, and sibling
# RunwayBreaker DebrisEngine (UStaticMeshComponent).
# Sibling fields RunwayRack, HangarRack,
# HeatManifold, DebrisPortWing, DebrisPayloadBay,
# DebrisEngine must not satisfy the primary lock.
...
# leftover isolated-test drafts #107–#555,
# leftover searchlight-track-runtime-defaults
...
# unlocked for THIS test's primary PortEngine lock. Leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRunwayBreakerBoss::PortEngine",
    "SkyguardRunwayBreakerBoss.cpp",
    "SetExposed",
...
class RunwayBreakerPortEngineFieldDeclContractTests(unittest.TestCase):
    def test_runway_breaker_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, PORT_ENGINE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, PORT_ENGINE_FIELD)
    ...
    def test_missing_port_engine_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRunwayBreakerBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission06|Boss"', section)
        self.assertTrue(
            has_declaration(section, PORT_ENGINE_FIELD),
            section,
        )
        self.assertNotIn("UPROPERTY", PORT_ENGINE_FIELD)
        self.assertNotIn("VisibleAnywhere", PORT_ENGINE_FIELD)
        self.assertNotIn("BlueprintReadOnly", PORT_ENGINE_FIELD)
        self.assertNotIn("Category", PORT_ENGINE_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_PORT)
        self.assertNotIn("BlueprintCallable", UPROPERTY_PORT)
        self.assertIn("Skyguard|Mission06|Boss", UPROPERTY_PORT)
        self.assertIn("Mission06", UPROPERTY_PORT)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, PORT_ENGINE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* PortEngine;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> PortEngine;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> PortEngine;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, PORT_ENGINE_FIELD)
            self.assertIn("PortEngine", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_port_engine_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, PORT_ENGINE_FIELD),
            PORT_ENGINE_FIELD,
        )
        self.assertTrue(has_declaration(section, PORT_ENGINE_FIELD))
        self.assertEqual(declaration_count(section, PORT_ENGINE_FIELD), 1)
        self.assertTrue(
            PORT_ENGINE_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            PORT_ENGINE_FIELD,
        )
        self.assertTrue(PORT_ENGINE_FIELD.endswith(";"), PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", PORT_ENGINE_FIELD)
        self.assertNotIn("HangarRack", PORT_ENGINE_FIELD)
        self.assertNotIn("HeatManifold", PORT_ENGINE_FIELD)
        self.assertNotIn("RunwayRack", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisPortWing", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisPayloadBay", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisEngine", PORT_ENGINE_FIELD)
        self.assertNotIn("TargetingCamera", PORT_ENGINE_FIELD)
        self.assertNotIn("LeftActuator", PORT_ENGINE_FIELD)
        self.assertNotIn("RightActuator", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisLeftWing", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisRightWing", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisCoolingDoor", PORT_ENGINE_FIELD)
        self.assertNotIn("SignatureModulator", PORT_ENGINE_FIELD)
        self.assertNotIn("RadarReceiver", PORT_ENGINE_FIELD)
        self.assertNotIn("CoolingDoor", PORT_ENGINE_FIELD)
        self.assertNotIn("PortLatch", PORT_ENGINE_FIELD)
        self.assertNotIn("NoseCamera", PORT_ENGINE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_ENGINE_FIELD,
        )
        self.assertIn("TObjectPtr<", PORT_ENGINE_FIELD)
        self.assertNotIn("TArray<", PORT_ENGINE_FIELD)
        self.assertNotIn("=", PORT_ENGINE_FIELD)
        self.assertNotIn("INDEX_NONE", PORT_ENGINE_FIELD)
        self.assertNotIn("UFUNCTION", PORT_ENGINE_FIELD)
        self.assertNotIn("{", PORT_ENGINE_FIELD)
        self.assertNotIn("}", PORT_ENGINE_FIELD)
        self.assertNotIn("return ", PORT_ENGINE_FIELD)
        self.assertNotIn("CommandAntenna", PORT_ENGINE_FIELD)
        self.assertNotIn("ControlLinkage", PORT_ENGINE_FIELD)
        self.assertNotIn("NoseCamera", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisCenter", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisTail", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisNose", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisSpine", PORT_ENGINE_FIELD)
        self.assertNotIn("AdvanceEncounter", PORT_ENGINE_FIELD)
        self.assertNotIn("ResetEncounterState", PORT_ENGINE_FIELD)
        self.assertNotIn("IsRouteStateSafe", PORT_ENGINE_FIELD)
        self.assertNotIn("GetRouteProgress", PORT_ENGINE_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", PORT_ENGINE_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", PORT_ENGINE_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", PORT_ENGINE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PORT_ENGINE_FIELD)
        self.assertNotIn("RadarNode", PORT_ENGINE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", PORT_ENGINE_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", PORT_ENGINE_FIELD)
        self.assertNotIn("IssueClimbCommand", PORT_ENGINE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", PORT_ENGINE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", PORT_ENGINE_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", PORT_ENGINE_FIELD)
        self.assertNotIn("GetFinaleStage", PORT_ENGINE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", PORT_ENGINE_FIELD)
        self.assertNotIn("IsWreckDiverted", PORT_ENGINE_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", PORT_ENGINE_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", PORT_ENGINE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PORT_ENGINE_FIELD)
        self.assertNotIn("GetGunnerMount", PORT_ENGINE_FIELD)
        self.assertNotIn("FaceWorldLocation", PORT_ENGINE_FIELD)
        self.assertNotIn("PortGuidanceArray", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisArmorPort", PORT_ENGINE_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_ENGINE_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_ENGINE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", PORT_ENGINE_FIELD)
        self.assertNotIn("SetLightningExposed", PORT_ENGINE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", PORT_ENGINE_FIELD)
        self.assertNotIn("HullCollider", PORT_ENGINE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, PORT_ENGINE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {PORT_ENGINE_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, PORT_ENGINE_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{PORT_ENGINE_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, PORT_ENGINE_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, PORT_ENGINE_FIELD),
            section,
    ...
    def test_assigned_port_engine_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "PortEngine = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, PORT_ENGINE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("PortEngine", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, PORT_ENGINE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{PORT_ENGINE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, PORT_ENGINE_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, PORT_ENGINE_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            PORT_ENGINE_FIELD.startswith("UFUNCTION"),
            PORT_ENGINE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, PORT_ENGINE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", PORT_ENGINE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_port_engine_cpp_body(self) -> None:
        locked_only = f"{PORT_ENGINE_FIELD}\n"
        self.assertNotIn("{", PORT_ENGINE_FIELD)
        self.assertNotIn("}", PORT_ENGINE_FIELD)
        self.assertNotIn("return ", PORT_ENGINE_FIELD)
        ...
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", PORT_ENGINE_FIELD)
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", locked_only)
        self.assertNotIn("return false", PORT_ENGINE_FIELD)
        self.assertNotIn("return true", PORT_ENGINE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", PORT_ENGINE_FIELD)
        self.assertNotIn("SetExposed", PORT_ENGINE_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            PORT_ENGINE_FIELD,
        )
        self.assertNotIn("SetBossPhase", PORT_ENGINE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", PORT_ENGINE_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{PORT_ENGINE_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("PortDischargeBoom", PORT_ENGINE_FIELD)
        self.assertNotIn("StarboardDischargeBoom", PORT_ENGINE_FIELD)
        self.assertNotIn("EngineIntake", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisPortPanel", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisStarboardPanel", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisIntakePanel", PORT_ENGINE_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", PORT_ENGINE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            PORT_ENGINE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("EncounterController", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("ControlServo", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("DispenserCenter", PORT_ENGINE_FIELD)
        self.assertNotIn("MaxReleasesPerBay", PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("SetContactIdentified", PORT_ENGINE_FIELD)
        self.assertNotIn("OpenOrbitExposure", PORT_ENGINE_FIELD)
        self.assertNotIn("IsContactIdentified", PORT_ENGINE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), PORT_ENGINE_FIELD)

    def test_debris_cooling_door_does_not_satisfy_port_engine(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, PORT_ENGINE_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("DispenserStarboard", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("GetManeuver", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), PORT_ENGINE_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", PORT_ENGINE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("SetSearchlightTracked", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        self.assertNotIn("CoolingDoor", PORT_ENGINE_FIELD)
        self.assertNotIn("SignatureModulator", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("SetLightningExposed", PORT_ENGINE_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", PORT_ENGINE_FIELD)
        self.assertNotIn("IsLightningExposed", PORT_ENGINE_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), PORT_ENGINE_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, PORT_ENGINE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardGunner", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("SkyguardRadarNode", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_ENGINE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", PORT_ENGINE_FIELD)
        self.assertNotIn("MaxIntegrity", PORT_ENGINE_FIELD)
        self.assertNotIn("CurrentIntegrity", PORT_ENGINE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", PORT_ENGINE_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            PORT_ENGINE_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{PORT_ENGINE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, PORT_ENGINE_FIELD),
            PORT_ENGINE_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRunwayBreakerBoss::PortEngine", section)
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{PORT_ENGINE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, PORT_ENGINE_FIELD.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, PORT_ENGINE_FIELD)

    def test_contract_is_port_engine_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(PORT_ENGINE_FIELD.endswith(";"))
        self.assertNotIn("=", PORT_ENGINE_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        self.assertNotIn("DebrisEngine", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisRightWing", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardRoadHunterBoss", PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", PORT_ENGINE_FIELD)
        self.assertNotEqual(PORT_ENGINE_FIELD, ROAD_HUNTER_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("PortLatch", PORT_ENGINE_FIELD)
        self.assertNotIn("StarboardLatch", PORT_ENGINE_FIELD)
        self.assertNotIn("DecoyPods", PORT_ENGINE_FIELD)
        self.assertNotIn("ElevatorLinkage", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", PORT_ENGINE_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_runway_breaker_port_engine_is_not_breakwater_engine(self) -> None:
        locked_only = f"{PORT_ENGINE_FIELD}\n"
        self.assertNotEqual(PORT_ENGINE_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(PORT_ENGINE_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(collapsed(PORT_ENGINE_FIELD), collapsed(ENGINE_FIELD))
        self.assertNotEqual(PORT_ENGINE_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardLastFlightBoss", PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", PORT_ENGINE_FIELD)
        self.assertNotIn("DebrisEngine", PORT_ENGINE_FIELD)
        self.assertNotIn("TargetingCamera", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", PORT_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, PORT_ENGINE_FIELD)
        self.assertNotIn("OpticalTracker", PORT_ENGINE_FIELD)
        self.assertNotIn("WeaponServo", PORT_ENGINE_FIELD)
        self.assertNotIn("CountermeasurePod", PORT_ENGINE_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", PORT_ENGINE_FIELD)
    ...
    def test_hangar_rack_does_not_satisfy_port_engine(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, PORT_ENGINE_FIELD))
        self.assertNotEqual(HANGAR_RACK_FIELD, PORT_ENGINE_FIELD)
        self.assertNotIn("HangarRack", PORT_ENGINE_FIELD)

    def test_heat_manifold_and_runway_rack_do_not_satisfy_port_engine(
        self,
    ) -> None:
        for region, name in (
            (f"\t{HEAT_MANIFOLD_FIELD}\n", "HeatManifold"),
    ...
    def test_runway_breaker_debris_fields_do_not_satisfy_port_engine(
        self,
    ) -> None:
        for region, name in (
            (f"\t{RUNWAY_BREAKER_DEBRIS_PORT_WING_FIELD}\n", "DebrisPortWing"),
    ...
    def test_debris_engine_does_not_satisfy_port_engine(self) -> None:
    ...
    def test_targeting_camera_does_not_satisfy_port_engine(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, PORT_ENGINE_FIELD))
        self.assertNotEqual(TARGETING_CAMERA_FIELD, PORT_ENGINE_FIELD)
        self.assertNotIn("TargetingCamera", PORT_ENGINE_FIELD)
        self.assertIn("PortEngine", PORT_ENGINE_FIELD)

    def test_left_and_right_actuators_do_not_satisfy_port_engine(self) -> None:
        for region, name in (
            (f"\t{LEFT_ACTUATOR_FIELD}\n", "LeftActuator"),
            (f"\t{RIGHT_ACTUATOR_FIELD}\n", "RightActuator"),
        ):
