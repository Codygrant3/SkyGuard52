# THIS IS leftover-safe FSkyguardCampaignMissionSpec Climax.
# origin/main form: plain C++ field
# `ESkyguardClimaxKind Climax = ESkyguardClimaxKind::PatrolShip;`
# with in-struct initializer ::PatrolShip. NOT a UPROPERTY wrap.
...
# FastAttacker / GroundArmor / HeavyAttacker / RotorScout
# instead of PatrolShip. Fail-closed if type is
# ESkyguardThreatKind instead of ESkyguardClimaxKind.
...
# Harbor Breaker Contact. Harbor fail-closed is ONLY 40/80.
...
# ShoreKind / SupportKind / ExtractKind / bNightIdentity /
...
# ShoreKind / SupportKind / ExtractKind siblings
# (those are ESkyguardThreatKind).
# THIS IS NOT leftover analog climax-kind-enum-contract
# #b4eb.
...
# ::PatrolShip. Type is ESkyguardClimaxKind, identifier
# is Climax. Parse window comes from leftover-safe
# CampaignMissionSpec MissionId #1521 (stop before
# namespace SkyguardCampaignRoster).
# THIS SLOT IS NOT leftover ASkyguardPatrolShipBoss
# class body.
...
TARGET_WRONG_BARE = "ESkyguardClimaxKind Climax;"
TARGET_WRONG_GROUND = (
    "ESkyguardThreatKind Climax = "
...
TARGET_WRONG_EQ_NONE = "ESkyguardClimaxKind Climax = NAME_None;"
TARGET_WRONG_FALSE = "ESkyguardClimaxKind Climax = false;"
TARGET_WRONG_TRUE = "ESkyguardClimaxKind Climax = true;"
TARGET_WRONG_ZERO = "ESkyguardClimaxKind Climax = 0.f;"
TARGET_WRONG_FLOAT = "float Climax;"
TARGET_WRONG_HEALTH = "float Climax = 160.f;"
TARGET_WRONG_BOOL = "bool Climax;"
TARGET_WRONG_INT = "int32 Climax;"
TARGET_WRONG_FNAME = "FName Climax;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_campaign_mission_spec_climax"
...
LEFTOVER_ANALOG_CLIMAX_KIND_ENUM = (
...
CLIMAX_DECL_RE = re.compile(
    r"ESkyguardClimaxKind\s+Climax\s*=\s*"
    r"ESkyguardClimaxKind\s*::\s*PatrolShip\s*;"
)
CLIMAX_INIT_RE = re.compile(
    r"ESkyguardClimaxKind\s+Climax\s*=\s*([^;]+);"
)
THREAT_KIND_CLIMAX_RE = re.compile(
    r"ESkyguardThreatKind\s+Climax\b"
)
WRONG_INITIALIZERS = (
    "ESkyguardThreatKind::FastAttacker",
    "ESkyguardThreatKind::GroundArmor",
    "ESkyguardThreatKind::HeavyAttacker",
    "ESkyguardThreatKind::RotorScout",
    "ESkyguardClimaxKind::FastAttacker",
    "ESkyguardClimaxKind::GroundArmor",
    "ESkyguardClimaxKind::HeavyAttacker",
    "ESkyguardClimaxKind::RotorScout",
)
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
...
def climax_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardClimaxKind\s+Climax\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # FastAttacker, leftover ShoreKind GroundArmor,
    # leftover SupportKind HeavyAttacker, leftover
    # ExtractKind RotorScout, leftover StormRainBeatKit
    # Kinds, leftover Harbor Breaker Contact, leftover
    # ASkyguardPatrolShipBoss class body, leftover analog
    # climax-kind-enum-contract, `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` when origin/main locks
    # ::PatrolShip. Fail-closed if type is
    # ESkyguardThreatKind. Do not accept leftover TheaterKit
    # Category clones as this CampaignMissionSpec parse
    # window.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CLIMAX_DECL_RE.search(compact) is None:
        return False
    if THREAT_KIND_CLIMAX_RE.search(compact):
        return False
    if re.search(r"float\s+Climax\b", compact):
        return False
    if re.search(r"FName\s+Climax\b", compact):
        return False
    if re.search(r"bool\s+Climax\b", compact):
        return False
    if re.search(r"int32\s+Climax\b", compact):
        return False
    if climax_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    for match in CLIMAX_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != "ESkyguardClimaxKind::PatrolShip":
            return False
    for leftover_init in WRONG_INITIALIZERS:
        if leftover_init in compact and leftover_init.split("::")[1] != (
            ...
            if re.search(
                r"Climax\s*=\s*" + re.escape(leftover_init),
                compact,
            ):
                return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardClimaxKind\s+Climax\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardClimaxKind Climax is missing "
...
class CampaignMissionSpecClimaxFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_enum_not_uproperty(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardClimaxKind Climax"),
        )
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("::PatrolShip", LOCKED_DECL)
        ...
        self.assertNotIn("::RotorScout", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_EXTRACT)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("Climax", str(raised.exception))
    ...
    def test_leftover_storm_rain_kinds_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn("Climax", LEFTOVER_STORM_KINDS_DECL)
        leftover = (
    ...
    def test_missing_climax_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_kind_fields_do_not_satisfy_climax(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SHORE}\n"
    ...
    def test_initializer_must_be_patrol_ship(self) -> None:
        missing = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
        ...
        self.assertNotIn(
            "Climax = ESkyguardThreatKind::GroundArmor",
            compact_origin,
        )
        self.assertNotIn(
            "Climax = ESkyguardThreatKind::HeavyAttacker",
            compact_origin,
        )
        self.assertNotIn(
            "Climax = ESkyguardThreatKind::RotorScout",
            compact_origin,
        )
        self.assertNotIn(
            "Climax = ESkyguardThreatKind::FastAttacker",
            compact_origin,
        )
        self.assertNotIn("Climax = NAME_None", compact_origin)
        self.assertNotIn("Climax = false", compact_origin)
        self.assertNotIn("Climax = true", compact_origin)
        self.assertNotIn("Climax = 0.f", compact_origin)
        self.assertNotIn("Climax = 160.f", compact_origin)
        self.assertNotIn(
            "ESkyguardThreatKind Climax =",
            compact_origin,
        )

    def test_climax_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("ESkyguardClimaxKind ", LOCKED_DECL)
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    climax_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(climax_has_uproperty_wrap(origin), origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tESkyguardClimaxKind\n"
            "\tClimax = ESkyguardClimaxKind::PatrolShip;\n",
            "\tESkyguardClimaxKind   Climax = "
            "ESkyguardClimaxKind::PatrolShip;\n",
            "\tESkyguardClimaxKind\tClimax = "
            "ESkyguardClimaxKind::PatrolShip;\n",
    ...
    def test_leftover_patrol_ship_boss_class_does_not_satisfy(self) -> None:
        leftover = (
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_PATROL)
        self.assertNotIn(STOP_BEFORE_PATROL, LOCKED_DECL)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_ANALOG_CLIMAX_KIND_ENUM)
        self.assertIn(LEFTOVER_ANALOG_CLIMAX_KIND_ENUM, LOCKED_SCRIPTS)
        ...
        section = spec_section(threat_type)
        self.assertFalse(has_declaration(section, LOCKED_DECL), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Climax", str(raised.exception))

    def test_harbor_breaker_contact_is_not_this_spec(self) -> None:
        ...
        self.assertNotEqual(FIELD_NAME, "HarborContact")
        self.assertNotEqual(FIELD_NAME, "ContactKind")
        self.assertNotEqual(FIELD_NAME, "ShoreKind")
        self.assertNotEqual(FIELD_NAME, "SupportKind")
        self.assertNotEqual(FIELD_NAME, "ExtractKind")
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_PATROL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CONTACT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec Climax field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "climax_field_decl_contract.py"
        ))
        self.assertIn("campaign_mission_spec", Path(__file__).name)
        self.assertIn("climax", Path(__file__).name)
        self.assertNotIn("contact_kind", Path(__file__).name)
        self.assertNotIn("climax_kind_enum", Path(__file__).name)
        self.assertNotIn("campaign_roster_lookup", Path(__file__).name)
        ...
        self.assertIn(LEFTOVER_SPEC_EXTRACT_KIND, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertIn("::PatrolShip", locked_only)
        ...
        self.assertNotIn("::RotorScout", locked_only)
        self.assertNotIn("ESkyguardThreatKind", locked_only)
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_SPEC_EXTRACT_KIND)
