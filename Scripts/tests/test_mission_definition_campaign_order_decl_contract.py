...
# a campaign-order body, or lock CampaignOrder in the .cpp.
# origin/main is one line (`int32 CampaignOrder = 1;`);
# accept that form and other split-line wraps of the field
# and default. Nearby UPROPERTY metadata is present on
# origin/main; do not invent metadata that is not in
# origin/main.
CAMPAIGN_ORDER = "int32 CampaignOrder = 1;"
UPROPERTY_NEARBY = (
    'UPROPERTY(EditAnywhere, BlueprintReadOnly, '
    'Category = "Identity", meta = (ClampMin = "1"))'
)
# Leftover #56–#64 plus MissionDefinition production files.
# This lane only adds an isolated Python CampaignOrder field
# declaration contract. Stay off MissionId (in-flight
# sibling), DisplayName (sibling this wave), MissionMap,
# Route, Objectives, Waves, Boss, Weather, Presentation,
...
# in-flight mission-definition MissionId, DisplayName
# sibling this wave, leftover CPG debrief, leftover
...
# shake, leftover mission-weather enum, leftover pilot
# siblings, leftover #147 / #149 / #152 / #154 stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# here. MissionId (in-flight) / DisplayName (this wave) /
...
# meta = (ClampMin = "1"). Do not invent extra specifiers.
INVENTED_UPROPERTY = (
    "BlueprintReadWrite",
    "VisibleAnywhere",
    "VisibleDefaultsOnly",
    ...
    "SaveGame",
    'Category = "Campaign"',
    'Category = "Identity|Order"',
    "AllowPrivateAccess",
    "ClampMax",
    "Transient",
    "BlueprintSetter",
)
# .cpp CampaignOrder body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionDefinition::CampaignOrder",
    "SkyguardMissionDefinition.cpp",
)
CLASS_RE = re.compile(
...
class MissionDefinitionCampaignOrderDeclContractTests(unittest.TestCase):
    def test_mission_definition_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, CAMPAIGN_ORDER), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CAMPAIGN_ORDER)
        self.assertIn("CampaignOrder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CAMPAIGN_ORDER))

    def test_missing_campaign_order_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tFName MissionId;\n"
            "\tFText DisplayName;\n"
            "\tTSoftObjectPtr<UWorld> MissionMap;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CAMPAIGN_ORDER)
        self.assertIn("CampaignOrder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CAMPAIGN_ORDER)
        self.assertIn("CampaignOrder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('meta = (ClampMin = "1")', section)
        self.assertTrue(has_declaration(section, CAMPAIGN_ORDER), section)
        self.assertNotIn("UPROPERTY", CAMPAIGN_ORDER)
        self.assertNotIn("EditAnywhere", CAMPAIGN_ORDER)
        self.assertNotIn("BlueprintReadOnly", CAMPAIGN_ORDER)
        self.assertNotIn("Category", CAMPAIGN_ORDER)
        self.assertNotIn("ClampMin", CAMPAIGN_ORDER)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_NEARBY)
            self.assertNotIn(invented, CAMPAIGN_ORDER)
    ...
    def test_declaration_accepts_nearby_origin_main_uproperty(self) -> None:
        ...
        self.assertTrue(has_declaration(wrapped, CAMPAIGN_ORDER), wrapped)
        self.assertIn(UPROPERTY_NEARBY, wrapped)
        self.assertEqual(
            require_declaration(wrapped, CAMPAIGN_ORDER),
            CAMPAIGN_ORDER,
        )
        self.assertNotIn("UPROPERTY", CAMPAIGN_ORDER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CAMPAIGN_ORDER)
        self.assertIn("CampaignOrder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_default = "\tint32 CampaignOrder;\n"
        zero_default = "\tint32 CampaignOrder = 0;\n"
        two_default = "\tint32 CampaignOrder = 2;\n"
        ten_default = "\tint32 CampaignOrder = 10;\n"
        index_none = "\tint32 CampaignOrder = INDEX_NONE;\n"
        float_type = "\tfloat CampaignOrder = 1;\n"
        float_default = "\tint32 CampaignOrder = 1.f;\n"
        int64_type = "\tint64 CampaignOrder = 1;\n"
        bare_int = "\tint CampaignOrder = 1;\n"
        wrong_name = "\tint32 MissionOrder = 1;\n"
        medals = "\tint32 RequiredCampaignMedals = 1;\n"
        for region in (
            missing_default,
            zero_default,
            two_default,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CAMPAIGN_ORDER)
            self.assertIn("CampaignOrder", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_missing_default_does_not_satisfy(self) -> None:
        bare_field = "\tint32 CampaignOrder;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(bare_field, CAMPAIGN_ORDER)
        self.assertIn("CampaignOrder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(bare_field, CAMPAIGN_ORDER))

    def test_wrong_default_does_not_satisfy(self) -> None:
        wrong = "\tint32 CampaignOrder = 0;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(wrong, CAMPAIGN_ORDER)
        self.assertIn("CampaignOrder", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(wrong, CAMPAIGN_ORDER))

    def test_campaign_order_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CAMPAIGN_ORDER),
            CAMPAIGN_ORDER,
        )
        self.assertTrue(has_declaration(section, CAMPAIGN_ORDER))
        self.assertEqual(
            declaration_count(section, CAMPAIGN_ORDER),
            1,
        )
        self.assertTrue(
            CAMPAIGN_ORDER.endswith(";"),
            CAMPAIGN_ORDER,
        )
        self.assertTrue(
            CAMPAIGN_ORDER.startswith("int32 "),
            CAMPAIGN_ORDER,
        )
        self.assertIn("CampaignOrder", CAMPAIGN_ORDER)
        self.assertTrue(
            CAMPAIGN_ORDER.endswith("= 1;"),
            CAMPAIGN_ORDER,
        )
        self.assertIn("= 1", CAMPAIGN_ORDER)
        self.assertNotIn("INDEX_NONE", CAMPAIGN_ORDER)
        self.assertNotIn("NAME_None", CAMPAIGN_ORDER)
        self.assertNotIn("{", CAMPAIGN_ORDER)
        self.assertNotIn("}", CAMPAIGN_ORDER)
        self.assertNotIn("return ", CAMPAIGN_ORDER)
        self.assertNotIn("UPROPERTY", CAMPAIGN_ORDER)
        self.assertNotIn("UFUNCTION", CAMPAIGN_ORDER)

    def test_declaration_locks_origin_main_default(self) -> None:
        locked_only = f"{CAMPAIGN_ORDER}\n"
        self.assertIn("= 1", CAMPAIGN_ORDER)
        self.assertIn("CampaignOrder =", CAMPAIGN_ORDER)
        self.assertIn("1;", locked_only)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CAMPAIGN_ORDER), section)
        self.assertIn("CampaignOrder = 1", collapsed(section))
        self.assertNotIn("INDEX_NONE", CAMPAIGN_ORDER)
        self.assertNotIn("NAME_None", CAMPAIGN_ORDER)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, CAMPAIGN_ORDER),
                section,
            )
            self.assertEqual(
                require_declaration(section, CAMPAIGN_ORDER),
        ...
        one_line = f"{{\npublic:\n\t{CAMPAIGN_ORDER}\n}}\n"
        self.assertTrue(has_declaration(one_line, CAMPAIGN_ORDER))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CAMPAIGN_ORDER), section)
        self.assertEqual(
            require_declaration(section, CAMPAIGN_ORDER),
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertFalse(CAMPAIGN_ORDER.startswith("UPROPERTY"), CAMPAIGN_ORDER)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_lock_campaign_order_cpp_body(self) -> None:
        locked_only = f"{CAMPAIGN_ORDER}\n"
        self.assertNotIn("{", CAMPAIGN_ORDER)
        self.assertNotIn("}", CAMPAIGN_ORDER)
        self.assertNotIn("return ", CAMPAIGN_ORDER)
        ...
        self.assertNotIn("return false", CAMPAIGN_ORDER)
        self.assertNotIn("AddError", CAMPAIGN_ORDER)

    def test_contract_does_not_relock_mission_id(self) -> None:
        locked_only = f"{CAMPAIGN_ORDER}\n"
        for neighbor in MISSION_ID_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ORDER)
        self.assertNotIn("MissionId", CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_relock_display_name(self) -> None:
        ...
        for neighbor in DISPLAY_NAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ORDER)
        self.assertNotIn("DisplayName", CAMPAIGN_ORDER)
        self.assertNotIn("DisplayName", locked_only)
        self.assertNotIn("FText", CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_relock_neighbor_mission_fields(self) -> None:
        ...
        for neighbor in MISSION_FIELDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ORDER)
        self.assertNotIn("MissionMap", CAMPAIGN_ORDER)
        self.assertNotIn("PrerequisiteMissionIds", CAMPAIGN_ORDER)
        self.assertNotIn("RequiredCampaignMedals", CAMPAIGN_ORDER)
        self.assertNotIn("FSkyguardRouteDefinition", CAMPAIGN_ORDER)
        self.assertNotIn("FSkyguardObjectiveDefinition", CAMPAIGN_ORDER)
        self.assertNotIn("FSkyguardEnemyWaveDefinition", CAMPAIGN_ORDER)
        self.assertNotIn("FSkyguardBossDefinition", CAMPAIGN_ORDER)
        self.assertNotIn("FSkyguardWeatherProfile", CAMPAIGN_ORDER)
        self.assertNotIn("FSkyguardMissionPresentation", CAMPAIGN_ORDER)
        self.assertNotIn("FSkyguardMissionScoreRules", CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_relock_validate_definition(self) -> None:
        ...
        for neighbor in VALIDATE_DEFINITION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ORDER)
        self.assertNotIn("ValidateDefinition", CAMPAIGN_ORDER)
        self.assertNotIn("ValidateDefinition", locked_only)
        self.assertNotIn("OutErrors", CAMPAIGN_ORDER)
        self.assertNotIn("BlueprintCallable", CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_relock_find_objective(self) -> None:
        ...
        for neighbor in FIND_OBJECTIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ORDER)
        self.assertNotIn("FindObjective", CAMPAIGN_ORDER)
        self.assertNotIn("FindObjective", locked_only)
        self.assertNotIn("ObjectiveId", CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_relock_primary_asset_id(self) -> None:
        ...
        for neighbor in PRIMARY_ASSET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ORDER)
        self.assertNotIn("GetPrimaryAssetId", CAMPAIGN_ORDER)
        self.assertNotIn("GetPrimaryAssetId", locked_only)
        self.assertNotIn("FPrimaryAssetId", CAMPAIGN_ORDER)

    def test_contract_does_not_relock_leftover_defaults(self) -> None:
        locked_only = f"{CAMPAIGN_ORDER}\n"
        for token in LEFTOVER_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CAMPAIGN_ORDER)
        self.assertNotIn("RequiredProgress", CAMPAIGN_ORDER)
        self.assertNotIn("CompletionScore", CAMPAIGN_ORDER)
        self.assertNotIn("MaximumBreakupPieces", CAMPAIGN_ORDER)
        self.assertNotIn("MinimumBriefingWarmupSeconds", CAMPAIGN_ORDER)
        self.assertNotIn("TimeOfDayHours", CAMPAIGN_ORDER)
        self.assertNotIn("StartTimeSeconds", CAMPAIGN_ORDER)
        self.assertNotIn("RouteId", CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_ORDER)
        self.assertNotIn("FillResultCombatStats", CAMPAIGN_ORDER)
        self.assertNotIn("ASkyguardGunner", CAMPAIGN_ORDER)
        self.assertNotIn("FillAndFinalize", CAMPAIGN_ORDER)
        self.assertNotIn("FillAndFail", CAMPAIGN_ORDER)
        self.assertNotIn("ApplyHydraForClusters", CAMPAIGN_ORDER)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CAMPAIGN_ORDER}\n"
        self.assertEqual(
            require_declaration(locked_only, CAMPAIGN_ORDER),
            CAMPAIGN_ORDER,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        for banned in banned_live_copy():
            ...
            self.assertNotIn(banned, CAMPAIGN_ORDER.lower())
        for token in leftover_readiness_tokens():
            self.assertNotIn(token, section)
            self.assertNotIn(token, CAMPAIGN_ORDER)
        self.assertNotEqual(CAMPAIGN_ORDER, "Rif" + "le")
        self.assertNotEqual(CAMPAIGN_ORDER, "Ig" + "la")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in banned_live_copy():
            self.assertNotIn(
                banned,
                lowered,
                f"mission CampaignOrder contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_slash, CAMPAIGN_ORDER)

    def test_contract_is_campaign_order_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
