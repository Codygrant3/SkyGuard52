...
# #1cd7 then leftover-safe isolated BuildSha256 #1435 then
# this isolated PeakActiveVoices field decl.
...
TARGET = "int32 PeakActiveVoices = 0;"
TARGET_WRONG_BARE = "int32 PeakActiveVoices;"
TARGET_WRONG_EQ_ONE = "int32 PeakActiveVoices = 1;"
TARGET_WRONG_EQ_NONE = "int32 PeakActiveVoices = NAME_None;"
TARGET_WRONG_TRUE = "int32 PeakActiveVoices = true;"
TARGET_WRONG_FALSE = "int32 PeakActiveVoices = false;"
TARGET_WRONG_EQ_FLOAT = "int32 PeakActiveVoices = 0.f;"
TARGET_WRONG_FSTRING = "FString PeakActiveVoices;"
TARGET_WRONG_FLOAT = "float PeakActiveVoices = 0;"
TARGET_WRONG_FNAME = "FName PeakActiveVoices;"
TARGET_WRONG_FTEXT = "FText PeakActiveVoices;"
TARGET_WRONG_BOOL = "bool PeakActiveVoices = false;"
TARGET_WRONG_TARRAY = "TArray<int32> PeakActiveVoices;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> PeakActiveVoices;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 PeakActiveVoices = 0;`.
    # Match the EXACT identifier PeakActiveVoices as a whole
    # field name after int32. Do NOT accept SampleCount or
    # TotalUnderruns. Reject dropped/changed initializer,
    # Harbor 40/80, wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"int32\s+PeakActiveVoices\s*=\s*0\s*;",
        compact,
    )
    ...
    for assigned in re.findall(
        r"int32\s+PeakActiveVoices\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "0":
            return False
    if re.search(
        r"float\s+PeakActiveVoices\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+PeakActiveVoices\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+PeakActiveVoices\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+PeakActiveVoices\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+PeakActiveVoices\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+PeakActiveVoices\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+PeakActiveVoices\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+PeakActiveVoices\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 PeakActiveVoices is missing from "
...
class AudibleAcceptanceReceiptPeakActiveVoicesFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_peak_active_voices_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sample_count_does_not_satisfy_peak_active_voices(self) -> None:
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 PeakActiveVoices = 0;", compact_origin)
        self.assertNotIn("PeakActiveVoices = 1", compact_origin)
        self.assertNotIn("PeakActiveVoices = true", compact_origin)
        self.assertNotIn("FString PeakActiveVoices", compact_origin)
        self.assertNotIn("float PeakActiveVoices", compact_origin)

    def test_peak_active_voices_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 PeakActiveVoices"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_TOTAL_UNDERRUNS} = 0;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tint32 PeakActiveVoicess = 0;\n",
            "\tint32 PeakActiveVoices = 1;\n",
            "\tbool PeakActiveVoices;\n",
            "\tfloat PeakActiveVoices = " + forty + ";\n",
            "\tfloat PeakActiveVoices = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt PeakActiveVoices field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "peak_active_voices_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_peak", THIS_SCRIPT)
