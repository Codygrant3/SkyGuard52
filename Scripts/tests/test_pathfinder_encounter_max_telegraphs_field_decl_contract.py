...
# then `int32 MaxTelegraphsPerEncounter = 16;` (HAS initializer).
# Type is **int32**, not float. ClampMin IS `"0"` (integer string, not `"0.2"`).
# ClampMax IS `"32"`. Default IS `= 16`.
...
# It is NOT leftover analog pathfinder-encounter-controller-field.
# It is NOT leftover analog MaxLateralOffsetCm #1667 (leave sibling
# uncontracted).
# It is NOT leftover analog MaxHeightFromOriginCm /
# MaxSimulationStepSeconds / leftover analog GlobalVoiceLimit int32 /
# leftover analog Coastal EpicTreeBudget int32 / leftover Route
# #1660-#1668.
...
# if ClampMin is missing, if ClampMax is missing, if the default is not
# `= 16`, if LOCKED_DECL is still `float` or ClampMin is still `"0.2"` /
# leftover unique Safety ClampMin `"1"` ClampMax `"16"` /
# leftover analog `"100.0"`,
# if Category is Skyguard|Apache / Skyguard|Combat / Skyguard|Theater /
...
# Fail-closed if wrap Category is leftover Route
# `Skyguard|Boss|Encounter|Route` / Commands / Safety
# / leftover analog Category `Skyguard|Boss|Encounter` UFUNCTION.
...
# same-name 45000.f).
# Do NOT claim leftover Route unique LockWindowSpeedCmPerSecond /
...
# Do NOT claim Safety siblings MaxLateralOffsetCm /
# MaxHeightFromOriginCm / MaxSimulationStepSeconds.
# Do NOT claim leftover analog MinHeightFromOriginCm = -
# plus Harbor 80 float token.
# Do NOT claim Commands CommandLateralBiasCm / BreakClimbCm.
# Do NOT claim leftover unique Telegraph interval fields
...
# Do NOT claim leftover unique Safety MaxSimulationSubsteps = 8.
...
# Do NOT claim leftover analog Pathfinder EncounterController
# TObjectPtr field.
...
# Clone wrap was leftover unique Safety MaxSimulationSubsteps
# Category="Skyguard|Boss|Encounter|Safety", meta=(ClampMin="1", ClampMax="16")
# wrapping `int32 MaxSimulationSubsteps = 8;`
# — RETARGET hard: Category Safety to Telegraph,
# ClampMin from `"1"` to `"0"`, ClampMax from `"16"` to `"32"`,
# LOCKED_DECL from `int32 MaxSimulationSubsteps = 8;`
# to `int32 MaxTelegraphsPerEncounter = 16;`.
# Alternate float Telegraph clone wraps
# `float ApproachAttackIntervalSeconds = 6.5f;` with ClampMin `"0.2"`
# — RETARGET type to int32, ClampMin `"0.2"` to `"0"`, add ClampMax `"32"`.
# wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Telegraph", meta=(ClampMin="0", ClampMax="32")
# (NO spaces around =).
# LOCKED_DECL is `int32 MaxTelegraphsPerEncounter = 16;`
...
# and NOT leftover analog `float MaxTelegraphsPerEncounter = 16;`
# and NOT leftover unique `int32 MaxSimulationSubsteps = 8;`
# and NOT leftover unique `float ApproachAttackIntervalSeconds = 6.5f;`
...
# AS THE LOCKED DECL.
# Fail-closed if LOCKED_DECL is still `float` / still
# MaxSimulationSubsteps / 8 / FName WeatherIdentity / 160.f /
# leftover unique float 6.5f / ClampMin is still `"0.2"` /
# leftover unique Safety ClampMin `"1"` ClampMax `"16"` /
# leftover analog `"100.0"`.
...
# or leftover unique Safety `Skyguard|Boss|Encounter|Safety`
...
# or default is not 16.
# Default is `= 16` not `= 8` / `= 6.5f` / `= 3600.f` / `= 0.05f` /
# `= 850.f` / `= 700.f` / `= 1250.f` / `= 9000.f`.
...
TARGET = "int32 MaxTelegraphsPerEncounter = 16;"
TARGET_WRONG_BARE = "int32 MaxTelegraphsPerEncounter;"
TARGET_WRONG_INIT = "int32 MaxTelegraphsPerEncounter = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "int32 MaxTelegraphsPerEncounter = true;"
TARGET_WRONG_ZERO = "int32 MaxTelegraphsPerEncounter = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MaxTelegraphsPerEncounter;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MaxTelegraphsPerEncounter;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MaxTelegraphsPerEncounter;"
...
STOP_BEFORE_SKYLINE_STYLE = "ObservedPhase"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_max_telegraphs"
...
TARGET_WRONG_FLOAT_SUBSTEPS = "float MaxTelegraphsPerEncounter = 16;"
TARGET_WRONG_SUBSTEPS = "int32 MaxSimulationSubsteps = 8;"
...
SIBLING_MAX_SIMULATION_SUBSTEPS = "MaxSimulationSubsteps"
...
MAX_TELEGRAPHS_DECL_RE = re.compile(
    r"int32\s+MaxTelegraphsPerEncounter\s*=\s*16\s*;"
)
MAX_TELEGRAPHS_BARE_RE = re.compile(
    r"int32\s+MaxTelegraphsPerEncounter\s*;"
)
...
LEFTOVER_FLOAT_SUBSTEPS_DECL_RE = re.compile(
    r"float\s+MaxTelegraphsPerEncounter\s*=\s*16\s*;"
)
...
LEFTOVER_SIMULATION_SUBSTEPS_DECL_RE = re.compile(
    r"int32\s+MaxSimulationSubsteps\b"
)
LEFTOVER_APPROACH_ATTACK_DECL_RE = re.compile(
    r"float\s+ApproachAttackIntervalSeconds\s*=\s*6\.5f\b"
)
LEFTOVER_LOCK_WINDOW_ATTACK_DECL_RE = re.compile(
    r"float\s+LockWindowAttackIntervalSeconds\s*=\s*8\.f\b"
)
LEFTOVER_CRITICAL_ATTACK_DECL_RE = re.compile(
    r"float\s+CriticalAttackIntervalSeconds\s*=\s*4\.8f\b"
)
LEFTOVER_TELEGRAPH_LEAD_DECL_RE = re.compile(
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_MAX_SIMULATION_SUBSTEPS,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 MaxTelegraphsPerEncounter = 16;`.
    ...
    # MaxTelegraphsPerEncounter without the 16 initializer.
    # Fail-closed if LOCKED_DECL is still float or leftover analog
    # float MaxLateralOffsetCm = 3600.f / 0.05f / ClampMin "100.0" /
    # leftover unique Safety MaxSimulationSubsteps = 8 /
    # leftover unique float ApproachAttackIntervalSeconds = 6.5f /
    # ClampMin "0.2" / ClampMin "1" / ClampMax "16".
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MAX_TELEGRAPHS_DECL_RE.search(compact) is None:
        return False
    if MAX_TELEGRAPHS_BARE_RE.search(compact):
        return False
    ...
    if LEFTOVER_VOICE_LIMIT_DECL_RE.search(compact):
        return False
    if LEFTOVER_SIMULATION_SUBSTEPS_DECL_RE.search(compact):
        return False
    if LEFTOVER_APPROACH_ATTACK_DECL_RE.search(compact):
        return False
    if LEFTOVER_LOCK_WINDOW_ATTACK_DECL_RE.search(compact):
        return False
    if LEFTOVER_CRITICAL_ATTACK_DECL_RE.search(compact):
        return False
    if LEFTOVER_TELEGRAPH_LEAD_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "MaxTelegraphsPerEncounter":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+MaxTelegraphsPerEncounter\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 MaxTelegraphsPerEncounter "
...
class PathfinderEncounterMaxTelegraphsPerEncounterFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxTelegraphsPerEncounter"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MaxTelegraphsPerEncounter"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaxTelegraphsPerEncounter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_max_telegraphs_per_encounter_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', section)
        ...
        self.assertIn('ClampMin="0"', specifiers)
        ...
        self.assertNotIn('ClampMin="0.05"', specifiers)
        self.assertIn("ClampMax", specifiers)
        self.assertIn('ClampMax="32"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("float MaxTelegraphsPerEncounter = 16;", compact_origin)
        self.assertNotIn(
            "int32 MaxTelegraphsPerEncounter;",
            compact_origin.split("MaxTelegraphsPerEncounter = 16;")[0]
            + compact_origin.split("MaxTelegraphsPerEncounter = 16;")[-1]
            if "MaxTelegraphsPerEncounter = 16;" in compact_origin
            else compact_origin,
        )
    ...
    def test_max_telegraphs_per_encounter_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 MaxTelegraphsPerEncounter"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 16", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SUBSTEPS)
        ...
        self.assertNotIn("4.8f", LOCKED_DECL)
        self.assertNotIn("1.2f", LOCKED_DECL)
        self.assertNotIn("8.f", LOCKED_DECL)
        ...
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_TELEGRAPH)
        self.assertNotEqual(
            UPROPERTY_EDIT_READWRITE,
            TARGET_WRONG_FLOAT_TELEGRAPH,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 MaxTelegraphsPerEncounters = 8;\n",
            "\tfloat MaxTelegraphsPerEncounter = 16;\n",
            "\tFName MaxTelegraphsPerEncounter = NAME_None;\n",
            "\tint32 MaxTelegraphsPerEncounter = " + forty + ";\n",
            "\tint32 MaxTelegraphsPerEncounter = " + eighty + ";\n",
            f"\t{TARGET_WRONG_LATERAL}\n",
            f"\t{TARGET_WRONG_SIM_STEP}\n",
            f"\t{TARGET_WRONG_FLOAT_SUBSTEPS}\n",
            f"\t{TARGET_WRONG_SUBSTEPS}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxTelegraphsPerEncounter"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "MaxTelegraphsPerEncounter"))
        self.assertTrue(has_identifier(section, "MaxTelegraphsPerEncounter"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
        ...
        self.assertFalse(has_identifier(section, SIBLING_MAX_SIMULATION_SUBSTEPS))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="100.0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="1.0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="1"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="0.2"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="0.05"', UPROPERTY_EDIT_READWRITE)
        self.assertIn("ClampMax", UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMax="32"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMax="16"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter max simulation substeps field decl contract "
