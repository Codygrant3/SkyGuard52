# THIS IS leftover-safe FSkyguardStormRainBeatKit BeatCount.
# origin/main form: in-struct static constexpr
# `static constexpr int32 BeatCount = 7;`
# THIS IS NOT leftover analog beats bulk.
# THIS IS NOT leftover Night/Day kit `Beats[7]` isolated.
# THIS IS NOT leftover array fields Kinds / Threats /
# Stations / Calls that *use* BeatCount as extent.
...
# Do NOT contract sibling fields MissionId / Title /
# WeatherIdentity / WeatherLabel / Weather /
# bHydraForClusters / Kinds / Threats / Stations / Calls.
# Fail-closed if this lane claims leftover Night/Day
# kit Beats[7] instead of in-struct BeatCount.
# Fail-closed if initializer is not `7`, if `static` or
# `constexpr` is dropped, or if type is not `int32`.
...
# THIS IS NOT leftover Night kit Beats isolated #1486.
# THIS IS NOT leftover Day kit Beats isolated #1487.
# THIS IS NOT leftover-safe StormRain Threats isolated.
# THIS IS NOT leftover-safe StormRain Kinds / Stations /
# Calls isolated. Isolated field decl does not relock
# leftover array fields that use BeatCount as extent.
...
# leftover Night `Beats[7]` / leftover Day `Beats[7]` /
# leftover Threats[BeatCount] / invented `BeatCount[7]`,
# retarget: storage is static constexpr, type is int32,
# identifier is BeatCount, initializer is `7`, locked
# decl is the plain field, not a UPROPERTY wrap.
...
TARGET = "static constexpr int32 BeatCount = 7;"
TARGET_WRONG_NO_STATIC = "constexpr int32 BeatCount = 7;"
TARGET_WRONG_NO_CONSTEXPR = "static int32 BeatCount = 7;"
TARGET_WRONG_NO_BOTH = "int32 BeatCount = 7;"
TARGET_WRONG_CONST = "static const int32 BeatCount = 7;"
TARGET_WRONG_BARE = "static constexpr int32 BeatCount;"
TARGET_WRONG_SIX = "static constexpr int32 BeatCount = 6;"
TARGET_WRONG_EIGHT = "static constexpr int32 BeatCount = 8;"
TARGET_WRONG_ZERO = "static constexpr int32 BeatCount = 0;"
TARGET_WRONG_ZERO_F = "static constexpr int32 BeatCount = 0.f;"
TARGET_WRONG_FALSE = "static constexpr int32 BeatCount = false;"
TARGET_WRONG_TRUE = "static constexpr int32 BeatCount = true;"
TARGET_WRONG_BRACES = "static constexpr int32 BeatCount = {};"
TARGET_WRONG_SEVEN_F = "static constexpr int32 BeatCount = 7.f;"
TARGET_WRONG_FLOAT = "static constexpr float BeatCount = 7;"
TARGET_WRONG_UINT = "static constexpr uint32 BeatCount = 7;"
TARGET_WRONG_INT = "static constexpr int BeatCount = 7;"
TARGET_WRONG_BOOL = "static constexpr bool BeatCount = 7;"
TARGET_WRONG_FNAME = "static constexpr FName BeatCount = 7;"
TARGET_WRONG_INLINE = "inline static constexpr int32 BeatCount = 7;"
TARGET_WRONG_KINDS = (
...
TARGET_WRONG_THREATS_SEVEN = "ESkyguardThreatKind Threats[7] = {};"
...
CLONE_THREATS_FIELD_DECL = (
    "Scripts/tests/test_storm_rain_beat_kit_threats"
    "_field_decl_contract.py"
)
CLONE_KINDS_FIELD_DECL = (
...
BEAT_COUNT_DECL_RE = re.compile(
    r"static\s+constexpr\s+int32\s+BeatCount\s*=\s*7\s*;"
)
BEAT_COUNT_INIT_RE = re.compile(
    r"(?:inline\s+)?(?:static\s+)?(?:constexpr\s+)?"
    r"(?:const\s+)?int32\s+BeatCount\s*=\s*([^;]+);"
)
...
def leftover_array_extent_decls() -> tuple[str, ...]:
    return (
        TARGET_WRONG_KINDS,
...
def leftover_beats_array_decls() -> tuple[str, ...]:
    return (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `static constexpr int32 BeatCount = 7;`.
    # Do not accept dropped static / constexpr, type
    # other than int32, initializer other than 7,
    # leftover Night/Day Beats[7], leftover array
    # fields that use BeatCount as extent, leftover
    # sibling identifiers as the field name, or a
    # UPROPERTY wrap as the locked decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if BEAT_COUNT_DECL_RE.search(compact) is None:
        return False
    if re.search(r"\binline\s+static\s+constexpr\s+int32\s+BeatCount\b", compact):
        return False
    if re.search(r"\bconstexpr\s+int32\s+BeatCount\b", compact):
        if re.search(
            r"\bstatic\s+constexpr\s+int32\s+BeatCount\b",
            compact,
        ) is None:
            return False
    if re.search(r"\bstatic\s+int32\s+BeatCount\b", compact):
        if re.search(
            r"\bstatic\s+constexpr\s+int32\s+BeatCount\b",
            compact,
        ) is None:
            return False
    if re.search(r"\bfloat\s+BeatCount\b", compact):
        return False
    if re.search(r"\buint32\s+BeatCount\b", compact):
        return False
    if re.search(r"\bbool\s+BeatCount\b", compact):
        return False
    if re.search(r"\bFName\s+BeatCount\b", compact):
        return False
    if re.search(r"\bstatic\s+const\s+int32\s+BeatCount\b", compact):
        return False
    if re.search(
        r"static\s+constexpr\s+int32\s+BeatCount\s*;",
        compact,
    ) and not re.search(
        r"static\s+constexpr\s+int32\s+BeatCount\s*=",
        compact,
        ...
        return False
    for match in BEAT_COUNT_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != "7":
            return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static constexpr int32 BeatCount\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for static constexpr int32 BeatCount is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*static constexpr int32 BeatCount\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on static constexpr int32 BeatCount is not "
            f"the locked decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "BeatCount"):
        raise AssertionError(
            "UPROPERTY clone landed on BeatCount; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "BeatCount"):
            raise AssertionError(
                f"{token} clone landed on BeatCount; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class StormRainBeatKitBeatCountFieldDeclContractTests(unittest.TestCase):
    def test_storm_rain_beat_kit_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BeatCount"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("static constexpr int32"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("static", LOCKED_DECL)
        self.assertIn("constexpr", LOCKED_DECL)
        ...
        self.assertNotIn("Beats[7]", LOCKED_DECL)
        self.assertNotIn("[BeatCount]", LOCKED_DECL)
        ...
        self.assertNotIn(SIBLING_CALLS, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_KEEP_THERMAL, LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_NO_STATIC)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_NO_CONSTEXPR)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_NIGHT_BEATS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_DAY_BEATS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_THREATS)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("BeatCount", str(raised.exception))
    ...
    def test_does_not_claim_leftover_day_night_loadout_or_campaign(self) -> None:
        ...
        self.assertIn(TARGET_WRONG_DAY_BEAT_THREAT, day_header)
        ...
        self.assertIn(TARGET_WRONG_NIGHT_BEAT_THREAT, night_header)
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BeatCount"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_beat_count_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_title_does_not_satisfy_beat_count(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_TITLE}\n"
    ...
    def test_leftover_array_extent_fields_do_not_satisfy(self) -> None:
        leftovers = leftover_array_extent_decls() + (
            TARGET_WRONG_THREATS_SEVEN,
        )
        ...
        self.assertNotIn(SIBLING_CALLS, LOCKED_DECL)
        self.assertNotIn("[BeatCount]", LOCKED_DECL)
        self.assertNotEqual(THIS_SCRIPT, CLONE_THREATS_FIELD_DECL)
        self.assertIn(CLONE_THREATS_FIELD_DECL, LOCKED_SCRIPTS)
        self.assertIn(CLONE_KINDS_FIELD_DECL, LOCKED_SCRIPTS)

    def test_leftover_night_day_beats_array_does_not_satisfy(self) -> None:
        leftovers = leftover_beats_array_decls()
        for leftover_decl in leftovers:
            leftover = (
                f"struct {STRUCT_NAME}\n"
                "{\n"
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertNotEqual(LOCKED_DECL, leftover_decl)
        self.assertNotEqual(THIS_SCRIPT, CLONE_NIGHT_KIT_BEATS_FIELD_DECL)
        self.assertNotEqual(THIS_SCRIPT, CLONE_DAY_KIT_BEATS_FIELD_DECL)
        self.assertIn(CLONE_NIGHT_KIT_BEATS_FIELD_DECL, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DAY_KIT_BEATS_FIELD_DECL, LOCKED_SCRIPTS)

    def test_leftover_night_day_beat_threat_does_not_satisfy(self) -> None:
        leftovers = leftover_beat_threat_decls()
        for leftover_decl in leftovers:
            leftover = (
                f"struct {STRUCT_NAME}\n"
                "{\n"
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertNotEqual(LOCKED_DECL, leftover_decl)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_NIGHT_BEAT_THREAT_FIELD_DECL)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_DAY_BEAT_THREAT_FIELD_DECL)
        self.assertIn(LEFTOVER_NIGHT_BEAT_THREAT_FIELD_DECL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_DAY_BEAT_THREAT_FIELD_DECL, LOCKED_SCRIPTS)
    ...
    def test_static_and_constexpr_are_required(self) -> None:
        dropped = (
            TARGET_WRONG_NO_STATIC,
            TARGET_WRONG_NO_CONSTEXPR,
            TARGET_WRONG_NO_BOTH,
            TARGET_WRONG_CONST,
            TARGET_WRONG_INLINE,
        )
        for leftover_decl in dropped:
            leftover = (
                f"struct {STRUCT_NAME}\n"
                "{\n"
                f"\t{leftover_decl}\n"
    ...
    def test_initializer_must_be_seven(self) -> None:
        missing = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
        ...
        section = spec_section(invented_six)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("BeatCount", str(raised.exception))
        ...
        self.assertNotIn("BeatCount = 6", compact_origin)
        self.assertNotIn("BeatCount = 8", compact_origin)
        self.assertNotIn("BeatCount = false", compact_origin)
        self.assertNotIn("BeatCount = true", compact_origin)
        self.assertNotIn("BeatCount = 0.f", compact_origin)
        self.assertNotIn("BeatCount = {}", compact_origin)
        self.assertNotIn(
            "static constexpr int32 BeatCount;",
    ...
    def test_beat_count_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("static constexpr int32 BeatCount"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("uint32 ", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_TRUE}\n",
            f"\t{TARGET_WRONG_BRACES}\n",
            f"\t{TARGET_WRONG_SEVEN_F}\n",
            f"\t{TARGET_WRONG_FLOAT}\n",
            f"\t{TARGET_WRONG_UINT}\n",
            f"\t{TARGET_WRONG_INT}\n",
            f"\t{TARGET_WRONG_BOOL}\n",
            f"\t{TARGET_WRONG_FNAME}\n",
            f"\t{TARGET_WRONG_INLINE}\n",
            f"\t{TARGET_WRONG_KINDS}\n",
            f"\t{TARGET_WRONG_THREATS}\n",
            f"\t{TARGET_WRONG_STATIONS}\n",
            f"\t{TARGET_WRONG_CALLS}\n",
            f"\t{TARGET_WRONG_THREATS_SEVEN}\n",
            f"\t{TARGET_WRONG_HYDRA}\n",
            f"\t{TARGET_WRONG_TITLE}\n",
            f"\t{TARGET_WRONG_KEEP_THERMAL}\n",
            f"\t{TARGET_WRONG_LABEL}\n",
            f"\t{TARGET_WRONG_NIGHT_BEAT_THREAT}\n",
            f"\t{TARGET_WRONG_DAY_BEAT_THREAT}\n",
            f"\t{TARGET_WRONG_NIGHT_BEATS}\n",
            ...
            "\tstatic constexpr int32 Beat = 7;\n",
            "\tstatic constexpr int32 BeatCounts = 7;\n",
            "\tstatic constexpr int32 BeatCount = " + forty + ";\n",
            "\tstatic constexpr int32 BeatCount = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_kit_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BeatCount"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "storm-rain-beat-kit BeatCount field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "beat_count_field_decl_contract.py"
        ))
        self.assertIn("storm_rain_beat_kit", Path(__file__).name)
        self.assertNotIn("keep_thermal", Path(__file__).name)
        self.assertNotIn("night_sortie_beat_kit_beats", Path(__file__).name)
        self.assertNotIn("day_sortie_beat_kit_beats", Path(__file__).name)
        self.assertNotIn("SkyguardStormRainBeatKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_ANALOG_STORM_KIT_THREATS, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_CALLS, locked_only)
