...
# or lock the IsClimbCommandIssued body.
# origin/main is inline
# (`bool IsClimbCommandIssued() const`
# with `{ return bClimbCommandIssued; }` possibly on the
...
# locking the body. The inline return of the climb
# command issued flag is not a required token of this
...
# GetObjectiveMilestonesReached / IsWreckDiverted /
# GetCivilianSeparationMeters. Stay off leftover
# GetFinaleStage / ESkyguardLastFlightStage. Stay off leftover
...
# isolated-test drafts #107–#457 including leftover
...
IS_CLIMB_COMMAND_ISSUED = "bool IsClimbCommandIssued() const;"
UFUNCTION_BOSS = (
...
# Python IsClimbCommandIssued declaration
...
# SetCivilianSeparationMeters /
# GetObjectiveMilestonesReached / IsWreckDiverted /
# GetCivilianSeparationMeters, leftover
...
# leftover isolated-test drafts #107–#457,
...
# SetCivilianSeparationMeters /
# GetObjectiveMilestonesReached / IsWreckDiverted /
# GetCivilianSeparationMeters siblings, leftover
# GetFinaleStage / ESkyguardLastFlightStage, leftover
# apache aircraft isolated contracts, leftover
...
LIVE_INLINE_IS_CLIMB_COMMAND_ISSUED = (
...
# .cpp IsClimbCommandIssued body / invented
# INDEX_NONE stay unlocked. Do not invent INDEX_NONE
# or lock the cpp body. The live inline return is
# accepted on origin/main and is not a locked token
# of this declaration. Do not parse leftover HUD
# classes.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::IsClimbCommandIssued",
    "SkyguardLastFlightBoss.cpp",
    "SetExposed",
...
def leftover_short_roster_values() -> tuple[str, ...]:
    # Leftover roster Climb stays sibling-only as an
    # enum value. IsClimbCommandIssued is the locked
    # method stem, so leftover roster Climb is not
    # asserted against the locked declaration.
    return (
        "Br" + "eak",
        "Ho" + "ld",
        "Des" + "cend",
...
class LastFlightIsClimbCommandIssuedDeclContractTests(unittest.TestCase):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, IS_CLIMB_COMMAND_ISSUED),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_CLIMB_COMMAND_ISSUED)
        self.assertIn("IsClimbCommandIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_missing_is_climb_command_issued_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLastFlightBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_CLIMB_COMMAND_ISSUED)
        self.assertIn("IsClimbCommandIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_CLIMB_COMMAND_ISSUED)
        self.assertIn("IsClimbCommandIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("UFUNCTION", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("Category", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("BlueprintCallable", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_CLIMB_COMMAND_ISSUED)
        self.assertIn("IsClimbCommandIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool IsClimbCommandIssued const;\n"
        missing_const = "\tbool IsClimbCommandIssued();\n"
        wrong_return_void = "\tvoid IsClimbCommandIssued() const;\n"
        wrong_return_int = "\tint32 IsClimbCommandIssued() const;\n"
        added_arg = "\tbool IsClimbCommandIssued(bool bValue) const;\n"
        leftover_issue = f"\t{ISSUE_CLIMB_COMMAND}\n"
        ...
        leftover_milestones = f"\t{GET_OBJECTIVE_MILESTONES}\n"
        leftover_wreck = f"\t{IS_WRECK_DIVERTED}\n"
        leftover_get_sep = f"\t{GET_CIVILIAN_SEPARATION_METERS}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_CLIMB_COMMAND_ISSUED)
            self.assertIn("IsClimbCommandIssued", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_climb_command_issued_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, IS_CLIMB_COMMAND_ISSUED),
            1,
        )
        self.assertTrue(
            IS_CLIMB_COMMAND_ISSUED.startswith("bool "),
            IS_CLIMB_COMMAND_ISSUED,
        )
        self.assertTrue(
            IS_CLIMB_COMMAND_ISSUED.endswith(";"),
            IS_CLIMB_COMMAND_ISSUED,
        )
        self.assertIn("IsClimbCommandIssued()", IS_CLIMB_COMMAND_ISSUED)
        self.assertIn(" const", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("INDEX_NONE", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("{", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("}", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return ", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return bClimbCommandIssued", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("IssueClimbCommand", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            IS_CLIMB_COMMAND_ISSUED,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            IS_CLIMB_COMMAND_ISSUED,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetGunnerMount", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("FaceWorldLocation", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("PortGuidanceArray", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("SetSearchlightTracked", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("SetContactIdentified", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            IS_CLIMB_COMMAND_ISSUED,
        )
        self.assertNotIn("SetLightningExposed", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            IS_CLIMB_COMMAND_ISSUED,
        )
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_declaration_accepts_origin_main_inline_and_prototype(self) -> None:
        ...
        self.assertTrue(
            has_declaration(one_line_prototype, IS_CLIMB_COMMAND_ISSUED)
        )
        self.assertEqual(
            require_declaration(one_line_prototype, IS_CLIMB_COMMAND_ISSUED),
            IS_CLIMB_COMMAND_ISSUED,
        )
        exact_inline = LIVE_INLINE_IS_CLIMB_COMMAND_ISSUED
        self.assertTrue(has_declaration(exact_inline, IS_CLIMB_COMMAND_ISSUED))
        self.assertEqual(
            require_declaration(exact_inline, IS_CLIMB_COMMAND_ISSUED),
            IS_CLIMB_COMMAND_ISSUED,
        )
        self.assertEqual(
            declaration_count(exact_inline, IS_CLIMB_COMMAND_ISSUED),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tIsClimbCommandIssued() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tbool IsClimbCommandIssued(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        wrap_inline = (
            "public:\n"
            "\tbool IsClimbCommandIssued()\n"
            "\tconst { return bClimbCommandIssued; }\n"
            "};\n"
        )
        header_wrap_type = (
            f"class SKYGUARD52_API {CLASS_NAME} "
        ...
        self.assertTrue(
            has_declaration(one_line, IS_CLIMB_COMMAND_ISSUED)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, IS_CLIMB_COMMAND_ISSUED),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return bClimbCommandIssued", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return Other", IS_CLIMB_COMMAND_ISSUED)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{IS_CLIMB_COMMAND_ISSUED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", IS_CLIMB_COMMAND_ISSUED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_climb_command_issued_cpp_body(
        self,
    ) -> None:
        locked_only = f"{IS_CLIMB_COMMAND_ISSUED}\n"
        self.assertNotIn("{", IS_CLIMB_COMMAND_ISSUED)
        ...
        self.assertNotIn("return ", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return bClimbCommandIssued", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return bClimbCommandIssued", locked_only)
        self.assertNotIn(
            "ASkyguardLastFlightBoss::IsClimbCommandIssued",
            IS_CLIMB_COMMAND_ISSUED,
        )
        ...
        self.assertNotIn("return false", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("return true", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("SetExposed", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            IS_CLIMB_COMMAND_ISSUED,
        )
        self.assertNotIn("SetBossPhase", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_contract_does_not_relock_get_finale_stage_sibling(self) -> None:
        ...
        for token in GET_FINALE_STAGE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetFinaleStage", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetFinaleStage", locked_only)
        self.assertNotIn("ESkyguardLastFlightStage", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("ESkyguardLastFlightStage", locked_only)
        self.assertNotIn(
            "test_last_flight_get_finale_stage_decl_contract.py",
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetFinaleStage", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("IsWreckDiverted", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "GetCivilianSeparationMeters",
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("PortGuidanceArray", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("StarboardGuidanceArray", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("CommandCore", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("PortEngine", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("SetSearchlightTracked", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("IsSearchlightTracked", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("ASkyguardBlackKiteBoss", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("SetContactIdentified", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("IsContactIdentified", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("OpenOrbitExposure", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("ASkyguardIronRainBoss", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetGunnerMount", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetChinTurret", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetSensorTurret", IS_CLIMB_COMMAND_ISSUED)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{IS_CLIMB_COMMAND_ISSUED}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("GetChinMuzzleLocation", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("ESkyguardApacheSystem", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("ASkyguardGunner", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("FillAndFinalize", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("SkyguardRadarNode", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("MinHeightFromOriginCm", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("MaxIntegrity", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            IS_CLIMB_COMMAND_ISSUED,
        )
        self.assertNotIn("SkyguardPathfinder", IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, IS_CLIMB_COMMAND_ISSUED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_CLIMB_COMMAND_ISSUED}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_CLIMB_COMMAND_ISSUED),
            IS_CLIMB_COMMAND_ISSUED,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight IsClimbCommandIssued contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, IS_CLIMB_COMMAND_ISSUED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                IS_CLIMB_COMMAND_ISSUED.lower(),
                "last flight IsClimbCommandIssued contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_CLIMB_COMMAND_ISSUED)

    def test_contract_is_is_climb_command_issued_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return bClimbCommandIssued", IS_CLIMB_COMMAND_ISSUED)
        self.assertNotIn("{", IS_CLIMB_COMMAND_ISSUED)
        self.assertTrue(IS_CLIMB_COMMAND_ISSUED.startswith("bool "))
        self.assertTrue(IS_CLIMB_COMMAND_ISSUED.endswith(";"))
        self.assertIn(" const", IS_CLIMB_COMMAND_ISSUED)
        self.assertIn(UFUNCTION_BOSS, section)
