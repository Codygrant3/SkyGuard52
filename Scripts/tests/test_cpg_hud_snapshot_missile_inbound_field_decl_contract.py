# THIS IS leftover-safe FSkyguardCpgHudSnapshot bMissileInbound.
...
# `bool bMissileInbound = false;` with in-struct initializer
# `= false`. Type is bool. Identifier is bMissileInbound.
# Fail-closed if bMissileInbound is missing or renamed, if the
# type is not bool, if the initializer is missing, is
# leftover StormRain `= true`, or is not `= false`, or if
# UPROPERTY / Category clones land.
...
# locked decl.
# Fail-closed if leftover StormRainBeatKit
# `bool bHydraForClusters = true` (#1494) is locked as THIS
# HUD field. HUD snapshot `bool bMissileInbound = false`
# is NOT leftover StormRain `bool bHydraForClusters = true`.
# Fail-closed if leftover ContactMark `bool bLocked = false`
# or `bool bSeeking = false` is locked as THIS field.
# Fail-closed if leftover CpgDebriefSnapshot
# `bool bValid = false` / `bool bWon = false` is locked as
# THIS field.
# Fail-closed if leftover LoadoutSpec
# `int32 FlareCount = 6` (#1467) is locked as THIS HUD
# field. HUD snapshot `bool bMissileInbound = false` is NOT
# leftover LoadoutSpec `int32 FlareCount = 6`.
# Fail-closed if leftover HUD `int32 FlareCount = 0` is
# locked as THIS field. Do not claim sibling FlareCount.
# Fail-closed if the parse window is leftover
# FSkyguardCpgContactMark instead of
...
# Do NOT git-show leftover SkyguardGunshipTypes.h as the
# parse source. Parse source is origin/main
# Source/Skyguard52/SkyguardCpgHud.h.
...
# ThreatCount / HeadingDegrees / LockProgress /
# FlareCount.
...
# #1467. Keep that file in LOCKED_SCRIPTS. Do not reopen
# it. Do not copy its parse window.
...
# LockPhase #1508 / RangeMeters #1509 / SightMode #1510 /
# ThreatCount #1511 / HeadingDegrees / LockProgress /
# FlareCount.
# THIS IS NOT leftover StormRainBeatKit #1490-#1500.
# THIS IS NOT leftover StormRain #1494
# bHydraForClusters (`bool bHydraForClusters = true`).
# Clone LOCKED_SCRIPTS from leftover-safe #1300. Clone
# #1300 is UPROPERTY-based. This lane is a PLAIN struct.
# Better HUD parse-window retarget: leftover-safe #1502
# WeaponLine (FString, no initializer). RETARGET: this
# field is bool with `= false`.
# StormRain leftover-safe #1494 bHydraForClusters is a
# bool-with-initializer plain-struct retarget
# (`bool bHydraForClusters = true`) — still a different
# identifier and initializer (`true` vs `false`).
...
# leftover `FString WeaponLine;` (no initializer) /
# leftover `bool bHydraForClusters = true;` / leftover
# ContactMark `bool bLocked = false;` / leftover HUD
# `int32 FlareCount = 0;` / leftover LoadoutSpec
# `int32 FlareCount = 6;`, retarget: type is bool,
# identifier is bMissileInbound, initializer is `= false`,
# locked decl is the plain field, not a UPROPERTY wrap,
# not leftover StormRain `= true`, not leftover
# ContactMark `bLocked`, not leftover LoadoutSpec `= 6`.
...
TARGET = "bool bMissileInbound = false;"
TARGET_WRONG_BARE = "bool bMissileInbound;"
TARGET_WRONG_EQ_EMPTY = 'bool bMissileInbound = TEXT("");'
TARGET_WRONG_EQ_NONE = "bool bMissileInbound = NAME_None;"
TARGET_WRONG_TRUE = "bool bMissileInbound = true;"
TARGET_WRONG_ZERO = "bool bMissileInbound = 0;"
TARGET_WRONG_ONE = "bool bMissileInbound = 1;"
TARGET_WRONG_ZERO_F = "bool bMissileInbound = 0.f;"
TARGET_WRONG_SIX = "bool bMissileInbound = 6;"
TARGET_WRONG_INT = "int32 bMissileInbound = false;"
TARGET_WRONG_FLOAT = "float bMissileInbound;"
TARGET_WRONG_HEALTH = "float bMissileInbound = 160.f;"
TARGET_WRONG_INT32 = "int32 bMissileInbound;"
TARGET_WRONG_FSTRING = "FString bMissileInbound;"
TARGET_WRONG_FNAME = "FName bMissileInbound;"
TARGET_WRONG_TCHAR = 'const TCHAR* bMissileInbound = TEXT("");'
TARGET_WRONG_THREAT_COUNT = "int32 ThreatCount = 0;"
TARGET_WRONG_FLARE_COUNT = "int32 FlareCount = 0;"
...
SIBLING_THREAT_COUNT_DECL = "int32 ThreatCount = 0;"
SIBLING_FLARE_COUNT_DECL = "int32 FlareCount = 0;"
LEFTOVER_HYDRA_DECL = "bool bHydraForClusters = true;"
LEFTOVER_KEEP_THERMAL_DECL = "bool bKeepThermal = true;"
...
MISSILE_INBOUND_INIT_RE = re.compile(
    r"bool\s+bMissileInbound\s*=\s*false\s*;"
)
MISSILE_INBOUND_BARE_RE = re.compile(r"bool\s+bMissileInbound\s*;")
MISSILE_INBOUND_WRONG_INIT_RE = re.compile(
    r"bool\s+bMissileInbound\s*=\s*(?:true|0\.f|0|1|6|NAME_None)"
)
MISSILE_INBOUND_INIT_CAPTURE_RE = re.compile(
    r"bool\s+bMissileInbound\s*=\s*([^;]+);"
)
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
...
def leftover_analog_bool_decls() -> tuple[str, ...]:
    return (
        LEFTOVER_HYDRA_DECL,
        LEFTOVER_KEEP_THERMAL_DECL,
        LEFTOVER_CONTACT_LOCKED,
        LEFTOVER_CONTACT_SEEKING,
        LEFTOVER_DEBRIEF_VALID,
        LEFTOVER_DEBRIEF_WON,
...
def missile_inbound_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*bool\s+bMissileInbound\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bMissileInbound = false;`.
    # Do not accept a missing initializer, `= true`, leftover
    # StormRain Hydra `= true`, leftover ContactMark
    # `bLocked` / `bSeeking`, leftover debrief `bValid` /
    # `bWon`, leftover HUD FlareCount, leftover LoadoutSpec
    # FlareCount `= 6`, leftover ThreatLine, leftover Beat
    # Threat, leftover ContactMark Label, leftover debrief
    # Score, leftover LoadoutSpec parse window, or
    # UPROPERTY / Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MISSILE_INBOUND_WRONG_INIT_RE.search(compact):
        return False
    if MISSILE_INBOUND_BARE_RE.search(
        compact
    ) and not MISSILE_INBOUND_INIT_RE.search(compact):
        return False
    if re.search(r"int32\s+bMissileInbound\b", compact):
        return False
    if re.search(r"float\s+bMissileInbound\b", compact):
        return False
    if re.search(r"FString\s+bMissileInbound\b", compact):
        return False
    if re.search(r"FName\s+bMissileInbound\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*bMissileInbound\b", compact):
        return False
    if MISSILE_INBOUND_INIT_RE.search(compact) is None:
        return False
    for match in MISSILE_INBOUND_INIT_CAPTURE_RE.finditer(compact):
        if collapsed(match.group(1)) != "false":
            return False
    if missile_inbound_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    if re.search(r"bool\s+bHydraForClusters\b", compact) and not has_identifier(
        compact,
        "bMissileInbound",
    ):
        return False
    if re.search(r"bool\s+bLocked\b", compact) and not has_identifier(
        ...
        return False
    if re.search(r"int32\s+FlareCount\b", compact) and not has_identifier(
        compact,
        "bMissileInbound",
    ):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool bMissileInbound\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bMissileInbound is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*bool bMissileInbound\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on bool bMissileInbound is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "bMissileInbound"):
        raise AssertionError(
            "UPROPERTY clone landed on bMissileInbound; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "bMissileInbound"):
            raise AssertionError(
                f"{token} clone landed on bMissileInbound; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotMissileInboundFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bMissileInbound"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "bool bMissileInbound = false;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("bool bMissileInbound"))
        ...
        self.assertIn("bMissileInbound", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_THREAT_COUNT)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIX)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_DAY_THREAT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_NIGHT_THREAT)
        self.assertNotEqual(LOCKED_DECL, SIBLING_THREAT_COUNT_DECL)
        self.assertNotEqual(LOCKED_DECL, SIBLING_FLARE_COUNT_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_LOADOUT_FLARE_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_HYDRA_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_CONTACT_LOCKED)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("bMissileInbound", str(raised.exception))
    ...
    def test_leftover_beat_threat_headers_do_not_satisfy(self) -> None:
        ...
        self.assertIn(LEFTOVER_HYDRA_DECL, storm_header)
        self.assertNotIn(LOCKED_DECL, storm_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        mixed = (
            ...
            "{\n"
            f"\t{LEFTOVER_CONTACT_LABEL}\n"
            f"\t{LEFTOVER_CONTACT_LOCKED}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        section = spec_section(mixed)
        self.assertFalse(has_identifier(section, "bMissileInbound"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_missile_inbound_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_THREAT_LINE}\n"
            f"\t{TARGET_WRONG_WEAPON_LINE}\n"
            f"\t{SIBLING_THREAT_COUNT_DECL}\n"
            f"\t{SIBLING_FLARE_COUNT_DECL}\n"
    ...
    def test_sibling_flare_count_does_not_satisfy_missile_inbound(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{SIBLING_FLARE_COUNT_DECL}\n"
    ...
    def test_sibling_threat_count_does_not_satisfy_missile_inbound(
        self,
    ) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_threat_line_does_not_satisfy_missile_inbound(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_THREAT_LINE}\n"
    ...
    def test_leftover_contact_locked_does_not_satisfy(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{LEFTOVER_CONTACT_LOCKED}\n"
    ...
    def test_leftover_hydra_true_does_not_satisfy(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{LEFTOVER_HYDRA_DECL}\n"
            "};\n"
        )
        section = spec_section(leftover)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_HYDRA_DECL)
        self.assertNotIn("= true", LOCKED_DECL)
        self.assertIn(LEFTOVER_STORM_HYDRA, LOCKED_SCRIPTS)
    ...
    def test_initializer_missing_or_true_fails_closed(self) -> None:
        invented = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
        ...
        for wrong in (
            ...
            LEFTOVER_KEEP_THERMAL_DECL,
            LEFTOVER_CONTACT_LOCKED,
            SIBLING_FLARE_COUNT_DECL,
            LEFTOVER_LOADOUT_FLARE_DECL,
        ):
            wrapped = (
                f"struct {STRUCT_NAME}\n"
                "{\n"
        ...
        self.assertIn("bool bMissileInbound = false;", compact_origin)
        self.assertNotIn("bMissileInbound = TEXT", compact_origin)
        self.assertNotIn("bMissileInbound = NAME_None", compact_origin)
        self.assertNotIn("bool bMissileInbound = true;", compact_origin)
        self.assertNotIn("bMissileInbound = 0.f", compact_origin)
        self.assertNotIn("bMissileInbound = 160.f", compact_origin)
        self.assertNotIn("bMissileInbound = 6;", compact_origin)
        self.assertNotIn(
            "bool bMissileInbound;",
            compact_origin.replace("bool bMissileInbound = false;", ""),
        )

    def test_missile_inbound_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bMissileInbound"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn(SIBLING_FLARE_COUNT_DECL, LOCKED_DECL)
        self.assertNotIn(TARGET_WRONG_THREAT_COUNT, LOCKED_DECL)
        self.assertNotIn(TARGET_WRONG_THREAT_LINE, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_HYDRA_DECL, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_CONTACT_LOCKED, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_THREAT_COUNT}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{LEFTOVER_HYDRA_DECL}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{LEFTOVER_CONTACT_LOCKED}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{SIBLING_FLARE_COUNT_DECL}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{LEFTOVER_HYDRA_DECL}\n",
            f"\t{LEFTOVER_KEEP_THERMAL_DECL}\n",
            f"\t{LEFTOVER_CONTACT_LOCKED}\n",
            f"\t{LEFTOVER_CONTACT_SEEKING}\n",
            f"\t{LEFTOVER_DEBRIEF_VALID}\n",
            f"\t{LEFTOVER_DEBRIEF_WON}\n",
            f"\t{LEFTOVER_DEBRIEF_MISSION_TITLE}\n",
            f"\t{LEFTOVER_DEBRIEF_OUTCOME}\n",
            f"\t{LEFTOVER_DEBRIEF_SCORE}\n",
            f"\t{LEFTOVER_CONTACT_LABEL}\n",
            "\tbool bMissileInbounds = false;\n",
            "\tbool bMissileInboundFlag = false;\n",
            "\tFString bMissileInbound;\n",
            "\tint32 bMissileInbound = false;\n",
            "\tfloat bMissileInbound = " + forty + ";\n",
            "\tfloat bMissileInbound = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    missile_inbound_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(missile_inbound_has_uproperty_wrap(origin), origin)
    ...
    def test_does_not_contract_sibling_hud_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        self.assertNotIn(SIBLING_THREAT_COUNT_DECL, LOCKED_DECL)
        self.assertNotIn(SIBLING_FLARE_COUNT_DECL, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_LOADOUT_FLARE_DECL, LOCKED_DECL)
        self.assertNotIn(TARGET_WRONG_THREAT_LINE, LOCKED_DECL)
        ...
        self.assertIn(LOCKED_DECL, section)
    ...
    def test_hud_missile_inbound_is_not_leftover_loadout_spec_flare_count(
        self,
    ) -> None:
        self.assertEqual(LOCKED_DECL, "bool bMissileInbound = false;")
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_LOADOUT_FLARE_DECL)
        self.assertNotEqual(LOCKED_DECL, SIBLING_FLARE_COUNT_DECL)
        self.assertNotIn("= 6", LOCKED_DECL)
        self.assertEqual(LEFTOVER_LOADOUT_FLARE_DECL, "int32 FlareCount = 6;")
        section = spec_section(origin_main_header())
        self.assertIn(LOCKED_DECL, section)
        self.assertNotIn(LEFTOVER_LOADOUT_FLARE_DECL, section)
        self.assertNotIn(LEFTOVER_LOADOUT_FLARE_DECL, LOCKED_DECL)
        ...
        leftover_section = spec_section(leftover)
        self.assertFalse(
            has_declaration(leftover_section, LOCKED_DECL),
            leftover_section,
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_section, LOCKED_DECL)
        self.assertIn("bMissileInbound", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
    ...
    def test_parse_source_is_cpg_hud_not_leftover_loadout_spec(self) -> None:
        self.assertEqual(HEADER_PATH, "Source/Skyguard52/SkyguardCpgHud.h")
        self.assertNotEqual(HEADER_PATH, LEFTOVER_LOADOUT_HEADER)
        self.assertNotIn("GunshipTypes", HEADER_PATH)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_LOADOUT_SPEC)
        header = origin_main_header()
        self.assertIn(STRUCT_NAME, header)
        self.assertIn(LOCKED_DECL, header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot missile-inbound field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "missile_inbound_field_decl_contract.py"
        ))
        self.assertIn("cpg_hud_snapshot", Path(__file__).name)
        self.assertNotIn("SkyguardCpgHud.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardCpgDebrief.h", THIS_SCRIPT)
        self.assertNotIn("threat_count", Path(__file__).name)
        self.assertNotIn("flare_count", Path(__file__).name)
        self.assertNotIn("loadout_spec", Path(__file__).name)
        self.assertNotIn("hydra_for_clusters", Path(__file__).name)
        ...
        self.assertIn(LEFTOVER_HUD_THREAT_COUNT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_LOCK_PHASE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_RANGE_METERS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_SIGHT_MODE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_STATION_STATUS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_HEADING_DEGREES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_LOCK_PROGRESS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_FLARE_COUNT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_THREAT_COUNT_DECL, locked_only)
        self.assertNotIn(SIBLING_FLARE_COUNT_DECL, locked_only)
        ...
        self.assertNotIn(LEFTOVER_HYDRA_DECL, locked_only)
        self.assertNotIn(LEFTOVER_CONTACT_LOCKED, locked_only)
        for field in leftover_neighbor_hit_fields():
        ...
        for decl in leftover_debrief_decls():
            self.assertNotIn(decl, locked_only)
        for decl in leftover_analog_bool_decls():
            self.assertNotIn(decl, locked_only)
