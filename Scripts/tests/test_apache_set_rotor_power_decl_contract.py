...
# or lock the SetRotorPower body in the .cpp.
...
# AimChinTurret sibling in this wave (do not create
# test_apache_aim_chin_turret_decl_contract.py). Stay
# off AimChinTurret, IssuePilotCommand,
...
# HandleDebriefKey, and leftover SortiePresentationWidgets
# stay sibling-only.
SET_ROTOR_POWER = ("void SetRotorPower(float NormalizedPower);")
UFUNCTION_APACHE = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Apache")'
)
# Leftover #56–#64 plus SkyguardApacheAircraft production
# files. This lane only adds an isolated Python
# SetRotorPower declaration contract on
# ASkyguardApacheAircraft. SortiePresentationWidgets is
# not the class under test. Stay off mount getters,
# GetChinMuzzleLocation, AimChinTurret, IssuePilotCommand,
# GetPilotCommand, GetPilotConfirmationsIssued,
# SetOrbitFocus, FaceWorldLocation, SetSensorView,
# SetFirstPersonInterior, SetDirectFlightInput,
# GetForwardSpeed, ApplyDamage, GetDamageFraction,
...
# leftover AimChinTurret sibling, and dirty workspace
# paths.
LOCKED = {
...
# apache-cpg-feel #8951, leftover AimChinTurret sibling
# in this wave, leftover DebriefWidget Configure /
...
# AimChinTurret, IssuePilotCommand, GetPilotCommand,
...
# .cpp SetRotorPower body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::SetRotorPower",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheSetRotorPowerDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_ROTOR_POWER), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_ROTOR_POWER)
        self.assertIn("SetRotorPower", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SET_ROTOR_POWER))

    def test_missing_set_rotor_power_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_ROTOR_POWER)
        self.assertIn("SetRotorPower", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_ROTOR_POWER)
        self.assertIn("SetRotorPower", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_ROTOR_POWER), section)
        self.assertNotIn("BlueprintPure", SET_ROTOR_POWER)
        self.assertNotIn("UFUNCTION", SET_ROTOR_POWER)
        self.assertNotIn("Category", SET_ROTOR_POWER)
        self.assertNotIn("BlueprintCallable", SET_ROTOR_POWER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_ROTOR_POWER)
        self.assertIn("SetRotorPower", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetRotorPower;\n"
        wrong_return_bool = "\tbool SetRotorPower(float NormalizedPower);\n"
        wrong_return_float = "\tfloat SetRotorPower(float NormalizedPower);\n"
        added_const = "\tvoid SetRotorPower(float NormalizedPower) const;\n"
        wrong_type = "\tvoid SetRotorPower(int NormalizedPower);\n"
        wrong_double = "\tvoid SetRotorPower(double NormalizedPower);\n"
        added_arg = (
            "\tvoid SetRotorPower(float NormalizedPower, float Extra);\n"
        )
        no_args = "\tvoid SetRotorPower();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_ROTOR_POWER)
            self.assertIn("SetRotorPower", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_rotor_power_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SET_ROTOR_POWER),
            SET_ROTOR_POWER,
        )
        self.assertTrue(has_declaration(section, SET_ROTOR_POWER))
        self.assertEqual(declaration_count(section, SET_ROTOR_POWER), 1)
        self.assertTrue(SET_ROTOR_POWER.startswith("void "), SET_ROTOR_POWER)
        self.assertTrue(SET_ROTOR_POWER.endswith(";"), SET_ROTOR_POWER)
        self.assertIn("SetRotorPower(", SET_ROTOR_POWER)
        self.assertIn("float NormalizedPower", SET_ROTOR_POWER)
        self.assertNotIn("INDEX_NONE", SET_ROTOR_POWER)
        self.assertNotIn("{", SET_ROTOR_POWER)
        self.assertNotIn("}", SET_ROTOR_POWER)
        self.assertNotIn("return ", SET_ROTOR_POWER)
        self.assertNotIn(" const", SET_ROTOR_POWER)
        self.assertNotIn("AimChinTurret", SET_ROTOR_POWER)
        self.assertNotIn("GetChinMuzzleLocation", SET_ROTOR_POWER)
        self.assertNotIn("GetChinTurret", SET_ROTOR_POWER)
        self.assertNotIn("GetRotorPowerScale", SET_ROTOR_POWER)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetRotorPower(float NormalizedPower);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, SET_ROTOR_POWER), 1)
        one_line = f"{{\npublic:\n\t{SET_ROTOR_POWER}\n}}\n"
        self.assertTrue(has_declaration(one_line, SET_ROTOR_POWER))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SET_ROTOR_POWER), section)
        self.assertEqual(
            require_declaration(section, SET_ROTOR_POWER),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_ROTOR_POWER}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_ROTOR_POWER)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_ROTOR_POWER)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_rotor_power_cpp_body(self) -> None:
        locked_only = f"{SET_ROTOR_POWER}\n"
        self.assertNotIn("{", SET_ROTOR_POWER)
        self.assertNotIn("}", SET_ROTOR_POWER)
        self.assertNotIn("return ", SET_ROTOR_POWER)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", SET_ROTOR_POWER)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", SET_ROTOR_POWER)
        self.assertNotIn("return true", SET_ROTOR_POWER)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{SET_ROTOR_POWER}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_ROTOR_POWER)
        self.assertNotIn("GetGunnerMount", SET_ROTOR_POWER)
        self.assertNotIn("GetEyeMount", SET_ROTOR_POWER)
        self.assertNotIn("GetWeaponMount", SET_ROTOR_POWER)
        self.assertNotIn("GetChinTurret", SET_ROTOR_POWER)
        self.assertNotIn("GetPilotMount", SET_ROTOR_POWER)
        self.assertNotIn("GetSensorTurret", SET_ROTOR_POWER)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{SET_ROTOR_POWER}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ROTOR_POWER)
        self.assertNotIn("GetChinMuzzleLocation", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_pilot_command_helpers(self) -> None:
        ...
        for neighbor in PILOT_COMMAND_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ROTOR_POWER)
        self.assertNotIn("IssuePilotCommand", SET_ROTOR_POWER)
        self.assertNotIn("GetPilotCommand", SET_ROTOR_POWER)
        self.assertNotIn("GetPilotConfirmationsIssued", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ROTOR_POWER)
        self.assertNotIn("SetOrbitFocus", SET_ROTOR_POWER)
        self.assertNotIn("FaceWorldLocation", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ROTOR_POWER)
        self.assertNotIn("SetSensorView", SET_ROTOR_POWER)
        self.assertNotIn("SetFirstPersonInterior", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ROTOR_POWER)
        self.assertNotIn("ApplyDamage", SET_ROTOR_POWER)
        self.assertNotIn("GetDamageFraction", SET_ROTOR_POWER)
        self.assertNotIn("ApplySystemHit", SET_ROTOR_POWER)
        self.assertNotIn("ApplyHit", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ROTOR_POWER)
        self.assertNotIn("IsSensorLive", SET_ROTOR_POWER)
        self.assertNotIn("IsThermalAvailable", SET_ROTOR_POWER)
        self.assertNotIn("IsSensorViewActive", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_leftover_chin_muzzle(self) -> None:
        ...
        for token in LEFTOVER_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_ROTOR_POWER)
        self.assertNotIn("GetChinMuzzleLocation", SET_ROTOR_POWER)
        self.assertNotIn("test_apache_chin_muzzle_tests.py", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_ROTOR_POWER)
        self.assertNotIn("ESkyguardApacheSystem", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_leftover_cpg_feel(self) -> None:
        ...
        for token in LEFTOVER_CPG_FEEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_ROTOR_POWER)
        self.assertNotIn("test_apache_cpg_feel_contract.py", SET_ROTOR_POWER)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_ROTOR_POWER)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", SET_ROTOR_POWER)
        self.assertNotIn("USkyguardBriefingWidget", SET_ROTOR_POWER)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            SET_ROTOR_POWER,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_ROTOR_POWER)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_ROTOR_POWER)
        self.assertNotIn("ESkyguardMissionSkylineStyle", SET_ROTOR_POWER)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_ROTOR_POWER}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_ROTOR_POWER),
            SET_ROTOR_POWER,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardApacheAircraft::SetRotorPower", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SetRotorPower contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, SET_ROTOR_POWER.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache SetRotorPower contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_ROTOR_POWER)

    def test_contract_is_set_rotor_power_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", SET_ROTOR_POWER)
        self.assertNotIn("{", SET_ROTOR_POWER)
        self.assertTrue(SET_ROTOR_POWER.startswith("void "))
        self.assertTrue(SET_ROTOR_POWER.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
