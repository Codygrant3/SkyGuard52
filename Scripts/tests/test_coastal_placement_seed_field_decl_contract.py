# THIS IS leftover-safe ASkyguardCoastalEnvironmentDirector PlacementSeed.
...
# then `int32 PlacementSeed = 5201;` (HAS initializer).
...
# BlueprintReadWrite IS present. Default IS `= 5201`.
# This is a unique coastal director vegetation seed int32.
...
# Stop BEFORE sibling EpicTreeBudget as a claimed slot.
# THIS IS leftover-safe because it is the coastal director ACTOR
# vegetation seed int32, not leftover
# FSkyguardEnvironmentReadiness TreeInstanceCount / ShrubInstanceCount,
# not leftover sibling EpicTreeBudget / EpicShrubBudget /
# TreeInstances / ShrubInstances / WindSource,
# not leftover analog coastal-env-director-empty-fail-closed bulk,
# not leftover analog Root / VFXPool / Readiness / RouteLengthCm /
...
# Clone wrap was VisibleAnywhere BlueprintReadOnly Category
# Skyguard|Theater wrapping FName WeatherIdentity with NO
# initializer — RETARGET hard.
# Better same-wrap clone is EditAnywhere BlueprintReadWrite
# wrapping `float LookYawLimit = 95.f;` Category Skyguard|Combat
# — RETARGET hard to Category Skyguard|Environment|Vegetation
# wrapping `int32 PlacementSeed = 5201;`.
# Fail-closed if this test still asserts FName WeatherIdentity /
# Category="Skyguard|Theater" / 160.f / FireRate = 12.0f /
# RecoilPitch = 0.92f AS THE LOCKED DECL.
...
# leftover readiness bLicensedVegetationApproved /
# bPCGGenerationAuthorized / bAuthoredLandscapeSurfaceExposed /
# bReadyForAuthoredPCGGeneration / leftover TreeInstances /
# ShrubInstances / WindSource TObjectPtr.
# Harbor leftover is only the split 40 / 80 float tokens.
# PlacementSeed is 5201 not Harbor leftover float tokens.
...
TARGET = "int32 PlacementSeed = 5201;"
TARGET_WRONG_BARE = "int32 PlacementSeed;"
TARGET_WRONG_INIT = "int32 PlacementSeed = nullptr;"
TARGET_WRONG_FALSE = "int32 PlacementSeed = false;"
TARGET_WRONG_TRUE = "int32 PlacementSeed = true;"
TARGET_WRONG_ZERO = "int32 PlacementSeed = 0;"
TARGET_WRONG_FLOAT = "float PlacementSeed = 5201.f;"
TARGET_WRONG_TREE = (
...
TARGET_WRONG_QUALITY = (
    "ESkyguardEnvironmentQuality Quality = "
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 PlacementSeed = 5201;`.
    # Do not accept leftover FName WeatherIdentity,
    # leftover `= 160.f`, leftover FireRate / RecoilPitch,
    # leftover TreeInstances / ShrubInstances / WindSource,
    # leftover Quality / EpicTreeBudget, leftover
    # Mission01 OceanTiles, leftover readiness bools,
    # leftover TreeInstanceCount, leftover LookYawLimit,
    # leftover Harbor 40 / 80, or a bare PlacementSeed
    # without the 5201 initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SEED_DECL_RE.search(compact) is None:
        return False
    if SEED_BARE_RE.search(compact):
        return False
    ...
    if RECOIL_PITCH_DECL_RE.search(compact):
        return False
    if WRONG_SHRUB_RE.search(compact):
        return False
    ...
    if WRONG_WIND_RE.search(compact):
        return False
    if WRONG_ROOT_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"\bTreeInstances\b", compact) and "PlacementSeed" not in compact:
        return False
    if "TreeInstanceCount" in compact:
        return False
    if "bLicensedVegetationApproved" in compact:
        return False
    if "bPCGGenerationAuthorized" in compact:
        return False
    if "bAuthoredLandscapeSurfaceExposed" in compact:
        return False
    if "bReadyForAuthoredPCGGeneration" in compact:
        return False
    return True
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == SIBLING_EPIC_TREE:
            break
        if ident != "PlacementSeed":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, SIBLING_EPIC_TREE):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+PlacementSeed\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 PlacementSeed "
...
class CoastalPlacementSeedFieldDeclContractTests(unittest.TestCase):
    def test_coastal_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PlacementSeed"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_EPIC_TREE), body)
        self.assertFalse(has_identifier(section, SIBLING_EPIC_TREE), section)
        self.assertFalse(has_identifier(section, SIBLING_TREE), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "PlacementSeed"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("PlacementSeed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_placement_seed_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 PlacementSeed = 5201;", compact_origin)
        self.assertNotIn("PlacementSeed = 160.f", compact_origin)
    ...
    def test_placement_seed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertNotIn("bReadyForAuthoredPCGGeneration", LOCKED_DECL)
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 PlacementSeed = 5201;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_QUALITY}\n",
            f"\t{TARGET_WRONG_EPIC_TREE}\n",
            f"\t{TARGET_WRONG_LICENSED_APPROVED}\n",
            f"\t{TARGET_WRONG_PCG_AUTH}\n",
            f"\t{TARGET_WRONG_AUTHORED_SURFACE}\n",
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 PlacementSeeds = 5201;\n",
            "\tfloat PlacementSeed = 5201.f;\n",
            "\tbool PlacementSeed = true;\n",
            "\tint32 PlacementSeed = " + forty + ";\n",
            "\tint32 PlacementSeed = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_epic_tree_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "PlacementSeed"))
        self.assertFalse(has_identifier(section, SIBLING_TREE))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_EPIC_TREE))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn(
            'Category="Skyguard"',
            UPROPERTY_EDIT_READWRITE.replace(
                'Category="Skyguard|Environment|Vegetation"',
                "",
            ),
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "coastal PlacementSeed field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "coastal_placement_seed_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCoastalEnvironmentDirector.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("Quality", locked_only)
        self.assertNotIn("WindStrength", locked_only)
        self.assertNotIn("WindSpeed", locked_only)
        self.assertNotIn("RouteLengthCm", locked_only)
        self.assertNotIn("AppliedWeather", locked_only)
