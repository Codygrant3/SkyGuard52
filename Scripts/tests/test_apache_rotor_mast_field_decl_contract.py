# THIS IS leftover-safe ASkyguardApacheAircraft RotorMast.
...
# then BARE `TObjectPtr<UStaticMeshComponent> RotorMast;`
# (no initializer).
...
# Stop BEFORE sibling MainRotor as a claimed slot.
# Do NOT claim MainRotor / MainRotorCross / TailRotor /
# RotorPower / StubWingRight / StubWingLeft /
# VerticalTail / TailBoom / Fuselage / Nose / Canopy /
# SilhouetteMesh / AircraftRoot / HoverBobCentimeters /
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> RotorMast;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> RotorMast;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> RotorMast = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> RotorMast = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> RotorMast = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> RotorMast;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> RotorMast;"
...
STOP_BEFORE_MAIN_ROTOR = "MainRotor"
...
ROTOR_MAST_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+RotorMast\s*;"
)
ROTOR_MAST_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+RotorMast\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer on RotorMast as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ROTOR_MAST_DECL_RE.search(compact) is None:
        return False
    if ROTOR_MAST_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_MAIN_ROTOR):
            break
    ...
    if has_identifier(window, SIBLING_MAIN_ROTOR):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_MAIN_ROTOR}"
        )
    for later in (
        SIBLING_MAIN_ROTOR_CROSS,
        SIBLING_HORIZONTAL_TAIL,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+RotorMast\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> RotorMast "
...
class ApacheRotorMastFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RotorMast"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_MAIN_ROTOR), body)
        self.assertFalse(
            has_identifier(section, SIBLING_MAIN_ROTOR),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_TAIL_ROTOR), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RotorMast"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("RotorMast", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_rotor_mast_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> RotorMast;", compact_origin)
        self.assertNotIn("RotorMast = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_rotor_mast_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> RotorMast"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> RotorMast;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> RotorMasts;\n",
            "\tint32 RotorMast = 140;\n",
            "\tbool RotorMast = true;\n",
            "\tfloat RotorMast = " + forty + ";\n",
            "\tfloat RotorMast = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_main_rotor_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_MAIN_ROTOR))
        ...
        self.assertNotIn(TARGET_WRONG_MAIN_ROTOR, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache RotorMast field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "rotor_mast_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
