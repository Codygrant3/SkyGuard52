# THIS IS leftover-safe ASkyguardApacheAircraft HorizontalTail.
...
# then BARE `TObjectPtr<UStaticMeshComponent> HorizontalTail;`
# (no initializer).
...
# Stop BEFORE sibling GearNose as a claimed slot.
# Do NOT claim sibling VerticalTail / TailBoom / TailRotor /
# MainRotorCross / RadarDome / GearNose / GearLeft / GearRight /
# Canopy / Fuselage / AircraftRoot / SilhouetteMesh.
# THIS IS leftover-safe ASkyguardApacheAircraft.HorizontalTail.
# It is NOT leftover VerticalTail #1563, NOT leftover TailBoom
# #1561, NOT leftover TailRotor #1568, NOT leftover analog
# apache-aircraft-empty-fail-closed #851b.
# Type is `TObjectPtr<UStaticMeshComponent>`.
# Do NOT parse leftover analog apache-aircraft-empty-fail-closed
# #851b (keep bulk in LOCKED_SCRIPTS).
# Do NOT parse leftover Apache UFUNCTION decls #411-#426,
# leftover analog apache-set-rotor-power / apache-get-rotor-rpm /
# apache-aim-chin-turret / apache-set-sensor-view /
# apache-is-canopy-glass-cracked UFUNCTION.
# Do NOT parse leftover analog apache-hull-collider,
# leftover analog apache-mount-fail-closed, leftover analog
# apache-cpg-feel, leftover ProtectAsset #1296/#1297,
# leftover LoadoutSpec HullIntegrity #1475, leftover ArcadeLook
# UPROPERTY, leftover GuidedLockRules methods, leftover
# CampaignMissionSpec, leftover Harbor Breaker
# Approach/Contact/Shore, leftover campaign-roster
# function decls / analog #85ab.
# Already-opened leftover Apache fields stay leftover:
# RadarDome, MainRotorCross, VerticalTail, TailBoom, TailRotor,
# GearNose, Canopy, Fuselage, AircraftRoot, SilhouetteMesh,
# HoverBobCentimeters, RotorPower, MaxIntegrity,
# CurrentIntegrity.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> HorizontalTail;`
# NOT `FName WeatherIdentity;`, NOT leftover sibling
# `TObjectPtr<UStaticMeshComponent> VerticalTail;`, NOT leftover
# `TObjectPtr<UStaticMeshComponent> TailBoom;`, NOT leftover
# `TObjectPtr<UStaticMeshComponent> TailRotor;`, NOT leftover
# `TObjectPtr<UStaticMeshComponent> MainRotorCross;`, NOT leftover
# `TObjectPtr<UStaticMeshComponent> GearNose;`, and NOT
# `TObjectPtr<USceneComponent>`.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# / TObjectPtr<USceneComponent> / leftover VerticalTail /
# leftover TailBoom / leftover TailRotor /
# leftover MainRotorCross / leftover GearNose AS THE LOCKED DECL.
...
TARGET = "TObjectPtr<UStaticMeshComponent> HorizontalTail;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> HorizontalTail = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> HorizontalTail = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> HorizontalTail = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> HorizontalTail;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> HorizontalTail;"
...
STOP_BEFORE_GEAR_NOSE = "GearNose"
...
HORIZONTAL_TAIL_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HorizontalTail\s*;"
)
HORIZONTAL_TAIL_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HorizontalTail\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover sibling VerticalTail / TailBoom / TailRotor /
    # MainRotorCross / RadarDome / GearNose / Fuselage /
    # AircraftRoot / USceneComponent, or an initializer on
    # HorizontalTail as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if HORIZONTAL_TAIL_DECL_RE.search(compact) is None:
        return False
    if HORIZONTAL_TAIL_INIT_RE.search(compact):
        return False
    if WEATHER_IDENTITY_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"TObjectPtr<USceneComponent>\s+HorizontalTail\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_GEAR_NOSE):
            break
    ...
    if has_identifier(window, SIBLING_GEAR_NOSE):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+HorizontalTail\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> HorizontalTail "
...
class ApacheHorizontalTailFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HorizontalTail"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_MAIN_ROTOR_CROSS), body)
        self.assertTrue(
            has_identifier(section, SIBLING_MAIN_ROTOR_CROSS),
            section,
        )
        ...
        self.assertTrue(has_identifier(section, SIBLING_TAIL_BOOM), section)
        ...
        self.assertTrue(has_identifier(section, SIBLING_ENGINE_RIGHT), section)
        ...
        self.assertTrue(has_identifier(section, SIBLING_CANOPY), section)
        self.assertTrue(has_identifier(section, SIBLING_NIGHT_VISION_TURRET), section)
        self.assertTrue(has_identifier(section, SIBLING_SILHOUETTE_MESH), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HorizontalTail"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("HorizontalTail", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_horizontal_tail_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> HorizontalTail;", compact_origin)
        self.assertNotIn("HorizontalTail = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_horizontal_tail_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> HorizontalTail"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_FUSELAGE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_AIRCRAFT_ROOT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_CANOPY))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_ENGINE_LEFT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_ENGINE_RIGHT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_MAIN_ROTOR_CROSS))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_RADAR_DOME))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_VERTICAL_TAIL))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_TAIL_BOOM))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_TAIL_ROTOR))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_GEAR_NOSE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_PILOT_CANOPY))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SILHOUETTE_MESH))
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> HorizontalTail;")
        self.assertNotEqual(LOCKED_DECL, "FName WeatherIdentity;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> Canopy;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> VerticalTail;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> TailBoom;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> TailRotor;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> MainRotorCross;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> RadarDome;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> GearNose;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<USceneComponent> HorizontalTail;")
        self.assertNotEqual(LOCKED_DECL, "FName WeatherIdentity;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAIN_ROTOR_CROSS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RADAR_DOME)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VERTICAL_TAIL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TAIL_BOOM)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TAIL_ROTOR)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> HorizontalTails;\n",
            "\tint32 HorizontalTail = 140;\n",
            "\tbool HorizontalTail = true;\n",
            "\tfloat HorizontalTail = " + forty + ";\n",
            "\tfloat HorizontalTail = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_gear_nose_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, SIBLING_CANOPY))
        ...
        self.assertTrue(has_identifier(section, SIBLING_NIGHT_VISION_TURRET))
        ...
        self.assertTrue(has_identifier(section, SIBLING_TAIL_BOOM))
        self.assertTrue(has_identifier(section, SIBLING_TAIL_ROTOR))
        self.assertFalse(has_identifier(section, SIBLING_GEAR_NOSE))
        ...
        self.assertNotIn(TARGET_WRONG_GEAR_LEFT, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_ENGINE_LEFT))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_RIGHT))
        self.assertTrue(has_identifier(section, SIBLING_ENGINE_RIGHT))
        self.assertTrue(has_identifier(leaked, SIBLING_RADAR_DOME))
        self.assertTrue(has_identifier(section, SIBLING_RADAR_DOME))
        self.assertTrue(has_identifier(leaked, "HorizontalTail"))
        self.assertTrue(has_identifier(section, "HorizontalTail"))
        self.assertTrue(has_identifier(leaked, SIBLING_CANOPY))
        self.assertTrue(has_identifier(section, SIBLING_CANOPY))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache HorizontalTail field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "horizontal_tail_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_FUSELAGE))
        self.assertFalse(has_identifier(locked_only, SIBLING_AIRCRAFT_ROOT))
        self.assertFalse(has_identifier(locked_only, SIBLING_CANOPY))
        self.assertFalse(has_identifier(locked_only, SIBLING_ENGINE_LEFT))
        self.assertFalse(has_identifier(locked_only, SIBLING_ENGINE_RIGHT))
        self.assertFalse(has_identifier(locked_only, SIBLING_MAIN_ROTOR_CROSS))
        self.assertFalse(has_identifier(locked_only, SIBLING_PILOT_CANOPY))
        self.assertFalse(has_identifier(locked_only, SIBLING_SILHOUETTE_MESH))
