# THIS IS leftover-safe USkyguardArcadeLookComponent
# ChromaticAberration.
...
# then `float ChromaticAberration = 0.35f;`.
...
# Identifier IS ChromaticAberration. Initializer IS `= 0.35f`.
...
# type is not float, if identifier is not
# ChromaticAberration, or if initializer is not
# `= 0.35f`.
...
# bEnabled / Contrast / Saturation / Gain /
# Gamma / BloomIntensity / Vignette / Grain
# as this slot.
...
# GuidedLockConeDegrees.
# ChromaticAberration = 0.35f is NOT leftover analog
# GuidedLockRules HelmetLockSeconds = 0.35f.
# Lock the FULL decl including identifier
# ChromaticAberration.
# Fail-closed if HelmetLockSeconds /
# CannonRecoilPitch / SkyguardApacheCpgFeel is
# treated as this slot.
# Do NOT parse leftover GuidedLockRules
# DetectProgressEnd / HelmetLockSeconds /
...
# RETARGET: type is float, identifier is
# ChromaticAberration, initializer is 0.35f,
# UPROPERTY is EditAnywhere BlueprintReadWrite
...
# FName WeatherIdentity / no initializer / leftover
# TheaterKit 160.f as the locked decl.
# Better same-class retarget leftover-safe ArcadeLook
# bEnabled / Contrast / Gamma. Copy that parse window.
# Do NOT claim bEnabled or Contrast or Gamma as this slot.
...
GUNSHIP_TYPES_HEADER = "Source/Skyguard52/SkyguardGunshipTypes.h"
APACHE_CPG_FEEL_NS = "SkyguardApacheCpgFeel"
TARGET = "float ChromaticAberration = 0.35f;"
TARGET_WRONG_BARE = "float ChromaticAberration;"
TARGET_WRONG_FALSE = "float ChromaticAberration = false;"
TARGET_WRONG_TRUE = "float ChromaticAberration = true;"
TARGET_WRONG_ZERO = "float ChromaticAberration = 0.f;"
TARGET_WRONG_ONE = "float ChromaticAberration = 1.f;"
TARGET_WRONG_NO_F = "float ChromaticAberration = 0.35;"
TARGET_WRONG_FNAME = "FName ChromaticAberration;"
TARGET_WRONG_INT = "int32 ChromaticAberration = 0;"
TARGET_WRONG_BOOL = "bool ChromaticAberration = true;"
...
TARGET_WRONG_CANNON_RATE = "float ChromaticAberration = 12.0f;"
TARGET_WRONG_CANNON_RECOIL = "float CannonRecoilPitch = 0.92f;"
TARGET_WRONG_CANNON_DAMAGE = "float ChromaticAberration = 22.0f;"
TARGET_WRONG_TIME_OF_DAY = "float ChromaticAberration = 12.f;"
TARGET_WRONG_SENSOR_ACQUIRE = "float ChromaticAberration = 5.5f;"
TARGET_WRONG_DETECT_PROGRESS = "float ChromaticAberration = 0.22f;"
TARGET_WRONG_CLOUD = "float ChromaticAberration = 0.25f;"
TARGET_WRONG_CONTRAST = "float Contrast = 1.18f;"
TARGET_WRONG_ENABLED = "bool bEnabled = true;"
TARGET_WRONG_GAMMA = "float Gamma = 0.92f;"
TARGET_WRONG_BLOOM = "float BloomIntensity = 0.55f;"
TARGET_WRONG_VIGNETTE = "float Vignette = 0.42f;"
TARGET_WRONG_GRAIN = "float Grain = 0.08f;"
TARGET_WRONG_SATURATION = "float Saturation = 1.12f;"
TARGET_WRONG_GAIN = "float Gain = 1.08f;"
TARGET_WRONG_HELMET_LOCK = "float HelmetLockSeconds = 0.35f;"
TARGET_WRONG_GUIDED_CONE = "float GuidedLockConeDegrees = 6.0f;"
...
STOP_BEFORE_BLOOM = "float BloomIntensity = 0.55f;"
STOP_BEFORE_GAMMA = "float Gamma = 0.92f;"
STOP_BEFORE_VIGNETTE = "float Vignette = 0.42f;"
STOP_BEFORE_GRAIN = "float Grain = 0.08f;"
...
LEFTOVER_GUIDED_LOCK_CONE = "GuidedLockConeDegrees"
...
LEFTOVER_DETECT_PROGRESS_END = "DetectProgressEnd"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Lock the FULL decl including identifier
    # ChromaticAberration.
    # Do not accept leftover analog GuidedLockRules
    # `float HelmetLockSeconds = 0.35f;` just because
    # the initializer is also 0.35f.
    # Do not accept leftover analog apache-cpg-feel
    # `float CannonRecoilPitch = 0.92f;` /
    # `float GuidedLockConeDegrees = 6.0f;`.
    # Do not accept bare `float ChromaticAberration;` /
    # leftover TheaterKit WeatherIdentity / leftover
    # 160.f Health / leftover apache-cpg-feel
    # CannonFireRate 12.0f / CannonDamage 22.0f /
    # leftover Contrast 1.18f / leftover Gamma 0.92f /
    # leftover TimeOfDayHours 12.f / leftover
    # DetectProgressEnd 0.22f / leftover
    # SensorAcquireDegrees 5.5f / leftover CloudCoverage
    # 0.25f when origin/main has ChromaticAberration
    # = 0.35f.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+ChromaticAberration\s*=\s*0\.35f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+HelmetLockSeconds\s*=\s*0\.35f\s*;",
        compact,
    ) and re.search(
        r"float\s+ChromaticAberration\s*=\s*0\.35f\s*;",
        compact,
        ...
        return False
    if re.search(
        r"float\s+CannonRecoilPitch\s*=\s*0\.92f\s*;",
        compact,
    ) and re.search(
        r"float\s+ChromaticAberration\s*=\s*0\.35f\s*;",
        compact,
        ...
        return False
    if APACHE_CPG_FEEL_NS in compact and not has_identifier(
        compact,
        "ChromaticAberration",
    ):
        return False
    if re.search(
        r"FName\s+ChromaticAberration\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+ChromaticAberration\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+ChromaticAberration\b",
        compact,
    ):
        return False
    if re.search(
    ...
    if re.search(
        ...
        return False
    if re.search(
        r"float\s+ChromaticAberration\s*;",
        compact,
    ):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*float\s+ChromaticAberration\s*=\s*0\.35f\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if LEFTOVER_CANNON_RECOIL_PITCH in window:
                raise AssertionError(
                    "claimed window includes leftover "
                    "CannonRecoilPitch"
                )
            if LEFTOVER_HELMET_LOCK_SECONDS in window:
                raise AssertionError(
                    "claimed window includes leftover "
                    "HelmetLockSeconds"
                )
            if APACHE_CPG_FEEL_NS in window:
                raise AssertionError(
                    "claimed window includes leftover "
    ...
    raise AssertionError(
        "UPROPERTY for float ChromaticAberration = 0.35f; is missing from "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+ChromaticAberration\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float ChromaticAberration is missing from "
        f"origin/main:{HEADER_PATH} class {CLASS_NAME} public "
        "UPROPERTY fields"
    )
...
class ArcadeLookChromaticAberrationFieldDeclContractTests(unittest.TestCase):
    def test_arcade_look_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, APACHE_CPG_FEEL_NS)
        body = class_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "ChromaticAberration"), section)
        ...
        self.assertNotIn(APACHE_CPG_FEEL_NS, section)
        self.assertNotIn(LEFTOVER_CANNON_RECOIL_PITCH, section)
        for sibling in sibling_uncontracted_decls():
        ...
        self.assertNotIn(SIBLING_BLOOM, claimed)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ChromaticAberration"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ChromaticAberration", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_chromatic_aberration_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn("CannonRecoilPitch", str(raised.exception))
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float ChromaticAberration = 0.35f;", compact_origin)
        self.assertNotIn("ChromaticAberration = NAME_None", compact_origin)
        self.assertNotIn("ChromaticAberration = false", compact_origin)
        self.assertNotIn("ChromaticAberration = true", compact_origin)
        self.assertNotIn("ChromaticAberration = 0.f", compact_origin)
        self.assertNotIn("ChromaticAberration = 160.f", compact_origin)
        self.assertNotIn("ChromaticAberration = 12.0f", compact_origin)
        self.assertNotIn("ChromaticAberration = 22.0f", compact_origin)
        self.assertNotIn("ChromaticAberration = 1.18f", compact_origin)
        self.assertNotIn("ChromaticAberration = 0.92f", compact_origin)
        self.assertNotIn("ChromaticAberration = 0.22f", compact_origin)

    def test_chromatic_aberration_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float ChromaticAberration"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 0.35f", LOCKED_DECL)
        self.assertIn("ChromaticAberration", LOCKED_DECL)
        ...
        self.assertNotIn(APACHE_CPG_FEEL_NS, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ENABLED}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_GAMMA}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HELMET_LOCK}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 0.35f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 0.35f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            f"\t{TARGET_WRONG_DETECT_PROGRESS}\n",
            ...
            f"\t{TARGET_WRONG_BLOOM}\n",
            f"\t{TARGET_WRONG_VIGNETTE}\n",
            f"\t{TARGET_WRONG_GRAIN}\n",
            f"\t{TARGET_WRONG_HELMET_LOCK}\n",
            f"\t{TARGET_WRONG_GUIDED_CONE}\n",
            "\tfloat ChromaticAberrations = 0.35f;\n",
            "\tint32 ChromaticAberration = 1;\n",
            "\tbool ChromaticAberration = true;\n",
            "\tfloat ChromaticAberration = " + forty + ";\n",
            "\tfloat ChromaticAberration = " + eighty + ";\n",
        )
    ...
    def test_enabled_contrast_and_gamma_siblings_are_not_this_slot(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENABLED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CONTRAST)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GAMMA)
        self.assertNotIn(SIBLING_ENABLED, LOCKED_DECL)
        self.assertNotIn(SIBLING_CONTRAST, LOCKED_DECL)
        self.assertNotIn(SIBLING_GAMMA, LOCKED_DECL)
        self.assertNotEqual(THIS_SCRIPT, SIBLING_ENABLED_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, SIBLING_CONTRAST_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, SIBLING_GAMMA_SCRIPT)
        section = spec_section(origin_main_header())
        ...
        self.assertTrue(has_identifier(section, SIBLING_GAMMA), section)
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ENABLED}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_ENABLED}\n", LOCKED_DECL)
        self.assertIn("ChromaticAberration", str(raised.exception))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_CONTRAST}\n", LOCKED_DECL)
        self.assertIn("ChromaticAberration", str(raised.exception))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_GAMMA}\n", LOCKED_DECL)
        self.assertIn("ChromaticAberration", str(raised.exception))

    def test_claimed_window_excludes_sibling_grade_fields(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("ChromaticAberration", claimed)
        self.assertIn("= 0.35f", claimed)
        ...
        self.assertNotIn(SIBLING_GAMMA, claimed)
        self.assertNotIn(SIBLING_BLOOM, claimed)
        self.assertNotIn(STOP_BEFORE_BLOOM, claimed)
        ...
        self.assertNotIn(LEFTOVER_CANNON_RECOIL_PITCH, claimed)
        mixed = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn(SIBLING_BLOOM, window)
    ...
    def test_does_not_parse_neighbors(self) -> None:
        ...
        self.assertNotIn(APACHE_CPG_FEEL_NS, header)
        self.assertNotIn(GUNSHIP_TYPES_HEADER, header)
        ...
        self.assertNotIn(APACHE_CPG_FEEL_NS, leaked)
        self.assertNotIn(LEFTOVER_CANNON_RECOIL_PITCH, leaked)
    ...
    def test_leftover_analog_feel_initializers_do_not_satisfy(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CANNON_RECOIL)
        self.assertNotEqual(LOCKED_DECL, "float CannonDamage = 22.0f;")
        self.assertEqual(LOCKED_DECL, "float ChromaticAberration = 0.35f;")
        self.assertIn("ChromaticAberration", LOCKED_DECL)
        self.assertNotIn("CannonRecoilPitch", LOCKED_DECL)
        self.assertNotIn(APACHE_CPG_FEEL_NS, LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GUIDED_CONE)
        feel = (
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_GUIDED_CONE}\n", LOCKED_DECL)
        )
        namespaced = (
            "namespace "
            + APACHE_CPG_FEEL_NS
            + "\n{\n\tconstexpr "
            + TARGET_WRONG_CANNON_RECOIL
            + "\n}\n"
        )
        self.assertFalse(has_declaration(namespaced, LOCKED_DECL))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(namespaced, LOCKED_DECL)
        self.assertIn("ChromaticAberration", str(raised.exception))
    ...
    def test_leftover_guided_lock_degrees_do_not_satisfy(self) -> None:
        ...
        guided = (
            ...
            f"\tfloat {LEFTOVER_HELMET_ACQUIRE_DEGREES} = 12.0f;\n"
            f"\tfloat {LEFTOVER_SENSOR_LOCK_SECONDS} = 0.55f;\n"
            f"\tfloat {LEFTOVER_SENSOR_ACQUIRE_DEGREES} = 5.5f;\n"
            f"\tfloat {LEFTOVER_DETECT_PROGRESS_END} = 0.22f;\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\tstatic void {STOP_BEFORE_APPLY_WORLD_MOOD_WEATHER}();\n"
            f"\tbool {SIBLING_ENABLED} = true;\n"
            f"\tfloat {SIBLING_CONTRAST} = 1.18f;\n"
            f"\tfloat {SIBLING_SATURATION} = 1.12f;\n"
            f"\t{TARGET_WRONG_GAMMA}\n"
            f"\t{TARGET_WRONG_HELMET_LOCK}\n"
            f"\t{TARGET_WRONG_CANNON_RECOIL}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "arcade-look ChromaticAberration field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "chromatic_aberration_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardArcadeLookComponent.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardGuidedLockRules.h", THIS_SCRIPT)
        ...
        self.assertIn(SIBLING_GAMMA_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_header_path_is_arcade_look_component_only(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENABLED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GAMMA)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HELMET_LOCK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEATHER_IDENTITY)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(APACHE_CPG_FEEL_NS, locked_only)
        self.assertNotIn(LEFTOVER_CANNON_RECOIL_PITCH, locked_only)
        self.assertNotIn(LEFTOVER_GUIDED_LOCK_CONE, locked_only)
        self.assertNotIn(SIBLING_CONTRAST, claimed)
        self.assertNotIn(SIBLING_ENABLED, claimed)
        self.assertNotIn(SIBLING_GAMMA, claimed)
        self.assertNotIn(SIBLING_BLOOM, claimed)
