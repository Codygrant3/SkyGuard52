# THIS IS leftover-safe FSkyguardStormRainBeatKit Title.
...
# Fail-closed if this lane claims leftover WeatherLabel
# instead of Title. Same type and TEXT("") must not steal
# the sibling label.
...
# bare `FName MissionId;`, retarget: type is const TCHAR*,
# identifier is Title, initializer is TEXT(""), locked
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept missing TEXT(""), WeatherLabel, FName Title,
    # `= NAME_None` / `= false` / `= true` / `= 0.f` /
    # `= 160.f` when origin/main locks TEXT(""). Do not
    # accept sibling WeatherLabel as this Title decl.
...
class StormRainBeatKitTitleFieldDeclContractTests(unittest.TestCase):
    ...
    def test_weather_label_does_not_satisfy_title(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_LABEL}\n"
