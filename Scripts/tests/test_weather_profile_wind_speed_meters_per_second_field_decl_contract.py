# THIS IS leftover-safe FSkyguardWeatherProfile WindSpeedMetersPerSecond.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "0.0"))
# then `float WindSpeedMetersPerSecond = 5.f;` (initializer IS `5.f`,
# not `5.0f` / `5.0` / `5` / Harbor 40/80 / `0.f`).
# THIS IS leftover-safe isolated UPROPERTY with no Category.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# ClampMin = "0.0" IS present. Initializer IS `= 5.f`.
...
# if the initializer is not `= 5.f`, if meta ClampMin
...
# Parse STRUCT `FSkyguardWeatherProfile` body ONLY after
# `struct FSkyguardWeatherProfile`. Stop at
# `struct FSkyguardMissionPresentation` and later structs
...
# (leftover mission-definition Weather #357).
...
# Do NOT contract sibling fields ProfileId /
# Weather / TimeOfDayHours / Precipitation /
# CloudCoverage.
...
# #1332-#1337 or leftover #1332-#1350.
# THIS IS NOT leftover ProfileId draft #1350.
...
# the bulk).
# Analog: leftover weather-profile-defaults then this
# isolated WindSpeedMetersPerSecond field decl, same as leftover
# route-point-defaults then isolated PointId,
...
# `FName ProfileId` / no `=` / ClampMin missing,
# retarget: type is float WindSpeedMetersPerSecond,
# initializer is `= 5.f`, specifiers are EditAnywhere +
# BlueprintReadOnly + meta = (ClampMin = "0.0"),
# no Category.
...
STRUCT_NAME = "FSkyguardWeatherProfile"
...
TARGET = "float WindSpeedMetersPerSecond = 5.f;"
TARGET_WRONG_EQ_NONE = "float WindSpeedMetersPerSecond = 5.0f;"
TARGET_WRONG_FALSE = "float WindSpeedMetersPerSecond = false;"
TARGET_WRONG_TRUE = "float WindSpeedMetersPerSecond = true;"
TARGET_WRONG_ZERO = "float WindSpeedMetersPerSecond = 0.f;"
TARGET_WRONG_FLOAT = "int32 WindSpeedMetersPerSecond = 5;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "float WindSpeedMetersPerSecond;"
TARGET_WRONG_FIVE = "float WindSpeedMetersPerSecond = 5;"
TARGET_WRONG_FIVE_POINT_ZERO = (
    "float WindSpeedMetersPerSecond = 5.0;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Reject missing initializer, `5.0f` / `5.0` / `5` /
    # Harbor 40/80 / `0.f` / leftover Health. Do not accept
    # sibling ProfileId / Weather / TimeOfDayHours /
    # Precipitation / CloudCoverage fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+WindSpeedMetersPerSecond\s*=\s*5\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+WindSpeedMetersPerSecond\s*=\s*5\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+WindSpeedMetersPerSecond\s*=\s*5\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+WindSpeedMetersPerSecond\s*=\s*5\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+WindSpeedMetersPerSecond\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+WindSpeedMetersPerSecond\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+WindSpeedMetersPerSecond\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+WindSpeedMetersPerSecond\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float WindSpeedMetersPerSecond is missing from "
...
def require_wind_speed_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            "WindSpeedMetersPerSecond UPROPERTY"
        )
    ...
    if 'ClampMin = "0.0"' not in specifiers:
        raise AssertionError(
            'ClampMin = "0.0" is missing or wrong on origin/main:'
            f"{HEADER_PATH} struct {STRUCT_NAME} "
            "WindSpeedMetersPerSecond UPROPERTY"
        )
...
class WeatherProfileWindSpeedMetersPerSecondFieldDeclContractTests(unittest.TestCase):
    def test_weather_profile_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "WindSpeedMetersPerSecond"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "WindSpeedMetersPerSecond"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("WindSpeedMetersPerSecond", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_wind_speed_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_wind_speed_specifiers(section)
    ...
    def test_initializer_fails_closed_when_not_five_f(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
        ...
        self.assertIn("float WindSpeedMetersPerSecond = 5.f;", compact_origin)
        self.assertNotIn("WindSpeedMetersPerSecond = 5.0f", compact_origin)
        self.assertNotIn("WindSpeedMetersPerSecond = false", compact_origin)
        self.assertNotIn("WindSpeedMetersPerSecond = true", compact_origin)
        self.assertNotIn("WindSpeedMetersPerSecond = 0.f", compact_origin)
        self.assertNotIn("WindSpeedMetersPerSecond = 160.f", compact_origin)

    def test_wind_speed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float WindSpeedMetersPerSecond"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 5.f", LOCKED_DECL)
        self.assertNotIn("= 5.0f", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FIVE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FIVE_POINT_ZERO}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_FIVE}\n",
            f"\t{TARGET_WRONG_FIVE_POINT_ZERO}\n",
            leftover_primary,
            leftover_guided,
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_wind_speed_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(compact_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        with_max = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wind_speed_specifiers(spec_section(with_max))
    ...
    def test_does_not_contract_sibling_weather_profile_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin = "0.0"', UPROPERTY_EDIT_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("Category", UPROPERTY_EDIT_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weather-profile WindSpeedMetersPerSecond field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weather_profile_wind_speed_meters_per_second_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
    ...
    def test_sibling_weather_profile_fields_stay_unclaimed(self) -> None:
        remaining = (
            REMAINING_WEATHER_SCRIPT,
            REMAINING_TIME_OF_DAY_SCRIPT,
            REMAINING_PRECIPITATION_SCRIPT,
        ...
        self.assertIn(LEFTOVER_WEATHER_PROFILE_PROFILE_ID, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_WEATHER_PROFILE_PROFILE_ID, THIS_SCRIPT)
