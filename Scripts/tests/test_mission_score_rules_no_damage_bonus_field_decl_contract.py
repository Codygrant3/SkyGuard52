...
# if the initializer is not `= 1500`, if meta ClampMin
...
# PerfectAccuracyBonus / BronzeThreshold / SilverThreshold /
# GoldThreshold.
...
# isolated NoDamageBonus field decl, same as leftover
...
# `FName WeatherIdentity` / `float NoDamageBonus`
# / `int32 UnitCount` / no `=` / ClampMin missing,
# retarget: type is int32 NoDamageBonus,
# initializer is `= 1500`, specifiers are EditAnywhere +
# BlueprintReadOnly + meta = (ClampMin = "0"),
# no Category.
...
TARGET = "int32 NoDamageBonus = 1500;"
TARGET_WRONG_EQ_NONE = "int32 NoDamageBonus = NAME_None;"
TARGET_WRONG_FALSE = "int32 NoDamageBonus = false;"
TARGET_WRONG_TRUE = "int32 NoDamageBonus = true;"
TARGET_WRONG_ZERO = "int32 NoDamageBonus = 0;"
TARGET_WRONG_FLOAT = "float NoDamageBonus = 1500;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "int32 NoDamageBonus;"
TARGET_WRONG_BONUS = "int32 NoDamageBonus = 2500;"
TARGET_WRONG_FNAME = "FName NoDamageBonus;"
TARGET_WRONG_BRONZE = "int32 BronzeThreshold = 5000;"
...
SIBLING_BRONZE = "BronzeThreshold"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `= 5000` / Harbor 40/80 / leftover Health / float /
    # FName / sibling CompletionScore = 5000. Do not accept
    ...
    if re.search(
        r"int32\s+NoDamageBonus\s*=\s*1500\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+NoDamageBonus\s*=\s*0\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+NoDamageBonus\s*=\s*2500\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+NoDamageBonus\s*=\s*5000\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+NoDamageBonus\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+NoDamageBonus\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+NoDamageBonus\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+NoDamageBonus\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 NoDamageBonus is missing from "
...
def require_no_damage_bonus_specifiers(section: str) -> str:
...
class MissionScoreRulesNoDamageBonusFieldDeclContractTests(unittest.TestCase):
    def test_mission_score_rules_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "NoDamageBonus"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "NoDamageBonus"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("NoDamageBonus", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_no_damage_bonus_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tint32 {SIBLING_BRONZE} = 5000;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_no_damage_bonus_specifiers(section)
    ...
    def test_initializer_fails_closed_when_not_1500(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
        ...
        self.assertIn("int32 NoDamageBonus = 1500;", compact_origin)
        self.assertNotIn("NoDamageBonus = NAME_None", compact_origin)
        self.assertNotIn("NoDamageBonus = false", compact_origin)
        self.assertNotIn("NoDamageBonus = true", compact_origin)
        self.assertNotIn("NoDamageBonus = 0.f", compact_origin)
        self.assertNotIn("NoDamageBonus = 160.f", compact_origin)

    def test_no_damage_bonus_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 NoDamageBonus"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 1500", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BRONZE}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_BRONZE}\n",
            f"\t{TARGET_WRONG_COMPLETION}\n",
            leftover_primary,
            leftover_guided,
            f"\tFText {SIBLING_SPEAKER};\n",
            "\tFName NoDamageBonuses;\n",
            "\tbool NoDamageBonus;\n",
            "\tfloat NoDamageBonus = " + forty + ";\n",
            "\tfloat NoDamageBonus = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_no_damage_bonus_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(compact_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        with_max = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(EditAnywhere, BlueprintReadOnly, "
            'meta = (ClampMin = "0", ClampMax = "1.0"))\n'
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_no_damage_bonus_specifiers(spec_section(with_max))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-score-rules NoDamageBonus field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_score_rules_no_damage_bonus_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
