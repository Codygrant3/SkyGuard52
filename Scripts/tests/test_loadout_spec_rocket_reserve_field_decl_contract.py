# THIS IS leftover-safe FSkyguardLoadoutSpec RocketReserve.
...
# CannonReserve / RocketMagazineSize / GuidedMagazineSize /
...
# CpgFeel constant (CannonReserve / RocketMagazineSize /
# etc.).
...
# is not Harbor. Sibling HullIntegrity is not this slot
# and is not Harbor.
...
TARGET_WRONG_BARE = "int32 RocketReserve;"
TARGET_WRONG_ZERO = "int32 RocketReserve = 0;"
TARGET_WRONG_ZERO_F = "int32 RocketReserve = 0.f;"
TARGET_WRONG_FALSE = "int32 RocketReserve = false;"
TARGET_WRONG_HUNDRED = "int32 RocketReserve = 100;"
TARGET_WRONG_SIX = "int32 RocketReserve = 6;"
TARGET_WRONG_CPG_FEEL = (
    "int32 RocketReserve = SkyguardApacheCpgFeel::RocketMagazineSize;"
)
TARGET_WRONG_CANNON_RESERVE = (
    "int32 RocketReserve = SkyguardApacheCpgFeel::CannonReserve;"
)
TARGET_WRONG_GUIDED_RESERVE = (
    "int32 RocketReserve = SkyguardApacheCpgFeel::GuidedReserve;"
)
TARGET_WRONG_FLOAT = (
    "float RocketReserve = SkyguardApacheCpgFeel::RocketReserve;"
)
TARGET_WRONG_BOOL = (
    "bool RocketReserve = SkyguardApacheCpgFeel::RocketReserve;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `int32 RocketReserve = SkyguardApacheCpgFeel::RocketReserve;`.
    # Do not accept literals `= 0`, `= 6`, `= 100`, `= 0.f`,
    # `= false`, missing initializer, or a different CpgFeel
    # constant (CannonReserve / RocketMagazineSize / etc.).
    # Do not accept sibling FlareCount / HullIntegrity.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+RocketReserve\s*=\s*"
        r"SkyguardApacheCpgFeel::RocketReserve\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"int32\s+RocketReserve\s*=\s*0\b", compact):
        return False
    if re.search(r"int32\s+RocketReserve\s*=\s*6\b", compact):
        return False
    if re.search(r"int32\s+RocketReserve\s*=\s*100\b", compact):
        return False
    if re.search(r"int32\s+RocketReserve\s*=\s*0\.f", compact):
        return False
    if re.search(r"int32\s+RocketReserve\s*=\s*false\b", compact):
        return False
    if re.search(
        r"int32\s+RocketReserve\s*=\s*"
    ...
    if re.search(
        ...
        return False
    if re.search(r"float\s+RocketReserve\b", compact):
        return False
    if re.search(r"bool\s+RocketReserve\b", compact):
        return False
    if re.search(r"int32\s+RocketReserve\s*;", compact) and not re.search(
        r"int32\s+RocketReserve\s*=",
        compact,
    ):
        return False
    return True
...
class LoadoutSpecRocketReserveFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RocketReserve"), section)
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RocketReserve"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("RocketReserve", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_rocket_reserve_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 RocketReserve"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("SkyguardApacheCpgFeel::RocketReserve", LOCKED_DECL)
    ...
    def test_hud_snapshot_zero_initializer_fails_closed(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_ZERO}\n"
        ...
        self.assertNotIn("int32 RocketReserve = 0;", compact_origin)
        self.assertNotIn("int32 RocketReserve = 6;", compact_origin)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 RocketReserves = "
            "SkyguardApacheCpgFeel::RocketReserve;\n",
            "\tint32 RocketReserve = " + forty + ";\n",
            "\tint32 RocketReserve = " + eighty + ";\n",
        )
    ...
    def test_rocket_reserve_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RocketReserve"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CANNON_RESERVE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec RocketReserve field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "rocket_reserve_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_LOADOUT_DISPLAY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOADOUT_SLOT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("namespace " + LEFTOVER_CPG_FEEL, locked_only)
        self.assertNotIn("constexpr", locked_only)
        self.assertIn(LEFTOVER_CPG_FEEL + "::RocketReserve", locked_only)
