...
# INDEX_NONE or lock DistrictLengthCm construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# DistrictLengthCm distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep DistrictLengthCm
...
# layout floats RouteLengthCm (in-flight/drafted) /
...
# one-line / split-line wraps. Locked 7500.f is
...
# 7500.f value. Do not lock leftover
...
LOCKED_DISTRICT_LENGTH_VALUE = "7500.f"
...
# DistrictLengthCm field declaration contract on
...
# RouteLengthCm / environment Root
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::DistrictLengthCm",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01DistrictLengthCmFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DISTRICT_LENGTH_CM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, DISTRICT_LENGTH_CM_FIELD))

    def test_missing_district_length_cm_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("EditAnywhere", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("EditInstanceOnly", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("BlueprintReadWrite", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("VisibleAnywhere", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("BlueprintReadOnly", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("Category", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("ClampMin", DISTRICT_LENGTH_CM_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tfloat* DistrictLengthCm;\n"
        )
        as_array = (
            "\tTArray<float> DistrictLengthCm;\n"
        )
        no_init = "\tfloat DistrictLengthCm;\n"
        int_type = "\tint32 DistrictLengthCm = 45000;\n"
        double_type = "\tdouble DistrictLengthCm = 7500.f;\n"
        wrong_init = "\tfloat DistrictLengthCm = 0.f;\n"
        wrong_scale = "\tfloat DistrictLengthCm = 40000.f;\n"
        scene = "\tTObjectPtr<USceneComponent> DistrictLengthCm;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> DistrictLengthCm;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> DistrictLengthCm;\n"
        )
        weak = (
        ...
        assigned = (
            "\tfloat DistrictLengthCm = nullptr;\n"
        )
        soft = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DISTRICT_LENGTH_CM_FIELD)
            self.assertIn("DistrictLengthCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_district_length_cm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DISTRICT_LENGTH_CM_FIELD),
            DISTRICT_LENGTH_CM_FIELD,
        )
        self.assertTrue(has_declaration(section, DISTRICT_LENGTH_CM_FIELD))
        self.assertEqual(
            declaration_count(section, DISTRICT_LENGTH_CM_FIELD),
            1,
        )
        self.assertTrue(
            DISTRICT_LENGTH_CM_FIELD.startswith("float DistrictLengthCm"),
            DISTRICT_LENGTH_CM_FIELD,
        )
        self.assertTrue(DISTRICT_LENGTH_CM_FIELD.endswith(";"), DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertIn(
            "float",
            DISTRICT_LENGTH_CM_FIELD,
        )
        self.assertIn("=", DISTRICT_LENGTH_CM_FIELD)
        self.assertIn(LOCKED_DISTRICT_LENGTH_VALUE, DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("TObjectPtr<", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("TArray<", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("TSoftObjectPtr", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("nullptr", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("INDEX_NONE", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("UFUNCTION", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("{", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("}", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("return ", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("OceanTiles", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("LandTiles", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("RouteExclusion", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("bRouteExclusionValid", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("BeachTiles", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("InlandVegetationPCG", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("LandScatterBounds", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("ProductionLandscape", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("AuthoredPCGGraph", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("RouteLengthCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("BeachWidthCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("InlandExtentCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("SeawardExtentCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            DISTRICT_LENGTH_CM_FIELD,
        )
        ...
        self.assertNotIn("Root", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetReadiness", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("Briefing", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("AudioDirector", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("RadioChatter", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("SortiePresentation", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("CampaignDefinition", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("MissionDefinition", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("bAutoInitialize", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetAircraft", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("ConfigureFromMission", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("AdvanceBriefing", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("SetAssetsReady", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("CanLaunch", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetElapsedSeconds", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetBriefingState", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetBriefingText", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetRadioChatter", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetPresentation", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("GetMissionTitle", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("AcknowledgeBriefing", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("LaunchSortie", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("HullCollider", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("OpticalTracker", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("WeaponServo", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("CountermeasurePod", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("RadarNode", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("HarborIndustrial", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("MaxIntegrity", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", DISTRICT_LENGTH_CM_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT} {DISTRICT_LENGTH_CM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{DISTRICT_LENGTH_CM_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DISTRICT_LENGTH_CM_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DISTRICT_LENGTH_CM_FIELD),
            section,
    ...
    def test_assigned_district_length_cm_does_not_satisfy(self) -> None:
        assigned = (
            "\tfloat DistrictLengthCm = 0.f;\n"
        )
        missing_init = "\tfloat DistrictLengthCm;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DISTRICT_LENGTH_CM_FIELD))
        with self.assertRaises(AssertionError) as raised_missing:
            require_declaration(missing_init, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", str(raised_missing.exception))
        self.assertFalse(has_declaration(missing_init, DISTRICT_LENGTH_CM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DistrictLengthCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DISTRICT_LENGTH_CM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DISTRICT_LENGTH_CM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DISTRICT_LENGTH_CM_FIELD.startswith("UFUNCTION"),
            DISTRICT_LENGTH_CM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DISTRICT_LENGTH_CM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_district_length_cm_cpp_body(self) -> None:
        locked_only = f"{DISTRICT_LENGTH_CM_FIELD}\n"
        self.assertNotIn("{", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("}", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("return ", DISTRICT_LENGTH_CM_FIELD)
        ...
        self.assertNotIn("return false", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("return true", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertTrue(
            DISTRICT_LENGTH_CM_FIELD.startswith("float DistrictLengthCm")
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, LOCKED_DISTRICT_LENGTH_VALUE)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn(LOCKED_DISTRICT_LENGTH_VALUE, leftover_harbor_tokens())
    ...
    def test_harbor_adjacent_fields_are_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual(LOCKED_DISTRICT_LENGTH_VALUE, token)
            self.assertNotIn(token, leftover_harbor_tokens())

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DISTRICT_LENGTH_CM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DISTRICT_LENGTH_CM_FIELD),
            DISTRICT_LENGTH_CM_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISTRICT_LENGTH_CM_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, DISTRICT_LENGTH_CM_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 DistrictLengthCm field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{DISTRICT_LENGTH_CM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DISTRICT_LENGTH_CM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DISTRICT_LENGTH_CM_FIELD.lower(),
                "mission01 DistrictLengthCm contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DISTRICT_LENGTH_CM_FIELD)

    def test_contract_is_district_length_cm_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("DistrictLengthCm", DISTRICT_LENGTH_CM_FIELD)
        self.assertTrue(DISTRICT_LENGTH_CM_FIELD.endswith(";"))
        self.assertIn("=", DISTRICT_LENGTH_CM_FIELD)
        self.assertIn(LOCKED_DISTRICT_LENGTH_VALUE, DISTRICT_LENGTH_CM_FIELD)
        self.assertNotIn("TObjectPtr<", DISTRICT_LENGTH_CM_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT, section)

    def test_sibling_environment_fields_do_not_satisfy_district_length_cm(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DISTRICT_LENGTH_CM_FIELD))
            self.assertNotEqual(DISTRICT_LENGTH_CM_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_district_length_cm(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            ...
            "\t * This comment is not the DistrictLengthCm declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn("DistrictLengthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, DISTRICT_LENGTH_CM_FIELD)
        )
        self.assertNotIn("float DistrictLengthCm = 7500.f;", comment_only)
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISTRICT_LENGTH_CM_FIELD)
            self.assertNotEqual(token, DISTRICT_LENGTH_CM_FIELD)
        self.assertIn(LOCKED_DISTRICT_LENGTH_VALUE, DISTRICT_LENGTH_CM_FIELD)
        self.assertNotEqual(LOCKED_DISTRICT_LENGTH_VALUE, "45000.f")
        self.assertNotEqual(LOCKED_DISTRICT_LENGTH_VALUE, "2800.f")
        self.assertNotEqual(LOCKED_DISTRICT_LENGTH_VALUE, "5200.f")
        self.assertNotEqual(LOCKED_DISTRICT_LENGTH_VALUE, "1800.f")
        self.assertNotEqual(LOCKED_DISTRICT_LENGTH_VALUE, "18000.f")
        self.assertNotEqual(LOCKED_DISTRICT_LENGTH_VALUE, "20000.f")
