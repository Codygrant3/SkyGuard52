# THIS IS leftover-safe ASkyguardApacheAircraft MainRotorCross.
...
# then BARE `TObjectPtr<UStaticMeshComponent> MainRotorCross;`
# (no initializer).
...
# Stop BEFORE sibling HorizontalTail as a claimed slot.
# Do NOT claim sibling MainRotor / RotorMast / TailRotor /
# RadarDome / EngineLeft / EngineRight / HorizontalTail /
# GearNose / Canopy / Fuselage / AircraftRoot /
# leftover SilhouetteMesh.
# Do NOT claim RotorPower, HoverBobCentimeters,
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> MainRotorCross;`
# NOT leftover `MainRotor` / `RotorMast` / `TailRotor` /
# `RadarDome`, NOT `TObjectPtr<USceneComponent>`, NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> MainRotorCross;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> MainRotorCross = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> MainRotorCross = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> MainRotorCross = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MainRotorCross;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MainRotorCross;"
...
STOP_BEFORE_HORIZONTAL_TAIL = "HorizontalTail"
...
LEFTOVER_APACHE_MAIN_ROTOR = (
    "Scripts/tests/test_apache_main_rotor_field_decl_contract.py"
)
LEFTOVER_APACHE_ROTOR_MAST = (
    "Scripts/tests/test_apache_rotor_mast_field_decl_contract.py"
)
LEFTOVER_APACHE_TAIL_ROTOR = (
    "Scripts/tests/test_apache_tail_rotor_field_decl_contract.py"
)
LEFTOVER_APACHE_RADAR_DOME = (
    "Scripts/tests/test_apache_radar_dome_field_decl_contract.py"
)
...
MAIN_ROTOR_CROSS_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+MainRotorCross\s*;"
)
MAIN_ROTOR_CROSS_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+MainRotorCross\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover Fuselage / AircraftRoot / MainRotor /
    # RotorMast / TailRotor / RadarDome, leftover
    # TObjectPtr<USceneComponent>, or an initializer
    # on MainRotorCross as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MAIN_ROTOR_CROSS_DECL_RE.search(compact) is None:
        return False
    if MAIN_ROTOR_CROSS_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_HORIZONTAL_TAIL):
            break
    ...
    if has_identifier(window, SIBLING_HORIZONTAL_TAIL):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_HORIZONTAL_TAIL}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+MainRotorCross\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> MainRotorCross "
...
class ApacheMainRotorCrossFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MainRotorCross"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_HORIZONTAL_TAIL),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MainRotorCross"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MainRotorCross", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_main_rotor_cross_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> MainRotorCross;", compact_origin)
        self.assertNotIn("MainRotorCross = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_main_rotor_cross_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> MainRotorCross"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_MAIN_ROTOR))
        ...
        self.assertNotIn(SIBLING_ENGINE_LEFT, LOCKED_DECL)
        self.assertNotIn(SIBLING_ENGINE_RIGHT, LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAIN_ROTOR)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROTOR_MAST)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SILHOUETTE_MESH)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RADAR_DOME}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SCENE}\n", LOCKED_DECL)
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> MainRotorCross;")
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAIN_ROTOR)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROTOR_MAST)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TAIL_ROTOR)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RADAR_DOME)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SILHOUETTE_MESH}\n",
            f"\t{TARGET_WRONG_HORIZONTAL_TAIL}\n",
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> MainRotorCrosses;\n",
            "\tint32 MainRotorCross = 140;\n",
            "\tbool MainRotorCross = true;\n",
            "\tfloat MainRotorCross = " + forty + ";\n",
            "\tfloat MainRotorCross = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_horizontal_tail_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache MainRotorCross field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "main_rotor_cross_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_MAIN_ROTOR))
        ...
        self.assertNotIn(SIBLING_ENGINE_RIGHT, locked_only)
