...
# origin/main form: split-line static BlueprintPure wrap
...
# compact LOCKED_DECL (NO spaces around wrap =):
# static FSkyguardLandscapeMaterialCompilationResult AuditLandscapeMaterialCompilation(ALandscape* Landscape, UMaterialInterface* ExpectedParentMaterial);
# (declaration-only, no inline body required).
# Comment: "Read-only generated-resource audit; never waits, mutates, or saves."
# THIS IS leftover-safe unique analog library UFUNCTION wrapping Category path
# Skyguard|Mission01|Environment|Validation.
# Leftover unique LandscapeMaterialCompilationResult FIELD drafts (#1223-#1232)
# wrap struct members with NO Category and explicitly
# do NOT parse this library.
# Identifier AuditLandscapeMaterialCompilation is NOT leftover analog field
# bSuccess and NOT leftover unique AuditLandscapeVisibleReadiness #1696
# and NOT leftover unique FinishLandscapeMaterialCompilation and NOT leftover
# unique Authoring AuditGovernedLandscapeAndGraph #1691.
# Category IS `Skyguard|Mission01|Environment|Validation`.
# BlueprintPure IS present. BlueprintCallable IS absent. const IS absent.
...
# stay BlueprintCallable, if method const is present, if the
# return type is not FSkyguardLandscapeMaterialCompilationResult,
# if static is missing, or if Category is Authoring / Grounding /
# bare Environment / Skyguard|Apache without |Damage / Skyguard|Combat
...
# Do NOT parse VisibleAudit / CaptureConfiguration / AuthoringResult structs.
...
# Do NOT claim leftover unique Authoring UFUNCTIONs #1691-#1693
...
# Clone leftover unique Validation BlueprintPure GetActiveRHIAndFeatureLevel
# KEEP Category Validation. KEEP BlueprintPure (NOT BlueprintCallable).
# RETARGET return FString -> FSkyguardLandscapeMaterialCompilationResult.
# RETARGET method GetActiveRHIAndFeatureLevel() no-args ->
# AuditLandscapeMaterialCompilation(ALandscape* Landscape,
# UMaterialInterface* ExpectedParentMaterial).
# Clone leftover unique PrepareGovernedLandscape #1694 RETARGET
# Callable -> Pure, VisibleAudit -> MaterialCompilationResult,
# method -> AuditLandscapeMaterialCompilation,
# GovernedMaterial -> ExpectedParentMaterial.
...
# KEEP BlueprintPure. LOCKED_DECL is NOT `int32 GetSortieHits() const;`.
# Clone leftover unique AreResolvedProductionLoopRoutesComplete #1659
# for BlueprintPure RETARGET.
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# Category Production — RETARGET Production -> Validation
# AND BlueprintCallable -> BlueprintPure AND
# void EnsureDefaultEntries(); -> this LOCKED_DECL.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is AuditLandscapeMaterialCompilation static struct-return.
...
# `bool bSuccess = false;` and NOT GetActiveRHIAndFeatureLevel and NOT
...
# SampleLandscapeHeight and NOT `void EnsureDefaultEntries();`.
# Fail-closed if this test still asserts FName
# WeatherIdentity / 160.f / FireRate = 12.0f /
# RecoilPitch = 0.92f / RouteLengthCm = 45000.f /
# GetSortieHits / ResetSortieCombatStats /
# GetInvertVerticalLook / bSuccess = false /
# EnsureDefaultEntries / GetActiveRHIAndFeatureLevel /
# FinishLandscapeMaterialCompilation /
# AuditLandscapeVisibleReadiness /
# PrepareGovernedLandscapeForVisibleValidation /
# AuthorGovernedLandscapeAndGraph / SampleLandscapeHeight
# AS THE LOCKED DECL.
...
TARGET_WRONG_BARE = (
    "void AuditLandscapeMaterialCompilation();"
)
...
TARGET_WRONG_FLOAT = (
    "float AuditLandscapeMaterialCompilation();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void AuditLandscapeMaterialCompilations();"
...
TARGET_WRONG_TRUE = "bool AuditLandscapeMaterialCompilation = true;"
TARGET_WRONG_ZERO = "bool AuditLandscapeMaterialCompilation = 0.f;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_mission01_audit_landscape_material_compilation"
    "_decl_contract.py"
)
...
METHOD_SIGNATURE_RE = re.compile(
    ...
    r"AuditLandscapeMaterialCompilation\("
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bAuditLandscapeMaterialCompilation\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+AuditLandscapeMaterialCompilation\b", compact):
        return False
    if re.search(r"\bbool\s+AuditLandscapeMaterialCompilation\b", compact):
        return False
    if re.search(r"\bfloat\s+AuditLandscapeMaterialCompilation\b", compact):
        return False
    if re.search(r"\bint32\s+AuditLandscapeMaterialCompilation\b", compact):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            ...
            r"AuditLandscapeMaterialCompilation\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for AuditLandscapeMaterialCompilation "
...
class Mission01AuditLandscapeMaterialCompilationDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "AuditLandscapeMaterialCompilation"),
            section,
        )
    ...
    def test_missing_audit_landscape_material_compilation_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotEqual(
            specifiers,
            'BlueprintPure, Category="Skyguard|Mission01|Environment"',
        )
        self.assertNotEqual(
            specifiers,
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "AuditLandscapeMaterialCompilation()const",
    ...
    def test_audit_landscape_material_compilation_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid AuditLandscapeMaterialCompilationX();\n",
            "\tint32 AuditLandscapeMaterialCompilation = 1;\n",
            "\tFName AuditLandscapeMaterialCompilation = NAME_None;\n",
            "\tvoid AuditLandscapeMaterialCompilation() = "
            + forty
            + ";\n",
            "\tvoid AuditLandscapeMaterialCompilation() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "AuditLandscapeMaterialCompilation")
        )
        self.assertTrue(
            has_identifier(section, "AuditLandscapeMaterialCompilation")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 audit landscape material compilation "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_audit_landscape_material_compilation"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
