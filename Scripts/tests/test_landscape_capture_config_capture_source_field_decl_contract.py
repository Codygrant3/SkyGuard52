...
# CaptureSource FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FString CaptureSource;
# one-line / split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS bare `FString CaptureSource;` NOT `= false` / `= true` / `= 0`.
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# CaptureSource FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FString CaptureSource; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS bare `FString CaptureSource;` NOT `= true` / `= false`. Fail closed if the type
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# CaptureSource FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# FString CaptureSource; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# CaptureSource FIELD (declaration-only). It is
...
TARGET = "FString CaptureSource;"
TARGET_WRONG = "FName CaptureSource;"
LOCKED_DECL = TARGET
...
def alternate_bool_defaults(declaration: str) -> tuple[str, ...]:
    ...
    if "FName CaptureSource;" in declaration:
        forms.append(declaration.replace("FName CaptureSource;", "FString CaptureSource;"))
    if "FText CaptureSource;" in declaration:
        forms.append(declaration.replace("FText CaptureSource;", "FString CaptureSource;"))
    unique: list[str] = []
    for form in forms:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FString\s+CaptureSource\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FString CaptureSource is missing from "
...
class LandscapeCaptureConfigCaptureSourceFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("CaptureSource", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_capture_source_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_capture_source_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString CaptureSource"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("CaptureSource;"), LOCKED_DECL)
        self.assertTrue(has_one_declaration(section, TARGET), section)
        self.assertFalse(has_one_declaration(section, TARGET_WRONG))
        self.assertNotIn("FName CaptureSource;", section)
        ...
        self.assertIn("CaptureSource", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("CaptureSource", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FString CaptureSource"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("CaptureSource", locked_only)
    ...
    def test_contract_is_target_field_declaration_only(
        ...
        self.assertTrue(LOCKED_DECL.startswith("FString CaptureSource"))
        self.assertIn("CaptureSource", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.endswith("CaptureSource;"))
