# THIS IS leftover-safe ASkyguardGunner HipFov.
...
# then `float HipFov = 85.f;` (HAS initializer).
# Do NOT claim leftover LookYawLimit #1618 / LookPitchMin
# #1617 / leftover LookPitchMax / leftover in-flight
# MouseSensitivity / leftover in-flight ADSFov = 42.f
# (not Harbor 40 / 80) as this slot.
# Stop BEFORE sibling FireRate. Do not lock FireRate.
# LOCKED_DECL initializer is 85.f, NOT leftover Harbor
# 40 / 80 and NOT leftover analog apache-cpg-feel
...
# BlueprintReadWrite IS present. Default IS `= 85.f`.
# This is a combat look float, not a CPG mesh/text slot
...
# Stop BEFORE sibling FireRate as a claimed slot.
...
# LookYawLimit / LookPitchMin / LookPitchMax /
...
TARGET = "float HipFov = 85.f;"
TARGET_WRONG_BARE = "float HipFov;"
TARGET_WRONG_INIT = "float HipFov = nullptr;"
TARGET_WRONG_FALSE = "float HipFov = false;"
TARGET_WRONG_TRUE = "float HipFov = true;"
TARGET_WRONG_ZERO = "float HipFov = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> HipFov;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> HipFov;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> HipFov;"
...
STOP_BEFORE_FIRE_RATE = "FireRate"
...
HIP_FOV_DECL_RE = re.compile(
    r"float\s+HipFov\s*=\s*85\.f\s*;"
)
HIP_FOV_BARE_RE = re.compile(
    r"float\s+HipFov\s*;"
)
HIP_FOV_WRONG_INIT_RE = re.compile(
    r"float\s+HipFov\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float HipFov = 85.f;`.
    ...
    # Harbor 40 / 80, leftover ADSFov = 42.f, or a
    # bare HipFov without the 85.f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if HIP_FOV_DECL_RE.search(compact) is None:
        return False
    if HIP_FOV_BARE_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+TraceRange\b", compact):
        return False
    if re.search(r"float\s+HipFov\s*=\s*" + "40" + r"\.f\b", compact):
        return False
    if re.search(r"float\s+HipFov\s*=\s*" + "80" + r"\.f\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_FIRE_RATE:
            break
        if ident != "HipFov":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_FIRE_RATE):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_FIRE_RATE}"
        )
...
class GunnerHipFovFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HipFov"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_FIRE_RATE), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_FIRE_RATE),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HipFov"), section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float HipFov = 85.f;", compact_origin)
        self.assertNotIn("HipFov = 160.f", compact_origin)
    ...
    def test_hip_fov_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float HipFov = 85.f;")
        self.assertNotEqual(LOCKED_DECL, "float ADSFov = 42.f;")
    ...
    def test_does_not_claim_sibling_fire_rate_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "HipFov"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_FIRE_RATE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_FIRE_RATE))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "HipFov"))
        self.assertTrue(has_identifier(section, "HipFov"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_FIRE_RATE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_FIRE_RATE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner HipFov field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_hip_fov_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
