# THIS IS leftover-safe ASkyguardApacheAircraft StubWingRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> StubWingRight;`
# (no initializer).
...
# Stop BEFORE sibling RotorMast as a claimed slot.
# Do NOT claim StubWingLeft / TailBoom / VerticalTail /
# TailRotor / HorizontalTail / RotorMast / MainRotor /
# Fuselage / Nose / Canopy / SilhouetteMesh / AircraftRoot /
# RotorPower / HoverBobCentimeters / MaxIntegrity /
# CurrentIntegrity / HullCollider / PilotCanopy /
# ChinTurret / ChinHousing / ChinBarrel.
# StubWingRight is NOT leftover StubWingLeft.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> StubWingRight;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> StubWingRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> StubWingRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> StubWingRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> StubWingRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> StubWingRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> StubWingRight;"
...
STOP_BEFORE_ROTOR_MAST = "RotorMast"
...
STUB_WING_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+StubWingRight\s*;"
)
STUB_WING_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+StubWingRight\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover VerticalTail / Fuselage / AircraftRoot, or an
    # initializer on StubWingRight as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if STUB_WING_RIGHT_DECL_RE.search(compact) is None:
        return False
    if STUB_WING_RIGHT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_ROTOR_MAST):
            break
    ...
    if has_identifier(window, SIBLING_ROTOR_MAST):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_ROTOR_MAST}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+StubWingRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> StubWingRight "
...
class ApacheStubWingRightFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "StubWingRight"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ROTOR_MAST), body)
        self.assertFalse(
            has_identifier(section, SIBLING_ROTOR_MAST),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "StubWingRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("StubWingRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_stub_wing_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> StubWingRight;", compact_origin)
        self.assertNotIn("StubWingRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_stub_wing_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> StubWingRight"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> StubWingRight;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> StubWingRights;\n",
            "\tint32 StubWingRight = 140;\n",
            "\tbool StubWingRight = true;\n",
            "\tfloat StubWingRight = " + forty + ";\n",
            "\tfloat StubWingRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_rotor_mast_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_ROTOR_MAST))
        ...
        self.assertNotIn(TARGET_WRONG_ROTOR_MAST, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache StubWingRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "stub_wing_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
