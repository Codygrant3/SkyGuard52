# THIS IS leftover-safe FSkyguardEnemyWaveDefinition WaveId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName WaveId;` (no initializer).
...
# Do NOT contract sibling fields StartTimeSeconds /
...
# isolated WaveId field decl, same as leftover
...
# `FName PointId`, retarget: type is FName WaveId,
...
TARGET = "FName WaveId;"
TARGET_WRONG_EQ_NONE = "FName WaveId = NAME_None;"
TARGET_WRONG_FALSE = "FName WaveId = false;"
TARGET_WRONG_TRUE = "FName WaveId = true;"
TARGET_WRONG_ZERO = "FName WaveId = 0.f;"
TARGET_WRONG_FLOAT = "float WaveId;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName WaveId;`.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+WaveId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName WaveId is missing from "
...
def require_wave_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} WaveId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} WaveId"
        )
...
class EnemyWaveWaveIdFieldDeclContractTests(unittest.TestCase):
    def test_enemy_wave_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "WaveId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("WaveId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_wave_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_wave_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName WaveId;", compact_origin)
        self.assertNotIn("WaveId = NAME_None", compact_origin)
        self.assertNotIn("WaveId = false", compact_origin)
        self.assertNotIn("WaveId = true", compact_origin)
        self.assertNotIn("WaveId = 0.f", compact_origin)
        self.assertNotIn("WaveId = 160.f", compact_origin)

    def test_wave_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName WaveId"),
            LOCKED_DECL,
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_wave_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wave_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wave_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wave_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_wave_id_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "enemy-wave WaveId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enemy_wave_wave_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
