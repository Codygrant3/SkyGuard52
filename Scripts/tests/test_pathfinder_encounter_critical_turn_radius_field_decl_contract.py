...
# then `float CriticalTurnRadiusCm = 1500.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Route`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"100.0"`. Default IS `= 1500.f`.
...
# UFUNCTION hashes three seven six through three eight two).
...
# LockWindowSpeedCmPerSecond / CriticalSpeedCmPerSecond /
# IngressSwayWavelengthCm = 9000.f
# (leave siblings uncontracted).
...
# if ClampMin is missing, if the default is not `= 1500.f`, if Category is
...
# leftover-safe unique Category path `Skyguard|Boss|Encounter|Route`.
...
# Clone wrap was Category="Skyguard|Boss|Encounter|Route" wrapping
# `float LockWindowSpeedCmPerSecond = 850.f;` or leftover analog
# `float IngressSwayWavelengthCm = 9000.f;`
# — RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Route", meta=(ClampMin="100.0")
# (NO spaces around =). Clone LOCKED_DECL is a speed / wavelength
# float. THIS LOCKED_DECL IS `float CriticalTurnRadiusCm = 1500.f;`.
# LOCKED_DECL is `float CriticalTurnRadiusCm = 1500.f;`
...
# and NOT leftover LookYawLimit = 95.f
...
# / CriticalSpeedCmPerSecond / IngressSwayWavelengthCm
# AS THE LOCKED DECL.
...
# Fail-closed if ClampMin is missing or default is not 1500.f.
...
TARGET = "float CriticalTurnRadiusCm = 1500.f;"
TARGET_WRONG_BARE = "float CriticalTurnRadiusCm;"
TARGET_WRONG_INIT = "float CriticalTurnRadiusCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float CriticalTurnRadiusCm = true;"
TARGET_WRONG_ZERO = "float CriticalTurnRadiusCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CriticalTurnRadiusCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CriticalTurnRadiusCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CriticalTurnRadiusCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "CriticalTurnSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_critical_turn_radius"
...
CRITICAL_TURN_RADIUS_DECL_RE = re.compile(
    r"float\s+CriticalTurnRadiusCm\s*=\s*1500\.f\s*;"
)
CRITICAL_TURN_RADIUS_BARE_RE = re.compile(
    r"float\s+CriticalTurnRadiusCm\s*;"
)
LOCK_WINDOW_SPEED_LEFTOVER_RE = re.compile(
    r"float\s+LockWindowSpeedCmPerSecond\s*=\s*850\.f\b"
)
CRITICAL_SPEED_LEFTOVER_RE = re.compile(
    r"float\s+CriticalSpeedCmPerSecond\s*=\s*700\.f\b"
)
INGRESS_SWAY_AMPLITUDE_LEFTOVER_RE = re.compile(
    r"float\s+IngressSwayAmplitudeCm\s*=\s*320\.f\b"
)
LOCK_WINDOW_CLIMB_CM_LEFTOVER_RE = re.compile(
    r"float\s+LockWindowClimbCm\s*=\s*650\.f\b"
)
INGRESS_SWAY_WAVELENGTH_LEFTOVER_RE = re.compile(
    r"float\s+IngressSwayWavelengthCm\s*=\s*9000\.f\b"
)
CRITICAL_TURN_SECONDS_LEFTOVER_RE = re.compile(
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float CriticalTurnRadiusCm = 1500.f;`.
    ...
    # CriticalTurnRadiusCm without the 1500.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CRITICAL_TURN_RADIUS_DECL_RE.search(compact) is None:
        return False
    if CRITICAL_TURN_RADIUS_BARE_RE.search(compact):
        return False
    ...
    if CLIMB_SECONDS_DECL_RE.search(compact):
        return False
    if LOCK_WINDOW_SPEED_LEFTOVER_RE.search(compact):
        return False
    if CRITICAL_SPEED_LEFTOVER_RE.search(compact):
        return False
    if INGRESS_SWAY_AMPLITUDE_LEFTOVER_RE.search(compact):
        return False
    if LOCK_WINDOW_CLIMB_CM_LEFTOVER_RE.search(compact):
        return False
    if INGRESS_SWAY_WAVELENGTH_LEFTOVER_RE.search(compact):
        return False
    if CRITICAL_TURN_SECONDS_LEFTOVER_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CriticalTurnRadiusCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CriticalTurnRadiusCm "
...
class PathfinderEncounterCriticalTurnRadiusFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalTurnRadiusCm"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_CRITICAL_SPEED), body)
        self.assertFalse(has_identifier(section, SIBLING_CRITICAL_SPEED), section)
        self.assertTrue(has_identifier(body, SIBLING_INGRESS_SWAY_AMPLITUDE), body)
        self.assertFalse(has_identifier(section, SIBLING_INGRESS_SWAY_AMPLITUDE), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CriticalTurnRadiusCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CriticalTurnRadiusCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_critical_turn_radius_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float CriticalTurnRadiusCm;",
            compact_origin.split("CriticalTurnRadiusCm = 1500.f;")[0]
            + compact_origin.split("CriticalTurnRadiusCm = 1500.f;")[-1]
            if "CriticalTurnRadiusCm = 1500.f;" in compact_origin
            else compact_origin,
        )
        self.assertNotIn("float ApproachSpeedCmPerSecond = 1250.f;", LOCKED_DECL)
        self.assertNotIn("float LockWindowSpeedCmPerSecond = 850.f;", LOCKED_DECL)
        self.assertNotIn("float CriticalSpeedCmPerSecond = 700.f;", LOCKED_DECL)
        self.assertNotIn("float IngressSwayAmplitudeCm = 320.f;", LOCKED_DECL)
        self.assertNotIn("float IngressSwayWavelengthCm = 9000.f;", LOCKED_DECL)
        self.assertNotIn("float LockWindowClimbCm = 650.f;", LOCKED_DECL)
        self.assertNotIn("float LockWindowClimbSeconds = 2.4f;", LOCKED_DECL)
        self.assertNotIn("float CriticalTurnSeconds = 5.5f;", LOCKED_DECL)
        self.assertNotIn("float RouteLengthCm = 45000.f;", LOCKED_DECL)
    ...
    def test_critical_turn_radius_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CriticalTurnRadiusCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1500.f", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TURN_SECONDS)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat CriticalTurnRadiusCms = 1500.f;\n",
            "\tint32 CriticalTurnRadiusCm = 9000;\n",
            "\tFName CriticalTurnRadiusCm = NAME_None;\n",
            "\tfloat CriticalTurnRadiusCm = " + forty + ";\n",
            "\tfloat CriticalTurnRadiusCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalTurnRadiusCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CriticalTurnRadiusCm"))
        self.assertTrue(has_identifier(section, "CriticalTurnRadiusCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter critical turn radius field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_critical_turn_radius_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
