...
# then `float LightSpeedMin = 1300.f;`
# (HAS initializer).
...
# BlueprintReadWrite IS present. Default IS `= 1300.f`.
# KEEP EditAnywhere BlueprintReadWrite Category Skyguard|Spawning.
# KEEP default `1300.f`. KEEP specifiers matching origin/main.
# Word-boundary: `LightSpeedMin` is NOT leftover analog
...
# split 40 / 80 float tokens. 1300.f is the origin/main default
...
# HeavyCruiseSpeed / HeavyLaneModulo / bMixedThreatRoster /
...
# Stop BEFORE sibling LightSpeedMax as a claimed slot.
...
# LOCKED_DECL is `float LightSpeedMin = 1300.f;`
...
# NOT leftover analog `float HeavyCruiseSpeed = 1150.f;`
...
TARGET = "float LightSpeedMin = 1300.f;"
TARGET_WRONG_BARE = "float LightSpeedMin;"
TARGET_WRONG_INIT = "float LightSpeedMin = nullptr;"
TARGET_WRONG_FALSE = "bool bSpawningEnabled = true;"
TARGET_WRONG_TRUE = "float LightSpeedMin = true;"
TARGET_WRONG_ZERO = "float LightSpeedMin = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LightSpeedMin;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LightSpeedMin;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LightSpeedMin;"
...
SIBLING_HEAVY_CRUISE = "HeavyCruiseSpeed"
...
STOP_BEFORE_ROUTE_LENGTH_CM = "LightSpeedMax"
...
LIGHT_SPEED_MIN_DECL_RE = re.compile(
    r"float\s+LightSpeedMin\s*=\s*1300\.f\s*;"
)
LIGHT_SPEED_MIN_BARE_RE = re.compile(
    r"float\s+LightSpeedMin\s*;"
)
LIGHT_SPEED_MAX_DECL_RE = re.compile(
    r"float\s+LightSpeedMax\s*=\s*1750\.f\b"
)
HEAVY_CRUISE_DECL_RE = re.compile(
    r"float\s+HeavyCruiseSpeed\s*=\s*1150\.f\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # LightSpeedMax / HeavyCruiseSpeed / HeavyLaneModulo,
    ...
    # bare LightSpeedMin without `= 1300.f`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LIGHT_SPEED_MIN_DECL_RE.search(compact) is None:
        return False
    if LIGHT_SPEED_MIN_BARE_RE.search(compact):
        return False
    ...
    if LIGHT_SPEED_MAX_DECL_RE.search(compact):
        return False
    if HEAVY_CRUISE_DECL_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LightSpeedMin\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LightSpeedMin "
...
class DroneSpawnerLightSpeedMinFieldDeclContractTests(unittest.TestCase):
    def test_drone_spawner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LightSpeedMin"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_HEAVY_CRUISE), body)
        self.assertFalse(has_identifier(section, SIBLING_HEAVY_CRUISE), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertIn("LightSpeedMin", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_light_speed_min_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("float HeavyCruiseSpeed = 1150.f;", compact_origin)
        self.assertNotIn("bool bSpawningEnabled = true;", compact_origin)
        self.assertNotIn("float SpawnInterval = 1.85f;", compact_origin)

    def test_light_speed_min_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LightSpeedMin"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1300.f", LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "float LightSpeedMin = 1300.f;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HEAVY_CRUISE)
        ...
        self.assertNotIn("HeavyCruiseSpeed", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat LightSpeedMins = 1300.f;\n",
            "\tint32 LightSpeedMin = 1300;\n",
            "\tfloat LightSpeedMin = 1750.f;\n",
            "\tfloat LightSpeedMin = " + forty + ";\n",
            "\tfloat LightSpeedMin = " + eighty + ";\n",
    ...
    def test_does_not_claim_sibling_spawn_interval_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LightSpeedMin"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LightSpeedMin"))
        self.assertTrue(has_identifier(section, "LightSpeedMin"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_SET_SPAWNING))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "drone spawner light speed min field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "drone_spawner_light_speed_min_field_decl_contract.py"
        ))
