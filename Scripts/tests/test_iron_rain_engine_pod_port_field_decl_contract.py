...
# INDEX_NONE or lock EnginePodPort construction
...
# EnginePodPort;`); accept that form and other
...
# EnginePodPort field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardIronRainBoss::EnginePodPort",
    "SkyguardIronRainBoss.cpp",
    "SetExposed",
...
class IronRainEnginePodPortFieldDeclContractTests(unittest.TestCase):
    def test_iron_rain_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ENGINE_POD_PORT_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ENGINE_POD_PORT_FIELD)
        self.assertIn("EnginePodPort", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ENGINE_POD_PORT_FIELD)
    ...
    def test_missing_engine_pod_port_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardIronRainBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ENGINE_POD_PORT_FIELD)
        self.assertIn("EnginePodPort", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ENGINE_POD_PORT_FIELD)
        self.assertIn("EnginePodPort", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("VisibleAnywhere", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("BlueprintReadOnly", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("Category", ENGINE_POD_PORT_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ENGINE_POD_PORT_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ENGINE_POD_PORT_FIELD)
        self.assertIn("EnginePodPort", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* EnginePodPort;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> EnginePodPort;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> EnginePodPort;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ENGINE_POD_PORT_FIELD)
            self.assertIn("EnginePodPort", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_engine_pod_port_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ENGINE_POD_PORT_FIELD),
            ENGINE_POD_PORT_FIELD,
        )
        self.assertTrue(has_declaration(section, ENGINE_POD_PORT_FIELD))
        self.assertEqual(declaration_count(section, ENGINE_POD_PORT_FIELD), 1)
        self.assertTrue(
            ENGINE_POD_PORT_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            ENGINE_POD_PORT_FIELD,
        )
        self.assertTrue(ENGINE_POD_PORT_FIELD.endswith(";"), ENGINE_POD_PORT_FIELD)
        self.assertIn("EnginePodPort", ENGINE_POD_PORT_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            ENGINE_POD_PORT_FIELD,
        )
        self.assertIn("TObjectPtr<", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("TArray<", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("=", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("INDEX_NONE", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("UFUNCTION", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("{", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("}", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("return ", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("CommandAntenna;", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ControlLinkage", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("NoseCamera", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisCenter", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisTail", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisNose", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisSpine", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("AdvanceEncounter", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ResetEncounterState", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("IsRouteStateSafe", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetRouteProgress", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("RadarNode", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("IssueClimbCommand", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetFinaleStage", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("IsClimbCommandIssued", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("IsWreckDiverted", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetGunnerMount", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("FaceWorldLocation", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("PortGuidanceArray", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisArmorPort", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetSearchlightTracked", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetContactIdentified", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetLightningExposed", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("HullCollider", ENGINE_POD_PORT_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, ENGINE_POD_PORT_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {ENGINE_POD_PORT_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, ENGINE_POD_PORT_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{ENGINE_POD_PORT_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ENGINE_POD_PORT_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ENGINE_POD_PORT_FIELD),
            section,
    ...
    def test_assigned_engine_pod_port_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "EnginePodPort = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ENGINE_POD_PORT_FIELD)
        self.assertIn("EnginePodPort", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ENGINE_POD_PORT_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("EnginePodPort", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ENGINE_POD_PORT_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ENGINE_POD_PORT_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ENGINE_POD_PORT_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, ENGINE_POD_PORT_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ENGINE_POD_PORT_FIELD.startswith("UFUNCTION"),
            ENGINE_POD_PORT_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ENGINE_POD_PORT_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ENGINE_POD_PORT_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_engine_pod_port_cpp_body(self) -> None:
        locked_only = f"{ENGINE_POD_PORT_FIELD}\n"
        self.assertNotIn("{", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("}", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("return ", ENGINE_POD_PORT_FIELD)
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SkyguardIronRainBoss.cpp", locked_only)
        self.assertNotIn("return false", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("return true", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetExposed", ENGINE_POD_PORT_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            ENGINE_POD_PORT_FIELD,
        )
        self.assertNotIn("SetBossPhase", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", ENGINE_POD_PORT_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{ENGINE_POD_PORT_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("PortDischargeBoom", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("StarboardDischargeBoom", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("EngineIntake", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisPortPanel", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisStarboardPanel", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DebrisIntakePanel", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", ENGINE_POD_PORT_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            ENGINE_POD_PORT_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("EncounterController", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ControlServo", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DispenserPort", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DispenserCenter", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("MaxReleasesPerBay", ENGINE_POD_PORT_FIELD)
        self.assertIn("EnginePodPort", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("DispenserStarboard", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetManeuver", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), ENGINE_POD_PORT_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), ENGINE_POD_PORT_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), ENGINE_POD_PORT_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), ENGINE_POD_PORT_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), ENGINE_POD_PORT_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, ENGINE_POD_PORT_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetSearchlightTracked", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetContactIdentified", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("OpenOrbitExposure", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SetLightningExposed", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("IsLightningExposed", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", ENGINE_POD_PORT_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), ENGINE_POD_PORT_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), ENGINE_POD_PORT_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ASkyguardGunner", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SkyguardRadarNode", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("MaxIntegrity", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("CurrentIntegrity", ENGINE_POD_PORT_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ENGINE_POD_PORT_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            ENGINE_POD_PORT_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ENGINE_POD_PORT_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ENGINE_POD_PORT_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ENGINE_POD_PORT_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ENGINE_POD_PORT_FIELD),
            ENGINE_POD_PORT_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardIronRainBoss::EnginePodPort", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain EnginePodPort field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{ENGINE_POD_PORT_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ENGINE_POD_PORT_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ENGINE_POD_PORT_FIELD.lower(),
                "iron rain EnginePodPort contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ENGINE_POD_PORT_FIELD)

    def test_contract_is_engine_pod_port_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(ENGINE_POD_PORT_FIELD.endswith(";"))
        self.assertNotIn("=", ENGINE_POD_PORT_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
