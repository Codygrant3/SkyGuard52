# THIS IS leftover-safe ASkyguardApacheAircraft NightVisionTurret.
...
# then BARE `TObjectPtr<UStaticMeshComponent> NightVisionTurret;`
# (no initializer).
...
# Stop BEFORE sibling PilotCanopy as a claimed slot.
# Do NOT claim PilotCanopy, SensorTurret, SensorBall,
# ChinTurret, ChinHousing, ChinBarrel, TailRotor,
# MainRotor, RotorMast, MainRotorCross, AircraftRoot,
# Fuselage, Nose, Canopy, SilhouetteMesh, RadarDome,
# MaxIntegrity, CurrentIntegrity, HullCollider,
# GunnerMount, PilotMount, EyeMount, WeaponMount.
# NightVisionTurret is NOT leftover analog
# apache-set-sensor-view UFUNCTION, NOT leftover analog
# apache-get-sensor-quality UFUNCTION, NOT leftover
# sibling SensorTurret (USceneComponent) / SensorBall /
# ChinTurret. Type is TObjectPtr<UStaticMeshComponent>.
# Do NOT parse leftover analog apache-set-rotor-power
# UFUNCTION (that is not this TObjectPtr field).
# Do NOT parse leftover analog apache-set-sensor-view
# or apache-get-sensor-quality UFUNCTION decls.
...
# apache-is-canopy-glass-cracked UFUNCTION, leftover analog
...
# StubWingRight is an in-flight sibling, not this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> NightVisionTurret;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> NightVisionTurret;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> NightVisionTurret = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> NightVisionTurret = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> NightVisionTurret = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> NightVisionTurret;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> NightVisionTurret;"
...
STOP_BEFORE_PILOT_CANOPY = "PilotCanopy"
...
NIGHT_VISION_TURRET_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+NightVisionTurret\s*;"
)
NIGHT_VISION_TURRET_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+NightVisionTurret\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling SensorTurret (USceneComponent), or an
    # initializer on NightVisionTurret as the slot.
    # Fail-closed if LOCKED_DECL is still
    # TObjectPtr<USceneComponent>.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if NIGHT_VISION_TURRET_DECL_RE.search(compact) is None:
        return False
    if NIGHT_VISION_TURRET_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_PILOT_CANOPY):
            break
    ...
    if has_identifier(window, SIBLING_PILOT_CANOPY):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_PILOT_CANOPY}"
        )
    for later in (
        SIBLING_PILOT_CANOPY,
        SIBLING_MAIN_ROTOR_CROSS,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+NightVisionTurret\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> NightVisionTurret "
...
class ApacheNightVisionTurretFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "NightVisionTurret"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_PILOT_CANOPY), body)
        self.assertFalse(
            has_identifier(section, SIBLING_PILOT_CANOPY),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_PILOT_CANOPY), section)
        ...
        self.assertFalse(has_identifier(section, SIBLING_WEAPON_MOUNT), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "NightVisionTurret"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("NightVisionTurret", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_night_vision_turret_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> NightVisionTurret;", compact_origin)
        self.assertNotIn("NightVisionTurret = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_night_vision_turret_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> NightVisionTurret"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn(SIBLING_SENSOR_BALL, LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CHIN_TURRET)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SENSOR_BALL}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CHIN_TURRET}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CHIN_HOUSING}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> NightVisionTurret;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<USceneComponent> NightVisionTurret;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> NightVisionTurrets;\n",
            "\tint32 NightVisionTurret = 140;\n",
            "\tbool NightVisionTurret = true;\n",
            "\tfloat NightVisionTurret = " + forty + ";\n",
            "\tfloat NightVisionTurret = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_pilot_canopy_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_PILOT_CANOPY))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache NightVisionTurret field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "night_vision_turret_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_PILOT_MOUNT, locked_only)
        self.assertNotIn(SIBLING_EYE_MOUNT, locked_only)
        self.assertNotIn(SIBLING_WEAPON_MOUNT, locked_only)
        self.assertNotIn("USceneComponent", locked_only)
