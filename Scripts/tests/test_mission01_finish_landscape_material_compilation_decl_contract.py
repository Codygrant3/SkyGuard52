...
# then static FSkyguardLandscapeMaterialCompilationResult
# FinishLandscapeMaterialCompilation(
...
# Leftover unique LandscapeMaterialCompilationResult FIELD drafts
# (#1223-#1232) wrap struct members with NO Category and explicitly
# do NOT parse this library.
# Identifier FinishLandscapeMaterialCompilation is NOT leftover
# analog MaterialCompilation field bSuccess / MaterialResourceCount
# and NOT leftover unique PrepareGovernedLandscapeForVisibleValidation
# (#1694) / AuditLandscapeVisibleReadiness (#1696) /
# GetActiveRHIAndFeatureLevel (#1695) and NOT leftover unique
# Authoring (#1691-#1693) and NOT leftover unique SampleLandscapeHeight
# (#626 Grounding).
...
# return type is not FSkyguardLandscapeMaterialCompilationResult,
...
# Do NOT parse struct FSkyguardLandscapeMaterialCompilationResult
# (leftover unique field drafts #1223-#1232).
# Do NOT parse VisibleAudit / CaptureConfiguration / AuthoringResult
# structs.
...
# FinishLandscapeMaterialCompilation.
# RETARGET second arg GovernedMaterial -> ExpectedParentMaterial.
# Clone leftover unique Authoring #1692 SHA
# 828adaae3891fa9079064c593c1c74c7bd00369c — RETARGET Authoring ->
# Validation.
...
# LOCKED_DECL is FinishLandscapeMaterialCompilation
...
# void FinishLandscapeMaterialCompilation
# AS THE LOCKED DECL.
...
LEFTOVER_MATERIAL_COMPILATION_STRUCT = "FSkyguardLandscapeMaterialCompilationResult"
...
TARGET_WRONG_NO_CONST = (
    ...
    "FinishLandscapeMaterialCompilation("
    "ALandscape* Landscape, "
    "UMaterialInterface* ExpectedParentMaterial) const;"
)
...
TARGET_WRONG_FLOAT = (
    "float FinishLandscapeMaterialCompilation();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void FinishLandscapeMaterialCompilations();"
...
TARGET_WRONG_AUDIT_VISIBLE = (
    "static FSkyguardLandscapeMaterialCompilationResult AuditLandscapeVisibleReadiness("
)
...
TARGET_WRONG_TRUE = "bool FinishLandscapeMaterialCompilation = true;"
TARGET_WRONG_ZERO = "bool FinishLandscapeMaterialCompilation = 0.f;"
...
SIBLING_ROUTING = "PrepareGovernedLandscapeForVisibleValidation"
...
STOP_BEFORE_SKYLINE_STYLE = "AuditLandscapeMaterialCompilation"
...
LOCKED_SCRIPTS = (
    ...
    "_decl_contract.py",
    "Scripts/tests/test_landscape_material_compilation_material_resource_count"
    "_field_decl_contract.py",
    "Scripts/tests/test_landscape_material_compilation_landscape_component_count"
    "_field_decl_contract.py",
    "Scripts/tests/test_landscape_material_compilation_compilation_finished_resource_count"
    "_field_decl_contract.py",
    "Scripts/tests/test_landscape_material_compilation_valid_shader_map_resource_count"
    "_field_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bstatic\s+FSkyguardLandscapeMaterialCompilationResult\s+"
    r"FinishLandscapeMaterialCompilation\("
    r"ALandscape\* Landscape,"
    r"UMaterialInterface\* ExpectedParentMaterial\)(?:;|\{)"
)
...
LEFTOVER_MATERIAL_RESOURCE_COUNT_RE = re.compile(
    r"\bint32\s+MaterialResourceCount\b"
)
...
def leftover_analog_field_in_region(compact: str) -> bool:
    ...
    if LEFTOVER_SUCCESS_FIELD_RE.search(compact):
        return True
    if LEFTOVER_MATERIAL_RESOURCE_COUNT_RE.search(compact):
        return True
    return False
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bFinishLandscapeMaterialCompilation\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+FinishLandscapeMaterialCompilation\b", compact):
        return False
    if re.search(r"\bbool\s+FinishLandscapeMaterialCompilation\b", compact):
        return False
    if re.search(r"\bfloat\s+FinishLandscapeMaterialCompilation\b", compact):
        return False
    if re.search(r"\bint32\s+FinishLandscapeMaterialCompilation\b", compact):
        return False
...
def spec_section(header: str) -> str:
    ...
    method = "FinishLandscapeMaterialCompilation"
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static\s+FSkyguardLandscapeMaterialCompilationResult\s+"
            r"FinishLandscapeMaterialCompilation\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for FinishLandscapeMaterialCompilation "
...
class Mission01FinishLandscapeMaterialCompilationDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_MATERIAL_COMPILATION_STRUCT)
        ...
        self.assertTrue(
            has_identifier(section, "FinishLandscapeMaterialCompilation"),
            section,
        )
    ...
    def test_missing_finish_landscape_material_compilation_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "FinishLandscapeMaterialCompilation()const",
    ...
    def test_finish_landscape_material_compilation_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotIn("MaterialResourceCount", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid FinishLandscapeMaterialCompilationX();\n",
            "\tint32 FinishLandscapeMaterialCompilation = 1;\n",
            "\tFName FinishLandscapeMaterialCompilation = NAME_None;\n",
            "\tvoid FinishLandscapeMaterialCompilation() = "
            + forty
            + ";\n",
            "\tvoid FinishLandscapeMaterialCompilation() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "FinishLandscapeMaterialCompilation")
        )
        self.assertTrue(
            has_identifier(section, "FinishLandscapeMaterialCompilation")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 prepare governed landscape for visible validation "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_finish_landscape_material_compilation"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
