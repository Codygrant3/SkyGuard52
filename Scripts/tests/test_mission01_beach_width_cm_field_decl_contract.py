...
# bAutoLaunchAfterBriefing. Keep BeachWidthCm
...
# ShorelineLandOffsetCm / InlandExtentCm / SeawardExtentCm.
...
# one-line / split-line wraps. Locked 1800.f is
...
# 1800.f value. Do not lock leftover
...
LOCKED_BEACH_WIDTH_VALUE = "1800.f"
...
# BeachWidthCm field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::BeachWidthCm",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01BeachWidthCmFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, BEACH_WIDTH_CM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, BEACH_WIDTH_CM_FIELD))

    def test_missing_beach_width_cm_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("EditAnywhere", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("EditInstanceOnly", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("BlueprintReadWrite", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("VisibleAnywhere", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("BlueprintReadOnly", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("Category", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("ClampMin", BEACH_WIDTH_CM_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, BEACH_WIDTH_CM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        as_array = (
            "\tTArray<float> BeachWidthCm;\n"
        )
        no_init = "\tfloat BeachWidthCm;\n"
        int_type = "\tint32 BeachWidthCm = 45000;\n"
        double_type = "\tdouble BeachWidthCm = 1800.f;\n"
        wrong_init = "\tfloat BeachWidthCm = 0.f;\n"
        wrong_scale = "\tfloat BeachWidthCm = 40000.f;\n"
        scene = "\tTObjectPtr<USceneComponent> BeachWidthCm;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> BeachWidthCm;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> BeachWidthCm;\n"
        )
        weak = (
        ...
        leftover_width = "\tfloat DistrictLengthCm = 7500.f;\n"
        for region in (
            raw_pointer,
            as_array,
            no_init,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, BEACH_WIDTH_CM_FIELD)
            self.assertIn("BeachWidthCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_beach_width_cm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, BEACH_WIDTH_CM_FIELD),
            BEACH_WIDTH_CM_FIELD,
        )
        self.assertTrue(has_declaration(section, BEACH_WIDTH_CM_FIELD))
        self.assertEqual(
            declaration_count(section, BEACH_WIDTH_CM_FIELD),
            1,
        )
        self.assertTrue(
            BEACH_WIDTH_CM_FIELD.startswith("float BeachWidthCm"),
            BEACH_WIDTH_CM_FIELD,
        )
        self.assertTrue(BEACH_WIDTH_CM_FIELD.endswith(";"), BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", BEACH_WIDTH_CM_FIELD)
        self.assertIn(
            "float",
            BEACH_WIDTH_CM_FIELD,
        )
        self.assertIn("=", BEACH_WIDTH_CM_FIELD)
        self.assertIn(LOCKED_BEACH_WIDTH_VALUE, BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("TObjectPtr<", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("TArray<", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("TSoftObjectPtr", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("nullptr", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("INDEX_NONE", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("UFUNCTION", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("{", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("}", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("return ", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("OceanTiles", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("LandTiles", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("RouteExclusion", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("bRouteExclusionValid", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("BeachTiles", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("InlandVegetationPCG", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("LandScatterBounds", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("ProductionLandscape", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("AuthoredPCGGraph", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("RouteLengthCm", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("DistrictLengthCm", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("InlandExtentCm", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("SeawardExtentCm", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            BEACH_WIDTH_CM_FIELD,
        )
        ...
        self.assertNotIn("Root", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetReadiness", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("Briefing", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("AudioDirector", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("RadioChatter", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("SortiePresentation", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("CampaignDefinition", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("MissionDefinition", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("bAutoInitialize", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetAircraft", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("ConfigureFromMission", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("AdvanceBriefing", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("SetAssetsReady", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("CanLaunch", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetElapsedSeconds", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetBriefingState", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetBriefingText", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetRadioChatter", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetPresentation", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("GetMissionTitle", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("AcknowledgeBriefing", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("LaunchSortie", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("HullCollider", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("OpticalTracker", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("WeaponServo", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("CountermeasurePod", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("RadarNode", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("HarborIndustrial", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("MaxIntegrity", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", BEACH_WIDTH_CM_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, BEACH_WIDTH_CM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT} {BEACH_WIDTH_CM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{BEACH_WIDTH_CM_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, BEACH_WIDTH_CM_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, BEACH_WIDTH_CM_FIELD),
            section,
    ...
    def test_assigned_beach_width_cm_does_not_satisfy(self) -> None:
        assigned = (
            "\tfloat BeachWidthCm = 0.f;\n"
        )
        missing_init = "\tfloat BeachWidthCm;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, BEACH_WIDTH_CM_FIELD))
        with self.assertRaises(AssertionError) as raised_missing:
            require_declaration(missing_init, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", str(raised_missing.exception))
        self.assertFalse(has_declaration(missing_init, BEACH_WIDTH_CM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("BeachWidthCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, BEACH_WIDTH_CM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{BEACH_WIDTH_CM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, BEACH_WIDTH_CM_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            BEACH_WIDTH_CM_FIELD.startswith("UFUNCTION"),
            BEACH_WIDTH_CM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", BEACH_WIDTH_CM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_beach_width_cm_cpp_body(self) -> None:
        locked_only = f"{BEACH_WIDTH_CM_FIELD}\n"
        self.assertNotIn("{", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("}", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("return ", BEACH_WIDTH_CM_FIELD)
        ...
        self.assertNotIn("return false", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("return true", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", BEACH_WIDTH_CM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", BEACH_WIDTH_CM_FIELD)
        self.assertTrue(
            BEACH_WIDTH_CM_FIELD.startswith("float BeachWidthCm")
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, BEACH_WIDTH_CM_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, BEACH_WIDTH_CM_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, BEACH_WIDTH_CM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", BEACH_WIDTH_CM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", BEACH_WIDTH_CM_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, LOCKED_BEACH_WIDTH_VALUE)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEACH_WIDTH_CM_FIELD)
        self.assertNotIn(LOCKED_BEACH_WIDTH_VALUE, leftover_harbor_tokens())
    ...
    def test_harbor_adjacent_fields_are_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual(LOCKED_BEACH_WIDTH_VALUE, token)
            self.assertNotIn(token, leftover_harbor_tokens())

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{BEACH_WIDTH_CM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, BEACH_WIDTH_CM_FIELD),
            BEACH_WIDTH_CM_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BEACH_WIDTH_CM_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, BEACH_WIDTH_CM_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 BeachWidthCm field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{BEACH_WIDTH_CM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, BEACH_WIDTH_CM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                BEACH_WIDTH_CM_FIELD.lower(),
                "mission01 BeachWidthCm contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, BEACH_WIDTH_CM_FIELD)

    def test_contract_is_beach_width_cm_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("BeachWidthCm", BEACH_WIDTH_CM_FIELD)
        self.assertTrue(BEACH_WIDTH_CM_FIELD.endswith(";"))
        self.assertIn("=", BEACH_WIDTH_CM_FIELD)
        self.assertIn(LOCKED_BEACH_WIDTH_VALUE, BEACH_WIDTH_CM_FIELD)
        self.assertNotIn("TObjectPtr<", BEACH_WIDTH_CM_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT, section)

    def test_sibling_environment_fields_do_not_satisfy_beach_width_cm(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, BEACH_WIDTH_CM_FIELD))
            self.assertNotEqual(BEACH_WIDTH_CM_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_beach_width_cm(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            "\t/**\n"
            "\t * Mission 1 beach width is authored in centimeters.\n"
            "\t * This comment is not the BeachWidthCm declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, BEACH_WIDTH_CM_FIELD)
        self.assertIn("BeachWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, BEACH_WIDTH_CM_FIELD)
        )
        self.assertNotIn("float BeachWidthCm = 1800.f;", comment_only)
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEACH_WIDTH_CM_FIELD)
            self.assertNotEqual(token, BEACH_WIDTH_CM_FIELD)
        self.assertIn(LOCKED_BEACH_WIDTH_VALUE, BEACH_WIDTH_CM_FIELD)
        self.assertNotEqual(LOCKED_BEACH_WIDTH_VALUE, "45000.f")
        self.assertNotEqual(LOCKED_BEACH_WIDTH_VALUE, "2800.f")
        self.assertNotEqual(LOCKED_BEACH_WIDTH_VALUE, "5200.f")
        self.assertNotEqual(LOCKED_BEACH_WIDTH_VALUE, "7500.f")
        self.assertNotEqual(LOCKED_BEACH_WIDTH_VALUE, "18000.f")
        self.assertNotEqual(LOCKED_BEACH_WIDTH_VALUE, "20000.f")
