...
# INDEX_NONE or lock GetConvoyWorldLocation
# construction in the .cpp. origin/main has no
# inline body; do not invent a body. This is
# leftover-safe Mission03 Convoy
# GetConvoyWorldLocation on
# ASkyguardMission03IntegrationDirector. It is
# NOT leftover convoy-route-state enum #d34e
# (do not lock enum values), NOT leftover
# Mission03 wave-state enum #0242 (do not lock
# enum values), NOT leftover Harbor Mission02 /
# leftover ESkyguardMission02WaveState / leftover
# Harbor #6/#8/#9, NOT leftover BindRuntimeActors
# (retired live mount in the locked decl), NOT
# leftover GetAircraft (returns the retired live
# mount), and NOT leftover HandleDroneCityImpact.
...
# GetConvoyWorldLocation / GetCurrentWaveIndex /
# GetRemainingThreatsInWave / GetConvoyRouteState /
# GetConvoyRouteAlpha / GetConvoyIntegrity /
# GetDayBeatKit. Do not lock
# leftover Mission01 GetGunner / GetPathfinder.
...
# #614, leftover environment-readiness defaults
# #6b9d / #b931, leftover Environment GetReadiness,
# leftover mission-map-get-readiness, leftover
# Mission03 wave-state enum #0242, leftover
# convoy-route-state enum #d34e.
# origin/main has no inline body; do not invent
# a body. Accept origin/main one-line /
# split-line UFUNCTION wraps. Nearby origin/main
...
# Waves, not Integration, not Environment, not
...
# isolated-test drafts #107–#641, leftover Apache
...
# Mission03 wave-state enum #0242 values.
# Do not lock leftover convoy-route-state enum
# #d34e values.
...
# contracts, leftover Mission01 GetReadiness
# #614, leftover Environment GetReadiness,
# leftover mission-map-get-readiness, leftover
# Harbor Mission02, sibling
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission03IntegrationDirector::GetConvoyWorldLocation",
    "SkyguardMission03IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission03GetConvoyWorldLocationDeclContractTests(unittest.TestCase):
    def test_mission03_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_CONVOY_WORLD_LOCATION),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_CONVOY_WORLD_LOCATION)
        self.assertIn("GetConvoyWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_CONVOY_WORLD_LOCATION)
    ...
    def test_missing_get_convoy_world_location_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission03IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_CONVOY_WORLD_LOCATION)
        self.assertIn("GetConvoyWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_CONVOY_WORLD_LOCATION)
        self.assertIn("GetConvoyWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("BlueprintPure", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Category", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("BlueprintCallable", UFUNCTION_CONVOY)
        self.assertIn("Skyguard|Mission03|Convoy", UFUNCTION_CONVOY)
        self.assertNotIn("Mission01", UFUNCTION_CONVOY)
        ...
        self.assertIn("BlueprintPure", UFUNCTION_CONVOY)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_CONVOY_WORLD_LOCATION)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_CONVOY_WORLD_LOCATION)
        self.assertIn("GetConvoyWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid GetConvoyWorldLocation() const;\n"
        as_bool = "\tbool GetConvoyWorldLocation() const;\n"
        as_const = (
            "\tFVector "
        ...
        with_arg = (
            "\tFVector "
            "GetConvoyWorldLocation(float DeltaSeconds) const;\n"
        )
        renamed = (
            "\tFVector "
            "GetReady() const;\n"
        )
        short_name = "\tFSkyguardMission03IntegrationReadiness Readiness;\n"
        ...
        leftover_m01_ready = f"\t{M01_GET_READINESS}\n"
        ...
        leftover_route_alpha = f"\t{GET_CONVOY_ROUTE_ALPHA}\n"
        leftover_integrity = f"\t{GET_CONVOY_INTEGRITY}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_CONVOY_WORLD_LOCATION)
            self.assertIn("GetConvoyWorldLocation", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_convoy_world_location_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_CONVOY_WORLD_LOCATION),
            GET_CONVOY_WORLD_LOCATION,
        )
        self.assertTrue(has_declaration(section, GET_CONVOY_WORLD_LOCATION))
        self.assertEqual(
            declaration_count(section, GET_CONVOY_WORLD_LOCATION),
            1,
        )
        self.assertTrue(
            GET_CONVOY_WORLD_LOCATION.startswith("FVector"),
            GET_CONVOY_WORLD_LOCATION,
        )
        self.assertTrue(
            GET_CONVOY_WORLD_LOCATION.endswith(" const;"),
            GET_CONVOY_WORLD_LOCATION,
        )
        self.assertTrue(
            GET_CONVOY_WORLD_LOCATION.endswith(";"),
            GET_CONVOY_WORLD_LOCATION,
        )
        self.assertIn("GetConvoyWorldLocation", GET_CONVOY_WORLD_LOCATION)
        self.assertIn("GetConvoyWorldLocation()", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn(
            "ESkyguardMission03WaveState",
            GET_CONVOY_WORLD_LOCATION,
        )
        self.assertNotIn("EnvironmentReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("MissionMapReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Mission01IntegrationReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Mission03IntegrationReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("TArray<", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("TObjectPtr", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("TSoftObjectPtr", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("INDEX_NONE", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("UFUNCTION", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("{", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("}", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("return ", GET_CONVOY_WORLD_LOCATION)
        self.assertIn("const;", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Root", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Briefing", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("AudioDirector", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("RadioChatter", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("SortiePresentation", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("CampaignDefinition", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("MissionDefinition", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Readiness;", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("bAutoInitialize", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("PathfinderSpawnLocation", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetAircraft", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("BindRuntimeActors", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("HandleDroneCityImpact", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("StartNextWave", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("NotifyThreatDestroyed", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("AdvanceConvoyByDistance", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("NotifyConvoyDamage", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetWaveState", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetCurrentWaveIndex", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetRemainingThreatsInWave", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetConvoyRouteState", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetConvoyRouteAlpha", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetConvoyIntegrity", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetDayBeatKit", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("RoadHunterSpawnLocation", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ConfigureMissionDefinition", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("NotifyObjectiveProgress", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("InitializePlayableMission", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("IsCorePlayableReady", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("SynchronizeRuntimeState", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetObjectiveRuntime", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetGunner", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetPathfinder", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetMissionId", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ValidateMissionContract", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ConfigureFromMission", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("AdvanceBriefing", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("SetAssetsReady", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("AcknowledgeAndLaunch", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("CanLaunch", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetElapsedSeconds", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetBriefingState", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetBriefingText", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetRadioChatter", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetPresentation", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("GetMissionTitle", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("AcknowledgeBriefing", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("LaunchSortie", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("HullCollider", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("OpticalTracker", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("WeaponServo", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("CountermeasurePod", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("MinHeightFromOriginCm", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("RadarNode", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ESkyguardMission02WaveState", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("HarborIndustrial", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("MaxIntegrity", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("CurrentIntegrity", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("FillAndFinalize", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("FillAndFail", GET_CONVOY_WORLD_LOCATION)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_CONVOY_WORLD_LOCATION)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_CONVOY} {GET_CONVOY_WORLD_LOCATION}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_CONVOY_WORLD_LOCATION}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_CONVOY_WORLD_LOCATION))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_CONVOY_WORLD_LOCATION),
            section,
    ...
    def test_environment_category_does_not_satisfy_waves(self) -> None:
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_CONVOY)
        self.assertNotEqual(leftover_m02, UFUNCTION_CONVOY)
        self.assertNotEqual(leftover_harbor, UFUNCTION_CONVOY)
        self.assertNotEqual(leftover_integration, UFUNCTION_CONVOY)
        self.assertNotEqual(leftover_waves, UFUNCTION_CONVOY)
        self.assertNotIn(leftover_m01, UFUNCTION_CONVOY)
        self.assertNotIn(leftover_m02, UFUNCTION_CONVOY)
        self.assertNotIn(leftover_harbor, UFUNCTION_CONVOY)
        self.assertNotIn(leftover_integration, UFUNCTION_CONVOY)
        self.assertNotIn(leftover_waves, UFUNCTION_CONVOY)
        self.assertNotIn("Mission01", UFUNCTION_CONVOY)
        self.assertNotIn("Integration", UFUNCTION_CONVOY)
        self.assertNotIn("Waves", UFUNCTION_CONVOY)
        self.assertIn("Convoy", UFUNCTION_CONVOY)
        self.assertNotIn("Objectives", UFUNCTION_CONVOY)

    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_CONVOY_WORLD_LOCATION)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_CONVOY_WORLD_LOCATION)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_convoy_world_location_cpp_body(self) -> None:
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
        self.assertNotIn("{", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("}", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("return ", GET_CONVOY_WORLD_LOCATION)
        ...
        self.assertNotIn("return Other", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("return WaveState", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("return WaveState", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("return Readiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("return false", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("return true", GET_CONVOY_WORLD_LOCATION)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONVOY_WORLD_LOCATION)
        self.assertIn("GetConvoyWorldLocation", GET_CONVOY_WORLD_LOCATION)
        self.assertTrue(GET_CONVOY_WORLD_LOCATION.startswith("FVector"))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_CONVOY_WORLD_LOCATION)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_mission03_wave_state_enum(
        self,
    ) -> None:
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
        for token in LEFTOVER_MISSION03_WAVE_STATE_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn(
            "ESkyguardMission03WaveState",
    ...
    def test_contract_does_not_relock_leftover_convoy_route_state_enum(
        self,
    ) -> None:
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
        for token in LEFTOVER_CONVOY_ROUTE_STATE_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn(
            "ESkyguardConvoyRouteState",
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("CurrentIntegrity", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_CONVOY_WORLD_LOCATION)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_CONVOY_WORLD_LOCATION)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_CONVOY_WORLD_LOCATION),
            GET_CONVOY_WORLD_LOCATION,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CONVOY_WORLD_LOCATION)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission03 GetConvoyWorldLocation method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_CONVOY_WORLD_LOCATION.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_CONVOY_WORLD_LOCATION.lower(),
                "mission03 GetConvoyWorldLocation contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_CONVOY_WORLD_LOCATION)

    def test_contract_is_get_convoy_world_location_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(GET_CONVOY_WORLD_LOCATION.startswith("FVector"))
        self.assertIn("GetConvoyWorldLocation", GET_CONVOY_WORLD_LOCATION)
        self.assertIn("GetConvoyWorldLocation() const", GET_CONVOY_WORLD_LOCATION)
        self.assertTrue(GET_CONVOY_WORLD_LOCATION.endswith(" const;"))
        self.assertTrue(GET_CONVOY_WORLD_LOCATION.endswith(";"))
        self.assertNotIn("EnvironmentReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("MissionMapReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Mission01IntegrationReadiness", GET_CONVOY_WORLD_LOCATION)
        self.assertNotIn("Mission03IntegrationReadiness", GET_CONVOY_WORLD_LOCATION)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_convoy_world_location(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_CONVOY_WORLD_LOCATION)
            )
            self.assertNotEqual(GET_CONVOY_WORLD_LOCATION, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetConvoyWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_CONVOY_WORLD_LOCATION))
        locked_only = f"{GET_CONVOY_WORLD_LOCATION}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tFVector "
            "GetConvoyWorldLocation() const { return Readiness; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission03_get_convoy_world_location"
            "_decl_contract.py"
        )
