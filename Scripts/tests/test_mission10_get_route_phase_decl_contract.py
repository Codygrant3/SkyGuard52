...
# INDEX_NONE or lock GetRoutePhase
# construction in the .cpp. This is leftover-safe
# Mission10 Waves GetRoutePhase METHOD on
# ASkyguardMission10IntegrationDirector. It is
# NOT leftover Mission10 route-phase enum #701a /
# branch cursor/mission10-route-phase-enum-
# contract-701a, NOT leftover Mission09
# GetWaveState #760 / branch cursor/mission09-
# get-wave-state-decl-contract-c332, NOT leftover
# Mission08 GetWaveState #741, NOT leftover
# Mission10 GetReadiness, NOT leftover Mission09
# GetReadiness #758, NOT leftover Mission08
...
# leftover Mission01 / Mission03 GetReadiness,
...
# GetObjectiveRuntime /
# GetReadiness / GetWaveState / GetRemainingThreatsInWave /
...
# GetRoutePhase() const { return RoutePhase; }`);
...
# Mission09 / Integration / Search / Protection / Payload /
# Targets / Objectives / Boss / Destruction / Briefing /
...
# Protection / Performance / Objectives, not leftover
# Mission10 Integration GetReadiness.
...
# drafts #674–#777+, leftover Apache
...
# Mission10 route-phase enum #701a — THIS IS THE
# METHOD, NOT THE ENUM. Do not reopen leftover
# Mission09 GetWaveState #760 or leftover
# Mission08 GetWaveState #741. This API is
# GetRoutePhase, not GetWaveState. Do not reopen leftover
...
# #658–#673. Do not reopen open drafts #674–#777+.
...
# leftover Mission10 GetReadiness,
# leftover Mission09 GetWaveState #760,
...
# an isolated Python GetRoutePhase
...
LEFTOVER_MISSION10_ROUTE_PHASE_ENUM_NOT_LOCKED = (
    ...
    "cursor/mission10-route-phase-enum-contract-701a",
)
...
CPP_AND_INVENTED = (
    ...
    "SkyguardMission09IntegrationDirector.cpp",
    "ASkyguardMission10IntegrationDirector::GetRoutePhase",
    "SkyguardMission10IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission10GetRoutePhaseDeclContractTests(unittest.TestCase):
    def test_mission10_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_ROUTE_PHASE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_ROUTE_PHASE)
    ...
    def test_missing_get_route_phase_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_ROUTE_PHASE)
        self.assertNotIn("BlueprintPure", GET_ROUTE_PHASE)
        self.assertNotIn("Category", GET_ROUTE_PHASE)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_ROUTE_PHASE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetRoutePhase() const;\n"
        )
        as_const = (
        ...
        with_arg = (
            "\tconst FSkyguardMission09IntegrationReadiness& "
            "GetRoutePhase(int32 Amount) const;\n"
        )
        renamed = (
            "\tconst FSkyguardMission09IntegrationReadiness& "
            "GetReadyState() const;\n"
        )
        short_name = "\tbool GetRoutePhase() const;\n"
        leftover_m06_type = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_ROUTE_PHASE)
            self.assertIn("GetRoutePhase", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_route_phase_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_ROUTE_PHASE),
            GET_ROUTE_PHASE,
        )
        self.assertTrue(has_declaration(section, GET_ROUTE_PHASE))
        self.assertEqual(
            declaration_count(section, GET_ROUTE_PHASE),
            1,
        )
        self.assertTrue(
            GET_ROUTE_PHASE.startswith("ESkyguardMission10RoutePhase"),
            GET_ROUTE_PHASE,
        )
        self.assertTrue(
            GET_ROUTE_PHASE.startswith("ESkyguardMission10RoutePhase"),
        ...
        self.assertIn("ESkyguardMission10RoutePhase", GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", GET_ROUTE_PHASE)
        self.assertNotIn(";", GET_ROUTE_PHASE)
        self.assertIn("return RoutePhase;", section)
        self.assertIn(UFUNCTION_WAVES, section)
        self.assertIn(" const", GET_ROUTE_PHASE)
        self.assertIn("()", GET_ROUTE_PHASE)
        self.assertNotIn("TArray<", GET_ROUTE_PHASE)
        self.assertNotIn("TObjectPtr", GET_ROUTE_PHASE)
        self.assertNotIn("TSoftObjectPtr", GET_ROUTE_PHASE)
        self.assertNotIn("INDEX_NONE", GET_ROUTE_PHASE)
        self.assertNotIn("UFUNCTION", GET_ROUTE_PHASE)
        self.assertNotIn("{", GET_ROUTE_PHASE)
        self.assertNotIn("}", GET_ROUTE_PHASE)
        self.assertNotIn("return ", GET_ROUTE_PHASE)
        self.assertIn(" const", GET_ROUTE_PHASE)
        self.assertNotIn("Root", GET_ROUTE_PHASE)
        self.assertNotIn("Briefing", GET_ROUTE_PHASE)
        self.assertNotIn("AudioDirector", GET_ROUTE_PHASE)
        self.assertNotIn("RadioChatter", GET_ROUTE_PHASE)
        self.assertNotIn("SortiePresentation", GET_ROUTE_PHASE)
        self.assertNotIn("CampaignDefinition", GET_ROUTE_PHASE)
        self.assertNotIn("MissionDefinition", GET_ROUTE_PHASE)
        self.assertNotIn("Readiness;", GET_ROUTE_PHASE)
        self.assertNotIn("bAutoInitialize", GET_ROUTE_PHASE)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_ROUTE_PHASE)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_ROUTE_PHASE)
        self.assertNotIn("PathfinderSpawnLocation", GET_ROUTE_PHASE)
        self.assertNotIn("GetAircraft", GET_ROUTE_PHASE)
        self.assertNotIn("BindRuntimeActors", GET_ROUTE_PHASE)
        self.assertNotIn("HandleDroneCityImpact", GET_ROUTE_PHASE)
        self.assertNotIn("InitializePlayableMission", GET_ROUTE_PHASE)
        self.assertNotIn("StartNextWave", GET_ROUTE_PHASE)
        self.assertNotIn("AdvanceConvoyByDistance", GET_ROUTE_PHASE)
        self.assertNotIn("NotifyConvoyDamage", GET_ROUTE_PHASE)
        self.assertNotIn("GetWaveState", GET_ROUTE_PHASE)
        self.assertNotIn("GetReadiness", GET_ROUTE_PHASE)
        self.assertNotIn("GetConvoyRouteState", GET_ROUTE_PHASE)
        self.assertNotIn("GetDayBeatKit", GET_ROUTE_PHASE)
        self.assertNotIn("GetNightBeatKit", GET_ROUTE_PHASE)
        self.assertNotIn("StartSearchlightWindow", GET_ROUTE_PHASE)
        self.assertNotIn("AdvanceSearchlightTrack", GET_ROUTE_PHASE)
        self.assertNotIn("NotifySubstationDamage", GET_ROUTE_PHASE)
        self.assertNotIn("GetRemainingThreatsInWave", GET_ROUTE_PHASE)
        self.assertNotIn("GetSearchlightRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("GetSubstationIntegrity", GET_ROUTE_PHASE)
        self.assertNotIn("StartPayloadWindow", GET_ROUTE_PHASE)
        self.assertNotIn("AdvancePayloadWindow", GET_ROUTE_PHASE)
        self.assertNotIn("TryJamActivePayload", GET_ROUTE_PHASE)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_ROUTE_PHASE)
        self.assertNotIn("GetPayloadWindow", GET_ROUTE_PHASE)
        self.assertNotIn("GetTargetRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("GetSurvivingTargetCount", GET_ROUTE_PHASE)
        self.assertNotIn("GetDayBeatKind", GET_ROUTE_PHASE)
        self.assertNotIn("GetDayBeatIndex", GET_ROUTE_PHASE)
        self.assertNotIn("TickDayBeatKit", GET_ROUTE_PHASE)
        self.assertNotIn("HandleBossPhaseChanged", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission07IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission06WaveState", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission07WaveState", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission08WaveState", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission09WaveState", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission08ProtectedTarget", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission09ProtectedTarget", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardMission08ProtectedTargetRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardMission09ProtectedTargetRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardMission08IntegrationReadiness", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardMission09IntegrationReadiness", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardMission09PoolRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardMission09PoolBudget", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission08IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("BindCampaignRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("GetPoolRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardSearchSector", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardSearchTrackRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("ClassifyFalseTrack", GET_ROUTE_PHASE)
        self.assertNotIn("ConfirmRadarGhostIdentification", GET_ROUTE_PHASE)
        self.assertNotIn("NotifyProtectedTargetDamage", GET_ROUTE_PHASE)
        self.assertNotIn("AdvanceReinforcementTimer", GET_ROUTE_PHASE)
        self.assertNotIn("GetSearchSector", GET_ROUTE_PHASE)
        self.assertNotIn("GetClassifiedFalseTrackCount", GET_ROUTE_PHASE)
        self.assertNotIn("IsHostileContactConfirmed", GET_ROUTE_PHASE)
        self.assertNotIn("GetProtectedTarget", GET_ROUTE_PHASE)
        self.assertNotIn("GetReinforcementTimeRemaining", GET_ROUTE_PHASE)
        self.assertNotIn("GetNightBeatKind", GET_ROUTE_PHASE)
        self.assertNotIn("GetNightBeatIndex", GET_ROUTE_PHASE)
        self.assertNotIn("TickNightBeatKit", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardAirfieldTarget", GET_ROUTE_PHASE)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_ROUTE_PHASE)
        self.assertNotIn("RoadHunterSpawnLocation", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_ROUTE_PHASE)
        self.assertNotIn("ConfigureMissionDefinition", GET_ROUTE_PHASE)
        self.assertNotIn("NotifyObjectiveProgress", GET_ROUTE_PHASE)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_ROUTE_PHASE)
        self.assertNotIn("SynchronizeRuntimeState", GET_ROUTE_PHASE)
        self.assertNotIn("NotifyThreatDestroyed", GET_ROUTE_PHASE)
        self.assertNotIn("IsCorePlayableReady", GET_ROUTE_PHASE)
        self.assertNotIn("GetObjectiveRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("GetGunner", GET_ROUTE_PHASE)
        self.assertNotIn("GetPathfinder", GET_ROUTE_PHASE)
        self.assertNotIn("GetMissionId", GET_ROUTE_PHASE)
        self.assertNotIn("ValidateMissionContract", GET_ROUTE_PHASE)
        self.assertNotIn("ConfigureFromMission", GET_ROUTE_PHASE)
        self.assertNotIn("AdvanceBriefing", GET_ROUTE_PHASE)
        self.assertNotIn("SetAssetsReady", GET_ROUTE_PHASE)
        self.assertNotIn("AcknowledgeAndLaunch", GET_ROUTE_PHASE)
        self.assertNotIn("CanLaunch", GET_ROUTE_PHASE)
        self.assertNotIn("GetElapsedSeconds", GET_ROUTE_PHASE)
        self.assertNotIn("GetBriefingState", GET_ROUTE_PHASE)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_ROUTE_PHASE)
        self.assertNotIn("GetBriefingText", GET_ROUTE_PHASE)
        self.assertNotIn("GetRadioChatter", GET_ROUTE_PHASE)
        self.assertNotIn("GetPresentation", GET_ROUTE_PHASE)
        self.assertNotIn("GetMissionTitle", GET_ROUTE_PHASE)
        self.assertNotIn("AcknowledgeBriefing", GET_ROUTE_PHASE)
        self.assertNotIn("LaunchSortie", GET_ROUTE_PHASE)
        self.assertNotIn("HullCollider", GET_ROUTE_PHASE)
        self.assertNotIn("OpticalTracker", GET_ROUTE_PHASE)
        self.assertNotIn("WeaponServo", GET_ROUTE_PHASE)
        self.assertNotIn("CountermeasurePod", GET_ROUTE_PHASE)
        self.assertNotIn("MinHeightFromOriginCm", GET_ROUTE_PHASE)
        self.assertNotIn("RadarNode", GET_ROUTE_PHASE)
        self.assertNotIn("ESkyguardMission02WaveState", GET_ROUTE_PHASE)
        self.assertNotIn("HarborIndustrial", GET_ROUTE_PHASE)
        self.assertNotIn("MaxIntegrity", GET_ROUTE_PHASE)
        self.assertNotIn("CurrentIntegrity", GET_ROUTE_PHASE)
        self.assertNotIn("FillAndFinalize", GET_ROUTE_PHASE)
        self.assertNotIn("FillAndFail", GET_ROUTE_PHASE)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_ROUTE_PHASE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_WAVES} {GET_ROUTE_PHASE}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_ROUTE_PHASE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_ROUTE_PHASE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_ROUTE_PHASE),
            section,
    ...
    def test_environment_category_does_not_satisfy_waves(self) -> None:
        ...
        self.assertNotEqual(leftover_m08_waves, UFUNCTION_WAVES)
        ...
        self.assertNotEqual(leftover_m08_ready, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09_ready, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09_waves, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m10_safety, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09_protection, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09_performance, UFUNCTION_WAVES)
        self.assertNotEqual(leftover_m09_objectives, UFUNCTION_WAVES)
        ...
        self.assertNotIn(leftover_m08_waves, UFUNCTION_WAVES)
        ...
        self.assertNotIn(leftover_m08_ready, UFUNCTION_WAVES)
        self.assertNotIn(leftover_m09_ready, UFUNCTION_WAVES)
        self.assertNotIn(leftover_m09_waves, UFUNCTION_WAVES)
        self.assertNotIn(leftover_m10_safety, UFUNCTION_WAVES)
        self.assertNotIn(leftover_m09_protection, UFUNCTION_WAVES)
        self.assertNotIn(leftover_m09_performance, UFUNCTION_WAVES)
        self.assertNotIn(leftover_m09_objectives, UFUNCTION_WAVES)
        self.assertNotIn("Mission01", UFUNCTION_WAVES)
        self.assertNotIn("Mission03", UFUNCTION_WAVES)
        self.assertNotIn("Mission04", UFUNCTION_WAVES)
        self.assertNotIn("Mission05", UFUNCTION_WAVES)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_ROUTE_PHASE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_route_phase_cpp_body(self) -> None:
        locked_only = f"{GET_ROUTE_PHASE}\n"
        self.assertNotIn("{", GET_ROUTE_PHASE)
        self.assertNotIn("}", GET_ROUTE_PHASE)
        self.assertNotIn("return ", GET_ROUTE_PHASE)
        ...
        self.assertNotIn("return false", GET_ROUTE_PHASE)
        self.assertNotIn("return true", GET_ROUTE_PHASE)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_ROUTE_PHASE}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", GET_ROUTE_PHASE)
        self.assertTrue(GET_ROUTE_PHASE.startswith("ESkyguardMission10RoutePhase"))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_ROUTE_PHASE)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_ROUTE_PHASE)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_ROUTE_PHASE)
        self.assertNotIn("CurrentIntegrity", GET_ROUTE_PHASE)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_ROUTE_PHASE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_ROUTE_PHASE)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn("550.f", GET_ROUTE_PHASE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_ROUTE_PHASE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_ROUTE_PHASE),
            GET_ROUTE_PHASE,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_PHASE)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardIronRainBoss", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardRadarGhostBoss", GET_ROUTE_PHASE)
        self.assertNotIn("ASkyguardTempestBoss", section)
        self.assertNotIn("ASkyguardLastFlightBoss", GET_ROUTE_PHASE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission10 GetRoutePhase method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_ROUTE_PHASE}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_ROUTE_PHASE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_ROUTE_PHASE.lower(),
                "mission10 GetRoutePhase contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_ROUTE_PHASE)

    def test_contract_is_get_route_phase_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_route_phase(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_ROUTE_PHASE)
            )
            self.assertNotEqual(GET_ROUTE_PHASE, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetRoutePhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_ROUTE_PHASE))
        locked_only = f"{GET_ROUTE_PHASE}\n"
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_get_route_phase"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_mission07_get_readiness_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_m07_type, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", str(raised.exception))
    ...
    def test_sibling_mission08_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("NotifyThreatDestroyed", GET_ROUTE_PHASE)
        self.assertNotIn("StartNextWave", GET_ROUTE_PHASE)
        self.assertNotIn("StartHoistWindow", GET_ROUTE_PHASE)
        self.assertNotIn("AdvanceHoistWindow", GET_ROUTE_PHASE)
        self.assertNotIn("ValidateWeaponRelease", GET_ROUTE_PHASE)
        self.assertNotIn("IsCorePlayableReady", GET_ROUTE_PHASE)
        self.assertNotIn("SynchronizeRuntimeState", GET_ROUTE_PHASE)
        self.assertNotIn("GetObjectiveRuntime", GET_ROUTE_PHASE)
        self.assertNotIn("GetWaveState", GET_ROUTE_PHASE)
    ...
    def test_leftover_mission08_get_readiness_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_m08_type, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", str(raised.exception))
        self.assertNotIn(
            "cursor/mission08-get-readiness-decl-contract-c332",
            GET_ROUTE_PHASE,
    ...
    def test_leftover_mission09_get_readiness_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_m09_type, GET_ROUTE_PHASE)
        self.assertIn("GetRoutePhase", str(raised.exception))
        self.assertNotIn(
            "cursor/mission09-get-readiness-decl-contract-c332",
            GET_ROUTE_PHASE,
    ...
    def test_contract_does_not_relock_leftover_mission10_route_phase_enum(
        ...
        self.assertIn("GetRoutePhase", GET_ROUTE_PHASE)
        self.assertIn("ESkyguardMission10RoutePhase", GET_ROUTE_PHASE)
        self.assertNotIn("UENUM", GET_ROUTE_PHASE)
        self.assertNotIn("enum class", GET_ROUTE_PHASE)
        self.assertNotIn(
            "cursor/mission10-route-phase-enum-contract-701a",
            GET_ROUTE_PHASE,
        )
        # METHOD, not leftover route-phase enum #701a.
    ...
    def test_contract_does_not_relock_leftover_mission10_protected_group(
        ...
        self.assertNotIn("Protection", UFUNCTION_WAVES)
        self.assertNotIn("GetProtectedGroup", GET_ROUTE_PHASE)
    ...
    def test_leftover_mission10_sibling_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("HandleDroneCityImpact", GET_ROUTE_PHASE)
        self.assertNotIn("UFUNCTION", GET_DAY_BEAT_KIT)
        self.assertNotIn("StartPhaseWave", GET_ROUTE_PHASE)
        self.assertNotIn("InitializePlayableMission", GET_ROUTE_PHASE)
        self.assertNotIn("ValidateWeaponRelease", GET_ROUTE_PHASE)
        self.assertNotIn("GetReadiness", GET_ROUTE_PHASE)
        self.assertNotIn("GetWaveState", GET_ROUTE_PHASE)
