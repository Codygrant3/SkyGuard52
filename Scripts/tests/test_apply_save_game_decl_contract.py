...
# Declaration presence only. Do not invent INDEX_NONE, save-game
# payloads, slot-save behavior, or lock the ApplySaveGame body
...
# This lane only adds an isolated Python ApplySaveGame
# declaration contract. Stay off BuildSaveGame /
# SaveCampaignToSlot / LoadCampaignFromSlot /
# DeleteCampaignSlot / RetrySaveLastDebrief / GetMissionRecords
# (sibling in-flight this wave), leftover campaign-save
# empty-fail-closed, leftover campaign-roster lookup #111,
# LoadCampaignProgressAfterConfigure (#290),
# CalculateMissionScore / CalculateMedalTier, GetRouteRuntime,
# FillResultCombatStats / FillAndFinalize / FillAndFail /
# ApplyHydraForClusters (leftover ASkyguardGunner*), leftover
# CPG debrief #284/#195/#130/#8ccd, leftover
...
# Yak leftover live copy, FSkyguardMission0NIntegrationReadiness
...
# BuildSaveGame / SaveCampaignToSlot / LoadCampaignFromSlot /
# DeleteCampaignSlot / RetrySaveLastDebrief / GetMissionRecords
# are sibling in-flight this wave. FillResultCombatStats takes
# leftover ASkyguardGunner*. Leftover CPG debrief
# #284/#195/#130/#8ccd stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# .cpp ApplySaveGame body / invented save-game payloads /
# slot-save behavior stay unlocked. Do not invent INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "LoadGameFromSlot",
    "DoesSaveGameExist",
    "USkyguardCampaignSubsystem::ApplySaveGame",
    "SkyguardCampaignSubsystem.cpp",
)
...
class ApplySaveGameDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, APPLY_SAVE_GAME), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, APPLY_SAVE_GAME)
        self.assertIn("ApplySaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, APPLY_SAVE_GAME))

    def test_missing_apply_save_game_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, APPLY_SAVE_GAME)
        self.assertIn("ApplySaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, APPLY_SAVE_GAME)
        self.assertIn("ApplySaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_neighbor_save_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_saves, APPLY_SAVE_GAME)
        self.assertIn("ApplySaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = "\tbool ApplySaveGame();\n"
        missing_const = (
            "\tbool ApplySaveGame(USkyguardCampaignSaveGame* SaveGame);\n"
        )
        wrong_return = (
            "\tvoid ApplySaveGame("
        ...
        slot_only = (
            "\tbool ApplySaveGame(const FString& SlotName);\n"
        )
        for region in (
            missing_arg,
            missing_const,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, APPLY_SAVE_GAME)
            self.assertIn("ApplySaveGame", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_apply_save_game_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, APPLY_SAVE_GAME),
            APPLY_SAVE_GAME,
        )
        self.assertTrue(has_declaration(section, APPLY_SAVE_GAME))
        self.assertEqual(declaration_count(section, APPLY_SAVE_GAME), 1)
        self.assertTrue(APPLY_SAVE_GAME.endswith(";"), APPLY_SAVE_GAME)
        self.assertTrue(APPLY_SAVE_GAME.startswith("bool "), APPLY_SAVE_GAME)
        self.assertIn(
            "const USkyguardCampaignSaveGame* SaveGame",
            APPLY_SAVE_GAME,
        )
        self.assertNotIn("INDEX_NONE", APPLY_SAVE_GAME)
        self.assertNotIn("{", APPLY_SAVE_GAME)
        self.assertNotIn("}", APPLY_SAVE_GAME)
        self.assertNotIn("return ", APPLY_SAVE_GAME)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_arg = (
            "public:\n"
            "\tbool ApplySaveGame(const USkyguardCampaignSaveGame*\n"
            "\t\tSaveGame);\n"
            "};\n"
        )
        wrap_ptr = (
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, APPLY_SAVE_GAME), 1)
        one_line = f"{{\npublic:\n\t{APPLY_SAVE_GAME}\n}}\n"
        self.assertTrue(has_declaration(one_line, APPLY_SAVE_GAME))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, APPLY_SAVE_GAME), section)
        self.assertEqual(
            require_declaration(section, APPLY_SAVE_GAME),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{APPLY_SAVE_GAME}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", APPLY_SAVE_GAME)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", APPLY_SAVE_GAME)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_save_payloads_or_slot_behavior(
        ...
        locked_only = f"{APPLY_SAVE_GAME}\n"
        self.assertNotIn("return ", APPLY_SAVE_GAME)
        self.assertNotIn("return false", APPLY_SAVE_GAME)
        self.assertNotIn("return true", APPLY_SAVE_GAME)
        self.assertNotIn("SlotName", APPLY_SAVE_GAME)
        self.assertNotIn("UserIndex", APPLY_SAVE_GAME)
        for token in SAVE_PAYLOADS_AND_SLOT_NOT_LOCKED:
            self.assertNotIn(token, APPLY_SAVE_GAME)
        ...
        self.assertNotIn('TEXT("Skyguard52Campaign")', APPLY_SAVE_GAME)

    def test_contract_does_not_lock_apply_save_game_cpp_body(self) -> None:
        locked_only = f"{APPLY_SAVE_GAME}\n"
        self.assertNotIn("{", APPLY_SAVE_GAME)
        self.assertNotIn("}", APPLY_SAVE_GAME)
        self.assertNotIn("return ", APPLY_SAVE_GAME)
        ...
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", APPLY_SAVE_GAME)
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", locked_only)
        self.assertNotIn("DuplicateObject", APPLY_SAVE_GAME)
        self.assertNotIn("MigrateCampaignSave", APPLY_SAVE_GAME)
        self.assertNotIn("MissionRecords.Reset", APPLY_SAVE_GAME)
        self.assertNotIn("ClearActiveMissionRuntime", APPLY_SAVE_GAME)
        self.assertNotIn("GetTransientPackage", APPLY_SAVE_GAME)

    def test_contract_does_not_relock_get_active_mission(self) -> None:
        locked_only = f"{APPLY_SAVE_GAME}\n"
        for neighbor in GET_ACTIVE_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn(
            "USkyguardMissionDefinition* GetActiveMission() const",
            APPLY_SAVE_GAME,
        )
    ...
    def test_contract_does_not_relock_configure_can_start_or_start(self) -> None:
        ...
        for neighbor in CONFIGURE_CAN_START_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("ConfigureCampaign", APPLY_SAVE_GAME)
        self.assertNotIn("CanStartMission", APPLY_SAVE_GAME)
        self.assertNotIn("StartMission", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_is_mission_unlocked(self) -> None:
        ...
        for neighbor in IS_MISSION_UNLOCKED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("IsMissionUnlocked", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("AddObjectiveProgress", APPLY_SAVE_GAME)
        self.assertNotIn("FailObjective", APPLY_SAVE_GAME)
        self.assertNotIn("CompleteSurviveObjectiveIfIntact", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("CompleteActiveMission", APPLY_SAVE_GAME)
        self.assertNotIn("FinalizeActiveMission", APPLY_SAVE_GAME)
        self.assertNotIn("FailActiveMission", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_fill_result_or_gunner_paths(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("FillResultCombatStats", APPLY_SAVE_GAME)
        self.assertNotIn("ASkyguardGunner", APPLY_SAVE_GAME)
        self.assertNotIn("FillAndFinalize", APPLY_SAVE_GAME)
        self.assertNotIn("FillAndFail", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("AcknowledgeDebrief", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_debrief_neighbors(self) -> None:
        ...
        for neighbor in DEBRIEF_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("RetrySaveLastDebrief", APPLY_SAVE_GAME)
        self.assertNotIn("GetLastDebrief", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_travel_helpers(self) -> None:
        ...
        for neighbor in TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("CanTravelToNextMission", APPLY_SAVE_GAME)
        self.assertNotIn("GetNextMissionMapPackageName", APPLY_SAVE_GAME)
        self.assertNotIn("TravelToNextMission", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_runtime_getters(self) -> None:
        ...
        for neighbor in RUNTIME_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("GetObjectiveRuntime", APPLY_SAVE_GAME)
        self.assertNotIn("GetRouteRuntime", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_save_game_siblings(self) -> None:
        ...
        for neighbor in SAVE_GAME_SIBLINGS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("BuildSaveGame", APPLY_SAVE_GAME)
        self.assertNotIn("SaveCampaignToSlot", APPLY_SAVE_GAME)
        self.assertNotIn("LoadCampaignFromSlot", APPLY_SAVE_GAME)
        self.assertNotIn("DeleteCampaignSlot", APPLY_SAVE_GAME)
        self.assertNotIn("RetrySaveLastDebrief", APPLY_SAVE_GAME)
        self.assertNotIn("GetMissionRecords", APPLY_SAVE_GAME)
        self.assertNotIn("IsValidCampaignSlotName", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_leftover_campaign_save_empty(
        ...
        for token in LEFTOVER_CAMPAIGN_SAVE_EMPTY_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_SAVE_GAME)
            self.assertNotIn(token, section)
        self.assertNotIn("empty-file", APPLY_SAVE_GAME)
        self.assertNotIn("empty-file", locked_only)
        self.assertNotIn("EmptyFailClosed", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_score_or_medal(self) -> None:
        ...
        for neighbor in SCORE_AND_MEDAL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("CalculateMissionScore", APPLY_SAVE_GAME)
        self.assertNotIn("CalculateMedalTier", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_SAVE_GAME)
        self.assertNotIn("GetActiveMissionElapsedSeconds", APPLY_SAVE_GAME)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{APPLY_SAVE_GAME}\n"
        self.assertEqual(
            require_declaration(locked_only, APPLY_SAVE_GAME),
            APPLY_SAVE_GAME,
        )
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign ApplySaveGame contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, APPLY_SAVE_GAME.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", APPLY_SAVE_GAME)

    def test_contract_is_apply_save_game_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
