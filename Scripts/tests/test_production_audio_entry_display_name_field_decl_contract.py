...
# type is FText, no `=`, BlueprintReadWrite, no Category.
...
TARGET = "FText DisplayName;"
TARGET_WRONG_EQ_NONE = "FText DisplayName = FText::GetEmpty();"
TARGET_WRONG_FALSE = "FText DisplayName = false;"
TARGET_WRONG_TRUE = "FText DisplayName = true;"
TARGET_WRONG_ZERO = "FText DisplayName = 0.f;"
TARGET_WRONG_FLOAT = "float DisplayName;"
TARGET_WRONG_FNAME = "FName DisplayName;"
TARGET_WRONG_BOOL = "bool DisplayName;"
TARGET_WRONG_INT = "int32 DisplayName;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FText DisplayName;`.
    # Do not accept `=` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` / leftover float Health when
    # origin/main has a bare FText. Do not accept sibling
    # Category / SourceStatus / Sound fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FText\s+DisplayName\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+DisplayName\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+DisplayName\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+DisplayName\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+DisplayName\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FText\s+DisplayName\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FText DisplayName is missing from "
...
class ProductionAudioEntryDisplayNameFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FText DisplayName;", compact_origin)
        self.assertNotIn("DisplayName = FText::GetEmpty()", compact_origin)
        self.assertNotIn("DisplayName = false", compact_origin)
        self.assertNotIn("DisplayName = true", compact_origin)
        self.assertNotIn("DisplayName = 0.f", compact_origin)
        self.assertNotIn("DisplayName = 160.f", compact_origin)
    ...
    def test_display_name_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText DisplayName"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 DisplayName = 1;\n",
            "\tbool DisplayName;\n",
            "\tfloat DisplayName = " + forty + ";\n",
            "\tfloat DisplayName = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry DisplayName field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "display_name_field_decl_contract.py"
        ))
