# THIS IS leftover-safe FSkyguardGuidedLockRules HelmetLockSeconds.
# origin/main form: BARE plain C++ static constexpr field
# `static constexpr float HelmetLockSeconds = 2.40f;`
# with the paper lock value 2.40f. NOT a UPROPERTY wrap.
...
# is invented. Locked initializer is 2.40f, not analog 1.80f
# and not RocketReloadSeconds 2.3f.
...
# GuidedLockSeconds = 1.80f (keep analog tests in
# LOCKED_SCRIPTS). This slot is HelmetLockSeconds = 2.40f.
...
# Do NOT contract sibling fields SensorLockSeconds /
...
TARGET = "static constexpr float HelmetLockSeconds = 2.40f;"
TARGET_WRONG_ANALOG = (
    "static constexpr float GuidedLockSeconds = 1.80f;"
)
TARGET_WRONG_ANALOG_VALUE = (
    "static constexpr float HelmetLockSeconds = 1.80f;"
)
TARGET_WRONG_ROCKET = (
    "static constexpr float HelmetLockSeconds = 2.3f;"
)
TARGET_WRONG_BARE = "static constexpr float HelmetLockSeconds;"
TARGET_WRONG_NO_STATIC = (
    "constexpr float HelmetLockSeconds = 2.40f;"
)
TARGET_WRONG_NO_CONSTEXPR = (
    "static float HelmetLockSeconds = 2.40f;"
)
TARGET_WRONG_FLOAT_ONLY = "float HelmetLockSeconds = 2.40f;"
TARGET_WRONG_INT = "static constexpr int32 HelmetLockSeconds = 2;"
TARGET_WRONG_BOOL = "static constexpr bool HelmetLockSeconds = true;"
TARGET_WRONG_SHORT = (
    "static constexpr float HelmetLockSeconds = 2.4f;"
)
TARGET_WRONG_NO_F = (
    "static constexpr float HelmetLockSeconds = 2.40;"
)
TARGET_WRONG_SENSOR = (
...
def helmet_lock_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*static\s+constexpr\s+float\s+"
        r"HelmetLockSeconds\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # RocketReloadSeconds = 2.3f, missing static/constexpr,
    ...
    compact = collapsed(region)
    if re.search(
        r"static\s+constexpr\s+float\s+HelmetLockSeconds\s*=\s*2\.40f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"HelmetLockSeconds\s*=\s*1\.80f",
        compact,
    ):
        return False
    if re.search(
        r"HelmetLockSeconds\s*=\s*2\.3f\b",
        compact,
    ):
        return False
    if re.search(
        ...
        return False
    if helmet_lock_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static\s+constexpr\s+float\s+HelmetLockSeconds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for static constexpr float HelmetLockSeconds is "
...
def require_no_uproperty_wrap(section: str) -> None:
    ...
    if re.search(
        ...
        raise AssertionError(
            "UPROPERTY wrap on HelmetLockSeconds is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "HelmetLockSeconds"):
        raise AssertionError(
            "UPROPERTY clone landed on HelmetLockSeconds; locked decl "
            f"is bare {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "HelmetLockSeconds"):
            raise AssertionError(
                f"{token} clone landed on HelmetLockSeconds; locked "
                f"decl is bare {LOCKED_DECL}"
            )


class GuidedLockRulesHelmetLockSecondsFieldDeclContractTests(
    unittest.TestCase
):
    def test_guided_lock_rules_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HelmetLockSeconds"), section)
    ...
    def test_locked_decl_is_plain_constexpr_not_uproperty(self) -> None:
        ...
        self.assertIn("HelmetLockSeconds", LOCKED_DECL)
        self.assertIn("= 2.40f", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("HelmetLockSeconds", str(raised.exception))
    ...
    def test_method_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HelmetLockSeconds"), section)
        self.assertNotIn("PhaseFromProgress", section)
    ...
    def test_missing_helmet_lock_seconds_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_sibling_sensor_lock_does_not_satisfy(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SENSOR}\n"
    ...
    def test_analog_guided_lock_seconds_fails_closed(self) -> None:
        analog = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_ANALOG}\n"
        ...
        rocket_section = spec_section(rocket)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(rocket_section, LOCKED_DECL)
        self.assertIn("HelmetLockSeconds", str(raised.exception))
        ...
        self.assertIn("HelmetLockSeconds=2.40f;", compact_origin)
        self.assertNotIn("HelmetLockSeconds=1.80f", compact_origin)
        self.assertNotIn("GuidedLockSeconds", compact_origin)
        self.assertNotIn("=2.3f", compact_origin)

    def test_helmet_lock_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tstatic constexpr float HelmetLockSecond = 2.40f;\n",
            "\tstatic constexpr float HelmetLockSeconds = "
            + forty
            + ";\n",
            "\tstatic constexpr float HelmetLockSeconds = "
            + eighty
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    helmet_lock_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(helmet_lock_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "guided-lock-rules HelmetLockSeconds field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "helmet_lock_seconds_field_decl_contract.py"
        ))
