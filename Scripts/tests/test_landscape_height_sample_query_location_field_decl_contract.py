...
# QueryLocation FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FVector QueryLocation = FVector::ZeroVector; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeHeightSample
# QueryLocation FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# FVector QueryLocation = FVector::ZeroVector; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# BlueprintReadOnly FVector QueryLocation = FVector::ZeroVector;
...
# QueryLocation FIELD (declaration-only). It is
...
TARGET = "FVector QueryLocation = FVector::ZeroVector;"
TARGET_WRONG = "FVector QueryLocation = FVector::OneVector;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FVector\s+QueryLocation\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FVector QueryLocation is missing from "
...
class LandscapeHeightSampleQueryLocationFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("QueryLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_query_location_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_query_location_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FVector QueryLocation"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("QueryLocation", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("QueryLocation", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FVector QueryLocation"))
    ...
    def test_sibling_director_fields_do_not_satisfy_query_location(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
