...
# origin/main form: split-line static BlueprintCallable wrap
# UFUNCTION(BlueprintCallable, Category="Skyguard|Mission01|Environment|Validation")
# then static FSkyguardLandscapeVisibleAudit
# SetTransientLandscapeDiagnosticMaterialSynchronized(
#   ALandscape* Landscape,
#   UMaterialInterface* Material);
# (declaration-only, no inline body required).
# Compact LOCKED_DECL (NO spaces around `=`):
# static FSkyguardLandscapeVisibleAudit SetTransientLandscapeDiagnosticMaterialSynchronized(ALandscape* Landscape, UMaterialInterface* Material);
...
# wrap struct members with NO Category and explicitly
# do NOT parse this library.
# Identifier SetTransientLandscapeDiagnosticMaterialSynchronized is NOT leftover
# analog VisibleAudit field bSuccess and NOT leftover unique
# BeginTransientLandscapeDiagnosticMaterialDeferred #1697 and NOT leftover unique
# SetTransientLandscapeDiagnosticMaterial (bool sibling, no Synchronized suffix).
...
# static IS present. method const IS absent. Return type IS
# FSkyguardLandscapeVisibleAudit.
# Second arg is Material (NOT GovernedMaterial).
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# stay BlueprintPure, if method const is present, if the
...
# without Skyguard| / Category="Skyguard|Mission01|Environment"
# without |Validation / Category="Skyguard|Mission01|Environment|Authoring"
# / |Grounding / |Landscape / |PCG / |Layout / |Materials /
# |Visibility / bare Environment.
# Accept one-line and split-line UFUNCTION wraps.
# Compact whitespace when matching.
# Word-boundary identifier matching so
# SetTransientLandscapeDiagnosticMaterial does not match this
# Synchronized slot and vice versa.
...
# (leftover unique field drafts).
# Do NOT parse FSkyguardMission01EnvironmentAuthoringResult /
# CaptureConfiguration / MaterialCompilation structs.
# Do NOT parse leftover unique Authoring UFUNCTIONs #1691-#1693.
...
# as this slot (leave them uncontracted).
# Do NOT claim leftover unique AuthorGovernedLandscapeAndGraph /
# AuthorGovernedLandscapeWithExistingGraph /
# AuditGovernedLandscapeAndGraph.
# Do NOT claim leftover unique SampleLandscapeHeight /
# SampleLandscapeFootprint.
# Clone leftover unique Validation UFUNCTION
# BeginTransientLandscapeDiagnosticMaterialDeferred — RETARGET hard:
# KEEP Category Validation. KEEP BlueprintCallable.
# KEEP return FSkyguardLandscapeVisibleAudit.
# RETARGET method BeginTransientLandscapeDiagnosticMaterialDeferred ->
# SetTransientLandscapeDiagnosticMaterialSynchronized.
# Keep args ALandscape* Landscape, UMaterialInterface* Material.
# Clone leftover unique PrepareGovernedLandscape #1694 — RETARGET
# method; second arg stays Material not GovernedMaterial.
# Clone leftover unique Authoring #1692 wrapping
# AuthorGovernedLandscapeAndGraph — RETARGET Authoring -> Validation.
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# Category Skyguard|Audio|Production — RETARGET Category
# Production -> Skyguard|Mission01|Environment|Validation.
# KEEP BlueprintCallable (NOT BlueprintPure).
# NOT leftover void EnsureDefaultEntries();.
# Clone leftover unique BlueprintPure GetSortieHits
# Category Skyguard|Combat|Sortie — RETARGET Category
# Combat|Sortie -> Skyguard|Mission01|Environment|Validation.
# KEEP BlueprintCallable (NOT BlueprintPure).
# NOT leftover int32 GetSortieHits() const;.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is SetTransientLandscapeDiagnosticMaterialSynchronized
# static struct-return with ALandscape* Landscape,
# UMaterialInterface* Material.
# NOT FName WeatherIdentity; and NOT int32 GetSortieHits() const; and NOT
...
# AuthorGovernedLandscapeAndGraph and NOT
# AuthorGovernedLandscapeWithExistingGraph and NOT
# AuditGovernedLandscapeAndGraph and NOT
# SampleLandscapeHeight and NOT SampleLandscapeFootprint
# and NOT GetActiveRHIAndFeatureLevel and NOT
# PrepareGovernedLandscapeForVisibleValidation and NOT
# AuditLandscapeVisibleReadiness and NOT
# FinishLandscapeMaterialCompilation and NOT
# BeginTransientLandscapeDiagnosticMaterialDeferred and NOT
# SetTransientLandscapeDiagnosticMaterial (without Synchronized) and NOT
# ConfigureLandscapeSceneCaptureDiagnostic and NOT
# static bool SetTransientLandscapeDiagnosticMaterial and NOT
# FireRate = 12.0f / RecoilPitch = 0.92f / 160.f /
...
# GetInvertVerticalLook / GetAppliedLookSensitivity /
# bSuccess = false / AuthorGovernedLandscapeAndGraph /
# AuthorGovernedLandscapeWithExistingGraph /
...
# FinishLandscapeMaterialCompilation /
# BeginTransientLandscapeDiagnosticMaterialDeferred /
# SetTransientLandscapeDiagnosticMaterial (without Synchronized) /
# ConfigureLandscapeSceneCaptureDiagnostic /
# FireRate = 12.0f / RecoilPitch = 0.92f / leftover Boom /
# leftover VegetationStartCullDistanceCm /
# void EnsureDefaultEntries() /
# void SetTransientLandscapeDiagnosticMaterialSynchronized
# AS THE LOCKED DECL.
...
TARGET_WRONG_FLOAT = (
    "float SetTransientLandscapeDiagnosticMaterialSynchronized();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void SetTransientLandscapeDiagnosticMaterialSynchronizeds();"
...
TARGET_WRONG_LEFTOVER_PREPARE = (
    "static FSkyguardLandscapeVisibleAudit "
    "PrepareGovernedLandscapeForVisibleValidation("
    "ALandscape* Landscape, "
    "UMaterialInterface* GovernedMaterial);"
)
TARGET_WRONG_SET_TRANSIENT = (
    "static bool SetTransientLandscapeDiagnosticMaterial("
)
TARGET_WRONG_SET_TRANSIENT_SYNC = (
    "static FSkyguardLandscapeVisibleAudit "
    "BeginTransientLandscapeDiagnosticMaterialDeferred("
    "ALandscape* Landscape, "
    "UMaterialInterface* Material);"
)
TARGET_WRONG_CONFIGURE_CAPTURE = (
...
TARGET_WRONG_TRUE = "bool SetTransientLandscapeDiagnosticMaterialSynchronized = true;"
TARGET_WRONG_ZERO = "bool SetTransientLandscapeDiagnosticMaterialSynchronized = 0.f;"
...
STOP_BEFORE_EVENT_DEFINITIONS = "GetActiveRHIAndFeatureLevel"
SIBLING_INITIALIZE_REQUIRED = "AuthorGovernedLandscapeWithExistingGraph"
SIBLING_EVALUATE_READINESS = "AuditGovernedLandscapeAndGraph"
SIBLING_CONFIGURE_ROUTING = "SampleLandscapeHeight"
SIBLING_UNBOUND_CATEGORIES = "SampleLandscapeFootprint"
SIBLING_FIND_ENTRY = "GetActiveRHIAndFeatureLevel"
SIBLING_GET_REQUIRED = "AuditLandscapeVisibleReadiness"
SIBLING_GET_DISPLAY = "AuthorGovernedLandscapeAndGraph"
SIBLING_ENTRIES = "GetActiveRHIAndFeatureLevel"
SIBLING_ROUTING = "FinishLandscapeMaterialCompilation"
...
STOP_BEFORE_SKYLINE_STYLE = "AuditLandscapeVisibleReadiness"
...
THIS_SCRIPT = (
    "Scripts/tests/test_mission01_set_transient_landscape_diagnostic_material_synchronized"
    "_decl_contract.py"
)
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        LEFTOVER_THEATER_LAMP_COUNT,
        LEFTOVER_GUNNER_TRACE_RANGE,
        f"{prefix}test_mission01_begin_transient_landscape_diagnostic_material_deferred"
        "_decl_contract.py",
        f"{prefix}test_mission01_prepare_governed_landscape_for_visible_validation"
        "_decl_contract.py",
        f"{prefix}test_mission01_audit_landscape_visible_readiness"
        "_decl_contract.py",
...
LOCKED_SCRIPTS = (
    "Scripts/tests/test_mission01_begin_transient_landscape_diagnostic_material_deferred"
    "_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    ...
    r"SetTransientLandscapeDiagnosticMaterialSynchronized\("
    r"ALandscape\* Landscape,"
    r"UMaterialInterface\* Material\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept a method const qualifier, a void/bool/float/int32
    # rename, leftover unique PrepareGovernedLandscapeForVisibleValidation,
    # leftover siblings AuthorGovernedLandscapeWithExistingGraph /
    # AuditGovernedLandscapeAndGraph, leftover unique
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bSetTransientLandscapeDiagnosticMaterialSynchronized\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+SetTransientLandscapeDiagnosticMaterialSynchronized\b", compact):
        return False
    if re.search(r"\bbool\s+SetTransientLandscapeDiagnosticMaterialSynchronized\b", compact):
        return False
    if re.search(r"\bfloat\s+SetTransientLandscapeDiagnosticMaterialSynchronized\b", compact):
        return False
    if re.search(r"\bint32\s+SetTransientLandscapeDiagnosticMaterialSynchronized\b", compact):
        return False
...
def spec_section(header: str) -> str:
    ...
    method = "SetTransientLandscapeDiagnosticMaterialSynchronized"
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            ...
            r"SetTransientLandscapeDiagnosticMaterialSynchronized\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for SetTransientLandscapeDiagnosticMaterialSynchronized "
...
class Mission01SetTransientLandscapeDiagnosticMaterialSynchronizedDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "SetTransientLandscapeDiagnosticMaterialSynchronized"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, "AuditLandscapeVisibleReadiness"), body)
        self.assertFalse(has_identifier(section, "AuditLandscapeVisibleReadiness"), section)
        self.assertTrue(has_identifier(body, "BeginTransientLandscapeDiagnosticMaterialDeferred"), body)
        self.assertFalse(has_identifier(section, "BeginTransientLandscapeDiagnosticMaterialDeferred"), section)
        self.assertTrue(has_identifier(body, "SetTransientLandscapeDiagnosticMaterial"), body)
        self.assertFalse(has_identifier(section, "SetTransientLandscapeDiagnosticMaterial"), section)
        self.assertTrue(has_identifier(section, "SetTransientLandscapeDiagnosticMaterialSynchronized"), section)
        self.assertTrue(
            has_identifier(body, "PrepareGovernedLandscapeForVisibleValidation"),
            body,
        )
        self.assertFalse(
            has_identifier(section, "PrepareGovernedLandscapeForVisibleValidation"),
            section,
        )
    ...
    def test_missing_set_transient_landscape_diagnostic_material_synchronized_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("|Landscape", specifiers)
        ...
        self.assertNotEqual(
            specifiers,
            'BlueprintCallable, Category="Skyguard|Mission01|Environment"',
        )
        self.assertNotEqual(
            specifiers,
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "SetTransientLandscapeDiagnosticMaterialSynchronized()const",
    ...
    def test_set_transient_landscape_diagnostic_material_synchronized_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LEFTOVER_PREPARE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SET_TRANSIENT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SET_TRANSIENT_SYNC)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CONFIGURE_CAPTURE)
        self.assertNotIn("GovernedMaterial", LOCKED_DECL)
        self.assertIn("UMaterialInterface* Material", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid SetTransientLandscapeDiagnosticMaterialSynchronizedX();\n",
            "\tint32 SetTransientLandscapeDiagnosticMaterialSynchronized = 1;\n",
            "\tFName SetTransientLandscapeDiagnosticMaterialSynchronized = NAME_None;\n",
            "\tvoid SetTransientLandscapeDiagnosticMaterialSynchronized() = "
            + forty
            + ";\n",
            "\tvoid SetTransientLandscapeDiagnosticMaterialSynchronized() = "
            ...
            f"\t{TARGET_WRONG_SET_TRANSIENT}\n",
            f"\t{TARGET_WRONG_SET_TRANSIENT_SYNC}\n",
            "\tstatic bool SetTransientLandscapeDiagnosticMaterial(\n"
            "\t\tALandscape* Landscape,\n"
            "\t\tUMaterialInterface* Material);\n",
        )
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "SetTransientLandscapeDiagnosticMaterialSynchronized")
        )
        self.assertTrue(
            has_identifier(section, "SetTransientLandscapeDiagnosticMaterialSynchronized")
        )
        ...
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, "AuditLandscapeVisibleReadiness"))
        self.assertFalse(has_identifier(section, "AuditLandscapeVisibleReadiness"))
        self.assertTrue(has_identifier(leaked, "BeginTransientLandscapeDiagnosticMaterialDeferred"))
        self.assertFalse(has_identifier(section, "BeginTransientLandscapeDiagnosticMaterialDeferred"))
        self.assertTrue(has_identifier(leaked, "SetTransientLandscapeDiagnosticMaterial"))
        self.assertFalse(has_identifier(section, "SetTransientLandscapeDiagnosticMaterial"))
        self.assertTrue(has_identifier(section, "SetTransientLandscapeDiagnosticMaterialSynchronized"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 begin transient landscape diagnostic material deferred "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_set_transient_landscape_diagnostic_material_synchronized"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "SetTransientLandscapeDiagnosticMaterial"))
        self.assertTrue(has_identifier(LOCKED_DECL, "SetTransientLandscapeDiagnosticMaterialSynchronized"))
        ...
        self.assertNotIn("RecoilPitch = 0.92f", LOCKED_DECL)
        self.assertNotIn("RouteLengthCm = 45000.f", LOCKED_DECL)
        for leftover in leftover_apache_ufunction_tokens():
