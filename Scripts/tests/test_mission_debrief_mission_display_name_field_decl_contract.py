# THIS IS leftover-safe FSkyguardMissionDebrief::MissionDisplayName.
# origin/main form: split-line UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `FText MissionDisplayName;` with no Category= and no
# initializer. If a clone asserts Category and origin/main has none,
# drop that assertion. If a clone asserts `= 0` / `= false` / an
# initializer, drop that and accept the bare decl. If a clone
# asserts meta=(MultiLine="true"), drop that; origin/main has no
# MultiLine on this field (Narrative does).
...
# Do NOT contract other Debrief fields (State, Result, Narrative,
# bNewBestScore, bNewBestMedal, bProgressSaved, SaveSlotName,
# NextMissionId, NextMissionDisplayName, NextMissionMap,
...
# no Category=. Accept bare `FText MissionDisplayName;`. Fail-closed
# if type/decl missing, renamed, Category= added, or an initializer
# appears.
...
# siblings, leftover MissionDebrief State / Result siblings.
...
TARGET = "FText MissionDisplayName;"
TARGET_WRONG_ZERO = "FText MissionDisplayName = 0;"
TARGET_WRONG_FALSE = "FText MissionDisplayName = false;"
TARGET_WRONG_ONE = "FText MissionDisplayName = 1;"
TARGET_WRONG_FLOAT = "FText MissionDisplayName = 0.f;"
TARGET_WRONG_TRUE = "FText MissionDisplayName = true;"
TARGET_WRONG_INIT = "FText MissionDisplayName = FText();"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # FSkyguardMissionDebrief::MissionDisplayName.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FText\s+MissionDisplayName\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FText MissionDisplayName is missing from "
...
class MissionDebriefMissionDisplayNameFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("MissionDisplayName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_mission_display_name_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
    ...
    def test_mission_display_name_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText MissionDisplayName"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "FText MissionDisplayName;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            "\tFText MissionDisplayName = 0;\n",
            "\tFText MissionDisplayName = false;\n",
            "\tbool MissionDisplayName = false;\n",
            "\tFString MissionDisplayName;\n",
            "\tFText DisplayName;\n",
            "\tFText NextMissionDisplayName;\n",
            "\tFText Narrative;\n",
            "\tFText MissionDisplayName = FText();\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief MissionDisplayName field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_mission_display_name"
            "_field_decl_contract.py"
        ))
