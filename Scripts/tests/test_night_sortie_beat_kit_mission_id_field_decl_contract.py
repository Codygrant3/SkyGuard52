# THIS IS leftover-safe FSkyguardNightSortieBeatKit MissionId.
...
# `FName MissionId;` with NO in-struct initializer.
# Fail-closed if MissionId is missing or renamed, if the
# type is not FName, if an initializer is invented, or if
...
# bKeepThermal / Beats.
# Do NOT claim leftover DaySortieBeatKit MissionId.
# THIS IS NOT leftover analog night-sortie-beat-kit-fields
# #254 / #9787 (keep that file in LOCKED_SCRIPTS).
...
# VisibleAnywhere / FName WeatherIdentity as the locked
# decl, retarget: type is FName, identifier is MissionId,
# no `=`, no UPROPERTY.
...
TARGET_WRONG_BOOL = "bool MissionId = true;"
TARGET_WRONG_KIND = (
    "ESkyguardNightSortieBeatKind MissionId = "
...
SIBLING_BEATS = "Beats"
SIBLING_WEATHER_DECL = "FName WeatherIdentity;"
SIBLING_KEEP_THERMAL_DECL = "bool bKeepThermal = true;"
SIBLING_BEATS_DECL = "FSkyguardNightSortieBeat Beats[7];"
...
def mission_id_has_uproperty_wrap(region: str) -> bool:
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # WeatherIdentity / bKeepThermal / Beats. Do not accept
    ...
    if re.search(
        ...
        return False
    if mission_id_has_uproperty_wrap(region):
        return False
...
class NightSortieBeatKitMissionIdFieldDeclContractTests(unittest.TestCase):
    def test_night_sortie_beat_kit_struct_exists(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_BEAT, header)
        self.assertNotIn(STOP_BEFORE_BEAT, section)
        self.assertNotIn(STOP_BEFORE_BEAT, body)
        ...
        self.assertNotIn(SIBLING_WEATHER_IDENTITY, LOCKED_DECL)
        self.assertNotIn(SIBLING_KEEP_THERMAL, LOCKED_DECL)
        self.assertNotIn(SIBLING_BEATS, LOCKED_DECL)
        ...
        self.assertNotIn(LEFTOVER_DAY_KIT, LOCKED_DECL)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            spec_section(leftover_day_kit)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_leftover_beat_namespace_or_day_kit_does_not_satisfy(self) -> None:
        mixed = (
        ...
        self.assertNotIn(STOP_BEFORE_BEAT, section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(STOP_BEFORE_DAY_KIT, section)
        self.assertNotIn(NIGHT_EYES, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_clone_uproperty_is_not_the_locked_decl(self) -> None:
        ...
        self.assertNotIn(SIBLING_WEATHER_IDENTITY, LOCKED_DECL)
        self.assertIn("MissionId", LOCKED_DECL)
    ...
    def test_mission_id_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("FName MissionId"), LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_KIND}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_KIND}\n", LOCKED_DECL)
        self.assertIn("MissionId", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\t{SIBLING_WEATHER_DECL}\n",
            f"\t{SIBLING_KEEP_THERMAL_DECL}\n",
            f"\t{SIBLING_BEATS_DECL}\n",
            "\tFName MissionIds;\n",
            "\tint32 MissionId = 0;\n",
            "\tbool MissionId = false;\n",
    ...
    def test_uproperty_or_category_clones_fail_closed(self) -> None:
        ...
        self.assertTrue(mission_id_has_uproperty_wrap(section), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MissionId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("MissionId", str(raised.exception))
        origin = spec_section(origin_main_header())
        self.assertFalse(mission_id_has_uproperty_wrap(origin), origin)
        for token in clone_uproperty_locked_tokens():
    ...
    def test_does_not_contract_sibling_weather_keep_thermal_or_beats(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        self.assertNotIn(SIBLING_WEATHER_DECL, LOCKED_DECL)
        self.assertNotIn(SIBLING_KEEP_THERMAL_DECL, LOCKED_DECL)
        self.assertNotIn(SIBLING_BEATS_DECL, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(STOP_BEFORE_BEAT, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_DAY_KIT, LOCKED_DECL)
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, SIBLING_WEATHER_IDENTITY), section)
        self.assertTrue(has_identifier(section, SIBLING_KEEP_THERMAL), section)
        self.assertTrue(has_identifier(section, SIBLING_BEATS), section)
        for field in leftover_neighbor_hit_fields():
    ...
    def test_does_not_parse_leftover_beat_enum_namespace_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(LEFTOVER_BEAT_STRUCT, header)
        self.assertNotIn(STOP_BEFORE_BEAT, section)
        self.assertIn(STOP_BEFORE_NAMESPACE, header)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "night-sortie-beat-kit MissionId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "night_sortie_beat_kit_mission_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardNightSortieBeatKit.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardRadarNode.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_NIGHT_BEAT_KIND_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_BEAT_CALL_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_BEAT_THREAT_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_KIT_FIELDS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_KIT_BEATS, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_BEAT, locked_only)
