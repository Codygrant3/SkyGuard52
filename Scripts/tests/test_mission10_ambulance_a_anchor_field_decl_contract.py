...
# INDEX_NONE or lock AmbulanceAAnchor
# construction in the .cpp. This is leftover-safe
# Mission10 Evacuation AmbulanceAAnchor
...
# drafts #674–#787+, leftover Apache
...
LOCKED_DECL = AMBULANCE_A_ANCHOR_FIELD
...
# an isolated Python AmbulanceAAnchor
...
class Mission10AmbulanceAAnchorFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("AmbulanceAAnchor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_ambulance_a_anchor_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tUSceneComponent* AmbulanceAAnchor;\n"
        )
        as_const = (
    ...
    def test_ambulance_a_anchor_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("AmbulanceAAnchor", LOCKED_DECL)
    ...
    def test_contract_is_ambulance_a_anchor_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_ambulance_a_anchor(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_ambulance_a_anchor"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
