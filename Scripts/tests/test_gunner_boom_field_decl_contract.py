# THIS IS leftover-safe ASkyguardGunner Boom.
...
# then BARE `TObjectPtr<USpringArmComponent> Boom;`
# (no initializer).
...
# Stop BEFORE sibling GunnerCamera as a claimed slot.
# Do NOT claim GunnerCamera, leftover CPG mesh kit
# CpgStation through CpgConsoleRight, leftover CPG
# text, leftover combat floats, leftover
# FireCameraShakeClass, leftover
# MinimumSafeSideFireYaw, leftover CpgSightHud,
# leftover HandMesh / ForearmMesh / FrontSight /
# RearSight, or leftover analog Apache TailBoom.
# Boom is NOT leftover Apache TailBoom (different
# class ASkyguardApacheAircraft, different type
# UStaticMeshComponent). Boom is NOT leftover
# analog apache-hull-collider / apache-mount-fail-closed
# / apache-cpg-feel FireRate / RecoilPitch /
# leftover LoadoutSpec / Category Skyguard|Apache
# ammo / leftover analog gun-fire-camera-shake /
# leftover analog GetCameraShakeScale.
...
# and NOT Skyguard|Combat and NOT
# Skyguard|Combat|Feedback and NOT
# Skyguard|Combat|Safety.
# LOCKED_DECL is `TObjectPtr<USpringArmComponent> Boom;`
# NOT `FName WeatherIdentity;` and NOT
# `float CurrentIntegrity = 140.f` and NOT
# `TObjectPtr<UStaticMeshComponent> CpgConsoleRight;`
# and NOT `TObjectPtr<USceneComponent> CpgStation;`
# and NOT `TObjectPtr<UCameraComponent> Boom;`
# and NOT leftover Apache TailBoom.
...
# / TObjectPtr<UCameraComponent> Boom / Apache TailBoom
# AS THE LOCKED DECL.
...
# Boom fail-closed if LOCKED_DECL is leftover
# UStaticMeshComponent Boom / USceneComponent Boom /
# UCameraComponent Boom / TailBoom / CpgConsoleRight /
# CpgStation / FireRate = 12.0f / RecoilPitch = 0.92f /
# FlareCount = 6 / leftover CPG mesh kit.
...
TARGET = "TObjectPtr<USpringArmComponent> Boom;"
TARGET_WRONG_INIT = "TObjectPtr<USpringArmComponent> Boom = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USpringArmComponent> Boom = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USpringArmComponent> Boom = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> Boom;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> Boom;"
...
TARGET_WRONG_TEXT = "TObjectPtr<UTextRenderComponent> Boom;"
...
TARGET_WRONG_STATIC = "TObjectPtr<UStaticMeshComponent> Boom;"
TARGET_WRONG_CAMERA = "TObjectPtr<UCameraComponent> Boom;"
...
BOOM_INIT_RE = re.compile(
    r"TObjectPtr<USpringArmComponent>\s+Boom\s*="
)
WRONG_CAMERA_BOOM_RE = re.compile(
    r"TObjectPtr<UCameraComponent>\s+Boom\b"
)
WRONG_STATIC_BOOM_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+Boom\b"
)
WRONG_SCENE_BOOM_RE = re.compile(
    r"TObjectPtr<USceneComponent>\s+Boom\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgConsoleRight / CpgStation /
    # leftover UCameraComponent Boom / leftover
    # UStaticMeshComponent Boom / leftover Apache
    # TailBoom, or an initializer on Boom
    # as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if BOOM_DECL_RE.search(compact) is None:
        return False
    if BOOM_INIT_RE.search(compact):
        return False
    if WRONG_CAMERA_BOOM_RE.search(compact):
        return False
    if WRONG_STATIC_BOOM_RE.search(compact):
        return False
    if WRONG_SCENE_BOOM_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if "CpgConsoleRight" in compact and "Boom;" not in compact:
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_GUNNER_CAMERA):
            break
    ...
    if has_identifier(window, SIBLING_GUNNER_CAMERA):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_GUNNER_CAMERA}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USpringArmComponent>\s+Boom\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USpringArmComponent> Boom "
...
class GunnerBoomFieldDeclContractTests(unittest.TestCase):
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Boom"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Boom", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_boom_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<USpringArmComponent> Boom;", compact_origin)
        self.assertNotIn("Boom = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_boom_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USpringArmComponent> Boom"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("USpringArmComponent", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<USpringArmComponent> Boom;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USpringArmComponent> Booms;\n",
            "\tint32 Boom = 140;\n",
            "\tbool Boom = true;\n",
            "\tfloat Boom = " + forty + ";\n",
            "\tfloat Boom = " + eighty + ";\n",
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tTObjectPtr<USpringArmComponent>\n\tBoom;\n",
            "\tTObjectPtr<USpringArmComponent>   Boom;\n",
            "\tTObjectPtr<USpringArmComponent>\tBoom;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_VISIBLE_READONLY}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_gunner_camera_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "Boom"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgConsoleRight"))
        self.assertTrue(has_identifier(section, "Boom"))
        self.assertFalse(has_identifier(section, SIBLING_GUNNER_CAMERA))
        self.assertFalse(has_identifier(section, SIBLING_CPG_CONSOLE_RIGHT))
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_RAIL_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_RAIL_LEFT))
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_RAIL_RIGHT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_RAIL_RIGHT))
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_MPD_RIGHT_TEXT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_MPD_RIGHT_TEXT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_TEDAC))
        self.assertFalse(has_identifier(section, SIBLING_CPG_MPD_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_MPD_RIGHT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner Boom field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_boom_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
