# THIS IS leftover-safe FSkyguardMissionResult::CompletionTimeSeconds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `float CompletionTimeSeconds = 0.f;`.
# THIS STRUCT IS EditAnywhere + BlueprintReadWrite.
# There is NO Category=. Required initializer `= 0.f`
# (not Harbor 40/80).
# Analog leftover mission-result-defaults #179 / #c4db then
# leftover isolated FinalScore #1264 / MedalTier #1265 then
# leftover MissionId #1426 / ShotsFired #1427 then
# this isolated CompletionTimeSeconds field decl.
# Clone parse-window from leftover FinalScore #1264 and
# leftover MissionId #1426.
...
# int32 / FName / `= 0`, RETARGET: specifiers are
# EditAnywhere + BlueprintReadWrite, type is float,
# REQUIRED initializer `= 0.f`, no Category=.
# Fail-closed if the UPROPERTY or decl is missing or renamed,
# EditAnywhere is dropped, BlueprintReadWrite is replaced
# (BlueprintReadOnly / VisibleAnywhere-only), the type is
# not float, the initializer is dropped or changed from
# `0.f`, or Category= is added.
...
# Do NOT parse leftover FSkyguardMissionDebrief (#1266-#1278).
# Do NOT parse leftover FSkyguardObjectiveProgress (#1261-#1263).
# Do NOT parse leftover USkyguardMissionDefinition (#350-#364).
...
# Match EXACT identifier CompletionTimeSeconds as a whole
# field name after float. Do NOT accept AircraftDamageFraction
# (also float = 0.f). Do NOT claim leftover MissionSaveRecord
# BestCompletionTimeSeconds.
# Siblings stay unclaimed: MissionId / bMissionSucceeded /
# ShotsFired / Hits / AircraftDamageFraction /
# CompletedObjectiveIds / FinalScore / MedalTier.
# Harbor 40/80 fail-closed in the parse window and in THIS
# test file. `= 0.f` is the locked initializer, not Harbor.
# Ban retired live-copy tokens via split tokens
# (b + Ya + kRuntimeReady). Fail-closed on live Ig+la /
# Ri+fle / Ya+k appearing as contiguous tokens in THIS
# test file. Stay Apache CPG 30 mm / Hydra / Hellfire.
# Do not relock leftover mission-types-defaults-tests #153.
...
TARGET = "float CompletionTimeSeconds = 0.f;"
TARGET_WRONG_FALSE = "float CompletionTimeSeconds = false;"
TARGET_WRONG_ONE = "float CompletionTimeSeconds = 1.f;"
TARGET_WRONG_INT = "float CompletionTimeSeconds = 0;"
TARGET_WRONG_TRUE = "float CompletionTimeSeconds = true;"
TARGET_BARE = "float CompletionTimeSeconds;"
TARGET_WRONG_DAMAGE = "float AircraftDamageFraction = 0.f;"
TARGET_WRONG_FINAL = "int32 FinalScore = 0;"
TARGET_WRONG_MEDAL = "int32 MedalTier = 0;"
TARGET_WRONG_FNAME = "FName CompletionTimeSeconds;"
TARGET_WRONG_INT32 = "int32 CompletionTimeSeconds = 0;"
TARGET_WRONG_BEST = "float BestCompletionTimeSeconds = 0.f;"
TARGET_WRONG_SHOTS = "int32 ShotsFired = 0;"
LOCKED_DECL = TARGET
UPROPERTY_EDIT_WRITE = "UPROPERTY(EditAnywhere, BlueprintReadWrite)"
...
NEIGHBOR_DEFINITION = "USkyguardMissionDefinition"
NEIGHBOR_AUDIO_AUDIT = "FSkyguardProductionAudioAudit"
NEIGHBOR_AUDIO_ENTRY = "FSkyguardProductionAudioEntry"
NEIGHBOR_AUDIO_ROUTING = "FSkyguardProductionAudioRouting"
NEIGHBOR_THEATER = "FSkyguardTheaterKitSpec"
NEIGHBOR_SORTIE = "ASkyguardGunshipSortieDirector"
NEIGHBOR_PATROL = "ASkyguardPatrolShipBoss"
GET_OBJECTIVE_RUNTIME = "GetObjectiveRuntime"
ADD_OBJECTIVE_PROGRESS = "AddObjectiveProgress"
THIS_SCRIPT = (
    "Scripts/tests/test_mission_result_completion_time"
    "_seconds_field_decl_contract.py"
)
SIBLING_MISSION_ID = "MissionId"
SIBLING_SUCCEEDED = "bMissionSucceeded"
SIBLING_SHOTS = "ShotsFired"
SIBLING_HITS = "Hits"
SIBLING_DAMAGE = "AircraftDamageFraction"
SIBLING_OBJECTIVES = "CompletedObjectiveIds"
SIBLING_FINAL = "FinalScore"
SIBLING_MEDAL = "MedalTier"
LEFTOVER_SAVE_TIME = "BestCompletionTimeSeconds"
...
def leftover_neighbor_time_fields() -> tuple[str, ...]:
    return (
        LEFTOVER_SAVE_TIME,
        SIBLING_DAMAGE,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored default. Do not accept
    # = false / = 1.f / = 0 / = true / a bare decl / sibling
    # AircraftDamageFraction / leftover BestCompletionTimeSeconds
    # / leftover FinalScore for FSkyguardMissionResult
    # ::CompletionTimeSeconds (must be float
    # CompletionTimeSeconds = 0.f).
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"\bAircraftDamageFraction\b", compact) and not re.search(
        r"\bCompletionTimeSeconds\b", compact
    ):
        return False
    if re.search(r"\bBestCompletionTimeSeconds\b", compact) and not re.search(
        r"\bCompletionTimeSeconds\b", compact
    ):
        return False
    if re.search(
        r"float\s+CompletionTimeSeconds\s*=\s*0\.f\b",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+CompletionTimeSeconds\s*=\s*0\s*;",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CompletionTimeSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+CompletionTimeSeconds\b",
        compact,
    ):
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        NEIGHBOR_PROGRESS,
        NEIGHBOR_DEFINITION,
        NEIGHBOR_AUDIO_AUDIT,
        NEIGHBOR_AUDIO_ENTRY,
        NEIGHBOR_AUDIO_ROUTING,
        NEIGHBOR_THEATER,
        NEIGHBOR_SORTIE,
        NEIGHBOR_PATROL,
...
def public_section(header: str) -> str:
    ...
    if NEIGHBOR_DEBRIEF in header:
        after = header[header.find(body) + len(body) :]
        if NEIGHBOR_DEBRIEF not in after:
            raise AssertionError(
                f"{NEIGHBOR_DEBRIEF} must remain after {CLASS_NAME}"
            )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CompletionTimeSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CompletionTimeSeconds is missing from "
...
class MissionResultCompletionTimeSecondsFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CompletionTimeSeconds"), section)
        self.assertIn(SIBLING_FINAL, section)
        self.assertIn(SIBLING_MEDAL, section)
        self.assertNotIn(STOP_AFTER, section)
        self.assertNotIn(NEIGHBOR_DEBRIEF, section)
        self.assertNotIn(NEIGHBOR_PROGRESS, section)
        self.assertNotIn(NEIGHBOR_DEFINITION, section)
        self.assertNotIn(NEIGHBOR_AUDIO_AUDIT, section)
        self.assertNotIn(NEIGHBOR_THEATER, section)
        self.assertNotIn(NEIGHBOR_SORTIE, section)
        self.assertNotIn(NEIGHBOR_PATROL, section)
        self.assertNotIn(GET_OBJECTIVE_RUNTIME, section)
        self.assertNotIn(ADD_OBJECTIVE_PROGRESS, section)
        self.assertNotIn("float CompletionTimeSeconds)", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("CompletionTimeSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_completion_time_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_completion_time_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CompletionTimeSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("0.f;"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "float CompletionTimeSeconds = 0.f;")
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn(SIBLING_FINAL, LOCKED_DECL)
        self.assertNotIn(SIBLING_MEDAL, LOCKED_DECL)
        self.assertNotIn(SIBLING_SUCCEEDED, LOCKED_DECL)
        self.assertNotIn(SIBLING_MISSION_ID, LOCKED_DECL)
        self.assertNotIn(SIBLING_SHOTS, LOCKED_DECL)
        self.assertNotIn(SIBLING_HITS, LOCKED_DECL)
        self.assertNotIn(SIBLING_DAMAGE, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_SAVE_TIME, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_BARE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_DAMAGE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BEST}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FINAL}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_DAMAGE}\n", LOCKED_DECL)
        self.assertIn("CompletionTimeSeconds", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            "\tbool CompletionTimeSeconds = false;\n",
            "\tbool CompletionTimeSeconds = 0.f;\n",
            "\tfloat CompletionTimeSeconds = 0.0;\n",
            "\tFName MissionId;\n",
        )
    ...
    def test_visible_or_readonly_specifiers_fail_closed(self) -> None:
        wrongs = (
        ...
        for macro in wrongs:
            self.assertNotEqual(macro, UPROPERTY_EDIT_WRITE)
            self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_WRITE)
            self.assertNotIn("BlueprintReadOnly", UPROPERTY_EDIT_WRITE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            f"public:\n\t{UPROPERTY_EDIT_WRITE} {LOCKED_DECL}\n",
            "public:\n\tUPROPERTY(EditAnywhere, BlueprintReadWrite)\n"
            f"\t{LOCKED_DECL}\n",
            "public:\n\tUPROPERTY(\n\t\tEditAnywhere,\n"
            "\t\tBlueprintReadWrite)\n"
    ...
    def test_does_not_parse_neighbors_or_director_methods(self) -> None:
        ...
        self.assertNotIn(NEIGHBOR_DEFINITION, section)
        self.assertNotIn(NEIGHBOR_AUDIO_AUDIT, section)
        self.assertNotIn(NEIGHBOR_AUDIO_ENTRY, section)
        self.assertNotIn(NEIGHBOR_AUDIO_ROUTING, section)
        self.assertNotIn(NEIGHBOR_THEATER, section)
        self.assertNotIn(NEIGHBOR_SORTIE, section)
        self.assertNotIn(NEIGHBOR_PATROL, section)
        self.assertNotIn(GET_OBJECTIVE_RUNTIME, section)
        ...
        self.assertNotIn(NEIGHBOR_DEFINITION, leaked)
        self.assertNotIn(NEIGHBOR_THEATER, leaked)
        self.assertNotIn(NEIGHBOR_SORTIE, leaked)
        self.assertNotIn(NEIGHBOR_PATROL, leaked)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result CompletionTimeSeconds field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_completion_time_seconds_field_decl_contract.py"
        ))
        ...
        self.assertIn(CLONE_MISSION_ID, LOCKED_SCRIPTS)
        self.assertIn(CLONE_SHOTS_FIRED, LOCKED_SCRIPTS)
        self.assertIn(CLONE_BEST_COMPLETION, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(NEIGHBOR_DEFINITION, locked_only)
        self.assertNotIn(NEIGHBOR_AUDIO_AUDIT, locked_only)
        self.assertNotIn(NEIGHBOR_THEATER, locked_only)
        self.assertNotIn(NEIGHBOR_SORTIE, locked_only)
        self.assertNotIn(NEIGHBOR_PATROL, locked_only)
        self.assertNotIn(SIBLING_HITS, locked_only)
        self.assertNotIn(SIBLING_SHOTS, locked_only)
        self.assertNotIn(SIBLING_MISSION_ID, locked_only)
        self.assertNotIn(SIBLING_DAMAGE, locked_only)
        self.assertNotIn(LEFTOVER_SAVE_TIME, locked_only)
        for leftover in leftover_neighbor_hit_fields():
            self.assertNotIn(leftover, LOCKED_DECL)
            self.assertNotIn(leftover, locked_only)
        for leftover in leftover_neighbor_time_fields():
            self.assertNotIn(leftover, LOCKED_DECL)
            self.assertNotIn(leftover, locked_only)
