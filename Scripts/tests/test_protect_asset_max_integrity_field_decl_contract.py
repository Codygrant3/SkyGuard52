# THIS IS leftover-safe ASkyguardProtectAsset::MaxIntegrity.
# origin/main form: one-line or split-line
# UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Skyguard|Campaign")
# then `float MaxIntegrity = 100.f;`.
# THIS IS UPROPERTY(EditAnywhere, BlueprintReadWrite,
# Category="Skyguard|Campaign"). It is NOT
# BlueprintReadOnly-only. Category IS `Skyguard|Campaign`.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if Category is missing, or if the
# default is not `= 100.f`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS public section ONLY after
# `class SKYGUARD52_API ASkyguardProtectAsset`, stop at
# `protected:`.
# Do NOT parse protected Root / Hull / Volume.
# Do NOT contract sibling CurrentIntegrity
# (sibling worker).
# Do NOT contract leftover-safe UFUNCTION siblings
# ApplyDamage / IsDestroyed / GetIntegrityFraction /
# ResetIntegrity.
# THIS IS NOT leftover Apache MaxIntegrity closed
# #411-#426 (different class ASkyguardApacheAircraft).
# THIS IS NOT leftover protect-asset-cargo-proxy #60.
# THIS IS NOT leftover ProtectAsset UFUNCTION drafts
# #1288-#1291.
# THIS IS NOT leftover radar-node-presentation #56.
# THIS IS NOT leftover radar-node-reset-gameplay-tests #66.
# THIS IS NOT leftover audio-telemetry-defaults #175 /
# #a6b7 / test_audio_telemetry_defaults*.
# THIS IS NOT leftover audio-director-telemetry-fail-closed
# #138 / #9efc.
# THIS IS NOT leftover audio-telemetry PeakActiveVoices
# #1287 / test_audio_telemetry_peak_active_voices*.
# Do NOT parse enum class ESkyguardAudioEvent (contains
# leftover Ri+fle / Ig+la).
# Do NOT parse FSkyguardAudioEventDefinition.
# Do NOT parse ESkyguardListenerPerspective body.
# Do NOT parse ESkyguardBossWeapon.
# THIS IS NOT leftover BossTelemetry Ri+fleHits / Ig+laHits.
...
# THIS IS NOT leftover FSkyguardMissionResult::FinalScore /
# MedalTier siblings.
...
# THIS IS NOT leftover MissionDebrief Result #1266 through
# CampaignComplete #1278.
# THIS IS NOT leftover BossTelemetry #1279-#1280.
# THIS IS NOT leftover HowToFlyRow #1258-#1260.
# Do NOT parse later structs/enums/classes.
# Do NOT parse USkyguardMission01EnvironmentAuthoringLibrary,
# ASkyguardMission01EnvironmentDirector, landscape/authoring
# structs, ASkyguardPropSpinner, ESkyguardBriefingPictogram,
# USkyguardSortiePresentationComponent methods,
# EditAnywhere FSkyguardMissionResult gameplay fields,
# leftover AddObjectiveProgress, leftover BindRuntimeActors /
# HandleDroneCityImpact, leftover GetStormRainBeatKit
# without UFUNCTION, leftover Ya+k spawn / b+Ya+kRuntimeReady.
# Do NOT parse ASkyguardApacheAircraft.
...
HEADER_PATH = "Source/Skyguard52/SkyguardProtectAsset.h"
CLASS_NAME = "ASkyguardProtectAsset"
TARGET = "float MaxIntegrity = 100.f;"
TARGET_WRONG_BARE = "float MaxIntegrity;"
TARGET_WRONG_FALSE = "float MaxIntegrity = false;"
TARGET_WRONG_TRUE = "float MaxIntegrity = true;"
TARGET_WRONG_INT = "float MaxIntegrity = 100;"
TARGET_WRONG_ZERO = "float MaxIntegrity = 0.f;"
TARGET_WRONG_TYPE = "int32 MaxIntegrity = 100;"
TARGET_WRONG_SIBLING = "float CurrentIntegrity = 100.f;"
LOCKED_DECL = TARGET
UPROPERTY_EDIT = (
...
STOP_BEFORE_ROOT = "TObjectPtr<USceneComponent> Root"
STOP_BEFORE_HULL = "TObjectPtr<UStaticMeshComponent> Hull"
STOP_BEFORE_VOLUME = "TObjectPtr<UBoxComponent> Volume"
STOP_BEFORE_AUDIO_EVENT = "enum class ESkyguardAudioEvent"
STOP_BEFORE_LISTENER = "enum class ESkyguardListenerPerspective"
...
SIBLING_APPLY_DAMAGE = "ApplyDamage"
SIBLING_IS_DESTROYED = "IsDestroyed"
SIBLING_GET_INTEGRITY_FRACTION = "GetIntegrityFraction"
SIBLING_RESET_INTEGRITY = "ResetIntegrity"
UNCONTRACTED_CURRENT_INTEGRITY = "CurrentIntegrity"
...
CLONE_RADAR_NODE_IS_DESTROYED = (
    "Scripts/tests/test_radar_node_is_destroyed"
    "_decl_contract.py"
)
CLONE_RADAR_NODE_APPLY_DAMAGE = (
    "Scripts/tests/test_radar_node_apply_damage"
    "_decl_contract.py"
)
CLONE_RADAR_NODE_RESET_NODE = (
    "Scripts/tests/test_radar_node_reset_node"
    "_decl_contract.py"
)
LEFTOVER_APACHE_MAX_INTEGRITY = (
    "Scripts/tests/test_apache_max_integrity.py"
)
LEFTOVER_APACHE_MAX_INTEGRITY_CONTRACT = (
    "Scripts/tests/test_apache_max_integrity_contract.py"
)
LEFTOVER_APACHE_MAX_INTEGRITY_TESTS = (
    "Scripts/tests/test_apache_max_integrity_tests.py"
)
...
MAX_INTEGRITY_DECL_RE = re.compile(
    r"\bfloat\s+MaxIntegrity\s*=\s*100\.f\s*;"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_APPLY_DAMAGE,
        SIBLING_IS_DESTROYED,
        SIBLING_GET_INTEGRITY_FRACTION,
        SIBLING_RESET_INTEGRITY,
        UNCONTRACTED_CURRENT_INTEGRITY,
    )


def protected_component_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_ROOT,
        STOP_BEFORE_HULL,
        STOP_BEFORE_VOLUME,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `= 100.f` default. Do not
    # accept a bare decl, `= false` / `= true` / `= 0.f` /
    # `= 100` / int32 rename / sibling CurrentIntegrity when
    # origin/main has `float MaxIntegrity = 100.f;`.
    del declaration
    compact = collapsed(region)
    if MAX_INTEGRITY_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+MaxIntegrity\s*=\s*false\b", compact):
        return False
    if re.search(r"float\s+MaxIntegrity\s*=\s*true\b", compact):
        return False
    if re.search(r"float\s+MaxIntegrity\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"int32\s+MaxIntegrity\b", compact):
        return False
    if re.search(r"float\s+MaxIntegrity\s*;", compact):
        return False
    if re.search(
        r"float\s+MaxIntegrity\s*=\s*100\s*;",
        compact,
    ) and MAX_INTEGRITY_DECL_RE.search(compact) is None:
        return False
    return True
...
def count_one_declaration(region: str, declaration: str) -> int:
    del declaration
    compact = collapsed(region)
    return len(MAX_INTEGRITY_DECL_RE.findall(compact))
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        "ESkyguardAudioEvent::",
        f"class SKYGUARD52_API {LEFTOVER_APACHE_AIRCRAFT_CLASS}",
        f"class {LEFTOVER_APACHE_AIRCRAFT_CLASS}",
    )
...
def public_section(header: str) -> str:
    ...
    for protected_decl in (
        STOP_BEFORE_PROTECTED,
        STOP_BEFORE_ROOT,
        STOP_BEFORE_HULL,
        STOP_BEFORE_VOLUME,
    ):
        if protected_decl in section:
            raise AssertionError(
                f"{CLASS_NAME} parse window includes {protected_decl}"
            )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MaxIntegrity\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaxIntegrity is missing from "
        f"origin/main:{HEADER_PATH} class {CLASS_NAME} public section"
    )
...
class ProtectAssetMaxIntegrityFieldDeclContractTests(unittest.TestCase):
    def test_protect_asset_class_exists(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_LISTENER, section)
        self.assertNotIn(STOP_BEFORE_EVENT_DEF, section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAPON, section)
        self.assertNotIn(LEFTOVER_APACHE_AIRCRAFT_CLASS, section)

    def test_missing_class_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            class_body(
                "class ASkyguardUnrelatedProtectAsset "
                ": public AActor\n{\n};\n"
            )
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        mixed = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        for token in protected_component_tokens()[:1]:
            self.assertNotIn(token, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn("BlueprintReadWrite", section)
        self.assertIn('Category="Skyguard|Campaign"', section)
        self.assertIn("EditAnywhere", UPROPERTY_EDIT)
        self.assertIn("BlueprintReadWrite", UPROPERTY_EDIT)
        self.assertIn("Category", UPROPERTY_EDIT)
        ...
        self.assertIn('Category="Skyguard|Campaign"', specifiers)
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float MaxIntegrity = 100.f;", compact_origin)
        self.assertNotIn("int32 MaxIntegrity", compact_origin)
    ...
    def test_max_integrity_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MaxIntegrity"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TYPE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SIBLING}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_INT}\n", LOCKED_DECL)
        self.assertIn("MaxIntegrity", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tfloat MaximumIntegrity = 100.f;\n",
            "\tfloat MaxIntegrityy = 100.f;\n",
            "\tvoid ResetIntegrity();\n",
        )
    ...
    def test_readonly_or_missing_category_fails_closed(self) -> None:
        readonly_wrap = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        section = public_section(readonly_wrap)
        specifiers = attached_uproperty_specifiers(section)
        ...
        self.assertIn('Category="Skyguard|Campaign"', origin)
        missing_category = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        missing = attached_uproperty_specifiers(
            public_section(missing_category)
        )
    ...
    def test_does_not_contract_sibling_integrity_api(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(LEFTOVER_APACHE_AIRCRAFT_CLASS, LOCKED_DECL)
        section = public_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_protected_components_or_neighbors(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        leaked = class_body(header)
        self.assertIn(STOP_BEFORE_PROTECTED, leaked)
        self.assertNotIn(STOP_BEFORE_PROTECTED, section)
        for token in protected_component_tokens():
            self.assertIn(token, leaked)
            self.assertNotIn(token, section)
        self.assertNotIn(STOP_BEFORE_AUDIO_EVENT, header)
        self.assertNotIn(STOP_BEFORE_LISTENER, header)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_LISTENER, section)
        self.assertNotIn(STOP_BEFORE_LISTENER, leaked)
        self.assertNotIn(STOP_BEFORE_EVENT_DEF, section)
        self.assertNotIn(STOP_BEFORE_EVENT_DEF, leaked)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAPON, section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAPON, leaked)
        self.assertNotIn(LEFTOVER_APACHE_AIRCRAFT_CLASS, section)
        self.assertNotIn(LEFTOVER_APACHE_AIRCRAFT_CLASS, leaked)

    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tvoid {SIBLING_APPLY_DAMAGE}(float Amount);\n"
            f"\tbool {SIBLING_IS_DESTROYED}() const;\n"
            f"\tfloat {SIBLING_GET_INTEGRITY_FRACTION}() const;\n"
            f"\tvoid {SIBLING_RESET_INTEGRITY}();\n"
            f"\tfloat {UNCONTRACTED_CURRENT_INTEGRITY} = 100.f;\n"
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("EditAnywhere", UPROPERTY_EDIT)
        self.assertIn("BlueprintReadWrite", UPROPERTY_EDIT)
        self.assertIn('Category="Skyguard|Campaign"', UPROPERTY_EDIT)
        self.assertNotIn("BlueprintReadOnly", UPROPERTY_EDIT)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT)
        self.assertNotIn("MultiLine", UPROPERTY_EDIT)
        self.assertNotIn("ClampMin", UPROPERTY_EDIT)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("{", LOCKED_DECL)
        self.assertNotIn("return ", LOCKED_DECL)
        self.assertNotIn("SkyguardProtectAsset.cpp", THIS_SCRIPT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "protect-asset MaxIntegrity field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_protect_asset_max_integrity"
            "_field_decl_contract.py"
        ))
        self.assertIn(CLONE_RESET_INTEGRITY, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RADAR_NODE_IS_DESTROYED, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RADAR_NODE_APPLY_DAMAGE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RADAR_NODE_RESET_NODE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_APACHE_MAX_INTEGRITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_APACHE_MAX_INTEGRITY_CONTRACT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_APACHE_MAX_INTEGRITY_TESTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_PEAK_ACTIVE_VOICES, LOCKED_SCRIPTS)
        self.assertIn(CLONE_PLAYED_EVENTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_REQUESTED_EVENTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_REJECTED_BY_COOLDOWN, LOCKED_SCRIPTS)
        self.assertIn(CLONE_REJECTED_BY_CONCURRENCY, LOCKED_SCRIPTS)
        self.assertIn(CLONE_REJECTED_MISSING_ASSET, LOCKED_SCRIPTS)
        self.assertIn(CLONE_PRIORITY_EVICTIONS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_WEAK_POINTS_DESTROYED, LOCKED_SCRIPTS)
        self.assertIn(CLONE_PILOT_COMMANDS_ISSUED, LOCKED_SCRIPTS)
        self.assertIn(CLONE_CAMPAIGN_COMPLETE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_TELEMETRY_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_TELEMETRY_DEFAULTS_PY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_TELEMETRY_DEFAULTS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_DIRECTOR_TELEMETRY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_DIRECTOR_TELEMETRY_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_DIRECTOR_TELEMETRY_CONTRACT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("ESkyguardListenerPerspective", locked_only)
        ...
        self.assertNotIn(LEFTOVER_APACHE_AIRCRAFT_CLASS, locked_only)
        for token in protected_component_tokens():
            self.assertNotIn(token, locked_only)
