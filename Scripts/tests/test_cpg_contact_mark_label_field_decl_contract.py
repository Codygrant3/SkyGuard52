# THIS IS leftover-safe FSkyguardCpgContactMark Label.
...
# `FString Label;` with NO in-struct initializer.
# Fail-closed if Label is missing or renamed, if the
...
# locked decl.
# Fail-closed if an initializer is invented on Label.
# Label here is NOT leftover ContactMark sibling
# WorldLocation #1516 / bLocked / bSeeking / LockAlpha.
# Label here is NOT leftover HUD snapshot LockLine #1505 /
# LockPhase #1508 / LockProgress #1512.
# WorldLocation leftovers #1340 / #1386 / #1390 stay locked.
...
# Do NOT parse leftover RoutePoint / MissionObjectiveAnchor /
# MissionLandmarkAnchor.
...
# Do NOT contract sibling fields WorldLocation / bLocked /
# bSeeking / LockAlpha.
...
# #1501-#1515. Keep leftover-safe WeaponLine #1502 and
# leftover LockLine #1505 / LockPhase #1508 /
# LockProgress #1512 in LOCKED_SCRIPTS.
...
# #1516 WorldLocation (ContactMark only; stop before
# SkyguardCpgWeaponLabel). HUD FString retarget is
# leftover-safe #1502 WeaponLine (FString, no initializer).
# RETARGET: type is FString, identifier is Label, no `=`,
# locked decl is the plain field, not a UPROPERTY wrap,
# parse STRUCT FSkyguardCpgContactMark, stop before
# SkyguardCpgWeaponLabel.
...
TARGET = "FString Label;"
TARGET_WRONG_EQ_EMPTY = 'FString Label = TEXT("");'
TARGET_WRONG_EQ_NONE = "FString Label = NAME_None;"
TARGET_WRONG_FALSE = "FString Label = false;"
TARGET_WRONG_TRUE = "FString Label = true;"
TARGET_WRONG_ZERO = "FString Label = 0.f;"
TARGET_WRONG_FLOAT = "float Label;"
TARGET_WRONG_HEALTH = "float Label = 160.f;"
TARGET_WRONG_BOOL = "bool Label;"
TARGET_WRONG_INT = "int32 Label;"
TARGET_WRONG_FNAME = "FName Label;"
TARGET_WRONG_TCHAR = 'const TCHAR* Label = TEXT("");'
...
HUD_SIBLING_LOCK_LINE = "LockLine"
HUD_SIBLING_LOCK_PHASE = "LockPhase"
HUD_SIBLING_LOCK_PROGRESS = "LockProgress"
...
LABEL_BARE_RE = re.compile(r"FString\s+Label\s*;")
LABEL_INIT_RE = re.compile(r"FString\s+Label\s*=")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def label_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*FString\s+Label\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString Label;`.
    # Do not accept `= TEXT("")` / `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` when origin/main locks
    # a bare FString. Do not accept leftover sibling
    # WorldLocation / bLocked / bSeeking / LockAlpha,
    # leftover HUD snapshot WeaponLine / LockLine /
    # LockPhase / LockProgress / HeadingDegrees, leftover
    # debrief MissionTitle, or leftover LoadoutSpec
    # FlareCount. Do not accept leftover RoutePoint /
    # MissionObjectiveAnchor / MissionLandmarkAnchor
    # UPROPERTY wraps. Do not accept UPROPERTY / Category
    # clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LABEL_INIT_RE.search(compact):
        return False
    if re.search(r"float\s+Label\b", compact):
        return False
    if re.search(r"bool\s+Label\b", compact):
        return False
    if re.search(r"int32\s+Label\b", compact):
        return False
    if re.search(r"FName\s+Label\b", compact):
        return False
    if re.search(r"FVector\s+Label\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*Label\b", compact):
        return False
    if LABEL_BARE_RE.search(compact) is None:
        return False
    if label_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString Label\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString Label is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FString Label\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FString Label is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "Label"):
        raise AssertionError(
            "UPROPERTY clone landed on Label; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "Label"):
            raise AssertionError(
                f"{token} clone landed on Label; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgContactMarkLabelFieldDeclContractTests(unittest.TestCase):
    def test_cpg_contact_mark_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Label"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString Label;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("FString Label"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("=", LOCKED_DECL)
        self.assertIn("Label", LOCKED_DECL)
        ...
        self.assertNotIn(HUD_SIBLING_LOCK_LINE, LOCKED_DECL)
        self.assertNotIn(HUD_SIBLING_LOCK_PHASE, LOCKED_DECL)
        self.assertNotIn(HUD_SIBLING_LOCK_PROGRESS, LOCKED_DECL)
    ...
    def test_hud_snapshot_before_contact_mark_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Label"), section)
        self.assertNotIn(STOP_BEFORE_HUD_SNAPSHOT, section)
        self.assertNotIn(LEFTOVER_HUD_SNAPSHOT, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_label_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_world_location_does_not_satisfy_label(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WORLD}\n"
    ...
    def test_leftover_hud_lock_line_phase_progress_do_not_satisfy(self) -> None:
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString Label;", compact_origin)
        self.assertNotIn("Label = TEXT", compact_origin)
        self.assertNotIn("Label = NAME_None", compact_origin)
        self.assertNotIn("Label = false", compact_origin)
        self.assertNotIn("Label = true", compact_origin)
        self.assertNotIn("Label = 0.f", compact_origin)
        self.assertNotIn("Label = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("Label = " + forty, compact_origin)
        self.assertNotIn("Label = " + eighty, compact_origin)

    def test_label_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString Label"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_WORLD}\n", LOCKED_DECL)
        self.assertIn("Label", str(raised.exception))
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    label_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(label_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-contact-mark Label field decl contract "
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(HUD_SIBLING_LOCK_LINE, locked_only)
        self.assertNotIn(HUD_SIBLING_LOCK_PHASE, locked_only)
        self.assertNotIn(HUD_SIBLING_LOCK_PROGRESS, locked_only)
        for field in leftover_neighbor_hit_fields():
