...
# or lock the OpenDispenserBay body in the .cpp.
...
# isolated-test drafts #107–#442, leftover
...
# Python OpenDispenserBay declaration contract
...
# .cpp OpenDispenserBay body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardIronRainBoss::OpenDispenserBay",
    "SkyguardIronRainBoss.cpp",
    "SetExposed",
    "IsValidIndex",
    "BayReleaseCounts",
...
class IronRainOpenDispenserBayDeclContractTests(unittest.TestCase):
    def test_iron_rain_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, OPEN_DISPENSER_BAY),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, OPEN_DISPENSER_BAY)
        self.assertIn("OpenDispenserBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, OPEN_DISPENSER_BAY))

    def test_missing_open_dispenser_bay_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardIronRainBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, OPEN_DISPENSER_BAY)
        self.assertIn("OpenDispenserBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, OPEN_DISPENSER_BAY)
        self.assertIn("OpenDispenserBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", OPEN_DISPENSER_BAY)
        self.assertNotIn("UFUNCTION", OPEN_DISPENSER_BAY)
        self.assertNotIn("Category", OPEN_DISPENSER_BAY)
        self.assertNotIn("BlueprintCallable", OPEN_DISPENSER_BAY)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, OPEN_DISPENSER_BAY)
        self.assertIn("OpenDispenserBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool OpenDispenserBay;\n"
        wrong_return_void = "\tvoid OpenDispenserBay(int32 BayIndex);\n"
        wrong_return_int = "\tint32 OpenDispenserBay(int32 BayIndex);\n"
        added_const = "\tbool OpenDispenserBay(int32 BayIndex) const;\n"
        wrong_type = "\tbool OpenDispenserBay(int BayIndex);\n"
        missing_arg = "\tbool OpenDispenserBay();\n"
        added_arg = (
            "\tbool OpenDispenserBay(int32 BayIndex, float Extra);\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, OPEN_DISPENSER_BAY)
            self.assertIn("OpenDispenserBay", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_open_dispenser_bay_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, OPEN_DISPENSER_BAY))
        self.assertEqual(
            declaration_count(section, OPEN_DISPENSER_BAY),
            1,
        )
        self.assertTrue(
            OPEN_DISPENSER_BAY.startswith("bool "),
            OPEN_DISPENSER_BAY,
        )
        self.assertTrue(
            OPEN_DISPENSER_BAY.endswith(";"),
            OPEN_DISPENSER_BAY,
        )
        self.assertIn("OpenDispenserBay(", OPEN_DISPENSER_BAY)
        self.assertIn("int32 BayIndex", OPEN_DISPENSER_BAY)
        self.assertNotIn("INDEX_NONE", OPEN_DISPENSER_BAY)
        self.assertNotIn("{", OPEN_DISPENSER_BAY)
        self.assertNotIn("}", OPEN_DISPENSER_BAY)
        self.assertNotIn("return ", OPEN_DISPENSER_BAY)
        self.assertNotIn(" const", OPEN_DISPENSER_BAY)
        self.assertNotIn("ReleasePooledEscort", OPEN_DISPENSER_BAY)
        self.assertNotIn("IssueClimbCommand", OPEN_DISPENSER_BAY)
        self.assertNotIn("IssueCrossCommand", OPEN_DISPENSER_BAY)
        self.assertNotIn("OpenUpperEngineExposure", OPEN_DISPENSER_BAY)
        self.assertNotIn("GetChinMuzzleLocation", OPEN_DISPENSER_BAY)
        self.assertNotIn("GetGunnerMount", OPEN_DISPENSER_BAY)
        self.assertNotIn("FaceWorldLocation", OPEN_DISPENSER_BAY)
        self.assertNotIn("DispenserPort", OPEN_DISPENSER_BAY)
        self.assertNotIn("SetSearchlightTracked", OPEN_DISPENSER_BAY)
        self.assertNotIn("SetContactIdentified", OPEN_DISPENSER_BAY)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            OPEN_DISPENSER_BAY,
        )
        self.assertNotIn("SetLightningExposed", OPEN_DISPENSER_BAY)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, OPEN_DISPENSER_BAY)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tOpenDispenserBay(int32 BayIndex);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tbool OpenDispenserBay(\n"
            "\t\tint32 BayIndex);\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{OPEN_DISPENSER_BAY}\n}}\n"
        self.assertTrue(has_declaration(one_line, OPEN_DISPENSER_BAY))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, OPEN_DISPENSER_BAY),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{OPEN_DISPENSER_BAY}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", OPEN_DISPENSER_BAY)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", OPEN_DISPENSER_BAY)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_open_dispenser_bay_cpp_body(
        self,
    ) -> None:
        locked_only = f"{OPEN_DISPENSER_BAY}\n"
        self.assertNotIn("{", OPEN_DISPENSER_BAY)
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", OPEN_DISPENSER_BAY)
        self.assertNotIn("SkyguardIronRainBoss.cpp", locked_only)
        self.assertNotIn("return false", OPEN_DISPENSER_BAY)
        self.assertNotIn("return true", OPEN_DISPENSER_BAY)
        self.assertNotIn("SetExposed", OPEN_DISPENSER_BAY)
        self.assertNotIn("IsValidIndex", OPEN_DISPENSER_BAY)
        self.assertNotIn("BayReleaseCounts", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_release_pooled_escort_sibling(
        ...
        for token in RELEASE_POOLED_ESCORT_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("ReleasePooledEscort", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("GetDestroyedDispenserCount", OPEN_DISPENSER_BAY)
        self.assertNotIn("GetManeuver", OPEN_DISPENSER_BAY)
        self.assertNotIn("MaxReleasesPerBay", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("DispenserPort", OPEN_DISPENSER_BAY)
        self.assertNotIn("DispenserCenter", OPEN_DISPENSER_BAY)
        self.assertNotIn("EnginePodPort", OPEN_DISPENSER_BAY)
        self.assertNotIn("FuelControlPort", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("SetSearchlightTracked", OPEN_DISPENSER_BAY)
        self.assertNotIn("IsSearchlightTracked", OPEN_DISPENSER_BAY)
        self.assertNotIn("ASkyguardBlackKiteBoss", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("SetContactIdentified", OPEN_DISPENSER_BAY)
        self.assertNotIn("IsContactIdentified", OPEN_DISPENSER_BAY)
        self.assertNotIn("OpenOrbitExposure", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", OPEN_DISPENSER_BAY)
        self.assertNotIn("ASkyguardLifelineHunterBoss", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("SetLightningExposed", OPEN_DISPENSER_BAY)
        self.assertNotIn("ApplyCorrectiveBankGust", OPEN_DISPENSER_BAY)
        self.assertNotIn("ASkyguardTempestBoss", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("GetGunnerMount", OPEN_DISPENSER_BAY)
        self.assertNotIn("GetChinTurret", OPEN_DISPENSER_BAY)
        self.assertNotIn("GetSensorTurret", OPEN_DISPENSER_BAY)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{OPEN_DISPENSER_BAY}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, OPEN_DISPENSER_BAY)
        self.assertNotIn("GetChinMuzzleLocation", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("ESkyguardApacheSystem", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("ASkyguardGunner", OPEN_DISPENSER_BAY)
        self.assertNotIn("FillAndFinalize", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("SkyguardRadarNode", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", OPEN_DISPENSER_BAY)
        self.assertNotIn("USkyguardBriefingWidget", OPEN_DISPENSER_BAY)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            OPEN_DISPENSER_BAY,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", OPEN_DISPENSER_BAY)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            OPEN_DISPENSER_BAY,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OPEN_DISPENSER_BAY)
        self.assertNotIn("MinHeightFromOriginCm", OPEN_DISPENSER_BAY)
        self.assertNotIn("MaxIntegrity", OPEN_DISPENSER_BAY)
        self.assertNotIn("SkyguardApacheAircraft.h", OPEN_DISPENSER_BAY)
        self.assertNotIn("SkyguardPathfinder", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, OPEN_DISPENSER_BAY)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", OPEN_DISPENSER_BAY)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{OPEN_DISPENSER_BAY}\n"
        self.assertEqual(
            require_declaration(locked_only, OPEN_DISPENSER_BAY),
            OPEN_DISPENSER_BAY,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain OpenDispenserBay contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, OPEN_DISPENSER_BAY.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                OPEN_DISPENSER_BAY.lower(),
                "iron rain OpenDispenserBay contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, OPEN_DISPENSER_BAY)

    def test_contract_is_open_dispenser_bay_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", OPEN_DISPENSER_BAY)
        self.assertNotIn("{", OPEN_DISPENSER_BAY)
        self.assertTrue(OPEN_DISPENSER_BAY.startswith("bool "))
        self.assertTrue(OPEN_DISPENSER_BAY.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)
