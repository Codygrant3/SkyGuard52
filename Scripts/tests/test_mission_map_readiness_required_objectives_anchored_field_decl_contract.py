...
# bRequiredObjectivesAnchored FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe MissionMapReadiness
# bRequiredObjectivesAnchored. THIS IS NOT leftover mission-map-
# readiness-defaults #3a2a. THIS IS NOT leftover
# mission-map GetReadiness / ValidateAssembly /
# RebuildRouteSpline / IsPointInsideFlightClearance.
# THIS IS NOT leftover MissionMapReadiness
# bDefinitionValid #1155 or in-flight
...
TARGET = "bool bRequiredObjectivesAnchored = false;"
TARGET_WRONG = "bool bRequiredObjectivesAnchored = true;"
LOCKED_DECL = TARGET
...
class MissionMapReadinessRequiredObjectivesAnchoredFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bRequiredObjectivesAnchored", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_required_objectives_anchored_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_required_objectives_anchored_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bRequiredObjectivesAnchored"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bRequiredObjectivesAnchored", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
			"\tbool\n"
            "\tbRequiredObjectivesAnchored = false;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bRequiredObjectivesAnchored", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bRequiredObjectivesAnchored"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bRequiredObjectivesAnchored", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_required_objectives_anchored(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission_map_readiness_required"
                "_objectives_anchored_field_decl_contract.py"
            )
        )
        self.assertIn(
