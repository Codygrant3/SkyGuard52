# THIS IS leftover-safe FSkyguardBriefingRadioRow LineId.
# origin/main form: split-line UPROPERTY(VisibleAnywhere,
# BlueprintReadOnly) then BARE `FName LineId;` (no initializer,
...
# bare FName LineId;. Fail closed if type/decl missing, renamed,
# Category= is added, or an initializer appears when origin/main
# is bare.
# Parse STRUCT public section ONLY after struct FSkyguardBriefingRadioRow
# and BEFORE struct FSkyguardHowToFlyRow.
# Do NOT parse USkyguardSortiePresentationComponent methods.
# Do NOT parse FSkyguardBriefingCard / FSkyguardHowToFlyRow.
...
# THIS IS leftover-safe FSkyguardBriefingRadioRow
...
# struct FSkyguardBriefingRadioRow and BEFORE
...
# Error #1232.
# Declaration-only. Accept origin/main split-line UPROPERTY and
# FName LineId;. Fail closed if type/decl
...
STRUCT_NAME = "FSkyguardBriefingRadioRow"
...
# BlueprintReadOnly FName LineId;
...
TARGET = "FName LineId;"
TARGET_WRONG = "FName LineId = NAME_None;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer for FSkyguardBriefingRadioRow::
    # Error (must be bare FName LineId;).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FName\s+LineId\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FName LineId is missing from "
...
class BriefingRadioRowLineIdFieldDeclContractTests(unittest.TestCase):
    def test_briefing_radio_row_struct_exists(self) -> None:
        ...
        self.assertIn("LineId", section)
        self.assertIn("Speaker", section)
        self.assertIn("Subtitle", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("LineId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_line_id_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_line_id_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName LineId"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("LineId", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("LineId", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FName LineId"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("LineId", locked_only)
    ...
    def test_this_file_bans_live_retired_tokens_case_sensitive(self) -> None:
        ...
        for banned in live:
            self.assertNotIn(
                banned,
                file_text,
                "briefing-radio-row LineId field contract contains live "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_briefing_radio_row_line_id"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_parser_does_not_land_on_anchors_or_director(self) -> None:
        ...
        match = CLASS_RE.search(header)
        self.assertIsNotNone(match, header)
        self.assertIn("FSkyguardBriefingRadioRow", match.group(0))
