# THIS IS leftover-safe FSkyguardBossWeakPointDefinition Integrity.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "1.0"))
# then `float Integrity = 100.f;`.
# THIS IS leftover-safe isolated UPROPERTY field decl.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# ClampMin IS `"1.0"`. Spaces around `=` in meta ARE required.
# Initializer IS `= 100.f` (not Harbor 40 + ".f" / 80 + ".f").
# There is NO Category. Fail-closed if Category= is added.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if EditAnywhere is dropped, if specifiers
# drop to BlueprintReadWrite, if type is not float, if
# initializer is not `= 100.f`, or if ClampMin is missing
# or wrong. Accept one-line and split-line UPROPERTY wraps.
...
STOP_BEFORE_BOSS_DEF = "struct FSkyguardBossDefinition"
LEFTOVER_COMPONENT = "USkyguardBossWeakPointComponent"
LEFTOVER_MISSION_DEF = "USkyguardMissionDefinition"
...
TARGET = "float Integrity = 100.f;"
TARGET_WRONG_BARE = "float Integrity;"
TARGET_WRONG_EQ_NONE = "float Integrity = NAME_None;"
TARGET_WRONG_FALSE = "float Integrity = false;"
TARGET_WRONG_TRUE = "float Integrity = true;"
TARGET_WRONG_ZERO = "float Integrity = 0.f;"
TARGET_WRONG_FNAME = "FName Integrity;"
TARGET_WRONG_INT = "int32 Integrity = 100;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_CLAMP_ZERO = (
    'UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "0.0"))'
)
TARGET_WRONG_NO_META = "UPROPERTY(EditAnywhere, BlueprintReadOnly)"
TARGET_WRONG_WRITE = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
    'meta = (ClampMin = "1.0"))'
)
TARGET_WRONG_VISIBLE = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
    'meta = (ClampMin = "1.0"))'
)
TARGET_WRONG_CATEGORY = (
    "UPROPERTY(EditAnywhere, BlueprintReadOnly, "
    'Category="Skyguard|Theater", meta = (ClampMin = "1.0"))'
)
TARGET_WRONG_BOSS_CATEGORY = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
...
LEFTOVER_COMPONENT_INTEGRITY = (
    "Scripts/tests/test_weak_point_integrity"
    "_field_decl_contract.py"
)
...
LEFTOVER_COMPONENT_DESTROYED_SCRIPT = (
    "Scripts/tests/test_weak_point_destroyed"
    "_field_decl_contract.py"
)
LEFTOVER_COMPONENT_EXPOSED_SCRIPT = (
    "Scripts/tests/test_weak_point_exposed"
    "_field_decl_contract.py"
)
SIBLING_WEAK_POINT_ID_SCRIPT = (
    "Scripts/tests/test_boss_weak_point_definition_weak_point_id"
    "_field_decl_contract.py"
)
SIBLING_REQUIRED_WEAPON_SCRIPT = (
    "Scripts/tests/test_boss_weak_point_definition_required_weapon"
    "_field_decl_contract.py"
)
SIBLING_EXPOSES_SCRIPT = (
...
def leftover_component_accepts_tokens() -> tuple[str, ...]:
    return (
        "bAccepts" + "Ri" + "fle",
        "bAccepts" + "Ig" + "la",
    )
...
def leftover_component_runtime_tokens() -> tuple[str, ...]:
    return (
    ...
    ) + leftover_component_accepts_tokens()


def leftover_same_header_structs() -> tuple[str, ...]:
    return (
        STOP_BEFORE_BOSS_DEF,
        "struct " + LEFTOVER_OBJECTIVE_DEF,
        "struct " + LEFTOVER_OBJECTIVE_PROGRESS,
        "struct " + LEFTOVER_SCORE_RULES,
        "struct " + LEFTOVER_WEATHER_PROFILE,
        "struct " + LEFTOVER_ROUTE_DEF,
    )


def leftover_other_file_types() -> tuple[str, ...]:
    return (
        LEFTOVER_COMPONENT,
        LEFTOVER_MISSION_DEF,
        ACTOR_NAME,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float Integrity = 100.f`.
    # Do not accept bare `float Integrity;` / `= 0.f` /
    # Harbor 40 + ".f" / 80 + ".f" / leftover FName / int32 when
    # origin/main has float Integrity = 100.f. Do not
    # accept sibling WeakPointId / RequiredWeapon /
    # ExposesWeakPointId fields.
    ...
    if forty in compact or eighty in compact:
        return False
    if re.search(r"FName\s+Integrity\b", compact):
        return False
    if re.search(r"int32\s+Integrity\b", compact):
        return False
    if re.search(r"bool\s+Integrity\b", compact):
        return False
    if re.search(
        r"float\s+Integrity\s*=\s*100\.f\s*;",
        compact,
    ) is None:
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        f"class {ACTOR_NAME}",
        f"class SKYGUARD52_API {LEFTOVER_COMPONENT}",
        f"class {LEFTOVER_COMPONENT}",
        f"class SKYGUARD52_API {LEFTOVER_MISSION_DEF}",
        f"class {LEFTOVER_MISSION_DEF}",
    ) + leftover_component_runtime_tokens()


def spec_section(header: str) -> str:
    body = struct_body(header)
    if STOP_BEFORE_BOSS_DEF in body:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_BOSS_DEF}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+Integrity\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float Integrity is missing from "
...
class BossWeakPointDefinitionIntegrityFieldDeclContractTests(unittest.TestCase):
    def test_boss_weak_point_definition_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_COMPONENT)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_MISSION_DEF)
        ...
        self.assertTrue(has_identifier(section, "Integrity"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_BOSS_DEF, header)
        self.assertNotIn(STOP_BEFORE_BOSS_DEF, section)
        self.assertNotIn(STOP_BEFORE_BOSS_DEF, body)
        self.assertNotIn(LEFTOVER_COMPONENT, header)
        self.assertNotIn(LEFTOVER_COMPONENT, section)
        self.assertNotIn(LEFTOVER_COMPONENT, body)
        self.assertNotIn(LEFTOVER_MISSION_DEF, header)
        self.assertNotIn(LEFTOVER_MISSION_DEF, section)
    ...
    def test_boss_definition_or_component_does_not_satisfy(self) -> None:
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tGENERATED_BODY()\n"
            "};\n"
            f"{STOP_BEFORE_BOSS_DEF}\n"
            "{{\n"
        ...
        self.assertFalse(has_identifier(section, "Integrity"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_DEF, section)
        self.assertNotIn(LEFTOVER_COMPONENT, section)
        self.assertNotIn(TARGET_WRONG_BOSS_CATEGORY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Integrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_integrity_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn(CLAMP_MIN_TOKEN, section)
        self.assertIn('ClampMin = "1.0"', UPROPERTY_EDIT_READONLY)
    ...
    def test_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float Integrity = 100.f;", compact_origin)
        self.assertNotIn("Integrity = NAME_None", compact_origin)
        self.assertNotIn("Integrity = false", compact_origin)
        self.assertNotIn("Integrity = true", compact_origin)
        self.assertNotIn("Integrity = 0.f", compact_origin)
        self.assertNotIn("Integrity = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("Integrity = " + forty, compact_origin)
        self.assertNotIn("Integrity = " + eighty, compact_origin)

    def test_integrity_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float Integrity"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 100.f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_WEAK_POINT_ID};\n",
            "\tfloat Integrities = 100.f;\n",
            "\tint32 Integrity = 100;\n",
            "\tbool Integrity;\n",
            "\tfloat Integrity = " + forty + ";\n",
            "\tfloat Integrity = " + eighty + ";\n",
        )
    ...
    def test_missing_edit_anywhere_or_clamp_min_fails_closed(self) -> None:
        no_edit = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        self.assertIn('ClampMin = "1.0"', origin)
        ...
        bare_meta = attached_uproperty_specifiers(spec_section(no_clamp))
        self.assertNotIn("ClampMin", bare_meta)
        ...
        leftover_specs = attached_uproperty_specifiers(
            spec_section(leftover_component)
        )
        self.assertIn("VisibleAnywhere", leftover_specs)
        self.assertIn("Category", leftover_specs)
        self.assertNotIn("EditAnywhere", leftover_specs)
        self.assertNotIn("ClampMin", leftover_specs)
        self.assertIn("EditAnywhere", origin)
        self.assertIn("ClampMin", origin)
        self.assertNotIn("Category", origin)
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_CLAMP_ZERO}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        zero_clamp = attached_uproperty_specifiers(
            spec_section(wrong_clamp)
        )
        self.assertIn('ClampMin = "0.0"', zero_clamp)
        self.assertNotIn('ClampMin = "1.0"', zero_clamp)
        self.assertIn('ClampMin = "1.0"', origin)
    ...
    def test_does_not_contract_sibling_weak_point_fields(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_BOSS_DEF, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_COMPONENT, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
        ...
        for leftover in leftover_component_runtime_tokens():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, LOCKED_DECL)

    def test_does_not_parse_boss_definition_component_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        leaked = struct_body(header)
        self.assertIn(STOP_BEFORE_BOSS_DEF, header)
        self.assertNotIn(STOP_BEFORE_BOSS_DEF, section)
        self.assertNotIn(STOP_BEFORE_BOSS_DEF, leaked)
        for same in leftover_same_header_structs():
            self.assertIn(same, header)
            self.assertNotIn(same, section)
            self.assertNotIn(same, leaked)
        for other in leftover_other_file_types():
            self.assertNotIn(other, header)
            self.assertNotIn(other, section)
            self.assertNotIn(other, leaked)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_COMPONENT, section)
        self.assertNotIn(LEFTOVER_COMPONENT, leaked)

    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tconst FSkyguardBossDefinition& {RESOLVE}"
            "(float Integrity);\n"
            f"\tint32 {NUM_KITS}();\n"
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("EditAnywhere", UPROPERTY_EDIT_READONLY)
        self.assertIn("BlueprintReadOnly", UPROPERTY_EDIT_READONLY)
        self.assertIn('meta = (ClampMin = "1.0")', UPROPERTY_EDIT_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("Category", UPROPERTY_EDIT_READONLY)
        self.assertNotIn('Category="Skyguard|Theater"', UPROPERTY_EDIT_READONLY)
        self.assertNotIn('Category="Skyguard|Boss"', UPROPERTY_EDIT_READONLY)
    ...
    def test_harbor_40_80_tokens_fail_closed_if_present(self) -> None:
        ...
        self.assertNotIn(forty, file_text)
        self.assertNotIn(eighty, file_text)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-weak-point-definition Integrity field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "integrity_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_BOSS_DEF, locked_only)
        self.assertNotIn(LEFTOVER_COMPONENT, locked_only)
        self.assertNotIn(LEFTOVER_MISSION_DEF, locked_only)
