...
# BlueprintReadOnly TArray<FSkyguardLandscapeHeightSample> Samples; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# Samples FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly TArray<FSkyguardLandscapeHeightSample> Samples; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# Samples FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# TArray<FSkyguardLandscapeHeightSample> Samples; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeFootprintSampleResult Samples
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly TArray<FSkyguardLandscapeHeightSample> Samples;
...
# Samples FIELD (declaration-only). It is
...
TARGET = "TArray<FSkyguardLandscapeHeightSample> Samples;"
TARGET_WRONG = "int32 Samples;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*TArray<FSkyguardLandscapeHeightSample>\s+Samples\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for TArray<FSkyguardLandscapeHeightSample> Samples is missing from "
...
class LandscapeFootprintSampleSamplesFieldDeclContractTests(unittest.TestCase):
    def test_landscape_footprint_sample_result_struct_exists(self) -> None:
        ...
        self.assertIn("TArray<FSkyguardLandscapeHeightSample>", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Samples", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_samples_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_samples_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FSkyguardLandscapeHeightSample> Samples"),
            LOCKED_DECL,
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
			"\tTArray<FSkyguardLandscapeHeightSample>\n"
            "\tSamples;\n"
            "private:\n"
            "};\n"
        )
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("Samples", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("TArray<FSkyguardLandscapeHeightSample> Samples"))
    ...
    def test_sibling_director_fields_do_not_satisfy_height_centimeters(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
