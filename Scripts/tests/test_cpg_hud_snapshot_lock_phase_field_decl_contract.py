# THIS IS leftover-safe FSkyguardCpgHudSnapshot LockPhase.
...
# `ESkyguardGuidedLockPhase LockPhase =
# ESkyguardGuidedLockPhase::Search;` with in-struct Search.
# Fail-closed if LockPhase is missing or renamed, if the
# type is not ESkyguardGuidedLockPhase, if the initializer
# is missing or not `= ESkyguardGuidedLockPhase::Search`,
# or if UPROPERTY / Category clones land.
...
# Do NOT lock leftover ContactMark `bool bLocked = false`.
...
# LoadoutSpec `int32 FlareCount = 6` (#1467).
# Do NOT contract sibling fields WeaponLine / RangeLine /
# ThreatLine / EufdLine / LockLine / SightLine /
# StationStatus / SightMode / RangeMeters / ThreatCount /
# HeadingDegrees / LockProgress / FlareCount /
# bMissileInbound.
# LockPhase is NOT leftover sibling LockLine
# (`FString LockLine;`), NOT leftover sibling LockProgress
# (`float LockProgress = 0.f`), and NOT leftover
# ContactMark `bool bLocked = false`.
# THIS IS NOT leftover HUD RangeLine #1501.
# THIS IS NOT leftover HUD WeaponLine #1502.
# THIS IS NOT leftover HUD ThreatLine #1503.
# THIS IS NOT leftover StormRainBeatKit isolated
# #1490-#1500 (including leftover Weather #1495).
# THIS IS NOT leftover WeatherProfile Weather #1353
# (`ESkyguardMissionWeather::Clear` plus UPROPERTY wrap).
...
# #260 / #6879. Isolated field decl does not relock the
# analog bulk.
...
# leftover `FString WeaponLine;` with NO initializer /
# leftover `ESkyguardMissionWeather Weather =
# ESkyguardMissionWeather::Storm` / leftover
# `ESkyguardMissionWeather::Clear`, retarget: type is
# ESkyguardGuidedLockPhase, identifier is LockPhase,
# initializer is ESkyguardGuidedLockPhase::Search,
...
TARGET_WRONG_BARE = "ESkyguardGuidedLockPhase LockPhase;"
TARGET_WRONG_DETECT = (
    "ESkyguardGuidedLockPhase LockPhase = "
...
TARGET_WRONG_EQ_NONE = "ESkyguardGuidedLockPhase LockPhase = NAME_None;"
TARGET_WRONG_FALSE = "ESkyguardGuidedLockPhase LockPhase = false;"
TARGET_WRONG_TRUE = "ESkyguardGuidedLockPhase LockPhase = true;"
TARGET_WRONG_ZERO = "ESkyguardGuidedLockPhase LockPhase = 0.f;"
TARGET_WRONG_FLOAT = "float LockPhase;"
TARGET_WRONG_HEALTH = "float LockPhase = 160.f;"
TARGET_WRONG_BOOL = "bool LockPhase;"
TARGET_WRONG_INT = "int32 LockPhase;"
TARGET_WRONG_FNAME = "FName LockPhase;"
TARGET_WRONG_FSTRING = "FString LockPhase;"
TARGET_WRONG_LOCK_LINE = "FString LockLine;"
TARGET_WRONG_LOCK_PROGRESS = "float LockProgress = 0.f;"
TARGET_WRONG_SIGHT = (
    "ESkyguardCpgSightMode SightMode = "
    "ESkyguardCpgSightMode::Helmet;"
)
TARGET_WRONG_WEAPON = "FString WeaponLine;"
TARGET_WRONG_RANGE = "FString RangeLine;"
TARGET_WRONG_THREAT = "FString ThreatLine;"
TARGET_WRONG_STORM_WEATHER = (
    "ESkyguardMissionWeather Weather = "
    "ESkyguardMissionWeather::Storm;"
)
TARGET_WRONG_PROFILE_CLEAR = (
    "ESkyguardMissionWeather Weather = "
...
LEFTOVER_STORM_KIT_THREATS = (
    "Scripts/tests/test_storm_rain_beat_kit_threats"
    "_field_decl_contract.py"
)
LEFTOVER_STORM_KIT_STATIONS = (
    "Scripts/tests/test_storm_rain_beat_kit_stations"
    "_field_decl_contract.py"
)
LEFTOVER_STORM_KIT_BEAT_COUNT = (
    "Scripts/tests/test_storm_rain_beat_kit_beat_count"
    "_field_decl_contract.py"
)
LEFTOVER_STORM_KIT_CALLS = (
...
def leftover_hud_isolated_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_HUD_RANGE_LINE,
        LEFTOVER_HUD_WEAPON_LINE,
        LEFTOVER_HUD_THREAT_LINE,
...
LOCK_PHASE_DECL_RE = re.compile(
    r"ESkyguardGuidedLockPhase\s+LockPhase\s*=\s*"
    r"ESkyguardGuidedLockPhase::Search\s*;"
)
LOCK_PHASE_INIT_RE = re.compile(
    r"ESkyguardGuidedLockPhase\s+LockPhase\s*=\s*([^;]+);"
)
LOCK_PHASE_BARE_RE = re.compile(r"ESkyguardGuidedLockPhase\s+LockPhase\s*;")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def leftover_wrong_lock_phase_inits() -> tuple[str, ...]:
    return (
        TARGET_WRONG_BARE,
        TARGET_WRONG_DETECT,
        TARGET_WRONG_TRACK,
        ...
        TARGET_WRONG_TRUE,
        TARGET_WRONG_ZERO,
        TARGET_WRONG_LOCK_LINE,
        TARGET_WRONG_LOCK_PROGRESS,
        TARGET_WRONG_SIGHT,
        TARGET_WRONG_WEAPON,
        TARGET_WRONG_RANGE,
        TARGET_WRONG_THREAT,
...
def lock_phase_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardGuidedLockPhase\s+LockPhase\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardGuidedLockPhase LockPhase =
    # ESkyguardGuidedLockPhase::Search;`.
    # Do not accept bare LockPhase, leftover Detect / Track /
    # Lock, leftover sibling LockLine / LockProgress /
    # leftover ContactMark bLocked, leftover SightMode,
    # leftover HUD WeaponLine / RangeLine / ThreatLine,
    # leftover StormRain Weather #1495, leftover
    # WeatherProfile Weather #1353, `= NAME_None` /
    # `= false` / `= true` / `= 0.f` / `= 160.f` when
    # origin/main locks Search. Do not accept leftover
    # LoadoutSpec FlareCount. Do not accept UPROPERTY /
    # Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LOCK_PHASE_DECL_RE.search(compact) is None:
        return False
    if LOCK_PHASE_BARE_RE.search(compact):
        return False
    if re.search(r"float\s+LockPhase\b", compact):
        return False
    if re.search(r"bool\s+LockPhase\b", compact):
        return False
    if re.search(r"int32\s+LockPhase\b", compact):
        return False
    if re.search(r"FName\s+LockPhase\b", compact):
        return False
    if re.search(r"FString\s+LockPhase\b", compact):
        return False
    if lock_phase_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    for match in LOCK_PHASE_INIT_RE.finditer(compact):
        init = collapsed(match.group(1))
        if init != "ESkyguardGuidedLockPhase::Search":
            return False
    return True
...
def spec_section(header: str) -> str:
    ...
    if STOP_BEFORE_STORM_KIT in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes leftover "
            f"{STOP_BEFORE_STORM_KIT}"
        )
    if STOP_BEFORE_WEATHER_PROFILE in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes leftover "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardGuidedLockPhase LockPhase\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardGuidedLockPhase LockPhase is missing "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*ESkyguardGuidedLockPhase LockPhase\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on ESkyguardGuidedLockPhase LockPhase is "
            f"not the locked decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "LockPhase"):
        raise AssertionError(
            "UPROPERTY clone landed on LockPhase; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "LockPhase"):
            raise AssertionError(
                f"{token} clone landed on LockPhase; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotLockPhaseFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockPhase"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardGuidedLockPhase LockPhase")
        )
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("= ESkyguardGuidedLockPhase::Search", LOCKED_DECL)
        self.assertIn("LockPhase", LOCKED_DECL)
        self.assertIn("ESkyguardGuidedLockPhase", LOCKED_DECL)
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_LOCK_LINE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_LOCK_PROGRESS))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_WEAPON_LINE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_FLARE_COUNT))
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIGHT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STORM_WEATHER)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PROFILE_CLEAR)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("LockPhase", str(raised.exception))
    ...
    def test_leftover_debrief_loadout_storm_and_profile_do_not_satisfy(
        self,
    ) -> None:
        leftovers = (
            LEFTOVER_DEBRIEF_SNAPSHOT,
    ...
    def test_does_not_claim_leftover_storm_rain_or_weather_profile(
        self,
    ) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STORM_WEATHER)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PROFILE_CLEAR)
        ...
        self.assertIn(TARGET_WRONG_STORM_WEATHER, storm_header)
        with self.assertRaises(AssertionError) as raised:
            spec_section(storm_header)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        profile_header = leftover_weather_profile_header()
        self.assertIn(LEFTOVER_WEATHER_PROFILE, profile_header)
        self.assertIn(TARGET_WRONG_PROFILE_CLEAR, profile_header)
        self.assertIn(CLONE_UPROPERTY_PROFILE, profile_header)
        self.assertIn("GENERATED_BODY", profile_header)
        with self.assertRaises(AssertionError) as raised:
            spec_section(profile_header)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        leftover_as_this_kit = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{CLONE_UPROPERTY_PROFILE}\n"
            f"\t{TARGET_WRONG_PROFILE_CLEAR}\n"
            "};\n"
        )
        section = spec_section(leftover_as_this_kit)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LockPhase"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_LOCKED, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LockPhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lock_phase_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEAPON}\n"
            f"\t{TARGET_WRONG_RANGE}\n"
    ...
    def test_lock_line_does_not_satisfy_lock_phase(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_LOCK_LINE}\n"
        ...
        self.assertTrue(has_identifier(section, SIBLING_LOCK_LINE), section)
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_LOCK_LINE))

    def test_lock_progress_does_not_satisfy_lock_phase(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_LOCK_PROGRESS}\n"
    ...
    def test_leftover_contact_blocked_does_not_satisfy(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{LEFTOVER_CONTACT_LOCKED}\n"
        ...
        self.assertIn("LockPhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_identifier(LOCKED_DECL, "bLocked"))

    def test_leftover_weapon_line_does_not_satisfy(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEAPON}\n"
    ...
    def test_leftover_sight_mode_does_not_satisfy(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SIGHT}\n"
            "};\n"
        )
        section = spec_section(leftover)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("LockPhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SIGHT_MODE))
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        section = spec_section(leftover_detect)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LockPhase", str(raised.exception))
        leftover_lock = (
        ...
        section = spec_section(leftover_lock)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LockPhase", str(raised.exception))
        ...
        self.assertNotIn(
            "LockPhase = ESkyguardGuidedLockPhase::Detect",
            compact_origin,
        )
        self.assertNotIn(
            "LockPhase = ESkyguardGuidedLockPhase::Track",
            compact_origin,
        )
        self.assertNotIn(
            "LockPhase = ESkyguardGuidedLockPhase::Lock;",
            compact_origin,
        )
        self.assertNotIn("LockPhase = NAME_None", compact_origin)
        self.assertNotIn("LockPhase = false", compact_origin)
        self.assertNotIn("LockPhase = true", compact_origin)
        self.assertNotIn("LockPhase = 0.f", compact_origin)
        self.assertNotIn("LockPhase = 160.f", compact_origin)

    def test_lock_phase_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("= ESkyguardGuidedLockPhase::Search", LOCKED_DECL)
        self.assertNotIn("= ESkyguardGuidedLockPhase::Detect", LOCKED_DECL)
        self.assertNotIn("= ESkyguardGuidedLockPhase::Track", LOCKED_DECL)
        self.assertNotIn("= ESkyguardGuidedLockPhase::Lock", LOCKED_DECL)
        ...
        self.assertNotIn("ESkyguardMissionWeather", LOCKED_DECL)
        self.assertNotIn("ESkyguardCpgSightMode", LOCKED_DECL)
        for token in FORBIDDEN_LOCKED_MACRO_TOKENS:
            self.assertNotIn(token, LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        ...
        for wrong in leftover_wrong_lock_phase_inits():
            self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_LOCK_LINE}\n", LOCKED_DECL)
        self.assertIn("LockPhase", str(raised.exception))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(
                f"\t{LEFTOVER_CONTACT_LOCKED}\n",
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{LEFTOVER_CONTACT_LABEL}\n",
            f"\t{LEFTOVER_CONTACT_LOCKED}\n",
            "\tESkyguardGuidedLockPhase LockPhases = "
            "ESkyguardGuidedLockPhase::Search;\n",
            "\tint32 LockPhase;\n",
            "\tbool LockPhase;\n",
            "\tfloat LockPhase = " + forty + ";\n",
            "\tfloat LockPhase = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    lock_phase_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(lock_phase_has_uproperty_wrap(origin), origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tESkyguardGuidedLockPhase\n"
            "\tLockPhase = ESkyguardGuidedLockPhase::Search;\n",
            "\tESkyguardGuidedLockPhase   LockPhase = "
            "ESkyguardGuidedLockPhase::Search;\n",
            "\tESkyguardGuidedLockPhase\tLockPhase = "
            "ESkyguardGuidedLockPhase::Search;\n",
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        for helper in namespace_helper_tokens():
            self.assertNotIn(helper, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{LOCK_PHASE_LABEL}();\n", LOCKED_DECL)
        self.assertIn("LockPhase", str(raised.exception))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot LockPhase field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "lock_phase_field_decl_contract.py"
        ))
        ...
        for script in leftover_storm_rain_isolated_scripts():
            self.assertIn(script, LOCKED_SCRIPTS)
        for script in leftover_hud_isolated_scripts():
            self.assertIn(script, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_WEATHER_PROFILE_WEATHER, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertTrue(has_identifier(locked_only, "LockPhase"))
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_LOCK_LINE))
        self.assertFalse(has_identifier(locked_only, SIBLING_LOCK_PROGRESS))
        self.assertFalse(has_identifier(locked_only, SIBLING_WEAPON_LINE))
        self.assertFalse(has_identifier(locked_only, SIBLING_FLARE_COUNT))
        for helper in namespace_helper_tokens():
