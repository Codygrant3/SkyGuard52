...
# if the initializer is not `= 1000`, if meta ClampMin
...
# Type / RequiredProgress / bRequiredForMissionSuccess /
# bFailureEndsMission.
...
# then this isolated ScoreReward field decl, same as leftover
...
# `float ScoreReward` / no `=` / ClampMin missing,
# retarget: type is int32 ScoreReward,
# initializer is `= 1000`, specifiers are EditAnywhere +
...
TARGET = "int32 ScoreReward = 1000;"
TARGET_WRONG_EQ_NONE = "int32 ScoreReward = NAME_None;"
TARGET_WRONG_FALSE = "int32 ScoreReward = false;"
TARGET_WRONG_TRUE = "int32 ScoreReward = true;"
TARGET_WRONG_ZERO = "int32 ScoreReward = 0;"
TARGET_WRONG_FLOAT = "float ScoreReward = 1000;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "int32 ScoreReward;"
TARGET_WRONG_REWARD = "int32 ScoreReward = 1;"
TARGET_WRONG_FNAME = "FName ScoreReward;"
TARGET_WRONG_SCORE = "int32 RequiredProgress = 1000;"
...
REMAINING_REQUIRED_PROGRESS_SCRIPT = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Reject missing initializer, `= 0` / `= 1` /
    # Harbor 40/80 / leftover Health / float / FName /
    # sibling RequiredProgress = 1000. Do not accept
    # sibling ObjectiveId / DisplayName / Type /
    # RequiredProgress / bRequiredForMissionSuccess /
    # bFailureEndsMission fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+ScoreReward\s*=\s*1000\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+ScoreReward\s*=\s*0\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+ScoreReward\s*=\s*1\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+ScoreReward\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+ScoreReward\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+ScoreReward\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+ScoreReward\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 ScoreReward is missing from "
...
def require_score_reward_specifiers(section: str) -> str:
...
class ObjectiveDefinitionScoreRewardFieldDeclContractTests(unittest.TestCase):
    def test_objective_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ScoreReward"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ScoreReward"), section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("ScoreReward", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_score_reward_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_score_reward_specifiers(section)
    ...
    def test_initializer_fails_closed_when_not_1000(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
        ...
        self.assertIn("int32 ScoreReward = 1000;", compact_origin)
        self.assertNotIn("ScoreReward = NAME_None", compact_origin)
        self.assertNotIn("ScoreReward = false", compact_origin)
        self.assertNotIn("ScoreReward = true", compact_origin)
        self.assertNotIn("ScoreReward = 0.f", compact_origin)
        self.assertNotIn("ScoreReward = 160.f", compact_origin)

    def test_score_reward_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 ScoreReward"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 1000", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFName ScoreRewards;\n",
            "\tbool ScoreReward;\n",
            "\tfloat ScoreReward = " + forty + ";\n",
            "\tfloat ScoreReward = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_score_reward_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(compact_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        with_max = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(EditAnywhere, BlueprintReadOnly, "
            'meta = (ClampMin = "0", ClampMax = "1000.0"))\n'
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_score_reward_specifiers(spec_section(with_max))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "objective-definition ScoreReward field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "objective_definition_score_reward_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
