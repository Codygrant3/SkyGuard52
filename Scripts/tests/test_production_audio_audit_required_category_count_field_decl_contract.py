# THIS IS leftover-safe FSkyguardProductionAudioAudit RequiredCategoryCount.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(BlueprintReadOnly)
# then `int32 RequiredCategoryCount = 0;`.
...
# int32, initializer dropped or changed from `0`, or Category=
# added.
...
# (class methods, Entries, Routing,
# Category="Skyguard|Audio|Production").
# Do NOT parse leftover `FSkyguardProductionAudioRouting`
# or leftover `FSkyguardProductionAudioEntry`.
# Do NOT parse `enum class ESkyguardProductionAudioCategory`
# (Ri+fleMuzzle / Ig+laSearch). Keep enums OUT of the window
# by starting at `struct FSkyguardProductionAudioAudit`.
# Sibling decls stay unclaimed: BoundProductionSourceCount /
# ExplicitMissingSourceCount / QATestOnlyCount /
# MissingCategoryEntries / InvalidSourceEntries /
...
# THIS IS NOT leftover production-audio-entry isolated decls.
# THIS IS NOT leftover production-audio-routing isolated decls.
...
# If a clone asserts EditAnywhere / BlueprintReadWrite /
# no initializer / TSoftObjectPtr / parse window
# FSkyguardProductionAudioEntry or Routing, retarget:
# parse `FSkyguardProductionAudioAudit` only, UPROPERTY is
# `BlueprintReadOnly` with NO EditAnywhere, type is `int32`,
# REQUIRED initializer `= 0`, no Category=.
# Harbor 40/80 fail-closed in the parse window and in THIS
# test file. `= 0` is the locked initializer, not Harbor.
...
TARGET = "int32 RequiredCategoryCount = 0;"
TARGET_WRONG_BARE = "int32 RequiredCategoryCount;"
TARGET_WRONG_FALSE = "int32 RequiredCategoryCount = false;"
TARGET_WRONG_TRUE = "int32 RequiredCategoryCount = true;"
TARGET_WRONG_ZERO = "int32 RequiredCategoryCount = 0.f;"
TARGET_WRONG_FORTY = "int32 RequiredCategoryCount = 40;"
TARGET_WRONG_EIGHTY = "int32 RequiredCategoryCount = 80;"
TARGET_WRONG_FLOAT = "float RequiredCategoryCount = 0;"
TARGET_WRONG_FNAME = "FName RequiredCategoryCount = 0;"
TARGET_WRONG_BOOL = "bool RequiredCategoryCount = 0;"
TARGET_WRONG_FSTRING = "FString RequiredCategoryCount = 0;"
TARGET_WRONG_TARRAY = "TArray<FName> RequiredCategoryCount;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> RequiredCategoryCount;"
...
CLASS_FIELD_ENTRIES = "TArray<FSkyguardProductionAudioEntry> Entries"
CLASS_FIELD_ROUTING = "FSkyguardProductionAudioRouting Routing"
CLASS_CATEGORY_SPEC = 'Category="Skyguard|Audio|Production"'
CLASS_METHOD_INITIALIZE = "InitializeRequiredEntries"
CLASS_METHOD_ENSURE = "EnsureDefaultEntries"
CLASS_METHOD_EVALUATE = "EvaluateReadiness"
CLASS_METHOD_CONFIGURE = "ConfigureRoutingTopology"
...
FIELD_NAME = "RequiredCategoryCount"
...
THIS_SCRIPT = (
    "Scripts/tests/test_production_audio_audit_required_category_count"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ENTRY_SOUND = (
    "Scripts/tests/test_production_audio_entry_sound"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ENTRY_DISPLAY_NAME = (
    "Scripts/tests/test_production_audio_entry_display_name"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ENTRY_PROVENANCE_ID = (
    "Scripts/tests/test_production_audio_entry_provenance_id"
    "_field_decl_contract.py"
)
DRAFTED_DISPLAY_NAME = LEFTOVER_PRODUCTION_AUDIO_ENTRY_DISPLAY_NAME
DRAFTED_PROVENANCE_ID = LEFTOVER_PRODUCTION_AUDIO_ENTRY_PROVENANCE_ID
...
LEFTOVER_PRODUCTION_AUDIO_ROUTING_DEFAULTS_TESTS = (
    "Scripts/tests/test_production_audio_routing_defaults_tests.py"
)
...
def leftover_production_audio_entry_field_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/test_production_audio_entry_"
    suffix = "_field_decl_contract.py"
    return (
        prefix + "category" + suffix,
        prefix + "display_name" + suffix,
        prefix + "source_status" + suffix,
        prefix + "sound" + suffix,
        prefix + "attenuation" + suffix,
        prefix + "concurrency" + suffix,
        prefix + "output_submix" + suffix,
        prefix + "provenance_id" + suffix,
        prefix + "source_sha256" + suffix,
    )


def leftover_production_audio_routing_field_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/test_production_audio_routing_"
    suffix = "_field_decl_contract.py"
    return (
        prefix + "master_submix" + suffix,
        prefix + "cockpit_submix" + suffix,
        prefix + "exterior_submix" + suffix,
        prefix + "weapons_submix" + suffix,
        prefix + "explosions_submix" + suffix,
        prefix + "radio_submix" + suffix,
        prefix + "cockpit_sound_mix" + suffix,
        prefix + "cockpit_exterior_attenuation" + suffix,
    )
...
def leftover_production_audio_scripts() -> tuple[str, ...]:
    return (
        ...
        LEFTOVER_PRODUCTION_AUDIO_ENTRY_DISPLAY_NAME,
        LEFTOVER_PRODUCTION_AUDIO_ENTRY_PROVENANCE_ID,
        *leftover_production_audio_entry_field_scripts(),
        *leftover_production_audio_routing_field_scripts(),
...
def leftover_bank_class_tokens() -> tuple[str, ...]:
    return (
        CLASS_FIELD_ENTRIES,
        CLASS_FIELD_ROUTING,
        CLASS_CATEGORY_SPEC,
        CLASS_METHOD_INITIALIZE,
        CLASS_METHOD_ENSURE,
        CLASS_METHOD_EVALUATE,
        CLASS_METHOD_CONFIGURE,
        STOP_BEFORE_BANK_API,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 RequiredCategoryCount = 0;`.
    # Do not accept a bare decl, Harbor 40/80, `= false`,
    # `= true`, `= 0.f`, float / bool / FName / FString /
    # TArray / TSoftObjectPtr when origin/main requires
    # int32 with initializer `= 0`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+RequiredCategoryCount\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+RequiredCategoryCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+RequiredCategoryCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+RequiredCategoryCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]+>\s+RequiredCategoryCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]+>\s+RequiredCategoryCount\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredCategoryCount\s*;",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredCategoryCount\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredCategoryCount\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredCategoryCount\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredCategoryCount\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredCategoryCount\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredCategoryCount\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def parse_window(header: str) -> str:
    ...
    stop_token = STOP_BEFORE_BANK_API
    ...
    # live-copy tokens, leftover Entry, and leftover Routing
    # stay OUT of the parse window.
    ...
    routing_at = header.find(STOP_BEFORE_ROUTING)
    if routing_at >= 0 and start <= routing_at:
        raise AssertionError(
            f"{STRUCT_NAME} parse window started at "
            f"{STOP_BEFORE_ROUTING}"
        )
...
def struct_body(header: str) -> str:
    window = header
    if (
        STRUCT_RE.search(header) is not None
        and STOP_BEFORE_BANK_API in header
    ):
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+RequiredCategoryCount\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 RequiredCategoryCount is missing from "
...
class ProductionAudioAuditRequiredCategoryCountFieldDeclContractTests(
    unittest.TestCase
):
    def test_production_audio_audit_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, "FSkyguardProductionAudioEntry")
        self.assertNotEqual(STRUCT_NAME, "FSkyguardProductionAudioRouting")
        body = struct_body(header)
        ...
        self.assertIn(STOP_BEFORE_BANK_API, header)
        self.assertNotIn(STOP_BEFORE_BANK_API, body)
        ...
        self.assertNotIn(CLASS_FIELD_ENTRIES, section)
        self.assertNotIn(CLASS_FIELD_ROUTING, section)
        self.assertNotIn(CLASS_CATEGORY_SPEC, section)
    ...
    def test_neighbor_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_UCLASS, section)
        self.assertNotIn(STOP_BEFORE_BANK_API, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_required_category_count_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 RequiredCategoryCount = 0;", compact_origin)
        self.assertNotIn("RequiredCategoryCount = false", compact_origin)
        self.assertNotIn("RequiredCategoryCount = true", compact_origin)
        self.assertNotIn("RequiredCategoryCount = 0.f", compact_origin)
        self.assertNotIn("RequiredCategoryCount = 160.f", compact_origin)

    def test_required_category_count_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RequiredCategoryCount"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TARRAY}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SOFT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FORTY}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_EIGHTY}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_BOUND_PRODUCTION_SOURCE_COUNT} = 0;\n",
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tint32 RequiredCategoryCounts = 0;\n",
            "\tint32 RequiredCategoryCoun = 0;\n",
            "\tint32 RequiredCategoryCount = 1;\n",
            "\tint32 RequiredCategoryCount = " + forty + ";\n",
            "\tint32 RequiredCategoryCount = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_edit_anywhere_or_readwrite_fails_closed(
        self,
    ) -> None:
    ...
    def test_does_not_contract_sibling_audit_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_QA_TEST_ONLY_COUNT),
            section,
        )
        self.assertTrue(
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_INVALID_SOURCE_ENTRIES),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_SOUND_BINDINGS),
            section,
        )
        self.assertTrue(
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_ROUTING_ASSETS),
            section,
        )
        self.assertTrue(
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_PRODUCTION_READY),
            section,
        )
    ...
    def test_does_not_parse_neighbors_or_leftover_banks(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_BANK_API, header)
        self.assertNotIn(STOP_BEFORE_BANK_API, section)
        self.assertNotIn(STOP_BEFORE_BANK_API, leaked)
        self.assertNotIn(STOP_BEFORE_BANK_API, window)
        self.assertEqual(header.find(STOP_BEFORE_BANK_NAIVE), -1)
        self.assertIn(STOP_BEFORE_ENTRY, header)
        self.assertNotIn(STOP_BEFORE_ENTRY, section)
        ...
        for leftover in leftover_bank_class_tokens():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, leaked)
            if leftover != STOP_BEFORE_BANK_NAIVE:
                self.assertNotIn(leftover, window)
    ...
    def test_naive_bank_class_stop_does_not_match(self) -> None:
        header = origin_main_header()
        self.assertEqual(header.find(STOP_BEFORE_BANK_NAIVE), -1)
        self.assertGreater(header.find(STOP_BEFORE_BANK_API), 0)
        naive_window = header[header.find(f"struct {STRUCT_NAME}"):]
        self.assertIn(CLASS_FIELD_ENTRIES, naive_window)
        self.assertIn(CLASS_FIELD_ROUTING, naive_window)
        self.assertIn(CLASS_CATEGORY_SPEC, naive_window)
        window = parse_window(header)
        self.assertNotIn(CLASS_FIELD_ENTRIES, window)
        self.assertNotIn(CLASS_FIELD_ROUTING, window)
        self.assertNotIn(CLASS_CATEGORY_SPEC, window)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit RequiredCategoryCount field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "required_category_count_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ENTRY_SOUND, LOCKED_SCRIPTS)
        ...
        for script in leftover_production_audio_entry_field_scripts():
            self.assertIn(script, LOCKED_SCRIPTS)
            self.assertNotEqual(script, THIS_SCRIPT)
        for script in leftover_production_audio_routing_field_scripts():
            self.assertIn(script, LOCKED_SCRIPTS)
            self.assertNotEqual(script, THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_BANK_API, locked_only)
