...
# GetTelemetry / public fields.
# Stay off leftover enum contract #60cb;
# the return type is part of this locked
# method declaration. Stay off leftover
...
# Python GetBossPhase declaration
...
# .cpp GetBossPhase body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBossDroneBase::GetBossPhase",
    "SkyguardBossDroneBase.cpp",
    "SetBossPhase",
...
class BossDroneGetBossPhaseDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_BOSS_PHASE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_BOSS_PHASE)
        self.assertIn("GetBossPhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_BOSS_PHASE)
    ...
    def test_missing_get_boss_phase_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_BOSS_PHASE)
        self.assertIn("GetBossPhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_BOSS_PHASE)
        self.assertIn("GetBossPhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_BOSS_PHASE)
        self.assertNotIn("UFUNCTION", GET_BOSS_PHASE)
        self.assertNotIn("Category", GET_BOSS_PHASE)
        self.assertNotIn("BlueprintCallable", GET_BOSS_PHASE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_BOSS_PHASE)
        self.assertIn("GetBossPhase", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tESkyguardBossPhase GetBossPhase();\n"
        missing_parens = "\tESkyguardBossPhase GetBossPhase;\n"
        wrong_return_void = "\tvoid GetBossPhase() const;\n"
        wrong_return_bool = "\tbool GetBossPhase() const;\n"
        wrong_return_float = "\tfloat GetBossPhase() const;\n"
        wrong_return_int32 = "\tint32 GetBossPhase() const;\n"
        added_arg = (
            "\tESkyguardBossPhase GetBossPhase(int32 Extra) const;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_BOSS_PHASE)
            self.assertIn("GetBossPhase", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_boss_phase_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, GET_BOSS_PHASE),
            1,
        )
        self.assertTrue(
            GET_BOSS_PHASE.startswith("ESkyguardBossPhase "),
            GET_BOSS_PHASE,
        )
        self.assertTrue(
            GET_BOSS_PHASE.endswith(";"),
            GET_BOSS_PHASE,
        )
        self.assertIn(
            "GetBossPhase()",
        ...
        self.assertIn(" const", GET_BOSS_PHASE)
        self.assertNotIn("INDEX_NONE", GET_BOSS_PHASE)
        self.assertNotIn("{", GET_BOSS_PHASE)
        self.assertNotIn("}", GET_BOSS_PHASE)
        self.assertNotIn("return ", GET_BOSS_PHASE)
        self.assertNotIn(
            "DefeatDebrisComponents",
            GET_BOSS_PHASE,
        )
        self.assertNotIn("ApplyWeaponHit", GET_BOSS_PHASE)
        self.assertNotIn("IssuePilotCommand", GET_BOSS_PHASE)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            GET_BOSS_PHASE,
        )
        ...
        self.assertNotIn("GetDefeatDebrisPieceCount", GET_BOSS_PHASE)
        self.assertNotIn("GetTelemetry", GET_BOSS_PHASE)
        self.assertNotIn("GetChinMuzzleLocation", GET_BOSS_PHASE)
        self.assertNotIn("GetGunnerMount", GET_BOSS_PHASE)
        self.assertNotIn("FaceWorldLocation", GET_BOSS_PHASE)
        self.assertNotIn("WeakPoints", GET_BOSS_PHASE)
        self.assertNotIn("SetSearchlightTracked", GET_BOSS_PHASE)
        self.assertNotIn("SetContactIdentified", GET_BOSS_PHASE)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            GET_BOSS_PHASE,
        )
        self.assertNotIn("SetLightningExposed", GET_BOSS_PHASE)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            GET_BOSS_PHASE,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, GET_BOSS_PHASE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_name = (
            "public:\n"
            "\tESkyguardBossPhase GetBossPhase\n"
            "\t() const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_BOSS_PHASE}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, GET_BOSS_PHASE)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_BOSS_PHASE),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_BOSS_PHASE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_BOSS_PHASE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_BOSS_PHASE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_boss_phase_cpp_body(
        self,
    ) -> None:
        locked_only = f"{GET_BOSS_PHASE}\n"
        self.assertNotIn("{", GET_BOSS_PHASE)
        ...
        self.assertNotIn("return false", GET_BOSS_PHASE)
        self.assertNotIn("return true", GET_BOSS_PHASE)
        self.assertNotIn(
            "DefeatDebrisComponents",
            GET_BOSS_PHASE,
        )
        self.assertNotIn("SetBossPhase", GET_BOSS_PHASE)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            GET_BOSS_PHASE,
        )
        self.assertNotIn("RegisterDefeatDebris", GET_BOSS_PHASE)
        self.assertNotIn("CleanupDefeatDebris", GET_BOSS_PHASE)
        self.assertNotIn("HandleDefeated", GET_BOSS_PHASE)

    def test_contract_does_not_relock_apply_weapon_hit_sibling(self) -> None:
        locked_only = f"{GET_BOSS_PHASE}\n"
        for token in APPLY_WEAPON_HIT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("ApplyWeaponHit", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", GET_BOSS_PHASE)
        self.assertNotIn("PortGuidanceArray", GET_BOSS_PHASE)
        self.assertNotIn("CommandCore", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("SetSearchlightTracked", GET_BOSS_PHASE)
        self.assertNotIn("IsSearchlightTracked", GET_BOSS_PHASE)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("SetContactIdentified", GET_BOSS_PHASE)
        self.assertNotIn("IsContactIdentified", GET_BOSS_PHASE)
        self.assertNotIn("OpenOrbitExposure", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", GET_BOSS_PHASE)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            GET_BOSS_PHASE,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", GET_BOSS_PHASE)
        self.assertNotIn("ASkyguardIronRainBoss", GET_BOSS_PHASE)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            GET_BOSS_PHASE,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("GetGunnerMount", GET_BOSS_PHASE)
        self.assertNotIn("GetChinTurret", GET_BOSS_PHASE)
        self.assertNotIn("GetSensorTurret", GET_BOSS_PHASE)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_BOSS_PHASE}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BOSS_PHASE)
        self.assertNotIn("GetChinMuzzleLocation", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("ESkyguardApacheSystem", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("ASkyguardGunner", GET_BOSS_PHASE)
        self.assertNotIn("FillAndFinalize", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("SkyguardRadarNode", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", GET_BOSS_PHASE)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            GET_BOSS_PHASE,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_BOSS_PHASE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_BOSS_PHASE)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            GET_BOSS_PHASE,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BOSS_PHASE)
        self.assertNotIn("MinHeightFromOriginCm", GET_BOSS_PHASE)
        self.assertNotIn("MaxIntegrity", GET_BOSS_PHASE)
        self.assertNotIn("CurrentIntegrity", GET_BOSS_PHASE)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            GET_BOSS_PHASE,
        )
        self.assertNotIn("SkyguardPathfinder", GET_BOSS_PHASE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_BOSS_PHASE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_BOSS_PHASE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_BOSS_PHASE),
            GET_BOSS_PHASE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone GetBossPhase contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_BOSS_PHASE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_BOSS_PHASE.lower(),
                "boss drone GetBossPhase contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_BOSS_PHASE)

    def test_contract_is_get_boss_phase_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", GET_BOSS_PHASE)
        self.assertNotIn("{", GET_BOSS_PHASE)
        self.assertTrue(GET_BOSS_PHASE.startswith("ESkyguardBossPhase "))
        self.assertTrue(GET_BOSS_PHASE.endswith(";"))
        self.assertIn(" const", GET_BOSS_PHASE)
        self.assertIn(UFUNCTION_BOSS, section)
