# THIS IS leftover-safe FSkyguardMissionResult::AircraftDamageFraction.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `float AircraftDamageFraction = 0.f;`.
...
# this isolated AircraftDamageFraction field decl.
# Clone parse-window from leftover FinalScore #1264
# (cursor/mission-result-final-score-field-decl-contract-c332)
# and leftover MissionId #1426
# (cursor/mission-result-mission-id-field-decl-contract-c332).
...
# Match EXACT identifier AircraftDamageFraction as a whole
# field name after float. Do NOT accept
# CompletionTimeSeconds (also float ... = 0.f).
# Siblings stay unclaimed: MissionId / bMissionSucceeded /
# ShotsFired / Hits / CompletionTimeSeconds /
...
# test file (not the whole header). Stay Apache CPG 30 mm /
# Hydra / Hellfire.
...
TARGET = "float AircraftDamageFraction = 0.f;"
TARGET_WRONG_FALSE = "float AircraftDamageFraction = false;"
TARGET_WRONG_ONE = "float AircraftDamageFraction = 1.f;"
TARGET_WRONG_INT = "float AircraftDamageFraction = 0;"
TARGET_WRONG_TRUE = "float AircraftDamageFraction = true;"
TARGET_BARE = "float AircraftDamageFraction;"
TARGET_WRONG_TIME = "float CompletionTimeSeconds = 0.f;"
TARGET_WRONG_FINAL = "int32 FinalScore = 0;"
TARGET_WRONG_MEDAL = "int32 MedalTier = 0;"
TARGET_WRONG_FNAME = "FName AircraftDamageFraction;"
TARGET_WRONG_INT32 = "int32 AircraftDamageFraction = 0;"
LOCKED_DECL = TARGET
...
NEIGHBOR_AUDIO_BANK = "USkyguardAudioProductionBank"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # CompletionTimeSeconds / leftover FinalScore for
    # FSkyguardMissionResult::AircraftDamageFraction
    # (must be float AircraftDamageFraction = 0.f).
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"\bCompletionTimeSeconds\b", compact) and not re.search(
        r"\bAircraftDamageFraction\b", compact
    ):
        return False
    if re.search(
        r"float\s+AircraftDamageFraction\s*=\s*0\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+AircraftDamageFraction\s*=\s*0\s*;",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+AircraftDamageFraction\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+AircraftDamageFraction\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+AircraftDamageFraction\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float AircraftDamageFraction is missing from "
...
class MissionResultAircraftDamageFractionFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "AircraftDamageFraction"), section)
        ...
        self.assertNotIn(NEIGHBOR_AUDIO_BANK, section)
        ...
        self.assertNotIn("float AircraftDamageFraction)", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("AircraftDamageFraction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_aircraft_damage_fraction_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_aircraft_damage_fraction_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float AircraftDamageFraction"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("0.f;"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "float AircraftDamageFraction = 0.f;")
        ...
        self.assertNotIn(SIBLING_TIME, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TIME}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_TIME}\n", LOCKED_DECL)
        self.assertIn("AircraftDamageFraction", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            "\tfloat AircraftDamage = 0.f;\n",
            "\tbool AircraftDamageFraction = false;\n",
            "\tfloat AircraftDamageFraction = 0;\n",
            "\tfloat AircraftDamageFraction = 0.0;\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result AircraftDamageFraction field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_aircraft_damage_fraction_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(NEIGHBOR_AUDIO_BANK, locked_only)
