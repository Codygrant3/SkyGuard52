...
# or lock the GetFinalScore body.
...
# CPG debrief fail-closed. This is not leftover
# mission-debrief defaults. This is not leftover
# sortie-presentation fail-closed. Stay off
...
# Configure / GetPresentation /
# GetDebriefNarrative / IsProgressSaved,
# leftover mission-debrief defaults,
# leftover sortie-presentation state enum,
# leftover HarborIndustrial skyline enum (not a
# Harbor clock retune), and sibling widget
# contracts stay sibling-only.
GET_FINAL_SCORE = ("int32 GetFinalScore() const;")
UFUNCTION_DEBRIEF = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Presentation|Debrief")'
)
# Leftover #56–#64 plus this class's production files.
# This lane only adds an isolated Python GetFinalScore
# declaration contract. Stay off Configure,
...
# BriefingWidget GetPresentation, leftover briefing
# widget Configure / GetMissionTitle / GetBriefingText
...
# fail-closed, leftover CPG debrief, leftover
# mission-debrief defaults, leftover Gunner helpers,
# leftover Harbor clocks, leftover theater-kit /
...
# GetDebriefNarrative / IsProgressSaved, leftover
# sortie-presentation state enum, leftover CPG
# debrief fail-closed #8ccd, leftover
# sortie-presentation fail-closed #7600, and dirty
# workspace paths.
LOCKED = {
...
# leftover CPG debrief, leftover mission-debrief
# defaults, leftover objective-runtime /
...
# DebriefWidget GetPresentation, leftover
# GetDebrief, leftover GetDebriefNarrative,
# leftover IsProgressSaved, leftover
# GetPresentationState, leftover AcknowledgeDebrief,
# leftover RetrySave, leftover TravelNext, leftover
# HandleDebriefKey, leftover sortie-presentation
# state enum, leftover CPG debrief fail-closed
# #8ccd, leftover sortie-presentation fail-closed
# #7600, and leftover BriefingWidget stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# .cpp GetFinalScore body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardDebriefWidget::GetFinalScore",
    "SkyguardSortiePresentationWidgets.cpp",
)
SIBLING_TYPES = (
...
class DebriefWidgetGetFinalScoreDeclContractTests(unittest.TestCase):
    def test_debrief_widget_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_FINAL_SCORE),
            section,
        )
    ...
    def test_briefing_widget_does_not_satisfy_class(self) -> None:
        briefing_only = (
            ...
            "{\n"
            "public:\n"
            "\tint32 GetFinalScore() const { return Score; }\n"
            "\tFText GetMissionTitle() const;\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_FINAL_SCORE)
        self.assertIn("GetFinalScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_FINAL_SCORE))

    def test_missing_get_final_score_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tvoid Configure("
            "USkyguardSortiePresentationComponent* InPresentation);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_FINAL_SCORE)
        self.assertIn("GetFinalScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_FINAL_SCORE)
        self.assertIn("GetFinalScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_FINAL_SCORE)
        self.assertNotIn("UFUNCTION", GET_FINAL_SCORE)
        self.assertNotIn("Category", GET_FINAL_SCORE)
        self.assertNotIn("BlueprintCallable", GET_FINAL_SCORE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_FINAL_SCORE)
        self.assertIn("GetFinalScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tint32 GetFinalScore();\n"
        wrong_return_void = "\tvoid GetFinalScore() const;\n"
        wrong_return_bool = "\tbool GetFinalScore() const;\n"
        wrong_return_float = "\tfloat GetFinalScore() const;\n"
        added_arg = "\tint32 GetFinalScore(bool bReady) const;\n"
        wrong_name = (
            "\tESkyguardSortiePresentationState "
            "GetPresentationState() const;\n"
        )
        ...
        int64_return = "\tint64 GetFinalScore() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_FINAL_SCORE)
            self.assertIn("GetFinalScore", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_final_score_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_FINAL_SCORE),
            GET_FINAL_SCORE,
        )
        self.assertTrue(has_declaration(section, GET_FINAL_SCORE))
        self.assertEqual(
            declaration_count(section, GET_FINAL_SCORE),
            1,
        )
        self.assertTrue(
            GET_FINAL_SCORE.startswith("int32 "),
            GET_FINAL_SCORE,
        )
        self.assertTrue(
            GET_FINAL_SCORE.endswith(";"),
            GET_FINAL_SCORE,
        )
        self.assertIn("GetFinalScore()", GET_FINAL_SCORE)
        self.assertIn(" const", GET_FINAL_SCORE)
        self.assertIn("int32", GET_FINAL_SCORE)
        self.assertNotIn("INDEX_NONE", GET_FINAL_SCORE)
        self.assertNotIn("{", GET_FINAL_SCORE)
        self.assertNotIn("}", GET_FINAL_SCORE)
        self.assertNotIn("return ", GET_FINAL_SCORE)
        self.assertNotIn("return Score", GET_FINAL_SCORE)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_FINAL_SCORE}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_FINAL_SCORE))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_FINAL_SCORE),
            GET_FINAL_SCORE,
        )
        exact_inline = "int32 GetFinalScore() const { return Score; }"
        self.assertTrue(has_declaration(exact_inline, GET_FINAL_SCORE))
        self.assertEqual(
            require_declaration(exact_inline, GET_FINAL_SCORE),
            GET_FINAL_SCORE,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_FINAL_SCORE),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{GET_FINAL_SCORE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_FINAL_SCORE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_FINAL_SCORE),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_FINAL_SCORE)
        self.assertNotIn("return Score", GET_FINAL_SCORE)
        self.assertNotIn("return Other", GET_FINAL_SCORE)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_FINAL_SCORE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_FINAL_SCORE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_FINAL_SCORE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_final_score_body(self) -> None:
        locked_only = f"{GET_FINAL_SCORE}\n"
        self.assertNotIn("{", GET_FINAL_SCORE)
        self.assertNotIn("}", GET_FINAL_SCORE)
        self.assertNotIn("return ", GET_FINAL_SCORE)
        self.assertNotIn("return Score", GET_FINAL_SCORE)
        self.assertNotIn("return Score", locked_only)
        self.assertNotIn(
            "USkyguardDebriefWidget::GetFinalScore",
            GET_FINAL_SCORE,
        )
        ...
        self.assertNotIn("return false", GET_FINAL_SCORE)
        self.assertNotIn("return true", GET_FINAL_SCORE)

    def test_contract_does_not_relock_constructor(self) -> None:
        locked_only = f"{GET_FINAL_SCORE}\n"
        for neighbor in CONSTRUCTOR_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("ObjectInitializer", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_configure(self) -> None:
        ...
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("Configure(", GET_FINAL_SCORE)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("GetPresentation()", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_get_debrief(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("GetDebrief()", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_get_debrief_narrative(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NARRATIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("GetDebriefNarrative", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_is_progress_saved(self) -> None:
        ...
        for neighbor in IS_PROGRESS_SAVED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("IsProgressSaved", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_get_presentation_state(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("GetPresentationState", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("AcknowledgeDebrief", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_retry_save(self) -> None:
        ...
        for neighbor in RETRY_SAVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("RetrySave", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_travel_next(self) -> None:
        ...
        for neighbor in TRAVEL_NEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("TravelNext", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_handle_debrief_key(self) -> None:
        ...
        for neighbor in HANDLE_DEBRIEF_KEY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("HandleDebriefKey", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_FINAL_SCORE)
        self.assertNotIn("Unconfigured", GET_FINAL_SCORE)
        self.assertNotIn("Warming", GET_FINAL_SCORE)
        self.assertNotIn("Launched", GET_FINAL_SCORE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_FINAL_SCORE,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_FINAL_SCORE)
        self.assertNotIn("FSkyguardBriefingCard", GET_FINAL_SCORE)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_FINAL_SCORE)
        self.assertNotIn("FSkyguardHowToFlyRow", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_FINAL_SCORE)
        self.assertNotIn("FillResultCombatStats", GET_FINAL_SCORE)
        self.assertNotIn("ASkyguardGunner", GET_FINAL_SCORE)
        self.assertNotIn("FillAndFinalize", GET_FINAL_SCORE)
        self.assertNotIn("FillAndFail", GET_FINAL_SCORE)
        self.assertNotIn("ApplyHydraForClusters", GET_FINAL_SCORE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_FINAL_SCORE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_FINAL_SCORE),
            GET_FINAL_SCORE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "debrief widget GetFinalScore contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_FINAL_SCORE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"debrief widget GetFinalScore contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_FINAL_SCORE)

    def test_contract_is_get_final_score_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_FINAL_SCORE.startswith("int32 "))
        self.assertTrue(GET_FINAL_SCORE.endswith(";"))
        self.assertIn(" const", GET_FINAL_SCORE)
        self.assertIn(UFUNCTION_DEBRIEF, section)
