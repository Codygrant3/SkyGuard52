...
# leftover harness MaximumTruePeakDbTP = -1.f).
...
# Sibling receipt float MaximumAudioThreadMs = 0.f lives in
# this struct — do not treat `= 0` + `.f` as Harbor. Harbor
# is 40 + `.f` / 80 + `.f` only. `= -120.f` is not Harbor.
...
# then this isolated MaximumTruePeakDbTP field decl.
...
# MaximumAudioThreadMs / bPackagedDevelopmentBuild /
# bAudibleDeviceObserved / bCalibratedMetering /
# bProductionBankReady / bAccepted.
# Identifier bounds: match EXACT `MaximumTruePeakDbTP` as a
...
# required initializer `= -120.f`, UPROPERTY is
...
TARGET = "float MaximumTruePeakDbTP = -120.f;"
TARGET_WRONG_BARE = "float MaximumTruePeakDbTP;"
TARGET_WRONG_EQ_ZERO = "float MaximumTruePeakDbTP = 0.f;"
TARGET_WRONG_EQ_NEG_ONE = "float MaximumTruePeakDbTP = -1.f;"
TARGET_WRONG_EQ_NONE = "float MaximumTruePeakDbTP = NAME_None;"
TARGET_WRONG_TRUE = "float MaximumTruePeakDbTP = true;"
TARGET_WRONG_FALSE = "float MaximumTruePeakDbTP = false;"
TARGET_WRONG_FSTRING = "FString MaximumTruePeakDbTP;"
TARGET_WRONG_INT = "int32 MaximumTruePeakDbTP = 0;"
TARGET_WRONG_FNAME = "FName MaximumTruePeakDbTP;"
TARGET_WRONG_FTEXT = "FText MaximumTruePeakDbTP;"
TARGET_WRONG_BOOL = "bool MaximumTruePeakDbTP = false;"
TARGET_WRONG_TARRAY = "TArray<float> MaximumTruePeakDbTP;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MaximumTruePeakDbTP;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_THREAD_MS = "float MaximumAudioThreadMs = 0.f;"
TARGET_WRONG_PEAK_VOICES = "int32 PeakActiveVoices = 0;"
...
FIELD_NAME = "MaximumTruePeakDbTP"
...
THIS_SCRIPT = (
    ...
    "_true_peak_dbtp_field_decl_contract.py"
)
CLONE_BUILD_SHA256 = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Match the EXACT identifier MaximumTruePeakDbTP as a
    # whole field name after float. Do NOT accept sibling
    # MaximumAudioThreadMs. Reject dropped/changed
    # initializer (including `= 0.f` or leftover harness
    # `= -1.f`), Harbor 40/80, wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"float\s+MaximumTruePeakDbTP\s*=\s*-120\.f\s*;",
        compact,
    )
    if re.search(r"\bMaximumAudioThreadMs\b", compact) and exact is None:
        return False
    if re.search(r"\bPeakActiveVoices\b", compact) and exact is None:
        return False
    if re.search(
        r"float\s+MaximumTruePeakDbTP\s*;",
        compact,
    ) and re.search(
        r"float\s+MaximumTruePeakDbTP\s*=",
        compact,
    ...
    for assigned in re.findall(
        r"float\s+MaximumTruePeakDbTP\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "-120.f":
            return False
    if re.search(
        r"int32\s+MaximumTruePeakDbTP\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MaximumTruePeakDbTP\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+MaximumTruePeakDbTP\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MaximumTruePeakDbTP\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MaximumTruePeakDbTP\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+MaximumTruePeakDbTP\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MaximumTruePeakDbTP\b",
        compact,
    ):
        return False
    if exact is None:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MaximumTruePeakDbTP\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaximumTruePeakDbTP is missing from "
...
class AudibleAcceptanceReceiptMaximumTruePeakDbtpFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_maximum_true_peak_dbtp_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            ...
            f"\tfloat {SIBLING_MAXIMUM_AUDIO_THREAD_MS} = 0.f;\n"
    ...
    def test_maximum_audio_thread_ms_does_not_satisfy_true_peak(self) -> None:
        thread_only = f"\t{TARGET_WRONG_THREAD_MS}\n"
        self.assertFalse(has_declaration(thread_only, LOCKED_DECL))
        self.assertFalse(has_identifier(thread_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(thread_only, SIBLING_MAXIMUM_AUDIO_THREAD_MS)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(thread_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        voices_only = f"\t{TARGET_WRONG_PEAK_VOICES}\n"
        self.assertFalse(has_declaration(voices_only, LOCKED_DECL))
        self.assertFalse(has_identifier(voices_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(voices_only, SIBLING_PEAK_ACTIVE_VOICES)
        )
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{TARGET_WRONG_THREAD_MS}\n"
            f"\t{TARGET_WRONG_PEAK_VOICES}\n"
            "};\n"
        )
        section = spec_section(mixed)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float MaximumTruePeakDbTP = -120.f;", compact_origin)
        self.assertNotIn("MaximumTruePeakDbTP = 0.f", compact_origin)
        self.assertNotIn("MaximumTruePeakDbTP = -1.f", compact_origin)
        self.assertNotIn("MaximumTruePeakDbTP = true", compact_origin)
        self.assertNotIn("int32 MaximumTruePeakDbTP", compact_origin)

    def test_maximum_true_peak_dbtp_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MaximumTruePeakDbTP"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= -120.f", LOCKED_DECL)
        self.assertNotIn("= 0.f", LOCKED_DECL)
        self.assertNotIn("= -1.f", LOCKED_DECL)
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn(SIBLING_MAXIMUM_AUDIO_THREAD_MS, LOCKED_DECL)
        ...
        wrongs = (
            ...
            TARGET_WRONG_SOFT,
            TARGET_WRONG_HEALTH,
            TARGET_WRONG_THREAD_MS,
            TARGET_WRONG_PEAK_VOICES,
        )
        for wrong in wrongs:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tfloat {SIBLING_MAXIMUM_AUDIO_THREAD_MS} = 0.f;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tfloat MaximumTruePeakDbTPs = -120.f;\n",
            "\tfloat MaximumTruePeakDbTP = 0.f;\n",
            "\tbool MaximumTruePeakDbTP;\n",
            "\tfloat MaximumTruePeakDbTP = " + forty + ";\n",
            "\tfloat MaximumTruePeakDbTP = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt MaximumTruePeakDbTP field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "maximum_true_peak_dbtp_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_maximum_true_peak", THIS_SCRIPT)
