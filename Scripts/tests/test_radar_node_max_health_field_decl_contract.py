# THIS IS leftover-safe ASkyguardRadarNode::MaxHealth.
# origin/main form: one-line
# UPROPERTY(EditAnywhere, BlueprintReadWrite,
# Category="Skyguard|Campaign")
# then `float MaxHealth = 160.f;`.
...
# BlueprintReadOnly-only. Category IS
# `Skyguard|Campaign`. Initializer IS `= 160.f`.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if Category is missing, or if the
# default is not `= 160.f`.
# Accept one-line and split-line UPROPERTY wraps.
# If a clone asserts BlueprintReadOnly-only and
# origin/main is EditAnywhere BlueprintReadWrite,
# match origin/main (keep EditAnywhere
# BlueprintReadWrite).
# Parse CLASS public section ONLY after
# `class SKYGUARD52_API ASkyguardRadarNode`, stop at
# `protected:`.
# Do NOT parse protected Body / Dish.
# Do NOT contract sibling Health (later slot).
# Do NOT contract leftover-safe UFUNCTION siblings
# ApplyDamage / IsDestroyed / ResetNode
# (#1292-#1294).
...
# #411-#426.
# THIS IS NOT leftover RadarNode UFUNCTION drafts
# #1292-#1294.
# THIS IS NOT leftover protect-asset-cargo-proxy #60.
# THIS IS NOT leftover ProtectAsset IsDestroyed #1288
# (different class ASkyguardProtectAsset).
...
# Do NOT parse ASkyguardProtectAsset.
# Skip Pictogram.
...
HEADER_PATH = "Source/Skyguard52/SkyguardRadarNode.h"
CLASS_NAME = "ASkyguardRadarNode"
TARGET = "float MaxHealth = 160.f;"
TARGET_WRONG_BARE = "float MaxHealth;"
TARGET_WRONG_ZERO = "float MaxHealth = 0.f;"
TARGET_WRONG_INT = "int32 MaxHealth = 160;"
TARGET_WRONG_BOOL = "bool MaxHealth = true;"
TARGET_WRONG_RENAME = "float MaxIntegrity = 160.f;"
TARGET_WRONG_SIBLING_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SIBLING_APPLY = "void ApplyDamage(float Amount);"
TARGET_WRONG_SIBLING_RESET = "void ResetNode();"
TARGET_WRONG_SIBLING_IS_DESTROYED = "bool IsDestroyed() const;"
LOCKED_DECL = TARGET
UPROPERTY_EDIT = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Campaign")'
...
STOP_BEFORE_BODY = "TObjectPtr<UStaticMeshComponent> Body"
STOP_BEFORE_DISH = "TObjectPtr<UStaticMeshComponent> Dish"
...
SIBLING_RESET_NODE = "ResetNode"
UNCONTRACTED_HEALTH = "float Health"
...
MAX_HEALTH_DECL_RE = re.compile(
    r"\bfloat\s+MaxHealth\s*=\s*160\.f\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on leftover-safe isolated UPROPERTY
    # `float MaxHealth = 160.f;`. Do not accept a missing
    # initializer, a renamed field, a non-float type, sibling
    # Health, or leftover-safe UFUNCTION siblings ApplyDamage /
    # IsDestroyed / ResetNode when origin/main has
    # `float MaxHealth = 160.f;`.
    del declaration
    compact = collapsed(region)
    if MAX_HEALTH_DECL_RE.search(compact) is None:
        return False
    if re.search(r"\b(?:int32|bool|void|double)\s+MaxHealth\b", compact):
        return False
    return True
...
def count_one_declaration(region: str, declaration: str) -> int:
    del declaration
    compact = collapsed(region)
    return len(MAX_HEALTH_DECL_RE.findall(compact))
...
def attached_upropperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MaxHealth\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaxHealth is missing from "
...
class RadarNodeMaxHealthFieldDeclContractTests(unittest.TestCase):
    def test_radar_node_class_exists(self) -> None:
        ...
        self.assertIn("MaxHealth", section)
    ...
    def test_missing_class_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            class_body(
                "class ASkyguardUnrelatedRadarNode "
                ": public AActor\n{\n};\n"
            )
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertNotIn("MaxHealth", section)
        for token in protected_component_tokens()[:1]:
            self.assertNotIn(token, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaxHealth", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_max_health_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_upropperty_macro_alone_does_not_satisfy(self) -> None:
    ...
    def test_origin_main_upropperty_metadata_is_present(self) -> None:
        ...
        specifiers = attached_upropperty_specifiers(section)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn("float MaxHealth = 160.f;", compact_origin)
        self.assertNotIn("int32 MaxHealth", compact_origin)

    def test_max_health_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MaxHealth"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 160.f", LOCKED_DECL)
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tvoid Apply" + leftover_retired_primary_hits_field() + "();\n"
        )
        leftover_guided = (
            "\tvoid Apply" + leftover_retired_guided_hits_field() + "();\n"
        )
        harbor_forty = "\tfloat MaxHealth = " + "40" + ".f;\n"
        harbor_eighty = "\tfloat MaxHealth = " + "80" + ".f;\n"
        wrongs = (
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_ZERO}\n",
            f"\t{TARGET_WRONG_INT}\n",
            ...
            f"\t{TARGET_WRONG_SIBLING_HEALTH}\n",
            f"\t{TARGET_WRONG_SIBLING_APPLY}\n",
            f"\t{TARGET_WRONG_SIBLING_RESET}\n",
            f"\t{TARGET_WRONG_SIBLING_IS_DESTROYED}\n",
            leftover_primary,
            leftover_guided,
            harbor_forty,
            harbor_eighty,
    ...
    def test_readonly_instead_of_readwrite_fails_closed(self) -> None:
        readonly_wrap = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        origin = attached_upropperty_specifiers(
    ...
    def test_missing_category_fails_closed(self) -> None:
        ...
        section = public_section(no_category)
        specifiers = attached_upropperty_specifiers(section)
    ...
    def test_does_not_contract_sibling_radar_api(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(LEFTOVER_PROTECT_ASSET_CLASS, LOCKED_DECL)
        section = public_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\tvoid {SIBLING_RESET_NODE}();\n"
            f"\t{UNCONTRACTED_HEALTH} = 160.f;\n"
    ...
    def test_declaration_does_not_invent_upropperty_metadata(self) -> None:
        self.assertEqual(
            UPROPERTY_EDIT,
            'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
            'Category="Skyguard|Campaign")',
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("{", LOCKED_DECL)
        self.assertNotIn("return ", LOCKED_DECL)
        self.assertNotIn("SkyguardRadarNode.cpp", THIS_SCRIPT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar-node MaxHealth field decl contract "
    ...
    def test_contract_is_target_upropperty_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
