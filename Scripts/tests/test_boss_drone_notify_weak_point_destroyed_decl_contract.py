...
# or lock the NotifyWeakPointDestroyed body.
# origin/main is the live split-line form
# with no UFUNCTION and no inline body
# (`void NotifyWeakPointDestroyed(` plus
# USkyguardBossWeakPointComponent* WeakPoint,
# ESkyguardBossWeapon Weapon);).
# lock that full parameter list and accept
# that split-line form, one-line wrap, and
# split-line wraps. Do not invent a
# UFUNCTION on this method. origin/main
# has none. Do not lock the body (there
# is none on origin/main). Parse the
# public class section of
# ASkyguardBossDroneBase only.
# Stay off leftover strike / lock-eligible
# surfaces. Stay off leftover sibling
# GetDefeatDebrisPieceCount /
# GetMaxDefeatDebrisPieces /
# IssuePilotCommand /
# ApplyWeaponHit. Stay off leftover
...
# isolated-test drafts #107–#469 including leftover
# searchlight-track-runtime-defaults #7347,
# leftover settings-apply-broadcast #1268,
# leftover mesh-bind-slot-fields #6829,
...
# Harbor-sensitive and unlocked. Leftover
# ESkyguardBossWeapon enum contract stays
# sibling-only; the parameter type is part of
...
# origin/main has no UFUNCTION on this method.
# Do not invent one. Leftover invented macros
# do not satisfy the declaration lock.
LEFTOVER_INVENTED_UFUNCTION = (
...
# Python NotifyWeakPointDestroyed declaration
# contract on ASkyguardBossDroneBase. Stay off
# leftover sibling GetDefeatDebrisPieceCount /
# GetMaxDefeatDebrisPieces / IssuePilotCommand /
# ApplyWeaponHit, leftover
...
# leftover isolated-test drafts #107–#469,
# leftover searchlight-track-runtime-defaults
# #7347, leftover settings-apply-broadcast
# #1268, leftover mesh-bind-slot-fields
# #6829, leftover BlackKite siblings, leftover
...
# GetMaxDefeatDebrisPieces /
# ApplyWeaponHit siblings, leftover
# GetBossPhase / GetTelemetry, leftover
# apache aircraft isolated contracts, leftover
...
# not locked here. Sibling GetDefeatDebrisPieceCount,
# IssuePilotCommand, GetMaxDefeatDebrisPieces,
# ApplyWeaponHit, leftover
# GetBossPhase / GetTelemetry, leftover
# strike / lock-eligible surfaces, leftover
# USkyguardBossWeakPointComponent fields, leftover
...
# Leftover mesh-bind-slot-fields #6829 stays unlocked.
LEFTOVER_MESH_BIND_NOT_LOCKED = (
    "test_mesh_bind_slot_fields_contract.py",
    "FSkyguardMeshBindSlot",
)
...
# unlocked. The parameter type is part of the
# locked NotifyWeakPointDestroyed declaration; do not add a
...
# .cpp NotifyWeakPointDestroyed body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBossDroneBase::NotifyWeakPointDestroyed",
    "SkyguardBossDroneBase.cpp",
    "SetBossPhase",
...
class BossDroneNotifyWeakPointDestroyedDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, NOTIFY_WEAK_POINT_DESTROYED),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertIn("NotifyWeakPointDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_missing_notify_weak_point_destroyed_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertIn("NotifyWeakPointDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{LEFTOVER_INVENTED_UFUNCTION}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertIn("NotifyWeakPointDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_locked_declaration_has_no_ufunction(self) -> None:
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, NOTIFY_WEAK_POINT_DESTROYED),
            section,
        )
        self.assertNotIn("UFUNCTION", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("BlueprintCallable", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("BlueprintPure", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("Category", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "Skyguard|Boss",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        compact = collapsed(section)
        index = compact.find("NotifyWeakPointDestroyed")
        self.assertGreaterEqual(index, 0, compact)
        prefix = compact[max(0, index - 80) : index]
        self.assertNotIn("UFUNCTION", prefix)
        self.assertNotIn(LEFTOVER_INVENTED_UFUNCTION, NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertIn("NotifyWeakPointDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        simplified = f"\t{leftover_simplified_apply_weapon_hit()}\n"
        missing_weak_point = (
            "\tvoid NotifyWeakPointDestroyed("
            "ESkyguardBossWeapon Weapon);\n"
        )
        ...
        missing_parens = "\tvoid NotifyWeakPointDestroyed;\n"
        wrong_return_bool = (
            "\tbool NotifyWeakPointDestroyed("
            "USkyguardBossWeakPointComponent* WeakPoint, "
            "ESkyguardBossWeapon Weapon);\n"
        )
        wrong_return_int32 = (
            "\tint32 NotifyWeakPointDestroyed("
            "USkyguardBossWeakPointComponent* WeakPoint, "
            "ESkyguardBossWeapon Weapon);\n"
        )
        added_const = (
            "\tvoid NotifyWeakPointDestroyed("
            "USkyguardBossWeakPointComponent* WeakPoint, "
            "ESkyguardBossWeapon Weapon) const;\n"
        )
        leftover_handle = (
        ...
        leftover_notify = f"\t{APPLY_WEAPON_HIT}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, NOTIFY_WEAK_POINT_DESTROYED)
            self.assertIn("NotifyWeakPointDestroyed", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_notify_weak_point_destroyed_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, NOTIFY_WEAK_POINT_DESTROYED),
            1,
        )
        self.assertTrue(
            NOTIFY_WEAK_POINT_DESTROYED.startswith("void "),
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertTrue(
            NOTIFY_WEAK_POINT_DESTROYED.endswith(";"),
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertIn("NotifyWeakPointDestroyed(", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertIn(
            "USkyguardBossWeakPointComponent*",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertIn("WeakPoint", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertIn("ESkyguardBossWeapon", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("UPrimitiveComponent*", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("HitComponent", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("float Damage", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("FVector HitLocation", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("FVector HitDirection", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotEqual(
            NOTIFY_WEAK_POINT_DESTROYED,
            leftover_simplified_apply_weapon_hit(),
        )
        self.assertNotIn(" const", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("INDEX_NONE", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("{", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("}", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("return ", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "DefeatDebrisComponents",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertNotIn("GetDefeatDebrisPieceCount", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("IssuePilotCommand", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        ...
        self.assertNotIn("GetBossPhase", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetTelemetry", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetChinMuzzleLocation", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetGunnerMount", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("FaceWorldLocation", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("WeakPoints", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("SetSearchlightTracked", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("SetContactIdentified", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertNotIn("SetLightningExposed", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{NOTIFY_WEAK_POINT_DESTROYED}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, NOTIFY_WEAK_POINT_DESTROYED)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, NOTIFY_WEAK_POINT_DESTROYED),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("return ", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("return false", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "DefeatDebrisComponents",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{NOTIFY_WEAK_POINT_DESTROYED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", NOTIFY_WEAK_POINT_DESTROYED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_notify_weak_point_destroyed_cpp_body(
        self,
    ) -> None:
        locked_only = f"{NOTIFY_WEAK_POINT_DESTROYED}\n"
        self.assertNotIn("{", NOTIFY_WEAK_POINT_DESTROYED)
        ...
        self.assertNotIn("return false", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("return true", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "DefeatDebrisComponents",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertNotIn("SetBossPhase", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertNotIn("RegisterDefeatDebris", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("CleanupDefeatDebris", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("HandleDefeated", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_get_defeat_debris_piece_count_sibling(self) -> None:
        ...
        for token in GET_DEFEAT_DEBRIS_PIECE_COUNT_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetDefeatDebrisPieceCount", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_apply_weapon_hit_sibling(
        self,
    ) -> None:
        locked_only = f"{NOTIFY_WEAK_POINT_DESTROYED}\n"
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("PortGuidanceArray", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("CommandCore", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("SetSearchlightTracked", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("IsSearchlightTracked", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("ASkyguardBlackKiteBoss", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("SetContactIdentified", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("IsContactIdentified", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("OpenOrbitExposure", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("ASkyguardIronRainBoss", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetGunnerMount", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetChinTurret", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetSensorTurret", NOTIFY_WEAK_POINT_DESTROYED)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{NOTIFY_WEAK_POINT_DESTROYED}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("GetChinMuzzleLocation", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("ESkyguardApacheSystem", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("ASkyguardGunner", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("FillAndFinalize", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("SkyguardRadarNode", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            NOTIFY_WEAK_POINT_DESTROYED,
        )

    def test_contract_does_not_relock_leftover_mesh_bind_slot_fields(
        self,
    ) -> None:
        locked_only = f"{NOTIFY_WEAK_POINT_DESTROYED}\n"
        for token in LEFTOVER_MESH_BIND_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "test_mesh_bind_slot_fields_contract.py",
        ...
        self.assertNotIn("FSkyguardMeshBindSlot", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("MinHeightFromOriginCm", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("MaxIntegrity", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        self.assertNotIn("SkyguardPathfinder", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{NOTIFY_WEAK_POINT_DESTROYED}\n"
        self.assertEqual(
            require_declaration(locked_only, NOTIFY_WEAK_POINT_DESTROYED),
            NOTIFY_WEAK_POINT_DESTROYED,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone NotifyWeakPointDestroyed contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, NOTIFY_WEAK_POINT_DESTROYED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                NOTIFY_WEAK_POINT_DESTROYED.lower(),
                "boss drone NotifyWeakPointDestroyed contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, NOTIFY_WEAK_POINT_DESTROYED)

    def test_contract_is_notify_weak_point_destroyed_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("{", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertTrue(NOTIFY_WEAK_POINT_DESTROYED.startswith("void "))
        self.assertTrue(NOTIFY_WEAK_POINT_DESTROYED.endswith(";"))
        self.assertIn(
            "USkyguardBossWeakPointComponent*",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
    ...
    def test_simplified_or_sibling_signature_is_not_origin_main(
        self,
    ) -> None:
        simplified = leftover_simplified_apply_weapon_hit()
        self.assertNotEqual(simplified, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("USkyguardBossWeakPointComponent", simplified)
        self.assertIn(
            "USkyguardBossWeakPointComponent*",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
        ...
        self.assertFalse(has_declaration(section, simplified))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, simplified)
        self.assertIn("ApplyWeaponHit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_contract_does_not_add_boss_weapon_enum_contract(self) -> None:
        ...
        for token in LEFTOVER_BOSS_WEAPON_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertIn("ESkyguardBossWeapon", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("enum class", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("UENUM", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn(
            "test_boss_weapon_enum_contract.py",
            NOTIFY_WEAK_POINT_DESTROYED,
        )
    ...
    def test_contract_does_not_lock_public_fields(self) -> None:
        ...
        for token in PUBLIC_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("OnBossPhaseChanged", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("OnPilotCommandNative", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("CurrentPilotCommand", NOTIFY_WEAK_POINT_DESTROYED)
    ...
    def test_harbor_adjacent_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_SEPARATION:
            ...
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("MinimumCivilianSeparationMeters", NOTIFY_WEAK_POINT_DESTROYED)
        self.assertNotIn("MinimumWeaponSeparationMeters", NOTIFY_WEAK_POINT_DESTROYED)
