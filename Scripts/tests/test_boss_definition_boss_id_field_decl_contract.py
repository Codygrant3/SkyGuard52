# THIS IS leftover-safe FSkyguardBossDefinition BossId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName BossId;` (no initializer).
...
# Do NOT contract sibling fields Callsign / WeakPoints /
...
# isolated BossId field decl, same as leftover
...
# FName BossId, no `=`, specifiers are EditAnywhere +
...
TARGET = "FName BossId;"
TARGET_WRONG_EQ_NONE = "FName BossId = NAME_None;"
TARGET_WRONG_FALSE = "FName BossId = false;"
TARGET_WRONG_TRUE = "FName BossId = true;"
TARGET_WRONG_ZERO = "FName BossId = 0.f;"
TARGET_WRONG_FLOAT = "float BossId;"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+BossId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName BossId is missing from "
...
def require_boss_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} BossId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} BossId"
        )
...
class BossDefinitionBossIdFieldDeclContractTests(unittest.TestCase):
    def test_boss_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BossId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BossId"), section)
        self.assertNotIn(STOP_BEFORE_WEAK_POINT_DEF, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        ...
        self.assertIn("BossId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_boss_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_boss_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName BossId;", compact_origin)
        self.assertNotIn("BossId = NAME_None", compact_origin)
        self.assertNotIn("BossId = false", compact_origin)
        self.assertNotIn("BossId = true", compact_origin)
        self.assertNotIn("BossId = 0.f", compact_origin)
        self.assertNotIn("BossId = 160.f", compact_origin)

    def test_boss_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName BossId"),
            LOCKED_DECL,
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_boss_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_boss_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_boss_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_boss_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_boss_id_specifiers(spec_section(with_category))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "boss_definition_boss_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
