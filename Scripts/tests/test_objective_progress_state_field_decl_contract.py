# THIS IS leftover-safe FSkyguardObjectiveProgress::State.
# origin/main form: split-line
# `UPROPERTY(VisibleAnywhere, BlueprintReadOnly)`
# then `ESkyguardMissionObjectiveState State =
# ESkyguardMissionObjectiveState::Inactive;` (initializer IS present,
...
# `= 0` / `= 0.f`, do not copy those; this field is the Inactive
# enum default.
# THIS IS NOT leftover objective-progress-defaults #182 / #ae90.
# THIS IS NOT leftover AddObjectiveProgress #316.
# THIS IS NOT leftover HowToFlyRow defaults #191 / #9ab2.
# THIS IS NOT leftover HowToFlyRow StepId #1258 / InputHint #1259
# / Instruction (in-flight sibling).
# THIS IS NOT leftover BriefingRadioRow Subtitle #1257 / Speaker
# #1255 / LineId #1256.
# THIS IS NOT leftover BriefingCard #1251-#1254.
# THIS IS NOT leftover AuthoringResult #1233-#1250.
# THIS IS NOT leftover mission director GetObjectiveRuntime.
# THIS IS NOT leftover Mission07/08/09/10 method/field ranges
# already drafted.
# Do not contract leftover pictogram enum values from the
# retired live roster.
# Parse STRUCT public section ONLY after
# struct FSkyguardObjectiveProgress and BEFORE
...
# no Category=. Accept Inactive enum default.
# Fail-closed if type/decl missing, renamed, Category= added, or
# default is not ESkyguardMissionObjectiveState::Inactive.
...
TARGET_WRONG_BARE = "ESkyguardMissionObjectiveState State;"
TARGET_WRONG_ACTIVE = (
    "ESkyguardMissionObjectiveState State = "
    "ESkyguardMissionObjectiveState::Active;"
)
TARGET_WRONG_FALSE = "ESkyguardMissionObjectiveState State = false;"
TARGET_WRONG_ZERO = "ESkyguardMissionObjectiveState State = 0;"
TARGET_WRONG_FLOAT = "ESkyguardMissionObjectiveState State = 0.f;"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
STOP_AFTER = "struct FSkyguardMissionResult"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored Inactive enum default. Do not
    # accept a bare decl or `= false` / `= 0` / `= 0.f` when
    # origin/main has ESkyguardMissionObjectiveState::Inactive.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    for match in re.finditer(r"UPROPERTY\(", compact):
        ...
        if depth != 0:
            continue
        after = compact[index:]
        if re.match(r"\s*ESkyguardMissionObjectiveState\s+State\b", after):
            return compact[start : index - 1]
    raise AssertionError(
        "UPROPERTY for ESkyguardMissionObjectiveState State is missing from "
...
class ObjectiveProgressStateFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            class_body(
                "struct FSkyguardUnrelatedProgress "
                ": public AActor\n{\n};\n"
            )
    ...
    def test_other_struct_does_not_satisfy(self) -> None:
        other = (
            "struct FOtherObjectiveProgress\n"
            "{\n"
            "public:\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
    ...
    def test_state_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardMissionObjectiveState State"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("Inactive", LOCKED_DECL)
        self.assertIn("ESkyguardMissionObjectiveState::Inactive", LOCKED_DECL)
        self.assertNotIn("Active", LOCKED_DECL.replace("Inactive", ""))
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ACTIVE}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_ACTIVE}\n", LOCKED_DECL)
        self.assertIn("State", str(raised.exception))

    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_ACTIVE}\n",
            ...
            "\tESkyguardMissionObjectiveState ObjectiveState = "
            "ESkyguardMissionObjectiveState::Inactive;\n",
            "\tESkyguardMissionObjectiveState ProgressState = "
            "ESkyguardMissionObjectiveState::Inactive;\n",
        )
    ...
    def test_bare_decl_without_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("ESkyguardMissionObjectiveState::Inactive", origin)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "public:\n\tESkyguardMissionObjectiveState\n"
            "\tState = ESkyguardMissionObjectiveState::Inactive;\n",
            "public:\n\tESkyguardMissionObjectiveState   State = "
            "ESkyguardMissionObjectiveState::Inactive;\n",
            "public:\n\tESkyguardMissionObjectiveState\tState = "
            "ESkyguardMissionObjectiveState::Inactive;\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "objective-progress State field contract contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_objective_progress_state_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertIn("State = ESkyguardMissionObjectiveState::Inactive;", locked_only)
        self.assertNotIn("ObjectiveId", locked_only)
        self.assertNotIn("CurrentProgress", locked_only)
