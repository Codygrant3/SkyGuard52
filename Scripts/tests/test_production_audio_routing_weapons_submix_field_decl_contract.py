# THIS IS leftover-safe FSkyguardProductionAudioRouting WeaponsSubmix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundSubmixBase> WeaponsSubmix;`
# (no initializer).
...
# ExteriorSubmix / ExplosionsSubmix / RadioSubmix /
# CockpitSoundMix / CockpitExteriorAttenuation /
# CockpitLowPassHz.
# Use identifier bounds so sibling MasterSubmix / CockpitSubmix /
# RadioSubmix do not false-positive on the Submix substring.
# Analog leftover production-audio-routing-defaults then
# isolated WeaponsSubmix field decl.
# THIS IS NOT leftover MasterSubmix field decl analog.
# THIS IS NOT leftover CockpitSubmix field decl analog.
# THIS IS NOT leftover ProductionAudioEntry OutputSubmix.
# THIS IS NOT leftover production-audio-audit-defaults analog.
...
TARGET = "TSoftObjectPtr<USoundSubmixBase> WeaponsSubmix;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundSubmixBase> WeaponsSubmix = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundSubmixBase> WeaponsSubmix = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundSubmixBase> WeaponsSubmix = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundSubmixBase> WeaponsSubmix = 0.f;"
TARGET_WRONG_FLOAT = "float WeaponsSubmix;"
TARGET_WRONG_FNAME = "FName WeaponsSubmix;"
TARGET_WRONG_BOOL = "bool WeaponsSubmix;"
TARGET_WRONG_INT = "int32 WeaponsSubmix;"
TARGET_WRONG_FTEXT = "FText WeaponsSubmix;"
TARGET_WRONG_PTR = "USoundSubmixBase* WeaponsSubmix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> WeaponsSubmix;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundMix> WeaponsSubmix;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> WeaponsSubmix;"
TARGET_WRONG_FSTRING = "FString WeaponsSubmix;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+WeaponsSubmix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundSubmixBase\s*>\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundMix\s*>\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+WeaponsSubmix\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+WeaponsSubmix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundSubmixBase> WeaponsSubmix is missing from "
...
class ProductionAudioRoutingWeaponsSubmixFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_weapons_submix_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("WeaponsSubmix = nullptr", compact_origin)
        self.assertNotIn("WeaponsSubmix = false", compact_origin)
        self.assertNotIn("WeaponsSubmix = true", compact_origin)
        self.assertNotIn("WeaponsSubmix = 0.f", compact_origin)
        self.assertNotIn("WeaponsSubmix = 160.f", compact_origin)

    def test_weapons_submix_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<USoundSubmixBase> WeaponsSubmixes;\n",
            "\tint32 WeaponsSubmix = 1;\n",
            "\tbool WeaponsSubmix;\n",
            "\tfloat WeaponsSubmix = " + forty + ";\n",
            "\tfloat WeaponsSubmix = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing WeaponsSubmix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weapons_submix_field_decl_contract.py"
        ))
