...
# Type / RequiredProgress / bFailureEndsMission / ScoreReward.
# THIS IS NOT leftover objective-definition-defaults #181 / #b29f.
# THIS IS NOT leftover objective-progress-defaults #182.
# THIS IS NOT leftover ObjectiveProgress fields #1261-#1263.
# THIS IS NOT leftover mission-score-rules-defaults #185.
# THIS IS NOT leftover mission-types-defaults-tests #153.
# THIS IS NOT leftover #1332-#1361.
# THIS IS NOT leftover TheaterKit #1299-#1331.
# Analog: leftover objective-definition-defaults then isolated
# bRequiredForMissionSuccess field decl.
# If a clone asserts FText / FName / VisibleAnywhere /
# Category="Skyguard|Theater", retarget: type is bool,
...
TARGET = "bool bRequiredForMissionSuccess = true;"
TARGET_WRONG_BARE = "bool bRequiredForMissionSuccess;"
TARGET_WRONG_FALSE = "bool bRequiredForMissionSuccess = false;"
TARGET_WRONG_ZERO = "bool bRequiredForMissionSuccess = 0;"
TARGET_WRONG_FLOAT = "float bRequiredForMissionSuccess = true;"
TARGET_WRONG_FNAME = "FName bRequiredForMissionSuccess;"
TARGET_WRONG_FTEXT = "FText bRequiredForMissionSuccess;"
TARGET_WRONG_INT = "int32 bRequiredForMissionSuccess = true;"
...
LOCKED_DECL = TARGET
UPROPERTY_EDIT_READONLY = "UPROPERTY(EditAnywhere, BlueprintReadOnly)"
STOP_BEFORE_FORMATION = "struct FSkyguardEnemyFormationDefinition"
...
SIBLING_FAILURE_ENDS = "bFailureEndsMission"
SIBLING_SCORE_REWARD = "ScoreReward"
FIELD_NAME = "bRequiredForMissionSuccess"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare / `= false` / `= 0` /
    # `= 0.f` / leftover float Health when origin/main
    # has bool = true. Do not accept sibling ObjectiveId /
    # DisplayName / Type / RequiredProgress fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"bool\s+bRequiredForMissionSuccess\s*=\s*false",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bRequiredForMissionSuccess\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bRequiredForMissionSuccess\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+bRequiredForMissionSuccess\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bRequiredForMissionSuccess\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+bRequiredForMissionSuccess\s*=\s*true",
        compact,
    ) is None:
        return False
    return True
...
def parse_window(header: str) -> str:
    ...
    stop_token = STOP_BEFORE_FORMATION
    ...
    if STOP_BEFORE_FORMATION in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_FORMATION}"
        )
    return window
...
def struct_body(header: str) -> str:
    window = header
    if STRUCT_RE.search(header) is not None and STOP_BEFORE_FORMATION in header:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bRequiredForMissionSuccess\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bRequiredForMissionSuccess is missing from "
...
class ObjectiveDefinitionRequiredForMissionSuccessFieldDeclContractTests(
    unittest.TestCase
):
    def test_objective_definition_struct_exists(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_FORMATION, header)
        self.assertNotIn(STOP_BEFORE_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_FORMATION, body)
    ...
    def test_missing_required_for_success_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bRequiredForMissionSuccess = true;", compact_origin)
        self.assertNotIn("bRequiredForMissionSuccess = false", compact_origin)
        self.assertNotIn("bRequiredForMissionSuccess = 0.f", compact_origin)
        self.assertNotIn("bRequiredForMissionSuccess = 160.f", compact_origin)

    def test_required_for_success_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bRequiredForMissionSuccess"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFName {SIBLING_OBJECTIVE_ID};\n",
            f"\tFText {SIBLING_DISPLAY_NAME};\n",
            "\tbool bRequiredForMissionSuccesses = true;\n",
            "\tint32 bRequiredForMissionSuccess = 1;\n",
            "\tFText bRequiredForMissionSuccess;\n",
            "\tfloat bRequiredForMissionSuccess = " + forty + ";\n",
            "\tfloat bRequiredForMissionSuccess = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            "\tbool   bRequiredForMissionSuccess = true;\n",
            "\tbool\tbRequiredForMissionSuccess = true;\n",
            "\tbool bRequiredForMissionSuccess =\n\ttrue;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READONLY}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_contract_sibling_objective_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, SIBLING_OBJECTIVE_ID), section)
        self.assertTrue(has_identifier(section, SIBLING_DISPLAY_NAME), section)
        self.assertTrue(
            has_identifier(section, SIBLING_REQUIRED_PROGRESS),
            section,
        )
        self.assertTrue(has_identifier(section, SIBLING_FAILURE_ENDS), section)
        self.assertTrue(has_identifier(section, SIBLING_SCORE_REWARD), section)
        self.assertIn("ESkyguardMissionObjectiveType Type", section)
        self.assertTrue(has_identifier(section, FIELD_NAME), section)
    ...
    def test_does_not_parse_neighbors_or_mission_definition(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_FORMATION, header)
        self.assertNotIn(STOP_BEFORE_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_FORMATION, leaked)
        self.assertNotIn(STOP_BEFORE_FORMATION, window)
        self.assertIn(STOP_BEFORE_PROGRESS, header)
        self.assertNotIn(STOP_BEFORE_PROGRESS, section)
        ...
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, window)
        self.assertIn(STOP_BEFORE_WEATHER, header)
        self.assertNotIn(STOP_BEFORE_WEATHER, section)
        self.assertNotIn(STOP_BEFORE_WEATHER, leaked)
        self.assertNotIn(STOP_BEFORE_WEATHER, window)
        self.assertIn(STOP_BEFORE_ROUTE, header)
        self.assertNotIn(STOP_BEFORE_ROUTE, section)
        self.assertNotIn(STOP_BEFORE_ROUTE, leaked)
        self.assertNotIn(STOP_BEFORE_ROUTE, window)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertEqual(HEADER_PATH, "Source/Skyguard52/SkyguardMissionTypes.h")
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "objective-definition bRequiredForMissionSuccess field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "required_for_mission_success_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_FORMATION, locked_only)
        for helper in namespace_helper_tokens():
