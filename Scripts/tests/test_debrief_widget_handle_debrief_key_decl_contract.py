...
# or lock the HandleDebriefKey body in the .cpp.
# origin/main is one line (`bool HandleDebriefKey(FKey Key);`);
...
# CPG debrief fail-closed #8ccd. This is not
...
# defaults). Stay off NativeOnKeyDown. Leftover
...
# leftover BriefingWidget isolated contracts,
# leftover RetrySaveLastDebrief, leftover
# apache-aim-chin-turret, and leftover
# NativeOnKeyDown stay sibling-only.
HANDLE_DEBRIEF_KEY = ("bool HandleDebriefKey(FKey Key);")
UFUNCTION_DEBRIEF = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Presentation|Debrief")'
)
# Leftover #56–#64 plus SortiePresentationWidgets and
# SortiePresentationComponent production files.
# This lane only adds an isolated Python HandleDebriefKey
...
# NativeOnKeyDown on this class. Stay off leftover
# campaign-subsystem RetrySaveLastDebrief, leftover
...
# HarborIndustrial, leftover RetrySaveLastDebrief,
# leftover apache-aim-chin-turret, leftover
# NativeOnKeyDown, and dirty workspace paths.
LOCKED = {
...
# RetrySaveLastDebrief, leftover CPG debrief
...
# RetrySave / TravelNext, leftover
# apache-aim-chin-turret, leftover NativeOnKeyDown,
# and sibling DebriefWidget neighbors stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# GetDebriefNarrative / GetFinalScore /
# IsProgressSaved / GetPresentationState /
# AcknowledgeDebrief / RetrySave / TravelNext stay
# sibling-only. NativeOnKeyDown stays off.
UNLOCKED_NEIGHBORS = (
...
# Leftover NativeOnKeyDown stays unlocked. Do not lock
# the protected override.
NATIVE_ON_KEY_DOWN_NOT_LOCKED = (
    "NativeOnKeyDown",
...
# Leftover apache-aim-chin-turret sibling in this wave
# stays unlocked. Do not create or edit it.
APACHE_AIM_CHIN_NOT_LOCKED = (
...
# locking HandleDebriefKey. Stay off GetDebrief. Only
# leftover default fields / sibling file are listed
# here.
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardDebriefWidget::HandleDebriefKey",
    "SkyguardSortiePresentationWidgets.cpp",
)
...
class DebriefWidgetHandleDebriefKeyDeclContractTests(unittest.TestCase):
    def test_debrief_widget_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, HANDLE_DEBRIEF_KEY), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, HANDLE_DEBRIEF_KEY)
        self.assertIn("HandleDebriefKey", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, HANDLE_DEBRIEF_KEY))

    def test_missing_handle_debrief_key_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tvoid Configure("
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, HANDLE_DEBRIEF_KEY)
        self.assertIn("HandleDebriefKey", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, HANDLE_DEBRIEF_KEY)
        self.assertIn("HandleDebriefKey", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, HANDLE_DEBRIEF_KEY), section)
        self.assertNotIn("BlueprintPure", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("UFUNCTION", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("Category", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("BlueprintCallable", HANDLE_DEBRIEF_KEY)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, HANDLE_DEBRIEF_KEY)
        self.assertIn("HandleDebriefKey", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool HandleDebriefKey;\n"
        wrong_return_void = "\tvoid HandleDebriefKey(FKey Key);\n"
        wrong_return_int = "\tint32 HandleDebriefKey(FKey Key);\n"
        added_const = "\tbool HandleDebriefKey(FKey Key) const;\n"
        missing_arg = "\tbool HandleDebriefKey();\n"
        added_arg = "\tbool HandleDebriefKey(FKey Key, bool bRepeat);\n"
        leftover_native = (
            "\tvirtual FReply NativeOnKeyDown("
            "const FGeometry& InGeometry, "
            "const FKeyEvent& InKeyEvent) override;\n"
        )
        leftover_retry = "\tbool RetrySave();\n"
        leftover_travel = "\tbool TravelNext();\n"
        leftover_ack = "\tbool AcknowledgeDebrief();\n"
        leftover_campaign = (
            "\tbool RetrySaveLastDebrief("
            "const FString& SlotName, int32 UserIndex);\n"
        )
        leftover_campaign_bare = "\tbool RetrySaveLastDebrief();\n"
        neighbor_configure = (
            "\tvoid Configure("
            "USkyguardSortiePresentationComponent* InPresentation);\n"
        )
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, HANDLE_DEBRIEF_KEY)
            self.assertIn("HandleDebriefKey", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_handle_debrief_key_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, HANDLE_DEBRIEF_KEY),
            HANDLE_DEBRIEF_KEY,
        )
        self.assertTrue(has_declaration(section, HANDLE_DEBRIEF_KEY))
        self.assertEqual(declaration_count(section, HANDLE_DEBRIEF_KEY), 1)
        self.assertTrue(HANDLE_DEBRIEF_KEY.startswith("bool "), HANDLE_DEBRIEF_KEY)
        self.assertTrue(HANDLE_DEBRIEF_KEY.endswith(";"), HANDLE_DEBRIEF_KEY)
        self.assertIn("HandleDebriefKey(FKey Key)", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("INDEX_NONE", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("{", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("}", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("return ", HANDLE_DEBRIEF_KEY)
        self.assertNotIn(" const", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("NativeOnKeyDown", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("RetrySaveLastDebrief", HANDLE_DEBRIEF_KEY)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tHandleDebriefKey(FKey Key);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tbool HandleDebriefKey(\n"
            "\t\tFKey Key);\n"
            "private:\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, HANDLE_DEBRIEF_KEY), 1)
        one_line = f"{{\npublic:\n\t{HANDLE_DEBRIEF_KEY}\n}}\n"
        self.assertTrue(has_declaration(one_line, HANDLE_DEBRIEF_KEY))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, HANDLE_DEBRIEF_KEY), section)
        self.assertEqual(
            require_declaration(section, HANDLE_DEBRIEF_KEY),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("return ", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("return false", HANDLE_DEBRIEF_KEY)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{HANDLE_DEBRIEF_KEY}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", HANDLE_DEBRIEF_KEY)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_handle_debrief_key_cpp_body(self) -> None:
        locked_only = f"{HANDLE_DEBRIEF_KEY}\n"
        self.assertNotIn("{", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("}", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("return ", HANDLE_DEBRIEF_KEY)
        ...
        self.assertNotIn("return false", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("return true", HANDLE_DEBRIEF_KEY)

    def test_contract_does_not_relock_configure(self) -> None:
        locked_only = f"{HANDLE_DEBRIEF_KEY}\n"
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("Configure", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetPresentation", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_get_debrief(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetDebrief", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_get_debrief_narrative(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NARRATIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetDebriefNarrative", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_get_final_score(self) -> None:
        ...
        for neighbor in GET_FINAL_SCORE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetFinalScore", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_is_progress_saved(self) -> None:
        ...
        for neighbor in IS_PROGRESS_SAVED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("IsProgressSaved", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_get_presentation_state(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetPresentationState", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("AcknowledgeDebrief", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_retry_save(self) -> None:
        ...
        for neighbor in RETRY_SAVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("RetrySave", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_travel_next(self) -> None:
        ...
        for neighbor in TRAVEL_NEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("TravelNext", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("TravelNext", locked_only)

    def test_contract_does_not_relock_native_on_key_down(self) -> None:
        locked_only = f"{HANDLE_DEBRIEF_KEY}\n"
        for token in NATIVE_ON_KEY_DOWN_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("NativeOnKeyDown", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("NativeOnKeyDown", locked_only)
    ...
    def test_contract_does_not_relock_retry_save_last_debrief(self) -> None:
        ...
        self.assertNotIn("USkyguardCampaignSubsystem", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_apache_aim_chin_turret(self) -> None:
        locked_only = f"{HANDLE_DEBRIEF_KEY}\n"
        for token in APACHE_AIM_CHIN_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
        self.assertNotIn(
            "test_apache_aim_chin_turret_decl_contract.py",
    ...
    def test_contract_does_not_relock_briefing_widget_configure(self) -> None:
        ...
        for token in BRIEFING_WIDGET_CONFIGURE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("USkyguardBriefingWidget", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("ConfigureFromMission", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("ConfigureFromMission", locked_only)
        self.assertNotIn("USkyguardMissionBriefingComponent", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_briefing_component_neighbors(
        ...
        for neighbor in (
            ...
            *GET_RADIO_CHATTER_NOT_LOCKED,
            ...
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("SetAssetsReady", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("AdvanceBriefing", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("AcknowledgeAndLaunch", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("CanLaunch", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetBriefingState", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetElapsedSeconds", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetMinimumWarmupSeconds", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("GetRadioChatter", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("ESkyguardMissionBriefingState", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("Unconfigured", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("Warming", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("Launched", HANDLE_DEBRIEF_KEY)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            HANDLE_DEBRIEF_KEY,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("FSkyguardBriefingCard", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("FSkyguardBriefingRadioRow", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("FSkyguardHowToFlyRow", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation(
        ...
        for token in LEFTOVER_SORTIE_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("SkyguardSortiePresentationFailClosed", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation_state(
        ...
        self.assertNotIn("SortieActive", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("DebriefReady", HANDLE_DEBRIEF_KEY)
        self.assertNotIn(
            "test_sortie_presentation_state_enum_contract.py",
            HANDLE_DEBRIEF_KEY,
        )
    ...
    def test_contract_does_not_relock_leftover_mission_debrief_defaults(
        ...
        for token in LEFTOVER_MISSION_DEBRIEF_DEFAULTS_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
            self.assertNotIn(token, section)
        self.assertNotIn("bNewBestScore", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("bProgressSaved", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("ESkyguardMissionDebriefState", HANDLE_DEBRIEF_KEY)
        self.assertNotIn(
            "test_mission_debrief_defaults_contract.py",
            HANDLE_DEBRIEF_KEY,
        )
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HANDLE_DEBRIEF_KEY)
        self.assertNotIn("FillResultCombatStats", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("ASkyguardGunner", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("FillAndFinalize", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("FillAndFail", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("ApplyHydraForClusters", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_bind_hud_or_theater(self) -> None:
        ...
        self.assertNotIn("test_campaign_theater_kit_contract.py", HANDLE_DEBRIEF_KEY)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANDLE_DEBRIEF_KEY)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", HANDLE_DEBRIEF_KEY)
        self.assertNotIn("ESkyguardMissionSkylineStyle", HANDLE_DEBRIEF_KEY)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{HANDLE_DEBRIEF_KEY}\n"
        self.assertEqual(
            require_declaration(locked_only, HANDLE_DEBRIEF_KEY),
            HANDLE_DEBRIEF_KEY,
        )
    ...
    def test_contract_parses_public_section_not_briefing_private_or_cpp(
        ...
        self.assertNotIn("USkyguardDebriefWidget::HandleDebriefKey", section)
    ...
    def test_contract_does_not_parse_briefing_widget(self) -> None:
        ...
        for token in BRIEFING_WIDGET_NOT_LOCKED:
            ...
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardBriefingWidget", section)
        self.assertNotIn("GetMissionTitle", HANDLE_DEBRIEF_KEY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "debrief widget HandleDebriefKey contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, HANDLE_DEBRIEF_KEY.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"debrief widget HandleDebriefKey contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, HANDLE_DEBRIEF_KEY)

    def test_contract_is_handle_debrief_key_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(HANDLE_DEBRIEF_KEY.startswith("bool "))
        self.assertTrue(HANDLE_DEBRIEF_KEY.endswith(";"))
        self.assertIn(UFUNCTION_DEBRIEF, section)
