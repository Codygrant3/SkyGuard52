...
# then BARE `TArray<FName> MissingSoundBindings;` (no initializer).
# THIS STRUCT IS BlueprintReadOnly. There is NO EditAnywhere.
# There is NO Category=. There is NO initializer (bare TArray decl).
...
# InvalidSourceEntries / MissingAttenuationBindings /
# MissingConcurrencyBindings / MissingOutputSubmixBindings /
# MissingRoutingAssets / bCategoryContractComplete /
# bProductionReady.
# Match the EXACT identifier MissingSoundBindings as a whole
# field name after TArray<FName>. Do NOT accept a substring
# match on Missing or Bindings. Do NOT claim sibling Missing
# arrays.
...
# mission-types-defaults-tests #153).
...
# Fail-closed if an initializer is added.
...
TARGET = "TArray<FName> MissingSoundBindings;"
TARGET_WRONG_INIT = "TArray<FName> MissingSoundBindings = {};"
TARGET_WRONG_FALSE = "TArray<FName> MissingSoundBindings = false;"
TARGET_WRONG_TRUE = "TArray<FName> MissingSoundBindings = true;"
TARGET_WRONG_ZERO = "TArray<FName> MissingSoundBindings = 0;"
TARGET_WRONG_FORTY = "TArray<FName> MissingSoundBindings = 40;"
TARGET_WRONG_EIGHTY = "TArray<FName> MissingSoundBindings = 80;"
TARGET_WRONG_FLOAT = "float MissingSoundBindings;"
TARGET_WRONG_FNAME = "FName MissingSoundBindings;"
TARGET_WRONG_BOOL = "bool MissingSoundBindings = false;"
TARGET_WRONG_FSTRING = "FString MissingSoundBindings;"
TARGET_WRONG_INT32 = "int32 MissingSoundBindings = 0;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MissingSoundBindings;"
...
TARGET_WRONG_SIBLING_ATTENUATION = "TArray<FName> MissingAttenuationBindings;"
TARGET_WRONG_SIBLING_CONCURRENCY = "TArray<FName> MissingConcurrencyBindings;"
...
FIELD_NAME = "MissingSoundBindings"
...
THIS_SCRIPT = (
    ...
    "_sound_bindings_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `TArray<FName> MissingSoundBindings;`.
    # Reject added initializer, Harbor 40/80, false, float, bool,
    # int32, FName, FString, TSoftObjectPtr, sibling Missing arrays.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"TArray\s*<\s*FName\s*>\s+MissingSoundBindings\s*;",
        compact,
    )
    if exact is None:
        return False
    if re.search(
        r"\bMissingSoundBindings\s*=",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MissingSoundBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MissingSoundBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MissingSoundBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MissingSoundBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MissingSoundBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MissingSoundBindings\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+MissingSoundBindings\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> MissingSoundBindings is missing from "
...
class ProductionAudioAuditMissingSoundBindingsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_sound_bindings_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_or_harbor_change_fails_closed(self) -> None:
        added_init = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
        ...
        section = spec_section(added_init)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("TArray<FName> MissingSoundBindings;", compact_origin)
        self.assertNotIn("MissingSoundBindings =", compact_origin)
        self.assertNotIn("MissingSoundBindings = 40", compact_origin)
        self.assertNotIn("MissingSoundBindings = 80", compact_origin)
        self.assertNotIn("MissingSoundBindings = false", compact_origin)
        self.assertNotIn("MissingSoundBindings = {}", compact_origin)

    def test_missing_sound_bindings_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> MissingSoundBindings"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SIBLING_ATTENUATION}\n",
            f"\t{TARGET_WRONG_SIBLING_CONCURRENCY}\n",
            f"\t{TARGET_WRONG_SIBLING_OUTPUT}\n",
            f"\t{TARGET_WRONG_SIBLING_ROUTING}\n",
            "\tTArray<FName> MissingSoundBinding;\n",
            "\tTArray<FName> MissingBindings;\n",
            "\tbool MissingSoundBindings;\n",
            "\tfloat MissingSoundBindings = " + forty + ";\n",
            "\tfloat MissingSoundBindings = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_audit_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_CATEGORY_ENTRIES), section
        )
        self.assertTrue(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit MissingSoundBindings field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "missing_sound_bindings_field_decl_contract.py"
        ))
