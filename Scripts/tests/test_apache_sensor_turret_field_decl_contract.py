# THIS IS leftover-safe ASkyguardApacheAircraft SensorTurret.
...
# then BARE `TObjectPtr<USceneComponent> SensorTurret;` (no initializer).
# THIS IS A SCENE COMPONENT, NOT A STATIC MESH.
...
# Stop BEFORE sibling SensorBall as a claimed slot.
# Do NOT claim SensorBall / NightVisionTurret / ChinTurret /
...
# SensorTurret is NOT leftover analog apache-set-sensor-view
# UFUNCTION, NOT leftover analog apache-get-sensor-quality
# UFUNCTION, NOT leftover analog apache-get-sensor-turret
# UFUNCTION, NOT leftover analog apache-mount-fail-closed,
# NOT leftover sibling SensorBall (UStaticMeshComponent) /
# ChinTurret (also USceneComponent but a different slot) /
# NightVisionTurret.
...
# Skyguard|Apache NOT Skyguard|Theater. LOCKED_DECL is
# `TObjectPtr<USceneComponent> SensorTurret;` NOT
# `FName WeatherIdentity;` and NOT
# `TObjectPtr<UStaticMeshComponent>` (SilhouetteMesh
# analog / ChinHousing sibling type). This slot also has
# NO initializer, like the clone, but type / identifier /
# Category must retarget.
# Fail-closed if this test still asserts
# FName WeatherIdentity / Category="Skyguard|Theater" /
# 160.f / TObjectPtr<UStaticMeshComponent> AS THE LOCKED
# DECL.
# Do NOT refill leftover AircraftRoot #1557 /
# SilhouetteMesh #1556 / Fuselage #1560 / Canopy #1559 /
# Nose #1558 / StubWingLeft #1562 / StubWingRight #1564 /
# VerticalTail #1563 / TailBoom #1561 /
# HoverBobCentimeters #1555 / RotorPower #1554 /
# MaxIntegrity #1553 / CurrentIntegrity #1552 /
# MainRotor #1566 / RotorMast #1565 / ChinTurret sibling.
...
LOCKED_DECL = "TObjectPtr<USceneComponent> SensorTurret;"
...
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> SensorTurret = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> SensorTurret = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> SensorTurret = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> SensorTurret = 0.f;"
TARGET_WRONG_FLOAT = "float SensorTurret;"
TARGET_WRONG_FNAME = "FName SensorTurret;"
TARGET_WRONG_INT = "int32 SensorTurret = 0;"
TARGET_WRONG_BOOL = "bool SensorTurret = true;"
...
STOP_BEFORE_SENSOR_BALL = "SensorBall"
...
LEFTOVER_AIM_CHIN_TURRET = (
    "Scripts/tests/test_apache_aim_chin_turret"
    "_decl_contract.py"
)
LEFTOVER_GET_CHIN_SLEW = (
    "Scripts/tests/test_apache_get_chin_slew_scale"
    "_decl_contract.py"
)
LEFTOVER_GET_CHIN_FIRE = (
    "Scripts/tests/test_apache_get_chin_fire_scale"
    "_decl_contract.py"
)
LEFTOVER_IS_CHIN_TURRET_DOWN = (
    "Scripts/tests/test_apache_is_chin_turret_down"
    "_decl_contract.py"
)
LEFTOVER_IS_CANOPY_GLASS_CRACKED = (
    "Scripts/tests/test_apache_is_canopy_glass_cracked"
    "_decl_contract.py"
)
...
LEFTOVER_GET_SENSOR_QUALITY = (
    "Scripts/tests/test_apache_get_sensor_quality"
    "_decl_contract.py"
)
LEFTOVER_GET_SENSOR_TURRET = (
    "Scripts/tests/test_apache_get_sensor_turret"
    "_decl_contract.py"
)
...
GET_CHIN_TURRET = "GetChinTurret"
GET_SENSOR_TURRET = "GetSensorTurret"
...
TARGET_WRONG_STATIC_MESH = "TObjectPtr<UStaticMeshComponent> SensorTurret;"
...
SENSOR_TURRET_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*SensorTurret\s*;"
)
SENSOR_TURRET_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*SensorTurret\s*="
)
SENSOR_TURRET_FLOAT_RE = re.compile(r"float\s+SensorTurret\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if SENSOR_TURRET_INIT_RE.search(compact):
        return False
    if SENSOR_TURRET_FLOAT_RE.search(compact):
        return False
    if SENSOR_TURRET_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+SensorTurret\b", compact):
        return False
    if re.search(r"int32\s+SensorTurret\b", compact):
        return False
    if re.search(r"bool\s+SensorTurret\b", compact):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*UStaticMeshComponent\s*>\s*SensorTurret\b",
        compact,
    ):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*TObjectPtr<USceneComponent>\s*SensorTurret\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if STOP_BEFORE_SENSOR_BALL in window:
                raise AssertionError(
                    "claimed window includes sibling SensorBall"
                )
            if "UStaticMeshComponent" in window:
                raise AssertionError(
                    "claimed window uses leftover UStaticMeshComponent "
                    "instead of USceneComponent SensorTurret"
                )
            if TARGET_WRONG_AIRCRAFT_ROOT in window and (
                LOCKED_DECL not in window
            ):
                raise AssertionError(
                    "claimed window is leftover AircraftRoot"
                )
            if TARGET_WRONG_CHIN_TURRET in window and (
                LOCKED_DECL not in window
            ):
                raise AssertionError(
                    "claimed window is leftover ChinTurret"
                )
            return window
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> SensorTurret; is "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*SensorTurret\b",
            compact[index:],
        ):
...
class ApacheSensorTurretFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SensorTurret"), section)
        ...
        self.assertNotIn(SIBLING_CHIN_HOUSING, claimed)
        self.assertNotIn(SIBLING_CHIN_BARREL, claimed)
        self.assertNotIn(SIBLING_CHIN_TURRET, claimed)
        self.assertNotIn(SIBLING_SENSOR_BALL, claimed)
        self.assertNotIn(SIBLING_NIGHT_VISION_TURRET, claimed)
        ...
        self.assertNotIn("UStaticMeshComponent", claimed)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("SensorTurret", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_sensor_turret_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        ...
        self.assertIn("SensorTurret", LOCKED_DECL)
    ...
    def test_locked_decl_requires_bare_pointer(self) -> None:
        ...
        self.assertNotIn("SensorTurret = nullptr", compact_origin)
        self.assertNotIn("SensorTurret = NAME_None", compact_origin)
        self.assertNotIn("SensorTurret = false", compact_origin)
        self.assertNotIn("SensorTurret = true", compact_origin)
        self.assertNotIn("SensorTurret = 0.f", compact_origin)
        self.assertNotIn("SensorTurret = 160.f", compact_origin)

    def test_sensor_turret_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> SensorTurret"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USceneComponent> SensorTurrets;\n",
            ...
            "\tTObjectPtr<UStaticMeshComponent> SilhouetteMesh;\n",
            "\tfloat SensorTurret = " + forty + ";\n",
            "\tfloat SensorTurret = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_CHIN_BARREL))
        ...
        self.assertIn(STOP_BEFORE_SENSOR_BALL, section)
        self.assertNotIn(STOP_BEFORE_SENSOR_BALL, claimed)
    ...
    def test_claimed_window_stops_before_chin_housing(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("SensorTurret", claimed)
        ...
        self.assertNotIn(SIBLING_SENSOR_BALL, window)
        self.assertNotIn("UStaticMeshComponent", window)
    ...
    def test_parse_window_starts_at_protected_stops_before_private(self) -> None:
        ...
        self.assertIn("SensorTurret", class_body(header))
        self.assertIn("SensorTurret", protected)
        self.assertIn("SensorTurret", section)
        self.assertIn("GetSensorTurret", public)
        self.assertIn("GetChinTurret", public)
    ...
    def test_does_not_parse_leftover_apache_ufunction_decls(self) -> None:
        ...
        self.assertIn(GET_CHIN_TURRET, public)
        self.assertIn(GET_SENSOR_TURRET, public)
        self.assertIn(GET_SENSOR_QUALITY, public)
        self.assertIn(AIM_CHIN_TURRET, public)
        self.assertIn(GET_CHIN_SLEW_SCALE, public)
        self.assertIn(GET_CHIN_FIRE_SCALE, public)
        self.assertIn(IS_CHIN_TURRET_DOWN, public)
        self.assertNotIn(SET_SENSOR_VIEW, claimed)
        self.assertNotIn(GET_ROTOR_RPM, claimed)
        self.assertNotIn(GET_CHIN_TURRET, claimed)
        self.assertNotIn(GET_SENSOR_TURRET, claimed)
        self.assertNotIn(GET_SENSOR_QUALITY, claimed)
        self.assertNotIn(AIM_CHIN_TURRET, claimed)
        self.assertNotIn(GET_CHIN_SLEW_SCALE, claimed)
        self.assertNotIn(GET_CHIN_FIRE_SCALE, claimed)
        self.assertNotIn(IS_CHIN_TURRET_DOWN, claimed)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SensorTurret field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "sensor_turret_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AIM_CHIN_TURRET, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GET_CHIN_SLEW, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GET_CHIN_FIRE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_IS_CHIN_TURRET_DOWN, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_IS_CANOPY_GLASS_CRACKED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_APACHE_CHIN_TURRET, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GET_SENSOR_QUALITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GET_SENSOR_TURRET, LOCKED_SCRIPTS)
    ...
    def test_header_path_is_apache_aircraft_only(self) -> None:
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<UStaticMeshComponent> SensorTurret;",
        )
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STATIC_MESH)
        self.assertNotIn("UStaticMeshComponent", LOCKED_DECL)
        self.assertNotEqual(
