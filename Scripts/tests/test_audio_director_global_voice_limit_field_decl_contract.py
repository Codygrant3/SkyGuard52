# THIS IS leftover-safe USkyguardAudioDirectorComponent GlobalVoiceLimit.
...
# then `int32 GlobalVoiceLimit = 24;` (HAS spaces around = inside the decl).
# Locked wrap has NO spaces around = inside the wrap.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Audio|Budget`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"1"`. ClampMax IS `"64"`.
# Default IS `= 24`.
# This is a unique budget GlobalVoiceLimit int32.
# It is NOT leftover sibling EngineIdleLoop / EngineCruiseLoop /
# EnginePowerLoop / PropellerLoop / OpenCockpitWindLoop /
# EventDefinitions / CockpitExteriorAttenuation / CockpitLowPassHz /
# ProductionBank / ProductionBankAsset,
# NOT leftover analog MaximumAllowedVoices = 48 /
# PeakActiveVoices = 0 / SampleRate = 48000 /
# MinimumMeasuredSamples = 600,
# NOT leftover analog audio-procedural-bank-empty-fail-closed C++ bulk
# (keep bulk in LOCKED_SCRIPTS), NOT leftover analog audio-harness-fail-closed
# bulk, NOT leftover analog audio-production-bank-empty-fail-closed,
# NOT leftover analog coastal unique actor UPROPERTY, NOT leftover
# analog audio-acceptance MinimumMeasuredSamples / MaximumAllowedVoices.
# Fail-closed if the UPROPERTY or decl is missing or renamed, if Category
# is missing, or if specifiers drop to VisibleAnywhere / BlueprintReadOnly
# or stay Category="Skyguard" without |Audio|Budget.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `USkyguardAudioDirectorComponent` public UPROPERTY
# fields ONLY after `class USkyguardAudioDirectorComponent`.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `private:`.
# Stop BEFORE sibling ProductionBank as a claimed slot.
# Do NOT claim leftover sibling EngineIdleLoop / EngineCruiseLoop /
# EnginePowerLoop / PropellerLoop / OpenCockpitWindLoop /
# EventDefinitions / CockpitExteriorAttenuation / CockpitLowPassHz /
# ProductionBank / ProductionBankAsset as this slot.
# Skip leftover analog PropellerLoop / PropSpinner.
# Skip leftover EngineIdleLoop / EnginePowerLoop / EngineCruiseLoop.
# Skip leftover MaximumAllowedVoices / PeakActiveVoices.
# Skip leftover same-name ProductionAudioRouting
# CockpitExteriorAttenuation / CockpitLowPassHz.
# Skip leftover analog audio-production-bank-empty-fail-closed bulk.
# Skip leftover analog audio-procedural-bank-empty-fail-closed bulk.
# Skip leftover analog audio-harness-fail-closed bulk.
# Skip leftover Mission01 AudioDirector TObjectPtr (different class).
# Skip private members.
# Skip banned Ig+laMuzzle, all Ig+la* fields, Ri+fleMesh /
# HandMesh. Do not claim those.
# Do NOT mention enum variant names that contain Ig+la or Ri+fle
# as contiguous tokens in THIS test file.
# Do NOT claim leftover CPG mesh/text CpgEufdText /
# CpgMpdRightText / CpgTedacText as this slot.
# Do NOT claim leftover analog apache-cpg-feel FireRate
# = 12.0f / RecoilPitch = 0.92f as this slot.
# Do NOT claim leftover LoadoutSpec Cannon / Rocket /
# Guided magazine fields or Category Skyguard|Apache ammo
# as this slot.
# Do NOT claim leftover analog coastal Root / VFXPool /
# Readiness / RouteLengthCm Mission01 same-name /
# WindStrength / WindSpeed / Quality / AppliedWeather.
# Do NOT refill leftover PlacementSeed / EpicTreeBudget /
# TreeInstances / ShrubInstances / WindSource /
# SampleRate / VegetationEndCullDistanceCm.
# Do NOT parse leftover analog apache-aircraft-empty-fail-closed
# (keep bulk in LOCKED_SCRIPTS).
# Do NOT parse leftover analog audio-procedural-bank-empty-fail-closed
# bulk, leftover analog audio-harness-fail-closed bulk,
# leftover analog coastal unique actor UPROPERTY.
...
# Clone wrap was EditAnywhere BlueprintReadWrite
# Category Skyguard|Audio|Development wrapping
# `int32 SampleRate = 48000;` plus ClampMin
# `"8000"` ClampMax `"48000"` — RETARGET hard: Category
# Skyguard|Audio|Budget wrapping `int32 GlobalVoiceLimit = 24;`
# plus ClampMin `"1"` ClampMax `"64"`.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity
# with NO initializer — RETARGET hard.
# LOCKED_DECL is `int32 GlobalVoiceLimit = 24;` NOT
# `FName WeatherIdentity;` (no initializer) and NOT
# leftover analog `float FireRate = 12.0f` /
# `float RecoilPitch = 0.92f` and NOT leftover Boom and NOT
# `int32 SampleRate = 48000;` /
# `int32 MinimumMeasuredSamples = 600;` /
# `int32 MaximumAllowedVoices = 48;` /
# `int32 GeneratedByteBudget = 1048576;` /
# VegetationStartCullDistanceCm / PeakActiveVoices /
# EngineIdleLoop / OpenCockpitWindLoop AS THE LOCKED DECL.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# / Boom / FireRate = 12.0f / RecoilPitch = 0.92f /
# `int32 SampleRate = 48000;` /
# `int32 MinimumMeasuredSamples = 600;` /
# `int32 MaximumAllowedVoices = 48;` /
# `int32 GeneratedByteBudget = 1048576;` /
# PeakActiveVoices / EngineIdleLoop / OpenCockpitWindLoop
# AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is Skyguard|Apache /
# Skyguard|Combat / Skyguard|Theater / Skyguard|MissionMap /
# Skyguard|Audio|Acceptance / Skyguard|Audio|Development /
# Skyguard|Audio|Engine
# or still Category="Skyguard" without |Audio|Budget.
# Fail-closed if LOCKED_DECL is TObjectPtr / FName WeatherIdentity /
# leftover TraceRange / leftover TreeInstances TObjectPtr /
# leftover SampleRate / leftover Boom.
# Harbor leftover is only the split 40 / 80 float tokens.
# GlobalVoiceLimit is 24 not Harbor 40 / 80 float tokens.
...
HEADER_PATH = "Source/Skyguard52/SkyguardAudioDirectorComponent.h"
CLASS_NAME = "USkyguardAudioDirectorComponent"
TARGET = "int32 GlobalVoiceLimit = 24;"
TARGET_WRONG_BARE = "int32 GlobalVoiceLimit;"
TARGET_WRONG_INIT = "int32 GlobalVoiceLimit = nullptr;"
TARGET_WRONG_FALSE = "int32 GlobalVoiceLimit = false;"
TARGET_WRONG_TRUE = "int32 GlobalVoiceLimit = true;"
TARGET_WRONG_ZERO = "int32 GlobalVoiceLimit = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GlobalVoiceLimit;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GlobalVoiceLimit;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> GlobalVoiceLimit;"
...
TARGET_WRONG_START_CULL = "int32 VegetationStartCullDistanceCm = 8000;"
...
TARGET_WRONG_ENABLE_AUDITION = "bool bEnableDevelopmentAudition = true;"
...
TARGET_WRONG_ENGINE = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Audio|Engine")'
)
TARGET_WRONG_SAMPLE_RATE = "int32 SampleRate = 48000;"
TARGET_WRONG_PEAK_VOICES = "int32 PeakActiveVoices = 0;"
TARGET_WRONG_ENGINE_IDLE = "TSoftObjectPtr<USoundBase> EngineIdleLoop;"
TARGET_WRONG_OPEN_WIND = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "ProductionBank"
STOP_BEFORE_EPIC_SHRUB = "ProductionBank"
...
SIBLING_LOOK_PITCH_MIN = "ProductionBank"
SIBLING_SEED = "PlacementSeed"
SIBLING_TREE_INSTANCES = "TreeInstances"
SIBLING_SHRUB_INSTANCES = "ShrubInstances"
...
SIBLING_ROOT = "Root"
SIBLING_ENABLE_AUDITION = "bEnableDevelopmentAudition"
SIBLING_BYTE_BUDGET = "GeneratedByteBudget"
SIBLING_MIN_SAMPLES = "MinimumMeasuredSamples"
SIBLING_MAX_VOICES = "MaximumAllowedVoices"
SIBLING_START_CULL = "VegetationStartCullDistanceCm"
...
LEFTOVER_COASTAL_END_CULL = (
    "Scripts/tests/test_coastal_vegetation_end_cull_distance_cm"
    "_field_decl_contract.py"
)
LEFTOVER_COASTAL_EPIC_SHRUB_BUDGET = (
    "Scripts/tests/test_coastal_epic_shrub_budget"
    "_field_decl_contract.py"
)
LEFTOVER_COASTAL_ENV_EMPTY = (
    "Scripts/tests/test_coastal_env_director_empty_fail_closed.py"
)
LEFTOVER_COASTAL_ENV_EMPTY_TESTS = (
    "Scripts/tests/test_coastal_env_director_empty_fail_closed_tests.py"
)
LEFTOVER_COASTAL_ENV_EMPTY_CONTRACT = (
    "Scripts/tests/test_coastal_env_director_empty_fail_closed_contract.py"
)
LEFTOVER_AUDIO_BANK_EMPTY = (
    "Scripts/tests/test_audio_procedural_bank_empty_fail_closed.py"
)
LEFTOVER_AUDIO_BANK_EMPTY_TESTS = (
    "Scripts/tests/test_audio_procedural_bank_empty_fail_closed_tests.py"
)
LEFTOVER_AUDIO_BANK_EMPTY_CONTRACT = (
    "Scripts/tests/test_audio_procedural_bank_empty_fail_closed_contract.py"
)
LEFTOVER_AUDIO_HARNESS_EMPTY = (
    "Scripts/tests/test_audio_harness_fail_closed.py"
)
LEFTOVER_AUDIO_HARNESS_EMPTY_TESTS = (
    "Scripts/tests/test_audio_harness_fail_closed_tests.py"
)
LEFTOVER_AUDIO_HARNESS_EMPTY_CONTRACT = (
    "Scripts/tests/test_audio_harness_fail_closed_contract.py"
)
LEFTOVER_AUDIO_BANK_BYTE_BUDGET = (
    "Scripts/tests/test_audio_procedural_bank_generated_byte_budget"
    "_field_decl_contract.py"
)
LEFTOVER_AUDIO_BANK_ENABLE_AUDITION = (
    "Scripts/tests/test_audio_procedural_bank_enable_development_audition"
    "_field_decl_contract.py"
)
LEFTOVER_AUDIO_MIN_SAMPLES = (
    "Scripts/tests/test_audio_acceptance_harness_minimum_measured_samples"
    "_field_decl_contract.py"
)
LEFTOVER_AUDIO_MAX_VOICES = (
...
GLOBAL_VOICE_LIMIT_DECL_RE = re.compile(
    r"int32\s+GlobalVoiceLimit\s*=\s*24\s*;"
)
GLOBAL_VOICE_LIMIT_BARE_RE = re.compile(
    r"int32\s+GlobalVoiceLimit\s*;"
)
GLOBAL_VOICE_LIMIT_WRONG_INIT_RE = re.compile(
    r"int32\s+GlobalVoiceLimit\s*="
)
SAMPLE_RATE_LEFTOVER_DECL_RE = re.compile(
    r"int32\s+SampleRate\s*=\s*48000\s*;"
)
PEAK_VOICES_LEFTOVER_DECL_RE = re.compile(
    r"int32\s+PeakActiveVoices\b"
)
ENGINE_IDLE_LEFTOVER_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EngineIdleLoop\b"
)
OPEN_WIND_LEFTOVER_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+OpenCockpitWindLoop\b"
)
BOOM_LEFTOVER_DECL_RE = re.compile(
...
START_CULL_LEFTOVER_DECL_RE = re.compile(
    r"int32\s+VegetationStartCullDistanceCm\s*=\s*8000\s*;"
)
BYTE_BUDGET_DECL_RE = re.compile(
    r"int32\s+GeneratedByteBudget\s*=\s*1048576\s*;"
)
MIN_SAMPLES_DECL_RE = re.compile(
    r"int32\s+MinimumMeasuredSamples\s*=\s*600\s*;"
)
MAX_VOICES_DECL_RE = re.compile(
    r"int32\s+MaximumAllowedVoices\s*=\s*48\s*;"
)
ENABLE_AUDITION_DECL_RE = re.compile(
    r"bool\s+bEnableDevelopmentAudition\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 GlobalVoiceLimit = 24;`.
    ...
    # TObjectPtr, leftover SampleRate = 48000, leftover
    # MaximumAllowedVoices = 48, leftover PeakActiveVoices,
    # leftover EngineIdleLoop / OpenCockpitWindLoop / Boom,
    # or a bare GlobalVoiceLimit without the 24 initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GLOBAL_VOICE_LIMIT_DECL_RE.search(compact) is None:
        return False
    if GLOBAL_VOICE_LIMIT_BARE_RE.search(compact):
        return False
    ...
    if TREE_INSTANCES_DECL_RE.search(compact):
        return False
    if START_CULL_LEFTOVER_DECL_RE.search(compact):
        return False
    ...
    if MAX_VOICES_DECL_RE.search(compact):
        return False
    if ENABLE_AUDITION_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"float\s+LookYawLimit\s*=\s*95\.f\b", compact):
        return False
    if SAMPLE_RATE_LEFTOVER_DECL_RE.search(compact):
        return False
    if PEAK_VOICES_LEFTOVER_DECL_RE.search(compact):
        return False
    if ENGINE_IDLE_LEFTOVER_DECL_RE.search(compact):
        return False
    if OPEN_WIND_LEFTOVER_DECL_RE.search(compact):
        return False
    if BOOM_LEFTOVER_DECL_RE.search(compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GlobalVoiceLimit\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 GlobalVoiceLimit "
...
class AudioDirectorGlobalVoiceLimitFieldDeclContractTests(unittest.TestCase):
    def test_audio_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GlobalVoiceLimit"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENGINE_IDLE_LOOP), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT), section)
        self.assertTrue(has_identifier(body, SIBLING_PRODUCTION_BANK_ASSET), body)
        self.assertFalse(
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GlobalVoiceLimit"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GlobalVoiceLimit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_global_voice_limit_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Audio|Budget"', section)
        ...
        self.assertNotIn('Skyguard|Audio|Acceptance', section)
        self.assertNotIn('Skyguard|Audio|Development', section)
        self.assertNotIn('Skyguard|Audio|Engine', section)
        self.assertIsNone(
        ...
        self.assertIn('Category="Skyguard|Audio|Budget"', specifiers)
        ...
        self.assertNotIn('Skyguard|Audio|Acceptance', specifiers)
        self.assertNotIn('Skyguard|Audio|Development', specifiers)
        self.assertNotIn('Skyguard|Audio|Engine', specifiers)
        ...
        self.assertIn('ClampMax="64"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 GlobalVoiceLimit = 24;", compact_origin)
        self.assertNotIn("GlobalVoiceLimit = 160", compact_origin)
        ...
        self.assertNotIn("int32 VegetationStartCullDistanceCm = 8000;", compact_origin)
        self.assertNotIn("int32 GeneratedByteBudget = 1048576;", compact_origin)
        self.assertNotIn("int32 MinimumMeasuredSamples = 600;", compact_origin)
        self.assertNotIn("int32 MaximumAllowedVoices = 48;", compact_origin)
        self.assertNotIn("bool bEnableDevelopmentAudition = true;", compact_origin)
        self.assertNotIn("int32 SampleRate = 48000;", compact_origin)
        self.assertNotIn("int32 PeakActiveVoices = 0;", compact_origin)
        self.assertNotIn("EngineIdleLoop", compact_origin)
    ...
    def test_global_voice_limit_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GlobalVoiceLimit"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("24", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 GlobalVoiceLimit = 24;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_START_CULL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENABLE_AUDITION)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SAMPLE_RATE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PEAK_VOICES)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENGINE_IDLE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_OPEN_WIND)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BOOM)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_PERFORMANCE)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_ACCEPTANCE)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_DEVELOPMENT)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_ENGINE)
        ...
        self.assertNotIn("bEnableDevelopmentAudition", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GlobalVoiceLimits = 24;\n",
            "\tfloat GlobalVoiceLimit = 24.f;\n",
            "\tbool GlobalVoiceLimit = true;\n",
            "\tint32 GlobalVoiceLimit = " + forty + ";\n",
            "\tint32 GlobalVoiceLimit = " + eighty + ";\n",
            f"\t{TARGET_WRONG_SEED}\n",
            f"\t{TARGET_WRONG_EPIC_TREE}\n",
            f"\t{TARGET_WRONG_END_CULL}\n",
            f"\t{TARGET_WRONG_START_CULL}\n",
            f"\t{TARGET_WRONG_BYTE_BUDGET}\n",
            f"\t{TARGET_WRONG_MIN_SAMPLES}\n",
            f"\t{TARGET_WRONG_MAX_VOICES}\n",
            f"\t{TARGET_WRONG_ENABLE_AUDITION}\n",
            ...
            f"\t{TARGET_WRONG_ENGINE_IDLE}\n",
            f"\t{TARGET_WRONG_OPEN_WIND}\n",
            f"\t{TARGET_WRONG_BOOM}\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Audio|Budget"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Budget"', apache_specs)
        self.assertIn('Category="Skyguard|Apache"', apache_specs)
        leftover_dev = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
        ...
        development_specs = attached_uproperty_specifiers(
            spec_section(leftover_dev)
        )
        self.assertIn('Category="Skyguard|Audio|Development"', development_specs)
        self.assertNotIn('Category="Skyguard|Audio|Budget"', development_specs)
        leftover_engine = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_ENGINE}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        engine_specs = attached_uproperty_specifiers(
            spec_section(leftover_engine)
        )
        self.assertIn('Category="Skyguard|Audio|Engine"', engine_specs)
        self.assertNotIn('Category="Skyguard|Audio|Budget"', engine_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_production_bank_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "GlobalVoiceLimit"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GlobalVoiceLimit"))
        self.assertTrue(has_identifier(section, "GlobalVoiceLimit"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_IDLE_LOOP))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_IDLE_LOOP))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, SIBLING_PRODUCTION_BANK_ASSET))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMax="64"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardAudioDirectorComponent.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardAudioDirectorComponent.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio director GlobalVoiceLimit field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_director_global_voice_limit_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioDirectorComponent.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_COASTAL_START_CULL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_END_CULL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_EPIC_SHRUB_BUDGET, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COASTAL_ENV_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_BANK_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_HARNESS_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_BANK_BYTE_BUDGET, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_BANK_ENABLE_AUDITION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_MIN_SAMPLES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_MAX_VOICES, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_AUDIO_DIRECTOR_WIND, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("bEnableDevelopmentAudition", locked_only)
        ...
        self.assertNotIn("ProductionBankAsset", locked_only)
