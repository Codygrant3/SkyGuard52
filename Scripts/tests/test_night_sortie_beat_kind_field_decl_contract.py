# THIS IS leftover-safe FSkyguardNightSortieBeat Kind.
...
# FSkyguardNightSortieBeat IS a plain C++ struct.
...
# plus initializer ESkyguardNightSortieBeatKind::DarkIngress.
# Fail-closed if Kind is missing or renamed, if the type is
# not ESkyguardNightSortieBeatKind, if the initializer is
# missing or is not ESkyguardNightSortieBeatKind::DarkIngress,
...
# Do NOT parse leftover `FSkyguardDaySortieBeat`.
...
# retarget: type is ESkyguardNightSortieBeatKind,
# identifier is Kind, initializer IS
# ESkyguardNightSortieBeatKind::DarkIngress, no UPROPERTY.
...
LEFTOVER_KIT_STRUCT = "FSkyguardNightSortieBeatKit"
LEFTOVER_ENUM = "ESkyguardNightSortieBeatKind"
LEFTOVER_NAMESPACE = "SkyguardNightSortieBeatKit"
LEFTOVER_DAY_STRUCT = "FSkyguardDaySortieBeat"
LEFTOVER_STORM_KIT = "FSkyguardStormRainBeatKit"
TARGET = (
    "ESkyguardNightSortieBeatKind Kind = "
    "ESkyguardNightSortieBeatKind::DarkIngress;"
)
TARGET_WRONG_BARE = "ESkyguardNightSortieBeatKind Kind;"
TARGET_WRONG_THERMAL = (
    "ESkyguardNightSortieBeatKind Kind = "
    "ESkyguardNightSortieBeatKind::ThermalHunt;"
)
TARGET_WRONG_FALSE = "ESkyguardNightSortieBeatKind Kind = false;"
TARGET_WRONG_TRUE = "ESkyguardNightSortieBeatKind Kind = true;"
TARGET_WRONG_ZERO = "ESkyguardNightSortieBeatKind Kind = 0.f;"
TARGET_WRONG_FLOAT = "float Kind = 160.f;"
TARGET_WRONG_FNAME = "FName Kind;"
TARGET_WRONG_THREAT_TYPE = (
    "ESkyguardThreatKind Kind = ESkyguardThreatKind::FastAttacker;"
)
...
STOP_BEFORE_DAY = "struct FSkyguardDaySortieBeat"
...
def kind_has_uproperty_wrap(region: str) -> bool:
    ...
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardNightSortieBeatKind\s+Kind\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardNightSortieBeatKind Kind =
    # ESkyguardNightSortieBeatKind::DarkIngress;`.
    # Do not accept missing initializer, ThermalHunt,
    # leftover float Health, sibling Call / Threat, or
    # UPROPERTY / Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"ESkyguardNightSortieBeatKind\s+Kind\s*=\s*"
        r"ESkyguardNightSortieBeatKind::DarkIngress\s*;",
        compact,
    ) is None:
        return False
    ...
    if re.search(r"\bCategory\s*=", compact):
        return False
    if re.search(
        r"ESkyguardNightSortieBeatKind\s+Kind\s*;",
        compact,
    ) and not re.search(
        r"ESkyguardNightSortieBeatKind\s+Kind\s*=",
        compact,
...
class NightSortieBeatKindFieldDeclContractTests(unittest.TestCase):
    def test_night_sortie_beat_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_DAY_STRUCT)
    ...
    def test_clone_uproperty_is_not_the_locked_decl(self) -> None:
        ...
        self.assertIn("ESkyguardNightSortieBeatKind", LOCKED_DECL)
        self.assertIn("DarkIngress", LOCKED_DECL)
    ...
    def test_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("Kind = ESkyguardNightSortieBeatKind::ThermalHunt", compact_origin)
    ...
    def test_kind_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardNightSortieBeatKind Kind"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("= ThermalHunt", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_THERMAL}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_THERMAL}\n", LOCKED_DECL)
        self.assertIn("Kind", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tESkyguardNightSortieBeatKind Kinds = "
            "ESkyguardNightSortieBeatKind::DarkIngress;\n",
            "\tint32 Kind = 0;\n",
            "\tbool Kind = false;\n",
            "\tfloat Kind = " + forty + ";\n",
    ...
    def test_parse_window_excludes_leftover_enum_values(self) -> None:
        ...
        section = spec_section(header)
        leaked = struct_body(header)
        self.assertIn("DarkIngress", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "night-sortie-beat Kind field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "night_sortie_beat_kind_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardNightSortieBeatKit.cpp", THIS_SCRIPT)
