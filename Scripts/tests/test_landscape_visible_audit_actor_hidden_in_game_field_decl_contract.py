...
# bActorHiddenInGame FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bActorHiddenInGame = true; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# bActorHiddenInGame FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bActorHiddenInGame = true; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# BlueprintReadOnly bool bActorHiddenInGame = true;
...
# bActorHiddenInGame FIELD (declaration-only). It is
...
TARGET = "bool bActorHiddenInGame = true;"
TARGET_WRONG = "bool bActorHiddenInGame = false;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bActorHiddenInGame\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bActorHiddenInGame is missing from "
...
class LandscapeVisibleAuditActorHiddenInGameFieldDeclContractTests(unittest.TestCase):
    def test_landscape_visible_audit_struct_exists(self) -> None:
        ...
        self.assertIn("bActorHiddenInGame", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bActorHiddenInGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_actor_hidden_in_game_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_actor_hidden_in_game_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bActorHiddenInGame"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("bool bActorHiddenInGame = false;", section)
        ...
        self.assertIn("bActorHiddenInGame", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bActorHiddenInGame", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bActorHiddenInGame"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bActorHiddenInGame", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_actor_hidden_in_game(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
