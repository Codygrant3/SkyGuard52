...
# Skyguard|Radio wrapping
# USkyguardRadioChatterComponent::ClearQueue.
# origin/main form: one-line BlueprintCallable wrap
# UFUNCTION(BlueprintCallable, Category="Skyguard|Radio")
# then BARE `void ClearQueue();`
# (declaration-only, no inline body required).
# KEEP BlueprintCallable. No args. const method IS absent.
# Return type IS void. Declaration-only.
# Do NOT retarget to PrimeLines / EnqueueLine /
# AdvanceRadioState / GetQueuedLineCount /
# GetCurrentLineId / GetDroppedLineCount /
# GetPlayedLineCount.
# Word-boundary match ClearQueue vs PrimeLines vs
# EnqueueLine vs AdvanceRadioState vs GetQueuedLineCount.
# Identifier ClearQueue is NOT leftover analog PrimeLines /
# EnqueueLine / AdvanceRadioState and NOT leftover analog
# RadioLine fields.
...
# BlueprintCallable IS present. BlueprintPure IS absent.
# const method IS absent. Return type IS void. No args.
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# stay BlueprintPure, if const method is present, if
# args are present, if the return type is not void,
# or if Category is
...
# GetQueuedLineCount / GetCurrentLineId /
# GetDroppedLineCount / GetPlayedLineCount are
# uncontracted. Do NOT claim those siblings.
# Do NOT claim OnLineStarted delegate.
# Clone leftover unique TogglePause #1711 RETARGET
# Category Pause -> Radio. KEEP BlueprintCallable void
# no-args. FAIL-CLOSED if LOCKED_DECL is void TogglePause();.
# Clone leftover unique EnqueueLine #1712 RETARGET
# method/return/args. KEEP Category Skyguard|Radio KEEP
# BlueprintCallable. This slot is void no-args not bool
# with Line arg. FAIL-CLOSED if LOCKED_DECL is
# bool EnqueueLine(const FSkyguardRadioLine& Line);.
# Clone leftover unique PrimeLines RETARGET; this slot
# has NO args. FAIL-CLOSED if LOCKED_DECL is
# void PrimeLines(const TArray<FSkyguardRadioLine>& Lines);.
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# #1680 Category Skyguard|Audio|Production — RETARGET
# Category Production -> Skyguard|Radio.
# KEEP BlueprintCallable (NOT BlueprintPure).
# RETARGET void EnsureDefaultEntries(); -> void ClearQueue();.
...
# RETARGET; KEEP BlueprintCallable not BlueprintPure.
...
# LOCKED_DECL is void ClearQueue();
...
# and NOT bool EnqueueLine(const FSkyguardRadioLine& Line);
# and NOT void AdvanceRadioState(float DeltaSeconds);
...
TARGET = "void ClearQueue();"
TARGET_SPLIT = (
    "void\n"
    "\tClearQueue();"
)
TARGET_INLINE = "void ClearQueue() { }"
TARGET_WRONG_BARE = "void ClearQueue() const;"
TARGET_WRONG_NO_CONST = "bool ClearQueue();"
...
TARGET_WRONG_FLOAT = "float ClearQueue();"
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void ClearQueues();"
...
TARGET_WRONG_TRUE = "bool ClearQueue = true;"
TARGET_WRONG_ZERO = "bool ClearQueue = 0.f;"
...
SIBLING_CONFIGURE_ROUTING = "EnqueueLine"
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        "_decl_contract.py",
        f"{prefix}test_gunner_get_sortie_hits"
        "_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bvoid\s+ClearQueue\s*\(\s*\)\s*(?:;|\{)"
)
CLEAR_QUEUE_IDENT_RE = re.compile(r"\bClearQueue\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # const qualifier, extra args, a bool/float/int32 rename,
    # leftover unique EnsureDefaultEntries / TogglePause /
    # GetSortieHits, leftover sibling PrimeLines /
    # EnqueueLine / AdvanceRadioState, leftover analog
    # GetSortieHits / bSuccess = false, leftover FName
    # WeatherIdentity, leftover TObjectPtr ProductionBank,
    # leftover FireRate / RecoilPitch, leftover Boom,
    # leftover VegetationStartCullDistanceCm, leftover analog
    # PoolCapacity = 12, or Harbor 40 / 80.
    del declaration
    compact = collapsed(region)
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+ClearQueue\s*\(\s*\)\s*const\b", compact):
        return False
    if re.search(r"\bbool\s+ClearQueue\b", compact):
        return False
    if re.search(r"\bfloat\s+ClearQueue\b", compact):
        return False
    if re.search(r"\bint32\s+ClearQueue\b", compact):
        return False
    if re.search(
        r"\bvoid\s+ClearQueue\s*\(\s*[^)]+\s*\)",
        compact,
    ...
    if re.search(r"\bbool\s+EnqueueLine\s*\(", compact):
        return False
    if re.search(r"\bvoid\s+AdvanceRadioState\s*\(", compact):
        return False
...
class RadioChatterClearQueueDeclContractTests(
    unittest.TestCase
):
    def test_radio_chatter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "ClearQueue"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, "PrimeLines"), body)
        self.assertFalse(has_identifier(section, "PrimeLines"), section)
        self.assertTrue(has_identifier(body, "EnqueueLine"), body)
        self.assertFalse(has_identifier(section, "EnqueueLine"), section)
        self.assertTrue(has_identifier(body, "GetQueuedLineCount"), body)
    ...
    def test_missing_clear_queue_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertTrue(
            CLEAR_QUEUE_IDENT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
    ...
    def test_clear_queue_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "ClearQueue")
        )
        self.assertTrue(
            has_identifier(section, "ClearQueue")
        )
        ...
        self.assertTrue(has_identifier(leaked, "PrimeLines"))
        self.assertFalse(has_identifier(section, "PrimeLines"))
        self.assertTrue(has_identifier(leaked, "EnqueueLine"))
        self.assertFalse(has_identifier(section, "EnqueueLine"))
        self.assertTrue(has_identifier(leaked, "GetQueuedLineCount"))
        self.assertFalse(has_identifier(section, "GetQueuedLineCount"))
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "Line"))
        self.assertIn("ClearQueue", LOCKED_DECL)
        self.assertNotIn("EnqueueLine", LOCKED_DECL)
        for leftover in leftover_apache_ufunction_tokens():
            self.assertNotIn(leftover, locked_only)

    def test_word_boundary_clear_queue_vs_prime_enqueue_and_advance(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier(
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "ClearQueue"), section)
        self.assertFalse(has_identifier(section, "PrimeLines"), section)
        self.assertFalse(
    ...
    def test_locked_decl_is_not_clone_ensure_toggle_hits_or_bind(
        ...
        self.assertTrue(LOCKED_DECL.startswith("void ClearQueue("))
