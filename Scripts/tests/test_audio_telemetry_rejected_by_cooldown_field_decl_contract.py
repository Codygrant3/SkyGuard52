# THIS IS leftover-safe FSkyguardAudioTelemetry::RejectedByCooldown.
# origin/main form: split-line
# UPROPERTY(BlueprintReadOnly)
# then `int32 RejectedByCooldown = 0;` with no Category=.
...
# do not copy those; this field is
...
# (RequestedEvents, PlayedEvents, RejectedByConcurrency,
# RejectedMissingAsset, PriorityEvictions,
# PeakActiveVoices) — sibling workers own those.
...
TARGET = "int32 RejectedByCooldown = 0;"
TARGET_WRONG_BARE = "int32 RejectedByCooldown;"
TARGET_WRONG_FALSE = "int32 RejectedByCooldown = false;"
TARGET_WRONG_TRUE = "int32 RejectedByCooldown = true;"
TARGET_WRONG_FLOAT = "int32 RejectedByCooldown = 0.f;"
TARGET_WRONG_REQUESTED = "int32 RequestedEvents = 0;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # when origin/main has `int32 RejectedByCooldown = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+RejectedByCooldown\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RejectedByCooldown\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RejectedByCooldown\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RejectedByCooldown\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+RejectedByCooldown\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 RejectedByCooldown is missing from "
...
class AudioTelemetryRejectedByCooldownFieldDeclContractTests(unittest.TestCase):
    def test_audio_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("RejectedByCooldown", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RejectedByCooldown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_rejected_by_cooldown_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("VisibleAnywhere", attached_uproperty_specifiers(section))
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("RejectedByCooldown = 0", collapsed(origin))
        self.assertNotIn("RejectedByCooldown = false", collapsed(origin))
        self.assertNotIn("RejectedByCooldown = true", collapsed(origin))
        self.assertNotIn("RejectedByCooldown = 0.f", collapsed(origin))

    def test_rejected_by_cooldown_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RejectedByCooldown"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_REQUESTED}\n",
            leftover_primary,
            leftover_guided,
            "\tbool RejectedByCooldown = 0;\n",
            "\tint32 RejectedCooldown = 0;\n",
            "\tint32 RejectedByCooldowns = 0;\n",
            "\tFText RejectedByCooldown = 0;\n",
            "\tint32 RejectedByCooldown{};\n",
        )
    ...
    def test_visible_anywhere_added_fails_closed(self) -> None:
        ...
        self.assertNotIn("VisibleAnywhere", origin)
        self.assertEqual(origin, "BlueprintReadOnly")
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio-telemetry RejectedByCooldown field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_audio_telemetry_rejected_by_cooldown"
            "_field_decl_contract.py"
        ))
