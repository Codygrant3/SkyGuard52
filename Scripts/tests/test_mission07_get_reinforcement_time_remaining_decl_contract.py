...
# INDEX_NONE or lock GetReinforcementTimeRemaining
# construction in the .cpp. This is leftover-safe
# Mission07 Boss GetReinforcementTimeRemaining METHOD on
# ASkyguardMission07IntegrationDirector. It is
# NOT leftover AdvanceReinforcementTimer #716, NOT leftover
# ClassifyFalseTrack #710, NOT leftover Mission07
# GetWaveState, NOT leftover Mission07
...
# leftover AdvanceReinforcementTimer #716,
...
# origin/main is a split-line inline-body method
# (`float GetReinforcementTimeRemaining() const`
# with `{ return ReinforcementTimeRemaining; }`);
...
# not leftover AdvanceReinforcementTimer #716.
...
# an isolated Python GetReinforcementTimeRemaining
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission07IntegrationDirector::GetReinforcementTimeRemaining",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission07GetReinforcementTimeRemainingDeclContractTests(unittest.TestCase):
    def test_mission07_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_REINFORCEMENT_TIME_REMAINING),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{GET_REINFORCEMENT_TIME_REMAINING}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn("GetReinforcementTimeRemaining", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_missing_get_reinforcement_time_remaining_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission07IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn("GetReinforcementTimeRemaining", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn("GetReinforcementTimeRemaining", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("BlueprintPure", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("Category", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("BlueprintCallable", UFUNCTION_BOSS)
        self.assertIn("Skyguard|Mission07|Boss", UFUNCTION_BOSS)
        self.assertNotIn("Mission01", UFUNCTION_BOSS)
        self.assertNotIn("Mission03", UFUNCTION_BOSS)
        self.assertNotIn("Mission04", UFUNCTION_BOSS)
        self.assertNotIn("Mission05", UFUNCTION_BOSS)
        self.assertNotIn("Mission06", UFUNCTION_BOSS)
        self.assertNotIn("Integration", UFUNCTION_BOSS)
        self.assertNotIn("Waves", UFUNCTION_BOSS)
        self.assertNotIn("Protection", UFUNCTION_BOSS)
        self.assertNotIn("Payload", UFUNCTION_BOSS)
        self.assertNotIn("Targets", UFUNCTION_BOSS)
        self.assertNotIn("Objectives", UFUNCTION_BOSS)
        self.assertIn("BlueprintPure", UFUNCTION_BOSS)
        self.assertNotIn("Environment", UFUNCTION_BOSS)
        self.assertNotIn("Briefing", UFUNCTION_BOSS)
        self.assertNotIn("Mission02", UFUNCTION_BOSS)
        self.assertNotIn("Search", UFUNCTION_BOSS)
        self.assertNotIn("Destruction", UFUNCTION_BOSS)
        self.assertNotIn("Apache", UFUNCTION_BOSS)
        self.assertNotIn("Mission06", UFUNCTION_BOSS)
        self.assertNotIn("Mission08", UFUNCTION_BOSS)
        self.assertNotIn("Mission10", UFUNCTION_BOSS)
        self.assertNotIn("Encounter", UFUNCTION_BOSS)
        self.assertNotIn("Safety", UFUNCTION_BOSS)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_BOSS)
            self.assertNotIn(invented, GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn("GetReinforcementTimeRemaining", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetReinforcementTimeRemaining() const;\n"
        )
        as_bool = "\tbool GetReinforcementTimeRemaining() const;\n"
        missing_const = (
            "\tfloat GetReinforcementTimeRemaining();\n"
        )
        with_arg = (
            "\tint32 "
            "GetReinforcementTimeRemaining(int32 Count) const;\n"
        )
        renamed = (
        ...
        leftover_classified_count = f"\t{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
        leftover_advance_timer = f"\t{ADVANCE_REINFORCEMENT_TIMER}\n"
        ...
        for region in (
            ...
            short_name,
            leftover_classify,
            leftover_classified_count,
            leftover_advance_timer,
            leftover_m07_wave,
            leftover_m06_type,
            leftover_m05_type,
            leftover_m04_type,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_REINFORCEMENT_TIME_REMAINING)
            self.assertIn("GetReinforcementTimeRemaining", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_reinforcement_time_remaining_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_REINFORCEMENT_TIME_REMAINING),
            GET_REINFORCEMENT_TIME_REMAINING,
        )
        self.assertTrue(has_declaration(section, GET_REINFORCEMENT_TIME_REMAINING))
        self.assertEqual(
            declaration_count(section, GET_REINFORCEMENT_TIME_REMAINING),
            1,
        )
        self.assertTrue(
            GET_REINFORCEMENT_TIME_REMAINING.startswith("float "),
            GET_REINFORCEMENT_TIME_REMAINING,
        )
        self.assertIn("float", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn("GetReinforcementTimeRemaining", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn(";", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn("()", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("TArray<", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("TObjectPtr", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("TSoftObjectPtr", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("INDEX_NONE", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("UFUNCTION", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("{", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("}", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("return ", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn(" const", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("Root", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("Briefing", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AudioDirector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("RadioChatter", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("SortiePresentation", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("CampaignDefinition", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("MissionDefinition", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("Readiness", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("bAutoInitialize", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("PathfinderSpawnLocation", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetAircraft", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("BindRuntimeActors", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("HandleDroneCityImpact", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("InitializePlayableMission", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("StartNextWave", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AdvanceConvoyByDistance", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("NotifyConvoyDamage", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("IsCorePlayableReady", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetConvoyRouteState", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetDayBeatKit", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetNightBeatKit", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("StartSearchlightWindow", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AdvanceSearchlightTrack", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("NotifySubstationDamage", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetRemainingThreatsInWave", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetSearchlightRuntime", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetSubstationIntegrity", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("StartPayloadWindow", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AdvancePayloadWindow", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("TryJamActivePayload", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetPayloadWindow", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetTargetRuntime", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetSurvivingTargetCount", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetDayBeatKind", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetDayBeatIndex", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("TickDayBeatKit", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("HandleBossPhaseChanged", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ESkyguardMission06WaveState", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ESkyguardSearchSector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("FSkyguardSearchTrackRuntime", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ClassifyFalseTrack", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ConfirmRadarGhostIdentification", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("NotifyProtectedTargetDamage", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AdvanceReinforcementTimer", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetSearchSector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetWaveState", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("IsHostileContactConfirmed", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetProtectedTarget", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetClassifiedFalseTrackCount", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetNightBeatKind", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetNightBeatIndex", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("TickNightBeatKit", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ESkyguardAirfieldTarget", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("RoadHunterSpawnLocation", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ConfigureMissionDefinition", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("NotifyObjectiveProgress", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("SynchronizeRuntimeState", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("NotifyThreatDestroyed", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetReadiness", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetObjectiveRuntime", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetGunner", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetPathfinder", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetMissionId", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ValidateMissionContract", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ConfigureFromMission", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AdvanceBriefing", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("SetAssetsReady", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AcknowledgeAndLaunch", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("CanLaunch", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetElapsedSeconds", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetBriefingState", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetBriefingText", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetRadioChatter", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetPresentation", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("GetMissionTitle", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("AcknowledgeBriefing", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("LaunchSortie", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("HullCollider", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("OpticalTracker", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("WeaponServo", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("CountermeasurePod", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("MinHeightFromOriginCm", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("RadarNode", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ESkyguardMission02WaveState", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("HarborIndustrial", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("MaxIntegrity", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("CurrentIntegrity", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("FillAndFinalize", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("FillAndFail", GET_REINFORCEMENT_TIME_REMAINING)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tfloat\n"
            "\tGetReinforcementTimeRemaining() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_BOSS} {GET_REINFORCEMENT_TIME_REMAINING}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_REINFORCEMENT_TIME_REMAINING}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_REINFORCEMENT_TIME_REMAINING))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_REINFORCEMENT_TIME_REMAINING),
            section,
    ...
    def test_environment_category_does_not_satisfy_boss(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_BOSS)
        self.assertNotIn("Briefing", UFUNCTION_BOSS)
        environment = (
        ...
        self.assertNotEqual(environment, UFUNCTION_BOSS)
        self.assertNotEqual(briefing, UFUNCTION_BOSS)
        self.assertNotIn(environment, UFUNCTION_BOSS)
        self.assertNotIn(briefing, UFUNCTION_BOSS)
        leftover_m01 = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_m02, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_harbor, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_m03, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_m04, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_m05, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_m06, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_waves, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_m05_waves, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_integration, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_m07_waves, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_search, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_protection, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_timer, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_payload, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_targets, UFUNCTION_BOSS)
        self.assertNotEqual(leftover_objectives, UFUNCTION_BOSS)
        self.assertNotIn(leftover_m01, UFUNCTION_BOSS)
        self.assertNotIn(leftover_m02, UFUNCTION_BOSS)
        self.assertNotIn(leftover_harbor, UFUNCTION_BOSS)
        self.assertNotIn(leftover_m03, UFUNCTION_BOSS)
        self.assertNotIn(leftover_m04, UFUNCTION_BOSS)
        self.assertNotIn(leftover_m05, UFUNCTION_BOSS)
        self.assertNotIn(leftover_m06, UFUNCTION_BOSS)
        self.assertNotIn(leftover_m05_waves, UFUNCTION_BOSS)
        self.assertNotIn(leftover_integration, UFUNCTION_BOSS)
        self.assertNotIn("Mission01", UFUNCTION_BOSS)
        self.assertNotIn("Mission03", UFUNCTION_BOSS)
        self.assertNotIn("Mission04", UFUNCTION_BOSS)
        self.assertNotIn("Mission05", UFUNCTION_BOSS)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_REINFORCEMENT_TIME_REMAINING}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_REINFORCEMENT_TIME_REMAINING)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_reinforcement_time_remaining_cpp_body(self) -> None:
        locked_only = f"{GET_REINFORCEMENT_TIME_REMAINING}\n"
        self.assertNotIn("{", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("}", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("return ", GET_REINFORCEMENT_TIME_REMAINING)
        ...
        self.assertNotIn("return false", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("return true", GET_REINFORCEMENT_TIME_REMAINING)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_REINFORCEMENT_TIME_REMAINING}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn("GetReinforcementTimeRemaining", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertTrue(GET_REINFORCEMENT_TIME_REMAINING.startswith("float "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("CurrentIntegrity", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_REINFORCEMENT_TIME_REMAINING}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_REINFORCEMENT_TIME_REMAINING),
            GET_REINFORCEMENT_TIME_REMAINING,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", GET_REINFORCEMENT_TIME_REMAINING)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 GetReinforcementTimeRemaining method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_REINFORCEMENT_TIME_REMAINING}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_REINFORCEMENT_TIME_REMAINING.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_REINFORCEMENT_TIME_REMAINING.lower(),
                "mission07 GetReinforcementTimeRemaining contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_REINFORCEMENT_TIME_REMAINING)

    def test_contract_is_get_reinforcement_time_remaining_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_reinforcement_time_remaining(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_REINFORCEMENT_TIME_REMAINING)
            )
            self.assertNotEqual(GET_REINFORCEMENT_TIME_REMAINING, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetReinforcementTimeRemaining", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_REINFORCEMENT_TIME_REMAINING))
        locked_only = f"{GET_REINFORCEMENT_TIME_REMAINING}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tfloat GetReinforcementTimeRemaining() const { return 0; }\n"
            "};\n"
        )
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("HandleBossPhaseChanged", GET_REINFORCEMENT_TIME_REMAINING)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_BOSS)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission07_get_reinforcement_time_remaining"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_get_reinforcement_time_remaining"
                "_decl_contract.py"
            )
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission07_advance_reinforcement_timer"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
