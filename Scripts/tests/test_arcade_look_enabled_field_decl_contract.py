# THIS IS leftover-safe USkyguardArcadeLookComponent bEnabled.
...
# then BARE `bool bEnabled = true;` (initializer IS true).
...
# Stop BEFORE the next UPROPERTY sibling (Contrast)
...
# Do NOT contract sibling fields Contrast / Saturation /
...
# RETARGET: type is bool, identifier is bEnabled,
# initializer is true, wrap is EditAnywhere
...
LOCKED_DECL = "bool bEnabled = true;"
...
TARGET_WRONG_BARE = "bool bEnabled;"
TARGET_WRONG_FALSE = "bool bEnabled = false;"
TARGET_WRONG_ZERO = "bool bEnabled = 0.f;"
TARGET_WRONG_FNAME = "FName bEnabled;"
TARGET_WRONG_FLOAT = "float bEnabled = true;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
STOP_BEFORE_CONTRAST = "float Contrast = 1.18f;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bEnabled = true;`.
    ...
    # bKeepThermal. Do not accept `= false`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"bool\s+bEnabled\s*=\s*true\s*;", compact) is None:
        return False
    if re.search(r"FName\s+WeatherIdentity\b", compact):
        return False
    if re.search(r"bool\s+bEnabled\s*=\s*false\s*;", compact):
        return False
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*bool\s+bEnabled\s*=\s*true\s*;",
            after,
        )
    ...
    raise AssertionError(
        "UPROPERTY for bool bEnabled = true; is missing from "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bEnabled\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bEnabled is missing from "
        f"origin/main:{HEADER_PATH} class {CLASS_NAME} public "
        "UPROPERTY fields"
    )
...
class ArcadeLookEnabledFieldDeclContractTests(unittest.TestCase):
    def test_arcade_look_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bEnabled"), section)
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        self.assertIn("bEnabled", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_locked_decl_is_not_leftover_theater_kit_weather_identity(
        ...
        self.assertEqual(LOCKED_DECL, "bool bEnabled = true;")
        ...
        self.assertIn("bEnabled", LOCKED_DECL)
    ...
    def test_locked_decl_requires_true_initializer(self) -> None:
        self.assertIn("= true", LOCKED_DECL)
        ...
        self.assertIn("bool bEnabled = true;", compact_origin)
        self.assertNotIn("bool bEnabled;", compact_origin.replace(
            "bool bEnabled = true;",
            "",
        ))

    def test_enabled_declaration_matches_origin_main(self) -> None:
        section = class_public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bEnabled"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= true", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tbool bEnableds = true;\n",
            "\tint32 bEnabled = true;\n",
            "\tfloat bEnabled = " + forty + ";\n",
            "\tfloat bEnabled = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_arcade_look_fields(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_CONTRAST, section)
        self.assertNotIn(STOP_BEFORE_CONTRAST, claimed)

    def test_claimed_window_stops_before_contrast(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("bEnabled", claimed)
        self.assertIn("= true", claimed)
    ...
    def test_leftover_theater_kit_wrap_is_not_locked_wrap(self) -> None:
        ...
        self.assertNotIn("= true", LEFTOVER_THEATER_KIT_DECL)
        self.assertIn("= true", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "arcade-look bEnabled field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enabled_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardArcadeLookComponent.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardGuidedLockRules.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_ARCADE_LOOK_FAIL_CLOSED, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_CONTRAST, locked_only)
        self.assertNotIn(SIBLING_CONTRAST, claimed)
