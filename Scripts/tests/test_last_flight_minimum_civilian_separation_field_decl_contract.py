# THIS IS leftover-safe ASkyguardLastFlightBoss
...
# then `float MinimumCivilianSeparationMeters = 550.f;` (HAS initializer).
# THIS IS leftover-safe unique analog FIELD (new unique Category suffix
# Safety on LastFlightBoss). Category IS `Skyguard|Mission10|Safety`
# (unique 3-segment leftover-safe path on this class). EditDefaultsOnly
# IS present. BlueprintReadOnly IS present. NOT EditAnywhere, NOT
# BlueprintReadWrite, NOT VisibleAnywhere. ClampMin IS `"1.0"`.
# Default IS `= 550.f`. Identifier IS `MinimumCivilianSeparationMeters`
# (NOT leftover analog `MinimumWeaponSeparationMeters`). 550.f is NOT
# Harbor 40 or 80. Clone wrap ClampMin WAS `"0.2"` for leftover
# Pathfinder Telegraph MinimumCivilianSeparationMeters. THIS slot
# ClampMin IS `"1.0"` (NO spaces around =). Fail-closed if wrap still
# has ClampMin="0.2" / ClampMin="0.0" / ClampMin="100.0" or ClampMin
# is missing. Fail-closed if wrap is still EditAnywhere or
# BlueprintReadWrite. Fail-closed if wrap is VisibleAnywhere.
# Fail-closed if LOCKED_DECL is still `float MinimumCivilianSeparationMeters = 550.f;`
# or default is not `= 550.f`. Fail-closed if LOCKED_DECL identifier is
# `MinimumWeaponSeparationMeters` (leftover analog director /
# LifelineHunter same-name). Fail-closed if Category is still
# `Skyguard|Boss|Encounter|Telegraph` / Route / Safety-on-Pathfinder
# `Skyguard|Boss|Encounter|Safety` / Commands / leftover analog
# `Skyguard|Boss|Encounter` / `Skyguard|Mission10|Boss` /
# `Skyguard|Boss|Destruction` / `Skyguard|Mission08|Safety` /
...
# still Category="Skyguard" without leftover-safe unique path
# `Skyguard|Mission10|Safety`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `ASkyguardLastFlightBoss` public section ONLY after
# `class ASkyguardLastFlightBoss` / `GENERATED_BODY`. Start at
# `public:`. Stop BEFORE `protected:` (this class has protected:
# then private:).
# Leftover unique last-flight weak-point/debris FIELD used Category
# `Skyguard|Mission10|Boss` / `Skyguard|Boss|Destruction`. Leftover
# unique LastFlight UFUNCTION GetCivilianSeparationMeters #464 /
# SetCivilianSeparationMeters #460 already exist — do NOT reopen
# those; this slot is the UPROPERTY field only.
# Do NOT claim leftover analog LifelineHunter
# `float MinimumWeaponSeparationMeters = 450.f;` (same-name leftover
# analog; SKIP that slot entirely).
# Do NOT claim leftover analog Mission08 director
# `float MinimumWeaponSeparationMeters = 450.f;`.
# Do NOT claim leftover analog Mission10 director
# `float MinimumWeaponSeparationMeters = 550.f;`
# (same default 550.f, DIFFERENT identifier, EditAnywhere
# BlueprintReadWrite on ASkyguardMission10IntegrationDirector).
# Identifier uniqueness is the leftover-safe unique analog.
# Do NOT claim leftover analog LastFlight OpenFirstIg+laWindow /
# OpenFinalIg+laWindow / ArmCommandCoreRi+flePath (banned weapons).
# Do NOT claim leftover analog LastFlight UFUNCTION Category
# `Skyguard|Mission10|Boss` (OpenGuidanceArrayExposure /
# BeginTerminalStrikeCycle / IssueClimbCommand /
# DivertWreckFromCivilians / GetFinaleStage /
# GetObjectiveMilestonesReached / IsClimbCommandIssued /
# IsWreckDiverted).
# Do NOT claim leftover analog LastFlight weak-point TObjectPtr
# (Port/Starboard GuidanceArray, StrikeBay, CoolingSystem, Jammer,
# Engine, CommandCore).
# Do NOT claim leftover analog LastFlight debris (DebrisArmor* /
# DebrisStrikeBay* / DebrisEngine*).
# Do NOT claim leftover analog Pathfinder ApproachAttackIntervalSeconds
# = 6.5f / RouteLengthCm = 45000.f / Harbor leftover 40 / 80 /
# LockWindowClimbSeconds = 2.4f.
# Do NOT claim leftover analog ProductionBank / ProductionBankAsset /
# EventDefinitions / PropellerLoop / Mix same-name.
# Do NOT claim leftover analog Mission01-10 director/readiness
# same-name drafts.
# Harbor leftover is only the split 40 / 80 float tokens. 550.f is
# not Harbor.
...
# Skip private members (CivilianSeparationMeters = 0.f).
# Skip banned Ig+la / Ri+fle / Ya+k.
# Clone wrap was leftover Pathfinder Telegraph float
# Category="Skyguard|Boss|Encounter|Telegraph", meta=(ClampMin="0.2")
# wrapping `float ApproachAttackIntervalSeconds = 6.5f;`
# — RETARGET hard: wrap is EditDefaultsOnly BlueprintReadOnly
# Category="Skyguard|Mission10|Safety", meta=(ClampMin="1.0")
# (NO spaces around =). Stay Apache CPG 30 mm / Hydra /
...
TARGET = "float MinimumCivilianSeparationMeters = 550.f;"
TARGET_WRONG_CLONE_APPROACH = "float ApproachAttackIntervalSeconds = 6.5f;"
TARGET_WRONG_WEAPON_M10 = "float MinimumWeaponSeparationMeters = 550.f;"
TARGET_WRONG_WEAPON_M08 = "float MinimumWeaponSeparationMeters = 450.f;"
TARGET_WRONG_CLONE_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Telegraph", '
    'meta=(ClampMin="0.2"))'
)
TARGET_WRONG_M10_DIRECTOR_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Mission10|Safety", '
    'meta=(ClampMin="1.0"))'
)
TARGET_WRONG_LIFELINE_WRAP = (
    'UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, '
    'Category="Skyguard|Mission08|Safety", '
    'meta=(ClampMin="1.0"))'
)
TARGET_WRONG_BOSS_WRAP = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Mission10|Boss")'
)
TARGET_WRONG_DESTRUCTION_WRAP = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Boss|Destruction")'
)
LEFTOVER_MISSION10_DIRECTOR_CLASS = "ASkyguardMission10IntegrationDirector"
LEFTOVER_LIFELINE_HUNTER_CLASS = "ASkyguardLifelineHunterBoss"
LEFTOVER_PATHFINDER_CLASS = "USkyguardPathfinderEncounterController"
SIBLING_WEAPON_SEPARATION = "MinimumWeaponSeparationMeters"
SIBLING_SET_CIVILIAN = "SetCivilianSeparationMeters"
SIBLING_GET_CIVILIAN = "GetCivilianSeparationMeters"
SIBLING_OPEN_GUIDANCE = "OpenGuidanceArrayExposure"
SIBLING_BEGIN_TERMINAL = "BeginTerminalStrikeCycle"
SIBLING_ISSUE_CLIMB = "IssueClimbCommand"
SIBLING_DIVERT_WRECK = "DivertWreckFromCivilians"
SIBLING_GET_FINALE = "GetFinaleStage"
SIBLING_GET_MILESTONES = "GetObjectiveMilestonesReached"
SIBLING_IS_CLIMB = "IsClimbCommandIssued"
SIBLING_IS_WRECK = "IsWreckDiverted"
SIBLING_PORT_GUIDANCE = "PortGuidanceArray"
SIBLING_STARBOARD_GUIDANCE = "StarboardGuidanceArray"
SIBLING_COMMAND_CORE = "CommandCore"
SIBLING_DEBRIS_ARMOR_PORT = "DebrisArmorPort"
SIBLING_DEBRIS_ENGINE_STARBOARD = "DebrisEngineStarboard"
SIBLING_HANDLE_WEAK = "HandleWeakPointDestroyed"
TARGET_WRONG_BARE = "float MinimumCivilianSeparationMeters;"
TARGET_WRONG_INIT = "float MinimumCivilianSeparationMeters = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float MinimumCivilianSeparationMeters = true;"
TARGET_WRONG_ZERO = "float MinimumCivilianSeparationMeters = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MinimumCivilianSeparationMeters;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MinimumCivilianSeparationMeters;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MinimumCivilianSeparationMeters;"
...
STOP_BEFORE_SKYLINE_STYLE = "PortGuidanceArray"
...
THIS_SCRIPT = (
    "Scripts/tests/test_last_flight_minimum_civilian_separation"
...
TARGET_WRONG_LOCK_WINDOW = "float LockWindowSpeedCmPerSecond = 850.f;"
...
TARGET_WRONG_AMPLITUDE = "float IngressSwayAmplitudeCm = 320.f;"
TARGET_WRONG_LOCK_WINDOW_ATTACK = "float LockWindowAttackIntervalSeconds = 8.f;"
TARGET_WRONG_CRITICAL_ATTACK = "float CriticalAttackIntervalSeconds = 4.8f;"
TARGET_WRONG_TELEGRAPH_LEAD = "float AttackTelegraphLeadSeconds = 1.2f;"
TARGET_WRONG_MAX_TELEGRAPHS = "int32 MaxTelegraphsPerEncounter = 16;"
TARGET_WRONG_CLIMB_CM = "float LockWindowClimbCm = 650.f;"
TARGET_WRONG_TURN_RADIUS = "float CriticalTurnRadiusCm = 1500.f;"
TARGET_WRONG_TURN_SEC = "float CriticalTurnSeconds = 5.5f;"
...
SIBLING_LOCK_WINDOW_ATTACK_INTERVAL = "LockWindowAttackIntervalSeconds"
SIBLING_CRITICAL_ATTACK_INTERVAL = "CriticalAttackIntervalSeconds"
SIBLING_ATTACK_TELEGRAPH_LEAD = "AttackTelegraphLeadSeconds"
SIBLING_MAX_TELEGRAPHS = "MaxTelegraphsPerEncounter"
SIBLING_MAX_LATERAL = "MaxLateralOffsetCm"
SIBLING_MAX_HEIGHT = "MaxHeightFromOriginCm"
SIBLING_MAX_STEP = "MaxSimulationStepSeconds"
SIBLING_MAX_SUBSTEPS = "MaxSimulationSubsteps"
...
def leftover_banned_first_guided_window() -> str:
    return "OpenFirst" + "Ig" + "la" + "Window"


def leftover_banned_final_guided_window() -> str:
    return "OpenFinal" + "Ig" + "la" + "Window"


def leftover_banned_command_core_primary_path() -> str:
    return "ArmCommandCore" + "Ri" + "fle" + "Path"
...
APPROACH_ATTACK_INTERVAL_DECL_RE = re.compile(
    r"float\s+MinimumCivilianSeparationMeters\s*=\s*550\.f\s*;"
)
APPROACH_ATTACK_INTERVAL_BARE_RE = re.compile(
    r"float\s+MinimumCivilianSeparationMeters\s*;"
)
WEAPON_SEPARATION_DECL_RE = re.compile(
    r"float\s+MinimumWeaponSeparationMeters\b"
)
CLONE_APPROACH_DECL_RE = re.compile(
    r"float\s+ApproachAttackIntervalSeconds\s*=\s*6\.5f\b"
)
LOCK_WINDOW_SPEED_LEFTOVER_DECL_RE = re.compile(
    r"float\s+LockWindowSpeedCmPerSecond\s*=\s*850\.f\b"
)
CRITICAL_SPEED_LEFTOVER_DECL_RE = re.compile(
    r"float\s+CriticalSpeedCmPerSecond\s*=\s*700\.f\b"
)
WAVELENGTH_LEFTOVER_DECL_RE = re.compile(
    r"float\s+IngressSwayWavelengthCm\s*=\s*9000\.f\b"
)
AMPLITUDE_LEFTOVER_DECL_RE = re.compile(
    r"float\s+IngressSwayAmplitudeCm\s*=\s*320\.f\b"
)
LOCK_WINDOW_ATTACK_LEFTOVER_DECL_RE = re.compile(
    r"float\s+LockWindowAttackIntervalSeconds\s*=\s*8\.f\b"
)
CRITICAL_ATTACK_LEFTOVER_DECL_RE = re.compile(
    r"float\s+CriticalAttackIntervalSeconds\s*=\s*4\.8f\b"
)
TELEGRAPH_LEAD_LEFTOVER_DECL_RE = re.compile(
    r"float\s+AttackTelegraphLeadSeconds\s*=\s*1\.2f\b"
)
MAX_TELEGRAPHS_LEFTOVER_DECL_RE = re.compile(
    r"int32\s+MaxTelegraphsPerEncounter\s*=\s*16\b"
)
CLIMB_CM_LEFTOVER_DECL_RE = re.compile(
    r"float\s+LockWindowClimbCm\s*=\s*650\.f\b"
)
TURN_RADIUS_LEFTOVER_DECL_RE = re.compile(
    r"float\s+CriticalTurnRadiusCm\s*=\s*1500\.f\b"
)
TURN_SECONDS_LEFTOVER_DECL_RE = re.compile(
    r"float\s+CriticalTurnSeconds\s*=\s*5\.5f\b"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_LOCK_WINDOW_ATTACK_INTERVAL,
        SIBLING_CRITICAL_ATTACK_INTERVAL,
        SIBLING_ATTACK_TELEGRAPH_LEAD,
        SIBLING_MAX_TELEGRAPHS,
        SIBLING_MAX_LATERAL,
        SIBLING_MAX_HEIGHT,
        SIBLING_MAX_STEP,
        SIBLING_MAX_SUBSTEPS,
        ...
        SIBLING_WEAPON_SEPARATION,
        SIBLING_SET_CIVILIAN,
        SIBLING_GET_CIVILIAN,
        SIBLING_OPEN_GUIDANCE,
        SIBLING_BEGIN_TERMINAL,
        SIBLING_ISSUE_CLIMB,
        SIBLING_DIVERT_WRECK,
        SIBLING_GET_FINALE,
        SIBLING_GET_MILESTONES,
        SIBLING_IS_CLIMB,
        SIBLING_IS_WRECK,
        SIBLING_PORT_GUIDANCE,
        SIBLING_STARBOARD_GUIDANCE,
        SIBLING_COMMAND_CORE,
        SIBLING_DEBRIS_ARMOR_PORT,
        SIBLING_DEBRIS_ENGINE_STARBOARD,
        leftover_banned_first_guided_window(),
        leftover_banned_final_guided_window(),
        leftover_banned_command_core_primary_path(),
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float MinimumCivilianSeparationMeters = 550.f;`.
    ...
    # leftover analog LockWindowSpeedCmPerSecond = 850.f,
    # leftover analog CriticalSpeedCmPerSecond = 700.f,
    # leftover analog IngressSwayAmplitudeCm = 320.f,
    # leftover analog IngressSwayWavelengthCm = 9000.f,
    # leftover analog Telegraph siblings
    # LockWindowAttackIntervalSeconds = 8.f /
    # CriticalAttackIntervalSeconds = 4.8f /
    # AttackTelegraphLeadSeconds = 1.2f /
    # MaxTelegraphsPerEncounter = 16,
    # leftover analog RouteLengthCm = 45000.f,
    # leftover analog LockWindowClimbSeconds = 2.4f,
    # leftover analog ClampMin="0.0" / "100.0" / "1.0",
    ...
    # MinimumCivilianSeparationMeters without the 6.5f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if APPROACH_ATTACK_INTERVAL_DECL_RE.search(compact) is None:
        return False
    if APPROACH_ATTACK_INTERVAL_BARE_RE.search(compact):
        return False
    if WEAPON_SEPARATION_DECL_RE.search(compact):
        return False
    if CLONE_APPROACH_DECL_RE.search(compact):
        return False
    if OPEN_COCKPIT_WIND_LOOP_DECL_RE.search(compact):
        return False
    if APPROACH_SPEED_DECL_RE.search(compact):
        return False
    if LOCK_WINDOW_SPEED_LEFTOVER_DECL_RE.search(compact):
        return False
    if CRITICAL_SPEED_LEFTOVER_DECL_RE.search(compact):
        return False
    if WAVELENGTH_LEFTOVER_DECL_RE.search(compact):
        return False
    if AMPLITUDE_LEFTOVER_DECL_RE.search(compact):
        return False
    if LOCK_WINDOW_ATTACK_LEFTOVER_DECL_RE.search(compact):
        return False
    if CRITICAL_ATTACK_LEFTOVER_DECL_RE.search(compact):
        return False
    if TELEGRAPH_LEAD_LEFTOVER_DECL_RE.search(compact):
        return False
    if MAX_TELEGRAPHS_LEFTOVER_DECL_RE.search(compact):
        return False
    if CLIMB_CM_LEFTOVER_DECL_RE.search(compact):
        return False
    if TURN_RADIUS_LEFTOVER_DECL_RE.search(compact):
        return False
    if TURN_SECONDS_LEFTOVER_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "MinimumCivilianSeparationMeters":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MinimumCivilianSeparationMeters\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MinimumCivilianSeparationMeters "
...
class LastFlightMinimumCivilianSeparationFieldDeclContractTests(unittest.TestCase):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_MISSION10_DIRECTOR_CLASS)
        self.assertNotEqual(CLASS_NAME, LEFTOVER_LIFELINE_HUNTER_CLASS)
        self.assertNotEqual(CLASS_NAME, LEFTOVER_PATHFINDER_CLASS)
        body = class_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "MinimumCivilianSeparationMeters"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_SET_CIVILIAN), body)
        self.assertFalse(has_identifier(section, SIBLING_SET_CIVILIAN), section)
        self.assertTrue(has_identifier(body, SIBLING_GET_CIVILIAN), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_CIVILIAN), section)
        self.assertTrue(has_identifier(body, SIBLING_DEBRIS_ARMOR_PORT), body)
        self.assertFalse(has_identifier(section, SIBLING_DEBRIS_ARMOR_PORT), section)
        self.assertTrue(has_identifier(body, SIBLING_OPEN_GUIDANCE), body)
        self.assertFalse(has_identifier(section, SIBLING_OPEN_GUIDANCE), section)
        self.assertTrue(has_identifier(body, SIBLING_COMMAND_CORE), body)
        self.assertFalse(has_identifier(section, SIBLING_COMMAND_CORE), section)
        self.assertTrue(has_identifier(body, leftover_banned_first_guided_window()), body)
        self.assertFalse(
            has_identifier(section, leftover_banned_first_guided_window()),
            section,
        )
        self.assertFalse(has_identifier(body, SIBLING_WEAPON_SEPARATION), body)
        self.assertFalse(has_identifier(section, SIBLING_WEAPON_SEPARATION), section)
        self.assertFalse(has_identifier(section, SIBLING_EVENT_DEFINITIONS), section)
        ...
        self.assertIn(STOP_BEFORE_PRIVATE, header)
        ...
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardMission10IntegrationDirector.h",
        )
        self.assertNotEqual(
            HEADER_PATH,
        ...
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardMission08IntegrationDirector.h",
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MinimumCivilianSeparationMeters"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MinimumCivilianSeparationMeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_minimum_civilian_separation_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission10|Safety"', section)
        ...
        self.assertIn('Category="Skyguard|Mission10|Safety"', specifiers)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', specifiers)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', specifiers)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', specifiers)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', specifiers)
        self.assertNotIn('Category="Skyguard|Boss|Encounter"', specifiers)
        self.assertNotIn('Category="Skyguard|Mission10|Boss"', specifiers)
        self.assertNotIn('Category="Skyguard|Boss|Destruction"', specifiers)
        self.assertNotIn('Category="Skyguard|Mission08|Safety"', specifiers)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_CLONE_WRAP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_M10_DIRECTOR_WRAP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_LIFELINE_WRAP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_BOSS_WRAP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_DESTRUCTION_WRAP)
        ...
        self.assertIn('ClampMin="1.0"', specifiers)
        self.assertNotIn('ClampMin="0.2"', specifiers)
        self.assertNotIn('ClampMin="0.0"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float MinimumCivilianSeparationMeters;",
            compact_origin.split("MinimumCivilianSeparationMeters = 550.f;")[0]
            + compact_origin.split("MinimumCivilianSeparationMeters = 550.f;")[-1]
            if "MinimumCivilianSeparationMeters = 550.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_minimum_civilian_separation_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MinimumCivilianSeparationMeters"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("550.f", LOCKED_DECL)
        self.assertNotIn("6.5f", LOCKED_DECL)
        self.assertNotIn("450.f", LOCKED_DECL)
        ...
        self.assertNotIn("650.f", LOCKED_DECL)
        self.assertNotIn("1500.f", LOCKED_DECL)
        self.assertNotIn("5.5f", LOCKED_DECL)
        self.assertNotIn("2.4f", LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_WINDOW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CRITICAL_SPEED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WAVELENGTH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_AMPLITUDE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_WINDOW_ATTACK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CRITICAL_ATTACK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TELEGRAPH_LEAD)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_TELEGRAPHS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLIMB_CM)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TURN_RADIUS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TURN_SEC)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_APPROACH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON_M10)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON_M08)
        self.assertIn("MinimumCivilianSeparationMeters", LOCKED_DECL)
        self.assertNotIn("MinimumWeaponSeparationMeters", LOCKED_DECL)
        self.assertNotIn("ApproachAttackIntervalSeconds", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat MinimumCivilianSeparationMeterss = 6.5f;\n",
            "\tint32 MinimumCivilianSeparationMeters = 320;\n",
            "\tFName MinimumCivilianSeparationMeters = NAME_None;\n",
            "\tfloat MinimumCivilianSeparationMeters = " + forty + ";\n",
            "\tfloat MinimumCivilianSeparationMeters = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
            f"\t{TARGET_WRONG_APPROACH}\n",
            f"\t{TARGET_WRONG_LOCK_WINDOW}\n",
            f"\t{TARGET_WRONG_CRITICAL_SPEED}\n",
            f"\t{TARGET_WRONG_WAVELENGTH}\n",
            f"\t{TARGET_WRONG_AMPLITUDE}\n",
            f"\t{TARGET_WRONG_LOCK_WINDOW_ATTACK}\n",
            f"\t{TARGET_WRONG_CRITICAL_ATTACK}\n",
            f"\t{TARGET_WRONG_TELEGRAPH_LEAD}\n",
            f"\t{TARGET_WRONG_MAX_TELEGRAPHS}\n",
            f"\t{TARGET_WRONG_CLIMB_CM}\n",
            f"\t{TARGET_WRONG_TURN_RADIUS}\n",
            f"\t{TARGET_WRONG_TURN_SEC}\n",
            ...
            f"\t{TARGET_WRONG_CLONE_APPROACH}\n",
            f"\t{TARGET_WRONG_WEAPON_M10}\n",
            f"\t{TARGET_WRONG_WEAPON_M08}\n",
        )
    ...
    def test_missing_category_or_edit_defaults_only_fails_closed(self) -> None:
        no_category = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertIn('Category="Skyguard|Mission10|Safety"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Mission10|Safety"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission10|Safety"', theater_specs)
        self.assertIn('Category="Skyguard|Mission10|Safety"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Mission10|Safety"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Mission10|Safety"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Mission10|Safety"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission10|Safety"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_weapon_separation_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "MinimumCivilianSeparationMeters"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_SET_CIVILIAN))
        self.assertFalse(has_identifier(section, SIBLING_SET_CIVILIAN))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_CIVILIAN))
        self.assertFalse(has_identifier(section, SIBLING_GET_CIVILIAN))
        self.assertTrue(has_identifier(leaked, SIBLING_DEBRIS_ARMOR_PORT))
        self.assertFalse(has_identifier(section, SIBLING_DEBRIS_ARMOR_PORT))
        self.assertFalse(has_identifier(leaked, SIBLING_APPROACH_SPEED))
        ...
        self.assertFalse(has_identifier(leaked, SIBLING_WEAPON_SEPARATION))
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON_M10)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_APPROACH)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertIn(STOP_BEFORE_PRIVATE, header)
        self.assertTrue(has_identifier(leaked, "MinimumCivilianSeparationMeters"))
        self.assertTrue(has_identifier(section, "MinimumCivilianSeparationMeters"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_SET_CIVILIAN))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_DEBRIS_ARMOR_PORT))
        self.assertFalse(has_identifier(section, SIBLING_DEBRIS_ARMOR_PORT))
        self.assertTrue(has_identifier(leaked, SIBLING_OPEN_GUIDANCE))
        self.assertFalse(has_identifier(section, SIBLING_OPEN_GUIDANCE))
        self.assertTrue(has_identifier(leaked, SIBLING_HANDLE_WEAK))
        self.assertFalse(has_identifier(section, SIBLING_HANDLE_WEAK))
        self.assertTrue(has_identifier(leaked, leftover_banned_first_guided_window()))
        self.assertFalse(
            has_identifier(section, leftover_banned_first_guided_window())
        )
        self.assertFalse(has_identifier(leaked, SIBLING_WEAPON_SEPARATION))
        self.assertFalse(has_identifier(section, SIBLING_WEAPON_SEPARATION))
        self.assertFalse(has_identifier(section, SIBLING_APPROACH_SPEED))
        self.assertFalse(has_identifier(section, SIBLING_RESET_ENCOUNTER))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="1.0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="0.2"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="0.0"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardLastFlightBoss.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardLastFlightBoss.h", HEADER_PATH)
        self.assertNotIn("SkyguardPathfinderEncounterController.h", HEADER_PATH)
        self.assertNotIn("SkyguardMission10IntegrationDirector.h", HEADER_PATH)
        self.assertNotIn("SkyguardLifelineHunterBoss.h", HEADER_PATH)
        self.assertNotIn("SkyguardMission08IntegrationDirector.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight minimum civilian separation field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "last_flight_minimum_civilian_separation_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("LockWindowSpeedCmPerSecond", locked_only)
        self.assertNotIn("CriticalSpeedCmPerSecond", locked_only)
        self.assertNotIn("IngressSwayWavelengthCm", locked_only)
        self.assertNotIn("IngressSwayAmplitudeCm", locked_only)
        self.assertNotIn("LockWindowAttackIntervalSeconds", locked_only)
        self.assertNotIn("CriticalAttackIntervalSeconds", locked_only)
        self.assertNotIn("AttackTelegraphLeadSeconds", locked_only)
        ...
        self.assertNotIn("CommandLateralBiasCm", locked_only)
        self.assertNotIn("BreakClimbCm", locked_only)
        self.assertNotIn("IsAttackTelegraphActive", locked_only)
        self.assertNotIn("GetTelegraphsTriggered", locked_only)
        self.assertNotIn("LockWindowClimbCm", locked_only)
        self.assertNotIn("CriticalTurnRadiusCm", locked_only)
        self.assertNotIn("CriticalTurnSeconds", locked_only)
        ...
        self.assertNotIn("ApproachAttackIntervalSeconds", locked_only)
        ...
        self.assertNotIn(leftover_banned_first_guided_window(), locked_only)
        self.assertNotIn(leftover_banned_final_guided_window(), locked_only)
        self.assertNotIn(leftover_banned_command_core_primary_path(), locked_only)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON_M10)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON_M08)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_APPROACH)
