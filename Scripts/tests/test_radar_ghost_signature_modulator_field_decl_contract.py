...
# INDEX_NONE or lock SignatureModulator construction
# in the .cpp. This is a FIELD contract on
# ASkyguardRadarGhostBoss, not leftover RadarGhost
# public methods already drafted
# (SetContactIdentified / OpenOrbitExposure /
# IsContactIdentified) and not leftover
# RadarGhost sibling fields RadarReceiver /
# CoolingDoor / Engine / DebrisPortEWPanel /
# DebrisStarboardEWPanel / DebrisCoolingDoor.
# Stay off leftover rear-aspect window /
# break-finish named surfaces. Stay off the
...
# SignatureModulator;`); accept that form and other
...
# SignatureModulator field declaration contract on
# ASkyguardRadarGhostBoss. Stay off leftover
# sibling RadarGhost fields, leftover drafted
# RadarGhost methods, leftover Iron Rain
...
# isolated RadarGhost field contracts and leftover
# RadarGhost method contracts stay unlocked for
# THIS test's primary lock. Leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRadarGhostBoss::SignatureModulator",
    "SkyguardRadarGhostBoss.cpp",
    "SetExposed",
...
class RadarGhostSignatureModulatorFieldDeclContractTests(unittest.TestCase):
    def test_radar_ghost_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SIGNATURE_MODULATOR_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, SIGNATURE_MODULATOR_FIELD)
    ...
    def test_missing_signature_modulator_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRadarGhostBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_MODULATOR}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_MODULATOR, section)
        ...
        self.assertNotIn("UPROPERTY", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("VisibleAnywhere", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("BlueprintReadOnly", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("Category", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_MODULATOR)
        self.assertNotIn("BlueprintCallable", UPROPERTY_MODULATOR)
        self.assertIn("Skyguard|Mission07|Boss", UPROPERTY_MODULATOR)
        self.assertIn("Mission07", UPROPERTY_MODULATOR)
        self.assertNotIn("Destruction", UPROPERTY_MODULATOR)
        self.assertNotIn("Encounter", UPROPERTY_MODULATOR)
        self.assertNotIn("Mission10", UPROPERTY_MODULATOR)
        self.assertNotIn("Safety", UPROPERTY_MODULATOR)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_MODULATOR)
            self.assertNotIn(invented, SIGNATURE_MODULATOR_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* SignatureModulator;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> SignatureModulator;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> SignatureModulator;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SIGNATURE_MODULATOR_FIELD)
            self.assertIn("SignatureModulator", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_signature_modulator_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SIGNATURE_MODULATOR_FIELD),
            SIGNATURE_MODULATOR_FIELD,
        )
        self.assertTrue(has_declaration(section, SIGNATURE_MODULATOR_FIELD))
        self.assertEqual(declaration_count(section, SIGNATURE_MODULATOR_FIELD), 1)
        self.assertTrue(
            SIGNATURE_MODULATOR_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            SIGNATURE_MODULATOR_FIELD,
        )
        self.assertTrue(SIGNATURE_MODULATOR_FIELD.endswith(";"), SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", SIGNATURE_MODULATOR_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            SIGNATURE_MODULATOR_FIELD,
        )
        self.assertIn("TObjectPtr<", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("TArray<", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("=", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("INDEX_NONE", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("UFUNCTION", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("{", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("}", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("return ", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("CommandAntenna", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ControlLinkage", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("NoseCamera", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisCenter", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisTail", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisNose", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisSpine", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("AdvanceEncounter", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ResetEncounterState", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("IsRouteStateSafe", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetRouteProgress", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("RadarNode", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("IssueClimbCommand", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetFinaleStage", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("IsClimbCommandIssued", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("IsWreckDiverted", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetGunnerMount", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("FaceWorldLocation", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("PortGuidanceArray", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisArmorPort", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetSearchlightTracked", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetContactIdentified", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetLightningExposed", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("HullCollider", SIGNATURE_MODULATOR_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, SIGNATURE_MODULATOR_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_MODULATOR}\n"
            f"\t{SIGNATURE_MODULATOR_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MODULATOR} {SIGNATURE_MODULATOR_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, SIGNATURE_MODULATOR_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{SIGNATURE_MODULATOR_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, SIGNATURE_MODULATOR_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SIGNATURE_MODULATOR_FIELD),
            section,
    ...
    def test_assigned_signature_modulator_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "SignatureModulator = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, SIGNATURE_MODULATOR_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("SignatureModulator", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, SIGNATURE_MODULATOR_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{SIGNATURE_MODULATOR_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, SIGNATURE_MODULATOR_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_MODULATOR, section)
        self.assertTrue(has_declaration(section, SIGNATURE_MODULATOR_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            SIGNATURE_MODULATOR_FIELD.startswith("UFUNCTION"),
            SIGNATURE_MODULATOR_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SIGNATURE_MODULATOR_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", SIGNATURE_MODULATOR_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_signature_modulator_cpp_body(self) -> None:
        locked_only = f"{SIGNATURE_MODULATOR_FIELD}\n"
        self.assertNotIn("{", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("}", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("return ", SIGNATURE_MODULATOR_FIELD)
        ...
        self.assertNotIn("SkyguardRadarGhostBoss.cpp", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SkyguardRadarGhostBoss.cpp", locked_only)
        self.assertNotIn("return false", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("return true", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("CreateDefaultSubobject", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetExposed", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            SIGNATURE_MODULATOR_FIELD,
        )
        self.assertNotIn("SetBossPhase", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", SIGNATURE_MODULATOR_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{SIGNATURE_MODULATOR_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("PortDischargeBoom", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("StarboardDischargeBoom", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("EngineIntake", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisPortPanel", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisStarboardPanel", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisIntakePanel", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", SIGNATURE_MODULATOR_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            SIGNATURE_MODULATOR_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("EncounterController", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ControlServo", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DispenserCenter", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("MaxReleasesPerBay", SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DispenserStarboard", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetManeuver", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), SIGNATURE_MODULATOR_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, SIGNATURE_MODULATOR_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetSearchlightTracked", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetContactIdentified", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("OpenOrbitExposure", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_sibling_radar_ghost_fields(self) -> None:
        ...
        for token in SIBLING_RADAR_GHOST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("RadarReceiver", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("CoolingDoor", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisPortEWPanel", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisStarboardEWPanel", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("DebrisCoolingDoor", SIGNATURE_MODULATOR_FIELD)
        self.assertIn("SignatureModulator", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SetLightningExposed", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("IsLightningExposed", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ASkyguardGunner", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SkyguardRadarNode", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("MaxIntegrity", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("CurrentIntegrity", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            SIGNATURE_MODULATOR_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SIGNATURE_MODULATOR_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", SIGNATURE_MODULATOR_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SIGNATURE_MODULATOR_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, SIGNATURE_MODULATOR_FIELD),
            SIGNATURE_MODULATOR_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRadarGhostBoss::SignatureModulator", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar ghost SignatureModulator field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{SIGNATURE_MODULATOR_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SIGNATURE_MODULATOR_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SIGNATURE_MODULATOR_FIELD.lower(),
                "radar ghost SignatureModulator contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SIGNATURE_MODULATOR_FIELD)

    def test_contract_is_signature_modulator_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(SIGNATURE_MODULATOR_FIELD.endswith(";"))
        self.assertNotIn("=", SIGNATURE_MODULATOR_FIELD)
        self.assertIn(UPROPERTY_MODULATOR, section)
