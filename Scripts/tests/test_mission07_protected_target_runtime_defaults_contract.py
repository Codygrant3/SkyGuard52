...
# Search-track (#219) and search-sector (#218) are sibling Mission 07 contracts.
LOCKED_SCRIPTS = (
...
# full ESkyguardMission07ProtectedTarget enumerator set.
PUBLIC_FIELDS = (
...
# Search-track (#219), search-sector (#218), readiness (bYakRuntimeReady),
...
def in_class_defaults(body: str) -> dict[str, str]:
    return {
        ...
        for name, value in re.findall(
            r"(?:ESkyguardMission07ProtectedTarget|int32|bool)\s+(\w+)\s*=\s*([^;]+);",
            body,
        )
    }


class Mission07ProtectedTargetRuntimeDefaultsContractTests(unittest.TestCase):
    ...
    def test_contract_does_not_lock_search_track_sector_readiness_or_wave(
        self,
    ) -> None:
        body = protected_target_body(origin_main_header())
        defaults = in_class_defaults(body)
        ...
        self.assertNotIn("FSkyguardSearchTrackRuntime", body)
        self.assertNotIn("ESkyguardSearchSector", body)
        ...
        self.assertNotIn("FishingFleet", body)
