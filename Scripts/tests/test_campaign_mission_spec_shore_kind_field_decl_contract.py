# THIS IS leftover-safe FSkyguardCampaignMissionSpec ShoreKind.
# origin/main form: plain C++ field
# `ESkyguardThreatKind ShoreKind = ESkyguardThreatKind::GroundArmor;`
# with in-struct initializer ::GroundArmor. NOT a UPROPERTY wrap.
...
# FastAttacker / HeavyAttacker / RotorScout / PatrolShip
# instead of GroundArmor.
...
# Harbor Breaker Shore. Harbor fail-closed is ONLY 40/80.
...
# SupportKind / ExtractKind siblings.
...
# ::GroundArmor. Parse window comes from leftover-safe
...
TARGET_WRONG_BARE = "ESkyguardThreatKind ShoreKind;"
TARGET_WRONG_GROUND = (
    "ESkyguardThreatKind ShoreKind = "
...
TARGET_WRONG_EQ_NONE = "ESkyguardThreatKind ShoreKind = NAME_None;"
TARGET_WRONG_FALSE = "ESkyguardThreatKind ShoreKind = false;"
TARGET_WRONG_TRUE = "ESkyguardThreatKind ShoreKind = true;"
TARGET_WRONG_ZERO = "ESkyguardThreatKind ShoreKind = 0.f;"
TARGET_WRONG_FLOAT = "float ShoreKind;"
TARGET_WRONG_HEALTH = "float ShoreKind = 160.f;"
TARGET_WRONG_BOOL = "bool ShoreKind;"
TARGET_WRONG_INT = "int32 ShoreKind;"
TARGET_WRONG_FNAME = "FName ShoreKind;"
...
LEFTOVER_DAY_BEAT_THREAT_SCRIPT = (
...
SHORE_KIND_DECL_RE = re.compile(
    r"ESkyguardThreatKind\s+ShoreKind\s*=\s*"
    r"ESkyguardThreatKind\s*::\s*GroundArmor\s*;"
)
SHORE_KIND_INIT_RE = re.compile(
    r"ESkyguardThreatKind\s+ShoreKind\s*=\s*([^;]+);"
)
WRONG_INITIALIZERS = (
    "ESkyguardThreatKind::FastAttacker",
...
def shore_kind_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardThreatKind\s+ShoreKind\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # FastAttacker, leftover SupportKind HeavyAttacker,
    # leftover ExtractKind RotorScout, leftover Climax
    # PatrolShip, leftover StormRainBeatKit Kinds, leftover
    # Harbor Breaker Shore, `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` when origin/main locks
    # ::GroundArmor. Do not accept leftover TheaterKit
    # Category clones as this CampaignMissionSpec parse
    # window.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SHORE_KIND_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+ShoreKind\b", compact):
        return False
    if re.search(r"FName\s+ShoreKind\b", compact):
        return False
    if re.search(r"bool\s+ShoreKind\b", compact):
        return False
    if re.search(r"int32\s+ShoreKind\b", compact):
        return False
    if shore_kind_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    for match in SHORE_KIND_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != "ESkyguardThreatKind::GroundArmor":
            return False
    for leftover_init in WRONG_INITIALIZERS:
        if leftover_init in compact and leftover_init.split("::")[1] != (
            ...
            if re.search(
                r"ShoreKind\s*=\s*" + re.escape(leftover_init),
                compact,
            ):
                return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardThreatKind\s+ShoreKind\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardThreatKind ShoreKind is missing "
...
class CampaignMissionSpecShoreKindFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_enum_not_uproperty(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardThreatKind ShoreKind"),
        )
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("::GroundArmor", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CONTACT_KIND)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_DAY_BEAT_THREAT_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("ShoreKind", str(raised.exception))
    ...
    def test_leftover_storm_rain_kinds_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn("ShoreKind", LEFTOVER_STORM_KINDS_DECL)
        leftover = (
    ...
    def test_missing_shore_kind_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_kind_fields_do_not_satisfy_shore_kind(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_CONTACT_KIND}\n"
    ...
    def test_initializer_must_be_ground_armor(self) -> None:
        missing = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
        ...
        self.assertNotIn(
            "ShoreKind = ESkyguardThreatKind::FastAttacker",
            compact_origin,
        )
        self.assertNotIn(
            "ShoreKind = ESkyguardThreatKind::HeavyAttacker",
            compact_origin,
        )
        self.assertNotIn(
            "ShoreKind = ESkyguardThreatKind::RotorScout",
            compact_origin,
        )
        self.assertNotIn(
            "ShoreKind = ESkyguardThreatKind::PatrolShip",
            compact_origin,
        )
        self.assertNotIn("ShoreKind = NAME_None", compact_origin)
        self.assertNotIn("ShoreKind = false", compact_origin)
        self.assertNotIn("ShoreKind = true", compact_origin)
        self.assertNotIn("ShoreKind = 0.f", compact_origin)
        self.assertNotIn("ShoreKind = 160.f", compact_origin)

    def test_shore_kind_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tESkyguardThreatKind ShoreKinds = "
            "ESkyguardThreatKind::GroundArmor;\n",
            "\tint32 ShoreKind;\n",
            "\tbool ShoreKind;\n",
            "\tfloat ShoreKind = " + forty + ";\n",
            "\tfloat ShoreKind = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    shore_kind_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(shore_kind_has_uproperty_wrap(origin), origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tESkyguardThreatKind\n"
            "\tShoreKind = ESkyguardThreatKind::GroundArmor;\n",
            "\tESkyguardThreatKind   ShoreKind = "
            "ESkyguardThreatKind::GroundArmor;\n",
            "\tESkyguardThreatKind\tShoreKind = "
            "ESkyguardThreatKind::GroundArmor;\n",
    ...
    def test_harbor_breaker_shore_is_not_this_spec(self) -> None:
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec ShoreKind field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "shore_kind_field_decl_contract.py"
        ))
        self.assertIn("campaign_mission_spec", Path(__file__).name)
        self.assertIn("shore_kind", Path(__file__).name)
        ...
        self.assertIn(LEFTOVER_DAY_BEAT_THREAT_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_DAY_BEAT_THREAT_SCRIPT)
