# THIS IS leftover-safe ASkyguardGunner IsVerticalLookInverted.
# origin/main form: one-line and split-line UFUNCTION wraps
# UFUNCTION(BlueprintPure, Category="Skyguard|Settings")
# wrapping `bool IsVerticalLookInverted() const { return bInvertVerticalLookApplied; }`.
# LOCKED_DECL may be declaration-only
# `bool IsVerticalLookInverted() const;` if the matcher strips
# inline bodies, OR the full origin/main inline form.
# Accept declaration-only AND the origin/main inline body.
# Do NOT fail-closed on `{ return bInvertVerticalLookApplied; }`.
...
# prefix). Identifier IsVerticalLookInverted is NOT leftover analog
# GetInvertVerticalLook (different identifier, different class).
# This is leftover-safe unique analog, NOT leftover analog Settings
# and NOT leftover analog Combat|Sortie.
# BlueprintPure IS present. const IS present. Return type IS bool.
...
# const is missing, if the return type is not bool, or if Category
...
# GetAppliedLookSensitivity / GetAppliedCameraShakeScale
...
# Do NOT claim leftover unique Gunner FIELD Boom / LookYawLimit /
# LookPitchMin / LookPitchMax / CPG kit.
...
# GetInvertVerticalLook / SetInvertVerticalLook (different class;
# different identifier GetInvertVerticalLook vs
# IsVerticalLookInverted; Category Settings|Input without Skyguard|).
...
# method GetSortieHits → IsVerticalLookInverted, return int32 →
# bool. KEEP BlueprintPure. KEEP const. Accept declaration-only
# AND origin/main inline body.
# Clone leftover unique AudioDirector Production UFUNCTION
# AreResolvedProductionLoopRoutesComplete #1659 is BlueprintPure bool
# const — RETARGET class AudioDirector → ASkyguardGunner, Category
# Audio|Production → Skyguard|Settings, method →
# IsVerticalLookInverted. KEEP BlueprintPure. KEEP const.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity —
# RETARGET hard.
# LOCKED_DECL is `bool IsVerticalLookInverted() const;`
# NOT `int32 GetSortieHits() const;` and NOT
# `FName WeatherIdentity;` and NOT `float GetAppliedLookSensitivity() const;`
# and NOT `float GetAppliedCameraShakeScale() const;` and NOT
...
TARGET = "bool IsVerticalLookInverted() const;"
TARGET_WRONG_BARE = "bool IsVerticalLookInverted();"
TARGET_WRONG_INIT = "bool IsVerticalLookInverted() = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "bool IsVerticalLookInverted() = true;"
TARGET_WRONG_ZERO = "bool IsVerticalLookInverted() = 0.f;"
TARGET_WRONG_NO_CONST = "bool IsVerticalLookInverted();"
TARGET_WRONG_FLOAT = "float IsVerticalLookInverted() const;"
TARGET_WRONG_FNAME_RET = "FName IsVerticalLookInverted() const;"
...
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> IsVerticalLookInverted;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> IsVerticalLookInverted;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> IsVerticalLookInverted;"
...
SIBLING_GET_APPLIED_SHAKE = "GetAppliedCameraShakeScale"
...
TARGET_WRONG_SHAKE_SCALE = "float GetAppliedCameraShakeScale() const;"
TARGET_WRONG_GET_INVERT = "bool GetInvertVerticalLook() const;"
TARGET_WRONG_SET_INVERT = "void SetInvertVerticalLook(bool bValue);"
TARGET_WRONG_ROUTE_LENGTH = "float RouteLengthCm = 45000.f;"
LEFTOVER_GAME_USER_SETTINGS_CLASS = "USkyguardGameUserSettings"
...
GET_SORTIE_HITS_RE = re.compile(
    r"bool\s+IsVerticalLookInverted\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+bInvertVerticalLookApplied\s*;\s*\})"
)
GET_SORTIE_HITS_WRONG_TYPE_RE = re.compile(
    r"(?:float|FName|int32|FLinearColor|void)\s+"
    r"IsVerticalLookInverted\b"
)
...
GET_SORTIE_HITS_NO_CONST_RE = re.compile(
    r"bool\s+IsVerticalLookInverted\s*\(\s*\)\s*;"
)
...
def has_one_declaration(region: str, declaration: str) -> bool:
    ...
    pattern = re.compile(
        re.escape(stem)
        + r"\s*(?:;|\{[^}]*return\s+bInvertVerticalLookApplied\s*;\s*\})"
    )
    return pattern.search(compact_region) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `bool IsVerticalLookInverted() const;`
    # OR origin/main inline `{ return bInvertVerticalLookApplied; }`.
    # Do not accept missing const, FName / float /
    # FLinearColor / int32 return types, leftover clone
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident in (
            ...
            break
        if ident != "IsVerticalLookInverted":
            cursor = end
            continue
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+IsVerticalLookInverted\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for bool IsVerticalLookInverted "
...
class GunnerIsVerticalLookInvertedDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_GAME_USER_SETTINGS_CLASS)
        body = class_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "IsVerticalLookInverted"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_GET_APPLIED_SHAKE), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_APPLIED_SHAKE), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "IsVerticalLookInverted"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("IsVerticalLookInverted", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_is_vertical_look_inverted_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            collapsed("bool IsVerticalLookInverted();"),
            compact_origin,
        )
    ...
    def test_is_vertical_look_inverted_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool IsVerticalLookInverted"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_SENS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SHAKE_SCALE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GET_INVERT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SET_INVERT)
        ...
        self.assertNotIn("GetInvertVerticalLook", LOCKED_DECL)
        self.assertNotIn("SetInvertVerticalLook", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool IsVerticalLookInverteds() const;\n",
            "\tfloat IsVerticalLookInverted() const;\n",
            "\tFName IsVerticalLookInverted() const;\n",
            "\tbool IsVerticalLookInverted() const { return " + forty + "; }\n",
            "\tbool IsVerticalLookInverted() const { return " + eighty + "; }\n",
            f"\t{TARGET_WRONG_LAMP}\n",
            f"\t{TARGET_WRONG_PEAK}\n",
            ...
            f"\t{TARGET_WRONG_SHAKE_SCALE}\n",
            f"\t{TARGET_WRONG_GET_INVERT}\n",
    ...
    def test_missing_category_or_blueprint_pure_fails_closed(self) -> None:
        ...
        gus_settings_specs = attached_uproperty_specifiers(
            spec_section(leftover_gus_settings)
        )
        self.assertIn('Category="Settings"', gus_settings_specs)
        self.assertNotIn('Category="Skyguard|Settings"', gus_settings_specs)
        leftover_gus_input = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        gus_input_specs = attached_uproperty_specifiers(
            spec_section(leftover_gus_input)
        )
        self.assertIn('Category="Settings|Input"', gus_input_specs)
        self.assertNotIn('Category="Skyguard|Settings"', gus_input_specs)
        leftover_gus_audio = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        gus_audio_specs = attached_uproperty_specifiers(
            spec_section(leftover_gus_audio)
        )
        self.assertIn('Category="Settings|Audio"', gus_audio_specs)
        self.assertNotIn('Category="Skyguard|Settings"', gus_audio_specs)
        leftover_gus_access = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_SETTINGS_ACCESS}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        gus_access_specs = attached_uproperty_specifiers(
            spec_section(leftover_gus_access)
        )
        self.assertIn('Category="Settings|Accessibility"', gus_access_specs)
        self.assertNotIn('Category="Skyguard|Settings"', gus_access_specs)
        leftover_sortie = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_ufunctions_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "IsVerticalLookInverted"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_GET_APPLIED_LOOK))
        self.assertFalse(has_identifier(section, SIBLING_GET_APPLIED_LOOK))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_APPLIED_SHAKE))
        self.assertFalse(has_identifier(section, SIBLING_GET_APPLIED_SHAKE))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "IsVerticalLookInverted"))
        self.assertTrue(has_identifier(section, "IsVerticalLookInverted"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_SORTIE_SHOTS))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner IsVerticalLookInverted decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_is_vertical_look_inverted_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_SETTINGS_GET_INVERT, LOCKED_SCRIPTS)
        leftovers = (
            LEFTOVER_MISSION_RESULT_DEFAULTS,
            LEFTOVER_MISSION_DEBRIEF_DEFAULTS,
            LEFTOVER_OBJECTIVE_PROGRESS_DEFAULTS,
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("GetAppliedCameraShakeScale", locked_only)
        self.assertNotIn("GetInvertVerticalLook", locked_only)
        self.assertNotIn("RouteLengthCm", locked_only)
        self.assertNotIn("45000.f", locked_only)
        self.assertNotIn(LEFTOVER_GAME_USER_SETTINGS_CLASS, locked_only)
