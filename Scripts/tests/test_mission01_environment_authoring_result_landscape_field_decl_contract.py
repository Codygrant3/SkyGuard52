...
# Landscape. THIS IS NOT leftover mission01-environment-authoring-
# result-defaults #df1a. THIS IS NOT leftover director
# ProductionLandscape #591. THIS IS NOT leftover
# EnvironmentReadiness bProductionLandscapeBound. THIS IS NOT leftover
# MaterialCompilation LandscapeComponentCount #1227.
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# Landscape FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly TObjectPtr<ALandscape> Landscape; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category and NO
# initializer). Fail closed if the type or decl is missing, renamed,
# or Category= is added. Parse STRUCT public section only AFTER
...
# leftover director ProductionLandscape #591, leftover
# CaptureConfigurationResult #1214-#1224, leftover
...
# Landscape. THIS IS NOT leftover VisibleAudit
...
# DIFFERENT field).
# THIS IS leftover-safe FSkyguardLandscapeMaterialCompilationResult
# Landscape FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# TObjectPtr<ALandscape> Landscape; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeMaterialCompilationResult
# Landscape. THIS IS NOT leftover VisibleAudit bSuccess #1199.
...
# Do NOT parse FSkyguardLandscapeVisibleAudit (leftover bSuccess #1199).
# THIS IS leftover-safe FSkyguardLandscapeMaterialCompilationResult
# Landscape. THIS IS NOT leftover
...
# BlueprintReadOnly TObjectPtr<ALandscape> Landscape;
...
TARGET = "TObjectPtr<ALandscape> Landscape;"
TARGET_WRONG = "TObjectPtr<ALandscape> Landscape = nullptr;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Landscape (must be bare, not = nullptr).
    # THIS IS NOT leftover VisibleAudit bSuccess #1199.
    # THIS IS NOT leftover FootprintSampleResult bSuccess #1189.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*TObjectPtr\s*<\s*ALandscape\s*>\s+Landscape\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for TObjectPtr<ALandscape> Landscape is missing from "
...
class Mission01EnvironmentAuthoringResultLandscapeFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Landscape", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_landscape_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_landscape_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<ALandscape> Landscape"),
            LOCKED_DECL,
        )
        ...
        # Bare TObjectPtr<ALandscape> Landscape; keeps angle brackets.
        self.assertNotIn("TArray<", LOCKED_DECL)
        self.assertNotIn("ESkyguardMission05ProtectedTarget", LOCKED_DECL)
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<ALandscape> "),
            LOCKED_DECL,
        )
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("Landscape", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("TObjectPtr<ALandscape> Landscape"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("Landscape", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_landscape(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
