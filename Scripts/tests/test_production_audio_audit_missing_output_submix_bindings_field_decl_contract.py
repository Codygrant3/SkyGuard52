...
# then BARE `TArray<FName> MissingOutputSubmixBindings;`.
...
# QATestOnlyCount / MissingCategoryEntries /
# InvalidSourceEntries / MissingSoundBindings /
# MissingAttenuationBindings / MissingConcurrencyBindings /
# MissingRoutingAssets / bCategoryContractComplete /
# bProductionReady.
# Match the EXACT identifier MissingOutputSubmixBindings as a
# whole field name after TArray<FName>. Do NOT accept a
# substring match on Missing, Output, Submix, or Bindings.
# Do NOT claim sibling Missing* arrays.
...
TARGET = "TArray<FName> MissingOutputSubmixBindings;"
TARGET_WRONG_EQ_EMPTY = "TArray<FName> MissingOutputSubmixBindings = {};"
TARGET_WRONG_EQ_ZERO = "TArray<FName> MissingOutputSubmixBindings = 0;"
TARGET_WRONG_FALSE = "TArray<FName> MissingOutputSubmixBindings = false;"
TARGET_WRONG_TRUE = "TArray<FName> MissingOutputSubmixBindings = true;"
TARGET_WRONG_ZERO = "TArray<FName> MissingOutputSubmixBindings = 0.f;"
TARGET_WRONG_FORTY = "TArray<FName> MissingOutputSubmixBindings = 40;"
TARGET_WRONG_EIGHTY = "TArray<FName> MissingOutputSubmixBindings = 80;"
TARGET_WRONG_FLOAT = "float MissingOutputSubmixBindings = 0;"
TARGET_WRONG_INT32 = "int32 MissingOutputSubmixBindings = 0;"
TARGET_WRONG_FNAME = "FName MissingOutputSubmixBindings;"
TARGET_WRONG_BOOL = "bool MissingOutputSubmixBindings = false;"
TARGET_WRONG_FSTRING = "FString MissingOutputSubmixBindings;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MissingOutputSubmixBindings;"
...
TARGET_WRONG_SIBLING_CONCUR = "TArray<FName> MissingConcurrencyBindings;"
...
SIBLING_MISSING_CONCURRENCY_BINDINGS = "MissingConcurrencyBindings"
...
FIELD_NAME = "MissingOutputSubmixBindings"
...
THIS_SCRIPT = (
    ...
    "_output_submix_bindings_field_decl_contract.py"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_MISSING_CONCURRENCY_BINDINGS,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored bare `TArray<FName> MissingOutputSubmixBindings;`.
    # Reject added initializer (`= {}`, `= 0`, Harbor 40/80),
    # int32, float, bool, FName, FString, TSoftObjectPtr.
    # Match the EXACT identifier MissingOutputSubmixBindings after
    # TArray<FName>. Do not accept a substring match on Missing,
    # Output, Submix, or Bindings. Do not claim sibling Missing*
    # arrays.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+MissingOutputSubmixBindings\s*=",
        compact,
    ):
        return False
    if re.search(
        r"MissingOutputSubmixBindings\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingOutputSubmixBindings\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingOutputSubmixBindings\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingOutputSubmixBindings\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingOutputSubmixBindings\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MissingOutputSubmixBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MissingOutputSubmixBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MissingOutputSubmixBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MissingOutputSubmixBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MissingOutputSubmixBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MissingOutputSubmixBindings\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+MissingOutputSubmixBindings\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> MissingOutputSubmixBindings is missing from "
...
class ProductionAudioAuditMissingOutputSubmixBindingsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_output_submix_bindings_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_added_fails_closed(self) -> None:
        ...
        self.assertIn("TArray<FName> MissingOutputSubmixBindings;", compact_origin)
        self.assertNotIn("MissingOutputSubmixBindings = 40", compact_origin)
        self.assertNotIn("MissingOutputSubmixBindings = 80", compact_origin)
        self.assertNotIn("MissingOutputSubmixBindings = false", compact_origin)
        self.assertNotIn("MissingOutputSubmixBindings = {}", compact_origin)
        self.assertNotIn("MissingOutputSubmixBindings = 0", compact_origin)

    def test_missing_output_submix_bindings_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> MissingOutputSubmixBindings"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tTArray<FName> MissingOutputSubmixBindingss;\n",
            "\tTArray<FName> MissingOutputSubmixBindings = 1;\n",
            "\tbool MissingOutputSubmixBindings;\n",
            "\tfloat MissingOutputSubmixBindings = " + forty + ";\n",
            "\tfloat MissingOutputSubmixBindings = " + eighty + ";\n",
    ...
    def test_does_not_contract_sibling_audit_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_CONCURRENCY_BINDINGS),
            section,
        )
        self.assertTrue(
    ...
    def test_exact_identifier_rejects_substring_missing_siblings(self) -> None:
        ...
        self.assertFalse(has_identifier(exact, "Output"))
        self.assertFalse(has_identifier(exact, "Submix"))
        self.assertFalse(has_identifier(exact, "Bindings"))
        self.assertFalse(has_identifier(exact, "OutputSubmixBindings"))
        self.assertNotEqual(FIELD_NAME, "Missing")
        self.assertNotEqual(FIELD_NAME, "Output")
        self.assertNotEqual(FIELD_NAME, "Submix")
        ...
        siblings = (
            ...
            f"TArray<FName> {SIBLING_MISSING_CONCURRENCY_BINDINGS};",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit MissingOutputSubmixBindings field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "missing_output_submix_bindings_field_decl_contract.py"
        ))
