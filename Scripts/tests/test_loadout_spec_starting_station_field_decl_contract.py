# THIS IS leftover-safe FSkyguardLoadoutSpec StartingStation.
...
# Fail-closed if a TheaterKit UPROPERTY clone still asserts
# UPROPERTY / Category / VisibleAnywhere / EditAnywhere /
# BlueprintReadOnly / GENERATED_BODY / USTRUCT as the locked decl.
# Locked decl must be the BARE field above.
# Parse ONLY `struct FSkyguardLoadoutSpec`. Stop before
...
# Do NOT parse leftover SkyguardApacheCpgFeel namespace body.
...
# test_loadout_spec_defaults_contract.py (keep that file
...
# Do NOT parse leftover gunship-weapon-stations #149.
# Do NOT parse leftover CpgDebriefSnapshot isolated
# #1451-#1465. Do NOT parse leftover MeshBindSlot
# #1447-#1450. Do NOT parse leftover TheaterKit.
...
# Exact identifier vs sibling Loadout (different type)
# and vs CannonMagazineSize / CannonReserve /
...
# initializer missing, or not
# = ESkyguardGunshipWeapon::Cannon.
# Do not accept Hydra / Hellfire / Rocket as this
# field's initializer. Cannon station initializer is
# not Harbor. Harbor 40/80 fail-closed.
# Ban retired live-copy tokens via split tokens
# (b + Ya + kRuntimeReady). Stay Apache CPG 30 mm /
# Hydra / Hellfire. Fail-closed on live Ig+la / Ri+fle /
...
LOCKED_DECL = TARGET
TARGET_WRONG_BARE = "ESkyguardGunshipWeapon StartingStation;"
TARGET_WRONG_LOADOUT_TYPE = (
    "ESkyguardLoadout StartingStation = "
    "ESkyguardLoadout::Balanced;"
)
TARGET_WRONG_ROCKETS = (
    "ESkyguardGunshipWeapon StartingStation = "
...
TARGET_WRONG_HYDRA = (
    "ESkyguardGunshipWeapon StartingStation = "
    "ESkyguardGunshipWeapon::Hydra;"
)
TARGET_WRONG_HELLFIRE = (
    "ESkyguardGunshipWeapon StartingStation = "
    "ESkyguardGunshipWeapon::Hellfire;"
)
TARGET_WRONG_SIBLING_LOADOUT = (
    "ESkyguardLoadout Loadout = ESkyguardLoadout::Balanced;"
)
TARGET_WRONG_INT = "int32 StartingStation = 0;"
TARGET_WRONG_FLOAT = "float StartingStation = 0.f;"
TARGET_WRONG_BOOL = "bool StartingStation = false;"
CLONE_UPROPERTY = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
    'Category="Skyguard|Theater")'
)
CLONE_USTRUCT = "USTRUCT(BlueprintType)"
CLONE_GENERATED_BODY = "GENERATED_BODY()"
...
STOP_BEFORE_DEBRIEF_SNAP = "FSkyguardCpgDebriefSnapshot"
STOP_BEFORE_HUD_SNAP = "FSkyguardCpgHudSnapshot"
...
STOP_BEFORE_AUDIO_EVENT_DEF = "FSkyguardAudioEventDefinition"
STOP_BEFORE_MISSION_DEF = "FSkyguardMissionDefinition"
...
def leftover_cpg_debrief_isolated_scripts() -> tuple[str, ...]:
    prefix = (
        "Scripts/tests/test_cpg_debrief_snapshot_"
    )
    return (
        prefix + "mission_title_field_decl_contract.py",
        prefix + "valid_field_decl_contract.py",
        prefix + "won_field_decl_contract.py",
        prefix + "outcome_narrative_field_decl_contract.py",
        prefix + "medal_field_decl_contract.py",
        prefix + "score_field_decl_contract.py",
        prefix + "cargo_percent_field_decl_contract.py",
        prefix + "hits_field_decl_contract.py",
        prefix + "shots_fired_field_decl_contract.py",
        prefix + "radar_dead_field_decl_contract.py",
        prefix + "destroyed_systems_field_decl_contract.py",
        prefix + "selected_loadout_field_decl_contract.py",
        prefix + "cannon_ready_field_decl_contract.py",
        prefix + "rocket_ready_field_decl_contract.py",
        prefix + "guided_ready_field_decl_contract.py",
    )


def leftover_mesh_bind_slot_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/test_mesh_bind_slot_"
    return (
        prefix + "fields_contract.py",
        prefix + "notes_field_decl_contract.py",
        prefix + "proxy_fallback_field_decl_contract.py",
        prefix + "slot_id_field_decl_contract.py",
        prefix + "preferred_field_decl_contract.py",
    )
...
def rejected_starting_station_initializer(compact: str) -> bool:
    match = re.search(
        r"\bStartingStation\s*=\s*([^;]+);",
        compact,
    )
    if match is None:
        return False
    value = match.group(1)
    for banned in ("Hydra", "Hellfire", "Rocket", "Rockets"):
        if banned in value:
            return True
    return False
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare field
    # ESkyguardGunshipWeapon StartingStation =
    # ESkyguardGunshipWeapon::Cannon;
    # Do not accept missing initializer, sibling Loadout
    # (different type), Hydra / Hellfire / Rocket
    # initializers, or a TheaterKit UPROPERTY clone.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"ESkyguardLoadout\s+StartingStation\b", compact):
        return False
    if re.search(r"(?:int32|float|bool|FName)\s+StartingStation\b", compact):
        return False
    if rejected_starting_station_initializer(compact):
        return False
    if re.search(
        r"ESkyguardGunshipWeapon\s+StartingStation\s*;",
        compact,
    ) and not re.search(
        r"ESkyguardGunshipWeapon\s+StartingStation\s*=",
        compact,
        ...
        return False
    if re.search(
        r"ESkyguardGunshipWeapon\s+StartingStation\s*=\s*"
        r"ESkyguardGunshipWeapon::Cannon\s*;",
        compact,
    ) is None:
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_FEEL,
        STOP_BEFORE_APACHE,
        STOP_BEFORE_DEBRIEF_SNAP,
        STOP_BEFORE_HUD_SNAP,
        STOP_BEFORE_SORTIE,
        STOP_BEFORE_MESH_CATALOG,
        STOP_BEFORE_AUDIO_HARNESS,
        STOP_BEFORE_AUDIO_BANK,
        STOP_BEFORE_AUDIO_EVENT_DEF,
...
def spec_section(header: str) -> str:
    ...
    # Stop before SkyguardResolveLoadout even if a leftover
    # clone pasted the free function into the struct window.
    resolve = body.find(STOP_BEFORE_RESOLVE)
    if resolve != -1:
        body = body[:resolve]
...
class LoadoutSpecStartingStationFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_DEBRIEF_SNAP)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_THEATER_SPEC)
        body = struct_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "StartingStation"), section)
        ...
        for sibling in sibling_uncontracted_decls():
            ...
            self.assertNotEqual(LOCKED_DECL, sibling)
        self.assertNotIn(SIBLING_LOADOUT, LOCKED_DECL)
        self.assertNotIn(SIBLING_CANNON_MAG, LOCKED_DECL)
        self.assertNotIn(SIBLING_PLAYSTYLE, LOCKED_DECL)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        debrief = (
            f"struct {STOP_BEFORE_DEBRIEF_SNAP}\n"
            "{\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            spec_section(debrief)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        theater = (
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "StartingStation"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("StartingStation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_starting_station_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SIBLING_LOADOUT}\n"
            "};\n"
        )
    ...
    def test_clone_uproperty_is_not_the_locked_decl(self) -> None:
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertNotEqual(LOCKED_DECL, CLONE_UPROPERTY)
        self.assertNotEqual(LOCKED_DECL, CLONE_USTRUCT)
        self.assertNotEqual(LOCKED_DECL, CLONE_GENERATED_BODY)
        for token in CLONE_UPROPERTY_TOKENS:
    ...
    def test_origin_main_plain_struct_has_no_uproperty(self) -> None:
    ...
    def test_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "StartingStation = ESkyguardGunshipWeapon::Rockets",
            compact_origin,
        )
        self.assertNotIn("StartingStation = ESkyguardGunshipWeapon::Hydra", compact_origin)
        self.assertNotIn(
            "StartingStation = ESkyguardGunshipWeapon::Hellfire",
            compact_origin,
        )

    def test_starting_station_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardGunshipWeapon StartingStation"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= ESkyguardGunshipWeapon::Cannon", LOCKED_DECL)
        self.assertIn("ESkyguardGunshipWeapon ", LOCKED_DECL)
        self.assertNotIn("ESkyguardLoadout ", LOCKED_DECL)
        ...
        self.assertNotIn("Hydra", LOCKED_DECL)
        self.assertNotIn("Hellfire", LOCKED_DECL)
        self.assertNotIn("Rocket", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_LOADOUT_TYPE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ROCKETS}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HYDRA}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HELLFIRE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_GUIDED}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SIBLING_LOADOUT}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_type_initializer_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
        ...
        wrongs = (
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_LOADOUT_TYPE}\n",
            f"\t{TARGET_WRONG_ROCKETS}\n",
            f"\t{TARGET_WRONG_GUIDED}\n",
            f"\t{TARGET_WRONG_HYDRA}\n",
            f"\t{TARGET_WRONG_HELLFIRE}\n",
            f"\t{TARGET_WRONG_SIBLING_LOADOUT}\n",
            f"\t{TARGET_WRONG_INT}\n",
            ...
            leftover_primary,
            leftover_guided,
            "\tESkyguardGunshipWeapon StartingStations = "
            "ESkyguardGunshipWeapon::Cannon;\n",
            "\tESkyguardLoadout StartingStation = "
            "ESkyguardLoadout::Balanced;\n",
            "\tfloat StartingStation = " + forty + ";\n",
            "\tfloat StartingStation = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for region in uproperty_wraps:
            self.assertTrue(has_declaration(region, LOCKED_DECL), region)
            compact = collapsed(region)
            self.assertIn("UPROPERTY", compact)
            self.assertNotIn("UPROPERTY", LOCKED_DECL)
    ...
    def test_does_not_parse_leftover_enum_feel_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_FEEL, header)
        self.assertNotIn(STOP_BEFORE_FEEL, section)
        self.assertNotIn(STOP_BEFORE_FEEL, leaked)
        self.assertIn(STOP_BEFORE_PATROL, header)
        self.assertNotIn(STOP_BEFORE_PATROL, section)
        self.assertNotIn(STOP_BEFORE_PATROL, leaked)
        self.assertIn(STOP_BEFORE_FROM_SLOT, header)
        self.assertNotIn(STOP_BEFORE_FROM_SLOT, section)
        self.assertNotIn(STOP_BEFORE_DISPLAY, section)
        self.assertNotIn(LEFTOVER_APACHE_CLASS, section)
        self.assertNotIn(LEFTOVER_APACHE_CLASS, leaked)
        self.assertNotIn(STOP_BEFORE_DEBRIEF_SNAP, header)
        self.assertNotIn(STOP_BEFORE_DEBRIEF_SNAP, section)
        self.assertNotIn(STOP_BEFORE_HUD_SNAP, header)
        self.assertNotIn(STOP_BEFORE_HUD_SNAP, section)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for leftover in leftover_audio_event_enum_tokens():
            ...
            self.assertNotIn(leftover, leaked)
            self.assertNotIn(leftover, header)
        self.assertNotIn("UMETA(DisplayName = \"30mm Cannon\")", section)
        self.assertNotIn("UMETA(DisplayName = \"Rocket Pods\")", section)
        self.assertNotIn("UMETA(DisplayName = \"Guided Missile\")", section)
        self.assertNotIn("UMETA(DisplayName = \"Balanced\")", section)
    ...
    def test_harbor_40_80_tokens_fail_closed_if_present(self) -> None:
        ...
        self.assertNotIn("Incoming" + "Radar", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec StartingStation field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "starting_station_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_THEATER_KIT_WEATHER, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SELECTED, LOCKED_SCRIPTS)

    def test_leftover_analog_and_isolated_siblings_stay_locked(self) -> None:
        leftovers = (
            LEFTOVER_LOADOUT_SPEC_DEFAULTS,
            LEFTOVER_RESOLVE_LOADOUT,
            LEFTOVER_LOADOUT_DISPLAY_NAME,
            ...
            LEFTOVER_CPG_DEBRIEF_DEFAULTS,
            LEFTOVER_CPG_DEBRIEF_SELECTED,
        ) + leftover_cpg_debrief_isolated_scripts() + leftover_mesh_bind_slot_scripts()
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("FSkyguardCpgHudSnapshot", locked_only)
        ...
        self.assertNotIn("FSkyguardMissionDefinition", locked_only)
