...
# Mission06 IntegrationReadiness bSortiePresentationReady
...
# NOT leftover Mission06 director SortiePresentation
# FIELD #984 / branch cursor/mission06-
...
# drafts #674-#1004. Do not reopen leftover
...
class Mission06SortiePresentationReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 bSortiePresentationReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission06 bSortiePresentationReady contains "
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        for token in LEFTOVER_MISSION06_OBJECTIVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION06_WAVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION06_PROTECTED_TARGETS_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
