# THIS IS leftover-safe ASkyguardGunner BaseDamage.
...
# then `float BaseDamage = 22.0f;` (HAS initializer).
# Do NOT claim leftover ADSFov / MouseSensitivity /
# LookPitchMax #1619 / LookYawLimit #1618 / LookPitchMin
# #1617 / leftover in-flight HipFov / leftover in-flight
# TraceRange as this slot.
# Stop BEFORE sibling RecoilPitch. Do not claim RecoilPitch
# leftover analog apache-cpg-feel 0.92f, skip forever.
# LOCKED_DECL initializer is 22.0f, NOT leftover Harbor
# 40 / 80.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Combat`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS `= 22.0f`.
# This is a combat damage float, not a CPG mesh/text slot
...
# Stop BEFORE sibling RecoilPitch as a claimed slot.
...
# LOCKED_DECL is `float BaseDamage = 22.0f;` NOT
...
# Combat damage float fail-closed if LOCKED_DECL is
...
TARGET = "float BaseDamage = 22.0f;"
TARGET_WRONG_BARE = "float BaseDamage;"
TARGET_WRONG_INIT = "float BaseDamage = nullptr;"
TARGET_WRONG_FALSE = "float BaseDamage = false;"
TARGET_WRONG_TRUE = "float BaseDamage = true;"
TARGET_WRONG_ZERO = "float BaseDamage = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> BaseDamage;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> BaseDamage;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> BaseDamage;"
...
STOP_BEFORE_RECOIL_PITCH = "RecoilPitch"
...
BASE_DAMAGE_DECL_RE = re.compile(
    r"float\s+BaseDamage\s*=\s*22\.0f\s*;"
)
BASE_DAMAGE_BARE_RE = re.compile(
    r"float\s+BaseDamage\s*;"
)
BASE_DAMAGE_WRONG_INIT_RE = re.compile(
    r"float\s+BaseDamage\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float BaseDamage = 22.0f;`.
    ...
    # Harbor 40 / 80, leftover ADSFov / HipFov /
    # TraceRange, or a bare BaseDamage without
    # the 22.0f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if BASE_DAMAGE_DECL_RE.search(compact) is None:
        return False
    if BASE_DAMAGE_BARE_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+ADSFov\b", compact):
        return False
    if re.search(r"float\s+TraceRange\b", compact):
        return False
    if re.search(r"float\s+BaseDamage\s*=\s*" + "40" + r"\.f\b", compact):
        return False
    if re.search(r"float\s+BaseDamage\s*=\s*" + "80" + r"\.f\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_RECOIL_PITCH:
            break
        if ident != "BaseDamage":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_RECOIL_PITCH):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_RECOIL_PITCH}"
        )
...
class GunnerBaseDamageFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BaseDamage"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_RECOIL_PITCH), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_RECOIL_PITCH),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_ADS_FOV), body)
        self.assertFalse(has_identifier(section, SIBLING_ADS_FOV), section)
        self.assertTrue(has_identifier(body, SIBLING_TRACE_RANGE), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BaseDamage"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("BaseDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_base_damage_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float BaseDamage = 22.0f;", compact_origin)
        self.assertNotIn("BaseDamage = 160.f", compact_origin)
    ...
    def test_base_damage_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float BaseDamage = 22.0f;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_LOOK_PITCH_MIN}\n",
            f"\t{TARGET_WRONG_LOOK_PITCH_MAX}\n",
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat BaseDamages = 22.0f;\n",
            "\tint32 BaseDamage = 22;\n",
            "\tbool BaseDamage = true;\n",
            "\tfloat BaseDamage = " + forty + ";\n",
            "\tfloat BaseDamage = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_hip_fov_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "BaseDamage"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_RECOIL_PITCH))
        self.assertFalse(has_identifier(section, STOP_BEFORE_RECOIL_PITCH))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "BaseDamage"))
        self.assertTrue(has_identifier(section, "BaseDamage"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_RECOIL_PITCH))
        self.assertFalse(has_identifier(section, STOP_BEFORE_RECOIL_PITCH))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner BaseDamage field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_base_damage_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
