# THIS IS leftover-safe FSkyguardCpgHudSnapshot WeaponLine.
...
# `FString WeaponLine;` with NO in-struct initializer.
# Fail-closed if WeaponLine is missing or renamed, if the
...
# Do NOT contract sibling fields RangeLine / ThreatLine /
...
# retarget: type is FString, identifier is WeaponLine,
...
TARGET = "FString WeaponLine;"
TARGET_WRONG_EQ_EMPTY = 'FString WeaponLine = TEXT("");'
TARGET_WRONG_EQ_NONE = "FString WeaponLine = NAME_None;"
TARGET_WRONG_FALSE = "FString WeaponLine = false;"
TARGET_WRONG_TRUE = "FString WeaponLine = true;"
TARGET_WRONG_ZERO = "FString WeaponLine = 0.f;"
TARGET_WRONG_FLOAT = "float WeaponLine;"
TARGET_WRONG_HEALTH = "float WeaponLine = 160.f;"
TARGET_WRONG_BOOL = "bool WeaponLine;"
TARGET_WRONG_INT = "int32 WeaponLine;"
TARGET_WRONG_FNAME = "FName WeaponLine;"
TARGET_WRONG_TCHAR = 'const TCHAR* WeaponLine = TEXT("");'
...
WEAPON_LINE_BARE_RE = re.compile(r"FString\s+WeaponLine\s*;")
WEAPON_LINE_INIT_RE = re.compile(r"FString\s+WeaponLine\s*=")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def weapon_line_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*FString\s+WeaponLine\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString WeaponLine;`.
    # Do not accept `= TEXT("")` / `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` when origin/main locks
    # a bare FString. Do not accept sibling RangeLine or
    # leftover ContactMark Label / leftover debrief
    # MissionTitle. Do not accept leftover LoadoutSpec
    # FlareCount. Do not accept UPROPERTY / Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if WEAPON_LINE_INIT_RE.search(compact):
        return False
    if re.search(r"float\s+WeaponLine\b", compact):
        return False
    if re.search(r"bool\s+WeaponLine\b", compact):
        return False
    if re.search(r"int32\s+WeaponLine\b", compact):
        return False
    if re.search(r"FName\s+WeaponLine\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*WeaponLine\b", compact):
        return False
    if WEAPON_LINE_BARE_RE.search(compact) is None:
        return False
    if weapon_line_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString WeaponLine\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString WeaponLine is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FString WeaponLine\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FString WeaponLine is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "WeaponLine"):
        raise AssertionError(
            "UPROPERTY clone landed on WeaponLine; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "WeaponLine"):
            raise AssertionError(
                f"{token} clone landed on WeaponLine; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotWeaponLineFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "WeaponLine"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString WeaponLine;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("FString WeaponLine"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("=", LOCKED_DECL)
        self.assertIn("WeaponLine", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("WeaponLine", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "WeaponLine"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_weapon_line_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_range_line_does_not_satisfy_weapon_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_RANGE}\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString WeaponLine;", compact_origin)
        self.assertNotIn("WeaponLine = TEXT", compact_origin)
        self.assertNotIn("WeaponLine = NAME_None", compact_origin)
        self.assertNotIn("WeaponLine = false", compact_origin)
        self.assertNotIn("WeaponLine = true", compact_origin)
        self.assertNotIn("WeaponLine = 0.f", compact_origin)
        self.assertNotIn("WeaponLine = 160.f", compact_origin)

    def test_weapon_line_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString WeaponLine"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 WeaponLine;\n",
            "\tbool WeaponLine;\n",
            "\tfloat WeaponLine = " + forty + ";\n",
            "\tfloat WeaponLine = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    weapon_line_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(weapon_line_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot WeaponLine field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weapon_line_field_decl_contract.py"
        ))
