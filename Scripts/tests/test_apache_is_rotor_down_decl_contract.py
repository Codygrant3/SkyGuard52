...
# or lock the IsRotorDown body in the .cpp.
...
# IsCanopyGlassCracked, AreEnginesDown,
# IsChinTurretDown, GetSensorQuality, and remaining
# unused neighbors (IsSystemDown,
# FindNearestLiveSystem, GetChinSlewScale,
...
# #107–#417, leftover IsChinTurretDown sibling in
# this wave, leftover GetSensorQuality sibling in
# this wave, leftover AreEnginesDown sibling, and
...
# IsRotorDown declaration contract on
...
# GetForwardSpeed, ApplyDamage, GetDamageFraction,
# IsCanopyGlassCracked, AreEnginesDown,
# IsChinTurretDown, GetSensorQuality, ApplySystemHit,
# ApplyHit,
...
# sibling in this wave, leftover GetSensorQuality
# sibling in this wave, and dirty workspace paths.
LOCKED = {
...
# IsRotorDown return type is not a leftover
# roster value lock.
LEFTOVER_NOT_LOCKED = (
    "ESkyguardApacheSystem",
...
# .cpp IsRotorDown body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::IsRotorDown",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheIsRotorDownDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, IS_ROTOR_DOWN), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_ROTOR_DOWN)
        self.assertIn("IsRotorDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, IS_ROTOR_DOWN))

    def test_missing_is_rotor_down_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_ROTOR_DOWN)
        self.assertIn("IsRotorDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_ROTOR_DOWN)
        self.assertIn("IsRotorDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, IS_ROTOR_DOWN), section)
        self.assertNotIn("BlueprintPure", IS_ROTOR_DOWN)
        self.assertNotIn("UFUNCTION", IS_ROTOR_DOWN)
        self.assertNotIn("Category", IS_ROTOR_DOWN)
        self.assertNotIn("BlueprintCallable", IS_ROTOR_DOWN)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_ROTOR_DOWN)
        self.assertIn("IsRotorDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool IsRotorDown const;\n"
        wrong_return_void = "\tvoid IsRotorDown() const;\n"
        wrong_return_float = "\tfloat IsRotorDown() const;\n"
        wrong_return_int = "\tint32 IsRotorDown() const;\n"
        missing_const = "\tbool IsRotorDown();\n"
        added_arg = "\tbool IsRotorDown(float Extra) const;\n"
        leftover_canopy = (
            "\tbool IsCanopyGlassCracked() const "
            "{ return bCanopyGlassCracked; }\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_ROTOR_DOWN)
            self.assertIn("IsRotorDown", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_rotor_down_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, IS_ROTOR_DOWN),
            IS_ROTOR_DOWN,
        )
        self.assertTrue(has_declaration(section, IS_ROTOR_DOWN))
        self.assertEqual(declaration_count(section, IS_ROTOR_DOWN), 1)
        self.assertTrue(
            IS_ROTOR_DOWN.startswith("bool "),
            IS_ROTOR_DOWN,
        )
        self.assertTrue(IS_ROTOR_DOWN.endswith(";"), IS_ROTOR_DOWN)
        self.assertIn("IsRotorDown()", IS_ROTOR_DOWN)
        self.assertIn(" const", IS_ROTOR_DOWN)
        self.assertNotIn("INDEX_NONE", IS_ROTOR_DOWN)
        self.assertNotIn("{", IS_ROTOR_DOWN)
        self.assertNotIn("}", IS_ROTOR_DOWN)
        self.assertNotIn("return ", IS_ROTOR_DOWN)
        self.assertNotIn("IsCanopyGlassCracked", IS_ROTOR_DOWN)
        self.assertNotIn("IsChinTurretDown", IS_ROTOR_DOWN)
        self.assertNotIn("AreEnginesDown", IS_ROTOR_DOWN)
        self.assertNotIn("IsSystemDown", IS_ROTOR_DOWN)
        self.assertNotIn("GetEnginePowerScale", IS_ROTOR_DOWN)
        self.assertNotIn("GetDamageFraction", IS_ROTOR_DOWN)
        self.assertNotIn("ApplyDamage", IS_ROTOR_DOWN)
        self.assertNotIn("GetForwardSpeed", IS_ROTOR_DOWN)
        self.assertNotIn("FaceWorldLocation", IS_ROTOR_DOWN)
        self.assertNotIn("ApplySystemHit", IS_ROTOR_DOWN)
        self.assertNotIn("ApplyHit", IS_ROTOR_DOWN)
        self.assertNotIn("IsSensorLive", IS_ROTOR_DOWN)
        self.assertNotIn("IsThermalAvailable", IS_ROTOR_DOWN)
        self.assertNotIn("IsSensorViewActive", IS_ROTOR_DOWN)
        self.assertNotIn("GetChinMuzzleLocation", IS_ROTOR_DOWN)
        self.assertNotIn("GetChinTurret", IS_ROTOR_DOWN)
        self.assertNotIn("GetRotorPowerScale", IS_ROTOR_DOWN)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, IS_ROTOR_DOWN),
                section,
            )
            self.assertEqual(
                require_declaration(section, IS_ROTOR_DOWN),
            ...
            self.assertEqual(
                declaration_count(section, IS_ROTOR_DOWN),
                1,
            )
        one_line = f"{{\npublic:\n\t{IS_ROTOR_DOWN}\n}}\n"
        self.assertTrue(has_declaration(one_line, IS_ROTOR_DOWN))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, IS_ROTOR_DOWN), section)
        self.assertEqual(
            require_declaration(section, IS_ROTOR_DOWN),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{IS_ROTOR_DOWN}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", IS_ROTOR_DOWN)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", IS_ROTOR_DOWN)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_rotor_down_cpp_body(self) -> None:
        locked_only = f"{IS_ROTOR_DOWN}\n"
        self.assertNotIn("{", IS_ROTOR_DOWN)
        self.assertNotIn("}", IS_ROTOR_DOWN)
        self.assertNotIn("return ", IS_ROTOR_DOWN)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", IS_ROTOR_DOWN)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", IS_ROTOR_DOWN)
        self.assertNotIn("return true", IS_ROTOR_DOWN)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{IS_ROTOR_DOWN}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_ROTOR_DOWN)
        self.assertNotIn("GetGunnerMount", IS_ROTOR_DOWN)
        self.assertNotIn("GetEyeMount", IS_ROTOR_DOWN)
        self.assertNotIn("GetWeaponMount", IS_ROTOR_DOWN)
        self.assertNotIn("GetChinTurret", IS_ROTOR_DOWN)
        self.assertNotIn("GetPilotMount", IS_ROTOR_DOWN)
        self.assertNotIn("GetSensorTurret", IS_ROTOR_DOWN)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{IS_ROTOR_DOWN}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_ROTOR_DOWN)
        self.assertNotIn("GetChinMuzzleLocation", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_aim_chin_turret(self) -> None:
        ...
        for neighbor in AIM_CHIN_TURRET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_ROTOR_DOWN)
        self.assertNotIn("AimChinTurret", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_set_rotor_power(self) -> None:
        ...
        for neighbor in SET_ROTOR_POWER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_ROTOR_DOWN)
        self.assertNotIn("SetRotorPower", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_issue_pilot_command(self) -> None:
        ...
        for neighbor in ISSUE_PILOT_COMMAND_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_ROTOR_DOWN)
        self.assertNotIn("IssuePilotCommand", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_get_pilot_helpers(self) -> None:
        ...
        for neighbor in GET_PILOT_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_ROTOR_DOWN)
        self.assertNotIn("GetPilotCommand", IS_ROTOR_DOWN)
        self.assertNotIn("GetPilotConfirmationsIssued", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_ROTOR_DOWN)
        self.assertNotIn("SetOrbitFocus", IS_ROTOR_DOWN)
        self.assertNotIn("FaceWorldLocation", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_ROTOR_DOWN)
        self.assertNotIn("SetSensorView", IS_ROTOR_DOWN)
        self.assertNotIn("SetFirstPersonInterior", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_ROTOR_DOWN)
        self.assertNotIn("ESkyguardApacheSystem", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_leftover_direct_flight_input_sibling(
        ...
        self.assertNotIn("SetDirectFlightInput", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_leftover_get_sensor_quality_sibling(
        ...
        self.assertNotIn("GetSensorQuality", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_ROTOR_DOWN)
        self.assertNotIn("Pursuit", IS_ROTOR_DOWN)
        self.assertNotIn("OrbitLeft", IS_ROTOR_DOWN)
        self.assertNotIn("AttackRun", IS_ROTOR_DOWN)
        self.assertNotIn("FaceTarget", IS_ROTOR_DOWN)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_ROTOR_DOWN)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", IS_ROTOR_DOWN)
        self.assertNotIn("USkyguardBriefingWidget", IS_ROTOR_DOWN)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            IS_ROTOR_DOWN,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_ROTOR_DOWN)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", IS_ROTOR_DOWN)
        self.assertNotIn("ESkyguardMissionSkylineStyle", IS_ROTOR_DOWN)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_ROTOR_DOWN}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_ROTOR_DOWN),
            IS_ROTOR_DOWN,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache IsRotorDown contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, IS_ROTOR_DOWN.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache IsRotorDown contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_ROTOR_DOWN)

    def test_contract_is_is_rotor_down_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", IS_ROTOR_DOWN)
        self.assertNotIn("{", IS_ROTOR_DOWN)
        self.assertTrue(IS_ROTOR_DOWN.startswith("bool "))
        self.assertTrue(IS_ROTOR_DOWN.endswith(";"))
        self.assertIn(" const", IS_ROTOR_DOWN)
        self.assertIn(UFUNCTION_APACHE, section)
