# THIS IS leftover-safe ASkyguardMission01EnvironmentDirector
...
# then `bool bUseAuthoredLandscapeSurface = false;`
# (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Mission01|Environment|Landscape` (NO spaces
# around =). EditInstanceOnly IS present. BlueprintReadOnly IS
# present. Default IS `= false`.
# THIS IS the director ACTOR bool, not leftover
# FSkyguardMission01EnvironmentReadiness struct member
# `bool bAuthoredLandscapeSurfaceExposed = false;`
# (VisibleAnywhere, BlueprintReadOnly, no Category) on leftover
# branch cursor/mission01-environment-readiness-authored-
# landscape-surface-exposed-field-decl-contract-c332.
# IMPORTANT WRAP DIFFERENCE vs the two PCG sibling bools:
# this slot is BlueprintReadOnly (NOT BlueprintReadWrite) and
# Category Landscape (NOT PCG). Fail-closed if wrap is
# BlueprintReadWrite or Category ...|PCG or Category ...|Visibility.
# Fail-closed if wrap is VisibleAnywhere or EditAnywhere.
# Clone wrap was EditAnywhere BlueprintReadWrite Category =
# "Skyguard|MissionMap" wrapping FName AssemblyRevision WITH
# initializer — RETARGET hard: EditInstanceOnly (not EditAnywhere),
# BlueprintReadOnly (not BlueprintReadWrite),
# Category="Skyguard|Mission01|Environment|Landscape"
# (NO spaces around =).
# LOCKED_DECL is `bool bUseAuthoredLandscapeSurface = false;`
# NOT `FName AssemblyRevision`, NOT `FName WeatherIdentity`,
# NOT leftover readiness `bool bAuthoredLandscapeSurfaceExposed = false;`,
# NOT leftover `bool bLicensedVegetationLibraryApproved = false;`,
# NOT leftover `bool bAllowAuthoredPCGGeneration = false;`,
# NOT leftover `bool bEnableCoastalHazeTransition = true;`.
# Fail-closed if this test still asserts FName WeatherIdentity /
# Category="Skyguard|Theater" / 160.f / FName AssemblyRevision /
# LookYawLimit = 95.f / TArray RoutePoints /
# bool bAuthoredLandscapeSurfaceExposed = false; AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is Skyguard|Combat or
# Skyguard|Theater or Skyguard|MissionMap or ...|PCG or
# ...|Visibility or Category="Skyguard" without
# |Mission01|Environment|Landscape.
# Parse CLASS `ASkyguardMission01EnvironmentDirector` public
# UPROPERTY fields ONLY after `class ASkyguardMission01EnvironmentDirector`.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `private:`.
# Stop BEFORE sibling Readiness as a claimed slot.
# Do NOT parse leftover struct
# `FSkyguardMission01EnvironmentReadiness`.
# Do NOT claim leftover Mission01 Root / OceanTiles / BeachTiles /
# LandTiles / RouteExclusion / LandScatterBounds /
# InlandVegetationPCG / ProductionLandscape / AuthoredPCGGraph /
# bLicensedVegetationLibraryApproved / bAllowAuthoredPCGGeneration /
# RouteLengthCm / DistrictLengthCm / Readiness / CoastalHaze* /
# LandMaterial / OceanMaterial / BeachMaterial as this slot.
# Do NOT claim leftover Mission01-10 director/readiness same-name
# drafts. Do NOT reopen leftover #536-#1636.
# Skip banned Ig+laMuzzle, all Ig+la* fields, Ri+fleMesh /
# leftover analog apache-cpg-feel FireRate 12.0f / RecoilPitch
# 0.92f / RocketSalvo* / SelectedGunshipWeapon / LoadoutSpec
# Apache ammo.
...
TARGET = "bool bUseAuthoredLandscapeSurface = false;"
TARGET_WRONG_BARE = "bool bUseAuthoredLandscapeSurface;"
TARGET_WRONG_INIT = "bool bUseAuthoredLandscapeSurface = nullptr;"
TARGET_WRONG_FALSE = "bool bUseAuthoredLandscapeSurface = true;"
TARGET_WRONG_TRUE = "bool bUseAuthoredLandscapeSurface = true;"
TARGET_WRONG_ZERO = "bool bUseAuthoredLandscapeSurface = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> bUseAuthoredLandscapeSurface;"
TARGET_WRONG_EXPOSED = "bool bAuthoredLandscapeSurfaceExposed = false;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> bUseAuthoredLandscapeSurface;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> bUseAuthoredLandscapeSurface;"
...
LOCKED_DECL = TARGET
UPROPERTY_INSTANCE_READONLY = (
    'UPROPERTY(EditInstanceOnly, BlueprintReadOnly, '
    'Category="Skyguard|Mission01|Environment|Landscape")'
)
UPROPERTY_EDIT_READWRITE = UPROPERTY_INSTANCE_READONLY
UPROPERTY_LEFTOVER_PCG = (
    'UPROPERTY(EditInstanceOnly, BlueprintReadWrite, '
    'Category="Skyguard|Mission01|Environment|PCG")'
)
UPROPERTY_LEFTOVER_VISIBILITY = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Mission01|Environment|Visibility")'
)
UPROPERTY_LEFTOVER_READINESS = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
)
...
STOP_BEFORE_SKYLINE_STYLE = "Readiness"
STOP_BEFORE_READINESS = "Readiness"
...
LEFTOVER_OBJECTIVE_ANCHORS = (
    "Scripts/tests/test_mission_map_objective_anchors"
    "_field_decl_contract.py"
)
LEFTOVER_LANDMARK_ANCHORS = (
    "Scripts/tests/test_mission_map_landmark_anchors"
    "_field_decl_contract.py"
)
LEFTOVER_READINESS_EXPOSED = (
    "Scripts/tests/test_mission01_environment_readiness_"
    "authored_landscape_surface_exposed_field_decl_contract.py"
)
LEFTOVER_LICENSED_VEG = (
    "Scripts/tests/test_mission01_licensed_vegetation_library_"
    "approved_field_decl_contract.py"
)
LEFTOVER_ALLOW_PCG = (
...
USE_AUTHORED_DECL_RE = re.compile(
    r"bool\s+bUseAuthoredLandscapeSurface\s*=\s*false\s*;"
)
USE_AUTHORED_BARE_RE = re.compile(
    r"bool\s+bUseAuthoredLandscapeSurface\s*;"
)
EXPOSED_DECL_RE = re.compile(
    r"bool\s+bAuthoredLandscapeSurfaceExposed\s*=\s*false\s*;"
)
LICENSED_DECL_RE = re.compile(
    r"bool\s+bLicensedVegetationLibraryApproved\s*=\s*false\s*;"
)
ALLOW_PCG_DECL_RE = re.compile(
    r"bool\s+bAllowAuthoredPCGGeneration\s*=\s*false\s*;"
)
HAZE_DECL_RE = re.compile(
    r"bool\s+bEnableCoastalHazeTransition\s*=\s*true\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover float LookYawLimit = 95.f, leftover `= 160.f`,
    # leftover analog apache-cpg-feel FireRate / RecoilPitch,
    # leftover TArray RoutePoints, leftover readiness
    # bAuthoredLandscapeSurfaceExposed, leftover PCG sibling
    # bools, leftover Visibility haze bool, leftover CPG
    # mesh/text, leftover LoadoutSpec magazine fields,
    # leftover Harbor 40 / 80, or a bare
    # bUseAuthoredLandscapeSurface without `= false`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if USE_AUTHORED_DECL_RE.search(compact) is None:
        return False
    if USE_AUTHORED_BARE_RE.search(compact) and "= false" not in compact:
        return False
    if EXPOSED_DECL_RE.search(compact):
        return False
    if LICENSED_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "bUseAuthoredLandscapeSurface":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bUseAuthoredLandscapeSurface\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bUseAuthoredLandscapeSurface "
...
class Mission01UseAuthoredLandscapeSurfaceFieldDeclContractTests(unittest.TestCase):
    def test_mission01_environment_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bUseAuthoredLandscapeSurface"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_READINESS), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_READINESS),
            section,
        )
        self.assertTrue(has_identifier(body, "ProductionLandscape"), body)
        self.assertFalse(has_identifier(section, "ProductionLandscape"), section)
        self.assertTrue(has_identifier(body, "AuthoredPCGGraph"), body)
        self.assertFalse(
            has_identifier(section, "AuthoredPCGGraph"),
            section,
        )
        self.assertTrue(has_identifier(body, "LandTiles"), body)
        self.assertFalse(has_identifier(section, "LandTiles"), section)
        ...
        self.assertTrue(
            has_identifier(body, "bLicensedVegetationLibraryApproved"),
            body,
        )
        self.assertFalse(
            has_identifier(section, "bLicensedVegetationLibraryApproved"),
            section,
        )
        self.assertTrue(
            has_identifier(body, "bAllowAuthoredPCGGeneration"),
            body,
        )
        self.assertFalse(
            has_identifier(section, "bAllowAuthoredPCGGeneration"),
            section,
        )
        self.assertTrue(
            has_identifier(body, "bEnableCoastalHazeTransition"),
            body,
        )
        self.assertFalse(
            has_identifier(section, "bEnableCoastalHazeTransition"),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bUseAuthoredLandscapeSurface"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("bUseAuthoredLandscapeSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_use_authored_landscape_surface_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn("EditInstanceOnly", section)
        self.assertIn("BlueprintReadOnly", section)
        self.assertIn('Category="Skyguard|Mission01|Environment|Landscape"', section)
        ...
        self.assertNotIn("|PCG", section)
        self.assertNotIn("|Visibility", section)
        self.assertIsNone(
            re.search(r'Category="Skyguard"(?!\|)', section)
        )
        self.assertIn("EditInstanceOnly", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertIn("EditInstanceOnly", specifiers)
        self.assertIn("BlueprintReadOnly", specifiers)
        self.assertIn('Category="Skyguard|Mission01|Environment|Landscape"', specifiers)
    ...
    def test_use_authored_landscape_surface_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bUseAuthoredLandscapeSurface"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotIn("AssemblyRevision", LOCKED_DECL)
        self.assertNotIn("bAuthoredLandscapeSurfaceExposed", LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_EXPOSED)
        ...
        self.assertNotIn("|PCG", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("|Visibility", UPROPERTY_EDIT_READWRITE)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, UPROPERTY_LEFTOVER_PCG)
        self.assertNotEqual(
            UPROPERTY_EDIT_READWRITE,
            UPROPERTY_LEFTOVER_VISIBILITY,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool bUseAuthoredLandscapeSurfaces = false;\n",
            "\tint32 bUseAuthoredLandscapeSurface = 1;\n",
            "\tbool bUseAuthoredLandscapeSurface = true;\n",
            f"\t{TARGET_WRONG_EXPOSED}\n",
            f"\t{TARGET_WRONG_LICENSED}\n",
            f"\t{TARGET_WRONG_ALLOW_PCG}\n",
            f"\t{TARGET_WRONG_HAZE}\n",
            f"\t{TARGET_WRONG_ROUTE_POINTS}\n",
            "\tbool bUseAuthoredLandscapeSurface = " + forty + ";\n",
            "\tbool bUseAuthoredLandscapeSurface = " + eighty + ";\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission01|Environment|Landscape"', origin)
        ...
        self.assertIn("EditInstanceOnly", origin)
        ...
        self.assertNotIn("EditInstanceOnly", dropped)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tbool\n\tbUseAuthoredLandscapeSurface = false;\n",
            "\tbool   bUseAuthoredLandscapeSurface = false;\n",
            "\tbool\tbUseAuthoredLandscapeSurface = false;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_readiness_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "bUseAuthoredLandscapeSurface"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "bUseAuthoredLandscapeSurface"))
        self.assertTrue(has_identifier(section, "bUseAuthoredLandscapeSurface"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_READINESS))
        self.assertFalse(has_identifier(section, STOP_BEFORE_READINESS))
        self.assertTrue(has_identifier(leaked, "ProductionLandscape"))
        self.assertFalse(has_identifier(section, "ProductionLandscape"))
        self.assertTrue(has_identifier(leaked, "AuthoredPCGGraph"))
        self.assertFalse(has_identifier(section, "AuthoredPCGGraph"))
        self.assertTrue(has_identifier(leaked, SIBLING_READINESS))
        self.assertFalse(has_identifier(section, SIBLING_READINESS))
        self.assertFalse(has_identifier(section, "AssemblyRevision"))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardMission01EnvironmentDirector.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardMission01EnvironmentDirector.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 use-authored-landscape-surface field decl "
                f"contract contains {banned}; declaration is Apache CPG 30 mm / "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_use_authored_landscape_surface_field_decl_contract.py"
        ))
        self.assertIn(LEFTOVER_ASSEMBLY_REVISION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTE_POINTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_OBJECTIVE_ANCHORS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LANDMARK_ANCHORS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_READINESS_EXPOSED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LICENSED_VEG, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ALLOW_PCG, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HAZE_FADE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HAZE_DENSITY, LOCKED_SCRIPTS)
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
