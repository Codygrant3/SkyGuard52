...
# or lock the GetEnginePowerScale body in the .cpp.
...
# GetEnginePowerScale declaration contract on
...
# this wave, and dirty workspace paths.
LOCKED = {
...
# GetEnginePowerScale return type is not a leftover
# roster value lock.
LEFTOVER_NOT_LOCKED = (
    "ESkyguardApacheSystem",
...
# .cpp GetEnginePowerScale body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::GetEnginePowerScale",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheGetEnginePowerScaleDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_ENGINE_POWER_SCALE), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_ENGINE_POWER_SCALE)
        self.assertIn("GetEnginePowerScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_ENGINE_POWER_SCALE))

    def test_missing_get_engine_power_scale_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_ENGINE_POWER_SCALE)
        self.assertIn("GetEnginePowerScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_ENGINE_POWER_SCALE)
        self.assertIn("GetEnginePowerScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_ENGINE_POWER_SCALE), section)
        self.assertNotIn("BlueprintPure", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("UFUNCTION", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("Category", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("BlueprintCallable", GET_ENGINE_POWER_SCALE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_ENGINE_POWER_SCALE)
        self.assertIn("GetEnginePowerScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tfloat GetEnginePowerScale const;\n"
        wrong_return_void = "\tvoid GetEnginePowerScale() const;\n"
        wrong_return_bool = "\tbool GetEnginePowerScale() const;\n"
        wrong_return_int = "\tint32 GetEnginePowerScale() const;\n"
        missing_const = "\tfloat GetEnginePowerScale();\n"
        added_arg = "\tfloat GetEnginePowerScale(float Extra) const;\n"
        leftover_canopy = (
            "\tbool IsCanopyGlassCracked() const "
            "{ return bCanopyGlassCracked; }\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_ENGINE_POWER_SCALE)
            self.assertIn("GetEnginePowerScale", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_engine_power_scale_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_ENGINE_POWER_SCALE),
            GET_ENGINE_POWER_SCALE,
        )
        self.assertTrue(has_declaration(section, GET_ENGINE_POWER_SCALE))
        self.assertEqual(declaration_count(section, GET_ENGINE_POWER_SCALE), 1)
        self.assertTrue(
            GET_ENGINE_POWER_SCALE.startswith("float "),
            GET_ENGINE_POWER_SCALE,
        )
        self.assertTrue(GET_ENGINE_POWER_SCALE.endswith(";"), GET_ENGINE_POWER_SCALE)
        self.assertIn("GetEnginePowerScale()", GET_ENGINE_POWER_SCALE)
        self.assertIn(" const", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("INDEX_NONE", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("{", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("}", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("return ", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IsCanopyGlassCracked", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("AreEnginesDown", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IsChinTurretDown", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IsRotorDown", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IsSystemDown", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetSensorQuality", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetDamageFraction", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("ApplyDamage", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetForwardSpeed", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("FaceWorldLocation", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("ApplySystemHit", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("ApplyHit", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IsSensorLive", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IsThermalAvailable", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IsSensorViewActive", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetChinMuzzleLocation", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetChinTurret", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetRotorPowerScale", GET_ENGINE_POWER_SCALE)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tfloat\n"
            "\tGetEnginePowerScale() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tfloat GetEnginePowerScale(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, GET_ENGINE_POWER_SCALE),
                section,
            )
            self.assertEqual(
                require_declaration(section, GET_ENGINE_POWER_SCALE),
            ...
            self.assertEqual(
                declaration_count(section, GET_ENGINE_POWER_SCALE),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_ENGINE_POWER_SCALE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_ENGINE_POWER_SCALE))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_ENGINE_POWER_SCALE), section)
        self.assertEqual(
            require_declaration(section, GET_ENGINE_POWER_SCALE),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_ENGINE_POWER_SCALE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_ENGINE_POWER_SCALE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_engine_power_scale_cpp_body(self) -> None:
        locked_only = f"{GET_ENGINE_POWER_SCALE}\n"
        self.assertNotIn("{", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("}", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("return ", GET_ENGINE_POWER_SCALE)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("return true", GET_ENGINE_POWER_SCALE)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{GET_ENGINE_POWER_SCALE}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetGunnerMount", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetEyeMount", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetWeaponMount", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetChinTurret", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetPilotMount", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetSensorTurret", GET_ENGINE_POWER_SCALE)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_ENGINE_POWER_SCALE}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetChinMuzzleLocation", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_aim_chin_turret(self) -> None:
        ...
        for neighbor in AIM_CHIN_TURRET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("AimChinTurret", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_set_rotor_power(self) -> None:
        ...
        for neighbor in SET_ROTOR_POWER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("SetRotorPower", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_issue_pilot_command(self) -> None:
        ...
        for neighbor in ISSUE_PILOT_COMMAND_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("IssuePilotCommand", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_get_pilot_helpers(self) -> None:
        ...
        for neighbor in GET_PILOT_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetPilotCommand", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("GetPilotConfirmationsIssued", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("SetOrbitFocus", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("FaceWorldLocation", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("SetSensorView", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("SetFirstPersonInterior", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("ESkyguardApacheSystem", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_leftover_direct_flight_input_sibling(
        ...
        self.assertNotIn("SetDirectFlightInput", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_leftover_get_chin_slew_scale_sibling(
        ...
        self.assertNotIn("GetChinSlewScale", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_leftover_get_chin_fire_scale_sibling(
        ...
        self.assertNotIn("GetChinFireScale", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ENGINE_POWER_SCALE)
        self.assertNotIn("Pursuit", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("OrbitLeft", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("AttackRun", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("FaceTarget", GET_ENGINE_POWER_SCALE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_ENGINE_POWER_SCALE)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("USkyguardBriefingWidget", GET_ENGINE_POWER_SCALE)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            GET_ENGINE_POWER_SCALE,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_ENGINE_POWER_SCALE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("ESkyguardMissionSkylineStyle", GET_ENGINE_POWER_SCALE)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_ENGINE_POWER_SCALE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_ENGINE_POWER_SCALE),
            GET_ENGINE_POWER_SCALE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GetEnginePowerScale contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_ENGINE_POWER_SCALE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache GetEnginePowerScale contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_ENGINE_POWER_SCALE)

    def test_contract_is_get_engine_power_scale_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", GET_ENGINE_POWER_SCALE)
        self.assertNotIn("{", GET_ENGINE_POWER_SCALE)
        self.assertTrue(GET_ENGINE_POWER_SCALE.startswith("float "))
        self.assertTrue(GET_ENGINE_POWER_SCALE.endswith(";"))
        self.assertIn(" const", GET_ENGINE_POWER_SCALE)
        self.assertIn(UFUNCTION_APACHE, section)
