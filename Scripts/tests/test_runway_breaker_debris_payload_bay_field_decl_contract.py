...
# INDEX_NONE or lock DebrisPayloadBay construction
# in the .cpp. This is a FIELD contract on
# ASkyguardRunwayBreakerBoss, not leftover
# USkyguardBossWeakPointComponent member
# contract. Keep DebrisPayloadBay distinct from
# sibling RunwayBreaker fields RunwayRack,
# HangarRack, HeatManifold, PortEngine,
# DebrisPortWing, and DebrisEngine. Those
# siblings must not satisfy the primary lock.
# Keep RunwayBreaker DebrisEngine / PortEngine
# distinct from leftover RoadHunter / Breakwater
# Engine and DebrisEngine (different classes;
# do not lock those here). origin/main is a
# one-line field
# (`TObjectPtr<UStaticMeshComponent>
# DebrisPayloadBay;`); accept that form and other
...
# as the primary lock. Stay off leftover
...
# isolated-test drafts #107–#556 including
...
# LastFlight / BossDrone / RoadHunter /
# Breakwater drafts, leftover
...
# Leftover RoadHunter / Breakwater fields. Same Engine /
# DebrisEngine declaration text, different class. Not the
# primary lock.
...
# DebrisPayloadBay destruction-mesh field
...
# isolated RunwayBreaker RunwayRack / HangarRack
# and RoadHunter field contracts stay
# unlocked for THIS test's primary DebrisPayloadBay lock.
# Sibling HeatManifold stays off the primary lock. Leftover
...
LEFTOVER_RADAR_GHOST_NOT_LOCKED = (
    ...
    "test_radar_ghost_signature_modulator_field_decl_contract.py",
    "test_radar_ghost_radar_receiver_field_decl_contract.py",
    "test_radar_ghost_cooling_door_field_decl_contract.py",
    "test_radar_ghost_debris_cooling_door_field_decl_contract.py",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRunwayBreakerBoss::DebrisPayloadBay",
    "SkyguardRunwayBreakerBoss.cpp",
    "SetExposed",
...
class RunwayBreakerDebrisPayloadBayFieldDeclContractTests(unittest.TestCase):
    def test_runway_breaker_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_missing_debris_payload_bay_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRunwayBreakerBoss();\n"
            ...
            f"\t{DEBRIS_PORT_EW_PANEL_FIELD}\n"
            f"\t{DEBRIS_STARBOARD_EW_PANEL_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("Category", DEBRIS_PAYLOAD_BAY_FIELD)
        ...
        self.assertNotIn("Mission09", UPROPERTY_PORT)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisPayloadBay;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisPayloadBay;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DebrisPayloadBay;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_PAYLOAD_BAY_FIELD)
            self.assertIn("DebrisPayloadBay", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_payload_bay_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD),
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_PAYLOAD_BAY_FIELD), 1)
        self.assertTrue(
            DEBRIS_PAYLOAD_BAY_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
        self.assertTrue(DEBRIS_PAYLOAD_BAY_FIELD.endswith(";"), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisCoolingDoor", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("RadarReceiver", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("TArray<", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("=", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("{", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("}", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("return ", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisCenter", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("RadarNode", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_PAYLOAD_BAY_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {DEBRIS_PAYLOAD_BAY_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DEBRIS_PAYLOAD_BAY_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DEBRIS_PAYLOAD_BAY_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_PAYLOAD_BAY_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD),
            section,
    ...
    def test_assigned_debris_payload_bay_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisPayloadBay = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_PAYLOAD_BAY_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            f"\t{DISPENSER_STARBOARD_FIELD}\n",
            f"\t{COMMAND_ANTENNA_PORT_FIELD}\n",
            f"\t{COMMAND_ANTENNA_STARBOARD_FIELD}\n",
            f"\t{DECOY_CONTROLLER_FIELD}\n",
            f"\t{ENGINE_POD_PORT_FIELD}\n",
            ...
            self.assertIn("DebrisPayloadBay", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_PAYLOAD_BAY_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_PAYLOAD_BAY_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_PAYLOAD_BAY_FIELD.startswith("UFUNCTION"),
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_PAYLOAD_BAY_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_PAYLOAD_BAY_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_payload_bay_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        self.assertNotIn("{", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("}", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("return ", DEBRIS_PAYLOAD_BAY_FIELD)
        ...
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("return true", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_PAYLOAD_BAY_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisIntakePanel", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DispenserCenter", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("IsContactIdentified", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), DEBRIS_PAYLOAD_BAY_FIELD)

    def test_debris_cooling_door_does_not_satisfy_debris_payload_bay(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PAYLOAD_BAY_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DispenserStarboard", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetManeuver", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), DEBRIS_PAYLOAD_BAY_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("IsLightningExposed", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_PAYLOAD_BAY_FIELD),
            DEBRIS_PAYLOAD_BAY_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRunwayBreakerBoss::DebrisPayloadBay", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "runway breaker DebrisPayloadBay field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_PAYLOAD_BAY_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_PAYLOAD_BAY_FIELD.lower(),
                "runway breaker DebrisPayloadBay contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_PAYLOAD_BAY_FIELD)

    def test_contract_is_debris_payload_bay_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DEBRIS_PAYLOAD_BAY_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_runway_breaker_fields(
        ...
        for token in SIBLING_RUNWAY_BREAKER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("RunwayRack", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("HangarRack", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("HeatManifold", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisPortWing", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn(
            "Scripts/tests/test_runway_breaker_runway_rack_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        for token in SIBLING_ROAD_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("LeftActuator", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("RightActuator", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardRoadHunterBoss", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn(
            "Scripts/tests/test_road_hunter_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("StarboardLatch", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("DecoyPods", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ElevatorLinkage", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_debris_payload_bay_is_not_road_hunter_or_breakwater_engine(self) -> None:
        locked_only = f"{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        self.assertNotEqual(DEBRIS_PAYLOAD_BAY_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_PAYLOAD_BAY_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_PAYLOAD_BAY_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(collapsed(DEBRIS_PAYLOAD_BAY_FIELD), collapsed(ENGINE_FIELD))
        self.assertEqual(ROAD_HUNTER_ENGINE_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertEqual(ROAD_HUNTER_ENGINE_FIELD, RADAR_GHOST_ENGINE_FIELD)
        ...
        for token in LEFTOVER_BREAKWATER_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("Engine;", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_runway_breaker_port_engine_is_not_road_hunter_engine(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PAYLOAD_BAY_FIELD))
        self.assertNotEqual(PORT_ENGINE_FIELD, DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DEBRIS_PAYLOAD_BAY_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("OpticalTracker", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("WeaponServo", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("CountermeasurePod", DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_PAYLOAD_BAY_FIELD)

    def test_debris_engine_does_not_satisfy_debris_payload_bay(self) -> None:
        leftover = f"\t{RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
    ...
    def test_port_engine_does_not_satisfy_debris_payload_bay(self) -> None:
        leftover = f"\t{PORT_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
    ...
    def test_runway_rack_and_hangar_rack_do_not_satisfy_debris_payload_bay(
        self,
    ) -> None:
        for region, name in (
            (f"\t{RUNWAY_RACK_FIELD}\n", "RunwayRack"),
    ...
    def test_heat_manifold_does_not_satisfy_debris_payload_bay(self) -> None:
        leftover = f"\t{HEAT_MANIFOLD_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertIn("DebrisPayloadBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_PAYLOAD_BAY_FIELD))
        self.assertNotEqual(HEAT_MANIFOLD_FIELD, DEBRIS_PAYLOAD_BAY_FIELD)
        self.assertNotIn("HeatManifold", DEBRIS_PAYLOAD_BAY_FIELD)

    def test_contract_does_not_relock_leftover_index_of(self) -> None:
        locked_only = f"{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        for token in LEFTOVER_INDEX_OF_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PAYLOAD_BAY_FIELD)
