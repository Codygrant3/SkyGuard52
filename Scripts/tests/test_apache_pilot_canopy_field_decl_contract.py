# THIS IS leftover-safe ASkyguardApacheAircraft PilotCanopy.
...
# then BARE `TObjectPtr<UStaticMeshComponent> PilotCanopy;`
# (no initializer).
...
# Stop BEFORE sibling EngineLeft as a claimed slot.
# Do NOT claim leftover Canopy #1559, NightVisionTurret,
# SensorTurret, SensorBall, ChinTurret, ChinHousing,
# ChinBarrel, TailRotor, MainRotor, RotorMast,
# AircraftRoot, Fuselage, Nose, SilhouetteMesh,
# EngineLeft, EngineRight, RadarDome, MaxIntegrity,
# CurrentIntegrity, HullCollider, PilotMount,
# GunnerMount, EyeMount, WeaponMount.
# PilotCanopy is NOT leftover Canopy #1559
# (`TObjectPtr<UStaticMeshComponent> Canopy;`),
# NOT leftover analog apache-is-canopy-glass-cracked
...
TARGET = "TObjectPtr<UStaticMeshComponent> PilotCanopy;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> PilotCanopy = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> PilotCanopy = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> PilotCanopy = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> PilotCanopy;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> PilotCanopy;"
...
STOP_BEFORE_ENGINE_LEFT = "EngineLeft"
...
PILOT_CANOPY_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+PilotCanopy\s*;"
)
PILOT_CANOPY_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+PilotCanopy\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover sibling Canopy / EngineLeft / Fuselage /
    # AircraftRoot / USceneComponent, or an initializer
    # on PilotCanopy as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if PILOT_CANOPY_DECL_RE.search(compact) is None:
        return False
    if PILOT_CANOPY_INIT_RE.search(compact):
        return False
    if WEATHER_IDENTITY_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"TObjectPtr<USceneComponent>\s+PilotCanopy\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_ENGINE_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_ENGINE_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_ENGINE_LEFT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+PilotCanopy\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> PilotCanopy "
...
class ApachePilotCanopyFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PilotCanopy"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENGINE_LEFT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_ENGINE_LEFT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "PilotCanopy"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("PilotCanopy", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_pilot_canopy_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> PilotCanopy;", compact_origin)
        self.assertNotIn("PilotCanopy = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_pilot_canopy_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> PilotCanopy"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> PilotCanopy;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> PilotCanopies;\n",
            "\tint32 PilotCanopy = 140;\n",
            "\tbool PilotCanopy = true;\n",
            "\tfloat PilotCanopy = " + forty + ";\n",
            "\tfloat PilotCanopy = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_engine_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_ENGINE_LEFT))
        ...
        self.assertNotIn(TARGET_WRONG_ENGINE_LEFT, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache PilotCanopy field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pilot_canopy_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
