...
# wrapping `FName GetCurrentLineId() const { return bPlaying ? CurrentLine.LineId : NAME_None; }`.
# LOCKED_DECL may be declaration-only
# `FName GetCurrentLineId() const;` if the matcher strips
# inline bodies, OR the full origin/main inline form.
# Accept declaration-only AND the origin/main inline body.
# Do NOT fail-closed on `{ return bPlaying ? CurrentLine.LineId : NAME_None; }`.
...
# NOT this CLASS UFUNCTION. Word-boundary GetCurrentLineId
# is NOT leftover analog LineId.
# Identifier GetCurrentLineId is NOT leftover analog
# GetQueuedLineCount, GetDroppedLineCount, GetPlayedLineCount,
...
# const method IS present. Return type IS FName. No args.
...
# the return type is not FName, or if Category is
...
# 4e878e0750eaed9a830a48e36e94d949f608491b RETARGET return to FName
# and method to GetCurrentLineId.
...
# keep Category Skyguard|Radio and identifier GetCurrentLineId.
# LOCKED_DECL is FName GetCurrentLineId() const;
...
# and NOT leftover analog RadioLine FName LineId;.
...
TARGET = "FName GetCurrentLineId() const;"
TARGET_SPLIT = (
    "FName GetCurrentLineId()\n"
    "\tconst;"
)
TARGET_INLINE = (
    "FName GetCurrentLineId() const "
    "{ return bPlaying ? CurrentLine.LineId : NAME_None; }"
)
TARGET_WRONG_BARE = "FName GetCurrentLineId();"
...
TARGET_WRONG_PLAYED = "int32 GetPlayedLineCount() const;"
...
TARGET_WRONG_FLOAT = "float GetCurrentLineId() const;"
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "FName GetCurrentLineIds() const;"
...
TARGET_WRONG_TRUE = "FName GetCurrentLineId = true;"
TARGET_WRONG_ZERO = "FName GetCurrentLineId = 0.f;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_radio_chatter_get_current_line_id"
    "_decl_contract.py"
)
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bFName\s+GetCurrentLineId\(\)\s*const\s*(?:;|\{)"
)
GET_CURRENT_LINE_ID_RE = re.compile(
    r"FName\s+GetCurrentLineId\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+bPlaying\s*\?\s*CurrentLine\.LineId"
    r"\s*:\s*NAME_None\s*;\s*\})"
)
GET_CURRENT_LINE_ID_WRONG_TYPE_RE = re.compile(
    r"(?:float|int32|bool|void|FLinearColor)\s+"
    r"GetCurrentLineId\b"
)
GET_CURRENT_LINE_ID_NO_CONST_RE = re.compile(
    r"FName\s+GetCurrentLineId\s*\(\s*\)\s*;"
)
ENQUEUE_LINE_IDENT_RE = re.compile(r"\bEnqueueLine\b")
GET_CURRENT_LINE_ID_IDENT_RE = re.compile(r"\bGetCurrentLineId\b")
...
def has_one_declaration(region: str, declaration: str) -> bool:
    ...
    pattern = re.compile(
        re.escape(stem)
        + r"\s*(?:;|\{[^}]*return\s+bPlaying\s*\?\s*"
        r"CurrentLine\.LineId\s*:\s*NAME_None\s*;\s*\})"
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # FName GetCurrentLineId() const; OR origin/main inline
    # `{ return bPlaying ? CurrentLine.LineId : NAME_None; }`.
    # Do not fail-closed on that inline body.
    # Do not accept missing const, a void/float/int32/bool
    ...
    if GET_CURRENT_LINE_ID_RE.search(compact) is None and (
        METHOD_SIGNATURE_RE.search(compact) is None
    ):
        return False
    if GET_CURRENT_LINE_ID_WRONG_TYPE_RE.search(compact):
        return False
    if GET_CURRENT_LINE_ID_NO_CONST_RE.search(compact) and (
        GET_CURRENT_LINE_ID_RE.search(compact) is None
    ):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+GetCurrentLineId\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for GetCurrentLineId "
...
class RadioChatterGetCurrentLineIdDeclContractTests(
    unittest.TestCase
):
    def test_radio_chatter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "GetCurrentLineId"),
            section,
        )
    ...
    def test_missing_get_current_line_id_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertTrue(
            GET_CURRENT_LINE_ID_IDENT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
    ...
    def test_get_current_line_id_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tFName GetCurrentLineIdX() const;\n",
            "\tint32 GetCurrentLineId = 1;\n",
            "\tFName LineId;\n",
            "\tFName GetCurrentLineId() = "
            + forty
            + ";\n",
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "GetCurrentLineId")
        )
        self.assertTrue(
            has_identifier(section, "GetCurrentLineId")
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_chatter_get_current_line_id"
            "_decl_contract.py"
        ))
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertIn("GetCurrentLineId", LOCKED_DECL)
    ...
    def test_word_boundary_get_current_line_id_vs_siblings(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier(
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "GetCurrentLineId"), section)
    ...
    def test_does_not_parse_radio_line_struct_as_this_slot(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LineId"), section)
        self.assertIn("CurrentLine.LineId", collapsed(section))
    ...
    def test_locked_decl_is_not_clone_ensure_toggle_hits_or_bind(
        ...
        self.assertTrue(LOCKED_DECL.startswith("FName GetCurrentLineId("))
