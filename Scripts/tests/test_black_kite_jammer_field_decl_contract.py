...
# PowerBus as the primary assertion. Do not lock
# leftover sibling debris fields DebrisPortVane,
# DebrisStarboardVane, DebrisJammer as the
...
class BlackKiteJammerFieldDeclContractTests(unittest.TestCase):
    def test_black_kite_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, JAMMER_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, JAMMER_FIELD)
        self.assertIn("Jammer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, JAMMER_FIELD)
    ...
    def test_missing_jammer_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBlackKiteBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, JAMMER_FIELD)
        self.assertIn("Jammer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, JAMMER_FIELD)
        self.assertIn("Jammer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", JAMMER_FIELD)
        self.assertNotIn("VisibleAnywhere", JAMMER_FIELD)
        self.assertNotIn("BlueprintReadOnly", JAMMER_FIELD)
        self.assertNotIn("Category", JAMMER_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, JAMMER_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, JAMMER_FIELD)
        self.assertIn("Jammer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* Jammer;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> Jammer;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, JAMMER_FIELD)
            self.assertIn("Jammer", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_jammer_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, JAMMER_FIELD),
            1,
        )
        self.assertTrue(
            JAMMER_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            JAMMER_FIELD,
        )
        self.assertTrue(
            JAMMER_FIELD.endswith(";"),
            JAMMER_FIELD,
        )
        self.assertIn("Jammer", JAMMER_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            JAMMER_FIELD,
        )
        self.assertIn("TObjectPtr<", JAMMER_FIELD)
        self.assertNotIn("TArray<", JAMMER_FIELD)
        self.assertNotIn("=", JAMMER_FIELD)
        self.assertNotIn("INDEX_NONE", JAMMER_FIELD)
        self.assertNotIn("UFUNCTION", JAMMER_FIELD)
        self.assertNotIn("{", JAMMER_FIELD)
        self.assertNotIn("}", JAMMER_FIELD)
        self.assertNotIn("return ", JAMMER_FIELD)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            JAMMER_FIELD,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", JAMMER_FIELD)
        self.assertNotIn("IssueClimbCommand", JAMMER_FIELD)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            JAMMER_FIELD,
        )
        ...
        self.assertNotIn("GetCivilianSeparationMeters", JAMMER_FIELD)
        self.assertNotIn("GetFinaleStage", JAMMER_FIELD)
        self.assertNotIn("IsClimbCommandIssued", JAMMER_FIELD)
        self.assertNotIn("IsWreckDiverted", JAMMER_FIELD)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            JAMMER_FIELD,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", JAMMER_FIELD)
        self.assertNotIn("GetGunnerMount", JAMMER_FIELD)
        self.assertNotIn("FaceWorldLocation", JAMMER_FIELD)
        self.assertNotIn("StarboardGuidanceArray", JAMMER_FIELD)
        self.assertNotIn("CommandCore", JAMMER_FIELD)
        self.assertNotIn("SetSearchlightTracked", JAMMER_FIELD)
        self.assertNotIn("SetContactIdentified", JAMMER_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            JAMMER_FIELD,
        )
        self.assertNotIn("SetLightningExposed", JAMMER_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            JAMMER_FIELD,
        )
        self.assertNotIn("HullCollider", JAMMER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, JAMMER_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{JAMMER_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, JAMMER_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, JAMMER_FIELD),
    ...
    def test_assigned_jammer_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "Jammer = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, JAMMER_FIELD)
        self.assertIn("Jammer", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, JAMMER_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            self.assertIn("Jammer", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, JAMMER_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{JAMMER_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, JAMMER_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, JAMMER_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            JAMMER_FIELD.startswith("UFUNCTION"),
            JAMMER_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, JAMMER_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", JAMMER_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_jammer_cpp_body(
        self,
    ) -> None:
        locked_only = f"{JAMMER_FIELD}\n"
        self.assertNotIn("{", JAMMER_FIELD)
        ...
        self.assertNotIn("return false", JAMMER_FIELD)
        self.assertNotIn("return true", JAMMER_FIELD)
        self.assertNotIn("CreateDefaultSubobject", JAMMER_FIELD)
        self.assertNotIn("SetExposed", JAMMER_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            JAMMER_FIELD,
        )
        self.assertNotIn("SetBossPhase", JAMMER_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_apply_strike(), JAMMER_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), JAMMER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, JAMMER_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_sibling_black_kite_fields(self) -> None:
        ...
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("PortNavigationVane", JAMMER_FIELD)
        self.assertNotIn("StarboardNavigationVane", JAMMER_FIELD)
        self.assertNotIn("PowerBus", JAMMER_FIELD)
        self.assertNotIn("DebrisPortVane", JAMMER_FIELD)
        self.assertNotIn("DebrisStarboardVane", JAMMER_FIELD)
        self.assertNotIn("DebrisJammer", JAMMER_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            JAMMER_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_last_flight_fields(self) -> None:
        ...
        for token in LEFTOVER_LAST_FLIGHT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("PortGuidanceArray", JAMMER_FIELD)
        self.assertNotIn("StarboardGuidanceArray", JAMMER_FIELD)
        self.assertNotIn("CommandCore", JAMMER_FIELD)
        self.assertNotIn("PortEngine", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", JAMMER_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_strike_bay_mechanism(
        ...
        for token in LEFTOVER_PORT_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_strike_bay_mechanism(
        ...
        for token in LEFTOVER_STARBOARD_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_cooling_system(
        ...
        for token in LEFTOVER_PORT_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("PortCoolingSystem", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_cooling_system(
        ...
        for token in LEFTOVER_STARBOARD_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("StarboardCoolingSystem", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_searchlight_methods(
        ...
        for token in LEFTOVER_SEARCHLIGHT_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("SetSearchlightTracked", JAMMER_FIELD)
        self.assertNotIn("IsSearchlightTracked", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_last_flight_jammer(self) -> None:
        ...
        self.assertNotIn("ASkyguardLastFlightBoss::Jammer", JAMMER_FIELD)
        self.assertNotEqual(
            HEADER_PATH,
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("SetContactIdentified", JAMMER_FIELD)
        self.assertNotIn("OpenOrbitExposure", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("SetLightningExposed", JAMMER_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", JAMMER_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            JAMMER_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, JAMMER_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("ASkyguardGunner", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("SkyguardRadarNode", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, JAMMER_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", JAMMER_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", JAMMER_FIELD)
        self.assertNotIn("MaxIntegrity", JAMMER_FIELD)
        self.assertNotIn("CurrentIntegrity", JAMMER_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", JAMMER_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, JAMMER_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, JAMMER_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_NOT_LOCKED:
            ...
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, JAMMER_FIELD)
        self.assertNotIn("MinimumCivilianSeparationMeters", JAMMER_FIELD)
        self.assertNotIn("550.f", JAMMER_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{JAMMER_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, JAMMER_FIELD),
            JAMMER_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "Black Kite Jammer field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{JAMMER_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, JAMMER_FIELD.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, JAMMER_FIELD)

    def test_contract_is_jammer_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(JAMMER_FIELD.endswith(";"))
        self.assertNotIn("=", JAMMER_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
        self.assertIn("Skyguard|Mission04|Boss", UPROPERTY_PORT)
        self.assertNotIn("Mission10", UPROPERTY_PORT)
        self.assertNotIn("ASkyguardLastFlightBoss", JAMMER_FIELD)
        self.assertNotIn(
            "test_last_flight_jammer_field_decl_contract.py",
            locked_only,
        )
