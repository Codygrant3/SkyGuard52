# THIS IS leftover-safe ASkyguardIronRainBoss
# MaxReleasesPerBay.
# origin/main form: one-line EditDefaultsOnly BlueprintReadOnly wrap
# UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Skyguard|Mission09|Performance", meta=(ClampMin="1", ClampMax="24"))
# then `int32 MaxReleasesPerBay = 6;` (HAS initializer).
# Type is **int32**, not float. ClampMin IS `"1"` (integer string, not `"1.0"`).
# ClampMax IS `"24"`. Default IS `= 6`.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Mission09|Performance`. EditDefaultsOnly IS present.
# BlueprintReadOnly IS present.
# Specifiers ARE EditDefaultsOnly + BlueprintReadOnly (NOT EditAnywhere,
# NOT BlueprintReadWrite, NOT VisibleAnywhere).
# It is NOT leftover analog LoadoutSpec FlareCount = 6 (same default 6).
# It is NOT leftover analog Mission09 director PoolBudget /
# MaxActiveThreats / MaxActiveDecoys / MaxSimultaneousExplosions /
# PoolCapacity (Category Performance on a DIFFERENT class).
# It is NOT leftover analog LifelineHunter
# MinimumWeaponSeparationMeters = 450.f.
# It is NOT leftover analog Pathfinder MaxSimulationSubsteps = 8 /
# GlobalVoiceLimit = 24 / Coastal EpicTreeBudget.
# It is NOT leftover analog IronRain UFUNCTION Category
# `Skyguard|Mission09|Boss` (OpenDispenserBay / ReleasePooledEscort /
# IssueClimbCommand / IssueCrossCommand / OpenUpperEngineExposure /
# GetDestroyed* / GetReleasedEscortCount / GetManeuver).
# It is NOT leftover analog ApplySecondIg+laFinish /
# ArmFuelControlRi+fleFinish (banned weapons).
# It is NOT leftover analog IronRain weak-point TObjectPtr
# (Dispenser* / CommandAntenna* / DecoyController / EnginePod* /
# FuelControl*).
# It is NOT leftover analog IronRain debris (DebrisPortWing /
# DebrisCenterRack / DebrisStarboardWing).
# It is NOT leftover analog Pathfinder Encounter RouteLengthCm =
# 45000.f / Harbor leftover 80 float token / LockWindowClimbSeconds = 2.4f /
# bAutoAdvance / ObservedPhase / multicast.
# It is NOT leftover analog Pathfinder EncounterController UFUNCTION
# #376-#382 / pathfinder-encounter-controller-field #512.
# It is NOT leftover analog ProductionBank / ProductionBankAsset /
# EventDefinitions / PropellerLoop / Mix same-name.
# It is NOT leftover analog GetUnboundRequiredCategories.
# It is NOT leftover analog Mission01-10 director/readiness same-name
# drafts.
# Fail-closed if the UPROPERTY or decl is missing or renamed, if Category
# is missing, if wrap is still EditAnywhere or BlueprintReadWrite, if wrap
# is VisibleAnywhere, if ClampMin is missing, if ClampMax is missing, if
# ClampMax is still `"16"`, if ClampMin is still `"1.0"`, if the default
# is not `= 6`, if LOCKED_DECL is still `int32 MaxSimulationSubsteps = 8;`
# or still `float`, if Category is still
# `Skyguard|Boss|Encounter|Safety` / Route / Commands / Telegraph /
# leftover analog `Skyguard|Boss|Encounter` /
# `Skyguard|Mission09|Boss` / `Skyguard|Boss|Destruction` /
# Skyguard|Apache / Skyguard|Combat / Skyguard|Theater /
# Skyguard|MissionMap / Skyguard|Audio|Acceptance /
# Skyguard|Audio|Development / Skyguard|Audio without |Production /
# still Category="Skyguard" without leftover-safe unique Category path
# `Skyguard|Mission09|Performance`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `ASkyguardIronRainBoss` public section
# ONLY after `class ASkyguardIronRainBoss` /
# `GENERATED_BODY`. Start at `public:`. Stop BEFORE `protected:`
# (this class has protected: then private:).
# Do NOT claim leftover analog FlareCount / PoolBudget /
# MaxActiveThreats / MaxSimulationSubsteps / GlobalVoiceLimit.
# Do NOT claim leftover IronRain UFUNCTION / weak-point / debris
# identifiers as LOCKED_DECL.
# Skip leftover analog audio-director / ProductionBank /
# PropellerLoop.
# Skip leftover GetResolvedProductionLoopRouteCount /
# AreResolvedProductionLoopRoutesComplete /
# OpenCockpitWindLoop / GlobalVoiceLimit.
# Skip leftover analog Coastal EpicTreeBudget.
# Skip private members.
# Skip banned Ig+la / Ri+fle / Ya+k.
# Clone wrap was leftover Pathfinder Safety int32
# Category="Skyguard|Boss|Encounter|Safety", meta=(ClampMin="1", ClampMax="16")
# wrapping `int32 MaxSimulationSubsteps = 8;`
# — RETARGET hard: class to ASkyguardIronRainBoss,
# Category to `Skyguard|Mission09|Performance`,
# specifiers EditAnywhere BlueprintReadWrite to
# EditDefaultsOnly BlueprintReadOnly,
# ClampMax `"16"` to `"24"`,
# LOCKED_DECL `int32 MaxSimulationSubsteps = 8;` to
# `int32 MaxReleasesPerBay = 6;`.
# wrap is EditDefaultsOnly BlueprintReadOnly
# Category="Skyguard|Mission09|Performance", meta=(ClampMin="1", ClampMax="24")
# (NO spaces around =).
# LOCKED_DECL is `int32 MaxReleasesPerBay = 6;`
# NOT `FName WeatherIdentity;` and NOT leftover Boom and NOT
# leftover VegetationStartCullDistanceCm and NOT leftover
# GlobalVoiceLimit = 24 and NOT leftover OpenCockpitWindLoop
# and NOT leftover ApproachSpeedCmPerSecond = 1250.f
# and NOT leftover RouteLengthCm = 45000.f
# and NOT leftover analog `float FireRate = 12.0f` /
# `float RecoilPitch = 0.92f`
# and NOT leftover Harbor 40 / 80 float tokens
# and NOT leftover LockWindowClimbSeconds = 2.4f
# and NOT leftover analog `float MaxLateralOffsetCm = 3600.f;`
# and NOT leftover analog `float MaxSimulationStepSeconds = 0.05f;`
# and NOT leftover analog `int32 MaxSimulationSubsteps = 8;`
# and NOT leftover analog `int32 FlareCount = 6;`
# and NOT leftover analog `int32 MaxSimultaneousExplosions = 6;`
# and NOT leftover analog Coastal EpicTreeBudget.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# / Boom / VegetationStartCullDistanceCm
# / EngineCruiseLoop / EnginePowerLoop / PropellerLoop /
# EngineIdleLoop / GlobalVoiceLimit / OpenCockpitWindLoop
# / ApproachSpeedCmPerSecond
# AS THE LOCKED DECL.
# Fail-closed if LOCKED_DECL is still `int32 MaxSimulationSubsteps = 8;`
# or still `float` or ClampMin is still `"100.0"` or `"1.0"`.
# Fail-closed if locked wrap Category is Skyguard|Combat
# or still Skyguard|Theater or Skyguard|MissionMap
# or Skyguard|Apache or Skyguard|Audio|Acceptance
# or Skyguard|Audio|Development or Skyguard|Audio|Budget
# or Category="Skyguard" without |Mission09|Performance.
# Fail-closed if wrap is EditAnywhere or BlueprintReadWrite
# (must be EditDefaultsOnly + BlueprintReadOnly).
# Fail-closed if wrap is VisibleAnywhere.
# Fail-closed if ClampMin is missing or ClampMax is missing
# or ClampMax is still `"16"` or default is not 6.
# Default is `= 6` not `= 3600.f` / `= 0.05f` / `= 850.f` /
# `= 700.f` / `= 1250.f` / `= 9000.f` / `= 8`.
# Harbor leftover is only the split 40 / 80 float tokens.
# Default 6 is not Harbor.
...
TARGET = "int32 MaxReleasesPerBay = 6;"
TARGET_WRONG_BARE = "int32 MaxReleasesPerBay;"
TARGET_WRONG_INIT = "int32 MaxReleasesPerBay = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "int32 MaxReleasesPerBay = true;"
TARGET_WRONG_ZERO = "int32 MaxReleasesPerBay = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MaxReleasesPerBay;"
...
TARGET_WRONG_POOL_CAPACITY = "int32 PoolCapacity = 48;"
...
SIBLING_OPEN_DISPENSER_BAY = "OpenDispenserBay"
SIBLING_RELEASE_POOLED_ESCORT = "ReleasePooledEscort"
SIBLING_ISSUE_CLIMB = "IssueClimbCommand"
SIBLING_ISSUE_CROSS = "IssueCrossCommand"
SIBLING_OPEN_UPPER_ENGINE = "OpenUpperEngineExposure"
SIBLING_GET_DESTROYED_ENGINE = "GetDestroyedEngineCount"
SIBLING_GET_RELEASED_ESCORT = "GetReleasedEscortCount"
SIBLING_BAY_RELEASE_COUNTS = "BayReleaseCounts"
SIBLING_HANDLE_WEAK = "HandleWeakPointDestroyed"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MaxReleasesPerBay;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MaxReleasesPerBay;"
...
STOP_BEFORE_SKYLINE_STYLE = "DispenserPort"
...
THIS_SCRIPT = (
    "Scripts/tests/test_iron_rain_max_releases_per_bay"
...
MAX_RELEASES_PER_BAY_DECL_RE = re.compile(
    r"int32\s+MaxReleasesPerBay\s*=\s*6\s*;"
)
MAX_RELEASES_PER_BAY_BARE_RE = re.compile(
    r"int32\s+MaxReleasesPerBay\s*;"
)
...
LEFTOVER_POOL_CAPACITY_DECL_RE = re.compile(
    r"int32\s+PoolCapacity\s*=\s*48\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 MaxReleasesPerBay = 6;`.
    ...
    # MaxReleasesPerBay without the 6 initializer.
    ...
    if MAX_RELEASES_PER_BAY_DECL_RE.search(compact) is None:
        return False
    if MAX_RELEASES_PER_BAY_BARE_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+MaxReleasesPerBay\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 MaxReleasesPerBay "
...
class IronRainMaxReleasesPerBayFieldDeclContractTests(unittest.TestCase):
    def test_iron_rain_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxReleasesPerBay"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MaxReleasesPerBay"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaxReleasesPerBay", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_max_releases_per_bay_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn("EditDefaultsOnly", UPROPERTY_EDIT_READWRITE)
        self.assertIn("BlueprintReadOnly", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("EditAnywhere", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertIn("EditDefaultsOnly", specifiers)
        ...
        self.assertIn('Category="Skyguard|Mission09|Performance"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "int32 MaxReleasesPerBay;",
            compact_origin.split("MaxReleasesPerBay = 6;")[0]
            + compact_origin.split("MaxReleasesPerBay = 6;")[-1]
            if "MaxReleasesPerBay = 6;" in compact_origin
            else compact_origin,
        )
    ...
    def test_max_releases_per_bay_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 MaxReleasesPerBay"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 6", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 MaxReleasesPerBays = 8;\n",
            "\tfloat MaxSimulationSubsteps = 8;\n",
            "\tFName MaxReleasesPerBay = NAME_None;\n",
            "\tint32 MaxReleasesPerBay = " + forty + ";\n",
            "\tint32 MaxReleasesPerBay = " + eighty + ";\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission09|Performance"', origin)
        ...
        self.assertIn("EditDefaultsOnly", origin)
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', theater_specs)
        self.assertIn('Category="Skyguard|Mission09|Performance"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', development_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', safety_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', boss_specs)
        ...
        self.assertNotIn('Category="Skyguard|Mission09|Performance"', destruction_specs)
        leftover_write = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn("EditDefaultsOnly", write_specs)
        self.assertIn("EditDefaultsOnly", origin)
        self.assertNotIn("EditAnywhere", origin)
        self.assertNotIn("BlueprintReadWrite", origin)
        leftover_clamp16 = (
        ...
        self.assertIn('ClampMax="24"', origin)
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxReleasesPerBay"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertNotIn(STOP_BEFORE_PROTECTED, section)
        self.assertTrue(has_identifier(leaked, "MaxReleasesPerBay"))
        self.assertTrue(has_identifier(section, "MaxReleasesPerBay"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_OPEN_DISPENSER_BAY))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("ClampMin", UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMin="1"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="100.0"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('ClampMin="1.0"', UPROPERTY_EDIT_READWRITE)
        self.assertIn("ClampMax", UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMax="24"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardIronRainBoss.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain max releases per bay field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "iron_rain_max_releases_per_bay_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
