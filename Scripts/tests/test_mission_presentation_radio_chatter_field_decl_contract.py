# THIS IS leftover-safe FSkyguardMissionPresentation RadioChatter.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `TArray<FText> RadioChatter;` (no initializer).
# THIS IS leftover-safe isolated UPROPERTY without Category.
# EditAnywhere IS present.
# BlueprintReadOnly IS present. There is NO default.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if EditAnywhere is dropped, if Category
# is added, or if specifiers drop to BlueprintReadWrite.
...
# (leftover presentation field decls).
...
# Do NOT parse leftover FSkyguardRadioLine public UPROPERTY.
# Do NOT contract sibling fields Briefing / SuccessDebrief /
# FailureDebrief / MinimumBriefingWarmupSeconds.
# THIS IS NOT leftover mission-presentation-defaults.
# Analog leftover mission-presentation-defaults then isolated
# RadioChatter field decl.
# THIS IS NOT leftover USkyguardMissionDefinition presentation
# field decls.
# THIS IS NOT leftover FSkyguardRadioLine isolated fields.
# Keep bulk leftover files in LOCKED_SCRIPTS; isolated field
# decl does not relock test_campaign_theater_kit_contract.py
# or leftover mission-types-defaults-tests.
...
LEFTOVER_MISSION_DEFINITION_CLASS = "USkyguardMissionDefinition"
LEFTOVER_RADIO_LINE_STRUCT = "FSkyguardRadioLine"
TARGET = "TArray<FText> RadioChatter;"
TARGET_WRONG_EQ_EMPTY = "TArray<FText> RadioChatter = {};"
TARGET_WRONG_BRACE = "TArray<FText> RadioChatter{};"
TARGET_WRONG_EQ_NONE = "TArray<FText> RadioChatter = NAME_None;"
TARGET_WRONG_FALSE = "TArray<FText> RadioChatter = false;"
TARGET_WRONG_TRUE = "TArray<FText> RadioChatter = true;"
TARGET_WRONG_ZERO = "TArray<FText> RadioChatter = 0.f;"
TARGET_WRONG_FTEXT = "FText RadioChatter;"
TARGET_WRONG_FNAME = "TArray<FName> RadioChatter;"
TARGET_WRONG_RADIO_LINE = "TArray<FSkyguardRadioLine> RadioChatter;"
TARGET_WRONG_HEALTH = "float RadioChatter = 160.f;"
...
STOP_BEFORE_BOSS = "struct FSkyguardBossDefinition"
STOP_BEFORE_WEAK_POINT_DEF = "struct FSkyguardBossWeakPointDefinition"
STOP_BEFORE_WEATHER = "struct FSkyguardWeatherProfile"
STOP_BEFORE_OBJECTIVE_DEF = "struct FSkyguardObjectiveDefinition"
STOP_BEFORE_OBJECTIVE_PROGRESS = "struct FSkyguardObjectiveProgress"
STOP_BEFORE_ROUTE = "struct FSkyguardRouteDefinition"
STOP_BEFORE_MISSION_DEFINITION = "class USkyguardMissionDefinition"
STOP_BEFORE_RADIO_LINE = "struct FSkyguardRadioLine"
STOP_BEFORE_BRIEFING_CARD = "struct FSkyguardBriefingCard"
STOP_BEFORE_BRIEFING_RADIO_ROW = "struct FSkyguardBriefingRadioRow"
STOP_BEFORE_HOW_TO_FLY = "struct FSkyguardHowToFlyRow"
...
GET_RADIO_CHATTER = "GetRadioChatter"
PRIME_LINES = "PrimeLines"
ENQUEUE_LINE = "EnqueueLine"
VALIDATE_DEFINITION = "ValidateDefinition"
FIND_OBJECTIVE = "FindObjective"
GET_PRIMARY_ASSET_ID = "GetPrimaryAssetId"
SIBLING_BRIEFING = "Briefing"
SIBLING_SUCCESS_DEBRIEF = "SuccessDebrief"
SIBLING_FAILURE_DEBRIEF = "FailureDebrief"
SIBLING_WARMUP = "MinimumBriefingWarmupSeconds"
...
LOCKED_SCRIPTS = LOCKED_SCRIPTS + EXTRA_LOCKED_SCRIPTS
...
def leftover_radio_line_fields() -> tuple[str, ...]:
    return (
        "LineId",
        "Speaker",
        "Subtitle",
...
def leftover_neighbor_methods() -> tuple[str, ...]:
    return (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `TArray<FText> RadioChatter;`.
    # Do not accept `= {}` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` / leftover float Health when
    # origin/main has a bare TArray<FText>. Do not accept
    # sibling Briefing / SuccessDebrief / FailureDebrief /
    # MinimumBriefingWarmupSeconds fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray<FText>\s+RadioChatter\s*=",
        compact,
    ):
        return False
    if re.search(
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_WEATHER,
        STOP_BEFORE_OBJECTIVE_DEF,
        STOP_BEFORE_OBJECTIVE_PROGRESS,
        STOP_BEFORE_ROUTE,
        STOP_BEFORE_MISSION_DEFINITION,
        STOP_BEFORE_RADIO_LINE,
        STOP_BEFORE_BRIEFING_CARD,
        STOP_BEFORE_BRIEFING_RADIO_ROW,
        STOP_BEFORE_HOW_TO_FLY,
        ...
        f"class SKYGUARD52_API {LEFTOVER_MISSION_DEFINITION_CLASS}",
        f"class {LEFTOVER_MISSION_DEFINITION_CLASS}",
    )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray<FText>\s+RadioChatter\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FText> RadioChatter is missing from "
...
class MissionPresentationRadioChatterFieldDeclContractTests(unittest.TestCase):
    def test_mission_presentation_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_MISSION_DEFINITION_CLASS)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_RADIO_LINE_STRUCT)
        ...
        self.assertTrue(has_identifier(section, "RadioChatter"), section)
        ...
        for method in leftover_neighbor_methods():
            self.assertNotIn(method, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        self.assertNotIn(STOP_BEFORE_RADIO_LINE, section)
        self.assertNotIn(STOP_BEFORE_BRIEFING_CARD, section)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        other = (
            f"struct {LEFTOVER_RADIO_LINE_STRUCT}\n"
            "{\n"
    ...
    def test_leftover_definition_or_radio_line_does_not_satisfy(self) -> None:
        mixed = (
        ...
        self.assertFalse(has_identifier(section, "RadioChatter"), section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("TArray<FText> RadioChatter;", compact_origin)
        self.assertNotIn("RadioChatter = {}", compact_origin)
        self.assertNotIn("RadioChatter = NAME_None", compact_origin)
        self.assertNotIn("RadioChatter = false", compact_origin)
        self.assertNotIn("RadioChatter = true", compact_origin)
        self.assertNotIn("RadioChatter = 0.f", compact_origin)
        self.assertNotIn("RadioChatter = 160.f", compact_origin)
    ...
    def test_radio_chatter_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FText> RadioChatter"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("TArray<FText> ", LOCKED_DECL)
        ...
        self.assertNotIn("FSkyguardMissionPresentation", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        for field in leftover_radio_line_fields():
            self.assertNotIn(field, LOCKED_DECL)
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RADIO_LINE}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tTArray<FText> " + leftover_retired_primary_hits_field() + ";\n"
        )
        leftover_guided = (
            "\tTArray<FText> " + leftover_retired_guided_hits_field() + ";\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            f"\t{TARGET_WRONG_EQ_EMPTY}\n",
            f"\t{TARGET_WRONG_BRACE}\n",
            ...
            f"\t{TARGET_WRONG_RADIO_LINE}\n",
            f"\t{TARGET_WRONG_HEALTH}\n",
            leftover_primary,
            leftover_guided,
            f"\tFText {SIBLING_BRIEFING};\n",
            "\tTArray<FText> RadioChatters;\n",
            "\tint32 RadioChatter;\n",
            "\tbool RadioChatter;\n",
            "\tfloat RadioChatter = " + forty + ";\n",
            "\tfloat RadioChatter = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_dropped_edit_anywhere_fails_closed(self) -> None:
        ...
        section = spec_section(with_category)
        specifiers = attached_uproperty_specifiers(section)
    ...
    def test_does_not_contract_sibling_presentation_fields(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_RADIO_LINE_STRUCT, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_leftover_neighbors(self) -> None:
        ...
        for absent in (
            ...
            LEFTOVER_MISSION_DEFINITION_CLASS,
            LEFTOVER_RADIO_LINE_STRUCT,
        ):
            self.assertNotIn(absent, header)
            self.assertNotIn(absent, section)
            self.assertNotIn(absent, leaked)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tTArray<FText> {GET_RADIO_CHATTER}();\n"
            f"\tvoid {PRIME_LINES}();\n"
            f"\tbool {ENQUEUE_LINE}();\n"
            f"\tFText {SIBLING_BRIEFING};\n"
            f"\t{TARGET_WRONG_HEALTH}\n"
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Presentation"', UPROPERTY_EDIT_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-presentation RadioChatter field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_chatter_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardRadioChatterComponent.h", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_RADIO_LINE_DURATION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_RADIO_LINE_COOLDOWN, LOCKED_SCRIPTS)
        self.assertIn(SIBLING_BRIEFING_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(SIBLING_SUCCESS_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(SIBLING_FAILURE_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(SIBLING_WARMUP_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, locked_only)
        self.assertNotIn(LEFTOVER_RADIO_LINE_STRUCT, locked_only)
