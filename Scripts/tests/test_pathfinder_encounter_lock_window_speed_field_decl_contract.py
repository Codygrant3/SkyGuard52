...
# then `float LockWindowSpeedCmPerSecond = 850.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Route`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"100.0"`. Default IS `= 850.f`.
...
# if ClampMin is missing, if the default is not `= 850.f`, if Category is
...
# (NO spaces around =).
# LOCKED_DECL is `float LockWindowSpeedCmPerSecond = 850.f;`
...
# Fail-closed if ClampMin is missing or default is not 850.f.
...
TARGET = "float LockWindowSpeedCmPerSecond = 850.f;"
TARGET_WRONG_BARE = "float LockWindowSpeedCmPerSecond;"
TARGET_WRONG_INIT = "float LockWindowSpeedCmPerSecond = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float LockWindowSpeedCmPerSecond = true;"
TARGET_WRONG_ZERO = "float LockWindowSpeedCmPerSecond = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LockWindowSpeedCmPerSecond;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LockWindowSpeedCmPerSecond;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LockWindowSpeedCmPerSecond;"
...
STOP_BEFORE_SKYLINE_STYLE = "CriticalSpeedCmPerSecond"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_lock_window_speed"
...
LOCK_WINDOW_SPEED_DECL_RE = re.compile(
    r"float\s+LockWindowSpeedCmPerSecond\s*=\s*850\.f\s*;"
)
LOCK_WINDOW_SPEED_BARE_RE = re.compile(
    r"float\s+LockWindowSpeedCmPerSecond\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float LockWindowSpeedCmPerSecond = 850.f;`.
    ...
    # LockWindowSpeedCmPerSecond without the 850.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LOCK_WINDOW_SPEED_DECL_RE.search(compact) is None:
        return False
    if LOCK_WINDOW_SPEED_BARE_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "LockWindowSpeedCmPerSecond":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LockWindowSpeedCmPerSecond\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LockWindowSpeedCmPerSecond "
...
class PathfinderEncounterLockWindowSpeedFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockWindowSpeedCmPerSecond"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LockWindowSpeedCmPerSecond"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LockWindowSpeedCmPerSecond", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lock_window_speed_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float LockWindowSpeedCmPerSecond;",
            compact_origin.split("LockWindowSpeedCmPerSecond = 850.f;")[0]
            + compact_origin.split("LockWindowSpeedCmPerSecond = 850.f;")[-1]
            if "LockWindowSpeedCmPerSecond = 850.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_lock_window_speed_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LockWindowSpeedCmPerSecond"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat LockWindowSpeedCmPerSeconds = 850.f;\n",
            "\tint32 LockWindowSpeedCmPerSecond = 850;\n",
            "\tFName LockWindowSpeedCmPerSecond = NAME_None;\n",
            "\tfloat LockWindowSpeedCmPerSecond = " + forty + ";\n",
            "\tfloat LockWindowSpeedCmPerSecond = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockWindowSpeedCmPerSecond"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LockWindowSpeedCmPerSecond"))
        self.assertTrue(has_identifier(section, "LockWindowSpeedCmPerSecond"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter lock window speed field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_lock_window_speed_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
