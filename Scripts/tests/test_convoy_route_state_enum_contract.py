...
HEADER_NAME = "SkyguardMission03IntegrationDirector.h"
# Leftover #56–#64 plus Mission 03 production sources (merged #99).
...
LOCKED = {
    ...
    "SkyguardMission03IntegrationDirectorTests.cpp",
...
# Apache CPG convoy escort progression. Not Yak-52 intercept states.
LIVE_STATES = [
    "Holding",
    "Advancing",
    "TunnelReached",
...
# Wave state, Mission 03 readiness (bYakRuntimeReady), and siblings stay unlocked.
SIBLING_TYPES = (
...
class ConvoyRouteStateEnumContractTests(unittest.TestCase):
    def test_convoy_route_state_enum_exists(self) -> None:
    ...
    def test_missing_enum_fails_closed(self) -> None:
        ...
        self.assertIn("ESkyguardConvoyRouteState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_enumerators_match_live_order(self) -> None:
        header = origin_main(HEADER_NAME)
        enumerators = enum_enumerators(header, "ESkyguardConvoyRouteState")
        ...
        body = enum_body(header, "ESkyguardConvoyRouteState")
    ...
    def test_convoy_route_state_enum_does_not_invent_index_none(self) -> None:
        header = origin_main(HEADER_NAME)
        enumerators = enum_enumerators(header, "ESkyguardConvoyRouteState")
        body = enum_body(header, "ESkyguardConvoyRouteState")
    ...
    def test_convoy_route_state_enum_does_not_require_rifle_or_igla(self) -> None:
    ...
    def test_contract_is_convoy_route_state_enum_only(self) -> None:
        header = origin_main(HEADER_NAME)
        body = enum_body(header, "ESkyguardConvoyRouteState")
