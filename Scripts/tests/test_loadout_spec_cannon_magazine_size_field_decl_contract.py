# THIS IS leftover-safe FSkyguardLoadoutSpec CannonMagazineSize.
...
# (split tokens).
# Exact identifier vs siblings CannonReserve /
# RocketMagazineSize / RocketReserve / GuidedMagazineSize /
...
TARGET_WRONG_BARE = "int32 CannonMagazineSize;"
TARGET_WRONG_ZERO = "int32 CannonMagazineSize = 0;"
TARGET_WRONG_ZERO_F = "int32 CannonMagazineSize = 0.f;"
TARGET_WRONG_FALSE = "int32 CannonMagazineSize = false;"
TARGET_WRONG_HUNDRED = "int32 CannonMagazineSize = 100;"
TARGET_WRONG_SIX = "int32 CannonMagazineSize = 6;"
TARGET_WRONG_CPG_FEEL = (
    "int32 CannonMagazineSize = SkyguardApacheCpgFeel::CannonReserve;"
)
...
TARGET_WRONG_FLOAT = (
    "float CannonMagazineSize = "
    "SkyguardApacheCpgFeel::CannonMagazineSize;"
)
TARGET_WRONG_BOOL = (
    "bool CannonMagazineSize = "
    "SkyguardApacheCpgFeel::CannonMagazineSize;"
)
...
def locked_cpg_feel_initializer() -> str:
    return "SkyguardApacheCpgFeel::CannonMagazineSize"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling CannonReserve / RocketMagazineSize /
    # HullIntegrity / PlaystyleLine.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+CannonMagazineSize\s*=\s*"
        r"SkyguardApacheCpgFeel::CannonMagazineSize\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"int32\s+CannonMagazineSize\s*=\s*0\b", compact):
        return False
    if re.search(r"int32\s+CannonMagazineSize\s*=\s*0\.f", compact):
        return False
    if re.search(r"int32\s+CannonMagazineSize\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+CannonMagazineSize\s*=\s*100\b", compact):
        return False
    if re.search(r"int32\s+CannonMagazineSize\s*=\s*6\b", compact):
        return False
    if re.search(
        r"int32\s+CannonMagazineSize\s*=\s*"
    ...
    if re.search(
        ...
        return False
    if re.search(r"float\s+CannonMagazineSize\b", compact):
        return False
    if re.search(r"bool\s+CannonMagazineSize\b", compact):
        return False
    if re.search(r"int32\s+CannonMagazineSize\s*;", compact) and not re.search(
        r"int32\s+CannonMagazineSize\s*=",
        compact,
    ):
        return False
    return True
...
class LoadoutSpecCannonMagazineSizeFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CannonMagazineSize"), section)
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CannonMagazineSize"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CannonMagazineSize", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cannon_magazine_size_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 CannonMagazineSize"))
    ...
    def test_literal_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("int32 CannonMagazineSize = 0;", compact_origin)
        self.assertNotIn("int32 CannonMagazineSize = 6;", compact_origin)
        self.assertNotIn("int32 CannonMagazineSize = 100;", compact_origin)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            "SkyguardApacheCpgFeel::GuidedReserve;\n",
            f"\tint32 {SIBLING_FLARE} = 6;\n",
            "\tint32 CannonMagazineSizes = "
            "SkyguardApacheCpgFeel::CannonMagazineSize;\n",
            "\tint32 CannonMagazineSize = " + forty + ";\n",
            "\tint32 CannonMagazineSize = " + eighty + ";\n",
        )
    ...
    def test_cannon_magazine_size_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 CannonMagazineSize"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("SkyguardApacheCpgFeel::CannonMagazineSize", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec CannonMagazineSize field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cannon_magazine_size_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
