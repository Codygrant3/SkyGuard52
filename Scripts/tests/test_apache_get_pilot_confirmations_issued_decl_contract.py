...
# or lock the GetPilotConfirmationsIssued body.
# origin/main is inline
# (`int32 GetPilotConfirmationsIssued() const`
# with `{ return PilotConfirmationsIssued; }`
# possibly on following lines);
# accept that exact inline form, a one-line prototype
# (`int32 GetPilotConfirmationsIssued() const;`),
# other split-line wraps, and other inline bodies
...
# of ASkyguardApacheAircraft only. Do not lock
...
# pilot-confirm-command / confirm-line contracts.
# This is not leftover AimChinTurret / SetRotorPower /
# IssuePilotCommand / GetPilotCommand siblings in this
# wave (do not create those files). Stay off
# AimChinTurret, SetRotorPower, IssuePilotCommand,
# GetPilotCommand, SetOrbitFocus, FaceWorldLocation,
# SetSensorView, SetFirstPersonInterior,
# SetDirectFlightInput, GetForwardSpeed, ApplyDamage,
# GetDamageFraction, ApplySystemHit, ApplyHit,
# IsSensorLive, IsThermalAvailable, IsSensorViewActive,
# and remaining public methods. Leftover briefing /
...
# DebriefWidget TravelNext / HandleDebriefKey, and
# leftover SortiePresentationWidgets stay sibling-only.
GET_CONFIRMATIONS = ("int32 GetPilotConfirmationsIssued() const;")
UFUNCTION_APACHE = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Apache")'
)
# Leftover #56–#64 plus SkyguardApacheAircraft production
# files. This lane only adds an isolated Python
# GetPilotConfirmationsIssued declaration contract on
...
# IssuePilotCommand, GetPilotCommand, SetOrbitFocus,
...
# pilot-confirm-command / confirm-line, leftover CPG
# HUD / sight HUD, leftover drafts #56–#64, leftover
# ApacheSystem enum values, leftover Harbor clocks,
# leftover skyline HarborIndustrial, leftover
# DebriefWidget isolated contracts, leftover
# BriefingWidget isolated contracts, leftover gun-fire
# camera shake, leftover sortie-hud-host fail-closed,
# leftover AimChinTurret / SetRotorPower /
# IssuePilotCommand / GetPilotCommand siblings, and
...
# pilot-command-roster #b593, leftover
# pilot-confirm-command / confirm-line, leftover
# AimChinTurret / SetRotorPower / IssuePilotCommand /
# GetPilotCommand siblings in this wave, leftover
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_pilot_confirm_command_contract.py",
    "Scripts/tests/test_pilot_confirm_command.py",
    "Scripts/tests/test_pilot_confirm_line_contract.py",
    "Scripts/tests/test_pilot_confirm_line_decl_contract.py",
    "Scripts/tests/test_pilot_confirm_line.py",
...
# GetDamageFraction, ApplySystemHit, ApplyHit,
# IsSensorLive, IsThermalAvailable, IsSensorViewActive,
# and remaining public methods stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# Leftover IssuePilotCommand / GetPilotCommand siblings
# in this wave stay unlocked. Do not create those files.
...
# Leftover pilot-confirm-command / confirm-line stay
# unlocked. This lane is declaration presence only.
LEFTOVER_PILOT_CONFIRM_COMMAND_NOT_LOCKED = (
    "test_pilot_confirm_command_decl_contract.py",
    "test_pilot_confirm_command_contract.py",
    "test_pilot_confirm_command.py",
)
LEFTOVER_PILOT_CONFIRM_LINE_NOT_LOCKED = (
    "test_pilot_confirm_line_contract.py",
    "test_pilot_confirm_line_decl_contract.py",
    "test_pilot_confirm_line.py",
)
...
# commands / loadout / lock-phase / leftover Gunner
...
# .cpp GetPilotConfirmationsIssued body / invented
# INDEX_NONE stay unlocked. Do not invent INDEX_NONE
# or lock the cpp body. origin/main inline
# `return PilotConfirmationsIssued` is accepted as
# presence, not a locked implementation contract.
# Do not parse leftover HUD classes.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "ASkyguardApacheAircraft::GetPilotConfirmationsIssued",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheGetPilotConfirmationsIssuedDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_CONFIRMATIONS), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_CONFIRMATIONS)
        self.assertIn("GetPilotConfirmationsIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_CONFIRMATIONS))

    def test_missing_get_confirmations_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_CONFIRMATIONS)
        self.assertIn("GetPilotConfirmationsIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_CONFIRMATIONS)
        self.assertIn("GetPilotConfirmationsIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_CONFIRMATIONS), section)
        self.assertNotIn("BlueprintPure", GET_CONFIRMATIONS)
        self.assertNotIn("UFUNCTION", GET_CONFIRMATIONS)
        self.assertNotIn("Category", GET_CONFIRMATIONS)
        self.assertNotIn("BlueprintCallable", GET_CONFIRMATIONS)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_CONFIRMATIONS)
        self.assertIn("GetPilotConfirmationsIssued", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tint32 GetPilotConfirmationsIssued const;\n"
        missing_const = "\tint32 GetPilotConfirmationsIssued();\n"
        wrong_return_void = (
            "\tvoid GetPilotConfirmationsIssued() const;\n"
        )
        wrong_return_bool = (
            "\tbool GetPilotConfirmationsIssued() const;\n"
        )
        wrong_return_float = (
            "\tfloat GetPilotConfirmationsIssued() const;\n"
        )
        added_arg = (
            "\tint32 GetPilotConfirmationsIssued(int32 Extra) const;\n"
        )
        leftover_command = (
            "\tESkyguardPilotCommand GetPilotCommand() const "
            "{ return CurrentPilotCommand; }\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_CONFIRMATIONS)
            self.assertIn("GetPilotConfirmationsIssued", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_confirmations_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_CONFIRMATIONS),
            GET_CONFIRMATIONS,
        )
        self.assertTrue(has_declaration(section, GET_CONFIRMATIONS))
        self.assertEqual(declaration_count(section, GET_CONFIRMATIONS), 1)
        self.assertTrue(
            GET_CONFIRMATIONS.startswith("int32 "),
            GET_CONFIRMATIONS,
        )
        self.assertTrue(GET_CONFIRMATIONS.endswith(";"), GET_CONFIRMATIONS)
        self.assertIn("GetPilotConfirmationsIssued()", GET_CONFIRMATIONS)
        self.assertIn(" const", GET_CONFIRMATIONS)
        self.assertNotIn("INDEX_NONE", GET_CONFIRMATIONS)
        self.assertNotIn("{", GET_CONFIRMATIONS)
        self.assertNotIn("}", GET_CONFIRMATIONS)
        self.assertNotIn("return ", GET_CONFIRMATIONS)
        self.assertNotIn("return PilotConfirmationsIssued", GET_CONFIRMATIONS)
        self.assertNotIn("AimChinTurret", GET_CONFIRMATIONS)
        self.assertNotIn("SetRotorPower", GET_CONFIRMATIONS)
        self.assertNotIn("IssuePilotCommand", GET_CONFIRMATIONS)
        self.assertNotIn("GetPilotCommand()", GET_CONFIRMATIONS)
        self.assertNotIn("GetChinMuzzleLocation", GET_CONFIRMATIONS)
        self.assertNotIn("GetChinTurret", GET_CONFIRMATIONS)
        self.assertNotIn("GetRotorPowerScale", GET_CONFIRMATIONS)
    ...
    def test_declaration_accepts_origin_main_inline_and_prototype(
        ...
        self.assertTrue(
            has_declaration(one_line_prototype, GET_CONFIRMATIONS)
        )
        self.assertEqual(
            require_declaration(one_line_prototype, GET_CONFIRMATIONS),
            GET_CONFIRMATIONS,
        )
        exact_inline = (
            "int32 GetPilotConfirmationsIssued() "
            "const { return PilotConfirmationsIssued; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_CONFIRMATIONS))
        self.assertEqual(
            require_declaration(exact_inline, GET_CONFIRMATIONS),
            GET_CONFIRMATIONS,
        )
        self.assertEqual(declaration_count(exact_inline, GET_CONFIRMATIONS), 1)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_CONFIRMATIONS), section)
        self.assertEqual(
            require_declaration(section, GET_CONFIRMATIONS),
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tint32\n"
            "\tGetPilotConfirmationsIssued() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, GET_CONFIRMATIONS),
                section,
            )
            self.assertEqual(
                require_declaration(section, GET_CONFIRMATIONS),
            ...
            self.assertEqual(
                declaration_count(section, GET_CONFIRMATIONS),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_CONFIRMATIONS}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_CONFIRMATIONS))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_CONFIRMATIONS), section)
        self.assertEqual(
            require_declaration(section, GET_CONFIRMATIONS),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_CONFIRMATIONS)
        self.assertNotIn("return PilotConfirmationsIssued", GET_CONFIRMATIONS)
        self.assertNotIn("return Other", GET_CONFIRMATIONS)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_CONFIRMATIONS}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_CONFIRMATIONS)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_CONFIRMATIONS)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_confirmations_body(self) -> None:
        locked_only = f"{GET_CONFIRMATIONS}\n"
        self.assertNotIn("{", GET_CONFIRMATIONS)
        self.assertNotIn("}", GET_CONFIRMATIONS)
        self.assertNotIn("return ", GET_CONFIRMATIONS)
        self.assertNotIn("return PilotConfirmationsIssued", GET_CONFIRMATIONS)
        self.assertNotIn("return PilotConfirmationsIssued", locked_only)
        self.assertNotIn(
            "ASkyguardApacheAircraft::GetPilotConfirmationsIssued",
            GET_CONFIRMATIONS,
        )
        self.assertNotIn("SkyguardApacheAircraft.cpp", GET_CONFIRMATIONS)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", GET_CONFIRMATIONS)
        self.assertNotIn("return true", GET_CONFIRMATIONS)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{GET_CONFIRMATIONS}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONFIRMATIONS)
        self.assertNotIn("GetGunnerMount", GET_CONFIRMATIONS)
        self.assertNotIn("GetEyeMount", GET_CONFIRMATIONS)
        self.assertNotIn("GetWeaponMount", GET_CONFIRMATIONS)
        self.assertNotIn("GetChinTurret", GET_CONFIRMATIONS)
        self.assertNotIn("GetPilotMount", GET_CONFIRMATIONS)
        self.assertNotIn("GetSensorTurret", GET_CONFIRMATIONS)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_CONFIRMATIONS}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CONFIRMATIONS)
        self.assertNotIn("GetChinMuzzleLocation", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CONFIRMATIONS)
        self.assertNotIn("SetOrbitFocus", GET_CONFIRMATIONS)
        self.assertNotIn("FaceWorldLocation", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CONFIRMATIONS)
        self.assertNotIn("SetSensorView", GET_CONFIRMATIONS)
        self.assertNotIn("SetFirstPersonInterior", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CONFIRMATIONS)
        self.assertNotIn("ApplyDamage", GET_CONFIRMATIONS)
        self.assertNotIn("GetDamageFraction", GET_CONFIRMATIONS)
        self.assertNotIn("ApplySystemHit", GET_CONFIRMATIONS)
        self.assertNotIn("ApplyHit", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CONFIRMATIONS)
        self.assertNotIn("IsSensorLive", GET_CONFIRMATIONS)
        self.assertNotIn("IsThermalAvailable", GET_CONFIRMATIONS)
        self.assertNotIn("IsSensorViewActive", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_leftover_chin_muzzle(self) -> None:
        ...
        for token in LEFTOVER_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_CONFIRMATIONS)
        self.assertNotIn("GetChinMuzzleLocation", GET_CONFIRMATIONS)
        self.assertNotIn("test_apache_chin_muzzle_tests.py", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONFIRMATIONS)
        self.assertNotIn("ESkyguardApacheSystem", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_leftover_cpg_feel(self) -> None:
        ...
        for token in LEFTOVER_CPG_FEEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONFIRMATIONS)
        self.assertNotIn("test_apache_cpg_feel_contract.py", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_leftover_pilot_command_roster(
        ...
        for token in LEFTOVER_PILOT_COMMAND_ROSTER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONFIRMATIONS)
        self.assertNotIn("test_pilot_command_roster_contract.py", GET_CONFIRMATIONS)

    def test_contract_does_not_relock_leftover_pilot_confirm_command(
        self,
    ) -> None:
        locked_only = f"{GET_CONFIRMATIONS}\n"
        for token in LEFTOVER_PILOT_CONFIRM_COMMAND_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONFIRMATIONS)
        self.assertNotIn(
            "test_pilot_confirm_command_decl_contract.py",
    ...
    def test_contract_does_not_relock_leftover_pilot_confirm_line(self) -> None:
        locked_only = f"{GET_CONFIRMATIONS}\n"
        for token in LEFTOVER_PILOT_CONFIRM_LINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CONFIRMATIONS)
        self.assertNotIn("test_pilot_confirm_line_contract.py", GET_CONFIRMATIONS)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_CONFIRMATIONS)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", GET_CONFIRMATIONS)
        self.assertNotIn("USkyguardBriefingWidget", GET_CONFIRMATIONS)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            GET_CONFIRMATIONS,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_CONFIRMATIONS)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_CONFIRMATIONS)
        self.assertNotIn("ESkyguardMissionSkylineStyle", GET_CONFIRMATIONS)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_CONFIRMATIONS}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_CONFIRMATIONS),
            GET_CONFIRMATIONS,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GetPilotConfirmationsIssued contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_CONFIRMATIONS.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "apache GetPilotConfirmationsIssued contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, GET_CONFIRMATIONS.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_CONFIRMATIONS)

    def test_contract_is_get_confirmations_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return PilotConfirmationsIssued", GET_CONFIRMATIONS)
        self.assertNotIn("{", GET_CONFIRMATIONS)
        self.assertTrue(GET_CONFIRMATIONS.startswith("int32 "))
        self.assertTrue(GET_CONFIRMATIONS.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
