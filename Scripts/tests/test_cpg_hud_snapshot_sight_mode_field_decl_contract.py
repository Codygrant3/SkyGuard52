# THIS IS leftover-safe FSkyguardCpgHudSnapshot SightMode.
# origin/main form: BARE plain C++ field
# `ESkyguardCpgSightMode SightMode =
# ESkyguardCpgSightMode::Helmet;`
# with in-struct initializer Helmet. NOT a UPROPERTY wrap.
...
# `ESkyguardCpgSightMode SightMode =
# ESkyguardCpgSightMode::Helmet;`.
# Fail-closed if SightMode is missing or renamed, if the
# type is not ESkyguardCpgSightMode, if the initializer is
# missing or not `= ESkyguardCpgSightMode::Helmet`, or if
...
# SightMode is NOT leftover sibling SightLine
# (`FString SightLine;`).
...
# StationStatus / LockPhase / RangeMeters / ThreatCount /
...
# #1505. Keep those files in LOCKED_SCRIPTS.
...
# leftover `ESkyguardMissionWeather::Storm` / leftover
# `FString WeaponLine;` / leftover `FString SightLine;`,
# retarget: type is ESkyguardCpgSightMode, identifier is
# SightMode, initializer is ESkyguardCpgSightMode::Helmet,
...
TARGET_WRONG_BARE = "ESkyguardCpgSightMode SightMode;"
TARGET_WRONG_SENSOR = (
    "ESkyguardCpgSightMode SightMode = "
    "ESkyguardCpgSightMode::TargetingSensor;"
)
TARGET_WRONG_EQ_NONE = "ESkyguardCpgSightMode SightMode = NAME_None;"
TARGET_WRONG_FALSE = "ESkyguardCpgSightMode SightMode = false;"
TARGET_WRONG_TRUE = "ESkyguardCpgSightMode SightMode = true;"
TARGET_WRONG_ZERO = "ESkyguardCpgSightMode SightMode = 0.f;"
TARGET_WRONG_FLOAT = "float SightMode;"
TARGET_WRONG_HEALTH = "float SightMode = 160.f;"
TARGET_WRONG_BOOL = "bool SightMode;"
TARGET_WRONG_INT = "int32 SightMode;"
TARGET_WRONG_FNAME = "FName SightMode;"
TARGET_WRONG_TCHAR = 'const TCHAR* SightMode = TEXT("");'
TARGET_WRONG_SIGHT_LINE = "FString SightLine;"
...
TARGET_WRONG_LOCK_PHASE_ON_SIGHT = (
    "ESkyguardGuidedLockPhase SightMode = "
    "ESkyguardGuidedLockPhase::Search;"
)
TARGET_WRONG_STORM_WEATHER = (
    "ESkyguardMissionWeather SightMode = "
...
SIGHT_MODE_DECL_RE = re.compile(
    r"ESkyguardCpgSightMode\s+SightMode\s*=\s*"
    r"ESkyguardCpgSightMode::Helmet\s*;"
)
SIGHT_MODE_INIT_RE = re.compile(
    r"ESkyguardCpgSightMode\s+SightMode\s*=\s*([^;]+);"
)
SIGHT_MODE_BARE_RE = re.compile(r"ESkyguardCpgSightMode\s+SightMode\s*;")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def leftover_wrong_sight_mode_inits() -> tuple[str, ...]:
    return (
        TARGET_WRONG_BARE,
        TARGET_WRONG_SENSOR,
...
def sight_mode_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardCpgSightMode\s+SightMode\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardCpgSightMode SightMode =
    # ESkyguardCpgSightMode::Helmet;`.
    # Do not accept bare SightMode, leftover TargetingSensor,
    # leftover sibling SightLine, leftover WeaponLine,
    # leftover LockPhase, leftover StormRain Weather,
    # leftover profile #1353 Clear, leftover ContactMark
    # Label / leftover debrief MissionTitle. Do not accept
    # leftover LoadoutSpec FlareCount. Do not accept
    # UPROPERTY / Category clones. Do not accept
    # `= NAME_None` / `= false` / `= true` / `= 0.f` /
    # `= 160.f` when origin/main locks Helmet.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SIGHT_MODE_DECL_RE.search(compact) is None:
        return False
    if SIGHT_MODE_BARE_RE.search(compact):
        return False
    if re.search(r"float\s+SightMode\b", compact):
        return False
    if re.search(r"bool\s+SightMode\b", compact):
        return False
    if re.search(r"int32\s+SightMode\b", compact):
        return False
    if re.search(r"FName\s+SightMode\b", compact):
        return False
    if re.search(r"FString\s+SightMode\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*SightMode\b", compact):
        return False
    if re.search(r"ESkyguardGuidedLockPhase\s+SightMode\b", compact):
        return False
    if re.search(r"ESkyguardMissionWeather\s+SightMode\b", compact):
        return False
    for match in SIGHT_MODE_INIT_RE.finditer(compact):
        init = collapsed(match.group(1))
        if init != "ESkyguardCpgSightMode::Helmet":
            return False
    if sight_mode_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardCpgSightMode\s+SightMode\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardCpgSightMode SightMode is missing "
...
def require_no_uproperty_wrap(section: str) -> None:
    ...
    if re.search(
        ...
        raise AssertionError(
            "UPROPERTY wrap on ESkyguardCpgSightMode SightMode is "
            f"not the locked decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "SightMode"):
        raise AssertionError(
            "UPROPERTY clone landed on SightMode; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "SightMode"):
            raise AssertionError(
                f"{token} clone landed on SightMode; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotSightModeFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SightMode"), section)
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SIGHT_LINE))
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardCpgSightMode SightMode"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("= ESkyguardCpgSightMode::Helmet", LOCKED_DECL)
        self.assertNotIn("= ESkyguardCpgSightMode::TargetingSensor", LOCKED_DECL)
        self.assertTrue(has_identifier(LOCKED_DECL, "SightMode"))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SIGHT_LINE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_WEAPON_LINE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_LOCK_PHASE))
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIGHT_LINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RANGE)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("SightMode", str(raised.exception))
    ...
    def test_does_not_claim_leftover_storm_rain_or_weather_profile(
        ...
        self.assertIn(LEFTOVER_STORM_RAIN_KIT, storm_header)
        self.assertIn(
            "ESkyguardMissionWeather Weather = "
            "ESkyguardMissionWeather::Storm;",
            storm_header,
        ...
        section = spec_section(leftover_as_this)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SightMode"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
    ...
    def test_missing_sight_mode_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEAPON}\n"
            f"\t{TARGET_WRONG_SIGHT_LINE}\n"
    ...
    def test_sight_line_does_not_satisfy_sight_mode(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SIGHT_LINE}\n"
        ...
        self.assertTrue(has_identifier(section, SIBLING_SIGHT_LINE), section)
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SIGHT_LINE))

    def test_weapon_line_does_not_satisfy_sight_mode(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEAPON}\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        section = spec_section(leftover_sensor)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SightMode", str(raised.exception))
        ...
        self.assertNotIn(
            "SightMode = ESkyguardCpgSightMode::TargetingSensor",
            compact_origin,
        )
        self.assertNotIn("SightMode = NAME_None", compact_origin)
        self.assertNotIn("SightMode = false", compact_origin)
        self.assertNotIn("SightMode = true", compact_origin)
        self.assertNotIn("SightMode = 0.f", compact_origin)
        self.assertNotIn("SightMode = 160.f", compact_origin)

    def test_sight_mode_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("ESkyguardCpgSightMode ", LOCKED_DECL)
        ...
        for wrong in leftover_wrong_sight_mode_inits():
            self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_SENSOR}\n", LOCKED_DECL)
        self.assertIn("SightMode", str(raised.exception))
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_SIGHT_LINE}\n", LOCKED_DECL)
        self.assertIn("SightMode", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_LOCK_PHASE_ON_SIGHT}\n",
            f"\t{TARGET_WRONG_STORM_WEATHER}\n",
            ...
            f"\t{LEFTOVER_CONTACT_LABEL}\n",
            "\tESkyguardCpgSightMode SightModes = "
            "ESkyguardCpgSightMode::Helmet;\n",
            "\tint32 SightMode;\n",
            "\tbool SightMode;\n",
            "\tfloat SightMode = " + forty + ";\n",
            "\tfloat SightMode = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    sight_mode_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(sight_mode_has_uproperty_wrap(origin), origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tESkyguardCpgSightMode\n"
            "\tSightMode = ESkyguardCpgSightMode::Helmet;\n",
            "\tESkyguardCpgSightMode   SightMode = "
            "ESkyguardCpgSightMode::Helmet;\n",
            "\tESkyguardCpgSightMode\tSightMode = "
            "ESkyguardCpgSightMode::Helmet;\n",
    ...
    def test_does_not_contract_sibling_hud_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_SIGHT_LINE), section)
        self.assertIn(TARGET_WRONG_SIGHT_LINE, section)
        self.assertNotIn(TARGET_WRONG_SIGHT_LINE, LOCKED_DECL)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        for helper in namespace_helper_tokens():
            self.assertNotIn(helper, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{SIGHT_LABEL}();\n", LOCKED_DECL)
        self.assertIn("SightMode", str(raised.exception))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot SightMode field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "sight_mode_field_decl_contract.py"
        ))
        self.assertIn("cpg_hud_snapshot", Path(__file__).name)
        self.assertIn("sight_mode", Path(__file__).name)
        self.assertNotIn("sight_line_field", Path(__file__).name)
        self.assertNotIn("weapon_line_field", Path(__file__).name)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_SIGHT_LINE))
        self.assertFalse(has_identifier(locked_only, SIBLING_WEAPON_LINE))
        self.assertFalse(has_identifier(locked_only, SIBLING_LOCK_PHASE))
        self.assertNotIn(HUD_SIBLING_FLARE_DECL, locked_only)
        self.assertNotIn(LEFTOVER_LOADOUT_FLARE_DECL, locked_only)
        self.assertNotIn(TARGET_WRONG_SIGHT_LINE, locked_only)
        self.assertNotIn(TARGET_WRONG_BARE, locked_only)
        self.assertNotIn(TARGET_WRONG_SENSOR, locked_only)
        self.assertNotIn(TARGET_WRONG_STORM_WEATHER, locked_only)
        for field in leftover_neighbor_hit_fields():
