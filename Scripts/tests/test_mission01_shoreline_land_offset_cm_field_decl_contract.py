...
# INDEX_NONE or lock ShorelineLandOffsetCm construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# ShorelineLandOffsetCm distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep ShorelineLandOffsetCm
...
# (in-flight this wave) / BeachWidthCm /
...
# one-line / split-line wraps. Locked 5200.f is
...
# 5200.f value. Do not lock leftover
...
LOCKED_SHORELINE_LAND_OFFSET_VALUE = "5200.f"
...
# ShorelineLandOffsetCm field declaration contract on
...
# RouteCorridorHalfWidthCm / environment Root
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::ShorelineLandOffsetCm",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01ShorelineLandOffsetCmFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SHORELINE_LAND_OFFSET_CM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SHORELINE_LAND_OFFSET_CM_FIELD))

    def test_missing_shoreline_land_offset_cm_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("EditAnywhere", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("EditInstanceOnly", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("BlueprintReadWrite", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("VisibleAnywhere", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("BlueprintReadOnly", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("Category", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("ClampMin", SHORELINE_LAND_OFFSET_CM_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tfloat* ShorelineLandOffsetCm;\n"
        )
        as_array = (
            "\tTArray<float> ShorelineLandOffsetCm;\n"
        )
        no_init = "\tfloat ShorelineLandOffsetCm;\n"
        int_type = "\tint32 ShorelineLandOffsetCm = 45000;\n"
        double_type = "\tdouble ShorelineLandOffsetCm = 5200.f;\n"
        wrong_init = "\tfloat ShorelineLandOffsetCm = 0.f;\n"
        wrong_scale = "\tfloat ShorelineLandOffsetCm = 40000.f;\n"
        scene = "\tTObjectPtr<USceneComponent> ShorelineLandOffsetCm;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> ShorelineLandOffsetCm;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> ShorelineLandOffsetCm;\n"
        )
        weak = (
        ...
        assigned = (
            "\tfloat ShorelineLandOffsetCm = nullptr;\n"
        )
        soft = (
        ...
        leftover_shore = f"\t{ROUTE_LENGTH_CM_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SHORELINE_LAND_OFFSET_CM_FIELD)
            self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_shoreline_land_offset_cm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SHORELINE_LAND_OFFSET_CM_FIELD),
            SHORELINE_LAND_OFFSET_CM_FIELD,
        )
        self.assertTrue(has_declaration(section, SHORELINE_LAND_OFFSET_CM_FIELD))
        self.assertEqual(
            declaration_count(section, SHORELINE_LAND_OFFSET_CM_FIELD),
            1,
        )
        self.assertTrue(
            SHORELINE_LAND_OFFSET_CM_FIELD.startswith("float ShorelineLandOffsetCm"),
            SHORELINE_LAND_OFFSET_CM_FIELD,
        )
        self.assertTrue(SHORELINE_LAND_OFFSET_CM_FIELD.endswith(";"), SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn(
            "float",
            SHORELINE_LAND_OFFSET_CM_FIELD,
        )
        self.assertIn("=", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn(LOCKED_SHORELINE_LAND_OFFSET_VALUE, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("TObjectPtr<", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("TArray<", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("TSoftObjectPtr", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("nullptr", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("INDEX_NONE", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("UFUNCTION", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("{", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("}", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("return ", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("OceanTiles", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("LandTiles", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("RouteExclusion", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("bRouteExclusionValid", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("BeachTiles", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("InlandVegetationPCG", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("LandScatterBounds", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("ProductionLandscape", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("AuthoredPCGGraph", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("DistrictLengthCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("RouteLengthCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("BeachWidthCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("InlandExtentCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("SeawardExtentCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            SHORELINE_LAND_OFFSET_CM_FIELD,
        )
        ...
        self.assertNotIn("Root", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetReadiness", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("Briefing", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("AudioDirector", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("RadioChatter", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("SortiePresentation", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("CampaignDefinition", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("MissionDefinition", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("bAutoInitialize", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetAircraft", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("ConfigureFromMission", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("AdvanceBriefing", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("SetAssetsReady", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("CanLaunch", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetElapsedSeconds", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetBriefingState", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetBriefingText", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetRadioChatter", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetPresentation", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("GetMissionTitle", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("AcknowledgeBriefing", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("LaunchSortie", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("HullCollider", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("OpticalTracker", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("WeaponServo", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("CountermeasurePod", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("RadarNode", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("HarborIndustrial", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("MaxIntegrity", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", SHORELINE_LAND_OFFSET_CM_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT} {SHORELINE_LAND_OFFSET_CM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{SHORELINE_LAND_OFFSET_CM_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, SHORELINE_LAND_OFFSET_CM_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SHORELINE_LAND_OFFSET_CM_FIELD),
            section,
    ...
    def test_assigned_shoreline_land_offset_cm_does_not_satisfy(self) -> None:
        assigned = (
            "\tfloat ShorelineLandOffsetCm = 0.f;\n"
        )
        missing_init = "\tfloat ShorelineLandOffsetCm;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, SHORELINE_LAND_OFFSET_CM_FIELD))
        with self.assertRaises(AssertionError) as raised_missing:
            require_declaration(missing_init, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", str(raised_missing.exception))
        self.assertFalse(has_declaration(missing_init, SHORELINE_LAND_OFFSET_CM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, SHORELINE_LAND_OFFSET_CM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{SHORELINE_LAND_OFFSET_CM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            SHORELINE_LAND_OFFSET_CM_FIELD.startswith("UFUNCTION"),
            SHORELINE_LAND_OFFSET_CM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", SHORELINE_LAND_OFFSET_CM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_shoreline_land_offset_cm_cpp_body(self) -> None:
        locked_only = f"{SHORELINE_LAND_OFFSET_CM_FIELD}\n"
        self.assertNotIn("{", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("}", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("return ", SHORELINE_LAND_OFFSET_CM_FIELD)
        ...
        self.assertNotIn("return false", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("return true", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertTrue(
            SHORELINE_LAND_OFFSET_CM_FIELD.startswith("float ShorelineLandOffsetCm")
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, LOCKED_SHORELINE_LAND_OFFSET_VALUE)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn(LOCKED_SHORELINE_LAND_OFFSET_VALUE, leftover_harbor_tokens())
    ...
    def test_harbor_adjacent_fields_are_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual(LOCKED_SHORELINE_LAND_OFFSET_VALUE, token)
            self.assertNotIn(token, leftover_harbor_tokens())

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SHORELINE_LAND_OFFSET_CM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, SHORELINE_LAND_OFFSET_CM_FIELD),
            SHORELINE_LAND_OFFSET_CM_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SHORELINE_LAND_OFFSET_CM_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, SHORELINE_LAND_OFFSET_CM_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 ShorelineLandOffsetCm field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{SHORELINE_LAND_OFFSET_CM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SHORELINE_LAND_OFFSET_CM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SHORELINE_LAND_OFFSET_CM_FIELD.lower(),
                "mission01 ShorelineLandOffsetCm contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SHORELINE_LAND_OFFSET_CM_FIELD)

    def test_contract_is_shoreline_land_offset_cm_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("ShorelineLandOffsetCm", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertTrue(SHORELINE_LAND_OFFSET_CM_FIELD.endswith(";"))
        self.assertIn("=", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn(LOCKED_SHORELINE_LAND_OFFSET_VALUE, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotIn("TObjectPtr<", SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT, section)

    def test_sibling_environment_fields_do_not_satisfy_shoreline_land_offset_cm(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, SHORELINE_LAND_OFFSET_CM_FIELD))
            self.assertNotEqual(SHORELINE_LAND_OFFSET_CM_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_shoreline_land_offset_cm(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            ...
            "\t * This comment is not the ShorelineLandOffsetCm declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn("ShorelineLandOffsetCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, SHORELINE_LAND_OFFSET_CM_FIELD)
        )
        self.assertNotIn("float ShorelineLandOffsetCm = 5200.f;", comment_only)
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SHORELINE_LAND_OFFSET_CM_FIELD)
            self.assertNotEqual(token, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertIn(LOCKED_SHORELINE_LAND_OFFSET_VALUE, SHORELINE_LAND_OFFSET_CM_FIELD)
        self.assertNotEqual(LOCKED_SHORELINE_LAND_OFFSET_VALUE, "7500.f")
        self.assertNotEqual(LOCKED_SHORELINE_LAND_OFFSET_VALUE, "2800.f")
        self.assertNotEqual(LOCKED_SHORELINE_LAND_OFFSET_VALUE, "45000.f")
        self.assertNotEqual(LOCKED_SHORELINE_LAND_OFFSET_VALUE, "1800.f")
        self.assertNotEqual(LOCKED_SHORELINE_LAND_OFFSET_VALUE, "18000.f")
        self.assertNotEqual(LOCKED_SHORELINE_LAND_OFFSET_VALUE, "20000.f")
