# THIS IS leftover-safe FSkyguardCampaignMissionSpec BeatSeconds.
# origin/main form: BARE plain C++ field
# `float BeatSeconds[7] = {120.f, 240.f, 360.f, 480.f, 600.f, 780.f, 900.f};`
# inside plain struct FSkyguardCampaignMissionSpec only.
# NOT a UPROPERTY wrap. NOT USTRUCT. NO Category.
...
# Fail-closed if clone still asserts Category="Skyguard|Theater" /
# FName WeatherIdentity / `= 160.f` / leftover Harbor 40/80.
# THIS SLOT IS NOT leftover Harbor Breaker Approach/Contact/Shore.
# Harbor fail-closed is ONLY 40/80. BeatSeconds uses
# 120/240/360/480/600/780/900. Fail-closed if this test invents
# Harbor 40/80 as the locked initializer.
# THIS IS NOT leftover analog campaign-roster-lookup-tests.
# THIS IS NOT leftover StormRainBeatKit BeatCount.
# THIS IS NOT leftover CampaignMissionSpec TimeOfDayHours (12.f)
# or WeatherProfile TimeOfDayHours.
...
# Success / Failure / Weather / WeatherIdentity / WeatherLabel /
# TimeOfDayHours / ContactKind / ShoreKind / SupportKind /
# ExtractKind / Climax / bNightIdentity / bStormRocketContract.
# Clone LOCKED_SCRIPTS from leftover-safe TheaterKitSpec
# WeatherIdentity, then leftover-safe CampaignMissionSpec
# MissionId parse-window retarget. Clone is UPROPERTY-based.
# RETARGET: type is float BeatSeconds[7], initializer is
# {120.f, 240.f, 360.f, 480.f, 600.f, 780.f, 900.f}, locked
# decl is the plain field, parse window is
# FSkyguardCampaignMissionSpec.
...
TARGET_WRONG_BARE = "float BeatSeconds[7];"
TARGET_WRONG_EQ_NONE = "FName BeatSeconds = NAME_None;"
TARGET_WRONG_FALSE = "bool BeatSeconds = false;"
TARGET_WRONG_TRUE = "bool BeatSeconds = true;"
TARGET_WRONG_ZERO = "float BeatSeconds[7] = {0.f};"
TARGET_WRONG_FNAME = "FName BeatSeconds;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_BOOL = "bool BeatSeconds;"
TARGET_WRONG_INT = "int32 BeatSeconds;"
TARGET_WRONG_BEAT_COUNT = "static constexpr int32 BeatCount = 7;"
TARGET_WRONG_TIME_OF_DAY = "float TimeOfDayHours = 12.f;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_campaign_mission_spec_beat_seconds"
...
def leftover_neighbor_field_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_STORM_KIT_BEAT_COUNT,
        LEFTOVER_WEATHER_PROFILE_TIME_OF_DAY,
    )
...
LOCKED_SCRIPTS = (
    ...
    + leftover_neighbor_field_scripts()
    + (
        CLONE_THEATER_WEATHER_IDENTITY_SCRIPT,
        LEFTOVER_CONTACT_MARK_WORLD,
        LEFTOVER_ANALOG_STORM_KIT_BULK,
...
def leftover_harbor_interval_literals() -> tuple[str, ...]:
    forty = "40" + ".f"
    eighty = "80" + ".f"
    return (forty, eighty, forty + ", " + eighty)


def leftover_harbor_tokens() -> tuple[str, ...]:
    return leftover_harbor_clock_tokens() + leftover_harbor_interval_literals()


def harbor_interval_token_re(value: str) -> re.Pattern[str]:
    # Harbor 40/80 must not match leftover-safe 240.f / 480.f.
    return re.compile(r"(?<![0-9])" + re.escape(value) + r"(?![0-9A-Za-z])")
...
LOCKED_ARRAY_RE = re.compile(
    r"float\s+BeatSeconds\s*\[\s*7\s*\]\s*=\s*"
    r"\{120\.f,240\.f,360\.f,480\.f,600\.f,780\.f,900\.f\}"
)
...
def beat_seconds_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*float\s+BeatSeconds\b",
        compact,
    ) is not None


def beat_seconds_uses_harbor_clock(region: str) -> bool:
    compact = collapsed(region)
    forty = "40" + ".f"
    eighty = "80" + ".f"
    match = re.search(
        r"float\s+BeatSeconds\s*\[[^\]]*\]\s*=\s*\{([^}]*)\}",
        compact,
    )
    if match is None:
        return False
    body = match.group(1)
    if harbor_interval_token_re(forty).search(body):
        return True
    if harbor_interval_token_re(eighty).search(body):
        return True
    return False
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `float BeatSeconds[7] = {120.f, 240.f, 360.f, 480.f, 600.f, 780.f, 900.f};`.
    # Do not accept a missing initializer, leftover Harbor 40/80,
    # leftover TimeOfDayHours 12.f, leftover BeatCount, leftover
    # float Health = 160.f, leftover FName MissionId, or leftover
    # TheaterKit UPROPERTY / Category clones. Do not accept sibling
    # MissionId / Title / WeatherIdentity / TimeOfDayHours fields.
    if declaration != LOCKED_DECL:
        return False
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LOCKED_ARRAY_RE.search(compact) is None:
        return False
    if beat_seconds_uses_harbor_clock(region):
        return False
    if re.search(r"float\s+BeatSeconds\s*\[\s*7\s*\]\s*;", compact):
        return False
    if re.search(r"FName\s+BeatSeconds\b", compact):
        return False
    if re.search(r"bool\s+BeatSeconds\b", compact):
        return False
    if re.search(r"int32\s+BeatSeconds\b", compact):
        return False
    if re.search(
        r"static\s+constexpr\s+int32\s+BeatCount\b",
        compact,
    ):
        return False
    if beat_seconds_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+BeatSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float BeatSeconds is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    ...
    if re.search(
        ...
        raise AssertionError(
            "UPROPERTY wrap on float BeatSeconds is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "BeatSeconds"):
        raise AssertionError(
            "UPROPERTY clone landed on BeatSeconds; locked decl is "
            f"bare {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "BeatSeconds"):
            raise AssertionError(
                f"{token} clone landed on BeatSeconds; locked decl is "
                f"bare {LOCKED_DECL}"
            )


class CampaignMissionSpecBeatSecondsFieldDeclContractTests(unittest.TestCase):
    def test_campaign_mission_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BeatSeconds"), section)
        ...
        self.assertNotIn(LEFTOVER_BEAT_COUNT, LOCKED_DECL)
    ...
    def test_locked_decl_is_bare_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("float BeatSeconds[7]"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("[7]", LOCKED_DECL)
        self.assertIn(LOCKED_INITIALIZER, LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TIME_OF_DAY)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("BeatSeconds", str(raised.exception))
    ...
    def test_does_not_claim_leftover_storm_day_night_or_result(self) -> None:
        ...
        self.assertIn(TARGET_WRONG_BEAT_COUNT, storm_header)
        self.assertNotIn("float BeatSeconds[7]", storm_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn("float BeatSeconds[7]", day_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn("float BeatSeconds[7]", night_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn(LEFTOVER_WEATHER_PROFILE, result_header)
        self.assertIn(TARGET_WRONG_TIME_OF_DAY, result_header)
        self.assertNotIn("float BeatSeconds[7]", result_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BeatSeconds"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_missing_beat_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_mission_id_does_not_satisfy_beat_seconds(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_MISSION_ID}\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertTrue(beat_seconds_uses_harbor_clock(harbor_section))
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIsNotNone(LOCKED_ARRAY_RE.search(compact_origin))
        self.assertNotIn("BeatSeconds = NAME_None", compact_origin)
        self.assertNotIn("BeatSeconds = false", compact_origin)
        self.assertNotIn("BeatSeconds = true", compact_origin)
        self.assertNotIn("BeatSeconds[7] = {0.f}", compact_origin)
        self.assertNotIn("Health = 160.f", compact_origin)
        self.assertFalse(beat_seconds_uses_harbor_clock(origin))

    def test_beat_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float BeatSeconds[7]"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BEAT_COUNT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(
                f"\t{TARGET_WRONG_WEATHER_IDENTITY}\n",
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 BeatSeconds;\n",
            "\tbool BeatSeconds;\n",
            "\tfloat BeatSeconds[7] = {" + forty + "};\n",
            "\tfloat BeatSeconds[7] = {" + eighty + "};\n",
            "\tfloat BeatSeconds[2] = {" + forty + ", " + eighty + "};\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    beat_seconds_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(beat_seconds_has_uproperty_wrap(origin), origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        rejected = (
            ...
            f"\t{TARGET_WRONG_TIME_OF_DAY}\n",
            f"\t{TARGET_WRONG_BEAT_COUNT}\n",
            "\tfloat BeatSeconds[7] = {" + forty + ", " + eighty + "};\n",
        )
    ...
    def test_does_not_contract_sibling_spec_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BeatSeconds"))
        self.assertFalse(has_identifier(section, LEFTOVER_FLARE_COUNT))
        self.assertFalse(has_identifier(section, LEFTOVER_BEAT_COUNT))
        for field in leftover_neighbor_hit_fields():
    ...
    def test_does_not_parse_leftover_kits_or_neighbors(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_BEAT_COUNT, header)
    ...
    def test_harbor_40_80_tokens_fail_closed_if_present(self) -> None:
        ...
        self.assertIsNone(harbor_interval_token_re(forty).search(LOCKED_DECL))
        self.assertIsNone(harbor_interval_token_re(eighty).search(LOCKED_DECL))
        self.assertIsNone(harbor_interval_token_re(forty).search(file_text))
        self.assertIsNone(harbor_interval_token_re(eighty).search(file_text))
        self.assertIn(LOCKED_INITIALIZER, LOCKED_DECL)
        self.assertNotEqual(
            LOCKED_INITIALIZER,
            "{" + forty + ", " + eighty + "}",
        ...
        for token in leftover_harbor_director_tokens():
            self.assertNotIn(token, section)
            self.assertNotIn(token, LOCKED_DECL)
        self.assertFalse(beat_seconds_uses_harbor_clock(section))
        self.assertFalse(beat_seconds_uses_harbor_clock(LOCKED_DECL))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec BeatSeconds field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "beat_seconds_field_decl_contract.py"
        ))
        ...
        self.assertNotIn("beat_count", Path(__file__).name)
        self.assertNotIn("time_of_day", Path(__file__).name)
        self.assertNotIn("roster_lookup", Path(__file__).name)
        self.assertNotIn("SkyguardCampaignRoster.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_STORM_KIT_BEAT_COUNT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_BEAT_COUNT, locked_only)
        for field in leftover_neighbor_hit_fields():
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_STORM_KIT_BEAT_COUNT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_WEATHER_PROFILE_TIME_OF_DAY)
