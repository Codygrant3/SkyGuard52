# THIS IS leftover-safe FSkyguardAudioTelemetry::RejectedMissingAsset.
# origin/main form: split-line
# UPROPERTY(BlueprintReadOnly)
# then `int32 RejectedMissingAsset = 0;` with no Category=.
...
# do not copy those; this field is `int32 RejectedMissingAsset = 0;`.
...
TARGET = "int32 RejectedMissingAsset = 0;"
TARGET_WRONG_BARE = "int32 RejectedMissingAsset;"
TARGET_WRONG_FALSE = "int32 RejectedMissingAsset = false;"
TARGET_WRONG_TRUE = "int32 RejectedMissingAsset = true;"
TARGET_WRONG_FLOAT = "int32 RejectedMissingAsset = 0.f;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # when origin/main has `int32 RejectedMissingAsset = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+RejectedMissingAsset\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RejectedMissingAsset\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RejectedMissingAsset\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RejectedMissingAsset\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+RejectedMissingAsset\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 RejectedMissingAsset is missing from "
...
class AudioTelemetryRejectedMissingAssetFieldDeclContractTests(unittest.TestCase):
    def test_audio_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("RejectedMissingAsset", section)
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("RejectedMissingAsset = 0", collapsed(origin))
        self.assertNotIn("RejectedMissingAsset = false", collapsed(origin))
    ...
    def test_played_events_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RejectedMissingAsset"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool RejectedMissingAsset = 0;\n",
            "\tint32 PlayedEvent = 0;\n",
            "\tint32 RejectedMissingAssets = 0;\n",
            "\tFText RejectedMissingAsset = 0;\n",
            "\tint32 RejectedMissingAsset{};\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio-telemetry RejectedMissingAsset field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_audio_telemetry_rejected_missing_asset"
            "_field_decl_contract.py"
        ))
