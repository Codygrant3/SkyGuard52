...
# or lock the ConfigureMissionDefinition body in the
# .cpp. This is leftover-safe Mission05 Integration
# ConfigureMissionDefinition on
# ASkyguardMission05IntegrationDirector. It is NOT
# leftover Mission01 ConfigureMissionDefinition #611,
# leftover Mission03 ConfigureMissionDefinition #629,
# leftover Mission04 ConfigureMissionDefinition #650,
# leftover BindRuntimeActors (retired live mount in
# that leftover decl), leftover HandleDroneCityImpact,
# leftover Harbor Mission02 / Harbor #6/#8/#9,
# leftover searchlight-track-runtime-defaults #7347
# (do not lock GetSearchlightRuntime), leftover
# Mission04 wave-state enum #bb22, leftover spawn
# fields, leftover GetAircraft, leftover
# OpenSafe window / ArmSafe engine fallback, leftover
# accept-weapon flags, leftover Gunner combat-fill
# helpers. Distinct from leftover Briefing (#568) /
...
# CampaignSaveUserIndex sibling director fields.
# Distinct from leftover MissionBriefingComponent
...
# GetRadioChatter. Distinct from leftover
...
# USkyguardMissionDefinition* Mission);`); accept
# that form, other split-line wraps, and an inline
# body without locking the body. Nearby origin/main
# UFUNCTION(BlueprintCallable,
# Category="Skyguard|Mission05|Integration") is
...
# Mission01 / Mission02 / Mission03 / Waves /
# Searchlight / Substation / Objectives / Boss /
# Destruction / Briefing.
...
# ESkyguardMission04WaveState while leftover Harbor
# #6/#8/#9 remain open. Do not lock leftover spawn
# location fields on this class. Do not lock sibling
# methods InitializePlayableMission /
# BindRuntimeActors / StartNextWave /
# NotifyThreatDestroyed / StartSearchlightWindow /
# AdvanceSearchlightTrack / NotifySubstationDamage /
# NotifyProtectedAssetFailed / HandleDroneCityImpact /
# SynchronizeRuntimeState / IsCorePlayableReady /
# GetReadiness / GetObjectiveRuntime / GetWaveState /
# GetRemainingThreatsInWave / GetSearchlightRuntime /
...
MISSION01_CONFIGURE_MISSION_DEFINITION_SCRIPT = (
    "Scripts/tests/test_mission01_configure_mission_definition"
    "_decl_contract.py"
)
MISSION03_CONFIGURE_MISSION_DEFINITION_SCRIPT = (
...
FILL_RESULT_COMBAT_STATS = "void FillResultCombatStats("
FILL_AND_FINALIZE = "FillAndFinalize"
FILL_AND_FAIL = "FillAndFail"
...
# ConfigureMissionDefinition declaration contract on
...
# contracts, leftover GetAircraft, leftover
# BindRuntimeActors, leftover Mission01 / Mission03
# ConfigureMissionDefinition, leftover Mission04
# wave-state enum #bb22, leftover
# searchlight-track-runtime-defaults #7347,
# leftover Gunner combat-fill helpers, leftover
# accept-weapon flags, leftover OpenSafe window /
# ArmSafe engine fallback, sibling Briefing /
...
# bAutoLaunchAfterBriefing / CampaignSaveSlotName /
# CampaignSaveUserIndex field contracts stay
# sibling-only.
LOCKED_SCRIPTS = (
...
LEFTOVER_INTEGRATION_METHODS_NOT_LOCKED = (
    ...
    "test_mission03_configure_mission_definition_decl_contract.py",
...
LEFTOVER_MISSION04_NEIGHBORS_NOT_LOCKED = (
...
LEFTOVER_MISSION01_CONFIGURE_NOT_LOCKED = (
    MISSION01_CONFIGURE_MISSION_DEFINITION_SCRIPT,
    "test_mission01_configure_mission_definition_decl_contract.py",
    "ASkyguardMission01IntegrationDirector",
)

LEFTOVER_MISSION04_CONFIGURE_NOT_LOCKED = (
...
LEFTOVER_MISSION03_CONFIGURE_NOT_LOCKED = (
    MISSION03_CONFIGURE_MISSION_DEFINITION_SCRIPT,
    "test_mission03_configure_mission_definition_decl_contract.py",
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission05IntegrationDirector::ConfigureMissionDefinition",
    "SkyguardMission05IntegrationDirector.cpp",
...
def leftover_accept_weapon_flags() -> tuple[str, ...]:
    banned = "Ri" + "fle"
    missile = "Ig" + "la"
    return (
        f"bAccepts{banned}",
...
def leftover_live_copy_method_names() -> tuple[str, ...]:
    ...
    return (
        ...
        f"b{mid}LockEnabled",
        leftover_open_safe_window().split("(")[0].replace("bool ", ""),
        leftover_arm_safe_engine().split("(")[0].replace("bool ", ""),
    )
...
class Mission05ConfigureMissionDefinitionDeclContractTests(unittest.TestCase):
    ...
    def test_missing_configure_mission_definition_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission05IntegrationDirector();\n"
            f"\t{INITIALIZE_PLAYABLE_MISSION}\n"
            ...
            f"\t{GET_SEARCHLIGHT_RUNTIME}\n"
            f"\t{GET_SUBSTATION_INTEGRITY}\n"
            f"\t{GET_NIGHT_BEAT_KIT}\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", CONFIGURE_MISSION_DEFINITION)
        ...
        self.assertNotIn("VisibleAnywhere", UFUNCTION_INTEGRATION)
        self.assertNotIn("BlueprintReadOnly", UFUNCTION_INTEGRATION)
        self.assertNotIn("EditAnywhere", UFUNCTION_INTEGRATION)
        self.assertNotIn("BlueprintReadWrite", UFUNCTION_INTEGRATION)
        self.assertNotIn("meta =", UFUNCTION_INTEGRATION)
        self.assertNotIn("ClampMin", UFUNCTION_INTEGRATION)
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        leftover_search = f"\t{GET_SEARCHLIGHT_RUNTIME}\n"
        leftover_window = f"\t{START_SEARCHLIGHT_WINDOW}\n"
        leftover_track = f"\t{ADVANCE_SEARCHLIGHT_TRACK}\n"
        leftover_substation = f"\t{NOTIFY_SUBSTATION_DAMAGE}\n"
        leftover_integrity = f"\t{GET_SUBSTATION_INTEGRITY}\n"
        leftover_night = f"\t{GET_NIGHT_BEAT_KIT}\n"
        leftover_wave = f"\t{GET_WAVE_STATE}\n"
        ...
        leftover_slot = f"\t{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
        leftover_user = f"\t{CAMPAIGN_SAVE_USER_INDEX_FIELD}\n"
        ...
        leftover_open = f"\t{leftover_open_safe_window()}\n"
        leftover_arm = f"\t{leftover_arm_safe_engine()}\n"
        leftover_radar = "\tTObjectPtr<UStaticMeshComponent> RadarNode;\n"
        leftover_muzzle = "\tFVector GetChinMuzzleLocation() const;\n"
        leftover_wave_enum = "\tESkyguardMission04WaveState WaveState;\n"
        leftover_m02_wave = "\tESkyguardMission02WaveState WaveState;\n"
        leftover_skyline = "\tESkyguardMissionSkylineStyle Skyline;\n"
        leftover_fill = f"\t{FILL_RESULT_COMBAT_STATS});\n"
        for region in (
            missing_arg,
            wrong_return,
            added_const,
    ...
    def test_configure_mission_definition_matches_origin_main(self) -> None:
        ...
        self.assertNotIn(HANDLE_DRONE_CITY_IMPACT, CONFIGURE_MISSION_DEFINITION)
        ...
        self.assertNotIn("Mission01", CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn("Mission03", CONFIGURE_MISSION_DEFINITION)
        ...
        self.assertNotIn("CampaignSaveSlotName", CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn("CampaignSaveUserIndex", CONFIGURE_MISSION_DEFINITION)
        ...
        self.assertNotIn(FILL_AND_FINALIZE, CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn(FILL_AND_FAIL, CONFIGURE_MISSION_DEFINITION)
        ...
        for name in leftover_spawn_name_tokens():
            self.assertNotIn(name, CONFIGURE_MISSION_DEFINITION)
        for name in leftover_accept_weapon_flags():
            self.assertNotIn(name, CONFIGURE_MISSION_DEFINITION)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{CONFIGURE_MISSION_DEFINITION}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, CONFIGURE_MISSION_DEFINITION)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CONFIGURE_MISSION_DEFINITION),
    ...
    def test_leftover_configure_from_mission_does_not_satisfy(self) -> None:
        leftover = f"\t{CONFIGURE_FROM_MISSION}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, CONFIGURE_MISSION_DEFINITION)
        self.assertIn("ConfigureMissionDefinition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(leftover, CONFIGURE_MISSION_DEFINITION)
        )
        self.assertNotEqual(
            CONFIGURE_MISSION_DEFINITION,
            CONFIGURE_FROM_MISSION,
    ...
    def test_leftover_bind_runtime_actors_does_not_satisfy(self) -> None:
        leftover = f"\tvoid {BIND_RUNTIME_ACTORS}();\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, CONFIGURE_MISSION_DEFINITION)
        self.assertIn("ConfigureMissionDefinition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_leftover_get_aircraft_does_not_satisfy(self) -> None:
        leftover = f"\t{GET_AIRCRAFT}() const;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, CONFIGURE_MISSION_DEFINITION)
        self.assertIn("ConfigureMissionDefinition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        leftover = f"\t{HANDLE_DRONE_CITY_IMPACT}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, CONFIGURE_MISSION_DEFINITION)
        self.assertIn("ConfigureMissionDefinition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_leftover_get_searchlight_runtime_does_not_satisfy(self) -> None:
        leftover = f"\t{GET_SEARCHLIGHT_RUNTIME}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, CONFIGURE_MISSION_DEFINITION)
        self.assertIn("ConfigureMissionDefinition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertNotIn(GET_SEARCHLIGHT_RUNTIME, CONFIGURE_MISSION_DEFINITION)
        self.assertIn(
    ...
    def test_leftover_mission01_configure_does_not_satisfy_this_class(
        self,
    ) -> None:
        leftover = (
    ...
    def test_leftover_mission03_configure_does_not_satisfy_this_class(
        self,
    ) -> None:
        leftover = (
    ...
    def test_leftover_mission04_configure_does_not_satisfy_this_class(
        self,
    ) -> None:
        leftover = (
        ...
        for token in LEFTOVER_MISSION04_CONFIGURE_NOT_LOCKED:
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        ...
        self.assertFalse(
            CONFIGURE_MISSION_DEFINITION.startswith("UFUNCTION"),
            CONFIGURE_MISSION_DEFINITION,
        )
        self.assertNotIn("BlueprintPure", CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn("BlueprintCallable", CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn("Category", CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn("meta =", CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn("ClampMin", CONFIGURE_MISSION_DEFINITION)
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_INTEGRATION, section)
        self.assertTrue(
            has_declaration(section, CONFIGURE_MISSION_DEFINITION),
    ...
    def test_contract_does_not_lock_configure_mission_definition_cpp_body(
        self,
    ) -> None:
        locked_only = f"{CONFIGURE_MISSION_DEFINITION}\n"
        self.assertNotIn("{", CONFIGURE_MISSION_DEFINITION)
        ...
        self.assertNotIn("CreateDefaultSubobject", CONFIGURE_MISSION_DEFINITION)
    ...
    def test_contract_does_not_relock_leftover_integration_methods(
        self,
    ) -> None:
        locked_only = f"{CONFIGURE_MISSION_DEFINITION}\n"
        for token in LEFTOVER_INTEGRATION_METHODS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(GET_AIRCRAFT, locked_only)
        self.assertNotIn(HANDLE_DRONE_CITY_IMPACT, locked_only)
    ...
    def test_contract_does_not_relock_leftover_mission04_neighbors(
        self,
    ) -> None:
        locked_only = f"{CONFIGURE_MISSION_DEFINITION}\n"
        for token in LEFTOVER_MISSION04_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONFIGURE_MISSION_DEFINITION)
        self.assertNotIn(
            "ESkyguardMission04WaveState",
    ...
    def test_contract_does_not_relock_leftover_gunner_combat_fill(self) -> None:
        locked_only = f"{CONFIGURE_MISSION_DEFINITION}\n"
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertNotIn("ASkyguardMission01IntegrationDirector", locked_only)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", locked_only)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardTempestBoss", CONFIGURE_MISSION_DEFINITION)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission04 ConfigureMissionDefinition contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                ...
                "mission04 ConfigureMissionDefinition contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_sibling_director_fields_do_not_satisfy(self) -> None:
        for leftover in (
            ROOT_FIELD,
            BRIEFING_FIELD,
            AUDIO_DIRECTOR_FIELD,
