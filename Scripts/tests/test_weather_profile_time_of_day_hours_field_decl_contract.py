# THIS IS leftover-safe FSkyguardWeatherProfile TimeOfDayHours.
...
# if the initializer is not `= 12.f`, if ClampMin /
# ClampMax are missing or wrong, or if Harbor tokens
...
# Do NOT contract sibling fields ProfileId / Weather /
# WindSpeedMetersPerSecond / Precipitation / CloudCoverage.
...
# THIS IS NOT leftover ProfileId draft #1350 / #c332.
...
# isolated TimeOfDayHours field decl, same as leftover
...
# float TimeOfDayHours, initializer is `= 12.f`,
# specifiers are EditAnywhere + BlueprintReadOnly +
# meta ClampMin "0.0" / ClampMax "24.0", no Category.
...
TARGET = "float TimeOfDayHours = 12.f;"
TARGET_WRONG_EQ_NONE = "float TimeOfDayHours = NAME_None;"
TARGET_WRONG_FALSE = "float TimeOfDayHours = false;"
TARGET_WRONG_TRUE = "float TimeOfDayHours = true;"
TARGET_WRONG_ZERO = "float TimeOfDayHours = 0.f;"
TARGET_WRONG_BARE = "float TimeOfDayHours;"
TARGET_WRONG_FNAME = "FName TimeOfDayHours;"
TARGET_WRONG_TWELVE_INT = "float TimeOfDayHours = 12;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float TimeOfDayHours = 12.f;`.
    # Do not accept bare `float TimeOfDayHours;`,
    # `= NAME_None` / `= false` / `= true` / `= 0.f` /
    # `= 160.f` / `= 12` / leftover float Health when
    # origin/main has `= 12.f`. Do not accept sibling
    # ProfileId / Weather / WindSpeedMetersPerSecond /
    # Precipitation / CloudCoverage fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FName\s+TimeOfDayHours\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+TimeOfDayHours\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+TimeOfDayHours\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+TimeOfDayHours\s*=\s*12\.f\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+TimeOfDayHours\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float TimeOfDayHours is missing from "
...
def require_time_of_day_hours_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} TimeOfDayHours UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} TimeOfDayHours"
        )
    ...
    if 'ClampMin = "0.0"' not in specifiers:
        raise AssertionError(
            "ClampMin is missing or wrong on origin/main:"
            f"{HEADER_PATH} struct {STRUCT_NAME} TimeOfDayHours UPROPERTY"
        )
    if 'ClampMax = "24.0"' not in specifiers:
        raise AssertionError(
            "ClampMax is missing or wrong on origin/main:"
            f"{HEADER_PATH} struct {STRUCT_NAME} TimeOfDayHours UPROPERTY"
        )
    return specifiers
...
class WeatherProfileTimeOfDayHoursFieldDeclContractTests(unittest.TestCase):
    def test_weather_profile_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "TimeOfDayHours"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "TimeOfDayHours"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("TimeOfDayHours", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_time_of_day_hours_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_time_of_day_hours_specifiers(section)
        ...
        self.assertIn('ClampMax = "24.0"', specifiers)
        self.assertIn(
            'meta = (ClampMin = "0.0", ClampMax = "24.0")',
            UPROPERTY_EDIT_READONLY,
        )
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float TimeOfDayHours = 12.f;", compact_origin)
        self.assertNotIn("TimeOfDayHours = NAME_None", compact_origin)
        self.assertNotIn("TimeOfDayHours = false", compact_origin)
        self.assertNotIn("TimeOfDayHours = true", compact_origin)
        self.assertNotIn("TimeOfDayHours = 0.f", compact_origin)
        self.assertNotIn("TimeOfDayHours = 160.f", compact_origin)
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BARE}\n", LOCKED_DECL)
        )

    def test_time_of_day_hours_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float TimeOfDayHours"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 12.f", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TWELVE_INT}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_TWELVE_INT}\n",
            f"\t{TARGET_WRONG_HEALTH}\n",
            leftover_primary,
            leftover_guided,
            f"\tFText {SIBLING_SPEAKER};\n",
            "\tFName TimeOfDayHours;\n",
            "\tint32 TimeOfDayHours = 12;\n",
            "\tbool TimeOfDayHours;\n",
            "\tfloat TimeOfDayHours = " + forty + ";\n",
            "\tfloat TimeOfDayHours = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_time_of_day_hours_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_time_of_day_hours_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_time_of_day_hours_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_time_of_day_hours_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_time_of_day_hours_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_time_of_day_hours_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_time_of_day_hours_specifiers(spec_section(wrong_max))
        self.assertIn("ClampMax", str(raised.exception))
        self.assertIn('ClampMin = "0.0"', origin)
        self.assertIn('ClampMax = "24.0"', origin)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMax = "24.0"', UPROPERTY_EDIT_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weather-profile TimeOfDayHours field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weather_profile_time_of_day_hours_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_PROFILE_ID_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_sibling_weather_profile_fields_stay_unclaimed(self) -> None:
        ...
        self.assertIn(LEFTOVER_PROFILE_ID_SCRIPT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_PROFILE_ID_SCRIPT, THIS_SCRIPT)
