# THIS IS leftover-safe FSkyguardMissionSaveRecord BestScore.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `int32 BestScore = 0;`.
...
# MigrateCampaignSave).
# Sibling decls stay unclaimed: bCompleted / BestMedalTier /
# BestCompletionTimeSeconds.
# Analog leftover mission-save-record-defaults #206 / #ac38
# then isolated BestScore field decl.
...
# `= false` / `= true` / bare int32 / bool, retarget:
# type is int32, initializer is `= 0`, BlueprintReadWrite,
# no Category.
...
TARGET = "int32 BestScore = 0;"
TARGET_WRONG_BARE = "int32 BestScore;"
TARGET_WRONG_TRUE = "int32 BestScore = true;"
TARGET_WRONG_ONE = "int32 BestScore = 1;"
TARGET_WRONG_FALSE = "int32 BestScore = false;"
TARGET_WRONG_FLOAT = "float BestScore = 0;"
TARGET_WRONG_FNAME = "FName BestScore;"
TARGET_WRONG_FTEXT = "FText BestScore;"
TARGET_WRONG_BOOL = "bool BestScore = 0;"
...
LEFTOVER_MISSION_SAVE_RECORD_COMPLETED = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `int32 BestScore = 0;`.
    # Do not accept bare / `= true` / `= false` / `= 1` /
    # `= 0.f` / leftover float Health when origin/main
    # has int32 = 0. Do not accept sibling bCompleted /
    # BestMedalTier / BestCompletionTimeSeconds fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+BestScore\s*=\s*true",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BestScore\s*=\s*false",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BestScore\s*=\s*1\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BestScore\s*=\s*0\.f",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BestScore\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+BestScore\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+BestScore\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+BestScore\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BestScore\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+BestScore\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 BestScore is missing from "
...
class MissionSaveRecordBestScoreFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_best_score_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 BestScore = 0;", compact_origin)
        self.assertNotIn("BestScore = true", compact_origin)
        self.assertNotIn("BestScore = false", compact_origin)
        self.assertNotIn("BestScore = 0.f", compact_origin)
        self.assertNotIn("BestScore = 160.f", compact_origin)

    def test_best_score_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 BestScore"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_COMPLETED};\n",
            f"\tFText {SIBLING_BEST_MEDAL};\n",
            "\tint32 BestScores = 0;\n",
            "\tbool BestScore = false;\n",
            "\tFText BestScore;\n",
            "\tfloat BestScore = " + forty + ";\n",
            "\tfloat BestScore = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-save-record BestScore field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "best_score_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_MISSION_SAVE_RECORD_COMPLETED, LOCKED_SCRIPTS)
