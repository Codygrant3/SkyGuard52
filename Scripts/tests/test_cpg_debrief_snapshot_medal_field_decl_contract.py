# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot::Medal.
# origin/main form is the BARE PLAIN C++ field:
# `int32 Medal = 0;`
...
# not `int32`, initializer is missing or is not `= 0`,
# or someone writes `= 0.f` / `= false` / `= 100`.
# Exact identifier vs siblings `Score`, `ShotsFired`,
# and `Hits` (same type+initializer).
# Do NOT claim sibling fields: bValid, bWon, MissionTitle,
# OutcomeNarrative, Score, ShotsFired, Hits,
...
# leftover retired live-copy types (split tokens).
...
TARGET = "int32 Medal = 0;"
TARGET_WRONG_FALSE = "int32 Medal = false;"
TARGET_WRONG_HUNDRED = "int32 Medal = 100;"
TARGET_WRONG_FLOAT = "int32 Medal = 0.f;"
TARGET_WRONG_BARE = "int32 Medal;"
TARGET_WRONG_BOOL = "bool Medal = 0;"
TARGET_WRONG_BOOL_FALSE = "bool Medal = false;"
TARGET_WRONG_SCORE = "int32 Score = 0;"
TARGET_WRONG_SHOTS = "int32 ShotsFired = 0;"
TARGET_WRONG_HITS = "int32 Hits = 0;"
LOCKED_DECL = TARGET
LEFTOVER_THEATER_CATEGORY = 'Category="Skyguard|Theater"'
...
STOP_BEFORE_PATROL_CLASS = "ASkyguardPatrolShipBoss"
...
SIBLING_OUTCOME = "OutcomeNarrative"
SIBLING_SCORE = "Score"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 Medal = 0;`.
    # Do not accept `= 0.f` / `= false` / `= 100` / bare `int32 Medal;`
    # / leftover siblings `int32 Score = 0;` / `int32 ShotsFired = 0;`
    # / `int32 Hits = 0;` (same type+initializer, different
    # identifier). Do not accept bool / float retargets.
    # `= 0` on int32 is not Harbor 40/80.
    compact_decl = collapsed(declaration)
    if re.search(r"\bint32\s+Medal\s*=\s*0\s*;", compact_decl) is None:
        return False
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"\bint32\s+Medal\s*=\s*0\s*;", compact) is None:
        return False
    if re.search(r"\bint32\s+Medal\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"\bint32\s+Medal\s*=\s*false\b", compact):
        return False
    if re.search(r"\bint32\s+Medal\s*=\s*100\b", compact):
        return False
    if re.search(r"\bint32\s+Medal\s*;", compact):
        return False
    if re.search(r"\bbool\s+Medal\b", compact):
        return False
    if re.search(r"\bfloat\s+Medal\b", compact):
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return leftover_function_tokens() + (
        ...
        f"class SKYGUARD52_API {STOP_BEFORE_PATROL_CLASS}",
        f"class {STOP_BEFORE_PATROL_CLASS} :",
...
def spec_section(header: str) -> str:
    ...
    for stop in leftover_function_tokens():
        if stop in section:
            raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str | None:
    # PLAIN struct: leftover clones that still require UPROPERTY
    # must fail-closed. Return None when no wrap is attached.
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+Medal\b", compact[index:]
        ):
...
class CpgDebriefSnapshotMedalFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_PATROL_CLASS)
        body = struct_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "Medal"), section)
        ...
        self.assertIn(STOP_BEFORE_CAPTURE_SIG, collapsed(header))
        self.assertNotIn(STOP_BEFORE_CAPTURE_SIG, collapsed(section))
        self.assertNotIn(STOP_BEFORE_CAPTURE_SIG, collapsed(body))
    ...
    def test_parse_window_stops_before_capture_and_copy_helpers(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        for stop in leftover_function_tokens():
            self.assertNotIn(stop, section)
            self.assertNotIn(stop, leaked)
        capture_at = header.find(STOP_BEFORE_CAPTURE_SIG)
        struct_at = header.find(f"struct {STRUCT_NAME}")
        self.assertGreater(capture_at, struct_at)
        self.assertGreater(capture_at, header.find(leaked))

    def test_does_not_parse_patrol_ship_class_body(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        leaked = struct_body(header)
        self.assertIn(f"class {STOP_BEFORE_PATROL_CLASS};", header)
        self.assertNotIn(f"class {STOP_BEFORE_PATROL_CLASS} :", header)
        self.assertNotIn(STOP_BEFORE_PATROL_CLASS, section)
        self.assertNotIn(STOP_BEFORE_PATROL_CLASS, leaked)
    ...
    def test_actor_or_helper_declaration_does_not_satisfy(self) -> None:
        mixed = (
        ...
        self.assertFalse(has_identifier(section, "Medal"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        self.assertNotIn(STOP_BEFORE_PATROL_CLASS, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Medal", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_medal_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SCORE}\n"
    ...
    def test_clone_uproperty_or_category_asserts_fail_closed(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_THEATER_CATEGORY, LOCKED_DECL)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, LOCKED_DECL)

    def test_invented_uproperty_wrap_is_not_origin_main(self) -> None:
        wrapped = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        self.assertIsNotNone(leftover_specs)
        self.assertIn("VisibleAnywhere", leftover_specs or "")
        self.assertIn("BlueprintReadOnly", leftover_specs or "")
        self.assertIn(LEFTOVER_THEATER_CATEGORY, leftover_specs or "")

    def test_medal_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 Medal"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_SCORE}\n", LOCKED_DECL)
        self.assertIn("Medal", str(raised.exception))

    def test_sibling_same_type_int32_zero_does_not_satisfy(self) -> None:
        self.assertEqual(TARGET_WRONG_SCORE, "int32 Score = 0;")
        self.assertEqual(TARGET_WRONG_SHOTS, "int32 ShotsFired = 0;")
        self.assertEqual(TARGET_WRONG_HITS, "int32 Hits = 0;")
        self.assertEqual(LOCKED_DECL, "int32 Medal = 0;")
        ...
        self.assertTrue(has_identifier(TARGET_WRONG_SCORE, SIBLING_SCORE))
        self.assertTrue(has_identifier(TARGET_WRONG_SHOTS, SIBLING_SHOTS))
        self.assertTrue(has_identifier(TARGET_WRONG_HITS, SIBLING_HITS))
        self.assertFalse(has_identifier(TARGET_WRONG_SCORE, "Medal"))
        self.assertTrue(has_identifier(LOCKED_DECL, "Medal"))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SCORE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_SHOTS))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_HITS))
        origin = spec_section(origin_main_header())
        self.assertTrue(has_identifier(origin, "Medal"))
        self.assertTrue(has_identifier(origin, SIBLING_SCORE))
        self.assertTrue(has_identifier(origin, SIBLING_SHOTS))
        self.assertTrue(has_identifier(origin, SIBLING_HITS))
        self.assertTrue(has_declaration(origin, LOCKED_DECL))
        self.assertFalse(has_declaration(origin, TARGET_WRONG_SCORE))
        self.assertFalse(has_declaration(origin, TARGET_WRONG_SHOTS))
        self.assertFalse(has_declaration(origin, TARGET_WRONG_HITS))
        for sibling_decl in (
            TARGET_WRONG_SCORE,
            TARGET_WRONG_SHOTS,
            TARGET_WRONG_HITS,
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for region in leftover_wraps:
            self.assertTrue(has_declaration(region, LOCKED_DECL), region)
            self.assertIsNotNone(attached_uproperty_specifiers(region))
    ...
    def test_does_not_parse_leftover_neighbors(self) -> None:
        ...
        leaked = struct_body(header)
        for stop in leftover_function_tokens():
            if stop == STOP_BEFORE_CAPTURE_SIG:
                self.assertIn(stop, collapsed(header))
            else:
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tFSkyguardCpgDebriefSnapshot {STOP_BEFORE_CAPTURE}();\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot Medal field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "medal_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_PY, LOCKED_SCRIPTS)
        self.assertIn(
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_TESTS,
            LOCKED_SCRIPTS,
        )
        self.assertIn(LEFTOVER_CPG_DEBRIEF_FAIL_CLOSED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_FAIL_CLOSED_TESTS, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_MESH_BIND_SLOT_NOTES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MESH_BIND_SLOT_PROXY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MESH_BIND_SLOT_SLOT_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MESH_BIND_SLOT_PREFERRED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIBLE_ACCEPTANCE_ACCEPTED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_RESULT_MISSION_SUCCEEDED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_VALID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ISOLATED_WON, LOCKED_SCRIPTS)
    ...
    def test_leftover_analog_bulk_and_fail_closed_stay_locked(self) -> None:
        leftovers = (
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS,
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_PY,
            LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DEFAULTS_TESTS,
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertIn("Medal", locked_only)
        for sibling in sibling_uncontracted_decls():
