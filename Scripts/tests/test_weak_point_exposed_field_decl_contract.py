...
# INDEX_NONE or lock bExposed construction in the
...
# only. Keep bExposed distinct from sibling
# WeakPointId (#566) and Integrity (in flight)
# plus unused sibling fields bDestroyed /
# MaxIntegrity and leftover accept-weapon flags
# on this component. Do not lock leftover accept
# flags. Do not lock leftover ApplyWeaponDamage /
# SetExposed / AcceptsWeapon methods
# (sibling-only). This is not leftover Apache
# MaxIntegrity / CurrentIntegrity. WeakPoint
...
# bExposed field declaration contract on
...
# (in flight), leftover boss-weak-point-defaults
# #2242, leftover LifelineHunter debris field
# contracts #563-#565, leftover RadarGhost /
# Breakwater / RoadHunter / RunwayBreaker field
# contracts #534-#562, leftover Pathfinder /
# Tempest / IronRain / BlackKite / LastFlight /
# BossDrone field contracts, leftover Apache
# HullCollider #425 stay sibling-only.
LOCKED_SCRIPTS = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardBossWeakPointComponent::bExposed",
    "SkyguardBossWeakPointComponent.cpp",
...
class WeakPointExposedFieldDeclContractTests(unittest.TestCase):
    def test_weak_point_component_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, EXPOSED_FIELD), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, EXPOSED_FIELD)
        self.assertIn("bExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, EXPOSED_FIELD))

    def test_missing_exposed_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardBossWeakPointComponent();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, EXPOSED_FIELD)
        self.assertIn("bExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, EXPOSED_FIELD)
        self.assertIn("bExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, EXPOSED_FIELD), section)
        self.assertNotIn("UPROPERTY", EXPOSED_FIELD)
        self.assertNotIn("EditAnywhere", EXPOSED_FIELD)
        self.assertNotIn("BlueprintReadWrite", EXPOSED_FIELD)
        self.assertNotIn("Category", EXPOSED_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, EXPOSED_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, EXPOSED_FIELD)
        self.assertIn("bExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_default = "\tbool bExposed;\n"
        false_assigned = "\tbool bExposed = false;\n"
        uint_type = "\tuint8 bExposed = true;\n"
        int_type = "\tint32 bExposed = true;\n"
        float_type = "\tfloat bExposed = true;\n"
        name_type = "\tFName bExposed = true;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, EXPOSED_FIELD)
            self.assertIn("bExposed", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_exposed_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, EXPOSED_FIELD),
            EXPOSED_FIELD,
        )
        self.assertTrue(has_declaration(section, EXPOSED_FIELD))
        self.assertEqual(declaration_count(section, EXPOSED_FIELD), 1)
        self.assertTrue(EXPOSED_FIELD.startswith("bool "), EXPOSED_FIELD)
        self.assertTrue(EXPOSED_FIELD.endswith(";"), EXPOSED_FIELD)
        self.assertIn("bExposed", EXPOSED_FIELD)
        self.assertIn("true", EXPOSED_FIELD)
        self.assertIn(" = ", EXPOSED_FIELD)
        self.assertNotIn("INDEX_NONE", EXPOSED_FIELD)
        self.assertNotIn("false", EXPOSED_FIELD)
        self.assertNotIn("TEXT(", EXPOSED_FIELD)
        self.assertNotIn("UFUNCTION", EXPOSED_FIELD)
        self.assertNotIn("{", EXPOSED_FIELD)
        self.assertNotIn("}", EXPOSED_FIELD)
        self.assertNotIn("return ", EXPOSED_FIELD)
        self.assertNotIn("ApplyWeaponDamage", EXPOSED_FIELD)
        self.assertNotIn("SetExposed", EXPOSED_FIELD)
        self.assertNotIn("AcceptsWeapon", EXPOSED_FIELD)
        self.assertNotIn("WeakPointId", EXPOSED_FIELD)
        self.assertNotIn("MaxIntegrity", EXPOSED_FIELD)
        self.assertNotIn("Integrity", EXPOSED_FIELD)
        self.assertNotIn("bDestroyed", EXPOSED_FIELD)
        self.assertNotIn("HullCollider", EXPOSED_FIELD)
        self.assertNotIn("OpticalTracker", EXPOSED_FIELD)
        self.assertNotIn("WeaponServo", EXPOSED_FIELD)
        self.assertNotIn("CountermeasurePod", EXPOSED_FIELD)
        for name in leftover_weak_point_accept_flags():
            self.assertNotIn(name, EXPOSED_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_BOSS} {EXPOSED_FIELD}\n"
            "};\n"
        )
        ...
        for header in headers:
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, EXPOSED_FIELD),
                section,
            )
            self.assertEqual(
                require_declaration(section, EXPOSED_FIELD),
            ...
            self.assertEqual(declaration_count(section, EXPOSED_FIELD), 1)
        one_line = f"{{\npublic:\n\t{EXPOSED_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, EXPOSED_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, EXPOSED_FIELD), section)
        self.assertEqual(
            require_declaration(section, EXPOSED_FIELD),
    ...
    def test_false_exposed_does_not_satisfy(self) -> None:
        assigned = "\tbool bExposed = false;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, EXPOSED_FIELD)
        self.assertIn("bExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, EXPOSED_FIELD))

    def test_uninitialized_exposed_does_not_satisfy(self) -> None:
        bare = "\tbool bExposed;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(bare, EXPOSED_FIELD)
        self.assertIn("bExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(bare, EXPOSED_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("bExposed", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, EXPOSED_FIELD))
    ...
    def test_leftover_methods_do_not_satisfy(self) -> None:
        for leftover in LEFTOVER_METHODS:
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, EXPOSED_FIELD))
            self.assertNotIn(leftover, EXPOSED_FIELD)

    def test_set_exposed_method_does_not_satisfy_field_lock(self) -> None:
        region = f"\t{SET_EXPOSED}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, EXPOSED_FIELD)
        self.assertIn("bExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, EXPOSED_FIELD))
        self.assertNotIn(SET_EXPOSED, EXPOSED_FIELD)
        self.assertNotIn("SetExposed", EXPOSED_FIELD)
    ...
    def test_delegate_param_form_does_not_satisfy_field_lock(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, EXPOSED_FIELD))
        self.assertNotIn(DELEGATE_PARAM_FORM, EXPOSED_FIELD)

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{EXPOSED_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, EXPOSED_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(EXPOSED_FIELD.startswith("UFUNCTION"), EXPOSED_FIELD)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, EXPOSED_FIELD), section)
    ...
    def test_contract_does_not_lock_exposed_cpp_body(self) -> None:
        locked_only = f"{EXPOSED_FIELD}\n"
        self.assertNotIn("{", EXPOSED_FIELD)
        self.assertNotIn("}", EXPOSED_FIELD)
        self.assertNotIn("return ", EXPOSED_FIELD)
        ...
        self.assertNotIn("return false", EXPOSED_FIELD)
        self.assertNotIn("return true", EXPOSED_FIELD)
        self.assertNotIn("CreateDefaultSubobject", EXPOSED_FIELD)
        self.assertNotIn("RefreshAuthoredWeakPointRegistry", EXPOSED_FIELD)
        self.assertNotIn("SetBossPhase", EXPOSED_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", EXPOSED_FIELD)
    ...
    def test_contract_does_not_relock_sibling_weak_point_id(self) -> None:
        ...
        for token in LEFTOVER_WEAK_POINT_ID_NOT_LOCKED:
            ...
            self.assertNotIn(token, EXPOSED_FIELD)
        self.assertIn(
            "Scripts/tests/test_weak_point_id_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{EXPOSED_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, EXPOSED_FIELD),
            EXPOSED_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, EXPOSED_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, EXPOSED_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", EXPOSED_FIELD)
        self.assertNotIn("CurrentIntegrity", EXPOSED_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", EXPOSED_FIELD)
        leftover_header = "SkyguardPathfinderEncounterController.h"
        self.assertNotIn(leftover_header, locked_only)
        self.assertNotIn(leftover_header, EXPOSED_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, EXPOSED_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", EXPOSED_FIELD)
    ...
    def test_weak_point_integrity_is_not_harbor_40_80(self) -> None:
        locked_only = f"{EXPOSED_FIELD}\n"
        section = public_section(origin_main_header())
        sibling_integrity = INTEGRITY_FIELD
        sibling_max = MAX_INTEGRITY_FIELD
        for token in leftover_harbor_tokens():
            self.assertNotEqual(token, sibling_integrity)
            self.assertNotEqual(token, sibling_max)
            self.assertNotEqual(token, "100.f")
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, EXPOSED_FIELD)
        self.assertTrue(has_declaration(section, sibling_integrity), section)
        self.assertTrue(has_declaration(section, sibling_max), section)
        self.assertNotIn(sibling_integrity, EXPOSED_FIELD)
        self.assertNotIn(sibling_max, EXPOSED_FIELD)
        self.assertNotIn("100.f", EXPOSED_FIELD)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weak point bExposed field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{EXPOSED_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, EXPOSED_FIELD.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, EXPOSED_FIELD)

    def test_contract_is_weak_point_exposed_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", EXPOSED_FIELD)
        self.assertNotIn("{", EXPOSED_FIELD)
        self.assertTrue(EXPOSED_FIELD.startswith("bool "))
        self.assertTrue(EXPOSED_FIELD.endswith(";"))
        self.assertIn("true", EXPOSED_FIELD)
        self.assertEqual(EXPOSED_FIELD, "bool bExposed = true;")
        self.assertIn(UPROPERTY_BOSS, section)
        self.assertEqual(
            HEADER_PATH,
