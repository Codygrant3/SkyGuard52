# THIS IS leftover-safe ASkyguardGunner CpgEufd.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgEufd;`
# (no initializer).
...
# Stop BEFORE sibling CpgTedacBezel as a claimed slot.
...
# CpgTedacBezel, CpgEufdText, Boom, GunnerCamera.
# Do NOT claim leftover CpgTedac / CpgDash / CpgCanopyBow /
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgCockpit /
# CpgStation (in-flight siblings). CpgMpdLeft and
# CpgMpdRight are sibling in-flight slots.
# Do NOT claim CpgTedac, CpgTedacBezel, or CpgEufdText
# as this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgEufd;`
...
# CpgEufd fail-closed if LOCKED_DECL is leftover
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgEufd;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgEufd = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgEufd = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgEufd = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgEufd;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgEufd;"
...
CPG_EUFD_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgEufd\s*;"
)
CPG_EUFD_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgEufd\s*="
)
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_TEDAC_BEZEL):
            break
    ...
    if has_identifier(window, SIBLING_CPG_TEDAC_BEZEL):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_TEDAC_BEZEL}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgEufd\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgEufd "
...
class GunnerCpgEufdFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgEufd"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_TEDAC_BEZEL), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_TEDAC_BEZEL),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgEufd"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgEufd", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_eufd_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgEufd;", compact_origin)
        self.assertNotIn("CpgEufd = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_eufd_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgEufd"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgEufd;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgEufds;\n",
            "\tint32 CpgEufd = 140;\n",
            "\tbool CpgEufd = true;\n",
            "\tfloat CpgEufd = " + forty + ";\n",
            "\tfloat CpgEufd = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_tedac_bezel_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "CpgEufd"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgEufd"))
        self.assertTrue(has_identifier(section, "CpgEufd"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgEufd field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_eufd_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
