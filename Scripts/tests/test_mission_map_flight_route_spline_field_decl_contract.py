# THIS IS leftover-safe ASkyguardMissionMapAssemblyDirector
# FlightRouteSpline.
...
# then BARE `TObjectPtr<USplineComponent> FlightRouteSpline;`
# (no initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|MissionMap` with spaces around =.
# VisibleAnywhere IS present. BlueprintReadOnly IS present.
# There is NO default.
...
# Parse CLASS `ASkyguardMissionMapAssemblyDirector` public
# UPROPERTY fields ONLY after
# `class ASkyguardMissionMapAssemblyDirector`.
# Start at `public:` after GENERATED_BODY.
# Stop BEFORE `private:`.
# Stop BEFORE sibling MissionDefinition as a claimed slot.
# Do NOT claim Root, MissionDefinition, MissionId,
# AssemblyRevision, SkylineStyle, WeatherProfileId,
# RoutePoints, ObjectiveAnchors, LandmarkAnchors,
# FlightClearanceRadiusCentimeters,
# FlightClearanceVerticalCentimeters, or Readiness.
# FlightRouteSpline is NOT leftover UFUNCTION
# RebuildRouteSpline (function, not this field).
# FlightRouteSpline is NOT leftover
# FSkyguardMissionMapReadiness struct fields.
# FlightRouteSpline is NOT leftover Gunner Boom
# (different class ASkyguardGunner, different type
# USpringArmComponent, Category Skyguard).
# FlightRouteSpline is NOT leftover TheaterKit Root /
# instances (different class, Category Skyguard|Theater,
# type USceneComponent).
# FlightRouteSpline is NOT leftover analog
# apache-cpg-feel FireRate / RecoilPitch /
# leftover LoadoutSpec.
...
# Skyguard|MissionMap with spaces around = NOT
# Category="Skyguard" and NOT Skyguard|Apache and NOT
# Skyguard|Theater and NOT Skyguard|Combat and NOT
# Skyguard|Combat|Feedback and NOT
# Skyguard|Combat|Safety.
# LOCKED_DECL is `TObjectPtr<USplineComponent> FlightRouteSpline;`
...
# and NOT `TObjectPtr<USceneComponent> Root;` and NOT
# `TObjectPtr<USkyguardMissionDefinition> MissionDefinition;`
# and NOT `FName MissionId;` and NOT
# `FSkyguardMissionMapReadiness Readiness;` and NOT
# leftover RebuildRouteSpline as the locked decl.
...
# / TObjectPtr<USkyguardMissionDefinition> MissionDefinition
# / FName MissionId / FSkyguardMissionMapReadiness Readiness
# / FireRate = 12.0f / RecoilPitch = 0.92f / FlareCount = 6
# / RebuildRouteSpline AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is Skyguard|Apache
# or Skyguard|Theater or Skyguard|Combat or
# Skyguard|Combat|Feedback or Skyguard|Combat|Safety
# or Category="Skyguard" without MissionMap.
...
TARGET = "TObjectPtr<USplineComponent> FlightRouteSpline;"
TARGET_WRONG_INIT = "TObjectPtr<USplineComponent> FlightRouteSpline = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USplineComponent> FlightRouteSpline = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USplineComponent> FlightRouteSpline = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> FlightRouteSpline;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> FlightRouteSpline;"
...
TARGET_WRONG_REBUILD = "void RebuildRouteSpline();"
...
TARGET_WRONG_TEXT = "TObjectPtr<UTextRenderComponent> FlightRouteSpline;"
...
TARGET_WRONG_STATIC = "TObjectPtr<UStaticMeshComponent> FlightRouteSpline;"
TARGET_WRONG_CAMERA = "TObjectPtr<UCameraComponent> FlightRouteSpline;"
...
SIBLING_REBUILD_ROUTE_SPLINE = "RebuildRouteSpline"
...
LEFTOVER_REBUILD_ROUTE_SPLINE = (
    "Scripts/tests/test_mission_map_rebuild_route_spline"
    "_decl_contract.py"
)
LEFTOVER_MISSION_MAP_READINESS_VALID = (
    "Scripts/tests/test_mission_map_readiness_definition_valid"
    "_field_decl_contract.py"
)
...
FIELD_DECL_RE = re.compile(
    r"TObjectPtr<USplineComponent>\s+FlightRouteSpline\s*;"
)
FIELD_INIT_RE = re.compile(
    r"TObjectPtr<USplineComponent>\s+FlightRouteSpline\s*="
)
WRONG_CAMERA_FIELD_RE = re.compile(
    r"TObjectPtr<UCameraComponent>\s+FlightRouteSpline\b"
)
WRONG_STATIC_FIELD_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+FlightRouteSpline\b"
)
WRONG_SCENE_FIELD_RE = re.compile(
    r"TObjectPtr<USceneComponent>\s+FlightRouteSpline\b"
)
WRONG_SPRING_FIELD_RE = re.compile(
    r"TObjectPtr<USpringArmComponent>\s+FlightRouteSpline\b"
)
WRONG_BOOM_DECL_RE = re.compile(
    r"TObjectPtr<USpringArmComponent>\s+Boom\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover `= 160.f`, leftover Gunner Boom,
    # leftover CpgConsoleRight / Root /
    # leftover MissionDefinition / MissionId /
    # leftover Readiness / leftover
    # RebuildRouteSpline as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if FIELD_DECL_RE.search(compact) is None:
        return False
    if FIELD_INIT_RE.search(compact):
        return False
    if WRONG_CAMERA_FIELD_RE.search(compact):
        return False
    if WRONG_STATIC_FIELD_RE.search(compact):
        return False
    if WRONG_SCENE_FIELD_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if WRONG_SPRING_FIELD_RE.search(compact):
        return False
    if WRONG_BOOM_DECL_RE.search(compact) and FIELD_DECL_RE.search(compact) is None:
        return False
    if "CpgConsoleRight" in compact and "FlightRouteSpline;" not in compact:
        return False
    return True
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_MISSION_DEFINITION):
            break
        if has_identifier(field_text, SIBLING_ROOT):
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, SIBLING_MISSION_DEFINITION):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_MISSION_DEFINITION}"
        )
    if has_identifier(window, SIBLING_ROOT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_ROOT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USplineComponent>\s+FlightRouteSpline\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USplineComponent> FlightRouteSpline "
...
class MissionMapFlightRouteSplineFieldDeclContractTests(
    unittest.TestCase
):
    def test_mission_map_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "FlightRouteSpline"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_MISSION_DEFINITION), body)
        self.assertTrue(has_identifier(body, SIBLING_ROOT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_MISSION_DEFINITION),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "FlightRouteSpline"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("FlightRouteSpline", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_flight_route_spline_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category = "Skyguard|MissionMap"', section)
        ...
        self.assertIn('Category = "Skyguard|MissionMap"', specifiers)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<USplineComponent> FlightRouteSpline;", compact_origin)
        self.assertNotIn("FlightRouteSpline = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_flight_route_spline_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USplineComponent> FlightRouteSpline"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("USplineComponent", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<USplineComponent> FlightRouteSpline;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_REBUILD)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USplineComponent> FlightRouteSplines;\n",
            "\tint32 FlightRouteSpline = 140;\n",
            "\tbool FlightRouteSpline = true;\n",
            "\tfloat FlightRouteSpline = " + forty + ";\n",
            "\tfloat FlightRouteSpline = " + eighty + ";\n",
            f"\t{TARGET_WRONG_BOOM}\n",
            f"\t{TARGET_WRONG_ROOT}\n",
            f"\t{TARGET_WRONG_MISSION_DEF}\n",
            f"\t{TARGET_WRONG_MISSION_ID}\n",
            f"\t{TARGET_WRONG_READINESS}\n",
            f"\t{TARGET_WRONG_REBUILD}\n",
    ...
    def test_missing_category_or_visible_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category = "Skyguard|MissionMap"', origin)
        ...
        self.assertIsNotNone(
            re.search(
                r'Category = "Skyguard\|MissionMap"',
                origin,
            )
        )
        self.assertIsNone(
    ...
    def test_does_not_claim_sibling_mission_definition_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "FlightRouteSpline"))
        self.assertFalse(has_identifier(section, SIBLING_MISSION_DEFINITION))
        self.assertFalse(has_identifier(section, SIBLING_ROOT))
        self.assertFalse(has_identifier(section, SIBLING_MISSION_ID))
        self.assertFalse(has_identifier(section, SIBLING_READINESS))
        self.assertFalse(has_identifier(section, SIBLING_REBUILD_ROUTE_SPLINE))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_MISSION_DEFINITION))
        self.assertTrue(has_identifier(leaked, SIBLING_ROOT))
        self.assertTrue(has_identifier(leaked, SIBLING_MISSION_ID))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertIn("SkyguardMissionMapAssemblyDirector.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission map FlightRouteSpline field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_map_flight_route_spline_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        self.assertIn(LEFTOVER_GUNNER_BOOM, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_ROOT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_REBUILD_ROUTE_SPLINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_MAP_READINESS_VALID, LOCKED_SCRIPTS)
