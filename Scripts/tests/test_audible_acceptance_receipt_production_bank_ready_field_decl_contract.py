# THIS IS leftover-safe FSkyguardAudibleAcceptanceReceipt
# bProductionBankReady.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(BlueprintReadOnly)
# then `bool bProductionBankReady = false;`.
# THIS STRUCT IS BlueprintReadOnly. There is NO EditAnywhere.
# There is NO Category=. Initializer is exactly `= false`.
# Fail-closed if missing/renamed field, EditAnywhere added,
# BlueprintReadWrite replacing BlueprintReadOnly, type not
# bool, initializer dropped or changed to `= true`, or
# Category= added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardAudibleAcceptanceReceipt` body ONLY after
# `struct FSkyguardAudibleAcceptanceReceipt`. Stop before
# `class SKYGUARD52_API USkyguardAudioAcceptanceHarness`
# (the SKYGUARD52_API macro is required; a naive
# `class USkyguardAudioAcceptanceHarness` find returns -1 and
# leaks leftover harness class fields:
# Category="Skyguard|Audio|Acceptance",
# MinimumMeasuredSamples = 600, MaximumAllowedVoices = 48).
# Do NOT parse leftover `USkyguardAudioAcceptanceHarness`.
# Do NOT parse leftover ProductionAudio Audit/Entry/Routing.
# Do NOT parse leftover `USkyguardAudioProductionBank`.
# Do NOT parse leftover `FSkyguardMissionResult`.
# Do NOT parse leftover `FSkyguardMissionDebrief`.
# Do NOT parse leftover `FSkyguardObjectiveProgress`.
# Do NOT parse leftover `FSkyguardRouteDefinition`.
# Do NOT parse leftover `FSkyguardRoutePoint`.
# Do NOT parse leftover `FSkyguardRadioLine`.
# Do NOT parse leftover TheaterKit.
# Do NOT parse leftover `FSkyguardAudioTelemetry`.
# Do NOT parse leftover ASkyguardGunshipSortieDirector.
# Do NOT parse leftover ASkyguardPatrolShipBoss.
# Fail-closed only if live leftover tokens / Harbor 40/80
# appear IN THE PARSE WINDOW or IN THE TEST FILE.
# Sibling receipt floats MaximumAudioThreadMs (`= 0` + `.f`)
# and MaximumTruePeakDbTP (`= -120` + `.f`) live in this
# struct — do not treat those as Harbor. Harbor is 40 + `.f`
# / 80 + `.f` only.
# Analog leftover audible-acceptance-receipt-defaults #193 /
# #1cd7 then leftover-safe isolated BuildSha256 #1435 through
# MaximumTruePeakDbTP #1441 then this isolated
# bProductionBankReady field decl.
# THIS IS NOT leftover #193 / #1cd7 (keep that bulk file
# in LOCKED_SCRIPTS).
# THIS IS NOT leftover ProductionAudio Audit count fields
# (#1412 BoundProductionSourceCount etc).
# THIS IS NOT leftover ProductionAudioAudit bProductionReady
# #1423 (different type/struct; keep that isolated audit
# field decl in LOCKED_SCRIPTS).
# THIS IS NOT leftover AudioTelemetry PeakActiveVoices
# #1282 (different type/struct).
# THIS IS NOT leftover USkyguardAudioProductionBank class
# (do not parse).
# THIS IS NOT leftover USkyguardAudioAcceptanceHarness class
# (do not parse; has MaximumAllowedVoices = 48).
# THIS IS NOT leftover enum class
# ESkyguardProductionAudioCategory.
# THIS IS NOT leftover MissionResult #1264/#1265/#1426-#1432.
# THIS IS NOT leftover RouteDefinition RouteId #1434 /
# Points #1433.
# THIS IS NOT leftover MeshBindSlot #194 / #6829.
# THIS IS NOT leftover-safe isolated BuildSha256 #1435 /
# SampleCount #1436 / EvidenceSha256 #1437 /
# PeakActiveVoices #1438 / TotalUnderruns #1439 /
# MaximumAudioThreadMs #1440 / MaximumTruePeakDbTP #1441
# (keep those sibling decls in LOCKED_SCRIPTS).
# Sibling decls stay unclaimed: BuildSha256 / EvidenceSha256 /
# SampleCount / PeakActiveVoices / TotalUnderruns /
# MaximumAudioThreadMs / MaximumTruePeakDbTP /
# bPackagedDevelopmentBuild / bAudibleDeviceObserved /
# bCalibratedMetering / bAccepted.
# Identifier bounds: match EXACT `bProductionBankReady` as a
# whole field name after `bool`. Do NOT accept sibling bools
# bPackagedDevelopmentBuild / bAudibleDeviceObserved /
# bCalibratedMetering / bAccepted.
# Do NOT parse leftover production-audio-audit isolated
# decls or leftover production-audio-entry isolated decls.
# Keep leftover MissionResult / RouteDefinition /
# ProductionAudio / MissionDebrief / harness class /
# bank class / AudioTelemetry / BuildSha256 #1435 through
# MaximumTruePeakDbTP #1441 / leftover Audit
# bProductionReady #1423 in LOCKED_SCRIPTS comments and
# tuples. Isolated field decl does not relock those.
# Keep bulk leftover files in LOCKED_SCRIPTS (do not relock
# test_campaign_theater_kit_contract.py or leftover
# mission-types-defaults-tests #153). Clone LOCKED_SCRIPTS
# from leftover-safe draft #1300.
# If a clone asserts VisibleAnywhere /
# Category="Skyguard|Theater" / BlueprintReadWrite /
# `FName WeatherIdentity` / required initializer /
# EditAnywhere, RETARGET: parse
# FSkyguardAudibleAcceptanceReceipt only, type is bool,
# required initializer `= false`, UPROPERTY is
# BlueprintReadOnly with NO EditAnywhere, no Category=.
# Harbor 40/80 fail-closed in the parse window and in THIS
...
HEADER_PATH = "Source/Skyguard52/SkyguardAudioAcceptanceHarness.h"
STRUCT_NAME = "FSkyguardAudibleAcceptanceReceipt"
ACTOR_NAME = "ASkyguardCampaignTheaterKit"
NAMESPACE_NAME = "SkyguardCampaignTheaterKit"
TARGET = "bool bProductionBankReady = false;"
TARGET_WRONG_BARE = "bool bProductionBankReady;"
TARGET_WRONG_EQ_ONE = "bool bProductionBankReady = true;"
TARGET_WRONG_EQ_NONE = "bool bProductionBankReady = NAME_None;"
TARGET_WRONG_TRUE = "bool bProductionBankReady = true;"
TARGET_WRONG_FALSE = "int32 bProductionBankReady = false;"
TARGET_WRONG_EQ_FLOAT = "bool bProductionBankReady = 0.f;"
TARGET_WRONG_FSTRING = "FString bProductionBankReady;"
TARGET_WRONG_FLOAT = "float bProductionBankReady = false;"
TARGET_WRONG_FNAME = "FName bProductionBankReady;"
TARGET_WRONG_FTEXT = "FText bProductionBankReady;"
TARGET_WRONG_BOOL = "int32 bProductionBankReady = 0;"
TARGET_WRONG_TARRAY = "TArray<bool> bProductionBankReady;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> bProductionBankReady;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SAMPLE = "bool bPackagedDevelopmentBuild = false;"
TARGET_WRONG_UNDERRUNS = "bool bAudibleDeviceObserved = false;"
TARGET_WRONG_CATEGORY = (
    "UPROPERTY(BlueprintReadOnly, "
    'Category="Skyguard|Audio|Production")'
)
LOCKED_DECL = TARGET
UPROPERTY_READONLY = "UPROPERTY(BlueprintReadOnly)"
STOP_BEFORE_HARNESS = (
    "class SKYGUARD52_API USkyguardAudioAcceptanceHarness"
)
STOP_BEFORE_HARNESS_NAIVE = "class USkyguardAudioAcceptanceHarness"
STOP_BEFORE_BANK = (
    "class SKYGUARD52_API USkyguardAudioProductionBank"
)
STOP_BEFORE_BANK_NAIVE = "class USkyguardAudioProductionBank"
STOP_BEFORE_ENTRY = "struct FSkyguardProductionAudioEntry"
STOP_BEFORE_ROUTING = "struct FSkyguardProductionAudioRouting"
STOP_BEFORE_AUDIT = "struct FSkyguardProductionAudioAudit"
STOP_BEFORE_CATEGORY_ENUM = "enum class ESkyguardProductionAudioCategory"
STOP_BEFORE_SOURCE_STATUS_ENUM = "enum class ESkyguardAudioSourceStatus"
STOP_BEFORE_PROCEDURAL = "USkyguardAudioProceduralBankComponent"
STOP_BEFORE_UCLASS = "UCLASS(ClassGroup=(Skyguard)"
STOP_BEFORE_UCLASS_BLUEPRINT = "UCLASS(BlueprintType)"
STOP_BEFORE_PROGRESS = "struct FSkyguardObjectiveProgress"
STOP_BEFORE_SCORE_RULES = "struct FSkyguardMissionScoreRules"
STOP_BEFORE_WEATHER = "struct FSkyguardWeatherProfile"
STOP_BEFORE_ROUTE = "struct FSkyguardRouteDefinition"
STOP_BEFORE_MISSION_DEF = "USkyguardMissionDefinition"
...
STOP_BEFORE_MISSION_DEBRIEF = "struct FSkyguardMissionDebrief"
...
SIBLING_BUILD_SHA256 = "BuildSha256"
SIBLING_EVIDENCE_SHA256 = "EvidenceSha256"
SIBLING_SAMPLE_COUNT = "SampleCount"
SIBLING_PEAK_ACTIVE_VOICES = "PeakActiveVoices"
SIBLING_TOTAL_UNDERRUNS = "TotalUnderruns"
SIBLING_MAXIMUM_AUDIO_THREAD_MS = "MaximumAudioThreadMs"
SIBLING_MAXIMUM_TRUE_PEAK_DB_TP = "MaximumTruePeakDbTP"
SIBLING_PACKAGED_DEVELOPMENT_BUILD = "bPackagedDevelopmentBuild"
SIBLING_AUDIBLE_DEVICE_OBSERVED = "bAudibleDeviceObserved"
SIBLING_CALIBRATED_METERING = "bCalibratedMetering"
SIBLING_ACCEPTED = "bAccepted"
LEFTOVER_HARNESS_CLASS = "USkyguardAudioAcceptanceHarness"
LEFTOVER_SAVE_GAME_CLASS = "USkyguardCampaignSaveGame"
STOP_BEFORE_SAVE_GAME_API = (
    "class SKYGUARD52_API USkyguardCampaignSaveGame"
)
LEFTOVER_SAVE_VERSION = "SaveVersion"
FIELD_NAME = "bProductionBankReady"
...
THIS_SCRIPT = (
    "Scripts/tests/test_audible_acceptance_receipt_production"
    "_bank_ready_field_decl_contract.py"
)
CLONE_BUILD_SHA256 = (
    "Scripts/tests/test_audible_acceptance_receipt_build"
    "_sha256_field_decl_contract.py"
)
CLONE_RECEIPT_SAMPLE_COUNT = (
    "Scripts/tests/test_audible_acceptance_receipt_sample"
    "_count_field_decl_contract.py"
)
CLONE_RECEIPT_EVIDENCE_SHA256 = (
    "Scripts/tests/test_audible_acceptance_receipt_evidence"
    "_sha256_field_decl_contract.py"
)
CLONE_RECEIPT_PEAK_ACTIVE_VOICES = (
    "Scripts/tests/test_audible_acceptance_receipt_peak"
    "_active_voices_field_decl_contract.py"
)
CLONE_RECEIPT_TOTAL_UNDERRUNS = (
    "Scripts/tests/test_audible_acceptance_receipt_total"
    "_underruns_field_decl_contract.py"
)
CLONE_RECEIPT_MAXIMUM_AUDIO_THREAD_MS = (
    "Scripts/tests/test_audible_acceptance_receipt_maximum"
    "_audio_thread_ms_field_decl_contract.py"
)
CLONE_RECEIPT_MAXIMUM_TRUE_PEAK_DBTP = (
    "Scripts/tests/test_audible_acceptance_receipt_maximum"
    "_true_peak_db_tp_field_decl_contract.py"
)
CLONE_RECEIPT_PACKAGED_DEVELOPMENT_BUILD = (
    "Scripts/tests/test_audible_acceptance_receipt_packaged"
    "_development_build_field_decl_contract.py"
)
CLONE_RECEIPT_AUDIBLE_DEVICE_OBSERVED = (
    "Scripts/tests/test_audible_acceptance_receipt_audible"
    "_device_observed_field_decl_contract.py"
)
CLONE_RECEIPT_CALIBRATED_METERING = (
    "Scripts/tests/test_audible_acceptance_receipt_calibrated"
    "_metering_field_decl_contract.py"
)
LEFTOVER_AUDIT_PRODUCTION_READY = (
    "Scripts/tests/test_production_audio_audit_production"
    "_ready_field_decl_contract.py"
)
LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS = (
    "Scripts/tests/test_audible_acceptance_receipt_defaults"
    "_contract.py"
)
LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS_PY = (
    "Scripts/tests/test_audible_acceptance_receipt_defaults.py"
)
LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS_TESTS = (
    "Scripts/tests/test_audible_acceptance_receipt_defaults"
    "_tests.py"
)
HARNESS_BEGIN_EVIDENCE_RUN = "BeginEvidenceRun"
HARNESS_RECORD_MEASURED_SAMPLE = "RecordMeasuredSample"
HARNESS_COMPLETE_EVIDENCE_RUN = "CompleteEvidenceRun"
HARNESS_GET_RECEIPT = "GetReceipt"
HARNESS_MINIMUM_MEASURED_SAMPLES = "MinimumMeasuredSamples"
HARNESS_MAXIMUM_ALLOWED_VOICES = "MaximumAllowedVoices"
HARNESS_CATEGORY = 'Category="Skyguard|Audio|Acceptance"'

LEFTOVER_PRODUCTION_AUDIO_AUDIT_DEFAULTS = (
    "Scripts/tests/test_production_audio_audit_defaults_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_AUDIT_DEFAULTS_PY = (
    "Scripts/tests/test_production_audio_audit_defaults.py"
)
LEFTOVER_PRODUCTION_AUDIO_AUDIT_DEFAULTS_TESTS = (
    "Scripts/tests/test_production_audio_audit_defaults_tests.py"
)
DRAFTED_DISPLAY_NAME = (
    "Scripts/tests/test_production_audio_entry_display_name"
    "_field_decl_contract.py"
)
DRAFTED_PROVENANCE_ID = (
    "Scripts/tests/test_production_audio_entry_provenance_id"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ENTRY_DEFAULTS = (
    "Scripts/tests/test_production_audio_entry_defaults_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ENTRY_DEFAULTS_PY = (
    "Scripts/tests/test_production_audio_entry_defaults.py"
)
LEFTOVER_PRODUCTION_AUDIO_ENTRY_DEFAULTS_TESTS = (
    "Scripts/tests/test_production_audio_entry_defaults_tests.py"
)
LEFTOVER_AUDIO_SOURCE_STATUS_ENUM = (
...
LEFTOVER_AUDIO_PRODUCTION_BANK_EMPTY_TESTS = (
    "Scripts/tests/test_audio_production_bank_empty_fail_closed_tests.py"
)
LEFTOVER_AUDIO_PRODUCTION_BANK_EMPTY_CONTRACT = (
    "Scripts/tests/test_audio_production_bank_empty_fail_closed_contract.py"
)
LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY = (
    "Scripts/tests/test_audio_procedural_bank_empty_fail_closed.py"
)
LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY_TESTS = (
    "Scripts/tests/test_audio_procedural_bank_empty_fail_closed_tests.py"
)
LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY_CONTRACT = (
    "Scripts/tests/test_audio_procedural_bank_empty_fail_closed_contract.py"
)
LEFTOVER_SAVE_RECORD_COMPLETED = (
    "Scripts/tests/test_mission_save_record_completed"
    "_field_decl_contract.py"
)
LEFTOVER_SAVE_RECORD_BEST_SCORE = (
    "Scripts/tests/test_mission_save_record_best_score"
    "_field_decl_contract.py"
)
LEFTOVER_LANDMARK_MISSION_EXCLUSIVE = (
    "Scripts/tests/test_mission_landmark_anchor_mission_exclusive"
    "_field_decl_contract.py"
)
LEFTOVER_LANDMARK_WORLD_LOCATION = (
    "Scripts/tests/test_mission_landmark_anchor_world_location"
    "_field_decl_contract.py"
)
LEFTOVER_LANDMARK_ROLE = (
    "Scripts/tests/test_mission_landmark_anchor_role"
    "_field_decl_contract.py"
)
LEFTOVER_LANDMARK_ID = (
    "Scripts/tests/test_mission_landmark_anchor_landmark_id"
    "_field_decl_contract.py"
)
LEFTOVER_OBJECTIVE_ANCHOR_WORLD_LOCATION = (
    "Scripts/tests/test_mission_objective_anchor_world_location"
    "_field_decl_contract.py"
)
LEFTOVER_OBJECTIVE_ANCHOR_OBJECTIVE_ID = (
    "Scripts/tests/test_mission_objective_anchor_objective_id"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS = (
    "Scripts/tests/test_mission_save_record_defaults_contract.py"
)
LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS_PY = (
    "Scripts/tests/test_mission_save_record_defaults.py"
)
LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS_TESTS = (
    "Scripts/tests/test_mission_save_record_defaults_tests.py"
)
LEFTOVER_CAMPAIGN_SAVE_EMPTY = (
    "Scripts/tests/test_campaign_save_empty_fail_closed.py"
)
LEFTOVER_CAMPAIGN_SAVE_VERSION = (
    "Scripts/tests/test_campaign_save_version_field_decl_contract.py"
)
LEFTOVER_CAMPAIGN_SAVE_CAMPAIGN_ID = (
    "Scripts/tests/test_campaign_save_campaign_id_decl_contract.py"
)
LEFTOVER_CAMPAIGN_SAVE_MISSION_RECORDS = (
    "Scripts/tests/test_campaign_save_mission_records_decl_contract.py"
)
LEFTOVER_CAMPAIGN_SAVE_SAVED_AT_UTC = (
    "Scripts/tests/test_campaign_save_saved_at_utc_decl_contract.py"
)
LEFTOVER_MIN_SUPPORTED_SAVE_VERSION = (
    "Scripts/tests/test_min_supported_save_version_decl_contract.py"
)
LEFTOVER_CURRENT_SAVE_VERSION = (
    "Scripts/tests/test_current_save_version_decl_contract.py"
)
LEFTOVER_MIGRATE_CAMPAIGN_SAVE = (
    "Scripts/tests/test_migrate_campaign_save_decl_contract.py"
)
LEFTOVER_MISSION_PRESENTATION_DEFAULTS = (
    "Scripts/tests/test_mission_presentation_defaults_contract.py"
)
LEFTOVER_OBJECTIVE_ANCHOR_DEFAULTS = (
...
CLONE_SUCCESS_DEBRIEF = (
    "Scripts/tests/test_mission_presentation_success_debrief"
    "_field_decl_contract.py"
)
LEFTOVER_CAMPAIGN_ID_FIELD = "CampaignId"
LEFTOVER_MISSION_RECORDS_FIELD = "MissionRecords"
LEFTOVER_SAVED_AT_UTC_FIELD = "SavedAtUtc"
LEFTOVER_MIN_SUPPORTED_FIELD = "MinSupportedSaveVersion"
LEFTOVER_CURRENT_SAVE_FIELD = "CurrentSaveVersion"
LEFTOVER_MIGRATE_FIELD = "MigrateCampaignSave"
CLONE_DISPLAY_NAME = (
    "Scripts/tests/test_objective_definition_display_name"
    "_field_decl_contract.py"
)
CLONE_OBJECTIVE_ID_DEFINITION = (
    "Scripts/tests/test_objective_definition_objective_id"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION_TYPES_DEFAULTS = (
    "Scripts/tests/test_mission_types_defaults.py"
)
LEFTOVER_MISSION_TYPES_DEFAULTS_TESTS = (
    "Scripts/tests/test_mission_types_defaults_tests.py"
)
...
LEFTOVER_OBJECTIVE_DEFINITION_DEFAULTS_PY = (
    "Scripts/tests/test_objective_definition_defaults.py"
)
LEFTOVER_OBJECTIVE_DEFINITION_DEFAULTS_TESTS = (
    "Scripts/tests/test_objective_definition_defaults_tests.py"
)
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_weather_profile_weather_field_decl_contract.py",
    "Scripts/tests/test_weather_profile_wind_speed_meters_per_second_field_decl_contract.py",
    "Scripts/tests/test_weather_profile_time_of_day_hours_field_decl_contract.py",
    "Scripts/tests/test_weather_profile_precipitation_field_decl_contract.py",
    "Scripts/tests/test_weather_profile_cloud_coverage_field_decl_contract.py",
    "Scripts/tests/test_enemy_formation_defaults_contract.py",
    "Scripts/tests/test_enemy_formation_formation_id_field_decl_contract.py",
    "Scripts/tests/test_enemy_formation_formation_field_decl_contract.py",
    "Scripts/tests/test_enemy_formation_unit_count_field_decl_contract.py",
    "Scripts/tests/test_enemy_formation_spacing_centimeters_field_decl_contract.py",
    "Scripts/tests/test_enemy_wave_defaults_contract.py",
    "Scripts/tests/test_enemy_wave_wave_id_field_decl_contract.py",
    "Scripts/tests/test_enemy_wave_start_time_seconds_field_decl_contract.py",
    "Scripts/tests/test_enemy_wave_formations_field_decl_contract.py",
    "Scripts/tests/test_enemy_wave_completion_objective_id_field_decl_contract.py",
    "Scripts/tests/test_route_point_defaults_contract.py",
    "Scripts/tests/test_route_point_point_id_field_decl_contract.py",
    "Scripts/tests/test_route_point_world_location_field_decl_contract.py",
    "Scripts/tests/test_route_point_target_airspeed_kph_field_decl_contract.py",
    "Scripts/tests/test_route_point_look_ahead_seconds_field_decl_contract.py",
    "Scripts/tests/test_route_point_allow_combat_orbit_field_decl_contract.py",
    "Scripts/tests/test_radio_line_defaults_contract.py",
    "Scripts/tests/test_radio_line_subtitle_field_decl_contract.py",
    "Scripts/tests/test_radio_line_speaker_field_decl_contract.py",
    "Scripts/tests/test_radio_line_line_id_field_decl_contract.py",
    "Scripts/tests/test_radio_line_sound_field_decl_contract.py",
    "Scripts/tests/test_radio_line_estimated_duration_seconds_field_decl_contract.py",
    "Scripts/tests/test_radio_line_priority_field_decl_contract.py",
    "Scripts/tests/test_radio_line_cooldown_seconds_field_decl_contract.py",
) + leftover_live_copy_boss_scripts()


def leftover_sibling_audit_field_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/test_production_audio_audit_"
    suffix = "_field_decl_contract.py"
    return (
        prefix + "bound_production_source_count" + suffix,
        prefix + "required_category_count" + suffix,
        prefix + "explicit_missing_source_count" + suffix,
        prefix + "qa_test_only_count" + suffix,
        prefix + "missing_category_entries" + suffix,
        prefix + "invalid_source_entries" + suffix,
        prefix + "missing_sound_bindings" + suffix,
        prefix + "missing_attenuation_bindings" + suffix,
        prefix + "missing_concurrency_bindings" + suffix,
        prefix + "missing_output_submix_bindings" + suffix,
        prefix + "missing_routing_assets" + suffix,
        prefix + "category_contract_complete" + suffix,
        prefix + "production_ready" + suffix,
    )


def leftover_entry_routing_isolated_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/"
    return (
        prefix + "test_production_audio_entry_sound"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_display_name"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_provenance_id"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_category"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_source_status"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_attenuation"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_concurrency"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_output_submix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_entry_source_sha256"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_master_submix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_cockpit_submix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_exterior_submix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_weapons_submix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_explosions_submix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_radio_submix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_cockpit_sound_mix"
        "_field_decl_contract.py",
        prefix + "test_production_audio_routing_cockpit_exterior"
        "_attenuation_field_decl_contract.py",
        prefix + "test_production_audio_routing_cockpit_low"
        "_pass_hz_field_decl_contract.py",
    )
...
def leftover_save_game_field_tokens() -> tuple[str, ...]:
    return (
        LEFTOVER_SAVE_VERSION,
        LEFTOVER_CAMPAIGN_ID_FIELD,
        LEFTOVER_MISSION_RECORDS_FIELD,
        LEFTOVER_SAVED_AT_UTC_FIELD,
        LEFTOVER_MIN_SUPPORTED_FIELD,
        LEFTOVER_CURRENT_SAVE_FIELD,
        LEFTOVER_MIGRATE_FIELD,
    )


def leftover_save_record_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS,
        LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS_PY,
        LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS_TESTS,
        ...
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_result_completion_time_seconds"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_result_aircraft_damage_fraction"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_result_completed_objective_ids"
        "_field_decl_contract.py",
        CLONE_SUCCESS_DEBRIEF,
        "Scripts/tests/test_mission_presentation_defaults.py",
        "Scripts/tests/test_mission_presentation_defaults_tests.py",
        "Scripts/tests/test_mission_presentation_briefing"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_presentation_radio_chatter"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_presentation_failure_debrief"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_presentation_minimum_briefing"
        "_warmup_seconds_field_decl_contract.py",
        "Scripts/tests/test_mission_score_rules_gold_threshold"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_score_rules_silver_threshold"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_score_rules_bronze_threshold"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_score_rules_no_damage_bonus"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_score_rules_perfect_accuracy_bonus"
        "_field_decl_contract.py",
        "Scripts/tests/test_mission_score_rules_completion_score"
        "_field_decl_contract.py",
        "Scripts/tests/test_objective_definition_failure_ends_mission"
        "_field_decl_contract.py",
        "Scripts/tests/test_objective_definition_score_reward"
        "_field_decl_contract.py",
        "Scripts/tests/test_objective_definition_type"
        "_field_decl_contract.py",
        "Scripts/tests/test_objective_definition_required_progress"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_weak_point_defaults_contract.py",
        "Scripts/tests/test_boss_weak_point_definition_integrity"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_weak_point_definition_exposes_weak_point_id"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_weak_point_definition_required_weapon"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_weak_point_definition_weak_point_id"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_definition_defaults_contract.py",
        "Scripts/tests/test_boss_definition_maximum_breakup_pieces"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_definition_weak_points"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_definition_defeat_objective_id"
        "_field_decl_contract.py",
        "Scripts/tests/test_boss_definition_boss_id_field_decl_contract.py",
        "Scripts/tests/test_boss_definition_callsign_field_decl_contract.py",
        "Scripts/tests/test_mission_definition_presentation"
        "_decl_contract.py",
        "Scripts/tests/test_mission_definition_mission_id"
        "_decl_contract.py",
        "Scripts/tests/test_mission_definition_route_decl_contract.py",
        "Scripts/tests/test_mission_definition_objectives"
        "_decl_contract.py",
        "Scripts/tests/test_mission_definition_find_objective"
        "_decl_contract.py",
    )


def leftover_production_audio_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_PRODUCTION_AUDIO_ENTRY_DEFAULTS,
        LEFTOVER_PRODUCTION_AUDIO_ENTRY_DEFAULTS_PY,
        LEFTOVER_PRODUCTION_AUDIO_ENTRY_DEFAULTS_TESTS,
        ...
        LEFTOVER_PRODUCTION_AUDIO_AUDIT_DEFAULTS_PY,
        LEFTOVER_PRODUCTION_AUDIO_AUDIT_DEFAULTS_TESTS,
    ) + leftover_sibling_audit_field_scripts() + leftover_entry_routing_isolated_scripts()


LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_save_record_scripts()
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_production_audio_scripts()


def leftover_sibling_receipt_field_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/test_audible_acceptance_receipt_"
    suffix = "_field_decl_contract.py"
    return (
        prefix + "build_sha256" + suffix,
        prefix + "evidence_sha256" + suffix,
        prefix + "sample_count" + suffix,
        prefix + "peak_active_voices" + suffix,
        prefix + "total_underruns" + suffix,
        prefix + "maximum_audio_thread_ms" + suffix,
        prefix + "maximum_true_peak_db_tp" + suffix,
        prefix + "maximum_true_peak_dbtp" + suffix,
        prefix + "packaged_development_build" + suffix,
        prefix + "audible_device_observed" + suffix,
        prefix + "calibrated_metering" + suffix,
        prefix + "accepted" + suffix,
    )


def leftover_audible_acceptance_defaults_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS,
        LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS_PY,
        LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS_TESTS,
...
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_sibling_receipt_field_scripts()
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_audible_acceptance_defaults_scripts()



def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_BUILD_SHA256,
        SIBLING_EVIDENCE_SHA256,
        SIBLING_SAMPLE_COUNT,
        SIBLING_PEAK_ACTIVE_VOICES,
        SIBLING_TOTAL_UNDERRUNS,
        SIBLING_MAXIMUM_AUDIO_THREAD_MS,
        SIBLING_MAXIMUM_TRUE_PEAK_DB_TP,
        SIBLING_PACKAGED_DEVELOPMENT_BUILD,
        SIBLING_AUDIBLE_DEVICE_OBSERVED,
        SIBLING_CALIBRATED_METERING,
        SIBLING_ACCEPTED,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bProductionBankReady = false;`.
    # Match the EXACT identifier bProductionBankReady as a whole
    # field name after bool. Do NOT accept sibling bools
    # bPackagedDevelopmentBuild / bAudibleDeviceObserved /
    # bCalibratedMetering / bAccepted. Reject dropped/changed
    # initializer (`= true` or bare), Harbor 40/80, wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    exact = re.search(
        r"bool\s+bProductionBankReady\s*=\s*false\s*;",
        compact,
    )
    if re.search(r"\bbPackagedDevelopmentBuild\b", compact) and exact is None:
        return False
    if re.search(r"\bbAudibleDeviceObserved\b", compact) and exact is None:
        return False
    if re.search(r"\bbCalibratedMetering\b", compact) and exact is None:
        return False
    if re.search(r"\bbAccepted\b", compact) and exact is None:
        return False
    if re.search(
        r"bool\s+bProductionBankReady\s*;",
        compact,
    ) and re.search(
        r"bool\s+bProductionBankReady\s*=",
        compact,
    ...
    for assigned in re.findall(
        r"bool\s+bProductionBankReady\s*=\s*([^;]+);",
        compact,
    ):
        if assigned.strip() != "false":
            return False
    if re.search(
        r"float\s+bProductionBankReady\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bProductionBankReady\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bProductionBankReady\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+bProductionBankReady\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+bProductionBankReady\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+bProductionBankReady\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+bProductionBankReady\b",
        compact,
    ):
        return False
    if exact is None:
...
def earlier_leftover_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_AUDIT,
        STOP_BEFORE_ENTRY,
        STOP_BEFORE_ROUTING,
        STOP_BEFORE_CATEGORY_ENUM,
        STOP_BEFORE_SOURCE_STATUS_ENUM,
        STOP_BEFORE_MISSION_RESULT,
        STOP_BEFORE_MISSION_DEBRIEF,
        "struct FSkyguardTheaterKitSpec",
        STOP_BEFORE_BANK,
    )


def parse_window(header: str) -> str:
    start_token = f"struct {STRUCT_NAME}"
    stop_token = STOP_BEFORE_HARNESS
    start = header.find(start_token)
    if start < 0:
        raise AssertionError(
            f"{STRUCT_NAME} is missing from origin/main:{HEADER_PATH}"
        )
    for earlier in earlier_leftover_tokens():
        at = header.find(earlier)
        if at >= 0 and start <= at:
            raise AssertionError(
                f"{STRUCT_NAME} parse window started at {earlier}"
            )
    if header.find(STOP_BEFORE_HARNESS_NAIVE) >= 0:
        raise AssertionError(
            f"naive {STOP_BEFORE_HARNESS_NAIVE} is present; "
            "leftover class fields would leak"
        )
    stop = header.find(stop_token, start)
    if stop < 0:
        raise AssertionError(
            f"{STRUCT_NAME} parse window is missing {stop_token}"
        )
    span = header[start:stop]
    body_start = span.find("{")
    if body_start < 0:
        raise AssertionError(
            f"{STRUCT_NAME} struct body is missing from "
            f"origin/main:{HEADER_PATH}"
        )
    depth = 0
    end = None
    for index, char in enumerate(span[body_start:], body_start):
        if char == "{":
            depth += 1
        elif char == "}":
            depth -= 1
    ...
    for leaked in (
        ...
        STOP_BEFORE_UCLASS,
        STOP_BEFORE_UCLASS_BLUEPRINT,
        STOP_BEFORE_SAVE_GAME_API,
        STOP_BEFORE_MISSION_RESULT,
        STOP_BEFORE_MISSION_DEBRIEF,
        HARNESS_BEGIN_EVIDENCE_RUN,
        HARNESS_RECORD_MEASURED_SAMPLE,
        HARNESS_COMPLETE_EVIDENCE_RUN,
        HARNESS_GET_RECEIPT,
        HARNESS_MINIMUM_MEASURED_SAMPLES,
        HARNESS_MAXIMUM_ALLOWED_VOICES,
        HARNESS_CATEGORY,
    ):
        if leaked in window:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes {leaked}"
            )
    for leaked in leftover_save_game_field_tokens():
        if leaked in window:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes {leaked}"
            )
    for banned in leftover_live_copy_tokens():
        if banned in window.lower():
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes leftover live-copy token"
            )
...
def struct_body(header: str) -> str:
    window = header
    if STRUCT_RE.search(header) is not None and STOP_BEFORE_HARNESS in header:
        window = parse_window(header)
    match = STRUCT_RE.search(window)
    if match is None:
        raise AssertionError(
            f"{STRUCT_NAME} is missing from origin/main:{HEADER_PATH}"
        )
    start = window.index("{", match.start())
    depth = 0
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_UCLASS_BLUEPRINT,
        STOP_BEFORE_SAVE_GAME_API,
        STOP_BEFORE_PROGRESS,
        STOP_BEFORE_SCORE_RULES,
        STOP_BEFORE_WEATHER,
        STOP_BEFORE_ROUTE,
        STOP_BEFORE_MISSION_DEF,
        ...
        f"class SKYGUARD52_API {ACTOR_NAME}",
        f"class {ACTOR_NAME}",
        STOP_BEFORE_SAVE_GAME_API,
        *leftover_save_game_field_tokens(),
    )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bProductionBankReady\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bProductionBankReady is missing from "
...
class AudibleAcceptanceReceiptProductionBankReadyFieldDeclContractTests(
    unittest.TestCase
):
    def test_audible_acceptance_receipt_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, ACTOR_NAME)
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_MISSION_DEF)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_ENTRY)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_ROUTING)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_AUDIT)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_MISSION_RESULT)
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_MISSION_DEBRIEF)
        body = struct_body(header)
        ...
        self.assertIn(STOP_BEFORE_UCLASS, header)
        self.assertNotIn(STOP_BEFORE_UCLASS, section)
        self.assertNotIn(STOP_BEFORE_UCLASS, body)
        self.assertIn(STOP_BEFORE_HARNESS, header)
        self.assertNotIn(STOP_BEFORE_HARNESS, body)
        self.assertEqual(header.find(STOP_BEFORE_HARNESS_NAIVE), -1)
        self.assertNotIn(STOP_BEFORE_ENTRY, header)
        self.assertNotIn(STOP_BEFORE_ROUTING, header)
        self.assertNotIn(STOP_BEFORE_AUDIT, header)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, header)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, header)
        self.assertNotIn(STOP_BEFORE_BANK, header)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, body)
        self.assertNotIn(STOP_BEFORE_SAVE_GAME_API, section)
        self.assertNotIn(STOP_BEFORE_SAVE_GAME_API, body)
        self.assertNotIn(STOP_BEFORE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_WEATHER, section)
        self.assertNotIn(STOP_BEFORE_ROUTE, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, body)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(ACTOR_NAME, body)
        ...
        self.assertNotIn("FSkyguardTheaterKitSpec", section)
        self.assertNotIn("Category=", section)
        self.assertNotIn(HARNESS_CATEGORY, section)
        self.assertNotIn(HARNESS_BEGIN_EVIDENCE_RUN, section)
        self.assertNotIn(HARNESS_MINIMUM_MEASURED_SAMPLES, section)
        self.assertNotIn(HARNESS_MAXIMUM_ALLOWED_VOICES, section)
        self.assertNotIn(STOP_BEFORE_HARNESS, section)
        self.assertNotIn(STOP_BEFORE_HARNESS_NAIVE, section)
        self.assertNotIn(STOP_BEFORE_BANK, section)

    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedAudibleAcceptanceReceipt\n{\n};\n"
            )
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        harness = (
            f"class {LEFTOVER_HARNESS_CLASS}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
    ...
    def test_neighbor_struct_declaration_does_not_satisfy(self) -> None:
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tGENERATED_BODY()\n"
            "};\n"
            f"{STOP_BEFORE_UCLASS})\n"
            f"{STOP_BEFORE_HARNESS}\n"
            "{{\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
            "}}\n"
            f"class {STOP_BEFORE_MISSION_DEF}\n"
            "{{\n"
            "public:\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
    ...
    def test_missing_production_bank_ready_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFString {SIBLING_BUILD_SHA256};\n"
            f"\tFString {SIBLING_EVIDENCE_SHA256};\n"
            f"\tint32 {SIBLING_SAMPLE_COUNT} = 0;\n"
            f"\tint32 {SIBLING_TOTAL_UNDERRUNS} = 0;\n"
            f"\tbool {SIBLING_ACCEPTED} = false;\n"
            "};\n"
        )
        ...
        self.assertFalse(has_identifier(section, FIELD_NAME), section)
        self.assertTrue(
            has_identifier(section, SIBLING_SAMPLE_COUNT), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_ACCEPTED), section
        )

    def test_sibling_bools_do_not_satisfy_production_bank_ready(self) -> None:
        sample_only = f"\t{TARGET_WRONG_SAMPLE}\n"
        self.assertFalse(has_declaration(sample_only, LOCKED_DECL))
        self.assertFalse(has_identifier(sample_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(sample_only, SIBLING_PACKAGED_DEVELOPMENT_BUILD)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(sample_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        underruns_only = f"\t{TARGET_WRONG_UNDERRUNS}\n"
        self.assertFalse(has_declaration(underruns_only, LOCKED_DECL))
        self.assertFalse(has_identifier(underruns_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(underruns_only, SIBLING_AUDIBLE_DEVICE_OBSERVED)
        )
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{TARGET_WRONG_SAMPLE}\n"
            f"\t{TARGET_WRONG_UNDERRUNS}\n"
            "};\n"
        )
        section = spec_section(mixed)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_READONLY}\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn(UPROPERTY_READONLY, section)
        self.assertIn("BlueprintReadOnly", section)
        self.assertNotIn("EditAnywhere", section)
        self.assertNotIn("BlueprintReadWrite", section)
        self.assertNotIn("Category=", section)
        self.assertIn("BlueprintReadOnly", UPROPERTY_READONLY)
        self.assertNotIn("EditAnywhere", UPROPERTY_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_READONLY)
        self.assertNotIn("Category", UPROPERTY_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_READONLY)
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        dropped = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
        ...
        changed = (
            ...
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{TARGET_WRONG_EQ_ONE}\n"
            "};\n"
        )
        changed_section = spec_section(changed)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(changed_section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("bool bProductionBankReady = false;", compact_origin)
        self.assertNotIn("bProductionBankReady = true", compact_origin)
        self.assertNotIn("int32 bProductionBankReady", compact_origin)
        self.assertNotIn("FString bProductionBankReady", compact_origin)
        self.assertNotIn("float bProductionBankReady", compact_origin)

    def test_production_bank_ready_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bProductionBankReady"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("FText ", LOCKED_DECL)
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn(SIBLING_SAMPLE_COUNT, LOCKED_DECL)
        self.assertNotIn(SIBLING_TOTAL_UNDERRUNS, LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        wrongs = (
            ...
            TARGET_WRONG_TRUE,
            TARGET_WRONG_FALSE,
            TARGET_WRONG_EQ_FLOAT,
            TARGET_WRONG_FSTRING,
            TARGET_WRONG_FLOAT,
            TARGET_WRONG_FNAME,
            TARGET_WRONG_FTEXT,
            TARGET_WRONG_BOOL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_TOTAL_UNDERRUNS} = 0;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tbool bProductionBankReadys = false;\n",
            "\tbool bProductionBankReady = true;\n",
            "\tint32 bProductionBankReady;\n",
            "\tfloat bProductionBankReady = " + forty + ";\n",
            "\tfloat bProductionBankReady = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn("BlueprintReadWrite", origin)
        self.assertIn("BlueprintReadOnly", origin)
        self.assertNotIn("VisibleAnywhere", origin)
        added_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_CATEGORY}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        category_specs = attached_uproperty_specifiers(
            spec_section(added_category)
        )
        ...
        edit_specs = attached_uproperty_specifiers(spec_section(added_edit))
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_READONLY}\n\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_READONLY} {LOCKED_DECL}\n",
            "\tUPROPERTY(BlueprintReadOnly)\n"
            f"\t{LOCKED_DECL}\n",
            "\tUPROPERTY(\n\t\tBlueprintReadOnly)\n"
            f"\t{LOCKED_DECL}\n",
    ...
    def test_does_not_contract_sibling_receipt_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(
            has_identifier(section, SIBLING_BUILD_SHA256), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_EVIDENCE_SHA256), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_SAMPLE_COUNT), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_PEAK_ACTIVE_VOICES), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_TOTAL_UNDERRUNS), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MAXIMUM_AUDIO_THREAD_MS), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MAXIMUM_TRUE_PEAK_DB_TP), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_PACKAGED_DEVELOPMENT_BUILD),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_AUDIBLE_DEVICE_OBSERVED),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_CALIBRATED_METERING), section
        )
        self.assertTrue(has_identifier(section, SIBLING_ACCEPTED), section)
        self.assertNotIn(LEFTOVER_MISSION_RECORDS_FIELD, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_SAVE_VERSION, LOCKED_DECL)
        self.assertTrue(has_identifier(section, FIELD_NAME), section)
        self.assertNotIn(FIELD_NAME, sibling_uncontracted_decls())
        for field in leftover_neighbor_hit_fields():
    ...
    def test_does_not_parse_harness_or_leftover_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_HARNESS, header)
        self.assertNotIn(STOP_BEFORE_HARNESS, section)
        self.assertNotIn(STOP_BEFORE_HARNESS, leaked)
        self.assertNotIn(STOP_BEFORE_HARNESS, window)
        self.assertEqual(header.find(STOP_BEFORE_HARNESS_NAIVE), -1)
        self.assertNotIn(STOP_BEFORE_HARNESS_NAIVE, section)
        self.assertNotIn(STOP_BEFORE_HARNESS_NAIVE, leaked)
        self.assertNotIn(STOP_BEFORE_HARNESS_NAIVE, window)
        self.assertIn(STOP_BEFORE_UCLASS, header)
        self.assertNotIn(STOP_BEFORE_UCLASS, section)
        self.assertNotIn(STOP_BEFORE_UCLASS, leaked)
        self.assertNotIn(STOP_BEFORE_UCLASS, window)
        self.assertIn(HARNESS_BEGIN_EVIDENCE_RUN, header)
        self.assertNotIn(HARNESS_BEGIN_EVIDENCE_RUN, section)
        self.assertNotIn(HARNESS_BEGIN_EVIDENCE_RUN, leaked)
        self.assertNotIn(HARNESS_BEGIN_EVIDENCE_RUN, window)
        self.assertIn(HARNESS_RECORD_MEASURED_SAMPLE, header)
        self.assertNotIn(HARNESS_RECORD_MEASURED_SAMPLE, window)
        self.assertIn(HARNESS_COMPLETE_EVIDENCE_RUN, header)
        self.assertNotIn(HARNESS_COMPLETE_EVIDENCE_RUN, window)
        self.assertIn(HARNESS_GET_RECEIPT, header)
        self.assertNotIn(HARNESS_GET_RECEIPT, window)
        self.assertIn(HARNESS_MINIMUM_MEASURED_SAMPLES, header)
        self.assertNotIn(HARNESS_MINIMUM_MEASURED_SAMPLES, window)
        self.assertIn(HARNESS_MAXIMUM_ALLOWED_VOICES, header)
        self.assertNotIn(HARNESS_MAXIMUM_ALLOWED_VOICES, window)
        self.assertIn(HARNESS_CATEGORY, header)
        self.assertNotIn(HARNESS_CATEGORY, section)
        self.assertNotIn(HARNESS_CATEGORY, leaked)
        self.assertNotIn(HARNESS_CATEGORY, window)
        self.assertNotIn(STOP_BEFORE_ENTRY, header)
        self.assertNotIn(STOP_BEFORE_ENTRY, window)
        self.assertNotIn(STOP_BEFORE_ROUTING, header)
        self.assertNotIn(STOP_BEFORE_AUDIT, header)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, header)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, header)
        self.assertNotIn(STOP_BEFORE_BANK, header)
        self.assertNotIn(STOP_BEFORE_BANK_NAIVE, header)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, window)
        self.assertNotIn(STOP_BEFORE_SAVE_GAME_API, header)
        self.assertNotIn(STOP_BEFORE_SAVE_GAME_API, window)
        for leftover in leftover_save_game_field_tokens():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, leaked)
            self.assertNotIn(leftover, window)
        self.assertNotIn(STOP_BEFORE_PROGRESS, header)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, header)
        self.assertNotIn(STOP_BEFORE_WEATHER, header)
        self.assertNotIn(STOP_BEFORE_ROUTE, header)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, header)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, header)
        self.assertNotIn(ACTOR_NAME, header)
        self.assertNotIn(STOP_BEFORE_APPLY, header)
        self.assertNotIn("FSkyguardTheaterKitSpec", header)
        self.assertNotIn(STOP_BEFORE_MISSION_RESULT, header)
        self.assertNotIn(STOP_BEFORE_MISSION_DEBRIEF, header)
    ...
    def test_naive_harness_class_stop_is_not_present(self) -> None:
        header = origin_main_header()
        self.assertEqual(header.find(STOP_BEFORE_HARNESS_NAIVE), -1)
        self.assertIn(STOP_BEFORE_HARNESS, header)
        self.assertTrue(
            STOP_BEFORE_HARNESS.startswith("class SKYGUARD52_API ")
        )
        window = parse_window(header)
        self.assertNotIn(STOP_BEFORE_HARNESS, window)
        self.assertNotIn(HARNESS_BEGIN_EVIDENCE_RUN, window)
        ...
        self.assertNotIn(HARNESS_MAXIMUM_ALLOWED_VOICES, window)
        self.assertNotIn(HARNESS_CATEGORY, window)
        self.assertNotIn("EvaluateReadiness", window)
        self.assertNotIn("InitializeRequiredEntries", window)
        self.assertNotIn("TArray<FSkyguardProductionAudioEntry> Entries", window)
        self.assertNotIn("FSkyguardProductionAudioRouting Routing", window)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for leftover in leftover_audio_event_enum_tokens():
            ...
            self.assertNotIn(leftover, window)
        self.assertNotIn(STOP_BEFORE_HARNESS, window)
        self.assertNotIn(STOP_BEFORE_BANK, window)
        self.assertNotIn(STOP_BEFORE_ENTRY, window)
        self.assertNotIn(STOP_BEFORE_ROUTING, window)
        self.assertNotIn(STOP_BEFORE_AUDIT, window)
        self.assertNotIn(STOP_BEFORE_MISSION_RESULT, window)
        self.assertNotIn(STOP_BEFORE_MISSION_DEBRIEF, window)
        self.assertNotIn(LEFTOVER_APACHE_CLASS, section)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\tvoid {HARNESS_BEGIN_EVIDENCE_RUN}();\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_READONLY)
        self.assertNotIn("Category", UPROPERTY_READONLY)
        self.assertNotIn('Category="Skyguard|Theater"', UPROPERTY_READONLY)
        self.assertNotIn('Category="Skyguard|Campaign"', UPROPERTY_READONLY)
        self.assertNotIn(HARNESS_CATEGORY, UPROPERTY_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_READONLY)
        self.assertNotIn("MultiLine", UPROPERTY_READONLY)
        self.assertNotIn("ClampMin", UPROPERTY_READONLY)
        self.assertNotIn("ClampMax", UPROPERTY_READONLY)
        for invented in INVENTED_FIELD_META:
            self.assertNotIn(invented, LOCKED_DECL)
            self.assertNotIn(invented, UPROPERTY_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt bProductionBankReady field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "production_bank_ready_field_decl_contract.py"
        ))
        self.assertIn("audible_acceptance_receipt_production", THIS_SCRIPT)
        self.assertNotIn("audio_telemetry_peak", THIS_SCRIPT)
        self.assertIn(CLONE_BUILD_SHA256, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_BUILD_SHA256, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_SAMPLE_COUNT, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_SAMPLE_COUNT, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_EVIDENCE_SHA256, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_EVIDENCE_SHA256, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_PEAK_ACTIVE_VOICES, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_PEAK_ACTIVE_VOICES, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_TOTAL_UNDERRUNS, LOCKED_SCRIPTS)
        self.assertNotEqual(CLONE_RECEIPT_TOTAL_UNDERRUNS, THIS_SCRIPT)
        self.assertIn(CLONE_RECEIPT_MAXIMUM_AUDIO_THREAD_MS, LOCKED_SCRIPTS)
        self.assertNotEqual(
            CLONE_RECEIPT_MAXIMUM_AUDIO_THREAD_MS, THIS_SCRIPT
        )
        self.assertIn(CLONE_RECEIPT_MAXIMUM_TRUE_PEAK_DBTP, LOCKED_SCRIPTS)
        self.assertNotEqual(
            CLONE_RECEIPT_MAXIMUM_TRUE_PEAK_DBTP, THIS_SCRIPT
        )
        self.assertIn(
            CLONE_RECEIPT_PACKAGED_DEVELOPMENT_BUILD, LOCKED_SCRIPTS
        )
        self.assertNotEqual(
            CLONE_RECEIPT_PACKAGED_DEVELOPMENT_BUILD, THIS_SCRIPT
        )
        self.assertIn(
            CLONE_RECEIPT_AUDIBLE_DEVICE_OBSERVED, LOCKED_SCRIPTS
        )
        self.assertNotEqual(
            CLONE_RECEIPT_AUDIBLE_DEVICE_OBSERVED, THIS_SCRIPT
        )
        self.assertIn(CLONE_RECEIPT_CALIBRATED_METERING, LOCKED_SCRIPTS)
        self.assertNotEqual(
            CLONE_RECEIPT_CALIBRATED_METERING, THIS_SCRIPT
        )
        self.assertIn(LEFTOVER_AUDIT_PRODUCTION_READY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIT_PRODUCTION_READY, THIS_SCRIPT)
        self.assertIn(
            LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS, LOCKED_SCRIPTS
        )
        ...
        self.assertIn(LEFTOVER_AUDIO_SOURCE_STATUS_ENUM, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_SAVE_RECORD_COMPLETED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DISPLAY_NAME, LOCKED_SCRIPTS)
        self.assertIn(CLONE_OBJECTIVE_ID_DEFINITION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_TYPES_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_TYPES_DEFAULTS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_TYPES_DEFAULTS_CONTRACT, LOCKED_SCRIPTS)
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_OBJECTIVE_DEFINITION_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_SCORE_RULES_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_WEATHER_IDENTITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_SAVE_RECORD_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CAMPAIGN_SAVE_MISSION_RECORDS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CAMPAIGN_SAVE_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_PRESENTATION_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_OBJECTIVE_ANCHOR_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LANDMARK_ANCHOR_DEFAULTS, LOCKED_SCRIPTS)
        ...
        for sibling_script in leftover_sibling_receipt_field_scripts():
            self.assertIn(sibling_script, LOCKED_SCRIPTS)
            self.assertNotEqual(sibling_script, THIS_SCRIPT)
        for sibling_script in leftover_sibling_audit_field_scripts():
            self.assertIn(sibling_script, LOCKED_SCRIPTS)
            self.assertNotEqual(sibling_script, THIS_SCRIPT)
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            LEFTOVER_MISSION_TYPES_DEFAULTS_CONTRACT,
            LEFTOVER_MISSION_SCORE_RULES_DEFAULTS,
            *leftover_save_record_scripts(),
            *leftover_production_audio_scripts(),
            *leftover_audible_acceptance_defaults_scripts(),
            CLONE_BUILD_SHA256,
            CLONE_RECEIPT_SAMPLE_COUNT,
            CLONE_RECEIPT_EVIDENCE_SHA256,
            CLONE_RECEIPT_PEAK_ACTIVE_VOICES,
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_UCLASS, locked_only)
        self.assertNotIn(STOP_BEFORE_SAVE_GAME_API, locked_only)
        self.assertNotIn(STOP_BEFORE_ENTRY, locked_only)
        self.assertNotIn(STOP_BEFORE_ROUTING, locked_only)
        self.assertNotIn(STOP_BEFORE_AUDIT, locked_only)
        self.assertNotIn(STOP_BEFORE_HARNESS, locked_only)
        self.assertNotIn(STOP_BEFORE_BANK, locked_only)
        self.assertNotIn(LEFTOVER_MISSION_RECORDS_FIELD, locked_only)
        self.assertNotIn(LEFTOVER_SAVE_VERSION, locked_only)
        self.assertNotIn(LEFTOVER_MIGRATE_FIELD, locked_only)
        self.assertNotIn(SIBLING_BUILD_SHA256, locked_only)
        self.assertNotIn(SIBLING_EVIDENCE_SHA256, locked_only)
        self.assertNotIn(SIBLING_SAMPLE_COUNT, locked_only)
        self.assertNotIn(SIBLING_TOTAL_UNDERRUNS, locked_only)
        for helper in namespace_helper_tokens():
