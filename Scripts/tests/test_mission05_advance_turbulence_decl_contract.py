...
# INDEX_NONE or lock AdvanceTurbulence
# construction in the .cpp. This is leftover-safe
# Mission05 Storm AdvanceTurbulence METHOD
# on ASkyguardMission05IntegrationDirector. Distinct
# from leftover storm-runtime-defaults #89c9
# (struct defaults, not this method). It is
# NOT leftover GetStormRuntime, NOT leftover
# TriggerLightningWindow (sibling), NOT leftover
# BindRuntimeActors (retired live mount in the
# locked decl), NOT leftover GetAircraft
# (returns the retired live mount), NOT leftover
# HandleDroneCityImpact, NOT leftover Harbor
# Mission02 / Harbor #6/#8/#9, NOT leftover
# HandleBossPhaseChanged without Blueprint
# category, and NOT leftover Mission05
# InitializePlayableMission #666 or leftover
...
# lock leftover GetStormRuntime. Do not lock
# leftover HandleBossPhaseChanged. Do not lock
# sibling Integration / Waves / Storm /
# Protection methods
# InitializePlayableMission /
# ConfigureMissionDefinition /
# BindRuntimeActors / StartNextWave /
# NotifyThreatDestroyed /
# leftover TriggerLightningWindow /
...
# #648, leftover Mission05 InitializePlayableMission
# #666, leftover Mission05 ConfigureMissionDefinition
# #665, leftover Mission04 wave-state enum #bb22,
# leftover Mission05 wave-state enum #ad28,
# leftover Mission05 protected-target enum #aaa2,
# leftover Mission05 protected-target-runtime-defaults #ec8b,
...
# body. Do NOT invent a body. Nearby origin/main
...
# Category is Skyguard|Mission05|Storm, not
# Integration, not Waves, not Protection, not
# Searchlight, not Environment, not leftover
# briefing-widget, not leftover
# MissionBriefingComponent methods, not leftover
# Mission01 Integration, not leftover
# Mission03 Integration, not leftover
# Harbor Mission02 Integration, not leftover
# Mission04 Searchlight StartSearchlightWindow.
# Stay off leftover drafts #56–#64, leftover Harbor
# #6/#8/#9, leftover theater-kit #59, leftover
# isolated-test drafts #107–#669, leftover Apache
...
# drafts #536–#669. Leftover Mission05 StartNextWave
# #668 / NotifyThreatDestroyed #669 stay listed,
# not locked. Leftover TriggerLightningWindow is a
# sibling Storm method, listed not locked.
...
# #608, leftover Mission03 InitializePlayableMission
# #628, leftover Mission05 InitializePlayableMission
# #666, leftover Mission05 ConfigureMissionDefinition
# #665, leftover Harbor Mission02, leftover
...
# Mission05 StartNextWave #668 / NotifyThreatDestroyed
# #669, leftover BindRuntimeActors, leftover
...
LEFTOVER_TRIGGER_LIGHTNING_WINDOW_NOT_LOCKED = (
    TRIGGER_LIGHTNING_WINDOW,
    "TriggerLightningWindow",
    "DurationSeconds",
    "test_mission05_trigger_lightning_window_decl_contract.py",
)
...
LEFTOVER_BIND_RUNTIME_ACTORS_NOT_LOCKED = (
    BIND_RUNTIME_ACTORS,
    "test_mission05_bind_runtime_actors_decl_contract.py",
    "test_mission01_bind_runtime_actors_decl_contract.py",
    "test_mission03_bind_runtime_actors_decl_contract.py",
    "test_mission04_bind_runtime_actors_decl_contract.py",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission05IntegrationDirector::AdvanceTurbulence",
    "SkyguardMission05IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission05AdvanceTurbulenceDeclContractTests(unittest.TestCase):
    def test_mission05_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ADVANCE_TURBULENCE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ADVANCE_TURBULENCE)
    ...
    def test_missing_advance_turbulence_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission05IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_STORM}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_STORM, section)
        ...
        self.assertNotIn("UFUNCTION", ADVANCE_TURBULENCE)
        self.assertNotIn("BlueprintCallable", ADVANCE_TURBULENCE)
        self.assertNotIn("Category", ADVANCE_TURBULENCE)
        self.assertNotIn("BlueprintPure", UFUNCTION_STORM)
        self.assertIn("Skyguard|Mission05|Storm", UFUNCTION_STORM)
        self.assertNotIn("Mission01", UFUNCTION_STORM)
        self.assertNotIn("Mission03", UFUNCTION_STORM)
        self.assertIn("BlueprintCallable", UFUNCTION_STORM)
        self.assertNotIn("Environment", UFUNCTION_STORM)
        self.assertNotIn("Briefing", UFUNCTION_STORM)
        self.assertNotIn("Mission02", UFUNCTION_STORM)
        self.assertNotIn("Boss", UFUNCTION_STORM)
        self.assertNotIn("Destruction", UFUNCTION_STORM)
        self.assertNotIn("Apache", UFUNCTION_STORM)
        self.assertNotIn("Mission07", UFUNCTION_STORM)
        self.assertNotIn("Mission10", UFUNCTION_STORM)
        self.assertNotIn("Encounter", UFUNCTION_STORM)
        self.assertNotIn("Safety", UFUNCTION_STORM)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_STORM)
            self.assertNotIn(invented, ADVANCE_TURBULENCE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid AdvanceTurbulence(float DeltaSeconds, float Turbulence, bool bMaintainingAim);\n"
        as_const = "\tbool AdvanceTurbulence(float DeltaSeconds, float Turbulence, bool bMaintainingAim) const;\n"
        with_arg = "\tbool AdvanceTurbulence(float DeltaSeconds);\n"
        renamed = "\tbool AdvanceStorm();\n"
        short_name = "\tbool Turbulence();\n"
        ...
        leftover_trigger = f"\t{TRIGGER_LIGHTNING_WINDOW}\n"
        leftover_m05_wave = f"\t{START_NEXT_WAVE}\n"
        leftover_m05_threat = f"\t{NOTIFY_THREAT_DESTROYED}\n"
        leftover_advance_window = f"\t{ADVANCE_LIGHTNING_WINDOW}\n"
        leftover_storm_runtime = f"\t{GET_STORM_RUNTIME}\n"
        leftover_boss_phase = f"\t{HANDLE_BOSS_PHASE_CHANGED}\n"
        for region in (
            as_void,
            as_const,
            with_arg,
            ...
            leftover_m05_wave,
            leftover_m05_threat,
            leftover_advance_window,
            leftover_storm_runtime,
            leftover_boss_phase,
        ):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ADVANCE_TURBULENCE)
            self.assertIn("AdvanceTurbulence", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_advance_turbulence_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ADVANCE_TURBULENCE),
            ADVANCE_TURBULENCE,
        )
        self.assertTrue(has_declaration(section, ADVANCE_TURBULENCE))
        self.assertEqual(
            declaration_count(section, ADVANCE_TURBULENCE),
            1,
        )
        self.assertTrue(
            ADVANCE_TURBULENCE.startswith("bool "),
            ADVANCE_TURBULENCE,
        )
        self.assertTrue(
            ADVANCE_TURBULENCE.endswith("bool bMaintainingAim);"),
            ADVANCE_TURBULENCE,
        )
        self.assertIn("AdvanceTurbulence", ADVANCE_TURBULENCE)
        self.assertIn("DeltaSeconds", ADVANCE_TURBULENCE)
        self.assertIn("Turbulence", ADVANCE_TURBULENCE)
        self.assertIn("bMaintainingAim", ADVANCE_TURBULENCE)
        self.assertNotIn("DurationSeconds", ADVANCE_TURBULENCE)
        self.assertNotIn("TArray<", ADVANCE_TURBULENCE)
        self.assertNotIn("TObjectPtr", ADVANCE_TURBULENCE)
        self.assertNotIn("TSoftObjectPtr", ADVANCE_TURBULENCE)
        self.assertNotIn("INDEX_NONE", ADVANCE_TURBULENCE)
        self.assertNotIn("UFUNCTION", ADVANCE_TURBULENCE)
        self.assertNotIn("{", ADVANCE_TURBULENCE)
        self.assertNotIn("}", ADVANCE_TURBULENCE)
        self.assertNotIn("return ", ADVANCE_TURBULENCE)
        self.assertNotIn("const;", ADVANCE_TURBULENCE)
        self.assertNotIn("Root", ADVANCE_TURBULENCE)
        self.assertNotIn("Briefing", ADVANCE_TURBULENCE)
        self.assertNotIn("AudioDirector", ADVANCE_TURBULENCE)
        self.assertNotIn("RadioChatter", ADVANCE_TURBULENCE)
        self.assertNotIn("SortiePresentation", ADVANCE_TURBULENCE)
        self.assertNotIn("CampaignDefinition", ADVANCE_TURBULENCE)
        self.assertNotIn("MissionDefinition", ADVANCE_TURBULENCE)
        self.assertNotIn("Readiness", ADVANCE_TURBULENCE)
        self.assertNotIn("bAutoInitialize", ADVANCE_TURBULENCE)
        self.assertNotIn("bAllowBoundedActorSpawning", ADVANCE_TURBULENCE)
        self.assertNotIn("bAutoLaunchAfterBriefing", ADVANCE_TURBULENCE)
        self.assertNotIn("PathfinderSpawnLocation", ADVANCE_TURBULENCE)
        self.assertNotIn("GetAircraft", ADVANCE_TURBULENCE)
        self.assertNotIn("BindRuntimeActors", ADVANCE_TURBULENCE)
        self.assertNotIn("HandleDroneCityImpact", ADVANCE_TURBULENCE)
        self.assertNotIn("StartNextWave", ADVANCE_TURBULENCE)
        self.assertNotIn("NotifyThreatDestroyed", ADVANCE_TURBULENCE)
        self.assertNotIn("AdvanceConvoyByDistance", ADVANCE_TURBULENCE)
        self.assertNotIn("NotifyConvoyDamage", ADVANCE_TURBULENCE)
        self.assertNotIn("GetWaveState", ADVANCE_TURBULENCE)
        self.assertNotIn("GetConvoyRouteState", ADVANCE_TURBULENCE)
        self.assertNotIn("GetDayBeatKit", ADVANCE_TURBULENCE)
        self.assertNotIn("GetNightBeatKit", ADVANCE_TURBULENCE)
        self.assertNotIn("StartSearchlightWindow", ADVANCE_TURBULENCE)
        self.assertNotIn("AdvanceSearchlightTrack", ADVANCE_TURBULENCE)
        self.assertNotIn("NotifySubstationDamage", ADVANCE_TURBULENCE)
        self.assertNotIn("GetRemainingThreatsInWave", ADVANCE_TURBULENCE)
        self.assertNotIn("GetSearchlightRuntime", ADVANCE_TURBULENCE)
        self.assertNotIn("GetSubstationIntegrity", ADVANCE_TURBULENCE)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", ADVANCE_TURBULENCE)
        self.assertNotIn("ConvoyRuntimeAnchor", ADVANCE_TURBULENCE)
        self.assertNotIn("RoadHunterSpawnLocation", ADVANCE_TURBULENCE)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", ADVANCE_TURBULENCE)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", ADVANCE_TURBULENCE)
        self.assertNotIn("ConfigureMissionDefinition", ADVANCE_TURBULENCE)
        self.assertNotIn("NotifyObjectiveProgress", ADVANCE_TURBULENCE)
        self.assertNotIn("NotifyProtectedAssetFailed", ADVANCE_TURBULENCE)
        self.assertNotIn("SynchronizeRuntimeState", ADVANCE_TURBULENCE)
        self.assertNotIn("IsCorePlayableReady", ADVANCE_TURBULENCE)
        self.assertNotIn("GetReadiness", ADVANCE_TURBULENCE)
        self.assertNotIn("GetObjectiveRuntime", ADVANCE_TURBULENCE)
        self.assertNotIn("GetGunner", ADVANCE_TURBULENCE)
        self.assertNotIn("GetPathfinder", ADVANCE_TURBULENCE)
        self.assertNotIn("GetMissionId", ADVANCE_TURBULENCE)
        self.assertNotIn("ValidateMissionContract", ADVANCE_TURBULENCE)
        self.assertNotIn("ConfigureFromMission", ADVANCE_TURBULENCE)
        self.assertNotIn("AdvanceBriefing", ADVANCE_TURBULENCE)
        self.assertNotIn("SetAssetsReady", ADVANCE_TURBULENCE)
        self.assertNotIn("AcknowledgeAndLaunch", ADVANCE_TURBULENCE)
        self.assertNotIn("CanLaunch", ADVANCE_TURBULENCE)
        self.assertNotIn("GetElapsedSeconds", ADVANCE_TURBULENCE)
        self.assertNotIn("GetBriefingState", ADVANCE_TURBULENCE)
        self.assertNotIn("GetMinimumWarmupSeconds", ADVANCE_TURBULENCE)
        self.assertNotIn("GetBriefingText", ADVANCE_TURBULENCE)
        self.assertNotIn("GetRadioChatter", ADVANCE_TURBULENCE)
        self.assertNotIn("GetPresentation", ADVANCE_TURBULENCE)
        self.assertNotIn("GetMissionTitle", ADVANCE_TURBULENCE)
        self.assertNotIn("AcknowledgeBriefing", ADVANCE_TURBULENCE)
        self.assertNotIn("LaunchSortie", ADVANCE_TURBULENCE)
        self.assertNotIn("HullCollider", ADVANCE_TURBULENCE)
        self.assertNotIn("OpticalTracker", ADVANCE_TURBULENCE)
        self.assertNotIn("WeaponServo", ADVANCE_TURBULENCE)
        self.assertNotIn("CountermeasurePod", ADVANCE_TURBULENCE)
        self.assertNotIn("MinHeightFromOriginCm", ADVANCE_TURBULENCE)
        self.assertNotIn("RadarNode", ADVANCE_TURBULENCE)
        self.assertNotIn("ESkyguardMission02WaveState", ADVANCE_TURBULENCE)
        self.assertNotIn("HarborIndustrial", ADVANCE_TURBULENCE)
        self.assertNotIn("MaxIntegrity", ADVANCE_TURBULENCE)
        self.assertNotIn("CurrentIntegrity", ADVANCE_TURBULENCE)
        self.assertNotIn("FillAndFinalize", ADVANCE_TURBULENCE)
        self.assertNotIn("FillAndFail", ADVANCE_TURBULENCE)
        self.assertNotIn("static ", ADVANCE_TURBULENCE)
        self.assertNotIn("InitializePlayableMission", ADVANCE_TURBULENCE)
        self.assertNotIn("ConfigureMissionDefinition", ADVANCE_TURBULENCE)
        self.assertNotIn("AdvanceLightningWindow", ADVANCE_TURBULENCE)
        self.assertNotIn("TriggerLightningWindow", ADVANCE_TURBULENCE)
        self.assertNotIn("GetStormRuntime", ADVANCE_TURBULENCE)
        self.assertNotIn("HandleBossPhaseChanged", ADVANCE_TURBULENCE)
        self.assertNotIn("NotifyProtectedTargetDamage", ADVANCE_TURBULENCE)
        self.assertNotIn("GetProtectedTarget", ADVANCE_TURBULENCE)
        self.assertNotIn("GetSurvivingTargetCount", ADVANCE_TURBULENCE)
        self.assertNotIn("GetStormRainBeatKit", ADVANCE_TURBULENCE)
        self.assertNotIn("ESkyguardMission05WaveState", ADVANCE_TURBULENCE)
        self.assertNotIn("ESkyguardMission05ProtectedTarget", ADVANCE_TURBULENCE)
        self.assertNotIn("FSkyguardStormRuntime", ADVANCE_TURBULENCE)
        self.assertNotIn("WindowSeconds", ADVANCE_TURBULENCE)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ADVANCE_TURBULENCE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_spaces = (
            "public:\n"
            "\tbool   AdvanceTurbulence(float DeltaSeconds, float Turbulence, bool bMaintainingAim);\n"
            "private:\n"
            "};\n"
        )
        wrap_tab = (
            "public:\n"
            "\tbool\t"
            "AdvanceTurbulence(float DeltaSeconds, float Turbulence, bool bMaintainingAim);\n"
            "};\n"
        )
        wrap_indent = (
            "public:\n"
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_STORM} {ADVANCE_TURBULENCE}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ADVANCE_TURBULENCE}\n}}\n"
        self.assertTrue(has_declaration(one_line, ADVANCE_TURBULENCE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADVANCE_TURBULENCE),
            section,
    ...
    def test_environment_category_does_not_satisfy_storm(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_STORM)
        self.assertNotIn("Briefing", UFUNCTION_STORM)
        environment = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(environment, UFUNCTION_STORM)
        self.assertNotEqual(briefing, UFUNCTION_STORM)
        self.assertNotIn(environment, UFUNCTION_STORM)
        self.assertNotIn(briefing, UFUNCTION_STORM)
        leftover_m01 = (
            'UFUNCTION(BlueprintCallable, '
        ...
        leftover_integration = UFUNCTION_INTEGRATION
        leftover_waves = UFUNCTION_WAVES
        leftover_protection = UFUNCTION_PROTECTION
        leftover_searchlight = UFUNCTION_SEARCHLIGHT
        self.assertNotEqual(leftover_m01, UFUNCTION_STORM)
        self.assertNotEqual(leftover_m02, UFUNCTION_STORM)
        self.assertNotEqual(leftover_harbor, UFUNCTION_STORM)
        self.assertNotEqual(leftover_m03, UFUNCTION_STORM)
        self.assertNotEqual(leftover_integration, UFUNCTION_STORM)
        self.assertNotEqual(leftover_waves, UFUNCTION_STORM)
        self.assertNotEqual(leftover_protection, UFUNCTION_STORM)
        self.assertNotEqual(leftover_searchlight, UFUNCTION_STORM)
        self.assertNotIn(leftover_m01, UFUNCTION_STORM)
        self.assertNotIn(leftover_m02, UFUNCTION_STORM)
        self.assertNotIn(leftover_harbor, UFUNCTION_STORM)
        self.assertNotIn(leftover_m03, UFUNCTION_STORM)
        self.assertNotIn("Mission01", UFUNCTION_STORM)
        self.assertNotIn("Mission03", UFUNCTION_STORM)
        self.assertNotIn("Integration", UFUNCTION_STORM)
        self.assertNotIn("Waves", UFUNCTION_STORM)
        self.assertNotIn("Protection", UFUNCTION_STORM)
        self.assertNotIn("Searchlight", UFUNCTION_STORM)
        self.assertNotIn("Environment", UFUNCTION_STORM)
        self.assertIn("Storm", UFUNCTION_STORM)

    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{ADVANCE_TURBULENCE}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, ADVANCE_TURBULENCE)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ADVANCE_TURBULENCE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_advance_turbulence_cpp_body(self) -> None:
        locked_only = f"{ADVANCE_TURBULENCE}\n"
        self.assertNotIn("{", ADVANCE_TURBULENCE)
        self.assertNotIn("}", ADVANCE_TURBULENCE)
        self.assertNotIn("return ", ADVANCE_TURBULENCE)
        ...
        self.assertNotIn("return false", ADVANCE_TURBULENCE)
        self.assertNotIn("return true", ADVANCE_TURBULENCE)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{ADVANCE_TURBULENCE}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", ADVANCE_TURBULENCE)
        self.assertTrue(ADVANCE_TURBULENCE.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ADVANCE_TURBULENCE)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, ADVANCE_TURBULENCE)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ADVANCE_TURBULENCE)
        self.assertNotIn("CurrentIntegrity", ADVANCE_TURBULENCE)
        self.assertNotIn("SkyguardApacheAircraft.h", ADVANCE_TURBULENCE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ADVANCE_TURBULENCE)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ADVANCE_TURBULENCE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ADVANCE_TURBULENCE}\n"
        self.assertEqual(
            require_declaration(locked_only, ADVANCE_TURBULENCE),
            ADVANCE_TURBULENCE,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_TURBULENCE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 AdvanceTurbulence method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ADVANCE_TURBULENCE}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ADVANCE_TURBULENCE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ADVANCE_TURBULENCE.lower(),
                "mission05 AdvanceTurbulence contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ADVANCE_TURBULENCE)

    def test_contract_is_advance_turbulence_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(ADVANCE_TURBULENCE.startswith("bool "))
        self.assertIn("AdvanceTurbulence", ADVANCE_TURBULENCE)
        self.assertTrue(ADVANCE_TURBULENCE.endswith(";"))
        self.assertIn(UFUNCTION_STORM, section)

    def test_sibling_director_fields_do_not_satisfy_advance_turbulence(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, ADVANCE_TURBULENCE)
            )
            self.assertNotEqual(ADVANCE_TURBULENCE, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, ADVANCE_TURBULENCE))
        locked_only = f"{ADVANCE_TURBULENCE}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool AdvanceTurbulence(float DeltaSeconds, float Turbulence, bool bMaintainingAim) { return false; }\n"
            "};\n"
        )
    ...
    def test_leftover_get_storm_runtime_does_not_satisfy(self) -> None:
        region = f"\t{GET_STORM_RUNTIME}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_leftover_bind_runtime_actors_does_not_satisfy(self) -> None:
        region = f"\t{BIND_RUNTIME_ACTORS}();\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        locked_only = f"{ADVANCE_TURBULENCE}\n"
        for token in LEFTOVER_BIND_RUNTIME_ACTORS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_TURBULENCE)
        self.assertIn(
            "Scripts/tests/test_mission05_bind_runtime_actors"
    ...
    def test_leftover_handle_boss_phase_changed_does_not_satisfy(self) -> None:
        region = f"\t{HANDLE_BOSS_PHASE_CHANGED}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_TURBULENCE)
        self.assertNotIn("BlueprintCallable", HANDLE_BOSS_PHASE_CHANGED)
        self.assertNotIn("Category", HANDLE_BOSS_PHASE_CHANGED)
    ...
    def test_leftover_mission05_configure_does_not_satisfy(self) -> None:
        region = f"\t{CONFIGURE_MISSION_DEFINITION}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_leftover_mission05_wave_state_enum_does_not_satisfy(self) -> None:
        locked_only = f"{ADVANCE_TURBULENCE}\n"
    ...
    def test_leftover_mission05_protected_target_does_not_satisfy(self) -> None:
        locked_only = f"{ADVANCE_TURBULENCE}\n"
    ...
    def test_leftover_trigger_lightning_window_does_not_satisfy(self) -> None:
        region = f"\t{TRIGGER_LIGHTNING_WINDOW}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, ADVANCE_TURBULENCE)
        self.assertIn("AdvanceTurbulence", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, ADVANCE_TURBULENCE))
        locked_only = f"{ADVANCE_TURBULENCE}\n"
        for token in LEFTOVER_TRIGGER_LIGHTNING_WINDOW_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_TURBULENCE)
        self.assertIn(
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_briefing_configure_from_mission"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
