...
LOCKED_THREATS = "ESkyguardThreatKind Threats[BeatCount] = {};"
# Leftover #56–#64 plus StormRainBeatKit production sources. This lane
# only adds an isolated Python Threats[BeatCount] contract.
LOCKED = {
...
# (in-flight sibling), day/night Beats[7] (#251/#252), night-kit fields
# (opening now), and loadout defaults (#253) stay sibling-only.
...
# Apache CPG storm-rain kit Threats[BeatCount] only. BeatCount is the
...
def threats_declaration(body: str) -> str:
    match = re.search(
        r"ESkyguardThreatKind\s+Threats\[BeatCount\]\s*=\s*\{\};",
        body,
    )
    if match is None:
        raise AssertionError(
            f"ESkyguardThreatKind Threats[BeatCount] is missing from "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME}"
        )
    return match.group(0)


def threats_array_bound(body: str) -> str:
    match = re.search(
        r"ESkyguardThreatKind\s+Threats\[([A-Za-z_][A-Za-z0-9_]*)\]\s*=\s*\{\};",
        body,
    )
    if match is None:
        raise AssertionError(
...
class StormRainBeatKitThreatsContractTests(unittest.TestCase):
    ...
    def test_missing_threats_declaration_fails_closed(self) -> None:
        ...
        self.assertIn("Threats[BeatCount]", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(STRUCT_NAME, str(raised.exception))

    def test_threats_is_declared_as_beatcount_array(self) -> None:
        body = kit_body(origin_main_header())
        self.assertEqual(threats_declaration(body), LOCKED_THREATS)
        self.assertIn(LOCKED_THREATS, body)
        self.assertEqual(threats_array_bound(body), "BeatCount")
        self.assertNotEqual(threats_array_bound(body), "7")
        self.assertNotEqual(threats_array_bound(body), "INDEX_NONE")
        self.assertNotIn("Threats[7]", body)
        self.assertNotIn("Threats[6]", body)
        self.assertNotIn("Threats[8]", body)
        self.assertNotIn("Threats[INDEX_NONE]", body)
    ...
    def test_contract_does_not_relock_kit_defaults(self) -> None:
        declaration = threats_declaration(kit_body(origin_main_header()))
        self.assertEqual(declaration, LOCKED_THREATS)
    ...
    def test_contract_does_not_lock_threat_kind_enumerators(self) -> None:
        declaration = threats_declaration(kit_body(origin_main_header()))
        self.assertEqual(declaration, LOCKED_THREATS)
        for name in THREAT_KIND_ENUMERATORS_NOT_LOCKED:
            self.assertNotIn(name, declaration)
            self.assertNotIn(f"ESkyguardThreatKind::{name}", declaration)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        body = kit_body(origin_main_header())
        declaration = threats_declaration(body)
        self.assertEqual(declaration, LOCKED_THREATS)
        ...
        self.assertNotEqual(
            declaration,
            "ESkyguardThreatKind Rifle[BeatCount] = {};",
        )
    ...
    def test_struct_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                ...
                "storm-rain beat-kit Threats[BeatCount] is Apache CPG "
                "Hydra/Storm identity, not Yak",
            )

    def test_contract_is_storm_rain_beat_kit_threats_only(self) -> None:
        header = origin_main_header()
        body = kit_body(header)
        declaration = threats_declaration(body)
        self.assertIn(f"struct {STRUCT_NAME}", header)
        ...
        self.assertNotIn("namespace SkyguardStormRainBeatKits", body)
        self.assertNotIn("Kinds[BeatCount]", declaration)
        self.assertNotEqual(declaration, "Rifle Threats[BeatCount] = {};")
