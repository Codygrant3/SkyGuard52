...
# THIS IS leftover-safe isolated UFUNCTION(BlueprintPure).
# Category IS `Skyguard|Theater`. BlueprintPure IS present.
# THIS IS NOT BlueprintCallable.
# Fail-closed if the UFUNCTION or signature is missing
# or renamed, if Category is missing, if specifiers
# drop to BlueprintCallable, if const is missing, or if
# the return type is not FName.
...
# Analog: leftover bulk
# test_campaign_theater_kit_contract.py string-contains
# then isolated CardId.
# Sibling UFUNCTIONs GetAppliedKitId / GetNamedLandmark /
# GetBuildingTint / GetAppliedSpec / GetRoadInstanceCount /
# ApplyTheaterKit stay uncontracted.
# Public UPROPERTY fields stay uncontracted.
...
# GetAppliedWeatherIdentity does not block this isolated
...
# #1315-#1319 (Root / RoadInstances / LampInstances /
# BuildingInstances / NamedLandmarkMesh; keep those files
# in LOCKED_SCRIPTS).
...
# asserts UPROPERTY FName WeatherIdentity, retarget: this
# is UFUNCTION BlueprintPure FName
# GetAppliedWeatherIdentity() const, Category is
# Skyguard|Theater, parse window is the actor public section.
...
TARGET = (
    ...
    "{ return AppliedSpec.WeatherIdentity; }"
)
TARGET_WRONG_BARE = "FName GetAppliedWeatherIdentity();"
TARGET_WRONG_NO_CONST = (
    "FName GetAppliedWeatherIdentity() "
    "{ return AppliedSpec.WeatherIdentity; }"
)
TARGET_WRONG_NO_BODY = "FName GetAppliedWeatherIdentity() const;"
TARGET_WRONG_TRUE = (
    "FName GetAppliedWeatherIdentity() const { return true; }"
)
TARGET_WRONG_FALSE = (
    "FName GetAppliedWeatherIdentity() const { return false; }"
)
TARGET_WRONG_VOID = (
    "void GetAppliedWeatherIdentity() const "
    "{ return AppliedSpec.WeatherIdentity; }"
)
TARGET_WRONG_BOOL = (
    "bool GetAppliedWeatherIdentity() const "
    "{ return AppliedSpec.WeatherIdentity; }"
)
TARGET_WRONG_FLOAT = (
    "float GetAppliedWeatherIdentity() const "
    "{ return AppliedSpec.WeatherIdentity; }"
)
TARGET_WRONG_SIBLING = (
...
SIBLING_GET_APPLIED_KIT_ID = "GetAppliedKitId"
SIBLING_GET_NAMED_LANDMARK = "GetNamedLandmark"
SIBLING_GET_BUILDING_TINT = "GetBuildingTint"
SIBLING_GET_APPLIED_SPEC = "GetAppliedSpec"
SIBLING_GET_ROAD_INSTANCE_COUNT = "GetRoadInstanceCount"
SIBLING_APPLY_THEATER_KIT = "ApplyTheaterKit"
LEFTOVER_SPEC_WEATHER_IDENTITY = "WeatherIdentity"
LEFTOVER_SPEC_KIT_ID = "KitId"
LEFTOVER_SPEC_LANDMARK_SET = "LandmarkSet"
LEFTOVER_SPEC_BUILDING_KIT = "BuildingKit"
LEFTOVER_SPEC_BUILDING_TINT = "BuildingTint"
LEFTOVER_SPEC_LAMP_TREATMENT = "LampTreatment"
LEFTOVER_SPEC_LAMP_COLOR = "LampColor"
LEFTOVER_SPEC_LAMP_INTENSITY = "LampIntensity"
LEFTOVER_SPEC_ROAD_TREATMENT = "RoadTreatment"
LEFTOVER_SPEC_ROAD_TINT = "RoadTint"
LEFTOVER_SPEC_NAMED_LANDMARK = "NamedLandmark"
LEFTOVER_SPEC_LANDMARK_TINT = "LandmarkTint"
LEFTOVER_SPEC_SILHOUETTE_KIT = "SilhouetteKit"
LEFTOVER_SPEC_SILHOUETTE_TINT = "SilhouetteTint"
LEFTOVER_SPEC_LANDMARK_MESH_INDEX = "LandmarkMeshIndex"
LEFTOVER_SPEC_LANDMARK_SCALE = "LandmarkScale"
...
LEFTOVER_THEATER_KIT_NAMED_LANDMARK_MESH = (
...
LEFTOVER_THEATER_KIT_ROAD_INSTANCES = (
    "Scripts/tests/test_theater_kit_road_instances"
    "_field_decl_contract.py"
)
LEFTOVER_THEATER_KIT_LAMP_INSTANCES = (
    "Scripts/tests/test_theater_kit_lamp_instances"
    "_field_decl_contract.py"
)
LEFTOVER_THEATER_KIT_BUILDING_INSTANCES = (
    "Scripts/tests/test_theater_kit_building_instances"
    "_field_decl_contract.py"
)
LEFTOVER_THEATER_KIT_SPEC_WEATHER_IDENTITY = (
...
def sibling_uncontracted_fields() -> tuple[str, ...]:
    return (
        SIBLING_ROOT,
...
def sibling_uncontracted_methods() -> tuple[str, ...]:
    return (
        SIBLING_GET_APPLIED_KIT_ID,
        SIBLING_GET_NAMED_LANDMARK,
        SIBLING_GET_BUILDING_TINT,
        SIBLING_GET_APPLIED_SPEC,
        SIBLING_GET_ROAD_INSTANCE_COUNT,
        SIBLING_APPLY_THEATER_KIT,
    )


def leftover_spec_field_names() -> tuple[str, ...]:
    # WeatherIdentity is leftover #1300 spec field AND appears
    # in THIS method name / body. Do not treat that identifier
    # as a sibling-field leak inside LOCKED_DECL.
    return (
        LEFTOVER_SPEC_KIT_ID,
        LEFTOVER_SPEC_LANDMARK_SET,
        LEFTOVER_SPEC_BUILDING_KIT,
        LEFTOVER_SPEC_BUILDING_TINT,
        LEFTOVER_SPEC_LAMP_TREATMENT,
        LEFTOVER_SPEC_LAMP_COLOR,
        LEFTOVER_SPEC_LAMP_INTENSITY,
        LEFTOVER_SPEC_ROAD_TREATMENT,
        LEFTOVER_SPEC_ROAD_TINT,
        LEFTOVER_SPEC_NAMED_LANDMARK,
        LEFTOVER_SPEC_LANDMARK_TINT,
        LEFTOVER_SPEC_SILHOUETTE_KIT,
        LEFTOVER_SPEC_SILHOUETTE_TINT,
        LEFTOVER_SPEC_LANDMARK_MESH_INDEX,
        LEFTOVER_SPEC_LANDMARK_SCALE,
    )


def leftover_theater_kit_spec_drafts() -> tuple[str, ...]:
    return (
        LEFTOVER_THEATER_KIT_SPEC_LANDMARK_SET,
        LEFTOVER_THEATER_KIT_SPEC_WEATHER_IDENTITY,
        LEFTOVER_THEATER_KIT_SPEC_KIT_ID,
...
def leftover_theater_kit_actor_field_drafts() -> tuple[str, ...]:
    return (
        LEFTOVER_THEATER_KIT_ROOT,
        LEFTOVER_THEATER_KIT_ROAD_INSTANCES,
        LEFTOVER_THEATER_KIT_LAMP_INSTANCES,
...
def leftover_ufunction_tokens() -> tuple[str, ...]:
    # Sibling / leftover UFUNCTIONs. THIS target
    # GetAppliedWeatherIdentity is omitted on purpose.
    return (
        STOP_BEFORE_APPLY,
        SIBLING_APPLY_THEATER_KIT,
        SIBLING_GET_APPLIED_KIT_ID,
...
def leftover_private_mesh_tokens() -> tuple[str, ...]:
    return (
        PRIVATE_CUBE_MESH,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored origin/main inline
    # `FName GetAppliedWeatherIdentity() const
    # { return AppliedSpec.WeatherIdentity; }`.
    # Do not accept a bare decl, missing const, void /
    # bool / float rename, or `return true` / `return false`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FName\s+GetAppliedWeatherIdentity\s*\(\s*\)\s*const"
        r"\s*\{\s*return\s+AppliedSpec\.WeatherIdentity\s*;\s*\}",
        compact,
    ) is None:
        return False
    if re.search(r"void\s+GetAppliedWeatherIdentity\b", compact):
        return False
    if re.search(r"bool\s+GetAppliedWeatherIdentity\b", compact):
        return False
    if re.search(r"float\s+GetAppliedWeatherIdentity\b", compact):
        return False
    if re.search(
        r"FName\s+GetAppliedWeatherIdentity\s*\(\s*\)\s*const\s*;",
        compact,
    ):
        return False
    if re.search(
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        *leftover_private_mesh_tokens(),
    )


def spec_section(header: str) -> str:
    # Leftover #1299-#1314 parse the spec struct. THIS lane must
    # not use that window. Keep the helper only so fail-closed
    # tests can prove the spec body is not THIS contract.
    raise AssertionError(
        f"THIS contract must not parse {STRUCT_NAME}; "
        f"parse {ACTOR_NAME} public section only"
    )
...
def actor_section(header: str) -> str:
    ...
    if next_access is None:
        raise AssertionError(
            f"{ACTOR_NAME} private section is missing from "
            f"origin/main:{HEADER_PATH}"
        )
    access_name = next_access.group(1)
    if access_name != "private":
        raise AssertionError(
            f"{ACTOR_NAME} public section must stop at private:; "
            f"found {access_name}:"
        )
    section = rest[: next_access.start()]
    if "private:" in section:
        raise AssertionError(
            f"{ACTOR_NAME} parse window includes private:"
        )
    ...
    for stop in (
        ...
        if stop in section:
            raise AssertionError(
                f"{ACTOR_NAME} parse window includes {stop}"
            )
    for mesh in leftover_private_mesh_tokens():
        if mesh in section:
            raise AssertionError(
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+GetAppliedWeatherIdentity\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for FName GetAppliedWeatherIdentity is missing from "
        f"origin/main:{HEADER_PATH} class {ACTOR_NAME} public section"
    )
...
class TheaterKitGetAppliedWeatherIdentityDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        for mesh in leftover_private_mesh_tokens():
            ...
            self.assertNotIn(mesh, section)
        self.assertIn(STOP_BEFORE_APPLY, header)
        self.assertIn(STOP_BEFORE_APPLY, section)
        self.assertNotIn(STOP_BEFORE_APPLY, LOCKED_DECL)
        for sibling in sibling_uncontracted_fields():
            self.assertNotIn(sibling, LOCKED_DECL)
        for leftover_fn in leftover_ufunction_tokens():
            self.assertNotIn(leftover_fn, LOCKED_DECL)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        mixed = (
            ...
            f"\t{UFUNCTION_PURE}\n"
            f"\tTObjectPtr<USceneComponent> {SIBLING_ROOT};\n"
            "private:\n"
        ...
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(f"struct {STRUCT_NAME}", section)
        self.assertNotIn(PRIVATE_CUBE_MESH, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GetAppliedWeatherIdentity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_applied_weather_identity_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Theater"', UFUNCTION_PURE)
        self.assertNotIn("EditAnywhere", UFUNCTION_PURE)
        self.assertNotIn("VisibleAnywhere", UFUNCTION_PURE)
        self.assertNotIn("BlueprintReadWrite", UFUNCTION_PURE)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn(
            "GetAppliedWeatherIdentity()const "
            "{ return AppliedSpec.WeatherIdentity; }",
            compact_origin,
        )
        self.assertNotIn(
            "GetAppliedWeatherIdentity()const { return true; }",
            compact_origin,
        )
        self.assertNotIn(
            "GetAppliedWeatherIdentity()const { return false; }",
            compact_origin,
        )

    def test_get_applied_weather_identity_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName GetAppliedWeatherIdentity()"),
            LOCKED_DECL,
        )
        self.assertIn("const", LOCKED_DECL)
        self.assertIn("return AppliedSpec.WeatherIdentity", LOCKED_DECL)
        ...
        for sibling in sibling_uncontracted_fields():
            self.assertNotIn(sibling, LOCKED_DECL)
        for method in sibling_uncontracted_methods():
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_NO_BODY}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_NO_CONST}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SPEC_FIELD}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_NO_BODY}\n", LOCKED_DECL)
        self.assertIn("GetAppliedWeatherIdentity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SIBLING}\n",
            f"\t{TARGET_WRONG_SPEC_FIELD}\n",
            f"\t{TARGET_WRONG_HEALTH}\n",
            leftover_primary,
            leftover_guided,
            f"\tTObjectPtr<USceneComponent> {SIBLING_ROOT};\n",
            f"\tFName {SIBLING_GET_APPLIED_KIT_ID}() const;\n",
            "\tFName GetAppliedWeatherIdentityy() const "
            "{ return AppliedSpec.WeatherIdentity; }\n",
            "\tFName GetWeatherIdentity() const "
            "{ return AppliedSpec.WeatherIdentity; }\n",
            "\tfloat GetAppliedWeatherIdentity() const = " + forty + ";\n",
            "\tfloat GetAppliedWeatherIdentity() const = " + eighty + ";\n",
        )
    ...
    def test_missing_category_or_callable_instead_of_pure_fails_closed(self) -> None:
        no_category = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
        ...
        callable_specs = attached_ufunction_specifiers(
            actor_section(callable_only)
        )
    ...
    def test_does_not_contract_sibling_methods_or_fields(self) -> None:
        for sibling in sibling_uncontracted_fields():
            self.assertNotIn(sibling, LOCKED_DECL)
        for method in sibling_uncontracted_methods():
            self.assertNotIn(method, LOCKED_DECL)
        ...
        for method in sibling_uncontracted_methods():
            self.assertTrue(has_identifier(section, method), method)
        for leftover_fn in leftover_ufunction_tokens():
            self.assertNotIn(leftover_fn, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, section)
        for spec_field in leftover_spec_field_names():
            self.assertFalse(
                has_identifier(LOCKED_DECL, spec_field),
                spec_field,
            )
    ...
    def test_does_not_parse_spec_namespace_or_private(self) -> None:
        header = origin_main_header()
        section = actor_section(header)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        ...
        self.assertIn("Category", UFUNCTION_PURE)
        self.assertIn('Category="Skyguard|Theater"', UFUNCTION_PURE)
        self.assertNotIn('Category="Skyguard|Campaign"', UFUNCTION_PURE)
        self.assertNotIn("VisibleAnywhere", UFUNCTION_PURE)
        self.assertNotIn("BlueprintCallable", UFUNCTION_PURE)
        self.assertNotIn("EditAnywhere", UFUNCTION_PURE)
        self.assertNotIn("BlueprintReadWrite", UFUNCTION_PURE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn(SIBLING_GET_APPLIED_KIT_ID, LOCKED_DECL)
        self.assertNotIn(SIBLING_GET_NAMED_LANDMARK, LOCKED_DECL)
        self.assertNotIn(SIBLING_APPLY_THEATER_KIT, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_APPLY, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetAppliedWeatherIdentity decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "get_applied_weather_identity_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardRadarNode.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_THEATER_KIT_BULK, LOCKED_SCRIPTS)
        for leftover in leftover_theater_kit_spec_drafts():
            self.assertIn(leftover, LOCKED_SCRIPTS)
            self.assertNotEqual(leftover, THIS_SCRIPT)
        for leftover in leftover_theater_kit_weather_apply_scripts():
            self.assertIn(leftover, LOCKED_SCRIPTS)
            self.assertNotEqual(leftover, THIS_SCRIPT)
        for leftover in leftover_theater_kit_actor_field_drafts():
            self.assertIn(leftover, LOCKED_SCRIPTS)
            self.assertNotEqual(leftover, THIS_SCRIPT)
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            CLONE_NEXT_MISSION_UNLOCKED,
            LEFTOVER_THEATER_KIT_BULK,
            *leftover_theater_kit_spec_drafts(),
            *leftover_theater_kit_weather_apply_scripts(),
            *leftover_theater_kit_actor_field_drafts(),
            "Scripts/tests/test_briefing_card_defaults_contract.py",
            "Scripts/tests/test_briefing_radio_row_defaults_contract.py",
            "Scripts/tests/test_how_to_fly_row_defaults_contract.py",
            "Scripts/tests/test_briefing_card_priority_field_decl_contract.py",
    ...
    def test_contract_is_target_ufunction_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_ROOT, locked_only)
        self.assertNotIn(SIBLING_ROAD_INSTANCES, locked_only)
        self.assertNotIn(SIBLING_BUILDING_INSTANCES, locked_only)
        self.assertNotIn(SIBLING_LAMP_INSTANCES, locked_only)
        self.assertNotIn(SIBLING_SILHOUETTE_INSTANCES, locked_only)
        self.assertNotIn(SIBLING_NAMED_LANDMARK_MESH, locked_only)
        for leftover_fn in leftover_ufunction_tokens():
            self.assertNotIn(leftover_fn, locked_only)
        for mesh in leftover_private_mesh_tokens():
