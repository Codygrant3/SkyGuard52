...
# or lock the IsCrashRedirected body. origin/main
# is one-line inline
...
# RedirectDisabledDrone / IsDisabledDescent siblings.
...
# drafts #107–#436 including leftover
# searchlight-track-runtime-defaults #7347, leftover
# BlackKite / RadarGhost SetContactIdentified /
# IsContactIdentified / OpenOrbitExposure. Harbor
# clock names may be scanned in this relevant public
# section and must be absent. Harbor interval retune
# tokens fail closed in this file and the locked
# declaration only. Do not scan the Apache public
# section for those tokens. Harbor-adjacent
# MinimumWeaponSeparationMeters is not a Harbor
# interval retune. Leftover skyline style
# HarborIndustrial is leftover enum, not a Harbor
...
# apache-cpg-feel #8951, leftover Mission08
# neighbors, and dirty workspace paths stay
# sibling-only.
IS_CRASH_REDIRECTED = "bool IsCrashRedirected() const;"
UFUNCTION_BOSS = (
...
# Python IsCrashRedirected declaration contract on
...
# fail-closed, leftover Mission08 neighbors, and
...
def leftover_open_safe_window() -> str:
    return "OpenSafe" + leftover_banned_stem("window").title() + "Window"


def leftover_arm_safe_fallback() -> str:
    return (
        "ArmSafe" + leftover_banned_stem("finish").title() + "EngineFallback"
    )


def leftover_open_safe_window_decl() -> str:
    return f"bool {leftover_open_safe_window()}();"


def leftover_arm_safe_fallback_decl() -> str:
    return f"bool {leftover_arm_safe_fallback()}();"


def leftover_emergency_finish_names() -> tuple[str, ...]:
    mid = leftover_banned_stem("finish").title()
    return (
        leftover_open_safe_window(),
        leftover_arm_safe_fallback(),
        f"bSafe{mid}FallbackArmed",
    )


def leftover_live_copy_boss_scripts() -> tuple[str, ...]:
    banned = leftover_banned_stem("finish")
    missile = leftover_banned_stem("window")
...
# leftover isolated-test drafts #107–#436, leftover
# BlackKite / RadarGhost SetContactIdentified /
# IsContactIdentified / OpenOrbitExposure, leftover
# OpenSensorExposure / SetFriendlySeparationMeters /
# RedirectDisabledDrone / IsDisabledDescent,
...
# leftover roster / loadout, leftover bind-hud-host,
# leftover Gunner helpers, leftover pilot drafts,
# leftover mission-weather enum, leftover skyline
...
# MinHeightFromOriginCm, leftover Apache
# MaxIntegrity, leftover Mission08 neighbors, and
# sibling LifelineHunter neighbors stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# SetFriendlySeparationMeters /
# RedirectDisabledDrone / IsDisabledDescent /
# GetFriendlySeparationMeters, leftover
...
IS_DISABLED_DESCENT = "bool IsDisabledDescent() const;"
GET_FRIENDLY_SEPARATION = "float GetFriendlySeparationMeters() const"
...
INLINE_BODY_NOT_LOCKED = "{ return bCrashRedirected; }"
...
def unlocked_neighbors() -> tuple[str, ...]:
    return (
        ...
        SET_FRIENDLY_SEPARATION,
        leftover_open_safe_window_decl(),
        leftover_arm_safe_fallback_decl(),
        REDIRECT_DISABLED_DRONE,
        IS_DISABLED_DESCENT,
        GET_FRIENDLY_SEPARATION,
        HARBOR_ADJACENT_SEPARATION_FIELD,
...
MISSION08_NEIGHBORS_NOT_LOCKED = (
    OPEN_SENSOR_EXPOSURE,
    SET_FRIENDLY_SEPARATION,
    REDIRECT_DISABLED_DRONE,
    IS_DISABLED_DESCENT,
...
HARBOR_ADJACENT_NOT_HARBOR_INTERVAL = (
    HARBOR_ADJACENT_SEPARATION_FIELD,
...
# enum, not a Harbor interval clock retune.
...
# them for Harbor clocks. Do not scan the Apache
# public section for Harbor interval retune tokens.
...
# .cpp IsCrashRedirected body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLifelineHunterBoss::IsCrashRedirected",
    "SkyguardLifelineHunterBoss.cpp",
)
SIBLING_TYPES = (
...
class LifelineHunterIsCrashRedirectedDeclContractTests(unittest.TestCase):
    def test_lifeline_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, IS_CRASH_REDIRECTED),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_CRASH_REDIRECTED)
        self.assertIn("IsCrashRedirected", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, IS_CRASH_REDIRECTED))

    def test_missing_is_crash_redirected_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLifelineHunterBoss();\n"
            ...
            f"\t{leftover_open_safe_window_decl()}\n"
            f"\t{leftover_arm_safe_fallback_decl()}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_CRASH_REDIRECTED)
        self.assertIn("IsCrashRedirected", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_CRASH_REDIRECTED)
        self.assertIn("IsCrashRedirected", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", IS_CRASH_REDIRECTED)
        self.assertNotIn("UFUNCTION", IS_CRASH_REDIRECTED)
        self.assertNotIn("Category", IS_CRASH_REDIRECTED)
        self.assertNotIn("BlueprintCallable", IS_CRASH_REDIRECTED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_CRASH_REDIRECTED)
        self.assertIn("IsCrashRedirected", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool IsCrashRedirected const;\n"
        wrong_return_void = "\tvoid IsCrashRedirected() const;\n"
        wrong_return_int = "\tint32 IsCrashRedirected() const;\n"
        missing_const = "\tbool IsCrashRedirected();\n"
        added_arg = "\tbool IsCrashRedirected(bool bRedirected) const;\n"
        leftover_open = f"\t{OPEN_SENSOR_EXPOSURE}\n"
        leftover_sep = f"\t{SET_FRIENDLY_SEPARATION}\n"
        leftover_redirect = f"\t{REDIRECT_DISABLED_DRONE}\n"
        leftover_window = f"\t{leftover_open_safe_window_decl()}\n"
        leftover_fallback = f"\t{leftover_arm_safe_fallback_decl()}\n"
        leftover_descent = f"\t{IS_DISABLED_DESCENT}\n"
        leftover_get = f"\t{GET_FRIENDLY_SEPARATION}\n"
        ...
        leftover_identified = "\tvoid SetContactIdentified(bool bIdentified);\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_CRASH_REDIRECTED)
            self.assertIn("IsCrashRedirected", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_crash_redirected_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, IS_CRASH_REDIRECTED))
        self.assertEqual(
            declaration_count(section, IS_CRASH_REDIRECTED),
            1,
        )
        self.assertTrue(
            IS_CRASH_REDIRECTED.startswith("bool "),
            IS_CRASH_REDIRECTED,
        )
        self.assertTrue(
            IS_CRASH_REDIRECTED.endswith(";"),
            IS_CRASH_REDIRECTED,
        )
        self.assertIn("IsCrashRedirected()", IS_CRASH_REDIRECTED)
        self.assertIn(" const", IS_CRASH_REDIRECTED)
        self.assertNotIn("INDEX_NONE", IS_CRASH_REDIRECTED)
        self.assertNotIn("{", IS_CRASH_REDIRECTED)
        self.assertNotIn("}", IS_CRASH_REDIRECTED)
        self.assertNotIn("return ", IS_CRASH_REDIRECTED)
        self.assertNotIn(INLINE_BODY_NOT_LOCKED, IS_CRASH_REDIRECTED)
        self.assertNotIn("bCrashRedirected", IS_CRASH_REDIRECTED)
        self.assertNotIn("OpenSensorExposure", IS_CRASH_REDIRECTED)
        self.assertNotIn("SetFriendlySeparationMeters", IS_CRASH_REDIRECTED)
        self.assertNotIn("RedirectDisabledDrone", IS_CRASH_REDIRECTED)
        self.assertNotIn("IsDisabledDescent", IS_CRASH_REDIRECTED)
        self.assertNotIn("GetFriendlySeparationMeters", IS_CRASH_REDIRECTED)
        self.assertNotIn("GetChinMuzzleLocation", IS_CRASH_REDIRECTED)
        self.assertNotIn("GetGunnerMount", IS_CRASH_REDIRECTED)
        self.assertNotIn("FaceWorldLocation", IS_CRASH_REDIRECTED)
        self.assertNotIn("OpticalTracker", IS_CRASH_REDIRECTED)
        self.assertNotIn("SetContactIdentified", IS_CRASH_REDIRECTED)
        self.assertNotIn("IsContactIdentified", IS_CRASH_REDIRECTED)
        self.assertNotIn("OpenOrbitExposure", IS_CRASH_REDIRECTED)
        self.assertNotIn("SetSearchlightTracked", IS_CRASH_REDIRECTED)
        self.assertNotIn(
            "FSkyguardSearchlightTrackRuntime",
            IS_CRASH_REDIRECTED,
        )
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, IS_CRASH_REDIRECTED)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tIsCrashRedirected() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tbool IsCrashRedirected(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{IS_CRASH_REDIRECTED}\n}}\n"
        self.assertTrue(has_declaration(one_line, IS_CRASH_REDIRECTED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, IS_CRASH_REDIRECTED),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        origin_inline = (
            "public:\n"
            "\tbool IsCrashRedirected() const "
            "{ return bCrashRedirected; }\n"
            "};\n"
        )
        split_inline = (
            "public:\n"
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", IS_CRASH_REDIRECTED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", IS_CRASH_REDIRECTED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_crash_redirected_cpp_body(
        self,
    ) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        self.assertNotIn("{", IS_CRASH_REDIRECTED)
        ...
        self.assertNotIn("return false", IS_CRASH_REDIRECTED)
        self.assertNotIn("return true", IS_CRASH_REDIRECTED)
        self.assertNotIn("bCrashRedirected", IS_CRASH_REDIRECTED)

    def test_contract_does_not_relock_mission08_neighbors(self) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        for token in MISSION08_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        self.assertNotIn("OpenSensorExposure", IS_CRASH_REDIRECTED)
        ...
        self.assertNotIn("GetFriendlySeparationMeters", locked_only)

    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        self.assertNotIn(leftover_open_safe_window_decl(), locked_only)
        self.assertNotIn(
            leftover_open_safe_window_decl(),
            IS_CRASH_REDIRECTED,
        )
        self.assertNotIn(leftover_arm_safe_fallback_decl(), locked_only)
        self.assertNotIn(
            leftover_arm_safe_fallback_decl(),
            IS_CRASH_REDIRECTED,
        )
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("OpticalTracker", IS_CRASH_REDIRECTED)
        self.assertNotIn("WeaponServo", IS_CRASH_REDIRECTED)
        self.assertNotIn("CountermeasurePod", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        self.assertNotIn("GetGunnerMount", IS_CRASH_REDIRECTED)
        self.assertNotIn("GetChinTurret", IS_CRASH_REDIRECTED)
        self.assertNotIn("GetSensorTurret", IS_CRASH_REDIRECTED)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_CRASH_REDIRECTED)
        self.assertNotIn("GetChinMuzzleLocation", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        self.assertNotIn("ESkyguardApacheSystem", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        self.assertNotIn("ASkyguardGunner", IS_CRASH_REDIRECTED)
        self.assertNotIn("FillAndFinalize", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        self.assertNotIn("SkyguardRadarNode", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", IS_CRASH_REDIRECTED)
        self.assertNotIn("USkyguardBriefingWidget", IS_CRASH_REDIRECTED)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            IS_CRASH_REDIRECTED,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", IS_CRASH_REDIRECTED)
        self.assertNotIn("ESkyguardMissionSkylineStyle", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        self.assertNotIn("MinHeightFromOriginCm", IS_CRASH_REDIRECTED)
        self.assertNotIn("MaxIntegrity", IS_CRASH_REDIRECTED)
        self.assertNotIn("SkyguardApacheAircraft.h", IS_CRASH_REDIRECTED)
        self.assertNotIn("SkyguardPathfinder", IS_CRASH_REDIRECTED)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, IS_CRASH_REDIRECTED)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", IS_CRASH_REDIRECTED)
    ...
    def test_minimum_weapon_separation_is_not_harbor_interval(self) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        adjacent = HARBOR_ADJACENT_SEPARATION_FIELD
        self.assertNotIn(adjacent, locked_only)
        self.assertNotIn(adjacent, IS_CRASH_REDIRECTED)
        self.assertNotIn(
            "MinimumWeaponSeparationMeters",
            IS_CRASH_REDIRECTED,
        )
        self.assertNotIn("450.f", IS_CRASH_REDIRECTED)
        for token in leftover_harbor_tokens():
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_CRASH_REDIRECTED),
            IS_CRASH_REDIRECTED,
        )
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        # section for those tokens. Harbor-adjacent
        # MinimumWeaponSeparationMeters is not a Harbor
        # interval retune. Apache MaxIntegrity is not a
    ...
    def test_harbor_interval_tokens_fail_closed_if_present(self) -> None:
        locked_only = f"{IS_CRASH_REDIRECTED}\n"
        file_text = this_file_text()
        for token in leftover_harbor_tokens():
            self.assertNotIn(token, locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "lifeline hunter IsCrashRedirected contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, IS_CRASH_REDIRECTED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                IS_CRASH_REDIRECTED.lower(),
                f"lifeline hunter IsCrashRedirected contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_CRASH_REDIRECTED)

    def test_contract_is_is_crash_redirected_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        for token in HARBOR_ADJACENT_NOT_HARBOR_INTERVAL:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CRASH_REDIRECTED)
        ...
        self.assertNotIn("return ", IS_CRASH_REDIRECTED)
        self.assertNotIn("{", IS_CRASH_REDIRECTED)
        self.assertTrue(IS_CRASH_REDIRECTED.startswith("bool "))
        self.assertTrue(IS_CRASH_REDIRECTED.endswith(";"))
        self.assertIn(" const", IS_CRASH_REDIRECTED)
        self.assertIn(UFUNCTION_BOSS, section)
