...
# then `int32 MaxSimulationSubsteps = 8;` (HAS initializer).
# Type is **int32**, not float. ClampMin IS `"1"` (integer string, not `"1.0"`).
# ClampMax IS `"16"`. Default IS `= 8`.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Safety`. EditAnywhere IS present.
...
# `= 8`, if LOCKED_DECL is still `float` or ClampMin is still `"100.0"`,
...
# leftover-safe unique Category path `Skyguard|Boss|Encounter|Safety`.
# Fail-closed if wrap Category is leftover Route
# `Skyguard|Boss|Encounter|Route` / Commands / Telegraph
...
# Clone wrap was leftover Pathfinder Safety draft MaxLateralOffsetCm
# Category="Skyguard|Boss|Encounter|Safety", meta=(ClampMin="100.0")
# wrapping `float MaxLateralOffsetCm = 3600.f;`
# — RETARGET hard: type from float to int32,
# ClampMin from `"100.0"` to `"1"` (integer string, not `"1.0"`),
# ADD ClampMax `"16"`.
# wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Safety", meta=(ClampMin="1", ClampMax="16")
# (NO spaces around =).
# LOCKED_DECL is `int32 MaxSimulationSubsteps = 8;`
...
# and NOT leftover analog `float MaxSimulationSubsteps = 8;`
...
# AS THE LOCKED DECL.
# Fail-closed if LOCKED_DECL is still `float` or ClampMin is still `"100.0"`.
...
# or Category="Skyguard" without |Boss|Encounter|Safety.
...
# or default is not 8.
# Default is `= 8` not `= 3600.f` / `= 0.05f` / `= 850.f` /
# `= 700.f` / `= 1250.f` / `= 9000.f`.
...
TARGET = "int32 MaxSimulationSubsteps = 8;"
TARGET_WRONG_BARE = "int32 MaxSimulationSubsteps;"
TARGET_WRONG_INIT = "int32 MaxSimulationSubsteps = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "int32 MaxSimulationSubsteps = true;"
TARGET_WRONG_ZERO = "int32 MaxSimulationSubsteps = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MaxSimulationSubsteps;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MaxSimulationSubsteps;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MaxSimulationSubsteps;"
...
STOP_BEFORE_SKYLINE_STYLE = "CommandLateralBiasCm"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_max_simulation_substeps"
...
MAX_SIMULATION_SUBSTEPS_DECL_RE = re.compile(
    r"int32\s+MaxSimulationSubsteps\s*=\s*8\s*;"
)
MAX_SIMULATION_SUBSTEPS_BARE_RE = re.compile(
    r"int32\s+MaxSimulationSubsteps\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 MaxSimulationSubsteps = 8;`.
    ...
    # MaxSimulationSubsteps without the 8 initializer.
    ...
    if MAX_SIMULATION_SUBSTEPS_DECL_RE.search(compact) is None:
        return False
    if MAX_SIMULATION_SUBSTEPS_BARE_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "MaxSimulationSubsteps":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+MaxSimulationSubsteps\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 MaxSimulationSubsteps "
...
class PathfinderEncounterMaxSimulationSubstepsFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxSimulationSubsteps"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MaxSimulationSubsteps"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaxSimulationSubsteps", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_max_simulation_substeps_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Safety"', section)
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Safety"', specifiers)
        ...
        self.assertIn('ClampMax="16"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "int32 MaxSimulationSubsteps;",
            compact_origin.split("MaxSimulationSubsteps = 8;")[0]
            + compact_origin.split("MaxSimulationSubsteps = 8;")[-1]
            if "MaxSimulationSubsteps = 8;" in compact_origin
            else compact_origin,
        )
    ...
    def test_max_simulation_substeps_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 MaxSimulationSubsteps"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 8", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 MaxSimulationSubstepss = 8;\n",
            "\tfloat MaxSimulationSubsteps = 8;\n",
            "\tFName MaxSimulationSubsteps = NAME_None;\n",
            "\tint32 MaxSimulationSubsteps = " + forty + ";\n",
            "\tint32 MaxSimulationSubsteps = " + eighty + ";\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Safety"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', theater_specs)
        self.assertIn('Category="Skyguard|Boss|Encounter|Safety"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxSimulationSubsteps"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "MaxSimulationSubsteps"))
        self.assertTrue(has_identifier(section, "MaxSimulationSubsteps"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMax="16"', UPROPERTY_EDIT_READWRITE)
