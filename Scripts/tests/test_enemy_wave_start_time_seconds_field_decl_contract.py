# THIS IS leftover-safe FSkyguardEnemyWaveDefinition StartTimeSeconds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "0.0"))
# then `float StartTimeSeconds = 0.f;` (initializer IS `0.f`,
# not `0.0f` / `0.0` / `0` / Harbor 40/80 / leftover `2.f`).
...
# if meta = (ClampMin = "0.0") is missing or changed,
# or if the initializer is missing or not `= 0.f`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardEnemyWaveDefinition` body ONLY after
# `struct FSkyguardEnemyWaveDefinition`. Stop at
# `struct FSkyguardBossWeakPointDefinition` and later structs.
...
# Do NOT contract sibling fields WaveId /
# Formations / CompletionObjectiveId.
...
# WorldLocation #1340, or TargetAirspeedKph / LookAhead /
# CombatOrbit drafts.
...
# Analog: leftover enemy-wave-defaults then this
# isolated StartTimeSeconds field decl, same as leftover
...
# `FName PointId` / `FName WaveId` / no ClampMin /
# no `=`, retarget: type is float StartTimeSeconds,
# initializer is `= 0.f`, specifiers are EditAnywhere +
# BlueprintReadOnly + meta = (ClampMin = "0.0"), no Category.
...
STRUCT_NAME = "FSkyguardEnemyWaveDefinition"
...
TARGET = "float StartTimeSeconds = 0.f;"
TARGET_WRONG_EQ_NONE = "float StartTimeSeconds = 0.0f;"
TARGET_WRONG_FALSE = "float StartTimeSeconds = false;"
TARGET_WRONG_TRUE = "float StartTimeSeconds = true;"
TARGET_WRONG_ZERO = "float StartTimeSeconds = 0;"
TARGET_WRONG_FLOAT = "FName StartTimeSeconds;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "float StartTimeSeconds;"
TARGET_WRONG_LOOKAHEAD = "float StartTimeSeconds = 2.f;"
TARGET_WRONG_POINT_ZERO = "float StartTimeSeconds = 0.0;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float StartTimeSeconds = 0.f`.
    # Reject missing initializer, `0.0f` / `0.0` / `0` /
    # Harbor 40/80 / leftover look-ahead `2.f` / leftover Health.
    # Do not accept sibling WaveId / Formations /
    # CompletionObjectiveId fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+StartTimeSeconds\s*=\s*0\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+StartTimeSeconds\s*=\s*0\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+StartTimeSeconds\s*=\s*0\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+StartTimeSeconds\s*=\s*0\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+StartTimeSeconds\s*=\s*2\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+StartTimeSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+StartTimeSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+StartTimeSeconds\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+StartTimeSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float StartTimeSeconds is missing from "
...
def require_start_time_seconds_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} StartTimeSeconds "
            "UPROPERTY"
        )
    ...
    if 'ClampMin = "0.0"' not in specifiers:
        raise AssertionError(
            'meta = (ClampMin = "0.0") is missing or changed on '
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} "
            "StartTimeSeconds UPROPERTY"
        )
    return specifiers


class EnemyWaveStartTimeSecondsFieldDeclContractTests(unittest.TestCase):
    def test_enemy_wave_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "StartTimeSeconds"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "WaveId"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_DEFINITION, section)
        ...
        self.assertIn("StartTimeSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_start_time_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_WAVE_ID};\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_start_time_seconds_specifiers(section)
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float StartTimeSeconds = 0.f;", compact_origin)
        self.assertNotIn("StartTimeSeconds = 0.0f", compact_origin)
        self.assertNotIn("StartTimeSeconds = 0.0;", compact_origin)
        self.assertNotIn("StartTimeSeconds = 0;", compact_origin)
        self.assertNotIn("StartTimeSeconds = 2.f", compact_origin)
        self.assertNotIn("StartTimeSeconds = 160.f", compact_origin)
        self.assertNotIn("StartTimeSeconds = false", compact_origin)

    def test_start_time_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float StartTimeSeconds"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tfloat StartTimeSecond = 0.f;\n",
            "\tint32 StartTimeSeconds = 0;\n",
            "\tbool StartTimeSeconds;\n",
            f"\t{TARGET_WRONG_EMPTY}\n",
            f"\t{TARGET_WRONG_LOOKAHEAD}\n",
            f"\t{TARGET_WRONG_POINT_ZERO}\n",
            "\tfloat StartTimeSeconds = " + forty + ";\n",
            "\tfloat StartTimeSeconds = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_start_time_seconds_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_start_time_seconds_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_start_time_seconds_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_start_time_seconds_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_start_time_seconds_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_start_time_seconds_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_start_time_seconds_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
    ...
    def test_does_not_contract_sibling_enemy_wave_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('meta = (ClampMin = "0.0")', UPROPERTY_EDIT_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("Category", UPROPERTY_EDIT_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "enemy-wave StartTimeSeconds field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enemy_wave_start_time_seconds_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_FORMATIONS, locked_only)
