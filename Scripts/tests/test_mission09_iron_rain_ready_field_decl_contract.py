...
# bIronRainReady FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission09 readiness-struct
# bIronRainReady. THIS IS NOT leftover Mission08
# FSkyguardMission08IntegrationReadiness fields
# (Mission08 has no bIronRainReady; do not retarget
# to a different Mission08 field name as the lock).
# THIS IS NOT leftover Mission09 director
# IronRainSpawnLocation #1096. THIS IS NOT leftover
# Mission09 director PoolBudget #1099. THIS IS NOT
# leftover Mission09 director Readiness #1100. THIS
# IS NOT leftover Mission08 readiness-struct
# bGunnerReady / leftover Mission08 director fields.
# THIS IS NOT leftover Mission07/06/05/04/03/02/01/10
# same-name readiness-struct drafts. THIS IS NOT
# leftover Mission09 Blueprint UFUNCTION methods.
# THIS IS NOT leftover director-class public
# UPROPERTY fields. THIS IS NOT leftover spawn-
# location / leftover runtime-ready flag (skip leftover
# b + Ya + kRuntimeReady forever; it sits earlier in
# this same struct between bMapAssemblyReady and
# bGunnerReady -- do not test it). THIS IS NOT leftover
# Mission08/09 wave-state enums, protected-target
# enums, protected-target-runtime-defaults, leftover
# hoist-window-runtime-defaults #ec79, leftover
# Mission09 pool-runtime-defaults #5426 / leftover
# Mission09 pool-budget-defaults #4537.
...
# merge #665 #666 #668-#673, or open drafts #674-#1103.
...
# bMissionDefinitionValid #1101 / bCampaignDefinitionValid
# #1102 / bCampaignRuntimeStarted #1103.
...
# INDEX_NONE or lock bIronRainReady
# construction in the .cpp. This is leftover-safe
# Mission09 IntegrationReadiness bIronRainReady
...
# director Readiness #1100 / MissionDefinition #1092 / IronRainSpawnLocation #1096 / PoolBudget #1099,
...
# drafts #674-#1103 including leftover Mission02
...
# drafts #674-#1103. Do not reopen leftover
...
IRON_RAIN_READY_TRUE = (
    "bool bIronRainReady = true;"
)
LOCKED_DECL = IRON_RAIN_READY
...
# an isolated Python bIronRainReady
...
LEFTOVER_MISSION09_CAMPAIGN_DEFINITION_VALID_NOT_LOCKED = (
...
LEFTOVER_MISSION09_CAMPAIGN_RUNTIME_STARTED_NOT_LOCKED = (
...
LEFTOVER_MISSION09_IRON_RAIN_SPAWN_NOT_LOCKED = (
    "test_mission09_iron_rain_spawn_location_field_decl_contract.py",
    "cursor/mission09-iron-rain-spawn-location-field-decl-contract-c332",
    IRON_RAIN_SPAWN_LOCATION,
)
...
class Mission09IronRainReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bIronRainReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_iron_rain_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_iron_rain_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bIronRainReady"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bIronRainReady", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bIronRainReady", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bIronRainReady"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bIronRainReady", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission09 bIronRainReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission09 bIronRainReady contains "
    ...
    def test_contract_is_root_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_mission09_iron_rain"
            "_spawn_location_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission09_pool_budget"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        for token in LEFTOVER_MISSION09_MISSION_DEFINITION_VALID_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION09_CAMPAIGN_DEFINITION_VALID_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION09_CAMPAIGN_RUNTIME_STARTED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION09_IRON_RAIN_SPAWN_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION09_POOL_BUDGET_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
