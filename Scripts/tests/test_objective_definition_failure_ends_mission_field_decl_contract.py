...
# bool, if the initializer is not `= false`, or if Harbor
# tokens built as 40 + ".f" / 80 + ".f" appear in the
# parse window.
...
# then this isolated bFailureEndsMission field decl, same
# as leftover weather-profile-defaults then isolated Weather,
...
# retarget: type is bool bFailureEndsMission = false,
# specifiers are EditAnywhere + BlueprintReadOnly,
# no Category.
...
TARGET = "bool bFailureEndsMission = false;"
TARGET_WRONG_BARE = "bool bFailureEndsMission;"
TARGET_WRONG_TRUE = "bool bFailureEndsMission = true;"
TARGET_WRONG_ONE = "bool bFailureEndsMission = 1;"
TARGET_WRONG_ZERO = "bool bFailureEndsMission = 0;"
TARGET_WRONG_EQ_NONE = "bool bFailureEndsMission = NAME_None;"
TARGET_WRONG_FALSE = "bool bFailureEndsMission = False;"
TARGET_WRONG_FNAME = "FName bFailureEndsMission;"
TARGET_WRONG_FLOAT = "float bFailureEndsMission;"
TARGET_WRONG_INT = "int32 bFailureEndsMission = false;"
TARGET_WRONG_REQUIRED = "bool bRequiredForMissionSuccess = true;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept a bare field, `= true` / 1 / 0 /
    # FName / float / leftover Health, or sibling
    # ObjectiveId / DisplayName / Type / RequiredProgress /
    # bRequiredForMissionSuccess / ScoreReward fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"bool\s+bFailureEndsMission\s*=\s*false\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"bool\s+bFailureEndsMission\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bFailureEndsMission\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bFailureEndsMission\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bFailureEndsMission\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+bFailureEndsMission\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bFailureEndsMission\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bFailureEndsMission is "
        f"missing from origin/main:{HEADER_PATH} struct "
        f"{STRUCT_NAME} body"
    )
...
def require_type_specifiers(section: str) -> str:
...
class ObjectiveDefinitionFailureEndsMissionFieldDeclContractTests(
    unittest.TestCase
):
    def test_objective_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bFailureEndsMission"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bFailureEndsMission"), section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("bFailureEndsMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_failure_ends_mission_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_type_specifiers(section)
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
        ...
        self.assertNotIn("bFailureEndsMission = NAME_None", compact_origin)
        self.assertNotIn("bFailureEndsMission = true", compact_origin)
        self.assertNotIn("bFailureEndsMission = 0", compact_origin)
        self.assertNotIn("bFailureEndsMission = 1", compact_origin)
        self.assertNotIn("bFailureEndsMission = 160.f", compact_origin)
        self.assertNotIn(
            "bool bRequiredForMissionSuccess = false;",
            compact_origin,
        )

    def test_failure_ends_mission_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bFailureEndsMission"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("ESkyguardMissionObjectiveType", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_REQUIRED}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_INT}\n",
            f"\t{TARGET_WRONG_REQUIRED}\n",
            f"\t{TARGET_WRONG_HEALTH}\n",
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_OBJECTIVE_ID};\n",
            f"\tFText {SIBLING_DISPLAY_NAME};\n",
            f"\tESkyguardMissionObjectiveType {SIBLING_TYPE} = "
            "ESkyguardMissionObjectiveType::DestroyTargets;\n",
            f"\tbool {SIBLING_REQUIRED_FOR_SUCCESS} = true;\n",
            "\tbool bFailureEndsMissio = false;\n",
            "\tbool bFailureEndsMissions = false;\n",
            "\tfloat bFailureEndsMission = " + forty + ";\n",
            "\tfloat bFailureEndsMission = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_type_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_type_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_type_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_type_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_type_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "objective-definition bFailureEndsMission field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "objective_definition_failure_ends_mission"
            "_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
