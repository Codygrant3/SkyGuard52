# THIS IS leftover-safe ASkyguardGunner CpgMpdRightText.
...
# then BARE `TObjectPtr<UTextRenderComponent> CpgMpdRightText;`
# (no initializer).
...
# Stop BEFORE sibling CpgEufdText as a claimed slot.
...
# CpgTedacText, CpgMpdLeftText, CpgEufdText, Boom,
# GunnerCamera, HandMesh.
# Do NOT claim leftover CpgGripRight / CpgGripLeft /
# CpgReticleV / CpgReticleH / CpgTedacBezel as this slot.
# This is a TEXT slot. Do not lock leftover mesh CpgMpdRight
# #1602 as this slot. Do not lock leftover mesh CpgMpdLeft
# #1603. Do not lock leftover CpgConsoleRight #1612 /
# CpgRailLeft #1611 / CpgRailRight #1610. Fail-closed
# leftover Apache Canopy #1559 / PilotCanopy #1572.
# CpgMpdRightText fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UTextRenderComponent> CpgMpdRightText;`
...
# CpgMpdRightText fail-closed if LOCKED_DECL is leftover
# mesh CpgMpdRight / leftover mesh CpgMpdLeft / CpgRailRight /
# CpgRailLeft / CpgConsoleRight / CpgReticleV / CpgGripRight /
# CpgGripLeft / CpgReticleH / CpgTedacBezel / CpgTedacText /
# CpgMpdLeftText / CpgEufdText / CpgEufd / CpgTedac /
...
TARGET = "TObjectPtr<UTextRenderComponent> CpgMpdRightText;"
TARGET_WRONG_INIT = "TObjectPtr<UTextRenderComponent> CpgMpdRightText = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UTextRenderComponent> CpgMpdRightText = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UTextRenderComponent> CpgMpdRightText = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgMpdRightText;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgMpdRightText;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CpgMpdRightText;"
...
CPG_MPD_RIGHT_TEXT_DECL_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgMpdRightText\s*;"
)
CPG_MPD_RIGHT_TEXT_INIT_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgMpdRightText\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover mesh CpgMpdRight / leftover mesh CpgMpdLeft /
    # CpgRailRight / CpgRailLeft / CpgConsoleRight / CpgReticleV /
    # leftover Apache Canopy, leftover Apache
    # PilotCanopy, or an initializer on CpgMpdRightText
    ...
    if CPG_MPD_RIGHT_TEXT_DECL_RE.search(compact) is None:
        return False
    if CPG_MPD_RIGHT_TEXT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_EUFD_TEXT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_EUFD_TEXT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_EUFD_TEXT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UTextRenderComponent>\s+CpgMpdRightText\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UTextRenderComponent> CpgMpdRightText "
...
class GunnerCpgMpdRightTextFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgMpdRightText"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_EUFD_TEXT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgMpdRightText"), section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UTextRenderComponent> CpgMpdRightText;", compact_origin)
        self.assertNotIn("CpgMpdRightText = 160.f", compact_origin)
    ...
    def test_cpg_mpd_left_text_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UTextRenderComponent> CpgMpdRightText"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_CPG_MPD_RIGHT))
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UTextRenderComponent> CpgMpdRightText;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UTextRenderComponent> CpgMpdRightTexts;\n",
            "\tint32 CpgMpdRightText = 140;\n",
            "\tbool CpgMpdRightText = true;\n",
            "\tfloat CpgMpdRightText = " + forty + ";\n",
            "\tfloat CpgMpdRightText = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_eufd_text_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "CpgMpdRightText"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgMpdRightText"))
        self.assertTrue(has_identifier(section, "CpgMpdRightText"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgMpdRightText field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_mpd_right_text_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_CPG_MPD_RIGHT))
