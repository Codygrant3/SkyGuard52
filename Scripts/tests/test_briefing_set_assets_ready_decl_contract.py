...
# or lock the SetAssetsReady body in the .cpp.
...
# This lane only adds an isolated Python SetAssetsReady
# declaration contract. Stay off ConfigureFromMission
# (in-flight sibling), AdvanceBriefing (sibling this
# wave), AcknowledgeAndLaunch, CanLaunch,
...
# leftover ConfigureFromMission, and leftover
# AdvanceBriefing stay sibling-only.
LOCKED_SCRIPTS = (
...
# here. ConfigureFromMission / AdvanceBriefing /
...
# .cpp SetAssetsReady body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionBriefingComponent::SetAssetsReady",
    "SkyguardMissionBriefingComponent.cpp",
)
SIBLING_TYPES = (
...
class BriefingSetAssetsReadyDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_ASSETS_READY), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_ASSETS_READY)
        self.assertIn("SetAssetsReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SET_ASSETS_READY))

    def test_missing_set_assets_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
            "\tbool ConfigureFromMission("
            "USkyguardMissionDefinition* Mission);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_ASSETS_READY)
        self.assertIn("SetAssetsReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_ASSETS_READY)
        self.assertIn("SetAssetsReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_ASSETS_READY), section)
        self.assertNotIn("BlueprintPure", SET_ASSETS_READY)
        self.assertNotIn("UFUNCTION", SET_ASSETS_READY)
        self.assertNotIn("Category", SET_ASSETS_READY)
        self.assertNotIn("BlueprintCallable", SET_ASSETS_READY)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_ASSETS_READY)
        self.assertIn("SetAssetsReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = "\tvoid SetAssetsReady();\n"
        wrong_return = "\tbool SetAssetsReady(bool bReady);\n"
        added_const = "\tvoid SetAssetsReady(bool bReady) const;\n"
        int_arg = "\tvoid SetAssetsReady(int32 bReady);\n"
        const_arg = "\tvoid SetAssetsReady(const bool bReady);\n"
        for region in (
            missing_arg,
            wrong_return,
            added_const,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_ASSETS_READY)
            self.assertIn("SetAssetsReady", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_assets_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SET_ASSETS_READY),
            SET_ASSETS_READY,
        )
        self.assertTrue(has_declaration(section, SET_ASSETS_READY))
        self.assertEqual(declaration_count(section, SET_ASSETS_READY), 1)
        self.assertTrue(SET_ASSETS_READY.startswith("void "), SET_ASSETS_READY)
        self.assertTrue(SET_ASSETS_READY.endswith(";"), SET_ASSETS_READY)
        self.assertIn("bool bReady", SET_ASSETS_READY)
        self.assertNotIn("INDEX_NONE", SET_ASSETS_READY)
        self.assertNotIn("{", SET_ASSETS_READY)
        self.assertNotIn("}", SET_ASSETS_READY)
        self.assertNotIn("return ", SET_ASSETS_READY)
        self.assertNotIn(" const", SET_ASSETS_READY)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetAssetsReady(bool bReady);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, SET_ASSETS_READY),
                section,
            )
            self.assertEqual(
                require_declaration(section, SET_ASSETS_READY),
            ...
            self.assertEqual(
                declaration_count(section, SET_ASSETS_READY),
                1,
            )
        one_line = f"{{\npublic:\n\t{SET_ASSETS_READY}\n}}\n"
        self.assertTrue(has_declaration(one_line, SET_ASSETS_READY))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SET_ASSETS_READY), section)
        self.assertEqual(
            require_declaration(section, SET_ASSETS_READY),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_ASSETS_READY}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_ASSETS_READY)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_ASSETS_READY)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_assets_ready_cpp_body(self) -> None:
        locked_only = f"{SET_ASSETS_READY}\n"
        self.assertNotIn("{", SET_ASSETS_READY)
        self.assertNotIn("}", SET_ASSETS_READY)
        self.assertNotIn("return ", SET_ASSETS_READY)
        ...
        self.assertNotIn("return false", SET_ASSETS_READY)
        self.assertNotIn("return true", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("ConfigureFromMission", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_advance_briefing(self) -> None:
        ...
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("AdvanceBriefing", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_acknowledge_and_launch(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_AND_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("AcknowledgeAndLaunch", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_can_launch(self) -> None:
        ...
        for neighbor in CAN_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("CanLaunch", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_get_briefing_state(self) -> None:
        ...
        for neighbor in GET_BRIEFING_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("GetBriefingState", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_SECONDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("GetElapsedSeconds", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_get_minimum_warmup_seconds(self) -> None:
        ...
        for neighbor in GET_MINIMUM_WARMUP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("GetMinimumWarmupSeconds", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("GetBriefingText", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_get_radio_chatter(self) -> None:
        ...
        for neighbor in GET_RADIO_CHATTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("GetRadioChatter", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_ASSETS_READY)
        self.assertNotIn("Unconfigured", SET_ASSETS_READY)
        self.assertNotIn("Warming", SET_ASSETS_READY)
        self.assertNotIn("Launched", SET_ASSETS_READY)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            SET_ASSETS_READY,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_ASSETS_READY)
        self.assertNotIn("FSkyguardBriefingCard", SET_ASSETS_READY)
        self.assertNotIn("FSkyguardBriefingRadioRow", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ASSETS_READY)
        self.assertNotIn("FillResultCombatStats", SET_ASSETS_READY)
        self.assertNotIn("ASkyguardGunner", SET_ASSETS_READY)
        self.assertNotIn("FillAndFinalize", SET_ASSETS_READY)
        self.assertNotIn("FillAndFail", SET_ASSETS_READY)
        self.assertNotIn("ApplyHydraForClusters", SET_ASSETS_READY)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_ASSETS_READY}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_ASSETS_READY),
            SET_ASSETS_READY,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing SetAssetsReady contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, SET_ASSETS_READY.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing SetAssetsReady contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_ASSETS_READY)

    def test_contract_is_set_assets_ready_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(SET_ASSETS_READY.startswith("void "))
        self.assertTrue(SET_ASSETS_READY.endswith(";"))
        self.assertIn(UFUNCTION_BRIEFING, section)
