# THIS IS leftover-safe ASkyguardApacheAircraft EngineLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> EngineLeft;`
# (no initializer).
...
# Stop BEFORE sibling EngineRight as a claimed slot.
# Do NOT claim leftover PilotCanopy (in-flight/opening),
# leftover Canopy #1559, NightVisionTurret, SensorBall,
# SensorTurret, RadarDome, EngineRight, ChinTurret,
# ChinHousing, ChinBarrel, TailRotor, MainRotor,
# RotorMast, AircraftRoot, Fuselage, Nose,
# SilhouetteMesh, MaxIntegrity, CurrentIntegrity,
# HullCollider, PilotMount, GunnerMount, EyeMount,
# WeaponMount.
# EngineLeft is NOT leftover analog
# apache-is-canopy-glass-cracked UFUNCTION, NOT leftover
# Canopy #1559, NOT leftover PilotCanopy
# (in-flight/opening), NOT leftover analog
...
# PilotCanopy, Canopy, NightVisionTurret, SensorBall,
# SensorTurret, RadarDome, EngineRight, TailRotor,
# ChinTurret, MainRotor, RotorMast, StubWingRight,
# VerticalTail, StubWingLeft, TailBoom, Fuselage, Nose,
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> EngineLeft;`
# NOT `FName WeatherIdentity;`, NOT leftover sibling
# `TObjectPtr<UStaticMeshComponent> PilotCanopy;`, NOT
# leftover `TObjectPtr<UStaticMeshComponent> Canopy;`,
# NOT leftover `TObjectPtr<UStaticMeshComponent> EngineRight;`,
# and NOT `TObjectPtr<USceneComponent>`.
...
# leftover PilotCanopy / leftover EngineRight
# AS THE LOCKED DECL.
...
TARGET = "TObjectPtr<UStaticMeshComponent> EngineLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> EngineLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> EngineLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> EngineLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> EngineLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> EngineLeft;"
...
STOP_BEFORE_ENGINE_RIGHT = "EngineRight"
...
LEFTOVER_APACHE_NIGHT_VISION_TURRET = (
...
ENGINE_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+EngineLeft\s*;"
)
ENGINE_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+EngineLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover sibling PilotCanopy / Canopy / EngineRight /
    # Fuselage / AircraftRoot / USceneComponent, or an
    # initializer on EngineLeft as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ENGINE_LEFT_DECL_RE.search(compact) is None:
        return False
    if ENGINE_LEFT_INIT_RE.search(compact):
        return False
    if WEATHER_IDENTITY_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"TObjectPtr<USceneComponent>\s+EngineLeft\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_ENGINE_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_ENGINE_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_ENGINE_RIGHT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+EngineLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> EngineLeft "
...
class ApacheEngineLeftFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EngineLeft"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENGINE_RIGHT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_ENGINE_RIGHT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EngineLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("EngineLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_engine_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> EngineLeft;", compact_origin)
        self.assertNotIn("EngineLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_engine_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> EngineLeft"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> EngineLeft;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> EngineLefts;\n",
            "\tint32 EngineLeft = 140;\n",
            "\tbool EngineLeft = true;\n",
            "\tfloat EngineLeft = " + forty + ";\n",
            "\tfloat EngineLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_engine_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_ENGINE_RIGHT))
        ...
        self.assertNotIn(TARGET_WRONG_ENGINE_RIGHT, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache EngineLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "engine_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
