# THIS IS leftover-safe ASkyguardCampaignTheaterKit LampInstances.
...
# BuildingInstances / SilhouetteInstances / NamedLandmarkMesh /
# AppliedSpec.
...
# LampInstances does not block this isolated UPROPERTY.
...
TARGET = (
    f"TObjectPtr<{HISM}> LampInstances;"
)
TARGET_WRONG_EQ_NONE = (
    f"TObjectPtr<{HISM}> LampInstances = nullptr;"
)
TARGET_WRONG_FALSE = (
    f"TObjectPtr<{HISM}> LampInstances = false;"
)
TARGET_WRONG_TRUE = (
    f"TObjectPtr<{HISM}> LampInstances = true;"
)
TARGET_WRONG_ZERO = (
    f"TObjectPtr<{HISM}> LampInstances = 0.f;"
)
TARGET_WRONG_FLOAT = "float LampInstances;"
TARGET_WRONG_SCENE = (
    "TObjectPtr<USceneComponent> LampInstances;"
)
TARGET_WRONG_STATIC = (
    "TObjectPtr<UStaticMeshComponent> LampInstances;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # LampInstances;`. Do not accept `= nullptr` /
    ...
    if re.search(
        rf"TObjectPtr\s*<\s*{type_name}\s*>\s*LampInstances\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LampInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USceneComponent\s*>\s*LampInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*UStaticMeshComponent\s*>\s*LampInstances\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    raise AssertionError(
        "UPROPERTY for TObjectPtr LampInstances is missing "
...
class TheaterKitLampInstancesFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LampInstances"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("LampInstances", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lamp_instances_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("LampInstances = nullptr", compact_origin)
        self.assertNotIn("LampInstances = false", compact_origin)
        self.assertNotIn("LampInstances = true", compact_origin)
        self.assertNotIn("LampInstances = 0.f", compact_origin)
        self.assertNotIn("LampInstances = 160.f", compact_origin)

    def test_lamp_instances_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("LampInstances", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 LampInstances;\n",
            "\tbool LampInstances;\n",
            "\tfloat LampInstances = " + forty + ";\n",
            "\tfloat LampInstances = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit LampInstances field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "lamp_instances_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
