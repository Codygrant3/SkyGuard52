# THIS IS leftover-safe FSkyguardCampaignMissionSpec ContactKind.
# origin/main form: plain C++ field
# `ESkyguardThreatKind ContactKind = ESkyguardThreatKind::FastAttacker;`
# with in-struct initializer ::FastAttacker. NOT a UPROPERTY wrap.
...
# GroundArmor / HeavyAttacker / RotorScout / PatrolShip
# instead of FastAttacker.
...
# WeatherLabel / TimeOfDayHours / BeatSeconds / ShoreKind /
...
# #85ab.
# Do NOT claim leftover CampaignMissionSpec ShoreKind /
...
# ::FastAttacker. Parse window comes from leftover-safe
...
TARGET_WRONG_BARE = "ESkyguardThreatKind ContactKind;"
TARGET_WRONG_GROUND = (
    "ESkyguardThreatKind ContactKind = "
...
TARGET_WRONG_EQ_NONE = "ESkyguardThreatKind ContactKind = NAME_None;"
TARGET_WRONG_FALSE = "ESkyguardThreatKind ContactKind = false;"
TARGET_WRONG_TRUE = "ESkyguardThreatKind ContactKind = true;"
TARGET_WRONG_ZERO = "ESkyguardThreatKind ContactKind = 0.f;"
TARGET_WRONG_FLOAT = "float ContactKind;"
TARGET_WRONG_HEALTH = "float ContactKind = 160.f;"
TARGET_WRONG_BOOL = "bool ContactKind;"
TARGET_WRONG_INT = "int32 ContactKind;"
TARGET_WRONG_FNAME = "FName ContactKind;"
...
CONTACT_KIND_DECL_RE = re.compile(
    r"ESkyguardThreatKind\s+ContactKind\s*=\s*"
    r"ESkyguardThreatKind\s*::\s*FastAttacker\s*;"
)
CONTACT_KIND_INIT_RE = re.compile(
    r"ESkyguardThreatKind\s+ContactKind\s*=\s*([^;]+);"
)
WRONG_INITIALIZERS = (
    "ESkyguardThreatKind::GroundArmor",
...
def contact_kind_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardThreatKind\s+ContactKind\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardThreatKind ContactKind =
    # ESkyguardThreatKind::FastAttacker;`.
    # Do not accept missing initializer, leftover ShoreKind
    # GroundArmor, leftover SupportKind HeavyAttacker,
    # leftover ExtractKind RotorScout, leftover Climax
    # PatrolShip, leftover StormRainBeatKit Kinds, leftover
    # Harbor Breaker Contact, `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` when origin/main locks
    # ::FastAttacker. Do not accept leftover TheaterKit
    # Category clones as this CampaignMissionSpec parse
    # window.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CONTACT_KIND_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+ContactKind\b", compact):
        return False
    if re.search(r"FName\s+ContactKind\b", compact):
        return False
    if re.search(r"bool\s+ContactKind\b", compact):
        return False
    if re.search(r"int32\s+ContactKind\b", compact):
        return False
    if contact_kind_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    for match in CONTACT_KIND_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != "ESkyguardThreatKind::FastAttacker":
            return False
    for leftover_init in WRONG_INITIALIZERS:
        if leftover_init in compact and leftover_init.split("::")[1] != (
            ...
            if re.search(
                r"ContactKind\s*=\s*" + re.escape(leftover_init),
                compact,
            ):
                return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardThreatKind\s+ContactKind\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardThreatKind ContactKind is missing "
...
class CampaignMissionSpecContactKindFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_enum_not_uproperty(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardThreatKind ContactKind"),
        )
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("::FastAttacker", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("ContactKind", str(raised.exception))
    ...
    def test_leftover_storm_rain_kinds_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn("ContactKind", LEFTOVER_STORM_KINDS_DECL)
        leftover = (
    ...
    def test_missing_contact_kind_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_kind_fields_do_not_satisfy_contact_kind(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SHORE}\n"
    ...
    def test_initializer_must_be_fast_attacker(self) -> None:
        missing = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
        ...
        self.assertNotIn(
            "ContactKind = ESkyguardThreatKind::GroundArmor",
            compact_origin,
        )
        self.assertNotIn(
            "ContactKind = ESkyguardThreatKind::HeavyAttacker",
            compact_origin,
        )
        self.assertNotIn(
            "ContactKind = ESkyguardThreatKind::RotorScout",
            compact_origin,
        )
        self.assertNotIn(
            "ContactKind = ESkyguardThreatKind::PatrolShip",
            compact_origin,
        )
        self.assertNotIn("ContactKind = NAME_None", compact_origin)
        self.assertNotIn("ContactKind = false", compact_origin)
        self.assertNotIn("ContactKind = true", compact_origin)
        self.assertNotIn("ContactKind = 0.f", compact_origin)
        self.assertNotIn("ContactKind = 160.f", compact_origin)

    def test_contact_kind_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tESkyguardThreatKind ContactKinds = "
            "ESkyguardThreatKind::FastAttacker;\n",
            "\tint32 ContactKind;\n",
            "\tbool ContactKind;\n",
            "\tfloat ContactKind = " + forty + ";\n",
            "\tfloat ContactKind = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    contact_kind_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(contact_kind_has_uproperty_wrap(origin), origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tESkyguardThreatKind\n"
            "\tContactKind = ESkyguardThreatKind::FastAttacker;\n",
            "\tESkyguardThreatKind   ContactKind = "
            "ESkyguardThreatKind::FastAttacker;\n",
            "\tESkyguardThreatKind\tContactKind = "
            "ESkyguardThreatKind::FastAttacker;\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec ContactKind field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "contact_kind_field_decl_contract.py"
        ))
        self.assertIn("campaign_mission_spec", Path(__file__).name)
        self.assertIn("contact_kind", Path(__file__).name)
