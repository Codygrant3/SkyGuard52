# THIS IS leftover-safe ASkyguardCampaignTheaterKit BuildingInstances.
...
# BuildingInstances;` (no initializer).
...
# BuildingInstances does not block this isolated UPROPERTY.
...
TARGET = (
    f"TObjectPtr<{HISM}> BuildingInstances;"
)
TARGET_WRONG_EQ_NONE = (
    f"TObjectPtr<{HISM}> BuildingInstances = nullptr;"
)
TARGET_WRONG_FALSE = (
    f"TObjectPtr<{HISM}> BuildingInstances = false;"
)
TARGET_WRONG_TRUE = (
    f"TObjectPtr<{HISM}> BuildingInstances = true;"
)
TARGET_WRONG_ZERO = (
    f"TObjectPtr<{HISM}> BuildingInstances = 0.f;"
)
TARGET_WRONG_FLOAT = "float BuildingInstances;"
TARGET_WRONG_SCENE = (
    "TObjectPtr<USceneComponent> BuildingInstances;"
)
TARGET_WRONG_STATIC = (
    "TObjectPtr<UStaticMeshComponent> BuildingInstances;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # BuildingInstances;`. Do not accept `= nullptr` /
    # `= false` / `= true` / `= 0.f` / `= 160.f` /
    # leftover float Health when origin/main has a bare
    # TObjectPtr. Do not accept sibling Root /
    # RoadInstances / LampInstances fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    type_name = re.escape(HISM)
    if re.search(
        rf"TObjectPtr\s*<\s*{type_name}\s*>\s*BuildingInstances\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BuildingInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USceneComponent\s*>\s*BuildingInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*UStaticMeshComponent\s*>\s*BuildingInstances\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            ...
            r"\s*BuildingInstances\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr BuildingInstances is missing "
...
class TheaterKitBuildingInstancesFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BuildingInstances"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("BuildingInstances", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_building_instances_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("BuildingInstances = nullptr", compact_origin)
        self.assertNotIn("BuildingInstances = false", compact_origin)
        self.assertNotIn("BuildingInstances = true", compact_origin)
        self.assertNotIn("BuildingInstances = 0.f", compact_origin)
        self.assertNotIn("BuildingInstances = 160.f", compact_origin)

    def test_building_instances_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("BuildingInstances", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 BuildingInstances;\n",
            "\tbool BuildingInstances;\n",
            "\tfloat BuildingInstances = " + forty + ";\n",
            "\tfloat BuildingInstances = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit BuildingInstances field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "building_instances_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
