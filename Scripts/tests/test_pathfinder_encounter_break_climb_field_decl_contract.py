...
# then `float BreakClimbCm = 260.f;` (HAS initializer).
...
# BlueprintReadWrite IS present. ClampMin IS `"0.0"`. Default IS `= 260.f`.
# Clone wrap ClampMin WAS `"0.0"` leftover Amplitude Route wrap.
# THIS slot ClampMin IS `"0.0"` (NO spaces around =). Category IS
# Commands, NOT leftover Route / Safety / Telegraph.
# Default IS `= 260.f` not leftover analog `= 320.f` /
# leftover analog `LockWindowClimbCm = 650.f` /
# leftover analog `CommandLateralBiasCm = 850.f`.
# 260.f is NOT Harbor 40 or 80.
...
# It is NOT leftover analog ApproachSpeedCmPerSecond /
# LockWindowSpeedCmPerSecond = 850.f (leave siblings uncontracted).
...
# Fail-closed if wrap Category is still leftover Route
# `Skyguard|Boss|Encounter|Route` / leftover Safety /
# leftover Telegraph.
...
# same-name 45000.f).
# Do NOT claim sibling RouteLengthCm = 45000.f /
# ApproachSpeedCmPerSecond = 1250.f /
# LockWindowSpeedCmPerSecond = 850.f /
# CriticalSpeedCmPerSecond = 700.f /
# IngressSwayWavelengthCm = 9000.f / IngressSwayAmplitudeCm = 320.f /
# LockWindowClimbCm = 650.f / CommandLateralBiasCm = 850.f /
# LockWindowClimbSeconds = 2.4f / CriticalTurnRadiusCm = 1500.f /
# CriticalTurnSeconds = 5.5f as this slot.
# Do NOT claim leftover analog MinHeightFromOriginCm = -
# plus Harbor 80 float token.
# Do NOT claim sibling CommandLateralBiasCm = 850.f
# (leave uncontracted).
# Do NOT claim leftover analog LockWindowClimbCm = 650.f /
# LockWindowClimbSeconds = 2.4f.
# Do NOT claim leftover Safety / leftover Route #1660-#1668.
...
# Clone wrap was leftover Amplitude Category=
# "Skyguard|Boss|Encounter|Route" wrapping
# `float IngressSwayAmplitudeCm = 320.f;`
# — RETARGET hard: Category from Route to Commands,
# LOCKED_DECL from IngressSwayAmplitudeCm = 320.f to
# BreakClimbCm = 260.f. wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Commands", meta=(ClampMin="0.0")
# (NO spaces around =). Fail-closed if wrap still has
# leftover Route / Safety / Telegraph Category, or ClampMin="100.0".
# LOCKED_DECL is `float BreakClimbCm = 260.f;`
...
# and NOT leftover CommandLateralBiasCm = 850.f
...
# or Category="Skyguard" without |Boss|Encounter|Route.
...
# or default is not 260.f, or default is leftover analog 320.f /
# 650.f / 850.f.
# Fail-closed if locked wrap Category is still leftover Route
# Skyguard|Boss|Encounter|Route / leftover Safety /
# leftover Telegraph.
...
TARGET = "float BreakClimbCm = 260.f;"
TARGET_WRONG_BARE = "float BreakClimbCm;"
TARGET_WRONG_INIT = "float BreakClimbCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float BreakClimbCm = true;"
TARGET_WRONG_ZERO = "float BreakClimbCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> BreakClimbCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> BreakClimbCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> BreakClimbCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "ApproachAttackIntervalSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_break_climb"
...
TARGET_WRONG_LEFTOVER_AMPLITUDE = "float IngressSwayAmplitudeCm = 320.f;"
TARGET_WRONG_CLIMB_CM = "float LockWindowClimbCm = 650.f;"
TARGET_WRONG_COMMAND_LATERAL = "float CommandLateralBiasCm = 850.f;"
...
BREAK_CLIMB_DECL_RE = re.compile(
    r"float\s+BreakClimbCm\s*=\s*260\.f\s*;"
)
BREAK_CLIMB_BARE_RE = re.compile(
    r"float\s+BreakClimbCm\s*;"
)
...
COMMAND_LATERAL_LEFTOVER_DECL_RE = re.compile(
    r"float\s+CommandLateralBiasCm\s*=\s*850\.f\b"
)
MIN_HEIGHT_LEFTOVER_DECL_RE = re.compile(
    r"float\s+MinHeightFromOriginCm\s*=\s*-80\.f\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float BreakClimbCm = 260.f;`.
    ...
    # BreakClimbCm without the 260.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if BREAK_CLIMB_DECL_RE.search(compact) is None:
        return False
    if BREAK_CLIMB_BARE_RE.search(compact):
        return False
    ...
    if AMPLITUDE_LEFTOVER_DECL_RE.search(compact):
        return False
    if COMMAND_LATERAL_LEFTOVER_DECL_RE.search(compact):
        return False
    if MIN_HEIGHT_LEFTOVER_DECL_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+BreakClimbCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float BreakClimbCm "
...
class PathfinderEncounterBreakClimbFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BreakClimbCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BreakClimbCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("BreakClimbCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_break_climb_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_BARE_COMMANDS)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float BreakClimbCm;",
            compact_origin.split("BreakClimbCm = 260.f;")[0]
            + compact_origin.split("BreakClimbCm = 260.f;")[-1]
            if "BreakClimbCm = 260.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_break_climb_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LEFTOVER_AMPLITUDE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_COMMAND_LATERAL)
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        leftover_route = (
            ...
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_ROUTE_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        route_specs = attached_uproperty_specifiers(
            spec_section(leftover_route)
        )
        self.assertIn('Category="Skyguard|Boss|Encounter|Route"', route_specs)
        self.assertNotIn(
            'Category="Skyguard|Boss|Encounter|Commands"',
            route_specs,
        )
        ...
        leftover_telegraph = (
            ...
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_TELEGRAPH_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        telegraph_specs = attached_uproperty_specifiers(
            spec_section(leftover_telegraph)
        )
        self.assertIn(
            'Category="Skyguard|Boss|Encounter|Telegraph"',
            telegraph_specs,
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BreakClimbCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "BreakClimbCm"))
        self.assertTrue(has_identifier(section, "BreakClimbCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_break_climb_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
