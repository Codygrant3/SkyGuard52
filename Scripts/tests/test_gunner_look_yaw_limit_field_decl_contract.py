# THIS IS leftover-safe ASkyguardGunner LookYawLimit.
...
# Stop BEFORE sibling LookPitchMin as a claimed slot.
...
# LOCKED_DECL is `float LookYawLimit = 95.f;` NOT
...
TARGET = "float LookYawLimit = 95.f;"
TARGET_WRONG_BARE = "float LookYawLimit;"
TARGET_WRONG_INIT = "float LookYawLimit = nullptr;"
TARGET_WRONG_FALSE = "float LookYawLimit = false;"
TARGET_WRONG_TRUE = "float LookYawLimit = true;"
TARGET_WRONG_ZERO = "float LookYawLimit = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LookYawLimit;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LookYawLimit;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LookYawLimit;"
...
LOOK_YAW_LIMIT_BARE_RE = re.compile(
    r"float\s+LookYawLimit\s*;"
)
LOOK_YAW_LIMIT_WRONG_INIT_RE = re.compile(
    r"float\s+LookYawLimit\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float LookYawLimit = 95.f;`.
    ...
    # Harbor 40 / 80, or a bare LookYawLimit without
    # the 95.f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LOOK_YAW_LIMIT_DECL_RE.search(compact) is None:
        return False
    if LOOK_YAW_LIMIT_BARE_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LookYawLimit\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LookYawLimit "
...
class GunnerLookYawLimitFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LookYawLimit"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LookYawLimit"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LookYawLimit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_look_yaw_limit_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float LookYawLimit = 95.f;", compact_origin)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float LookYawLimit = 95.f;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat LookYawLimits = 95.f;\n",
            "\tint32 LookYawLimit = 95;\n",
            "\tbool LookYawLimit = true;\n",
            "\tfloat LookYawLimit = " + forty + ";\n",
            "\tfloat LookYawLimit = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_look_pitch_min_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LookYawLimit"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LookYawLimit"))
        self.assertTrue(has_identifier(section, "LookYawLimit"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner LookYawLimit field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_look_yaw_limit_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
