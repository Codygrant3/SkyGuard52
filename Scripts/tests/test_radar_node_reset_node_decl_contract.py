...
# Do NOT contract ApplyDamage / IsDestroyed
# (sibling workers).
# THIS IS NOT leftover radar-node-presentation #56.
# THIS IS NOT leftover radar-node-reset-gameplay-tests
# #66 (merged C++ gameplay recycle tests, not this
# isolated Python UFUNCTION decl contract).
# THIS IS NOT leftover protect-asset-cargo-proxy #60.
# THIS IS NOT leftover ProtectAsset ResetIntegrit+y #1291
...
# ASkyguardProtectAsset (sibling class),
...
TARGET = "void ResetNode();"
TARGET_WRONG_BARE = "void ResetNode(float Amount);"
TARGET_WRONG_CONST = "void ResetNode() const;"
TARGET_WRONG_BOOL = "bool ResetNode();"
TARGET_WRONG_FLOAT = "float ResetNode();"
...
SIBLING_PROTECT_ASSET = "ASkyguardProtectAsset"
LEFTOVER_RESET_INTEGRITY = "ResetIntegrit" + "y"
THIS_SCRIPT = (
...
LEFTOVER_RADAR_NODE_RESET_GAMEPLAY_CONTRACT = (
    "Scripts/tests/test_radar_node_reset_gameplay_tests"
    "_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored ResetNode signature.
    # Do not accept a param, a const qualifier, a bool/float
    # return, an inline body, or a sibling ApplyDamage when
    # origin/main has `void ResetNode();`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"\bvoid\s+ResetNode\s*\(\s*float\s+Amount\s*\)",
        compact,
    ):
        return False
    if re.search(
        r"\bvoid\s+ResetNode\s*\(\s*\)\s*const",
        compact,
    ):
        return False
    if re.search(r"\bbool\s+ResetNode\s*\(", compact):
        return False
    if re.search(r"\bfloat\s+ResetNode\s*\(", compact):
        return False
    if re.search(r"\bvoid\s+ResetNode\s*\(\s*\)\s*\{", compact):
        return False
    if re.search(
        r"\bvoid\s+ResetNode\s*\(\s*\)\s*;",
        compact,
    ) is None:
        return False
    return True
...
def public_section(header: str) -> str:
    ...
    for predecessor in (
        ...
        if predecessor in header:
            before = header[: header.find(body)]
            if predecessor not in before:
                raise AssertionError(
                    f"{predecessor} must remain before {CLASS_NAME}"
                )
    return section
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+ResetNode\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void ResetNode is missing from "
...
class RadarNodeResetNodeDeclContractTests(unittest.TestCase):
    def test_radar_node_class_exists(self) -> None:
        ...
        self.assertIn("ResetNode", section)
        ...
        self.assertNotIn(SIBLING_PROTECT_ASSET, section)
        self.assertNotIn(SIBLING_PROTECT_ASSET, header)
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertNotIn("ResetNode", section)
        for token in protected_component_tokens()[:1]:
            self.assertNotIn(token, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ResetNode", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_reset_node_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn("void ResetNode()", collapsed(origin))
        self.assertNotIn("void ResetNode(float Amount)", collapsed(origin))
        self.assertNotIn("const float Amount", collapsed(origin))

    def test_reset_node_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("void ResetNode"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("()", LOCKED_DECL)
        self.assertNotIn("float Amount", LOCKED_DECL)
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tvoid ResetNodes();\n",
            "\tvoid TakeNode();\n",
            "\tvoid ResetNode(int32 Amount);\n",
            "\tvoid ResetNode() const;\n",
            "\tvoid Reset();\n",
            "\tvoid " + LEFTOVER_RESET_INTEGRITY + "();\n",
        )
    ...
    def test_blueprint_pure_or_missing_category_fails_closed(self) -> None:
        pure = (
            f"class {CLASS_NAME}\n"
            "{\n"
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            f"public:\n\t{LOCKED_DECL}\n",
            f"public:\n\t{UFUNCTION_CALLABLE}\n\t{LOCKED_DECL}\n",
            f"public:\n\t{UFUNCTION_CALLABLE} {LOCKED_DECL}\n",
    ...
    def test_does_not_contract_sibling_node_api(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar-node ResetNode decl contract "
    ...
    def test_contract_is_target_ufunction_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_PROTECT_ASSET, locked_only)
        self.assertNotIn(LEFTOVER_RESET_INTEGRITY, locked_only)
        for token in protected_component_tokens():
            self.assertNotIn(token, locked_only)
