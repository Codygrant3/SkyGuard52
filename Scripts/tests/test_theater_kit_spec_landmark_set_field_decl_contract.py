# THIS IS leftover-safe FSkyguardTheaterKitSpec::LandmarkSet.
# origin/main form: one-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Skyguard|Theater")
# then BARE `FName LandmarkSet;` (no initializer).
...
# specifiers drop to BlueprintReadWrite /
# EditAnywhere-only. There is NO default.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT body ONLY after
...
# Do NOT contract sibling WeatherIdentity / KitId /
# BuildingKit / BuildingTint / LampTreatment /
# LampColor / LampIntensity / RoadTreatment /
# RoadTint / NamedLandmark / LandmarkTint /
# SilhouetteKit / SilhouetteTint /
# LandmarkMeshIndex / LandmarkScale.
# THIS IS NOT leftover theater-kit-weather-apply #59
# (ApplyTheaterKitToWorld gameplay).
# THIS IS NOT leftover merged bulk
# test_campaign_theater_kit_contract.py (keep that
# file in LOCKED_SCRIPTS; isolated field decl does
# not relock the bulk).
# THIS IS NOT leftover MeshBindSlot #194 / #6829.
# THIS IS NOT leftover ProtectAsset / RadarNode
# #1288-#1298.
# Analog: leftover briefing-card-defaults then
# isolated CardId field decls.
# If a Health clone asserts `float` / `= 160.f` /
# Category="Skyguard|Campaign", retarget: type is
# FName, no `=`, Category is Skyguard|Theater.
# Do NOT parse enum class ESkyguardAudioEvent
# (contains leftover Ri+fle / Ig+la).
...
# Do NOT parse leftover retired aircraft class.
...
TARGET = "FName LandmarkSet;"
TARGET_WRONG_DEFAULT = "FName LandmarkSet = NAME_None;"
TARGET_WRONG_FLOAT = "float LandmarkSet;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_FALSE = "FName LandmarkSet = false;"
TARGET_WRONG_TRUE = "FName LandmarkSet = true;"
TARGET_WRONG_ZERO = "FName LandmarkSet = 0.f;"
TARGET_WRONG_INT = "FName LandmarkSet = 160;"
...
STOP_BEFORE_PATROL_SHIP = "ASkyguardPatrolShipBoss"
...
APPLY_THEATER_KIT_TO_WORLD = "ApplyTheaterKitToWorld"
APPLY_THEATER_KIT = "ApplyTheaterKit"
...
LEFTOVER_MESH_BIND_SLOT_FIELDS_CONTRACT = (
    "Scripts/tests/test_mesh_bind_slot_fields_contract.py"
)
...
def leftover_retired_aircraft_class() -> str:
...
def leftover_namespace_helpers() -> tuple[str, ...]:
    return (
        NAMESPACE_NUM_KITS,
...
def leftover_actor_api() -> tuple[str, ...]:
    return (
        APPLY_THEATER_KIT_TO_WORLD,
        APPLY_THEATER_KIT,
        STOP_BEFORE_ACTOR,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare FName LandmarkSet.
    # Do not accept an initializer, leftover Health
    # `float` / `= 160.f`, or sibling FName fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"FName\s+LandmarkSet\s*=", compact):
        return False
    if re.search(r"float\s+LandmarkSet\b", compact):
        return False
    if re.search(r"float\s+Health\b", compact) and "LandmarkSet" not in compact:
        return False
    if re.search(r"FName\s+LandmarkSet\s*;", compact) is None:
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        STOP_BEFORE_PATROL_SHIP,
        STOP_BEFORE_GUNNER,
        STOP_BEFORE_WEAK_POINT,
        leftover_retired_aircraft_class(),
        ...
        f"class SKYGUARD52_API {STOP_BEFORE_ACTOR}",
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+LandmarkSet\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName LandmarkSet is missing from "
...
class TheaterKitSpecLandmarkSetFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_ACTOR)
        ...
        self.assertTrue(has_identifier(section, "LandmarkSet"), section)
        ...
        self.assertNotIn(APPLY_THEATER_KIT_TO_WORLD, section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LandmarkSet"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(STOP_BEFORE_ACTOR, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LandmarkSet", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_landmark_set_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tFName WeatherIdentity;\n"
    ...
    def test_initializer_fails_closed_bare_decl_is_required(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
            f"\t{TARGET_WRONG_DEFAULT}\n"
            "};\n"
        )
        ...
        self.assertIn("FName LandmarkSet;", compact_origin)
        self.assertNotIn("LandmarkSet =", compact_origin)
        self.assertNotIn("LandmarkSet = false", compact_origin)
        self.assertNotIn("LandmarkSet = true", compact_origin)
        self.assertNotIn("LandmarkSet = 0.f", compact_origin)

    def test_landmark_set_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName LandmarkSet"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_DEFAULT}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_DEFAULT}\n", LOCKED_DECL)
        self.assertIn("LandmarkSet", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_category_or_visible_anywhere_fails_closed(self) -> None:
        ...
        rewritten = attached_uproperty_specifiers(
            spec_section(write_only)
        )
        self.assertIn("EditAnywhere", rewritten)
        self.assertIn("BlueprintReadWrite", rewritten)
        ...
        wrong_cat = attached_uproperty_specifiers(
            spec_section(campaign)
        )
        self.assertIn('Category="Skyguard|Campaign"', wrong_cat)
        self.assertNotIn('Category="Skyguard|Theater"', wrong_cat)
        self.assertIn('Category="Skyguard|Theater"', origin)
    ...
    def test_does_not_contract_sibling_theater_fields(self) -> None:
        ...
        self.assertNotIn(APPLY_THEATER_KIT_TO_WORLD, LOCKED_DECL)
        ...
        for helper in leftover_namespace_helpers():
            self.assertNotIn(helper, LOCKED_DECL)
        for actor_api in leftover_actor_api():
            self.assertNotIn(actor_api, LOCKED_DECL)
            self.assertNotIn(actor_api, section)
    ...
    def test_does_not_parse_actor_namespace_or_neighbors(self) -> None:
        ...
        self.assertIn(APPLY_THEATER_KIT_TO_WORLD, header)
        self.assertNotIn(APPLY_THEATER_KIT_TO_WORLD, section)
        self.assertNotIn(APPLY_THEATER_KIT_TO_WORLD, leaked)
        ...
        self.assertNotIn(STOP_BEFORE_SORTIE_DIRECTOR, header)
        self.assertNotIn(STOP_BEFORE_PATROL_SHIP, header)
        self.assertNotIn(STOP_BEFORE_GUNNER, header)
        self.assertNotIn(STOP_BEFORE_WEAK_POINT, header)
        self.assertNotIn(leftover_retired_aircraft_class(), header)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\tFName {SIBLING_BUILDING_KIT};\n"
            f"\tFName {SIBLING_NAMED_LANDMARK};\n"
            f"\tvoid {APPLY_THEATER_KIT_TO_WORLD}();\n"
            f"\tvoid {APPLY_THEATER_KIT}(FName WeatherIdentity);\n"
            "\tint32 NumKits();\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit LandmarkSet field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "landmark_set_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_THEATER_KIT_WEATHER_APPLY_CONTRACT, LOCKED_SCRIPTS)
        ...
        self.assertIn(CLONE_THEATER_KIT_ID, LOCKED_SCRIPTS)
        self.assertIn(CLONE_THEATER_NAMED_LANDMARK, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(APPLY_THEATER_KIT_TO_WORLD, locked_only)
        ...
        self.assertNotIn("float Health", locked_only)
