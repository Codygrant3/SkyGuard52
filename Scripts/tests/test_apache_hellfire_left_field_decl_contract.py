# THIS IS leftover-safe ASkyguardApacheAircraft HellfireLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> HellfireLeft;`
# (no initializer).
...
# Stop BEFORE sibling HellfireRight as a claimed slot.
# Do NOT claim HellfireRight, HydraLeft, HydraRight,
# PylonLeft, PylonRight, GearLeft, GearRight,
# GunnerMount, PilotMount, EyeMount, WeaponMount,
# HullCollider, RotorPower.
# THIS IS leftover-safe ASkyguardApacheAircraft.HellfireLeft.
# It is NOT leftover HydraLeft (opened draft), NOT leftover
# HydraRight (in-flight sibling), NOT leftover PylonLeft,
# NOT leftover HellfireRight, NOT leftover LoadoutSpec
# GuidedReserve / GuidedMagazineSize, NOT leftover analog
# apache-cpg-feel, NOT leftover gunship-weapon-stations,
# NOT leftover StormRainBeatKit bHydraForClusters.
# It is NOT leftover analog apache-hull-collider, NOT leftover
# analog apache-mount-fail-closed, NOT leftover analog
...
# leftover analog apache-hydra-right-field,
# leftover analog apache-pylon-left-field.
# HellfireRight / HydraLeft / HydraRight / PylonLeft are
# sibling slots being opened, not this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> HellfireLeft;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> HellfireLeft;` and NOT
# leftover `HydraLeft` / leftover `HydraRight` / leftover
# `PylonLeft` / leftover `HellfireRight` / leftover
# LoadoutSpec `GuidedReserve` / leftover StormRain
# `bHydraForClusters` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> HellfireLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> HellfireLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> HellfireLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> HellfireLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> HellfireLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> HellfireLeft;"
...
TARGET_WRONG_HYDRA_FOR_CLUSTERS = (
...
STOP_BEFORE_HELLFIRE_RIGHT = "HellfireRight"
...
HELLFIRE_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HellfireLeft\s*;"
)
HELLFIRE_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+HellfireLeft\s*="
)
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_HELLFIRE_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_HELLFIRE_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_HELLFIRE_RIGHT}"
        )
    for later in (
        SIBLING_HELLFIRE_RIGHT,
        SIBLING_GUNNER_MOUNT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+HellfireLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> HellfireLeft "
...
class ApacheHellfireLeftFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HellfireLeft"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HellfireLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("HellfireLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_hellfire_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> HellfireLeft;", compact_origin)
        self.assertNotIn("HellfireLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_hellfire_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> HellfireLeft"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_HYDRA_RIGHT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_PYLON_LEFT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_HELLFIRE_RIGHT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_GEAR_NOSE))
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> HellfireLeft;")
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> HellfireLeft;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> HellfireLefts;\n",
            "\tint32 HellfireLeft = 140;\n",
            "\tbool HellfireLeft = true;\n",
            "\tfloat HellfireLeft = " + forty + ";\n",
            "\tfloat HellfireLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_hellfire_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache HellfireLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "hellfire_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_HYDRA_RIGHT))
        self.assertFalse(has_identifier(locked_only, SIBLING_PYLON_LEFT))
        self.assertFalse(has_identifier(locked_only, SIBLING_HELLFIRE_RIGHT))
        self.assertFalse(has_identifier(locked_only, SIBLING_GEAR_NOSE))
