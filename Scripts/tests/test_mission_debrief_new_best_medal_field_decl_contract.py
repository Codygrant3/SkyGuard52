# THIS IS leftover-safe FSkyguardMissionDebrief::bNewBestMedal.
# origin/main form: split-line
# `UPROPERTY(VisibleAnywhere, BlueprintReadOnly)`
# then `bool bNewBestMedal = false;` (initializer IS `= false`,
...
# has none, drop that assertion. If a clone asserts `= 0` /
# `= 0.f` / `= true`, do not copy those; this field is `= false`.
...
# MissionDisplayName, Narrative, bNewBestScore, bProgressSaved,
...
# no Category=. Accept `= false` initializer.
...
TARGET = "bool bNewBestMedal = false;"
TARGET_WRONG_BARE = "bool bNewBestMedal;"
TARGET_WRONG_TRUE = "bool bNewBestMedal = true;"
TARGET_WRONG_ZERO = "bool bNewBestMedal = 0;"
TARGET_WRONG_FLOAT = "bool bNewBestMedal = 0.f;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `= false` default. Do not accept
    # a bare decl or `= true` / `= 0` / `= 0.f` when origin/main
    # has bool bNewBestMedal = false.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    for match in re.finditer(r"UPROPERTY\(", compact):
        ...
        if depth != 0:
            continue
        after = compact[index:]
        if re.match(r"\s*bool\s+bNewBestMedal\b", after):
            return compact[start : index - 1]
    raise AssertionError(
        "UPROPERTY for bool bNewBestMedal is missing from "
...
class MissionDebriefNewBestMedalFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bNewBestMedal", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_new_best_medal_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_new_best_medal_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bNewBestMedal"),
            LOCKED_DECL,
        )
    ...
    def test_bare_decl_without_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("= false", origin)
        self.assertIn("bNewBestMedal", origin)
    ...
    def test_does_not_contract_sibling_fields_or_neighbors(self) -> None:
        self.assertIn("bNewBestMedal", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief bNewBestMedal field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_new_best_medal_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertIn("bNewBestMedal = false;", locked_only)
