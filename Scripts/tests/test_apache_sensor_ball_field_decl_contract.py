# THIS IS leftover-safe ASkyguardApacheAircraft SensorBall.
...
# then BARE `TObjectPtr<UStaticMeshComponent> SensorBall;`
# (no initializer).
...
# Stop BEFORE sibling NightVisionTurret as a claimed slot.
# Do NOT claim SensorTurret / NightVisionTurret / ChinTurret /
# ChinHousing / ChinBarrel / TailRotor / MainRotor /
# RotorMast / MainRotorCross / AircraftRoot / Fuselage /
# Nose / Canopy / SilhouetteMesh / RadarDome / PilotCanopy /
...
# Do NOT parse leftover sibling SensorTurret
# (USceneComponent). Type is TObjectPtr<UStaticMeshComponent>.
...
# SensorTurret / ChinHousing / ChinBarrel are sibling
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> SensorBall;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> SensorTurret;` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> SensorBall;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> SensorBall = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> SensorBall = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> SensorBall = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> SensorBall;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> SensorBall;"
...
SENSOR_BALL_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+SensorBall\s*;"
)
SENSOR_BALL_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+SensorBall\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover Fuselage / TailBoom / AircraftRoot, or an
    # initializer on SensorBall as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SENSOR_BALL_DECL_RE.search(compact) is None:
        return False
    if SENSOR_BALL_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_NIGHT_VISION):
            break
    ...
    if has_identifier(window, SIBLING_NIGHT_VISION):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_NIGHT_VISION}"
        )
    for later in (
        SIBLING_PILOT_CANOPY,
        SIBLING_RADAR_DOME,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+SensorBall\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> SensorBall "
...
class ApacheSensorBallFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SensorBall"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_NIGHT_VISION), body)
        self.assertFalse(
            has_identifier(section, SIBLING_NIGHT_VISION),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SensorBall"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SensorBall", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_sensor_ball_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> SensorBall;", compact_origin)
        self.assertNotIn("SensorBall = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_sensor_ball_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> SensorBall"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> SensorBall;")
        self.assertNotEqual(LOCKED_DECL, "FName WeatherIdentity;")
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> SensorBall;",
        )
        self.assertNotEqual(
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> SensorBalls;\n",
            "\tint32 SensorBall = 140;\n",
            "\tbool SensorBall = true;\n",
            "\tfloat SensorBall = " + forty + ";\n",
            "\tfloat SensorBall = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_night_vision_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_NIGHT_VISION))
        ...
        self.assertNotIn(TARGET_WRONG_NIGHT_VISION, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SensorBall field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "sensor_ball_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
