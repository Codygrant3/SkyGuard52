...
# INDEX_NONE or lock StartPhaseWave
# construction in the .cpp. This is leftover-safe
# Mission10 Waves StartPhaseWave
# on ASkyguardMission10IntegrationDirector. It is
# NOT leftover Mission09 StartNextWave
# #753 / branch cursor/mission09-start-
# next-wave-decl-contract-c332, NOT leftover
# Mission08 StartNextWave
# #731 / branch cursor/mission08-start-
# next-wave-decl-contract-c332, NOT leftover
# Mission07 StartNextWave,
# NOT leftover Mission06 StartNextWave,
# NOT leftover Mission05 StartNextWave
# #668, NOT leftover Mission01 / Mission03 /
# Mission04 StartNextWave, NOT leftover
# Mission10 InitializePlayableMission, NOT leftover
...
# Do not lock leftover Mission10 InitializePlayableMission. Do not
# lock leftover BindRuntimeActors. Do not
...
# (`bool StartPhaseWave();`); accept that
...
# leftover Mission09 StartNextWave #753, leftover
# Mission08 StartNextWave #731, leftover closed
# Mission05 StartNextWave #668, leftover Mission07 /
# Mission06 / Mission04 / Mission03 StartNextWave,
# leftover Mission10 InitializePlayableMission,
...
# an isolated Python StartPhaseWave
# method declaration contract on
# ASkyguardMission10IntegrationDirector. It is
# StartPhaseWave, not leftover StartNextWave.
LOCKED = {
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission10IntegrationDirector::StartPhaseWave",
    "SkyguardMission10IntegrationDirector.cpp",
    "SkyguardMission09IntegrationDirector.cpp",
    "SkyguardMission08IntegrationDirector.cpp",
    "SkyguardMission06IntegrationDirector.cpp",
...
class Mission10StartPhaseWaveDeclContractTests(unittest.TestCase):
    def test_mission10_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, START_PHASE_WAVE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, START_PHASE_WAVE)
        self.assertIn("StartPhaseWave", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, START_PHASE_WAVE)
    ...
    def test_missing_start_phase_wave_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission10IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, START_PHASE_WAVE)
        self.assertIn("StartPhaseWave", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, START_PHASE_WAVE)
        self.assertIn("StartPhaseWave", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", START_PHASE_WAVE)
        self.assertNotIn("BlueprintCallable", START_PHASE_WAVE)
        self.assertNotIn("Category", START_PHASE_WAVE)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, START_PHASE_WAVE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, START_PHASE_WAVE)
        self.assertIn("StartPhaseWave", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid StartPhaseWave();\n"
        as_const = "\tbool StartPhaseWave() const;\n"
        with_arg = "\tbool StartPhaseWave(bool bForce);\n"
        renamed = "\tbool StartPhase();\n"
        short_name = "\tbool StartWave();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, START_PHASE_WAVE)
            self.assertIn("StartPhaseWave", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_start_phase_wave_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, START_PHASE_WAVE),
            START_PHASE_WAVE,
        )
        self.assertTrue(has_declaration(section, START_PHASE_WAVE))
        self.assertEqual(
            declaration_count(section, START_PHASE_WAVE),
            1,
        )
        self.assertTrue(
            START_PHASE_WAVE.startswith("bool "),
            START_PHASE_WAVE,
        )
        self.assertTrue(
            START_PHASE_WAVE.endswith(";"),
            START_PHASE_WAVE,
        )
        self.assertIn("StartPhaseWave", START_PHASE_WAVE)
        self.assertIn("()", START_PHASE_WAVE)
        self.assertNotIn("TArray<", START_PHASE_WAVE)
        self.assertNotIn("TObjectPtr", START_PHASE_WAVE)
        self.assertNotIn("TSoftObjectPtr", START_PHASE_WAVE)
        self.assertNotIn("INDEX_NONE", START_PHASE_WAVE)
        self.assertNotIn("UFUNCTION", START_PHASE_WAVE)
        self.assertNotIn("{", START_PHASE_WAVE)
        self.assertNotIn("}", START_PHASE_WAVE)
        self.assertNotIn("return ", START_PHASE_WAVE)
        self.assertNotIn("const;", START_PHASE_WAVE)
        self.assertNotIn("Root", START_PHASE_WAVE)
        self.assertNotIn("Briefing", START_PHASE_WAVE)
        self.assertNotIn("AudioDirector", START_PHASE_WAVE)
        self.assertNotIn("RadioChatter", START_PHASE_WAVE)
        self.assertNotIn("SortiePresentation", START_PHASE_WAVE)
        self.assertNotIn("CampaignDefinition", START_PHASE_WAVE)
        self.assertNotIn("MissionDefinition", START_PHASE_WAVE)
        self.assertNotIn("Readiness", START_PHASE_WAVE)
        self.assertNotIn("bAutoInitialize", START_PHASE_WAVE)
        self.assertNotIn("bAllowBoundedActorSpawning", START_PHASE_WAVE)
        self.assertNotIn("bAutoLaunchAfterBriefing", START_PHASE_WAVE)
        self.assertNotIn("PathfinderSpawnLocation", START_PHASE_WAVE)
        self.assertNotIn("GetAircraft", START_PHASE_WAVE)
        self.assertNotIn("BindRuntimeActors", START_PHASE_WAVE)
        self.assertNotIn("HandleDroneCityImpact", START_PHASE_WAVE)
        self.assertNotIn("StartNextWave", START_PHASE_WAVE)
        self.assertNotIn("NotifyThreatDestroyed", START_PHASE_WAVE)
        self.assertNotIn("AdvanceConvoyByDistance", START_PHASE_WAVE)
        self.assertNotIn("NotifyConvoyDamage", START_PHASE_WAVE)
        self.assertNotIn("GetWaveState", START_PHASE_WAVE)
        self.assertNotIn("GetConvoyRouteState", START_PHASE_WAVE)
        self.assertNotIn("GetDayBeatKit", START_PHASE_WAVE)
        self.assertNotIn("GetNightBeatKit", START_PHASE_WAVE)
        self.assertNotIn("StartSearchlightWindow", START_PHASE_WAVE)
        self.assertNotIn("AdvanceSearchlightTrack", START_PHASE_WAVE)
        self.assertNotIn("NotifySubstationDamage", START_PHASE_WAVE)
        self.assertNotIn("GetRemainingThreatsInWave", START_PHASE_WAVE)
        self.assertNotIn("GetSearchlightRuntime", START_PHASE_WAVE)
        self.assertNotIn("GetSubstationIntegrity", START_PHASE_WAVE)
        self.assertNotIn("StartPayloadWindow", START_PHASE_WAVE)
        self.assertNotIn("AdvancePayloadWindow", START_PHASE_WAVE)
        self.assertNotIn("TryJamActivePayload", START_PHASE_WAVE)
        self.assertNotIn("NotifyAirfieldTargetDamage", START_PHASE_WAVE)
        self.assertNotIn("GetPayloadWindow", START_PHASE_WAVE)
        self.assertNotIn("GetTargetRuntime", START_PHASE_WAVE)
        self.assertNotIn("GetSurvivingTargetCount", START_PHASE_WAVE)
        self.assertNotIn("GetDayBeatKind", START_PHASE_WAVE)
        self.assertNotIn("GetDayBeatIndex", START_PHASE_WAVE)
        self.assertNotIn("TickDayBeatKit", START_PHASE_WAVE)
        self.assertNotIn("HandleBossPhaseChanged", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission06WaveState", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission07WaveState", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission07IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission08IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission08WaveState", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission09WaveState", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission09ProtectedTarget", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardMission09ProtectedTargetRuntime", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardMission09PoolRuntime", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardMission09PoolBudget", START_PHASE_WAVE)
        self.assertNotIn("BindCampaignRuntime", START_PHASE_WAVE)
        self.assertNotIn("GetPoolRuntime", START_PHASE_WAVE)
        self.assertNotIn("IronRainSpawnLocation", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission08ProtectedTarget", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardMission08ProtectedTargetRuntime", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardHoistWindowRuntime", START_PHASE_WAVE)
        self.assertNotIn("StartHoistWindow", START_PHASE_WAVE)
        self.assertNotIn("AdvanceHoistWindow", START_PHASE_WAVE)
        self.assertNotIn("ValidateWeaponRelease", START_PHASE_WAVE)
        self.assertNotIn("GetHoistRuntime", START_PHASE_WAVE)
        self.assertNotIn("GetRejectedWeaponReleases", START_PHASE_WAVE)
        self.assertNotIn("GetStormRainBeatKit", START_PHASE_WAVE)
        self.assertNotIn("ApplyStormRainPlayContract", START_PHASE_WAVE)
        self.assertNotIn("GetStormRainBeatKind", START_PHASE_WAVE)
        self.assertNotIn("TickStormRainBeatKit", START_PHASE_WAVE)
        self.assertNotIn("RescueHelicopterAnchor", START_PHASE_WAVE)
        self.assertNotIn("LifelineHunterSpawnLocation", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardSearchSector", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardSearchTrackRuntime", START_PHASE_WAVE)
        self.assertNotIn("ClassifyFalseTrack", START_PHASE_WAVE)
        self.assertNotIn("ConfirmRadarGhostIdentification", START_PHASE_WAVE)
        self.assertNotIn("NotifyProtectedTargetDamage", START_PHASE_WAVE)
        self.assertNotIn("AdvanceReinforcementTimer", START_PHASE_WAVE)
        self.assertNotIn("GetSearchSector", START_PHASE_WAVE)
        self.assertNotIn("GetClassifiedFalseTrackCount", START_PHASE_WAVE)
        self.assertNotIn("IsHostileContactConfirmed", START_PHASE_WAVE)
        self.assertNotIn("GetProtectedTarget", START_PHASE_WAVE)
        self.assertNotIn("GetReinforcementTimeRemaining", START_PHASE_WAVE)
        self.assertNotIn("GetNightBeatKind", START_PHASE_WAVE)
        self.assertNotIn("GetNightBeatIndex", START_PHASE_WAVE)
        self.assertNotIn("TickNightBeatKit", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardAirfieldTarget", START_PHASE_WAVE)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ConvoyRuntimeAnchor", START_PHASE_WAVE)
        self.assertNotIn("RoadHunterSpawnLocation", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", START_PHASE_WAVE)
        self.assertNotIn("ConfigureMissionDefinition", START_PHASE_WAVE)
        self.assertNotIn("NotifyObjectiveProgress", START_PHASE_WAVE)
        self.assertNotIn("NotifyProtectedAssetFailed", START_PHASE_WAVE)
        self.assertNotIn("SynchronizeRuntimeState", START_PHASE_WAVE)
        self.assertNotIn("IsCorePlayableReady", START_PHASE_WAVE)
        self.assertNotIn("GetReadiness", START_PHASE_WAVE)
        self.assertNotIn("GetObjectiveRuntime", START_PHASE_WAVE)
        self.assertNotIn("GetGunner", START_PHASE_WAVE)
        self.assertNotIn("GetPathfinder", START_PHASE_WAVE)
        self.assertNotIn("GetMissionId", START_PHASE_WAVE)
        self.assertNotIn("ValidateMissionContract", START_PHASE_WAVE)
        self.assertNotIn("ConfigureFromMission", START_PHASE_WAVE)
        self.assertNotIn("AdvanceBriefing", START_PHASE_WAVE)
        self.assertNotIn("SetAssetsReady", START_PHASE_WAVE)
        self.assertNotIn("AcknowledgeAndLaunch", START_PHASE_WAVE)
        self.assertNotIn("CanLaunch", START_PHASE_WAVE)
        self.assertNotIn("GetElapsedSeconds", START_PHASE_WAVE)
        self.assertNotIn("GetBriefingState", START_PHASE_WAVE)
        self.assertNotIn("GetMinimumWarmupSeconds", START_PHASE_WAVE)
        self.assertNotIn("GetBriefingText", START_PHASE_WAVE)
        self.assertNotIn("GetRadioChatter", START_PHASE_WAVE)
        self.assertNotIn("GetPresentation", START_PHASE_WAVE)
        self.assertNotIn("GetMissionTitle", START_PHASE_WAVE)
        self.assertNotIn("AcknowledgeBriefing", START_PHASE_WAVE)
        self.assertNotIn("LaunchSortie", START_PHASE_WAVE)
        self.assertNotIn("HullCollider", START_PHASE_WAVE)
        self.assertNotIn("OpticalTracker", START_PHASE_WAVE)
        self.assertNotIn("WeaponServo", START_PHASE_WAVE)
        self.assertNotIn("CountermeasurePod", START_PHASE_WAVE)
        self.assertNotIn("MinHeightFromOriginCm", START_PHASE_WAVE)
        self.assertNotIn("RadarNode", START_PHASE_WAVE)
        self.assertNotIn("ESkyguardMission02WaveState", START_PHASE_WAVE)
        self.assertNotIn("HarborIndustrial", START_PHASE_WAVE)
        self.assertNotIn("MaxIntegrity", START_PHASE_WAVE)
        self.assertNotIn("CurrentIntegrity", START_PHASE_WAVE)
        self.assertNotIn("FillAndFinalize", START_PHASE_WAVE)
        self.assertNotIn("FillAndFail", START_PHASE_WAVE)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, START_PHASE_WAVE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_WAVES} {START_PHASE_WAVE}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{START_PHASE_WAVE}\n}}\n"
        self.assertTrue(has_declaration(one_line, START_PHASE_WAVE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, START_PHASE_WAVE),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{START_PHASE_WAVE}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, START_PHASE_WAVE)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", START_PHASE_WAVE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_start_phase_wave_cpp_body(self) -> None:
        locked_only = f"{START_PHASE_WAVE}\n"
        self.assertNotIn("{", START_PHASE_WAVE)
        self.assertNotIn("}", START_PHASE_WAVE)
        self.assertNotIn("return ", START_PHASE_WAVE)
        ...
        self.assertNotIn("return false", START_PHASE_WAVE)
        self.assertNotIn("return true", START_PHASE_WAVE)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{START_PHASE_WAVE}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, START_PHASE_WAVE)
        self.assertIn("StartPhaseWave", START_PHASE_WAVE)
        self.assertTrue(START_PHASE_WAVE.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, START_PHASE_WAVE)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, START_PHASE_WAVE)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", START_PHASE_WAVE)
        self.assertNotIn("CurrentIntegrity", START_PHASE_WAVE)
        self.assertNotIn("SkyguardApacheAircraft.h", START_PHASE_WAVE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, START_PHASE_WAVE)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", START_PHASE_WAVE)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn("550.f", START_PHASE_WAVE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{START_PHASE_WAVE}\n"
        self.assertEqual(
            require_declaration(locked_only, START_PHASE_WAVE),
            START_PHASE_WAVE,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, START_PHASE_WAVE)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardIronRainBoss", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardRadarGhostBoss", START_PHASE_WAVE)
        self.assertNotIn("ASkyguardTempestBoss", section)
        self.assertNotIn("ASkyguardLastFlightBoss", START_PHASE_WAVE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn(
            "ASkyguardMission08IntegrationDirector::StartNextWave",
            section,
        )
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("{", START_PHASE_WAVE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission10 StartPhaseWave method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{START_PHASE_WAVE}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, START_PHASE_WAVE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                START_PHASE_WAVE.lower(),
                "mission10 StartPhaseWave contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, START_PHASE_WAVE)

    def test_contract_is_start_phase_wave_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(START_PHASE_WAVE.startswith("bool "))
        self.assertIn("StartPhaseWave", START_PHASE_WAVE)
        self.assertTrue(START_PHASE_WAVE.endswith(";"))
        self.assertIn(UFUNCTION_WAVES, section)

    def test_sibling_director_fields_do_not_satisfy_start_phase_wave(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, START_PHASE_WAVE)
            )
            self.assertNotEqual(START_PHASE_WAVE, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("StartPhaseWave", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, START_PHASE_WAVE))
        locked_only = f"{START_PHASE_WAVE}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool StartPhaseWave() { return false; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_start_phase_wave"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_mission10_route_phase_enum(
        ...
        self.assertNotIn("GetRoutePhase", START_PHASE_WAVE)
    ...
    def test_contract_does_not_relock_leftover_mission10_protected_group(
        ...
        self.assertNotIn("Protection", UFUNCTION_WAVES)
        self.assertNotIn("GetProtectedGroup", START_PHASE_WAVE)
    ...
    def test_leftover_mission10_sibling_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("HandleDroneCityImpact", START_PHASE_WAVE)
        self.assertNotIn("UFUNCTION", GET_DAY_BEAT_KIT)
        self.assertNotIn("InitializePlayableMission", START_PHASE_WAVE)
        self.assertNotIn("StartNextWave", START_PHASE_WAVE)
        self.assertNotIn("GetRoutePhase", START_PHASE_WAVE)
