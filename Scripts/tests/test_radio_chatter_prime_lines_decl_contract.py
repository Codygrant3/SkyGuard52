...
# USkyguardRadioChatterComponent::PrimeLines.
# origin/main form: one-line BlueprintCallable wrap
# UFUNCTION(BlueprintCallable, Category="Skyguard|Radio")
# then BARE `void PrimeLines(
# const TArray<FSkyguardRadioLine>& Lines);`
# (declaration-only, no inline body required).
# KEEP Lines arg from origin/main.
# KEEP BlueprintCallable. Do NOT retarget to
# EnqueueLine / AdvanceRadioState /
# ClearQueue / GetQueuedLineCount /
# GetCurrentLineId / GetDroppedLineCount /
# GetPlayedLineCount.
# Word-boundary match PrimeLines vs EnqueueLine vs
# AdvanceRadioState.
# Identifier PrimeLines is NOT leftover analog
# EnqueueLine and NOT leftover analog RadioLine fields.
...
# Lines arg is missing, if the return type is not void,
# or if Category is
...
# Category="Skyguard|Presentation" without leftover-safe
# unique |HUD suffix for a HUD slot.
...
# EnqueueLine / AdvanceRadioState / ClearQueue /
...
# Clone leftover unique BindPresentation #1707 KEEP-args
# BlueprintCallable — RETARGET Category HUD -> Radio
# and method/args. KEEP BlueprintCallable KEEP args.
...
# LOCKED_DECL is void PrimeLines(
# const TArray<FSkyguardRadioLine>& Lines);
...
# and NOT void ClearQueue(); and NOT leftover analog
...
TARGET_WRONG_NO_CONST = (
    "void PrimeLines("
    "const TArray<FSkyguardRadioLine>& Lines) const;"
)
TARGET_WRONG_REFRESH = "void EnqueueLine();"
TARGET_WRONG_REBIND = "void AdvanceRadioState();"
TARGET_WRONG_BIND = (
    "void BindPresentation("
    "USkyguardSortiePresentationComponent* InPresentation);"
)
TARGET_WRONG_TOGGLE = "void TogglePause();"
TARGET_WRONG_ENQUEUE_LINE = (
    "bool EnqueueLine(const FSkyguardRadioLine& Line);"
)
TARGET_WRONG_ADVANCE_RADIO = "void AdvanceRadioState(float DeltaSeconds);"
TARGET_WRONG_CLEAR_QUEUE = "void ClearQueue();"
TARGET_WRONG_POOL_CAPACITY = "int32 PoolCapacity = 12;"
TARGET_WRONG_GET_BOUND = (
    "const TArray<FSkyguardRadioLine>& GetCurrentLineId() const;"
)
TARGET_WRONG_GET_BOUND_INLINE = (
    "const TArray<FSkyguardRadioLine>& GetCurrentLineId() const "
    "{ return Presentation; }"
)
TARGET_WRONG_ACTIVATION = "int32 GetQueuedLineCount() const;"
...
TARGET_WRONG_RENAME = "void PrimeLiness(const TArray<FSkyguardRadioLine>& Lines);"
...
TARGET_WRONG_TRUE = "bool PrimeLines = true;"
TARGET_WRONG_ZERO = "bool PrimeLines = 0.f;"
...
STOP_BEFORE_EVENT_DEFINITIONS = "GetCurrentLineId"
SIBLING_INITIALIZE_REQUIRED = "EnqueueLine"
SIBLING_EVALUATE_READINESS = "AdvanceRadioState"
SIBLING_CONFIGURE_ROUTING = "ClearQueue"
SIBLING_UNBOUND_CATEGORIES = "GetCurrentLineId"
SIBLING_FIND_ENTRY = "GetPlayedLineCount"
SIBLING_GET_REQUIRED = "GetDroppedLineCount"
SIBLING_GET_DISPLAY = "OnLineStarted"
SIBLING_ENTRIES = "EnqueueLine"
...
STOP_BEFORE_SKYLINE_STYLE = "EnqueueLine"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bvoid\s+PrimeLines\("
    r"const TArray<FSkyguardRadioLine>& Lines\)(?:;|\{)"
)
BIND_PRESENTATION_IDENT_RE = re.compile(r"\bPrimeLines\b")
GET_BOUND_PRESENTATION_IDENT_RE = re.compile(r"\bGetCurrentLineId\b")
ENQUEUE_LINE_IDENT_RE = re.compile(r"\bEnqueueLine\b")
ADVANCE_RADIO_STATE_IDENT_RE = re.compile(r"\bAdvanceRadioState\b")
LEFTOVER_BIND_PRESENTATION_IDENT_RE = re.compile(r"\bBindPresentation\b")
LEFTOVER_TOGGLE_PAUSE_IDENT_RE = re.compile(r"\bTogglePause\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # rename, leftover unique PrimeLines,
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bPrimeLines\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bbool\s+PrimeLines\b", compact):
        return False
    if re.search(r"\bfloat\s+PrimeLines\b", compact):
        return False
    if re.search(r"\bint32\s+PrimeLines\b", compact):
        return False
    if re.search(
        r"\bstatic\s+FSkyguardLandscapeVisibleAudit\s+PrimeLines\b",
        compact,
    ):
        return False
    if GET_BOUND_PRESENTATION_IDENT_RE.search(compact) and (
        ...
        return False
    if ENQUEUE_LINE_IDENT_RE.search(compact) and (
        METHOD_SIGNATURE_RE.search(compact) is None
    ):
        return False
    if ADVANCE_RADIO_STATE_IDENT_RE.search(compact) and (
        METHOD_SIGNATURE_RE.search(compact) is None
    ):
        return False
    if LEFTOVER_BIND_PRESENTATION_IDENT_RE.search(compact):
        return False
    if LEFTOVER_TOGGLE_PAUSE_IDENT_RE.search(compact):
        return False
...
class RadioChatterPrimeLinesDeclContractTests(
    unittest.TestCase
):
    def test_radio_chatter_component_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "PrimeLines"),
            section,
        )
    ...
    def test_missing_prime_lines_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn("GetCurrentLineId", collapsed(LOCKED_DECL))
        self.assertTrue(
        ...
        self.assertFalse(
            has_identifier(
                "const TArray<FSkyguardRadioLine>& "
    ...
    def test_prime_lines_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertIn("Lines", LOCKED_DECL)
        self.assertIn(
            "const TArray<FSkyguardRadioLine>&",
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TOGGLE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BIND)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENQUEUE_LINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ADVANCE_RADIO)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid PrimeLinesX(const TArray<FSkyguardRadioLine>& Lines);\n",
            "\tint32 PrimeLines = 1;\n",
            "\tFName PrimeLines = NAME_None;\n",
            "\tvoid PrimeLines() = "
            + forty
            + ";\n",
            "\tvoid PrimeLines() = "
            ...
            f"\t{TARGET_WRONG_BIND}\n",
            f"\t{TARGET_WRONG_TOGGLE}\n",
            f"\t{TARGET_WRONG_ENQUEUE_LINE}\n",
            f"\t{TARGET_WRONG_ADVANCE_RADIO}\n",
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "PrimeLines")
        )
        self.assertTrue(
            has_identifier(section, "PrimeLines")
        )
