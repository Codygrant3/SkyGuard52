...
# Error. THIS IS NOT leftover CaptureConfig Error #1222. THIS IS NOT
# leftover VisibleAudit Error #1213. THIS IS NOT leftover Footprint
# Error #1198. THIS IS NOT leftover HeightSample Error #1188. THIS IS
# NOT leftover landscape-material-compilation-defaults #01d4.
# LOCK leftover landscape-material-compilation-defaults #01d4.
# LOCK leftover landscape-capture-config-defaults #d6fd.
# LOCK leftover mission01-environment-authoring-result-defaults #df1a.
# LOCK leftover CaptureConfigurationResult public UPROPERTY #1214-#1224
# including leftover CaptureConfig Error #1222 / ViewMode #1224.
# LOCK leftover VisibleAudit Error #1213.
# LOCK leftover Footprint Error #1198.
# LOCK leftover HeightSample Error #1188.
# LOCK leftover MaterialCompilation bSuccess #1223 /
# LandscapeComponentCount #1227 / GeneratedMaterialInstanceCount #1225 /
# MaterialResourceCount #1226 / CompilationFinishedResourceCount #1228 /
# ValidShaderMapResourceCount (open draft).
...
# Do NOT parse FSkyguardLandscapeVisibleAudit (leftover Error #1213).
# Do NOT parse FSkyguardLandscapeCaptureConfigurationResult
# (leftover Error #1222).
# Do NOT parse FSkyguardLandscapeFootprintSampleResult
# (leftover Error #1198).
# Do NOT parse FSkyguardLandscapeHeightSample (leftover Error #1188).
# Declaration-only. Accept origin/main split-line UPROPERTY and bare
# FString Error; with no initializer and NO Category=.
...
# THIS IS leftover-safe FSkyguardLandscapeMaterialCompilationResult
# Error. THIS IS NOT leftover VisibleAudit
...
# DEFAULT IS bare `FString Error;` NOT `= false` / `= true` / `= 0`. Fail closed if the type
# or decl is missing, renamed, Category= is added, or a default
# initializer is invented. Parse STRUCT public section only AFTER
# struct FSkyguardLandscapeMaterialCompilationResult and BEFORE
# struct FSkyguardMission01EnvironmentAuthoringResult.
# Do NOT parse USkyguardMission01EnvironmentAuthoringLibrary.
# Do NOT parse ASkyguardMission01EnvironmentDirector.
...
# FString Error; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS bare `FString Error;` NOT `= false` / `= true` / `= 0` (NOT leftover
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare decl. Do not accept an
    # invented initializer or a renamed type for
    # FSkyguardLandscapeMaterialCompilationResult::Error
    # (must be bare FString Error; not FName / FText / = TEXT).
...
class LandscapeMaterialCompilationErrorFieldDeclContractTests(unittest.TestCase):
    ...
    def test_error_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.endswith("Error;"), LOCKED_DECL)
    ...
    def test_contract_is_target_field_declaration_only(
        ...
        self.assertTrue(LOCKED_DECL.startswith("FString Error"))
        self.assertIn("Error", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.endswith("Error;"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("GetProtectedGroup", LOCKED_DECL)
        self.assertNotIn("FSkyguardMission10ProtectedRuntime", LOCKED_DECL)
        self.assertIn(UPROPERTY_MISSION10, section)

    def test_sibling_director_fields_do_not_satisfy_error(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
