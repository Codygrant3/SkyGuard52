# THIS IS leftover-safe FSkyguardCampaignMissionSpec SupportKind.
# origin/main form: plain C++ field
# `ESkyguardThreatKind SupportKind = ESkyguardThreatKind::HeavyAttacker;`
# with in-struct initializer ::HeavyAttacker. NOT a UPROPERTY wrap.
...
# FastAttacker / GroundArmor / RotorScout / PatrolShip
# instead of HeavyAttacker.
...
# ShoreKind / ExtractKind / Climax / bNightIdentity /
...
# (FastAttacker) / ShoreKind (GroundArmor) / ExtractKind
# siblings.
...
# ::HeavyAttacker. Parse window comes from leftover-safe
...
TARGET_WRONG_BARE = "ESkyguardThreatKind SupportKind;"
TARGET_WRONG_FAST = (
    "ESkyguardThreatKind SupportKind = "
...
TARGET_WRONG_EQ_NONE = "ESkyguardThreatKind SupportKind = NAME_None;"
TARGET_WRONG_FALSE = "ESkyguardThreatKind SupportKind = false;"
TARGET_WRONG_TRUE = "ESkyguardThreatKind SupportKind = true;"
TARGET_WRONG_ZERO = "ESkyguardThreatKind SupportKind = 0.f;"
TARGET_WRONG_FLOAT = "float SupportKind;"
TARGET_WRONG_HEALTH = "float SupportKind = 160.f;"
TARGET_WRONG_BOOL = "bool SupportKind;"
TARGET_WRONG_INT = "int32 SupportKind;"
TARGET_WRONG_FNAME = "FName SupportKind;"
...
SUPPORT_KIND_DECL_RE = re.compile(
    r"ESkyguardThreatKind\s+SupportKind\s*=\s*"
    r"ESkyguardThreatKind\s*::\s*HeavyAttacker\s*;"
)
SUPPORT_KIND_INIT_RE = re.compile(
    r"ESkyguardThreatKind\s+SupportKind\s*=\s*([^;]+);"
)
WRONG_INITIALIZERS = (
    "ESkyguardThreatKind::FastAttacker",
...
def support_kind_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*ESkyguardThreatKind\s+SupportKind\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # ::HeavyAttacker. Do not accept leftover TheaterKit
    # Category clones as this CampaignMissionSpec parse
    # window.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SUPPORT_KIND_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+SupportKind\b", compact):
        return False
    if re.search(r"FName\s+SupportKind\b", compact):
        return False
    if re.search(r"bool\s+SupportKind\b", compact):
        return False
    if re.search(r"int32\s+SupportKind\b", compact):
        return False
    if support_kind_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    for match in SUPPORT_KIND_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != "ESkyguardThreatKind::HeavyAttacker":
            return False
    for leftover_init in WRONG_INITIALIZERS:
        if leftover_init in compact and leftover_init.split("::")[1] != (
            ...
            if re.search(
                r"SupportKind\s*=\s*" + re.escape(leftover_init),
                compact,
            ):
                return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardThreatKind\s+SupportKind\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardThreatKind SupportKind is missing "
...
class CampaignMissionSpecSupportKindFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_enum_not_uproperty(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardThreatKind SupportKind"),
        )
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("::HeavyAttacker", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("SupportKind", str(raised.exception))
    ...
    def test_leftover_storm_rain_kinds_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn("SupportKind", LEFTOVER_STORM_KINDS_DECL)
        leftover = (
    ...
    def test_missing_support_kind_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_kind_fields_do_not_satisfy_support_kind(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_CONTACT}\n"
    ...
    def test_initializer_must_be_heavy_attacker(self) -> None:
        missing = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
        ...
        self.assertNotIn(
            "SupportKind = ESkyguardThreatKind::FastAttacker",
            compact_origin,
        )
        self.assertNotIn(
            "SupportKind = ESkyguardThreatKind::GroundArmor",
            compact_origin,
        )
        self.assertNotIn(
            "SupportKind = ESkyguardThreatKind::RotorScout",
            compact_origin,
        )
        self.assertNotIn(
            "SupportKind = ESkyguardThreatKind::PatrolShip",
            compact_origin,
        )
        self.assertNotIn("SupportKind = NAME_None", compact_origin)
        self.assertNotIn("SupportKind = false", compact_origin)
        self.assertNotIn("SupportKind = true", compact_origin)
        self.assertNotIn("SupportKind = 0.f", compact_origin)
        self.assertNotIn("SupportKind = 160.f", compact_origin)

    def test_support_kind_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_FAST}\n", LOCKED_DECL)
        self.assertIn("SupportKind", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tESkyguardThreatKind SupportKinds = "
            "ESkyguardThreatKind::HeavyAttacker;\n",
            "\tint32 SupportKind;\n",
            "\tbool SupportKind;\n",
            "\tfloat SupportKind = " + forty + ";\n",
            "\tfloat SupportKind = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    support_kind_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(support_kind_has_uproperty_wrap(origin), origin)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tESkyguardThreatKind\n"
            "\tSupportKind = ESkyguardThreatKind::HeavyAttacker;\n",
            "\tESkyguardThreatKind   SupportKind = "
            "ESkyguardThreatKind::HeavyAttacker;\n",
            "\tESkyguardThreatKind\tSupportKind = "
            "ESkyguardThreatKind::HeavyAttacker;\n",
    ...
    def test_harbor_breaker_is_not_this_spec(self) -> None:
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec SupportKind field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "support_kind_field_decl_contract.py"
        ))
        self.assertIn("campaign_mission_spec", Path(__file__).name)
        self.assertIn("support_kind", Path(__file__).name)
