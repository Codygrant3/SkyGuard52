# THIS IS leftover-safe FSkyguardStormRainBeatKit WeatherIdentity.
...
# WeatherLabel / Weather / bHydraForClusters / Kinds / Threats /
# Stations / Calls.
# Do NOT claim leftover analog Weather enum field or WeatherLabel.
...
# Category="Skyguard|Theater" / VisibleAnywhere / float /
# `= 160.f`, retarget: type is FName, no `=`, locked decl is
# the bare field inside FSkyguardStormRainBeatKit only.
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # WeatherLabel / Weather enum / BeatCount / Title.
...
class StormRainBeatKitWeatherIdentityFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_does_not_claim_leftover_weather_label_or_weather_enum(
        self,
    ) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEATHER_LABEL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEATHER)
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_WEATHER}\n", LOCKED_DECL)
        self.assertIn("WeatherIdentity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        siblings_only = (
    ...
    def test_weather_identity_declaration_matches_origin_main(self) -> None:
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, LEFTOVER_NIGHT_KEEP_THERMAL))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "storm-rain-beat-kit WeatherIdentity field decl contract "
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, LEFTOVER_NIGHT_KEEP_THERMAL))
        for field in leftover_neighbor_hit_fields():
        ...
        self.assertNotIn(TARGET_WRONG_WEATHER_LABEL, locked_only)
        self.assertNotIn(TARGET_WRONG_WEATHER, locked_only)
        for helper in namespace_helper_tokens():
