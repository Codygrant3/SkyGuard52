# THIS IS leftover-safe FSkyguardMissionPresentation MinimumBriefingWarmupSeconds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "0.0"))
# then `float MinimumBriefingWarmupSeconds = 3.f;` (initializer IS `3.f`,
# not `3.0f` / `3.0` / `3` / Harbor 40/80 / `0.f` / `5.f`).
# THIS IS leftover-safe isolated UPROPERTY with no Category.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# ClampMin = "0.0" IS present. Initializer IS `= 3.f`.
...
# drop to BlueprintReadWrite, if type is not float,
# if the initializer is not `= 3.f`, if ClampMin is
# missing or wrong, or if Category= is added.
# Harbor tokens built as 40 + ".f" / 80 + ".f" fail-closed
# if they appear in the parse window.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardMissionPresentation` body ONLY after
# `struct FSkyguardMissionPresentation`. Stop at
# `struct FSkyguardMissionScoreRules`.
# Do NOT parse leftover `FSkyguardBossDefinition`.
...
# (leftover presentation field decls including Presentation #359).
# Do NOT parse leftover briefing-card / briefing-radio-row /
# how-to-fly-row / briefing-widget / briefing-fail-closed /
# sortie-presentation-fail-closed.
# Do NOT contract sibling fields Briefing / RadioChatter /
# SuccessDebrief / FailureDebrief.
# Those sibling decls remain unclaimed.
# THIS IS NOT leftover mission-presentation-defaults
# #186 / #157e (keep that file in LOCKED_SCRIPTS; isolated
# MinimumBriefingWarmupSeconds field decl does not relock
# the defaults bulk).
# THIS IS NOT leftover USkyguardMissionDefinition
# Presentation #359.
...
# Analog leftover mission-presentation-defaults then
# isolated MinimumBriefingWarmupSeconds field decl.
...
# MultiLine / bare FText Briefing / no `=` /
# ClampMin missing, retarget: type is float
# MinimumBriefingWarmupSeconds, initializer is `= 3.f`,
# specifiers are EditAnywhere + BlueprintReadOnly +
# meta = (ClampMin = "0.0"), no Category.
...
STRUCT_NAME = "FSkyguardMissionPresentation"
TARGET = "float MinimumBriefingWarmupSeconds = 3.f;"
TARGET_WRONG_EQ_NONE = "float MinimumBriefingWarmupSeconds = 3.0f;"
TARGET_WRONG_FALSE = "float MinimumBriefingWarmupSeconds = false;"
TARGET_WRONG_TRUE = "float MinimumBriefingWarmupSeconds = true;"
TARGET_WRONG_ZERO = "float MinimumBriefingWarmupSeconds = 0.f;"
TARGET_WRONG_FLOAT = "int32 MinimumBriefingWarmupSeconds = 3;"
TARGET_WRONG_FNAME = "FName MinimumBriefingWarmupSeconds;"
TARGET_WRONG_EMPTY = "float MinimumBriefingWarmupSeconds;"
TARGET_WRONG_THREE = "float MinimumBriefingWarmupSeconds = 3;"
TARGET_WRONG_THREE_POINT_ZERO = (
    "float MinimumBriefingWarmupSeconds = 3.0;"
)
TARGET_WRONG_FIVE = "float MinimumBriefingWarmupSeconds = 5.f;"
TARGET_WRONG_FTEXT = "FText MinimumBriefingWarmupSeconds;"
...
SIBLING_BRIEFING = "Briefing"
SIBLING_RADIO_CHATTER = "RadioChatter"
SIBLING_SUCCESS_DEBRIEF = "SuccessDebrief"
SIBLING_FAILURE_DEBRIEF = "FailureDebrief"
...
LEFTOVER_THEATER_KIT_NAMESPACE = "SkyguardCampaignTheaterKit"
THIS_SCRIPT = (
    "Scripts/tests/test_mission_presentation_minimum"
    "_briefing_warmup_seconds_field_decl_contract.py"
)
LEFTOVER_MISSION_PRESENTATION_BRIEFING = (
    "Scripts/tests/test_mission_presentation_briefing"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION_PRESENTATION_RADIO_CHATTER = (
...
LEFTOVER_MISSION_TYPES_DEFAULTS_PY = (
...
LEFTOVER_MISSION_DEFINITION_PRESENTATION = (
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_BRIEFING,
        SIBLING_RADIO_CHATTER,
        SIBLING_SUCCESS_DEBRIEF,
        SIBLING_FAILURE_DEBRIEF,
    )


def leftover_neighbor_struct_tokens() -> tuple[str, ...]:
    return (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Reject missing initializer, `3.0f` / `3.0` / `3` /
    # Harbor 40/80 / `0.f` / leftover Health / `5.f`.
    # Do not accept sibling Briefing / RadioChatter /
    # SuccessDebrief / FailureDebrief fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+MinimumBriefingWarmupSeconds\s*=\s*3\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+MinimumBriefingWarmupSeconds\s*=\s*3\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MinimumBriefingWarmupSeconds\s*=\s*3\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MinimumBriefingWarmupSeconds\s*=\s*3\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MinimumBriefingWarmupSeconds\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MinimumBriefingWarmupSeconds\s*=\s*5\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MinimumBriefingWarmupSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MinimumBriefingWarmupSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+MinimumBriefingWarmupSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MinimumBriefingWarmupSeconds\b",
        compact,
    ):
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return leftover_neighbor_struct_tokens() + (
        ...
        f"namespace {LEFTOVER_THEATER_KIT_NAMESPACE}",
...
def spec_section(header: str) -> str:
    ...
    if STOP_BEFORE_SCORE_RULES in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_SCORE_RULES}"
        )
    forty = "40" + ".f"
    eighty = "80" + ".f"
    for token in (forty, eighty):
        if token in section:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes harbor token {token}"
            )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MinimumBriefingWarmupSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MinimumBriefingWarmupSeconds is missing from "
...
def require_warmup_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            "MinimumBriefingWarmupSeconds UPROPERTY"
        )
    ...
    if "VisibleAnywhere" in specifiers:
        raise AssertionError(
            "VisibleAnywhere instead of EditAnywhere on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} "
            "MinimumBriefingWarmupSeconds UPROPERTY"
        )
    if "MultiLine" in specifiers:
        raise AssertionError(
            "MultiLine is added on origin/main:"
            f"{HEADER_PATH} struct {STRUCT_NAME} "
            "MinimumBriefingWarmupSeconds UPROPERTY"
        )
    return specifiers
...
class MissionPresentationMinimumBriefingWarmupSecondsFieldDeclContractTests(unittest.TestCase):
    def test_mission_presentation_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_MISSION_DEFINITION)
        ...
        self.assertNotEqual(STRUCT_NAME, "FSkyguardTheaterKitSpec")
        body = struct_body(header)
        self.assertTrue(body.startswith("{"), body)
        self.assertTrue(body.endswith("}"), body)
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertTrue(has_identifier(section, "MinimumBriefingWarmupSeconds"), section)
        ...
        self.assertNotIn(LEFTOVER_THEATER_KIT_ACTOR, header)
        self.assertNotIn(LEFTOVER_THEATER_KIT_NAMESPACE, header)
        self.assertNotIn("ApplyTheaterKitToWorld", header)
        self.assertNotIn("WeatherIdentity", section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedMissionPresentation\n{\n};\n"
            )
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        leftover_def = (
            f"class {STOP_BEFORE_MISSION_DEFINITION}\n"
            "{\n"
            "public:\n"
            '\tUPROPERTY(EditAnywhere, BlueprintReadOnly, '
    ...
    def test_score_rules_or_leftover_definition_does_not_satisfy(self) -> None:
        mixed = (
        ...
        self.assertFalse(has_identifier(section, "MinimumBriefingWarmupSeconds"), section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MinimumBriefingWarmupSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_warmup_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tTArray<FText> {SIBLING_RADIO_CHATTER};\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_warmup_specifiers(section)
    ...
    def test_initializer_fails_closed_when_not_three_f(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
        ...
        self.assertNotIn(
            "MinimumBriefingWarmupSeconds = 3.0f",
            compact_origin,
        )
        self.assertNotIn(
            "MinimumBriefingWarmupSeconds = false",
            compact_origin,
        )
        self.assertNotIn(
            "MinimumBriefingWarmupSeconds = true",
            compact_origin,
        )
        self.assertNotIn(
            "MinimumBriefingWarmupSeconds = 0.f",
            compact_origin,
        )
        self.assertNotIn(
            "MinimumBriefingWarmupSeconds = 160.f",
            compact_origin,
        )
        self.assertNotIn(
            "MinimumBriefingWarmupSeconds = 5.f",
            compact_origin,
        )

    def test_warmup_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MinimumBriefingWarmupSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 3.f", LOCKED_DECL)
        self.assertNotIn("= 3.0f", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_THREE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_THREE_POINT_ZERO}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 3.f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 3.f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            f"\t{TARGET_WRONG_THREE}\n",
            f"\t{TARGET_WRONG_THREE_POINT_ZERO}\n",
            f"\t{TARGET_WRONG_FIVE}\n",
            f"\t{TARGET_WRONG_FTEXT}\n",
            leftover_primary,
            leftover_guided,
            f"\tFText {SIBLING_BRIEFING};\n",
            f"\tTArray<FText> {SIBLING_RADIO_CHATTER};\n",
            f"\tFText {SIBLING_SUCCESS_DEBRIEF};\n",
            f"\tFText {SIBLING_FAILURE_DEBRIEF};\n",
            "\tfloat MinimumBriefingWarmupSecondss = 3.f;\n",
            "\tint32 MinimumBriefingWarmupSeconds;\n",
            "\tbool MinimumBriefingWarmupSeconds;\n",
            "\tfloat MinimumBriefingWarmupSeconds = " + forty + ";\n",
            "\tfloat MinimumBriefingWarmupSeconds = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_clamp_min_fails_closed(self) -> None:
        origin = require_warmup_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(write_only))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        added_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(added_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(compact_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        with_max = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_warmup_specifiers(spec_section(with_max))
    ...
    def test_does_not_contract_sibling_presentation_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_leftover_neighbors(self) -> None:
        ...
        self.assertNotIn("struct FSkyguardBriefingCard", header)
        self.assertNotIn("struct FSkyguardBriefingRadioRow", header)
        self.assertNotIn("struct FSkyguardHowToFlyRow", header)
        ...
        self.assertNotIn(LEFTOVER_THEATER_KIT_ACTOR, header)
        self.assertNotIn("WeatherIdentity", header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-presentation MinimumBriefingWarmupSeconds field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "minimum_briefing_warmup_seconds_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_THEATER_KIT_BULK, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_WEATHER_IDENTITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_PRESENTATION_BRIEFING, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_PRESENTATION_RADIO_CHATTER, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_PRESENTATION_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_PRESENTATION_DEFAULTS_PY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_PRESENTATION_DEFAULTS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_TYPES_DEFAULTS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_TYPES_DEFAULTS_PY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_TYPES_DEFAULTS_CONTRACT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_DEFINITION_PRESENTATION, LOCKED_SCRIPTS)
    ...
    def test_leftover_mission_presentation_defaults_stay_locked(self) -> None:
        leftovers = (
            LEFTOVER_MISSION_PRESENTATION_DEFAULTS,
            LEFTOVER_MISSION_PRESENTATION_DEFAULTS_PY,
            LEFTOVER_MISSION_PRESENTATION_DEFAULTS_TESTS,
