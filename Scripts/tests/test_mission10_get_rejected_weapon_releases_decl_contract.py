...
CLASS_NAME = "ASkyguardMission10IntegrationDirector"
# Declaration presence only. Do not invent
# INDEX_NONE or lock GetRejectedWeaponReleases
# construction in the .cpp. This is leftover-safe
# Mission10 Safety GetRejectedWeaponReleases
# METHOD on ASkyguardMission10IntegrationDirector.
# THIS is GetRejectedWeaponReleases, NOT leftover
# Mission08 GetRejectedWeaponReleases #745 / branch
# cursor/mission08-get-rejected-weapon-
# releases-decl-contract-c332, NOT leftover
# BindRuntimeActors. It is NOT leftover Mission09
...
# #7898, leftover Mission08 GetRejectedWeaponReleases
...
# inline body
# (`int32 GetRejectedWeaponReleases() const { return RejectedWeaponReleases; }`);
# accept that form, other one-line / split-line wraps, and
# an inline body without locking a different body.
# Nearby origin/main
...
# Category is Skyguard|Mission10|Safety, not
...
# not leftover Mission10 Waves / Integration /
...
# GetRejectedWeaponReleases #745. Do not reopen leftover
...
REQUIRED_DECL = GET_REJECTED_WEAPON_RELEASES
ORIGIN_MAIN_INLINE = (
    "int32 GetRejectedWeaponReleases() const\n"
    "\t{\n"
...
# an isolated Python GetRejectedWeaponReleases
# METHOD declaration contract on
# ASkyguardMission10IntegrationDirector.
# THIS is GetRejectedWeaponReleases, NOT leftover
# BindRuntimeActors, NOT leftover Mission08
# GetRejectedWeaponReleases #745, NOT leftover
# Mission09 GetObjectiveRuntime #764.
LOCKED = {
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission10IntegrationDirector::GetRejectedWeaponReleases",
    "ASkyguardMission08IntegrationDirector::GetRejectedWeaponReleases",
...
class Mission10GetRejectedWeaponReleasesDeclContractTests(unittest.TestCase):
    def test_mission10_class_exists(self) -> None:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("GetRejectedWeaponReleases", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, REQUIRED_DECL)
    ...
    def test_missing_get_rejected_weapon_releases_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission10IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn("Skyguard|Mission10|Safety", UFUNCTION_SAFETY)
        ...
        self.assertNotIn("Mission05", UFUNCTION_SAFETY)
        self.assertNotIn("Mission06", UFUNCTION_SAFETY)
        self.assertNotIn("Waves", UFUNCTION_SAFETY)
        self.assertNotIn("Search", UFUNCTION_SAFETY)
        self.assertNotIn("Protection", UFUNCTION_SAFETY)
        self.assertNotIn("Payload", UFUNCTION_SAFETY)
        self.assertNotIn("Targets", UFUNCTION_SAFETY)
        self.assertNotIn("Objectives", UFUNCTION_SAFETY)
        self.assertNotIn("Mission07", UFUNCTION_SAFETY)
        self.assertNotIn("Mission08", UFUNCTION_SAFETY)
        self.assertNotIn("Hoist", UFUNCTION_SAFETY)
        self.assertNotIn("Performance", UFUNCTION_SAFETY)
        self.assertNotIn("Integration", UFUNCTION_SAFETY)
        self.assertNotIn("Rescue", UFUNCTION_SAFETY)
        self.assertIn("BlueprintPure", UFUNCTION_SAFETY)
        self.assertNotIn("Environment", UFUNCTION_SAFETY)
        self.assertNotIn("Briefing", UFUNCTION_SAFETY)
        self.assertNotIn("Mission02", UFUNCTION_SAFETY)
        self.assertNotIn("Boss", UFUNCTION_SAFETY)
        self.assertNotIn("Destruction", UFUNCTION_SAFETY)
        self.assertNotIn("Apache", UFUNCTION_SAFETY)
        self.assertNotIn("Mission06", UFUNCTION_SAFETY)
        self.assertNotIn("Mission07", UFUNCTION_SAFETY)
        self.assertNotIn("Mission09", UFUNCTION_SAFETY)
        self.assertNotIn("Encounter", UFUNCTION_SAFETY)
        self.assertNotIn("Integration", UFUNCTION_SAFETY)
        self.assertNotIn("Hoist", UFUNCTION_SAFETY)
        self.assertNotIn("Rescue", UFUNCTION_SAFETY)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_SAFETY)
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid GetRejectedWeaponReleases() const;\n"
        as_const = "\tint32 GetRejectedWeaponReleases();\n"
        with_arg = "\tint32 GetRejectedWeaponReleases(int32 Amount) const;\n"
        renamed = "\tint32 GetRejectedWeaponRelease() const;\n"
        short_name = "\tint32 RejectedWeaponReleases() const;\n"
    ...
    def test_get_rejected_weapon_releases_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, REQUIRED_DECL),
            REQUIRED_DECL,
        ...
        self.assertTrue(
            REQUIRED_DECL.startswith("int32 "),
            REQUIRED_DECL,
        )
        self.assertTrue(
            REQUIRED_DECL.endswith(" const"),
            REQUIRED_DECL,
        )
        self.assertFalse(REQUIRED_DECL.endswith(";"), REQUIRED_DECL)
        self.assertIn("GetRejectedWeaponReleases", REQUIRED_DECL)
        self.assertIn("GetRejectedWeaponReleases()", REQUIRED_DECL)
        self.assertIn("int32", REQUIRED_DECL)
        ...
        self.assertNotIn("UFUNCTION", REQUIRED_DECL)
        self.assertNotIn("{", REQUIRED_DECL)
        self.assertNotIn("}", REQUIRED_DECL)
        self.assertNotIn("return ", REQUIRED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tint32\n"
            "\tGetRejectedWeaponReleases() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_name = (
            "public:\n"
            "\tint32 GetRejectedWeaponReleases(\n"
            "\t\t) const;\n"
            "};\n"
        )
        wrap_ufunction = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SAFETY} {REQUIRED_DECL}\n"
            "};\n"
        )
    ...
    def test_environment_category_does_not_satisfy_safety(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_SAFETY)
        self.assertNotIn("Briefing", UFUNCTION_SAFETY)
        environment = (
        ...
        self.assertNotEqual(environment, UFUNCTION_SAFETY)
        self.assertNotEqual(briefing, UFUNCTION_SAFETY)
        self.assertNotIn(environment, UFUNCTION_SAFETY)
        self.assertNotIn(briefing, UFUNCTION_SAFETY)
        leftover_m01 = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m02, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_harbor, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m03, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m04, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m05, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m06, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m07, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m08, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m09_waves, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_m09_pool, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_hoist, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_safety, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_waves, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_payload, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_targets, UFUNCTION_SAFETY)
        self.assertNotEqual(leftover_objectives, UFUNCTION_SAFETY)
        self.assertNotIn(leftover_m01, UFUNCTION_SAFETY)
        self.assertNotIn(leftover_m02, UFUNCTION_SAFETY)
        self.assertNotIn(leftover_harbor, UFUNCTION_SAFETY)
        self.assertNotIn(leftover_m03, UFUNCTION_SAFETY)
        self.assertNotIn(leftover_m04, UFUNCTION_SAFETY)
        self.assertNotIn(leftover_m05, UFUNCTION_SAFETY)
        self.assertNotIn(leftover_m06, UFUNCTION_SAFETY)
        self.assertNotIn("Mission01", UFUNCTION_SAFETY)
        self.assertNotIn("Mission03", UFUNCTION_SAFETY)
        self.assertNotIn("Mission04", UFUNCTION_SAFETY)
        self.assertNotIn("Mission05", UFUNCTION_SAFETY)
    ...
    def test_contract_does_not_lock_get_rejected_weapon_releases_cpp_body(self) -> None:
        locked_only = f"{REQUIRED_DECL}\n"
        self.assertNotIn("{", REQUIRED_DECL)
        self.assertNotIn("}", REQUIRED_DECL)
        self.assertNotIn("return ", REQUIRED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("GetRejectedWeaponReleases", REQUIRED_DECL)
        self.assertTrue(REQUIRED_DECL.startswith("int32 "))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission10 GetRejectedWeaponReleases method contract "
    ...
    def test_contract_is_get_rejected_weapon_releases_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(REQUIRED_DECL.startswith("int32 "))
        self.assertIn("GetRejectedWeaponReleases", REQUIRED_DECL)
        self.assertTrue(REQUIRED_DECL.endswith(" const"))
        self.assertFalse(REQUIRED_DECL.endswith(";"))
        self.assertIn(ORIGIN_MAIN_INLINE, section)
        self.assertIn(UFUNCTION_SAFETY, section)

    def test_sibling_director_fields_do_not_satisfy_get_rejected_weapon_releases(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_leftover_bind_runtime_actors_is_not_the_required_decl(
        ...
        self.assertNotEqual(BIND_RUNTIME_ACTORS, "GetRejectedWeaponReleases")
        self.assertNotIn(BIND_RUNTIME_ACTORS, REQUIRED_DECL)
        self.assertTrue(
            REQUIRED_DECL.startswith("int32 GetRejectedWeaponReleases("),
            REQUIRED_DECL,
        )
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            ...
            self.assertNotIn(token, REQUIRED_DECL)
        self.assertNotIn("HandleBossPhaseChanged", REQUIRED_DECL)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_SAFETY)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_get_rejected_weapon_releases"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_contract_does_not_reopen_leftover_mission08_get_rejected(
        ...
        self.assertNotEqual(leftover_m08_safety, UFUNCTION_SAFETY)
        self.assertNotIn(
            "cursor/mission08-get-rejected-weapon-releases-decl-contract-c332",
            REQUIRED_DECL,
        )
