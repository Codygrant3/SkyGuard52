...
# INDEX_NONE or lock bActive
# construction in the .cpp. This is leftover-safe
# Mission04 SearchlightTrackRuntime bActive
...
# GetSearchlightRuntime or leftover defaults),
# NOT leftover Mission04 IntegrationReadiness
# bSearchlightsReady FIELD #935 / branch
# cursor/mission04-searchlights-ready-field-
# decl-contract-c332 (not the director, not
...
# (do not lock GetSearchlightRuntime / leftover defaults).
...
# drafts #674-#941 including leftover Mission02
...
LOCKED_DECL = SEARCHLIGHT_TRACK_ACTIVE_FIELD
...
SEARCHLIGHT_TRACK_ACTIVE_FIELD_TRUE = (
    "bool bActive = true;"
)
...
LEFTOVER_SEARCHLIGHT_TRACK_SIBLING_FIELDS_NOT_LOCKED = (
    SEARCHLIGHT_TRACK_BOSS_TRACKED_FIELD,
    SEARCHLIGHT_TRACK_REMAINING_SECONDS_FIELD,
    SEARCHLIGHT_TRACK_HELD_SECONDS_FIELD,
    SEARCHLIGHT_TRACK_COMPLETED_PASSES_FIELD,
...
class Mission04SearchlightTrackActiveFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bActive", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_searchlight_track_active_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_searchlight_track_active_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bActive"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("= false;"), LOCKED_DECL)
        self.assertIn("bActive", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_true = (
            "public:\n"
            f"\t{UPROPERTY_MISSION10}\n"
            f"\t{SEARCHLIGHT_TRACK_ACTIVE_FIELD_TRUE}\n"
            "};\n"
        )
        wrap_true_one_line = (
            "public:\n"
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, SEARCHLIGHT_TRACK_ACTIVE_FIELD_TRUE),
                section,
            )
            self.assertEqual(
                require_declaration(
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bActive", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bActive"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission04 searchlight-track bActive field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission04 searchlight-track bActive contains "
    ...
    def test_contract_is_searchlight_track_active_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_searchlight_track_active(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission04_searchlight_track_active"
            "_field_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission04_searchlight_track_active"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
