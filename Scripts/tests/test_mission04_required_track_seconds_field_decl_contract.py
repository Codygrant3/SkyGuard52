...
# INDEX_NONE or lock RequiredTrackSeconds
# construction in the .cpp. This is leftover-safe
# Mission04 RequiredTrackSeconds FIELD
# (declaration-only, one-line / split-line
# UPROPERTY wraps plus = 3.f, ClampMin 0.1,
# float default 3.f) on
...
# NOT leftover Mission04 MissedTrackDamage sibling,
...
UPROPERTY_MISSION04_CLAMP = 'meta=(ClampMin="0.1")'
...
LOCKED_DECL = REQUIRED_TRACK_SECONDS_FIELD
...
# an isolated Python RequiredTrackSeconds
...
class Mission04RequiredTrackSecondsFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RequiredTrackSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_required_track_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_required_track_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float RequiredTrackSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("RequiredTrackSeconds", LOCKED_DECL)
        self.assertIn("= 3.f", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("RequiredTrackSeconds", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float RequiredTrackSeconds"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("RequiredTrackSeconds", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission04 RequiredTrackSeconds field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission04 RequiredTrackSeconds contains "
    ...
    def test_contract_is_required_track_seconds_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_required_track_seconds(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        assigned = "\tfloat RequiredTrackSeconds = 1.f;\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission04_required_track_seconds_field"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission04_required_track_seconds_field"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_mission10_campaign_save_user_index_does_not_satisfy(
        ...
        self.assertNotIn("MissedTrackDamage", LOCKED_DECL)
    ...
    def test_contract_does_not_parse_searchlight_track_runtime_struct(
        ...
        self.assertNotIn(MISSED_TRACK_DAMAGE_FIELD, LOCKED_DECL)
