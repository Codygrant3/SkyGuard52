# THIS IS leftover-safe FSkyguardTheaterKitSpec NamedLandmark.
...
# then BARE `FName NamedLandmark;` (no initializer).
...
TARGET = "FName NamedLandmark;"
TARGET_WRONG_EQ_NONE = "FName NamedLandmark = NAME_None;"
TARGET_WRONG_FALSE = "FName NamedLandmark = false;"
TARGET_WRONG_TRUE = "FName NamedLandmark = true;"
TARGET_WRONG_ZERO = "FName NamedLandmark = 0.f;"
TARGET_WRONG_FLOAT = "float NamedLandmark;"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+NamedLandmark\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName NamedLandmark is missing from "
...
class TheaterKitSpecNamedLandmarkFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "NamedLandmark"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "NamedLandmark"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("NamedLandmark", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_named_landmark_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName NamedLandmark;", compact_origin)
        self.assertNotIn("NamedLandmark = NAME_None", compact_origin)
        self.assertNotIn("NamedLandmark = false", compact_origin)
        self.assertNotIn("NamedLandmark = true", compact_origin)
        self.assertNotIn("NamedLandmark = 0.f", compact_origin)
        self.assertNotIn("NamedLandmark = 160.f", compact_origin)

    def test_named_landmark_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName NamedLandmark"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            "\tFName NamedLandmarks;\n",
            "\tint32 NamedLandmark;\n",
            "\tbool NamedLandmark;\n",
            "\tfloat NamedLandmark = " + forty + ";\n",
            "\tfloat NamedLandmark = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec NamedLandmark field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "named_landmark_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(CLONE_BUILDING_KIT, LOCKED_SCRIPTS)
        self.assertIn(CLONE_BUILDING_TINT, LOCKED_SCRIPTS)
        self.assertIn(CLONE_LAMP_TREATMENT, LOCKED_SCRIPTS)
        self.assertIn(CLONE_LAMP_COLOR, LOCKED_SCRIPTS)
        self.assertIn(CLONE_LAMP_INTENSITY, LOCKED_SCRIPTS)
        self.assertIn(CLONE_ROAD_TREATMENT, LOCKED_SCRIPTS)
        self.assertIn(CLONE_ROAD_TINT, LOCKED_SCRIPTS)
