# THIS IS leftover-safe FSkyguardRouteDefinition RouteId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName RouteId;` (no initializer).
...
# if Category= is added, if an initializer is added,
# or if the type is not FName.
...
# Do NOT parse leftover `FSkyguardRoutePoint` body except
# as the sibling Points TArray element type
# (leftover isolated RoutePoint #1339-#1343,
# leftover route-point-defaults #180 / #0ac5).
...
# or leftover `USkyguardAudioProductionBank`.
...
# Do NOT contract sibling field Points.
# Sibling Points stays unclaimed.
# Do NOT claim leftover RoutePoint PointId.
# Analog leftover route-definition-fields #192 / #dbff
# then leftover isolated RoutePoint fields #1339-#1343,
# then this isolated RouteId field decl.
...
# THIS IS NOT leftover isolated RoutePoint PointId #1339
# or leftover WorldLocation / TargetAirspeedKph /
# LookAheadSeconds / bAllowCombatOrbit.
# THIS IS NOT leftover RadioLine isolated fields
# #1332-#1337 or leftover CooldownSeconds.
...
# UPROPERTY exhausted #1364-#1370.
# Clone parse-window style from leftover RoutePoint
# PointId #1339. Clone LOCKED_SCRIPTS from leftover-safe
# draft #1300 (theater-kit-spec-weather-identity).
# If a clone asserts VisibleAnywhere /
# Category="Skyguard|Theater" / BlueprintReadWrite /
# `FName WeatherIdentity` / `FName PointId` /
# required initializer, retarget: type is FName RouteId,
...
TARGET = "FName RouteId;"
TARGET_WRONG_EQ_NONE = "FName RouteId = NAME_None;"
TARGET_WRONG_FALSE = "FName RouteId = false;"
TARGET_WRONG_TRUE = "FName RouteId = true;"
TARGET_WRONG_ZERO = "FName RouteId = 0.f;"
TARGET_WRONG_FLOAT = "float RouteId;"
...
SIBLING_POINTS = "Points"
LEFTOVER_ROUTE_POINT_POINT_ID_FIELD = "PointId"
...
def leftover_route_point_field_names() -> tuple[str, ...]:
    return (
        LEFTOVER_ROUTE_POINT_POINT_ID_FIELD,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+RouteId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName RouteId is missing from "
...
def require_route_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} RouteId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} RouteId"
        )
...
class RouteDefinitionRouteIdFieldDeclContractTests(unittest.TestCase):
    def test_route_definition_struct_exists(self) -> None:
        ...
        self.assertEqual(STRUCT_NAME, ROUTE_DEFINITION_NAME)
        ...
        self.assertTrue(has_identifier(section, "RouteId"), section)
        self.assertFalse(
            has_identifier(section, LEFTOVER_ROUTE_POINT_POINT_ID_FIELD),
            section,
        )
        ...
        self.assertTrue(has_identifier(section, SIBLING_POINTS), section)
        self.assertTrue(has_identifier(section, ROUTE_POINT_NAME), section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedRoutePoint\n{\n};\n"
            )
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RouteId"), section)
        self.assertFalse(has_identifier(section, "PointId"), section)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        self.assertNotIn(OBJECTIVE_DEFINITION_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("RouteId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_route_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tTArray<{ROUTE_POINT_NAME}> {SIBLING_POINTS};\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_route_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName RouteId;", compact_origin)
        self.assertNotIn("RouteId = NAME_None", compact_origin)
        self.assertNotIn("RouteId = false", compact_origin)
        self.assertNotIn("RouteId = true", compact_origin)
        self.assertNotIn("RouteId = 0.f", compact_origin)
        self.assertNotIn("RouteId = 160.f", compact_origin)

    def test_route_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName RouteId"),
            LOCKED_DECL,
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_route_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_route_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_route_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_route_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_route_id_specifiers(spec_section(with_category))
    ...
    def test_does_not_contract_sibling_points_field(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        for leftover in leftover_route_point_field_names():
    ...
    def test_does_not_parse_route_point_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertFalse(
            has_identifier(leaked, LEFTOVER_ROUTE_POINT_POINT_ID_FIELD),
            leaked,
        )
        self.assertIn(STOP_BEFORE_OBJECTIVE, header)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "route-definition RouteId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "route_definition_route_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_POINTS, locked_only)
        self.assertNotIn(LEFTOVER_ROUTE_POINT_POINT_ID_FIELD, locked_only)
        ...
        self.assertNotIn(ROUTE_POINT_NAME, locked_only)
        self.assertNotIn(OBJECTIVE_DEFINITION_NAME, locked_only)
        for helper in namespace_helper_tokens():
