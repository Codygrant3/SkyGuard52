# THIS IS leftover-safe FSkyguardEnemyFormationDefinition Formation.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then `ESkyguardFormationType Formation = ESkyguardFormationType::Vee;`
# (initializer IS `= ESkyguardFormationType::Vee`, no Category).
# THIS IS leftover-safe isolated UPROPERTY with no Category.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# There is NO Category=. There is NO ClampMin.
...
# ESkyguardFormationType, if the initializer is missing,
# or if the initializer is not `= ESkyguardFormationType::Vee`.
# Harbor 40/80 fail-closed in the accepted decl.
...
# Do NOT reopen leftover ESkyguardFormationType enum contracts;
# only lock this UPROPERTY decl.
...
# (in-flight leftover / remaining) / UnitCount /
# SpacingCentimeters.
...
# THIS IS NOT leftover RoutePoint isolated #1339-#1343.
...
# THIS IS NOT leftover FSkyguardAudioEventDefinition /
# ESkyguardAudioEvent.
# Analog: leftover enemy-formation-defaults then this
# isolated Formation field decl, same as leftover
# enemy-wave-defaults then isolated WaveId.
# If a clone asserts VisibleAnywhere /
# Category="Skyguard|Theater" / BlueprintReadWrite /
# `FName WeatherIdentity` / `FName WaveId` /
# `FName FormationId` / bare no `=`, retarget: type is
# ESkyguardFormationType Formation, initializer is
# `= ESkyguardFormationType::Vee`, specifiers are
...
TARGET = (
    "ESkyguardFormationType Formation = "
    "ESkyguardFormationType::Vee;"
)
TARGET_WRONG_EQ_NONE = (
    "ESkyguardFormationType Formation = "
    "ESkyguardFormationType::Line;"
)
TARGET_WRONG_FALSE = "ESkyguardFormationType Formation = false;"
TARGET_WRONG_TRUE = "ESkyguardFormationType Formation = true;"
TARGET_WRONG_ZERO = "ESkyguardFormationType Formation = 0.f;"
TARGET_WRONG_FLOAT = "float Formation;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "ESkyguardFormationType Formation;"
TARGET_WRONG_TRAIL = (
    "ESkyguardFormationType Formation = "
    "ESkyguardFormationType::Trail;"
)
TARGET_WRONG_FNAME = "FName Formation;"
TARGET_WRONG_UNIT = "int32 Formation = 3;"
...
STOP_BEFORE_FORMATION_ENUM = "enum class ESkyguardFormationType"
...
SIBLING_START_TIME = SIBLING_FORMATION_ID
SIBLING_FORMATIONS = SIBLING_UNIT_COUNT
SIBLING_COMPLETION_OBJECTIVE = SIBLING_SPACING
...
REMAINING_SPACING_SCRIPT = (
    "Scripts/tests/test_enemy_formation_spacing_centimeters"
    "_field_decl_contract.py"
)
LEFTOVER_FORMATION_TYPE_ENUM = (
    "Scripts/tests/test_formation_type_enum_contract.py"
)
LEFTOVER_FORMATION_TYPE_ENUM_PY = (
    "Scripts/tests/test_formation_type_enum.py"
)
LEFTOVER_FORMATION_TYPE_ENUM_TESTS = (
    "Scripts/tests/test_formation_type_enum_contract_tests.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Reject missing initializer, ::Line / ::Trail /
    # Harbor 40/80 / leftover Health / FName Formation /
    # int32 Formation / float Formation. Do not accept
    # sibling FormationId / UnitCount / SpacingCentimeters.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"ESkyguardFormationType\s+Formation\s*=\s*"
        r"ESkyguardFormationType::Vee\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"ESkyguardFormationType\s+Formation\s*=\s*"
        r"ESkyguardFormationType::Line\b",
        compact,
    ):
        return False
    if re.search(
        r"ESkyguardFormationType\s+Formation\s*=\s*"
        r"ESkyguardFormationType::Trail\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+Formation\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Formation\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Formation\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardFormationType\s+Formation\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardFormationType Formation is missing from "
...
def require_formation_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} Formation UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} Formation"
        )
...
class EnemyFormationFormationFieldDeclContractTests(unittest.TestCase):
    def test_enemy_formation_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Formation"), section)
        ...
        self.assertIn(STOP_BEFORE_FORMATION_ENUM, header)
        self.assertNotIn(STOP_BEFORE_FORMATION_ENUM, section)
        self.assertNotIn(STOP_BEFORE_FORMATION_ENUM, body)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Formation"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_WAVE, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_DEFINITION, section)
        ...
        self.assertIn("Formation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_formation_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_formation_specifiers(section)
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "Formation = ESkyguardFormationType::Line",
            compact_origin,
        )
        self.assertNotIn(
            "Formation = ESkyguardFormationType::Trail",
            compact_origin,
        )
        self.assertNotIn("Formation = false", compact_origin)
        self.assertNotIn("Formation = true", compact_origin)
        self.assertNotIn("Formation = 0.f", compact_origin)
        self.assertNotIn("Formation = 160.f", compact_origin)
        self.assertNotIn(
            "ESkyguardFormationType Formation;",
    ...
    def test_formation_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardFormationType Formation"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= ESkyguardFormationType::Vee", LOCKED_DECL)
        self.assertNotIn("= ESkyguardFormationType::Line", LOCKED_DECL)
        self.assertNotIn("= ESkyguardFormationType::Trail", LOCKED_DECL)
        ...
        self.assertIn("ESkyguardFormationType ", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TRAIL}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FNAME}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_UNIT}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_TRAIL}\n",
            f"\t{TARGET_WRONG_FNAME}\n",
            f"\t{TARGET_WRONG_UNIT}\n",
            "\tESkyguardFormationType Formations = "
            "ESkyguardFormationType::Vee;\n",
            "\tint32 Formation = 3;\n",
            "\tbool Formation;\n",
            "\tfloat Formation = " + forty + ";\n",
            "\tfloat Formation = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_formation_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formation_specifiers(spec_section(with_category))
    ...
    def test_does_not_parse_neighbor_structs(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_FORMATION_ENUM, header)
        self.assertNotIn(STOP_BEFORE_FORMATION_ENUM, section)
        self.assertNotIn(STOP_BEFORE_FORMATION_ENUM, leaked)
        self.assertIn(STOP_BEFORE_ROUTE_POINT, header)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "enemy-formation Formation field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enemy_formation_formation_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("Formations", locked_only)
        self.assertNotIn("StartTimeSeconds", locked_only)
