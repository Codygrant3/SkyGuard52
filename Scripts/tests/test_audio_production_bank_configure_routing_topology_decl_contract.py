...
# then BARE `bool ConfigureRoutingTopology();`
# (declaration-only, no inline body required, NOT const).
# THIS IS leftover-safe isolated UPrimaryDataAsset UFUNCTION.
# Category IS `Skyguard|Audio|Production`. BlueprintCallable IS present.
# const IS absent. Return type IS bool. Declaration-only.
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# drop to BlueprintPure, if const is present, if the
# return type is not bool, or if Category is
...
# InitializeRequiredEntries / EnsureDefaultEntries /
# EvaluateReadiness / GetUnboundRequiredCategories as this slot
# (leave them uncontracted).
# Do NOT claim leftover unique AudioDirector Production
# UFUNCTION ConfigureRoutingTopology /
...
# Skip leftover analog Production Audio Entry / Routing /
# Audit struct fields (MasterSubmix / CockpitSubmix /
# CockpitExteriorAttenuation / CockpitLowPassHz).
# Skip leftover analog Mix CockpitExteriorAttenuation
# / CockpitLowPassHz.
...
# InitializeRequiredEntries / EnsureDefaultEntries and NOT
...
# Fail-closed if LOCKED_DECL still has const.
...
# Do NOT copy bank header enum names.
...
TARGET = "bool ConfigureRoutingTopology();"
TARGET_INLINE = (
    "bool ConfigureRoutingTopology() { return false; }"
)
TARGET_WRONG_CONST = "bool ConfigureRoutingTopology() const;"
TARGET_WRONG_CONST_INLINE = (
    "bool ConfigureRoutingTopology() const { return false; }"
)
TARGET_WRONG_VOID = "void ConfigureRoutingTopology();"
TARGET_WRONG_FLOAT = "float ConfigureRoutingTopology();"
TARGET_WRONG_INT = "int32 ConfigureRoutingTopology();"
TARGET_WRONG_RENAME = "bool ConfigureRoutingTopologies();"
...
TARGET_WRONG_TRUE = "bool ConfigureRoutingTopology = true;"
TARGET_WRONG_ZERO = "bool ConfigureRoutingTopology = 0.f;"
...
SIBLING_UNBOUND_REQUIRED = "GetUnboundRequiredCategories"
...
TARGET_WRONG_LEFTOVER_DIRECTOR = (
    "bool AreResolvedProductionLoopRoutesComplete() const;"
)
TARGET_WRONG_ROUTING = "FSkyguardProductionAudioRouting Routing;"
TARGET_WRONG_ENTRIES = (
    "TArray<FSkyguardProductionAudioEntry> Entries;"
)
TARGET_WRONG_MASTER_SUBMIX = (
    "TSoftObjectPtr<USoundSubmixBase> MasterSubmix;"
)
LEFTOVER_AUDIO_DIRECTOR_ARE_RESOLVED = (
    "Scripts/tests/test_audio_director_are_resolved_production_loop_routes_complete"
    "_decl_contract.py"
)
LEFTOVER_AUDIO_DIRECTOR_GET_RESOLVED_COUNT = (
...
STOP_BEFORE_EVENT_DEFINITIONS = "EventDefinitions"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bbool\s+ConfigureRoutingTopology\s*\(\s*\)\s*(?!const)(?:;|\{)"
)
...
def leftover_analog_field_in_region(compact: str) -> bool:
    ...
    if re.search(r"FName\s+WeatherIdentity\b", compact):
        return True
    if re.search(r"FSkyguardProductionAudioRouting\b", compact):
        return True
    if re.search(r"\bMasterSubmix\b", compact):
        return True
    if re.search(r"\bCockpitExteriorAttenuation\b", compact):
        return True
    if re.search(r"\bCockpitLowPassHz\b", compact):
        return True
    if re.search(
        r"\bAreResolvedProductionLoopRoutesComplete\b",
        compact,
    ):
        return True
    if re.search(r"LockWindowAttackInterval\b", compact):
        return True
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # const qualifier, a void/float/int32 rename, extra params,
    # leftover unique AudioDirector
    # AreResolvedProductionLoopRoutesComplete, leftover analog
    # IsDestroyed / GetLampInstanceCount /
    # GetResolvedProductionLoopRouteCount /
    # InitializeRequiredEntries / EnsureDefaultEntries,
    # leftover FName WeatherIdentity, leftover analog Routing
    # struct fields, leftover GlobalVoiceLimit = 24,
    ...
    if re.search(
        ...
        return False
    if "AreResolvedProductionLoopRoutesComplete" in compact:
        return False
    if re.search(r"\bvoid\s+ConfigureRoutingTopology\b", compact):
        return False
    if re.search(r"\bfloat\s+ConfigureRoutingTopology\b", compact):
        return False
    if re.search(r"\bint32\s+ConfigureRoutingTopology\b", compact):
        return False
    if re.search(
        r"\bbool\s+ConfigureRoutingTopology\s*\(\s*[^)]+\s*\)",
        compact,
    ):
        return False
...
def spec_section(header: str) -> str:
    ...
    event = re.search(r"\bEventDefinitions\b", section)
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+ConfigureRoutingTopology\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for bool ConfigureRoutingTopology "
...
class AudioProductionBankConfigureRoutingTopologyDeclContractTests(
    unittest.TestCase
):
    def test_audio_production_bank_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "ConfigureRoutingTopology"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_INITIALIZE_REQUIRED), body)
        self.assertTrue(
            has_identifier(section, SIBLING_INITIALIZE_REQUIRED),
            section,
        )
        self.assertFalse(
            has_identifier(LOCKED_DECL, SIBLING_INITIALIZE_REQUIRED)
        )
        self.assertTrue(has_identifier(body, "HasBoundObject"), body)
        self.assertFalse(has_identifier(section, "HasBoundObject"), section)
    ...
    def test_missing_configure_routing_topology_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_CALLABLE}\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIsNone(
            re.search(r'Category="Skyguard"(?!\|)', UFUNCTION_CALLABLE)
        )
        self.assertIn("BlueprintCallable", UFUNCTION_CALLABLE)
        self.assertIn("Category", UFUNCTION_CALLABLE)
        self.assertIn(
            'Category="Skyguard|Audio|Production"',
            UFUNCTION_CALLABLE,
        )
        self.assertNotIn("BlueprintPure", UFUNCTION_CALLABLE)
        self.assertNotIn("VisibleAnywhere", UFUNCTION_CALLABLE)
        self.assertNotIn("BlueprintReadOnly", UFUNCTION_CALLABLE)
    ...
    def test_configure_routing_topology_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CONST}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_CONST}\n", LOCKED_DECL)
        self.assertIn(
            "ConfigureRoutingTopology",
            str(raised.exception),
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LEFTOVER_DIRECTOR)
        self.assertNotIn("AreResolvedProductionLoopRoutesComplete", LOCKED_DECL)
        self.assertNotIn("const", LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CONST)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTING)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENTRIES)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MASTER_SUBMIX)
        self.assertNotIn("MasterSubmix", LOCKED_DECL)
        self.assertNotIn("CockpitExteriorAttenuation", LOCKED_DECL)
        self.assertNotIn("CockpitLowPassHz", LOCKED_DECL)
        ...
        self.assertNotIn("LockWindowAttackInterval", LOCKED_DECL)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_AUDIO_DIRECTOR_ARE_RESOLVED)
        self.assertNotEqual(
            THIS_SCRIPT,
            LEFTOVER_AUDIO_DIRECTOR_GET_RESOLVED_COUNT,
        )
        ...
        self.assertNotEqual(UFUNCTION_CALLABLE, UFUNCTION_AUDIO_ONLY)
        self.assertIsNone(
            re.search(r'Category\s*=\s*"Skyguard"(?!\|)', UFUNCTION_CALLABLE)
        )
        self.assertIsNone(
            re.search(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool ConfigureRoutingTopologys() const;\n",
            "\tint32 ConfigureRoutingTopology = 1;\n",
            "\tFName ConfigureRoutingTopology = NAME_None;\n",
            "\tbool ConfigureRoutingTopology() const = "
            + forty
            + ";\n",
            "\tbool ConfigureRoutingTopology() const = "
            + eighty
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            f"\t{LOCKED_DECL}\n",
            f"\t{UFUNCTION_CALLABLE}\n\t{LOCKED_DECL}\n",
            f"\t{UFUNCTION_CALLABLE} {LOCKED_DECL}\n",
            "\tUFUNCTION(BlueprintCallable, "
            'Category="Skyguard|Audio|Production")\n'
            f"\t{LOCKED_DECL}\n",
            "\tUFUNCTION(\n\t\tBlueprintCallable, "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "ConfigureRoutingTopology")
        )
        self.assertTrue(
            has_identifier(section, "ConfigureRoutingTopology")
        )
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_INITIALIZE_REQUIRED))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_INITIALIZE_REQUIRED))
        self.assertTrue(has_identifier(leaked, "HasBoundObject"))
        self.assertFalse(has_identifier(section, "HasBoundObject"))
        self.assertFalse(has_identifier(section, SIBLING_ENGINE_IDLE_LOOP))
        self.assertFalse(has_identifier(section, SIBLING_EVENT_DEFINITIONS))
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        ...
        self.assertNotIn("VisibleAnywhere", UFUNCTION_CALLABLE)
        self.assertNotIn("BlueprintReadOnly", UFUNCTION_CALLABLE)
        self.assertNotIn("MultiLine", UFUNCTION_CALLABLE)
        self.assertNotIn("ClampMin", UFUNCTION_CALLABLE)
        self.assertNotIn("ClampMax", UFUNCTION_CALLABLE)
        for invented in INVENTED_DECL_META:
            self.assertNotIn(invented, LOCKED_DECL)
            self.assertNotIn(invented, UFUNCTION_CALLABLE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio production bank configure routing topology "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_production_bank_configure_routing_topology_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioProductionBank.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardAudioDirectorComponent.h", THIS_SCRIPT)
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertNotIn("MasterSubmix", locked_only)
        self.assertNotIn("CockpitSubmix", locked_only)
