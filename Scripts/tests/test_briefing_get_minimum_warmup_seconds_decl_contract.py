...
# or lock the GetMinimumWarmupSeconds body.
...
# leftover mission-briefing-state enum values. Leftover
# briefing-fail-closed #9fe9, leftover briefing-card
# defaults #8c3f, leftover briefing-radio-row defaults
# #bf8e, leftover mission-briefing-state enum, leftover
# radio-chatter empty-fail-closed, leftover
# campaign-roster #111, leftover campaign-save
# empty-fail-closed, and mission-definition field /
# method contracts stay sibling-only.
GET_MINIMUM_WARMUP_SECONDS = "float GetMinimumWarmupSeconds() const;"
UFUNCTION_BRIEFING = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Briefing")'
)
# Leftover #56–#64 plus BriefingComponent production files.
# This lane only adds an isolated Python GetMinimumWarmupSeconds
# declaration contract. Stay off ConfigureFromMission
# (draft #366), SetAssetsReady (draft #368),
# AdvanceBriefing (draft #367), AcknowledgeAndLaunch
# (sibling just pushed), CanLaunch (in-flight),
# GetBriefingState (in-flight), GetElapsedSeconds,
...
# mission-definition field / method contracts including
# FindObjective #364 and ValidateDefinition #365, leftover
...
# leftover ConfigureFromMission, leftover SetAssetsReady,
# leftover AdvanceBriefing, leftover AcknowledgeAndLaunch,
# leftover CanLaunch, leftover GetBriefingState,
# and leftover GetElapsedSeconds stay sibling-only.
LOCKED_SCRIPTS = (
...
# AdvanceBriefing / AcknowledgeAndLaunch / CanLaunch /
# GetBriefingState / GetElapsedSeconds /
...
# .cpp GetMinimumWarmupSeconds body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline `return MinimumWarmupSeconds` is
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionBriefingComponent::GetMinimumWarmupSeconds",
    "SkyguardMissionBriefingComponent.cpp",
)
SIBLING_TYPES = (
...
class BriefingGetMinimumWarmupSecondsDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_MINIMUM_WARMUP_SECONDS),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_MINIMUM_WARMUP_SECONDS)
        self.assertIn("GetMinimumWarmupSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_MINIMUM_WARMUP_SECONDS))

    def test_missing_get_minimum_warmup_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_MINIMUM_WARMUP_SECONDS)
        self.assertIn("GetMinimumWarmupSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_MINIMUM_WARMUP_SECONDS)
        self.assertIn("GetMinimumWarmupSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("UFUNCTION", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("Category", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("BlueprintCallable", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_MINIMUM_WARMUP_SECONDS)
        self.assertIn("GetMinimumWarmupSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tfloat GetMinimumWarmupSeconds();\n"
        wrong_return_void = "\tvoid GetMinimumWarmupSeconds() const;\n"
        wrong_return_bool = "\tbool GetMinimumWarmupSeconds() const;\n"
        added_arg = "\tfloat GetMinimumWarmupSeconds(bool bReady) const;\n"
        wrong_name = "\tfloat GetElapsedSeconds() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_MINIMUM_WARMUP_SECONDS)
            self.assertIn("GetMinimumWarmupSeconds", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_minimum_warmup_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_MINIMUM_WARMUP_SECONDS),
            GET_MINIMUM_WARMUP_SECONDS,
        )
        self.assertTrue(has_declaration(section, GET_MINIMUM_WARMUP_SECONDS))
        self.assertEqual(
            declaration_count(section, GET_MINIMUM_WARMUP_SECONDS),
            1,
        )
        self.assertTrue(
            GET_MINIMUM_WARMUP_SECONDS.startswith("float "),
            GET_MINIMUM_WARMUP_SECONDS,
        )
        self.assertTrue(
            GET_MINIMUM_WARMUP_SECONDS.endswith(";"),
            GET_MINIMUM_WARMUP_SECONDS,
        )
        self.assertIn("GetMinimumWarmupSeconds()", GET_MINIMUM_WARMUP_SECONDS)
        self.assertIn(" const", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("INDEX_NONE", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("{", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("}", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return ", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return MinimumWarmupSeconds", GET_MINIMUM_WARMUP_SECONDS)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_MINIMUM_WARMUP_SECONDS}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_MINIMUM_WARMUP_SECONDS))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_MINIMUM_WARMUP_SECONDS),
            GET_MINIMUM_WARMUP_SECONDS,
        )
        exact_inline = (
            "float GetMinimumWarmupSeconds() const { return MinimumWarmupSeconds; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_MINIMUM_WARMUP_SECONDS))
        self.assertEqual(
            require_declaration(exact_inline, GET_MINIMUM_WARMUP_SECONDS),
            GET_MINIMUM_WARMUP_SECONDS,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_MINIMUM_WARMUP_SECONDS),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tfloat\n"
            "\tGetMinimumWarmupSeconds() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tfloat GetMinimumWarmupSeconds(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        wrap_inline = (
            "public:\n"
            "\tfloat GetMinimumWarmupSeconds()\n"
            "\tconst { return MinimumWarmupSeconds; }\n"
            "};\n"
        )
        header_wrap_type = (
            f"class SKYGUARD52_API {CLASS_NAME} "
        ...
        one_line = f"{{\npublic:\n\t{GET_MINIMUM_WARMUP_SECONDS}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_MINIMUM_WARMUP_SECONDS))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_MINIMUM_WARMUP_SECONDS),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return MinimumWarmupSeconds", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return Other", GET_MINIMUM_WARMUP_SECONDS)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_MINIMUM_WARMUP_SECONDS}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_MINIMUM_WARMUP_SECONDS)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_minimum_warmup_seconds_body(self) -> None:
        locked_only = f"{GET_MINIMUM_WARMUP_SECONDS}\n"
        self.assertNotIn("{", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("}", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return ", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return MinimumWarmupSeconds", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return MinimumWarmupSeconds", locked_only)
        self.assertNotIn(
            "USkyguardMissionBriefingComponent::GetMinimumWarmupSeconds",
            GET_MINIMUM_WARMUP_SECONDS,
        )
        ...
        self.assertNotIn("return false", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("return true", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("ConfigureFromMission", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_set_assets_ready(self) -> None:
        ...
        for neighbor in SET_ASSETS_READY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("SetAssetsReady", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("SetAssetsReady", locked_only)

    def test_contract_does_not_relock_advance_briefing(self) -> None:
        locked_only = f"{GET_MINIMUM_WARMUP_SECONDS}\n"
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("AdvanceBriefing", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_acknowledge_and_launch(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_AND_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("AcknowledgeAndLaunch", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_can_launch(self) -> None:
        ...
        for neighbor in CAN_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("CanLaunch", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_get_briefing_state(self) -> None:
        ...
        for neighbor in GET_BRIEFING_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("GetBriefingState", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_SECONDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("GetElapsedSeconds", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("GetBriefingText", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_get_radio_chatter(self) -> None:
        ...
        for neighbor in GET_RADIO_CHATTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("GetRadioChatter", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("Unconfigured", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("Warming", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("Launched", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_MINIMUM_WARMUP_SECONDS,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("FSkyguardBriefingCard", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("FillResultCombatStats", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("ASkyguardGunner", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("FillAndFinalize", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("FillAndFail", GET_MINIMUM_WARMUP_SECONDS)
        self.assertNotIn("ApplyHydraForClusters", GET_MINIMUM_WARMUP_SECONDS)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_MINIMUM_WARMUP_SECONDS}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_MINIMUM_WARMUP_SECONDS),
            GET_MINIMUM_WARMUP_SECONDS,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing GetMinimumWarmupSeconds contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_MINIMUM_WARMUP_SECONDS.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing GetMinimumWarmupSeconds contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_MINIMUM_WARMUP_SECONDS)

    def test_contract_is_get_minimum_warmup_seconds_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_MINIMUM_WARMUP_SECONDS.startswith("float "))
        self.assertTrue(GET_MINIMUM_WARMUP_SECONDS.endswith(";"))
        self.assertIn(" const", GET_MINIMUM_WARMUP_SECONDS)
        self.assertIn(UFUNCTION_BRIEFING, section)
