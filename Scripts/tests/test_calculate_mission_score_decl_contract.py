...
# score value, medal tier, or mission-result payload, or
# lock the CalculateMissionScore body in the .cpp.
...
# CalculateMissionScore declaration contract. Stay off
# CalculateMedalTier (unused sibling for a later worker),
# GetMissionRecords / GetRouteRuntime (in-flight this wave),
# CompleteActiveMission / FinalizeActiveMission /
# FailActiveMission, FillResultCombatStats /
# FillAndFinalize / FillAndFail / ApplyHydraForClusters
# (leftover ASkyguardGunner*), leftover CPG debrief
...
# defaults #deae, leftover mission-result defaults #c4db,
...
# Isolated-test drafts stay off this lane. CalculateMedalTier,
...
# and newly drafted campaign-subsystem siblings stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# CalculateMedalTier is an unused sibling for a later worker.
# GetMissionRecords / GetRouteRuntime are in-flight this wave.
# CompleteActiveMission / FinalizeActiveMission /
# FailActiveMission stay sibling-only.
...
CALCULATE_MEDAL_TIER_NOT_LOCKED = (
    "static int32 CalculateMedalTier(",
)
...
# Do not lock production score-rule field defaults.
LEFTOVER_SCORE_RULES_DEFAULTS_NOT_LOCKED = (
    "CompletionScore",
...
LEFTOVER_MISSION_RESULT_DEFAULTS_NOT_LOCKED = (
    "bMissionSucceeded",
    "ShotsFired",
...
# apply / leftover FillAndFinalize / FillAndFail / leftover
# settings invert-look stay unlocked.
LEFTOVER_NOT_LOCKED = (
...
# Invented score value / medal tier / mission-result payload
# stay unlocked. Do not invent INDEX_NONE or lock the cpp
# body. Do not invent UFUNCTION.
INVENTED_SCORE = (
    "return 0",
    "CompletionScore",
    "PerfectAccuracyBonus",
...
# .cpp CalculateMissionScore body / invented return values
# stay unlocked. Do not invent INDEX_NONE or lock the cpp
# body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
...
class CalculateMissionScoreDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CALCULATE_MISSION_SCORE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CALCULATE_MISSION_SCORE)
        self.assertIn("CalculateMissionScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CALCULATE_MISSION_SCORE))

    def test_missing_score_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CALCULATE_MISSION_SCORE)
        self.assertIn("CalculateMissionScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CALCULATE_MISSION_SCORE)
        self.assertIn("CalculateMissionScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        for token in INVENTED_UFUNCTION:
            self.assertNotIn(token, CALCULATE_MISSION_SCORE)
            self.assertNotIn(token, locked_only)
        self.assertFalse(
            CALCULATE_MISSION_SCORE.startswith("UFUNCTION"),
            CALCULATE_MISSION_SCORE,
        )
    ...
    def test_neighbor_getters_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_getters, CALCULATE_MISSION_SCORE)
        self.assertIn("CalculateMissionScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_static = (
            "\tint32 CalculateMissionScore("
            "const USkyguardMissionDefinition& Mission,"
            "const FSkyguardMissionResult& Result);\n"
        )
        wrong_return = (
            "\tstatic float CalculateMissionScore("
            "const USkyguardMissionDefinition& Mission,"
            "const FSkyguardMissionResult& Result);\n"
        )
        missing_mission = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CALCULATE_MISSION_SCORE)
            self.assertIn("CalculateMissionScore", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_score_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CALCULATE_MISSION_SCORE),
            CALCULATE_MISSION_SCORE,
        )
        self.assertTrue(has_declaration(section, CALCULATE_MISSION_SCORE))
        self.assertEqual(
            declaration_count(section, CALCULATE_MISSION_SCORE),
            1,
        )
        self.assertTrue(
            CALCULATE_MISSION_SCORE.startswith("static int32 "),
            CALCULATE_MISSION_SCORE,
        )
        self.assertIn(
            "const USkyguardMissionDefinition& Mission",
        ...
        self.assertTrue(CALCULATE_MISSION_SCORE.endswith(");"))
        self.assertNotIn("INDEX_NONE", CALCULATE_MISSION_SCORE)
        self.assertNotIn("{", CALCULATE_MISSION_SCORE)
        self.assertNotIn("}", CALCULATE_MISSION_SCORE)
        self.assertNotIn("return ", CALCULATE_MISSION_SCORE)
        self.assertNotIn("UFUNCTION", CALCULATE_MISSION_SCORE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{CALCULATE_MISSION_SCORE}\n}}\n"
        self.assertTrue(has_declaration(one_line, CALCULATE_MISSION_SCORE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CALCULATE_MISSION_SCORE),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CALCULATE_MISSION_SCORE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_declaration_does_not_invent_score_value(self) -> None:
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        for token in INVENTED_SCORE:
            self.assertNotIn(token, CALCULATE_MISSION_SCORE)
            self.assertNotIn(token, locked_only)
    ...
    def test_declaration_does_not_invent_medal_tier(self) -> None:
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        for token in INVENTED_MEDAL:
            self.assertNotIn(token, CALCULATE_MISSION_SCORE)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("CalculateMedalTier", CALCULATE_MISSION_SCORE)
        self.assertNotIn("CalculateMedalTier", locked_only)
        self.assertNotIn("GoldThreshold", CALCULATE_MISSION_SCORE)
        self.assertNotIn("MedalTier = 0", CALCULATE_MISSION_SCORE)

    def test_declaration_does_not_invent_mission_result_payload(self) -> None:
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        for token in LEFTOVER_MISSION_RESULT_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, CALCULATE_MISSION_SCORE)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("bMissionSucceeded", CALCULATE_MISSION_SCORE)
        self.assertNotIn("FinalScore", CALCULATE_MISSION_SCORE)
        self.assertNotIn("ShotsFired", CALCULATE_MISSION_SCORE)
        self.assertNotIn("CompletedObjectiveIds", CALCULATE_MISSION_SCORE)

    def test_contract_does_not_lock_score_cpp_body(self) -> None:
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        self.assertNotIn("{", CALCULATE_MISSION_SCORE)
        self.assertNotIn("}", CALCULATE_MISSION_SCORE)
        self.assertNotIn("return ", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_calculate_medal_tier(self) -> None:
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        for neighbor in CALCULATE_MEDAL_TIER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("CalculateMedalTier", CALCULATE_MISSION_SCORE)
        self.assertNotIn("CalculateMedalTier", locked_only)
        self.assertNotIn("FSkyguardMissionScoreRules", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_leftover_score_rules_defaults(
        ...
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        section = public_section(origin_main_header())
        for token in LEFTOVER_SCORE_RULES_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("GetActiveMissionElapsedSeconds", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_configure_can_start_or_start(self) -> None:
        ...
        for neighbor in CONFIGURE_CAN_START_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("ConfigureCampaign", CALCULATE_MISSION_SCORE)
        self.assertNotIn("CanStartMission", CALCULATE_MISSION_SCORE)
        self.assertNotIn("StartMission", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_is_mission_unlocked(self) -> None:
        ...
        for neighbor in IS_MISSION_UNLOCKED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("IsMissionUnlocked", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("AddObjectiveProgress", CALCULATE_MISSION_SCORE)
        self.assertNotIn("FailObjective", CALCULATE_MISSION_SCORE)
        self.assertNotIn("CompleteSurviveObjectiveIfIntact", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("CompleteActiveMission", CALCULATE_MISSION_SCORE)
        self.assertNotIn("FinalizeActiveMission", CALCULATE_MISSION_SCORE)
        self.assertNotIn("FailActiveMission", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("FillResultCombatStats", CALCULATE_MISSION_SCORE)
        self.assertNotIn("ASkyguardGunner", CALCULATE_MISSION_SCORE)
        self.assertNotIn("FillAndFinalize", CALCULATE_MISSION_SCORE)
        self.assertNotIn("FillAndFail", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("AcknowledgeDebrief", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_debrief_neighbors(self) -> None:
        ...
        for neighbor in DEBRIEF_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("RetrySaveLastDebrief", CALCULATE_MISSION_SCORE)
        self.assertNotIn("GetLastDebrief", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_travel_helpers(self) -> None:
        ...
        for neighbor in TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("CanTravelToNextMission", CALCULATE_MISSION_SCORE)
        self.assertNotIn("GetNextMissionMapPackageName", CALCULATE_MISSION_SCORE)
        self.assertNotIn("TravelToNextMission", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_runtime_getters(self) -> None:
        ...
        for neighbor in RUNTIME_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("GetObjectiveRuntime", CALCULATE_MISSION_SCORE)
        self.assertNotIn("GetRouteRuntime", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_get_mission_records(self) -> None:
        ...
        for neighbor in GET_MISSION_RECORDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("GetMissionRecords", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_save_game_helpers(self) -> None:
        ...
        for neighbor in SAVE_GAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MISSION_SCORE)
        self.assertNotIn("ApplySaveGame", CALCULATE_MISSION_SCORE)
        self.assertNotIn("BuildSaveGame", CALCULATE_MISSION_SCORE)
        self.assertNotIn("SaveCampaignToSlot", CALCULATE_MISSION_SCORE)
        self.assertNotIn("LoadCampaignFromSlot", CALCULATE_MISSION_SCORE)
        self.assertNotIn("DeleteCampaignSlot", CALCULATE_MISSION_SCORE)
        self.assertNotIn("IsValidCampaignSlotName", CALCULATE_MISSION_SCORE)
        self.assertNotIn("GetEarnedCampaignMedals", CALCULATE_MISSION_SCORE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CALCULATE_MISSION_SCORE}\n"
        self.assertEqual(
            require_declaration(locked_only, CALCULATE_MISSION_SCORE),
            CALCULATE_MISSION_SCORE,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(CALCULATE_MISSION_SCORE, "Rifle")
        self.assertNotEqual(CALCULATE_MISSION_SCORE, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign CalculateMissionScore contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, CALCULATE_MISSION_SCORE.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", CALCULATE_MISSION_SCORE)

    def test_contract_is_score_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
