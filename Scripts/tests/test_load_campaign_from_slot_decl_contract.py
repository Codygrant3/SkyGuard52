...
# or lock the LoadCampaignFromSlot body in the .cpp.
...
# LoadCampaignFromSlot declaration contract. Stay
# off SaveCampaignToSlot / DeleteCampaignSlot /
...
# SaveCampaignToSlot / DeleteCampaignSlot /
...
# .cpp LoadCampaignFromSlot body / invented return values /
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return 0",
    "USkyguardCampaignSubsystem::LoadCampaignFromSlot",
    "SkyguardCampaignSubsystem.cpp",
    "SaveGameToSlot",
    "LoadGameFromSlot",
    "DoesSaveGameExist",
...
class LoadCampaignFromSlotDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, LOAD_CAMPAIGN), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOAD_CAMPAIGN)
        self.assertIn("LoadCampaignFromSlot", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, LOAD_CAMPAIGN))

    def test_missing_load_campaign_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, LOAD_CAMPAIGN)
        self.assertIn("LoadCampaignFromSlot", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, LOAD_CAMPAIGN)
        self.assertIn("LoadCampaignFromSlot", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, LOAD_CAMPAIGN)
        self.assertIn("LoadCampaignFromSlot", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, LOAD_CAMPAIGN)
            self.assertIn("LoadCampaignFromSlot", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_save_or_delete_slot_does_not_satisfy(self) -> None:
        save_only = (
            "\tbool SaveCampaignToSlot(\n"
            '\t\tconst FString& SlotName = TEXT("Skyguard52Campaign"),\n'
            "\t\tint32 UserIndex = 0) const;\n"
        ...
        for region in (save_only, delete_only):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, LOAD_CAMPAIGN)
            self.assertIn("LoadCampaignFromSlot", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, LOAD_CAMPAIGN))
    ...
    def test_retry_save_last_debrief_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(retry_only, LOAD_CAMPAIGN)
        self.assertIn("LoadCampaignFromSlot", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(retry_only, LOAD_CAMPAIGN))

    def test_load_campaign_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOAD_CAMPAIGN),
            LOAD_CAMPAIGN,
        )
        self.assertTrue(has_declaration(section, LOAD_CAMPAIGN))
        self.assertEqual(declaration_count(section, LOAD_CAMPAIGN), 1)
        self.assertTrue(LOAD_CAMPAIGN.startswith("bool "), LOAD_CAMPAIGN)
        self.assertTrue(LOAD_CAMPAIGN.endswith(");"), LOAD_CAMPAIGN)
        self.assertFalse(LOAD_CAMPAIGN.endswith(") const;"), LOAD_CAMPAIGN)
        self.assertIn(
            'const FString& SlotName = TEXT("Skyguard52Campaign")',
            LOAD_CAMPAIGN,
        )
        self.assertIn("int32 UserIndex = 0", LOAD_CAMPAIGN)
        self.assertNotIn("INDEX_NONE", LOAD_CAMPAIGN)
        self.assertNotIn("{", LOAD_CAMPAIGN)
        self.assertNotIn("}", LOAD_CAMPAIGN)
        self.assertNotIn("return ", LOAD_CAMPAIGN)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_args = (
            "public:\n"
            "\tbool LoadCampaignFromSlot(const FString&\n"
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, LOAD_CAMPAIGN), 1)
        one_line = f"{{\npublic:\n\t{LOAD_CAMPAIGN}\n}}\n"
        self.assertTrue(has_declaration(one_line, LOAD_CAMPAIGN))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, LOAD_CAMPAIGN), section)
        self.assertEqual(
            require_declaration(section, LOAD_CAMPAIGN),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{LOAD_CAMPAIGN}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", LOAD_CAMPAIGN)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", LOAD_CAMPAIGN)
        self.assertNotIn("UserIndex = INDEX_NONE", LOAD_CAMPAIGN)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_save_payload_or_slot_contents(
        ...
        locked_only = f"{LOAD_CAMPAIGN}\n"
        self.assertNotIn("return ", LOAD_CAMPAIGN)
        self.assertNotIn("return true", LOAD_CAMPAIGN)
        self.assertNotIn("return false", LOAD_CAMPAIGN)
        for token in SAVE_PAYLOADS_AND_SLOT_NOT_LOCKED:
            self.assertNotIn(token, LOAD_CAMPAIGN)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("SaveGameToSlot", LOAD_CAMPAIGN)
        self.assertNotIn("LoadGameFromSlot", LOAD_CAMPAIGN)
        self.assertNotIn("DoesSaveGameExist", LOAD_CAMPAIGN)
        self.assertNotIn("DuplicateObject", LOAD_CAMPAIGN)
        self.assertNotIn("MigrateCampaignSave", LOAD_CAMPAIGN)
        self.assertNotIn("CurrentSaveVersion", LOAD_CAMPAIGN)
        self.assertNotIn("BestScore", LOAD_CAMPAIGN)
        self.assertNotIn("BestMedalTier", LOAD_CAMPAIGN)
        self.assertNotIn("LastDebrief.bProgressSaved", LOAD_CAMPAIGN)

    def test_contract_does_not_lock_load_campaign_cpp_body(self) -> None:
        locked_only = f"{LOAD_CAMPAIGN}\n"
        self.assertNotIn("{", LOAD_CAMPAIGN)
        self.assertNotIn("}", LOAD_CAMPAIGN)
        self.assertNotIn("return ", LOAD_CAMPAIGN)
        ...
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", LOAD_CAMPAIGN)
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", locked_only)
        self.assertNotIn("SaveGameToSlot", LOAD_CAMPAIGN)
        self.assertNotIn("LoadGameFromSlot", LOAD_CAMPAIGN)
        self.assertNotIn("DoesSaveGameExist", LOAD_CAMPAIGN)
        self.assertNotIn("ClearActiveMissionRuntime", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_configure_can_start_or_start(self) -> None:
        ...
        for neighbor in CONFIGURE_CAN_START_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("ConfigureCampaign", LOAD_CAMPAIGN)
        self.assertNotIn("CanStartMission", LOAD_CAMPAIGN)
        self.assertNotIn("StartMission", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_is_mission_unlocked(self) -> None:
        ...
        for neighbor in IS_MISSION_UNLOCKED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("IsMissionUnlocked", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("AddObjectiveProgress", LOAD_CAMPAIGN)
        self.assertNotIn("FailObjective", LOAD_CAMPAIGN)
        self.assertNotIn("CompleteSurviveObjectiveIfIntact", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("CompleteActiveMission", LOAD_CAMPAIGN)
        self.assertNotIn("FinalizeActiveMission", LOAD_CAMPAIGN)
        self.assertNotIn("FailActiveMission", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("FillResultCombatStats", LOAD_CAMPAIGN)
        self.assertNotIn("ASkyguardGunner", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_fill_and_finalize_or_fail(self) -> None:
        ...
        for token in FILL_AND_NOT_LOCKED:
            ...
            self.assertNotIn(token, LOAD_CAMPAIGN)
            self.assertNotIn(token, section)
        self.assertNotIn("FillAndFinalize", LOAD_CAMPAIGN)
        self.assertNotIn("FillAndFail", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_elapsed_seconds(self) -> None:
        ...
        for neighbor in ELAPSED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("GetActiveMissionElapsedSeconds", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("AcknowledgeDebrief", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_debrief_neighbors(self) -> None:
        ...
        for neighbor in DEBRIEF_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("RetrySaveLastDebrief", LOAD_CAMPAIGN)
        self.assertNotIn("GetLastDebrief", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_travel_helpers(self) -> None:
        ...
        for neighbor in TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("CanTravelToNextMission", LOAD_CAMPAIGN)
        self.assertNotIn("GetNextMissionMapPackageName", LOAD_CAMPAIGN)
        self.assertNotIn("TravelToNextMission", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_runtime_getters(self) -> None:
        ...
        for neighbor in RUNTIME_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("GetObjectiveRuntime", LOAD_CAMPAIGN)
        self.assertNotIn("GetRouteRuntime", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_save_game_siblings(self) -> None:
        ...
        for neighbor in SAVE_GAME_SIBLINGS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("ApplySaveGame", LOAD_CAMPAIGN)
        self.assertNotIn("BuildSaveGame", LOAD_CAMPAIGN)
        self.assertNotIn("SaveCampaignToSlot", LOAD_CAMPAIGN)
        self.assertNotIn("DeleteCampaignSlot", LOAD_CAMPAIGN)
        self.assertNotIn("RetrySaveLastDebrief", LOAD_CAMPAIGN)
        self.assertNotIn("GetMissionRecords", LOAD_CAMPAIGN)
        self.assertNotIn("IsValidCampaignSlotName", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_score_or_medal_helpers(self) -> None:
        ...
        for neighbor in SCORE_MEDAL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LOAD_CAMPAIGN)
        self.assertNotIn("CalculateMissionScore", LOAD_CAMPAIGN)
        self.assertNotIn("CalculateMedalTier", LOAD_CAMPAIGN)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{LOAD_CAMPAIGN}\n"
        self.assertEqual(
            require_declaration(locked_only, LOAD_CAMPAIGN),
            LOAD_CAMPAIGN,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(LOAD_CAMPAIGN, "Rifle")
        self.assertNotEqual(LOAD_CAMPAIGN, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign LoadCampaignFromSlot contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, LOAD_CAMPAIGN.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", LOAD_CAMPAIGN)

    def test_contract_is_load_campaign_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
