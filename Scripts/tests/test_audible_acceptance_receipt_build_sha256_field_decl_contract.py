...
# #1cd7 then isolated field decl.
# Sibling decls stay unclaimed: EvidenceSha256 / SampleCount /
# PeakActiveVoices / TotalUnderruns / MaximumAudioThreadMs /
# MaximumTruePeakDbTP / bPackagedDevelopmentBuild /
# bAudibleDeviceObserved / bCalibratedMetering /
# bProductionBankReady / bAccepted.
# Identifier bounds: match EXACT `BuildSha256` as a whole
# field name after `FString`. Do NOT accept `EvidenceSha256`.
...
# If a clone asserts int32 / required `= 0` / parse window
# FSkyguardProductionAudioAudit, RETARGET: parse
# FSkyguardAudibleAcceptanceReceipt only, type is FString,
# BARE decl with NO initializer, UPROPERTY is
...
TARGET = "FString BuildSha256;"
TARGET_WRONG_EQ_EMPTY = 'FString BuildSha256 = TEXT("");'
TARGET_WRONG_EQ_QUOTED = 'FString BuildSha256 = "";'
TARGET_WRONG_EQ_NONE = "FString BuildSha256 = NAME_None;"
TARGET_WRONG_TRUE = "FString BuildSha256 = true;"
TARGET_WRONG_FALSE = "FString BuildSha256 = false;"
TARGET_WRONG_ZERO = "FString BuildSha256 = 0;"
TARGET_WRONG_INT = "int32 BuildSha256 = 0;"
TARGET_WRONG_FLOAT = "float BuildSha256;"
TARGET_WRONG_FNAME = "FName BuildSha256;"
TARGET_WRONG_FTEXT = "FText BuildSha256;"
TARGET_WRONG_BOOL = "bool BuildSha256 = false;"
TARGET_WRONG_TARRAY = "TArray<FName> BuildSha256;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> BuildSha256;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EVIDENCE = "FString EvidenceSha256;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString BuildSha256;`.
    # Match the EXACT identifier BuildSha256 as a whole field
    # name after FString. Do NOT accept EvidenceSha256.
    # Reject initializer, Harbor 40/80, wrong types.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"\bEvidenceSha256\b", compact) and re.search(
        r"FString\s+BuildSha256\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        ...
        return False
    if re.search(
        r"float\s+BuildSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+BuildSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+BuildSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BuildSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+BuildSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+BuildSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+BuildSha256\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString\s+BuildSha256\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString BuildSha256 is missing from "
...
class AudibleAcceptanceReceiptBuildSha256FieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_build_sha256_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_evidence_sha256_does_not_satisfy_build_sha256(self) -> None:
        evidence_only = f"\t{TARGET_WRONG_EVIDENCE}\n"
        self.assertFalse(has_declaration(evidence_only, LOCKED_DECL))
        self.assertFalse(has_identifier(evidence_only, FIELD_NAME))
        self.assertTrue(
            has_identifier(evidence_only, SIBLING_EVIDENCE_SHA256)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(evidence_only, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        mixed = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_added_initializer_fails_closed(self) -> None:
        assigned = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
        ...
        quoted = (
            ...
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{TARGET_WRONG_EQ_QUOTED}\n"
            "};\n"
        )
        quoted_section = spec_section(quoted)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(quoted_section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("FString BuildSha256;", compact_origin)
        self.assertNotIn("BuildSha256 =", compact_origin)
        self.assertNotIn("BuildSha256 = true", compact_origin)
        self.assertNotIn("BuildSha256 = 0", compact_origin)
        self.assertNotIn("int32 BuildSha256", compact_origin)

    def test_build_sha256_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString BuildSha256"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn(SIBLING_EVIDENCE_SHA256, LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tint32 {SIBLING_SAMPLE_COUNT} = 0;\n",
            f"\tbool {SIBLING_ACCEPTED} = false;\n",
            "\tFString BuildSha256s;\n",
            "\tFString BuildSha256 = 1;\n",
            "\tbool BuildSha256;\n",
            "\tfloat BuildSha256 = " + forty + ";\n",
            "\tfloat BuildSha256 = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt BuildSha256 field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "build_sha256_field_decl_contract.py"
        ))
        self.assertIn(
