# THIS IS leftover-safe FSkyguardTheaterKitSpec RoadTreatment.
...
# then BARE `FName RoadTreatment;` (no initializer).
...
# LampColor / LampIntensity / RoadTint / NamedLandmark /
...
TARGET = "FName RoadTreatment;"
TARGET_WRONG_EQ_NONE = "FName RoadTreatment = NAME_None;"
TARGET_WRONG_FALSE = "FName RoadTreatment = false;"
TARGET_WRONG_TRUE = "FName RoadTreatment = true;"
TARGET_WRONG_ZERO = "FName RoadTreatment = 0.f;"
TARGET_WRONG_FLOAT = "float RoadTreatment;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName RoadTreatment;`.
    ...
    if re.search(
        r"FName\s+RoadTreatment\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+RoadTreatment\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+RoadTreatment\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName RoadTreatment is missing from "
...
class TheaterKitSpecRoadTreatmentFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RoadTreatment"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RoadTreatment"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("RoadTreatment", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_road_treatment_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName RoadTreatment;", compact_origin)
        self.assertNotIn("RoadTreatment = NAME_None", compact_origin)
        self.assertNotIn("RoadTreatment = false", compact_origin)
        self.assertNotIn("RoadTreatment = true", compact_origin)
        self.assertNotIn("RoadTreatment = 0.f", compact_origin)
        self.assertNotIn("RoadTreatment = 160.f", compact_origin)

    def test_road_treatment_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName RoadTreatment"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFName {SIBLING_LAMP_TREATMENT};\n",
            "\tFName RoadTreatments;\n",
            "\tint32 RoadTreatment;\n",
            "\tbool RoadTreatment;\n",
            "\tfloat RoadTreatment = " + forty + ";\n",
            "\tfloat RoadTreatment = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec RoadTreatment field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "road_treatment_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_THEATER_KIT_SPEC_LANDMARK_SET, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_SPEC_WEATHER_IDENTITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_SPEC_KIT_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_SPEC_LAMP_TREATMENT, LOCKED_SCRIPTS)
