# THIS IS leftover-safe ASkyguardGunner CpgTedacBezel.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgTedacBezel;`
# (no initializer).
...
# Stop BEFORE sibling CpgReticleH as a claimed slot.
...
# CpgReticleH, CpgTedacText, Boom, GunnerCamera.
# Do NOT claim leftover CpgTedac / CpgDash / CpgCanopyBow /
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgCockpit /
# CpgStation (in-flight leftovers). CpgMpdLeft and
# CpgMpdRight are leftover once drafted this pass.
# Do NOT claim CpgTedac, CpgTedacText, CpgEufd, or
# CpgReticleH as this slot.
# CpgTedacBezel fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgTedacBezel;`
...
# CpgTedacBezel fail-closed if LOCKED_DECL is leftover
# CpgTedac / CpgDash / CpgKneeRight / CpgKneeLeft /
# CpgSeatBack / CpgCockpit / CpgStation / CpgSeatPan /
# CpgCanopyBow / CpgMpdLeft / CpgMpdRight / CpgEufd /
# CpgReticleH / leftover Apache Canopy / leftover
# Apache PilotCanopy.
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgTedacBezel;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgTedacBezel = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgTedacBezel = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgTedacBezel = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgTedacBezel;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgTedacBezel;"
...
CPG_TEDAC_BEZEL_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgTedacBezel\s*;"
)
CPG_TEDAC_BEZEL_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgTedacBezel\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgDash / CpgKneeRight / CpgCanopyBow /
    # CpgSeatBack / CpgSeatPan / CpgCockpit / CpgStation /
    # leftover Apache Canopy, or an initializer on
    # CpgTedac as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_TEDAC_BEZEL_DECL_RE.search(compact) is None:
        return False
    if CPG_TEDAC_BEZEL_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_RETICLE_H):
            break
    ...
    if has_identifier(window, SIBLING_CPG_RETICLE_H):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_RETICLE_H}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgTedacBezel\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgTedacBezel "
...
class GunnerCpgTedacBezelFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgTedacBezel"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_RETICLE_H), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_RETICLE_H),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgTedacBezel"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgTedacBezel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_tedac_bezel_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgTedacBezel;", compact_origin)
        self.assertNotIn("CpgTedacBezel = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_tedac_bezel_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgTedacBezel"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgTedacBezel;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgTedacBezels;\n",
            "\tint32 CpgTedacBezel = 140;\n",
            "\tbool CpgTedacBezel = true;\n",
            "\tfloat CpgTedacBezel = " + forty + ";\n",
            "\tfloat CpgTedacBezel = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_reticle_h_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_CPG_TEDAC))
        self.assertTrue(has_identifier(leaked, "CpgTedacBezel"))
        self.assertTrue(has_identifier(section, "CpgTedacBezel"))
        self.assertFalse(has_identifier(section, SIBLING_CPG_RETICLE_H))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgTedacBezel field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_tedac_bezel_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(TARGET_WRONG_TEDAC, locked_only)
