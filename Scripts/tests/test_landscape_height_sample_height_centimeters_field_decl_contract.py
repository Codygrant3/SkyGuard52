...
# THIS IS leftover-safe FSkyguardLandscapeHeightSample
# HeightCentimeters FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float HeightCentimeters = 0.f; one-line /
...
# (after struct FSkyguardLandscapeHeightSample, before
# struct FSkyguardLandscapeFootprintSampleResult). Do NOT
...
STRUCT_NAME = "FSkyguardLandscapeHeightSample"
...
# THIS IS leftover-safe FSkyguardLandscapeHeightSample
# HeightCentimeters FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# float HeightCentimeters = 0.f; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeHeightSample HeightCentimeters
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float HeightCentimeters = 0.f;
...
# FSkyguardLandscapeHeightSample, before
...
# leftover-safe FSkyguardLandscapeHeightSample
# HeightCentimeters FIELD (declaration-only). It is
...
TARGET = "float HeightCentimeters = 0.f;"
TARGET_WRONG = "float HeightCentimeters = 1.f;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*float\s+HeightCentimeters\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for float HeightCentimeters is missing from "
...
class LandscapeHeightSampleHeightCentimetersFieldDeclContractTests(unittest.TestCase):
    def test_landscape_height_sample_struct_exists(self) -> None:
        ...
        self.assertIn("QueryLocation", section)
        self.assertIn("bValid", section)
        self.assertIn("HeightCentimeters", section)
        self.assertIn("HeightfieldSource", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("HeightCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_height_centimeters_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_height_centimeters_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float HeightCentimeters"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("HeightCentimeters", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("HeightCentimeters", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float HeightCentimeters"))
    ...
    def test_parser_does_not_land_on_anchors_or_director(self) -> None:
        ...
        match = CLASS_RE.search(header)
        self.assertIsNotNone(match, header)
        self.assertIn("FSkyguardLandscapeHeightSample", match.group(0))
