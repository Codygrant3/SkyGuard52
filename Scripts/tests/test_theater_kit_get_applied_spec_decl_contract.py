# THIS IS leftover-safe ASkyguardCampaignTheaterKit::GetAppliedSpec.
...
# THIS IS UFUNCTION(BlueprintPure) ONLY. It is NOT
# BlueprintCallable. Category IS `Skyguard|Theater`.
# Fail-closed if the UFUNCTION or signature is missing
# or renamed, if Category is missing, if BlueprintCallable
# replaces BlueprintPure, if const is missing on the
# method, if const is missing on the return reference,
# or if the return type is not
# `const FSkyguardTheaterKitSpec&`.
# Accept one-line and split-line UFUNCTION wraps.
# Accept the origin/main inline body without locking
# the body, plus a trailing-semicolon form.
# Parse class `ASkyguardCampaignTheaterKit` public section
# ONLY. Stop at `private:`.
# Do NOT parse `struct FSkyguardTheaterKitSpec` body
# (leftover isolated spec fields #1299-#1314).
...
# GetLampInstanceCount / GetSilhouetteInstanceCount /
# ApplyTheaterKit stay uncontracted.
# Public UPROPERTY fields stay uncontracted, including
# leftover AppliedSpec field #1321.
...
# in LOCKED_SCRIPTS; isolated method decl does not relock
# the bulk).
# THIS IS NOT leftover TheaterKitSpec field drafts
# #1299-#1314 (keep those files in LOCKED_SCRIPTS).
# THIS IS NOT leftover actor UPROPERTY #1315-#1321
# (BuildingInstances / LampInstances / RoadInstances /
# Root / NamedLandmarkMesh / SilhouetteInstances /
# AppliedSpec; keep those files in LOCKED_SCRIPTS).
# THIS IS NOT leftover actor UFUNCTION
# GetAppliedWeatherIdentity / GetAppliedKitId /
# GetBuildingTint (keep those files in LOCKED_SCRIPTS).
...
# CardId field decls, then leftover actor AppliedSpec
# UPROPERTY #1321 then leftover GetAppliedWeatherIdentity /
# GetAppliedKitId UFUNCTIONs, then this isolated
# GetAppliedSpec UFUNCTION.
# If a clone asserts UPROPERTY / spec struct /
# Category="Skyguard|Campaign" / FName WeatherIdentity /
# FName GetAppliedKitId / float / `= 160.f`, retarget:
# this is UFUNCTION(BlueprintPure,
...
TARGET = "const FSkyguardTheaterKitSpec& GetAppliedSpec() const;"
TARGET_INLINE = (
    "const FSkyguardTheaterKitSpec& GetAppliedSpec() const "
    "{ return AppliedSpec; }"
)
TARGET_WRONG_BARE = "const FSkyguardTheaterKitSpec& GetAppliedSpec();"
TARGET_WRONG_NO_CONST = (
    "const FSkyguardTheaterKitSpec& GetAppliedSpec() "
    "{ return AppliedSpec; }"
)
TARGET_WRONG_NO_RETURN_CONST = (
    "FSkyguardTheaterKitSpec& GetAppliedSpec() const;"
)
TARGET_WRONG_BY_VALUE = "FSkyguardTheaterKitSpec GetAppliedSpec() const;"
TARGET_WRONG_VOID = "void GetAppliedSpec() const;"
TARGET_WRONG_FLOAT = "float GetAppliedSpec() const;"
TARGET_WRONG_BOOL = "bool GetAppliedSpec() const;"
TARGET_WRONG_FNAME = "FName GetAppliedSpec() const;"
TARGET_WRONG_RENAME = "const FSkyguardTheaterKitSpec& GetApplied() const;"
TARGET_WRONG_SIBLING_WEATHER = (
    "FName GetAppliedWeatherIdentity() const;"
)
TARGET_WRONG_SIBLING_KIT = "FName GetAppliedKitId() const;"
TARGET_WRONG_SIBLING_LANDMARK = "FName GetNamedLandmark() const;"
TARGET_WRONG_SIBLING_TINT = "FLinearColor GetBuildingTint() const;"
TARGET_WRONG_FIELD = "FSkyguardTheaterKitSpec AppliedSpec;"
...
GET_APPLIED_SPEC_SIGNATURE_RE = re.compile(
    r"\bconst\s+FSkyguardTheaterKitSpec\s*&\s+"
    r"GetAppliedSpec\s*\(\s*\)\s*const\s*(?:;|\{)"
)
INVENTED_UFUNCTION = (
    "BlueprintCallable",
    "VisibleAnywhere",
...
def sibling_names_not_in_locked_decl() -> tuple[str, ...]:
    # GetAppliedSpec contains the AppliedSpec stem; do not treat
    # that substring as contracting leftover AppliedSpec field
    # #1321. ApplyTheaterKit is also a prefix of leftover
    # ApplyTheaterKitToWorld, but that token is not in LOCKED_DECL.
    return tuple(
        name
...
def leftover_spec_names_not_in_locked_decl() -> tuple[str, ...]:
    return leftover_spec_field_names()
...
def leftover_theater_kit_actor_field_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_THEATER_KIT_ROOT,
        LEFTOVER_THEATER_KIT_ROAD_INSTANCES,
        LEFTOVER_THEATER_KIT_BUILDING_INSTANCES,
...
def leftover_theater_kit_actor_ufunction_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_THEATER_KIT_GET_APPLIED_WEATHER_IDENTITY,
        LEFTOVER_THEATER_KIT_GET_APPLIED_KIT_ID,
        LEFTOVER_THEATER_KIT_GET_BUILDING_TINT,
...
def leftover_ufunction_tokens() -> tuple[str, ...]:
    ...
    # GetAppliedSpec is omitted on purpose.
    return (
        STOP_BEFORE_APPLY,
        "ApplyTheaterKit",
        "GetAppliedWeatherIdentity",
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored BlueprintPure signature
    # `const FSkyguardTheaterKitSpec& GetAppliedSpec() const`.
    # Accept origin/main inline body without locking the
    # body, plus a trailing semicolon form. Do not accept a
    # missing method const, a missing const on the return
    # reference, a void/float/bool/FName rename, extra
    # params, leftover AppliedSpec UPROPERTY, sibling
    # GetAppliedWeatherIdentity / GetAppliedKitId /
    # GetNamedLandmark / GetBuildingTint, or leftover
    # ApplyTheaterKitToWorld when origin/main has
    # `const FSkyguardTheaterKitSpec& GetAppliedSpec() const`.
    del declaration
    compact = collapsed(region)
    if GET_APPLIED_SPEC_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+GetAppliedSpec\b", compact):
        return False
    if re.search(r"\bfloat\s+GetAppliedSpec\b", compact):
        return False
    if re.search(r"\bbool\s+GetAppliedSpec\b", compact):
        return False
    if re.search(r"\bFName\s+GetAppliedSpec\b", compact):
        return False
    if re.search(
        r"(?<!const )FSkyguardTheaterKitSpec\s*&\s+GetAppliedSpec\b",
        compact,
    ):
        return False
    if re.search(
        r"\bconst\s+FSkyguardTheaterKitSpec\s+GetAppliedSpec\b",
        compact,
    ):
        return False
    if re.search(
        r"\bconst\s+FSkyguardTheaterKitSpec\s*&\s+"
        r"GetAppliedSpec\s*\(\s*[^)]+\s*\)",
        compact,
    ):
        return False
    return True
...
def count_one_declaration(region: str, declaration: str) -> int:
    del declaration
    compact = collapsed(region)
    return len(GET_APPLIED_SPEC_SIGNATURE_RE.findall(compact))
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*const\s+FSkyguardTheaterKitSpec\s*&\s+"
            r"GetAppliedSpec\b",
            compact[index:],
        ):
    ...
    raise AssertionError(
        "UFUNCTION for const FSkyguardTheaterKitSpec& "
        "GetAppliedSpec is missing from "
...
class TheaterKitGetAppliedSpecDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, TARGET_INLINE), section)
        self.assertTrue(has_identifier(section, "GetAppliedSpec"), section)
        ...
        for sibling in sibling_names_not_in_locked_decl():
            self.assertNotIn(sibling, LOCKED_DECL)
        for leftover in leftover_spec_names_not_in_locked_decl():
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetAppliedSpec", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_applied_spec_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
            f"\tTObjectPtr<USceneComponent> "
            f"{SIBLING_ROOT};\n"
            "private:\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn(
            "GetAppliedSpec()const { return AppliedSpec; }",
            compact_origin,
        )
        self.assertNotIn("void GetAppliedSpec", compact_origin)
        self.assertNotIn("float GetAppliedSpec", compact_origin)
        self.assertNotIn("bool GetAppliedSpec", compact_origin)

    def test_get_applied_spec_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(has_declaration(section, TARGET_INLINE))
        self.assertEqual(declaration_count(section, LOCKED_DECL), 1)
        self.assertTrue(
            LOCKED_DECL.startswith("const FSkyguardTheaterKitSpec& GetAppliedSpec()"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("const", LOCKED_DECL)
        self.assertIn("const FSkyguardTheaterKitSpec&", LOCKED_DECL)
        ...
        self.assertTrue(
            has_declaration(f"\t{TARGET_INLINE}\n", LOCKED_DECL)
        )
        self.assertTrue(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_NO_RETURN_CONST}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BY_VALUE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FIELD}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_NO_CONST}\n", LOCKED_DECL)
        self.assertIn("GetAppliedSpec", str(raised.exception))
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_NO_RETURN_CONST}\n",
            f"\t{TARGET_WRONG_BY_VALUE}\n",
            f"\t{TARGET_WRONG_FNAME}\n",
            f"\t{TARGET_WRONG_FIELD}\n",
            f"\t{TARGET_WRONG_SIBLING_WEATHER}\n",
            f"\t{TARGET_WRONG_SIBLING_LANDMARK}\n",
            f"\t{TARGET_WRONG_SIBLING_TINT}\n",
            f"\t{TARGET_WRONG_SIBLING_KIT}\n",
            leftover_primary,
            leftover_guided,
    ...
    def test_callable_instead_of_pure_fails_closed(self) -> None:
        callable_wrap = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
        ...
        section = actor_section(callable_wrap)
        specifiers = attached_ufunction_specifiers(section)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            f"public:\n\t{UFUNCTION_PURE}\n\t{LOCKED_DECL}\n",
            f"public:\n\t{UFUNCTION_PURE}\n\t{TARGET_INLINE}\n",
            f"public:\n\t{UFUNCTION_PURE} {LOCKED_DECL}\n",
            f"public:\n\t{UFUNCTION_PURE} {TARGET_INLINE}\n",
            "public:\n\tUFUNCTION(BlueprintPure, "
            'Category="Skyguard|Theater")\n'
            f"\t{TARGET_INLINE}\n",
            "public:\n\tUFUNCTION(\n\t\tBlueprintPure, "
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetAppliedSpec decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_THEATER_KIT_ROAD_INSTANCES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_BUILDING_INSTANCES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_LAMP_INSTANCES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_NAMED_LANDMARK_MESH, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_SILHOUETTE_INSTANCES, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_APPLIED_SPEC, LOCKED_SCRIPTS)
        self.assertIn(
            LEFTOVER_THEATER_KIT_GET_APPLIED_WEATHER_IDENTITY,
            LOCKED_SCRIPTS,
        )
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            *leftover_theater_kit_actor_field_scripts(),
            *leftover_theater_kit_actor_ufunction_scripts(),
            "Scripts/tests/test_briefing_card_defaults_contract.py",
            "Scripts/tests/test_briefing_radio_row_defaults_contract.py",
            "Scripts/tests/test_how_to_fly_row_defaults_contract.py",
            "Scripts/tests/test_briefing_card_priority_field_decl_contract.py",
    ...
    def test_contract_is_target_ufunction_declaration_only(self) -> None:
        ...
        self.assertIn(STRUCT_NAME, locked_only)
        self.assertIn("GetAppliedSpec", locked_only)
        self.assertIn("const FSkyguardTheaterKitSpec&", locked_only)
        self.assertNotIn("FName ", locked_only)
        self.assertNotIn("WeatherIdentity", locked_only)
        self.assertNotIn("struct FSkyguardTheaterKitSpec", locked_only)
        for helper in namespace_helper_tokens():
