...
# INDEX_NONE or lock TryJamActivePayload
# construction in the .cpp. This is leftover-
# safe Mission06 Payload TryJamActivePayload on
# ASkyguardMission06IntegrationDirector. It is
# NOT leftover payload-window-runtime-defaults
# #f114, NOT leftover StartPayloadWindow /
# AdvancePayloadWindow (sibling methods, list
# them unlocked), NOT leftover Mission05
# StartNextWave #668, NOT leftover Mission03 /
# Mission04 StartNextWave, NOT leftover Mission01 /
...
# Mission04 StartNextWave, leftover Mission06
# StartNextWave, leftover Mission04
...
# (`bool TryJamActivePayload();`); accept that
...
# StartNextWave method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission06IntegrationDirector::TryJamActivePayload",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission06TryJamActivePayloadDeclContractTests(unittest.TestCase):
    def test_mission06_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, TRY_JAM_ACTIVE_PAYLOAD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertIn("TryJamActivePayload", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_missing_initialize_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertIn("TryJamActivePayload", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertIn("TryJamActivePayload", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("BlueprintCallable", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("Category", TRY_JAM_ACTIVE_PAYLOAD)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertIn("TryJamActivePayload", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid TryJamActivePayload();\n"
        as_const = "\tbool TryJamActivePayload() const;\n"
        with_arg = "\tbool TryJamActivePayload(bool bForce);\n"
        renamed = "\tbool TryJam();\n"
        short_name = "\tbool JamActivePayload();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, TRY_JAM_ACTIVE_PAYLOAD)
            self.assertIn("TryJamActivePayload", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_initialize_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, TRY_JAM_ACTIVE_PAYLOAD))
        self.assertEqual(
            declaration_count(section, TRY_JAM_ACTIVE_PAYLOAD),
            1,
        )
        self.assertTrue(
            TRY_JAM_ACTIVE_PAYLOAD.startswith("bool "),
            TRY_JAM_ACTIVE_PAYLOAD,
        )
        self.assertTrue(
            TRY_JAM_ACTIVE_PAYLOAD.endswith(";"),
            TRY_JAM_ACTIVE_PAYLOAD,
        )
        self.assertIn("TryJamActivePayload", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertIn("()", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("TArray<", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("TObjectPtr", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("TSoftObjectPtr", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("INDEX_NONE", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("UFUNCTION", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("{", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("}", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("return ", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("const;", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("Root", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("Briefing", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("AudioDirector", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("RadioChatter", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("SortiePresentation", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("CampaignDefinition", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("MissionDefinition", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("Readiness", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("bAutoInitialize", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("bAllowBoundedActorSpawning", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("bAutoLaunchAfterBriefing", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("PathfinderSpawnLocation", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetAircraft", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("BindRuntimeActors", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("HandleDroneCityImpact", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("InitializePlayableMission", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("NotifyThreatDestroyed", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("AdvanceConvoyByDistance", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("NotifyConvoyDamage", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetWaveState", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetConvoyRouteState", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetDayBeatKit", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetNightBeatKit", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("StartSearchlightWindow", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("AdvanceSearchlightTrack", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("NotifySubstationDamage", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetRemainingThreatsInWave", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetSearchlightRuntime", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetSubstationIntegrity", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("StartPayloadWindow", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("AdvancePayloadWindow", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("StartNextWave", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("NotifyAirfieldTargetDamage", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetPayloadWindow", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetTargetRuntime", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetSurvivingTargetCount", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetDayBeatKind", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetDayBeatIndex", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("TickDayBeatKit", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("HandleBossPhaseChanged", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ESkyguardMission06WaveState", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ESkyguardAirfieldTarget", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ConvoyRuntimeAnchor", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("RoadHunterSpawnLocation", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ConfigureMissionDefinition", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("NotifyObjectiveProgress", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("NotifyProtectedAssetFailed", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("SynchronizeRuntimeState", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("IsCorePlayableReady", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetReadiness", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetObjectiveRuntime", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetGunner", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetPathfinder", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetMissionId", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ValidateMissionContract", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ConfigureFromMission", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("AdvanceBriefing", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("SetAssetsReady", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("AcknowledgeAndLaunch", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("CanLaunch", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetElapsedSeconds", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetBriefingState", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetMinimumWarmupSeconds", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetBriefingText", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetRadioChatter", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetPresentation", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("GetMissionTitle", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("AcknowledgeBriefing", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("LaunchSortie", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("HullCollider", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("OpticalTracker", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("WeaponServo", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("CountermeasurePod", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("MinHeightFromOriginCm", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("RadarNode", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ESkyguardMission02WaveState", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("HarborIndustrial", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("MaxIntegrity", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("CurrentIntegrity", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("FillAndFinalize", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("FillAndFail", TRY_JAM_ACTIVE_PAYLOAD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_PAYLOAD} {TRY_JAM_ACTIVE_PAYLOAD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{TRY_JAM_ACTIVE_PAYLOAD}\n}}\n"
        self.assertTrue(has_declaration(one_line, TRY_JAM_ACTIVE_PAYLOAD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, TRY_JAM_ACTIVE_PAYLOAD),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", TRY_JAM_ACTIVE_PAYLOAD)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_lock_initialize_cpp_body(self) -> None:
        ...
        self.assertNotIn("return false", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("return true", TRY_JAM_ACTIVE_PAYLOAD)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{TRY_JAM_ACTIVE_PAYLOAD}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertIn("TryJamActivePayload", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertTrue(TRY_JAM_ACTIVE_PAYLOAD.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("CurrentIntegrity", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("SkyguardApacheAircraft.h", TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{TRY_JAM_ACTIVE_PAYLOAD}\n"
        self.assertEqual(
            require_declaration(locked_only, TRY_JAM_ACTIVE_PAYLOAD),
            TRY_JAM_ACTIVE_PAYLOAD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 TryJamActivePayload method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{TRY_JAM_ACTIVE_PAYLOAD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, TRY_JAM_ACTIVE_PAYLOAD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                TRY_JAM_ACTIVE_PAYLOAD.lower(),
                "mission06 TryJamActivePayload contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, TRY_JAM_ACTIVE_PAYLOAD)

    def test_contract_is_try_jam_active_payload_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(TRY_JAM_ACTIVE_PAYLOAD.startswith("bool "))
        self.assertIn("TryJamActivePayload", TRY_JAM_ACTIVE_PAYLOAD)
        self.assertTrue(TRY_JAM_ACTIVE_PAYLOAD.endswith(";"))
        self.assertIn(UFUNCTION_PAYLOAD, section)
    ...
    def test_sibling_director_fields_do_not_satisfy_initialize(
        ...
        for leftover in (
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, TRY_JAM_ACTIVE_PAYLOAD)
            )
            self.assertNotEqual(TRY_JAM_ACTIVE_PAYLOAD, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("TryJamActivePayload", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, TRY_JAM_ACTIVE_PAYLOAD))
        locked_only = f"{TRY_JAM_ACTIVE_PAYLOAD}\n"
    ...
    def test_contract_does_not_relock_leftover_payload_window(self) -> None:
        ...
        self.assertNotIn(START_PAYLOAD_WINDOW, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn(ADVANCE_PAYLOAD_WINDOW, TRY_JAM_ACTIVE_PAYLOAD)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", TRY_JAM_ACTIVE_PAYLOAD)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_try_jam_active_payload"
                "_decl_contract.py"
            )
        )
        self.assertIn(
