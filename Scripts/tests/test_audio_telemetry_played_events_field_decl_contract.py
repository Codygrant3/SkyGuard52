# THIS IS leftover-safe FSkyguardAudioTelemetry::PlayedEvents.
# origin/main form: split-line
# UPROPERTY(BlueprintReadOnly)
# then `int32 PlayedEvents = 0;` with no Category=.
...
# do not copy those; this field is `int32 PlayedEvents = 0;`.
...
# #138 / #9efc.
# Do NOT contract RequestedEvents, RejectedByCooldown,
# RejectedByConcurrency, RejectedMissingAsset,
...
TARGET = "int32 PlayedEvents = 0;"
TARGET_WRONG_BARE = "int32 PlayedEvents;"
TARGET_WRONG_FALSE = "int32 PlayedEvents = false;"
TARGET_WRONG_TRUE = "int32 PlayedEvents = true;"
TARGET_WRONG_FLOAT = "int32 PlayedEvents = 0.f;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # when origin/main has `int32 PlayedEvents = 0;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+PlayedEvents\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PlayedEvents\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PlayedEvents\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PlayedEvents\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+PlayedEvents\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 PlayedEvents is missing from "
...
class AudioTelemetryPlayedEventsFieldDeclContractTests(unittest.TestCase):
    def test_audio_telemetry_struct_exists(self) -> None:
        ...
        self.assertIn("PlayedEvents", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("PlayedEvents", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("PlayedEvents = 0", collapsed(origin))
        self.assertNotIn("PlayedEvents = false", collapsed(origin))
        self.assertNotIn("PlayedEvents = true", collapsed(origin))
        self.assertNotIn("PlayedEvents = 0.f", collapsed(origin))
    ...
    def test_played_events_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 PlayedEvents"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tbool PlayedEvents = 0;\n",
            "\tint32 PlayedEvent = 0;\n",
            "\tint32 PlayedEventss = 0;\n",
            "\tFText PlayedEvents = 0;\n",
            "\tint32 PlayedEvents{};\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio-telemetry PlayedEvents field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_audio_telemetry_played_events"
            "_field_decl_contract.py"
        ))
