# THIS IS leftover-safe FSkyguardNightSortieBeatKit Beats.
...
# bKeepThermal. Fail-closed if Beats is missing or renamed,
# type is not FSkyguardNightSortieBeat, array bound is not
# `7`, or UPROPERTY / Category clones land.
# Do NOT claim leftover DaySortieBeatKit `Beats[7]`.
# THIS IS NOT leftover analog night-sortie-beat-kit-beats
# #252 / #b4a6 (keep bulk analog in LOCKED_SCRIPTS).
...
# FSkyguardNightSortieBeat, bound is `7`, locked decl is
# the bare field.
...
TARGET = "FSkyguardNightSortieBeat Beats[7];"
TARGET_WRONG_EQ_NONE = "FSkyguardNightSortieBeat Beats[7] = {};"
TARGET_WRONG_FALSE = "FSkyguardNightSortieBeat Beats = false;"
TARGET_WRONG_TRUE = "FSkyguardNightSortieBeat Beats = true;"
TARGET_WRONG_ZERO = "FSkyguardNightSortieBeat Beats[7] = 0.f;"
...
TARGET_WRONG_DAY = "FSkyguardDaySortieBeat Beats[7];"
TARGET_WRONG_BOUND_SIX = "FSkyguardNightSortieBeat Beats[6];"
TARGET_WRONG_BOUND_EIGHT = "FSkyguardNightSortieBeat Beats[8];"
TARGET_WRONG_BARE = "FSkyguardNightSortieBeat Beats;"
TARGET_WRONG_TARRAY = "TArray<FSkyguardNightSortieBeat> Beats;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `FSkyguardNightSortieBeat Beats[7];`.
    # Fail-closed if type is not FSkyguardNightSortieBeat.
    # Fail-closed if array bound is not `7`.
    # Fail-closed if Beats is missing or renamed.
    # Do not accept leftover DaySortieBeat `Beats[7]`.
    ...
    if re.search(
        r"FSkyguardDaySortieBeat\s+Beats\b",
        compact,
    ):
        return False
    if re.search(
        r"FSkyguardNightSortieBeat\s+Beats\s*=",
        compact,
    ):
        return False
    if re.search(
        r"FSkyguardNightSortieBeat\s+Beats\s*\[\s*7\s*\]\s*=",
        compact,
    ):
        return False
    if re.search(
        ...
        return False
    if re.search(
        r"FSkyguardNightSortieBeat\s+Beats\s*\[\s*(?!7\s*\])",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FSkyguardNightSortieBeat\s+Beats\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FSkyguardNightSortieBeat Beats is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FSkyguardNightSortieBeat\s+Beats\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FSkyguardNightSortieBeat Beats is not the "
...
class NightSortieBeatKitBeatsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_locked_decl_is_bare_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FSkyguardNightSortieBeat Beats[7];")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(
            LOCKED_DECL.startswith("FSkyguardNightSortieBeat Beats")
        )
    ...
    def test_does_not_claim_leftover_day_sortie_beats(self) -> None:
        ...
        self.assertIn("FSkyguardDaySortieBeat Beats[7];", day_header)
        self.assertNotIn("FSkyguardNightSortieBeat Beats[7];", day_header)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn("FSkyguardNightSortieBeat Beats[7];", storm_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FSkyguardNightSortieBeat Beats[7];", compact_origin)
        ...
        self.assertNotIn("FSkyguardDaySortieBeat Beats[7];", compact_origin)
    ...
    def test_beats_declaration_matches_origin_main(self) -> None:
        ...
        self.assertIn("FSkyguardNightSortieBeat ", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFSkyguardNightSortieBeat Beat[7];\n",
            "\tFSkyguardNightSortieBeat Beatses[7];\n",
            "\tfloat Beats[7] = " + forty + ";\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "night-sortie-beat-kit Beats field decl contract "
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("FSkyguardDaySortieBeat", locked_only)
