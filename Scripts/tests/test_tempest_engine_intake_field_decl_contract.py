...
# INDEX_NONE or lock EngineIntake construction in the
...
# EngineIntake;`); accept that form and other
...
# PortDischargeBoom, DebrisPortPanel,
...
# Python EngineIntake field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardTempestBoss::EngineIntake",
    "SkyguardTempestBoss.cpp",
    "SetExposed",
...
class TempestEngineIntakeFieldDeclContractTests(unittest.TestCase):
    def test_tempest_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ENGINE_INTAKE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ENGINE_INTAKE_FIELD)
        self.assertIn("EngineIntake", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ENGINE_INTAKE_FIELD))

    def test_missing_engine_intake_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardTempestBoss();\n"
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardDischargeBoom;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ENGINE_INTAKE_FIELD)
        self.assertIn("EngineIntake", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ENGINE_INTAKE_FIELD)
        self.assertIn("EngineIntake", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ENGINE_INTAKE_FIELD), section)
        self.assertNotIn("UPROPERTY", ENGINE_INTAKE_FIELD)
        self.assertNotIn("VisibleAnywhere", ENGINE_INTAKE_FIELD)
        self.assertNotIn("BlueprintReadOnly", ENGINE_INTAKE_FIELD)
        self.assertNotIn("Category", ENGINE_INTAKE_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ENGINE_INTAKE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ENGINE_INTAKE_FIELD)
        self.assertIn("EngineIntake", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tUSkyguardBossWeakPointComponent* EngineIntake;\n"
        as_array = (
            "\tTArray<TObjectPtr<USkyguardBossWeakPointComponent>> "
            "EngineIntake;\n"
        )
        scene = "\tTObjectPtr<USceneComponent> EngineIntake;\n"
        static_mesh = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ENGINE_INTAKE_FIELD)
            self.assertIn("EngineIntake", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_engine_intake_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ENGINE_INTAKE_FIELD),
            ENGINE_INTAKE_FIELD,
        )
        self.assertTrue(has_declaration(section, ENGINE_INTAKE_FIELD))
        self.assertEqual(declaration_count(section, ENGINE_INTAKE_FIELD), 1)
        self.assertTrue(
            ENGINE_INTAKE_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            ENGINE_INTAKE_FIELD,
        )
        self.assertTrue(ENGINE_INTAKE_FIELD.endswith(";"), ENGINE_INTAKE_FIELD)
        self.assertIn("EngineIntake", ENGINE_INTAKE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            ENGINE_INTAKE_FIELD,
        )
        self.assertIn("TObjectPtr<", ENGINE_INTAKE_FIELD)
        self.assertNotIn("TArray<", ENGINE_INTAKE_FIELD)
        self.assertNotIn("=", ENGINE_INTAKE_FIELD)
        self.assertNotIn("INDEX_NONE", ENGINE_INTAKE_FIELD)
        self.assertNotIn("UFUNCTION", ENGINE_INTAKE_FIELD)
        self.assertNotIn("{", ENGINE_INTAKE_FIELD)
        self.assertNotIn("}", ENGINE_INTAKE_FIELD)
        self.assertNotIn("return ", ENGINE_INTAKE_FIELD)
        self.assertNotIn("StarboardDischargeBoom", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ControlServo", ENGINE_INTAKE_FIELD)
        self.assertNotIn("PortDischargeBoom", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisPortPanel", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisStarboardPanel", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisIntakePanel", ENGINE_INTAKE_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", ENGINE_INTAKE_FIELD)
        self.assertNotIn("CommandAntenna", ENGINE_INTAKE_FIELD)
        self.assertNotIn("NoseCamera", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ControlLinkage", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisNose", ENGINE_INTAKE_FIELD)
        self.assertNotIn(
            "TObjectPtr<USkyguardBossWeakPointComponent> Engine;",
            ENGINE_INTAKE_FIELD,
        )
        self.assertNotIn("DebrisTail", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisSpine", ENGINE_INTAKE_FIELD)
        self.assertNotIn("EncounterController", ENGINE_INTAKE_FIELD)
        self.assertNotIn("AdvanceEncounter", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ResetEncounterState", ENGINE_INTAKE_FIELD)
        self.assertNotIn("IsRouteStateSafe", ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetRouteProgress", ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", ENGINE_INTAKE_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", ENGINE_INTAKE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ENGINE_INTAKE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", ENGINE_INTAKE_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", ENGINE_INTAKE_FIELD)
        self.assertNotIn("IssueClimbCommand", ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetGunnerMount", ENGINE_INTAKE_FIELD)
        self.assertNotIn("FaceWorldLocation", ENGINE_INTAKE_FIELD)
        self.assertNotIn("PortGuidanceArray", ENGINE_INTAKE_FIELD)
        self.assertNotIn("CommandCore", ENGINE_INTAKE_FIELD)
        self.assertNotIn("PortEngine", ENGINE_INTAKE_FIELD)
        self.assertNotIn("StarboardEngine", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetSearchlightTracked", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetContactIdentified", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetLightningExposed", ENGINE_INTAKE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", ENGINE_INTAKE_FIELD)
        self.assertNotIn("HullCollider", ENGINE_INTAKE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, ENGINE_INTAKE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION05} {ENGINE_INTAKE_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, ENGINE_INTAKE_FIELD), 1)
        one_line = f"{{\npublic:\n\t{ENGINE_INTAKE_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ENGINE_INTAKE_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ENGINE_INTAKE_FIELD), section)
        self.assertEqual(
            require_declaration(section, ENGINE_INTAKE_FIELD),
    ...
    def test_assigned_engine_intake_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "EngineIntake = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ENGINE_INTAKE_FIELD)
        self.assertIn("EngineIntake", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ENGINE_INTAKE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            self.assertIn("EngineIntake", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ENGINE_INTAKE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ENGINE_INTAKE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ENGINE_INTAKE_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ENGINE_INTAKE_FIELD.startswith("UFUNCTION"),
            ENGINE_INTAKE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ENGINE_INTAKE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ENGINE_INTAKE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_engine_intake_cpp_body(self) -> None:
        locked_only = f"{ENGINE_INTAKE_FIELD}\n"
        self.assertNotIn("{", ENGINE_INTAKE_FIELD)
        self.assertNotIn("}", ENGINE_INTAKE_FIELD)
        self.assertNotIn("return ", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ASkyguardTempestBoss::EngineIntake", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SkyguardTempestBoss.cpp", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SkyguardTempestBoss.cpp", locked_only)
        self.assertNotIn("return false", ENGINE_INTAKE_FIELD)
        self.assertNotIn("return true", ENGINE_INTAKE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetExposed", ENGINE_INTAKE_FIELD)
        self.assertNotIn("RefreshAuthoredWeakPointRegistry", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetBossPhase", ENGINE_INTAKE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", ENGINE_INTAKE_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{ENGINE_INTAKE_FIELD}\n"
        for token in TEMPEST_SIBLING_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("StarboardDischargeBoom", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ControlServo", ENGINE_INTAKE_FIELD)
        self.assertNotIn("PortDischargeBoom", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisPortPanel", ENGINE_INTAKE_FIELD)
        ...
        self.assertNotIn("DebrisIntakePanel", ENGINE_INTAKE_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", ENGINE_INTAKE_FIELD)
        self.assertIn("USkyguardBossWeakPointComponent", ENGINE_INTAKE_FIELD)
        self.assertIn("EngineIntake", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_get_civilian_separation_meters_sibling(
        ...
        for token in GET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetFinaleStage", ENGINE_INTAKE_FIELD)
        self.assertNotIn("IssueClimbCommand", ENGINE_INTAKE_FIELD)
        self.assertNotIn("IsWreckDiverted", ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_finale_stage(self) -> None:
        ...
        for token in GET_FINALE_STAGE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetFinaleStage", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_is_climb_command_issued_sibling(
        ...
        for token in IS_CLIMB_COMMAND_ISSUED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), ENGINE_INTAKE_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), ENGINE_INTAKE_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), ENGINE_INTAKE_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), ENGINE_INTAKE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), ENGINE_INTAKE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, ENGINE_INTAKE_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_weak_point_component_fields(
        ...
        self.assertNotIn("UStaticMeshComponent", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_guidance_array(
        ...
        for token in LEFTOVER_STARBOARD_GUIDANCE_ARRAY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_strike_bay_mechanism(
        ...
        for token in LEFTOVER_PORT_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_strike_bay_mechanism(
        ...
        for token in LEFTOVER_STARBOARD_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_cooling_system(self) -> None:
        ...
        for token in LEFTOVER_PORT_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("PortCoolingSystem", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_cooling_system(
        ...
        for token in LEFTOVER_STARBOARD_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("StarboardCoolingSystem", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_jammer(self) -> None:
        ...
        for token in LEFTOVER_JAMMER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("Jammer", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_last_flight_debris_fields(
        ...
        for token in LEFTOVER_LAST_FLIGHT_DEBRIS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisArmorPort", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisArmorStarboard", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisStrikeBayPort", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisStrikeBayStarboard", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisEnginePort", ENGINE_INTAKE_FIELD)
        self.assertNotIn("DebrisEngineStarboard", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetSearchlightTracked", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetContactIdentified", ENGINE_INTAKE_FIELD)
        self.assertNotIn("OpenOrbitExposure", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", ENGINE_INTAKE_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", ENGINE_INTAKE_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            ENGINE_INTAKE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, ENGINE_INTAKE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("ASkyguardGunner", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("SkyguardRadarNode", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(self) -> None:
        ...
        self.assertNotIn("USkyguardDebriefWidget", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, ENGINE_INTAKE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ENGINE_INTAKE_FIELD)
        self.assertNotIn("MaxIntegrity", ENGINE_INTAKE_FIELD)
        self.assertNotIn("CurrentIntegrity", ENGINE_INTAKE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ENGINE_INTAKE_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ENGINE_INTAKE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ENGINE_INTAKE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ENGINE_INTAKE_FIELD),
            ENGINE_INTAKE_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss::EngineIntake", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "tempest EngineIntake field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{ENGINE_INTAKE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ENGINE_INTAKE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ENGINE_INTAKE_FIELD.lower(),
                "tempest EngineIntake contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ENGINE_INTAKE_FIELD)

    def test_contract_is_engine_intake_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("USkyguardBossWeakPointComponent", ENGINE_INTAKE_FIELD)
        self.assertTrue(ENGINE_INTAKE_FIELD.endswith(";"))
        self.assertNotIn("=", ENGINE_INTAKE_FIELD)
        self.assertIn(UPROPERTY_MISSION05, section)
