# THIS IS leftover-safe FSkyguardStormRainBeatKit WeatherLabel.
...
# or leftover campaign WeatherLabel as this kit field.
...
# WeatherIdentity / Weather / bHydraForClusters / Kinds /
# Threats / Stations / Calls.
# Fail-closed if this lane claims leftover Title instead of
# WeatherLabel. Same type and TEXT("") must not steal the
# sibling Title. Fail-closed if this lane claims leftover
# WeatherIdentity or leftover Weather enum.
...
# THIS IS NOT leftover isolated Title field decl
# storm-rain-beat-kit-title-field-decl-contract.
# THIS IS NOT leftover isolated WeatherIdentity field decl.
# THIS IS NOT leftover DaySortieBeatKit isolated MissionId.
# THIS IS NOT leftover NightSortieBeatKit isolated
...
# bare `FName MissionId;` / leftover `const TCHAR* Title =
# TEXT("");`, retarget: type is const TCHAR*, identifier is
# WeatherLabel, initializer is TEXT(""), locked decl is the
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept missing TEXT(""), leftover Title,
    # FName WeatherIdentity, Weather enum, `= NAME_None` /
    # `= false` / `= true` / `= 0.f` / `= 160.f` when
    # origin/main locks TEXT(""). Do not accept sibling Title
    # as this WeatherLabel decl. Same type and TEXT("") must
    # not steal leftover Title.
...
class StormRainBeatKitWeatherLabelFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, 'const TCHAR* WeatherLabel = TEXT("");')
        ...
        self.assertIn("WeatherLabel", LOCKED_DECL)
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_TITLE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_WEATHER_IDENTITY))
    ...
    def test_does_not_claim_leftover_day_night_loadout_or_campaign(self) -> None:
        ...
        self.assertIn(LOCKED_DECL, campaign_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_title_does_not_satisfy_weather_label(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_TITLE}\n"
    ...
    def test_weather_identity_does_not_satisfy_weather_label(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_weather_enum_does_not_satisfy_weather_label(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEATHER}\n"
        ...
        section = spec_section(leftover)
        self.assertTrue(has_identifier(section, SIBLING_WEATHER), section)
        self.assertFalse(has_declaration(section, LOCKED_DECL), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("WeatherLabel", str(raised.exception))
    ...
    def test_initializer_must_be_empty_text(self) -> None:
        ...
        self.assertIn('const TCHAR* WeatherLabel = TEXT("");', compact_origin)
    ...
    def test_weather_label_declaration_matches_origin_main(self) -> None:
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_WEATHER}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_does_not_contract_sibling_kit_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_WEATHER_IDENTITY))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "storm-rain-beat-kit WeatherLabel field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_ISOLATED_TITLE)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(
            has_identifier(locked_only, SIBLING_WEATHER_IDENTITY)
        )
        self.assertFalse(has_identifier(locked_only, SIBLING_WEATHER))
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TITLE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_IDENTITY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEATHER)
        for harbor in leftover_harbor_beat_tokens():
