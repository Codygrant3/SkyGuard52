# THIS IS leftover-safe FSkyguardGuidedLockRules SensorAcquireDegrees.
# origin/main form: BARE plain C++ static constexpr field
# `static constexpr float SensorAcquireDegrees = 5.5f;`
# with the paper lock value 5.5f. NOT a UPROPERTY wrap.
...
# is invented. Locked initializer is 5.5f, not analog 5.4f
# RocketSpreadDegrees and not GuidedLockConeDegrees 6.0f.
...
# RocketSpreadDegrees = 5.4f (close but not 5.5f).
# Do NOT parse leftover analog GuidedLockConeDegrees = 6.0f.
# Do NOT parse leftover analog GuidedLockSeconds = 1.80f
# (keep analog tests in LOCKED_SCRIPTS). This slot is
# SensorAcquireDegrees = 5.5f.
...
# TimeOfDayHours 12.f is not this slot.
...
# THIS IS NOT leftover GuidedLockRules HelmetAcquireDegrees
# #1538 (12.0f).
...
# THIS IS NOT leftover CpgHudSnapshot SightMode #1510.
...
# HelmetLockSeconds. Do NOT claim HelmetLockSeconds as
# this slot.
...
LEFTOVER_ANALOG_ROCKET_SPREAD = "RocketSpreadDegrees"
LEFTOVER_ANALOG_CONE = "GuidedLockConeDegrees"
...
TARGET = "static constexpr float SensorAcquireDegrees = 5.5f;"
TARGET_WRONG_ANALOG = (
    "static constexpr float GuidedLockSeconds = 1.80f;"
)
TARGET_WRONG_ROCKET_SPREAD = (
    "static constexpr float RocketSpreadDegrees = 5.4f;"
)
TARGET_WRONG_CONE = (
    "static constexpr float GuidedLockConeDegrees = 6.0f;"
)
TARGET_WRONG_ANALOG_VALUE = (
    "static constexpr float SensorAcquireDegrees = 5.4f;"
)
TARGET_WRONG_CONE_VALUE = (
    "static constexpr float SensorAcquireDegrees = 6.0f;"
)
TARGET_WRONG_HELMET_VALUE = (
    "static constexpr float SensorAcquireDegrees = 12.0f;"
)
TARGET_WRONG_TOD = (
    "static constexpr float TimeOfDayHours = 12.f;"
)
TARGET_WRONG_BARE = "static constexpr float SensorAcquireDegrees;"
TARGET_WRONG_NO_STATIC = (
    "constexpr float SensorAcquireDegrees = 5.5f;"
)
TARGET_WRONG_NO_CONSTEXPR = (
    "static float SensorAcquireDegrees = 5.5f;"
)
TARGET_WRONG_FLOAT_ONLY = "float SensorAcquireDegrees = 5.5f;"
TARGET_WRONG_INT = "static constexpr int32 SensorAcquireDegrees = 5;"
TARGET_WRONG_BOOL = "static constexpr bool SensorAcquireDegrees = true;"
TARGET_WRONG_LONG = (
    "static constexpr float SensorAcquireDegrees = 5.50f;"
)
TARGET_WRONG_NO_F = (
    "static constexpr float SensorAcquireDegrees = 5.5;"
)
TARGET_WRONG_HELMET_LOCK = (
...
def leftover_analog_feel_tokens() -> tuple[str, ...]:
    return (
        LEFTOVER_ANALOG_GUIDED,
        LEFTOVER_ANALOG_ROCKET_SPREAD,
        LEFTOVER_ANALOG_CONE,
...
def sensor_acquire_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*static\s+constexpr\s+float\s+"
        r"SensorAcquireDegrees\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `static constexpr float SensorAcquireDegrees = 5.5f;`.
    # Do not accept analog RocketSpreadDegrees = 5.4f,
    # GuidedLockConeDegrees = 6.0f, GuidedLockSeconds = 1.80f,
    # leftover HelmetAcquireDegrees = 12.0f, leftover
    # HelmetLockSeconds = 2.40f, missing static/constexpr,
    # sibling SensorLockSeconds, or a UPROPERTY / Category
    # clone from leftover TheaterKit #1300.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"static\s+constexpr\s+float\s+SensorAcquireDegrees\s*=\s*5\.5f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"SensorAcquireDegrees\s*=\s*5\.4f",
        compact,
    ):
        return False
    if re.search(
        r"SensorAcquireDegrees\s*=\s*6\.0f",
        compact,
    ):
        return False
    if re.search(
        r"SensorAcquireDegrees\s*=\s*12\.0f",
        compact,
    ):
        return False
    if re.search(
        r"SensorAcquireDegrees\s*=\s*5\.50f",
        compact,
    ):
        return False
    if re.search(
        r"\bRocketSpreadDegrees\b",
        compact,
    ):
        return False
    if re.search(
        r"\bGuidedLockConeDegrees\b",
        compact,
    ):
        return False
    if re.search(
    ...
    if re.search(
        ...
        return False
    if sensor_acquire_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static\s+constexpr\s+float\s+SensorAcquireDegrees\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for static constexpr float SensorAcquireDegrees is "
...
def require_no_uproperty_wrap(section: str) -> None:
    ...
    if re.search(
        ...
        raise AssertionError(
            "UPROPERTY wrap on SensorAcquireDegrees is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(
        section, "SensorAcquireDegrees"
    ):
        raise AssertionError(
            "UPROPERTY clone landed on SensorAcquireDegrees; locked "
    ...
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(
            ...
            raise AssertionError(
                f"{token} clone landed on SensorAcquireDegrees; locked "
                f"decl is bare {LOCKED_DECL}"
            )


class GuidedLockRulesSensorAcquireDegreesFieldDeclContractTests(
    unittest.TestCase
):
    def test_guided_lock_rules_struct_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "SensorAcquireDegrees"), section
        )
        ...
        self.assertNotIn(LEFTOVER_ANALOG_ROCKET_SPREAD, section)
        self.assertNotIn(LEFTOVER_ANALOG_CONE, section)
        self.assertNotIn(LEFTOVER_TIME_OF_DAY, section)
    ...
    def test_locked_decl_is_plain_constexpr_not_uproperty(self) -> None:
        ...
        self.assertIn("SensorAcquireDegrees", LOCKED_DECL)
        self.assertIn("= 5.5f", LOCKED_DECL)
        self.assertNotIn("5.4f", LOCKED_DECL)
        self.assertNotIn("6.0f", LOCKED_DECL)
        ...
        self.assertNotIn(LEFTOVER_ANALOG_ROCKET_SPREAD, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_ANALOG_CONE, LOCKED_DECL)
        self.assertNotIn(SIBLING_HELMET_LOCK, LOCKED_DECL)
        self.assertNotIn(SIBLING_HELMET_ACQUIRE, LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROCKET_SPREAD)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CONE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HELMET_LOCK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HELMET_ACQUIRE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SENSOR_LOCK)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("SensorAcquireDegrees", str(raised.exception))
    ...
    def test_does_not_claim_leftover_apache_cpg_feel_or_lock_break(
        ...
        self.assertIn(LEFTOVER_ANALOG_ROCKET_SPREAD, feel_header)
        self.assertIn("5.4f", feel_header)
        self.assertIn(LEFTOVER_ANALOG_CONE, feel_header)
        self.assertIn("6.0f", feel_header)
        ...
        self.assertIn(LEFTOVER_TIME_OF_DAY, roster_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_sensor_acquire_degrees_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_analog_rocket_spread_and_cone_fail_closed(self) -> None:
        rocket = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_ROCKET_SPREAD}\n"
        ...
        cone_section = spec_section(cone)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(cone_section, LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        cone_value = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_CONE_VALUE}\n"
            "};\n"
        )
        cone_value_section = spec_section(cone_value)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(cone_value_section, LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        guided = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        guided_section = spec_section(guided)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(guided_section, LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        ...
        self.assertIn("SensorAcquireDegrees=5.5f;", compact_origin)
        self.assertNotIn("SensorAcquireDegrees=5.4f", compact_origin)
        self.assertNotIn("SensorAcquireDegrees=6.0f", compact_origin)
        self.assertNotIn("RocketSpreadDegrees", compact_origin)
        self.assertNotIn("GuidedLockConeDegrees", compact_origin)
        self.assertNotIn("GuidedLockSeconds", compact_origin)

    def test_leftover_helmet_acquire_and_lock_do_not_satisfy(self) -> None:
        helmet_acquire = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_HELMET_ACQUIRE}\n"
        ...
        section = spec_section(helmet_acquire)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        lock_section = spec_section(helmet_lock)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(lock_section, LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        helmet_value = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        helmet_value_section = spec_section(helmet_value)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(helmet_value_section, LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        tod = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        tod_section = spec_section(tod)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(tod_section, LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HELMET_ACQUIRE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HELMET_LOCK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TOD)
        self.assertNotIn(LEFTOVER_TIME_OF_DAY, LOCKED_DECL)

    def test_sensor_acquire_degrees_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ROCKET_SPREAD}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CONE}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HELMET_ACQUIRE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_ROCKET_SPREAD}\n", LOCKED_DECL)
        self.assertIn("SensorAcquireDegrees", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tstatic constexpr float SensorAcquireDegree = 5.5f;\n",
            "\tstatic constexpr float SensorAcquireDegrees = "
            + forty
            + ";\n",
            "\tstatic constexpr float SensorAcquireDegrees = "
            + eighty
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    sensor_acquire_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(sensor_acquire_has_uproperty_wrap(origin), origin)
    ...
    def test_does_not_contract_sibling_rules_fields(self) -> None:
        ...
        self.assertFalse(has_identifier(section, LEFTOVER_ANALOG_ROCKET_SPREAD))
        self.assertFalse(has_identifier(section, LEFTOVER_ANALOG_CONE))
        self.assertFalse(has_identifier(section, LEFTOVER_TIME_OF_DAY))
        for field in leftover_neighbor_hit_fields():
    ...
    def test_does_not_parse_leftover_headers_or_neighbors(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_ANALOG_ROCKET_SPREAD, header)
        self.assertNotIn(LEFTOVER_ANALOG_CONE, header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "guided-lock-rules SensorAcquireDegrees field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_CAMPAIGN_TIME_OF_DAY, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        for leftover in leftover_analog_feel_tokens():
            self.assertNotIn(leftover, locked_only)
        for token in FORBIDDEN_LOCKED_MACRO_TOKENS:
            self.assertNotIn(token, locked_only)
        ...
        self.assertNotIn(LEFTOVER_ANALOG_ROCKET_SPREAD, locked_only)
        self.assertNotIn(LEFTOVER_ANALOG_CONE, locked_only)
        self.assertNotIn(LEFTOVER_TIME_OF_DAY, locked_only)
