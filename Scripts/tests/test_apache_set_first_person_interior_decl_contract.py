...
# or lock the SetFirstPersonInterior body in the .cpp.
...
# SetOrbitFocus, FaceWorldLocation, or SetSensorView
# siblings. Stay off mount getters,
...
# in this wave, and leftover SortiePresentationWidgets
# stay sibling-only.
SET_FIRST_PERSON_INTERIOR = (
    "void SetFirstPersonInterior(bool bInterior);"
)
UFUNCTION_APACHE = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Apache")'
...
# SetFirstPersonInterior declaration contract on
...
# .cpp SetFirstPersonInterior body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::SetFirstPersonInterior",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheSetFirstPersonInteriorDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SET_FIRST_PERSON_INTERIOR),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_FIRST_PERSON_INTERIOR)
        self.assertIn("SetFirstPersonInterior", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, SET_FIRST_PERSON_INTERIOR)
    ...
    def test_missing_set_first_person_interior_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_FIRST_PERSON_INTERIOR)
        self.assertIn("SetFirstPersonInterior", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_FIRST_PERSON_INTERIOR)
        self.assertIn("SetFirstPersonInterior", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("UFUNCTION", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("Category", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("BlueprintCallable", SET_FIRST_PERSON_INTERIOR)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_FIRST_PERSON_INTERIOR)
        self.assertIn("SetFirstPersonInterior", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetFirstPersonInterior;\n"
        wrong_return_bool = "\tbool SetFirstPersonInterior(bool bInterior);\n"
        wrong_return_int = "\tint32 SetFirstPersonInterior(bool bInterior);\n"
        added_const = "\tvoid SetFirstPersonInterior(bool bInterior) const;\n"
        wrong_type = "\tvoid SetFirstPersonInterior(int bInterior);\n"
        wrong_float = "\tvoid SetFirstPersonInterior(float bInterior);\n"
        wrong_value = "\tvoid SetFirstPersonInterior(FVector bInterior);\n"
        added_arg = (
            "\tvoid SetFirstPersonInterior(bool bInterior, float Extra);\n"
        )
        no_args = "\tvoid SetFirstPersonInterior();\n"
        leftover_orbit = (
            "\tvoid SetOrbitFocus(const FVector& WorldLocation);\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_FIRST_PERSON_INTERIOR)
            self.assertIn("SetFirstPersonInterior", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_first_person_interior_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, SET_FIRST_PERSON_INTERIOR))
        self.assertEqual(
            declaration_count(section, SET_FIRST_PERSON_INTERIOR),
            1,
        )
        self.assertTrue(
            SET_FIRST_PERSON_INTERIOR.startswith("void "),
            SET_FIRST_PERSON_INTERIOR,
        )
        self.assertTrue(
            SET_FIRST_PERSON_INTERIOR.endswith(";"),
            SET_FIRST_PERSON_INTERIOR,
        )
        self.assertIn("SetFirstPersonInterior(", SET_FIRST_PERSON_INTERIOR)
        self.assertIn("bool bInterior", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("INDEX_NONE", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("{", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("}", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("return ", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn(" const", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("SetOrbitFocus", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("SetRotorPower", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("AimChinTurret", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetChinMuzzleLocation", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("IssuePilotCommand", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetPilotCommand", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn(
            "GetPilotConfirmationsIssued",
            SET_FIRST_PERSON_INTERIOR,
        )
        self.assertNotIn("FaceWorldLocation", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("SetSensorView", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetChinTurret", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetRotorPowerScale", SET_FIRST_PERSON_INTERIOR)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetFirstPersonInterior(bool bInterior);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tvoid SetFirstPersonInterior(\n"
            "\t\tbool bInterior);\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{SET_FIRST_PERSON_INTERIOR}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, SET_FIRST_PERSON_INTERIOR)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SET_FIRST_PERSON_INTERIOR),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_FIRST_PERSON_INTERIOR}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_FIRST_PERSON_INTERIOR)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_first_person_interior_cpp_body(
        self,
    ) -> None:
        locked_only = f"{SET_FIRST_PERSON_INTERIOR}\n"
        self.assertNotIn("{", SET_FIRST_PERSON_INTERIOR)
        ...
        self.assertNotIn("return false", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("return true", SET_FIRST_PERSON_INTERIOR)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{SET_FIRST_PERSON_INTERIOR}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetGunnerMount", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetEyeMount", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetWeaponMount", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetChinTurret", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetPilotMount", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetSensorTurret", SET_FIRST_PERSON_INTERIOR)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{SET_FIRST_PERSON_INTERIOR}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetChinMuzzleLocation", SET_FIRST_PERSON_INTERIOR)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("ApplyDamage", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("GetDamageFraction", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("ApplySystemHit", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("ApplyHit", SET_FIRST_PERSON_INTERIOR)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("IsSensorLive", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("IsThermalAvailable", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("IsSensorViewActive", SET_FIRST_PERSON_INTERIOR)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("ESkyguardApacheSystem", SET_FIRST_PERSON_INTERIOR)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("Pursuit", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("OrbitLeft", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("AttackRun", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("FaceTarget", SET_FIRST_PERSON_INTERIOR)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_FIRST_PERSON_INTERIOR)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("USkyguardBriefingWidget", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            SET_FIRST_PERSON_INTERIOR,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_FIRST_PERSON_INTERIOR)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            SET_FIRST_PERSON_INTERIOR,
        )
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_FIRST_PERSON_INTERIOR}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_FIRST_PERSON_INTERIOR),
            SET_FIRST_PERSON_INTERIOR,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SetFirstPersonInterior contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, SET_FIRST_PERSON_INTERIOR.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache SetFirstPersonInterior contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_FIRST_PERSON_INTERIOR)

    def test_contract_is_set_first_person_interior_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", SET_FIRST_PERSON_INTERIOR)
        self.assertNotIn("{", SET_FIRST_PERSON_INTERIOR)
        self.assertTrue(SET_FIRST_PERSON_INTERIOR.startswith("void "))
        self.assertTrue(SET_FIRST_PERSON_INTERIOR.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
