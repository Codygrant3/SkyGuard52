# THIS IS leftover-safe FSkyguardCpgContactMark bSeeking.
...
# `bool bSeeking = false;` with that in-struct initializer.
# Fail-closed if bSeeking is missing or renamed, if the
...
# bSeeking here is NOT leftover HUD LockLine #1505,
# leftover HUD LockPhase #1508, leftover HUD LockProgress
# #1512, leftover ContactMark WorldLocation #1516,
# leftover ContactMark Label, leftover ContactMark
# bLocked, or leftover ContactMark LockAlpha.
...
# and siblings in LOCKED_SCRIPTS.
# THIS IS NOT leftover ContactMark WorldLocation #1516.
# Keep that file in LOCKED_SCRIPTS.
...
# type is bool, identifier is bSeeking, initializer
# is `= false`, parse STRUCT FSkyguardCpgContactMark,
...
TARGET = "bool bSeeking = false;"
TARGET_WRONG_BARE = "bool bSeeking;"
TARGET_WRONG_EQ_EMPTY = 'bool bSeeking = TEXT("");'
TARGET_WRONG_EQ_NONE = "bool bSeeking = NAME_None;"
TARGET_WRONG_TRUE = "bool bSeeking = true;"
TARGET_WRONG_ZERO = "bool bSeeking = 0;"
TARGET_WRONG_ONE = "bool bSeeking = 1;"
TARGET_WRONG_ZERO_F = "bool bSeeking = 0.f;"
TARGET_WRONG_FLOAT = "float bSeeking = 0.f;"
TARGET_WRONG_HEALTH = "float bSeeking = 160.f;"
TARGET_WRONG_INT = "int32 bSeeking;"
TARGET_WRONG_INT_FALSE = "int32 bSeeking = false;"
TARGET_WRONG_FNAME = "FName bSeeking;"
TARGET_WRONG_FSTRING = "FString bSeeking;"
TARGET_WRONG_TCHAR = 'const TCHAR* bSeeking = TEXT("");'
TARGET_WRONG_FVECTOR = "FVector bSeeking = FVector::ZeroVector;"
...
SEEKING_LOCKED_RE = re.compile(
    r"bool\s+bSeeking\s*=\s*false\s*;"
)
SEEKING_BARE_RE = re.compile(r"bool\s+bSeeking\s*;")
SEEKING_INIT_RE = re.compile(
    r"bool\s+bSeeking\s*=\s*([^;]+);"
)
...
def seeking_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*bool\s+bSeeking\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `bool bSeeking = false;`.
    ...
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"float\s+bSeeking\b", compact):
        return False
    if re.search(r"FString\s+bSeeking\b", compact):
        return False
    if re.search(r"FVector\s+bSeeking\b", compact):
        return False
    if re.search(r"int32\s+bSeeking\b", compact):
        return False
    if re.search(r"FName\s+bSeeking\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*bSeeking\b", compact):
        return False
    if SEEKING_BARE_RE.search(compact) and (
        SEEKING_LOCKED_RE.search(compact) is None
    ):
        return False
    init_match = SEEKING_INIT_RE.search(compact)
    if init_match is None:
        return False
    if init_match.group(1).strip() != LOCKED_INITIALIZER:
        return False
    if SEEKING_LOCKED_RE.search(compact) is None:
        return False
    if seeking_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool bSeeking\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bSeeking is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*bool bSeeking\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on bool bSeeking is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "bSeeking"):
        raise AssertionError(
            "UPROPERTY clone landed on bSeeking; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "bSeeking"):
            raise AssertionError(
                f"{token} clone landed on bSeeking; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgContactMarkSeekingFieldDeclContractTests(unittest.TestCase):
    def test_cpg_contact_mark_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bSeeking"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "bool bSeeking = false;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("bool bSeeking"))
        ...
        self.assertIn("bSeeking", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        for clone_locked in (
            ...
            self.assertIn("bSeeking", str(raised.exception))
    ...
    def test_hud_snapshot_before_contact_mark_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bSeeking"), section)
        self.assertNotIn(STOP_BEFORE_HUD_SNAPSHOT, section)
        self.assertNotIn(LEFTOVER_HUD_SNAPSHOT, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_seeking_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_contact_mark_fields_do_not_satisfy_seeking(self) -> None:
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bSeeking = false;", compact_origin)
        self.assertNotIn("bSeeking = TEXT", compact_origin)
        self.assertNotIn("bSeeking = NAME_None", compact_origin)
        self.assertNotIn("bSeeking = true", compact_origin)
        self.assertNotIn("bSeeking = 0.f", compact_origin)
        self.assertNotIn("bSeeking = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("bSeeking = " + forty, compact_origin)
        self.assertNotIn("bSeeking = " + eighty, compact_origin)

    def test_seeking_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(LOCKED_DECL.startswith("bool bSeeking"), LOCKED_DECL)
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    seeking_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(seeking_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-contact-mark bSeeking field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "seeking_field_decl_contract.py"
        ))
