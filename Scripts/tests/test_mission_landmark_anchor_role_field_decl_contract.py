# THIS IS leftover-safe FSkyguardMissionLandmarkAnchor Role.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName Role;` (no initializer).
...
# Do NOT contract sibling fields LandmarkId / WorldLocation /
# bMissionExclusive.
...
# THIS IS NOT leftover ObjectiveAnchor WorldLocation #1386.
...
# retarget: type is FName Role, no `=`, specifiers
...
TARGET = "FName Role;"
TARGET_WRONG_EQ_NONE = "FName Role = NAME_None;"
TARGET_WRONG_FALSE = "FName Role = false;"
TARGET_WRONG_TRUE = "FName Role = true;"
TARGET_WRONG_ZERO = "FName Role = 0.f;"
TARGET_WRONG_FLOAT = "float Role;"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+Role\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName Role is missing from "
...
def require_role_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} Role UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} Role"
        )
...
class MissionLandmarkAnchorRoleFieldDeclContractTests(unittest.TestCase):
    def test_landmark_anchor_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Role"), section)
    ...
    def test_objective_or_readiness_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Role"), section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_ANCHOR, section)
        self.assertNotIn(STOP_BEFORE_MAP_READINESS, section)
        self.assertNotIn(ACTOR_NAME, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_role_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_role_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName Role;", compact_origin)
        self.assertNotIn("Role = NAME_None", compact_origin)
        self.assertNotIn("Role = false", compact_origin)
        self.assertNotIn("Role = true", compact_origin)
        self.assertNotIn("Role = 0.f", compact_origin)
        self.assertNotIn("Role = 160.f", compact_origin)

    def test_role_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName Role"),
            LOCKED_DECL,
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_role_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_role_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_role_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_role_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_role_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-landmark-anchor Role field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_landmark_anchor_role_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionMapAssemblyDirector.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_LANDMARK_ID_SCRIPT, LOCKED_SCRIPTS)
