...
# INDEX_NONE or lock InlandExtentCm construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# InlandExtentCm distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep InlandExtentCm
...
# ShorelineLandOffsetCm / BeachWidthCm (in-flight this
# wave) / SeawardExtentCm.
...
# one-line / split-line wraps. Locked 18000.f is
...
# 18000.f value. Do not lock leftover
...
LOCKED_INLAND_EXTENT_VALUE = "18000.f"
...
# InlandExtentCm field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::InlandExtentCm",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01InlandExtentCmFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, INLAND_EXTENT_CM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, INLAND_EXTENT_CM_FIELD))

    def test_missing_inland_extent_cm_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("EditAnywhere", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("EditInstanceOnly", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("BlueprintReadWrite", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("VisibleAnywhere", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("BlueprintReadOnly", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("Category", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("ClampMin", INLAND_EXTENT_CM_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, INLAND_EXTENT_CM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tfloat* InlandExtentCm;\n"
        )
        as_array = (
            "\tTArray<float> InlandExtentCm;\n"
        )
        no_init = "\tfloat InlandExtentCm;\n"
        int_type = "\tint32 InlandExtentCm = 45000;\n"
        double_type = "\tdouble InlandExtentCm = 18000.f;\n"
        wrong_init = "\tfloat InlandExtentCm = 0.f;\n"
        wrong_scale = "\tfloat InlandExtentCm = 40000.f;\n"
        scene = "\tTObjectPtr<USceneComponent> InlandExtentCm;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> InlandExtentCm;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> InlandExtentCm;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, INLAND_EXTENT_CM_FIELD)
            self.assertIn("InlandExtentCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_inland_extent_cm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, INLAND_EXTENT_CM_FIELD),
            INLAND_EXTENT_CM_FIELD,
        )
        self.assertTrue(has_declaration(section, INLAND_EXTENT_CM_FIELD))
        self.assertEqual(
            declaration_count(section, INLAND_EXTENT_CM_FIELD),
            1,
        )
        self.assertTrue(
            INLAND_EXTENT_CM_FIELD.startswith("float InlandExtentCm"),
            INLAND_EXTENT_CM_FIELD,
        )
        self.assertTrue(INLAND_EXTENT_CM_FIELD.endswith(";"), INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", INLAND_EXTENT_CM_FIELD)
        self.assertIn(
            "float",
            INLAND_EXTENT_CM_FIELD,
        )
        self.assertIn("=", INLAND_EXTENT_CM_FIELD)
        self.assertIn(LOCKED_INLAND_EXTENT_VALUE, INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("TObjectPtr<", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("TArray<", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("TSoftObjectPtr", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("nullptr", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("INDEX_NONE", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("UFUNCTION", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("{", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("}", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("return ", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("OceanTiles", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("LandTiles", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("RouteExclusion", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("bRouteExclusionValid", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("BeachTiles", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("InlandVegetationPCG", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("LandScatterBounds", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("ProductionLandscape", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("AuthoredPCGGraph", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("RouteLengthCm", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("BeachWidthCm", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("DistrictLengthCm", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("SeawardExtentCm", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            INLAND_EXTENT_CM_FIELD,
        )
        ...
        self.assertNotIn("Root", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetReadiness", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("Briefing", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("AudioDirector", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("RadioChatter", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("SortiePresentation", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("CampaignDefinition", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("MissionDefinition", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("bAutoInitialize", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetAircraft", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("ConfigureFromMission", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("AdvanceBriefing", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("SetAssetsReady", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("CanLaunch", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetElapsedSeconds", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetBriefingState", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetBriefingText", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetRadioChatter", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetPresentation", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("GetMissionTitle", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("AcknowledgeBriefing", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("LaunchSortie", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("HullCollider", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("OpticalTracker", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("WeaponServo", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("CountermeasurePod", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("RadarNode", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("HarborIndustrial", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("MaxIntegrity", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", INLAND_EXTENT_CM_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, INLAND_EXTENT_CM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT} {INLAND_EXTENT_CM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{INLAND_EXTENT_CM_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, INLAND_EXTENT_CM_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, INLAND_EXTENT_CM_FIELD),
            section,
    ...
    def test_assigned_inland_extent_cm_does_not_satisfy(self) -> None:
        assigned = (
            "\tfloat InlandExtentCm = 0.f;\n"
        )
        missing_init = "\tfloat InlandExtentCm;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, INLAND_EXTENT_CM_FIELD))
        with self.assertRaises(AssertionError) as raised_missing:
            require_declaration(missing_init, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", str(raised_missing.exception))
        self.assertFalse(has_declaration(missing_init, INLAND_EXTENT_CM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("InlandExtentCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, INLAND_EXTENT_CM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{INLAND_EXTENT_CM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, INLAND_EXTENT_CM_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            INLAND_EXTENT_CM_FIELD.startswith("UFUNCTION"),
            INLAND_EXTENT_CM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", INLAND_EXTENT_CM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_inland_extent_cm_cpp_body(self) -> None:
        locked_only = f"{INLAND_EXTENT_CM_FIELD}\n"
        self.assertNotIn("{", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("}", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("return ", INLAND_EXTENT_CM_FIELD)
        ...
        self.assertNotIn("return false", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("return true", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", INLAND_EXTENT_CM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", INLAND_EXTENT_CM_FIELD)
        self.assertTrue(
            INLAND_EXTENT_CM_FIELD.startswith("float InlandExtentCm")
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, INLAND_EXTENT_CM_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, INLAND_EXTENT_CM_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, INLAND_EXTENT_CM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", INLAND_EXTENT_CM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", INLAND_EXTENT_CM_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, LOCKED_INLAND_EXTENT_VALUE)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, INLAND_EXTENT_CM_FIELD)
        self.assertNotIn(LOCKED_INLAND_EXTENT_VALUE, leftover_harbor_tokens())
    ...
    def test_harbor_adjacent_fields_are_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual(LOCKED_INLAND_EXTENT_VALUE, token)
            self.assertNotIn(token, leftover_harbor_tokens())

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{INLAND_EXTENT_CM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, INLAND_EXTENT_CM_FIELD),
            INLAND_EXTENT_CM_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, INLAND_EXTENT_CM_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, INLAND_EXTENT_CM_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 InlandExtentCm field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{INLAND_EXTENT_CM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, INLAND_EXTENT_CM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                INLAND_EXTENT_CM_FIELD.lower(),
                "mission01 InlandExtentCm contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, INLAND_EXTENT_CM_FIELD)

    def test_contract_is_inland_extent_cm_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("InlandExtentCm", INLAND_EXTENT_CM_FIELD)
        self.assertTrue(INLAND_EXTENT_CM_FIELD.endswith(";"))
        self.assertIn("=", INLAND_EXTENT_CM_FIELD)
        self.assertIn(LOCKED_INLAND_EXTENT_VALUE, INLAND_EXTENT_CM_FIELD)
        self.assertNotIn("TObjectPtr<", INLAND_EXTENT_CM_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT, section)

    def test_sibling_environment_fields_do_not_satisfy_inland_extent_cm(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, INLAND_EXTENT_CM_FIELD))
            self.assertNotEqual(INLAND_EXTENT_CM_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_inland_extent_cm(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            ...
            "\t * This comment is not the InlandExtentCm declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, INLAND_EXTENT_CM_FIELD)
        self.assertIn("InlandExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, INLAND_EXTENT_CM_FIELD)
        )
        self.assertNotIn("float InlandExtentCm = 18000.f;", comment_only)
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, INLAND_EXTENT_CM_FIELD)
            self.assertNotEqual(token, INLAND_EXTENT_CM_FIELD)
        self.assertIn(LOCKED_INLAND_EXTENT_VALUE, INLAND_EXTENT_CM_FIELD)
        self.assertNotEqual(LOCKED_INLAND_EXTENT_VALUE, "45000.f")
        self.assertNotEqual(LOCKED_INLAND_EXTENT_VALUE, "2800.f")
        self.assertNotEqual(LOCKED_INLAND_EXTENT_VALUE, "5200.f")
        self.assertNotEqual(LOCKED_INLAND_EXTENT_VALUE, "1800.f")
        self.assertNotEqual(LOCKED_INLAND_EXTENT_VALUE, "7500.f")
        self.assertNotEqual(LOCKED_INLAND_EXTENT_VALUE, "20000.f")
