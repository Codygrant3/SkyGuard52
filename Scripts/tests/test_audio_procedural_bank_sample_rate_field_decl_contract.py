# THIS IS leftover-safe USkyguardAudioProceduralBankComponent SampleRate.
...
# then `int32 SampleRate = 48000;` (HAS spaces around = inside the decl).
# Locked wrap has NO spaces around = inside the wrap.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Audio|Development`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"8000"`. ClampMax IS `"48000"`.
# Default IS `= 48000`.
# This is a unique development SampleRate int32.
# It is NOT leftover sibling GeneratedByteBudget / bEnableDevelopmentAudition,
...
# bulk, NOT leftover analog coastal unique actor UPROPERTY, NOT leftover
...
# or stay Category="Skyguard" without |Audio|Development.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `USkyguardAudioProceduralBankComponent` public UPROPERTY
# fields ONLY after `class USkyguardAudioProceduralBankComponent`.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `private:`.
# Stop BEFORE sibling GeneratedByteBudget as a claimed slot.
...
# `"1000"` — RETARGET hard: Category Skyguard|Audio|Development
# wrapping `int32 SampleRate = 48000;` plus ClampMin `"8000"`
# ClampMax `"48000"`.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity
# with NO initializer — RETARGET hard.
# LOCKED_DECL is `int32 SampleRate = 48000;` NOT
...
# `int32 GeneratedByteBudget = 1048576;` AS THE LOCKED DECL.
...
# Skyguard|Audio|Acceptance / Skyguard|Environment|Performance
# or still Category="Skyguard" without |Audio|Development.
...
# SampleRate is 48000 not Harbor 40 / 80 float tokens.
...
HEADER_PATH = "Source/Skyguard52/SkyguardAudioProceduralBankComponent.h"
CLASS_NAME = "USkyguardAudioProceduralBankComponent"
TARGET = "int32 SampleRate = 48000;"
TARGET_WRONG_BARE = "int32 SampleRate;"
TARGET_WRONG_INIT = "int32 SampleRate = nullptr;"
TARGET_WRONG_FALSE = "int32 SampleRate = false;"
TARGET_WRONG_TRUE = "int32 SampleRate = true;"
TARGET_WRONG_ZERO = "int32 SampleRate = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> SampleRate;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> SampleRate;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> SampleRate;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "GeneratedByteBudget"
STOP_BEFORE_EPIC_SHRUB = "GeneratedByteBudget"
...
SAMPLE_RATE_WRONG_INIT_RE = re.compile(
    r"int32\s+SampleRate\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 SampleRate = 48000;`.
    ...
    # TObjectPtr, or a bare SampleRate without
    # the 8000 initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SAMPLE_RATE_DECL_RE.search(compact) is None:
        return False
...
class AudioProceduralBankSampleRateFieldDeclContractTests(unittest.TestCase):
    def test_audio_procedural_bank_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SampleRate"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENABLE_AUDITION), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT), section)
        self.assertTrue(has_identifier(body, SIBLING_BYTE_BUDGET), body)
        self.assertFalse(
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SampleRate"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SampleRate", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_sample_rate_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Audio|Development"', section)
        ...
        self.assertIn('Category="Skyguard|Audio|Development"', specifiers)
        ...
        self.assertIn('ClampMin="8000"', specifiers)
        self.assertIn('ClampMax="48000"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 SampleRate = 48000;", compact_origin)
        self.assertNotIn("SampleRate = 160", compact_origin)
    ...
    def test_sample_rate_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 SampleRate = 48000;")
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Audio|Development"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Development"', apache_specs)
        self.assertIn('Category="Skyguard|Apache"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
    ...
    def test_does_not_claim_sibling_epic_shrub_budget_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "SampleRate"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "SampleRate"))
        self.assertTrue(has_identifier(section, "SampleRate"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertTrue(has_identifier(leaked, SIBLING_ENABLE_AUDITION))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(leaked, SIBLING_ENABLE_AUDITION))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertTrue(has_identifier(leaked, SIBLING_BYTE_BUDGET))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="8000"', UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMax="48000"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertIn("SkyguardAudioProceduralBankComponent.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio procedural bank SampleRate field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_procedural_bank_sample_rate_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioProceduralBankComponent.cpp", THIS_SCRIPT)
