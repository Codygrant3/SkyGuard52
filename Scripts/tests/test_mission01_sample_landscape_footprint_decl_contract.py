...
CLASS_NAME = "USkyguardMission01LandscapeGroundingLibrary"
# Declaration presence only. Do not invent
# INDEX_NONE or lock SampleLandscapeFootprint
# construction in the .cpp. This is a leftover-safe
# Grounding METHOD contract on
# USkyguardMission01LandscapeGroundingLibrary. It is
# NOT leftover landscape-footprint-sample-defaults
# #af83 (do not lock the struct defaults), NOT
# leftover landscape-height-sample-defaults #46a3,
# NOT leftover environment-readiness defaults #6b9d /
# #b931, NOT leftover Environment GetReadiness, NOT
# leftover IsRouteExclusionSafe, and NOT leftover
# bUseAuthoredLandscapeSurface. Distinct from sibling
# SampleLandscapeHeight. Distinct from leftover
...
# HasContinuousCoastline. Distinct from leftover
# Integration methods InitializePlayableMission /
...
# SampleLandscapeFootprint(ALandscapeProxy*
# Landscape, const TArray<FVector>&
# WorldLocations);`); accept that form, other
# one-line / split-line wraps, and an inline body
...
# class section of
# USkyguardMission01LandscapeGroundingLibrary only.
# Category is Skyguard|Mission01|Environment|Grounding,
# not Environment|PCG, not Environment|Landscape,
...
# Mission01LandscapeGroundingLibrary public section
# and must be absent. Pathfinder MinHeightFromOriginCm
...
FOOTPRINT_SUCCESS_DEFAULT = "bool bSuccess = false;"
FOOTPRINT_REQUIRED_COUNT_DEFAULT = "int32 RequiredSampleCount = 0;"
FOOTPRINT_VALID_COUNT_DEFAULT = "int32 ValidSampleCount = 0;"
FOOTPRINT_SUPPORTED_FRACTION_DEFAULT = "float SupportedFraction = 0.f;"
FOOTPRINT_MIN_HEIGHT_DEFAULT = "float MinimumHeightCentimeters = 0.f;"
FOOTPRINT_MAX_HEIGHT_DEFAULT = "float MaximumHeightCentimeters = 0.f;"
FOOTPRINT_MEAN_HEIGHT_DEFAULT = "float MeanHeightCentimeters = 0.f;"
FOOTPRINT_HEIGHT_DELTA_DEFAULT = "float HeightDeltaCentimeters = 0.f;"
FOOTPRINT_SAMPLES_FIELD = (
    "TArray<FSkyguardLandscapeHeightSample> Samples;"
)
HEIGHT_VALID_DEFAULT = "bool bValid = false;"
HEIGHT_QUERY_LOCATION_DEFAULT = (
    "FVector QueryLocation = FVector::ZeroVector;"
)
HEIGHT_CENTIMETERS_DEFAULT = "float HeightCentimeters = 0.f;"
# Leftover #56–#64 plus leftover Harbor #6/#8/#9,
# leftover theater-kit #59, leftover #107–#619, plus
# SkyguardMission01LandscapeGroundingLibrary
# production files. This lane only adds an isolated
# Python SampleLandscapeFootprint method declaration
# contract on USkyguardMission01LandscapeGroundingLibrary.
LOCKED = {
...
# leftover environment-readiness defaults #6b9d /
# #b931, leftover landscape-footprint-sample-defaults
# #af83, leftover landscape-height-sample-defaults
# #46a3, leftover campaign-definition method
# contracts, sibling Mission01 Integration /
# Environment field and method contracts stay
# sibling-only.
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission01_refresh_authored_environment_bindings_decl_contract.py",
...
SIBLING_ENVIRONMENT_METHODS_NOT_LOCKED = (
    ...
    "test_mission01_refresh_authored_environment_bindings_decl_contract.py",
...
SIBLING_GROUNDING_HEIGHT_NOT_LOCKED = (
    "SampleLandscapeHeight",
    SAMPLE_LANDSCAPE_HEIGHT,
    "FSkyguardLandscapeHeightSample",
    "test_mission01_sample_landscape_height_decl_contract.py",
)
LEFTOVER_FOOTPRINT_SAMPLE_DEFAULTS_NOT_LOCKED = (
    FOOTPRINT_SUCCESS_DEFAULT,
    FOOTPRINT_REQUIRED_COUNT_DEFAULT,
    FOOTPRINT_VALID_COUNT_DEFAULT,
    FOOTPRINT_SUPPORTED_FRACTION_DEFAULT,
    FOOTPRINT_MIN_HEIGHT_DEFAULT,
    FOOTPRINT_MAX_HEIGHT_DEFAULT,
    FOOTPRINT_MEAN_HEIGHT_DEFAULT,
    FOOTPRINT_HEIGHT_DELTA_DEFAULT,
    FOOTPRINT_SAMPLES_FIELD,
    "bSuccess = false",
...
LEFTOVER_HEIGHT_SAMPLE_DEFAULTS_NOT_LOCKED = (
    HEIGHT_VALID_DEFAULT,
    HEIGHT_QUERY_LOCATION_DEFAULT,
    HEIGHT_CENTIMETERS_DEFAULT,
    "bValid = false",
...
class Mission01SampleLandscapeFootprintDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SAMPLE_LANDSCAPE_FOOTPRINT),
            section,
        )
    ...
    def test_missing_class_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            class_body(
                "class SKYGUARD52_API USkyguardUnrelatedLibrary "
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            "class SKYGUARD52_API UOtherGroundingLibrary "
            ": public UBlueprintFunctionLibrary\n"
            "{\n"
            "public:\n"
            f"\t{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn("SampleLandscapeFootprint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, SAMPLE_LANDSCAPE_FOOTPRINT)
    ...
    def test_missing_sample_landscape_footprint_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            f"\t{SAMPLE_LANDSCAPE_HEIGHT}\n"
            ...
            f"\t{FOOTPRINT_SUCCESS_DEFAULT}\n"
            f"\t{HEIGHT_VALID_DEFAULT}\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn("SampleLandscapeFootprint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_GROUNDING}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn("SampleLandscapeFootprint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_GROUNDING, section)
        ...
        self.assertNotIn("UFUNCTION", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("BlueprintCallable", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("BlueprintPure", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("Category", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("BlueprintCallable", UFUNCTION_GROUNDING)
        self.assertIn(
            "Skyguard|Mission01|Environment|Grounding",
            UFUNCTION_GROUNDING,
        )
        self.assertIn("BlueprintPure", UFUNCTION_GROUNDING)
        self.assertNotIn("Integration", UFUNCTION_GROUNDING)
        self.assertNotIn("Briefing", UFUNCTION_GROUNDING)
        self.assertNotIn("Mission02", UFUNCTION_GROUNDING)
        self.assertNotIn("Boss", UFUNCTION_GROUNDING)
        self.assertNotIn("Destruction", UFUNCTION_GROUNDING)
        self.assertNotIn("Apache", UFUNCTION_GROUNDING)
        self.assertNotIn("Mission07", UFUNCTION_GROUNDING)
        self.assertNotIn("Mission10", UFUNCTION_GROUNDING)
        self.assertNotIn("Encounter", UFUNCTION_GROUNDING)
        self.assertNotIn("Safety", UFUNCTION_GROUNDING)
        self.assertNotIn("|PCG", UFUNCTION_GROUNDING)
        self.assertNotIn("|Landscape", UFUNCTION_GROUNDING)
        self.assertNotIn("|Layout", UFUNCTION_GROUNDING)
        self.assertNotIn("|Materials", UFUNCTION_GROUNDING)
        self.assertNotIn("|Visibility", UFUNCTION_GROUNDING)
        self.assertIn("|Grounding", UFUNCTION_GROUNDING)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_GROUNDING)
            self.assertNotIn(invented, SAMPLE_LANDSCAPE_FOOTPRINT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn("SampleLandscapeFootprint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tstatic void SampleLandscapeFootprint("
            "ALandscapeProxy* Landscape, "
            "const TArray<FVector>& WorldLocations);\n"
        )
        missing_static = (
        ...
        height_return = (
            "\tstatic FSkyguardLandscapeHeightSample "
            "SampleLandscapeFootprint("
            "ALandscapeProxy* Landscape, "
            "const TArray<FVector>& WorldLocations);\n"
        )
        extra_arg = (
            ...
            "SampleLandscapeFootprint("
            "ALandscapeProxy* Landscape, "
            "const TArray<FVector>& WorldLocations, int32 Count);\n"
        )
        vector_not_array = (
            "\tstatic FSkyguardLandscapeFootprintSampleResult "
            "SampleLandscapeFootprint("
        ...
        as_const = (
            ...
            "SampleLandscapeFootprint("
            "ALandscapeProxy* Landscape, "
            "const TArray<FVector>& WorldLocations) const;\n"
        )
        renamed = (
            "\tstatic FSkyguardLandscapeFootprintSampleResult "
            "SampleFootprint("
        ...
        leftover_height = f"\t{SAMPLE_LANDSCAPE_HEIGHT}\n"
        ...
        leftover_layout = f"\t{REBUILD_PRODUCTION_LAYOUT}\n"
        leftover_refresh = f"\t{REFRESH_AUTHORED_BINDINGS}\n"
        ...
        leftover_success = f"\t{FOOTPRINT_SUCCESS_DEFAULT}\n"
        leftover_height_valid = f"\t{HEIGHT_VALID_DEFAULT}\n"
        for region in (
            as_void,
            missing_static,
            height_return,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SAMPLE_LANDSCAPE_FOOTPRINT)
            self.assertIn("SampleLandscapeFootprint", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_sample_landscape_footprint_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, SAMPLE_LANDSCAPE_FOOTPRINT))
        self.assertEqual(
            declaration_count(section, SAMPLE_LANDSCAPE_FOOTPRINT),
            1,
        )
        self.assertTrue(
            SAMPLE_LANDSCAPE_FOOTPRINT.startswith("static "),
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        self.assertTrue(
            SAMPLE_LANDSCAPE_FOOTPRINT.endswith(");"),
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        self.assertIn(
            "SampleLandscapeFootprint",
        ...
        self.assertIn("ALandscapeProxy*", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn(
            "const TArray<FVector>& WorldLocations",
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        self.assertNotIn("SampleLandscapeHeight", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn(
            "FSkyguardLandscapeHeightSample",
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        self.assertNotIn("TObjectPtr", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("TSoftObjectPtr", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("INDEX_NONE", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("UFUNCTION", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("{", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("}", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("return ", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn(" const;", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("void ", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("Root", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("Briefing", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("AudioDirector", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("RadioChatter", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("SortiePresentation", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("CampaignDefinition", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("MissionDefinition", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("Readiness", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("bAutoInitialize", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("bAllowBoundedActorSpawning", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("bAutoLaunchAfterBriefing", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("PathfinderSpawnLocation", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetAircraft", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("BindRuntimeActors", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("HandleDroneCityImpact", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("ConfigureMissionDefinition", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("InitializePlayableMission", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("NotifyProtectedAssetFailed", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("SynchronizeRuntimeState", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("NotifyObjectiveProgress", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetReadiness", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetObjectiveRuntime", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetGunner", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetPathfinder", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetMissionId", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("ValidateMissionContract", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("ConfigureFromMission", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("AdvanceBriefing", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("SetAssetsReady", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("AcknowledgeAndLaunch", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("CanLaunch", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetElapsedSeconds", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetBriefingState", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetMinimumWarmupSeconds", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetBriefingText", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetRadioChatter", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetPresentation", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("GetMissionTitle", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("AcknowledgeBriefing", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("LaunchSortie", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("HullCollider", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("OpticalTracker", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("WeaponServo", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("CountermeasurePod", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("MinHeightFromOriginCm", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("RadarNode", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn(
            "ESkyguardMission02WaveState",
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        self.assertNotIn("HarborIndustrial", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("MaxIntegrity", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("CurrentIntegrity", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("FillAndFinalize", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("FillAndFail", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("IsRouteExclusionSafe", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("bRouteExclusionValid", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        ...
        self.assertNotIn("IsPointAllowedForPCG", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("RebuildProductionLayout", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn(
            "RefreshAuthoredEnvironmentBindings",
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        ...
        self.assertNotIn("HasContinuousCoastline", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("IsCorePlayableReady", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("bSuccess = false", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("RequiredSampleCount = 0", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("bValid = false", SAMPLE_LANDSCAPE_FOOTPRINT)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, SAMPLE_LANDSCAPE_FOOTPRINT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_spaces = (
            "public:\n"
            "\tstatic   FSkyguardLandscapeFootprintSampleResult   "
            "SampleLandscapeFootprint(  ALandscapeProxy* Landscape,  "
            "const TArray<FVector>& WorldLocations);\n"
            "private:\n"
            "};\n"
        )
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_GROUNDING} {SAMPLE_LANDSCAPE_FOOTPRINT}\n"
            "};\n"
        )
        ...
        header_wrap_type = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public UBlueprintFunctionLibrary\n{{\n{wrap_type}"
        )
        header_wrap_spaces = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public UBlueprintFunctionLibrary\n{{\n{wrap_spaces}"
        ...
        one_line = f"{{\npublic:\n\t{SAMPLE_LANDSCAPE_FOOTPRINT}\n}}\n"
        self.assertTrue(has_declaration(one_line, SAMPLE_LANDSCAPE_FOOTPRINT))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SAMPLE_LANDSCAPE_FOOTPRINT),
            section,
    ...
    def test_wrong_category_does_not_satisfy_grounding(self) -> None:
        self.assertNotIn("Integration", UFUNCTION_GROUNDING)
        self.assertNotIn("Briefing", UFUNCTION_GROUNDING)
        integration = (
        ...
        self.assertNotEqual(integration, UFUNCTION_GROUNDING)
        self.assertNotEqual(briefing, UFUNCTION_GROUNDING)
        self.assertNotEqual(leftover_callable, UFUNCTION_GROUNDING)
        self.assertNotEqual(leftover_pcg, UFUNCTION_GROUNDING)
        self.assertNotEqual(leftover_landscape, UFUNCTION_GROUNDING)
        self.assertNotEqual(leftover_environment, UFUNCTION_GROUNDING)
        self.assertNotIn(integration, UFUNCTION_GROUNDING)
        self.assertNotIn(briefing, UFUNCTION_GROUNDING)
        self.assertNotIn(leftover_callable, UFUNCTION_GROUNDING)
        self.assertNotIn(leftover_pcg, UFUNCTION_GROUNDING)
        self.assertNotIn(leftover_landscape, UFUNCTION_GROUNDING)

    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, SAMPLE_LANDSCAPE_FOOTPRINT)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", SAMPLE_LANDSCAPE_FOOTPRINT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_sample_landscape_footprint_cpp_body(
        self,
    ) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        self.assertNotIn("{", SAMPLE_LANDSCAPE_FOOTPRINT)
        ...
        self.assertNotIn("return false", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("return true", SAMPLE_LANDSCAPE_FOOTPRINT)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn("SampleLandscapeFootprint", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertTrue(SAMPLE_LANDSCAPE_FOOTPRINT.startswith("static "))
    ...
    def test_contract_does_not_relock_sibling_grounding_height(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        for token in SIBLING_GROUNDING_HEIGHT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_footprint_sample_defaults(
        self,
    ) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        for token in LEFTOVER_FOOTPRINT_SAMPLE_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_height_sample_defaults(
        self,
    ) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        for token in LEFTOVER_HEIGHT_SAMPLE_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", SAMPLE_LANDSCAPE_FOOTPRINT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        self.assertEqual(
            require_declaration(locked_only, SAMPLE_LANDSCAPE_FOOTPRINT),
            SAMPLE_LANDSCAPE_FOOTPRINT,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SAMPLE_LANDSCAPE_FOOTPRINT)
        ...
        self.assertNotIn("RefreshAuthoredEnvironmentBindings", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 SampleLandscapeFootprint method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{SAMPLE_LANDSCAPE_FOOTPRINT}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SAMPLE_LANDSCAPE_FOOTPRINT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SAMPLE_LANDSCAPE_FOOTPRINT.lower(),
                "mission01 SampleLandscapeFootprint contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SAMPLE_LANDSCAPE_FOOTPRINT)

    def test_contract_is_sample_landscape_footprint_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(SAMPLE_LANDSCAPE_FOOTPRINT.endswith(");"))
        self.assertIn(UFUNCTION_GROUNDING, section)

    def test_sibling_director_fields_do_not_satisfy_sample_landscape_footprint(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, SAMPLE_LANDSCAPE_FOOTPRINT)
            )
            self.assertNotEqual(SAMPLE_LANDSCAPE_FOOTPRINT, leftover)
        self.assertIn(
    ...
    def test_sibling_grounding_height_does_not_satisfy(self) -> None:
        region = f"\t{SAMPLE_LANDSCAPE_HEIGHT}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertIn("SampleLandscapeFootprint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, SAMPLE_LANDSCAPE_FOOTPRINT))
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("return ", SAMPLE_LANDSCAPE_FOOTPRINT)
        self.assertNotIn("return Other", SAMPLE_LANDSCAPE_FOOTPRINT)
    ...
    def test_declaration_accepts_split_parameter_wrap(self) -> None:
        ...
        for wrap in (wrap_params, wrap_type_and_params, wrap_ufunction_split):
            header = (
                f"class SKYGUARD52_API {CLASS_NAME} "
                f": public UBlueprintFunctionLibrary\n{{\n{wrap}"
            )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_mission01_has_continuous_coastline"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission01_is_route_exclusion_safe"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
