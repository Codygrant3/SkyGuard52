...
# INDEX_NONE or lock HangarRack construction
...
# (split tokens only). Keep HangarRack
# distinct from sibling RunwayRack. Keep
# RunwayBreaker DebrisEngine / PortEngine
...
# satisfy the primary lock. This is not a
# leftover USkyguardBossWeakPointComponent member
# contract. Do not lock leftover accept flags
# on that component. Do not open
# SkyguardBossWeakPointComponent.h as the
# locked header. origin/main is a one-line field
# (`TObjectPtr<USkyguardBossWeakPointComponent>
# HangarRack;`); accept that form and other
...
# isolated-test drafts #107–#553 including
...
# Leftover RoadHunter field. Same Engine declaration text,
...
# leftover #107–#553, plus
# SkyguardRoadHunterBoss production files.
# This lane only adds an isolated Python
# HangarRack field declaration contract on
...
# leftover isolated-test drafts #107–#553,
# leftover searchlight-track-runtime-defaults
...
# unlocked for THIS test's primary HangarRack lock. Leftover
...
LEFTOVER_RUNWAY_RACK_NOT_LOCKED = (
    "test_runway_breaker_runway_rack_field_decl_contract.py",
    "RunwayRack",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRunwayBreakerBoss::HangarRack",
    "SkyguardRunwayBreakerBoss.cpp",
    "SetExposed",
...
class RunwayBreakerHangarRackFieldDeclContractTests(unittest.TestCase):
    def test_runway_breaker_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, HANGAR_RACK_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, HANGAR_RACK_FIELD)
    ...
    def test_missing_hangar_rack_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRunwayBreakerBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", HANGAR_RACK_FIELD)
        self.assertNotIn("VisibleAnywhere", HANGAR_RACK_FIELD)
        self.assertNotIn("BlueprintReadOnly", HANGAR_RACK_FIELD)
        self.assertNotIn("Category", HANGAR_RACK_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, HANGAR_RACK_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* HangarRack;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> HangarRack;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> HangarRack;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, HANGAR_RACK_FIELD)
            self.assertIn("HangarRack", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_hangar_rack_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, HANGAR_RACK_FIELD),
            HANGAR_RACK_FIELD,
        )
        self.assertTrue(has_declaration(section, HANGAR_RACK_FIELD))
        self.assertEqual(declaration_count(section, HANGAR_RACK_FIELD), 1)
        self.assertTrue(
            HANGAR_RACK_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            HANGAR_RACK_FIELD,
        )
        self.assertTrue(HANGAR_RACK_FIELD.endswith(";"), HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)
        self.assertNotIn("RunwayRack", HANGAR_RACK_FIELD)
        self.assertNotIn("HeatManifold", HANGAR_RACK_FIELD)
        self.assertNotIn("PortEngine", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisPortWing", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisPayloadBay", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisEngine", HANGAR_RACK_FIELD)
        self.assertNotIn("TargetingCamera", HANGAR_RACK_FIELD)
        self.assertNotIn("LeftActuator", HANGAR_RACK_FIELD)
        self.assertNotIn("RightActuator", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisLeftWing", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisRightWing", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisCoolingDoor", HANGAR_RACK_FIELD)
        self.assertNotIn("SignatureModulator", HANGAR_RACK_FIELD)
        self.assertNotIn("RadarReceiver", HANGAR_RACK_FIELD)
        self.assertNotIn("CoolingDoor", HANGAR_RACK_FIELD)
        self.assertNotIn("PortLatch", HANGAR_RACK_FIELD)
        self.assertNotIn("NoseCamera", HANGAR_RACK_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            HANGAR_RACK_FIELD,
        )
        self.assertIn("TObjectPtr<", HANGAR_RACK_FIELD)
        self.assertNotIn("TArray<", HANGAR_RACK_FIELD)
        self.assertNotIn("=", HANGAR_RACK_FIELD)
        self.assertNotIn("INDEX_NONE", HANGAR_RACK_FIELD)
        self.assertNotIn("UFUNCTION", HANGAR_RACK_FIELD)
        self.assertNotIn("{", HANGAR_RACK_FIELD)
        self.assertNotIn("}", HANGAR_RACK_FIELD)
        self.assertNotIn("return ", HANGAR_RACK_FIELD)
        self.assertNotIn("CommandAntenna", HANGAR_RACK_FIELD)
        self.assertNotIn("ControlLinkage", HANGAR_RACK_FIELD)
        self.assertNotIn("NoseCamera", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisCenter", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisTail", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisNose", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisSpine", HANGAR_RACK_FIELD)
        self.assertNotIn("AdvanceEncounter", HANGAR_RACK_FIELD)
        self.assertNotIn("ResetEncounterState", HANGAR_RACK_FIELD)
        self.assertNotIn("IsRouteStateSafe", HANGAR_RACK_FIELD)
        self.assertNotIn("GetRouteProgress", HANGAR_RACK_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", HANGAR_RACK_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", HANGAR_RACK_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", HANGAR_RACK_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", HANGAR_RACK_FIELD)
        self.assertNotIn("RadarNode", HANGAR_RACK_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", HANGAR_RACK_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", HANGAR_RACK_FIELD)
        self.assertNotIn("IssueClimbCommand", HANGAR_RACK_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", HANGAR_RACK_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", HANGAR_RACK_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", HANGAR_RACK_FIELD)
        self.assertNotIn("GetFinaleStage", HANGAR_RACK_FIELD)
        self.assertNotIn("IsClimbCommandIssued", HANGAR_RACK_FIELD)
        self.assertNotIn("IsWreckDiverted", HANGAR_RACK_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", HANGAR_RACK_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", HANGAR_RACK_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", HANGAR_RACK_FIELD)
        self.assertNotIn("GetGunnerMount", HANGAR_RACK_FIELD)
        self.assertNotIn("FaceWorldLocation", HANGAR_RACK_FIELD)
        self.assertNotIn("PortGuidanceArray", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisArmorPort", HANGAR_RACK_FIELD)
        self.assertNotIn("SetSearchlightTracked", HANGAR_RACK_FIELD)
        self.assertNotIn("SetContactIdentified", HANGAR_RACK_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", HANGAR_RACK_FIELD)
        self.assertNotIn("SetLightningExposed", HANGAR_RACK_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", HANGAR_RACK_FIELD)
        self.assertNotIn("HullCollider", HANGAR_RACK_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, HANGAR_RACK_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {HANGAR_RACK_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, HANGAR_RACK_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{HANGAR_RACK_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, HANGAR_RACK_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, HANGAR_RACK_FIELD),
            section,
    ...
    def test_assigned_hangar_rack_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "HangarRack = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, HANGAR_RACK_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("HangarRack", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, HANGAR_RACK_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{HANGAR_RACK_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, HANGAR_RACK_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, HANGAR_RACK_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            HANGAR_RACK_FIELD.startswith("UFUNCTION"),
            HANGAR_RACK_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, HANGAR_RACK_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", HANGAR_RACK_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_hangar_rack_cpp_body(self) -> None:
        locked_only = f"{HANGAR_RACK_FIELD}\n"
        self.assertNotIn("{", HANGAR_RACK_FIELD)
        self.assertNotIn("}", HANGAR_RACK_FIELD)
        self.assertNotIn("return ", HANGAR_RACK_FIELD)
        ...
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", HANGAR_RACK_FIELD)
        self.assertNotIn("SkyguardRunwayBreakerBoss.cpp", locked_only)
        self.assertNotIn("return false", HANGAR_RACK_FIELD)
        self.assertNotIn("return true", HANGAR_RACK_FIELD)
        self.assertNotIn("CreateDefaultSubobject", HANGAR_RACK_FIELD)
        self.assertNotIn("SetExposed", HANGAR_RACK_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            HANGAR_RACK_FIELD,
        )
        self.assertNotIn("SetBossPhase", HANGAR_RACK_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", HANGAR_RACK_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{HANGAR_RACK_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("PortDischargeBoom", HANGAR_RACK_FIELD)
        self.assertNotIn("StarboardDischargeBoom", HANGAR_RACK_FIELD)
        self.assertNotIn("EngineIntake", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisPortPanel", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisStarboardPanel", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisIntakePanel", HANGAR_RACK_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", HANGAR_RACK_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            HANGAR_RACK_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("EncounterController", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("ControlServo", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("DispenserCenter", HANGAR_RACK_FIELD)
        self.assertNotIn("MaxReleasesPerBay", HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("SetContactIdentified", HANGAR_RACK_FIELD)
        self.assertNotIn("OpenOrbitExposure", HANGAR_RACK_FIELD)
        self.assertNotIn("IsContactIdentified", HANGAR_RACK_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), HANGAR_RACK_FIELD)

    def test_debris_cooling_door_does_not_satisfy_engine(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HANGAR_RACK_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, HANGAR_RACK_FIELD)
        self.assertNotIn("UStaticMeshComponent", HANGAR_RACK_FIELD)
        self.assertNotIn(
            "USkyguardBossWeakPointComponent",
            DEBRIS_COOLING_DOOR_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("DispenserStarboard", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("GetManeuver", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), HANGAR_RACK_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", HANGAR_RACK_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("SetSearchlightTracked", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        self.assertNotIn("CoolingDoor", HANGAR_RACK_FIELD)
        self.assertNotIn("SignatureModulator", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("SetLightningExposed", HANGAR_RACK_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", HANGAR_RACK_FIELD)
        self.assertNotIn("IsLightningExposed", HANGAR_RACK_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), HANGAR_RACK_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, HANGAR_RACK_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardGunner", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("SkyguardRadarNode", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANGAR_RACK_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", HANGAR_RACK_FIELD)
        self.assertNotIn("MaxIntegrity", HANGAR_RACK_FIELD)
        self.assertNotIn("CurrentIntegrity", HANGAR_RACK_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", HANGAR_RACK_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            HANGAR_RACK_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{HANGAR_RACK_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, HANGAR_RACK_FIELD),
            HANGAR_RACK_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRunwayBreakerBoss::HangarRack", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "runway breaker HangarRack field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{HANGAR_RACK_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, HANGAR_RACK_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                HANGAR_RACK_FIELD.lower(),
                "runway breaker HangarRack contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, HANGAR_RACK_FIELD)

    def test_contract_is_hangar_rack_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(HANGAR_RACK_FIELD.endswith(";"))
        self.assertNotIn("=", HANGAR_RACK_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_runway_breaker_fields(
        ...
        self.assertNotIn("DebrisEngine", HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)
        self.assertNotEqual(HANGAR_RACK_FIELD, RUNWAY_RACK_FIELD)
    ...
    def test_hangar_rack_is_distinct_from_runway_rack(self) -> None:
        leftover = f"\t{RUNWAY_RACK_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HANGAR_RACK_FIELD))
        self.assertNotEqual(RUNWAY_RACK_FIELD, HANGAR_RACK_FIELD)
        self.assertNotIn("RunwayRack", HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)
        self.assertTrue(RUNWAY_RACK_FIELD.endswith("RunwayRack;"))
        self.assertTrue(HANGAR_RACK_FIELD.endswith("HangarRack;"))
        self.assertIn(
            "Scripts/tests/test_runway_breaker_runway_rack_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        ...
        for token in LEFTOVER_RUNWAY_RACK_NOT_LOCKED:
            self.assertNotIn(token, HANGAR_RACK_FIELD)

    def test_heat_manifold_does_not_satisfy_hangar_rack(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HANGAR_RACK_FIELD))
        self.assertNotEqual(HEAT_MANIFOLD_FIELD, HANGAR_RACK_FIELD)
        self.assertNotIn("HeatManifold", HANGAR_RACK_FIELD)

    def test_port_engine_does_not_satisfy_hangar_rack(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HANGAR_RACK_FIELD))
        self.assertNotEqual(PORT_ENGINE_FIELD, HANGAR_RACK_FIELD)
        self.assertNotIn("PortEngine", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        self.assertNotIn("DebrisEngine", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisRightWing", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardRoadHunterBoss", HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("PortLatch", HANGAR_RACK_FIELD)
        self.assertNotIn("StarboardLatch", HANGAR_RACK_FIELD)
        self.assertNotIn("DecoyPods", HANGAR_RACK_FIELD)
        self.assertNotIn("ElevatorLinkage", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", HANGAR_RACK_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_runway_breaker_port_engine_is_not_leftover_engine(self) -> None:
        locked_only = f"{HANGAR_RACK_FIELD}\n"
        self.assertNotEqual(PORT_ENGINE_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotEqual(PORT_ENGINE_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(PORT_ENGINE_FIELD, RADAR_GHOST_ENGINE_FIELD)
        ...
        self.assertEqual(collapsed(ROAD_HUNTER_ENGINE_FIELD), collapsed(ENGINE_FIELD))
        ...
        self.assertNotIn("ASkyguardBreakwaterBoss", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss", HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)
        self.assertNotIn("PortEngine", HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisEngine", HANGAR_RACK_FIELD)
        leftover = f"\t{PORT_ENGINE_FIELD}\n"
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", HANGAR_RACK_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HANGAR_RACK_FIELD)
        self.assertNotIn("OpticalTracker", HANGAR_RACK_FIELD)
        self.assertNotIn("WeaponServo", HANGAR_RACK_FIELD)
        self.assertNotIn("CountermeasurePod", HANGAR_RACK_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", HANGAR_RACK_FIELD)
    ...
    def test_debris_engine_does_not_satisfy_hangar_rack(self) -> None:
        leftover = f"\t{RUNWAY_BREAKER_DEBRIS_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
    ...
    def test_debris_port_wing_does_not_satisfy_hangar_rack(self) -> None:
        leftover = f"\t{RUNWAY_BREAKER_DEBRIS_PORT_WING_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_debris_payload_bay_does_not_satisfy_hangar_rack(self) -> None:
        leftover = f"\t{DEBRIS_PAYLOAD_BAY_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HANGAR_RACK_FIELD))
        self.assertNotEqual(DEBRIS_PAYLOAD_BAY_FIELD, HANGAR_RACK_FIELD)
        self.assertNotIn("DebrisPayloadBay", HANGAR_RACK_FIELD)

    def test_targeting_camera_does_not_satisfy_hangar_rack(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, HANGAR_RACK_FIELD))
        self.assertNotEqual(TARGETING_CAMERA_FIELD, HANGAR_RACK_FIELD)
        self.assertNotIn("TargetingCamera", HANGAR_RACK_FIELD)
        self.assertIn("HangarRack", HANGAR_RACK_FIELD)

    def test_left_and_right_actuators_do_not_satisfy_hangar_rack(self) -> None:
        for region, name in (
            (f"\t{LEFT_ACTUATOR_FIELD}\n", "LeftActuator"),
            (f"\t{RIGHT_ACTUATOR_FIELD}\n", "RightActuator"),
        ):
    ...
    def test_road_hunter_and_breakwater_engine_do_not_satisfy_hangar_rack(
        self,
    ) -> None:
        for region, name in (
            (f"\t{ROAD_HUNTER_ENGINE_FIELD}\n", "ASkyguardRoadHunterBoss"),
            ...
            self.assertFalse(has_declaration(region, HANGAR_RACK_FIELD))
            self.assertNotIn(name, HANGAR_RACK_FIELD)
        self.assertNotEqual(HANGAR_RACK_FIELD, ROAD_HUNTER_ENGINE_FIELD)
        self.assertNotEqual(HANGAR_RACK_FIELD, BREAKWATER_ENGINE_FIELD)
