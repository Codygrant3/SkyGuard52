...
# INDEX_NONE or lock StartSearchlightWindow
# construction in the .cpp. This is leftover-safe
# Mission04 Searchlight StartSearchlightWindow on
...
# leftover GetSearchlightRuntime. Distinct from
# leftover Mission04 wave-state enum #bb22
# (do not lock enum values). Distinct from
# leftover Mission03 StartNextWave #630. Distinct
# from leftover Mission04 StartNextWave. Not
...
# Mission03 Waves StartNextWave #630, not leftover
# Mission04 Waves StartNextWave, not leftover
# GetSearchlightRuntime, not leftover
# searchlight-track-runtime-defaults #7347.
...
# leftover Mission03 StartNextWave #630.
...
# StartSearchlightWindow method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission04IntegrationDirector::StartSearchlightWindow",
    "SkyguardMission04IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission04StartSearchlightWindowDeclContractTests(unittest.TestCase):
    def test_mission04_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, START_SEARCHLIGHT_WINDOW),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, START_SEARCHLIGHT_WINDOW)
        self.assertIn("StartSearchlightWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, START_SEARCHLIGHT_WINDOW)
    ...
    def test_missing_start_searchlight_window_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission04IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, START_SEARCHLIGHT_WINDOW)
        self.assertIn("StartSearchlightWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, START_SEARCHLIGHT_WINDOW)
        self.assertIn("StartSearchlightWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("BlueprintPure", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("Category", START_SEARCHLIGHT_WINDOW)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, START_SEARCHLIGHT_WINDOW)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, START_SEARCHLIGHT_WINDOW)
        self.assertIn("StartSearchlightWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid StartSearchlightWindow(float WindowSeconds);\n"
        as_const = "\tbool StartSearchlightWindow(float WindowSeconds) const;\n"
        as_int = "\tint32 StartSearchlightWindow(float WindowSeconds);\n"
        with_arg = "\tbool StartSearchlightWindow(int32 Amount = 1);\n"
        ...
        as_void_name = "\tvoid StartSearchlightWindow(float WindowSeconds) const;\n"
        renamed = "\tbool StartSearchWindow();\n"
        short_name = "\tbool SearchlightWindow();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, START_SEARCHLIGHT_WINDOW)
            self.assertIn("StartSearchlightWindow", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_start_searchlight_window_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, START_SEARCHLIGHT_WINDOW),
            START_SEARCHLIGHT_WINDOW,
        )
        self.assertTrue(has_declaration(section, START_SEARCHLIGHT_WINDOW))
        self.assertEqual(
            declaration_count(section, START_SEARCHLIGHT_WINDOW),
            1,
        )
        self.assertTrue(
            START_SEARCHLIGHT_WINDOW.startswith("bool "),
            START_SEARCHLIGHT_WINDOW,
        )
        self.assertTrue(
            START_SEARCHLIGHT_WINDOW.endswith("(float WindowSeconds);"),
            START_SEARCHLIGHT_WINDOW,
        )
        self.assertIn("StartSearchlightWindow", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("static ", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn(
            "const USkyguardMissionDefinition* Mission",
            START_SEARCHLIGHT_WINDOW,
        )
        ...
        self.assertNotIn("EnvironmentReadiness", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("MissionMapReadiness", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("TObjectPtr", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("TSoftObjectPtr", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("INDEX_NONE", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("UFUNCTION", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("{", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("}", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("return ", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn(" const;", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("Root", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("Briefing", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("AudioDirector", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("RadioChatter", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("SortiePresentation", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("CampaignDefinition", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("TSoftObjectPtr<USkyguardMissionDefinition>", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("Readiness;", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("bAutoInitialize", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("bAllowBoundedActorSpawning", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("bAutoLaunchAfterBriefing", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("PathfinderSpawnLocation", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetAircraft", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("BindRuntimeActors", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("HandleDroneCityImpact", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ConfigureMissionDefinition", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("InitializePlayableMission", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("NotifyProtectedAssetFailed", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("SynchronizeRuntimeState", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("NotifyObjectiveProgress", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("IsCorePlayableReady", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetObjectiveRuntime", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetGunner", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetPathfinder", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetReadiness", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetMissionId", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ValidateDefinition", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ConfigureFromMission", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("AdvanceBriefing", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("SetAssetsReady", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("AcknowledgeAndLaunch", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("CanLaunch", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetElapsedSeconds", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetBriefingState", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetMinimumWarmupSeconds", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetBriefingText", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetRadioChatter", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetPresentation", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetMissionTitle", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("AcknowledgeBriefing", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("LaunchSortie", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("HullCollider", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("OpticalTracker", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("WeaponServo", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("CountermeasurePod", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("MinHeightFromOriginCm", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("RadarNode", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ESkyguardMission02WaveState", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ESkyguardMission03WaveState", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("NotifyThreatDestroyed", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetWaveState", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetCurrentWaveIndex", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetRemainingThreatsInWave", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("AdvanceConvoyByDistance", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("NotifyConvoyDamage", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ValidateMissionContract", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("StartNextWave", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("AdvanceSearchlightTrack", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("NotifySubstationDamage", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetSearchlightRuntime", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetSubstationIntegrity", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("GetNightBeatKit", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ESkyguardMission04WaveState", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("HarborIndustrial", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("MaxIntegrity", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("CurrentIntegrity", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("FillAndFinalize", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("FillAndFail", START_SEARCHLIGHT_WINDOW)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, START_SEARCHLIGHT_WINDOW)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tStartSearchlightWindow(float WindowSeconds);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction = (
            ...
            f"\t{START_SEARCHLIGHT_WINDOW}\n"
            "};\n"
        )
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SEARCHLIGHT} {START_SEARCHLIGHT_WINDOW}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{START_SEARCHLIGHT_WINDOW}\n}}\n"
        self.assertTrue(has_declaration(one_line, START_SEARCHLIGHT_WINDOW))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, START_SEARCHLIGHT_WINDOW),
            section,
    ...
    def test_environment_category_does_not_satisfy_searchlight(self) -> None:
        ...
        self.assertNotEqual(waves, UFUNCTION_SEARCHLIGHT)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{START_SEARCHLIGHT_WINDOW}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, START_SEARCHLIGHT_WINDOW)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", START_SEARCHLIGHT_WINDOW)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_start_searchlight_window_cpp_body(self) -> None:
        locked_only = f"{START_SEARCHLIGHT_WINDOW}\n"
        self.assertNotIn("{", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("}", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("return ", START_SEARCHLIGHT_WINDOW)
        ...
        self.assertNotIn("return false", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("return true", START_SEARCHLIGHT_WINDOW)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{START_SEARCHLIGHT_WINDOW}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, START_SEARCHLIGHT_WINDOW)
        self.assertIn("StartSearchlightWindow", START_SEARCHLIGHT_WINDOW)
        self.assertTrue(START_SEARCHLIGHT_WINDOW.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, START_SEARCHLIGHT_WINDOW)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, START_SEARCHLIGHT_WINDOW)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("CurrentIntegrity", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("SkyguardApacheAircraft.h", START_SEARCHLIGHT_WINDOW)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", START_SEARCHLIGHT_WINDOW)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{START_SEARCHLIGHT_WINDOW}\n"
        self.assertEqual(
            require_declaration(locked_only, START_SEARCHLIGHT_WINDOW),
            START_SEARCHLIGHT_WINDOW,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, START_SEARCHLIGHT_WINDOW)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission04 StartSearchlightWindow method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{START_SEARCHLIGHT_WINDOW}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, START_SEARCHLIGHT_WINDOW.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                START_SEARCHLIGHT_WINDOW.lower(),
                "mission04 StartSearchlightWindow contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, START_SEARCHLIGHT_WINDOW)

    def test_contract_is_start_searchlight_window_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("TArray<FText>& OutErrors", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("static ", START_SEARCHLIGHT_WINDOW)
        self.assertTrue(
            START_SEARCHLIGHT_WINDOW.endswith(");"),
            START_SEARCHLIGHT_WINDOW,
        )
        self.assertTrue(START_SEARCHLIGHT_WINDOW.endswith(";"))
        self.assertNotIn("EnvironmentReadiness", START_SEARCHLIGHT_WINDOW)
        self.assertNotIn("MissionMapReadiness", START_SEARCHLIGHT_WINDOW)
        self.assertIn(UFUNCTION_SEARCHLIGHT, section)

    def test_sibling_director_fields_do_not_satisfy_start_searchlight_window(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, START_SEARCHLIGHT_WINDOW)
            )
            self.assertNotEqual(START_SEARCHLIGHT_WINDOW, leftover)
        self.assertIn(
    ...
    def test_leftover_mission_definition_validate_does_not_satisfy(self) -> None:
        ...
        self.assertNotIn(MISSION_DEFINITION_VALIDATE, START_SEARCHLIGHT_WINDOW)
        self.assertIn(
            "Scripts/tests/test_mission_definition_validate_definition"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
