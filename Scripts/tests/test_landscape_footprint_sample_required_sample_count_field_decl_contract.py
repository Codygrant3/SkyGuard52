...
# RequiredSampleCount FIELD
# THIS IS NOT leftover LandscapeHeightSample RequiredSampleCount.
...
# BlueprintReadOnly int32 RequiredSampleCount = 0; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# RequiredSampleCount FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 RequiredSampleCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# RequiredSampleCount FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# int32 RequiredSampleCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeHeightSample RequiredSampleCount
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 RequiredSampleCount = 0;
...
# RequiredSampleCount FIELD (declaration-only). It is
...
TARGET = "int32 RequiredSampleCount = 0;"
TARGET_WRONG = "float RequiredSampleCount = 0;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+RequiredSampleCount\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 RequiredSampleCount is missing from "
...
class LandscapeFootprintSampleRequiredSampleCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RequiredSampleCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_required_sample_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_required_sample_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RequiredSampleCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("RequiredSampleCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("RequiredSampleCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 RequiredSampleCount"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("RequiredSampleCount", locked_only)
