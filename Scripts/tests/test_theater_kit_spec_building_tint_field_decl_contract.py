# THIS IS leftover-safe FSkyguardTheaterKitSpec BuildingTint.
...
# LandmarkSet / BuildingKit / LampTreatment / LampColor /
# LampIntensity / RoadTreatment / RoadTint / NamedLandmark /
# LandmarkTint / SilhouetteKit / SilhouetteTint /
...
# the bulk).
# THIS IS NOT leftover sibling drafts #1299 / #1300 / #1301.
...
TARGET = "FLinearColor BuildingTint = FLinearColor::White;"
TARGET_WRONG_EQ_NONE = "FLinearColor BuildingTint = NAME_None;"
TARGET_WRONG_FALSE = "FLinearColor BuildingTint = false;"
TARGET_WRONG_TRUE = "FLinearColor BuildingTint = true;"
TARGET_WRONG_ZERO = "FLinearColor BuildingTint = 0.f;"
TARGET_WRONG_FLOAT = "float BuildingTint;"
TARGET_WRONG_BARE = "FLinearColor BuildingTint;"
TARGET_WRONG_FNAME = "FName BuildingTint;"
TARGET_WRONG_BLACK = "FLinearColor BuildingTint = FLinearColor::Black;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # LandmarkSet / BuildingKit fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FName\s+BuildingTint\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BuildingTint\b",
        compact,
    ):
        return False
    if re.search(
        r"FLinearColor\s+BuildingTint\s*;",
        compact,
    ):
        return False
    assigned = re.findall(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FLinearColor\s+BuildingTint\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FLinearColor BuildingTint is missing from "
...
class TheaterKitSpecBuildingTintFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BuildingTint"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BuildingTint"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("BuildingTint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_building_tint_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_decl_fails_closed_when_origin_has_white_default(self) -> None:
        ...
        self.assertNotIn("BuildingTint = NAME_None", compact_origin)
        self.assertNotIn("BuildingTint = false", compact_origin)
        self.assertNotIn("BuildingTint = true", compact_origin)
        self.assertNotIn("BuildingTint = 0.f", compact_origin)
        self.assertNotIn("BuildingTint = 160.f", compact_origin)
        self.assertNotIn("BuildingTint = FLinearColor::Black", compact_origin)

    def test_building_tint_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FLinearColor BuildingTint"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFLinearColor BuildingTints = FLinearColor::White;\n",
            "\tint32 BuildingTint;\n",
            "\tbool BuildingTint;\n",
            "\tfloat BuildingTint = " + forty + ";\n",
            "\tfloat BuildingTint = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tFLinearColor\n\tBuildingTint = FLinearColor::White;\n",
            "\tFLinearColor   BuildingTint = FLinearColor::White;\n",
            "\tFLinearColor\tBuildingTint = FLinearColor::White;\n",
            "\tFLinearColor BuildingTint =\n\tFLinearColor::White;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_VISIBLE_READONLY}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec BuildingTint field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "building_tint_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
