...
# INDEX_NONE or lock StarboardLatch construction
# in the .cpp. This is a FIELD contract on
# ASkyguardBreakwaterBoss, not leftover Breakwater
# sibling fields PortLatch / DecoyPods / Engine /
# ElevatorLinkage / DebrisPortPanel /
# DebrisEngine / DebrisStarboardPanel and not
...
# isolated-test drafts #107–#539 including
...
# leftover isolated-test drafts #107–#539,
# leftover searchlight-track-runtime-defaults
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBreakwaterBoss::StarboardLatch",
    "SkyguardBreakwaterBoss.cpp",
    "SetExposed",
...
class BreakwaterStarboardLatchFieldDeclContractTests(unittest.TestCase):
    def test_breakwater_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, STARBOARD_LATCH_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, STARBOARD_LATCH_FIELD)
    ...
    def test_missing_starboard_latch_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardBreakwaterBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_LATCH}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_LATCH, section)
        self.assertIn("VisibleAnywhere", section)
        self.assertIn("BlueprintReadOnly", section)
        self.assertIn('Category="Skyguard|Mission02|Boss"', section)
        self.assertTrue(
            has_declaration(section, STARBOARD_LATCH_FIELD),
            section,
        )
        self.assertNotIn("UPROPERTY", STARBOARD_LATCH_FIELD)
        self.assertNotIn("VisibleAnywhere", STARBOARD_LATCH_FIELD)
        self.assertNotIn("BlueprintReadOnly", STARBOARD_LATCH_FIELD)
        self.assertNotIn("Category", STARBOARD_LATCH_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_LATCH)
        self.assertNotIn("BlueprintCallable", UPROPERTY_LATCH)
        self.assertIn("Skyguard|Mission02|Boss", UPROPERTY_LATCH)
        self.assertIn("Mission02", UPROPERTY_LATCH)
        self.assertNotIn("Destruction", UPROPERTY_LATCH)
        self.assertNotIn("Encounter", UPROPERTY_LATCH)
        self.assertNotIn("Mission10", UPROPERTY_LATCH)
        self.assertNotIn("Safety", UPROPERTY_LATCH)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_LATCH)
            self.assertNotIn(invented, STARBOARD_LATCH_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* StarboardLatch;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> StarboardLatch;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> StarboardLatch;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, STARBOARD_LATCH_FIELD)
            self.assertIn("StarboardLatch", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_starboard_latch_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, STARBOARD_LATCH_FIELD),
            STARBOARD_LATCH_FIELD,
        )
        self.assertTrue(has_declaration(section, STARBOARD_LATCH_FIELD))
        self.assertEqual(declaration_count(section, STARBOARD_LATCH_FIELD), 1)
        self.assertTrue(
            STARBOARD_LATCH_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            STARBOARD_LATCH_FIELD,
        )
        self.assertTrue(STARBOARD_LATCH_FIELD.endswith(";"), STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", STARBOARD_LATCH_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_LATCH_FIELD,
        )
        self.assertIn("TObjectPtr<", STARBOARD_LATCH_FIELD)
        self.assertNotIn("TArray<", STARBOARD_LATCH_FIELD)
        self.assertNotIn("=", STARBOARD_LATCH_FIELD)
        self.assertNotIn("INDEX_NONE", STARBOARD_LATCH_FIELD)
        self.assertNotIn("UFUNCTION", STARBOARD_LATCH_FIELD)
        self.assertNotIn("{", STARBOARD_LATCH_FIELD)
        self.assertNotIn("}", STARBOARD_LATCH_FIELD)
        self.assertNotIn("return ", STARBOARD_LATCH_FIELD)
        self.assertNotIn("CommandAntenna", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ControlLinkage", STARBOARD_LATCH_FIELD)
        self.assertNotIn("NoseCamera", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisCenter", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisTail", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisNose", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisSpine", STARBOARD_LATCH_FIELD)
        self.assertNotIn("AdvanceEncounter", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ResetEncounterState", STARBOARD_LATCH_FIELD)
        self.assertNotIn("IsRouteStateSafe", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetRouteProgress", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", STARBOARD_LATCH_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", STARBOARD_LATCH_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", STARBOARD_LATCH_FIELD)
        self.assertNotIn("RadarNode", STARBOARD_LATCH_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", STARBOARD_LATCH_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", STARBOARD_LATCH_FIELD)
        self.assertNotIn("IssueClimbCommand", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetFinaleStage", STARBOARD_LATCH_FIELD)
        self.assertNotIn("IsClimbCommandIssued", STARBOARD_LATCH_FIELD)
        self.assertNotIn("IsWreckDiverted", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetGunnerMount", STARBOARD_LATCH_FIELD)
        self.assertNotIn("FaceWorldLocation", STARBOARD_LATCH_FIELD)
        self.assertNotIn("PortGuidanceArray", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisArmorPort", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetLightningExposed", STARBOARD_LATCH_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", STARBOARD_LATCH_FIELD)
        self.assertNotIn("HullCollider", STARBOARD_LATCH_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, STARBOARD_LATCH_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_LATCH} {STARBOARD_LATCH_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, STARBOARD_LATCH_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{STARBOARD_LATCH_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, STARBOARD_LATCH_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, STARBOARD_LATCH_FIELD),
            section,
    ...
    def test_assigned_starboard_latch_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardLatch = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, STARBOARD_LATCH_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("StarboardLatch", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, STARBOARD_LATCH_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{STARBOARD_LATCH_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, STARBOARD_LATCH_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_LATCH, section)
        self.assertTrue(has_declaration(section, STARBOARD_LATCH_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            STARBOARD_LATCH_FIELD.startswith("UFUNCTION"),
            STARBOARD_LATCH_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, STARBOARD_LATCH_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", STARBOARD_LATCH_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_starboard_latch_cpp_body(self) -> None:
        locked_only = f"{STARBOARD_LATCH_FIELD}\n"
        self.assertNotIn("{", STARBOARD_LATCH_FIELD)
        self.assertNotIn("}", STARBOARD_LATCH_FIELD)
        self.assertNotIn("return ", STARBOARD_LATCH_FIELD)
        ...
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", locked_only)
        self.assertNotIn("return false", STARBOARD_LATCH_FIELD)
        self.assertNotIn("return true", STARBOARD_LATCH_FIELD)
        self.assertNotIn("CreateDefaultSubobject", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetExposed", STARBOARD_LATCH_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            STARBOARD_LATCH_FIELD,
        )
        self.assertNotIn("SetBossPhase", STARBOARD_LATCH_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", STARBOARD_LATCH_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{STARBOARD_LATCH_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("PortDischargeBoom", STARBOARD_LATCH_FIELD)
        self.assertNotIn("StarboardDischargeBoom", STARBOARD_LATCH_FIELD)
        self.assertNotIn("EngineIntake", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisPortPanel", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisStarboardPanel", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisIntakePanel", STARBOARD_LATCH_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", STARBOARD_LATCH_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_LATCH_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("EncounterController", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("ControlServo", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("DispenserCenter", STARBOARD_LATCH_FIELD)
        self.assertNotIn("MaxReleasesPerBay", STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("DispenserStarboard", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetManeuver", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("PortLatch", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DecoyPods", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ElevatorLinkage", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisEngine", STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", STARBOARD_LATCH_FIELD)
    ...
    def test_breakwater_engine_is_not_radar_ghost_or_pathfinder_engine(
        ...
        locked_only = f"{STARBOARD_LATCH_FIELD}\n"
        self.assertNotEqual(BREAKWATER_ENGINE_FIELD, STARBOARD_LATCH_FIELD)
        ...
        self.assertNotIn("Engine;", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), STARBOARD_LATCH_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_LATCH_FIELD)
        self.assertNotIn("OpenOrbitExposure", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SignatureModulator", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_sibling_radar_ghost_fields(self) -> None:
        ...
        for token in SIBLING_RADAR_GHOST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("RadarReceiver", STARBOARD_LATCH_FIELD)
        self.assertNotIn("CoolingDoor", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisPortEWPanel", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisStarboardEWPanel", STARBOARD_LATCH_FIELD)
        self.assertNotIn("DebrisCoolingDoor", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SignatureModulator", STARBOARD_LATCH_FIELD)
        self.assertIn("StarboardLatch", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("SetLightningExposed", STARBOARD_LATCH_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", STARBOARD_LATCH_FIELD)
        self.assertNotIn("IsLightningExposed", STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), STARBOARD_LATCH_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, STARBOARD_LATCH_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("ASkyguardGunner", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("SkyguardRadarNode", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, STARBOARD_LATCH_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", STARBOARD_LATCH_FIELD)
        self.assertNotIn("MaxIntegrity", STARBOARD_LATCH_FIELD)
        self.assertNotIn("CurrentIntegrity", STARBOARD_LATCH_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", STARBOARD_LATCH_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            STARBOARD_LATCH_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, STARBOARD_LATCH_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", STARBOARD_LATCH_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{STARBOARD_LATCH_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, STARBOARD_LATCH_FIELD),
            STARBOARD_LATCH_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardBreakwaterBoss::StarboardLatch", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "breakwater StarboardLatch field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{STARBOARD_LATCH_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, STARBOARD_LATCH_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                STARBOARD_LATCH_FIELD.lower(),
                "breakwater StarboardLatch contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, STARBOARD_LATCH_FIELD)

    def test_contract_is_starboard_latch_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(STARBOARD_LATCH_FIELD.endswith(";"))
        self.assertNotIn("=", STARBOARD_LATCH_FIELD)
        self.assertIn(UPROPERTY_LATCH, section)
