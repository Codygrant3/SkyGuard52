...
# THIS IS leftover-safe unique analog library UFUNCTION
# wrapping unused Category path
# Skyguard|Mission01|Environment|Validation.
# Leftover unique LandscapeVisibleAudit FIELD drafts
# wrap struct members with NO Category and explicitly
# do NOT parse this library.
# Identifier PrepareGovernedLandscapeForVisibleValidation is NOT leftover
# analog VisibleAudit field bSuccess / VisibleComponentCount and NOT
# leftover unique SampleLandscapeHeight (#626 Grounding) and NOT leftover
# unique Authoring AuthorGovernedLandscapeAndGraph (#1692).
...
# static IS present. Return type IS
...
# wrapping leftover unique AuthorGovernedLandscapeAndGraph
# on USkyguardMission01EnvironmentAuthoringLibrary — RETARGET hard:
# Category Authoring -> Skyguard|Mission01|Environment|Validation,
# decl -> PrepareGovernedLandscapeForVisibleValidation static struct-return
# (not AuthorGovernedLandscapeAndGraph).
...
# LOCKED_DECL is PrepareGovernedLandscapeForVisibleValidation
...
# void PrepareGovernedLandscapeForVisibleValidation
# AS THE LOCKED DECL.
...
TARGET_WRONG_FLOAT = (
    "float PrepareGovernedLandscapeForVisibleValidation();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void PrepareGovernedLandscapeForVisibleValidations();"
...
TARGET_WRONG_TRUE = "bool PrepareGovernedLandscapeForVisibleValidation = true;"
TARGET_WRONG_ZERO = "bool PrepareGovernedLandscapeForVisibleValidation = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    ...
    r"PrepareGovernedLandscapeForVisibleValidation\("
    r"ALandscape\* Landscape,"
    r"UMaterialInterface\* GovernedMaterial\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bPrepareGovernedLandscapeForVisibleValidation\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+PrepareGovernedLandscapeForVisibleValidation\b", compact):
        return False
    if re.search(r"\bbool\s+PrepareGovernedLandscapeForVisibleValidation\b", compact):
        return False
    if re.search(r"\bfloat\s+PrepareGovernedLandscapeForVisibleValidation\b", compact):
        return False
    if re.search(r"\bint32\s+PrepareGovernedLandscapeForVisibleValidation\b", compact):
        return False
...
def spec_section(header: str) -> str:
    ...
    method = "PrepareGovernedLandscapeForVisibleValidation"
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            ...
            r"PrepareGovernedLandscapeForVisibleValidation\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for PrepareGovernedLandscapeForVisibleValidation "
...
class Mission01PrepareGovernedLandscapeForVisibleValidationDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_prepare_governed_landscape_for_visible_validation_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "PrepareGovernedLandscapeForVisibleValidation()const",
    ...
    def test_prepare_governed_landscape_for_visible_validation_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid PrepareGovernedLandscapeForVisibleValidationX();\n",
            "\tint32 PrepareGovernedLandscapeForVisibleValidation = 1;\n",
            "\tFName PrepareGovernedLandscapeForVisibleValidation = NAME_None;\n",
            "\tvoid PrepareGovernedLandscapeForVisibleValidation() = "
            + forty
            + ";\n",
            "\tvoid PrepareGovernedLandscapeForVisibleValidation() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "PrepareGovernedLandscapeForVisibleValidation")
        )
        self.assertTrue(
            has_identifier(section, "PrepareGovernedLandscapeForVisibleValidation")
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_prepare_governed_landscape_for_visible_validation"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
