# THIS IS leftover-safe ASkyguardGunner GunnerCamera.
...
# then BARE `TObjectPtr<UCameraComponent> GunnerCamera;`
# (no initializer).
...
# Stop BEFORE sibling Ri + fleMesh as a claimed slot.
# Do NOT claim Boom as this slot (in-flight sibling).
# Do NOT claim leftover CPG mesh kit CpgStation through
# CpgConsoleRight / CPG text / combat floats /
# FireCameraShakeClass / MinimumSafeSideFireYaw /
# CpgSightHud.
# GunnerCamera is NOT leftover analog FireCameraShakeClass
# (TSubclassOf<UCameraShakeBase>, Category
# Skyguard|Combat|Feedback).
# GunnerCamera is NOT leftover analog gun-fire-camera-shake
# tests (keep bulk in LOCKED_SCRIPTS).
# Do NOT claim leftover CpgEufd, leftover
# CpgMpdLeft, leftover CpgMpdRight,
# leftover CpgTedac, leftover CpgDash,
# leftover CpgCanopyBow, leftover CpgSeatPan,
# leftover CpgKneeRight, leftover
# CpgKneeLeft, leftover CpgSeatBack,
# leftover CpgCockpit, leftover CpgStation.
# Do NOT claim leftover CpgGripRight,
# leftover CpgGripLeft, leftover CpgReticleV,
# leftover CpgReticleH, leftover
# CpgTedacBezel. GunnerCamera is NOT leftover
# HandMesh. Do NOT claim CpgGripRight, CpgGripLeft,
# CpgReticleV, CpgReticleH, CpgTedacBezel, CpgRailLeft,
# CpgRailRight, CpgTedacText, CpgConsoleRight, or HandMesh
# as this slot.
# GunnerCamera fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UCameraComponent> GunnerCamera;`
# NOT `FName WeatherIdentity;` and NOT
# `float CurrentIntegrity = 140.f` and NOT
# `TObjectPtr<UStaticMeshComponent> CpgConsoleRight;` and NOT
# `TObjectPtr<USceneComponent> CpgStation;` and NOT
# `TObjectPtr<USpringArmComponent> GunnerCamera;` and NOT
...
# / TObjectPtr<USpringArmComponent> GunnerCamera
# / TSubclassOf<UCameraShakeBase> FireCameraShakeClass
# AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is Skyguard|Apache.
# Camera slot fail-closed if LOCKED_DECL is
# `TObjectPtr<USceneComponent>` or
# `TObjectPtr<UStaticMeshComponent>` or
# `TObjectPtr<USpringArmComponent>` or
# `TObjectPtr<UTextRenderComponent>`.
# GunnerCamera fail-closed if LOCKED_DECL is leftover
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgCockpit /
# CpgStation / CpgSeatPan / CpgCanopyBow / CpgDash /
# CpgTedac / CpgMpdLeft / CpgMpdRight / CpgEufd /
# CpgTedacBezel / CpgReticleH / CpgReticleV /
# CpgGripLeft / CpgGripRight / CpgRailLeft / CpgConsoleRight /
# leftover Apache Canopy / leftover Apache PilotCanopy /
# leftover Apache GunnerMount / PilotMount / EyeMount /
# WeaponMount / FireCameraShakeClass / Boom.
...
TARGET = "TObjectPtr<UCameraComponent> GunnerCamera;"
TARGET_WRONG_INIT = "TObjectPtr<UCameraComponent> GunnerCamera = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UCameraComponent> GunnerCamera = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UCameraComponent> GunnerCamera = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GunnerCamera;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GunnerCamera;"
TARGET_WRONG_STATIC = "TObjectPtr<UStaticMeshComponent> GunnerCamera;"
TARGET_WRONG_SPRING = "TObjectPtr<USpringArmComponent> GunnerCamera;"
TARGET_WRONG_SHAKE = "TSubclassOf<UCameraShakeBase> FireCameraShakeClass;"
...
TARGET_WRONG_TEXT = "TObjectPtr<UTextRenderComponent> GunnerCamera;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_gunner_camera"
...
LEFTOVER_SET_CAMERA_SHAKE_SCALE = (
...
LEFTOVER_CPG_SIGHT_HUD = (
    "Scripts/tests/test_gunner_cpg_sight_hud"
    "_field_decl_contract.py"
)
LEFTOVER_MIN_SAFE_YAW = (
...
GUNNER_CAMERA_DECL_RE = re.compile(
    r"TObjectPtr<UCameraComponent>\s+GunnerCamera\s*;"
)
GUNNER_CAMERA_INIT_RE = re.compile(
    r"TObjectPtr<UCameraComponent>\s+GunnerCamera\s*="
)
GUNNER_CAMERA_SPRING_RE = re.compile(
    r"TObjectPtr<USpringArmComponent>\s+GunnerCamera\b"
)
GUNNER_CAMERA_STATIC_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+GunnerCamera\b"
)
FIRE_SHAKE_DECL_RE = re.compile(
    r"TSubclassOf<UCameraShakeBase>\s+FireCameraShakeClass\b"
)
CONSOLE_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgConsoleRight\b"
)
STATION_DECL_RE = re.compile(
    r"TObjectPtr<USceneComponent>\s+CpgStation\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgConsoleRight / CpgStation / Boom /
    # FireCameraShakeClass / SpringArm GunnerCamera,
    # leftover CpgKneeRight / CpgCanopyBow / CpgDash /
    # CpgSeatBack / CpgSeatPan / CpgCockpit /
    # CpgTedac / CpgMpdLeft / CpgMpdRight / CpgEufd /
    # CpgTedacBezel / CpgReticleH / CpgReticleV /
    # CpgGripLeft / CpgGripRight / leftover Apache
    # Canopy, or an initializer on GunnerCamera
    # as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GUNNER_CAMERA_DECL_RE.search(compact) is None:
        return False
    if GUNNER_CAMERA_INIT_RE.search(compact):
        return False
    if GUNNER_CAMERA_SPRING_RE.search(compact):
        return False
    if GUNNER_CAMERA_STATIC_RE.search(compact):
        return False
    if FIRE_SHAKE_DECL_RE.search(compact):
        return False
    if CONSOLE_RIGHT_DECL_RE.search(compact):
        return False
    if STATION_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, leftover_banned_primary_mesh()):
            break
    ...
    if has_identifier(window, leftover_banned_primary_mesh()):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{leftover_banned_primary_mesh()}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UCameraComponent>\s+GunnerCamera\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UCameraComponent> GunnerCamera "
...
class GunnerCameraFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GunnerCamera"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, leftover_banned_primary_mesh()), body)
        self.assertFalse(
            has_identifier(section, leftover_banned_primary_mesh()),
            section,
        )
        self.assertTrue(has_identifier(section, SIBLING_BOOM), section)
        ...
        self.assertFalse(has_identifier(section, SIBLING_CPG_CONSOLE_RIGHT), section)
        self.assertFalse(has_identifier(section, "FireCameraShakeClass"), section)
        self.assertNotIn(SIBLING_BOOM, LOCKED_DECL)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GunnerCamera"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GunnerCamera", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_gunner_camera_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UCameraComponent> GunnerCamera;", compact_origin)
        self.assertNotIn("GunnerCamera = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_gunner_camera_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UCameraComponent> GunnerCamera"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("UCameraComponent", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UCameraComponent> GunnerCamera;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPRING)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STATIC)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SHAKE)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SPRING}\n",
            f"\t{TARGET_WRONG_STATIC}\n",
            f"\t{TARGET_WRONG_SHAKE}\n",
            f"\t{TARGET_WRONG_FIRE_RATE}\n",
            f"\t{TARGET_WRONG_RECOIL}\n",
            f"\t{TARGET_WRONG_FLARE}\n",
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UCameraComponent> GunnerCameras;\n",
            "\tint32 GunnerCamera = 140;\n",
            "\tbool GunnerCamera = true;\n",
            "\tfloat GunnerCamera = " + forty + ";\n",
            "\tfloat GunnerCamera = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_rail_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        self.assertNotIn(SIBLING_CURRENT_INTEGRITY, LOCKED_DECL)
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, SIBLING_BOOM))
        self.assertTrue(has_identifier(section, "GunnerCamera"))
        ...
        self.assertNotIn(TARGET_WRONG_CONSOLE_RIGHT, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GunnerCamera"))
        self.assertTrue(has_identifier(section, "GunnerCamera"))
        self.assertTrue(has_identifier(leaked, leftover_banned_primary_mesh()))
        self.assertFalse(has_identifier(section, leftover_banned_primary_mesh()))
        ...
        self.assertFalse(has_identifier(section, SIBLING_CPG_DASH))
        self.assertFalse(has_identifier(section, SIBLING_CPG_CONSOLE_RIGHT))
        self.assertFalse(has_identifier(section, "FireCameraShakeClass"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner GunnerCamera field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_camera_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_SET_CAMERA_SHAKE_SCALE, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_FILL_RESULT_COMBAT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIAGARA_CATALOG, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GAME_USER_SETTINGS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_SIGHT_HUD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MIN_SAFE_YAW, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_BOOM, locked_only)
