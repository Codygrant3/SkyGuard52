...
# HeightfieldSource FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FString HeightfieldSource; one-line /
...
# #af83/#202. LOCK leftover-safe LandscapeHeightSample
# bValid sibling #1184 and QueryLocation sibling #1185.
# Do not edit them. LOCK leftover-safe sibling #1180
...
# THIS IS leftover-safe FSkyguardLandscapeHeightSample
# HeightfieldSource FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# FString HeightfieldSource; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeHeightSample HeightfieldSource
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FString HeightfieldSource;
...
# HeightfieldSource FIELD (declaration-only). It is
...
TARGET = "FString HeightfieldSource;"
TARGET_WRONG = "FName HeightfieldSource;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FString\s+HeightfieldSource\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FString HeightfieldSource is missing from "
...
class LandscapeHeightSampleHeightfieldSourceFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("HeightfieldSource", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_heightfield_source_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_heightfield_source_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString HeightfieldSource"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("HeightfieldSource;"), LOCKED_DECL)
        ...
        self.assertIn("HeightfieldSource", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("HeightfieldSource", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FString HeightfieldSource"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("HeightfieldSource", locked_only)
    ...
    def test_contract_is_target_field_declaration_only(
        ...
        self.assertTrue(LOCKED_DECL.startswith("FString HeightfieldSource"))
        self.assertIn("HeightfieldSource", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.endswith("HeightfieldSource;"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("GetProtectedGroup", LOCKED_DECL)
        self.assertNotIn("FSkyguardMission10ProtectedRuntime", LOCKED_DECL)
        self.assertIn(UPROPERTY_MISSION10, section)

    def test_sibling_director_fields_do_not_satisfy_heightfield_source(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
