...
# INDEX_NONE or lock GetProtectedTarget
# construction in the .cpp. This is leftover-safe
# Mission05 Protection GetProtectedTarget
# METHOD on ASkyguardMission05IntegrationDirector. It is
# NOT leftover Mission05 protected-target enum #aaa2
# (do not add an enum-roster contract), NOT leftover
# protected-target-runtime-defaults #ec8b STRUCT, NOT
# leftover NotifyProtectedTargetDamage. It is NOT leftover
# BindRuntimeActors (retired live mount in the locked
# decl), NOT leftover GetAircraft (returns the retired
# live mount), NOT leftover HandleDroneCityImpact, NOT
# leftover Harbor Mission02 / Harbor #6/#8/#9, NOT leftover
# Mission05 wave-state enum #ad28, leftover
# storm-runtime-defaults #89c9 struct, leftover
# HandleBossPhaseChanged without Blueprint category,
# leftover searchlight-track-runtime-defaults #7347
# (do not lock GetSearchlightRuntime). Distinct from leftover
...
# GetReadiness, leftover Mission05
# InitializePlayableMission #666, leftover Mission05
...
# Mission04, not Waves, not Searchlight, not
# Substation, not Objectives, not Boss, not Storm,
# not Integration, not Destruction, not Briefing,
# not Environment, not leftover briefing-widget,
# not leftover MissionBriefingComponent methods,
# not leftover Mission01 Integration, not leftover
# Mission03 Integration, not leftover
# Mission04 Integration, not leftover
# Harbor Mission02 Integration. Do not lock leftover
...
# GetProtectedTarget method declaration
...
# #656, leftover Harbor Mission02, leftover
# Mission04 wave-state enum #bb22, leftover
# Mission05 wave-state enum #ad28, leftover
# Mission05 protected-target enum #aaa2, leftover
# Mission05 protected-target-runtime-defaults #ec8b,
...
# contracts stay sibling-only. Skip leftover #664.
# Do not reopen leftover #668/#669.
LOCKED_SCRIPTS = (
...
class Mission05GetProtectedTargetDeclContractTests(unittest.TestCase):
    def test_mission05_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_PROTECTED_TARGET),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_PROTECTED_TARGET)
    ...
    def test_missing_get_protected_target_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission05IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_PROTECTED_TARGET)
        self.assertNotIn("BlueprintPure", GET_PROTECTED_TARGET)
        self.assertNotIn("Category", GET_PROTECTED_TARGET)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_PROTECTED_TARGET)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_bool = "\tbool GetProtectedTarget() const;\n"
        as_void = "\tvoid GetProtectedTarget() const;\n"
        with_arg = (
            "\tFSkyguardMission05ProtectedTargetRuntime GetProtectedTarget(int32 Wave) const;\n"
        )
        renamed = "\tFSkyguardMission05ProtectedTargetRuntime GetState() const;\n"
        short_name = "\tFSkyguardMission05ProtectedTargetRuntime GetWave() const;\n"
        leftover_synchronize = f"\t{SYNCHRONIZE_RUNTIME_STATE}\n"
        leftover_m03_wave_state = f"\t{LEFTOVER_MISSION03_GET_WAVE_STATE}\n"
        ...
        leftover_wave_state = f"\t{LEFTOVER_MISSION04_GET_WAVE_STATE}\n"
        ...
        for region in (
            ...
            renamed,
            short_name,
            leftover_synchronize,
            leftover_m03_wave_state,
            leftover_configure,
            leftover_notify,
            leftover_failed,
            leftover_init,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_PROTECTED_TARGET)
            self.assertIn("GetProtectedTarget", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_protected_target_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_PROTECTED_TARGET),
            GET_PROTECTED_TARGET,
        )
        self.assertTrue(has_declaration(section, GET_PROTECTED_TARGET))
        self.assertEqual(
            declaration_count(section, GET_PROTECTED_TARGET),
            1,
        )
        self.assertTrue(
            GET_PROTECTED_TARGET.startswith("FSkyguardMission05ProtectedTargetRuntime "),
            GET_PROTECTED_TARGET,
        )
        self.assertTrue(
            GET_PROTECTED_TARGET.endswith(";"),
            GET_PROTECTED_TARGET,
        )
        self.assertIn("GetProtectedTarget", GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget(", GET_PROTECTED_TARGET)
        self.assertNotIn("TArray<", GET_PROTECTED_TARGET)
        self.assertNotIn("TObjectPtr", GET_PROTECTED_TARGET)
        self.assertNotIn("TSoftObjectPtr", GET_PROTECTED_TARGET)
        self.assertNotIn("INDEX_NONE", GET_PROTECTED_TARGET)
        self.assertNotIn("UFUNCTION", GET_PROTECTED_TARGET)
        self.assertNotIn("{", GET_PROTECTED_TARGET)
        self.assertNotIn("}", GET_PROTECTED_TARGET)
        self.assertNotIn("return ", GET_PROTECTED_TARGET)
        self.assertIn(" const;", GET_PROTECTED_TARGET)
        self.assertNotIn("Root", GET_PROTECTED_TARGET)
        self.assertNotIn("Briefing", GET_PROTECTED_TARGET)
        self.assertNotIn("AudioDirector", GET_PROTECTED_TARGET)
        self.assertNotIn("RadioChatter", GET_PROTECTED_TARGET)
        self.assertNotIn("SortiePresentation", GET_PROTECTED_TARGET)
        self.assertNotIn("CampaignDefinition", GET_PROTECTED_TARGET)
        self.assertNotIn("MissionDefinition", GET_PROTECTED_TARGET)
        self.assertNotIn("Readiness", GET_PROTECTED_TARGET)
        self.assertNotIn("bAutoInitialize", GET_PROTECTED_TARGET)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_PROTECTED_TARGET)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_PROTECTED_TARGET)
        self.assertNotIn("PathfinderSpawnLocation", GET_PROTECTED_TARGET)
        self.assertNotIn("GetAircraft", GET_PROTECTED_TARGET)
        self.assertNotIn("BindRuntimeActors", GET_PROTECTED_TARGET)
        self.assertNotIn("HandleDroneCityImpact", GET_PROTECTED_TARGET)
        self.assertNotIn("StartNextWave", GET_PROTECTED_TARGET)
        self.assertNotIn("NotifyThreatDestroyed", GET_PROTECTED_TARGET)
        self.assertNotIn("AdvanceConvoyByDistance", GET_PROTECTED_TARGET)
        self.assertNotIn("NotifyConvoyDamage", GET_PROTECTED_TARGET)
        self.assertNotIn("SynchronizeRuntimeState", GET_PROTECTED_TARGET)
        self.assertNotIn("GetConvoyRouteState", GET_PROTECTED_TARGET)
        self.assertNotIn("GetDayBeatKit", GET_PROTECTED_TARGET)
        self.assertNotIn("GetNightBeatKit", GET_PROTECTED_TARGET)
        self.assertNotIn("StartSearchlightWindow", GET_PROTECTED_TARGET)
        self.assertNotIn("AdvanceSearchlightTrack", GET_PROTECTED_TARGET)
        self.assertNotIn("NotifySubstationDamage", GET_PROTECTED_TARGET)
        self.assertNotIn("GetRemainingThreatsInWave", GET_PROTECTED_TARGET)
        self.assertNotIn("GetSearchlightRuntime", GET_PROTECTED_TARGET)
        self.assertNotIn("GetSubstationIntegrity", GET_PROTECTED_TARGET)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_PROTECTED_TARGET)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_PROTECTED_TARGET)
        self.assertNotIn("RoadHunterSpawnLocation", GET_PROTECTED_TARGET)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_PROTECTED_TARGET)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_PROTECTED_TARGET)
        self.assertNotIn("ConfigureMissionDefinition", GET_PROTECTED_TARGET)
        self.assertNotIn("NotifyObjectiveProgress", GET_PROTECTED_TARGET)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_PROTECTED_TARGET)
        self.assertNotIn("NotifyProtectedTargetDamage", GET_PROTECTED_TARGET)
        self.assertNotIn("GetWaveState", GET_PROTECTED_TARGET)
        self.assertNotIn("GetStormRuntime", GET_PROTECTED_TARGET)
        self.assertNotIn("GetSurvivingTargetCount", GET_PROTECTED_TARGET)
        self.assertNotIn("InitializePlayableMission", GET_PROTECTED_TARGET)
        self.assertNotIn("IsCorePlayableReady", GET_PROTECTED_TARGET)
        self.assertNotIn("GetReadiness", GET_PROTECTED_TARGET)
        self.assertNotIn("GetObjectiveRuntime", GET_PROTECTED_TARGET)
        self.assertNotIn("GetGunner", GET_PROTECTED_TARGET)
        self.assertNotIn("GetPathfinder", GET_PROTECTED_TARGET)
        self.assertNotIn("GetMissionId", GET_PROTECTED_TARGET)
        self.assertNotIn("ValidateMissionContract", GET_PROTECTED_TARGET)
        self.assertNotIn("ConfigureFromMission", GET_PROTECTED_TARGET)
        self.assertNotIn("AdvanceBriefing", GET_PROTECTED_TARGET)
        self.assertNotIn("SetAssetsReady", GET_PROTECTED_TARGET)
        self.assertNotIn("AcknowledgeAndLaunch", GET_PROTECTED_TARGET)
        self.assertNotIn("CanLaunch", GET_PROTECTED_TARGET)
        self.assertNotIn("GetElapsedSeconds", GET_PROTECTED_TARGET)
        self.assertNotIn("GetBriefingState", GET_PROTECTED_TARGET)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_PROTECTED_TARGET)
        self.assertNotIn("GetBriefingText", GET_PROTECTED_TARGET)
        self.assertNotIn("GetRadioChatter", GET_PROTECTED_TARGET)
        self.assertNotIn("GetPresentation", GET_PROTECTED_TARGET)
        self.assertNotIn("GetMissionTitle", GET_PROTECTED_TARGET)
        self.assertNotIn("AcknowledgeBriefing", GET_PROTECTED_TARGET)
        self.assertNotIn("LaunchSortie", GET_PROTECTED_TARGET)
        self.assertNotIn("HullCollider", GET_PROTECTED_TARGET)
        self.assertNotIn("OpticalTracker", GET_PROTECTED_TARGET)
        self.assertNotIn("WeaponServo", GET_PROTECTED_TARGET)
        self.assertNotIn("CountermeasurePod", GET_PROTECTED_TARGET)
        self.assertNotIn("MinHeightFromOriginCm", GET_PROTECTED_TARGET)
        self.assertNotIn("RadarNode", GET_PROTECTED_TARGET)
        self.assertNotIn("ESkyguardMission02WaveState", GET_PROTECTED_TARGET)
        self.assertNotIn("HarborIndustrial", GET_PROTECTED_TARGET)
        self.assertNotIn("MaxIntegrity", GET_PROTECTED_TARGET)
        self.assertNotIn("CurrentIntegrity", GET_PROTECTED_TARGET)
        self.assertNotIn("FillAndFinalize", GET_PROTECTED_TARGET)
        self.assertNotIn("FillAndFail", GET_PROTECTED_TARGET)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_PROTECTED_TARGET)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_PROTECTION} {GET_PROTECTED_TARGET}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_PROTECTED_TARGET}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_PROTECTED_TARGET))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_PROTECTED_TARGET),
            section,
    ...
    def test_environment_category_does_not_satisfy_protection(self) -> None:
        ...
        self.assertNotEqual(leftover_m05_protection, UFUNCTION_PROTECTION)
        ...
        self.assertNotIn(leftover_m05_protection, UFUNCTION_PROTECTION)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_PROTECTED_TARGET}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_PROTECTED_TARGET)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_PROTECTED_TARGET)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_protected_target_cpp_body(self) -> None:
        locked_only = f"{GET_PROTECTED_TARGET}\n"
        self.assertNotIn("{", GET_PROTECTED_TARGET)
        self.assertNotIn("}", GET_PROTECTED_TARGET)
        self.assertNotIn("return ", GET_PROTECTED_TARGET)
        ...
        self.assertNotIn("return false", GET_PROTECTED_TARGET)
        self.assertNotIn("return true", GET_PROTECTED_TARGET)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_PROTECTED_TARGET}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget", GET_PROTECTED_TARGET)
        self.assertTrue(
            GET_PROTECTED_TARGET.startswith("FSkyguardMission05ProtectedTargetRuntime "),
            GET_PROTECTED_TARGET,
    ...
    def test_contract_does_not_relock_sibling_integration_methods(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_mission04_synchronize_runtime_state"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_PROTECTED_TARGET)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_PROTECTED_TARGET)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_PROTECTED_TARGET)
        self.assertNotIn("CurrentIntegrity", GET_PROTECTED_TARGET)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_PROTECTED_TARGET)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_PROTECTED_TARGET)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_PROTECTED_TARGET)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_PROTECTED_TARGET}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_PROTECTED_TARGET),
            GET_PROTECTED_TARGET,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PROTECTED_TARGET)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_PROTECTED_TARGET)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_PROTECTED_TARGET)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", section)
        self.assertNotIn("ASkyguardTempestBoss", GET_PROTECTED_TARGET)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 GetProtectedTarget method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_PROTECTED_TARGET}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_PROTECTED_TARGET.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_PROTECTED_TARGET.lower(),
                "mission05 GetProtectedTarget contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_PROTECTED_TARGET)

    def test_contract_is_get_protected_target_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertIn("GetProtectedTarget", GET_PROTECTED_TARGET)
        self.assertTrue(GET_PROTECTED_TARGET.endswith(";"))
        self.assertIn(UFUNCTION_PROTECTION, section)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_protected_target(
        ...
        for leftover in (
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_PROTECTED_TARGET)
            )
            self.assertNotEqual(GET_PROTECTED_TARGET, leftover)
        self.assertIn(
    ...
    def test_leftover_mission03_synchronize_does_not_satisfy(self) -> None:
        leftover = (
        ...
        for token in LEFTOVER_MISSION03_SYNCHRONIZE_NOT_LOCKED:
            self.assertNotIn(token, GET_PROTECTED_TARGET)
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetProtectedTarget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_PROTECTED_TARGET))
        locked_only = f"{GET_PROTECTED_TARGET}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        inline = (
            ...
            "\t\tESkyguardMission05ProtectedTarget Target) const\n"
            "\t{\n"
            "\t\treturn FSkyguardMission05ProtectedTargetRuntime();\n"
            "\t}\n"
            "};\n"
        )
        origin_inline = (
        ...
        one_line_inline = (
            "public:\n"
            "\tFSkyguardMission05ProtectedTargetRuntime GetProtectedTarget("
            "ESkyguardMission05ProtectedTarget Target) const { return {}; }\n"
            "};\n"
        )
    ...
    def test_leftover_mission04_synchronize_does_not_satisfy(self) -> None:
        leftover = (
        ...
        for token in LEFTOVER_MISSION04_SYNCHRONIZE_NOT_LOCKED:
            self.assertNotIn(token, GET_PROTECTED_TARGET)
    ...
    def test_leftover_handle_boss_phase_changed_does_not_satisfy(self) -> None:
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PROTECTED_TARGET)
        self.assertNotIn("HandleBossPhaseChanged", GET_PROTECTED_TARGET)
    ...
    def test_leftover_mission03_get_wave_state_does_not_satisfy(self) -> None:
        ...
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        region = f"\t{LEFTOVER_MISSION03_GET_WAVE_STATE}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget", str(raised.exception))
        self.assertFalse(has_declaration(region, GET_PROTECTED_TARGET))
    ...
    def test_leftover_mission04_get_wave_state_does_not_satisfy(self) -> None:
        ...
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        region = f"\t{LEFTOVER_MISSION04_GET_WAVE_STATE}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, GET_PROTECTED_TARGET)
        self.assertIn("GetProtectedTarget", str(raised.exception))
        self.assertFalse(has_declaration(region, GET_PROTECTED_TARGET))
