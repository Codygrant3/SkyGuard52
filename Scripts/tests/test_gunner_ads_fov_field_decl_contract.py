# THIS IS leftover-safe ASkyguardGunner ADSFov.
...
# then `float ADSFov = 42.f;` (HAS initializer).
# Do NOT claim leftover LookYawLimit #1618 / LookPitchMin
# #1617 / leftover in-flight LookPitchMax / leftover
# in-flight sibling MouseSensitivity as this slot.
# Stop BEFORE sibling HipFov. LOCKED_DECL initializer
# is 42.f, NOT leftover Harbor 40 / 80.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Combat`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS `= 42.f`.
...
# Stop BEFORE sibling HipFov as a claimed slot.
...
# LOCKED_DECL is `float ADSFov = 42.f;` NOT
...
TARGET = "float ADSFov = 42.f;"
TARGET_WRONG_BARE = "float ADSFov;"
TARGET_WRONG_INIT = "float ADSFov = nullptr;"
TARGET_WRONG_FALSE = "float ADSFov = false;"
TARGET_WRONG_TRUE = "float ADSFov = true;"
TARGET_WRONG_ZERO = "float ADSFov = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> ADSFov;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> ADSFov;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> ADSFov;"
...
ADS_FOV_DECL_RE = re.compile(
    r"float\s+ADSFov\s*=\s*42\.f\s*;"
)
ADS_FOV_BARE_RE = re.compile(
    r"float\s+ADSFov\s*;"
)
ADS_FOV_WRONG_INIT_RE = re.compile(
    r"float\s+ADSFov\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float ADSFov = 42.f;`.
    ...
    # Harbor 40 / 80, or a bare ADSFov without
    # the 42.f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ADS_FOV_DECL_RE.search(compact) is None:
        return False
    if ADS_FOV_BARE_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+HipFov\b", compact):
        return False
    if re.search(r"float\s+ADSFov\s*=\s*" + "40" + r"\.f\b", compact):
        return False
    if re.search(r"float\s+ADSFov\s*=\s*" + "80" + r"\.f\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    if has_identifier(window, STOP_BEFORE_HIP_FOV):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_HIP_FOV}"
        )
...
class GunnerAdsFovFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ADSFov"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_HIP_FOV), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_HIP_FOV),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ADSFov"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ADSFov", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float ADSFov = 42.f;", compact_origin)
        self.assertNotIn("ADSFov = 160.f", compact_origin)
    ...
    def test_ads_fov_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float ADSFov = 42.f;")
    ...
    def test_does_not_claim_sibling_hip_fov_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ADSFov"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_HIP_FOV))
        self.assertFalse(has_identifier(section, STOP_BEFORE_HIP_FOV))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "ADSFov"))
        self.assertTrue(has_identifier(section, "ADSFov"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_HIP_FOV))
        self.assertFalse(has_identifier(section, STOP_BEFORE_HIP_FOV))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner ADSFov field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_ads_fov_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
