...
# then BARE `TArray<FName> MissingCategoryEntries;`.
...
# bProductionReady.
# Match the EXACT identifier MissingCategoryEntries as a
# whole field name after TArray<FName>. Do NOT accept a
# substring match on Missing. Do NOT claim sibling Missing*
# arrays.
...
TARGET = "TArray<FName> MissingCategoryEntries;"
TARGET_WRONG_EQ_EMPTY = "TArray<FName> MissingCategoryEntries = {};"
TARGET_WRONG_EQ_ZERO = "TArray<FName> MissingCategoryEntries = 0;"
TARGET_WRONG_FALSE = "TArray<FName> MissingCategoryEntries = false;"
TARGET_WRONG_TRUE = "TArray<FName> MissingCategoryEntries = true;"
TARGET_WRONG_ZERO = "TArray<FName> MissingCategoryEntries = 0.f;"
TARGET_WRONG_FORTY = "TArray<FName> MissingCategoryEntries = 40;"
TARGET_WRONG_EIGHTY = "TArray<FName> MissingCategoryEntries = 80;"
TARGET_WRONG_FLOAT = "float MissingCategoryEntries = 0;"
TARGET_WRONG_INT32 = "int32 MissingCategoryEntries = 0;"
TARGET_WRONG_FNAME = "FName MissingCategoryEntries;"
TARGET_WRONG_BOOL = "bool MissingCategoryEntries = false;"
TARGET_WRONG_FSTRING = "FString MissingCategoryEntries;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MissingCategoryEntries;"
...
FIELD_NAME = "MissingCategoryEntries"
...
THIS_SCRIPT = (
    ...
    "_category_entries_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored bare `TArray<FName> MissingCategoryEntries;`.
    # Reject added initializer (`= {}`, `= 0`, Harbor 40/80),
    # int32, float, bool, FName, FString, TSoftObjectPtr.
    # Match the EXACT identifier MissingCategoryEntries after
    ...
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+MissingCategoryEntries\s*=",
        compact,
    ):
        return False
    if re.search(
        r"MissingCategoryEntries\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingCategoryEntries\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingCategoryEntries\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingCategoryEntries\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingCategoryEntries\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MissingCategoryEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MissingCategoryEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MissingCategoryEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MissingCategoryEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MissingCategoryEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MissingCategoryEntries\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+MissingCategoryEntries\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> MissingCategoryEntries is missing from "
...
class ProductionAudioAuditMissingCategoryEntriesFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_category_entries_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_added_fails_closed(self) -> None:
        ...
        self.assertIn("TArray<FName> MissingCategoryEntries;", compact_origin)
        self.assertNotIn("MissingCategoryEntries = 40", compact_origin)
        self.assertNotIn("MissingCategoryEntries = 80", compact_origin)
        self.assertNotIn("MissingCategoryEntries = false", compact_origin)
        self.assertNotIn("MissingCategoryEntries = {}", compact_origin)
        self.assertNotIn("MissingCategoryEntries = 0", compact_origin)

    def test_missing_category_entries_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> MissingCategoryEntries"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tTArray<FName> MissingCategoryEntriess;\n",
            "\tTArray<FName> MissingCategoryEntries = 1;\n",
            "\tbool MissingCategoryEntries;\n",
            "\tfloat MissingCategoryEntries = " + forty + ";\n",
            "\tfloat MissingCategoryEntries = " + eighty + ";\n",
    ...
    def test_exact_identifier_rejects_substring_missing_siblings(self) -> None:
        ...
        self.assertFalse(has_identifier(exact, "CategoryEntries"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit MissingCategoryEntries field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "missing_category_entries_field_decl_contract.py"
        ))
