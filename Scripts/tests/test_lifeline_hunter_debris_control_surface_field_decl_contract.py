...
# INDEX_NONE or lock DebrisControlSurface construction
# in the .cpp. This is a FIELD contract on
# ASkyguardLifelineHunterBoss, not leftover LifelineHunter
# public methods (already drafted) and not leftover
# LifelineHunter sibling fields. Keep DebrisControlSurface
# distinct from DebrisPrimarySensor / DebrisSecondarySensor. This is not a leftover
# USkyguardBossWeakPointComponent member
# contract. origin/main is a one-line field
# (`TObjectPtr<UStaticMeshComponent>
# DebrisControlSurface;`); accept that form and other
...
# Do not lock leftover sibling LifelineHunter
# fields OpticalTracker, WeaponServo,
# CountermeasurePod, Engine, DebrisPrimarySensor, or
# DebrisSecondarySensor as the primary lock.
# Stay off leftover RadarGhost public methods
# SetContactIdentified / OpenOrbitExposure /
# IsContactIdentified. Stay off leftover
# constructor / HandleWeakPointDestroyed.
# Stay off leftover rear-aspect window /
# break-finish named surfaces. Stay off leftover
# Iron Rain DispenserPort / sibling field
# contracts and leftover Iron Rain public
# methods. Stay off leftover Tempest
# ControlServo / sibling field contracts and
# leftover Tempest lightning / gust /
# lock-stability methods SetLightningExposed /
...
# locked header. Stay off leftover RadarNode.
# Stay off leftover BossDrone
...
# leftover #107–#561, plus
# SkyguardLifelineHunterBoss production files.
# This lane only adds an isolated Python
# DebrisControlSurface field declaration contract on
# ASkyguardLifelineHunterBoss. Stay off leftover
# sibling LifelineHunter fields, leftover
# LifelineHunter public methods, leftover Iron
# Rain DispenserPort and sibling Iron Rain
# fields, leftover Iron Rain public methods,
# leftover Tempest ControlServo and sibling
# Tempest fields, leftover Tempest
...
# leftover isolated-test drafts #107–#561,
# leftover searchlight-track-runtime-defaults
# #7347, leftover BlackKite siblings,
# leftover RadarGhost siblings / method contracts,
# leftover LifelineHunter siblings, leftover
# Pathfinder siblings, leftover Tempest
# siblings, leftover IronRain siblings,
# leftover LastFlight siblings
...
# isolated RadarGhost boss field contracts stay
# unlocked for THIS test's primary lock. Leftover
# RadarGhost public methods, leftover Iron Rain
# DispenserPort / sibling Iron Rain field
# contracts, leftover Iron Rain public methods,
# leftover Tempest ControlServo / sibling Tempest
# field contracts, leftover Tempest lightning /
# gust / lock-stability methods, leftover Tempest
# emergency-finish / stabilized-lock methods,
# leftover Pathfinder / LastFlight / BossDrone /
# Apache / Gunner / settings / input-capture /
# briefing / debrief isolated contracts, leftover
# Harbor #6/#8/#9, leftover theater-kit #59,
# leftover drafts #56–#64, leftover isolated-test
# drafts #107–#561 stay sibling-only.
LOCKED_SCRIPTS = (
...
LEFTOVER_COOLING_DOOR_NOT_LOCKED = (
...
LEFTOVER_DEBRIS_STARBOARD_EW_PANEL_NOT_LOCKED = (
    "test_radar_ghost_debris_starboard_ew_panel_field_decl_contract.py",
    "DebrisStarboardEWPanel",
)
LEFTOVER_DEBRIS_PORT_EW_PANEL_NOT_LOCKED = (
    "test_radar_ghost_debris_port_ew_panel_field_decl_contract.py",
    "DebrisPortEWPanel",
)
LEFTOVER_DISPENSER_PORT_NOT_LOCKED = (
    "test_iron_rain_dispenser_port_field_decl_contract.py",
    "DispenserPort",
)
...
LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED = (
    ...
    "test_lifeline_hunter_debris_secondary_sensor_field_decl_contract.py",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLifelineHunterBoss::DebrisControlSurface",
    "SkyguardLifelineHunterBoss.cpp",
    "SetExposed",
...
class LifelineHunterDebrisControlSurfaceFieldDeclContractTests(unittest.TestCase):
    def test_lifeline_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_missing_debris_control_surface_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardLifelineHunterBoss();\n"
            ...
            f"\t{MINIMUM_WEAPON_SEPARATION_FIELD}\n"
            f"\t{OPEN_SENSOR_EXPOSURE}\n"
            f"\t{SET_FRIENDLY_SEPARATION}\n"
            f"\t{REDIRECT_DISABLED_DRONE}\n"
            f"\t{leftover_open_safe_window()}\n"
            f"\t{leftover_arm_safe_engine_fallback()}\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("Category", DEBRIS_CONTROL_SURFACE_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisControlSurface;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisControlSurface;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DebrisControlSurface;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_CONTROL_SURFACE_FIELD)
            self.assertIn("DebrisControlSurface", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_control_surface_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD),
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_CONTROL_SURFACE_FIELD), 1)
        self.assertTrue(
            DEBRIS_CONTROL_SURFACE_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        self.assertTrue(DEBRIS_CONTROL_SURFACE_FIELD.endswith(";"), DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("TArray<", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("=", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("{", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("}", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("return ", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisCenter", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("RadarNode", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_CONTROL_SURFACE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_DESTRUCTION} {DEBRIS_CONTROL_SURFACE_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DEBRIS_CONTROL_SURFACE_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DEBRIS_CONTROL_SURFACE_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_CONTROL_SURFACE_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD),
            section,
    ...
    def test_assigned_debris_control_surface_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisControlSurface = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_CONTROL_SURFACE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DebrisControlSurface", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_CONTROL_SURFACE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_CONTROL_SURFACE_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_DESTRUCTION, section)
        self.assertTrue(has_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_CONTROL_SURFACE_FIELD.startswith("UFUNCTION"),
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_CONTROL_SURFACE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_CONTROL_SURFACE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_control_surface_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        self.assertNotIn("{", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("}", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("return ", DEBRIS_CONTROL_SURFACE_FIELD)
        ...
        self.assertNotIn("SkyguardLifelineHunterBoss.cpp", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SkyguardLifelineHunterBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("return true", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_CONTROL_SURFACE_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisIntakePanel", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DispenserCenter", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DispenserStarboard", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetManeuver", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_CONTROL_SURFACE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, DEBRIS_CONTROL_SURFACE_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("IsContactIdentified", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_radar_ghost_fields(self) -> None:
        ...
        for token in SIBLING_RADAR_GHOST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("RadarReceiver", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(COOLING_DOOR_FIELD, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisStarboardEWPanel", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisPortEWPanel", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", DEBRIS_CONTROL_SURFACE_FIELD)


    def test_cooling_door_does_not_satisfy_debris_control_surface(self) -> None:
        leftover = f"\t{COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_CONTROL_SURFACE_FIELD))
        self.assertNotEqual(DEBRIS_CONTROL_SURFACE_FIELD, COOLING_DOOR_FIELD)
        self.assertNotIn(
            "USkyguardBossWeakPointComponent",
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        self.assertNotIn("UStaticMeshComponent", COOLING_DOOR_FIELD)
        self.assertTrue(
            DEBRIS_CONTROL_SURFACE_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
    ...
    def test_contract_does_not_relock_leftover_dispenser_port(self) -> None:
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        for token in LEFTOVER_DISPENSER_PORT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DispenserPort", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("OpticalTracker", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisPrimarySensor", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisSecondarySensor", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("IsLightningExposed", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_CONTROL_SURFACE_FIELD),
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        ...
        self.assertNotIn(COOLING_DOOR_FIELD, locked_only)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardLifelineHunterBoss::DebrisControlSurface", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "lifeline hunter DebrisControlSurface field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_CONTROL_SURFACE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_CONTROL_SURFACE_FIELD.lower(),
                "lifeline hunter DebrisControlSurface contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_CONTROL_SURFACE_FIELD)

    def test_contract_is_debris_control_surface_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("DebrisSecondarySensor", locked_only)
        ...
        self.assertTrue(DEBRIS_CONTROL_SURFACE_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn(UPROPERTY_DESTRUCTION, section)

    def test_sibling_debris_fields_do_not_satisfy_control_surface(self) -> None:
        for leftover in (
            DEBRIS_PRIMARY_SENSOR_FIELD,
            DEBRIS_SECONDARY_SENSOR_FIELD,
        ):
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, DEBRIS_CONTROL_SURFACE_FIELD)
            )
            self.assertNotEqual(DEBRIS_CONTROL_SURFACE_FIELD, leftover)
        self.assertIn(
    ...
    def test_minimum_weapon_separation_is_not_the_primary_lock(self) -> None:
        leftover = f"\t{MINIMUM_WEAPON_SEPARATION_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(leftover, DEBRIS_CONTROL_SURFACE_FIELD)
        )
        self.assertNotIn(
            "MinimumWeaponSeparationMeters",
            DEBRIS_CONTROL_SURFACE_FIELD,
        )
        self.assertNotIn("450.f", DEBRIS_CONTROL_SURFACE_FIELD)
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        for token in HARBOR_ADJACENT_NOT_LOCKED:
            self.assertNotIn(token, leftover_harbor_tokens())
            self.assertNotIn(token, locked_only)
    ...
    def test_contract_does_not_relock_sibling_lifeline_hunter_fields(
        ...
        for token in SIBLING_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("OpticalTracker", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("WeaponServo", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("CountermeasurePod", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisPrimarySensor", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("DebrisSecondarySensor", DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertIn("DebrisControlSurface", DEBRIS_CONTROL_SURFACE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_CONTROL_SURFACE_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DEBRIS_CONTROL_SURFACE_FIELD)

    def test_runway_breaker_debris_scripts_stay_sibling_only(self) -> None:
        self.assertIn(
            "Scripts/tests/test_runway_breaker_debris_engine"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_runway_breaker_debris_port_wing"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_runway_breaker_debris_payload_bay"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        locked_only = f"{DEBRIS_CONTROL_SURFACE_FIELD}\n"
        ...
        self.assertNotIn("ASkyguardRunwayBreakerBoss", DEBRIS_CONTROL_SURFACE_FIELD)
