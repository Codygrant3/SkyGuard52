...
# Mission07 IntegrationReadiness bCampaignRuntimeStarted
...
# This readiness-struct field sits after bSortiePresentationReady.
# Skip leftover b + Ya + kRuntimeReady forever (it sits
# after bMapAssemblyReady and before bGunnerReady). This contract
# is bCampaignRuntimeStarted after bSortiePresentationReady. Skip leftover
...
# leftover Mission06 bCampaignRuntimeStarted #1007 /
# leftover Mission06 bAudioReady #1006 /
# leftover Mission06 bBriefingReady #1004 /
# leftover Mission06 bSortiePresentationReady #1005 /
# leftover Mission06 bProtectedTargetsReady #1002.
# leftover Mission07 director Root #1011 through
# Readiness #1027 / leftover Mission07
# readiness-struct bMissionDefinitionValid #1028
# through bWavesReady #1034 / leftover
# bProtectedTargetsReady #1035 / leftover
# bSearchRuntimeReady #1036 / leftover
# bBriefingReady #1037. Sibling isolated contracts
# leftover-safe Mission07 bAudioReady and leftover-
# safe Mission07 bSortiePresentationReady stay
# sibling-only.
...
LEFTOVER_MISSION07_SIBLING_READY_FIELDS_NOT_LOCKED = (
...
class Mission07CampaignRuntimeStartedFieldDeclContractTests(unittest.TestCase):
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 bCampaignRuntimeStarted field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission07 bCampaignRuntimeStarted contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_campaign_runtime"
                "_started_field_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        for token in LEFTOVER_MISSION07_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_READINESS_STRUCT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION06_CAMPAIGN_RUNTIME_STARTED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_SIBLING_READY_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
