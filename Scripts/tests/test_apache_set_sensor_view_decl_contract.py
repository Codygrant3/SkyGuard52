...
# or lock the SetSensorView body in the .cpp.
...
# ESkyguardApacheSystem enum values. Do not lock
# IsSensorViewActive / IsSensorLive /
# IsThermalAvailable. This is not leftover
...
# SetOrbitFocus / FaceWorldLocation siblings. Stay
# off mount getters, GetChinMuzzleLocation,
# AimChinTurret, SetRotorPower, IssuePilotCommand,
# GetPilotCommand, GetPilotConfirmationsIssued,
# SetOrbitFocus, FaceWorldLocation,
...
# in this wave, leftover IssuePilotCommand /
# GetPilotCommand / GetPilotConfirmationsIssued
# siblings, and leftover SortiePresentationWidgets
# stay sibling-only.
SET_SENSOR_VIEW = ("void SetSensorView(bool bInSensor);")
UFUNCTION_APACHE = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Apache")'
)
# Leftover #56–#64 plus SkyguardApacheAircraft production
# files. This lane only adds an isolated Python
# SetSensorView declaration contract on
...
# FaceWorldLocation, SetFirstPersonInterior,
...
# FaceWorldLocation sibling, and dirty workspace
# paths.
LOCKED = {
...
# IssuePilotCommand sibling, leftover GetPilotCommand
# sibling, leftover GetPilotConfirmationsIssued
# sibling, leftover SetOrbitFocus sibling in this
# wave, leftover FaceWorldLocation sibling in this
# wave, leftover DebriefWidget Configure /
...
INTERIOR_NOT_LOCKED = ("void SetFirstPersonInterior(bool bInterior);",)
...
# .cpp SetSensorView body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::SetSensorView",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheSetSensorViewDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_SENSOR_VIEW), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_SENSOR_VIEW)
        self.assertIn("SetSensorView", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SET_SENSOR_VIEW))

    def test_missing_set_sensor_view_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_SENSOR_VIEW)
        self.assertIn("SetSensorView", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_SENSOR_VIEW)
        self.assertIn("SetSensorView", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_SENSOR_VIEW), section)
        self.assertNotIn("BlueprintPure", SET_SENSOR_VIEW)
        self.assertNotIn("UFUNCTION", SET_SENSOR_VIEW)
        self.assertNotIn("Category", SET_SENSOR_VIEW)
        self.assertNotIn("BlueprintCallable", SET_SENSOR_VIEW)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_SENSOR_VIEW)
        self.assertIn("SetSensorView", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetSensorView;\n"
        wrong_return_bool = "\tbool SetSensorView(bool bInSensor);\n"
        wrong_return_int = "\tint32 SetSensorView(bool bInSensor);\n"
        added_const = "\tvoid SetSensorView(bool bInSensor) const;\n"
        wrong_type = "\tvoid SetSensorView(int bInSensor);\n"
        wrong_float = "\tvoid SetSensorView(float bInSensor);\n"
        added_arg = "\tvoid SetSensorView(bool bInSensor, float Extra);\n"
        no_args = "\tvoid SetSensorView();\n"
        ...
        leftover_orbit = "\tvoid SetOrbitFocus(const FVector& WorldLocation);\n"
        leftover_face = "\tvoid FaceWorldLocation(const FVector& WorldLocation);\n"
        leftover_interior = "\tvoid SetFirstPersonInterior(bool bInterior);\n"
        leftover_live = "\tbool IsSensorLive() const;\n"
        leftover_thermal = "\tbool IsThermalAvailable() const;\n"
        leftover_active = "\tbool IsSensorViewActive() const;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_SENSOR_VIEW)
            self.assertIn("SetSensorView", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_sensor_view_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SET_SENSOR_VIEW),
            SET_SENSOR_VIEW,
        )
        self.assertTrue(has_declaration(section, SET_SENSOR_VIEW))
        self.assertEqual(declaration_count(section, SET_SENSOR_VIEW), 1)
        self.assertTrue(
            SET_SENSOR_VIEW.startswith("void "),
            SET_SENSOR_VIEW,
        )
        self.assertTrue(SET_SENSOR_VIEW.endswith(";"), SET_SENSOR_VIEW)
        self.assertIn("SetSensorView(", SET_SENSOR_VIEW)
        self.assertIn("bool bInSensor", SET_SENSOR_VIEW)
        self.assertNotIn("INDEX_NONE", SET_SENSOR_VIEW)
        self.assertNotIn("{", SET_SENSOR_VIEW)
        self.assertNotIn("}", SET_SENSOR_VIEW)
        self.assertNotIn("return ", SET_SENSOR_VIEW)
        self.assertNotIn(" const", SET_SENSOR_VIEW)
        self.assertNotIn("SetRotorPower", SET_SENSOR_VIEW)
        self.assertNotIn("AimChinTurret", SET_SENSOR_VIEW)
        self.assertNotIn("GetChinMuzzleLocation", SET_SENSOR_VIEW)
        self.assertNotIn("IssuePilotCommand", SET_SENSOR_VIEW)
        self.assertNotIn("GetPilotCommand", SET_SENSOR_VIEW)
        self.assertNotIn("GetPilotConfirmationsIssued", SET_SENSOR_VIEW)
        self.assertNotIn("SetOrbitFocus", SET_SENSOR_VIEW)
        self.assertNotIn("FaceWorldLocation", SET_SENSOR_VIEW)
        self.assertNotIn("SetFirstPersonInterior", SET_SENSOR_VIEW)
        self.assertNotIn("IsSensorLive", SET_SENSOR_VIEW)
        self.assertNotIn("IsThermalAvailable", SET_SENSOR_VIEW)
        self.assertNotIn("IsSensorViewActive", SET_SENSOR_VIEW)
        self.assertNotIn("GetChinTurret", SET_SENSOR_VIEW)
        self.assertNotIn("GetRotorPowerScale", SET_SENSOR_VIEW)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetSensorView(bool bInSensor);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, SET_SENSOR_VIEW),
                section,
            )
            self.assertEqual(
                require_declaration(section, SET_SENSOR_VIEW),
            ...
            self.assertEqual(
                declaration_count(section, SET_SENSOR_VIEW),
                1,
            )
        one_line = f"{{\npublic:\n\t{SET_SENSOR_VIEW}\n}}\n"
        self.assertTrue(has_declaration(one_line, SET_SENSOR_VIEW))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SET_SENSOR_VIEW), section)
        self.assertEqual(
            require_declaration(section, SET_SENSOR_VIEW),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_SENSOR_VIEW}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_SENSOR_VIEW)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_SENSOR_VIEW)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_sensor_view_cpp_body(self) -> None:
        locked_only = f"{SET_SENSOR_VIEW}\n"
        self.assertNotIn("{", SET_SENSOR_VIEW)
        self.assertNotIn("}", SET_SENSOR_VIEW)
        self.assertNotIn("return ", SET_SENSOR_VIEW)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", SET_SENSOR_VIEW)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", SET_SENSOR_VIEW)
        self.assertNotIn("return true", SET_SENSOR_VIEW)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{SET_SENSOR_VIEW}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_SENSOR_VIEW)
        self.assertNotIn("GetGunnerMount", SET_SENSOR_VIEW)
        self.assertNotIn("GetEyeMount", SET_SENSOR_VIEW)
        self.assertNotIn("GetWeaponMount", SET_SENSOR_VIEW)
        self.assertNotIn("GetChinTurret", SET_SENSOR_VIEW)
        self.assertNotIn("GetPilotMount", SET_SENSOR_VIEW)
        self.assertNotIn("GetSensorTurret", SET_SENSOR_VIEW)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{SET_SENSOR_VIEW}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_SENSOR_VIEW)
        self.assertNotIn("GetChinMuzzleLocation", SET_SENSOR_VIEW)
    ...
    def test_contract_does_not_relock_first_person_interior(self) -> None:
        locked_only = f"{SET_SENSOR_VIEW}\n"
        for neighbor in INTERIOR_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_SENSOR_VIEW)
        self.assertNotIn("SetFirstPersonInterior", SET_SENSOR_VIEW)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_SENSOR_VIEW)
        self.assertNotIn("ApplyDamage", SET_SENSOR_VIEW)
        self.assertNotIn("GetDamageFraction", SET_SENSOR_VIEW)
        self.assertNotIn("ApplySystemHit", SET_SENSOR_VIEW)
        self.assertNotIn("ApplyHit", SET_SENSOR_VIEW)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_SENSOR_VIEW)
        self.assertNotIn("ESkyguardApacheSystem", SET_SENSOR_VIEW)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_SENSOR_VIEW)
        self.assertNotIn("Pursuit", SET_SENSOR_VIEW)
        self.assertNotIn("OrbitLeft", SET_SENSOR_VIEW)
        self.assertNotIn("AttackRun", SET_SENSOR_VIEW)
        self.assertNotIn("FaceTarget", SET_SENSOR_VIEW)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_SENSOR_VIEW)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", SET_SENSOR_VIEW)
        self.assertNotIn("USkyguardBriefingWidget", SET_SENSOR_VIEW)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            SET_SENSOR_VIEW,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_SENSOR_VIEW)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_SENSOR_VIEW)
        self.assertNotIn("ESkyguardMissionSkylineStyle", SET_SENSOR_VIEW)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_SENSOR_VIEW}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_SENSOR_VIEW),
            SET_SENSOR_VIEW,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SetSensorView contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, SET_SENSOR_VIEW.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache SetSensorView contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_SENSOR_VIEW)

    def test_contract_is_set_sensor_view_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", SET_SENSOR_VIEW)
        self.assertNotIn("{", SET_SENSOR_VIEW)
        self.assertTrue(SET_SENSOR_VIEW.startswith("void "))
        self.assertTrue(SET_SENSOR_VIEW.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
