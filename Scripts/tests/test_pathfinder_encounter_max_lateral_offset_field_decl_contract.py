...
# if ClampMin is missing, if the default is not `= 3600.f`, if Category is
...
# Do NOT claim Safety siblings MaxHeightFromOriginCm /
# MaxSimulationStepSeconds / MaxSimulationSubsteps.
...
# Clone wrap was leftover Pathfinder Route draft LockWindowSpeed
...
# (NO spaces around =).
# LOCKED_DECL is `float MaxLateralOffsetCm = 3600.f;`
...
# Fail-closed if ClampMin is missing or default is not 3600.f.
# Default is `= 3600.f` not `= 850.f` / `= 700.f` / `= 1250.f` /
# `= 9000.f`.
...
TARGET = "float MaxLateralOffsetCm = 3600.f;"
TARGET_WRONG_BARE = "float MaxLateralOffsetCm;"
TARGET_WRONG_INIT = "float MaxLateralOffsetCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float MaxLateralOffsetCm = true;"
TARGET_WRONG_ZERO = "float MaxLateralOffsetCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MaxLateralOffsetCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MaxLateralOffsetCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MaxLateralOffsetCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "MinHeightFromOriginCm"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_max_lateral_offset"
...
MAX_LATERAL_OFFSET_BARE_RE = re.compile(
    r"float\s+MaxLateralOffsetCm\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float MaxLateralOffsetCm = 3600.f;`.
    ...
    # MaxLateralOffsetCm without the 3600.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MAX_LATERAL_OFFSET_DECL_RE.search(compact) is None:
        return False
    if MAX_LATERAL_OFFSET_BARE_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MaxLateralOffsetCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaxLateralOffsetCm "
...
class PathfinderEncounterMaxLateralOffsetFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxLateralOffsetCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MaxLateralOffsetCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaxLateralOffsetCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_max_lateral_offset_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            ...
            compact_origin.split("MaxLateralOffsetCm = 3600.f;")[0]
            + compact_origin.split("MaxLateralOffsetCm = 3600.f;")[-1]
            if "MaxLateralOffsetCm = 3600.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_max_lateral_offset_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MaxLateralOffsetCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("3600.f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat MaxLateralOffsetCms = 3600.f;\n",
            "\tint32 MaxLateralOffsetCm = 850;\n",
            "\tFName MaxLateralOffsetCm = NAME_None;\n",
            "\tfloat MaxLateralOffsetCm = " + forty + ";\n",
            "\tfloat MaxLateralOffsetCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxLateralOffsetCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "MaxLateralOffsetCm"))
        self.assertTrue(has_identifier(section, "MaxLateralOffsetCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_max_lateral_offset_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
