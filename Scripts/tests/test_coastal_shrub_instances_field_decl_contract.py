# THIS IS leftover-safe ASkyguardCoastalEnvironmentDirector ShrubInstances.
...
# then BARE `TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances;`
# (no initializer).
...
# Stop BEFORE sibling WindSource as a claimed slot.
# THIS IS leftover-safe because it is the coastal director
# ACTOR TObjectPtr vegetation component, not leftover
# FSkyguardEnvironmentReadiness ShrubInstanceCount (int32 count),
# not leftover Mission01 OceanTiles/BeachTiles/LandTiles
# (different class), not leftover sibling TreeInstances,
# not leftover analog coastal-env-director-empty-fail-closed bulk.
# Do NOT claim leftover Root / TreeInstances / WindSource /
...
# Do NOT claim leftover Mission01 unique actor bools, leftover
# MissionMap, leftover Gunner Boom through CpgStation.
...
# Skyguard|MissionMap.
# LOCKED_DECL is `TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances;`
# NOT `FName WeatherIdentity;` and NOT leftover Boom and NOT
# leftover TreeInstances and NOT leftover Root / WindSource.
...
TARGET = "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances;"
TARGET_WRONG_INIT = "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> ShrubInstances;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> ShrubInstances;"
...
TARGET_WRONG_TEXT = "TObjectPtr<UTextRenderComponent> ShrubInstances;"
...
TARGET_WRONG_STATIC = "TObjectPtr<UStaticMeshComponent> ShrubInstances;"
TARGET_WRONG_CAMERA = "TObjectPtr<UCameraComponent> ShrubInstances;"
...
TARGET_WRONG_SHRUB_COUNT = "int32 ShrubInstanceCount = 0;"
...
SIBLING_SHRUB_COUNT = "ShrubInstanceCount"
...
LEFTOVER_ENV_READINESS_SHRUB_COUNT = (
...
LEFTOVER_LICENSED_VEGETATION = (
    "Scripts/tests/test_mission01_environment_readiness"
    "_licensed_vegetation_approved_field_decl_contract.py"
)
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        LEFTOVER_LICENSED_VEGETATION,
        LEFTOVER_ASSEMBLY_REVISION,
        f"{prefix}test_coastal_env_director_empty_fail_closed.py",
        f"{prefix}test_coastal_environment_director_empty_fail_closed.py",
...
SHRUB_DECL_RE = re.compile(
    r"TObjectPtr<UHierarchicalInstancedStaticMeshComponent>\s+ShrubInstances\s*;"
)
SHRUB_INIT_RE = re.compile(
    r"TObjectPtr<UHierarchicalInstancedStaticMeshComponent>\s+ShrubInstances\s*="
)
WRONG_CAMERA_SHRUB_RE = re.compile(
    r"TObjectPtr<UCameraComponent>\s+ShrubInstances\b"
)
WRONG_STATIC_SHRUB_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+ShrubInstances\b"
)
WRONG_SCENE_SHRUB_RE = re.compile(
    r"TObjectPtr<USceneComponent>\s+ShrubInstances\b"
)
WRONG_TAIL_BOOM_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+TailBoom\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # TreeInstances as the locked decl, leftover
    # ShrubInstanceCount, or an initializer on
    # ShrubInstances as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SHRUB_DECL_RE.search(compact) is None:
        return False
    if SHRUB_INIT_RE.search(compact):
        return False
    if WRONG_CAMERA_SHRUB_RE.search(compact):
        return False
    if WRONG_STATIC_SHRUB_RE.search(compact):
        return False
    if WRONG_SCENE_SHRUB_RE.search(compact):
        return False
    if WRONG_TAIL_BOOM_RE.search(compact):
        return False
    if WEATHER_IDENTITY_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if "CpgConsoleRight" in compact and "ShrubInstances;" not in compact:
        return False
    if re.search(r"TObjectPtr<USpringArmComponent>\s+Boom\b", compact):
        return False
    if re.search(r"bool\s+bLicensedVegetationLibraryApproved\b", compact):
        return False
    if re.search(r"FName\s+AssemblyRevision\b", compact):
        return False
    if re.search(r"LookYawLimit\s*=\s*95\.f\b", compact):
        return False
    return True
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_WIND_SOURCE):
            break
    ...
    if has_identifier(window, SIBLING_WIND_SOURCE):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_WIND_SOURCE}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UHierarchicalInstancedStaticMeshComponent>\s+ShrubInstances\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances "
...
class CoastalShrubInstancesFieldDeclContractTests(unittest.TestCase):
    def test_coastal_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ShrubInstances"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn("public:", header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_WIND_SOURCE), body)
        self.assertFalse(
            has_identifier(section, SIBLING_WIND_SOURCE),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_TREE_INSTANCES), body)
        self.assertTrue(has_identifier(section, SIBLING_TREE_INSTANCES), section)
        self.assertNotIn(SIBLING_TREE_INSTANCES, LOCKED_DECL)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ShrubInstances"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ShrubInstances", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_shrub_instances_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Environment|Vegetation"', specifiers)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances;", compact_origin)
        self.assertNotIn("Boom = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_shrub_instances_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("UHierarchicalInstancedStaticMeshComponent", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstances;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TREE)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SHRUB_COUNT)
        ...
        self.assertNotIn("ShrubInstanceCount", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> ShrubInstance;\n",
            "\tint32 ShrubInstances = 140;\n",
            "\tbool ShrubInstances = true;\n",
            "\tfloat ShrubInstances = " + forty + ";\n",
            "\tfloat ShrubInstances = " + eighty + ";\n",
    ...
    def test_missing_category_or_visible_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Environment|Vegetation"', origin)
        ...
        self.assertIsNotNone(
            re.search(
                r'Category="Skyguard\|Environment\|Vegetation"',
                origin,
            )
        )
    ...
    def test_does_not_claim_sibling_wind_source_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "ShrubInstances"))
        self.assertFalse(has_identifier(section, SIBLING_WIND_SOURCE))
        self.assertFalse(has_identifier(section, SIBLING_GUNNER_CAMERA))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_WIND_SOURCE))
        self.assertTrue(has_identifier(section, "ShrubInstances"))
        self.assertFalse(has_identifier(section, SIBLING_WIND_SOURCE))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_QUALITY))
        self.assertFalse(has_identifier(section, SIBLING_READINESS))
        self.assertTrue(has_identifier(leaked, SIBLING_READINESS))
        self.assertFalse(has_identifier(section, SIBLING_APPLIED_WEATHER))
        self.assertFalse(has_identifier(section, SIBLING_CPG_TEDAC))
        self.assertFalse(has_identifier(section, SIBLING_CPG_MPD_LEFT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_MPD_RIGHT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "coastal ShrubInstances field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "coastal_shrub_instances_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_ENV_READINESS_SHRUB_COUNT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("ShrubInstanceCount", locked_only)
