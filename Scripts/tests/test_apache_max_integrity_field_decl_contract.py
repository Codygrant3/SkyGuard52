# THIS IS leftover-safe ASkyguardApacheAircraft MaxIntegrity.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite,
# Category="Skyguard|Apache|Damage", meta=(ClampMin="1.0"))
# then BARE `float MaxIntegrity = 140.f;`.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Apache|Damage`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Type IS float.
# Initializer IS `= 140.f`. meta ClampMin IS `1.0`.
...
# type is not float, or if initializer is not `= 140.f`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `ASkyguardApacheAircraft` public
# UPROPERTY fields ONLY after
# `class ASkyguardApacheAircraft`.
# Stop BEFORE `protected:`.
# Stop BEFORE sibling CurrentIntegrity / HullCollider
# as a claimed slot.
...
# Do NOT parse leftover ASkyguardProtectAsset MaxIntegrity
# (different class; same identifier is analog leftover
# — lock FULL class ASkyguardApacheAircraft + identifier
# MaxIntegrity).
# Do NOT parse leftover LoadoutSpec HullIntegrity
# (140.f analog; identifier is HullIntegrity not
# MaxIntegrity).
...
# analog campaign-roster-lookup-tests, leftover
# campaign-roster function decls.
...
# Better same-wrap retarget: leftover-safe ArcadeLook
# bEnabled / Contrast were EditAnywhere
# BlueprintReadWrite Category="Skyguard|Arcade"
# with NO ClampMin.
# RETARGET: type is float, identifier is MaxIntegrity,
# initializer is 140.f, wrap is EditAnywhere
# BlueprintReadWrite Category="Skyguard|Apache|Damage"
# plus meta ClampMin="1.0".
# Fail-closed if this test still asserts
# UPROPERTY / Category / VisibleAnywhere /
# EditAnywhere / BlueprintReadOnly / GENERATED_BODY /
# USTRUCT / FName WeatherIdentity /
# Category="Skyguard|Theater" / 160.f AS THE LOCKED
# DECL. Wrap assertions for THIS slot's EditAnywhere
# BlueprintReadWrite Category Apache|Damage + meta
# ClampMin are required.
...
LOCKED_DECL = "float MaxIntegrity = 140.f;"
UPROPERTY_EDIT_WRITE = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
    'Category="Skyguard|Apache|Damage", '
    'meta=(ClampMin="1.0"))'
)
UPROPERTY_EDIT_WRITE_SPLIT = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
    'Category="Skyguard|Apache|Damage",\n'
    '\t\tmeta=(ClampMin="1.0"))'
)
...
TARGET_WRONG_ONE = "float MaxIntegrity = 1.f;"
TARGET_WRONG_NO_F = "float MaxIntegrity = 140;"
TARGET_WRONG_FNAME = "FName MaxIntegrity;"
TARGET_WRONG_INT = "int32 MaxIntegrity = 0;"
TARGET_WRONG_BOOL = "bool MaxIntegrity = true;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_PROTECT = "float MaxIntegrity = 100.f;"
TARGET_WRONG_HULL = "float HullIntegrity = 140.f;"
...
TARGET_WRONG_CANNON_RATE = "float MaxIntegrity = 12.0f;"
TARGET_WRONG_CANNON_RECOIL = "float MaxIntegrity = 0.92f;"
TARGET_WRONG_CANNON_DAMAGE = "float MaxIntegrity = 22.0f;"
...
TARGET_WRONG_NO_META_WRAP = (
    "UPROPERTY(EditAnywhere, BlueprintReadWrite, "
    'Category="Skyguard|Apache|Damage")'
)
STOP_BEFORE_PROTECTED = "protected:"
STOP_BEFORE_CURRENT = "float CurrentIntegrity = 140.f;"
STOP_BEFORE_HULL_COLLIDER = "TObjectPtr<UBoxComponent> HullCollider;"
...
def leftover_harbor_tokens() -> tuple[str, ...]:
    ...
    # the authored 140.f initializer as Harbor 40.f.
    forty = "40" + ".f"
    eighty = "80" + ".f"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float MaxIntegrity = 140.f;`.
    # Do not accept leftover TheaterKit WeatherIdentity /
    # leftover ProtectAsset MaxIntegrity = 100.f /
    # leftover LoadoutSpec HullIntegrity /
    # leftover ArcadeLook Contrast / bEnabled /
    # leftover 160.f Health when origin/main has
    # ASkyguardApacheAircraft MaxIntegrity = 140.f.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+MaxIntegrity\s*=\s*140\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"FName\s+WeatherIdentity\b", compact):
        return False
    if re.search(r"float\s+HullIntegrity\b", compact):
        return False
    if re.search(r"float\s+Contrast\b", compact):
        return False
    if re.search(r"bool\s+bEnabled\b", compact):
        return False
    if re.search(r"float\s+MaxIntegrity\s*=\s*100\.f\s*;", compact):
        return False
    if re.search(r"float\s+MaxIntegrity\s*=\s*160\.f\s*;", compact):
        return False
    if re.search(r"FName\s+MaxIntegrity\b", compact):
        return False
    if re.search(r"int32\s+MaxIntegrity\b", compact):
        return False
    if re.search(r"bool\s+MaxIntegrity\b", compact):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*float\s+MaxIntegrity\s*=\s*140\.f\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if LEFTOVER_HULL_INTEGRITY in window:
                raise AssertionError(
                    "claimed window includes leftover LoadoutSpec "
                    "HullIntegrity"
                )
            if STOP_BEFORE_PROTECTED in window:
                raise AssertionError(
                    "claimed window includes protected:"
                )
            return window
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaxIntegrity = 140.f; is missing from "
...
class ApacheMaxIntegrityFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxIntegrity"), section)
        ...
        self.assertNotIn(STOP_BEFORE_PROTECTED, claimed)
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin="1.0"', UPROPERTY_EDIT_WRITE)
        self.assertIn("meta=", UPROPERTY_EDIT_WRITE)
        ...
        self.assertIn('Category="Skyguard|Apache|Damage"', specifiers)
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        self.assertEqual(LOCKED_DECL, "float MaxIntegrity = 140.f;")
        ...
        self.assertIn("MaxIntegrity", LOCKED_DECL)
    ...
    def test_locked_decl_requires_140_initializer(self) -> None:
        self.assertIn("= 140.f", LOCKED_DECL)
        ...
        self.assertIn("float MaxIntegrity = 140.f;", compact_origin)
        self.assertNotIn("MaxIntegrity = 100.f", compact_origin)
        self.assertNotIn("MaxIntegrity = 160.f", compact_origin)
        self.assertNotIn("MaxIntegrity = NAME_None", compact_origin)
        self.assertNotIn("MaxIntegrity = false", compact_origin)
        self.assertNotIn("MaxIntegrity = true", compact_origin)
        self.assertNotIn("MaxIntegrity = 0.f", compact_origin)
    ...
    def test_max_integrity_declaration_matches_origin_main(self) -> None:
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HULL}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat MaxIntegrities = 140.f;\n",
            "\tint32 MaxIntegrity = 140;\n",
            "\tbool MaxIntegrity = true;\n",
            "\tfloat MaxIntegrity = " + forty + ";\n",
            "\tfloat MaxIntegrity = " + eighty + ";\n",
        )
    ...
    def test_missing_category_edit_or_clamp_fails_closed(self) -> None:
        no_category = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertIn('Category="Skyguard|Apache|Damage"', origin)
        ...
        no_meta = (
            ...
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_NO_META_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        no_meta_specs = attached_uproperty_specifiers(
            spec_section(no_meta)
        )
        self.assertNotIn("ClampMin", no_meta_specs)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        section = spec_section(header)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertIn(UPROPERTY_EDIT_WRITE_SPLIT, section)
        self.assertIn(
            collapsed(UPROPERTY_EDIT_WRITE),
            collapsed(section),
        )

    def test_does_not_contract_sibling_integrity_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertIn(STOP_BEFORE_CURRENT, section)
        self.assertNotIn(STOP_BEFORE_CURRENT, claimed)
        self.assertIn(SIBLING_HULL_COLLIDER, section)
        self.assertNotIn(SIBLING_HULL_COLLIDER, claimed)

    def test_claimed_window_stops_before_current_integrity(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("MaxIntegrity", claimed)
        self.assertIn("= 140.f", claimed)
        self.assertIn("UPROPERTY", claimed)
        self.assertIn("EditAnywhere", claimed)
        self.assertIn("BlueprintReadWrite", claimed)
        self.assertIn('Category="Skyguard|Apache|Damage"', claimed)
        self.assertIn('ClampMin="1.0"', claimed)
        self.assertNotIn(SIBLING_CURRENT_INTEGRITY, claimed)
        self.assertNotIn(STOP_BEFORE_CURRENT, claimed)
        self.assertNotIn(SIBLING_HULL_COLLIDER, claimed)
        self.assertNotIn(STOP_BEFORE_PROTECTED, claimed)
        mixed = (
            ...
            f"\t{LOCKED_DECL}\n"
            f"\t{TARGET_WRONG_READONLY_WRAP}\n"
            f"\t{STOP_BEFORE_CURRENT}\n"
            "};\n"
        )
        window = claimed_field_window(mixed)
        self.assertTrue(has_declaration(window, LOCKED_DECL), window)
        self.assertNotIn(SIBLING_CURRENT_INTEGRITY, window)

    def test_parse_window_stops_before_protected(self) -> None:
        header = origin_main_header()
        public = public_section(header)
        section = spec_section(header)
        self.assertNotIn(STOP_BEFORE_PROTECTED, public)
        self.assertNotIn(STOP_BEFORE_PROTECTED, section)
        self.assertIn("AircraftRoot", class_body(header))
        self.assertNotIn("AircraftRoot", public)
        self.assertNotIn("AircraftRoot", section)
        mixed = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_parse_leftover_protect_asset_max_integrity(self) -> None:
        self.assertNotEqual(CLASS_NAME, LEFTOVER_PROTECT_ASSET_CLASS)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_PROTECT_ASSET_DECL)
        self.assertNotEqual(
            UPROPERTY_EDIT_WRITE,
    ...
    def test_leftover_theater_kit_wrap_is_not_locked_wrap(self) -> None:
        ...
        self.assertNotIn("= 140.f", LEFTOVER_THEATER_KIT_DECL)
        self.assertIn("= 140.f", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache MaxIntegrity field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "max_integrity_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_CURRENT, locked_only)
        self.assertNotIn(SIBLING_CURRENT_INTEGRITY, claimed)
        self.assertNotIn(SIBLING_HULL_COLLIDER, claimed)
