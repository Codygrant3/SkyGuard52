# THIS IS leftover-safe FSkyguardMissionDebrief::SaveSlotName.
# origin/main form: split-line
# `UPROPERTY(VisibleAnywhere, BlueprintReadOnly)`
# then `FString SaveSlotName;` (bare FString, NO initializer,
# no Category=). If a clone asserts `= false` / `= 0` / an
# initializer, drop that and accept the bare decl. If a clone
# asserts Category= and origin/main has none, drop that
# assertion.
...
# THIS IS NOT leftover FSkyguardObjectiveProgress::State (sibling
# ObjectiveProgress State contract; different struct).
# THIS IS NOT leftover HowToFlyRow #1258-#1260.
# THIS IS NOT leftover ObjectiveProgress ObjectiveId / CurrentProgress
# #1261 / #1262 / State #1263.
# THIS IS NOT leftover MissionResult FinalScore / MedalTier
# #1264 / #1265.
# THIS IS NOT leftover MissionDebrief Result #1266 / State #1267 /
# MissionDisplayName #1268.
...
# bProgressSaved, NextMissionId, NextMissionDisplayName,
# NextMissionMap, bNextMissionUnlocked, bCampaignComplete).
# Do not contract FSkyguardMissionResult fields.
# Parse STRUCT public section ONLY after
# struct FSkyguardMissionDebrief and stop at that struct's closing
# }; before any following USTRUCT/UCLASS/UENUM.
# Do NOT parse enum class ESkyguardMissionDebriefState body except
# as a neighbor type.
# Do NOT parse FSkyguardMissionResult / FSkyguardObjectiveProgress.
# Do NOT parse USkyguardMission01EnvironmentAuthoringLibrary.
# Do NOT parse ASkyguardMission01EnvironmentDirector.
# Do NOT parse landscape/authoring structs.
...
# Do NOT parse FSkyguardHowToFlyRow / FSkyguardBriefingCard /
# FSkyguardBriefingRadioRow.
# Do NOT parse leftover GetObjectiveRuntime / AddObjectiveProgress.
# Accept split-line UPROPERTY VisibleAnywhere, BlueprintReadOnly,
# no Category=. Accept bare `FString SaveSlotName;`.
# Fail-closed if type/decl missing, renamed, Category= added, or
# an initializer appears.
...
# mission-result-defaults #c4db, leftover mission-debrief-defaults
# #4220, leftover briefing-card-defaults #178/#8c3f, leftover
# briefing-radio-row-defaults #190/#bf8e, leftover how-to-fly-row
# defaults #191/#9ab2, leftover AuthoringResult through #1250,
# leftover BriefingCard #1251-#1254, leftover RadioRow Speaker
# #1255 / LineId #1256 / Subtitle #1257.
...
TARGET = "FString SaveSlotName;"
TARGET_WRONG_FALSE = "FString SaveSlotName = false;"
TARGET_WRONG_ZERO = "FString SaveSlotName = 0;"
TARGET_WRONG_TRUE = "FString SaveSlotName = true;"
TARGET_WRONG_FLOAT = "FString SaveSlotName = 0.f;"
TARGET_WRONG_INIT = "FString SaveSlotName = FString();"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
# Debrief is last in SkyguardMissionTypes.h. No following
# USTRUCT/UCLASS/UENUM. Sentinel must not leak into the window.
STOP_AFTER = "UCLASS("
NEIGHBOR_RESULT = "struct FSkyguardMissionResult"
NEIGHBOR_PROGRESS = "struct FSkyguardObjectiveProgress"
ENUM_DEBRIEF_HEAD = "enum class ESkyguardMissionDebriefState"
NEIGHBOR_CARD = "struct FSkyguardBriefingCard"
NEIGHBOR_RADIO = "struct FSkyguardBriefingRadioRow"
NEIGHBOR_HOW_TO_FLY = "struct FSkyguardHowToFlyRow"
COMPONENT_CLASS = "USkyguardSortiePresentationComponent"
ENUM_HEAD = "enum class ESkyguardBriefingPictogram"
AUTHORING_LIB = "USkyguardMission01EnvironmentAuthoringLibrary"
AUTHORING_DIR = "ASkyguardMission01EnvironmentDirector"
AUTHORING_READY = "FSkyguardMission01EnvironmentReadiness"
PROP_SPINNER = "ASkyguardPropSpinner"
ADD_PROGRESS = "AddObjectiveProgress"
GET_RUNTIME = "GetObjectiveRuntime"
THIS_SCRIPT = (
    "Scripts/tests/test_mission_debrief_save_slot_name"
    "_field_decl_contract.py"
)
CLONE_DEBRIEF_NEW_BEST_MEDAL = (
    "Scripts/tests/test_mission_debrief_new_best_medal"
    "_field_decl_contract.py"
)
CLONE_DEBRIEF_PROGRESS_SAVED = (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare decl. Do not accept = 0 /
    # = false / = 1 / = 0.f / = true / FString() for
    # FSkyguardMissionDebrief::SaveSlotName.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    for match in re.finditer(r"UPROPERTY\(", compact):
        start = match.end()
        depth = 1
        index = start
        ...
        if depth != 0:
            continue
        after = compact[index:]
        if re.match(r"\s*FString\s+SaveSlotName\b", after):
            return compact[start : index - 1]
    raise AssertionError(
        "UPROPERTY for FString SaveSlotName is missing from "
...
class MissionDebriefSaveSlotNameFieldDeclContractTests(unittest.TestCase):
    def test_mission_debrief_struct_exists(self) -> None:
        ...
        self.assertIn("SaveSlotName", section)
        self.assertIn("State", section)
        self.assertIn("Result", section)
        self.assertIn("MissionDisplayName", section)
        ...
        self.assertNotIn(ENUM_DEBRIEF_HEAD, section)
        self.assertNotIn(NEIGHBOR_CARD, section)
        self.assertNotIn(NEIGHBOR_RADIO, section)
        self.assertNotIn(NEIGHBOR_HOW_TO_FLY, section)
        self.assertNotIn(COMPONENT_CLASS, section)
        self.assertNotIn(ENUM_HEAD, section)
        self.assertNotIn(AUTHORING_LIB, section)
        self.assertNotIn(AUTHORING_DIR, section)
        self.assertNotIn(AUTHORING_READY, section)
        self.assertNotIn(PROP_SPINNER, section)
        self.assertNotIn(ADD_PROGRESS, section)
        self.assertNotIn(GET_RUNTIME, section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("SaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_save_slot_name_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_save_slot_name_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString SaveSlotName"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "FString SaveSlotName;")
        self.assertNotIn("=", LOCKED_DECL.replace(";", ""))
        ...
        self.assertNotIn("Unavailable", LOCKED_DECL)
        self.assertNotIn("Inactive", LOCKED_DECL)
        self.assertNotIn("Ready", LOCKED_DECL)
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn("Result", LOCKED_DECL)
        self.assertNotIn("MissionDisplayName", LOCKED_DECL)
        self.assertNotIn("Pictogram", LOCKED_DECL)
        self.assertNotIn("Priority", LOCKED_DECL)
        self.assertFalse(
    ...
    def test_initializer_fails_closed(self) -> None:
        dropped = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_VISIBLE}\n"
        ...
        section = public_section(dropped)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertNotIn("= false", LOCKED_DECL)
        self.assertIn("SaveSlotName", origin)
        self.assertIn("FString SaveSlotName;", origin)
    ...
    def test_does_not_contract_sibling_fields_or_neighbors(self) -> None:
        self.assertIn("SaveSlotName", LOCKED_DECL)
        self.assertNotIn("State", LOCKED_DECL)
        self.assertNotIn("Result", LOCKED_DECL)
        self.assertNotIn("MissionDisplayName", LOCKED_DECL)
        self.assertNotIn("Narrative", LOCKED_DECL)
        self.assertNotIn("bNewBestScore", LOCKED_DECL)
        self.assertNotIn("bNewBestMedal", LOCKED_DECL)
        self.assertNotIn("bProgressSaved", LOCKED_DECL)
        self.assertNotIn("NextMissionId", LOCKED_DECL)
        self.assertNotIn("NextMissionDisplayName", LOCKED_DECL)
        self.assertNotIn("NextMissionMap", LOCKED_DECL)
        self.assertNotIn("bNextMissionUnlocked", LOCKED_DECL)
        self.assertNotIn("bCampaignComplete", LOCKED_DECL)
        self.assertNotIn("FinalScore", LOCKED_DECL)
        self.assertNotIn("MedalTier", LOCKED_DECL)
        self.assertNotIn("ObjectiveId", LOCKED_DECL)
        self.assertNotIn("CurrentProgress", LOCKED_DECL)
        self.assertNotIn("Pictogram", LOCKED_DECL)
        self.assertNotIn("Priority", LOCKED_DECL)
        self.assertNotIn("Title", LOCKED_DECL)
        self.assertNotIn("CardId", LOCKED_DECL)
        self.assertNotIn("LineId", LOCKED_DECL)
        self.assertNotIn("Speaker", LOCKED_DECL)
        self.assertNotIn("Body", LOCKED_DECL)
        self.assertNotIn("Subtitle", LOCKED_DECL)
        self.assertNotIn("StepId", LOCKED_DECL)
        self.assertNotIn("InputHint", LOCKED_DECL)
        self.assertNotIn("Instruction", LOCKED_DECL)
        self.assertNotIn("MultiLine", LOCKED_DECL)
        self.assertNotIn("meta=", LOCKED_DECL)
        section = public_section(origin_main_header())
        self.assertIn("Result", section)
        self.assertIn("MissionDisplayName", section)
        self.assertIn("Narrative", section)
        self.assertIn("bNewBestScore", section)
        self.assertIn("bNewBestMedal", section)
        self.assertIn("bProgressSaved", section)
        self.assertIn("SaveSlotName", section)
        self.assertIn("bCampaignComplete", section)
        for leftover in leftover_pictogram_values():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, LOCKED_DECL)

    def test_does_not_parse_neighbors_or_leftover_methods(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIn(NEIGHBOR_RESULT, header)
        self.assertIn(NEIGHBOR_PROGRESS, header)
        self.assertIn(ENUM_DEBRIEF_HEAD, header)
        self.assertNotIn(STOP_AFTER, header)
        self.assertNotIn(STOP_AFTER, section)
        self.assertNotIn(NEIGHBOR_RESULT, section)
        self.assertNotIn(NEIGHBOR_PROGRESS, section)
        self.assertNotIn(ENUM_DEBRIEF_HEAD, section)
        ...
        self.assertNotIn(ENUM_DEBRIEF_HEAD, leaked)
        self.assertNotIn(ADD_PROGRESS, leaked)
        self.assertNotIn(GET_RUNTIME, leaked)
        self.assertNotIn("GetBriefingCards", leaked)
        self.assertNotIn("BuildHowToFlyRows", leaked)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            "\tint32 FinalScore = 0;\n"
            "\tint32 MedalTier = 0;\n"
            "\tvoid AddObjectiveProgress(FName ObjectiveId, int32 Amount);\n"
            "\tFSkyguardObjectiveProgress GetObjectiveRuntime("
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief SaveSlotName field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_save_slot_name_field_decl_contract.py"
        ))
        self.assertIn(CLONE_DEBRIEF_STATE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEBRIEF_RESULT, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEBRIEF_DISPLAY_NAME, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEBRIEF_NARRATIVE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEBRIEF_NEW_BEST_SCORE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEBRIEF_NEW_BEST_MEDAL, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEBRIEF_PROGRESS_SAVED, LOCKED_SCRIPTS)
        self.assertIn(CLONE_OBJECTIVE_ID, LOCKED_SCRIPTS)
        self.assertIn(CLONE_CURRENT_PROGRESS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_OBJECTIVE_STATE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_ADD_PROGRESS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RESULT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_DEBRIEF_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RESULT_FINAL_SCORE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RESULT_MEDAL_TIER, LOCKED_SCRIPTS)
        self.assertIn(CLONE_INSTRUCTION, LOCKED_SCRIPTS)
        ...
        self.assertIn(CLONE_LINE_ID, LOCKED_SCRIPTS)
        self.assertIn(CLONE_SPEAKER, LOCKED_SCRIPTS)
        self.assertIn(CLONE_SUBTITLE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_STEP_ID, LOCKED_SCRIPTS)
        self.assertIn(CLONE_INPUT_HINT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertIn("FString SaveSlotName;", locked_only)
        self.assertNotIn("State", locked_only)
        self.assertNotIn("Result", locked_only)
        self.assertNotIn("MissionDisplayName", locked_only)
        self.assertNotIn("bNewBestScore", locked_only)
        self.assertNotIn("bNewBestMedal", locked_only)
