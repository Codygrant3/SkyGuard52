# THIS IS leftover-safe FSkyguardNightSortieBeat Threat.
...
# missing or is not ESkyguardThreatKind::FastAttacker, or
# if a UPROPERTY / Category clone lands as the locked decl.
# Do NOT claim leftover DaySortieBeat
...
# locked decl is the bare FastAttacker field.
...
TARGET_WRONG_KIND_TYPE = (
    "ESkyguardNightSortieBeatKind Threat = "
    "ESkyguardThreatKind::FastAttacker;"
)
TARGET_WRONG_RENAME = (
    "ESkyguardThreatKind Threats = ESkyguardThreatKind::FastAttacker;"
)
TARGET_WRONG_KIND_ID = (
    "ESkyguardThreatKind Kind = ESkyguardThreatKind::FastAttacker;"
)
TARGET_WRONG_CALL_ID = (
    "ESkyguardThreatKind Call = ESkyguardThreatKind::FastAttacker;"
)
TARGET_WRONG_HEAVY = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardThreatKind Threat = ESkyguardThreatKind::FastAttacker;`.
    # Do not accept missing initializer, leftover DaySortieBeat
    # GroundArmor, wrong type, sibling Kind / Call, or a
    ...
    if re.search(
        ...
        r"ESkyguardThreatKind::FastAttacker\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"ESkyguardThreatKind\s+Threat\s*=\s*"
        r"ESkyguardThreatKind::GroundArmor\b",
        compact,
    ):
        return False
    if re.search(
    ...
    if re.search(
        ...
        r"ESkyguardNightSortieBeatKind)\s+Threat\b",
        compact,
    ):
        return False
...
class NightSortieBeatThreatFieldDeclContractTests(unittest.TestCase):
    ...
    def test_origin_main_threat_is_bare_plain_cpp(self) -> None:
        ...
        self.assertNotIn("GroundArmor", section)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_initializer_fails_closed_when_missing_or_wrong(self) -> None:
        ...
        armor = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_GROUND_ARMOR}\n"
            "};\n"
        )
        armor_section = spec_section(armor)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(armor_section, LOCKED_DECL)
        self.assertIn("Threat", str(raised.exception))
        ...
        self.assertNotIn("Threat = ESkyguardThreatKind::GroundArmor", compact_origin)
    ...
    def test_threat_declaration_matches_origin_main(self) -> None:
        ...
        self.assertIn("= ESkyguardThreatKind::FastAttacker", LOCKED_DECL)
        self.assertNotIn("GroundArmor", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_GROUND_ARMOR}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_does_not_claim_day_sortie_ground_armor(self) -> None:
        day = (
            f"struct {DAY_STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_GROUND_ARMOR}\n"
        ...
        section = spec_section(day)
        self.assertNotIn("GroundArmor", section)
        self.assertNotIn(DAY_STRUCT_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Threat", str(raised.exception))
        origin = spec_section(origin_main_header())
        self.assertNotIn("GroundArmor", origin)
        self.assertNotIn(TARGET_WRONG_GROUND_ARMOR, origin)
        self.assertNotIn(TARGET_WRONG_GROUND_ARMOR, LOCKED_DECL)
    ...
    def test_does_not_parse_enum_kit_namespace_or_neighbors(self) -> None:
        ...
        self.assertNotIn(DAY_STRUCT_NAME, header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "night-sortie-beat Threat field decl contract "
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(DAY_STRUCT_NAME, locked_only)
