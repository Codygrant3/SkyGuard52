# THIS IS leftover-safe ASkyguardApacheAircraft PilotMount.
...
# then BARE `TObjectPtr<USceneComponent> PilotMount;` (no initializer).
...
# Stop BEFORE sibling EyeMount as a claimed slot.
# Do not claim GunnerMount / EyeMount / WeaponMount /
# HellfireLeft / HellfireRight / PilotCanopy /
# leftover PilotMount analog AircraftRoot / HullCollider /
# RotorPower.
# PilotMount fail-closed if LOCKED_DECL is leftover analog
# apache-mount-fail-closed, leftover PilotCanopy,
# leftover GunnerMount, leftover EyeMount, leftover
...
# `TObjectPtr<USceneComponent> PilotMount;` NOT
...
LOCKED_DECL = "TObjectPtr<USceneComponent> PilotMount;"
...
LEFTOVER_WEAPON_MOUNT_DECL = (
    "TObjectPtr<USceneComponent> WeaponMount;"
)
LEFTOVER_PILOT_CANOPY_DECL = (
    "TObjectPtr<UStaticMeshComponent> PilotCanopy;"
)
LEFTOVER_HELLFIRE_LEFT_DECL = (
    "TObjectPtr<UStaticMeshComponent> HellfireLeft;"
)
LEFTOVER_HELLFIRE_RIGHT_DECL = (
    "TObjectPtr<UStaticMeshComponent> HellfireRight;"
)
LEFTOVER_MESH_TYPE_DECL = (
    "TObjectPtr<UStaticMeshComponent> PilotMount;"
)
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_apache_pilot_canopy_field_decl_contract.py",
...
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> PilotMount = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> PilotMount = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> PilotMount = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> PilotMount = 0.f;"
TARGET_WRONG_FLOAT = "float PilotMount;"
TARGET_WRONG_FNAME = "FName PilotMount;"
TARGET_WRONG_INT = "int32 PilotMount = 0;"
TARGET_WRONG_BOOL = "bool PilotMount = true;"
...
STOP_BEFORE_EYE_MOUNT = "EyeMount"
...
PILOT_MOUNT_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*PilotMount\s*;"
)
PILOT_MOUNT_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*PilotMount\s*="
)
PILOT_MOUNT_FLOAT_RE = re.compile(r"float\s+PilotMount\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if PILOT_MOUNT_INIT_RE.search(compact):
        return False
    if PILOT_MOUNT_FLOAT_RE.search(compact):
        return False
    if PILOT_MOUNT_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+PilotMount\b", compact):
        return False
    if re.search(r"int32\s+PilotMount\b", compact):
        return False
    if re.search(r"bool\s+PilotMount\b", compact):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*TObjectPtr<USceneComponent>\s*PilotMount\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if STOP_BEFORE_EYE_MOUNT in window:
                raise AssertionError(
                    "claimed window includes sibling EyeMount"
                )
            return window
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> PilotMount; is "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*PilotMount\b",
            compact[index:],
        ):
...
class ApachePilotMountFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PilotMount"), section)
        ...
        self.assertNotIn(SIBLING_PILOT_CANOPY, claimed)
        self.assertNotIn(SIBLING_AIRCRAFT_ROOT, claimed)
        self.assertNotIn(STOP_BEFORE_PRIVATE, claimed)
        self.assertNotIn(STOP_BEFORE_EYE_MOUNT, claimed)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("PilotMount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_pilot_mount_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_WEAPON_MOUNT_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_PILOT_CANOPY_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_HELLFIRE_LEFT_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_HELLFIRE_RIGHT_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_MESH_TYPE_DECL)
        ...
        self.assertNotIn("PilotCanopy", LOCKED_DECL)
        ...
        self.assertIn("PilotMount", LOCKED_DECL)
    ...
    def test_locked_decl_requires_bare_pointer(self) -> None:
        ...
        self.assertNotIn("PilotMount = nullptr", compact_origin)
        self.assertNotIn("PilotMount = NAME_None", compact_origin)
        self.assertNotIn("PilotMount = false", compact_origin)
        self.assertNotIn("PilotMount = true", compact_origin)
        self.assertNotIn("PilotMount = 0.f", compact_origin)
        self.assertNotIn("PilotMount = 160.f", compact_origin)

    def test_pilot_mount_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> PilotMount"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USceneComponent> PilotMounts;\n",
            "\tTObjectPtr<UStaticMeshComponent> PilotMount;\n",
            "\tTObjectPtr<USceneComponent> Root;\n",
            "\tFName WeatherIdentity;\n",
            "\tfloat PilotMount = " + forty + ";\n",
            "\tfloat PilotMount = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_fields(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_EYE_MOUNT, section)
        self.assertNotIn(STOP_BEFORE_EYE_MOUNT, claimed)
    ...
    def test_claimed_window_stops_before_eye_mount(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("PilotMount", claimed)
        ...
        self.assertNotIn(SIBLING_EYE_MOUNT, window)
    ...
    def test_parse_window_starts_at_protected_stops_before_private(self) -> None:
        ...
        self.assertIn("PilotMount", class_body(header))
        self.assertIn("PilotMount", protected)
        self.assertIn("PilotMount", section)
        self.assertFalse(
            has_declaration(public, LOCKED_DECL), public
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache PilotMount field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pilot_mount_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_PILOT_CANOPY_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_IS_CANOPY_CRACKED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SET_ROTOR_POWER_SCRIPT, LOCKED_SCRIPTS)
