# THIS IS leftover-safe FSkyguardRoutePoint WorldLocation.
...
# Do NOT contract sibling fields PointId / TargetAirspeedKph /
# LookAheadSeconds / bAllowCombatOrbit.
# PointId is in-flight leftover
# cursor/route-point-point-id-field-decl-contract-c332
# (`FName PointId;` no initializer).
# TargetAirspeedKph remaining (`float TargetAirspeedKph = 220.f;`).
# LookAheadSeconds remaining (`float LookAheadSeconds = 2.f;`).
# bAllowCombatOrbit remaining (`bool bAllowCombatOrbit = true;`).
# Do NOT reopen leftover RadioLine isolated fields #1332-#1337
# or in-flight CooldownSeconds.
# Do NOT reopen leftover radio-line-defaults #172 / #1dec,
# leftover radio-chatter empty fail-closed, or TheaterKit
# #1299-#1331. Keep leftover bulk
# test_campaign_theater_kit_contract.py locked.
# Analog: leftover route-point-defaults #180 then this isolated
# WorldLocation decl, same as leftover radio-line-defaults #172
# then isolated RadioLine fields, same as leftover
# briefing-card-defaults then CardId.
# If a clone asserts FName / FText / float / no `=` /
# Category="Skyguard|Theater" / VisibleAnywhere /
# BlueprintReadWrite / ClampMin, retarget: type is FVector,
# initializer is `= FVector::ZeroVector`, EditAnywhere +
...
SIBLING_SOUND = SIBLING_ALLOW_COMBAT_ORBIT
...
SIBLING_BUILDING_TINT = SIBLING_ALLOW_COMBAT_ORBIT
...
LEFTOVER_ROUTE_POINT_COMBAT_ORBIT = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept sibling PointId / TargetAirspeedKph /
    # LookAheadSeconds / bAllowCombatOrbit fields.
...
class RoutePointWorldLocationFieldDeclContractTests(unittest.TestCase):
    def test_route_point_struct_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "WorldLocation"),
            section,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            "\tFVector WorldLocatio;\n",
            "\tint32 WorldLocation = 2;\n",
            "\tbool WorldLocation;\n",
            "\tFText WorldLocation;\n",
            f"\t{TARGET_WRONG_TWO}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "route-point WorldLocation field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_ROUTE_POINT_COMBAT_ORBIT, LOCKED_SCRIPTS)
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "world_location_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("bAllowCombatOrbit", locked_only)
        for helper in namespace_helper_tokens():
