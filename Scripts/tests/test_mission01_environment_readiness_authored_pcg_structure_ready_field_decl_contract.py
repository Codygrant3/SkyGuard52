...
# bAuthoredPCGStructureReady FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bAuthoredPCGStructureReady = false; one-line /
...
# ASkyguardMission01EnvironmentDirector. Not leftover director class.
# Not leftover Mission01 environment-readiness-defaults #6b9d/#204.
# Not leftover director OceanTiles #585 / BeachTiles #586 / LandTiles
# #587 / RouteExclusion #588 / ProductionLandscape #591 /
# AuthoredPCGGraph #594. Not leftover HasContinuousCoastline #623 /
# IsPointAllowedForPCG #622. LOCK leftover drafts #56-#64 plus leftover
# enum/defaults PRs including leftover Mission01 environment-readiness-
# defaults #6b9d/#204. Harbor 40/80 fail-closed in this NEW Mission01
# file (Mission08/09-only). Ban retired live-copy tokens via split
# tokens (b + Ya + kRuntimeReady).
# THIS IS leftover-safe FSkyguardMission01EnvironmentReadiness
# bAuthoredPCGStructureReady FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bAuthoredPCGStructureReady = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentReadiness
# bAuthoredPCGStructureReady FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bAuthoredPCGStructureReady = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardMission01EnvironmentReadiness bAuthoredPCGStructureReady
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bAuthoredPCGStructureReady = false;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardMission01EnvironmentReadiness
# bAuthoredPCGStructureReady. THIS IS NOT leftover mission-map-
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardMission01EnvironmentReadiness
# bAuthoredPCGStructureReady. THIS IS NOT leftover Mission05
...
# Mission01 EnvironmentReadiness bAuthoredPCGStructureReady
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bAuthoredPCGStructureReady = false;
# one-line / split-line UPROPERTY wraps;
...
# bAuthoredPCGStructureReady FIELD (declaration-only). It is
...
# Harbor 40/80 fail-closed in this NEW Mission01
...
TARGET = "bool bAuthoredPCGStructureReady = false;"
TARGET_WRONG = "bool bAuthoredPCGStructureReady = true;"
LOCKED_DECL = TARGET
...
def unlocked_neighbors() -> tuple[str, ...]:
    return (
        ...
        "TObjectPtr<UPCGGraphInterface> AuthoredPCGGraph;",
...
def field_uproperty_specifiers(section: str, field_name: str) -> str:
    pattern = re.compile(
        r"UPROPERTY\s*\((.*?)\)\s*bool\s+" + re.escape(field_name),
        re.DOTALL,
    )
    match = pattern.search(section)
    if match is None:
        raise AssertionError(
            f"UPROPERTY for {field_name} is missing from "
...
class Mission01EnvironmentReadinessAuthoredPcgStructureReadyFieldDeclContractTests(unittest.TestCase):
    def test_environment_readiness_struct_exists(self) -> None:
        ...
        self.assertIn("bAuthoredPCGStructureReady", section)
    ...
    def test_origin_main_field_uproperty_rejects_added_category(self) -> None:
        section = public_section(origin_main_header())
        specifiers = field_uproperty_specifiers(
            section,
        ...
        with self.assertRaises(AssertionError):
            if "Category" in field_uproperty_specifiers(
                invented,
                "bAuthoredPCGStructureReady",
            ):
                raise AssertionError(
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bAuthoredPCGStructureReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_authored_pcg_structure_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_authored_pcg_structure_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bAuthoredPCGStructureReady"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bAuthoredPCGStructureReady", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bAuthoredPCGStructureReady", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bAuthoredPCGStructureReady"))
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        # This Mission01 field default false is not Harbor 40/80.
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bAuthoredPCGStructureReady", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_authored_pcg_structure_ready(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
