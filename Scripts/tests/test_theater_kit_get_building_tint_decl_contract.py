# THIS IS leftover-safe ASkyguardCampaignTheaterKit GetBuildingTint.
# origin/main form: one-line and split-line UFUNCTION wraps
# UFUNCTION(BlueprintPure, Category="Skyguard|Theater")
# then `FLinearColor GetBuildingTint() const` with inline
...
# const IS present. Return type IS FLinearColor.
...
# if the return type is not FLinearColor.
...
# GetAppliedKitId / GetNamedLandmark / GetAppliedSpec
# stay uncontracted.
...
TARGET = "FLinearColor GetBuildingTint() const"
TARGET_INLINE_BODY = "{ return AppliedSpec.BuildingTint; }"
TARGET_WRONG_EQ_NONE = "FLinearColor GetBuildingTint() = NAME_None;"
TARGET_WRONG_FALSE = "FLinearColor GetBuildingTint() = false;"
TARGET_WRONG_TRUE = "FLinearColor GetBuildingTint() = true;"
TARGET_WRONG_ZERO = "FLinearColor GetBuildingTint() = 0.f;"
TARGET_WRONG_FLOAT = "float GetBuildingTint() const"
TARGET_WRONG_FNAME = "FName GetBuildingTint() const"
TARGET_WRONG_NO_CONST = "FLinearColor GetBuildingTint()"
...
GET_BUILDING_TINT_RE = re.compile(
    r"FLinearColor\s+GetBuildingTint\s*\(\s*\)\s*const"
)
GET_BUILDING_TINT_NO_CONST_RE = re.compile(
    r"FLinearColor\s+GetBuildingTint\s*\(\s*\)\s*(?!const)"
)
GET_BUILDING_TINT_WRONG_TYPE_RE = re.compile(
    r"(?:FName|float|int32|bool)\s+GetBuildingTint\b"
)
GET_BUILDING_TINT_BODY_RE = re.compile(
    r"FLinearColor\s+GetBuildingTint\s*\(\s*\)\s*const"
    r"\s*\{\s*return\s+AppliedSpec\s*\.\s*BuildingTint\s*;\s*\}"
)
INVENTED_UPROPERTY = (
    "EditAnywhere",
    "BlueprintReadWrite",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `FLinearColor GetBuildingTint() const`.
    # Do not accept missing const, FName / float return
    # types, BlueprintCallable wraps, or leftover float
    # Health. Do not accept sibling GetAppliedSpec /
    # GetNamedLandmark UFUNCTIONs.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GET_BUILDING_TINT_WRONG_TYPE_RE.search(compact):
        return False
    if GET_BUILDING_TINT_RE.search(compact) is None:
        return False
    return True
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FLinearColor\s+GetBuildingTint\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for FLinearColor GetBuildingTint is missing "
...
class TheaterKitGetBuildingTintDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetBuildingTint"), section)
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        spec = (
            ...
            f"\t{LOCKED_DECL} {TARGET_INLINE_BODY}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            actor_section(spec)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetBuildingTint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_building_tint_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertIsNotNone(GET_BUILDING_TINT_BODY_RE.search(collapsed(section)))
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        initialized = (
            ...
            "public:\n"
            f"\t{UFUNCTION_PURE_THEATER}\n"
            f"\t{TARGET_WRONG_NO_CONST} {TARGET_INLINE_BODY}\n"
            "private:\n"
            "};\n"
        )
    ...
    def test_get_building_tint_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FLinearColor GetBuildingTint"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("const"), LOCKED_DECL)
        self.assertIn("const", LOCKED_DECL)
        self.assertIn("FLinearColor", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            f"{SIBLING_ROAD_INSTANCES};\n",
            "\tFLinearColor GetBuildingTints() const;\n",
            "\tFName GetAppliedWeatherIdentity() const;\n",
            "\tFName GetAppliedKitId() const;\n",
            "\tFName GetNamedLandmark() const;\n",
            "\tconst FSkyguardTheaterKitSpec& GetAppliedSpec() const;\n",
            "\tint32 GetBuildingTint() const;\n",
            "\tbool GetBuildingTint() const;\n",
            "\tFName GetBuildingTint() const;\n",
            "\tfloat GetBuildingTint() const { return "
            + forty
            + "; }\n",
            "\tfloat GetBuildingTint() const { return "
            + eighty
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tFLinearColor\n\tGetBuildingTint() const;\n",
            "\tFLinearColor   GetBuildingTint() const;\n",
            "\tFLinearColor\tGetBuildingTint() const;\n",
            "\tFLinearColor GetBuildingTint(\n\t) const;\n",
            "\tFLinearColor GetBuildingTint()  const\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{LOCKED_DECL} {TARGET_INLINE_BODY}\n",
            f"\t{UFUNCTION_PURE_THEATER}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetBuildingTint decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "get_building_tint_decl_contract.py"
        ))
