# THIS IS leftover-safe FSkyguardProductionAudioRouting CockpitSoundMix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundMix> CockpitSoundMix;`
# (no initializer).
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Bare TSoftObjectPtr<USoundMix>,
# no initializer.
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# TSoftObjectPtr<USoundMix>, initializer added, Category= added.
...
# (exhausted isolated decls plus in-flight SourceStatus).
...
# sibling CockpitLowPassHz / CockpitExteriorAttenuation live
# in this struct and stay unclaimed.
...
# RadioSubmix / CockpitExteriorAttenuation /
# CockpitLowPassHz.
# Use identifier bounds so leftover sibling CockpitSubmix
# (TSoftObjectPtr<USoundSubmixBase>) does not false-positive
# on the Cockpit substring. This slot is CockpitSoundMix /
# TSoftObjectPtr<USoundMix>.
# Analog leftover production-audio-routing-defaults then
# isolated CockpitSoundMix field decl.
# THIS IS NOT leftover CockpitSubmix field decl.
# THIS IS NOT leftover CockpitSoundMix sibling MasterSubmix.
# THIS IS NOT leftover WeaponsSubmix / ExplosionsSubmix /
# ExteriorSubmix isolated decls.
# THIS IS NOT leftover ProductionAudioEntry OutputSubmix.
...
# ESkyguardAudioEvent.
# If a clone asserts bool / `= false` / VisibleAnywhere /
# BlueprintReadOnly / Category="Skyguard|Theater" / parse
# window FSkyguardProductionAudioEntry, retarget:
# parse FSkyguardProductionAudioRouting only, type is
# TSoftObjectPtr<USoundMix>, no `=`, BlueprintReadWrite,
# no Category.
...
TARGET = "TSoftObjectPtr<USoundMix> CockpitSoundMix;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundMix> CockpitSoundMix = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundMix> CockpitSoundMix = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundMix> CockpitSoundMix = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundMix> CockpitSoundMix = 0.f;"
TARGET_WRONG_FLOAT = "float CockpitSoundMix;"
TARGET_WRONG_FNAME = "FName CockpitSoundMix;"
TARGET_WRONG_BOOL = "bool CockpitSoundMix;"
TARGET_WRONG_INT = "int32 CockpitSoundMix;"
TARGET_WRONG_FTEXT = "FText CockpitSoundMix;"
TARGET_WRONG_PTR = "USoundMix* CockpitSoundMix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundMix> CockpitSoundMix;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundSubmixBase> CockpitSoundMix;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> CockpitSoundMix;"
TARGET_WRONG_FSTRING = "FString CockpitSoundMix;"
...
LOCKED_DECL = TARGET
UPROPERTY_EDIT_READWRITE = "UPROPERTY(EditAnywhere, BlueprintReadWrite)"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept `=` / `= false` / `= true` /
    # `= 0.f` / leftover float Health when
    # origin/main has a bare TSoftObjectPtr. Do not accept
    # sibling CockpitSubmix / RadioSubmix / WeaponsSubmix.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundMix\s*>\s+CockpitSoundMix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundMix\s*\*\s*CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundMix\s*>\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+CockpitSoundMix\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundMix\s*>\s+CockpitSoundMix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundMix> CockpitSoundMix is missing from "
...
class ProductionAudioRoutingCockpitSoundMixFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_cockpit_sound_mix_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("CockpitSoundMix = nullptr", compact_origin)
        self.assertNotIn("CockpitSoundMix = false", compact_origin)
        self.assertNotIn("CockpitSoundMix = true", compact_origin)
        self.assertNotIn("CockpitSoundMix = 0.f", compact_origin)
        self.assertNotIn("CockpitSoundMix = 160.f", compact_origin)

    def test_cockpit_sound_mix_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("TSoftObjectPtr<USoundMix>", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<USoundMix> CockpitSoundMixes;\n",
            "\tint32 CockpitSoundMix = 1;\n",
            "\tbool CockpitSoundMix;\n",
            "\tfloat CockpitSoundMix = " + forty + ";\n",
            "\tfloat CockpitSoundMix = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_routing_fields(self) -> None:
        ...
        self.assertFalse(has_identifier(FIELD_NAME, SIBLING_COCKPIT_SUBMIX))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing CockpitSoundMix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_MASTER_SUBMIX_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COCKPIT_SUBMIX_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAPONS_SUBMIX_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_EXPLOSIONS_SUBMIX_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_EXTERIOR_SUBMIX_FIELD, LOCKED_SCRIPTS)
        self.assertTrue(Path(__file__).name.endswith(
            "cockpit_sound_mix_field_decl_contract.py"
        ))
