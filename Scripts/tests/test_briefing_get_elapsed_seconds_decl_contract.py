...
# or lock the GetElapsedSeconds body.
...
GET_ELAPSED_SECONDS = "float GetElapsedSeconds() const;"
UFUNCTION_BRIEFING = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Briefing")'
)
# Leftover #56–#64 plus BriefingComponent production files.
# This lane only adds an isolated Python GetElapsedSeconds
...
# GetBriefingState (in-flight), GetMinimumWarmupSeconds,
...
# leftover CanLaunch, and leftover GetBriefingState
# stay sibling-only.
LOCKED_SCRIPTS = (
...
# .cpp GetElapsedSeconds body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline `return ElapsedSeconds` is
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionBriefingComponent::GetElapsedSeconds",
    "SkyguardMissionBriefingComponent.cpp",
)
SIBLING_TYPES = (
...
class BriefingGetElapsedSecondsDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_ELAPSED_SECONDS),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_ELAPSED_SECONDS)
        self.assertIn("GetElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_ELAPSED_SECONDS))

    def test_missing_get_elapsed_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_ELAPSED_SECONDS)
        self.assertIn("GetElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_ELAPSED_SECONDS)
        self.assertIn("GetElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_ELAPSED_SECONDS)
        self.assertNotIn("UFUNCTION", GET_ELAPSED_SECONDS)
        self.assertNotIn("Category", GET_ELAPSED_SECONDS)
        self.assertNotIn("BlueprintCallable", GET_ELAPSED_SECONDS)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_ELAPSED_SECONDS)
        self.assertIn("GetElapsedSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tfloat GetElapsedSeconds();\n"
        wrong_return_void = "\tvoid GetElapsedSeconds() const;\n"
        wrong_return_bool = "\tbool GetElapsedSeconds() const;\n"
        added_arg = "\tfloat GetElapsedSeconds(bool bReady) const;\n"
        wrong_name = "\tfloat GetMinimumWarmupSeconds() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_ELAPSED_SECONDS)
            self.assertIn("GetElapsedSeconds", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_elapsed_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_ELAPSED_SECONDS),
            GET_ELAPSED_SECONDS,
        )
        self.assertTrue(has_declaration(section, GET_ELAPSED_SECONDS))
        self.assertEqual(
            declaration_count(section, GET_ELAPSED_SECONDS),
            1,
        )
        self.assertTrue(
            GET_ELAPSED_SECONDS.startswith("float "),
            GET_ELAPSED_SECONDS,
        )
        self.assertTrue(
            GET_ELAPSED_SECONDS.endswith(";"),
            GET_ELAPSED_SECONDS,
        )
        self.assertIn("GetElapsedSeconds()", GET_ELAPSED_SECONDS)
        self.assertIn(" const", GET_ELAPSED_SECONDS)
        self.assertNotIn("INDEX_NONE", GET_ELAPSED_SECONDS)
        self.assertNotIn("{", GET_ELAPSED_SECONDS)
        self.assertNotIn("}", GET_ELAPSED_SECONDS)
        self.assertNotIn("return ", GET_ELAPSED_SECONDS)
        self.assertNotIn("return ElapsedSeconds", GET_ELAPSED_SECONDS)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_ELAPSED_SECONDS}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_ELAPSED_SECONDS))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_ELAPSED_SECONDS),
            GET_ELAPSED_SECONDS,
        )
        exact_inline = (
            "float GetElapsedSeconds() const { return ElapsedSeconds; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_ELAPSED_SECONDS))
        self.assertEqual(
            require_declaration(exact_inline, GET_ELAPSED_SECONDS),
            GET_ELAPSED_SECONDS,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_ELAPSED_SECONDS),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{GET_ELAPSED_SECONDS}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_ELAPSED_SECONDS))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_ELAPSED_SECONDS),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_ELAPSED_SECONDS)
        self.assertNotIn("return ElapsedSeconds", GET_ELAPSED_SECONDS)
        self.assertNotIn("return Other", GET_ELAPSED_SECONDS)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_ELAPSED_SECONDS}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_ELAPSED_SECONDS)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_ELAPSED_SECONDS)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_elapsed_seconds_body(self) -> None:
        locked_only = f"{GET_ELAPSED_SECONDS}\n"
        self.assertNotIn("{", GET_ELAPSED_SECONDS)
        self.assertNotIn("}", GET_ELAPSED_SECONDS)
        self.assertNotIn("return ", GET_ELAPSED_SECONDS)
        self.assertNotIn("return ElapsedSeconds", GET_ELAPSED_SECONDS)
        self.assertNotIn("return ElapsedSeconds", locked_only)
        self.assertNotIn(
            "USkyguardMissionBriefingComponent::GetElapsedSeconds",
            GET_ELAPSED_SECONDS,
        )
        ...
        self.assertNotIn("return false", GET_ELAPSED_SECONDS)
        self.assertNotIn("return true", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("ConfigureFromMission", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_set_assets_ready(self) -> None:
        ...
        for neighbor in SET_ASSETS_READY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("SetAssetsReady", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_advance_briefing(self) -> None:
        ...
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("AdvanceBriefing", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_acknowledge_and_launch(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_AND_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("AcknowledgeAndLaunch", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_can_launch(self) -> None:
        ...
        for neighbor in CAN_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("CanLaunch", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_get_briefing_state(self) -> None:
        ...
        for neighbor in GET_BRIEFING_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("GetBriefingState", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_get_minimum_warmup_seconds(self) -> None:
        ...
        for neighbor in GET_MINIMUM_WARMUP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("GetBriefingText", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_get_radio_chatter(self) -> None:
        ...
        for neighbor in GET_RADIO_CHATTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("GetRadioChatter", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_ELAPSED_SECONDS)
        self.assertNotIn("Unconfigured", GET_ELAPSED_SECONDS)
        self.assertNotIn("Warming", GET_ELAPSED_SECONDS)
        self.assertNotIn("Launched", GET_ELAPSED_SECONDS)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_ELAPSED_SECONDS,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ELAPSED_SECONDS)
        self.assertNotIn("FSkyguardBriefingCard", GET_ELAPSED_SECONDS)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ELAPSED_SECONDS)
        self.assertNotIn("FillResultCombatStats", GET_ELAPSED_SECONDS)
        self.assertNotIn("ASkyguardGunner", GET_ELAPSED_SECONDS)
        self.assertNotIn("FillAndFinalize", GET_ELAPSED_SECONDS)
        self.assertNotIn("FillAndFail", GET_ELAPSED_SECONDS)
        self.assertNotIn("ApplyHydraForClusters", GET_ELAPSED_SECONDS)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_ELAPSED_SECONDS}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_ELAPSED_SECONDS),
            GET_ELAPSED_SECONDS,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing GetElapsedSeconds contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_ELAPSED_SECONDS.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing GetElapsedSeconds contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_ELAPSED_SECONDS)

    def test_contract_is_get_elapsed_seconds_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_ELAPSED_SECONDS.startswith("float "))
        self.assertTrue(GET_ELAPSED_SECONDS.endswith(";"))
        self.assertIn(" const", GET_ELAPSED_SECONDS)
        self.assertIn(UFUNCTION_BRIEFING, section)
