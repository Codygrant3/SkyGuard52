...
# or lock the ConfigureFromMission body in the .cpp.
...
# This lane only adds an isolated Python ConfigureFromMission
# declaration contract. Stay off SetAssetsReady,
...
# gun-fire camera shake, and leftover mission-weather enum
# stay sibling-only.
LOCKED_SCRIPTS = (
...
# here. SetAssetsReady / AdvanceBriefing /
...
# .cpp ConfigureFromMission body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionBriefingComponent::ConfigureFromMission",
    "SkyguardMissionBriefingComponent.cpp",
)
SIBLING_TYPES = (
...
class BriefingConfigureFromMissionDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CONFIGURE_FROM_MISSION),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CONFIGURE_FROM_MISSION)
        self.assertIn("ConfigureFromMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CONFIGURE_FROM_MISSION))

    def test_missing_configure_from_mission_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CONFIGURE_FROM_MISSION)
        self.assertIn("ConfigureFromMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CONFIGURE_FROM_MISSION)
        self.assertIn("ConfigureFromMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", CONFIGURE_FROM_MISSION)
        self.assertNotIn("UFUNCTION", CONFIGURE_FROM_MISSION)
        self.assertNotIn("Category", CONFIGURE_FROM_MISSION)
        self.assertNotIn("BlueprintCallable", CONFIGURE_FROM_MISSION)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CONFIGURE_FROM_MISSION)
        self.assertIn("ConfigureFromMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = "\tbool ConfigureFromMission();\n"
        wrong_return = (
            "\tvoid ConfigureFromMission("
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CONFIGURE_FROM_MISSION)
            self.assertIn("ConfigureFromMission", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_configure_from_mission_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, CONFIGURE_FROM_MISSION))
        self.assertEqual(
            declaration_count(section, CONFIGURE_FROM_MISSION),
            1,
        )
        self.assertTrue(
            CONFIGURE_FROM_MISSION.startswith("bool "),
            CONFIGURE_FROM_MISSION,
        )
        self.assertTrue(
            CONFIGURE_FROM_MISSION.endswith(";"),
            CONFIGURE_FROM_MISSION,
        )
        self.assertIn(
            "USkyguardMissionDefinition* Mission",
        ...
        self.assertNotIn("INDEX_NONE", CONFIGURE_FROM_MISSION)
        self.assertNotIn("{", CONFIGURE_FROM_MISSION)
        self.assertNotIn("}", CONFIGURE_FROM_MISSION)
        self.assertNotIn("return ", CONFIGURE_FROM_MISSION)
        self.assertNotIn(" const", CONFIGURE_FROM_MISSION)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ptr = (
            "public:\n"
            "\tbool ConfigureFromMission(USkyguardMissionDefinition*\n"
            "\t\tMission);\n"
            "};\n"
        )
        wrap_arg = (
        ...
        one_line = f"{{\npublic:\n\t{CONFIGURE_FROM_MISSION}\n}}\n"
        self.assertTrue(has_declaration(one_line, CONFIGURE_FROM_MISSION))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CONFIGURE_FROM_MISSION),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", CONFIGURE_FROM_MISSION)
        self.assertNotIn("return ", CONFIGURE_FROM_MISSION)
        self.assertNotIn("return false", CONFIGURE_FROM_MISSION)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{CONFIGURE_FROM_MISSION}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", CONFIGURE_FROM_MISSION)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", CONFIGURE_FROM_MISSION)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_configure_from_mission_cpp_body(
        self,
    ) -> None:
        locked_only = f"{CONFIGURE_FROM_MISSION}\n"
        self.assertNotIn("{", CONFIGURE_FROM_MISSION)
        ...
        self.assertNotIn("return false", CONFIGURE_FROM_MISSION)
        self.assertNotIn("return true", CONFIGURE_FROM_MISSION)

    def test_contract_does_not_relock_set_assets_ready(self) -> None:
        locked_only = f"{CONFIGURE_FROM_MISSION}\n"
        for neighbor in SET_ASSETS_READY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("SetAssetsReady", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_advance_briefing(self) -> None:
        ...
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("AdvanceBriefing", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_acknowledge_and_launch(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_AND_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("AcknowledgeAndLaunch", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_can_launch(self) -> None:
        ...
        for neighbor in CAN_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("CanLaunch", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_get_briefing_state(self) -> None:
        ...
        for neighbor in GET_BRIEFING_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("GetBriefingState", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_SECONDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("GetElapsedSeconds", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_get_minimum_warmup_seconds(self) -> None:
        ...
        for neighbor in GET_MINIMUM_WARMUP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("GetMinimumWarmupSeconds", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("GetBriefingText", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_get_radio_chatter(self) -> None:
        ...
        for neighbor in GET_RADIO_CHATTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("GetRadioChatter", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, CONFIGURE_FROM_MISSION)
        self.assertNotIn("Unconfigured", CONFIGURE_FROM_MISSION)
        self.assertNotIn("Warming", CONFIGURE_FROM_MISSION)
        self.assertNotIn("Launched", CONFIGURE_FROM_MISSION)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            CONFIGURE_FROM_MISSION,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONFIGURE_FROM_MISSION)
        self.assertNotIn("FSkyguardBriefingCard", CONFIGURE_FROM_MISSION)
        self.assertNotIn("FSkyguardBriefingRadioRow", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE_FROM_MISSION)
        self.assertNotIn("FillResultCombatStats", CONFIGURE_FROM_MISSION)
        self.assertNotIn("ASkyguardGunner", CONFIGURE_FROM_MISSION)
        self.assertNotIn("FillAndFinalize", CONFIGURE_FROM_MISSION)
        self.assertNotIn("FillAndFail", CONFIGURE_FROM_MISSION)
        self.assertNotIn("ApplyHydraForClusters", CONFIGURE_FROM_MISSION)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CONFIGURE_FROM_MISSION}\n"
        self.assertEqual(
            require_declaration(locked_only, CONFIGURE_FROM_MISSION),
            CONFIGURE_FROM_MISSION,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing ConfigureFromMission contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, CONFIGURE_FROM_MISSION.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing ConfigureFromMission contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CONFIGURE_FROM_MISSION)

    def test_contract_is_configure_from_mission_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(CONFIGURE_FROM_MISSION.startswith("bool "))
        self.assertTrue(CONFIGURE_FROM_MISSION.endswith(";"))
        self.assertIn(UFUNCTION_BRIEFING, section)
