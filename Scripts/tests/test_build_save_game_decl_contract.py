...
# save-game payload, slot name, or persistence result, or
# lock the BuildSaveGame body in the .cpp. origin/main is
# one line (`USkyguardCampaignSaveGame* BuildSaveGame() const;`);
...
# This lane only adds an isolated Python BuildSaveGame
# declaration contract. Stay off ApplySaveGame /
# GetMissionRecords (sibling in-flight this wave),
# SaveCampaignToSlot / LoadCampaignFromSlot /
# DeleteCampaignSlot / RetrySaveLastDebrief /
# IsValidCampaignSlotName #306, leftover campaign-save
# empty-fail-closed, leftover campaign-roster lookup #111,
# LoadCampaignProgressAfterConfigure (#290),
# CalculateMissionScore #322 / CalculateMedalTier (sibling
...
# ApplySaveGame / GetMissionRecords are sibling in-flight this
# wave. SaveCampaignToSlot / LoadCampaignFromSlot /
# DeleteCampaignSlot / RetrySaveLastDebrief /
# IsValidCampaignSlotName #306 stay sibling-only.
...
# .cpp BuildSaveGame body / invented save-game payloads /
# slot names / persistence results stay unlocked. Do not
...
CPP_AND_INVENTED = (
    ...
    "LoadGameFromSlot",
    "DoesSaveGameExist",
    "USkyguardCampaignSubsystem::BuildSaveGame",
    "SkyguardCampaignSubsystem.cpp",
)
...
class BuildSaveGameDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, BUILD_SAVE_GAME), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, BUILD_SAVE_GAME)
        self.assertIn("BuildSaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, BUILD_SAVE_GAME))

    def test_missing_build_save_game_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, BUILD_SAVE_GAME)
        self.assertIn("BuildSaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, BUILD_SAVE_GAME)
        self.assertIn("BuildSaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_neighbor_save_helpers_do_not_satisfy(self) -> None:
        other_saves = (
            "\tbool ApplySaveGame("
            "const USkyguardCampaignSaveGame* SaveGame);\n"
            "\tbool SaveCampaignToSlot(\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_saves, BUILD_SAVE_GAME)
        self.assertIn("BuildSaveGame", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tUSkyguardCampaignSaveGame* BuildSaveGame();\n"
        wrong_return = "\tbool BuildSaveGame() const;\n"
        void_return = "\tvoid BuildSaveGame() const;\n"
        extra_slot = (
            "\tUSkyguardCampaignSaveGame* BuildSaveGame("
        ...
        non_pointer = "\tUSkyguardCampaignSaveGame BuildSaveGame() const;\n"
        for region in (
            missing_const,
            wrong_return,
            void_return,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, BUILD_SAVE_GAME)
            self.assertIn("BuildSaveGame", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_build_save_game_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, BUILD_SAVE_GAME),
            BUILD_SAVE_GAME,
        )
        self.assertTrue(has_declaration(section, BUILD_SAVE_GAME))
        self.assertEqual(declaration_count(section, BUILD_SAVE_GAME), 1)
        self.assertTrue(BUILD_SAVE_GAME.endswith("const;"), BUILD_SAVE_GAME)
        self.assertTrue(
            BUILD_SAVE_GAME.startswith("USkyguardCampaignSaveGame* "),
            BUILD_SAVE_GAME,
        )
        self.assertIn("BuildSaveGame()", BUILD_SAVE_GAME)
        self.assertNotIn("INDEX_NONE", BUILD_SAVE_GAME)
        self.assertNotIn("{", BUILD_SAVE_GAME)
        self.assertNotIn("}", BUILD_SAVE_GAME)
        self.assertNotIn("return ", BUILD_SAVE_GAME)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, BUILD_SAVE_GAME), 1)
        one_line = f"{{\npublic:\n\t{BUILD_SAVE_GAME}\n}}\n"
        self.assertTrue(has_declaration(one_line, BUILD_SAVE_GAME))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, BUILD_SAVE_GAME), section)
        self.assertEqual(
            require_declaration(section, BUILD_SAVE_GAME),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{BUILD_SAVE_GAME}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", BUILD_SAVE_GAME)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", BUILD_SAVE_GAME)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_declaration_does_not_invent_save_payloads_or_slot_behavior(
        self,
    ) -> None:
        locked_only = f"{BUILD_SAVE_GAME}\n"
        self.assertNotIn("return ", BUILD_SAVE_GAME)
        self.assertNotIn("return nullptr", BUILD_SAVE_GAME)
        self.assertNotIn("return false", BUILD_SAVE_GAME)
        self.assertNotIn("return true", BUILD_SAVE_GAME)
        self.assertNotIn("SlotName", BUILD_SAVE_GAME)
        self.assertNotIn("UserIndex", BUILD_SAVE_GAME)
        for token in SAVE_PAYLOADS_AND_SLOT_NOT_LOCKED:
            self.assertNotIn(token, BUILD_SAVE_GAME)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("NewObject", locked_only)
        self.assertNotIn("CreateSaveGameObject", locked_only)
        self.assertNotIn("DuplicateObject", locked_only)
        self.assertNotIn("MigrateCampaignSave", locked_only)
        self.assertNotIn("SaveGameToSlot", locked_only)
        self.assertNotIn("LoadGameFromSlot", locked_only)
        self.assertNotIn("DoesSaveGameExist", locked_only)
        self.assertNotIn('TEXT("Skyguard52Campaign")', BUILD_SAVE_GAME)

    def test_contract_does_not_lock_build_save_game_cpp_body(self) -> None:
        locked_only = f"{BUILD_SAVE_GAME}\n"
        self.assertNotIn("{", BUILD_SAVE_GAME)
        self.assertNotIn("}", BUILD_SAVE_GAME)
        self.assertNotIn("return ", BUILD_SAVE_GAME)
        ...
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", BUILD_SAVE_GAME)
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", locked_only)
        self.assertNotIn("NewObject", BUILD_SAVE_GAME)
        self.assertNotIn("CreateSaveGameObject", BUILD_SAVE_GAME)
        self.assertNotIn("DuplicateObject", BUILD_SAVE_GAME)
        self.assertNotIn("MigrateCampaignSave", BUILD_SAVE_GAME)
        self.assertNotIn("MissionRecords.Reset", BUILD_SAVE_GAME)
        self.assertNotIn("ClearActiveMissionRuntime", BUILD_SAVE_GAME)
        self.assertNotIn("GetTransientPackage", BUILD_SAVE_GAME)

    def test_contract_does_not_relock_get_active_mission(self) -> None:
        locked_only = f"{BUILD_SAVE_GAME}\n"
        for neighbor in GET_ACTIVE_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn(
            "USkyguardMissionDefinition* GetActiveMission() const",
            BUILD_SAVE_GAME,
        )
    ...
    def test_contract_does_not_relock_configure_can_start_or_start(self) -> None:
        ...
        for neighbor in CONFIGURE_CAN_START_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("ConfigureCampaign", BUILD_SAVE_GAME)
        self.assertNotIn("CanStartMission", BUILD_SAVE_GAME)
        self.assertNotIn("StartMission", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_is_mission_unlocked(self) -> None:
        ...
        for neighbor in IS_MISSION_UNLOCKED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("IsMissionUnlocked", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("AddObjectiveProgress", BUILD_SAVE_GAME)
        self.assertNotIn("FailObjective", BUILD_SAVE_GAME)
        self.assertNotIn("CompleteSurviveObjectiveIfIntact", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("CompleteActiveMission", BUILD_SAVE_GAME)
        self.assertNotIn("FinalizeActiveMission", BUILD_SAVE_GAME)
        self.assertNotIn("FailActiveMission", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_fill_result_or_gunner_paths(self) -> None:
        locked_only = f"{BUILD_SAVE_GAME}\n"
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("FillResultCombatStats", BUILD_SAVE_GAME)
        self.assertNotIn("ASkyguardGunner", BUILD_SAVE_GAME)
        self.assertNotIn("FillAndFinalize", BUILD_SAVE_GAME)
        self.assertNotIn("FillAndFail", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("AcknowledgeDebrief", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_debrief_neighbors(self) -> None:
        ...
        for neighbor in DEBRIEF_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("RetrySaveLastDebrief", BUILD_SAVE_GAME)
        self.assertNotIn("GetLastDebrief", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_travel_helpers(self) -> None:
        ...
        for neighbor in TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("CanTravelToNextMission", BUILD_SAVE_GAME)
        self.assertNotIn("GetNextMissionMapPackageName", BUILD_SAVE_GAME)
        self.assertNotIn("TravelToNextMission", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_runtime_getters(self) -> None:
        ...
        for neighbor in RUNTIME_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("GetObjectiveRuntime", BUILD_SAVE_GAME)
        self.assertNotIn("GetRouteRuntime", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_save_game_siblings(self) -> None:
        ...
        for neighbor in SAVE_GAME_SIBLINGS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("ApplySaveGame", BUILD_SAVE_GAME)
        self.assertNotIn("SaveCampaignToSlot", BUILD_SAVE_GAME)
        self.assertNotIn("LoadCampaignFromSlot", BUILD_SAVE_GAME)
        self.assertNotIn("DeleteCampaignSlot", BUILD_SAVE_GAME)
        self.assertNotIn("RetrySaveLastDebrief", BUILD_SAVE_GAME)
        self.assertNotIn("GetMissionRecords", BUILD_SAVE_GAME)
        self.assertNotIn("IsValidCampaignSlotName", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_leftover_campaign_save_empty(
        ...
        for token in LEFTOVER_CAMPAIGN_SAVE_EMPTY_NOT_LOCKED:
            ...
            self.assertNotIn(token, BUILD_SAVE_GAME)
            self.assertNotIn(token, section)
        self.assertNotIn("empty-file", BUILD_SAVE_GAME)
        self.assertNotIn("empty-file", locked_only)
        self.assertNotIn("EmptyFailClosed", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_score_or_medal(self) -> None:
        ...
        for neighbor in SCORE_AND_MEDAL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("CalculateMissionScore", BUILD_SAVE_GAME)
        self.assertNotIn("CalculateMedalTier", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BUILD_SAVE_GAME)
        self.assertNotIn("GetActiveMissionElapsedSeconds", BUILD_SAVE_GAME)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{BUILD_SAVE_GAME}\n"
        self.assertEqual(
            require_declaration(locked_only, BUILD_SAVE_GAME),
            BUILD_SAVE_GAME,
        )
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign BuildSaveGame contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, BUILD_SAVE_GAME.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", BUILD_SAVE_GAME)

    def test_contract_is_build_save_game_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
