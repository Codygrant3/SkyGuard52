...
# BlueprintReadOnly) then BARE `FName StepId;` (no initializer,
...
# bare FName StepId;. Fail closed if type/decl missing, renamed,
...
# Error #1232.
# Declaration-only. Accept origin/main split-line UPROPERTY and
# FName StepId;. Fail closed if type/decl
...
# BlueprintReadOnly FName StepId; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
HEADER_PATH = "Source/Skyguard52/SkyguardSortiePresentationComponent.h"
STRUCT_NAME = "FSkyguardHowToFlyRow"
...
# BlueprintReadOnly FName StepId;
...
TARGET = "FName StepId;"
TARGET_WRONG = "FName StepId = NAME_None;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Error (must be bare FName StepId;).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FName\s+StepId\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FName StepId is missing from "
...
class HowToFlyRowStepIdFieldDeclContractTests(unittest.TestCase):
    def test_how_to_fly_row_struct_exists(self) -> None:
        ...
        self.assertIn("StepId", section)
        self.assertIn("InputHint", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("StepId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_step_id_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_step_id_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName StepId"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("StepId", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("StepId", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FName StepId"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("StepId", locked_only)
    ...
    def test_this_file_bans_live_retired_tokens_case_sensitive(self) -> None:
        ...
        for banned in live:
            self.assertNotIn(
                banned,
                file_text,
                "how-to-fly-row StepId field contract contains live "
    ...
    def test_parser_does_not_land_on_anchors_or_director(self) -> None:
        ...
        match = CLASS_RE.search(header)
        self.assertIsNotNone(match, header)
        self.assertIn("FSkyguardHowToFlyRow", match.group(0))
