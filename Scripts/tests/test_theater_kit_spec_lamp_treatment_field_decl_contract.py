# THIS IS leftover-safe FSkyguardTheaterKitSpec LampTreatment.
...
# then BARE `FName LampTreatment;` (no initializer).
...
# LandmarkSet / BuildingKit / BuildingTint / LampColor /
...
TARGET = "FName LampTreatment;"
TARGET_WRONG_EQ_NONE = "FName LampTreatment = NAME_None;"
TARGET_WRONG_FALSE = "FName LampTreatment = false;"
TARGET_WRONG_TRUE = "FName LampTreatment = true;"
TARGET_WRONG_ZERO = "FName LampTreatment = 0.f;"
TARGET_WRONG_FLOAT = "float LampTreatment;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName LampTreatment;`.
    ...
    if re.search(
        r"FName\s+LampTreatment\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LampTreatment\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+LampTreatment\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName LampTreatment is missing from "
...
class TheaterKitSpecLampTreatmentFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LampTreatment"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LampTreatment"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LampTreatment", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lamp_treatment_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName LampTreatment;", compact_origin)
        self.assertNotIn("LampTreatment = NAME_None", compact_origin)
        self.assertNotIn("LampTreatment = false", compact_origin)
        self.assertNotIn("LampTreatment = true", compact_origin)
        self.assertNotIn("LampTreatment = 0.f", compact_origin)
        self.assertNotIn("LampTreatment = 160.f", compact_origin)

    def test_lamp_treatment_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName LampTreatment"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFName LampTreatments;\n",
            "\tint32 LampTreatment;\n",
            "\tbool LampTreatment;\n",
            "\tfloat LampTreatment = " + forty + ";\n",
            "\tfloat LampTreatment = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec LampTreatment field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "lamp_treatment_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
