...
# CompletedObjectiveIds). Do NOT contract MedalTier.
...
# no Category=. Accept `int32 FinalScore = 0`. Fail-closed if
...
TARGET = "int32 FinalScore = 0;"
TARGET_WRONG_FALSE = "int32 FinalScore = false;"
TARGET_WRONG_ONE = "int32 FinalScore = 1;"
TARGET_WRONG_FLOAT = "int32 FinalScore = 0.f;"
TARGET_WRONG_TRUE = "int32 FinalScore = true;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # / = 0.f / = true for FSkyguardMissionResult::FinalScore
    # (must be = 0).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+FinalScore\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 FinalScore is missing from "
...
class MissionResultFinalScoreFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertIn("FinalScore", section)
        self.assertIn("MedalTier", section)
        self.assertIn("MissionId", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("FinalScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_final_score_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_final_score_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 FinalScore"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            "\tint32 FinalScore = 1;\n",
            "\tint32 FinalScore = false;\n",
            "\tbool FinalScore = false;\n",
            "\tbool FinalScore = 0;\n",
            "\tint32 Score = 0;\n",
            "\tint32 MedalTier = 0;\n",
            "\tint32 FinalScore;\n",
            "\tint32 FinalScore = 0.0;\n",
    ...
    def test_does_not_contract_neighbors_or_director_methods(self) -> None:
        self.assertNotIn("MedalTier", LOCKED_DECL)
        self.assertNotIn("MissionId", LOCKED_DECL)
        self.assertNotIn("bMissionSucceeded", LOCKED_DECL)
        self.assertNotIn("ShotsFired", LOCKED_DECL)
        self.assertNotIn("Hits", LOCKED_DECL)
        self.assertNotIn("AircraftDamageFraction", LOCKED_DECL)
        self.assertNotIn("CompletionTimeSeconds", LOCKED_DECL)
        self.assertNotIn("CompletedObjectiveIds", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result FinalScore field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_final_score_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("Hits", locked_only)
        self.assertNotIn("AircraftDamageFraction", locked_only)
        self.assertNotIn("CompletionTimeSeconds", locked_only)
        self.assertNotIn("CompletedObjectiveIds", locked_only)
