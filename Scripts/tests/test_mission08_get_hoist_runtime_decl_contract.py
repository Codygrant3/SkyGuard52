...
# INDEX_NONE or lock GetHoistRuntime
# construction in the .cpp. This is leftover-safe
# Mission08 Hoist GetHoistRuntime METHOD on
# ASkyguardMission08IntegrationDirector. It is
# NOT leftover hoist-window-runtime-defaults
# #ec79, NOT leftover StartHoistWindow #732,
# NOT leftover AdvanceHoistWindow #734, NOT
# leftover BindRuntimeActors (retired live mount),
# NOT leftover Mission08 wave-state enum
# #68fc / #237, NOT leftover Mission08
# protected-target enum #a66a / #229, NOT leftover
# Mission08 protected-target-runtime-defaults
# #75e6 / #226, NOT leftover Mission07
# GetWaveState #719, NOT leftover
# IsCorePlayableReady, NOT leftover
# Mission06 GetWaveState #701, NOT leftover
# Mission05 GetWaveState, NOT leftover
# Mission01 / Mission03 / Mission04 GetWaveState,
...
# leftover Mission03 GetWaveState,
# leftover Mission04 GetWaveState,
# leftover Mission05 GetWaveState,
...
# GetHoistRuntime() const { return HoistRuntime; }`);
...
# Category is Skyguard|Mission08|Hoist, not
...
# Boss / Destruction / Briefing / Waves / Rescue /
...
# drafts #674–#740, leftover Apache
...
# GetWaveState #718. Do not reopen leftover
...
# Do not reopen leftover StartHoistWindow #732
# or leftover AdvanceHoistWindow #734 as decls
# in THIS test. Do not reopen open drafts #674–#740.
...
# an isolated Python GetHoistRuntime
...
class Mission08GetHoistRuntimeDeclContractTests(unittest.TestCase):
    def test_mission08_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_HOIST_RUNTIME),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_HOIST_RUNTIME)
        self.assertIn("GetHoistRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_HOIST_RUNTIME)
    ...
    def test_missing_get_hoist_runtime_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission08IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_HOIST_RUNTIME)
        self.assertIn("GetHoistRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_HOIST}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_HOIST_RUNTIME)
        self.assertIn("GetHoistRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_HOIST, section)
        ...
        self.assertNotIn("UFUNCTION", GET_HOIST_RUNTIME)
        self.assertNotIn("BlueprintPure", GET_HOIST_RUNTIME)
        self.assertNotIn("Category", GET_HOIST_RUNTIME)
        self.assertNotIn("BlueprintCallable", UFUNCTION_HOIST)
        self.assertIn("Skyguard|Mission08|Hoist", UFUNCTION_HOIST)
        self.assertNotIn("Mission01", UFUNCTION_HOIST)
        self.assertNotIn("Mission03", UFUNCTION_HOIST)
        self.assertNotIn("Mission04", UFUNCTION_HOIST)
        self.assertNotIn("Mission05", UFUNCTION_HOIST)
        self.assertNotIn("Mission06", UFUNCTION_HOIST)
        self.assertNotIn("Integration", UFUNCTION_HOIST)
        self.assertNotIn("Waves", UFUNCTION_HOIST)
        self.assertNotIn("Search", UFUNCTION_HOIST)
        self.assertNotIn("Protection", UFUNCTION_HOIST)
        self.assertNotIn("Payload", UFUNCTION_HOIST)
        self.assertNotIn("Targets", UFUNCTION_HOIST)
        self.assertNotIn("Objectives", UFUNCTION_HOIST)
        self.assertIn("BlueprintPure", UFUNCTION_HOIST)
        self.assertNotIn("Environment", UFUNCTION_HOIST)
        self.assertNotIn("Briefing", UFUNCTION_HOIST)
        self.assertNotIn("Mission02", UFUNCTION_HOIST)
        self.assertNotIn("Boss", UFUNCTION_HOIST)
        self.assertNotIn("Destruction", UFUNCTION_HOIST)
        self.assertNotIn("Apache", UFUNCTION_HOIST)
        self.assertNotIn("Mission06", UFUNCTION_HOIST)
        self.assertNotIn("Mission07", UFUNCTION_HOIST)
        self.assertNotIn("Mission10", UFUNCTION_HOIST)
        self.assertNotIn("Encounter", UFUNCTION_HOIST)
        self.assertNotIn("Safety", UFUNCTION_HOIST)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_HOIST)
            self.assertNotIn(invented, GET_HOIST_RUNTIME)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_HOIST_RUNTIME)
        self.assertIn("GetHoistRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetHoistRuntime() const;\n"
        )
        as_bool = "\tbool GetHoistRuntime() const;\n"
        missing_const = (
            "\tconst FSkyguardHoistWindowRuntime& GetHoistRuntime();\n"
        )
        with_arg = (
            "\tconst FSkyguardHoistWindowRuntime& "
            "GetHoistRuntime(int32 Window) const;\n"
        )
        renamed = (
            "\tconst FSkyguardHoistWindowRuntime& GetHoistWindow() const;\n"
        )
        short_name = "\tconst FSkyguardHoistWindowRuntime& GetHoist() const;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_HOIST_RUNTIME)
            self.assertIn("GetHoistRuntime", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_hoist_runtime_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_HOIST_RUNTIME),
            GET_HOIST_RUNTIME,
        )
        self.assertTrue(has_declaration(section, GET_HOIST_RUNTIME))
        self.assertEqual(
            declaration_count(section, GET_HOIST_RUNTIME),
            1,
        )
        self.assertTrue(
            GET_HOIST_RUNTIME.startswith("const "),
            GET_HOIST_RUNTIME,
        )
        self.assertIn("FSkyguardHoistWindowRuntime", GET_HOIST_RUNTIME)
        self.assertIn("GetHoistRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn(";", GET_HOIST_RUNTIME)
        self.assertIn("return HoistRuntime;", section)
        self.assertIn(UFUNCTION_HOIST, section)
        self.assertIn(" const", GET_HOIST_RUNTIME)
        self.assertIn("()", GET_HOIST_RUNTIME)
        self.assertNotIn("TArray<", GET_HOIST_RUNTIME)
        self.assertNotIn("TObjectPtr", GET_HOIST_RUNTIME)
        self.assertNotIn("TSoftObjectPtr", GET_HOIST_RUNTIME)
        self.assertNotIn("INDEX_NONE", GET_HOIST_RUNTIME)
        self.assertNotIn("UFUNCTION", GET_HOIST_RUNTIME)
        self.assertNotIn("{", GET_HOIST_RUNTIME)
        self.assertNotIn("}", GET_HOIST_RUNTIME)
        self.assertNotIn("return ", GET_HOIST_RUNTIME)
        self.assertIn(" const", GET_HOIST_RUNTIME)
        self.assertNotIn("Root", GET_HOIST_RUNTIME)
        self.assertNotIn("Briefing", GET_HOIST_RUNTIME)
        self.assertNotIn("AudioDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("RadioChatter", GET_HOIST_RUNTIME)
        self.assertNotIn("SortiePresentation", GET_HOIST_RUNTIME)
        self.assertNotIn("CampaignDefinition", GET_HOIST_RUNTIME)
        self.assertNotIn("MissionDefinition", GET_HOIST_RUNTIME)
        self.assertNotIn("Readiness;", GET_HOIST_RUNTIME)
        self.assertNotIn("bAutoInitialize", GET_HOIST_RUNTIME)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_HOIST_RUNTIME)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_HOIST_RUNTIME)
        self.assertNotIn("PathfinderSpawnLocation", GET_HOIST_RUNTIME)
        self.assertNotIn("GetAircraft", GET_HOIST_RUNTIME)
        self.assertNotIn("BindRuntimeActors", GET_HOIST_RUNTIME)
        self.assertNotIn("HandleDroneCityImpact", GET_HOIST_RUNTIME)
        self.assertNotIn("InitializePlayableMission", GET_HOIST_RUNTIME)
        self.assertNotIn("StartNextWave", GET_HOIST_RUNTIME)
        self.assertNotIn("AdvanceConvoyByDistance", GET_HOIST_RUNTIME)
        self.assertNotIn("NotifyConvoyDamage", GET_HOIST_RUNTIME)
        self.assertNotIn("GetReadiness", GET_HOIST_RUNTIME)
        self.assertNotIn("GetWaveState", GET_HOIST_RUNTIME)
        self.assertNotIn("GetConvoyRouteState", GET_HOIST_RUNTIME)
        self.assertNotIn("GetDayBeatKit", GET_HOIST_RUNTIME)
        self.assertNotIn("GetNightBeatKit", GET_HOIST_RUNTIME)
        self.assertNotIn("StartSearchlightWindow", GET_HOIST_RUNTIME)
        self.assertNotIn("AdvanceSearchlightTrack", GET_HOIST_RUNTIME)
        self.assertNotIn("NotifySubstationDamage", GET_HOIST_RUNTIME)
        self.assertNotIn("GetRemainingThreatsInWave", GET_HOIST_RUNTIME)
        self.assertNotIn("GetSearchlightRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("GetSubstationIntegrity", GET_HOIST_RUNTIME)
        self.assertNotIn("StartPayloadWindow", GET_HOIST_RUNTIME)
        self.assertNotIn("AdvancePayloadWindow", GET_HOIST_RUNTIME)
        self.assertNotIn("TryJamActivePayload", GET_HOIST_RUNTIME)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_HOIST_RUNTIME)
        self.assertNotIn("GetPayloadWindow", GET_HOIST_RUNTIME)
        self.assertNotIn("GetTargetRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("GetSurvivingTargetCount", GET_HOIST_RUNTIME)
        self.assertNotIn("GetDayBeatKind", GET_HOIST_RUNTIME)
        self.assertNotIn("GetDayBeatIndex", GET_HOIST_RUNTIME)
        self.assertNotIn("TickDayBeatKit", GET_HOIST_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardMission07IntegrationDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("ESkyguardMission06WaveState", GET_HOIST_RUNTIME)
        self.assertNotIn("ESkyguardMission07WaveState", GET_HOIST_RUNTIME)
        self.assertIn("FSkyguardHoistWindowRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", GET_HOIST_RUNTIME)
        self.assertNotIn("ESkyguardMission08ProtectedTarget", GET_HOIST_RUNTIME)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("FSkyguardMission08ProtectedTargetRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("ESkyguardSearchSector", GET_HOIST_RUNTIME)
        self.assertNotIn("FSkyguardSearchTrackRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("ClassifyFalseTrack", GET_HOIST_RUNTIME)
        self.assertNotIn("ConfirmRadarGhostIdentification", GET_HOIST_RUNTIME)
        self.assertNotIn("NotifyProtectedTargetDamage", GET_HOIST_RUNTIME)
        self.assertNotIn("AdvanceReinforcementTimer", GET_HOIST_RUNTIME)
        self.assertNotIn("GetSearchSector", GET_HOIST_RUNTIME)
        self.assertNotIn("GetClassifiedFalseTrackCount", GET_HOIST_RUNTIME)
        self.assertNotIn("IsHostileContactConfirmed", GET_HOIST_RUNTIME)
        self.assertNotIn("GetProtectedTarget", GET_HOIST_RUNTIME)
        self.assertNotIn("GetReinforcementTimeRemaining", GET_HOIST_RUNTIME)
        self.assertNotIn("GetNightBeatKind", GET_HOIST_RUNTIME)
        self.assertNotIn("GetNightBeatIndex", GET_HOIST_RUNTIME)
        self.assertNotIn("TickNightBeatKit", GET_HOIST_RUNTIME)
        self.assertNotIn("ESkyguardAirfieldTarget", GET_HOIST_RUNTIME)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_HOIST_RUNTIME)
        self.assertNotIn("RoadHunterSpawnLocation", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_HOIST_RUNTIME)
        self.assertNotIn("ConfigureMissionDefinition", GET_HOIST_RUNTIME)
        self.assertNotIn("NotifyObjectiveProgress", GET_HOIST_RUNTIME)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_HOIST_RUNTIME)
        self.assertNotIn("SynchronizeRuntimeState", GET_HOIST_RUNTIME)
        self.assertNotIn("NotifyThreatDestroyed", GET_HOIST_RUNTIME)
        self.assertNotIn("IsCorePlayableReady", GET_HOIST_RUNTIME)
        self.assertNotIn("GetObjectiveRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("GetGunner", GET_HOIST_RUNTIME)
        self.assertNotIn("GetPathfinder", GET_HOIST_RUNTIME)
        self.assertNotIn("GetMissionId", GET_HOIST_RUNTIME)
        self.assertNotIn("ValidateMissionContract", GET_HOIST_RUNTIME)
        self.assertNotIn("ConfigureFromMission", GET_HOIST_RUNTIME)
        self.assertNotIn("AdvanceBriefing", GET_HOIST_RUNTIME)
        self.assertNotIn("SetAssetsReady", GET_HOIST_RUNTIME)
        self.assertNotIn("AcknowledgeAndLaunch", GET_HOIST_RUNTIME)
        self.assertNotIn("CanLaunch", GET_HOIST_RUNTIME)
        self.assertNotIn("GetElapsedSeconds", GET_HOIST_RUNTIME)
        self.assertNotIn("GetBriefingState", GET_HOIST_RUNTIME)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_HOIST_RUNTIME)
        self.assertNotIn("GetBriefingText", GET_HOIST_RUNTIME)
        self.assertNotIn("GetRadioChatter", GET_HOIST_RUNTIME)
        self.assertNotIn("GetPresentation", GET_HOIST_RUNTIME)
        self.assertNotIn("GetMissionTitle", GET_HOIST_RUNTIME)
        self.assertNotIn("AcknowledgeBriefing", GET_HOIST_RUNTIME)
        self.assertNotIn("LaunchSortie", GET_HOIST_RUNTIME)
        self.assertNotIn("HullCollider", GET_HOIST_RUNTIME)
        self.assertNotIn("OpticalTracker", GET_HOIST_RUNTIME)
        self.assertNotIn("WeaponServo", GET_HOIST_RUNTIME)
        self.assertNotIn("CountermeasurePod", GET_HOIST_RUNTIME)
        self.assertNotIn("MinHeightFromOriginCm", GET_HOIST_RUNTIME)
        self.assertNotIn("RadarNode", GET_HOIST_RUNTIME)
        self.assertNotIn("ESkyguardMission02WaveState", GET_HOIST_RUNTIME)
        self.assertNotIn("HarborIndustrial", GET_HOIST_RUNTIME)
        self.assertNotIn("MaxIntegrity", GET_HOIST_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_HOIST_RUNTIME)
        self.assertNotIn("FillAndFinalize", GET_HOIST_RUNTIME)
        self.assertNotIn("FillAndFail", GET_HOIST_RUNTIME)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_HOIST_RUNTIME)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_HOIST} {GET_HOIST_RUNTIME}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_HOIST_RUNTIME}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_HOIST_RUNTIME))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_HOIST_RUNTIME),
            section,
    ...
    def test_environment_category_does_not_satisfy_hoist(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_HOIST)
        self.assertNotIn("Briefing", UFUNCTION_HOIST)
        environment = (
        ...
        self.assertNotEqual(environment, UFUNCTION_HOIST)
        self.assertNotEqual(briefing, UFUNCTION_HOIST)
        self.assertNotIn(environment, UFUNCTION_HOIST)
        self.assertNotIn(briefing, UFUNCTION_HOIST)
        leftover_m01 = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m02, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_harbor, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m03, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m04, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m05, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m06, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_waves, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m05_waves, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_integration, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m07_ready, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m08_integration, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m08_waves_callable, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m08_hoist, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m08_safety, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m08_protection, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_payload, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_targets, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_objectives, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m01, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m02, UFUNCTION_HOIST)
        self.assertNotIn(leftover_harbor, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m03, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m04, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m05, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m06, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m05_waves, UFUNCTION_HOIST)
        self.assertNotIn(leftover_integration, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m07_ready, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m08_integration, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m08_waves_callable, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m08_hoist, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m08_safety, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m08_protection, UFUNCTION_HOIST)
        self.assertNotIn("Mission01", UFUNCTION_HOIST)
        self.assertNotIn("Mission03", UFUNCTION_HOIST)
        self.assertNotIn("Mission04", UFUNCTION_HOIST)
        self.assertNotIn("Mission05", UFUNCTION_HOIST)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_HOIST_RUNTIME}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_HOIST_RUNTIME)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_HOIST_RUNTIME)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_hoist_runtime_cpp_body(self) -> None:
        locked_only = f"{GET_HOIST_RUNTIME}\n"
        self.assertNotIn("{", GET_HOIST_RUNTIME)
        self.assertNotIn("}", GET_HOIST_RUNTIME)
        self.assertNotIn("return ", GET_HOIST_RUNTIME)
        self.assertNotIn(
            "ASkyguardMission08IntegrationDirector::GetHoistRuntime",
            GET_HOIST_RUNTIME,
        )
        self.assertNotIn(
            "SkyguardMission08IntegrationDirector.cpp",
        ...
        self.assertNotIn("return false", GET_HOIST_RUNTIME)
        self.assertNotIn("return true", GET_HOIST_RUNTIME)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_HOIST_RUNTIME}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_HOIST_RUNTIME)
        self.assertIn("GetHoistRuntime", GET_HOIST_RUNTIME)
        self.assertTrue(GET_HOIST_RUNTIME.startswith("const "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_HOIST_RUNTIME)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_HOIST_RUNTIME)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_HOIST_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_HOIST_RUNTIME)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_HOIST_RUNTIME)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_HOIST_RUNTIME)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_HOIST_RUNTIME}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_HOIST_RUNTIME),
            GET_HOIST_RUNTIME,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_HOIST_RUNTIME)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_HOIST_RUNTIME)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", GET_HOIST_RUNTIME)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission08 GetHoistRuntime method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_HOIST_RUNTIME}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_HOIST_RUNTIME.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_HOIST_RUNTIME.lower(),
                "mission08 GetHoistRuntime contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_HOIST_RUNTIME)

    def test_contract_is_get_hoist_runtime_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_hoist_runtime(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_HOIST_RUNTIME)
            )
            self.assertNotEqual(GET_HOIST_RUNTIME, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetHoistRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_HOIST_RUNTIME))
        locked_only = f"{GET_HOIST_RUNTIME}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tconst FSkyguardHoistWindowRuntime& GetHoistRuntime() const { return HoistRuntime; }\n"
            "};\n"
        )
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_HOIST_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", GET_HOIST_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_HOIST)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission08_get_hoist_runtime"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_mission07_get_wave_state_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover_m07_ready_type, GET_HOIST_RUNTIME)
        self.assertIn("GetHoistRuntime", str(raised.exception))
    ...
    def test_contract_does_not_relock_leftover_mission08_wave_state_enum(
        ...
        self.assertNotIn("ESkyguardMission08WaveState", GET_HOIST_RUNTIME)
        self.assertNotIn("Integration", UFUNCTION_HOIST)
    ...
    def test_contract_does_not_relock_leftover_hoist_window(self) -> None:
        ...
        self.assertIn("Hoist", UFUNCTION_HOIST)
        self.assertNotIn("UFUNCTION", START_HOIST_WINDOW)
        self.assertNotIn("UFUNCTION", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("StartHoistWindow", GET_HOIST_RUNTIME)
        self.assertNotIn("AdvanceHoistWindow", GET_HOIST_RUNTIME)
    ...
    def test_sibling_mission08_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("StartHoistWindow", GET_HOIST_RUNTIME)
        self.assertNotIn("AdvanceHoistWindow", GET_HOIST_RUNTIME)
        self.assertNotIn("ValidateWeaponRelease", GET_HOIST_RUNTIME)
        self.assertNotIn("IsCorePlayableReady", GET_HOIST_RUNTIME)
        self.assertNotIn("SynchronizeRuntimeState", GET_HOIST_RUNTIME)
        self.assertNotIn("GetObjectiveRuntime", GET_HOIST_RUNTIME)
        self.assertNotIn("GetReadiness", GET_HOIST_RUNTIME)
