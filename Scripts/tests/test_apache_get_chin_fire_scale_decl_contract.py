...
# or lock the GetChinFireScale body in the .cpp.
...
# GetChinTurret mount getter (stay off mount getters
...
# (#420), IsRotorDown (#418), GetSensorQuality (#419),
# leftover GetChinSlewScale sibling this wave, leftover
# GetEnginePowerScale sibling this wave, and remaining
# unused neighbors (IsSystemDown, FindNearestLiveSystem,
...
# #107–#420, and leftover SortiePresentationWidgets
# stay sibling-only.
GET_CHIN_FIRE_SCALE = (
    "float GetChinFireScale() const;"
)
...
# GetChinFireScale declaration contract on
...
# sibling this wave, leftover GetEnginePowerScale
# sibling this wave, and dirty
# workspace paths.
LOCKED = {
...
# GetEnginePowerScale, and remaining
...
# Leftover GetChinSlewScale sibling this wave stays
# unlocked. Do not create or edit that file.
LEFTOVER_GET_CHIN_SLEW_SCALE_NOT_LOCKED = (
    "test_apache_get_chin_slew_scale_decl_contract.py",
    "GetChinSlewScale",
)
# Leftover GetEnginePowerScale sibling this wave stays
# unlocked. Do not create or edit that file.
...
# GetChinFireScale return type is not a leftover
# roster value lock. Leftover GetSensorQuality /
# GetChinSlewScale / GetEnginePowerScale stay
# sibling-only.
LEFTOVER_NOT_LOCKED = (
    "ESkyguardApacheSystem",
...
# .cpp GetChinFireScale body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::GetChinFireScale",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheGetChinFireScaleDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_CHIN_FIRE_SCALE), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_CHIN_FIRE_SCALE)
        self.assertIn("GetChinFireScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_CHIN_FIRE_SCALE))

    def test_missing_get_chin_fire_scale_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_CHIN_FIRE_SCALE)
        self.assertIn("GetChinFireScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_CHIN_FIRE_SCALE)
        self.assertIn("GetChinFireScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_CHIN_FIRE_SCALE), section)
        self.assertNotIn("BlueprintPure", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("UFUNCTION", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("Category", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("BlueprintCallable", GET_CHIN_FIRE_SCALE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_CHIN_FIRE_SCALE)
        self.assertIn("GetChinFireScale", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tfloat GetChinFireScale const;\n"
        wrong_return_void = "\tvoid GetChinFireScale() const;\n"
        wrong_return_bool = "\tbool GetChinFireScale() const;\n"
        wrong_return_int = "\tint32 GetChinFireScale() const;\n"
        missing_const = "\tfloat GetChinFireScale();\n"
        added_arg = "\tfloat GetChinFireScale(float Extra) const;\n"
        leftover_canopy = (
            "\tbool IsCanopyGlassCracked() const "
            "{ return bCanopyGlassCracked; }\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_CHIN_FIRE_SCALE)
            self.assertIn("GetChinFireScale", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_chin_fire_scale_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_CHIN_FIRE_SCALE),
            GET_CHIN_FIRE_SCALE,
        )
        self.assertTrue(has_declaration(section, GET_CHIN_FIRE_SCALE))
        self.assertEqual(declaration_count(section, GET_CHIN_FIRE_SCALE), 1)
        self.assertTrue(
            GET_CHIN_FIRE_SCALE.startswith("float "),
            GET_CHIN_FIRE_SCALE,
        )
        self.assertTrue(GET_CHIN_FIRE_SCALE.endswith(";"), GET_CHIN_FIRE_SCALE)
        self.assertIn("GetChinFireScale()", GET_CHIN_FIRE_SCALE)
        self.assertIn(" const", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("INDEX_NONE", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("{", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("}", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("return ", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IsCanopyGlassCracked", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("AreEnginesDown", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IsChinTurretDown", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IsRotorDown", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IsSystemDown", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetSensorQuality", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetChinSlewScale", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetEnginePowerScale", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetDamageFraction", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("ApplyDamage", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetForwardSpeed", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("FaceWorldLocation", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("ApplySystemHit", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("ApplyHit", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IsSensorLive", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IsThermalAvailable", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IsSensorViewActive", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetChinMuzzleLocation", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetChinTurret", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetRotorPowerScale", GET_CHIN_FIRE_SCALE)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tfloat\n"
            "\tGetChinFireScale() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tfloat GetChinFireScale(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, GET_CHIN_FIRE_SCALE),
                section,
            )
            self.assertEqual(
                require_declaration(section, GET_CHIN_FIRE_SCALE),
            ...
            self.assertEqual(
                declaration_count(section, GET_CHIN_FIRE_SCALE),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_CHIN_FIRE_SCALE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_CHIN_FIRE_SCALE))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_CHIN_FIRE_SCALE), section)
        self.assertEqual(
            require_declaration(section, GET_CHIN_FIRE_SCALE),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_CHIN_FIRE_SCALE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_CHIN_FIRE_SCALE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_chin_fire_scale_cpp_body(self) -> None:
        locked_only = f"{GET_CHIN_FIRE_SCALE}\n"
        self.assertNotIn("{", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("}", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("return ", GET_CHIN_FIRE_SCALE)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("return true", GET_CHIN_FIRE_SCALE)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{GET_CHIN_FIRE_SCALE}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetGunnerMount", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetEyeMount", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetWeaponMount", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetChinTurret", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetPilotMount", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetSensorTurret", GET_CHIN_FIRE_SCALE)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_CHIN_FIRE_SCALE}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetChinMuzzleLocation", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_aim_chin_turret(self) -> None:
        ...
        for neighbor in AIM_CHIN_TURRET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("AimChinTurret", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_set_rotor_power(self) -> None:
        ...
        for neighbor in SET_ROTOR_POWER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("SetRotorPower", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_issue_pilot_command(self) -> None:
        ...
        for neighbor in ISSUE_PILOT_COMMAND_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("IssuePilotCommand", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_get_pilot_helpers(self) -> None:
        ...
        for neighbor in GET_PILOT_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetPilotCommand", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("GetPilotConfirmationsIssued", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("SetOrbitFocus", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("FaceWorldLocation", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("SetSensorView", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("SetFirstPersonInterior", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("ESkyguardApacheSystem", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_leftover_direct_flight_input_sibling(
        ...
        self.assertNotIn("SetDirectFlightInput", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CHIN_FIRE_SCALE)
        self.assertNotIn("Pursuit", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("OrbitLeft", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("AttackRun", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("FaceTarget", GET_CHIN_FIRE_SCALE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_CHIN_FIRE_SCALE)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("USkyguardBriefingWidget", GET_CHIN_FIRE_SCALE)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            GET_CHIN_FIRE_SCALE,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_CHIN_FIRE_SCALE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("ESkyguardMissionSkylineStyle", GET_CHIN_FIRE_SCALE)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_CHIN_FIRE_SCALE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_CHIN_FIRE_SCALE),
            GET_CHIN_FIRE_SCALE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GetChinFireScale contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_CHIN_FIRE_SCALE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache GetChinFireScale contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_CHIN_FIRE_SCALE)

    def test_contract_is_get_chin_fire_scale_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", GET_CHIN_FIRE_SCALE)
        self.assertNotIn("{", GET_CHIN_FIRE_SCALE)
        self.assertTrue(GET_CHIN_FIRE_SCALE.startswith("float "))
        self.assertTrue(GET_CHIN_FIRE_SCALE.endswith(";"))
        self.assertIn(" const", GET_CHIN_FIRE_SCALE)
        self.assertIn(UFUNCTION_APACHE, section)
