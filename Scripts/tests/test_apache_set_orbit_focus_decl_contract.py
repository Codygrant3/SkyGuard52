...
# or lock the SetOrbitFocus body in the .cpp.
...
# SetOrbitFocus declaration contract on
...
# sibling, leftover FaceWorldLocation sibling,
# leftover SetSensorView sibling, and dirty
# workspace paths.
LOCKED = {
...
# .cpp SetOrbitFocus body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::SetOrbitFocus",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheSetOrbitFocusDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_ORBIT_FOCUS), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_ORBIT_FOCUS)
        self.assertIn("SetOrbitFocus", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SET_ORBIT_FOCUS))

    def test_missing_set_orbit_focus_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_ORBIT_FOCUS)
        self.assertIn("SetOrbitFocus", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_ORBIT_FOCUS)
        self.assertIn("SetOrbitFocus", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_ORBIT_FOCUS), section)
        self.assertNotIn("BlueprintPure", SET_ORBIT_FOCUS)
        self.assertNotIn("UFUNCTION", SET_ORBIT_FOCUS)
        self.assertNotIn("Category", SET_ORBIT_FOCUS)
        self.assertNotIn("BlueprintCallable", SET_ORBIT_FOCUS)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_ORBIT_FOCUS)
        self.assertIn("SetOrbitFocus", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetOrbitFocus;\n"
        wrong_return_bool = (
            "\tbool SetOrbitFocus(const FVector& WorldLocation);\n"
        )
        wrong_return_int = (
            "\tint32 SetOrbitFocus(const FVector& WorldLocation);\n"
        )
        added_const = (
            "\tvoid SetOrbitFocus(const FVector& WorldLocation) const;\n"
        )
        wrong_type = "\tvoid SetOrbitFocus(int WorldLocation);\n"
        wrong_rotator = (
            "\tvoid SetOrbitFocus(const FRotator& WorldLocation);\n"
        )
        wrong_value = "\tvoid SetOrbitFocus(FVector WorldLocation);\n"
        added_arg = (
            "\tvoid SetOrbitFocus("
            "const FVector& WorldLocation, float Extra);\n"
        )
        no_args = "\tvoid SetOrbitFocus();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_ORBIT_FOCUS)
            self.assertIn("SetOrbitFocus", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_orbit_focus_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SET_ORBIT_FOCUS),
            SET_ORBIT_FOCUS,
        )
        self.assertTrue(has_declaration(section, SET_ORBIT_FOCUS))
        self.assertEqual(declaration_count(section, SET_ORBIT_FOCUS), 1)
        self.assertTrue(
            SET_ORBIT_FOCUS.startswith("void "),
            SET_ORBIT_FOCUS,
        )
        self.assertTrue(SET_ORBIT_FOCUS.endswith(";"), SET_ORBIT_FOCUS)
        self.assertIn("SetOrbitFocus(", SET_ORBIT_FOCUS)
        self.assertIn("const FVector& WorldLocation", SET_ORBIT_FOCUS)
        self.assertNotIn("INDEX_NONE", SET_ORBIT_FOCUS)
        self.assertNotIn("{", SET_ORBIT_FOCUS)
        self.assertNotIn("}", SET_ORBIT_FOCUS)
        self.assertNotIn("return ", SET_ORBIT_FOCUS)
        self.assertNotIn(" const", SET_ORBIT_FOCUS)
        self.assertNotIn("SetRotorPower", SET_ORBIT_FOCUS)
        self.assertNotIn("AimChinTurret", SET_ORBIT_FOCUS)
        self.assertNotIn("GetChinMuzzleLocation", SET_ORBIT_FOCUS)
        self.assertNotIn("IssuePilotCommand", SET_ORBIT_FOCUS)
        self.assertNotIn("GetPilotCommand", SET_ORBIT_FOCUS)
        self.assertNotIn("GetPilotConfirmationsIssued", SET_ORBIT_FOCUS)
        self.assertNotIn("FaceWorldLocation", SET_ORBIT_FOCUS)
        self.assertNotIn("SetSensorView", SET_ORBIT_FOCUS)
        self.assertNotIn("GetChinTurret", SET_ORBIT_FOCUS)
        self.assertNotIn("GetRotorPowerScale", SET_ORBIT_FOCUS)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetOrbitFocus(const FVector& WorldLocation);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, SET_ORBIT_FOCUS),
                section,
            )
            self.assertEqual(
                require_declaration(section, SET_ORBIT_FOCUS),
            ...
            self.assertEqual(
                declaration_count(section, SET_ORBIT_FOCUS),
                1,
            )
        one_line = f"{{\npublic:\n\t{SET_ORBIT_FOCUS}\n}}\n"
        self.assertTrue(has_declaration(one_line, SET_ORBIT_FOCUS))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SET_ORBIT_FOCUS), section)
        self.assertEqual(
            require_declaration(section, SET_ORBIT_FOCUS),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_ORBIT_FOCUS}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_ORBIT_FOCUS)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_ORBIT_FOCUS)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_orbit_focus_cpp_body(self) -> None:
        locked_only = f"{SET_ORBIT_FOCUS}\n"
        self.assertNotIn("{", SET_ORBIT_FOCUS)
        self.assertNotIn("}", SET_ORBIT_FOCUS)
        self.assertNotIn("return ", SET_ORBIT_FOCUS)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", SET_ORBIT_FOCUS)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", SET_ORBIT_FOCUS)
        self.assertNotIn("return true", SET_ORBIT_FOCUS)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{SET_ORBIT_FOCUS}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_ORBIT_FOCUS)
        self.assertNotIn("GetGunnerMount", SET_ORBIT_FOCUS)
        self.assertNotIn("GetEyeMount", SET_ORBIT_FOCUS)
        self.assertNotIn("GetWeaponMount", SET_ORBIT_FOCUS)
        self.assertNotIn("GetChinTurret", SET_ORBIT_FOCUS)
        self.assertNotIn("GetPilotMount", SET_ORBIT_FOCUS)
        self.assertNotIn("GetSensorTurret", SET_ORBIT_FOCUS)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{SET_ORBIT_FOCUS}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ORBIT_FOCUS)
        self.assertNotIn("GetChinMuzzleLocation", SET_ORBIT_FOCUS)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, SET_ORBIT_FOCUS)
        self.assertNotIn("SetSensorView", SET_ORBIT_FOCUS)
        self.assertNotIn("SetFirstPersonInterior", SET_ORBIT_FOCUS)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ORBIT_FOCUS)
        self.assertNotIn("ApplyDamage", SET_ORBIT_FOCUS)
        self.assertNotIn("GetDamageFraction", SET_ORBIT_FOCUS)
        self.assertNotIn("ApplySystemHit", SET_ORBIT_FOCUS)
        self.assertNotIn("ApplyHit", SET_ORBIT_FOCUS)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_ORBIT_FOCUS)
        self.assertNotIn("IsSensorLive", SET_ORBIT_FOCUS)
        self.assertNotIn("IsThermalAvailable", SET_ORBIT_FOCUS)
        self.assertNotIn("IsSensorViewActive", SET_ORBIT_FOCUS)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_ORBIT_FOCUS)
        self.assertNotIn("ESkyguardApacheSystem", SET_ORBIT_FOCUS)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_ORBIT_FOCUS)
        self.assertNotIn("Pursuit", SET_ORBIT_FOCUS)
        self.assertNotIn("OrbitLeft", SET_ORBIT_FOCUS)
        self.assertNotIn("AttackRun", SET_ORBIT_FOCUS)
        self.assertNotIn("FaceTarget", SET_ORBIT_FOCUS)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_ORBIT_FOCUS)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", SET_ORBIT_FOCUS)
        self.assertNotIn("USkyguardBriefingWidget", SET_ORBIT_FOCUS)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            SET_ORBIT_FOCUS,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_ORBIT_FOCUS)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_ORBIT_FOCUS)
        self.assertNotIn("ESkyguardMissionSkylineStyle", SET_ORBIT_FOCUS)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_ORBIT_FOCUS}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_ORBIT_FOCUS),
            SET_ORBIT_FOCUS,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SetOrbitFocus contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, SET_ORBIT_FOCUS.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache SetOrbitFocus contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_ORBIT_FOCUS)

    def test_contract_is_set_orbit_focus_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", SET_ORBIT_FOCUS)
        self.assertNotIn("{", SET_ORBIT_FOCUS)
        self.assertTrue(SET_ORBIT_FOCUS.startswith("void "))
        self.assertTrue(SET_ORBIT_FOCUS.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
