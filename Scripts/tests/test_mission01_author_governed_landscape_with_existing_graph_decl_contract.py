...
# then split-line static struct-return
# static FSkyguardMission01EnvironmentAuthoringResult
# AuthorGovernedLandscapeWithExistingGraph(
# Director / HeightmapSourcePath / GraphPackagePath).
# THIS IS leftover-safe unique analog library UFUNCTION wrapping
# unused Category path Skyguard|Mission01|Environment|Authoring.
# Leftover unique AuthoringResult FIELD drafts wrap struct
# members with NO Category and explicitly do NOT parse this
# library. Identifier AuthorGovernedLandscapeWithExistingGraph
# is NOT leftover analog AuthorGovernedLandscapeAndGraph and
# NOT leftover unique SampleLandscapeHeight / SampleLandscapeFootprint
# and NOT sibling AuditGovernedLandscapeAndGraph and NOT
# struct field bSuccess / Landscape / Graph.
# Category IS `Skyguard|Mission01|Environment|Authoring`.
# BlueprintCallable IS present. const IS absent. static IS
# present. Return type IS FSkyguardMission01EnvironmentAuthoringResult.
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# stay BlueprintPure, if const is present, if static is
# missing, if the return type is not
# FSkyguardMission01EnvironmentAuthoringResult, or if Category
# is Skyguard|Apache without |Damage / Skyguard|Combat
# without |Sortie/|Feedback/|Safety / Skyguard|Theater /
...
# Category="Skyguard|Mission01|Environment" without |Authoring /
# Category="Skyguard|Mission01|Environment|Grounding" /
# |Validation / |Landscape / |PCG / |Layout / |Materials /
# |Visibility.
# Accept one-line and split-line UFUNCTION wraps.
# Parse CLASS `USkyguardMission01EnvironmentAuthoringLibrary`
# public section ONLY after
# `class USkyguardMission01EnvironmentAuthoringLibrary` /
# GENERATED_BODY. Isolate the matching UFUNCTION. Do NOT parse
# struct `FSkyguardMission01EnvironmentAuthoringResult`.
# Do NOT parse VisibleAudit / CaptureConfiguration /
# MaterialCompilation structs. Do NOT parse Validation
# UFUNCTIONs. Do NOT parse `ASkyguardMission01EnvironmentDirector`.
# Do NOT claim siblings AuthorGovernedLandscapeAndGraph /
# AuditGovernedLandscapeAndGraph.
# Clone leftover unique BlueprintCallable UFUNCTION
# EnsureDefaultEntries Category Skyguard|Audio|Production
# — RETARGET class -> USkyguardMission01EnvironmentAuthoringLibrary,
# wrap Category Production -> Skyguard|Mission01|Environment|Authoring,
# decl -> AuthorGovernedLandscapeWithExistingGraph static
# struct-return (not void EnsureDefaultEntries).
# Clone leftover unique BlueprintPure GetSortieHits for
# UFUNCTION contract shape — RETARGET Category Combat|Sortie
# -> Skyguard|Mission01|Environment|Authoring. KEEP
# BlueprintCallable (NOT BlueprintPure). NOT leftover
# int32 GetSortieHits() const;.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is AuthorGovernedLandscapeWithExistingGraph(
# static struct-return. NOT `FName WeatherIdentity;` and NOT
# int32 GetSortieHits() const; and NOT
# void ResetSortieCombatStats(); and NOT
# bool GetInvertVerticalLook() const; and NOT
# float GetAppliedLookSensitivity() const; and NOT
# bool bSuccess = false; and NOT
# AuthorGovernedLandscapeAndGraph /
# AuditGovernedLandscapeAndGraph / SampleLandscapeHeight /
# SampleLandscapeFootprint / GetActiveRHIAndFeatureLevel /
# AuditLandscapeVisibleReadiness /
# PrepareGovernedLandscapeForVisibleValidation /
# FinishLandscapeMaterialCompilation and NOT
# void EnsureDefaultEntries(); and NOT 160.f /
# FireRate = 12.0f / RecoilPitch = 0.92f /
# RouteLengthCm = 45000.f.
# Fail-closed if this test still asserts those leftover
# analog decls AS THE LOCKED DECL.
...
LEFTOVER_AUDIO_PRODUCTION_BANK_CLASS = "USkyguardAudioProductionBank"
LEFTOVER_GUNNER_CLASS = "ASkyguardGunner"
STRUCT_NAME = "FSkyguardMission01EnvironmentAuthoringResult"
DIRECTOR_CLASS = "ASkyguardMission01EnvironmentDirector"
METHOD_NAME = "AuthorGovernedLandscapeWithExistingGraph"
...
TARGET_WRONG_BARE = (
    ...
    "ASkyguardMission01EnvironmentDirector* Director, "
    "const FString& HeightmapSourcePath, "
    "const FString& GraphPackagePath) const;"
)
...
TARGET_WRONG_FLOAT = (
    "float AuthorGovernedLandscapeWithExistingGraph("
    "ASkyguardMission01EnvironmentDirector* Director, "
    "const FString& HeightmapSourcePath, "
    "const FString& GraphPackagePath);"
)
...
TARGET_WRONG_RENAME = (
    "static FSkyguardMission01EnvironmentAuthoringResult "
    "AuthorGovernedLandscapeWithExistingGraphs("
    "ASkyguardMission01EnvironmentDirector* Director, "
    "const FString& HeightmapSourcePath, "
    "const FString& GraphPackagePath);"
)
...
TARGET_WRONG_SAMPLE_HEIGHT = "float SampleLandscapeHeight();"
TARGET_WRONG_SAMPLE_FOOTPRINT = "float SampleLandscapeFootprint();"
TARGET_WRONG_ACTIVE_RHI = "static FString GetActiveRHIAndFeatureLevel();"
TARGET_WRONG_VISIBLE_AUDIT = (
...
TARGET_WRONG_INVERT = "bool GetInvertVerticalLook() const;"
TARGET_WRONG_SENSITIVITY = "float GetAppliedLookSensitivity() const;"
...
TARGET_WRONG_TRUE = "bool AuthorGovernedLandscapeWithExistingGraph = true;"
TARGET_WRONG_ZERO = "bool AuthorGovernedLandscapeWithExistingGraph = 0.f;"
...
UFUNCTION_PURE = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Mission01|Environment|Authoring")'
)
...
UFUNCTION_GROUNDING = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Mission01|Environment|Grounding")'
)
...
UFUNCTION_LAYOUT = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Mission01|Environment|Layout")'
)
UFUNCTION_MATERIALS = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Mission01|Environment|Materials")'
)
UFUNCTION_VISIBILITY = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Mission01|Environment|Visibility")'
)
UFUNCTION_SETTINGS = (
...
STOP_BEFORE_EVENT_DEFINITIONS = "AuditGovernedLandscapeAndGraph"
SIBLING_INITIALIZE_REQUIRED = "AuthorGovernedLandscapeAndGraph"
SIBLING_AND_GRAPH = "AuthorGovernedLandscapeAndGraph"
SIBLING_AUDIT_AND_GRAPH = "AuditGovernedLandscapeAndGraph"
SIBLING_SAMPLE_HEIGHT = "SampleLandscapeHeight"
SIBLING_SAMPLE_FOOTPRINT = "SampleLandscapeFootprint"
SIBLING_ACTIVE_RHI = "GetActiveRHIAndFeatureLevel"
SIBLING_VISIBLE_AUDIT = "AuditLandscapeVisibleReadiness"
SIBLING_PREPARE_VISIBLE = "PrepareGovernedLandscapeForVisibleValidation"
SIBLING_FINISH_COMPILE = "FinishLandscapeMaterialCompilation"
SIBLING_AUDIT_COMPILE = "AuditLandscapeMaterialCompilation"
SIBLING_BEGIN_DIAGNOSTIC = "BeginTransientLandscapeDiagnosticMaterialDeferred"
SIBLING_CONFIGURE_CAPTURE = "ConfigureLandscapeSceneCaptureDiagnostic"
SIBLING_SET_TRANSIENT = "SetTransientLandscapeDiagnosticMaterial"
SIBLING_SET_TRANSIENT_SYNC = "SetTransientLandscapeDiagnosticMaterialSynchronized"
...
STOP_BEFORE_SKYLINE_STYLE = "GetActiveRHIAndFeatureLevel"
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        LEFTOVER_SAMPLE_HEIGHT,
        LEFTOVER_SAMPLE_FOOTPRINT,
        f"{prefix}test_mission01_environment_authoring_result_success"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_landscape"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_graph"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_error"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_visible_audit"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_graph_node_count"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_graph_edge_count"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_landscape_component_count"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_graph_node_setting_classes"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_landscape_guid_valid"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_landscape_transform_exact"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_graph_contract_valid"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_authored_structure_ready"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_licensed_mesh_slots_empty"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_generation_locked"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_generated_pcg_component_count"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_generated_pcg_instance_count"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_environment_authoring_result_route_and_beach_generated_instances_zero"
        "_field_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bstatic\s+FSkyguardMission01EnvironmentAuthoringResult\s+"
    r"AuthorGovernedLandscapeWithExistingGraph\s*\("
    r"[^;]*?HeightmapSourcePath[^;]*?GraphPackagePath\s*\)\s*(?:;|\{)"
)
LEFTOVER_CLONE_RESOLVED_RE = re.compile(
    r"\bvoid\s+EnsureDefaultEntries\b"
)
LEFTOVER_SIBLING_INIT_RE = re.compile(
    r"\bAuthorGovernedLandscapeAndGraph\b"
)
LEFTOVER_SIBLING_AUDIT_RE = re.compile(
...
LEFTOVER_GET_SORTIE_HITS_RE = re.compile(
    r"\bint32\s+GetSortieHits\s*\(\s*\)\s*const\b"
)
LEFTOVER_SUCCESS_FIELD_RE = re.compile(
    r"\bbool\s+bSuccess\s*=\s*false\s*;"
)
LEFTOVER_ROUTE_LENGTH_RE = re.compile(
    r"RouteLengthCm\s*=\s*45000\.f\b"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_AND_GRAPH,
        SIBLING_AUDIT_AND_GRAPH,
        SIBLING_SAMPLE_HEIGHT,
        SIBLING_SAMPLE_FOOTPRINT,
        SIBLING_ACTIVE_RHI,
        SIBLING_VISIBLE_AUDIT,
        SIBLING_PREPARE_VISIBLE,
        SIBLING_FINISH_COMPILE,
        SIBLING_AUDIT_COMPILE,
        SIBLING_BEGIN_DIAGNOSTIC,
        SIBLING_CONFIGURE_CAPTURE,
        SIBLING_SET_TRANSIENT,
        SIBLING_SET_TRANSIENT_SYNC,
...
def leftover_analog_field_in_region(compact: str) -> bool:
    checks = (
        ...
        LEFTOVER_GET_SORTIE_HITS_RE,
        LEFTOVER_SUCCESS_FIELD_RE,
        LEFTOVER_ROUTE_LENGTH_RE,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored BlueprintCallable static
    # struct-return AuthorGovernedLandscapeWithExistingGraph(.
    # Accept origin/main split-line trailing-semicolon form
    # and an inline body without locking the body. Do not
    # accept a trailing const qualifier, a void/bool/float/
    # int32 rename, leftover analog AuthorGovernedLandscapeAndGraph /
    # AuditGovernedLandscapeAndGraph / SampleLandscapeHeight /
    # SampleLandscapeFootprint / GetSortieHits /
    # leftover FName WeatherIdentity, leftover bool bSuccess,
    # leftover FireRate / RecoilPitch, leftover RouteLengthCm,
    # leftover Boom, or Harbor 40 / 80.
    del declaration
    compact = collapsed(region)
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"AuthorGovernedLandscapeWithExistingGraph\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+AuthorGovernedLandscapeWithExistingGraph\b", compact):
        return False
    if re.search(r"\bbool\s+AuthorGovernedLandscapeWithExistingGraph\b", compact):
        return False
    if re.search(r"\bfloat\s+AuthorGovernedLandscapeWithExistingGraph\b", compact):
        return False
    if re.search(r"\bint32\s+AuthorGovernedLandscapeWithExistingGraph\b", compact):
        return False
...
def field_identifier(field_text: str) -> str:
    ...
    match = re.search(
        r"(?:static\s+)?"
        r"(?:FSkyguard\w+|void|float|int32|bool|uint8|FName|FString|"
        r"FVector|FRotator|FText|ESkyguard\w+|"
...
def spec_section(header: str) -> str:
    ...
    if has_identifier(window, STOP_BEFORE_EVENT_DEFINITIONS):
        raise AssertionError(
            f"{CLASS_NAME} parse window includes "
            f"{STOP_BEFORE_EVENT_DEFINITIONS}"
        )
    if "struct FSkyguardMission01EnvironmentAuthoringResult" in window:
        raise AssertionError(
            f"{CLASS_NAME} parse window includes leftover unique "
            "AuthoringResult struct"
        )
    if "struct FSkyguardLandscapeVisibleAudit" in window:
        raise AssertionError(
            f"{CLASS_NAME} parse window includes VisibleAudit struct"
        )
    if "struct FSkyguardLandscapeCaptureConfigurationResult" in window:
        raise AssertionError(
            f"{CLASS_NAME} parse window includes CaptureConfiguration struct"
        )
    if "struct FSkyguardLandscapeMaterialCompilationResult" in window:
        raise AssertionError(
            f"{CLASS_NAME} parse window includes MaterialCompilation struct"
        )
    if "class ASkyguardMission01EnvironmentDirector" in window:
        raise AssertionError(
            f"{CLASS_NAME} parse window includes {DIRECTOR_CLASS}"
        )
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static\s+FSkyguardMission01EnvironmentAuthoringResult\s+"
            r"AuthorGovernedLandscapeWithExistingGraph\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for AuthorGovernedLandscapeWithExistingGraph "
...
class Mission01AuthorGovernedLandscapeWithExistingGraphDeclContractTests(
    unittest.TestCase
):
    def test_authoring_library_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_AUDIO_PRODUCTION_BANK_CLASS)
        self.assertNotEqual(CLASS_NAME, LEFTOVER_GUNNER_CLASS)
        self.assertNotEqual(CLASS_NAME, STRUCT_NAME)
        self.assertNotEqual(CLASS_NAME, DIRECTOR_CLASS)
        body = class_body(header)
        ...
        self.assertTrue(
            has_identifier(section, METHOD_NAME),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_AND_GRAPH), body)
        self.assertFalse(has_identifier(section, SIBLING_AND_GRAPH), section)
        self.assertTrue(has_identifier(body, SIBLING_AUDIT_AND_GRAPH), body)
        self.assertFalse(has_identifier(section, SIBLING_AUDIT_AND_GRAPH), section)
        self.assertTrue(has_identifier(body, SIBLING_VISIBLE_AUDIT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_VISIBLE_AUDIT),
            section,
        )
        self.assertTrue(has_identifier(body, STOP_BEFORE_SKYLINE_STYLE), body)
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE), section)
        self.assertTrue(has_identifier(body, SIBLING_FINISH_COMPILE), body)
        self.assertFalse(has_identifier(section, SIBLING_FINISH_COMPILE), section)
    ...
    def test_missing_author_governed_landscape_with_existing_graph_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission01|Environment|Authoring"', section)
        ...
        self.assertIn('Category="Skyguard|Mission01|Environment|Authoring"', specifiers)
        ...
        self.assertTrue("|Authoring" in specifiers)
        self.assertNotEqual(
            specifiers,
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "AuthorGovernedLandscapeWithExistingGraph()const",
    ...
    def test_author_governed_landscape_with_existing_graph_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertIsNone(
            re.search(r"\)\s*const\b", LOCKED_DECL)
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_INVERT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SENSITIVITY)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIBLING_AUDIT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SAMPLE_HEIGHT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SAMPLE_FOOTPRINT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ACTIVE_RHI)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VISIBLE_AUDIT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PREPARE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FINISH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VOID)
        self.assertNotIn(SIBLING_AND_GRAPH, LOCKED_DECL)
        self.assertNotIn(SIBLING_AUDIT_AND_GRAPH, LOCKED_DECL)
        self.assertNotIn(SIBLING_SAMPLE_HEIGHT, LOCKED_DECL)
        self.assertNotIn(SIBLING_SAMPLE_FOOTPRINT, LOCKED_DECL)
        self.assertNotIn(SIBLING_ACTIVE_RHI, LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tstatic FSkyguardMission01EnvironmentAuthoringResult "
            "AuthorGovernedLandscapeWithExistingGraphs();\n",
            "\tint32 AuthorGovernedLandscapeWithExistingGraph = 1;\n",
            "\tFName AuthorGovernedLandscapeWithExistingGraph = NAME_None;\n",
            "\tvoid AuthorGovernedLandscapeWithExistingGraph() = "
            + forty
            + ";\n",
            ...
            f"\t{TARGET_WRONG_INVERT}\n",
            f"\t{TARGET_WRONG_SENSITIVITY}\n",
            f"\t{TARGET_WRONG_SUCCESS}\n",
            f"\t{TARGET_WRONG_ROUTE_LENGTH}\n",
            f"\t{TARGET_WRONG_SIBLING_INIT}\n",
            f"\t{TARGET_WRONG_SIBLING_AUDIT}\n",
            f"\t{TARGET_WRONG_SAMPLE_HEIGHT}\n",
            f"\t{TARGET_WRONG_SAMPLE_FOOTPRINT}\n",
            f"\t{TARGET_WRONG_ACTIVE_RHI}\n",
            f"\t{TARGET_WRONG_VISIBLE_AUDIT}\n",
            f"\t{TARGET_WRONG_PREPARE}\n",
            f"\t{TARGET_WRONG_FINISH}\n",
        )
    ...
    def test_missing_category_or_blueprint_callable_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission01|Environment|Authoring"', origin)
        ...
        leftover_landscape = (
            ...
            "{\n"
            "public:\n"
            f"\t{UFUNCTION_LANDSCAPE}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        landscape_specs = attached_ufunction_specifiers(
            spec_section(leftover_landscape)
        )
        self.assertIn(
        ...
        pcg_specs = attached_ufunction_specifiers(
            spec_section(leftover_pcg)
        )
        self.assertIn(
        ...
        layout_specs = attached_ufunction_specifiers(
            spec_section(leftover_layout)
        )
        self.assertIn(
        ...
        leftover_materials = (
            ...
            "{\n"
            "public:\n"
            f"\t{UFUNCTION_MATERIALS}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        materials_specs = attached_ufunction_specifiers(
            spec_section(leftover_materials)
        )
        self.assertIn(
    ...
    def test_does_not_claim_sibling_initialize_required_entries_or_neighbors(
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_AND_GRAPH))
        self.assertFalse(has_identifier(section, SIBLING_AND_GRAPH))
        self.assertTrue(has_identifier(leaked, SIBLING_AUDIT_AND_GRAPH))
        self.assertFalse(has_identifier(section, SIBLING_AUDIT_AND_GRAPH))
        self.assertTrue(has_identifier(leaked, SIBLING_VISIBLE_AUDIT))
        self.assertFalse(has_identifier(section, SIBLING_VISIBLE_AUDIT))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, METHOD_NAME)
        )
        self.assertTrue(
            has_identifier(section, METHOD_NAME)
        )
        ...
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_FINISH_COMPILE))
        self.assertFalse(has_identifier(section, SIBLING_FINISH_COMPILE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio production bank ensure default entries "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_author_governed_landscape_with_existing_graph"
            "_decl_contract.py"
        ))
        self.assertNotIn(
            "SkyguardMission01EnvironmentAuthoringLibrary.cpp",
