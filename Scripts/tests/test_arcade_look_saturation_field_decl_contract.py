# THIS IS leftover-safe USkyguardArcadeLookComponent Saturation.
...
# Stop BEFORE the next UPROPERTY sibling (Gain)
# as a claimed slot. Stop before any private: section.
# Do NOT claim leftover-safe ArcadeLook bEnabled as
# this slot. Copy that parse window only.
...
# DetectProgressEnd (plain static constexpr floats;
# do not parse SkyguardGuidedLockRules.h).
# Do NOT parse leftover campaign-roster-lookup-tests.
# Do NOT contract sibling fields bEnabled / Contrast /
# Gain / Gamma / BloomIntensity / Vignette / Grain /
# ChromaticAberration.
...
# RETARGET: type is float, identifier is Saturation,
# initializer is 1.12f, wrap is EditAnywhere
...
LOCKED_DECL = "float Saturation = 1.12f;"
...
LEFTOVER_THEATER_KIT_160_LOCKED = "float Saturation = 160.f;"
...
TARGET_WRONG_BARE = "float Saturation;"
TARGET_WRONG_ZERO = "float Saturation = 0.f;"
TARGET_WRONG_FALSE = "float Saturation = false;"
TARGET_WRONG_FNAME = "FName Saturation;"
TARGET_WRONG_BOOL = "bool Saturation = 1.12f;"
TARGET_WRONG_TWELVE = "float Saturation = 12.0f;"
TARGET_WRONG_TWELVE_F = "float Saturation = 12.f;"
TARGET_WRONG_RECOIL = "float Saturation = 0.92f;"
TARGET_WRONG_CONTRAST = "float Contrast = 1.18f;"
TARGET_WRONG_GAIN = "float Gain = 1.08f;"
STOP_BEFORE_GAIN = "float Gain = 1.08f;"
STOP_BEFORE_PRIVATE = "private:"
PREVIOUS_CONTRAST = "float Contrast = 1.18f;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float Saturation = 1.12f;`.
    ...
    # apache-cpg-feel 12.0f / 0.92f or GuidedLockRules
    # HelmetAcquireDegrees 12.0f.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"float\s+Saturation\s*=\s*1\.12f\s*;", compact) is None:
        return False
    if re.search(r"FName\s+WeatherIdentity\b", compact):
        return False
    if re.search(r"float\s+Saturation\s*=\s*160\.f\s*;", compact):
        return False
    if re.search(r"float\s+Saturation\s*=\s*12\.0f\s*;", compact):
        return False
    if re.search(r"float\s+Saturation\s*=\s*12\.f\s*;", compact):
        return False
    if re.search(r"float\s+Saturation\s*=\s*0\.92f\s*;", compact):
        return False
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*float\s+Saturation\s*=\s*1\.12f\s*;",
            after,
        )
    ...
    raise AssertionError(
        "UPROPERTY for float Saturation = 1.12f; is missing from "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+Saturation\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float Saturation is missing from "
        f"origin/main:{HEADER_PATH} class {CLASS_NAME} public "
        "UPROPERTY fields"
    )
...
class ArcadeLookSaturationFieldDeclContractTests(unittest.TestCase):
    def test_arcade_look_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Saturation"), section)
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        self.assertIn("Saturation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_locked_decl_is_not_leftover_theater_kit_weather_identity(
        ...
        self.assertEqual(LOCKED_DECL, "float Saturation = 1.12f;")
        ...
        self.assertIn("Saturation", LOCKED_DECL)
        self.assertIn("= 1.12f", LOCKED_DECL)
    ...
    def test_locked_decl_requires_1_12f_initializer(self) -> None:
        self.assertIn("= 1.12f", LOCKED_DECL)
        ...
        self.assertIn("float Saturation = 1.12f;", compact_origin)
        self.assertNotIn("float Saturation;", compact_origin.replace(
            "float Saturation = 1.12f;",
            "",
        ))

    def test_saturation_declaration_matches_origin_main(self) -> None:
        section = class_public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float Saturation"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 1.12f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 1.12f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 1.12f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat Saturations = 1.12f;\n",
            "\tint32 Saturation = 1.12f;\n",
            "\tfloat Saturation = " + forty + ";\n",
            "\tfloat Saturation = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_arcade_look_fields(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_GAIN, section)
        self.assertNotIn(STOP_BEFORE_GAIN, claimed)
        self.assertIn(PREVIOUS_CONTRAST, section)
        self.assertNotIn(PREVIOUS_CONTRAST, claimed)
        self.assertIn(LEFTOVER_SIBLING_ENABLED, section)
        self.assertNotIn(LEFTOVER_SIBLING_ENABLED, claimed)

    def test_claimed_window_stops_before_gain(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("Saturation", claimed)
        self.assertIn("= 1.12f", claimed)
        ...
        self.assertNotIn(SIBLING_GAIN, window)
    ...
    def test_leftover_theater_kit_wrap_is_not_locked_wrap(self) -> None:
        ...
        self.assertNotIn("= 1.12f", LEFTOVER_THEATER_KIT_DECL)
        self.assertIn("= 1.12f", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "arcade-look Saturation field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "saturation_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardArcadeLookComponent.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardGuidedLockRules.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_ARCADE_LOOK_FAIL_CLOSED, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_GAIN, locked_only)
        self.assertNotIn(SIBLING_ENABLED, claimed)
        self.assertNotIn(SIBLING_CONTRAST, claimed)
        self.assertNotIn(SIBLING_GAIN, claimed)
