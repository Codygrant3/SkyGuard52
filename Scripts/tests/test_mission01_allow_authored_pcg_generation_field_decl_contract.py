...
# origin/main form: one-line EditInstanceOnly BlueprintReadWrite wrap
# UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category="Skyguard|Mission01|Environment|PCG")
# then `bool bAllowAuthoredPCGGeneration = false;`
# (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Mission01|Environment|PCG` with NO spaces
# around `=`. EditInstanceOnly IS present.
# BlueprintReadWrite IS present. Default IS `= false`.
# This is the director ACTOR bool, not leftover
# FSkyguardMission01EnvironmentReadiness struct member
# `bool bPCGGenerationAuthorized = false;` (VisibleAnywhere,
# BlueprintReadOnly, no Category) and not leftover readiness
# `bool bReadyForAuthoredPCGGeneration = false;`.
# This is a public bool authorization field, not a combat look
# float, not leftover Theater WeatherIdentity, not leftover
# MissionMap AssemblyRevision, and not leftover analog
# apache-cpg-feel FireRate / RecoilPitch. Fail-closed if the
# UPROPERTY or decl is missing or renamed, if Category is missing,
# or if specifiers drop to VisibleAnywhere / BlueprintReadOnly
# / EditAnywhere or stay Category="Skyguard|Combat" /
# Category="Skyguard|Theater" / Category="Skyguard|MissionMap".
...
# bLicensedVegetationLibraryApproved / RouteLengthCm /
# DistrictLengthCm / Readiness / CoastalHaze* /
# bUseAuthoredLandscapeSurface / LandMaterial / OceanMaterial /
# BeachMaterial as this slot.
# Do NOT claim leftover Mission01-10 director/readiness
# same-name drafts.
# Do NOT claim leftover FName WeatherIdentity TheaterKit #1300
# (different class, Category Theater, no initializer).
# Do NOT claim leftover FName AssemblyRevision MissionMap #1630
# (EditAnywhere, Category MissionMap, FName initializer).
# Do NOT claim leftover TArray RoutePoints / LookYawLimit = 95.f.
...
# Category="Skyguard|Theater" wrapping FName WeatherIdentity
# with NO initializer — RETARGET hard.
# Better same-wrap clone was EditAnywhere BlueprintReadWrite
# Category = "Skyguard|MissionMap" wrapping FName
# AssemblyRevision WITH initializer — RETARGET hard:
# wrap is EditInstanceOnly BlueprintReadWrite
# Category="Skyguard|Mission01|Environment|PCG"
# (NO spaces around =).
# LOCKED_DECL is `bool bAllowAuthoredPCGGeneration = false;`
# NOT `FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");`
# NOT `FName WeatherIdentity;` (no initializer)
# NOT `bool bPCGGenerationAuthorized = false;`
# NOT `bool bReadyForAuthoredPCGGeneration = false;`
# NOT `bool bLicensedVegetationLibraryApproved = false;`
# NOT `bool bEnableCoastalHazeTransition = true;`
# NOT `bool bUseAuthoredLandscapeSurface = false;`
# NOT leftover `float LookYawLimit = 95.f;`
...
# TArray RoutePoints / bool bPCGGenerationAuthorized = false;
# / bool bReadyForAuthoredPCGGeneration = false;
# AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is Skyguard|Combat
# or Skyguard|Theater or Skyguard|MissionMap or
# Category="Skyguard" without |Mission01|Environment|PCG.
# Fail-closed if wrap is VisibleAnywhere or BlueprintReadOnly
# or EditAnywhere (must be EditInstanceOnly + BlueprintReadWrite).
...
TARGET = "bool bAllowAuthoredPCGGeneration = false;"
TARGET_WRONG_BARE = "bool bAllowAuthoredPCGGeneration;"
TARGET_WRONG_INIT = "bool bAllowAuthoredPCGGeneration = nullptr;"
TARGET_WRONG_FALSE = "bool bAllowAuthoredPCGGeneration = true;"
TARGET_WRONG_TRUE = "bool bAllowAuthoredPCGGeneration = true;"
TARGET_WRONG_ZERO = "bool bAllowAuthoredPCGGeneration = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> bAllowAuthoredPCGGeneration;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> bAllowAuthoredPCGGeneration;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> bAllowAuthoredPCGGeneration;"
...
STOP_BEFORE_ROUTE_LENGTH_CM = "RouteLengthCm"
...
ALLOW_AUTHORED_BARE_RE = re.compile(
    r"bool\s+bAllowAuthoredPCGGeneration\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bare bAllowAuthoredPCGGeneration without `= false`.
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_ROUTE_LENGTH_CM:
            break
        if ident != "bAllowAuthoredPCGGeneration":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bAllowAuthoredPCGGeneration\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bAllowAuthoredPCGGeneration "
...
class Mission01AllowAuthoredPCGGenerationFieldDeclContractTests(unittest.TestCase):
    def test_mission01_environment_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bAllowAuthoredPCGGeneration"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bAllowAuthoredPCGGeneration"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("bAllowAuthoredPCGGeneration", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_allow_authored_pcg_generation_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission01|Environment|PCG"', section)
        ...
        self.assertIn("|Mission01|Environment|PCG", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertIn('Category="Skyguard|Mission01|Environment|PCG"', specifiers)
    ...
    def test_allow_authored_pcg_generation_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bAllowAuthoredPCGGeneration"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("false", LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "bool bAllowAuthoredPCGGeneration = false;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool bAllowAuthoredPCGGenerations = false;\n",
            "\tint32 bAllowAuthoredPCGGeneration = 1;\n",
            "\tbool bAllowAuthoredPCGGeneration = true;\n",
            "\tbool bAllowAuthoredPCGGeneration = " + forty + ";\n",
            "\tbool bAllowAuthoredPCGGeneration = " + eighty + ";\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Mission01|Environment|PCG"', origin)
    ...
    def test_does_not_claim_sibling_route_length_cm_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bAllowAuthoredPCGGeneration"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "bAllowAuthoredPCGGeneration"))
        self.assertTrue(has_identifier(section, "bAllowAuthoredPCGGeneration"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_LICENSED_VEG))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 allow authored PCG generation field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_allow_authored_pcg_generation_field_decl_contract.py"
        ))
