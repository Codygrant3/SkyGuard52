...
# then `bool bMixedThreatRoster = true;`
# (HAS initializer). KEEP default `true`. KEEP EditAnywhere
# BlueprintReadWrite Category Skyguard|Spawning. KEEP specifiers
# matching origin/main. The origin/main comment `When true,
# lanes mix aerial, rotor, boat, and armor threats.` is
# documentation, not the slot.
...
# Word-boundary: `bMixedThreatRoster` is NOT leftover analog
# `bSpawningEnabled`, NOT leftover analog UFUNCTION
# `SetSpawningEnabled`, NOT leftover analog Mission*
# `bAllowBoundedActorSpawning`, NOT leftover analog
# `HeavyLaneModulo`.
...
# `void SetSpawningEnabled(bool bEnabled);`. LOCKED_DECL is
# NOT leftover `bool bSpawningEnabled = true;`. LOCKED_DECL
# is NOT leftover `FName WeatherIdentity`. LOCKED_DECL is NOT
# leftover `FireRate = 12.0f` / `RecoilPitch = 0.92f` /
...
# LightSpeed* / HeavyCruiseSpeed / MaxActiveDrones /
# SpawnInterval in this PR.
...
TARGET = "bool bMixedThreatRoster = true;"
TARGET_WRONG_BARE = "bool bMixedThreatRoster;"
TARGET_WRONG_INIT = "bool bMixedThreatRoster = nullptr;"
TARGET_WRONG_FALSE = "bool bMixedThreatRoster = false;"
TARGET_WRONG_TRUE = "bool bMixedThreatRoster = false;"
TARGET_WRONG_ZERO = "bool bMixedThreatRoster = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> bMixedThreatRoster;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> bMixedThreatRoster;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> bMixedThreatRoster;"
...
STOP_BEFORE_ROUTE_LENGTH_CM = "LightSpeedMin"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # LightSpeedMin / LightSpeedMax / HeavyCruiseSpeed,
    ...
    # 45000.f, leftover Harbor 40 / 80, or a bare
    # bMixedThreatRoster without `= true`.
    ...
    if re.search(r"\bLightSpeedMin\b", compact):
        return False
    if re.search(r"\bLightSpeedMax\b", compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bMixedThreatRoster\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bMixedThreatRoster "
...
class DroneSpawnerMixedThreatRosterFieldDeclContractTests(unittest.TestCase):
    def test_drone_spawner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bMixedThreatRoster"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertIn("bMixedThreatRoster", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_mixed_threat_roster_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_mixed_threat_roster_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bMixedThreatRoster"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("true", LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "bool bMixedThreatRoster = true;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool bMixedThreatRosters = false;\n",
            "\tint32 bMixedThreatRoster = 1;\n",
            "\tbool bMixedThreatRoster = false;\n",
            "\tbool bMixedThreatRoster = " + forty + ";\n",
            "\tbool bMixedThreatRoster = " + eighty + ";\n",
    ...
    def test_does_not_claim_sibling_route_length_cm_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bMixedThreatRoster"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "bMixedThreatRoster"))
        self.assertTrue(has_identifier(section, "bMixedThreatRoster"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_LICENSED_VEG))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "drone spawner mixed threat roster field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "drone_spawner_mixed_threat_roster_field_decl_contract.py"
        ))
    ...
    def test_word_boundary_mixed_threat_roster_vs_spawning_analogs(self) -> None:
        self.assertTrue(
            has_identifier(
                "bool bMixedThreatRoster = true;",
                "bMixedThreatRoster",
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "LightSpeedMax"))
