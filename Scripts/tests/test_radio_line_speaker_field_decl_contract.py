# THIS IS leftover-safe FSkyguardRadioLine Speaker FIELD
# (declaration-only). origin/main form is split-line
# UPROPERTY(EditAnywhere, BlueprintReadWrite) then BARE
...
# Fail-closed on missing/renamed decl, dropped EditAnywhere,
# BlueprintReadOnly instead of BlueprintReadWrite, or type
# not FText. Speaker has no default initializer.
# Parse STRUCT `FSkyguardRadioLine` body ONLY after
# `struct FSkyguardRadioLine`. Stop before
...
# Do NOT contract sibling fields LineId / Subtitle / Sound /
# Priority / EstimatedDurationSeconds / CooldownSeconds.
# Do NOT contract leftover briefing-radio-row Speaker
# (VisibleAnywhere / BlueprintReadOnly FText Speaker).
# THIS IS NOT leftover theater-kit-spec Speaker #1300.
...
# isolated CardId; leftover radio-line-defaults #172 then
# this isolated Speaker decl.
# If a clone asserts FName / VisibleAnywhere /
# BlueprintReadOnly / Category="Skyguard|Theater",
# retarget: type is FText, EditAnywhere, BlueprintReadWrite,
# no Category=, no `=`.
...
TARGET_WRONG_EQ_NONE = "FText Speaker = FText();"
TARGET_WRONG_FALSE = "FText Speaker = false;"
TARGET_WRONG_TRUE = "FText Speaker = true;"
TARGET_WRONG_ZERO = "FText Speaker = 0.f;"
TARGET_WRONG_FLOAT = "float Speaker;"
TARGET_WRONG_NAME = "FName Speaker;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_CAMPAIGN = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard|Campaign")'
)
TARGET_WRONG_READONLY = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
)
LOCKED_DECL = TARGET
...
UPROPERTY_VISIBLE_READONLY = TARGET_WRONG_READONLY
...
ADVANCE_RADIO_STATE = "AdvanceRadioState"
CLEAR_QUEUE = "ClearQueue"
GET_QUEUED_LINE_COUNT = "GetQueuedLineCount"
GET_CURRENT_LINE_ID = "GetCurrentLineId"
GET_DROPPED_LINE_COUNT = "GetDroppedLineCount"
GET_PLAYED_LINE_COUNT = "GetPlayedLineCount"
MAX_QUEUED_LINES = "MaxQueuedLines"
INTER_LINE_GAP_SECONDS = "InterLineGapSeconds"
ON_LINE_STARTED = "OnLineStarted"
ON_LINE_FINISHED = "OnLineFinished"
...
LEFTOVER_BRIEFING_GET_RADIO_CHATTER = (
...
LEFTOVER_BRIEFING_RADIO_ROW_SPEAKER = (
...
def leftover_component_tokens() -> tuple[str, ...]:
    return (
        ...
        ADVANCE_RADIO_STATE,
        CLEAR_QUEUE,
        GET_QUEUED_LINE_COUNT,
        GET_CURRENT_LINE_ID,
        GET_DROPPED_LINE_COUNT,
        GET_PLAYED_LINE_COUNT,
        MAX_QUEUED_LINES,
        INTER_LINE_GAP_SECONDS,
        ON_LINE_STARTED,
        ON_LINE_FINISHED,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FText Speaker;`.
    # Do not accept `= FText()` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` / leftover FName Speaker when
    # origin/main has a bare FText. Do not accept sibling
    # LineId / Subtitle / Sound / Priority fields.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FText\s+Speaker\b", compact[index:]
        ):
...
class RadioLineSpeakerFieldDeclContractTests(unittest.TestCase):
    def test_radio_line_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Speaker"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Speaker"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_speaker_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FText Speaker;", compact_origin)
        self.assertNotIn("Speaker = FText()", compact_origin)
        self.assertNotIn("Speaker = false", compact_origin)
        self.assertNotIn("Speaker = true", compact_origin)
        self.assertNotIn("Speaker = 0.f", compact_origin)
        self.assertNotIn("Speaker = 160.f", compact_origin)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_NAME}\n",
            f"\tFName {SIBLING_LINE_ID};\n",
            f"\tFText {SIBLING_SUBTITLE};\n",
            "\tFText Speakers;\n",
            "\tint32 Speaker;\n",
            "\tbool Speaker;\n",
            "\tfloat Speaker = " + forty + ";\n",
            "\tfloat Speaker = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_only_fails_closed(self) -> None:
        ...
        leftover_row = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        leftover_specs = attached_uproperty_specifiers(
            spec_section(leftover_row)
        )
    ...
    def test_does_not_parse_component_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        for token in leftover_component_tokens():
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertTrue(HEADER_PATH.endswith("SkyguardRadioChatterComponent.h"))
        self.assertNotIn("SkyguardSortiePresentationComponent.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio-line Speaker field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "speaker_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardRadioChatterComponent.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_BRIEFING_GET_RADIO_CHATTER, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_KIT_WEATHER_APPLY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_BRIEFING_RADIO_ROW_SPEAKER, LOCKED_SCRIPTS)
