...
# INDEX_NONE or lock HasContinuousCoastline
...
# leftover RebuildProductionLayout, NOT
...
# IsPointAllowedForPCG / RebuildProductionLayout /
...
# (`bool HasContinuousCoastline() const;`); accept that
...
# HasContinuousCoastline method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::HasContinuousCoastline",
    "SkyguardMission01EnvironmentDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission01HasContinuousCoastlineDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, HAS_CONTINUOUS_COASTLINE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, HAS_CONTINUOUS_COASTLINE)
        self.assertIn("HasContinuousCoastline", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, HAS_CONTINUOUS_COASTLINE)
    ...
    def test_missing_has_continuous_coastline_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, HAS_CONTINUOUS_COASTLINE)
        self.assertIn("HasContinuousCoastline", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, HAS_CONTINUOUS_COASTLINE)
        self.assertIn("HasContinuousCoastline", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("BlueprintCallable", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("BlueprintPure", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("Category", HAS_CONTINUOUS_COASTLINE)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, HAS_CONTINUOUS_COASTLINE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, HAS_CONTINUOUS_COASTLINE)
        self.assertIn("HasContinuousCoastline", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid HasContinuousCoastline() const;\n"
        missing_const = "\tbool HasContinuousCoastline();\n"
        extra_arg = "\tbool HasContinuousCoastline(int32 Amount) const;\n"
        extra_args = (
            "\tbool HasContinuousCoastline("
            "FName ObjectiveId, int32 Amount = 1) const;\n"
        )
        as_int = "\tint32 HasContinuousCoastline() const;\n"
        renamed = "\tbool HasCoastline() const;\n"
        short_name = "\tbool HasContinuousShore() const;\n"
        getter = "\tbool GetContinuousCoastline() const;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, HAS_CONTINUOUS_COASTLINE)
            self.assertIn("HasContinuousCoastline", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_has_continuous_coastline_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, HAS_CONTINUOUS_COASTLINE),
            HAS_CONTINUOUS_COASTLINE,
        )
        self.assertTrue(has_declaration(section, HAS_CONTINUOUS_COASTLINE))
        self.assertEqual(
            declaration_count(section, HAS_CONTINUOUS_COASTLINE),
            1,
        )
        self.assertTrue(
            HAS_CONTINUOUS_COASTLINE.startswith("bool "),
            HAS_CONTINUOUS_COASTLINE,
        )
        self.assertTrue(
            HAS_CONTINUOUS_COASTLINE.endswith(" const;"),
            HAS_CONTINUOUS_COASTLINE,
        )
        self.assertIn("HasContinuousCoastline", HAS_CONTINUOUS_COASTLINE)
        self.assertIn("HasContinuousCoastline()", HAS_CONTINUOUS_COASTLINE)
        self.assertTrue(
            HAS_CONTINUOUS_COASTLINE.endswith("() const;"),
            HAS_CONTINUOUS_COASTLINE,
        )
        self.assertNotIn("TArray<", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("TObjectPtr", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("TSoftObjectPtr", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("INDEX_NONE", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("UFUNCTION", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("{", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("}", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("return ", HAS_CONTINUOUS_COASTLINE)
        self.assertIn(" const;", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("void ", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("Root", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("Briefing", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("AudioDirector", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("RadioChatter", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("SortiePresentation", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("CampaignDefinition", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("MissionDefinition", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("Readiness", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("bAutoInitialize", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("bAllowBoundedActorSpawning", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("bAutoLaunchAfterBriefing", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("PathfinderSpawnLocation", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetAircraft", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("BindRuntimeActors", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("HandleDroneCityImpact", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("ConfigureMissionDefinition", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("InitializePlayableMission", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("NotifyProtectedAssetFailed", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("SynchronizeRuntimeState", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("NotifyObjectiveProgress", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetReadiness", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetObjectiveRuntime", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetGunner", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetPathfinder", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetMissionId", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("ValidateMissionContract", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("ConfigureFromMission", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("AdvanceBriefing", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("SetAssetsReady", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("AcknowledgeAndLaunch", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("CanLaunch", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetElapsedSeconds", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetBriefingState", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetMinimumWarmupSeconds", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetBriefingText", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetRadioChatter", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetPresentation", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("GetMissionTitle", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("AcknowledgeBriefing", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("LaunchSortie", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("HullCollider", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("OpticalTracker", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("WeaponServo", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("CountermeasurePod", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("MinHeightFromOriginCm", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("RadarNode", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("ESkyguardMission02WaveState", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("HarborIndustrial", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("MaxIntegrity", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("CurrentIntegrity", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("FillAndFinalize", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("FillAndFail", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("IsRouteExclusionSafe", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("bRouteExclusionValid", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            HAS_CONTINUOUS_COASTLINE,
        )
        self.assertNotIn("bAllowAuthoredPCGGeneration", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("bProductionLandscapeBound", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("bUseAuthoredLandscapeSurface", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("IsPointAllowedForPCG", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("RebuildProductionLayout", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn(
            "RefreshAuthoredEnvironmentBindings",
            HAS_CONTINUOUS_COASTLINE,
        )
        self.assertNotIn("IsAuthoredEnvironmentReady", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("IsPCGGenerationAuthorized", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("IsCorePlayableReady", HAS_CONTINUOUS_COASTLINE)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, HAS_CONTINUOUS_COASTLINE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_ENVIRONMENT} {HAS_CONTINUOUS_COASTLINE}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{HAS_CONTINUOUS_COASTLINE}\n}}\n"
        self.assertTrue(has_declaration(one_line, HAS_CONTINUOUS_COASTLINE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, HAS_CONTINUOUS_COASTLINE),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{HAS_CONTINUOUS_COASTLINE}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, HAS_CONTINUOUS_COASTLINE)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", HAS_CONTINUOUS_COASTLINE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_has_continuous_coastline_cpp_body(self) -> None:
        locked_only = f"{HAS_CONTINUOUS_COASTLINE}\n"
        self.assertNotIn("{", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("}", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("return ", HAS_CONTINUOUS_COASTLINE)
        ...
        self.assertNotIn("return false", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("return true", HAS_CONTINUOUS_COASTLINE)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{HAS_CONTINUOUS_COASTLINE}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, HAS_CONTINUOUS_COASTLINE)
        self.assertIn("HasContinuousCoastline", HAS_CONTINUOUS_COASTLINE)
        self.assertTrue(HAS_CONTINUOUS_COASTLINE.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, HAS_CONTINUOUS_COASTLINE)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("CurrentIntegrity", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("SkyguardApacheAircraft.h", HAS_CONTINUOUS_COASTLINE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", HAS_CONTINUOUS_COASTLINE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{HAS_CONTINUOUS_COASTLINE}\n"
        self.assertEqual(
            require_declaration(locked_only, HAS_CONTINUOUS_COASTLINE),
            HAS_CONTINUOUS_COASTLINE,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, HAS_CONTINUOUS_COASTLINE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 HasContinuousCoastline method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{HAS_CONTINUOUS_COASTLINE}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, HAS_CONTINUOUS_COASTLINE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                HAS_CONTINUOUS_COASTLINE.lower(),
                "mission01 HasContinuousCoastline contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, HAS_CONTINUOUS_COASTLINE)

    def test_contract_is_has_continuous_coastline_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertIn(" const;", HAS_CONTINUOUS_COASTLINE)
        self.assertTrue(HAS_CONTINUOUS_COASTLINE.endswith(";"))
        self.assertIn(UFUNCTION_ENVIRONMENT, section)

    def test_sibling_director_fields_do_not_satisfy_has_continuous_coastline(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, HAS_CONTINUOUS_COASTLINE)
            )
            self.assertNotEqual(HAS_CONTINUOUS_COASTLINE, leftover)
        self.assertIn(
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("return ", HAS_CONTINUOUS_COASTLINE)
        self.assertNotIn("return Other", HAS_CONTINUOUS_COASTLINE)
