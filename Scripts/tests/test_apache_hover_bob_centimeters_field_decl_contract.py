# THIS IS leftover-safe ASkyguardApacheAircraft HoverBobCentimeters.
...
# then BARE `float HoverBobCentimeters = 10.f;`.
...
# Initializer IS `= 10.f`. There is NO ClampMin / ClampMax
...
# Category is Skyguard|Theater or Skyguard|Arcade or
# Skyguard|Apache|Damage, if type is not float, or
# if initializer is not `= 10.f`.
...
# Claim ONLY this UPROPERTY. Do NOT claim sibling
# RotorPower (in-flight) or any TObjectPtr mesh slots.
# Do NOT claim in-flight public MaxIntegrity /
# CurrentIntegrity siblings.
# Do NOT parse leftover Apache UFUNCTION decls
# #411-#426 including leftover apache-set-rotor-power
...
# Better EditAnywhere BlueprintReadWrite retarget:
# leftover-safe ArcadeLook bEnabled / Contrast.
# RETARGET HARD: type is float, identifier is
# HoverBobCentimeters, initializer is 10.f,
# UPROPERTY is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Apache" with NO ClampMin /
# ClampMax meta (NOT Arcade, NOT Theater,
# NOT Apache|Damage).
...
# Fail-closed if this test still asserts
# RotorPower / 0.88f / ClampMin / ClampMax
# AS THE LOCKED DECL.
...
TARGET = "float HoverBobCentimeters = 10.f;"
TARGET_WRONG_BARE = "float HoverBobCentimeters;"
TARGET_WRONG_FALSE = "float HoverBobCentimeters = false;"
TARGET_WRONG_TRUE = "float HoverBobCentimeters = true;"
TARGET_WRONG_ZERO = "float HoverBobCentimeters = 0.f;"
TARGET_WRONG_ONE = "float HoverBobCentimeters = 1.f;"
TARGET_WRONG_NO_F = "float HoverBobCentimeters = 10;"
TARGET_WRONG_FNAME = "FName HoverBobCentimeters;"
TARGET_WRONG_INT = "int32 HoverBobCentimeters = 0;"
TARGET_WRONG_BOOL = "bool HoverBobCentimeters = true;"
...
TARGET_WRONG_CANNON_RATE = "float HoverBobCentimeters = 12.0f;"
TARGET_WRONG_CANNON_RECOIL = "float HoverBobCentimeters = 0.92f;"
TARGET_WRONG_CANNON_DAMAGE = "float HoverBobCentimeters = 22.0f;"
TARGET_WRONG_TIME_OF_DAY = "float HoverBobCentimeters = 12.f;"
TARGET_WRONG_SENSOR_ACQUIRE = "float HoverBobCentimeters = 5.5f;"
TARGET_WRONG_CLOUD = "float HoverBobCentimeters = 0.25f;"
...
def leftover_mesh_slots() -> tuple[str, ...]:
    return (
        "AircraftRoot",
        "SilhouetteMesh",
        "Fuselage",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare `float HoverBobCentimeters;` /
    # leftover TheaterKit WeatherIdentity / leftover
    # 160.f Health / leftover ArcadeLook Contrast 1.18f /
    # leftover RotorPower 0.88f / leftover MaxIntegrity.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+HoverBobCentimeters\s*=\s*10\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"FName\s+HoverBobCentimeters\b", compact):
        return False
    if re.search(r"int32\s+HoverBobCentimeters\b", compact):
        return False
    if re.search(r"bool\s+HoverBobCentimeters\b", compact):
        return False
    if re.search(r"FName\s+WeatherIdentity\b", compact):
        return False
    if re.search(r"float\s+HoverBobCentimeters\s*;", compact):
        return False
    return True
...
def claimed_field_window_from_section(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and field is not None:
            ...
            if "TObjectPtr" in window:
                raise AssertionError(
                    "claimed window includes TObjectPtr mesh slot"
                )
            for sibling in (
                SIBLING_MAX_INTEGRITY,
                SIBLING_CURRENT_INTEGRITY,
    ...
    raise AssertionError(
        "UPROPERTY for float HoverBobCentimeters = 10.f; is missing from "
...
def spec_section(header: str) -> str:
    ...
    if SIBLING_ROTOR in claimed:
        raise AssertionError(
            f"{CLASS_NAME} claimed window includes {SIBLING_ROTOR}"
        )
    if "TObjectPtr" in claimed:
        raise AssertionError(
            f"{CLASS_NAME} claimed window includes TObjectPtr mesh slot"
        )
    return claimed
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+HoverBobCentimeters\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float HoverBobCentimeters is missing from "
        f"origin/main:{HEADER_PATH} class {CLASS_NAME} protected "
        "UPROPERTY fields"
    )
...
class ApacheHoverBobCentimetersFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(protected, "HoverBobCentimeters"), protected)
        self.assertTrue(has_identifier(protected, SIBLING_ROTOR), protected)
        ...
        self.assertTrue(has_identifier(section, "HoverBobCentimeters"), section)
        self.assertIn("UPROPERTY", section)
        self.assertNotIn(SIBLING_ROTOR, section)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            spec_section(mixed)
        self.assertIn("HoverBobCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_hover_bob_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('ClampMin="0.0"', section)
        self.assertNotIn('ClampMax="1.0"', section)
        ...
        self.assertNotIn('Category="Skyguard|Apache|Damage"', UPROPERTY_EDIT_WRITE)
        ...
        self.assertNotIn('ClampMax="1.0"', specifiers)
        ...
        self.assertNotIn('Category="Skyguard|Apache|Damage"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float HoverBobCentimeters = 10.f;", compact_origin)
        self.assertNotIn("HoverBobCentimeters = NAME_None", compact_origin)
        self.assertNotIn("HoverBobCentimeters = false", compact_origin)
        self.assertNotIn("HoverBobCentimeters = true", compact_origin)
        self.assertNotIn("HoverBobCentimeters = 0.f", compact_origin)
        self.assertNotIn("HoverBobCentimeters = 160.f", compact_origin)
        self.assertNotIn("HoverBobCentimeters = 1.18f", compact_origin)
        self.assertNotIn("HoverBobCentimeters = 0.88f", compact_origin)
        self.assertNotIn("HoverBobCentimeters = " + "14" + "0.f", compact_origin)

    def test_hover_bob_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float HoverBobCentimeters"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 10.f", LOCKED_DECL)
        ...
        self.assertNotIn("RotorPower", LOCKED_DECL)
        ...
        self.assertNotIn("= 0.88f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 10.f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 10.f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            leftover_guided,
            f"\tfloat {SIBLING_ROTOR} = 0.88f;\n",
            "\tfloat HoverBobCentimeterss = 10.f;\n",
            "\tint32 HoverBobCentimeters = 1;\n",
            "\tbool HoverBobCentimeters = true;\n",
            "\tvoid SetRotorPower(float NormalizedPower);\n",
            "\tfloat HoverBobCentimeters = " + forty + ";\n",
            "\tfloat HoverBobCentimeters = " + eighty + ";\n",
        )
    ...
    def test_theater_kit_leftover_wrap_fails_closed(self) -> None:
        ...
        self.assertNotEqual(UPROPERTY_EDIT_WRITE, TARGET_WRONG_ROTOR_WRAP)
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        damage = (
            ...
            "{\n"
            "protected:\n"
            f"\t{TARGET_WRONG_DAMAGE_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            "private:\n"
            "};\n"
        )
        damage_specs = attached_uproperty_specifiers(spec_section(damage))
        self.assertIn('Category="Skyguard|Apache|Damage"', damage_specs)
        self.assertNotEqual(
            'Category="Skyguard|Apache"',
            'Category="Skyguard|Apache|Damage"',
        )
    ...
    def test_does_not_claim_sibling_rotor_power(self) -> None:
        header = origin_main_header()
        protected = protected_section(header)
        section = spec_section(header)
        self.assertIn(SIBLING_ROTOR, protected)
        self.assertNotIn(SIBLING_ROTOR, section)
        self.assertNotIn(SIBLING_ROTOR, LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROTOR)
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ROTOR}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_ROTOR}\n", LOCKED_DECL)
        self.assertIn("HoverBobCentimeters", str(raised.exception))
    ...
    def test_does_not_claim_in_flight_integrity_siblings(self) -> None:
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_APACHE_MAX_INTEGRITY)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_APACHE_CURRENT_INTEGRITY)
    ...
    def test_does_not_contract_sibling_or_leftover_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(protected, SIBLING_ROTOR), protected)
        self.assertFalse(has_identifier(section, SIBLING_ROTOR), section)
        self.assertNotIn("TObjectPtr", section)
        for mesh in leftover_mesh_slots():
            self.assertTrue(has_identifier(protected, mesh), mesh)
    ...
    def test_clone_source_theater_kit_weather_identity_is_not_locked_decl(
        ...
        self.assertIn("float ", LOCKED_DECL)
        self.assertIn("HoverBobCentimeters", LOCKED_DECL)
        self.assertIn("10.f", LOCKED_DECL)
        self.assertNotIn("FName ", LOCKED_DECL)
        self.assertNotIn("WeatherIdentity", LOCKED_DECL)
        self.assertNotIn("160.f", LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("BlueprintReadWrite", UPROPERTY_EDIT_WRITE)
        self.assertIn('Category="Skyguard|Apache"', UPROPERTY_EDIT_WRITE)
        self.assertNotIn('ClampMin="0.0"', UPROPERTY_EDIT_WRITE)
        self.assertNotIn('ClampMax="1.0"', UPROPERTY_EDIT_WRITE)
        self.assertNotIn("ClampMin", UPROPERTY_EDIT_WRITE)
        self.assertNotIn("ClampMax", UPROPERTY_EDIT_WRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache HoverBobCentimeters field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "hover_bob_centimeters_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
    ...
    def test_header_path_is_apache_aircraft_only(self) -> None:
        ...
        self.assertEqual(CLASS_NAME, "ASkyguardApacheAircraft")
        self.assertEqual(LOCKED_DECL, "float HoverBobCentimeters = 10.f;")
        self.assertEqual(
            UPROPERTY_EDIT_WRITE,
            'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
            'Category="Skyguard|Apache")',
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_ROTOR, locked_only)
        self.assertNotIn("TObjectPtr", locked_only)
        self.assertNotIn("0.88f", locked_only)
