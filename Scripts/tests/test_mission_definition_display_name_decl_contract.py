...
# declaration contract. Stay off MissionId (sibling draft
# this wave), CampaignOrder, MissionMap, Route, Objectives,
# Waves, Boss, Weather, Presentation, ScoreRules,
# PrerequisiteMissionIds, RequiredCampaignMedals,
# GetPrimaryAssetId, ValidateDefinition, FindObjective,
# leftover CPG debrief copy #284, leftover
# objective-definition defaults #b29f, leftover enemy-wave
# defaults #dc07, leftover boss-definition defaults #cc27,
# leftover weather-profile defaults #7dd0, leftover
# mission-presentation defaults #157e, leftover
# mission-score-rules defaults #deae, leftover
# route-definition fields #dbff, leftover campaign-roster
# #111, leftover campaign-save empty-fail-closed,
# campaign definition DisplayName / CampaignId, leftover
# CPG debrief #284/#195/#130/#8ccd, leftover
# objective-runtime fail-closed, leftover route-runtime
# fail-closed, leftover bind-hud-host, leftover gun-fire
# camera shake #8860, leftover mission-weather enum #96d2,
# leftover pilot #117/#120/#128/#129/#170,
...
# lookup, leftover CPG debrief copy / snapshot / fail-closed,
...
# broadcast, leftover objective / route / mission-score /
# weather / boss / wave / presentation defaults, leftover
# route-definition fields, campaign definition DisplayName /
# CampaignId, in-flight MissionId, leftover bind-hud-host,
# leftover gun-fire camera shake, leftover mission-weather
# enum, leftover pilot siblings, leftover #147 / #149 / #152
# / #154 stay sibling-only.
LOCKED_SCRIPTS = (
...
# here. MissionId (sibling draft this wave) /
# GetPrimaryAssetId / ValidateDefinition / FindObjective /
# CampaignOrder / MissionMap / Route / Objectives / Waves /
# Boss / Weather / Presentation / ScoreRules /
# PrerequisiteMissionIds / RequiredCampaignMedals stay
# sibling-only.
UNLOCKED_NEIGHBORS = (
    "FName MissionId;",
...
# Campaign definition DisplayName / CampaignId stay unlocked.
# This lane is DisplayName on USkyguardMissionDefinition only.
...
# USkyguardCampaignSubsystem helpers stay unlocked.
...
# Leftover defaults / leftover route-definition fields stay
# unlocked as sibling contracts. Type names may appear as
# neighbors; do not treat them as this lane's lock.
LEFTOVER_DEFAULTS_NOT_LOCKED = (
    "test_objective_definition_defaults_contract",
    "test_enemy_wave_defaults_contract",
    "test_boss_definition_defaults_contract",
    "test_weather_profile_defaults_contract",
    "test_mission_presentation_defaults_contract",
    "test_mission_score_rules_defaults_contract",
    "test_route_definition_fields_contract",
)
# Leftover #147 / #149 / #152 / #154 / leftover bind-hud-host
# / leftover gun-fire camera shake / leftover mission-weather
# enum / leftover Gunner FillAnd* stay unlocked.
LEFTOVER_NOT_LOCKED = (
...
# Sibling CampaignOrder / RequiredCampaignMedals keep their
# own meta ClampMin; do not treat that as this field's lock.
INVENTED_UPROPERTY = (
    "BlueprintReadWrite",
    "VisibleAnywhere",
    "VisibleDefaultsOnly",
...
INVENTED_FIELD_UPROPERTY = INVENTED_UPROPERTY + (
    "meta =",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionDefinition::DisplayName",
    "SkyguardMissionDefinition.cpp",
)
SIBLING_TYPES = (
...
class MissionDefinitionDisplayNameDeclContractTests(unittest.TestCase):
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        for invented in INVENTED_FIELD_UPROPERTY:
            self.assertNotIn(invented, DISPLAY_NAME)
            self.assertNotIn(invented, UPROPERTY_IDENTITY)
    ...
    def test_declaration_does_not_invent_display_name_text(self) -> None:
        ...
        self.assertNotIn("First Contact", DISPLAY_NAME)
    ...
    def test_contract_does_not_lock_display_name_cpp_body(self) -> None:
        ...
        self.assertNotIn("DisplayName.IsEmpty", DISPLAY_NAME)
        self.assertNotIn("DisplayName.IsEmpty", locked_only)
    ...
    def test_contract_does_not_relock_mission_id(self) -> None:
        ...
        self.assertNotIn("MissionId", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_campaign_order(self) -> None:
        ...
        for neighbor in CAMPAIGN_ORDER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("CampaignOrder", DISPLAY_NAME)
        self.assertNotIn("CampaignOrder", locked_only)
        self.assertNotIn("ClampMin", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_mission_map(self) -> None:
        ...
        for neighbor in MISSION_MAP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("MissionMap", DISPLAY_NAME)
        self.assertNotIn("MissionMap", locked_only)
        self.assertNotIn("TSoftObjectPtr", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_route(self) -> None:
        ...
        for neighbor in ROUTE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("FSkyguardRouteDefinition", DISPLAY_NAME)
        self.assertNotIn("FSkyguardRouteDefinition", locked_only)
        self.assertNotIn("Route", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("Objectives", DISPLAY_NAME)
        self.assertNotIn("Objectives", locked_only)
        self.assertNotIn("FSkyguardObjectiveDefinition", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_waves(self) -> None:
        ...
        for neighbor in WAVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("Waves", DISPLAY_NAME)
        self.assertNotIn("Waves", locked_only)
        self.assertNotIn("FSkyguardEnemyWaveDefinition", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_boss(self) -> None:
        ...
        for neighbor in BOSS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("Boss", DISPLAY_NAME)
        self.assertNotIn("Boss", locked_only)
        self.assertNotIn("FSkyguardBossDefinition", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_weather(self) -> None:
        ...
        for neighbor in WEATHER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("Weather", DISPLAY_NAME)
        self.assertNotIn("Weather", locked_only)
        self.assertNotIn("FSkyguardWeatherProfile", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_presentation(self) -> None:
        ...
        for neighbor in PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("Presentation", DISPLAY_NAME)
        self.assertNotIn("Presentation", locked_only)
        self.assertNotIn("FSkyguardMissionPresentation", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_score_rules(self) -> None:
        ...
        for neighbor in SCORE_RULES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("ScoreRules", DISPLAY_NAME)
        self.assertNotIn("ScoreRules", locked_only)
        self.assertNotIn("FSkyguardMissionScoreRules", DISPLAY_NAME)

    def test_contract_does_not_relock_prerequisite_ids(self) -> None:
        locked_only = f"{DISPLAY_NAME}\n"
        for neighbor in PREREQUISITE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("PrerequisiteMissionIds", DISPLAY_NAME)
        self.assertNotIn("PrerequisiteMissionIds", locked_only)

    def test_contract_does_not_relock_required_campaign_medals(self) -> None:
        locked_only = f"{DISPLAY_NAME}\n"
        for neighbor in REQUIRED_MEDALS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("RequiredCampaignMedals", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_find_objective(self) -> None:
        ...
        for neighbor in FIND_OBJECTIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
        self.assertNotIn("FindObjective", DISPLAY_NAME)
        self.assertNotIn("FindObjective", locked_only)
        self.assertNotIn("ObjectiveId", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_campaign_definition(self) -> None:
        locked_only = f"{DISPLAY_NAME}\n"
        section = public_section(origin_main_header())
        for neighbor in CAMPAIGN_DEFINITION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, DISPLAY_NAME)
            self.assertNotIn(neighbor, section)
        self.assertNotIn("USkyguardCampaignDefinition", DISPLAY_NAME)
    ...
    def test_contract_does_not_relock_leftover_defaults_siblings(self) -> None:
        locked_only = f"{DISPLAY_NAME}\n"
    ...
    def test_contract_parses_public_section_not_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("DisplayName.IsEmpty", section)
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"mission DisplayName contains "
