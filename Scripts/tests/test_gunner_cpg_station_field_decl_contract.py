...
# or `Skyguard|Theater`. Scene slots fail-closed if
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent>`.
# CpgStation fail-closed if LOCKED_DECL is leftover
# AircraftRoot / GunnerMount / CpgCockpit or leftover
# retired live-copy seat kit names built from split
# tokens. Accept one-line and split-line UPROPERTY wraps.
# Parse ONLY class `ASkyguardGunner`.
# Start at `protected:` (or parse the class body but
# claim ONLY this UPROPERTY). Stop BEFORE `private:`.
# Stop BEFORE sibling CpgCockpit as a claimed slot.
# Do not claim CpgCockpit / CpgSeatBack / CpgSeatPan /
# Boom / GunnerCamera / CpgDash / CpgTedac.
...
# EyeMount). Do NOT parse leftover ProtectAsset
# MaxIntegrity / CurrentIntegrity (different class).
...
# Clone source leftover-safe AircraftRoot wrap IS
# VisibleAnywhere BlueprintReadOnly — keep those
# specifiers — but RETARGET hard: Category is
# Skyguard NOT Skyguard|Apache. LOCKED_DECL is
# `TObjectPtr<USceneComponent> CpgStation;` NOT
# `TObjectPtr<USceneComponent> AircraftRoot;` and
# NOT `FName WeatherIdentity;`. This slot also has NO
# initializer, like the clone, but type / identifier /
# Category / class must retarget.
...
LOCKED_DECL = "TObjectPtr<USceneComponent> CpgStation;"
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_apache_eye_mount_field_decl_contract.py",
...
LEFTOVER_APACHE_AIRCRAFT_HEADER = (
    "Source/Skyguard52/SkyguardApacheAircraft.h"
)
LEFTOVER_APACHE_AIRCRAFT_WRAP = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
    'Category="Skyguard|Apache")'
)
LEFTOVER_AIRCRAFT_ROOT_DECL = (
    "TObjectPtr<USceneComponent> AircraftRoot;"
)
LEFTOVER_GUNNER_MOUNT_DECL = (
    "TObjectPtr<USceneComponent> GunnerMount;"
)
LEFTOVER_EYE_MOUNT_DECL = (
    "TObjectPtr<USceneComponent> EyeMount;"
)
LEFTOVER_STATIC_MESH_STATION = (
    "TObjectPtr<UStaticMeshComponent> CpgStation;"
)
LEFTOVER_CPG_COCKPIT_DECL = (
...
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> CpgStation = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> CpgStation = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> CpgStation = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> CpgStation = 0.f;"
TARGET_WRONG_FLOAT = "float CpgStation;"
TARGET_WRONG_FNAME = "FName CpgStation;"
TARGET_WRONG_INT = "int32 CpgStation = 0;"
TARGET_WRONG_BOOL = "bool CpgStation = true;"
...
TARGET_WRONG_SILHOUETTE = "TObjectPtr<UStaticMeshComponent> CpgCockpit;"
...
STOP_BEFORE_CPG_COCKPIT = "CpgCockpit"
...
LEFTOVER_GUNNER_MOUNT_SCRIPT = (
    "Scripts/tests/test_apache_gunner_mount"
    "_field_decl_contract.py"
)
LEFTOVER_EYE_MOUNT_SCRIPT = (
...
CPG_STATION_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*CpgStation\s*;"
)
CPG_STATION_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*CpgStation\s*="
)
CPG_STATION_FLOAT_RE = re.compile(r"float\s+CpgStation\b")
...
def leftover_apache_aircraft_fields() -> tuple[str, ...]:
    return (
        SIBLING_ROTOR_POWER,
        SIBLING_HOVER_BOB,
...
def leftover_banned_seat_kit_decls() -> tuple[str, ...]:
    retired_primary = "Ri" + "fle" + "Mesh"
    retired_tube = "Ig" + "la" + "Tube"
    retired_muzzle = "Ig" + "la" + "Muzzle"
    return (
        f"TObjectPtr<UStaticMeshComponent> {retired_primary};",
        f"TObjectPtr<UStaticMeshComponent> {retired_tube};",
        f"TObjectPtr<USceneComponent> {retired_muzzle};",
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if CPG_STATION_INIT_RE.search(compact):
        return False
    if CPG_STATION_FLOAT_RE.search(compact):
        return False
    if CPG_STATION_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+CpgStation\b", compact):
        return False
    if re.search(r"int32\s+CpgStation\b", compact):
        return False
    if re.search(r"bool\s+CpgStation\b", compact):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*TObjectPtr<USceneComponent>\s*CpgStation\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if STOP_BEFORE_CPG_COCKPIT in window:
                raise AssertionError(
    ...
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> CpgStation; is "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*CpgStation\b",
            compact[index:],
        ):
...
class GunnerCpgStationFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgStation"), section)
        ...
        self.assertNotIn(SIBLING_CPG_COCKPIT, claimed)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("CpgStation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_station_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard"', UPROPERTY_VISIBLE_READONLY)
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_AIRCRAFT_ROOT_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_GUNNER_MOUNT_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_EYE_MOUNT_DECL)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_STATIC_MESH_STATION)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_CPG_COCKPIT_DECL)
        self.assertNotEqual(
            UPROPERTY_VISIBLE_READONLY,
            LEFTOVER_APACHE_AIRCRAFT_WRAP,
        )
        for leftover in leftover_banned_seat_kit_decls():
        ...
        self.assertIn("CpgStation", LOCKED_DECL)
        self.assertNotIn("AircraftRoot", LOCKED_DECL)
    ...
    def test_locked_decl_requires_bare_pointer(self) -> None:
        ...
        self.assertNotIn("CpgStation = nullptr", compact_origin)
        self.assertNotIn("CpgStation = NAME_None", compact_origin)
        self.assertNotIn("CpgStation = false", compact_origin)
        self.assertNotIn("CpgStation = true", compact_origin)
        self.assertNotIn("CpgStation = 0.f", compact_origin)
        self.assertNotIn("CpgStation = 160.f", compact_origin)

    def test_cpg_station_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> CpgStation"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USceneComponent> CpgStations;\n",
            "\tTObjectPtr<UStaticMeshComponent> CpgStation;\n",
            "\tTObjectPtr<USceneComponent> AircraftRoot;\n",
            "\tTObjectPtr<USceneComponent> GunnerMount;\n",
            f"\t{LEFTOVER_STATIC_MESH_STATION}\n",
            f"\t{LEFTOVER_CPG_COCKPIT_DECL}\n",
            "\tTObjectPtr<USceneComponent> Root;\n",
            "\tFName WeatherIdentity;\n",
            "\tfloat CpgStation = " + forty + ";\n",
            "\tfloat CpgStation = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_GUNNER_CAMERA))
        ...
        self.assertIn(STOP_BEFORE_CPG_COCKPIT, section)
        self.assertNotIn(STOP_BEFORE_CPG_COCKPIT, claimed)
        public = public_section(origin_main_header())
        self.assertNotIn("CpgStation", public)
        for leftover in leftover_apache_aircraft_fields():
            self.assertNotIn(leftover, LOCKED_DECL)
            self.assertNotIn(leftover, claimed)
            self.assertFalse(has_identifier(public, leftover))

    def test_claimed_window_stops_before_cpg_cockpit(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("CpgStation", claimed)
        ...
        self.assertIn('Category="Skyguard"', claimed)
        self.assertNotIn('Category="Skyguard|Apache"', claimed)
        self.assertNotIn(SIBLING_CPG_COCKPIT, claimed)
        self.assertNotIn(SIBLING_ROTOR_POWER, claimed)
        self.assertNotIn(SIBLING_HOVER_BOB, claimed)
        self.assertNotIn(SIBLING_MAX_INTEGRITY, claimed)
        ...
        self.assertNotIn(SIBLING_CPG_COCKPIT, window)
    ...
    def test_parse_window_starts_at_protected_stops_before_private(self) -> None:
        ...
        self.assertIn("CpgStation", class_body(header))
        self.assertIn("CpgStation", protected)
        self.assertIn("CpgStation", section)
        self.assertNotIn("CpgStation", public)
        self.assertIn(SIBLING_CPG_COCKPIT, protected)
        self.assertIn(SIBLING_CPG_COCKPIT, section)
        self.assertNotIn(SIBLING_CPG_COCKPIT, public)
        self.assertNotIn(SIBLING_MAX_INTEGRITY, public)
    ...
    def test_does_not_parse_leftover_apache_ufunction_decls(self) -> None:
        ...
        for name in leftover_ufunction_names():
            self.assertFalse(has_identifier(public, name), name)
            self.assertFalse(has_identifier(section, name), name)
            self.assertFalse(has_identifier(claimed, name), name)
            self.assertNotIn(name, LOCKED_DECL)
        self.assertFalse(has_identifier(header, SET_SENSOR_VIEW))
        self.assertFalse(has_identifier(header, GET_ROTOR_RPM))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardGunner.h",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgStation field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cpg_station_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_GUNNER_MOUNT_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_EYE_MOUNT_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_header_path_is_gunner_only(self) -> None:
        self.assertEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardGunner.h",
        )
        ...
        self.assertNotIn("160.f", LOCKED_DECL)
        self.assertTrue(HEADER_PATH.endswith("SkyguardGunner.h"))
        self.assertNotEqual(HEADER_PATH, LEFTOVER_APACHE_AIRCRAFT_HEADER)
        ...
        self.assertNotIn("ApacheAircraft", HEADER_PATH)
