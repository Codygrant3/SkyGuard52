# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot ShotsFired.
...
TARGET_WRONG_BOOL = "bool ShotsFired = 0;"
TARGET_WRONG_FLOAT_TYPE = "float ShotsFired = 0;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 ShotsFired = 0;`.
    # Do not accept missing initializer, `= 0.f`, `= false`,
    # `= 100`, sibling Score / Medal / Hits, or a
    # leftover theater UPROPERTY wrap as the locked decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"int32\s+ShotsFired\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"int32\s+ShotsFired\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+ShotsFired\s*=\s*true\b", compact):
        return False
    if re.search(r"int32\s+ShotsFired\s*=\s*100\b", compact):
        return False
    if re.search(r"(?:float|bool|FName|FString)\s+ShotsFired\b", compact):
        return False
    if re.search(r"int32\s+ShotsFired\s*=\s*0\s*;", compact) is None:
        return False
    return True
...
class CpgDebriefSnapshotShotsFiredFieldDeclContractTests(unittest.TestCase):
    ...
    def test_capture_function_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ShotsFired"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
    ...
    def test_origin_main_shots_fired_is_plain_int32_zero(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 ShotsFired"), LOCKED_DECL)
        ...
        self.assertIn("int32 ShotsFired = 0;", compact)
        self.assertNotIn("ShotsFired = 0.f", compact)
        self.assertNotIn("ShotsFired = false", compact)
        self.assertNotIn("ShotsFired = 100", compact)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            "\tint32 ShotsFireds = 0;\n",
            "\tint32 ShotsFiredValue = 0;\n",
            "\tint32 ShotsFiredCount = 0;\n",
            "\tfloat ShotsFired = " + forty + ";\n",
            "\tfloat ShotsFired = " + eighty + ";\n",
        )
    ...
    def test_exact_identifier_vs_same_type_siblings(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ShotsFired"), section)
        self.assertIn("ShotsFired", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot ShotsFired field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "shots_fired_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
