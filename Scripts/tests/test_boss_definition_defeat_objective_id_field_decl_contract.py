# THIS IS leftover-safe FSkyguardBossDefinition DefeatObjectiveId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName DefeatObjectiveId;` (no initializer).
...
# WeakPoints / MaximumBreakupPieces.
# Those sibling decls remain unclaimed.
# THIS IS NOT leftover boss-definition-defaults #cc27.
# Analog leftover boss-definition-defaults then this
# isolated DefeatObjectiveId field decl, same as leftover
...
# `FName WeatherIdentity` / `FName PointId` /
# `FName LineId` / `FName ProfileId`, retarget: type is
# FName DefeatObjectiveId, no `=`, specifiers are EditAnywhere +
...
TARGET = "FName DefeatObjectiveId;"
TARGET_WRONG_EQ_NONE = "FName DefeatObjectiveId = NAME_None;"
TARGET_WRONG_FALSE = "FName DefeatObjectiveId = false;"
TARGET_WRONG_TRUE = "FName DefeatObjectiveId = true;"
TARGET_WRONG_ZERO = "FName DefeatObjectiveId = 0.f;"
TARGET_WRONG_FLOAT = "float DefeatObjectiveId;"
...
SIBLING_MAXIMUM_BREAKUP_PIECES = "MaximumBreakupPieces"
...
REMAINING_MAXIMUM_BREAKUP_PIECES_SCRIPT = (
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_WEAK_POINTS,
        SIBLING_BOSS_ID,
        SIBLING_MAXIMUM_BREAKUP_PIECES,
    )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+DefeatObjectiveId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName DefeatObjectiveId is missing from "
...
def require_defeat_objective_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} DefeatObjectiveId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} DefeatObjectiveId"
        )
...
class BossDefinitionDefeatObjectiveIdFieldDeclContractTests(unittest.TestCase):
    def test_boss_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "DefeatObjectiveId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "DefeatObjectiveId"), section)
        self.assertNotIn(STOP_BEFORE_WEAK_POINT_DEF, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        ...
        self.assertIn("DefeatObjectiveId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_defeat_objective_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_defeat_objective_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName DefeatObjectiveId;", compact_origin)
        self.assertNotIn("DefeatObjectiveId = NAME_None", compact_origin)
        self.assertNotIn("DefeatObjectiveId = false", compact_origin)
        self.assertNotIn("DefeatObjectiveId = true", compact_origin)
        self.assertNotIn("DefeatObjectiveId = 0.f", compact_origin)
        self.assertNotIn("DefeatObjectiveId = 160.f", compact_origin)

    def test_defeat_objective_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName DefeatObjectiveId"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFText {SIBLING_CALLSIGN};\n",
            "\tFName DefeatObjectiveIds;\n",
            "\tint32 DefeatObjectiveId;\n",
            "\tbool DefeatObjectiveId;\n",
            "\tfloat DefeatObjectiveId = " + forty + ";\n",
            "\tfloat DefeatObjectiveId = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_defeat_objective_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_defeat_objective_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_defeat_objective_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_defeat_objective_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_defeat_objective_id_specifiers(spec_section(with_category))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-definition DefeatObjectiveId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "boss_definition_defeat_objective_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_MAXIMUM_BREAKUP_PIECES, locked_only)
