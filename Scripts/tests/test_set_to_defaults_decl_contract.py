...
# values, or the SetToDefaults .cpp body.
SET_TO_DEFAULTS_DECL = "virtual void SetToDefaults() override;"
# Leftover #56–#64 plus GameUserSettings production files.
# This lane only adds an isolated Python SetToDefaults
# declaration contract. Stay off ValidateSettings (in-flight
# sibling), ApplySettings override / OnSettingsApplied /
# invert-look (#134), SetCameraShakeScale / GetCameraShakeScale,
# leftover gun-fire camera shake tests (#8860),
# GetSkyguardGameUserSettings (#292), ApplyAndSaveSettings
# (#294), volume/sensitivity drafts #293–#297, leftover
# Apache CPG feel constants (#118), leftover Harbor #6/#8/#9,
# leftover theater-kit #59, leftover flare/HUD #57/#61/#62,
# leftover drafts #56–#64, leftover #147 ApacheSystem, leftover
# #149 weapon stations, leftover #152 pilot commands, leftover
# #154 loadout/lock-phase, ApplyHydraForClusters, CPG mesh/art,
...
# Isolated-test drafts stay off this lane. Sibling GameUserSettings
# getters/setters, invert-look / ApplySettings broadcast (#134),
# leftover Apache CPG feel constants (#118), leftover theater-kit
# #59, leftover gun-fire camera shake #8860, and the in-flight
# ValidateSettings declaration stay sibling-only.
LOCKED_SCRIPTS = (
...
VALIDATE_SETTINGS_NOT_LOCKED = "virtual void ValidateSettings() override;"
GETTER_NOT_LOCKED = (
    "static USkyguardGameUserSettings* GetSkyguardGameUserSettings();"
)
...
# .cpp SetToDefaults body stays unlocked. Do not lock Super
# or the default-field writes.
SET_TO_DEFAULTS_BODY_NOT_LOCKED = (
    "Super::SetToDefaults()",
    "MasterVolume = 1.f",
    ...
    "USkyguardGameUserSettings::SetToDefaults",
)
...
# Leftover #147 / #149 / #152 / #154, leftover gun-fire camera
# shake #8860, and Hydra cluster helper stay unlocked. Do not
# take leftover ASkyguardGunner*.
...
# invent INDEX_NONE or lock a specific SetToDefaults body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return nullptr",
...
class SetToDefaultsDeclContractTests(unittest.TestCase):
    def test_game_user_settings_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, SET_TO_DEFAULTS_DECL), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_TO_DEFAULTS_DECL)
        self.assertIn("SetToDefaults", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertNotIn(SET_TO_DEFAULTS_DECL, section)

    def test_missing_set_to_defaults_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_TO_DEFAULTS_DECL)
        self.assertIn("SetToDefaults", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_validate_settings_does_not_satisfy_set_to_defaults(self) -> None:
        validate_only = "\tvirtual void ValidateSettings() override;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(validate_only, SET_TO_DEFAULTS_DECL)
        self.assertIn("SetToDefaults", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(validate_only, SET_TO_DEFAULTS_DECL))

    def test_apply_settings_does_not_satisfy_set_to_defaults(self) -> None:
        apply_only = (
            "\tvirtual void ApplySettings("
            "bool bCheckForCommandLineOverrides) override;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(apply_only, SET_TO_DEFAULTS_DECL)
        self.assertIn("SetToDefaults", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(apply_only, SET_TO_DEFAULTS_DECL))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_TO_DEFAULTS_DECL)
        self.assertIn("SetToDefaults", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_set_to_defaults_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SET_TO_DEFAULTS_DECL),
            SET_TO_DEFAULTS_DECL,
        )
        self.assertTrue(has_declaration(section, SET_TO_DEFAULTS_DECL))
        self.assertEqual(declaration_count(section, SET_TO_DEFAULTS_DECL), 1)
        self.assertTrue(SET_TO_DEFAULTS_DECL.endswith(";"), SET_TO_DEFAULTS_DECL)
        self.assertTrue(
            SET_TO_DEFAULTS_DECL.startswith("virtual "),
            SET_TO_DEFAULTS_DECL,
        )
        self.assertIn("override", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("INDEX_NONE", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("return ", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("{", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("}", SET_TO_DEFAULTS_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, SET_TO_DEFAULTS_DECL),
                section,
            )
            self.assertEqual(
                require_declaration(section, SET_TO_DEFAULTS_DECL),
            ...
            self.assertEqual(
                declaration_count(section, SET_TO_DEFAULTS_DECL),
                1,
            )
        one_line = f"{{\npublic:\n\t{SET_TO_DEFAULTS_DECL}\n}}\n"
        self.assertTrue(has_declaration(one_line, SET_TO_DEFAULTS_DECL))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, SET_TO_DEFAULTS_DECL), section)
        self.assertEqual(
            require_declaration(section, SET_TO_DEFAULTS_DECL),
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("INDEX_NONE", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("NAME_None", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("{", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("}", SET_TO_DEFAULTS_DECL)
        ...
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, SET_TO_DEFAULTS_DECL)
            self.assertNotIn(token, section)

    def test_contract_does_not_lock_set_to_defaults_cpp_body(self) -> None:
        locked_only = f"{SET_TO_DEFAULTS_DECL}\n"
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(locked_only, SET_TO_DEFAULTS_DECL),
            SET_TO_DEFAULTS_DECL,
        )
        for token in SET_TO_DEFAULTS_BODY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_TO_DEFAULTS_DECL)
            self.assertNotIn(token, section)
        self.assertNotIn("{", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("}", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("Super::", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("SkyguardGameUserSettings.cpp", SET_TO_DEFAULTS_DECL)
        self.assertNotIn(
            "USkyguardGameUserSettings::SetToDefaults",
            SET_TO_DEFAULTS_DECL,
        )

    def test_contract_does_not_relock_validate_settings(self) -> None:
        locked_only = f"{SET_TO_DEFAULTS_DECL}\n"
        self.assertNotIn(VALIDATE_SETTINGS_NOT_LOCKED, locked_only)
        self.assertNotIn(VALIDATE_SETTINGS_NOT_LOCKED, SET_TO_DEFAULTS_DECL)
        self.assertNotIn("ValidateSettings", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_relock_get_skyguard_game_user_settings(
        ...
        self.assertNotIn(GETTER_NOT_LOCKED, SET_TO_DEFAULTS_DECL)
        self.assertNotIn("GetSkyguardGameUserSettings", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_relock_apply_and_save_settings(self) -> None:
        ...
        self.assertNotIn(APPLY_AND_SAVE_NOT_LOCKED, SET_TO_DEFAULTS_DECL)
        self.assertNotIn("ApplyAndSaveSettings", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("ApplyAndSaveSettings", locked_only)
        self.assertNotIn("bCheckForCommandLineOverrides", SET_TO_DEFAULTS_DECL)

    def test_contract_does_not_relock_volume(self) -> None:
        locked_only = f"{SET_TO_DEFAULTS_DECL}\n"
        for neighbor in VOLUME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_TO_DEFAULTS_DECL)
        self.assertNotIn("SetMasterVolume", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("GetMasterVolume", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("SetMasterVolume", locked_only)
        self.assertNotIn("GetMasterVolume", locked_only)
        self.assertNotIn("return MasterVolume", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_relock_mouse_sensitivity(self) -> None:
        ...
        for neighbor in MOUSE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_TO_DEFAULTS_DECL)
        self.assertNotIn("SetMouseSensitivity", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("GetMouseSensitivity", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_relock_invert_look_or_apply_broadcast(
        ...
        for neighbor in INVERT_LOOK_NOT_LOCKED + APPLY_BROADCAST_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_TO_DEFAULTS_DECL)
        self.assertNotIn("SetInvertVerticalLook", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("GetInvertVerticalLook", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("ApplySettings", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("OnSettingsApplied", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("bInvertVerticalLook", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_relock_camera_shake(self) -> None:
        ...
        for neighbor in CAMERA_SHAKE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_TO_DEFAULTS_DECL)
        self.assertNotIn("SetCameraShakeScale", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("GetCameraShakeScale", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_relock_leftover_lanes(self) -> None:
        ...
        for token in LEFTOVER_LANES_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_TO_DEFAULTS_DECL)
            self.assertNotIn(token, section)
        self.assertNotIn("ApplyHydraForClusters", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("ASkyguardGunner*", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_parses_public_section_not_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("return GEngine", section)
        self.assertNotIn("Super::SetToDefaults", section)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("return GEngine", section)
        self.assertNotIn("return nullptr", section)
        self.assertNotIn("Super::SetToDefaults", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("{", SET_TO_DEFAULTS_DECL)
        self.assertNotIn("}", SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        self.assertNotIn(HARBOR_INCOMING, SET_TO_DEFAULTS_DECL)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(SET_TO_DEFAULTS_DECL, "Rifle")
        self.assertNotEqual(SET_TO_DEFAULTS_DECL, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"SetToDefaults public section contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not Yak",
            )
            self.assertNotIn(banned, SET_TO_DEFAULTS_DECL.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", SET_TO_DEFAULTS_DECL)

    def test_contract_is_set_to_defaults_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("SetCameraShakeScale", locked_only)
        self.assertNotIn("GetCameraShakeScale", locked_only)
        self.assertNotIn("Super::SetToDefaults", locked_only)
        for token in FEEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_TO_DEFAULTS_DECL)
        for token in LEFTOVER_LANES_NOT_LOCKED:
