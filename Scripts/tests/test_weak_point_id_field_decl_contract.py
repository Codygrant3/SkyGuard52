...
# INDEX_NONE or lock WeakPointId construction in the
...
# (`FName WeakPointId = NAME_None;`); accept that
...
# only. Keep WeakPointId distinct from sibling
# unused fields Integrity / bExposed / bDestroyed /
# MaxIntegrity and leftover accept-weapon flags on
# this component. Do not lock leftover accept
# flags. Do not lock leftover ApplyWeaponDamage /
# SetExposed / AcceptsWeapon methods
# (sibling-only). Stay off leftover LifelineHunter
...
# must be absent. WeakPoint MaxIntegrity is not
# Harbor 40/80; do not treat it as Harbor.
...
# WeakPointId field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardBossWeakPointComponent::WeakPointId",
    "SkyguardBossWeakPointComponent.cpp",
...
class WeakPointIdFieldDeclContractTests(unittest.TestCase):
    def test_weak_point_component_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, WEAK_POINT_ID), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, WEAK_POINT_ID)
        self.assertIn("WeakPointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, WEAK_POINT_ID))
    ...
    def test_missing_weak_point_id_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, WEAK_POINT_ID)
        self.assertIn("WeakPointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, WEAK_POINT_ID)
        self.assertIn("WeakPointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, WEAK_POINT_ID), section)
        self.assertNotIn("UPROPERTY", WEAK_POINT_ID)
        self.assertNotIn("EditAnywhere", WEAK_POINT_ID)
        self.assertNotIn("BlueprintReadWrite", WEAK_POINT_ID)
        self.assertNotIn("Category", WEAK_POINT_ID)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, WEAK_POINT_ID)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, WEAK_POINT_ID)
        self.assertIn("WeakPointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_default = "\tFName WeakPointId;\n"
        index_none = "\tFName WeakPointId = INDEX_NONE;\n"
        text_assigned = '\tFName WeakPointId = TEXT("Engine");\n'
        wrong_type = "\tFString WeakPointId = NAME_None;\n"
        text_type = "\tFText WeakPointId = NAME_None;\n"
        int_type = "\tint32 WeakPointId = NAME_None;\n"
        float_type = "\tfloat WeakPointId = NAME_None;\n"
        delegate_param = f"\t{DELEGATE_PARAM_FORM},\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, WEAK_POINT_ID)
            self.assertIn("WeakPointId", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_weak_point_id_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, WEAK_POINT_ID))
        self.assertEqual(declaration_count(section, WEAK_POINT_ID), 1)
        self.assertTrue(WEAK_POINT_ID.startswith("FName "), WEAK_POINT_ID)
        self.assertTrue(WEAK_POINT_ID.endswith(";"), WEAK_POINT_ID)
        self.assertIn("WeakPointId", WEAK_POINT_ID)
        self.assertIn("NAME_None", WEAK_POINT_ID)
        self.assertIn(" = ", WEAK_POINT_ID)
        self.assertNotIn("INDEX_NONE", WEAK_POINT_ID)
        self.assertNotIn("TEXT(", WEAK_POINT_ID)
        self.assertNotIn("UFUNCTION", WEAK_POINT_ID)
        self.assertNotIn("{", WEAK_POINT_ID)
        self.assertNotIn("}", WEAK_POINT_ID)
        self.assertNotIn("return ", WEAK_POINT_ID)
        self.assertNotIn("ApplyWeaponDamage", WEAK_POINT_ID)
        self.assertNotIn("SetExposed", WEAK_POINT_ID)
        self.assertNotIn("AcceptsWeapon", WEAK_POINT_ID)
        self.assertNotIn("MaxIntegrity", WEAK_POINT_ID)
        self.assertNotIn("Integrity", WEAK_POINT_ID)
        self.assertNotIn("bExposed", WEAK_POINT_ID)
        self.assertNotIn("bDestroyed", WEAK_POINT_ID)
        self.assertNotIn("HullCollider", WEAK_POINT_ID)
        self.assertNotIn("OpticalTracker", WEAK_POINT_ID)
        self.assertNotIn("WeaponServo", WEAK_POINT_ID)
        self.assertNotIn("CountermeasurePod", WEAK_POINT_ID)
        for name in leftover_weak_point_accept_flags():
            self.assertNotIn(name, WEAK_POINT_ID)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_BOSS} {WEAK_POINT_ID}\n"
            "};\n"
        )
        ...
        for header in headers:
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, WEAK_POINT_ID),
                section,
            )
            self.assertEqual(
                require_declaration(section, WEAK_POINT_ID),
            ...
            self.assertEqual(declaration_count(section, WEAK_POINT_ID), 1)
        one_line = f"{{\npublic:\n\t{WEAK_POINT_ID}\n}}\n"
        self.assertTrue(has_declaration(one_line, WEAK_POINT_ID))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, WEAK_POINT_ID), section)
        self.assertEqual(
            require_declaration(section, WEAK_POINT_ID),
    ...
    def test_assigned_text_weak_point_id_does_not_satisfy(self) -> None:
        assigned = '\tFName WeakPointId = TEXT("Engine");\n'
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, WEAK_POINT_ID)
        self.assertIn("WeakPointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, WEAK_POINT_ID))

    def test_uninitialized_weak_point_id_does_not_satisfy(self) -> None:
        bare = "\tFName WeakPointId;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(bare, WEAK_POINT_ID)
        self.assertIn("WeakPointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(bare, WEAK_POINT_ID))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("WeakPointId", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, WEAK_POINT_ID))
    ...
    def test_leftover_methods_do_not_satisfy(self) -> None:
        for leftover in LEFTOVER_METHODS:
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, WEAK_POINT_ID))
            self.assertNotIn(leftover, WEAK_POINT_ID)
    ...
    def test_delegate_param_form_does_not_satisfy_field_lock(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, WEAK_POINT_ID))
        self.assertNotIn(DELEGATE_PARAM_FORM, WEAK_POINT_ID)

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{WEAK_POINT_ID}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, WEAK_POINT_ID)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(WEAK_POINT_ID.startswith("UFUNCTION"), WEAK_POINT_ID)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, WEAK_POINT_ID), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertIn("NAME_None", section)

    def test_contract_does_not_lock_weak_point_id_cpp_body(self) -> None:
        locked_only = f"{WEAK_POINT_ID}\n"
        self.assertNotIn("{", WEAK_POINT_ID)
        self.assertNotIn("}", WEAK_POINT_ID)
        self.assertNotIn("return ", WEAK_POINT_ID)
        ...
        self.assertNotIn("return false", WEAK_POINT_ID)
        self.assertNotIn("return true", WEAK_POINT_ID)
        self.assertNotIn("CreateDefaultSubobject", WEAK_POINT_ID)
        self.assertNotIn("RefreshAuthoredWeakPointRegistry", WEAK_POINT_ID)
        self.assertNotIn("SetBossPhase", WEAK_POINT_ID)
        self.assertNotIn("HandleWeakPointDestroyed", WEAK_POINT_ID)
    ...
    def test_contract_does_not_relock_leftover_defaults_struct(self) -> None:
        ...
        for token in LEFTOVER_DEFAULTS_STRUCT_NOT_LOCKED:
            ...
            self.assertNotIn(token, WEAK_POINT_ID)
        self.assertIn(
            "Scripts/tests/test_boss_weak_point_defaults_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{WEAK_POINT_ID}\n"
        self.assertEqual(
            require_declaration(locked_only, WEAK_POINT_ID),
            WEAK_POINT_ID,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, WEAK_POINT_ID)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, WEAK_POINT_ID)
        self.assertNotIn("MinHeightFromOriginCm", WEAK_POINT_ID)
        self.assertNotIn("CurrentIntegrity", WEAK_POINT_ID)
        self.assertNotIn("SkyguardApacheAircraft.h", WEAK_POINT_ID)
        leftover_header = "SkyguardPathfinderEncounterController.h"
        self.assertNotIn(leftover_header, locked_only)
        self.assertNotIn(leftover_header, WEAK_POINT_ID)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, WEAK_POINT_ID)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", WEAK_POINT_ID)
    ...
    def test_max_integrity_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn(sibling, WEAK_POINT_ID)
        self.assertNotIn("100.f", WEAK_POINT_ID)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weak point WeakPointId field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{WEAK_POINT_ID}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, WEAK_POINT_ID.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                WEAK_POINT_ID.lower(),
                "weak point WeakPointId contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, WEAK_POINT_ID)

    def test_contract_is_weak_point_id_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", WEAK_POINT_ID)
        self.assertNotIn("{", WEAK_POINT_ID)
        self.assertTrue(WEAK_POINT_ID.startswith("FName "))
        self.assertTrue(WEAK_POINT_ID.endswith(";"))
        self.assertIn("NAME_None", WEAK_POINT_ID)
        self.assertEqual(WEAK_POINT_ID, "FName WeakPointId = NAME_None;")
        self.assertIn(UPROPERTY_BOSS, section)
        self.assertEqual(
            HEADER_PATH,
