...
# then `float MaxSimulationStepSeconds = 0.05f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Safety`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"0.01"`. ClampMax IS `"0.25"`.
# Default IS `= 0.05f`.
...
# if ClampMin / ClampMax are missing, if the default is not `= 0.05f`,
...
# Fail-closed if wrap still has ClampMin `"100.0"` / `"0.0"` / `"1.0"`
# without ClampMax, or if default is `2.4f`.
...
# MaxHeightFromOriginCm / MaxSimulationSubsteps.
...
# Category="Skyguard|Boss|Encounter|Safety" wrapping
# `float MaxLateralOffsetCm = 3600.f;` with ClampMin `"100.0"` only
# — RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Safety",
# meta=(ClampMin="0.01", ClampMax="0.25")
# (NO spaces around =).
# LOCKED_DECL is `float MaxSimulationStepSeconds = 0.05f;`
...
# and NOT leftover analog MaxLateralOffsetCm = 3600.f.
...
# Fail-closed if ClampMin / ClampMax are missing or default is not 0.05f.
# Default is `= 0.05f` not `= 3600.f` / `= 850.f` / `= 700.f` /
# `= 1250.f` / `= 9000.f` / `= 2.4f`.
...
TARGET = "float MaxSimulationStepSeconds = 0.05f;"
TARGET_WRONG_BARE = "float MaxSimulationStepSeconds;"
TARGET_WRONG_INIT = "float MaxSimulationStepSeconds = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float MaxSimulationStepSeconds = true;"
TARGET_WRONG_ZERO = "float MaxSimulationStepSeconds = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MaxSimulationStepSeconds;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MaxSimulationStepSeconds;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MaxSimulationStepSeconds;"
...
STOP_BEFORE_SKYLINE_STYLE = "MaxSimulationSubsteps"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_max_simulation_step_seconds"
...
TARGET_WRONG_SAFETY_LATERAL_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Safety", '
    'meta=(ClampMin="100.0"))'
)
TARGET_WRONG_CLAMP_ZERO = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Safety", '
    'meta=(ClampMin="0.0"))'
)
TARGET_WRONG_CLAMP_ONE = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Route", '
    'meta=(ClampMin="1.0"))'
)
...
MAX_SIMULATION_STEP_SECONDS_DECL_RE = re.compile(
    r"float\s+MaxSimulationStepSeconds\s*=\s*0\.05f\s*;"
)
MAX_SIMULATION_STEP_SECONDS_BARE_RE = re.compile(
    r"float\s+MaxSimulationStepSeconds\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float MaxSimulationStepSeconds = 0.05f;`.
    ...
    # MaxSimulationStepSeconds without the 0.05f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MAX_SIMULATION_STEP_SECONDS_DECL_RE.search(compact) is None:
        return False
    if MAX_SIMULATION_STEP_SECONDS_BARE_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "MaxSimulationStepSeconds":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MaxSimulationStepSeconds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MaxSimulationStepSeconds "
...
class PathfinderEncounterMaxSimulationStepSecondsFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxSimulationStepSeconds"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MaxSimulationStepSeconds"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaxSimulationStepSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_max_simulation_step_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_SAFETY_LATERAL_WRAP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_CLAMP_ZERO)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_CLAMP_ONE)
        ...
        self.assertIn('ClampMin="0.01"', specifiers)
        self.assertIn('ClampMax="0.25"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float MaxSimulationStepSeconds;",
            compact_origin.split("MaxSimulationStepSeconds = 0.05f;")[0]
            + compact_origin.split("MaxSimulationStepSeconds = 0.05f;")[-1]
            if "MaxSimulationStepSeconds = 0.05f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_max_simulation_step_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MaxSimulationStepSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("0.05f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat MaxSimulationStepSecondss = 0.05f;\n",
            "\tint32 MaxSimulationStepSeconds = 850;\n",
            "\tFName MaxSimulationStepSeconds = NAME_None;\n",
            "\tfloat MaxSimulationStepSeconds = " + forty + ";\n",
            "\tfloat MaxSimulationStepSeconds = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
            ...
            "\tfloat MaxSimulationStepSeconds = 2.4f;\n",
            "\tfloat MaxSimulationStepSeconds = 3600.f;\n",
            f"\t{TARGET_WRONG_BOOM}\n",
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tfloat\n\tMaxSimulationStepSeconds = 0.05f;\n",
            "\tfloat   MaxSimulationStepSeconds = 0.05f;\n",
            "\tfloat\tMaxSimulationStepSeconds = 0.05f;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaxSimulationStepSeconds"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "MaxSimulationStepSeconds"))
        self.assertTrue(has_identifier(section, "MaxSimulationStepSeconds"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="0.01"', UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMax="0.25"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter max lateral offset field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_max_simulation_step_seconds_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
