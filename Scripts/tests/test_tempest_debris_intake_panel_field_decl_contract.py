...
# INDEX_NONE or lock DebrisIntakePanel construction in the
...
# DebrisIntakePanel;`); accept that form and other
...
# drafts #107–#518, plus TempestBoss
# production files. This lane only adds an isolated
# Python DebrisIntakePanel field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardTempestBoss::DebrisIntakePanel",
    "SkyguardTempestBoss.cpp",
    "SetExposed",
...
class TempestDebrisIntakePanelFieldDeclContractTests(unittest.TestCase):
    def test_tempest_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_INTAKE_PANEL_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("DebrisIntakePanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, DEBRIS_INTAKE_PANEL_FIELD))

    def test_missing_debris_intake_panel_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardTempestBoss();\n"
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardDischargeBoom;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("DebrisIntakePanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("DebrisIntakePanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, DEBRIS_INTAKE_PANEL_FIELD), section)
        self.assertNotIn("UPROPERTY", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("Category", DEBRIS_INTAKE_PANEL_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("DebrisIntakePanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tUSkyguardBossWeakPointComponent* DebrisIntakePanel;\n"
        as_array = (
            "\tTArray<TObjectPtr<USkyguardBossWeakPointComponent>> "
            "DebrisIntakePanel;\n"
        )
        scene = "\tTObjectPtr<USceneComponent> DebrisIntakePanel;\n"
        static_mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DebrisIntakePanel;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_INTAKE_PANEL_FIELD)
            self.assertIn("DebrisIntakePanel", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_intake_panel_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_INTAKE_PANEL_FIELD),
            DEBRIS_INTAKE_PANEL_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_INTAKE_PANEL_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_INTAKE_PANEL_FIELD), 1)
        self.assertTrue(
            DEBRIS_INTAKE_PANEL_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_INTAKE_PANEL_FIELD,
        )
        self.assertTrue(DEBRIS_INTAKE_PANEL_FIELD.endswith(";"), DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("DebrisIntakePanel", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_INTAKE_PANEL_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("TArray<", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("=", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("{", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("}", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("return ", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("Engine", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("CommandCore", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("StarboardEngine", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_INTAKE_PANEL_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_DESTRUCTION} {DEBRIS_INTAKE_PANEL_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, DEBRIS_INTAKE_PANEL_FIELD), 1)
        one_line = f"{{\npublic:\n\t{DEBRIS_INTAKE_PANEL_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_INTAKE_PANEL_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_INTAKE_PANEL_FIELD), section)
        self.assertEqual(
            require_declaration(section, DEBRIS_INTAKE_PANEL_FIELD),
    ...
    def test_assigned_debris_intake_panel_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisIntakePanel = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("DebrisIntakePanel", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_INTAKE_PANEL_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            self.assertIn("DebrisIntakePanel", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_INTAKE_PANEL_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_INTAKE_PANEL_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_INTAKE_PANEL_FIELD.startswith("UFUNCTION"),
            DEBRIS_INTAKE_PANEL_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_INTAKE_PANEL_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_INTAKE_PANEL_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_intake_panel_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_INTAKE_PANEL_FIELD}\n"
        self.assertNotIn("{", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("}", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("return ", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ASkyguardTempestBoss::DebrisIntakePanel", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SkyguardTempestBoss.cpp", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SkyguardTempestBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("return true", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("RefreshAuthoredWeakPointRegistry", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetBossPhase", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_INTAKE_PANEL_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_INTAKE_PANEL_FIELD}\n"
        for token in TEMPEST_SIBLING_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_INTAKE_PANEL_FIELD)
        ...
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn("DebrisIntakePanel", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_get_civilian_separation_meters_sibling(
        ...
        for token in GET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_finale_stage(self) -> None:
        ...
        for token in GET_FINALE_STAGE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_is_climb_command_issued_sibling(
        ...
        for token in IS_CLIMB_COMMAND_ISSUED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_INTAKE_PANEL_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, DEBRIS_INTAKE_PANEL_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_guidance_array(
        ...
        for token in LEFTOVER_STARBOARD_GUIDANCE_ARRAY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("StarboardGuidanceArray", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_strike_bay_mechanism(
        ...
        for token in LEFTOVER_PORT_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_strike_bay_mechanism(
        ...
        for token in LEFTOVER_STARBOARD_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_cooling_system(self) -> None:
        ...
        for token in LEFTOVER_PORT_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("PortCoolingSystem", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_cooling_system(
        ...
        for token in LEFTOVER_STARBOARD_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("StarboardCoolingSystem", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_jammer(self) -> None:
        ...
        for token in LEFTOVER_JAMMER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("Jammer", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_last_flight_debris_fields(
        ...
        for token in LEFTOVER_LAST_FLIGHT_DEBRIS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisArmorStarboard", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisStrikeBayPort", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisStrikeBayStarboard", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisEnginePort", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("DebrisEngineStarboard", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            DEBRIS_INTAKE_PANEL_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(self) -> None:
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_INTAKE_PANEL_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_INTAKE_PANEL_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_INTAKE_PANEL_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_INTAKE_PANEL_FIELD),
            DEBRIS_INTAKE_PANEL_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss::DebrisIntakePanel", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "tempest DebrisIntakePanel field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_INTAKE_PANEL_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_INTAKE_PANEL_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_INTAKE_PANEL_FIELD.lower(),
                "tempest DebrisIntakePanel contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_INTAKE_PANEL_FIELD)

    def test_contract_is_debris_intake_panel_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("UStaticMeshComponent", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertTrue(DEBRIS_INTAKE_PANEL_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_INTAKE_PANEL_FIELD)
        self.assertIn(UPROPERTY_DESTRUCTION, section)
