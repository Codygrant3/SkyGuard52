# THIS IS leftover-safe ASkyguardGunner CpgGripLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgGripLeft;`
# (no initializer).
...
# Stop BEFORE sibling CpgGripRight as a claimed slot.
...
# CpgTedacBezel, CpgReticleH, CpgReticleV, CpgGripRight,
# CpgConsoleRight, Boom, GunnerCamera, HandMesh.
...
# #1593. CpgTedacBezel and CpgReticleH are leftover
# once drafted this pass. CpgReticleV is a sibling
# in-flight slot. Do NOT claim CpgGripRight,
# CpgReticleV, CpgReticleH, or HandMesh as this slot.
# CpgGripLeft fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgGripLeft;`
...
# CpgGripLeft fail-closed if LOCKED_DECL is leftover
...
# CpgGripRight / leftover Apache Canopy /
# leftover Apache PilotCanopy.
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgGripLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgGripLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgGripLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgGripLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgGripLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgGripLeft;"
...
CPG_GRIP_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgGripLeft\s*;"
)
CPG_GRIP_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgGripLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # CpgTedac / CpgMpdLeft / CpgMpdRight / leftover
    # Apache Canopy, leftover CpgReticleH, leftover
    # CpgReticleV, leftover CpgGripRight, or an
    # initializer on CpgGripLeft as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_GRIP_LEFT_DECL_RE.search(compact) is None:
        return False
    if CPG_GRIP_LEFT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_GRIP_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_GRIP_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_GRIP_RIGHT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgGripLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgGripLeft "
...
class GunnerCpgGripLeftFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgGripLeft"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_GRIP_RIGHT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_GRIP_RIGHT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgGripLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgGripLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_grip_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgGripLeft;", compact_origin)
        self.assertNotIn("CpgGripLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_grip_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgGripLeft"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgGripLeft;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgGripLefts;\n",
            "\tint32 CpgGripLeft = 140;\n",
            "\tbool CpgGripLeft = true;\n",
            "\tfloat CpgGripLeft = " + forty + ";\n",
            "\tfloat CpgGripLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_grip_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgGripLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_grip_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
