# THIS IS leftover-safe USkyguardAudioDirectorComponent OpenCockpitWindLoop.
...
# then BARE `TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;`
# (no initializer).
...
# Stop BEFORE sibling GlobalVoiceLimit as a claimed slot.
...
# (NO spaces around =).
# LOCKED_DECL is `TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;`
# NOT `FName WeatherIdentity;` and NOT `160.f` and NOT
# leftover Boom and NOT leftover `int32 MinimumMeasuredSamples`
# and NOT leftover `int32 SampleRate` and NOT leftover
# `int32 GeneratedByteBudget` and NOT leftover
...
TARGET = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop;"
TARGET_WRONG_BARE = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop = nullptr;"
TARGET_WRONG_INIT = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop = false;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> OpenCockpitWindLoop;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> OpenCockpitWindLoop;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> OpenCockpitWindLoop;"
...
STOP_BEFORE_SKYLINE_STYLE = "GlobalVoiceLimit"
...
OPEN_COCKPIT_WIND_LOOP_INIT_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+OpenCockpitWindLoop\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bReadyForAuthoredPCGGeneration, or an initializer
    # on OpenCockpitWindLoop (`= nullptr` / `= false` / `= true`).
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if OPEN_COCKPIT_WIND_LOOP_DECL_RE.search(compact) is None:
        return False
    if OPEN_COCKPIT_WIND_LOOP_INIT_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr<USoundBase>\s+OpenCockpitWindLoop\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundBase> OpenCockpitWindLoop "
...
class AudioDirectorOpenCockpitWindLoopFieldDeclContractTests(unittest.TestCase):
    def test_audio_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "OpenCockpitWindLoop"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_EVENT_DEFINITIONS), body)
        self.assertFalse(
            has_identifier(section, SIBLING_EVENT_DEFINITIONS),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_GLOBAL_VOICE_LIMIT), body)
        self.assertFalse(has_identifier(section, SIBLING_GLOBAL_VOICE_LIMIT), section)
        self.assertTrue(has_identifier(body, SIBLING_PROPELLER_LOOP), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "OpenCockpitWindLoop"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("OpenCockpitWindLoop", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_open_cockpit_wind_loop_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "TSoftObjectPtr<USoundBase> OpenCockpitWindLoop = nullptr;",
            compact_origin,
        )
    ...
    def test_open_cockpit_wind_loop_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSoftObjectPtr<USoundBase> OpenCockpitWindLoop"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTSoftObjectPtr<USoundBase> OpenCockpitWindLoops;\n",
            "\tint32 OpenCockpitWindLoop = 1;\n",
            "\tFName OpenCockpitWindLoop = NAME_None;\n",
            "\tTSoftObjectPtr<USoundBase> OpenCockpitWindLoop = " + forty + ";\n",
            "\tTSoftObjectPtr<USoundBase> OpenCockpitWindLoop = " + eighty + ";\n",
            f"\t{TARGET_WRONG_BOOM}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Audio|Engine"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Audio|Engine"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_engine_cruise_loop_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "OpenCockpitWindLoop"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_POWER_LOOP))
        self.assertFalse(has_identifier(section, SIBLING_ENGINE_POWER_LOOP))
        self.assertTrue(has_identifier(leaked, SIBLING_EVENT_DEFINITIONS))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "OpenCockpitWindLoop"))
        self.assertTrue(has_identifier(section, "OpenCockpitWindLoop"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_IDLE_LOOP))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_EVENT_DEFINITIONS))
        self.assertFalse(has_identifier(section, SIBLING_EVENT_DEFINITIONS))
        self.assertTrue(has_identifier(leaked, SIBLING_GLOBAL_VOICE_LIMIT))
        self.assertFalse(has_identifier(section, SIBLING_GLOBAL_VOICE_LIMIT))
        self.assertTrue(has_identifier(leaked, SIBLING_EVENT_DEFINITIONS))
        self.assertFalse(has_identifier(section, SIBLING_EVENT_DEFINITIONS))
        self.assertTrue(has_identifier(leaked, SIBLING_PRODUCTION_BANK))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio director open cockpit wind loop field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_director_open_cockpit_wind_loop_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
