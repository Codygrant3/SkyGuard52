...
# or lock the SetContactIdentified body in the .cpp.
...
# named surfaces. Stay off sibling
# IsContactIdentified / OpenOrbitExposure. Stay off
...
SET_CONTACT_IDENTIFIED = "void SetContactIdentified(bool bIdentified);"
...
# isolated Python SetContactIdentified declaration
...
# leftover RadarGhost IsContactIdentified /
...
# not locked here. Sibling OpenOrbitExposure /
# IsContactIdentified, leftover rear-aspect window /
...
# .cpp SetContactIdentified body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRadarGhostBoss::SetContactIdentified",
    "SkyguardRadarGhostBoss.cpp",
)
SIBLING_TYPES = (
...
class RadarGhostSetContactIdentifiedDeclContractTests(unittest.TestCase):
    def test_radar_ghost_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SET_CONTACT_IDENTIFIED),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_CONTACT_IDENTIFIED)
        self.assertIn("SetContactIdentified", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SET_CONTACT_IDENTIFIED))

    def test_missing_set_contact_identified_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tbool OpenOrbitExposure();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_CONTACT_IDENTIFIED)
        self.assertIn("SetContactIdentified", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_CONTACT_IDENTIFIED)
        self.assertIn("SetContactIdentified", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("UFUNCTION", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("Category", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("BlueprintCallable", SET_CONTACT_IDENTIFIED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_CONTACT_IDENTIFIED)
        self.assertIn("SetContactIdentified", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetContactIdentified;\n"
        wrong_return_bool = "\tbool SetContactIdentified(bool bIdentified);\n"
        wrong_return_int = "\tint32 SetContactIdentified(bool bIdentified);\n"
        added_const = "\tvoid SetContactIdentified(bool bIdentified) const;\n"
        wrong_type = "\tvoid SetContactIdentified(int bIdentified);\n"
        wrong_float = "\tvoid SetContactIdentified(float bIdentified);\n"
        wrong_name = "\tvoid SetContactIdentified(bool bValue);\n"
        missing_arg_name = "\tvoid SetContactIdentified(bool);\n"
        added_arg = (
            "\tvoid SetContactIdentified(bool bIdentified, bool Extra);\n"
        )
        no_args = "\tvoid SetContactIdentified();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_CONTACT_IDENTIFIED)
            self.assertIn("SetContactIdentified", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_contact_identified_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, SET_CONTACT_IDENTIFIED))
        self.assertEqual(declaration_count(section, SET_CONTACT_IDENTIFIED), 1)
        self.assertTrue(
            SET_CONTACT_IDENTIFIED.startswith("void "),
            SET_CONTACT_IDENTIFIED,
        )
        self.assertTrue(
            SET_CONTACT_IDENTIFIED.endswith(";"),
            SET_CONTACT_IDENTIFIED,
        )
        self.assertIn("SetContactIdentified(", SET_CONTACT_IDENTIFIED)
        self.assertIn("bool bIdentified", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("INDEX_NONE", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("{", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("}", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("return ", SET_CONTACT_IDENTIFIED)
        self.assertNotIn(" const", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("OpenOrbitExposure", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("IsContactIdentified", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SetSearchlightTracked", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("GetChinMuzzleLocation", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("GetGunnerMount", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("FaceWorldLocation", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SignatureModulator", SET_CONTACT_IDENTIFIED)
        for token in leftover_named_surfaces():
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetContactIdentified(bool bIdentified);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tvoid SetContactIdentified(\n"
            "\t\tbool bIdentified);\n"
            "private:\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, SET_CONTACT_IDENTIFIED),
                1,
            )
        one_line = f"{{\npublic:\n\t{SET_CONTACT_IDENTIFIED}\n}}\n"
        self.assertTrue(has_declaration(one_line, SET_CONTACT_IDENTIFIED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SET_CONTACT_IDENTIFIED),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("return ", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("bContactIdentified = bIdentified", SET_CONTACT_IDENTIFIED)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_CONTACT_IDENTIFIED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_CONTACT_IDENTIFIED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_contact_identified_cpp_body(
        self,
    ) -> None:
        locked_only = f"{SET_CONTACT_IDENTIFIED}\n"
        self.assertNotIn("{", SET_CONTACT_IDENTIFIED)
        ...
        self.assertNotIn("SkyguardRadarGhostBoss.cpp", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SkyguardRadarGhostBoss.cpp", locked_only)
        self.assertNotIn("return false", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("return true", SET_CONTACT_IDENTIFIED)

    def test_contract_does_not_relock_orbit_or_is_contact_siblings(
        self,
    ) -> None:
        locked_only = f"{SET_CONTACT_IDENTIFIED}\n"
    ...
    def test_contract_does_not_relock_leftover_named_surfaces(self) -> None:
        ...
        for sibling in leftover_boss_script_names():
            self.assertNotIn(sibling, SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_weak_point_fields(self) -> None:
        ...
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SignatureModulator", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("RadarReceiver", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("CoolingDoor", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_black_kite_siblings(
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("ASkyguardBlackKiteBoss", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SetSearchlightTracked", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_empty_fail_closed(self) -> None:
        ...
        for token in LEFTOVER_EMPTY_FAIL_CLOSED_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("GetGunnerMount", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("GetChinTurret", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("ESkyguardApacheSystem", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_apache_isolated(self) -> None:
        ...
        for token in LEFTOVER_APACHE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("FaceWorldLocation", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("MaxIntegrity", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("ASkyguardGunner", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_settings_input(self) -> None:
        ...
        for token in LEFTOVER_SETTINGS_INPUT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SetInvertVerticalLook", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("IsCaptureActive", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("USkyguardBriefingWidget", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("ESkyguardMissionSkylineStyle", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_scan_wrong_headers(self) -> None:
        ...
        for token in LEFTOVER_WRONG_HEADER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("MinHeightFromOriginCm", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("MaxIntegrity", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SkyguardPathfinder", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SkyguardApacheAircraft.h", SET_CONTACT_IDENTIFIED)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("ASkyguardRadarNode", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("SkyguardRadarNode.h", locked_only)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_CONTACT_IDENTIFIED}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_CONTACT_IDENTIFIED),
            SET_CONTACT_IDENTIFIED,
        )
        for neighbor in UNLOCKED_NEIGHBORS:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_CONTACT_IDENTIFIED)
        self.assertNotIn("OpenOrbitExposure", locked_only)
        self.assertNotIn("IsContactIdentified", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar ghost SetContactIdentified contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        file_text = this_file_text()
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SET_CONTACT_IDENTIFIED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SET_CONTACT_IDENTIFIED.lower(),
                f"radar ghost SetContactIdentified contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_CONTACT_IDENTIFIED)

    def test_contract_is_set_contact_identified_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", SET_CONTACT_IDENTIFIED)
        self.assertNotIn("{", SET_CONTACT_IDENTIFIED)
        self.assertTrue(SET_CONTACT_IDENTIFIED.startswith("void "))
        self.assertTrue(SET_CONTACT_IDENTIFIED.endswith(";"))
        self.assertIn(UFUNCTION_MISSION07_BOSS, section)
