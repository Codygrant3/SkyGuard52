...
# Mission05 IntegrationReadiness bMapAssemblyReady
...
LOCKED_DECL = MAP_ASSEMBLY_READY_FIELD
...
class Mission05MapAssemblyReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_true = (
            "public:\n"
            f"\t{UPROPERTY_MISSION10}\n"
            f"\t{MAP_ASSEMBLY_READY_FIELD_TRUE}\n"
            "};\n"
        )
        wrap_true_one_line = (
            "public:\n"
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, MAP_ASSEMBLY_READY_FIELD_TRUE),
                section,
            )
            self.assertEqual(
                require_declaration(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 bMapAssemblyReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission05 bMapAssemblyReady contains "
    ...
    def test_contract_is_map_assembly_ready_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_map_assembly_ready(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission05_map_assembly_ready"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
