...
# then BARE `TArray<FName> MissingRoutingAssets;`.
...
# MissingOutputSubmixBindings / bCategoryContractComplete /
# bProductionReady.
# Match the EXACT identifier MissingRoutingAssets as a
# whole field name after TArray<FName>. Do NOT accept a
# substring match on Missing or Routing.
...
TARGET = "TArray<FName> MissingRoutingAssets;"
TARGET_WRONG_EQ_EMPTY = "TArray<FName> MissingRoutingAssets = {};"
TARGET_WRONG_EQ_ZERO = "TArray<FName> MissingRoutingAssets = 0;"
TARGET_WRONG_FALSE = "TArray<FName> MissingRoutingAssets = false;"
TARGET_WRONG_TRUE = "TArray<FName> MissingRoutingAssets = true;"
TARGET_WRONG_ZERO = "TArray<FName> MissingRoutingAssets = 0.f;"
TARGET_WRONG_FORTY = "TArray<FName> MissingRoutingAssets = 40;"
TARGET_WRONG_EIGHTY = "TArray<FName> MissingRoutingAssets = 80;"
TARGET_WRONG_FLOAT = "float MissingRoutingAssets = 0;"
TARGET_WRONG_INT32 = "int32 MissingRoutingAssets = 0;"
TARGET_WRONG_FNAME = "FName MissingRoutingAssets;"
TARGET_WRONG_BOOL = "bool MissingRoutingAssets = false;"
TARGET_WRONG_FSTRING = "FString MissingRoutingAssets;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MissingRoutingAssets;"
...
TARGET_WRONG_SIBLING_OUTPUT = "TArray<FName> MissingOutputSubmixBindings;"
...
FIELD_NAME = "MissingRoutingAssets"
...
THIS_SCRIPT = (
    ...
    "_routing_assets_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored bare `TArray<FName> MissingRoutingAssets;`.
    # Reject added initializer (`= {}`, `= 0`, Harbor 40/80),
    # int32, float, bool, FName, FString, TSoftObjectPtr.
    # Match the EXACT identifier MissingRoutingAssets after
    # TArray<FName>. Do not accept a substring match on Missing
    # or Routing. Do not claim sibling Missing* arrays.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+MissingRoutingAssets\s*=",
        compact,
    ):
        return False
    if re.search(
        r"MissingRoutingAssets\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingRoutingAssets\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingRoutingAssets\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingRoutingAssets\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingRoutingAssets\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MissingRoutingAssets\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MissingRoutingAssets\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MissingRoutingAssets\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MissingRoutingAssets\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MissingRoutingAssets\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MissingRoutingAssets\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+MissingRoutingAssets\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> MissingRoutingAssets is missing from "
...
class ProductionAudioAuditMissingRoutingAssetsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_routing_assets_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_added_fails_closed(self) -> None:
        ...
        self.assertIn("TArray<FName> MissingRoutingAssets;", compact_origin)
        self.assertNotIn("MissingRoutingAssets = 40", compact_origin)
        self.assertNotIn("MissingRoutingAssets = 80", compact_origin)
        self.assertNotIn("MissingRoutingAssets = false", compact_origin)
        self.assertNotIn("MissingRoutingAssets = {}", compact_origin)
        self.assertNotIn("MissingRoutingAssets = 0", compact_origin)

    def test_missing_routing_assets_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> MissingRoutingAssets"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tTArray<FName> MissingRoutingAssetss;\n",
            "\tTArray<FName> MissingRoutingAssets = 1;\n",
            "\tbool MissingRoutingAssets;\n",
            "\tfloat MissingRoutingAssets = " + forty + ";\n",
            "\tfloat MissingRoutingAssets = " + eighty + ";\n",
    ...
    def test_does_not_contract_sibling_audit_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_OUTPUT_SUBMIX_BINDINGS), section
        )
        self.assertTrue(
    ...
    def test_exact_identifier_rejects_substring_missing_siblings(self) -> None:
        ...
        self.assertFalse(has_identifier(exact, "Routing"))
        self.assertFalse(has_identifier(exact, "Assets"))
        self.assertFalse(has_identifier(exact, "RoutingAssets"))
        self.assertNotEqual(FIELD_NAME, "Missing")
        self.assertNotEqual(FIELD_NAME, "Routing")
        self.assertNotEqual(FIELD_NAME, "Assets")
        self.assertTrue(FIELD_NAME.startswith("Missing"))
        self.assertTrue(FIELD_NAME.endswith("Assets"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit MissingRoutingAssets field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "missing_routing_assets_field_decl_contract.py"
        ))
