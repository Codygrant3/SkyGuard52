...
# INDEX_NONE or lock InlandVegetationPCG construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# InlandVegetationPCG distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep InlandVegetationPCG
# distinct from leftover RouteExclusion /
# IsRouteExclusionSafe / bRouteExclusionValid and
# leftover bLicensedVegetationLibraryApproved and
...
# ProductionLandscape, AuthoredPCGGraph. Do not
# lock leftover environment-readiness defaults
...
# InlandVegetationPCG field declaration contract on
...
# LandScatterBounds environment field contracts
# stay sibling-only.
LOCKED_SCRIPTS = (
...
LEFTOVER_LICENSED_VEGETATION_NOT_LOCKED = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::InlandVegetationPCG",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01InlandVegetationPcgFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, INLAND_VEGETATION_PCG_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, INLAND_VEGETATION_PCG_FIELD)
        self.assertIn("InlandVegetationPCG", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, INLAND_VEGETATION_PCG_FIELD))

    def test_missing_inland_vegetation_pcg_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, INLAND_VEGETATION_PCG_FIELD)
        self.assertIn("InlandVegetationPCG", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, INLAND_VEGETATION_PCG_FIELD)
        self.assertIn("InlandVegetationPCG", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("VisibleAnywhere", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("BlueprintReadOnly", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("Category", INLAND_VEGETATION_PCG_FIELD)
        ...
        self.assertIn("VisibleAnywhere", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertIn("BlueprintReadOnly", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotEqual(
            UPROPERTY_MISSION01_ENVIRONMENT_PCG,
            UPROPERTY_MISSION01_ENVIRONMENT,
        )
        ...
        self.assertNotIn("EditAnywhere", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, INLAND_VEGETATION_PCG_FIELD)
        self.assertIn("InlandVegetationPCG", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUPCGComponent* InlandVegetationPCG;\n"
        )
        as_array = (
            "\tTArray<TObjectPtr<"
            "UPCGComponent>> "
        ...
        scene = "\tTObjectPtr<USceneComponent> InlandVegetationPCG;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> InlandVegetationPCG;\n"
        instanced = (
            "\tTObjectPtr<UBoxComponent> InlandVegetationPCG;\n"
        )
        weak = (
            "\tTWeakObjectPtr<UPCGComponent> "
            "InlandVegetationPCG;\n"
        )
        assigned = (
            "\tTObjectPtr<UPCGComponent> "
            "InlandVegetationPCG = nullptr;\n"
        )
        soft = (
            "\tTSoftObjectPtr<UPCGComponent> "
            "InlandVegetationPCG;\n"
        )
        ...
        leftover_pcg = f"\t{LAND_SCATTER_BOUNDS_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, INLAND_VEGETATION_PCG_FIELD)
            self.assertIn("InlandVegetationPCG", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_inland_vegetation_pcg_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, INLAND_VEGETATION_PCG_FIELD),
            INLAND_VEGETATION_PCG_FIELD,
        )
        self.assertTrue(has_declaration(section, INLAND_VEGETATION_PCG_FIELD))
        self.assertEqual(
            declaration_count(section, INLAND_VEGETATION_PCG_FIELD),
            1,
        )
        self.assertTrue(
            INLAND_VEGETATION_PCG_FIELD.startswith(
                "TObjectPtr<UPCGComponent> "
            ),
            INLAND_VEGETATION_PCG_FIELD,
        )
        self.assertTrue(INLAND_VEGETATION_PCG_FIELD.endswith(";"), INLAND_VEGETATION_PCG_FIELD)
        self.assertIn("InlandVegetationPCG", INLAND_VEGETATION_PCG_FIELD)
        self.assertIn(
            "UPCGComponent",
            INLAND_VEGETATION_PCG_FIELD,
        )
        self.assertIn("TObjectPtr<", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("TArray<", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("TSoftObjectPtr", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("=", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("nullptr", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("INDEX_NONE", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("UFUNCTION", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("{", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("}", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("return ", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("OceanTiles", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("LandTiles", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("RouteExclusion", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("bRouteExclusionValid", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("BeachTiles", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("LandScatterBounds", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("bLicensedVegetationLibraryApproved", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("ProductionLandscape", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("AuthoredPCGGraph", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("Root", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetReadiness", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("Briefing", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("AudioDirector", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("RadioChatter", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("SortiePresentation", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("CampaignDefinition", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("MissionDefinition", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("bAutoInitialize", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetAircraft", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("ConfigureFromMission", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("AdvanceBriefing", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("SetAssetsReady", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("CanLaunch", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetElapsedSeconds", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetBriefingState", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetBriefingText", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetRadioChatter", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetPresentation", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("GetMissionTitle", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("AcknowledgeBriefing", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("LaunchSortie", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("HullCollider", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("OpticalTracker", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("WeaponServo", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("CountermeasurePod", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("RadarNode", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("HarborIndustrial", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("MaxIntegrity", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("CurrentIntegrity", INLAND_VEGETATION_PCG_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_PCG} {INLAND_VEGETATION_PCG_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{INLAND_VEGETATION_PCG_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, INLAND_VEGETATION_PCG_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, INLAND_VEGETATION_PCG_FIELD),
            section,
    ...
    def test_assigned_inland_vegetation_pcg_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UPCGComponent> "
            "InlandVegetationPCG = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, INLAND_VEGETATION_PCG_FIELD)
        self.assertIn("InlandVegetationPCG", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, INLAND_VEGETATION_PCG_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("InlandVegetationPCG", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, INLAND_VEGETATION_PCG_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{INLAND_VEGETATION_PCG_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            INLAND_VEGETATION_PCG_FIELD.startswith("UFUNCTION"),
            INLAND_VEGETATION_PCG_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", INLAND_VEGETATION_PCG_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_inland_vegetation_pcg_cpp_body(self) -> None:
        locked_only = f"{INLAND_VEGETATION_PCG_FIELD}\n"
        self.assertNotIn("{", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("}", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("return ", INLAND_VEGETATION_PCG_FIELD)
        ...
        self.assertNotIn("return false", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("return true", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("CreateDefaultSubobject", INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, INLAND_VEGETATION_PCG_FIELD)
        self.assertIn("InlandVegetationPCG", INLAND_VEGETATION_PCG_FIELD)
        self.assertTrue(
            INLAND_VEGETATION_PCG_FIELD.startswith(
                "TObjectPtr<UPCGComponent> "
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, INLAND_VEGETATION_PCG_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("CurrentIntegrity", INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, INLAND_VEGETATION_PCG_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{INLAND_VEGETATION_PCG_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, INLAND_VEGETATION_PCG_FIELD),
            INLAND_VEGETATION_PCG_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, INLAND_VEGETATION_PCG_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, INLAND_VEGETATION_PCG_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 InlandVegetationPCG field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{INLAND_VEGETATION_PCG_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, INLAND_VEGETATION_PCG_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                INLAND_VEGETATION_PCG_FIELD.lower(),
                "mission01 InlandVegetationPCG contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, INLAND_VEGETATION_PCG_FIELD)

    def test_contract_is_inland_vegetation_pcg_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("InlandVegetationPCG", INLAND_VEGETATION_PCG_FIELD)
        self.assertTrue(INLAND_VEGETATION_PCG_FIELD.endswith(";"))
        self.assertNotIn("=", INLAND_VEGETATION_PCG_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_PCG, section)

    def test_sibling_environment_fields_do_not_satisfy_inland_vegetation_pcg(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, INLAND_VEGETATION_PCG_FIELD))
            self.assertNotEqual(INLAND_VEGETATION_PCG_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_inland_vegetation_pcg(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
