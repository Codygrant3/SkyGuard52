...
# VisibleAudit (the nested field on AuthoringResult). THIS IS NOT
# leftover landscape-visible-audit-defaults #0bce. THIS IS NOT leftover
# VisibleAudit public UPROPERTY #1199-#1213. Do NOT parse struct
# FSkyguardLandscapeVisibleAudit itself.
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# VisibleAudit FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FSkyguardLandscapeVisibleAudit VisibleAudit; bare
# nested-struct decl, NO initializer; one-line / split-line UPROPERTY
# wraps; origin/main has NO Category). Fail closed if the type or decl
# is missing, renamed, or Category= is added. Parse STRUCT public
# section only AFTER struct FSkyguardMission01EnvironmentAuthoringResult
# and BEFORE class USkyguardMission01EnvironmentAuthoringLibrary.
...
# Do NOT parse FSkyguardLandscapeVisibleAudit struct body (leftover
# exhausted #1199-#1213).
...
# #df1a, leftover landscape-visible-audit-defaults #0bce, leftover
# VisibleAudit public UPROPERTY #1199-#1213, leftover AuthoringResult
# bSuccess #1235 / Landscape #1233 / Graph #1234, leftover
# MaterialCompilation through Error #1232.
# LOCK leftover-safe EnvironmentAuthoringResult Success / Landscape /
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# Graph. THIS IS NOT leftover mission01-environment-authoring-result-
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# Graph FIELD (declaration-only; VisibleAnywhere / BlueprintReadOnly
# FSkyguardLandscapeVisibleAudit VisibleAudit; bare decl, NO initializer; one-line /
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# Graph. THIS IS NOT leftover VisibleAudit
...
# DIFFERENT field).
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# Graph FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# FSkyguardLandscapeVisibleAudit VisibleAudit; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# BARE TObjectPtr, NO initializer. Fail closed if the type
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# Graph. THIS IS NOT leftover VisibleAudit bSuccess #1199.
...
# FSkyguardLandscapeVisibleAudit VisibleAudit; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# BARE TObjectPtr, NO initializer (NOT leftover
...
# THIS IS leftover-safe FSkyguardMission01EnvironmentAuthoringResult
# Graph FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FSkyguardLandscapeVisibleAudit VisibleAudit; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly FSkyguardLandscapeVisibleAudit VisibleAudit;
...
TARGET = "FSkyguardLandscapeVisibleAudit VisibleAudit;"
TARGET_WRONG = "FSkyguardLandscapeVisibleAudit VisibleAudit = {};"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare nested struct. Do not accept an
    # initializer for FSkyguardMission01EnvironmentAuthoringResult::
    # VisibleAudit (must be bare FSkyguardLandscapeVisibleAudit
    # VisibleAudit;). THIS IS leftover-safe AuthoringResult
    # VisibleAudit. THIS IS NOT leftover VisibleAudit public
    # UPROPERTY #1199-#1213. THIS IS NOT leftover
    # landscape-visible-audit-defaults #0bce.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FSkyguardLandscapeVisibleAudit\s+VisibleAudit\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FSkyguardLandscapeVisibleAudit VisibleAudit is missing from "
...
class Mission01EnvironmentAuthoringResultVisibleAuditFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("VisibleAudit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_visible_audit_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_visible_audit_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FSkyguardLandscapeVisibleAudit VisibleAudit"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("VisibleAudit;"), LOCKED_DECL)
        self.assertNotIn("= nullptr", LOCKED_DECL)
        self.assertNotIn(" = ", LOCKED_DECL)
        self.assertFalse(
            has_declaration(
                "\tFSkyguardLandscapeVisibleAudit VisibleAudit = nullptr;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn("ESkyguardMission05ProtectedTarget", LOCKED_DECL)
        self.assertTrue(
            LOCKED_DECL.startswith("FSkyguardLandscapeVisibleAudit "),
            LOCKED_DECL,
        )
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("VisibleAudit", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FSkyguardLandscapeVisibleAudit VisibleAudit"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("VisibleAudit", locked_only)
    ...
    def test_contract_is_target_field_declaration_only(
        ...
        self.assertTrue(LOCKED_DECL.startswith("FSkyguardLandscapeVisibleAudit VisibleAudit"))
        self.assertIn("VisibleAudit", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.endswith("VisibleAudit;"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("GetProtectedGroup", LOCKED_DECL)
        self.assertNotIn("FSkyguardMission10ProtectedRuntime", LOCKED_DECL)
        self.assertIn(UPROPERTY_MISSION10, section)

    def test_sibling_director_fields_do_not_satisfy_visible_audit(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
