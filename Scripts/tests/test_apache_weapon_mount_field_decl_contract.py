# THIS IS leftover-safe ASkyguardApacheAircraft WeaponMount.
...
# then BARE `TObjectPtr<USceneComponent> WeaponMount;` (no initializer).
...
# Type is `TObjectPtr<USceneComponent>`, NOT
...
# Stop BEFORE sibling RotorPower as a claimed slot
# (RotorPower is leftover analog).
# Do not claim GunnerMount / PilotMount / EyeMount /
# HellfireLeft / HellfireRight / ChinTurret /
# AircraftRoot / HullCollider / RotorPower /
# SilhouetteMesh / HoverBobCentimeters /
# MaxIntegrity / CurrentIntegrity.
# Do NOT parse leftover Apache UFUNCTION decls
# GetWeaponMount / AimChinTurret / SetRotorPower /
...
# analog apache-aim-chin-turret UFUNCTION, leftover
# analog apache-set-rotor-power UFUNCTION, leftover
# analog apache-cpg-feel, leftover analog
# apache-own-ship-systems, leftover analog
# gunship-weapon-stations, leftover analog
# apache-aircraft-root-field.
...
# `TObjectPtr<USceneComponent> WeaponMount;` NOT
# `FName WeatherIdentity;`. Same-type clone is
# leftover-safe AircraftRoot (TObjectPtr scene).
# This slot also has NO initializer, like the clone,
# but type / identifier / Category must retarget.
...
# `TObjectPtr<UStaticMeshComponent>`.
# WeaponMount fail-closed if LOCKED_DECL is leftover
# analog apache-mount-fail-closed, leftover ChinTurret,
# leftover GunnerMount, leftover PilotMount, leftover
# EyeMount, leftover AircraftRoot, leftover RotorPower,
# leftover HellfireLeft, leftover HellfireRight.
...
LOCKED_DECL = "TObjectPtr<USceneComponent> WeaponMount;"
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_gunship_weapon_stations_tests.py",
...
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> WeaponMount = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> WeaponMount = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> WeaponMount = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> WeaponMount = 0.f;"
TARGET_WRONG_FLOAT = "float WeaponMount;"
TARGET_WRONG_FNAME = "FName WeaponMount;"
TARGET_WRONG_INT = "int32 WeaponMount = 0;"
TARGET_WRONG_BOOL = "bool WeaponMount = true;"
...
TARGET_WRONG_MESH_TYPE = "TObjectPtr<UStaticMeshComponent> WeaponMount;"
...
STOP_BEFORE_SILHOUETTE = "SilhouetteMesh"
STOP_BEFORE_ROTOR = "RotorPower"
...
GET_WEAPON_MOUNT = "GetWeaponMount"
...
LEFTOVER_AIRCRAFT_ROOT_SCRIPT = (
    "Scripts/tests/test_apache_aircraft_root"
    "_field_decl_contract.py"
)
LEFTOVER_SILHOUETTE_MESH_SCRIPT = (
    "Scripts/tests/test_apache_silhouette_mesh"
    "_field_decl_contract.py"
)
LEFTOVER_ROTOR_POWER_SCRIPT = (
...
LEFTOVER_GUNSHIP_WEAPON_STATIONS_TESTS = (
    "Scripts/tests/test_gunship_weapon_stations_tests.py"
)
...
WEAPON_MOUNT_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*WeaponMount\s*;"
)
WEAPON_MOUNT_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*WeaponMount\s*="
)
WEAPON_MOUNT_FLOAT_RE = re.compile(r"float\s+WeaponMount\b")
...
def leftover_sibling_mount_decls() -> tuple[str, ...]:
    return (
        TARGET_WRONG_AIRCRAFT_ROOT,
        TARGET_WRONG_GUNNER_MOUNT,
        TARGET_WRONG_PILOT_MOUNT,
        TARGET_WRONG_EYE_MOUNT,
        TARGET_WRONG_CHIN_TURRET,
        TARGET_WRONG_HELLFIRE_LEFT,
        TARGET_WRONG_HELLFIRE_RIGHT,
        TARGET_WRONG_MESH_TYPE,
        TARGET_WRONG_ROTOR,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if WEAPON_MOUNT_INIT_RE.search(compact):
        return False
    if WEAPON_MOUNT_FLOAT_RE.search(compact):
        return False
    if WEAPON_MOUNT_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+WeaponMount\b", compact):
        return False
    if re.search(r"int32\s+WeaponMount\b", compact):
        return False
    if re.search(r"bool\s+WeaponMount\b", compact):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*TObjectPtr<USceneComponent>\s*WeaponMount\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if STOP_BEFORE_SILHOUETTE in window:
                raise AssertionError(
                    "claimed window includes sibling SilhouetteMesh"
                )
            if STOP_BEFORE_ROTOR in window:
                raise AssertionError(
                    "claimed window includes leftover RotorPower"
                )
            if SIBLING_AIRCRAFT_ROOT in window:
                raise AssertionError(
                    "claimed window includes leftover AircraftRoot"
                )
            if SIBLING_GUNNER_MOUNT in window:
                raise AssertionError(
                    "claimed window includes leftover GunnerMount"
                )
            if SIBLING_PILOT_MOUNT in window:
                raise AssertionError(
                    "claimed window includes leftover PilotMount"
                )
            if SIBLING_EYE_MOUNT in window:
                raise AssertionError(
                    "claimed window includes leftover EyeMount"
                )
            if SIBLING_CHIN_TURRET in window:
                raise AssertionError(
                    "claimed window includes leftover ChinTurret"
                )
            if SIBLING_HELLFIRE_LEFT in window:
                raise AssertionError(
                    "claimed window includes leftover HellfireLeft"
                )
            if SIBLING_HELLFIRE_RIGHT in window:
                raise AssertionError(
                    "claimed window includes leftover HellfireRight"
                )
            return window
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> WeaponMount; is "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*WeaponMount\b",
            compact[index:],
        ):
...
class ApacheWeaponMountFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "WeaponMount"), section)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("WeaponMount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_weapon_mount_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MESH_TYPE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROTOR)
        ...
        self.assertIn("WeaponMount", LOCKED_DECL)
    ...
    def test_locked_decl_requires_bare_pointer(self) -> None:
        ...
        self.assertNotIn("WeaponMount = nullptr", compact_origin)
        self.assertNotIn("WeaponMount = NAME_None", compact_origin)
        self.assertNotIn("WeaponMount = false", compact_origin)
        self.assertNotIn("WeaponMount = true", compact_origin)
        self.assertNotIn("WeaponMount = 0.f", compact_origin)
        self.assertNotIn("WeaponMount = 160.f", compact_origin)

    def test_weapon_mount_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> WeaponMount"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USceneComponent> WeaponMounts;\n",
            "\tTObjectPtr<UStaticMeshComponent> WeaponMount;\n",
            f"\t{TARGET_WRONG_MESH_TYPE}\n",
            ...
            "\tfloat WeaponMount = " + forty + ";\n",
            "\tfloat WeaponMount = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_EYE_MOUNT))
        ...
        self.assertIn(STOP_BEFORE_SILHOUETTE, section)
        self.assertNotIn(STOP_BEFORE_SILHOUETTE, claimed)
        self.assertIn(STOP_BEFORE_ROTOR, section)
        self.assertNotIn(STOP_BEFORE_ROTOR, claimed)
    ...
    def test_claimed_window_stops_before_rotor_power(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("WeaponMount", claimed)
        ...
        self.assertNotIn(SIBLING_EYE_MOUNT, claimed)
        ...
        self.assertNotIn(SIBLING_ROTOR_POWER, window)
        self.assertNotIn(STOP_BEFORE_ROTOR, window)
    ...
    def test_parse_window_starts_at_protected_stops_before_private(self) -> None:
        ...
        self.assertIn("WeaponMount", class_body(header))
        self.assertIn("WeaponMount", protected)
        self.assertIn("WeaponMount", section)
        self.assertNotIn(LOCKED_DECL, public)
        self.assertIn(GET_WEAPON_MOUNT, public)
        self.assertNotIn(GET_WEAPON_MOUNT, section)
        self.assertIn(SIBLING_MAX_INTEGRITY, public)
        self.assertNotIn(SIBLING_MAX_INTEGRITY, section)
        mixed = (
            f"class {CLASS_NAME}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache WeaponMount field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "weapon_mount_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AIRCRAFT_ROOT_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SILHOUETTE_MESH_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROTOR_POWER_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AIM_CHIN_TURRET, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SET_ROTOR_POWER, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNSHIP_WEAPON_STATIONS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_GUNSHIP_WEAPON_STATIONS_TESTS, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(GET_WEAPON_MOUNT, locked_only)
        ...
        self.assertNotIn("ChinTurret", locked_only)
        self.assertNotIn("HellfireLeft", locked_only)
        self.assertNotIn("HellfireRight", locked_only)
        self.assertNotIn("UStaticMeshComponent", locked_only)
        for leftover_decl in leftover_sibling_mount_decls():
            self.assertNotEqual(LOCKED_DECL, leftover_decl)
            self.assertFalse(
                has_declaration(f"\t{leftover_decl}\n", LOCKED_DECL)
            )
