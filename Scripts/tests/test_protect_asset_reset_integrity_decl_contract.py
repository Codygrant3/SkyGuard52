# THIS IS leftover-safe ASkyguardProtectAsset::ResetIntegrity.
# origin/main form: split-line
# UFUNCTION(BlueprintCallable, Category="Skyguard|Campaign")
# then `void ResetIntegrity();`.
...
# GetIntegrityFraction (sibling workers).
...
TARGET = "void ResetIntegrity();"
TARGET_WRONG_BARE = "void ResetIntegrity(float Amount);"
TARGET_WRONG_CONST = "void ResetIntegrity() const;"
TARGET_WRONG_BOOL = "bool ResetIntegrity();"
TARGET_WRONG_FLOAT = "float ResetIntegrity();"
...
UNCONTRACTED_MAX_INTEGRITY = "MaxIntegrity"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored ResetIntegrity signature.
    # Do not accept a param, a const qualifier, a bool/float
    # return, an inline body, or a sibling ApplyDamage when
    # origin/main has `void ResetIntegrity();`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"\bvoid\s+ResetIntegrity\s*\(\s*float\s+Amount\s*\)",
        compact,
    ):
        return False
    if re.search(
        r"\bvoid\s+ResetIntegrity\s*\(\s*\)\s*const",
        compact,
    ):
        return False
    if re.search(r"\bbool\s+ResetIntegrity\s*\(", compact):
        return False
    if re.search(r"\bfloat\s+ResetIntegrity\s*\(", compact):
        return False
    if re.search(r"\bvoid\s+ResetIntegrity\s*\(\s*\)\s*\{", compact):
        return False
    if re.search(
        r"\bvoid\s+ResetIntegrity\s*\(\s*\)\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+ResetIntegrity\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void ResetIntegrity is missing from "
...
class ProtectAssetResetIntegrityDeclContractTests(unittest.TestCase):
    def test_protect_asset_class_exists(self) -> None:
        ...
        self.assertIn("ResetIntegrity", section)
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertNotIn("ResetIntegrity", section)
        for token in protected_component_tokens()[:1]:
            self.assertNotIn(token, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ResetIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_reset_integrity_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn("void ResetIntegrity()", collapsed(origin))
        self.assertNotIn("void ResetIntegrity(float Amount)", collapsed(origin))
        self.assertNotIn("const float Amount", collapsed(origin))

    def test_reset_integrity_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("void ResetIntegrity"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            "\tvoid ResetInt();\n",
            "\tvoid ResetIntegrities();\n",
            "\tvoid TakeIntegrity();\n",
            "\tvoid ResetIntegrity(int32 Amount);\n",
            "\tvoid ResetIntegrity() const;\n",
            "\tvoid Reset();\n",
        )
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\tfloat {UNCONTRACTED_MAX_INTEGRITY} = 100.f;\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "protect-asset ResetIntegrity decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_protect_asset_reset_integrity"
            "_decl_contract.py"
        ))
