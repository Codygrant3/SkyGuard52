# THIS IS leftover-safe ASkyguardApacheAircraft Nose.
...
# then BARE `TObjectPtr<UStaticMeshComponent> Nose;` (no initializer).
...
# Stop BEFORE sibling Canopy as a claimed slot.
# Do not claim Fuselage / SilhouetteMesh / AircraftRoot /
...
# Do NOT parse leftover analog apache-hull-collider, leftover
# analog apache-mount-fail-closed, leftover analog
# apache-cpg-feel, leftover analog apache-own-ship-systems.
# Do NOT parse leftover analog apache-silhouette-mesh-field
# or leftover analog apache-aircraft-root-field (keep those
...
# Approach/Contact/Shore, leftover ASkyguardGunshipSortieDirector.
...
# `TObjectPtr<UStaticMeshComponent> Nose;` NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> Nose;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> Nose = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> Nose = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> Nose = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> Nose;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> Nose;"
...
NOSE_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+Nose\s*;"
)
NOSE_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+Nose\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover Fuselage / SilhouetteMesh / AircraftRoot /
    # GearNose, or an initializer on Nose as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if NOSE_DECL_RE.search(compact) is None:
        return False
    if NOSE_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CANOPY):
            break
    ...
    if has_identifier(window, SIBLING_CANOPY):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+Nose\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> Nose "
...
class ApacheNoseFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Nose"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_CANOPY),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Nose"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Nose", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_nose_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> Nose;", compact_origin)
        self.assertNotIn("Nose = 160.f", compact_origin)
        self.assertNotIn("TObjectPtr<UStaticMeshComponent> GearNose;", LOCKED_DECL)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_nose_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> Nose"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_GEAR_NOSE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> Nose;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> Noses;\n",
            "\tint32 Nose = 140;\n",
            "\tbool Nose = true;\n",
            "\tfloat Nose = " + forty + ";\n",
            "\tfloat Nose = " + eighty + ";\n",
    ...
    def test_does_not_claim_sibling_canopy_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertFalse(has_identifier(section, SIBLING_CANOPY))
        self.assertTrue(has_identifier(leaked, SIBLING_FUSELAGE))
        self.assertTrue(has_identifier(section, SIBLING_FUSELAGE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache Nose field decl contract "
