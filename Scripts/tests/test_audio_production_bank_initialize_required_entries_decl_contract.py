# THIS IS leftover-safe USkyguardAudioProductionBank
...
# then declaration-only
# `void InitializeRequiredEntries();`
# (no inline body required).
# THIS IS leftover-safe isolated UFUNCTION.
# Category IS `Skyguard|Audio|Production`. BlueprintCallable IS present.
# const IS absent. Return type IS void.
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# drop to BlueprintPure, if const is present, if
# the return type is not void, or if Category is
...
# Parse class `USkyguardAudioProductionBank` public
# section ONLY after `class USkyguardAudioProductionBank`
# / `GENERATED_BODY`. Start at `public:`. Stop BEFORE
# `private:`. Stop BEFORE first public UPROPERTY
# (`Entries` / `Routing`).
# Do NOT claim sibling UFUNCTION EnsureDefaultEntries /
# EvaluateReadiness / ConfigureRoutingTopology /
# GetUnboundRequiredCategories.
# Do NOT claim leftover unique AudioDirector Production
# UFUNCTION GetResolvedProductionLoopRouteCount /
# AreResolvedProductionLoopRoutesComplete.
# Do NOT claim leftover analog ProductionBank /
# ProductionBankAsset UPROPERTY.
# Do NOT claim leftover analog EventDefinitions /
# FSkyguardAudioEventDefinition.
# Do NOT claim leftover analog PropellerLoop / PropSpinner.
# Do NOT claim leftover EngineIdleLoop / EngineCruiseLoop /
# EnginePowerLoop / OpenCockpitWindLoop / GlobalVoiceLimit.
# Skip leftover analog ProductionAudioRouting
# CockpitExteriorAttenuation / CockpitLowPassHz.
# Skip leftover analog audio-production-bank-empty-fail-closed.
# Skip leftover analog audio-procedural-bank-empty-fail-closed.
# Skip leftover analog audio-harness-fail-closed.
# Skip leftover analog audio-director-engine-state /
# telemetry / suppression / bank-null / world-event /
# listener-perspective fail-closed bulk.
# Skip leftover Mission01 AudioDirector TObjectPtr.
# Skip leftover analog theater-kit-get-lamp-instance-count
# Category Skyguard|Theater.
# Skip leftover analog Pathfinder Encounter unique FIELD.
...
# — RETARGET hard: wrap is BlueprintCallable
# Category="Skyguard|Audio|Production" (NO spaces around =).
# Class USkyguardAudioDirectorComponent →
# USkyguardAudioProductionBank. Method →
# InitializeRequiredEntries. Return void. No const.
# Fail-closed if wrap still has BlueprintPure.
...
# `int32 GetResolvedProductionLoopRouteCount() const;` and NOT
# `bool AreResolvedProductionLoopRoutesComplete() const;` and NOT
# `int32 GetLampInstanceCount() const;` and NOT
# `int32 GlobalVoiceLimit = 24;` and NOT leftover
# TObjectPtr ProductionBank / ProductionBankAsset and NOT leftover
# OpenCockpitWindLoop and NOT leftover
# EngineIdleLoop and NOT leftover
# `int32 MaximumAllowedVoices = 48;` and NOT leftover
# `int32 PeakActiveVoices = 0;` and NOT leftover
# `float FireRate = 12.0f` / `float RecoilPitch = 0.92f`
# and NOT leftover Boom and NOT leftover
# VegetationStartCullDistanceCm and NOT leftover sibling
# EnsureDefaultEntries / EvaluateReadiness /
# ConfigureRoutingTopology / GetUnboundRequiredCategories
# and NOT leftover analog struct Entries / Routing.
# Fail-closed if this test still asserts FName
# WeatherIdentity / GetResolvedProductionLoopRouteCount /
# AreResolvedProductionLoopRoutesComplete /
# GetLampInstanceCount / GlobalVoiceLimit = 24 /
# TObjectPtr ProductionBank / ProductionBankAsset /
# OpenCockpitWindLoop / EngineIdleLoop /
# MaximumAllowedVoices = 48 /
# PeakActiveVoices = 0 / FireRate = 12.0f /
# RecoilPitch = 0.92f / leftover Boom / leftover
# VegetationStartCullDistanceCm AS THE LOCKED DECL.
...
# or Skyguard|Audio|Development or
# Category="Skyguard|Audio" without |Production
# or Category="Skyguard" without |Audio|Production.
# Fail-closed if wrap is BlueprintPure
# (must be BlueprintCallable).
# Fail-closed if const is present or return type is
# not void.
...
CLASS_NAME = "USkyguardAudioProductionBank"
TARGET = "void InitializeRequiredEntries();"
TARGET_WRONG_BARE = "void InitializeRequiredEntries() const;"
TARGET_WRONG_INIT = "void InitializeRequiredEntries() = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "void InitializeRequiredEntries() = true;"
TARGET_WRONG_ZERO = "void InitializeRequiredEntries() = 0.f;"
TARGET_WRONG_NO_CONST = "void InitializeRequiredEntries() const;"
TARGET_WRONG_FLOAT = "float InitializeRequiredEntries();"
TARGET_WRONG_FNAME_RET = "FName InitializeRequiredEntries();"
TARGET_WRONG_BODY = "void InitializeRequiredEntries() { }"
...
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> InitializeRequiredEntries;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> InitializeRequiredEntries;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> InitializeRequiredEntries;"
...
LOCKED_DECL = TARGET
UFUNCTION_CALLABLE_PRODUCTION = (
    'UFUNCTION(BlueprintCallable, '
    'Category="Skyguard|Audio|Production")'
)
UPROPERTY_EDIT_READWRITE = UFUNCTION_CALLABLE_PRODUCTION
UFUNCTION_LEFTOVER_AUDIO = (
    'UFUNCTION(BlueprintPure, '
    'Category="Skyguard|Audio")'
)
UFUNCTION_LEFTOVER_PURE = (
    'UFUNCTION(BlueprintPure, '
    'Category="Skyguard|Audio|Production")'
)
...
STOP_BEFORE_SKYLINE_STYLE = "EnsureDefaultEntries"
...
THIS_SCRIPT = (
    "Scripts/tests/test_audio_production_bank_initialize"
    "_required_entries_decl_contract.py"
)
...
SIBLING_GET_RESOLVED_ROUTE_COUNT = "GetResolvedProductionLoopRouteCount"
SIBLING_ENSURE_DEFAULT_ENTRIES = "EnsureDefaultEntries"
SIBLING_EVALUATE_READINESS = "EvaluateReadiness"
SIBLING_CONFIGURE_ROUTING = "ConfigureRoutingTopology"
SIBLING_GET_UNBOUND_CATEGORIES = "GetUnboundRequiredCategories"
SIBLING_ENTRIES = "Entries"
SIBLING_ROUTING = "Routing"
SIBLING_FIND_ENTRY = "FindEntry"
SIBLING_GET_REQUIRED_CATEGORIES = "GetRequiredCategories"
SIBLING_GET_CATEGORY_DISPLAY_NAME = "GetCategoryDisplayName"
...
TARGET_WRONG_GET_RESOLVED = (
    "int32 GetResolvedProductionLoopRouteCount() const;"
)
TARGET_WRONG_PRODUCTION_BANK = (
    "TObjectPtr<USkyguardAudioProductionBank> ProductionBank;"
)
TARGET_WRONG_PRODUCTION_BANK_ASSET = (
    "TObjectPtr<USkyguardAudioProductionBank> ProductionBankAsset;"
)
...
INITIALIZE_REQUIRED_ENTRIES_RE = re.compile(
    r"void\s+InitializeRequiredEntries\s*\(\s*\)\s*;"
)
INITIALIZE_REQUIRED_ENTRIES_WRONG_TYPE_RE = re.compile(
    r"(?:float|FName|bool|FLinearColor|int32)\s+"
    r"InitializeRequiredEntries\b"
)
INITIALIZE_REQUIRED_ENTRIES_BODY_RE = re.compile(
    r"void\s+InitializeRequiredEntries\s*\(\s*\)\s*(?:const\s*)?\{"
)
INITIALIZE_REQUIRED_ENTRIES_CONST_RE = re.compile(
    r"void\s+InitializeRequiredEntries\s*\(\s*\)\s*const\b"
)
OPEN_COCKPIT_WIND_LOOP_DECL_RE = INITIALIZE_REQUIRED_ENTRIES_RE
OPEN_COCKPIT_WIND_LOOP_INIT_RE = INITIALIZE_REQUIRED_ENTRIES_BODY_RE
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_GET_RESOLVED_ROUTE_COUNT,
        SIBLING_ENSURE_DEFAULT_ENTRIES,
        SIBLING_EVALUATE_READINESS,
        SIBLING_CONFIGURE_ROUTING,
        SIBLING_GET_UNBOUND_CATEGORIES,
        SIBLING_ENTRIES,
        SIBLING_ROUTING,
        SIBLING_FIND_ENTRY,
        SIBLING_GET_REQUIRED_CATEGORIES,
        SIBLING_GET_CATEGORY_DISPLAY_NAME,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `void InitializeRequiredEntries();`.
    # Do not accept const, FName / float / int32 /
    # FLinearColor / bool return types, an inline body,
    # BlueprintPure wraps, leftover analog
    # GetResolvedProductionLoopRouteCount, leftover analog
    # AreResolvedProductionLoopRoutesComplete, leftover analog
    # GetLampInstanceCount, leftover FName WeatherIdentity,
    # leftover GlobalVoiceLimit = 24, leftover
    # TObjectPtr ProductionBank / ProductionBankAsset, leftover
    ...
    if INITIALIZE_REQUIRED_ENTRIES_WRONG_TYPE_RE.search(compact):
        return False
    if INITIALIZE_REQUIRED_ENTRIES_BODY_RE.search(compact):
        return False
    if INITIALIZE_REQUIRED_ENTRIES_RE.search(compact) is None:
        return False
    if INITIALIZE_REQUIRED_ENTRIES_CONST_RE.search(compact):
        return False
...
def public_section(header: str) -> str:
    ...
    event = re.search(r"\bEntries\b", section)
    if event is not None:
        uprop = section.rfind("UPROPERTY", 0, event.start())
        if uprop != -1:
            section = section[:uprop]
        else:
            section = section[: event.start()]
    if has_identifier(section, SIBLING_ENTRIES):
        raise AssertionError(
            f"{CLASS_NAME} parse window includes {SIBLING_ENTRIES}"
        )
    return section
...
def field_identifier(field_text: str) -> str:
    ...
    match = re.search(
        ...
        r"void|float|int32|bool|uint8|FName|FString|FVector|FRotator|FText|"
        r"ESkyguard\w+)"
        r"\s+(\w+)\b",
        compact,
    )
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident in (
            ...
            break
        if ident != "InitializeRequiredEntries":
            cursor = semi + 1
            continue
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+InitializeRequiredEntries\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void InitializeRequiredEntries "
...
class AudioProductionBankInitializeRequiredEntriesDeclContractTests(unittest.TestCase):
    def test_audio_production_bank_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "InitializeRequiredEntries"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENSURE_DEFAULT_ENTRIES), body)
        self.assertFalse(has_identifier(section, SIBLING_ENSURE_DEFAULT_ENTRIES), section)
        self.assertTrue(has_identifier(body, SIBLING_EVALUATE_READINESS), body)
        self.assertFalse(has_identifier(section, SIBLING_EVALUATE_READINESS), section)
        self.assertTrue(has_identifier(body, SIBLING_CONFIGURE_ROUTING), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CONFIGURE_ROUTING),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_GET_UNBOUND_CATEGORIES), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_UNBOUND_CATEGORIES), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_FIND_ENTRY), body)
        self.assertFalse(has_identifier(section, SIBLING_FIND_ENTRY), section)
        self.assertTrue(has_identifier(body, SIBLING_GET_REQUIRED_CATEGORIES), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_REQUIRED_CATEGORIES), section)
        self.assertTrue(has_identifier(body, SIBLING_GET_CATEGORY_DISPLAY_NAME), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_CATEGORY_DISPLAY_NAME), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "InitializeRequiredEntries"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("InitializeRequiredEntries", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_initialize_required_entries_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", section)
        self.assertIn('Category="Skyguard|Audio|Production"', section)
        ...
        self.assertIn("BlueprintCallable", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("BlueprintPure", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertIn('Category="Skyguard|Audio|Production"', specifiers)
        self.assertNotIn("BlueprintPure", specifiers)
    ...
    def test_const_or_wrong_return_fails_closed(self) -> None:
        bare = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertIn(
            collapsed("void InitializeRequiredEntries();"),
            compact_origin,
        )
        self.assertNotIn(
            collapsed("void InitializeRequiredEntries() const;"),
            compact_origin,
        )
    ...
    def test_initialize_required_entries_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("void InitializeRequiredEntries"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GET_RESOLVED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_COMPLETE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PRODUCTION_BANK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PRODUCTION_BANK_ASSET)
        ...
        self.assertNotIn("ProductionBankAsset", LOCKED_DECL)
        ...
        self.assertNotIn("LockWindowAttackIntervalSeconds", LOCKED_DECL)
        self.assertNotIn("ApproachSpeedCmPerSecond", LOCKED_DECL)
        self.assertNotIn("CriticalSpeedCmPerSecond", LOCKED_DECL)
        self.assertNotIn("MaxTelegraphsPerEncounter", LOCKED_DECL)
        self.assertNotIn("BlueprintPure", UPROPERTY_EDIT_READWRITE)
        self.assertIn("BlueprintCallable", UPROPERTY_EDIT_READWRITE)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GetResolvedProductionLoopRouteCounts() const;\n",
            "\tfloat GetResolvedProductionLoopRouteCount() const;\n",
            "\tFName GetResolvedProductionLoopRouteCount() const;\n",
            "\tint32 GetResolvedProductionLoopRouteCount() const { return " + forty + "; }\n",
            "\tint32 GetResolvedProductionLoopRouteCount() const { return " + eighty + "; }\n",
            f"\t{TARGET_WRONG_LAMP}\n",
            f"\t{TARGET_WRONG_PEAK}\n",
    ...
    def test_missing_category_or_blueprint_callable_fails_closed(self) -> None:
        no_category = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertIn('Category="Skyguard|Audio|Production"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Production"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Production"', theater_specs)
        self.assertIn('Category="Skyguard|Audio|Production"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Production"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Audio|Production"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Audio|Production"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Production"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertIn("BlueprintPure", instance_specs)
        self.assertNotIn("BlueprintCallable", instance_specs)
        self.assertIn("BlueprintCallable", origin)
        self.assertNotIn("BlueprintPure", origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Production"', audio_only_specs)
        self.assertIn('Category="Skyguard|Audio|Production"', origin)
        leftover_callable = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{UFUNCTION_LEFTOVER_PURE}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        ...
        self.assertIn("BlueprintPure", callable_specs)
        self.assertNotIn("BlueprintCallable", callable_specs)
        self.assertIn("BlueprintCallable", origin)
        self.assertNotIn("BlueprintPure", origin)
    ...
    def test_does_not_claim_sibling_ufunctions_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "InitializeRequiredEntries"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_ENSURE_DEFAULT_ENTRIES))
        self.assertFalse(has_identifier(section, SIBLING_ENSURE_DEFAULT_ENTRIES))
        self.assertTrue(has_identifier(leaked, SIBLING_EVALUATE_READINESS))
        self.assertFalse(has_identifier(section, SIBLING_EVALUATE_READINESS))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "InitializeRequiredEntries"))
        self.assertTrue(has_identifier(section, "InitializeRequiredEntries"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_ENSURE_DEFAULT_ENTRIES))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_CONFIGURE_ROUTING))
        self.assertFalse(has_identifier(section, SIBLING_CONFIGURE_ROUTING))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_UNBOUND_CATEGORIES))
        self.assertFalse(has_identifier(section, SIBLING_GET_UNBOUND_CATEGORIES))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardAudioProductionBank.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardAudioProductionBank.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio production bank InitializeRequiredEntries decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_production_bank_initialize_required_entries_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        self.assertNotEqual(LEFTOVER_ASSEMBLY_REVISION, THIS_SCRIPT)
        if LEFTOVER_ASSEMBLY_REVISION in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_ASSEMBLY_REVISION, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_READINESS_LICENSED_VEGETATION, THIS_SCRIPT)
        if LEFTOVER_READINESS_LICENSED_VEGETATION in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_READINESS_LICENSED_VEGETATION, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_ROUTE_POINTS, THIS_SCRIPT)
        if LEFTOVER_ROUTE_POINTS in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_ROUTE_POINTS, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_ALLOW_AUTHORED_PCG, THIS_SCRIPT)
        if LEFTOVER_ALLOW_AUTHORED_PCG in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_ALLOW_AUTHORED_PCG, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_USE_AUTHORED_LANDSCAPE, THIS_SCRIPT)
        if LEFTOVER_USE_AUTHORED_LANDSCAPE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_USE_AUTHORED_LANDSCAPE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_ENABLE_COASTAL_HAZE, THIS_SCRIPT)
        if LEFTOVER_ENABLE_COASTAL_HAZE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_ENABLE_COASTAL_HAZE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_LOOK_YAW_LIMIT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_LOOK_YAW_LIMIT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_LOOK_YAW_LIMIT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_THEATER_WEATHER_IDENTITY, THIS_SCRIPT)
        if LEFTOVER_THEATER_WEATHER_IDENTITY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_THEATER_WEATHER_IDENTITY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_SILHOUETTE_MESH, THIS_SCRIPT)
        if LEFTOVER_APACHE_SILHOUETTE_MESH in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_SILHOUETTE_MESH, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_COCKPIT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_COCKPIT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_COCKPIT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_STATION, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_STATION in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_STATION, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_SEAT_BACK, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_SEAT_BACK in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_SEAT_BACK, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_SEAT_PAN, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_SEAT_PAN in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_SEAT_PAN, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_KNEE_LEFT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_KNEE_LEFT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_KNEE_LEFT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_KNEE_RIGHT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_KNEE_RIGHT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_KNEE_RIGHT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_CANOPY_BOW, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_CANOPY_BOW in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_CANOPY_BOW, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_DASH, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_DASH in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_DASH, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_TEDAC, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_TEDAC in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_TEDAC, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_MPD_LEFT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_MPD_LEFT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_MPD_LEFT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_MPD_RIGHT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_MPD_RIGHT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_MPD_RIGHT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_EUFD, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_EUFD in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_EUFD, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_TEDAC_BEZEL, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_TEDAC_BEZEL in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_TEDAC_BEZEL, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_RETICLE_H, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_RETICLE_H in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_RETICLE_H, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_RETICLE_V, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_RETICLE_V in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_RETICLE_V, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_GRIP_LEFT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_GRIP_LEFT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_GRIP_LEFT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_GRIP_RIGHT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_GRIP_RIGHT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_GRIP_RIGHT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_CONSOLE_RIGHT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_CONSOLE_RIGHT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_CONSOLE_RIGHT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_RAIL_LEFT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_RAIL_LEFT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_RAIL_LEFT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_RAIL_RIGHT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_RAIL_RIGHT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_RAIL_RIGHT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_TEDAC_TEXT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_TEDAC_TEXT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_TEDAC_TEXT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_MPD_LEFT_TEXT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_MPD_LEFT_TEXT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_MPD_LEFT_TEXT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_MPD_RIGHT_TEXT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_MPD_RIGHT_TEXT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_MPD_RIGHT_TEXT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_CPG_EUFD_TEXT, THIS_SCRIPT)
        if LEFTOVER_GUNNER_CPG_EUFD_TEXT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_CPG_EUFD_TEXT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_LOADOUT_CANNON_MAGAZINE, THIS_SCRIPT)
        if LEFTOVER_LOADOUT_CANNON_MAGAZINE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_LOADOUT_CANNON_MAGAZINE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_LOADOUT_ROCKET_MAGAZINE, THIS_SCRIPT)
        if LEFTOVER_LOADOUT_ROCKET_MAGAZINE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_LOADOUT_ROCKET_MAGAZINE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_LOADOUT_GUIDED_MAGAZINE, THIS_SCRIPT)
        if LEFTOVER_LOADOUT_GUIDED_MAGAZINE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_LOADOUT_GUIDED_MAGAZINE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNSHIP_WEAPON_STATIONS, THIS_SCRIPT)
        if LEFTOVER_GUNSHIP_WEAPON_STATIONS in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNSHIP_WEAPON_STATIONS, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_CANOPY, THIS_SCRIPT)
        if LEFTOVER_APACHE_CANOPY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_CANOPY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_PILOT_CANOPY, THIS_SCRIPT)
        if LEFTOVER_APACHE_PILOT_CANOPY in LOCKED_SCRIPTS:
        ...
        self.assertNotEqual(LEFTOVER_APACHE_HULL_COLLIDER, THIS_SCRIPT)
        if LEFTOVER_APACHE_HULL_COLLIDER in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_HULL_COLLIDER, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_CURRENT_INTEGRITY, THIS_SCRIPT)
        if LEFTOVER_APACHE_CURRENT_INTEGRITY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_CURRENT_INTEGRITY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_MAX_INTEGRITY, THIS_SCRIPT)
        if LEFTOVER_APACHE_MAX_INTEGRITY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_MAX_INTEGRITY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_EMPTY, THIS_SCRIPT)
        if LEFTOVER_APACHE_EMPTY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_EMPTY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_EMPTY_CONTRACT, THIS_SCRIPT)
        if LEFTOVER_APACHE_EMPTY_CONTRACT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_EMPTY_CONTRACT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_OWN_SHIP, THIS_SCRIPT)
        if LEFTOVER_APACHE_OWN_SHIP in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_OWN_SHIP, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_CPG_FEEL, THIS_SCRIPT)
        if LEFTOVER_APACHE_CPG_FEEL in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_CPG_FEEL, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_APACHE_MOUNT, THIS_SCRIPT)
        if LEFTOVER_APACHE_MOUNT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_APACHE_MOUNT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_LOADOUT_HULL_INTEGRITY, THIS_SCRIPT)
        if LEFTOVER_LOADOUT_HULL_INTEGRITY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_LOADOUT_HULL_INTEGRITY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_CAMPAIGN_ROSTER_LOOKUP, THIS_SCRIPT)
        if LEFTOVER_CAMPAIGN_ROSTER_LOOKUP in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_CAMPAIGN_ROSTER_LOOKUP, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_SORTIE_DIRECTOR_EMPTY, THIS_SCRIPT)
        if LEFTOVER_SORTIE_DIRECTOR_EMPTY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_SORTIE_DIRECTOR_EMPTY, LOCKED_SCRIPTS)
        for leftover in leftover_apache_ufunction_scripts():
            self.assertNotEqual(leftover, THIS_SCRIPT)
            if leftover in LOCKED_SCRIPTS:
        ...
        self.assertNotEqual(LEFTOVER_MISSION01_LICENSED_VEGETATION_LIBRARY, THIS_SCRIPT)
        if LEFTOVER_MISSION01_LICENSED_VEGETATION_LIBRARY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_MISSION01_LICENSED_VEGETATION_LIBRARY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_ARCADE_LOOK_ENABLED, THIS_SCRIPT)
        if LEFTOVER_ARCADE_LOOK_ENABLED in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_ARCADE_LOOK_ENABLED, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_CAMPAIGN_NIGHT_IDENTITY, THIS_SCRIPT)
        if LEFTOVER_CAMPAIGN_NIGHT_IDENTITY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_CAMPAIGN_NIGHT_IDENTITY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY, THIS_SCRIPT)
        if LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_HARNESS_FAIL_CLOSED, THIS_SCRIPT)
        if LEFTOVER_AUDIO_HARNESS_FAIL_CLOSED in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_HARNESS_FAIL_CLOSED, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS, THIS_SCRIPT)
        if LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIBLE_ACCEPTANCE_RECEIPT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_COASTAL_PLACEMENT_SEED, THIS_SCRIPT)
        if LEFTOVER_COASTAL_PLACEMENT_SEED in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_COASTAL_PLACEMENT_SEED, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_MINIMUM_MEASURED_SAMPLES, THIS_SCRIPT)
        if LEFTOVER_MINIMUM_MEASURED_SAMPLES in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_MINIMUM_MEASURED_SAMPLES, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_MAXIMUM_ALLOWED_VOICES, THIS_SCRIPT)
        if LEFTOVER_MAXIMUM_ALLOWED_VOICES in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_MAXIMUM_ALLOWED_VOICES, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_GUNNER_BOOM, THIS_SCRIPT)
        if LEFTOVER_GUNNER_BOOM in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_GUNNER_BOOM, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_ENGINE_CRUISE, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_ENGINE_CRUISE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_ENGINE_CRUISE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_ENGINE_POWER, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_ENGINE_POWER in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_ENGINE_POWER, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_PROPELLER, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_PROPELLER in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_PROPELLER, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_ENGINE_IDLE, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_ENGINE_IDLE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_ENGINE_IDLE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_OPEN_COCKPIT, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_OPEN_COCKPIT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_OPEN_COCKPIT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_THEATER_KIT_GET_LAMP, THIS_SCRIPT)
        if LEFTOVER_THEATER_KIT_GET_LAMP in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_THEATER_KIT_GET_LAMP, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_VOICE_LIMIT, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_VOICE_LIMIT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_VOICE_LIMIT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_EVENTS, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_EVENTS in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_EVENTS, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_COCKPIT_EXT, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_COCKPIT_EXT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_COCKPIT_EXT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_COCKPIT_LP, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_COCKPIT_LP in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_COCKPIT_LP, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_PRODUCTION_BANK, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_PRODUCTION_BANK in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_PRODUCTION_BANK, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_DIRECTOR_PRODUCTION_BANK_ASSET, THIS_SCRIPT)
        if LEFTOVER_AUDIO_DIRECTOR_PRODUCTION_BANK_ASSET in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_DIRECTOR_PRODUCTION_BANK_ASSET, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_ROUTING_COCKPIT_EXT, THIS_SCRIPT)
        if LEFTOVER_ROUTING_COCKPIT_EXT in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_ROUTING_COCKPIT_EXT, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_ROUTING_COCKPIT_LP, THIS_SCRIPT)
        if LEFTOVER_ROUTING_COCKPIT_LP in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_ROUTING_COCKPIT_LP, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_AUDIO_PRODUCTION_BANK_EMPTY, THIS_SCRIPT)
        if LEFTOVER_AUDIO_PRODUCTION_BANK_EMPTY in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_AUDIO_PRODUCTION_BANK_EMPTY, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_MISSION01_AUDIO_DIRECTOR, THIS_SCRIPT)
        if LEFTOVER_MISSION01_AUDIO_DIRECTOR in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_MISSION01_AUDIO_DIRECTOR, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_PROCEDURAL_BANK_SAMPLE_RATE, THIS_SCRIPT)
        if LEFTOVER_PROCEDURAL_BANK_SAMPLE_RATE in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_PROCEDURAL_BANK_SAMPLE_RATE, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_PROCEDURAL_BANK_BYTE_BUDGET, THIS_SCRIPT)
        if LEFTOVER_PROCEDURAL_BANK_BYTE_BUDGET in LOCKED_SCRIPTS:
            self.assertIn(LEFTOVER_PROCEDURAL_BANK_BYTE_BUDGET, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_PROCEDURAL_BANK_AUDITION, THIS_SCRIPT)
        if LEFTOVER_PROCEDURAL_BANK_AUDITION in LOCKED_SCRIPTS:
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("EnsureDefaultEntries", locked_only)
