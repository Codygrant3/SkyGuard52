...
# then `float ApproachAttackIntervalSeconds = 6.5f;` (HAS initializer).
...
# BlueprintReadWrite IS present. ClampMin IS `"0.2"`. Default IS `= 6.5f`.
# Clone wrap ClampMin WAS `"0.0"` for leftover Amplitude. THIS slot
# ClampMin IS `"0.2"` (NO spaces around =). Fail-closed if wrap still
# has ClampMin="0.0" / ClampMin="100.0" / ClampMin="1.0". 6.5f is NOT
# Harbor 40 or 80.
...
# Do NOT claim Telegraph siblings LockWindowAttackIntervalSeconds = 8.f /
...
# `"1.0"`, if the default is not `= 6.5f`, if Category is
...
# wrapping `float IngressSwayAmplitudeCm = 320.f;`
...
# LOCKED_DECL is `float ApproachAttackIntervalSeconds = 6.5f;`
...
# LockWindowAttackIntervalSeconds / CriticalAttackIntervalSeconds /
...
# / `"100.0"` / `"1.0"`, or default is not 6.5f.
...
TARGET = "float ApproachAttackIntervalSeconds = 6.5f;"
TARGET_WRONG_BARE = "float ApproachAttackIntervalSeconds;"
TARGET_WRONG_INIT = "float ApproachAttackIntervalSeconds = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float ApproachAttackIntervalSeconds = true;"
TARGET_WRONG_ZERO = "float ApproachAttackIntervalSeconds = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> ApproachAttackIntervalSeconds;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> ApproachAttackIntervalSeconds;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> ApproachAttackIntervalSeconds;"
...
STOP_BEFORE_SKYLINE_STYLE = "LockWindowAttackIntervalSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_approach_attack_interval"
...
APPROACH_ATTACK_INTERVAL_DECL_RE = re.compile(
    r"float\s+ApproachAttackIntervalSeconds\s*=\s*6\.5f\s*;"
)
APPROACH_ATTACK_INTERVAL_BARE_RE = re.compile(
    r"float\s+ApproachAttackIntervalSeconds\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float ApproachAttackIntervalSeconds = 6.5f;`.
    ...
    # ApproachAttackIntervalSeconds without the 6.5f initializer.
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "ApproachAttackIntervalSeconds":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+ApproachAttackIntervalSeconds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float ApproachAttackIntervalSeconds "
...
class PathfinderEncounterApproachAttackIntervalFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ApproachAttackIntervalSeconds"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ApproachAttackIntervalSeconds"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ApproachAttackIntervalSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_approach_attack_interval_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float ApproachAttackIntervalSeconds;",
            compact_origin.split("ApproachAttackIntervalSeconds = 6.5f;")[0]
            + compact_origin.split("ApproachAttackIntervalSeconds = 6.5f;")[-1]
            if "ApproachAttackIntervalSeconds = 6.5f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_approach_attack_interval_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float ApproachAttackIntervalSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("6.5f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat ApproachAttackIntervalSecondss = 6.5f;\n",
            "\tint32 ApproachAttackIntervalSeconds = 320;\n",
            "\tFName ApproachAttackIntervalSeconds = NAME_None;\n",
            "\tfloat ApproachAttackIntervalSeconds = " + forty + ";\n",
            "\tfloat ApproachAttackIntervalSeconds = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ApproachAttackIntervalSeconds"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "ApproachAttackIntervalSeconds"))
        self.assertTrue(has_identifier(section, "ApproachAttackIntervalSeconds"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_approach_attack_interval_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
