...
# INDEX_NONE or lock RunwayBreakerSpawnRotation
# construction in the .cpp. This is leftover-safe
# Mission06 RunwayBreakerSpawnRotation FIELD
...
# This contract is RunwayBreakerSpawnRotation
...
# Skyguard|Mission06 / FRotator(0.f, 180.f, 0.f),
...
# Lock RunwayBreakerSpawnRotation only. Do not lock
...
RUNWAY_BREAKER_SPAWN_ROTATION_FIELD = (
    "FRotator RunwayBreakerSpawnRotation = "
    "FRotator(0.f, 180.f, 0.f);"
)
...
LOCKED_DECL = RUNWAY_BREAKER_SPAWN_ROTATION_FIELD
...
# an isolated Python RunwayBreakerSpawnRotation
...
def standalone_harbor_interval_present(text: str, token: str) -> bool:
    ...
    # Mission08-only values. origin/main Mission06 yaw 1-8-0
    # must not count as a Harbor 80 interval.
...
class Mission06RunwayBreakerSpawnRotationFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RunwayBreakerSpawnRotation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_runway_breaker_spawn_rotation_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_runway_breaker_spawn_rotation_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FRotator RunwayBreakerSpawnRotation"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("RunwayBreakerSpawnRotation", LOCKED_DECL)
        self.assertIn("= FRotator(0.f, 180.f, 0.f)", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tFRotator\n"
            "\tRunwayBreakerSpawnRotation = FRotator(0.f, 180.f, 0.f);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("RunwayBreakerSpawnRotation", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 RunwayBreakerSpawnRotation field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission06 RunwayBreakerSpawnRotation field contains "
    ...
    def test_contract_is_runway_breaker_spawn_rotation_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        for token in leftover_short_roster_values():
            ...
            self.assertNotEqual(token, "RunwayBreakerSpawnRotation")
    ...
    def test_sibling_director_fields_do_not_satisfy_runway_breaker_spawn_rotation(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        assigned = "\tFRotator RunwayBreakerSpawnRotation = FRotator(0.f, 0.f, 0.f);\n"
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_airfield_target_runtime_does_not_satisfy_runway_breaker_spawn_rotation(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
    ...
    def test_payload_window_runtime_does_not_satisfy_runway_breaker_spawn_rotation(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
