# THIS IS leftover-safe USkyguardAudioDirectorComponent
# EnginePowerLoop.
...
# then BARE `TSoftObjectPtr<USoundBase> EnginePowerLoop;`
# (no initializer).
...
# drop to VisibleAnywhere / BlueprintReadOnly / EditInstanceOnly
...
# or Category="Skyguard" without |Audio|Engine.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `USkyguardAudioDirectorComponent` public
# UPROPERTY fields ONLY after `class USkyguardAudioDirectorComponent`.
# Start at `public:` after GENERATED_BODY. Stop BEFORE `private:`.
# Stop BEFORE sibling PropellerLoop as a claimed slot.
# Do NOT claim EngineIdleLoop, EngineCruiseLoop, PropellerLoop,
# OpenCockpitWindLoop, GlobalVoiceLimit, EventDefinitions,
# CockpitExteriorAttenuation, CockpitLowPassHz, ProductionBank,
# ProductionBankAsset.
# Skip leftover analog PropellerLoop (Ya + k / PropSpinner).
...
# coastal unique actor UPROPERTY, leftover analog
# Mission01 unique actor bools.
...
# bUseAuthoredLandscapeSurface as this slot.
# Do NOT claim leftover FName WeatherIdentity TheaterKit
# (different class, Category Theater, no initializer).
# Do NOT claim leftover FName AssemblyRevision MissionMap
# (EditInstanceOnly, Category MissionMap, FName initializer).
# Do NOT claim leftover TArray RoutePoints.
...
# Category="Skyguard" wrapping
# `TObjectPtr<USpringArmComponent> Boom;`
# — RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Audio|Engine"
# (NO spaces around =). This slot has NO initializer.
# LOCKED_DECL is `TSoftObjectPtr<USoundBase> EnginePowerLoop;`
# NOT `FName WeatherIdentity;` and NOT `160.f` and NOT
# leftover Boom / MinimumMeasuredSamples / SampleRate /
# GeneratedByteBudget / VegetationStartCullDistanceCm /
# EngineIdleLoop / EngineCruiseLoop / PropellerLoop /
...
# Fail-closed if decl gains `= nullptr` / `= false` / `= true`.
# Public soft-ptr fail-closed if LOCKED_DECL is leftover
...
TARGET = "TSoftObjectPtr<USoundBase> EnginePowerLoop;"
TARGET_WRONG_BARE = "TObjectPtr<USoundBase> EnginePowerLoop;"
TARGET_WRONG_INIT = "TSoftObjectPtr<USoundBase> EnginePowerLoop = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "bool bEnabled = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundBase> EnginePowerLoop = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> EnginePowerLoop;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> EnginePowerLoop;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> EnginePowerLoop;"
...
STOP_BEFORE_SKYLINE_STYLE = "PropellerLoop"
...
TARGET_WRONG_ENGINE_CRUISE = "TSoftObjectPtr<USoundBase> EngineCruiseLoop;"
...
TARGET_WRONG_VOICE_LIMIT = "int32 GlobalVoiceLimit = 24;"
TARGET_WRONG_EVENTS = "TArray<FSkyguardAudioEventDefinition> EventDefinitions;"
TARGET_WRONG_BANK = "TObjectPtr<USkyguardAudioProductionBank> ProductionBank;"
TARGET_WRONG_BANK_ASSET = (
...
TARGET_WRONG_LOW_PASS = "float CockpitLowPassHz = 7200.f;"
TARGET_WRONG_BOOM = "TObjectPtr<USpringArmComponent> Boom;"
TARGET_WRONG_VEG_CULL = "float VegetationStartCullDistanceCm;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundBase> EnginePowerLoop;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundWave> EnginePowerLoop;"
TARGET_WRONG_FALSE_INIT = "TSoftObjectPtr<USoundBase> EnginePowerLoop = false;"
TARGET_WRONG_TRUE_INIT = "TSoftObjectPtr<USoundBase> EnginePowerLoop = true;"
...
LEFTOVER_PRODUCTION_AUDIO_ROUTING_ATTEN = (
    "Scripts/tests/test_production_audio_routing_cockpit_exterior_attenuation"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ROUTING_LOW_PASS = (
...
LEFTOVER_COASTAL_VEG_START_CULL = (
...
ENGINE_POWER_LOOP_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EnginePowerLoop\s*;"
)
ENGINE_POWER_LOOP_INIT_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EnginePowerLoop\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover Mission01 `bool bLicensedVegetationLibraryApproved
    # = false;`, leftover ArcadeLook `bool bEnabled = true;`,
    # leftover CampaignMissionSpec bNightIdentity, leftover
    # coastal PlacementSeed, leftover analog
    # `int32 MinimumMeasuredSamples = 600;` /
    # `int32 MaximumAllowedVoices = 48;`, leftover sibling
    # SampleRate / GeneratedByteBudget, leftover `= 160.f`,
    ...
    # EnginePowerLoop without true.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ENGINE_POWER_LOOP_DECL_RE.search(compact) is None:
        return False
    if ENGINE_POWER_LOOP_INIT_RE.search(compact):
        return False
...
def field_identifier(field_text: str) -> str:
    ...
    match = re.search(
        r"(?:TSoftObjectPtr<[^>]+>|TObjectPtr<[^>]+>|TSubclassOf<[^>]+>|TArray<[^>]+>|float|int32|bool|"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr<USoundBase>\s+EnginePowerLoop\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundBase> EnginePowerLoop "
...
class AudioDirectorEnginePowerLoopFieldDeclContractTests(unittest.TestCase):
    def test_audio_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EnginePowerLoop"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_ENGINE_CRUISE_LOOP), body)
        self.assertFalse(
            has_identifier(section, SIBLING_ENGINE_CRUISE_LOOP),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_WAVES), body)
        self.assertFalse(has_identifier(section, SIBLING_WAVES), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EnginePowerLoop"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("EnginePowerLoop", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_engine_power_loop_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_engine_power_loop_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSoftObjectPtr<USoundBase> EnginePowerLoop"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENGINE_CRUISE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PROPELLER)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WIND)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VOICE_LIMIT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_EVENTS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BANK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BANK_ASSET)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ATTENUATION)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOW_PASS)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BOOM)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VEG_CULL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_OBJECT_PTR)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WAVE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FALSE_INIT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TRUE_INIT)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTSoftObjectPtr<USoundBase> EnginePowerLoops;\n",
            "\tint32 EnginePowerLoop = 1;\n",
            "\tFName EnginePowerLoop = NAME_None;\n",
            "\tTSoftObjectPtr<USoundBase> EnginePowerLoop = " + forty + ";\n",
            "\tTSoftObjectPtr<USoundBase> EnginePowerLoop = " + eighty + ";\n",
            ...
            f"\t{TARGET_WRONG_ENGINE_CRUISE}\n",
            f"\t{TARGET_WRONG_PROPELLER}\n",
            f"\t{TARGET_WRONG_WIND}\n",
            f"\t{TARGET_WRONG_VOICE_LIMIT}\n",
            f"\t{TARGET_WRONG_EVENTS}\n",
            f"\t{TARGET_WRONG_BANK}\n",
            f"\t{TARGET_WRONG_BANK_ASSET}\n",
            f"\t{TARGET_WRONG_ATTENUATION}\n",
            f"\t{TARGET_WRONG_LOW_PASS}\n",
            f"\t{TARGET_WRONG_BOOM}\n",
            f"\t{TARGET_WRONG_VEG_CULL}\n",
            f"\t{TARGET_WRONG_OBJECT_PTR}\n",
            f"\t{TARGET_WRONG_WAVE}\n",
            f"\t{TARGET_WRONG_FALSE_INIT}\n",
            f"\t{TARGET_WRONG_TRUE_INIT}\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Audio|Engine"', theater_specs)
        self.assertIn('Category="Skyguard|Audio|Engine"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Audio|Engine"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Audio|Engine"', perf_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_skyline_style_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "EnginePowerLoop"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_CRUISE_LOOP))
        self.assertFalse(has_identifier(section, SIBLING_ENGINE_CRUISE_LOOP))
        self.assertTrue(has_identifier(leaked, SIBLING_WAVES))
        self.assertFalse(has_identifier(section, SIBLING_WAVES))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "EnginePowerLoop"))
        self.assertTrue(has_identifier(section, "EnginePowerLoop"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_CRUISE_LOOP))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio director engine power loop field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_director_engine_power_loop_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ROUTING_ATTEN, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ROUTING_LOW_PASS, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_COASTAL_VEG_START_CULL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_HARNESS_FAIL_CLOSED, LOCKED_SCRIPTS)
