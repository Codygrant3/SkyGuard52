...
# THIS IS leftover-safe Mission09 PoolRuntime
# PeakActive FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly int32 = 0;
# one-line / split-line UPROPERTY wraps; origin/main
# has NO Category). PARSE
# FSkyguardMission09PoolRuntime STRUCT
# public section only (after struct
# FSkyguardMission09PoolRuntime, before
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission09 PoolRuntime
# PeakActive. THIS IS NOT leftover Mission08
...
# THIS IS NOT leftover Mission09 PoolRuntime
# Available #1119 / leftover Mission09
# PoolRuntime Active #1121 / leftover
# Mission09 max-simultaneous-explosions.
...
# INDEX_NONE or lock PeakActive
# construction in the .cpp. This is leftover-safe
# Mission09 PoolRuntime PeakActive
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 default 0,
# one-line / split-line UPROPERTY wraps) on
# FSkyguardMission09PoolRuntime. It is
# leftover-safe Mission09 PoolRuntime
# PeakActive FIELD (declaration-only). It is
...
# struct section of
# FSkyguardMission09PoolRuntime only
# between struct FSkyguardMission09PoolRuntime
...
# This PoolRuntime field sits after Active in the public section.
# Skip leftover b + Ya + kRuntimeReady forever (it sits
# after bMapAssemblyReady and before bGunnerReady). This contract
# is PeakActive after Active and before Recycled. Skip leftover
...
class Mission09PoolRuntimePeakActiveFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("PeakActive", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_peak_active_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_peak_active_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 PeakActive"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("PeakActive", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("PeakActive", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 PeakActive"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("PeakActive", locked_only)
    ...
    def test_contract_is_peak_active_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_peak_active(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission09_pool_runtime_peak_active"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
