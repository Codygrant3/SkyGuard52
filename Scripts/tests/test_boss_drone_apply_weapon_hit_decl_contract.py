...
# or lock the ApplyWeaponHit body.
# origin/main is the live split-line form
# with no inline body
# (`bool ApplyWeaponHit(` plus
...
# split-line wraps. Do not invent the
# simplified signature
# `bool ApplyWeaponHit(float Damage, FVector
# HitLocation, FVector HitDirection);`.
# Nearby origin/main
...
# isolated-test drafts #107–#464 including leftover
...
# Python ApplyWeaponHit declaration
...
# leftover isolated-test drafts #107–#464,
...
# locked ApplyWeaponHit declaration; do not add a
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBossDroneBase::ApplyWeaponHit",
    "SkyguardBossDroneBase.cpp",
    "SetBossPhase",
...
class BossDroneApplyWeaponHitDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, APPLY_WEAPON_HIT),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, APPLY_WEAPON_HIT)
    ...
    def test_missing_apply_weapon_hit_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, APPLY_WEAPON_HIT)
        self.assertIn("ApplyWeaponHit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, APPLY_WEAPON_HIT)
        self.assertIn("ApplyWeaponHit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", APPLY_WEAPON_HIT)
        self.assertNotIn("UFUNCTION", APPLY_WEAPON_HIT)
        self.assertNotIn("Category", APPLY_WEAPON_HIT)
        self.assertNotIn("BlueprintCallable", APPLY_WEAPON_HIT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, APPLY_WEAPON_HIT)
        self.assertIn("ApplyWeaponHit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        simplified = f"\t{leftover_simplified_apply_weapon_hit()}\n"
        missing_hit_component = (
            "\tbool ApplyWeaponHit("
            "ESkyguardBossWeapon Weapon, "
            "float Damage, FVector HitLocation, "
            "FVector HitDirection);\n"
        )
        missing_weapon = (
            "\tbool ApplyWeaponHit("
        ...
        missing_parens = "\tbool ApplyWeaponHit;\n"
        wrong_return_void = (
            "\tvoid ApplyWeaponHit("
            "UPrimitiveComponent* HitComponent, "
            "ESkyguardBossWeapon Weapon, "
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, APPLY_WEAPON_HIT)
            self.assertIn("ApplyWeaponHit", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_apply_weapon_hit_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, APPLY_WEAPON_HIT),
            1,
        )
        self.assertTrue(
            APPLY_WEAPON_HIT.startswith("bool "),
            APPLY_WEAPON_HIT,
        )
        self.assertTrue(
            APPLY_WEAPON_HIT.endswith(";"),
            APPLY_WEAPON_HIT,
        )
        self.assertIn("ApplyWeaponHit(", APPLY_WEAPON_HIT)
        self.assertIn("UPrimitiveComponent*", APPLY_WEAPON_HIT)
        self.assertIn("HitComponent", APPLY_WEAPON_HIT)
        self.assertIn("ESkyguardBossWeapon", APPLY_WEAPON_HIT)
        self.assertIn("float Damage", APPLY_WEAPON_HIT)
        self.assertIn("FVector HitLocation", APPLY_WEAPON_HIT)
        self.assertIn("FVector HitDirection", APPLY_WEAPON_HIT)
        self.assertNotEqual(
            APPLY_WEAPON_HIT,
            leftover_simplified_apply_weapon_hit(),
        )
        self.assertNotIn(" const", APPLY_WEAPON_HIT)
        self.assertNotIn("INDEX_NONE", APPLY_WEAPON_HIT)
        self.assertNotIn("{", APPLY_WEAPON_HIT)
        self.assertNotIn("}", APPLY_WEAPON_HIT)
        self.assertNotIn("return ", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "DefeatDebrisComponents",
            APPLY_WEAPON_HIT,
        )
        self.assertNotIn("GetDefeatDebrisPieceCount", APPLY_WEAPON_HIT)
        self.assertNotIn("IssuePilotCommand", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            APPLY_WEAPON_HIT,
        )
        ...
        self.assertNotIn("GetBossPhase", APPLY_WEAPON_HIT)
        self.assertNotIn("GetTelemetry", APPLY_WEAPON_HIT)
        self.assertNotIn("GetChinMuzzleLocation", APPLY_WEAPON_HIT)
        self.assertNotIn("GetGunnerMount", APPLY_WEAPON_HIT)
        self.assertNotIn("FaceWorldLocation", APPLY_WEAPON_HIT)
        self.assertNotIn("WeakPoints", APPLY_WEAPON_HIT)
        self.assertNotIn("SetSearchlightTracked", APPLY_WEAPON_HIT)
        self.assertNotIn("SetContactIdentified", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            APPLY_WEAPON_HIT,
        )
        self.assertNotIn("SetLightningExposed", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            APPLY_WEAPON_HIT,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, APPLY_WEAPON_HIT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_one_line = (
            "public:\n"
            "\tbool ApplyWeaponHit(UPrimitiveComponent* "
            "HitComponent, ESkyguardBossWeapon Weapon, "
            "float Damage, FVector HitLocation, "
            "FVector HitDirection);\n"
            "private:\n"
            "};\n"
        ...
        wrap_params = (
            "public:\n"
            "\tbool ApplyWeaponHit(UPrimitiveComponent*\n"
            "\tHitComponent, ESkyguardBossWeapon\n"
            "\tWeapon, float Damage, FVector\n"
            "\tHitLocation, FVector HitDirection);\n"
            "};\n"
        )
        header_wrap_origin = (
        ...
        one_line = f"{{\npublic:\n\t{APPLY_WEAPON_HIT}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, APPLY_WEAPON_HIT)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, APPLY_WEAPON_HIT),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        live_inline = (
            ...
            "ESkyguardBossWeapon Weapon, "
            "float Damage, FVector HitLocation, "
            "FVector HitDirection) { return false; }\n"
            "};\n"
        )
        split_inline = (
            "public:\n"
        ...
        self.assertNotIn("}", APPLY_WEAPON_HIT)
        self.assertNotIn("return ", APPLY_WEAPON_HIT)
        self.assertNotIn("return false", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "DefeatDebrisComponents",
            APPLY_WEAPON_HIT,
        )
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{APPLY_WEAPON_HIT}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", APPLY_WEAPON_HIT)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", APPLY_WEAPON_HIT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_apply_weapon_hit_cpp_body(
        self,
    ) -> None:
        locked_only = f"{APPLY_WEAPON_HIT}\n"
        self.assertNotIn("{", APPLY_WEAPON_HIT)
        ...
        self.assertNotIn("return false", APPLY_WEAPON_HIT)
        self.assertNotIn("return true", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "DefeatDebrisComponents",
            APPLY_WEAPON_HIT,
        )
        self.assertNotIn("SetBossPhase", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            APPLY_WEAPON_HIT,
        )
        self.assertNotIn("RegisterDefeatDebris", APPLY_WEAPON_HIT)
        self.assertNotIn("CleanupDefeatDebris", APPLY_WEAPON_HIT)
        self.assertNotIn("HandleDefeated", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_get_defeat_debris_piece_count_sibling(self) -> None:
        ...
        for token in GET_DEFEAT_DEBRIS_PIECE_COUNT_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("GetDefeatDebrisPieceCount", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", APPLY_WEAPON_HIT)
        self.assertNotIn("PortGuidanceArray", APPLY_WEAPON_HIT)
        self.assertNotIn("CommandCore", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("SetSearchlightTracked", APPLY_WEAPON_HIT)
        self.assertNotIn("IsSearchlightTracked", APPLY_WEAPON_HIT)
        self.assertNotIn("ASkyguardBlackKiteBoss", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("SetContactIdentified", APPLY_WEAPON_HIT)
        self.assertNotIn("IsContactIdentified", APPLY_WEAPON_HIT)
        self.assertNotIn("OpenOrbitExposure", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            APPLY_WEAPON_HIT,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", APPLY_WEAPON_HIT)
        self.assertNotIn("ASkyguardIronRainBoss", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            APPLY_WEAPON_HIT,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("GetGunnerMount", APPLY_WEAPON_HIT)
        self.assertNotIn("GetChinTurret", APPLY_WEAPON_HIT)
        self.assertNotIn("GetSensorTurret", APPLY_WEAPON_HIT)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{APPLY_WEAPON_HIT}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, APPLY_WEAPON_HIT)
        self.assertNotIn("GetChinMuzzleLocation", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("ESkyguardApacheSystem", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("ASkyguardGunner", APPLY_WEAPON_HIT)
        self.assertNotIn("FillAndFinalize", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("SkyguardRadarNode", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            APPLY_WEAPON_HIT,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_WEAPON_HIT)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            APPLY_WEAPON_HIT,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("MinHeightFromOriginCm", APPLY_WEAPON_HIT)
        self.assertNotIn("MaxIntegrity", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            APPLY_WEAPON_HIT,
        )
        self.assertNotIn("SkyguardPathfinder", APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, APPLY_WEAPON_HIT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{APPLY_WEAPON_HIT}\n"
        self.assertEqual(
            require_declaration(locked_only, APPLY_WEAPON_HIT),
            APPLY_WEAPON_HIT,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone ApplyWeaponHit contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, APPLY_WEAPON_HIT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                APPLY_WEAPON_HIT.lower(),
                "boss drone ApplyWeaponHit contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, APPLY_WEAPON_HIT)

    def test_contract_is_apply_weapon_hit_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", APPLY_WEAPON_HIT)
        self.assertNotIn("{", APPLY_WEAPON_HIT)
        self.assertTrue(APPLY_WEAPON_HIT.startswith("bool "))
        self.assertTrue(APPLY_WEAPON_HIT.endswith(";"))
        self.assertIn("UPrimitiveComponent*", APPLY_WEAPON_HIT)
        self.assertIn("ESkyguardBossWeapon", APPLY_WEAPON_HIT)
        self.assertNotIn(" const", APPLY_WEAPON_HIT)
        self.assertIn(UFUNCTION_BOSS, section)
    ...
    def test_simplified_three_arg_signature_is_not_origin_main(self) -> None:
        simplified = leftover_simplified_apply_weapon_hit()
        self.assertNotEqual(simplified, APPLY_WEAPON_HIT)
        self.assertNotIn("UPrimitiveComponent", simplified)
        self.assertNotIn("ESkyguardBossWeapon", simplified)
        self.assertIn("UPrimitiveComponent", APPLY_WEAPON_HIT)
        self.assertIn("ESkyguardBossWeapon", APPLY_WEAPON_HIT)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, APPLY_WEAPON_HIT), section)
    ...
    def test_contract_does_not_add_boss_weapon_enum_contract(self) -> None:
        ...
        for token in LEFTOVER_BOSS_WEAPON_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertIn("ESkyguardBossWeapon", APPLY_WEAPON_HIT)
        self.assertNotIn("enum class", APPLY_WEAPON_HIT)
        self.assertNotIn("UENUM", APPLY_WEAPON_HIT)
        self.assertNotIn(
            "test_boss_weapon_enum_contract.py",
            APPLY_WEAPON_HIT,
        )
    ...
    def test_contract_does_not_lock_public_fields(self) -> None:
        ...
        for token in PUBLIC_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("OnBossPhaseChanged", APPLY_WEAPON_HIT)
        self.assertNotIn("OnPilotCommandNative", APPLY_WEAPON_HIT)
        self.assertNotIn("CurrentPilotCommand", APPLY_WEAPON_HIT)
    ...
    def test_harbor_adjacent_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_SEPARATION:
            ...
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, APPLY_WEAPON_HIT)
        self.assertNotIn("MinimumCivilianSeparationMeters", APPLY_WEAPON_HIT)
        self.assertNotIn("MinimumWeaponSeparationMeters", APPLY_WEAPON_HIT)
