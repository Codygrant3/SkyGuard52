...
# bJammed FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission06 PayloadWindowRuntime
# bJammed FIELD (declaration-only). THIS IS NOT leftover
...
# THIS IS NOT leftover airfield-target enum #222/#14d2,
...
# PayloadWindow Target / RemainingSeconds. THIS IS NOT leftover
...
# #1130-#1134. LOCK leftover Mission07
# SearchTrackRuntime TrackId/Sector and leftover
# Mission07 PTR Target #1137 / Integrity #1138 /
# leftover Mission07 PTR bDestroyed #1140. LOCK leftover Mission09
...
# construction in the .cpp. THIS IS NOT leftover
# Mission06 AirfieldTargetRuntime Integrity
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 default 100,
# one-line / split-line UPROPERTY wraps) on
# FSkyguardAirfieldTargetRuntime. It is
# leftover-safe Mission06 PayloadWindowRuntime
# bJammed FIELD (declaration-only). It is
...
# This PayloadWindowRuntime field sits after RemainingSeconds
# (after bActive / Target / RemainingSeconds) in the public section.
# Skip leftover b + Ya + kRuntimeReady forever (it sits
# after bMapAssemblyReady and before bGunnerReady). This contract
# is bJammed after bActive / Target / RemainingSeconds. THIS IS NOT
...
LEFTOVER_PAYLOAD_WINDOW_NOT_LOCKED = (
    ...
    "test_mission06_payload_window_runtime_remaining_seconds_field_decl_contract.py",
)
...
class Mission06PayloadWindowRuntimeJammedFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bJammed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_jammed_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_jammed_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bJammed"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bJammed", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bJammed", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bJammed"))
        self.assertNotIn("int32", LOCKED_DECL)
        self.assertNotIn("ESkyguardAirfieldTarget", LOCKED_DECL)
        self.assertNotIn("Category=", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bJammed", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 jammed field contract "
    ...
    def test_contract_is_jammed_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_jammed(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission06_payload_window_runtime_jammed"
            "_field_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_payload_window_runtime_jammed"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
