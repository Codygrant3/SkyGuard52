...
# INDEX_NONE or lock DispenserCenter construction
# in the .cpp. This is a FIELD contract on
# ASkyguardIronRainBoss, not leftover Iron Rain
# dispenser / climb / cross / engine methods
# (already drafted) and not leftover Iron Rain
# live-copy finish methods. Leftover Tempest
# lightning / gust / lock-stability methods
# and leftover Tempest emergency-finish /
# stabilized-lock methods stay sibling-only.
...
# DispenserCenter;`); accept that form and other
...
# fields DispenserPort, DispenserStarboard,
...
# sibling Tempest fields PortDischargeBoom,
# StarboardDischargeBoom, EngineIntake,
# ControlServo, DebrisPortPanel,
# DebrisStarboardPanel, DebrisIntakePanel, or
# RequiredLockStabilitySeconds as the
# primary lock. Stay off leftover Iron Rain
# methods OpenDispenserBay /
# ReleasePooledEscort / IssueClimbCommand /
# IssueCrossCommand / OpenUpperEngineExposure
# / GetDestroyed* / GetReleasedEscortCount /
# GetManeuver. Stay off leftover Tempest
# lightning / gust / lock-stability methods
# SetLightningExposed /
...
LEFTOVER_IRON_RAIN_NOT_LOCKED = (
    ...
    "GetManeuver",
    "ASkyguardTempestBoss::IssueClimbCommand",
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardIronRainBoss::DispenserCenter",
    "SkyguardIronRainBoss.cpp",
    "SetExposed",
...
class IronRainDispenserCenterFieldDeclContractTests(unittest.TestCase):
    def test_tempest_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DISPENSER_CENTER_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DISPENSER_CENTER_FIELD)
        self.assertIn("DispenserCenter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DISPENSER_CENTER_FIELD)
    ...
    def test_missing_dispenser_center_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardIronRainBoss();\n"
            f"\t{DISPENSER_PORT_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DISPENSER_CENTER_FIELD)
        self.assertIn("DispenserCenter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_CENTER}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DISPENSER_CENTER_FIELD)
        self.assertIn("DispenserCenter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_CENTER, section)
        ...
        self.assertNotIn("UPROPERTY", DISPENSER_CENTER_FIELD)
        self.assertNotIn("VisibleAnywhere", DISPENSER_CENTER_FIELD)
        self.assertNotIn("BlueprintReadOnly", DISPENSER_CENTER_FIELD)
        self.assertNotIn("Category", DISPENSER_CENTER_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_CENTER)
        self.assertNotIn("BlueprintCallable", UPROPERTY_CENTER)
        self.assertIn("Skyguard|Mission09|Boss", UPROPERTY_CENTER)
        self.assertIn("Mission09", UPROPERTY_CENTER)
        self.assertNotIn("Mission04", UPROPERTY_CENTER)
        self.assertNotIn("Mission05", UPROPERTY_CENTER)
        self.assertNotIn("Destruction", UPROPERTY_CENTER)
        self.assertNotIn("Encounter", UPROPERTY_CENTER)
        self.assertNotIn("Mission10", UPROPERTY_CENTER)
        self.assertNotIn("Safety", UPROPERTY_CENTER)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_CENTER)
            self.assertNotIn(invented, DISPENSER_CENTER_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DISPENSER_CENTER_FIELD)
        self.assertIn("DispenserCenter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* DispenserCenter;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DispenserCenter;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> DispenserCenter;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DISPENSER_CENTER_FIELD)
            self.assertIn("DispenserCenter", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_dispenser_center_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DISPENSER_CENTER_FIELD),
            DISPENSER_CENTER_FIELD,
        )
        self.assertTrue(has_declaration(section, DISPENSER_CENTER_FIELD))
        self.assertEqual(declaration_count(section, DISPENSER_CENTER_FIELD), 1)
        self.assertTrue(
            DISPENSER_CENTER_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            DISPENSER_CENTER_FIELD,
        )
        self.assertTrue(DISPENSER_CENTER_FIELD.endswith(";"), DISPENSER_CENTER_FIELD)
        self.assertIn("DispenserCenter", DISPENSER_CENTER_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            DISPENSER_CENTER_FIELD,
        )
        self.assertIn("TObjectPtr<", DISPENSER_CENTER_FIELD)
        self.assertNotIn("TArray<", DISPENSER_CENTER_FIELD)
        self.assertNotIn("=", DISPENSER_CENTER_FIELD)
        self.assertNotIn("INDEX_NONE", DISPENSER_CENTER_FIELD)
        self.assertNotIn("UFUNCTION", DISPENSER_CENTER_FIELD)
        self.assertNotIn("{", DISPENSER_CENTER_FIELD)
        self.assertNotIn("}", DISPENSER_CENTER_FIELD)
        self.assertNotIn("return ", DISPENSER_CENTER_FIELD)
        self.assertNotIn("CommandAntenna", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ControlLinkage", DISPENSER_CENTER_FIELD)
        self.assertNotIn("NoseCamera", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisCenter", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisTail", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisNose", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisSpine", DISPENSER_CENTER_FIELD)
        self.assertNotIn("AdvanceEncounter", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ResetEncounterState", DISPENSER_CENTER_FIELD)
        self.assertNotIn("IsRouteStateSafe", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetRouteProgress", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DISPENSER_CENTER_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DISPENSER_CENTER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DISPENSER_CENTER_FIELD)
        self.assertNotIn("RadarNode", DISPENSER_CENTER_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DISPENSER_CENTER_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DISPENSER_CENTER_FIELD)
        self.assertNotIn("IssueClimbCommand", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetFinaleStage", DISPENSER_CENTER_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DISPENSER_CENTER_FIELD)
        self.assertNotIn("IsWreckDiverted", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetGunnerMount", DISPENSER_CENTER_FIELD)
        self.assertNotIn("FaceWorldLocation", DISPENSER_CENTER_FIELD)
        self.assertNotIn("PortGuidanceArray", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisArmorPort", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetSearchlightTracked", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetContactIdentified", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetLightningExposed", DISPENSER_CENTER_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DISPENSER_CENTER_FIELD)
        self.assertNotIn("HullCollider", DISPENSER_CENTER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DISPENSER_CENTER_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_CENTER} {DISPENSER_CENTER_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DISPENSER_CENTER_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DISPENSER_CENTER_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DISPENSER_CENTER_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DISPENSER_CENTER_FIELD),
            section,
    ...
    def test_assigned_dispenser_center_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DispenserCenter = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DISPENSER_CENTER_FIELD)
        self.assertIn("DispenserCenter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DISPENSER_CENTER_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DispenserCenter", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DISPENSER_CENTER_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DISPENSER_CENTER_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DISPENSER_CENTER_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_CENTER, section)
        self.assertTrue(has_declaration(section, DISPENSER_CENTER_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DISPENSER_CENTER_FIELD.startswith("UFUNCTION"),
            DISPENSER_CENTER_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DISPENSER_CENTER_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DISPENSER_CENTER_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_dispenser_center_cpp_body(self) -> None:
        locked_only = f"{DISPENSER_CENTER_FIELD}\n"
        self.assertNotIn("{", DISPENSER_CENTER_FIELD)
        self.assertNotIn("}", DISPENSER_CENTER_FIELD)
        self.assertNotIn("return ", DISPENSER_CENTER_FIELD)
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SkyguardIronRainBoss.cpp", locked_only)
        self.assertNotIn("return false", DISPENSER_CENTER_FIELD)
        self.assertNotIn("return true", DISPENSER_CENTER_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetExposed", DISPENSER_CENTER_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DISPENSER_CENTER_FIELD,
        )
        self.assertNotIn("SetBossPhase", DISPENSER_CENTER_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DISPENSER_CENTER_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DISPENSER_CENTER_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("PortDischargeBoom", DISPENSER_CENTER_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DISPENSER_CENTER_FIELD)
        self.assertNotIn("EngineIntake", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisPortPanel", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DebrisIntakePanel", DISPENSER_CENTER_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ControlServo", DISPENSER_CENTER_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            DISPENSER_CENTER_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("EncounterController", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("DispenserPort", DISPENSER_CENTER_FIELD)
        self.assertNotIn("DispenserStarboard", DISPENSER_CENTER_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DISPENSER_CENTER_FIELD)
        self.assertIn("DispenserCenter", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DISPENSER_CENTER_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DISPENSER_CENTER_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DISPENSER_CENTER_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DISPENSER_CENTER_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DISPENSER_CENTER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, DISPENSER_CENTER_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetSearchlightTracked", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetContactIdentified", DISPENSER_CENTER_FIELD)
        self.assertNotIn("OpenOrbitExposure", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("SetLightningExposed", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DISPENSER_CENTER_FIELD)
        self.assertNotIn("IsLightningExposed", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DISPENSER_CENTER_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DISPENSER_CENTER_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DISPENSER_CENTER_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DISPENSER_CENTER_FIELD)
        self.assertNotIn("OpenDispenserBay", DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetManeuver", DISPENSER_CENTER_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DISPENSER_CENTER_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("ASkyguardGunner", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("SkyguardRadarNode", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DISPENSER_CENTER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DISPENSER_CENTER_FIELD)
        self.assertNotIn("MaxIntegrity", DISPENSER_CENTER_FIELD)
        self.assertNotIn("CurrentIntegrity", DISPENSER_CENTER_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DISPENSER_CENTER_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DISPENSER_CENTER_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DISPENSER_CENTER_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DISPENSER_CENTER_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DISPENSER_CENTER_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DISPENSER_CENTER_FIELD),
            DISPENSER_CENTER_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardIronRainBoss::DispenserCenter", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain DispenserCenter field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DISPENSER_CENTER_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DISPENSER_CENTER_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DISPENSER_CENTER_FIELD.lower(),
                "iron rain DispenserCenter contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DISPENSER_CENTER_FIELD)

    def test_contract_is_dispenser_center_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DISPENSER_CENTER_FIELD.endswith(";"))
        self.assertNotIn("=", DISPENSER_CENTER_FIELD)
        self.assertIn(UPROPERTY_CENTER, section)
