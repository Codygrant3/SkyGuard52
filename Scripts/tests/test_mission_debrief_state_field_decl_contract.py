# THIS IS leftover-safe FSkyguardMissionDebrief::State.
# origin/main form: split-line
# `UPROPERTY(VisibleAnywhere, BlueprintReadOnly)`
# then `ESkyguardMissionDebriefState State =
# ESkyguardMissionDebriefState::Unavailable;` (initializer IS present,
# no Category=). If a clone asserts no initializer, drop that and
# accept origin/main. If a clone asserts Category= and origin/main
# has none, drop that assertion. If a clone asserts `= false` /
# `= 0` / `= 0.f` / Inactive, do not copy those; this field is the
# Unavailable enum default.
...
# Do not contract other Debrief fields (Result, MissionDisplayName,
# Narrative, bNewBestScore, bNewBestMedal, bProgressSaved,
# SaveSlotName, NextMissionId, NextMissionDisplayName,
...
# no Category=. Accept Unavailable enum default.
# Fail-closed if type/decl missing, renamed, Category= added, or
# default is not ESkyguardMissionDebriefState::Unavailable.
...
TARGET_WRONG_BARE = "ESkyguardMissionDebriefState State;"
TARGET_WRONG_READY = (
    "ESkyguardMissionDebriefState State = "
    "ESkyguardMissionDebriefState::Ready;"
)
TARGET_WRONG_INACTIVE = (
    "ESkyguardMissionDebriefState State = "
    "ESkyguardMissionDebriefState::Inactive;"
)
TARGET_WRONG_FALSE = "ESkyguardMissionDebriefState State = false;"
TARGET_WRONG_ZERO = "ESkyguardMissionDebriefState State = 0;"
TARGET_WRONG_FLOAT = "ESkyguardMissionDebriefState State = 0.f;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored Unavailable enum default. Do not
    # accept a bare decl or `= false` / `= 0` / `= 0.f` / Inactive when
    # origin/main has ESkyguardMissionDebriefState::Unavailable.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    for match in re.finditer(r"UPROPERTY\(", compact):
        ...
        if depth != 0:
            continue
        after = compact[index:]
        if re.match(r"\s*ESkyguardMissionDebriefState\s+State\b", after):
            return compact[start : index - 1]
    raise AssertionError(
        "UPROPERTY for ESkyguardMissionDebriefState State is missing from "
...
class MissionDebriefStateFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_state_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_state_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardMissionDebriefState State"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("Unavailable", LOCKED_DECL)
        self.assertIn("ESkyguardMissionDebriefState::Unavailable", LOCKED_DECL)
        self.assertNotIn("Inactive", LOCKED_DECL)
        self.assertNotIn("Ready", LOCKED_DECL)
        self.assertNotIn("Acknowledged", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_READY}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_INACTIVE}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_READY}\n", LOCKED_DECL)
        self.assertIn("State", str(raised.exception))

    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_READY}\n",
            f"\t{TARGET_WRONG_INACTIVE}\n",
            f"\t{TARGET_WRONG_FALSE}\n",
            f"\t{TARGET_WRONG_ZERO}\n",
            f"\t{TARGET_WRONG_FLOAT}\n",
            "\tint32 State = 0;\n",
            "\tbool State = false;\n",
            "\tESkyguardMissionObjectiveState State = "
            "ESkyguardMissionObjectiveState::Inactive;\n",
            "\tFSkyguardMissionResult Result;\n",
            "\tFText MissionDisplayName;\n",
            "\tESkyguardMissionDebriefState DebriefState = "
            "ESkyguardMissionDebriefState::Unavailable;\n",
            "\tESkyguardMissionDebriefState ProgressState = "
            "ESkyguardMissionDebriefState::Unavailable;\n",
        )
    ...
    def test_bare_decl_without_initializer_fails_closed(self) -> None:
        dropped = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_VISIBLE}\n"
        ...
        self.assertIn("ESkyguardMissionDebriefState::Unavailable", origin)
    ...
    def test_does_not_contract_sibling_fields_or_neighbors(self) -> None:
        ...
        self.assertNotIn("SaveSlotName", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief State field contract contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_state_field_decl_contract.py"
        ))
