...
# INDEX_NONE or lock DecoyController construction
...
# DecoyController;`); accept that form and other
...
# isolated-test drafts #107–#523 including
...
# leftover isolated-test drafts #107–#523,
# leftover searchlight-track-runtime-defaults
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardIronRainBoss::DecoyController",
    "SkyguardIronRainBoss.cpp",
    "SetExposed",
...
class IronRainDecoyControllerFieldDeclContractTests(unittest.TestCase):
    def test_iron_rain_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DECOY_CONTROLLER_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DECOY_CONTROLLER_FIELD)
        self.assertIn("DecoyController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DECOY_CONTROLLER_FIELD)
    ...
    def test_missing_decoy_controller_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardIronRainBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DECOY_CONTROLLER_FIELD)
        self.assertIn("DecoyController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DECOY_CONTROLLER_FIELD)
        self.assertIn("DecoyController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("VisibleAnywhere", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("BlueprintReadOnly", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("Category", DECOY_CONTROLLER_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DECOY_CONTROLLER_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DECOY_CONTROLLER_FIELD)
        self.assertIn("DecoyController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* DecoyController;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DecoyController;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> DecoyController;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DECOY_CONTROLLER_FIELD)
            self.assertIn("DecoyController", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_decoy_controller_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DECOY_CONTROLLER_FIELD),
            DECOY_CONTROLLER_FIELD,
        )
        self.assertTrue(has_declaration(section, DECOY_CONTROLLER_FIELD))
        self.assertEqual(declaration_count(section, DECOY_CONTROLLER_FIELD), 1)
        self.assertTrue(
            DECOY_CONTROLLER_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            DECOY_CONTROLLER_FIELD,
        )
        self.assertTrue(DECOY_CONTROLLER_FIELD.endswith(";"), DECOY_CONTROLLER_FIELD)
        self.assertIn("DecoyController", DECOY_CONTROLLER_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            DECOY_CONTROLLER_FIELD,
        )
        self.assertIn("TObjectPtr<", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("TArray<", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("=", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("INDEX_NONE", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("UFUNCTION", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("{", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("}", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("return ", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("CommandAntenna;", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ControlLinkage", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("NoseCamera", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisCenter", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisTail", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisNose", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisSpine", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("AdvanceEncounter", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ResetEncounterState", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("IsRouteStateSafe", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetRouteProgress", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("RadarNode", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("IssueClimbCommand", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetFinaleStage", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("IsWreckDiverted", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetGunnerMount", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("FaceWorldLocation", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("PortGuidanceArray", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisArmorPort", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetSearchlightTracked", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetContactIdentified", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetLightningExposed", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("HullCollider", DECOY_CONTROLLER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DECOY_CONTROLLER_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {DECOY_CONTROLLER_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DECOY_CONTROLLER_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DECOY_CONTROLLER_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DECOY_CONTROLLER_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DECOY_CONTROLLER_FIELD),
            section,
    ...
    def test_assigned_decoy_controller_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DecoyController = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DECOY_CONTROLLER_FIELD)
        self.assertIn("DecoyController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DECOY_CONTROLLER_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DecoyController", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DECOY_CONTROLLER_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DECOY_CONTROLLER_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DECOY_CONTROLLER_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, DECOY_CONTROLLER_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DECOY_CONTROLLER_FIELD.startswith("UFUNCTION"),
            DECOY_CONTROLLER_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DECOY_CONTROLLER_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DECOY_CONTROLLER_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_decoy_controller_cpp_body(self) -> None:
        locked_only = f"{DECOY_CONTROLLER_FIELD}\n"
        self.assertNotIn("{", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("}", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("return ", DECOY_CONTROLLER_FIELD)
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SkyguardIronRainBoss.cpp", locked_only)
        self.assertNotIn("return false", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("return true", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetExposed", DECOY_CONTROLLER_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DECOY_CONTROLLER_FIELD,
        )
        self.assertNotIn("SetBossPhase", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DECOY_CONTROLLER_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DECOY_CONTROLLER_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("PortDischargeBoom", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("EngineIntake", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisPortPanel", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DebrisIntakePanel", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DECOY_CONTROLLER_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            DECOY_CONTROLLER_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("EncounterController", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ControlServo", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DispenserPort", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DispenserCenter", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DECOY_CONTROLLER_FIELD)
        self.assertIn("DecoyController", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_command_antenna_port(
        self,
    ) -> None:
        locked_only = f"{DECOY_CONTROLLER_FIELD}\n"
        for token in LEFTOVER_COMMAND_ANTENNA_PORT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("CommandAntennaPort", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("DispenserStarboard", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetManeuver", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DECOY_CONTROLLER_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DECOY_CONTROLLER_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DECOY_CONTROLLER_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DECOY_CONTROLLER_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DECOY_CONTROLLER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, DECOY_CONTROLLER_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetSearchlightTracked", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetContactIdentified", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("OpenOrbitExposure", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SetLightningExposed", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("IsLightningExposed", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DECOY_CONTROLLER_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DECOY_CONTROLLER_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DECOY_CONTROLLER_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardGunner", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SkyguardRadarNode", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("MaxIntegrity", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("CurrentIntegrity", DECOY_CONTROLLER_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DECOY_CONTROLLER_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DECOY_CONTROLLER_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DECOY_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DECOY_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DECOY_CONTROLLER_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DECOY_CONTROLLER_FIELD),
            DECOY_CONTROLLER_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardIronRainBoss::DecoyController", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain DecoyController field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DECOY_CONTROLLER_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DECOY_CONTROLLER_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DECOY_CONTROLLER_FIELD.lower(),
                "iron rain DecoyController contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DECOY_CONTROLLER_FIELD)

    def test_contract_is_decoy_controller_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DECOY_CONTROLLER_FIELD.endswith(";"))
        self.assertNotIn("=", DECOY_CONTROLLER_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
