# THIS IS leftover-safe FSkyguardCpgHudSnapshot StationStatus.
...
# `FString StationStatus;` with NO in-struct initializer.
# Fail-closed if StationStatus is missing or renamed, if the
# type is not FString, if an initializer is invented, or if
...
# StationStatus is NOT leftover sibling WeaponLine / RangeLine /
# ThreatLine / EufdLine. HUD sibling `float RangeMeters = -1.f` stays
# unclaimed.
...
# field decls #1451-#1465. Keep those files in LOCKED_SCRIPTS.
# THIS IS NOT leftover StormRainBeatKit isolated
# #1490-#1500 or leftover analog storm-rain-beat-kit
# fields / defaults / calls.
# THIS IS NOT leftover Night/Day Beat.Threat
# #1477/#1481 or leftover analog night/day beat-kit.
# THIS IS NOT leftover sibling HUD WeaponLine / RangeLine /
# ThreatLine / EufdLine isolated field decls. Keep those files in
# LOCKED_SCRIPTS.
...
# retarget: type is FString, identifier is StationStatus,
# no `=`, locked decl is the plain field, not a
# UPROPERTY wrap.
...
TARGET = "FString StationStatus;"
TARGET_WRONG_EQ_EMPTY = 'FString StationStatus = TEXT("");'
TARGET_WRONG_EQ_NONE = "FString StationStatus = NAME_None;"
TARGET_WRONG_FALSE = "FString StationStatus = false;"
TARGET_WRONG_TRUE = "FString StationStatus = true;"
TARGET_WRONG_ZERO = "FString StationStatus = 0.f;"
TARGET_WRONG_FLOAT = "float StationStatus;"
TARGET_WRONG_HEALTH = "float StationStatus = 160.f;"
TARGET_WRONG_BOOL = "bool StationStatus;"
TARGET_WRONG_INT = "int32 StationStatus;"
TARGET_WRONG_FNAME = "FName StationStatus;"
TARGET_WRONG_TCHAR = 'const TCHAR* StationStatus = TEXT("");'
TARGET_WRONG_WEAPON = "FString WeaponLine;"
TARGET_WRONG_RANGE = "FString RangeLine;"
TARGET_WRONG_THREAT = "FString ThreatLine;"
TARGET_WRONG_EUFD = "FString EufdLine;"
...
STATION_STATUS_BARE_RE = re.compile(r"FString\s+StationStatus\s*;")
STATION_STATUS_INIT_RE = re.compile(r"FString\s+StationStatus\s*=")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def station_status_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*FString\s+StationStatus\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString StationStatus;`.
    # Do not accept `= TEXT("")` / `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` when origin/main locks
    # a bare FString. Do not accept sibling WeaponLine or
    # leftover ContactMark Label / leftover debrief
    # MissionTitle. Do not accept leftover RangeMeters.
    # Do not accept leftover LoadoutSpec FlareCount.
    # Do not accept UPROPERTY / Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if STATION_STATUS_INIT_RE.search(compact):
        return False
    if re.search(r"float\s+StationStatus\b", compact):
        return False
    if re.search(r"bool\s+StationStatus\b", compact):
        return False
    if re.search(r"int32\s+StationStatus\b", compact):
        return False
    if re.search(r"FName\s+StationStatus\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*StationStatus\b", compact):
        return False
    if re.search(r"float\s+RangeMeters\b", compact) and (
        STATION_STATUS_BARE_RE.search(compact) is None
    ):
        return False
    if STATION_STATUS_BARE_RE.search(compact) is None:
        return False
    if station_status_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString StationStatus\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString StationStatus is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FString StationStatus\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FString StationStatus is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "StationStatus"):
        raise AssertionError(
            "UPROPERTY clone landed on StationStatus; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "StationStatus"):
            raise AssertionError(
                f"{token} clone landed on StationStatus; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotStationStatusFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "StationStatus"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString StationStatus;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("FString StationStatus"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("=", LOCKED_DECL)
        self.assertIn("StationStatus", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_THREAT)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("StationStatus", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "StationStatus"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_station_status_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_weapon_line_does_not_satisfy_station_status(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEAPON}\n"
    ...
    def test_range_line_does_not_satisfy_station_status(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_threat_line_does_not_satisfy_station_status(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_eufd_line_does_not_satisfy_station_status(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_EUFD}\n"
    ...
    def test_leftover_range_meters_does_not_satisfy_station_status(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        section = spec_section(leftover)
        self.assertTrue(has_identifier(section, SIBLING_RANGE_METERS), section)
        self.assertFalse(has_declaration(section, LOCKED_DECL), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("StationStatus", str(raised.exception))
        ...
        self.assertTrue(has_identifier(origin, SIBLING_RANGE_METERS), origin)
        self.assertIn(TARGET_WRONG_RANGE_METERS, origin)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RANGE_METERS)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString StationStatus;", compact_origin)
        self.assertNotIn("StationStatus = TEXT", compact_origin)
        self.assertNotIn("StationStatus = NAME_None", compact_origin)
        self.assertNotIn("StationStatus = false", compact_origin)
        self.assertNotIn("StationStatus = true", compact_origin)
        self.assertNotIn("StationStatus = 0.f", compact_origin)
        self.assertNotIn("StationStatus = 160.f", compact_origin)

    def test_station_status_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString StationStatus"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RANGE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_THREAT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_EUFD}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{LEFTOVER_CONTACT_LABEL}\n",
            "\tFString StationStatuss;\n",
            "\tint32 StationStatus;\n",
            "\tbool StationStatus;\n",
            "\tfloat StationStatus = " + forty + ";\n",
            "\tfloat StationStatus = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    station_status_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(station_status_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot StationStatus field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "station_status_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_SIBLING_WEAPON_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SIBLING_RANGE_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SIBLING_THREAT_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SIBLING_EUFD_LINE, LOCKED_SCRIPTS)
