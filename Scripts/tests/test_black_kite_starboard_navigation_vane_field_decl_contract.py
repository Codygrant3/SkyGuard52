...
# INDEX_NONE or lock StarboardNavigationVane
...
# StarboardNavigationVane;`); accept that form and
...
# only. Do not lock leftover
# MinimumCivilianSeparationMeters. Stay off
# leftover sibling BlackKite fields
# PortNavigationVane, Jammer, PowerBus,
# DebrisPortVane, DebrisStarboardVane,
# DebrisJammer as the primary lock. Stay off
# leftover LastFlight fields
# PortGuidanceArray,
...
# LastFlight sibling debris fields
...
# leftover theater-kit #59 / leftover theater-kit /
...
# BossDrone drafts, leftover BlackKite
# sibling field contracts, leftover patrol-ship
...
# Harbor-adjacent.
STARBOARD_NAVIGATION_VANE_FIELD = (
...
# Python StarboardNavigationVane field declaration
...
# BlackKite sibling fields, leftover
...
# IsSearchlightTracked, leftover BlackKite
# sibling fields, leftover RadarGhost
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBlackKiteBoss::StarboardNavigationVane",
    "SkyguardBlackKiteBoss.cpp",
    "SetExposed",
...
class BlackKiteStarboardNavigationVaneFieldDeclContractTests(unittest.TestCase):
    def test_black_kite_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, STARBOARD_NAVIGATION_VANE_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{STARBOARD_NAVIGATION_VANE_FIELD}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn("StarboardNavigationVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_missing_starboard_navigation_vane_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBlackKiteBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn("StarboardNavigationVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn("StarboardNavigationVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("VisibleAnywhere", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("BlueprintReadOnly", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("Category", STARBOARD_NAVIGATION_VANE_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn("StarboardNavigationVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* StarboardNavigationVane;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> StarboardNavigationVane;\n"
        )
        weak = (
        ...
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardNavigationVane = nullptr;\n"
        )
        static_mesh = (
        ...
        leftover_debris_port_vane = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisPortVane;\n"
        )
        leftover_debris_starboard_vane = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisStarboardVane;\n"
        )
        leftover_debris_jammer_field = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisJammer;\n"
        )
        wrong_name = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, STARBOARD_NAVIGATION_VANE_FIELD)
            self.assertIn("StarboardNavigationVane", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_starboard_navigation_vane_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, STARBOARD_NAVIGATION_VANE_FIELD),
            1,
        )
        self.assertTrue(
            STARBOARD_NAVIGATION_VANE_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        self.assertTrue(
            STARBOARD_NAVIGATION_VANE_FIELD.endswith(";"),
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        self.assertIn("StarboardNavigationVane", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn(
        ...
        self.assertIn("TObjectPtr<", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("TArray<", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("=", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("INDEX_NONE", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("UFUNCTION", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("{", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("}", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("return ", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("IssueClimbCommand", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        ...
        self.assertNotIn("GetCivilianSeparationMeters", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("GetFinaleStage", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("IsWreckDiverted", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("GetGunnerMount", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("FaceWorldLocation", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("CommandCore", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        self.assertNotIn("SetLightningExposed", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        self.assertNotIn("HullCollider", STARBOARD_NAVIGATION_VANE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {STARBOARD_NAVIGATION_VANE_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{STARBOARD_NAVIGATION_VANE_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, STARBOARD_NAVIGATION_VANE_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, STARBOARD_NAVIGATION_VANE_FIELD),
    ...
    def test_assigned_starboard_navigation_vane_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardNavigationVane = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn("StarboardNavigationVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, STARBOARD_NAVIGATION_VANE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            self.assertIn("StarboardNavigationVane", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, STARBOARD_NAVIGATION_VANE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{STARBOARD_NAVIGATION_VANE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, STARBOARD_NAVIGATION_VANE_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, STARBOARD_NAVIGATION_VANE_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            STARBOARD_NAVIGATION_VANE_FIELD.startswith("UFUNCTION"),
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, STARBOARD_NAVIGATION_VANE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", STARBOARD_NAVIGATION_VANE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_starboard_navigation_vane_cpp_body(
        self,
    ) -> None:
        locked_only = f"{STARBOARD_NAVIGATION_VANE_FIELD}\n"
        self.assertNotIn("{", STARBOARD_NAVIGATION_VANE_FIELD)
        ...
        self.assertNotIn("return false", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("return true", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SetExposed", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
        self.assertNotIn("SetBossPhase", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_apply_strike(), STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), STARBOARD_NAVIGATION_VANE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, STARBOARD_NAVIGATION_VANE_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_black_kite_fields(self) -> None:
        ...
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("PortGuidanceArray", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("PortCoolingSystem", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("StarboardCoolingSystem", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("CommandCore", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("PortEngine", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("Jammer", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_engine(self) -> None:
        ...
        for token in LEFTOVER_STARBOARD_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("StarboardEngine", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_sibling_debris_fields(
        ...
        for token in LEFTOVER_SIBLING_DEBRIS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisArmorStarboard", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisStrikeBayPort", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisStrikeBayStarboard", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisEnginePort", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisEngineStarboard", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SetSearchlightTracked", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("IsSearchlightTracked", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("PortNavigationVane", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("PowerBus", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisPortVane", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisStarboardVane", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("DebrisJammer", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SetContactIdentified", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("OpenOrbitExposure", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SetLightningExposed", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("ASkyguardGunner", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SkyguardRadarNode", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("MaxIntegrity", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("CurrentIntegrity", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, STARBOARD_NAVIGATION_VANE_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn("MinimumWeaponSeparationMeters", section)
        self.assertNotIn("450.f", section)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{STARBOARD_NAVIGATION_VANE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, STARBOARD_NAVIGATION_VANE_FIELD),
            STARBOARD_NAVIGATION_VANE_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "black kite StarboardNavigationVane field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{STARBOARD_NAVIGATION_VANE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, STARBOARD_NAVIGATION_VANE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                STARBOARD_NAVIGATION_VANE_FIELD.lower(),
                "black kite StarboardNavigationVane contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, STARBOARD_NAVIGATION_VANE_FIELD)

    def test_contract_is_starboard_navigation_vane_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(STARBOARD_NAVIGATION_VANE_FIELD.endswith(";"))
        self.assertNotIn("=", STARBOARD_NAVIGATION_VANE_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
