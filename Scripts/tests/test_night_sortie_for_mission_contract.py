...
FOR_MISSION = "const FSkyguardNightSortieBeatKit& ForMission(FName MissionId);"
# Leftover #56–#64 plus NightSortieBeatKit production sources.
# This lane only adds an isolated Python ForMission declaration
...
# DownedBird() sequences, SequencesDiffer (in-flight sibling),
# BeatIndexForElapsed (#268), KindAt (opening now), Beats[7]
# (#252), bKeepThermal (#250), beat-kind enum (#246), beat
# defaults (#247), and kit FName fields (#254) stay sibling-only.
LOCKED_SCRIPTS = (
...
# Which kit ForMission returns stays unlocked. Do not invent
# NightEyes / DownedBird / M04 / M07 return mapping.
...
class NightSortieForMissionContractTests(unittest.TestCase):
    def test_night_sortie_namespace_exists(self) -> None:
        ...
        self.assertIn(FOR_MISSION, body)
    ...
    def test_missing_for_mission_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, FOR_MISSION)
    ...
    def test_for_mission_declaration_matches_origin_main(self) -> None:
        ...
        self.assertEqual(require_declaration(body, FOR_MISSION), FOR_MISSION)
        self.assertIn(FOR_MISSION, body)
        self.assertEqual(body.count(FOR_MISSION), 1)
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", FOR_MISSION)
        self.assertNotIn("INDEX_NONE", FOR_MISSION)
        self.assertNotIn("NAME_None", FOR_MISSION)
        self.assertNotIn("{", FOR_MISSION)
        self.assertNotIn("}", FOR_MISSION)
    ...
    def test_contract_does_not_invent_which_kit_is_returned(self) -> None:
        locked_only = f"{FOR_MISSION}\n"
        self.assertEqual(require_declaration(locked_only, FOR_MISSION), FOR_MISSION)
        for token in KIT_RETURNS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, FOR_MISSION)
        self.assertNotIn("NightEyes", FOR_MISSION)
        self.assertNotIn("DownedBird", FOR_MISSION)
        self.assertNotIn("M04", FOR_MISSION)
        self.assertNotIn("M07", FOR_MISSION)
    ...
    def test_contract_does_not_relock_sequences_differ(self) -> None:
        ...
        self.assertNotIn("SequencesDiffer", FOR_MISSION)
    ...
    def test_contract_does_not_relock_kind_at(self) -> None:
        ...
        self.assertNotIn("KindAt", FOR_MISSION)
        self.assertNotIn("KindAt", locked_only)
        self.assertNotIn("ESkyguardNightSortieBeatKind KindAt(", (FOR_MISSION,))
    ...
    def test_contract_does_not_relock_beats_enum_defaults_or_fields(
        ...
        self.assertNotIn(BEATS_NOT_LOCKED, FOR_MISSION)
        self.assertNotIn(KEEP_THERMAL_NOT_LOCKED, body)
        self.assertNotIn(KEEP_THERMAL_NOT_LOCKED, FOR_MISSION)
    ...
    def test_contract_does_not_read_cpp_or_kit_returns(self) -> None:
        body = namespace_body(origin_main_header())
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, FOR_MISSION)
        self.assertNotIn("SkyguardNightSortieBeatKit.cpp", body)
        self.assertNotIn("SkyguardNightSortieBeatKit::ForMission", body)
        for token in KIT_RETURNS_NOT_LOCKED:
            self.assertNotIn(token, FOR_MISSION)
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"night-sortie ForMission contains {banned}; "
