# THIS IS leftover-safe ASkyguardGunner MouseSensitivity.
...
# then LOCKED_DECL `float MouseSensitivity = 1.1f;`.
...
# Initializer IS `= 1.1f`. There is NO ClampMin / ClampMax
...
# float, or if initializer is not `= 1.1f`.
...
# ADSFov. Do NOT claim leftover LookYawLimit = 95.f,
# leftover LookPitchMin = -35.f, leftover in-flight
# LookPitchMax = 55.f, leftover ADSFov, or leftover
# HipFov as this slot. Do NOT claim leftover analog
...
# Do NOT refill leftover #1299-#1618.
# Clone source leftover-safe LookYawLimit #1618
# (EditAnywhere BlueprintReadWrite Category
# Skyguard|Combat wrapping float LookYawLimit = 95.f
# WITH initializer). Evolved leftover-safe sibling
# is in-flight LookPitchMax (same Combat wrap,
# stop-before ADSFov). Alternative clone
# #1300 Theater FName WeatherIdentity has NO
# initializer — RETARGET hard if started there.
# RETARGET HARD: initializer is `= 1.1f;`. Wrap is
# EditAnywhere BlueprintReadWrite
# Category="Skyguard|Combat". Type is float, not
# TObjectPtr. Identifier is MouseSensitivity.
...
TARGET = "float MouseSensitivity = 1.1f;"
TARGET_WRONG_BARE = "float MouseSensitivity;"
TARGET_WRONG_FALSE = "float MouseSensitivity = false;"
TARGET_WRONG_TRUE = "float MouseSensitivity = true;"
TARGET_WRONG_ZERO = "float MouseSensitivity = 0.f;"
TARGET_WRONG_ONE = "float MouseSensitivity = 1.f;"
TARGET_WRONG_NO_F = "float MouseSensitivity = 1.1;"
TARGET_WRONG_FNAME = "FName MouseSensitivity;"
TARGET_WRONG_INT = "int32 MouseSensitivity = 0;"
TARGET_WRONG_BOOL = "bool MouseSensitivity = true;"
...
TARGET_WRONG_CANNON_RATE = "float MouseSensitivity = 12.0f;"
TARGET_WRONG_CANNON_RECOIL = "float MouseSensitivity = 0.92f;"
TARGET_WRONG_CANNON_DAMAGE = "float MouseSensitivity = 22.0f;"
TARGET_WRONG_TIME_OF_DAY = "float MouseSensitivity = 12.f;"
TARGET_WRONG_SENSOR_ACQUIRE = "float MouseSensitivity = 5.5f;"
TARGET_WRONG_CLOUD = "float MouseSensitivity = 0.25f;"
TARGET_WRONG_YAW_AS_SLOT = "float MouseSensitivity = 95.f;"
TARGET_WRONG_PITCH_MIN_AS_SLOT = "float MouseSensitivity = -35.f;"
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_gunner_look_pitch_max_field_decl_contract.py",
    "Scripts/tests/test_apache_cpg_feel_contract.py",
    "Scripts/tests/test_gunship_weapon_stations_contract.py",
...
MOUSE_SENSITIVITY_DECL_RE = re.compile(
    r"float\s+MouseSensitivity\s*=\s*1\.1f\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare `float MouseSensitivity;` /
    ...
    if MOUSE_SENSITIVITY_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+MouseSensitivity\b", compact):
        return False
    if re.search(r"int32\s+MouseSensitivity\b", compact):
        return False
    if re.search(r"bool\s+MouseSensitivity\b", compact):
        return False
    if re.search(r"TObjectPtr\s*<\s*UTextRenderComponent\s*>\s+MouseSensitivity\b", compact):
        return False
    if WEATHER_IDENTITY_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+MouseSensitivity\s*;", compact):
        return False
    return True
...
def claimed_field_window_from_section(section: str) -> str:
    ...
    raise AssertionError(
        "UPROPERTY for float MouseSensitivity = 1.1f; is missing from "
...
def spec_section(header: str) -> str:
    ...
    if SIBLING_ADS_FOV in claimed:
        raise AssertionError(
            f"{CLASS_NAME} claimed window includes {SIBLING_ADS_FOV}"
        )
    if SIBLING_HIP_FOV in claimed:
        raise AssertionError(
            f"{CLASS_NAME} claimed window includes {SIBLING_HIP_FOV}"
        )
    if SIBLING_LOOK_PITCH_MAX in claimed:
        raise AssertionError(
            f"{CLASS_NAME} claimed window includes {SIBLING_LOOK_PITCH_MAX}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MouseSensitivity\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MouseSensitivity is missing from "
        f"origin/main:{HEADER_PATH} class {CLASS_NAME} protected "
        "UPROPERTY fields"
    )
...
class GunnerMouseSensitivityFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(protected, "MouseSensitivity"), protected)
        self.assertTrue(has_identifier(protected, SIBLING_ADS_FOV), protected)
        self.assertTrue(has_identifier(protected, SIBLING_HIP_FOV), protected)
        ...
        self.assertTrue(has_identifier(protected, SIBLING_LOOK_PITCH_MAX), protected)
        ...
        self.assertTrue(has_identifier(section, "MouseSensitivity"), section)
        self.assertIn("UPROPERTY", section)
        self.assertNotIn(SIBLING_ADS_FOV, section)
        self.assertNotIn(SIBLING_HIP_FOV, section)
        self.assertNotIn(SIBLING_LOOK_YAW, section)
        self.assertNotIn(SIBLING_LOOK_PITCH_MIN, section)
        self.assertNotIn(SIBLING_LOOK_PITCH_MAX, section)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            spec_section(mixed)
        self.assertIn("MouseSensitivity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_mouse_sensitivity_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float MouseSensitivity = 1.1f;", compact_origin)
        self.assertNotIn("MouseSensitivity = NAME_None", compact_origin)
        self.assertNotIn("MouseSensitivity = false", compact_origin)
        self.assertNotIn("MouseSensitivity = true", compact_origin)
        self.assertNotIn("MouseSensitivity = 0.f", compact_origin)
        self.assertNotIn("MouseSensitivity = 160.f", compact_origin)
        self.assertNotIn("MouseSensitivity = 12.0f", compact_origin)
        self.assertNotIn("MouseSensitivity = 0.92f", compact_origin)
        self.assertNotIn("MouseSensitivity = 95.f", compact_origin)

    def test_mouse_sensitivity_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MouseSensitivity"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 1.1f", LOCKED_DECL)
        ...
        self.assertNotIn("= 55.f", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 1.1f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 1.1f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat MouseSensitivitys = 1.1f;\n",
            "\tint32 MouseSensitivity = 1;\n",
            "\tbool MouseSensitivity = true;\n",
            "\tfloat MouseSensitivity = " + forty + ";\n",
            "\tfloat MouseSensitivity = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_in_flight_look_siblings(self) -> None:
        ...
        self.assertIn(SIBLING_LOOK_PITCH_MAX, protected)
        self.assertIn(SIBLING_ADS_FOV, protected)
        self.assertIn(SIBLING_HIP_FOV, protected)
        self.assertNotIn(SIBLING_LOOK_YAW, section)
        self.assertNotIn(SIBLING_LOOK_PITCH_MIN, section)
        self.assertNotIn(SIBLING_LOOK_PITCH_MAX, section)
        self.assertNotIn(SIBLING_ADS_FOV, section)
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_GUNNER_LOOK_PITCH_MAX)
    ...
    def test_parse_window_is_protected_uproperty_only(self) -> None:
        ...
        self.assertIn("MouseSensitivity", protected)
        self.assertIn(SIBLING_ADS_FOV, protected)
        self.assertNotIn(SIBLING_ADS_FOV, section)
    ...
    def test_clone_source_theater_kit_weather_identity_is_not_locked_decl(
        ...
        self.assertIn("float ", LOCKED_DECL)
        self.assertIn("MouseSensitivity", LOCKED_DECL)
        self.assertIn("1.1f", LOCKED_DECL)
        self.assertNotIn("FName ", LOCKED_DECL)
        self.assertNotIn("WeatherIdentity", LOCKED_DECL)
        self.assertNotIn("160.f", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner MouseSensitivity field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mouse_sensitivity_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_header_path_is_gunner_mouse_sensitivity_only(self) -> None:
        self.assertEqual(HEADER_PATH, "Source/Skyguard52/SkyguardGunner.h")
        self.assertEqual(CLASS_NAME, "ASkyguardGunner")
        self.assertEqual(LOCKED_DECL, "float MouseSensitivity = 1.1f;")
        self.assertEqual(
            UPROPERTY_EDIT_WRITE,
            'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
            'Category="Skyguard|Combat")',
