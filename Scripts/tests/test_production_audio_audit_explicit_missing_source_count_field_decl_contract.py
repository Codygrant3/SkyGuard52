...
# then `int32 ExplicitMissingSourceCount = 0;`.
...
# THIS IS NOT leftover BoundProductionSourceCount #1412.
# THIS IS NOT leftover production-audio-audit-defaults
# #196 / #5e51 analog (this is the isolated field decl).
# Sibling decls stay unclaimed: RequiredCategoryCount /
# BoundProductionSourceCount / QATestOnlyCount /
...
TARGET = "int32 ExplicitMissingSourceCount = 0;"
TARGET_WRONG_BARE = "int32 ExplicitMissingSourceCount;"
TARGET_WRONG_FALSE = "int32 ExplicitMissingSourceCount = false;"
TARGET_WRONG_TRUE = "int32 ExplicitMissingSourceCount = true;"
TARGET_WRONG_ZERO = "int32 ExplicitMissingSourceCount = 0.f;"
TARGET_WRONG_FORTY = "int32 ExplicitMissingSourceCount = 40;"
TARGET_WRONG_EIGHTY = "int32 ExplicitMissingSourceCount = 80;"
TARGET_WRONG_FLOAT = "float ExplicitMissingSourceCount = 0;"
TARGET_WRONG_FNAME = "FName ExplicitMissingSourceCount;"
TARGET_WRONG_BOOL = "bool ExplicitMissingSourceCount = false;"
TARGET_WRONG_FSTRING = "FString ExplicitMissingSourceCount;"
TARGET_WRONG_TARRAY = "TArray<FName> ExplicitMissingSourceCount;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> ExplicitMissingSourceCount;"
...
FIELD_NAME = "ExplicitMissingSourceCount"
...
THIS_SCRIPT = (
    "Scripts/tests/test_production_audio_audit_explicit"
    "_missing_source_count_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 ExplicitMissingSourceCount = 0;`.
    ...
    if re.search(
        r"int32\s+ExplicitMissingSourceCount\s*;",
        compact,
    ) and re.search(
        r"int32\s+ExplicitMissingSourceCount\s*=",
        compact,
        ...
        return False
    if re.search(
        r"ExplicitMissingSourceCount\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"ExplicitMissingSourceCount\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"ExplicitMissingSourceCount\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"ExplicitMissingSourceCount\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"ExplicitMissingSourceCount\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+ExplicitMissingSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+ExplicitMissingSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+ExplicitMissingSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+ExplicitMissingSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+ExplicitMissingSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+ExplicitMissingSourceCount\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+ExplicitMissingSourceCount\s*=\s*0\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+ExplicitMissingSourceCount\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 ExplicitMissingSourceCount is missing from "
...
class ProductionAudioAuditExplicitMissingSourceCountFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_explicit_missing_source_count_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_drop_or_harbor_change_fails_closed(self) -> None:
        ...
        self.assertIn("int32 ExplicitMissingSourceCount = 0;", compact_origin)
        self.assertNotIn("ExplicitMissingSourceCount = 40", compact_origin)
        self.assertNotIn("ExplicitMissingSourceCount = 80", compact_origin)
        self.assertNotIn("ExplicitMissingSourceCount = false", compact_origin)
        self.assertNotIn("ExplicitMissingSourceCount;", compact_origin.replace(
            "int32 ExplicitMissingSourceCount = 0;", ""
        ))

    def test_explicit_missing_source_count_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 ExplicitMissingSourceCount"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tint32 ExplicitMissingSourceCounts = 0;\n",
            "\tint32 ExplicitMissingSourceCount = 1;\n",
            "\tbool ExplicitMissingSourceCount;\n",
            "\tfloat ExplicitMissingSourceCount = " + forty + ";\n",
            "\tfloat ExplicitMissingSourceCount = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit ExplicitMissingSourceCount field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "explicit_missing_source_count_field_decl_contract.py"
        ))
