...
# bActive FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission06 PayloadWindowRuntime
# bActive FIELD (declaration-only). THIS IS NOT leftover
...
# Integrity #1139 / in-flight bDestroyed. THIS IS NOT leftover
...
# bActive FIELD (declaration-only). It is
...
# This AirfieldTargetRuntime field sits after Target and before
# bDestroyed in the public section.
# Skip leftover b + Ya + kRuntimeReady forever (it sits
# after bMapAssemblyReady and before bGunnerReady). This contract
# is bActive before Target / RemainingSeconds / bJammed. THIS IS NOT
...
class Mission06PayloadWindowRuntimeActiveFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_active_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_active_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 Integrity field contract "
    ...
    def test_contract_is_active_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_active(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission06_payload_window_runtime_active"
            "_field_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_payload_window_runtime_active"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
