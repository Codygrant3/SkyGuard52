...
# INDEX_NONE or lock DebrisRightWing construction
...
# DebrisEngine and not leftover
# emergency-finish named surfaces
# (split tokens only). Keep RoadHunter
# DebrisRightWing distinct from leftover
# sibling DebrisEngine / DebrisLeftWing
# (same destruction mesh type, different
# field). Keep RoadHunter DebrisEngine
# distinct from leftover Breakwater
# DebrisEngine (different class; do not lock
# Breakwater here). Weak-point RightActuator
# must not satisfy this mesh field. Sibling
# fields TargetingCamera, LeftActuator,
# RightActuator, Engine, DebrisLeftWing,
# DebrisEngine must not satisfy the primary
# lock. This is not a leftover
...
# locked header. origin/main is a one-line
# field (`TObjectPtr<UStaticMeshComponent>
# DebrisRightWing;`); accept that form and
# other one-line / split-line wraps. The
# element type appears on origin/main and
# must be part of the locked field
# declaration. Nearby origin/main
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly,
# Category="Skyguard|Boss|Destruction") is
# required as present. Accept one-line and
# split-line UPROPERTY wraps. Parse the
# public class section of
# ASkyguardRoadHunterBoss only.
...
# isolated-test drafts #107–#550 including
...
# leftover isolated-test drafts #107–#550,
# leftover searchlight-track-runtime-defaults
...
# unlocked for THIS test's primary DebrisRightWing lock. Leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRoadHunterBoss::DebrisRightWing",
    "SkyguardRoadHunterBoss.cpp",
    "SetExposed",
...
class RoadHunterDebrisRightWingFieldDeclContractTests(unittest.TestCase):
    def test_road_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_RIGHT_WING_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_missing_debris_right_wing_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardRoadHunterBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("Category", DEBRIS_RIGHT_WING_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisRightWing;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisRightWing;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> DebrisRightWing;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_RIGHT_WING_FIELD)
            self.assertIn("DebrisRightWing", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_right_wing_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DEBRIS_RIGHT_WING_FIELD),
            DEBRIS_RIGHT_WING_FIELD,
        )
        self.assertTrue(has_declaration(section, DEBRIS_RIGHT_WING_FIELD))
        self.assertEqual(declaration_count(section, DEBRIS_RIGHT_WING_FIELD), 1)
        self.assertTrue(
            DEBRIS_RIGHT_WING_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_RIGHT_WING_FIELD,
        )
        self.assertTrue(DEBRIS_RIGHT_WING_FIELD.endswith(";"), DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("LeftActuator", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("RightActuator", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisLeftWing", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("Engine;", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisCoolingDoor", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("RadarReceiver", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("CoolingDoor", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_RIGHT_WING_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_RIGHT_WING_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("TArray<", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("=", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("{", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("}", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("return ", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("CommandAntenna", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ControlLinkage", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("NoseCamera", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisCenter", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisTail", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisNose", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisSpine", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("AdvanceEncounter", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ResetEncounterState", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("IsRouteStateSafe", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetRouteProgress", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("RadarNode", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisArmorPort", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("HullCollider", DEBRIS_RIGHT_WING_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {DEBRIS_RIGHT_WING_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DEBRIS_RIGHT_WING_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DEBRIS_RIGHT_WING_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DEBRIS_RIGHT_WING_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_RIGHT_WING_FIELD),
            section,
    ...
    def test_assigned_debris_right_wing_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisRightWing = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_RIGHT_WING_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("DebrisRightWing", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_RIGHT_WING_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_RIGHT_WING_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, DEBRIS_RIGHT_WING_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_RIGHT_WING_FIELD.startswith("UFUNCTION"),
            DEBRIS_RIGHT_WING_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_RIGHT_WING_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_RIGHT_WING_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_right_wing_cpp_body(self) -> None:
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        self.assertNotIn("{", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("}", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("return ", DEBRIS_RIGHT_WING_FIELD)
        ...
        self.assertNotIn("SkyguardRoadHunterBoss.cpp", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SkyguardRoadHunterBoss.cpp", locked_only)
        self.assertNotIn("return false", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("return true", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_RIGHT_WING_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_RIGHT_WING_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("PortDischargeBoom", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("EngineIntake", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisPortPanel", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisIntakePanel", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DEBRIS_RIGHT_WING_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_RIGHT_WING_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("EncounterController", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ControlServo", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DispenserCenter", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost_methods(
        ...
        for token in LEFTOVER_RADAR_GHOST_METHODS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("IsContactIdentified", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_open_rear_aspect_window(), DEBRIS_RIGHT_WING_FIELD)

    def test_debris_cooling_door_does_not_satisfy_debris_right_wing(self) -> None:
        leftover = f"\t{DEBRIS_COOLING_DOOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_RIGHT_WING_FIELD))
        self.assertNotEqual(DEBRIS_COOLING_DOOR_FIELD, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(
            "USkyguardBossWeakPointComponent",
            DEBRIS_COOLING_DOOR_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DispenserStarboard", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetManeuver", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_arm_emergency_finish(), locked_only)
        self.assertNotIn(leftover_arm_emergency_finish(), DEBRIS_RIGHT_WING_FIELD)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        self.assertNotIn("CoolingDoor", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SignatureModulator", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("IsLightningExposed", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DEBRIS_RIGHT_WING_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_RIGHT_WING_FIELD),
            DEBRIS_RIGHT_WING_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss::DebrisRightWing", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "road hunter DebrisRightWing field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_RIGHT_WING_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_RIGHT_WING_FIELD.lower(),
                "road hunter DebrisRightWing contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_RIGHT_WING_FIELD)

    def test_contract_is_debris_right_wing_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DEBRIS_RIGHT_WING_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_RIGHT_WING_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("PortLatch", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("StarboardLatch", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DecoyPods", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ElevatorLinkage", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", DEBRIS_RIGHT_WING_FIELD)
        self.assertIn(
            "Scripts/tests/test_breakwater_engine_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_road_hunter_debris_right_wing_is_not_breakwater_debris(self) -> None:
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        self.assertNotEqual(DEBRIS_RIGHT_WING_FIELD, BREAKWATER_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_RIGHT_WING_FIELD, BREAKWATER_DEBRIS_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_RIGHT_WING_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(collapsed(DEBRIS_RIGHT_WING_FIELD), collapsed(ENGINE_FIELD))
        ...
        self.assertNotIn("ASkyguardBreakwaterBoss", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardRadarGhostBoss", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardPathfinderBoss", DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("DebrisEngine", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_mission02_wave_state(
        ...
        for token in LEFTOVER_MISSION02_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", DEBRIS_RIGHT_WING_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter_fields(
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("OpticalTracker", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("WeaponServo", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("CountermeasurePod", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DEBRIS_RIGHT_WING_FIELD)


    def test_road_hunter_debris_right_wing_is_not_radar_ghost_debris(self) -> None:
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        self.assertNotEqual(DEBRIS_RIGHT_WING_FIELD, RADAR_GHOST_ENGINE_FIELD)
        self.assertNotEqual(DEBRIS_RIGHT_WING_FIELD, DEBRIS_COOLING_DOOR_FIELD)
        self.assertEqual(CLASS_NAME, "ASkyguardRoadHunterBoss")
        self.assertNotEqual(CLASS_NAME, "ASkyguardRadarGhostBoss")
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardRadarGhostBoss.h")
    ...
    def test_road_hunter_debris_right_wing_is_not_pathfinder_debris(self) -> None:
        locked_only = f"{DEBRIS_RIGHT_WING_FIELD}\n"
        self.assertNotEqual(
            collapsed(DEBRIS_RIGHT_WING_FIELD),
    ...
    def test_debris_engine_does_not_satisfy_debris_right_wing(self) -> None:
        leftover = f"\t{ROAD_HUNTER_DEBRIS_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("UStaticMeshComponent", ROAD_HUNTER_DEBRIS_ENGINE_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn(
            "USkyguardBossWeakPointComponent",
            DEBRIS_RIGHT_WING_FIELD,
    ...
    def test_targeting_camera_does_not_satisfy_debris_right_wing(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_RIGHT_WING_FIELD))
        self.assertNotEqual(TARGETING_CAMERA_FIELD, DEBRIS_RIGHT_WING_FIELD)
        self.assertNotIn("TargetingCamera", DEBRIS_RIGHT_WING_FIELD)
        self.assertIn("DebrisRightWing", DEBRIS_RIGHT_WING_FIELD)

    def test_left_and_right_actuators_do_not_satisfy_debris_right_wing(self) -> None:
        for region, name in (
            (f"\t{LEFT_ACTUATOR_FIELD}\n", "LeftActuator"),
            (f"\t{RIGHT_ACTUATOR_FIELD}\n", "RightActuator"),
        ):
