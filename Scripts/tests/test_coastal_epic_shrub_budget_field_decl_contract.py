# THIS IS leftover-safe ASkyguardCoastalEnvironmentDirector EpicShrubBudget.
...
# then `int32 EpicShrubBudget = 480;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Environment|Vegetation`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS `= 480`.
# This is a unique coastal director epic shrub-budget int32.
# It is NOT leftover FSkyguardEnvironmentReadiness.ShrubInstanceCount,
# NOT leftover sibling PlacementSeed / EpicTreeBudget / TreeInstances /
...
# or stay Category="Skyguard" without |Environment|Vegetation.
...
# Stop BEFORE sibling RouteLengthCm as a claimed slot.
# Do NOT claim leftover readiness ShrubInstanceCount, leftover
# ShrubInstances TObjectPtr, leftover TreeInstances, leftover
# PlacementSeed = 5201, leftover EpicTreeBudget = 240.
# Do NOT claim leftover analog apache-cpg-feel FireRate = 12.0f /
# RecoilPitch = 0.92f as this slot.
# Do NOT parse leftover analog coastal-env-director-empty-fail-closed
# (keep bulk in LOCKED_SCRIPTS).
# Do NOT parse leftover Apache UFUNCTION decls.
# Do NOT refill leftover TreeInstances / ShrubInstances / WindSource.
# Clone wrap was EditAnywhere BlueprintReadWrite Category Skyguard|Combat
# wrapping `float LookYawLimit = 95.f;` — RETARGET hard: wrap is
# EditAnywhere BlueprintReadWrite Category Skyguard|Environment|Vegetation
# wrapping `int32 EpicShrubBudget = 480;`.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity with
# NO initializer — RETARGET hard.
# LOCKED_DECL is `int32 EpicShrubBudget = 480;` NOT
# `FName WeatherIdentity;` (no initializer) and NOT
# leftover analog `float FireRate = 12.0f` /
# `float RecoilPitch = 0.92f` and NOT
# `int32 PlacementSeed = 5201;` and NOT
# `int32 EpicTreeBudget = 240;` and NOT leftover
# `int32 ShrubInstanceCount = 0;` and NOT leftover
# ShrubInstances TObjectPtr.
...
# Skyguard|Combat / Skyguard|Theater / Skyguard|MissionMap or
# still Category="Skyguard" without |Environment|Vegetation.
# Fail-closed if LOCKED_DECL is TObjectPtr / FName WeatherIdentity /
# leftover readiness ShrubInstanceCount / leftover ShrubInstances
# TObjectPtr.
# Harbor leftover is only the split 40 / 80 float tokens.
# EpicShrubBudget is 480 not Harbor forty / eighty.
...
TARGET = "int32 EpicShrubBudget = 480;"
TARGET_WRONG_BARE = "int32 EpicShrubBudget;"
TARGET_WRONG_INIT = "int32 EpicShrubBudget = nullptr;"
TARGET_WRONG_FALSE = "int32 EpicShrubBudget = false;"
TARGET_WRONG_TRUE = "int32 EpicShrubBudget = true;"
TARGET_WRONG_ZERO = "int32 EpicShrubBudget = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> EpicShrubBudget;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> EpicShrubBudget;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> EpicShrubBudget;"
...
TARGET_WRONG_COMBAT_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Combat")'
)
TARGET_WRONG_APACHE_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Apache")'
)
TARGET_WRONG_MISSION_MAP_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|MissionMap")'
)
TARGET_WRONG_BARE_SKYGUARD_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard")'
)
...
STOP_BEFORE_ROUTE_LENGTH = "RouteLengthCm"
...
SIBLING_SHRUB_INSTANCE_COUNT = "ShrubInstanceCount"
SIBLING_TREE_INSTANCE_COUNT = "TreeInstanceCount"
...
LEFTOVER_MISSION01_ROUTE_LENGTH = (
...
EPIC_SHRUB_BUDGET_DECL_RE = re.compile(
    r"int32\s+EpicShrubBudget\s*=\s*480\s*;"
)
EPIC_SHRUB_BUDGET_BARE_RE = re.compile(
    r"int32\s+EpicShrubBudget\s*;"
)
LOOK_YAW_LIMIT_DECL_RE = re.compile(
    r"float\s+LookYawLimit\s*=\s*95\.f\s*;"
)
PLACEMENT_SEED_DECL_RE = re.compile(
    r"int32\s+PlacementSeed\s*=\s*5201\b"
)
EPIC_TREE_BUDGET_DECL_RE = re.compile(
    r"int32\s+EpicTreeBudget\s*=\s*240\b"
)
SHRUB_INSTANCE_COUNT_DECL_RE = re.compile(
    r"int32\s+ShrubInstanceCount\b"
)
SHRUB_INSTANCES_DECL_RE = re.compile(
    r"TObjectPtr<UHierarchicalInstancedStaticMeshComponent>"
    r"\s+ShrubInstances\b"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_SHRUB_INSTANCE_COUNT,
        SIBLING_TREE_INSTANCE_COUNT,
        SIBLING_OCEAN_TILES,
        SIBLING_BEACH_TILES,
        SIBLING_LAND_TILES,
        SIBLING_CPG_STATION,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 EpicShrubBudget = 480;`.
    # Do not accept leftover FName WeatherIdentity,
    # leftover `= 160.f`, leftover analog apache-cpg-feel
    # FireRate / RecoilPitch, leftover PlacementSeed /
    # EpicTreeBudget, leftover readiness ShrubInstanceCount,
    # leftover ShrubInstances TObjectPtr, leftover
    # EpicShrubBudget = 95.f, leftover Harbor 40 / 80, or a
    # bare EpicShrubBudget without the 480 initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if EPIC_SHRUB_BUDGET_DECL_RE.search(compact) is None:
        return False
    if EPIC_SHRUB_BUDGET_BARE_RE.search(compact):
        return False
    ...
    if PLACEMENT_SEED_DECL_RE.search(compact):
        return False
    if EPIC_TREE_BUDGET_DECL_RE.search(compact):
        return False
    if SHRUB_INSTANCE_COUNT_DECL_RE.search(compact):
        return False
    if SHRUB_INSTANCES_DECL_RE.search(compact):
        return False
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_ROUTE_LENGTH:
            break
        if ident != "EpicShrubBudget":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_ROUTE_LENGTH):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_ROUTE_LENGTH}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+EpicShrubBudget\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 EpicShrubBudget "
...
class CoastalEpicShrubBudgetFieldDeclContractTests(unittest.TestCase):
    def test_coastal_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EpicShrubBudget"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_ROUTE_LENGTH), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_ROUTE_LENGTH),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_TREE_INSTANCES), body)
        self.assertFalse(has_identifier(section, SIBLING_TREE_INSTANCES), section)
        self.assertTrue(has_identifier(body, SIBLING_SHRUB_INSTANCES), body)
        self.assertFalse(
            has_identifier(section, SIBLING_SHRUB_INSTANCES),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_PLACEMENT_SEED), body)
        self.assertFalse(has_identifier(section, SIBLING_PLACEMENT_SEED), section)
        self.assertTrue(has_identifier(body, SIBLING_EPIC_TREE_BUDGET), body)
        self.assertFalse(has_identifier(section, SIBLING_EPIC_TREE_BUDGET), section)
        ...
        self.assertFalse(has_identifier(section, SIBLING_VFX_POOL), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_WIND_STRENGTH), body)
        self.assertFalse(has_identifier(section, SIBLING_WIND_STRENGTH), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EpicShrubBudget"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("EpicShrubBudget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_epic_shrub_budget_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMax="2048"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 EpicShrubBudget = 480;", compact_origin)
        self.assertNotIn("EpicShrubBudget = 160.f", compact_origin)
    ...
    def test_epic_shrub_budget_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 EpicShrubBudget"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("480", LOCKED_DECL)
        self.assertNotIn("5201", LOCKED_DECL)
        self.assertNotIn("240", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 EpicShrubBudget = 480;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SHRUB_INSTANCES}\n",
            f"\t{TARGET_WRONG_YAW}\n",
            f"\t{TARGET_WRONG_CANNON_MAG}\n",
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 EpicShrubBudgets = 480;\n",
            "\tfloat EpicShrubBudget = 480.f;\n",
            "\tbool EpicShrubBudget = true;\n",
            "\tfloat EpicShrubBudget = " + forty + ";\n",
            "\tfloat EpicShrubBudget = " + eighty + ";\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Environment|Vegetation"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Environment|Vegetation"', theater_specs)
        combat = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_COMBAT_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        ...
        mission_map = (
            ...
            "{\n"
            "public:\n"
            f"\t{TARGET_WRONG_MISSION_MAP_WRAP}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        mission_map_specs = attached_uproperty_specifiers(
            spec_section(mission_map)
        )
        self.assertIn('Category="Skyguard|MissionMap"', mission_map_specs)
        self.assertNotIn(
            'Category="Skyguard|Environment|Vegetation"',
            mission_map_specs,
        )
    ...
    def test_does_not_claim_sibling_route_length_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EpicShrubBudget"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH))
        ...
        self.assertFalse(has_identifier(section, SIBLING_PLACEMENT_SEED))
        self.assertFalse(has_identifier(section, SIBLING_EPIC_TREE_BUDGET))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "EpicShrubBudget"))
        self.assertTrue(has_identifier(section, "EpicShrubBudget"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH))
        self.assertTrue(has_identifier(leaked, SIBLING_TREE_INSTANCES))
        self.assertFalse(has_identifier(section, SIBLING_TREE_INSTANCES))
        self.assertTrue(has_identifier(leaked, SIBLING_SHRUB_INSTANCES))
        self.assertFalse(has_identifier(section, SIBLING_SHRUB_INSTANCES))
        self.assertTrue(has_identifier(leaked, SIBLING_PLACEMENT_SEED))
        self.assertFalse(has_identifier(section, SIBLING_PLACEMENT_SEED))
        self.assertTrue(has_identifier(leaked, SIBLING_EPIC_TREE_BUDGET))
        self.assertFalse(has_identifier(section, SIBLING_EPIC_TREE_BUDGET))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMax="2048"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "coastal EpicShrubBudget field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "coastal_epic_shrub_budget_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCoastalEnvironmentDirector.cpp", THIS_SCRIPT)
