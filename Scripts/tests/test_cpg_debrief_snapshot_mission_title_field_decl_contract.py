# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot MissionTitle.
...
# (`= TEXT("")` / `= FString()` / `= false`).
# Exact identifier vs sibling OutcomeNarrative (same type,
...
TARGET = "FString MissionTitle;"
TARGET_WRONG_TEXT = 'FString MissionTitle = TEXT("");'
TARGET_WRONG_FSTRING = "FString MissionTitle = FString();"
TARGET_WRONG_FALSE = "FString MissionTitle = false;"
TARGET_WRONG_TRUE = "FString MissionTitle = true;"
TARGET_WRONG_NONE = "FString MissionTitle = NAME_None;"
TARGET_WRONG_ZERO = "FString MissionTitle = 0;"
TARGET_WRONG_FNAME = "FName MissionTitle;"
TARGET_WRONG_BOOL = "bool MissionTitle;"
TARGET_WRONG_INT = "int32 MissionTitle;"
TARGET_WRONG_FLOAT = "float MissionTitle;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
SIBLING_NARRATIVE = "FString OutcomeNarrative;"
LOCKED_DECL = TARGET
...
SIBLING_WON = "bWon"
SIBLING_NARRATIVE_ID = "OutcomeNarrative"
SIBLING_SCORE = "Score"
SIBLING_MEDAL = "Medal"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString MissionTitle;`.
    # Do not accept `= TEXT("")` / `= FString()` / `= false` /
    # `= true` / leftover FName / sibling OutcomeNarrative.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FString\s+MissionTitle\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:FName|bool|int32|float|uint32)\s+MissionTitle\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString\s+MissionTitle\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString MissionTitle is missing from "
...
def require_plain_field(header: str) -> str:
    ...
    if "UPROPERTY" in section:
        raise AssertionError(
            f"{STRUCT_NAME} MissionTitle must stay a bare FString "
            "with no UPROPERTY"
        )
    for banned in (
        "VisibleAnywhere",
        ...
        if banned in section:
            raise AssertionError(
                f"{STRUCT_NAME} MissionTitle must not carry {banned}"
            )
    return LOCKED_DECL


class CpgDebriefSnapshotMissionTitleFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MissionTitle"), section)
    ...
    def test_capture_or_patrol_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MissionTitle"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        self.assertNotIn(STOP_BEFORE_PATROL, section)
        self.assertNotIn(STOP_BEFORE_COPY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MissionTitle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_mission_title_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{SIBLING_NARRATIVE}\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString MissionTitle;", compact_origin)
        self.assertNotIn("MissionTitle = TEXT(\"\")", compact_origin)
        self.assertNotIn("MissionTitle = FString()", compact_origin)
        self.assertNotIn("MissionTitle = false", compact_origin)
        self.assertNotIn("MissionTitle = true", compact_origin)
        self.assertNotIn("MissionTitle = NAME_None", compact_origin)

    def test_mission_title_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString MissionTitle"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{SIBLING_NARRATIVE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{SIBLING_NARRATIVE}\n", LOCKED_DECL)
        self.assertIn("MissionTitle", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\t{SIBLING_NARRATIVE}\n",
            "\tFString MissionTitles;\n",
            "\tFString Title;\n",
            "\tint32 MissionTitle;\n",
            "\tbool MissionTitle;\n",
            "\tfloat MissionTitle = " + forty + ";\n",
            "\tfloat MissionTitle = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_snapshot_fields(self) -> None:
        ...
        self.assertNotIn(SIBLING_NARRATIVE, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
            self.assertTrue(has_identifier(section, sibling), sibling)
        self.assertTrue(has_one_declaration(section, SIBLING_NARRATIVE), section)
        self.assertNotEqual(LOCKED_DECL, SIBLING_NARRATIVE)
        for field in leftover_neighbor_hit_fields():
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString MissionTitle;")
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot MissionTitle field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_title_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_NARRATIVE, locked_only)
