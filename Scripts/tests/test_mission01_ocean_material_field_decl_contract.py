...
# INDEX_NONE or lock OceanMaterial construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# OceanMaterial distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep OceanMaterial
...
# InlandExtentCm / SeawardExtentCm.
...
# Category is
# Skyguard|Mission01|Environment|Materials, not
# Environment alone, not Environment|Layout, not
# Environment|PCG, not Environment|Landscape, not
...
# OceanMaterial field declaration contract on
...
# BeachWidthCm / SeawardExtentCm / environment Root
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::OceanMaterial",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01OceanMaterialFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, OCEAN_MATERIAL_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, OCEAN_MATERIAL_FIELD))

    def test_missing_ocean_material_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("EditAnywhere", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("EditInstanceOnly", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("BlueprintReadWrite", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("VisibleAnywhere", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("BlueprintReadOnly", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("Category", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("ClampMin", OCEAN_MATERIAL_FIELD)
        ...
        self.assertNotIn("meta=", UPROPERTY_MISSION01_ENVIRONMENT_MATERIALS)
        self.assertNotEqual(
            UPROPERTY_MISSION01_ENVIRONMENT_MATERIALS,
            UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT,
        )
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, OCEAN_MATERIAL_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        as_array = (
            "\tTArray<TObjectPtr<UMaterialInterface>> OceanMaterial;\n"
        )
        float_type = "\tfloat OceanMaterial;\n"
        int_type = "\tint32 OceanMaterial = 0;\n"
        double_type = "\tdouble OceanMaterial = 0.0;\n"
        wrong_init = (
            "\tTObjectPtr<UMaterialInterface> OceanMaterial = nullptr;\n"
        )
        scene = "\tTObjectPtr<USceneComponent> OceanMaterial;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> OceanMaterial;\n"
        instanced = (
        ...
        leftover_beach_material = f"\t{BEACH_MATERIAL_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, OCEAN_MATERIAL_FIELD)
            self.assertIn("OceanMaterial", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_ocean_material_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, OCEAN_MATERIAL_FIELD),
            OCEAN_MATERIAL_FIELD,
        )
        self.assertTrue(has_declaration(section, OCEAN_MATERIAL_FIELD))
        self.assertEqual(
            declaration_count(section, OCEAN_MATERIAL_FIELD),
            1,
        )
        self.assertTrue(
            OCEAN_MATERIAL_FIELD.startswith(
                "TObjectPtr<UMaterialInterface> "
            ),
            OCEAN_MATERIAL_FIELD,
        )
        self.assertTrue(OCEAN_MATERIAL_FIELD.endswith(";"), OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", OCEAN_MATERIAL_FIELD)
        self.assertIn(
            "UMaterialInterface",
            OCEAN_MATERIAL_FIELD,
        )
        self.assertIn("TObjectPtr<", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("=", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("float", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("TArray<", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("TSoftObjectPtr", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("nullptr", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("INDEX_NONE", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("UFUNCTION", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("{", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("}", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("return ", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("OceanTiles", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("LandTiles", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("RouteExclusion", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("bRouteExclusionValid", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("BeachTiles", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("InlandVegetationPCG", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("LandScatterBounds", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("ProductionLandscape", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("AuthoredPCGGraph", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("RouteLengthCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("BeachWidthCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("DistrictLengthCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("SeawardExtentCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("InlandExtentCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("BeachMaterial", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("LandMaterial", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("OceanTiles", OCEAN_MATERIAL_FIELD)
        self.assertNotIn(
            "bUseAuthoredLandscapeSurface",
            OCEAN_MATERIAL_FIELD,
        ...
        self.assertNotIn("Root", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetReadiness", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("Briefing", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("AudioDirector", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("RadioChatter", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("SortiePresentation", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("CampaignDefinition", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("MissionDefinition", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("bAutoInitialize", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetAircraft", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("ConfigureFromMission", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("AdvanceBriefing", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("SetAssetsReady", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("CanLaunch", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetElapsedSeconds", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetBriefingState", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetBriefingText", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetRadioChatter", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetPresentation", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("GetMissionTitle", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("AcknowledgeBriefing", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("LaunchSortie", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("HullCollider", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("OpticalTracker", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("WeaponServo", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("CountermeasurePod", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("RadarNode", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("HarborIndustrial", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("MaxIntegrity", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("CurrentIntegrity", OCEAN_MATERIAL_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, OCEAN_MATERIAL_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_MATERIALS} {OCEAN_MATERIAL_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{OCEAN_MATERIAL_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, OCEAN_MATERIAL_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, OCEAN_MATERIAL_FIELD),
            section,
    ...
    def test_assigned_ocean_material_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UMaterialInterface> OceanMaterial = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, OCEAN_MATERIAL_FIELD))
        self.assertTrue(has_declaration(
            f"\t{OCEAN_MATERIAL_FIELD}\n",
            OCEAN_MATERIAL_FIELD,
        ))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("OceanMaterial", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, OCEAN_MATERIAL_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{OCEAN_MATERIAL_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, OCEAN_MATERIAL_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            OCEAN_MATERIAL_FIELD.startswith("UFUNCTION"),
            OCEAN_MATERIAL_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", OCEAN_MATERIAL_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_ocean_material_cpp_body(self) -> None:
        locked_only = f"{OCEAN_MATERIAL_FIELD}\n"
        self.assertNotIn("{", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("}", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("return ", OCEAN_MATERIAL_FIELD)
        ...
        self.assertNotIn("return false", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("return true", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("CreateDefaultSubobject", OCEAN_MATERIAL_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", OCEAN_MATERIAL_FIELD)
        self.assertTrue(
            OCEAN_MATERIAL_FIELD.startswith(
                "TObjectPtr<UMaterialInterface>"
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, OCEAN_MATERIAL_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, OCEAN_MATERIAL_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, OCEAN_MATERIAL_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("CurrentIntegrity", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", OCEAN_MATERIAL_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, OCEAN_MATERIAL_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", OCEAN_MATERIAL_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, "18000.f")
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OCEAN_MATERIAL_FIELD)
        self.assertNotIn("18000.f", leftover_harbor_tokens())
    ...
    def test_harbor_adjacent_fields_are_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual("18000.f", token)
            self.assertNotIn(token, leftover_harbor_tokens())

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{OCEAN_MATERIAL_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, OCEAN_MATERIAL_FIELD),
            OCEAN_MATERIAL_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, OCEAN_MATERIAL_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, OCEAN_MATERIAL_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 OceanMaterial field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{OCEAN_MATERIAL_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, OCEAN_MATERIAL_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                OCEAN_MATERIAL_FIELD.lower(),
                "mission01 OceanMaterial contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, OCEAN_MATERIAL_FIELD)

    def test_contract_is_ocean_material_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("OceanMaterial", OCEAN_MATERIAL_FIELD)
        self.assertTrue(OCEAN_MATERIAL_FIELD.endswith(";"))
        self.assertNotIn("=", OCEAN_MATERIAL_FIELD)
        self.assertIn("TObjectPtr<", OCEAN_MATERIAL_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_MATERIALS, section)

    def test_sibling_environment_fields_do_not_satisfy_ocean_material(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, OCEAN_MATERIAL_FIELD))
            self.assertNotEqual(OCEAN_MATERIAL_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_ocean_material(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            "\t/**\n"
            "\t * Mission 1 ocean material is an authored interface.\n"
            "\t * This comment is not the OceanMaterial declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, OCEAN_MATERIAL_FIELD)
        self.assertIn("OceanMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, OCEAN_MATERIAL_FIELD)
        )
        self.assertNotIn(
            "TObjectPtr<UMaterialInterface> OceanMaterial;",
            comment_only,
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, OCEAN_MATERIAL_FIELD)
            self.assertNotEqual(token, OCEAN_MATERIAL_FIELD)
        self.assertNotIn("45000.f", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("2800.f", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("5200.f", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("1800.f", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("7500.f", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("20000.f", OCEAN_MATERIAL_FIELD)
        self.assertNotIn("18000.f", OCEAN_MATERIAL_FIELD)
        self.assertNotEqual(OCEAN_MATERIAL_FIELD, INLAND_EXTENT_CM_FIELD)
        self.assertNotEqual(OCEAN_MATERIAL_FIELD, BEACH_MATERIAL_FIELD)
        self.assertNotEqual(OCEAN_MATERIAL_FIELD, LAND_MATERIAL_FIELD)
        self.assertNotEqual(OCEAN_MATERIAL_FIELD, OCEAN_TILES_FIELD)
