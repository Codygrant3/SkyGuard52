...
# values, sequence contents, or which beats HunterKiller returns.
# origin/main may split the factory as
# const FSkyguardDaySortieBeatKit& /
# HunterKiller();
HUNTER_KILLER = "const FSkyguardDaySortieBeatKit& HunterKiller();"
HUNTER_KILLER_HEAD = "HunterKiller("
# Leftover #56–#64 plus DaySortieBeatKit production sources/tests.
# This lane only adds an isolated Python HunterKiller
...
# Isolated-test drafts stay off this lane. Draft BrokenHighway()
# factory, in-flight DustOffensive() factory, ForMission (#273),
# SequencesDiffer (#269), BeatIndexForElapsed (#265), KindAt (#266),
# on-main beat-kit sequence contents, Beats[7] (#251), beat-kind
# enum (#244), beat defaults (#249), and kit FName fields (#256)
# stay sibling-only.
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_day_sortie_dust_offensive_decl_contract.py",
...
class DaySortieHunterKillerDeclContractTests(unittest.TestCase):
    def test_day_sortie_namespace_exists(self) -> None:
        ...
        self.assertEqual(
            require_declaration(body, HUNTER_KILLER),
            HUNTER_KILLER,
        )
    ...
    def test_missing_hunter_killer_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "{\n"
            "\tconst FSkyguardDaySortieBeatKit& BrokenHighway();\n"
            "\tconst FSkyguardDaySortieBeatKit& DustOffensive();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, HUNTER_KILLER)
        self.assertIn("HunterKiller", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(NAMESPACE_NAME, str(raised.exception))

    def test_hunter_killer_declaration_matches_origin_main(self) -> None:
        body = namespace_body(origin_main_header())
        self.assertEqual(
            require_declaration(body, HUNTER_KILLER),
            HUNTER_KILLER,
        )
        self.assertIn(HUNTER_KILLER_HEAD, body)
        self.assertEqual(body.count(HUNTER_KILLER_HEAD), 1)
        self.assertTrue(HUNTER_KILLER.endswith(";"), HUNTER_KILLER)
        self.assertNotIn("INDEX_NONE", HUNTER_KILLER)
        self.assertNotIn("return ", HUNTER_KILLER)

    def test_hunter_killer_declaration_may_be_split_across_lines(
        self,
    ) -> None:
        split_return = (
            "{\n"
        ...
        one_line = "{\n\t" + HUNTER_KILLER + "\n}\n"
        self.assertEqual(
            require_declaration(split_return, HUNTER_KILLER),
            HUNTER_KILLER,
        )
        self.assertEqual(
            require_declaration(split_parens, HUNTER_KILLER),
            HUNTER_KILLER,
        )
        self.assertEqual(
            require_declaration(one_line, HUNTER_KILLER),
            HUNTER_KILLER,
        )
        self.assertIn(HUNTER_KILLER_HEAD, split_return)
        self.assertIn(HUNTER_KILLER_HEAD, split_parens)
        body = namespace_body(origin_main_header())
        self.assertEqual(
            require_declaration(body, HUNTER_KILLER),
            HUNTER_KILLER,
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", HUNTER_KILLER)
        self.assertNotIn("INDEX_NONE", HUNTER_KILLER)
        self.assertNotIn("NAME_None", HUNTER_KILLER)
        self.assertNotIn("{", HUNTER_KILLER)
        self.assertNotIn("}", HUNTER_KILLER)
        ...
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, HUNTER_KILLER)
            if token not in ("return ", "HunterKillerKit()"):
    ...
    def test_declaration_does_not_invent_which_beats_the_factory_returns(
        ...
        body = namespace_body(origin_main_header())
        self.assertNotIn("return HunterKiller()", HUNTER_KILLER)
        self.assertNotIn("return HunterKillerKit()", HUNTER_KILLER)
        self.assertNotIn("return BrokenHighway()", HUNTER_KILLER)
        self.assertNotIn("return DustOffensive()", HUNTER_KILLER)
        self.assertNotIn("return HunterKiller()", body)
        self.assertNotIn("return HunterKillerKit()", body)
        self.assertNotIn("HunterKillerKit()", HUNTER_KILLER)
        self.assertNotIn("HunterKillerKit()", body)
        self.assertNotIn("Beats[0]", HUNTER_KILLER)
        self.assertNotIn("Beats[0]", body)
        for name in BEAT_KINDS_NOT_LOCKED:
            self.assertNotIn(name, HUNTER_KILLER)
            self.assertNotIn(name, body)
        self.assertNotEqual(HUNTER_KILLER, "BrokenHighway()")
        self.assertNotEqual(HUNTER_KILLER, "DustOffensive()")

    def test_contract_does_not_relock_kit_sequences(self) -> None:
        locked_only = f"{HUNTER_KILLER}\n"
        self.assertEqual(
            require_declaration(locked_only, HUNTER_KILLER),
            HUNTER_KILLER,
        )
    ...
    def test_contract_does_not_relock_sequences_differ(self) -> None:
        ...
        self.assertNotIn("SequencesDiffer", HUNTER_KILLER)
    ...
    def test_contract_does_not_relock_beat_index(self) -> None:
        ...
        self.assertNotIn("BeatIndexForElapsed", HUNTER_KILLER)
    ...
    def test_contract_does_not_relock_kind_at(self) -> None:
        ...
        self.assertNotIn("KindAt", HUNTER_KILLER)
        self.assertNotIn("KindAt", locked_only)
        self.assertNotIn("ESkyguardDaySortieBeatKind KindAt(", (HUNTER_KILLER,))
    ...
    def test_contract_does_not_relock_beats_enum_defaults_or_fields(
        ...
        self.assertNotIn(BEATS_NOT_LOCKED, HUNTER_KILLER)
    ...
    def test_contract_does_not_read_cpp_or_which_beats_tables(self) -> None:
        ...
        self.assertNotIn("SkyguardDaySortieBeatKit::HunterKiller", body)
        self.assertNotIn("return BrokenHighway()", body)
        self.assertNotIn("return DustOffensive()", body)
        self.assertNotIn("return HunterKiller()", body)
        self.assertNotIn("HunterKillerKit()", body)
        self.assertNotIn("DuskIngress", body)
        self.assertNotIn("TelStrike", body)
        self.assertNotIn("SensorTrack", body)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(HUNTER_KILLER, "Rifle")
        self.assertNotEqual(HUNTER_KILLER, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"day-sortie HunterKiller contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not Yak",
            )
            self.assertNotIn(banned, HUNTER_KILLER.lower())

    def test_contract_is_hunter_killer_declaration_only(self) -> None:
        header = origin_main_header()
        body = namespace_body(header)
        self.assertIn(f"namespace {NAMESPACE_NAME}", header)
        self.assertEqual(
        ...
        for neighbor in UNLOCKED_NEIGHBORS:
            ...
            self.assertNotIn(neighbor, HUNTER_KILLER)
        for name in KIT_SEQUENCES_NOT_LOCKED:
            self.assertNotIn(name, locked_only)
        self.assertNotIn("SequencesDiffer", locked_only)
        self.assertNotIn("BeatIndexForElapsed", locked_only)
