# THIS IS leftover-safe ASkyguardApacheAircraft RadarDome.
...
# then BARE `TObjectPtr<UStaticMeshComponent> RadarDome;`
# (no initializer).
...
# Stop BEFORE sibling MainRotorCross as a claimed slot.
# Do NOT claim sibling EngineLeft / EngineRight /
# MainRotorCross / HorizontalTail / PilotCanopy /
# NightVisionTurret / SensorBall / Canopy / Fuselage /
# AircraftRoot / SilhouetteMesh.
# RadarDome is NOT leftover analog apache-set-sensor-view
# UFUNCTION, NOT leftover analog apache-get-sensor-quality
# UFUNCTION, NOT leftover analog apache-is-canopy-glass-cracked
# UFUNCTION, NOT leftover Canopy #1559, NOT leftover
# EngineLeft / EngineRight (in-flight/siblings), NOT leftover
# analog apache-aircraft-empty-fail-closed #851b, NOT leftover
# analog apache-hull-collider, NOT leftover analog
# apache-mount-fail-closed.
...
# leftover analog apache-cpg-feel, leftover ProtectAsset
# MaxIntegrity / CurrentIntegrity, leftover LoadoutSpec
# HullIntegrity, leftover TheaterKit WeatherIdentity,
# leftover ArcadeLook UPROPERTY, leftover GuidedLockRules,
# leftover CampaignMissionSpec, leftover Harbor Breaker
# Approach/Contact/Shore, leftover campaign-roster
# function decls / analog lookup tests.
# Already-opened leftover Apache fields stay leftover:
# TailRotor, ChinTurret, MainRotor, RotorMast,
# StubWingRight, VerticalTail, StubWingLeft, TailBoom,
# Fuselage, Canopy, Nose, AircraftRoot, SilhouetteMesh,
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> RadarDome;`
# NOT `FName WeatherIdentity;`, NOT leftover sibling
# `TObjectPtr<UStaticMeshComponent> Canopy;`, NOT leftover
# `TObjectPtr<UStaticMeshComponent> EngineLeft;`, NOT leftover
# `TObjectPtr<UStaticMeshComponent> EngineRight;`, NOT leftover
# `TObjectPtr<UStaticMeshComponent> MainRotorCross;`, and NOT
# `TObjectPtr<USceneComponent>`.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# / TObjectPtr<USceneComponent> / leftover Canopy /
# leftover EngineLeft / leftover EngineRight /
# leftover MainRotorCross AS THE LOCKED DECL.
...
TARGET = "TObjectPtr<UStaticMeshComponent> RadarDome;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> RadarDome = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> RadarDome = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> RadarDome = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> RadarDome;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> RadarDome;"
...
STOP_BEFORE_MAIN_ROTOR_CROSS = "MainRotorCross"
...
RADAR_DOME_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+RadarDome\s*;"
)
RADAR_DOME_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+RadarDome\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover sibling Canopy / EngineLeft / EngineRight /
    # MainRotorCross / Fuselage / AircraftRoot /
    # USceneComponent, or an initializer on RadarDome
    # as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if RADAR_DOME_DECL_RE.search(compact) is None:
        return False
    if RADAR_DOME_INIT_RE.search(compact):
        return False
    if WEATHER_IDENTITY_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"TObjectPtr<USceneComponent>\s+RadarDome\b", compact):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_MAIN_ROTOR_CROSS):
            break
    ...
    if has_identifier(window, SIBLING_MAIN_ROTOR_CROSS):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_MAIN_ROTOR_CROSS}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+RadarDome\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> RadarDome "
...
class ApacheRadarDomeFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RadarDome"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RadarDome"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("RadarDome", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_radar_dome_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> RadarDome;", compact_origin)
        self.assertNotIn("RadarDome = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_radar_dome_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> RadarDome"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> RadarDome;")
        ...
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> EngineLeft;")
        self.assertNotEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> EngineRight;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> RadarDomes;\n",
            "\tint32 RadarDome = 140;\n",
            "\tbool RadarDome = true;\n",
            "\tfloat RadarDome = " + forty + ";\n",
            "\tfloat RadarDome = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_main_rotor_cross_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache RadarDome field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radar_dome_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
