...
# INDEX_NONE or lock RouteCorridorHalfWidthCm construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# RouteCorridorHalfWidthCm distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep RouteCorridorHalfWidthCm
...
# DistrictLengthCm (in-flight this wave) /
...
# one-line / split-line wraps. Locked 2800.f is
...
# meta=(ClampMin="100.0")) is required as
...
# 2800.f value. Do not lock leftover
...
ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD = "float RouteCorridorHalfWidthCm = 2800.f;"
...
LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE = "2800.f"
...
# leftover theater-kit #59, leftover #107–#591, plus
# SkyguardMission01EnvironmentDirector production
# files. This lane only adds an isolated Python
# RouteCorridorHalfWidthCm field declaration contract on
...
# (#568/#572/#574–#591), leftover briefing-widget
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::RouteCorridorHalfWidthCm",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01RouteCorridorHalfWidthCmFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD))

    def test_missing_route_corridor_half_width_cm_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin="100.0"', section)
        self.assertTrue(
            has_declaration(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD),
            section,
        )
        self.assertNotIn("UPROPERTY", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("EditAnywhere", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("EditInstanceOnly", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("BlueprintReadWrite", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("VisibleAnywhere", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("BlueprintReadOnly", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("Category", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("ClampMin", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        ...
        self.assertIn('ClampMin="100.0"', UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT)
        self.assertNotEqual(
            UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT,
            UPROPERTY_MISSION01_ENVIRONMENT,
        )
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tfloat* RouteCorridorHalfWidthCm;\n"
        )
        as_array = (
            "\tTArray<float> RouteCorridorHalfWidthCm;\n"
        )
        no_init = "\tfloat RouteCorridorHalfWidthCm;\n"
        int_type = "\tint32 RouteCorridorHalfWidthCm = 45000;\n"
        double_type = "\tdouble RouteCorridorHalfWidthCm = 2800.f;\n"
        wrong_init = "\tfloat RouteCorridorHalfWidthCm = 0.f;\n"
        wrong_scale = "\tfloat RouteCorridorHalfWidthCm = 40000.f;\n"
        scene = "\tTObjectPtr<USceneComponent> RouteCorridorHalfWidthCm;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> RouteCorridorHalfWidthCm;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> RouteCorridorHalfWidthCm;\n"
        )
        weak = (
        ...
        assigned = (
            "\tfloat RouteCorridorHalfWidthCm = nullptr;\n"
        )
        soft = (
        ...
        leftover_corridor = f"\t{ROUTE_LENGTH_CM_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
            self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_route_corridor_half_width_cm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD),
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD,
        )
        self.assertTrue(has_declaration(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD))
        self.assertEqual(
            declaration_count(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD),
            1,
        )
        self.assertTrue(
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD.startswith("float RouteCorridorHalfWidthCm"),
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD,
        )
        self.assertTrue(ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD.endswith(";"), ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn(
            "float",
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD,
        )
        self.assertIn("=", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("TObjectPtr<", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("TArray<", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("TSoftObjectPtr", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("nullptr", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("INDEX_NONE", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("UFUNCTION", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("{", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("}", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("return ", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("OceanTiles", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("LandTiles", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("RouteExclusion", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("bRouteExclusionValid", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("BeachTiles", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("InlandVegetationPCG", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("LandScatterBounds", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("ProductionLandscape", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("AuthoredPCGGraph", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("DistrictLengthCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("RouteLengthCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("BeachWidthCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("InlandExtentCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("SeawardExtentCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD,
        )
        ...
        self.assertNotIn("Root", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetReadiness", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("Briefing", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("AudioDirector", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("RadioChatter", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("SortiePresentation", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("CampaignDefinition", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("MissionDefinition", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("bAutoInitialize", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetAircraft", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("ConfigureFromMission", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("AdvanceBriefing", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("SetAssetsReady", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("CanLaunch", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetElapsedSeconds", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetBriefingState", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetBriefingText", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetRadioChatter", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetPresentation", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("GetMissionTitle", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("AcknowledgeBriefing", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("LaunchSortie", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("HullCollider", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("OpticalTracker", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("WeaponServo", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("CountermeasurePod", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("RadarNode", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("HarborIndustrial", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("MaxIntegrity", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tfloat\n"
            "\tRouteCorridorHalfWidthCm = 2800.f;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT} {ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD),
            section,
    ...
    def test_assigned_route_corridor_half_width_cm_does_not_satisfy(self) -> None:
        assigned = (
            "\tfloat RouteCorridorHalfWidthCm = 0.f;\n"
        )
        missing_init = "\tfloat RouteCorridorHalfWidthCm;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD))
        with self.assertRaises(AssertionError) as raised_missing:
            require_declaration(missing_init, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", str(raised_missing.exception))
        self.assertFalse(has_declaration(missing_init, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD.startswith("UFUNCTION"),
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_route_corridor_half_width_cm_cpp_body(self) -> None:
        locked_only = f"{ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD}\n"
        self.assertNotIn("{", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("}", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("return ", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        ...
        self.assertNotIn("return false", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("return true", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertTrue(
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD.startswith("float RouteCorridorHalfWidthCm")
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, leftover_harbor_tokens())
    ...
    def test_harbor_adjacent_fields_are_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, token)
            self.assertNotIn(token, leftover_harbor_tokens())

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD),
            ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 RouteCorridorHalfWidthCm field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD.lower(),
                "mission01 RouteCorridorHalfWidthCm contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)

    def test_contract_is_route_corridor_half_width_cm_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("RouteCorridorHalfWidthCm", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertTrue(ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD.endswith(";"))
        self.assertIn("=", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotIn("TObjectPtr<", ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT, section)

    def test_sibling_environment_fields_do_not_satisfy_route_corridor_half_width_cm(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD))
            self.assertNotEqual(ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_route_corridor_half_width_cm(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            ...
            "\t * This comment is not the RouteCorridorHalfWidthCm declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn("RouteCorridorHalfWidthCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        )
        self.assertNotIn("float RouteCorridorHalfWidthCm = 2800.f;", comment_only)
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
            self.assertNotEqual(token, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertIn(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, ROUTE_CORRIDOR_HALF_WIDTH_CM_FIELD)
        self.assertNotEqual(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, "7500.f")
        self.assertNotEqual(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, "45000.f")
        self.assertNotEqual(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, "5200.f")
        self.assertNotEqual(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, "1800.f")
        self.assertNotEqual(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, "18000.f")
        self.assertNotEqual(LOCKED_ROUTE_CORRIDOR_HALF_WIDTH_VALUE, "20000.f")
