# THIS IS leftover-safe ASkyguardGunner CpgRailLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgRailLeft;`
# (no initializer).
...
# Stop BEFORE sibling CpgRailRight as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeLeft, CpgKneeRight, CpgCanopyBow,
# CpgDash, CpgTedac, CpgMpdLeft, CpgMpdRight,
# CpgEufd, CpgTedacBezel, CpgReticleH, CpgReticleV,
# CpgGripLeft, CpgGripRight, CpgConsoleRight, CpgRailRight,
# CpgTedacText, Boom, GunnerCamera, HandMesh.
# Do NOT claim leftover CpgEufd / CpgTedac / CpgDash /
# CpgCanopyBow / CpgKneeRight / CpgKneeLeft / CpgSeatBack /
# CpgCockpit / CpgStation (in-flight siblings). CpgMpdLeft
# and CpgMpdRight are leftover siblings. CpgTedacBezel
# and CpgReticleH are sibling in-flight slots.
# Do NOT claim CpgReticleH, CpgReticleV, CpgTedacBezel,
# CpgEufd, CpgGripLeft, CpgGripRight, CpgConsoleRight,
# CpgRailRight, CpgTedacText, or HandMesh as this slot.
# CpgRailLeft fail-closed if leftover Apache Canopy
# or leftover Apache PilotCanopy — do not lock those.
# Fail-closed leftover Apache GunnerMount / PilotMount /
# EyeMount / WeaponMount. Fail-closed leftover
# CpgTedacBezel / CpgReticleH / CpgReticleV /
# CpgGripLeft / CpgGripRight. Do not refill those.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgRailLeft;`
...
# CpgRailLeft fail-closed if LOCKED_DECL is leftover
# CpgEufd / CpgTedacBezel / CpgReticleH / CpgReticleV /
# leftover CpgGripLeft / leftover CpgGripRight /
# CpgTedac / CpgDash / CpgKneeRight / CpgKneeLeft /
# CpgSeatBack / CpgCockpit / CpgStation / CpgSeatPan /
# CpgCanopyBow / CpgMpdLeft / CpgMpdRight / leftover
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgRailLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgRailLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgRailLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgRailLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgRailLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgRailLeft;"
...
TARGET_WRONG_TEXT = "TObjectPtr<UTextRenderComponent> CpgRailLeft;"
...
STOP_BEFORE_CONSOLE_RIGHT = "CpgConsoleRight"
...
CPG_RAIL_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgRailLeft\s*;"
)
CPG_RAIL_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgRailLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgEufd / CpgTedacBezel / CpgReticleH /
    # leftover CpgReticleV / leftover CpgGripLeft /
    # leftover CpgGripRight / CpgTedac / CpgDash /
    # CpgCanopyBow / CpgSeatBack / CpgSeatPan / CpgCockpit /
    # CpgStation / leftover Apache Canopy, or an
    # initializer on CpgRailLeft as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_RAIL_LEFT_DECL_RE.search(compact) is None:
        return False
    if CPG_RAIL_LEFT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_RAIL_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_RAIL_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_RAIL_RIGHT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgRailLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgRailLeft "
...
class GunnerCpgRailLeftFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgRailLeft"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_RAIL_RIGHT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_RAIL_RIGHT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgRailLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgRailLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_rail_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgRailLeft;", compact_origin)
        self.assertNotIn("CpgRailLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_rail_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgRailLeft"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgRailLeft;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgRailLefts;\n",
            "\tint32 CpgRailLeft = 140;\n",
            "\tbool CpgRailLeft = true;\n",
            "\tfloat CpgRailLeft = " + forty + ";\n",
            "\tfloat CpgRailLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_rail_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "CpgRailLeft"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgRailLeft"))
        self.assertTrue(has_identifier(section, "CpgRailLeft"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgRailLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_rail_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
