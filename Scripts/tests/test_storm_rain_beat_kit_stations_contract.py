...
LOCKED_STATIONS = "ESkyguardGunshipWeapon Stations[BeatCount] = {};"
# Leftover #56–#64 plus StormRainBeatKit production sources. This lane
# only adds an isolated Python Stations[BeatCount] contract.
LOCKED = {
...
# Isolated-test drafts #107–#254 and newer stay off this lane.
# Storm-rain-beat-kind enum (#245), kit defaults (#248), Kinds[BeatCount]
# (opening), Threats[BeatCount] (in-flight), day/night Beats[7]
# (#251/#252), day-kit fields (opening), night-kit fields (#254), and
# loadout defaults (#253) stay sibling-only.
...
# Apache CPG storm-rain kit Stations[BeatCount] only. BeatCount is the
# array bound identifier. Do not re-lock BeatCount=7 / Weather=Storm /
# bHydraForClusters=true (#248). Live station type is
# ESkyguardGunshipWeapon (Cannon / Rockets / GuidedMissile).
DEFAULTS_NOT_LOCKED = (
...
GUNSHIP_WEAPON_ENUMERATORS_NOT_LOCKED = (
    "Cannon",
    "Rockets",
    "GuidedMissile",
)
...
def stations_declaration(body: str) -> str:
    match = re.search(
        r"ESkyguardGunshipWeapon\s+Stations\[BeatCount\]\s*=\s*\{\};",
        body,
    )
    if match is None:
        raise AssertionError(
            f"ESkyguardGunshipWeapon Stations[BeatCount] is missing from "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME}"
        )
    return match.group(0)


def stations_array_bound(body: str) -> str:
    match = re.search(
        r"ESkyguardGunshipWeapon\s+Stations\[([A-Za-z_][A-Za-z0-9_]*)\]\s*=\s*\{\};",
        body,
    )
    if match is None:
        raise AssertionError(
...
class StormRainBeatKitStationsContractTests(unittest.TestCase):
    ...
    def test_missing_stations_declaration_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            stations_declaration(
                "{\n\tstatic constexpr int32 BeatCount = 7;\n"
                "\tESkyguardStormRainBeatKind Kinds[BeatCount] = {};\n"
                "\tESkyguardThreatKind Threats[BeatCount] = {};\n};\n"
            )
        self.assertIn("Stations[BeatCount]", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(STRUCT_NAME, str(raised.exception))

    def test_stations_is_declared_as_beatcount_array(self) -> None:
        body = kit_body(origin_main_header())
        self.assertEqual(stations_declaration(body), LOCKED_STATIONS)
        self.assertIn(LOCKED_STATIONS, body)
        self.assertEqual(stations_array_bound(body), "BeatCount")
        self.assertNotEqual(stations_array_bound(body), "7")
        self.assertNotEqual(stations_array_bound(body), "INDEX_NONE")
        self.assertNotIn("Stations[7]", body)
        self.assertNotIn("Stations[6]", body)
        self.assertNotIn("Stations[8]", body)
        self.assertNotIn("Stations[INDEX_NONE]", body)
    ...
    def test_contract_does_not_relock_kit_defaults(self) -> None:
        declaration = stations_declaration(kit_body(origin_main_header()))
        self.assertEqual(declaration, LOCKED_STATIONS)
    ...
    def test_contract_does_not_lock_unlocked_members(self) -> None:
        ...
        for token in UNLOCKED_ARRAY_DECLARATIONS:
            self.assertNotEqual(declaration, token)
            self.assertNotIn(token, declaration)
        self.assertNotIn("Kinds[BeatCount]", declaration)
    ...
    def test_contract_does_not_lock_gunship_weapon_enumerators(self) -> None:
        declaration = stations_declaration(kit_body(origin_main_header()))
        self.assertEqual(declaration, LOCKED_STATIONS)
        self.assertIn("ESkyguardGunshipWeapon", declaration)
        self.assertNotIn("enum class ESkyguardGunshipWeapon", declaration)
        for name in GUNSHIP_WEAPON_ENUMERATORS_NOT_LOCKED:
            self.assertNotIn(f"ESkyguardGunshipWeapon::{name}", declaration)
            self.assertNotIn(name, declaration)
        self.assertNotIn("Igla", declaration)
        self.assertNotEqual(
            declaration,
            "ESkyguardGunshipWeapon Igla[BeatCount] = {};",
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        body = kit_body(origin_main_header())
        declaration = stations_declaration(body)
        self.assertEqual(declaration, LOCKED_STATIONS)
        ...
        self.assertNotEqual(
            declaration,
            "ESkyguardGunshipWeapon Rifle[BeatCount] = {};",
        )
        self.assertNotEqual(
            declaration,
            "ESkyguardGunshipWeapon Igla[BeatCount] = {};",
    ...
    def test_struct_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                ...
                "storm-rain beat-kit Stations[BeatCount] is Apache CPG "
                "30 mm / Hydra / Hellfire, not Yak",
            )

    def test_contract_is_storm_rain_beat_kit_stations_only(self) -> None:
        header = origin_main_header()
        body = kit_body(header)
        declaration = stations_declaration(body)
        self.assertIn(f"struct {STRUCT_NAME}", header)
        ...
        self.assertNotIn("enum class", body)
        self.assertNotIn("namespace SkyguardStormRainBeatKits", body)
        self.assertNotEqual(declaration, "Rifle Stations[BeatCount] = {};")
        self.assertNotEqual(
            declaration,
            "ESkyguardGunshipWeapon Igla[BeatCount] = {};",
        )
