...
# then static FSkyguardMission01EnvironmentAuthoringResult
# AuthorGovernedLandscapeAndGraph(
...
# Skyguard|Mission01|Environment|Authoring.
# Leftover unique AuthoringResult FIELD drafts
# wrap struct members with NO Category and explicitly
# do NOT parse this library.
# Identifier AuthorGovernedLandscapeAndGraph is NOT leftover
# analog struct field bSuccess / Landscape / Graph and NOT
# leftover unique SampleLandscapeHeight / SampleLandscapeFootprint
# (Grounding) and NOT sibling
...
# BlueprintCallable IS present. const IS absent.
...
# without |Authoring / Category="Skyguard|Mission01|Environment|Grounding"
...
# Do NOT claim siblings AuthorGovernedLandscapeWithExistingGraph /
# AuditGovernedLandscapeAndGraph as this slot
# (leave them uncontracted).
# Do NOT claim leftover unique SampleLandscapeHeight /
# SampleLandscapeFootprint.
# Do NOT claim GetActiveRHIAndFeatureLevel /
...
# wrapping leftover unique void AuthorGovernedLandscapeAndGraph();
# on USkyguardAudioProductionBank — RETARGET hard:
# class -> USkyguardMission01EnvironmentAuthoringLibrary,
# Category Production -> Skyguard|Mission01|Environment|Authoring,
# decl -> AuthorGovernedLandscapeAndGraph static struct-return
# (not void AuthorGovernedLandscapeAndGraph).
# Clone leftover unique BlueprintPure GetSortieHits
# Category Skyguard|Combat|Sortie — RETARGET Category
# Combat|Sortie -> Skyguard|Mission01|Environment|Authoring.
...
# LOCKED_DECL is AuthorGovernedLandscapeAndGraph(
...
# bSuccess = false / AuthorGovernedLandscapeWithExistingGraph /
...
# void AuthorGovernedLandscapeAndGraph
# AS THE LOCKED DECL.
...
TARGET_WRONG_FLOAT = (
    "float AuthorGovernedLandscapeAndGraph();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void AuthorGovernedLandscapeAndGraphs();"
...
TARGET_WRONG_TRUE = "bool AuthorGovernedLandscapeAndGraph = true;"
TARGET_WRONG_ZERO = "bool AuthorGovernedLandscapeAndGraph = 0.f;"
...
SIBLING_GET_DISPLAY = "PrepareGovernedLandscapeForVisibleValidation"
...
STOP_BEFORE_SKYLINE_STYLE = "PrepareGovernedLandscapeForVisibleValidation"
...
METHOD_SIGNATURE_RE = re.compile(
    ...
    r"AuthorGovernedLandscapeAndGraph\("
    r"ASkyguardMission01EnvironmentDirector\* Director,"
    r"const FString& HeightmapSourcePath,"
    r"const FString& GraphPackagePath\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # rename, leftover unique AuthorGovernedLandscapeAndGraph,
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bAuthorGovernedLandscapeAndGraph\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+AuthorGovernedLandscapeAndGraph\b", compact):
        return False
    if re.search(r"\bbool\s+AuthorGovernedLandscapeAndGraph\b", compact):
        return False
    if re.search(r"\bfloat\s+AuthorGovernedLandscapeAndGraph\b", compact):
        return False
    if re.search(r"\bint32\s+AuthorGovernedLandscapeAndGraph\b", compact):
        return False
...
def spec_section(header: str) -> str:
    ...
    authoring = 'Category="Skyguard|Mission01|Environment|Authoring"'
    if authoring in section:
        idx = section.find(authoring)
        uf = section.rfind("UFUNCTION", 0, idx)
        if uf != -1:
            section = section[uf:]
        validation = 'Category="Skyguard|Mission01|Environment|Validation"'
        val = section.find(validation)
        if val != -1:
            uf2 = section.rfind("UFUNCTION", 0, val)
            if uf2 > 0:
                section = section[:uf2]
        grounding = 'Category="Skyguard|Mission01|Environment|Grounding"'
        ground = section.find(grounding)
        if ground != -1:
            uf3 = section.rfind("UFUNCTION", 0, ground)
            if uf3 > 0:
                section = section[:uf3]
    if STOP_BEFORE_PRIVATE in section:
        raise AssertionError(
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            ...
            r"AuthorGovernedLandscapeAndGraph\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for AuthorGovernedLandscapeAndGraph "
...
class Mission01AuthorGovernedLandscapeAndGraphDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "AuthorGovernedLandscapeAndGraph"),
            section,
        )
    ...
    def test_missing_author_governed_landscape_and_graph_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "AuthorGovernedLandscapeAndGraph()const",
    ...
    def test_author_governed_landscape_and_graph_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid AuthorGovernedLandscapeAndGraphX();\n",
            "\tint32 AuthorGovernedLandscapeAndGraph = 1;\n",
            "\tFName AuthorGovernedLandscapeAndGraph = NAME_None;\n",
            "\tvoid AuthorGovernedLandscapeAndGraph() = "
            + forty
            + ";\n",
            "\tvoid AuthorGovernedLandscapeAndGraph() = "
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 author governed landscape and graph "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_author_governed_landscape_and_graph"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
