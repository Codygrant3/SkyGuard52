...
# BlueprintReadOnly int32 ValidSampleCount = 0; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# ValidSampleCount FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 ValidSampleCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# ValidSampleCount FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# int32 ValidSampleCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeHeightSample ValidSampleCount
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 ValidSampleCount = 0;
...
# ValidSampleCount FIELD (declaration-only). It is
...
TARGET = "int32 ValidSampleCount = 0;"
TARGET_WRONG = "float ValidSampleCount = 0;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+ValidSampleCount\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 ValidSampleCount is missing from "
...
class LandscapeFootprintSampleValidSampleCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("ValidSampleCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_valid_sample_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_valid_sample_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 ValidSampleCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("ValidSampleCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("ValidSampleCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 ValidSampleCount"))
