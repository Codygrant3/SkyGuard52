...
# Do NOT parse leftover #1332-#1353 RadioLine / RoutePoint /
# EnemyWave / EnemyFormation / WeatherProfile isolated
# field drafts.
# Do NOT parse leftover TheaterKit #1299-#1331.
# Do NOT contract sibling fields PerfectAccuracyBonus /
# NoDamageBonus / BronzeThreshold / SilverThreshold /
# GoldThreshold.
...
# isolated CompletionScore field decl, same as leftover
...
# `FName WeatherIdentity` / `float CompletionScore`
# / `int32 UnitCount` / no `=` / ClampMin missing,
# retarget: type is int32 CompletionScore,
...
TARGET = "int32 CompletionScore = 5000;"
TARGET_WRONG_EQ_NONE = "int32 CompletionScore = NAME_None;"
TARGET_WRONG_FALSE = "int32 CompletionScore = false;"
TARGET_WRONG_TRUE = "int32 CompletionScore = true;"
TARGET_WRONG_ZERO = "int32 CompletionScore = 0;"
TARGET_WRONG_FLOAT = "float CompletionScore = 5000;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "int32 CompletionScore;"
TARGET_WRONG_BONUS = "int32 CompletionScore = 2500;"
TARGET_WRONG_FNAME = "FName CompletionScore;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling BronzeThreshold = 5000. Do not accept
    # sibling PerfectAccuracyBonus / NoDamageBonus /
    # BronzeThreshold / SilverThreshold / GoldThreshold.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+CompletionScore\s*=\s*5000\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+CompletionScore\s*=\s*0\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CompletionScore\s*=\s*2500\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CompletionScore\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+CompletionScore\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CompletionScore\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+CompletionScore\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 CompletionScore is missing from "
...
def require_completion_score_specifiers(section: str) -> str:
...
class MissionScoreRulesCompletionScoreFieldDeclContractTests(unittest.TestCase):
    def test_mission_score_rules_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CompletionScore"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CompletionScore"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("CompletionScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_completion_score_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_completion_score_specifiers(section)
    ...
    def test_initializer_fails_closed_when_not_5000(self) -> None:
        ...
        self.assertIn("int32 CompletionScore = 5000;", compact_origin)
        self.assertNotIn("CompletionScore = NAME_None", compact_origin)
        self.assertNotIn("CompletionScore = false", compact_origin)
        self.assertNotIn("CompletionScore = true", compact_origin)
        self.assertNotIn("CompletionScore = 0.f", compact_origin)
        self.assertNotIn("CompletionScore = 160.f", compact_origin)

    def test_completion_score_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 CompletionScore"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFName CompletionScores;\n",
            "\tbool CompletionScore;\n",
            "\tfloat CompletionScore = " + forty + ";\n",
            "\tfloat CompletionScore = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_completion_score_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(compact_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        with_max = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_completion_score_specifiers(spec_section(with_max))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-score-rules CompletionScore field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_score_rules_completion_score_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
