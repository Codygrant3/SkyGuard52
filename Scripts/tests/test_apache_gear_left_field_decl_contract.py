# THIS IS leftover-safe ASkyguardApacheAircraft GearLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> GearLeft;`
# (no initializer).
...
# Stop BEFORE sibling GearRight as a claimed slot.
# Do NOT claim GearNose / GearRight / StubWingLeft /
# PylonLeft / HydraLeft / HellfireLeft / Nose /
...
# GearNose / GearRight / StubWingLeft / PylonLeft /
# HydraLeft / HellfireLeft / Nose / HorizontalTail are
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> GearLeft;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> GearLeft;` and NOT
# leftover `GearNose` / leftover `GearRight` / leftover
# `Nose` / leftover `StubWingLeft` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> GearLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> GearLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> GearLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> GearLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GearLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GearLeft;"
...
STOP_BEFORE_GEAR_RIGHT = "GearRight"
...
GEAR_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+GearLeft\s*;"
)
GEAR_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+GearLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover GearNose / GearRight / Nose / StubWingLeft,
    # leftover Fuselage / AircraftRoot / SilhouetteMesh,
    # leftover TObjectPtr<USceneComponent>, or an
    # initializer on GearLeft as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GEAR_LEFT_DECL_RE.search(compact) is None:
        return False
    if GEAR_LEFT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_GEAR_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_GEAR_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_GEAR_RIGHT}"
        )
    for later in (
        SIBLING_GEAR_RIGHT,
        SIBLING_PYLON_LEFT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+GearLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> GearLeft "
...
class ApacheGearLeftFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GearLeft"), section)
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_GEAR_NOSE),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GearLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GearLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_gear_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> GearLeft;", compact_origin)
        self.assertNotIn("GearLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_gear_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> GearLeft"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> GearLeft;")
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> GearLeft;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> GearLefts;\n",
            "\tint32 GearLeft = 140;\n",
            "\tbool GearLeft = true;\n",
            "\tfloat GearLeft = " + forty + ";\n",
            "\tfloat GearLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_gear_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_GEAR_RIGHT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GearLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gear_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
