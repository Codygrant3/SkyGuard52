...
# INDEX_NONE or lock ConfirmRadarGhostIdentification
# construction in the .cpp. This is leftover-safe
# Mission07 Search ConfirmRadarGhostIdentification
# on ASkyguardMission07IntegrationDirector. It is
# NOT leftover radar-ghost-is-contact-identified,
# NOT leftover search-sector enum #b4d4, NOT
# leftover search-track-runtime-defaults #8266,
# NOT leftover Mission07 InitializePlayableMission,
# NOT leftover Mission06 IsCorePlayableReady #696,
# NOT leftover BindRuntimeActors (retired live
# mount in the locked decl), NOT leftover retired
# mount spawn fields, NOT leftover
# HandleDroneCityImpact, NOT leftover Harbor
# Mission02 / Harbor #6/#8/#9, NOT leftover
# Mission07 wave-state enum #74d8, NOT leftover
# Mission07 protected-target enum #18f2, NOT
# leftover Mission07 protected-target-runtime
# defaults #866c, NOT leftover GetNightBeatKit /
...
# BindRuntimeActors / ClassifyFalseTrack /
...
# GetSearchSector / GetClassifiedFalseTrackCount /
# IsHostileContactConfirmed / GetProtectedTarget /
...
# `bool bEngineSoundObserved);`); accept that
...
# leftover Mission07 Integration, not leftover
...
# airfield-target-runtime-defaults #6ad8, leftover
# payload-window-runtime-defaults #f114, leftover
# BindRuntimeActors, leftover
# radar-ghost-is-contact-identified, plus leftover
# #56–#64 subsystem production files. This lane
# only adds an isolated Python
# ConfirmRadarGhostIdentification method
...
LEFTOVER_RADAR_GHOST_CONTACT_NOT_LOCKED = (
    "IsContactIdentified",
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission07IntegrationDirector::ConfirmRadarGhostIdentification",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission07ConfirmRadarGhostIdentificationDeclContractTests(unittest.TestCase):
    def test_mission07_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CONFIRM_RADAR_GHOST_IDENTIFICATION),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("ConfirmRadarGhostIdentification", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_missing_confirm_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission07IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("ConfirmRadarGhostIdentification", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("ConfirmRadarGhostIdentification", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("BlueprintCallable", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("Category", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("BlueprintPure", UFUNCTION_SEARCH)
        ...
        self.assertIn("BlueprintCallable", UFUNCTION_SEARCH)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("ConfirmRadarGhostIdentification", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid ConfirmRadarGhostIdentification("
            "bool bExhaustObserved, "
            "bool bShadowObserved, "
            "bool bEngineSoundObserved);\n"
        )
        as_const = (
            ...
            "bool bExhaustObserved, "
            "bool bShadowObserved, "
            "bool bEngineSoundObserved) const;\n"
        )
        with_arg = (
            "\tbool ConfirmRadarGhostIdentification("
            "bool bExhaustObserved);\n"
        )
        renamed = "\tbool ConfirmRadarGhost();\n"
        short_name = "\tbool ConfirmIdentification();\n"
        leftover_init = f"\t{INITIALIZE_PLAYABLE_MISSION}\n"
        leftover_contact = "\tbool IsContactIdentified() const;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CONFIRM_RADAR_GHOST_IDENTIFICATION)
            self.assertIn("ConfirmRadarGhostIdentification", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_confirm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CONFIRM_RADAR_GHOST_IDENTIFICATION),
            CONFIRM_RADAR_GHOST_IDENTIFICATION,
        )
        self.assertTrue(has_declaration(section, CONFIRM_RADAR_GHOST_IDENTIFICATION))
        self.assertEqual(
            declaration_count(section, CONFIRM_RADAR_GHOST_IDENTIFICATION),
            1,
        )
        self.assertTrue(
            CONFIRM_RADAR_GHOST_IDENTIFICATION.startswith("bool "),
            CONFIRM_RADAR_GHOST_IDENTIFICATION,
        )
        self.assertTrue(
            CONFIRM_RADAR_GHOST_IDENTIFICATION.endswith(";"),
            CONFIRM_RADAR_GHOST_IDENTIFICATION,
        )
        self.assertIn("ConfirmRadarGhostIdentification", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("bExhaustObserved", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("bShadowObserved", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("bEngineSoundObserved", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("TArray<", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("TObjectPtr", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("TSoftObjectPtr", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("INDEX_NONE", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("UFUNCTION", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("{", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("}", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("return ", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("const;", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("Root", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("Briefing", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AudioDirector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("RadioChatter", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("SortiePresentation", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("CampaignDefinition", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("MissionDefinition", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("Readiness", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("bAutoInitialize", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("bAllowBoundedActorSpawning", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("bAutoLaunchAfterBriefing", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("PathfinderSpawnLocation", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetAircraft", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("BindRuntimeActors", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("HandleDroneCityImpact", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("StartNextWave", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("NotifyThreatDestroyed", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AdvanceConvoyByDistance", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("NotifyConvoyDamage", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetWaveState", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetConvoyRouteState", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetDayBeatKit", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetNightBeatKit", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("StartSearchlightWindow", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AdvanceSearchlightTrack", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("NotifySubstationDamage", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetRemainingThreatsInWave", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetSearchlightRuntime", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetSubstationIntegrity", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("StartPayloadWindow", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AdvancePayloadWindow", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("TryJamActivePayload", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("NotifyAirfieldTargetDamage", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetPayloadWindow", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetTargetRuntime", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetSurvivingTargetCount", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetDayBeatKind", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetDayBeatIndex", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("TickDayBeatKit", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("HandleBossPhaseChanged", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ESkyguardMission06WaveState", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ESkyguardMission07WaveState", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ESkyguardSearchSector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("FSkyguardSearchTrackRuntime", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ClassifyFalseTrack", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("InitializePlayableMission", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("IsContactIdentified", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("NotifyProtectedTargetDamage", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AdvanceReinforcementTimer", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetSearchSector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetClassifiedFalseTrackCount", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("IsHostileContactConfirmed", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetProtectedTarget", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetReinforcementTimeRemaining", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetNightBeatKind", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetNightBeatIndex", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("TickNightBeatKit", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ESkyguardAirfieldTarget", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ConvoyRuntimeAnchor", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("RoadHunterSpawnLocation", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ConfigureMissionDefinition", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("NotifyObjectiveProgress", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("NotifyProtectedAssetFailed", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("SynchronizeRuntimeState", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("IsCorePlayableReady", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetReadiness", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetObjectiveRuntime", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetGunner", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetPathfinder", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetMissionId", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ValidateMissionContract", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ConfigureFromMission", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AdvanceBriefing", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("SetAssetsReady", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AcknowledgeAndLaunch", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("CanLaunch", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetElapsedSeconds", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetBriefingState", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetMinimumWarmupSeconds", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetBriefingText", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetRadioChatter", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetPresentation", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("GetMissionTitle", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("AcknowledgeBriefing", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("LaunchSortie", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("HullCollider", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("OpticalTracker", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("WeaponServo", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("CountermeasurePod", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("MinHeightFromOriginCm", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("RadarNode", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ESkyguardMission02WaveState", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("HarborIndustrial", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("MaxIntegrity", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("CurrentIntegrity", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("FillAndFinalize", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("FillAndFail", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SEARCH} {CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
            "};\n"
        )
        ...
        self.assertTrue(
            has_declaration(one_line, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CONFIRM_RADAR_GHOST_IDENTIFICATION),
    ...
    def test_environment_category_does_not_satisfy_search(self) -> None:
        ...
        self.assertNotEqual(leftover_m07_integration, UFUNCTION_SEARCH)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_confirm_cpp_body(self) -> None:
        locked_only = f"{CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
        self.assertNotIn("{", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("}", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("return ", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        ...
        self.assertNotIn("return false", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("return true", CONFIRM_RADAR_GHOST_IDENTIFICATION)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn("ConfirmRadarGhostIdentification", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertTrue(CONFIRM_RADAR_GHOST_IDENTIFICATION.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("CurrentIntegrity", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("SkyguardApacheAircraft.h", CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
        self.assertEqual(
            require_declaration(locked_only, CONFIRM_RADAR_GHOST_IDENTIFICATION),
            CONFIRM_RADAR_GHOST_IDENTIFICATION,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", CONFIRM_RADAR_GHOST_IDENTIFICATION)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 ConfirmRadarGhostIdentification method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, CONFIRM_RADAR_GHOST_IDENTIFICATION.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                CONFIRM_RADAR_GHOST_IDENTIFICATION.lower(),
                "mission07 ConfirmRadarGhostIdentification contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CONFIRM_RADAR_GHOST_IDENTIFICATION)

    def test_contract_is_confirm_radar_ghost_identification_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(CONFIRM_RADAR_GHOST_IDENTIFICATION.startswith("bool "))
        self.assertIn("ConfirmRadarGhostIdentification", CONFIRM_RADAR_GHOST_IDENTIFICATION)
        self.assertTrue(CONFIRM_RADAR_GHOST_IDENTIFICATION.endswith(";"))
        self.assertIn(UFUNCTION_SEARCH, section)

    def test_sibling_director_fields_do_not_satisfy_confirm(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, CONFIRM_RADAR_GHOST_IDENTIFICATION)
            )
            self.assertNotEqual(CONFIRM_RADAR_GHOST_IDENTIFICATION, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("ConfirmRadarGhostIdentification", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, CONFIRM_RADAR_GHOST_IDENTIFICATION))
        locked_only = f"{CONFIRM_RADAR_GHOST_IDENTIFICATION}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            ...
            "bool bExhaustObserved, "
            "bool bShadowObserved, "
            "bool bEngineSoundObserved) { return false; }\n"
            "};\n"
        )
    ...
    def test_leftover_mission05_initialize_does_not_satisfy(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_radar_ghost_is_contact_identified"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission07_confirm_radar_ghost"
            "_identification_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_confirm_radar_ghost"
                "_identification_decl_contract.py"
            )
        )
        self.assertIn(
