...
# Skyguard|Combat|Sortie (unique unused 3-segment suffix
# Sortie). This is leftover-safe unique analog, NOT
# leftover analog Combat.
...
# Skyguard|Combat WITHOUT the unique leftover-safe
# suffix |Sortie, or Skyguard|Combat|Feedback /
# Skyguard|Combat|Safety (leftover unique FIELD
# #1625/#1626), or Skyguard|Apache / Skyguard|Theater /
...
# UPROPERTY.
# Do NOT claim sibling Combat|Sortie UFUNCTION
# ResetSortieCombatStats / GetSortieShotsFired
...
# Do NOT claim leftover analog MissionResult Hits or
# CpgDebriefSnapshot Hits (different identifiers).
# THIS identifier is GetSortieHits.
...
# (declaration-only) on USkyguardAudioDirectorComponent.
# RETARGET hard: class → ASkyguardGunner, Category →
# Skyguard|Combat|Sortie, method → GetSortieHits
# (int32 const).
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is `int32 GetSortieHits() const;`
# NOT `int32 GetResolvedProductionLoopRouteCount() const;`
# and NOT `FName WeatherIdentity;` and NOT
# `int32 GetLampInstanceCount() const;` and NOT
# `int32 GetSortieShotsFired() const;` and NOT
# `void ResetSortieCombatStats();` and NOT `Hits =`
# and NOT leftover `float FireRate = 12.0f`.
# Fail-closed if this test still asserts
# GetResolvedProductionLoopRouteCount / WeatherIdentity /
# GetLampInstanceCount / GetSortieShotsFired /
# ResetSortieCombatStats / Hits = / FireRate=12.0f
# AS THE LOCKED DECL.
...
TARGET = "int32 GetSortieHits() const;"
TARGET_WRONG_BARE = "int32 GetSortieHits();"
TARGET_WRONG_INIT = "int32 GetSortieHits() = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "int32 GetSortieHits() = true;"
TARGET_WRONG_ZERO = "int32 GetSortieHits() = 0.f;"
TARGET_WRONG_NO_CONST = "int32 GetSortieHits();"
TARGET_WRONG_FLOAT = "float GetSortieHits() const;"
TARGET_WRONG_FNAME_RET = "FName GetSortieHits() const;"
...
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GetSortieHits;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GetSortieHits;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> GetSortieHits;"
...
GET_SORTIE_HITS_RE = re.compile(
    r"int32\s+GetSortieHits\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+SortieHits\s*;\s*\})"
)
...
def has_one_declaration(region: str, declaration: str) -> bool:
    ...
    pattern = re.compile(
        re.escape(stem)
        + r"\s*(?:;|\{[^}]*return\s+SortieHits\s*;\s*\})"
    )
    return pattern.search(compact_region) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 GetSortieHits() const;`
    # OR origin/main inline `{ return SortieHits; }`.
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetSortieHits\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for int32 GetSortieHits "
...
class GunnerGetSortieHitsDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetSortieHits"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GetSortieHits"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GetSortieHits", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_sortie_hits_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_get_sortie_hits_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GetSortieHits"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GetSortieHitss() const;\n",
            "\tfloat GetSortieHits() const;\n",
            "\tFName GetSortieHits() const;\n",
            "\tint32 GetSortieHits() const { return " + forty + "; }\n",
            "\tint32 GetSortieHits() const { return " + eighty + "; }\n",
            f"\t{TARGET_WRONG_LAMP}\n",
            f"\t{TARGET_WRONG_PEAK}\n",
    ...
    def test_does_not_claim_sibling_ufunctions_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetSortieHits"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GetSortieHits"))
        self.assertTrue(has_identifier(section, "GetSortieHits"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_SORTIE_SHOTS))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner GetSortieHits decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_get_sortie_hits_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
