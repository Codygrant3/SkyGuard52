...
# then BARE `bool EnqueueLine(const FSkyguardRadioLine& Line);`
# (declaration-only, no inline body required).
# KEEP Line arg from origin/main.
# KEEP BlueprintCallable. Do NOT retarget to
# PrimeLines / AdvanceRadioState / ClearQueue /
# GetQueuedLineCount / GetCurrentLineId /
# GetDroppedLineCount / GetPlayedLineCount.
# Word-boundary match EnqueueLine vs
# PrimeLines vs AdvanceRadioState.
...
# Identifier EnqueueLine is NOT leftover analog
# PrimeLines and NOT leftover analog RadioLine fields.
# BlueprintCallable IS present. BlueprintPure IS absent.
# const method IS absent. Return type IS bool.
...
# Line arg is missing, if the return type is not bool,
# or if Category is
...
# RETARGET Category HUD -> Radio and method/return/args.
# KEEP BlueprintCallable KEEP arg. This slot returns bool
# not void. FAIL-CLOSED if LOCKED_DECL is void BindPresentation(.
...
# KEEP arg and bool return (TogglePause is void no-args).
...
# LOCKED_DECL is bool EnqueueLine(const FSkyguardRadioLine& Line);
...
TARGET = "bool EnqueueLine(const FSkyguardRadioLine& Line);"
TARGET_SPLIT = (
    "bool EnqueueLine(\n"
    "\tconst FSkyguardRadioLine& Line);"
)
TARGET_INLINE = TARGET
TARGET_WRONG_BARE = "bool EnqueueLine();"
TARGET_WRONG_NO_CONST = (
    "bool EnqueueLine(const FSkyguardRadioLine& Line) const;"
)
...
TARGET_WRONG_FLOAT = (
    "float EnqueueLine(const FSkyguardRadioLine& Line);"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "bool EnqueueLines(const FSkyguardRadioLine& Line);"
...
TARGET_WRONG_TRUE = "bool EnqueueLine = true;"
TARGET_WRONG_ZERO = "bool EnqueueLine = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bbool\s+EnqueueLine\("
    r"const FSkyguardRadioLine\s*&\s*Line\)(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept a method const qualifier, a void/float/int32
    # rename, leftover unique EnsureDefaultEntries /
    # TogglePause / BindPresentation / GetSortieHits,
    # leftover sibling PrimeLines / AdvanceRadioState /
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bEnqueueLine\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+EnqueueLine\b", compact):
        return False
    if re.search(r"\bfloat\s+EnqueueLine\b", compact):
        return False
    if re.search(r"\bint32\s+EnqueueLine\b", compact):
        return False
    if re.search(r"\bbool\s+EnqueueLine\s*\(\s*\)", compact):
        return False
...
class RadioChatterEnqueueLineDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_enqueue_line_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_enqueue_line_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_identifier(LOCKED_DECL, "Line"))
        self.assertIn(
            "FSkyguardRadioLine",
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool EnqueueLineX(const FSkyguardRadioLine& Line);\n",
            "\tint32 EnqueueLine = 1;\n",
            "\tFName EnqueueLine = NAME_None;\n",
            "\tbool EnqueueLine() = "
            + forty
            + ";\n",
            "\tbool EnqueueLine() = "
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertTrue(has_identifier(LOCKED_DECL, "Line"))
        self.assertIn("EnqueueLine", LOCKED_DECL)
        for leftover in leftover_apache_ufunction_tokens():
            self.assertNotIn(leftover, locked_only)

    def test_word_boundary_enqueue_line_vs_prime_lines_and_advance(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier(
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "EnqueueLine"), section)
    ...
    def test_locked_decl_is_not_clone_ensure_toggle_hits_or_bind(
        ...
        self.assertTrue(LOCKED_DECL.startswith("bool EnqueueLine("))
        self.assertIn("const FSkyguardRadioLine& Line", LOCKED_DECL)
        self.assertIsNone(re.search(r"\)\s*const\b", LOCKED_DECL))
        self.assertIn(
            'Category="Skyguard|Radio"',
            LOCKED_UFUNCTION_WRAP,
