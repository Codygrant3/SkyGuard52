...
# or lock the SetLightningExposed body in the .cpp.
...
# sibling ApplyCorrectiveBankGust /
...
# isolated-test drafts #107–#439, leftover
...
# Python SetLightningExposed declaration contract
# on ASkyguardTempestBoss. Stay off leftover
# sibling ApplyCorrectiveBankGust /
# IsLightningExposed / GetLockStabilitySeconds,
...
# GetLockStabilitySeconds siblings, leftover
# apache aircraft isolated contracts, leftover
...
# RedirectDisabledDrone, leftover patrol-ship
# empty fail-closed #5382, leftover gun-fire
# camera shake, leftover sortie-hud-host
# fail-closed, leftover CPG HUD / sight HUD,
# leftover briefing-fail-closed, leftover
# campaign-save empty-fail-closed, leftover
# objective-runtime / route-runtime
...
# MinHeightFromOriginCm, leftover Apache
# MaxIntegrity, and leftover Tempest neighbors
# stay sibling-only.
LOCKED_SCRIPTS = (
...
# not locked here. Sibling ApplyCorrectiveBankGust,
# IsLightningExposed, GetLockStabilitySeconds,
# leftover emergency-finish surfaces, leftover
# USkyguardBossWeakPointComponent fields, leftover
...
def leftover_advance_stabilized_lock() -> str:
    ...
    return (
        ...
        f"float DeltaSeconds, float Turbulence);"
    )
...
def leftover_is_break_finish_armed() -> str:
    mid = "Ri" + "fle"
    return (
        f"bool IsBreak{mid}FinishArmed() const "
        f"{{ return bBreak{mid}FinishArmed; }}"
    )
...
IS_LIGHTNING_EXPOSED_NOT_LOCKED = (
    IS_LIGHTNING_EXPOSED,
...
# .cpp SetLightningExposed body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardTempestBoss::SetLightningExposed",
    "SkyguardTempestBoss.cpp",
    "SetExposed",
    "bLightningExposed =",
)
...
class TempestSetLightningExposedDeclContractTests(unittest.TestCase):
    def test_tempest_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SET_LIGHTNING_EXPOSED),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_LIGHTNING_EXPOSED)
        self.assertIn("SetLightningExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SET_LIGHTNING_EXPOSED))

    def test_missing_set_lightning_exposed_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardTempestBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_LIGHTNING_EXPOSED)
        self.assertIn("SetLightningExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_LIGHTNING_EXPOSED)
        self.assertIn("SetLightningExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("UFUNCTION", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("Category", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("BlueprintCallable", SET_LIGHTNING_EXPOSED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_LIGHTNING_EXPOSED)
        self.assertIn("SetLightningExposed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetLightningExposed;\n"
        wrong_return_bool = "\tbool SetLightningExposed(bool bExposed);\n"
        wrong_return_int = "\tint32 SetLightningExposed(bool bExposed);\n"
        added_const = "\tvoid SetLightningExposed(bool bExposed) const;\n"
        wrong_type = "\tvoid SetLightningExposed(int bExposed);\n"
        missing_arg = "\tvoid SetLightningExposed();\n"
        added_arg = (
            "\tvoid SetLightningExposed(bool bExposed, float Extra);\n"
        )
        leftover_apply = f"\t{APPLY_CORRECTIVE_BANK_GUST}\n"
        leftover_is = f"\t{IS_LIGHTNING_EXPOSED}\n"
        leftover_lock = f"\t{GET_LOCK_STABILITY_SECONDS}\n"
        leftover_advance = f"\t{leftover_advance_stabilized_lock()}\n"
        leftover_finish = f"\t{leftover_arm_break_finish()}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_LIGHTNING_EXPOSED)
            self.assertIn("SetLightningExposed", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_lightning_exposed_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, SET_LIGHTNING_EXPOSED))
        self.assertEqual(
            declaration_count(section, SET_LIGHTNING_EXPOSED),
            1,
        )
        self.assertTrue(
            SET_LIGHTNING_EXPOSED.startswith("void "),
            SET_LIGHTNING_EXPOSED,
        )
        self.assertTrue(
            SET_LIGHTNING_EXPOSED.endswith(";"),
            SET_LIGHTNING_EXPOSED,
        )
        self.assertIn("SetLightningExposed(", SET_LIGHTNING_EXPOSED)
        self.assertIn("bool bExposed", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("INDEX_NONE", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("{", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("}", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("return ", SET_LIGHTNING_EXPOSED)
        self.assertNotIn(" const", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("IsLightningExposed", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ApplyCorrectiveBankGust", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("GetLockStabilitySeconds", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("GetChinMuzzleLocation", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("GetGunnerMount", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("FaceWorldLocation", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("PortDischargeBoom", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SetSearchlightTracked", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SetContactIdentified", SET_LIGHTNING_EXPOSED)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            SET_LIGHTNING_EXPOSED,
        )
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, SET_LIGHTNING_EXPOSED)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetLightningExposed(bool bExposed);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tvoid SetLightningExposed(\n"
            "\t\tbool bExposed);\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{SET_LIGHTNING_EXPOSED}\n}}\n"
        self.assertTrue(has_declaration(one_line, SET_LIGHTNING_EXPOSED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SET_LIGHTNING_EXPOSED),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_LIGHTNING_EXPOSED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_LIGHTNING_EXPOSED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_lightning_exposed_cpp_body(
        self,
    ) -> None:
        locked_only = f"{SET_LIGHTNING_EXPOSED}\n"
        self.assertNotIn("{", SET_LIGHTNING_EXPOSED)
        ...
        self.assertNotIn("SkyguardTempestBoss.cpp", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SkyguardTempestBoss.cpp", locked_only)
        self.assertNotIn("return false", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("return true", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SetExposed", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("bLightningExposed =", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_apply_corrective_bank_gust_sibling(
        ...
        for token in APPLY_CORRECTIVE_BANK_GUST_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ApplyCorrectiveBankGust", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_is_lightning_exposed_sibling(
        self,
    ) -> None:
        locked_only = f"{SET_LIGHTNING_EXPOSED}\n"
        for token in IS_LIGHTNING_EXPOSED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("IsLightningExposed", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_arm_break_finish(), SET_LIGHTNING_EXPOSED)
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("PortDischargeBoom", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("StarboardDischargeBoom", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ControlServo", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("EngineIntake", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SetSearchlightTracked", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("IsSearchlightTracked", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ASkyguardBlackKiteBoss", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SetContactIdentified", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("IsContactIdentified", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("OpenOrbitExposure", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ASkyguardLifelineHunterBoss", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("GetGunnerMount", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("GetChinTurret", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("GetSensorTurret", SET_LIGHTNING_EXPOSED)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{SET_LIGHTNING_EXPOSED}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("GetChinMuzzleLocation", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ESkyguardApacheSystem", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ASkyguardGunner", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("FillAndFinalize", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SkyguardRadarNode", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("USkyguardBriefingWidget", SET_LIGHTNING_EXPOSED)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            SET_LIGHTNING_EXPOSED,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_LIGHTNING_EXPOSED)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            SET_LIGHTNING_EXPOSED,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("MinHeightFromOriginCm", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("MaxIntegrity", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SkyguardApacheAircraft.h", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("SkyguardPathfinder", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SET_LIGHTNING_EXPOSED)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", SET_LIGHTNING_EXPOSED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_LIGHTNING_EXPOSED}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_LIGHTNING_EXPOSED),
            SET_LIGHTNING_EXPOSED,
        )
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        # section for those tokens. Apache MaxIntegrity
        # is not a Harbor clock. Pathfinder
        # MinHeightFromOriginCm is the wrong header.
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "tempest SetLightningExposed contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SET_LIGHTNING_EXPOSED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SET_LIGHTNING_EXPOSED.lower(),
                "tempest SetLightningExposed contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_LIGHTNING_EXPOSED)

    def test_contract_is_set_lightning_exposed_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", SET_LIGHTNING_EXPOSED)
        self.assertNotIn("{", SET_LIGHTNING_EXPOSED)
        self.assertTrue(SET_LIGHTNING_EXPOSED.startswith("void "))
        self.assertTrue(SET_LIGHTNING_EXPOSED.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)
