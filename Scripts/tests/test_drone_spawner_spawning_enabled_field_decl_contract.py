...
# then `bool bSpawningEnabled = true;`
# (HAS initializer).
...
# BlueprintReadWrite IS present. Default IS `= true`.
# KEEP EditAnywhere BlueprintReadWrite Category Skyguard|Spawning.
# KEEP default `true`. KEEP specifiers matching origin/main.
# Word-boundary: `bSpawningEnabled` is NOT leftover analog
# UFUNCTION `SetSpawningEnabled` (drafted this wave), NOT leftover
# analog Mission* `bAllowBoundedActorSpawning`, NOT leftover analog
# `bMixedThreatRoster`.
...
# split 40 / 80 float tokens.
# Do NOT wrap Category=`Skyguard` fields `SpawnInterval` or
# `MaxActiveDrones`. Do NOT wrap HeavyLaneModulo /
# bMixedThreatRoster / LightSpeed* / HeavyCruiseSpeed /
# MaxActiveDrones / SpawnInterval in this PR.
# Parse CLASS `ASkyguardDroneSpawner` public UPROPERTY window only.
# Do NOT parse leftover analog drone-spawner-empty-fail-closed bulk
# #161/#df16 as the slot (behavioral).
# Start at `public:` after GENERATED_BODY. Stop BEFORE `protected:`.
# Stop BEFORE sibling SpawnInterval as a claimed slot.
...
# Clone leftover unique analog Mission01 actor bool #1637 SHA
# aa042fb45058011582010f3d633f0c4b3a098707 RETARGET to
# bSpawningEnabled KEEP true.
# Clone leftover unique analog SetSpawningEnabled SHA
# ebbc1fd01dd17368d06692c56ad4750481f607be RETARGET (that one is
...
# LOCKED_DECL is `bool bSpawningEnabled = true;`
...
# NOT `bool bSpawningEnabled = false;`
...
# TArray RoutePoints / bool bSpawningEnabled = false;
...
# Public bool fail-closed if LOCKED_DECL is leftover
...
TARGET = "bool bSpawningEnabled = true;"
TARGET_WRONG_BARE = "bool bSpawningEnabled;"
TARGET_WRONG_INIT = "bool bSpawningEnabled = nullptr;"
TARGET_WRONG_FALSE = "bool bSpawningEnabled = false;"
TARGET_WRONG_TRUE = "int32 bSpawningEnabled = true;"
TARGET_WRONG_ZERO = "bool bSpawningEnabled = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> bSpawningEnabled;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> bSpawningEnabled;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> bSpawningEnabled;"
...
STOP_BEFORE_ROUTE_LENGTH_CM = "SpawnInterval"
...
SPAWNING_ENABLED_DECL_RE = re.compile(
    r"bool\s+bSpawningEnabled\s*=\s*true\s*;"
)
SPAWNING_ENABLED_BARE_RE = re.compile(
    r"bool\s+bSpawningEnabled\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bare bSpawningEnabled without `= false`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SPAWNING_ENABLED_DECL_RE.search(compact) is None:
        return False
    if SPAWNING_ENABLED_BARE_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bSpawningEnabled\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bSpawningEnabled "
...
class DroneSpawnerSpawningEnabledFieldDeclContractTests(unittest.TestCase):
    def test_drone_spawner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bSpawningEnabled"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertIn("bSpawningEnabled", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_spawning_enabled_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_spawning_enabled_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool bSpawningEnableds = false;\n",
            "\tint32 bSpawningEnabled = 1;\n",
            "\tbool bSpawningEnabled = false;\n",
            "\tbool bSpawningEnabled = " + forty + ";\n",
            "\tbool bSpawningEnabled = " + eighty + ";\n",
    ...
    def test_does_not_claim_sibling_spawn_interval_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "bSpawningEnabled"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "bSpawningEnabled"))
        self.assertTrue(has_identifier(section, "bSpawningEnabled"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_SET_SPAWNING))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "drone spawner spawning enabled field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "drone_spawner_spawning_enabled_field_decl_contract.py"
        ))
