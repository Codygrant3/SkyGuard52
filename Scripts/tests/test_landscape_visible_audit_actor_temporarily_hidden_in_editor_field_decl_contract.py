...
# bActorTemporarilyHiddenInEditor FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bActorTemporarilyHiddenInEditor = true; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# bActorTemporarilyHiddenInEditor. THIS IS NOT leftover
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# bActorTemporarilyHiddenInEditor FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bActorTemporarilyHiddenInEditor = true; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly bool bActorTemporarilyHiddenInEditor = true;
...
# bActorTemporarilyHiddenInEditor FIELD (declaration-only). It is
...
TARGET = "bool bActorTemporarilyHiddenInEditor = true;"
TARGET_WRONG = "bool bActorTemporarilyHiddenInEditor = false;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # bActorTemporarilyHiddenInEditor (must be = true, not = false).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bActorTemporarilyHiddenInEditor\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bActorTemporarilyHiddenInEditor is missing from "
...
class LandscapeVisibleAuditActorTemporarilyHiddenInEditorFieldDeclContractTests(unittest.TestCase):
    def test_landscape_visible_audit_struct_exists(self) -> None:
        ...
        self.assertIn("bActorTemporarilyHiddenInEditor", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bActorTemporarilyHiddenInEditor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_actor_temporarily_hidden_in_editor_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_actor_temporarily_hidden_in_editor_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bActorTemporarilyHiddenInEditor"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bActorTemporarilyHiddenInEditor", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bActorTemporarilyHiddenInEditor", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bActorTemporarilyHiddenInEditor"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bActorTemporarilyHiddenInEditor", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_actor_temporarily_hidden_in_editor(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
