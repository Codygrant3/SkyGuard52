# THIS IS leftover-safe ASkyguardCoastalEnvironmentDirector EpicTreeBudget.
...
# then `int32 EpicTreeBudget = 240;` (HAS initializer; NO spaces around =).
...
# BlueprintReadWrite IS present. Default IS `= 240`.
# This is a unique coastal director epic tree-budget int32.
# It is NOT leftover FSkyguardEnvironmentReadiness.TreeInstanceCount,
# NOT leftover sibling PlacementSeed / EpicShrubBudget / TreeInstances /
...
# Stop BEFORE sibling EpicShrubBudget as a claimed slot.
...
# Do NOT refill leftover TreeInstances / ShrubInstances /
# WindSource.
...
# or still Category="Skyguard" without |Environment|Vegetation.
# Fail-closed if LOCKED_DECL is TObjectPtr / FName WeatherIdentity
# / leftover readiness TreeInstanceCount / leftover TreeInstances
# TObjectPtr.
# Harbor leftover is only the split 40 / 80 float tokens.
# EpicTreeBudget is 240 not Harbor forty / eighty float tokens.
...
TARGET = "int32 EpicTreeBudget = 240;"
TARGET_WRONG_BARE = "int32 EpicTreeBudget;"
TARGET_WRONG_INIT = "int32 EpicTreeBudget = nullptr;"
TARGET_WRONG_FALSE = "int32 EpicTreeBudget = false;"
TARGET_WRONG_TRUE = "int32 EpicTreeBudget = true;"
TARGET_WRONG_ZERO = "int32 EpicTreeBudget = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> EpicTreeBudget;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> EpicTreeBudget;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> EpicTreeBudget;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "EpicShrubBudget"
STOP_BEFORE_EPIC_SHRUB = "EpicShrubBudget"
...
SIBLING_LOOK_PITCH_MIN = "EpicShrubBudget"
...
EPIC_TREE_BUDGET_BARE_RE = re.compile(
    r"int32\s+EpicTreeBudget\s*;"
)
EPIC_TREE_BUDGET_WRONG_INIT_RE = re.compile(
    r"int32\s+EpicTreeBudget\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 EpicTreeBudget = 240;`.
    ...
    # TObjectPtr, or a bare EpicTreeBudget without
    # the 240 initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if EPIC_TREE_BUDGET_DECL_RE.search(compact) is None:
        return False
    if EPIC_TREE_BUDGET_BARE_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+EpicTreeBudget\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 EpicTreeBudget "
...
class CoastalEpicTreeBudgetFieldDeclContractTests(unittest.TestCase):
    def test_coastal_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EpicTreeBudget"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EpicTreeBudget"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("EpicTreeBudget", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_epic_tree_budget_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMax="1024"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 EpicTreeBudget = 240;", compact_origin)
        self.assertNotIn("EpicTreeBudget = 160", compact_origin)
    ...
    def test_epic_tree_budget_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 EpicTreeBudget = 240;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 EpicTreeBudgets = 240;\n",
            "\tfloat EpicTreeBudget = 240.f;\n",
            "\tbool EpicTreeBudget = true;\n",
            "\tint32 EpicTreeBudget = " + forty + ";\n",
            "\tint32 EpicTreeBudget = " + eighty + ";\n",
            f"\t{TARGET_WRONG_SEED}\n",
    ...
    def test_does_not_claim_sibling_epic_shrub_budget_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EpicTreeBudget"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "EpicTreeBudget"))
        self.assertTrue(has_identifier(section, "EpicTreeBudget"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMax="1024"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "coastal EpicTreeBudget field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "coastal_epic_tree_budget_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCoastalEnvironmentDirector.cpp", THIS_SCRIPT)
