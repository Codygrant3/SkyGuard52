...
# INDEX_NONE or lock HeldSeconds
# construction in the .cpp. This is leftover-safe
# Mission04 SearchlightTrackRuntime HeldSeconds
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float default 0.f / 0 / 100,
# one-line / split-line UPROPERTY wraps) on
# FSkyguardSearchlightTrackRuntime. It is
...
# GetSearchlightRuntime, RemainingSeconds, bActive,
# bBossTracked, or CompletedPasses as the subject),
# NOT leftover Mission04 SearchlightTrackRuntime
# RemainingSeconds FIELD sibling / branch cursor/
# mission04-searchlight-track-remaining-seconds-
# field-decl-contract-c332,
# NOT leftover Mission04 SearchlightTrackRuntime
# bActive FIELD sibling / branch cursor/
# mission04-searchlight-track-active-field-
# decl-contract-c332,
...
# not FSkyguardMission04IntegrationReadiness, not leftover
...
# and the locked FSkyguardSearchlightTrackRuntime
...
REMAINING_SECONDS_FIELD = (
    "float RemainingSeconds = 0.f;"
)
SEARCHLIGHT_TRACK_ACTIVE_FIELD = "bool bActive = false;"
SEARCHLIGHT_TRACK_BOSS_TRACKED_FIELD = "bool bBossTracked = false;"
SEARCHLIGHT_TRACK_HELD_SECONDS_FIELD = "float HeldSeconds = 0.f;"
LOCKED_DECL = SEARCHLIGHT_TRACK_HELD_SECONDS_FIELD
SEARCHLIGHT_TRACK_COMPLETED_PASSES_FIELD = (
    "int32 CompletedPasses = 0;"
)
REMAINING_SECONDS_FIELD_HUNDRED = (
    "float RemainingSeconds = 100;"
)
REMAINING_SECONDS_FIELD_ZERO = (
    "float RemainingSeconds = 0;"
)
HELD_SECONDS_FIELD_HUNDRED = (
    "float HeldSeconds = 100;"
)
HELD_SECONDS_FIELD_ZERO = (
    "float HeldSeconds = 0;"
)
REQUIRED_TRACK_SECONDS_FIELD = "float RequiredTrackSeconds"
MISSED_TRACK_DAMAGE_FIELD = "int32 MissedTrackDamage"
...
# an isolated Python HeldSeconds
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission04_searchlight_track_remaining_seconds_field_decl_contract.py",
    "Scripts/tests/test_mission04_searchlight_track_active_field_decl_contract.py",
...
# Alternate HeldSeconds defaults are accepted as declaration
# forms without becoming LOCKED_DECL. Leftover RemainingSeconds
# (sibling isolated remaining-seconds field contract) is not
# the subject. Leftover bActive (sibling isolated active field
# contract) is not the subject.
...
def unlocked_neighbors() -> tuple[str, ...]:
    return (
        ...
        SEARCHLIGHTS_READY_FIELD_TRUE,
        CAMPAIGN_RUNTIME_STARTED_FIELD,
        SEARCHLIGHT_TRACK_ACTIVE_FIELD,
        SEARCHLIGHT_TRACK_BOSS_TRACKED_FIELD,
        REMAINING_SECONDS_FIELD,
        SEARCHLIGHT_TRACK_COMPLETED_PASSES_FIELD,
        REMAINING_SECONDS_FIELD_HUNDRED,
...
class Mission04SearchlightTrackHeldSecondsFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("HeldSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_held_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_held_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float HeldSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("= 0.f;"), LOCKED_DECL)
        self.assertIn("HeldSeconds", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            accepted = (
                HELD_SECONDS_FIELD_ZERO
                if " = 0;" in section and " = 0.f;" not in section
                else HELD_SECONDS_FIELD_HUNDRED
            )
            self.assertTrue(
                has_declaration(section, accepted),
                section,
            )
            self.assertEqual(
                require_declaration(
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("HeldSeconds", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float HeldSeconds"))
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        # This Mission04 field default 0.f is not Harbor 40/80.
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("HeldSeconds", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission04 HeldSeconds field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission04 HeldSeconds contains "
    ...
    def test_contract_is_held_seconds_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_held_seconds(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission04_searchlight_track"
            "_held_seconds_field_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission04_searchlight_track"
                "_held_seconds_field_decl_contract.py"
            )
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission04_searchlight_track"
            "_remaining_seconds_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission04_searchlight_track"
