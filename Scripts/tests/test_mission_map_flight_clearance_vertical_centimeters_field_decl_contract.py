...
# then `float FlightClearanceVerticalCentimeters = 2500.f;`
# (HAS initializer). PUBLIC UPROPERTY, not protected.
...
# meta ClampMin 500.0 IS present. Default IS `= 2500.f`.
# This is a mission-map flight-clearance vertical float, not
# leftover UFUNCTION IsPointInsideFlightClearance, not
# leftover FSkyguardMissionMapReadiness.RouteLengthCentimeters
# = 0.f, not leftover sibling
# FlightClearanceRadiusCentimeters = 3000.f, not leftover
# analog apache-cpg-feel FireRate / RecoilPitch, not leftover
# LookYawLimit = 95.f, not leftover Theater WeatherIdentity,
# not leftover Harbor 40 / 80, not leftover TheaterKit
# 160.f, not leftover LoadoutSpec / Ig+la* / Ri+fleMesh.
# Fail-closed if the UPROPERTY or decl is missing or
# renamed, if Category is missing, if Category is still
# Skyguard|Combat or Skyguard|Theater, or if specifiers
...
# Stop BEFORE `private:`.
# Stop BEFORE sibling Readiness
# as a claimed slot.
# Do NOT claim Root, FlightRouteSpline, MissionDefinition,
# MissionId, AssemblyRevision, SkylineStyle,
...
# Category Skyguard|Combat LookYawLimit = 95.f —
# RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category = "Skyguard|MissionMap" with spaces around =
# plus meta ClampMin 500.0.
# LOCKED_DECL is
# `float FlightClearanceVerticalCentimeters = 2500.f;`
# NOT `float LookYawLimit = 95.f;`, NOT
# `FName WeatherIdentity;` (no initializer), NOT
# leftover analog `float FireRate = 12.0f` /
# `float RecoilPitch = 0.92f`, NOT leftover
# `float FlightClearanceRadiusCentimeters = 3000.f;`,
# NOT leftover `float RouteLengthCentimeters = 0.f;`,
# NOT leftover UFUNCTION IsPointInsideFlightClearance,
# NOT leftover FlareCount = 6, NOT leftover 160.f.
# Fail-closed if this test still asserts FName
# WeatherIdentity / Category="Skyguard|Theater" / 160.f
# / LookYawLimit = 95.f / FireRate = 12.0f /
# RecoilPitch = 0.92f / FlareCount = 6 /
# IsPointInsideFlightClearance /
# RouteLengthCentimeters / sibling 3000.f
# AS THE LOCKED DECL.
# Fail-closed if locked wrap Category is still
# Skyguard|Combat or Skyguard|Theater.
...
TARGET = "float FlightClearanceVerticalCentimeters = 2500.f;"
TARGET_WRONG_BARE = "float FlightClearanceVerticalCentimeters;"
TARGET_WRONG_INIT = "float FlightClearanceVerticalCentimeters = nullptr;"
TARGET_WRONG_FALSE = "float FlightClearanceVerticalCentimeters = false;"
TARGET_WRONG_TRUE = "float FlightClearanceVerticalCentimeters = true;"
TARGET_WRONG_ZERO = "float FlightClearanceVerticalCentimeters = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> FlightClearanceVerticalCentimeters;"
...
TARGET_WRONG_VERTICAL = (
    "float FlightClearanceRadiusCentimeters = 3000.f;"
)
TARGET_WRONG_ROUTE_LENGTH = "float RouteLengthCentimeters = 0.f;"
TARGET_WRONG_FLARE_COUNT = "int32 FlareCount = 6;"
TARGET_WRONG_IS_POINT = "IsPointInsideFlightClearance"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> FlightClearanceVerticalCentimeters;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> FlightClearanceVerticalCentimeters;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "Readiness"
...
LEFTOVER_IS_POINT_INSIDE = "IsPointInsideFlightClearance"
LEFTOVER_LOOK_YAW_LIMIT_SCRIPT = (
    "Scripts/tests/test_gunner_look_yaw_limit"
    "_field_decl_contract.py"
)
LEFTOVER_IS_POINT_SCRIPT = (
    "Scripts/tests/test_mission_map_is_point_inside"
    "_flight_clearance_decl_contract.py"
)
LEFTOVER_ROUTE_LENGTH_SCRIPT = (
    "Scripts/tests/test_mission_map_readiness_route"
    "_length_centimeters_field_decl_contract.py"
)
LEFTOVER_RADIUS_SCRIPT = (
    "Scripts/tests/test_mission_map_flight_clearance"
    "_radius_centimeters_field_decl_contract.py"
)
LEFTOVER_ASSEMBLY_REVISION_SCRIPT = (
...
THIS_SCRIPT = (
    "Scripts/tests/test_mission_map_flight_clearance"
    "_vertical_centimeters_field_decl_contract.py"
)
...
RADIUS_DECL_RE = re.compile(
    r"float\s+FlightClearanceVerticalCentimeters\s*=\s*2500\.f\s*;"
)
RADIUS_BARE_RE = re.compile(
    r"float\s+FlightClearanceVerticalCentimeters\s*;"
)
...
VERTICAL_DECL_RE = re.compile(
    r"float\s+FlightClearanceRadiusCentimeters\s*=\s*3000\.f\b"
)
ROUTE_LENGTH_DECL_RE = re.compile(
    r"float\s+RouteLengthCentimeters\s*=\s*0\.f\b"
)
FLARE_COUNT_DECL_RE = re.compile(
    r"int32\s+FlareCount\s*=\s*6\b"
)
IS_POINT_DECL_RE = re.compile(
    r"IsPointInsideFlightClearance\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Harbor 40 / 80, leftover LookYawLimit = 95.f,
    # leftover FlightClearanceRadiusCentimeters = 3000.f,
    # leftover RouteLengthCentimeters = 0.f,
    # leftover FlareCount = 6, leftover
    # IsPointInsideFlightClearance, or a bare vertical
    # without the 2500.f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if RADIUS_DECL_RE.search(compact) is None:
        return False
    if RADIUS_BARE_RE.search(compact):
        return False
    if LOOK_YAW_LIMIT_DECL_RE.search(compact):
        return False
    if ASSEMBLY_REVISION_DECL_RE.search(compact):
        return False
    if VERTICAL_DECL_RE.search(compact):
        return False
    if ROUTE_LENGTH_DECL_RE.search(compact):
        return False
    if FLARE_COUNT_DECL_RE.search(compact):
        return False
    if IS_POINT_DECL_RE.search(compact):
        return False
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_LOOK_PITCH_MIN:
            break
        if ident != "FlightClearanceVerticalCentimeters":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+FlightClearanceVerticalCentimeters\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float FlightClearanceVerticalCentimeters "
...
class MissionMapFlightClearanceVerticalCentimetersFieldDeclContractTests(unittest.TestCase):
    def test_mission_map_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "FlightClearanceVerticalCentimeters"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, LEFTOVER_IS_POINT_INSIDE), body)
        self.assertFalse(
            has_identifier(section, LEFTOVER_IS_POINT_INSIDE),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_LOOK_PITCH_MIN), section)
        self.assertFalse(
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertIn("FlightClearanceVerticalCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_radius_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin = "500.0"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("LookYawLimit = 95.f", compact_origin)
        self.assertNotIn(
            "FlightClearanceRadiusCentimeters = 3000.f",
            compact_origin,
        )
        self.assertNotIn("RouteLengthCentimeters = 0.f", compact_origin)
    ...
    def test_look_yaw_limit_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float FlightClearanceVerticalCentimeters"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("2500.f", LOCKED_DECL)
        self.assertNotIn("95.f", LOCKED_DECL)
        self.assertNotIn("3000.f", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RADIUS)
        self.assertNotIn("AssemblyRevision", LOCKED_DECL)
        self.assertNotIn("3000.f", LOCKED_DECL)
        self.assertNotIn("FName ", LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VERTICAL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_LENGTH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FLARE_COUNT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_IS_POINT)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_VERTICAL}\n",
            f"\t{TARGET_WRONG_RADIUS}\n",
            f"\t{TARGET_WRONG_ASSEMBLY_REVISION}\n",
            f"\t{TARGET_WRONG_ROUTE_LENGTH}\n",
            f"\t{TARGET_WRONG_FLARE_COUNT}\n",
            f"\t{TARGET_WRONG_IS_POINT}\n",
            "\tfloat FlightClearanceVerticalCentimeterss = 2500.f;\n",
            "\tint32 FlightClearanceVerticalCentimeters = 2500;\n",
            "\tbool FlightClearanceVerticalCentimeters = true;\n",
            "\tfloat FlightClearanceVerticalCentimeters = " + forty + ";\n",
            "\tfloat FlightClearanceVerticalCentimeters = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tfloat\n\tFlightClearanceVerticalCentimeters = 2500.f;\n",
            "\tfloat   FlightClearanceVerticalCentimeters = 2500.f;\n",
            "\tfloat\tFlightClearanceVerticalCentimeters = 2500.f;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_look_pitch_min_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "FlightClearanceVerticalCentimeters")
        )
        self.assertTrue(
            has_identifier(section, "FlightClearanceVerticalCentimeters")
        )
        ...
        self.assertTrue(has_identifier(leaked, LEFTOVER_IS_POINT_INSIDE))
        self.assertFalse(has_identifier(section, LEFTOVER_IS_POINT_INSIDE))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin = "500.0"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map FlightClearanceVerticalCentimeters "
                "field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission_map_flight_clearance_vertical_centimeters"
            "_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        self.assertIn(LEFTOVER_LOOK_YAW_LIMIT_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_IS_POINT_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTE_LENGTH_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_RADIUS_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ASSEMBLY_REVISION_SCRIPT, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(TARGET_WRONG_LOOK_YAW, locked_only)
        self.assertNotIn(TARGET_WRONG_VERTICAL, locked_only)
        self.assertNotIn(TARGET_WRONG_RADIUS, locked_only)
        self.assertNotIn(TARGET_WRONG_ASSEMBLY_REVISION, locked_only)
        self.assertNotIn(TARGET_WRONG_ROUTE_LENGTH, locked_only)
        self.assertNotIn("3000.f", locked_only)
        self.assertNotIn("AssemblyRevision", locked_only)
        self.assertNotIn("FName WeatherIdentity", locked_only)
        self.assertNotIn(TARGET_WRONG_FLARE_COUNT, locked_only)
        self.assertNotIn(LEFTOVER_IS_POINT_INSIDE, locked_only)
        self.assertNotIn('Skyguard|Combat', locked_only)
        self.assertNotIn('Skyguard|Theater', locked_only)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_YAW)
        self.assertNotEqual(
            UPROPERTY_EDIT_READWRITE,
            'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
