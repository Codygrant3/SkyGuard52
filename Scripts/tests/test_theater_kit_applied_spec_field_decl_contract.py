# THIS IS leftover-safe ASkyguardCampaignTheaterKit AppliedSpec.
...
# then BARE `FSkyguardTheaterKitSpec AppliedSpec;` (no initializer).
...
# AppliedSpec does not block this isolated UPROPERTY
# decl, same as leftover briefing-card-defaults then
# isolated CardId.
# Sibling fields Root / RoadInstances / BuildingInstances /
# LampInstances / SilhouetteInstances /
# NamedLandmarkMesh stay uncontracted.
# Do not contract spec fields such as WeatherIdentity.
...
# (Root / RoadInstances / BuildingInstances / LampInstances;
...
# CardId field decls.
# If a clone asserts FName WeatherIdentity / float /
# `= 160.f` / Category="Skyguard|Campaign" / parses the
# spec struct body, retarget: type is
# FSkyguardTheaterKitSpec, name is AppliedSpec, no `=`,
# Category is Skyguard|Theater, parse window is the
# actor public section. This contracts the actor member
# AppliedSpec, not fields inside FSkyguardTheaterKitSpec.
...
TARGET = "FSkyguardTheaterKitSpec AppliedSpec;"
TARGET_WRONG_EQ_NONE = "FSkyguardTheaterKitSpec AppliedSpec = NAME_None;"
TARGET_WRONG_FALSE = "FSkyguardTheaterKitSpec AppliedSpec = false;"
TARGET_WRONG_TRUE = "FSkyguardTheaterKitSpec AppliedSpec = true;"
TARGET_WRONG_ZERO = "FSkyguardTheaterKitSpec AppliedSpec = 0.f;"
TARGET_WRONG_FLOAT = "float AppliedSpec;"
...
APPLIED_SPEC_DECL_RE = re.compile(
    r"FSkyguardTheaterKitSpec\s+AppliedSpec\s*;"
)
APPLIED_SPEC_INIT_RE = re.compile(
    r"FSkyguardTheaterKitSpec\s+AppliedSpec\s*="
)
APPLIED_SPEC_FLOAT_RE = re.compile(r"float\s+AppliedSpec\b")
INVENTED_UPROPERTY = (
    "EditAnywhere",
    "BlueprintReadWrite",
    "BlueprintCallable",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `= 0.f` / `= 160.f` / leftover float Health / leftover
    # `FName WeatherIdentity` when origin/main has a bare
    # FSkyguardTheaterKitSpec. Do not accept sibling Root /
    # RoadInstances / BuildingInstances fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if APPLIED_SPEC_INIT_RE.search(compact):
        return False
    if APPLIED_SPEC_FLOAT_RE.search(compact):
        return False
    if APPLIED_SPEC_DECL_RE.search(compact) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FSkyguardTheaterKitSpec\s+AppliedSpec\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FSkyguardTheaterKitSpec AppliedSpec is missing "
...
class TheaterKitAppliedSpecFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "AppliedSpec"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("AppliedSpec", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_applied_spec_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FSkyguardTheaterKitSpec AppliedSpec;", compact_origin)
        self.assertNotIn("AppliedSpec = NAME_None", compact_origin)
        self.assertNotIn("AppliedSpec = false", compact_origin)
        self.assertNotIn("AppliedSpec = true", compact_origin)
        self.assertNotIn("AppliedSpec = 0.f", compact_origin)
        self.assertNotIn("AppliedSpec = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity;", compact_origin)

    def test_applied_spec_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FSkyguardTheaterKitSpec AppliedSpec"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("FSkyguardTheaterKitSpec", LOCKED_DECL)
        self.assertIn("AppliedSpec", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFSkyguardTheaterKitSpec AppliedSpecs;\n",
            "\tint32 AppliedSpec;\n",
            "\tbool AppliedSpec;\n",
            "\tFName AppliedSpec;\n",
            "\tfloat AppliedSpec = " + forty + ";\n",
            "\tfloat AppliedSpec = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit AppliedSpec field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "applied_spec_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
