# THIS IS leftover-safe ASkyguardGunner CpgMpdLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgMpdLeft;`
# (no initializer).
...
# Stop BEFORE sibling CpgMpdRight as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeLeft, CpgKneeRight, CpgCanopyBow,
# CpgDash, CpgTedac, CpgMpdRight, CpgEufd,
# CpgTedacBezel, CpgMpdLeftText, Boom, GunnerCamera.
# Do NOT claim leftover CpgDash / CpgCanopyBow /
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgSeatPan /
# CpgCockpit / CpgStation (in-flight siblings).
# CpgTedac is a sibling in-flight slot.
# CpgCanopyBow and CpgDash are leftover once drafted.
# CpgMpdLeft fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgMpdLeft;`
...
# CpgMpdLeft fail-closed if LOCKED_DECL is leftover
# CpgDash / CpgCanopyBow / CpgKneeRight / CpgKneeLeft /
# CpgSeatBack / CpgSeatPan / CpgCockpit / CpgStation /
# CpgTedac / CpgMpdRight / leftover Apache Canopy /
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgMpdLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgMpdLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgMpdLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgMpdLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgMpdLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgMpdLeft;"
...
CPG_MPD_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgMpdLeft\s*;"
)
CPG_MPD_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgMpdLeft\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgKneeRight / CpgCanopyBow / CpgSeatBack /
    # CpgSeatPan / CpgCockpit / CpgStation / leftover
    # Apache Canopy, or an initializer on CpgMpdLeft as
    # the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_MPD_LEFT_DECL_RE.search(compact) is None:
        return False
    if CPG_MPD_LEFT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_MPD_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_MPD_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_MPD_RIGHT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgMpdLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgMpdLeft "
...
class GunnerCpgMpdLeftFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgMpdLeft"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_MPD_RIGHT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_MPD_RIGHT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgMpdLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgMpdLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_mpd_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgMpdLeft;", compact_origin)
        self.assertNotIn("CpgMpdLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_mpd_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgMpdLeft"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgMpdLeft;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgMpdLefts;\n",
            "\tint32 CpgMpdLeft = 140;\n",
            "\tbool CpgMpdLeft = true;\n",
            "\tfloat CpgMpdLeft = " + forty + ";\n",
            "\tfloat CpgMpdLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_mpd_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertIn(TARGET_WRONG_TEDAC, section)
        self.assertIn(TARGET_WRONG_DASH, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_MPD_RIGHT))
        self.assertFalse(has_identifier(section, SIBLING_CPG_MPD_RIGHT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgMpdLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_mpd_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
