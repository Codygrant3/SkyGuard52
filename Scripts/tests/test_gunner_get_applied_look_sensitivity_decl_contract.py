# THIS IS leftover-safe ASkyguardGunner GetAppliedLookSensitivity.
# origin/main form: one-line and split-line UFUNCTION wraps
# UFUNCTION(BlueprintPure, Category="Skyguard|Settings")
# wrapping `float GetAppliedLookSensitivity() const { return AppliedLookSensitivity; }`.
# LOCKED_DECL may be declaration-only
# `float GetAppliedLookSensitivity() const;` if the matcher strips
# inline bodies, OR the full origin/main inline form.
# Accept declaration-only AND the origin/main inline body.
# Do NOT fail-closed on `{ return AppliedLookSensitivity; }`.
...
# wraps Category="Settings" / "Settings|Input" WITHOUT the Skyguard|
# prefix). This is leftover-safe unique analog, NOT leftover analog
# Settings and NOT leftover analog Combat|Sortie.
...
# unique leftover-safe suffix, or Skyguard|Settings (leftover
...
# GetAppliedCameraShakeScale / IsVerticalLookInverted
# (other workers; leave uncontracted).
# Do NOT claim leftover unique Combat|Sortie GetAppliedLookSensitivity #1685 /
...
# Do NOT claim leftover unique Gunner FIELD Boom / MouseSensitivity
# #1620 / LookYawLimit / FireCameraShakeClass /
# MinimumSafeSideFireYaw / CPG kit.
...
# GetMouseSensitivity / SetMouseSensitivity / GetInvertVerticalLook
# / SetInvertVerticalLook / GetCameraShakeScale (different class;
# Category Settings without Skyguard|).
...
# Clone leftover unique BlueprintPure int32 UFUNCTION GetAppliedLookSensitivity
# #1685. RETARGET hard: Category Combat|Sortie → Skyguard|Settings,
# method GetAppliedLookSensitivity → GetAppliedLookSensitivity, return int32 →
# float. KEEP BlueprintPure. KEEP const. Accept declaration-only
...
# RETARGET hard.
# LOCKED_DECL is `float GetAppliedLookSensitivity() const;`
# NOT `int32 GetAppliedLookSensitivity() const;` and NOT
# `FName WeatherIdentity;` and NOT
# `float GetAppliedCameraShakeScale() const;` and NOT
# `bool IsVerticalLookInverted() const;` and NOT
# `float GetMouseSensitivity() const;` and NOT
...
# MouseSensitivity = 1.1f FIELD (leftover unique #1620; this slot
# is the UFUNCTION not that FIELD).
...
TARGET = "float GetAppliedLookSensitivity() const;"
TARGET_WRONG_BARE = "float GetAppliedLookSensitivity();"
TARGET_WRONG_INIT = "float GetAppliedLookSensitivity() = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float GetAppliedLookSensitivity() = true;"
TARGET_WRONG_ZERO = "float GetAppliedLookSensitivity() = 0.f;"
TARGET_WRONG_NO_CONST = "float GetAppliedLookSensitivity();"
TARGET_WRONG_INT32 = "int32 GetAppliedLookSensitivity() const;"
TARGET_WRONG_FNAME_RET = "FName GetAppliedLookSensitivity() const;"
...
TARGET_WRONG_GET_SORTIE_HITS_INLINE = (
    "int32 GetSortieHits() const { return SortieHits; }"
)
TARGET_WRONG_CAMERA_SHAKE = (
    "float GetAppliedCameraShakeScale() const;"
)
TARGET_WRONG_INVERT_LOOK = (
    "bool IsVerticalLookInverted() const;"
)
TARGET_WRONG_MOUSE_SENS_FN = "float GetMouseSensitivity() const;"
TARGET_WRONG_MOUSE_SENS_FIELD = "float MouseSensitivity = 1.1f;"
...
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> GetAppliedLookSensitivity;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> GetAppliedLookSensitivity;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> GetAppliedLookSensitivity;"
...
LEFTOVER_GUNNER_MOUSE_SENSITIVITY = (
...
SIBLING_GET_CAMERA_SHAKE = "GetAppliedCameraShakeScale"
SIBLING_IS_VERTICAL_LOOK = "IsVerticalLookInverted"
SIBLING_GET_MOUSE_SENSITIVITY = "GetMouseSensitivity"
SIBLING_SET_MOUSE_SENSITIVITY = "SetMouseSensitivity"
SIBLING_GET_INVERT_VERTICAL = "GetInvertVerticalLook"
SIBLING_SET_INVERT_VERTICAL = "SetInvertVerticalLook"
...
GET_SORTIE_HITS_RE = re.compile(
    r"float\s+GetAppliedLookSensitivity\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+AppliedLookSensitivity\s*;\s*\})"
)
GET_SORTIE_HITS_WRONG_TYPE_RE = re.compile(
    r"(?:int32|FName|bool|FLinearColor|void)\s+"
    r"GetAppliedLookSensitivity\b"
)
GET_SORTIE_HITS_WRONG_BODY_RE = re.compile(
    r"int32\s+GetResolvedProductionLoopRouteCount\s*\(\s*\)\s*const\s*\{"
)
GET_SORTIE_HITS_NO_CONST_RE = re.compile(
    r"float\s+GetAppliedLookSensitivity\s*\(\s*\)\s*;"
)
OPEN_COCKPIT_WIND_LOOP_DECL_RE = GET_SORTIE_HITS_RE
OPEN_COCKPIT_WIND_LOOP_INIT_RE = GET_SORTIE_HITS_WRONG_BODY_RE
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_GET_CAMERA_SHAKE,
        SIBLING_IS_VERTICAL_LOOK,
        SIBLING_GET_MOUSE_SENSITIVITY,
        SIBLING_SET_MOUSE_SENSITIVITY,
        SIBLING_GET_INVERT_VERTICAL,
        SIBLING_SET_INVERT_VERTICAL,
...
def has_one_declaration(region: str, declaration: str) -> bool:
    ...
    pattern = re.compile(
        re.escape(stem)
        + r"\s*(?:;|\{[^}]*return\s+AppliedLookSensitivity\s*;\s*\})"
    )
    return pattern.search(compact_region) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float GetAppliedLookSensitivity() const;`
    # OR origin/main inline `{ return AppliedLookSensitivity; }`.
    ...
    if GET_SORTIE_HITS_WRONG_TYPE_RE.search(compact):
        return False
    if GET_SORTIE_HITS_WRONG_BODY_RE.search(compact):
        return False
    if GET_SORTIE_HITS_RE.search(compact) is None:
        return False
    if GET_SORTIE_HITS_NO_CONST_RE.search(compact) and (
        GET_SORTIE_HITS_RE.search(compact) is None
    ):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident in (
            ...
            break
        if ident != "GetAppliedLookSensitivity":
            cursor = end
            continue
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+GetAppliedLookSensitivity\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for float GetAppliedLookSensitivity "
...
class GunnerGetAppliedLookSensitivityDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetAppliedLookSensitivity"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_GET_CAMERA_SHAKE), body)
        self.assertFalse(has_identifier(section, SIBLING_GET_CAMERA_SHAKE), section)
        self.assertTrue(has_identifier(body, SIBLING_IS_VERTICAL_LOOK), body)
        self.assertFalse(has_identifier(section, SIBLING_IS_VERTICAL_LOOK), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "GetAppliedLookSensitivity"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("GetAppliedLookSensitivity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_applied_look_sensitivity_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('Category="Settings|Input"', section)
        self.assertNotIn('Category="Settings|Audio"', section)
        self.assertNotIn('Category="Settings|Accessibility"', section)
        self.assertIsNone(
        ...
        self.assertNotIn('Skyguard|Combat|Sortie', specifiers)
        self.assertNotIn('Skyguard|Combat|Feedback', specifiers)
        self.assertNotIn('Skyguard|Combat|Safety', specifiers)
        self.assertNotIn('Category="Settings"', specifiers)
        self.assertNotIn('Category="Settings|Input"', specifiers)
        self.assertIsNone(
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            collapsed("float GetAppliedLookSensitivity();"),
            compact_origin,
        )
    ...
    def test_get_applied_look_sensitivity_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float GetAppliedLookSensitivity"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GET_SORTIE_HITS_INLINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CAMERA_SHAKE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_INVERT_LOOK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MOUSE_SENS_FN)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MOUSE_SENS_FIELD)
        ...
        self.assertNotIn("GetAppliedCameraShakeScale", LOCKED_DECL)
        self.assertNotIn("IsVerticalLookInverted", LOCKED_DECL)
        self.assertNotIn("GetMouseSensitivity", LOCKED_DECL)
        self.assertNotIn("MouseSensitivity", LOCKED_DECL)
        self.assertNotIn("45000.f", LOCKED_DECL)
        self.assertNotIn("1.1f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat GetAppliedLookSensitivitys() const;\n",
            "\tint32 GetAppliedLookSensitivity() const;\n",
            "\tFName GetAppliedLookSensitivity() const;\n",
            "\tfloat GetAppliedLookSensitivity() const { return " + forty + "; }\n",
            "\tfloat GetAppliedLookSensitivity() const { return " + eighty + "; }\n",
            f"\t{TARGET_WRONG_LAMP}\n",
            f"\t{TARGET_WRONG_PEAK}\n",
            ...
            f"\t{TARGET_WRONG_GET_SORTIE_HITS_INLINE}\n",
            f"\t{TARGET_WRONG_CAMERA_SHAKE}\n",
            f"\t{TARGET_WRONG_INVERT_LOOK}\n",
            f"\t{TARGET_WRONG_MOUSE_SENS_FN}\n",
            f"\t{TARGET_WRONG_MOUSE_SENS_FIELD}\n",
    ...
    def test_missing_category_or_blueprint_pure_fails_closed(self) -> None:
        ...
        self.assertNotIn('Category="Settings|Input"', origin)
        self.assertNotIn('Category="Settings|Audio"', origin)
        self.assertNotIn('Category="Settings|Accessibility"', origin)
        self.assertIsNone(
        ...
        settings_a11y_specs = attached_uproperty_specifiers(
            spec_section(leftover_settings_a11y)
        )
        self.assertIn(
            'Category="Settings|Accessibility"',
            settings_a11y_specs,
        )
        self.assertNotIn('Category="Skyguard|Settings"', settings_a11y_specs)
        wraps = (
            "\tfloat\n\tGetAppliedLookSensitivity() const;\n",
            "\tfloat   GetAppliedLookSensitivity() const;\n",
            "\tfloat\tGetAppliedLookSensitivity() const;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_ufunctions_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetAppliedLookSensitivity"))
        ...
        self.assertFalse(has_identifier(section, SIBLING_GET_SORTIE_HITS))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_CAMERA_SHAKE))
        self.assertFalse(has_identifier(section, SIBLING_GET_CAMERA_SHAKE))
        self.assertTrue(has_identifier(leaked, SIBLING_IS_VERTICAL_LOOK))
        self.assertFalse(has_identifier(section, SIBLING_IS_VERTICAL_LOOK))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "GetAppliedLookSensitivity"))
        self.assertTrue(has_identifier(section, "GetAppliedLookSensitivity"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_GET_SORTIE_HITS))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner GetAppliedLookSensitivity decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_get_applied_look_sensitivity_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_GUNNER_MOUSE_SENSITIVITY, LOCKED_SCRIPTS)
        self.assertNotIn(THIS_SCRIPT, (
            LEFTOVER_GUNNER_GET_SORTIE_HITS,
            LEFTOVER_GUNNER_GET_SORTIE_SHOTS,
            LEFTOVER_GUNNER_RESET_SORTIE,
        ))
