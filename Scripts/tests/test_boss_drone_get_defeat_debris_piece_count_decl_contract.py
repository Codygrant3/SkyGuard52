...
# or lock the GetDefeatDebrisPieceCount body.
# origin/main is the live one-line inline form
# (`int32 GetDefeatDebrisPieceCount() const
# { return DefeatDebrisComponents.Num(); }`);
# lock
# `int32 GetDefeatDebrisPieceCount() const;`
...
# only. Stay off leftover strike /
# lock-eligible surfaces. Stay off leftover
# sibling ApplyWeaponHit /
# IssuePilotCommand /
...
# Python GetDefeatDebrisPieceCount declaration
...
# .cpp GetDefeatDebrisPieceCount body / invented
# INDEX_NONE stay unlocked. Do not invent INDEX_NONE
# or lock the cpp body. Do not parse leftover HUD
# classes. Do not lock the live inline
# DefeatDebrisComponents.Num() body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "ASkyguardBossDroneBase::GetDefeatDebrisPieceCount",
    "SkyguardBossDroneBase.cpp",
    "SetBossPhase",
...
class BossDroneGetDefeatDebrisPieceCountDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_DEFEAT_DEBRIS_PIECE_COUNT),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertIn("GetDefeatDebrisPieceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_missing_get_defeat_debris_piece_count_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertIn("GetDefeatDebrisPieceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_BOSS_DESTRUCTION}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertIn("GetDefeatDebrisPieceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_BOSS_DESTRUCTION, section)
        self.assertTrue(
            has_declaration(section, GET_DEFEAT_DEBRIS_PIECE_COUNT),
            section,
        )
        self.assertIn("BlueprintPure", UFUNCTION_BOSS_DESTRUCTION)
        self.assertNotIn("BlueprintPure", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("UFUNCTION", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("Category", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("BlueprintCallable", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("BlueprintCallable", UFUNCTION_BOSS_DESTRUCTION)
        self.assertIn("Skyguard|Boss|Destruction", UFUNCTION_BOSS_DESTRUCTION)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertIn("GetDefeatDebrisPieceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tint32 GetDefeatDebrisPieceCount();\n"
        missing_parens = "\tint32 GetDefeatDebrisPieceCount;\n"
        wrong_return_void = "\tvoid GetDefeatDebrisPieceCount() const;\n"
        wrong_return_bool = "\tbool GetDefeatDebrisPieceCount() const;\n"
        wrong_return_float = "\tfloat GetDefeatDebrisPieceCount() const;\n"
        added_arg = (
            "\tint32 GetDefeatDebrisPieceCount(int32 Extra) const;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_DEFEAT_DEBRIS_PIECE_COUNT)
            self.assertIn("GetDefeatDebrisPieceCount", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_defeat_debris_piece_count_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, GET_DEFEAT_DEBRIS_PIECE_COUNT),
            1,
        )
        self.assertTrue(
            GET_DEFEAT_DEBRIS_PIECE_COUNT.startswith("int32 "),
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        self.assertTrue(
            GET_DEFEAT_DEBRIS_PIECE_COUNT.endswith(";"),
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        self.assertIn(
        ...
        self.assertIn(" const", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("INDEX_NONE", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("{", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("}", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("return ", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "DefeatDebrisComponents",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        self.assertNotIn("ApplyWeaponHit", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("IssuePilotCommand", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        ...
        self.assertNotIn("GetBossPhase", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("GetTelemetry", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("GetChinMuzzleLocation", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("GetGunnerMount", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("FaceWorldLocation", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("WeakPoints", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("SetSearchlightTracked", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("SetContactIdentified", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        self.assertNotIn("SetLightningExposed", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, GET_DEFEAT_DEBRIS_PIECE_COUNT)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tint32\n"
            "\tGetDefeatDebrisPieceCount() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tint32 GetDefeatDebrisPieceCount\n"
            "\t() const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_DEFEAT_DEBRIS_PIECE_COUNT}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_DEFEAT_DEBRIS_PIECE_COUNT),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_DEFEAT_DEBRIS_PIECE_COUNT}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_defeat_debris_piece_count_cpp_body(
        self,
    ) -> None:
        locked_only = f"{GET_DEFEAT_DEBRIS_PIECE_COUNT}\n"
        self.assertNotIn("{", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        ...
        self.assertNotIn("return false", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("return true", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "DefeatDebrisComponents",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        self.assertNotIn("SetBossPhase", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        self.assertNotIn("RegisterDefeatDebris", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("CleanupDefeatDebris", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("HandleDefeated", GET_DEFEAT_DEBRIS_PIECE_COUNT)

    def test_contract_does_not_relock_apply_weapon_hit_sibling(self) -> None:
        locked_only = f"{GET_DEFEAT_DEBRIS_PIECE_COUNT}\n"
        for token in APPLY_WEAPON_HIT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("ApplyWeaponHit", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("PortGuidanceArray", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("CommandCore", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("SetSearchlightTracked", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("IsSearchlightTracked", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("SetContactIdentified", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("IsContactIdentified", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("OpenOrbitExposure", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("ASkyguardIronRainBoss", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("GetGunnerMount", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("GetChinTurret", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("GetSensorTurret", GET_DEFEAT_DEBRIS_PIECE_COUNT)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_DEFEAT_DEBRIS_PIECE_COUNT}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("GetChinMuzzleLocation", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("ESkyguardApacheSystem", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("ASkyguardGunner", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("FillAndFinalize", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("SkyguardRadarNode", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("MinHeightFromOriginCm", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("MaxIntegrity", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
        self.assertNotIn("SkyguardPathfinder", GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_DEFEAT_DEBRIS_PIECE_COUNT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_DEFEAT_DEBRIS_PIECE_COUNT}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_DEFEAT_DEBRIS_PIECE_COUNT),
            GET_DEFEAT_DEBRIS_PIECE_COUNT,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone GetDefeatDebrisPieceCount contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_DEFEAT_DEBRIS_PIECE_COUNT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_DEFEAT_DEBRIS_PIECE_COUNT.lower(),
                "boss drone GetDefeatDebrisPieceCount contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_DEFEAT_DEBRIS_PIECE_COUNT)

    def test_contract_is_get_defeat_debris_piece_count_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertNotIn("{", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertTrue(GET_DEFEAT_DEBRIS_PIECE_COUNT.startswith("int32 "))
        self.assertTrue(GET_DEFEAT_DEBRIS_PIECE_COUNT.endswith(";"))
        self.assertIn(" const", GET_DEFEAT_DEBRIS_PIECE_COUNT)
        self.assertIn(UFUNCTION_BOSS_DESTRUCTION, section)
