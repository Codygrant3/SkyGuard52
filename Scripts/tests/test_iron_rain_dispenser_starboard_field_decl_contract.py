...
# INDEX_NONE or lock DispenserStarboard
# construction in the .cpp. This is a FIELD
# contract on ASkyguardIronRainBoss, not leftover
# Iron Rain methods (already drafted) and not
# leftover Iron Rain finish methods. This is
# not a leftover
# USkyguardBossWeakPointComponent member
# contract. origin/main is a one-line field
# (`TObjectPtr<USkyguardBossWeakPointComponent>
# DispenserStarboard;`); accept that form and
...
# Category="Skyguard|Mission09|Boss") is required
# as present. Accept one-line and split-line
# UPROPERTY wraps. Parse the public class
# section of ASkyguardIronRainBoss only.
# Do not lock leftover sibling Iron Rain
# fields DispenserPort, DispenserCenter,
# CommandAntennaPort, CommandAntennaStarboard,
# DecoyController, EnginePodPort,
# EnginePodCenter, EnginePodStarboard,
# FuelControlPort, FuelControlStarboard,
# DebrisPortWing, DebrisCenterRack,
# DebrisStarboardWing, or MaxReleasesPerBay
# as the primary lock. Stay off leftover
# Iron Rain methods OpenDispenserBay /
...
# Stay off leftover constructor / BeginPlay /
# HandleWeakPointDestroyed. Stay off leftover
# Tempest ControlServo / PortDischargeBoom /
# StarboardDischargeBoom / EngineIntake fields
# and leftover Tempest lightning / gust /
# lock-stability methods. Stay off leftover
...
# isolated-test drafts #107–#519 including
...
# leftover #107–#519, plus
# SkyguardIronRainBoss production files.
# This lane only adds an isolated Python
# DispenserStarboard field declaration
# contract on ASkyguardIronRainBoss. Stay
# off leftover sibling Iron Rain fields,
# leftover Iron Rain methods, leftover
# Iron Rain finish methods, leftover
...
# leftover isolated-test drafts #107–#519,
# leftover searchlight-track-runtime-defaults
...
# siblings including leftover ControlServo,
...
# isolated Iron Rain boss field contracts stay
# unlocked for THIS test's primary lock. Leftover
# Iron Rain methods, leftover Iron Rain finish
# methods, leftover Tempest ControlServo /
# discharge / intake field contracts, leftover
# Pathfinder / LastFlight / BossDrone / Apache /
# Gunner / settings / input-capture / briefing /
# debrief isolated contracts, leftover Harbor
# #6/#8/#9, leftover theater-kit #59, leftover
# drafts #56–#64, leftover isolated-test drafts
# #107–#519 stay sibling-only.
LOCKED_SCRIPTS = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardIronRainBoss::DispenserStarboard",
    "SkyguardIronRainBoss.cpp",
    "SetExposed",
...
class IronRainDispenserStarboardFieldDeclContractTests(unittest.TestCase):
    def test_iron_rain_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DISPENSER_STARBOARD_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DISPENSER_STARBOARD_FIELD)
        self.assertIn("DispenserStarboard", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DISPENSER_STARBOARD_FIELD)
    ...
    def test_missing_dispenser_starboard_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardIronRainBoss();\n"
            f"\t{DISPENSER_PORT_FIELD}\n"
            ...
            f"\t{FUEL_CONTROL_PORT_FIELD}\n"
            f"\t{DEBRIS_PORT_WING_FIELD}\n"
            f"\t{MAX_RELEASES_PER_BAY_FIELD}\n"
            f"\t{OPEN_DISPENSER_BAY}\n"
            f"\t{PORT_DISCHARGE_BOOM_FIELD}\n"
            f"\t{STARBOARD_DISCHARGE_BOOM_FIELD}\n"
            f"\t{ENGINE_INTAKE_FIELD}\n"
            f"\t{DEBRIS_PORT_PANEL_FIELD}\n"
            f"\t{DEBRIS_STARBOARD_PANEL_FIELD}\n"
            f"\t{DEBRIS_INTAKE_PANEL_FIELD}\n"
            f"\t{REQUIRED_LOCK_STABILITY_SECONDS_FIELD}\n"
            f"\t{SET_LIGHTNING_EXPOSED}\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DISPENSER_STARBOARD_FIELD)
        self.assertIn("DispenserStarboard", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_STARBOARD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DISPENSER_STARBOARD_FIELD)
        self.assertIn("DispenserStarboard", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_STARBOARD, section)
        self.assertIn("VisibleAnywhere", section)
        self.assertIn("BlueprintReadOnly", section)
        self.assertIn('Category="Skyguard|Mission09|Boss"', section)
        self.assertTrue(
            has_declaration(section, DISPENSER_STARBOARD_FIELD),
            section,
        )
        self.assertNotIn("UPROPERTY", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("VisibleAnywhere", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("BlueprintReadOnly", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("Category", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_STARBOARD)
        self.assertNotIn("BlueprintCallable", UPROPERTY_STARBOARD)
        self.assertIn("Skyguard|Mission09|Boss", UPROPERTY_STARBOARD)
        self.assertIn("Mission09", UPROPERTY_STARBOARD)
        self.assertNotIn("Destruction", UPROPERTY_STARBOARD)
        self.assertNotIn("Encounter", UPROPERTY_STARBOARD)
        self.assertNotIn("Mission10", UPROPERTY_STARBOARD)
        self.assertNotIn("Safety", UPROPERTY_STARBOARD)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_STARBOARD)
            self.assertNotIn(invented, DISPENSER_STARBOARD_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DISPENSER_STARBOARD_FIELD)
        self.assertIn("DispenserStarboard", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* DispenserStarboard;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DispenserStarboard;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> DispenserStarboard;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DISPENSER_STARBOARD_FIELD)
            self.assertIn("DispenserStarboard", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_dispenser_starboard_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, DISPENSER_STARBOARD_FIELD),
            DISPENSER_STARBOARD_FIELD,
        )
        self.assertTrue(has_declaration(section, DISPENSER_STARBOARD_FIELD))
        self.assertEqual(declaration_count(section, DISPENSER_STARBOARD_FIELD), 1)
        self.assertTrue(
            DISPENSER_STARBOARD_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            DISPENSER_STARBOARD_FIELD,
        )
        self.assertTrue(DISPENSER_STARBOARD_FIELD.endswith(";"), DISPENSER_STARBOARD_FIELD)
        self.assertIn("DispenserStarboard", DISPENSER_STARBOARD_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            DISPENSER_STARBOARD_FIELD,
        )
        self.assertIn("TObjectPtr<", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("TArray<", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("=", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("INDEX_NONE", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("UFUNCTION", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("{", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("}", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("return ", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("CommandAntenna", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ControlLinkage", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("NoseCamera", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisCenter", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisTail", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisNose", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisSpine", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("AdvanceEncounter", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ResetEncounterState", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("IsRouteStateSafe", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetRouteProgress", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("RadarNode", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("IssueClimbCommand", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetFinaleStage", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("IsWreckDiverted", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetGunnerMount", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("FaceWorldLocation", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("PortGuidanceArray", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisArmorPort", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetSearchlightTracked", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetContactIdentified", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetLightningExposed", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("HullCollider", DISPENSER_STARBOARD_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DISPENSER_STARBOARD_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_STARBOARD}\n"
            f"\t{DISPENSER_STARBOARD_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_STARBOARD} {DISPENSER_STARBOARD_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, DISPENSER_STARBOARD_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{DISPENSER_STARBOARD_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, DISPENSER_STARBOARD_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DISPENSER_STARBOARD_FIELD),
            section,
    ...
    def test_assigned_dispenser_starboard_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DispenserStarboard = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DISPENSER_STARBOARD_FIELD)
        self.assertIn("DispenserStarboard", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DISPENSER_STARBOARD_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            f"\t{DECOY_CONTROLLER_FIELD}\n",
            f"\t{ENGINE_POD_PORT_FIELD}\n",
            f"\t{ENGINE_POD_CENTER_FIELD}\n",
            f"\t{ENGINE_POD_STARBOARD_FIELD}\n",
            f"\t{FUEL_CONTROL_PORT_FIELD}\n",
            f"\t{FUEL_CONTROL_STARBOARD_FIELD}\n",
            f"\t{DEBRIS_PORT_WING_FIELD}\n",
            f"\t{DEBRIS_CENTER_RACK_FIELD}\n",
            f"\t{DEBRIS_STARBOARD_WING_FIELD}\n",
            f"\t{MAX_RELEASES_PER_BAY_FIELD}\n",
            f"\t{CONTROL_SERVO_FIELD}\n",
            f"\t{PORT_DISCHARGE_BOOM_FIELD}\n",
            f"\t{STARBOARD_DISCHARGE_BOOM_FIELD}\n",
            f"\t{ENGINE_INTAKE_FIELD}\n",
            f"\t{DEBRIS_PORT_PANEL_FIELD}\n",
            ...
            self.assertIn("DispenserStarboard", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DISPENSER_STARBOARD_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DISPENSER_STARBOARD_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DISPENSER_STARBOARD_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_STARBOARD, section)
        self.assertTrue(has_declaration(section, DISPENSER_STARBOARD_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DISPENSER_STARBOARD_FIELD.startswith("UFUNCTION"),
            DISPENSER_STARBOARD_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DISPENSER_STARBOARD_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DISPENSER_STARBOARD_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_dispenser_starboard_cpp_body(self) -> None:
        locked_only = f"{DISPENSER_STARBOARD_FIELD}\n"
        self.assertNotIn("{", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("}", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("return ", DISPENSER_STARBOARD_FIELD)
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SkyguardIronRainBoss.cpp", locked_only)
        self.assertNotIn("return false", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("return true", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetExposed", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DISPENSER_STARBOARD_FIELD,
        )
        self.assertNotIn("SetBossPhase", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DispenserPort", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DispenserCenter", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("CommandAntennaPort", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("CommandAntennaStarboard", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DecoyController", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("EnginePodPort", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("EnginePodCenter", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("EnginePodStarboard", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("FuelControlPort", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("FuelControlStarboard", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisPortWing", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisCenterRack", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisStarboardWing", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("MaxReleasesPerBay", DISPENSER_STARBOARD_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            DISPENSER_STARBOARD_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ControlServo", DISPENSER_STARBOARD_FIELD)

    def test_contract_does_not_relock_leftover_sibling_tempest_fields(self) -> None:
        locked_only = f"{DISPENSER_STARBOARD_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("PortDischargeBoom", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("StarboardDischargeBoom", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("EngineIntake", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisPortPanel", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisStarboardPanel", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("DebrisIntakePanel", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", DISPENSER_STARBOARD_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            DISPENSER_STARBOARD_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("EncounterController", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DISPENSER_STARBOARD_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, DISPENSER_STARBOARD_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetSearchlightTracked", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetContactIdentified", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("OpenOrbitExposure", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SetLightningExposed", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("IsLightningExposed", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("OpenDispenserBay", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ReleasePooledEscort", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("IssueCrossCommand", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetManeuver", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("BeginPlay", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ASkyguardGunner", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SkyguardRadarNode", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("MaxIntegrity", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("CurrentIntegrity", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DISPENSER_STARBOARD_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            DISPENSER_STARBOARD_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DISPENSER_STARBOARD_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", DISPENSER_STARBOARD_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DISPENSER_STARBOARD_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DISPENSER_STARBOARD_FIELD),
            DISPENSER_STARBOARD_FIELD,
        )
        ...
        self.assertNotIn("CommandAntennaStarboard", locked_only)
        self.assertNotIn("DecoyController", locked_only)
        self.assertNotIn("EnginePodPort", locked_only)
        self.assertNotIn("EnginePodCenter", locked_only)
        self.assertNotIn("EnginePodStarboard", locked_only)
        self.assertNotIn("FuelControlPort", locked_only)
        self.assertNotIn("FuelControlStarboard", locked_only)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardIronRainBoss::DispenserStarboard", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain DispenserStarboard field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DISPENSER_STARBOARD_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DISPENSER_STARBOARD_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DISPENSER_STARBOARD_FIELD.lower(),
                "iron rain DispenserStarboard contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DISPENSER_STARBOARD_FIELD)

    def test_contract_is_dispenser_starboard_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(DISPENSER_STARBOARD_FIELD.endswith(";"))
        self.assertNotIn("=", DISPENSER_STARBOARD_FIELD)
        self.assertIn(UPROPERTY_STARBOARD, section)
