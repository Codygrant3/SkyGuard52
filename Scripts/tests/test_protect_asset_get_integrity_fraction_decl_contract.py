# THIS IS leftover-safe ASkyguardProtectAsset::GetIntegrityFraction.
# origin/main form: split-line
# UFUNCTION(BlueprintPure, Category="Skyguard|Campaign")
# then `float GetIntegrityFraction() const;`
# THIS IS UFUNCTION(BlueprintPure) ONLY. Category IS
# `Skyguard|Campaign`. Fail-closed if the UFUNCTION or
# signature is missing or renamed, if BlueprintPure is
# dropped, if Category= is removed or changed, or if
# `const` is dropped. Accept origin/main split-line
# UFUNCTION(BlueprintPure, Category="Skyguard|Campaign").
# Parse CLASS public section ONLY after
# `class SKYGUARD52_API ASkyguardProtectAsset`, stop at
# `protected:`. Do not parse protected Root/Hull/Volume.
...
# Do NOT contract leftover BindCargoHull /
# MakeCargoHullBindSlot / GetHull / BeginPlay.
...
TARGET = "float GetIntegrityFraction() const;"
TARGET_WRONG_NO_CONST = "float GetIntegrityFraction();"
TARGET_WRONG_BOOL = "bool GetIntegrityFraction() const;"
TARGET_WRONG_VOID = "void GetIntegrityFraction() const;"
TARGET_WRONG_RENAME = "float GetIntegrity() const;"
TARGET_WRONG_PLURAL = "float GetIntegrityFractions() const;"
TARGET_WRONG_INLINE = "float GetIntegrityFraction() const { return 1.f; }"
TARGET_WRONG_SIBLING_APPLY = "void ApplyDamage(float Amount);"
TARGET_WRONG_SIBLING_DESTROYED = "bool IsDestroyed() const;"
TARGET_WRONG_SIBLING_RESET = "void ResetIntegrity();"
LOCKED_DECL = TARGET
UFUNCTION_PURE = 'UFUNCTION(BlueprintPure, Category="Skyguard|Campaign")'
...
def leftover_cargo_proxy_tokens() -> tuple[str, ...]:
    return (
        "BindCargoHull",
        "MakeCargoHullBindSlot",
        "GetHull",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept a missing const, inline body, rename,
    # or wrong return type when origin/main has
    # `float GetIntegrityFraction() const;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+GetIntegrityFraction\s*\(\s*\)\s*const\s*\{",
        compact,
    ):
        return False
    if re.search(
        r"float\s+GetIntegrityFraction\s*\(\s*\)\s*;",
        compact,
    ) and re.search(
        r"float\s+GetIntegrityFraction\s*\(\s*\)\s*const\s*;",
        compact,
...
def public_section(header: str) -> str:
    ...
    for member in PROTECTED_MEMBERS:
        if has_identifier(section, member):
            raise AssertionError(
                f"{CLASS_NAME} public section includes protected {member}"
            )
    return section
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+GetIntegrityFraction\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for float GetIntegrityFraction is missing from "
...
class ProtectAssetGetIntegrityFractionDeclContractTests(unittest.TestCase):
    def test_protect_asset_class_exists(self) -> None:
        ...
        for leftover in leftover_cargo_proxy_tokens():
    ...
    def test_missing_class_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            class_body(
                "class SKYGUARD52_API ASkyguardUnrelatedAsset "
                ": public AActor\n{\n};\n"
            )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            "class SKYGUARD52_API AOtherProtectAsset\n"
            "{\n"
            "public:\n"
            f"\t{UFUNCTION_PURE}\n"
            f"\t{LOCKED_DECL}\n"
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("GetIntegrityFraction", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_integrity_fraction_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn("Skyguard|Campaign", specifiers)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn("GetIntegrityFraction()const;", collapsed(origin))
        self.assertNotIn(
            "GetIntegrityFraction()const {",
            collapsed(origin),
        )

    def test_get_integrity_fraction_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float GetIntegrityFraction"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RENAME}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_INLINE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_BOOL}\n", LOCKED_DECL)
        self.assertIn("GetIntegrityFraction", str(raised.exception))
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_PLURAL}\n",
            f"\t{TARGET_WRONG_INLINE}\n",
            f"\t{TARGET_WRONG_SIBLING_APPLY}\n",
            f"\t{TARGET_WRONG_SIBLING_DESTROYED}\n",
            f"\t{TARGET_WRONG_SIBLING_RESET}\n",
            leftover_primary,
            leftover_guided,
            "\tfloat GetIntegrityFraction() const { return CurrentIntegrity; }\n",
            "\tint32 GetIntegrityFraction() const;\n",
            "\tfloat GetIntegrityFraction(float Amount) const;\n",
            "\tfloat GetIntegrityFraction() const\n",
        )
    ...
    def test_missing_or_wrong_ufunction_fails_closed(self) -> None:
        callable_only = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        section = public_section(callable_only)
        specifiers = attached_ufunction_specifiers(section)
        ...
        self.assertIn("Skyguard|Campaign", origin)

    def test_category_removed_fails_closed(self) -> None:
        uncategorized = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        section = public_section(uncategorized)
        specifiers = attached_ufunction_specifiers(section)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            'Category="Skyguard|Campaign")\n'
            f"\t{LOCKED_DECL}\n",
            "public:\n\tfloat GetIntegrityFraction()\n\tconst;\n",
        )
        for region in wraps:
            self.assertTrue(has_declaration(region, LOCKED_DECL), region)

    def test_does_not_contract_sibling_methods_or_integrity_fields(self) -> None:
        for method in SIBLING_METHODS:
            self.assertNotIn(method, LOCKED_DECL)
        for field in SIBLING_FIELDS:
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_parse_window_stops_before_protected_members(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        leaked = class_body(header)
        for member in PROTECTED_MEMBERS:
            self.assertFalse(has_identifier(section, member), member)
            self.assertTrue(has_identifier(leaked, member), member)
        self.assertIn("protected:", leaked)
        self.assertNotIn("protected:", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "protect-asset GetIntegrityFraction decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_protect_asset_get_integrity_fraction"
            "_decl_contract.py"
        ))
