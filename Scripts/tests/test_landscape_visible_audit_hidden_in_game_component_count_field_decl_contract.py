...
# HiddenInGameComponentCount FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# int32 HiddenInGameComponentCount = 0; one-line /
...
# LOCK in-flight RenderStateCreatedComponentCount script;
# do not edit it.
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# HiddenInGameComponentCount FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 HiddenInGameComponentCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# LandscapeComponentCount later; this ticket is HiddenInGameComponentCount only).
# LOCK leftover landscape-visible-audit-defaults #0bce.
# LOCK leftover Footprint bSuccess #1189.
...
# BlueprintReadOnly int32 HiddenInGameComponentCount = 0;
...
# HiddenInGameComponentCount FIELD (declaration-only). It is
...
TARGET = "int32 HiddenInGameComponentCount = 0;"
TARGET_WRONG = "float HiddenInGameComponentCount = 0;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+HiddenInGameComponentCount\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 HiddenInGameComponentCount is missing from "
...
class LandscapeVisibleAuditHiddenInGameComponentCountFieldDeclContractTests(unittest.TestCase):
    def test_landscape_visible_audit_struct_exists(self) -> None:
        ...
        self.assertIn("HiddenInGameComponentCount", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("HiddenInGameComponentCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_hidden_in_game_component_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_hidden_in_game_component_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 HiddenInGameComponentCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("HiddenInGameComponentCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("HiddenInGameComponentCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 HiddenInGameComponentCount"))
    ...
    def test_contract_does_not_relock_leftover_environment_readiness(
        ...
        self.assertIn(
            "Scripts/tests/test_landscape_visible_audit"
            "_render_state_created_component_count_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission01_environment_readiness"
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("HiddenInGameComponentCount", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_hidden_in_game_component_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
