# THIS IS leftover-safe FSkyguardObjectiveDefinition RequiredProgress.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "1"))
# then `int32 RequiredProgress = 1;` (initializer IS `= 1`,
# no Category).
# THIS IS leftover-safe isolated UPROPERTY with no Category.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# ClampMin = "1" IS present. Initializer IS `= 1`.
# There is NO Category=. Spaces around `=` in meta ARE required.
...
# if the initializer is not `= 1`, if meta ClampMin
# is missing or wrong, or if Harbor tokens
# built as 40 + ".f" / 80 + ".f" appear in the parse window.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardObjectiveDefinition` body ONLY after
# `struct FSkyguardObjectiveDefinition`. Stop at
# `struct FSkyguardEnemyFormationDefinition` and later structs
# (`FSkyguardObjectiveProgress`, `FSkyguardMissionScoreRules`,
# `FSkyguardWeatherProfile`, `FSkyguardRouteDefinition`,
# `USkyguardMissionDefinition` if present).
# Do NOT parse `FSkyguardObjectiveProgress`
# (leftover objective-progress-defaults #182 / leftover
# ObjectiveProgress fields #1261-#1263).
# Do NOT parse `FSkyguardMissionScoreRules`
# (leftover mission-score-rules-defaults #185 / #deae).
# Do NOT parse `FSkyguardWeatherProfile`
# (leftover weather-profile-defaults #184 / leftover
# weather-profile isolated ProfileId).
# Do NOT parse `FSkyguardEnemyFormationDefinition`
# (leftover enemy-formation-defaults #183 / #2f1a).
# Do NOT parse `FSkyguardEnemyWaveDefinition`
# (leftover enemy-wave-defaults #189 / #dc07).
# Do NOT parse `FSkyguardRouteDefinition`
# (leftover route-definition-fields #192 / #dbff).
# Do NOT parse `USkyguardMissionDefinition`.
# Do NOT parse leftover objective-definition-defaults
# #181 / #b29f.
# Do NOT parse leftover route-point-defaults #180 / #0ac5.
...
# isolated fields #1332-#1359.
# Do NOT contract sibling fields ObjectiveId / DisplayName /
# Type / bRequiredForMissionSuccess /
# bFailureEndsMission / ScoreReward.
...
# #153 / #f47d.
# THIS IS NOT leftover mission-definition Objectives.
...
# the bulk).
# Analog: leftover objective-definition-defaults #181
# then this isolated RequiredProgress field decl, same as leftover
...
# `FName WeatherIdentity` / `FName ObjectiveId` /
# `float RequiredProgress` / no `=` / ClampMin missing,
# retarget: type is int32 RequiredProgress,
# initializer is `= 1`, specifiers are EditAnywhere +
# BlueprintReadOnly + meta = (ClampMin = "1"),
# no Category.
...
STRUCT_NAME = "FSkyguardObjectiveDefinition"
...
TARGET = "int32 RequiredProgress = 1;"
TARGET_WRONG_EQ_NONE = "int32 RequiredProgress = NAME_None;"
TARGET_WRONG_FALSE = "int32 RequiredProgress = false;"
TARGET_WRONG_TRUE = "int32 RequiredProgress = true;"
TARGET_WRONG_ZERO = "int32 RequiredProgress = 0;"
TARGET_WRONG_FLOAT = "float RequiredProgress = 1;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "int32 RequiredProgress;"
TARGET_WRONG_REWARD = "int32 RequiredProgress = 1000;"
TARGET_WRONG_FNAME = "FName RequiredProgress;"
TARGET_WRONG_SCORE = "int32 ScoreReward = 1;"
...
SIBLING_OBJECTIVE_ID = "ObjectiveId"
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_OBJECTIVE_ID,
        SIBLING_DISPLAY_NAME,
        SIBLING_TYPE,
        SIBLING_REQUIRED_FOR_SUCCESS,
        SIBLING_FAILURE_ENDS_MISSION,
        SIBLING_SCORE_REWARD,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Reject missing initializer, `= 0` / `= 1000` /
    # Harbor 40/80 / leftover Health / float / FName /
    # sibling ScoreReward = 1. Do not accept
    # sibling ObjectiveId / DisplayName / Type /
    # bRequiredForMissionSuccess / bFailureEndsMission /
    # ScoreReward fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+RequiredProgress\s*=\s*1\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"int32\s+RequiredProgress\s*=\s*0\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredProgress\s*=\s*1000\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+RequiredProgress\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+RequiredProgress\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RequiredProgress\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+RequiredProgress\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 RequiredProgress is missing from "
...
def require_required_progress_specifiers(section: str) -> str:
    ...
    if 'ClampMin = "1"' not in specifiers:
        raise AssertionError(
            'ClampMin = "1" is missing or wrong on origin/main:'
            f"{HEADER_PATH} struct {STRUCT_NAME} "
            "RequiredProgress UPROPERTY"
        )
    if "ClampMax" in specifiers:
        raise AssertionError(
            "ClampMax is added on origin/main:"
            f"{HEADER_PATH} struct {STRUCT_NAME} "
            "RequiredProgress UPROPERTY"
        )
    return specifiers
...
class ObjectiveDefinitionRequiredProgressFieldDeclContractTests(unittest.TestCase):
    def test_objective_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RequiredProgress"), section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedObjectiveDefinition\n{\n};\n"
            )
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RequiredProgress"), section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEFINITION, section)
        ...
        self.assertIn("RequiredProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_required_progress_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_required_progress_specifiers(section)
    ...
    def test_initializer_fails_closed_when_not_1(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
        ...
        self.assertIn("int32 RequiredProgress = 1;", compact_origin)
        self.assertNotIn("RequiredProgress = NAME_None", compact_origin)
        self.assertNotIn("RequiredProgress = false", compact_origin)
        self.assertNotIn("RequiredProgress = true", compact_origin)
        self.assertNotIn("RequiredProgress = 0.f", compact_origin)
        self.assertNotIn("RequiredProgress = 160.f", compact_origin)

    def test_required_progress_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 RequiredProgress"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 1", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_REWARD}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_REWARD}\n",
            f"\t{TARGET_WRONG_FNAME}\n",
            f"\t{TARGET_WRONG_SCORE}\n",
            leftover_primary,
            leftover_guided,
            f"\tFText {SIBLING_DISPLAY_NAME};\n",
            "\tFName RequiredProgresses;\n",
            "\tbool RequiredProgress;\n",
            "\tfloat RequiredProgress = " + forty + ";\n",
            "\tfloat RequiredProgress = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_required_progress_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(with_category))
        ...
        self.assertIn('ClampMin = "1"', origin)
        no_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(EditAnywhere, BlueprintReadOnly)\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(compact_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        wrong_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(wrong_clamp))
        self.assertIn("ClampMin", str(raised.exception))
        with_max = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(EditAnywhere, BlueprintReadOnly, "
            'meta = (ClampMin = "1", ClampMax = "1.0"))\n'
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_required_progress_specifiers(spec_section(with_max))
    ...
    def test_does_not_contract_sibling_objective_definition_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin = "1"', UPROPERTY_EDIT_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_EDIT_READONLY)
        self.assertNotIn("Category", UPROPERTY_EDIT_READONLY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "objective-definition RequiredProgress field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "objective_definition_required_progress_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
    ...
    def test_sibling_objective_definition_fields_stay_unclaimed(self) -> None:
        remaining = (
            REMAINING_OBJECTIVE_ID_SCRIPT,
            REMAINING_DISPLAY_NAME_SCRIPT,
            REMAINING_TYPE_SCRIPT,
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_OBJECTIVE_ID, locked_only)
