# THIS IS leftover-safe ASkyguardCoastalEnvironmentDirector VegetationStartCullDistanceCm.
...
# then `int32 VegetationStartCullDistanceCm = 8000;` (HAS initializer; NO spaces around =).
...
# BlueprintReadWrite IS present. Default IS `= 8000`.
# This is a unique coastal director vegetation start-cull int32.
# It is NOT leftover FSkyguardEnvironmentReadiness.TreeInstanceCount /
# ShrubInstanceCount, NOT leftover sibling VegetationEndCullDistanceCm /
# PlacementSeed / EpicTreeBudget / EpicShrubBudget / TreeInstances /
...
# WindStrength / WindSpeed / Quality, NOT leftover Gunner TraceRange = 32000.f.
...
# Stop BEFORE sibling VegetationEndCullDistanceCm as a claimed slot.
...
# wrapping `int32 VegetationStartCullDistanceCm = 8000;` plus ClampMin.
# Also leftover-safe PlacementSeed is EditAnywhere BlueprintReadWrite
# Category Vegetation wrapping int32 PlacementSeed = 5201
# — RETARGET hard Category Vegetation to Performance and decl
# PlacementSeed to VegetationStartCullDistanceCm = 8000.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity
# with NO initializer — RETARGET hard.
# LOCKED_DECL is `int32 VegetationStartCullDistanceCm = 8000;` NOT
...
# VegetationStartCullDistanceCm is 8000 not Harbor 40 / 80 float tokens.
...
TARGET = "int32 VegetationStartCullDistanceCm = 8000;"
TARGET_WRONG_BARE = "int32 VegetationStartCullDistanceCm;"
TARGET_WRONG_INIT = "int32 VegetationStartCullDistanceCm = nullptr;"
TARGET_WRONG_FALSE = "int32 VegetationStartCullDistanceCm = false;"
TARGET_WRONG_TRUE = "int32 VegetationStartCullDistanceCm = true;"
TARGET_WRONG_ZERO = "int32 VegetationStartCullDistanceCm = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> VegetationStartCullDistanceCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> VegetationStartCullDistanceCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> VegetationStartCullDistanceCm;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "VegetationEndCullDistanceCm"
STOP_BEFORE_EPIC_SHRUB = "VegetationEndCullDistanceCm"
...
SIBLING_LOOK_PITCH_MIN = "VegetationEndCullDistanceCm"
...
START_CULL_DECL_RE = re.compile(
    r"int32\s+VegetationStartCullDistanceCm\s*=\s*8000\s*;"
)
START_CULL_BARE_RE = re.compile(
    r"int32\s+VegetationStartCullDistanceCm\s*;"
)
START_CULL_WRONG_INIT_RE = re.compile(
    r"int32\s+VegetationStartCullDistanceCm\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 VegetationStartCullDistanceCm = 8000;`.
    ...
    # TObjectPtr, or a bare VegetationStartCullDistanceCm without
    # the 8000 initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if START_CULL_DECL_RE.search(compact) is None:
        return False
    if START_CULL_BARE_RE.search(compact):
        return False
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_LOOK_PITCH_MIN:
            break
        if ident != "VegetationStartCullDistanceCm":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+VegetationStartCullDistanceCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 VegetationStartCullDistanceCm "
...
class CoastalVegetationStartCullDistanceCmFieldDeclContractTests(unittest.TestCase):
    def test_coastal_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "VegetationStartCullDistanceCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "VegetationStartCullDistanceCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("VegetationStartCullDistanceCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_vegetation_start_cull_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin="1000"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 VegetationStartCullDistanceCm = 8000;", compact_origin)
        self.assertNotIn("VegetationStartCullDistanceCm = 160", compact_origin)
    ...
    def test_vegetation_start_cull_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 VegetationStartCullDistanceCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("8000", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 VegetationStartCullDistanceCm = 8000;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 VegetationStartCullDistanceCms = 8000;\n",
            "\tfloat VegetationStartCullDistanceCm = 8000.f;\n",
            "\tbool VegetationStartCullDistanceCm = true;\n",
            "\tint32 VegetationStartCullDistanceCm = " + forty + ";\n",
            "\tint32 VegetationStartCullDistanceCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_SEED}\n",
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tint32\n\tVegetationStartCullDistanceCm = 8000;\n",
    ...
    def test_does_not_claim_sibling_epic_shrub_budget_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "VegetationStartCullDistanceCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "VegetationStartCullDistanceCm"))
        self.assertTrue(has_identifier(section, "VegetationStartCullDistanceCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="1000"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "coastal vegetation start-cull field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "coastal_vegetation_start_cull_distance_cm_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCoastalEnvironmentDirector.cpp", THIS_SCRIPT)
