# THIS IS leftover-safe FSkyguardProductionAudioRouting RadioSubmix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundSubmixBase> RadioSubmix;`
# (no initializer).
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Bare TSoftObjectPtr<USoundSubmixBase>,
# no initializer.
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# TSoftObjectPtr<USoundSubmixBase>, initializer added, Category= added.
...
# CockpitSoundMix / CockpitExteriorAttenuation /
# CockpitLowPassHz.
# Use identifier bounds so sibling MasterSubmix / CockpitSubmix /
# WeaponsSubmix / ExteriorSubmix / ExplosionsSubmix do not
# false-positive on the Submix substring.
# Analog leftover production-audio-routing-defaults then
# isolated RadioSubmix field decl.
# THIS IS NOT leftover MasterSubmix.
# THIS IS NOT leftover CockpitSubmix.
# THIS IS NOT leftover WeaponsSubmix.
# THIS IS NOT leftover ExplosionsSubmix.
# THIS IS NOT leftover ExteriorSubmix.
# THIS IS NOT leftover ProductionAudioEntry OutputSubmix.
# THIS IS NOT leftover RadioLine Sound.
...
# TSoftObjectPtr<USoundSubmixBase>, no `=`, BlueprintReadWrite,
# no Category.
...
TARGET = "TSoftObjectPtr<USoundSubmixBase> RadioSubmix;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundSubmixBase> RadioSubmix = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundSubmixBase> RadioSubmix = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundSubmixBase> RadioSubmix = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundSubmixBase> RadioSubmix = 0.f;"
TARGET_WRONG_FLOAT = "float RadioSubmix;"
TARGET_WRONG_FNAME = "FName RadioSubmix;"
TARGET_WRONG_BOOL = "bool RadioSubmix;"
TARGET_WRONG_INT = "int32 RadioSubmix;"
TARGET_WRONG_FTEXT = "FText RadioSubmix;"
TARGET_WRONG_PTR = "USoundSubmixBase* RadioSubmix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> RadioSubmix;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundMix> RadioSubmix;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> RadioSubmix;"
TARGET_WRONG_FSTRING = "FString RadioSubmix;"
...
LEFTOVER_RADIO_LINE_SOUND = (
    "Scripts/tests/test_radio_line_sound"
    "_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling MasterSubmix / CockpitSubmix / WeaponsSubmix.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+RadioSubmix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundSubmixBase\s*>\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundMix\s*>\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+RadioSubmix\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+RadioSubmix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundSubmixBase> RadioSubmix is missing from "
...
class ProductionAudioRoutingRadioSubmixFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_radio_submix_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("RadioSubmix = nullptr", compact_origin)
        self.assertNotIn("RadioSubmix = false", compact_origin)
        self.assertNotIn("RadioSubmix = true", compact_origin)
        self.assertNotIn("RadioSubmix = 0.f", compact_origin)
        self.assertNotIn("RadioSubmix = 160.f", compact_origin)

    def test_radio_submix_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("TSoftObjectPtr<USoundSubmixBase>", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<USoundSubmixBase> RadioSubmixes;\n",
            "\tint32 RadioSubmix = 1;\n",
            "\tbool RadioSubmix;\n",
            "\tfloat RadioSubmix = " + forty + ";\n",
            "\tfloat RadioSubmix = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tTSoftObjectPtr<USoundSubmixBase>\n\tRadioSubmix;\n",
            "\tTSoftObjectPtr<USoundSubmixBase>   RadioSubmix;\n",
            "\tTSoftObjectPtr<USoundSubmixBase>\tRadioSubmix;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing RadioSubmix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_ROUTING_WEAPONS_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTING_EXPLOSIONS_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTING_EXTERIOR_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_RADIO_LINE_SOUND, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_AUDIT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertTrue(Path(__file__).name.endswith(
            "radio_submix_field_decl_contract.py"
        ))
