# THIS IS leftover-safe FSkyguardCampaignMissionSpec MissionId.
...
# `= false`, Harbor 40/80). MissionId has NO initializer.
...
# Do NOT contract sibling fields Title / Brief / Success /
...
# Plain-struct FName style comes from leftover-safe
# StormRain MissionId, BUT the parse window is
# FSkyguardCampaignMissionSpec in SkyguardCampaignRoster.h
# (stop before namespace SkyguardCampaignRoster).
# Do NOT claim leftover StormRain MissionId.
# UPROPERTY rejection is copied from leftover-safe
# ContactMark WorldLocation. Do NOT copy that
# ContactMark parse window.
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Title / WeatherIdentity / BeatSeconds fields. Do not
    # invent an initializer. Do not accept UPROPERTY /
    # Category clones from leftover TheaterKit #1300 or
    # leftover MissionResult MissionId.
...
class CampaignMissionSpecMissionIdFieldDeclContractTests(unittest.TestCase):
    ...
    def test_sibling_title_does_not_satisfy_mission_id(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    mission_id_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec MissionId field decl contract "
