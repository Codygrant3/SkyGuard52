...
# or lock the IsPointInsideFlightClearance body in the .cpp.
...
# ASkyguardMissionMapAssemblyDirector::IsPointInsideFlightClearance,
# not RebuildRouteSpline (sibling this wave), not
# ValidateAssembly (sibling this wave), not
...
# Python IsPointInsideFlightClearance declaration
# contract. Stay off RebuildRouteSpline (sibling
# this wave), ValidateAssembly (sibling this wave),
# GetReadiness, and UPROPERTY fields on this class.
# Stay off leftover
# skyline-style enum #e017, leftover objective-anchor
# defaults #692c, leftover landmark-anchor defaults
...
# here. RebuildRouteSpline / ValidateAssembly /
...
# In-flight RebuildRouteSpline / ValidateAssembly
# declaration contracts stay unlocked.
LEFTOVER_MISSION_MAP_SIBLINGS_NOT_LOCKED = (
...
# .cpp IsPointInsideFlightClearance body / invented
...
class MissionMapIsPointInsideFlightClearanceDeclContractTests(unittest.TestCase):
    def test_mission_map_assembly_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, IS_POINT_INSIDE), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_POINT_INSIDE)
        self.assertIn("IsPointInsideFlightClearance", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, IS_POINT_INSIDE))

    def test_missing_is_point_inside_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMissionMapAssemblyDirector();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_POINT_INSIDE)
        self.assertIn("IsPointInsideFlightClearance", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_POINT_INSIDE)
        self.assertIn("IsPointInsideFlightClearance", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, IS_POINT_INSIDE),
            section,
        )
        self.assertNotIn("BlueprintPure", IS_POINT_INSIDE)
        self.assertNotIn("UFUNCTION", IS_POINT_INSIDE)
        self.assertNotIn("Category", IS_POINT_INSIDE)
        self.assertNotIn("BlueprintCallable", IS_POINT_INSIDE)
    ...
    def test_declaration_accepts_nearby_origin_main_ufunction(self) -> None:
        ...
        self.assertTrue(has_declaration(wrapped, IS_POINT_INSIDE), wrapped)
        self.assertIn(UFUNCTION_NEARBY, wrapped)
        self.assertEqual(
            require_declaration(wrapped, IS_POINT_INSIDE),
            IS_POINT_INSIDE,
        )
        self.assertNotIn("UFUNCTION", IS_POINT_INSIDE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_POINT_INSIDE)
        self.assertIn("IsPointInsideFlightClearance", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        missing_arg = (
            "\tbool IsPointInsideFlightClearance() const;\n"
        )
        wrong_return = (
            "\tvoid IsPointInsideFlightClearance("
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_POINT_INSIDE)
            self.assertIn("IsPointInsideFlightClearance", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_point_inside_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, IS_POINT_INSIDE),
            IS_POINT_INSIDE,
        )
        self.assertTrue(has_declaration(section, IS_POINT_INSIDE))
        self.assertEqual(declaration_count(section, IS_POINT_INSIDE), 1)
        self.assertTrue(
            IS_POINT_INSIDE.startswith("bool "),
            IS_POINT_INSIDE,
        )
        self.assertTrue(IS_POINT_INSIDE.endswith(";"), IS_POINT_INSIDE)
        self.assertTrue(
            IS_POINT_INSIDE.endswith(" const;"),
            IS_POINT_INSIDE,
        )
        self.assertIn("const FVector& WorldPoint", IS_POINT_INSIDE)
        self.assertIn(" const", IS_POINT_INSIDE)
        self.assertNotIn("INDEX_NONE", IS_POINT_INSIDE)
        self.assertNotIn("{", IS_POINT_INSIDE)
        self.assertNotIn("}", IS_POINT_INSIDE)
        self.assertNotIn("return ", IS_POINT_INSIDE)
        self.assertNotIn("UFUNCTION", IS_POINT_INSIDE)
        self.assertNotIn("static ", IS_POINT_INSIDE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_arg = (
            "public:\n"
            "\tbool IsPointInsideFlightClearance(const FVector&\n"
            "\t\tWorldPoint) const;\n"
            "};\n"
        )
        wrap_const = (
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, IS_POINT_INSIDE),
                1,
            )
        one_line = f"{{\npublic:\n\t{IS_POINT_INSIDE}\n}}\n"
        self.assertTrue(has_declaration(one_line, IS_POINT_INSIDE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, IS_POINT_INSIDE),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", IS_POINT_INSIDE)
        self.assertNotIn("return ", IS_POINT_INSIDE)
        self.assertNotIn("return false", IS_POINT_INSIDE)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{IS_POINT_INSIDE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", IS_POINT_INSIDE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", IS_POINT_INSIDE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_point_inside_cpp_body(self) -> None:
        locked_only = f"{IS_POINT_INSIDE}\n"
        self.assertNotIn("{", IS_POINT_INSIDE)
        self.assertNotIn("}", IS_POINT_INSIDE)
        self.assertNotIn("return ", IS_POINT_INSIDE)
        ...
        self.assertNotIn("return false", IS_POINT_INSIDE)
        self.assertNotIn("return true", IS_POINT_INSIDE)

    def test_contract_does_not_relock_rebuild_route_spline(self) -> None:
        locked_only = f"{IS_POINT_INSIDE}\n"
        for neighbor in REBUILD_ROUTE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_POINT_INSIDE)
        self.assertNotIn("RebuildRouteSpline", IS_POINT_INSIDE)
        self.assertNotIn("RebuildRouteSpline", locked_only)

    def test_contract_does_not_relock_validate_assembly(
        self,
    ) -> None:
        locked_only = f"{IS_POINT_INSIDE}\n"
        for neighbor in VALIDATE_ASSEMBLY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_POINT_INSIDE)
        self.assertNotIn("ValidateAssembly", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_get_readiness(self) -> None:
        ...
        for neighbor in GET_READINESS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_POINT_INSIDE)
        self.assertNotIn("GetReadiness", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_uprop_fields(self) -> None:
        ...
        for token in UPROP_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("UPROPERTY", IS_POINT_INSIDE)
        self.assertNotIn("UPROPERTY", locked_only)
        self.assertNotIn("SkylineStyle", IS_POINT_INSIDE)
        self.assertNotIn("ObjectiveAnchors", IS_POINT_INSIDE)
        self.assertNotIn("LandmarkAnchors", IS_POINT_INSIDE)
        self.assertNotIn("FlightRouteSpline", IS_POINT_INSIDE)
        self.assertNotIn("MissionDefinition", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_leftover_skyline_style_enum(self) -> None:
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("ESkyguardMissionSkylineStyle", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_leftover_map_defaults(self) -> None:
        ...
        for token in LEFTOVER_MAP_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("FSkyguardMissionObjectiveAnchor", IS_POINT_INSIDE)
        self.assertNotIn("FSkyguardMissionLandmarkAnchor", IS_POINT_INSIDE)
        self.assertNotIn("FSkyguardMissionMapReadiness", IS_POINT_INSIDE)
        self.assertNotIn(
            "test_mission_objective_anchor_defaults_contract.py",
            IS_POINT_INSIDE,
        )
    ...
    def test_contract_does_not_relock_leftover_mission_map_siblings(
        self,
    ) -> None:
        locked_only = f"{IS_POINT_INSIDE}\n"
        for token in LEFTOVER_MISSION_MAP_SIBLINGS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("RebuildRouteSpline", IS_POINT_INSIDE)
        self.assertNotIn("ValidateAssembly", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_leftover_validate_definition(
        ...
        for token in LEFTOVER_VALIDATE_DEFINITION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("ValidateDefinition", IS_POINT_INSIDE)
        self.assertNotIn("ValidateDefinition", locked_only)
        self.assertNotIn("USkyguardMissionDefinition", IS_POINT_INSIDE)
        self.assertNotIn("USkyguardCampaignDefinition", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_leftover_pathfinder_contracts(
        ...
        for token in LEFTOVER_PATHFINDER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("AdvanceEncounter", IS_POINT_INSIDE)
        self.assertNotIn("GetTelegraphsTriggered", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("Unconfigured", IS_POINT_INSIDE)
        self.assertNotIn("Warming", IS_POINT_INSIDE)
        self.assertNotIn("Launched", IS_POINT_INSIDE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            IS_POINT_INSIDE,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_POINT_INSIDE)
        self.assertNotIn("FSkyguardBriefingCard", IS_POINT_INSIDE)
        self.assertNotIn("FSkyguardBriefingRadioRow", IS_POINT_INSIDE)
        self.assertNotIn("FSkyguardBriefingCard", locked_only)
        self.assertNotIn("FSkyguardBriefingRadioRow", locked_only)
        self.assertNotIn("GetMinimumWarmupSeconds", IS_POINT_INSIDE)
        self.assertNotIn("GetBriefingText", IS_POINT_INSIDE)
        self.assertNotIn("GetRadioChatter", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_POINT_INSIDE)
        self.assertNotIn("FillResultCombatStats", IS_POINT_INSIDE)
        self.assertNotIn("ASkyguardGunner", IS_POINT_INSIDE)
        self.assertNotIn("FillAndFinalize", IS_POINT_INSIDE)
        self.assertNotIn("FillAndFail", IS_POINT_INSIDE)
        self.assertNotIn("ApplyHydraForClusters", IS_POINT_INSIDE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_POINT_INSIDE}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_POINT_INSIDE),
            IS_POINT_INSIDE,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("SkyguardMissionDefinition.h", IS_POINT_INSIDE)
        self.assertNotIn("SkyguardCampaignDefinition.h", IS_POINT_INSIDE)
    ...
    def test_harbor_industrial_is_not_a_harbor_clock_token(self) -> None:
        ...
        self.assertNotIn(leftover_style, IS_POINT_INSIDE)
        locked_only = f"{IS_POINT_INSIDE}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map IsPointInsideFlightClearance contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, IS_POINT_INSIDE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"mission-map IsPointInsideFlightClearance contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_POINT_INSIDE)

    def test_contract_is_is_point_inside_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(IS_POINT_INSIDE.startswith("bool "))
        self.assertTrue(IS_POINT_INSIDE.endswith(";"))
        self.assertIn(UFUNCTION_NEARBY, section)
