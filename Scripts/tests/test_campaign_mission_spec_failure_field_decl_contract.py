# THIS IS leftover-safe FSkyguardCampaignMissionSpec Failure.
# origin/main form: plain C++ field `const TCHAR* Failure = TEXT("");`
...
# that leftover parse window. StormRain Title is leftover.
# Do NOT parse leftover CampaignMissionSpec Title leftover-safe
# #1522, leftover MissionId leftover-safe #1521, leftover
# Brief, leftover Success as the locked decl.
# Do NOT parse leftover MissionPresentation FailureDebrief
# leftover-safe #1383 as this slot. That leftover is
# UPROPERTY FText FailureDebrief, not this plain TCHAR*.
...
# Success / Weather / WeatherIdentity / WeatherLabel /
...
# Clone LOCKED_SCRIPTS from leftover-safe #1300, then
# RETARGET via leftover-safe #1521 parse window and
# leftover-safe #1522 TCHAR* TEXT("") locking.
# If a clone asserts UPROPERTY / Category="Skyguard|Theater" /
# VisibleAnywhere / FName WeatherIdentity / `= 160.f` /
# leftover StormRainBeatKit Title / leftover Title /
# leftover FailureDebrief, retarget: type is
# const TCHAR*, identifier is Failure, initializer is
...
LEFTOVER_FAILURE_DEBRIEF = "FailureDebrief"
LEFTOVER_FAILURE_DEBRIEF_DECL = "FText FailureDebrief;"
NAMESPACE_NAME = "SkyguardCampaignRoster"
TARGET = 'const TCHAR* Failure = TEXT("");'
TARGET_WRONG_BARE = "const TCHAR* Failure;"
TARGET_WRONG_EQ_NONE = "const TCHAR* Failure = NAME_None;"
TARGET_WRONG_FALSE = "const TCHAR* Failure = false;"
TARGET_WRONG_TRUE = "const TCHAR* Failure = true;"
TARGET_WRONG_ZERO = "const TCHAR* Failure = 0.f;"
TARGET_WRONG_FLOAT = "float Failure;"
TARGET_WRONG_HEALTH = "float Failure = 160.f;"
TARGET_WRONG_BOOL = "bool Failure;"
TARGET_WRONG_INT = "int32 Failure;"
TARGET_WRONG_FNAME = "FName Failure;"
TARGET_WRONG_TEXT = 'const TCHAR* Failure = TEXT("Storm");'
...
LEFTOVER_FAILURE_DEBRIEF_SCRIPT = (
    "Scripts/tests/test_mission_presentation_failure_debrief"
    "_field_decl_contract.py"
)
LEFTOVER_ANALOG_MISSION_PRESENTATION_DEFAULTS = (
...
FAILURE_DECL_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*Failure\s*=\s*TEXT\s*\(\s*\"\"\s*\)\s*;"
)
FAILURE_INIT_RE = re.compile(
    r"const\s+TCHAR\s*\*\s*Failure\s*=\s*([^;]+);"
)
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `const TCHAR* Failure = TEXT("");`.
    # Do not accept missing TEXT(""), WeatherLabel, Brief,
    # Success, Title, FName Failure, `= NAME_None` /
    # `= false` / `= true` / `= 0.f` / `= 160.f` when
    # origin/main locks TEXT(""). Do not accept leftover
    # StormRainBeatKit Title or leftover CampaignMissionSpec
    # Title as this CampaignMissionSpec Failure parse window.
    # Do not accept leftover MissionPresentation FailureDebrief.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if FAILURE_DECL_RE.search(compact) is None:
        return False
    if re.search(r"float\s+Failure\b", compact):
        return False
    if re.search(r"FName\s+Failure\b", compact):
        return False
    if re.search(
        r"const\s+TCHAR\s*\*\s*WeatherLabel\b",
        compact,
    ) and not has_identifier(compact, "Failure"):
        return False
    for match in FAILURE_INIT_RE.finditer(compact):
...
def leftover_mission_presentation_header() -> str:
    result = subprocess.run(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*const TCHAR\* Failure\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for const TCHAR* Failure is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*const TCHAR\* Failure\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on const TCHAR* Failure is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "Failure"):
        raise AssertionError(
            "UPROPERTY clone landed on Failure; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "Failure"):
            raise AssertionError(
                f"{token} clone landed on Failure; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CampaignMissionSpecFailureFieldDeclContractTests(unittest.TestCase):
    def test_campaign_mission_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Failure"), section)
        ...
        self.assertNotIn(LEFTOVER_FAILURE_DEBRIEF, section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, 'const TCHAR* Failure = TEXT("");')
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("const TCHAR* Failure"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn('TEXT("")', LOCKED_DECL)
        self.assertIn("Failure", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_FAILURE_DEBRIEF_DECL)
    ...
    def test_leftover_storm_rain_title_does_not_satisfy(self) -> None:
        ...
        self.assertIn(TARGET_WRONG_TITLE, leftover)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TITLE)
        self.assertNotIn(LOCKED_DECL, leftover)
    ...
    def test_leftover_mission_presentation_failure_debrief_does_not_satisfy(self) -> None:
        leftover = leftover_mission_presentation_header()
        self.assertIn(LEFTOVER_MISSION_PRESENTATION, leftover)
        self.assertIn(LEFTOVER_FAILURE_DEBRIEF, leftover)
        self.assertIn(LEFTOVER_FAILURE_DEBRIEF_DECL, leftover)
        self.assertNotEqual(LOCKED_DECL, LEFTOVER_FAILURE_DEBRIEF_DECL)
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_FAILURE_DEBRIEF_SCRIPT)
        self.assertNotIn(LEFTOVER_FAILURE_DEBRIEF, LOCKED_DECL)
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(
                f"\t{LEFTOVER_FAILURE_DEBRIEF_DECL}\n",
                LOCKED_DECL,
            )
        self.assertIn("Failure", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_leftover_campaign_mission_spec_title_is_not_this_slot(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TITLE)
    ...
    def test_does_not_claim_leftover_storm_rain_or_neighbors(self) -> None:
        ...
        self.assertIn(TARGET_WRONG_TITLE, storm)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TITLE)
        self.assertNotIn(LOCKED_DECL, storm)
        with self.assertRaises(AssertionError) as raised:
            spec_section(storm)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Failure"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
    ...
    def test_missing_failure_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_sibling_tchar_fields_do_not_satisfy_failure(self) -> None:
        leftovers = (
            TARGET_WRONG_BRIEF,
            TARGET_WRONG_SUCCESS,
            TARGET_WRONG_TITLE,
    ...
    def test_initializer_must_be_empty_text(self) -> None:
        ...
        self.assertIn('const TCHAR* Failure = TEXT("");', compact_origin)
        self.assertNotIn("Failure = NAME_None", compact_origin)
        self.assertNotIn("Failure = false", compact_origin)
        self.assertNotIn("Failure = true", compact_origin)
        self.assertNotIn("Failure = 0.f", compact_origin)
        self.assertNotIn("Failure = 160.f", compact_origin)
        self.assertNotIn('Failure = TEXT("Storm")', compact_origin)

    def test_failure_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("const TCHAR* Failure"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tconst TCHAR* Failures = TEXT(\"\");\n",
            "\tint32 Failure;\n",
            "\tbool Failure;\n",
            "\tfloat Failure = " + forty + ";\n",
            "\tfloat Failure = " + eighty + ";\n",
        )
    ...
    def test_does_not_parse_leftover_storm_rain_or_neighbors(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_FAILURE_DEBRIEF, header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec Failure field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "failure_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_FAILURE_DEBRIEF_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ANALOG_MISSION_PRESENTATION_DEFAULTS, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_SUCCESS, locked_only)
        self.assertNotIn(LEFTOVER_FAILURE_DEBRIEF, locked_only)
