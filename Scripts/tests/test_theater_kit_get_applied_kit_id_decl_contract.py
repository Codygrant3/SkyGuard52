# THIS IS leftover-safe ASkyguardCampaignTheaterKit::GetAppliedKitId.
# origin/main form: split-line
# UFUNCTION(BlueprintPure, Category="Skyguard|Theater")
# then `FName GetAppliedKitId() const { return AppliedSpec.KitId; }`.
...
# GetNamedLandmark / GetBuildingTint / GetAppliedSpec
# stay uncontracted.
...
TARGET = "FName GetAppliedKitId() const;"
TARGET_INLINE = (
    "FName GetAppliedKitId() const { return AppliedSpec.KitId; }"
)
TARGET_WRONG_BARE = "FName GetAppliedKitId();"
TARGET_WRONG_NO_CONST = (
    "FName GetAppliedKitId() { return AppliedSpec.KitId; }"
)
TARGET_WRONG_VOID = "void GetAppliedKitId() const;"
TARGET_WRONG_FLOAT = "float GetAppliedKitId() const;"
TARGET_WRONG_BOOL = "bool GetAppliedKitId() const;"
TARGET_WRONG_RENAME = "FName GetAppliedKit() const;"
...
GET_APPLIED_KIT_ID_SIGNATURE_RE = re.compile(
    r"\bFName\s+GetAppliedKitId\s*\(\s*\)\s*const\s*(?:;|\{)"
)
INVENTED_UFUNCTION = (
    "BlueprintCallable",
    "VisibleAnywhere",
...
def leftover_spec_names_not_in_locked_decl() -> tuple[str, ...]:
    # GetAppliedKitId contains the KitId stem; do not treat that
    # substring as contracting leftover FName KitId.
    return tuple(
        name
        for name in leftover_spec_field_names()
        if name != SIBLING_KIT_ID
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `FName GetAppliedKitId() const`. Accept origin/main
    ...
    # GetAppliedWeatherIdentity / GetNamedLandmark /
    ...
    if GET_APPLIED_KIT_ID_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+GetAppliedKitId\b", compact):
        return False
    if re.search(r"\bfloat\s+GetAppliedKitId\b", compact):
        return False
    if re.search(r"\bbool\s+GetAppliedKitId\b", compact):
        return False
    if re.search(r"\bFName\s+GetAppliedKitId\s*\(\s*[^)]+\s*\)", compact):
        return False
    return True
...
def count_one_declaration(region: str, declaration: str) -> int:
    del declaration
    compact = collapsed(region)
    return len(GET_APPLIED_KIT_ID_SIGNATURE_RE.findall(compact))
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+GetAppliedKitId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for FName GetAppliedKitId is missing from "
...
class TheaterKitGetAppliedKitIdDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetAppliedKitId"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetAppliedKitId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_applied_kit_id_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn(
            "GetAppliedKitId()const { return AppliedSpec.KitId; }",
            compact_origin,
        )
        self.assertNotIn("void GetAppliedKitId", compact_origin)
        self.assertNotIn("float GetAppliedKitId", compact_origin)
        self.assertNotIn("bool GetAppliedKitId", compact_origin)

    def test_get_applied_kit_id_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName GetAppliedKitId()"),
            LOCKED_DECL,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetAppliedKitId decl contract "
    ...
    def test_contract_is_target_ufunction_declaration_only(self) -> None:
        ...
        self.assertIn("GetAppliedKitId", locked_only)
