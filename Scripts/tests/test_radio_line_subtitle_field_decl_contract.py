# THIS IS leftover-safe FSkyguardRadioLine Subtitle.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `FText Subtitle;` (no initializer, no Category=).
...
# if the type is not FText. Fail-closed if Category= is
# added when origin/main has none.
...
# Do NOT contract sibling fields LineId / Speaker / Sound /
...
TARGET = "FText Subtitle;"
TARGET_WRONG_EQ_NONE = "FText Subtitle = FText();"
TARGET_WRONG_FALSE = "FText Subtitle = false;"
TARGET_WRONG_TRUE = "FText Subtitle = true;"
TARGET_WRONG_ZERO = "FText Subtitle = 0.f;"
TARGET_WRONG_FLOAT = "float Subtitle;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "FText Subtitle = FText::GetEmpty();"
...
SIBLING_BUILDING_KIT = SIBLING_SOUND
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FText\s+Subtitle\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FText Subtitle is missing from "
...
class RadioLineSubtitleFieldDeclContractTests(unittest.TestCase):
    def test_radio_line_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Subtitle"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Subtitle"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Subtitle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_subtitle_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FText Subtitle;", compact_origin)
        self.assertNotIn("Subtitle = FText()", compact_origin)
        self.assertNotIn("Subtitle = false", compact_origin)
        self.assertNotIn("Subtitle = true", compact_origin)
        self.assertNotIn("Subtitle = 0.f", compact_origin)
        self.assertNotIn("Subtitle = 160.f", compact_origin)

    def test_subtitle_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText Subtitle"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 Subtitle;\n",
            "\tbool Subtitle;\n",
            "\tfloat Subtitle = " + forty + ";\n",
            "\tfloat Subtitle = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio-line Subtitle field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "subtitle_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
