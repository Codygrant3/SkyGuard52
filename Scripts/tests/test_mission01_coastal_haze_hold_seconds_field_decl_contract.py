...
# origin/main in-header initializer `= 12.f`.
# Nearby origin/main
...
# EditInstanceOnly. Require ClampMin="1.0".
# Locked 12.f is not Harbor 40/80.
# Stay off leftover drafts #56–#64, leftover Harbor
# #6/#8/#9, leftover theater-kit #59, leftover
# isolated-test drafts #107–#603, leftover Apache
...
# CoastalHazeHoldSeconds field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::CoastalHazeHoldSeconds",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01CoastalHazeHoldSecondsFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, COASTAL_HAZE_HOLD_SECONDS_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, COASTAL_HAZE_HOLD_SECONDS_FIELD))

    def test_missing_coastal_haze_hold_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("EditAnywhere", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("EditInstanceOnly", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("BlueprintReadWrite", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("VisibleAnywhere", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("BlueprintReadOnly", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("Category", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("ClampMin", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tfloat CoastalHazeHoldSeconds;\n"
        )
        as_array = (
            "\tTArray<float> CoastalHazeHoldSeconds;\n"
        )
        assigned = (
            "\tfloat CoastalHazeHoldSeconds = 8.f;\n"
        )
        soft = (
            "\tdouble CoastalHazeHoldSeconds = 12.f;\n"
        )
        wrong_type = "\tint32 CoastalHazeHoldSeconds = 12;\n"
        renamed = "\tfloat HazeHoldSeconds = 12.f;\n"
        ...
        scene = "\tTObjectPtr<USceneComponent> CoastalHazeHoldSeconds;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> CoastalHazeHoldSeconds;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> CoastalHazeHoldSeconds;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, COASTAL_HAZE_HOLD_SECONDS_FIELD)
            self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_coastal_haze_hold_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, COASTAL_HAZE_HOLD_SECONDS_FIELD),
            COASTAL_HAZE_HOLD_SECONDS_FIELD,
        )
        self.assertTrue(has_declaration(section, COASTAL_HAZE_HOLD_SECONDS_FIELD))
        self.assertEqual(
            declaration_count(section, COASTAL_HAZE_HOLD_SECONDS_FIELD),
            1,
        )
        self.assertTrue(
            COASTAL_HAZE_HOLD_SECONDS_FIELD.startswith("float "),
            COASTAL_HAZE_HOLD_SECONDS_FIELD,
        )
        self.assertTrue(COASTAL_HAZE_HOLD_SECONDS_FIELD.endswith(";"), COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("float", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("TObjectPtr<", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("= 12.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("OceanMaterial", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("BeachMaterial", COASTAL_HAZE_HOLD_SECONDS_FIELD)

        self.assertNotIn("LandMaterial", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CoastalHazeDelaySeconds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CoastalHazeFadeSeconds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("bEnableCoastalHazeTransition", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CoastalHazeDensityIncrease", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("TArray<", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("TSoftObjectPtr", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("nullptr", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("INDEX_NONE", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("UFUNCTION", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("{", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("}", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("return ", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("OceanTiles", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("LandTiles", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("RouteExclusion", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("bRouteExclusionValid", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("BeachTiles", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("InlandVegetationPCG", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("LandScatterBounds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("ProductionLandscape", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("AuthoredPCGGraph", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("DistrictLengthCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("BeachWidthCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("InlandExtentCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("SeawardExtentCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            COASTAL_HAZE_HOLD_SECONDS_FIELD,
        )
        ...
        self.assertNotIn("Root", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetReadiness", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("Briefing", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("AudioDirector", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("RadioChatter", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("SortiePresentation", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CampaignDefinition", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("MissionDefinition", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("bAutoInitialize", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetAircraft", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("ConfigureFromMission", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("AdvanceBriefing", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("SetAssetsReady", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CanLaunch", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetElapsedSeconds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetBriefingState", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetBriefingText", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetRadioChatter", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetPresentation", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("GetMissionTitle", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("AcknowledgeBriefing", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("LaunchSortie", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("HullCollider", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("OpticalTracker", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("WeaponServo", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CountermeasurePod", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("RadarNode", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("HarborIndustrial", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("MaxIntegrity", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CurrentIntegrity", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_VISIBILITY} {COASTAL_HAZE_HOLD_SECONDS_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{COASTAL_HAZE_HOLD_SECONDS_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, COASTAL_HAZE_HOLD_SECONDS_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, COASTAL_HAZE_HOLD_SECONDS_FIELD),
            section,
    ...
    def test_assigned_coastal_haze_hold_seconds_does_not_satisfy(self) -> None:
        assigned = (
            "\tfloat CoastalHazeHoldSeconds = 8.f;\n"
        )
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, COASTAL_HAZE_HOLD_SECONDS_FIELD))
        with self.assertRaises(AssertionError) as raised_ocean:
            require_declaration(leftover_ocean, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised_ocean.exception))
        self.assertFalse(has_declaration(leftover_ocean, COASTAL_HAZE_HOLD_SECONDS_FIELD))
        with self.assertRaises(AssertionError) as raised_beach:
            require_declaration(leftover_beach, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised_beach.exception))
        self.assertFalse(has_declaration(leftover_beach, COASTAL_HAZE_HOLD_SECONDS_FIELD))
        with self.assertRaises(AssertionError) as raised_enable:
            require_declaration(leftover_enable, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised_enable.exception))
        self.assertFalse(has_declaration(leftover_enable, COASTAL_HAZE_HOLD_SECONDS_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, COASTAL_HAZE_HOLD_SECONDS_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{COASTAL_HAZE_HOLD_SECONDS_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            COASTAL_HAZE_HOLD_SECONDS_FIELD.startswith("UFUNCTION"),
            COASTAL_HAZE_HOLD_SECONDS_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_coastal_haze_hold_seconds_cpp_body(self) -> None:
        locked_only = f"{COASTAL_HAZE_HOLD_SECONDS_FIELD}\n"
        self.assertNotIn("{", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("}", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("return ", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        ...
        self.assertNotIn("return false", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("return true", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CreateDefaultSubobject", COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertTrue(
            COASTAL_HAZE_HOLD_SECONDS_FIELD.startswith("float ")
        )
    ...
    def test_contract_does_not_relock_sibling_environment_scripts(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_mission01_coastal_haze_density_increase"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("CurrentIntegrity", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, "12.f")
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{COASTAL_HAZE_HOLD_SECONDS_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, COASTAL_HAZE_HOLD_SECONDS_FIELD),
            COASTAL_HAZE_HOLD_SECONDS_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, COASTAL_HAZE_HOLD_SECONDS_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            self.assertNotEqual(token, "12.f")
            self.assertNotEqual(token, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual(LEFTOVER_ROUTE_LENGTH_VALUE, token)
            self.assertNotEqual(token, "12.f")
            self.assertNotIn(token, leftover_harbor_tokens())
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 CoastalHazeHoldSeconds field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{COASTAL_HAZE_HOLD_SECONDS_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, COASTAL_HAZE_HOLD_SECONDS_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                COASTAL_HAZE_HOLD_SECONDS_FIELD.lower(),
                "mission01 CoastalHazeHoldSeconds contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, COASTAL_HAZE_HOLD_SECONDS_FIELD)

    def test_contract_is_coastal_haze_hold_seconds_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("float", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertTrue(COASTAL_HAZE_HOLD_SECONDS_FIELD.endswith(";"))
        self.assertIn("= 12.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("TObjectPtr<", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("OceanMaterial", COASTAL_HAZE_HOLD_SECONDS_FIELD)
    ...
    def test_sibling_environment_fields_do_not_satisfy_coastal_haze_hold_seconds(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, COASTAL_HAZE_HOLD_SECONDS_FIELD))
            self.assertNotEqual(COASTAL_HAZE_HOLD_SECONDS_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_coastal_haze_hold_seconds(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            ...
            "\t * This comment is not the CoastalHazeHoldSeconds declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertIn("CoastalHazeHoldSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        )
        self.assertNotIn(
            "float CoastalHazeHoldSeconds = 12.f;",
            comment_only,
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            ...
            self.assertNotIn(token, COASTAL_HAZE_HOLD_SECONDS_FIELD)
            self.assertNotEqual(token, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("RouteLengthCm", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("7500.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("2800.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("5200.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("1800.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("18000.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
        self.assertNotIn("20000.f", COASTAL_HAZE_HOLD_SECONDS_FIELD)
