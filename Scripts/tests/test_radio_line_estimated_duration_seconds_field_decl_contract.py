# THIS IS leftover-safe FSkyguardRadioLine EstimatedDurationSeconds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite, meta=(ClampMin="0.1"))
# then `float EstimatedDurationSeconds = 2.f;` (initializer IS `2.f`,
# not `2.0f` / `2.0` / `2` / Harbor 40/80 / `0.f`).
# THIS IS leftover-safe isolated UPROPERTY without Category.
# EditAnywhere IS present. BlueprintReadWrite IS present.
# ClampMin="0.1" IS present. Initializer IS `= 2.f`.
# There is NO Category=.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if EditAnywhere is dropped, if specifiers
# drop to BlueprintReadOnly / VisibleAnywhere-only, if
# the type is not float, if meta=(ClampMin="0.1") is
# missing or changed, if Category= is added, or if the
# initializer is missing or not `= 2.f`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardRadioLine` body ONLY after
# `struct FSkyguardRadioLine`. Stop at
# `DECLARE_DYNAMIC_MULTICAST_DELEGATE` or
# `class USkyguardRadioChatterComponent`.
# Do NOT parse `USkyguardRadioChatterComponent`.
# Do NOT parse leftover radio-chatter-empty-fail-closed
# #177 / #52ae.
# Do NOT parse leftover radio-chatter-empty-queue-fail-closed
# #150 / #5137.
# Do NOT parse leftover radio-chatter-empty-line-tests
# #121 / #fb2b.
# Do NOT parse leftover radio-line-defaults #172 / #1dec.
# Do NOT parse leftover radio-line Subtitle #1332.
# Do NOT contract sibling fields LineId / Speaker / Subtitle /
# Sound / Priority / CooldownSeconds.
# Do NOT contract USkyguardRadioChatterComponent UFUNCTIONs
# or UPROPERTY fields.
# THIS IS NOT leftover briefing-radio-row / HowToFly /
# mission RadioChatter drafts.
# THIS IS NOT leftover briefing-card-defaults then isolated
# CardId. Analog: leftover radio-line-defaults #172 then
# this isolated EstimatedDurationSeconds decl.
# If a clone asserts FText / FName / no `=` /
# Category="Skyguard|Theater" / VisibleAnywhere /
# BlueprintReadOnly / ClampMin missing, retarget: type is
# float, initializer is `= 2.f`, EditAnywhere +
# BlueprintReadWrite + meta=(ClampMin="0.1"), no Category=.
...
STRUCT_NAME = "FSkyguardRadioLine"
...
TARGET = "float EstimatedDurationSeconds = 2.f;"
TARGET_WRONG_EQ_NONE = "float EstimatedDurationSeconds = 2.0f;"
TARGET_WRONG_FALSE = "float EstimatedDurationSeconds = false;"
TARGET_WRONG_TRUE = "float EstimatedDurationSeconds = true;"
TARGET_WRONG_ZERO = "float EstimatedDurationSeconds = 0.f;"
TARGET_WRONG_FLOAT = "int32 EstimatedDurationSeconds = 2;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "float EstimatedDurationSeconds;"
TARGET_WRONG_TWO = "float EstimatedDurationSeconds = 2;"
TARGET_WRONG_TWO_POINT_ZERO = (
    "float EstimatedDurationSeconds = 2.0;"
)
...
UPROPERTY_VISIBLE_READONLY = UPROPERTY_EDIT_WRITE
STOP_BEFORE_DELEGATE = "DECLARE_DYNAMIC_MULTICAST_DELEGATE"
STOP_BEFORE_COMPONENT = "class USkyguardRadioChatterComponent"
STOP_BEFORE_NAMESPACE = STOP_BEFORE_DELEGATE
STOP_BEFORE_ACTOR = STOP_BEFORE_COMPONENT
...
SIBLING_LINE_ID = "LineId"
SIBLING_SPEAKER = "Speaker"
SIBLING_SUBTITLE = "Subtitle"
SIBLING_SOUND = "Sound"
SIBLING_PRIORITY = "Priority"
SIBLING_COOLDOWN = "CooldownSeconds"
SIBLING_KIT_ID = SIBLING_LINE_ID
SIBLING_LANDMARK_SET = SIBLING_SPEAKER
SIBLING_BUILDING_KIT = SIBLING_SUBTITLE
SIBLING_BUILDING_TINT = SIBLING_SOUND
SIBLING_LAMP_TREATMENT = SIBLING_PRIORITY
SIBLING_LAMP_COLOR = SIBLING_COOLDOWN
SIBLING_LAMP_INTENSITY = "MaxQueuedLines"
SIBLING_ROAD_TREATMENT = "InterLineGapSeconds"
SIBLING_ROAD_TINT = "OnLineStarted"
SIBLING_NAMED_LANDMARK = "OnLineFinished"
SIBLING_LANDMARK_TINT = "PrimeLines"
SIBLING_SILHOUETTE_KIT = "EnqueueLine"
SIBLING_SILHOUETTE_TINT = "AdvanceRadioState"
SIBLING_LANDMARK_MESH_INDEX = "ClearQueue"
SIBLING_LANDMARK_SCALE = "GetQueuedLineCount"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float EstimatedDurationSeconds = 2.f`.
    # Reject missing initializer, `2.0f` / `2.0` / `2` /
    # Harbor 40/80 / `0.f` / leftover Health. Do not accept
    # sibling LineId / Speaker / Subtitle / Sound / Priority /
    # CooldownSeconds fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+EstimatedDurationSeconds\s*=\s*2\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+EstimatedDurationSeconds\s*=\s*2\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+EstimatedDurationSeconds\s*=\s*2\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+EstimatedDurationSeconds\s*=\s*2\s*;",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+EstimatedDurationSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+EstimatedDurationSeconds\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+EstimatedDurationSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float EstimatedDurationSeconds is missing from "
...
class RadioLineEstimatedDurationSecondsFieldDeclContractTests(unittest.TestCase):
    def test_radio_line_struct_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "EstimatedDurationSeconds"),
            section,
        )
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_DELEGATE, header)
        self.assertNotIn(STOP_BEFORE_DELEGATE, section)
        self.assertNotIn(STOP_BEFORE_DELEGATE, body)
        self.assertIn(COMPONENT_NAME, header)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("EstimatedDurationSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_estimated_duration_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin="0.1"', specifiers)
    ...
    def test_wrong_or_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "EstimatedDurationSeconds = 0.f",
            compact_origin,
        )
        self.assertNotIn(
            "EstimatedDurationSeconds = 160.f",
            compact_origin,
        )
        self.assertNotIn(
            "EstimatedDurationSeconds = false",
            compact_origin,
        )

    def test_estimated_duration_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float EstimatedDurationSeconds"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            "\tfloat EstimatedDurationSecond;\n",
            "\tint32 EstimatedDurationSeconds = 2;\n",
            "\tbool EstimatedDurationSeconds;\n",
            "\tFText EstimatedDurationSeconds;\n",
            f"\t{TARGET_WRONG_TWO}\n",
            f"\t{TARGET_WRONG_TWO_POINT_ZERO}\n",
            f"\t{TARGET_WRONG_EMPTY}\n",
            "\tfloat EstimatedDurationSeconds = " + forty + ";\n",
            "\tfloat EstimatedDurationSeconds = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_only_fails_closed(self) -> None:
        ...
        self.assertIn('ClampMin="0.1"', origin)
        no_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tUPROPERTY(EditAnywhere, BlueprintReadWrite)\n"
        ...
        no_clamp_specs = attached_uproperty_specifiers(
            spec_section(no_clamp)
        )
        self.assertNotIn("ClampMin", no_clamp_specs)
        self.assertIn("ClampMin", origin)
        cooldown_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        self.assertNotIn('ClampMin="0.1"', cooldown_specs)
        self.assertIn('ClampMin="0.1"', origin)
        priority_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_does_not_contract_sibling_radio_line_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="0.1"', UPROPERTY_EDIT_WRITE)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_WRITE)
        self.assertNotIn("BlueprintReadOnly", UPROPERTY_EDIT_WRITE)
        self.assertNotIn("Category", UPROPERTY_EDIT_WRITE)
        self.assertNotIn('Category="Skyguard|Theater"', UPROPERTY_EDIT_WRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio-line EstimatedDurationSeconds field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "estimated_duration_seconds_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
