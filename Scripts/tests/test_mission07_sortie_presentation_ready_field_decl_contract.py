...
# Mission07 IntegrationReadiness bSortiePresentationReady
...
# bSortiePresentationReady FIELD (declaration-only). It is
# NOT leftover Mission06 bSortiePresentationReady
# FIELD #1005 / branch cursor/mission06-
# sortie-presentation-ready-field-decl-contract-c332,
# NOT leftover Mission07 director SortiePresentation
# FIELD #1014 / branch cursor/mission07-
...
# bAudioReady / bCampaignRuntimeStarted
# siblings spawned this wave,
# NOT leftover Mission02 bSortiePresentationReady
# FIELD #868 / branch cursor/mission02-
# sortie-presentation-ready-field-decl-contract-c332,
...
# sortie-presentation-ready-field-decl-contract-c332,
# NOT leftover Mission05 bSortiePresentationReady
# FIELD #974 / branch cursor/mission05-
# sortie-presentation-ready-field-decl-contract-c332,
...
# (in-flight),
# not leftover FSkyguardSearchTrackRuntime,
# not leftover FSkyguardMission07ProtectedTargetRuntime,
...
# drafts #674-#1037 including leftover Mission02
...
# fields. Do not parse
# FSkyguardSearchTrackRuntime. Do not parse
# FSkyguardMission07ProtectedTargetRuntime. Do not parse FSkyguardAirfieldTargetRuntime.
...
# drafts #674-#1037. Do not reopen leftover
...
# Mission05 bSortiePresentationReady #974.
...
class Mission07SortiePresentationReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        # This Mission07 field default false is not Harbor 40/80.
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 bSortiePresentationReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission07 bSortiePresentationReady contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_sortie_presentation"
                "_ready_field_decl_contract.py"
            )
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission07_search_runtime_ready"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        for token in LEFTOVER_SAME_NAME_SORTIE_PRESENTATION_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION06_SORTIE_PRESENTATION_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_WAVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_SEARCH_RUNTIME_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_PROTECTED_TARGETS_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_GUNNER_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_RADAR_GHOST_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_OBJECTIVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_MAP_ASSEMBLY_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_MISSION_DEFINITION_VALID_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_CAMPAIGN_DEFINITION_VALID_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION05_SORTIE_PRESENTATION_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
    ...
    def test_leftover_same_name_sortie_presentation_ready_contracts_stay_locked(
        ...
        self.assertNotIn(
            "Scripts/tests/test_mission07_sortie_presentation_ready"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission07_briefing_ready"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
