# THIS IS leftover-safe ASkyguardApacheAircraft PylonRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> PylonRight;`
# (no initializer).
...
# Stop BEFORE sibling HydraLeft as a claimed slot.
# Do NOT claim sibling HydraLeft / HydraRight /
# HellfireLeft / HellfireRight / PylonLeft / StubWingRight /
# StubWingLeft / GearLeft / GearRight / GearNose / Canopy /
# Fuselage / AircraftRoot / SilhouetteMesh.
# THIS IS leftover-safe ASkyguardApacheAircraft.PylonRight.
# It is NOT leftover StubWingRight, NOT leftover PylonLeft,
# NOT leftover StubWingLeft, NOT leftover analog
...
# leftover analog apache-gear-left-field,
# leftover analog apache-gear-right-field,
# leftover analog apache-pylon-left-field,
# leftover analog apache-stub-wing-right-field,
# leftover analog apache-stub-wing-left-field,
# leftover analog apache-hover-bob-centimeters-field,
# leftover analog apache-rotor-power-field.
# HydraLeft / HydraRight / HellfireLeft / HellfireRight /
# PylonLeft / StubWingRight / StubWingLeft / GearLeft /
# GearRight / GearNose are sibling slots being opened,
# not this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> PylonRight;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> PylonRight;` and NOT
# leftover `StubWingRight` / leftover `PylonLeft` /
# leftover `StubWingLeft` / leftover `HydraLeft` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> PylonRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> PylonRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> PylonRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> PylonRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> PylonRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> PylonRight;"
...
STOP_BEFORE_HYDRA_LEFT = "HydraLeft"
...
LEFTOVER_APACHE_STUB_WING_RIGHT = (
    "Scripts/tests/test_apache_stub_wing_right"
    "_field_decl_contract.py"
)
...
PYLON_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+PylonRight\s*;"
)
PYLON_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+PylonRight\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover PylonLeft / StubWingRight / StubWingLeft /
    # leftover HydraLeft / GearLeft,
    # leftover GearNose / Fuselage / AircraftRoot / SilhouetteMesh,
    # leftover TObjectPtr<USceneComponent>, or an initializer on
    # PylonRight as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if PYLON_RIGHT_DECL_RE.search(compact) is None:
        return False
    if PYLON_RIGHT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_HYDRA_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_HYDRA_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_HYDRA_LEFT}"
        )
    for later in (
        SIBLING_HYDRA_RIGHT,
        SIBLING_HELLFIRE_LEFT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+PylonRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> PylonRight "
...
class ApachePylonRightFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PylonRight"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_PYLON_LEFT), body)
        self.assertTrue(
            has_identifier(section, SIBLING_PYLON_LEFT),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_HYDRA_LEFT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_HYDRA_LEFT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "PylonRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("PylonRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_pylon_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> PylonRight;", compact_origin)
        self.assertNotIn("PylonRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_pylon_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> PylonRight"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> PylonRight;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STUB_WING_LEFT)
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> PylonRight;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> PylonRights;\n",
            "\tint32 PylonRight = 140;\n",
            "\tbool PylonRight = true;\n",
            "\tfloat PylonRight = " + forty + ";\n",
            "\tfloat PylonRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_hydra_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_HYDRA_LEFT))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_GEAR_RIGHT))
        self.assertTrue(has_identifier(leaked, SIBLING_GEAR_NOSE))
        self.assertTrue(has_identifier(section, SIBLING_GEAR_NOSE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache PylonRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pylon_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_APACHE_STUB_WING_RIGHT, LOCKED_SCRIPTS)
