# THIS IS leftover-safe FSkyguardEnemyWaveDefinition Formations.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `TArray<FSkyguardEnemyFormationDefinition> Formations;`
# (no initializer).
...
# Do NOT parse the body of `FSkyguardEnemyFormationDefinition`
# (leftover enemy-formation-defaults #183 / #2f1a) except as
# the TArray element type on this field.
...
# StartTimeSeconds / CompletionObjectiveId.
...
# WorldLocation #1340, or TargetAirspeedKph draft.
...
# isolated Formations field decl, same as leftover
...
TARGET = "TArray<FSkyguardEnemyFormationDefinition> Formations;"
TARGET_WRONG_EQ_NONE = (
    "TArray<FSkyguardEnemyFormationDefinition> Formations = {};"
)
TARGET_WRONG_FALSE = (
    "TArray<FSkyguardEnemyFormationDefinition> Formations = false;"
)
TARGET_WRONG_TRUE = (
    "TArray<FSkyguardEnemyFormationDefinition> Formations = true;"
)
TARGET_WRONG_ZERO = (
    "TArray<FSkyguardEnemyFormationDefinition> Formations = 0.f;"
)
TARGET_WRONG_FLOAT = "float Formations;"
TARGET_WRONG_FNAME = "FName Formations;"
TARGET_WRONG_TARRAY_FNAME = "TArray<FName> Formations;"
...
def leftover_formation_body_tokens() -> tuple[str, ...]:
    return (
        FORMATION_BODY_ID,
        FORMATION_BODY_UNIT_COUNT,
        FORMATION_BODY_SPACING,
        FORMATION_BODY_TYPE,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # WaveId / StartTimeSeconds / CompletionObjectiveId
    # fields. Type must be TArray of
    # FSkyguardEnemyFormationDefinition. Harbor 40/80
    # fail-closed on the accepted decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FSkyguardEnemyFormationDefinition\s*>"
        r"\s+Formations\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:FName|float|bool|int32)\s+Formations\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+Formations\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FSkyguardEnemyFormationDefinition"
            r"\s*>\s+Formations\b",
            compact[index:],
        ):
    ...
    raise AssertionError(
        "UPROPERTY for TArray<FSkyguardEnemyFormationDefinition> "
        "Formations is missing from "
...
def require_formations_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} Formations UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} Formations"
        )
...
class EnemyWaveFormationsFieldDeclContractTests(unittest.TestCase):
    def test_enemy_wave_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Formations"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Formations"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_DEFINITION, section)
        ...
        self.assertIn("Formations", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_formations_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_formations_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("Formations = NAME_None", compact_origin)
        self.assertNotIn("Formations = false", compact_origin)
        self.assertNotIn("Formations = true", compact_origin)
        self.assertNotIn("Formations = 0.f", compact_origin)
        self.assertNotIn("Formations = 160.f", compact_origin)
        self.assertNotIn("Formations = {}", compact_origin)

    def test_formations_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("FSkyguardEnemyFormationDefinition", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTArray<FSkyguardEnemyFormationDefinition> Formation;\n",
            "\tint32 Formations;\n",
            "\tbool Formations;\n",
            "\tfloat Formations = " + forty + ";\n",
            "\tfloat Formations = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_formations_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formations_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formations_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formations_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_formations_specifiers(spec_section(with_category))
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for leftover in leftover_formation_body_tokens():
            ...
            self.assertNotIn(leftover, LOCKED_DECL)
        self.assertIn(ENEMY_FORMATION_NAME, section)
        self.assertIn("TArray<" + ENEMY_FORMATION_NAME + ">", collapsed(section))
        self.assertNotIn("struct " + ENEMY_FORMATION_NAME, section)
        self.assertNotIn("struct " + ENEMY_FORMATION_NAME, leaked)
        for leftover in leftover_pictogram_values():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, leaked)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "enemy-wave Formations field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "enemy_wave_formations_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
