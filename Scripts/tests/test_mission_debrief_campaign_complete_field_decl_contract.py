# THIS IS leftover-safe FSkyguardMissionDebrief::bCampaignComplete.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `bool bCampaignComplete = false;` with no Category=.
...
# THIS IS NOT leftover FSkyguardMissionDebrief::bNextMissionUnlocked
# (sibling in-flight).
# THIS IS NOT leftover FSkyguardMissionDebrief::NextMissionDisplayName
# (different field).
# Do NOT contract other Debrief fields
# (State, Result, MissionDisplayName, Narrative,
...
# FSkyguardMissionDebrief, stop at that struct's closing };
# Do NOT parse FSkyguardMissionResult body,
# FSkyguardObjectiveProgress, enum class
# ESkyguardMissionDebriefState body except as nested type usage,
# landscape/authoring structs, ASkyguardPropSpinner,
# ESkyguardBriefingPictogram,
# USkyguardSortiePresentationComponent methods,
# briefing/how-to-fly rows, or leftover AddObjectiveProgress.
...
TARGET = "bool bCampaignComplete = false;"
TARGET_WRONG_BARE = "bool bCampaignComplete;"
TARGET_WRONG_TRUE = "bool bCampaignComplete = true;"
TARGET_WRONG_ZERO = "bool bCampaignComplete = 0;"
TARGET_WRONG_FLOAT = "bool bCampaignComplete = 0.f;"
TARGET_WRONG_UNLOCKED = "bool bNextMissionUnlocked = false;"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
STOP_BEFORE_ENUM = "enum class ESkyguardMissionDebriefState"
NEIGHBOR_RESULT_STRUCT = "struct FSkyguardMissionResult"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # origin/main has `bool bCampaignComplete = false;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"bool\s+bCampaignComplete\s*=\s*true\b", compact):
        return False
    if re.search(r"bool\s+bCampaignComplete\s*=\s*0(?:\.f)?\b", compact):
        return False
    if re.search(r"bool\s+bCampaignComplete\s*=\s*false\b", compact) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bCampaignComplete\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bCampaignComplete is missing from "
...
class MissionDebriefCampaignCompleteFieldDeclContractTests(unittest.TestCase):
    def test_mission_debrief_struct_exists(self) -> None:
        ...
        self.assertNotIn(NEIGHBOR_RESULT_STRUCT, section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bCampaignComplete", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_campaign_complete_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bCampaignComplete = false", collapsed(origin))
        self.assertNotIn("bCampaignComplete = true", collapsed(origin))
        self.assertNotIn("bCampaignComplete = 0", collapsed(origin))

    def test_campaign_complete_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bCampaignComplete"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            f"\t{TARGET_WRONG_UNLOCKED}\n",
            "\tint32 bCampaignComplete = false;\n",
            "\tbool bCampaignCompletes = false;\n",
            "\tbool CampaignComplete = false;\n",
            "\tFText bCampaignComplete = false;\n",
    ...
    def test_does_not_parse_neighbors_or_director_methods(self) -> None:
        ...
        self.assertIn(NEIGHBOR_RESULT_STRUCT, header)
        ...
        self.assertNotIn(NEIGHBOR_RESULT_STRUCT, leaked)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief bCampaignComplete field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_campaign_complete"
            "_field_decl_contract.py"
        ))
