# THIS IS leftover-safe USkyguardPauseHostComponent
# IsPauseMenuVisible.
# origin/main form: one-line BlueprintPure wrap
# UFUNCTION(BlueprintPure, Category="Skyguard|Pause")
# then BARE `bool IsPauseMenuVisible() const;`
# (declaration-only, no inline body required).
# THIS IS leftover-safe unique analog Skyguard|Pause UFUNCTION.
# Category IS `Skyguard|Pause`. BlueprintPure IS present.
# BlueprintCallable IS absent on THIS wrap.
# KEEP BlueprintPure. Do NOT switch to BlueprintCallable.
# const IS present. Return type IS bool.
# No args. Declaration-only.
# Word-boundary match IsPauseMenuVisible vs TogglePause.
# Identifier IsPauseMenuVisible is NOT leftover analog TogglePause
# #1711 SHA 8eb7b06cfe209a463f8195a4a5fc90d421d8a8dc and NOT leftover analog
# pause-host-fail-closed bulk #123 (behavioral; wrap Category
# Pause UFUNCTION declaration only).
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# drop to BlueprintCallable, if const is missing, if args are
# present, if the return type is not bool, or if Category is
...
# Parse CLASS `USkyguardPauseHostComponent` public
# section ONLY after `class USkyguardPauseHostComponent`
# / GENERATED_BODY. Start at `public:`. Stop BEFORE `private:`.
# Stop BEFORE first public UPROPERTY (`PauseWidgetClass`).
# Isolate THIS UFUNCTION window so sibling TogglePause /
# BindPauseInput are uncontracted.
# Do NOT claim sibling BlueprintCallable TogglePause.
# Do NOT claim sibling BindPauseInput (no UFUNCTION).
# Do NOT retarget to TogglePause / BindPauseInput /
# PauseWidgetClass / PauseWidgetZOrder.
# Do NOT claim leftover unique Validation BlueprintCallable
# PrepareGovernedLandscapeForVisibleValidation #1694.
# Do NOT claim leftover unique BlueprintCallable
# EnsureDefaultEntries #1680 Category Production.
# Do NOT claim leftover unique BlueprintPure GetSortieHits
# #1685 Category Combat|Sortie.
# Do NOT claim leftover unique Validation BlueprintPure
# GetActiveRHIAndFeatureLevel #1695.
# Clone leftover unique Validation BlueprintPure
# GetActiveRHIAndFeatureLevel #1695 RETARGET method/return.
# KEEP BlueprintPure.
# Clone leftover unique BlueprintPure GetSortieHits #1685
# Category Skyguard|Combat|Sortie — RETARGET Category
# Combat|Sortie -> Skyguard|Pause. KEEP BlueprintPure.
# FAIL-CLOSED if LOCKED_DECL is int32 GetSortieHits() const;
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# #1680 RETARGET Category Production -> Skyguard|Pause.
...
# method/specifiers; KEEP BlueprintPure not BlueprintCallable.
# FAIL-CLOSED if LOCKED_DECL is void TogglePause();
# Clone leftover unique GetBoundPresentation #1710 RETARGET
# Category HUD -> Pause. KEEP BlueprintPure KEEP const.
# Clone wrap leftover unique BlueprintCallable #1680 was
# Category="Skyguard|Audio|Production" wrapping
# `void EnsureDefaultEntries();` — RETARGET Category
# Production -> Skyguard|Pause. KEEP BlueprintPure
# not BlueprintCallable.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity — RETARGET hard.
# LOCKED_DECL is `bool IsPauseMenuVisible() const;`
# NOT `FName WeatherIdentity;` and NOT `160.f` and NOT
# `FireRate = 12.0f` and NOT `RecoilPitch = 0.92f` and NOT
...
# `void TogglePause();` and NOT
# `void BindPresentation(` and NOT
# `void RefreshFromPresentationState();` and NOT
# `void RebindIfNeeded();` and NOT
# `int32 GetActivationCount() const;` and NOT leftover analog
# `int32 PoolCapacity = 12` and NOT
# `void PrimeLines(` and NOT
# `bool EnqueueLine(` and NOT leftover
# IngressSwayWavelength wavelength leak.
...
HEADER_PATH = "Source/Skyguard52/SkyguardPauseHostComponent.h"
CLASS_NAME = "USkyguardPauseHostComponent"
LEFTOVER_AUDIO_DIRECTOR_CLASS = "USkyguardAudioDirectorComponent"
TARGET = "bool IsPauseMenuVisible() const;"
TARGET_WRONG_CLONE_ENSURE = "void EnsureDefaultEntries();"
TARGET_INLINE = (
    "bool IsPauseMenuVisible() const { }"
)
TARGET_WRONG_BARE = "bool IsPauseMenuVisible();"
TARGET_WRONG_NO_CONST = (
    "void IsPauseMenuVisible() const;"
)
TARGET_WRONG_VOID = "bool AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_FLOAT = "float IsPauseMenuVisible() const;"
TARGET_WRONG_INT = "int32 IsPauseMenuVisible() const;"
...
TARGET_WRONG_PRODUCTION_BANK_FIELD = (
    "TObjectPtr<USkyguardPauseHostComponent> ProductionBank;"
)
...
TARGET_WRONG_TRUE = "bool IsPauseMenuVisible = true;"
TARGET_WRONG_ZERO = "bool IsPauseMenuVisible = 0.f;"
...
TARGET_WRONG_REFRESH_PRESENTATION = "void RefreshFromPresentationState();"
TARGET_WRONG_TOGGLE_PAUSE = "void TogglePause();"
TARGET_WRONG_ACTIVATION_COUNT = "int32 GetActivationCount() const;"
TARGET_WRONG_WAVELENGTH = "float IngressSwayWavelength = 1800.f;"
TARGET_WRONG_REBIND = "void RebindIfNeeded();"
TARGET_WRONG_POOL_CAPACITY = "int32 PoolCapacity = 12"
TARGET_WRONG_PRIME_LINES = "void PrimeLines("
TARGET_WRONG_ENQUEUE_LINE = "bool EnqueueLine("
SIBLING_TOGGLE_PAUSE = "TogglePause"
SIBLING_BIND_PAUSE_INPUT = "BindPauseInput"
SIBLING_PAUSE_WIDGET_CLASS = "PauseWidgetClass"
SIBLING_PAUSE_WIDGET_ZORDER = "PauseWidgetZOrder"
SIBLING_BEGIN_PLAY = "BeginPlay"
LOCKED_DECL = TARGET
...
STOP_BEFORE_EVENT_DEFINITIONS = "PauseWidgetClass"
SIBLING_INITIALIZE_REQUIRED = "BindPauseInput"
...
SIBLING_ENTRIES = "PauseWidgetClass"
SIBLING_ROUTING = "PauseWidgetZOrder"
...
STOP_BEFORE_SKYLINE_STYLE = "ActiveWidget"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pause_host_is_pause_menu_visible"
    "_decl_contract.py"
)
...
LOCKED = {
    ...
    "SkyguardPauseHostComponentTests.cpp",
    "SkyguardPauseSettingsWidget.h",
    "SkyguardPauseSettingsWidget.cpp",
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/"
    return (
        f"{prefix}test_pause_host_fail_closed.py",
        f"{prefix}test_pause_host_fail_closed_tests.py",
        f"{prefix}test_pause_host_fail_closed_contract.py",
        f"{prefix}test_pause_host_toggle_pause_decl_contract.py",
        f"{prefix}test_sortie_hud_get_bound_presentation_decl_contract.py",
        f"{prefix}test_radio_chatter_enqueue_line_decl_contract.py",
        f"{prefix}test_pause_host_bind_pause_input_decl_contract.py",
        f"{prefix}test_pause_host_pause_widget_class_field_decl_contract.py",
        f"{prefix}test_audio_production_bank_ensure_default_entries"
        "_decl_contract.py",
        f"{prefix}test_gunner_get_sortie_hits_decl_contract.py",
        f"{prefix}test_gunner_reset_sortie_combat_stats_decl_contract.py",
        f"{prefix}test_sortie_hud_bind_presentation_decl_contract.py",
        f"{prefix}test_sortie_hud_refresh_from_presentation_state"
        "_decl_contract.py",
        f"{prefix}test_env_vfx_get_activation_count_decl_contract.py",
        f"{prefix}test_settings_get_invert_vertical_look_decl_contract.py",
        f"{prefix}test_pathfinder_encounter_ingress_sway_wavelength"
        "_field_decl_contract.py",
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_pause_host_fail_closed_tests.py",
    "Scripts/tests/test_pause_host_fail_closed_contract.py",
    "Scripts/tests/test_pause_host_toggle_pause_decl_contract.py",
    "Scripts/tests/test_pause_host_bind_pause_input_decl_contract.py",
    "Scripts/tests/test_pause_host_pause_widget_class_field_decl_contract.py",
    "Scripts/tests/test_audio_production_bank_ensure_default_entries_decl_contract.py",
    "Scripts/tests/test_gunner_get_sortie_hits_decl_contract.py",
    "Scripts/tests/test_gunner_reset_sortie_combat_stats_decl_contract.py",
    "Scripts/tests/test_sortie_hud_bind_presentation_decl_contract.py",
    "Scripts/tests/test_sortie_hud_refresh_from_presentation_state_decl_contract.py",
    "Scripts/tests/test_env_vfx_get_activation_count_decl_contract.py",
    "Scripts/tests/test_settings_get_invert_vertical_look_decl_contract.py",
    "Scripts/tests/test_pathfinder_encounter_ingress_sway_wavelength_field_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bbool\s+IsPauseMenuVisible\s*\(\s*\)\s*const\s*(?:;|\{)"
)
...
LEFTOVER_PRODUCTION_BANK_FIELD_RE = re.compile(
    r"TObjectPtr\s*<\s*USkyguardPauseHostComponent\s*>\s+ProductionBank\b"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_TOGGLE_PAUSE,
        SIBLING_BIND_PAUSE_INPUT,
        SIBLING_PAUSE_WIDGET_CLASS,
        SIBLING_PAUSE_WIDGET_ZORDER,
        SIBLING_BEGIN_PLAY,
...
def leftover_analog_field_in_region(compact: str) -> bool:
    ...
    if re.search(r"RouteLengthCm\s*=\s*45000\.f\b", compact):
        return True
    if re.search(r"IngressSwayWavelength\b", compact):
        return True
    if re.search(r"bool\s+bSuccess\s*=\s*false\s*;", compact):
        return True
    if re.search(r"int32\s+PoolCapacity\s*=\s*12\b", compact):
        return True
    return False
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # missing const qualifier, a void/float/int32 rename, extra
    # params, leftover unique TogglePause / EnsureDefaultEntries /
    # GetSortieHits / AreResolvedProductionLoopRoutesComplete /
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+IsPauseMenuVisible\b", compact):
        return False
    if re.search(
        r"\bbool\s+IsPauseMenuVisible\s*\(\s*\)\s*(?:;|\{)",
        compact,
    ):
        return False
    if re.search(r"\bfloat\s+IsPauseMenuVisible\b", compact):
        return False
    if re.search(r"\bint32\s+IsPauseMenuVisible\b", compact):
        return False
    if re.search(
        r"\bbool\s+IsPauseMenuVisible\s*\(\s*[^)]+\s*\)",
        compact,
...
def spec_section(header: str) -> str:
    ...
    event = re.search(r"\bPauseWidgetClass\b", section)
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+IsPauseMenuVisible\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for bool IsPauseMenuVisible "
...
class PauseHostIsPauseMenuVisibleDeclContractTests(
    unittest.TestCase
):
    def test_pause_host_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "IsPauseMenuVisible"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, "ResumeGame"), body)
        self.assertFalse(has_identifier(section, "ResumeGame"), section)
    ...
    def test_missing_is_pause_menu_visible_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Pause"', section)
        ...
        self.assertIn('Category="Skyguard|Pause"', specifiers)
        ...
        self.assertTrue("|Pause" in specifiers)
        self.assertFalse("|Production" in specifiers)
        self.assertNotEqual(
            specifiers,
        ...
        self.assertNotEqual(
            specifiers,
            'BlueprintCallable, Category="Skyguard"',
        )
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn("void TogglePause()const;", compact_origin)
    ...
    def test_is_pause_menu_visible_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_ENSURE)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_REFRESH_PRESENTATION)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PRIME_LINES)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENQUEUE_LINE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ACTIVATION_COUNT)
        ...
        self.assertNotIn(
            'Skyguard|Presentation|HUD',
            LOCKED_UFUNCTION_WRAP,
        )
        self.assertNotIn(
            'Skyguard|Presentation|Briefing',
            LOCKED_UFUNCTION_WRAP,
        )
        self.assertNotIn(
            'Skyguard|Presentation|Debrief',
            LOCKED_UFUNCTION_WRAP,
        )
        self.assertNotIn(
        ...
        self.assertNotIn(
            'Skyguard|Mission01|Environment|Validation',
            LOCKED_UFUNCTION_WRAP,
        )
        self.assertNotIn(
            'Skyguard|Radio',
        ...
        self.assertNotIn(
            'Skyguard|Boss|Encounter',
            LOCKED_UFUNCTION_WRAP,
        )
        self.assertNotIn(
            'Skyguard|Audio|Production',
            LOCKED_UFUNCTION_WRAP,
        )
        self.assertNotIn(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid IsPauseMenuVisibles();\n",
            "\tint32 IsPauseMenuVisible = 1;\n",
            "\tFName IsPauseMenuVisible = NAME_None;\n",
            "\tvoid IsPauseMenuVisible() = "
            + forty
            + ";\n",
            "\tvoid IsPauseMenuVisible() = "
            + eighty
    ...
    def test_missing_category_or_blueprint_pure_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Pause"', origin)
        ...
        self.assertNotIn('Skyguard|Audio|Production', origin)
        ...
        self.assertIn('Category="Skyguard|Environment|VFX"', vfx_specs)
        leftover_validation = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_bind_pause_input_or_neighbors(
        self,
    ) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        ...
        self.assertTrue(has_identifier(section, SIBLING_TOGGLE_PAUSE))
        self.assertTrue(has_identifier(section, SIBLING_BIND_PAUSE_INPUT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_TOGGLE_PAUSE))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_BIND_PAUSE_INPUT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_PAUSE_WIDGET_CLASS))
        self.assertFalse(
            has_identifier(TARGET_WRONG_TOGGLE_PAUSE, "IsPauseMenuVisible")
        )
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TOGGLE_PAUSE)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "IsPauseMenuVisible")
        )
        self.assertTrue(
            has_identifier(section, "IsPauseMenuVisible")
        )
        ...
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, "ResumeGame"))
        self.assertFalse(has_identifier(section, "ResumeGame"))
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("{", LOCKED_DECL)
        self.assertNotIn("return ", LOCKED_DECL)
        self.assertNotIn("SkyguardPauseHostComponent.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardPauseHostComponent.h", HEADER_PATH)
        self.assertNotIn("SkyguardAudioProductionBank.h", HEADER_PATH)
        self.assertNotIn("SkyguardAudioDirectorComponent.h", HEADER_PATH)
        self.assertNotIn("SkyguardPauseHostComponentTests.cpp", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pause host is pause menu visible "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pause_host_is_pause_menu_visible"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardPauseHostComponent.cpp", THIS_SCRIPT)
    ...
    def test_word_boundary_is_pause_menu_visible_vs_toggle_pause(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier("bool IsPauseMenuVisible() const;", "IsPauseMenuVisible")
        )
        self.assertFalse(
            has_identifier("void TogglePause();", "IsPauseMenuVisible")
        )
        self.assertFalse(
            has_identifier("bool IsPauseMenuVisible() const;", "TogglePause")
        )
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TOGGLE_PAUSE)
        self.assertFalse(has_identifier(LOCKED_DECL, "TogglePause"))
        self.assertFalse(has_identifier(LOCKED_DECL, "BindPauseInput"))
        self.assertFalse(has_identifier(LOCKED_DECL, "PauseWidgetClass"))
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "IsPauseMenuVisible"), section)
        self.assertTrue(
            has_identifier(section, "TogglePause"),
            section,
        )
        self.assertEqual(declaration_count(section, LOCKED_DECL), 1)
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertNotIn("BindPresentation", locked_only)
        self.assertNotIn("RefreshFromPresentationState", locked_only)
        self.assertNotIn("TogglePause", locked_only)
        self.assertNotIn("RebindIfNeeded", locked_only)
        self.assertNotIn("PoolCapacity", locked_only)
        self.assertNotIn("PrimeLines", locked_only)
        self.assertNotIn("EnqueueLine", locked_only)
        self.assertNotIn("GetActivationCount", locked_only)
        self.assertNotIn("RouteLengthCm", locked_only)
        self.assertNotIn("IngressSwayWavelength", locked_only)
        self.assertNotIn("bSuccess", locked_only)
        self.assertNotIn("PauseWidgetClass", locked_only)
        self.assertNotIn("BindPauseInput", locked_only)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_CLONE_ENSURE)
        for leftover in leftover_apache_ufunction_tokens():
