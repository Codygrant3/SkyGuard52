...
# debrief payload, persistence result, score, medal, or
# slot-save behavior, or lock the FinalizeActiveMission
...
# lane does not lock RetrySaveLastDebrief, GetLastDebrief,
# leftover CPG debrief drafts, or leftover campaign-save
# empty-fail-closed.
FINALIZE = (
    "bool FinalizeActiveMission("
    "FSkyguardMissionResult& InOutResult, "
...
# FinalizeActiveMission declaration contract. Stay
# off CompleteActiveMission (sibling in-flight this wave),
# FailActiveMission, FillResultCombatStats (takes leftover
# ASkyguardGunner*), FillAndFinalize / FillAndFail
# (leftover Gunner), ApplyHydraForClusters (leftover
# Gunner), RetrySaveLastDebrief / SaveCampaignToSlot /
# LoadCampaignFromSlot / DeleteCampaignSlot / GetLastDebrief
# / AcknowledgeDebrief, GetActiveMission /
# GetObjectiveRuntime / GetRouteRuntime /
# TravelToNextMission / GetActiveMissionElapsedSeconds,
# AddObjectiveProgress / FailObjective /
# CompleteSurviveObjectiveIfIntact, leftover
...
# CompleteActiveMission is a sibling in-flight this wave.
# FailActiveMission / FillResultCombatStats stay leftover
# neighbors. FillResultCombatStats takes leftover
# ASkyguardGunner*. Leftover CPG debrief #284/#195/#130/
# #8ccd stay sibling-only. GetActiveMissionElapsedSeconds
# is a newly drafted sibling.
UNLOCKED_NEIGHBORS = (
...
# Nearby origin/main comment tokens stay unlocked. Do not
# lock RetrySaveLastDebrief / GetLastDebrief / invented
# debrief payload / persistence result.
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return 0",
    "USkyguardCampaignSubsystem::FinalizeActiveMission",
    "SkyguardCampaignSubsystem.cpp",
    "BuildSuccessDebrief",
    "BuildFailureDebrief",
...
class FinalizeActiveMissionDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, FINALIZE), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, FINALIZE)
        self.assertIn("FinalizeActiveMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, FINALIZE))

    def test_missing_finalize_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, FINALIZE)
        self.assertIn("FinalizeActiveMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, FINALIZE)
        self.assertIn("FinalizeActiveMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, FINALIZE)
        self.assertIn("FinalizeActiveMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, FINALIZE)
            self.assertIn("FinalizeActiveMission", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_fail_active_mission_does_not_satisfy(self) -> None:
        fail_only = (
            "\tbool FailActiveMission(\n"
            "\t\tFSkyguardMissionResult& InOutResult,\n"
            '\t\tconst FString& SlotName = TEXT("Skyguard52Campaign"),\n'
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(fail_only, FINALIZE)
        self.assertIn("FinalizeActiveMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(fail_only, FINALIZE))

    def test_finalize_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, FINALIZE),
            FINALIZE,
        )
        self.assertTrue(has_declaration(section, FINALIZE))
        self.assertEqual(declaration_count(section, FINALIZE), 1)
        self.assertTrue(FINALIZE.startswith("bool "), FINALIZE)
        self.assertTrue(FINALIZE.endswith(");"), FINALIZE)
        self.assertIn("FSkyguardMissionResult& InOutResult", FINALIZE)
        self.assertIn(
            'const FString& SlotName = TEXT("Skyguard52Campaign")',
            FINALIZE,
        )
        self.assertIn("int32 UserIndex = 0", FINALIZE)
        self.assertNotIn("INDEX_NONE", FINALIZE)
        self.assertNotIn("{", FINALIZE)
        self.assertNotIn("}", FINALIZE)
        self.assertNotIn("return ", FINALIZE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_args = (
            "public:\n"
            "\tbool FinalizeActiveMission(FSkyguardMissionResult&\n"
            "\t\tInOutResult,\n"
            "\t\tconst FString&\n"
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, FINALIZE), 1)
        one_line = f"{{\npublic:\n\t{FINALIZE}\n}}\n"
        self.assertTrue(has_declaration(one_line, FINALIZE))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, FINALIZE), section)
        self.assertEqual(
            require_declaration(section, FINALIZE),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{FINALIZE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", FINALIZE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", FINALIZE)
        self.assertNotIn("UserIndex = INDEX_NONE", FINALIZE)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_debrief_or_persistence(
        ...
        locked_only = f"{FINALIZE}\n"
        self.assertNotIn("return ", FINALIZE)
        self.assertNotIn("return true", FINALIZE)
        self.assertNotIn("return false", FINALIZE)
        self.assertNotIn("LastDebrief.bProgressSaved", FINALIZE)
        self.assertNotIn("RetrySaveLastDebrief", FINALIZE)
        self.assertNotIn("GetLastDebrief", FINALIZE)
        self.assertNotIn("BuildSuccessDebrief", FINALIZE)
        self.assertNotIn("BuildFailureDebrief", FINALIZE)
        self.assertNotIn("CalculateMissionScore", FINALIZE)
        self.assertNotIn("CalculateMedalTier", FINALIZE)
        self.assertNotIn("SaveCampaignToSlot", FINALIZE)
        self.assertNotIn("LoadCampaignFromSlot", FINALIZE)
        self.assertNotIn("DeleteCampaignSlot", FINALIZE)
        self.assertNotIn("FillAndFinalize", FINALIZE)
        self.assertNotIn("FillAndFail", FINALIZE)
    ...
    def test_nearby_comment_does_not_lock_retry_save_or_last_debrief(
        self,
    ) -> None:
        locked_only = f"{FINALIZE}\n"
        for token in NEARBY_COMMENT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, FINALIZE)
        self.assertNotIn("RetrySaveLastDebrief", FINALIZE)
        self.assertNotIn("GetLastDebrief", FINALIZE)
        self.assertNotIn("LastDebrief.bProgressSaved", FINALIZE)
        self.assertNotIn("bool RetrySaveLastDebrief(", locked_only)
        self.assertNotIn(
            "const FSkyguardMissionDebrief& GetLastDebrief() const",
            locked_only,
        )

    def test_contract_does_not_lock_finalize_cpp_body(self) -> None:
        locked_only = f"{FINALIZE}\n"
        self.assertNotIn("{", FINALIZE)
        self.assertNotIn("}", FINALIZE)
        self.assertNotIn("return ", FINALIZE)
        ...
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", FINALIZE)
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", locked_only)
        self.assertNotIn("BuildSuccessDebrief", FINALIZE)
        self.assertNotIn("BuildFailureDebrief", FINALIZE)
        self.assertNotIn("ClearActiveMissionRuntime", FINALIZE)
        self.assertNotIn("LastDebrief.bProgressSaved", FINALIZE)
    ...
    def test_contract_does_not_relock_get_active_mission(self) -> None:
        ...
        for neighbor in GET_ACTIVE_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn(
            "USkyguardMissionDefinition* GetActiveMission() const",
            FINALIZE,
        )
    ...
    def test_contract_does_not_relock_configure_can_start_or_start(self) -> None:
        ...
        for neighbor in CONFIGURE_CAN_START_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("ConfigureCampaign", FINALIZE)
        self.assertNotIn("CanStartMission", FINALIZE)
        self.assertNotIn("StartMission", FINALIZE)
    ...
    def test_contract_does_not_relock_is_mission_unlocked(self) -> None:
        ...
        for neighbor in IS_MISSION_UNLOCKED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("IsMissionUnlocked", FINALIZE)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("AddObjectiveProgress", FINALIZE)
        self.assertNotIn("FailObjective", FINALIZE)
        self.assertNotIn("CompleteSurviveObjectiveIfIntact", FINALIZE)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("CompleteActiveMission", FINALIZE)
        self.assertNotIn("FailActiveMission", FINALIZE)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("FillResultCombatStats", FINALIZE)
        self.assertNotIn("ASkyguardGunner", FINALIZE)
    ...
    def test_contract_does_not_relock_elapsed_seconds(self) -> None:
        ...
        for neighbor in ELAPSED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("GetActiveMissionElapsedSeconds", FINALIZE)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("AcknowledgeDebrief", FINALIZE)
    ...
    def test_contract_does_not_relock_travel_helpers(self) -> None:
        ...
        for neighbor in TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("CanTravelToNextMission", FINALIZE)
        self.assertNotIn("GetNextMissionMapPackageName", FINALIZE)
        self.assertNotIn("TravelToNextMission", FINALIZE)
    ...
    def test_contract_does_not_relock_runtime_getters(self) -> None:
        ...
        for neighbor in RUNTIME_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("GetObjectiveRuntime", FINALIZE)
        self.assertNotIn("GetRouteRuntime", FINALIZE)
    ...
    def test_contract_does_not_relock_save_game_helpers(self) -> None:
        ...
        for neighbor in SAVE_GAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FINALIZE)
        self.assertNotIn("ApplySaveGame", FINALIZE)
        self.assertNotIn("BuildSaveGame", FINALIZE)
        self.assertNotIn("SaveCampaignToSlot", FINALIZE)
        self.assertNotIn("LoadCampaignFromSlot", FINALIZE)
        self.assertNotIn("DeleteCampaignSlot", FINALIZE)
        self.assertNotIn("IsValidCampaignSlotName", FINALIZE)
        self.assertNotIn("GetEarnedCampaignMedals", FINALIZE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{FINALIZE}\n"
        self.assertEqual(
            require_declaration(locked_only, FINALIZE),
            FINALIZE,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(FINALIZE, "Rifle")
        self.assertNotEqual(FINALIZE, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign FinalizeActiveMission contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, FINALIZE.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", FINALIZE)

    def test_contract_is_finalize_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
