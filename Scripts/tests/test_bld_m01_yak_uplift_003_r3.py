...
VERIFIER_PATH = REPO_ROOT / "Scripts" / "verify_bld_m01_yak_uplift_003_r3.py"
SPEC = importlib.util.spec_from_file_location("uplift003_r3_verifier", VERIFIER_PATH)
...
class Uplift003R3VerifierTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _copy_source_tree(self) -> None:
        for relative in (
            "Docs/AAA_Review/BLD_M01_YAK_UPLIFT_003_R3_CONTRACT.json",
            "Docs/AAA_Review/BLD_M01_YAK_UPLIFT_003_R3_COMPONENT_LEDGER.json",
            "Docs/AAA_Review/BLD_M01_YAK_UPLIFT_003_R3_SOURCE_AUDIT.json",
            ...
            "Saved/BuildAttempts/BLD_M01_YAK_UPLIFT_003_R2/attempt_01/stderr.log",
            "Saved/BuildAttempts/BLD_M01_YAK_UPLIFT_003_R2/attempt_01/stdout.log",
            "Saved/BuildAttempts/BLD_M01_YAK_UPLIFT_003_R2/attempt_01/terminal.json",
            ...
            "Scripts/blender_bld_m01_yak_uplift_003_r3.py",
    ...
    def _refresh_contract_record(self, key: str, relative: str) -> None:
        contract_path = "Docs/AAA_Review/BLD_M01_YAK_UPLIFT_003_R3_CONTRACT.json"
    ...
    def _refresh_overlay_record(self, key: str, relative: str) -> None:
        ...
        self._refresh_contract_record("r3_component_ledger", overlay_path)
    ...
    def _make_artifacts(self) -> dict:
        ...
        output_records = {}
        for key, payload in (
            ("blend", b"synthetic-r3-blend"),
            ("glb", b"synthetic-r3-glb"),
        ):
        ...
        for index, slot in enumerate(contract["matched_comparison_slots"]):
            ...
            path.parent.mkdir(parents=True, exist_ok=True)
            payload = f"synthetic-r3-comparison-{index}".encode()
            path.write_bytes(payload)
            comparisons.append(
                {
                    "slot": slot["slot"],
    ...
    def test_exception_count_drift_is_rejected(self) -> None:
        ...
        self._refresh_contract_record("r3_component_ledger", relative)
        with self.assertRaisesRegex(verifier.ValidationError, "eight R3 exceptions"):
    ...
    def test_stage_order_drift_is_rejected(self) -> None:
        relative = "Docs/AAA_Review/BLD_M01_YAK_UPLIFT_003_R3_CONTRACT.json"
    ...
    def test_r1_output_overlap_is_rejected(self) -> None:
        ...
        self._write_json(relative, contract)
        with self.assertRaisesRegex(verifier.ValidationError, "not isolated|overlaps R1/R2"):
            verifier.verify(self.root)

    def test_forbidden_blender_eevee_next_is_rejected(self) -> None:
        relative = "Scripts/blender_bld_m01_yak_uplift_003_r3.py"
        path = self.root / relative
        ...
        with self.assertRaisesRegex(
            verifier.ValidationError, "forbidden BLENDER_EEVEE_NEXT"
        ):
            verifier.verify(self.root)

    def test_render_compatibility_contract_drift_is_rejected(self) -> None:
        relative = "Docs/AAA_Review/BLD_M01_YAK_UPLIFT_003_R3_CONTRACT.json"
        contract = self._json(relative)
        contract["render_compatibility"]["required_engine"] = "BLENDER_WORKBENCH"
        self._write_json(relative, contract)
        with self.assertRaisesRegex(verifier.ValidationError, "compatibility contract"):
            verifier.verify(self.root)
    ...
    def test_tampered_r3_glb_is_rejected(self) -> None:
        self._make_artifacts()
        contract = self._json(
            "Docs/AAA_Review/BLD_M01_YAK_UPLIFT_003_R3_CONTRACT.json"
        )
