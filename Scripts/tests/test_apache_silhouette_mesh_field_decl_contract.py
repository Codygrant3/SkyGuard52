# THIS IS leftover-safe ASkyguardApacheAircraft SilhouetteMesh.
...
# then BARE `TObjectPtr<UStaticMeshComponent> SilhouetteMesh;`
# (no initializer).
...
# Stop BEFORE sibling Fuselage as a claimed slot.
# Do NOT claim AircraftRoot (in-flight), RotorPower,
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> SilhouetteMesh;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> SilhouetteMesh;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> SilhouetteMesh = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> SilhouetteMesh = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> SilhouetteMesh = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> SilhouetteMesh;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> SilhouetteMesh;"
...
SILHOUETTE_MESH_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+SilhouetteMesh\s*;"
)
SILHOUETTE_MESH_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+SilhouetteMesh\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover Fuselage / AircraftRoot, or an initializer
    # on SilhouetteMesh as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SILHOUETTE_MESH_DECL_RE.search(compact) is None:
        return False
    if SILHOUETTE_MESH_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_FUSELAGE):
            break
    ...
    if has_identifier(window, SIBLING_FUSELAGE):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+SilhouetteMesh\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> SilhouetteMesh "
...
class ApacheSilhouetteMeshFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SilhouetteMesh"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_FUSELAGE),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SilhouetteMesh"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SilhouetteMesh", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_silhouette_mesh_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> SilhouetteMesh;", compact_origin)
        self.assertNotIn("SilhouetteMesh = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_silhouette_mesh_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> SilhouetteMesh"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> SilhouetteMesh;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> SilhouetteMeshes;\n",
            "\tint32 SilhouetteMesh = 140;\n",
            "\tbool SilhouetteMesh = true;\n",
            "\tfloat SilhouetteMesh = " + forty + ";\n",
            "\tfloat SilhouetteMesh = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_fuselage_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_FUSELAGE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache SilhouetteMesh field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "silhouette_mesh_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
