...
# Declaration presence only. Do not invent INDEX_NONE, a medal
# count return value, or the GetEarnedCampaignMedals body in
# the .cpp.
GET_EARNED_MEDALS_DECL = "int32 GetEarnedCampaignMedals() const;"
# Leftover #56–#64 plus CampaignSubsystem production files.
# This lane only adds an isolated Python GetEarnedCampaignMedals
# declaration contract. Stay off ConfigureCampaign #302,
# CanStartMission #303, StartMission (newly drafted sibling),
# IsMissionUnlocked / IsValidCampaignSlotName (in-flight
# siblings), CalculateMissionScore / CalculateMedalTier,
# AcknowledgeDebrief / CanTravelToNextMission /
# GetNextMissionMapPackageName, leftover FillResultCombatStats
...
# CanStartMission / StartMission, leftover campaign-save
# empty-fail-closed, leftover campaign-roster lookup #111,
# leftover LoadCampaignProgressAfterConfigure (#290), leftover
...
# .cpp GetEarnedCampaignMedals body / invented medal-count
...
CPP_AND_INVENTED = (
    ...
    "USkyguardCampaignSubsystem::GetEarnedCampaignMedals",
    "SkyguardCampaignSubsystem.cpp",
)
SIBLING_TYPES = (
...
class GetEarnedCampaignMedalsDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_EARNED_MEDALS_DECL), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_EARNED_MEDALS_DECL)
        self.assertIn("GetEarnedCampaignMedals", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertNotIn(GET_EARNED_MEDALS_DECL, section)

    def test_missing_get_earned_campaign_medals_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_EARNED_MEDALS_DECL)
        self.assertIn("GetEarnedCampaignMedals", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_EARNED_MEDALS_DECL)
        self.assertIn("GetEarnedCampaignMedals", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_get_earned_campaign_medals_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_EARNED_MEDALS_DECL),
            GET_EARNED_MEDALS_DECL,
        )
        self.assertTrue(has_declaration(section, GET_EARNED_MEDALS_DECL))
        self.assertEqual(declaration_count(section, GET_EARNED_MEDALS_DECL), 1)
        self.assertTrue(
            GET_EARNED_MEDALS_DECL.endswith(";"),
            GET_EARNED_MEDALS_DECL,
        )
        self.assertTrue(
            GET_EARNED_MEDALS_DECL.startswith("int32 "),
            GET_EARNED_MEDALS_DECL,
        )
        self.assertIn("() const;", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("INDEX_NONE", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("return ", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("{", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("}", GET_EARNED_MEDALS_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tint32\n"
            "\tGetEarnedCampaignMedals() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_args = (
            "public:\n"
            "\tint32 GetEarnedCampaignMedals(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, GET_EARNED_MEDALS_DECL),
                section,
            )
            self.assertEqual(
                require_declaration(section, GET_EARNED_MEDALS_DECL),
            ...
            self.assertEqual(
                declaration_count(section, GET_EARNED_MEDALS_DECL),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_EARNED_MEDALS_DECL}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_EARNED_MEDALS_DECL))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_EARNED_MEDALS_DECL), section)
        self.assertEqual(
            require_declaration(section, GET_EARNED_MEDALS_DECL),
    ...
    def test_declaration_does_not_invent_index_none_or_medal_count(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertTrue(
        ...
        self.assertNotIn("return ", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("INDEX_NONE", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("NAME_None", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("{", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("}", GET_EARNED_MEDALS_DECL)
        ...
        self.assertNotIn("return Total", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("int32 Total = 0", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("BestMedalTier", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("= INDEX_NONE", section)
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, GET_EARNED_MEDALS_DECL)

    def test_contract_does_not_relock_configure_campaign(self) -> None:
        locked_only = f"{GET_EARNED_MEDALS_DECL}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_EARNED_MEDALS_DECL),
            GET_EARNED_MEDALS_DECL,
        )
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EARNED_MEDALS_DECL)
        self.assertNotIn("ConfigureCampaign", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("ConfigureCampaign", locked_only)
        self.assertNotIn("USkyguardCampaignDefinition", GET_EARNED_MEDALS_DECL)
    ...
    def test_contract_does_not_relock_can_start_mission(self) -> None:
        ...
        for neighbor in CAN_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EARNED_MEDALS_DECL)
        self.assertNotIn("CanStartMission", GET_EARNED_MEDALS_DECL)
    ...
    def test_contract_does_not_relock_start_mission(self) -> None:
        ...
        for neighbor in START_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EARNED_MEDALS_DECL)
        self.assertNotIn("StartMission", GET_EARNED_MEDALS_DECL)
    ...
    def test_contract_does_not_relock_unlock_or_slot_helpers(self) -> None:
        ...
        for neighbor in UNLOCK_AND_SLOT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EARNED_MEDALS_DECL)
        self.assertNotIn("IsMissionUnlocked", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("IsValidCampaignSlotName", GET_EARNED_MEDALS_DECL)
    ...
    def test_contract_does_not_relock_score_or_medal_tier(self) -> None:
        ...
        for neighbor in SCORE_AND_TIER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EARNED_MEDALS_DECL)
        self.assertNotIn("CalculateMissionScore", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("CalculateMedalTier", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("CalculateMissionScore", locked_only)
        self.assertNotIn("CalculateMedalTier", locked_only)

    def test_contract_does_not_relock_debrief_or_travel_helpers(self) -> None:
        locked_only = f"{GET_EARNED_MEDALS_DECL}\n"
        for neighbor in DEBRIEF_TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EARNED_MEDALS_DECL)
        self.assertNotIn("AcknowledgeDebrief", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("CanTravelToNextMission", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("GetNextMissionMapPackageName", GET_EARNED_MEDALS_DECL)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_EARNED_MEDALS_DECL)
        self.assertNotIn("FillResultCombatStats", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("ASkyguardGunner", GET_EARNED_MEDALS_DECL)
    ...
    def test_contract_parses_public_section_not_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("BestMedalTier", section)
        self.assertNotIn("int32 Total = 0", section)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("int32 Total = 0", section)
        self.assertNotIn("return Total", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("return 0", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("INDEX_NONE", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("{", GET_EARNED_MEDALS_DECL)
        self.assertNotIn("}", GET_EARNED_MEDALS_DECL)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(GET_EARNED_MEDALS_DECL, "Rifle")
        self.assertNotEqual(GET_EARNED_MEDALS_DECL, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"GetEarnedCampaignMedals contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not Yak",
            )
            self.assertNotIn(banned, GET_EARNED_MEDALS_DECL.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", GET_EARNED_MEDALS_DECL)

    def test_contract_is_get_earned_campaign_medals_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
