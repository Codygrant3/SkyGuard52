...
HEADER_NAME = "SkyguardNightSortieBeatKit.h"
...
# On-main night beat-kit contract stays locked here. Day/storm beat-kit
# contracts, Mission 04 wave-state (#231), audio-source-status, and
# Mission 09 pool-budget are sibling isolated contracts.
LOCKED_SCRIPTS = (
...
# Apache CPG night sortie identity. Not weather labels and not Harbor
# IncomingRadar 40/80. Twelve kinds only — declaration order from
# origin/main SkyguardNightSortieBeatKit.h.
LIVE_BEAT_KINDS = [
    "DarkIngress",
    "ThermalHunt",
    "RadarVanHunt",
...
# Night kit sequences, bKeepThermal, day/storm beat kits, and weather
# identities stay unlocked.
SIBLING_TYPES = (
...
class NightSortieBeatKindEnumContractTests(unittest.TestCase):
    ...
    def test_missing_enum_fails_closed(self) -> None:
        ...
        self.assertIn("ESkyguardNightSortieBeatKind", str(raised.exception))
    ...
    def test_enumerators_match_live_order(self) -> None:
        ...
        body = enum_body(header, "ESkyguardNightSortieBeatKind")
    ...
    def test_beat_kind_enum_is_not_a_weather_label(self) -> None:
        enumerators = enum_enumerators(
            origin_main(HEADER_NAME),
            "ESkyguardNightSortieBeatKind",
        )
        self.assertNotIn("BlackoutNight", enumerators)
        self.assertNotIn("IslandMist", enumerators)
        self.assertNotIn("StormFront", enumerators)
        self.assertNotEqual(
            enumerators,
            ["BlackoutNight", "IslandMist"],
        )

    def test_contract_is_night_sortie_beat_kind_enum_only(self) -> None:
        header = origin_main(HEADER_NAME)
        body = enum_body(header, "ESkyguardNightSortieBeatKind")
        self.assertIn("DarkIngress", body)
