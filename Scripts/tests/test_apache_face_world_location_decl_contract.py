...
# or lock the FaceWorldLocation body in the .cpp.
...
# in this wave, leftover GetPilotCommand sibling
# in this wave, leftover GetPilotConfirmationsIssued
# sibling in this wave, leftover SetOrbitFocus sibling
# in this wave, leftover SetSensorView sibling in this
# wave, leftover AimChinTurret sibling, leftover
# SetRotorPower sibling, and leftover
...
# FaceWorldLocation declaration contract on
...
# SetOrbitFocus sibling, leftover SetSensorView
...
# FaceWorldLocation parameter type is not a leftover
# roster value lock.
LEFTOVER_NOT_LOCKED = (
    "ESkyguardApacheSystem",
...
# .cpp FaceWorldLocation body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::FaceWorldLocation",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheFaceWorldLocationDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, FACE_WORLD_LOCATION), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, FACE_WORLD_LOCATION)
        self.assertIn("FaceWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, FACE_WORLD_LOCATION))

    def test_missing_face_world_location_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, FACE_WORLD_LOCATION)
        self.assertIn("FaceWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, FACE_WORLD_LOCATION)
        self.assertIn("FaceWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, FACE_WORLD_LOCATION), section)
        self.assertNotIn("BlueprintPure", FACE_WORLD_LOCATION)
        self.assertNotIn("UFUNCTION", FACE_WORLD_LOCATION)
        self.assertNotIn("Category", FACE_WORLD_LOCATION)
        self.assertNotIn("BlueprintCallable", FACE_WORLD_LOCATION)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, FACE_WORLD_LOCATION)
        self.assertIn("FaceWorldLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid FaceWorldLocation;\n"
        wrong_return_bool = (
            "\tbool FaceWorldLocation(const FVector& WorldLocation);\n"
        )
        wrong_return_int = (
            "\tint32 FaceWorldLocation(const FVector& WorldLocation);\n"
        )
        added_const = (
            "\tvoid FaceWorldLocation(const FVector& WorldLocation) const;\n"
        )
        wrong_type = "\tvoid FaceWorldLocation(int WorldLocation);\n"
        wrong_rotator = (
            "\tvoid FaceWorldLocation(const FRotator& WorldLocation);\n"
        )
        missing_ref = "\tvoid FaceWorldLocation(const FVector WorldLocation);\n"
        added_arg = (
            "\tvoid FaceWorldLocation("
            "const FVector& WorldLocation, float Extra);\n"
        )
        no_args = "\tvoid FaceWorldLocation();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, FACE_WORLD_LOCATION)
            self.assertIn("FaceWorldLocation", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_face_world_location_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, FACE_WORLD_LOCATION),
            FACE_WORLD_LOCATION,
        )
        self.assertTrue(has_declaration(section, FACE_WORLD_LOCATION))
        self.assertEqual(declaration_count(section, FACE_WORLD_LOCATION), 1)
        self.assertTrue(
            FACE_WORLD_LOCATION.startswith("void "),
            FACE_WORLD_LOCATION,
        )
        self.assertTrue(FACE_WORLD_LOCATION.endswith(";"), FACE_WORLD_LOCATION)
        self.assertIn("FaceWorldLocation(", FACE_WORLD_LOCATION)
        self.assertIn("const FVector& WorldLocation", FACE_WORLD_LOCATION)
        self.assertNotIn("INDEX_NONE", FACE_WORLD_LOCATION)
        self.assertNotIn("{", FACE_WORLD_LOCATION)
        self.assertNotIn("}", FACE_WORLD_LOCATION)
        self.assertNotIn("return ", FACE_WORLD_LOCATION)
        self.assertNotIn(" const", FACE_WORLD_LOCATION)
        self.assertNotIn("SetOrbitFocus", FACE_WORLD_LOCATION)
        self.assertNotIn("SetRotorPower", FACE_WORLD_LOCATION)
        self.assertNotIn("AimChinTurret", FACE_WORLD_LOCATION)
        self.assertNotIn("GetChinMuzzleLocation", FACE_WORLD_LOCATION)
        self.assertNotIn("IssuePilotCommand", FACE_WORLD_LOCATION)
        self.assertNotIn("GetPilotCommand", FACE_WORLD_LOCATION)
        self.assertNotIn("GetPilotConfirmationsIssued", FACE_WORLD_LOCATION)
        self.assertNotIn("SetSensorView", FACE_WORLD_LOCATION)
        self.assertNotIn("GetChinTurret", FACE_WORLD_LOCATION)
        self.assertNotIn("GetRotorPowerScale", FACE_WORLD_LOCATION)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tFaceWorldLocation(const FVector& WorldLocation);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, FACE_WORLD_LOCATION),
                section,
            )
            self.assertEqual(
                require_declaration(section, FACE_WORLD_LOCATION),
            ...
            self.assertEqual(
                declaration_count(section, FACE_WORLD_LOCATION),
                1,
            )
        one_line = f"{{\npublic:\n\t{FACE_WORLD_LOCATION}\n}}\n"
        self.assertTrue(has_declaration(one_line, FACE_WORLD_LOCATION))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, FACE_WORLD_LOCATION), section)
        self.assertEqual(
            require_declaration(section, FACE_WORLD_LOCATION),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{FACE_WORLD_LOCATION}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", FACE_WORLD_LOCATION)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", FACE_WORLD_LOCATION)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_face_world_location_cpp_body(self) -> None:
        locked_only = f"{FACE_WORLD_LOCATION}\n"
        self.assertNotIn("{", FACE_WORLD_LOCATION)
        self.assertNotIn("}", FACE_WORLD_LOCATION)
        self.assertNotIn("return ", FACE_WORLD_LOCATION)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", FACE_WORLD_LOCATION)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", FACE_WORLD_LOCATION)
        self.assertNotIn("return true", FACE_WORLD_LOCATION)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{FACE_WORLD_LOCATION}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, FACE_WORLD_LOCATION)
        self.assertNotIn("GetGunnerMount", FACE_WORLD_LOCATION)
        self.assertNotIn("GetEyeMount", FACE_WORLD_LOCATION)
        self.assertNotIn("GetWeaponMount", FACE_WORLD_LOCATION)
        self.assertNotIn("GetChinTurret", FACE_WORLD_LOCATION)
        self.assertNotIn("GetPilotMount", FACE_WORLD_LOCATION)
        self.assertNotIn("GetSensorTurret", FACE_WORLD_LOCATION)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{FACE_WORLD_LOCATION}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FACE_WORLD_LOCATION)
        self.assertNotIn("GetChinMuzzleLocation", FACE_WORLD_LOCATION)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, FACE_WORLD_LOCATION)
        self.assertNotIn("SetSensorView", FACE_WORLD_LOCATION)
        self.assertNotIn("SetFirstPersonInterior", FACE_WORLD_LOCATION)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FACE_WORLD_LOCATION)
        self.assertNotIn("ApplyDamage", FACE_WORLD_LOCATION)
        self.assertNotIn("GetDamageFraction", FACE_WORLD_LOCATION)
        self.assertNotIn("ApplySystemHit", FACE_WORLD_LOCATION)
        self.assertNotIn("ApplyHit", FACE_WORLD_LOCATION)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FACE_WORLD_LOCATION)
        self.assertNotIn("IsSensorLive", FACE_WORLD_LOCATION)
        self.assertNotIn("IsThermalAvailable", FACE_WORLD_LOCATION)
        self.assertNotIn("IsSensorViewActive", FACE_WORLD_LOCATION)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, FACE_WORLD_LOCATION)
        self.assertNotIn("ESkyguardApacheSystem", FACE_WORLD_LOCATION)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, FACE_WORLD_LOCATION)
        self.assertNotIn("Pursuit", FACE_WORLD_LOCATION)
        self.assertNotIn("OrbitLeft", FACE_WORLD_LOCATION)
        self.assertNotIn("AttackRun", FACE_WORLD_LOCATION)
        self.assertNotIn("FaceTarget", FACE_WORLD_LOCATION)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, FACE_WORLD_LOCATION)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", FACE_WORLD_LOCATION)
        self.assertNotIn("USkyguardBriefingWidget", FACE_WORLD_LOCATION)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            FACE_WORLD_LOCATION,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, FACE_WORLD_LOCATION)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", FACE_WORLD_LOCATION)
        self.assertNotIn("ESkyguardMissionSkylineStyle", FACE_WORLD_LOCATION)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{FACE_WORLD_LOCATION}\n"
        self.assertEqual(
            require_declaration(locked_only, FACE_WORLD_LOCATION),
            FACE_WORLD_LOCATION,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache FaceWorldLocation contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, FACE_WORLD_LOCATION.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache FaceWorldLocation contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, FACE_WORLD_LOCATION)

    def test_contract_is_face_world_location_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", FACE_WORLD_LOCATION)
        self.assertNotIn("{", FACE_WORLD_LOCATION)
        self.assertTrue(FACE_WORLD_LOCATION.startswith("void "))
        self.assertTrue(FACE_WORLD_LOCATION.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
