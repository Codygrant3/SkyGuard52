# THIS IS leftover-safe FSkyguardProductionAudioEntry SourceSha256.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `FString SourceSha256;` (no initializer).
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Fail-closed if missing/renamed,
# EditAnywhere dropped, BlueprintReadOnly replaces
# BlueprintReadWrite, type not FString, initializer added,
# or Category= added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardProductionAudioEntry` body ONLY after
# `struct FSkyguardProductionAudioEntry`. Stop at
# `struct FSkyguardProductionAudioRouting`.
# CRITICAL: start at the claimed struct so the preceding
# `enum class ESkyguardProductionAudioCategory` (contains
# Ri + fle / Ig + la tokens) stays OUT of the parse window.
# Do NOT parse `enum class ESkyguardAudioSourceStatus`
# (leftover #241 / #65ba).
# Do NOT fail the whole header for Ig + la / Ri + fle
# (those live in the preceding category enum).
# Fail-closed only if live Ig + la / Ri + fle / Ya + k /
# Harbor 40 + .f / 80 + .f appear IN THE PARSE WINDOW or IN
# THE TEST FILE.
# Do NOT parse leftover USkyguardAudioProductionBank /
# leftover USkyguardAudioProceduralBankComponent.
# Sibling decls stay unclaimed: Category / DisplayName /
# SourceStatus / Sound / Attenuation / Concurrency /
# OutputSubmix / ProvenanceId.
# Analog leftover production-audio-entry-defaults #198 /
# #314b then isolated SourceSha256 field decl.
# THIS IS NOT leftover DisplayName (in-flight).
# THIS IS NOT leftover ProvenanceId (in-flight).
...
# leftover ESkyguardAudioEvent.
# If a clone asserts FText / FName / VisibleAnywhere /
# BlueprintReadOnly / Category="Skyguard|Theater" /
# `= TEXT("")` / `= ""` / bool, retarget: type is FString,
# no initializer, BlueprintReadWrite, no Category.
...
TARGET = "FString SourceSha256;"
TARGET_WRONG_EQ_EMPTY = 'FString SourceSha256 = TEXT("");'
TARGET_WRONG_EQ_QUOTED = 'FString SourceSha256 = "";'
TARGET_WRONG_EQ_NONE = "FString SourceSha256 = NAME_None;"
TARGET_WRONG_TRUE = "FString SourceSha256 = true;"
TARGET_WRONG_FALSE = "FString SourceSha256 = false;"
TARGET_WRONG_ZERO = "FString SourceSha256 = 0;"
TARGET_WRONG_FLOAT = "float SourceSha256;"
TARGET_WRONG_FNAME = "FName SourceSha256;"
TARGET_WRONG_FTEXT = "FText SourceSha256;"
TARGET_WRONG_INT = "int32 SourceSha256;"
TARGET_WRONG_BOOL = "bool SourceSha256 = false;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString SourceSha256;`.
    # Do not accept `= TEXT("")` / `= ""` / `= NAME_None` /
    # `= false` / `= 0.f` / leftover float Health when
    # origin/main has a bare FString. Do not accept sibling
    # Category / DisplayName / SourceStatus / Sound /
    # Attenuation / Concurrency / OutputSubmix /
    # ProvenanceId fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FString\s+SourceSha256\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SourceSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+SourceSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+SourceSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+SourceSha256\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+SourceSha256\b",
        compact,
    ):
        return False
    if re.search(
...
def parse_window(header: str) -> str:
    ...
    # Start at the claimed struct so the preceding
    # `enum class ESkyguardProductionAudioCategory`
    # (Ri + fle / Ig + la tokens) and leftover
    # `enum class ESkyguardAudioSourceStatus` stay out.
    # Stop at struct FSkyguardProductionAudioRouting.
    # Never continue into leftover production-bank API
    # or leftover procedural-bank component.
    stop = header.find(STOP_BEFORE_ROUTING, start)
    if stop < 0:
        raise AssertionError(
            f"{STRUCT_NAME} parse window is missing {STOP_BEFORE_ROUTING}"
        )
    window = header[start:stop]
    if STOP_BEFORE_ROUTING in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_ROUTING}"
        )
    if STOP_BEFORE_CATEGORY_ENUM in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_CATEGORY_ENUM}"
        )
    if STOP_BEFORE_SOURCE_STATUS_ENUM in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes "
            f"{STOP_BEFORE_SOURCE_STATUS_ENUM}"
        )
    if STOP_BEFORE_PRODUCTION_BANK_API in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes "
            f"{STOP_BEFORE_PRODUCTION_BANK_API}"
        )
    if LEFTOVER_PROCEDURAL_BANK_CLASS in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes "
            f"{LEFTOVER_PROCEDURAL_BANK_CLASS}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString\s+SourceSha256\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString SourceSha256 is missing from "
...
class ProductionAudioEntrySourceSha256FieldDeclContractTests(
    unittest.TestCase
):
    def test_production_audio_entry_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_PROCEDURAL_BANK_CLASS)
        body = struct_body(header)
        ...
        self.assertIn(STOP_BEFORE_PRODUCTION_BANK_API, header)
        self.assertNotIn(STOP_BEFORE_PRODUCTION_BANK_API, section)
        self.assertNotIn(STOP_BEFORE_PRODUCTION_BANK_API, body)
    ...
    def test_missing_source_sha256_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_added_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("FString SourceSha256;", compact_origin)
        self.assertNotIn("SourceSha256 =", compact_origin)
        self.assertNotIn("SourceSha256 = true", compact_origin)
        self.assertNotIn("SourceSha256 = 0.f", compact_origin)
        self.assertNotIn("SourceSha256 = 160.f", compact_origin)

    def test_source_sha256_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString SourceSha256"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFName {SIBLING_PROVENANCE_ID};\n",
            f"\tFText {SIBLING_DISPLAY_NAME};\n",
            "\tFString SourceSha256s;\n",
            "\tint32 SourceSha256 = 1;\n",
            "\tFText SourceSha256;\n",
            "\tfloat SourceSha256 = " + forty + ";\n",
            "\tfloat SourceSha256 = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_production_audio_entry_fields(
        self,
    ) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, SIBLING_CATEGORY), section)
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_OUTPUT_SUBMIX),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_PROVENANCE_ID),
            section,
        )
        self.assertNotIn(LEFTOVER_BANK_ENTRIES, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, LOCKED_DECL)
        self.assertTrue(has_identifier(section, FIELD_NAME), section)
    ...
    def test_does_not_parse_leftover_category_enum_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_PRODUCTION_BANK_API, header)
        self.assertNotIn(STOP_BEFORE_PRODUCTION_BANK_API, section)
        self.assertNotIn(STOP_BEFORE_PRODUCTION_BANK_API, leaked)
        self.assertNotIn(STOP_BEFORE_PRODUCTION_BANK_API, window)
        self.assertIn(LEFTOVER_PRODUCTION_BANK_CLASS, header)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, section)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, leaked)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, window)
        self.assertNotIn(LEFTOVER_PROCEDURAL_BANK_CLASS, header)
        self.assertNotIn(LEFTOVER_PROCEDURAL_BANK_CLASS, section)
        self.assertNotIn(LEFTOVER_PROCEDURAL_BANK_CLASS, leaked)
        self.assertNotIn(LEFTOVER_PROCEDURAL_BANK_CLASS, window)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for banned in leftover_live_case_tokens():
            ...
            self.assertNotIn(banned, leaked)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry SourceSha256 field "
                "decl contract "
    ...
    def test_this_file_bans_live_retired_tokens_case_sensitive(self) -> None:
        file_text = this_file_text()
        section = spec_section(origin_main_header())
        window = parse_window(origin_main_header())
        for banned in leftover_live_case_tokens():
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "source_sha256_field_decl_contract.py"
        ))
        ...
        self.assertNotEqual(THIS_SCRIPT, IN_FLIGHT_DISPLAY_NAME)
        self.assertNotEqual(THIS_SCRIPT, IN_FLIGHT_PROVENANCE_ID)
        self.assertIn(
            "Scripts/tests/test_mission_result_defaults.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRODUCTION_BANK_API, locked_only)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, locked_only)
        self.assertNotIn(LEFTOVER_PROCEDURAL_BANK_CLASS, locked_only)
        self.assertNotIn(LEFTOVER_BANK_ENTRIES, locked_only)
        for helper in namespace_helper_tokens():
