...
# LockWindowAttackIntervalSeconds.
...
# then `float LockWindowAttackIntervalSeconds = 8.f;` (HAS initializer).
...
# BlueprintReadWrite IS present. ClampMin IS `"0.2"`. Default IS `= 8.f`.
# Clone wrap ClampMin WAS `"0.2"` leftover analog Approach
# AttackInterval Telegraph wrap. THIS slot ClampMin IS `"0.2"`
# (NO spaces around =). Default IS `= 8.f` not leftover analog
# `= 6.5f`. Fail-closed if wrap still has ClampMin="0.0" /
# ClampMin="100.0" / ClampMin="1.0". 8.f is NOT Harbor 40 or 80.
...
# Do NOT claim leftover analog ApproachAttackIntervalSeconds = 6.5f
# (already leftover-safe unique Telegraph, do not restack that file).
# Do NOT claim Telegraph siblings CriticalAttackIntervalSeconds = 4.8f /
...
# if ClampMin is missing, if ClampMin is still `"0.0"` / `"100.0"` /
# `"1.0"`, if the default is not `= 8.f`, if the default is leftover
# analog `= 6.5f` / leftover analog `LockWindowClimbSeconds = 2.4f` /
# leftover analog `LockWindowSpeedCmPerSecond = 850.f`, if Category is
...
# Clone wrap was leftover Amplitude
# Category="Skyguard|Boss|Encounter|Route", meta=(ClampMin="0.0")
# wrapping `float ApproachAttackIntervalSeconds = 6.5f;`
# — RETARGET hard: LOCKED_DECL from
# `float ApproachAttackIntervalSeconds = 6.5f;` to
# `float LockWindowAttackIntervalSeconds = 8.f;`.
# Default is `= 8.f` not `6.5f` / leftover analog
# `LockWindowClimbSeconds = 2.4f` / leftover
# `LockWindowSpeedCmPerSecond = 850.f`.
# Keep Category Telegraph and ClampMin `"0.2"`.
...
# LOCKED_DECL is `float LockWindowAttackIntervalSeconds = 8.f;`
# Default is `= 8.f` not leftover analog `= 6.5f` /
...
# and NOT leftover analog ApproachAttackIntervalSeconds = 6.5f
# and NOT leftover analog Telegraph siblings
# CriticalAttackIntervalSeconds / AttackTelegraphLeadSeconds /
...
# / IngressSwayAmplitudeCm / ApproachAttackIntervalSeconds = 6.5f
# AS THE LOCKED DECL.
...
# Fail-closed if ClampMin is missing, if ClampMin is still `"0.0"`
# / `"100.0"` / `"1.0"`, or default is not 8.f, or default is leftover
# analog 6.5f / leftover analog LockWindowClimbSeconds 2.4f / leftover
# analog LockWindowSpeedCmPerSecond 850.f.
...
TARGET = "float LockWindowAttackIntervalSeconds = 8.f;"
TARGET_WRONG_BARE = "float LockWindowAttackIntervalSeconds;"
TARGET_WRONG_INIT = "float LockWindowAttackIntervalSeconds = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float LockWindowAttackIntervalSeconds = true;"
TARGET_WRONG_ZERO = "float LockWindowAttackIntervalSeconds = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LockWindowAttackIntervalSeconds;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LockWindowAttackIntervalSeconds;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LockWindowAttackIntervalSeconds;"
...
STOP_BEFORE_SKYLINE_STYLE = "CriticalAttackIntervalSeconds"
...
APPROACH_ATTACK_INTERVAL_DECL_RE = re.compile(
    r"float\s+LockWindowAttackIntervalSeconds\s*=\s*8\.f\s*;"
)
APPROACH_ATTACK_INTERVAL_BARE_RE = re.compile(
    r"float\s+LockWindowAttackIntervalSeconds\s*;"
)
...
APPROACH_ATTACK_LEFTOVER_DECL_RE = re.compile(
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float LockWindowAttackIntervalSeconds = 8.f;`.
    ...
    # leftover analog ApproachAttackIntervalSeconds = 6.5f,
    ...
    # LockWindowAttackIntervalSeconds without the 8.f initializer.
    ...
    if AMPLITUDE_LEFTOVER_DECL_RE.search(compact):
        return False
    if APPROACH_ATTACK_LEFTOVER_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "LockWindowAttackIntervalSeconds":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LockWindowAttackIntervalSeconds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LockWindowAttackIntervalSeconds "
...
class PathfinderEncounterLockWindowAttackIntervalFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockWindowAttackIntervalSeconds"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LockWindowAttackIntervalSeconds"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LockWindowAttackIntervalSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lock_window_attack_interval_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin="0.2"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float LockWindowAttackIntervalSeconds;",
            compact_origin.split("LockWindowAttackIntervalSeconds = 8.f;")[0]
            + compact_origin.split("LockWindowAttackIntervalSeconds = 8.f;")[-1]
            if "LockWindowAttackIntervalSeconds = 8.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_lock_window_attack_interval_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LockWindowAttackIntervalSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("8.f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat LockWindowAttackIntervalSecondss = 8.f;\n",
            "\tint32 LockWindowAttackIntervalSeconds = 320;\n",
            "\tFName LockWindowAttackIntervalSeconds = NAME_None;\n",
            "\tfloat LockWindowAttackIntervalSeconds = " + forty + ";\n",
            "\tfloat LockWindowAttackIntervalSeconds = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockWindowAttackIntervalSeconds"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LockWindowAttackIntervalSeconds"))
        self.assertTrue(has_identifier(section, "LockWindowAttackIntervalSeconds"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="0.2"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter lock window attack interval field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_lock_window_attack_interval_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
