# THIS IS leftover-safe ASkyguardApacheAircraft MainRotor.
...
# then BARE `TObjectPtr<UStaticMeshComponent> MainRotor;`
# (no initializer).
...
# Stop BEFORE sibling TailRotor as a claimed slot.
# Do NOT claim TailRotor / RotorMast / MainRotorCross /
# RotorPower / StubWingRight / StubWingLeft / VerticalTail /
# TailBoom / Fuselage / Nose / Canopy / SilhouetteMesh /
# AircraftRoot / HoverBobCentimeters / MaxIntegrity /
# CurrentIntegrity / HullCollider / PilotCanopy.
# RotorMast is a sibling in-flight slot, not this slot.
# StubWingRight is an in-flight sibling, not this slot.
# Do NOT parse leftover analog apache-set-rotor-power
# UFUNCTION or leftover analog apache-get-rotor-rpm
# UFUNCTION as this slot.
...
# (#1556), leftover analog apache-hover-bob-centimeters
# (#1555), leftover analog apache-rotor-power-field (#1554),
# leftover analog apache-max-integrity-field (#1553),
# leftover analog apache-current-integrity-field (#1552),
# leftover analog apache-aircraft-root-field (#1557),
# leftover analog apache-nose-field (#1558), leftover analog
# apache-canopy-field (#1559), leftover analog
# apache-fuselage-field (#1560), leftover analog
# apache-tail-boom-field (#1561), leftover analog
# apache-stub-wing-left-field (#1562), leftover analog
# apache-vertical-tail-field (#1563).
# Do NOT parse leftover ProtectAsset MaxIntegrity (#1296) /
# CurrentIntegrity (#1297).
# Do NOT parse leftover LoadoutSpec HullIntegrity (#1475).
# Do NOT parse leftover TheaterKit WeatherIdentity (#1300).
...
# campaign-roster-lookup-tests (#85ab), leftover campaign-roster
# function decls.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> MainRotor;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> MainRotor;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> MainRotor = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> MainRotor = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> MainRotor = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MainRotor;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MainRotor;"
...
STOP_BEFORE_TAIL_ROTOR = "TailRotor"
...
MAIN_ROTOR_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+MainRotor\s*;"
)
MAIN_ROTOR_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+MainRotor\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover TailRotor / RotorMast / MainRotorCross /
    # VerticalTail / Fuselage / TailBoom / AircraftRoot, or an
    # initializer on MainRotor as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MAIN_ROTOR_DECL_RE.search(compact) is None:
        return False
    if MAIN_ROTOR_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_TAIL_ROTOR):
            break
    ...
    if has_identifier(window, SIBLING_TAIL_ROTOR):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_TAIL_ROTOR}"
        )
    for later in (
        SIBLING_HORIZONTAL_TAIL,
        SIBLING_MAIN_ROTOR_CROSS,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+MainRotor\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> MainRotor "
...
class ApacheMainRotorFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MainRotor"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_TAIL_ROTOR),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, SIBLING_VERTICAL_TAIL), body)
        self.assertTrue(has_identifier(body, SIBLING_HORIZONTAL_TAIL), body)
        self.assertTrue(has_identifier(body, SIBLING_STUB_WING_LEFT), body)
        self.assertTrue(has_identifier(body, SIBLING_STUB_WING_RIGHT), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MainRotor"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MainRotor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_main_rotor_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> MainRotor;", compact_origin)
        self.assertNotIn("MainRotor = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_main_rotor_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> MainRotor"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> MainRotor;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> MainRotors;\n",
            "\tint32 MainRotor = 140;\n",
            "\tbool MainRotor = true;\n",
            "\tfloat MainRotor = " + forty + ";\n",
            "\tfloat MainRotor = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_tail_rotor_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_TAIL_ROTOR))
        ...
        self.assertNotIn(TARGET_WRONG_TAIL_ROTOR, section)
        self.assertNotIn(SIBLING_FUSELAGE, LOCKED_DECL)
        self.assertNotIn(SIBLING_TAIL_BOOM, LOCKED_DECL)
        self.assertNotIn(SIBLING_VERTICAL_TAIL, LOCKED_DECL)
        self.assertNotIn(SIBLING_HORIZONTAL_TAIL, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache MainRotor field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "main_rotor_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
