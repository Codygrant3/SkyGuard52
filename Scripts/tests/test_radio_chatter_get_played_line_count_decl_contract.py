...
# wrapping `int32 GetPlayedLineCount() const { return PlayedLines; }`.
# LOCKED_DECL may be declaration-only
# `int32 GetPlayedLineCount() const;` if the matcher strips
# inline bodies, OR the full origin/main inline form.
# Accept declaration-only AND the origin/main inline body.
# Do NOT fail-closed on `{ return PlayedLines; }`.
...
# do NOT claim Speaker/Subtitle/Sound/Priority/
# EstimatedDurationSeconds/CooldownSeconds as this slot.
# leftover unique RadioLine LineId #1334 is a USTRUCT field,
# NOT this CLASS UFUNCTION. Word-boundary GetPlayedLineCount
# is NOT leftover analog LineId.
# Identifier GetPlayedLineCount is NOT leftover analog
# GetQueuedLineCount, GetDroppedLineCount, GetCurrentLineId,
# ClearQueue, PrimeLines #1713, EnqueueLine #1712,
# AdvanceRadioState #1715.
# BlueprintPure IS present. BlueprintCallable IS absent on THIS wrap.
...
# Skyguard|Pause / Skyguard|Theater /
# Skyguard|Apache without |Damage / Skyguard|Combat without
# |Sortie/|Feedback/|Safety / Skyguard|MissionMap /
...
# Do NOT parse the USTRUCT body as this slot.
...
# Clone leftover unique Validation BlueprintPure #1695 SHA
# 44ff9ffdf3359acfed757b1588de83a1a9aabe98 RETARGET method.
# Clone leftover unique BlueprintPure GetSortieHits #1685 SHA
# 4e878e0750eaed9a830a48e36e94d949f608491b RETARGET method/class/Category.
...
# FAIL-CLOSED if LOCKED_DECL is bool EnqueueLine(.
...
# Clone leftover unique GetCurrentLineId SHA
# a4921c269990776c320d67d7d0597804a28b15af RETARGET
# (FName return — keep int32 GetPlayedLineCount here).
# FAIL-CLOSED if LOCKED_DECL is FName GetCurrentLineId() const;.
# Clone leftover unique TogglePause RETARGET;
...
# RETARGET; KEEP BlueprintPure not BlueprintCallable.
...
# keep Category Skyguard|Radio and identifier GetPlayedLineCount.
# LOCKED_DECL is int32 GetPlayedLineCount() const;
...
# and NOT bool EnqueueLine(const FSkyguardRadioLine& Line); and NOT
# void AdvanceRadioState(float DeltaSeconds); and NOT
# void ClearQueue(); and NOT leftover analog int32 GetQueuedLineCount() const;
# and NOT leftover analog int32 GetDroppedLineCount() const;
# and NOT leftover analog FName GetCurrentLineId() const;
...
TARGET = "int32 GetPlayedLineCount() const;"
TARGET_SPLIT = (
    "int32 GetPlayedLineCount()\n"
    "\tconst;"
)
TARGET_INLINE = (
...
TARGET_WRONG_BARE = "int32 GetPlayedLineCount();"
TARGET_WRONG_NO_CONST = (
    "int32 GetPlayedLineCount();"
)
TARGET_WRONG_RADIO_LINE_ID = "FName LineId;"
TARGET_WRONG_QUEUED = (
    "int32 GetQueuedLineCount() const { return Queue.Num(); }"
)
TARGET_WRONG_DROPPED = "int32 GetDroppedLineCount() const;"
TARGET_WRONG_PLAYED = "FName GetCurrentLineId() const;"
...
TARGET_WRONG_ACTIVATION = "bool EnqueueLine(const FSkyguardRadioLine& Line);"
...
TARGET_WRONG_FLOAT = "float GetPlayedLineCount() const;"
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "int32 GetPlayedLineCounts() const;"
...
TARGET_WRONG_TRUE = "int32 GetPlayedLineCount = true;"
TARGET_WRONG_ZERO = "int32 GetPlayedLineCount = 0.f;"
...
UFUNCTION_PURE = LOCKED_UFUNCTION_WRAP
UFUNCTION_CALLABLE = (
...
SIBLING_FIND_ENTRY = "EnqueueLine"
SIBLING_GET_REQUIRED = "GetDroppedLineCount"
SIBLING_GET_DISPLAY = "GetCurrentLineId"
...
THIS_SCRIPT = (
    "Scripts/tests/test_radio_chatter_get_played_line_count"
    "_decl_contract.py"
)
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_radio_chatter_enqueue_line_decl_contract.py",
    "Scripts/tests/test_radio_chatter_prime_lines_decl_contract.py",
    "Scripts/tests/test_radio_chatter_advance_radio_state_decl_contract.py",
    "Scripts/tests/test_radio_chatter_get_current_line_id_decl_contract.py",
    "Scripts/tests/test_radio_chatter_clear_queue_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bint32\s+GetPlayedLineCount\(\)\s*const\s*(?:;|\{)"
)
GET_PLAYED_LINE_COUNT_RE = re.compile(
    r"int32\s+GetPlayedLineCount\s*\(\s*\)\s*const\s*"
    r"(?:;|\{[^}]*return\s+PlayedLines\s*;\s*\})"
)
GET_PLAYED_LINE_COUNT_WRONG_TYPE_RE = re.compile(
    r"(?:float|FName|bool|void|FLinearColor)\s+"
    r"GetPlayedLineCount\b"
)
GET_PLAYED_LINE_COUNT_NO_CONST_RE = re.compile(
    r"int32\s+GetPlayedLineCount\s*\(\s*\)\s*;"
)
ENQUEUE_LINE_IDENT_RE = re.compile(r"\bEnqueueLine\b")
GET_PLAYED_LINE_COUNT_IDENT_RE = re.compile(r"\bGetPlayedLineCount\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # int32 GetPlayedLineCount() const; OR origin/main inline
    # `{ return PlayedLines; }`.
    # Do not fail-closed on that inline body.
    # Do not accept missing const, a void/float/FName/bool
    ...
    # leftover analog GetSortieHits / bSuccess = false,
    # leftover FName WeatherIdentity, leftover analog
    # RadioLine FName LineId; field, leftover TObjectPtr
    # ProductionBank, leftover FireRate / RecoilPitch,
    # leftover Boom, leftover VegetationStartCullDistanceCm,
    # leftover analog PoolCapacity = 12, or Harbor 40 / 80.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if GET_PLAYED_LINE_COUNT_RE.search(compact) is None and (
        METHOD_SIGNATURE_RE.search(compact) is None
    ):
        return False
    if GET_PLAYED_LINE_COUNT_WRONG_TYPE_RE.search(compact):
        return False
    if GET_PLAYED_LINE_COUNT_NO_CONST_RE.search(compact) and (
        GET_PLAYED_LINE_COUNT_RE.search(compact) is None
    ):
        return False
    ...
    if re.search(r"\bint32\s+GetSortieHits\s*\(", compact):
        return False
    if re.search(r"\bFName\s+GetCurrentLineId\s*\(", compact):
        return False
    if re.search(r"\bint32\s+GetQueuedLineCount\s*\(", compact):
        return False
    if re.search(r"\bint32\s+GetDroppedLineCount\s*\(", compact):
        return False
    if re.search(r"\bFName\s+LineId\s*;", compact) and (
        GET_PLAYED_LINE_COUNT_RE.search(compact) is None
        and METHOD_SIGNATURE_RE.search(compact) is None
    ):
        return False
    if re.search(r"PoolCapacity\s*=\s*12\b", compact):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetPlayedLineCount\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for GetPlayedLineCount "
...
class RadioChatterGetPlayedLineCountDeclContractTests(
    unittest.TestCase
):
    def test_radio_chatter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "GetPlayedLineCount"),
            section,
        )
    ...
    def test_missing_get_played_line_count_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertTrue(
            GET_PLAYED_LINE_COUNT_IDENT_RE.search(LOCKED_DECL)
        )
        self.assertIsNone(
    ...
    def test_get_played_line_count_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 GetPlayedLineCountX() const;\n",
            "\tint32 GetPlayedLineCount = 1;\n",
            "\tFName LineId;\n",
            "\tint32 GetPlayedLineCount() = "
            + forty
            + ";\n",
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "GetPlayedLineCount")
        )
        self.assertTrue(
            has_identifier(section, "GetPlayedLineCount")
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_chatter_get_played_line_count"
            "_decl_contract.py"
        ))
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertIn("GetPlayedLineCount", LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "LineId"))
    ...
    def test_word_boundary_get_played_line_count_vs_siblings(
        self,
    ) -> None:
        self.assertTrue(
            has_identifier(
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "GetDroppedLineCount"))
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RADIO_LINE_ID)
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "GetPlayedLineCount"), section)
        ...
        self.assertFalse(has_identifier(section, "GetDroppedLineCount"), section)
    ...
    def test_does_not_parse_radio_line_struct_as_this_slot(self) -> None:
        ...
        self.assertTrue(has_identifier(body, "LineId"), body)
        self.assertTrue(has_identifier(section, "PlayedLines"), section)
        self.assertIn("return PlayedLines", collapsed(section))
        self.assertNotIn("CurrentLine.LineId", collapsed(section))
    ...
    def test_locked_decl_is_not_clone_ensure_toggle_hits_or_bind(
        ...
        self.assertTrue(LOCKED_DECL.startswith("int32 GetPlayedLineCount("))
