...
# mission-record payload, or lock the MinSupportedSaveVersion
# body in the .cpp.
# origin/main is one line
# (`static constexpr int32 MinSupportedSaveVersion = 1;`);
# accept that form and other split-line wraps.
# Declaration presence of this constexpr, including the
# origin/main initializer `= 1`, only.
MIN_SUPPORTED = "static constexpr int32 MinSupportedSaveVersion = 1;"
# Leftover #56–#64 plus CampaignSaveGame production files.
# This lane only adds an isolated Python MinSupportedSaveVersion
# declaration contract. Stay off CurrentSaveVersion
# (sibling in-flight), leftover campaign-save
# empty-fail-closed, leftover mission-save-record defaults
# #ac38 (FSkyguardMissionSaveRecord fields),
# MigrateCampaignSave #338 (sibling draft this wave),
...
# settings invert-look, CurrentSaveVersion, and
# MigrateCampaignSave stay sibling-only.
LOCKED_SCRIPTS = (
...
# CurrentSaveVersion is a sibling in-flight. MigrateCampaignSave
# is a sibling draft this wave (#338).
UNLOCKED_NEIGHBORS = (
...
# .cpp MinSupportedSaveVersion body / invented save-version
# runtime return, migration result, or mission-record
# payload stay unlocked. Do not invent INDEX_NONE or lock
# the cpp body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
...
class MinSupportedSaveVersionDeclContractTests(unittest.TestCase):
    def test_campaign_save_game_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, MIN_SUPPORTED), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, MIN_SUPPORTED)
        self.assertIn("MinSupportedSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, MIN_SUPPORTED))

    def test_missing_min_supported_save_version_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tstatic constexpr int32 CurrentSaveVersion = 2;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, MIN_SUPPORTED)
        self.assertIn("MinSupportedSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, MIN_SUPPORTED)
        self.assertIn("MinSupportedSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{MIN_SUPPORTED}\n"
        self.assertNotIn("UFUNCTION", MIN_SUPPORTED)
        self.assertNotIn("UFUNCTION", locked_only)
        self.assertFalse(MIN_SUPPORTED.startswith("UFUNCTION"), MIN_SUPPORTED)
    ...
    def test_neighbor_fields_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_fields, MIN_SUPPORTED)
        self.assertIn("MinSupportedSaveVersion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_static = "\tconstexpr int32 MinSupportedSaveVersion = 1;\n"
        missing_constexpr = "\tstatic int32 MinSupportedSaveVersion = 1;\n"
        wrong_type = "\tstatic constexpr int MinSupportedSaveVersion = 1;\n"
        wrong_initializer = (
            "\tstatic constexpr int32 MinSupportedSaveVersion = 2;\n"
        )
        zero_initializer = (
            "\tstatic constexpr int32 MinSupportedSaveVersion = 0;\n"
        )
        no_initializer = "\tstatic constexpr int32 MinSupportedSaveVersion;\n"
        current_save = (
            "\tstatic constexpr int32 CurrentSaveVersion = 2;\n"
        )
        current_save_as_one = (
        ...
        save_version_field = "\tint32 SaveVersion = CurrentSaveVersion;\n"
        for region in (
            missing_static,
            missing_constexpr,
            wrong_type,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, MIN_SUPPORTED)
            self.assertIn("MinSupportedSaveVersion", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_min_supported_save_version_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, MIN_SUPPORTED))
        self.assertEqual(declaration_count(section, MIN_SUPPORTED), 1)
        self.assertTrue(
            MIN_SUPPORTED.startswith("static constexpr int32 "),
            MIN_SUPPORTED,
        )
        self.assertIn("MinSupportedSaveVersion", MIN_SUPPORTED)
        self.assertTrue(MIN_SUPPORTED.endswith("= 1;"), MIN_SUPPORTED)
        self.assertIn("= 1", MIN_SUPPORTED)
        self.assertNotIn("INDEX_NONE", MIN_SUPPORTED)
        self.assertNotIn("UFUNCTION", MIN_SUPPORTED)
        self.assertNotIn("{", MIN_SUPPORTED)
        self.assertNotIn("}", MIN_SUPPORTED)
        self.assertNotIn("return ", MIN_SUPPORTED)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_name = (
            "public:\n"
            "\tstatic constexpr int32\n"
            "\tMinSupportedSaveVersion = 1;\n"
            "private:\n"
            "};\n"
        )
        wrap_type = (
            "public:\n"
            "\tstatic constexpr\n"
            "\tint32 MinSupportedSaveVersion = 1;\n"
            "private:\n"
            "};\n"
        )
        wrap_static = (
            "public:\n"
            "\tstatic\n"
            "\tconstexpr int32 MinSupportedSaveVersion = 1;\n"
            "};\n"
        )
        wrap_equals = (
            "public:\n"
            "\tstatic constexpr int32 MinSupportedSaveVersion =\n"
            "\t\t1;\n"
            "};\n"
        )
        header_wrap_name = (
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, MIN_SUPPORTED), 1)
        one_line = f"{{\npublic:\n\t{MIN_SUPPORTED}\n}}\n"
        self.assertTrue(has_declaration(one_line, MIN_SUPPORTED))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, MIN_SUPPORTED), section)
        self.assertEqual(
            require_declaration(section, MIN_SUPPORTED),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", MIN_SUPPORTED)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_migration_result(self) -> None:
        locked_only = f"{MIN_SUPPORTED}\n"
        self.assertNotIn("FSkyguardMigrationResult", MIN_SUPPORTED)
        self.assertNotIn("ESkyguardMigrationResult", MIN_SUPPORTED)
    ...
    def test_declaration_does_not_invent_save_version_runtime_return(
        ...
        locked_only = f"{MIN_SUPPORTED}\n"
        self.assertNotIn("return CurrentSaveVersion", MIN_SUPPORTED)
        self.assertNotIn("return MinSupportedSaveVersion", MIN_SUPPORTED)
        self.assertNotIn("return SaveVersion", MIN_SUPPORTED)
        ...
        self.assertNotIn("return ", MIN_SUPPORTED)
        self.assertFalse(MIN_SUPPORTED.startswith("static int32 "), MIN_SUPPORTED)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_mission_record_payload(self) -> None:
        ...
        for token in MISSION_SAVE_RECORD_NOT_LOCKED:
            ...
            self.assertNotIn(token, MIN_SUPPORTED)
        self.assertNotIn("BestScore", MIN_SUPPORTED)
        self.assertNotIn("BestMedalTier", MIN_SUPPORTED)
        self.assertNotIn("BestCompletionTimeSeconds", MIN_SUPPORTED)
        self.assertNotIn("bCompleted", MIN_SUPPORTED)
    ...
    def test_contract_does_not_lock_min_supported_save_version_cpp_body(
        self,
    ) -> None:
        locked_only = f"{MIN_SUPPORTED}\n"
        self.assertNotIn("{", MIN_SUPPORTED)
        ...
        self.assertNotIn("SkyguardCampaignSaveGame.cpp", MIN_SUPPORTED)
        self.assertNotIn("SkyguardCampaignSaveGame.cpp", locked_only)
        self.assertNotIn("already-v2", MIN_SUPPORTED)
        self.assertNotIn("Identity migrate", MIN_SUPPORTED)
        self.assertNotIn("NewObject", MIN_SUPPORTED)

    def test_contract_does_not_relock_current_save_version(self) -> None:
        locked_only = f"{MIN_SUPPORTED}\n"
        for neighbor in CURRENT_SAVE_VERSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MIN_SUPPORTED)
        self.assertNotIn("CurrentSaveVersion", MIN_SUPPORTED)
        self.assertNotIn("CurrentSaveVersion", locked_only)
        self.assertNotIn("= 2;", MIN_SUPPORTED)
    ...
    def test_contract_does_not_relock_migrate_campaign_save(self) -> None:
        ...
        for neighbor in MIGRATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MIN_SUPPORTED)
        self.assertNotIn("MigrateCampaignSave", MIN_SUPPORTED)
    ...
    def test_contract_does_not_relock_save_fields(self) -> None:
        ...
        for neighbor in SAVE_FIELDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, MIN_SUPPORTED)
        self.assertNotIn("CampaignId", MIN_SUPPORTED)
        self.assertNotIn("MissionRecords", MIN_SUPPORTED)
        self.assertNotIn("SavedAtUtc", MIN_SUPPORTED)
        ...
        self.assertNotIn("int32 SaveVersion = CurrentSaveVersion;", MIN_SUPPORTED)
        self.assertNotIn(
            "int32 SaveVersion = CurrentSaveVersion;",
            locked_only,
        )
    ...
    def test_contract_does_not_relock_leftover_mission_save_record(
        ...
        for token in MISSION_SAVE_RECORD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, MIN_SUPPORTED)
        self.assertNotIn("FSkyguardMissionSaveRecord", MIN_SUPPORTED)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{MIN_SUPPORTED}\n"
        self.assertEqual(
            require_declaration(locked_only, MIN_SUPPORTED),
            MIN_SUPPORTED,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(MIN_SUPPORTED, "Rifle")
        self.assertNotEqual(MIN_SUPPORTED, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign MinSupportedSaveVersion contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, MIN_SUPPORTED.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", MIN_SUPPORTED)

    def test_contract_is_min_supported_save_version_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
