...
# INDEX_NONE or lock AuthoredPCGGraph construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# AuthoredPCGGraph distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep AuthoredPCGGraph
...
# RouteExclusion (#588), LandScatterBounds,
# InlandVegetationPCG (in-flight),
# ProductionLandscape (in-flight). Do not lock
# leftover bLicensedVegetationLibraryApproved /
# leftover bProductionLandscapeBound. Do not lock
# leftover environment-readiness defaults
# #6b9d / #b931. Do not lock leftover
...
# Category="Skyguard|Mission01|Environment|PCG") is
# required as present. Accept one-line and
# split-line UPROPERTY wraps. Do not lock the
# preceding comment block as the declaration.
# Parse the public class section of
# ASkyguardMission01EnvironmentDirector only.
# Category is Skyguard|Mission01|Environment|PCG, not
# Environment alone, not Landscape, not Mission02 /
# Boss / Destruction / Integration director
# Skyguard|Mission01. Specifiers are EditAnywhere +
# BlueprintReadWrite, not VisibleAnywhere /
# BlueprintReadOnly. Type is TSoftObjectPtr, not
# TObjectPtr. Stay off leftover drafts #56–#64,
# leftover Harbor #6/#8/#9, leftover theater-kit
# #59, leftover isolated-test drafts #107–#588,
...
# RadarNode, leftover named boss methods, leftover
# LifelineHunter OpticalTracker / WeaponServo /
...
UPROPERTY_MISSION01_ENVIRONMENT_PCG_VISIBLE = (
    "UPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
...
# leftover theater-kit #59, leftover #107–#588, plus
# SkyguardMission01EnvironmentDirector production
# files. This lane only adds an isolated Python
# AuthoredPCGGraph field declaration contract on
...
# (#568/#572/#574–#583), leftover briefing-widget
...
# leftover GetAircraft, sibling OceanTiles /
# BeachTiles / LandTiles / RouteExclusion /
# LandScatterBounds / InlandVegetationPCG /
# ProductionLandscape environment field contracts
# stay sibling-only.
LOCKED_SCRIPTS = (
...
LEFTOVER_LICENSED_AND_BOUND_NOT_LOCKED = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::AuthoredPCGGraph",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01AuthoredPcgGraphFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, AUTHORED_PCG_GRAPH_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, AUTHORED_PCG_GRAPH_FIELD))

    def test_missing_authored_pcg_graph_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_MISSION01_ENVIRONMENT_PCG}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_preceding_comment_block_does_not_satisfy(self) -> None:
        comment_only = f"\t/**\n\t * {GOVERNED_GRAPH_COMMENT}\n\t */\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(comment_only, AUTHORED_PCG_GRAPH_FIELD))
        self.assertNotIn(GOVERNED_GRAPH_COMMENT, AUTHORED_PCG_GRAPH_FIELD)

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_PCG, section)
        ...
        self.assertNotIn("UPROPERTY", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("VisibleAnywhere", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("BlueprintReadOnly", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("Category", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("BlueprintCallable", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertIn("Skyguard|Mission01|Environment|PCG", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertIn("PCG", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertIn("EditAnywhere", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertIn("BlueprintReadWrite", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("BlueprintReadOnly", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("EditInstanceOnly", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Landscape", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotEqual(
            UPROPERTY_MISSION01_ENVIRONMENT_PCG,
            UPROPERTY_MISSION01_ENVIRONMENT_PCG_VISIBLE,
        )
        ...
        self.assertIn("EditAnywhere", section)
        self.assertIn("BlueprintReadWrite", section)
        self.assertNotIn("Mission02", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Boss", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Destruction", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Apache", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Mission07", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Mission10", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Encounter", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn("Safety", UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotEqual(
            UPROPERTY_MISSION01_ENVIRONMENT_PCG,
            UPROPERTY_INTEGRATION,
        )
        self.assertNotIn(UPROPERTY_INTEGRATION, UPROPERTY_MISSION01_ENVIRONMENT_PCG)
        self.assertNotIn(
            'Category="Skyguard|Mission01")',
            UPROPERTY_MISSION01_ENVIRONMENT_PCG,
        )
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_MISSION01_ENVIRONMENT_PCG)
            self.assertNotIn(invented, AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUPCGGraphInterface* AuthoredPCGGraph;\n"
        )
        as_array = (
        ...
        scene = "\tTObjectPtr<USceneComponent> AuthoredPCGGraph;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> AuthoredPCGGraph;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> AuthoredPCGGraph;\n"
        )
        weak = (
            "\tTWeakObjectPtr<UPCGGraphInterface> "
            "AuthoredPCGGraph;\n"
        )
        assigned = (
            "\tTSoftObjectPtr<UPCGGraphInterface> "
            "AuthoredPCGGraph = nullptr;\n"
        )
        hard = (
            "\tTObjectPtr<UPCGGraphInterface> "
            "AuthoredPCGGraph;\n"
        )
        leftover_comment = f"\t{GOVERNED_GRAPH_COMMENT}\n"
        soft = (
            "\tTSoftObjectPtr<UBoxComponent> "
            "AuthoredPCGGraph;\n"
        )
        ...
        leftover_scatter = f"\t{BEACH_TILES_FIELD}\n"
        ...
        leftover_graph = f"\t{LAND_SCATTER_BOUNDS_FIELD}\n"
        leftover_licensed = f"\t{LICENSED_VEGETATION_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, AUTHORED_PCG_GRAPH_FIELD)
            self.assertIn("AuthoredPCGGraph", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_authored_pcg_graph_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, AUTHORED_PCG_GRAPH_FIELD),
            AUTHORED_PCG_GRAPH_FIELD,
        )
        self.assertTrue(has_declaration(section, AUTHORED_PCG_GRAPH_FIELD))
        self.assertEqual(
            declaration_count(section, AUTHORED_PCG_GRAPH_FIELD),
            1,
        )
        self.assertTrue(
            AUTHORED_PCG_GRAPH_FIELD.startswith(
                "TSoftObjectPtr<UPCGGraphInterface> "
            ),
            AUTHORED_PCG_GRAPH_FIELD,
        )
        self.assertTrue(AUTHORED_PCG_GRAPH_FIELD.endswith(";"), AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn(
            "UPCGGraphInterface",
            AUTHORED_PCG_GRAPH_FIELD,
        )
        self.assertTrue(
            AUTHORED_PCG_GRAPH_FIELD.startswith("TSoftObjectPtr<"),
            AUTHORED_PCG_GRAPH_FIELD,
        )
        self.assertNotIn("TArray<", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("TObjectPtr<UPCGGraphInterface>", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("UBoxComponent", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("=", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("nullptr", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("INDEX_NONE", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("UFUNCTION", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("{", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("}", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("return ", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("OceanTiles", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("LandTiles", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("RouteExclusion", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("bRouteExclusionValid", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("BeachTiles", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("InlandVegetationPCG", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("ProductionLandscape", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("LandScatterBounds", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("bLicensedVegetationLibraryApproved", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("bProductionLandscapeBound", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("Root", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetReadiness", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("Briefing", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("AudioDirector", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("RadioChatter", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("SortiePresentation", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("CampaignDefinition", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("MissionDefinition", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("bAutoInitialize", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetAircraft", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("ConfigureFromMission", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("AdvanceBriefing", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("SetAssetsReady", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("CanLaunch", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetElapsedSeconds", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetBriefingState", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetBriefingText", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetRadioChatter", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetPresentation", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("GetMissionTitle", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("AcknowledgeBriefing", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("LaunchSortie", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("HullCollider", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("OpticalTracker", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("WeaponServo", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("CountermeasurePod", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("RadarNode", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("HarborIndustrial", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("MaxIntegrity", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("CurrentIntegrity", AUTHORED_PCG_GRAPH_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_PCG}\n"
            f"\t{AUTHORED_PCG_GRAPH_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_PCG} {AUTHORED_PCG_GRAPH_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{AUTHORED_PCG_GRAPH_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, AUTHORED_PCG_GRAPH_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, AUTHORED_PCG_GRAPH_FIELD),
            section,
    ...
    def test_assigned_authored_pcg_graph_does_not_satisfy(self) -> None:
        assigned = (
            "\tTSoftObjectPtr<UPCGGraphInterface> "
            "AuthoredPCGGraph = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, AUTHORED_PCG_GRAPH_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("AuthoredPCGGraph", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, AUTHORED_PCG_GRAPH_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{AUTHORED_PCG_GRAPH_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            AUTHORED_PCG_GRAPH_FIELD.startswith("UFUNCTION"),
            AUTHORED_PCG_GRAPH_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", AUTHORED_PCG_GRAPH_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_authored_pcg_graph_cpp_body(self) -> None:
        locked_only = f"{AUTHORED_PCG_GRAPH_FIELD}\n"
        self.assertNotIn("{", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("}", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("return ", AUTHORED_PCG_GRAPH_FIELD)
        ...
        self.assertNotIn("return false", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("return true", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("CreateDefaultSubobject", AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn("AuthoredPCGGraph", AUTHORED_PCG_GRAPH_FIELD)
        self.assertTrue(
            AUTHORED_PCG_GRAPH_FIELD.startswith(
                "TSoftObjectPtr<UPCGGraphInterface> "
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("CurrentIntegrity", AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, AUTHORED_PCG_GRAPH_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{AUTHORED_PCG_GRAPH_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, AUTHORED_PCG_GRAPH_FIELD),
            AUTHORED_PCG_GRAPH_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AUTHORED_PCG_GRAPH_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, AUTHORED_PCG_GRAPH_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 AuthoredPCGGraph field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{AUTHORED_PCG_GRAPH_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, AUTHORED_PCG_GRAPH_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                AUTHORED_PCG_GRAPH_FIELD.lower(),
                "mission01 AuthoredPCGGraph contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, AUTHORED_PCG_GRAPH_FIELD)

    def test_contract_is_authored_pcg_graph_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("AuthoredPCGGraph", AUTHORED_PCG_GRAPH_FIELD)
        self.assertTrue(AUTHORED_PCG_GRAPH_FIELD.endswith(";"))
        self.assertNotIn("=", AUTHORED_PCG_GRAPH_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_PCG, section)

    def test_sibling_environment_fields_do_not_satisfy_authored_pcg_graph(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, AUTHORED_PCG_GRAPH_FIELD))
            self.assertNotEqual(AUTHORED_PCG_GRAPH_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_authored_pcg_graph(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
