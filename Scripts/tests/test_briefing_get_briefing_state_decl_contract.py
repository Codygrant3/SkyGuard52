...
# or lock the GetBriefingState body.
...
# This lane only adds an isolated Python GetBriefingState
# declaration contract. Stay off ConfigureFromMission
# (draft #366), SetAssetsReady (sibling draft),
# AdvanceBriefing (sibling draft), AcknowledgeAndLaunch
# (in-flight), CanLaunch (sibling in-flight this wave),
...
# and leftover CanLaunch stay sibling-only.
LOCKED_SCRIPTS = (
...
# .cpp GetBriefingState body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline `return State` is accepted
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardMissionBriefingComponent::GetBriefingState",
    "SkyguardMissionBriefingComponent.cpp",
)
SIBLING_TYPES = (
...
class BriefingGetBriefingStateDeclContractTests(unittest.TestCase):
    def test_briefing_component_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_BRIEFING_STATE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_BRIEFING_STATE)
        self.assertIn("GetBriefingState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_BRIEFING_STATE))

    def test_missing_get_briefing_state_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tUSkyguardMissionBriefingComponent();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_BRIEFING_STATE)
        self.assertIn("GetBriefingState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_BRIEFING_STATE)
        self.assertIn("GetBriefingState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_BRIEFING_STATE)
        self.assertNotIn("UFUNCTION", GET_BRIEFING_STATE)
        self.assertNotIn("Category", GET_BRIEFING_STATE)
        self.assertNotIn("BlueprintCallable", GET_BRIEFING_STATE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_BRIEFING_STATE)
        self.assertIn("GetBriefingState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tESkyguardMissionBriefingState GetBriefingState();\n"
        wrong_return_void = "\tvoid GetBriefingState() const;\n"
        wrong_return_bool = "\tbool GetBriefingState() const;\n"
        added_arg = (
            "\tESkyguardMissionBriefingState GetBriefingState("
            "bool bReady) const;\n"
        )
        wrong_name = (
            "\tESkyguardMissionBriefingState GetBriefingText() const;\n"
        )
        for region in (
            missing_const,
            wrong_return_void,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_BRIEFING_STATE)
            self.assertIn("GetBriefingState", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_briefing_state_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_BRIEFING_STATE),
            GET_BRIEFING_STATE,
        )
        self.assertTrue(has_declaration(section, GET_BRIEFING_STATE))
        self.assertEqual(
            declaration_count(section, GET_BRIEFING_STATE),
            1,
        )
        self.assertTrue(
            GET_BRIEFING_STATE.startswith("ESkyguardMissionBriefingState "),
            GET_BRIEFING_STATE,
        )
        self.assertTrue(
            GET_BRIEFING_STATE.endswith(";"),
            GET_BRIEFING_STATE,
        )
        self.assertIn("GetBriefingState()", GET_BRIEFING_STATE)
        self.assertIn(" const", GET_BRIEFING_STATE)
        self.assertIn("ESkyguardMissionBriefingState", GET_BRIEFING_STATE)
        self.assertNotIn("INDEX_NONE", GET_BRIEFING_STATE)
        self.assertNotIn("{", GET_BRIEFING_STATE)
        self.assertNotIn("}", GET_BRIEFING_STATE)
        self.assertNotIn("return ", GET_BRIEFING_STATE)
        self.assertNotIn("return State", GET_BRIEFING_STATE)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_BRIEFING_STATE}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_BRIEFING_STATE))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_BRIEFING_STATE),
            GET_BRIEFING_STATE,
        )
        exact_inline = (
            "ESkyguardMissionBriefingState GetBriefingState() "
        ...
        self.assertTrue(has_declaration(exact_inline, GET_BRIEFING_STATE))
        self.assertEqual(
            require_declaration(exact_inline, GET_BRIEFING_STATE),
            GET_BRIEFING_STATE,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_BRIEFING_STATE),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{GET_BRIEFING_STATE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_BRIEFING_STATE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_BRIEFING_STATE),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_BRIEFING_STATE)
        self.assertNotIn("return State", GET_BRIEFING_STATE)
        self.assertNotIn("return Other", GET_BRIEFING_STATE)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_BRIEFING_STATE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_BRIEFING_STATE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_BRIEFING_STATE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_briefing_state_body(self) -> None:
        locked_only = f"{GET_BRIEFING_STATE}\n"
        self.assertNotIn("{", GET_BRIEFING_STATE)
        self.assertNotIn("}", GET_BRIEFING_STATE)
        self.assertNotIn("return ", GET_BRIEFING_STATE)
        self.assertNotIn("return State", GET_BRIEFING_STATE)
        self.assertNotIn("return State", locked_only)
        self.assertNotIn(
            "USkyguardMissionBriefingComponent::GetBriefingState",
            GET_BRIEFING_STATE,
        )
        ...
        self.assertNotIn("return false", GET_BRIEFING_STATE)
        self.assertNotIn("return true", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("ConfigureFromMission", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_set_assets_ready(self) -> None:
        ...
        for neighbor in SET_ASSETS_READY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("SetAssetsReady", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_advance_briefing(self) -> None:
        ...
        for neighbor in ADVANCE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("AdvanceBriefing", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_acknowledge_and_launch(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_AND_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("AcknowledgeAndLaunch", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_can_launch(self) -> None:
        ...
        for neighbor in CAN_LAUNCH_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("CanLaunch", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_get_elapsed_seconds(self) -> None:
        ...
        for neighbor in GET_ELAPSED_SECONDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("GetElapsedSeconds", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_get_minimum_warmup_seconds(self) -> None:
        ...
        for neighbor in GET_MINIMUM_WARMUP_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("GetBriefingText", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_get_radio_chatter(self) -> None:
        ...
        for neighbor in GET_RADIO_CHATTER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("GetRadioChatter", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_BRIEFING_STATE)
        self.assertNotIn("Unconfigured", GET_BRIEFING_STATE)
        self.assertNotIn("Warming", GET_BRIEFING_STATE)
        self.assertNotIn("Launched", GET_BRIEFING_STATE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_BRIEFING_STATE,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_BRIEFING_STATE)
        self.assertNotIn("FSkyguardBriefingCard", GET_BRIEFING_STATE)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_BRIEFING_STATE)
        self.assertNotIn("FillResultCombatStats", GET_BRIEFING_STATE)
        self.assertNotIn("ASkyguardGunner", GET_BRIEFING_STATE)
        self.assertNotIn("FillAndFinalize", GET_BRIEFING_STATE)
        self.assertNotIn("FillAndFail", GET_BRIEFING_STATE)
        self.assertNotIn("ApplyHydraForClusters", GET_BRIEFING_STATE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_BRIEFING_STATE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_BRIEFING_STATE),
            GET_BRIEFING_STATE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing GetBriefingState contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_BRIEFING_STATE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing GetBriefingState contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_BRIEFING_STATE)

    def test_contract_is_get_briefing_state_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_BRIEFING_STATE.endswith(";"))
        self.assertIn(" const", GET_BRIEFING_STATE)
        self.assertIn(UFUNCTION_BRIEFING, section)
