# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot DestroyedSystems.
...
# not TArray<ESkyguardPatrolShipSystem>, or an initializer
# is added.
# Do not claim sibling fields: bValid, bWon, MissionTitle,
# OutcomeNarrative, Score, Medal, ShotsFired, Hits,
# CargoPercent, bRadarDead, SelectedLoadout, CannonReady,
...
# isolated #1447-#1450.
# Do not parse leftover patrol-ship-system-enum #11a8.
...
# OutcomeNarrative #1454 / Medal #1455 / Score #1456 /
# CargoPercent #1457 / Hits #1458 / ShotsFired #1459.
...
TARGET = "TArray<ESkyguardPatrolShipSystem> DestroyedSystems;"
TARGET_WRONG_INIT_EMPTY = (
    "TArray<ESkyguardPatrolShipSystem> DestroyedSystems = {};"
)
TARGET_WRONG_FALSE = (
    "TArray<ESkyguardPatrolShipSystem> DestroyedSystems = false;"
)
TARGET_WRONG_TRUE = (
    "TArray<ESkyguardPatrolShipSystem> DestroyedSystems = true;"
)
TARGET_WRONG_ZERO = (
    "TArray<ESkyguardPatrolShipSystem> DestroyedSystems = 0;"
)
TARGET_WRONG_FLOAT = (
    "TArray<ESkyguardPatrolShipSystem> DestroyedSystems = 0.f;"
)
TARGET_WRONG_INT32 = "int32 DestroyedSystems;"
TARGET_WRONG_FNAME = "FName DestroyedSystems;"
TARGET_WRONG_BOOL = "bool DestroyedSystems;"
TARGET_WRONG_FLOAT_TYPE = "float DestroyedSystems;"
TARGET_WRONG_TARRAY_INT = "TArray<int32> DestroyedSystems;"
TARGET_WRONG_TARRAY_NAME = "TArray<FName> DestroyedSystems;"
...
STOP_BEFORE_PATROL_ENUM = "enum class ESkyguardPatrolShipSystem"
...
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_CARGO_PERCENT = (
    "Scripts/tests/test_cpg_debrief_snapshot_cargo_percent"
    "_field_decl_contract.py"
)
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_HITS = (
    "Scripts/tests/test_cpg_debrief_snapshot_hits"
    "_field_decl_contract.py"
)
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_SHOTS_FIRED = (
    "Scripts/tests/test_cpg_debrief_snapshot_shots_fired"
    "_field_decl_contract.py"
)
LEFTOVER_PATROL_SHIP_SYSTEM_ENUM = (
    "Scripts/tests/test_patrol_ship_system_enum_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `TArray<ESkyguardPatrolShipSystem> DestroyedSystems;`.
    # Do not accept an initializer, a wrong element type,
    # sibling snapshot fields, or a leftover theater
    # UPROPERTY wrap as the locked decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*ESkyguardPatrolShipSystem\s*>\s+"
        r"DestroyedSystems\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:float|bool|FName|FString|int32)\s+DestroyedSystems\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<\s*(?:int32|FName|FString|bool)\s*>\s+"
        r"DestroyedSystems\b",
        compact,
    ):
        return False
    if re.search(
...
class CpgDebriefSnapshotDestroyedSystemsFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "DestroyedSystems"), section)
    ...
    def test_capture_function_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "DestroyedSystems"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        ...
        self.assertIn("DestroyedSystems", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_destroyed_systems_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_destroyed_systems_is_bare_tarray(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
        ...
        self.assertIn("ESkyguardPatrolShipSystem", LOCKED_DECL)
        ...
        self.assertNotIn("DestroyedSystems =", compact)
        self.assertNotIn("DestroyedSystems = {}", compact)
        self.assertNotIn("DestroyedSystems = false", compact)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tTArray<int32> " + leftover_retired_primary_hits_field() + ";\n"
        )
        leftover_guided = (
            "\tTArray<int32> " + leftover_retired_guided_hits_field() + ";\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            f"\t{TARGET_WRONG_INIT_EMPTY}\n",
            ...
            f"\t{TARGET_WRONG_TARRAY_INT}\n",
            f"\t{TARGET_WRONG_TARRAY_NAME}\n",
            leftover_primary,
            leftover_guided,
            "\tTArray<ESkyguardPatrolShipSystem> DestroyedSystem;\n",
            "\tTArray<ESkyguardPatrolShipSystem> DestroyedSystemsList;\n",
            "\tTArray<ESkyguardPatrolShipSystem> DestroyedSystemCount;\n",
            "\tfloat DestroyedSystems = " + forty + ";\n",
            "\tfloat DestroyedSystems = " + eighty + ";\n",
        )
    ...
    def test_locked_decl_rejects_wrong_initializers(self) -> None:
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_INIT_EMPTY}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TARRAY_INT}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_INIT_EMPTY}\n", LOCKED_DECL)
        self.assertIn("DestroyedSystems", str(raised.exception))

    def test_exact_identifier_vs_sibling_snapshot_fields(self) -> None:
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "DestroyedSystems"), section)
        self.assertIn("DestroyedSystems", LOCKED_DECL)
        self.assertIn("TArray<ESkyguardPatrolShipSystem>", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TARRAY_INT)
        self.assertTrue(
            has_one_declaration(section, "int32 ShotsFired = 0;"),
            section,
        )
        self.assertTrue(
            has_one_declaration(section, "int32 Hits = 0;"),
            section,
        )
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tTArray<ESkyguardPatrolShipSystem>\n\tDestroyedSystems;\n",
            "\tTArray<ESkyguardPatrolShipSystem>   DestroyedSystems;\n",
            "\tTArray<ESkyguardPatrolShipSystem>\tDestroyedSystems;\n",
            f"\t{LOCKED_DECL}\n",
            "TArray<ESkyguardPatrolShipSystem> DestroyedSystems ;",
    ...
    def test_parse_window_excludes_leftover_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PATROL_ENUM, section)
        self.assertNotIn(STOP_BEFORE_PATROL_ENUM, leaked)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot DestroyedSystems field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "destroyed_systems_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_CARGO_PERCENT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_HITS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_SHOTS_FIRED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PATROL_SHIP_SYSTEM_ENUM, LOCKED_SCRIPTS)
