...
# INDEX_NONE or lock AdvanceHoistWindow
# construction in the .cpp. This is leftover-safe
# Mission08 Hoist AdvanceHoistWindow
...
# #708, NOT leftover Mission05 StartNextWave
# #668, NOT leftover Mission03 / Mission04 /
# Mission06 StartNextWave, NOT leftover Mission08
...
# BindRuntimeActors / NotifyThreatDestroyed /
...
# (`bool AdvanceHoistWindow(float DeltaSeconds,
# bool bCoverMaintained);`); accept that
...
# Waves / Safety / Rescue, not Environment,
...
# not leftover Mission08 Waves, not leftover
# Harbor Mission02.
...
# Do not reopen open drafts #674–#731.
...
# an isolated Python AdvanceHoistWindow
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission08IntegrationDirector::AdvanceHoistWindow",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission08AdvanceHoistWindowDeclContractTests(unittest.TestCase):
    def test_mission08_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ADVANCE_HOIST_WINDOW),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ADVANCE_HOIST_WINDOW)
        self.assertIn("AdvanceHoistWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ADVANCE_HOIST_WINDOW)
    ...
    def test_missing_initialize_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ADVANCE_HOIST_WINDOW)
        self.assertIn("AdvanceHoistWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ADVANCE_HOIST_WINDOW)
        self.assertIn("AdvanceHoistWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintCallable", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("Category", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("BlueprintPure", UFUNCTION_HOIST)
        ...
        self.assertIn("BlueprintCallable", UFUNCTION_HOIST)
        ...
        self.assertNotIn("Rescue", UFUNCTION_HOIST)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_HOIST)
            self.assertNotIn(invented, ADVANCE_HOIST_WINDOW)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ADVANCE_HOIST_WINDOW)
        self.assertIn("AdvanceHoistWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid AdvanceHoistWindow("
            "float DeltaSeconds, bool bCoverMaintained);\n"
        )
        as_const = (
            "\tbool AdvanceHoistWindow("
            "float DeltaSeconds, bool bCoverMaintained) const;\n"
        )
        with_arg = "\tbool AdvanceHoistWindow(float DeltaSeconds);\n"
        renamed = "\tbool AdvanceHoist();\n"
        short_name = "\tbool HoistWindow();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ADVANCE_HOIST_WINDOW)
            self.assertIn("AdvanceHoistWindow", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_initialize_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ADVANCE_HOIST_WINDOW))
        self.assertEqual(
            declaration_count(section, ADVANCE_HOIST_WINDOW),
            1,
        )
        self.assertTrue(
            ADVANCE_HOIST_WINDOW.startswith("bool "),
            ADVANCE_HOIST_WINDOW,
        )
        self.assertTrue(
            ADVANCE_HOIST_WINDOW.endswith(";"),
            ADVANCE_HOIST_WINDOW,
        )
        self.assertIn("AdvanceHoistWindow", ADVANCE_HOIST_WINDOW)
        self.assertIn("(float DeltaSeconds, bool bCoverMaintained)", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("TArray<", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("TObjectPtr", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("TSoftObjectPtr", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("INDEX_NONE", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("UFUNCTION", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("{", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("}", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("return ", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("const;", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("Root", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("Briefing", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AudioDirector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("RadioChatter", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("SortiePresentation", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("CampaignDefinition", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("MissionDefinition", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("Readiness", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("bAutoInitialize", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("bAllowBoundedActorSpawning", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("bAutoLaunchAfterBriefing", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("PathfinderSpawnLocation", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetAircraft", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("BindRuntimeActors", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("HandleDroneCityImpact", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("InitializePlayableMission", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifyThreatDestroyed", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AdvanceConvoyByDistance", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifyConvoyDamage", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetWaveState", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetConvoyRouteState", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetDayBeatKit", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetNightBeatKit", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("StartSearchlightWindow", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AdvanceSearchlightTrack", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifySubstationDamage", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetRemainingThreatsInWave", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetSearchlightRuntime", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetSubstationIntegrity", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("StartPayloadWindow", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AdvancePayloadWindow", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("TryJamActivePayload", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifyAirfieldTargetDamage", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetPayloadWindow", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetTargetRuntime", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetSurvivingTargetCount", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetDayBeatKind", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetDayBeatIndex", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("TickDayBeatKit", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ESkyguardMission06WaveState", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ESkyguardMission07WaveState", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ESkyguardSearchSector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("FSkyguardSearchTrackRuntime", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ClassifyFalseTrack", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ConfirmRadarGhostIdentification", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifyProtectedTargetDamage", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AdvanceReinforcementTimer", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetSearchSector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetClassifiedFalseTrackCount", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("IsHostileContactConfirmed", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetProtectedTarget", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetReinforcementTimeRemaining", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetNightBeatKind", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetNightBeatIndex", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("TickNightBeatKit", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ESkyguardAirfieldTarget", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ConvoyRuntimeAnchor", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("RoadHunterSpawnLocation", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ConfigureMissionDefinition", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifyObjectiveProgress", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifyProtectedAssetFailed", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("SynchronizeRuntimeState", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("IsCorePlayableReady", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetReadiness", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetObjectiveRuntime", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetGunner", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetPathfinder", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetMissionId", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ValidateMissionContract", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ConfigureFromMission", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AdvanceBriefing", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("SetAssetsReady", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AcknowledgeAndLaunch", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("CanLaunch", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetElapsedSeconds", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetBriefingState", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetMinimumWarmupSeconds", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetBriefingText", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetRadioChatter", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetPresentation", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("GetMissionTitle", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("AcknowledgeBriefing", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("LaunchSortie", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("HullCollider", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("OpticalTracker", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("WeaponServo", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("CountermeasurePod", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("MinHeightFromOriginCm", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("RadarNode", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ESkyguardMission02WaveState", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("HarborIndustrial", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("MaxIntegrity", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("CurrentIntegrity", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("FillAndFinalize", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("FillAndFail", ADVANCE_HOIST_WINDOW)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ADVANCE_HOIST_WINDOW)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tAdvanceHoistWindow(float DeltaSeconds, bool bCoverMaintained);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_HOIST} {ADVANCE_HOIST_WINDOW}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ADVANCE_HOIST_WINDOW}\n}}\n"
        self.assertTrue(has_declaration(one_line, ADVANCE_HOIST_WINDOW))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADVANCE_HOIST_WINDOW),
            section,
    ...
    def test_environment_category_does_not_satisfy_waves(self) -> None:
        ...
        self.assertNotEqual(leftover_m07_waves, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_m08_waves, UFUNCTION_HOIST)
        self.assertNotEqual(leftover_safety, UFUNCTION_HOIST)
        ...
        self.assertNotIn(leftover_m07_waves, UFUNCTION_HOIST)
        self.assertNotIn(leftover_m08_waves, UFUNCTION_HOIST)
        self.assertNotIn(leftover_safety, UFUNCTION_HOIST)
        self.assertNotIn("Mission01", UFUNCTION_HOIST)
        self.assertNotIn("Mission03", UFUNCTION_HOIST)
        self.assertNotIn("Mission04", UFUNCTION_HOIST)
        self.assertNotIn("Mission05", UFUNCTION_HOIST)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{ADVANCE_HOIST_WINDOW}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, ADVANCE_HOIST_WINDOW)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ADVANCE_HOIST_WINDOW)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_lock_initialize_cpp_body(self) -> None:
        ...
        self.assertNotIn("return false", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("return true", ADVANCE_HOIST_WINDOW)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{ADVANCE_HOIST_WINDOW}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_HOIST_WINDOW)
        self.assertIn("AdvanceHoistWindow", ADVANCE_HOIST_WINDOW)
        self.assertTrue(ADVANCE_HOIST_WINDOW.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ADVANCE_HOIST_WINDOW)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, ADVANCE_HOIST_WINDOW)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("CurrentIntegrity", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("SkyguardApacheAircraft.h", ADVANCE_HOIST_WINDOW)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ADVANCE_HOIST_WINDOW)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ADVANCE_HOIST_WINDOW}\n"
        self.assertEqual(
            require_declaration(locked_only, ADVANCE_HOIST_WINDOW),
            ADVANCE_HOIST_WINDOW,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_HOIST_WINDOW)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", ADVANCE_HOIST_WINDOW)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission08 AdvanceHoistWindow method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ADVANCE_HOIST_WINDOW}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ADVANCE_HOIST_WINDOW.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ADVANCE_HOIST_WINDOW.lower(),
                "mission08 AdvanceHoistWindow contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ADVANCE_HOIST_WINDOW)

    def test_contract_is_advance_hoist_window_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(ADVANCE_HOIST_WINDOW.startswith("bool "))
        self.assertIn("AdvanceHoistWindow", ADVANCE_HOIST_WINDOW)
        self.assertIn("float DeltaSeconds", ADVANCE_HOIST_WINDOW)
        self.assertIn("bool bCoverMaintained", ADVANCE_HOIST_WINDOW)
        self.assertTrue(ADVANCE_HOIST_WINDOW.endswith(";"))
        self.assertIn(UFUNCTION_HOIST, section)

    def test_sibling_director_fields_do_not_satisfy_advance_hoist_window(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, ADVANCE_HOIST_WINDOW)
            )
            self.assertNotEqual(ADVANCE_HOIST_WINDOW, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("AdvanceHoistWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, ADVANCE_HOIST_WINDOW))
        locked_only = f"{ADVANCE_HOIST_WINDOW}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool AdvanceHoistWindow(float DeltaSeconds, bool bCoverMaintained) { return false; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission08_advance_hoist_window"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_mission08_wave_state_enum(
        ...
        self.assertNotIn("ESkyguardMission08WaveState", ADVANCE_HOIST_WINDOW)
    ...
    def test_sibling_mission08_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("ConfigureMissionDefinition", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("NotifyThreatDestroyed", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("StartHoistWindow", ADVANCE_HOIST_WINDOW)
        self.assertNotIn("StartNextWave", ADVANCE_HOIST_WINDOW)
