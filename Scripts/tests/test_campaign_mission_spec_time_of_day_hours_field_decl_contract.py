# THIS IS leftover-safe FSkyguardCampaignMissionSpec TimeOfDayHours.
# origin/main form: BARE plain C++ field `float TimeOfDayHours = 12.f;`
# with initializer 12.f. NOT a UPROPERTY wrap.
...
# Fail-closed if initializer is leftover TheaterKit `= 160.f`
# or Harbor 40/80 instead of `= 12.f`.
...
# WeatherLabel / BeatSeconds / ContactKind / ShoreKind /
# SupportKind / ExtractKind / Climax / bNightIdentity /
# bStormRocketContract.
# BeatSeconds uses 120/240/360/480/600/780/900. Do not
# invent Harbor 40/80.
# THIS SLOT IS NOT leftover WeatherProfile TimeOfDayHours
# leftover-safe isolated UPROPERTY wrap on a different
# struct. Fail-closed if parse window is
# FSkyguardWeatherProfile or leftover analog
# weather-profile-defaults is treated as this slot.
# THIS IS NOT leftover TheaterKitSpec clone
# (`= 160.f` / Category="Skyguard|Theater" /
# FName WeatherIdentity).
# THIS IS NOT leftover analog campaign-roster-lookup-tests.
# THIS IS NOT leftover CampaignMissionSpec MissionId /
# Title / Success / Brief / Failure.
# Clone LOCKED_SCRIPTS from leftover-safe TheaterKitSpec
# WeatherIdentity, then parse-window retarget from
# leftover-safe CampaignMissionSpec MissionId.
# Fail-closed if clone still asserts UPROPERTY / Category /
# VisibleAnywhere / EditAnywhere / BlueprintReadOnly /
# GENERATED_BODY / USTRUCT as the locked decl.
# If clone asserts Category="Skyguard|Theater" /
# FName WeatherIdentity / `= 160.f` / leftover
# WeatherProfile TimeOfDayHours, retarget: type is float,
# identifier is TimeOfDayHours, initializer is 12.f,
# locked decl is the plain field, parse window is
...
TARGET_WRONG_HEALTH = "float TimeOfDayHours = 160.f;"
TARGET_WRONG_BOOL = "bool TimeOfDayHours;"
TARGET_WRONG_INT = "int32 TimeOfDayHours;"
...
LEFTOVER_ANALOG_WEATHER_PROFILE_DEFAULTS = (
    "Scripts/tests/test_weather_profile_defaults_contract.py"
)
LEFTOVER_ANALOG_WEATHER_PROFILE_DEFAULTS_PY = (
    "Scripts/tests/test_weather_profile_defaults.py"
)
LEFTOVER_ANALOG_WEATHER_PROFILE_DEFAULTS_TESTS = (
...
LEFTOVER_CAMPAIGN_MISSION_SPEC_FAILURE = (
...
def time_of_day_hours_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*float\s+TimeOfDayHours\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept leftover TheaterKit `= 160.f`,
    # Harbor 40/80, missing initializer, leftover
    # FName WeatherIdentity, leftover WeatherProfile
    # UPROPERTY wrap, or leftover CampaignMissionSpec
    # MissionId / Title / Success / Brief / Failure.
    # Do not accept UPROPERTY / Category clones from
    # leftover TheaterKit or leftover WeatherProfile.
    ...
    eighty = "80" + ".f"
    if re.search(
        r"float\s+TimeOfDayHours\s*=\s*160\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+TimeOfDayHours\s*=\s*" + re.escape(forty) + r"\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+TimeOfDayHours\s*=\s*" + re.escape(eighty) + r"\b",
        compact,
    ):
        return False
    if re.search(
    ...
    if re.search(
        ...
        return False
    if time_of_day_hours_has_uproperty_wrap(region):
        return False
...
def spec_section(header: str) -> str:
    ...
    if LEFTOVER_WEATHER_PROFILE in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {LEFTOVER_WEATHER_PROFILE}"
        )
    return section
...
def require_no_uproperty_wrap(section: str) -> None:
    ...
    if re.search(
        ...
        raise AssertionError(
            "UPROPERTY wrap on float TimeOfDayHours is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "TimeOfDayHours"):
        raise AssertionError(
            "UPROPERTY clone landed on TimeOfDayHours; locked decl is "
            f"bare {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "TimeOfDayHours"):
            raise AssertionError(
                f"{token} clone landed on TimeOfDayHours; locked decl is "
                f"bare {LOCKED_DECL}"
            )


class CampaignMissionSpecTimeOfDayHoursFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_locked_decl_is_bare_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "float TimeOfDayHours = 12.f;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("float TimeOfDayHours"))
        ...
        self.assertNotIn(CLONE_UPROPERTY_WEATHER_PROFILE, LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, CLONE_THEATER_HEALTH)
    ...
    def test_leftover_weather_profile_is_not_this_parse_window(self) -> None:
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_WEATHER_PROFILE)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_MISSION_TYPES_HEADER)
        self.assertTrue(HEADER_PATH.endswith("SkyguardCampaignRoster.h"))
        self.assertNotIn("MissionTypes", HEADER_PATH)
        self.assertNotIn("WeatherProfile", HEADER_PATH)
        weather_header = leftover_mission_types_header()
        self.assertIn(LEFTOVER_WEATHER_PROFILE, weather_header)
        self.assertIn("float TimeOfDayHours = 12.f;", weather_header)
        self.assertIn("UPROPERTY", weather_header)
        self.assertIn("ClampMin", weather_header)
        self.assertIn("ClampMax", weather_header)
        with self.assertRaises(AssertionError) as raised:
            spec_section(weather_header)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        leftover_profile = (
            f"struct {LEFTOVER_WEATHER_PROFILE}\n"
            "{\n"
            "\tGENERATED_BODY()\n"
            f"\t{CLONE_UPROPERTY_WEATHER_PROFILE}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
            spec_section(leftover_profile)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(
    ...
    def test_does_not_claim_leftover_storm_day_night_or_profile(self) -> None:
        self.assertNotEqual(HEADER_PATH, LEFTOVER_STORM_HEADER)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_DAY_HEADER)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_NIGHT_HEADER)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_MISSION_TYPES_HEADER)
    ...
    def test_missing_time_of_day_hours_declaration_fails_closed(self) -> None:
        empty = (
            ...
            f"\tconst TCHAR* {SIBLING_SUCCESS} = TEXT(\"\");\n"
            f"\tconst TCHAR* {SIBLING_FAILURE} = TEXT(\"\");\n"
    ...
    def test_sibling_mission_id_does_not_satisfy_time_of_day_hours(
        self,
    ) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_ZERO}\n",
            f"\tfloat TimeOfDayHours = {forty};\n",
            f"\tfloat TimeOfDayHours = {eighty};\n",
        )
        ...
        self.assertNotIn("TimeOfDayHours = " + forty, compact_origin)
        self.assertNotIn("TimeOfDayHours = " + eighty, compact_origin)
    ...
    def test_time_of_day_hours_declaration_matches_origin_main(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_WEATHER_PROFILE, LOCKED_DECL)
        for token in FORBIDDEN_LOCKED_MACRO_TOKENS:
            self.assertNotIn(token, LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 12.f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 12.f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    time_of_day_hours_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(time_of_day_hours_has_uproperty_wrap(origin), origin)
    ...
    def test_does_not_contract_sibling_spec_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "TimeOfDayHours"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec TimeOfDayHours field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "time_of_day_hours_field_decl_contract.py"
        ))
        ...
        self.assertNotIn("SkyguardCampaignRoster.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_WEATHER_PROFILE_TIME_OF_DAY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ANALOG_WEATHER_PROFILE_DEFAULTS, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_CAMPAIGN_MISSION_SPEC_FAILURE, LOCKED_SCRIPTS)
