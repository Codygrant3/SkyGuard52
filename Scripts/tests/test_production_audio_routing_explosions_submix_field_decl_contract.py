# THIS IS leftover-safe FSkyguardProductionAudioRouting ExplosionsSubmix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundSubmixBase> ExplosionsSubmix;`
# (no initializer).
...
# ExteriorSubmix / WeaponsSubmix / RadioSubmix /
# CockpitSoundMix / CockpitExteriorAttenuation /
# CockpitLowPassHz.
# Use identifier bounds so sibling MasterSubmix / CockpitSubmix /
# RadioSubmix / WeaponsSubmix do not false-positive on the
# Submix substring.
# Analog leftover production-audio-routing-defaults #197 /
# #fc92 then isolated ExplosionsSubmix field decl.
# THIS IS NOT leftover MasterSubmix #1403.
# THIS IS NOT leftover CockpitSubmix #1405.
# THIS IS NOT leftover ProductionAudioEntry OutputSubmix #1401.
# THIS IS NOT leftover production-audio-audit-defaults #196 /
# #5e51.
...
TARGET = "TSoftObjectPtr<USoundSubmixBase> ExplosionsSubmix;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundSubmixBase> ExplosionsSubmix = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundSubmixBase> ExplosionsSubmix = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundSubmixBase> ExplosionsSubmix = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundSubmixBase> ExplosionsSubmix = 0.f;"
TARGET_WRONG_FLOAT = "float ExplosionsSubmix;"
TARGET_WRONG_FNAME = "FName ExplosionsSubmix;"
TARGET_WRONG_BOOL = "bool ExplosionsSubmix;"
TARGET_WRONG_INT = "int32 ExplosionsSubmix;"
TARGET_WRONG_FTEXT = "FText ExplosionsSubmix;"
TARGET_WRONG_PTR = "USoundSubmixBase* ExplosionsSubmix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> ExplosionsSubmix;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundMix> ExplosionsSubmix;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> ExplosionsSubmix;"
TARGET_WRONG_FSTRING = "FString ExplosionsSubmix;"
...
def leftover_production_audio_scripts() -> tuple[str, ...]:
    return (
        ...
        "Scripts/tests/test_objective_definition_defaults_contract.py",
        "Scripts/tests/test_objective_definition_defaults_tests.py",
        "Scripts/tests/test_objective_definition_display_name_field_decl_contract.py",
        "Scripts/tests/test_objective_definition_objective_id_field_decl_contract.py",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `= nullptr` / `= 0.f` / leftover float Health when
    # origin/main has a bare TSoftObjectPtr. Do not accept
    # sibling MasterSubmix / CockpitSubmix / RadioSubmix /
    # WeaponsSubmix.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+ExplosionsSubmix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundSubmixBase\s*>\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundMix\s*>\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+ExplosionsSubmix\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+ExplosionsSubmix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundSubmixBase> ExplosionsSubmix is missing from "
...
class ProductionAudioRoutingExplosionsSubmixFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_explosions_submix_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("ExplosionsSubmix = nullptr", compact_origin)
        self.assertNotIn("ExplosionsSubmix = false", compact_origin)
        self.assertNotIn("ExplosionsSubmix = true", compact_origin)
        self.assertNotIn("ExplosionsSubmix = 0.f", compact_origin)
        self.assertNotIn("ExplosionsSubmix = 160.f", compact_origin)

    def test_explosions_submix_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<USoundSubmixBase> ExplosionsSubmixes;\n",
            "\tint32 ExplosionsSubmix = 1;\n",
            "\tbool ExplosionsSubmix;\n",
            "\tfloat ExplosionsSubmix = " + forty + ";\n",
            "\tfloat ExplosionsSubmix = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing ExplosionsSubmix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_MASTER_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COCKPIT_SUBMIX, LOCKED_SCRIPTS)
        self.assertTrue(Path(__file__).name.endswith(
            "explosions_submix_field_decl_contract.py"
        ))
