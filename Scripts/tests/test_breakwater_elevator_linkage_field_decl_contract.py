...
# INDEX_NONE or lock ElevatorLinkage construction
# in the .cpp. This is a FIELD contract on
# ASkyguardBreakwaterBoss, not leftover
# emergency-finish methods and not leftover
# Breakwater sibling fields PortLatch /
# StarboardLatch / DecoyPods / Engine /
# DebrisPortPanel / DebrisEngine /
# DebrisStarboardPanel. Stay off leftover
# constructor and HandleWeakPointDestroyed.
# Keep Breakwater Engine distinct from leftover
# RadarGhost Engine. Keep Breakwater
# DebrisPortPanel distinct from leftover
# Tempest DebrisPortPanel. This is not a leftover
# USkyguardBossWeakPointComponent member
# contract. origin/main is a one-line field
# (`TObjectPtr<USkyguardBossWeakPointComponent>
# ElevatorLinkage;`); accept that form and other
...
# isolated-test drafts #107–#542 including
...
# Harbor wave-state enum, and leftover
# RadarGhost SignatureModulator stay sibling-only.
...
# leftover isolated-test drafts #107–#542,
# leftover searchlight-track-runtime-defaults
...
# leftover Harbor wave-state enum, leftover
# DebriefWidget isolated contracts,
...
# RadarGhost field contracts, leftover RadarGhost
# method contracts, and leftover emergency-finish
# methods stay unlocked for THIS test's primary
# lock. Leftover
# Iron Rain public methods, leftover Tempest
...
LEFTOVER_TEMPEST_DEBRIS_PORT_PANEL_CONTRACT = (
    "test_tempest_debris_port_panel_field_decl_contract.py",
)
LEFTOVER_BREAKWATER_DEBRIS_PORT_PANEL_CONTRACT = (
    "test_breakwater_debris_port_panel_field_decl_contract.py",
)
LEFTOVER_HARBOR_WAVE_STATE_NOT_LOCKED = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBreakwaterBoss::ElevatorLinkage",
    "SkyguardBreakwaterBoss.cpp",
    "SetExposed",
...
class BreakwaterElevatorLinkageFieldDeclContractTests(unittest.TestCase):
    def test_breakwater_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ELEVATOR_LINKAGE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ELEVATOR_LINKAGE_FIELD)
    ...
    def test_missing_elevator_linkage_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardBreakwaterBoss();\n"
            ...
            f"\t{RADAR_GHOST_SIGNATURE_MODULATOR_FIELD}\n"
            f"\t{DEBRIS_PORT_PANEL_FIELD}\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("VisibleAnywhere", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("BlueprintReadOnly", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("Category", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_PORT)
        self.assertNotIn("BlueprintCallable", UPROPERTY_PORT)
        self.assertIn("Skyguard|Mission02|Boss", UPROPERTY_PORT)
        self.assertIn("Mission02", UPROPERTY_PORT)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ELEVATOR_LINKAGE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardBossWeakPointComponent* ElevatorLinkage;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> ElevatorLinkage;\n"
        )
        mesh = (
            "\tTObjectPtr<UStaticMeshComponent> ElevatorLinkage;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ELEVATOR_LINKAGE_FIELD)
            self.assertIn("ElevatorLinkage", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_elevator_linkage_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ELEVATOR_LINKAGE_FIELD),
            ELEVATOR_LINKAGE_FIELD,
        )
        self.assertTrue(has_declaration(section, ELEVATOR_LINKAGE_FIELD))
        self.assertEqual(declaration_count(section, ELEVATOR_LINKAGE_FIELD), 1)
        self.assertTrue(
            ELEVATOR_LINKAGE_FIELD.startswith(
                "TObjectPtr<USkyguardBossWeakPointComponent> "
            ),
            ELEVATOR_LINKAGE_FIELD,
        )
        self.assertTrue(ELEVATOR_LINKAGE_FIELD.endswith(";"), ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", ELEVATOR_LINKAGE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            ELEVATOR_LINKAGE_FIELD,
        )
        self.assertIn("TObjectPtr<", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("TArray<", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("=", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("INDEX_NONE", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("UFUNCTION", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("{", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("}", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("return ", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("CommandAntenna", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ControlLinkage", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("NoseCamera", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisCenter", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisTail", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisNose", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisSpine", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("AdvanceEncounter", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ResetEncounterState", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("IsRouteStateSafe", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetRouteProgress", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("RadarNode", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("IssueClimbCommand", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetFinaleStage", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("IsWreckDiverted", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetGunnerMount", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("FaceWorldLocation", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("PortGuidanceArray", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisArmorPort", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetSearchlightTracked", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetContactIdentified", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetLightningExposed", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("HullCollider", ELEVATOR_LINKAGE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, ELEVATOR_LINKAGE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {ELEVATOR_LINKAGE_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, ELEVATOR_LINKAGE_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{ELEVATOR_LINKAGE_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ELEVATOR_LINKAGE_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ELEVATOR_LINKAGE_FIELD),
            section,
    ...
    def test_assigned_elevator_linkage_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "ElevatorLinkage = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ELEVATOR_LINKAGE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("ElevatorLinkage", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ELEVATOR_LINKAGE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ELEVATOR_LINKAGE_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, ELEVATOR_LINKAGE_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ELEVATOR_LINKAGE_FIELD.startswith("UFUNCTION"),
            ELEVATOR_LINKAGE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ELEVATOR_LINKAGE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ELEVATOR_LINKAGE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_elevator_linkage_cpp_body(self) -> None:
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        self.assertNotIn("{", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("}", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("return ", ELEVATOR_LINKAGE_FIELD)
        ...
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SkyguardBreakwaterBoss.cpp", locked_only)
        self.assertNotIn("return false", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("return true", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetExposed", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            ELEVATOR_LINKAGE_FIELD,
        )
        self.assertNotIn("SetBossPhase", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", ELEVATOR_LINKAGE_FIELD)

    def test_contract_does_not_relock_sibling_tempest_fields(self) -> None:
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        for token in SIBLING_TEMPEST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("PortDischargeBoom", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("StarboardDischargeBoom", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("EngineIntake", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisPortPanel", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisStarboardPanel", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisIntakePanel", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("RequiredLockStabilitySeconds", ELEVATOR_LINKAGE_FIELD)
        self.assertIn(
            "USkyguardBossWeakPointComponent",
            ELEVATOR_LINKAGE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_encounter_controller(
        ...
        for token in LEFTOVER_ENCOUNTER_CONTROLLER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("EncounterController", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_control_servo(self) -> None:
        ...
        for token in LEFTOVER_CONTROL_SERVO_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ControlServo", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_iron_rain_fields(self) -> None:
        ...
        for token in SIBLING_IRON_RAIN_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DispenserCenter", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("MaxReleasesPerBay", ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_dispenser_starboard(
        ...
        for token in LEFTOVER_DISPENSER_STARBOARD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DispenserStarboard", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_maneuver(self) -> None:
        ...
        for token in LEFTOVER_GET_MANEUVER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetManeuver", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), ELEVATOR_LINKAGE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, ELEVATOR_LINKAGE_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetSearchlightTracked", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetContactIdentified", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("OpenOrbitExposure", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_radar_ghost_fields(self) -> None:
        ...
        for token in SIBLING_RADAR_GHOST_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("RadarReceiver", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("CoolingDoor", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisPortEWPanel", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisStarboardEWPanel", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisCoolingDoor", ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_sibling_breakwater_fields(self) -> None:
        ...
        for token in SIBLING_BREAKWATER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("StarboardLatch", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DecoyPods", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("PortLatch", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisEngine", ELEVATOR_LINKAGE_FIELD)
        self.assertIn("ElevatorLinkage", ELEVATOR_LINKAGE_FIELD)

    def test_breakwater_engine_is_not_radar_ghost_engine(self) -> None:
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        self.assertNotEqual(
            "test_breakwater_engine_field_decl_contract.py",
            "test_radar_ghost_engine_field_decl_contract.py",
        ...
        for token in LEFTOVER_BREAKWATER_ENGINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("Engine", ELEVATOR_LINKAGE_FIELD)

    def test_breakwater_debris_port_panel_is_not_tempest(
        self,
    ) -> None:
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        self.assertNotEqual(
        ...
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardTempestBoss.h")
        self.assertNotEqual(CLASS_NAME, "ASkyguardTempestBoss")
        for token in LEFTOVER_TEMPEST_DEBRIS_PORT_PANEL_CONTRACT:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        for token in LEFTOVER_BREAKWATER_DEBRIS_PORT_PANEL_CONTRACT:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("DebrisPortPanel", ELEVATOR_LINKAGE_FIELD)

    def test_contract_does_not_relock_leftover_harbor_wave_state(self) -> None:
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        for token in LEFTOVER_HARBOR_WAVE_STATE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest_methods(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SetLightningExposed", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ApplyCorrectiveBankGust", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("IsLightningExposed", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetLockStabilitySeconds", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(leftover_advance_stabilized_lock(), ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(leftover_arm_break_finish(), ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(leftover_is_break_finish_armed(), ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardGunner", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SkyguardRadarNode", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("MaxIntegrity", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("CurrentIntegrity", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            ELEVATOR_LINKAGE_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ELEVATOR_LINKAGE_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ELEVATOR_LINKAGE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ELEVATOR_LINKAGE_FIELD),
            ELEVATOR_LINKAGE_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardBreakwaterBoss::ElevatorLinkage", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "breakwater ElevatorLinkage field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{ELEVATOR_LINKAGE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ELEVATOR_LINKAGE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ELEVATOR_LINKAGE_FIELD.lower(),
                "breakwater ElevatorLinkage contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ELEVATOR_LINKAGE_FIELD)

    def test_contract_is_elevator_linkage_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(ELEVATOR_LINKAGE_FIELD.endswith(";"))
        self.assertNotIn("=", ELEVATOR_LINKAGE_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
