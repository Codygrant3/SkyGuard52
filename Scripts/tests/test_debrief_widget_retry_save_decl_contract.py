...
# or lock the RetrySave body in the .cpp.
# origin/main is one line (`bool RetrySave();`);
...
# campaign-subsystem RetrySaveLastDebrief. This is not
...
# GetDebriefNarrative / GetFinalScore /
# IsProgressSaved / GetPresentationState /
# AcknowledgeDebrief, leftover BriefingWidget
...
# TravelNext / HandleDebriefKey, and sibling
...
# locking RetrySave. Stay off GetDebrief. Only leftover
...
# .cpp RetrySave body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardDebriefWidget::RetrySave",
    "SkyguardSortiePresentationWidgets.cpp",
)
...
class DebriefWidgetRetrySaveDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RetrySave", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, RETRY_SAVE), section)
        self.assertNotIn("BlueprintPure", RETRY_SAVE)
        self.assertNotIn("UFUNCTION", RETRY_SAVE)
        self.assertNotIn("Category", RETRY_SAVE)
        self.assertNotIn("BlueprintCallable", RETRY_SAVE)
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool RetrySave;\n"
        wrong_return_void = "\tvoid RetrySave();\n"
        wrong_return_int = "\tint32 RetrySave();\n"
        added_const = "\tbool RetrySave() const;\n"
        added_arg = "\tbool RetrySave(FName SlotName);\n"
        leftover_campaign = (
            "\tbool RetrySaveLastDebrief("
    ...
    def test_retry_save_declaration_matches_origin_main(self) -> None:
        ...
        self.assertIn("RetrySave()", RETRY_SAVE)
        self.assertNotIn("INDEX_NONE", RETRY_SAVE)
        self.assertNotIn("{", RETRY_SAVE)
        self.assertNotIn("}", RETRY_SAVE)
        self.assertNotIn("return ", RETRY_SAVE)
        self.assertNotIn(" const", RETRY_SAVE)
        self.assertNotIn("RetrySaveLastDebrief", RETRY_SAVE)
    ...
    def test_contract_does_not_lock_retry_save_cpp_body(self) -> None:
        ...
        self.assertNotIn("}", RETRY_SAVE)
        self.assertNotIn("return ", RETRY_SAVE)
        self.assertNotIn("USkyguardDebriefWidget::RetrySave", RETRY_SAVE)
        self.assertNotIn("SkyguardSortiePresentationWidgets.cpp", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_configure(self) -> None:
        ...
        self.assertNotIn("Configure", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("GetPresentation", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_get_debrief(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("GetDebrief", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_get_debrief_narrative(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NARRATIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("GetDebriefNarrative", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_get_final_score(self) -> None:
        ...
        for neighbor in GET_FINAL_SCORE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("GetFinalScore", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_is_progress_saved(self) -> None:
        ...
        for neighbor in IS_PROGRESS_SAVED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("IsProgressSaved", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_get_presentation_state(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("GetPresentationState", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_travel_next(self) -> None:
        ...
        for neighbor in TRAVEL_NEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("TravelNext", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_handle_debrief_key(self) -> None:
        ...
        for neighbor in HANDLE_DEBRIEF_KEY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("HandleDebriefKey", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_retry_save_last_debrief(self) -> None:
        ...
        self.assertNotIn("USkyguardCampaignSubsystem", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_briefing_widget_configure(self) -> None:
        ...
        for token in BRIEFING_WIDGET_CONFIGURE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RETRY_SAVE)
        self.assertNotIn("USkyguardBriefingWidget", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("ConfigureFromMission", RETRY_SAVE)
        self.assertNotIn("ConfigureFromMission", locked_only)
        self.assertNotIn("USkyguardMissionBriefingComponent", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_briefing_component_neighbors(
        ...
        for neighbor in (
            ...
            *GET_RADIO_CHATTER_NOT_LOCKED,
            ...
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RETRY_SAVE)
        self.assertNotIn("SetAssetsReady", RETRY_SAVE)
        self.assertNotIn("AdvanceBriefing", RETRY_SAVE)
        self.assertNotIn("AcknowledgeAndLaunch", RETRY_SAVE)
        self.assertNotIn("CanLaunch", RETRY_SAVE)
        self.assertNotIn("GetBriefingState", RETRY_SAVE)
        self.assertNotIn("GetElapsedSeconds", RETRY_SAVE)
        self.assertNotIn("GetMinimumWarmupSeconds", RETRY_SAVE)
        self.assertNotIn("GetRadioChatter", RETRY_SAVE)
        self.assertNotIn("ESkyguardMissionBriefingState", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RETRY_SAVE)
        self.assertNotIn("Unconfigured", RETRY_SAVE)
        self.assertNotIn("Warming", RETRY_SAVE)
        self.assertNotIn("Launched", RETRY_SAVE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            RETRY_SAVE,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RETRY_SAVE)
        self.assertNotIn("FSkyguardBriefingCard", RETRY_SAVE)
        self.assertNotIn("FSkyguardBriefingRadioRow", RETRY_SAVE)
        self.assertNotIn("FSkyguardHowToFlyRow", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation(
        ...
        for token in LEFTOVER_SORTIE_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RETRY_SAVE)
        self.assertNotIn("SkyguardSortiePresentationFailClosed", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation_state(
        ...
        self.assertNotIn("SortieActive", RETRY_SAVE)
        self.assertNotIn("DebriefReady", RETRY_SAVE)
        self.assertNotIn(
            "test_sortie_presentation_state_enum_contract.py",
            RETRY_SAVE,
        )
    ...
    def test_contract_does_not_relock_leftover_mission_debrief_defaults(
        ...
        for token in LEFTOVER_MISSION_DEBRIEF_DEFAULTS_NOT_LOCKED:
            ...
            self.assertNotIn(token, RETRY_SAVE)
            self.assertNotIn(token, section)
        self.assertNotIn("bNewBestScore", RETRY_SAVE)
        self.assertNotIn("bProgressSaved", RETRY_SAVE)
        self.assertNotIn("ESkyguardMissionDebriefState", RETRY_SAVE)
        self.assertNotIn(
            "test_mission_debrief_defaults_contract.py",
            RETRY_SAVE,
        )
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        self.assertNotIn("ApplyHydraForClusters", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_bind_hud_or_theater(self) -> None:
        ...
        for token in LEFTOVER_BIND_HUD_AND_THEATER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RETRY_SAVE)
        self.assertNotIn("test_bind_hud_host_presentation_tests.py", RETRY_SAVE)
        self.assertNotIn("test_campaign_theater_kit_contract.py", RETRY_SAVE)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, RETRY_SAVE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", RETRY_SAVE)
        self.assertNotIn("ESkyguardMissionSkylineStyle", RETRY_SAVE)
    ...
    def test_contract_parses_public_section_not_briefing_private_or_cpp(
        ...
        self.assertNotIn("USkyguardDebriefWidget::RetrySave", section)
    ...
    def test_contract_does_not_parse_briefing_widget(self) -> None:
        ...
        for token in BRIEFING_WIDGET_NOT_LOCKED:
            ...
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardBriefingWidget", section)
        self.assertNotIn("GetMissionTitle", RETRY_SAVE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "debrief widget RetrySave contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"debrief widget RetrySave contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, RETRY_SAVE)
    ...
    def test_contract_is_retry_save_declaration_only(self) -> None:
        ...
        self.assertTrue(RETRY_SAVE.startswith("bool "))
        self.assertTrue(RETRY_SAVE.endswith(";"))
        self.assertIn(UFUNCTION_DEBRIEF, section)
