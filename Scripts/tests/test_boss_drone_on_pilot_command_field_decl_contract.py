...
# INDEX_NONE or lock OnPilotCommand broadcasts
# in the .cpp. This is a FIELD contract for the
# OnPilotCommand delegate UPROPERTY, not leftover
# pilot-command-roster enum contract #b593, and
# not leftover Apache / BossDrone
# IssuePilotCommand methods. Lock
# field-declaration presence of
# `FSkyguardPilotCommandEvent OnPilotCommand;`.
# origin/main is a one-line field. Accept
# one-line / split-line wraps. Nearby
# origin/main
# UPROPERTY(BlueprintAssignable,
# Category="Skyguard|Boss") is required as
# present. Accept one-line and split-line
# UPROPERTY wraps. Do not lock leftover
# OnPilotCommandNative or the file-scope
# DECLARE_DYNAMIC_MULTICAST_DELEGATE as the
# primary lock. The type appears on
...
# OnBossPhaseChanged, OnPilotCommandNative.
# Stay off leftover
...
# field. Stay off leftover OnPilotCommandNative.
...
# isolated-test drafts #107–#476 including leftover
...
# Harbor 40/80.
ON_PILOT_COMMAND_FIELD = "FSkyguardPilotCommandEvent OnPilotCommand;"
UPROPERTY_COMMAND = (
    'UPROPERTY(BlueprintAssignable, '
...
# leftover isolated-test drafts #107–#476,
...
LEFTOVER_ON_PILOT_COMMAND_NATIVE_NOT_LOCKED = (
    "OnPilotCommandNative",
    "FSkyguardPilotCommandNativeEvent OnPilotCommandNative;",
)
...
# specifiers. Neighbor OnBossPhaseChanged may
...
# .cpp OnPilotCommand broadcasts / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBossDroneBase::OnPilotCommand",
    "SkyguardBossDroneBase.cpp",
...
FIELD_DECL_RE = re.compile(
    r"FSkyguardPilotCommandEvent\s+OnPilotCommand\s*;"
)


def has_declaration(region: str, declaration: str) -> bool:
    if declaration == ON_PILOT_COMMAND_FIELD:
...
class BossDroneOnPilotCommandFieldDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ON_PILOT_COMMAND_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ON_PILOT_COMMAND_FIELD)
        self.assertIn("OnPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ON_PILOT_COMMAND_FIELD)
    ...
    def test_missing_on_pilot_command_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ON_PILOT_COMMAND_FIELD)
        self.assertIn("OnPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ON_PILOT_COMMAND_FIELD)
        self.assertIn("OnPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("BlueprintAssignable", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("VisibleAnywhere", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("BlueprintReadOnly", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("Category", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_COMMAND)
        self.assertNotIn("BlueprintCallable", UPROPERTY_COMMAND)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_COMMAND)
        self.assertNotIn("BlueprintReadOnly", UPROPERTY_COMMAND)
        self.assertIn("BlueprintAssignable", UPROPERTY_COMMAND)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ON_PILOT_COMMAND_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ON_PILOT_COMMAND_FIELD)
        self.assertIn("OnPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tFSkyguardBossPhaseEvent OnPilotCommand;\n"
        box = "\tint32 OnPilotCommand;\n"
        scene = "\tFSkyguardPilotCommandEvent PilotCommand;\n"
        weak = "\tTObjectPtr<FSkyguardPilotCommandEvent> OnPilotCommand;\n"
        assigned = "\tFSkyguardPilotCommandEvent OnPilotCommand = nullptr;\n"
        leftover_native = (
            "\tFSkyguardPilotCommandNativeEvent OnPilotCommandNative;\n"
        )
        leftover_declare = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ON_PILOT_COMMAND_FIELD)
            self.assertIn("OnPilotCommand", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_on_pilot_command_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ON_PILOT_COMMAND_FIELD),
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertTrue(
            has_declaration(section, ON_PILOT_COMMAND_FIELD)
        )
        self.assertEqual(
            declaration_count(section, ON_PILOT_COMMAND_FIELD),
            1,
        )
        self.assertTrue(
            ON_PILOT_COMMAND_FIELD.startswith("FSkyguardPilotCommandEvent "),
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertTrue(
            ON_PILOT_COMMAND_FIELD.endswith(";"),
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertIn("OnPilotCommand", ON_PILOT_COMMAND_FIELD)
        self.assertIn("FSkyguardPilotCommandEvent", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("CurrentPilotCommand", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("OnPilotCommandNative", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "DECLARE_DYNAMIC_MULTICAST_DELEGATE",
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("=", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("INDEX_NONE", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("UFUNCTION", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetBossPhase", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("{", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("}", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("return ", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("ApplyWeaponHit", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("IssuePilotCommand", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            ON_PILOT_COMMAND_FIELD,
        )
        ...
        self.assertNotIn("GetDefeatDebrisPieceCount", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetTelemetry", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetGunnerMount", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("FaceWorldLocation", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("WeakPoints", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetSearchlightTracked", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetContactIdentified", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("SetLightningExposed", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            ON_PILOT_COMMAND_FIELD,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, ON_PILOT_COMMAND_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_COMMAND} {ON_PILOT_COMMAND_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ON_PILOT_COMMAND_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, ON_PILOT_COMMAND_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ON_PILOT_COMMAND_FIELD),
    ...
    def test_on_pilot_command_native_and_declare_do_not_satisfy(
        self,
    ) -> None:
        native = "\tFSkyguardPilotCommandNativeEvent OnPilotCommandNative;\n"
        declare = (
            "DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(\n"
            "\tFSkyguardPilotCommandEvent,\n"
            "\tESkyguardPilotCommand, Command);\n"
        ...
        assigned_wrong = (
            "\tFSkyguardPilotCommandEvent OnPilotCommand = nullptr;\n"
        )
        for region in (native, declare, current, assigned_wrong):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ON_PILOT_COMMAND_FIELD)
            self.assertIn("OnPilotCommand", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ON_PILOT_COMMAND_FIELD))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ON_PILOT_COMMAND_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ON_PILOT_COMMAND_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_COMMAND, section)
        self.assertTrue(has_declaration(section, ON_PILOT_COMMAND_FIELD), section)

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{ON_PILOT_COMMAND_FIELD}\n"
        self.assertNotIn("UFUNCTION", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("UFUNCTION", locked_only)
        self.assertFalse(ON_PILOT_COMMAND_FIELD.startswith("UFUNCTION"), ON_PILOT_COMMAND_FIELD)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ON_PILOT_COMMAND_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ON_PILOT_COMMAND_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_on_pilot_command_cpp_body(
        self,
    ) -> None:
        locked_only = f"{ON_PILOT_COMMAND_FIELD}\n"
        self.assertNotIn("{", ON_PILOT_COMMAND_FIELD)
        ...
        self.assertNotIn("return false", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("return true", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("SetBossPhase", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("RegisterDefeatDebris", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("CleanupDefeatDebris", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("HandleDefeated", ON_PILOT_COMMAND_FIELD)

    def test_contract_does_not_relock_get_boss_phase_sibling(self) -> None:
        locked_only = f"{ON_PILOT_COMMAND_FIELD}\n"
        for token in GET_BOSS_PHASE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetBossPhase", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_sibling_public_fields(self) -> None:
        ...
        for token in PUBLIC_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("Root", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("BodyMesh", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("WeakPoints", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("CurrentPilotCommand", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("Telemetry", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("OnBossPhaseChanged", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("OnPilotCommandNative", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_hull_collider(self) -> None:
        ...
        for token in LEFTOVER_HULL_COLLIDER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("HullCollider", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_sibling_root_field(self) -> None:
        ...
        for token in LEFTOVER_ROOT_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("TObjectPtr<USceneComponent> Root;", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "test_boss_drone_root_field_decl_contract.py",
            ON_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_on_pilot_command_native(self) -> None:
        locked_only = f"{ON_PILOT_COMMAND_FIELD}\n"
        for token in LEFTOVER_ON_PILOT_COMMAND_NATIVE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("OnPilotCommandNative", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("PortGuidanceArray", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("CommandCore", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetSearchlightTracked", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("IsSearchlightTracked", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetContactIdentified", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("IsContactIdentified", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("OpenOrbitExposure", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            ON_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            ON_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetGunnerMount", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetChinTurret", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetSensorTurret", ON_PILOT_COMMAND_FIELD)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{ON_PILOT_COMMAND_FIELD}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("ASkyguardGunner", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("FillAndFinalize", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("SkyguardRadarNode", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            ON_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            ON_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("MaxIntegrity", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("CurrentIntegrity", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            ON_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("SkyguardPathfinder", ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ON_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ON_PILOT_COMMAND_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ON_PILOT_COMMAND_FIELD),
            ON_PILOT_COMMAND_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone OnPilotCommand field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ON_PILOT_COMMAND_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ON_PILOT_COMMAND_FIELD.lower(),
                "boss drone OnPilotCommand contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ON_PILOT_COMMAND_FIELD)

    def test_contract_is_on_pilot_command_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", ON_PILOT_COMMAND_FIELD)
        self.assertNotIn("{", ON_PILOT_COMMAND_FIELD)
        self.assertTrue(ON_PILOT_COMMAND_FIELD.startswith("FSkyguardPilotCommandEvent "))
        self.assertTrue(ON_PILOT_COMMAND_FIELD.endswith(";"))
        self.assertNotIn("=", ON_PILOT_COMMAND_FIELD)
        self.assertIn(UPROPERTY_COMMAND, section)
