...
# or lock the GetPresentationState body.
...
# Accept that exact one-line prototype,
...
# CPG debrief fail-closed #8ccd. This is not leftover
# mission-debrief defaults. This is not leftover
# sortie-presentation fail-closed #7600. This is not
# leftover sortie-presentation state enum #6c40.
# Stay off GetDebrief (leftover mission-debrief
# defaults). Do not lock enum values.
...
# flare / HUD, leftover BriefingWidget
# declaration contracts, leftover DebriefWidget
# Configure / GetPresentation / GetDebrief /
# GetDebriefNarrative / GetFinalScore /
# IsProgressSaved / AcknowledgeDebrief /
# RetrySave / TravelNext / HandleDebriefKey,
# leftover mission-debrief defaults, leftover
# sortie-presentation state enum, leftover
# HarborIndustrial skyline enum (not a
# Harbor clock retune), and sibling widget
...
# GetPresentationState declaration contract. Stay off
# Configure, GetPresentation, GetDebrief (leftover
# mission-debrief defaults), GetDebriefNarrative,
# GetFinalScore, IsProgressSaved, AcknowledgeDebrief,
# RetrySave, TravelNext, and HandleDebriefKey on
# this class. Stay off leftover BriefingWidget
# GetPresentation, leftover briefing widget
...
# fail-closed #7600, leftover CPG debrief
# fail-closed #8ccd, leftover mission-debrief
# defaults, leftover Gunner helpers, leftover
# Harbor clocks, leftover theater-kit / flare /
...
# readiness, leftover USkyguardBriefingWidget, leftover
# DebriefWidget Configure / GetPresentation, leftover
# GetDebrief, leftover GetDebriefNarrative, leftover
# GetFinalScore, leftover IsProgressSaved, leftover
# AcknowledgeDebrief, leftover RetrySave, leftover
# TravelNext, leftover HandleDebriefKey, leftover
# sortie-presentation state enum #6c40, and dirty
# workspace paths.
LOCKED = {
...
# #7600, leftover CPG debrief fail-closed #8ccd,
# leftover mission-debrief defaults, leftover
...
# leftover gun-fire camera shake, leftover
# mission-weather enum, leftover briefing-component
# declaration contracts through GetRadioChatter,
# leftover BriefingWidget Configure / GetPresentation /
...
# AcknowledgeDebrief, leftover RetrySave, leftover
# TravelNext, leftover HandleDebriefKey, leftover
# sortie-presentation state enum #6c40, and
# leftover BriefingWidget stay sibling-only.
LOCKED_SCRIPTS = (
...
# here. Configure / GetPresentation / GetDebrief /
# GetDebriefNarrative / GetFinalScore /
# IsProgressSaved / AcknowledgeDebrief /
...
# Leftover BriefingWidget GetPresentation stays
# unlocked. Do not re-lock leftover BriefingWidget.
LEFTOVER_BRIEFING_WIDGET_GET_PRESENTATION_NOT_LOCKED = (
    "test_briefing_widget_get_presentation_decl_contract.py",
    "USkyguardBriefingWidget",
    "Category=\"Skyguard|Presentation|Briefing\"",
)
# Leftover DebriefWidget GetPresentation stays
# unlocked. Sibling this wave.
LEFTOVER_DEBRIEF_WIDGET_GET_PRESENTATION_NOT_LOCKED = (
    "test_debrief_widget_get_presentation_decl_contract.py",
)
# Leftover BriefingWidget helpers stay unlocked.
...
# Leftover sortie-presentation fail-closed files stay
# unlocked. Do not re-lock leftover fail-closed #7600
# without treating leftover HarborIndustrial as a
# Harbor clock retune.
...
# Leftover sortie-presentation state enum #6c40 stays
# unlocked. Do not lock enum values. Declaration
# return type is presence only.
...
# Do not lock GetDebrief.
LEFTOVER_MISSION_DEBRIEF_NOT_LOCKED = (
...
# fail-closed #8ccd / empty-capture stay unlocked.
...
# .cpp GetPresentationState body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main one-line prototype is accepted
# as presence; an inline body is accepted as
# presence, not a locked implementation contract.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "USkyguardDebriefWidget::GetPresentationState",
    "SkyguardSortiePresentationWidgets.cpp",
)
SIBLING_TYPES = (
...
def leftover_skyline_enum_token() -> str:
...
class DebriefWidgetGetPresentationStateDeclContractTests(unittest.TestCase):
    def test_debrief_widget_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_PRESENTATION_STATE),
            section,
        )
    ...
    def test_briefing_widget_does_not_satisfy_class(self) -> None:
        briefing_only = (
            ...
            "GetPresentationState() const { return Other; }\n"
            "\tFText GetMissionTitle() const;\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_PRESENTATION_STATE)
        self.assertIn("GetPresentationState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_PRESENTATION_STATE))

    def test_missing_get_presentation_state_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tvoid Configure("
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_PRESENTATION_STATE)
        self.assertIn("GetPresentationState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_PRESENTATION_STATE)
        self.assertIn("GetPresentationState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_PRESENTATION_STATE)
        self.assertNotIn("UFUNCTION", GET_PRESENTATION_STATE)
        self.assertNotIn("Category", GET_PRESENTATION_STATE)
        self.assertNotIn("BlueprintCallable", GET_PRESENTATION_STATE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_PRESENTATION_STATE)
        self.assertIn("GetPresentationState", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = (
            "\tESkyguardSortiePresentationState GetPresentationState();\n"
        )
        wrong_return_void = "\tvoid GetPresentationState() const;\n"
        wrong_return_bool = "\tbool GetPresentationState() const;\n"
        wrong_return_int = "\tint32 GetPresentationState() const;\n"
        added_arg = (
            "\tESkyguardSortiePresentationState "
            "GetPresentationState(bool bReady) const;\n"
        )
        wrong_name = "\tint32 GetFinalScore() const;\n"
        wrong_presentation = (
            "\tUSkyguardSortiePresentationComponent* "
            "GetPresentation() const;\n"
        )
        wrong_debrief = "\tFSkyguardMissionDebrief GetDebrief() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_PRESENTATION_STATE)
            self.assertIn("GetPresentationState", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_presentation_state_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, GET_PRESENTATION_STATE))
        self.assertEqual(
            declaration_count(section, GET_PRESENTATION_STATE),
            1,
        )
        self.assertTrue(
            GET_PRESENTATION_STATE.startswith(
                "ESkyguardSortiePresentationState "
            ),
            GET_PRESENTATION_STATE,
        )
        self.assertTrue(
            GET_PRESENTATION_STATE.endswith(";"),
            GET_PRESENTATION_STATE,
        )
        self.assertIn("GetPresentationState()", GET_PRESENTATION_STATE)
        self.assertIn(" const", GET_PRESENTATION_STATE)
        self.assertIn(
            "ESkyguardSortiePresentationState",
            GET_PRESENTATION_STATE,
        )
        self.assertNotIn("INDEX_NONE", GET_PRESENTATION_STATE)
        self.assertNotIn("{", GET_PRESENTATION_STATE)
        self.assertNotIn("}", GET_PRESENTATION_STATE)
        self.assertNotIn("return ", GET_PRESENTATION_STATE)
        self.assertNotIn("return Other", GET_PRESENTATION_STATE)
    ...
    def test_declaration_accepts_origin_main_inline_and_prototype(
        ...
        self.assertTrue(
            has_declaration(one_line_prototype, GET_PRESENTATION_STATE)
        )
        self.assertEqual(
            require_declaration(one_line_prototype, GET_PRESENTATION_STATE),
            GET_PRESENTATION_STATE,
        )
        exact_inline = (
            "ESkyguardSortiePresentationState GetPresentationState() "
        ...
        self.assertTrue(has_declaration(exact_inline, GET_PRESENTATION_STATE))
        self.assertEqual(
            require_declaration(exact_inline, GET_PRESENTATION_STATE),
            GET_PRESENTATION_STATE,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_PRESENTATION_STATE),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{GET_PRESENTATION_STATE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_PRESENTATION_STATE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_PRESENTATION_STATE),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_PRESENTATION_STATE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_PRESENTATION_STATE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_presentation_state_body(self) -> None:
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        self.assertNotIn("{", GET_PRESENTATION_STATE)
        self.assertNotIn("}", GET_PRESENTATION_STATE)
        self.assertNotIn("return ", GET_PRESENTATION_STATE)
        ...
        self.assertNotIn("return false", GET_PRESENTATION_STATE)
        self.assertNotIn("return true", GET_PRESENTATION_STATE)

    def test_contract_does_not_relock_constructor(self) -> None:
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        for neighbor in CONSTRUCTOR_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("ObjectInitializer", GET_PRESENTATION_STATE)
        self.assertNotIn("ObjectInitializer", locked_only)
    ...
    def test_contract_does_not_relock_configure(self) -> None:
        ...
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("Configure(", GET_PRESENTATION_STATE)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("GetPresentation()", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_get_debrief(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("GetDebrief()", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_get_debrief_narrative(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NARRATIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("GetDebriefNarrative", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_get_final_score(self) -> None:
        ...
        for neighbor in GET_FINAL_SCORE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("GetFinalScore", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_is_progress_saved(self) -> None:
        ...
        for neighbor in IS_PROGRESS_SAVED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("IsProgressSaved", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("AcknowledgeDebrief", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_retry_save(self) -> None:
        ...
        for neighbor in RETRY_SAVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("RetrySave", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_travel_next(self) -> None:
        ...
        for neighbor in TRAVEL_NEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("TravelNext", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_handle_debrief_key(self) -> None:
        ...
        for neighbor in HANDLE_DEBRIEF_KEY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("HandleDebriefKey", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_PRESENTATION_STATE)
        self.assertNotIn("Unconfigured", GET_PRESENTATION_STATE)
        self.assertNotIn("Warming", GET_PRESENTATION_STATE)
        self.assertNotIn("Launched", GET_PRESENTATION_STATE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_PRESENTATION_STATE,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PRESENTATION_STATE)
        self.assertNotIn("FSkyguardBriefingCard", GET_PRESENTATION_STATE)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_PRESENTATION_STATE)
        self.assertNotIn("FSkyguardHowToFlyRow", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_leftover_briefing_widget_get_presentation(
        self,
    ) -> None:
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        section = public_section(origin_main_header())
        body = class_body(origin_main_header())
        for token in LEFTOVER_BRIEFING_WIDGET_GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_leftover_debrief_widget_get_presentation(
        self,
    ) -> None:
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        for token in LEFTOVER_DEBRIEF_WIDGET_GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PRESENTATION_STATE)
        self.assertNotIn("GetPresentation()", GET_PRESENTATION_STATE)
        self.assertNotIn("GetPresentation()", locked_only)
    ...
    def test_contract_does_not_relock_briefing_widget(self) -> None:
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        section = public_section(origin_main_header())
        body = class_body(origin_main_header())
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PRESENTATION_STATE)
        self.assertNotIn("FillResultCombatStats", GET_PRESENTATION_STATE)
        self.assertNotIn("ASkyguardGunner", GET_PRESENTATION_STATE)
        self.assertNotIn("FillAndFinalize", GET_PRESENTATION_STATE)
        self.assertNotIn("FillAndFail", GET_PRESENTATION_STATE)
        self.assertNotIn("ApplyHydraForClusters", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_leftover_mission_debrief_defaults(
        ...
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        section = public_section(origin_main_header())
        for token in LEFTOVER_MISSION_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PRESENTATION_STATE)
            self.assertNotIn(token, section)
        self.assertNotIn("GetDebrief()", GET_PRESENTATION_STATE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_PRESENTATION_STATE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_PRESENTATION_STATE),
            GET_PRESENTATION_STATE,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("DebriefReady", section)
        self.assertNotIn("SaveFailure", section)
        self.assertNotIn("TravelReady", section)
        self.assertNotIn("TravelBlocked", section)
        self.assertNotIn("CampaignComplete", section)
        self.assertNotIn("SortieActive", section)
        self.assertNotIn("NativeConstruct", section)
        self.assertNotIn("NativeOnKeyDown", section)
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotIn(token, locked_only)
        self.assertNotIn(
            leftover_skyline_enum_token(),
            leftover_harbor_tokens(),
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "debrief widget GetPresentationState contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_PRESENTATION_STATE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"debrief widget GetPresentationState contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_PRESENTATION_STATE)

    def test_contract_is_get_presentation_state_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_PRESENTATION_STATE.endswith(";"))
        self.assertIn(" const", GET_PRESENTATION_STATE)
        self.assertIn(UFUNCTION_DEBRIEF, section)
