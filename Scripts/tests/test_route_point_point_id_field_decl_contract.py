# THIS IS leftover-safe FSkyguardRoutePoint PointId.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `FName PointId;` (no initializer).
...
# if Category= is added, or if the type is not FName.
...
# Do NOT contract sibling fields WorldLocation /
# TargetAirspeedKph / LookAheadSeconds /
# bAllowCombatOrbit.
# WorldLocation is in-flight leftover
# route-point-world-location-field-decl-contract.
# TargetAirspeedKph / LookAheadSeconds /
# bAllowCombatOrbit remain unclaimed.
...
# #1332-#1337 or in-flight CooldownSeconds.
...
# Analog: leftover route-point-defaults then this
# isolated PointId field decl, same as leftover
# radio-line-defaults then isolated RadioLine fields,
...
# retarget: type is FName PointId, no `=`, specifiers
...
TARGET = "FName PointId;"
TARGET_WRONG_EQ_NONE = "FName PointId = NAME_None;"
TARGET_WRONG_FALSE = "FName PointId = false;"
TARGET_WRONG_TRUE = "FName PointId = true;"
TARGET_WRONG_ZERO = "FName PointId = 0.f;"
TARGET_WRONG_FLOAT = "float PointId;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName PointId;`.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+PointId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName PointId is missing from "
...
def require_point_id_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} PointId UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} PointId"
        )
...
class RoutePointPointIdFieldDeclContractTests(unittest.TestCase):
    def test_route_point_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PointId"), section)
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("PointId", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_point_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_point_id_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName PointId;", compact_origin)
        self.assertNotIn("PointId = NAME_None", compact_origin)
        self.assertNotIn("PointId = false", compact_origin)
        self.assertNotIn("PointId = true", compact_origin)
        self.assertNotIn("PointId = 0.f", compact_origin)
        self.assertNotIn("PointId = 160.f", compact_origin)

    def test_point_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName PointId"),
            LOCKED_DECL,
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_point_id_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_point_id_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_point_id_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_point_id_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_point_id_specifiers(spec_section(with_category))
    ...
    def test_does_not_parse_route_definition_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "route-point PointId field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "route_point_point_id_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
