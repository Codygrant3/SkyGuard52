...
# Mode FIELD (declaration-only; VisibleAnywhere / BlueprintReadOnly
# ESkyguardLandscapeCaptureDiagnosticMode Mode =
# ESkyguardLandscapeCaptureDiagnosticMode::Lit; one-line /
# split-line UPROPERTY wraps and split default; origin/main
# has NO Category). THIS IS NOT leftover
# landscape-capture-config-defaults #d6fd. THIS IS NOT leftover
# landscape-capture-diagnostic-enum #ce50 (enum type body;
# LOCK that script; this is the Mode UPROPERTY on
# CaptureConfigurationResult). THIS IS NOT leftover
# landscape-visible-audit-defaults #0bce. THIS IS NOT leftover
# VisibleAudit BoundsMaximum / BoundsMinimum /
# bBoundsFiniteAndNonzero / bSuccess. Parse STRUCT public
# section only AFTER struct
# FSkyguardLandscapeCaptureConfigurationResult and BEFORE
...
# default is not ESkyguardLandscapeCaptureDiagnosticMode::Lit.
...
# THIS IS leftover-safe FSkyguardLandscapeCaptureConfigurationResult
# Mode FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly ESkyguardLandscapeCaptureDiagnosticMode Mode =
# ESkyguardLandscapeCaptureDiagnosticMode::Lit; one-line /
# split-line UPROPERTY wraps and split default; origin/main
# has NO Category). THIS IS NOT leftover VisibleAudit
# BoundsMaximum.
...
TARGET = "ESkyguardLandscapeCaptureDiagnosticMode Mode = ESkyguardLandscapeCaptureDiagnosticMode::Lit;"
TARGET_WRONG = "bool bSuccess = false;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored default. Do not accept a sibling
    # bool / VisibleAudit default for
    # FSkyguardLandscapeCaptureConfigurationResult::Mode
    # (must be = ESkyguardLandscapeCaptureDiagnosticMode::Lit).
...
def public_section(header: str) -> str:
    ...
    if public is None:
        ...
        leaked = (
            ...
            "enum class ESkyguardLandscapeCaptureDiagnosticMode",
        )
        haystack = body + "\n" + section
        for token in leaked:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*ESkyguardLandscapeCaptureDiagnosticMode\s+Mode\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for ESkyguardLandscapeCaptureDiagnosticMode Mode is missing from "
...
class LandscapeCaptureConfigModeFieldDeclContractTests(unittest.TestCase):
    def test_landscape_capture_config_struct_exists(self) -> None:
        ...
        self.assertNotIn("FSkyguardMission01EnvironmentReadiness", section)
        self.assertNotIn("enum class ESkyguardLandscapeCaptureDiagnosticMode", section)
    ...
    def test_missing_mode_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_mode_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardLandscapeCaptureDiagnosticMode Mode"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("Lit;"), LOCKED_DECL)
        self.assertNotIn("= false", LOCKED_DECL)
        self.assertIn("= ESkyguardLandscapeCaptureDiagnosticMode::Lit", LOCKED_DECL)
        self.assertFalse(
            has_declaration(
                "\tESkyguardLandscapeCaptureDiagnosticMode Mode = ESkyguardLandscapeCaptureDiagnosticMode::LandscapeCoverage;\n",
                LOCKED_DECL,
            )
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertNotIn("ESkyguardMission05ProtectedTarget", LOCKED_DECL)
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardLandscapeCaptureDiagnosticMode "),
            LOCKED_DECL,
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_true_default = (
            "public:\n"
            f"\t{UPROPERTY_MISSION10}\n"
            "\tESkyguardLandscapeCaptureDiagnosticMode Mode = ESkyguardLandscapeCaptureDiagnosticMode::Lit;\n"
            "};\n"
        )
        wrap_origin_split_default = (
            "public:\n"
            "\tUPROPERTY(VisibleAnywhere, BlueprintReadOnly)\n"
            "\tESkyguardLandscapeCaptureDiagnosticMode Mode =\n"
        ...
        header_wrap_origin_split_default = (
            f"struct {CLASS_NAME}\n{{\n{wrap_origin_split_default}"
        )
        for header in (
            header_wrap_type,
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("Mode", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("ESkyguardLandscapeCaptureDiagnosticMode Mode"))
    ...
    def test_contract_does_not_relock_leftover_environment_readiness(
        ...
        self.assertIn(
            "Scripts/tests/test_landscape_capture_diagnostic"
            "_enum_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("Mode", locked_only)
    ...
    def test_contract_is_target_field_declaration_only(
        ...
        self.assertNotIn("BoundsMaximum", LOCKED_DECL)
        self.assertNotIn("bBoundsFiniteAndNonzero", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.endswith("Lit;"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("GetProtectedGroup", LOCKED_DECL)
        self.assertNotIn("FSkyguardMission10ProtectedRuntime", LOCKED_DECL)
        self.assertIn(UPROPERTY_MISSION10, section)

    def test_sibling_director_fields_do_not_satisfy_mode(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
