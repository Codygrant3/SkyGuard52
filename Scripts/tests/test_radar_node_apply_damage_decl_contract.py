# THIS IS leftover-safe ASkyguardRadarNode::ApplyDamage.
# origin/main form: split-line
# UFUNCTION(BlueprintCallable, Category="Skyguard|Campaign")
# then `void ApplyDamage(float Amount);`.
...
TARGET = "void ApplyDamage(float Amount);"
TARGET_WRONG_BARE = "void ApplyDamage();"
TARGET_WRONG_CONST = "void ApplyDamage(const float Amount);"
TARGET_WRONG_BOOL = "bool ApplyDamage(float Amount);"
TARGET_WRONG_FLOAT = "float ApplyDamage(float Amount);"
TARGET_WRONG_SIBLING = "void ResetNode();"
LOCKED_DECL = TARGET
UFUNCTION_CALLABLE = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Campaign")'
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored ApplyDamage signature.
    # Do not accept a bare `void ApplyDamage();`, a const
    # param, a bool/float return, or a sibling ResetNode
    # when origin/main has `void ApplyDamage(float Amount);`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"\bvoid\s+ApplyDamage\s*\(\s*\)", compact):
        return False
    if re.search(
        r"\bvoid\s+ApplyDamage\s*\(\s*const\s+float\s+Amount\s*\)",
        compact,
    ):
        return False
    if re.search(r"\bbool\s+ApplyDamage\s*\(", compact):
        return False
    if re.search(r"\bfloat\s+ApplyDamage\s*\(", compact):
        return False
    if re.search(
        r"\bvoid\s+ApplyDamage\s*\(\s*float\s+Amount\s*\)\s*;",
        compact,
    ) is None:
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        "class SKYGUARD52_API ASkyguardProtectAsset",
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+ApplyDamage\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void ApplyDamage is missing from "
...
class RadarNodeApplyDamageDeclContractTests(unittest.TestCase):
    def test_radar_node_class_exists(self) -> None:
        ...
        self.assertIn("ApplyDamage", section)
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertNotIn("ApplyDamage", section)
        for token in protected_component_tokens()[:1]:
            self.assertNotIn(token, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("ApplyDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_apply_damage_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn("void ApplyDamage(float Amount)", collapsed(origin))
        self.assertNotIn("void ApplyDamage()", collapsed(origin).replace(
            "void ApplyDamage(float Amount)",
            "",
        ))
        self.assertNotIn("const float Amount", collapsed(origin))

    def test_apply_damage_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("void ApplyDamage"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("(float Amount)", LOCKED_DECL)
        self.assertNotIn("const float Amount", LOCKED_DECL)
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid ApplyDmg(float Amount);\n",
            "\tvoid ApplyDamages(float Amount);\n",
            "\tvoid TakeDamage(float Amount);\n",
            "\tvoid ApplyDamage(int32 Amount);\n",
            "\tvoid ApplyDamage(float Amount) const;\n",
            "\tvoid ResetNode();\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar-node ApplyDamage decl contract "
