...
# or lock the GetPilotCommand body.
# origin/main is inline
# (`ESkyguardPilotCommand GetPilotCommand() const`
# with `{ return CurrentPilotCommand; }` possibly on the
...
# locking the body. `return CurrentPilotCommand;` is not a
...
# GetPilotConfirmationsIssued siblings in this wave (do
# not create those files). Stay off AimChinTurret,
# SetRotorPower, IssuePilotCommand,
...
GET_PILOT_COMMAND = ("ESkyguardPilotCommand GetPilotCommand() const;")
UFUNCTION_APACHE = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Apache")'
)
# Leftover #56–#64 plus SkyguardApacheAircraft production
# files. This lane only adds an isolated Python
# GetPilotCommand declaration contract on
...
# IssuePilotCommand, GetPilotConfirmationsIssued,
...
# IssuePilotCommand / GetPilotConfirmationsIssued
# siblings, leftover pilot-command-roster #b593, and
...
# SetRotorPower / IssuePilotCommand /
# GetPilotConfirmationsIssued siblings in this wave,
# leftover pilot-command-roster #b593, leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::GetPilotCommand",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheGetPilotCommandDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_PILOT_COMMAND), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_PILOT_COMMAND)
        self.assertIn("GetPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_PILOT_COMMAND))

    def test_missing_get_pilot_command_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_PILOT_COMMAND)
        self.assertIn("GetPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_PILOT_COMMAND)
        self.assertIn("GetPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_PILOT_COMMAND), section)
        self.assertNotIn("BlueprintPure", GET_PILOT_COMMAND)
        self.assertNotIn("UFUNCTION", GET_PILOT_COMMAND)
        self.assertNotIn("Category", GET_PILOT_COMMAND)
        self.assertNotIn("BlueprintCallable", GET_PILOT_COMMAND)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_PILOT_COMMAND)
        self.assertIn("GetPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tESkyguardPilotCommand GetPilotCommand();\n"
        wrong_return_void = "\tvoid GetPilotCommand() const;\n"
        wrong_return_bool = "\tbool GetPilotCommand() const;\n"
        added_arg = (
            "\tESkyguardPilotCommand GetPilotCommand(bool bReady) const;\n"
        )
        leftover_issue = (
            "\tvoid IssuePilotCommand(ESkyguardPilotCommand Command);\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_PILOT_COMMAND)
            self.assertIn("GetPilotCommand", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_pilot_command_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_PILOT_COMMAND),
            GET_PILOT_COMMAND,
        )
        self.assertTrue(has_declaration(section, GET_PILOT_COMMAND))
        self.assertEqual(declaration_count(section, GET_PILOT_COMMAND), 1)
        self.assertTrue(
            GET_PILOT_COMMAND.startswith("ESkyguardPilotCommand "),
            GET_PILOT_COMMAND,
        )
        self.assertTrue(GET_PILOT_COMMAND.endswith(";"), GET_PILOT_COMMAND)
        self.assertIn("GetPilotCommand()", GET_PILOT_COMMAND)
        self.assertIn(" const", GET_PILOT_COMMAND)
        self.assertNotIn("INDEX_NONE", GET_PILOT_COMMAND)
        self.assertNotIn("{", GET_PILOT_COMMAND)
        self.assertNotIn("}", GET_PILOT_COMMAND)
        self.assertNotIn("return ", GET_PILOT_COMMAND)
        self.assertNotIn("return CurrentPilotCommand", GET_PILOT_COMMAND)
        self.assertNotIn("AimChinTurret", GET_PILOT_COMMAND)
        self.assertNotIn("SetRotorPower", GET_PILOT_COMMAND)
        self.assertNotIn("IssuePilotCommand", GET_PILOT_COMMAND)
        self.assertNotIn("GetPilotConfirmationsIssued", GET_PILOT_COMMAND)
        self.assertNotIn("GetChinMuzzleLocation", GET_PILOT_COMMAND)
        self.assertNotIn("GetChinTurret", GET_PILOT_COMMAND)
        self.assertNotIn("GetRotorPowerScale", GET_PILOT_COMMAND)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_PILOT_COMMAND}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_PILOT_COMMAND))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_PILOT_COMMAND),
            GET_PILOT_COMMAND,
        )
        exact_inline = (
            "ESkyguardPilotCommand GetPilotCommand() "
            "const { return CurrentPilotCommand; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_PILOT_COMMAND))
        self.assertEqual(
            require_declaration(exact_inline, GET_PILOT_COMMAND),
            GET_PILOT_COMMAND,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_PILOT_COMMAND),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_PILOT_COMMAND),
            section,
        )
        self.assertEqual(
            require_declaration(section, GET_PILOT_COMMAND),
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, GET_PILOT_COMMAND), 1)
        one_line = f"{{\npublic:\n\t{GET_PILOT_COMMAND}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_PILOT_COMMAND))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_PILOT_COMMAND), section)
        self.assertEqual(
            require_declaration(section, GET_PILOT_COMMAND),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_PILOT_COMMAND)
        self.assertNotIn("return CurrentPilotCommand", GET_PILOT_COMMAND)
        self.assertNotIn("return Other", GET_PILOT_COMMAND)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_PILOT_COMMAND}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_PILOT_COMMAND)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_PILOT_COMMAND)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_pilot_command_body(self) -> None:
        locked_only = f"{GET_PILOT_COMMAND}\n"
        self.assertNotIn("{", GET_PILOT_COMMAND)
        self.assertNotIn("}", GET_PILOT_COMMAND)
        self.assertNotIn("return ", GET_PILOT_COMMAND)
        self.assertNotIn("return CurrentPilotCommand", GET_PILOT_COMMAND)
        self.assertNotIn("return CurrentPilotCommand", locked_only)
        self.assertNotIn(
            "ASkyguardApacheAircraft::GetPilotCommand",
            GET_PILOT_COMMAND,
        )
        self.assertNotIn("SkyguardApacheAircraft.cpp", GET_PILOT_COMMAND)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", GET_PILOT_COMMAND)
        self.assertNotIn("return true", GET_PILOT_COMMAND)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{GET_PILOT_COMMAND}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PILOT_COMMAND)
        self.assertNotIn("GetGunnerMount", GET_PILOT_COMMAND)
        self.assertNotIn("GetEyeMount", GET_PILOT_COMMAND)
        self.assertNotIn("GetWeaponMount", GET_PILOT_COMMAND)
        self.assertNotIn("GetChinTurret", GET_PILOT_COMMAND)
        self.assertNotIn("GetPilotMount", GET_PILOT_COMMAND)
        self.assertNotIn("GetSensorTurret", GET_PILOT_COMMAND)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_PILOT_COMMAND}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PILOT_COMMAND)
        self.assertNotIn("GetChinMuzzleLocation", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PILOT_COMMAND)
        self.assertNotIn("SetOrbitFocus", GET_PILOT_COMMAND)
        self.assertNotIn("FaceWorldLocation", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PILOT_COMMAND)
        self.assertNotIn("SetSensorView", GET_PILOT_COMMAND)
        self.assertNotIn("SetFirstPersonInterior", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PILOT_COMMAND)
        self.assertNotIn("ApplyDamage", GET_PILOT_COMMAND)
        self.assertNotIn("GetDamageFraction", GET_PILOT_COMMAND)
        self.assertNotIn("ApplySystemHit", GET_PILOT_COMMAND)
        self.assertNotIn("ApplyHit", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PILOT_COMMAND)
        self.assertNotIn("IsSensorLive", GET_PILOT_COMMAND)
        self.assertNotIn("IsThermalAvailable", GET_PILOT_COMMAND)
        self.assertNotIn("IsSensorViewActive", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_chin_muzzle(self) -> None:
        ...
        for token in LEFTOVER_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_PILOT_COMMAND)
        self.assertNotIn("GetChinMuzzleLocation", GET_PILOT_COMMAND)
        self.assertNotIn("test_apache_chin_muzzle_tests.py", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PILOT_COMMAND)
        self.assertNotIn("ESkyguardApacheSystem", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_cpg_feel(self) -> None:
        ...
        for token in LEFTOVER_CPG_FEEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PILOT_COMMAND)
        self.assertNotIn("test_apache_cpg_feel_contract.py", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_pilot_command_roster(
        ...
        self.assertNotIn("Pursuit", GET_PILOT_COMMAND)
        self.assertNotIn("OrbitLeft", GET_PILOT_COMMAND)
        self.assertNotIn("AttackRun", GET_PILOT_COMMAND)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_PILOT_COMMAND)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", GET_PILOT_COMMAND)
        self.assertNotIn("USkyguardBriefingWidget", GET_PILOT_COMMAND)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            GET_PILOT_COMMAND,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_PILOT_COMMAND)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_PILOT_COMMAND)
        self.assertNotIn("ESkyguardMissionSkylineStyle", GET_PILOT_COMMAND)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_PILOT_COMMAND}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_PILOT_COMMAND),
            GET_PILOT_COMMAND,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardApacheAircraft::GetPilotCommand", section)
        self.assertNotIn("Pursuit", GET_PILOT_COMMAND)
        self.assertNotIn("OrbitLeft", GET_PILOT_COMMAND)
        self.assertNotIn("AttackRun", GET_PILOT_COMMAND)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GetPilotCommand contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_PILOT_COMMAND.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache GetPilotCommand contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_PILOT_COMMAND)

    def test_contract_is_get_pilot_command_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return CurrentPilotCommand", GET_PILOT_COMMAND)
        self.assertNotIn("{", GET_PILOT_COMMAND)
        self.assertTrue(GET_PILOT_COMMAND.startswith("ESkyguardPilotCommand "))
        self.assertTrue(GET_PILOT_COMMAND.endswith(";"))
        self.assertIn(" const", GET_PILOT_COMMAND)
        self.assertIn(UFUNCTION_APACHE, section)
