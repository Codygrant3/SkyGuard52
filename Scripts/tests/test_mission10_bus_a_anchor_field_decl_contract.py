...
# INDEX_NONE or lock BusAAnchor
# construction in the .cpp. This is leftover-safe
# Mission10 Evacuation BusAAnchor
...
# HighwayConvoyAnchor field sibling (in-flight),
...
# HighwayConvoyAnchor / BusBAnchor / AmbulanceAAnchor /
...
LOCKED_DECL = BUS_A_ANCHOR_FIELD
...
class Mission10BusAAnchorFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("BusAAnchor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_bus_a_anchor_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
    ...
    def test_bus_a_anchor_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("BusAAnchor", LOCKED_DECL)
    ...
    def test_contract_is_bus_a_anchor_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_bus_a_anchor(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
