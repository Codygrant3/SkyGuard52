# THIS IS leftover-safe ASkyguardProtectAsset::IsDestroyed.
...
# Do NOT contract MaxIntegrity / CurrentIntegrity.
# Do NOT contract ApplyDamage / GetIntegrityFraction /
# ResetIntegrity (sibling workers).
...
# Do NOT contract other AudioTelemetry fields
# (RequestedEvents #1281, PlayedEvents #1282,
# RejectedByCooldown #1283, RejectedByConcurrency,
# RejectedMissingAsset, PriorityEvictions).
...
TARGET = "bool IsDestroyed() const { return bDead; }"
TARGET_WRONG_BARE = "bool IsDestroyed();"
TARGET_WRONG_NO_CONST = "bool IsDestroyed() { return bDead; }"
TARGET_WRONG_NO_BODY = "bool IsDestroyed() const;"
TARGET_WRONG_TRUE = "bool IsDestroyed() const { return true; }"
TARGET_WRONG_FALSE = "bool IsDestroyed() const { return false; }"
TARGET_WRONG_VOID = "void IsDestroyed() const { return bDead; }"
TARGET_WRONG_SIBLING = "void ApplyDamage(float Amount);"
LOCKED_DECL = TARGET
UFUNCTION_PURE = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Campaign")'
)
...
LEFTOVER_PROTECT_ASSET_CARGO_PROXY = (
    "Scripts/tests/test_protect_asset_cargo_proxy"
    "_contract.py"
)
CLONE_PLAYED_EVENTS = (
...
SIBLING_PUBLIC_METHODS = (
    "ApplyDamage",
    "GetIntegrityFraction",
    "ResetIntegrity",
)
SIBLING_PUBLIC_FIELDS = (
    "MaxIntegrity",
    "CurrentIntegrity",
)
PROTECTED_MEMBERS = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept a bare decl, missing const, or
    # `return true` / `return false` / void rename.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"bool\s+IsDestroyed\s*\(\s*\)\s*const\s*\{\s*return\s+bDead\s*;\s*\}",
        compact,
    ) is None:
        return False
    if re.search(r"void\s+IsDestroyed\b", compact):
        return False
    if re.search(r"bool\s+IsDestroyed\s*\(\s*\)\s*const\s*;", compact):
        return False
    if re.search(
        r"bool\s+IsDestroyed\s*\(\s*\)\s*const\s*\{\s*return\s+true\s*;",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+IsDestroyed\s*\(\s*\)\s*const\s*\{\s*return\s+false\s*;",
        compact,
    ):
        return False
    return True
...
class ProtectAssetIsDestroyedDeclContractTests(unittest.TestCase):
    def test_protect_asset_class_exists(self) -> None:
        header = origin_main_header()
        self.assertIn(CLASS_NAME, header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertIn("class SKYGUARD52_API ASkyguardProtectAsset", header)
        body = class_body(header)
        ...
        for method in SIBLING_PUBLIC_METHODS:
            self.assertIn(method, section)
        for field in SIBLING_PUBLIC_FIELDS:
            self.assertIn(field, section)
        for member in PROTECTED_MEMBERS:
            self.assertFalse(
                has_identifier(section, member),
                member,
            )
        for field in OTHER_TELEMETRY_FIELDS:
        ...
        self.assertNotIn(STOP_BEFORE_ROOT, section)
        self.assertNotIn(STOP_BEFORE_HULL, section)
        self.assertNotIn(STOP_BEFORE_VOLUME, section)
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "IsDestroyed()const { return true; }",
            compact_origin,
        )
        self.assertNotIn(
            "IsDestroyed()const { return false; }",
            compact_origin,
        )
    ...
    def test_is_destroyed_declaration_matches_origin_main(self) -> None:
        ...
        self.assertIn("return bDead", LOCKED_DECL)
    ...
    def test_does_not_contract_sibling_methods_or_fields(self) -> None:
        ...
        for field in SIBLING_PUBLIC_FIELDS:
            self.assertIn(field, section)
        for member in PROTECTED_MEMBERS:
            self.assertFalse(has_identifier(section, member), member)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, section)
        for nested in RESULT_NESTED_MEMBERS:
            self.assertFalse(has_identifier(section, nested), nested)
    ...
    def test_does_not_parse_protected_or_director_methods(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertIn(STOP_BEFORE_ROOT, header)
        self.assertIn(STOP_BEFORE_HULL, header)
        self.assertIn(STOP_BEFORE_VOLUME, header)
        ...
        self.assertIn(STOP_BEFORE_ROOT, leaked)
        self.assertIn(STOP_BEFORE_HULL, leaked)
        self.assertIn(STOP_BEFORE_VOLUME, leaked)
        self.assertNotIn(GET_OBJECTIVE_RUNTIME, leaked)
        self.assertNotIn(ADD_OBJECTIVE_PROGRESS, leaked)
        self.assertNotIn(BIND_RUNTIME_ACTORS, leaked)
        self.assertNotIn(HANDLE_DRONE_CITY_IMPACT, leaked)
        self.assertNotIn(GET_STORM_RAIN_BEAT_KIT, leaked)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tint32 {leftover_retired_primary_hits_field()} = 0;\n"
            f"\tint32 {leftover_retired_guided_hits_field()} = 0;\n"
            "\tvoid ApplyDamage(float Amount);\n"
            "\tfloat GetIntegrityFraction() const;\n"
            "\tvoid ResetIntegrity();\n"
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("ApplyDamage", LOCKED_DECL)
        self.assertNotIn("ResetIntegrity", LOCKED_DECL)
        self.assertNotIn("GetIntegrityFraction", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "protect-asset IsDestroyed decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_protect_asset_is_destroyed"
            "_decl_contract.py"
        ))
    ...
    def test_contract_is_target_ufunction_declaration_only(self) -> None:
        ...
        self.assertNotIn("GetIntegrityFraction", locked_only)
        self.assertNotIn("ResetIntegrity", locked_only)
