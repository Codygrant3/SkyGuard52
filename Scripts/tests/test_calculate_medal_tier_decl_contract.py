...
# medal-tier value, score, or mission-result payload, or lock
# the CalculateMedalTier body in the .cpp. origin/main is
# split-line
...
# one.
CALCULATE_MEDAL_TIER = (
    "static int32 CalculateMedalTier("
    "const FSkyguardMissionScoreRules& Rules, "
    "int32 Score);"
)
# Leftover #56–#64 plus CampaignSubsystem production files.
# This lane only adds an isolated Python CalculateMedalTier
# declaration contract. Stay off CalculateMissionScore
# (sibling in-flight), GetMissionRecords, GetRouteRuntime
# (sibling in-flight), CompleteActiveMission #318 /
# FinalizeActiveMission #319 / FailActiveMission (sibling
# draft this wave), FillResultCombatStats / FillAndFinalize /
# FillAndFail / ApplyHydraForClusters (leftover
# ASkyguardGunner*), leftover CPG debrief
# #284/#195/#130/#8ccd, leftover campaign-save
# empty-fail-closed, leftover campaign-roster #111,
# LoadCampaignProgress #290, leftover mission-score-rules
...
# #56–#64, leftover #147 ApacheSystem, leftover #149 weapon
# stations, leftover #152 pilot commands, leftover #154
# loadout / lock-phase, leftover settings invert-look /
# ApplySettings broadcast #134, Harbor IncomingRadar 40/80,
...
# Isolated-test drafts stay off this lane. CalculateMissionScore,
# leftover mission-score-rules defaults, leftover
# mission-result defaults, leftover campaign-save
# empty-fail-closed, leftover campaign-roster lookup, leftover
# LoadCampaignProgressAfterConfigure, leftover CPG debrief
# copy / snapshot / fail-closed, leftover objective-runtime
...
# CalculateMissionScore is a sibling in-flight this wave.
# GetMissionRecords / GetRouteRuntime stay sibling-only.
# CompleteActiveMission / FinalizeActiveMission /
# FailActiveMission are sibling drafts this wave.
...
CALCULATE_MISSION_SCORE_NOT_LOCKED = (
    "static int32 CalculateMissionScore(",
)
...
# Leftover mission-score-rules defaults #deae stay unlocked.
# Field names only; do not invent threshold values.
LEFTOVER_SCORE_RULES_NOT_LOCKED = (
    "CompletionScore",
...
# Leftover mission-result defaults #c4db stay unlocked.
# Do not invent a mission-result payload.
...
# Invented medal-tier value / score / mission-result payload
# stay unlocked. Declaration presence only.
INVENTED_MEDAL_PAYLOAD = (
    "return 0",
    "return 1",
    "return 2",
    "return 3",
...
# .cpp CalculateMedalTier body / invented return values stay
...
CPP_AND_INVENTED = (
    ...
    "SilverThreshold",
    "BronzeThreshold",
    "USkyguardCampaignSubsystem::CalculateMedalTier",
    "SkyguardCampaignSubsystem.cpp",
)
SIBLING_TYPES = (
...
class CalculateMedalTierDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CALCULATE_MEDAL_TIER),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CALCULATE_MEDAL_TIER)
        self.assertIn("CalculateMedalTier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CALCULATE_MEDAL_TIER))

    def test_missing_medal_tier_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CALCULATE_MEDAL_TIER)
        self.assertIn("CalculateMedalTier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CALCULATE_MEDAL_TIER)
        self.assertIn("CalculateMedalTier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{CALCULATE_MEDAL_TIER}\n"
        self.assertNotIn("UFUNCTION", CALCULATE_MEDAL_TIER)
        self.assertNotIn("BlueprintCallable", CALCULATE_MEDAL_TIER)
        self.assertNotIn("BlueprintPure", CALCULATE_MEDAL_TIER)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CALCULATE_MEDAL_TIER)
        self.assertIn("CalculateMedalTier", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_static = (
            "\tint32 CalculateMedalTier("
            "const FSkyguardMissionScoreRules& Rules, "
            "int32 Score);\n"
        )
        missing_rules = (
            "\tstatic int32 CalculateMedalTier(int32 Score);\n"
        )
        missing_score = (
            "\tstatic int32 CalculateMedalTier("
            "const FSkyguardMissionScoreRules& Rules);\n"
        ...
        wrong_return = (
            "\tstatic float CalculateMedalTier("
            "const FSkyguardMissionScoreRules& Rules, "
            "int32 Score);\n"
        )
        wrong_rules_type = (
            "\tstatic int32 CalculateMedalTier("
            "const FSkyguardMissionResult& Rules, "
            "int32 Score);\n"
        )
        non_const_rules = (
            "\tstatic int32 CalculateMedalTier("
            "FSkyguardMissionScoreRules& Rules, "
            "int32 Score);\n"
        )
        pointer_rules = (
            "\tstatic int32 CalculateMedalTier("
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CALCULATE_MEDAL_TIER)
            self.assertIn("CalculateMedalTier", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_medal_tier_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CALCULATE_MEDAL_TIER),
            CALCULATE_MEDAL_TIER,
        )
        self.assertTrue(has_declaration(section, CALCULATE_MEDAL_TIER))
        self.assertEqual(declaration_count(section, CALCULATE_MEDAL_TIER), 1)
        self.assertTrue(
            CALCULATE_MEDAL_TIER.endswith(";"),
            CALCULATE_MEDAL_TIER,
        )
        self.assertTrue(
            CALCULATE_MEDAL_TIER.startswith("static int32 "),
            CALCULATE_MEDAL_TIER,
        )
        self.assertIn("CalculateMedalTier(", CALCULATE_MEDAL_TIER)
        self.assertIn(
            "const FSkyguardMissionScoreRules& Rules",
            CALCULATE_MEDAL_TIER,
        )
        self.assertIn("int32 Score", CALCULATE_MEDAL_TIER)
        self.assertNotIn("INDEX_NONE", CALCULATE_MEDAL_TIER)
        self.assertNotIn("UFUNCTION", CALCULATE_MEDAL_TIER)
        self.assertNotIn("{", CALCULATE_MEDAL_TIER)
        self.assertNotIn("}", CALCULATE_MEDAL_TIER)
        self.assertNotIn("return ", CALCULATE_MEDAL_TIER)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_args = (
            "public:\n"
            "\tstatic int32 CalculateMedalTier("
            "const FSkyguardMissionScoreRules&\n"
            "\t\tRules,\n"
            "\t\tint32\n"
            "\t\tScore);\n"
            "};\n"
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, CALCULATE_MEDAL_TIER),
                1,
            )
        one_line = f"{{\npublic:\n\t{CALCULATE_MEDAL_TIER}\n}}\n"
        self.assertTrue(has_declaration(one_line, CALCULATE_MEDAL_TIER))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CALCULATE_MEDAL_TIER),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CALCULATE_MEDAL_TIER)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_declaration_does_not_invent_medal_tier_value_or_score(
        self,
    ) -> None:
        locked_only = f"{CALCULATE_MEDAL_TIER}\n"
        for token in INVENTED_MEDAL_PAYLOAD:
            self.assertNotIn(token, CALCULATE_MEDAL_TIER)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("return 0", CALCULATE_MEDAL_TIER)
        self.assertNotIn("return 1", CALCULATE_MEDAL_TIER)
        self.assertNotIn("return 2", CALCULATE_MEDAL_TIER)
        self.assertNotIn("return 3", CALCULATE_MEDAL_TIER)
        self.assertNotIn("MedalTier = 0", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FinalScore = 0", CALCULATE_MEDAL_TIER)
        self.assertNotIn("GoldThreshold", CALCULATE_MEDAL_TIER)
        self.assertNotIn("SilverThreshold", CALCULATE_MEDAL_TIER)
        self.assertNotIn("BronzeThreshold", CALCULATE_MEDAL_TIER)

    def test_declaration_does_not_invent_mission_result_payload(self) -> None:
        locked_only = f"{CALCULATE_MEDAL_TIER}\n"
        for token in LEFTOVER_MISSION_RESULT_NOT_LOCKED:
            self.assertNotIn(token, CALCULATE_MEDAL_TIER)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("FSkyguardMissionResult", CALCULATE_MEDAL_TIER)
        self.assertNotIn("bMissionSucceeded", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FinalScore", CALCULATE_MEDAL_TIER)
        self.assertNotIn("CompletedObjectiveIds", CALCULATE_MEDAL_TIER)

    def test_contract_does_not_lock_medal_tier_cpp_body(self) -> None:
        locked_only = f"{CALCULATE_MEDAL_TIER}\n"
        self.assertNotIn("{", CALCULATE_MEDAL_TIER)
        self.assertNotIn("}", CALCULATE_MEDAL_TIER)
        self.assertNotIn("return ", CALCULATE_MEDAL_TIER)
        ...
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", CALCULATE_MEDAL_TIER)
        self.assertNotIn("SkyguardCampaignSubsystem.cpp", locked_only)
        self.assertNotIn("GoldThreshold", CALCULATE_MEDAL_TIER)
        self.assertNotIn("SilverThreshold", CALCULATE_MEDAL_TIER)
        self.assertNotIn("BronzeThreshold", CALCULATE_MEDAL_TIER)

    def test_contract_does_not_relock_calculate_mission_score(self) -> None:
        locked_only = f"{CALCULATE_MEDAL_TIER}\n"
        for neighbor in CALCULATE_MISSION_SCORE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("CalculateMissionScore", CALCULATE_MEDAL_TIER)
        self.assertNotIn("CalculateMissionScore", locked_only)
        self.assertNotIn("USkyguardMissionDefinition", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FSkyguardMissionResult", CALCULATE_MEDAL_TIER)

    def test_contract_does_not_relock_leftover_score_rules_defaults(
        self,
    ) -> None:
        locked_only = f"{CALCULATE_MEDAL_TIER}\n"
        section = public_section(origin_main_header())
        for token in LEFTOVER_SCORE_RULES_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_configure_can_start_or_start(self) -> None:
        ...
        for neighbor in CONFIGURE_CAN_START_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("ConfigureCampaign", CALCULATE_MEDAL_TIER)
        self.assertNotIn("CanStartMission", CALCULATE_MEDAL_TIER)
        self.assertNotIn("StartMission", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_is_mission_unlocked(self) -> None:
        ...
        for neighbor in IS_MISSION_UNLOCKED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("IsMissionUnlocked", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_objectives(self) -> None:
        ...
        for neighbor in OBJECTIVES_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("AddObjectiveProgress", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FailObjective", CALCULATE_MEDAL_TIER)
        self.assertNotIn("CompleteSurviveObjectiveIfIntact", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("CompleteActiveMission", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FinalizeActiveMission", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FailActiveMission", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("FillResultCombatStats", CALCULATE_MEDAL_TIER)
        self.assertNotIn("ASkyguardGunner", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FillAndFinalize", CALCULATE_MEDAL_TIER)
        self.assertNotIn("FillAndFail", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_elapsed_seconds(self) -> None:
        ...
        for neighbor in ELAPSED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("GetActiveMissionElapsedSeconds", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("AcknowledgeDebrief", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_debrief_neighbors(self) -> None:
        ...
        for neighbor in DEBRIEF_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("RetrySaveLastDebrief", CALCULATE_MEDAL_TIER)
        self.assertNotIn("GetLastDebrief", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_travel_helpers(self) -> None:
        ...
        for neighbor in TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("CanTravelToNextMission", CALCULATE_MEDAL_TIER)
        self.assertNotIn("GetNextMissionMapPackageName", CALCULATE_MEDAL_TIER)
        self.assertNotIn("TravelToNextMission", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_runtime_getters(self) -> None:
        ...
        for neighbor in RUNTIME_GETTERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("GetObjectiveRuntime", CALCULATE_MEDAL_TIER)
        self.assertNotIn("GetRouteRuntime", CALCULATE_MEDAL_TIER)
        self.assertNotIn("GetMissionRecords", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_save_game_helpers(self) -> None:
        ...
        for neighbor in SAVE_GAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CALCULATE_MEDAL_TIER)
        self.assertNotIn("ApplySaveGame", CALCULATE_MEDAL_TIER)
        self.assertNotIn("BuildSaveGame", CALCULATE_MEDAL_TIER)
        self.assertNotIn("SaveCampaignToSlot", CALCULATE_MEDAL_TIER)
        self.assertNotIn("LoadCampaignFromSlot", CALCULATE_MEDAL_TIER)
        self.assertNotIn("DeleteCampaignSlot", CALCULATE_MEDAL_TIER)
        self.assertNotIn("IsValidCampaignSlotName", CALCULATE_MEDAL_TIER)
        self.assertNotIn("GetEarnedCampaignMedals", CALCULATE_MEDAL_TIER)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CALCULATE_MEDAL_TIER}\n"
        self.assertEqual(
            require_declaration(locked_only, CALCULATE_MEDAL_TIER),
            CALCULATE_MEDAL_TIER,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(CALCULATE_MEDAL_TIER, "Rifle")
        self.assertNotEqual(CALCULATE_MEDAL_TIER, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign CalculateMedalTier contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, CALCULATE_MEDAL_TIER.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", CALCULATE_MEDAL_TIER)
    ...
    def test_locked_scripts_list_sibling_isolated_contracts(self) -> None:
        ...
        self.assertIn("test_fail_active_mission_decl_contract.py", scripts)
    ...
    def test_contract_is_medal_tier_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("static int32 CalculateMedalTier(", CALCULATE_MEDAL_TIER)
        self.assertIn(
            "const FSkyguardMissionScoreRules& Rules",
            CALCULATE_MEDAL_TIER,
        )
