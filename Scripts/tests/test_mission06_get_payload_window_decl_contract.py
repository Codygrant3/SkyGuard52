...
# GetTargetRuntime / GetSurvivingTargetCount /
...
# #681, leftover Mission06
# InitializePlayableMission, leftover Mission06
# ConfigureMissionDefinition, leftover Mission05
# InitializePlayableMission #666, leftover
# Mission05 GetSurvivingTargetCount,
# leftover Mission04 wave-state enum #bb22,
# leftover Mission06 wave-state enum #fa65,
# leftover airfield-target enum #14d2 (do not add
# an enum-roster contract), leftover
...
# Category is Skyguard|Mission06|Payload, not
# Mission01 / Mission02 / Mission03 / Mission04 /
# Mission05 / Integration / Waves / Targets /
...
# #681, not leftover Harbor Mission02 Integration.
...
# #668/#669/#681. Closed leftover #658–#673.
# Do not reopen leftover payload-window-runtime-
# defaults #f114.
...
# leftover Mission05 GetReadiness #681, leftover
# Mission01 / Mission03 / Mission04 GetReadiness,
# leftover env-readiness #6b9d / #b931, leftover
# Mission06 InitializePlayableMission, leftover
# Mission06 ConfigureMissionDefinition, leftover
# Mission06 wave-state enum #fa65,
...
# GetPayloadWindow method declaration
...
LEFTOVER_MISSION01_GET_PAYLOAD_WINDOW_NOT_LOCKED = (
    "ASkyguardMission01IntegrationDirector",
    "SkyguardMission01IntegrationDirector.h",
    "test_mission01_get_readiness_decl_contract.py",
...
LEFTOVER_MISSION03_GET_PAYLOAD_WINDOW_NOT_LOCKED = (
    "ASkyguardMission03IntegrationDirector",
    "SkyguardMission03IntegrationDirector.h",
...
LEFTOVER_MISSION04_GET_PAYLOAD_WINDOW_NOT_LOCKED = (
    "ASkyguardMission04IntegrationDirector",
    "SkyguardMission04IntegrationDirector.h",
    "test_mission04_get_readiness_decl_contract.py",
...
LEFTOVER_MISSION05_GET_PAYLOAD_WINDOW_NOT_LOCKED = (
    "ASkyguardMission05IntegrationDirector",
    "SkyguardMission05IntegrationDirector.h",
    "test_mission05_get_readiness_decl_contract.py",
...
LEFTOVER_ENVIRONMENT_GET_PAYLOAD_WINDOW_NOT_LOCKED = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission06IntegrationDirector::GetPayloadWindow",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission06GetPayloadWindowDeclContractTests(unittest.TestCase):
    def test_mission06_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_PAYLOAD_WINDOW),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_PAYLOAD_WINDOW)
        self.assertIn("GetPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_PAYLOAD_WINDOW)
    ...
    def test_missing_get_payload_window_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission06IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_PAYLOAD_WINDOW)
        self.assertIn("GetPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_PAYLOAD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_PAYLOAD_WINDOW)
        self.assertIn("GetPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_PAYLOAD, section)
        ...
        self.assertNotIn("UFUNCTION", GET_PAYLOAD_WINDOW)
        self.assertNotIn("BlueprintPure", GET_PAYLOAD_WINDOW)
        self.assertNotIn("Category", GET_PAYLOAD_WINDOW)
        self.assertNotIn("BlueprintCallable", UFUNCTION_PAYLOAD)
        self.assertIn("Skyguard|Mission06|Payload", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission01", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission03", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission04", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission05", UFUNCTION_PAYLOAD)
        self.assertNotIn("Waves", UFUNCTION_PAYLOAD)
        self.assertNotIn("Integration", UFUNCTION_PAYLOAD)
        self.assertNotIn("Targets", UFUNCTION_PAYLOAD)
        self.assertNotIn("Objectives", UFUNCTION_PAYLOAD)
        self.assertIn("BlueprintPure", UFUNCTION_PAYLOAD)
        self.assertNotIn("Environment", UFUNCTION_PAYLOAD)
        self.assertNotIn("Briefing", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission02", UFUNCTION_PAYLOAD)
        self.assertNotIn("Boss", UFUNCTION_PAYLOAD)
        self.assertNotIn("Destruction", UFUNCTION_PAYLOAD)
        self.assertNotIn("Apache", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission07", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission10", UFUNCTION_PAYLOAD)
        self.assertNotIn("Encounter", UFUNCTION_PAYLOAD)
        self.assertNotIn("Safety", UFUNCTION_PAYLOAD)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_PAYLOAD)
            self.assertNotIn(invented, GET_PAYLOAD_WINDOW)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_PAYLOAD_WINDOW)
        self.assertIn("GetPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetPayloadWindow() const;\n"
        )
        missing_const = (
            "\tconst FSkyguardPayloadWindowRuntime& "
            "GetPayloadWindow();\n"
        ...
        with_arg = (
            "\tconst FSkyguardPayloadWindowRuntime& "
            "GetPayloadWindow(float DeltaSeconds) const;\n"
        )
        renamed = (
            "\tconst FSkyguardPayloadWindowRuntime& "
            "GetReadyWindow() const;\n"
        )
        short_name = "\tbool GetPayloadWindow() const;\n"
        leftover_m05_type = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_PAYLOAD_WINDOW)
            self.assertIn("GetPayloadWindow", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_payload_window_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_PAYLOAD_WINDOW),
            GET_PAYLOAD_WINDOW,
        )
        self.assertTrue(has_declaration(section, GET_PAYLOAD_WINDOW))
        self.assertEqual(
            declaration_count(section, GET_PAYLOAD_WINDOW),
            1,
        )
        self.assertTrue(
            GET_PAYLOAD_WINDOW.startswith("const "),
            GET_PAYLOAD_WINDOW,
        )
        self.assertIn("FSkyguardPayloadWindowRuntime", GET_PAYLOAD_WINDOW)
        self.assertIn("GetPayloadWindow", GET_PAYLOAD_WINDOW)
        self.assertNotIn(";", GET_PAYLOAD_WINDOW)
        self.assertIn(" const", GET_PAYLOAD_WINDOW)
        self.assertIn("()", GET_PAYLOAD_WINDOW)
        self.assertNotIn("TArray<", GET_PAYLOAD_WINDOW)
        self.assertNotIn("TObjectPtr", GET_PAYLOAD_WINDOW)
        self.assertNotIn("TSoftObjectPtr", GET_PAYLOAD_WINDOW)
        self.assertNotIn("INDEX_NONE", GET_PAYLOAD_WINDOW)
        self.assertNotIn("UFUNCTION", GET_PAYLOAD_WINDOW)
        self.assertNotIn("{", GET_PAYLOAD_WINDOW)
        self.assertNotIn("}", GET_PAYLOAD_WINDOW)
        self.assertNotIn("return ", GET_PAYLOAD_WINDOW)
        self.assertNotIn("TArray<", GET_PAYLOAD_WINDOW)
        self.assertNotIn("Root", GET_PAYLOAD_WINDOW)
        self.assertNotIn("Briefing", GET_PAYLOAD_WINDOW)
        self.assertNotIn("AudioDirector", GET_PAYLOAD_WINDOW)
        self.assertNotIn("RadioChatter", GET_PAYLOAD_WINDOW)
        self.assertNotIn("SortiePresentation", GET_PAYLOAD_WINDOW)
        self.assertNotIn("CampaignDefinition", GET_PAYLOAD_WINDOW)
        self.assertNotIn("Readiness;", GET_PAYLOAD_WINDOW)
        self.assertNotIn("bAutoInitialize", GET_PAYLOAD_WINDOW)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_PAYLOAD_WINDOW)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_PAYLOAD_WINDOW)
        self.assertNotIn("PathfinderSpawnLocation", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetAircraft", GET_PAYLOAD_WINDOW)
        self.assertNotIn("BindRuntimeActors", GET_PAYLOAD_WINDOW)
        self.assertNotIn("HandleDroneCityImpact", GET_PAYLOAD_WINDOW)
        self.assertNotIn("StartNextWave", GET_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyThreatDestroyed", GET_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceConvoyByDistance", GET_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyConvoyDamage", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetWaveState", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetConvoyRouteState", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatKit", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetNightBeatKit", GET_PAYLOAD_WINDOW)
        self.assertNotIn("StartSearchlightWindow", GET_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceSearchlightTrack", GET_PAYLOAD_WINDOW)
        self.assertNotIn("NotifySubstationDamage", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetRemainingThreatsInWave", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetSearchlightRuntime", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetSubstationIntegrity", GET_PAYLOAD_WINDOW)
        self.assertNotIn("StartPayloadWindow", GET_PAYLOAD_WINDOW)
        self.assertNotIn("AdvancePayloadWindow", GET_PAYLOAD_WINDOW)
        self.assertNotIn("TryJamActivePayload", GET_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetReadiness", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetTargetRuntime", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetSurvivingTargetCount", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatKind", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatIndex", GET_PAYLOAD_WINDOW)
        self.assertNotIn("TickDayBeatKit", GET_PAYLOAD_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardMission06WaveState", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardAirfieldTarget", GET_PAYLOAD_WINDOW)
        self.assertNotIn("FSkyguardMission06IntegrationReadiness", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_PAYLOAD_WINDOW)
        self.assertNotIn("RoadHunterSpawnLocation", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_PAYLOAD_WINDOW)
        self.assertNotIn("InitializePlayableMission", GET_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyObjectiveProgress", GET_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_PAYLOAD_WINDOW)
        self.assertNotIn("SynchronizeRuntimeState", GET_PAYLOAD_WINDOW)
        self.assertNotIn("IsCorePlayableReady", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ConfigureMissionDefinition", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetObjectiveRuntime", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetGunner", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetPathfinder", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetMissionId", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ValidateMissionContract", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ConfigureFromMission", GET_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceBriefing", GET_PAYLOAD_WINDOW)
        self.assertNotIn("SetAssetsReady", GET_PAYLOAD_WINDOW)
        self.assertNotIn("AcknowledgeAndLaunch", GET_PAYLOAD_WINDOW)
        self.assertNotIn("CanLaunch", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetElapsedSeconds", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetBriefingState", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetBriefingText", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetRadioChatter", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetPresentation", GET_PAYLOAD_WINDOW)
        self.assertNotIn("GetMissionTitle", GET_PAYLOAD_WINDOW)
        self.assertNotIn("AcknowledgeBriefing", GET_PAYLOAD_WINDOW)
        self.assertNotIn("LaunchSortie", GET_PAYLOAD_WINDOW)
        self.assertNotIn("HullCollider", GET_PAYLOAD_WINDOW)
        self.assertNotIn("OpticalTracker", GET_PAYLOAD_WINDOW)
        self.assertNotIn("WeaponServo", GET_PAYLOAD_WINDOW)
        self.assertNotIn("CountermeasurePod", GET_PAYLOAD_WINDOW)
        self.assertNotIn("MinHeightFromOriginCm", GET_PAYLOAD_WINDOW)
        self.assertNotIn("RadarNode", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardMission02WaveState", GET_PAYLOAD_WINDOW)
        self.assertNotIn("HarborIndustrial", GET_PAYLOAD_WINDOW)
        self.assertNotIn("MaxIntegrity", GET_PAYLOAD_WINDOW)
        self.assertNotIn("CurrentIntegrity", GET_PAYLOAD_WINDOW)
        self.assertNotIn("FillAndFinalize", GET_PAYLOAD_WINDOW)
        self.assertNotIn("FillAndFail", GET_PAYLOAD_WINDOW)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_PAYLOAD_WINDOW)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_PAYLOAD} {GET_PAYLOAD_WINDOW}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_PAYLOAD_WINDOW}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_PAYLOAD_WINDOW))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_PAYLOAD_WINDOW),
            section,
    ...
    def test_environment_category_does_not_satisfy_integration(self) -> None:
        ...
        self.assertNotEqual(environment, UFUNCTION_PAYLOAD)
        self.assertNotEqual(briefing, UFUNCTION_PAYLOAD)
        self.assertNotIn(environment, UFUNCTION_PAYLOAD)
        self.assertNotIn(briefing, UFUNCTION_PAYLOAD)
        leftover_m01 = (
            'UFUNCTION(BlueprintPure, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_m02, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_harbor, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_m03, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_m04, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_m05, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_waves, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_integration, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_targets, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_objectives, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_m01, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_m02, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_harbor, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_m03, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_m04, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_m05, UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission01", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission03", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission04", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission05", UFUNCTION_PAYLOAD)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_PAYLOAD_WINDOW)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_PAYLOAD_WINDOW)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_payload_window_cpp_body(self) -> None:
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
        self.assertNotIn("{", GET_PAYLOAD_WINDOW)
        self.assertNotIn("}", GET_PAYLOAD_WINDOW)
        self.assertNotIn("return ", GET_PAYLOAD_WINDOW)
        ...
        self.assertNotIn("return false", GET_PAYLOAD_WINDOW)
        self.assertNotIn("return true", GET_PAYLOAD_WINDOW)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PAYLOAD_WINDOW)
        self.assertIn("GetPayloadWindow", GET_PAYLOAD_WINDOW)
        self.assertTrue(GET_PAYLOAD_WINDOW.startswith("const "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_PAYLOAD_WINDOW)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_PAYLOAD_WINDOW)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_PAYLOAD_WINDOW)
        self.assertNotIn("CurrentIntegrity", GET_PAYLOAD_WINDOW)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_PAYLOAD_WINDOW)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_PAYLOAD_WINDOW)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_PAYLOAD_WINDOW),
            GET_PAYLOAD_WINDOW,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_PAYLOAD_WINDOW)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_PAYLOAD_WINDOW)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 GetPayloadWindow method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_PAYLOAD_WINDOW.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_PAYLOAD_WINDOW.lower(),
                "mission06 GetPayloadWindow contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_PAYLOAD_WINDOW)

    def test_contract_is_get_payload_window_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_payload_window(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_PAYLOAD_WINDOW)
            )
            self.assertNotEqual(GET_PAYLOAD_WINDOW, leftover)
        self.assertIn(
    ...
    def test_leftover_mission01_get_payload_window_does_not_satisfy(self) -> None:
        leftover = (
        ...
        for token in LEFTOVER_MISSION01_GET_PAYLOAD_WINDOW_NOT_LOCKED:
            self.assertNotIn(token, GET_PAYLOAD_WINDOW)

    def test_leftover_mission03_get_payload_window_does_not_satisfy(self) -> None:
        leftover = (
        ...
        for token in LEFTOVER_MISSION03_GET_PAYLOAD_WINDOW_NOT_LOCKED:
            self.assertNotIn(token, GET_PAYLOAD_WINDOW)

    def test_leftover_harbor_mission02_get_payload_window_does_not_satisfy(
        self,
    ) -> None:
        leftover = (
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetPayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_PAYLOAD_WINDOW))
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tconst FSkyguardPayloadWindowRuntime& "
            "GetPayloadWindow() const { return PayloadWindow; }\n"
            "};\n"
        )
    ...
    def test_leftover_mission04_get_payload_window_does_not_satisfy(self) -> None:
        leftover = (
        ...
        for token in LEFTOVER_MISSION04_GET_PAYLOAD_WINDOW_NOT_LOCKED:
            self.assertNotIn(token, GET_PAYLOAD_WINDOW)

    def test_leftover_mission05_get_payload_window_does_not_satisfy(self) -> None:
        leftover = (
        ...
        for token in LEFTOVER_MISSION05_GET_PAYLOAD_WINDOW_NOT_LOCKED:
            self.assertNotIn(token, GET_PAYLOAD_WINDOW)
        self.assertNotIn("Mission05", UFUNCTION_PAYLOAD)
    ...
    def test_contract_does_not_relock_leftover_mission05_surviving_count(
        ...
        self.assertNotIn("GetSurvivingTargetCount", GET_PAYLOAD_WINDOW)
        self.assertNotIn("Protection", UFUNCTION_PAYLOAD)
    ...
    def test_contract_does_not_relock_leftover_integration_window(self) -> None:
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_PAYLOAD_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", GET_PAYLOAD_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_PAYLOAD)
    ...
    def test_leftover_environment_get_payload_window_does_not_satisfy(self) -> None:
        leftover = (
        ...
        self.assertIn(CLASS_NAME, str(raised.exception))
        locked_only = f"{GET_PAYLOAD_WINDOW}\n"
        for token in LEFTOVER_ENVIRONMENT_GET_PAYLOAD_WINDOW_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_PAYLOAD_WINDOW)
        self.assertNotIn("IsCorePlayableReady", GET_PAYLOAD_WINDOW)
        self.assertNotIn("Environment", UFUNCTION_PAYLOAD)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission06_get_payload_window"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_get_payload_window"
                "_decl_contract.py"
            )
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission01_environment_readiness_defaults"
            "_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
