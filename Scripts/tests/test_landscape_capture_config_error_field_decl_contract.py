...
# DEFAULT IS bare `FString Error;` NOT `= false` / `= true` / `= 0`.
...
# Do NOT parse FSkyguardLandscapeVisibleAudit (leftover Error #1213 is a DIFFERENT struct).
...
# THIS IS NOT leftover CaptureSource sibling field.
# LOCK leftover landscape-capture-config-defaults #d6fd.
# LOCK leftover VisibleAudit Error #1213, leftover Footprint Error #1198,
# leftover HeightSample Error #1188, leftover CaptureSource, leftover Success.
...
# DEFAULT IS bare `FString Error;` NOT `= true` / `= false`. Fail closed if the type
...
# Error FIELD (declaration-only). It is
...
def alternate_bool_defaults(declaration: str) -> tuple[str, ...]:
    ...
    if "FName Error;" in declaration:
        forms.append(declaration.replace("FName Error;", "FString Error;"))
    if "FText Error;" in declaration:
        forms.append(declaration.replace("FText Error;", "FString Error;"))
    unique: list[str] = []
    for form in forms:
...
class LandscapeCaptureConfigErrorFieldDeclContractTests(unittest.TestCase):
    def test_landscape_capture_config_struct_exists(self) -> None:
        ...
        self.assertNotIn("ViewMode", LOCKED_DECL)
    ...
    def test_error_declaration_matches_origin_main(self) -> None:
        ...
        self.assertNotIn("FName Error;", section)
