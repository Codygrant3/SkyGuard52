...
# or lock the GetDebriefNarrative body.
# origin/main is one-line
# (`FText GetDebriefNarrative() const;`);
# accept that exact one-line prototype, split-line
...
# sortie-presentation fail-closed #7600. Stay off
# GetDebrief (leftover mission-debrief defaults).
# Do not lock FSkyguardMissionDebrief GetDebrief() const.
...
# Configure / GetPresentation, leftover
...
GET_DEBRIEF_NARRATIVE = ("FText GetDebriefNarrative() const;")
UFUNCTION_DEBRIEF = (
    'UFUNCTION(BlueprintPure, Category="Skyguard|Presentation|Debrief")'
)
# Leftover #56–#64 plus this class's production files.
# This lane only adds an isolated Python
# GetDebriefNarrative declaration contract. Stay off
# Configure, GetPresentation (sibling this wave),
# GetDebrief (leftover mission-debrief defaults),
# GetFinalScore, IsProgressSaved,
# GetPresentationState, AcknowledgeDebrief,
...
# GetDebrief, leftover GetFinalScore, leftover
# IsProgressSaved, leftover GetPresentationState,
# leftover AcknowledgeDebrief, leftover RetrySave,
# leftover TravelNext, leftover HandleDebriefKey,
# leftover sortie-presentation state enum, and dirty
# workspace paths.
LOCKED = {
...
# leftover AcknowledgeDebrief, leftover RetrySave,
# leftover TravelNext, leftover HandleDebriefKey,
# leftover sortie-presentation state enum, and
...
# GetFinalScore / IsProgressSaved / GetPresentationState /
...
# GetPresentationState / ESkyguardSortiePresentationState
# live in this class public section and stay unlocked
...
# .cpp GetDebriefNarrative body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main one-line prototype is accepted
# as presence; an inline body is accepted as presence,
# not a locked implementation contract.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "USkyguardDebriefWidget::GetDebriefNarrative",
    "SkyguardSortiePresentationWidgets.cpp",
)
SIBLING_TYPES = (
...
class DebriefWidgetGetDebriefNarrativeDeclContractTests(unittest.TestCase):
    def test_debrief_widget_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_DEBRIEF_NARRATIVE),
            section,
        )
    ...
    def test_briefing_widget_does_not_satisfy_class(self) -> None:
        briefing_only = (
            ...
            "{\n"
            "public:\n"
            "\tFText GetDebriefNarrative() const { return Other; }\n"
            "\tFText GetMissionTitle() const;\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_DEBRIEF_NARRATIVE)
        self.assertIn("GetDebriefNarrative", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_DEBRIEF_NARRATIVE))

    def test_missing_get_debrief_narrative_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tvoid Configure("
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_DEBRIEF_NARRATIVE)
        self.assertIn("GetDebriefNarrative", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_DEBRIEF_NARRATIVE)
        self.assertIn("GetDebriefNarrative", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("UFUNCTION", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("Category", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("BlueprintCallable", GET_DEBRIEF_NARRATIVE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_DEBRIEF_NARRATIVE)
        self.assertIn("GetDebriefNarrative", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tFText GetDebriefNarrative();\n"
        wrong_return_void = "\tvoid GetDebriefNarrative() const;\n"
        wrong_return_bool = "\tbool GetDebriefNarrative() const;\n"
        wrong_return_string = "\tFString GetDebriefNarrative() const;\n"
        added_arg = "\tFText GetDebriefNarrative(bool bReady) const;\n"
        leftover_get_debrief = (
            "\tFSkyguardMissionDebrief GetDebrief() const;\n"
        )
        wrong_name_presentation = (
            "\tUSkyguardSortiePresentationComponent* "
            "GetPresentation() const;\n"
        )
        ref_return = "\tconst FText& GetDebriefNarrative() const;\n"
        for region in (
            missing_const,
            wrong_return_void,
            wrong_return_bool,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_DEBRIEF_NARRATIVE)
            self.assertIn("GetDebriefNarrative", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_debrief_narrative_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_DEBRIEF_NARRATIVE),
            GET_DEBRIEF_NARRATIVE,
        )
        self.assertTrue(has_declaration(section, GET_DEBRIEF_NARRATIVE))
        self.assertEqual(
            declaration_count(section, GET_DEBRIEF_NARRATIVE),
            1,
        )
        self.assertTrue(
            GET_DEBRIEF_NARRATIVE.startswith("FText "),
            GET_DEBRIEF_NARRATIVE,
        )
        self.assertTrue(
            GET_DEBRIEF_NARRATIVE.endswith(";"),
            GET_DEBRIEF_NARRATIVE,
        )
        self.assertIn("GetDebriefNarrative()", GET_DEBRIEF_NARRATIVE)
        self.assertIn(" const", GET_DEBRIEF_NARRATIVE)
        self.assertIn("FText", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("GetDebrief()", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("INDEX_NONE", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("{", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("}", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("return ", GET_DEBRIEF_NARRATIVE)
    ...
    def test_declaration_accepts_origin_main_inline_and_prototype(
        ...
        self.assertTrue(
            has_declaration(one_line_prototype, GET_DEBRIEF_NARRATIVE)
        )
        self.assertEqual(
            require_declaration(one_line_prototype, GET_DEBRIEF_NARRATIVE),
            GET_DEBRIEF_NARRATIVE,
        )
        exact_inline = (
            "FText GetDebriefNarrative() const { return Other; }"
        )
        self.assertTrue(has_declaration(exact_inline, GET_DEBRIEF_NARRATIVE))
        self.assertEqual(
            require_declaration(exact_inline, GET_DEBRIEF_NARRATIVE),
            GET_DEBRIEF_NARRATIVE,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_DEBRIEF_NARRATIVE),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
        ...
        self.assertNotIn("{", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("return Other", GET_DEBRIEF_NARRATIVE)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tFText\n"
            "\tGetDebriefNarrative() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tFText GetDebriefNarrative(\n"
            "\t) const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_DEBRIEF_NARRATIVE}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_DEBRIEF_NARRATIVE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_DEBRIEF_NARRATIVE),
            section,
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_DEBRIEF_NARRATIVE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_DEBRIEF_NARRATIVE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_debrief_narrative_body(self) -> None:
        locked_only = f"{GET_DEBRIEF_NARRATIVE}\n"
        self.assertNotIn("{", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("}", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("return ", GET_DEBRIEF_NARRATIVE)
        ...
        self.assertNotIn("return false", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("return true", GET_DEBRIEF_NARRATIVE)

    def test_contract_does_not_relock_constructor(self) -> None:
        locked_only = f"{GET_DEBRIEF_NARRATIVE}\n"
        for neighbor in CONSTRUCTOR_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("ObjectInitializer", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_configure(self) -> None:
        ...
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("Configure(", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("GetPresentation()", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_get_final_score(self) -> None:
        ...
        for neighbor in GET_FINAL_SCORE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("GetFinalScore", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_is_progress_saved(self) -> None:
        ...
        for neighbor in IS_PROGRESS_SAVED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("IsProgressSaved", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_get_presentation_state(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("GetPresentationState", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_acknowledge_debrief(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("AcknowledgeDebrief", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_retry_save(self) -> None:
        ...
        for neighbor in RETRY_SAVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("RetrySave", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_travel_next(self) -> None:
        ...
        for neighbor in TRAVEL_NEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("TravelNext", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_handle_debrief_key(self) -> None:
        ...
        for neighbor in HANDLE_DEBRIEF_KEY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("HandleDebriefKey", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("Unconfigured", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("Warming", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("Launched", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_DEBRIEF_NARRATIVE,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("FSkyguardBriefingCard", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("FSkyguardHowToFlyRow", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("FillResultCombatStats", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("ASkyguardGunner", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("FillAndFinalize", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("FillAndFail", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("ApplyHydraForClusters", GET_DEBRIEF_NARRATIVE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_DEBRIEF_NARRATIVE}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_DEBRIEF_NARRATIVE),
            GET_DEBRIEF_NARRATIVE,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "debrief widget GetDebriefNarrative contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_DEBRIEF_NARRATIVE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"debrief widget GetDebriefNarrative contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_DEBRIEF_NARRATIVE)

    def test_contract_is_get_debrief_narrative_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_DEBRIEF_NARRATIVE.startswith("FText "))
        self.assertTrue(GET_DEBRIEF_NARRATIVE.endswith(";"))
        self.assertIn(" const", GET_DEBRIEF_NARRATIVE)
        self.assertNotIn("GetDebrief()", GET_DEBRIEF_NARRATIVE)
        self.assertIn(UFUNCTION_DEBRIEF, section)
