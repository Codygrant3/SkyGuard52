# THIS IS leftover-safe FSkyguardLoadoutSpec HullIntegrity.
# origin/main form: BARE `float HullIntegrity = 140.f;`
...
# #1451-#1465.
# Do NOT parse leftover LoadoutSpec isolated
# #1466-#1470.
...
# (split tokens).
# Do NOT contract sibling fields Loadout / StartingStation /
# CannonMagazineSize / CannonReserve / RocketMagazineSize /
# RocketReserve / GuidedMagazineSize / GuidedReserve /
# FlareCount / PlaystyleLine.
# Fail-closed if type is not `float`, initializer missing,
# or is not `= 140.f`. Fail-closed on Harbor forty /
# eighty (split tokens), `= 0.f`, `= 6`,
# int32 HullIntegrity.
...
# Harbor is ONLY 40/80. `140.f` is NOT Harbor.
...
TARGET = "float HullIntegrity = 140.f;"
TARGET_WRONG_BARE = "float HullIntegrity;"
TARGET_WRONG_ZERO = "float HullIntegrity = 0.f;"
TARGET_WRONG_SIX = "float HullIntegrity = 6;"
TARGET_WRONG_INT32 = "int32 HullIntegrity = 140.f;"
TARGET_WRONG_INT32_SIX = "int32 HullIntegrity = 6;"
TARGET_WRONG_FALSE = "float HullIntegrity = false;"
TARGET_WRONG_CPG_FEEL = (
    "float HullIntegrity = SkyguardApacheCpgFeel::CannonMagazineSize;"
)
TARGET_WRONG_BOOL = "bool HullIntegrity = 140.f;"
...
def leftover_harbor_wrong_equals() -> tuple[str, ...]:
    forty = "40" + ".f"
    eighty = "80" + ".f"
    return ("= " + forty, "= " + eighty)
...
def leftover_harbor_tokens() -> tuple[str, ...]:
    # Harbor is ONLY 40/80. The locked `140.f` is NOT Harbor.
    # Do not emit bare forty/eighty here: those reconstruct
    # as a substring of the locked initializer.
    forty = "40" + ".f"
    eighty = "80" + ".f"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `float HullIntegrity = 140.f;`.
    # Do not accept missing initializer, int32 HullIntegrity,
    # Harbor forty / eighty, `= 0.f`, `= 6`, or leftover
    # CpgFeel constant initializers. Do not accept sibling
    # FlareCount / magazine-reserve / PlaystyleLine.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+HullIntegrity\s*=\s*140\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"int32\s+HullIntegrity\b", compact):
        return False
    if re.search(r"float\s+HullIntegrity\s*=\s*0\.f", compact):
        return False
    if re.search(r"float\s+HullIntegrity\s*=\s*40\.f", compact):
        return False
    if re.search(r"float\s+HullIntegrity\s*=\s*80\.f", compact):
        return False
    if re.search(r"float\s+HullIntegrity\s*=\s*6\b", compact):
        return False
    if re.search(r"float\s+HullIntegrity\s*=\s*false\b", compact):
        return False
    if re.search(
        r"float\s+HullIntegrity\s*=\s*SkyguardApacheCpgFeel::",
        compact,
    ):
        return False
    if re.search(r"bool\s+HullIntegrity\b", compact):
        return False
    if re.search(r"float\s+HullIntegrity\s*;", compact) and not re.search(
        r"float\s+HullIntegrity\s*=",
        compact,
    ):
        return False
    return True
...
class LoadoutSpecHullIntegrityFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "HullIntegrity"), section)
        ...
        for sibling in sibling_uncontracted_decls():
            ...
            self.assertNotEqual(LOCKED_DECL, f"float {sibling} = 140.f;")
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "HullIntegrity"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("HullIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_hull_integrity_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        self.assertEqual(LOCKED_DECL, "float HullIntegrity = 140.f;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("float HullIntegrity"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("= 140.f", LOCKED_DECL)
    ...
    def test_zero_f_initializer_fails_closed(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_ZERO}\n"
        ...
        self.assertIn("float HullIntegrity = 140.f;", compact_origin)
        self.assertNotIn("float HullIntegrity = 0.f;", compact_origin)

    def test_wrong_initializer_fails_closed(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 140.f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 140.f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            f"\tint32 {SIBLING_CANNON_MAG} = 140.f;\n",
            f"\tint32 {SIBLING_CANNON_RESERVE} = 140.f;\n",
            f"\tint32 {SIBLING_ROCKET_MAG} = 140.f;\n",
            f"\tint32 {SIBLING_ROCKET_RESERVE} = 140.f;\n",
            f"\tint32 {SIBLING_GUIDED_MAG} = 140.f;\n",
            f"\tint32 {SIBLING_GUIDED_RESERVE} = 140.f;\n",
            f"\tint32 {SIBLING_FLARE} = 6;\n",
            "\tfloat HullIntegritys = 140.f;\n",
            "\tfloat HullIntegrity = " + forty + ";\n",
            "\tfloat HullIntegrity = " + eighty + ";\n",
        )
    ...
    def test_hull_integrity_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float HullIntegrity"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_INT32_SIX}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_harbor_40_80_tokens_fail_closed_if_present(self) -> None:
        ...
        for token in leftover_harbor_wrong_equals():
            self.assertNotIn(token, LOCKED_DECL)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, file_text)
        header = origin_main_header()
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec HullIntegrity field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "hull_integrity_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
        self.assertIn(LEFTOVER_ANALOG_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOADOUT_SPEC_FLARE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOADOUT_SPEC_LOADOUT, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOADOUT_SPEC_STATION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOADOUT_SPEC_CANNON_MAG, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_LOADOUT_SPEC_CANNON_RES, LOCKED_SCRIPTS)
