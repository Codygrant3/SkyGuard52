...
# INDEX_NONE or lock RequiredCoveredSeconds
# construction in the .cpp. This is leftover-safe
# Mission08 RequiredCoveredSeconds FIELD
# (declaration-only, one-line / split-line
# UPROPERTY wraps, ClampMin on the same line
# as Category or on a following wrap line,
# float default 4.f / 4.0f) on
# ASkyguardMission08IntegrationDirector. Note
# EditAnywhere / BlueprintReadWrite /
# Category Skyguard|Mission08|Hoist /
# float = 4.f, not VisibleAnywhere /
# BlueprintReadOnly / TObjectPtr. It is
# NOT leftover LifelineHunterSpawnLocation
...
# NOT leftover MinimumWeaponSeparationMeters
# (sibling this wave), NOT leftover
# MaximumProtectedTargetIntegrity, NOT leftover
# Readiness, NOT leftover retired-mount spawn
# location / rotation (skip leftover retired-
# mount spawn forever). It is
...
# #1044 through bAllowBoundedActorSpawning #1057.
...
# origin/main is a one-line UPROPERTY plus
# float RequiredCoveredSeconds = 4.f with
# ClampMin meta on the same Category line;
# accept that form, other one-line / split-line
# wraps, ClampMin on a following wrap line,
# and 4.0f / 4.f float default spellings
...
# Category is Skyguard|Mission08|Campaign, not
...
UPROPERTY_MISSION08_CLAMP = 'meta=(ClampMin="0.1")'
...
LOCKED_DECL = REQUIRED_COVERED_SECONDS_FIELD
...
# an isolated Python RequiredCoveredSeconds
...
LEFTOVER_MISSION08_MAXIMUM_PROTECTED_TARGET_INTEGRITY_NOT_LOCKED = (
    MAXIMUM_PROTECTED_TARGET_INTEGRITY_FIELD,
    "MaximumProtectedTargetIntegrity",
)
...
def float_default_equivalents(declaration: str) -> tuple[str, ...]:
    variants = [declaration]
    if " = 4.f" in declaration:
        variants.append(declaration.replace(" = 4.f", " = 4.0f"))
    if " = 4.0f" in declaration:
        variants.append(declaration.replace(" = 4.0f", " = 4.f"))
    return tuple(dict.fromkeys(variants))


def has_declaration(region: str, declaration: str) -> bool:
    for candidate in float_default_equivalents(declaration):
        if candidate in region:
            return True
        compact_region = collapsed(region)
        compact_decl = collapsed(candidate)
        if compact_decl in compact_region:
            return True
        stem = declaration_stem(candidate)
        if not stem:
            continue
        # Accept `;` or an inline `{` body after the signature
        # without locking that body. Accept 4.f / 4.0f.
        pattern = re.compile(re.escape(stem) + r"\s*(?:;|{|=)")
        if pattern.search(compact_region) is not None:
            return True
    return False
...
def declaration_count(region: str, declaration: str) -> int:
    ...
    for candidate in float_default_equivalents(declaration):
        if candidate in region:
            total = max(total, region.count(candidate))
            seen = True
            continue
        compact_region = collapsed(region)
        compact_decl = collapsed(candidate)
        if compact_decl in compact_region:
            total = max(total, compact_region.count(compact_decl))
            seen = True
            continue
        stem = declaration_stem(candidate)
        if not stem:
            continue
        pattern = re.compile(re.escape(stem) + r"\s*(?:;|{|=)")
        found = len(pattern.findall(compact_region))
        if found:
...
class Mission08RequiredCoveredSecondsFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("RequiredCoveredSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_required_covered_seconds_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin="0.1"', section)
        ...
        self.assertIn("Skyguard|Mission08|Hoist", UPROPERTY_MISSION08)
        self.assertNotIn("Skyguard|Mission08|Campaign", UPROPERTY_MISSION08)
        self.assertNotIn("Skyguard|Mission08|Safety", UPROPERTY_MISSION08)
    ...
    def test_required_covered_seconds_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float RequiredCoveredSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("RequiredCoveredSeconds", LOCKED_DECL)
        self.assertIn("= 4.f", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_float_4_0 = (
            "public:\n"
            "\tfloat RequiredCoveredSeconds = 4.0f;\n"
            "};\n"
        )
        header_wrap_type = (
            f"class SKYGUARD52_API {CLASS_NAME} "
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("RequiredCoveredSeconds", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float RequiredCoveredSeconds"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission08 RequiredCoveredSeconds field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission08 CampaignSaveUserIndex field contains "
    ...
    def test_contract_is_required_covered_seconds_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_required_covered_seconds(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission08_required_covered_seconds_field"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission08_required_covered_seconds_field"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_airfield_target_runtime_does_not_satisfy_required_covered_seconds(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
    ...
    def test_payload_window_runtime_does_not_satisfy_required_covered_seconds(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
    ...
    def test_search_track_runtime_does_not_satisfy_required_covered_seconds(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
    ...
    def test_protected_target_runtime_does_not_satisfy_required_covered_seconds(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
    ...
    def test_leftover_mission08_sibling_campaign_fields_stay_locked(
        ...
        for leftover in (
            "Scripts/tests/test_mission08_minimum_weapon_separation"
            "_meters_field_decl_contract.py",
            "Scripts/tests/test_mission08_lifeline_hunter_spawn"
            "_location_field_decl_contract.py",
            "Scripts/tests/test_mission08_lifeline_hunter_spawn"
            "_rotation_field_decl_contract.py",
            "Scripts/tests/test_mission08_campaign_save_user_index"
            "_field_decl_contract.py",
            "Scripts/tests/test_mission08_campaign_save_slot_name"
            "_field_decl_contract.py",
    ...
    def test_leftover_mission08_lifeline_and_safety_siblings_stay_locked(
        self,
    ) -> None:
        for leftover in (
            "Scripts/tests/test_mission08_lifeline_hunter_spawn"
            ...
            self.assertIn(leftover, LOCKED_SCRIPTS)
        leftover_location = f"\t{LIFELINE_HUNTER_SPAWN_LOCATION}\n"
        leftover_rotation = f"\t{LIFELINE_HUNTER_SPAWN_ROTATION}\n"
        leftover_sep = f"\t{MINIMUM_WEAPON_SEPARATION_FIELD}\n"
        leftover_integrity = (
            f"\t{MAXIMUM_PROTECTED_TARGET_INTEGRITY_FIELD}\n"
        )
        for region in (
            leftover_location,
            leftover_rotation,
    ...
    def test_declaration_accepts_4_0f_float_default_spelling(self) -> None:
        header = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            ": public AActor\n{\npublic:\n"
            "\tfloat RequiredCoveredSeconds = 4.0f;\n"
