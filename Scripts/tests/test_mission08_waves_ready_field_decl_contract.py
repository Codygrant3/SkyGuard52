...
# INDEX_NONE or lock bWavesReady
# construction in the .cpp. This is leftover-safe
# Mission08 IntegrationReadiness bWavesReady
...
# bWavesReady FIELD (declaration-only). It is
...
# FIELD #1003 / branch cursor/mission06-
# waves-ready-field-decl-contract-c332,
# NOT leftover Mission08 wave-state enum
# #68fc / branch cursor/mission08-
# wave-state-enum-contract-c332,
# NOT leftover Mission08 bObjectivesReady
# FIELD (sibling this wave) / branch cursor/
# mission08-objectives-ready-field-decl-
# contract-c332, NOT leftover Mission08
# bLifelineHunterReady FIELD (in-flight) /
# branch cursor/mission08-lifeline-hunter-
# ready-field-decl-contract-c332, NOT leftover
# Mission08 bGunnerReady FIELD #1069 / branch
# cursor/mission08-gunner-ready-field-decl-
# contract-c332,
...
WAVES_READY_TRUE = (
    "bool bWavesReady = true;"
)
LOCKED_DECL = WAVES_READY
...
# an isolated Python bWavesReady
...
class Mission08WavesReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bWavesReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_waves_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_waves_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bWavesReady"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bWavesReady", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bWavesReady", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bWavesReady"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bWavesReady", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission08 bWavesReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission08 bWavesReady contains "
    ...
    def test_contract_is_waves_ready_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_waves_ready(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_mission08_lifeline_hunter_ready"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        for token in LEFTOVER_SAME_NAME_GUNNER_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_SAME_NAME_WAVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        ...
        for token in LEFTOVER_MISSION08_OBJECTIVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION08_LIFELINE_HUNTER_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION07_WAVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
    ...
    def test_leftover_same_name_waves_ready_contracts_stay_locked(
        self,
    ) -> None:
