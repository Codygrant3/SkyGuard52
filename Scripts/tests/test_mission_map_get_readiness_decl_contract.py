...
# locking the body (`return Readiness;` is not a locked
# implementation contract). Nearby origin/main
...
# FSkyguardMissionMapReadiness field defaults. This is
# ASkyguardMissionMapAssemblyDirector::GetReadiness,
# not RebuildRouteSpline (sibling this wave), not
# ValidateAssembly (sibling this wave), not
# IsPointInsideFlightClearance (sibling this wave), not
...
# Python GetReadiness declaration
# contract. Stay off RebuildRouteSpline (sibling
# this wave), ValidateAssembly (sibling this wave),
# IsPointInsideFlightClearance (sibling this wave),
# and UPROPERTY fields on this class.
# Stay off leftover
# skyline-style enum #e017, leftover objective-anchor
# defaults #692c, leftover landmark-anchor defaults
...
# map-readiness field defaults stay sibling-only.
LOCKED_SCRIPTS = (
...
# IsPointInsideFlightClearance / UPROPERTY fields /
# leftover skyline style / leftover objective-anchor /
# leftover landmark-anchor / leftover map-readiness
# field defaults stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# .cpp GetReadiness body / invented INDEX_NONE stay
# unlocked. Do not invent INDEX_NONE or lock the
# inline body. origin/main inline `return Readiness`
# is accepted as present without locking that body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
class MissionMapGetReadinessDeclContractTests(unittest.TestCase):
    def test_mission_map_assembly_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_READINESS), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_READINESS))
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintCallable", GET_READINESS)
    ...
    def test_declaration_accepts_nearby_origin_main_ufunction(self) -> None:
        ...
        self.assertTrue(has_declaration(wrapped, GET_READINESS), wrapped)
        self.assertIn(UFUNCTION_NEARBY, wrapped)
        self.assertEqual(
            require_declaration(wrapped, GET_READINESS),
            GET_READINESS,
        )
        self.assertNotIn("UFUNCTION", GET_READINESS)
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        wrong_return_void = "\tvoid GetReadiness() const;\n"
        wrong_return_bool = "\tbool GetReadiness() const;\n"
        by_value = (
            "\tconst FSkyguardMissionMapReadiness GetReadiness() const;\n"
        )
        mutable_ref = (
            "\tFSkyguardMissionMapReadiness& GetReadiness() const;\n"
        )
        added_arg = (
            "\tconst FSkyguardMissionMapReadiness& GetReadiness("
            "bool bForce) const;\n"
    ...
    def test_get_readiness_declaration_matches_origin_main(self) -> None:
        ...
        self.assertEqual(declaration_count(section, GET_READINESS), 1)
        self.assertTrue(
            GET_READINESS.startswith("const FSkyguardMissionMapReadiness& "),
            GET_READINESS,
        )
        self.assertTrue(GET_READINESS.endswith(";"), GET_READINESS)
        self.assertTrue(
        ...
        self.assertIn("FSkyguardMissionMapReadiness", GET_READINESS)
        ...
        self.assertNotIn("UFUNCTION", GET_READINESS)
        self.assertNotIn("static ", GET_READINESS)
        self.assertNotEqual(GET_READINESS, GET_READINESS_INLINE)

    def test_declaration_accepts_origin_main_inline_and_prototype(
        self,
    ) -> None:
        one_line_prototype = f"{{\npublic:\n\t{GET_READINESS}\n}}\n"
        self.assertTrue(has_declaration(one_line_prototype, GET_READINESS))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_READINESS),
            GET_READINESS,
        )
        exact_inline = GET_READINESS_INLINE
        self.assertTrue(has_declaration(exact_inline, GET_READINESS))
        self.assertEqual(
            require_declaration(exact_inline, GET_READINESS),
            GET_READINESS,
        )
        self.assertEqual(declaration_count(exact_inline, GET_READINESS), 1)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_READINESS), section)
        self.assertEqual(
            require_declaration(section, GET_READINESS),
        ...
        self.assertTrue(has_declaration(section, GET_READINESS_INLINE), section)
        self.assertNotIn("{", GET_READINESS)
        self.assertNotIn("return Readiness", GET_READINESS)
        self.assertNotEqual(GET_READINESS, GET_READINESS_INLINE)
    ...
    def test_contract_does_not_lock_get_readiness_body(self) -> None:
        locked_only = f"{GET_READINESS}\n"
        self.assertNotIn("{", GET_READINESS)
        self.assertNotIn("}", GET_READINESS)
        self.assertNotIn("return ", GET_READINESS)
        self.assertNotIn("return Readiness", GET_READINESS)
        self.assertNotIn("return Readiness", locked_only)
        self.assertNotIn(
            "ASkyguardMissionMapAssemblyDirector::GetReadiness",
            GET_READINESS,
        )
    ...
    def test_contract_does_not_relock_rebuild_route_spline(self) -> None:
        ...
        self.assertNotIn("RebuildRouteSpline", GET_READINESS)
    ...
    def test_contract_does_not_relock_validate_assembly(
        ...
        for neighbor in VALIDATE_ASSEMBLY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_READINESS)
        self.assertNotIn("ValidateAssembly", GET_READINESS)
        self.assertNotIn("ValidateAssembly", locked_only)

    def test_contract_does_not_relock_is_point_inside(self) -> None:
        locked_only = f"{GET_READINESS}\n"
        for neighbor in IS_POINT_INSIDE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_READINESS)
        self.assertNotIn("IsPointInsideFlightClearance", GET_READINESS)
    ...
    def test_contract_does_not_relock_uprop_fields(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", GET_READINESS)
        self.assertNotIn("UPROPERTY", locked_only)
        self.assertNotIn("SkylineStyle", GET_READINESS)
        self.assertNotIn("ObjectiveAnchors", GET_READINESS)
        self.assertNotIn("LandmarkAnchors", GET_READINESS)
        self.assertNotIn("FlightRouteSpline", GET_READINESS)
    ...
    def test_contract_does_not_relock_leftover_skyline_style_enum(self) -> None:
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_READINESS)
        self.assertNotIn("ESkyguardMissionSkylineStyle", GET_READINESS)
    ...
    def test_contract_does_not_relock_leftover_map_defaults(self) -> None:
        ...
        for token in LEFTOVER_MAP_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_READINESS)
        self.assertNotIn("FSkyguardMissionObjectiveAnchor", GET_READINESS)
        self.assertNotIn("FSkyguardMissionLandmarkAnchor", GET_READINESS)
        self.assertNotIn("bool bDefinitionValid = false;", GET_READINESS)
        self.assertNotIn("int32 RoutePointCount = 0;", GET_READINESS)
        self.assertNotIn("float RouteLengthCentimeters = 0.f;", GET_READINESS)
        self.assertNotIn(
            "test_mission_objective_anchor_defaults_contract.py",
            GET_READINESS,
        )
    ...
    def test_contract_does_not_relock_leftover_validate_definition(
        ...
        for token in LEFTOVER_VALIDATE_DEFINITION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_READINESS)
        self.assertNotIn("ValidateDefinition", GET_READINESS)
        self.assertNotIn("ValidateDefinition", locked_only)
        self.assertNotIn("USkyguardMissionDefinition", GET_READINESS)
        self.assertNotIn("USkyguardCampaignDefinition", GET_READINESS)
    ...
    def test_contract_does_not_relock_leftover_pathfinder_contracts(
        ...
        for token in LEFTOVER_PATHFINDER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_READINESS)
        self.assertNotIn("AdvanceEncounter", GET_READINESS)
        self.assertNotIn("GetTelegraphsTriggered", GET_READINESS)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_READINESS)
        self.assertNotIn("Unconfigured", GET_READINESS)
        self.assertNotIn("Warming", GET_READINESS)
        self.assertNotIn("Launched", GET_READINESS)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            GET_READINESS,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_READINESS)
        self.assertNotIn("FSkyguardBriefingCard", GET_READINESS)
        self.assertNotIn("FSkyguardBriefingRadioRow", GET_READINESS)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_READINESS)
        self.assertNotIn("FillResultCombatStats", GET_READINESS)
        self.assertNotIn("ASkyguardGunner", GET_READINESS)
        self.assertNotIn("FillAndFinalize", GET_READINESS)
        self.assertNotIn("FillAndFail", GET_READINESS)
        self.assertNotIn("ApplyHydraForClusters", GET_READINESS)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("SkyguardMissionDefinition.h", GET_READINESS)
        self.assertNotIn("SkyguardCampaignDefinition.h", GET_READINESS)
    ...
    def test_harbor_industrial_is_not_a_harbor_clock_token(self) -> None:
        ...
        self.assertNotIn(leftover_style, GET_READINESS)
        locked_only = f"{GET_READINESS}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-map GetReadiness contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"mission-map GetReadiness contains {banned}; "
    ...
    def test_contract_is_get_readiness_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(GET_READINESS.endswith(";"))
        self.assertTrue(GET_READINESS.endswith(" const;"))
        self.assertIn("GetReadiness()", GET_READINESS)
        self.assertNotEqual(GET_READINESS, GET_READINESS_INLINE)
        self.assertIn(UFUNCTION_NEARBY, section)
