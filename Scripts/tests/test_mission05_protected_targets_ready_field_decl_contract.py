...
# Mission05 IntegrationReadiness bProtectedTargetsReady
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool default false,
# one-line / split-line UPROPERTY wraps) on
# FSkyguardMission05IntegrationReadiness. SKIP leftover
# retired-mount runtime-ready forever. This contract
# is bProtectedTargetsReady only. It is
# NOT leftover retired-mount spawn location /
# spawn rotation / runtime-ready struct field,
# NOT leftover Mission05 protected-target enum
# #aaa2 / leftover protected-target-runtime-
# defaults #ec8b,
# NOT leftover Mission10 bProtectedGroupsReady
# FIELD #818 / branch cursor/mission10-
# protected-groups-ready-field-decl-contract-c332,
# NOT leftover Mission05 bGunnerReady sibling
# FIELD #967 / branch cursor/mission05-
# gunner-ready-field-decl-contract-c332,
# NOT leftover Mission05 bTempestReady sibling
# FIELD #966 / branch cursor/mission05-
# tempest-ready-field-decl-contract-c332,
# NOT leftover Mission05 bObjectivesReady sibling
...
PROTECTED_TARGETS_READY_FIELD = (
    "bool bProtectedTargetsReady = false;"
)
PROTECTED_TARGETS_READY_FIELD_TRUE = (
...
LOCKED_DECL = PROTECTED_TARGETS_READY_FIELD
...
LEFTOVER_SAME_NAME_READINESS_DRAFTS_NOT_LOCKED = (
...
class Mission05ProtectedTargetsReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_true = (
            "public:\n"
            f"\t{UPROPERTY_MISSION10}\n"
            f"\t{PROTECTED_TARGETS_READY_FIELD_TRUE}\n"
            "};\n"
        )
        wrap_true_one_line = (
            "public:\n"
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, PROTECTED_TARGETS_READY_FIELD_TRUE),
                section,
            )
            self.assertEqual(
                require_declaration(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 bProtectedTargetsReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission05 bProtectedTargetsReady contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission05_protected_targets_ready"
                "_field_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_same_name_ready_and_readiness_contracts_stay_locked(
        self,
    ) -> None:
        for leftover in LEFTOVER_SAME_NAME_GUNNER_READY_NOT_LOCKED:
            self.assertIn("Scripts/tests/" + leftover, LOCKED_SCRIPTS)
        for leftover in LEFTOVER_SAME_NAME_READINESS_DRAFTS_NOT_LOCKED:
            self.assertIn("Scripts/tests/" + leftover, LOCKED_SCRIPTS)
        for leftover in LEFTOVER_MISSION05_GUNNER_READY_NOT_LOCKED:
            if leftover.endswith(".py"):
                self.assertIn("Scripts/tests/" + leftover, LOCKED_SCRIPTS)
        for leftover in LEFTOVER_MISSION05_TEMPEST_READY_NOT_LOCKED:
            if leftover.endswith(".py"):
                self.assertIn("Scripts/tests/" + leftover, LOCKED_SCRIPTS)
        for leftover in LEFTOVER_MISSION05_OBJECTIVES_READY_NOT_LOCKED:
            if leftover.endswith(".py"):
                self.assertIn("Scripts/tests/" + leftover, LOCKED_SCRIPTS)
        ...
        self.assertIn(
            "Scripts/tests/test_mission10_protected_groups_ready"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
