# THIS IS leftover-safe FSkyguardBossDefinition WeakPoints.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly)
# then BARE `TArray<FSkyguardBossWeakPointDefinition> WeakPoints;`
# (no initializer).
...
# if Category= is added, if the type is not
# TArray<FSkyguardBossWeakPointDefinition>,
# or if an initializer is added.
...
# except as the TArray element type on this field.
...
# Do NOT parse leftover boss-definition-defaults
# #187 / #cc27.
...
# DefeatObjectiveId / MaximumBreakupPieces.
# THIS IS NOT leftover boss-definition-defaults
# #187 / #cc27.
# THIS IS NOT leftover USkyguardMissionDefinition::Boss #358.
# THIS IS NOT leftover FSkyguardBossWeakPointDefinition
# isolated fields.
# THIS IS NOT leftover boss-weak-point-defaults #188 / #2242.
# THIS IS NOT leftover enemy-wave-defaults #189 / #dc07.
# THIS IS NOT leftover enemy-wave Formations field decl.
...
TARGET = "TArray<FSkyguardBossWeakPointDefinition> WeakPoints;"
TARGET_WRONG_EQ_NONE = (
    "TArray<FSkyguardBossWeakPointDefinition> WeakPoints = {};"
)
TARGET_WRONG_FALSE = (
    "TArray<FSkyguardBossWeakPointDefinition> WeakPoints = false;"
)
TARGET_WRONG_TRUE = (
    "TArray<FSkyguardBossWeakPointDefinition> WeakPoints = true;"
)
TARGET_WRONG_ZERO = (
    "TArray<FSkyguardBossWeakPointDefinition> WeakPoints = 0.f;"
)
TARGET_WRONG_FLOAT = "float WeakPoints;"
TARGET_WRONG_FNAME = "FName WeakPoints;"
TARGET_WRONG_TARRAY_FNAME = "TArray<FName> WeakPoints;"
...
SIBLING_DEFEAT_OBJECTIVE = "DefeatObjectiveId"
SIBLING_MAXIMUM_BREAKUP = "MaximumBreakupPieces"
WEAK_POINT_BODY_ID = "WeakPointId"
WEAK_POINT_BODY_REQUIRED_WEAPON = "RequiredWeapon"
WEAK_POINT_BODY_EXPOSES = "ExposesWeakPointId"
WEAK_POINT_BODY_INTEGRITY = "Integrity"
SIBLING_WAVE_ID = "WaveId"
SIBLING_START_TIME = "StartTimeSeconds"
SIBLING_COMPLETION_OBJECTIVE = "CompletionObjectiveId"
FORMATION_BODY_ID = "FormationId"
FORMATION_BODY_UNIT_COUNT = "UnitCount"
FORMATION_BODY_SPACING = "SpacingCentimeters"
FORMATION_BODY_TYPE = "ESkyguardFormationType"
...
LEFTOVER_COMPONENT_MAX_INTEGRITY = (
    "Scripts/tests/test_weak_point_max_integrity_field_decl_contract.py"
)
SIBLING_WAVE_ID_SCRIPT = (
    "Scripts/tests/test_enemy_wave_wave_id"
    "_field_decl_contract.py"
)
SIBLING_START_TIME_SCRIPT = (
    "Scripts/tests/test_enemy_wave_start_time_seconds"
    "_field_decl_contract.py"
)
REMAINING_COMPLETION_OBJECTIVE_SCRIPT = (
    "Scripts/tests/test_enemy_wave_completion_objective_id"
    "_field_decl_contract.py"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_BOSS_ID,
        SIBLING_CALLSIGN,
        SIBLING_DEFEAT_OBJECTIVE,
        SIBLING_MAXIMUM_BREAKUP,
    )


def leftover_formation_body_tokens() -> tuple[str, ...]:
    return leftover_weak_point_body_tokens()


def leftover_weak_point_body_tokens() -> tuple[str, ...]:
    return (
        WEAK_POINT_BODY_ID,
        WEAK_POINT_BODY_REQUIRED_WEAPON,
        WEAK_POINT_BODY_EXPOSES,
        WEAK_POINT_BODY_INTEGRITY,
    )


def leftover_boss_definition_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_ENEMY_WAVE_FORMATIONS,
        LEFTOVER_BOSS_DEFINITION_DEFAULTS,
        LEFTOVER_BOSS_DEFINITION_DEFAULTS_PY,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # origin/main has a bare TArray. Do not accept sibling
    # BossId / Callsign / DefeatObjectiveId /
    # MaximumBreakupPieces fields. Type must be TArray of
    # FSkyguardBossWeakPointDefinition. Harbor 40/80
    # fail-closed on the accepted decl and parse window.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FSkyguardBossWeakPointDefinition\s*>"
        r"\s+WeakPoints\s*=",
        compact,
    ):
        return False
    if re.search(
        r"(?:FName|float|bool|int32)\s+WeakPoints\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+WeakPoints\b",
        compact,
    ):
        return False
    if re.search(
    ...
    forty = "40" + ".f"
    eighty = "80" + ".f"
    if forty in compact or eighty in compact:
        return False
    return True
...
def spec_section(header: str) -> str:
    ...
    if forty in section or eighty in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes harbor clock tokens"
        )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FSkyguardBossWeakPointDefinition"
            r"\s*>\s+WeakPoints\b",
            compact[index:],
        ):
    ...
    raise AssertionError(
        "UPROPERTY for TArray<FSkyguardBossWeakPointDefinition> "
        "WeakPoints is missing from "
...
def require_weak_points_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} WeakPoints UPROPERTY"
        )
    if "BlueprintReadWrite" in specifiers:
        raise AssertionError(
            "BlueprintReadWrite instead of BlueprintReadOnly on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} WeakPoints"
        )
...
class BossDefinitionWeakPointsFieldDeclContractTests(unittest.TestCase):
    def test_boss_definition_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "WeakPoints"), section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedBossDefinition\n{\n};\n"
            )
    ...
    def test_later_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "WeakPoints"), section)
        self.assertNotIn(STOP_BEFORE_BOSS_WEAK_POINT, section)
        self.assertNotIn(STOP_BEFORE_ENEMY_FORMATION, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_POINT, section)
        self.assertNotIn(STOP_BEFORE_ROUTE_DEFINITION, section)
        ...
        self.assertIn("WeakPoints", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_weak_points_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_weak_points_specifiers(section)
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("WeakPoints = NAME_None", compact_origin)
        self.assertNotIn("WeakPoints = false", compact_origin)
        self.assertNotIn("WeakPoints = true", compact_origin)
        self.assertNotIn("WeakPoints = 0.f", compact_origin)
        self.assertNotIn("WeakPoints = 160.f", compact_origin)
        self.assertNotIn("WeakPoints = {}", compact_origin)

    def test_weak_points_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("FSkyguardBossWeakPointDefinition", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_TARRAY_FNAME}\n",
            "\tTArray<FSkyguardBossWeakPointDefinition> WeakPoint;\n",
            "\tint32 WeakPoints;\n",
            "\tbool WeakPoints;\n",
            "\tfloat WeakPoints = " + forty + ";\n",
            "\tfloat WeakPoints = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_write_fails_closed(self) -> None:
        origin = require_weak_points_specifiers(
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weak_points_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weak_points_specifiers(spec_section(read_write))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weak_points_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_weak_points_specifiers(spec_section(with_category))
    ...
    def test_does_not_contract_sibling_boss_definition_fields(self) -> None:
        ...
        for leftover in leftover_formation_body_tokens():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, LOCKED_DECL)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for leftover in leftover_formation_body_tokens():
            ...
            self.assertNotIn(leftover, LOCKED_DECL)
        self.assertIn(BOSS_WEAK_POINT_NAME, section)
        self.assertIn("TArray<" + BOSS_WEAK_POINT_NAME + ">", collapsed(section))
        self.assertNotIn("struct " + BOSS_WEAK_POINT_NAME, section)
        self.assertNotIn("struct " + BOSS_WEAK_POINT_NAME, leaked)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, section)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, leaked)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, section)
        self.assertNotIn(STOP_BEFORE_SCORE_RULES, leaked)
    ...
    def test_harbor_40_80_tokens_fail_closed_if_present(self) -> None:
        ...
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn(forty, section)
        self.assertNotIn(eighty, section)
        for token in leftover_harbor_tokens():
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-definition WeakPoints field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "boss_definition_weak_points_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionDefinition.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(SIBLING_WAVE_ID_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(SIBLING_START_TIME_SCRIPT, LOCKED_SCRIPTS)
        self.assertIn(REMAINING_COMPLETION_OBJECTIVE_SCRIPT, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_WEAK_POINT_ID_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_REQUIRED_WEAPON_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_EXPOSES_WEAK_POINT_ID_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_INTEGRITY_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_WEAK_POINT_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_EXPOSED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_DESTROYED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_INTEGRITY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_COMPONENT_MAX_INTEGRITY, LOCKED_SCRIPTS)
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = (
            ...
            *leftover_radio_defaults_scripts(),
            ...
            REMAINING_LOOK_AHEAD_SCRIPT,
            REMAINING_ALLOW_COMBAT_ORBIT_SCRIPT,
            *leftover_boss_definition_scripts(),
        )
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_DEFEAT_OBJECTIVE, locked_only)
        self.assertNotIn(SIBLING_MAXIMUM_BREAKUP, locked_only)
        self.assertNotIn(SIBLING_WAVE_ID, locked_only)
        self.assertNotIn(SIBLING_START_TIME, locked_only)
        self.assertNotIn(SIBLING_COMPLETION_OBJECTIVE, locked_only)
        ...
        self.assertNotIn(SCORE_RULES_NAME, locked_only)
