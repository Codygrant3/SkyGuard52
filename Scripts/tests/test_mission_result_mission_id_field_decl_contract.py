...
# int32 / required initializer, RETARGET: specifiers are
# EditAnywhere + BlueprintReadWrite, type is FName, BARE
# decl with NO initializer, no Category=.
...
# not FName, an initializer is added, or Category= is added.
...
# Match EXACT identifier MissionId as a whole field name
# after FName. Do NOT claim leftover FinalScore / MedalTier.
# Siblings stay unclaimed: bMissionSucceeded / ShotsFired /
...
TARGET_WRONG_ZERO = "FName MissionId = 0;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `= 0` / leftover int32 FinalScore when origin/main
    # has a bare FName. Do not accept sibling fields.
...
class MissionResultMissionIdFieldDeclContractTests(unittest.TestCase):
    def test_mission_result_struct_exists(self) -> None:
        ...
        self.assertNotIn("FName MissionId)", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-result MissionId field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_result_mission_id_field_decl_contract.py"
        ))
