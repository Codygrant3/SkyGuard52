...
# INDEX_NONE or lock RouteExclusion construction in
# the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector, the PCG
# route-exclusion box field. Do not lock
# sibling environment fields Root / OceanTiles /
# BeachTiles / LandTiles / LandScatterBounds /
...
# leftover bool bRouteExclusionValid, leftover
# IsRouteExclusionSafe(), leftover
# environment-readiness defaults (#6b9d /
# #b931), leftover GetReadiness, leftover Mission01
...
# not Environment alone, not Mission02 / Boss /
...
B_ROUTE_EXCLUSION_VALID = "bool bRouteExclusionValid = false;"
...
# RouteExclusion field declaration contract on
...
# environment OceanTiles / BeachTiles / LandTiles
# field contracts stay unlocked by this file.
# Leftover Mission01 Integration field
...
LEFTOVER_ROUTE_EXCLUSION_HELPERS_NOT_LOCKED = (
    IS_ROUTE_EXCLUSION_SAFE,
    B_ROUTE_EXCLUSION_VALID,
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::RouteExclusion",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01RouteExclusionFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ROUTE_EXCLUSION_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ROUTE_EXCLUSION_FIELD)
        self.assertIn("RouteExclusion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ROUTE_EXCLUSION_FIELD))

    def test_missing_route_exclusion_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BEACH_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ROUTE_EXCLUSION_FIELD)
        self.assertIn("RouteExclusion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_PCG}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ROUTE_EXCLUSION_FIELD)
        self.assertIn("RouteExclusion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PCG, section)
        ...
        self.assertIn("|PCG", UPROPERTY_PCG)
        self.assertTrue(UPROPERTY_PCG.endswith('|PCG")'), UPROPERTY_PCG)
        self.assertNotEqual(
            UPROPERTY_PCG,
            "UPROPERTY(VisibleAnywhere, BlueprintReadOnly, "
        ...
        self.assertNotIn("UPROPERTY", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("VisibleAnywhere", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("BlueprintReadOnly", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("Category", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_PCG)
        self.assertNotIn("BlueprintCallable", UPROPERTY_PCG)
        self.assertIn("Skyguard|Mission01|Environment|PCG", UPROPERTY_PCG)
        self.assertIn("VisibleAnywhere", UPROPERTY_PCG)
        self.assertIn("BlueprintReadOnly", UPROPERTY_PCG)
        self.assertNotIn("EditAnywhere", UPROPERTY_PCG)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_PCG)
        self.assertNotIn("Mission02", UPROPERTY_PCG)
        self.assertNotIn("Boss", UPROPERTY_PCG)
        self.assertNotIn("Destruction", UPROPERTY_PCG)
        self.assertNotIn("Apache", UPROPERTY_PCG)
        self.assertNotIn("Mission07", UPROPERTY_PCG)
        self.assertNotIn("Mission10", UPROPERTY_PCG)
        self.assertNotIn("Encounter", UPROPERTY_PCG)
        self.assertNotIn("Safety", UPROPERTY_PCG)
        self.assertNotIn(
            'Category="Skyguard|Mission01"',
            UPROPERTY_PCG,
        )
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_PCG)
            self.assertNotIn(invented, ROUTE_EXCLUSION_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ROUTE_EXCLUSION_FIELD)
        self.assertIn("RouteExclusion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUBoxComponent* RouteExclusion;\n"
        )
        as_array = (
            "\tTArray<TObjectPtr<"
            "UBoxComponent>> "
        ...
        scene = "\tTObjectPtr<USceneComponent> RouteExclusion;\n"
        static_mesh = (
            "\tTObjectPtr<UStaticMeshComponent> RouteExclusion;\n"
        )
        weak = (
            "\tTWeakObjectPtr<"
            "UBoxComponent> "
        ...
        leftover_route = f"\t{OCEAN_TILES_FIELD}\n"
        ...
        leftover_is_safe = f"\t{IS_ROUTE_EXCLUSION_SAFE}\n"
        leftover_valid = f"\t{B_ROUTE_EXCLUSION_VALID}\n"
        for region in (
            raw_pointer,
            as_array,
            scene,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ROUTE_EXCLUSION_FIELD)
            self.assertIn("RouteExclusion", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_route_exclusion_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ROUTE_EXCLUSION_FIELD),
            ROUTE_EXCLUSION_FIELD,
        )
        self.assertTrue(has_declaration(section, ROUTE_EXCLUSION_FIELD))
        self.assertEqual(
            declaration_count(section, ROUTE_EXCLUSION_FIELD),
            1,
        )
        self.assertTrue(
            ROUTE_EXCLUSION_FIELD.startswith(
                "TObjectPtr<UBoxComponent> "
            ),
            ROUTE_EXCLUSION_FIELD,
        )
        self.assertTrue(ROUTE_EXCLUSION_FIELD.endswith(";"), ROUTE_EXCLUSION_FIELD)
        self.assertIn("RouteExclusion", ROUTE_EXCLUSION_FIELD)
        self.assertIn(
            "UBoxComponent",
            ROUTE_EXCLUSION_FIELD,
        )
        self.assertIn("TObjectPtr<", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("TArray<", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("TSoftObjectPtr", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("=", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("INDEX_NONE", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("UFUNCTION", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("{", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("}", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("return ", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("OceanTiles", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("BeachTiles", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("LandTiles", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("LandScatterBounds", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("InlandVegetationPCG", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("ProductionLandscape", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("AuthoredPCGGraph", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetReadiness", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("Briefing", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("AudioDirector", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("RadioChatter", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("SortiePresentation", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("CampaignDefinition", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("MissionDefinition", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("Readiness", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetAircraft", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("ConfigureFromMission", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("AdvanceBriefing", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("SetAssetsReady", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("CanLaunch", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetElapsedSeconds", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetBriefingState", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetBriefingText", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetRadioChatter", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetPresentation", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("GetMissionTitle", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("AcknowledgeBriefing", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("LaunchSortie", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("HullCollider", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("OpticalTracker", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("WeaponServo", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("CountermeasurePod", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("RadarNode", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("HarborIndustrial", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("MaxIntegrity", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("CurrentIntegrity", ROUTE_EXCLUSION_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ROUTE_EXCLUSION_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PCG} {ROUTE_EXCLUSION_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ROUTE_EXCLUSION_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ROUTE_EXCLUSION_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ROUTE_EXCLUSION_FIELD),
            section,
    ...
    def test_assigned_route_exclusion_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UBoxComponent> "
            "RouteExclusion = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ROUTE_EXCLUSION_FIELD)
        self.assertIn("RouteExclusion", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ROUTE_EXCLUSION_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            f"\t{B_ROUTE_EXCLUSION_VALID}\n",
        ):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ROUTE_EXCLUSION_FIELD)
            self.assertIn("RouteExclusion", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ROUTE_EXCLUSION_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ROUTE_EXCLUSION_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ROUTE_EXCLUSION_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ROUTE_EXCLUSION_FIELD.startswith("UFUNCTION"),
            ROUTE_EXCLUSION_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ROUTE_EXCLUSION_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_route_exclusion_cpp_body(self) -> None:
        locked_only = f"{ROUTE_EXCLUSION_FIELD}\n"
        self.assertNotIn("{", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("}", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("return ", ROUTE_EXCLUSION_FIELD)
        ...
        self.assertNotIn("return false", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("return true", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ROUTE_EXCLUSION_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ROUTE_EXCLUSION_FIELD)
        self.assertIn("RouteExclusion", ROUTE_EXCLUSION_FIELD)
        self.assertIn(
            "UBoxComponent",
            ROUTE_EXCLUSION_FIELD,
    ...
    def test_contract_does_not_relock_leftover_route_exclusion_helpers(
        self,
    ) -> None:
        locked_only = f"{ROUTE_EXCLUSION_FIELD}\n"
        for token in LEFTOVER_ROUTE_EXCLUSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("bRouteExclusionValid", ROUTE_EXCLUSION_FIELD)
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ROUTE_EXCLUSION_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("CurrentIntegrity", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ROUTE_EXCLUSION_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ROUTE_EXCLUSION_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ROUTE_EXCLUSION_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ROUTE_EXCLUSION_FIELD),
            ROUTE_EXCLUSION_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ROUTE_EXCLUSION_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, ROUTE_EXCLUSION_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 RouteExclusion field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ROUTE_EXCLUSION_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ROUTE_EXCLUSION_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ROUTE_EXCLUSION_FIELD.lower(),
                "mission01 RouteExclusion contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ROUTE_EXCLUSION_FIELD)

    def test_contract_is_route_exclusion_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("RouteExclusion", ROUTE_EXCLUSION_FIELD)
        self.assertNotIn("=", ROUTE_EXCLUSION_FIELD)
        self.assertTrue(ROUTE_EXCLUSION_FIELD.endswith(";"))
        self.assertIn(UPROPERTY_PCG, section)

    def test_sibling_environment_fields_do_not_satisfy_route_exclusion(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ROUTE_EXCLUSION_FIELD))
            self.assertNotEqual(ROUTE_EXCLUSION_FIELD, leftover)
