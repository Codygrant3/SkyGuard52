...
# not lock Breakwater here). Keep DebrisEngine
# distinct from leftover Breakwater DebrisEngine,
# leftover RunwayBreaker DebrisEngine, and
# sibling RoadHunter Engine. Weak-point Engine /
# LeftActuator must not satisfy this mesh field.
# This is not a
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardRoadHunterBoss::DebrisEngine",
    "SkyguardRoadHunterBoss.cpp",
    "SetExposed",
...
class RoadHunterDebrisEngineFieldDeclContractTests(unittest.TestCase):
    ...
    def test_left_actuator_weak_point_does_not_satisfy_mesh_field(self) -> None:
        leftover = f"\t{LEFT_ACTUATOR_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_ENGINE_FIELD)
        self.assertIn("DebrisEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, DEBRIS_ENGINE_FIELD))
        self.assertNotEqual(LEFT_ACTUATOR_FIELD, DEBRIS_ENGINE_FIELD)
        self.assertIn("USkyguardBossWeakPointComponent", LEFT_ACTUATOR_FIELD)
        self.assertIn("UStaticMeshComponent", DEBRIS_ENGINE_FIELD)
        self.assertNotIn("LeftActuator", DEBRIS_ENGINE_FIELD)
        self.assertNotIn("DebrisLeftWing", LEFT_ACTUATOR_FIELD)
    ...
    def test_contract_does_not_lock_debris_engine_cpp_body(self) -> None:
        ...
        self.assertNotIn("SkyguardRoadHunterBoss.cpp", DEBRIS_ENGINE_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("DebrisEngine", locked_only)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardRoadHunterBoss::DebrisEngine", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "road hunter DebrisEngine field contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                ...
                "road hunter DebrisEngine contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_relock_sibling_road_hunter_fields(self) -> None:
        ...
        self.assertNotIn("DebrisRightWing", DEBRIS_ENGINE_FIELD)
        self.assertIn("DebrisEngine", DEBRIS_ENGINE_FIELD)
        self.assertNotIn(ROAD_HUNTER_ENGINE_FIELD, DEBRIS_ENGINE_FIELD)
    ...
    def test_road_hunter_engine_is_not_breakwater_engine(self) -> None:
        locked_only = f"{DEBRIS_ENGINE_FIELD}\n"
        self.assertNotEqual(ROAD_HUNTER_ENGINE_FIELD, DEBRIS_ENGINE_FIELD)
    ...
    def test_road_hunter_debris_engine_is_not_breakwater_debris_engine(
        ...
        for token in LEFTOVER_RUNWAY_BREAKER_DEBRIS_ENGINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_ENGINE_FIELD)
        self.assertNotIn("ASkyguardBreakwaterBoss", DEBRIS_ENGINE_FIELD)
        self.assertNotIn("ASkyguardRunwayBreakerBoss", DEBRIS_ENGINE_FIELD)
        self.assertIn("DebrisEngine", DEBRIS_ENGINE_FIELD)
        self.assertNotIn("DebrisLeftWing", DEBRIS_ENGINE_FIELD)

    def test_road_hunter_engine_weak_point_does_not_satisfy_debris_engine(
        self,
    ) -> None:
        leftover = f"\t{ROAD_HUNTER_ENGINE_FIELD}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, DEBRIS_ENGINE_FIELD)
        self.assertIn("DebrisEngine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_breakwater_and_runway_classes_do_not_satisfy(self) -> None:
        for other_name in (
            "ASkyguardBreakwaterBoss",
            "ASkyguardRunwayBreakerBoss",
        ):
            other = (
                f"class SKYGUARD52_API {other_name} "
                ": public AActor\n"
                "{\n"
                "public:\n"
                f"\t{DEBRIS_ENGINE_FIELD}\n"
