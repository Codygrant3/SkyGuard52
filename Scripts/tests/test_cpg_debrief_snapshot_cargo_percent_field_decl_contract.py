# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot CargoPercent.
...
# not int32, initializer is missing or is not `= 100`, or
# someone writes `= 0` / `= 0.f` / `= false`.
# Exact identifier vs siblings Score, Medal, ShotsFired,
# and Hits (same type, different initializer). Do not
# claim sibling fields: bValid, bWon, MissionTitle,
# OutcomeNarrative, Score, Medal, ShotsFired, Hits,
# bRadarDead, DestroyedSystems, SelectedLoadout,
...
# Harbor 40/80 fail-closed. `= 100` on int32 is not Harbor.
...
TARGET = "int32 CargoPercent = 100;"
TARGET_WRONG_NONE = "int32 CargoPercent;"
TARGET_WRONG_FALSE = "int32 CargoPercent = false;"
TARGET_WRONG_TRUE = "int32 CargoPercent = true;"
TARGET_WRONG_FLOAT = "int32 CargoPercent = 0.f;"
TARGET_WRONG_ZERO = "int32 CargoPercent = 0;"
TARGET_WRONG_ONE = "int32 CargoPercent = 1;"
...
TARGET_WRONG_FNAME = "FName CargoPercent;"
TARGET_WRONG_BOOL = "bool CargoPercent = 100;"
TARGET_WRONG_FLOAT_TYPE = "float CargoPercent = 100;"
LOCKED_DECL = TARGET
...
LEFTOVER_CPG_DEBRIEF_SNAPSHOT_OUTCOME = (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 CargoPercent = 100;`.
    # Do not accept missing initializer, `= 0`, `= 0.f`,
    # `= false`, sibling Score / Medal / ShotsFired / Hits,
    # or a leftover theater UPROPERTY wrap as the locked decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"int32\s+CargoPercent\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"int32\s+CargoPercent\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+CargoPercent\s*=\s*true\b", compact):
        return False
    if re.search(r"int32\s+CargoPercent\s*=\s*0\b", compact):
        return False
    if re.search(r"(?:float|bool|FName|FString)\s+CargoPercent\b", compact):
        return False
    if re.search(r"int32\s+CargoPercent\s*=\s*100\s*;", compact) is None:
        return False
    return True
...
class CpgDebriefSnapshotCargoPercentFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CargoPercent"), section)
    ...
    def test_capture_function_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CargoPercent"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        ...
        self.assertIn("CargoPercent", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cargo_percent_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_cargo_percent_is_plain_int32_hundred(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
        ...
        self.assertEqual(LOCKED_DECL, "int32 CargoPercent = 100;")
        self.assertTrue(LOCKED_DECL.startswith("int32 CargoPercent"), LOCKED_DECL)
        ...
        self.assertIn("int32 CargoPercent = 100;", compact)
        self.assertNotIn("CargoPercent = 0.f", compact)
        self.assertNotIn("CargoPercent = false", compact)
        self.assertNotIn("CargoPercent = 0;", compact)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 CargoPercents = 100;\n",
            "\tint32 CargoPercentValue = 100;\n",
            "\tint32 FinalCargoPercent = 100;\n",
            "\tfloat CargoPercent = " + forty + ";\n",
            "\tfloat CargoPercent = " + eighty + ";\n",
        )
    ...
    def test_exact_identifier_vs_same_type_siblings(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CargoPercent"), section)
        self.assertIn("CargoPercent", LOCKED_DECL)
        ...
        self.assertTrue(
            has_one_declaration(section, "int32 Score = 0;"),
            section,
        )
        self.assertNotIn("Score", LOCKED_DECL)
        self.assertNotIn("= 0;", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot CargoPercent field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cargo_percent_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_OUTCOME, LOCKED_SCRIPTS)
