# THIS IS leftover-safe FSkyguardProductionAudioEntry OutputSubmix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundSubmixBase> OutputSubmix;`
# (no initializer).
...
# TSoftObjectPtr<USoundSubmixBase>, initializer added,
# Category= added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardProductionAudioEntry` body ONLY after
# `struct FSkyguardProductionAudioEntry`. Stop before
# `struct FSkyguardProductionAudioRouting`.
# Routing siblings MasterSubmix / CockpitSubmix /
# ExteriorSubmix / WeaponsSubmix / ExplosionsSubmix live
# AFTER the stop token — do NOT claim them.
...
# Sibling decls stay unclaimed: Category / DisplayName /
# SourceStatus / Sound / Attenuation / Concurrency /
# ProvenanceId / SourceSha256.
# Analog leftover production-audio-entry-defaults #198 /
# #314b then isolated OutputSubmix field decl.
# THIS IS NOT leftover DisplayName #1395.
# THIS IS NOT leftover ProvenanceId #1396.
# THIS IS NOT leftover Sound #1397.
# THIS IS NOT leftover SourceSha256 #1398.
# THIS IS NOT leftover Attenuation #1399.
# THIS IS NOT leftover production-audio-routing-defaults
# #fc92.
# THIS IS NOT leftover audio-source-status-enum #241.
# THIS IS NOT leftover audio-production-bank-empty-fail-closed.
...
# BlueprintReadOnly / Category="Skyguard|Theater" / FText,
# retarget: type is TSoftObjectPtr<USoundSubmixBase>,
# no `=`, BlueprintReadWrite, no Category.
...
STRUCT_NAME = "FSkyguardProductionAudioEntry"
ACTOR_NAME = "ASkyguardCampaignTheaterKit"
NAMESPACE_NAME = "SkyguardCampaignTheaterKit"
TARGET = "TSoftObjectPtr<USoundSubmixBase> OutputSubmix;"
TARGET_WRONG_EQ_NONE = (
    "TSoftObjectPtr<USoundSubmixBase> OutputSubmix = nullptr;"
)
TARGET_WRONG_FALSE = (
    "TSoftObjectPtr<USoundSubmixBase> OutputSubmix = false;"
)
TARGET_WRONG_TRUE = (
    "TSoftObjectPtr<USoundSubmixBase> OutputSubmix = true;"
)
TARGET_WRONG_ZERO = (
    "TSoftObjectPtr<USoundSubmixBase> OutputSubmix = 0.f;"
)
TARGET_WRONG_FLOAT = "float OutputSubmix;"
TARGET_WRONG_FNAME = "FName OutputSubmix;"
TARGET_WRONG_BOOL = "bool OutputSubmix;"
TARGET_WRONG_INT = "int32 OutputSubmix;"
TARGET_WRONG_FTEXT = "FText OutputSubmix;"
TARGET_WRONG_FSTRING = "FString OutputSubmix;"
TARGET_WRONG_PTR = "USoundSubmixBase* OutputSubmix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> OutputSubmix;"
TARGET_WRONG_SOUND = "TSoftObjectPtr<USoundBase> OutputSubmix;"
TARGET_WRONG_ATTENUATION = (
    "TSoftObjectPtr<USoundAttenuation> OutputSubmix;"
)
TARGET_WRONG_CONCURRENCY = (
    "TSoftObjectPtr<USoundConcurrency> OutputSubmix;"
)
...
SIBLING_CATEGORY_FIELD = "Category"
SIBLING_DISPLAY_NAME = "DisplayName"
SIBLING_SOURCE_STATUS = "SourceStatus"
SIBLING_SOUND = "Sound"
SIBLING_ATTENUATION = "Attenuation"
SIBLING_CONCURRENCY = "Concurrency"
SIBLING_PROVENANCE_ID = "ProvenanceId"
SIBLING_SOURCE_SHA256 = "SourceSha256"
ROUTING_SIBLING_MASTER = "MasterSubmix"
ROUTING_SIBLING_COCKPIT = "CockpitSubmix"
ROUTING_SIBLING_EXTERIOR = "ExteriorSubmix"
ROUTING_SIBLING_WEAPONS = "WeaponsSubmix"
ROUTING_SIBLING_EXPLOSIONS = "ExplosionsSubmix"
...
LEFTOVER_PRODUCTION_AUDIO_ENTRY_SOURCE_SHA256 = (
    "Scripts/tests/test_production_audio_entry_source_sha256"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ENTRY_ATTENUATION = (
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_CATEGORY_FIELD,
        SIBLING_DISPLAY_NAME,
        SIBLING_SOURCE_STATUS,
        SIBLING_SOUND,
        SIBLING_ATTENUATION,
        SIBLING_CONCURRENCY,
        SIBLING_PROVENANCE_ID,
        SIBLING_SOURCE_SHA256,
    )


def routing_sibling_tokens() -> tuple[str, ...]:
    return (
        ROUTING_SIBLING_MASTER,
        ROUTING_SIBLING_COCKPIT,
        ROUTING_SIBLING_EXTERIOR,
        ROUTING_SIBLING_WEAPONS,
        ROUTING_SIBLING_EXPLOSIONS,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling Category / DisplayName / SourceStatus / Sound
    # fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+OutputSubmix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundSubmixBase\s*>\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundAttenuation\s*>\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundConcurrency\s*>\s+OutputSubmix\b",
        compact,
    ):
        return False
    if re.search(
...
def parse_window(header: str) -> str:
    ...
    stop_token = STOP_BEFORE_ROUTING
    ...
    # live-copy tokens and leftover source-status enum stay
    # OUT of the parse window.
    ...
    for leaked in routing_sibling_tokens():
        if leaked in window:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes {leaked}"
            )
...
def struct_body(header: str) -> str:
    window = header
    if STRUCT_RE.search(header) is not None and STOP_BEFORE_ROUTING in header:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+OutputSubmix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundSubmixBase> OutputSubmix is missing from "
...
class ProductionAudioEntryOutputSubmixFieldDeclContractTests(
    unittest.TestCase
):
    def test_production_audio_entry_struct_exists(self) -> None:
    ...
    def test_missing_output_submix_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tESkyguardProductionAudioCategory {SIBLING_CATEGORY_FIELD} = "
            "ESkyguardProductionAudioCategory::EngineIdle;\n"
            f"\tFName {SIBLING_PROVENANCE_ID};\n"
            "};\n"
        )
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("OutputSubmix = nullptr", compact_origin)
        self.assertNotIn("OutputSubmix = false", compact_origin)
        self.assertNotIn("OutputSubmix = true", compact_origin)
        self.assertNotIn("OutputSubmix = 0.f", compact_origin)
        self.assertNotIn("OutputSubmix = 160.f", compact_origin)

    def test_output_submix_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SOUND}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ATTENUATION}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CONCURRENCY}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SOUND}\n",
            f"\t{TARGET_WRONG_ATTENUATION}\n",
            f"\t{TARGET_WRONG_CONCURRENCY}\n",
            f"\t{TARGET_WRONG_FSTRING}\n",
            f"\t{TARGET_WRONG_HEALTH}\n",
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_PROVENANCE_ID};\n",
            f"\tFString {SIBLING_SOURCE_SHA256};\n",
            f"\tFText {SIBLING_DISPLAY_NAME};\n",
            "\tTSoftObjectPtr<USoundSubmixBase> OutputSubmixes;\n",
            "\tint32 OutputSubmix = 1;\n",
            "\tbool OutputSubmix;\n",
            "\tfloat OutputSubmix = " + forty + ";\n",
            "\tfloat OutputSubmix = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_entry_fields(self) -> None:
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, SIBLING_CATEGORY_FIELD), section)
        self.assertTrue(has_identifier(section, SIBLING_DISPLAY_NAME), section)
        self.assertTrue(has_identifier(section, SIBLING_SOURCE_STATUS), section)
        self.assertTrue(has_identifier(section, SIBLING_SOUND), section)
        self.assertTrue(has_identifier(section, SIBLING_ATTENUATION), section)
        self.assertTrue(has_identifier(section, SIBLING_CONCURRENCY), section)
        self.assertTrue(has_identifier(section, SIBLING_PROVENANCE_ID), section)
        self.assertTrue(has_identifier(section, SIBLING_SOURCE_SHA256), section)
    ...
    def test_does_not_parse_neighbors_or_leftover_banks(self) -> None:
        ...
        for leaked_routing in routing_sibling_tokens():
            self.assertNotIn(leaked_routing, section)
            self.assertNotIn(leaked_routing, leaked)
            self.assertNotIn(leaked_routing, window)
            self.assertNotIn(leaked_routing, LOCKED_DECL)
        self.assertIn(STOP_BEFORE_CATEGORY_ENUM, header)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry OutputSubmix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ENTRY_DISPLAY_NAME, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ENTRY_PROVENANCE_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ENTRY_SOUND, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ENTRY_SOURCE_SHA256, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ENTRY_ATTENUATION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_PRODUCTION_AUDIO_ROUTING_DEFAULTS, LOCKED_SCRIPTS)
        self.assertTrue(Path(__file__).name.endswith(
            "output_submix_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        for leaked_routing in routing_sibling_tokens():
            self.assertNotIn(leaked_routing, locked_only)
