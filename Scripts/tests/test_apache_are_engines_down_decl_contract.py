...
# or lock the AreEnginesDown body in the .cpp.
...
# IsCanopyGlassCracked, and remaining unused neighbors
...
# #107–#414, and leftover SortiePresentationWidgets
# stay sibling-only.
ARE_ENGINES_DOWN = (
    "bool AreEnginesDown() const;"
)
...
# AreEnginesDown declaration contract on
...
# leftover IsCanopyGlassCracked sibling, and dirty
# workspace paths.
LOCKED = {
...
# AreEnginesDown return type is not a leftover
# roster value lock.
LEFTOVER_NOT_LOCKED = (
    "ESkyguardApacheSystem",
...
# .cpp AreEnginesDown body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::AreEnginesDown",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheAreEnginesDownDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ARE_ENGINES_DOWN), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ARE_ENGINES_DOWN)
        self.assertIn("AreEnginesDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ARE_ENGINES_DOWN))

    def test_missing_are_engines_down_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ARE_ENGINES_DOWN)
        self.assertIn("AreEnginesDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ARE_ENGINES_DOWN)
        self.assertIn("AreEnginesDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ARE_ENGINES_DOWN), section)
        self.assertNotIn("BlueprintPure", ARE_ENGINES_DOWN)
        self.assertNotIn("UFUNCTION", ARE_ENGINES_DOWN)
        self.assertNotIn("Category", ARE_ENGINES_DOWN)
        self.assertNotIn("BlueprintCallable", ARE_ENGINES_DOWN)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ARE_ENGINES_DOWN)
        self.assertIn("AreEnginesDown", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool AreEnginesDown const;\n"
        wrong_return_void = "\tvoid AreEnginesDown() const;\n"
        wrong_return_float = "\tfloat AreEnginesDown() const;\n"
        wrong_return_int = "\tint32 AreEnginesDown() const;\n"
        missing_const = "\tbool AreEnginesDown();\n"
        added_arg = "\tbool AreEnginesDown(float Extra) const;\n"
        leftover_canopy = (
            "\tbool IsCanopyGlassCracked() const "
            "{ return bCanopyGlassCracked; }\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ARE_ENGINES_DOWN)
            self.assertIn("AreEnginesDown", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_are_engines_down_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ARE_ENGINES_DOWN),
            ARE_ENGINES_DOWN,
        )
        self.assertTrue(has_declaration(section, ARE_ENGINES_DOWN))
        self.assertEqual(declaration_count(section, ARE_ENGINES_DOWN), 1)
        self.assertTrue(
            ARE_ENGINES_DOWN.startswith("bool "),
            ARE_ENGINES_DOWN,
        )
        self.assertTrue(ARE_ENGINES_DOWN.endswith(";"), ARE_ENGINES_DOWN)
        self.assertIn("AreEnginesDown()", ARE_ENGINES_DOWN)
        self.assertIn(" const", ARE_ENGINES_DOWN)
        self.assertNotIn("INDEX_NONE", ARE_ENGINES_DOWN)
        self.assertNotIn("{", ARE_ENGINES_DOWN)
        self.assertNotIn("}", ARE_ENGINES_DOWN)
        self.assertNotIn("return ", ARE_ENGINES_DOWN)
        self.assertNotIn("IsCanopyGlassCracked", ARE_ENGINES_DOWN)
        self.assertNotIn("IsChinTurretDown", ARE_ENGINES_DOWN)
        self.assertNotIn("IsRotorDown", ARE_ENGINES_DOWN)
        self.assertNotIn("IsSystemDown", ARE_ENGINES_DOWN)
        self.assertNotIn("GetEnginePowerScale", ARE_ENGINES_DOWN)
        self.assertNotIn("GetDamageFraction", ARE_ENGINES_DOWN)
        self.assertNotIn("ApplyDamage", ARE_ENGINES_DOWN)
        self.assertNotIn("GetForwardSpeed", ARE_ENGINES_DOWN)
        self.assertNotIn("FaceWorldLocation", ARE_ENGINES_DOWN)
        self.assertNotIn("ApplySystemHit", ARE_ENGINES_DOWN)
        self.assertNotIn("ApplyHit", ARE_ENGINES_DOWN)
        self.assertNotIn("IsSensorLive", ARE_ENGINES_DOWN)
        self.assertNotIn("IsThermalAvailable", ARE_ENGINES_DOWN)
        self.assertNotIn("IsSensorViewActive", ARE_ENGINES_DOWN)
        self.assertNotIn("GetChinMuzzleLocation", ARE_ENGINES_DOWN)
        self.assertNotIn("GetChinTurret", ARE_ENGINES_DOWN)
        self.assertNotIn("GetRotorPowerScale", ARE_ENGINES_DOWN)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tAreEnginesDown() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, ARE_ENGINES_DOWN),
                section,
            )
            self.assertEqual(
                require_declaration(section, ARE_ENGINES_DOWN),
            ...
            self.assertEqual(
                declaration_count(section, ARE_ENGINES_DOWN),
                1,
            )
        one_line = f"{{\npublic:\n\t{ARE_ENGINES_DOWN}\n}}\n"
        self.assertTrue(has_declaration(one_line, ARE_ENGINES_DOWN))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ARE_ENGINES_DOWN), section)
        self.assertEqual(
            require_declaration(section, ARE_ENGINES_DOWN),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{ARE_ENGINES_DOWN}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", ARE_ENGINES_DOWN)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", ARE_ENGINES_DOWN)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_are_engines_down_cpp_body(self) -> None:
        locked_only = f"{ARE_ENGINES_DOWN}\n"
        self.assertNotIn("{", ARE_ENGINES_DOWN)
        self.assertNotIn("}", ARE_ENGINES_DOWN)
        self.assertNotIn("return ", ARE_ENGINES_DOWN)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", ARE_ENGINES_DOWN)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", ARE_ENGINES_DOWN)
        self.assertNotIn("return true", ARE_ENGINES_DOWN)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{ARE_ENGINES_DOWN}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ARE_ENGINES_DOWN)
        self.assertNotIn("GetGunnerMount", ARE_ENGINES_DOWN)
        self.assertNotIn("GetEyeMount", ARE_ENGINES_DOWN)
        self.assertNotIn("GetWeaponMount", ARE_ENGINES_DOWN)
        self.assertNotIn("GetChinTurret", ARE_ENGINES_DOWN)
        self.assertNotIn("GetPilotMount", ARE_ENGINES_DOWN)
        self.assertNotIn("GetSensorTurret", ARE_ENGINES_DOWN)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{ARE_ENGINES_DOWN}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ARE_ENGINES_DOWN)
        self.assertNotIn("GetChinMuzzleLocation", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_aim_chin_turret(self) -> None:
        ...
        for neighbor in AIM_CHIN_TURRET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ARE_ENGINES_DOWN)
        self.assertNotIn("AimChinTurret", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_set_rotor_power(self) -> None:
        ...
        for neighbor in SET_ROTOR_POWER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ARE_ENGINES_DOWN)
        self.assertNotIn("SetRotorPower", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_issue_pilot_command(self) -> None:
        ...
        for neighbor in ISSUE_PILOT_COMMAND_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ARE_ENGINES_DOWN)
        self.assertNotIn("IssuePilotCommand", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_get_pilot_helpers(self) -> None:
        ...
        for neighbor in GET_PILOT_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ARE_ENGINES_DOWN)
        self.assertNotIn("GetPilotCommand", ARE_ENGINES_DOWN)
        self.assertNotIn("GetPilotConfirmationsIssued", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ARE_ENGINES_DOWN)
        self.assertNotIn("SetOrbitFocus", ARE_ENGINES_DOWN)
        self.assertNotIn("FaceWorldLocation", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ARE_ENGINES_DOWN)
        self.assertNotIn("SetSensorView", ARE_ENGINES_DOWN)
        self.assertNotIn("SetFirstPersonInterior", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ARE_ENGINES_DOWN)
        self.assertNotIn("ESkyguardApacheSystem", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_leftover_direct_flight_input_sibling(
        ...
        self.assertNotIn("SetDirectFlightInput", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ARE_ENGINES_DOWN)
        self.assertNotIn("Pursuit", ARE_ENGINES_DOWN)
        self.assertNotIn("OrbitLeft", ARE_ENGINES_DOWN)
        self.assertNotIn("AttackRun", ARE_ENGINES_DOWN)
        self.assertNotIn("FaceTarget", ARE_ENGINES_DOWN)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, ARE_ENGINES_DOWN)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", ARE_ENGINES_DOWN)
        self.assertNotIn("USkyguardBriefingWidget", ARE_ENGINES_DOWN)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            ARE_ENGINES_DOWN,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ARE_ENGINES_DOWN)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ARE_ENGINES_DOWN)
        self.assertNotIn("ESkyguardMissionSkylineStyle", ARE_ENGINES_DOWN)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ARE_ENGINES_DOWN}\n"
        self.assertEqual(
            require_declaration(locked_only, ARE_ENGINES_DOWN),
            ARE_ENGINES_DOWN,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache AreEnginesDown contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, ARE_ENGINES_DOWN.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache AreEnginesDown contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ARE_ENGINES_DOWN)

    def test_contract_is_are_engines_down_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", ARE_ENGINES_DOWN)
        self.assertNotIn("{", ARE_ENGINES_DOWN)
        self.assertTrue(ARE_ENGINES_DOWN.startswith("bool "))
        self.assertTrue(ARE_ENGINES_DOWN.endswith(";"))
        self.assertIn(" const", ARE_ENGINES_DOWN)
        self.assertIn(UFUNCTION_APACHE, section)
