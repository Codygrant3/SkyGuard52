# THIS IS leftover-safe FSkyguardWeatherProfile CloudCoverage.
# Analog leftover weather-profile-defaults #184 then isolated
# per-field decl. Do NOT reopen leftover #184.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "0.0", ClampMax = "1.0"))
# then `float CloudCoverage = 0.25f;`.
# THIS IS leftover-safe isolated UPROPERTY with NO Category.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# Type IS float. Initializer IS `= 0.25f`.
# meta ClampMin / ClampMax spaces around `=` are required.
...
# if type is not float, if initializer is not `= 0.25f`,
# if meta ClampMin / ClampMax is missing or wrong,
...
# Do NOT parse leftover #357 Weather on the mission
# definition class.
# Do NOT contract sibling fields ProfileId / Weather /
# TimeOfDayHours / WindSpeedMetersPerSecond / Precipitation.
...
# THIS IS NOT leftover Weather field-decl branch.
# THIS IS NOT leftover Precipitation field-decl branch.
# THIS IS NOT leftover TheaterKit #1299-#1331.
# THIS IS NOT leftover #1332-#1353.
# Analog leftover briefing-card-defaults then isolated
# CardId field decls.
# If a clone asserts Category="Skyguard|Theater" /
# VisibleAnywhere / bare FName, retarget: type is float,
# initializer is `= 0.25f`, no Category.
...
TARGET = "float CloudCoverage = 0.25f;"
TARGET_WRONG_BARE = "float CloudCoverage;"
TARGET_WRONG_FALSE = "float CloudCoverage = false;"
TARGET_WRONG_TRUE = "float CloudCoverage = true;"
TARGET_WRONG_ZERO_D = "float CloudCoverage = 0.0f;"
TARGET_WRONG_ONE = "float CloudCoverage = 1.f;"
TARGET_WRONG_PRECIP = "float CloudCoverage = 0.f;"
TARGET_WRONG_FNAME = "FName CloudCoverage;"
TARGET_WRONG_INT = "int32 CloudCoverage = 0;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float CloudCoverage = 0.25f;`.
    # Do not accept bare `float CloudCoverage;` / `= 0.0f` /
    # `= 1.f` / `= 0.f` / leftover Precipitation when
    # origin/main has `= 0.25f`. Do not accept sibling
    # ProfileId / Weather / TimeOfDayHours /
    # WindSpeedMetersPerSecond / Precipitation fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+CloudCoverage\s*=\s*0\.25f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"FName\s+CloudCoverage\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CloudCoverage\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CloudCoverage\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CloudCoverage\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CloudCoverage is missing from "
...
class WeatherProfileCloudCoverageFieldDeclContractTests(unittest.TestCase):
    def test_weather_profile_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CloudCoverage"), section)
    ...
    def test_neighbor_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CloudCoverage"), section)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, section)
        self.assertNotIn(MISSION_DEF_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CloudCoverage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cloud_coverage_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float CloudCoverage = 0.25f;", compact_origin)
        self.assertNotIn("CloudCoverage = NAME_None", compact_origin)
        self.assertNotIn("CloudCoverage = false", compact_origin)
        self.assertNotIn("CloudCoverage = true", compact_origin)
        self.assertNotIn("CloudCoverage = 160.f", compact_origin)
        self.assertNotIn("CloudCoverage = 0.0f", compact_origin)
        self.assertNotIn("CloudCoverage = 1.f", compact_origin)

    def test_cloud_coverage_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CloudCoverage"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 0.25f", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ZERO_D}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 0.f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 0.f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            f"\t{TARGET_WRONG_TRUE}\n",
            f"\t{TARGET_WRONG_ZERO_D}\n",
            f"\t{TARGET_WRONG_ONE}\n",
            f"\t{TARGET_WRONG_PRECIP}\n",
            ...
            leftover_guided,
            f"\tFName {SIBLING_PROFILE_ID};\n",
            f"\tfloat {SIBLING_PRECIPITATION} = 0.f;\n",
            "\tfloat CloudCoverages = 0.25f;\n",
            "\tint32 CloudCoverage = 0;\n",
            "\tbool CloudCoverage;\n",
            "\tfloat CloudCoverage = " + forty + ";\n",
            "\tfloat CloudCoverage = " + eighty + ";\n",
        )
    ...
    def test_category_or_wrong_specifiers_fail_closed(self) -> None:
        ...
        self.assertNotIn("ClampMax", no_clamp)
        ...
        self.assertIn('ClampMax = "24.0"', wrong_max)
        self.assertNotIn('ClampMax = "1.0"', wrong_max)
        self.assertIn('ClampMax = "1.0"', origin)
        ...
        self.assertNotIn('ClampMin = "0.0"', nospace)
        self.assertNotIn('ClampMax = "1.0"', nospace)
        self.assertIn('ClampMin = "0.0"', origin)
        self.assertIn('ClampMax = "1.0"', origin)
    ...
    def test_does_not_contract_sibling_weather_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "weather-profile CloudCoverage field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cloud_coverage_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_WEATHER_PROFILE_TIME_OF_DAY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEATHER_PROFILE_WIND_SPEED, LOCKED_SCRIPTS)
