# THIS IS leftover-safe ASkyguardGunner CpgReticleH.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgReticleH;`
# (no initializer).
...
# Stop BEFORE sibling CpgReticleV as a claimed slot.
...
# CpgTedacBezel, CpgReticleV, CpgGripLeft, Boom,
# GunnerCamera.
# Do NOT claim leftover CpgTedac #1601, leftover
# CpgDash #1600, leftover CpgCanopyBow #1599,
# leftover CpgSeatPan #1598, leftover CpgKneeRight
# #1597, leftover CpgKneeLeft #1596, leftover
# CpgSeatBack #1595, leftover CpgCockpit #1594,
# leftover CpgStation #1593. CpgEufd and
# CpgTedacBezel are sibling in-flight slots.
# CpgMpdLeft and CpgMpdRight are leftover once
# drafted this pass. Do NOT claim CpgReticleV,
# CpgTedacBezel, or CpgEufd as this slot.
# CpgReticleH fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgReticleH;`
...
# CpgReticleH fail-closed if LOCKED_DECL is leftover
...
# CpgTedacBezel / CpgReticleV / leftover Apache Canopy /
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgReticleH;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgReticleH = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgReticleH = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgReticleH = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgReticleH;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgReticleH;"
...
CPG_RETICLE_H_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgReticleH\s*;"
)
CPG_RETICLE_H_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgReticleH\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Apache Canopy, or an initializer on CpgReticleH
    # as the slot.
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_RETICLE_V):
            break
    ...
    if has_identifier(window, SIBLING_CPG_RETICLE_V):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_RETICLE_V}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgReticleH\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgReticleH "
...
class GunnerCpgReticleHFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgReticleH"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_RETICLE_V), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_RETICLE_V),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgReticleH"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgReticleH", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_reticle_h_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgReticleH;", compact_origin)
        self.assertNotIn("CpgReticleH = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_reticle_h_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgReticleH"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgReticleH;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgReticleHs;\n",
            "\tint32 CpgReticleH = 140;\n",
            "\tbool CpgReticleH = true;\n",
            "\tfloat CpgReticleH = " + forty + ";\n",
            "\tfloat CpgReticleH = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_reticle_v_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgReticleH field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_reticle_h_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
