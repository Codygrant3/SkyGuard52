...
# or lock the AcknowledgeDebrief body in the .cpp.
...
# campaign-subsystem AcknowledgeDebrief (#322). This
# is not leftover CPG debrief fail-closed #8ccd.
# This is not leftover mission-debrief defaults.
# This is not leftover sortie-presentation
# fail-closed #7600. Stay off GetDebrief (leftover
# mission-debrief defaults). Stay off leftover
# briefing-widget AcknowledgeBriefing (#391).
# Leftover briefing-fail-closed, leftover
# briefing-card defaults, leftover briefing-radio-row
# defaults, leftover how-to-fly-row defaults,
...
# commands / loadout / lock-phase, leftover drafts
# #56–#64, leftover sortie-presentation state enum,
# leftover mission-debrief-state enum, leftover
# skyline style HarborIndustrial (leftover enum, not
# a Harbor 40/80 retune), leftover campaign
# subsystem AcknowledgeDebrief, leftover
# presentation-component AcknowledgeDebrief, leftover
# BriefingWidget AcknowledgeBriefing, and sibling
# DebriefWidget isolated contracts stay sibling-only.
ACKNOWLEDGE_DEBRIEF = "bool AcknowledgeDebrief();"
UFUNCTION_DEBRIEF = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Presentation|Debrief")'
)
...
# AcknowledgeDebrief declaration contract on
# USkyguardDebriefWidget. Stay off Configure,
# GetPresentation, GetDebrief, GetDebriefNarrative,
# GetFinalScore, IsProgressSaved,
# GetPresentationState, RetrySave, TravelNext, and
# HandleDebriefKey on this class. Stay off leftover
# campaign-subsystem AcknowledgeDebrief #322,
# leftover CPG debrief fail-closed #8ccd, leftover
# mission-debrief defaults, leftover
# sortie-presentation fail-closed #7600, leftover
# briefing-widget AcknowledgeBriefing #391, leftover
...
# USkyguardCampaignSubsystem, leftover
# presentation-component AcknowledgeDebrief,
...
# campaign-subsystem AcknowledgeDebrief #322,
# leftover BriefingWidget Configure /
# GetPresentation / GetMissionTitle /
# GetBriefingText / AcknowledgeBriefing /
# LaunchSortie, leftover CPG debrief fail-closed
# #8ccd, leftover mission-debrief defaults, leftover
# sortie-presentation fail-closed #7600, leftover
# briefing-fail-closed, leftover briefing-card
# defaults, leftover briefing-radio-row defaults,
# leftover how-to-fly-row defaults, leftover
...
# mission-debrief-state enum, leftover skyline
# HarborIndustrial, sibling DebriefWidget
# Configure / GetPresentation / GetDebrief /
# GetDebriefNarrative / GetFinalScore /
# IsProgressSaved / GetPresentationState /
# RetrySave / TravelNext / HandleDebriefKey, and
# sibling GetPresentationState / RetrySave in this
# wave stay sibling-only.
LOCKED_SCRIPTS = (
...
# locked here. Configure / GetPresentation /
# GetDebrief / GetDebriefNarrative / GetFinalScore /
# IsProgressSaved / GetPresentationState /
# RetrySave / TravelNext / HandleDebriefKey stay
# sibling-only.
UNLOCKED_NEIGHBORS = (
...
# Leftover campaign-subsystem AcknowledgeDebrief
# (draft #322) stays unlocked. Same one-line form
# lives on USkyguardCampaignSubsystem; this lane
# parses USkyguardDebriefWidget only.
CAMPAIGN_ACKNOWLEDGE_DEBRIEF_NOT_LOCKED = (
    "test_acknowledge_debrief_decl_contract.py",
    "USkyguardCampaignSubsystem",
)
# Leftover presentation-component AcknowledgeDebrief
# stays unlocked. Same one-line form lives on
# USkyguardSortiePresentationComponent.
COMPONENT_ACKNOWLEDGE_DEBRIEF_NOT_LOCKED = (
    "USkyguardSortiePresentationComponent::AcknowledgeDebrief",
)
# Leftover BriefingWidget AcknowledgeBriefing
# (draft #391) and later BriefingWidget methods
# stay unlocked.
BRIEFING_WIDGET_ACKNOWLEDGE_BRIEFING_NOT_LOCKED = (
...
# locking AcknowledgeDebrief. Stay off GetDebrief.
# FSkyguardMissionDebrief is a neighbor return type,
# so only leftover default fields / sibling file
# are listed here.
...
# .cpp AcknowledgeDebrief body / invented INDEX_NONE
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardDebriefWidget::AcknowledgeDebrief",
    "SkyguardSortiePresentationWidgets.cpp",
)
...
class DebriefWidgetAcknowledgeDebriefDeclContractTests(unittest.TestCase):
    def test_debrief_widget_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_DEBRIEF), section)
    ...
    def test_campaign_subsystem_does_not_satisfy_class(self) -> None:
        leftover = (
    ...
    def test_presentation_component_does_not_satisfy_class(self) -> None:
        leftover = (
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ACKNOWLEDGE_DEBRIEF)
        self.assertIn("AcknowledgeDebrief", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ACKNOWLEDGE_DEBRIEF))
    ...
    def test_missing_acknowledge_debrief_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ACKNOWLEDGE_DEBRIEF)
        self.assertIn("AcknowledgeDebrief", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ACKNOWLEDGE_DEBRIEF)
        self.assertIn("AcknowledgeDebrief", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_DEBRIEF), section)
        self.assertNotIn("BlueprintPure", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("UFUNCTION", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("Category", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("BlueprintCallable", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ACKNOWLEDGE_DEBRIEF)
        self.assertIn("AcknowledgeDebrief", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        wrong_return = "\tvoid AcknowledgeDebrief();\n"
        added_const = "\tbool AcknowledgeDebrief() const;\n"
        added_arg = "\tbool AcknowledgeDebrief(bool bForce);\n"
        int_return = "\tint32 AcknowledgeDebrief();\n"
        sibling_name = "\tbool AcknowledgeBriefing();\n"
        retry = "\tbool RetrySave();\n"
        for region in (
            wrong_return,
            added_const,
            added_arg,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ACKNOWLEDGE_DEBRIEF)
            self.assertIn("AcknowledgeDebrief", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_briefing_acknowledge_briefing_does_not_satisfy(self) -> None:
        leftover = "\tbool AcknowledgeBriefing();\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, ACKNOWLEDGE_DEBRIEF)
        self.assertIn("AcknowledgeDebrief", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, ACKNOWLEDGE_DEBRIEF))
        self.assertNotIn("AcknowledgeBriefing", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_acknowledge_debrief_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_DEBRIEF))
        self.assertEqual(declaration_count(section, ACKNOWLEDGE_DEBRIEF), 1)
        self.assertTrue(
            ACKNOWLEDGE_DEBRIEF.startswith("bool "),
            ACKNOWLEDGE_DEBRIEF,
        )
        self.assertTrue(ACKNOWLEDGE_DEBRIEF.endswith(";"), ACKNOWLEDGE_DEBRIEF)
        self.assertIn("AcknowledgeDebrief()", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("INDEX_NONE", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("{", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("}", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("return ", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn(" const", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, ACKNOWLEDGE_DEBRIEF),
                section,
            )
            self.assertEqual(
                require_declaration(section, ACKNOWLEDGE_DEBRIEF),
            ...
            self.assertEqual(declaration_count(section, ACKNOWLEDGE_DEBRIEF), 1)
        one_line = f"{{\npublic:\n\t{ACKNOWLEDGE_DEBRIEF}\n}}\n"
        self.assertTrue(has_declaration(one_line, ACKNOWLEDGE_DEBRIEF))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_DEBRIEF), section)
        self.assertEqual(
            require_declaration(section, ACKNOWLEDGE_DEBRIEF),
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("return ", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("return false", ACKNOWLEDGE_DEBRIEF)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{ACKNOWLEDGE_DEBRIEF}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", ACKNOWLEDGE_DEBRIEF)
        section = public_section(origin_main_header())
    ...
    def test_contract_does_not_lock_acknowledge_debrief_cpp_body(self) -> None:
        ...
        self.assertNotIn("return false", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("return true", ACKNOWLEDGE_DEBRIEF)

    def test_contract_does_not_relock_configure(self) -> None:
        locked_only = f"{ACKNOWLEDGE_DEBRIEF}\n"
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("Configure(", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetPresentation", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_get_debrief(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetDebrief", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_get_debrief_narrative(self) -> None:
        ...
        for neighbor in GET_DEBRIEF_NARRATIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetDebriefNarrative", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_get_final_score(self) -> None:
        ...
        for neighbor in GET_FINAL_SCORE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetFinalScore", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_is_progress_saved(self) -> None:
        ...
        for neighbor in IS_PROGRESS_SAVED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("IsProgressSaved", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_get_presentation_state(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetPresentationState", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_retry_save(self) -> None:
        ...
        for neighbor in RETRY_SAVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("RetrySave", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_travel_next(self) -> None:
        ...
        for neighbor in TRAVEL_NEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("TravelNext", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_handle_debrief_key(self) -> None:
        ...
        for neighbor in HANDLE_DEBRIEF_KEY_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("HandleDebriefKey", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("HandleDebriefKey", locked_only)

    def test_contract_does_not_relock_campaign_acknowledge_debrief(
        self,
    ) -> None:
        locked_only = f"{ACKNOWLEDGE_DEBRIEF}\n"
        for token in CAMPAIGN_ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("USkyguardCampaignSubsystem", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_component_acknowledge_debrief(
        self,
    ) -> None:
        locked_only = f"{ACKNOWLEDGE_DEBRIEF}\n"
        for token in COMPONENT_ACKNOWLEDGE_DEBRIEF_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn(
            "USkyguardSortiePresentationComponent::AcknowledgeDebrief",
    ...
    def test_contract_does_not_relock_briefing_widget_acknowledge_briefing(
        self,
    ) -> None:
        locked_only = f"{ACKNOWLEDGE_DEBRIEF}\n"
        for token in BRIEFING_WIDGET_ACKNOWLEDGE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("AcknowledgeBriefing", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("AcknowledgeBriefing", locked_only)
        self.assertNotIn("USkyguardBriefingWidget", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn(
            "test_briefing_widget_acknowledge_briefing_decl_contract.py",
            ACKNOWLEDGE_DEBRIEF,
        )
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("ConfigureFromMission", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("ConfigureFromMission", locked_only)
        self.assertNotIn("USkyguardMissionBriefingComponent", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_briefing_component_neighbors(
        ...
        for neighbor in (
            ...
            *GET_RADIO_CHATTER_NOT_LOCKED,
            ...
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("SetAssetsReady", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("AdvanceBriefing", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("AcknowledgeAndLaunch", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("CanLaunch", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetBriefingState", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetElapsedSeconds", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetMinimumWarmupSeconds", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("GetRadioChatter", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("ESkyguardMissionBriefingState", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("Unconfigured", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("Warming", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("Launched", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            ACKNOWLEDGE_DEBRIEF,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("FSkyguardBriefingCard", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("FSkyguardBriefingRadioRow", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("FSkyguardHowToFlyRow", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation_state(
        ...
        self.assertNotIn("SortieActive", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("DebriefReady", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn(
            "test_sortie_presentation_state_enum_contract.py",
            ACKNOWLEDGE_DEBRIEF,
        )
    ...
    def test_contract_does_not_relock_leftover_mission_debrief_defaults(
        ...
        for token in LEFTOVER_MISSION_DEBRIEF_DEFAULTS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ACKNOWLEDGE_DEBRIEF)
            self.assertNotIn(token, section)
        self.assertNotIn("bNewBestScore", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("bProgressSaved", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("ESkyguardMissionDebriefState", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn(
            "test_mission_debrief_defaults_contract.py",
            ACKNOWLEDGE_DEBRIEF,
        )
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("FillResultCombatStats", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("ASkyguardGunner", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("FillAndFinalize", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("FillAndFail", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("ApplyHydraForClusters", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_bind_hud_or_theater(self) -> None:
        ...
        self.assertNotIn("test_campaign_theater_kit_contract.py", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ACKNOWLEDGE_DEBRIEF)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ACKNOWLEDGE_DEBRIEF)
        self.assertNotIn("ESkyguardMissionSkylineStyle", ACKNOWLEDGE_DEBRIEF)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ACKNOWLEDGE_DEBRIEF}\n"
        self.assertEqual(
            require_declaration(locked_only, ACKNOWLEDGE_DEBRIEF),
            ACKNOWLEDGE_DEBRIEF,
        )
    ...
    def test_contract_parses_public_section_not_briefing_private_or_cpp(
        ...
        self.assertNotIn("USkyguardCampaignSubsystem", section)
        self.assertNotIn("USkyguardCampaignSubsystem", body)
        ...
        self.assertNotIn("USkyguardDebriefWidget::AcknowledgeDebrief", section)
    ...
    def test_contract_does_not_parse_briefing_widget(self) -> None:
        ...
        for token in BRIEFING_WIDGET_NOT_LOCKED:
            ...
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardBriefingWidget", section)
        self.assertNotIn("GetMissionTitle", ACKNOWLEDGE_DEBRIEF)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "debrief widget AcknowledgeDebrief contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, ACKNOWLEDGE_DEBRIEF.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "debrief widget AcknowledgeDebrief contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, ACKNOWLEDGE_DEBRIEF.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ACKNOWLEDGE_DEBRIEF)
    ...
    def test_contract_is_acknowledge_debrief_declaration_only(self) -> None:
        ...
        self.assertTrue(ACKNOWLEDGE_DEBRIEF.startswith("bool "))
        self.assertTrue(ACKNOWLEDGE_DEBRIEF.endswith(";"))
        self.assertIn(UFUNCTION_DEBRIEF, section)
