...
# then static FSkyguardLandscapeCaptureConfigurationResult
# ConfigureLandscapeSceneCaptureDiagnostic(
...
# Leftover unique LandscapeCaptureConfigurationResult FIELD drafts
# (#1214-#1224) wrap struct members with NO Category and explicitly
# do NOT parse this library.
# Identifier ConfigureLandscapeSceneCaptureDiagnostic is NOT leftover
# analog CaptureConfiguration field bSuccess / Mode / ViewMode
# and NOT leftover unique BeginTransientLandscapeDiagnosticMaterialDeferred
# (#1697) / FinishLandscapeMaterialCompilation (#1698) /
# AuditLandscapeMaterialCompilation (#1699) /
...
# Three args: Capture, Landscape, Mode.
...
# return type is not FSkyguardLandscapeCaptureConfigurationResult,
...
# Fail-closed if Category is Authoring / Grounding / bare Environment /
# Theater / MissionMap / Settings without Skyguard|.
...
# Do NOT parse struct FSkyguardLandscapeCaptureConfigurationResult
# (leftover unique field drafts #1214-#1224).
# Do NOT parse VisibleAudit / MaterialCompilation / AuthoringResult
# structs.
...
# PrepareGovernedLandscapeForVisibleValidation — RETARGET hard:
# KEEP Category Validation. KEEP BlueprintCallable.
# RETARGET return FSkyguardLandscapeVisibleAudit ->
# FSkyguardLandscapeCaptureConfigurationResult.
# RETARGET method PrepareGovernedLandscapeForVisibleValidation ->
# ConfigureLandscapeSceneCaptureDiagnostic.
# RETARGET args to USceneCaptureComponent2D* Capture, ALandscape*
# Landscape, ESkyguardLandscapeCaptureDiagnosticMode Mode.
# Clone leftover unique FinishLandscapeMaterialCompilation #1698 SHA
# 7ee687c08ffefe36deca0985ad8579703dc11cbf — RETARGET method/return/args.
# Clone leftover unique BlueprintCallable EnsureDefaultEntries
# #1680 SHA 27ffb2a529b5067ad204e9c35e46f650a02403a4
...
# 4e878e0750eaed9a830a48e36e94d949f608491b — RETARGET Category
# Combat|Sortie -> Skyguard|Mission01|Environment|Validation.
# KEEP BlueprintCallable (NOT BlueprintPure).
# LOCKED_DECL is NOT leftover int32 GetSortieHits() const;.
# Clone #1300 SHA c4679feb43f4b97d3a814c088e3b6c344e15831f
# is Category Theater wrapping FName WeatherIdentity with no
# initializer — RETARGET hard.
# LOCKED_DECL is ConfigureLandscapeSceneCaptureDiagnostic
# static struct-return. NOT FName WeatherIdentity;
...
# SetTransientLandscapeDiagnosticMaterial and NOT
...
# void ConfigureLandscapeSceneCaptureDiagnostic
# AS THE LOCKED DECL.
...
LEFTOVER_CAPTURE_CONFIG_STRUCT = "FSkyguardLandscapeCaptureConfigurationResult"
...
TARGET_WRONG_NO_CONST = (
    ...
    "USceneCaptureComponent2D* Capture, "
    "ALandscape* Landscape, "
    "ESkyguardLandscapeCaptureDiagnosticMode Mode) const;"
)
...
TARGET_WRONG_FLOAT = (
    "float ConfigureLandscapeSceneCaptureDiagnostic();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void ConfigureLandscapeSceneCaptureDiagnostics();"
...
TARGET_WRONG_AUDIT_VISIBLE = (
    "static FSkyguardLandscapeCaptureConfigurationResult AuditLandscapeVisibleReadiness("
)
...
TARGET_WRONG_VIEW_MODE = "FString ViewMode;"
TARGET_WRONG_MODE_FIELD = (
    "ESkyguardLandscapeCaptureDiagnosticMode Mode = "
    "ESkyguardLandscapeCaptureDiagnosticMode::Lit;"
)
TARGET_WRONG_BEGIN_TRANSIENT = (
...
TARGET_WRONG_TRUE = "bool ConfigureLandscapeSceneCaptureDiagnostic = true;"
TARGET_WRONG_ZERO = "bool ConfigureLandscapeSceneCaptureDiagnostic = 0.f;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_mission01_configure_landscape_scene_capture_diagnostic"
    "_decl_contract.py"
)
...
LOCKED_SCRIPTS = (
    ...
    "_decl_contract.py",
    "Scripts/tests/test_mission01_finish_landscape_material_compilation"
    "_decl_contract.py",
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bstatic\s+FSkyguardLandscapeCaptureConfigurationResult\s+"
    r"ConfigureLandscapeSceneCaptureDiagnostic\("
    r"USceneCaptureComponent2D\* Capture,"
    r"ALandscape\* Landscape,"
    r"ESkyguardLandscapeCaptureDiagnosticMode Mode\)(?:;|\{)"
)
...
LEFTOVER_VIEW_MODE_FIELD_RE = re.compile(
    r"FString\s+ViewMode\b"
)
LEFTOVER_MODE_FIELD_RE = re.compile(
    r"ESkyguardLandscapeCaptureDiagnosticMode\s+Mode\s*="
)
...
def leftover_analog_field_in_region(compact: str) -> bool:
    ...
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return True
    if LEFTOVER_SUCCESS_FIELD_RE.search(compact):
        return True
    if LEFTOVER_VIEW_MODE_FIELD_RE.search(compact):
        return True
    if LEFTOVER_MODE_FIELD_RE.search(compact):
        return True
    return False
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bConfigureLandscapeSceneCaptureDiagnostic\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+ConfigureLandscapeSceneCaptureDiagnostic\b", compact):
        return False
    if re.search(r"\bbool\s+ConfigureLandscapeSceneCaptureDiagnostic\b", compact):
        return False
    if re.search(r"\bfloat\s+ConfigureLandscapeSceneCaptureDiagnostic\b", compact):
        return False
    if re.search(r"\bint32\s+ConfigureLandscapeSceneCaptureDiagnostic\b", compact):
        return False
...
def spec_section(header: str) -> str:
    ...
    method = "ConfigureLandscapeSceneCaptureDiagnostic"
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static\s+FSkyguardLandscapeCaptureConfigurationResult\s+"
            r"ConfigureLandscapeSceneCaptureDiagnostic\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for ConfigureLandscapeSceneCaptureDiagnostic "
...
class Mission01ConfigureLandscapeSceneCaptureDiagnosticDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_CAPTURE_CONFIG_STRUCT)
        ...
        self.assertTrue(
            has_identifier(section, "ConfigureLandscapeSceneCaptureDiagnostic"),
            section,
        )
    ...
    def test_missing_configure_landscape_scene_capture_diagnostic_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "ConfigureLandscapeSceneCaptureDiagnostic()const",
    ...
    def test_configure_landscape_scene_capture_diagnostic_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VIEW_MODE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MODE_FIELD)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BEGIN_TRANSIENT)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid ConfigureLandscapeSceneCaptureDiagnosticX();\n",
            "\tint32 ConfigureLandscapeSceneCaptureDiagnostic = 1;\n",
            "\tFName ConfigureLandscapeSceneCaptureDiagnostic = NAME_None;\n",
            "\tvoid ConfigureLandscapeSceneCaptureDiagnostic() = "
            + forty
            + ";\n",
            "\tvoid ConfigureLandscapeSceneCaptureDiagnostic() = "
            ...
            f"\t{TARGET_WRONG_VIEW_MODE}\n",
            f"\t{TARGET_WRONG_MODE_FIELD}\n",
            f"\t{TARGET_WRONG_BEGIN_TRANSIENT}\n",
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "ConfigureLandscapeSceneCaptureDiagnostic")
        )
        self.assertTrue(
            has_identifier(section, "ConfigureLandscapeSceneCaptureDiagnostic")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 configure landscape scene capture diagnostic "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_configure_landscape_scene_capture_diagnostic"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
