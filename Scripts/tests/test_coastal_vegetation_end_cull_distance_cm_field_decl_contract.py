# THIS IS leftover-safe ASkyguardCoastalEnvironmentDirector VegetationEndCullDistanceCm.
...
# then `int32 VegetationEndCullDistanceCm = 32000;` (HAS initializer; NO spaces around =).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Environment|Performance`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS `= 32000`.
# This is a unique coastal director vegetation end-cull int32.
# It is NOT leftover FSkyguardEnvironmentReadiness.TreeInstanceCount,
# NOT leftover sibling VegetationStartCullDistanceCm / PlacementSeed / EpicTreeBudget / EpicShrubBudget / TreeInstances /
# ShrubInstances, NOT leftover analog coastal-env-director-empty-fail-closed
# bulk, NOT leftover analog Root / VFXPool / Readiness / RouteLengthCm /
# WindStrength / WindSpeed / Quality.
...
# or stay Category="Skyguard" without |Environment|Performance.
...
# Stop BEFORE sibling WindStrength as a claimed slot.
...
# Do NOT refill leftover PlacementSeed / EpicTreeBudget / VegetationStartCullDistanceCm / TreeInstances / ShrubInstances /
# WindSource.
...
# Category Skyguard|Combat wrapping `float LookYawLimit = 95.f;`
# — RETARGET hard: Category Skyguard|Environment|Performance
# wrapping `int32 VegetationEndCullDistanceCm = 32000;` plus ClampMin.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity
# with NO initializer — RETARGET hard.
# LOCKED_DECL is `int32 VegetationEndCullDistanceCm = 32000;` NOT
...
# Skyguard|Environment|Vegetation or still Category="Skyguard"
# without |Environment|Performance.
# Fail-closed if LOCKED_DECL is TObjectPtr / FName WeatherIdentity
# / leftover `float TraceRange = 32000.f;` / leftover TreeInstances
# TObjectPtr.
# Harbor leftover is only the split 40 / 80 float tokens.
# VegetationEndCullDistanceCm is int32 32000 not Harbor forty / eighty float tokens
# and not leftover float TraceRange = 32000.f.
...
TARGET = "int32 VegetationEndCullDistanceCm = 32000;"
TARGET_WRONG_BARE = "int32 VegetationEndCullDistanceCm;"
TARGET_WRONG_INIT = "int32 VegetationEndCullDistanceCm = nullptr;"
TARGET_WRONG_FALSE = "int32 VegetationEndCullDistanceCm = false;"
TARGET_WRONG_TRUE = "int32 VegetationEndCullDistanceCm = true;"
TARGET_WRONG_ZERO = "int32 VegetationEndCullDistanceCm = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> VegetationEndCullDistanceCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> VegetationEndCullDistanceCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> VegetationEndCullDistanceCm;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "WindStrength"
STOP_BEFORE_EPIC_SHRUB = "WindStrength"
...
SIBLING_LOOK_PITCH_MIN = "WindStrength"
...
VEGETATION_END_CULL_DECL_RE = re.compile(
    r"int32\s+VegetationEndCullDistanceCm\s*=\s*32000\s*;"
)
VEGETATION_END_CULL_BARE_RE = re.compile(
    r"int32\s+VegetationEndCullDistanceCm\s*;"
)
VEGETATION_END_CULL_WRONG_INIT_RE = re.compile(
    r"int32\s+VegetationEndCullDistanceCm\s*="
)
PLACEMENT_SEED_DECL_RE = re.compile(
    r"int32\s+PlacementSeed\s*=\s*5201\s*;"
)
EPIC_TREE_BUDGET_LEFTOVER_RE = re.compile(
    r"int32\s+EpicTreeBudget\s*=\s*240\s*;"
)
TRACE_RANGE_LEFTOVER_RE = re.compile(
    r"float\s+TraceRange\s*=\s*32000\.f\b"
)
START_CULL_LEFTOVER_RE = re.compile(
    r"int32\s+VegetationStartCullDistanceCm\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 VegetationEndCullDistanceCm = 32000;`.
    ...
    # leftover EpicTreeBudget = 240,
    # leftover float TraceRange = 32000.f,
    # leftover VegetationStartCullDistanceCm,
    # leftover TreeInstanceCount, leftover TreeInstances
    # TObjectPtr, or a bare VegetationEndCullDistanceCm without
    # the 32000 initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if VEGETATION_END_CULL_DECL_RE.search(compact) is None:
        return False
    if VEGETATION_END_CULL_BARE_RE.search(compact):
        return False
    ...
    if PLACEMENT_SEED_DECL_RE.search(compact):
        return False
    if EPIC_TREE_BUDGET_LEFTOVER_RE.search(compact):
        return False
    if TRACE_RANGE_LEFTOVER_RE.search(compact):
        return False
    if START_CULL_LEFTOVER_RE.search(compact):
        return False
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_LOOK_PITCH_MIN:
            break
        if ident != "VegetationEndCullDistanceCm":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+VegetationEndCullDistanceCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 VegetationEndCullDistanceCm "
...
class CoastalVegetationEndCullDistanceCmFieldDeclContractTests(unittest.TestCase):
    def test_coastal_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "VegetationEndCullDistanceCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "VegetationEndCullDistanceCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("VegetationEndCullDistanceCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_vegetation_end_cull_distance_cm_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Environment|Performance"', section)
        ...
        self.assertIn('Category="Skyguard|Environment|Performance"', specifiers)
        ...
        self.assertIn('ClampMin="2000"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 VegetationEndCullDistanceCm = 32000;", compact_origin)
        self.assertNotIn("VegetationEndCullDistanceCm = 160", compact_origin)
    ...
    def test_vegetation_end_cull_distance_cm_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 VegetationEndCullDistanceCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("32000", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 VegetationEndCullDistanceCm = 32000;")
        ...
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_VEGETATION)
        self.assertNotIn("TraceRange", LOCKED_DECL)
        self.assertNotIn("32000.f", LOCKED_DECL)
        self.assertNotIn("EpicTreeBudget", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 VegetationEndCullDistanceCms = 32000;\n",
            "\tfloat VegetationEndCullDistanceCm = 32000.f;\n",
            "\tbool VegetationEndCullDistanceCm = true;\n",
            "\tint32 VegetationEndCullDistanceCm = " + forty + ";\n",
            "\tint32 VegetationEndCullDistanceCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_SEED}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Environment|Performance"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Environment|Performance"', apache_specs)
        self.assertIn('Category="Skyguard|Apache"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tint32\n\tVegetationEndCullDistanceCm = 32000;\n",
    ...
    def test_does_not_claim_sibling_wind_strength_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "VegetationEndCullDistanceCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "VegetationEndCullDistanceCm"))
        self.assertTrue(has_identifier(section, "VegetationEndCullDistanceCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="2000"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "coastal VegetationEndCullDistanceCm field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "coastal_vegetation_end_cull_distance_cm_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCoastalEnvironmentDirector.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("32000.f", locked_only)
        for leftover in leftover_apache_ufunction_tokens():
