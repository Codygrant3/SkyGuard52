...
# INDEX_NONE or lock GetProtectedGroup
# construction in the .cpp. This is leftover-safe
# Mission10 Protection GetProtectedGroup
...
LOCKED_DECL = GET_PROTECTED_GROUP
...
# an isolated Python GetProtectedGroup
...
class Mission10GetProtectedGroupDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("GetProtectedGroup", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_get_protected_group_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
    ...
    def test_get_protected_group_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FSkyguardMission10ProtectedRuntime"),
            LOCKED_DECL,
        )
        self.assertTrue(
            LOCKED_DECL.startswith("FSkyguardMission10ProtectedRuntime"),
        ...
        self.assertIn("FSkyguardMission10ProtectedRuntime", LOCKED_DECL)
        self.assertIn("ESkyguardMission10ProtectedGroup", LOCKED_DECL)
        self.assertIn("GetProtectedGroup", LOCKED_DECL)
        self.assertNotIn(";", LOCKED_DECL)
        self.assertIn(UFUNCTION_PROTECTION, section)
        self.assertIn(" const", LOCKED_DECL)
        self.assertIn("Group", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_origin_body = (
            ...
            "\t\tESkyguardMission10ProtectedGroup Group) const\n"
            "\t{\n"
            "\t\treturn FSkyguardMission10ProtectedRuntime();\n"
            "\t}\n"
            "};\n"
        )
        wrap_ufunction = (
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("GetProtectedGroup", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FSkyguardMission10ProtectedRuntime"))
    ...
    def test_contract_is_get_protected_group_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_protected_group(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_get_protected_group"
                "_decl_contract.py"
            )
        )
        self.assertIn(
