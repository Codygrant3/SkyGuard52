...
def progress_body(header: str) -> str:
    start = header.index("struct FSkyguardObjectiveProgress")
...
def in_class_defaults(body: str) -> dict[str, str]:
    return {
        ...
        for name, value in re.findall(
            r"(?:ESkyguardMissionObjectiveState|int32)\s+(\w+)\s*=\s*([^;]+);",
            body,
        )
    }


class ObjectiveProgressDefaultsContractTests(unittest.TestCase):
    def test_progress_struct_exists(self) -> None:
        header = origin_main(HEADER_NAME)
        self.assertIn("struct FSkyguardObjectiveProgress", header)
        self.assertIn("USTRUCT(BlueprintType)", header)
        self.assertIn("GENERATED_BODY()", progress_body(header))

    def test_public_fields_match_origin_main_in_order(self) -> None:
        body = progress_body(origin_main(HEADER_NAME))
    ...
    def test_in_class_defaults_match_origin_main(self) -> None:
        ...
        self.assertIn("int32 CurrentProgress = 0;", body)
        self.assertIn(
            "ESkyguardMissionObjectiveState State = ESkyguardMissionObjectiveState::Inactive;",
            body,
        )
    ...
    def test_struct_does_not_re_lock_result_enums_or_harbor(self) -> None:
        body = progress_body(origin_main(HEADER_NAME))
        self.assertNotIn("struct FSkyguardMissionResult", body)
        self.assertNotIn("struct FSkyguardMissionDebrief", body)
        self.assertNotIn("enum class ESkyguardMissionObjectiveState", body)
        for name in OTHER_TYPES:
            self.assertNotIn(name, body)
        for token in ENUMERATOR_LIST:
            self.assertNotIn(token, body)
        for token in HARBOR_TUNING:
            self.assertNotIn(token, body)

    def test_struct_bans_igla_yak_rifle(self) -> None:
        lowered = progress_body(origin_main(HEADER_NAME)).lower()
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"FSkyguardObjectiveProgress contains {banned}",
            )
