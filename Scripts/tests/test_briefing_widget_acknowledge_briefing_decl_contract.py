...
# or lock the AcknowledgeBriefing body in the .cpp.
...
# USkyguardDebriefWidget. This is not leftover
# briefing-fail-closed #9fe9 and not
# USkyguardMissionBriefingComponent::AcknowledgeAndLaunch
# (draft #369). Leftover sortie-presentation fail-closed
# #7600, leftover briefing-fail-closed #9fe9, leftover
...
# sibling Configure, and sibling GetPresentation
# stay sibling-only.
ACKNOWLEDGE_BRIEFING = "bool AcknowledgeBriefing();"
UFUNCTION_BRIEFING = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Presentation|Briefing")'
)
...
# AcknowledgeBriefing declaration contract on
# USkyguardBriefingWidget. Stay off Configure,
# GetPresentation, GetMissionTitle, GetBriefingText,
# GetBriefingCards, GetRadioRows, GetHowToFlyRows,
# and LaunchSortie on this class. Stay off leftover
...
# leftover CPG debrief, leftover Gunner helpers,
...
# USkyguardDebriefWidget, leftover
# AcknowledgeAndLaunch, and dirty workspace paths.
LOCKED = {
...
# GetRadioChatter including
# test_briefing_acknowledge_and_launch_decl_contract.py,
# sibling test_briefing_widget_configure_decl_contract.py,
# sibling test_briefing_widget_get_presentation_decl_contract.py,
# leftover CPG debrief, leftover
...
# USkyguardDebriefWidget, leftover
# AcknowledgeAndLaunch, leftover Configure,
# leftover GetPresentation, leftover
# GetMissionTitle, leftover GetBriefingText,
# leftover GetBriefingCards, leftover GetRadioRows,
# leftover GetHowToFlyRows, leftover LaunchSortie,
# and leftover ConfigureFromMission stay sibling-only.
LOCKED_SCRIPTS = (
...
# GetBriefingCards / GetRadioRows / GetHowToFlyRows /
# LaunchSortie stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
# stay unlocked. AcknowledgeAndLaunch (#369) is not
# this widget declaration.
...
# leftover briefing-component ConfigureFromMission /
# leftover widget Configure / leftover
# GetPresentation stay unlocked.
...
# .cpp AcknowledgeBriefing body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. Do not parse USkyguardDebriefWidget.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    "return false",
    "return true",
    "USkyguardBriefingWidget::AcknowledgeBriefing",
    "SkyguardSortiePresentationWidgets.cpp",
)
...
class BriefingWidgetAcknowledgeBriefingDeclContractTests(unittest.TestCase):
    def test_briefing_widget_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_BRIEFING), section)
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            "class SKYGUARD52_API UOtherBriefingWidget "
            ": public UUserWidget\n"
            "{\n"
            "public:\n"
            f"\t{ACKNOWLEDGE_BRIEFING}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ACKNOWLEDGE_BRIEFING)
        self.assertIn("AcknowledgeBriefing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ACKNOWLEDGE_BRIEFING))

    def test_missing_acknowledge_briefing_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tvoid Configure("
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ACKNOWLEDGE_BRIEFING)
        self.assertIn("AcknowledgeBriefing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ACKNOWLEDGE_BRIEFING)
        self.assertIn("AcknowledgeBriefing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_BRIEFING), section)
        self.assertNotIn("BlueprintPure", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("UFUNCTION", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("Category", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("BlueprintCallable", ACKNOWLEDGE_BRIEFING)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ACKNOWLEDGE_BRIEFING)
        self.assertIn("AcknowledgeBriefing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        wrong_return = "\tvoid AcknowledgeBriefing();\n"
        added_const = "\tbool AcknowledgeBriefing() const;\n"
        added_arg = "\tbool AcknowledgeBriefing(bool bForce);\n"
        int_return = "\tint32 AcknowledgeBriefing();\n"
        sibling_name = "\tbool AcknowledgeAndLaunch();\n"
        launch = "\tbool LaunchSortie();\n"
        for region in (
            wrong_return,
            added_const,
            added_arg,
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ACKNOWLEDGE_BRIEFING)
            self.assertIn("AcknowledgeBriefing", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_acknowledge_and_launch_does_not_satisfy(self) -> None:
        leftover = "\tbool AcknowledgeAndLaunch();\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(leftover, ACKNOWLEDGE_BRIEFING)
        self.assertIn("AcknowledgeBriefing", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(leftover, ACKNOWLEDGE_BRIEFING))
        self.assertNotIn("AcknowledgeAndLaunch", ACKNOWLEDGE_BRIEFING)

    def test_acknowledge_briefing_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, ACKNOWLEDGE_BRIEFING))
        self.assertEqual(declaration_count(section, ACKNOWLEDGE_BRIEFING), 1)
        self.assertTrue(
            ACKNOWLEDGE_BRIEFING.startswith("bool "),
            ACKNOWLEDGE_BRIEFING,
        )
        self.assertTrue(
            ACKNOWLEDGE_BRIEFING.endswith(";"),
            ACKNOWLEDGE_BRIEFING,
        )
        self.assertIn("AcknowledgeBriefing()", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("INDEX_NONE", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("{", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("}", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("return ", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn(" const", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("AcknowledgeAndLaunch", ACKNOWLEDGE_BRIEFING)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, ACKNOWLEDGE_BRIEFING),
                section,
            )
            self.assertEqual(
                require_declaration(section, ACKNOWLEDGE_BRIEFING),
            ...
            self.assertEqual(
                declaration_count(section, ACKNOWLEDGE_BRIEFING),
                1,
            )
        one_line = f"{{\npublic:\n\t{ACKNOWLEDGE_BRIEFING}\n}}\n"
        self.assertTrue(has_declaration(one_line, ACKNOWLEDGE_BRIEFING))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ACKNOWLEDGE_BRIEFING),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("return ", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("return false", ACKNOWLEDGE_BRIEFING)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{ACKNOWLEDGE_BRIEFING}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", ACKNOWLEDGE_BRIEFING)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_acknowledge_briefing_cpp_body(self) -> None:
        locked_only = f"{ACKNOWLEDGE_BRIEFING}\n"
        self.assertNotIn("{", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("}", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("return ", ACKNOWLEDGE_BRIEFING)
        ...
        self.assertNotIn("return false", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("return true", ACKNOWLEDGE_BRIEFING)

    def test_contract_does_not_relock_configure(self) -> None:
        locked_only = f"{ACKNOWLEDGE_BRIEFING}\n"
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("Configure(", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("Configure(", locked_only)
        self.assertNotIn("InPresentation", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_get_presentation(self) -> None:
        ...
        for neighbor in GET_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetPresentation", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_get_mission_title(self) -> None:
        ...
        for neighbor in GET_MISSION_TITLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetMissionTitle", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetBriefingText", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_get_briefing_cards(self) -> None:
        ...
        for neighbor in GET_BRIEFING_CARDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetBriefingCards", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_get_radio_rows(self) -> None:
        ...
        for neighbor in GET_RADIO_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetRadioRows", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_get_how_to_fly_rows(self) -> None:
        ...
        for neighbor in GET_HOW_TO_FLY_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetHowToFlyRows", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_launch_sortie(self) -> None:
        ...
        for neighbor in LAUNCH_SORTIE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("LaunchSortie", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_configure_from_mission(self) -> None:
        ...
        for neighbor in CONFIGURE_FROM_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("ConfigureFromMission", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("ConfigureFromMission", locked_only)
        self.assertNotIn("USkyguardMissionBriefingComponent", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_briefing_component_neighbors(
        ...
        for neighbor in (
            ...
            *GET_RADIO_CHATTER_NOT_LOCKED,
            ...
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("SetAssetsReady", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("AdvanceBriefing", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("AcknowledgeAndLaunch", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("CanLaunch", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetBriefingState", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetElapsedSeconds", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetMinimumWarmupSeconds", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("GetRadioChatter", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("ESkyguardMissionBriefingState", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_leftover_briefing_state_enum(
        ...
        for token in LEFTOVER_ENUM_NOT_LOCKED:
            ...
            self.assertNotIn(token, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("Unconfigured", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("Warming", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("Launched", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn(
            "test_mission_briefing_state_enum_contract.py",
            ACKNOWLEDGE_BRIEFING,
        )
    ...
    def test_contract_does_not_relock_leftover_briefing_siblings(self) -> None:
        ...
        for token in LEFTOVER_BRIEFING_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("FSkyguardBriefingCard", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("FSkyguardBriefingRadioRow", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("FSkyguardHowToFlyRow", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_leftover_sortie_presentation(
        ...
        for token in LEFTOVER_SORTIE_PRESENTATION_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("SkyguardSortiePresentationFailClosed", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("FillResultCombatStats", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("ASkyguardGunner", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("FillAndFinalize", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("FillAndFail", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("ApplyHydraForClusters", ACKNOWLEDGE_BRIEFING)
    ...
    def test_contract_does_not_relock_bind_hud_or_theater(self) -> None:
        ...
        for token in LEFTOVER_BIND_HUD_AND_THEATER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("test_bind_hud_host_presentation_tests.py", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn("test_campaign_theater_kit_contract.py", ACKNOWLEDGE_BRIEFING)
        self.assertNotIn(
            "test_briefing_widget_configure_decl_contract.py",
            ACKNOWLEDGE_BRIEFING,
        )
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ACKNOWLEDGE_BRIEFING}\n"
        self.assertEqual(
            require_declaration(locked_only, ACKNOWLEDGE_BRIEFING),
            ACKNOWLEDGE_BRIEFING,
        )
    ...
    def test_contract_parses_public_section_not_debrief_private_or_cpp(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("RetrySave", section)
        self.assertNotIn("TravelNext", section)
        self.assertNotIn("HandleDebriefKey", section)
        ...
        self.assertNotIn("RuntimeLaunchButton", section)
        self.assertEqual(
            require_declaration(section, ACKNOWLEDGE_BRIEFING),
            ACKNOWLEDGE_BRIEFING,
        )
    ...
    def test_contract_does_not_parse_debrief_widget(self) -> None:
        locked_only = f"{ACKNOWLEDGE_BRIEFING}\n"
        section = public_section(origin_main_header())
        for token in DEBRIEF_WIDGET_NOT_LOCKED:
            ...
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", section)
        self.assertNotIn("GetDebrief", ACKNOWLEDGE_BRIEFING)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing widget AcknowledgeBriefing contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, ACKNOWLEDGE_BRIEFING.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "briefing widget AcknowledgeBriefing contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, ACKNOWLEDGE_BRIEFING.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ACKNOWLEDGE_BRIEFING)

    def test_contract_is_acknowledge_briefing_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(ACKNOWLEDGE_BRIEFING.startswith("bool "))
        self.assertTrue(ACKNOWLEDGE_BRIEFING.endswith(";"))
        self.assertIn(UFUNCTION_BRIEFING, section)
