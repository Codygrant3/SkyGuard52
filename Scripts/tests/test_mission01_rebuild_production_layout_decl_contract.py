...
# INDEX_NONE or lock RebuildProductionLayout
...
# IsPointAllowedForPCG / HasContinuousCoastline /
...
# (`void RebuildProductionLayout();`); accept that
...
# isolated-test drafts #107–#618, leftover Apache
...
# RebuildProductionLayout method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::RebuildProductionLayout",
    "SkyguardMission01EnvironmentDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission01RebuildProductionLayoutDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, REBUILD_PRODUCTION_LAYOUT),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, REBUILD_PRODUCTION_LAYOUT)
        self.assertIn("RebuildProductionLayout", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_missing_rebuild_production_layout_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, REBUILD_PRODUCTION_LAYOUT)
        self.assertIn("RebuildProductionLayout", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, REBUILD_PRODUCTION_LAYOUT)
        self.assertIn("RebuildProductionLayout", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("BlueprintCallable", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("BlueprintPure", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("Category", REBUILD_PRODUCTION_LAYOUT)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, REBUILD_PRODUCTION_LAYOUT)
        self.assertIn("RebuildProductionLayout", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid RebuildProductionLayout() const;\n"
        missing_const = "\tbool RebuildProductionLayout();\n"
        extra_arg = "\tvoid RebuildProductionLayout(int32 Amount);\n"
        extra_args = (
            "\tvoid RebuildProductionLayout("
            "FName ObjectiveId, int32 Amount = 1);\n"
        )
        as_int = "\tint32 RebuildProductionLayout();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, REBUILD_PRODUCTION_LAYOUT)
            self.assertIn("RebuildProductionLayout", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_rebuild_production_layout_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, REBUILD_PRODUCTION_LAYOUT),
            REBUILD_PRODUCTION_LAYOUT,
        )
        self.assertTrue(has_declaration(section, REBUILD_PRODUCTION_LAYOUT))
        self.assertEqual(
            declaration_count(section, REBUILD_PRODUCTION_LAYOUT),
            1,
        )
        self.assertTrue(
            REBUILD_PRODUCTION_LAYOUT.startswith("void "),
            REBUILD_PRODUCTION_LAYOUT,
        )
        self.assertTrue(
            REBUILD_PRODUCTION_LAYOUT.endswith(";"),
            REBUILD_PRODUCTION_LAYOUT,
        )
        self.assertIn("RebuildProductionLayout", REBUILD_PRODUCTION_LAYOUT)
        self.assertIn("RebuildProductionLayout()", REBUILD_PRODUCTION_LAYOUT)
        self.assertTrue(
            REBUILD_PRODUCTION_LAYOUT.endswith("();"),
            REBUILD_PRODUCTION_LAYOUT,
        )
        self.assertNotIn("TArray<", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("TObjectPtr", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("TSoftObjectPtr", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("INDEX_NONE", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("UFUNCTION", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("{", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("}", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("return ", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn(" const;", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("bool ", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("Root", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("Briefing", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("AudioDirector", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("RadioChatter", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("SortiePresentation", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("CampaignDefinition", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("MissionDefinition", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("Readiness", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("bAutoInitialize", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("bAllowBoundedActorSpawning", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("bAutoLaunchAfterBriefing", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("PathfinderSpawnLocation", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetAircraft", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("BindRuntimeActors", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("HandleDroneCityImpact", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("ConfigureMissionDefinition", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("InitializePlayableMission", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("NotifyProtectedAssetFailed", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("SynchronizeRuntimeState", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("NotifyObjectiveProgress", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetReadiness", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetObjectiveRuntime", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetGunner", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetPathfinder", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetMissionId", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("ValidateMissionContract", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("ConfigureFromMission", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("AdvanceBriefing", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("SetAssetsReady", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("AcknowledgeAndLaunch", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("CanLaunch", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetElapsedSeconds", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetBriefingState", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetMinimumWarmupSeconds", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetBriefingText", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetRadioChatter", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetPresentation", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("GetMissionTitle", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("AcknowledgeBriefing", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("LaunchSortie", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("HullCollider", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("OpticalTracker", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("WeaponServo", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("CountermeasurePod", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("MinHeightFromOriginCm", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("RadarNode", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("ESkyguardMission02WaveState", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("HarborIndustrial", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("MaxIntegrity", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("CurrentIntegrity", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("FillAndFinalize", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("FillAndFail", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("IsRouteExclusionSafe", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("bRouteExclusionValid", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            REBUILD_PRODUCTION_LAYOUT,
        )
        self.assertNotIn("bAllowAuthoredPCGGeneration", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("bProductionLandscapeBound", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("bUseAuthoredLandscapeSurface", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("IsPointAllowedForPCG", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("HasContinuousCoastline", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn(
            "RefreshAuthoredEnvironmentBindings",
            REBUILD_PRODUCTION_LAYOUT,
        )
        self.assertNotIn("IsAuthoredEnvironmentReady", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("IsPCGGenerationAuthorized", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("IsCorePlayableReady", REBUILD_PRODUCTION_LAYOUT)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_ENVIRONMENT} {REBUILD_PRODUCTION_LAYOUT}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{REBUILD_PRODUCTION_LAYOUT}\n}}\n"
        self.assertTrue(has_declaration(one_line, REBUILD_PRODUCTION_LAYOUT))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, REBUILD_PRODUCTION_LAYOUT),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{REBUILD_PRODUCTION_LAYOUT}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", REBUILD_PRODUCTION_LAYOUT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_rebuild_production_layout_cpp_body(self) -> None:
        locked_only = f"{REBUILD_PRODUCTION_LAYOUT}\n"
        self.assertNotIn("{", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("}", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("return ", REBUILD_PRODUCTION_LAYOUT)
        ...
        self.assertNotIn("return false", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("return true", REBUILD_PRODUCTION_LAYOUT)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{REBUILD_PRODUCTION_LAYOUT}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, REBUILD_PRODUCTION_LAYOUT)
        self.assertIn("RebuildProductionLayout", REBUILD_PRODUCTION_LAYOUT)
        self.assertTrue(REBUILD_PRODUCTION_LAYOUT.startswith("void "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, REBUILD_PRODUCTION_LAYOUT)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("CurrentIntegrity", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("SkyguardApacheAircraft.h", REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{REBUILD_PRODUCTION_LAYOUT}\n"
        self.assertEqual(
            require_declaration(locked_only, REBUILD_PRODUCTION_LAYOUT),
            REBUILD_PRODUCTION_LAYOUT,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 RebuildProductionLayout method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{REBUILD_PRODUCTION_LAYOUT}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, REBUILD_PRODUCTION_LAYOUT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                REBUILD_PRODUCTION_LAYOUT.lower(),
                "mission01 RebuildProductionLayout contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, REBUILD_PRODUCTION_LAYOUT)

    def test_contract_is_rebuild_production_layout_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn(" const;", REBUILD_PRODUCTION_LAYOUT)
        self.assertTrue(REBUILD_PRODUCTION_LAYOUT.endswith(";"))
        self.assertIn(UFUNCTION_ENVIRONMENT, section)

    def test_sibling_director_fields_do_not_satisfy_rebuild_production_layout(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, REBUILD_PRODUCTION_LAYOUT)
            )
            self.assertNotEqual(REBUILD_PRODUCTION_LAYOUT, leftover)
        self.assertIn(
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("return ", REBUILD_PRODUCTION_LAYOUT)
        self.assertNotIn("return Other", REBUILD_PRODUCTION_LAYOUT)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission01_rebuild_production_layout"
            "_decl_contract.py"
        )
