...
# `FSkyguardMissionScoreRules`, `FSkyguardRouteDefinition`.
# Do NOT contract sibling fields BossId / WeakPoints /
# DefeatObjectiveId / MaximumBreakupPieces.
# THIS IS NOT leftover boss-definition-defaults.
# Analog leftover boss-definition-defaults then isolated
# Callsign field decl.
# THIS IS NOT leftover USkyguardMissionDefinition Boss
# field.
...
# the bulk).
# Do not relock leftover mission-types-defaults-tests.
# If a clone asserts VisibleAnywhere /
# Category="Skyguard|Theater" / FName WeatherIdentity,
# retarget: type is FText, no `=`, EditAnywhere,
...
LEFTOVER_WEAK_POINT_STRUCT = "FSkyguardBossWeakPointDefinition"
LEFTOVER_OBJECTIVE_DEFINITION = "FSkyguardObjectiveDefinition"
LEFTOVER_OBJECTIVE_PROGRESS = "FSkyguardObjectiveProgress"
LEFTOVER_SCORE_RULES = "FSkyguardMissionScoreRules"
LEFTOVER_ROUTE_DEFINITION = "FSkyguardRouteDefinition"
STOP_BEFORE_WEATHER = "struct FSkyguardWeatherProfile"
TARGET = "FText Callsign;"
TARGET_WRONG_EQ_NONE = "FText Callsign = FText::GetEmpty();"
TARGET_WRONG_FALSE = "FText Callsign = false;"
TARGET_WRONG_TRUE = "FText Callsign = true;"
TARGET_WRONG_ZERO = "FText Callsign = 0.f;"
TARGET_WRONG_FLOAT = "float Callsign;"
TARGET_WRONG_FNAME = "FName Callsign;"
...
LEFTOVER_INTEGRITY = "Integrity"
LEFTOVER_MISSION_DEFINITION_BOSS = "FSkyguardBossDefinition Boss;"
...
def leftover_unparsed_field_decls() -> tuple[str, ...]:
    return (
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_WEATHER,
        f"struct {LEFTOVER_WEAK_POINT_STRUCT}",
        f"struct {LEFTOVER_OBJECTIVE_DEFINITION}",
        f"struct {LEFTOVER_OBJECTIVE_PROGRESS}",
        f"struct {LEFTOVER_SCORE_RULES}",
        f"struct {LEFTOVER_ROUTE_DEFINITION}",
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FText\s+Callsign\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FText Callsign is missing from "
...
class BossDefinitionCallsignFieldDeclContractTests(unittest.TestCase):
    def test_boss_definition_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_WEAK_POINT_STRUCT)
        ...
        self.assertTrue(has_identifier(section, "Callsign"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_WEATHER, header)
        self.assertNotIn(STOP_BEFORE_WEATHER, section)
        self.assertNotIn(STOP_BEFORE_WEATHER, body)
        self.assertIn(f"struct {LEFTOVER_WEAK_POINT_STRUCT}", header)
        self.assertNotIn(f"struct {LEFTOVER_WEAK_POINT_STRUCT}", section)
        self.assertNotIn(f"struct {LEFTOVER_WEAK_POINT_STRUCT}", body)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, header)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, section)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_BOSS, section)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_BOSS, body)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for leftover in leftover_unparsed_field_decls():
        ...
        self.assertNotIn(STOP_BEFORE_WEAK_POINT, section)
    ...
    def test_neighbor_or_mission_definition_does_not_satisfy(self) -> None:
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tGENERATED_BODY()\n"
            "};\n"
            f"{STOP_BEFORE_WEATHER}\n"
            "{{\n"
            f"\tFText Callsign;\n"
            "}}\n"
            f"class {LEFTOVER_MISSION_DEFINITION_CLASS}\n"
            "{{\n"
            "public:\n"
            f"\t{LEFTOVER_MISSION_DEFINITION_BOSS}\n"
            f"\t{UPROPERTY_EDIT_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
            "}};\n"
        )
        section = spec_section(mixed)
        self.assertFalse(has_identifier(section, "Callsign"), section)
        self.assertNotIn(STOP_BEFORE_WEATHER, section)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, section)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_BOSS, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Callsign", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_callsign_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FText Callsign;", compact_origin)
        self.assertNotIn("Callsign = FText::GetEmpty()", compact_origin)
        self.assertNotIn("Callsign = false", compact_origin)
        self.assertNotIn("Callsign = true", compact_origin)
        self.assertNotIn("Callsign = 0.f", compact_origin)
        self.assertNotIn("Callsign = 160.f", compact_origin)

    def test_callsign_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText Callsign"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 Callsign;\n",
            "\tbool Callsign;\n",
            "\tfloat Callsign = " + forty + ";\n",
            "\tfloat Callsign = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_boss_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for leftover in leftover_unparsed_field_decls():
            self.assertNotIn(leftover, LOCKED_DECL)
        ...
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_BOSS, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_leftover_structs_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(f"struct {LEFTOVER_WEAK_POINT_STRUCT}", header)
        self.assertNotIn(f"struct {LEFTOVER_WEAK_POINT_STRUCT}", section)
        self.assertNotIn(f"struct {LEFTOVER_WEAK_POINT_STRUCT}", leaked)
        self.assertIn(f"struct {LEFTOVER_OBJECTIVE_DEFINITION}", header)
        self.assertNotIn(f"struct {LEFTOVER_OBJECTIVE_DEFINITION}", section)
        self.assertNotIn(f"struct {LEFTOVER_OBJECTIVE_DEFINITION}", leaked)
        self.assertIn(f"struct {LEFTOVER_OBJECTIVE_PROGRESS}", header)
        self.assertNotIn(f"struct {LEFTOVER_OBJECTIVE_PROGRESS}", section)
        self.assertNotIn(f"struct {LEFTOVER_OBJECTIVE_PROGRESS}", leaked)
        self.assertIn(f"struct {LEFTOVER_SCORE_RULES}", header)
        self.assertNotIn(f"struct {LEFTOVER_SCORE_RULES}", section)
        self.assertNotIn(f"struct {LEFTOVER_SCORE_RULES}", leaked)
        self.assertIn(f"struct {LEFTOVER_ROUTE_DEFINITION}", header)
        self.assertNotIn(f"struct {LEFTOVER_ROUTE_DEFINITION}", section)
        self.assertNotIn(f"struct {LEFTOVER_ROUTE_DEFINITION}", leaked)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, header)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, section)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_CLASS, leaked)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_BOSS, section)
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_BOSS, leaked)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\t{LEFTOVER_MISSION_DEFINITION_BOSS}\n"
            f"\tFName {SIBLING_BOSS_ID};\n"
            f"\tTArray<{LEFTOVER_WEAK_POINT_STRUCT}> "
            f"{SIBLING_WEAK_POINTS};\n"
            f"\tFName {SIBLING_DEFEAT_OBJECTIVE_ID};\n"
            f"\tint32 {SIBLING_MAXIMUM_BREAKUP_PIECES} = 3;\n"
            f"\t{TARGET_WRONG_HEALTH}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss-definition Callsign field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "callsign_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_MISSION_DEFINITION_BOSS_DECL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEAK_POINT_ID_DECL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_REQUIRED_WEAPON_DECL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_EXPOSES_WEAK_POINT_ID_DECL, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_MISSION_DEFINITION_BOSS, locked_only)
        self.assertNotIn(LEFTOVER_WEAK_POINT_STRUCT, locked_only)
        self.assertNotIn(STOP_BEFORE_WEATHER, locked_only)
