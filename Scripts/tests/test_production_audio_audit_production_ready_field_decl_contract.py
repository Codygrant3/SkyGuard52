...
# then `bool bProductionReady = false;`.
...
# bool, initializer dropped or changed from false, or Category=
# added.
...
# MissingOutputSubmixBindings / MissingRoutingAssets /
...
# BlueprintReadOnly with NO EditAnywhere, type is bool,
# REQUIRED initializer `= false`, no Category=.
# Harbor 40/80 fail-closed in the parse window and in THIS
# test file. `= false` is the locked initializer, not Harbor.
# If the BoundProductionSourceCount clone asserts int32 /
# required `= 0`, RETARGET: type is bool, REQUIRED
# initializer `= false`.
...
TARGET = "bool bProductionReady = false;"
TARGET_WRONG_BARE = "bool bProductionReady;"
TARGET_WRONG_FALSE = "int32 bProductionReady = false;"
TARGET_WRONG_TRUE = "bool bProductionReady = true;"
TARGET_WRONG_ZERO = "bool bProductionReady = 0;"
TARGET_WRONG_FORTY = "bool bProductionReady = 40;"
TARGET_WRONG_EIGHTY = "bool bProductionReady = 80;"
TARGET_WRONG_FLOAT = "float bProductionReady = false;"
TARGET_WRONG_FNAME = "FName bProductionReady;"
TARGET_WRONG_BOOL = "int32 bProductionReady = 0;"
TARGET_WRONG_FSTRING = "FString bProductionReady;"
TARGET_WRONG_TARRAY = "TArray<FName> bProductionReady;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> bProductionReady;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `bool bProductionReady = false;`.
    # Reject dropped initializer, Harbor 40/80, true, 0, float, int32,
    # FName, FString, TArray, TSoftObjectPtr.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"bool\s+bProductionReady\s*;",
        compact,
    ) and re.search(
        r"bool\s+bProductionReady\s*=",
        compact,
        ...
        return False
    if re.search(
        r"bProductionReady\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"bProductionReady\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"bProductionReady\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"bProductionReady\s*=\s*0\b",
        compact,
    ):
        return False
    if re.search(
        r"bProductionReady\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+bProductionReady\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+bProductionReady\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+bProductionReady\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+bProductionReady\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<[^>]*>\s+bProductionReady\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+bProductionReady\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+bProductionReady\s*=\s*false\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bProductionReady\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bProductionReady is missing from "
...
class ProductionAudioAuditProductionReadyFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_production_ready_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tint32 {SIBLING_REQUIRED_CATEGORY_COUNT} = 0;\n"
            f"\tbool {SIBLING_BOUND_PRODUCTION_SOURCE_COUNT} = false;\n"
            "};\n"
        )
    ...
    def test_initializer_drop_or_harbor_change_fails_closed(self) -> None:
        ...
        self.assertIn("bool bProductionReady = false;", compact_origin)
        self.assertNotIn("bProductionReady = 40", compact_origin)
        self.assertNotIn("bProductionReady = 80", compact_origin)
        self.assertNotIn("bProductionReady = true", compact_origin)
        self.assertNotIn("bProductionReady;", compact_origin.replace(
            "bool bProductionReady = false;", ""
        ))

    def test_production_ready_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bProductionReady"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tbool {SIBLING_BOUND_PRODUCTION_SOURCE_COUNT} = false;\n",
            "\tbool bProductionReadies = false;\n",
            "\tbool bProductionReady = true;\n",
            "\tint32 bProductionReady = 0;\n",
            "\tfloat bProductionReady = " + forty + ";\n",
            "\tfloat bProductionReady = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit bProductionReady field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "production_ready_field_decl_contract.py"
        ))
