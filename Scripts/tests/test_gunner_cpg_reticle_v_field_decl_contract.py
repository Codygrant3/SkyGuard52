# THIS IS leftover-safe ASkyguardGunner CpgReticleV.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgReticleV;`
# (no initializer).
...
# Stop BEFORE sibling CpgGripLeft as a claimed slot.
...
# CpgEufd, CpgTedacBezel, CpgReticleH, CpgGripLeft,
# CpgGripRight, Boom, GunnerCamera.
...
# Do NOT claim CpgReticleH, CpgTedacBezel, CpgEufd, or
# CpgGripLeft as this slot.
# CpgCanopyBow fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgReticleV;`
...
# CpgReticleV fail-closed if LOCKED_DECL is leftover
# CpgEufd / CpgTedacBezel / CpgReticleH / CpgGripLeft /
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgReticleV;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgReticleV = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgReticleV = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgReticleV = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgReticleV;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgReticleV;"
...
CPG_RETICLE_V_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgReticleV\s*;"
)
CPG_RETICLE_V_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgReticleV\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # CpgGripLeft / CpgTedac / CpgDash / CpgKneeRight /
    # CpgCanopyBow / CpgSeatBack / CpgSeatPan / CpgCockpit /
    # CpgStation / leftover Apache Canopy, or an
    # initializer on CpgReticleV as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_RETICLE_V_DECL_RE.search(compact) is None:
        return False
    if CPG_RETICLE_V_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_GRIP_LEFT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_GRIP_LEFT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_GRIP_LEFT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgReticleV\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgReticleV "
...
class GunnerCpgReticleVFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgReticleV"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_GRIP_LEFT), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_GRIP_LEFT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgReticleV"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgReticleV", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_reticle_v_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgReticleV;", compact_origin)
        self.assertNotIn("CpgReticleV = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_reticle_v_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgReticleV"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgReticleV;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgReticleVs;\n",
            "\tint32 CpgReticleV = 140;\n",
            "\tbool CpgReticleV = true;\n",
            "\tfloat CpgReticleV = " + forty + ";\n",
            "\tfloat CpgReticleV = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_grip_left_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "CpgReticleV"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgReticleV"))
        self.assertTrue(has_identifier(section, "CpgReticleV"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgReticleV field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_reticle_v_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
