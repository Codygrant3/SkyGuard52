...
# then static bool SetTransientLandscapeDiagnosticMaterial(
#   ALandscape* Landscape,
#   UMaterialInterface* Material);
# (declaration-only, no inline body required).
# Compact LOCKED_DECL (NO spaces around `=`):
# static bool SetTransientLandscapeDiagnosticMaterial(ALandscape* Landscape, UMaterialInterface* Material);
...
# Identifier SetTransientLandscapeDiagnosticMaterial is NOT leftover
# analog bool bSuccess = false; (struct field) and NOT leftover unique
# BeginTransientLandscapeDiagnosticMaterialDeferred #1697 and NOT leftover unique
# SetTransientLandscapeDiagnosticMaterialSynchronized (leave sibling uncontracted).
...
# static IS present. method const IS absent. Return type IS bool.
...
# return type is not bool, if static is missing, or if Category is
...
# RETARGET return FSkyguardLandscapeVisibleAudit -> bool.
...
# Clone leftover unique AreResolvedProductionLoopRoutesComplete
# for bool RETARGET; KEEP BlueprintCallable not BlueprintPure; drop const.
...
# KEEP BlueprintCallable (NOT BlueprintPure).
# NOT leftover int32 GetSortieHits() const; and NOT leftover
# analog bool bSuccess = false;.
# Clone #1300 is Category Theater wrapping FName
# WeatherIdentity with no initializer — RETARGET hard.
# LOCKED_DECL is SetTransientLandscapeDiagnosticMaterial
# static bool with ALandscape* Landscape,
# UMaterialInterface* Material.
...
# SetTransientLandscapeDiagnosticMaterialSynchronized and NOT
...
# void SetTransientLandscapeDiagnosticMaterial
# AS THE LOCKED DECL.
...
TARGET_WRONG_FLOAT = (
    "float SetTransientLandscapeDiagnosticMaterial();"
)
TARGET_WRONG_INT = "int32 GetSortieHits() const;"
TARGET_WRONG_RENAME = "void SetTransientLandscapeDiagnosticMaterials();"
...
TARGET_WRONG_SET_TRANSIENT = (
    "static bool BeginTransientLandscapeDiagnosticMaterialDeferred("
)
...
TARGET_WRONG_TRUE = "bool SetTransientLandscapeDiagnosticMaterial = true;"
TARGET_WRONG_ZERO = "bool SetTransientLandscapeDiagnosticMaterial = 0.f;"
...
THIS_SCRIPT = (
    "Scripts/tests/test_mission01_set_transient_landscape_diagnostic_material"
    "_decl_contract.py"
)
LEFTOVER_BEGIN_TRANSIENT_DEFERRED = (
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bstatic\s+bool\s+"
    r"SetTransientLandscapeDiagnosticMaterial\("
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # FSkyguardLandscapeVisibleAudit rename, leftover unique
    # BeginTransientLandscapeDiagnosticMaterialDeferred,
    # leftover unique SetTransientLandscapeDiagnosticMaterialSynchronized,
    # leftover unique PrepareGovernedLandscapeForVisibleValidation,
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(
        r"\bSetTransientLandscapeDiagnosticMaterial\s*\([^)]*\)\s*const\b",
        compact,
    ):
        return False
    if re.search(r"\bvoid\s+SetTransientLandscapeDiagnosticMaterial\b", compact):
        return False
    if re.search(
        r"\bFSkyguardLandscapeVisibleAudit\s+SetTransientLandscapeDiagnosticMaterial\b",
        compact,
    ):
        return False
    if re.search(r"\bfloat\s+SetTransientLandscapeDiagnosticMaterial\b", compact):
        return False
    if re.search(r"\bint32\s+SetTransientLandscapeDiagnosticMaterial\b", compact):
        return False
...
def spec_section(header: str) -> str:
    ...
    method = "SetTransientLandscapeDiagnosticMaterial"
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*static\s+bool\s+"
            r"SetTransientLandscapeDiagnosticMaterial\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for SetTransientLandscapeDiagnosticMaterial "
...
class Mission01SetTransientLandscapeDiagnosticMaterialDeclContractTests(
    unittest.TestCase
):
    def test_mission01_environment_authoring_library_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "SetTransientLandscapeDiagnosticMaterial"),
            section,
        )
    ...
    def test_missing_set_transient_landscape_diagnostic_material_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "SetTransientLandscapeDiagnosticMaterial()const",
    ...
    def test_set_transient_landscape_diagnostic_material_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid SetTransientLandscapeDiagnosticMaterialX();\n",
            "\tint32 SetTransientLandscapeDiagnosticMaterial = 1;\n",
            "\tFName SetTransientLandscapeDiagnosticMaterial = NAME_None;\n",
            "\tvoid SetTransientLandscapeDiagnosticMaterial() = "
            + forty
            + ";\n",
            "\tvoid SetTransientLandscapeDiagnosticMaterial() = "
            ...
            f"\t{TARGET_WRONG_CONFIGURE_CAPTURE}\n",
            f"\t{TARGET_WRONG_FINISH_COMPILE}\n",
            f"\t{TARGET_WRONG_LEFTOVER_PREPARE}\n",
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "SetTransientLandscapeDiagnosticMaterial")
        )
        self.assertTrue(
            has_identifier(section, "SetTransientLandscapeDiagnosticMaterial")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 set transient landscape diagnostic material "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "mission01_set_transient_landscape_diagnostic_material"
            "_decl_contract.py"
        ))
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_BEGIN_TRANSIENT_DEFERRED)
        self.assertIn(LEFTOVER_BEGIN_TRANSIENT_DEFERRED, LOCKED_SCRIPTS)
        self.assertNotIn("SkyguardMission01EnvironmentAuthoringLibrary.cpp", THIS_SCRIPT)
