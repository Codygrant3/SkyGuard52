# THIS IS leftover-safe FSkyguardLoadoutSpec FlareCount.
# origin/main form: BARE `int32 FlareCount = 6;`
...
# Locked decl must be the BARE field `int32 FlareCount = 6;`.
...
# (`int32 FlareCount = 0;` is a different struct).
...
# (split tokens).
# Do NOT contract sibling fields CannonMagazineSize /
# CannonReserve / RocketMagazineSize / RocketReserve /
# GuidedMagazineSize / GuidedReserve (same type int32,
# different initializer — those use SkyguardApacheCpgFeel::
# constants; this uses `= 6`). Also vs Loadout /
# StartingStation / HullIntegrity / PlaystyleLine.
# Fail-closed if type is not `int32`, initializer missing
# or is not `= 6`. Fail-closed on `= 0` (that is leftover
# CpgHudSnapshot.FlareCount), `= 0.f`, `= false`, `= 100`,
# CpgFeel constant initializers.
...
# Harbor 40/80 fail-closed. `= 6` on int32 is not Harbor.
# Sibling HullIntegrity is not this slot and is not Harbor.
...
TARGET = "int32 FlareCount = 6;"
TARGET_WRONG_BARE = "int32 FlareCount;"
TARGET_WRONG_ZERO = "int32 FlareCount = 0;"
TARGET_WRONG_ZERO_F = "int32 FlareCount = 0.f;"
TARGET_WRONG_FALSE = "int32 FlareCount = false;"
TARGET_WRONG_HUNDRED = "int32 FlareCount = 100;"
TARGET_WRONG_CPG_FEEL = (
    "int32 FlareCount = SkyguardApacheCpgFeel::CannonMagazineSize;"
)
TARGET_WRONG_FLOAT = "float FlareCount = 6;"
TARGET_WRONG_BOOL = "bool FlareCount = 6;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `int32 FlareCount = 6;`.
    # Do not accept leftover HudSnapshot `= 0`, `= 0.f`,
    # `= false`, `= 100`, missing initializer, or leftover
    # CpgFeel constant initializers. Do not accept sibling
    # CannonMagazineSize / HullIntegrity / PlaystyleLine.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"int32\s+FlareCount\s*=\s*6\s*;", compact) is None:
        return False
    if re.search(r"int32\s+FlareCount\s*=\s*0\b", compact):
        return False
    if re.search(r"int32\s+FlareCount\s*=\s*0\.f", compact):
        return False
    if re.search(r"int32\s+FlareCount\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+FlareCount\s*=\s*100\b", compact):
        return False
    if re.search(
        r"int32\s+FlareCount\s*=\s*SkyguardApacheCpgFeel::",
        compact,
    ):
        return False
    if re.search(r"float\s+FlareCount\b", compact):
        return False
    if re.search(r"bool\s+FlareCount\b", compact):
        return False
    if re.search(r"int32\s+FlareCount\s*;", compact) and not re.search(
        r"int32\s+FlareCount\s*=",
        compact,
    ):
        return False
    return True
...
class LoadoutSpecFlareCountFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "FlareCount"), section)
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "FlareCount"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("FlareCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_flare_count_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 FlareCount = 6;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("int32 FlareCount"))
    ...
    def test_hud_snapshot_zero_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 FlareCount = 6;", compact_origin)
        self.assertNotIn("int32 FlareCount = 0;", compact_origin)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 FlareCounts = 6;\n",
            "\tint32 FlareCount = " + forty + ";\n",
            "\tint32 FlareCount = " + eighty + ";\n",
        )
    ...
    def test_flare_count_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 FlareCount"),
            LOCKED_DECL,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec FlareCount field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "flare_count_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
