# THIS IS leftover-safe USkyguardPathfinderEncounterController
...
# then `float AttackTelegraphLeadSeconds = 1.2f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Telegraph`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"0.05"`. Default IS `= 1.2f`.
# Clone wrap ClampMin WAS `"0.2"` leftover unique Telegraph
# ApproachAttackIntervalSeconds. THIS slot ClampMin IS `"0.05"`
# (NO spaces around =). Fail-closed if wrap still has ClampMin="0.2"
# leftover unique Telegraph ApproachAttackIntervalSeconds /
# leftover in-flight LockWindowAttackIntervalSeconds /
# leftover sibling CriticalAttackIntervalSeconds.
# Fail-closed if wrap still has ClampMin="0.0" / ClampMin="100.0" /
# ClampMin="1.0". 1.2f is NOT leftover 6.5f and NOT Harbor 40 or 80.
# Category IS Telegraph (NOT leftover Route, NOT leftover Safety,
# NOT leftover Commands, NOT leftover analog Category
# `Skyguard|Boss|Encounter` UFUNCTION).
# It is NOT leftover analog coastal/Mission01 RouteLengthCm = 45000.f.
# It is NOT leftover analog Pathfinder EncounterController UFUNCTION
# AdvanceEncounter / ResetEncounterState / IsRouteStateSafe /
# GetRouteProgress / GetEffectiveSpeedMultiplier /
# IsAttackTelegraphActive / GetTelegraphsTriggered (#376-#382).
# It is NOT leftover analog pathfinder-encounter-controller-field #512.
# It is NOT leftover analog IngressSwayAmplitudeCm = 320.f
# (leave leftover Amplitude uncontracted).
# Do NOT claim leftover unique Telegraph
# ApproachAttackIntervalSeconds = 6.5f (#1672).
# Do NOT claim leftover in-flight LockWindowAttackIntervalSeconds = 8.f
# ClampMin `"0.2"`.
# Do NOT claim leftover sibling CriticalAttackIntervalSeconds = 4.8f
# ClampMin `"0.2"`.
# Do NOT claim leftover sibling MaxTelegraphsPerEncounter = 16 int32.
# Do NOT claim leftover unique Commands CommandLateralBiasCm = 850.f
# (#1673) / BreakClimbCm = 260.f (#1674).
# Fail-closed if the UPROPERTY or decl is missing or renamed, if Category
# is missing, if specifiers drop to VisibleAnywhere / BlueprintReadOnly,
# if ClampMin is missing, if ClampMin is still `"0.2"` / `"0.0"` /
# `"100.0"` / `"1.0"`, if the default is not `= 1.2f`, if Category is
# Skyguard|Apache / Skyguard|Combat / Skyguard|Theater /
# Skyguard|MissionMap / leftover Route / leftover Safety /
# leftover Commands / leftover analog Category
# `Skyguard|Boss|Encounter` without `|Telegraph` /
# Skyguard|Audio|Acceptance /
# Skyguard|Audio|Development / still Category="Skyguard" without
# leftover-safe unique Category path `Skyguard|Boss|Encounter|Telegraph`.
# Accept one-line and split-line UPROPERTY wraps.
# Parse CLASS `USkyguardPathfinderEncounterController` public section
# ONLY after `class USkyguardPathfinderEncounterController` /
# `GENERATED_BODY`. Start at `public:`. Stop BEFORE `private:`.
# Do NOT claim sibling RouteLengthCm (leftover analog coastal/Mission01
# same-name 45000.f).
# Do NOT claim leftover analog Route unique LockWindowSpeedCmPerSecond /
# ApproachSpeedCmPerSecond / CriticalSpeedCmPerSecond /
# IngressSwayAmplitudeCm / IngressSwayWavelengthCm /
# LockWindowClimbCm / LockWindowClimbSeconds /
# CriticalTurnRadiusCm / CriticalTurnSeconds as this slot.
# Do NOT claim leftover analog MinHeightFromOriginCm = -
# plus Harbor 80 float token.
# Do NOT claim leftover analog LockWindowClimbSeconds = 2.4f
# (HelmetLockSeconds analog).
# Do NOT claim leftover analog EncounterController UFUNCTION
# AdvanceEncounter / ResetEncounterState / IsRouteStateSafe /
# GetRouteProgress / GetEffectiveSpeedMultiplier /
# IsAttackTelegraphActive / GetTelegraphsTriggered (#376-#382).
# Do NOT claim leftover analog pathfinder-encounter-controller-field
# #512 TObjectPtr field.
# Do NOT claim leftover analog Pathfinder debris.
# Do NOT claim leftover analog apache-cpg-feel FireRate = 12.0f /
# RecoilPitch = 0.92f.
...
# Clone wrap was leftover unique Telegraph
# Category="Skyguard|Boss|Encounter|Telegraph", meta=(ClampMin="0.2")
# wrapping `float ApproachAttackIntervalSeconds = 6.5f;`
# — RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Telegraph", meta=(ClampMin="0.05")
# (NO spaces around =). Fail-closed if wrap still has
# ClampMin="0.2" leftover unique Telegraph /
# leftover in-flight LockWindowAttackIntervalSeconds /
# leftover sibling CriticalAttackIntervalSeconds, or
# ClampMin="0.0" / "100.0" / "1.0" from leftover Amplitude /
# leftover Route / leftover analog Safety int32.
# Fail-closed if Category is still leftover Route / leftover
# Safety / leftover Commands.
# LOCKED_DECL is `float AttackTelegraphLeadSeconds = 1.2f;`
# NOT leftover `float ApproachAttackIntervalSeconds = 6.5f;`
# and NOT `FName WeatherIdentity;` and NOT leftover Boom and NOT
...
# and NOT leftover LockWindowSpeedCmPerSecond = 850.f
# and NOT leftover CriticalSpeedCmPerSecond = 700.f
# and NOT leftover IngressSwayAmplitudeCm = 320.f
# and NOT leftover IngressSwayWavelengthCm = 9000.f
# and NOT leftover LockWindowClimbCm = 650.f
# and NOT leftover CriticalTurnRadiusCm = 1500.f
# and NOT leftover CriticalTurnSeconds = 5.5f
...
# and NOT leftover analog Telegraph siblings
# ApproachAttackIntervalSeconds / LockWindowAttackIntervalSeconds /
# CriticalAttackIntervalSeconds / MaxTelegraphsPerEncounter
# and NOT leftover unique Commands CommandLateralBiasCm /
# BreakClimbCm.
...
# / ApproachSpeedCmPerSecond / LockWindowSpeedCmPerSecond
# / IngressSwayAmplitudeCm
# / ApproachAttackIntervalSeconds / 6.5f
# AS THE LOCKED DECL.
...
# or leftover Route or leftover Safety or leftover Commands
# or leftover analog Category `Skyguard|Boss|Encounter`
# without `|Telegraph`
# or Category="Skyguard" without |Boss|Encounter|Telegraph.
# Fail-closed if wrap is VisibleAnywhere or BlueprintReadOnly
# (must be EditAnywhere + BlueprintReadWrite).
# Fail-closed if ClampMin is missing, if ClampMin is still `"0.2"`
# / `"0.0"` / `"100.0"` / `"1.0"`, or default is not 1.2f,
# or default is leftover unique Telegraph 6.5f.
...
TARGET = "float AttackTelegraphLeadSeconds = 1.2f;"
TARGET_WRONG_BARE = "float AttackTelegraphLeadSeconds;"
TARGET_WRONG_INIT = "float AttackTelegraphLeadSeconds = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float AttackTelegraphLeadSeconds = true;"
TARGET_WRONG_ZERO = "float AttackTelegraphLeadSeconds = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> AttackTelegraphLeadSeconds;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> AttackTelegraphLeadSeconds;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> AttackTelegraphLeadSeconds;"
...
STOP_BEFORE_SKYLINE_STYLE = "MaxTelegraphsPerEncounter"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_attack_telegraph_lead"
...
TARGET_WRONG_APPROACH_ATTACK = "float ApproachAttackIntervalSeconds = 6.5f;"
...
SIBLING_APPROACH_ATTACK_INTERVAL = "ApproachAttackIntervalSeconds"
...
ATTACK_TELEGRAPH_LEAD_DECL_RE = re.compile(
    r"float\s+AttackTelegraphLeadSeconds\s*=\s*1\.2f\s*;"
)
ATTACK_TELEGRAPH_LEAD_BARE_RE = re.compile(
    r"float\s+AttackTelegraphLeadSeconds\s*;"
)
...
APPROACH_ATTACK_INTERVAL_LEFTOVER_DECL_RE = re.compile(
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_APPROACH_ATTACK_INTERVAL,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float AttackTelegraphLeadSeconds = 1.2f;`.
    ...
    # leftover unique Telegraph ApproachAttackIntervalSeconds = 6.5f,
    ...
    # leftover unique Commands CommandLateralBiasCm = 850.f /
    # BreakClimbCm = 260.f,
    # leftover analog RouteLengthCm = 45000.f,
    # leftover analog LockWindowClimbSeconds = 2.4f,
    # leftover analog ClampMin="0.2" / "0.0" / "100.0" / "1.0",
    ...
    # AttackTelegraphLeadSeconds without the 1.2f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ATTACK_TELEGRAPH_LEAD_DECL_RE.search(compact) is None:
        return False
    if ATTACK_TELEGRAPH_LEAD_BARE_RE.search(compact):
        return False
    ...
    if CRITICAL_ATTACK_LEFTOVER_DECL_RE.search(compact):
        return False
    if APPROACH_ATTACK_INTERVAL_LEFTOVER_DECL_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "AttackTelegraphLeadSeconds":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+AttackTelegraphLeadSeconds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float AttackTelegraphLeadSeconds "
...
class PathfinderEncounterAttackTelegraphLeadFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "AttackTelegraphLeadSeconds"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_APPROACH_SPEED), body)
        self.assertFalse(has_identifier(section, SIBLING_APPROACH_SPEED), section)
        self.assertTrue(has_identifier(body, SIBLING_ROUTE_LENGTH_CM), body)
        self.assertFalse(has_identifier(section, SIBLING_ROUTE_LENGTH_CM), section)
        self.assertTrue(has_identifier(body, SIBLING_LOCK_WINDOW_CLIMB_SECONDS), body)
        self.assertFalse(
            has_identifier(section, SIBLING_EVENT_DEFINITIONS),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_ADVANCE_ENCOUNTER), body)
        self.assertFalse(has_identifier(section, SIBLING_ADVANCE_ENCOUNTER), section)
        self.assertTrue(has_identifier(body, SIBLING_RESET_ENCOUNTER), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "AttackTelegraphLeadSeconds"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("AttackTelegraphLeadSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_attack_telegraph_lead_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Telegraph"', section)
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Telegraph"', specifiers)
        ...
        self.assertIn('ClampMin="0.05"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float AttackTelegraphLeadSeconds;",
            compact_origin.split("AttackTelegraphLeadSeconds = 1.2f;")[0]
            + compact_origin.split("AttackTelegraphLeadSeconds = 1.2f;")[-1]
            if "AttackTelegraphLeadSeconds = 1.2f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_attack_telegraph_lead_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float AttackTelegraphLeadSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1.2f", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_APPROACH_ATTACK)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat AttackTelegraphLeadSecondss = 1.2f;\n",
            "\tint32 AttackTelegraphLeadSeconds = 320;\n",
            "\tFName AttackTelegraphLeadSeconds = NAME_None;\n",
            "\tfloat AttackTelegraphLeadSeconds = " + forty + ";\n",
            "\tfloat AttackTelegraphLeadSeconds = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
            ...
            f"\t{TARGET_WRONG_APPROACH_ATTACK}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Telegraph"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', theater_specs)
        self.assertIn('Category="Skyguard|Boss|Encounter|Telegraph"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "AttackTelegraphLeadSeconds"))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
        self.assertFalse(has_identifier(section, SIBLING_APPROACH_SPEED))
        self.assertTrue(has_identifier(leaked, SIBLING_RESET_ENCOUNTER))
        self.assertFalse(has_identifier(section, SIBLING_RESET_ENCOUNTER))
        self.assertTrue(has_identifier(leaked, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "AttackTelegraphLeadSeconds"))
        self.assertTrue(has_identifier(section, "AttackTelegraphLeadSeconds"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
        self.assertFalse(has_identifier(section, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
        self.assertTrue(has_identifier(leaked, SIBLING_ADVANCE_ENCOUNTER))
        self.assertFalse(has_identifier(section, SIBLING_ADVANCE_ENCOUNTER))
        self.assertTrue(has_identifier(leaked, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
        self.assertFalse(has_identifier(section, SIBLING_LOCK_WINDOW_CLIMB_SECONDS))
        self.assertTrue(has_identifier(leaked, SIBLING_PATHFINDER))
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('ClampMin="0.05"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardPathfinderEncounterController.cpp", THIS_SCRIPT)
        self.assertIn("SkyguardPathfinderEncounterController.h", HEADER_PATH)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder encounter ingress sway amplitude field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_attack_telegraph_lead_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
