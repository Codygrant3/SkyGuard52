# THIS IS leftover-safe FSkyguardBriefingCard::Body.
# origin/main form: split-line
# `UPROPERTY(VisibleAnywhere, BlueprintReadOnly, meta=(MultiLine="true"))`
# then BARE `FText Body;` (no initializer, no Category=).
...
# THIS IS NOT leftover Title field.
# THIS IS NOT leftover GetBriefingText #390.
# Do not contract ESkyguardBriefingPictogram leftover pictogram values.
...
# meta=(MultiLine="true"), no Category=. Accept bare `FText Body;`.
...
TARGET = "FText Body;"
TARGET_WRONG_INIT = "FText Body = FText();"
TARGET_WRONG_EMPTY = "FText Body = FText::GetEmpty();"
LOCKED_DECL = TARGET
UPROPERTY_VISIBLE = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, meta=(MultiLine="true"))'
)
UPROPERTY_NO_MULTILINE = "UPROPERTY(VisibleAnywhere, BlueprintReadOnly)"
STOP_AFTER = "struct FSkyguardBriefingRadioRow"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer when origin/main has `FText Body;`.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    for match in re.finditer(r"UPROPERTY\(", compact):
        ...
        if depth != 0:
            continue
        after = compact[index:]
        if re.match(r"\s*FText\s+Body\b", after):
            return compact[start : index - 1]
    raise AssertionError(
        "UPROPERTY for FText Body is missing from "
...
class BriefingCardBodyFieldDeclContractTests(unittest.TestCase):
    def test_briefing_card_struct_exists(self) -> None:
        ...
        self.assertNotIn("FText Body)", section)
    ...
    def test_missing_body_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_body_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(LOCKED_DECL.startswith("FText Body"), LOCKED_DECL)
    ...
    def test_does_not_parse_neighbors_or_component_methods(self) -> None:
        ...
        self.assertIn(NEIGHBOR_HOW_TO_FLY, header)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing-card Body field contract contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_briefing_card_body_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        locked_only = f"{LOCKED_DECL}\n"
        self.assertIn("Body;", locked_only)
