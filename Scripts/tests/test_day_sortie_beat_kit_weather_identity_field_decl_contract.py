# THIS IS leftover-safe FSkyguardDaySortieBeatKit WeatherIdentity.
...
# Do NOT contract sibling fields MissionId / Beats.
# Day kit has NO bKeepThermal. Do not invent it.
# Do NOT claim leftover NightSortieBeatKit WeatherIdentity
# (#1483 isolated), leftover DaySortieBeat Kind/Call/Threat
# (#1480 / #1481 / #1482), leftover LoadoutSpec, leftover
# Harbor directors.
...
# test_day_sortie_beat_kit_contract.py.
# THIS IS NOT leftover-safe NightSortieBeatKit WeatherIdentity
# #1483 isolated. Keep analog bulk and leftover Night isolated
# in LOCKED_SCRIPTS; isolated field decl does not relock them.
...
# the bare field inside FSkyguardDaySortieBeatKit only.
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # MissionId / Beats fields. Do not invent leftover Night
    # kit bKeepThermal on this Day kit.
...
def leftover_beat_struct_def(region: str):
    return re.search(
...
def spec_section(header: str) -> str:
    ...
    leftover_beat_def = leftover_beat_struct_def(section)
...
class DaySortieBeatKitWeatherIdentityFieldDeclContractTests(
    unittest.TestCase
):
    def test_day_sortie_beat_kit_struct_exists(self) -> None:
        ...
        self.assertIsNone(leftover_beat_struct_def(section), section)
        self.assertIsNone(leftover_beat_struct_def(body), body)
    ...
    def test_does_not_claim_leftover_night_sortie_weather_identity(
        self,
    ) -> None:
    ...
    def test_does_not_parse_leftover_beat_enum_or_neighbors(self) -> None:
        ...
        leftover_beat_in_header = leftover_beat_struct_def(header)
        self.assertIsNotNone(leftover_beat_in_header, header)
        self.assertIsNone(leftover_beat_struct_def(section), section)
        self.assertIsNone(leftover_beat_struct_def(leaked), leaked)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "day-sortie-beat-kit WeatherIdentity field decl contract "
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_NIGHT_KEEP_THERMAL, locked_only)
        for field in leftover_neighbor_hit_fields():
