# THIS IS leftover-safe FSkyguardCpgHudSnapshot ThreatLine.
# origin/main form: plain C++ field `FString ThreatLine;`
...
# (#1477/#1481). ThreatLine is NOT leftover ThreatCount,
...
# bMissileInbound.
# Fail-closed if this lane claims leftover ThreatCount
# or leftover Beat.Threat instead of ThreatLine.
...
# `= FString()` / leftover `int32 ThreatCount = 0` /
# leftover Beat `ESkyguardThreatKind Threat`, retarget:
# type is FString, identifier is ThreatLine, no
...
TARGET = "FString ThreatLine;"
TARGET_WRONG_TEXT = 'FString ThreatLine = TEXT("");'
TARGET_WRONG_FSTRING = "FString ThreatLine = FString();"
TARGET_WRONG_FALSE = "FString ThreatLine = false;"
TARGET_WRONG_TRUE = "FString ThreatLine = true;"
TARGET_WRONG_NONE = "FString ThreatLine = NAME_None;"
TARGET_WRONG_ZERO = "FString ThreatLine = 0.f;"
TARGET_WRONG_FLOAT = "float ThreatLine;"
TARGET_WRONG_HEALTH = "float ThreatLine = 160.f;"
TARGET_WRONG_BOOL = "bool ThreatLine;"
TARGET_WRONG_INT = "int32 ThreatLine;"
TARGET_WRONG_FNAME = "FName ThreatLine;"
TARGET_WRONG_TCHAR = "const TCHAR* ThreatLine;"
TARGET_WRONG_TCHAR_TEXT = 'const TCHAR* ThreatLine = TEXT("");'
...
THREAT_LINE_DECL_RE = re.compile(r"FString\s+ThreatLine\s*;")
THREAT_LINE_INIT_RE = re.compile(r"FString\s+ThreatLine\s*=")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored bare `FString ThreatLine;`.
    # Do not accept invented `= TEXT("")` / `= FString()` /
    # `= false` / leftover ThreatCount / leftover Beat.Threat /
    # leftover ContactMark Label / leftover MissionTitle.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if THREAT_LINE_INIT_RE.search(compact):
        return False
    if THREAT_LINE_DECL_RE.search(compact) is None:
        return False
    if re.search(
    ...
    if re.search(
        ...
        compact,
    ) and not has_identifier(compact, "ThreatLine"):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString ThreatLine\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString ThreatLine is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FString ThreatLine\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FString ThreatLine is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "ThreatLine"):
        raise AssertionError(
            "UPROPERTY clone landed on ThreatLine; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "ThreatLine"):
            raise AssertionError(
                f"{token} clone landed on ThreatLine; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotThreatLineFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "ThreatLine"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString ThreatLine;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("FString ThreatLine"))
        ...
        self.assertIn("ThreatLine", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("ThreatLine", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "ThreatLine"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_threat_line_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_threat_count_does_not_satisfy_threat_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_COUNT}\n"
    ...
    def test_initializer_must_not_exist(self) -> None:
        ...
        self.assertIn("FString ThreatLine;", compact_origin)
        self.assertNotIn("ThreatLine = TEXT(\"\")", compact_origin)
        self.assertNotIn("ThreatLine = FString()", compact_origin)
        self.assertNotIn("ThreatLine = NAME_None", compact_origin)
        self.assertNotIn("ThreatLine = false", compact_origin)
        self.assertNotIn("ThreatLine = true", compact_origin)
        self.assertNotIn("ThreatLine = 0.f", compact_origin)
        self.assertNotIn("ThreatLine = 160.f", compact_origin)

    def test_threat_line_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString ThreatLine"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 ThreatLine;\n",
            "\tbool ThreatLine;\n",
            "\tfloat ThreatLine = " + forty + ";\n",
            "\tfloat ThreatLine = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot ThreatLine field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "threat_line_field_decl_contract.py"
        ))
