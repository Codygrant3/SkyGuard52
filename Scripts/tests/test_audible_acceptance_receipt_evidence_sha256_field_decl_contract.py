...
# then BARE `FString EvidenceSha256;` (no initializer).
# BlueprintReadOnly IS present.
# There is NO EditAnywhere.
# There is NO Category=.
# Bare FString, NO initializer.
# Exact identifier EvidenceSha256 (sibling BuildSha256 also
# contains substring Sha256 -- do not claim BuildSha256).
# Fail-closed if: field missing/renamed, EditAnywhere added,
# BlueprintReadWrite replaces BlueprintReadOnly, type is not
# FString, initializer is added, or Category= is added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardAudibleAcceptanceReceipt` body ONLY
# after `struct FSkyguardAudibleAcceptanceReceipt`.
# Stop BEFORE `class SKYGUARD52_API USkyguardAudioAcceptanceHarness`.
# The API macro is REQUIRED in the stop token.
# Naive `class USkyguardAudioAcceptanceHarness` find returns -1
# and leaks leftover harness class fields
# (`Category="Skyguard|Audio|Acceptance"`,
# `MinimumMeasuredSamples = 600`,
# `MaximumAllowedVoices = 48`).
# Do NOT parse the leftover harness class body.
# Do NOT parse leftover USkyguardAudioProductionBank
# (has Category="Skyguard|Audio|Production").
# Do NOT parse leftover enum class ESkyguardProductionAudioCategory
# (Ri + fleMuzzle / Ig + laSearch).
# Do NOT claim sibling fields: BuildSha256, SampleCount,
...
# #1cd7 then leftover-safe isolated BuildSha256 in-flight
# then this isolated EvidenceSha256 field decl.
# THIS IS NOT leftover #193 / #1cd7 (keep that bulk file
# in LOCKED_SCRIPTS).
# THIS IS NOT leftover ProductionAudio Audit/Entry/Routing
# field decls #1395-#1425.
...
# THIS IS NOT leftover MissionDebrief / ObjectiveProgress /
# RoutePoint / RadioLine / TheaterKit isolated decls.
...
# BlueprintReadWrite / `FName WeatherIdentity` / required
# initializer / EditAnywhere, RETARGET: type is FString,
# no initializer, BlueprintReadOnly, no Category, no
# EditAnywhere, no VisibleAnywhere.
# Harbor 40/80 fail-closed. This field has no Harbor initializer.
...
HARNESS_CLASS = "USkyguardAudioAcceptanceHarness"
ACTOR_NAME = "ASkyguardCampaignTheaterKit"
NAMESPACE_NAME = "SkyguardCampaignTheaterKit"
FIELD_NAME = "EvidenceSha256"
TARGET = "FString EvidenceSha256;"
TARGET_WRONG_EQ_EMPTY = 'FString EvidenceSha256 = TEXT("");'
TARGET_WRONG_EQ_QUOTED = 'FString EvidenceSha256 = "";'
TARGET_WRONG_EQ_NONE = "FString EvidenceSha256 = NAME_None;"
TARGET_WRONG_TRUE = "FString EvidenceSha256 = true;"
TARGET_WRONG_FALSE = "FString EvidenceSha256 = false;"
TARGET_WRONG_ZERO = "FString EvidenceSha256 = 0;"
TARGET_WRONG_FLOAT = "float EvidenceSha256;"
TARGET_WRONG_FNAME = "FName EvidenceSha256;"
TARGET_WRONG_FTEXT = "FText EvidenceSha256;"
TARGET_WRONG_INT = "int32 EvidenceSha256;"
TARGET_WRONG_BOOL = "bool EvidenceSha256 = false;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SIBLING_BUILD = "FString BuildSha256;"
...
STOP_BEFORE_HARNESS_API = (
    "class SKYGUARD52_API USkyguardAudioAcceptanceHarness"
)
NAIVE_HARNESS_STOP = "class USkyguardAudioAcceptanceHarness"
HARNESS_CATEGORY = 'Category="Skyguard|Audio|Acceptance"'
HARNESS_MIN_SAMPLES = "MinimumMeasuredSamples = 600"
HARNESS_MAX_VOICES = "MaximumAllowedVoices = 48"
BANK_CATEGORY = 'Category="Skyguard|Audio|Production"'
...
SIBLING_MAXIMUM_TRUE_PEAK_DBTP = "MaximumTruePeakDbTP"
...
LEFTOVER_PROCEDURAL_BANK_CLASS = "USkyguardAudioProceduralBankComponent"
STOP_BEFORE_PRODUCTION_BANK_API = (
...
LEFTOVER_BANK_ENTRIES = "Entries"
LEFTOVER_BANK_ROUTING_FIELD = "Routing"
...
LEFTOVER_AUDIO_PRODUCTION_BANK_EMPTY_CONTRACT = (
    "Scripts/tests/test_audio_production_bank_empty_fail_closed"
    "_contract.py"
)
...
LEFTOVER_AUDIO_PROCEDURAL_BANK_EMPTY_CONTRACT = (
    "Scripts/tests/test_audio_procedural_bank_empty_fail_closed"
    "_contract.py"
)
IN_FLIGHT_DISPLAY_NAME = (
    "Scripts/tests/test_production_audio_entry_display_name"
    "_field_decl_contract.py"
)
IN_FLIGHT_PROVENANCE_ID = (
...
LOCKED = {
    ...
    "SkyguardAudioProceduralBankTests.cpp",
...
def leftover_receipt_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_AUDIBLE_RECEIPT_DEFAULTS,
        LEFTOVER_AUDIBLE_RECEIPT_DEFAULTS_PY,
        LEFTOVER_AUDIBLE_RECEIPT_DEFAULTS_TESTS,
...
LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_receipt_scripts()
...
def leftover_leaked_harness_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_HARNESS_API,
        HARNESS_CLASS,
        HARNESS_CATEGORY,
...
def leftover_leaked_bank_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_ROUTING,
        STOP_BEFORE_AUDIT,
        ...
        STOP_BEFORE_PRODUCTION_BANK_API,
        LEFTOVER_PRODUCTION_BANK_CLASS,
        LEFTOVER_PROCEDURAL_BANK_CLASS,
        BANK_CATEGORY,
        LEFTOVER_BANK_ENTRIES,
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        SIBLING_MAXIMUM_TRUE_PEAK_DBTP,
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString EvidenceSha256;`.
    # Do not accept `= TEXT("")` / `= ""` / `= NAME_None` /
    # `= false` / `= 0.f` / leftover float Health when
    # origin/main has a bare FString. Do not accept sibling
    # BuildSha256 (also contains substring Sha256) or other
    # receipt fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"FString\s+EvidenceSha256\s*=", compact):
        return False
    if re.search(r"float\s+EvidenceSha256\b", compact):
        return False
    if re.search(r"FName\s+EvidenceSha256\b", compact):
        return False
    if re.search(r"FText\s+EvidenceSha256\b", compact):
        return False
    if re.search(r"int32\s+EvidenceSha256\b", compact):
        return False
    if re.search(r"bool\s+EvidenceSha256\b", compact):
        return False
    if re.search(r"FString\s+BuildSha256\b", compact) and re.search(
        r"FString\s+EvidenceSha256\s*;",
        compact,
    ) is None:
        return False
    if re.search(r"FString\s+EvidenceSha256\s*;", compact) is None:
        return False
    return True
...
def parse_window(header: str) -> str:
    ...
    # Naive `class USkyguardAudioAcceptanceHarness` is not a
    # substring of `class SKYGUARD52_API USkyguardAudioAcceptanceHarness`
    # (`SKYGUARD52_API ` sits between `class` and the type).
    # find() returns -1 and a slice to -1 leaks the leftover
    # harness class body.
    if header.find(NAIVE_HARNESS_STOP) != -1:
        raise AssertionError(
            f"{STRUCT_NAME} parse window used naive {NAIVE_HARNESS_STOP}"
        )
    stop = header.find(STOP_BEFORE_HARNESS_API, start)
    if stop < 0:
        raise AssertionError(
            f"{STRUCT_NAME} parse window is missing "
            f"{STOP_BEFORE_HARNESS_API}"
        )
    window = header[start:stop]
    if STOP_BEFORE_HARNESS_API in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_HARNESS_API}"
        )
    for token in leftover_leaked_harness_tokens():
        if token in window:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes {token}"
            )
    for token in leftover_leaked_bank_tokens():
        if token in window:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes {token}"
            )
    for banned in leftover_live_case_tokens():
        if banned in window:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes {banned}"
            )
...
def struct_body(header: str) -> str:
    window = header
    if STRUCT_RE.search(header) is not None:
        if STOP_BEFORE_HARNESS_API in header:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString\s+EvidenceSha256\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString EvidenceSha256 is missing from "
...
class AudibleAcceptanceReceiptEvidenceSha256FieldDeclContractTests(
    unittest.TestCase
):
    def test_receipt_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, HARNESS_CLASS)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_PRODUCTION_BANK_CLASS)
        ...
        self.assertIn(STOP_BEFORE_HARNESS_API, header)
        self.assertNotIn(STOP_BEFORE_HARNESS_API, section)
        self.assertNotIn(STOP_BEFORE_HARNESS_API, body)
        self.assertNotIn(HARNESS_CLASS, section)
        self.assertNotIn(HARNESS_CLASS, body)
        ...
        self.assertNotIn(HARNESS_MIN_SAMPLES, section)
        self.assertNotIn(HARNESS_MAX_VOICES, section)
        self.assertNotIn(HARNESS_CATEGORY, section)
        self.assertNotIn(BANK_CATEGORY, section)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, section)
        self.assertNotIn(LEFTOVER_APACHE_CLASS, section)

    def test_naive_harness_class_stop_token_returns_minus_one(self) -> None:
        header = origin_main_header()
        self.assertEqual(header.find(NAIVE_HARNESS_STOP), -1)
        self.assertGreaterEqual(header.find(STOP_BEFORE_HARNESS_API), 0)
        start = header.find(f"struct {STRUCT_NAME}")
        leaked = header[start:]
        self.assertIn(HARNESS_MIN_SAMPLES, leaked)
        self.assertIn(HARNESS_MAX_VOICES, leaked)
        self.assertIn(HARNESS_CATEGORY, leaked)
        window = parse_window(header)
        self.assertNotIn("MinimumMeasuredSamples", window)
        self.assertNotIn("MaximumAllowedVoices", window)
        self.assertNotIn(HARNESS_CATEGORY, window)
        self.assertNotIn(STOP_BEFORE_HARNESS_API, window)
        self.assertNotIn(HARNESS_CLASS, window)

    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedAudibleReceipt\n{\n};\n"
            )
    ...
    def test_other_type_does_not_satisfy(self) -> None:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        bank = (
            f"class {LEFTOVER_PRODUCTION_BANK_CLASS}\n"
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
    ...
    def test_harness_or_bank_declaration_does_not_satisfy(self) -> None:
        mixed = (
            ...
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{LOCKED_DECL}\n"
            f"\tint32 MinimumMeasuredSamples = 600;\n"
            "}};\n"
        )
    ...
    def test_missing_evidence_sha256_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_added_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("FString EvidenceSha256;", compact_origin)
        self.assertNotIn("EvidenceSha256 =", compact_origin)
        self.assertNotIn("EvidenceSha256 = true", compact_origin)
        self.assertNotIn("EvidenceSha256 = 0.f", compact_origin)
        self.assertNotIn("EvidenceSha256 = 160.f", compact_origin)

    def test_evidence_sha256_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString EvidenceSha256"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("Category", LOCKED_DECL)
        self.assertNotIn(SIBLING_BUILD_SHA256, LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_EQ_QUOTED}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SIBLING_BUILD}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_SIBLING_BUILD}\n", LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SIBLING_BUILD}\n",
            leftover_primary,
            leftover_guided,
            f"\tFString {SIBLING_BUILD_SHA256};\n",
            "\tFString EvidenceSha256s;\n",
            "\tint32 EvidenceSha256 = 1;\n",
            "\tFText EvidenceSha256;\n",
            "\tfloat EvidenceSha256 = " + forty + ";\n",
            "\tfloat EvidenceSha256 = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        category_specs = attached_uproperty_specifiers(added_category)
        ...
        acceptance_specs = attached_uproperty_specifiers(acceptance)
        self.assertIn("Category", acceptance_specs)
        self.assertIn("Skyguard|Audio|Acceptance", acceptance_specs)
        self.assertNotIn("Category", origin)
        added_edit = (
            f"\t{TARGET_WRONG_EDIT}\n"
            f"\t{LOCKED_DECL}\n"
        )
        edit_specs = attached_uproperty_specifiers(added_edit)
        self.assertIn("EditAnywhere", edit_specs)
        self.assertNotIn("EditAnywhere", origin)
        write_only = (
            f"\t{TARGET_WRONG_WRITE}\n"
            f"\t{LOCKED_DECL}\n"
        )
        write_specs = attached_uproperty_specifiers(write_only)
    ...
    def test_does_not_contract_sibling_receipt_fields(self) -> None:
        ...
        self.assertNotIn(HARNESS_CLASS, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
        ...
        self.assertNotIn(SIBLING_BUILD_SHA256, LOCKED_DECL)
        self.assertIn("Sha256", SIBLING_BUILD_SHA256)
        self.assertIn("Sha256", FIELD_NAME)
        self.assertNotEqual(FIELD_NAME, SIBLING_BUILD_SHA256)
        for field in leftover_neighbor_hit_fields():
    ...
    def test_does_not_parse_harness_bank_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_HARNESS_API, header)
        self.assertNotIn(STOP_BEFORE_HARNESS_API, section)
        self.assertNotIn(STOP_BEFORE_HARNESS_API, leaked)
        self.assertNotIn(STOP_BEFORE_HARNESS_API, window)
        self.assertIn(HARNESS_CLASS, header)
        self.assertNotIn(HARNESS_CLASS, section)
        self.assertNotIn(HARNESS_CLASS, leaked)
        self.assertNotIn(HARNESS_CLASS, window)
        self.assertIn(HARNESS_MIN_SAMPLES, header)
        self.assertNotIn(HARNESS_MIN_SAMPLES, section)
        self.assertNotIn(HARNESS_MIN_SAMPLES, leaked)
        self.assertNotIn(HARNESS_MIN_SAMPLES, window)
        self.assertIn(HARNESS_MAX_VOICES, header)
        self.assertNotIn(HARNESS_MAX_VOICES, section)
        self.assertNotIn(HARNESS_MAX_VOICES, leaked)
        self.assertNotIn(HARNESS_MAX_VOICES, window)
        self.assertIn(HARNESS_CATEGORY, header)
        self.assertNotIn(HARNESS_CATEGORY, section)
        self.assertNotIn(HARNESS_CATEGORY, leaked)
        self.assertNotIn(HARNESS_CATEGORY, window)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, header)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, section)
        self.assertNotIn(BANK_CATEGORY, header)
        self.assertNotIn(BANK_CATEGORY, section)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertNotIn(BANK_CATEGORY, UPROPERTY_READONLY)
        self.assertNotIn("EditAnywhere", UPROPERTY_READONLY)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_READONLY)
        self.assertNotIn("BlueprintReadWrite", UPROPERTY_READONLY)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("SkyguardAudioAcceptanceHarness.cpp", THIS_SCRIPT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audible-acceptance-receipt EvidenceSha256 field decl "
                f"contract contains {banned}; declaration is Apache CPG "
                "30 mm / Hydra / Hellfire, not leftover live cop" + "y",
            )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "evidence_sha256_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioAcceptanceHarness.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardAudioProductionBank.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_AUDIBLE_RECEIPT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIBLE_RECEIPT_DEFAULTS_PY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIBLE_RECEIPT_DEFAULTS_TESTS, LOCKED_SCRIPTS)
        self.assertIn(IN_FLIGHT_BUILD_SHA256, LOCKED_SCRIPTS)
    ...
    def test_leftover_mission_result_defaults_stay_locked(self) -> None:
        leftovers = leftover_receipt_scripts() + (
            LEFTOVER_MISSION_RESULT_DEFAULTS,
            LEFTOVER_MISSION_DEBRIEF_DEFAULTS,
            LEFTOVER_OBJECTIVE_PROGRESS_DEFAULTS,
            LEFTOVER_ADD_OBJECTIVE_PROGRESS,
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(HARNESS_CLASS, locked_only)
        self.assertNotIn(LEFTOVER_PRODUCTION_BANK_CLASS, locked_only)
        ...
        self.assertNotIn(STOP_BEFORE_HARNESS_API, locked_only)
        for helper in namespace_helper_tokens():
