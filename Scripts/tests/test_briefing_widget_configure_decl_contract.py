...
# (draft #366). Leftover sortie-presentation fail-closed
...
# loadout / lock-phase, and leftover drafts #56–#64
# stay sibling-only.
CONFIGURE = (
    "void Configure(USkyguardSortiePresentationComponent* InPresentation);"
)
...
# Stay off GetPresentation, GetMissionTitle,
# GetBriefingText, GetBriefingCards, GetRadioRows,
# GetHowToFlyRows, AcknowledgeBriefing, and
# LaunchSortie on this class. Stay off leftover
...
# USkyguardDebriefWidget, and dirty workspace paths.
LOCKED = {
...
# USkyguardDebriefWidget, and leftover
# ConfigureFromMission stay sibling-only.
LOCKED_SCRIPTS = (
...
# locked here. GetPresentation / GetMissionTitle /
# GetBriefingText / GetBriefingCards / GetRadioRows /
# GetHowToFlyRows / AcknowledgeBriefing / LaunchSortie
# stay sibling-only.
UNLOCKED_NEIGHBORS = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "USkyguardBriefingWidget::Configure",
    "SkyguardSortiePresentationWidgets.cpp",
)
...
class BriefingWidgetConfigureDeclContractTests(unittest.TestCase):
    ...
    def test_contract_does_not_lock_configure_cpp_body(self) -> None:
        ...
        self.assertNotIn("}", CONFIGURE)
        self.assertNotIn("return ", CONFIGURE)
        self.assertNotIn("USkyguardBriefingWidget::Configure", CONFIGURE)
    ...
    def test_contract_does_not_relock_get_briefing_text(self) -> None:
        ...
        for neighbor in GET_BRIEFING_TEXT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE)
        self.assertNotIn("GetBriefingText", CONFIGURE)
    ...
    def test_contract_does_not_relock_get_briefing_cards(self) -> None:
        ...
        for neighbor in GET_BRIEFING_CARDS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE)
        self.assertNotIn("GetBriefingCards", CONFIGURE)
    ...
    def test_contract_does_not_relock_get_radio_rows(self) -> None:
        ...
        for neighbor in GET_RADIO_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE)
        self.assertNotIn("GetRadioRows", CONFIGURE)
    ...
    def test_contract_does_not_relock_get_how_to_fly_rows(self) -> None:
        ...
        for neighbor in GET_HOW_TO_FLY_ROWS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE)
        self.assertNotIn("GetHowToFlyRows", CONFIGURE)
    ...
    def test_contract_does_not_relock_acknowledge_briefing(self) -> None:
        ...
        for neighbor in ACKNOWLEDGE_BRIEFING_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE)
        self.assertNotIn("AcknowledgeBriefing", CONFIGURE)
    ...
    def test_contract_does_not_relock_launch_sortie(self) -> None:
        ...
        for neighbor in LAUNCH_SORTIE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CONFIGURE)
        self.assertNotIn("LaunchSortie", CONFIGURE)
    ...
    def test_contract_parses_public_section_not_debrief_private_or_cpp(
        ...
        self.assertNotIn("USkyguardBriefingWidget::Configure", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "briefing widget Configure contract contains "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"briefing widget Configure contains {banned}; "
