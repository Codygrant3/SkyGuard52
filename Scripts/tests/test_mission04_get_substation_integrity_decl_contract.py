...
# INDEX_NONE or lock GetSubstationIntegrity
# construction in the .cpp. This is leftover-safe
# Mission04 Substation GetSubstationIntegrity
# METHOD on ASkyguardMission04IntegrationDirector.
# Distinct from leftover Mission03 GetConvoyIntegrity
# #645. It is NOT leftover BindRuntimeActors
# (retired live mount in the locked decl), NOT
# leftover GetAircraft (returns the retired live
# mount), NOT leftover HandleDroneCityImpact, NOT
# leftover Harbor Mission02 / Harbor #6/#8/#9, NOT
# leftover Mission04 wave-state enum #bb22
# (do not lock enum values), NOT leftover
# GetSearchlightRuntime #7347, and NOT leftover
# Mission03 GetConvoyIntegrity #645. Distinct from
# leftover briefing-widget / MissionBriefingComponent
# methods.
...
# methods InitializePlayableMission /
...
# GetSearchlightRuntime / GetMissionId /
...
# leftover Mission03 GetConvoyIntegrity
# #645, leftover Mission04 NotifySubstationDamage
# #654, leftover Mission04 GetReadiness
# #658, leftover Mission04 GetWaveState
# #659, leftover Mission04 GetObjectiveRuntime
# #660, leftover Mission04 wave-state enum #bb22
# (do not lock enum values), leftover
...
# Category is Skyguard|Mission04|Substation, not
# Integration, not Waves, not Searchlight, not
...
GET_CONVOY_INTEGRITY = "int32 GetConvoyIntegrity() const;"
...
LEFTOVER_MISSION03_GET_CONVOY_INTEGRITY_NOT_LOCKED = (
    "ASkyguardMission03IntegrationDirector",
    "SkyguardMission03IntegrationDirector.h",
    "test_mission03_get_convoy_integrity_decl_contract.py",
    GET_CONVOY_INTEGRITY,
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission04IntegrationDirector::GetSubstationIntegrity",
    "SkyguardMission04IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission04GetSubstationIntegrityDeclContractTests(unittest.TestCase):
    def test_mission04_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_SUBSTATION_INTEGRITY),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_SUBSTATION_INTEGRITY)
    ...
    def test_missing_get_substation_integrity_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission04IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_SUBSTATION}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_SUBSTATION, section)
        ...
        self.assertNotIn("UFUNCTION", GET_SUBSTATION_INTEGRITY)
        ...
        self.assertNotIn("Integration", UFUNCTION_SUBSTATION)
        self.assertNotIn("Waves", UFUNCTION_SUBSTATION)
        self.assertNotIn("Searchlight", UFUNCTION_SUBSTATION)
        self.assertNotIn("Convoy", UFUNCTION_SUBSTATION)
        self.assertNotIn("Environment", UFUNCTION_SUBSTATION)
        self.assertNotIn("Briefing", UFUNCTION_SUBSTATION)
        self.assertNotIn("Mission02", UFUNCTION_SUBSTATION)
        self.assertNotIn("Boss", UFUNCTION_SUBSTATION)
        self.assertNotIn("Destruction", UFUNCTION_SUBSTATION)
        self.assertNotIn("Apache", UFUNCTION_SUBSTATION)
        self.assertNotIn("Mission07", UFUNCTION_SUBSTATION)
        self.assertNotIn("Mission10", UFUNCTION_SUBSTATION)
        self.assertNotIn("Encounter", UFUNCTION_SUBSTATION)
        self.assertNotIn("Safety", UFUNCTION_SUBSTATION)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_SUBSTATION)
            self.assertNotIn(invented, GET_SUBSTATION_INTEGRITY)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid GetSubstationIntegrity() const;\n"
        as_bool = "\tbool GetSubstationIntegrity() const;\n"
        as_const = (
            "\tint32 "
        ...
        with_arg = (
            "\tint32 "
            "GetSubstationIntegrity(int32 Damage) const;\n"
        )
        renamed = (
            "\tint32 "
            "GetConvoyIntegrity() const;\n"
        )
        short_name = "\tFSkyguardMission04IntegrationReadiness Readiness;\n"
        ...
        leftover_wave_state = f"\t{INITIALIZE_PLAYABLE_MISSION}\n"
        ...
        leftover_integrity = f"\t{GET_WAVE_STATE}\n"
        leftover_convoy_integrity = f"\t{GET_CONVOY_INTEGRITY}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_SUBSTATION_INTEGRITY)
            self.assertIn("GetSubstationIntegrity", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_substation_integrity_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_SUBSTATION_INTEGRITY),
            GET_SUBSTATION_INTEGRITY,
        )
        self.assertTrue(has_declaration(section, GET_SUBSTATION_INTEGRITY))
        self.assertEqual(
            declaration_count(section, GET_SUBSTATION_INTEGRITY),
            1,
        )
        self.assertTrue(
            GET_SUBSTATION_INTEGRITY.startswith("int32"),
            GET_SUBSTATION_INTEGRITY,
        )
        self.assertTrue(
            GET_SUBSTATION_INTEGRITY.endswith(" const"),
            GET_SUBSTATION_INTEGRITY,
        )
        self.assertFalse(
            GET_SUBSTATION_INTEGRITY.endswith(";"),
            GET_SUBSTATION_INTEGRITY,
        )
        self.assertIn("GetSubstationIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity()", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn(
            "ESkyguardMission04WaveState",
            GET_SUBSTATION_INTEGRITY,
        )
        self.assertNotIn("ESkyguardMission03WaveState", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("TArray<", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("TObjectPtr", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("TSoftObjectPtr", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("INDEX_NONE", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("UFUNCTION", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("{", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("}", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return ", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("const;", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("Root", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("Briefing", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("AudioDirector", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("RadioChatter", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("SortiePresentation", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("CampaignDefinition", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("MissionDefinition", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("Readiness;", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("Mission04IntegrationReadiness", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("bAutoInitialize", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("PathfinderSpawnLocation", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetAircraft", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("BindRuntimeActors", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("HandleDroneCityImpact", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("StartNextWave", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("NotifyThreatDestroyed", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("AdvanceConvoyByDistance", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("NotifyConvoyDamage", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetConvoyRouteState", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetDayBeatKit", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetNightBeatKit", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("StartSearchlightWindow", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("AdvanceSearchlightTrack", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("NotifySubstationDamage", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetRemainingThreatsInWave", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetSearchlightRuntime", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetWaveState", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetConvoyIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("RoadHunterSpawnLocation", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ConfigureMissionDefinition", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("NotifyObjectiveProgress", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("SynchronizeRuntimeState", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("IsCorePlayableReady", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetReadiness", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetObjectiveRuntime", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetGunner", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetPathfinder", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetMissionId", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ValidateMissionContract", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ConfigureFromMission", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("AdvanceBriefing", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("SetAssetsReady", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("AcknowledgeAndLaunch", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("CanLaunch", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetElapsedSeconds", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetBriefingState", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetBriefingText", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetRadioChatter", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetPresentation", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("GetMissionTitle", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("AcknowledgeBriefing", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("LaunchSortie", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("HullCollider", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("OpticalTracker", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("WeaponServo", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("CountermeasurePod", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("MinHeightFromOriginCm", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("RadarNode", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ESkyguardMission02WaveState", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("HarborIndustrial", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("MaxIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("CurrentIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("FillAndFinalize", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("FillAndFail", GET_SUBSTATION_INTEGRITY)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_SUBSTATION_INTEGRITY)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction = (
            "public:\n"
            f"\t{UFUNCTION_SUBSTATION}\n"
            f"\t{GET_SUBSTATION_INTEGRITY}\n"
            "};\n"
        )
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SUBSTATION} {GET_SUBSTATION_INTEGRITY}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_SUBSTATION_INTEGRITY}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_SUBSTATION_INTEGRITY))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_SUBSTATION_INTEGRITY),
            section,
    ...
    def test_environment_category_does_not_satisfy_substation(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_SUBSTATION)
        self.assertNotIn("Briefing", UFUNCTION_SUBSTATION)
        environment = (
            'UFUNCTION(BlueprintPure, '
        ...
        self.assertNotEqual(environment, UFUNCTION_SUBSTATION)
        self.assertNotEqual(briefing, UFUNCTION_SUBSTATION)
        self.assertNotIn(environment, UFUNCTION_SUBSTATION)
        self.assertNotIn(briefing, UFUNCTION_SUBSTATION)
        leftover_m01 = (
            'UFUNCTION(BlueprintPure, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_SUBSTATION)
        self.assertNotEqual(leftover_m02, UFUNCTION_SUBSTATION)
        self.assertNotEqual(leftover_harbor, UFUNCTION_SUBSTATION)
        self.assertNotEqual(leftover_m03, UFUNCTION_SUBSTATION)
        self.assertNotEqual(leftover_integration, UFUNCTION_SUBSTATION)
        self.assertNotEqual(leftover_waves, UFUNCTION_SUBSTATION)
        self.assertNotEqual(leftover_searchlight, UFUNCTION_SUBSTATION)
        self.assertNotEqual(leftover_convoy, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_m01, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_m02, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_harbor, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_m03, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_integration, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_waves, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_searchlight, UFUNCTION_SUBSTATION)
        self.assertNotIn(leftover_convoy, UFUNCTION_SUBSTATION)
        self.assertNotIn("Mission01", UFUNCTION_SUBSTATION)
        self.assertNotIn("Mission03", UFUNCTION_SUBSTATION)
        self.assertNotIn("Integration", UFUNCTION_SUBSTATION)
        self.assertNotIn("Waves", UFUNCTION_SUBSTATION)
        self.assertNotIn("Searchlight", UFUNCTION_SUBSTATION)
        self.assertNotIn("Convoy", UFUNCTION_SUBSTATION)
        self.assertNotIn("Objectives", UFUNCTION_SUBSTATION)

    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_SUBSTATION_INTEGRITY}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_SUBSTATION_INTEGRITY)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_SUBSTATION_INTEGRITY)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_substation_integrity_cpp_body(self) -> None:
        locked_only = f"{GET_SUBSTATION_INTEGRITY}\n"
        self.assertNotIn("{", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("}", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return ", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn(
            "ASkyguardMission04IntegrationDirector::InitializePlayableMission",
            GET_SUBSTATION_INTEGRITY,
        )
        self.assertNotIn(
            "SkyguardMission04IntegrationDirector.cpp",
        ...
        self.assertNotIn("return false", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return true", GET_SUBSTATION_INTEGRITY)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_SUBSTATION_INTEGRITY}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertTrue(GET_SUBSTATION_INTEGRITY.startswith("int32"))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_SUBSTATION_INTEGRITY)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_SUBSTATION_INTEGRITY)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("CurrentIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_SUBSTATION_INTEGRITY)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_SUBSTATION_INTEGRITY)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_SUBSTATION_INTEGRITY}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_SUBSTATION_INTEGRITY),
            GET_SUBSTATION_INTEGRITY,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_SUBSTATION_INTEGRITY)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_SUBSTATION_INTEGRITY)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("{", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("}", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return Other", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return WaveState", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return SubstationIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return Readiness", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return false", GET_SUBSTATION_INTEGRITY)
        self.assertNotIn("return true", GET_SUBSTATION_INTEGRITY)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission04 GetSubstationIntegrity method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_SUBSTATION_INTEGRITY}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_SUBSTATION_INTEGRITY.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_SUBSTATION_INTEGRITY.lower(),
                "mission04 GetSubstationIntegrity contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_SUBSTATION_INTEGRITY)

    def test_contract_is_get_substation_integrity_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(GET_SUBSTATION_INTEGRITY.startswith("int32"))
        self.assertIn("GetSubstationIntegrity", GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity() const", GET_SUBSTATION_INTEGRITY)
        self.assertTrue(GET_SUBSTATION_INTEGRITY.endswith(" const"))
        self.assertFalse(GET_SUBSTATION_INTEGRITY.endswith(";"))
        self.assertNotIn("ESkyguardMission03WaveState", GET_SUBSTATION_INTEGRITY)
        self.assertIn(UFUNCTION_SUBSTATION, section)

    def test_sibling_director_fields_do_not_satisfy_get_substation_integrity(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_SUBSTATION_INTEGRITY)
            )
            self.assertNotEqual(GET_SUBSTATION_INTEGRITY, leftover)
        self.assertIn(
    ...
    def test_leftover_mission03_get_convoy_integrity_does_not_satisfy(
        self,
    ) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission03IntegrationDirector "
        ...
        for token in LEFTOVER_MISSION03_GET_CONVOY_INTEGRITY_NOT_LOCKED:
            self.assertNotIn(token, GET_SUBSTATION_INTEGRITY)
        region = f"\t{GET_CONVOY_INTEGRITY}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, GET_SUBSTATION_INTEGRITY)
        self.assertIn("GetSubstationIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetSubstationIntegrity", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_SUBSTATION_INTEGRITY))
        locked_only = f"{GET_SUBSTATION_INTEGRITY}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tint32 "
            "GetSubstationIntegrity() const { return Readiness; }\n"
            "};\n"
        )
