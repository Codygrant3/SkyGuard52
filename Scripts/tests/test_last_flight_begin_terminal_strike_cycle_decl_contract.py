...
# or lock the BeginTerminalStrikeCycle body in the .cpp.
...
# Category="Skyguard|Mission10|Boss") is accepted as
...
# isolated-test drafts #107–#453, leftover
...
# leftover isolated-test drafts #107–#453,
...
# OpenGuidanceArrayExposure / IssueClimbCommand /
# DivertWreckFromCivilians /
# SetCivilianSeparationMeters siblings, leftover
# apache aircraft isolated contracts, leftover
...
# IssueCrossCommand / OpenDispenserBay, leftover
...
# MaxIntegrity, and leftover Last Flight
...
# not locked here. Sibling OpenGuidanceArrayExposure,
# IssueClimbCommand, DivertWreckFromCivilians,
# SetCivilianSeparationMeters, leftover
...
GET_CIVILIAN_SEPARATION = (
    "float GetCivilianSeparationMeters() const"
)
# Harbor-adjacent field on this class. Presence is
# not locked here and is not a Harbor interval
# retune.
...
# .cpp BeginTerminalStrikeCycle body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLastFlightBoss::BeginTerminalStrikeCycle",
    "SkyguardLastFlightBoss.cpp",
    "SetExposed",
...
class LastFlightBeginTerminalStrikeCycleDeclContractTests(unittest.TestCase):
    def test_last_flight_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, BEGIN_TERMINAL_STRIKE_CYCLE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertIn("BeginTerminalStrikeCycle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_missing_begin_terminal_strike_cycle_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLastFlightBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertIn("BeginTerminalStrikeCycle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertIn("BeginTerminalStrikeCycle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("UFUNCTION", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("Category", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("BlueprintCallable", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertIn("BeginTerminalStrikeCycle", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tbool BeginTerminalStrikeCycle;\n"
        wrong_return_void = "\tvoid BeginTerminalStrikeCycle();\n"
        wrong_return_int = "\tint32 BeginTerminalStrikeCycle();\n"
        added_const = "\tbool BeginTerminalStrikeCycle() const;\n"
        added_arg = "\tbool BeginTerminalStrikeCycle(int32 BayIndex);\n"
        leftover_arg = (
            "\tbool BeginTerminalStrikeCycle(int32 BayIndex, float Extra);\n"
        )
        ...
        leftover_separation = f"\t{SET_CIVILIAN_SEPARATION_METERS}\n"
        leftover_first = f"\t{leftover_open_first_window()}\n"
        leftover_final = f"\t{leftover_open_final_window()}\n"
        leftover_core = f"\t{leftover_arm_command_core_path()}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, BEGIN_TERMINAL_STRIKE_CYCLE)
            self.assertIn("BeginTerminalStrikeCycle", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_begin_terminal_strike_cycle_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, BEGIN_TERMINAL_STRIKE_CYCLE))
        self.assertEqual(
            declaration_count(section, BEGIN_TERMINAL_STRIKE_CYCLE),
            1,
        )
        self.assertTrue(
            BEGIN_TERMINAL_STRIKE_CYCLE.startswith("bool "),
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
        self.assertTrue(
            BEGIN_TERMINAL_STRIKE_CYCLE.endswith(";"),
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
        self.assertIn("BeginTerminalStrikeCycle(", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("int32 BayIndex", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("INDEX_NONE", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("{", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("}", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("return ", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(" const", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("OpenGuidanceArrayExposure", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("IssueClimbCommand", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("DivertWreckFromCivilians", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "SetCivilianSeparationMeters",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
        self.assertNotIn("GetChinMuzzleLocation", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("GetGunnerMount", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("FaceWorldLocation", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("PortGuidanceArray", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("SetSearchlightTracked", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("SetContactIdentified", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
        self.assertNotIn("SetLightningExposed", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("OpenUpperEngineExposure", BEGIN_TERMINAL_STRIKE_CYCLE)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{BEGIN_TERMINAL_STRIKE_CYCLE}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, BEGIN_TERMINAL_STRIKE_CYCLE)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, BEGIN_TERMINAL_STRIKE_CYCLE),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{BEGIN_TERMINAL_STRIKE_CYCLE}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", BEGIN_TERMINAL_STRIKE_CYCLE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_begin_terminal_strike_cycle_cpp_body(
        self,
    ) -> None:
        locked_only = f"{BEGIN_TERMINAL_STRIKE_CYCLE}\n"
        self.assertNotIn("{", BEGIN_TERMINAL_STRIKE_CYCLE)
        ...
        self.assertNotIn("return false", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("return true", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("SetExposed", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "bTerminalStrikeCycleOpen",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
    ...
    def test_contract_does_not_relock_open_guidance_array_exposure_sibling(
        ...
        for token in OPEN_GUIDANCE_ARRAY_EXPOSURE_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("OpenGuidanceArrayExposure", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        for token in GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("GetFinaleStage", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("PortGuidanceArray", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("StarboardGuidanceArray", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("PortEngine", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("CommandCore", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("SetSearchlightTracked", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("IsSearchlightTracked", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("ASkyguardBlackKiteBoss", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("SetContactIdentified", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("IsContactIdentified", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("OpenOrbitExposure", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("OpenUpperEngineExposure", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("ReleasePooledEscort", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("ASkyguardIronRainBoss", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("GetGunnerMount", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("GetChinTurret", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("GetSensorTurret", BEGIN_TERMINAL_STRIKE_CYCLE)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{BEGIN_TERMINAL_STRIKE_CYCLE}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("GetChinMuzzleLocation", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("ESkyguardApacheSystem", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("ASkyguardGunner", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("FillAndFinalize", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("SkyguardRadarNode", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("USkyguardBriefingWidget", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("MinHeightFromOriginCm", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("MaxIntegrity", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
        self.assertNotIn("SkyguardPathfinder", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", BEGIN_TERMINAL_STRIKE_CYCLE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{BEGIN_TERMINAL_STRIKE_CYCLE}\n"
        self.assertEqual(
            require_declaration(locked_only, BEGIN_TERMINAL_STRIKE_CYCLE),
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
    ...
    def test_contract_does_not_retune_harbor(self) -> None:
        ...
        # wrong header. Field
        # MinimumCivilianSeparationMeters = 550.f is
        # Harbor-adjacent and is not a Harbor
        # interval retune.
    ...
    def test_minimum_civilian_separation_is_not_harbor_retune(self) -> None:
        section = public_section(origin_main_header())
        locked_only = f"{BEGIN_TERMINAL_STRIKE_CYCLE}\n"
        file_text = this_file_text()
        # Harbor-adjacent field on this class. Do not
        # lock it here and do not treat 550.f as a
        # Harbor interval retune.
        self.assertNotIn(
            "MinimumCivilianSeparationMeters",
            BEGIN_TERMINAL_STRIKE_CYCLE,
        )
        self.assertNotIn("550.f", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("MinimumCivilianSeparationMeters", locked_only)
        self.assertNotIn(MINIMUM_CIVILIAN_SEPARATION, locked_only)
        self.assertNotIn(MINIMUM_CIVILIAN_SEPARATION, BEGIN_TERMINAL_STRIKE_CYCLE)
        for token in leftover_harbor_tokens():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, BEGIN_TERMINAL_STRIKE_CYCLE)
            self.assertNotIn(token, file_text)
        for token in leftover_harbor_clock_tokens():
            self.assertNotIn(token, section)
        self.assertNotEqual(MINIMUM_CIVILIAN_SEPARATION, leftover_harbor_tokens())
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "last flight BeginTerminalStrikeCycle contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, BEGIN_TERMINAL_STRIKE_CYCLE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                BEGIN_TERMINAL_STRIKE_CYCLE.lower(),
                "last flight BeginTerminalStrikeCycle contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, BEGIN_TERMINAL_STRIKE_CYCLE)

    def test_contract_is_begin_terminal_strike_cycle_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertNotIn("{", BEGIN_TERMINAL_STRIKE_CYCLE)
        self.assertTrue(BEGIN_TERMINAL_STRIKE_CYCLE.startswith("bool "))
        self.assertTrue(BEGIN_TERMINAL_STRIKE_CYCLE.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)
