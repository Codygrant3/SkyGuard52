# THIS IS leftover-safe FSkyguardProductionAudioRouting ExteriorSubmix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundSubmixBase> ExteriorSubmix;`
# (no initializer).
...
# WeaponsSubmix / ExplosionsSubmix / RadioSubmix /
# CockpitSoundMix / CockpitExteriorAttenuation /
# CockpitLowPassHz.
# Use identifier bounds so sibling MasterSubmix / CockpitSubmix /
# WeaponsSubmix / RadioSubmix do not false-positive on the
# Submix substring. ExteriorSubmix is NOT CockpitExteriorAttenuation.
# Analog leftover production-audio-routing-defaults then
# isolated ExteriorSubmix field decl.
# THIS IS NOT leftover ExteriorSubmix MasterSubmix analog.
...
TARGET = "TSoftObjectPtr<USoundSubmixBase> ExteriorSubmix;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundSubmixBase> ExteriorSubmix = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundSubmixBase> ExteriorSubmix = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundSubmixBase> ExteriorSubmix = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundSubmixBase> ExteriorSubmix = 0.f;"
TARGET_WRONG_FLOAT = "float ExteriorSubmix;"
TARGET_WRONG_FNAME = "FName ExteriorSubmix;"
TARGET_WRONG_BOOL = "bool ExteriorSubmix;"
TARGET_WRONG_INT = "int32 ExteriorSubmix;"
TARGET_WRONG_FTEXT = "FText ExteriorSubmix;"
TARGET_WRONG_PTR = "USoundSubmixBase* ExteriorSubmix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> ExteriorSubmix;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundMix> ExteriorSubmix;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> ExteriorSubmix;"
TARGET_WRONG_FSTRING = "FString ExteriorSubmix;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling MasterSubmix / CockpitSubmix / RadioSubmix / WeaponsSubmix.
    # ExteriorSubmix is not CockpitExteriorAttenuation.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+ExteriorSubmix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundSubmixBase\s*>\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundMix\s*>\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+ExteriorSubmix\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+ExteriorSubmix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundSubmixBase> ExteriorSubmix is missing from "
...
class ProductionAudioRoutingExteriorSubmixFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("ExteriorSubmix = nullptr", compact_origin)
        self.assertNotIn("ExteriorSubmix = false", compact_origin)
        self.assertNotIn("ExteriorSubmix = true", compact_origin)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tfloat {SIBLING_COCKPIT_EXTERIOR_ATTENUATION};\n",
            "\tTSoftObjectPtr<USoundSubmixBase> ExteriorSubmixes;\n",
            "\tint32 ExteriorSubmix = 1;\n",
            "\tbool ExteriorSubmix;\n",
            "\tfloat ExteriorSubmix = " + forty + ";\n",
            "\tfloat ExteriorSubmix = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_routing_fields(self) -> None:
        ...
        self.assertFalse(
            has_identifier(SIBLING_COCKPIT_EXTERIOR_ATTENUATION, FIELD_NAME)
        )
        self.assertNotEqual(FIELD_NAME, SIBLING_COCKPIT_EXTERIOR_ATTENUATION)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing ExteriorSubmix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "exterior_submix_field_decl_contract.py"
        ))
