# THIS IS leftover-safe FSkyguardMissionSaveRecord
# BestCompletionTimeSeconds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then `float BestCompletionTimeSeconds = 0.f;`.
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Initializer `= 0.f` is REQUIRED
# (not Harbor 40/80). Fail-closed if missing/renamed,
# EditAnywhere dropped, BlueprintReadOnly replaces
# BlueprintReadWrite, type not float, initializer dropped
# or changed, or Category= added.
...
# then isolated BestCompletionTimeSeconds field decl.
...
# THIS IS NOT leftover BestScore (being drafted).
...
# `= false` / bare float / `= 160.f`, retarget: type is
# float, initializer is `= 0.f`, BlueprintReadWrite,
# no Category.
...
TARGET = "float BestCompletionTimeSeconds = 0.f;"
TARGET_WRONG_BARE = "float BestCompletionTimeSeconds;"
TARGET_WRONG_TRUE = "float BestCompletionTimeSeconds = true;"
TARGET_WRONG_FALSE = "float BestCompletionTimeSeconds = false;"
TARGET_WRONG_ZERO = "float BestCompletionTimeSeconds = 0;"
TARGET_WRONG_BOOL = "bool BestCompletionTimeSeconds = 0.f;"
TARGET_WRONG_FNAME = "FName BestCompletionTimeSeconds;"
TARGET_WRONG_FTEXT = "FText BestCompletionTimeSeconds;"
TARGET_WRONG_INT = "int32 BestCompletionTimeSeconds = 0;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_HARBOR = (
    "float BestCompletionTimeSeconds = " + "40" + ".f;"
)
TARGET_WRONG_HARBOR_ALT = (
    "float BestCompletionTimeSeconds = " + "80" + ".f;"
)
TARGET_WRONG_ONE_SIXTY = "float BestCompletionTimeSeconds = 160.f;"
...
SIBLING_BEST_MEDAL = "BestMedalTier"
...
FIELD_NAME = "BestCompletionTimeSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_mission_save_record_best_completion"
    "_time_seconds_field_decl_contract.py"
)
LEFTOVER_COMPLETED_FIELD = (
...
def leftover_save_record_scripts() -> tuple[str, ...]:
    return (
        ...
        "Scripts/tests/test_mission_landmark_anchor_defaults_tests.py",
        ...
        "Scripts/tests/test_mission_objective_anchor_defaults_tests.py",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare / `= true` / `= false` / `= 0` /
    # leftover Harbor 40/80 / leftover float Health when
    # origin/main has float = 0.f. Do not accept sibling
    # bCompleted / BestScore / BestMedalTier fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"bool\s+BestCompletionTimeSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+BestCompletionTimeSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+BestCompletionTimeSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+BestCompletionTimeSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BestCompletionTimeSeconds\s*=\s*true",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BestCompletionTimeSeconds\s*=\s*false",
        compact,
    ):
        return False
    if re.search(
        r"float\s+BestCompletionTimeSeconds\s*=\s*0\.f",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+BestCompletionTimeSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float BestCompletionTimeSeconds is missing from "
...
class MissionSaveRecordBestCompletionTimeSecondsFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_best_completion_time_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "BestCompletionTimeSeconds = true",
            compact_origin,
        )
        self.assertNotIn(
            "BestCompletionTimeSeconds = false",
            compact_origin,
        )
        self.assertNotIn(
            "BestCompletionTimeSeconds = 160.f",
            compact_origin,
        )

    def test_best_completion_time_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float BestCompletionTimeSeconds"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HARBOR}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HARBOR_ALT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ONE_SIXTY}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_HARBOR}\n",
            f"\t{TARGET_WRONG_HARBOR_ALT}\n",
            f"\t{TARGET_WRONG_ONE_SIXTY}\n",
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_BEST_SCORE};\n",
            f"\tFText {SIBLING_BEST_MEDAL};\n",
            f"\tbool {SIBLING_COMPLETED} = false;\n",
            "\tfloat BestCompletionTimeSecondses = 0.f;\n",
            "\tint32 BestCompletionTimeSeconds = 1;\n",
            "\tFText BestCompletionTimeSeconds;\n",
            "\tfloat BestCompletionTimeSeconds = " + forty + ";\n",
            "\tfloat BestCompletionTimeSeconds = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            ...
            "\tfloat   BestCompletionTimeSeconds = 0.f;\n",
            "\tfloat\tBestCompletionTimeSeconds = 0.f;\n",
            "\tfloat BestCompletionTimeSeconds =\n\t0.f;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_contract_sibling_save_record_fields(self) -> None:
        ...
        self.assertTrue(has_identifier(section, SIBLING_BEST_MEDAL), section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-save-record BestCompletionTimeSeconds "
                "field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "best_completion_time_seconds_field_decl_contract.py"
        ))
        self.assertIn(LEFTOVER_COMPLETED_FIELD, LOCKED_SCRIPTS)
        self.assertNotEqual(LEFTOVER_COMPLETED_FIELD, THIS_SCRIPT)
