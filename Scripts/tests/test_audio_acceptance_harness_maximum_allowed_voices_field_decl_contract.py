# THIS IS leftover-safe USkyguardAudioAcceptanceHarness MaximumAllowedVoices.
...
# then `int32 MaximumAllowedVoices = 48;` (HAS initializer; spaces around =).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Audio|Acceptance`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"1"`. Default IS `= 48`.
# THIS IS leftover-safe unique harness THRESHOLD int32, not leftover
# FSkyguardAudibleAcceptanceReceipt.PeakActiveVoices #1438
# (`int32 PeakActiveVoices = 0;` BlueprintReadOnly, no Category) and not
# leftover AudioDirector `GlobalVoiceLimit = 24`.
# It is NOT leftover sibling MinimumMeasuredSamples / MaximumAudioThreadMs /
# MaximumTruePeakDbTP as a claimed slot.
# It is NOT leftover analog audio-harness-fail-closed #116/#f96c C++ bulk
# (keep bulk in LOCKED_SCRIPTS).
# This is a unique audio acceptance harness voice-threshold int32.
...
# Stop BEFORE sibling MaximumAudioThreadMs as a claimed slot.
# Skip leftover analog Root / VFXPool / Readiness / Quality / layout
# floats / WindStrength / WindSpeed / AppliedWeather / HullCollider /
# RotorPower / HoverBob / ShapeMaterial / Ig+la* / Ri+fleMesh / Ya+k*.
# Do NOT claim leftover receipt PeakActiveVoices / SampleCount /
# MaximumAudioThreadMs / MaximumTruePeakDbTP as this slot.
# Do NOT claim leftover sibling MinimumMeasuredSamples as this slot.
# Do NOT parse leftover analog audio-harness-fail-closed bulk, leftover
# analog audio-director `*`, leftover analog
...
# MissionMap actor fields #1630-#1636, leftover analog gunner Boom
# #1629, leftover analog Apache TObjectPtr through EyeMount #1592.
...
# Category Skyguard|Environment|Performance wrapping
# `int32 VegetationStartCullDistanceCm = 8000;` ClampMin `"1000"`
# — RETARGET hard: Category Skyguard|Audio|Acceptance
# wrapping `int32 MaximumAllowedVoices = 48;` plus ClampMin `"1"`.
# Clone #1300 is Category Theater wrapping FName WeatherIdentity
# with NO initializer — RETARGET hard.
# LOCKED_DECL is `int32 MaximumAllowedVoices = 48;` NOT
# `FName WeatherIdentity;` (no initializer) and NOT
# leftover analog `float LookYawLimit = 95.f` and NOT
# `int32 VegetationStartCullDistanceCm = 8000;` /
# `float TraceRange = 32000.f;` / `int32 PeakActiveVoices = 0;` /
# `int32 GlobalVoiceLimit = 24;` / `int32 MinimumMeasuredSamples = 600;`
# / `FName AssemblyRevision;` AS THE LOCKED DECL.
...
# MaximumAllowedVoices is 48 not Harbor 40 / 80.
...
TARGET = "int32 MaximumAllowedVoices = 48;"
TARGET_WRONG_BARE = "int32 MaximumAllowedVoices;"
TARGET_WRONG_INIT = "int32 MaximumAllowedVoices = nullptr;"
TARGET_WRONG_FALSE = "int32 MaximumAllowedVoices = false;"
TARGET_WRONG_TRUE = "int32 MaximumAllowedVoices = true;"
TARGET_WRONG_ZERO = "int32 MaximumAllowedVoices = 0;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MaximumAllowedVoices;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MaximumAllowedVoices;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MaximumAllowedVoices;"
...
STOP_BEFORE_LOOK_PITCH_MIN = "MaximumAudioThreadMs"
STOP_BEFORE_EPIC_SHRUB = "MaximumTruePeakDbTP"
...
SIBLING_LOOK_PITCH_MIN = "MaximumAudioThreadMs"
...
LEFTOVER_MISSION01_CLASS_NAME = "ASkyguardMission01EnvironmentDirector"
...
LEFTOVER_GUNNER_BOOM = (
    "Scripts/tests/test_gunner_boom_field_decl_contract.py"
)
...
def leftover_audio_acceptance_extra_scripts() -> tuple[str, ...]:
    prefix = "Scripts/tests/"
    receipt = (
        "production_bank_ready",
        "accepted",
        "calibrated_metering",
        "audible_device_observed",
        "packaged_development_build",
        "maximum_true_peak_dbtp",
        "maximum_audio_thread_ms",
        "total_underruns",
        "peak_active_voices",
        "evidence_sha256",
        "sample_count",
    ...
    coastal = (
        "vegetation_start_cull_distance_cm",
        "vegetation_end_cull_distance_cm",
        "epic_shrub_budget",
        "epic_tree_budget",
        "placement_seed",
        "shrub_instances",
    ...
    mission01_bools = (
        "use_authored_landscape_surface",
        "licensed_vegetation_library_approved",
        "allow_authored_pcg_generation",
    )
    mission_map = (
        "route_points",
        "landmark_anchors",
        "objective_anchors",
        "flight_clearance_vertical_centimeters",
        "flight_clearance_radius_centimeters",
        "flight_route_spline",
        "assembly_revision",
    )
    audio_director = (
        f"{prefix}test_audio_director_empty_fail_closed.py",
        f"{prefix}test_audio_director_empty_fail_closed_tests.py",
        f"{prefix}test_audio_director_empty_fail_closed_contract.py",
    )
    return (
        LEFTOVER_COASTAL_START_CULL,
        LEFTOVER_THEATER_WEATHER_IDENTITY,
        ...
        LEFTOVER_MISSION_MAP_ASSEMBLY,
        *(
            f"{prefix}test_audible_acceptance_receipt_{name}"
            "_field_decl_contract.py"
            for name in receipt
        ),
        *(
            f"{prefix}test_coastal_{name}_field_decl_contract.py"
            for name in coastal
        ),
        *(
            f"{prefix}test_mission01_{name}_field_decl_contract.py"
            for name in mission01_bools
        ),
        *(
            f"{prefix}test_mission_map_{name}_field_decl_contract.py"
            for name in mission_map
        ),
    ) + audio_director


LOCKED_SCRIPTS = LOCKED_SCRIPTS + leftover_audio_acceptance_extra_scripts()
...
VOICES_DECL_RE = re.compile(
    r"int32\s+MaximumAllowedVoices\s*=\s*48\s*;"
)
VOICES_BARE_RE = re.compile(
    r"int32\s+MaximumAllowedVoices\s*;"
)
VOICES_WRONG_INIT_RE = re.compile(
    r"int32\s+MaximumAllowedVoices\s*="
)
PEAK_ACTIVE_DECL_RE = re.compile(
    r"int32\s+PeakActiveVoices\s*=\s*0\s*;"
)
GLOBAL_VOICE_DECL_RE = re.compile(
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 MaximumAllowedVoices = 48;`.
    # Do not accept leftover FName WeatherIdentity,
    # leftover `= 160.f`, leftover analog LookYawLimit,
    # leftover receipt PeakActiveVoices = 0, leftover
    # AudioDirector GlobalVoiceLimit = 24, leftover sibling
    # MinimumMeasuredSamples = 600, leftover coastal
    # VegetationStartCullDistanceCm = 8000, leftover
    # TraceRange, leftover PlacementSeed, leftover
    # TreeInstances TObjectPtr, leftover AssemblyRevision,
    # or a bare MaximumAllowedVoices without the 48
    # initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if VOICES_DECL_RE.search(compact) is None:
        return False
    if VOICES_BARE_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+LookYawLimit\s*=\s*95\.f\b", compact):
        return False
    if PEAK_ACTIVE_DECL_RE.search(compact):
        return False
    if GLOBAL_VOICE_DECL_RE.search(compact):
        return False
    if MIN_SAMPLES_DECL_RE.search(compact):
        return False
    if START_CULL_LEFTOVER_DECL_RE.search(compact):
        return False
    if ASSEMBLY_DECL_RE.search(compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+MaximumAllowedVoices\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 MaximumAllowedVoices "
...
class AudioAcceptanceHarnessMaximumAllowedVoicesFieldDeclContractTests(unittest.TestCase):
    def test_harness_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MaximumAllowedVoices"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_MIN_SAMPLES), body)
        ...
        self.assertTrue(has_identifier(body, "Receipt"), body)
        self.assertFalse(has_identifier(section, SIBLING_CPG_TEDAC_TEXT), section)
        self.assertTrue(has_identifier(body, "BeginEvidenceRun"), body)
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE), section)
        self.assertTrue(has_identifier(body, "RecordMeasuredSample"), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MaximumAllowedVoices"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MaximumAllowedVoices", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_maximum_allowed_voices_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 MaximumAllowedVoices = 48;", compact_origin)
        self.assertNotIn("MaximumAllowedVoices = 160", compact_origin)
    ...
    def test_maximum_allowed_voices_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 MaximumAllowedVoices"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("48", LOCKED_DECL)
        self.assertNotIn("8000", LOCKED_DECL)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "int32 MaximumAllowedVoices = 48;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PEAK)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GLOBAL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tint32 MaximumAllowedVoicess = 48;\n",
            "\tfloat MaximumAllowedVoices = 48.f;\n",
            "\tbool MaximumAllowedVoices = true;\n",
            "\tint32 MaximumAllowedVoices = " + forty + ";\n",
            "\tint32 MaximumAllowedVoices = " + eighty + ";\n",
            f"\t{TARGET_WRONG_PEAK}\n",
            f"\t{TARGET_WRONG_GLOBAL}\n",
    ...
    def test_does_not_claim_sibling_minimum_measured_samples_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "MaximumAllowedVoices"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "MaximumAllowedVoices"))
        self.assertTrue(has_identifier(section, "MaximumAllowedVoices"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertFalse(has_identifier(section, STOP_BEFORE_LOOK_PITCH_MIN))
        self.assertTrue(has_identifier(leaked, SIBLING_MIN_SAMPLES))
        self.assertFalse(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(leaked, "Receipt"))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio acceptance harness maximum-allowed-voices "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_acceptance_harness_maximum_allowed_voices_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioAcceptanceHarness.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AUDIO_HARNESS_FAIL, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_AUDIO_HARNESS_FAIL_CONTRACT, LOCKED_SCRIPTS)
