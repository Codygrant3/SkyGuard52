...
# or lock the SetFriendlySeparationMeters body in the
# .cpp. origin/main is one line
# (`void SetFriendlySeparationMeters(float
# SeparationMeters);`); accept that form, other
...
# OpenSensorExposure sibling. Stay off leftover
...
# isolated-test drafts #107–#434, leftover
...
# Python SetFriendlySeparationMeters declaration
...
# OpenSensorExposure sibling, leftover apache
...
# leftover Apache MaxIntegrity, leftover
# GetFriendlySeparationMeters sibling, and leftover
...
# leftover GetFriendlySeparationMeters, leftover
...
# .cpp SetFriendlySeparationMeters body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardLifelineHunterBoss::SetFriendlySeparationMeters",
    "SkyguardLifelineHunterBoss.cpp",
    "FMath::Max",
)
SIBLING_TYPES = (
...
class LifelineHunterSetFriendlySeparationMetersDeclContractTests(
    unittest.TestCase
):
    def test_lifeline_hunter_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SET_FRIENDLY_SEPARATION_METERS),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SET_FRIENDLY_SEPARATION_METERS)
        self.assertIn("SetFriendlySeparationMeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_missing_set_friendly_separation_meters_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardLifelineHunterBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SET_FRIENDLY_SEPARATION_METERS)
        self.assertIn("SetFriendlySeparationMeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SET_FRIENDLY_SEPARATION_METERS)
        self.assertIn("SetFriendlySeparationMeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("UFUNCTION", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("Category", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("BlueprintCallable", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SET_FRIENDLY_SEPARATION_METERS)
        self.assertIn("SetFriendlySeparationMeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid SetFriendlySeparationMeters;\n"
        wrong_return_bool = (
            "\tbool SetFriendlySeparationMeters(float SeparationMeters);\n"
        )
        wrong_return_int = (
            "\tint32 SetFriendlySeparationMeters(float SeparationMeters);\n"
        )
        added_const = (
            "\tvoid SetFriendlySeparationMeters(float SeparationMeters) "
            "const;\n"
        ...
        wrong_type = (
            "\tvoid SetFriendlySeparationMeters(int SeparationMeters);\n"
        )
        missing_arg = "\tvoid SetFriendlySeparationMeters();\n"
        added_arg = (
            "\tvoid SetFriendlySeparationMeters("
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SET_FRIENDLY_SEPARATION_METERS)
            self.assertIn("SetFriendlySeparationMeters", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_set_friendly_separation_meters_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, SET_FRIENDLY_SEPARATION_METERS),
            1,
        )
        self.assertTrue(
            SET_FRIENDLY_SEPARATION_METERS.startswith("void "),
            SET_FRIENDLY_SEPARATION_METERS,
        )
        self.assertTrue(
            SET_FRIENDLY_SEPARATION_METERS.endswith(";"),
            SET_FRIENDLY_SEPARATION_METERS,
        )
        self.assertIn(
        ...
        self.assertIn("float SeparationMeters", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("INDEX_NONE", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("{", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("}", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("return ", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(" const", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("OpenSensorExposure", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(
            "GetFriendlySeparationMeters",
            SET_FRIENDLY_SEPARATION_METERS,
        )
        self.assertNotIn("GetChinMuzzleLocation", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("GetGunnerMount", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("FaceWorldLocation", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("OpticalTracker", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(
            "MinimumWeaponSeparationMeters",
            SET_FRIENDLY_SEPARATION_METERS,
        )
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, SET_FRIENDLY_SEPARATION_METERS)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tSetFriendlySeparationMeters(float SeparationMeters);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        one_line = f"{{\npublic:\n\t{SET_FRIENDLY_SEPARATION_METERS}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, SET_FRIENDLY_SEPARATION_METERS)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SET_FRIENDLY_SEPARATION_METERS),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{SET_FRIENDLY_SEPARATION_METERS}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", SET_FRIENDLY_SEPARATION_METERS)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_set_friendly_separation_meters_cpp_body(
        self,
    ) -> None:
        locked_only = f"{SET_FRIENDLY_SEPARATION_METERS}\n"
        self.assertNotIn("{", SET_FRIENDLY_SEPARATION_METERS)
        ...
        self.assertNotIn("return false", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("return true", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("FMath::Max", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_open_sensor_exposure_sibling(self) -> None:
        ...
        for token in OPEN_SENSOR_EXPOSURE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("OpenSensorExposure", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("OpticalTracker", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("WeaponServo", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("CountermeasurePod", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("SetSearchlightTracked", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("IsSearchlightTracked", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("ASkyguardBlackKiteBoss", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("SetContactIdentified", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("IsContactIdentified", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("OpenOrbitExposure", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("GetGunnerMount", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("GetChinTurret", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("GetSensorTurret", SET_FRIENDLY_SEPARATION_METERS)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{SET_FRIENDLY_SEPARATION_METERS}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("GetChinMuzzleLocation", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("ESkyguardApacheSystem", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("ASkyguardGunner", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("FillAndFinalize", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("SkyguardRadarNode", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            SET_FRIENDLY_SEPARATION_METERS,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            SET_FRIENDLY_SEPARATION_METERS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("MinHeightFromOriginCm", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("MaxIntegrity", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            SET_FRIENDLY_SEPARATION_METERS,
        )
        self.assertNotIn("SkyguardPathfinder", SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SET_FRIENDLY_SEPARATION_METERS)
    ...
    def test_minimum_weapon_separation_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn(adjacent, SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn(
            "MinimumWeaponSeparationMeters",
            SET_FRIENDLY_SEPARATION_METERS,
        )
        self.assertNotIn("450.f", SET_FRIENDLY_SEPARATION_METERS)
        for token in leftover_harbor_tokens():
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SET_FRIENDLY_SEPARATION_METERS}\n"
        self.assertEqual(
            require_declaration(locked_only, SET_FRIENDLY_SEPARATION_METERS),
            SET_FRIENDLY_SEPARATION_METERS,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "lifeline hunter SetFriendlySeparationMeters contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SET_FRIENDLY_SEPARATION_METERS.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SET_FRIENDLY_SEPARATION_METERS.lower(),
                "lifeline hunter SetFriendlySeparationMeters contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SET_FRIENDLY_SEPARATION_METERS)

    def test_contract_is_set_friendly_separation_meters_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", SET_FRIENDLY_SEPARATION_METERS)
        self.assertNotIn("{", SET_FRIENDLY_SEPARATION_METERS)
        self.assertTrue(SET_FRIENDLY_SEPARATION_METERS.startswith("void "))
        self.assertTrue(SET_FRIENDLY_SEPARATION_METERS.endswith(";"))
        self.assertIn(UFUNCTION_BOSS, section)
