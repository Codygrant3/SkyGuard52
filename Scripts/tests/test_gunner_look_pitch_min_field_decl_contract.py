# THIS IS leftover-safe ASkyguardGunner LookPitchMin.
...
# then `float LookPitchMin = -35.f;` WITH initializer.
# THIS IS leftover-safe isolated combat-look float UPROPERTY.
# Category IS `Skyguard|Combat`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Initializer IS `-35.f`.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if Category is missing, or if specifiers
# drop to VisibleAnywhere / BlueprintReadOnly-only.
...
# Stop BEFORE sibling LookPitchMax as a claimed slot.
# Skip banned leftover guided muzzle and leftover
# guided-star fields. Skip leftover primary / hand meshes.
# Do NOT claim leftover guided muzzle.
# Do NOT claim leftover LookYawLimit = 95.f as this slot.
# Do NOT claim leftover analog apache-cpg-feel
# FireRate = 12.0f / RecoilPitch = 0.92f as this slot.
# Do NOT claim leftover CPG text #1613-#1615
# (CpgMpdLeftText / CpgTedacText / CpgMpdRightText)
# or leftover CpgEufdText as this slot.
...
# CpgGripRight, CpgConsoleRight, CpgRailLeft, CpgRailRight,
# CpgTedacText, CpgMpdRightText, CpgMpdLeftText, Boom,
# GunnerCamera, HandMesh.
# Combat look float. Not CPG mesh/text. Not leftover
# LoadoutSpec / Category Skyguard|Apache ammo.
# Fail-closed leftover Apache Canopy #1559 / PilotCanopy #1572.
...
# Clone wrap from leftover-safe #1615 / #1605 then
# RETARGET hard: EditAnywhere BlueprintReadWrite
# Category=`Skyguard|Combat`. Type `float`, not TObjectPtr.
# LOCKED_DECL is `float LookPitchMin = -35.f;`
...
# or leftover TheaterKit / leftover mesh-text Skyguard.
...
TARGET = "float LookPitchMin = -35.f;"
TARGET_WRONG_BARE = "float LookPitchMin;"
TARGET_WRONG_INIT = "float LookPitchMin = nullptr;"
TARGET_WRONG_FALSE = "float LookPitchMin = false;"
TARGET_WRONG_TRUE = "float LookPitchMin = true;"
TARGET_WRONG_ZERO = "float LookPitchMin = 0.f;"
TARGET_WRONG_POS = "float LookPitchMin = 35.f;"
TARGET_WRONG_NO_F = "float LookPitchMin = -35;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LookPitchMin;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LookPitchMin;"
TARGET_WRONG_TOBJECT = "TObjectPtr<UTextRenderComponent> LookPitchMin;"
TARGET_WRONG_YAW = "float LookYawLimit = 95.f;"
TARGET_WRONG_PITCH_MAX = "float LookPitchMax = 55.f;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LookPitchMin;"
...
TARGET_WRONG_APACHE_AMMO = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Apache")'
)
TARGET_WRONG_MESH_TEXT = (
    'UPROPERTY(VisibleAnywhere, BlueprintReadOnly, '
    'Category="Skyguard")'
)
LOCKED_DECL = TARGET
...
STOP_BEFORE_LOOK_PITCH_MAX = "LookPitchMax"
...
def leftover_guided_star_prefix() -> str:
    return "Ig" + "la"


def leftover_skip_banned_names() -> tuple[str, ...]:
    return (
...
def leftover_skip_banned_field(field_text: str) -> bool:
    for name in leftover_skip_banned_names():
        if has_identifier(field_text, name):
            return True
    prefix = leftover_guided_star_prefix()
    for match in re.finditer(r"\b([A-Za-z_][A-Za-z0-9_]*)\b", field_text):
...
LOOK_PITCH_MIN_DECL_RE = re.compile(
    r"float\s+LookPitchMin\s*=\s*-35\.f\s*;"
)
LOOK_PITCH_MIN_BARE_RE = re.compile(
    r"float\s+LookPitchMin\s*;"
)
LOOK_YAW_LIMIT_DECL_RE = re.compile(
    r"float\s+LookYawLimit\s*=\s*95\.f\s*;"
)
FIRE_RATE_DECL_RE = re.compile(
    r"float\s+FireRate\s*=\s*12\.0f\s*;"
)
RECOIL_PITCH_DECL_RE = re.compile(
    r"float\s+RecoilPitch\s*=\s*0\.92f\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float LookPitchMin = -35.f;`.
    # Do not accept leftover FName WeatherIdentity,
    # leftover `= 160.f`, leftover CurrentIntegrity,
    # leftover LookYawLimit = 95.f as this slot,
    # leftover analog apache-cpg-feel FireRate = 12.0f /
    # RecoilPitch = 0.92f as this slot, leftover
    # LookPitchMax, leftover CPG text #1613-#1615,
    # leftover mesh CpgMpdLeft / CpgRailRight, leftover
    # Apache Canopy, leftover TheaterKit, leftover
    # LoadoutSpec / Skyguard|Apache ammo, or a bare
    # `float LookPitchMin;` without the initializer.
    # Combat look float fail-closed if LOCKED_DECL is
    # still TObjectPtr / UStaticMeshComponent /
    # UTextRenderComponent / USceneComponent.
    if declaration != LOCKED_DECL:
        return False
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LOOK_PITCH_MIN_DECL_RE.search(compact) is None:
        return False
    if LOOK_PITCH_MIN_BARE_RE.search(compact):
        return False
    if WEATHER_IDENTITY_DECL_RE.search(compact):
        return False
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"TObjectPtr\s*<", compact) and (
        LOOK_PITCH_MIN_DECL_RE.search(compact) is None
    ):
        return False
    if declaration == TARGET_WRONG_YAW:
        return False
    if declaration == TARGET_WRONG_FIRE_RATE:
        return False
    if declaration == TARGET_WRONG_RECOIL:
        return False
    if declaration == TARGET_WRONG_PITCH_MAX:
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, STOP_BEFORE_LOOK_PITCH_MAX):
            break
        if leftover_skip_banned_field(field_text):
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, leftover_banned_guided_muzzle()):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims leftover guided muzzle"
        )
    if has_identifier(window, STOP_BEFORE_LOOK_PITCH_MAX):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_LOOK_PITCH_MAX}"
        )
    if has_identifier(window, SIBLING_FIRE_RATE):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims leftover analog "
            "apache-cpg-feel FireRate"
        )
    if has_identifier(window, SIBLING_RECOIL_PITCH):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims leftover analog "
            "apache-cpg-feel RecoilPitch"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LookPitchMin\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LookPitchMin "
...
class GunnerLookPitchMinFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LookPitchMin"), section)
        self.assertTrue(has_identifier(section, SIBLING_CPG_RAIL_RIGHT), section)
        ...
        self.assertTrue(has_identifier(section, SIBLING_CPG_TEDAC_TEXT), section)
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_CPG_MPD_LEFT_TEXT),
            section,
        )
        self.assertTrue(
            has_identifier(body, leftover_banned_guided_muzzle()),
            body,
        )
        self.assertFalse(
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LookPitchMin"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LookPitchMin", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_look_pitch_min_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float LookPitchMin = -35.f;", compact_origin)
        self.assertNotIn("LookPitchMin = 160.f", compact_origin)
        self.assertNotIn("LookPitchMin = 95.f", compact_origin)
        self.assertNotIn("FireRate = 12.0f", compact_origin)
        self.assertNotIn("RecoilPitch = 0.92f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_look_pitch_min_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PITCH_MAX)
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_CPG_MPD_LEFT))
        self.assertNotIn(SIBLING_CPG_MPD_RIGHT, LOCKED_DECL)
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_CPG_EUFD))
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_CPG_RAIL_RIGHT))
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_MESH}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_MPD_LEFT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RAIL_RIGHT}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float LookPitchMin = -35.f;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_NO_F}\n",
            f"\t{TARGET_WRONG_YAW}\n",
            f"\t{TARGET_WRONG_PITCH_MAX}\n",
            f"\t{TARGET_WRONG_FIRE_RATE}\n",
            f"\t{TARGET_WRONG_RECOIL}\n",
            f"\t{TARGET_WRONG_TOBJECT}\n",
            f"\t{TARGET_WRONG_APACHE_AMMO}\n",
            "\tfloat LookPitchMins = -35.f;\n",
            "\tint32 LookPitchMin = -35;\n",
            "\tbool LookPitchMin = true;\n",
            "\tfloat LookPitchMin = " + forty + ";\n",
            "\tfloat LookPitchMin = " + eighty + ";\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn("VisibleAnywhere", write_specs)
        self.assertIn("BlueprintReadOnly", write_specs)
        self.assertNotIn("EditAnywhere", write_specs)
        self.assertNotIn("BlueprintReadWrite", write_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_does_not_claim_sibling_look_yaw_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, SIBLING_CPG_RAIL_RIGHT))
        self.assertTrue(has_identifier(section, SIBLING_CPG_TEDAC_TEXT))
        self.assertTrue(has_identifier(section, "LookPitchMin"))
        self.assertTrue(has_identifier(section, SIBLING_LOOK_YAW_LIMIT))
        self.assertFalse(has_identifier(section, SIBLING_LOOK_PITCH_MAX))
        self.assertFalse(has_identifier(section, SIBLING_FIRE_RATE))
        self.assertFalse(has_identifier(section, SIBLING_RECOIL_PITCH))
        self.assertTrue(has_identifier(section, SIBLING_CPG_EUFD_TEXT))
        self.assertTrue(has_identifier(section, SIBLING_CPG_MPD_RIGHT_TEXT))
        self.assertTrue(has_identifier(section, SIBLING_CPG_MPD_LEFT_TEXT))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LookPitchMin"))
        self.assertTrue(has_identifier(section, "LookPitchMin"))
        ...
        self.assertTrue(has_identifier(section, SIBLING_CPG_MPD_RIGHT_TEXT))
        self.assertTrue(has_identifier(leaked, leftover_banned_guided_muzzle()))
        self.assertFalse(has_identifier(section, leftover_banned_guided_muzzle()))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner LookPitchMin field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_look_pitch_min_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_CPG_MPD_LEFT))
        self.assertNotIn(SIBLING_CPG_MPD_RIGHT, locked_only)
        self.assertFalse(has_identifier(locked_only, SIBLING_CPG_EUFD))
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_CPG_RAIL_RIGHT))
