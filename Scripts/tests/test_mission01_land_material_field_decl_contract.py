...
# INDEX_NONE or lock LandMaterial construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# LandMaterial distinct from leftover sibling
# OceanMaterial / BeachMaterial (in-flight this
# wave) / LandTiles, leftover Mission01
...
# bAutoLaunchAfterBriefing. Keep LandMaterial
...
# not Mission02 / Boss / Destruction / Integration
# director Skyguard|Mission01. Specifiers are
# EditAnywhere + BlueprintReadWrite, not
# VisibleAnywhere / BlueprintReadOnly /
# EditInstanceOnly. No ClampMin.
...
# LandMaterial field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::LandMaterial",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01LandMaterialFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, LAND_MATERIAL_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, LAND_MATERIAL_FIELD))

    def test_missing_land_material_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", LAND_MATERIAL_FIELD)
        self.assertNotIn("EditAnywhere", LAND_MATERIAL_FIELD)
        self.assertNotIn("EditInstanceOnly", LAND_MATERIAL_FIELD)
        self.assertNotIn("BlueprintReadWrite", LAND_MATERIAL_FIELD)
        self.assertNotIn("VisibleAnywhere", LAND_MATERIAL_FIELD)
        self.assertNotIn("BlueprintReadOnly", LAND_MATERIAL_FIELD)
        self.assertNotIn("Category", LAND_MATERIAL_FIELD)
        self.assertNotIn("ClampMin", LAND_MATERIAL_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, LAND_MATERIAL_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_wrong_signature_does_not_satisfy(self) -> None:
        ...
        as_array = (
            "\tTArray<TObjectPtr<UMaterialInterface>> LandMaterial;\n"
        )
        assigned = (
            "\tTObjectPtr<UMaterialInterface> LandMaterial = nullptr;\n"
        )
        soft = (
            "\tTSoftObjectPtr<UMaterialInterface> LandMaterial;\n"
        )
        wrong_type = "\tTObjectPtr<UMaterial> LandMaterial;\n"
        renamed = "\tTObjectPtr<UMaterialInterface> LandMaterials;\n"
        ...
        scene = "\tTObjectPtr<USceneComponent> LandMaterial;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> LandMaterial;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> LandMaterial;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, LAND_MATERIAL_FIELD)
            self.assertIn("LandMaterial", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_land_material_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LAND_MATERIAL_FIELD),
            LAND_MATERIAL_FIELD,
        )
        self.assertTrue(has_declaration(section, LAND_MATERIAL_FIELD))
        self.assertEqual(
            declaration_count(section, LAND_MATERIAL_FIELD),
            1,
        )
        self.assertTrue(
            LAND_MATERIAL_FIELD.startswith(
                "TObjectPtr<UMaterialInterface> "
            ),
            LAND_MATERIAL_FIELD,
        )
        self.assertTrue(LAND_MATERIAL_FIELD.endswith(";"), LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", LAND_MATERIAL_FIELD)
        self.assertIn("UMaterialInterface", LAND_MATERIAL_FIELD)
        self.assertIn("TObjectPtr<", LAND_MATERIAL_FIELD)
        self.assertNotIn("=", LAND_MATERIAL_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, LAND_MATERIAL_FIELD)
        self.assertNotIn("OceanMaterial", LAND_MATERIAL_FIELD)
        self.assertNotIn("BeachMaterial", LAND_MATERIAL_FIELD)
        self.assertNotIn("TArray<", LAND_MATERIAL_FIELD)
        self.assertNotIn("TSoftObjectPtr", LAND_MATERIAL_FIELD)
        self.assertNotIn("nullptr", LAND_MATERIAL_FIELD)
        self.assertNotIn("INDEX_NONE", LAND_MATERIAL_FIELD)
        self.assertNotIn("UFUNCTION", LAND_MATERIAL_FIELD)
        self.assertNotIn("{", LAND_MATERIAL_FIELD)
        self.assertNotIn("}", LAND_MATERIAL_FIELD)
        self.assertNotIn("return ", LAND_MATERIAL_FIELD)
        self.assertNotIn("OceanTiles", LAND_MATERIAL_FIELD)
        self.assertNotIn("LandTiles", LAND_MATERIAL_FIELD)
        self.assertNotIn("RouteExclusion", LAND_MATERIAL_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", LAND_MATERIAL_FIELD)
        self.assertNotIn("bRouteExclusionValid", LAND_MATERIAL_FIELD)
        self.assertNotIn("BeachTiles", LAND_MATERIAL_FIELD)
        self.assertNotIn("InlandVegetationPCG", LAND_MATERIAL_FIELD)
        self.assertNotIn("LandScatterBounds", LAND_MATERIAL_FIELD)
        self.assertNotIn("ProductionLandscape", LAND_MATERIAL_FIELD)
        self.assertNotIn("AuthoredPCGGraph", LAND_MATERIAL_FIELD)
        self.assertNotIn("DistrictLengthCm", LAND_MATERIAL_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", LAND_MATERIAL_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", LAND_MATERIAL_FIELD)
        self.assertNotIn("BeachWidthCm", LAND_MATERIAL_FIELD)
        self.assertNotIn("InlandExtentCm", LAND_MATERIAL_FIELD)
        self.assertNotIn("SeawardExtentCm", LAND_MATERIAL_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            LAND_MATERIAL_FIELD,
        )
        ...
        self.assertNotIn("Root", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetReadiness", LAND_MATERIAL_FIELD)
        self.assertNotIn("Briefing", LAND_MATERIAL_FIELD)
        self.assertNotIn("AudioDirector", LAND_MATERIAL_FIELD)
        self.assertNotIn("RadioChatter", LAND_MATERIAL_FIELD)
        self.assertNotIn("SortiePresentation", LAND_MATERIAL_FIELD)
        self.assertNotIn("CampaignDefinition", LAND_MATERIAL_FIELD)
        self.assertNotIn("MissionDefinition", LAND_MATERIAL_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", LAND_MATERIAL_FIELD)
        self.assertNotIn("bAutoInitialize", LAND_MATERIAL_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", LAND_MATERIAL_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetAircraft", LAND_MATERIAL_FIELD)
        self.assertNotIn("ConfigureFromMission", LAND_MATERIAL_FIELD)
        self.assertNotIn("AdvanceBriefing", LAND_MATERIAL_FIELD)
        self.assertNotIn("SetAssetsReady", LAND_MATERIAL_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", LAND_MATERIAL_FIELD)
        self.assertNotIn("CanLaunch", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetElapsedSeconds", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetBriefingState", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetBriefingText", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetRadioChatter", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetPresentation", LAND_MATERIAL_FIELD)
        self.assertNotIn("GetMissionTitle", LAND_MATERIAL_FIELD)
        self.assertNotIn("AcknowledgeBriefing", LAND_MATERIAL_FIELD)
        self.assertNotIn("LaunchSortie", LAND_MATERIAL_FIELD)
        self.assertNotIn("HullCollider", LAND_MATERIAL_FIELD)
        self.assertNotIn("OpticalTracker", LAND_MATERIAL_FIELD)
        self.assertNotIn("WeaponServo", LAND_MATERIAL_FIELD)
        self.assertNotIn("CountermeasurePod", LAND_MATERIAL_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", LAND_MATERIAL_FIELD)
        self.assertNotIn("RadarNode", LAND_MATERIAL_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", LAND_MATERIAL_FIELD)
        self.assertNotIn("HarborIndustrial", LAND_MATERIAL_FIELD)
        self.assertNotIn("MaxIntegrity", LAND_MATERIAL_FIELD)
        self.assertNotIn("CurrentIntegrity", LAND_MATERIAL_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, LAND_MATERIAL_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_MATERIALS} {LAND_MATERIAL_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{LAND_MATERIAL_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, LAND_MATERIAL_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, LAND_MATERIAL_FIELD),
            section,
    ...
    def test_assigned_land_material_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UMaterialInterface> LandMaterial = nullptr;\n"
        )
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, LAND_MATERIAL_FIELD))
        with self.assertRaises(AssertionError) as raised_ocean:
            require_declaration(leftover_ocean, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised_ocean.exception))
        self.assertFalse(has_declaration(leftover_ocean, LAND_MATERIAL_FIELD))
        with self.assertRaises(AssertionError) as raised_beach:
            require_declaration(leftover_beach, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised_beach.exception))
        self.assertFalse(has_declaration(leftover_beach, LAND_MATERIAL_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("LandMaterial", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, LAND_MATERIAL_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{LAND_MATERIAL_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, LAND_MATERIAL_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            LAND_MATERIAL_FIELD.startswith("UFUNCTION"),
            LAND_MATERIAL_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", LAND_MATERIAL_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_land_material_cpp_body(self) -> None:
        locked_only = f"{LAND_MATERIAL_FIELD}\n"
        self.assertNotIn("{", LAND_MATERIAL_FIELD)
        self.assertNotIn("}", LAND_MATERIAL_FIELD)
        self.assertNotIn("return ", LAND_MATERIAL_FIELD)
        ...
        self.assertNotIn("return false", LAND_MATERIAL_FIELD)
        self.assertNotIn("return true", LAND_MATERIAL_FIELD)
        self.assertNotIn("CreateDefaultSubobject", LAND_MATERIAL_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", LAND_MATERIAL_FIELD)
        self.assertTrue(
            LAND_MATERIAL_FIELD.startswith(
                "TObjectPtr<UMaterialInterface> "
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, LAND_MATERIAL_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, LAND_MATERIAL_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, LAND_MATERIAL_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", LAND_MATERIAL_FIELD)
        self.assertNotIn("CurrentIntegrity", LAND_MATERIAL_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", LAND_MATERIAL_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, LAND_MATERIAL_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", LAND_MATERIAL_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{LAND_MATERIAL_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, LAND_MATERIAL_FIELD),
            LAND_MATERIAL_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, LAND_MATERIAL_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, LAND_MATERIAL_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 LandMaterial field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{LAND_MATERIAL_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, LAND_MATERIAL_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LAND_MATERIAL_FIELD.lower(),
                "mission01 LandMaterial contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, LAND_MATERIAL_FIELD)

    def test_contract_is_land_material_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("UMaterialInterface", LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", LAND_MATERIAL_FIELD)
        self.assertTrue(LAND_MATERIAL_FIELD.endswith(";"))
        self.assertNotIn("=", LAND_MATERIAL_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, LAND_MATERIAL_FIELD)
        self.assertIn("TObjectPtr<", LAND_MATERIAL_FIELD)
        self.assertNotIn("OceanMaterial", LAND_MATERIAL_FIELD)
    ...
    def test_sibling_environment_fields_do_not_satisfy_land_material(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, LAND_MATERIAL_FIELD))
            self.assertNotEqual(LAND_MATERIAL_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_land_material(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            "\t/**\n"
            "\t * Mission 1 inland surface material assignment.\n"
            "\t * This comment is not the LandMaterial declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, LAND_MATERIAL_FIELD)
        self.assertIn("LandMaterial", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, LAND_MATERIAL_FIELD)
        )
        self.assertNotIn(
            "TObjectPtr<UMaterialInterface> LandMaterial;",
            comment_only,
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LAND_MATERIAL_FIELD)
            self.assertNotEqual(token, LAND_MATERIAL_FIELD)
        self.assertNotIn("RouteLengthCm", LAND_MATERIAL_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, LAND_MATERIAL_FIELD)
        self.assertNotIn("7500.f", LAND_MATERIAL_FIELD)
        self.assertNotIn("2800.f", LAND_MATERIAL_FIELD)
        self.assertNotIn("5200.f", LAND_MATERIAL_FIELD)
        self.assertNotIn("1800.f", LAND_MATERIAL_FIELD)
        self.assertNotIn("18000.f", LAND_MATERIAL_FIELD)
        self.assertNotIn("20000.f", LAND_MATERIAL_FIELD)
