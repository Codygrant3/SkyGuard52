...
# INDEX_NONE or lock bEvacuationPresentationReady
# construction in the .cpp. This is leftover-safe
# Mission10 bEvacuationPresentationReady FIELD
...
# NOT leftover Mission10 SortiePresentation
...
# drafts #674-#814+, leftover Apache
...
EVACUATION_PRESENTATION_READY_FIELD = (
    "bool bEvacuationPresentationReady = false;"
)
LOCKED_DECL = EVACUATION_PRESENTATION_READY_FIELD
...
# an isolated Python bEvacuationPresentationReady
...
class Mission10EvacuationPresentationReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_evacuation_presentation_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_evacuation_presentation_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bEvacuationPresentationReady"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("bEvacuationPresentationReady", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tbEvacuationPresentationReady = false;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bEvacuationPresentationReady", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission10 bEvacuationPresentationReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission10 bEvacuationPresentationReady contains "
    ...
    def test_contract_is_evacuation_presentation_ready_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", LOCKED_DECL)
        self.assertNotIn("{", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bEvacuationPresentationReady"))
        self.assertIn("bEvacuationPresentationReady", LOCKED_DECL)
    ...
    def test_sibling_director_fields_do_not_satisfy_evacuation_presentation_ready(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        assigned = (
            "\tbool bEvacuationPresentationReady = true;\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission10_evacuation_presentation_ready_field"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_evacuation_presentation_ready_field"
                "_decl_contract.py"
            )
        )
        self.assertIn(
