# THIS IS leftover-safe FSkyguardMissionDebrief::NextMissionMap.
# origin/main form: split-line UPROPERTY(VisibleAnywhere,
# BlueprintReadOnly) then `TSoftObjectPtr<UWorld> NextMissionMap;` with no
# Category= and no initializer.
# If a clone asserts `= false` / `= 0` / `= nullptr` / an
# initializer, drop that and accept the bare decl. If a clone
# asserts Category= and origin/main has none, drop that
# assertion. If a clone asserts MultiLine meta and origin/main
# has none, drop that assertion.
...
# bProgressSaved, SaveSlotName, NextMissionDisplayName,
# NextMissionId, bNextMissionUnlocked, bCampaignComplete).
...
# no Category=. Accept bare `TSoftObjectPtr<UWorld> NextMissionMap;`. Fail-closed if
# type/decl missing, renamed, Category= added, or an initializer
# appears.
...
TARGET = "TSoftObjectPtr<UWorld> NextMissionMap;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<UWorld> NextMissionMap = 0;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<UWorld> NextMissionMap = false;"
TARGET_WRONG_ONE = "TSoftObjectPtr<UWorld> NextMissionMap = 1;"
TARGET_WRONG_FLOAT = "TSoftObjectPtr<UWorld> NextMissionMap = 0.f;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<UWorld> NextMissionMap = true;"
TARGET_WRONG_INIT = "TSoftObjectPtr<UWorld> NextMissionMap = TSoftObjectPtr<UWorld>();"
TARGET_WRONG_NONE = "TSoftObjectPtr<UWorld> NextMissionMap = nullptr;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # = false / = 1 / = 0.f / = true / TSoftObjectPtr() / nullptr for
    # FSkyguardMissionDebrief::NextMissionMap.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"TSoftObjectPtr\s*<\s*UWorld\s*>\s+NextMissionMap\s*=", compact):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    field = re.search(r"TSoftObjectPtr\s*<\s*UWorld\s*>\s+NextMissionMap\b", compact)
    if field is None:
        raise AssertionError(
            "UPROPERTY for TSoftObjectPtr<UWorld> NextMissionMap is missing from "
...
class MissionDebriefNextMissionMapFieldDeclContractTests(
    unittest.TestCase
):
    def test_mission_debrief_struct_exists(self) -> None:
        ...
        self.assertIn("NextMissionMap", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("NextMissionMap", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_next_mission_map_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
    ...
    def test_next_mission_map_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSoftObjectPtr<UWorld> NextMissionMap"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "TSoftObjectPtr<UWorld> NextMissionMap;")
        ...
        self.assertNotIn("TSoftObjectPtr<UWorld>()", LOCKED_DECL)
        self.assertNotIn("nullptr", LOCKED_DECL)
        self.assertIn("TSoftObjectPtr<UWorld>", LOCKED_DECL)
        self.assertIn("NextMissionMap", LOCKED_DECL)
        ...
        self.assertIn("UWorld", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            "\tTSoftObjectPtr<UWorld> NextMissionMap = 0;\n",
            "\tTSoftObjectPtr<UWorld> NextMissionMap = false;\n",
            "\tbool NextMissionMap = false;\n",
            "\tFName NextMissionMap;\n",
            "\tTSoftObjectPtr<UWorld> NextMissionId;\n",
            "\tFString NextMissionMap;\n",
            "\tFText NextMissionMap;\n",
            "\tFName NextMissionDisplayName;\n",
            "\tFText NextMissionDisplayName;\n",
            "\tFText MissionDisplayName;\n",
            "\tTSoftObjectPtr<UWorld> NextMissionMap = TSoftObjectPtr<UWorld>();\n",
            "\tTSoftObjectPtr<UWorld> NextMissionMap = nullptr;\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief NextMissionMap field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_next_mission_map"
            "_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("NextMissionDisplayName", locked_only)
        ...
        self.assertIn("NextMissionMap", locked_only)
