...
# then BARE `bool AreResolvedProductionLoopRoutesComplete() const;`
# (declaration-only, no inline body required).
# THIS IS leftover-safe isolated actor/component UFUNCTION.
# Category IS `Skyguard|Audio|Production`. BlueprintPure IS present.
# const IS present. Return type IS bool. Declaration-only.
...
# section ONLY after `class USkyguardAudioDirectorComponent`
# / GENERATED_BODY. Start at `public:`. Stop BEFORE `private:`.
# Stop BEFORE first public UPROPERTY (`EventDefinitions`).
# Do NOT claim sibling UFUNCTION
# GetResolvedProductionLoopRouteCount as this slot
# (leave it uncontracted).
# Do NOT claim sibling GetWindBlend / GetPowerBlend /
# GetCruiseBlend / GetIdleBlend / GetActiveVoiceCount /
# GetTelemetry / GetSuppressionAmount / AdvanceAudioState /
# ApplyHearingSuppression / SetListenerPerspective /
# SetEngineState / TriggerWorldEvent / TriggerEvent /
# GetProductionBankAudit / ApplyProductionBank /
# PrimeConfiguredAssets (those use Category="Skyguard|Audio"
# leftover analog, NOT |Production).
...
# Skip leftover EngineIdleLoop #1653 / EngineCruiseLoop #1655 /
# EnginePowerLoop #1654 / OpenCockpitWindLoop #1656 /
# GlobalVoiceLimit #1657.
# Skip leftover analog ProductionAudioRouting
# CockpitExteriorAttenuation #1409 / CockpitLowPassHz #1413.
# Skip leftover analog audio-production-bank-empty-fail-closed
# #6964.
# Skip leftover analog audio-procedural-bank-empty-fail-closed
# #145/#5ee6.
# Skip leftover analog audio-harness-fail-closed #116/#f96c.
...
# Skip leftover Mission01 AudioDirector TObjectPtr #572.
...
# (NO spaces around =).
# Better bool UFUNCTION BlueprintPure shape leftover analog
# radar-node-is-destroyed / protect-asset-is-destroyed
# — RETARGET class -> USkyguardAudioDirectorComponent,
# Category -> Skyguard|Audio|Production, method ->
# AreResolvedProductionLoopRoutesComplete.
# Leftover analog theater-kit-get-lamp-instance-count is
# int32 not bool and Category Skyguard|Theater — RETARGET
...
# LOCKED_DECL is `bool AreResolvedProductionLoopRoutesComplete() const;`
...
TARGET = "bool AreResolvedProductionLoopRoutesComplete() const;"
TARGET_INLINE = (
    "bool AreResolvedProductionLoopRoutesComplete() const { return false; }"
)
TARGET_WRONG_BARE = "bool AreResolvedProductionLoopRoutesComplete();"
TARGET_WRONG_NO_CONST = (
    "bool AreResolvedProductionLoopRoutesComplete() { return false; }"
)
TARGET_WRONG_VOID = "void AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_FLOAT = "float AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_INT = "int32 AreResolvedProductionLoopRoutesComplete() const;"
...
TARGET_WRONG_TRUE = "bool AreResolvedProductionLoopRoutesComplete = true;"
TARGET_WRONG_ZERO = "bool AreResolvedProductionLoopRoutesComplete = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bbool\s+AreResolvedProductionLoopRoutesComplete\s*\(\s*\)\s*const\s*(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+AreResolvedProductionLoopRoutesComplete\b", compact):
        return False
    if re.search(r"\bfloat\s+AreResolvedProductionLoopRoutesComplete\b", compact):
        return False
    if re.search(r"\bint32\s+AreResolvedProductionLoopRoutesComplete\b", compact):
        return False
    if re.search(
        r"\bbool\s+AreResolvedProductionLoopRoutesComplete\s*\(\s*[^)]+\s*\)",
        compact,
    ):
        return False
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+AreResolvedProductionLoopRoutesComplete\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for bool AreResolvedProductionLoopRoutesComplete "
...
class AudioDirectorAreResolvedProductionLoopRoutesCompleteDeclContractTests(
    unittest.TestCase
):
    def test_audio_director_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "AreResolvedProductionLoopRoutesComplete"),
            section,
        )
    ...
    def test_missing_are_resolved_production_loop_routes_complete_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "bool AreResolvedProductionLoopRoutesComplete();",
    ...
    def test_are_resolved_production_loop_routes_complete_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool AreResolvedProductionLoopRoutesCompletes() const;\n",
            "\tint32 AreResolvedProductionLoopRoutesComplete = 1;\n",
            "\tFName AreResolvedProductionLoopRoutesComplete = NAME_None;\n",
            "\tbool AreResolvedProductionLoopRoutesComplete() const = "
            + forty
            + ";\n",
            "\tbool AreResolvedProductionLoopRoutesComplete() const = "
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tbool\n\tAreResolvedProductionLoopRoutesComplete() const;\n",
            "\tbool   AreResolvedProductionLoopRoutesComplete() const;\n",
            "\tbool\tAreResolvedProductionLoopRoutesComplete() const;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UFUNCTION_PURE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_get_resolved_production_loop_route_count_or_neighbors(
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_GET_RESOLVED_COUNT))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "AreResolvedProductionLoopRoutesComplete")
        )
        self.assertTrue(
            has_identifier(section, "AreResolvedProductionLoopRoutesComplete")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio director are resolved production loop routes "
                "complete decl contract contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_director_are_resolved_production_loop_routes_complete"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
