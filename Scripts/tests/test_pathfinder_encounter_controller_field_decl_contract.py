...
# INDEX_NONE or lock EncounterController construction
# in the .cpp. This is a FIELD contract on
# ASkyguardPathfinderBoss, not leftover
# Pathfinder encounter-controller methods
# (already drafted) and not leftover RadarNode.
...
# EncounterController;`); accept that form and other
# one-line / split-line wraps. The element
# type appears on origin/main and must be
# part of the locked field declaration.
# Nearby origin/main
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly,
# Category="Skyguard|Boss|Encounter") is required as
# present. Accept one-line and split-line
# UPROPERTY wraps. Parse the public class
# section of ASkyguardPathfinderBoss only.
# Do not lock leftover sibling Pathfinder
# fields CommandAntenna, Engine,
# ControlLinkage, NoseCamera, DebrisCenter,
# DebrisTail, DebrisNose,
# DebrisSpine as the primary lock.
# Stay off leftover Pathfinder
# encounter-controller methods
# AdvanceEncounter / ResetEncounterState /
# IsRouteStateSafe / GetRouteProgress /
# GetEffectiveSpeedMultiplier /
# IsAttackTelegraphActive /
# GetTelegraphsTriggered. Stay off leftover
# Pathfinder MinHeightFromOriginCm (wrong
# header, not Harbor 40/80). Stay off leftover
# RadarNode. Stay off leftover
# USkyguardBossWeakPointComponent member
...
# leftover LastFlight PortGuidanceArray #481
# / DebrisArmorPort #491. Stay off leftover
# Pathfinder NoseCamera / Engine / CommandAntenna /
# DebrisNose / DebrisSpine sibling field contracts. Stay off leftover
...
# isolated-test drafts #107–#508 including
...
# leftover #107–#508, plus
# SkyguardPathfinderBoss production files.
# This lane only adds an isolated Python
# EncounterController field declaration contract on
# ASkyguardPathfinderBoss. Stay off leftover
# sibling Pathfinder fields, leftover
# Pathfinder encounter-controller methods,
...
# leftover LastFlight siblings including
# leftover PortGuidanceArray / DebrisArmorPort,
# leftover patrol-ship empty fail-closed
# #5382, leftover ApacheSystem enum values,
...
# isolated Pathfinder boss field contracts stay
# unlocked for THIS test's primary lock. Leftover
# Pathfinder encounter-controller methods, leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardPathfinderBoss::EncounterController",
    "SkyguardPathfinderBoss.cpp",
    "SetExposed",
...
class PathfinderEncounterControllerFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ENCOUNTER_CONTROLLER_FIELD),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            "class SKYGUARD52_API AOtherPathfinderBoss "
            ": public AActor\n"
            "{\n"
            "public:\n"
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn("EncounterController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_missing_encounter_controller_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardPathfinderBoss();\n"
            f"\t{COMMAND_ANTENNA_FIELD}\n"
            f"\t{ENGINE_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn("EncounterController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_ENCOUNTER}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn("EncounterController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_ENCOUNTER, section)
        self.assertIn("VisibleAnywhere", section)
        self.assertIn("BlueprintReadOnly", section)
        self.assertIn('Category="Skyguard|Boss|Encounter"', section)
        self.assertTrue(
            has_declaration(section, ENCOUNTER_CONTROLLER_FIELD),
            section,
        )
        self.assertNotIn("UPROPERTY", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("VisibleAnywhere", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("BlueprintReadOnly", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("Category", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_ENCOUNTER)
        self.assertNotIn("BlueprintCallable", UPROPERTY_ENCOUNTER)
        self.assertIn("Skyguard|Boss|Encounter", UPROPERTY_ENCOUNTER)
        self.assertIn("Encounter", UPROPERTY_ENCOUNTER)
        self.assertNotIn("Destruction", UPROPERTY_ENCOUNTER)
        self.assertNotIn("Mission10", UPROPERTY_ENCOUNTER)
        self.assertNotIn("Safety", UPROPERTY_ENCOUNTER)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_ENCOUNTER)
            self.assertNotIn(invented, ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn("EncounterController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUSkyguardPathfinderEncounterController* EncounterController;\n"
        )
        as_array = (
            "\tTArray<TObjectPtr<USkyguardPathfinderEncounterController>> "
            "EncounterController;\n"
        )
        scene = (
            "\tTObjectPtr<USceneComponent> EncounterController;\n"
        )
        mesh = (
            "\tTObjectPtr<USkyguardBossWeakPointComponent> EncounterController;\n"
        )
        weak = (
            "\tTWeakObjectPtr<USkyguardPathfinderEncounterController> "
            "EncounterController;\n"
        )
        assigned = (
            "\tTObjectPtr<USkyguardPathfinderEncounterController> "
            "EncounterController = nullptr;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ENCOUNTER_CONTROLLER_FIELD)
            self.assertIn("EncounterController", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_encounter_controller_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ENCOUNTER_CONTROLLER_FIELD),
            ENCOUNTER_CONTROLLER_FIELD,
        )
        self.assertTrue(has_declaration(section, ENCOUNTER_CONTROLLER_FIELD))
        self.assertEqual(declaration_count(section, ENCOUNTER_CONTROLLER_FIELD), 1)
        self.assertTrue(
            ENCOUNTER_CONTROLLER_FIELD.startswith(
                "TObjectPtr<USkyguardPathfinderEncounterController> "
            ),
            ENCOUNTER_CONTROLLER_FIELD,
        )
        self.assertTrue(ENCOUNTER_CONTROLLER_FIELD.endswith(";"), ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn("EncounterController", ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn(
            "USkyguardPathfinderEncounterController",
            ENCOUNTER_CONTROLLER_FIELD,
        )
        self.assertIn("TObjectPtr<", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("TArray<", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("=", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("INDEX_NONE", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("UFUNCTION", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("{", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("}", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("return ", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("CommandAntenna", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ControlLinkage", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("NoseCamera", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("DebrisCenter", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("DebrisTail", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("DebrisNose", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("DebrisSpine", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("AdvanceEncounter", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ResetEncounterState", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("IsRouteStateSafe", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetRouteProgress", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetEffectiveSpeedMultiplier", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("IsAttackTelegraphActive", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetTelegraphsTriggered", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("RadarNode", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("OpenGuidanceArrayExposure", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("IssueClimbCommand", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetCivilianSeparationMeters", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetFinaleStage", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("IsClimbCommandIssued", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("IsWreckDiverted", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetObjectiveMilestonesReached", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ESkyguardLastFlightStage", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetGunnerMount", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("FaceWorldLocation", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("PortGuidanceArray", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("DebrisArmorPort", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetSearchlightTracked", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetContactIdentified", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetLightningExposed", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("HullCollider", ENCOUNTER_CONTROLLER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_ENCOUNTER}\n"
            f"\t{ENCOUNTER_CONTROLLER_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_ENCOUNTER} {ENCOUNTER_CONTROLLER_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(
                declaration_count(section, ENCOUNTER_CONTROLLER_FIELD),
                1,
            )
        one_line = f"{{\npublic:\n\t{ENCOUNTER_CONTROLLER_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ENCOUNTER_CONTROLLER_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ENCOUNTER_CONTROLLER_FIELD),
            section,
    ...
    def test_assigned_encounter_controller_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardPathfinderEncounterController> "
            "EncounterController = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn("EncounterController", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ENCOUNTER_CONTROLLER_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("EncounterController", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ENCOUNTER_CONTROLLER_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ENCOUNTER_CONTROLLER_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ENCOUNTER_CONTROLLER_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_ENCOUNTER, section)
        self.assertTrue(has_declaration(section, ENCOUNTER_CONTROLLER_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ENCOUNTER_CONTROLLER_FIELD.startswith("UFUNCTION"),
            ENCOUNTER_CONTROLLER_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, ENCOUNTER_CONTROLLER_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ENCOUNTER_CONTROLLER_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_encounter_controller_cpp_body(self) -> None:
        locked_only = f"{ENCOUNTER_CONTROLLER_FIELD}\n"
        self.assertNotIn("{", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("}", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("return ", ENCOUNTER_CONTROLLER_FIELD)
        ...
        self.assertNotIn("SkyguardPathfinderBoss.cpp", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SkyguardPathfinderBoss.cpp", locked_only)
        self.assertNotIn("return false", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("return true", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetExposed", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            ENCOUNTER_CONTROLLER_FIELD,
        )
        self.assertNotIn("SetBossPhase", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("HandleWeakPointDestroyed", ENCOUNTER_CONTROLLER_FIELD)

    def test_contract_does_not_relock_sibling_pathfinder_fields(self) -> None:
        locked_only = f"{ENCOUNTER_CONTROLLER_FIELD}\n"
        for token in SIBLING_PATHFINDER_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("CommandAntenna", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ControlLinkage", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("NoseCamera", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("DebrisCenter", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_open_first_window(), ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn(leftover_open_final_window(), locked_only)
        self.assertNotIn(leftover_open_final_window(), ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn(leftover_arm_command_core_path(), locked_only)
        self.assertNotIn(leftover_arm_command_core_path(), ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn(leftover_apply_strike(), locked_only)
        self.assertNotIn(leftover_apply_strike(), ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), ENCOUNTER_CONTROLLER_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, ENCOUNTER_CONTROLLER_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetSearchlightTracked", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetContactIdentified", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("OpenOrbitExposure", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        for token in LEFTOVER_LIFELINE_HUNTER_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetFriendlySeparationMeters", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardLifelineHunterBoss", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SetLightningExposed", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        for token in LEFTOVER_IRON_RAIN_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("OpenUpperEngineExposure", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardGunner", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SkyguardRadarNode", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            ...
            self.assertNotIn(token, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("MaxIntegrity", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("CurrentIntegrity", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn(
            "SkyguardPathfinderEncounterController.h",
            ENCOUNTER_CONTROLLER_FIELD,
        )
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ENCOUNTER_CONTROLLER_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ENCOUNTER_CONTROLLER_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ENCOUNTER_CONTROLLER_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ENCOUNTER_CONTROLLER_FIELD),
            ENCOUNTER_CONTROLLER_FIELD,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardPathfinderBoss::EncounterController", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "pathfinder EncounterController field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{ENCOUNTER_CONTROLLER_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ENCOUNTER_CONTROLLER_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ENCOUNTER_CONTROLLER_FIELD.lower(),
                "pathfinder EncounterController contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ENCOUNTER_CONTROLLER_FIELD)

    def test_contract_is_encounter_controller_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(ENCOUNTER_CONTROLLER_FIELD.endswith(";"))
        self.assertNotIn("=", ENCOUNTER_CONTROLLER_FIELD)
        self.assertIn(UPROPERTY_ENCOUNTER, section)
