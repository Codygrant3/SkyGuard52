...
# or lock the GetRotorRPM body.
# origin/main is inline
# (`float GetRotorRPM() const`
# with a CurrentRotorRPM return possibly on the
...
# locking the body. The CurrentRotorRPM return is not a
# required token of this declaration lock. Nearby
# origin/main
# UFUNCTION(BlueprintPure, Category="Skyguard|Apache")
# is accepted as present. This Category is
# Skyguard|Apache, not Damage. Parse the public class
# section of ASkyguardApacheAircraft only. Do not lock
# ESkyguardPilotCommand enum values. Do not lock
# ESkyguardApacheSystem enum values. This is not leftover
# SetRotorPower sibling. This is not leftover
# GetRotorPowerScale sibling (this wave). This is not
# leftover apache-own-ship-systems #96c5 (that leftover
# locks ESkyguardApacheSystem enumerators, not this
# method; stay off ApplySystemHit / ApplyHit /
# IsSensorLive / IsThermalAvailable / IsSensorViewActive /
# IsSystemDown / FindNearestLiveSystem). This is not
...
# (#421), GetChinFireScale (#422), GetEnginePowerScale
# (#423), GetRotorPowerScale (sibling this wave),
# HullCollider (sibling this wave), and remaining unused
# neighbors (IsSystemDown, FindNearestLiveSystem).
# Leftover briefing / debrief widget isolated contracts,
...
# #107–#423, and leftover SortiePresentationWidgets
# stay sibling-only.
GET_ROTOR_RPM = (
    "float GetRotorRPM() const;"
)
...
# GetRotorRPM declaration contract on
...
# sibling this wave, leftover GetChinFireScale sibling
# this wave, leftover GetEnginePowerScale sibling
# this wave, leftover GetRotorPowerScale sibling this
# wave, leftover HullCollider sibling this wave, and
...
# HullCollider sibling in this wave, leftover
...
# GetRotorRPM return type is not a leftover
# roster value lock.
LEFTOVER_NOT_LOCKED = (
    "ESkyguardApacheSystem",
...
# .cpp GetRotorRPM body / invented INDEX_NONE stay
# unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline return of the current
# rotor rpm is accepted as presence, not a locked
# implementation contract. Do not parse leftover HUD
# classes.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::GetRotorRPM",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheGetRotorRPMDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_ROTOR_RPM), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_ROTOR_RPM)
        self.assertIn("GetRotorRPM", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_ROTOR_RPM))

    def test_missing_get_rotor_rpm_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_ROTOR_RPM)
        self.assertIn("GetRotorRPM", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_ROTOR_RPM)
        self.assertIn("GetRotorRPM", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_ROTOR_RPM), section)
        self.assertNotIn("BlueprintPure", GET_ROTOR_RPM)
        self.assertNotIn("UFUNCTION", GET_ROTOR_RPM)
        self.assertNotIn("Category", GET_ROTOR_RPM)
        self.assertNotIn("BlueprintCallable", GET_ROTOR_RPM)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_ROTOR_RPM)
        self.assertIn("GetRotorRPM", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tfloat GetRotorRPM const;\n"
        wrong_return_void = "\tvoid GetRotorRPM() const;\n"
        wrong_return_bool = "\tbool GetRotorRPM() const;\n"
        wrong_return_int = "\tint32 GetRotorRPM() const;\n"
        missing_const = "\tfloat GetRotorRPM();\n"
        added_arg = "\tfloat GetRotorRPM(float Extra) const;\n"
        leftover_canopy = (
            "\tbool IsCanopyGlassCracked() const "
            "{ return bCanopyGlassCracked; }\n"
        )
        ...
        leftover_sensor_quality = "\tfloat GetSensorQuality() const;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_ROTOR_RPM)
            self.assertIn("GetRotorRPM", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_rotor_rpm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_ROTOR_RPM),
            GET_ROTOR_RPM,
        )
        self.assertTrue(has_declaration(section, GET_ROTOR_RPM))
        self.assertEqual(declaration_count(section, GET_ROTOR_RPM), 1)
        self.assertTrue(
            GET_ROTOR_RPM.startswith("float "),
            GET_ROTOR_RPM,
        )
        self.assertTrue(GET_ROTOR_RPM.endswith(";"), GET_ROTOR_RPM)
        self.assertIn("GetRotorRPM()", GET_ROTOR_RPM)
        self.assertIn(" const", GET_ROTOR_RPM)
        self.assertNotIn("INDEX_NONE", GET_ROTOR_RPM)
        self.assertNotIn("{", GET_ROTOR_RPM)
        self.assertNotIn("}", GET_ROTOR_RPM)
        self.assertNotIn("return ", GET_ROTOR_RPM)
        self.assertNotIn("IsCanopyGlassCracked", GET_ROTOR_RPM)
        self.assertNotIn("AreEnginesDown", GET_ROTOR_RPM)
        self.assertNotIn("IsChinTurretDown", GET_ROTOR_RPM)
        self.assertNotIn("IsRotorDown", GET_ROTOR_RPM)
        self.assertNotIn("IsSystemDown", GET_ROTOR_RPM)
        self.assertNotIn("GetSensorQuality", GET_ROTOR_RPM)
        self.assertNotIn("GetDamageFraction", GET_ROTOR_RPM)
        self.assertNotIn("ApplyDamage", GET_ROTOR_RPM)
        self.assertNotIn("GetForwardSpeed", GET_ROTOR_RPM)
        self.assertNotIn("FaceWorldLocation", GET_ROTOR_RPM)
        self.assertNotIn("ApplySystemHit", GET_ROTOR_RPM)
        self.assertNotIn("ApplyHit", GET_ROTOR_RPM)
        self.assertNotIn("IsSensorLive", GET_ROTOR_RPM)
        self.assertNotIn("IsThermalAvailable", GET_ROTOR_RPM)
        self.assertNotIn("IsSensorViewActive", GET_ROTOR_RPM)
        self.assertNotIn("GetChinMuzzleLocation", GET_ROTOR_RPM)
        self.assertNotIn("GetChinTurret", GET_ROTOR_RPM)
        self.assertNotIn("GetRotorPowerScale", GET_ROTOR_RPM)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, GET_ROTOR_RPM),
                section,
            )
            self.assertEqual(
                require_declaration(section, GET_ROTOR_RPM),
            ...
            self.assertEqual(
                declaration_count(section, GET_ROTOR_RPM),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_ROTOR_RPM}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_ROTOR_RPM))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_ROTOR_RPM), section)
        self.assertEqual(
            require_declaration(section, GET_ROTOR_RPM),
    ...
    def test_declaration_accepts_origin_main_inline_and_prototype(
        ...
        self.assertTrue(has_declaration(one_line_prototype, GET_ROTOR_RPM))
        self.assertEqual(
            require_declaration(one_line_prototype, GET_ROTOR_RPM),
            GET_ROTOR_RPM,
        )
        exact_inline = (
            "float GetRotorRPM() "
        ...
        self.assertTrue(has_declaration(exact_inline, GET_ROTOR_RPM))
        self.assertEqual(
            require_declaration(exact_inline, GET_ROTOR_RPM),
            GET_ROTOR_RPM,
        )
        self.assertEqual(
            declaration_count(exact_inline, GET_ROTOR_RPM),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
        ...
        self.assertNotIn("{", GET_ROTOR_RPM)
        self.assertNotIn("return CurrentRotorRPM", GET_ROTOR_RPM)
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", GET_ROTOR_RPM)
        self.assertNotIn("return CurrentRotorRPM", GET_ROTOR_RPM)
        self.assertNotIn("return Other", GET_ROTOR_RPM)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_ROTOR_RPM}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_ROTOR_RPM)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_ROTOR_RPM)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_rotor_rpm_cpp_body(self) -> None:
        locked_only = f"{GET_ROTOR_RPM}\n"
        self.assertNotIn("{", GET_ROTOR_RPM)
        self.assertNotIn("}", GET_ROTOR_RPM)
        self.assertNotIn("return ", GET_ROTOR_RPM)
        self.assertNotIn("return CurrentRotorRPM", GET_ROTOR_RPM)
        self.assertNotIn("return CurrentRotorRPM", locked_only)
        self.assertNotIn(
            "ASkyguardApacheAircraft::GetRotorRPM",
            GET_ROTOR_RPM,
        )
        self.assertNotIn("SkyguardApacheAircraft.cpp", GET_ROTOR_RPM)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", GET_ROTOR_RPM)
        self.assertNotIn("return true", GET_ROTOR_RPM)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{GET_ROTOR_RPM}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ROTOR_RPM)
        self.assertNotIn("GetGunnerMount", GET_ROTOR_RPM)
        self.assertNotIn("GetEyeMount", GET_ROTOR_RPM)
        self.assertNotIn("GetWeaponMount", GET_ROTOR_RPM)
        self.assertNotIn("GetChinTurret", GET_ROTOR_RPM)
        self.assertNotIn("GetPilotMount", GET_ROTOR_RPM)
        self.assertNotIn("GetSensorTurret", GET_ROTOR_RPM)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_ROTOR_RPM}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROTOR_RPM)
        self.assertNotIn("GetChinMuzzleLocation", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_aim_chin_turret(self) -> None:
        ...
        for neighbor in AIM_CHIN_TURRET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROTOR_RPM)
        self.assertNotIn("AimChinTurret", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_set_rotor_power(self) -> None:
        ...
        for neighbor in SET_ROTOR_POWER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROTOR_RPM)
        self.assertNotIn("SetRotorPower", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_issue_pilot_command(self) -> None:
        ...
        for neighbor in ISSUE_PILOT_COMMAND_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROTOR_RPM)
        self.assertNotIn("IssuePilotCommand", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_get_pilot_helpers(self) -> None:
        ...
        for neighbor in GET_PILOT_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROTOR_RPM)
        self.assertNotIn("GetPilotCommand", GET_ROTOR_RPM)
        self.assertNotIn("GetPilotConfirmationsIssued", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROTOR_RPM)
        self.assertNotIn("SetOrbitFocus", GET_ROTOR_RPM)
        self.assertNotIn("FaceWorldLocation", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROTOR_RPM)
        self.assertNotIn("SetSensorView", GET_ROTOR_RPM)
        self.assertNotIn("SetFirstPersonInterior", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ROTOR_RPM)
        self.assertNotIn("ESkyguardApacheSystem", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_leftover_direct_flight_input_sibling(
        ...
        self.assertNotIn("SetDirectFlightInput", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_leftover_get_chin_slew_scale_sibling(
        ...
        self.assertNotIn("GetChinSlewScale", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_leftover_get_chin_fire_scale_sibling(
        ...
        self.assertNotIn("GetChinFireScale", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_leftover_get_engine_power_scale_sibling(
        ...
        self.assertNotIn("GetEnginePowerScale", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_leftover_hull_collider_sibling(
        self,
    ) -> None:
        locked_only = f"{GET_ROTOR_RPM}\n"
        ...
        self.assertNotIn("HullCollider", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_lock_pilot_command_enum_values(self) -> None:
        ...
        for token in leftover_short_roster_values():
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_ROTOR_RPM)
        self.assertNotIn("Pursuit", GET_ROTOR_RPM)
        self.assertNotIn("OrbitLeft", GET_ROTOR_RPM)
        self.assertNotIn("AttackRun", GET_ROTOR_RPM)
        self.assertNotIn("FaceTarget", GET_ROTOR_RPM)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_ROTOR_RPM)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", GET_ROTOR_RPM)
        self.assertNotIn("USkyguardBriefingWidget", GET_ROTOR_RPM)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            GET_ROTOR_RPM,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_ROTOR_RPM)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_ROTOR_RPM)
        self.assertNotIn("ESkyguardMissionSkylineStyle", GET_ROTOR_RPM)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_ROTOR_RPM}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_ROTOR_RPM),
            GET_ROTOR_RPM,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GetRotorRPM contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, GET_ROTOR_RPM.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache GetRotorRPM contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_ROTOR_RPM)

    def test_contract_is_get_rotor_rpm_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return CurrentRotorRPM", GET_ROTOR_RPM)
        self.assertNotIn("{", GET_ROTOR_RPM)
        self.assertTrue(GET_ROTOR_RPM.startswith("float "))
        self.assertTrue(GET_ROTOR_RPM.endswith(";"))
        self.assertIn(" const", GET_ROTOR_RPM)
        self.assertIn(UFUNCTION_APACHE, section)
