# THIS IS leftover-safe USkyguardAudioDirectorComponent EngineIdleLoop.
...
# then BARE `TSoftObjectPtr<USoundBase> EngineIdleLoop;`
# (no initializer).
...
# Stop BEFORE sibling EngineCruiseLoop as a claimed slot.
# Do NOT claim leftover sibling EngineCruiseLoop / EnginePowerLoop /
# PropellerLoop / OpenCockpitWindLoop / GlobalVoiceLimit /
...
# (NO spaces around =).
# LOCKED_DECL is `TSoftObjectPtr<USoundBase> EngineIdleLoop;`
...
TARGET = "TSoftObjectPtr<USoundBase> EngineIdleLoop;"
TARGET_WRONG_BARE = "TSoftObjectPtr<USoundBase> EngineIdleLoop = nullptr;"
TARGET_WRONG_INIT = "TSoftObjectPtr<USoundBase> EngineIdleLoop = false;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundBase> EngineIdleLoop = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundBase> EngineIdleLoop = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> EngineIdleLoop;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> EngineIdleLoop;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> EngineIdleLoop;"
...
STOP_BEFORE_SKYLINE_STYLE = "EngineCruiseLoop"
...
ENGINE_IDLE_LOOP_DECL_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EngineIdleLoop\s*;"
)
ENGINE_IDLE_LOOP_INIT_RE = re.compile(
    r"TSoftObjectPtr<USoundBase>\s+EngineIdleLoop\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # on EngineIdleLoop (`= nullptr` / `= false` / `= true`).
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ENGINE_IDLE_LOOP_DECL_RE.search(compact) is None:
        return False
    if ENGINE_IDLE_LOOP_INIT_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr<USoundBase>\s+EngineIdleLoop\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundBase> EngineIdleLoop "
...
class AudioDirectorEngineIdleLoopFieldDeclContractTests(unittest.TestCase):
    def test_audio_director_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EngineIdleLoop"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EngineIdleLoop"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("EngineIdleLoop", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_engine_idle_loop_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "TSoftObjectPtr<USoundBase> EngineIdleLoop = nullptr;",
            compact_origin,
        )
    ...
    def test_engine_idle_loop_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSoftObjectPtr<USoundBase> EngineIdleLoop"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTSoftObjectPtr<USoundBase> EngineIdleLoops;\n",
            "\tint32 EngineIdleLoop = 1;\n",
            "\tFName EngineIdleLoop = NAME_None;\n",
            "\tTSoftObjectPtr<USoundBase> EngineIdleLoop = " + forty + ";\n",
            "\tTSoftObjectPtr<USoundBase> EngineIdleLoop = " + eighty + ";\n",
            f"\t{TARGET_WRONG_BOOM}\n",
    ...
    def test_does_not_claim_sibling_engine_cruise_loop_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EngineIdleLoop"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "EngineIdleLoop"))
        self.assertTrue(has_identifier(section, "EngineIdleLoop"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_ENGINE_POWER_LOOP))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "audio director engine idle loop field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "audio_director_engine_idle_loop_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
