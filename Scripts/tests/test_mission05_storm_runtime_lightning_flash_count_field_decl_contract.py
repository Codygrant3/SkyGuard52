...
# LightningFlashCount FIELD (declaration-only;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission05 StormRuntime
# LightningFlashCount. THIS IS NOT leftover storm-runtime-
# defaults #89c9. THIS IS NOT leftover GetStormRuntime
# / AdvanceTurbulence / TriggerLightningWindow /
# AdvanceLightningWindow. THIS IS NOT leftover StormRuntime
# Turbulence (opening draft) / in-flight bLightningActive /
# in-flight LightningRemainingSeconds. LOCK those sibling
# scripts. THIS IS NOT leftover Mission05 PTR Target #1147 /
# Integrity #1148 / bDestroyed #1149. THIS IS NOT leftover
...
# Mission05 PTR Target (draft) / in-flight Integrity /
# in-flight bDestroyed. THIS IS NOT leftover
...
# #1139/#1141/#1142 and PayloadWindowRuntime #1143/#1144/#1145/#1146.
...
# Mission05 ProtectedTargetRuntime Target / Integrity /
# bDestroyed, not leftover
...
LIGHTNING_FLASH_COUNT_ALT = (
    "int32 LightningFlashCount = 1;"
)
LOCKED_DECL = LIGHTNING_FLASH_COUNT
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission05_storm_runtime_lightning_remaining_seconds_field_decl_contract.py",
...
LEFTOVER_STORM_RUNTIME_SIBLING_FIELDS = (
    "Turbulence",
    "bLightningActive",
    "LightningRemainingSeconds",
    "bMaintainingAim",
)
LEFTOVER_STORM_RUNTIME_METHODS_NOT_LOCKED = (
    "GetStormRuntime",
    "AdvanceTurbulence",
...
def reject_parser_landings(section: str) -> str:
    for forbidden in FORBIDDEN_PARSER_LANDINGS:
        if forbidden in section:
            raise AssertionError(
                f"parser landed on {forbidden} instead of {CLASS_NAME} "
                f"public section from origin/main:{HEADER_PATH}"
            )
    return section
...
def public_section(header: str) -> str:
    ...
    if public is None:
        ...
        return reject_parser_landings(body[1:close])
    start = public.end()
    rest = body[start:]
    next_access = ACCESS_RE.search(rest)
    if next_access is not None:
        return reject_parser_landings(rest[: next_access.start()])
    close = rest.rfind("}")
    if close == -1:
        raise AssertionError(
            f"{CLASS_NAME} public section is missing from "
    ...
    return reject_parser_landings(rest[:close])
...
class Mission05StormRuntimeLightningFlashCountFieldDeclContractTests(unittest.TestCase):
    def test_mission05_storm_runtime_struct_exists(self) -> None:
        ...
        self.assertIn("Turbulence", section)
        self.assertIn("bLightningActive", section)
        self.assertIn("LightningRemainingSeconds", section)
        self.assertIn("bMaintainingAim", section)
        self.assertNotIn("FSkyguardMission05ProtectedTargetRuntime", section)
        ...
        self.assertNotIn("GetStormRuntime", section)
        self.assertNotIn("AdvanceTurbulence", section)
        self.assertNotIn("TriggerLightningWindow", section)
        self.assertNotIn("AdvanceLightningWindow", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("LightningFlashCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_lightning_flash_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_lightning_flash_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 LightningFlashCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("LightningFlashCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("LightningFlashCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 LightningFlashCount"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("LightningFlashCount", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 StormRuntime LightningFlashCount field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission05 StormRuntime LightningFlashCount contains "
    ...
    def test_contract_is_lightning_flash_count_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_lightning_flash_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            ...
            "_lightning_flash_count_field_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission05_storm_runtime"
                "_lightning_flash_count_field_decl_contract.py"
            )
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission05_storm_runtime"
            "_lightning_remaining_seconds_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission04_mission_definition"
    ...
    def test_contract_does_not_parse_director_class_ptr_or_readiness(
        self,
    ) -> None:
        header = origin_main_header()
        match = CLASS_RE.search(header)
        ...
        self.assertNotIn("FSkyguardMission05ProtectedTargetRuntime", section)
        for token in LEFTOVER_STORM_RUNTIME_SIBLING_FIELDS:
            self.assertIn(token, section)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_STORM_RUNTIME_METHODS_NOT_LOCKED:
        ...
        for token in LEFTOVER_STORM_RUNTIME_NOT_LOCKED:
            if token.startswith("test_"):
                continue
            if token in LEFTOVER_STORM_RUNTIME_SIBLING_FIELDS:
                continue
            self.assertNotIn(token, section)
            self.assertNotIn(token, LOCKED_DECL)
