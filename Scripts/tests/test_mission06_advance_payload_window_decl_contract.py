...
# INDEX_NONE or lock AdvancePayloadWindow
# construction in the .cpp. This is leftover-safe Mission06
# Payload AdvancePayloadWindow on
# ASkyguardMission06IntegrationDirector. It is
# NOT leftover payload-window-runtime-defaults
# #f114 STRUCT, NOT leftover StartPayloadWindow
# (sibling method, listed unlocked), NOT leftover
...
# leftover airfield-target enum #14d2, NOT leftover
...
# Waves / Targets / Objectives methods
...
# leftover Mission06 StartNextWave, leftover
# Mission03 StartNextWave, leftover
...
# #666, not leftover Mission05 Waves #668, not
# leftover Mission06 Waves StartNextWave, not leftover
...
# AdvancePayloadWindow method declaration
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission06IntegrationDirector::AdvancePayloadWindow",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission06AdvancePayloadWindowDeclContractTests(unittest.TestCase):
    def test_mission06_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ADVANCE_PAYLOAD_WINDOW),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ADVANCE_PAYLOAD_WINDOW)
        self.assertIn("AdvancePayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_missing_advance_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission06IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ADVANCE_PAYLOAD_WINDOW)
        self.assertIn("AdvancePayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ADVANCE_PAYLOAD_WINDOW)
        self.assertIn("AdvancePayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("BlueprintCallable", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("Category", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("BlueprintPure", UFUNCTION_PAYLOAD)
        ...
        self.assertIn("BlueprintCallable", UFUNCTION_PAYLOAD)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ADVANCE_PAYLOAD_WINDOW)
        self.assertIn("AdvancePayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid AdvancePayloadWindow(float DeltaSeconds);\n"
        as_const = "\tbool AdvancePayloadWindow(float DeltaSeconds) const;\n"
        with_arg = "\tbool AdvancePayloadWindow(bool bForce);\n"
        renamed = "\tbool AdvancePayload();\n"
        short_name = "\tbool PayloadWindow();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ADVANCE_PAYLOAD_WINDOW)
            self.assertIn("AdvancePayloadWindow", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_advance_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ADVANCE_PAYLOAD_WINDOW),
            ADVANCE_PAYLOAD_WINDOW,
        )
        self.assertTrue(has_declaration(section, ADVANCE_PAYLOAD_WINDOW))
        self.assertEqual(
            declaration_count(section, ADVANCE_PAYLOAD_WINDOW),
            1,
        )
        self.assertTrue(
            ADVANCE_PAYLOAD_WINDOW.startswith("bool "),
            ADVANCE_PAYLOAD_WINDOW,
        )
        self.assertTrue(
            ADVANCE_PAYLOAD_WINDOW.endswith(";"),
            ADVANCE_PAYLOAD_WINDOW,
        )
        self.assertIn("AdvancePayloadWindow", ADVANCE_PAYLOAD_WINDOW)
        self.assertIn("float DeltaSeconds", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("TArray<", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("TObjectPtr", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("TSoftObjectPtr", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("INDEX_NONE", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("UFUNCTION", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("{", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("}", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("return ", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("const;", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("Root", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("Briefing", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("AudioDirector", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("RadioChatter", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("SortiePresentation", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("CampaignDefinition", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("MissionDefinition", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("Readiness", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("bAutoInitialize", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("bAllowBoundedActorSpawning", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("bAutoLaunchAfterBriefing", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("PathfinderSpawnLocation", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetAircraft", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("BindRuntimeActors", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("HandleDroneCityImpact", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("InitializePlayableMission", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyThreatDestroyed", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceConvoyByDistance", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyConvoyDamage", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetWaveState", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetConvoyRouteState", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatKit", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetNightBeatKit", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("StartSearchlightWindow", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceSearchlightTrack", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("NotifySubstationDamage", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetRemainingThreatsInWave", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetSearchlightRuntime", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetSubstationIntegrity", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("StartPayloadWindow", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("StartNextWave", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("TryJamActivePayload", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyAirfieldTargetDamage", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetPayloadWindow", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetTargetRuntime", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetSurvivingTargetCount", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatKind", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetDayBeatIndex", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("TickDayBeatKit", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("HandleBossPhaseChanged", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardMission06WaveState", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardAirfieldTarget", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ConvoyRuntimeAnchor", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("RoadHunterSpawnLocation", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ConfigureMissionDefinition", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyObjectiveProgress", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("NotifyProtectedAssetFailed", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("SynchronizeRuntimeState", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("IsCorePlayableReady", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetReadiness", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetObjectiveRuntime", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetGunner", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetPathfinder", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetMissionId", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ValidateMissionContract", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ConfigureFromMission", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("AdvanceBriefing", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("SetAssetsReady", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("AcknowledgeAndLaunch", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("CanLaunch", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetElapsedSeconds", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetBriefingState", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetMinimumWarmupSeconds", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetBriefingText", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetRadioChatter", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetPresentation", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("GetMissionTitle", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("AcknowledgeBriefing", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("LaunchSortie", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("HullCollider", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("OpticalTracker", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("WeaponServo", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("CountermeasurePod", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("MinHeightFromOriginCm", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("RadarNode", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ESkyguardMission02WaveState", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("HarborIndustrial", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("MaxIntegrity", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("CurrentIntegrity", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("FillAndFinalize", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("FillAndFail", ADVANCE_PAYLOAD_WINDOW)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tAdvancePayloadWindow(float DeltaSeconds);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_PAYLOAD} {ADVANCE_PAYLOAD_WINDOW}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ADVANCE_PAYLOAD_WINDOW}\n}}\n"
        self.assertTrue(has_declaration(one_line, ADVANCE_PAYLOAD_WINDOW))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADVANCE_PAYLOAD_WINDOW),
            section,
    ...
    def test_environment_category_does_not_satisfy_payload(self) -> None:
        self.assertNotIn("Environment", UFUNCTION_PAYLOAD)
        self.assertNotIn("Briefing", UFUNCTION_PAYLOAD)
        environment = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(leftover_m03_waves, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_m04_waves, UFUNCTION_PAYLOAD)
        self.assertNotEqual(leftover_m05_waves, UFUNCTION_PAYLOAD)
        ...
        self.assertNotIn(leftover_m03_waves, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_m04_waves, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_m05_waves, UFUNCTION_PAYLOAD)
        self.assertNotIn(leftover_integration, UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission01", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission03", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission04", UFUNCTION_PAYLOAD)
        self.assertNotIn("Mission05", UFUNCTION_PAYLOAD)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{ADVANCE_PAYLOAD_WINDOW}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ADVANCE_PAYLOAD_WINDOW)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_advance_cpp_body(self) -> None:
        locked_only = f"{ADVANCE_PAYLOAD_WINDOW}\n"
        self.assertNotIn("{", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("}", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("return ", ADVANCE_PAYLOAD_WINDOW)
        ...
        self.assertNotIn("return false", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("return true", ADVANCE_PAYLOAD_WINDOW)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{ADVANCE_PAYLOAD_WINDOW}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ADVANCE_PAYLOAD_WINDOW)
        self.assertIn("AdvancePayloadWindow", ADVANCE_PAYLOAD_WINDOW)
        self.assertTrue(ADVANCE_PAYLOAD_WINDOW.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, ADVANCE_PAYLOAD_WINDOW)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("CurrentIntegrity", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("SkyguardApacheAircraft.h", ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ADVANCE_PAYLOAD_WINDOW}\n"
        self.assertEqual(
            require_declaration(locked_only, ADVANCE_PAYLOAD_WINDOW),
            ADVANCE_PAYLOAD_WINDOW,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", ADVANCE_PAYLOAD_WINDOW)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", ADVANCE_PAYLOAD_WINDOW)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 AdvancePayloadWindow method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ADVANCE_PAYLOAD_WINDOW}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ADVANCE_PAYLOAD_WINDOW.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ADVANCE_PAYLOAD_WINDOW.lower(),
                "mission06 AdvancePayloadWindow contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ADVANCE_PAYLOAD_WINDOW)

    def test_contract_is_advance_payload_window_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(ADVANCE_PAYLOAD_WINDOW.startswith("bool "))
        self.assertIn("AdvancePayloadWindow", ADVANCE_PAYLOAD_WINDOW)
        self.assertTrue(ADVANCE_PAYLOAD_WINDOW.endswith(";"))
        self.assertIn(UFUNCTION_PAYLOAD, section)

    def test_sibling_director_fields_do_not_satisfy_advance(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, ADVANCE_PAYLOAD_WINDOW)
            )
            self.assertNotEqual(ADVANCE_PAYLOAD_WINDOW, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("AdvancePayloadWindow", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, ADVANCE_PAYLOAD_WINDOW))
        locked_only = f"{ADVANCE_PAYLOAD_WINDOW}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool AdvancePayloadWindow(float DeltaSeconds) { return false; }\n"
            "};\n"
        )
    ...
    def test_contract_does_not_relock_leftover_payload_window(self) -> None:
        ...
        self.assertNotIn("FSkyguardPayloadWindowRuntime", locked_only)
        self.assertNotIn("StartPayloadWindow", ADVANCE_PAYLOAD_WINDOW)
        self.assertIn("Payload", UFUNCTION_PAYLOAD)
        self.assertNotIn("Waves", UFUNCTION_PAYLOAD)
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_advance_payload_window"
                "_decl_contract.py"
            )
        )
        self.assertIn(
