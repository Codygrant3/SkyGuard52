...
# INDEX_NONE or lock bEnableCoastalHazeTransition
# construction in the .cpp. This is a FIELD contract
# on ASkyguardMission01EnvironmentDirector. Keep
# bEnableCoastalHazeTransition distinct from leftover
# sibling CoastalHazeDelaySeconds /
# CoastalHazeFadeSeconds / CoastalHazeHoldSeconds /
# CoastalHazeDensityIncrease, leftover OceanMaterial /
# BeachMaterial / LandMaterial / LandTiles, leftover Mission01
# Integration director fields Briefing
...
# bAutoLaunchAfterBriefing. Keep bEnableCoastalHazeTransition
...
# origin/main in-header initializer `= true`.
# Nearby origin/main
...
# leftover theater-kit #59, leftover #107–#597, plus
# SkyguardMission01EnvironmentDirector production
# files. This lane only adds an isolated Python
# bEnableCoastalHazeTransition field declaration contract on
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::bEnableCoastalHazeTransition",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01EnableCoastalHazeTransitionFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))

    def test_missing_enable_coastal_haze_transition_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("EditAnywhere", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("EditInstanceOnly", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("BlueprintReadWrite", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("VisibleAnywhere", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("BlueprintReadOnly", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("Category", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("ClampMin", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        ...
        self.assertNotIn("ClampMin", UPROPERTY_MISSION01_ENVIRONMENT_VISIBILITY)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tbool bEnableCoastalHazeTransition;\n"
        )
        as_array = (
            "\tTArray<bool> bEnableCoastalHazeTransition;\n"
        )
        assigned = (
            "\tbool bEnableCoastalHazeTransition = false;\n"
        )
        soft = (
            "\tuint8 bEnableCoastalHazeTransition = true;\n"
        )
        wrong_type = "\tint32 bEnableCoastalHazeTransition = 1;\n"
        renamed = "\tbool EnableCoastalHazeTransition = true;\n"
        ...
        scene = "\tTObjectPtr<USceneComponent> bEnableCoastalHazeTransition;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> bEnableCoastalHazeTransition;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> bEnableCoastalHazeTransition;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
            self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_enable_coastal_haze_transition_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD),
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD,
        )
        self.assertTrue(has_declaration(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))
        self.assertEqual(
            declaration_count(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD),
            1,
        )
        self.assertTrue(
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD.startswith("bool "),
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD,
        )
        self.assertTrue(ENABLE_COASTAL_HAZE_TRANSITION_FIELD.endswith(";"), ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bool", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("TObjectPtr<", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("= true", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("OceanMaterial", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("BeachMaterial", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)

        self.assertNotIn("LandMaterial", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CoastalHazeDelaySeconds", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CoastalHazeFadeSeconds", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CoastalHazeHoldSeconds", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CoastalHazeDensityIncrease", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("TArray<", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("TSoftObjectPtr", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("nullptr", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("INDEX_NONE", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("UFUNCTION", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("{", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("}", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("return ", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("OceanTiles", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("LandTiles", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("RouteExclusion", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("bRouteExclusionValid", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("BeachTiles", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("InlandVegetationPCG", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("LandScatterBounds", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("ProductionLandscape", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("AuthoredPCGGraph", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("DistrictLengthCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("BeachWidthCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("InlandExtentCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("SeawardExtentCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD,
        )
        ...
        self.assertNotIn("Root", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetReadiness", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("Briefing", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("AudioDirector", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("RadioChatter", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("SortiePresentation", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CampaignDefinition", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("MissionDefinition", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("bAutoInitialize", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetAircraft", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("ConfigureFromMission", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("AdvanceBriefing", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("SetAssetsReady", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CanLaunch", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetElapsedSeconds", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetBriefingState", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetBriefingText", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetRadioChatter", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetPresentation", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("GetMissionTitle", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("AcknowledgeBriefing", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("LaunchSortie", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("HullCollider", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("OpticalTracker", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("WeaponServo", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CountermeasurePod", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("RadarNode", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("HarborIndustrial", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("MaxIntegrity", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CurrentIntegrity", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tbEnableCoastalHazeTransition = true;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_VISIBILITY} {ENABLE_COASTAL_HAZE_TRANSITION_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{ENABLE_COASTAL_HAZE_TRANSITION_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD),
            section,
    ...
    def test_assigned_enable_coastal_haze_transition_does_not_satisfy(self) -> None:
        assigned = (
            "\tbool bEnableCoastalHazeTransition = false;\n"
        )
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))
        with self.assertRaises(AssertionError) as raised_ocean:
            require_declaration(leftover_ocean, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised_ocean.exception))
        self.assertFalse(has_declaration(leftover_ocean, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))
        with self.assertRaises(AssertionError) as raised_beach:
            require_declaration(leftover_beach, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised_beach.exception))
        self.assertFalse(has_declaration(leftover_beach, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{ENABLE_COASTAL_HAZE_TRANSITION_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD.startswith("UFUNCTION"),
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_enable_coastal_haze_transition_cpp_body(self) -> None:
        locked_only = f"{ENABLE_COASTAL_HAZE_TRANSITION_FIELD}\n"
        self.assertNotIn("{", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("}", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("return ", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        ...
        self.assertNotIn("return false", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("return true", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CreateDefaultSubobject", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertTrue(
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD.startswith("bool ")
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("CurrentIntegrity", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{ENABLE_COASTAL_HAZE_TRANSITION_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, ENABLE_COASTAL_HAZE_TRANSITION_FIELD),
            ENABLE_COASTAL_HAZE_TRANSITION_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, ENABLE_COASTAL_HAZE_TRANSITION_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 bEnableCoastalHazeTransition field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{ENABLE_COASTAL_HAZE_TRANSITION_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, ENABLE_COASTAL_HAZE_TRANSITION_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                ENABLE_COASTAL_HAZE_TRANSITION_FIELD.lower(),
                "mission01 bEnableCoastalHazeTransition contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)

    def test_contract_is_enable_coastal_haze_transition_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("bool", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertTrue(ENABLE_COASTAL_HAZE_TRANSITION_FIELD.endswith(";"))
        self.assertIn("= true", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("TObjectPtr<", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("OceanMaterial", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
    ...
    def test_sibling_environment_fields_do_not_satisfy_enable_coastal_haze_transition(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, ENABLE_COASTAL_HAZE_TRANSITION_FIELD))
            self.assertNotEqual(ENABLE_COASTAL_HAZE_TRANSITION_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_enable_coastal_haze_transition(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            "\t/**\n"
            "\t * Mission 1 coastal haze transition switch.\n"
            "\t * This comment is not the bEnableCoastalHazeTransition declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertIn("bEnableCoastalHazeTransition", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        )
        self.assertNotIn(
            "bool bEnableCoastalHazeTransition = true;",
            comment_only,
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
            self.assertNotEqual(token, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("RouteLengthCm", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn(LEFTOVER_ROUTE_LENGTH_VALUE, ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("7500.f", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("2800.f", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("5200.f", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("1800.f", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("18000.f", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
        self.assertNotIn("20000.f", ENABLE_COASTAL_HAZE_TRANSITION_FIELD)
