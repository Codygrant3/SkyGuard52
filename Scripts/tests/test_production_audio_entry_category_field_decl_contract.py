# THIS IS leftover-safe FSkyguardProductionAudioEntry Category.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then REQUIRED initializer
...
# Lock BlueprintReadWrite. Required EngineIdle initializer.
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# ESkyguardProductionAudioCategory, initializer dropped or
# changed from EngineIdle, or UPROPERTY Category= added.
...
# Category="Skyguard|Theater", retarget: type is
# ESkyguardProductionAudioCategory, REQUIRED initializer
# `= ESkyguardProductionAudioCategory::EngineIdle`,
# BlueprintReadWrite, no UPROPERTY Category=.
...
TARGET_WRONG_BARE = "ESkyguardProductionAudioCategory Category;"
TARGET_WRONG_EQ_NONE = (
    "ESkyguardProductionAudioCategory Category = nullptr;"
)
TARGET_WRONG_FALSE = "ESkyguardProductionAudioCategory Category = false;"
TARGET_WRONG_TRUE = "ESkyguardProductionAudioCategory Category = true;"
TARGET_WRONG_ZERO = "ESkyguardProductionAudioCategory Category = 0.f;"
TARGET_WRONG_FLOAT = "float Category;"
TARGET_WRONG_FNAME = "FName Category;"
TARGET_WRONG_FSTRING = "FString Category;"
TARGET_WRONG_BOOL = "bool Category;"
TARGET_WRONG_INT = "int32 Category;"
TARGET_WRONG_FTEXT = "FText Category;"
TARGET_WRONG_STATUS = (
    "ESkyguardAudioSourceStatus Category = "
    "ESkyguardAudioSourceStatus::MISSING_SOURCE;"
)
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> Category;"
TARGET_WRONG_CRUISE = (
    "ESkyguardProductionAudioCategory Category = "
    "ESkyguardProductionAudioCategory::EngineCruise;"
)
TARGET_WRONG_RETIRED_ENUM = (
    "ESkyguardProductionAudioCategory Category = "
    "ESkyguardProductionAudioCategory::Ri" + "fleMuzzle;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardProductionAudioCategory Category =
    # ESkyguardProductionAudioCategory::EngineIdle`.
    # Do not accept a bare `Category;`, a dropped or
    # changed initializer, `= false` / `= true` /
    # `= 0.f` / leftover float Health, or sibling
    # DisplayName / SourceStatus / Sound fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"ESkyguardProductionAudioCategory\s+Category\s*;",
        compact,
    ):
        return False
    if re.search(
        r"ESkyguardProductionAudioCategory\s+Category\s*=\s*"
        r"ESkyguardProductionAudioCategory\s*::\s*EngineIdle\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"ESkyguardProductionAudioCategory\s+Category\s*=\s*"
        r"ESkyguardProductionAudioCategory\s*::\s*(?!EngineIdle\b)",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Category\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+Category\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+Category\b",
        compact,
    ):
        return False
    if re.search(
        ...
        return False
    if re.search(
        r"int32\s+Category\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+Category\b",
        compact,
    ):
        return False
    if re.search(
        r"ESkyguardAudioSourceStatus\s+Category\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]+>\s+Category\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*ESkyguardProductionAudioCategory\s+Category\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for ESkyguardProductionAudioCategory Category is missing "
...
class ProductionAudioEntryCategoryFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_category_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_dropped_or_changed_initializer_fails_closed(self) -> None:
        ...
        retired = (
            ...
            "{\n"
            f"\t{UPROPERTY_EDIT_READWRITE}\n"
            f"\t{TARGET_WRONG_RETIRED_ENUM}\n"
            "};\n"
        )
        retired_section = spec_section(retired)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(retired_section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertNotIn("Category = nullptr", compact_origin)
        self.assertNotIn("Category = false", compact_origin)
        self.assertNotIn("Category = true", compact_origin)
        self.assertNotIn("Category = 0.f", compact_origin)
        self.assertNotIn("Category = 160.f", compact_origin)
        self.assertNotIn(
            "ESkyguardProductionAudioCategory Category;",
    ...
    def test_category_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("ESkyguardProductionAudioCategory Category"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("ESkyguardProductionAudioCategory", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_STATUS}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SOFT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_CRUISE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RETIRED_ENUM}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tESkyguardProductionAudioCategory Categories = "
            "ESkyguardProductionAudioCategory::EngineIdle;\n",
            "\tint32 Category = 1;\n",
            "\tbool Category;\n",
            "\tfloat Category = " + forty + ";\n",
            "\tfloat Category = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry Category field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "category_field_decl_contract.py"
        ))
