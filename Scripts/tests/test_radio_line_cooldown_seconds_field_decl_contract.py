# THIS IS leftover-safe FSkyguardRadioLine CooldownSeconds.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite, meta=(ClampMin="0.0"))
# then `float CooldownSeconds = 0.f;` (initializer IS `0.f`,
# not `0.0f` / `0.0` / `0` / Harbor 40/80 / `2.f`).
# THIS IS leftover-safe isolated UPROPERTY without Category.
# EditAnywhere IS present. BlueprintReadWrite IS present.
# ClampMin="0.0" IS present. Initializer IS `= 0.f`.
...
# the type is not float, if meta=(ClampMin="0.0") is
# missing or changed, if Category= is added, or if the
# initializer is missing or not `= 0.f`.
...
# Do NOT parse leftover radio-line Speaker #1333.
# Do NOT parse leftover radio-line LineId #1334.
# Do NOT contract sibling fields LineId / Speaker / Subtitle /
# Sound / Priority / EstimatedDurationSeconds.
...
# this isolated CooldownSeconds decl.
# If a clone asserts FText / FName / no `=` /
# Category="Skyguard|Theater" / VisibleAnywhere /
# BlueprintReadOnly / ClampMin missing / initializer `2.f`,
# retarget: type is float, initializer is `= 0.f`,
# EditAnywhere + BlueprintReadWrite + meta=(ClampMin="0.0"),
# no Category=.
...
TARGET = "float CooldownSeconds = 0.f;"
TARGET_WRONG_EQ_NONE = "float CooldownSeconds = 0.0f;"
TARGET_WRONG_FALSE = "float CooldownSeconds = false;"
TARGET_WRONG_TRUE = "float CooldownSeconds = true;"
TARGET_WRONG_ZERO = "float CooldownSeconds = 2.f;"
TARGET_WRONG_FLOAT = "int32 CooldownSeconds = 0;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_EMPTY = "float CooldownSeconds;"
TARGET_WRONG_TWO = "float CooldownSeconds = 0;"
TARGET_WRONG_TWO_POINT_ZERO = (
    "float CooldownSeconds = 0.0;"
)
...
SIBLING_COOLDOWN = SIBLING_ESTIMATED_DURATION
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float CooldownSeconds = 0.f`.
    # Reject missing initializer, `0.0f` / `0.0` / `0` /
    # Harbor 40/80 / `2.f` / leftover Health. Do not accept
    # sibling LineId / Speaker / Subtitle / Sound / Priority /
    # EstimatedDurationSeconds fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+CooldownSeconds\s*=\s*0\.f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"float\s+CooldownSeconds\s*=\s*0\.0f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CooldownSeconds\s*=\s*0\.0\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CooldownSeconds\s*=\s*0\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CooldownSeconds\s*=\s*2\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CooldownSeconds\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+CooldownSeconds\b",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CooldownSeconds\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CooldownSeconds is missing from "
...
class RadioLineCooldownSecondsFieldDeclContractTests(unittest.TestCase):
    def test_radio_line_struct_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "CooldownSeconds"),
            section,
        )
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("CooldownSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cooldown_seconds_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_cooldown_seconds_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CooldownSeconds"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tbool CooldownSeconds;\n",
            "\tFText CooldownSeconds;\n",
            f"\t{TARGET_WRONG_TWO}\n",
            f"\t{TARGET_WRONG_TWO_POINT_ZERO}\n",
            f"\t{TARGET_WRONG_EMPTY}\n",
            "\tfloat CooldownSeconds = " + forty + ";\n",
            "\tfloat CooldownSeconds = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_only_fails_closed(self) -> None:
        ...
        duration_specs = attached_uproperty_specifiers(
            spec_section(duration_clamp)
        )
        self.assertIn('ClampMin="0.1"', duration_specs)
        self.assertNotIn('ClampMin="0.0"', duration_specs)
        self.assertIn('ClampMin="0.0"', origin)
        priority_clamp = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio-line CooldownSeconds field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cooldown_seconds_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
