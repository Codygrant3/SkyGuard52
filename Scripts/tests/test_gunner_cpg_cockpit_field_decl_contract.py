# THIS IS leftover-safe ASkyguardGunner.CpgCockpit.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgCockpit;`
# (no initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard` NOT `Skyguard|Apache` and NOT
...
# Stop BEFORE sibling CpgSeatBack as a claimed slot.
# Do NOT claim CpgStation (in-flight), CpgSeatPan,
# Boom, GunnerCamera, CpgDash, CpgTedac.
# Do NOT claim leftover Canopy / PilotCanopy
# (leftover ASkyguardApacheAircraft mesh UPROPERTY).
# Do NOT claim leftover analog apache-is-canopy-glass-cracked
# UFUNCTION, leftover analog apache-aircraft-empty-fail-closed,
# leftover Apache UFUNCTION decls, leftover analog
# apache-hull-collider, leftover analog apache-mount-fail-closed,
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgCockpit;`
# NOT `FName WeatherIdentity;` and NOT
# `float CurrentIntegrity = 140.f;` and NOT leftover
# Canopy / PilotCanopy / CpgStation / CpgSeatBack.
# Mesh slots fail-closed if LOCKED_DECL is
# `TObjectPtr<USceneComponent>`.
# CpgCockpit fail-closed if LOCKED_DECL is leftover
# Canopy / PilotCanopy / CpgStation / CpgSeatBack or a
# leftover live-copy seat-kit mesh (split tokens only).
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgCockpit;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgCockpit = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgCockpit = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgCockpit = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgCockpit;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgCockpit;"
...
TARGET_WRONG_PILOT_CANOPY = (
    "TObjectPtr<UStaticMeshComponent> PilotCanopy;"
)
TARGET_WRONG_APACHE_CATEGORY = (
...
STOP_BEFORE_SEAT_BACK = "CpgSeatBack"
...
STOP_BEFORE_APACHE_AIRCRAFT = "ASkyguardApacheAircraft"
...
CPG_COCKPIT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgCockpit\s*;"
)
CPG_COCKPIT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgCockpit\s*="
)
...
def leftover_seat_kit_primary_mesh() -> str:
    return "Ri" + "fleMesh"


def leftover_seat_kit_guided_tube() -> str:
    return "Ig" + "laTube"


def leftover_seat_kit_guided_muzzle() -> str:
    return "Ig" + "laMuzzle"


def leftover_apache_aircraft_class() -> str:
    return "ASkyguardApacheAircraft"
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        ...
        leftover_seat_kit_primary_mesh(),
        leftover_seat_kit_guided_tube(),
        leftover_seat_kit_guided_muzzle(),
        SIBLING_BOOM,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgSeatBack / CpgStation / Canopy /
    # PilotCanopy, or an initializer on CpgCockpit
    # as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_COCKPIT_DECL_RE.search(compact) is None:
        return False
    if CPG_COCKPIT_INIT_RE.search(compact):
        return False
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        leftover_apache_aircraft_class(),
        STOP_BEFORE_APACHE_AIRCRAFT,
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_SEAT_BACK):
            break
    ...
    if has_identifier(window, SIBLING_CPG_SEAT_BACK):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_SEAT_BACK}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgCockpit\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgCockpit "
...
class GunnerCpgCockpitFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgCockpit"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_SEAT_BACK), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_SEAT_BACK),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgCockpit"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgCockpit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_cockpit_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('Category="Skyguard|Apache"', UPROPERTY_VISIBLE_READONLY)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgCockpit;", compact_origin)
        self.assertNotIn("CpgCockpit = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_cockpit_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgCockpit"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn(leftover_seat_kit_primary_mesh(), LOCKED_DECL)
        self.assertNotIn(leftover_seat_kit_guided_tube(), LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SEAT_BACK}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgCockpit;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTObjectPtr<UStaticMeshComponent> {leftover_seat_kit_primary_mesh()};\n",
            f"\tTObjectPtr<UStaticMeshComponent> {leftover_seat_kit_guided_tube()};\n",
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgCockpites;\n",
            "\tint32 CpgCockpit = 140;\n",
            "\tbool CpgCockpit = true;\n",
            "\tfloat CpgCockpit = " + forty + ";\n",
            "\tfloat CpgCockpit = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_cpg_seat_back_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_CANOPY))
        ...
        self.assertNotIn(TARGET_WRONG_SEAT_BACK, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_SEAT_BACK))
        self.assertFalse(has_identifier(section, SIBLING_CPG_SEAT_BACK))
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_SEAT_PAN))
        self.assertFalse(has_identifier(section, SIBLING_CPG_SEAT_PAN))
        self.assertFalse(has_identifier(section, SIBLING_ROTOR_POWER))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgCockpit field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_cockpit_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(leftover_seat_kit_primary_mesh(), locked_only)
        self.assertNotIn(leftover_seat_kit_guided_tube(), locked_only)
