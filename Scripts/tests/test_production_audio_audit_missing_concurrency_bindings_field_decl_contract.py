# THIS IS leftover-safe FSkyguardProductionAudioAudit
# MissingConcurrencyBindings.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(BlueprintReadOnly)
# then BARE `TArray<FName> MissingConcurrencyBindings;`.
# THIS STRUCT IS BlueprintReadOnly. There is NO EditAnywhere.
# There is NO Category=. There is NO initializer (bare TArray).
# Fail-closed if missing/renamed field, EditAnywhere added,
# BlueprintReadWrite replacing BlueprintReadOnly, type not
# TArray<FName>, an initializer is added, or Category= added.
# Accept one-line and split-line UPROPERTY wraps.
# Parse STRUCT `FSkyguardProductionAudioAudit` body ONLY after
# `struct FSkyguardProductionAudioAudit`. Stop before
# `class SKYGUARD52_API USkyguardAudioProductionBank`
# (the SKYGUARD52_API macro is required; a naive
# `class USkyguardAudioProductionBank` find returns -1 and
# leaks leftover class fields).
# Do NOT parse leftover `USkyguardAudioProductionBank`.
# Do NOT parse leftover `FSkyguardProductionAudioRouting`.
# Do NOT parse leftover `FSkyguardProductionAudioEntry`.
# Do NOT parse `enum class ESkyguardProductionAudioCategory`
# (keep that enum OUT of the parse window by starting at
# the claimed struct — leftover live-copy tokens live there).
# Do NOT parse `enum class ESkyguardAudioSourceStatus`
# (leftover #241 / #65ba).
# Do NOT fail the whole header for leftover live-copy tokens
# (those live in the preceding category enum).
# Fail-closed only if live leftover tokens / Harbor 40/80
# appear IN THE PARSE WINDOW or IN THE TEST FILE.
# Analog leftover production-audio-audit-defaults #196 /
# #5e51 then isolated field decl.
# Sibling decls stay unclaimed: RequiredCategoryCount /
# BoundProductionSourceCount / ExplicitMissingSourceCount /
# QATestOnlyCount / InvalidSourceEntries /
# MissingSoundBindings / MissingAttenuationBindings /
# MissingCategoryEntries / MissingOutputSubmixBindings /
# MissingRoutingAssets / bCategoryContractComplete /
# bProductionReady.
# Match the EXACT identifier MissingConcurrencyBindings as a
# whole field name after TArray<FName>. Do NOT accept a
# substring match on Missing or Bindings. Do NOT claim sibling
# Missing* / *Bindings arrays.
# Do NOT parse leftover production-audio-routing isolated
# decls or leftover production-audio-entry isolated decls.
# Do NOT parse leftover FSkyguardAudioEventDefinition /
# ESkyguardAudioEvent.
# Do NOT parse leftover TheaterKit.
# Do NOT parse leftover mission-definition.
...
# If a clone asserts int32 / required `= 0`, retarget:
# parse FSkyguardProductionAudioAudit only, UPROPERTY is
# BlueprintReadOnly with NO EditAnywhere, type is
# TArray<FName>, BARE decl with NO initializer, no Category=.
...
HEADER_PATH = "Source/Skyguard52/SkyguardAudioProductionBank.h"
STRUCT_NAME = "FSkyguardProductionAudioAudit"
ACTOR_NAME = "ASkyguardCampaignTheaterKit"
NAMESPACE_NAME = "SkyguardCampaignTheaterKit"
TARGET = "TArray<FName> MissingConcurrencyBindings;"
TARGET_WRONG_EQ_EMPTY = "TArray<FName> MissingConcurrencyBindings = {};"
TARGET_WRONG_EQ_ZERO = "TArray<FName> MissingConcurrencyBindings = 0;"
TARGET_WRONG_FALSE = "TArray<FName> MissingConcurrencyBindings = false;"
TARGET_WRONG_TRUE = "TArray<FName> MissingConcurrencyBindings = true;"
TARGET_WRONG_ZERO = "TArray<FName> MissingConcurrencyBindings = 0.f;"
TARGET_WRONG_FORTY = "TArray<FName> MissingConcurrencyBindings = 40;"
TARGET_WRONG_EIGHTY = "TArray<FName> MissingConcurrencyBindings = 80;"
TARGET_WRONG_FLOAT = "float MissingConcurrencyBindings = 0;"
TARGET_WRONG_INT32 = "int32 MissingConcurrencyBindings = 0;"
TARGET_WRONG_FNAME = "FName MissingConcurrencyBindings;"
TARGET_WRONG_BOOL = "bool MissingConcurrencyBindings = false;"
TARGET_WRONG_FSTRING = "FString MissingConcurrencyBindings;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> MissingConcurrencyBindings;"
TARGET_WRONG_SUBSTRING_MISSING = "TArray<FName> Missing;"
TARGET_WRONG_SUBSTRING_BINDINGS = "TArray<FName> Bindings;"
TARGET_WRONG_SIBLING_SOUND = "TArray<FName> MissingSoundBindings;"
TARGET_WRONG_SIBLING_ATTEN = "TArray<FName> MissingAttenuationBindings;"
TARGET_WRONG_SIBLING_CATEGORY = "TArray<FName> MissingCategoryEntries;"
TARGET_WRONG_SIBLING_SUBMIX = "TArray<FName> MissingOutputSubmixBindings;"
TARGET_WRONG_SIBLING_ROUTING = "TArray<FName> MissingRoutingAssets;"
TARGET_WRONG_SIBLING_INVALID = "TArray<FName> InvalidSourceEntries;"
...
STOP_BEFORE_UCLASS = "UCLASS(BlueprintType)"
...
SIBLING_REQUIRED_CATEGORY_COUNT = "RequiredCategoryCount"
SIBLING_BOUND_PRODUCTION_SOURCE_COUNT = "BoundProductionSourceCount"
SIBLING_EXPLICIT_MISSING_SOURCE_COUNT = "ExplicitMissingSourceCount"
SIBLING_QA_TEST_ONLY_COUNT = "QATestOnlyCount"
SIBLING_INVALID_SOURCE_ENTRIES = "InvalidSourceEntries"
SIBLING_MISSING_SOUND_BINDINGS = "MissingSoundBindings"
SIBLING_MISSING_ATTENUATION_BINDINGS = "MissingAttenuationBindings"
SIBLING_MISSING_CATEGORY_ENTRIES = "MissingCategoryEntries"
SIBLING_MISSING_OUTPUT_SUBMIX_BINDINGS = "MissingOutputSubmixBindings"
SIBLING_MISSING_ROUTING_ASSETS = "MissingRoutingAssets"
SIBLING_CATEGORY_CONTRACT_COMPLETE = "bCategoryContractComplete"
SIBLING_PRODUCTION_READY = "bProductionReady"
...
FIELD_NAME = "MissingConcurrencyBindings"
...
THIS_SCRIPT = (
    "Scripts/tests/test_production_audio_audit_missing"
    "_concurrency_bindings_field_decl_contract.py"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_REQUIRED_CATEGORY_COUNT,
        SIBLING_BOUND_PRODUCTION_SOURCE_COUNT,
        SIBLING_EXPLICIT_MISSING_SOURCE_COUNT,
        SIBLING_QA_TEST_ONLY_COUNT,
        SIBLING_INVALID_SOURCE_ENTRIES,
        SIBLING_MISSING_SOUND_BINDINGS,
        SIBLING_MISSING_ATTENUATION_BINDINGS,
        SIBLING_MISSING_CATEGORY_ENTRIES,
        SIBLING_MISSING_OUTPUT_SUBMIX_BINDINGS,
        SIBLING_MISSING_ROUTING_ASSETS,
        SIBLING_CATEGORY_CONTRACT_COMPLETE,
        SIBLING_PRODUCTION_READY,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored bare `TArray<FName> MissingConcurrencyBindings;`.
    # Reject added initializer (`= {}`, `= 0`, Harbor 40/80),
    # int32, float, bool, FName, FString, TSoftObjectPtr.
    # Match the EXACT identifier MissingConcurrencyBindings after
    # TArray<FName>. Do not accept a substring match on Missing
    # or Bindings. Do not claim sibling Missing* / *Bindings arrays.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+MissingConcurrencyBindings\s*=",
        compact,
    ):
        return False
    if re.search(
        r"MissingConcurrencyBindings\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingConcurrencyBindings\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingConcurrencyBindings\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingConcurrencyBindings\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"MissingConcurrencyBindings\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MissingConcurrencyBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MissingConcurrencyBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MissingConcurrencyBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MissingConcurrencyBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MissingConcurrencyBindings\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+MissingConcurrencyBindings\b",
        compact,
    ):
        return False
    if re.search(
...
def parse_window(header: str) -> str:
    ...
    if header.find(STOP_BEFORE_BANK_NAIVE) >= 0:
        raise AssertionError(
            f"naive {STOP_BEFORE_BANK_NAIVE} is present; "
...
def struct_body(header: str) -> str:
    window = header
    if STRUCT_RE.search(header) is not None and STOP_BEFORE_BANK in header:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+MissingConcurrencyBindings\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> MissingConcurrencyBindings is missing from "
...
class ProductionAudioAuditMissingConcurrencyBindingsFieldDeclContractTests(
    unittest.TestCase
):
    def test_production_audio_audit_struct_exists(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_ENTRY, header)
        self.assertNotIn(STOP_BEFORE_ENTRY, body)
        self.assertIn(STOP_BEFORE_ROUTING, header)
        self.assertNotIn(STOP_BEFORE_ROUTING, body)
        self.assertIn(STOP_BEFORE_CATEGORY_ENUM, header)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, body)
        self.assertIn(STOP_BEFORE_SOURCE_STATUS_ENUM, header)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, body)
        self.assertIn(STOP_BEFORE_BANK, header)
        self.assertNotIn(STOP_BEFORE_BANK, body)
        self.assertEqual(header.find(STOP_BEFORE_BANK_NAIVE), -1)
        ...
        self.assertNotIn(STOP_BEFORE_ENTRY, section)
        self.assertNotIn(STOP_BEFORE_ROUTING, section)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, section)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, section)
        self.assertNotIn(STOP_BEFORE_BANK, section)
        self.assertNotIn(STOP_BEFORE_BANK_NAIVE, section)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, section)

    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedProductionAudioAudit\n{\n};\n"
            )
    ...
    def test_missing_concurrency_bindings_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tint32 {SIBLING_REQUIRED_CATEGORY_COUNT} = 0;\n"
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        for invented in INVENTED_UPROPERTY:
            if invented == "Category":
                self.assertNotIn("Category=", LOCKED_DECL)
                continue
            self.assertNotIn(invented, LOCKED_DECL)

    def test_initializer_added_fails_closed(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
        ...
        forty = (
            ...
            "{\n"
            f"\t{UPROPERTY_READONLY}\n"
            f"\t{TARGET_WRONG_FORTY}\n"
            "};\n"
        )
        forty_section = spec_section(forty)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(forty_section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertIn("TArray<FName> MissingConcurrencyBindings;", compact_origin)
        self.assertNotIn("MissingConcurrencyBindings = 40", compact_origin)
        self.assertNotIn("MissingConcurrencyBindings = 80", compact_origin)
        self.assertNotIn("MissingConcurrencyBindings = false", compact_origin)
        self.assertNotIn("MissingConcurrencyBindings = {}", compact_origin)
        self.assertNotIn("MissingConcurrencyBindings = 0", compact_origin)

    def test_missing_concurrency_bindings_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> MissingConcurrencyBindings"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("TArray<FName>", LOCKED_DECL)
        ...
        wrongs = (
            ...
            TARGET_WRONG_SOFT,
            TARGET_WRONG_HEALTH,
            TARGET_WRONG_SIBLING_SOUND,
            TARGET_WRONG_SIBLING_ATTEN,
            TARGET_WRONG_SIBLING_CATEGORY,
            TARGET_WRONG_SIBLING_SUBMIX,
            TARGET_WRONG_SIBLING_ROUTING,
            TARGET_WRONG_SIBLING_INVALID,
        )
        for wrong in wrongs:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tFText " + leftover_retired_primary_hits_field() + ";\n"
        )
        leftover_guided = (
            "\tFText " + leftover_retired_guided_hits_field() + ";\n"
        )
        forty = "40"
        eighty = "80"
        wrongs = (
            ...
            f"\t{TARGET_WRONG_SIBLING_SOUND}\n",
            f"\t{TARGET_WRONG_SIBLING_ATTEN}\n",
            f"\t{TARGET_WRONG_SIBLING_CATEGORY}\n",
            f"\t{TARGET_WRONG_SIBLING_SUBMIX}\n",
            f"\t{TARGET_WRONG_SIBLING_ROUTING}\n",
            f"\t{TARGET_WRONG_SIBLING_INVALID}\n",
            leftover_primary,
            leftover_guided,
            f"\tint32 {SIBLING_REQUIRED_CATEGORY_COUNT} = 0;\n",
            f"\tint32 {SIBLING_BOUND_PRODUCTION_SOURCE_COUNT} = 0;\n",
            f"\tint32 {SIBLING_QA_TEST_ONLY_COUNT} = 0;\n",
            f"\tbool {SIBLING_PRODUCTION_READY} = false;\n",
            "\tTArray<FName> MissingConcurrencyBindingss;\n",
            "\tTArray<FName> MissingConcurrencyBindings = 1;\n",
            "\tbool MissingConcurrencyBindings;\n",
            "\tfloat MissingConcurrencyBindings = " + forty + ";\n",
            "\tfloat MissingConcurrencyBindings = " + eighty + ";\n",
            "\tTArray<FName> Missing;\n",
            "\tTArray<FName> Bindings;\n",
            "\tTArray<FName> MissingBindings;\n",
            "\tTArray<FName> MissingConcurrencyBinding;\n",
            f"\t{TARGET_WRONG_SUBSTRING_MISSING}\n",
            f"\t{TARGET_WRONG_SUBSTRING_BINDINGS}\n",
            f"\t{TARGET_WRONG_SIBLING_CATEGORY}\n",
        )
    ...
    def test_does_not_contract_sibling_audit_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(
            has_identifier(section, SIBLING_REQUIRED_CATEGORY_COUNT), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_EXPLICIT_MISSING_SOURCE_COUNT),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_QA_TEST_ONLY_COUNT), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_BOUND_PRODUCTION_SOURCE_COUNT),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_INVALID_SOURCE_ENTRIES), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_SOUND_BINDINGS), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_ATTENUATION_BINDINGS),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_CATEGORY_ENTRIES),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_OUTPUT_SUBMIX_BINDINGS),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_MISSING_ROUTING_ASSETS), section
        )
        self.assertTrue(
            has_identifier(section, SIBLING_CATEGORY_CONTRACT_COMPLETE),
            section,
        )
        self.assertTrue(
            has_identifier(section, SIBLING_PRODUCTION_READY), section
        )
    ...
    def test_exact_identifier_rejects_substring_missing_siblings(self) -> None:
        exact = f"\t{LOCKED_DECL}\n"
        self.assertTrue(has_identifier(exact, FIELD_NAME), exact)
        self.assertFalse(has_identifier(exact, "Missing"))
        self.assertFalse(has_identifier(exact, "Bindings"))
        self.assertFalse(has_identifier(exact, "ConcurrencyBindings"))
        self.assertNotEqual(FIELD_NAME, "Missing")
        self.assertNotEqual(FIELD_NAME, "Bindings")
        self.assertTrue(FIELD_NAME.startswith("Missing"))
        self.assertTrue(FIELD_NAME.endswith("Bindings"))
        self.assertTrue(has_declaration(exact, LOCKED_DECL))
        siblings = (
            TARGET_WRONG_SIBLING_SOUND,
            TARGET_WRONG_SIBLING_ATTEN,
            TARGET_WRONG_SIBLING_CATEGORY,
            ...
            TARGET_WRONG_SIBLING_ROUTING,
            TARGET_WRONG_SIBLING_INVALID,
            f"TArray<FName> {SIBLING_MISSING_SOUND_BINDINGS};",
            f"TArray<FName> {SIBLING_MISSING_ATTENUATION_BINDINGS};",
            f"TArray<FName> {SIBLING_MISSING_CATEGORY_ENTRIES};",
            f"TArray<FName> {SIBLING_MISSING_OUTPUT_SUBMIX_BINDINGS};",
            f"TArray<FName> {SIBLING_MISSING_ROUTING_ASSETS};",
            "TArray<FName> Missing;",
            "TArray<FName> Bindings;",
            TARGET_WRONG_SUBSTRING_MISSING,
            TARGET_WRONG_SUBSTRING_BINDINGS,
        )
        for sibling in siblings:
            region = f"\t{sibling}\n"
            self.assertFalse(has_identifier(region, FIELD_NAME), region)
    ...
    def test_does_not_parse_neighbors_or_leftover_banks(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_ENTRY, header)
        self.assertNotIn(STOP_BEFORE_ENTRY, section)
        self.assertNotIn(STOP_BEFORE_ENTRY, leaked)
        self.assertNotIn(STOP_BEFORE_ENTRY, window)
        self.assertIn(STOP_BEFORE_ROUTING, header)
        self.assertNotIn(STOP_BEFORE_ROUTING, section)
        self.assertNotIn(STOP_BEFORE_ROUTING, leaked)
        self.assertNotIn(STOP_BEFORE_ROUTING, window)
        self.assertIn(STOP_BEFORE_CATEGORY_ENUM, header)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, section)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, leaked)
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, window)
        self.assertIn(STOP_BEFORE_SOURCE_STATUS_ENUM, header)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, section)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, leaked)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, window)
        self.assertIn(STOP_BEFORE_BANK, header)
        self.assertNotIn(STOP_BEFORE_BANK, section)
        self.assertNotIn(STOP_BEFORE_BANK, leaked)
        self.assertNotIn(STOP_BEFORE_BANK, window)
        self.assertEqual(header.find(STOP_BEFORE_BANK_NAIVE), -1)
        self.assertNotIn(STOP_BEFORE_BANK_NAIVE, section)
        self.assertNotIn(STOP_BEFORE_BANK_NAIVE, leaked)
        self.assertNotIn(STOP_BEFORE_BANK_NAIVE, window)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, section)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, leaked)
        ...
        self.assertNotIn(STOP_BEFORE_SAVE_GAME_API, section)
        self.assertNotIn(STOP_BEFORE_SAVE_GAME_API, leaked)
        ...
        self.assertNotIn(STOP_BEFORE_PROGRESS, section)
        self.assertNotIn(STOP_BEFORE_PROGRESS, leaked)
        self.assertNotIn(STOP_BEFORE_PROGRESS, window)
        ...
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, leaked)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, window)
    ...
    def test_naive_bank_class_stop_is_not_present(self) -> None:
        header = origin_main_header()
        self.assertEqual(header.find(STOP_BEFORE_BANK_NAIVE), -1)
        self.assertIn(STOP_BEFORE_BANK, header)
        self.assertTrue(STOP_BEFORE_BANK.startswith("class SKYGUARD52_API "))
        window = parse_window(header)
        self.assertNotIn(STOP_BEFORE_BANK, window)
        self.assertNotIn("EvaluateReadiness", window)
        self.assertNotIn("InitializeRequiredEntries", window)
        self.assertNotIn("EnsureDefaultEntries", window)
        self.assertNotIn("ConfigureRoutingTopology", window)
        self.assertNotIn("GetUnboundRequiredCategories", window)
        self.assertNotIn("GetRequiredCategories", window)
        self.assertNotIn("GetCategoryDisplayName", window)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit MissingConcurrencyBindings field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "missing_concurrency_bindings_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_CATEGORY_ENUM, locked_only)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS_ENUM, locked_only)
        self.assertNotIn(STOP_BEFORE_BANK, locked_only)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, locked_only)
