...
# INDEX_NONE or lock GetTargetRuntime
# construction in the .cpp. This is leftover-safe
# Mission06 Targets GetTargetRuntime on
...
# NOT leftover Mission05 GetProtectedTarget
# #678, NOT leftover payload-window-runtime-defaults
# #f114, NOT leftover Mission05 GetRemainingThreatsInWave
...
# `ESkyguardAirfieldTarget Target) const;`); accept
# that form, other one-line / split-line wraps,
...
# Category is Skyguard|Mission06|Targets, not
# Mission01 / Mission02 / Mission03 / Mission04 /
# Mission05 / Integration / Waves / Payload /
...
# leftover Mission01 Targets, not leftover
# Mission03 Targets, not leftover Mission04
# Targets, not leftover Mission05 Protection
# #678, not leftover Mission05 Waves #677, not
...
# Mission05 GetProtectedTarget #678.
...
# GetTargetRuntime method declaration
...
LEFTOVER_MISSION01_GET_TARGET_RUNTIME_NOT_LOCKED = (
    "ASkyguardMission01IntegrationDirector",
    "SkyguardMission01IntegrationDirector.h",
    "test_mission01_get_remaining_threats_in_wave_decl_contract.py",
    'Category="Skyguard|Mission01|Integration"',
)
LEFTOVER_MISSION03_GET_TARGET_RUNTIME_NOT_LOCKED = (
    "ASkyguardMission03IntegrationDirector",
    "SkyguardMission03IntegrationDirector.h",
    "test_mission03_get_remaining_threats_in_wave_decl_contract.py",
    'Category="Skyguard|Mission03|Integration"',
)
LEFTOVER_MISSION04_GET_TARGET_RUNTIME_NOT_LOCKED = (
    "ASkyguardMission04IntegrationDirector",
    "SkyguardMission04IntegrationDirector.h",
    "test_mission04_get_remaining_threats_in_wave_decl_contract.py",
    'Category="Skyguard|Mission04|Integration"',
)
LEFTOVER_MISSION05_GET_TARGET_RUNTIME_NOT_LOCKED = (
...
LEFTOVER_MISSION06_GET_REMAINING_THREATS_IN_WAVE_NOT_LOCKED = (
    "test_mission06_get_remaining_threats_in_wave_decl_contract.py",
    GET_REMAINING_THREATS_IN_WAVE,
    "GetRemainingThreatsInWave",
    'Category="Skyguard|Mission06|Waves"',
...
class Mission06GetTargetRuntimeDeclContractTests(unittest.TestCase):
    def test_mission06_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_TARGET_RUNTIME),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_TARGET_RUNTIME)
        self.assertIn("GetTargetRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_TARGET_RUNTIME)
    ...
    def test_missing_get_target_runtime_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission06IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_TARGET_RUNTIME)
        self.assertIn("GetTargetRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_TARGETS}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_TARGET_RUNTIME)
        self.assertIn("GetTargetRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_TARGETS, section)
        ...
        self.assertNotIn("UFUNCTION", GET_TARGET_RUNTIME)
        self.assertNotIn("BlueprintPure", GET_TARGET_RUNTIME)
        self.assertNotIn("Category", GET_TARGET_RUNTIME)
        self.assertNotIn("BlueprintCallable", UFUNCTION_TARGETS)
        self.assertIn("Skyguard|Mission06|Targets", UFUNCTION_TARGETS)
        self.assertNotIn("Mission01", UFUNCTION_TARGETS)
        self.assertNotIn("Mission03", UFUNCTION_TARGETS)
        self.assertNotIn("Mission04", UFUNCTION_TARGETS)
        self.assertNotIn("Mission05", UFUNCTION_TARGETS)
        self.assertNotIn("Waves", UFUNCTION_TARGETS)
        self.assertNotIn("Integration", UFUNCTION_TARGETS)
        self.assertNotIn("Payload", UFUNCTION_TARGETS)
        self.assertNotIn("Objectives", UFUNCTION_TARGETS)
        self.assertIn("BlueprintPure", UFUNCTION_TARGETS)
        self.assertNotIn("Environment", UFUNCTION_TARGETS)
        self.assertNotIn("Briefing", UFUNCTION_TARGETS)
        self.assertNotIn("Mission02", UFUNCTION_TARGETS)
        self.assertNotIn("Boss", UFUNCTION_TARGETS)
        self.assertNotIn("Destruction", UFUNCTION_TARGETS)
        self.assertNotIn("Apache", UFUNCTION_TARGETS)
        self.assertNotIn("Mission07", UFUNCTION_TARGETS)
        self.assertNotIn("Mission10", UFUNCTION_TARGETS)
        self.assertNotIn("Encounter", UFUNCTION_TARGETS)
        self.assertNotIn("Safety", UFUNCTION_TARGETS)
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, UFUNCTION_TARGETS)
            self.assertNotIn(invented, GET_TARGET_RUNTIME)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_TARGET_RUNTIME)
        self.assertIn("GetTargetRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetTargetRuntime(ESkyguardAirfieldTarget Target) const;\n"
        )
        as_const = (
            "\tbool GetTargetRuntime(ESkyguardAirfieldTarget Target);\n"
        )
        with_arg = (
            "\tint32 GetTargetRuntime() const;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_TARGET_RUNTIME)
            self.assertIn("GetTargetRuntime", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_target_runtime_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_TARGET_RUNTIME),
            GET_TARGET_RUNTIME,
        )
        self.assertTrue(has_declaration(section, GET_TARGET_RUNTIME))
        self.assertEqual(
            declaration_count(section, GET_TARGET_RUNTIME),
            1,
        )
        self.assertTrue(
            GET_TARGET_RUNTIME.startswith(
                "FSkyguardAirfieldTargetRuntime "
            ),
            GET_TARGET_RUNTIME,
        )
        self.assertTrue(
            GET_TARGET_RUNTIME.endswith(";"),
            GET_TARGET_RUNTIME,
        )
        self.assertIn("GetTargetRuntime", GET_TARGET_RUNTIME)
        self.assertIn("(ESkyguardAirfieldTarget Target)", GET_TARGET_RUNTIME)
        self.assertNotIn("int32 Damage", GET_TARGET_RUNTIME)
        self.assertNotIn("TArray<", GET_TARGET_RUNTIME)
        self.assertNotIn("TObjectPtr", GET_TARGET_RUNTIME)
        self.assertNotIn("TSoftObjectPtr", GET_TARGET_RUNTIME)
        self.assertNotIn("INDEX_NONE", GET_TARGET_RUNTIME)
        self.assertNotIn("UFUNCTION", GET_TARGET_RUNTIME)
        self.assertNotIn("{", GET_TARGET_RUNTIME)
        self.assertNotIn("}", GET_TARGET_RUNTIME)
        self.assertNotIn("return ", GET_TARGET_RUNTIME)
        self.assertIn(" const;", GET_TARGET_RUNTIME)
        self.assertNotIn("Root", GET_TARGET_RUNTIME)
        self.assertNotIn("Briefing", GET_TARGET_RUNTIME)
        self.assertNotIn("AudioDirector", GET_TARGET_RUNTIME)
        self.assertNotIn("RadioChatter", GET_TARGET_RUNTIME)
        self.assertNotIn("SortiePresentation", GET_TARGET_RUNTIME)
        self.assertNotIn("CampaignDefinition", GET_TARGET_RUNTIME)
        self.assertNotIn("MissionDefinition", GET_TARGET_RUNTIME)
        self.assertNotIn("Readiness", GET_TARGET_RUNTIME)
        self.assertNotIn("bAutoInitialize", GET_TARGET_RUNTIME)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_TARGET_RUNTIME)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_TARGET_RUNTIME)
        self.assertNotIn("PathfinderSpawnLocation", GET_TARGET_RUNTIME)
        self.assertNotIn("GetAircraft", GET_TARGET_RUNTIME)
        self.assertNotIn("BindRuntimeActors", GET_TARGET_RUNTIME)
        self.assertNotIn("HandleDroneCityImpact", GET_TARGET_RUNTIME)
        self.assertNotIn("InitializePlayableMission", GET_TARGET_RUNTIME)
        self.assertNotIn("NotifyThreatDestroyed", GET_TARGET_RUNTIME)
        self.assertNotIn("AdvanceConvoyByDistance", GET_TARGET_RUNTIME)
        self.assertNotIn("NotifyConvoyDamage", GET_TARGET_RUNTIME)
        self.assertNotIn("GetWaveState", GET_TARGET_RUNTIME)
        self.assertNotIn("GetConvoyRouteState", GET_TARGET_RUNTIME)
        self.assertNotIn("GetDayBeatKit", GET_TARGET_RUNTIME)
        self.assertNotIn("GetNightBeatKit", GET_TARGET_RUNTIME)
        self.assertNotIn("StartSearchlightWindow", GET_TARGET_RUNTIME)
        self.assertNotIn("AdvanceSearchlightTrack", GET_TARGET_RUNTIME)
        self.assertNotIn("NotifySubstationDamage", GET_TARGET_RUNTIME)
        self.assertNotIn("IsCorePlayableReady", GET_TARGET_RUNTIME)
        self.assertNotIn("GetSearchlightRuntime", GET_TARGET_RUNTIME)
        self.assertNotIn("GetSubstationIntegrity", GET_TARGET_RUNTIME)
        self.assertNotIn("StartNextWave", GET_TARGET_RUNTIME)
        self.assertNotIn("AdvancePayloadWindow", GET_TARGET_RUNTIME)
        self.assertNotIn("TryJamActivePayload", GET_TARGET_RUNTIME)
        self.assertNotIn("StartPayloadWindow", GET_TARGET_RUNTIME)
        self.assertNotIn("GetPayloadWindow", GET_TARGET_RUNTIME)
        self.assertNotIn("GetSurvivingTargetCount", GET_TARGET_RUNTIME)
        self.assertNotIn("GetDayBeatKind", GET_TARGET_RUNTIME)
        self.assertNotIn("GetDayBeatIndex", GET_TARGET_RUNTIME)
        self.assertNotIn("TickDayBeatKit", GET_TARGET_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", GET_TARGET_RUNTIME)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_TARGET_RUNTIME)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_TARGET_RUNTIME)
        self.assertNotIn("ESkyguardMission06WaveState", GET_TARGET_RUNTIME)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", GET_TARGET_RUNTIME)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_TARGET_RUNTIME)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_TARGET_RUNTIME)
        self.assertNotIn("RoadHunterSpawnLocation", GET_TARGET_RUNTIME)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_TARGET_RUNTIME)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_TARGET_RUNTIME)
        self.assertNotIn("ConfigureMissionDefinition", GET_TARGET_RUNTIME)
        self.assertNotIn("NotifyObjectiveProgress", GET_TARGET_RUNTIME)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_TARGET_RUNTIME)
        self.assertNotIn("SynchronizeRuntimeState", GET_TARGET_RUNTIME)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_TARGET_RUNTIME)
        self.assertNotIn("GetReadiness", GET_TARGET_RUNTIME)
        self.assertNotIn("GetObjectiveRuntime", GET_TARGET_RUNTIME)
        self.assertNotIn("GetGunner", GET_TARGET_RUNTIME)
        self.assertNotIn("GetPathfinder", GET_TARGET_RUNTIME)
        self.assertNotIn("GetMissionId", GET_TARGET_RUNTIME)
        self.assertNotIn("ValidateMissionContract", GET_TARGET_RUNTIME)
        self.assertNotIn("ConfigureFromMission", GET_TARGET_RUNTIME)
        self.assertNotIn("AdvanceBriefing", GET_TARGET_RUNTIME)
        self.assertNotIn("SetAssetsReady", GET_TARGET_RUNTIME)
        self.assertNotIn("AcknowledgeAndLaunch", GET_TARGET_RUNTIME)
        self.assertNotIn("CanLaunch", GET_TARGET_RUNTIME)
        self.assertNotIn("GetElapsedSeconds", GET_TARGET_RUNTIME)
        self.assertNotIn("GetBriefingState", GET_TARGET_RUNTIME)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_TARGET_RUNTIME)
        self.assertNotIn("GetBriefingText", GET_TARGET_RUNTIME)
        self.assertNotIn("GetRadioChatter", GET_TARGET_RUNTIME)
        self.assertNotIn("GetPresentation", GET_TARGET_RUNTIME)
        self.assertNotIn("GetMissionTitle", GET_TARGET_RUNTIME)
        self.assertNotIn("AcknowledgeBriefing", GET_TARGET_RUNTIME)
        self.assertNotIn("LaunchSortie", GET_TARGET_RUNTIME)
        self.assertNotIn("HullCollider", GET_TARGET_RUNTIME)
        self.assertNotIn("OpticalTracker", GET_TARGET_RUNTIME)
        self.assertNotIn("WeaponServo", GET_TARGET_RUNTIME)
        self.assertNotIn("CountermeasurePod", GET_TARGET_RUNTIME)
        self.assertNotIn("MinHeightFromOriginCm", GET_TARGET_RUNTIME)
        self.assertNotIn("RadarNode", GET_TARGET_RUNTIME)
        self.assertNotIn("ESkyguardMission02WaveState", GET_TARGET_RUNTIME)
        self.assertNotIn("HarborIndustrial", GET_TARGET_RUNTIME)
        self.assertNotIn("MaxIntegrity", GET_TARGET_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_TARGET_RUNTIME)
        self.assertNotIn("FillAndFinalize", GET_TARGET_RUNTIME)
        self.assertNotIn("FillAndFail", GET_TARGET_RUNTIME)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_TARGET_RUNTIME)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_spaces = (
            "public:\n"
            "\tFSkyguardAirfieldTargetRuntime   GetTargetRuntime(ESkyguardAirfieldTarget Target) const;\n"
            "private:\n"
            "};\n"
        )
        wrap_tab = (
            "public:\n"
            "\tFSkyguardAirfieldTargetRuntime\t"
            "GetTargetRuntime(ESkyguardAirfieldTarget Target) const;\n"
            "};\n"
        )
        wrap_indent = (
            "public:\n"
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_TARGETS} {GET_TARGET_RUNTIME}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_TARGET_RUNTIME}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_TARGET_RUNTIME))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_TARGET_RUNTIME),
            section,
    ...
    def test_environment_category_does_not_satisfy_integration(self) -> None:
        ...
        self.assertNotEqual(environment, UFUNCTION_TARGETS)
        self.assertNotEqual(briefing, UFUNCTION_TARGETS)
        self.assertNotIn(environment, UFUNCTION_TARGETS)
        self.assertNotIn(briefing, UFUNCTION_TARGETS)
        leftover_m01 = (
            'UFUNCTION(BlueprintCallable, '
        ...
        self.assertNotEqual(leftover_m01, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m02, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_harbor, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m03, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m04, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m05, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m03_waves, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m04_waves, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m05_waves, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_integration, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_targets, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_m05_ready, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_waves, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_payload, UFUNCTION_TARGETS)
        self.assertNotEqual(leftover_objectives, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m01, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m02, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_harbor, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m03, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m04, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m05, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m03_waves, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m04_waves, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_m05_waves, UFUNCTION_TARGETS)
        self.assertNotIn(leftover_integration, UFUNCTION_TARGETS)
        self.assertNotIn("Mission01", UFUNCTION_TARGETS)
        self.assertNotIn("Mission03", UFUNCTION_TARGETS)
        self.assertNotIn("Mission04", UFUNCTION_TARGETS)
        self.assertNotIn("Mission05", UFUNCTION_TARGETS)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_TARGET_RUNTIME}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_TARGET_RUNTIME)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_TARGET_RUNTIME)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_target_runtime_cpp_body(self) -> None:
        locked_only = f"{GET_TARGET_RUNTIME}\n"
        self.assertNotIn("{", GET_TARGET_RUNTIME)
        self.assertNotIn("}", GET_TARGET_RUNTIME)
        self.assertNotIn("return ", GET_TARGET_RUNTIME)
        ...
        self.assertNotIn("return false", GET_TARGET_RUNTIME)
        self.assertNotIn("return true", GET_TARGET_RUNTIME)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_TARGET_RUNTIME}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_TARGET_RUNTIME)
        self.assertIn("GetTargetRuntime", GET_TARGET_RUNTIME)
        self.assertTrue(
            GET_TARGET_RUNTIME.startswith(
                "FSkyguardAirfieldTargetRuntime "
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_TARGET_RUNTIME)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_TARGET_RUNTIME)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_TARGET_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_TARGET_RUNTIME)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_TARGET_RUNTIME)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_TARGET_RUNTIME)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_TARGET_RUNTIME)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_TARGET_RUNTIME}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_TARGET_RUNTIME),
            GET_TARGET_RUNTIME,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_TARGET_RUNTIME)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_TARGET_RUNTIME)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_TARGET_RUNTIME)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 GetTargetRuntime method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_TARGET_RUNTIME}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_TARGET_RUNTIME.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_TARGET_RUNTIME.lower(),
                "mission06 GetTargetRuntime contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_TARGET_RUNTIME)

    def test_contract_is_get_target_runtime_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertIn("GetTargetRuntime", GET_TARGET_RUNTIME)
        self.assertTrue(GET_TARGET_RUNTIME.endswith(";"))
        self.assertIn(UFUNCTION_TARGETS, section)

    def test_sibling_director_fields_do_not_satisfy_get_target_runtime(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_TARGET_RUNTIME)
            )
            self.assertNotEqual(GET_TARGET_RUNTIME, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetTargetRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_TARGET_RUNTIME))
        locked_only = f"{GET_TARGET_RUNTIME}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tFSkyguardAirfieldTargetRuntime GetTargetRuntime(ESkyguardAirfieldTarget Target) const { return 0; }\n"
            "};\n"
        )
    ...
    def test_contract_does_not_relock_leftover_mission05_surviving_count(
        ...
        self.assertNotIn("GetSurvivingTargetCount", GET_TARGET_RUNTIME)
        self.assertNotIn("Protection", UFUNCTION_TARGETS)
    ...
    def test_contract_does_not_relock_leftover_mission06_wave_state_enum(
        ...
        self.assertIn("Targets", UFUNCTION_TARGETS)
    ...
    def test_contract_does_not_relock_leftover_handle_boss_phase_changed(
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_TARGET_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", GET_TARGET_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", UFUNCTION_TARGETS)
    ...
    def test_contract_does_not_relock_leftover_mission05_protected_target_damage(
        ...
        self.assertNotIn("NotifyProtectedTargetDamage", GET_TARGET_RUNTIME)
        self.assertNotIn("Protection", UFUNCTION_TARGETS)

    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission06_get_target_runtime"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission06_get_target_runtime"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_leftover_mission01_get_remaining_threats_in_wave_does_not_satisfy(self) -> None:
        ...
        for token in LEFTOVER_MISSION01_GET_TARGET_RUNTIME_NOT_LOCKED:
            self.assertNotIn(token, GET_TARGET_RUNTIME)
    ...
    def test_leftover_mission03_get_remaining_threats_in_wave_does_not_satisfy(self) -> None:
        ...
        for token in LEFTOVER_MISSION03_GET_TARGET_RUNTIME_NOT_LOCKED:
            self.assertNotIn(token, GET_TARGET_RUNTIME)
    ...
    def test_leftover_mission04_get_remaining_threats_in_wave_does_not_satisfy(self) -> None:
        ...
        for token in LEFTOVER_MISSION04_GET_TARGET_RUNTIME_NOT_LOCKED:
            self.assertNotIn(token, GET_TARGET_RUNTIME)


    def test_leftover_mission05_get_protected_target_does_not_satisfy(self) -> None:
        leftover = (
            "class SKYGUARD52_API ASkyguardMission05IntegrationDirector "
            ": public AActor\n"
            "{\n"
        ...
        for token in LEFTOVER_MISSION05_GET_PROTECTED_TARGET_NOT_LOCKED:
            self.assertNotIn(token, GET_TARGET_RUNTIME)
        self.assertNotIn("Mission05", UFUNCTION_TARGETS)
        self.assertNotIn("Protection", UFUNCTION_TARGETS)
        self.assertNotIn("GetProtectedTarget", GET_TARGET_RUNTIME)
    ...
    def test_leftover_mission05_get_remaining_threats_in_wave_does_not_satisfy(self) -> None:
        ...
        for token in LEFTOVER_MISSION05_GET_TARGET_RUNTIME_NOT_LOCKED:
            self.assertNotIn(token, GET_TARGET_RUNTIME)
        self.assertNotIn("Mission05", UFUNCTION_TARGETS)
