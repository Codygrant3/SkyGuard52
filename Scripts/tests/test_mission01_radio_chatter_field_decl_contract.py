...
# INDEX_NONE or lock RadioChatter construction in the
# .cpp. This is a FIELD contract on
# ASkyguardMission01IntegrationDirector, not leftover
# USkyguardRadioChatterComponent method / fail-closed
# contracts (already drafted) and not leftover
# briefing-widget getters. Keep RadioChatter
# distinct from sibling director fields Briefing /
# Root / AudioDirector / SortiePresentation /
...
# SkyguardRadioChatterComponent.h. Stay off leftover
# radio-chatter empty fail-closed / empty-queue /
# empty-line contracts. Stay off leftover
...
# RadioChatter;`); accept that form and other
...
# RadioChatter field declaration contract on
...
# #6b9d / #b931, sibling Briefing / AudioDirector /
# Root field contracts stay sibling-only. Leftover
# radio-chatter empty fail-closed / empty-queue /
# empty-line contracts stay sibling-only.
LOCKED_SCRIPTS = (
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01IntegrationDirector::RadioChatter",
    "SkyguardMission01IntegrationDirector.cpp",
...
class Mission01RadioChatterFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, RADIO_CHATTER_FIELD), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, RADIO_CHATTER_FIELD)
        self.assertIn("RadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, RADIO_CHATTER_FIELD))
    ...
    def test_missing_radio_chatter_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, RADIO_CHATTER_FIELD)
        self.assertIn("RadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, RADIO_CHATTER_FIELD)
        self.assertIn("RadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, RADIO_CHATTER_FIELD), section)
        self.assertNotIn("UPROPERTY", RADIO_CHATTER_FIELD)
        self.assertNotIn("VisibleAnywhere", RADIO_CHATTER_FIELD)
        self.assertNotIn("BlueprintReadOnly", RADIO_CHATTER_FIELD)
        self.assertNotIn("Category", RADIO_CHATTER_FIELD)
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, RADIO_CHATTER_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, RADIO_CHATTER_FIELD)
        self.assertIn("RadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tUSkyguardRadioChatterComponent* RadioChatter;\n"
        as_array = (
            "\tTArray<TObjectPtr<USkyguardRadioChatterComponent>> "
            "RadioChatter;\n"
        )
        scene = "\tTObjectPtr<USceneComponent> RadioChatter;\n"
        briefing = (
            "\tTObjectPtr<USkyguardMissionBriefingComponent> "
            "RadioChatter;\n"
        )
        weak = (
            "\tTWeakObjectPtr<USkyguardRadioChatterComponent> "
            "RadioChatter;\n"
        )
        assigned = (
            "\tTObjectPtr<USkyguardRadioChatterComponent> "
        ...
        soft = (
            "\tTSoftObjectPtr<USkyguardRadioChatterComponent> "
            "RadioChatter;\n"
        )
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, RADIO_CHATTER_FIELD)
            self.assertIn("RadioChatter", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_radio_chatter_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, RADIO_CHATTER_FIELD))
        self.assertEqual(declaration_count(section, RADIO_CHATTER_FIELD), 1)
        self.assertTrue(
            RADIO_CHATTER_FIELD.startswith(
                "TObjectPtr<USkyguardRadioChatterComponent> "
            ),
            RADIO_CHATTER_FIELD,
        )
        self.assertTrue(RADIO_CHATTER_FIELD.endswith(";"), RADIO_CHATTER_FIELD)
        self.assertIn("RadioChatter", RADIO_CHATTER_FIELD)
        self.assertIn(
            "USkyguardRadioChatterComponent",
            RADIO_CHATTER_FIELD,
        )
        self.assertIn("TObjectPtr<", RADIO_CHATTER_FIELD)
        self.assertNotIn("TArray<", RADIO_CHATTER_FIELD)
        self.assertNotIn("TSoftObjectPtr", RADIO_CHATTER_FIELD)
        self.assertNotIn("=", RADIO_CHATTER_FIELD)
        self.assertNotIn("INDEX_NONE", RADIO_CHATTER_FIELD)
        self.assertNotIn("UFUNCTION", RADIO_CHATTER_FIELD)
        self.assertNotIn("{", RADIO_CHATTER_FIELD)
        self.assertNotIn("}", RADIO_CHATTER_FIELD)
        self.assertNotIn("return ", RADIO_CHATTER_FIELD)
        self.assertNotIn("Briefing", RADIO_CHATTER_FIELD)
        self.assertNotIn("AudioDirector", RADIO_CHATTER_FIELD)
        self.assertNotIn("SortiePresentation", RADIO_CHATTER_FIELD)
        self.assertNotIn("CampaignDefinition", RADIO_CHATTER_FIELD)
        self.assertNotIn("MissionDefinition", RADIO_CHATTER_FIELD)
        self.assertNotIn("Readiness", RADIO_CHATTER_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetAircraft", RADIO_CHATTER_FIELD)
        self.assertNotIn("ConfigureFromMission", RADIO_CHATTER_FIELD)
        self.assertNotIn("AdvanceBriefing", RADIO_CHATTER_FIELD)
        self.assertNotIn("SetAssetsReady", RADIO_CHATTER_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", RADIO_CHATTER_FIELD)
        self.assertNotIn("CanLaunch", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetElapsedSeconds", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetBriefingState", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetBriefingText", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetRadioChatter", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetPresentation", RADIO_CHATTER_FIELD)
        self.assertNotIn("GetMissionTitle", RADIO_CHATTER_FIELD)
        self.assertNotIn("AcknowledgeBriefing", RADIO_CHATTER_FIELD)
        self.assertNotIn("LaunchSortie", RADIO_CHATTER_FIELD)
        self.assertNotIn("HullCollider", RADIO_CHATTER_FIELD)
        self.assertNotIn("OpticalTracker", RADIO_CHATTER_FIELD)
        self.assertNotIn("WeaponServo", RADIO_CHATTER_FIELD)
        self.assertNotIn("CountermeasurePod", RADIO_CHATTER_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", RADIO_CHATTER_FIELD)
        self.assertNotIn("RadarNode", RADIO_CHATTER_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", RADIO_CHATTER_FIELD)
        self.assertNotIn("HarborIndustrial", RADIO_CHATTER_FIELD)
        self.assertNotIn("MaxIntegrity", RADIO_CHATTER_FIELD)
        self.assertNotIn("CurrentIntegrity", RADIO_CHATTER_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, RADIO_CHATTER_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01} {RADIO_CHATTER_FIELD}\n"
            "};\n"
        )
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, RADIO_CHATTER_FIELD), 1)
        one_line = f"{{\npublic:\n\t{RADIO_CHATTER_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, RADIO_CHATTER_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, RADIO_CHATTER_FIELD), section)
        self.assertEqual(
            require_declaration(section, RADIO_CHATTER_FIELD),
    ...
    def test_assigned_radio_chatter_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<USkyguardRadioChatterComponent> "
            "RadioChatter = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, RADIO_CHATTER_FIELD)
        self.assertIn("RadioChatter", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, RADIO_CHATTER_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("RadioChatter", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, RADIO_CHATTER_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{RADIO_CHATTER_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, RADIO_CHATTER_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            RADIO_CHATTER_FIELD.startswith("UFUNCTION"),
            RADIO_CHATTER_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, RADIO_CHATTER_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", RADIO_CHATTER_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_radio_chatter_cpp_body(self) -> None:
        locked_only = f"{RADIO_CHATTER_FIELD}\n"
        self.assertNotIn("{", RADIO_CHATTER_FIELD)
        self.assertNotIn("}", RADIO_CHATTER_FIELD)
        self.assertNotIn("return ", RADIO_CHATTER_FIELD)
        ...
        self.assertNotIn("return false", RADIO_CHATTER_FIELD)
        self.assertNotIn("return true", RADIO_CHATTER_FIELD)
        self.assertNotIn("CreateDefaultSubobject", RADIO_CHATTER_FIELD)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{RADIO_CHATTER_FIELD}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, RADIO_CHATTER_FIELD)
        self.assertIn("RadioChatter", RADIO_CHATTER_FIELD)
        self.assertIn(
            "USkyguardRadioChatterComponent",
            RADIO_CHATTER_FIELD,
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, RADIO_CHATTER_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", RADIO_CHATTER_FIELD)
        self.assertNotIn("CurrentIntegrity", RADIO_CHATTER_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", RADIO_CHATTER_FIELD)

    def test_contract_does_not_open_leftover_radio_chatter_component_header(
        self,
    ) -> None:
        locked_only = f"{RADIO_CHATTER_FIELD}\n"
        file_text = this_file_text()
        leftover_header = "SkyguardRadioChatterComponent.h"
        self.assertNotIn(leftover_header, locked_only)
        self.assertNotIn(leftover_header, RADIO_CHATTER_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, RADIO_CHATTER_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", RADIO_CHATTER_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{RADIO_CHATTER_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, RADIO_CHATTER_FIELD),
            RADIO_CHATTER_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, RADIO_CHATTER_FIELD)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 RadioChatter field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{RADIO_CHATTER_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, RADIO_CHATTER_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                RADIO_CHATTER_FIELD.lower(),
                "mission01 RadioChatter contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, RADIO_CHATTER_FIELD)

    def test_contract_is_radio_chatter_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertTrue(RADIO_CHATTER_FIELD.endswith(";"))
        self.assertNotIn("=", RADIO_CHATTER_FIELD)
        self.assertIn(UPROPERTY_MISSION01, section)

    def test_sibling_director_fields_do_not_satisfy_radio_chatter(self) -> None:
        for leftover in (
            ROOT_FIELD,
            BRIEFING_FIELD,
            AUDIO_DIRECTOR_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, RADIO_CHATTER_FIELD))
            self.assertNotEqual(RADIO_CHATTER_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_root_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
