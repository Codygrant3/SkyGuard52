...
# INDEX_NONE or lock NotifyFuelTerminalDamage
# construction in the .cpp. This is leftover-safe
# Mission02 Objectives NotifyFuelTerminalDamage
# METHOD on ASkyguardMission02IntegrationDirector. THIS
# is NotifyFuelTerminalDamage, NOT leftover Mission02
# InitializePlayableMission #828 / leftover
# ConfigureMissionDefinition / leftover StartNextWave /
# leftover NotifyThreatDestroyed siblings, NOT leftover
# Mission10 NotifyProtectedGroupDamage #774, NOT leftover
# Mission10 StartPhaseWave
...
# (`bool NotifyFuelTerminalDamage(int32 Damage);`); accept that
...
# drafts #674–#830+, leftover Apache
...
# NotifyFuelTerminalDamage #731. Do not reopen leftover
# Mission09 NotifyFuelTerminalDamage #753. Do not reopen
# leftover Mission02 InitializePlayableMission
# #828 / ConfigureMissionDefinition /
# NotifyFuelTerminalDamage / NotifyThreatDestroyed
# siblings. Do not reopen leftover
# Mission10 StartPhaseWave. Do not reopen leftover
# same-name Mission10 NotifyProtectedGroupDamage
# #774. Do not reopen leftover Mission03
# NotifyConvoyDamage #632 / leftover Mission04
# NotifySubstationDamage #654 / leftover Mission05
# NotifyProtectedTargetDamage #673. Do not reopen
...
# leftover Mission01 / Mission03 #630 / Mission04
# #649 / Mission05 #668 / Mission06–#09 same-name
# NotifyFuelTerminalDamage drafts. Do not reopen
# open drafts #674–#830+.
...
NOTIFY_FUEL_TERMINAL_DAMAGE = "bool NotifyFuelTerminalDamage(int32 Damage);"
REQUIRED_DECL = NOTIFY_FUEL_TERMINAL_DAMAGE
...
# an isolated Python NotifyFuelTerminalDamage
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission05_notify_protected_target_damage_decl_contract.py",
...
LEFTOVER_MISSION01_START_NEXT_WAVE_NOT_LOCKED = (
    "test_mission01_start_next_wave_decl_contract.py",
    "ASkyguardMission01IntegrationDirector",
    "SkyguardMission01IntegrationDirector.h",
...
LEFTOVER_MISSION05_START_NEXT_WAVE_NOT_LOCKED = (
    ...
    "SkyguardMission05IntegrationDirector.h",
    'Category="Skyguard|Mission05|Waves"',
    "cursor/mission05-start-next-wave-decl-contract-c332",
)
...
LEFTOVER_MISSION08_START_NEXT_WAVE_NOT_LOCKED = (
    "test_mission08_start_next_wave_decl_contract.py",
    "ASkyguardMission08IntegrationDirector",
    "SkyguardMission08IntegrationDirector.h",
    'Category="Skyguard|Mission08|Waves"',
    "cursor/mission08-start-next-wave-decl-contract-c332",
)
LEFTOVER_MISSION09_START_NEXT_WAVE_NOT_LOCKED = (
    "test_mission09_start_next_wave_decl_contract.py",
    "ASkyguardMission09IntegrationDirector",
    "SkyguardMission09IntegrationDirector.h",
    'Category="Skyguard|Mission09|Waves"',
    "cursor/mission09-start-next-wave-decl-contract-c332",
)
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission02IntegrationDirector::NotifyFuelTerminalDamage",
    "SkyguardMission02IntegrationDirector.cpp",
...
def strip_default_arguments(text: str) -> str:
    compact = collapsed(text)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if compact_decl in compact_region:
        return True
    stripped_region = strip_default_arguments(region)
    stripped_decl = strip_default_arguments(declaration)
    if stripped_decl in stripped_region:
        return True
    stem = declaration_stem(stripped_decl)
    if not stem:
        return False
    # Accept `;` or an inline `{` body after the signature
    # without locking that body. Accept default arguments.
    pattern = re.compile(re.escape(stem) + r"\s*[;{]")
    return pattern.search(stripped_region) is not None
...
def declaration_count(region: str, declaration: str) -> int:
    ...
    stripped_decl = strip_default_arguments(declaration)
    if stripped_decl in stripped_region:
        return stripped_region.count(stripped_decl)
    stem = declaration_stem(stripped_decl)
    if not stem:
        return 0
    pattern = re.compile(re.escape(stem) + r"\s*[;{]")
    return len(pattern.findall(stripped_region))
...
class Mission02NotifyFuelTerminalDamageDeclContractTests(unittest.TestCase):
    def test_mission02_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, NOTIFY_FUEL_TERMINAL_DAMAGE),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn("NotifyFuelTerminalDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_missing_notify_fuel_terminal_damage_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission02IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn("NotifyFuelTerminalDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn("NotifyFuelTerminalDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("BlueprintCallable", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("Category", NOTIFY_FUEL_TERMINAL_DAMAGE)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn("NotifyFuelTerminalDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = "\tvoid NotifyFuelTerminalDamage(int32 Damage);\n"
        as_const = "\tbool NotifyFuelTerminalDamage(int32 Damage) const;\n"
        with_arg = "\tbool NotifyFuelTerminalDamage(bool bForce);\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, NOTIFY_FUEL_TERMINAL_DAMAGE)
            self.assertIn("NotifyFuelTerminalDamage", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_notify_fuel_terminal_damage_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, NOTIFY_FUEL_TERMINAL_DAMAGE),
            NOTIFY_FUEL_TERMINAL_DAMAGE,
        )
        self.assertTrue(has_declaration(section, NOTIFY_FUEL_TERMINAL_DAMAGE))
        self.assertEqual(
            declaration_count(section, NOTIFY_FUEL_TERMINAL_DAMAGE),
            1,
        )
        self.assertTrue(
            NOTIFY_FUEL_TERMINAL_DAMAGE.startswith("bool "),
            NOTIFY_FUEL_TERMINAL_DAMAGE,
        )
        self.assertTrue(
            NOTIFY_FUEL_TERMINAL_DAMAGE.endswith(";"),
            NOTIFY_FUEL_TERMINAL_DAMAGE,
        )
        self.assertIn("NotifyFuelTerminalDamage", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn("(int32 Damage)", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("TArray<", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("TObjectPtr", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("TSoftObjectPtr", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("INDEX_NONE", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("UFUNCTION", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("{", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("}", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("return ", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("const;", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("Root", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("Briefing", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AudioDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("RadioChatter", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("SortiePresentation", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("CampaignDefinition", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("MissionDefinition", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("Readiness", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("bAutoInitialize", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("bAllowBoundedActorSpawning", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("bAutoLaunchAfterBriefing", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("PathfinderSpawnLocation", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetAircraft", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("BindRuntimeActors", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("HandleDroneCityImpact", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("InitializePlayableMission", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("NotifyThreatDestroyed", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AdvanceConvoyByDistance", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("NotifyConvoyDamage", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetWaveState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetConvoyRouteState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetDayBeatKit", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetNightBeatKit", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("StartSearchlightWindow", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AdvanceSearchlightTrack", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("NotifySubstationDamage", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetRemainingThreatsInWave", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetSearchlightRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetSubstationIntegrity", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("StartPayloadWindow", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AdvancePayloadWindow", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("TryJamActivePayload", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("NotifyAirfieldTargetDamage", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetPayloadWindow", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetTargetRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetSurvivingTargetCount", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetDayBeatKind", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetDayBeatIndex", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("TickDayBeatKit", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("HandleBossPhaseChanged", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission06WaveState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission07WaveState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission07IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission08IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission08WaveState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission09WaveState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission09ProtectedTarget", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardMission09ProtectedTargetRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardMission09PoolRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardMission09PoolBudget", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("BindCampaignRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetPoolRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("IronRainSpawnLocation", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission08ProtectedTarget", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardMission08ProtectedTargetRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardHoistWindowRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("StartHoistWindow", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AdvanceHoistWindow", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ValidateWeaponRelease", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetHoistRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetRejectedWeaponReleases", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetStormRainBeatKit", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ApplyStormRainPlayContract", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetStormRainBeatKind", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("TickStormRainBeatKit", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("RescueHelicopterAnchor", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("LifelineHunterSpawnLocation", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardSearchSector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardSearchTrackRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ClassifyFalseTrack", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ConfirmRadarGhostIdentification", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("NotifyProtectedTargetDamage", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AdvanceReinforcementTimer", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetSearchSector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetClassifiedFalseTrackCount", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("IsHostileContactConfirmed", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetProtectedTarget", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetReinforcementTimeRemaining", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetNightBeatKind", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetNightBeatIndex", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("TickNightBeatKit", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardAirfieldTarget", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ConvoyRuntimeAnchor", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("RoadHunterSpawnLocation", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ConfigureMissionDefinition", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("NotifyObjectiveProgress", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("NotifyProtectedAssetFailed", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("SynchronizeRuntimeState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("IsCorePlayableReady", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetReadiness", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetObjectiveRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetGunner", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetPathfinder", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetMissionId", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ValidateMissionContract", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ConfigureFromMission", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AdvanceBriefing", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("SetAssetsReady", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AcknowledgeAndLaunch", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("CanLaunch", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetElapsedSeconds", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetBriefingState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetMinimumWarmupSeconds", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetBriefingText", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetRadioChatter", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetPresentation", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetMissionTitle", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("AcknowledgeBriefing", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("LaunchSortie", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("HullCollider", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("OpticalTracker", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("WeaponServo", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("CountermeasurePod", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("MinHeightFromOriginCm", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("RadarNode", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ESkyguardMission02WaveState", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("HarborIndustrial", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("MaxIntegrity", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("CurrentIntegrity", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FillAndFinalize", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("FillAndFail", NOTIFY_FUEL_TERMINAL_DAMAGE)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tNotifyFuelTerminalDamage(int32 Damage);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_name = (
            "public:\n"
            "\tbool NotifyFuelTerminalDamage(\n"
            "\t\tint32 Damage);\n"
            "};\n"
        )
        wrap_default = (
            "public:\n"
            "\tbool NotifyFuelTerminalDamage(int32 Damage = 0);\n"
            "};\n"
        )
        wrap_ufunction = (
            "public:\n"
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_OBJECTIVES} {NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{NOTIFY_FUEL_TERMINAL_DAMAGE}\n}}\n"
        self.assertTrue(has_declaration(one_line, NOTIFY_FUEL_TERMINAL_DAMAGE))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, NOTIFY_FUEL_TERMINAL_DAMAGE),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", NOTIFY_FUEL_TERMINAL_DAMAGE)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_notify_fuel_terminal_damage_cpp_body(self) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        self.assertNotIn("{", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("}", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("return ", NOTIFY_FUEL_TERMINAL_DAMAGE)
        ...
        self.assertNotIn("return false", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("return true", NOTIFY_FUEL_TERMINAL_DAMAGE)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn("NotifyFuelTerminalDamage", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertTrue(NOTIFY_FUEL_TERMINAL_DAMAGE.startswith("bool "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("CurrentIntegrity", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("SkyguardApacheAircraft.h", NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        self.assertNotIn("550.f", NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        self.assertEqual(
            require_declaration(locked_only, NOTIFY_FUEL_TERMINAL_DAMAGE),
            NOTIFY_FUEL_TERMINAL_DAMAGE,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardIronRainBoss", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardRadarGhostBoss", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("ASkyguardTempestBoss", section)
        self.assertNotIn("ASkyguardLastFlightBoss", NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission02 NotifyFuelTerminalDamage method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, NOTIFY_FUEL_TERMINAL_DAMAGE.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                NOTIFY_FUEL_TERMINAL_DAMAGE.lower(),
                "mission02 NotifyFuelTerminalDamage contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, NOTIFY_FUEL_TERMINAL_DAMAGE)

    def test_contract_is_notify_fuel_terminal_damage_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(NOTIFY_FUEL_TERMINAL_DAMAGE.startswith("bool "))
        self.assertIn("NotifyFuelTerminalDamage", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertTrue(NOTIFY_FUEL_TERMINAL_DAMAGE.endswith(";"))
        self.assertIn(UFUNCTION_OBJECTIVES, section)

    def test_sibling_director_fields_do_not_satisfy_notify_fuel_terminal_damage(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, NOTIFY_FUEL_TERMINAL_DAMAGE)
            )
            self.assertNotEqual(NOTIFY_FUEL_TERMINAL_DAMAGE, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("NotifyFuelTerminalDamage", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, NOTIFY_FUEL_TERMINAL_DAMAGE))
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tbool NotifyFuelTerminalDamage(int32 Damage) { return false; }\n"
            "};\n"
        )
    ...
    def test_leftover_mission05_initialize_does_not_satisfy(self) -> None:
        ...
        self.assertIn(
            "Scripts/tests/test_mission04_notify_substation_damage"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission05_notify_protected_target_damage"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission06_notify_airfield_target_damage"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission01_start_next_wave"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission02_notify_fuel_terminal_damage"
                "_decl_contract.py"
            )
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission07_notify_protected_target_damage"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_mission08_notify_protected_target_damage"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_does_not_relock_leftover_mission10_route_phase_enum(
        ...
        self.assertNotIn("GetRoutePhase", NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_contract_does_not_relock_leftover_mission10_protected_group(
        ...
        self.assertNotIn("Protection", UFUNCTION_OBJECTIVES)
        self.assertNotIn("GetProtectedGroup", NOTIFY_FUEL_TERMINAL_DAMAGE)
    ...
    def test_leftover_mission10_sibling_methods_do_not_satisfy(self) -> None:
        ...
        self.assertNotIn("HandleDroneCityImpact", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("UFUNCTION", GET_DAY_BEAT_KIT)
        self.assertNotIn("StartPhaseWave", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("GetRoutePhase", NOTIFY_FUEL_TERMINAL_DAMAGE)


    def test_contract_does_not_relock_leftover_mission01_start_next_wave(
        self,
    ) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        for token in LEFTOVER_MISSION01_START_NEXT_WAVE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn(
            "Scripts/tests/test_mission01_start_next_wave"
    ...
    def test_contract_does_not_relock_leftover_mission08_start_next_wave(
        self,
    ) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        for token in LEFTOVER_MISSION08_START_NEXT_WAVE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn(
            "Scripts/tests/test_mission08_start_next_wave"
    ...
    def test_contract_does_not_relock_leftover_mission09_start_next_wave(
        self,
    ) -> None:
        locked_only = f"{NOTIFY_FUEL_TERMINAL_DAMAGE}\n"
        for token in LEFTOVER_MISSION09_START_NEXT_WAVE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertIn(
            "Scripts/tests/test_mission09_start_next_wave"
    ...
    def test_leftover_mission02_initialize_does_not_satisfy(self) -> None:
        ...
        for token in LEFTOVER_MISSION02_INITIALIZE_NOT_LOCKED:
            self.assertNotIn(token, NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("InitializePlayableMission", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("StartNextWave", NOTIFY_FUEL_TERMINAL_DAMAGE)
        self.assertNotIn("Integration", UFUNCTION_OBJECTIVES)
        self.assertNotIn("Waves", UFUNCTION_OBJECTIVES)
