# THIS IS leftover-safe FSkyguardNightSortieBeatKit bKeepThermal.
# origin/main form: BARE `bool bKeepThermal = true;`
...
# Locked decl must be the BARE field `bool bKeepThermal = true;`.
# Parse STRUCT `FSkyguardNightSortieBeatKit` body ONLY after
# `struct FSkyguardNightSortieBeatKit`. Stop before
...
# Do NOT parse leftover analog night-sortie-beat-kit-defaults
# #250 / #2ca7 bulk.
# Do NOT parse leftover analog night-sortie-beat-kit-fields
# #254 / #9787.
# Do NOT parse leftover analog night-sortie-beat-kit-beats
# #252 / #b4a6.
# Do NOT parse leftover analog night-sortie-beat-defaults
# #247 / #5bf1.
# Do NOT parse leftover analog day-sortie-beat-defaults
# #249 / #ba98.
# Do NOT parse leftover NightSortieBeat isolated #1477-#1479.
# Do NOT parse leftover DaySortieBeat isolated Call #1480 /
# Threat #1481 / Kind.
# Do NOT parse leftover LoadoutSpec isolated #1466-#1476.
# Do NOT parse leftover TheaterKitSpec weather-identity #1300
# (UPROPERTY clone source; retarget hard).
# Do NOT contract sibling fields MissionId / WeatherIdentity /
# Beats. Exact identifier vs those siblings.
# Fail-closed if bKeepThermal is missing/renamed, type is not
# `bool`, initializer is missing or is not `true`, or
# UPROPERTY / Category clones land.
# Analog leftover night-sortie-beat-kit-defaults #250 / #2ca7
# (`Scripts/tests/test_night_sortie_beat_kit_defaults_contract.py`).
# Keep analog bulk in LOCKED_SCRIPTS. Do not reopen leftover
# analog bulk. Then this isolated FIELD decl.
# Harbor 40/80 fail-closed. Prefer split forty/eighty tokens.
...
LEFTOVER_BEAT = "FSkyguardNightSortieBeat"
LEFTOVER_KIND_ENUM = "ESkyguardNightSortieBeatKind"
...
TARGET = "bool bKeepThermal = true;"
TARGET_WRONG_BARE = "bool bKeepThermal;"
TARGET_WRONG_FALSE = "bool bKeepThermal = false;"
TARGET_WRONG_ZERO = "bool bKeepThermal = 0;"
TARGET_WRONG_ZERO_F = "bool bKeepThermal = 0.f;"
TARGET_WRONG_INT = "int32 bKeepThermal = true;"
TARGET_WRONG_FLOAT = "float bKeepThermal = true;"
TARGET_WRONG_FNAME = "FName bKeepThermal = true;"
TARGET_WRONG_SIX = "bool bKeepThermal = 6;"
...
STOP_BEFORE_BEAT = "struct FSkyguardNightSortieBeat\n"
STOP_BEFORE_KIND_ENUM = "enum class ESkyguardNightSortieBeatKind"
STOP_BEFORE_DAY_KIT = "struct FSkyguardDaySortieBeatKit"
STOP_BEFORE_STORM_KIT = "struct FSkyguardStormRainBeatKit"
STOP_BEFORE_NIGHT_EYES = "NightEyes"
STOP_BEFORE_DOWNED_BIRD = "DownedBird"
STOP_BEFORE_FOR_MISSION = "ForMission"
STOP_BEFORE_SEQUENCES = "SequencesDiffer"
STOP_BEFORE_BEAT_INDEX = "BeatIndexForElapsed"
STOP_BEFORE_KIND_AT = "KindAt"
...
LEFTOVER_NIGHT_KIT_BULK = (
    "Scripts/tests/test_night_sortie_beat_kit_contract.py"
)
LEFTOVER_DAY_KIT_BULK = (
    "Scripts/tests/test_day_sortie_beat_kit_contract.py"
)
LEFTOVER_STORM_KIT_BULK = (
    "Scripts/tests/test_storm_rain_beat_kit_contract.py"
)
LEFTOVER_NIGHT_BEAT_KIND = (
    "Scripts/tests/test_night_sortie_beat_kind_field_decl_contract.py"
)
LEFTOVER_NIGHT_BEAT_CALL = (
    "Scripts/tests/test_night_sortie_beat_call_field_decl_contract.py"
)
LEFTOVER_NIGHT_BEAT_THREAT = (
    "Scripts/tests/test_night_sortie_beat_threat_field_decl_contract.py"
)
LEFTOVER_DAY_BEAT_KIND = (
    "Scripts/tests/test_day_sortie_beat_kind_field_decl_contract.py"
)
LEFTOVER_DAY_BEAT_CALL = (
    "Scripts/tests/test_day_sortie_beat_call_field_decl_contract.py"
)
LEFTOVER_DAY_BEAT_THREAT = (
    "Scripts/tests/test_day_sortie_beat_threat_field_decl_contract.py"
)
...
LEFTOVER_ANALOG_LOADOUT_DEFAULTS = (
...
def leftover_kind_enum_tokens() -> tuple[str, ...]:
    return (
        "DarkIngress",
        "ThermalHunt",
        "RadarVanHunt",
...
def namespace_helper_tokens() -> tuple[str, ...]:
    return (
        STOP_BEFORE_NIGHT_EYES,
        STOP_BEFORE_DOWNED_BIRD,
        STOP_BEFORE_FOR_MISSION,
        STOP_BEFORE_SEQUENCES,
        STOP_BEFORE_BEAT_INDEX,
        STOP_BEFORE_KIND_AT,
    )
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `bool bKeepThermal = true;`.
    # Do not accept missing initializer, `= false`, `= 0`,
    # `= 0.f`, `= 6`, int32 / float / FName retargets, or
    # leftover FlareCount / WeatherIdentity clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"bool\s+bKeepThermal\s*=\s*true\s*;", compact) is None:
        return False
    if re.search(r"bool\s+bKeepThermal\s*=\s*false\b", compact):
        return False
    if re.search(r"bool\s+bKeepThermal\s*=\s*0\b", compact):
        return False
    if re.search(r"bool\s+bKeepThermal\s*=\s*0\.f", compact):
        return False
    if re.search(r"bool\s+bKeepThermal\s*=\s*6\b", compact):
        return False
    if re.search(r"int32\s+bKeepThermal\b", compact):
        return False
    if re.search(r"float\s+bKeepThermal\b", compact):
        return False
    if re.search(r"FName\s+bKeepThermal\b", compact):
        return False
    if re.search(r"bool\s+bKeepThermal\s*;", compact) and not re.search(
        r"bool\s+bKeepThermal\s*=",
        compact,
    ):
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        *namespace_helper_tokens(),
        ...
        f"struct {LEFTOVER_LOADOUT_SPEC}",
        f"struct {LEFTOVER_DAY_KIT}",
        f"struct {LEFTOVER_STORM_KIT}",
        f"enum class {LEFTOVER_KIND_ENUM}",
...
def spec_section(header: str) -> str:
    ...
    namespace_at = header.find(STOP_BEFORE_NAMESPACE)
    struct_at = STRUCT_RE.search(header)
    if struct_at is None:
        raise AssertionError(
            f"{STRUCT_NAME} is missing from origin/main:{HEADER_PATH}"
        )
    if namespace_at != -1 and namespace_at < struct_at.start():
        raise AssertionError(
    ...
    if STOP_BEFORE_KIND_ENUM in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_KIND_ENUM}"
        )
    if STOP_BEFORE_BEAT in section:
        raise AssertionError(
...
class NightSortieBeatKitKeepThermalFieldDeclContractTests(unittest.TestCase):
    def test_night_sortie_beat_kit_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_BEAT)
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_KIND_ENUM)
        ...
        self.assertTrue(has_identifier(section, "bKeepThermal"), section)
        ...
        for sibling in sibling_uncontracted_decls():
            ...
            self.assertNotEqual(LOCKED_DECL, f"bool {sibling} = true;")
        self.assertNotIn(LEFTOVER_KIND_ENUM, section)
        self.assertNotIn(STOP_BEFORE_BEAT, section)
    ...
    def test_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "bKeepThermal"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("bKeepThermal", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_keep_thermal_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_MISSION_ID};\n"
            f"\tFName {SIBLING_WEATHER_IDENTITY};\n"
            f"\t{LEFTOVER_BEAT} {SIBLING_BEATS}[7];\n"
            "};\n"
        )
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        self.assertEqual(LOCKED_DECL, "bool bKeepThermal = true;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("bool bKeepThermal"))
    ...
    def test_missing_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bool bKeepThermal = true;", compact_origin)
        self.assertNotIn("bool bKeepThermal;", compact_origin.replace(
            "bool bKeepThermal = true;",
            "",
        ))

    def test_false_initializer_fails_closed(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        compact_origin = collapsed(origin)
        self.assertIn("bool bKeepThermal = true;", compact_origin)
        self.assertNotIn("bool bKeepThermal = false;", compact_origin)
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        wrongs = (
            ...
            f"\tFName {SIBLING_WEATHER_IDENTITY};\n",
            f"\t{LEFTOVER_BEAT} {SIBLING_BEATS}[7];\n",
            "\tbool bKeepThermals = true;\n",
            "\tbool KeepThermal = true;\n",
            "\tbool bKeepThermal = " + forty + ";\n",
            "\tbool bKeepThermal = " + eighty + ";\n",
        )
    ...
    def test_keep_thermal_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bKeepThermal"),
            LOCKED_DECL,
        )
    ...
    def test_does_not_contract_sibling_kit_fields(self) -> None:
        ...
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, LOCKED_DECL)
        self.assertNotIn(LEFTOVER_BEAT, LOCKED_DECL)
    ...
    def test_does_not_parse_leftover_neighbors(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_KIND_ENUM, header)
        self.assertNotIn(STOP_BEFORE_KIND_ENUM, section)
        self.assertNotIn(STOP_BEFORE_KIND_ENUM, leaked)
        self.assertIn(STOP_BEFORE_BEAT.rstrip(), header)
        self.assertNotIn(STOP_BEFORE_BEAT, section)
        self.assertNotIn(STOP_BEFORE_BEAT, leaked)
        for token in leftover_beat_field_tokens():
        ...
        self.assertNotEqual(HEADER_PATH, "Source/Skyguard52/SkyguardGunshipTypes.h")
        self.assertNotEqual(
            HEADER_PATH,
        ...
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardDaySortieBeatKit.h",
        )
        self.assertNotEqual(
            HEADER_PATH,
    ...
    def test_parse_window_excludes_leftover_enum_and_namespace(self) -> None:
        ...
        section = spec_section(header)
        leaked = struct_body(header)
        for leftover in leftover_kind_enum_tokens():
            self.assertNotIn(leftover, section)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tconst {STRUCT_NAME}& {STOP_BEFORE_NIGHT_EYES}();\n"
            f"\tconst {STRUCT_NAME}& {STOP_BEFORE_DOWNED_BIRD}();\n"
            f"\tconst {STRUCT_NAME}& {STOP_BEFORE_FOR_MISSION}"
            "(FName MissionId);\n"
            f"\tbool {STOP_BEFORE_SEQUENCES}();\n"
            f"\tint32 {STOP_BEFORE_BEAT_INDEX}();\n"
            f"\t{LEFTOVER_KIND_ENUM} {STOP_BEFORE_KIND_AT}();\n"
            f"\tFName {SIBLING_MISSION_ID};\n"
            f"\tFName {SIBLING_WEATHER_IDENTITY};\n"
            f"\t{TARGET_WRONG_FALSE}\n"
            f"\t{TARGET_WRONG_FLARE}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "night-sortie-beat-kit bKeepThermal field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "keep_thermal_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardNightSortieBeatKit.cpp", THIS_SCRIPT)
        self.assertIn(LEFTOVER_ANALOG_KIT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ANALOG_KIT_FIELDS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ANALOG_KIT_BEATS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ANALOG_BEAT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ANALOG_DAY_BEAT_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_NIGHT_KIT_BULK, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_BEAT, locked_only)
