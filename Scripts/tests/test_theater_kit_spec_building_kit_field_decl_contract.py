# THIS IS leftover-safe FSkyguardTheaterKitSpec BuildingKit.
...
# then BARE `FName BuildingKit;` (no initializer).
...
# LandmarkSet / BuildingTint / LampTreatment / LampColor /
...
TARGET = "FName BuildingKit;"
TARGET_WRONG_EQ_NONE = "FName BuildingKit = NAME_None;"
TARGET_WRONG_FALSE = "FName BuildingKit = false;"
TARGET_WRONG_TRUE = "FName BuildingKit = true;"
TARGET_WRONG_ZERO = "FName BuildingKit = 0.f;"
TARGET_WRONG_FLOAT = "float BuildingKit;"
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+BuildingKit\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName BuildingKit is missing from "
...
class TheaterKitSpecBuildingKitFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "BuildingKit"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "BuildingKit"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("BuildingKit", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_building_kit_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FName BuildingKit;", compact_origin)
        self.assertNotIn("BuildingKit = NAME_None", compact_origin)
        self.assertNotIn("BuildingKit = false", compact_origin)
        self.assertNotIn("BuildingKit = true", compact_origin)
        self.assertNotIn("BuildingKit = 0.f", compact_origin)
        self.assertNotIn("BuildingKit = 160.f", compact_origin)

    def test_building_kit_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName BuildingKit"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 BuildingKit;\n",
            "\tbool BuildingKit;\n",
            "\tfloat BuildingKit = " + forty + ";\n",
            "\tfloat BuildingKit = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec BuildingKit field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "building_kit_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
