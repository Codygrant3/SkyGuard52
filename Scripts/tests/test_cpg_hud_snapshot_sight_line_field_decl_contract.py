# THIS IS leftover-safe FSkyguardCpgHudSnapshot SightLine.
...
# `FString SightLine;` with NO in-struct initializer.
# Fail-closed if SightLine is missing or renamed, if the
...
# Do NOT parse leftover StormRainBeatKit isolated
# (#1490-#1500). Do NOT parse leftover Night/Day
# Beat.Threat isolated (#1477/#1481).
# SightLine is NOT leftover sibling SightMode
...
# ThreatLine / EufdLine / LockLine / StationStatus /
...
# / defaults / calls or leftover Night/Day beat-kit.
...
# leftover sibling SightMode / leftover Beat.Threat,
# retarget: type is FString, identifier is SightLine,
...
TARGET = "FString SightLine;"
TARGET_WRONG_EQ_EMPTY = 'FString SightLine = TEXT("");'
TARGET_WRONG_EQ_NONE = "FString SightLine = NAME_None;"
TARGET_WRONG_FALSE = "FString SightLine = false;"
TARGET_WRONG_TRUE = "FString SightLine = true;"
TARGET_WRONG_ZERO = "FString SightLine = 0.f;"
TARGET_WRONG_FLOAT = "float SightLine;"
TARGET_WRONG_HEALTH = "float SightLine = 160.f;"
TARGET_WRONG_BOOL = "bool SightLine;"
TARGET_WRONG_INT = "int32 SightLine;"
TARGET_WRONG_FNAME = "FName SightLine;"
TARGET_WRONG_TCHAR = 'const TCHAR* SightLine = TEXT("");'
...
def leftover_storm_rain_isolated_scripts() -> tuple[str, ...]:
    ...
    return (
        prefix + "calls_field_decl_contract.py",
        prefix + "beat_count_field_decl_contract.py",
        prefix + "stations_field_decl_contract.py",
        prefix + "threats_field_decl_contract.py",
        prefix + "kinds_field_decl_contract.py",
        prefix + "weather_field_decl_contract.py",
        prefix + "hydra_for_clusters_field_decl_contract.py",
        prefix + "weather_label_field_decl_contract.py",
        prefix + "title_field_decl_contract.py",
        prefix + "weather_identity_field_decl_contract.py",
        prefix + "mission_id_field_decl_contract.py",
    )
...
LOCKED_SCRIPTS = (
    ...
    + (
        ...
        "Scripts/tests/test_day_sortie_beat_kit_defaults_contract.py",
    )
...
SIGHT_LINE_BARE_RE = re.compile(r"FString\s+SightLine\s*;")
SIGHT_LINE_INIT_RE = re.compile(r"FString\s+SightLine\s*=")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def sight_line_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*FString\s+SightLine\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString SightLine;`.
    # Do not accept `= TEXT("")` / `= NAME_None` / `= false` /
    # `= true` / `= 0.f` / `= 160.f` when origin/main locks
    # a bare FString. Do not accept sibling WeaponLine /
    # RangeLine / leftover sibling SightMode or leftover
    # ContactMark Label / leftover debrief MissionTitle.
    # Do not accept leftover LoadoutSpec FlareCount or
    # leftover Beat.Threat. Do not accept UPROPERTY /
    # Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if SIGHT_LINE_INIT_RE.search(compact):
        return False
    if re.search(r"float\s+SightLine\b", compact):
        return False
    if re.search(r"bool\s+SightLine\b", compact):
        return False
    if re.search(r"int32\s+SightLine\b", compact):
        return False
    if re.search(r"FName\s+SightLine\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*SightLine\b", compact):
        return False
    if SIGHT_LINE_BARE_RE.search(compact) is None:
        return False
    if sight_line_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString SightLine\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString SightLine is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FString SightLine\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FString SightLine is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "SightLine"):
        raise AssertionError(
            "UPROPERTY clone landed on SightLine; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "SightLine"):
            raise AssertionError(
                f"{token} clone landed on SightLine; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotSightLineFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SightLine"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString SightLine;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("FString SightLine"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("=", LOCKED_DECL)
        self.assertIn("SightLine", LOCKED_DECL)
        ...
        self.assertNotIn(SIBLING_SIGHT_MODE, LOCKED_DECL)
        self.assertNotIn(SIBLING_FLARE_COUNT, LOCKED_DECL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_WEAPON)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIGHT_MODE)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("SightLine", str(raised.exception))
    ...
    def test_does_not_claim_leftover_debrief_loadout_or_contact(self) -> None:
        ...
        self.assertIn(CLONE_STORM_TITLE, storm_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SightLine"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_sight_line_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_weapon_line_does_not_satisfy_sight_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_leftover_sight_mode_does_not_satisfy_sight_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_SIGHT_MODE}\n"
            "};\n"
        )
        section = spec_section(leftover)
        self.assertTrue(has_identifier(section, SIBLING_SIGHT_MODE), section)
        self.assertFalse(has_declaration(section, LOCKED_DECL), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SightLine", str(raised.exception))
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString SightLine;", compact_origin)
        self.assertNotIn("SightLine = TEXT", compact_origin)
        self.assertNotIn("SightLine = NAME_None", compact_origin)
        self.assertNotIn("SightLine = false", compact_origin)
        self.assertNotIn("SightLine = true", compact_origin)
        self.assertNotIn("SightLine = 0.f", compact_origin)
        self.assertNotIn("SightLine = 160.f", compact_origin)

    def test_sight_line_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString SightLine"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_RANGE}\n", LOCKED_DECL)
        self.assertIn("SightLine", str(raised.exception))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 SightLine;\n",
            "\tbool SightLine;\n",
            "\tfloat SightLine = " + forty + ";\n",
            "\tfloat SightLine = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    sight_line_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(sight_line_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot SightLine field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "sight_line_field_decl_contract.py"
        ))
    ...
    def test_leftover_analog_bulk_stays_locked(self) -> None:
        leftovers = leftover_debrief_isolated_scripts() + leftover_storm_rain_isolated_scripts() + (
            LEFTOVER_ANALOG_DEBRIEF_DEFAULTS,
            LEFTOVER_LOADOUT_FLARE,
            CLONE_THEATER_WEATHER_IDENTITY,
            CLONE_STORM_RAIN_TITLE,
