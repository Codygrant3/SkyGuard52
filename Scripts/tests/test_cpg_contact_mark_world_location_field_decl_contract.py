# THIS IS leftover-safe FSkyguardCpgContactMark WorldLocation.
...
# `FVector WorldLocation = FVector::ZeroVector;` with that
# in-struct initializer.
# Fail-closed if WorldLocation is missing or renamed, if the
# type is not FVector, if the initializer is missing or is
# not `= FVector::ZeroVector`, or if UPROPERTY / Category
# clones land.
...
# (split-token), or is not `= FVector::ZeroVector`.
# WorldLocation here is NOT leftover RoutePoint WorldLocation
# #1340, leftover MissionObjectiveAnchor WorldLocation #1386,
# leftover MissionLandmarkAnchor WorldLocation #1390, or
# leftover ContactMark sibling Label / bLocked / bSeeking /
# LockAlpha.
...
# (isolated #1501-#1511 plus in-flight FlareCount /
# bMissileInbound).
...
# Do NOT contract sibling fields Label / bLocked /
# bSeeking / LockAlpha.
...
# #1501-#1511 plus in-flight FlareCount / bMissileInbound.
# Keep leftover-safe WeaponLine #1502 and siblings in
# LOCKED_SCRIPTS.
...
# struct. Same-header HUD parse-window retarget is
# leftover-safe #1502 WeaponLine (FString, no initializer,
# parses leftover FSkyguardCpgHudSnapshot). RETARGET:
# type is FVector, identifier is WorldLocation, initializer
# is `= FVector::ZeroVector`, parse STRUCT
# FSkyguardCpgContactMark, stop before SkyguardCpgWeaponLabel.
# Do NOT copy leftover RoutePoint / MissionObjectiveAnchor /
...
TARGET_WRONG_BARE = "FVector WorldLocation;"
TARGET_WRONG_EQ_EMPTY = 'FVector WorldLocation = TEXT("");'
TARGET_WRONG_EQ_NONE = "FVector WorldLocation = NAME_None;"
...
TARGET_WRONG_ONE = "FVector WorldLocation = FVector::OneVector;"
TARGET_WRONG_FLOAT = "float WorldLocation = 0.f;"
TARGET_WRONG_HEALTH = "float WorldLocation = 160.f;"
TARGET_WRONG_BOOL = "bool WorldLocation;"
TARGET_WRONG_INT = "int32 WorldLocation;"
TARGET_WRONG_FNAME = "FName WorldLocation;"
TARGET_WRONG_FSTRING = "FString WorldLocation;"
TARGET_WRONG_TCHAR = 'const TCHAR* WorldLocation = TEXT("");'
...
WORLD_LOCKED_RE = re.compile(
    r"FVector\s+WorldLocation\s*=\s*FVector::ZeroVector\s*;"
)
WORLD_BARE_RE = re.compile(r"FVector\s+WorldLocation\s*;")
WORLD_INIT_RE = re.compile(
    r"FVector\s+WorldLocation\s*=\s*([^;]+);"
)
LOCKED_INITIALIZER = "FVector::ZeroVector"
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def world_location_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*FVector\s+WorldLocation\b",
        compact,
    ) is not None
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover sibling Label / bLocked / bSeeking /
    # LockAlpha, leftover HUD snapshot WeaponLine /
    ...
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"float\s+WorldLocation\b", compact):
        return False
    if re.search(r"FString\s+WorldLocation\b", compact):
        return False
    if re.search(r"bool\s+WorldLocation\b", compact):
        return False
    if re.search(r"int32\s+WorldLocation\b", compact):
        return False
    if re.search(r"FName\s+WorldLocation\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*WorldLocation\b", compact):
        return False
    if WORLD_BARE_RE.search(compact) and (
        WORLD_LOCKED_RE.search(compact) is None
    ):
        return False
    init_match = WORLD_INIT_RE.search(compact)
    if init_match is None:
        return False
    if init_match.group(1).strip() != LOCKED_INITIALIZER:
        return False
    if WORLD_LOCKED_RE.search(compact) is None:
        return False
    if world_location_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FVector WorldLocation\b", compact[index:]
        ):
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FVector WorldLocation\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FVector WorldLocation is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "WorldLocation"):
        raise AssertionError(
            "UPROPERTY clone landed on WorldLocation; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "WorldLocation"):
            raise AssertionError(
                f"{token} clone landed on WorldLocation; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgContactMarkWorldLocationFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("FVector WorldLocation"))
        ...
        self.assertIn("WorldLocation", LOCKED_DECL)
    ...
    def test_does_not_claim_leftover_headers_or_structs(self) -> None:
        ...
        self.assertIn(LOCKED_DECL, mission_types)
        with self.assertRaises(AssertionError) as raised:
        ...
        self.assertIn(LOCKED_DECL, map_assembly)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_sibling_label_does_not_satisfy_world_location(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_LABEL}\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn("WorldLocation = TEXT", compact_origin)
        self.assertNotIn("WorldLocation = NAME_None", compact_origin)
        self.assertNotIn("WorldLocation = false", compact_origin)
        self.assertNotIn("WorldLocation = true", compact_origin)
        self.assertNotIn("WorldLocation = FVector::OneVector", compact_origin)
        self.assertNotIn("WorldLocation = FVector::Zero()", compact_origin)
        self.assertNotIn("WorldLocation = 0.f", compact_origin)
        self.assertNotIn("WorldLocation = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("WorldLocation = " + forty, compact_origin)
        self.assertNotIn("WorldLocation = " + eighty, compact_origin)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tFString WorldLocation;\n",
            "\tint32 WorldLocation;\n",
            "\tbool WorldLocation;\n",
            "\tFVector WorldLocation = " + forty + ";\n",
            "\tFVector WorldLocation = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    world_location_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(world_location_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-contact-mark WorldLocation field decl contract "
