# THIS IS leftover-safe ASkyguardGunner CpgKneeRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgKneeRight;`
# (no initializer).
...
# Stop BEFORE sibling CpgCanopyBow as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeLeft, CpgCanopyBow, Boom,
# GunnerCamera, CpgDash.
# Do NOT claim leftover CpgSeatBack / CpgCockpit /
# CpgStation (in-flight siblings). CpgKneeLeft and
# CpgSeatPan are sibling in-flight slots.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgKneeRight;`
...
# CpgKneeRight fail-closed if LOCKED_DECL is leftover
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgKneeRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgKneeRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgKneeRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgKneeRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgKneeRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgKneeRight;"
...
STOP_BEFORE_CANOPY_BOW = "CpgCanopyBow"
...
CPG_KNEE_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgKneeRight\s*;"
)
CPG_KNEE_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgKneeRight\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # initializer on CpgKneeRight as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_KNEE_RIGHT_DECL_RE.search(compact) is None:
        return False
    if CPG_KNEE_RIGHT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_CANOPY_BOW):
            break
    ...
    if has_identifier(window, SIBLING_CPG_CANOPY_BOW):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_CANOPY_BOW}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgKneeRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgKneeRight "
...
class GunnerCpgKneeRightFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgKneeRight"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_CANOPY_BOW), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_CANOPY_BOW),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgKneeRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgKneeRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_knee_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgKneeRight;", compact_origin)
        self.assertNotIn("CpgKneeRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_knee_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgKneeRight"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgKneeRight;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgKneeRights;\n",
            "\tint32 CpgKneeRight = 140;\n",
            "\tbool CpgKneeRight = true;\n",
            "\tfloat CpgKneeRight = " + forty + ";\n",
            "\tfloat CpgKneeRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_canopy_bow_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(TARGET_WRONG_CANOPY_BOW, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertFalse(has_identifier(section, SIBLING_CPG_CANOPY_BOW))
        self.assertTrue(has_identifier(leaked, SIBLING_CPG_DASH))
        self.assertFalse(has_identifier(section, SIBLING_CPG_DASH))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgKneeRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_knee_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
