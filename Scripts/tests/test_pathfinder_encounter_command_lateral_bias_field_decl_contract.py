...
# then `float CommandLateralBiasCm = 850.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Boss|Encounter|Commands`. EditAnywhere IS present.
# BlueprintReadWrite IS present. ClampMin IS `"0.0"`. Default IS `= 850.f`.
# Clone wrap ClampMin WAS `"0.0"` leftover Amplitude Route. THIS slot
# ClampMin IS `"0.0"` (NO spaces around =). Category IS Commands
# (NOT Route, NOT Safety, NOT Telegraph). Fail-closed if wrap still
# has leftover Route / Safety / Telegraph Category.
# 850.f is NOT Harbor 40 or 80. leftover analog LockWindowSpeedCmPerSecond
# = 850.f is a different identifier; fail-closed if this test still
# asserts LockWindowSpeed as LOCKED_DECL. Default is `= 850.f` not
# leftover Amplitude `= 850.f`.
...
# It is NOT leftover analog CommandLateralBiasCm = 850.f
# (clone leftover Route #1663). Leave sibling BreakClimbCm = 260.f
# uncontracted.
...
# if ClampMin is missing, if ClampMin is still `"100.0"`, if the default
# is not `= 850.f`, if LOCKED_DECL is still CommandLateralBiasCm
# or LockWindowSpeedCmPerSecond, if Category is
# Skyguard|Apache / Skyguard|Combat / Skyguard|Theater /
# Skyguard|MissionMap / leftover Route
# `Skyguard|Boss|Encounter|Commands` / leftover Safety /
# leftover Telegraph / Skyguard|Audio|Acceptance /
# Skyguard|Audio|Development / still Category="Skyguard" without
# leftover-safe unique Category path `Skyguard|Boss|Encounter|Commands`.
...
# RecoilPitch = 0.92f.
# Do NOT claim sibling BreakClimbCm = 260.f.
# Do NOT claim leftover Safety MaxLateralOffsetCm /
# MaxHeightFromOriginCm / MaxSimulationStepSeconds /
# MaxSimulationSubsteps.
# Do NOT claim Telegraph interval fields / MaxTelegraphsPerEncounter.
...
# Category="Skyguard|Boss|Encounter|Commands", meta=(ClampMin="0.0")
# wrapping `float CommandLateralBiasCm = 850.f;`
# — RETARGET hard: wrap is EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Commands", meta=(ClampMin="0.0")
# (NO spaces around =). Fail-closed if wrap still has leftover
# Route / Safety / Telegraph Category.
# LOCKED_DECL is `float CommandLateralBiasCm = 850.f;`
...
# and NOT leftover analog LockWindowSpeedCmPerSecond = 850.f
# (different identifier) and NOT leftover CriticalSpeedCmPerSecond
# = 700.f and NOT leftover CommandLateralBiasCm = 850.f
...
# and NOT leftover analog `float BreakClimbCm = 260.f;`.
...
# or leftover Route `Skyguard|Boss|Encounter|Commands`
# or leftover Safety / leftover Telegraph
# or Category="Skyguard" without |Boss|Encounter|Commands.
# Fail-closed if wrap is VisibleAnywhere or BlueprintReadOnly
# (must be EditAnywhere + BlueprintReadWrite).
# Fail-closed if ClampMin is missing, if ClampMin is still `"100.0"`,
# or default is not 850.f.
...
TARGET = "float CommandLateralBiasCm = 850.f;"
TARGET_WRONG_BARE = "float CommandLateralBiasCm;"
TARGET_WRONG_INIT = "float CommandLateralBiasCm = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float CommandLateralBiasCm = true;"
TARGET_WRONG_ZERO = "float CommandLateralBiasCm = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CommandLateralBiasCm;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CommandLateralBiasCm;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CommandLateralBiasCm;"
...
STOP_BEFORE_SKYLINE_STYLE = "BreakClimbCm"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_command_lateral_bias"
...
TARGET_WRONG_BREAK_CLIMB = "float BreakClimbCm = 260.f;"
...
TARGET_WRONG_SAFETY_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Boss|Encounter|Safety")'
)
TARGET_WRONG_TELEGRAPH_WRAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
...
SIBLING_MAX_SIM_SUBSTEPS = "MaxSimulationSubsteps"
...
COMMAND_LATERAL_BIAS_DECL_RE = re.compile(
    r"float\s+CommandLateralBiasCm\s*=\s*850\.f\s*;"
)
COMMAND_LATERAL_BIAS_BARE_RE = re.compile(
    r"float\s+CommandLateralBiasCm\s*;"
)
AMPLITUDE_LEFTOVER_DECL_RE = re.compile(
    r"float\s+IngressSwayAmplitudeCm\s*=\s*320\.f\b"
)
BREAK_CLIMB_LEFTOVER_DECL_RE = re.compile(
    r"float\s+BreakClimbCm\s*=\s*260\.f\b"
)
MAX_LATERAL_LEFTOVER_DECL_RE = re.compile(
    r"float\s+MaxLateralOffsetCm\s*=\s*3600\.f\b"
)
MAX_HEIGHT_LEFTOVER_DECL_RE = re.compile(
    r"float\s+MaxHeightFromOriginCm\s*=\s*1200\.f\b"
)
MAX_SIM_STEP_LEFTOVER_DECL_RE = re.compile(
    r"float\s+MaxSimulationStepSeconds\s*=\s*0\.05f\b"
)
MAX_SUBSTEPS_LEFTOVER_DECL_RE = re.compile(
    r"int32\s+MaxSimulationSubsteps\s*=\s*8\b"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float CommandLateralBiasCm = 850.f;`.
    ...
    # leftover analog ClampMin="100.0",
    ...
    # CommandLateralBiasCm without the 850.f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if COMMAND_LATERAL_BIAS_DECL_RE.search(compact) is None:
        return False
    if COMMAND_LATERAL_BIAS_BARE_RE.search(compact):
        return False
    ...
    if AMPLITUDE_LEFTOVER_DECL_RE.search(compact):
        return False
    if BREAK_CLIMB_LEFTOVER_DECL_RE.search(compact):
        return False
    if MAX_LATERAL_LEFTOVER_DECL_RE.search(compact):
        return False
    if MAX_HEIGHT_LEFTOVER_DECL_RE.search(compact):
        return False
    if MAX_SIM_STEP_LEFTOVER_DECL_RE.search(compact):
        return False
    if MAX_SUBSTEPS_LEFTOVER_DECL_RE.search(compact):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CommandLateralBiasCm\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CommandLateralBiasCm "
...
class PathfinderEncounterCommandLateralBiasFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CommandLateralBiasCm"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CommandLateralBiasCm"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CommandLateralBiasCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_command_lateral_bias_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Commands"', section)
        ...
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_SAFETY_WRAP)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, TARGET_WRONG_TELEGRAPH_WRAP)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOCK_WINDOW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_AMPLITUDE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BREAK_CLIMB)
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Commands"', specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float CommandLateralBiasCm;",
            compact_origin.split("CommandLateralBiasCm = 850.f;")[0]
            + compact_origin.split("CommandLateralBiasCm = 850.f;")[-1]
            if "CommandLateralBiasCm = 850.f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_command_lateral_bias_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CommandLateralBiasCm"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("850.f", LOCKED_DECL)
        ...
        self.assertNotIn("320.f", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_HEIGHT)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat CommandLateralBiasCms = 850.f;\n",
            "\tint32 CommandLateralBiasCm = 320;\n",
            "\tFName CommandLateralBiasCm = NAME_None;\n",
            "\tfloat CommandLateralBiasCm = " + forty + ";\n",
            "\tfloat CommandLateralBiasCm = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
            f"\t{TARGET_WRONG_APPROACH}\n",
            f"\t{TARGET_WRONG_LOCK_WINDOW}\n",
            f"\t{TARGET_WRONG_AMPLITUDE}\n",
            f"\t{TARGET_WRONG_BREAK_CLIMB}\n",
            f"\t{TARGET_WRONG_LATERAL}\n",
            f"\t{TARGET_WRONG_MAX_HEIGHT}\n",
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Boss|Encounter|Commands"', origin)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Route"', origin)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Safety"', origin)
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Telegraph"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', apache_specs)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', theater_specs)
        self.assertIn('Category="Skyguard|Boss|Encounter|Commands"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', budget_specs)
        leftover_acceptance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', acceptance_specs)
        leftover_perf = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', perf_specs)
        ...
        self.assertNotIn('Category="Skyguard|Boss|Encounter|Commands"', development_specs)
        leftover_instance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CommandLateralBiasCm"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CommandLateralBiasCm"))
        self.assertTrue(has_identifier(section, "CommandLateralBiasCm"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_command_lateral_bias_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
