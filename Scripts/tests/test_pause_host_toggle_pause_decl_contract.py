...
# then BARE `void TogglePause();`
# (declaration-only, no inline body required).
# THIS IS leftover-safe unique analog Skyguard|Pause UFUNCTION.
# Category IS `Skyguard|Pause`. BlueprintCallable IS present.
# KEEP BlueprintCallable. const IS absent. Return type IS void.
# No args. Declaration-only.
# Word-boundary match TogglePause vs IsPauseMenuVisible.
# Identifier TogglePause is NOT leftover analog
# pause-host-fail-closed bulk #123 (behavioral; wrap Category
# Pause UFUNCTION declaration only) and NOT leftover analog
# IsPauseMenuVisible.
# Fail-closed if the UFUNCTION or decl is missing
# or renamed, if Category is missing, if specifiers
# stay BlueprintPure, if const is present, if args are
# present, if the return type is not void, or if Category is
...
# Do NOT claim sibling BindPauseInput (no UFUNCTION) as this
# slot (leave it uncontracted).
# Do NOT claim sibling BlueprintPure IsPauseMenuVisible.
# Do NOT retarget to IsPauseMenuVisible / BindPauseInput /
# PauseWidgetClass.
...
# Production -> Skyguard|Pause. KEEP BlueprintCallable.
# RETARGET void EnsureDefaultEntries(); -> void TogglePause();
...
# Combat|Sortie -> Skyguard|Pause. KEEP BlueprintCallable
# not BlueprintPure.
...
# LOCKED_DECL is `void TogglePause();`
...
# `bool IsPauseMenuVisible() const;` and NOT
# `int32 GetActivationCount() const;` and NOT leftover
...
TARGET_WRONG_TRUE = "bool TogglePause = true;"
TARGET_WRONG_ZERO = "bool TogglePause = 0.f;"
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        f"{prefix}test_pause_host_is_pause_menu_visible_decl_contract.py",
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_pause_host_is_pause_menu_visible_decl_contract.py",
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+TogglePause\s*\(\s*\)\s*const\b", compact):
        return False
    if re.search(r"\bbool\s+TogglePause\b", compact):
        return False
    if re.search(r"\bfloat\s+TogglePause\b", compact):
        return False
    if re.search(r"\bint32\s+TogglePause\b", compact):
        return False
    if re.search(
        r"\bvoid\s+TogglePause\s*\(\s*[^)]+\s*\)",
        compact,
...
class PauseHostTogglePauseDeclContractTests(
    unittest.TestCase
):
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_IS_PAUSE_VISIBLE)
    ...
    def test_does_not_claim_sibling_bind_pause_input_or_neighbors(
        ...
        self.assertTrue(has_identifier(section, SIBLING_IS_PAUSE_MENU_VISIBLE))
        self.assertTrue(has_identifier(section, SIBLING_BIND_PAUSE_INPUT))
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_IS_PAUSE_MENU_VISIBLE))
        ...
        self.assertFalse(
            has_identifier(TARGET_WRONG_IS_PAUSE_VISIBLE, "TogglePause")
        )
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_IS_PAUSE_VISIBLE)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "TogglePause")
        )
        self.assertTrue(
            has_identifier(section, "TogglePause")
        )
    ...
    def test_word_boundary_toggle_pause_vs_is_pause_menu_visible(
        self,
    ) -> None:
        self.assertTrue(has_identifier("void TogglePause();", "TogglePause"))
        self.assertFalse(
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_IS_PAUSE_VISIBLE)
        self.assertFalse(has_identifier(LOCKED_DECL, "IsPauseMenuVisible"))
        self.assertFalse(has_identifier(LOCKED_DECL, "BindPauseInput"))
        self.assertFalse(has_identifier(LOCKED_DECL, "PauseWidgetClass"))
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "TogglePause"), section)
        self.assertTrue(
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertNotIn("IsPauseMenuVisible", locked_only)
