# THIS IS leftover-safe ASkyguardRadarNode::IsDestroyed.
# origin/main form: split-line
# UFUNCTION(BlueprintPure, Category="Skyguard|Campaign")
# then `bool IsDestroyed() const { return bDead; }`.
# THIS IS UFUNCTION(BlueprintPure) ONLY. It is NOT
# BlueprintCallable. Category IS `Skyguard|Campaign`.
...
# Do NOT contract MaxHealth / Health.
# Do NOT contract ApplyDamage / ResetNode
# (sibling workers).
...
TARGET = "bool IsDestroyed() const;"
TARGET_INLINE = "bool IsDestroyed() const { return bDead; }"
TARGET_WRONG_BARE = "bool IsDestroyed();"
TARGET_WRONG_NO_CONST = "bool IsDestroyed() { return bDead; }"
TARGET_WRONG_VOID = "void IsDestroyed() const;"
TARGET_WRONG_FLOAT = "float IsDestroyed() const;"
TARGET_WRONG_RENAME = "bool IsDead() const;"
...
UNCONTRACTED_MAX_HEALTH = "MaxHealth"
UNCONTRACTED_HEALTH = "Health"
...
IS_DESTROYED_SIGNATURE_RE = re.compile(
    r"\bbool\s+IsDestroyed\s*\(\s*\)\s*const\s*(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `bool IsDestroyed() const`. Accept origin/main inline
    # body without locking the body, plus a trailing-semicolon
    # form. Do not accept a missing const, a void/float
    # rename, extra params, or sibling ApplyDamage / ResetNode
    # when origin/main has `bool IsDestroyed() const`.
    del declaration
    compact = collapsed(region)
    if IS_DESTROYED_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+IsDestroyed\b", compact):
        return False
    if re.search(r"\bfloat\s+IsDestroyed\b", compact):
        return False
    if re.search(r"\bbool\s+IsDestroyed\s*\(\s*[^)]+\s*\)", compact):
        return False
    return True
...
def count_one_declaration(region: str, declaration: str) -> int:
    del declaration
    compact = collapsed(region)
    return len(IS_DESTROYED_SIGNATURE_RE.findall(compact))
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+IsDestroyed\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for bool IsDestroyed is missing from "
...
class RadarNodeIsDestroyedDeclContractTests(unittest.TestCase):
    def test_radar_node_class_exists(self) -> None:
        ...
        self.assertIn("IsDestroyed", section)
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertNotIn("IsDestroyed", section)
        for token in protected_component_tokens()[:1]:
            self.assertNotIn(token, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("IsDestroyed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_is_destroyed_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertIn("IsDestroyed()const { return bDead; }", compact_origin)
        self.assertNotIn("void IsDestroyed", compact_origin)

    def test_is_destroyed_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool IsDestroyed()"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_signature_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tbool Destroyed() const { return bDead; }\n",
            "\tbool IsDestroyedd() const { return bDead; }\n",
            "\tvoid ResetNode();\n",
    ...
    def test_callable_instead_of_pure_fails_closed(self) -> None:
        ...
        section = public_section(callable_wrap)
        specifiers = attached_ufunction_specifiers(section)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "public:\n\tbool\n\tIsDestroyed() const { return bDead; }\n",
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            ...
            f"\tfloat {UNCONTRACTED_MAX_HEALTH} = 160.f;\n"
            f"\tfloat {UNCONTRACTED_HEALTH} = 160.f;\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar-node IsDestroyed decl contract "
