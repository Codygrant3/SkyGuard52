...
# BoundsMinimum FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FVector BoundsMinimum = FVector::ZeroVector;
# one-line / split-line UPROPERTY wraps; origin/main has NO Category).
# DEFAULT IS `= FVector::ZeroVector` NOT `= false` / `= true` / `= 0`.
# Fail closed if the type or decl is missing, renamed, Category= is
# added, or default is not FVector::ZeroVector. Parse STRUCT public
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# BoundsMinimum FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly FVector BoundsMinimum = FVector::ZeroVector; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeVisibleAudit
# BoundsMinimum FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# FVector BoundsMinimum = FVector::ZeroVector; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# BoundsMinimum FIELD (declaration-only). It is
...
TARGET = "FVector BoundsMinimum = FVector::ZeroVector;"
TARGET_WRONG = "FVector BoundsMinimum = FVector::OneVector;"
LOCKED_DECL = TARGET
...
def alternate_bool_defaults(declaration: str) -> tuple[str, ...]:
    ...
    if "= false;" in declaration:
        ...
        forms.append(declaration.replace("= false;", "= FVector::ZeroVector;"))
    if "= true;" in declaration:
        forms.append(declaration.replace("= true;", "= false;"))
        forms.append(declaration.replace("= true;", "= FVector::ZeroVector;"))
    if "= 0;" in declaration:
        forms.append(declaration.replace("= 0;", "= 1;"))
        forms.append(declaration.replace("= 0;", "= FVector::ZeroVector;"))
    if "= 1;" in declaration:
        forms.append(declaration.replace("= 1;", "= 0;"))
    if "= FVector::OneVector;" in declaration:
        forms.append(declaration.replace("= FVector::OneVector;", "= FVector::ZeroVector;"))
    if "= FVector::ZeroVector;" in declaration:
        forms.append(declaration.replace("= FVector::ZeroVector;", "= FVector::ZeroVector;"))
    unique: list[str] = []
    for form in forms:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FVector\s+BoundsMinimum\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FVector BoundsMinimum is missing from "
...
class LandscapeVisibleAuditBoundsMinimumFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("BoundsMinimum", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_bounds_minimum_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_bounds_minimum_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FVector BoundsMinimum"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith("ZeroVector;"), LOCKED_DECL)
        self.assertTrue(has_one_declaration(section, TARGET), section)
        self.assertFalse(has_one_declaration(section, TARGET_WRONG))
        self.assertNotIn("FVector BoundsMinimum = FVector::OneVector;", section)
        ...
        self.assertIn("BoundsMinimum", LOCKED_DECL)
        ...
        self.assertNotIn("ESkyguardMission05ProtectedTarget", LOCKED_DECL)
        self.assertTrue(
            LOCKED_DECL.startswith("FVector "),
            LOCKED_DECL,
        )
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("BoundsMinimum", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FVector BoundsMinimum"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("BoundsMinimum", locked_only)
    ...
    def test_contract_is_target_field_declaration_only(
        ...
        self.assertTrue(LOCKED_DECL.startswith("FVector BoundsMinimum"))
        self.assertIn("BoundsMinimum", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.endswith("ZeroVector;"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("GetProtectedGroup", LOCKED_DECL)
        self.assertNotIn("FSkyguardMission10ProtectedRuntime", LOCKED_DECL)
        self.assertIn(UPROPERTY_MISSION10, section)

    def test_sibling_director_fields_do_not_satisfy_bounds_minimum(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
