...
# or lock the IsCanopyGlassCracked body.
# origin/main is inline
# (`bool IsCanopyGlassCracked() const`
# with `{ return bCanopyGlassCracked; }` possibly on the
...
# locking the body. The inline return of the canopy-glass
...
# leftover apache-own-ship-systems #96c5 (that leftover
# locks ESkyguardApacheSystem enumerators). Do not lock
# ESkyguardApacheSystem values. Stay off leftover #96c5
# methods ApplySystemHit / ApplyHit / IsSensorLive /
# IsThermalAvailable / IsSensorViewActive. This is not
...
# apache-cpg-feel #8951. Stay off leftover
...
# AreEnginesDown siblings in this wave (do not create
# those files). Stay off remaining unused neighbors
# IsChinTurretDown / IsRotorDown / IsSystemDown /
# FindNearestLiveSystem / GetSensorQuality /
# GetChinSlewScale / GetChinFireScale /
# GetEnginePowerScale / GetRotorPowerScale /
# GetRotorRPM. Leftover briefing / debrief widget
...
# leftover drafts #56–#64, leftover isolated-test drafts
# #107–#414, leftover skyline style HarborIndustrial
...
# SetFirstPersonInterior, SetDirectFlightInput,
# GetForwardSpeed, ApplyDamage, GetDamageFraction,
# AreEnginesDown, ApplySystemHit, ApplyHit,
...
# leftover apache-cpg-feel #8951, leftover CPG HUD /
# sight HUD, leftover drafts #56–#64, leftover
# isolated-test drafts #107–#414, leftover ApacheSystem
# enum values, leftover Harbor clocks, leftover skyline
...
# SetFirstPersonInterior / SetDirectFlightInput /
# GetForwardSpeed / ApplyDamage / GetDamageFraction /
# AreEnginesDown siblings, leftover
...
# SetFirstPersonInterior / SetDirectFlightInput /
# GetForwardSpeed / ApplyDamage / GetDamageFraction /
# AreEnginesDown siblings in this wave, leftover
...
# leftover apache-cpg-feel, leftover isolated-test
# drafts #107–#414, and sibling Apache neighbors stay
# sibling-only.
LOCKED_SCRIPTS = (
...
# .cpp IsCanopyGlassCracked body / invented INDEX_NONE
# stay unlocked. Do not invent INDEX_NONE or lock the
# cpp body. origin/main inline return of the canopy-glass
# flag is accepted as presence, not a locked
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::IsCanopyGlassCracked",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheIsCanopyGlassCrackedDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, IS_CANOPY_GLASS_CRACKED),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, IS_CANOPY_GLASS_CRACKED)
        self.assertIn("IsCanopyGlassCracked", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, IS_CANOPY_GLASS_CRACKED))

    def test_missing_is_canopy_glass_cracked_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, IS_CANOPY_GLASS_CRACKED)
        self.assertIn("IsCanopyGlassCracked", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, IS_CANOPY_GLASS_CRACKED)
        self.assertIn("IsCanopyGlassCracked", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("UFUNCTION", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("Category", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("BlueprintCallable", IS_CANOPY_GLASS_CRACKED)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, IS_CANOPY_GLASS_CRACKED)
        self.assertIn("IsCanopyGlassCracked", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tbool IsCanopyGlassCracked();\n"
        wrong_return_void = "\tvoid IsCanopyGlassCracked() const;\n"
        wrong_return_float = "\tfloat IsCanopyGlassCracked() const;\n"
        added_arg = "\tbool IsCanopyGlassCracked(bool bReady) const;\n"
        leftover_issue = (
            "\tvoid IssuePilotCommand(ESkyguardPilotCommand Command);\n"
        )
        ...
        leftover_engines = "\tbool AreEnginesDown() const;\n"
        leftover_sensor = "\tbool IsSensorLive() const;\n"
        leftover_thermal = "\tbool IsThermalAvailable() const;\n"
        leftover_sensor_view = "\tbool IsSensorViewActive() const;\n"
        ...
        for region in (
            ...
            leftover_sensor,
            leftover_thermal,
            leftover_sensor_view,
            leftover_chin_down,
            leftover_rotor_down,
        ):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, IS_CANOPY_GLASS_CRACKED)
            self.assertIn("IsCanopyGlassCracked", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_is_canopy_glass_cracked_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, IS_CANOPY_GLASS_CRACKED))
        self.assertEqual(
            declaration_count(section, IS_CANOPY_GLASS_CRACKED),
            1,
        )
        self.assertTrue(
            IS_CANOPY_GLASS_CRACKED.startswith("bool "),
            IS_CANOPY_GLASS_CRACKED,
        )
        self.assertTrue(
            IS_CANOPY_GLASS_CRACKED.endswith(";"),
            IS_CANOPY_GLASS_CRACKED,
        )
        self.assertIn("IsCanopyGlassCracked()", IS_CANOPY_GLASS_CRACKED)
        self.assertIn(" const", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("INDEX_NONE", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("{", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("}", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return ", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return bCanopyGlassCracked", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("AimChinTurret", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("SetRotorPower", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("IssuePilotCommand", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetPilotCommand", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetPilotConfirmationsIssued", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetChinMuzzleLocation", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetChinTurret", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetRotorPowerScale", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("SetDirectFlightInput", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetForwardSpeed", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("ApplyDamage", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetDamageFraction", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("AreEnginesDown", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("IsSensorLive", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("IsThermalAvailable", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("IsSensorViewActive", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("ApplySystemHit", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("ApplyHit", IS_CANOPY_GLASS_CRACKED)
    ...
    def test_declaration_accepts_origin_main_inline_and_prototype(
        ...
        self.assertTrue(
            has_declaration(one_line_prototype, IS_CANOPY_GLASS_CRACKED)
        )
        self.assertEqual(
            require_declaration(one_line_prototype, IS_CANOPY_GLASS_CRACKED),
            IS_CANOPY_GLASS_CRACKED,
        )
        exact_inline = (
            "bool IsCanopyGlassCracked() "
            "const { return bCanopyGlassCracked; }"
        )
        self.assertTrue(has_declaration(exact_inline, IS_CANOPY_GLASS_CRACKED))
        self.assertEqual(
            require_declaration(exact_inline, IS_CANOPY_GLASS_CRACKED),
            IS_CANOPY_GLASS_CRACKED,
        )
        self.assertEqual(
            declaration_count(exact_inline, IS_CANOPY_GLASS_CRACKED),
            1,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{IS_CANOPY_GLASS_CRACKED}\n}}\n"
        self.assertTrue(has_declaration(one_line, IS_CANOPY_GLASS_CRACKED))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, IS_CANOPY_GLASS_CRACKED),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("return ", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return bCanopyGlassCracked", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return Other", IS_CANOPY_GLASS_CRACKED)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{IS_CANOPY_GLASS_CRACKED}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", IS_CANOPY_GLASS_CRACKED)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_is_canopy_glass_cracked_body(self) -> None:
        locked_only = f"{IS_CANOPY_GLASS_CRACKED}\n"
        self.assertNotIn("{", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("}", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return ", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return bCanopyGlassCracked", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return bCanopyGlassCracked", locked_only)
        self.assertNotIn(
            "ASkyguardApacheAircraft::IsCanopyGlassCracked",
            IS_CANOPY_GLASS_CRACKED,
        )
        self.assertNotIn("SkyguardApacheAircraft.cpp", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("return true", IS_CANOPY_GLASS_CRACKED)

    def test_contract_does_not_relock_mount_getters(self) -> None:
        locked_only = f"{IS_CANOPY_GLASS_CRACKED}\n"
        for token in MOUNT_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetGunnerMount", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetEyeMount", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetWeaponMount", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetChinTurret", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetPilotMount", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetSensorTurret", IS_CANOPY_GLASS_CRACKED)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{IS_CANOPY_GLASS_CRACKED}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("GetChinMuzzleLocation", IS_CANOPY_GLASS_CRACKED)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("SetOrbitFocus", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("FaceWorldLocation", IS_CANOPY_GLASS_CRACKED)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("SetSensorView", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("SetFirstPersonInterior", IS_CANOPY_GLASS_CRACKED)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("ESkyguardApacheSystem", IS_CANOPY_GLASS_CRACKED)
    ...
    def test_contract_does_not_relock_leftover_pilot_command_roster(
        ...
        self.assertNotIn("Pursuit", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("OrbitLeft", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("AttackRun", IS_CANOPY_GLASS_CRACKED)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CANOPY_GLASS_CRACKED)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("USkyguardBriefingWidget", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            IS_CANOPY_GLASS_CRACKED,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, IS_CANOPY_GLASS_CRACKED)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("ESkyguardMissionSkylineStyle", IS_CANOPY_GLASS_CRACKED)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{IS_CANOPY_GLASS_CRACKED}\n"
        self.assertEqual(
            require_declaration(locked_only, IS_CANOPY_GLASS_CRACKED),
            IS_CANOPY_GLASS_CRACKED,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache IsCanopyGlassCracked contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, IS_CANOPY_GLASS_CRACKED.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache IsCanopyGlassCracked contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, IS_CANOPY_GLASS_CRACKED)

    def test_contract_is_is_canopy_glass_cracked_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return bCanopyGlassCracked", IS_CANOPY_GLASS_CRACKED)
        self.assertNotIn("{", IS_CANOPY_GLASS_CRACKED)
        self.assertTrue(IS_CANOPY_GLASS_CRACKED.startswith("bool "))
        self.assertTrue(IS_CANOPY_GLASS_CRACKED.endswith(";"))
        self.assertIn(" const", IS_CANOPY_GLASS_CRACKED)
        self.assertIn(UFUNCTION_APACHE, section)
