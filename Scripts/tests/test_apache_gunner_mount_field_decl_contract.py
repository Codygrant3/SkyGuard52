# THIS IS leftover-safe ASkyguardApacheAircraft.GunnerMount.
# Type is TObjectPtr<USceneComponent>, NOT UStaticMeshComponent.
# NOT leftover analog apache-mount-fail-closed, NOT leftover
# analog apache-hull-collider, NOT leftover analog
# apache-aim-chin-turret UFUNCTION, NOT leftover ChinTurret,
# NOT leftover PilotMount / EyeMount / WeaponMount,
# NOT leftover AircraftRoot, NOT leftover HellfireLeft /
# HellfireRight / HydraLeft / HydraRight mesh slots.
...
# then BARE `TObjectPtr<USceneComponent> GunnerMount;` (no initializer).
...
# Stop BEFORE sibling PilotMount as a claimed slot.
# Do not claim PilotMount / EyeMount / WeaponMount /
# HellfireLeft / HellfireRight / HydraLeft / HydraRight /
# AircraftRoot / ChinTurret / HullCollider / RotorPower.
...
# `TObjectPtr<USceneComponent> GunnerMount;` NOT
# `FName WeatherIdentity;`. This slot also has NO
...
LOCKED_DECL = "TObjectPtr<USceneComponent> GunnerMount;"
...
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> GunnerMount = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> GunnerMount = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> GunnerMount = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> GunnerMount = 0.f;"
TARGET_WRONG_FLOAT = "float GunnerMount;"
TARGET_WRONG_FNAME = "FName GunnerMount;"
TARGET_WRONG_INT = "int32 GunnerMount = 0;"
TARGET_WRONG_BOOL = "bool GunnerMount = true;"
...
STOP_BEFORE_PILOT_MOUNT = "PilotMount"
...
TARGET_WRONG_STATIC_MESH = "TObjectPtr<UStaticMeshComponent> GunnerMount;"
...
LEFTOVER_AIM_CHIN_TURRET_SCRIPT = (
    "Scripts/tests/test_apache_aim_chin_turret"
    "_decl_contract.py"
)
GET_GUNNER_MOUNT = "GetGunnerMount"
GET_PILOT_MOUNT = "GetPilotMount"
...
GUNNER_MOUNT_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*GunnerMount\s*;"
)
GUNNER_MOUNT_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*GunnerMount\s*="
)
GUNNER_MOUNT_FLOAT_RE = re.compile(r"float\s+GunnerMount\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if GUNNER_MOUNT_INIT_RE.search(compact):
        return False
    if GUNNER_MOUNT_FLOAT_RE.search(compact):
        return False
    if GUNNER_MOUNT_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+GunnerMount\b", compact):
        return False
    if re.search(r"int32\s+GunnerMount\b", compact):
        return False
    if re.search(r"bool\s+GunnerMount\b", compact):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*TObjectPtr<USceneComponent>\s*GunnerMount\s*;",
            after,
        )
        if depth == 0 and field is not None:
            ...
            if STOP_BEFORE_PILOT_MOUNT in window:
                raise AssertionError(
                    "claimed window includes sibling PilotMount"
                )
            if TARGET_WRONG_AIRCRAFT_ROOT in window:
                raise AssertionError(
                    "claimed window is leftover AircraftRoot"
                )
            if TARGET_WRONG_CHIN_TURRET in window:
                raise AssertionError(
                    "claimed window is leftover ChinTurret"
                )
            if TARGET_WRONG_STATIC_MESH in window:
                raise AssertionError(
                    "claimed window is leftover UStaticMeshComponent"
                )
            if TARGET_WRONG_HELLFIRE_LEFT in window:
                raise AssertionError(
                    "claimed window is leftover HellfireLeft"
                )
            if TARGET_WRONG_HELLFIRE_RIGHT in window:
                raise AssertionError(
                    "claimed window is leftover HellfireRight"
                )
            if TARGET_WRONG_HYDRA_LEFT in window:
                raise AssertionError(
                    "claimed window is leftover HydraLeft"
                )
            if TARGET_WRONG_HYDRA_RIGHT in window:
                raise AssertionError(
                    "claimed window is leftover HydraRight"
                )
            return window
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> GunnerMount; is "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*GunnerMount\b",
            compact[index:],
        ):
...
class ApacheGunnerMountFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GunnerMount"), section)
        ...
        self.assertNotIn(SIBLING_HYDRA_LEFT, claimed)
        self.assertNotIn(SIBLING_HYDRA_RIGHT, claimed)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GunnerMount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_gunner_mount_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        ...
        self.assertNotIn("PilotMount", LOCKED_DECL)
        self.assertNotIn("EyeMount", LOCKED_DECL)
        self.assertNotIn("WeaponMount", LOCKED_DECL)
        self.assertNotIn("ChinTurret", LOCKED_DECL)
        self.assertNotIn("HellfireLeft", LOCKED_DECL)
        self.assertNotIn("HellfireRight", LOCKED_DECL)
        ...
        self.assertIn("GunnerMount", LOCKED_DECL)
    ...
    def test_locked_decl_requires_bare_pointer(self) -> None:
        ...
        self.assertNotIn("GunnerMount = nullptr", compact_origin)
        self.assertNotIn("GunnerMount = NAME_None", compact_origin)
        self.assertNotIn("GunnerMount = false", compact_origin)
        self.assertNotIn("GunnerMount = true", compact_origin)
        self.assertNotIn("GunnerMount = 0.f", compact_origin)
        self.assertNotIn("GunnerMount = 160.f", compact_origin)

    def test_gunner_mount_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> GunnerMount"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USceneComponent> GunnerMounts;\n",
            "\tTObjectPtr<UStaticMeshComponent> GunnerMount;\n",
            "\tTObjectPtr<USceneComponent> Root;\n",
            "\tFName WeatherIdentity;\n",
            "\tfloat GunnerMount = " + forty + ";\n",
            "\tfloat GunnerMount = " + eighty + ";\n",
    ...
    def test_does_not_contract_sibling_fields(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_PILOT_MOUNT, section)
        self.assertNotIn(STOP_BEFORE_PILOT_MOUNT, claimed)
        self.assertNotIn(SIBLING_AIRCRAFT_ROOT, claimed)
        self.assertNotIn(SIBLING_CHIN_TURRET, claimed)
    ...
    def test_claimed_window_stops_before_pilot_mount(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("GunnerMount", claimed)
    ...
    def test_parse_window_starts_at_protected_stops_before_private(self) -> None:
        ...
        self.assertIn("GunnerMount", class_body(header))
        self.assertIn("GunnerMount", protected)
        self.assertIn("GunnerMount", section)
        self.assertIn("GetGunnerMount", public)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache GunnerMount field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_mount_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_AIM_CHIN_TURRET_SCRIPT, LOCKED_SCRIPTS)
