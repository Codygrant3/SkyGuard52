...
# INDEX_NONE or lock SeawardExtentCm construction
# in the .cpp. This is a FIELD contract on
# ASkyguardMission01EnvironmentDirector. Keep
# SeawardExtentCm distinct from leftover
...
# bAutoLaunchAfterBriefing. Keep SeawardExtentCm
...
# BeachWidthCm / InlandExtentCm (in-flight this wave).
...
# one-line / split-line wraps. Locked 20000.f is
...
# meta=(ClampMin="1000.0")) is required as
...
# 20000.f value. Do not lock leftover
...
LOCKED_SEAWARD_EXTENT_VALUE = "20000.f"
...
# RouteCorridorHalfWidthCm / ShorelineLandOffsetCm /
# BeachWidthCm / InlandExtentCm / environment Root
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission01EnvironmentDirector::SeawardExtentCm",
    "SkyguardMission01EnvironmentDirector.cpp",
...
class Mission01SeawardExtentCmFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, SEAWARD_EXTENT_CM_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, SEAWARD_EXTENT_CM_FIELD))

    def test_missing_seaward_extent_cm_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission01EnvironmentDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{OCEAN_TILES_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('ClampMin="1000.0"', section)
        self.assertTrue(
            has_declaration(section, SEAWARD_EXTENT_CM_FIELD),
            section,
        )
        self.assertNotIn("UPROPERTY", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("EditAnywhere", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("EditInstanceOnly", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("BlueprintReadWrite", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("VisibleAnywhere", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("BlueprintReadOnly", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("Category", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("ClampMin", SEAWARD_EXTENT_CM_FIELD)
        ...
        self.assertIn('ClampMin="1000.0"', UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT)
        self.assertNotEqual(
            UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT,
            UPROPERTY_MISSION01_ENVIRONMENT,
        )
        ...
        for invented in INVENTED_UPROPERTY:
            ...
            self.assertNotIn(invented, SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tfloat* SeawardExtentCm;\n"
        )
        as_array = (
            "\tTArray<float> SeawardExtentCm;\n"
        )
        no_init = "\tfloat SeawardExtentCm;\n"
        int_type = "\tint32 SeawardExtentCm = 45000;\n"
        double_type = "\tdouble SeawardExtentCm = 20000.f;\n"
        wrong_init = "\tfloat SeawardExtentCm = 0.f;\n"
        wrong_scale = "\tfloat SeawardExtentCm = 40000.f;\n"
        scene = "\tTObjectPtr<USceneComponent> SeawardExtentCm;\n"
        static_mesh = "\tTObjectPtr<UStaticMeshComponent> SeawardExtentCm;\n"
        instanced = (
            "\tTObjectPtr<UInstancedStaticMeshComponent> SeawardExtentCm;\n"
        )
        weak = (
        ...
        leftover_seaward = f"\t{DISTRICT_LENGTH_CM_FIELD}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, SEAWARD_EXTENT_CM_FIELD)
            self.assertIn("SeawardExtentCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_seaward_extent_cm_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, SEAWARD_EXTENT_CM_FIELD),
            SEAWARD_EXTENT_CM_FIELD,
        )
        self.assertTrue(has_declaration(section, SEAWARD_EXTENT_CM_FIELD))
        self.assertEqual(
            declaration_count(section, SEAWARD_EXTENT_CM_FIELD),
            1,
        )
        self.assertTrue(
            SEAWARD_EXTENT_CM_FIELD.startswith("float SeawardExtentCm"),
            SEAWARD_EXTENT_CM_FIELD,
        )
        self.assertTrue(SEAWARD_EXTENT_CM_FIELD.endswith(";"), SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertIn(
            "float",
            SEAWARD_EXTENT_CM_FIELD,
        )
        self.assertIn("=", SEAWARD_EXTENT_CM_FIELD)
        self.assertIn(LOCKED_SEAWARD_EXTENT_VALUE, SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("TObjectPtr<", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("TArray<", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("TSoftObjectPtr", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("nullptr", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("INDEX_NONE", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("UFUNCTION", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("{", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("}", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("return ", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("OceanTiles", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("LandTiles", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("RouteExclusion", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("IsRouteExclusionSafe", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("bRouteExclusionValid", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("BeachTiles", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("InlandVegetationPCG", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("LandScatterBounds", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("ProductionLandscape", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("AuthoredPCGGraph", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("RouteLengthCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("RouteCorridorHalfWidthCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("ShorelineLandOffsetCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("BeachWidthCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("InlandExtentCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("DistrictLengthCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn(
            "bLicensedVegetationLibraryApproved",
            SEAWARD_EXTENT_CM_FIELD,
        )
        ...
        self.assertNotIn("Root", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetReadiness", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("Briefing", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("AudioDirector", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("RadioChatter", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("SortiePresentation", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("CampaignDefinition", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("MissionDefinition", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("bAutoInitialize", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetAircraft", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("ConfigureFromMission", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("AdvanceBriefing", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("SetAssetsReady", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("CanLaunch", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetElapsedSeconds", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetBriefingState", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetBriefingText", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetRadioChatter", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetPresentation", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("GetMissionTitle", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("AcknowledgeBriefing", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("LaunchSortie", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("HullCollider", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("OpticalTracker", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("WeaponServo", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("CountermeasurePod", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("RadarNode", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("HarborIndustrial", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("MaxIntegrity", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", SEAWARD_EXTENT_CM_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT} {SEAWARD_EXTENT_CM_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{SEAWARD_EXTENT_CM_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, SEAWARD_EXTENT_CM_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, SEAWARD_EXTENT_CM_FIELD),
            section,
    ...
    def test_assigned_seaward_extent_cm_does_not_satisfy(self) -> None:
        assigned = (
            "\tfloat SeawardExtentCm = 0.f;\n"
        )
        missing_init = "\tfloat SeawardExtentCm;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, SEAWARD_EXTENT_CM_FIELD))
        with self.assertRaises(AssertionError) as raised_missing:
            require_declaration(missing_init, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", str(raised_missing.exception))
        self.assertFalse(has_declaration(missing_init, SEAWARD_EXTENT_CM_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("SeawardExtentCm", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, SEAWARD_EXTENT_CM_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{SEAWARD_EXTENT_CM_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            SEAWARD_EXTENT_CM_FIELD.startswith("UFUNCTION"),
            SEAWARD_EXTENT_CM_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", SEAWARD_EXTENT_CM_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_seaward_extent_cm_cpp_body(self) -> None:
        locked_only = f"{SEAWARD_EXTENT_CM_FIELD}\n"
        self.assertNotIn("{", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("}", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("return ", SEAWARD_EXTENT_CM_FIELD)
        ...
        self.assertNotIn("return false", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("return true", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("CreateDefaultSubobject", SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_contract_does_not_relock_sibling_environment_fields(self) -> None:
        ...
        for token in SIBLING_ENVIRONMENT_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertTrue(
            SEAWARD_EXTENT_CM_FIELD.startswith("float SeawardExtentCm")
        )
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_get_readiness(self) -> None:
        ...
        self.assertNotIn(GET_READINESS, SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn(GET_READINESS_DECL, locked_only)
        self.assertNotIn(GET_READINESS_DECL, SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("CurrentIntegrity", SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in leftover_harbor_tokens():
            ...
            self.assertNotEqual(token, LOCKED_SEAWARD_EXTENT_VALUE)
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn(LOCKED_SEAWARD_EXTENT_VALUE, leftover_harbor_tokens())
    ...
    def test_harbor_adjacent_fields_are_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_VALUE_TOKENS:
            self.assertNotEqual(LOCKED_SEAWARD_EXTENT_VALUE, token)
            self.assertNotIn(token, leftover_harbor_tokens())

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{SEAWARD_EXTENT_CM_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, SEAWARD_EXTENT_CM_FIELD),
            SEAWARD_EXTENT_CM_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, SEAWARD_EXTENT_CM_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, SEAWARD_EXTENT_CM_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01EnvironmentDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 SeawardExtentCm field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{SEAWARD_EXTENT_CM_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, SEAWARD_EXTENT_CM_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                SEAWARD_EXTENT_CM_FIELD.lower(),
                "mission01 SeawardExtentCm contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, SEAWARD_EXTENT_CM_FIELD)

    def test_contract_is_seaward_extent_cm_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("SeawardExtentCm", SEAWARD_EXTENT_CM_FIELD)
        self.assertTrue(SEAWARD_EXTENT_CM_FIELD.endswith(";"))
        self.assertIn("=", SEAWARD_EXTENT_CM_FIELD)
        self.assertIn(LOCKED_SEAWARD_EXTENT_VALUE, SEAWARD_EXTENT_CM_FIELD)
        self.assertNotIn("TObjectPtr<", SEAWARD_EXTENT_CM_FIELD)
        self.assertIn(UPROPERTY_MISSION01_ENVIRONMENT_LAYOUT, section)

    def test_sibling_environment_fields_do_not_satisfy_seaward_extent_cm(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, SEAWARD_EXTENT_CM_FIELD))
            self.assertNotEqual(SEAWARD_EXTENT_CM_FIELD, leftover)
        self.assertIn(
            "Scripts/tests/test_mission01_ocean_tiles"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
    ...
    def test_sibling_director_fields_do_not_satisfy_seaward_extent_cm(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_preceding_comment_block_is_not_the_declaration(self) -> None:
        comment_only = (
            ...
            "\t * This comment is not the SeawardExtentCm declaration.\n"
            "\t */\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(comment_only, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn("SeawardExtentCm", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(comment_only, SEAWARD_EXTENT_CM_FIELD)
        )
        self.assertNotIn("float SeawardExtentCm = 20000.f;", comment_only)
    ...
    def test_contract_does_not_relock_leftover_layout_floats(
        ...
        for token in LEFTOVER_LAYOUT_FLOATS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, SEAWARD_EXTENT_CM_FIELD)
            self.assertNotEqual(token, SEAWARD_EXTENT_CM_FIELD)
        self.assertIn(LOCKED_SEAWARD_EXTENT_VALUE, SEAWARD_EXTENT_CM_FIELD)
        self.assertNotEqual(LOCKED_SEAWARD_EXTENT_VALUE, "45000.f")
        self.assertNotEqual(LOCKED_SEAWARD_EXTENT_VALUE, "2800.f")
        self.assertNotEqual(LOCKED_SEAWARD_EXTENT_VALUE, "5200.f")
        self.assertNotEqual(LOCKED_SEAWARD_EXTENT_VALUE, "1800.f")
        self.assertNotEqual(LOCKED_SEAWARD_EXTENT_VALUE, "18000.f")
        self.assertNotEqual(LOCKED_SEAWARD_EXTENT_VALUE, "7500.f")
