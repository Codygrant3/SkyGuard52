# THIS IS leftover-safe ASkyguardApacheAircraft TailRotor.
...
# then BARE `TObjectPtr<UStaticMeshComponent> TailRotor;`
# (no initializer).
...
# Stop BEFORE sibling ChinTurret as a claimed slot.
# Do NOT claim MainRotor / RotorMast / MainRotorCross /
# RotorPower / TailBoom / VerticalTail / HorizontalTail /
# StubWingLeft / StubWingRight / Fuselage / Nose / Canopy /
# SilhouetteMesh / AircraftRoot / HoverBobCentimeters /
# MaxIntegrity / CurrentIntegrity / HullCollider /
# PilotCanopy / ChinTurret.
# TailRotor is NOT leftover MainRotor / RotorMast /
# MainRotorCross / RotorPower / TailBoom / VerticalTail /
# HorizontalTail / StubWingRight.
# Do NOT refill leftover TailBoom / Nose / Canopy / Fuselage /
# AircraftRoot / SilhouetteMesh / HoverBobCentimeters /
# RotorPower / MaxIntegrity / CurrentIntegrity.
...
# leftover analog apache-set-rotor-power UFUNCTION,
# leftover analog apache-get-rotor-rpm UFUNCTION.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> TailRotor;`
# NOT `FName WeatherIdentity;`.
...
TARGET = "TObjectPtr<UStaticMeshComponent> TailRotor;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> TailRotor = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> TailRotor = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> TailRotor = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> TailRotor;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> TailRotor;"
...
STOP_BEFORE_CHIN_TURRET = "ChinTurret"
...
LEFTOVER_APACHE_AIRCRAFT_ROOT_FIELD = (
...
TAIL_ROTOR_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+TailRotor\s*;"
)
TAIL_ROTOR_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+TailRotor\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover MainRotor / RotorMast / StubWingRight, or an
    # initializer on TailRotor as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if TAIL_ROTOR_DECL_RE.search(compact) is None:
        return False
    if TAIL_ROTOR_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CHIN_TURRET):
            break
    ...
    if has_identifier(window, SIBLING_CHIN_TURRET):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CHIN_TURRET}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+TailRotor\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> TailRotor "
...
class ApacheTailRotorFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "TailRotor"), section)
        ...
        self.assertTrue(has_identifier(body, SIBLING_CHIN_TURRET), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CHIN_TURRET),
            section,
        )
        ...
        self.assertTrue(has_identifier(section, SIBLING_FUSELAGE), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "TailRotor"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("TailRotor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_tail_rotor_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> TailRotor;", compact_origin)
        self.assertNotIn("TailRotor = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_tail_rotor_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> TailRotor"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> TailRotor;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> TailRotors;\n",
            "\tint32 TailRotor = 140;\n",
            "\tbool TailRotor = true;\n",
            "\tfloat TailRotor = " + forty + ";\n",
            "\tfloat TailRotor = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_chin_turret_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, SIBLING_FUSELAGE))
        ...
        self.assertFalse(has_identifier(section, SIBLING_CHIN_TURRET))
        ...
        self.assertNotIn("TObjectPtr<USceneComponent> ChinTurret;", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache TailRotor field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "tail_rotor_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_APACHE_AIRCRAFT_ROOT_FIELD, LOCKED_SCRIPTS)
