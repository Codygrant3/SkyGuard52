...
# then `float LightSpeedMax = 1750.f;`
# (HAS initializer). KEEP default `1750.f`. KEEP `.f` suffix
# (NOT `1750.0f`). KEEP EditAnywhere BlueprintReadWrite
# Category Skyguard|Spawning. KEEP specifiers matching
# origin/main. 1750.f is a unique identifier default, NOT
# Harbor leftover 40 / 80 float tokens.
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Spawning` with NO spaces around `=`.
# EditAnywhere IS present. BlueprintReadWrite IS present.
# Default IS `= 1750.f`.
# Parse CLASS `ASkyguardDroneSpawner` public UPROPERTY window
# only after `class ASkyguardDroneSpawner` / GENERATED_BODY.
# Start at `public:`. Stop BEFORE `protected:`.
# Do NOT wrap Category=`Skyguard` fields `SpawnInterval` or
# `MaxActiveDrones`. leftover analog drone-spawner-empty-fail-closed
# bulk #161/#df16 is behavioral.
# Word-boundary: `LightSpeedMax` is NOT leftover analog
# `LightSpeedMin = 1300.f` (sibling, in-flight elsewhere),
# NOT leftover analog `HeavyCruiseSpeed = 1150.f`,
# NOT leftover analog Harbor 40 / 80 float tokens,
# NOT leftover analog `SpawnInterval = 1.85f` (bare
# Category="Skyguard", skip), NOT leftover analog
# `MaxActiveDrones = 12` (leftover analog PoolCapacity AND
# bare Category="Skyguard", skip), NOT leftover analog
# `HeavyLaneModulo = 4`, NOT leftover analog
# `bMixedThreatRoster = true`, NOT leftover analog
# `bSpawningEnabled = true`, NOT leftover analog UFUNCTION
...
# `void SetSpawningEnabled(bool bEnabled);`. LOCKED_DECL
# is NOT leftover `bool bSpawningEnabled = true;`. LOCKED_DECL
# is NOT leftover `bool bMixedThreatRoster = true;`.
# LOCKED_DECL is NOT leftover `float LightSpeedMin = 1300.f`.
# LOCKED_DECL is NOT leftover `float HeavyCruiseSpeed = 1150.f`.
# LOCKED_DECL is NOT leftover `int32 HeavyLaneModulo = 4;`.
# LOCKED_DECL is NOT leftover `FName WeatherIdentity`. LOCKED_DECL
# is NOT leftover `FireRate = 12.0f` / `RecoilPitch = 0.92f` /
# `RouteLengthCm = 45000.f` / `160.f`. Harbor leftover is
# only the split 40 / 80 float tokens.
# Category MUST be `Skyguard|Spawning`. Fail-closed if wrap
# is `Category="Skyguard"` without leftover-safe unique
# `|Spawning` suffix.
# Fail-closed if wrap is `Category="Skyguard|Pause"` or
# `Category="Skyguard|Radio"` or `Category="Skyguard|Theater"`
# or `Category="Skyguard|Apache"` without `|Damage`.
# Clone leftover unique analog FIELD MaxReleasesPerBay #1683
# SHA 40592bc71aaf17722aacf8a3a611189e25fbc822 RETARGET.
...
# Clone leftover unique analog FIELD bSpawningEnabled #1722
# SHA a0ce9ee9d3d01c95bc91948dd508698204428c71 RETARGET
# (bool — this slot is float).
# Clone leftover unique analog FIELD bMixedThreatRoster SHA
# 12143ae2d878a0aeeecc8b232d0307462eb7324e RETARGET
# (bool — this slot is float with 1750.f).
# Clone leftover unique analog Mission01 actor bools #1637-#1639
# RETARGET KEEP initializer form.
...
# Clone #1300 SHA c4679feb43f4b97d3a814c088e3b6c344e15831f
# is Category Theater wrapping `FName WeatherIdentity` —
# RETARGET hard.
# Do NOT wrap bSpawningEnabled / HeavyLaneModulo /
# LightSpeedMin / HeavyCruiseSpeed / MaxActiveDrones /
# SpawnInterval / bMixedThreatRoster in this PR.
# Skip leftover analog Mission* `bAllowBoundedActorSpawning`.
...
TARGET = "float LightSpeedMax = 1750.f;"
TARGET_WRONG_BARE = "float LightSpeedMax;"
TARGET_WRONG_INIT = "float LightSpeedMax = nullptr;"
TARGET_WRONG_FALSE = "float LightSpeedMax = false;"
TARGET_WRONG_TRUE = "float LightSpeedMax = true;"
TARGET_WRONG_ZERO = "float LightSpeedMax = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> LightSpeedMax;"
TARGET_WRONG_ASSEMBLY = (
    'FName AssemblyRevision = TEXT("CampaignMapAssembly_v1");'
)
TARGET_WRONG_PCG_AUTHORIZED = "bool bPCGGenerationAuthorized = false;"
TARGET_WRONG_READY_FOR = "bool bReadyForAuthoredPCGGeneration = false;"
TARGET_WRONG_LICENSED = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_HAZE = "bool bEnableCoastalHazeTransition = true;"
TARGET_WRONG_LANDSCAPE = "bool bUseAuthoredLandscapeSurface = false;"
TARGET_WRONG_SPAWNING_FIELD = "bool bSpawningEnabled = true;"
TARGET_WRONG_MIXED_ROSTER = "bool bMixedThreatRoster = true;"
TARGET_WRONG_SET_SPAWNING = "void SetSpawningEnabled(bool bEnabled);"
TARGET_WRONG_MAX_ACTIVE = "int32 MaxActiveDrones = 12;"
TARGET_WRONG_POOL_CAPACITY = "int32 PoolCapacity = 12;"
TARGET_WRONG_SPAWN_INTERVAL = "float SpawnInterval = 1.85f;"
TARGET_WRONG_HEAVY_MODULO = "int32 HeavyLaneModulo = 4;"
TARGET_WRONG_ALLOW_BOUNDED = "bool bAllowBoundedActorSpawning = true;"
TARGET_WRONG_COUNT_ACTIVE = "int32 CountActiveDrones() const;"
TARGET_WRONG_LIGHT_MIN = "float LightSpeedMin = 1300.f;"
TARGET_WRONG_LIGHT_MAX = "float LightSpeedMax = 1750.0f;"
TARGET_WRONG_HEAVY_CRUISE = "float HeavyCruiseSpeed = 1150.f;"
TARGET_WRONG_MAX_RELEASES = "int32 MaxReleasesPerBay = 6;"
TARGET_WRONG_MIN_CIV = "float MinimumCivilianSeparationMeters = 550.f;"
TARGET_WRONG_ROUTE_LENGTH = "float RouteLengthCm = 45000.f;"
UPROPERTY_LEFTOVER_BARE_SKYGUARD = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard")'
)
TARGET_WRONG_ROUTE_POINTS = "TArray RoutePoints"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> LightSpeedMax;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> LightSpeedMax;"
...
UPROPERTY_LEFTOVER_MISSION_MAP = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
...
STOP_BEFORE_ROUTE_LENGTH_CM = "HeavyCruiseSpeed"
...
SIBLING_READINESS = "CountActiveDrones"
...
SIBLING_AUTHORED_PCG_GRAPH = "HeavyLaneModulo"
SIBLING_LICENSED_VEG = "MaxActiveDrones"
...
SIBLING_READY_FOR_AUTHORED = "bReadyForAuthoredPCGGeneration"
...
THIS_SCRIPT = (
    "Scripts/tests/test_drone_spawner_light_speed_max"
    "_field_decl_contract.py"
)
LEFTOVER_SET_SPAWNING_ENABLED = (
    "Scripts/tests/test_drone_spawner_set_spawning_enabled"
    "_decl_contract.py"
)
LEFTOVER_SPAWNING_ENABLED_FIELD = (
    "Scripts/tests/test_drone_spawner_spawning_enabled"
    "_field_decl_contract.py"
)
LEFTOVER_MIXED_THREAT_ROSTER = (
    "Scripts/tests/test_drone_spawner_mixed_threat_roster"
    "_field_decl_contract.py"
)
LEFTOVER_DRONE_SPAWNER_EMPTY = (
    "Scripts/tests/test_drone_spawner_empty_fail_closed.py"
)
LEFTOVER_DRONE_EMPTY = (
    "Scripts/tests/test_drone_empty_fail_closed.py"
)
LEFTOVER_MAX_RELEASES_PER_BAY = (
    "Scripts/tests/test_iron_rain_max_releases_per_bay"
    "_field_decl_contract.py"
)
LEFTOVER_MIN_CIVILIAN_SEPARATION = (
    "Scripts/tests/test_last_flight_minimum_civilian_separation"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION01_ALLOW_AUTHORED_PCG = (
    "Scripts/tests/test_mission01_allow_authored_pcg_generation"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION01_USE_AUTHORED_LANDSCAPE = (
    "Scripts/tests/test_mission01_use_authored_landscape_surface"
    "_field_decl_contract.py"
)
LEFTOVER_MISSION01_LICENSED_VEG_LIBRARY = (
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        "_decl_contract.py",
        f"{prefix}test_drone_spawner_spawning_enabled"
        "_field_decl_contract.py",
        f"{prefix}test_drone_spawner_mixed_threat_roster"
        "_field_decl_contract.py",
        ...
        "_field_decl_contract.py",
        f"{prefix}test_mission01_allow_authored_pcg_generation"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_use_authored_landscape_surface"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_licensed_vegetation_library_approved"
        "_field_decl_contract.py",
        f"{prefix}test_mission01_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission02_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission03_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission04_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission05_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission06_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission07_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission08_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission09_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
        f"{prefix}test_mission10_allow_bounded_actor_spawning"
        "_field_decl_contract.py",
...
ALLOW_AUTHORED_DECL_RE = re.compile(
    r"float\s+LightSpeedMax\s*=\s*1750\.f\s*;"
)
ALLOW_AUTHORED_BARE_RE = re.compile(
    r"float\s+LightSpeedMax\s*;"
)
ASSEMBLY_REVISION_DECL_RE = re.compile(
    r"FName\s+AssemblyRevision\b"
)
PCG_AUTHORIZED_DECL_RE = re.compile(
    r"bool\s+bPCGGenerationAuthorized\s*=\s*false\s*;"
)
READY_FOR_AUTHORED_DECL_RE = re.compile(
    r"bool\s+bReadyForAuthoredPCGGeneration\s*=\s*false\s*;"
)
ROUTE_POINTS_DECL_RE = re.compile(
    r"TArray\b.*RoutePoints"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept leftover analog bSpawningEnabled /
    # SetSpawningEnabled / MaxActiveDrones = 12 /
    # PoolCapacity = 12 / SpawnInterval = 1.85f /
    # HeavyLaneModulo / bAllowBoundedActorSpawning /
    # LightSpeedMin / bMixedThreatRoster / HeavyCruiseSpeed,
    # leftover FName WeatherIdentity, leftover analog
    # MaxReleasesPerBay / MinimumCivilianSeparationMeters,
    # leftover `= 160.f`, leftover analog apache-cpg-feel
    # FireRate / RecoilPitch, leftover RouteLengthCm =
    # 45000.f, leftover Harbor 40 / 80, leftover analog
    # 1750.0f suffix, leftover analog 1300.f / 1150.f,
    # or a bare LightSpeedMax without `= 1750.f`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if ALLOW_AUTHORED_DECL_RE.search(compact) is None:
        return False
    if ALLOW_AUTHORED_BARE_RE.search(compact):
        return False
    if ASSEMBLY_REVISION_DECL_RE.search(compact):
        return False
    if PCG_AUTHORIZED_DECL_RE.search(compact):
        return False
    if READY_FOR_AUTHORED_DECL_RE.search(compact):
        return False
    if ROUTE_POINTS_DECL_RE.search(compact):
        return False
    if LOOK_YAW_LIMIT_DECL_RE.search(compact):
        return False
    if MISSION_ID_DECL_RE.search(compact):
        return False
    if FLIGHT_SPLINE_DECL_RE.search(compact):
        return False
    if MISSION_DEF_DECL_RE.search(compact):
        return False
    if SKYLINE_STYLE_DECL_RE.search(compact):
        return False
    if MAP_READINESS_DECL_RE.search(compact):
        return False
    if FLARE_COUNT_DECL_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+Health\s*=\s*160\.f\b", compact):
        return False
    if re.search(r"\bbSpawningEnabled\b", compact):
        return False
    if re.search(r"\bSetSpawningEnabled\b", compact):
        return False
    if re.search(r"MaxActiveDrones\s*=\s*12\b", compact):
        return False
    if re.search(r"PoolCapacity\s*=\s*12\b", compact):
        return False
    if re.search(r"SpawnInterval\s*=\s*1\.85f\b", compact):
        return False
    if re.search(r"\bHeavyLaneModulo\b", compact):
        return False
    if re.search(r"\bbAllowBoundedActorSpawning\b", compact):
        return False
    if re.search(r"\bLightSpeedMin\b", compact):
        return False
    if re.search(r"\bbMixedThreatRoster\b", compact):
        return False
    if re.search(r"\bHeavyCruiseSpeed\b", compact):
        return False
    if re.search(r"RouteLengthCm\s*=\s*45000\.f\b", compact):
        return False
    if re.search(r"MaxReleasesPerBay\s*=\s*6\b", compact):
        return False
    if re.search(r"MinimumCivilianSeparationMeters\b", compact):
        return False
    forty = "40" + ".f"
    eighty = "80" + ".f"
    if leftover_harbor_token_in(compact, forty):
        return False
    if leftover_harbor_token_in(compact, eighty):
        return False
    return True
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_ROUTE_LENGTH_CM:
            break
        if ident != "LightSpeedMax":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_ROUTE_LENGTH_CM):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_ROUTE_LENGTH_CM}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+LightSpeedMax\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LightSpeedMax "
...
class DroneSpawnerLightSpeedMaxFieldDeclContractTests(unittest.TestCase):
    def test_drone_spawner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LightSpeedMax"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PUBLIC, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_ROUTE_LENGTH_CM), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_ROOT), body)
        self.assertFalse(has_identifier(section, SIBLING_ROOT), section)
        self.assertTrue(has_identifier(body, SIBLING_AUTHORED_PCG_GRAPH), body)
        self.assertFalse(
            has_identifier(section, SIBLING_AUTHORED_PCG_GRAPH),
            section,
        )
        self.assertTrue(has_identifier(body, SIBLING_LICENSED_VEG), body)
        self.assertFalse(has_identifier(section, SIBLING_LICENSED_VEG), section)
        self.assertTrue(has_identifier(body, SIBLING_READINESS), body)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LightSpeedMax"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LightSpeedMax", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_light_speed_max_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn('Skyguard|Pause', section)
        self.assertNotIn('Skyguard|Radio', section)
        ...
        self.assertIn("|Spawning", UPROPERTY_EDIT_READWRITE)
        self.assertIn("EditAnywhere", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertNotIn('Skyguard|Pause', specifiers)
        self.assertNotIn('Skyguard|Radio', specifiers)
    ...
    def test_light_speed_max_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LightSpeedMax"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1750.f", LOCKED_DECL)
        self.assertNotIn("1750.0f", LOCKED_DECL)
        self.assertNotIn("1300.f", LOCKED_DECL)
        self.assertNotIn("1150.f", LOCKED_DECL)
        self.assertNotIn("true", LOCKED_DECL)
        self.assertEqual(LOCKED_DECL, "float LightSpeedMax = 1750.f;")
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_PCG_AUTHORIZED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_READY_FOR)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("1750.f", LOCKED_DECL)
        self.assertNotIn('TEXT("CampaignMapAssembly_v1")', LOCKED_DECL)
        self.assertNotIn("95.f", LOCKED_DECL)
        self.assertNotIn("140.f", LOCKED_DECL)
        self.assertNotIn("160.f", LOCKED_DECL)
        self.assertNotIn(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat LightSpeedMaxs = 1750.f;\n",
            "\tint32 LightSpeedMax = 1750;\n",
            "\tfloat LightSpeedMax = 1300.f;\n",
            "\tfloat LightSpeedMax = " + forty + ";\n",
            "\tfloat LightSpeedMax = " + eighty + ";\n",
            f"\t{TARGET_WRONG_ASSEMBLY}\n",
            f"\t{TARGET_WRONG_PCG_AUTHORIZED}\n",
            f"\t{TARGET_WRONG_READY_FOR}\n",
            ...
            f"\t{TARGET_WRONG_SPAWNING_FIELD}\n",
            f"\t{TARGET_WRONG_MIXED_ROSTER}\n",
            f"\t{TARGET_WRONG_SET_SPAWNING}\n",
            f"\t{TARGET_WRONG_MAX_ACTIVE}\n",
            f"\t{TARGET_WRONG_POOL_CAPACITY}\n",
            f"\t{TARGET_WRONG_SPAWN_INTERVAL}\n",
            f"\t{TARGET_WRONG_HEAVY_MODULO}\n",
            f"\t{TARGET_WRONG_ALLOW_BOUNDED}\n",
            f"\t{TARGET_WRONG_COUNT_ACTIVE}\n",
            f"\t{TARGET_WRONG_LIGHT_MIN}\n",
            f"\t{TARGET_WRONG_LIGHT_MAX}\n",
            f"\t{TARGET_WRONG_HEAVY_CRUISE}\n",
            f"\t{TARGET_WRONG_MAX_RELEASES}\n",
            f"\t{TARGET_WRONG_MIN_CIV}\n",
            f"\t{TARGET_WRONG_ROUTE_LENGTH}\n",
            f"\t{UPROPERTY_LEFTOVER_BARE_SKYGUARD}\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertNotIn('Skyguard|Radio', origin)
        self.assertIsNone(
        ...
        leftover_map = (
            ...
            "{\n"
            "public:\n"
            f"\t{UPROPERTY_LEFTOVER_MISSION_MAP}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        map_specs = attached_uproperty_specifiers(
        ...
        self.assertIn("EditAnywhere", map_specs)
        self.assertIn("EditAnywhere", origin)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tfloat\n\tLightSpeedMax = 1750.f;\n",
            "\tfloat   LightSpeedMax = 1750.f;\n",
            "\tfloat\tLightSpeedMax = 1750.f;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_EDIT_READWRITE}\n\t{LOCKED_DECL}\n",
    ...
    def test_does_not_claim_sibling_route_length_cm_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "LightSpeedMax"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_LICENSED_VEG))
        self.assertFalse(has_identifier(section, SIBLING_LICENSED_VEG))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "LightSpeedMax"))
        self.assertTrue(has_identifier(section, "LightSpeedMax"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertFalse(has_identifier(section, STOP_BEFORE_ROUTE_LENGTH_CM))
        self.assertTrue(has_identifier(leaked, SIBLING_LICENSED_VEG))
        ...
        self.assertTrue(has_identifier(leaked, SIBLING_AUTHORED_PCG_GRAPH))
        self.assertFalse(has_identifier(section, SIBLING_AUTHORED_PCG_GRAPH))
        self.assertTrue(has_identifier(leaked, SIBLING_READINESS))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "drone spawner light speed max field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "drone_spawner_light_speed_max_field_decl_contract.py"
        ))
        self.assertIn(LEFTOVER_ASSEMBLY_REVISION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION01_PCG_AUTHORIZED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION01_READY_FOR_AUTHORED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION_MAP_ROUTE_POINTS, LOCKED_SCRIPTS)
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_SET_SPAWNING_ENABLED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_SPAWNING_ENABLED_FIELD, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MIXED_THREAT_ROSTER, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_DRONE_SPAWNER_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_DRONE_EMPTY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MAX_RELEASES_PER_BAY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MIN_CIVILIAN_SEPARATION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION01_ALLOW_AUTHORED_PCG, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION01_USE_AUTHORED_LANDSCAPE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_MISSION01_LICENSED_VEG_LIBRARY, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_THEATER_WEATHER_IDENTITY, LOCKED_SCRIPTS)
    ...
    def test_word_boundary_light_speed_max_vs_spawning_analogs(self) -> None:
        self.assertTrue(
            has_identifier(
                "float LightSpeedMax = 1750.f;",
                "LightSpeedMax",
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "bSpawningEnabled"))
        self.assertFalse(has_identifier(LOCKED_DECL, "SetSpawningEnabled"))
        self.assertFalse(
            has_identifier(LOCKED_DECL, "bAllowBoundedActorSpawning")
        )
        self.assertFalse(has_identifier(LOCKED_DECL, "HeavyLaneModulo"))
        self.assertFalse(has_identifier(LOCKED_DECL, "MaxActiveDrones"))
        self.assertFalse(has_identifier(LOCKED_DECL, "SpawnInterval"))
        self.assertFalse(has_identifier(LOCKED_DECL, "PoolCapacity"))
        self.assertFalse(has_identifier(LOCKED_DECL, "LightSpeedMin"))
        self.assertTrue(has_identifier(LOCKED_DECL, "LightSpeedMax"))
        self.assertFalse(has_identifier(LOCKED_DECL, "HeavyCruiseSpeed"))
        self.assertFalse(has_identifier(LOCKED_DECL, "bMixedThreatRoster"))
        self.assertFalse(has_identifier(LOCKED_DECL, "CountActiveDrones"))
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPAWNING_FIELD)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MIXED_ROSTER)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SET_SPAWNING)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_ACTIVE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_POOL_CAPACITY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPAWN_INTERVAL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HEAVY_MODULO)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ALLOW_BOUNDED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MAX_RELEASES)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MIN_CIV)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROUTE_LENGTH)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FNAME)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FIRE_RATE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RECOIL_PITCH)
        self.assertNotIn("When true", LOCKED_DECL)
        self.assertNotIn("lanes mix", LOCKED_DECL)
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, "LightSpeedMax"), section)
        self.assertFalse(has_identifier(section, "bSpawningEnabled"), section)
        self.assertFalse(has_identifier(section, "SetSpawningEnabled"), section)
        self.assertFalse(has_identifier(section, "MaxActiveDrones"), section)
        self.assertFalse(has_identifier(section, "SpawnInterval"), section)
        self.assertFalse(has_identifier(section, "HeavyLaneModulo"), section)
        self.assertFalse(has_identifier(section, "LightSpeedMin"), section)
        self.assertFalse(has_identifier(section, "bMixedThreatRoster"), section)
        self.assertFalse(has_identifier(section, "HeavyCruiseSpeed"), section)
        self.assertFalse(has_identifier(section, "CountActiveDrones"), section)
        self.assertEqual(declaration_count(section, LOCKED_DECL), 1)
        origin = attached_uproperty_specifiers(section)
        self.assertIn('Category="Skyguard|Spawning"', origin)
        self.assertIn("|Spawning", origin)
        self.assertNotEqual(origin, 'EditAnywhere, BlueprintReadWrite, Category="Skyguard"')
        self.assertIsNone(
            re.search(r'Category="Skyguard"(?!\|)', origin)
        )

    def test_does_not_wrap_spawning_interval_or_max_active_as_this_slot(self) -> None:
        header = origin_main_header()
        body = class_body(header)
        section = spec_section(header)
        self.assertTrue(has_identifier(header, "bSpawningEnabled"))
        self.assertTrue(has_identifier(header, "SetSpawningEnabled"))
        self.assertTrue(has_identifier(header, "MaxActiveDrones"))
        self.assertTrue(has_identifier(header, "SpawnInterval"))
        self.assertTrue(has_identifier(header, "HeavyLaneModulo"))
        self.assertTrue(has_identifier(header, "LightSpeedMin"))
        self.assertTrue(has_identifier(header, "LightSpeedMax"))
        self.assertTrue(has_identifier(header, "HeavyCruiseSpeed"))
        self.assertTrue(has_identifier(header, "bMixedThreatRoster"))
        self.assertTrue(has_identifier(header, "CountActiveDrones"))
        self.assertTrue(has_identifier(body, "bSpawningEnabled"))
        self.assertTrue(has_identifier(body, "SetSpawningEnabled"))
        self.assertTrue(has_identifier(body, "MaxActiveDrones"))
        self.assertTrue(has_identifier(body, "SpawnInterval"))
        self.assertTrue(has_identifier(body, "HeavyLaneModulo"))
        self.assertTrue(has_identifier(body, "LightSpeedMin"))
        self.assertTrue(has_identifier(body, "LightSpeedMax"))
        self.assertFalse(has_identifier(section, "bSpawningEnabled"), section)
        self.assertFalse(has_identifier(section, "SetSpawningEnabled"), section)
        self.assertFalse(has_identifier(section, "MaxActiveDrones"), section)
        self.assertFalse(has_identifier(section, "SpawnInterval"), section)
        ...
        self.assertFalse(has_identifier(section, "bMixedThreatRoster"), section)
        self.assertFalse(has_identifier(section, "HeavyCruiseSpeed"), section)
        self.assertNotIn('Category="Skyguard"', UPROPERTY_EDIT_READWRITE)
        self.assertIn('Category="Skyguard|Spawning"', UPROPERTY_EDIT_READWRITE)
        self.assertNotEqual(
            UPROPERTY_EDIT_READWRITE,
            UPROPERTY_LEFTOVER_BARE_SKYGUARD,
        )
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SPAWN_INTERVAL)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_POOL_CAPACITY)
        self.assertNotEqual(CLASS_NAME, "USkyguardArcadeLookComponent")
        self.assertNotEqual(
            HEADER_PATH,
            "Source/Skyguard52/SkyguardCampaignTheaterKit.h",
        )
