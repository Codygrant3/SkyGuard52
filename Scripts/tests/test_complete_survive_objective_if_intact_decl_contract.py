...
# values, or lock the CompleteSurviveObjectiveIfIntact body
# in SkyguardCampaignSubsystem.cpp.
COMPLETE_SURVIVE_DECL = (
...
# CompleteSurviveObjectiveIfIntact declaration contract.
# Stay off AddObjectiveProgress / FailObjective (in-flight
# siblings), leftover objective-runtime fail-closed /
# leftover objective-runtime survive tests, leftover
# objective-progress defaults #ae90 / leftover
# objective-definition defaults #b29f, leftover CPG debrief
# #284/#195/#130/#8ccd, FillResultCombatStats (takes leftover
# ASkyguardGunner*), leftover campaign-save empty-fail-closed
# drafts, leftover campaign-roster lookup #111, leftover
...
# lock-phase, Harbor IncomingRadar 40/80, live-copy leftovers,
# FSkyguardMission0NIntegrationReadiness (bYakRuntimeReady),
# dirty D:\Skyguard52, and ApplyHydraForClusters.
LOCKED = {
...
# objective-runtime fail-closed, leftover objective-runtime
# survive tests, leftover objective-progress defaults #ae90,
...
# leftover flare/HUD #57/#61/#62, leftover #147 ApacheSystem,
# leftover #149 weapon stations, leftover #152 pilot
# commands, leftover #154 loadout / lock-phase, and
# in-flight campaign declaration siblings stay sibling-only.
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_objective_runtime_survive_intact_tests.py",
    "Scripts/tests/test_objective_survive_intact_tests.py",
...
# AddObjectiveProgress / FailObjective are in-flight siblings.
...
IN_FLIGHT_OBJECTIVE_NOT_LOCKED = (
    "bool AddObjectiveProgress(FName ObjectiveId, int32 Amount = 1);",
    "bool FailObjective(FName ObjectiveId);",
)
...
# Leftover objective-runtime fail-closed / survive tests
# stay unlocked. Do not require them and do not treat
# leftover SkyguardObjectiveRuntime.h as this lane's
# edit surface.
...
OBJECTIVE_RUNTIME_SURVIVE_NOT_LOCKED = (
    "SkyguardObjectiveRuntimeTests.cpp",
    "test_objective_runtime_survive_tests.py",
    "test_objective_runtime_survive_intact_tests.py",
    "test_objective_survive_intact_tests.py",
)
# Leftover objective-progress defaults #ae90 /
# leftover objective-definition defaults #b29f stay unlocked.
...
# .cpp CompleteSurviveObjectiveIfIntact body / invented
# return values stay unlocked. Do not invent INDEX_NONE
# and do not lock the .cpp body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
    ...
    "ObjectiveRuntime &&",
    "ObjectiveRuntime->CompleteSurviveObjectiveIfIntact",
    "USkyguardCampaignSubsystem::CompleteSurviveObjectiveIfIntact",
    "SkyguardCampaignSubsystem.cpp",
)
SIBLING_TYPES = (
...
class CompleteSurviveObjectiveIfIntactDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, COMPLETE_SURVIVE_DECL), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, COMPLETE_SURVIVE_DECL)
        self.assertIn("CompleteSurviveObjectiveIfIntact", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertNotIn(COMPLETE_SURVIVE_DECL, section)

    def test_missing_complete_survive_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, COMPLETE_SURVIVE_DECL)
        self.assertIn("CompleteSurviveObjectiveIfIntact", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, COMPLETE_SURVIVE_DECL)
        self.assertIn("CompleteSurviveObjectiveIfIntact", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_neighbor_objective_helpers_do_not_satisfy(self) -> None:
        other_helpers = (
            "\tbool AddObjectiveProgress("
            "FName ObjectiveId, int32 Amount = 1);\n"
            "\tbool FailObjective(FName ObjectiveId);\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, COMPLETE_SURVIVE_DECL)
        self.assertIn("CompleteSurviveObjectiveIfIntact", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_complete_survive_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, COMPLETE_SURVIVE_DECL),
            COMPLETE_SURVIVE_DECL,
        )
        self.assertTrue(has_declaration(section, COMPLETE_SURVIVE_DECL))
        self.assertEqual(declaration_count(section, COMPLETE_SURVIVE_DECL), 1)
        self.assertTrue(COMPLETE_SURVIVE_DECL.endswith(";"), COMPLETE_SURVIVE_DECL)
        self.assertTrue(
            COMPLETE_SURVIVE_DECL.startswith("bool "),
            COMPLETE_SURVIVE_DECL,
        )
        self.assertIn("(FName ObjectiveId)", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("INDEX_NONE", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("return ", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("const", COMPLETE_SURVIVE_DECL)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tbool\n"
            "\tCompleteSurviveObjectiveIfIntact(FName ObjectiveId);\n"
            "private:\n"
            "};\n"
        )
        wrap_args = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, COMPLETE_SURVIVE_DECL),
                section,
            )
            self.assertEqual(
                require_declaration(section, COMPLETE_SURVIVE_DECL),
            ...
            self.assertEqual(
                declaration_count(section, COMPLETE_SURVIVE_DECL),
                1,
            )
        one_line = f"{{\npublic:\n\t{COMPLETE_SURVIVE_DECL}\n}}\n"
        self.assertTrue(has_declaration(one_line, COMPLETE_SURVIVE_DECL))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, COMPLETE_SURVIVE_DECL), section)
        self.assertEqual(
            require_declaration(section, COMPLETE_SURVIVE_DECL),
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("INDEX_NONE", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("NAME_None", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("{", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("}", COMPLETE_SURVIVE_DECL)
        ...
        self.assertNotIn("return false", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("return true", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("= INDEX_NONE", section)
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, COMPLETE_SURVIVE_DECL)

    def test_contract_does_not_relock_configure_campaign(self) -> None:
        locked_only = f"{COMPLETE_SURVIVE_DECL}\n"
        self.assertEqual(
            require_declaration(locked_only, COMPLETE_SURVIVE_DECL),
            COMPLETE_SURVIVE_DECL,
        )
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("ConfigureCampaign", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("ConfigureCampaign", locked_only)
        self.assertNotIn("USkyguardCampaignDefinition", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_can_start_mission(self) -> None:
        ...
        for neighbor in CAN_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("CanStartMission", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_start_mission(self) -> None:
        ...
        for neighbor in START_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("StartMission", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("StartMission", locked_only)

    def test_contract_does_not_relock_add_progress_or_fail_objective(
        self,
    ) -> None:
        locked_only = f"{COMPLETE_SURVIVE_DECL}\n"
        for neighbor in IN_FLIGHT_OBJECTIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("AddObjectiveProgress", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("FailObjective", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("CompleteActiveMission", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("FinalizeActiveMission", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("FailActiveMission", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("FillResultCombatStats", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("ASkyguardGunner", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_debrief_or_travel(self) -> None:
        ...
        for neighbor in DEBRIEF_TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("AcknowledgeDebrief", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("CanTravelToNextMission", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("GetNextMissionMapPackageName", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("TravelToNextMission", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_unlock_or_slot_helpers(self) -> None:
        ...
        for neighbor in UNLOCK_AND_SLOT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("IsMissionUnlocked", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("IsValidCampaignSlotName", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_score_or_medal_tier(self) -> None:
        ...
        for neighbor in SCORE_AND_TIER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("CalculateMissionScore", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("CalculateMedalTier", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("GetEarnedCampaignMedals", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("CalculateMissionScore", locked_only)
        self.assertNotIn("CalculateMedalTier", locked_only)

    def test_contract_does_not_relock_runtime_neighbors(self) -> None:
        locked_only = f"{COMPLETE_SURVIVE_DECL}\n"
        for neighbor in RUNTIME_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("GetActiveMission", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("GetObjectiveRuntime", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("GetRouteRuntime", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_relock_leftover_objective_runtime_fail_closed(
        ...
        self.assertNotIn("SkyguardObjectiveRuntimeFailClosedTests.cpp", locked_only)

    def test_contract_does_not_relock_leftover_objective_runtime_survive(
        self,
    ) -> None:
        locked_only = f"{COMPLETE_SURVIVE_DECL}\n"
        for token in OBJECTIVE_RUNTIME_SURVIVE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("SkyguardObjectiveRuntimeTests.cpp", LOCKED)
        self.assertNotIn("test_objective_runtime_survive_tests.py", locked_only)
        self.assertNotIn(
            "test_objective_runtime_survive_intact_tests.py",
            locked_only,
        )
    ...
    def test_contract_does_not_relock_leftover_objective_defaults(self) -> None:
        ...
        for token in OBJECTIVE_DEFAULTS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, COMPLETE_SURVIVE_DECL)
        self.assertNotIn("test_objective_progress_defaults_contract.py", locked_only)
        self.assertNotIn(
            "test_objective_definition_defaults_contract.py",
            locked_only,
        )
        self.assertNotIn("FSkyguardObjectiveProgress", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("FSkyguardObjectiveDefinition", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("ObjectiveRuntime &&", COMPLETE_SURVIVE_DECL)
        self.assertNotIn(
            "ObjectiveRuntime->CompleteSurviveObjectiveIfIntact",
            COMPLETE_SURVIVE_DECL,
        )
        ...
        self.assertNotIn("{", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("}", COMPLETE_SURVIVE_DECL)
        self.assertNotIn("const FName ObjectiveId", COMPLETE_SURVIVE_DECL)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(COMPLETE_SURVIVE_DECL, "Rifle")
        self.assertNotEqual(COMPLETE_SURVIVE_DECL, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"CompleteSurviveObjectiveIfIntact contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not leftover live copy",
            )
            self.assertNotIn(banned, COMPLETE_SURVIVE_DECL.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", COMPLETE_SURVIVE_DECL)

    def test_contract_is_complete_survive_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
    ...
    def test_locked_scripts_list_leftover_objective_and_roster_siblings(
        ...
        self.assertIn("test_objective_runtime_survive_tests.py", scripts)
