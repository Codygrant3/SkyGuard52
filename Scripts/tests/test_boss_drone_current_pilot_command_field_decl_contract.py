...
# INDEX_NONE or lock CurrentPilotCommand writes
# in the .cpp. Lock field-declaration presence
# of `ESkyguardPilotCommand CurrentPilotCommand;`.
# origin/main is a one-line field with a live
# initializer
# (`ESkyguardPilotCommand CurrentPilotCommand =
# ESkyguardPilotCommand::Pursuit`);
# accept the bare declaration and that live
# initializer without adding a leftover enum
# contract. Accept one-line / split-line wraps.
# Nearby origin/main
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly,
# Category="Skyguard|Boss") is required as
# present. Accept one-line and split-line
# UPROPERTY wraps. The type appears on
# origin/main and is part of the locked field
# declaration. Parse the public class
...
# pilot-command-roster enum contract.
...
# isolated-test drafts #107–#473 including leftover
...
# Harbor 40/80.
CURRENT_PILOT_COMMAND_FIELD = "ESkyguardPilotCommand CurrentPilotCommand;"
CURRENT_PILOT_COMMAND_LIVE_INIT = (
    "ESkyguardPilotCommand CurrentPilotCommand = "
    "ESkyguardPilotCommand::Pursuit;"
)
UPROPERTY_COMMAND = (
...
# Python CurrentPilotCommand field
...
# leftover isolated-test drafts #107–#473,
...
# .cpp CurrentPilotCommand writes / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBossDroneBase::CurrentPilotCommand",
    "SkyguardBossDroneBase.cpp",
...
FIELD_DECL_RE = re.compile(
    r"ESkyguardPilotCommand\s+CurrentPilotCommand"
    r"(?:\s*=\s*ESkyguardPilotCommand\s*::\s*Pursuit)?\s*;"
)


def has_declaration(region: str, declaration: str) -> bool:
    if declaration == CURRENT_PILOT_COMMAND_FIELD:
...
class BossDroneCurrentPilotCommandFieldDeclContractTests(unittest.TestCase):
    def test_boss_drone_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CURRENT_PILOT_COMMAND_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CURRENT_PILOT_COMMAND_FIELD)
        self.assertIn("CurrentPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_missing_current_pilot_command_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBossDroneBase();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CURRENT_PILOT_COMMAND_FIELD)
        self.assertIn("CurrentPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_COMMAND}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CURRENT_PILOT_COMMAND_FIELD)
        self.assertIn("CurrentPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_COMMAND, section)
        ...
        self.assertNotIn("UPROPERTY", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("VisibleAnywhere", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("BlueprintReadOnly", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("Category", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_COMMAND)
        self.assertNotIn("BlueprintCallable", UPROPERTY_COMMAND)
        self.assertIn("Skyguard|Boss", UPROPERTY_COMMAND)
        self.assertNotIn("Destruction", UPROPERTY_COMMAND)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_COMMAND)
            self.assertNotIn(invented, CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CURRENT_PILOT_COMMAND_FIELD)
        self.assertIn("CurrentPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = "\tESkyguardBossPhase CurrentPilotCommand;\n"
        box = "\tint32 CurrentPilotCommand;\n"
        scene = "\tESkyguardPilotCommand PilotCommand;\n"
        weak = "\tTObjectPtr<ESkyguardPilotCommand> CurrentPilotCommand;\n"
        assigned = "\tESkyguardPilotCommand CurrentPilotCommand = nullptr;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CURRENT_PILOT_COMMAND_FIELD)
            self.assertIn("CurrentPilotCommand", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_current_pilot_command_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, CURRENT_PILOT_COMMAND_FIELD),
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertTrue(
            has_declaration(section, CURRENT_PILOT_COMMAND_FIELD)
        )
        self.assertEqual(
            declaration_count(section, CURRENT_PILOT_COMMAND_FIELD),
            1,
        )
        self.assertTrue(
            CURRENT_PILOT_COMMAND_FIELD.startswith("ESkyguardPilotCommand "),
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertTrue(
            CURRENT_PILOT_COMMAND_FIELD.endswith(";"),
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertIn("CurrentPilotCommand", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("=", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("INDEX_NONE", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("UFUNCTION", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetBossPhase", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("{", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("}", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("return ", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("ApplyWeaponHit", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("IssuePilotCommand", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "GetMaxDefeatDebrisPieces",
            CURRENT_PILOT_COMMAND_FIELD,
        )
        ...
        self.assertNotIn("GetDefeatDebrisPieceCount", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetTelemetry", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetGunnerMount", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("FaceWorldLocation", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("WeakPoints", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetSearchlightTracked", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetContactIdentified", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("SetLightningExposed", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            CURRENT_PILOT_COMMAND_FIELD,
        )
        ...
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_COMMAND} {CURRENT_PILOT_COMMAND_FIELD}\n"
            "};\n"
        )
        ...
        wrap_live_init = (
            "public:\n"
            f"\t{CURRENT_PILOT_COMMAND_LIVE_INIT}\n"
            "};\n"
        )
        wrap_live_init_split = (
            "public:\n"
            "\tESkyguardPilotCommand\n"
            "\tCurrentPilotCommand =\n"
            "\tESkyguardPilotCommand::Pursuit;\n"
        ...
        header_wrap_live_init = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public AActor\n{{\n{wrap_live_init}"
        )
        header_wrap_live_init_split = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public AActor\n{{\n{wrap_live_init_split}"
        )
        header_wrap_uproperty_live_init = (
            f"class SKYGUARD52_API {CLASS_NAME} "
            f": public AActor\n{{\n{wrap_uproperty_live_init}"
        )
        for header in (
        ...
        one_line = f"{{\npublic:\n\t{CURRENT_PILOT_COMMAND_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, CURRENT_PILOT_COMMAND_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CURRENT_PILOT_COMMAND_FIELD),
    ...
    def test_origin_main_initializer_is_accepted(self) -> None:
        initialized = (
            "\tESkyguardPilotCommand CurrentPilotCommand = "
            "ESkyguardPilotCommand::Pursuit;\n"
        )
        self.assertTrue(
            has_declaration(initialized, CURRENT_PILOT_COMMAND_FIELD)
        )
        self.assertEqual(
            require_declaration(initialized, CURRENT_PILOT_COMMAND_FIELD),
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertEqual(
            declaration_count(initialized, CURRENT_PILOT_COMMAND_FIELD),
            1,
        )
        self.assertEqual(
            CURRENT_PILOT_COMMAND_LIVE_INIT,
        ...
        assigned_wrong = (
            "\tESkyguardPilotCommand CurrentPilotCommand = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned_wrong, CURRENT_PILOT_COMMAND_FIELD)
        self.assertIn("CurrentPilotCommand", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(assigned_wrong, CURRENT_PILOT_COMMAND_FIELD)
        )
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        for region in (root, body_mesh, weak_points, phase, telemetry):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CURRENT_PILOT_COMMAND_FIELD)
            self.assertIn("CurrentPilotCommand", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, CURRENT_PILOT_COMMAND_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{CURRENT_PILOT_COMMAND_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, CURRENT_PILOT_COMMAND_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_COMMAND, section)
        self.assertTrue(has_declaration(section, CURRENT_PILOT_COMMAND_FIELD), section)

    def test_declaration_does_not_invent_ufunction(self) -> None:
        locked_only = f"{CURRENT_PILOT_COMMAND_FIELD}\n"
        self.assertNotIn("UFUNCTION", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("UFUNCTION", locked_only)
        self.assertFalse(CURRENT_PILOT_COMMAND_FIELD.startswith("UFUNCTION"), CURRENT_PILOT_COMMAND_FIELD)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, CURRENT_PILOT_COMMAND_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CURRENT_PILOT_COMMAND_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_current_pilot_command_cpp_body(
        self,
    ) -> None:
        locked_only = f"{CURRENT_PILOT_COMMAND_FIELD}\n"
        self.assertNotIn("{", CURRENT_PILOT_COMMAND_FIELD)
        ...
        self.assertNotIn("return false", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("return true", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "DefeatDebrisComponents",
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("SetBossPhase", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "HandleWeakPointDestroyed",
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("RegisterDefeatDebris", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("CleanupDefeatDebris", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("HandleDefeated", CURRENT_PILOT_COMMAND_FIELD)

    def test_contract_does_not_relock_get_boss_phase_sibling(self) -> None:
        locked_only = f"{CURRENT_PILOT_COMMAND_FIELD}\n"
        for token in GET_BOSS_PHASE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetBossPhase", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_sibling_public_fields(self) -> None:
        ...
        for token in PUBLIC_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("Root", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("BodyMesh", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("WeakPoints", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("Telemetry", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("OnBossPhaseChanged", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("OnPilotCommandNative", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_hull_collider(self) -> None:
        ...
        for token in LEFTOVER_HULL_COLLIDER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("HullCollider", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_sibling_root_field(self) -> None:
        ...
        for token in LEFTOVER_ROOT_FIELD_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("TObjectPtr<USceneComponent> Root;", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "test_boss_drone_root_field_decl_contract.py",
            CURRENT_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_live_copy_methods(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("WeakPoints", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("PortGuidanceArray", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("CommandCore", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetSearchlightTracked", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("IsSearchlightTracked", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("ASkyguardBlackKiteBoss", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("SetContactIdentified", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("IsContactIdentified", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("OpenOrbitExposure", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "ASkyguardLifelineHunterBoss",
            CURRENT_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        self.assertNotIn("ASkyguardTempestBoss", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("OpenDispenserBay", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("ASkyguardIronRainBoss", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            CURRENT_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetGunnerMount", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetChinTurret", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetSensorTurret", CURRENT_PILOT_COMMAND_FIELD)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{CURRENT_PILOT_COMMAND_FIELD}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("ASkyguardGunner", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("FillAndFinalize", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("SkyguardRadarNode", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "USkyguardBriefingWidget",
            CURRENT_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            CURRENT_PILOT_COMMAND_FIELD,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("MaxIntegrity", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("CurrentIntegrity", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn(
            "SkyguardApacheAircraft.h",
            CURRENT_PILOT_COMMAND_FIELD,
        )
        self.assertNotIn("SkyguardPathfinder", CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, CURRENT_PILOT_COMMAND_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CURRENT_PILOT_COMMAND_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, CURRENT_PILOT_COMMAND_FIELD),
            CURRENT_PILOT_COMMAND_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "boss drone CurrentPilotCommand field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, CURRENT_PILOT_COMMAND_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                CURRENT_PILOT_COMMAND_FIELD.lower(),
                "boss drone CurrentPilotCommand contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CURRENT_PILOT_COMMAND_FIELD)

    def test_contract_is_current_pilot_command_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", CURRENT_PILOT_COMMAND_FIELD)
        self.assertNotIn("{", CURRENT_PILOT_COMMAND_FIELD)
        self.assertTrue(CURRENT_PILOT_COMMAND_FIELD.startswith("ESkyguardPilotCommand "))
        self.assertTrue(CURRENT_PILOT_COMMAND_FIELD.endswith(";"))
        self.assertNotIn("=", CURRENT_PILOT_COMMAND_FIELD)
        self.assertIn(UPROPERTY_COMMAND, section)
