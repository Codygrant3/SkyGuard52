...
# values, or lock a specific GetRouteRuntime body.
...
GET_ROUTE_RUNTIME = "USkyguardRouteRuntime* GetRouteRuntime() const"
INLINE_BODY_FORM = (
    "USkyguardRouteRuntime* GetRouteRuntime() const "
    "{ return RouteRuntime; }"
)
# Leftover #56–#64 plus CampaignSubsystem production files.
# This lane only adds an isolated Python GetRouteRuntime
# declaration contract. Do not add leftover
# SkyguardRouteRuntime.h as a file this lane edits.
# Stay off leftover route-runtime fail-closed (#1822 and
# related), leftover objective-runtime fail-closed,
# GetObjectiveRuntime #313, GetActiveMission #311,
# GetActiveMissionElapsedSeconds #314, CompleteActiveMission /
# FinalizeActiveMission / FailActiveMission (siblings this
# wave), TravelToNextMission (in-flight sibling), leftover
# CPG debrief #284/#195/#130/#8ccd, FillResultCombatStats /
# FillAndFinalize / FillAndFail (takes leftover
...
# pilot commands, leftover #154 loadout / lock-phase,
# leftover settings invert-look / ApplySettings broadcast
# #134, Harbor IncomingRadar 40/80, live-copy leftovers,
...
# route-runtime fail-closed, leftover objective-runtime
# fail-closed, leftover campaign-save empty-fail-closed,
...
# FailActiveMission are siblings this wave.
# TravelToNextMission is an in-flight sibling.
...
# tokens stay unlocked. Do not require them and do not
# treat leftover SkyguardRouteRuntime.h as this lane's
# edit surface.
...
# INDEX_NONE or lock a specific GetRouteRuntime body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return nullptr",
    ...
    "return 0",
    "return -1",
    "USkyguardCampaignSubsystem::GetRouteRuntime",
    "SkyguardCampaignSubsystem.cpp",
)
SIBLING_TYPES = (
...
class GetRouteRuntimeDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, GET_ROUTE_RUNTIME), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_ROUTE_RUNTIME)
        self.assertIn("GetRouteRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertNotIn(GET_ROUTE_RUNTIME, section)

    def test_missing_get_route_runtime_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_ROUTE_RUNTIME)
        self.assertIn("GetRouteRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_ROUTE_RUNTIME)
        self.assertIn("GetRouteRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_neighbor_getters_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_getters, GET_ROUTE_RUNTIME)
        self.assertIn("GetRouteRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        non_const = "\tUSkyguardRouteRuntime* GetRouteRuntime();\n"
        parameterized = (
            "\tUSkyguardRouteRuntime* GetRouteRuntime("
            "FName RouteId) const;\n"
        )
        wrong_return = "\tint32 GetRouteRuntime() const;\n"
        for region in (non_const, parameterized, wrong_return):
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_ROUTE_RUNTIME)
            self.assertIn("GetRouteRuntime", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_route_runtime_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_ROUTE_RUNTIME),
            GET_ROUTE_RUNTIME,
        )
        self.assertTrue(has_declaration(section, GET_ROUTE_RUNTIME))
        self.assertEqual(declaration_count(section, GET_ROUTE_RUNTIME), 1)
        self.assertTrue(
            GET_ROUTE_RUNTIME.endswith("const"),
            GET_ROUTE_RUNTIME,
        )
        self.assertTrue(
            GET_ROUTE_RUNTIME.startswith("USkyguardRouteRuntime* "),
            GET_ROUTE_RUNTIME,
        )
        self.assertIn("GetRouteRuntime() const", GET_ROUTE_RUNTIME)
        self.assertNotIn("INDEX_NONE", GET_ROUTE_RUNTIME)
        self.assertNotIn("return ", GET_ROUTE_RUNTIME)
        self.assertNotIn("{", GET_ROUTE_RUNTIME)
        self.assertNotIn("}", GET_ROUTE_RUNTIME)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tUSkyguardRouteRuntime*\n"
            "\tGetRouteRuntime() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_const = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, GET_ROUTE_RUNTIME),
                section,
            )
            self.assertEqual(
                require_declaration(section, GET_ROUTE_RUNTIME),
            ...
            self.assertEqual(
                declaration_count(section, GET_ROUTE_RUNTIME),
                1,
            )
        one_line = f"{{\npublic:\n\t{GET_ROUTE_RUNTIME};\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_ROUTE_RUNTIME))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_ROUTE_RUNTIME),
            section,
    ...
    def test_declaration_accepts_inline_body_form(self) -> None:
        ...
        self.assertTrue(has_declaration(INLINE_BODY_FORM, GET_ROUTE_RUNTIME))
        self.assertNotIn("{", GET_ROUTE_RUNTIME)
        self.assertNotIn("return RouteRuntime", GET_ROUTE_RUNTIME)
        self.assertNotEqual(GET_ROUTE_RUNTIME, INLINE_BODY_FORM)
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, GET_ROUTE_RUNTIME), section)
    ...
    def test_declaration_does_not_require_inline_body(self) -> None:
        ...
        self.assertNotIn("return RouteRuntime", GET_ROUTE_RUNTIME)
        self.assertNotIn("return ", GET_ROUTE_RUNTIME)
        semicolon_only = f"{GET_ROUTE_RUNTIME};\n"
        self.assertEqual(
            require_declaration(semicolon_only, GET_ROUTE_RUNTIME),
            GET_ROUTE_RUNTIME,
        )
        self.assertNotIn(INLINE_BODY_FORM, GET_ROUTE_RUNTIME)
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", GET_ROUTE_RUNTIME)
        self.assertNotIn("INDEX_NONE", GET_ROUTE_RUNTIME)
        self.assertNotIn("NAME_None", GET_ROUTE_RUNTIME)
        self.assertNotIn("{", GET_ROUTE_RUNTIME)
        self.assertNotIn("}", GET_ROUTE_RUNTIME)
        ...
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, GET_ROUTE_RUNTIME)

    def test_contract_does_not_lock_get_route_runtime_body(self) -> None:
        locked_only = f"{GET_ROUTE_RUNTIME}\n"
        self.assertNotIn("{", GET_ROUTE_RUNTIME)
        self.assertNotIn("}", GET_ROUTE_RUNTIME)
        self.assertNotIn("return RouteRuntime", GET_ROUTE_RUNTIME)
        self.assertNotIn("return RouteRuntime", locked_only)
        self.assertNotIn(INLINE_BODY_FORM, GET_ROUTE_RUNTIME)
        self.assertNotIn(
            "USkyguardCampaignSubsystem::GetRouteRuntime",
            GET_ROUTE_RUNTIME,
    ...
    def test_contract_does_not_relock_configure_campaign(self) -> None:
        locked_only = f"{GET_ROUTE_RUNTIME}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_ROUTE_RUNTIME),
            GET_ROUTE_RUNTIME,
        )
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("ConfigureCampaign", GET_ROUTE_RUNTIME)
        self.assertNotIn("ConfigureCampaign", locked_only)
        self.assertNotIn("USkyguardCampaignDefinition", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_can_start_mission(self) -> None:
        ...
        for neighbor in CAN_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("CanStartMission", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_start_mission(self) -> None:
        ...
        for neighbor in START_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("StartMission", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_unlock_or_slot_helpers(self) -> None:
        ...
        for neighbor in UNLOCK_AND_SLOT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("IsMissionUnlocked", GET_ROUTE_RUNTIME)
        self.assertNotIn("IsValidCampaignSlotName", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_score_or_medal_tier(self) -> None:
        ...
        for neighbor in SCORE_AND_TIER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("CalculateMissionScore", GET_ROUTE_RUNTIME)
        self.assertNotIn("CalculateMedalTier", GET_ROUTE_RUNTIME)
        self.assertNotIn("GetEarnedCampaignMedals", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_debrief_or_travel(self) -> None:
        ...
        for neighbor in DEBRIEF_TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("AcknowledgeDebrief", GET_ROUTE_RUNTIME)
        self.assertNotIn("CanTravelToNextMission", GET_ROUTE_RUNTIME)
        self.assertNotIn("GetNextMissionMapPackageName", GET_ROUTE_RUNTIME)
        self.assertNotIn("TravelToNextMission", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("FillResultCombatStats", GET_ROUTE_RUNTIME)
        self.assertNotIn("ASkyguardGunner", GET_ROUTE_RUNTIME)
        self.assertNotIn("FillAndFinalize", GET_ROUTE_RUNTIME)
        self.assertNotIn("FillAndFail", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_active_mission_helpers(self) -> None:
        ...
        for neighbor in ACTIVE_MISSION_HELPERS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("CompleteActiveMission", GET_ROUTE_RUNTIME)
        self.assertNotIn("FinalizeActiveMission", GET_ROUTE_RUNTIME)
        self.assertNotIn("FailActiveMission", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_relock_get_active_mission_elapsed_seconds(
        ...
        for neighbor in GET_ELAPSED_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("GetActiveMissionElapsedSeconds", GET_ROUTE_RUNTIME)
        self.assertNotIn("GetActiveMissionElapsedSeconds", locked_only)

    def test_contract_does_not_relock_get_active_or_objective_runtime(
        self,
    ) -> None:
        locked_only = f"{GET_ROUTE_RUNTIME}\n"
        for neighbor in RUNTIME_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_ROUTE_RUNTIME)
        self.assertNotIn("GetActiveMission", GET_ROUTE_RUNTIME)
        self.assertNotIn("GetObjectiveRuntime", GET_ROUTE_RUNTIME)
    ...
    def test_contract_parses_public_section_not_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("TObjectPtr<USkyguardRouteRuntime>", section)
        self.assertEqual(
            require_declaration(section, GET_ROUTE_RUNTIME),
            GET_ROUTE_RUNTIME,
        )
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("return nullptr", GET_ROUTE_RUNTIME)
        self.assertNotIn("return INDEX_NONE", GET_ROUTE_RUNTIME)
        self.assertNotIn("INDEX_NONE", GET_ROUTE_RUNTIME)
        self.assertNotIn("{", GET_ROUTE_RUNTIME)
        self.assertNotIn("}", GET_ROUTE_RUNTIME)
        self.assertNotIn("return RouteRuntime", GET_ROUTE_RUNTIME)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(GET_ROUTE_RUNTIME, "Rifle")
        self.assertNotEqual(GET_ROUTE_RUNTIME, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"GetRouteRuntime contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not leftover live copy",
            )
            self.assertNotIn(banned, GET_ROUTE_RUNTIME.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", GET_ROUTE_RUNTIME)

    def test_contract_is_get_route_runtime_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
    ...
    def test_locked_scripts_list_leftover_route_and_sibling_contracts(
        self,
    ) -> None:
        scripts = "\n".join(LOCKED_SCRIPTS)
