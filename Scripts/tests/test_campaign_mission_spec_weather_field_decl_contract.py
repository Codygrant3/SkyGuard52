# THIS IS leftover-safe FSkyguardCampaignMissionSpec Weather.
# origin/main form: plain C++ field
# `ESkyguardMissionWeather Weather = ESkyguardMissionWeather::Clear;`
# with in-struct initializer ::Clear. NOT a UPROPERTY wrap.
...
# `ESkyguardMissionWeather::Storm` instead of `::Clear`.
...
# Weather leftover-safe #1495
# (`ESkyguardMissionWeather Weather =
# ESkyguardMissionWeather::Storm`).
# Do NOT parse leftover `FSkyguardWeatherProfile`
# Weather leftover-safe #1353 (UPROPERTY EditAnywhere
# wrap, same ::Clear initializer, different struct).
...
# Success / Failure / WeatherIdentity / WeatherLabel /
...
# invent Harbor 40/80.
# Do NOT claim leftover StormRainBeatKit Weather #1495.
# Do NOT claim leftover WeatherProfile Weather #1353.
# Do NOT claim leftover CampaignMissionSpec MissionId
# #1521 / Title #1522 / Brief #1524 / Success #1523.
# Do NOT claim leftover analog campaign-roster-lookup-tests
# #85ab / leftover campaign-roster-weather-enum-label.
...
# RETARGET to a plain-struct enum field with ::Clear.
# Parse window comes from leftover-safe CampaignMissionSpec
# MissionId #1521 (stop before namespace SkyguardCampaignRoster).
# Do NOT copy leftover StormRain Weather parse window.
# Do NOT copy leftover WeatherProfile Weather parse window.
...
TARGET_WRONG_HEALTH = "float Weather = 160.f;"
...
def leftover_weather_slot_scripts() -> tuple[str, ...]:
    return (
        LEFTOVER_STORM_KIT_WEATHER,
        LEFTOVER_WEATHER_PROFILE_WEATHER,
        LEFTOVER_STORM_KIT_WEATHER_IDENTITY,
...
WEATHER_DECL_RE = re.compile(
    ...
    r"ESkyguardMissionWeather\s*::\s*Clear\s*;"
)
...
def weather_has_uproperty_wrap(region: str) -> bool:
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `ESkyguardMissionWeather Weather =
    # ESkyguardMissionWeather::Clear;`.
    # Do not accept missing initializer, leftover StormRain
    # Weather `::Storm`, leftover WeatherIdentity /
    # WeatherLabel, `= NAME_None` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` when origin/main locks ::Clear.
    # Do not accept leftover WeatherProfile UPROPERTY wrap
    # or leftover TheaterKit Category clones as this
    ...
    if re.search(r"FName\s+Weather\b", compact):
        return False
    if re.search(r"bool\s+Weather\b", compact):
        return False
    if re.search(r"int32\s+Weather\b", compact):
        return False
    if weather_has_uproperty_wrap(region):
        return False
    if re.search(r"\bCategory\s*=", compact):
        return False
    for match in WEATHER_INIT_RE.finditer(compact):
        if collapsed(match.group(1)) != "ESkyguardMissionWeather::Clear":
            return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    raise AssertionError(
        "UPROPERTY for ESkyguardMissionWeather Weather is missing "
...
def require_no_uproperty_wrap(section: str) -> None:
    ...
    if re.search(
        ...
        raise AssertionError(
            "UPROPERTY wrap on ESkyguardMissionWeather Weather is "
...
class CampaignMissionSpecWeatherFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_decl_is_plain_cpp_enum_not_uproperty(self) -> None:
        ...
        self.assertIn("::Clear", LOCKED_DECL)
        self.assertNotIn("::Storm", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STORM)
    ...
    def test_leftover_kit_and_profile_structs_do_not_satisfy(self) -> None:
    ...
    def test_leftover_storm_rain_weather_does_not_satisfy(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_STORM)
        self.assertIn("::Storm", TARGET_WRONG_STORM)
        self.assertNotIn("::Storm", LOCKED_DECL)
        leftover = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_STORM}\n", LOCKED_DECL)
        self.assertIn("Weather", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_leftover_weather_profile_weather_does_not_satisfy(self) -> None:
        self.assertNotEqual(STRUCT_NAME, LEFTOVER_WEATHER_PROFILE)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_MISSION_TYPES_HEADER)
        leftover = (
        ...
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        wrapped = f"\t{CLONE_UPROPERTY_EDIT}\n\t{LOCKED_DECL}\n"
        self.assertFalse(has_declaration(wrapped, LOCKED_DECL), wrapped)
        self.assertTrue(weather_has_uproperty_wrap(wrapped), wrapped)

    def test_does_not_claim_leftover_storm_profile_or_neighbors(self) -> None:
        self.assertNotEqual(HEADER_PATH, LEFTOVER_STORM_HEADER)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_DAY_HEADER)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_NIGHT_HEADER)
        self.assertNotEqual(HEADER_PATH, LEFTOVER_MISSION_TYPES_HEADER)
        ...
        self.assertIn(TARGET_WRONG_STORM, storm_header)
        with self.assertRaises(AssertionError) as raised:
            spec_section(storm_header)
        self.assertIn(STRUCT_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        profile_header = leftover_mission_types_header()
        self.assertIn(LEFTOVER_WEATHER_PROFILE, profile_header)
        self.assertIn(CLONE_UPROPERTY_EDIT, profile_header)
        self.assertIn("ESkyguardMissionWeather Weather", profile_header)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_sibling_identity_and_label_do_not_satisfy_weather(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_must_be_clear_not_storm(self) -> None:
        missing = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
    ...
    def test_weather_declaration_matches_origin_main(self) -> None:
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_OVERCAST}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RAIN}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_NIGHT_CLEAR}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_IDENTITY}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    weather_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(weather_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "campaign-mission-spec Weather field decl contract "
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn("::Storm", locked_only)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_STORM_KIT_WEATHER)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_WEATHER_PROFILE_WEATHER)
