...
# INDEX_NONE or lock bLastFlightReady
# construction in the .cpp. This is leftover-safe
# Mission10 bLastFlightReady FIELD
...
# drafts #674-#811+, leftover Apache
...
LAST_FLIGHT_READY_FIELD = (
    "bool bLastFlightReady = false;"
)
LOCKED_DECL = LAST_FLIGHT_READY_FIELD
...
# an isolated Python bLastFlightReady
...
class Mission10LastFlightReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_missing_last_flight_ready_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_last_flight_ready_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bLastFlightReady"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("bLastFlightReady", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission10 bLastFlightReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission10 bLastFlightReady contains "
    ...
    def test_contract_is_last_flight_ready_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertNotIn("return ", LOCKED_DECL)
        self.assertNotIn("{", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bLastFlightReady"))
        self.assertIn("bLastFlightReady", LOCKED_DECL)
    ...
    def test_sibling_director_fields_do_not_satisfy_last_flight_ready(
        self,
    ) -> None:
        for leftover in (
            BRIEFING_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_last_flight_ready_field"
                "_decl_contract.py"
            )
        )
        self.assertIn(
