# THIS IS leftover-safe ASkyguardApacheAircraft AircraftRoot.
...
# then BARE `TObjectPtr<USceneComponent> AircraftRoot;` (no initializer).
...
# Stop BEFORE sibling SilhouetteMesh as a claimed slot.
...
# `TObjectPtr<USceneComponent> AircraftRoot;` NOT
...
LOCKED_DECL = "TObjectPtr<USceneComponent> AircraftRoot;"
...
TARGET_WRONG_EQ_NONE = "TObjectPtr<USceneComponent> AircraftRoot = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<USceneComponent> AircraftRoot = false;"
TARGET_WRONG_TRUE = "TObjectPtr<USceneComponent> AircraftRoot = true;"
TARGET_WRONG_ZERO = "TObjectPtr<USceneComponent> AircraftRoot = 0.f;"
TARGET_WRONG_FLOAT = "float AircraftRoot;"
TARGET_WRONG_FNAME = "FName AircraftRoot;"
TARGET_WRONG_INT = "int32 AircraftRoot = 0;"
TARGET_WRONG_BOOL = "bool AircraftRoot = true;"
...
AIRCRAFT_ROOT_DECL_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*AircraftRoot\s*;"
)
AIRCRAFT_ROOT_INIT_RE = re.compile(
    r"TObjectPtr\s*<\s*USceneComponent\s*>\s*AircraftRoot\s*="
)
AIRCRAFT_ROOT_FLOAT_RE = re.compile(r"float\s+AircraftRoot\b")
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if AIRCRAFT_ROOT_INIT_RE.search(compact):
        return False
    if AIRCRAFT_ROOT_FLOAT_RE.search(compact):
        return False
    if AIRCRAFT_ROOT_DECL_RE.search(compact) is None:
        return False
    if re.search(r"FName\s+AircraftRoot\b", compact):
        return False
    if re.search(r"int32\s+AircraftRoot\b", compact):
        return False
    if re.search(r"bool\s+AircraftRoot\b", compact):
        return False
    return True
...
def claimed_field_window(header: str) -> str:
    ...
    while True:
        ...
        after = compact[index:]
        field = re.match(
            r"\s*TObjectPtr<USceneComponent>\s*AircraftRoot\s*;",
            after,
        )
    ...
    raise AssertionError(
        "UPROPERTY for TObjectPtr<USceneComponent> AircraftRoot; is "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<USceneComponent>\s*AircraftRoot\b",
            compact[index:],
        ):
...
class ApacheAircraftRootFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "AircraftRoot"), section)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("AircraftRoot", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_aircraft_root_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_locked_decl_is_not_leftover_theater_or_arcade(self) -> None:
        ...
        self.assertIn("AircraftRoot", LOCKED_DECL)
    ...
    def test_locked_decl_requires_bare_pointer(self) -> None:
        ...
        self.assertNotIn("AircraftRoot = nullptr", compact_origin)
        self.assertNotIn("AircraftRoot = NAME_None", compact_origin)
        self.assertNotIn("AircraftRoot = false", compact_origin)
        self.assertNotIn("AircraftRoot = true", compact_origin)
        self.assertNotIn("AircraftRoot = 0.f", compact_origin)
        self.assertNotIn("AircraftRoot = 160.f", compact_origin)

    def test_aircraft_root_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<USceneComponent> AircraftRoot"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertNotIn("=", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<USceneComponent> AircraftRoots;\n",
            "\tTObjectPtr<UStaticMeshComponent> AircraftRoot;\n",
            "\tTObjectPtr<USceneComponent> Root;\n",
            "\tFName WeatherIdentity;\n",
            "\tfloat AircraftRoot = " + forty + ";\n",
            "\tfloat AircraftRoot = " + eighty + ";\n",
        )
    ...
    def test_claimed_window_stops_before_silhouette_mesh(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("AircraftRoot", claimed)
    ...
    def test_parse_window_starts_at_protected_stops_before_private(self) -> None:
        ...
        self.assertIn("AircraftRoot", protected)
        self.assertIn("AircraftRoot", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache AircraftRoot field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "aircraft_root_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
