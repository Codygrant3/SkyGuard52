# THIS IS leftover-safe ASkyguardApacheAircraft PylonLeft.
...
# then BARE `TObjectPtr<UStaticMeshComponent> PylonLeft;`
# (no initializer).
...
# Stop BEFORE sibling PylonRight as a claimed slot.
# Do NOT claim sibling PylonRight / HydraLeft / HydraRight /
# HellfireLeft / StubWingLeft / GearLeft / GearNose / Canopy /
# Fuselage / AircraftRoot / SilhouetteMesh.
# THIS IS leftover-safe ASkyguardApacheAircraft.PylonLeft.
# It is NOT leftover StubWingLeft, NOT leftover analog
...
# PylonRight / HydraLeft / HydraRight / HellfireLeft /
# StubWingLeft / GearLeft / GearNose are sibling slots
# being opened, not this slot.
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> PylonLeft;`
# NOT `FName WeatherIdentity;` and NOT
# `TObjectPtr<USceneComponent> PylonLeft;` and NOT
# leftover `StubWingLeft` / leftover `PylonRight` /
# leftover `HydraLeft` / leftover `GearLeft` and NOT
...
TARGET = "TObjectPtr<UStaticMeshComponent> PylonLeft;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> PylonLeft = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> PylonLeft = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> PylonLeft = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> PylonLeft;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> PylonLeft;"
...
STOP_BEFORE_PYLON_RIGHT = "PylonRight"
...
PYLON_LEFT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+PylonLeft\s*;"
)
PYLON_LEFT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+PylonLeft\s*="
)
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_PYLON_RIGHT):
            break
    ...
    if has_identifier(window, SIBLING_PYLON_RIGHT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_PYLON_RIGHT}"
        )
    for later in (
        SIBLING_HYDRA_LEFT,
        SIBLING_HYDRA_RIGHT,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+PylonLeft\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> PylonLeft "
...
class ApachePylonLeftFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PylonLeft"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_PYLON_RIGHT),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "PylonLeft"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("PylonLeft", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_pylon_left_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> PylonLeft;", compact_origin)
        self.assertNotIn("PylonLeft = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_pylon_left_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> PylonLeft"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> PylonLeft;")
        ...
        self.assertNotEqual(
            LOCKED_DECL,
            "TObjectPtr<USceneComponent> PylonLeft;",
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> PylonLefts;\n",
            "\tint32 PylonLeft = 140;\n",
            "\tbool PylonLeft = true;\n",
            "\tfloat PylonLeft = " + forty + ";\n",
            "\tfloat PylonLeft = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_pylon_right_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertFalse(has_identifier(section, SIBLING_PYLON_RIGHT))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache PylonLeft field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pylon_left_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardApacheAircraft.cpp", THIS_SCRIPT)
