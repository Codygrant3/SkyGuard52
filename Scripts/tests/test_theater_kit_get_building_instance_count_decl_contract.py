...
# then declaration-only `int32 GetBuildingInstanceCount() const;`
...
# Do NOT contract leftover BuildingInstances UPROPERTY #1315.
...
# GetRoadInstanceCount (keep those files in LOCKED_SCRIPTS).
# THIS IS NOT leftover BuildingInstances field #1315.
...
TARGET = "int32 GetBuildingInstanceCount() const;"
TARGET_WRONG_EQ_NONE = "int32 GetBuildingInstanceCount() = NAME_None;"
TARGET_WRONG_FALSE = "int32 GetBuildingInstanceCount() = false;"
TARGET_WRONG_TRUE = "int32 GetBuildingInstanceCount() = true;"
TARGET_WRONG_ZERO = "int32 GetBuildingInstanceCount() = 0.f;"
TARGET_WRONG_FLOAT = "float GetBuildingInstanceCount() const;"
TARGET_WRONG_FNAME = "FName GetBuildingInstanceCount() const;"
TARGET_WRONG_FLINEAR = "FLinearColor GetBuildingInstanceCount() const;"
TARGET_WRONG_NO_CONST = "int32 GetBuildingInstanceCount();"
TARGET_WRONG_BODY = (
    "int32 GetBuildingInstanceCount() const "
...
GET_BUILDING_INSTANCE_COUNT_RE = re.compile(
    r"int32\s+GetBuildingInstanceCount\s*\(\s*\)\s*const\s*;"
)
GET_BUILDING_INSTANCE_COUNT_NO_CONST_RE = re.compile(
    r"int32\s+GetBuildingInstanceCount\s*\(\s*\)\s*(?!const)"
)
GET_BUILDING_INSTANCE_COUNT_WRONG_TYPE_RE = re.compile(
    r"(?:FName|float|FLinearColor|bool|void)\s+GetBuildingInstanceCount\b"
)
GET_BUILDING_INSTANCE_COUNT_BODY_RE = re.compile(
    r"int32\s+GetBuildingInstanceCount\s*\(\s*\)\s*const"
    r"\s*\{"
)
INVENTED_UPROPERTY = (
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if GET_BUILDING_INSTANCE_COUNT_WRONG_TYPE_RE.search(compact):
        return False
    if GET_BUILDING_INSTANCE_COUNT_BODY_RE.search(compact):
        return False
    if GET_BUILDING_INSTANCE_COUNT_RE.search(compact) is None:
        return False
    return True
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+GetBuildingInstanceCount\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for int32 GetBuildingInstanceCount is missing "
...
class TheaterKitGetBuildingInstanceCountDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "GetBuildingInstanceCount"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetBuildingInstanceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_get_building_instance_count_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
        self.assertIsNone(GET_BUILDING_INSTANCE_COUNT_BODY_RE.search(collapsed(section)))
    ...
    def test_missing_const_or_wrong_return_fails_closed(self) -> None:
        ...
        self.assertIsNotNone(GET_BUILDING_INSTANCE_COUNT_RE.search(compact_origin))
        self.assertIsNone(GET_BUILDING_INSTANCE_COUNT_BODY_RE.search(compact_origin))
        self.assertNotIn("FName GetBuildingInstanceCount", compact_origin)
        self.assertNotIn("float GetBuildingInstanceCount", compact_origin)
        self.assertNotIn("FLinearColor GetBuildingInstanceCount", compact_origin)

    def test_get_building_instance_count_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 GetBuildingInstanceCount"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> "
            f"{SIBLING_BUILDING_INSTANCES};\n",
            "\tint32 GetBuildingInstanceCounts() const;\n",
            ...
            "\tbool GetBuildingInstanceCount() const;\n",
            "\tFName GetBuildingInstanceCount() const;\n",
            f"\t{TARGET_WRONG_BODY}\n",
            f"\t{TARGET_WRONG_SPEC_FIELD}\n",
            "\tfloat GetBuildingInstanceCount() const { return "
            + forty
            + "; }\n",
            "\tfloat GetBuildingInstanceCount() const { return "
            + eighty
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tint32\n\tGetBuildingInstanceCount() const;\n",
            "\tint32   GetBuildingInstanceCount() const;\n",
            "\tint32\tGetBuildingInstanceCount() const;\n",
            "\tint32 GetBuildingInstanceCount(\n\t) const;\n",
            "\tint32 GetBuildingInstanceCount()  const;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UFUNCTION_PURE_THEATER}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit GetBuildingInstanceCount decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "get_building_instance_count_decl_contract.py"
        ))
