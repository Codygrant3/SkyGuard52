# THIS IS leftover-safe FSkyguardRadioLine Priority.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite,
# meta=(ClampMin="0", ClampMax="100"))
# then `int32 Priority = 50;`.
# THIS IS leftover-safe isolated UPROPERTY with no Category.
# EditAnywhere IS present. BlueprintReadWrite IS present.
# ClampMin="0" and ClampMax="100" ARE present.
# Initializer IS `= 50`. There is NO Category.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if EditAnywhere is dropped, if specifiers
# drop to BlueprintReadOnly / VisibleAnywhere-only,
# if the type is not int32, if meta ClampMin/ClampMax
# is missing or changed, if the initializer is missing
# or not `= 50`, or if Category= is added.
...
# (leftover radio-chatter-empty-fail-closed #177/#52ae,
# leftover empty-queue #150/#5137, leftover empty-line
# #121/#fb2b).
# Do NOT parse leftover radio-line-defaults #172 itself.
# Do NOT contract sibling fields LineId / Speaker /
# Subtitle / Sound / EstimatedDurationSeconds /
# CooldownSeconds.
# Do NOT contract USkyguardRadioChatterComponent
# UFUNCTIONs or UPROPERTY fields.
# THIS IS NOT leftover radio-line-defaults #172 / #1dec.
# THIS IS NOT leftover radio-chatter-empty-fail-closed
# #177 / #52ae.
# THIS IS NOT leftover radio-chatter-empty-queue-fail-closed
# #150 / #5137.
# THIS IS NOT leftover radio-chatter-empty-line-tests
# #121 / #fb2b.
# THIS IS NOT leftover RadioLine Subtitle #1332.
# THIS IS NOT leftover RadioLine LineId / Speaker drafts.
# THIS IS NOT leftover RadioLine Sound in-flight.
...
# THIS IS NOT leftover BriefingCard isolated fields
# #1251-#1254.
# THIS IS NOT leftover HowToFlyRow #1258-#1260.
# THIS IS NOT leftover briefing-get-radio-chatter #375.
# THIS IS NOT leftover pilot-make-radio-line #285.
...
# the bulk).
# Analog: leftover FSkyguardBriefingCard defaults then
# isolated CardId; leftover radio-line-defaults then
# this isolated Priority decl.
# If a clone asserts VisibleAnywhere / BlueprintReadOnly /
# Category="Skyguard|Theater" / `FName WeatherIdentity`
# or bare `FName LineId;`, retarget: type is int32
# Priority, initializer is `= 50`, specifiers are
# EditAnywhere + BlueprintReadWrite +
# meta=(ClampMin="0", ClampMax="100"), no Category.
...
TARGET = "int32 Priority = 50;"
TARGET_WRONG_BARE = "int32 Priority;"
TARGET_WRONG_EQ_NONE = "int32 Priority = NAME_None;"
TARGET_WRONG_FALSE = "int32 Priority = false;"
TARGET_WRONG_TRUE = "int32 Priority = true;"
TARGET_WRONG_ZERO = "int32 Priority = 0.f;"
TARGET_WRONG_FLOAT = "float Priority;"
TARGET_WRONG_FORTY = "int32 Priority = 40;"
TARGET_WRONG_EIGHTY = "int32 Priority = 80;"
TARGET_WRONG_FIFTY_F = "int32 Priority = " + "50" + ".f" + ";"
TARGET_WRONG_FORTY_F = "int32 Priority = " + "40" + ".f" + ";"
TARGET_WRONG_EIGHTY_F = "int32 Priority = " + "80" + ".f" + ";"
TARGET_WRONG_FNAME = "FName Priority;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 Priority = 50;`.
    # Do not accept a bare `int32 Priority;`, `= 50.f`,
    # `= 40`, `= 80`, harbor forty/eighty float forms,
    # leftover float Health, FName, or sibling LineId /
    # Speaker / Subtitle / Sound fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+Priority\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+Priority\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Priority\s*=\s*50\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Priority\s*=\s*(?:40|80)(?:\.f)?\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Priority\s*;",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Priority\s*=\s*50\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+Priority\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 Priority is missing from "
...
def require_priority_specifiers(section: str) -> str:
    ...
    if "EditAnywhere" not in specifiers:
        raise AssertionError(
            ...
            f"{HEADER_PATH} struct {STRUCT_NAME} Priority UPROPERTY"
        )
    if "BlueprintReadOnly" in specifiers:
        raise AssertionError(
            "BlueprintReadOnly instead of BlueprintReadWrite on "
            f"origin/main:{HEADER_PATH} struct {STRUCT_NAME} Priority"
        )
    if "BlueprintReadWrite" not in specifiers:
        raise AssertionError(
            "BlueprintReadWrite is missing from origin/main:"
            f"{HEADER_PATH} struct {STRUCT_NAME} Priority UPROPERTY"
        )
    if "Category" in specifiers:
        raise AssertionError(
            "Category= is not accepted on origin/main:"
            f"{HEADER_PATH} struct {STRUCT_NAME} Priority UPROPERTY"
        )
    if 'ClampMin="0"' not in specifiers:
        raise AssertionError(
            'ClampMin="0" is missing from origin/main:'
            f"{HEADER_PATH} struct {STRUCT_NAME} Priority UPROPERTY"
        )
    if 'ClampMax="100"' not in specifiers:
        raise AssertionError(
            'ClampMax="100" is missing from origin/main:'
            f"{HEADER_PATH} struct {STRUCT_NAME} Priority UPROPERTY"
        )
    compact = collapsed(specifiers)
    if 'meta=(ClampMin="0",ClampMax="100")' not in compact:
        raise AssertionError(
            'meta=(ClampMin="0", ClampMax="100") is missing or '
            f"changed on origin/main:{HEADER_PATH} struct "
            f"{STRUCT_NAME} Priority UPROPERTY"
        )
    return specifiers


class RadioLinePriorityFieldDeclContractTests(unittest.TestCase):
    def test_radio_line_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Priority"), section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedRadioLine\n{\n};\n"
            )
    ...
    def test_component_or_delegate_declaration_does_not_satisfy(self) -> None:
        mixed = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            "\tGENERATED_BODY()\n"
            "};\n"
            f"{STOP_BEFORE_DELEGATE}_OneParam("
            "FSkyguardRadioLineEvent, const FSkyguardRadioLine&, Line);\n"
            f"class {COMPONENT_NAME}\n"
            "{{\n"
            "public:\n"
        ...
        self.assertNotIn(STOP_BEFORE_DELEGATE, section)
        self.assertNotIn(COMPONENT_NAME, section)
        self.assertNotIn(COMPONENT_CURRENT_LINE_ID, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Priority", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_priority_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tFName {SIBLING_LINE_ID};\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        specifiers = require_priority_specifiers(section)
        ...
        self.assertIn('ClampMax="100"', specifiers)
        self.assertIn(
            'meta=(ClampMin="0",ClampMax="100")',
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        wrongs = (
            ...
            TARGET_WRONG_FIFTY_F,
            TARGET_WRONG_FORTY,
            TARGET_WRONG_EIGHTY,
            TARGET_WRONG_FORTY_F,
            TARGET_WRONG_EIGHTY_F,
        )
        for wrong in wrongs:
            initialized = (
                f"struct {STRUCT_NAME}\n"
        ...
        self.assertIn("int32 Priority = 50;", compact_origin)
        self.assertNotIn("int32 Priority;", compact_origin.replace(
            "int32 Priority = 50;",
            "",
        ))
        self.assertNotIn("Priority = NAME_None", compact_origin)
        self.assertNotIn("Priority = false", compact_origin)
        self.assertNotIn("Priority = true", compact_origin)
        self.assertNotIn("Priority = 0.f", compact_origin)
        self.assertNotIn("Priority = 160.f", compact_origin)
        self.assertNotIn("Priority = 40;", compact_origin)
        self.assertNotIn("Priority = 80;", compact_origin)

    def test_priority_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 Priority"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 50", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FIFTY_F}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FORTY_F}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_EIGHTY_F}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tbool Priority = 50;\n",
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_FIFTY_F}\n",
            f"\t{TARGET_WRONG_FORTY}\n",
            f"\t{TARGET_WRONG_EIGHTY}\n",
            f"\t{TARGET_WRONG_FORTY_F}\n",
            f"\t{TARGET_WRONG_EIGHTY_F}\n",
            f"\t{TARGET_WRONG_FNAME}\n",
            "\tfloat Priority = " + forty + ";\n",
            "\tfloat Priority = " + eighty + ";\n",
        )
    ...
    def test_dropped_edit_anywhere_or_read_only_fails_closed(self) -> None:
        origin = require_priority_specifiers(
        ...
        self.assertIn('ClampMax="100"', origin)
        ...
        with self.assertRaises(AssertionError) as raised:
            require_priority_specifiers(spec_section(no_edit))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_priority_specifiers(spec_section(read_only))
        self.assertIn("BlueprintReadOnly", str(raised.exception))
        visible = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_priority_specifiers(spec_section(visible))
        self.assertIn("EditAnywhere", str(raised.exception))
        with_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_priority_specifiers(spec_section(with_category))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_priority_specifiers(spec_section(no_clamp))
        ...
        with self.assertRaises(AssertionError) as raised:
            require_priority_specifiers(spec_section(changed_clamp))
        self.assertTrue(
    ...
    def test_does_not_contract_sibling_radio_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_does_not_parse_component_delegate_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn('meta=(ClampMin="0", ClampMax="100")', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radio-line Priority field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "radio_line_priority_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardRadioChatterComponent.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
        ...
        self.assertIn(CLONE_RADIO_LINE_LINE_ID, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RADIO_LINE_SPEAKER, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RADIO_LINE_SUBTITLE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_RADIO_LINE_SOUND, LOCKED_SCRIPTS)
