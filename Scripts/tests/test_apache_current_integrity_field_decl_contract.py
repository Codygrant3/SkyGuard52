# THIS IS leftover-safe ASkyguardApacheAircraft CurrentIntegrity.
...
# then `float CurrentIntegrity = 140.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Apache|Damage`. VisibleAnywhere IS present.
# BlueprintReadOnly IS present. Default IS `= 140.f`.
...
# Parse CLASS `ASkyguardApacheAircraft` public UPROPERTY
# fields ONLY after `class ASkyguardApacheAircraft`.
# Stop BEFORE `protected:`.
# Stop BEFORE sibling HullCollider as a claimed slot.
# Do NOT claim sibling MaxIntegrity.
...
# Do NOT parse leftover ASkyguardProtectAsset CurrentIntegrity
...
# or leftover LoadoutSpec HullIntegrity (140.f analog;
# identifier is not CurrentIntegrity).
...
# Skyguard|Apache|Damage NOT Skyguard|Theater.
# LOCKED_DECL is `float CurrentIntegrity = 140.f;` NOT
...
# no initializer AS THE LOCKED DECL.
...
TARGET = "float CurrentIntegrity = 140.f;"
...
TARGET_WRONG_PROTECT = "float CurrentIntegrity = 100.f;"
...
STOP_BEFORE_HULL_COLLIDER = "HullCollider"
...
CURRENT_INTEGRITY_DECL_RE = re.compile(
    r"float\s+CurrentIntegrity\s*=\s*140\.f\s*;"
)
CURRENT_INTEGRITY_BARE_RE = re.compile(
    r"float\s+CurrentIntegrity\s*;"
)
CURRENT_INTEGRITY_WRONG_INIT_RE = re.compile(
    r"float\s+CurrentIntegrity\s*=\s*(?!140\.f\b)"
)
INVENTED_UPROPERTY = (
    "EditAnywhere",
    "BlueprintReadWrite",
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float CurrentIntegrity = 140.f;`.
    # Do not accept leftover bare FName WeatherIdentity,
    # leftover `= 160.f`, leftover ProtectAsset `= 100.f`,
    # leftover LoadoutSpec HullIntegrity, leftover
    # MaxIntegrity, or leftover HullCollider as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CURRENT_INTEGRITY_DECL_RE.search(compact) is None:
        return False
...
def public_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_HULL_COLLIDER):
            break
    ...
    if has_identifier(window, SIBLING_HULL_COLLIDER):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_HULL_COLLIDER}"
        )
...
class ApacheCurrentIntegrityFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CurrentIntegrity"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PROTECTED, section)
        self.assertTrue(has_identifier(body, SIBLING_HULL_COLLIDER), body)
        self.assertFalse(
            has_identifier(section, SIBLING_HULL_COLLIDER),
            section,
        )
    ...
    def test_protect_asset_same_identifier_does_not_satisfy(self) -> None:
        mixed = (
            f"class {LEFTOVER_PROTECT_ASSET_CLASS}\n"
            "{\n"
            "public:\n"
        ...
        self.assertFalse(has_identifier(section, "CurrentIntegrity"), section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_current_integrity_declaration_fails_closed(self) -> None:
        empty = (
            ...
            "public:\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
            f"\tfloat {SIBLING_MAX_INTEGRITY} = 140.f;\n"
            f"{STOP_BEFORE_PROTECTED}\n"
            "};\n"
        )
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float CurrentIntegrity = 140.f;", compact_origin)
        self.assertNotIn("CurrentIntegrity = 160.f", compact_origin)
        self.assertNotIn("CurrentIntegrity = 100.f", compact_origin)
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float CurrentIntegrity = 140.f;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat CurrentIntegrities = 140.f;\n",
            "\tint32 CurrentIntegrity = 140;\n",
            "\tbool CurrentIntegrity = true;\n",
            "\tfloat CurrentIntegrity = " + forty + ";\n",
            "\tfloat CurrentIntegrity = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_max_or_hull_collider(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache CurrentIntegrity field decl contract "
