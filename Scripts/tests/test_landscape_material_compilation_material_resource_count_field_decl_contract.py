...
# MaterialResourceCount FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# int32 MaterialResourceCount = 0; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeMaterialCompilationResult
# MaterialResourceCount. THIS IS NOT leftover VisibleAudit bSuccess #1199.
...
# MaterialResourceCount, NOT leftover
...
# Do NOT parse FSkyguardLandscapeVisibleAudit (leftover bSuccess #1199).
# THIS IS leftover-safe FSkyguardLandscapeMaterialCompilationResult
# MaterialResourceCount. THIS IS NOT leftover
...
# THIS IS leftover-safe FSkyguardLandscapeMaterialCompilationResult
# MaterialResourceCount FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly int32 MaterialResourceCount = 0; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# BlueprintReadOnly int32 MaterialResourceCount = 0;
...
TARGET = "int32 MaterialResourceCount = 0;"
TARGET_WRONG = "int32 MaterialResourceCount = 1;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # MaterialResourceCount (must be = 0, not = 1).
    # THIS IS NOT leftover VisibleAudit bSuccess #1199.
    # THIS IS NOT leftover FootprintSampleResult bSuccess #1189.
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*int32\s+MaterialResourceCount\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for int32 MaterialResourceCount is missing from "
...
class LandscapeMaterialCompilationMaterialResourceCountFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("MaterialResourceCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_material_resource_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_material_resource_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 MaterialResourceCount"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("MaterialResourceCount", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("MaterialResourceCount", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("int32 MaterialResourceCount"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("MaterialResourceCount", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_material_resource_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
