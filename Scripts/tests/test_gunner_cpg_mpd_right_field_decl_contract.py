# THIS IS leftover-safe ASkyguardGunner CpgMpdRight.
...
# then BARE `TObjectPtr<UStaticMeshComponent> CpgMpdRight;`
# (no initializer).
...
# Stop BEFORE sibling CpgEufd as a claimed slot.
# Do NOT claim CpgStation, CpgCockpit, CpgSeatBack,
# CpgSeatPan, CpgKneeLeft, CpgKneeRight, CpgCanopyBow,
# CpgDash, CpgTedac, CpgMpdLeft, CpgEufd, CpgTedacBezel,
# CpgMpdRightText, Boom, GunnerCamera.
# Do NOT claim leftover CpgCanopyBow / CpgDash /
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgCockpit /
# CpgStation (in-flight leftovers). CpgTedac and
# CpgMpdLeft are sibling in-flight slots.
# Do NOT claim CpgMpdLeft, CpgMpdRightText, or CpgEufd
# as this slot.
# CpgMpdRight fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> CpgMpdRight;`
...
# CpgMpdRight fail-closed if LOCKED_DECL is leftover
# CpgKneeRight / CpgKneeLeft / CpgSeatBack / CpgCockpit /
# CpgStation / CpgSeatPan / CpgCanopyBow / CpgDash /
# CpgTedac / CpgMpdLeft / CpgEufd / leftover
...
TARGET = "TObjectPtr<UStaticMeshComponent> CpgMpdRight;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> CpgMpdRight = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> CpgMpdRight = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> CpgMpdRight = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgMpdRight;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgMpdRight;"
...
CPG_MPD_RIGHT_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgMpdRight\s*;"
)
CPG_MPD_RIGHT_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+CpgMpdRight\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # CpgTedac / CpgMpdLeft / leftover Apache Canopy, or
    # an initializer on CpgMpdRight as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_MPD_RIGHT_DECL_RE.search(compact) is None:
        return False
    if CPG_MPD_RIGHT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_EUFD):
            break
    ...
    if has_identifier(window, SIBLING_CPG_EUFD):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+CpgMpdRight\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> CpgMpdRight "
...
class GunnerCpgMpdRightFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgMpdRight"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_EUFD), body)
        self.assertFalse(
            has_identifier(section, SIBLING_CPG_EUFD),
            section,
        )
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgMpdRight"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgMpdRight", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_mpd_right_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> CpgMpdRight;", compact_origin)
        self.assertNotIn("CpgMpdRight = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_mpd_right_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> CpgMpdRight"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> CpgMpdRight;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> CpgMpdRights;\n",
            "\tint32 CpgMpdRight = 140;\n",
            "\tbool CpgMpdRight = true;\n",
            "\tfloat CpgMpdRight = " + forty + ";\n",
            "\tfloat CpgMpdRight = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_eufd_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgMpdRight field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_mpd_right_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
