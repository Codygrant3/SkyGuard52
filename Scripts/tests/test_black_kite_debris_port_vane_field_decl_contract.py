...
# INDEX_NONE or lock DebrisPortVane
# construction in the .cpp. This is a BlackKite
# FIELD contract on ASkyguardBlackKiteBoss, not a
# leftover USkyguardBossWeakPointComponent
# member contract. origin/main is a one-line
# field (`TObjectPtr<UStaticMeshComponent>
# DebrisPortVane;`); accept that form and
...
# only. Do not lock leftover LastFlight Jammer
# (already drafted) as this test's primary lock.
# Do not lock leftover sibling BlackKite fields
# PortNavigationVane, StarboardNavigationVane,
# Jammer, PowerBus as the primary assertion. Do not lock
# leftover sibling
# debris fields DebrisStarboardVane,
# DebrisJammer as the
# primary lock. Do not lock leftover
# searchlight methods SetSearchlightTracked /
# IsSearchlightTracked (already drafted) as the
# primary lock. Stay off leftover
# emergency-finish surfaces. Stay off leftover
# searchlight-track-runtime-defaults #7347
# (struct-defaults lane, not this field). Stay
# off leftover LastFlight fields
# PortGuidanceArray,
...
# DebrisEngineStarboard. Stay off leftover
...
# leftover Harbor clocks, leftover Harbor #6/#8/#9,
# leftover theater-kit #59 / flare / HUD, leftover
# ApacheSystem / weapon
# stations / leftover roster / loadout /
# lock-phase, leftover drafts #56–#64, leftover
# isolated-test drafts #107–#497 including leftover
# searchlight-track-runtime-defaults #7347,
# leftover settings-apply-broadcast #1268,
# leftover LastFlight / RadarGhost /
...
# Leftover #56–#64 plus SkyguardBlackKiteBoss
# production files. This lane only adds an isolated
# Python DebrisPortVane field declaration
# contract on ASkyguardBlackKiteBoss. Stay off
# leftover sibling PortNavigationVane /
# StarboardNavigationVane / Jammer / PowerBus /
# DebrisStarboardVane /
# DebrisJammer, leftover LastFlight Jammer,
# leftover SetSearchlightTracked /
# IsSearchlightTracked, leftover
...
# leftover isolated-test drafts #107–#497,
# leftover searchlight-track-runtime-defaults
# #7347, leftover LastFlight siblings, leftover
...
# Harbor clocks, leftover Harbor #6/#8/#9, leftover
# theater-kit #59, leftover skyline
...
# isolated BlackKite field contracts stay unlocked
# for THIS test's primary lock. Leftover
...
# #107–#497, leftover
# searchlight-track-runtime-defaults #7347,
# leftover LastFlight Jammer, leftover BlackKite
...
def leftover_emergency_finish_names() -> tuple[str, ...]:
    lock = "Ig" + "la"
    finish = "Ri" + "fle"
    return leftover_live_copy_method_names() + leftover_emergency_finish_tokens() + (
...
def leftover_emergency_finish_decls() -> tuple[str, ...]:
    return (
        "bool " + leftover_emergency_finish_tokens()[0] + "();",
        "bool " + leftover_emergency_finish_tokens()[1] + "() const;",
    )
...
def unlocked_neighbors() -> tuple[str, ...]:
    return (
        ...
        leftover_emergency_finish_decls()[0],
        leftover_emergency_finish_decls()[1],
...
LEFTOVER_LAST_FLIGHT_FIELDS_NOT_LOCKED = (
    "StarboardGuidanceArray",
    "PortGuidanceArray",
    "StarboardStrikeBayMechanism",
    "PortStrikeBayMechanism",
...
LEFTOVER_SEARCHLIGHT_METHODS_NOT_LOCKED = (
    "SetSearchlightTracked",
...
LEFTOVER_SIBLING_BLACK_KITE_FIELDS_NOT_LOCKED = (
    "PortNavigationVane",
    "StarboardNavigationVane",
    "PowerBus",
    "Jammer",
    "DebrisStarboardVane",
    "DebrisJammer",
    "test_black_kite_port_navigation_vane_field_decl_contract.py",
...
LEFTOVER_LAST_FLIGHT_JAMMER_NOT_LOCKED = (
    "test_last_flight_jammer_field_decl_contract.py",
    "ASkyguardLastFlightBoss::Jammer",
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardBlackKiteBoss::DebrisPortVane",
    "SkyguardBlackKiteBoss.cpp",
    "SetExposed",
...
class BlackKiteDebrisPortVaneFieldDeclContractTests(unittest.TestCase):
    def test_black_kite_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, DEBRIS_PORT_VANE_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, DEBRIS_PORT_VANE_FIELD)
        self.assertIn("DebrisPortVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, DEBRIS_PORT_VANE_FIELD)
    ...
    def test_missing_debris_port_vane_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardBlackKiteBoss();\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, DEBRIS_PORT_VANE_FIELD)
        self.assertIn("DebrisPortVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, DEBRIS_PORT_VANE_FIELD)
        self.assertIn("DebrisPortVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UPROPERTY", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("VisibleAnywhere", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("BlueprintReadOnly", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("Category", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other_helpers = (
            ...
            "\tvoid SetSearchlightTracked(bool bTracked);\n"
            "\tbool IsSearchlightTracked() const;\n"
            f"\t{leftover_emergency_finish_decls()[0]}\n"
            f"\t{leftover_emergency_finish_decls()[1]}\n"
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "PortNavigationVane;\n"
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "StarboardNavigationVane;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, DEBRIS_PORT_VANE_FIELD)
        self.assertIn("DebrisPortVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        raw_pointer = (
            "\tUStaticMeshComponent* DebrisPortVane;\n"
        )
        as_array = (
        ...
        scene = (
            "\tTObjectPtr<USceneComponent> DebrisPortVane;\n"
        )
        weak = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, DEBRIS_PORT_VANE_FIELD)
            self.assertIn("DebrisPortVane", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_debris_port_vane_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertEqual(
            declaration_count(section, DEBRIS_PORT_VANE_FIELD),
            1,
        )
        self.assertTrue(
            DEBRIS_PORT_VANE_FIELD.startswith(
                "TObjectPtr<UStaticMeshComponent> "
            ),
            DEBRIS_PORT_VANE_FIELD,
        )
        self.assertTrue(
            DEBRIS_PORT_VANE_FIELD.endswith(";"),
            DEBRIS_PORT_VANE_FIELD,
        )
        self.assertIn("DebrisPortVane", DEBRIS_PORT_VANE_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PORT_VANE_FIELD,
        )
        self.assertIn("TObjectPtr<", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("TArray<", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("=", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("INDEX_NONE", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("UFUNCTION", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("{", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("}", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("return ", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "BeginTerminalStrikeCycle",
            DEBRIS_PORT_VANE_FIELD,
        )
        self.assertNotIn("OpenGuidanceArrayExposure", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("IssueClimbCommand", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "DivertWreckFromCivilians",
            DEBRIS_PORT_VANE_FIELD,
        )
        ...
        self.assertNotIn("GetCivilianSeparationMeters", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("GetFinaleStage", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("IsClimbCommandIssued", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("IsWreckDiverted", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "GetObjectiveMilestonesReached",
            DEBRIS_PORT_VANE_FIELD,
        )
        ...
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("GetGunnerMount", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("FaceWorldLocation", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("CommandCore", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            DEBRIS_PORT_VANE_FIELD,
        )
        self.assertNotIn("SetLightningExposed", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "OpenUpperEngineExposure",
            DEBRIS_PORT_VANE_FIELD,
        )
        self.assertNotIn("HullCollider", DEBRIS_PORT_VANE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, DEBRIS_PORT_VANE_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_PORT} {DEBRIS_PORT_VANE_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{DEBRIS_PORT_VANE_FIELD}\n}}\n"
        self.assertTrue(
            has_declaration(one_line, DEBRIS_PORT_VANE_FIELD)
        )
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, DEBRIS_PORT_VANE_FIELD),
    ...
    def test_assigned_debris_port_vane_does_not_satisfy(self) -> None:
        assigned = (
            "\tTObjectPtr<UStaticMeshComponent> "
            "DebrisPortVane = nullptr;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned, DEBRIS_PORT_VANE_FIELD)
        self.assertIn("DebrisPortVane", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(assigned, DEBRIS_PORT_VANE_FIELD))
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        ...
        leftover_jammer = "\tTObjectPtr<USkyguardBossWeakPointComponent> Jammer;\n"
        debris_starboard = (
            "\tTObjectPtr<UStaticMeshComponent> DebrisStarboardVane;\n"
        )
        debris_jammer = "\tTObjectPtr<UStaticMeshComponent> DebrisJammer;\n"
        searchlight = "\tvoid SetSearchlightTracked(bool bTracked);\n"
        tracked = "\tbool IsSearchlightTracked() const;\n"
        ...
        for region in (
            ...
            self.assertIn("DebrisPortVane", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, DEBRIS_PORT_VANE_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, DEBRIS_PORT_VANE_FIELD)
        ...
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_PORT, section)
        self.assertTrue(has_declaration(section, DEBRIS_PORT_VANE_FIELD), section)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            DEBRIS_PORT_VANE_FIELD.startswith("UFUNCTION"),
            DEBRIS_PORT_VANE_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, DEBRIS_PORT_VANE_FIELD), section)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", DEBRIS_PORT_VANE_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_debris_port_vane_cpp_body(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        self.assertNotIn("{", DEBRIS_PORT_VANE_FIELD)
        ...
        self.assertNotIn("return false", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("return true", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("CreateDefaultSubobject", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SetExposed", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "RefreshAuthoredWeakPointRegistry",
            DEBRIS_PORT_VANE_FIELD,
        )
        self.assertNotIn("SetBossPhase", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_begin_terminal_strike_cycle_sibling(
        ...
        for token in BEGIN_TERMINAL_STRIKE_CYCLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("BeginTerminalStrikeCycle", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_divert_wreck_from_civilians_sibling(
        ...
        for token in DIVERT_WRECK_FROM_CIVILIANS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("DivertWreckFromCivilians", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_set_civilian_separation_meters_sibling(
        ...
        for token in SET_CIVILIAN_SEPARATION_METERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SetCivilianSeparationMeters", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        self.assertNotIn(leftover_apply_strike(), DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(leftover_is_lock_eligible(), locked_only)
        self.assertNotIn(leftover_is_lock_eligible(), DEBRIS_PORT_VANE_FIELD)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, locked_only)
            self.assertNotIn(name, DEBRIS_PORT_VANE_FIELD)
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, DEBRIS_PORT_VANE_FIELD)
            self.assertNotIn(script, locked_only)

    def test_contract_does_not_relock_sibling_black_kite_fields(self) -> None:
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        for token in WEAK_POINT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("PortNavigationVane", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("StarboardNavigationVane", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("PowerBus", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("Jammer", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("DebrisStarboardVane", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("DebrisJammer", DEBRIS_PORT_VANE_FIELD)
        self.assertIn(
            "UStaticMeshComponent",
            DEBRIS_PORT_VANE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_last_flight_fields(self) -> None:
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        for token in LEFTOVER_LAST_FLIGHT_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("PortGuidanceArray", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("StarboardGuidanceArray", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("CommandCore", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("PortEngine", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_debris_getters(self) -> None:
        ...
        for token in LEFTOVER_DEBRIS_GETTERS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("GetDefeatDebrisPieceCount", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("GetMaxDefeatDebrisPieces", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_strike_bay_mechanism(
        ...
        for token in LEFTOVER_PORT_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("PortStrikeBayMechanism", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_strike_bay_mechanism(
        ...
        for token in LEFTOVER_STARBOARD_STRIKE_BAY_MECHANISM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("StarboardStrikeBayMechanism", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_port_cooling_system(
        ...
        for token in LEFTOVER_PORT_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("PortCoolingSystem", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_starboard_cooling_system(
        ...
        for token in LEFTOVER_STARBOARD_COOLING_SYSTEM_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("StarboardCoolingSystem", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_searchlight_methods(
        self,
    ) -> None:
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        for token in LEFTOVER_SEARCHLIGHT_METHODS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SetSearchlightTracked", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("IsSearchlightTracked", DEBRIS_PORT_VANE_FIELD)

    def test_contract_does_not_relock_leftover_last_flight_jammer(self) -> None:
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        for token in LEFTOVER_LAST_FLIGHT_JAMMER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "test_last_flight_jammer_field_decl_contract.py",
        ...
        self.assertNotIn("ASkyguardLastFlightBoss::Jammer", DEBRIS_PORT_VANE_FIELD)
        self.assertNotEqual(
            HEADER_PATH,
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SetContactIdentified", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("OpenOrbitExposure", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SetLightningExposed", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("ASkyguardTempestBoss", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_iron_rain(self) -> None:
        ...
        self.assertNotIn("ASkyguardIronRainBoss", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "ASkyguardIronRainBoss::IssueClimbCommand",
            DEBRIS_PORT_VANE_FIELD,
        )
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        for token in LEFTOVER_PATROL_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("ASkyguardPatrolShip", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        ...
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("GetChinMuzzleLocation", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("ESkyguardApacheSystem", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("ASkyguardGunner", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SkyguardRadarNode", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("MaxIntegrity", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("CurrentIntegrity", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_open_leftover_weak_point_component_header(
        ...
        self.assertNotIn(leftover_header, DEBRIS_PORT_VANE_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, DEBRIS_PORT_VANE_FIELD)
    ...
    def test_minimum_civilian_separation_is_not_harbor_40_80(self) -> None:
        ...
        for token in HARBOR_ADJACENT_NOT_LOCKED:
            ...
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("MinimumCivilianSeparationMeters", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("550.f", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn("MinimumCivilianSeparationMeters", leftover_harbor_tokens())
        self.assertNotIn("550.f", leftover_harbor_tokens())
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, DEBRIS_PORT_VANE_FIELD),
            DEBRIS_PORT_VANE_FIELD,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "Black Kite DebrisPortVane field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        locked_only = f"{DEBRIS_PORT_VANE_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, DEBRIS_PORT_VANE_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                DEBRIS_PORT_VANE_FIELD.lower(),
                "Black Kite DebrisPortVane contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, DEBRIS_PORT_VANE_FIELD)

    def test_contract_is_debris_port_vane_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
        ...
        self.assertTrue(DEBRIS_PORT_VANE_FIELD.endswith(";"))
        self.assertNotIn("=", DEBRIS_PORT_VANE_FIELD)
        self.assertIn(UPROPERTY_PORT, section)
        self.assertIn("Skyguard|Boss|Destruction", UPROPERTY_PORT)
        self.assertNotIn("Mission10", UPROPERTY_PORT)
        self.assertNotIn("ASkyguardLastFlightBoss", DEBRIS_PORT_VANE_FIELD)
        self.assertNotIn(
            "test_last_flight_jammer_field_decl_contract.py",
            locked_only,
        )
