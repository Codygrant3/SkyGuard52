# THIS IS leftover-safe FSkyguardProductionAudioEntry Attenuation.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundAttenuation> Attenuation;`
# (no initializer).
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Bare TSoftObjectPtr<USoundAttenuation>,
# no initializer.
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# TSoftObjectPtr<USoundAttenuation>, initializer added,
# Category= added.
...
# SourceStatus / Sound / Concurrency / OutputSubmix /
# ProvenanceId / SourceSha256.
# Analog leftover production-audio-entry-defaults #198 /
# #314b then isolated Attenuation field decl.
...
# retarget: type is TSoftObjectPtr<USoundAttenuation>,
...
TARGET = "TSoftObjectPtr<USoundAttenuation> Attenuation;"
TARGET_WRONG_EQ_NONE = (
    "TSoftObjectPtr<USoundAttenuation> Attenuation = nullptr;"
)
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundAttenuation> Attenuation = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundAttenuation> Attenuation = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundAttenuation> Attenuation = 0.f;"
TARGET_WRONG_FLOAT = "float Attenuation;"
TARGET_WRONG_FNAME = "FName Attenuation;"
TARGET_WRONG_BOOL = "bool Attenuation;"
TARGET_WRONG_INT = "int32 Attenuation;"
TARGET_WRONG_FTEXT = "FText Attenuation;"
TARGET_WRONG_SOUND = "TSoftObjectPtr<USoundBase> Attenuation;"
TARGET_WRONG_OBJECTPTR = "TObjectPtr<USoundAttenuation> Attenuation;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if re.search(
        r"TSoftObjectPtr<USoundAttenuation>\s+Attenuation\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Attenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+Attenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+Attenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Attenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+Attenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr<USoundBase>\s+Attenuation\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr<USoundAttenuation>\s+Attenuation\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr<USoundAttenuation>\s+Attenuation\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundAttenuation> Attenuation "
        "is missing from "
...
class ProductionAudioEntryAttenuationFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_attenuation_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("Attenuation = nullptr", compact_origin)
        self.assertNotIn("Attenuation = false", compact_origin)
        self.assertNotIn("Attenuation = true", compact_origin)
        self.assertNotIn("Attenuation = 0.f", compact_origin)
        self.assertNotIn("Attenuation = 160.f", compact_origin)

    def test_attenuation_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertIn("TSoftObjectPtr<USoundAttenuation>", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<USoundAttenuation> Attenuations;\n",
            "\tint32 Attenuation = 1;\n",
            "\tbool Attenuation;\n",
            "\tfloat Attenuation = " + forty + ";\n",
            "\tfloat Attenuation = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry Attenuation field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "attenuation_field_decl_contract.py"
        ))
