...
# INDEX_NONE or lock GetClassifiedFalseTrackCount
# construction in the .cpp. This is leftover-safe
# Mission07 Search GetClassifiedFalseTrackCount METHOD on
...
# an isolated Python GetClassifiedFalseTrackCount
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission07IntegrationDirector::GetClassifiedFalseTrackCount",
    "SkyguardMission06IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission07GetClassifiedFalseTrackCountDeclContractTests(unittest.TestCase):
    def test_mission07_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_CLASSIFIED_FALSE_TRACK_COUNT),
            section,
        )
    ...
    def test_other_class_does_not_satisfy(self) -> None:
        other = (
            ...
            "{\n"
            "public:\n"
            f"\t{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
            "};\n"
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn("GetClassifiedFalseTrackCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_missing_get_classified_false_track_count_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission07IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn("GetClassifiedFalseTrackCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn("GetClassifiedFalseTrackCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("BlueprintPure", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("Category", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn("GetClassifiedFalseTrackCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetClassifiedFalseTrackCount() const;\n"
        )
        as_bool = "\tbool GetClassifiedFalseTrackCount() const;\n"
        missing_const = (
            "\tint32 GetClassifiedFalseTrackCount();\n"
        )
        with_arg = (
            "\tint32 "
            "GetClassifiedFalseTrackCount(int32 Count) const;\n"
        )
        renamed = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_CLASSIFIED_FALSE_TRACK_COUNT)
            self.assertIn("GetClassifiedFalseTrackCount", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_classified_false_track_count_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_CLASSIFIED_FALSE_TRACK_COUNT),
            GET_CLASSIFIED_FALSE_TRACK_COUNT,
        )
        self.assertTrue(has_declaration(section, GET_CLASSIFIED_FALSE_TRACK_COUNT))
        self.assertEqual(
            declaration_count(section, GET_CLASSIFIED_FALSE_TRACK_COUNT),
            1,
        )
        self.assertTrue(
            GET_CLASSIFIED_FALSE_TRACK_COUNT.startswith("int32 "),
            GET_CLASSIFIED_FALSE_TRACK_COUNT,
        )
        self.assertIn("int32", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn("GetClassifiedFalseTrackCount", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn(";", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn("()", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("TArray<", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("TObjectPtr", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("TSoftObjectPtr", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("INDEX_NONE", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("UFUNCTION", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("{", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("}", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("return ", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn(" const", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("Root", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("Briefing", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AudioDirector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("RadioChatter", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("SortiePresentation", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("CampaignDefinition", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("MissionDefinition", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("Readiness", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("bAutoInitialize", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("PathfinderSpawnLocation", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetAircraft", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("BindRuntimeActors", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("HandleDroneCityImpact", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("InitializePlayableMission", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("StartNextWave", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AdvanceConvoyByDistance", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("NotifyConvoyDamage", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("IsCorePlayableReady", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetConvoyRouteState", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetDayBeatKit", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetNightBeatKit", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("StartSearchlightWindow", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AdvanceSearchlightTrack", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("NotifySubstationDamage", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetRemainingThreatsInWave", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetSearchlightRuntime", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetSubstationIntegrity", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("StartPayloadWindow", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AdvancePayloadWindow", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("TryJamActivePayload", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("NotifyAirfieldTargetDamage", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetPayloadWindow", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetTargetRuntime", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetSurvivingTargetCount", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetDayBeatKind", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetDayBeatIndex", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("TickDayBeatKit", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("HandleBossPhaseChanged", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardMission04IntegrationDirector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardMission05IntegrationDirector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardMission06IntegrationDirector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ESkyguardMission06WaveState", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ESkyguardMission07ProtectedTarget", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("FSkyguardMission07ProtectedTargetRuntime", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ESkyguardSearchSector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("FSkyguardSearchTrackRuntime", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ClassifyFalseTrack", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ConfirmRadarGhostIdentification", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("NotifyProtectedTargetDamage", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AdvanceReinforcementTimer", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetSearchSector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetWaveState", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("IsHostileContactConfirmed", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetProtectedTarget", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetReinforcementTimeRemaining", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetNightBeatKind", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetNightBeatIndex", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("TickNightBeatKit", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ESkyguardAirfieldTarget", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("FSkyguardPayloadWindowRuntime", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("RoadHunterSpawnLocation", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ConfigureMissionDefinition", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("NotifyObjectiveProgress", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("SynchronizeRuntimeState", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("NotifyThreatDestroyed", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetReadiness", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetObjectiveRuntime", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetGunner", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetPathfinder", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetMissionId", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ValidateMissionContract", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ConfigureFromMission", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AdvanceBriefing", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("SetAssetsReady", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AcknowledgeAndLaunch", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("CanLaunch", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetElapsedSeconds", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetBriefingState", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetBriefingText", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetRadioChatter", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetPresentation", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("GetMissionTitle", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("AcknowledgeBriefing", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("LaunchSortie", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("HullCollider", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("OpticalTracker", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("WeaponServo", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("CountermeasurePod", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("MinHeightFromOriginCm", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("RadarNode", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ESkyguardMission02WaveState", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("HarborIndustrial", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("MaxIntegrity", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("CurrentIntegrity", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("FillAndFinalize", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("FillAndFail", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tint32\n"
            "\tGetClassifiedFalseTrackCount() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_SEARCH} {GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_CLASSIFIED_FALSE_TRACK_COUNT))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_CLASSIFIED_FALSE_TRACK_COUNT),
            section,
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_classified_false_track_count_cpp_body(self) -> None:
        locked_only = f"{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
        self.assertNotIn("{", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("}", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("return ", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        ...
        self.assertNotIn("return false", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("return true", GET_CLASSIFIED_FALSE_TRACK_COUNT)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn("GetClassifiedFalseTrackCount", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertTrue(GET_CLASSIFIED_FALSE_TRACK_COUNT.startswith("int32 "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("CurrentIntegrity", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_CLASSIFIED_FALSE_TRACK_COUNT),
            GET_CLASSIFIED_FALSE_TRACK_COUNT,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("FSkyguardSearchlightTrackRuntime", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardApacheAircraft", section)
        self.assertNotIn("ASkyguardRadarNode", section)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_CLASSIFIED_FALSE_TRACK_COUNT)
        self.assertNotIn("ASkyguardIronRainBoss", section)
        self.assertNotIn("ASkyguardRadarGhostBoss", GET_CLASSIFIED_FALSE_TRACK_COUNT)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission07 GetClassifiedFalseTrackCount method contract "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_CLASSIFIED_FALSE_TRACK_COUNT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_CLASSIFIED_FALSE_TRACK_COUNT.lower(),
                "mission07 GetClassifiedFalseTrackCount contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_CLASSIFIED_FALSE_TRACK_COUNT)

    def test_contract_is_get_classified_false_track_count_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_classified_false_track_count(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_CLASSIFIED_FALSE_TRACK_COUNT)
            )
            self.assertNotEqual(GET_CLASSIFIED_FALSE_TRACK_COUNT, leftover)
        self.assertIn(
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetClassifiedFalseTrackCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_CLASSIFIED_FALSE_TRACK_COUNT))
        locked_only = f"{GET_CLASSIFIED_FALSE_TRACK_COUNT}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tint32 GetClassifiedFalseTrackCount() const { return 0; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission07_get_classified_false_track_count"
                "_decl_contract.py"
            )
        )
        self.assertIn(
