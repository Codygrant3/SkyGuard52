# THIS IS leftover-safe FSkyguardCpgDebriefSnapshot CannonReady.
...
# Exact identifier vs siblings RocketReady, GuidedReady,
# Score, Medal, ShotsFired, and Hits (same
# type+initializer). Do not claim sibling fields:
# bValid, bWon, MissionTitle, OutcomeNarrative, Score,
# Medal, ShotsFired, Hits, CargoPercent, bRadarDead,
# DestroyedSystems, SelectedLoadout, RocketReady,
# GuidedReady.
...
# RadarDead #1460 / DestroyedSystems #1461 /
# SelectedLoadout #1462.
...
TARGET = "int32 CannonReady = 0;"
TARGET_WRONG_NONE = "int32 CannonReady;"
TARGET_WRONG_FALSE = "int32 CannonReady = false;"
TARGET_WRONG_TRUE = "int32 CannonReady = true;"
TARGET_WRONG_FLOAT = "int32 CannonReady = 0.f;"
TARGET_WRONG_HUNDRED = "int32 CannonReady = 100;"
TARGET_WRONG_ONE = "int32 CannonReady = 1;"
...
TARGET_WRONG_ROCKET = "int32 RocketReady = 0;"
TARGET_WRONG_GUIDED = "int32 GuidedReady = 0;"
TARGET_WRONG_FNAME = "FName CannonReady;"
TARGET_WRONG_BOOL = "bool CannonReady = 0;"
TARGET_WRONG_FLOAT_TYPE = "float CannonReady = 0;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `int32 CannonReady = 0;`.
    # Do not accept missing initializer, `= 0.f`, `= false`,
    # `= 100`, sibling RocketReady / GuidedReady / Score /
    # Medal / ShotsFired / Hits, or a leftover theater
    # UPROPERTY wrap as the locked decl.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"int32\s+CannonReady\s*=\s*0\.f\b", compact):
        return False
    if re.search(r"int32\s+CannonReady\s*=\s*false\b", compact):
        return False
    if re.search(r"int32\s+CannonReady\s*=\s*true\b", compact):
        return False
    if re.search(r"int32\s+CannonReady\s*=\s*100\b", compact):
        return False
    if re.search(r"(?:float|bool|FName|FString)\s+CannonReady\b", compact):
        return False
    if re.search(r"int32\s+CannonReady\s*=\s*0\s*;", compact) is None:
        return False
    return True
...
class CpgDebriefSnapshotCannonReadyFieldDeclContractTests(unittest.TestCase):
    def test_cpg_debrief_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CannonReady"), section)
    ...
    def test_capture_function_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CannonReady"), section)
        self.assertNotIn(STOP_BEFORE_CAPTURE, section)
        ...
        self.assertIn("CannonReady", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cannon_ready_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_origin_main_cannon_ready_is_plain_int32_zero(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
        ...
        self.assertEqual(LOCKED_DECL, "int32 CannonReady = 0;")
        self.assertTrue(LOCKED_DECL.startswith("int32 CannonReady"), LOCKED_DECL)
        ...
        self.assertIn("int32 CannonReady = 0;", compact)
        self.assertNotIn("CannonReady = 0.f", compact)
        self.assertNotIn("CannonReady = false", compact)
        self.assertNotIn("CannonReady = 100", compact)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 CannonReadys = 0;\n",
            "\tint32 CannonReadyValue = 0;\n",
            "\tint32 CannonCount = 0;\n",
            "\tfloat CannonReady = " + forty + ";\n",
            "\tfloat CannonReady = " + eighty + ";\n",
        )
    ...
    def test_exact_identifier_vs_same_type_siblings(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CannonReady"), section)
        self.assertIn("CannonReady", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ROCKET)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_GUIDED)
        self.assertTrue(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-debrief-snapshot CannonReady field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "cannon_ready_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCpgDebrief.cpp", THIS_SCRIPT)
        ...
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_SHOTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_CARGO, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_RADAR, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_DESTROYED, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_CPG_DEBRIEF_SNAPSHOT_LOADOUT, LOCKED_SCRIPTS)
