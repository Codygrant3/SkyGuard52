...
# SourceStatus / Attenuation / Concurrency / OutputSubmix /
# ProvenanceId / SourceSha256.
# Analog leftover production-audio-entry-defaults #198 /
# #314b then isolated Sound field decl.
# THIS IS NOT leftover DisplayName (being drafted).
# THIS IS NOT leftover ProvenanceId (being drafted).
...
# BlueprintReadOnly / Category="Skyguard|Theater", retarget:
# type is TSoftObjectPtr<USoundBase>, no `=`, BlueprintReadWrite,
# no Category.
...
TARGET = "TSoftObjectPtr<USoundBase> Sound;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundBase> Sound = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundBase> Sound = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundBase> Sound = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundBase> Sound = 0.f;"
TARGET_WRONG_FLOAT = "float Sound;"
TARGET_WRONG_FNAME = "FName Sound;"
TARGET_WRONG_BOOL = "bool Sound;"
TARGET_WRONG_INT = "int32 Sound;"
TARGET_WRONG_FTEXT = "FText Sound;"
TARGET_WRONG_PTR = "USoundBase* Sound;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundBase> Sound;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundWave> Sound;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling Category / DisplayName / SourceStatus fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+Sound\s*=",
        compact,
    ):
        return False
    if re.search(
    ...
    if re.search(
        ...
        return False
    if re.search(
        r"USoundBase\s*\*\s*Sound\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundBase\s*>\s+Sound\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundWave\s*>\s+Sound\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundBase\s*>\s+Sound\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundBase> Sound is missing from "
...
class ProductionAudioEntrySoundFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_sound_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("TSoftObjectPtr<USoundBase> Sound;", compact_origin)
        self.assertNotIn("Sound = nullptr", compact_origin)
        self.assertNotIn("Sound = false", compact_origin)
        self.assertNotIn("Sound = true", compact_origin)
        self.assertNotIn("Sound = 0.f", compact_origin)
        self.assertNotIn("Sound = 160.f", compact_origin)

    def test_sound_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TSoftObjectPtr<USoundBase> Sound"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("TSoftObjectPtr<USoundBase>", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<USoundBase> Sounds;\n",
            "\tint32 Sound = 1;\n",
            "\tbool Sound;\n",
            "\tfloat Sound = " + forty + ";\n",
            "\tfloat Sound = " + eighty + ";\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry Sound field "
                "decl contract "
