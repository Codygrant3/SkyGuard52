...
# then BARE `void RefreshFromPresentationState();`
...
# RebindIfNeeded / GetBoundPresentation.
# Word-boundary match RefreshFromPresentationState vs BindPresentation.
...
# RebindIfNeeded / GetBoundPresentation as this slot
# (leave them uncontracted).
# Do NOT claim ShouldShowDebriefForState (NO UFUNCTION macro).
# Identifier RefreshFromPresentationState is NOT leftover analog
...
# LOCKED_DECL is `void RefreshFromPresentationState();`
...
TARGET = "void RefreshFromPresentationState();"
TARGET_INLINE = (
    "void RefreshFromPresentationState() { }"
)
TARGET_WRONG_BARE = "void RefreshFromPresentationState() const;"
TARGET_WRONG_NO_CONST = (
    "bool RefreshFromPresentationState();"
)
TARGET_WRONG_VOID = "bool AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_FLOAT = "float RefreshFromPresentationState();"
TARGET_WRONG_INT = "int32 RefreshFromPresentationState();"
TARGET_WRONG_RENAME = "void RefreshFromPresentationStat();"
...
TARGET_WRONG_TRUE = "bool RefreshFromPresentationState = true;"
TARGET_WRONG_ZERO = "bool RefreshFromPresentationState = 0.f;"
...
METHOD_SIGNATURE_RE = re.compile(
    r"\bvoid\s+RefreshFromPresentationState\s*\(\s*\)\s*(?:;|\{)"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+RefreshFromPresentationState\s*\(\s*\)\s*const\b", compact):
        return False
    if re.search(r"\bbool\s+RefreshFromPresentationState\b", compact):
        return False
    if re.search(r"\bfloat\s+RefreshFromPresentationState\b", compact):
        return False
    if re.search(r"\bint32\s+RefreshFromPresentationState\b", compact):
        return False
    if re.search(
        r"\bvoid\s+RefreshFromPresentationState\s*\(\s*[^)]+\s*\)",
        compact,
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+RefreshFromPresentationState\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void RefreshFromPresentationState "
...
class SortieHudRefreshFromPresentationStateDeclContractTests(
    unittest.TestCase
):
    def test_sortie_hud_host_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, "RefreshFromPresentationState"),
            section,
        )
    ...
    def test_missing_refresh_from_presentation_state_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "void RefreshFromPresentationState()const;",
    ...
    def test_refresh_from_presentation_state_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, "void RebindIfNeeded();")
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_YAW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MISSION_ID)
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, "RebindIfNeeded"))
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid RefreshFromPresentationStates();\n",
            "\tint32 RefreshFromPresentationState = 1;\n",
            "\tFName RefreshFromPresentationState = NAME_None;\n",
            "\tvoid RefreshFromPresentationState() = "
            + forty
            + ";\n",
            "\tvoid RefreshFromPresentationState() = "
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "RefreshFromPresentationState")
        )
        self.assertTrue(
            has_identifier(section, "RefreshFromPresentationState")
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "sortie hud refresh from presentation state "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "sortie_hud_refresh_from_presentation_state"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardAudioProductionBank.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, "RebindIfNeeded"))
