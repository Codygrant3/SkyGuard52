...
# only adds an isolated Python BeatIndexForElapsed declaration contract.
...
# Isolated-test drafts stay off this lane. KeepsHydraForClusters is
# opening now as a sibling. ApplyHydraForClusters stays unlocked.
# RiverHammer()/IronRain()/ForMission() sequences stay on-main.
# Kind enum (#245), kit defaults (#248), Kinds (#255), Threats (#257),
# Stations (#258), Calls (#259), MissionId/WeatherIdentity (#260),
# Title/WeatherLabel (#261), leftover theater-kit #59, and day-kit
# BeatIndexForElapsed stay sibling-only.
LOCKED_SCRIPTS = (
...
# Day-kit BeatIndexForElapsed is the same signature in a different
# header. This lane does not read that header.
...
# .cpp bodies / invented elapsed tables stay unlocked. Do not invent
# return values or elapsed-second tables.
CPP_AND_INVENTED = (
    "return ",
    "INDEX_NONE",
    "NAME_None",
...
class StormRainBeatIndexContractTests(unittest.TestCase):
    ...
    def test_keeps_hydra_does_not_satisfy_beat_index(self) -> None:
        keeps_only = (
            f"namespace {NAMESPACE_NAME}\n"
            "{\n"
            f"\t{UNLOCKED_KEEPS_HYDRA}\n"
            "}\n"
        )
        block = kits_namespace(keeps_only)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(block, LOCKED_DECLARATION)
        self.assertIn("BeatIndexForElapsed", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertNotIn(LOCKED_DECLARATION, block)
        self.assertIn(UNLOCKED_KEEPS_HYDRA, block)

    def test_apply_hydra_does_not_satisfy_beat_index(self) -> None:
        apply_only = (
            f"namespace {NAMESPACE_NAME}\n"
            "{\n"
            "\tbool ApplyHydraForClusters(\n"
    ...
    def test_name_or_wrong_signature_does_not_satisfy(self) -> None:
        wrong = (
            f"namespace {NAMESPACE_NAME}\n"
            "{\n"
            "\tint32 BeatIndexForElapsed();\n"
            "\tvoid BeatIndexForElapsed(FName MissionId, float ElapsedSeconds);\n"
            "\tint32 BeatIndexForElapsed(float ElapsedSeconds);\n"
            "}\n"
        )
    ...
    def test_beat_index_declaration_matches_origin_main(self) -> None:
        ...
        self.assertNotIn("return ", LOCKED_DECLARATION)
        self.assertNotIn("ASkyguardGunner", LOCKED_DECLARATION)
        self.assertNotIn("KeepsHydraForClusters", LOCKED_DECLARATION)
    ...
    def test_contract_does_not_lock_day_kit_beat_index(self) -> None:
        self.assertEqual(
            HEADER_PATH,
        ...
        self.assertNotIn("SkyguardDaySortieBeatKits", NAMESPACE_NAME)
    ...
    def test_contract_does_not_read_cpp_or_elapsed_tables(self) -> None:
        ...
        self.assertNotIn("ElapsedSeconds <", block)
        self.assertNotIn("ElapsedSeconds >", block)
        self.assertNotIn("ElapsedSeconds <=", LOCKED_DECLARATION)
        self.assertNotIn("ElapsedSeconds >=", LOCKED_DECLARATION)
    ...
    def test_namespace_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                ...
                "BeatIndexForElapsed is Apache CPG 30 mm / Hydra / "
                "Hellfire, not Yak",
            )
