# THIS IS leftover-safe ASkyguardApacheAircraft Canopy.
...
# then BARE `TObjectPtr<UStaticMeshComponent> Canopy;`
# (no initializer).
...
# Stop BEFORE sibling TailBoom as a claimed slot.
# Do NOT claim Nose / Fuselage / SilhouetteMesh / AircraftRoot /
...
# apache-cpg-feel, leftover analog
# apache-is-canopy-glass-cracked UFUNCTION, leftover analog
# apache-silhouette-mesh-field, leftover analog
...
# LOCKED_DECL is `TObjectPtr<UStaticMeshComponent> Canopy;`
...
TARGET = "TObjectPtr<UStaticMeshComponent> Canopy;"
TARGET_WRONG_INIT = "TObjectPtr<UStaticMeshComponent> Canopy = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UStaticMeshComponent> Canopy = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UStaticMeshComponent> Canopy = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> Canopy;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> Canopy;"
...
STOP_BEFORE_TAIL_BOOM = "TailBoom"
...
CANOPY_DECL_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+Canopy\s*;"
)
CANOPY_INIT_RE = re.compile(
    r"TObjectPtr<UStaticMeshComponent>\s+Canopy\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover TailBoom / Nose / Fuselage / SilhouetteMesh /
    # AircraftRoot / PilotCanopy, or an initializer
    # on Canopy as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CANOPY_DECL_RE.search(compact) is None:
        return False
    if CANOPY_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_TAIL_BOOM):
            break
    ...
    if has_identifier(window, SIBLING_TAIL_BOOM):
        raise AssertionError(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UStaticMeshComponent>\s+Canopy\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UStaticMeshComponent> Canopy "
...
class ApacheCanopyFieldDeclContractTests(unittest.TestCase):
    def test_apache_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Canopy"), section)
        ...
        self.assertFalse(
            has_identifier(section, SIBLING_TAIL_BOOM),
            section,
        )
        ...
        self.assertTrue(has_identifier(section, SIBLING_NOSE), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Canopy"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Canopy", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_canopy_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UStaticMeshComponent> Canopy;", compact_origin)
        self.assertNotIn("Canopy = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_canopy_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UStaticMeshComponent> Canopy"),
            LOCKED_DECL,
        )
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UStaticMeshComponent> Canopy;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UStaticMeshComponent> Canopies;\n",
            "\tint32 Canopy = 140;\n",
            "\tbool Canopy = true;\n",
            "\tfloat Canopy = " + forty + ";\n",
            "\tfloat Canopy = " + eighty + ";\n",
    ...
    def test_does_not_claim_sibling_tail_boom_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, SIBLING_NOSE))
        self.assertFalse(has_identifier(section, SIBLING_TAIL_BOOM))
        ...
        self.assertNotIn(TARGET_WRONG_TAIL_BOOM, section)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, SIBLING_TAIL_BOOM))
        self.assertFalse(has_identifier(section, SIBLING_TAIL_BOOM))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache Canopy field decl contract "
