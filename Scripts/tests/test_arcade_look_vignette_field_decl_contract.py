# THIS IS leftover-safe USkyguardArcadeLookComponent Vignette.
...
# type is not float, or if initializer is not `= 0.42f`.
...
# bEnabled / Contrast / Saturation / Gain / Gamma /
# BloomIntensity / Grain /
...
# This is Vignette = 0.42f, not those feel constants.
...
# RETARGET: type is float, identifier is Vignette,
# initializer is 0.42f, UPROPERTY is EditAnywhere
...
# initializer.
# Fail-closed leftover ArcadeLook Contrast 1.18f /
# Saturation 1.12f / Gain 1.08f / bEnabled.
# Fail-closed leftover apache-cpg-feel CannonRecoilPitch
# 0.92f / RocketSplashRadius 420.f (not 0.42f).
# Fail-closed leftover GuidedLockRules DetectProgressEnd
# 0.22f. Do NOT parse SkyguardGuidedLockRules.h.
# Better same-class retarget leftover-safe ArcadeLook
# bEnabled / Contrast. Copy that parse window. Do NOT
# claim bEnabled or Contrast as this slot.
...
TARGET = "float Vignette = 0.42f;"
TARGET_WRONG_BARE = "float Vignette;"
TARGET_WRONG_FALSE = "float Vignette = false;"
TARGET_WRONG_TRUE = "float Vignette = true;"
TARGET_WRONG_ZERO = "float Vignette = 0.f;"
TARGET_WRONG_ONE = "float Vignette = 1.f;"
TARGET_WRONG_NO_F = "float Vignette = 0.42;"
TARGET_WRONG_FNAME = "FName Vignette;"
TARGET_WRONG_INT = "int32 Vignette = 0;"
TARGET_WRONG_BOOL = "bool Vignette = true;"
...
TARGET_WRONG_CANNON_RATE = "float Vignette = 12.0f;"
TARGET_WRONG_CANNON_RECOIL = "float Vignette = 0.92f;"
TARGET_WRONG_CANNON_DAMAGE = "float Vignette = 22.0f;"
TARGET_WRONG_TIME_OF_DAY = "float Vignette = 12.f;"
TARGET_WRONG_SENSOR_ACQUIRE = "float Vignette = 5.5f;"
TARGET_WRONG_CLOUD = "float Vignette = 0.25f;"
TARGET_WRONG_CONTRAST_INIT = "float Vignette = 1.18f;"
TARGET_WRONG_SATURATION_INIT = "float Vignette = 1.12f;"
TARGET_WRONG_GAIN_INIT = "float Vignette = 1.08f;"
TARGET_WRONG_DETECT_PROGRESS = "float Vignette = 0.22f;"
TARGET_WRONG_ROCKET_SPLASH = "float Vignette = 420.f;"
TARGET_WRONG_SIBLING_CONTRAST = "float Contrast = 1.18f;"
TARGET_WRONG_SIBLING_SATURATION = "float Saturation = 1.12f;"
TARGET_WRONG_SIBLING_GAIN = "float Gain = 1.08f;"
...
LEFTOVER_ROCKET_SPLASH_RADIUS = "RocketSplashRadius"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float Vignette = 0.42f;`.
    # Do not accept bare `float Vignette;` / leftover
    ...
    # 0.25f when origin/main has Vignette = 0.42f.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+Vignette\s*=\s*0\.42f\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"FName\s+Vignette\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+Vignette\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+Vignette\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float Vignette is missing from "
...
class ArcadeLookVignetteFieldDeclContractTests(unittest.TestCase):
    def test_arcade_look_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Vignette"), section)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Vignette"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Vignette", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_vignette_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float Vignette = 0.42f;", compact_origin)
        self.assertNotIn("Vignette = NAME_None", compact_origin)
        self.assertNotIn("Vignette = false", compact_origin)
        self.assertNotIn("Vignette = true", compact_origin)
        self.assertNotIn("Vignette = 0.f", compact_origin)
        self.assertNotIn("Vignette = 160.f", compact_origin)
        self.assertNotIn("Vignette = 12.0f", compact_origin)
        self.assertNotIn("Vignette = 22.0f", compact_origin)

    def test_vignette_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertNotIn("= 420.f", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 0.42f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 0.42f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            f"\t{TARGET_WRONG_CONTRAST_INIT}\n",
            f"\t{TARGET_WRONG_SATURATION_INIT}\n",
            f"\t{TARGET_WRONG_GAIN_INIT}\n",
            f"\t{TARGET_WRONG_DETECT_PROGRESS}\n",
            f"\t{TARGET_WRONG_ROCKET_SPLASH}\n",
            f"\t{TARGET_WRONG_SIBLING_CONTRAST}\n",
            f"\t{TARGET_WRONG_SIBLING_SATURATION}\n",
            f"\t{TARGET_WRONG_SIBLING_GAIN}\n",
            leftover_primary,
            leftover_guided,
            f"\tfloat {SIBLING_SATURATION} = 1.12f;\n",
            "\tfloat Vignettes = 0.42f;\n",
            "\tint32 Vignette = 1;\n",
            "\tbool Vignette = true;\n",
            "\tfloat Vignette = " + forty + ";\n",
            "\tfloat Vignette = " + eighty + ";\n",
        )
    ...
    def test_leftover_arcade_look_siblings_are_not_this_slot(self) -> None:
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIBLING_CONTRAST)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIBLING_SATURATION)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_SIBLING_GAIN)
        self.assertNotEqual(LOCKED_DECL, "bool bEnabled = true;")
        ...
        self.assertNotIn(SIBLING_BLOOM, LOCKED_DECL)
        self.assertNotIn(SIBLING_GRAIN, LOCKED_DECL)
        self.assertNotIn(SIBLING_CHROMATIC, LOCKED_DECL)
        self.assertNotEqual(THIS_SCRIPT, SIBLING_CONTRAST_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, SIBLING_ENABLED_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, SIBLING_SATURATION_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, SIBLING_GAIN_SCRIPT)
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SIBLING_CONTRAST}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(
                f"\t{TARGET_WRONG_CONTRAST_INIT}\n",
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_GAIN_INIT}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(
                f"\t{TARGET_WRONG_SIBLING_CONTRAST}\n",
    ...
    def test_leftover_analog_feel_initializers_do_not_satisfy(self) -> None:
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ROCKET_SPLASH}\n", LOCKED_DECL)
        )
        self.assertNotIn("420.f", LOCKED_DECL)
        self.assertNotIn("0.92f", LOCKED_DECL)
    ...
    def test_clone_source_theater_kit_weather_identity_is_not_locked_decl(
        ...
        self.assertIn("float ", LOCKED_DECL)
        self.assertIn("Vignette", LOCKED_DECL)
        self.assertIn("0.42f", LOCKED_DECL)
        self.assertNotIn("FName ", LOCKED_DECL)
        self.assertNotIn("WeatherIdentity", LOCKED_DECL)
        self.assertNotIn("160.f", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "arcade-look Vignette field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "vignette_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardArcadeLookComponent.cpp", THIS_SCRIPT)
    ...
    def test_header_path_is_arcade_look_component_only(self) -> None:
        ...
        self.assertEqual(LOCKED_DECL, "float Vignette = 0.42f;")
        self.assertEqual(
            UPROPERTY_EDIT_WRITE,
            'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
            'Category="Skyguard|Arcade")',
