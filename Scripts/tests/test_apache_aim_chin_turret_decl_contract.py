...
# or lock the AimChinTurret body in the .cpp.
...
# apache-cpg-feel #8951. This is not leftover CPG HUD /
# sight HUD. Stay off SetRotorPower, IssuePilotCommand,
...
# apache-cpg-feel #8951, leftover DebriefWidget
...
# .cpp AimChinTurret body / invented INDEX_NONE stay
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardApacheAircraft::AimChinTurret",
    "SkyguardApacheAircraft.cpp",
)
SIBLING_TYPES = (
...
class ApacheAimChinTurretDeclContractTests(unittest.TestCase):
    def test_apache_aircraft_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, AIM_CHIN_TURRET), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, AIM_CHIN_TURRET)
        self.assertIn("AimChinTurret", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, AIM_CHIN_TURRET))

    def test_missing_aim_chin_turret_declaration_fails_closed(self) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, AIM_CHIN_TURRET)
        self.assertIn("AimChinTurret", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, AIM_CHIN_TURRET)
        self.assertIn("AimChinTurret", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue(has_declaration(section, AIM_CHIN_TURRET), section)
        self.assertNotIn("BlueprintPure", AIM_CHIN_TURRET)
        self.assertNotIn("UFUNCTION", AIM_CHIN_TURRET)
        self.assertNotIn("Category", AIM_CHIN_TURRET)
        self.assertNotIn("BlueprintCallable", AIM_CHIN_TURRET)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, AIM_CHIN_TURRET)
        self.assertIn("AimChinTurret", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_parens = "\tvoid AimChinTurret;\n"
        wrong_return_bool = (
            "\tbool AimChinTurret(const FRotator& WorldAim);\n"
        )
        wrong_return_rotator = (
            "\tFRotator AimChinTurret(const FRotator& WorldAim);\n"
        )
        added_const = (
            "\tvoid AimChinTurret(const FRotator& WorldAim) const;\n"
        )
        missing_const = "\tvoid AimChinTurret(FRotator& WorldAim);\n"
        missing_ref = "\tvoid AimChinTurret(const FRotator WorldAim);\n"
        wrong_type = "\tvoid AimChinTurret(const FVector& WorldAim);\n"
        added_arg = (
            "\tvoid AimChinTurret(const FRotator& WorldAim, float Extra);\n"
        )
        no_args = "\tvoid AimChinTurret();\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, AIM_CHIN_TURRET)
            self.assertIn("AimChinTurret", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_aim_chin_turret_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, AIM_CHIN_TURRET),
            AIM_CHIN_TURRET,
        )
        self.assertTrue(has_declaration(section, AIM_CHIN_TURRET))
        self.assertEqual(declaration_count(section, AIM_CHIN_TURRET), 1)
        self.assertTrue(AIM_CHIN_TURRET.startswith("void "), AIM_CHIN_TURRET)
        self.assertTrue(AIM_CHIN_TURRET.endswith(";"), AIM_CHIN_TURRET)
        self.assertIn("AimChinTurret(", AIM_CHIN_TURRET)
        self.assertIn("const FRotator& WorldAim", AIM_CHIN_TURRET)
        self.assertNotIn("INDEX_NONE", AIM_CHIN_TURRET)
        self.assertNotIn("{", AIM_CHIN_TURRET)
        self.assertNotIn("}", AIM_CHIN_TURRET)
        self.assertNotIn("return ", AIM_CHIN_TURRET)
        self.assertNotIn(" const", AIM_CHIN_TURRET)
        self.assertNotIn("GetChinMuzzleLocation", AIM_CHIN_TURRET)
        self.assertNotIn("GetChinTurret", AIM_CHIN_TURRET)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tvoid\n"
            "\tAimChinTurret(const FRotator& WorldAim);\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
        ...
        for header in (
            ...
            self.assertEqual(declaration_count(section, AIM_CHIN_TURRET), 1)
        one_line = f"{{\npublic:\n\t{AIM_CHIN_TURRET}\n}}\n"
        self.assertTrue(has_declaration(one_line, AIM_CHIN_TURRET))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, AIM_CHIN_TURRET), section)
        self.assertEqual(
            require_declaration(section, AIM_CHIN_TURRET),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", AIM_CHIN_TURRET)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_aim_chin_turret_cpp_body(self) -> None:
        locked_only = f"{AIM_CHIN_TURRET}\n"
        self.assertNotIn("{", AIM_CHIN_TURRET)
        self.assertNotIn("}", AIM_CHIN_TURRET)
        self.assertNotIn("return ", AIM_CHIN_TURRET)
        ...
        self.assertNotIn("SkyguardApacheAircraft.cpp", AIM_CHIN_TURRET)
        self.assertNotIn("SkyguardApacheAircraft.cpp", locked_only)
        self.assertNotIn("return false", AIM_CHIN_TURRET)
        self.assertNotIn("return true", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        self.assertNotIn("GetGunnerMount", AIM_CHIN_TURRET)
        self.assertNotIn("GetEyeMount", AIM_CHIN_TURRET)
        self.assertNotIn("GetWeaponMount", AIM_CHIN_TURRET)
        self.assertNotIn("GetChinTurret", AIM_CHIN_TURRET)
        self.assertNotIn("GetPilotMount", AIM_CHIN_TURRET)
        self.assertNotIn("GetSensorTurret", AIM_CHIN_TURRET)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{AIM_CHIN_TURRET}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AIM_CHIN_TURRET)
        self.assertNotIn("GetChinMuzzleLocation", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_set_rotor_power(self) -> None:
        ...
        for neighbor in SET_ROTOR_POWER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AIM_CHIN_TURRET)
        self.assertNotIn("SetRotorPower", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_pilot_command_helpers(self) -> None:
        ...
        for neighbor in PILOT_COMMAND_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AIM_CHIN_TURRET)
        self.assertNotIn("IssuePilotCommand", AIM_CHIN_TURRET)
        self.assertNotIn("GetPilotCommand", AIM_CHIN_TURRET)
        self.assertNotIn("GetPilotConfirmationsIssued", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_orbit_or_face(self) -> None:
        ...
        for neighbor in ORBIT_AND_FACE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AIM_CHIN_TURRET)
        self.assertNotIn("SetOrbitFocus", AIM_CHIN_TURRET)
        self.assertNotIn("FaceWorldLocation", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_sensor_view_or_interior(self) -> None:
        ...
        for neighbor in SENSOR_VIEW_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AIM_CHIN_TURRET)
        self.assertNotIn("SetSensorView", AIM_CHIN_TURRET)
        self.assertNotIn("SetFirstPersonInterior", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_damage_helpers(self) -> None:
        ...
        for neighbor in DAMAGE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AIM_CHIN_TURRET)
        self.assertNotIn("ApplyDamage", AIM_CHIN_TURRET)
        self.assertNotIn("GetDamageFraction", AIM_CHIN_TURRET)
        self.assertNotIn("ApplySystemHit", AIM_CHIN_TURRET)
        self.assertNotIn("ApplyHit", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_sensor_state_helpers(self) -> None:
        ...
        for neighbor in SENSOR_STATE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, AIM_CHIN_TURRET)
        self.assertNotIn("IsSensorLive", AIM_CHIN_TURRET)
        self.assertNotIn("IsThermalAvailable", AIM_CHIN_TURRET)
        self.assertNotIn("IsSensorViewActive", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_leftover_chin_muzzle(self) -> None:
        ...
        for token in LEFTOVER_CHIN_MUZZLE_NOT_LOCKED:
            ...
            self.assertNotIn(token, AIM_CHIN_TURRET)
        self.assertNotIn("GetChinMuzzleLocation", AIM_CHIN_TURRET)
        self.assertNotIn("test_apache_chin_muzzle_tests.py", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, AIM_CHIN_TURRET)
        self.assertNotIn("ESkyguardApacheSystem", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_leftover_cpg_feel(self) -> None:
        ...
        for token in LEFTOVER_CPG_FEEL_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, AIM_CHIN_TURRET)
        self.assertNotIn("test_apache_cpg_feel_contract.py", AIM_CHIN_TURRET)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        for token in LEFTOVER_WIDGET_DECL_NOT_LOCKED:
            ...
            self.assertNotIn(token, AIM_CHIN_TURRET)
            self.assertNotIn(token, section)
        self.assertNotIn("USkyguardDebriefWidget", AIM_CHIN_TURRET)
        self.assertNotIn("USkyguardBriefingWidget", AIM_CHIN_TURRET)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            AIM_CHIN_TURRET,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, AIM_CHIN_TURRET)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", AIM_CHIN_TURRET)
        self.assertNotIn("ESkyguardMissionSkylineStyle", AIM_CHIN_TURRET)

    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{AIM_CHIN_TURRET}\n"
        self.assertEqual(
            require_declaration(locked_only, AIM_CHIN_TURRET),
            AIM_CHIN_TURRET,
        )
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertNotIn("ASkyguardApacheAircraft::AimChinTurret", section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "apache AimChinTurret contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            ...
            self.assertNotIn(banned, AIM_CHIN_TURRET.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                f"apache AimChinTurret contains {banned}; "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, AIM_CHIN_TURRET)

    def test_contract_is_aim_chin_turret_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", AIM_CHIN_TURRET)
        self.assertNotIn("{", AIM_CHIN_TURRET)
        self.assertTrue(AIM_CHIN_TURRET.startswith("void "))
        self.assertTrue(AIM_CHIN_TURRET.endswith(";"))
        self.assertIn(UFUNCTION_APACHE, section)
