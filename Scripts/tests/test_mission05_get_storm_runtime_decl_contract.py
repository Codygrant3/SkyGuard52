...
# INDEX_NONE or lock GetStormRuntime
# construction in the .cpp. This is leftover-safe
# Mission05 Storm GetStormRuntime
# METHOD on ASkyguardMission05IntegrationDirector. It is
# NOT leftover storm-runtime-defaults #89c9 STRUCT,
# NOT leftover Mission05 wave-state enum #ad28, NOT
# leftover environment-readiness defaults #6b9d /
# #b931, NOT leftover Environment GetReadiness, NOT
# leftover Mission04 GetReadiness #658, NOT leftover
# Mission01 GetReadiness / leftover Mission03
# GetReadiness / leftover Mission05 GetReadiness.
# It is NOT leftover BindRuntimeActors
# (retired live mount in the locked decl), NOT leftover
...
# from leftover Mission01 / Mission03 / Mission04
# GetReadiness #658. Distinct from leftover
# briefing-widget / MissionBriefingComponent methods.
# not leftover Briefing / AudioDirector / Root /
# RadioChatter / SortiePresentation /
# CampaignDefinition / MissionDefinition /
# Readiness field / bAutoInitialize /
...
# lock sibling Integration / Waves / Protection /
# Objectives methods
...
# leftover Mission01 GetReadiness, leftover
# Mission03 GetReadiness, leftover Mission04
# GetReadiness #658, leftover Mission05
# GetReadiness, leftover Mission05 GetWaveState,
...
# Mission05 protected-target-runtime-defaults #ec8b,
# leftover storm-runtime-defaults #89c9 STRUCT
# (do not add a struct-defaults contract), leftover
# searchlight-track-runtime-defaults #7347
# (do not lock GetSearchlightRuntime).
# origin/main is a split-line method with an inline
# body; accept that form, other one-line /
...
# Mission01, not Mission02, not Mission03, not
# Mission04, not Integration, not Waves, not
# Searchlight, not Substation, not Objectives, not
# Boss, not Protection, not Destruction, not
# Briefing, not Environment, not leftover
...
# Mission04 Integration #658, not leftover
# Harbor Mission02 Integration, not leftover
# Mission05 Integration GetReadiness. Do not lock leftover
# BindRuntimeActors / leftover spawn-location fields /
# leftover HandleDroneCityImpact / leftover
...
# do not reopen leftover #536–#673, closed #668/#669,
# leftover Apache MaxIntegrity / CurrentIntegrity,
# leftover Apache mount getters #851b / own-ship
# #96c5 / chin muzzle #4e39, leftover
# settings-apply-broadcast #1268, leftover
# patrol-ship empty fail-closed #5382, leftover
# RadarNode, leftover named boss methods,
...
# GetStormRuntime method declaration
# contract on ASkyguardMission05IntegrationDirector.
# Distinct from leftover storm-runtime-defaults
# #89c9 STRUCT and leftover Mission05 GetReadiness.
LOCKED = {
...
# contracts, leftover Mission01 IsCorePlayableReady
# #613, leftover Mission03 IsCorePlayableReady
# #636, leftover Harbor Mission02, leftover
# Mission04 wave-state enum #bb22, leftover
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardMission05IntegrationDirector::GetStormRuntime",
    "SkyguardMission05IntegrationDirector.cpp",
)
SIBLING_TYPES = (
...
class Mission05GetStormRuntimeDeclContractTests(unittest.TestCase):
    def test_mission05_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_STORM_RUNTIME),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_STORM_RUNTIME)
        self.assertIn("GetStormRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, GET_STORM_RUNTIME)
    ...
    def test_missing_get_storm_runtime_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission05IntegrationDirector();\n"
            f"\t{ROOT_FIELD}\n"
            f"\t{BRIEFING_FIELD}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_STORM_RUNTIME)
        self.assertIn("GetStormRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_STORM_RUNTIME)
        self.assertIn("GetStormRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("UFUNCTION", GET_STORM_RUNTIME)
        self.assertNotIn("BlueprintPure", GET_STORM_RUNTIME)
        self.assertNotIn("Category", GET_STORM_RUNTIME)
        self.assertNotIn("BlueprintCallable", UFUNCTION_STORM)
        ...
        self.assertNotIn("Substation", UFUNCTION_STORM)
        self.assertNotIn("Objectives", UFUNCTION_STORM)
        self.assertNotIn("Destruction", UFUNCTION_STORM)
        self.assertNotIn("Boss", UFUNCTION_STORM)
        self.assertIn("BlueprintPure", UFUNCTION_STORM)
        ...
        for invented in INVENTED_UFUNCTION:
            ...
            self.assertNotIn(invented, GET_STORM_RUNTIME)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_STORM_RUNTIME)
        self.assertIn("GetStormRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        as_void = (
            "\tvoid GetStormRuntime() const;\n"
        )
        missing_const = (
            "\tconst FSkyguardStormRuntime& "
            "GetStormRuntime();\n"
        )
        with_arg = (
            "\tconst FSkyguardStormRuntime& "
            "GetStormRuntime(float DeltaSeconds) const;\n"
        )
        renamed = (
            "\tconst FSkyguardStormRuntime& "
            "GetStormState() const;\n"
        )
        short_name = "\tbool GetStormRuntime() const;\n"
        leftover_m04_type = (
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_STORM_RUNTIME)
            self.assertIn("GetStormRuntime", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_storm_runtime_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, GET_STORM_RUNTIME),
            GET_STORM_RUNTIME,
        )
        self.assertTrue(has_declaration(section, GET_STORM_RUNTIME))
        self.assertEqual(
            declaration_count(section, GET_STORM_RUNTIME),
            1,
        )
        self.assertTrue(
        ...
        self.assertIn("FSkyguardStormRuntime", GET_STORM_RUNTIME)
        self.assertIn("GetStormRuntime", GET_STORM_RUNTIME)
        self.assertNotIn("{", GET_STORM_RUNTIME)
        self.assertNotIn("}", GET_STORM_RUNTIME)
        self.assertNotIn("return ", GET_STORM_RUNTIME)
        self.assertIn("()", GET_STORM_RUNTIME)
        self.assertNotIn("TArray<", GET_STORM_RUNTIME)
        self.assertNotIn("TObjectPtr", GET_STORM_RUNTIME)
        self.assertNotIn("TSoftObjectPtr", GET_STORM_RUNTIME)
        self.assertNotIn("INDEX_NONE", GET_STORM_RUNTIME)
        self.assertNotIn("UFUNCTION", GET_STORM_RUNTIME)
        self.assertNotIn("{", GET_STORM_RUNTIME)
        self.assertNotIn("}", GET_STORM_RUNTIME)
        self.assertNotIn("return ", GET_STORM_RUNTIME)
        self.assertIn(" const", GET_STORM_RUNTIME)
        self.assertNotIn(";", GET_STORM_RUNTIME)
        self.assertNotIn("Root", GET_STORM_RUNTIME)
        self.assertNotIn("Briefing", GET_STORM_RUNTIME)
        self.assertNotIn("AudioDirector", GET_STORM_RUNTIME)
        self.assertNotIn("RadioChatter", GET_STORM_RUNTIME)
        self.assertNotIn("SortiePresentation", GET_STORM_RUNTIME)
        self.assertNotIn("CampaignDefinition", GET_STORM_RUNTIME)
        self.assertNotIn("MissionDefinition", GET_STORM_RUNTIME)
        self.assertNotIn("Readiness;", GET_STORM_RUNTIME)
        self.assertNotIn("bAutoInitialize", GET_STORM_RUNTIME)
        self.assertNotIn("bAllowBoundedActorSpawning", GET_STORM_RUNTIME)
        self.assertNotIn("bAutoLaunchAfterBriefing", GET_STORM_RUNTIME)
        self.assertNotIn("PathfinderSpawnLocation", GET_STORM_RUNTIME)
        self.assertNotIn("GetAircraft", GET_STORM_RUNTIME)
        self.assertNotIn("BindRuntimeActors", GET_STORM_RUNTIME)
        self.assertNotIn("HandleDroneCityImpact", GET_STORM_RUNTIME)
        self.assertNotIn("StartNextWave", GET_STORM_RUNTIME)
        self.assertNotIn("NotifyThreatDestroyed", GET_STORM_RUNTIME)
        self.assertNotIn("AdvanceConvoyByDistance", GET_STORM_RUNTIME)
        self.assertNotIn("NotifyConvoyDamage", GET_STORM_RUNTIME)
        self.assertNotIn("GetWaveState", GET_STORM_RUNTIME)
        self.assertNotIn("GetConvoyRouteState", GET_STORM_RUNTIME)
        self.assertNotIn("GetDayBeatKit", GET_STORM_RUNTIME)
        self.assertNotIn("GetNightBeatKit", GET_STORM_RUNTIME)
        self.assertNotIn("StartSearchlightWindow", GET_STORM_RUNTIME)
        self.assertNotIn("AdvanceSearchlightTrack", GET_STORM_RUNTIME)
        self.assertNotIn("NotifySubstationDamage", GET_STORM_RUNTIME)
        self.assertNotIn("GetRemainingThreatsInWave", GET_STORM_RUNTIME)
        self.assertNotIn("GetSearchlightRuntime", GET_STORM_RUNTIME)
        self.assertNotIn("GetSubstationIntegrity", GET_STORM_RUNTIME)
        self.assertNotIn("ASkyguardMission03IntegrationDirector", GET_STORM_RUNTIME)
        self.assertNotIn("ConvoyRuntimeAnchor", GET_STORM_RUNTIME)
        self.assertNotIn("RoadHunterSpawnLocation", GET_STORM_RUNTIME)
        self.assertNotIn("ASkyguardMission01IntegrationDirector", GET_STORM_RUNTIME)
        self.assertNotIn("ASkyguardMission02IntegrationDirector", GET_STORM_RUNTIME)
        self.assertNotIn("ConfigureMissionDefinition", GET_STORM_RUNTIME)
        self.assertNotIn("NotifyObjectiveProgress", GET_STORM_RUNTIME)
        self.assertNotIn("NotifyProtectedAssetFailed", GET_STORM_RUNTIME)
        self.assertNotIn("SynchronizeRuntimeState", GET_STORM_RUNTIME)
        self.assertNotIn("InitializePlayableMission", GET_STORM_RUNTIME)
        self.assertNotIn("IsCorePlayableReady", GET_STORM_RUNTIME)
        self.assertNotIn("GetObjectiveRuntime", GET_STORM_RUNTIME)
        self.assertNotIn("GetGunner", GET_STORM_RUNTIME)
        self.assertNotIn("GetPathfinder", GET_STORM_RUNTIME)
        self.assertNotIn("GetMissionId", GET_STORM_RUNTIME)
        self.assertNotIn("ValidateMissionContract", GET_STORM_RUNTIME)
        self.assertNotIn("ConfigureFromMission", GET_STORM_RUNTIME)
        self.assertNotIn("AdvanceBriefing", GET_STORM_RUNTIME)
        self.assertNotIn("SetAssetsReady", GET_STORM_RUNTIME)
        self.assertNotIn("AcknowledgeAndLaunch", GET_STORM_RUNTIME)
        self.assertNotIn("CanLaunch", GET_STORM_RUNTIME)
        self.assertNotIn("GetElapsedSeconds", GET_STORM_RUNTIME)
        self.assertNotIn("GetBriefingState", GET_STORM_RUNTIME)
        self.assertNotIn("GetMinimumWarmupSeconds", GET_STORM_RUNTIME)
        self.assertNotIn("GetBriefingText", GET_STORM_RUNTIME)
        self.assertNotIn("GetRadioChatter", GET_STORM_RUNTIME)
        self.assertNotIn("GetPresentation", GET_STORM_RUNTIME)
        self.assertNotIn("GetMissionTitle", GET_STORM_RUNTIME)
        self.assertNotIn("AcknowledgeBriefing", GET_STORM_RUNTIME)
        self.assertNotIn("LaunchSortie", GET_STORM_RUNTIME)
        self.assertNotIn("HullCollider", GET_STORM_RUNTIME)
        self.assertNotIn("OpticalTracker", GET_STORM_RUNTIME)
        self.assertNotIn("WeaponServo", GET_STORM_RUNTIME)
        self.assertNotIn("CountermeasurePod", GET_STORM_RUNTIME)
        self.assertNotIn("MinHeightFromOriginCm", GET_STORM_RUNTIME)
        self.assertNotIn("RadarNode", GET_STORM_RUNTIME)
        self.assertNotIn("ESkyguardMission02WaveState", GET_STORM_RUNTIME)
        self.assertNotIn("HarborIndustrial", GET_STORM_RUNTIME)
        self.assertNotIn("MaxIntegrity", GET_STORM_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_STORM_RUNTIME)
        self.assertNotIn("FillAndFinalize", GET_STORM_RUNTIME)
        self.assertNotIn("FillAndFail", GET_STORM_RUNTIME)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, GET_STORM_RUNTIME)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_ufunction_one_line = (
            "public:\n"
            f"\t{UFUNCTION_STORM} {GET_STORM_RUNTIME}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_STORM_RUNTIME}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_STORM_RUNTIME))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_STORM_RUNTIME),
            section,
    ...
    def test_environment_category_does_not_satisfy_storm(self) -> None:
        ...
        self.assertNotEqual(leftover_m04, UFUNCTION_STORM)
        self.assertNotEqual(leftover_m05_protection, UFUNCTION_STORM)
        self.assertNotEqual(leftover_m05_integration, UFUNCTION_STORM)
        self.assertNotEqual(leftover_m05_waves, UFUNCTION_STORM)
        ...
        self.assertNotIn(leftover_m04, UFUNCTION_STORM)
        self.assertNotIn(leftover_m05_protection, UFUNCTION_STORM)
        self.assertNotIn(leftover_m05_integration, UFUNCTION_STORM)
        self.assertNotIn(leftover_m05_waves, UFUNCTION_STORM)
        self.assertNotIn("Mission01", UFUNCTION_STORM)
        self.assertNotIn("Mission03", UFUNCTION_STORM)
        self.assertNotIn("Mission04", UFUNCTION_STORM)
        self.assertNotIn("Protection", UFUNCTION_STORM)
    ...
    def test_declaration_does_not_invent_ufunction_metadata(self) -> None:
        locked_only = f"{GET_STORM_RUNTIME}\n"
        for invented in INVENTED_UFUNCTION:
            self.assertNotIn(invented, GET_STORM_RUNTIME)
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", GET_STORM_RUNTIME)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_storm_runtime_cpp_body(self) -> None:
        locked_only = f"{GET_STORM_RUNTIME}\n"
        self.assertNotIn("{", GET_STORM_RUNTIME)
        self.assertNotIn("}", GET_STORM_RUNTIME)
        self.assertNotIn("return ", GET_STORM_RUNTIME)
        ...
        self.assertNotIn("return false", GET_STORM_RUNTIME)
        self.assertNotIn("return true", GET_STORM_RUNTIME)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{GET_STORM_RUNTIME}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_STORM_RUNTIME)
        self.assertIn("GetStormRuntime", GET_STORM_RUNTIME)
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, GET_STORM_RUNTIME)
        self.assertNotIn(BIND_RUNTIME_ACTORS, locked_only)
        self.assertNotIn(BIND_RUNTIME_ACTORS, GET_STORM_RUNTIME)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", GET_STORM_RUNTIME)
        self.assertNotIn("CurrentIntegrity", GET_STORM_RUNTIME)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_STORM_RUNTIME)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_STORM_RUNTIME)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_STORM_RUNTIME)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_STORM_RUNTIME}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_STORM_RUNTIME),
            GET_STORM_RUNTIME,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_STORM_RUNTIME)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 GetStormRuntime method contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_READINESS.lower(),
                "mission05 GetStormRuntime contains "
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_STORM_RUNTIME)

    def test_contract_is_get_storm_runtime_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_get_storm_runtime(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, GET_STORM_RUNTIME)
            )
            self.assertNotEqual(GET_STORM_RUNTIME, leftover)
        self.assertIn(
    ...
    def test_leftover_harbor_mission02_get_readiness_does_not_satisfy(
        self,
    ) -> None:
        leftover = (
    ...
    def test_leftover_handle_drone_city_impact_does_not_satisfy(self) -> None:
        ...
        self.assertIn("GetStormRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(region, GET_STORM_RUNTIME))
        locked_only = f"{GET_STORM_RUNTIME}\n"
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        one_line_inline = (
            "public:\n"
            "\tconst FSkyguardStormRuntime& "
            "GetStormRuntime() const { return StormRuntime; }\n"
            "};\n"
        )
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission05_get_storm_runtime"
            "_decl_contract.py"
        )
    ...
    def test_leftover_handle_boss_phase_changed_does_not_satisfy(self) -> None:
        ...
        for token in LEFTOVER_HANDLE_BOSS_PHASE_CHANGED_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_STORM_RUNTIME)
        self.assertNotIn("HandleBossPhaseChanged", GET_STORM_RUNTIME)
    ...
    def test_leftover_notify_protected_target_damage_does_not_satisfy(
        self,
    ) -> None:
        region = f"\t{NOTIFY_PROTECTED_TARGET_DAMAGE}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(region, GET_STORM_RUNTIME)
        self.assertIn("GetStormRuntime", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
