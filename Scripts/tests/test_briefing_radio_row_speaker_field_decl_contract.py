# THIS IS leftover-safe FSkyguardBriefingRadioRow::Speaker FIELD
# (declaration-only). origin/main form is split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly) then BARE
# `FText Speaker;` with no initializer and no Category=.
...
# THIS IS NOT leftover BriefingCard Title.
...
# Accept bare `FText Speaker;`. Fail-closed if type/decl
# missing, renamed, or Category= added.
# Harbor 40/80 fail-closed in this briefing-radio-row test file.
...
# through #1250, leftover BriefingCard Priority #1251,
...
TARGET = "FText Speaker;"
TARGET_WRONG = "FText Speaker = FText();"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Speaker (must be a bare FText Speaker; with no initializer).
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*FText\s+Speaker\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for FText Speaker is missing from "
...
class BriefingRadioRowSpeakerFieldDeclContractTests(unittest.TestCase):
    def test_briefing_radio_row_struct_exists(self) -> None:
        ...
        self.assertIn("FText Speaker;", section)
        self.assertNotIn("FText Speaker =", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("Speaker", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_speaker_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FText Speaker"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("Speaker", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(token, LOCKED_DECL)
        self.assertIn("Speaker", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FText Speaker"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("Speaker", locked_only)
        self.assertIn("FText Speaker;", locked_only)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                lowered,
                "briefing-radio-row Speaker field contract "
        ...
        for banned in ("Ig" + "la", "Ri" + "fle", "Ya" + "k"):
            self.assertNotIn(
                banned,
                file_text,
                "briefing-radio-row Speaker field contract contains a live "
