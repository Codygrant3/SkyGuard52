...
# bReadyForAuthoredPCGGeneration FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bReadyForAuthoredPCGGeneration = false; one-line /
...
# Hellfire.
# LOCK leftover-safe bPCGGenerationAuthorized sibling #1180.
# LOCK leftover-safe bAuthoredPCGStructureReady sibling #1181.
# LOCK leftover-safe bLicensedVegetationApproved sibling #1182.
# THIS IS NOT leftover bPCGGenerationAuthorized. THIS IS NOT leftover
# bAuthoredPCGStructureReady / bLicensedVegetationApproved.
# THIS IS leftover-safe FSkyguardMission01EnvironmentReadiness
# bReadyForAuthoredPCGGeneration FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# bool bReadyForAuthoredPCGGeneration = false; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).

ROOT = Path(__file__).resolve().parents[2]
SOURCE = ROOT / "Source" / "Skyguard52"
HEADER_PATH = "Source/Skyguard52/SkyguardMission01EnvironmentDirector.h"
STRUCT_NAME = "FSkyguardMission01EnvironmentReadiness"
...
# bPCGGenerationAuthorized. THIS IS NOT leftover
...
# FSkyguardMission01EnvironmentReadiness bReadyForAuthoredPCGGeneration
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bReadyForAuthoredPCGGeneration = false;
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardMission01EnvironmentReadiness
# bReadyForAuthoredPCGGeneration. THIS IS NOT leftover mission-map-
...
# UPROPERTY(Transient).
# THIS IS leftover-safe FSkyguardMission01EnvironmentReadiness
# bReadyForAuthoredPCGGeneration. THIS IS NOT leftover Mission05
...
# Mission01 EnvironmentReadiness bReadyForAuthoredPCGGeneration
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly bool bReadyForAuthoredPCGGeneration = false;
# one-line / split-line UPROPERTY wraps;
...
# bReadyForAuthoredPCGGeneration FIELD (declaration-only). It is
...
TARGET = "bool bReadyForAuthoredPCGGeneration = false;"
TARGET_WRONG = "bool bReadyForAuthoredPCGGeneration = true;"
LOCKED_DECL = TARGET
...
LOCKED_SCRIPTS = (
    ...
    "Scripts/tests/test_mission01_environment_readiness_licensed_vegetation_approved_field_decl_contract.py",
...
LEFTOVER_ENVIRONMENT_READINESS_NOT_LOCKED = (
    ...
    "bLicensedVegetationApproved",
    "test_mission01_environment_readiness_authored_pcg_structure_ready_field_decl_contract.py",
    "test_mission01_environment_readiness_licensed_vegetation_approved_field_decl_contract.py",
    "test_mission01_environment_readiness_pcg_generation_authorized_field_decl_contract.py",
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*bool\s+bReadyForAuthoredPCGGeneration\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for bool bReadyForAuthoredPCGGeneration is missing from "
...
class Mission01EnvironmentReadinessReadyForAuthoredPcgGenerationFieldDeclContractTests(unittest.TestCase):
    def test_environment_readiness_struct_exists(self) -> None:
        ...
        self.assertIn("bReadyForAuthoredPCGGeneration", section)
        self.assertIn("BeachTileCount", section)
        self.assertIn("LandTileCount", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bReadyForAuthoredPCGGeneration", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_ready_for_authored_pcg_generation_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_ready_for_authored_pcg_generation_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bReadyForAuthoredPCGGeneration"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("bReadyForAuthoredPCGGeneration", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("bReadyForAuthoredPCGGeneration", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("bool bReadyForAuthoredPCGGeneration"))
    ...
    def test_contract_does_not_relock_leftover_environment_readiness(
        ...
        self.assertNotIn("bPCGGenerationAuthorized", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("bReadyForAuthoredPCGGeneration", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_ready_for_authored_pcg_generation(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission01_environment_readiness"
                "_ready_for_authored_pcg_generation_field_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_parser_does_not_land_on_anchors_or_director(self) -> None:
        ...
        match = CLASS_RE.search(header)
        self.assertIsNotNone(match, header)
        self.assertIn("FSkyguardMission01EnvironmentReadiness", match.group(0))
