...
# a returned mission pointer, or lock the FindMission
# body in the .cpp.
...
# This lane only adds an isolated Python FindMission
# declaration contract. Stay off ValidateDefinition #331,
# GetPrimaryAssetId (sibling in-flight this wave), leftover
...
# #284/#195/#130/#8ccd, FillResultCombatStats / FillAndFinalize
# / FillAndFail / ApplyHydraForClusters (leftover
# ASkyguardGunner*), leftover Harbor #6/#8/#9, leftover
...
# broadcast, ValidateDefinition #331, GetPrimaryAssetId
# (sibling in-flight this wave), and newly drafted
...
# here. ValidateDefinition #331 / GetPrimaryAssetId /
# CampaignId / DisplayName / Missions stay sibling-only.
UNLOCKED_NEIGHBORS = (
    "FName CampaignId",
    "FText DisplayName",
...
# is FindMission on USkyguardCampaignDefinition only.
...
# .cpp FindMission body / invented returned mission pointer
...
CPP_AND_INVENTED = (
    ...
    "FindByPredicate",
    "Found->Get()",
    "USkyguardCampaignDefinition::FindMission",
    "SkyguardCampaignDefinition.cpp",
)
# Invented returned-mission-pointer tokens stay unlocked.
...
class FindMissionDeclContractTests(unittest.TestCase):
    def test_campaign_definition_class_exists(self) -> None:
        ...
        self.assertTrue(has_declaration(section, FIND_MISSION), section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, FIND_MISSION)
        self.assertIn("FindMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, FIND_MISSION))

    def test_missing_find_mission_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tFName CampaignId;\n"
            "\tFText DisplayName;\n"
            "\tTArray<TObjectPtr<USkyguardMissionDefinition>> Missions;\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, FIND_MISSION)
        self.assertIn("FindMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UFUNCTION_FIND}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, FIND_MISSION)
        self.assertIn("FindMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UFUNCTION_FIND, section)
        self.assertIn("BlueprintPure", section)
        self.assertIn('Category = "Campaign"', section)
        self.assertTrue(has_declaration(section, FIND_MISSION), section)
        self.assertNotIn("BlueprintCallable", FIND_MISSION)
        self.assertNotIn("BlueprintPure", FIND_MISSION)
        self.assertNotIn("UFUNCTION", FIND_MISSION)
        self.assertNotIn("Category", FIND_MISSION)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, FIND_MISSION)
        self.assertIn("FindMission", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_arg = "\tUSkyguardMissionDefinition* FindMission() const;\n"
        non_const = (
            "\tUSkyguardMissionDefinition* FindMission(FName MissionId);\n"
        )
        wrong_return = "\tvoid FindMission(FName MissionId) const;\n"
        bool_return = "\tbool FindMission(FName MissionId) const;\n"
        wrong_arg = (
    ...
    def test_find_mission_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, FIND_MISSION),
            FIND_MISSION,
        )
        self.assertTrue(has_declaration(section, FIND_MISSION))
        self.assertEqual(
            declaration_count(section, FIND_MISSION),
            1,
        )
        self.assertTrue(
            FIND_MISSION.endswith("const;"),
            FIND_MISSION,
        )
        self.assertTrue(
            FIND_MISSION.startswith("USkyguardMissionDefinition* "),
            FIND_MISSION,
        )
        self.assertIn("FName MissionId", FIND_MISSION)
        self.assertNotIn("INDEX_NONE", FIND_MISSION)
        self.assertNotIn("{", FIND_MISSION)
        self.assertNotIn("}", FIND_MISSION)
        self.assertNotIn("return ", FIND_MISSION)
        self.assertNotIn("nullptr", FIND_MISSION)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_type = (
            ...
            "\tFindMission(FName MissionId) const;\n"
            "private:\n"
            "};\n"
        )
        wrap_arg = (
            "public:\n"
            "\tUSkyguardMissionDefinition* FindMission(FName\n"
            "\t\tMissionId) const;\n"
            "};\n"
        )
        wrap_const = (
        ...
        for header in (
            ...
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, FIND_MISSION),
                section,
            )
            self.assertEqual(
                require_declaration(section, FIND_MISSION),
        ...
        one_line = f"{{\npublic:\n\t{FIND_MISSION}\n}}\n"
        self.assertTrue(has_declaration(one_line, FIND_MISSION))
        section = public_section(origin_main_header())
        self.assertTrue(has_declaration(section, FIND_MISSION), section)
        self.assertEqual(
            require_declaration(section, FIND_MISSION),
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{FIND_MISSION}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", FIND_MISSION)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", FIND_MISSION)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_declaration_does_not_invent_returned_mission_pointer(self) -> None:
        locked_only = f"{FIND_MISSION}\n"
        for token in RETURNED_POINTER_NOT_LOCKED:
            self.assertNotIn(token, FIND_MISSION)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("return ", FIND_MISSION)
        self.assertNotIn("nullptr", FIND_MISSION)
        self.assertNotIn("FindByPredicate", FIND_MISSION)
        self.assertNotIn("Found->Get()", FIND_MISSION)
        self.assertNotIn("{", FIND_MISSION)
        self.assertNotIn("}", FIND_MISSION)
        section = public_section(origin_main_header())
    ...
    def test_declaration_does_not_invent_campaign_id_contents(self) -> None:
        ...
        for token in CAMPAIGN_ID_CONTENTS_NOT_LOCKED:
            self.assertNotIn(token, FIND_MISSION)
            self.assertNotIn(token, locked_only)
        self.assertNotIn("TEXT(", FIND_MISSION)
        self.assertNotIn("CampaignId =", FIND_MISSION)

    def test_contract_does_not_lock_find_mission_cpp_body(self) -> None:
        locked_only = f"{FIND_MISSION}\n"
        self.assertNotIn("{", FIND_MISSION)
        self.assertNotIn("}", FIND_MISSION)
        self.assertNotIn("return ", FIND_MISSION)
        ...
        self.assertNotIn("FindByPredicate", FIND_MISSION)
        self.assertNotIn("Found->Get()", FIND_MISSION)
        self.assertNotIn("return nullptr", FIND_MISSION)

    def test_contract_does_not_relock_validate_definition(self) -> None:
        locked_only = f"{FIND_MISSION}\n"
        for neighbor in VALIDATE_DEFINITION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FIND_MISSION)
        self.assertNotIn("ValidateDefinition", FIND_MISSION)
        self.assertNotIn("ValidateDefinition", locked_only)
        self.assertNotIn("OutErrors", FIND_MISSION)
        self.assertNotIn("BlueprintCallable", FIND_MISSION)
    ...
    def test_contract_does_not_relock_primary_asset_id(self) -> None:
        ...
        for neighbor in PRIMARY_ASSET_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FIND_MISSION)
        self.assertNotIn("GetPrimaryAssetId", FIND_MISSION)
        self.assertNotIn("GetPrimaryAssetId", locked_only)
        self.assertNotIn("FPrimaryAssetId", FIND_MISSION)
    ...
    def test_contract_does_not_relock_campaign_fields(self) -> None:
        ...
        for neighbor in CAMPAIGN_FIELDS_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, FIND_MISSION)
        self.assertNotIn("CampaignId", FIND_MISSION)
        self.assertNotIn("DisplayName", FIND_MISSION)
        self.assertNotIn("Missions", FIND_MISSION)
    ...
    def test_contract_does_not_relock_campaign_subsystem_methods(self) -> None:
        ...
        self.assertNotIn("ConfigureCampaign", FIND_MISSION)
        self.assertNotIn("CanStartMission", FIND_MISSION)
        self.assertNotIn("StartMission", FIND_MISSION)
        self.assertNotIn("GetActiveMission", FIND_MISSION)
        self.assertNotIn("USkyguardCampaignSubsystem", FIND_MISSION)
    ...
    def test_contract_does_not_relock_save_game_helpers(self) -> None:
        ...
        for neighbor in SAVE_GAME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FIND_MISSION)
        self.assertNotIn("ApplySaveGame", FIND_MISSION)
        self.assertNotIn("BuildSaveGame", FIND_MISSION)
        self.assertNotIn("RetrySaveLastDebrief", FIND_MISSION)
        self.assertNotIn("SaveCampaignToSlot", FIND_MISSION)
        self.assertNotIn("LoadCampaignFromSlot", FIND_MISSION)
        self.assertNotIn("DeleteCampaignSlot", FIND_MISSION)
        self.assertNotIn("GetMissionRecords", FIND_MISSION)
    ...
    def test_contract_does_not_relock_fill_and_gunner_helpers(self) -> None:
        ...
        for neighbor in FILL_AND_GUNNER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, FIND_MISSION)
        self.assertNotIn("FillResultCombatStats", FIND_MISSION)
        self.assertNotIn("ASkyguardGunner", FIND_MISSION)
        self.assertNotIn("FillAndFinalize", FIND_MISSION)
        self.assertNotIn("FillAndFail", FIND_MISSION)
        self.assertNotIn("ApplyHydraForClusters", FIND_MISSION)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{FIND_MISSION}\n"
        self.assertEqual(
            require_declaration(locked_only, FIND_MISSION),
            FIND_MISSION,
        )
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(FIND_MISSION, "Rifle")
        self.assertNotEqual(FIND_MISSION, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"campaign FindMission contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, FIND_MISSION.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", FIND_MISSION)

    def test_contract_is_find_mission_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
