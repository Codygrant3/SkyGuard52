...
# UPROPERTY(Transient).
# THIS IS leftover-safe Mission09 readiness-struct
# bSortiePresentationReady. THIS IS NOT leftover
# Mission09 readiness-struct bPresentationReady
# (sibling). THIS IS NOT leftover Mission08
...
# same-name readiness-struct drafts. THIS IS NOT leftover
# Mission09 Blueprint UFUNCTION methods. THIS IS NOT leftover
# director-class public UPROPERTY fields. THIS IS NOT leftover
# spawn-location / leftover runtime-ready flag (skip leftover
# b + Ya + kRuntimeReady forever). THIS IS NOT leftover
...
# Mission09 IntegrationReadiness bSortiePresentationReady
...
LEFTOVER_MISSION09_PRESENTATION_READY_NOT_LOCKED = (
    "test_mission09_presentation_ready_field_decl_contract.py",
    "cursor/mission09-presentation-ready-field-decl-contract-c332",
...
class Mission09SortiePresentationReadyFieldDeclContractTests(unittest.TestCase):
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission09 bSortiePresentationReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission09 bSortiePresentationReady contains "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission09_sortie_presentation"
                "_ready_field_decl_contract.py"
            )
        )
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_mission09_presentation_ready"
            "_field_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
    ...
    def test_contract_skips_leftover_retired_runtime_ready(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, PRESENTATION_READY)
        self.assertNotIn(PRESENTATION_READY, LOCKED_DECL)
        leftover_m08_only = (
            "struct FSkyguardMission08IntegrationReadiness\n"
            "{\n"
            "public:\n"
        ...
        for token in LEFTOVER_MISSION08_SORTIE_PRESENTATION_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION09_PRESENTATION_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
