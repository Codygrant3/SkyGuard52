# THIS IS leftover-safe USkyguardArcadeLookComponent Gamma.
...
# Identifier IS Gamma. Initializer IS `= 0.92f`.
...
# type is not float, if identifier is not Gamma, or
# if initializer is not `= 0.92f`.
...
# Gamma = 0.92f is NOT leftover analog
# SkyguardApacheCpgFeel CannonRecoilPitch = 0.92f.
# Lock the FULL decl including identifier Gamma.
# Fail-closed if CannonRecoilPitch /
# SkyguardApacheCpgFeel is treated as this slot.
...
# RETARGET: type is float, identifier is Gamma,
# initializer is 0.92f, UPROPERTY is EditAnywhere
...
# bEnabled / Contrast. Copy that parse window.
# Do NOT claim bEnabled or Contrast as this slot.
...
TARGET = "float Gamma = 0.92f;"
TARGET_WRONG_BARE = "float Gamma;"
TARGET_WRONG_FALSE = "float Gamma = false;"
TARGET_WRONG_TRUE = "float Gamma = true;"
TARGET_WRONG_ZERO = "float Gamma = 0.f;"
TARGET_WRONG_ONE = "float Gamma = 1.f;"
TARGET_WRONG_NO_F = "float Gamma = 0.92;"
TARGET_WRONG_FNAME = "FName Gamma;"
TARGET_WRONG_INT = "int32 Gamma = 0;"
TARGET_WRONG_BOOL = "bool Gamma = true;"
...
TARGET_WRONG_CANNON_RATE = "float Gamma = 12.0f;"
TARGET_WRONG_CANNON_RECOIL = "float CannonRecoilPitch = 0.92f;"
TARGET_WRONG_CANNON_DAMAGE = "float Gamma = 22.0f;"
TARGET_WRONG_TIME_OF_DAY = "float Gamma = 12.f;"
TARGET_WRONG_SENSOR_ACQUIRE = "float Gamma = 5.5f;"
TARGET_WRONG_DETECT_PROGRESS = "float Gamma = 0.22f;"
TARGET_WRONG_CLOUD = "float Gamma = 0.25f;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if re.search(
        ...
        return False
    if APACHE_CPG_FEEL_NS in compact and not has_identifier(compact, "Gamma"):
        return False
    if re.search(
        r"FName\s+Gamma\b",
        compact,
...
def claimed_field_window(header: str) -> str:
    ...
    raise AssertionError(
        "UPROPERTY for float Gamma = 0.92f; is missing from "
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+Gamma\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float Gamma is missing from "
        f"origin/main:{HEADER_PATH} class {CLASS_NAME} public "
        "UPROPERTY fields"
    )
...
class ArcadeLookGammaFieldDeclContractTests(unittest.TestCase):
    def test_arcade_look_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Gamma"), section)
    ...
    def test_neighbor_class_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Gamma"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("Gamma", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_gamma_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float Gamma = 0.92f;", compact_origin)
        self.assertNotIn("Gamma = NAME_None", compact_origin)
        self.assertNotIn("Gamma = false", compact_origin)
        self.assertNotIn("Gamma = true", compact_origin)
        self.assertNotIn("Gamma = 0.f", compact_origin)
        self.assertNotIn("Gamma = 160.f", compact_origin)
        self.assertNotIn("Gamma = 12.0f", compact_origin)
        self.assertNotIn("Gamma = 22.0f", compact_origin)
        self.assertNotIn("Gamma = 1.18f", compact_origin)
        self.assertNotIn("Gamma = 0.22f", compact_origin)

    def test_gamma_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float Gamma"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 0.92f", LOCKED_DECL)
        self.assertIn("Gamma", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tfloat " + leftover_retired_primary_hits_field() + " = 0.92f;\n"
        )
        leftover_guided = (
            "\tfloat " + leftover_retired_guided_hits_field() + " = 0.92f;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
    ...
    def test_enabled_and_contrast_siblings_are_not_this_slot(self) -> None:
    ...
    def test_claimed_window_stops_before_bloom(self) -> None:
        claimed = claimed_field_window(origin_main_header())
        self.assertIn("Gamma", claimed)
        self.assertIn("= 0.92f", claimed)
    ...
    def test_leftover_analog_feel_initializers_do_not_satisfy(self) -> None:
        ...
        self.assertEqual(LOCKED_DECL, "float Gamma = 0.92f;")
        self.assertIn("Gamma", LOCKED_DECL)
        self.assertNotIn("CannonRecoilPitch", LOCKED_DECL)
        self.assertNotIn(APACHE_CPG_FEEL_NS, LOCKED_DECL)
        feel = (
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "arcade-look Gamma field decl contract "
