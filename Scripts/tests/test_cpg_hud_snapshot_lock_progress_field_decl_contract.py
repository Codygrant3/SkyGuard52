# THIS IS leftover-safe FSkyguardCpgHudSnapshot LockProgress.
...
# `float LockProgress = 0.f;` with that in-struct initializer.
# Fail-closed if LockProgress is missing or renamed, if the
# type is not float, if the initializer is missing or is
# not `0.f`, or if UPROPERTY / Category clones land.
...
# locked decl.
# Fail-closed if this lane locks leftover sibling
# `FString LockLine;` (leftover-safe #1505) or leftover
# sibling `ESkyguardGuidedLockPhase LockPhase =
# ESkyguardGuidedLockPhase::Search;` (leftover-safe #1508)
# instead of LockProgress. Fail-closed if this lane locks
# leftover sibling `float HeadingDegrees = 0.f` or leftover
# ContactMark `bool bLocked = false` / `float LockAlpha = 0.f`.
# Harbor 40/80 is fail-closed as a false LockProgress
# initializer via split tokens. The HUD initializer is
# `0.f`, not Harbor 40/80.
...
# ThreatCount / HeadingDegrees / FlareCount /
# bMissileInbound.
...
# WeaponLine #1502 / ThreatLine #1503 / EufdLine #1504 /
# LockLine #1505 / SightLine #1506 / StationStatus #1507 /
# LockPhase #1508. Keep those files in LOCKED_SCRIPTS.
...
# struct. HUD parse-window retarget is leftover-safe
# #1502 WeaponLine (FString, no initializer). RETARGET:
# type is float, identifier is LockProgress, initializer
# is `= 0.f`, locked decl is the plain field, not a
# UPROPERTY wrap. Leftover-safe #1508 LockPhase is a
# sibling enum. Leftover-safe #1505 LockLine is a sibling
# FString. Do not contract LockPhase or LockLine here.
...
TARGET = "float LockProgress = 0.f;"
TARGET_WRONG_BARE = "float LockProgress;"
TARGET_WRONG_EQ_EMPTY = 'float LockProgress = TEXT("");'
TARGET_WRONG_EQ_NONE = "float LockProgress = NAME_None;"
TARGET_WRONG_FALSE = "float LockProgress = false;"
TARGET_WRONG_TRUE = "float LockProgress = true;"
TARGET_WRONG_NEG_ONE = "float LockProgress = -1.f;"
TARGET_WRONG_FSTRING = "FString LockProgress;"
TARGET_WRONG_HEALTH = "float LockProgress = 160.f;"
TARGET_WRONG_BOOL = "bool LockProgress;"
TARGET_WRONG_INT = "int32 LockProgress;"
TARGET_WRONG_FNAME = "FName LockProgress;"
TARGET_WRONG_TCHAR = 'const TCHAR* LockProgress = TEXT("");'
...
TARGET_WRONG_RANGE_METERS = "float RangeMeters = -1.f;"
...
LOCK_PROGRESS_LOCKED_RE = re.compile(
    r"float\s+LockProgress\s*=\s*0\.f\s*;"
)
LOCK_PROGRESS_BARE_RE = re.compile(r"float\s+LockProgress\s*;")
LOCK_PROGRESS_INIT_RE = re.compile(
    r"float\s+LockProgress\s*=\s*([^;]+);"
)
LOCK_PROGRESS_WRONG_INIT_RE = re.compile(
    r"float\s+LockProgress\s*=(?!\s*0\.f\b)"
)
LOCK_LINE_LEFTOVER_RE = re.compile(r"FString\s+LockLine\s*;")
LOCKED_INITIALIZER = "0.f"
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def lock_progress_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*float\s+LockProgress\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float LockProgress = 0.f;`.
    # Do not accept a missing initializer, Harbor 40/80,
    # `= TEXT("")` / `= NAME_None` / `= false` / `= true` /
    # `= -1.f` / `= 160.f`. Do not accept leftover sibling
    # `FString LockLine;` (#1505) or leftover sibling
    # LockPhase (#1508) or leftover sibling HeadingDegrees
    # or leftover ContactMark bLocked / LockAlpha. Do not
    # accept leftover LoadoutSpec FlareCount. Do not accept
    # UPROPERTY / Category clones.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if LOCK_LINE_LEFTOVER_RE.search(compact) and (
        LOCK_PROGRESS_LOCKED_RE.search(compact) is None
    ):
        return False
    if re.search(r"ESkyguardGuidedLockPhase\s+LockPhase\b", compact) and (
        LOCK_PROGRESS_LOCKED_RE.search(compact) is None
    ):
        return False
    if re.search(r"float\s+HeadingDegrees\b", compact) and (
        LOCK_PROGRESS_LOCKED_RE.search(compact) is None
    ):
        return False
    if re.search(r"float\s+LockAlpha\b", compact) and (
        LOCK_PROGRESS_LOCKED_RE.search(compact) is None
    ):
        return False
    if re.search(r"bool\s+bLocked\b", compact) and (
        LOCK_PROGRESS_LOCKED_RE.search(compact) is None
    ):
        return False
    if re.search(r"FString\s+LockProgress\b", compact):
        return False
    if re.search(r"bool\s+LockProgress\b", compact):
        return False
    if re.search(r"int32\s+LockProgress\b", compact):
        return False
    if re.search(r"FName\s+LockProgress\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*LockProgress\b", compact):
        return False
    if LOCK_PROGRESS_BARE_RE.search(compact) and (
        LOCK_PROGRESS_LOCKED_RE.search(compact) is None
    ):
        return False
    init_match = LOCK_PROGRESS_INIT_RE.search(compact)
    if init_match is None:
        return False
    if init_match.group(1).strip() != LOCKED_INITIALIZER:
        return False
    if LOCK_PROGRESS_WRONG_INIT_RE.search(compact):
        return False
    if LOCK_PROGRESS_LOCKED_RE.search(compact) is None:
        return False
    if lock_progress_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float LockProgress\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float LockProgress is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*float LockProgress\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on float LockProgress is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "LockProgress"):
        raise AssertionError(
            "UPROPERTY clone landed on LockProgress; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "LockProgress"):
            raise AssertionError(
                f"{token} clone landed on LockProgress; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotLockProgressFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LockProgress"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "float LockProgress = 0.f;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("float LockProgress"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= 0.f", LOCKED_DECL)
        self.assertIn("LockProgress", LOCKED_DECL)
        self.assertIn("float", LOCKED_DECL)
        self.assertNotIn(SIBLING_RANGE_LINE, LOCKED_DECL)
        self.assertNotIn(SIBLING_WEAPON_LINE, LOCKED_DECL)
        self.assertNotIn(SIBLING_FLARE_COUNT, LOCKED_DECL)
        self.assertNotIn(SIBLING_LOCK_LINE, LOCKED_DECL)
        self.assertNotIn(SIBLING_LOCK_PHASE, LOCKED_DECL)
        self.assertNotIn(SIBLING_RANGE_METERS, LOCKED_DECL)
        self.assertNotIn(SIBLING_HEADING, LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("LockProgress", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LockProgress"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_lock_progress_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_range_line_does_not_satisfy_lock_progress(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_RANGE_LINE}\n"
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RANGE_LINE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_RANGE_LINE)
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float LockProgress = 0.f;", compact_origin)
        self.assertNotIn("LockProgress = TEXT", compact_origin)
        self.assertNotIn("LockProgress = NAME_None", compact_origin)
        self.assertNotIn("LockProgress = false", compact_origin)
        self.assertNotIn("LockProgress = true", compact_origin)
        self.assertNotIn("LockProgress = -1.f", compact_origin)
        self.assertNotIn("LockProgress = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("LockProgress = " + forty, compact_origin)
        self.assertNotIn("LockProgress = " + eighty, compact_origin)

    def test_lock_progress_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float LockProgress"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_NEG_ONE}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RANGE_LINE}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_HEADING}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_RANGE_METERS}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertIn("missing", str(raised.exception).lower())
        with self.assertRaises(AssertionError) as raised:
            require_declaration(f"\t{TARGET_WRONG_RANGE_LINE}\n", LOCKED_DECL)
        self.assertIn("LockProgress", str(raised.exception))
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{LEFTOVER_CONTACT_LABEL}\n",
            "\tFString LockProgress;\n",
            "\tint32 LockProgress;\n",
            "\tbool LockProgress;\n",
            "\tfloat LockProgress = " + forty + ";\n",
            "\tfloat LockProgress = " + eighty + ";\n",
            f"\t{TARGET_WRONG_NEG_ONE}\n",
            f"\t{TARGET_WRONG_LOCK_LINE}\n",
            f"\t{TARGET_WRONG_LOCK_PHASE}\n",
            f"\t{TARGET_WRONG_HEADING}\n",
            f"\t{TARGET_WRONG_RANGE_METERS}\n",
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    lock_progress_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(lock_progress_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot LockProgress field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "lock_progress_field_decl_contract.py"
        ))
        ...
        self.assertIn(LEFTOVER_HUD_RANGE_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_WEAPON_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_THREAT_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_EUFD_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_LOCK_LINE, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_HUD_SIGHT_LINE, LOCKED_SCRIPTS)
    ...
    def test_leftover_analog_bulk_stays_locked(self) -> None:
        leftovers = leftover_debrief_isolated_scripts() + (
            LEFTOVER_ANALOG_DEBRIEF_DEFAULTS,
            LEFTOVER_LOADOUT_FLARE,
            CLONE_THEATER_WEATHER_IDENTITY,
            LEFTOVER_ANALOG_STORM_KIT_FIELDS,
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_RANGE_LINE, locked_only)
        self.assertNotIn(SIBLING_WEAPON_LINE, locked_only)
        self.assertNotIn(SIBLING_FLARE_COUNT, locked_only)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_RANGE_LINE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_WEAPON_LINE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_THREAT_LINE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_EUFD_LINE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_LOCK_LINE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_SIGHT_LINE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_STATION_STATUS)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_LOCK_PHASE)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_RANGE_METERS)
    ...
    def test_lock_line_does_not_satisfy_lock_progress(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_LOCK_LINE}\n"
    ...
    def test_lock_phase_does_not_satisfy_lock_progress(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_LOCK_PHASE}\n"
    ...
    def test_heading_degrees_does_not_satisfy_lock_progress(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_HEADING}\n"
        ...
        self.assertIn("= 0.f", LOCKED_DECL)
        self.assertIn("= 0.f", TARGET_WRONG_HEADING)

    def test_leftover_contact_lock_tokens_do_not_satisfy(self) -> None:
        for wrong in (
            TARGET_WRONG_LOCK_ALPHA,
            TARGET_WRONG_BLOCKED,
        ):
            leftover = (
                f"struct {STRUCT_NAME}\n"
                "{\n"
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(section, LOCKED_DECL), section)
            self.assertNotEqual(LOCKED_DECL, wrong)

    def test_range_meters_does_not_satisfy_lock_progress(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_RANGE_METERS}\n"
        ...
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(section, LOCKED_DECL), section)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_RANGE_METERS)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_HUD_RANGE_METERS)
