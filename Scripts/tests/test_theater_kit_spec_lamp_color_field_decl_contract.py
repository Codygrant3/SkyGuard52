# THIS IS leftover-safe FSkyguardTheaterKitSpec LampColor.
...
# then `FLinearColor LampColor = FLinearColor::White;`.
...
# or if the default is not `= FLinearColor::White`.
...
# CardId field decls.
# If a clone asserts bare FName / no default /
...
TARGET = "FLinearColor LampColor = FLinearColor::White;"
TARGET_WRONG_EQ_NONE = "FLinearColor LampColor = NAME_None;"
TARGET_WRONG_FALSE = "FLinearColor LampColor = false;"
TARGET_WRONG_TRUE = "FLinearColor LampColor = true;"
TARGET_WRONG_ZERO = "FLinearColor LampColor = 0.f;"
TARGET_WRONG_FLOAT = "float LampColor;"
TARGET_WRONG_FNAME = "FName LampColor;"
TARGET_WRONG_BARE = "FLinearColor LampColor;"
TARGET_WRONG_BLACK = "FLinearColor LampColor = FLinearColor::Black;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept bare `FLinearColor LampColor;` /
    # `= NAME_None` / `= false` / `= true` /
    # `= 0.f` / `= 160.f` / leftover float Health /
    # FName LampColor / Black when origin/main has
    # FLinearColor with White default. Do not accept
    # sibling WeatherIdentity / KitId / LandmarkSet /
    # BuildingKit / LampTreatment fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FName\s+LampColor\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+LampColor\b",
        compact,
    ):
        return False
    if re.search(
        r"FLinearColor\s+LampColor\s*;",
        compact,
    ):
        return False
    if re.search(
        r"FLinearColor\s+LampColor\s*=\s*FLinearColor::White\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FLinearColor\s+LampColor\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FLinearColor LampColor is missing from "
...
class TheaterKitSpecLampColorFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "LampColor"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "LampColor"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("LampColor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_lamp_color_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_or_wrong_default_fails_closed(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
        ...
        black_section = spec_section(wrong_black)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(black_section, LOCKED_DECL)
        self.assertIn("LampColor", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertNotIn("LampColor = NAME_None", compact_origin)
        self.assertNotIn("LampColor = false", compact_origin)
        self.assertNotIn("LampColor = true", compact_origin)
        self.assertNotIn("LampColor = 0.f", compact_origin)
        self.assertNotIn("LampColor = 160.f", compact_origin)
        self.assertNotIn("LampColor = FLinearColor::Black", compact_origin)

    def test_lamp_color_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FLinearColor LampColor"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_BARE}\n",
            f"\t{TARGET_WRONG_BLACK}\n",
            "\tFLinearColor LampColors = FLinearColor::White;\n",
            "\tint32 LampColor;\n",
            "\tbool LampColor;\n",
            "\tfloat LampColor = " + forty + ";\n",
            "\tfloat LampColor = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec LampColor field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "lamp_color_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
