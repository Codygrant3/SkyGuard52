# THIS IS leftover-safe FSkyguardLoadoutSpec PlaystyleLine.
# origin/main form (exact, may wrap across two lines):
# 	const TCHAR* PlaystyleLine =
# 		TEXT("30 mm station, mixed cannon, rockets, missiles");
# Compact accepted form after whitespace-normalize:
# const TCHAR* PlaystyleLine = TEXT("30 mm station, mixed cannon, rockets, missiles");
...
# Locked decl must be the BARE field `PlaystyleLine` with
# type `const TCHAR*` and TEXT initializer containing
# `30 mm station, mixed cannon, rockets, missiles`.
...
# Do NOT parse leftover enum bodies.
...
# #1451-#1465.
# Do NOT parse leftover LoadoutSpec isolated #1466-#1470.
# Do NOT parse leftover MeshBindSlot.
# Do NOT parse leftover TheaterKit.
# Do NOT parse leftover MissionDefinition.
...
# (split tokens).
# Exact identifier vs siblings Loadout / StartingStation /
# magazine-reserve fields / FlareCount / HullIntegrity.
# Do not claim sibling fields.
# Fail-closed if type is not `const TCHAR*`, initializer
# missing, or string is not the origin/main TEXT.
# The PlaystyleLine TEXT must stay Apache CPG
# (30 mm / rockets / missiles) and must not mention
# leftover live-copy tokens (split in this file).
...
# Harbor 40/80 fail-closed. This TEXT line is not Harbor.
...
LOCKED_TEXT = "30 mm station, mixed cannon, rockets, missiles"
TARGET = (
    "const TCHAR* PlaystyleLine = TEXT(\"" + LOCKED_TEXT + "\");"
)
TARGET_WRONG_BARE = "const TCHAR* PlaystyleLine;"
TARGET_WRONG_EMPTY = "const TCHAR* PlaystyleLine = TEXT(\"\");"
TARGET_WRONG_NULL = "const TCHAR* PlaystyleLine = nullptr;"
TARGET_WRONG_OTHER = (
    "const TCHAR* PlaystyleLine = TEXT(\"cannon only\");"
)
TARGET_WRONG_INT = "int32 PlaystyleLine = 6;"
TARGET_WRONG_FLOAT = "float PlaystyleLine = 6;"
TARGET_WRONG_BOOL = "bool PlaystyleLine = false;"
TARGET_WRONG_FNAME = "FName PlaystyleLine;"
TARGET_WRONG_FSTRING = "FString PlaystyleLine;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `const TCHAR*`
    # PlaystyleLine TEXT initializer. Do not accept a
    # missing initializer, wrong type, leftover sibling
    # FlareCount / HullIntegrity / magazine-reserve
    # fields, or a TEXT string that is not origin/main.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    compact_locked = collapsed(LOCKED_DECL)
    if compact_locked not in compact:
        return False
    if re.search(r"\bint32\s+PlaystyleLine\b", compact):
        return False
    if re.search(r"\bfloat\s+PlaystyleLine\b", compact):
        return False
    if re.search(r"\bbool\s+PlaystyleLine\b", compact):
        return False
    if re.search(r"\bFName\s+PlaystyleLine\b", compact):
        return False
    if re.search(r"\bFString\s+PlaystyleLine\b", compact):
        return False
    if re.search(r"const TCHAR\* PlaystyleLine;", compact) and not re.search(
        r"const TCHAR\* PlaystyleLine =",
        compact,
    ):
        return False
    if "nullptr" in compact and "PlaystyleLine = nullptr" in compact:
        return False
    return True
...
class LoadoutSpecPlaystyleLineFieldDeclContractTests(unittest.TestCase):
    def test_loadout_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PlaystyleLine"), section)
    ...
    def test_resolve_loadout_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "PlaystyleLine"), section)
        self.assertNotIn(STOP_BEFORE_RESOLVE, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("PlaystyleLine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_playstyle_line_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_locked_decl_is_bare_plain_field(self) -> None:
        ...
        self.assertTrue(LOCKED_DECL.startswith("const TCHAR* PlaystyleLine"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("TEXT(\"" + LOCKED_TEXT + "\")", LOCKED_DECL)
    ...
    def test_missing_or_wrong_text_initializer_fails_closed(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_BARE}\n"
        ...
        self.assertIn("PlaystyleLine", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(f"\t{TARGET_WRONG_BARE}\n", LOCKED_DECL))
        self.assertFalse(has_declaration(f"\t{TARGET_WRONG_EMPTY}\n", LOCKED_DECL))
        self.assertFalse(has_declaration(f"\t{TARGET_WRONG_OTHER}\n", LOCKED_DECL))
        origin = spec_section(origin_main_header())
        compact_origin = collapsed(origin)
        self.assertIn(collapsed(LOCKED_DECL), compact_origin)
        self.assertNotIn(collapsed(TARGET_WRONG_BARE), compact_origin)
        self.assertNotIn(collapsed(TARGET_WRONG_EMPTY), compact_origin)
    ...
    def test_playstyle_line_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("const TCHAR* PlaystyleLine"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("SkyguardApacheCpgFeel::", LOCKED_DECL)
        self.assertNotIn("Category", LOCKED_DECL)
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_OTHER}\n", LOCKED_DECL)
        )
        self.assertFalse(
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FLARE}\n", LOCKED_DECL)
        )
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tconst TCHAR*\n\tPlaystyleLine = TEXT(\""
            + LOCKED_TEXT + "\");\n",
            "\tconst TCHAR*   PlaystyleLine = TEXT(\""
            + LOCKED_TEXT + "\");\n",
            "\tconst TCHAR*\tPlaystyleLine = TEXT(\""
            + LOCKED_TEXT + "\");\n",
            "\tconst TCHAR* PlaystyleLine =\n"
    ...
    def test_harbor_40_80_tokens_fail_closed_if_present(self) -> None:
        ...
        self.assertIn(LOCKED_TEXT, LOCKED_DECL)
        self.assertIn("30 mm", LOCKED_DECL)
        self.assertIn("rockets", LOCKED_DECL)
        self.assertIn("missiles", LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "loadout-spec PlaystyleLine field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "playstyle_line_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunshipTypes.cpp", THIS_SCRIPT)
        ...
        self.assertIn(CLONE_FLARE_COUNT, LOCKED_SCRIPTS)
        self.assertIn(CLONE_LOADOUT_FIELD, LOCKED_SCRIPTS)
        self.assertIn(CLONE_STARTING_STATION, LOCKED_SCRIPTS)
        self.assertIn(CLONE_CANNON_MAG, LOCKED_SCRIPTS)
        self.assertIn(CLONE_CANNON_RESERVE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_ROCKET_MAG, LOCKED_SCRIPTS)
        self.assertIn(CLONE_ROCKET_RESERVE, LOCKED_SCRIPTS)
        self.assertIn(CLONE_GUIDED_MAG, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(LEFTOVER_CPG_FEEL, locked_only)
