# THIS IS leftover-safe FSkyguardTheaterKitSpec SilhouetteTint.
...
# BlueprintReadOnly IS present. Default MUST be
# `= FLinearColor::White`.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if Category is missing, if specifiers
# drop to BlueprintReadWrite / EditAnywhere-only, or if
# the default is not `= FLinearColor::White`.
...
# Do NOT contract sibling fields WeatherIdentity / KitId /
...
# the bulk).
# THIS IS NOT leftover sibling drafts #1299-#1311.
...
# CardId field decls.
# If a WeatherIdentity clone asserts bare FName / no default
# / Category="Skyguard|Campaign", retarget: type is
# FLinearColor, default is `= FLinearColor::White`,
...
TARGET = "FLinearColor SilhouetteTint = FLinearColor::White;"
TARGET_WRONG_EQ_NONE = "FLinearColor SilhouetteTint = NAME_None;"
TARGET_WRONG_FALSE = "FLinearColor SilhouetteTint = false;"
TARGET_WRONG_TRUE = "FLinearColor SilhouetteTint = true;"
TARGET_WRONG_ZERO = "FLinearColor SilhouetteTint = 0.f;"
TARGET_WRONG_FLOAT = "float SilhouetteTint;"
TARGET_WRONG_BARE = "FLinearColor SilhouetteTint;"
TARGET_WRONG_FNAME = "FName SilhouetteTint;"
TARGET_WRONG_BLACK = "FLinearColor SilhouetteTint = FLinearColor::Black;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # `FLinearColor SilhouetteTint = FLinearColor::White;`.
    # Do not accept bare FLinearColor / FName / no default /
    # `= NAME_None` / `= false` / `= true` / `= 0.f` /
    # `= FLinearColor::Black` / leftover float Health when
    # origin/main default is `= FLinearColor::White`.
    # Do not accept sibling WeatherIdentity / KitId /
    # LandmarkSet / BuildingKit / BuildingTint fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FName\s+SilhouetteTint\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SilhouetteTint\b",
        compact,
    ):
        return False
    if re.search(
        r"FLinearColor\s+SilhouetteTint\s*;",
        compact,
    ):
        return False
    assigned = re.findall(
        r"FLinearColor\s+SilhouetteTint\s*=\s*(\S[^;]*);",
        compact,
    )
    if not assigned:
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FLinearColor\s+SilhouetteTint\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FLinearColor SilhouetteTint is missing from "
...
class TheaterKitSpecSilhouetteTintFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_spec_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "SilhouetteTint"), section)
    ...
    def test_actor_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SilhouetteTint"), section)
        self.assertNotIn(STOP_BEFORE_NAMESPACE, section)
        self.assertNotIn(ACTOR_NAME, section)
        self.assertNotIn(STOP_BEFORE_APPLY, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SilhouetteTint", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_silhouette_tint_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_bare_decl_fails_closed_when_origin_has_white_default(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_VISIBLE_READONLY}\n"
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(spec_section(wrong_none), LOCKED_DECL)
        self.assertIn("SilhouetteTint", str(raised.exception))
        ...
        self.assertNotIn("SilhouetteTint = NAME_None", compact_origin)
        self.assertNotIn("SilhouetteTint = false", compact_origin)
        self.assertNotIn("SilhouetteTint = true", compact_origin)
        self.assertNotIn("SilhouetteTint = 0.f", compact_origin)
        self.assertNotIn("SilhouetteTint = 160.f", compact_origin)
        self.assertNotIn("SilhouetteTint = FLinearColor::Black", compact_origin)

    def test_silhouette_tint_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FLinearColor SilhouetteTint"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= FLinearColor::White", LOCKED_DECL)
        ...
        self.assertNotIn("= FLinearColor::Black", LOCKED_DECL)
        self.assertIn("FLinearColor ", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BLACK}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tFLinearColor " + leftover_retired_primary_hits_field()
            + " = FLinearColor::White;\n"
        )
        leftover_guided = (
            "\tFLinearColor " + leftover_retired_guided_hits_field()
            + " = FLinearColor::White;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            f"\t{TARGET_WRONG_BLACK}\n",
            f"\t{TARGET_WRONG_HEALTH}\n",
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_KIT_ID};\n",
            f"\tFName {SIBLING_WEATHER_IDENTITY};\n",
            "\tFLinearColor SilhouetteTints = FLinearColor::White;\n",
            "\tint32 SilhouetteTint;\n",
            "\tbool SilhouetteTint;\n",
            "\tfloat SilhouetteTint = " + forty + ";\n",
            "\tfloat SilhouetteTint = " + eighty + ";\n",
        )
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tFLinearColor\n\tSilhouetteTint = FLinearColor::White;\n",
            "\tFLinearColor   SilhouetteTint = FLinearColor::White;\n",
            "\tFLinearColor\tSilhouetteTint = FLinearColor::White;\n",
            "\tFLinearColor SilhouetteTint =\n\tFLinearColor::White;\n",
            f"\t{LOCKED_DECL}\n",
            f"\t{UPROPERTY_VISIBLE_READONLY}\n\t{LOCKED_DECL}\n",
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit-spec SilhouetteTint field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "silhouette_tint_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
