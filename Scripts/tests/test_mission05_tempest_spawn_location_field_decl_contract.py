...
# INDEX_NONE or lock TempestSpawnLocation
# construction in the .cpp. This is leftover-safe
# Mission05 TempestSpawnLocation FIELD
# (declaration-only, one-line / split-line
# UPROPERTY wraps plus the FVector default)
# on ASkyguardMission05IntegrationDirector.
# SKIP leftover retired-mount spawn fields
# forever. This contract is TempestSpawnLocation
# only. It is NOT leftover Mission04
# BlackKiteSpawnLocation field drafts, NOT leftover
# Mission03 RoadHunterSpawnLocation field drafts,
# NOT leftover Mission02 BreakwaterSpawnLocation
# field drafts, NOT leftover Mission10
# LastFlightSpawnLocation field drafts, NOT leftover
...
# FVector(70000.f, 24000.f, 7600.f), not
...
LOCKED_DECL = TEMPEST_SPAWN_LOCATION
...
class Mission05TempestSpawnLocationFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("TempestSpawnLocation", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_tempest_spawn_location_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_tempest_spawn_location_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FVector TempestSpawnLocation"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("TempestSpawnLocation", LOCKED_DECL)
        self.assertIn("= FVector(70000.f, 24000.f, 7600.f)", LOCKED_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tFVector\n"
            "\tTempestSpawnLocation = FVector(70000.f, 24000.f, 7600.f);\n"
            "private:\n"
            "};\n"
        )
        wrap_spaces = (
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("TempestSpawnLocation", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("FVector TempestSpawnLocation"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission05 TempestSpawnLocation field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission05 TempestSpawnLocation contains "
    ...
    def test_contract_is_tempest_spawn_location_field_declaration_only(
        self,
    ) -> None:
        header = origin_main_header()
        section = public_section(header)
    ...
    def test_sibling_director_fields_do_not_satisfy_tempest_spawn_location(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission05_tempest_spawn_location_field"
                "_decl_contract.py"
            )
        )
        self.assertIn(
    ...
    def test_storm_runtime_struct_does_not_satisfy_tempest_spawn_location(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
    ...
    def test_readiness_struct_does_not_satisfy_tempest_spawn_location(
        self,
    ) -> None:
        leftover = (
            "USTRUCT(BlueprintType)\n"
