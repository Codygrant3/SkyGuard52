...
STRUCT_NAME = "FSkyguardMission06IntegrationReadiness"
...
# struct section of
# FSkyguardMission06IntegrationReadiness only
# (not ASkyguardMission06IntegrationDirector public
...
# the locked FSkyguardMission06IntegrationReadiness
# public section. Do not parse
# ASkyguardMission06IntegrationDirector director-class
...
# Mission05 bObjectivesReady #968 / leftover
# Mission05 bProtectedTargetsReady #970 / leftover
# Mission05 ProtectedTargetCount #978.
...
class Mission06ProtectedTargetsReadyFieldDeclContractTests(unittest.TestCase):
    def test_mission06_struct_exists(self) -> None:
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission06 bObjectivesReady field contract "
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                LOCKED_DECL.lower(),
                "mission06 bObjectivesReady contains "
    ...
    def test_contract_does_not_parse_director_class_or_storm_runtime(
        ...
        for token in LEFTOVER_SAME_NAME_OBJECTIVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION05_OBJECTIVES_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION05_PROTECTED_TARGETS_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION05_PROTECTED_TARGET_COUNT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
        for token in LEFTOVER_MISSION10_PROTECTED_GROUPS_READY_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, LOCKED_DECL)
