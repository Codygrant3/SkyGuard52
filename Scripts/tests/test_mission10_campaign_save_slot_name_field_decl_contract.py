...
# Mission10 CampaignSaveSlotName FIELD
...
class Mission10CampaignSaveSlotNameFieldDeclContractTests(unittest.TestCase):
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        this_script = (
            "Scripts/tests/test_mission10_campaign_save_slot_name_field"
            "_decl_contract.py"
        )
        self.assertNotIn(this_script, LOCKED_SCRIPTS)
        self.assertTrue(
            Path(__file__).name.endswith(
                "test_mission10_campaign_save_slot_name_field"
                "_decl_contract.py"
            )
        )
        self.assertIn(
