...
# then BARE `void DeactivateAllEffects();`
# (declaration-only, no inline body required).
# THIS IS leftover-safe unique analog environment VFX UFUNCTION.
# Category IS `Skyguard|Environment|VFX`. BlueprintCallable IS present.
...
# without Skyguard| / Category="Skyguard|Environment"
# without leftover-safe unique |VFX suffix /
...
# Stop BEFORE first public UPROPERTY (`PoolCapacity`).
# Do NOT claim sibling UFUNCTION
# ActivatePooledEffect as this slot
# (leave it uncontracted). Word-boundary match
# DeactivateAllEffects vs ActivatePooledEffect.
# Do NOT claim sibling GetAllocatedPoolSize /
# GetActivationCount / PoolCapacity.
# Do NOT retarget to ActivatePooledEffect /
# GetAllocatedPoolSize / GetActivationCount.
# Do NOT claim leftover analog Mission09 PoolCapacity #1116.
# Do NOT claim leftover analog environment-vfx-pool-fail-closed
# bulk #139 (behavioral; wrap Category VFX UFUNCTION
# declaration only).
# Do NOT claim leftover analog EnvironmentReadiness
# VFXPoolSize #1169.
# Identifier DeactivateAllEffects is NOT leftover analog
# Mission09 PoolCapacity #1116 and NOT leftover analog
# environment-vfx-pool-fail-closed bulk #139 and NOT leftover
# analog EnvironmentReadiness VFXPoolSize #1169.
# Clone leftover unique Validation BlueprintCallable #1694
# RETARGET method/return/args. KEEP BlueprintCallable.
# This slot has NO Material arg.
# Clone leftover unique BlueprintCallable #1680
# RETARGET Category Production → Skyguard|Environment|VFX.
# RETARGET void EnsureDefaultEntries(); → void DeactivateAllEffects();
# FAIL-CLOSED if LOCKED_DECL is void EnsureDefaultEntries();.
# Clone leftover unique Validation BeginTransient #1697
# RETARGET method; this slot has NO Material arg.
# Clone leftover unique BlueprintPure GetSortieHits #1685
# RETARGET Category Combat|Sortie → Skyguard|Environment|VFX.
...
# LOCKED_DECL is `void DeactivateAllEffects();`
# NOT `FName WeatherIdentity;` and NOT 160.f and NOT
# FireRate = 12.0f / RecoilPitch = 0.92f and NOT
# RouteLengthCm = 45000.f and NOT int32 GetSortieHits() const;
...
# and NOT bool ActivatePooledEffect( and NOT
...
# WeatherIdentity / 160.f / FireRate = 12.0f /
...
# void EnsureDefaultEntries(); / bool ActivatePooledEffect( /
# int32 GetAllocatedPoolSize() const; /
# int32 GetActivationCount() const; / int32 PoolCapacity = 12
# AS THE LOCKED DECL.
...
LEFTOVER_AUTHORING_LIBRARY_CLASS = (
    "USkyguardMission01EnvironmentAuthoringLibrary"
)
LEFTOVER_MISSION09_CLASS = "ASkyguardMission09IntegrationDirector"
LEFTOVER_READINESS_STRUCT = "FSkyguardEnvironmentReadiness"
LEFTOVER_AUDIO_DIRECTOR_CLASS = "USkyguardAudioDirectorComponent"
TARGET = "void DeactivateAllEffects();"
TARGET_INLINE = (
    "void DeactivateAllEffects() { }"
)
TARGET_WRONG_BARE = "void DeactivateAllEffects() const;"
TARGET_WRONG_NO_CONST = (
    "bool DeactivateAllEffects();"
)
TARGET_WRONG_VOID = "bool AreResolvedProductionLoopRoutesComplete() const;"
TARGET_WRONG_ENSURE_DEFAULT = "void EnsureDefaultEntries();"
TARGET_WRONG_ACTIVATE_POOLED = "bool ActivatePooledEffect("
TARGET_WRONG_ALLOCATED_POOL = "int32 GetAllocatedPoolSize() const;"
...
TARGET_WRONG_MATERIAL_ARG = (
    "void DeactivateAllEffects(UMaterialInterface* GovernedMaterial);"
)
TARGET_WRONG_VFX_POOL_SIZE = "int32 VFXPoolSize = 0;"
SIBLING_ACTIVATE_POOLED = "ActivatePooledEffect"
SIBLING_ALLOCATED_POOL = "GetAllocatedPoolSize"
SIBLING_ACTIVATION_COUNT = "GetActivationCount"
SIBLING_POOL_CAPACITY = "PoolCapacity"
UFUNCTION_ENVIRONMENT_ONLY = (
    'UFUNCTION(BlueprintCallable, Category="Skyguard|Environment")'
)
UFUNCTION_ENVIRONMENT_PERFORMANCE = (
...
TARGET_WRONG_TRUE = "bool DeactivateAllEffects = true;"
TARGET_WRONG_ZERO = "bool DeactivateAllEffects = 0.f;"
...
STOP_BEFORE_EVENT_DEFINITIONS = "PoolCapacity"
SIBLING_INITIALIZE_REQUIRED = "ActivatePooledEffect"
...
SIBLING_ENTRIES = "PoolCapacity"
SIBLING_ROUTING = "SmokeSystem"
...
STOP_BEFORE_SKYLINE_STYLE = "AllocatePool"
...
THIS_SCRIPT = (
    "Scripts/tests/test_environment_vfx_deactivate_all_effects"
    "_decl_contract.py"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_ACTIVATE_POOLED,
        SIBLING_ALLOCATED_POOL,
        SIBLING_ACTIVATION_COUNT,
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover unique EnsureDefaultEntries / ActivatePooledEffect /
    ...
    if METHOD_SIGNATURE_RE.search(compact) is None:
        return False
    if re.search(r"\bvoid\s+DeactivateAllEffects\s*\(\s*\)\s*const\b", compact):
        return False
    if re.search(r"\bbool\s+DeactivateAllEffects\b", compact):
        return False
    if re.search(r"\bfloat\s+DeactivateAllEffects\b", compact):
        return False
    if re.search(r"\bint32\s+DeactivateAllEffects\b", compact):
        return False
    if re.search(
        r"\bvoid\s+DeactivateAllEffects\s*\(\s*[^)]+\s*\)",
        compact,
...
def attached_ufunction_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*void\s+DeactivateAllEffects\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UFUNCTION for void DeactivateAllEffects "
...
class EnvironmentVFXDeactivateAllEffectsDeclContractTests(
    unittest.TestCase
):
    def test_environment_vfx_pool_class_exists(self) -> None:
        ...
        self.assertNotEqual(CLASS_NAME, LEFTOVER_AUTHORING_LIBRARY_CLASS)
        self.assertNotEqual(CLASS_NAME, LEFTOVER_MISSION09_CLASS)
        ...
        self.assertTrue(
            has_identifier(section, "DeactivateAllEffects"),
            section,
        )
        ...
        self.assertTrue(has_identifier(body, "ResolveSystem"), body)
        self.assertFalse(has_identifier(section, "ResolveSystem"), section)
    ...
    def test_missing_deactivate_all_effects_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertTrue("|VFX" in specifiers)
        self.assertNotEqual(
            specifiers,
    ...
    def test_missing_or_wrong_signature_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "void DeactivateAllEffects()const;",
    ...
    def test_deactivate_all_effects_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENSURE_DEFAULT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ACTIVATE_POOLED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ALLOCATED_POOL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_MATERIAL_ARG)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_VFX_POOL_SIZE)
        ...
        self.assertNotIn("VFXPoolSize", LOCKED_DECL)
        ...
        self.assertNotIn("UMaterialInterface", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_UFUNCTION_WRAP, UFUNCTION_ENVIRONMENT_ONLY)
        self.assertNotEqual(LOCKED_UFUNCTION_WRAP, UFUNCTION_ENVIRONMENT_PERFORMANCE)
        self.assertNotEqual(LOCKED_UFUNCTION_WRAP, UFUNCTION_VALIDATION)
        self.assertNotEqual(LOCKED_UFUNCTION_WRAP, UFUNCTION_AUTHORING)
        self.assertNotEqual(LOCKED_UFUNCTION_WRAP, UFUNCTION_COMBAT_SORTIE)
        ...
        self.assertNotEqual(LOCKED_UFUNCTION_WRAP, UFUNCTION_SETTINGS_BARE)
        self.assertNotEqual(LOCKED_UFUNCTION_WRAP, UFUNCTION_APACHE_DAMAGE)
        self.assertNotIn(
            'Category="Skyguard|Environment|Performance"',
            LOCKED_UFUNCTION_WRAP,
        )
        ...
        self.assertTrue("|VFX" in LOCKED_UFUNCTION_WRAP)
        self.assertIsNone(
        ...
        self.assertIsNone(
            re.search(
                r'Category\s*=\s*"Skyguard\|Environment"(?!\|)',
                LOCKED_UFUNCTION_WRAP,
            )
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tvoid DeactivateAllEffectss();\n",
            "\tint32 DeactivateAllEffects = 1;\n",
            "\tFName DeactivateAllEffects = NAME_None;\n",
            "\tvoid DeactivateAllEffects() = "
            + forty
            + ";\n",
            "\tvoid DeactivateAllEffects() = "
            ...
            f"\t{TARGET_WRONG_ENSURE_DEFAULT}\n",
            f"\t{TARGET_WRONG_ACTIVATE_POOLED}\n",
            f"\t{TARGET_WRONG_ALLOCATED_POOL}\n",
            f"\t{TARGET_WRONG_ACTIVATION_COUNT}\n",
            ...
            f"\t{TARGET_WRONG_MATERIAL_ARG}\n",
            f"\t{TARGET_WRONG_VFX_POOL_SIZE}\n",
        )
    ...
    def test_missing_category_or_blueprint_callable_fails_closed(self) -> None:
        ...
        leftover_performance = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
            f"\t{UFUNCTION_ENVIRONMENT_PERFORMANCE}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        performance_specs = attached_ufunction_specifiers(
            spec_section(leftover_performance)
        )
        self.assertIn(
            'Category="Skyguard|Environment|Performance"',
            performance_specs,
    ...
    def test_does_not_claim_sibling_activate_pooled_effect_or_neighbors(
        self,
    ) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(
            has_identifier(leaked, "DeactivateAllEffects")
        )
        self.assertTrue(
            has_identifier(section, "DeactivateAllEffects")
        )
        ...
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, "ResolveSystem"))
        self.assertFalse(has_identifier(section, "ResolveSystem"))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "environment_vfx_deactivate_all_effects"
            "_decl_contract.py"
        ))
        self.assertNotIn("SkyguardEnvironmentVFXPoolComponent.cpp", THIS_SCRIPT)
        ...
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_MISSION09_POOL_CAPACITY_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_ENV_READINESS_VFX_POOL_SIZE_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_ENSURE_DEFAULT_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_GET_SORTIE_HITS_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_PREPARE_VALIDATION_SCRIPT)
        self.assertNotEqual(THIS_SCRIPT, LEFTOVER_BEGIN_TRANSIENT_SCRIPT)
        self.assertNotIn(LEFTOVER_VFX_POOL_FAIL_CLOSED_CPP, THIS_SCRIPT)
    ...
    def test_contract_is_target_method_declaration_only(self) -> None:
        ...
        self.assertNotIn("EnsureDefaultEntries", LOCKED_DECL)
        self.assertNotIn("ActivatePooledEffect", LOCKED_DECL)
        self.assertNotIn("GetAllocatedPoolSize", locked_only)
        self.assertNotIn("GetActivationCount", locked_only)
        self.assertNotIn("PoolCapacity", locked_only)
        self.assertNotIn("VFXPoolSize", locked_only)
        ...
        self.assertNotIn("UMaterialInterface", locked_only)
        self.assertNotIn("GovernedMaterial", locked_only)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ENSURE_DEFAULT)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_ACTIVATE_POOLED)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_POOL_CAPACITY)
        for leftover in leftover_apache_ufunction_tokens():
