...
# or lock the GetDestroyedDispenserCount body in
# the .cpp. origin/main is one line
...
# Category="Skyguard|Mission09|Boss") is expected
# as present. Parse the public class section of
# ASkyguardIronRainBoss only. Stay off leftover
# emergency-finish surfaces. Stay off leftover
# sibling OpenDispenserBay / ReleasePooledEscort /
...
# isolated-test drafts #107–#448, leftover
...
# Python GetDestroyedDispenserCount declaration
# contract on ASkyguardIronRainBoss. Stay off
# leftover sibling OpenDispenserBay /
# ReleasePooledEscort / IssueClimbCommand /
# IssueCrossCommand / OpenUpperEngineExposure /
# GetDestroyedAntennaCount /
# GetDestroyedEngineCount / GetReleasedEscortCount,
...
# leftover isolated-test drafts #107–#448,
...
# .cpp GetDestroyedDispenserCount body / invented
...
CPP_AND_INVENTED = (
    ...
    "return false",
    "return true",
    "ASkyguardIronRainBoss::GetDestroyedDispenserCount",
    "SkyguardIronRainBoss.cpp",
    "Dispensers",
    "bDestroyed",
    "++Count",
...
class IronRainGetDestroyedDispenserCountDeclContractTests(unittest.TestCase):
    def test_iron_rain_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, GET_DESTROYED_DISPENSER_COUNT),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, GET_DESTROYED_DISPENSER_COUNT)
        self.assertIn("GetDestroyedDispenserCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, GET_DESTROYED_DISPENSER_COUNT))

    def test_missing_get_destroyed_dispenser_count_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
            "\tASkyguardIronRainBoss();\n"
            ...
            f"\t{GET_DESTROYED_ANTENNA_COUNT}\n"
            f"\t{GET_DESTROYED_ENGINE_COUNT}\n"
            f"\t{GET_RELEASED_ESCORT_COUNT}\n"
            "\tTObjectPtr<USkyguardBossWeakPointComponent> "
            "DispenserPort;\n"
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, GET_DESTROYED_DISPENSER_COUNT)
        self.assertIn("GetDestroyedDispenserCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, GET_DESTROYED_DISPENSER_COUNT)
        self.assertIn("GetDestroyedDispenserCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_origin_main_ufunction_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("BlueprintPure", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("UFUNCTION", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("Category", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("BlueprintCallable", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, GET_DESTROYED_DISPENSER_COUNT)
        self.assertIn("GetDestroyedDispenserCount", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        missing_const = "\tint32 GetDestroyedDispenserCount();\n"
        missing_parens = "\tint32 GetDestroyedDispenserCount;\n"
        wrong_return_void = "\tvoid GetDestroyedDispenserCount() const;\n"
        wrong_return_bool = "\tbool GetDestroyedDispenserCount() const;\n"
        wrong_return_float = (
            "\tfloat GetDestroyedDispenserCount() const;\n"
        )
        added_arg = (
            "\tint32 GetDestroyedDispenserCount(int32 Extra) const;\n"
        )
        leftover_open = f"\t{OPEN_DISPENSER_BAY}\n"
        ...
        leftover_antenna = f"\t{GET_DESTROYED_ANTENNA_COUNT}\n"
        leftover_engines = f"\t{GET_DESTROYED_ENGINE_COUNT}\n"
        leftover_escort = f"\t{GET_RELEASED_ESCORT_COUNT}\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, GET_DESTROYED_DISPENSER_COUNT)
            self.assertIn("GetDestroyedDispenserCount", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())

    def test_get_destroyed_dispenser_count_declaration_matches_origin_main(
        self,
    ) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertTrue(has_declaration(section, GET_DESTROYED_DISPENSER_COUNT))
        self.assertEqual(
            declaration_count(section, GET_DESTROYED_DISPENSER_COUNT),
            1,
        )
        self.assertTrue(
            GET_DESTROYED_DISPENSER_COUNT.startswith("int32 "),
            GET_DESTROYED_DISPENSER_COUNT,
        )
        self.assertTrue(
            GET_DESTROYED_DISPENSER_COUNT.endswith(";"),
            GET_DESTROYED_DISPENSER_COUNT,
        )
        self.assertIn(
        ...
        self.assertIn(" const", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("INDEX_NONE", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("{", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("}", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("return ", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("OpenDispenserBay", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ReleasePooledEscort", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("IssueClimbCommand", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("IssueCrossCommand", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("OpenUpperEngineExposure", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetDestroyedAntennaCount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetDestroyedEngineCount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetReleasedEscortCount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetChinMuzzleLocation", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetGunnerMount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("FaceWorldLocation", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("DispenserPort", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SetSearchlightTracked", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SetContactIdentified", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn(
            "SetFriendlySeparationMeters",
            GET_DESTROYED_DISPENSER_COUNT,
        )
        self.assertNotIn("SetLightningExposed", GET_DESTROYED_DISPENSER_COUNT)
        for name in leftover_emergency_finish_names():
            self.assertNotIn(name, GET_DESTROYED_DISPENSER_COUNT)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wrap_type = (
            "public:\n"
            "\tint32\n"
            "\tGetDestroyedDispenserCount() const;\n"
            "private:\n"
            "};\n"
        )
        wrap_name = (
            "public:\n"
            "\tint32 GetDestroyedDispenserCount\n"
            "\t() const;\n"
            "private:\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{GET_DESTROYED_DISPENSER_COUNT}\n}}\n"
        self.assertTrue(has_declaration(one_line, GET_DESTROYED_DISPENSER_COUNT))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, GET_DESTROYED_DISPENSER_COUNT),
            section,
    ...
    def test_declaration_accepts_inline_body_without_locking_it(self) -> None:
        ...
        self.assertNotIn("}", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("return ", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("return Count", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("Dispensers", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("bDestroyed", GET_DESTROYED_DISPENSER_COUNT)

    def test_declaration_does_not_invent_index_none(self) -> None:
        locked_only = f"{GET_DESTROYED_DISPENSER_COUNT}\n"
        self.assertNotIn("INDEX_NONE", locked_only)
        self.assertNotIn("INDEX_NONE", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("return INDEX_NONE", locked_only)
        self.assertNotIn("NAME_None", GET_DESTROYED_DISPENSER_COUNT)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_get_destroyed_dispenser_count_cpp_body(
        self,
    ) -> None:
        locked_only = f"{GET_DESTROYED_DISPENSER_COUNT}\n"
        self.assertNotIn("{", GET_DESTROYED_DISPENSER_COUNT)
        ...
        self.assertNotIn("SkyguardIronRainBoss.cpp", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SkyguardIronRainBoss.cpp", locked_only)
        self.assertNotIn("return Count", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("return false", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("return true", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("Dispensers", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("bDestroyed", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("++Count", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_release_pooled_escort_sibling(
        ...
        for token in RELEASE_POOLED_ESCORT_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ReleasePooledEscort", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_get_destroyed_antenna_count_sibling(
        self,
    ) -> None:
        locked_only = f"{GET_DESTROYED_DISPENSER_COUNT}\n"
        for token in GET_DESTROYED_ANTENNA_COUNT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetDestroyedAntennaCount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetDestroyedAntennaCount", locked_only)
        self.assertNotIn(
            "test_iron_rain_get_destroyed_antenna_count_decl_contract.py",
            GET_DESTROYED_DISPENSER_COUNT,
        )

    def test_contract_does_not_relock_get_destroyed_engine_count_sibling(
        self,
    ) -> None:
        locked_only = f"{GET_DESTROYED_DISPENSER_COUNT}\n"
        for token in GET_DESTROYED_ENGINE_COUNT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetDestroyedEngineCount", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_get_released_escort_count_sibling(
        self,
    ) -> None:
        locked_only = f"{GET_DESTROYED_DISPENSER_COUNT}\n"
        for token in GET_RELEASED_ESCORT_COUNT_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetReleasedEscortCount", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_getters(self) -> None:
        ...
        self.assertNotIn("GetDestroyedEngineCount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetReleasedEscortCount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetManeuver", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("MaxReleasesPerBay", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_emergency_finish(self) -> None:
        ...
        for script in leftover_live_copy_boss_scripts():
            self.assertNotIn(script, GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_weak_point_fields(self) -> None:
        ...
        self.assertNotIn("DispenserPort", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("DispenserCenter", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("EnginePodPort", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("FuelControlPort", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_black_kite(self) -> None:
        ...
        for token in LEFTOVER_BLACK_KITE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SetSearchlightTracked", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("IsSearchlightTracked", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ASkyguardBlackKiteBoss", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_radar_ghost(self) -> None:
        ...
        for token in LEFTOVER_RADAR_GHOST_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SetContactIdentified", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("IsContactIdentified", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("OpenOrbitExposure", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_lifeline_hunter(self) -> None:
        ...
        self.assertNotIn("OpenSensorExposure", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ASkyguardLifelineHunterBoss", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_tempest(self) -> None:
        ...
        for token in LEFTOVER_TEMPEST_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SetLightningExposed", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ApplyCorrectiveBankGust", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ASkyguardTempestBoss", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_patrol_ship(self) -> None:
        ...
        self.assertNotIn("ASkyguardPatrolShip", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_mount_getters(self) -> None:
        ...
        for token in MOUNT_GETTERS_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetGunnerMount", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetChinTurret", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetSensorTurret", GET_DESTROYED_DISPENSER_COUNT)

    def test_contract_does_not_relock_chin_muzzle(self) -> None:
        locked_only = f"{GET_DESTROYED_DISPENSER_COUNT}\n"
        for neighbor in GET_CHIN_MUZZLE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("GetChinMuzzleLocation", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_own_ship_systems(self) -> None:
        ...
        for token in LEFTOVER_OWN_SHIP_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ESkyguardApacheSystem", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_input_capture(self) -> None:
        ...
        self.assertNotIn("RecordPlayerEvent", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_gunner(self) -> None:
        ...
        for token in LEFTOVER_GUNNER_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ASkyguardGunner", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("FillAndFinalize", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_radar_node(self) -> None:
        ...
        for token in LEFTOVER_RADAR_NODE_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SkyguardRadarNode", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_leftover_widget_decl_siblings(
        ...
        self.assertNotIn("USkyguardDebriefWidget", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("USkyguardBriefingWidget", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn(
            "test_debrief_widget_travel_next_decl_contract.py",
            GET_DESTROYED_DISPENSER_COUNT,
        )
    ...
    def test_contract_does_not_relock_leftover_skyline_harbor_industrial(
        ...
        for token in LEFTOVER_SKYLINE_NOT_LOCKED:
            ...
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
            self.assertNotIn(token, section)
        self.assertNotIn("HarborIndustrial", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn(
            "ESkyguardMissionSkylineStyle",
            GET_DESTROYED_DISPENSER_COUNT,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        for token in WRONG_HARBOR_HEADERS_NOT_SCANNED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("MinHeightFromOriginCm", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("MaxIntegrity", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SkyguardApacheAircraft.h", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("SkyguardPathfinder", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", GET_DESTROYED_DISPENSER_COUNT)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{GET_DESTROYED_DISPENSER_COUNT}\n"
        self.assertEqual(
            require_declaration(locked_only, GET_DESTROYED_DISPENSER_COUNT),
            GET_DESTROYED_DISPENSER_COUNT,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "iron rain GetDestroyedDispenserCount contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, GET_DESTROYED_DISPENSER_COUNT.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                GET_DESTROYED_DISPENSER_COUNT.lower(),
                "iron rain GetDestroyedDispenserCount contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live copy",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, GET_DESTROYED_DISPENSER_COUNT)

    def test_contract_is_get_destroyed_dispenser_count_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertNotIn("return ", GET_DESTROYED_DISPENSER_COUNT)
        self.assertNotIn("{", GET_DESTROYED_DISPENSER_COUNT)
        self.assertTrue(GET_DESTROYED_DISPENSER_COUNT.startswith("int32 "))
        self.assertTrue(GET_DESTROYED_DISPENSER_COUNT.endswith(";"))
        self.assertIn(" const", GET_DESTROYED_DISPENSER_COUNT)
        self.assertIn(UFUNCTION_BOSS, section)
        self.assertIn("BlueprintPure", UFUNCTION_BOSS)
