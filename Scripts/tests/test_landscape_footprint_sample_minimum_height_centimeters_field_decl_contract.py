...
# MinimumHeightCentimeters FIELD
...
# THIS IS NOT leftover LandscapeHeightSample HeightCentimeters #1187 (different struct/name).
...
# BlueprintReadOnly float MinimumHeightCentimeters = 0.f; one-line /
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# MinimumHeightCentimeters FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float MinimumHeightCentimeters = 0.f; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
...
# THIS IS leftover-safe FSkyguardLandscapeFootprintSampleResult
# MinimumHeightCentimeters FIELD (declaration-only;
# VisibleAnywhere / BlueprintReadOnly
# float MinimumHeightCentimeters = 0.f; one-line /
# split-line UPROPERTY wraps; origin/main has NO Category).
# THIS IS NOT leftover Mission01 environment-readiness-defaults
...
# FSkyguardLandscapeFootprintSampleResult MinimumHeightCentimeters
# FIELD (declaration-only; VisibleAnywhere /
# BlueprintReadOnly float MinimumHeightCentimeters = 0.f;
...
# MinimumHeightCentimeters FIELD (declaration-only). It is
...
TARGET = "float MinimumHeightCentimeters = 0.f;"
TARGET_WRONG = "int32 MinimumHeightCentimeters = 0;"
LOCKED_DECL = TARGET
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    match = re.search(
        r"UPROPERTY\(([^)]*)\)\s*float\s+MinimumHeightCentimeters\b",
        compact,
    )
    if match is None:
        raise AssertionError(
            "UPROPERTY for float MinimumHeightCentimeters is missing from "
...
class LandscapeFootprintSampleMinimumHeightCentimetersFieldDeclContractTests(unittest.TestCase):
    def test_landscape_footprint_sample_result_struct_exists(self) -> None:
        ...
        self.assertIn("MinimumHeightCentimeters", section)
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("MinimumHeightCentimeters", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(
            has_declaration(section, LOCKED_DECL)
    ...
    def test_missing_minimum_height_centimeters_declaration_fails_closed(self) -> None:
        neighbors_only = (
            "\tASkyguardMission09IntegrationDirector();\n"
    ...
    def test_minimum_height_centimeters_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MinimumHeightCentimeters"),
            LOCKED_DECL,
        )
        ...
        self.assertIn("MinimumHeightCentimeters", LOCKED_DECL)
    ...
    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        ...
        self.assertIn("MinimumHeightCentimeters", LOCKED_DECL)
        self.assertTrue(LOCKED_DECL.startswith("float MinimumHeightCentimeters"))
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        ...
        self.assertIn("MinimumHeightCentimeters", locked_only)
    ...
    def test_sibling_director_fields_do_not_satisfy_minimum_height_centimeters(
        self,
    ) -> None:
        for leftover in (
            ROOT_FIELD,
