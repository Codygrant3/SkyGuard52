# THIS IS leftover-safe FSkyguardProductionAudioRouting MasterSubmix.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite)
# then BARE `TSoftObjectPtr<USoundSubmixBase> MasterSubmix;`
# (no initializer).
...
# Sibling decls stay unclaimed: CockpitSubmix / ExteriorSubmix /
# WeaponsSubmix / ExplosionsSubmix / RadioSubmix /
# CockpitSoundMix / CockpitExteriorAttenuation /
# CockpitLowPassHz.
# Use identifier bounds so sibling CockpitSubmix / RadioSubmix /
# WeaponsSubmix do not false-positive on the Submix substring.
# Analog leftover production-audio-routing-defaults then
# isolated MasterSubmix field decl.
...
TARGET = "TSoftObjectPtr<USoundSubmixBase> MasterSubmix;"
TARGET_WRONG_EQ_NONE = "TSoftObjectPtr<USoundSubmixBase> MasterSubmix = nullptr;"
TARGET_WRONG_FALSE = "TSoftObjectPtr<USoundSubmixBase> MasterSubmix = false;"
TARGET_WRONG_TRUE = "TSoftObjectPtr<USoundSubmixBase> MasterSubmix = true;"
TARGET_WRONG_ZERO = "TSoftObjectPtr<USoundSubmixBase> MasterSubmix = 0.f;"
TARGET_WRONG_FLOAT = "float MasterSubmix;"
TARGET_WRONG_FNAME = "FName MasterSubmix;"
TARGET_WRONG_BOOL = "bool MasterSubmix;"
TARGET_WRONG_INT = "int32 MasterSubmix;"
TARGET_WRONG_FTEXT = "FText MasterSubmix;"
TARGET_WRONG_PTR = "USoundSubmixBase* MasterSubmix;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> MasterSubmix;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundMix> MasterSubmix;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> MasterSubmix;"
TARGET_WRONG_FSTRING = "FString MasterSubmix;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+MasterSubmix\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<\s*USoundSubmixBase\s*>\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundMix\s*>\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<\s*USoundBase\s*>\s+MasterSubmix\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TSoftObjectPtr\s*<\s*USoundSubmixBase\s*>\s+MasterSubmix\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TSoftObjectPtr<USoundSubmixBase> MasterSubmix is missing from "
...
class ProductionAudioRoutingMasterSubmixFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("MasterSubmix = nullptr", compact_origin)
        self.assertNotIn("MasterSubmix = false", compact_origin)
        self.assertNotIn("MasterSubmix = true", compact_origin)
        self.assertNotIn("MasterSubmix = 0.f", compact_origin)
        self.assertNotIn("MasterSubmix = 160.f", compact_origin)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTSoftObjectPtr<USoundSubmixBase> MasterSubmixes;\n",
            "\tint32 MasterSubmix = 1;\n",
            "\tbool MasterSubmix;\n",
            "\tfloat MasterSubmix = " + forty + ";\n",
            "\tfloat MasterSubmix = " + eighty + ";\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing MasterSubmix field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "master_submix_field_decl_contract.py"
        ))
