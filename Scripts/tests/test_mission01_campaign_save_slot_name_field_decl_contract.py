...
# INDEX_NONE or lock CampaignSaveSlotName construction
# in the .cpp. This is a FIELD contract on the
# Mission 1 integration director, not leftover
# campaign-subsystem save-slot methods. Keep
# CampaignSaveSlotName distinct from sibling
# director fields Briefing (#568) / AudioDirector
# (#572) / Root (#574) / RadioChatter (#576) /
# SortiePresentation (#575) / CampaignDefinition
# (#577) / MissionDefinition (#578) / Readiness
# (#579) / bAutoInitialize /
# bAllowBoundedActorSpawning (in flight) /
# bAutoLaunchAfterBriefing (in flight) /
# CampaignSaveUserIndex. Do not lock leftover
# campaign-subsystem SaveCampaignToSlot /
# LoadCampaignFromSlot / IsValidCampaignSlotName.
...
# save-slot method decl contracts.
# origin/main is a one-line field
# (`FString CampaignSaveSlotName =
# TEXT("Skyguard52Campaign");`); accept that form
# and other one-line / split-line wraps including
# TEXT() form. Nearby origin/main
# UPROPERTY(EditAnywhere, BlueprintReadWrite,
# Category="Skyguard|Mission01|Campaign") is
...
# leftover theater-kit #59, leftover #107–#579, plus
# SkyguardMission01IntegrationDirector production
# files. This lane only adds an isolated Python
# CampaignSaveSlotName field declaration contract on
...
# contracts, sibling Briefing / AudioDirector /
# Root / RadioChatter / SortiePresentation /
# CampaignDefinition / MissionDefinition /
# Readiness / auto-initialize field contracts stay
# sibling-only. Leftover save-slot method decl
# contracts stay leftover-only.
LOCKED_SCRIPTS = (
...
SAVE_CAMPAIGN_TO_SLOT = "SaveCampaignToSlot"
LOAD_CAMPAIGN_FROM_SLOT = "LoadCampaignFromSlot"
IS_VALID_CAMPAIGN_SLOT_NAME = "IsValidCampaignSlotName"
LEFTOVER_SAVE_SLOT_METHODS_NOT_LOCKED = (
    SAVE_CAMPAIGN_TO_SLOT,
    LOAD_CAMPAIGN_FROM_SLOT,
    IS_VALID_CAMPAIGN_SLOT_NAME,
...
class Mission01CampaignSaveSlotNameFieldDeclContractTests(unittest.TestCase):
    def test_mission01_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, CAMPAIGN_SAVE_SLOT_NAME_FIELD),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertFalse(has_declaration(section, CAMPAIGN_SAVE_SLOT_NAME_FIELD))
    ...
    def test_missing_campaign_save_slot_name_declaration_fails_closed(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))

    def test_uproperty_macro_alone_does_not_satisfy(self) -> None:
        macro_only = f"\t{UPROPERTY_MISSION01}\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        section = public_section(origin_main_header())
        self.assertIn(UPROPERTY_MISSION01, section)
        ...
        self.assertNotIn("UPROPERTY", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("EditAnywhere", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("BlueprintReadWrite", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("Category", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("BlueprintPure", UPROPERTY_MISSION01)
        self.assertNotIn("BlueprintCallable", UPROPERTY_MISSION01)
        self.assertIn("Skyguard|Mission01", UPROPERTY_MISSION01)
        self.assertIn("Skyguard|Mission01|Campaign", UPROPERTY_MISSION01)
        self.assertIn("|Campaign", UPROPERTY_MISSION01)
        self.assertIn("EditAnywhere", UPROPERTY_MISSION01)
        self.assertIn("BlueprintReadWrite", UPROPERTY_MISSION01)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_MISSION01)
        self.assertNotIn("BlueprintReadOnly", UPROPERTY_MISSION01)
        self.assertNotIn("Mission02", UPROPERTY_MISSION01)
        self.assertNotIn("Boss", UPROPERTY_MISSION01)
        self.assertNotIn("Destruction", UPROPERTY_MISSION01)
        self.assertNotIn("Apache", UPROPERTY_MISSION01)
        self.assertNotIn("Mission07", UPROPERTY_MISSION01)
        self.assertNotIn("Mission10", UPROPERTY_MISSION01)
        self.assertNotIn("Encounter", UPROPERTY_MISSION01)
        self.assertNotIn("Safety", UPROPERTY_MISSION01)
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, UPROPERTY_MISSION01)
            self.assertNotIn(invented, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_wrong_signature_does_not_satisfy(self) -> None:
        unassigned = "\tFString CampaignSaveSlotName;\n"
        assigned_empty = (
            '\tFString CampaignSaveSlotName = TEXT("");\n'
        )
        assigned_other = (
            '\tFString CampaignSaveSlotName = TEXT("OtherSlot");\n'
        )
        as_name = (
            '\tFName CampaignSaveSlotName = '
            'TEXT("Skyguard52Campaign");\n'
        )
        as_text = (
            '\tFText CampaignSaveSlotName = '
            'TEXT("Skyguard52Campaign");\n'
        )
        as_int = "\tint32 CampaignSaveSlotName = 0;\n"
        ...
        for region in (
            ...
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
            self.assertIn("CampaignSaveSlotName", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_campaign_save_slot_name_declaration_matches_origin_main(self) -> None:
        ...
        self.assertTrue(has_declaration(section, CAMPAIGN_SAVE_SLOT_NAME_FIELD))
        self.assertEqual(
            declaration_count(section, CAMPAIGN_SAVE_SLOT_NAME_FIELD),
            1,
        )
        self.assertTrue(
            CAMPAIGN_SAVE_SLOT_NAME_FIELD.startswith("FString "),
            CAMPAIGN_SAVE_SLOT_NAME_FIELD,
        )
        self.assertTrue(CAMPAIGN_SAVE_SLOT_NAME_FIELD.endswith(";"), CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn('TEXT("Skyguard52Campaign")', CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("TArray<", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("TObjectPtr", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("TSoftObjectPtr", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("INDEX_NONE", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("UFUNCTION", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("{", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("}", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("return ", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("Root", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("Briefing", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("AudioDirector", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("RadioChatter", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("SortiePresentation", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("CampaignDefinition", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("MissionDefinition", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("Readiness", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("bAutoInitialize", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("bAllowBoundedActorSpawning", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("bAutoLaunchAfterBriefing", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("CampaignSaveUserIndex", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("SaveCampaignToSlot", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("LoadCampaignFromSlot", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("IsValidCampaignSlotName", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("PathfinderSpawnLocation", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetAircraft", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("ConfigureFromMission", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("AdvanceBriefing", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("SetAssetsReady", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("AcknowledgeAndLaunch", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("CanLaunch", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetElapsedSeconds", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetBriefingState", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetMinimumWarmupSeconds", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetBriefingText", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetRadioChatter", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetPresentation", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("GetMissionTitle", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("AcknowledgeBriefing", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("LaunchSortie", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("HullCollider", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("OpticalTracker", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("WeaponServo", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("CountermeasurePod", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("MinHeightFromOriginCm", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("RadarNode", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("ESkyguardMission02WaveState", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("HarborIndustrial", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("MaxIntegrity", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("CurrentIntegrity", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        for name in leftover_live_copy_method_names():
            self.assertNotIn(name, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        wrap_uproperty = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01}\n"
            f"\t{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
            "};\n"
        )
        wrap_uproperty_one_line = (
            "public:\n"
            f"\t{UPROPERTY_MISSION01} {CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
            "};\n"
        )
        ...
        one_line = f"{{\npublic:\n\t{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n}}\n"
        self.assertTrue(has_declaration(one_line, CAMPAIGN_SAVE_SLOT_NAME_FIELD))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, CAMPAIGN_SAVE_SLOT_NAME_FIELD),
            section,
    ...
    def test_unassigned_campaign_save_slot_name_does_not_satisfy(self) -> None:
        unassigned = "\tFString CampaignSaveSlotName;\n"
        with self.assertRaises(AssertionError) as raised:
            require_declaration(unassigned, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(has_declaration(unassigned, CAMPAIGN_SAVE_SLOT_NAME_FIELD))

    def test_wrong_default_campaign_save_slot_name_does_not_satisfy(self) -> None:
        assigned_other = (
            '\tFString CampaignSaveSlotName = TEXT("OtherSlot");\n'
        )
        with self.assertRaises(AssertionError) as raised:
            require_declaration(assigned_other, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertFalse(
            has_declaration(assigned_other, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        )
    ...
    def test_sibling_public_fields_do_not_satisfy(self) -> None:
        for region in (
            ...
            self.assertIn("CampaignSaveSlotName", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, CAMPAIGN_SAVE_SLOT_NAME_FIELD))

    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        locked_only = f"{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
        for invented in INVENTED_UPROPERTY:
            self.assertNotIn(invented, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
    ...
    def test_declaration_does_not_invent_ufunction(self) -> None:
        ...
        self.assertFalse(
            CAMPAIGN_SAVE_SLOT_NAME_FIELD.startswith("UFUNCTION"),
            CAMPAIGN_SAVE_SLOT_NAME_FIELD,
        )
        section = public_section(origin_main_header())
        self.assertTrue(
    ...
    def test_declaration_does_not_invent_index_none(self) -> None:
        ...
        self.assertNotIn("NAME_None", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        section = public_section(origin_main_header())
        self.assertNotIn("INDEX_NONE", section)
        self.assertNotIn("return INDEX_NONE", section)
        self.assertNotIn("NAME_None", section)

    def test_contract_does_not_lock_campaign_save_slot_name_cpp_body(self) -> None:
        locked_only = f"{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
        self.assertNotIn("{", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("}", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("return ", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        ...
        self.assertNotIn("return false", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("return true", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("CreateDefaultSubobject", CAMPAIGN_SAVE_SLOT_NAME_FIELD)

    def test_contract_does_not_relock_sibling_director_fields(self) -> None:
        locked_only = f"{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
        for token in SIBLING_DIRECTOR_FIELDS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn("CampaignSaveSlotName", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn('TEXT("Skyguard52Campaign")', CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertTrue(CAMPAIGN_SAVE_SLOT_NAME_FIELD.startswith("FString "))
    ...
    def test_contract_does_not_relock_leftover_get_aircraft(self) -> None:
        ...
        self.assertNotIn(GET_AIRCRAFT, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn(
            "Scripts/tests/test_mission01_get_aircraft_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_scan_wrong_harbor_headers(self) -> None:
        ...
        self.assertNotIn("MaxIntegrity", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("CurrentIntegrity", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("SkyguardApacheAircraft.h", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
    ...
    def test_contract_does_not_scan_apache_public_section_for_harbor(
        ...
        self.assertNotIn(apache_header, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertNotIn("ASkyguardApacheAircraft", locked_only)
        self.assertNotIn("ASkyguardApacheAircraft", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
    ...
    def test_contract_does_not_relock_neighbor_helpers(self) -> None:
        locked_only = f"{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
        self.assertEqual(
            require_declaration(locked_only, CAMPAIGN_SAVE_SLOT_NAME_FIELD),
            CAMPAIGN_SAVE_SLOT_NAME_FIELD,
        )
        for neighbor in unlocked_neighbors():
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
    ...
    def test_contract_parses_public_section_not_enum_private_or_cpp(
        ...
        self.assertEqual(declaration_count(section, CAMPAIGN_SAVE_SLOT_NAME_FIELD), 1)
        self.assertNotIn(
            "SkyguardMission01IntegrationDirector.cpp",
            section,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission01 CampaignSaveSlotName field contract contains "
    ...
    def test_contract_does_not_require_retired_live_mount(self) -> None:
        locked_only = f"{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(banned, CAMPAIGN_SAVE_SLOT_NAME_FIELD.lower())
    ...
    def test_declaration_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                CAMPAIGN_SAVE_SLOT_NAME_FIELD.lower(),
                "mission01 CampaignSaveSlotName contains "
                f"{banned}; declaration is Apache CPG 30 mm / Hydra / "
                "Hellfire, not leftover live cop" + "y",
            )
            self.assertNotIn(banned, locked_only.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn(dirty_fwd, CAMPAIGN_SAVE_SLOT_NAME_FIELD)

    def test_contract_is_campaign_save_slot_name_field_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
        ...
        self.assertIn("CampaignSaveSlotName", CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn('TEXT("Skyguard52Campaign")', CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertTrue(CAMPAIGN_SAVE_SLOT_NAME_FIELD.endswith(";"))
        self.assertIn(UPROPERTY_MISSION01, section)
    ...
    def test_sibling_director_fields_do_not_satisfy_campaign_save_slot_name(
        ...
        for leftover in (
            ...
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(has_declaration(region, CAMPAIGN_SAVE_SLOT_NAME_FIELD))
            self.assertNotEqual(CAMPAIGN_SAVE_SLOT_NAME_FIELD, leftover)
        self.assertIn(
        ...
        self.assertIn(
            "Scripts/tests/test_save_campaign_to_slot"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_load_campaign_from_slot"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
        self.assertIn(
            "Scripts/tests/test_is_valid_campaign_slot_name"
            "_decl_contract.py",
            LOCKED_SCRIPTS,
        )
    ...
    def test_contract_does_not_relock_leftover_save_slot_methods(
        self,
    ) -> None:
        locked_only = f"{CAMPAIGN_SAVE_SLOT_NAME_FIELD}\n"
        for token in LEFTOVER_SAVE_SLOT_METHODS_NOT_LOCKED:
            self.assertNotIn(token, locked_only)
            self.assertNotIn(token, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
        self.assertIn(
            "Scripts/tests/test_save_campaign_to_slot"
    ...
    def test_leftover_save_slot_methods_do_not_satisfy(self) -> None:
        leftover_save = (
            "bool SaveCampaignToSlot("
        ...
        for leftover in (leftover_save, leftover_load, leftover_valid):
            region = f"\t{leftover}\n"
            with self.assertRaises(AssertionError) as raised:
                require_declaration(region, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
            self.assertIn("CampaignSaveSlotName", str(raised.exception))
            self.assertIn("missing", str(raised.exception).lower())
            self.assertFalse(
                has_declaration(region, CAMPAIGN_SAVE_SLOT_NAME_FIELD)
            )
