# THIS IS leftover-safe FSkyguardMissionScoreRules PerfectAccuracyBonus.
...
# if type is not int32, if initializer is not `= 2500`,
...
# THIS IS NOT leftover #1332-#1355.
...
# retarget: type is int32, initializer is `= 2500`,
# no Category, ClampMin = "0" only.
...
TARGET = "int32 PerfectAccuracyBonus = 2500;"
TARGET_WRONG_BARE = "int32 PerfectAccuracyBonus;"
TARGET_WRONG_FALSE = "int32 PerfectAccuracyBonus = false;"
TARGET_WRONG_TRUE = "int32 PerfectAccuracyBonus = true;"
TARGET_WRONG_ZERO = "int32 PerfectAccuracyBonus = 0;"
TARGET_WRONG_COMPLETION = "int32 PerfectAccuracyBonus = 5000;"
TARGET_WRONG_NODAMAGE = "int32 PerfectAccuracyBonus = 1500;"
TARGET_WRONG_FLOAT = "float PerfectAccuracyBonus = 2500;"
TARGET_WRONG_FNAME = "FName PerfectAccuracyBonus;"
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 PerfectAccuracyBonus = 2500;`.
    # Do not accept bare `int32 PerfectAccuracyBonus;` / `= 0` /
    # `= 5000` / `= 1500` / leftover CompletionScore when
    # origin/main has `= 2500`. Do not accept sibling
    # CompletionScore / NoDamageBonus / BronzeThreshold /
    # SilverThreshold / GoldThreshold fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+PerfectAccuracyBonus\s*=\s*2500\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"FName\s+PerfectAccuracyBonus\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+PerfectAccuracyBonus\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+PerfectAccuracyBonus\s*;",
        compact,
    ):
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+PerfectAccuracyBonus\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 PerfectAccuracyBonus is missing from "
...
class MissionScoreRulesPerfectAccuracyBonusFieldDeclContractTests(unittest.TestCase):
    def test_mission_score_rules_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "PerfectAccuracyBonus"), section)
    ...
    def test_neighbor_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "PerfectAccuracyBonus"), section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(MISSION_DEF_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("PerfectAccuracyBonus", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_perfect_accuracy_bonus_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 PerfectAccuracyBonus = 2500;", compact_origin)
        self.assertNotIn("PerfectAccuracyBonus = NAME_None", compact_origin)
        self.assertNotIn("PerfectAccuracyBonus = false", compact_origin)
        self.assertNotIn("PerfectAccuracyBonus = true", compact_origin)
        self.assertNotIn("PerfectAccuracyBonus = 160.f", compact_origin)
        self.assertNotIn("PerfectAccuracyBonus = 0;", compact_origin)
        self.assertNotIn("PerfectAccuracyBonus = 5000", compact_origin)

    def test_perfect_accuracy_bonus_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 PerfectAccuracyBonus"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("= 2500", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 PerfectAccuracyBonuses = 2500;\n",
            "\tfloat PerfectAccuracyBonus = 2500;\n",
            "\tbool PerfectAccuracyBonus;\n",
            "\tint32 PerfectAccuracyBonus = " + forty + ";\n",
            "\tint32 PerfectAccuracyBonus = " + eighty + ";\n",
        )
    ...
    def test_category_or_wrong_specifiers_fail_closed(self) -> None:
        ...
        self.assertIn('ClampMin = "1"', wrong_min)
        self.assertNotIn('ClampMin = "0"', wrong_min)
        self.assertIn('ClampMin = "0"', origin)
        ...
        self.assertNotIn('ClampMin = "0"', nospace)
        self.assertIn('ClampMin = "0"', origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-score-rules PerfectAccuracyBonus field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "perfect_accuracy_bonus_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
