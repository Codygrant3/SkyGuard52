# THIS IS leftover-safe FSkyguardMissionDebrief::bNewBestScore.
# origin/main form: split-line
# UPROPERTY(VisibleAnywhere, BlueprintReadOnly)
# then `bool bNewBestScore = false;` with no Category=.
...
TARGET = "bool bNewBestScore = false;"
TARGET_WRONG_BARE = "bool bNewBestScore;"
TARGET_WRONG_TRUE = "bool bNewBestScore = true;"
TARGET_WRONG_ZERO = "bool bNewBestScore = 0;"
TARGET_WRONG_FLOAT = "bool bNewBestScore = 0.f;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # origin/main has `bool bNewBestScore = false;`.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(r"bool\s+bNewBestScore\s*=\s*true\b", compact):
        return False
    if re.search(r"bool\s+bNewBestScore\s*=\s*0(?:\.f)?\b", compact):
        return False
    if re.search(r"bool\s+bNewBestScore\s*=\s*false\b", compact) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*bool\s+bNewBestScore\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for bool bNewBestScore is missing from "
...
class MissionDebriefNewBestScoreFieldDeclContractTests(unittest.TestCase):
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertIn("bNewBestScore", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_new_best_score_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("bNewBestScore = false", collapsed(origin))
        self.assertNotIn("bNewBestScore = true", collapsed(origin))
        self.assertNotIn("bNewBestScore = 0", collapsed(origin))

    def test_new_best_score_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("bool bNewBestScore"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        wrongs = (
            ...
            "\tint32 bNewBestScore = false;\n",
            "\tbool bNewBestScores = false;\n",
            "\tbool NewBestScore = false;\n",
            "\tFText bNewBestScore = false;\n",
            "\tbool bNewBestScore{};\n",
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-debrief bNewBestScore field contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "test_mission_debrief_new_best_score_field_decl_contract.py"
        ))
