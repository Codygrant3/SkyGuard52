# THIS IS leftover-safe FSkyguardCpgHudSnapshot EufdLine.
...
# `FString EufdLine;` with NO in-struct initializer.
# Fail-closed if EufdLine is missing or renamed, if the
...
# EufdLine is NOT leftover sibling WeaponLine / RangeLine /
# ThreatLine. HUD sibling `float RangeMeters = -1.f` stays
# unclaimed.
...
# ThreatLine / LockLine / SightLine / StationStatus /
...
# ThreatLine isolated field decls. Keep those files in
# LOCKED_SCRIPTS.
...
# retarget: type is FString, identifier is EufdLine,
...
TARGET = "FString EufdLine;"
TARGET_WRONG_EQ_EMPTY = 'FString EufdLine = TEXT("");'
TARGET_WRONG_EQ_NONE = "FString EufdLine = NAME_None;"
TARGET_WRONG_FALSE = "FString EufdLine = false;"
TARGET_WRONG_TRUE = "FString EufdLine = true;"
TARGET_WRONG_ZERO = "FString EufdLine = 0.f;"
TARGET_WRONG_FLOAT = "float EufdLine;"
TARGET_WRONG_HEALTH = "float EufdLine = 160.f;"
TARGET_WRONG_BOOL = "bool EufdLine;"
TARGET_WRONG_INT = "int32 EufdLine;"
TARGET_WRONG_FNAME = "FName EufdLine;"
TARGET_WRONG_TCHAR = 'const TCHAR* EufdLine = TEXT("");'
...
EUFD_LINE_BARE_RE = re.compile(r"FString\s+EufdLine\s*;")
EUFD_LINE_INIT_RE = re.compile(r"FString\s+EufdLine\s*=")
INVENTED_FIELD_META = (
    "INDEX_NONE",
    "NAME_None",
    "return INDEX_NONE",
...
def eufd_line_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*FString\s+EufdLine\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FString EufdLine;`.
    ...
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if EUFD_LINE_INIT_RE.search(compact):
        return False
    if re.search(r"float\s+EufdLine\b", compact):
        return False
    if re.search(r"bool\s+EufdLine\b", compact):
        return False
    if re.search(r"int32\s+EufdLine\b", compact):
        return False
    if re.search(r"FName\s+EufdLine\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*EufdLine\b", compact):
        return False
    if re.search(r"float\s+RangeMeters\b", compact) and (
        EUFD_LINE_BARE_RE.search(compact) is None
    ):
        return False
    if EUFD_LINE_BARE_RE.search(compact) is None:
        return False
    if eufd_line_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FString EufdLine\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FString EufdLine is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*FString EufdLine\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on FString EufdLine is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "EufdLine"):
        raise AssertionError(
            "UPROPERTY clone landed on EufdLine; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "EufdLine"):
            raise AssertionError(
                f"{token} clone landed on EufdLine; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotEufdLineFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "EufdLine"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "FString EufdLine;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("FString EufdLine"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertNotIn("=", LOCKED_DECL)
        self.assertIn("EufdLine", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("EufdLine", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "EufdLine"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_eufd_line_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_weapon_line_does_not_satisfy_eufd_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_WEAPON}\n"
    ...
    def test_range_line_does_not_satisfy_eufd_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_threat_line_does_not_satisfy_eufd_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_leftover_range_meters_does_not_satisfy_eufd_line(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertIn("FString EufdLine;", compact_origin)
        self.assertNotIn("EufdLine = TEXT", compact_origin)
        self.assertNotIn("EufdLine = NAME_None", compact_origin)
        self.assertNotIn("EufdLine = false", compact_origin)
        self.assertNotIn("EufdLine = true", compact_origin)
        self.assertNotIn("EufdLine = 0.f", compact_origin)
        self.assertNotIn("EufdLine = 160.f", compact_origin)

    def test_eufd_line_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FString EufdLine"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 EufdLine;\n",
            "\tbool EufdLine;\n",
            "\tfloat EufdLine = " + forty + ";\n",
            "\tfloat EufdLine = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    eufd_line_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(eufd_line_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot EufdLine field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "eufd_line_field_decl_contract.py"
        ))
