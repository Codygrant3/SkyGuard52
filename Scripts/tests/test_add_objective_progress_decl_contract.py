...
# values, or lock the AddObjectiveProgress body in the .cpp.
# origin/main is the default-argument form:
# `bool AddObjectiveProgress(FName ObjectiveId, int32 Amount = 1);`
# Accept that form and split-line origin/main forms.
ADD_OBJECTIVE_PROGRESS_DECL = (
    "bool AddObjectiveProgress(FName ObjectiveId, int32 Amount = 1);"
)
# Leftover #56–#64 plus CampaignSubsystem production files.
# This lane only adds an isolated Python AddObjectiveProgress
# declaration contract. Stay off FailObjective (in-flight
# sibling), leftover objective-runtime fail-closed / leftover
...
# Isolated-test drafts stay off this lane. FailObjective,
...
# FailObjective is an in-flight sibling.
...
OBJECTIVE_RUNTIME_LEFTOVERS_NOT_LOCKED = (
...
# .cpp AddObjectiveProgress body / invented return values
# stay unlocked. Do not invent INDEX_NONE or lock the body.
CPP_AND_INVENTED = (
    "INDEX_NONE",
    "NAME_None",
    "return nullptr",
    ...
    "return 0",
    "return -1",
    "ObjectiveRuntime->AddProgress",
    "USkyguardCampaignSubsystem::AddObjectiveProgress",
    "SkyguardCampaignSubsystem.cpp",
...
class AddObjectiveProgressDeclContractTests(unittest.TestCase):
    def test_campaign_subsystem_class_exists(self) -> None:
        ...
        self.assertTrue(
            has_declaration(section, ADD_OBJECTIVE_PROGRESS_DECL),
            section,
        )
    ...
    def test_private_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertIn("AddObjectiveProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
        self.assertNotIn(ADD_OBJECTIVE_PROGRESS_DECL, section)

    def test_missing_add_objective_progress_declaration_fails_closed(
        self,
    ) -> None:
        neighbors_only = (
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(neighbors_only, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertIn("AddObjectiveProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        self.assertIn(CLASS_NAME, str(raised.exception))
    ...
    def test_ufunction_macro_alone_does_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(macro_only, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertIn("AddObjectiveProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        ...
        with self.assertRaises(AssertionError) as raised:
            require_declaration(other_helpers, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertIn("AddObjectiveProgress", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_add_objective_progress_declaration_matches_origin_main(
        self,
    ) -> None:
        ...
        self.assertTrue(has_declaration(section, ADD_OBJECTIVE_PROGRESS_DECL))
        self.assertEqual(
            declaration_count(section, ADD_OBJECTIVE_PROGRESS_DECL),
            1,
        )
        self.assertTrue(
            ADD_OBJECTIVE_PROGRESS_DECL.endswith(";"),
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
        self.assertTrue(
            ADD_OBJECTIVE_PROGRESS_DECL.startswith("bool "),
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
        self.assertIn(
            "AddObjectiveProgress(FName ObjectiveId, int32 Amount = 1)",
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
        self.assertIn("Amount = 1", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("INDEX_NONE", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("return ", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("const", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("{", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("}", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        ...
        one_line = f"{{\npublic:\n\t{ADD_OBJECTIVE_PROGRESS_DECL}\n}}\n"
        self.assertTrue(has_declaration(one_line, ADD_OBJECTIVE_PROGRESS_DECL))
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADD_OBJECTIVE_PROGRESS_DECL),
            section,
    ...
    def test_declaration_accepts_default_argument_form(self) -> None:
        ...
        wrap_amount = (
            ...
            "{\n"
            "public:\n"
            "\tbool AddObjectiveProgress(FName ObjectiveId,\n"
            "\t\tint32 Amount = 1);\n"
            "};\n"
        )
        for header in (default_header, split_default, wrap_amount):
            section = public_section(header)
            self.assertTrue(
                has_declaration(section, ADD_OBJECTIVE_PROGRESS_DECL),
                section,
        ...
        self.assertIn("Amount = 1", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertTrue(ADD_OBJECTIVE_PROGRESS_DECL.endswith(";"))
        no_default = "bool AddObjectiveProgress(FName ObjectiveId, int32 Amount);"
        self.assertNotEqual(ADD_OBJECTIVE_PROGRESS_DECL, no_default)
        section = public_section(origin_main_header())
        self.assertTrue(
            has_declaration(section, ADD_OBJECTIVE_PROGRESS_DECL),
            section,
        )
        self.assertIn("Amount = 1", section)

    def test_declaration_does_not_lock_cpp_body(self) -> None:
        self.assertNotIn("{", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("}", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("return ", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn(
        ...
        self.assertNotIn("const int32 Amount", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn(
            "SkyguardCampaignSubsystem.cpp",
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
        semicolon_only = f"{ADD_OBJECTIVE_PROGRESS_DECL}\n"
        self.assertEqual(
            require_declaration(semicolon_only, ADD_OBJECTIVE_PROGRESS_DECL),
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
    ...
    def test_declaration_does_not_invent_index_none_or_return_values(
        ...
        self.assertNotIn("return ", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("INDEX_NONE", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("NAME_None", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("{", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("}", ADD_OBJECTIVE_PROGRESS_DECL)
        ...
        self.assertNotIn("return false", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("return true", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("= INDEX_NONE", section)
        for token in CPP_AND_INVENTED:
            self.assertNotIn(token, ADD_OBJECTIVE_PROGRESS_DECL)

    def test_contract_does_not_relock_configure_campaign(self) -> None:
        locked_only = f"{ADD_OBJECTIVE_PROGRESS_DECL}\n"
        self.assertEqual(
            require_declaration(locked_only, ADD_OBJECTIVE_PROGRESS_DECL),
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
        for neighbor in CONFIGURE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("ConfigureCampaign", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_contract_does_not_relock_can_start_mission(self) -> None:
        ...
        for neighbor in CAN_START_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("CanStartMission", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_contract_does_not_relock_start_mission(self) -> None:
        ...
        for neighbor in START_MISSION_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("StartMission", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("StartMission", locked_only)

    def test_contract_does_not_relock_fail_objective(self) -> None:
        locked_only = f"{ADD_OBJECTIVE_PROGRESS_DECL}\n"
        for neighbor in FAIL_OBJECTIVE_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("FailObjective", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_contract_does_not_relock_survive_helpers(self) -> None:
        ...
        for neighbor in SURVIVE_NOT_LOCKED:
            ...
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn(
            "CompleteSurviveObjectiveIfIntact",
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
    ...
    def test_contract_does_not_relock_get_objective_runtime(self) -> None:
        ...
        for neighbor in GET_OBJECTIVE_RUNTIME_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("GetObjectiveRuntime", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_contract_does_not_relock_unlock_or_slot_helpers(self) -> None:
        ...
        for neighbor in UNLOCK_AND_SLOT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("IsMissionUnlocked", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_score_or_medal_tier(self) -> None:
        ...
        for neighbor in SCORE_AND_TIER_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("CalculateMissionScore", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("CalculateMedalTier", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn(
    ...
    def test_contract_does_not_relock_debrief_or_travel(self) -> None:
        ...
        for neighbor in DEBRIEF_TRAVEL_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("AcknowledgeDebrief", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("CanTravelToNextMission", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn(
            "GetNextMissionMapPackageName",
            ADD_OBJECTIVE_PROGRESS_DECL,
        )
        self.assertNotIn("TravelToNextMission", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_contract_does_not_relock_fill_result_combat_stats(self) -> None:
        ...
        for neighbor in FILL_COMBAT_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("FillResultCombatStats", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("ASkyguardGunner", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_contract_does_not_relock_get_route_or_active_mission(self) -> None:
        ...
        for neighbor in RUNTIME_NEIGHBORS_NOT_LOCKED:
            self.assertNotIn(neighbor, locked_only)
            self.assertNotIn(neighbor, ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("GetActiveMission", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("GetRouteRuntime", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("GetActiveMission", locked_only)
        self.assertNotIn("GetRouteRuntime", locked_only)

    def test_contract_does_not_relock_leftover_objective_runtime_or_defaults(
        self,
    ) -> None:
        locked_only = f"{ADD_OBJECTIVE_PROGRESS_DECL}\n"
        for token in OBJECTIVE_RUNTIME_LEFTOVERS_NOT_LOCKED:
    ...
    def test_contract_parses_public_section_not_private_or_cpp(self) -> None:
        ...
        self.assertNotIn("ObjectiveRuntime->AddProgress", section)
    ...
    def test_contract_does_not_read_cpp_or_invented_bodies(self) -> None:
        ...
        self.assertNotIn("return false", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("return true", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("return INDEX_NONE", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("INDEX_NONE", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("{", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("}", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("const FName ObjectiveId", ADD_OBJECTIVE_PROGRESS_DECL)
        self.assertNotIn("const int32 Amount", ADD_OBJECTIVE_PROGRESS_DECL)
    ...
    def test_contract_does_not_require_rifle_or_igla(self) -> None:
        ...
        self.assertNotEqual(ADD_OBJECTIVE_PROGRESS_DECL, "Rifle")
        self.assertNotEqual(ADD_OBJECTIVE_PROGRESS_DECL, "Igla")
    ...
    def test_declaration_bans_igla_yak_rifle(self) -> None:
        ...
        for banned in BANNED:
            self.assertNotIn(
                banned,
                lowered,
                f"AddObjectiveProgress contains {banned}; "
                "declaration is Apache CPG 30 mm / Hydra / Hellfire, "
                "not leftover live copy",
            )
            self.assertNotIn(banned, ADD_OBJECTIVE_PROGRESS_DECL.lower())
    ...
    def test_contract_does_not_read_dirty_workspace_header(self) -> None:
        ...
        self.assertNotIn("D:/Skyguard52", ADD_OBJECTIVE_PROGRESS_DECL)

    def test_contract_is_add_objective_progress_declaration_only(self) -> None:
        header = origin_main_header()
        section = public_section(header)
        self.assertIsNotNone(CLASS_RE.search(header), header)
        self.assertEqual(
