...
# Do NOT contract sibling MaxHealth
# (sibling draft).
# Do NOT contract leftover-safe UFUNCTION siblings
# ApplyDamage / IsDestroyed / ResetNode (#1292-#1294).
...
# (sibling class ASkyguardProtectAsset).
...
TARGET = "float Health = 160.f;"
TARGET_WRONG_BARE = "float Health;"
TARGET_WRONG_FALSE = "float Health = false;"
TARGET_WRONG_TRUE = "float Health = true;"
TARGET_WRONG_INT = "float Health = 160;"
TARGET_WRONG_ZERO = "float Health = 0.f;"
TARGET_WRONG_SIBLING = "float MaxHealth = 160.f;"
LOCKED_DECL = TARGET
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `= 160.f` default. Do not
    # accept a bare decl or `= false` / `= true` / `= 0.f`
    # / `= 160` when origin/main has
    # `float Health = 160.f;`. Do not accept sibling
    # MaxHealth or leftover-safe UFUNCTION siblings
    # ApplyDamage / IsDestroyed / ResetNode.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"float\s+Health\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Health\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Health\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Health\s*=\s*160\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+Health\s*=\s*160\.f\s*;",
        compact,
    ) is None:
        return False
    return True
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+Health\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float Health is missing from "
...
class RadarNodeHealthFieldDeclContractTests(unittest.TestCase):
    def test_radar_node_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "Health"), section)
    ...
    def test_protected_declaration_does_not_satisfy_public_lock(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "Health"), section)
    ...
    def test_missing_health_declaration_fails_closed(self) -> None:
        empty = (
            f"class {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("Health = 160.f", compact_origin)
        self.assertNotIn("Health = false", compact_origin)
        self.assertNotIn("Health = true", compact_origin)
        self.assertNotIn("Health = 0.f", compact_origin)

    def test_health_declaration_matches_origin_main(self) -> None:
        section = public_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float Health"),
            LOCKED_DECL,
        )
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "radar-node Health field decl contract "
