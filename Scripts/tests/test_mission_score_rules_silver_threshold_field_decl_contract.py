# THIS IS leftover-safe FSkyguardMissionScoreRules SilverThreshold.
# Analog leftover mission-score-rules-defaults #185 then isolated
# per-field decl. Do NOT reopen leftover #185.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadOnly, meta = (ClampMin = "1"))
# then `int32 SilverThreshold = 8000;`.
# THIS IS leftover-safe isolated UPROPERTY with NO Category.
# EditAnywhere IS present. BlueprintReadOnly IS present.
# Type IS int32. Initializer IS `= 8000`.
# meta ClampMin spaces around `=` are required.
# Fail-closed if the UPROPERTY or decl is missing
# or renamed, if EditAnywhere is dropped, if
# BlueprintReadWrite instead of BlueprintReadOnly,
# if type is not int32, if initializer is not `= 8000`,
# if meta ClampMin is missing or wrong,
# or if Category is added.
...
# Do NOT parse TheaterKit.
# Do NOT contract sibling fields CompletionScore /
# PerfectAccuracyBonus / NoDamageBonus / BronzeThreshold /
# GoldThreshold.
# THIS IS NOT leftover mission-score-rules-defaults #185.
# THIS IS NOT leftover weather-profile-defaults #184.
# THIS IS NOT leftover enemy-formation-defaults #183.
# THIS IS NOT leftover enemy-wave-defaults #189.
# THIS IS NOT leftover route-point-defaults #180.
# THIS IS NOT leftover route-definition-fields #192.
# THIS IS NOT leftover mission-types-defaults-tests #153.
# THIS IS NOT leftover #1332-#1358.
# THIS IS NOT leftover TheaterKit #1299-#1331.
# THIS IS NOT leftover weather-profile Precipitation
# field-decl branch.
# Analog leftover briefing-card-defaults then isolated
# CardId field decls.
# If a clone asserts Category="Skyguard|Theater" /
# VisibleAnywhere / bare FName / float / ClampMax,
# retarget: type is int32, initializer is `= 8000`,
# no Category, ClampMin = "1" only.
...
TARGET = "int32 SilverThreshold = 8000;"
TARGET_WRONG_BARE = "int32 SilverThreshold;"
TARGET_WRONG_FALSE = "int32 SilverThreshold = false;"
TARGET_WRONG_TRUE = "int32 SilverThreshold = true;"
TARGET_WRONG_ZERO = "int32 SilverThreshold = 0;"
TARGET_WRONG_COMPLETION = "int32 SilverThreshold = 5000;"
TARGET_WRONG_BONUS = "int32 SilverThreshold = 2500;"
TARGET_WRONG_NODAMAGE = "int32 SilverThreshold = 1500;"
TARGET_WRONG_GOLD = "int32 SilverThreshold = 11000;"
TARGET_WRONG_FLOAT = "float SilverThreshold = 8000;"
TARGET_WRONG_FNAME = "FName SilverThreshold;"
...
SIBLING_COMPLETION_SCORE = "CompletionScore"
SIBLING_PERFECT_ACCURACY = "PerfectAccuracyBonus"
SIBLING_NO_DAMAGE_BONUS = "NoDamageBonus"
...
LEFTOVER_WEATHER_PROFILE_WEATHER_FIELD = (
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_COMPLETION_SCORE,
        SIBLING_PERFECT_ACCURACY,
        SIBLING_NO_DAMAGE_BONUS,
        SIBLING_BRONZE,
        SIBLING_GOLD,
    )


def leftover_unparsed_types() -> tuple[str, ...]:
    return (
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `int32 SilverThreshold = 8000;`.
    # Do not accept bare `int32 SilverThreshold;` / `= 0` /
    # `= 5000` / `= 2500` / `= 1500` / leftover CompletionScore
    # when origin/main has `= 8000`. Do not accept sibling
    # CompletionScore / PerfectAccuracyBonus / NoDamageBonus /
    # BronzeThreshold / GoldThreshold fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"int32\s+SilverThreshold\s*=\s*8000\s*;",
        compact,
    ) is None:
        return False
    if re.search(
        r"FName\s+SilverThreshold\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+SilverThreshold\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+SilverThreshold\s*;",
        compact,
    ):
        return False
    return True
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return leftover_unparsed_types() + (
        ...
        f"class SKYGUARD52_API {THEATER_KIT_ACTOR}",
        f"class {THEATER_KIT_ACTOR}",
        f"class SKYGUARD52_API {MISSION_DEF_NAME}",
        f"class {MISSION_DEF_NAME}",
    )
...
def spec_section(header: str) -> str:
    ...
    for stop in leftover_unparsed_types():
        if stop in section:
            raise AssertionError(
    ...
    if forty in section or eighty in section:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes Harbor 40/80 tokens"
        )
    return section
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*int32\s+SilverThreshold\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for int32 SilverThreshold is missing from "
...
class MissionScoreRulesSilverThresholdFieldDeclContractTests(unittest.TestCase):
    def test_mission_score_rules_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, THEATER_KIT_SPEC)
        self.assertNotEqual(STRUCT_NAME, THEATER_KIT_ACTOR)
        self.assertNotEqual(STRUCT_NAME, MISSION_DEF_NAME)
        ...
        self.assertTrue(has_identifier(section, "SilverThreshold"), section)
        ...
        self.assertIn(STOP_BEFORE_ROUTE, header)
        ...
        self.assertNotIn(STOP_BEFORE_THEATER_SPEC, section)
        self.assertNotIn(STOP_BEFORE_THEATER_ACTOR, header)
        self.assertNotIn(STOP_BEFORE_THEATER_ACTOR, section)
    ...
    def test_neighbor_struct_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "SilverThreshold"), section)
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, section)
        self.assertNotIn(MISSION_DEF_NAME, section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("SilverThreshold", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_silver_threshold_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tint32 {SIBLING_COMPLETION_SCORE} = 5000;\n"
            "};\n"
        )
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("int32 SilverThreshold = 8000;", compact_origin)
        self.assertNotIn("SilverThreshold = NAME_None", compact_origin)
        self.assertNotIn("SilverThreshold = false", compact_origin)
        self.assertNotIn("SilverThreshold = true", compact_origin)
        self.assertNotIn("SilverThreshold = 160.f", compact_origin)
        self.assertNotIn("SilverThreshold = 0;", compact_origin)
        self.assertNotIn("SilverThreshold = 5000", compact_origin)

    def test_silver_threshold_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("int32 SilverThreshold"),
            LOCKED_DECL,
        )
        ...
        self.assertNotIn("= 5000", LOCKED_DECL)
        self.assertNotIn("= 2500", LOCKED_DECL)
        self.assertNotIn("= 1500", LOCKED_DECL)
        self.assertNotIn("= 11000", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        leftover_primary = (
            "\tint32 " + leftover_retired_primary_hits_field() + " = 2500;\n"
        )
        leftover_guided = (
            "\tint32 " + leftover_retired_guided_hits_field() + " = 2500;\n"
        )
        forty = "40" + ".f"
        eighty = "80" + ".f"
        wrongs = (
            ...
            leftover_guided,
            f"\tint32 {SIBLING_COMPLETION_SCORE} = 5000;\n",
            f"\tint32 {SIBLING_PERFECT_ACCURACY} = 2500;\n",
            f"\tint32 {SIBLING_NO_DAMAGE_BONUS} = 1500;\n",
            f"\tint32 {SIBLING_BRONZE} = 5000;\n",
            f"\tint32 {SIBLING_GOLD} = 11000;\n",
            "\tint32 SilverThresholds = 8000;\n",
            "\tfloat SilverThreshold = 8000;\n",
            "\tbool SilverThreshold;\n",
            "\tint32 SilverThreshold = " + forty + ";\n",
            "\tint32 SilverThreshold = " + eighty + ";\n",
        )
    ...
    def test_category_or_wrong_specifiers_fail_closed(self) -> None:
        added_category = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
        ...
        section = spec_section(added_category)
        specifiers = attached_uproperty_specifiers(section)
        ...
        no_edit = attached_uproperty_specifiers(
            spec_section(
                f"struct {STRUCT_NAME}\n"
                "{\n"
                f"\t{TARGET_WRONG_NO_EDIT}\n"
                f"\t{LOCKED_DECL}\n"
                "};\n"
            )
        )
        self.assertNotIn("EditAnywhere", no_edit)
        self.assertIn("EditAnywhere", origin)
        ...
        self.assertIn("BlueprintReadOnly", origin)
        no_clamp = attached_uproperty_specifiers(
            spec_section(
                f"struct {STRUCT_NAME}\n"
                "{\n"
        ...
        self.assertNotIn("ClampMin", no_clamp)
        self.assertIn("ClampMin", origin)
        self.assertNotIn("ClampMax", origin)
        wrong_min = attached_uproperty_specifiers(
            spec_section(
                f"struct {STRUCT_NAME}\n"
                "{\n"
        ...
        self.assertIn('ClampMin = "0"', wrong_min)
        self.assertNotIn('ClampMin = "1"', wrong_min)
        self.assertIn('ClampMin = "1"', origin)
        wrong_max = attached_uproperty_specifiers(
            spec_section(
                f"struct {STRUCT_NAME}\n"
                "{\n"
        ...
        self.assertIn("ClampMax", wrong_max)
        self.assertNotIn("ClampMax", origin)
        nospace = attached_uproperty_specifiers(
            spec_section(
                f"struct {STRUCT_NAME}\n"
                "{\n"
        ...
        self.assertNotIn('ClampMin = "1"', nospace)
        self.assertIn('ClampMin = "1"', origin)
    ...
    def test_does_not_contract_sibling_score_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertNotIn(STOP_BEFORE_OBJECTIVE_PROGRESS, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_WEATHER_PROFILE, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_PRESENTATION, LOCKED_DECL)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, LOCKED_DECL)
        section = spec_section(origin_main_header())
        for sibling in sibling_uncontracted_decls():
    ...
    def test_does_not_parse_neighbors(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_ROUTE, header)
        self.assertNotIn(STOP_BEFORE_ROUTE, section)
        self.assertNotIn(STOP_BEFORE_ROUTE, leaked)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, header)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, section)
        self.assertNotIn(STOP_BEFORE_MISSION_DEF, leaked)
        self.assertNotIn(STOP_BEFORE_THEATER_SPEC, header)
        self.assertNotIn(STOP_BEFORE_THEATER_SPEC, section)
        self.assertNotIn(STOP_BEFORE_THEATER_SPEC, leaked)
        self.assertNotIn(STOP_BEFORE_THEATER_ACTOR, header)
        self.assertNotIn(STOP_BEFORE_THEATER_ACTOR, section)
        self.assertNotIn(STOP_BEFORE_THEATER_ACTOR, leaked)
    ...
    def test_neighbor_helpers_do_not_satisfy(self) -> None:
        other = (
            f"\tFSkyguardWeatherProfile Weather;\n"
            f"\tint32 {SIBLING_COMPLETION_SCORE} = 5000;\n"
            f"\tint32 {SIBLING_NO_DAMAGE_BONUS} = 1500;\n"
            f"\t{TARGET_WRONG_HEALTH}\n"
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "mission-score-rules SilverThreshold field decl "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "silver_threshold_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardMissionTypes.cpp", THIS_SCRIPT)
        self.assertNotIn("SkyguardRadarNode.h", THIS_SCRIPT)
        self.assertIn(LEFTOVER_WEATHER_PROFILE_PRECIPITATION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEATHER_PROFILE_DEFAULTS, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_WEATHER_PROFILE_WEATHER_FIELD, LOCKED_SCRIPTS)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_ROUTE, locked_only)
        for leftover in leftover_unparsed_types():
