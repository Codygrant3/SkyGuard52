# THIS IS leftover-safe FSkyguardCpgHudSnapshot RangeMeters.
...
# `float RangeMeters = -1.f;` with that in-struct initializer.
# Fail-closed if RangeMeters is missing or renamed, if the
# type is not float, if the initializer is missing or is
# not `-1.f`, or if UPROPERTY / Category clones land.
...
# `FString RangeLine;` (leftover-safe #1501) instead of
# RangeMeters. Harbor 40/80 is fail-closed as a false
# RangeMeters initializer via split tokens.
...
# StationStatus / LockPhase / SightMode / ThreatCount /
...
# LockLine #1505. Keep those files in LOCKED_SCRIPTS.
...
# type is float, identifier is RangeMeters, initializer
# is `= -1.f`, locked decl is the plain field, not a
# UPROPERTY wrap.
...
TARGET = "float RangeMeters = -1.f;"
TARGET_WRONG_BARE = "float RangeMeters;"
TARGET_WRONG_EQ_EMPTY = 'float RangeMeters = TEXT("");'
TARGET_WRONG_EQ_NONE = "float RangeMeters = NAME_None;"
TARGET_WRONG_FALSE = "float RangeMeters = false;"
TARGET_WRONG_TRUE = "float RangeMeters = true;"
TARGET_WRONG_ZERO = "float RangeMeters = 0.f;"
TARGET_WRONG_FSTRING = "FString RangeMeters;"
TARGET_WRONG_HEALTH = "float RangeMeters = 160.f;"
TARGET_WRONG_BOOL = "bool RangeMeters;"
TARGET_WRONG_INT = "int32 RangeMeters;"
TARGET_WRONG_FNAME = "FName RangeMeters;"
TARGET_WRONG_TCHAR = 'const TCHAR* RangeMeters = TEXT("");'
...
RANGE_METERS_LOCKED_RE = re.compile(
...
def range_meters_has_uproperty_wrap(region: str) -> bool:
    compact = collapsed(region)
    return re.search(
        r"UPROPERTY\([^;]*\)\s*float\s+RangeMeters\b",
        compact,
    ) is not None


def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored `float RangeMeters = -1.f;`.
    # Do not accept a missing initializer, Harbor 40/80,
    # `= TEXT("")` / `= NAME_None` / `= false` / `= true` /
    # `= 0.f` / `= 160.f`. Do not accept leftover sibling
    # `FString RangeLine;` (#1501) or leftover ContactMark
    ...
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if RANGE_LINE_LEFTOVER_RE.search(compact) and (
        RANGE_METERS_LOCKED_RE.search(compact) is None
    ):
        return False
    if re.search(r"FString\s+RangeMeters\b", compact):
        return False
    if re.search(r"bool\s+RangeMeters\b", compact):
        return False
    if re.search(r"int32\s+RangeMeters\b", compact):
        return False
    if re.search(r"FName\s+RangeMeters\b", compact):
        return False
    if re.search(r"const\s+TCHAR\s*\*\s*RangeMeters\b", compact):
        return False
    ...
    if RANGE_METERS_WRONG_INIT_RE.search(compact):
        return False
    if RANGE_METERS_LOCKED_RE.search(compact) is None:
        return False
    if range_meters_has_uproperty_wrap(region):
        return False
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float RangeMeters\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float RangeMeters is missing from "
...
def require_no_uproperty_wrap(section: str) -> None:
    compact = collapsed(section)
    if re.search(
        r"UPROPERTY\([^)]*\)\s*float RangeMeters\b",
        compact,
    ):
        raise AssertionError(
            "UPROPERTY wrap on float RangeMeters is not the locked "
            f"decl for plain C++ struct {STRUCT_NAME}"
        )
    if "UPROPERTY" in section and has_identifier(section, "RangeMeters"):
        raise AssertionError(
            "UPROPERTY clone landed on RangeMeters; locked decl is "
            f"plain {LOCKED_DECL}"
        )
    for token in ("USTRUCT", "GENERATED_BODY"):
        if token in section and has_identifier(section, "RangeMeters"):
            raise AssertionError(
                f"{token} clone landed on RangeMeters; locked decl is "
                f"plain {LOCKED_DECL}"
            )


class CpgHudSnapshotRangeMetersFieldDeclContractTests(unittest.TestCase):
    def test_cpg_hud_snapshot_struct_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RangeMeters"), section)
    ...
    def test_locked_decl_is_plain_cpp_not_uproperty(self) -> None:
        self.assertEqual(LOCKED_DECL, "float RangeMeters = -1.f;")
        self.assertEqual(LOCKED_DECL, TARGET)
        self.assertTrue(LOCKED_DECL.startswith("float RangeMeters"))
        self.assertTrue(LOCKED_DECL.endswith(";"))
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("= -1.f", LOCKED_DECL)
        self.assertIn("RangeMeters", LOCKED_DECL)
    ...
    def test_clone_uproperty_as_locked_decl_fails_closed(self) -> None:
        ...
        self.assertIn("RangeMeters", str(raised.exception))
    ...
    def test_contact_mark_after_snapshot_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "RangeMeters"), section)
        self.assertNotIn(STOP_BEFORE_CONTACT_MARK, section)
        self.assertNotIn(LEFTOVER_CONTACT_MARK, section)
        with self.assertRaises(AssertionError) as raised:
    ...
    def test_missing_range_meters_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_range_line_does_not_satisfy_range_meters(self) -> None:
        leftover = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{TARGET_WRONG_RANGE_LINE}\n"
    ...
    def test_missing_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float RangeMeters = -1.f;", compact_origin)
        self.assertNotIn("RangeMeters = TEXT", compact_origin)
        self.assertNotIn("RangeMeters = NAME_None", compact_origin)
        self.assertNotIn("RangeMeters = false", compact_origin)
        self.assertNotIn("RangeMeters = true", compact_origin)
        self.assertNotIn("RangeMeters = 0.f", compact_origin)
        self.assertNotIn("RangeMeters = 160.f", compact_origin)
        forty = "40" + ".f"
        eighty = "80" + ".f"
        self.assertNotIn("RangeMeters = " + forty, compact_origin)
        self.assertNotIn("RangeMeters = " + eighty, compact_origin)

    def test_range_meters_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float RangeMeters"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tint32 RangeMeters;\n",
            "\tbool RangeMeters;\n",
            "\tfloat RangeMeters = " + forty + ";\n",
            "\tfloat RangeMeters = " + eighty + ";\n",
        )
    ...
    def test_uproperty_or_category_clone_fails_closed(self) -> None:
        ...
        for wrapped in wraps:
            ...
            if "UPROPERTY" in wrapped:
                self.assertTrue(
                    range_meters_has_uproperty_wrap(section),
                    section,
                )
                with self.assertRaises(AssertionError) as raised:
                    require_declaration(section, LOCKED_DECL)
        ...
        self.assertFalse(range_meters_has_uproperty_wrap(origin), origin)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "cpg-hud-snapshot RangeMeters field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "range_meters_field_decl_contract.py"
        ))
