# THIS IS leftover-safe FSkyguardProductionAudioRouting
# CockpitLowPassHz.
# origin/main form: one-line and split-line UPROPERTY wraps
# UPROPERTY(EditAnywhere, BlueprintReadWrite,
# meta=(ClampMin="100.0", ClampMax="20000.0"))
# then `float CockpitLowPassHz = 7200.f;`.
# THIS STRUCT USES BlueprintReadWrite (not BlueprintReadOnly).
# Lock BlueprintReadWrite. Required float initializer `= 7200.f`.
# Required meta ClampMin="100.0" ClampMax="20000.0".
# Fail-closed if missing/renamed, EditAnywhere dropped,
# BlueprintReadOnly replaces BlueprintReadWrite, type not
# float, initializer dropped or changed from 7200.f,
# ClampMin/ClampMax dropped or changed, Category= added.
# Accept one-line and split-line UPROPERTY wraps.
# Meta specifiers may be on the same UPROPERTY line.
# Parse STRUCT `FSkyguardProductionAudioRouting` body ONLY after
# `struct FSkyguardProductionAudioRouting`. Stop before
# `struct FSkyguardProductionAudioAudit`.
# Do NOT parse leftover `FSkyguardProductionAudioEntry`
# (exhausted isolated decls plus leftover Attenuation).
...
# Do NOT parse leftover `enum class ESkyguardAudioSourceStatus`.
# Do NOT parse leftover production-audio-routing-defaults analog.
# Do NOT fail the whole header for leftover live-copy tokens
# (those live in the preceding category enum).
# Fail-closed only if live leftover tokens / Harbor clock
# tokens appear IN THE PARSE WINDOW or IN THE TEST FILE.
# Harbor fail-closed is the split forty / eighty tokens only;
# locked initializer 7200.f is allowed and is NOT Harbor.
# Sibling CockpitExteriorAttenuation lives in this struct and
# stays unclaimed (its 0.72f Clamp meta is different).
# Do not parse leftover USkyguardAudioProductionBank /
# USkyguardAudioProceduralBankComponent.
# Sibling decls stay unclaimed: MasterSubmix / CockpitSubmix /
# ExteriorSubmix / WeaponsSubmix / ExplosionsSubmix /
# RadioSubmix / CockpitSoundMix / CockpitExteriorAttenuation.
# Use identifier bounds so leftover CockpitSubmix /
# CockpitSoundMix / leftover CockpitExteriorAttenuation do not
# false-positive on the Cockpit substring.
# Analog leftover production-audio-routing-defaults then
# isolated CockpitLowPassHz field decl.
# THIS IS NOT leftover CockpitExteriorAttenuation.
# THIS IS NOT leftover CockpitSubmix.
# THIS IS NOT leftover CockpitSoundMix.
# THIS IS NOT leftover ProductionAudioEntry Attenuation.
# THIS IS NOT leftover production-audio-audit-defaults.
# Keep bulk leftover files in LOCKED_SCRIPTS (do not relock
# test_campaign_theater_kit_contract.py or leftover
# mission-types-defaults-tests).
# Do NOT parse leftover MissionPresentation /
# ObjectiveAnchor / LandmarkAnchor / SaveRecord isolated decls.
# Do NOT parse leftover RadioLine / RoutePoint / EnemyWave /
# Formation / Weather / ScoreRules / ObjectiveDefinition /
# BossWeakPoint / BossDefinition isolated decls.
# Do NOT parse leftover USkyguardMissionDefinition.
...
# ESkyguardAudioEvent.
# If a clone asserts no initializer / TSoftObjectPtr / bool /
# `= false` / VisibleAnywhere / BlueprintReadOnly /
# Category="Skyguard|Theater" / parse window
# FSkyguardProductionAudioEntry / 0.72f, retarget:
# parse FSkyguardProductionAudioRouting only, type is
# float, REQUIRED initializer `= 7200.f`, REQUIRED meta
# ClampMin="100.0" ClampMax="20000.0", BlueprintReadWrite,
# no Category.
...
# Harbor forty / eighty fail-closed.
...
STRUCT_NAME = "FSkyguardProductionAudioRouting"
ACTOR_NAME = "ASkyguardCampaignTheaterKit"
NAMESPACE_NAME = "SkyguardCampaignTheaterKit"
TARGET = "float CockpitLowPassHz = 7200.f;"
TARGET_WRONG_BARE = "float CockpitLowPassHz;"
TARGET_WRONG_EQ_NONE = "float CockpitLowPassHz = nullptr;"
TARGET_WRONG_FALSE = "float CockpitLowPassHz = false;"
TARGET_WRONG_TRUE = "float CockpitLowPassHz = true;"
TARGET_WRONG_ZERO = "float CockpitLowPassHz = 0.f;"
TARGET_WRONG_SOFT = (
    "TSoftObjectPtr<USoundSubmixBase> CockpitLowPassHz;"
)
TARGET_WRONG_FNAME = "FName CockpitLowPassHz;"
TARGET_WRONG_BOOL = "bool CockpitLowPassHz;"
TARGET_WRONG_INT = "int32 CockpitLowPassHz;"
TARGET_WRONG_FTEXT = "FText CockpitLowPassHz;"
TARGET_WRONG_PTR = "USoundSubmixBase* CockpitLowPassHz;"
TARGET_WRONG_OBJECT_PTR = "TObjectPtr<USoundSubmixBase> CockpitLowPassHz;"
TARGET_WRONG_WAVE = "TSoftObjectPtr<USoundMix> CockpitLowPassHz;"
TARGET_WRONG_SOUND_BASE = "TSoftObjectPtr<USoundBase> CockpitLowPassHz;"
TARGET_WRONG_FSTRING = "FString CockpitLowPassHz;"
TARGET_WRONG_ATTENUATION = "float CockpitLowPassHz = 0.72f;"
...
UPROPERTY_EDIT_READWRITE_BARE = "UPROPERTY(EditAnywhere, BlueprintReadWrite)"
...
STOP_BEFORE_BANK = "USkyguardAudioProductionBank"
...
SIBLING_MASTER_SUBMIX = "MasterSubmix"
SIBLING_COCKPIT_SUBMIX = "CockpitSubmix"
SIBLING_EXTERIOR_SUBMIX = "ExteriorSubmix"
SIBLING_WEAPONS_SUBMIX = "WeaponsSubmix"
SIBLING_EXPLOSIONS_SUBMIX = "ExplosionsSubmix"
SIBLING_RADIO_SUBMIX = "RadioSubmix"
SIBLING_COCKPIT_SOUND_MIX = "CockpitSoundMix"
SIBLING_COCKPIT_EXTERIOR_ATTENUATION = "CockpitExteriorAttenuation"
...
THIS_SCRIPT = (
    "Scripts/tests/test_production_audio_routing_cockpit_low_pass"
    "_hz_field_decl_contract.py"
)
LEFTOVER_ROUTING_MASTER_SUBMIX = (
    "Scripts/tests/test_production_audio_routing_master_submix"
    "_field_decl_contract.py"
)
LEFTOVER_ROUTING_COCKPIT_SUBMIX = (
    "Scripts/tests/test_production_audio_routing_cockpit_submix"
    "_field_decl_contract.py"
)
LEFTOVER_ROUTING_EXTERIOR_SUBMIX = (
    "Scripts/tests/test_production_audio_routing_exterior_submix"
    "_field_decl_contract.py"
)
LEFTOVER_ROUTING_WEAPONS_SUBMIX = (
    "Scripts/tests/test_production_audio_routing_weapons_submix"
    "_field_decl_contract.py"
)
LEFTOVER_ROUTING_EXPLOSIONS_SUBMIX = (
    "Scripts/tests/test_production_audio_routing_explosions_submix"
    "_field_decl_contract.py"
)
LEFTOVER_ROUTING_RADIO_SUBMIX = (
    "Scripts/tests/test_production_audio_routing_radio_submix"
    "_field_decl_contract.py"
)
LEFTOVER_ROUTING_COCKPIT_SOUND_MIX = (
    "Scripts/tests/test_production_audio_routing_cockpit_sound_mix"
    "_field_decl_contract.py"
)
LEFTOVER_ROUTING_COCKPIT_EXTERIOR_ATTENUATION = (
    "Scripts/tests/test_production_audio_routing_cockpit_exterior"
    "_attenuation_field_decl_contract.py"
)
LEFTOVER_ENTRY_DISPLAY_NAME = (
    "Scripts/tests/test_production_audio_entry_display_name"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_PROVENANCE_ID = (
    "Scripts/tests/test_production_audio_entry_provenance_id"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_SOUND = (
    "Scripts/tests/test_production_audio_entry_sound"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_OUTPUT_SUBMIX = (
    "Scripts/tests/test_production_audio_entry_output_submix"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_CATEGORY = (
    "Scripts/tests/test_production_audio_entry_category"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_ATTENUATION = (
    "Scripts/tests/test_production_audio_entry_attenuation"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_CONCURRENCY = (
    "Scripts/tests/test_production_audio_entry_concurrency"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_SOURCE_SHA256 = (
    "Scripts/tests/test_production_audio_entry_source_sha256"
    "_field_decl_contract.py"
)
LEFTOVER_ENTRY_SOURCE_STATUS = (
    "Scripts/tests/test_production_audio_entry_source_status"
    "_field_decl_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ROUTING_DEFAULTS = (
    "Scripts/tests/test_production_audio_routing_defaults"
    "_contract.py"
)
LEFTOVER_PRODUCTION_AUDIO_ROUTING_DEFAULTS_PY = (
    "Scripts/tests/test_production_audio_routing_defaults.py"
)
LEFTOVER_PRODUCTION_AUDIO_ROUTING_DEFAULTS_TESTS = (
    "Scripts/tests/test_production_audio_routing_defaults"
    "_tests.py"
)
LEFTOVER_PRODUCTION_AUDIO_AUDIT_DEFAULTS = (
    "Scripts/tests/test_production_audio_audit_defaults"
    "_contract.py"
)
...
def sibling_uncontracted_decls() -> tuple[str, ...]:
    return (
        SIBLING_MASTER_SUBMIX,
        SIBLING_COCKPIT_SUBMIX,
        SIBLING_EXTERIOR_SUBMIX,
        SIBLING_WEAPONS_SUBMIX,
        SIBLING_EXPLOSIONS_SUBMIX,
        SIBLING_RADIO_SUBMIX,
        SIBLING_COCKPIT_SOUND_MIX,
        SIBLING_COCKPIT_EXTERIOR_ATTENUATION,
    )
...
LOCKED_SCRIPTS_FROM_THEATER_KIT_SPEC = (
    "Scripts/tests/test_mission08_objective_count_field_decl_contract.py",
    "Scripts/tests/test_mission01_objective_count_field_decl_contract.py",
    "Scripts/tests/test_mission02_objective_count_field_decl_contract.py",
    "Scripts/tests/test_mission03_objective_count_field_decl_contract.py",
...
LOCKED_SCRIPTS = tuple(
    dict.fromkeys(
        LOCKED_SCRIPTS + LOCKED_SCRIPTS_FROM_THEATER_KIT_SPEC
    )
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # Do not accept a bare field, `= false` / `= true` /
    # `= 0.f` / leftover 0.72f / leftover float Health /
    # Harbor forty / eighty / TSoftObjectPtr when
    # origin/main requires float + 7200.f. Do not accept
    # sibling MasterSubmix / CockpitSubmix / ExteriorSubmix /
    # CockpitExteriorAttenuation.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TSoftObjectPtr\s*<[^>]+>\s+CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr\s*<[^>]+>\s+CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"USoundSubmixBase\s*\*\s*CockpitLowPassHz\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CockpitLowPassHz\s*;",
        compact,
    ):
        return False
    if re.search(
        r"float\s+CockpitLowPassHz\s*=\s*7200\.f\s*;",
        compact,
    ) is None:
        return False
    return True
...
def parse_window(header: str) -> str:
    ...
    stop_token = STOP_BEFORE_AUDIT
    start = header.find(start_token)
    if start < 0:
        raise AssertionError(
            f"{STRUCT_NAME} is missing from origin/main:{HEADER_PATH}"
        )
    # Start at the claimed struct so leftover category-enum
    # live-copy tokens, leftover source-status enum, and
    # leftover ProductionAudioEntry stay OUT of the window.
    enum_at = header.find(STOP_BEFORE_CATEGORY_ENUM)
    if enum_at >= 0 and start <= enum_at:
        raise AssertionError(
            f"{STRUCT_NAME} parse window started at "
            f"{STOP_BEFORE_CATEGORY_ENUM}"
        )
    status_at = header.find(STOP_BEFORE_SOURCE_STATUS_ENUM)
    if status_at >= 0 and start <= status_at:
        raise AssertionError(
            f"{STRUCT_NAME} parse window started at "
            f"{STOP_BEFORE_SOURCE_STATUS_ENUM}"
        )
    entry_at = header.find(STOP_BEFORE_ENTRY)
    if entry_at >= 0 and start <= entry_at:
        raise AssertionError(
            f"{STRUCT_NAME} parse window started at "
            f"{STOP_BEFORE_ENTRY}"
        )
...
def struct_body(header: str) -> str:
    window = header
    if STRUCT_RE.search(header) is not None and STOP_BEFORE_AUDIT in header:
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CockpitLowPassHz\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CockpitLowPassHz is missing from "
...
class ProductionAudioRoutingCockpitLowPassHzFieldDeclContractTests(
    unittest.TestCase
):
    def test_production_audio_routing_struct_exists(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_AUDIT, header)
        self.assertNotIn(STOP_BEFORE_AUDIT, body)
        self.assertNotIn(STOP_BEFORE_AUDIT, section)
    ...
    def test_missing_struct_fails_closed(self) -> None:
        with self.assertRaises(AssertionError) as raised:
            struct_body(
                "struct FSkyguardUnrelatedProductionAudioEntry\n{\n};\n"
            )
    ...
    def test_missing_cockpit_low_pass_hz_declaration_fails_closed(
        self,
    ) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_MASTER_SUBMIX};\n"
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_COCKPIT_SUBMIX};\n"
            f"\tTSoftObjectPtr<USoundMix> {SIBLING_COCKPIT_SOUND_MIX};\n"
            f"\tfloat {SIBLING_COCKPIT_EXTERIOR_ATTENUATION} = 0.72f;\n"
            "};\n"
        )
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertNotIn("Category", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertIn('ClampMin="100.0"', specifiers)
        self.assertIn('ClampMax="20000.0"', specifiers)
        self.assertTrue(has_declaration(section, LOCKED_DECL), section)
    ...
    def test_initializer_fails_closed_when_dropped_or_changed(self) -> None:
        bare = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READWRITE}\n"
        ...
        flipped_section = spec_section(flipped)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(flipped_section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        low_pass = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READWRITE}\n"
            f"\t{TARGET_WRONG_ATTENUATION}\n"
            "};\n"
        )
        low_pass_section = spec_section(low_pass)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(low_pass_section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        ...
        self.assertNotIn("CockpitLowPassHz = nullptr", compact_origin)
        self.assertNotIn("CockpitLowPassHz = false", compact_origin)
        self.assertNotIn("CockpitLowPassHz = true", compact_origin)
        self.assertNotIn("CockpitLowPassHz = 0.f", compact_origin)
        self.assertNotIn("CockpitLowPassHz = 160.f", compact_origin)
        self.assertNotIn("CockpitLowPassHz = 0.72f", compact_origin)

    def test_cockpit_low_pass_hz_declaration_matches_origin_main(
        self,
    ) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
        ...
        self.assertIn("= 7200.f", LOCKED_DECL)
        ...
        self.assertNotIn("= 0.72f", LOCKED_DECL)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SOFT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FNAME}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BOOL}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_INT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FTEXT}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_PTR}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_OBJECT_PTR}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_WAVE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_SOUND_BASE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FSTRING}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_ATTENUATION}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_PTR}\n",
            f"\t{TARGET_WRONG_OBJECT_PTR}\n",
            f"\t{TARGET_WRONG_WAVE}\n",
            f"\t{TARGET_WRONG_SOUND_BASE}\n",
            ...
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_MASTER_SUBMIX};\n",
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_COCKPIT_SUBMIX};\n",
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_EXTERIOR_SUBMIX};\n",
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_RADIO_SUBMIX};\n",
            f"\tTSoftObjectPtr<USoundSubmixBase> {SIBLING_WEAPONS_SUBMIX};\n",
            f"\tfloat {SIBLING_COCKPIT_EXTERIOR_ATTENUATION} = 0.72f;\n",
            "\tTSoftObjectPtr<USoundAttenuation> Attenuation;\n",
            "\tint32 CockpitLowPassHz = 1;\n",
            "\tbool CockpitLowPassHz;\n",
            "\tfloat CockpitLowPassHz = " + forty + ";\n",
            "\tfloat CockpitLowPassHz = " + eighty + ";\n",
        )
    ...
    def test_added_category_or_dropped_edit_anywhere_fails_closed(self) -> None:
        ...
        read_specs = attached_uproperty_specifiers(
            spec_section(read_only)
        )
        self.assertIn("EditAnywhere", read_specs)
        self.assertIn("BlueprintReadOnly", read_specs)
        self.assertNotIn("BlueprintReadWrite", read_specs)
        self.assertIn("BlueprintReadWrite", origin)
        self.assertIn('ClampMin="100.0"', origin)
        self.assertIn('ClampMax="20000.0"', origin)
        dropped_meta = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READWRITE_BARE}\n"
            f"\t{LOCKED_DECL}\n"
            "};\n"
        )
        dropped_specs = attached_uproperty_specifiers(
            spec_section(dropped_meta)
        )
        self.assertNotIn("ClampMin", dropped_specs)
        self.assertNotIn("ClampMax", dropped_specs)
    ...
    def test_does_not_contract_sibling_routing_fields(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertNotIn(sibling, LOCKED_DECL)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        section = spec_section(origin_main_header())
        self.assertTrue(has_identifier(section, SIBLING_MASTER_SUBMIX), section)
        self.assertTrue(has_identifier(section, SIBLING_COCKPIT_SUBMIX), section)
        self.assertTrue(has_identifier(section, SIBLING_EXTERIOR_SUBMIX), section)
        self.assertTrue(has_identifier(section, SIBLING_WEAPONS_SUBMIX), section)
        self.assertTrue(has_identifier(section, SIBLING_EXPLOSIONS_SUBMIX), section)
        self.assertTrue(has_identifier(section, SIBLING_RADIO_SUBMIX), section)
        self.assertTrue(has_identifier(section, SIBLING_COCKPIT_SOUND_MIX), section)
        self.assertTrue(
            has_identifier(section, SIBLING_COCKPIT_EXTERIOR_ATTENUATION),
            section,
        )
        self.assertTrue(has_identifier(section, FIELD_NAME), section)
        self.assertFalse(has_identifier(SIBLING_MASTER_SUBMIX, FIELD_NAME))
        self.assertFalse(has_identifier(SIBLING_COCKPIT_SUBMIX, FIELD_NAME))
        self.assertFalse(has_identifier(SIBLING_EXTERIOR_SUBMIX, FIELD_NAME))
        self.assertFalse(has_identifier(SIBLING_RADIO_SUBMIX, FIELD_NAME))
        self.assertFalse(has_identifier(SIBLING_WEAPONS_SUBMIX, FIELD_NAME))
        self.assertFalse(has_identifier(SIBLING_COCKPIT_EXTERIOR_ATTENUATION, FIELD_NAME))
        self.assertNotIn(SIBLING_MASTER_SUBMIX, LOCKED_DECL)
        self.assertNotIn(SIBLING_COCKPIT_SUBMIX, LOCKED_DECL)
        self.assertNotIn(SIBLING_EXTERIOR_SUBMIX, LOCKED_DECL)
        self.assertNotIn(SIBLING_RADIO_SUBMIX, LOCKED_DECL)
        self.assertNotIn(SIBLING_WEAPONS_SUBMIX, LOCKED_DECL)
        self.assertNotIn(SIBLING_COCKPIT_EXTERIOR_ATTENUATION, LOCKED_DECL)
    ...
    def test_does_not_parse_neighbors_or_leftover_banks(self) -> None:
        ...
        self.assertIn(STOP_BEFORE_AUDIT, header)
        self.assertNotIn(STOP_BEFORE_AUDIT, section)
        self.assertNotIn(STOP_BEFORE_AUDIT, leaked)
    ...
    def test_declaration_does_not_invent_uproperty_metadata(self) -> None:
        ...
        self.assertIn("BlueprintReadWrite", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("Category", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('Category="Skyguard|Theater"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn('Category="Skyguard|Campaign"', UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("VisibleAnywhere", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("BlueprintReadOnly", UPROPERTY_EDIT_READWRITE)
        ...
        self.assertIn('ClampMin="100.0"', UPROPERTY_EDIT_READWRITE)
        self.assertIn('ClampMax="20000.0"', UPROPERTY_EDIT_READWRITE)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-routing CockpitLowPassHz field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertIn(LEFTOVER_ENTRY_DISPLAY_NAME, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ENTRY_PROVENANCE_ID, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ENTRY_SOUND, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ENTRY_OUTPUT_SUBMIX, LOCKED_SCRIPTS)
        ...
        self.assertIn(LEFTOVER_ROUTING_MASTER_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTING_COCKPIT_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTING_EXTERIOR_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTING_RADIO_SUBMIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTING_COCKPIT_SOUND_MIX, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ROUTING_COCKPIT_EXTERIOR_ATTENUATION, LOCKED_SCRIPTS)
        self.assertIn(LEFTOVER_ENTRY_ATTENUATION, LOCKED_SCRIPTS)
        self.assertTrue(Path(__file__).name.endswith(
            "cockpit_low_pass_hz_field_decl_contract.py"
        ))
