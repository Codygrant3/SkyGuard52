...
# then `float CriticalAttackIntervalSeconds = 4.8f;` (HAS initializer).
...
# BlueprintReadWrite IS present. ClampMin IS `"0.2"`. Default IS `= 4.8f`.
# Clone wrap ClampMin WAS `"0.2"` for leftover unique Telegraph
# ApproachAttackIntervalSeconds. THIS slot ClampMin IS `"0.2"`
# (NO spaces around =). Fail-closed if wrap still has
# ClampMin="0.0" / ClampMin="100.0" / ClampMin="1.0". 4.8f is NOT
# Harbor 40 or 80 and NOT leftover unique ApproachAttack 6.5f.
...
# Do NOT claim leftover in-flight
# LockWindowAttackIntervalSeconds = 8.f.
# Do NOT claim leftover analog Telegraph siblings
# AttackTelegraphLeadSeconds = 1.2f /
# MaxTelegraphsPerEncounter = 16.
# Do NOT claim leftover unique Commands
# CommandLateralBiasCm = 850.f (#1673) / BreakClimbCm = 260.f (#1674).
...
# `"1.0"`, if the default is not `= 4.8f`, if Category is
...
# — RETARGET hard: wrap stays EditAnywhere BlueprintReadWrite
# Category="Skyguard|Boss|Encounter|Telegraph", meta=(ClampMin="0.2")
# (NO spaces around =). LOCKED_DECL is
# `float CriticalAttackIntervalSeconds = 4.8f;`.
# Fail-closed if wrap still has
...
# LOCKED_DECL is `float CriticalAttackIntervalSeconds = 4.8f;`
# NOT leftover unique `float ApproachAttackIntervalSeconds = 6.5f;`
# NOT leftover in-flight `float LockWindowAttackIntervalSeconds = 8.f;`
...
# AttackTelegraphLeadSeconds / MaxTelegraphsPerEncounter.
...
# / `"100.0"` / `"1.0"`, or default is not 4.8f.
...
TARGET = "float CriticalAttackIntervalSeconds = 4.8f;"
TARGET_WRONG_BARE = "float CriticalAttackIntervalSeconds;"
TARGET_WRONG_INIT = "float CriticalAttackIntervalSeconds = nullptr;"
TARGET_WRONG_FALSE = "bool bLicensedVegetationLibraryApproved = false;"
TARGET_WRONG_TRUE = "float CriticalAttackIntervalSeconds = true;"
TARGET_WRONG_ZERO = "float CriticalAttackIntervalSeconds = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CriticalAttackIntervalSeconds;"
...
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CriticalAttackIntervalSeconds;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CriticalAttackIntervalSeconds;"
...
STOP_BEFORE_SKYLINE_STYLE = "AttackTelegraphLeadSeconds"
...
THIS_SCRIPT = (
    "Scripts/tests/test_pathfinder_encounter_critical_attack_interval"
...
def leftover_analog_extra_scripts() -> tuple[str, ...]:
    ...
    return (
        ...
        "_field_decl_contract.py",
        f"{prefix}test_pathfinder_encounter_approach_attack_interval"
        "_field_decl_contract.py",
...
CRITICAL_ATTACK_INTERVAL_DECL_RE = re.compile(
    r"float\s+CriticalAttackIntervalSeconds\s*=\s*4\.8f\s*;"
)
CRITICAL_ATTACK_INTERVAL_BARE_RE = re.compile(
    r"float\s+CriticalAttackIntervalSeconds\s*;"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float CriticalAttackIntervalSeconds = 4.8f;`.
    ...
    # leftover in-flight LockWindowAttackIntervalSeconds = 8.f,
    ...
    # bReadyForAuthoredPCGGeneration, leftover unique
    # ApproachAttackIntervalSeconds = 6.5f, or a bare
    # CriticalAttackIntervalSeconds without the 4.8f initializer.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CRITICAL_ATTACK_INTERVAL_DECL_RE.search(compact) is None:
        return False
    if CRITICAL_ATTACK_INTERVAL_BARE_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_SKYLINE_STYLE:
            break
        if ident != "CriticalAttackIntervalSeconds":
            cursor = semi + 1
            continue
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+CriticalAttackIntervalSeconds\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float CriticalAttackIntervalSeconds "
...
class PathfinderEncounterCriticalAttackIntervalFieldDeclContractTests(unittest.TestCase):
    def test_pathfinder_encounter_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalAttackIntervalSeconds"), section)
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CriticalAttackIntervalSeconds"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CriticalAttackIntervalSeconds", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_critical_attack_interval_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertNotIn(
            "float CriticalAttackIntervalSeconds;",
            compact_origin.split("CriticalAttackIntervalSeconds = 4.8f;")[0]
            + compact_origin.split("CriticalAttackIntervalSeconds = 4.8f;")[-1]
            if "CriticalAttackIntervalSeconds = 4.8f;" in compact_origin
            else compact_origin,
        )
    ...
    def test_critical_attack_interval_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float CriticalAttackIntervalSeconds"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("4.8f", LOCKED_DECL)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat CriticalAttackIntervalSecondss = 4.8f;\n",
            "\tint32 CriticalAttackIntervalSeconds = 320;\n",
            "\tFName CriticalAttackIntervalSeconds = NAME_None;\n",
            "\tfloat CriticalAttackIntervalSeconds = " + forty + ";\n",
            "\tfloat CriticalAttackIntervalSeconds = " + eighty + ";\n",
            f"\t{TARGET_WRONG_WIND}\n",
    ...
    def test_does_not_claim_sibling_approach_speed_or_neighbors(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CriticalAttackIntervalSeconds"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CriticalAttackIntervalSeconds"))
        self.assertTrue(has_identifier(section, "CriticalAttackIntervalSeconds"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_SKYLINE_STYLE))
        self.assertFalse(has_identifier(section, STOP_BEFORE_SKYLINE_STYLE))
        self.assertTrue(has_identifier(leaked, SIBLING_APPROACH_SPEED))
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "pathfinder_encounter_critical_attack_interval_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
