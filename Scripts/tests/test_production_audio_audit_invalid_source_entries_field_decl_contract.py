...
# then `TArray<FName> InvalidSourceEntries;` (bare, no initializer).
...
# TArray<FName>, an initializer is added, or Category=
# added.
...
# ExplicitMissingSourceCount / QATestOnlyCount /
# MissingCategoryEntries / BoundProductionSourceCount /
...
# If a clone asserts int32 / required `= 0` /
# EditAnywhere / BlueprintReadWrite / TSoftObjectPtr /
# parse window FSkyguardProductionAudioEntry or Routing,
# retarget: parse FSkyguardProductionAudioAudit only,
# UPROPERTY is BlueprintReadOnly with NO EditAnywhere,
# type is TArray<FName>, BARE decl with NO initializer,
# no Category=.
# Harbor 40/80 fail-closed in the parse window and in THIS
# test file. Fail-closed if an initializer is added.
...
TARGET = "TArray<FName> InvalidSourceEntries;"
TARGET_WRONG_EQ_EMPTY = "TArray<FName> InvalidSourceEntries = {};"
TARGET_WRONG_EQ_ZERO = "TArray<FName> InvalidSourceEntries = 0;"
TARGET_WRONG_FALSE = "TArray<FName> InvalidSourceEntries = false;"
TARGET_WRONG_TRUE = "TArray<FName> InvalidSourceEntries = true;"
TARGET_WRONG_ZERO = "TArray<FName> InvalidSourceEntries = 0.f;"
TARGET_WRONG_FORTY = "TArray<FName> InvalidSourceEntries = 40;"
TARGET_WRONG_EIGHTY = "TArray<FName> InvalidSourceEntries = 80;"
TARGET_WRONG_FLOAT = "float InvalidSourceEntries;"
TARGET_WRONG_FNAME = "FName InvalidSourceEntries;"
TARGET_WRONG_BOOL = "bool InvalidSourceEntries = false;"
TARGET_WRONG_FSTRING = "FString InvalidSourceEntries;"
TARGET_WRONG_INT32 = "int32 InvalidSourceEntries = 0;"
TARGET_WRONG_INT32_INT = "TArray<int32> InvalidSourceEntries;"
TARGET_WRONG_SOFT = "TSoftObjectPtr<USoundBase> InvalidSourceEntries;"
...
FIELD_NAME = "InvalidSourceEntries"
...
THIS_SCRIPT = (
    "Scripts/tests/test_production_audio_audit_invalid"
    "_source_entries_field_decl_contract.py"
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored bare `TArray<FName> InvalidSourceEntries;`.
    # Reject added initializer (`= {}`, `= 0`, Harbor 40/80),
    # false, float, bool, FName, FString, int32, TSoftObjectPtr.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"TArray\s*<\s*FName\s*>\s+InvalidSourceEntries\s*=",
        compact,
    ):
        return False
    if re.search(
        r"InvalidSourceEntries\s*=\s*40\b",
        compact,
    ):
        return False
    if re.search(
        r"InvalidSourceEntries\s*=\s*80\b",
        compact,
    ):
        return False
    if re.search(
        r"InvalidSourceEntries\s*=\s*false\b",
        compact,
    ):
        return False
    if re.search(
        r"InvalidSourceEntries\s*=\s*true\b",
        compact,
    ):
        return False
    if re.search(
        r"InvalidSourceEntries\s*=\s*0\.f\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+InvalidSourceEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"float\s+InvalidSourceEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+InvalidSourceEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+InvalidSourceEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+InvalidSourceEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"TSoftObjectPtr\s*<[^>]*>\s+InvalidSourceEntries\b",
        compact,
    ):
        return False
    if re.search(
        r"TArray\s*<\s*int32\s*>\s+InvalidSourceEntries\b",
        compact,
    ):
        return False
    if re.search(
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TArray\s*<\s*FName\s*>\s+InvalidSourceEntries\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TArray<FName> InvalidSourceEntries is missing from "
...
class ProductionAudioAuditInvalidSourceEntriesFieldDeclContractTests(
    unittest.TestCase
):
    ...
    def test_missing_invalid_source_entries_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        zero_section = spec_section(zero)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(zero_section, LOCKED_DECL)
        self.assertIn(FIELD_NAME, str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())
        forty = (
            f"struct {STRUCT_NAME}\n"
        ...
        self.assertIn("TArray<FName> InvalidSourceEntries;", compact_origin)
        self.assertNotIn("InvalidSourceEntries = {}", compact_origin)
        self.assertNotIn("InvalidSourceEntries = 0", compact_origin)
        self.assertNotIn("InvalidSourceEntries = 40", compact_origin)
        self.assertNotIn("InvalidSourceEntries = 80", compact_origin)
        self.assertNotIn("InvalidSourceEntries = false", compact_origin)

    def test_invalid_source_entries_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TArray<FName> InvalidSourceEntries"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTArray<FName> InvalidSourceEntriess;\n",
            "\tTArray<FName> InvalidSourceEntries = 1;\n",
            "\tbool InvalidSourceEntries;\n",
            "\tfloat InvalidSourceEntries = " + forty + ";\n",
            "\tfloat InvalidSourceEntries = " + eighty + ";\n",
            f"\t{TARGET_WRONG_INT32}\n",
            f"\t{TARGET_WRONG_INT32_INT}\n",
            f"\t{TARGET_WRONG_EQ_ZERO}\n",
        )
    ...
    def test_does_not_contract_sibling_audit_fields(self) -> None:
        ...
        self.assertTrue(
            has_identifier(section, SIBLING_BOUND_PRODUCTION_SOURCE_COUNT), section
        )
        self.assertTrue(
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-audit InvalidSourceEntries field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "invalid_source_entries_field_decl_contract.py"
        ))
