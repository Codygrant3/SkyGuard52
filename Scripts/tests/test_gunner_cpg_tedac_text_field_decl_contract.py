# THIS IS leftover-safe ASkyguardGunner CpgTedacText.
...
# then BARE `TObjectPtr<UTextRenderComponent> CpgTedacText;`
# (no initializer).
...
# Stop BEFORE sibling CpgMpdLeftText as a claimed slot.
...
# CpgMpdLeftText, CpgMpdRightText, CpgEufdText, Boom,
# GunnerCamera, HandMesh.
# Do NOT claim leftover CpgConsoleRight / CpgRailLeft /
# CpgRailRight / CpgReticleV / CpgGripLeft / CpgGripRight /
# CpgTedacBezel / CpgReticleH / leftover mesh CpgTedac
# as this slot.
# THIS IS a text slot. Do not lock
# TObjectPtr<UStaticMeshComponent> CpgTedacText.
# CpgTedacText fail-closed if leftover Apache Canopy
...
# LOCKED_DECL is `TObjectPtr<UTextRenderComponent> CpgTedacText;`
...
# Text slots fail-closed if LOCKED_DECL is
# `TObjectPtr<UStaticMeshComponent>` or
# `TObjectPtr<USceneComponent>`.
# CpgTedacText fail-closed if LOCKED_DECL is leftover
# CpgConsoleRight / CpgRailLeft / CpgRailRight /
# CpgReticleV / CpgGripLeft / CpgGripRight /
# CpgTedacBezel / CpgReticleH / leftover mesh CpgTedac /
# CpgEufd / CpgDash /
...
TARGET = "TObjectPtr<UTextRenderComponent> CpgTedacText;"
TARGET_WRONG_INIT = "TObjectPtr<UTextRenderComponent> CpgTedacText = nullptr;"
TARGET_WRONG_FALSE = "TObjectPtr<UTextRenderComponent> CpgTedacText = false;"
TARGET_WRONG_TRUE = "TObjectPtr<UTextRenderComponent> CpgTedacText = true;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> CpgTedacText;"
TARGET_WRONG_FNAME = "FName WeatherIdentity;"
TARGET_WRONG_HEALTH = "float Health = 160.f;"
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> CpgTedacText;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> CpgTedacText;"
...
STOP_BEFORE_MPD_LEFT_TEXT = "CpgMpdLeftText"
...
CPG_TEDAC_TEXT_DECL_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgTedacText\s*;"
)
CPG_TEDAC_TEXT_INIT_RE = re.compile(
    r"TObjectPtr<UTextRenderComponent>\s+CpgTedacText\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # leftover CpgConsoleRight / CpgRailLeft / CpgRailRight /
    # CpgReticleV / CpgGripLeft / CpgGripRight /
    # CpgTedacBezel / CpgReticleH / leftover mesh CpgTedac /
    # CpgDash / CpgKneeRight / CpgCanopyBow / CpgSeatBack /
    # CpgSeatPan / CpgCockpit / CpgStation / leftover
    # Apache Canopy, or an
    # initializer on CpgTedacText as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if CPG_TEDAC_TEXT_DECL_RE.search(compact) is None:
        return False
    if CPG_TEDAC_TEXT_INIT_RE.search(compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if has_identifier(field_text, SIBLING_CPG_MPD_LEFT_TEXT):
            break
    ...
    if has_identifier(window, SIBLING_CPG_MPD_LEFT_TEXT):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{SIBLING_CPG_MPD_LEFT_TEXT}"
        )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*TObjectPtr<UTextRenderComponent>\s+CpgTedacText\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for TObjectPtr<UTextRenderComponent> CpgTedacText "
...
class GunnerCpgTedacTextFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "CpgTedacText"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, SIBLING_CPG_MPD_LEFT_TEXT), body)
        self.assertFalse(
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "CpgTedacText"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("CpgTedacText", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_cpg_tedac_text_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("TObjectPtr<UTextRenderComponent> CpgTedacText;", compact_origin)
        self.assertNotIn("CpgTedacText = 160.f", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)
        self.assertNotIn("FName WeatherIdentity", compact_origin)

    def test_cpg_tedac_text_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("TObjectPtr<UTextRenderComponent> CpgTedacText"),
            LOCKED_DECL,
        )
        ...
        self.assertFalse(has_identifier(LOCKED_DECL, SIBLING_CPG_TEDAC))
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "TObjectPtr<UTextRenderComponent> CpgTedacText;")
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            "\tTObjectPtr<UTextRenderComponent> CpgTedacTexts;\n",
            "\tint32 CpgTedacText = 140;\n",
            "\tbool CpgTedacText = true;\n",
            "\tfloat CpgTedacText = " + forty + ";\n",
            "\tfloat CpgTedacText = " + eighty + ";\n",
        )
    ...
    def test_does_not_claim_sibling_mpd_left_text_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "CpgTedacText"))
    ...
    def test_does_not_parse_private_or_neighbors(self) -> None:
        ...
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(leaked, "CpgTedacText"))
        self.assertTrue(has_identifier(section, "CpgTedacText"))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner CpgTedacText field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_cpg_tedac_text_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertFalse(has_identifier(locked_only, SIBLING_CPG_TEDAC))
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TEDAC)
