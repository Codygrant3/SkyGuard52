...
# CRITICAL: start at the claimed struct so leftover
# `enum class ESkyguardProductionAudioCategory` stays OUT
# of the parse window (that enum contains retired live-copy
# tokens via split tokens). Do NOT parse leftover
# `enum class ESkyguardAudioSourceStatus` (leftover #241 /
# #65ba). Do NOT fail the whole header for those leftover
# live-copy tokens; fail-closed only if they appear IN THE
# PARSE WINDOW or IN THE TEST FILE.
...
# OutputSubmix / SourceSha256.
# Analog leftover production-audio-entry-defaults #198 /
# #314b then isolated ProvenanceId field decl.
...
# THIS IS NOT leftover audio-procedural-bank-empty-fail-closed.
...
# ObjectiveAnchor / LandmarkAnchor / SaveRecord isolated
# decls.
...
# ESkyguardAudioEvent.
# If a clone asserts FText / bool / VisibleAnywhere /
# BlueprintReadOnly / Category="Skyguard|Theater" /
# `= NAME_None` / `= false`, retarget: type is FName,
...
TARGET = "FName ProvenanceId;"
TARGET_WRONG_EQ_NONE = "FName ProvenanceId = NAME_None;"
TARGET_WRONG_FALSE = "FName ProvenanceId = false;"
TARGET_WRONG_TRUE = "FName ProvenanceId = true;"
TARGET_WRONG_ZERO = "FName ProvenanceId = 0.f;"
TARGET_WRONG_FLOAT = "float ProvenanceId;"
TARGET_WRONG_FTEXT = "FText ProvenanceId;"
TARGET_WRONG_FSTRING = "FString ProvenanceId;"
TARGET_WRONG_BOOL = "bool ProvenanceId;"
TARGET_WRONG_INT = "int32 ProvenanceId;"
...
STOP_BEFORE_SOURCE_STATUS = "enum class ESkyguardAudioSourceStatus"
...
def leftover_production_category_tokens() -> tuple[str, ...]:
    return (
        "Ri" + "fleMuzzle",
        "Ri" + "fleMechanical",
        "Ri" + "fleCasing",
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on the authored bare `FName ProvenanceId;`.
    # Do not accept `= NAME_None` / `= false` / `= true` /
    # `= 0.f` / leftover float Health when origin/main has
    # a bare FName. Do not accept sibling Category /
    # DisplayName / SourceStatus / Sound fields.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if re.search(
        r"FName\s+ProvenanceId\s*=",
        compact,
    ):
        return False
    if re.search(
        r"float\s+ProvenanceId\b",
        compact,
    ):
        return False
    if re.search(
        r"FText\s+ProvenanceId\b",
        compact,
    ):
        return False
    if re.search(
        r"FString\s+ProvenanceId\b",
        compact,
    ):
        return False
    if re.search(
        r"bool\s+ProvenanceId\b",
        compact,
    ):
        return False
    if re.search(
        r"int32\s+ProvenanceId\b",
        compact,
    ):
        return False
    if re.search(
...
def parse_window(header: str) -> str:
    ...
    # Stop at leftover routing struct. Start at the claimed
    # struct so leftover category / source-status enums stay
    ...
    if STOP_BEFORE_SOURCE_STATUS in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_SOURCE_STATUS}"
        )
    if STOP_BEFORE_BANK in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_BANK}"
        )
    if STOP_BEFORE_PROCEDURAL in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_PROCEDURAL}"
        )
    if STOP_BEFORE_AUDIT in window:
        raise AssertionError(
            f"{STRUCT_NAME} parse window includes {STOP_BEFORE_AUDIT}"
        )
    ...
    for banned in leftover_production_category_tokens():
        if banned in window:
            raise AssertionError(
                f"{STRUCT_NAME} parse window includes {banned}"
            )
...
def leaked_neighbor_tokens() -> tuple[str, ...]:
    return (
        ...
        *leftover_production_category_tokens(),
    )
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*FName\s+ProvenanceId\b", compact[index:]
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for FName ProvenanceId is missing from "
...
class ProductionAudioEntryProvenanceIdFieldDeclContractTests(
    unittest.TestCase
):
    def test_production_audio_entry_struct_exists(self) -> None:
        ...
        self.assertNotEqual(STRUCT_NAME, STOP_BEFORE_BANK)
        body = struct_body(header)
        ...
        self.assertIn(STOP_BEFORE_SOURCE_STATUS, header)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS, section)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS, body)
    ...
    def test_missing_provenance_id_declaration_fails_closed(self) -> None:
        empty = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
    ...
    def test_initializer_or_wrong_type_fails_closed(self) -> None:
        initialized = (
            f"struct {STRUCT_NAME}\n"
            "{\n"
            f"\t{UPROPERTY_EDIT_READWRITE}\n"
        ...
        self.assertIn("FName ProvenanceId;", compact_origin)
        self.assertNotIn("ProvenanceId = NAME_None", compact_origin)
        self.assertNotIn("ProvenanceId = true", compact_origin)
        self.assertNotIn("ProvenanceId = 0.f", compact_origin)
        self.assertNotIn("ProvenanceId = 160.f", compact_origin)

    def test_provenance_id_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("FName ProvenanceId"),
            LOCKED_DECL,
        )
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            leftover_primary,
            leftover_guided,
            f"\tFName {SIBLING_DISPLAY_NAME};\n",
            f"\tFString {SIBLING_SOURCE_SHA256};\n",
            "\tFName ProvenanceIds;\n",
            "\tFName ProvenanceId = NAME_None;\n",
            "\tfloat ProvenanceId = " + forty + ";\n",
            "\tfloat ProvenanceId = " + eighty + ";\n",
        )
    ...
    def test_does_not_parse_leftover_bank_or_neighbors(self) -> None:
        header = origin_main_header()
        section = spec_section(header)
        ...
        self.assertIn(STOP_BEFORE_SOURCE_STATUS, header)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS, section)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS, leaked)
        self.assertNotIn(STOP_BEFORE_SOURCE_STATUS, window)
        self.assertIn(STOP_BEFORE_BANK, header)
        self.assertNotIn(STOP_BEFORE_BANK, section)
        ...
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, header)
        self.assertNotIn(STOP_BEFORE_PROCEDURAL, section)
    ...
    def test_parse_window_excludes_leftover_weapon_enum_body(self) -> None:
        ...
        for leftover in leftover_audio_event_enum_tokens():
            ...
            self.assertNotIn(leftover, leaked)
            self.assertNotIn(leftover, window)
        for leftover in leftover_production_category_tokens():
            self.assertNotIn(leftover, section)
            self.assertNotIn(leftover, leaked)
            self.assertNotIn(leftover, window)
        self.assertNotIn(STOP_BEFORE_AUDIO_EVENT, section)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "production-audio-entry ProvenanceId field "
                "decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "provenance_id_field_decl_contract.py"
        ))
    ...
    def test_contract_is_target_field_declaration_only(self) -> None:
        ...
        self.assertNotIn(SIBLING_SOURCE_SHA256, locked_only)
        for helper in namespace_helper_tokens():
