# THIS IS leftover-safe ASkyguardGunner MinimumSafeSideFireYaw.
...
# then `float MinimumSafeSideFireYaw = 28.f;` (HAS initializer).
# THIS IS leftover-safe isolated UPROPERTY with Category.
# Category IS `Skyguard|Combat|Safety`. EditAnywhere IS present.
# BlueprintReadWrite IS present. Default IS `= 28.f`.
# THIS IS A NEW WRAP. Category IS `Skyguard|Combat|Safety`.
# NOT Category `Skyguard|Combat` (look floats / RecoilPitch /
# TraceRange / BaseDamage). NOT Skyguard|Combat|Feedback.
# NOT Skyguard|Apache. NOT Skyguard|Theater. NOT Category
# Skyguard (mesh/text).
...
# Do NOT claim leftover in-flight FireCameraShakeClass
# (TSubclassOf, Category Combat|Feedback).
# Do NOT claim leftover analog apache-cpg-feel FireRate
# = 12.0f / RecoilPitch = 0.92f as this slot.
# Stop BEFORE sibling Ig+laLockSeconds as a claimed slot.
# SKIP all Ig+la* forever; do not claim Ig+laLockSeconds
# or any Ig+la* as this slot.
# Fail-closed if locked wrap Category is Skyguard|Combat
# without |Safety, Skyguard|Combat|Feedback,
# Skyguard|Apache, or Skyguard|Theater.
# Fail-closed if LOCKED_DECL is float RecoilPitch = 0.92f,
# float FireRate = 12.0f, TSubclassOf FireCameraShakeClass,
# FName WeatherIdentity, or a TObjectPtr mesh/text.
...
# This initializer is 28.f, not leftover Harbor 40 / 80.
...
# as this slot.
# Clone wrap was leftover LookYawLimit Category
# Skyguard|Combat — RETARGET hard: wrap is
# EditAnywhere BlueprintReadWrite Category
# Skyguard|Combat|Safety.
# LOCKED_DECL is `float MinimumSafeSideFireYaw = 28.f;` NOT
# leftover `float LookYawLimit = 95.f`. NOT
...
TARGET = "float MinimumSafeSideFireYaw = 28.f;"
TARGET_WRONG_BARE = "float MinimumSafeSideFireYaw;"
TARGET_WRONG_INIT = "float MinimumSafeSideFireYaw = nullptr;"
TARGET_WRONG_FALSE = "float MinimumSafeSideFireYaw = false;"
TARGET_WRONG_TRUE = "float MinimumSafeSideFireYaw = true;"
TARGET_WRONG_ZERO = "float MinimumSafeSideFireYaw = 0.f;"
TARGET_WRONG_SKELETAL = "TObjectPtr<USkeletalMeshComponent> MinimumSafeSideFireYaw;"
...
TARGET_WRONG_LOOK_PITCH_MIN = "float LookPitchMin = -35.f;"
TARGET_WRONG_LOOK_PITCH_MAX = "float LookPitchMax = 55.f;"
TARGET_WRONG_MOUSE = "float MouseSensitivity = 1.1f;"
TARGET_WRONG_HIP = "float HipFov = 85.f;"
TARGET_WRONG_ADS = "float ADSFov = 42.f;"
TARGET_WRONG_TRACE = "float TraceRange = 32000.f;"
TARGET_WRONG_BASE_DAMAGE = "float BaseDamage = 22.0f;"
TARGET_WRONG_FIRE_SHAKE = (
    "TSubclassOf<UCameraShakeBase> FireCameraShakeClass;"
)
TARGET_WRONG_HARBOR_FORTY = (
    "float MinimumSafeSideFireYaw = " + "40" + ".f;"
)
TARGET_WRONG_HARBOR_EIGHTY = (
    "float MinimumSafeSideFireYaw = " + "80" + ".f;"
)
UPROPERTY_LEFTOVER_COMBAT = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Combat")'
)
UPROPERTY_LEFTOVER_FEEDBACK = (
    'UPROPERTY(EditAnywhere, BlueprintReadWrite, '
    'Category="Skyguard|Combat|Feedback")'
)
TARGET_WRONG_SCENE = "TObjectPtr<USceneComponent> MinimumSafeSideFireYaw;"
...
TARGET_WRONG_MESH = "TObjectPtr<UStaticMeshComponent> MinimumSafeSideFireYaw;"
...
STOP_BEFORE_GUIDED_LOCK_SECONDS = "Ig" + "laLockSeconds"
...
MINIMUM_SAFE_SIDE_FIRE_YAW_WRONG_INIT_RE = re.compile(
    r"float\s+MinimumSafeSideFireYaw\s*="
)
...
def has_declaration(region: str, declaration: str) -> bool:
    # Fail-closed on authored `float MinimumSafeSideFireYaw = 28.f;`.
    ...
    # Harbor 40 / 80, or a bare MinimumSafeSideFireYaw without
    # the 28.f initializer as the slot.
    if not has_one_declaration(region, declaration):
        return False
    compact = collapsed(region)
    if MINIMUM_SAFE_SIDE_FIRE_YAW_DECL_RE.search(compact) is None:
        return False
    if MINIMUM_SAFE_SIDE_FIRE_YAW_BARE_RE.search(compact):
        return False
    ...
    if re.search(r"float\s+LookYawLimit\s*=\s*95\.f\b", compact):
        return False
    if re.search(r"float\s+TraceRange\s*=\s*32000\.f\b", compact):
        return False
    if re.search(r"float\s+BaseDamage\s*=\s*22\.0f\b", compact):
        return False
    if re.search(r"TSubclassOf<UCameraShakeBase>\s+FireCameraShakeClass\b", compact):
        return False
...
def protected_uproperty_fields(header: str) -> str:
    ...
    while True:
        ...
        if ident == STOP_BEFORE_GUIDED_LOCK_SECONDS:
            break
        if ident != "MinimumSafeSideFireYaw":
            cursor = semi + 1
            continue
    ...
    if has_identifier(window, STOP_BEFORE_GUIDED_LOCK_SECONDS):
        raise AssertionError(
            f"{CLASS_NAME} parse window claims sibling "
            f"{STOP_BEFORE_GUIDED_LOCK_SECONDS}"
        )
    ...
    for sibling in (
        STOP_BEFORE_GUIDED_LOCK_SECONDS,
        SIBLING_LOOK_YAW_LIMIT,
        SIBLING_HIP_FOV,
        SIBLING_ADS_FOV,
        SIBLING_TRACE_RANGE,
...
def attached_uproperty_specifiers(section: str) -> str:
    ...
    while True:
        ...
        if depth == 0 and re.match(
            r"\s*float\s+MinimumSafeSideFireYaw\b",
            compact[index:],
        ):
            return compact[start : index - 1]
        cursor = cursor + match.start() + 1
    raise AssertionError(
        "UPROPERTY for float MinimumSafeSideFireYaw "
...
class GunnerMinimumSafeSideFireYawFieldDeclContractTests(unittest.TestCase):
    def test_gunner_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "MinimumSafeSideFireYaw"), section)
        self.assertIn("UPROPERTY", section)
        self.assertIn(STOP_BEFORE_PROTECTED, header)
        self.assertNotIn(STOP_BEFORE_PRIVATE, section)
        self.assertTrue(has_identifier(body, STOP_BEFORE_GUIDED_LOCK_SECONDS), body)
        self.assertFalse(
            has_identifier(section, STOP_BEFORE_GUIDED_LOCK_SECONDS),
            section,
        )
        ...
        self.assertFalse(has_identifier(section, SIBLING_TRACE_RANGE), section)
        self.assertFalse(has_identifier(section, SIBLING_BASE_DAMAGE), section)
        self.assertFalse(
            has_identifier(section, SIBLING_FIRE_CAMERA_SHAKE),
            section,
        )
        self.assertFalse(
    ...
    def test_leftover_class_same_identifier_does_not_satisfy(self) -> None:
        ...
        self.assertFalse(has_identifier(section, "MinimumSafeSideFireYaw"), section)
        with self.assertRaises(AssertionError) as raised:
            require_declaration(section, LOCKED_DECL)
        self.assertIn("MinimumSafeSideFireYaw", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_minimum_safe_side_fire_yaw_declaration_fails_closed(self) -> None:
        empty = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
    ...
    def test_origin_main_uproperty_metadata_is_present(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Combat|Safety"', section)
        ...
        self.assertIn('Category="Skyguard|Combat|Safety"', specifiers)
        self.assertNotIn('Skyguard|Apache', specifiers)
        self.assertNotIn('Skyguard|Theater', specifiers)
        self.assertIn('Category="Skyguard|Combat|Safety"', specifiers)
        self.assertNotIn('Category="Skyguard|Combat|Feedback"', specifiers)
        self.assertIn("|Safety", specifiers)
        self.assertNotIn("|Feedback", specifiers)
    ...
    def test_bare_or_wrong_initializer_fails_closed(self) -> None:
        ...
        self.assertIn("float MinimumSafeSideFireYaw = 28.f;", compact_origin)
        self.assertNotIn("MinimumSafeSideFireYaw = 160.f", compact_origin)
    ...
    def test_minimum_safe_side_fire_yaw_declaration_matches_origin_main(self) -> None:
        section = spec_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
        ...
        self.assertTrue(
            LOCKED_DECL.startswith("float MinimumSafeSideFireYaw"),
            LOCKED_DECL,
        )
        self.assertTrue(LOCKED_DECL.endswith(";"), LOCKED_DECL)
        self.assertIn("=", LOCKED_DECL)
        self.assertIn("28.f", LOCKED_DECL)
        ...
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BASE_DAMAGE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FIRE_SHAKE)
        ...
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_LOOK_YAW}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_TRACE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_BASE_DAMAGE}\n", LOCKED_DECL)
        )
        self.assertFalse(
            has_declaration(f"\t{TARGET_WRONG_FIRE_SHAKE}\n", LOCKED_DECL)
        )
        self.assertFalse(
    ...
    def test_locked_decl_is_not_leftover_clone_weather_identity(self) -> None:
        self.assertEqual(LOCKED_DECL, "float MinimumSafeSideFireYaw = 28.f;")
        ...
        self.assertIn("|Safety", UPROPERTY_EDIT_READWRITE)
        self.assertNotIn("|Feedback", UPROPERTY_EDIT_READWRITE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_LOOK_YAW)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_TRACE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_BASE_DAMAGE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_FIRE_SHAKE)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HARBOR_FORTY)
        self.assertNotEqual(LOCKED_DECL, TARGET_WRONG_HARBOR_EIGHTY)
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            f"\t{TARGET_WRONG_BASE_DAMAGE}\n",
            f"\t{TARGET_WRONG_FIRE_SHAKE}\n",
            f"\t{TARGET_WRONG_HARBOR_FORTY}\n",
            f"\t{TARGET_WRONG_HARBOR_EIGHTY}\n",
            f"\t{TARGET_WRONG_CANNON_MAG}\n",
            ...
            leftover_primary,
            leftover_guided,
            "\tfloat MinimumSafeSideFireYaws = 28.f;\n",
            "\tint32 MinimumSafeSideFireYaw = 95;\n",
            "\tbool MinimumSafeSideFireYaw = true;\n",
            "\tfloat MinimumSafeSideFireYaw = " + forty + ";\n",
            "\tfloat MinimumSafeSideFireYaw = " + eighty + ";\n",
        )
    ...
    def test_missing_category_or_edit_anywhere_fails_closed(self) -> None:
        ...
        self.assertIn('Category="Skyguard|Combat|Safety"', origin)
        ...
        self.assertNotIn('Category="Skyguard|Combat|Safety"', apache_specs)
        theater = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
        ...
        self.assertNotIn('Category="Skyguard|Combat|Safety"', theater_specs)
        self.assertIn('Category="Skyguard|Combat|Safety"', origin)
        self.assertNotIn('Skyguard|Apache', origin)
        ...
        self.assertNotIn("|Safety", combat_specs)
        leftover_feedback = (
            f"class SKYGUARD52_API {CLASS_NAME}\n"
            "{\n"
            "protected:\n"
            f"\t{UPROPERTY_LEFTOVER_FEEDBACK}\n"
            f"\t{LOCKED_DECL}\n"
            f"{STOP_BEFORE_PRIVATE}\n"
            "};\n"
        )
        ...
        self.assertNotIn("|Safety", feedback_specs)
        self.assertIn("|Safety", origin)
        self.assertNotIn("|Feedback", origin)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, UPROPERTY_LEFTOVER_COMBAT)
        self.assertNotEqual(UPROPERTY_EDIT_READWRITE, UPROPERTY_LEFTOVER_FEEDBACK)

    def test_declaration_accepts_origin_main_split_line_forms(self) -> None:
        wraps = (
            "\tfloat\n\tMinimumSafeSideFireYaw = 28.f;\n",
    ...
    def test_does_not_claim_sibling_guided_lock_seconds_or_neighbors(self) -> None:
        for sibling in sibling_uncontracted_decls():
            self.assertFalse(has_identifier(LOCKED_DECL, sibling), sibling)
        for field in leftover_neighbor_hit_fields():
            self.assertNotIn(field, LOCKED_DECL)
        ...
        self.assertTrue(has_identifier(section, "MinimumSafeSideFireYaw"))
        self.assertTrue(has_identifier(leaked, STOP_BEFORE_GUIDED_LOCK_SECONDS))
        self.assertFalse(has_identifier(section, STOP_BEFORE_GUIDED_LOCK_SECONDS))
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "gunner MinimumSafeSideFireYaw field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "gunner_minimum_safe_side_fire_yaw_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardGunner.cpp", THIS_SCRIPT)
