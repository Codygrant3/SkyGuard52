# THIS IS leftover-safe ASkyguardCampaignTheaterKit RoadInstances.
...
# then BARE `TObjectPtr<UHierarchicalInstancedStaticMeshComponent>
# RoadInstances;` (no initializer).
...
# Do NOT contract sibling fields Root / BuildingInstances /
# LampInstances / SilhouetteInstances / NamedLandmarkMesh /
# AppliedSpec.
...
# RoadInstances does not block this isolated UPROPERTY
...
# retarget: type is TObjectPtr HISM, no `=`, Category is
# Skyguard|Theater, parse the actor public section.
...
TARGET = f"{HISM_TYPE} RoadInstances;"
TARGET_WRONG_EQ_NONE = f"{HISM_TYPE} RoadInstances = nullptr;"
TARGET_WRONG_FALSE = f"{HISM_TYPE} RoadInstances = false;"
TARGET_WRONG_TRUE = f"{HISM_TYPE} RoadInstances = true;"
TARGET_WRONG_ZERO = f"{HISM_TYPE} RoadInstances = 0.f;"
TARGET_WRONG_FLOAT = "float RoadInstances;"
TARGET_WRONG_FNAME = "FName RoadInstances;"
TARGET_WRONG_ROOT_TYPE = "TObjectPtr<USceneComponent> RoadInstances;"
...
def has_declaration(region: str, declaration: str) -> bool:
    ...
    # sibling Root / BuildingInstances / LampInstances fields.
    ...
    if re.search(
        ...
        return False
    if re.search(
        r"float\s+RoadInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"FName\s+RoadInstances\b",
        compact,
    ):
        return False
    if re.search(
        r"TObjectPtr<USceneComponent>\s+RoadInstances\b",
        compact,
    ):
        return False
    if re.search(
...
class TheaterKitRoadInstancesFieldDeclContractTests(unittest.TestCase):
    def test_theater_kit_actor_class_exists(self) -> None:
        ...
        self.assertTrue(has_identifier(section, "RoadInstances"), section)
    ...
    def test_spec_or_namespace_declaration_does_not_satisfy(self) -> None:
        ...
        self.assertIn("RoadInstances", str(raised.exception))
        self.assertIn("missing", str(raised.exception).lower())

    def test_missing_road_instances_declaration_fails_closed(self) -> None:
        empty = (
            f"class {ACTOR_NAME}\n"
            "{\n"
            "public:\n"
    ...
    def test_initializer_fails_closed_when_origin_is_bare(self) -> None:
        ...
        self.assertNotIn("RoadInstances = nullptr", compact_origin)
        self.assertNotIn("RoadInstances = NAME_None", compact_origin)
        self.assertNotIn("RoadInstances = false", compact_origin)
        self.assertNotIn("RoadInstances = true", compact_origin)
        self.assertNotIn("RoadInstances = 0.f", compact_origin)
        self.assertNotIn("RoadInstances = 160.f", compact_origin)

    def test_road_instances_declaration_matches_origin_main(self) -> None:
        section = actor_section(origin_main_header())
        self.assertEqual(
            require_declaration(section, LOCKED_DECL),
            LOCKED_DECL,
    ...
    def test_wrong_default_and_renames_do_not_satisfy(self) -> None:
        ...
        wrongs = (
            ...
            "\tTObjectPtr<UHierarchicalInstancedStaticMeshComponent> RoadInstancess;\n",
            "\tint32 RoadInstances;\n",
            "\tbool RoadInstances;\n",
            "\tfloat RoadInstances = " + forty + ";\n",
            "\tfloat RoadInstances = " + eighty + ";\n",
        )
    ...
    def test_does_not_contract_sibling_theater_fields(self) -> None:
        ...
        for spec_field in leftover_spec_field_names():
            self.assertNotIn(spec_field, LOCKED_DECL)
    ...
    def test_this_file_bans_retired_live_copy(self) -> None:
        ...
        for banned in leftover_live_copy_tokens():
            self.assertNotIn(
                banned,
                file_text,
                "theater-kit RoadInstances field decl contract "
    ...
    def test_locked_scripts_do_not_include_this_file(self) -> None:
        ...
        self.assertTrue(Path(__file__).name.endswith(
            "road_instances_field_decl_contract.py"
        ))
        self.assertNotIn("SkyguardCampaignTheaterKit.cpp", THIS_SCRIPT)
