...
PREFIX = "AAA_L83_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L83"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L83"
...
def densify():
    ...
    log("loop83 mat palette size=%d" % len(mats))
    ...
    log("loop83 true-art slice26 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L83", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L83_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop83 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l82_freeze_true_art_slice26_niagara_material_push")
...
def main():
    log("loop83 L82 freeze + true-art slice26 niagara + material push start")
    ...
    log("Loop83 complete stills=%d" % (len(saved) if saved else 0))
