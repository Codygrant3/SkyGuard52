...
PREFIX = "AAA_L35_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L35"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L35"
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L35", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = []
    for name, cam, dist, ny, nz in stages:
        cams.append(("AAA_Cam_L35_%s" % name, cam, (0.0, 0.0, 0.0)))
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop35 stills\n")
...
def main():
    log("loop35 cockpit city mega densify keep prop lock start")
    ...
    log("Loop35 complete stills=%d" % (len(saved) if saved else 0))
