...
PREFIX = "AAA_L57_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L57"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L57"
...
def densify():
    ...
    log("loop57 mat palette size=%d" % len(mats))
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop57 wave04 thin art densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L57", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L57_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop57 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l56_freeze_wave04_luna_max_refined_thin_art_7\n")
...
def main():
    log("loop57 L56 freeze + wave04 max-refined thin art deltas (7 proposals) start")
    ...
    log("Loop57 complete stills=%d" % (len(saved) if saved else 0))
