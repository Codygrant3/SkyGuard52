"""Select one global rig from persistent-capture Recovery03."""
...
def load_module(name: str, path: Path):
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError("could not load selector helpers")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


BASE = load_module("skyguard_attempt03_selector_helpers_r03", BASE_SELECTOR_PATH)
...
def main() -> int:
    ...
    if output.exists():
        raise RuntimeError("immutable Recovery03 selector output already exists")
    ...
    if (
        ...
        raise RuntimeError("Recovery03 persistent pilot did not pass")
    if manifest.get("capture_count") != 72:
        raise RuntimeError("Recovery03 full sweep count differs from contract")
    ...
    for rig in contract["capture"]["rig_candidates"]:
        ...
        if len(records) != 9:
            raise RuntimeError("each Recovery03 rig must contain nine views")
        candidates.append(
            {
                **rig,
                ...
                "mean_penalty": round(
                    sum(record["penalty"] for record in records) / 9.0, 8
                ),
                "records": records,
            }
        )
    ...
    if not eligible:
        report = {
            "schema": "skyguard.m01.hero-grouped-topology-unreal-mapped-attempt03-recovery03-selection.v1",
            "gate": "FAIL_CLOSED_RECOVERY03_NO_GLOBAL_RIG_PASSED_ALL_NINE_HARD_BOUNDS",
    ...
    for record in sorted(
        ...
        if sha256_file(destination) != record["sha256"]:
            raise RuntimeError("Recovery03 canonical copy verification failed")
        reference = references[(record["family"], record["view"])]
        canonical.append(
            {
    ...
    if len(canonical) != 9:
        raise RuntimeError("Recovery03 canonical count differs from contract")
    report = {
        "schema": "skyguard.m01.hero-grouped-topology-unreal-mapped-attempt03-recovery03-selection.v1",
        "gate": "PASS_RECOVERY03_GLOBAL_RIG_SELECTED_AWAITING_ORIGINAL_RESOLUTION_REVIEW",
        "selected_utc": datetime.now(timezone.utc).isoformat(),
        "selected_rig_id": selected["rig_id"],
        "selected_rig": {
            key: selected[key]
            for key in ("rig_index", "key_lux", "fill_lux", "skylight")
        },
        "selection_key": {
