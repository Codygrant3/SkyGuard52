"""Editor-mode D3D12 eight-camera proof for the Recovery01 corridor. Does not save."""
...
HERE = ROOT / "Scripts/ToolchainWave08/m01_visible_environment_stage07b_material_terrain_district_variety01_recovery01_visual_proof01_recovery01"
CONTRACT_PATH = HERE / "stage07b_recovery01_visual_proof01_recovery01_contract.json"
CAMERAS_PATH = ROOT / "Docs/AAA_Review/M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_CAMERAS.json"
ATTEMPT = ROOT / "Saved/BuildAttempts/M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_RECOVERY01_VISUAL_PROOF01_RECOVERY01/attempt_01"
LAUNCHER = ROOT / "Saved/BuildAttempts/M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_RECOVERY01_VISUAL_PROOF01_RECOVERY01/launcher_attempt_01"
...
CONTRACT_ID = "M01-VISIBLE-ENVIRONMENT-STAGE07B-MATERIAL-TERRAIN-DISTRICT-VARIETY01-RECOVERY01-VISUAL-PROOF01-RECOVERY01"
_STATE = None
...
class Proof:
    ...
    def tick(self, _delta: float) -> None:
        ...
        if self.phase == "capture":
            if not self.camera_queue:
                ...
                return
            self.capture_one(self.camera_queue.pop(0))
    ...
    def capture_one(self, spec: dict[str, object]) -> None:
        ...
        actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
            ...
            unreal.Rotator(pitch=rotation["pitch"], yaw=rotation["yaw"], roll=0.0),
        )
        if actor is None:
    ...
    def finish(self, classification: str, error: str | None = None) -> None:
        ...
        payload = {
            "schema": "skyguard.m01-visible-environment-stage07b-recovery01.visual-proof01-recovery01-receipt.v1",
        ...
        elif classification.startswith("PASSED_") and (
            payload["p95_frame_ms"] > 16.7 or payload["max_frame_ms"] > 33.3 or payload["frames_over_50_ms"] != 0 or len(self.captures) != 8
        ):
            payload["classification"] = "FAILED_WITH_EVIDENCE"
