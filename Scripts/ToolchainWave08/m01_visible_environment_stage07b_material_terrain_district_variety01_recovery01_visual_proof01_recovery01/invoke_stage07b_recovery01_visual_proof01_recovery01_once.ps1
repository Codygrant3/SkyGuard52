...
$Capture = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage07b_material_terrain_district_variety01_recovery01_visual_proof01_recovery01\capture_stage07b_recovery01_visual_proof01_recovery01.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage07b_material_terrain_district_variety01_recovery01_visual_proof01_recovery01\stage07b_recovery01_visual_proof01_recovery01_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_RECOVERY01_VISUAL_PROOF01_RECOVERY01\attempt_01'
$LauncherRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_RECOVERY01_VISUAL_PROOF01_RECOVERY01\launcher_attempt_01'
...
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_RECOVERY01_VISUAL_PROOF01_RECOVERY01_TERMINAL_SUPERVISOR.json'
...
if ($OfflineContractTest) {
    ...
    if ($c.runtime.game_flag -ne $false) { throw 'Recovery01 proof must not use -game' }
    ...
    if ($script -notmatch [regex]::Escape('$execCmdArgument = ''-ExecCmds="'' + $execCmdValue + ''"''') -and $script -notmatch 'execCmdArgument = ''-ExecCmds="''') {
        # validated below by explicit construction in launch path
    }
    $gameArgument = "'" + '-' + 'game' + "'"
    ...
    [ordered]@{ classification = 'PASSED_OFFLINE_READY_FOR_SINGLE_STAGE07B_RECOVERY01_VISUAL_PROOF01_RECOVERY01'; unreal_launch_count = 0 } | ConvertTo-Json
    [Environment]::Exit(0)
}

$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage07b-recovery01.visual-proof01-recovery01.supervisor.v1'
