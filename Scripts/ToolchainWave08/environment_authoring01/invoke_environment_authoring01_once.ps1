...
$AttemptRoot = 'D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01\attempt_01'
$ExternalTerminal = 'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_TERMINAL_SUPERVISOR_MANIFEST.json'
$EmergencyReceipt = 'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_EMERGENCY_RECEIPT.jsonl'
$SourceAuthoring = 'D:\Skyguard52\Scripts\ToolchainWave08\environment_authoring01\author_m01_environment_authoring01.py'
$AttemptAuthoring = Join-Path $AttemptRoot 'author_m01_environment_authoring01.py'
...
$OutputMap = 'D:\SG52T08_ENV01\Content\ToolchainWave08\Environment\Lvl_M01_T08_EnvironmentAuthoring01.umap'
...
$ExpectedAuthoringHash = '96ddcf218536de059d643432c3bae9b79333404a7fa418e72e0d905394556241'
...
function Assert-PostflightAuthorities {
    ...
    if ($receiptValue.classification -ne 'PASSED_M01_ENVIRONMENT_AUTHORING01_AUTOMATIC') { throw 'Authoring receipt classification failed' }
    ...
    if (@($receiptValue.saved_assets).Count -ne 1 -or $receiptValue.saved_assets[0] -ne '/Game/ToolchainWave08/Environment/Lvl_M01_T08_EnvironmentAuthoring01') { throw 'Save allowlist evidence failed' }
...
if ($OfflineContractTest) {
    ...
    $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("SkyguardAuthoring01_" + [Guid]::NewGuid().ToString('N'))
    try {
    ...
    Write-Output 'CLASSIFICATION=PASSED_M01_ENVIRONMENT_AUTHORING01_OFFLINE_CONTRACT_TEST'
    exit 0
}
...
try {
    ...
    Write-JsonAtomic $PreflightReceipt ([ordered]@{schema='skyguard.toolchain-wave08.m01-authoring01-preflight.v1';classification='PASSED_READY_FOR_SINGLE_UNREAL_AUTHORING';checked_at_utc=[DateTime]::UtcNow.ToString('o');input_hash=$ExpectedInputHash;output_absent=$true;heavy_process_count=0})
    ...
    $classification='PASSED_M01_ENVIRONMENT_AUTHORING01_AUTOMATIC_AWAITING_VISUAL_PROOF'
    ...
    $terminalValue=[ordered]@{schema='skyguard.toolchain-wave08.m01-authoring01-terminal-supervisor.v1';classification=$classification;started_at_utc=$started.ToString('o');completed_at_utc=[DateTime]::UtcNow.ToString('o');preflight_passed=$preflightPassed;unreal_launch_count=if($null -eq $run){0}else{1};retry_count=0;exit_code=if($null -eq $run){$null}else{$run.ExitCode};exit_code_type=if($null -eq $run){$null}else{$run.ExitCodeType};timed_out=if($null -eq $run){$false}else{$run.TimedOut};failure=$failure;input_hash_after=if(Test-Path -LiteralPath $InputMap){Get-Sha256 $InputMap}else{$null};output_hash=if(Test-Path -LiteralPath $OutputMap){Get-Sha256 $OutputMap}else{$null};artifacts=$artifacts}
    ...
    if($classification -ne 'PASSED_M01_ENVIRONMENT_AUTHORING01_AUTOMATIC_AWAITING_VISUAL_PROOF'){exit 1}
}
