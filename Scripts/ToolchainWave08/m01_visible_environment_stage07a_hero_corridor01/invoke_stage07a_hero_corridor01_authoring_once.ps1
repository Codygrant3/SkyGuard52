...
$InputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage06DistrictCorrection01.umap'
$OutputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage07AHeroCorridor01.umap'
$Destination = 'D:\SG52T08_ENV01\Content\M01\VisibleEnvironmentStage07AHeroCorridor01'
$StandingAuthorization = Join-Path $Root 'Production\standing_heavy_process_authorization.json'
$Author = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage07a_hero_corridor01\author_m01_visible_environment_stage07a_hero_corridor01.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage07a_hero_corridor01\stage07a_hero_corridor01_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE07A_HERO_CORRIDOR01_AUTHORING01\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE07A_HERO_CORRIDOR01_AUTHORING01_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE07A_HERO_CORRIDOR01_AUTHORING01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1800
...
function Assert-Authorities {
    ...
    Assert-FileRecord $contractObject.input 'Stage06 map'
    foreach ($spec in $contractObject.authorities) { Assert-FileRecord $spec 'Stage7A authority' }
    foreach ($path in @($Project, $Editor, $StandingAuthorization, $Author, $Contract)) {
        if (-not [IO.File]::Exists($path)) { throw "Required Stage7A file missing: $path" }
    }
...
if ($OfflineContractTest) {
    ...
    [ordered]@{
        classification = 'PASSED_OFFLINE_READY_FOR_SINGLE_STAGE07A_AUTHORING'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage07a-hero-corridor01.authoring01.supervisor.v1'
...
try {
    ...
    if (-not $process.HasExited) {
        ...
        throw "Stage7A authoring exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit()
    $process.Refresh()
    ...
    if ($state.exit_code -ne 0 -or $state.exit_code_type -ne 'System.Int32') {
        throw "Stage7A authoring returned $($state.exit_code) $($state.exit_code_type)"
    }

    if (-not [IO.File]::Exists($Receipt)) { throw 'Stage7A authoring receipt missing' }
    ...
    $expected = 'PASSED_STAGE07A_HERO_CORRIDOR01_AUTHORING_AWAITING_CHECKPOINT_VISUAL'
    if ($state.receipt_classification -ne $expected) { throw "Stage7A authoring receipt failed: $($state.receipt_classification)" }
    if (-not [IO.File]::Exists($OutputMap)) { throw 'Stage7A output map missing' }
    $contractObject = Get-Content -LiteralPath $Contract -Raw | ConvertFrom-Json
    Assert-FileRecord $contractObject.input 'Stage06 map after authoring'
