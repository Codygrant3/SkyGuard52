...
$InputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage04Recovery03.umap'
$OutputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage05Composition01Recovery01.umap'
$Destination = 'D:\SG52T08_ENV01\Content\M01\VisibleEnvironmentStage05Composition01Recovery01'
$StandingAuthorization = Join-Path $Root 'Production\standing_heavy_process_authorization.json'
$FailureFreeze = Join-Path $Root 'Docs\AAA_Review\M01_VISIBLE_ENVIRONMENT_STAGE05_COMPOSITION01_AUTHORING01_ATTEMPT01_TERMINAL_FREEZE.json'
$Author = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage05_composition01_recovery01\author_m01_visible_environment_stage05_composition01_recovery01.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage05_composition01_recovery01\stage05_composition01_recovery01_authoring_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE05_COMPOSITION01_AUTHORING01_RECOVERY01\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE05_COMPOSITION01_AUTHORING01_RECOVERY01_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE05_COMPOSITION01_AUTHORING01_RECOVERY01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1800
...
$Expected = @{
    ...
    InputMap = @{ Bytes = 978545; Sha256 = '70b0929008acafcd4d2c943e9eba6de02d0533752081e6794494b872966a5c18' }
    StandingAuthorization = @{ Bytes = 2146; Sha256 = '48277d7edd869cc2b841c3993e8f7599bec5291ddf23e9c442b33e09b870c089' }
    FailureFreeze = @{ Bytes = 3809; Sha256 = 'da6c24bbf756ee5c984f566737f8f3a757a9f73ce834d86cf7d1d8011bb7f137' }
    Author = @{ Bytes = 3702; Sha256 = '42eef671fa5ee0f003cec0a2472e60e9acc8bb294c1329ca4eba8408a1e665ea' }
    Contract = @{ Bytes = 2043; Sha256 = '95f9f2b35a1cb4deb1f44101c5d01424817750ccfb7a1eae3f27d11ad138e9fd' }
}
...
function Assert-Authorities {
    ...
    Assert-File $InputMap $Expected.InputMap.Bytes $Expected.InputMap.Sha256 'Immutable Stage04 map'
    ...
    Assert-File $FailureFreeze $Expected.FailureFreeze.Bytes $Expected.FailureFreeze.Sha256 'Immutable failed Stage05 freeze'
...
if ($OfflineContractTest) {
    ...
    [ordered]@{
        classification = 'PASSED_OFFLINE_CONTRACT_READY_FOR_SINGLE_STAGE05_RECOVERY01_UNREAL_AUTHORING'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage05-composition01-recovery01.authoring01.supervisor.v1'
...
try {
    ...
    Copy-Item -LiteralPath $Author -Destination (Join-Path $AttemptRoot 'author_m01_visible_environment_stage05_composition01_recovery01.py')
    Copy-Item -LiteralPath $Contract -Destination (Join-Path $AttemptRoot 'stage05_composition01_recovery01_authoring_contract.json')
    ...
    if (-not $process.HasExited) {
        ...
        throw "Unreal Stage05 Recovery01 authoring exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit(); $process.Refresh()
    $state.exit_code = [int]$process.ExitCode
    $state.exit_code_type = $process.ExitCode.GetType().FullName
    if ($state.exit_code -ne 0) { throw "Unreal Stage05 Recovery01 authoring returned $($state.exit_code)" }
    if ($state.exit_code_type -ne 'System.Int32') { throw "Unexpected exit-code type: $($state.exit_code_type)" }
    if (-not [System.IO.File]::Exists($Receipt)) { throw "Stage05 Recovery01 receipt missing: $Receipt" }
    ...
    $expectedClassification = 'PASSED_STAGE05_COMPOSITION01_AUTHORING_AWAITING_POSTFLIGHT_AND_D3D12_PROOF'
    if ($state.receipt_classification -ne $expectedClassification) { throw "Receipt failed: $($state.receipt_classification)" }
    if (-not [System.IO.File]::Exists($OutputMap)) { throw 'Stage05 Recovery01 output map missing' }
    Assert-File $InputMap $Expected.InputMap.Bytes $Expected.InputMap.Sha256 'Stage04 map after Stage05 Recovery01 authoring'
