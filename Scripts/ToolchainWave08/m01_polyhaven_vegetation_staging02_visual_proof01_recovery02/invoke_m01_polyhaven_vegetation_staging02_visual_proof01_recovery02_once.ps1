...
# The Stage02 wrapper exposes the fully transformed proven supervisor only after
# an offline invocation. This invocation cannot create governed proof namespaces.
...
$replacements = @(
    @('PASSED_READY_FOR_STANDING_AUTHORIZED_SINGLE_M01_POLYHAVEN_VEGETATION_STAGING02_VISUAL_PROOF01_EXECUTION', 'PASSED_READY_FOR_STANDING_AUTHORIZED_SINGLE_M01_POLYHAVEN_VEGETATION_STAGING02_VISUAL_PROOF01_RECOVERY02_EXECUTION'),
    ...
    @('m01_polyhaven_vegetation_staging02_visual_proof01', 'm01_polyhaven_vegetation_staging02_visual_proof01_recovery02'),
    @('polyhaven-vegetation-staging02-visual-proof01', 'polyhaven-vegetation-staging02-visual-proof01-recovery02')
)
foreach ($pair in $replacements) {
    if (-not $transformed.Contains($pair[0])) { throw "Recovery02 binding token is absent: $($pair[0])" }
    $transformed = $transformed.Replace($pair[0], $pair[1])
}
if ($transformed.Contains('RECOVERY02_RECOVERY02') -or $transformed.Contains('recovery02_recovery02')) { throw 'Recovery02 duplicate suffix detected' }
if ([regex]::Matches($transformed, [regex]::Escape('Start-Process -FilePath $editor')).Count -ne 1) { throw 'Recovery02 supervisor must contain exactly one Unreal launch path' }
