...
CONTRACT_PATH = ROOT / "Scripts/ToolchainWave08/m01_polyhaven_vegetation_staging01/vegetation_staging01_contract.json"
ATTEMPT = ROOT / "Saved/BuildAttempts/M01_POLYHAVEN_VEGETATION_STAGING01/attempt_01"
...
def offline_contract_test() -> int:
    ...
    print("PASS_M01_POLYHAVEN_VEGETATION_STAGING01_OFFLINE_CONTRACT")
    return 0
...
def import_candidate(unreal: object, spec: dict[str, object], root_asset: str) -> tuple[object, dict[str, object]]:
    ...
    mesh_pipeline.set_editor_property("generate_lightmap_uvs", True)
...
def run_unreal() -> None:
    ...
    result: dict[str, object] = {
        "schema": "skyguard.m01-polyhaven-vegetation-staging01.authoring-receipt.v1",
    ...
    try:
        ...
        for spec in contract["assets"]:
            ...
            for index, (x_cm, y_cm, yaw, scale) in enumerate(placements, start=1):
                ...
                actor.set_folder_path(f"M01/SourceBacked/VegetationStaging01/{asset_id}")
