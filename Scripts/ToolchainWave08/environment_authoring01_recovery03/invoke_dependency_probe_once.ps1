...
$ScriptRoot = Join-Path $ProjectRoot 'Scripts\ToolchainWave08\environment_authoring01_recovery03'
...
$RecoveryOutput = Join-Path $IsolatedRoot 'Content\ToolchainWave08\Environment\Lvl_M01_T08_EnvironmentAuthoring01_Recovery03.umap'
...
function Get-TemporaryTestRoot([string]$Kind) {
    $base = Join-Path ([System.IO.Path]::GetTempPath()) 'Skyguard52\ToolchainWave08\EnvironmentAuthoring01Recovery03'
...
function Invoke-OfflineContractTest {
    ...
    $result = [ordered]@{schema='skyguard.toolchain-wave08.m01-authoring01-recovery03.offline-test.v1';classification=if($failures.Count-eq0){'PASS'}else{'FAIL'};test_root=$testRoot;success_terminal=$successTerminal;failure_terminal=$failureTerminal;emergency_receipt=$emergency;unreal_launch_count=0;retry_count=0;governed_before=$governedBefore;governed_after=$governedAfter;failures=$failures}
...
$Attempt = 'D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY03_DEPENDENCY_PROBE\attempt_01'
$Terminal = 'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY03_DEPENDENCY_PROBE_TERMINAL_MANIFEST.json'
$Emergency = 'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY03_DEPENDENCY_PROBE_EMERGENCY_RECEIPT.jsonl'
$FutureAuthoringAttempt = 'D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY03\attempt_01'
...
try {
    ...
    try {
        ...
        $terminalValue=[ordered]@{schema='skyguard.toolchain-wave08.m01-authoring01-recovery03.dependency-probe-terminal.v1';classification=$classification;started_at_utc=$started.ToString('o');completed_at_utc=[DateTime]::UtcNow.ToString('o');preflight_passed=$preflightPassed;unreal_launch_count=if($null-eq$run){0}else{1};retry_count=0;exit_code=if($null-eq$run){$null}else{$run.ExitCode};exit_code_type=if($null-eq$run){$null}else{$run.ExitCodeType};timed_out=if($null-eq$run){$false}else{$run.TimedOut};failure=$failure;input_hash_after=if(Test-Path -LiteralPath $InputMap){Get-Sha256 $InputMap}else{$null};original_output_exists=(Test-Path -LiteralPath $OriginalOutput);recovery_output_exists=(Test-Path -LiteralPath $RecoveryOutput);artifacts=$artifacts}
