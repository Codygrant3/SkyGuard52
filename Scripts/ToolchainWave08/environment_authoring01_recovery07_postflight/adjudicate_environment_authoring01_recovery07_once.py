...
ATTEMPT = ROOT / r"Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07\attempt_01"
MANIFEST = REPORTS / "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_TERMINAL_SUPERVISOR_MANIFEST.json"
EMERGENCY = REPORTS / "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_EMERGENCY_RECEIPT.jsonl"
...
EXECUTION_BINDING = AAA / "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_EXECUTION_PROMPT_BINDING_FREEZE.json"
...
POSTFLIGHT = REPORTS / "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_ATTEMPT01_POSTFLIGHT.json"
ADDENDUM = AAA / "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_ATTEMPT01_ADDENDUM_2026-08-08.md"
PHASE_ADDENDUM = AAA / "PHASE1_8_COMPLETION_AUDIT_ADDENDUM_TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_ATTEMPT01_2026-08-08.md"
FREEZE = AAA / "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_ATTEMPT01_TERMINAL_FREEZE.json"
NEXT_SUCCESS = AAA / "NEXT_PROMPT_TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF_OFFLINE_DESIGN.md"
NEXT_FAILURE = AAA / "NEXT_PROMPT_TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY08_OFFLINE_CORRECTION.md"

EXPECTED_OFFLINE_FREEZE = "602ba028acb18218ee8596a5a8813085abb65e8b9405adb1b7ff1f848edf44ef"
EXPECTED_INPUT = "5c8b7c6b0c6024767fa8a671523b5626213bd029d591270654080aed5fc7ced4"
SUCCESS_MANIFEST = "PASSED_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_AUTOMATIC_AWAITING_VISUAL_PROOF"
SUCCESS_RECEIPT = "PASSED_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_AUTOMATIC"
OUTPUT_ASSET = "/Game/ToolchainWave08/Environment/Lvl_M01_T08_EnvironmentAuthoring01_Recovery07"
...
def verify_execution_binding() -> None:
    require(EXECUTION_BINDING.is_file(), "Recovery07 execution binding is missing")
    binding = json.loads(EXECUTION_BINDING.read_text(encoding="utf-8-sig"))
    require(
        binding.get("classification") == "PASSED_READY_FOR_EXPLICIT_SINGLE_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_AUTHORIZATION",
        "Recovery07 execution binding classification mismatch",
    )
    members = binding.get("members") or []
    require(binding.get("member_count") == len(members), "Recovery07 execution binding member count mismatch")
    for expected in members:
        path = Path(expected.get("path", ""))
        require(path.is_file(), f"Recovery07 execution binding member missing: {path}")
        actual = record(path)
        require(actual["bytes"] == expected.get("bytes"), f"Recovery07 binding member byte mismatch: {path}")
        require(actual["sha256"] == expected.get("sha256"), f"Recovery07 binding member hash mismatch: {path}")
...
def failure_message(manifest: dict, receipt: dict | None) -> str:
    ...
    return "Governed Recovery07 execution failed without a detailed failure message"
...
def evaluate(manifest: dict, receipt: dict | None, *, input_hash: str, output_exists: bool, output_hash: str | None) -> dict:
    ...
    if not success:
        ...
        return {
            ...
            "next_gate": "OFFLINE_ONLY_RECOVERY08_CORRECTION_DESIGN",
        }
...
def run_offline_contract_test():
    ...
    print("CLASSIFICATION=PASSED_RECOVERY07_POSTFLIGHT_OFFLINE_CONTRACT_TEST")
...
def missing_manifest_outcome() -> dict:
    return {
        "classification": "FAILED_WITH_EVIDENCE",
        "failure": "Recovery07 terminal supervisor manifest is missing",
        "exit_code": None,
...
def freeze_missing_manifest() -> None:
    outcome = missing_manifest_outcome()
    integrity_failures = ["terminal supervisor manifest missing"]
    postflight = {
        "schema": "skyguard.toolchain-wave08.m01-authoring01-recovery07.attempt01-postflight.v1",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        **outcome,
        "unreal_launch_count": None,
        "retry_count": None,
        "exit_code_type": None,
        "timed_out": None,
    ...
    next_text = (
        "Resume only D:\\Skyguard52. Perform one offline-only Recovery08 correction design using the immutable "
        "Recovery07 missing-terminal-manifest evidence. Do not launch Unreal or Blender. Preserve the accepted "
        "input, the failed attempt, any emergency receipt, and any partial output. Diagnose only the terminal "
        "evidence lifecycle, create one bounded fresh-namespace correction, validate it offline, and stop after "
        "a separate one-shot execution prompt.\n"
    )
    write_new(NEXT_FAILURE, next_text)
    addendum = f"""# Toolchain Wave08 Mission 1 Environment Authoring01 Recovery07 Attempt01

Date: 2026-08-08

Classification: `FAILED_WITH_EVIDENCE`

Recovery07 ended without its required terminal supervisor manifest. Available attempt, emergency, and partial-output evidence was preserved without retry.

- Unreal launches: `unavailable`
- Retries: `unavailable`
...
- Next gate: `OFFLINE_ONLY_RECOVERY08_CORRECTION_DESIGN`
"""
    write_new(ADDENDUM, addendum)
    write_new(PHASE_ADDENDUM, "# Phase 1-8 Completion Audit Addendum: Recovery07 Attempt01\n\n" + addendum.split("\n", 1)[1])
    attempt_files = sorted(path for path in ATTEMPT.rglob("*") if path.is_file()) if ATTEMPT.is_dir() else []
    freeze_paths = [OFFLINE_FREEZE, EXECUTION_BINDING, *attempt_files, POSTFLIGHT, ADDENDUM, PHASE_ADDENDUM, INPUT_MAP, NEXT_FAILURE]
    if EMERGENCY.is_file():
        freeze_paths.append(EMERGENCY)
    ...
    for path in freeze_paths:
        ...
        if key not in seen and path.is_file():
            ...
            unique.append(path)
    terminal = {
        "schema": "skyguard.toolchain-wave08.m01-authoring01-recovery07.attempt01-terminal-freeze.v1",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        **outcome,
    ...
    print(json.dumps({"classification": terminal["classification"], "freeze": record(FREEZE), "member_count": terminal["member_count"], "next_prompt": record(NEXT_FAILURE)}, indent=2))
...
def main():
    ...
    require(digest(OFFLINE_FREEZE) == EXPECTED_OFFLINE_FREEZE, "Recovery07 offline freeze mismatch")
    ...
    if not MANIFEST.is_file():
        ...
        return
    require(MANIFEST.is_file(), "Recovery07 terminal supervisor manifest is missing")
    ...
    try:
        ...
        outcome = {
            "classification": "FAILED_WITH_EVIDENCE",
            "failure": f"Recovery07 postflight contract violation: {exc}",
            "exit_code": manifest.get("exit_code"),
            "output_map_exists": OUTPUT_MAP.is_file(),
            "output_sha256": digest(OUTPUT_MAP) if OUTPUT_MAP.is_file() else None,
            "receipt_present": receipt is not None,
    ...
    if integrity_failures:
        outcome = {**outcome, "classification": "FAILED_WITH_EVIDENCE", "failure": "; ".join(integrity_failures), "next_gate": "OFFLINE_ONLY_RECOVERY08_CORRECTION_DESIGN"}
    ...
    if outcome["classification"] == SUCCESS_MANIFEST:
        ...
        next_text = """Resume only D:\\Skyguard52. Perform one offline-only design gate for a mapped, full-resolution Mission 1 visual proof of the accepted Recovery07 environment output. Launch no Unreal or Blender. Verify the Recovery07 terminal freeze and output map, freeze rear-gunner gameplay and exterior cameras, visual and temporal rubrics, shader readiness, performance limits, evidence requirements, one-heavy-process and one-attempt rules, and create a separate one-shot Unreal proof prompt. Classify PASSED_READY_FOR_EXPLICIT_SINGLE_RECOVERY07_MAPPED_VISUAL_PROOF_AUTHORIZATION or FAILED_WITH_EVIDENCE.\n"""
        status = "Automatic Recovery07 authoring passed. Full-resolution visual acceptance remains unproven."
    else:
        next_path = NEXT_FAILURE
        next_text = f"""Resume only D:\\Skyguard52. Perform one offline-only Recovery08 correction design using the immutable Recovery07 failure evidence. Do not launch Unreal or Blender. Diagnose only this exact failure: {outcome['failure']!r}. Preserve the accepted input and every failed attempt, create a fresh Recovery08 namespace and one bounded correction, validate it offline, and stop after a separate one-shot execution prompt.\n"""
        status = f"Automatic Recovery07 authoring failed with immutable evidence: {outcome['failure']}"
...
"""
    ...
    freeze_paths = [OFFLINE_FREEZE, EXECUTION_BINDING, MANIFEST, *artifact_paths, POSTFLIGHT, ADDENDUM, PHASE_ADDENDUM, INPUT_MAP, next_path, *crash_evidence(manifest)]
    if RECEIPT.is_file() and RECEIPT not in freeze_paths:
        freeze_paths.append(RECEIPT)
    if OUTPUT_MAP.is_file():
        freeze_paths.append(OUTPUT_MAP)
