"""Bind the frozen Stage02 adjudicator to fresh Recovery04 evidence paths."""
...
REPLACEMENTS=(("M01_POLYHAVEN_VEGETATION_STAGING02_VISUAL_PROOF01","M01_POLYHAVEN_VEGETATION_STAGING02_VISUAL_PROOF01_RECOVERY04"),("M01-POLYHAVEN-VEGETATION-STAGING02-VISUAL-PROOF01","M01-POLYHAVEN-VEGETATION-STAGING02-VISUAL-PROOF01-RECOVERY04"),("M01PolyHavenVegetationStaging02VisualProof01.csv","M01PolyHavenVegetationStaging02VisualProof01Recovery04.csv"),("m01_polyhaven_vegetation_staging02_visual_proof01","m01_polyhaven_vegetation_staging02_visual_proof01_recovery04"),("polyhaven-vegetation-staging02-visual-proof01","polyhaven-vegetation-staging02-visual-proof01-recovery04"))
...
def transform_source()->str:
    ...
    n={"__name__":"stage02_adjudicator_binder_authority","__file__":str(SOURCE)};exec(compile(SOURCE.read_text(encoding="utf-8"),str(SOURCE),"exec"),n,n);s=n["transform_source"]()
    for old,new in REPLACEMENTS:
        if old not in s:raise RuntimeError(f"Recovery04 adjudicator token absent: {old}")
        s=s.replace(old,new)
    return s
