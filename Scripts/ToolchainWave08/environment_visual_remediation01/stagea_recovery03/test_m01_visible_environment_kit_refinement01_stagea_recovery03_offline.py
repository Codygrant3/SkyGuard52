...
VERIFIER = ROOT / "Scripts/ToolchainWave08/environment_visual_remediation01/stagea_recovery03/verify_m01_visible_environment_kit_refinement01_stagea_recovery03_offline.py"
...
verifier = load(VERIFIER, "stagea_recovery03_verifier")
wrapper = load(WRAPPER, "stagea_recovery03_bound_worker_test")


class StageARecovery03OfflineTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_failed_authorities_preserved_and_recovery03_fresh(self) -> None:
        self.assertEqual(13, verifier.verify_recovery01_freeze())
        self.assertEqual(4, verifier.verify_recovery02_terminal_freeze())
        self.assertFalse(any(path.exists() for path in verifier.FUTURE))

    def test_invalid_offline_invocation_cannot_write_governed_terminal(self) -> None:
        text = verifier.SUPERVISOR.read_text(encoding="utf-8")
        self.assertIn("$hasOfflineEvidenceRoot", text)
        self.assertIn("$writeTerminal = ($hasOfflineEvidenceRoot -or $AuthorizeSingleBlender)", text)
