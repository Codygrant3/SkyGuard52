...
OUTPUT = ROOT / r"Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery08_Checkpoint01"
SUPERVISOR_TERMINAL = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY08_CHECKPOINT01_TERMINAL_SUPERVISOR.json"
POSTFLIGHT = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY08_CHECKPOINT01_POSTFLIGHT.json"
WORKER = ROOT / r"Scripts\ToolchainWave08\environment_visual_remediation01\stagea_recovery08\build_m01_visible_environment_kit_refinement01_stagea_recovery08_checkpoint01.py"
WORKER_BYTES = 10929
WORKER_SHA256 = "0e7a1ffa28477a3dd94b326e1c31a56833e37bd2952df8b5eb87145f4b2193dc"
...
def main() -> int:
    ...
    require(WORKER.is_file() and WORKER.stat().st_size == WORKER_BYTES and sha256(WORKER) == WORKER_SHA256, "Worker authority drift")
    ...
    require(supervisor.get("classification") == "PASSED_AUTOMATIC_AWAITING_MANDATORY_POSTFLIGHT_AND_DIRECT_VISUAL_REVIEW", "Supervisor did not reach automatic checkpoint success")
    require(supervisor.get("exit_code") == 0 and supervisor.get("exit_code_type") == "System.Int32", "Blender exit-code authority failed")
    require(supervisor.get("blender_launch_count") == 1 and supervisor.get("retry_count") == 0 and supervisor.get("unreal_launch_count") == 0, "Launch-count contract failed")
    ...
    required_receipts = ["dimension_receipt.json", "topology_uv_receipt.json", "checkpoint_receipt.json", "source_parity_receipt.json", "artifact_inventory.json", "terminal_receipt.json"]
    receipts = {name: json.loads((OUTPUT / name).read_text(encoding="utf-8-sig")) for name in required_receipts}
    ...
    require(counts == {"buildings": 5, "vehicles": 8, "trees": 10, "streetlights": 10, "puddles": 6, "review_ocean": 1, "review_surf_foam": 1}, "Structural-count receipt failed")
    checkpoint_receipt = receipts["checkpoint_receipt.json"]
    require(checkpoint_receipt.get("passed") is True and checkpoint_receipt.get("count") == 9, "Checkpoint receipt failed")
    ...
    require(any(len(entry.get("calibration_passes", [])) > 0 for entry in night_entries), "No governed night calibration was recorded")
    for entry in night_entries:
        metrics = entry.get("metrics", {})
        require(float(metrics.get("mean_luma_linear", 0.0)) >= 0.008, f"Night mean luminance failed: {entry.get('camera')}")
        require(float(metrics.get("black_fraction_linear_0_01", 1.0)) <= 0.70, f"Night black fraction failed: {entry.get('camera')}")
        require(len(entry.get("calibration_passes", [])) <= 2, f"Night calibration pass limit failed: {entry.get('camera')}")
        require(float(entry.get("exposure", 99.0)) <= 7.0, f"Night exposure ceiling failed: {entry.get('camera')}")
    require(receipts["source_parity_receipt.json"].get("passed") is True and receipts["source_parity_receipt.json"].get("sha256") == WORKER_SHA256, "Source-parity receipt failed")
    ...
    require(terminal.get("status") == "CHECKPOINT_COMPLETED_AWAITING_DIRECT_FULL_RESOLUTION_VISUAL_REVIEW", "Worker terminal status failed")
    ...
    payload = {
        "schema": "skyguard.m01-visible-environment-kit-refinement01-stagea-recovery08-checkpoint01.postflight.v1",
        "gate": GATE,
        ...
        "night_calibration": {
            ...
            "total_calibration_passes": sum(len(entry.get("calibration_passes", [])) for entry in night_entries),
            "minimum_final_mean_luma_linear": min(float(entry["metrics"]["mean_luma_linear"]) for entry in night_entries),
            "maximum_final_black_fraction_linear_0_01": max(float(entry["metrics"]["black_fraction_linear_0_01"]) for entry in night_entries),
        ...
        "recovery07_output_geometry_reused": False,
