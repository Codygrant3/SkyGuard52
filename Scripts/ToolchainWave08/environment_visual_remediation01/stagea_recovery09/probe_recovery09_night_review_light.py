...
def load_generated_source() -> str:
    spec = importlib.util.spec_from_file_location("skyguard_r08_probe_source", R08_WORKER)
    if spec is None or spec.loader is None:
        raise RuntimeError("Could not load the frozen Recovery08 worker")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    source, _ = module.load_recovery08_source()
    return source
...
def main() -> int:
    ...
    namespace: dict[str, object] = {
        "__name__": "skyguard_recovery09_night_light_probe_embedded",
    ...
    root = namespace["collection"]("NONGOVERNED_RECOVERY09_NIGHT_LIGHT_PROBE")
    ...
    for index, (center_x, floors, style) in enumerate(specs, 1):
        namespace["build_midrise"](f"SM_M01_STAGEA_R05_Midrise_{style}_{index:02d}", center_x, floors, style, materials, district, collision, sockets)
    rig = namespace["add_review_rig"](scene)
    namespace["configure_condition"](scene, rig, "night", materials)
    ...
    candidates = (
        {"id": "A", "fill_energy": 2500.0, "moon_energy": 2500.0, "sun_energy": 0.45, "exposure": 1.5},
        {"id": "B", "fill_energy": 5000.0, "moon_energy": 4000.0, "sun_energy": 0.65, "exposure": 1.5},
        {"id": "C", "fill_energy": 5000.0, "moon_energy": 4000.0, "sun_energy": 0.65, "exposure": 2.5},
    )
    ...
    street_location, street_target, street_lens = cameras["street_close"]
    selected = None
    for candidate in candidates:
        namespace["point_camera"](rig["camera"], street_location, street_target, street_lens)
        apply_candidate(candidate, street_location, street_target)
        path = output / f"candidate_{candidate['id']}_street_close.png"
        metrics = namespace["render_and_measure"](scene, path)
        passed = metrics["mean_luma_linear"] >= 0.012 and metrics["black_fraction_linear_0_01"] <= 0.65
        result = {"candidate": candidate, "camera": "street_close", "metrics": metrics, "passed": passed, "path": str(path)}
        results.append(result)
        print(json.dumps(result, sort_keys=True), flush=True)
        if passed and selected is None:
            selected = candidate
    if selected is None:
        raise RuntimeError("No targeted-night-light candidate passed the probe bounds")

    for camera_id in ("coastal_route", "district_aerial"):
        location, target, lens = cameras[camera_id]
        namespace["point_camera"](rig["camera"], location, target, lens)
        apply_candidate(selected, location, target)
        path = output / f"selected_{selected['id']}_{camera_id}.png"
        metrics = namespace["render_and_measure"](scene, path)
        passed = metrics["mean_luma_linear"] >= 0.008 and metrics["black_fraction_linear_0_01"] <= 0.70
        result = {"candidate": selected, "camera": camera_id, "metrics": metrics, "passed": passed, "path": str(path)}
        results.append(result)
        print(json.dumps(result, sort_keys=True), flush=True)
        if not passed:
            raise RuntimeError(f"Selected candidate failed camera validation: {camera_id}")

    receipt = {
        "schema": "skyguard.m01-visible-environment-kit-refinement01-stagea-recovery09.night-review-light-probe.v1",
        "classification": "PASS",
    ...
    (output / "probe_receipt.json").write_text(json.dumps(receipt, indent=2) + "\n", encoding="utf-8", newline="\n")
