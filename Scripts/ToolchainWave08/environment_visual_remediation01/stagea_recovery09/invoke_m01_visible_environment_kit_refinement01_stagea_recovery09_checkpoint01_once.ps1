...
$Contract = Join-Path $Root 'Docs\Toolchain\ToolchainWave08\EnvironmentVisibleKitRefinement01StageARecovery09\execution_contract.json'
$Worker = Join-Path $Root 'Scripts\ToolchainWave08\environment_visual_remediation01\stagea_recovery09\build_m01_visible_environment_kit_refinement01_stagea_recovery09_checkpoint01.py'
$R08Freeze = Join-Path $Root 'Docs\AAA_Review\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY08_CHECKPOINT01_ATTEMPT01_TERMINAL_FREEZE.json'
$ProbeResult = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY09_NIGHT_REVIEW_LIGHT_PROBE_RESULT.json'
$Blender = 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY09_CHECKPOINT01\attempt_01'
$OutputRoot = Join-Path $Root 'Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery09_Checkpoint01'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY09_CHECKPOINT01_TERMINAL_SUPERVISOR.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY09_CHECKPOINT01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 3600
...
function Write-TerminalEvidence([string]$ManifestPath, [string]$EmergencyPath, $State) {
    try {
        ...
        try {
            ...
            if (-not (Test-Path -LiteralPath $parent)) { [IO.Directory]::CreateDirectory($parent) | Out-Null }
            $line = [ordered]@{
                schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery09-checkpoint01.emergency.v1'
...
function Get-PngDimensions([string]$Path) {
    ...
    for ($index = 0; $index -lt 8; $index++) { if ($bytes[$index] -ne $signature[$index]) { throw "Invalid PNG signature: $Path" } }
    if ([Text.Encoding]::ASCII.GetString($bytes, 12, 4) -ne 'IHDR') { throw "Missing PNG IHDR: $Path" }
...
function Assert-PngSet($Files, [int]$Width, [int]$Height, [string]$Label) {
    foreach ($file in @($Files)) {
        ...
        if ($dimensions.width -ne $Width -or $dimensions.height -ne $Height) { throw "$Label dimensions failed: $($file.FullName)" }
    }
}
...
function Assert-Authorities {
    Assert-File $R08Freeze 4327 'e5c23597ccc5090a2bde922672244cdb70bdf51b21b75e9b3bcabc954a4d0e27'
    $freeze = Get-Content -LiteralPath $R08Freeze -Raw | ConvertFrom-Json
    if ($freeze.classification -ne 'FAILED_WITH_EVIDENCE' -or [int]$freeze.member_count -ne 13 -or [int]$freeze.verified_members -ne 13) { throw 'Recovery08 terminal freeze drift' }
    foreach ($member in @($freeze.members)) { Assert-File ([string]$member.path) ([long]$member.bytes) ([string]$member.sha256) }
    Assert-File $Contract 4423 '49f1641687678842b60050e221a4ec0b99d9882ca9b004135bea3f9d2e8522c3'
    Assert-File $Worker 11322 '0b8baf2524a1f864acdc84bb6c1ab83da45432d42fa84d8ad300fdb9aea30cab'
    Assert-File $ProbeResult 1526 'd40869855977446a54cc3be97ce38921252e3dfbc343ccb68eb1a6ef17a1bdb8'
    ...
    if ($contractData.targeted_night_review_light_contract.probe_candidate -ne 'A' -or [double]$contractData.targeted_night_review_light_contract.fill_energy -ne 2500.0 -or [double]$contractData.targeted_night_review_light_contract.moon_energy -ne 2500.0 -or -not [bool]$contractData.targeted_night_review_light_contract.light_direction_recomputed_per_camera) { throw 'Targeted night review-light contract drift' }
    if ([int]$contractData.night_calibration_contract.maximum_calibration_passes_per_night_camera -ne 2 -or [double]$contractData.night_calibration_contract.exposure_hard_ceiling -ne 7.0) { throw 'Night calibration contract drift' }
}
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery09-checkpoint01.supervisor-terminal.v1'
    gate = $Gate
    classification = 'FAILED_WITH_EVIDENCE'
    terminal = $false
...
try {
    ...
    if ($OfflineContractTest) {
        ...
        foreach ($token in @('for calibration_index in range(2):','new_exposure = min(7.0','"night_lighting_calibration"','"night_review_lighting_aimed"','recovery08_attempt_or_output_reused":False')) {
        ...
        $state.receipt_states = [ordered]@{ recovery08_preserved = $true; targeted_night_review_lighting = $true; bounded_night_calibration = $true; checkpoint_only = $true }
        $exit = 0
    }
    else {
        ...
        $attemptSource = Join-Path $AttemptRoot 'source\build_m01_visible_environment_kit_refinement01_stagea_recovery09_checkpoint01.py'
        [IO.File]::Copy($Worker, $attemptSource, $false)
        [IO.File]::Copy($Contract, (Join-Path $AttemptRoot 'execution_contract.json'), $false)
        Assert-File $attemptSource 11322 '0b8baf2524a1f864acdc84bb6c1ab83da45432d42fa84d8ad300fdb9aea30cab'
        Write-JsonAtomic (Join-Path $AttemptRoot 'preflight_receipt.json') ([ordered]@{
            schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery09-checkpoint01.preflight.v1'
            at_utc = [DateTime]::UtcNow.ToString('o')
            recovery08_terminal_freeze_sha256 = 'e5c23597ccc5090a2bde922672244cdb70bdf51b21b75e9b3bcabc954a4d0e27'
            recovery08_terminal_members_verified = 13
            recovery_source_sha256 = '0b8baf2524a1f864acdc84bb6c1ab83da45432d42fa84d8ad300fdb9aea30cab'
        ...
        $arguments = @('--background','--factory-startup','--python',$attemptSource,'--','--output',$OutputRoot,'--asset-id','m01-visible-environment-kit-refinement01-stagea','--expected-source-sha256','0b8baf2524a1f864acdc84bb6c1ab83da45432d42fa84d8ad300fdb9aea30cab')
        ...
        if (-not (Test-Path -LiteralPath $OutputRoot)) { throw 'Blender did not create the Recovery09 output namespace' }
        ...
        $state.receipt_states = [ordered]@{
            ...
            source_parity = ([bool]$sourceParity.passed -and [string]$sourceParity.sha256 -eq '0b8baf2524a1f864acdc84bb6c1ab83da45432d42fa84d8ad300fdb9aea30cab')
