...
MODULE_PATH = Path(__file__).with_name("adjudicate_m01_visible_environment_kit_refinement01_stagea_recovery04.py")
SPEC = importlib.util.spec_from_file_location("stagea_recovery04_postflight", MODULE_PATH)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC and SPEC.loader
SPEC.loader.exec_module(MODULE)


class Recovery04PostflightTests(unittest.TestCase):
    ...
    def test_complete_fixture_passes(self):
        with tempfile.TemporaryDirectory(prefix="skyguard_stagea_recovery04_postflight_test_") as temporary:
