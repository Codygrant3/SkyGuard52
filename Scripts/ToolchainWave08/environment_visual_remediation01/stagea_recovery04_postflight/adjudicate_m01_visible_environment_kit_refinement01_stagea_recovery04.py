...
OUTPUT_ROOT = ROOT / r"Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery04"
ATTEMPT_ROOT = ROOT / r"Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04\attempt_01"
SUPERVISOR_MANIFEST = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04_TERMINAL_SUPERVISOR.json"
POSTFLIGHT_REPORT = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04_POSTFLIGHT.json"
VISUAL_MANIFEST = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04_VISUAL_REVIEW_MANIFEST.json"
OFFLINE_FREEZE = ROOT / r"Docs\AAA_Review\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04_OFFLINE_SOURCE_FREEZE.json"

EXPECTED_OFFLINE_FREEZE_SHA256 = "afaf4728bcc6de2e87d74fca15f6348533b88153ba5d5e47c57c96d584df707f"
EXPECTED_SOURCE_SHA256 = "bbd3cdc704ad9346c74f832eaae261e60ecc5597623dad0caba361b25b6f4159"
...
def verify_freeze() -> None:
    require(OFFLINE_FREEZE.is_file() and sha256(OFFLINE_FREEZE) == EXPECTED_OFFLINE_FREEZE_SHA256, "Recovery04 offline freeze mismatch")
    freeze = load_json(OFFLINE_FREEZE)
    require(freeze.get("classification") == "PASSED_READY_FOR_RECOVERY04_POSTFLIGHT_BINDING", "Recovery04 offline classification mismatch")
    members = freeze.get("members") or []
    require(freeze.get("member_count") == 16 and len(members) == 16, "Recovery04 freeze cardinality mismatch")
    for member in members:
        path = Path(str(member.get("path", "")))
        require(path.is_file(), f"Recovery04 freeze member is missing: {path}")
        require(path.stat().st_size == member.get("bytes"), f"Recovery04 freeze member byte mismatch: {path}")
        require(sha256(path) == member.get("sha256"), f"Recovery04 freeze member hash mismatch: {path}")
...
def validate_attempt(attempt_root: Path) -> dict[str, Any]:
    ...
    source = attempt_root / r"source\build_m01_visible_environment_kit_refinement01_stagea_recovery04.py"
    ...
    require(
        preflight_data.get("recovery_source_sha256") == EXPECTED_SOURCE_SHA256
        and preflight_data.get("recovery03_terminal_members_verified") == 7
        and preflight_data.get("recovery03_artifact_members_verified") == 24,
        "attempt preflight authority evidence failed",
    )
...
def evaluate(output_root: Path, attempt_root: Path, supervisor_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    report = {
        "schema": "skyguard.m01-visible-environment-kit-refinement01-stagea-recovery04.postflight.v1",
    ...
    visual = {
        "schema": "skyguard.m01-visible-environment-kit-refinement01-stagea-recovery04.visual-review-manifest.v1",
...
def make_fixture(root: Path) -> tuple[Path, Path, Path]:
    ...
    attempt_source = attempt / r"source\build_m01_visible_environment_kit_refinement01_stagea_recovery04.py"
    attempt_source.parent.mkdir(parents=True)
    authoritative_source = ROOT / r"Scripts\ToolchainWave08\environment_visual_remediation01\stagea_recovery04\build_m01_visible_environment_kit_refinement01_stagea_recovery04.py"
    shutil.copyfile(authoritative_source, attempt_source)
    (attempt / "execution_contract.json").write_text("{}\n", encoding="utf-8")
    write_new_json(
        attempt / "preflight_receipt.json",
        {
            "passed": True,
            "retry_count": 0,
            "heavy_process_count": 0,
            "recovery_source_sha256": EXPECTED_SOURCE_SHA256,
...
def offline_contract_test() -> None:
    with tempfile.TemporaryDirectory(prefix="skyguard_stagea_recovery04_postflight_") as temporary:
    ...
    print("CLASSIFICATION=PASSED_RECOVERY04_POSTFLIGHT_OFFLINE_CONTRACT_TEST")
