...
MODULE_PATH = Path(__file__).with_name(
    "adjudicate_m01_visible_environment_kit_refinement01_stagea_recovery09_checkpoint01.py"
)


def load_module():
    spec = importlib.util.spec_from_file_location("skyguard_r09_postflight_test", MODULE_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("Could not load Recovery09 postflight")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class Recovery09PostflightTests(unittest.TestCase):
    @classmethod
    ...
    def test_struct_unpack_is_big_endian(self) -> None:
        self.assertEqual(
            (2560, 1440),
            self.module.struct.unpack(">II", (2560).to_bytes(4, "big") + (1440).to_bytes(4, "big")),
        )
