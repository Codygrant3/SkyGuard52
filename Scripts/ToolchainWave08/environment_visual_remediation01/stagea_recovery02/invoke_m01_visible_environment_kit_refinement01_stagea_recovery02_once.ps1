...
$Contract = Join-Path $Root 'Docs\Toolchain\ToolchainWave08\EnvironmentVisibleKitRefinement01StageARecovery02\execution_contract.json'
...
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY02\attempt_01'
$OutputRoot = Join-Path $Root 'Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery02'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY02_TERMINAL_SUPERVISOR.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY02_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 2700
...
function Write-TerminalEvidence([string]$ManifestPath, [string]$EmergencyPath, $State) {
    try {
        ...
        try {
            ...
            if (-not (Test-Path -LiteralPath $parent)) { [IO.Directory]::CreateDirectory($parent) | Out-Null }
            $line = [ordered]@{
                schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery02.emergency.v1'
...
function Assert-Authorities {
    Assert-File $Contract 5430 'c4fda63c47f9e3416284f3dff0a810d99a3b67b7c9162ff83ed937cb0a5dc155'
    ...
    if ($contractData.gate -ne $Gate) { throw 'Recovery02 gate drift' }
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery02.supervisor-terminal.v1'
    gate = $Gate
    classification = 'FAILED_WITH_EVIDENCE'
    terminal = $false
...
$manifestPath = if ($OfflineContractTest -and -not [string]::IsNullOrWhiteSpace($OfflineEvidenceRoot)) {
    Join-Path $OfflineEvidenceRoot 'terminal_manifest.json'
} else { $TerminalManifest }
$emergencyPath = if ($OfflineContractTest -and -not [string]::IsNullOrWhiteSpace($OfflineEvidenceRoot)) {
...
try {
    ...
    if ($OfflineContractTest) {
        ...
        Write-JsonAtomic (Join-Path $AttemptRoot 'preflight_receipt.json') ([ordered]@{
            schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery02.preflight.v1'
            at_utc = [DateTime]::UtcNow.ToString('o')
        ...
        if (-not (Test-Path -LiteralPath $OutputRoot)) { throw 'Blender did not create the Recovery02 output namespace' }
        ...
        if ($blendFiles.Count -ne 1 -or $glbFiles.Count -ne 4 -or $checkpointFiles.Count -ne 3 -or $finalFiles.Count -ne 15 -or $textureFiles.Count -ne 5) {
            throw "Recovery02 output cardinality failed: $($state.output_counts | ConvertTo-Json -Compress)"
        }
