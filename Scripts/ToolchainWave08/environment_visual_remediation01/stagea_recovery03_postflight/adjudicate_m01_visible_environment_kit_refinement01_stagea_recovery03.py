...
OUTPUT_ROOT = ROOT / r"Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery03"
ATTEMPT_ROOT = ROOT / r"Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY03\attempt_01"
SUPERVISOR_MANIFEST = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY03_TERMINAL_SUPERVISOR.json"
POSTFLIGHT_REPORT = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY03_POSTFLIGHT.json"
VISUAL_MANIFEST = ROOT / r"Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY03_VISUAL_REVIEW_MANIFEST.json"
OFFLINE_FREEZE = ROOT / r"Docs\AAA_Review\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY03_OFFLINE_SOURCE_FREEZE.json"

EXPECTED_OFFLINE_FREEZE_SHA256 = "162a04da78bc337a957406a05b9f40c224a471bef3af267a483705eaba9b057d"
...
def verify_freeze() -> None:
    require(OFFLINE_FREEZE.is_file() and sha256(OFFLINE_FREEZE) == EXPECTED_OFFLINE_FREEZE_SHA256, "Recovery03 offline freeze mismatch")
    freeze = load_json(OFFLINE_FREEZE)
    require(freeze.get("classification") == "PASSED_READY_FOR_RECOVERY03_POSTFLIGHT_BINDING", "Recovery03 offline classification mismatch")
    members = freeze.get("members") or []
    require(freeze.get("member_count") == 10 and len(members) == 10, "Recovery03 freeze cardinality mismatch")
    for member in members:
        path = Path(str(member.get("path", "")))
        require(path.is_file(), f"Recovery03 freeze member is missing: {path}")
        require(path.stat().st_size == member.get("bytes"), f"Recovery03 freeze member byte mismatch: {path}")
        require(sha256(path) == member.get("sha256"), f"Recovery03 freeze member hash mismatch: {path}")
...
def evaluate(output_root: Path, attempt_root: Path, supervisor_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    report = {
        "schema": "skyguard.m01-visible-environment-kit-refinement01-stagea-recovery03.postflight.v1",
    ...
    visual = {
        "schema": "skyguard.m01-visible-environment-kit-refinement01-stagea-recovery03.visual-review-manifest.v1",
...
def offline_contract_test() -> None:
    with tempfile.TemporaryDirectory(prefix="skyguard_stagea_recovery03_postflight_") as temporary:
    ...
    print("CLASSIFICATION=PASSED_RECOVERY03_POSTFLIGHT_OFFLINE_CONTRACT_TEST")
