...
$Contract = Join-Path $Root 'Docs\Toolchain\ToolchainWave08\EnvironmentVisibleKitRefinement01StageARecovery07\execution_contract.json'
$RecoverySource = Join-Path $Root 'Scripts\ToolchainWave08\environment_visual_remediation01\stagea_recovery07\build_m01_visible_environment_kit_refinement01_stagea_recovery07_checkpoint01.py'
...
$Recovery06TerminalFreeze = Join-Path $Root 'Docs\AAA_Review\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY06_CHECKPOINT01_ATTEMPT01_TERMINAL_FREEZE.json'
$Recovery06FailureAnalysis = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY06_CHECKPOINT01_ATTEMPT01_FAILURE_ANALYSIS.json'
$Recovery06ArtifactInventory = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY06_CHECKPOINT01_ATTEMPT01_ARTIFACT_INVENTORY.json'
$Blender = 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY07_CHECKPOINT01\attempt_01'
$OutputRoot = Join-Path $Root 'Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery07_Checkpoint01'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY07_CHECKPOINT01_TERMINAL_SUPERVISOR.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY07_CHECKPOINT01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 3600
...
function Write-TerminalEvidence([string]$ManifestPath, [string]$EmergencyPath, $State) {
    try {
        ...
        try {
            ...
            if (-not (Test-Path -LiteralPath $parent)) { [IO.Directory]::CreateDirectory($parent) | Out-Null }
            $line = [ordered]@{
                schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery07-checkpoint01.emergency.v1'
...
function Assert-Recovery06TerminalFreeze {
    Assert-File $Recovery06TerminalFreeze 3131 '6bd9b7e450711ecb140ebe4267a4834fb99ecc5dc700910eec37fd84b57a3ad4'
    $freeze = Get-Content -LiteralPath $Recovery06TerminalFreeze -Raw | ConvertFrom-Json
    if ($freeze.classification -ne 'FAILED_WITH_EVIDENCE' -or [int]$freeze.member_count -ne 8 -or [int]$freeze.verified_members -ne 8) {
        throw 'Recovery06 terminal freeze classification or cardinality drift'
    }
    foreach ($member in @($freeze.members)) {
        Assert-File ([string]$member.path) ([long]$member.bytes) ([string]$member.sha256)
    }
    Assert-File $Recovery06FailureAnalysis 2428 '57a161971b44753f7f7776508115fd94cad646a2c5f5637840d10778667b6996'
    Assert-File $Recovery06ArtifactInventory 2455 'a48cdfa13e1da54e0327328eec2c9fd00cc8baf5b5c5d2c1562fd470fc96af60'
}

function Assert-Authorities {
    Assert-File $Contract 4011 'a532b0c07c78acebd16c78d26c1d166460ede377e08a5b1ac1c3555c60498d8f'
    Assert-File $RecoverySource 12591 '86133a4e47057cbfd015538f888569e682eea920ce52035ae46bff0b7160713c'
    ...
    Assert-Recovery06TerminalFreeze
    $contractData = Get-Content -LiteralPath $Contract -Raw | ConvertFrom-Json
    if ($contractData.gate -ne $Gate) { throw 'Recovery07 Checkpoint01 gate drift' }
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery07-checkpoint01.supervisor-terminal.v1'
    gate = $Gate
    classification = 'FAILED_WITH_EVIDENCE'
    terminal = $false
...
try {
    ...
    if ($OfflineContractTest) {
        ...
        if ($wrapperText -notmatch 'recovery06_attempt_or_output_reused": False') { throw 'Recovery06 attempt/output nonreuse receipt missing' }
        $state.preflight_passed = $true
        $state.classification = 'PASS'
        $state.receipt_states = [ordered]@{ png_fixtures = $true; checkpoint_only = $true; recovery06_preserved = $true; generated_call_graph = $true }
        $exit = 0
    }
    else {
        ...
        $attemptSource = Join-Path $AttemptRoot 'source\build_m01_visible_environment_kit_refinement01_stagea_recovery07_checkpoint01.py'
        [IO.File]::Copy($RecoverySource, $attemptSource, $false)
        [IO.File]::Copy($Contract, (Join-Path $AttemptRoot 'execution_contract.json'), $false)
        Assert-File $attemptSource 12591 '86133a4e47057cbfd015538f888569e682eea920ce52035ae46bff0b7160713c'
        Write-JsonAtomic (Join-Path $AttemptRoot 'preflight_receipt.json') ([ordered]@{
            schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery07-checkpoint01.preflight.v1'
            at_utc = [DateTime]::UtcNow.ToString('o')
            recovery06_terminal_freeze_sha256 = '6bd9b7e450711ecb140ebe4267a4834fb99ecc5dc700910eec37fd84b57a3ad4'
            ...
            recovery_source_sha256 = '86133a4e47057cbfd015538f888569e682eea920ce52035ae46bff0b7160713c'
        ...
        $arguments = @(
            ...
            '--expected-source-sha256', '86133a4e47057cbfd015538f888569e682eea920ce52035ae46bff0b7160713c'
        )
        ...
        if (-not (Test-Path -LiteralPath $OutputRoot)) { throw 'Blender did not create the Recovery07 Checkpoint01 output namespace' }
        ...
        $state.receipt_states = [ordered]@{
            ...
            source_parity = ([bool]$sourceParity.passed -and [string]$sourceParity.sha256 -eq '86133a4e47057cbfd015538f888569e682eea920ce52035ae46bff0b7160713c')
