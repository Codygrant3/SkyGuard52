...
$Contract = Join-Path $Root 'Docs\Toolchain\ToolchainWave08\EnvironmentVisibleKitRefinement01StageARecovery05\execution_contract.json'
$RecoverySource = Join-Path $Root 'Scripts\ToolchainWave08\environment_visual_remediation01\stagea_recovery05\build_m01_visible_environment_kit_refinement01_stagea_recovery05_checkpoint01.py'
...
$Recovery04TerminalFreeze = Join-Path $Root 'Docs\AAA_Review\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04_ATTEMPT01_TERMINAL_FREEZE.json'
$Recovery04FailureAnalysis = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04_ATTEMPT01_FAILURE_ANALYSIS.json'
$Recovery04VisualReview = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY04_ATTEMPT01_DIAGNOSTIC_VISUAL_REVIEW.json'
$Blender = 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY05_CHECKPOINT01\attempt_01'
$OutputRoot = Join-Path $Root 'Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery05_Checkpoint01'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY05_CHECKPOINT01_TERMINAL_SUPERVISOR.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY05_CHECKPOINT01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 3600
...
function Write-TerminalEvidence([string]$ManifestPath, [string]$EmergencyPath, $State) {
    try {
        ...
        try {
            ...
            if (-not (Test-Path -LiteralPath $parent)) { [IO.Directory]::CreateDirectory($parent) | Out-Null }
            $line = [ordered]@{
                schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery05-checkpoint01.emergency.v1'
...
function Assert-Recovery04TerminalFreeze {
    Assert-File $Recovery04TerminalFreeze 4822 'fda28107ec833226dd1b6dbaef626ca7f8607e7562079a95d46d1abacd0540e6'
    $freeze = Get-Content -LiteralPath $Recovery04TerminalFreeze -Raw | ConvertFrom-Json
    if ($freeze.classification -ne 'FAILED_WITH_EVIDENCE' -or [int]$freeze.member_count -ne 8) {
        throw 'Recovery04 terminal freeze classification or cardinality drift'
    }
    foreach ($member in @($freeze.members)) {
        Assert-File ([string]$member.path) ([long]$member.bytes) ([string]$member.sha256)
    }
    Assert-File $Recovery04FailureAnalysis 3369 '02851b47a993a676b97a41b5089c06011f6cd33209d589a7b257f792ab2c4485'
    Assert-File $Recovery04VisualReview 11027 '7558d332f38613443e34c08918dc746bd420db0dbed7b62ade815d96a4670bf2'
}

function Assert-Authorities {
    Assert-File $Contract 4004 '3433d66c286402756f0a76b5b70fc33689f2e4a2443219d543041734b285d6df'
    Assert-File $RecoverySource 79498 '11c87230cfd1464bc83fea7b4b7d14efe4341be5f1adeb0512116e5e0a3aaa95'
    ...
    Assert-Recovery04TerminalFreeze
    $contractData = Get-Content -LiteralPath $Contract -Raw | ConvertFrom-Json
    if ($contractData.gate -ne $Gate) { throw 'Recovery05 Checkpoint01 gate drift' }
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery05-checkpoint01.supervisor-terminal.v1'
    gate = $Gate
    classification = 'FAILED_WITH_EVIDENCE'
    terminal = $false
...
try {
    ...
    if ($OfflineContractTest) {
        ...
        if ($wrapperText -notmatch 'recovery04_output_geometry_reused": False') { throw 'Recovery04 geometry nonreuse receipt missing' }
        $state.preflight_passed = $true
        $state.classification = 'PASS'
        $state.receipt_states = [ordered]@{ png_fixtures = $true; checkpoint_only = $true; recovery04_preserved = $true }
        $exit = 0
    }
    else {
        ...
        $attemptSource = Join-Path $AttemptRoot 'source\build_m01_visible_environment_kit_refinement01_stagea_recovery05_checkpoint01.py'
        [IO.File]::Copy($RecoverySource, $attemptSource, $false)
        [IO.File]::Copy($Contract, (Join-Path $AttemptRoot 'execution_contract.json'), $false)
        Assert-File $attemptSource 79498 '11c87230cfd1464bc83fea7b4b7d14efe4341be5f1adeb0512116e5e0a3aaa95'
        Write-JsonAtomic (Join-Path $AttemptRoot 'preflight_receipt.json') ([ordered]@{
            schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery05-checkpoint01.preflight.v1'
            at_utc = [DateTime]::UtcNow.ToString('o')
            recovery04_terminal_freeze_sha256 = 'fda28107ec833226dd1b6dbaef626ca7f8607e7562079a95d46d1abacd0540e6'
            ...
            recovery_source_sha256 = '11c87230cfd1464bc83fea7b4b7d14efe4341be5f1adeb0512116e5e0a3aaa95'
        ...
        $arguments = @(
            ...
            '--expected-source-sha256', '11c87230cfd1464bc83fea7b4b7d14efe4341be5f1adeb0512116e5e0a3aaa95'
        )
        ...
        if (-not (Test-Path -LiteralPath $OutputRoot)) { throw 'Blender did not create the Recovery05 Checkpoint01 output namespace' }
        ...
        $state.receipt_states = [ordered]@{
            ...
            source_parity = ([bool]$sourceParity.passed -and [string]$sourceParity.sha256 -eq '11c87230cfd1464bc83fea7b4b7d14efe4341be5f1adeb0512116e5e0a3aaa95')
