...
$Contract = Join-Path $Root 'Docs\Toolchain\ToolchainWave08\EnvironmentVisibleKitRefinement01StageARecovery08\execution_contract.json'
$Worker = Join-Path $Root 'Scripts\ToolchainWave08\environment_visual_remediation01\stagea_recovery08\build_m01_visible_environment_kit_refinement01_stagea_recovery08_checkpoint01.py'
$R07Freeze = Join-Path $Root 'Docs\AAA_Review\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY07_CHECKPOINT01_ATTEMPT01_TERMINAL_FREEZE.json'
$Blender = 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY08_CHECKPOINT01\attempt_01'
$OutputRoot = Join-Path $Root 'Content\Skyguard\Meshes\Source\Mission01\VisibleEnvironmentKit_Refinement01_StageA_Recovery08_Checkpoint01'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY08_CHECKPOINT01_TERMINAL_SUPERVISOR.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_KIT_REFINEMENT01_STAGEA_RECOVERY08_CHECKPOINT01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 3600
...
function Write-TerminalEvidence([string]$ManifestPath, [string]$EmergencyPath, $State) {
    try {
        ...
        try {
            ...
            if (-not (Test-Path -LiteralPath $parent)) { [IO.Directory]::CreateDirectory($parent) | Out-Null }
            $line = [ordered]@{
                schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery08-checkpoint01.emergency.v1'
...
function Assert-Authorities {
    Assert-File $R07Freeze 4081 '8ef143be666036c3cc5e4dc5914419e990573d338d755920fd3422048a8c2f72'
    $freeze = Get-Content -LiteralPath $R07Freeze -Raw | ConvertFrom-Json
    if ($freeze.classification -ne 'FAILED_WITH_EVIDENCE' -or [int]$freeze.member_count -ne 12 -or [int]$freeze.verified_members -ne 12) { throw 'Recovery07 terminal freeze drift' }
    foreach ($member in @($freeze.members)) { Assert-File ([string]$member.path) ([long]$member.bytes) ([string]$member.sha256) }
    Assert-File $Contract 3913 '676daeba3b30f67f584c76b5c2e6683e53f84f090739a2b38a674e4dacac381d'
    Assert-File $Worker 10929 '0e7a1ffa28477a3dd94b326e1c31a56833e37bd2952df8b5eb87145f4b2193dc'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery08-checkpoint01.supervisor-terminal.v1'
    gate = $Gate
    classification = 'FAILED_WITH_EVIDENCE'
    terminal = $false
...
try {
    ...
    if ($OfflineContractTest) {
        ...
        foreach ($token in @('for calibration_index in range(2):','new_exposure = min(7.0','"night_lighting_calibration"','recovery07_attempt_or_output_reused": False')) {
        ...
        $state.receipt_states = [ordered]@{ recovery07_preserved = $true; bounded_night_calibration = $true; checkpoint_only = $true }
        $exit = 0
    }
    else {
        ...
        $attemptSource = Join-Path $AttemptRoot 'source\build_m01_visible_environment_kit_refinement01_stagea_recovery08_checkpoint01.py'
        [IO.File]::Copy($Worker, $attemptSource, $false)
        [IO.File]::Copy($Contract, (Join-Path $AttemptRoot 'execution_contract.json'), $false)
        Assert-File $attemptSource 10929 '0e7a1ffa28477a3dd94b326e1c31a56833e37bd2952df8b5eb87145f4b2193dc'
        Write-JsonAtomic (Join-Path $AttemptRoot 'preflight_receipt.json') ([ordered]@{
            schema = 'skyguard.m01-visible-environment-kit-refinement01-stagea-recovery08-checkpoint01.preflight.v1'
            at_utc = [DateTime]::UtcNow.ToString('o')
            recovery07_terminal_freeze_sha256 = '8ef143be666036c3cc5e4dc5914419e990573d338d755920fd3422048a8c2f72'
            recovery07_terminal_members_verified = 12
            recovery_source_sha256 = '0e7a1ffa28477a3dd94b326e1c31a56833e37bd2952df8b5eb87145f4b2193dc'
        ...
        $arguments = @('--background','--factory-startup','--python',$attemptSource,'--','--output',$OutputRoot,'--asset-id','m01-visible-environment-kit-refinement01-stagea','--expected-source-sha256','0e7a1ffa28477a3dd94b326e1c31a56833e37bd2952df8b5eb87145f4b2193dc')
        ...
        if (-not (Test-Path -LiteralPath $OutputRoot)) { throw 'Blender did not create the Recovery08 output namespace' }
        ...
        $state.receipt_states = [ordered]@{
            ...
            source_parity = ([bool]$sourceParity.passed -and [string]$sourceParity.sha256 -eq '0e7a1ffa28477a3dd94b326e1c31a56833e37bd2952df8b5eb87145f4b2193dc')
