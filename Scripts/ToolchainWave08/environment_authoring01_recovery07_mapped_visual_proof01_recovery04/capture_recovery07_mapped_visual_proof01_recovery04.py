...
CONTRACT_PATH = ROOT / (
    ...
    "MAPPED_VISUAL_PROOF01_RECOVERY04_CONTRACT.json"
)
CAMERAS_PATH = ROOT / (
    "Docs/AAA_Review/"
    "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_"
    "MAPPED_VISUAL_PROOF01_RECOVERY04_CAMERAS.json"
)
...
CSV_FILENAME = "Recovery07MappedVisualProof01Recovery04.csv"
EXPECTED_STARTUP_RECEIPT = ROOT / (
    "Saved/BuildAttempts/"
    "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_"
    "MAPPED_VISUAL_PROOF01_RECOVERY04/launcher_attempt_01/"
    "executor_startup_receipt.json"
)
EXPECTED_ACTOR_INVENTORY_RECEIPT = ROOT / (
    "Saved/BuildAttempts/"
    "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_"
    "MAPPED_VISUAL_PROOF01_RECOVERY04/launcher_attempt_01/"
    "runtime_actor_inventory.json"
)
EXPECTED_EARLY_TERMINAL_RECEIPT = ROOT / (
    "Saved/BuildAttempts/"
    "TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_"
    "MAPPED_VISUAL_PROOF01_RECOVERY04/launcher_attempt_01/"
    "executor_early_terminal.json"
)
_STATE = None
...
def actor_inventory_record(actor) -> dict:
    record = actor_transform_record(actor)
    record["tags"] = sorted(str(tag) for tag in actor.tags)
    record["object_path"] = actor.get_path_name()
    return record
...
def validate_actor_contract(
    actors: list, contract: dict
    ...
    for actor in actors:
        ...
        is_governed = label.startswith("M01_A01_") or any(
            tag == "Skyguard.Authoring01"
            or tag.startswith("Skyguard.Authoring01.")
            or tag.startswith("Skyguard.Environment.Mission01")
            for tag in tags
        )
        record = actor_inventory_record(actor)
        if is_governed:
            unexpected_governed.append(record)
        else:
            ungoverned.append(record)
    expected_governed_count = int(
        contract["world"]["expected_total_governed_actor_count"]
    )
    if len(governed) != expected_governed_count:
        raise RuntimeError(
            f"Expected {expected_governed_count} governed actors; "
            f"found {len(governed)}"
        )
    if unexpected_governed:
        raise RuntimeError(
            "Unexpected Skyguard-governed actors: "
            + ", ".join(item["label"] for item in unexpected_governed)
        )
    maximum_ungoverned = int(
        contract["world"]["maximum_ungoverned_editor_actors"]
    )
    if len(ungoverned) > maximum_ungoverned:
        raise RuntimeError(
            f"Expected at most {maximum_ungoverned} ungoverned editor actors; "
            f"found {len(ungoverned)}"
        )
    ...
    return landscape, terrain_material, {
        "raw_actor_count": len(actors),
        "governed_actor_count": len(governed),
        "unexpected_governed_actors": unexpected_governed,
        "ungoverned_editor_actor_count": len(ungoverned),
        "ungoverned_editor_actors": ungoverned,
    }


def main() -> None:
    global _STATE
    early_terminal = EXPECTED_EARLY_TERMINAL_RECEIPT.resolve()
    try:
        startup_receipt = Path(
            parse_switch("SkyguardRecovery07ProofStartupReceipt")
        ).resolve()
        actor_inventory_receipt = Path(
            parse_switch("SkyguardRecovery07ProofActorInventoryReceipt")
        ).resolve()
        requested_early_terminal = Path(
            parse_switch("SkyguardRecovery07ProofEarlyTerminalReceipt")
        ).resolve()
        if startup_receipt != EXPECTED_STARTUP_RECEIPT.resolve():
            raise RuntimeError("Executor startup receipt path mismatch")
        if actor_inventory_receipt != EXPECTED_ACTOR_INVENTORY_RECEIPT.resolve():
            raise RuntimeError("Actor inventory receipt path mismatch")
        if requested_early_terminal != early_terminal:
            raise RuntimeError("Executor early terminal receipt path mismatch")
        for path in (startup_receipt, actor_inventory_receipt, early_terminal):
            if path.exists():
                raise RuntimeError(f"Executor evidence path already exists: {path}")
        write_json_atomic(
            startup_receipt,
            {
        ...
        write_json_atomic(
            ...
            {
                "schema": "skyguard.t08.m01.recovery07-mapped-proof01-runtime-actor-inventory.v1",
                "contract_id": CONTRACT_ID,
                "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                "raw_actor_count": len(inventory),
                "actors": sorted(inventory, key=lambda item: (item["label"], item["object_path"])),
            },
        )
        landscape, terrain_material, actor_validation = validate_actor_contract(
            actors, contract
        )
        ...
        if not early_terminal.exists():
            write_json_atomic(
                early_terminal,
                {
                    "schema": "skyguard.t08.m01.recovery07-mapped-proof01-executor-early-terminal.v1",
                    "gate": "FAILED_WITH_EVIDENCE",
                    "contract_id": CONTRACT_ID,
                    "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
