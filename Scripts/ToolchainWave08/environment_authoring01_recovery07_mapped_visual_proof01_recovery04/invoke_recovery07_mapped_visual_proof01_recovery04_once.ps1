...
$contractId = 'T08-M01-ENV-AUTH01-RECOVERY07-MAPPED-VISUAL-PROOF01-RECOVERY04'
$scriptRoot = Join-Path $root 'Scripts\ToolchainWave08\environment_authoring01_recovery07_mapped_visual_proof01_recovery04'
$executor = Join-Path $scriptRoot 'capture_recovery07_mapped_visual_proof01_recovery04.py'
$adjudicator = Join-Path $scriptRoot 'adjudicate_recovery07_mapped_visual_proof01_recovery04_once.py'
$contract = Join-Path $root 'Docs\AAA_Review\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_CONTRACT.json'
$offlineFreeze = Join-Path $root 'Docs\AAA_Review\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_OFFLINE_DESIGN_FREEZE.json'
$bindingFreeze = Join-Path $root 'Docs\AAA_Review\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_EXECUTION_PROMPT_BINDING_FREEZE.json'
$attemptRoot = Join-Path $root 'Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04\attempt_01'
$launcherRoot = Join-Path $root 'Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04\launcher_attempt_01'
$startupReceipt = Join-Path $launcherRoot 'executor_startup_receipt.json'
$actorInventoryReceipt = Join-Path $launcherRoot 'runtime_actor_inventory.json'
$earlyTerminalReceipt = Join-Path $launcherRoot 'executor_early_terminal.json'
$preflightReceipt = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_EXECUTION_PREFLIGHT.json'
$terminalSupervisor = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_TERMINAL_SUPERVISOR.json'
$emergencyReceipt = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_EMERGENCY_RECEIPT.jsonl'
$postflightReport = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_POSTFLIGHT.json'
$profileFile = Join-Path $isolatedRoot 'Saved\Profiling\CSV\Recovery07MappedVisualProof01Recovery04.csv'
$executorStartupTimeoutSeconds = 120
$earlyTerminalGraceSeconds = 30
$timeoutSeconds = 900
...
$state = [ordered]@{
    ...
    executor_startup_timeout_seconds = $executorStartupTimeoutSeconds
    executor_startup_receipt_observed = $false
    executor_startup_timed_out = $false
    executor_startup_receipt = $null
    actor_inventory_receipt_observed = $false
    actor_inventory_receipt = $null
    executor_early_terminal_observed = $false
    executor_early_terminal_receipt = $null
    executor_early_terminal_grace_seconds = $earlyTerminalGraceSeconds
    executor_early_terminal_deadline_shortened = $false
    timed_out = $false
...
function Test-OfflineContract {
    ...
    if ($freeze.classification -ne 'PASSED_READY_FOR_EXPLICIT_SINGLE_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_AUTHORIZATION') { throw 'Offline freeze classification mismatch' }
    if ($binding.classification -ne 'PASSED_READY_FOR_EXPLICIT_SINGLE_RECOVERY07_MAPPED_VISUAL_PROOF01_RECOVERY04_AUTHORIZATION') { throw 'Binding freeze classification mismatch' }
    ...
    foreach ($future in @($attemptRoot, $launcherRoot, $startupReceipt, $actorInventoryReceipt, $earlyTerminalReceipt, $preflightReceipt, $terminalSupervisor, $emergencyReceipt, $postflightReport, $profileFile)) {
    ...
    $execValueNeedle = '$execCmdValue = "py $($executor.Replace'
    $execArgumentNeedle = '$execCmdArgument = ''-ExecCmds="'' + $execCmdValue + ''"'''
    if ($script -notmatch [regex]::Escape($execValueNeedle)) { throw 'Deferred Python command value is absent' }
    if ($script -notmatch [regex]::Escape($execArgumentNeedle)) { throw 'Quoted ExecCmds native argument is absent' }
    $legacyExecPrefix = '"-ExecCmds=py $(' + '$executor.Replace'
    if ($script -match [regex]::Escape($legacyExecPrefix)) { throw 'Recovery02 unquoted ExecCmds argument remains' }
    if ($executorStartupTimeoutSeconds -ne 120) { throw 'Executor startup timeout changed' }
    if ($earlyTerminalGraceSeconds -ne 30) { throw 'Executor early-terminal grace period changed' }
    if ($timeoutSeconds -ne 900) { throw 'Total supervisor timeout changed' }
    foreach ($requiredSwitch in @('SkyguardRecovery07ProofActorInventoryReceipt', 'SkyguardRecovery07ProofEarlyTerminalReceipt')) {
        if ($script -notmatch [regex]::Escape($requiredSwitch)) { throw "Required executor evidence switch is absent: $requiredSwitch" }
    }
    ...
    return [ordered]@{
        ...
        quoted_exec_cmd_transport = $true
        executor_startup_timeout_seconds = $executorStartupTimeoutSeconds
        supervisor_timeout_seconds = $timeoutSeconds
        unreal_launch_count = 0
        retry_count = 0
    }
}
...
try {
    ...
    elseif (-not $AuthorizeSingleUnrealProof) {
        ...
        $stdout = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01_recovery04.stdout.log'
        $stderr = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01_recovery04.stderr.log'
        $engineLog = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01_recovery04.engine.log'
        $processTree = Join-Path $launcherRoot 'process_tree_samples.jsonl'
        $postflightLog = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01_recovery04.postflight.log'
        ...
        $arguments = @(
            ...
            '-unattended',
            '-nop4',
            '-DisablePlugins=Fab,Bridge,EditorTelemetry,RuntimeTelemetry,EOSShared,SkyguardRecovery04,SkyguardRecovery04NativeRecovery01,SkyguardRecovery04NativeRecovery04,SkyguardRecovery04NativeRecovery05',
            ...
            "-SkyguardRecovery07ProofStartupReceipt=$($startupReceipt.Replace('\','/'))",
            "-SkyguardRecovery07ProofActorInventoryReceipt=$($actorInventoryReceipt.Replace('\','/'))",
            "-SkyguardRecovery07ProofEarlyTerminalReceipt=$($earlyTerminalReceipt.Replace('\','/'))",
            $execCmdArgument,
            "-abslog=$engineLog"
        )
        ...
        while (-not $process.HasExited -and [DateTime]::UtcNow -lt $deadline) {
            ...
            if (-not $state.executor_startup_receipt_observed -and (Test-Path -LiteralPath $startupReceipt -PathType Leaf)) {
                $startup = Assert-JsonObject $startupReceipt
                if ($startup.gate -ne 'EXECUTOR_INVOKED') { throw "Executor startup gate failed: $($startup.gate)" }
                if ($startup.contract_id -ne $contractId) { throw 'Executor startup contract mismatch' }
                $state.executor_startup_receipt_observed = $true
                $state.executor_startup_receipt = [ordered]@{
                    path = $startupReceipt
                    bytes = (Get-Item -LiteralPath $startupReceipt).Length
                    sha256 = Get-LowerSha256 $startupReceipt
                }
            }
            if (-not $state.actor_inventory_receipt_observed -and (Test-Path -LiteralPath $actorInventoryReceipt -PathType Leaf)) {
                $actorInventory = Assert-JsonObject $actorInventoryReceipt
                if ($actorInventory.contract_id -ne $contractId) { throw 'Runtime actor-inventory contract mismatch' }
                $state.actor_inventory_receipt_observed = $true
                $state.actor_inventory_receipt = [ordered]@{
                    path = $actorInventoryReceipt
                    bytes = (Get-Item -LiteralPath $actorInventoryReceipt).Length
                    sha256 = Get-LowerSha256 $actorInventoryReceipt
                    raw_actor_count = [int]$actorInventory.raw_actor_count
                }
            }
            if (-not $state.executor_early_terminal_observed -and (Test-Path -LiteralPath $earlyTerminalReceipt -PathType Leaf)) {
                $earlyTerminal = Assert-JsonObject $earlyTerminalReceipt
                if ($earlyTerminal.gate -ne 'FAILED_WITH_EVIDENCE') { throw "Executor early-terminal gate mismatch: $($earlyTerminal.gate)" }
                if ($earlyTerminal.contract_id -ne $contractId) { throw 'Executor early-terminal contract mismatch' }
                $state.executor_early_terminal_observed = $true
                $state.executor_early_terminal_receipt = [ordered]@{
                    path = $earlyTerminalReceipt
                    bytes = (Get-Item -LiteralPath $earlyTerminalReceipt).Length
                    sha256 = Get-LowerSha256 $earlyTerminalReceipt
                    error = [string]$earlyTerminal.error
                }
                $deadline = [DateTime]::UtcNow.AddSeconds($earlyTerminalGraceSeconds)
                $state.executor_early_terminal_deadline_shortened = $true
            }
            if (-not $state.executor_startup_receipt_observed -and [DateTime]::UtcNow -ge $startupDeadline) {
                $state.executor_startup_timed_out = $true
        ...
        if (-not $state.executor_startup_receipt_observed) { throw 'Unreal exited without an executor startup receipt' }
        if ($state.executor_early_terminal_observed) {
            throw "Executor terminated during governed preflight: $($state.executor_early_terminal_receipt.error)"
        }
