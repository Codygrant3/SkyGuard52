...
$OutputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage05Composition01.umap'
$Destination = 'D:\SG52T08_ENV01\Content\M01\VisibleEnvironmentStage05Composition01'
$StandingAuthorization = Join-Path $Root 'Production\standing_heavy_process_authorization.json'
$Author = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage05_composition01\author_m01_visible_environment_stage05_composition01.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage05_composition01\stage05_composition01_authoring_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE05_COMPOSITION01_AUTHORING01\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE05_COMPOSITION01_AUTHORING01_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE05_COMPOSITION01_AUTHORING01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1800
...
$Expected = @{
    ...
    Author = @{ Bytes = 21696; Sha256 = '59706c1e9257c5fe426d3e8bf1b36b444667ad3f91ae186615134abd0e3e5c33' }
    Contract = @{ Bytes = 7376; Sha256 = '2d611abad87e682740f12fe6152fd4dd657087dcf590d686f412295dceb037b2' }
}
...
function Assert-Authorities {
    ...
    Assert-File $Author $Expected.Author.Bytes $Expected.Author.Sha256 'Stage05 authoring source'
    Assert-File $Contract $Expected.Contract.Bytes $Expected.Contract.Sha256 'Stage05 authoring contract'
...
if ($OfflineContractTest) {
    ...
    [ordered]@{
        classification = 'PASSED_OFFLINE_CONTRACT_READY_FOR_SINGLE_STAGE05_UNREAL_AUTHORING'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage05-composition01.authoring01.supervisor.v1'
...
try {
    ...
    Copy-Item -LiteralPath $Author -Destination (Join-Path $AttemptRoot 'author_m01_visible_environment_stage05_composition01.py')
    Copy-Item -LiteralPath $Contract -Destination (Join-Path $AttemptRoot 'stage05_composition01_authoring_contract.json')
    ...
    if (-not $process.HasExited) {
        ...
        throw "Unreal Stage05 authoring exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit(); $process.Refresh()
    $state.exit_code = [int]$process.ExitCode
    $state.exit_code_type = $process.ExitCode.GetType().FullName
    if ($state.exit_code -ne 0) { throw "Unreal Stage05 authoring returned $($state.exit_code)" }
    if ($state.exit_code_type -ne 'System.Int32') { throw "Unexpected exit-code type: $($state.exit_code_type)" }
    if (-not [System.IO.File]::Exists($Receipt)) { throw "Stage05 receipt missing: $Receipt" }
    ...
    if (-not [System.IO.File]::Exists($OutputMap)) { throw 'Stage05 output map missing' }
    Assert-File $InputMap $Expected.InputMap.Bytes $Expected.InputMap.Sha256 'Stage04 map after Stage05 authoring'
