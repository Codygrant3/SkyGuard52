...
CONTRACT_PATH = ROOT / "Scripts/ToolchainWave08/m01_polyhaven_vegetation_staging02/vegetation_staging02_contract.json"
    ...
    for spec in contract["assets"]:
        ...
        for definition in spec["materials"]:
            for texture in definition["textures"]:
                texture_path = Path(str(texture["path"]))
                require(texture_path.is_file(), f"Material texture missing: {texture_path}")
                require(texture_path.stat().st_size == int(texture["bytes"]), f"Material texture byte mismatch: {texture_path}")
                require(sha256(texture_path) == str(texture["sha256"]), f"Material texture hash mismatch: {texture_path}")
    return rows
...
def offline_contract_test() -> int:
    ...
    require(contract["material_method"]["generic_gltf_material_translator_forbidden"] is True, "Generic translator prohibition changed")
    require(sum(len(asset["materials"]) for asset in contract["assets"]) == 7, "Explicit material count changed")
    require(all(any(texture["role"] == "alpha" for texture in material["textures"]) for asset in contract["assets"] for material in asset["materials"] if material["masked"]), "Masked material is missing alpha authority")
    compile(Path(__file__).read_text(encoding="utf-8"), __file__, "exec")
    print("PASS_M01_POLYHAVEN_VEGETATION_STAGING02_OFFLINE_CONTRACT")
    return 0
...
def import_source_texture(unreal: object, source: dict[str, object], destination: str, name: str) -> tuple[object, dict[str, object]]:
    source_path = Path(str(source["path"]))
    require(source_path.is_file(), f"Texture source missing: {source_path}")
    require(source_path.stat().st_size == int(source["bytes"]), f"Texture source byte mismatch: {source_path}")
    require(sha256(source_path) == str(source["sha256"]), f"Texture source hash mismatch: {source_path}")
    ...
    require(isinstance(texture, unreal.Texture2D), f"Texture import failed: {object_path}")
    ...
    unreal.EditorAssetLibrary.set_metadata_tag(texture, "Skyguard.SourceProvider", "Poly Haven")
    unreal.EditorAssetLibrary.set_metadata_tag(texture, "Skyguard.SourceLicense", "CC0-1.0")
    unreal.EditorAssetLibrary.set_metadata_tag(texture, "Skyguard.SourceSha256", str(source["sha256"]))
    unreal.EditorAssetLibrary.set_metadata_tag(texture, "Skyguard.RuntimePromotionAllowed", "false")
    unreal.EditorAssetLibrary.save_loaded_asset(texture, only_if_is_dirty=False)
    return texture, {
        "role": role,
        "source": record(source_path),
        "asset": texture.get_path_name(),
        "imported_objects": list(task.imported_object_paths or []),
    }


def create_explicit_material(unreal: object, definition: dict[str, object], destination: str) -> tuple[object, dict[str, object]]:
    material_id = str(definition["id"])
    texture_records: list[dict[str, object]] = []
    textures: dict[str, object] = {}
    for source in definition["textures"]:
        role = str(source["role"])
        texture, texture_record = import_source_texture(
            unreal,
            source,
            destination,
            f"T_M01_PHV02_{material_id}_{role}",
        )
        textures[role] = texture
        texture_records.append(texture_record)
    for role in ("diff", "normal", "arm"):
        require(role in textures, f"Material {material_id} is missing required {role} texture")
    masked = bool(definition["masked"])
    if masked:
        require("alpha" in textures, f"Masked material {material_id} has no authoritative alpha texture")

    name = f"M_M01_PHV02_{material_id}"
    material = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
        name, destination, unreal.Material, unreal.MaterialFactoryNew()
    )
    require(isinstance(material, unreal.Material), f"Material creation failed: {destination}/{name}")
    material.set_editor_property("two_sided", bool(definition["two_sided"]))
    material.set_editor_property("blend_mode", unreal.BlendMode.BLEND_MASKED if masked else unreal.BlendMode.BLEND_OPAQUE)
    if masked:
        material.set_editor_property("opacity_mask_clip_value", float(definition["opacity_mask_clip_value"]))
    mel = unreal.MaterialEditingLibrary
    mel.delete_all_material_expressions(material)
    samples: dict[str, object] = {}
    positions = {"diff": (-700, -220), "normal": (-700, -20), "arm": (-700, 180), "alpha": (-700, 380)}
    for role, texture in textures.items():
        x, y = positions[role]
        sample = mel.create_material_expression(material, unreal.MaterialExpressionTextureSample, x, y)
        ...
        samples[role] = sample
    mel.connect_material_property(samples["diff"], "RGB", unreal.MaterialProperty.MP_BASE_COLOR)
    mel.connect_material_property(samples["normal"], "RGB", unreal.MaterialProperty.MP_NORMAL)
    mel.connect_material_property(samples["arm"], "R", unreal.MaterialProperty.MP_AMBIENT_OCCLUSION)
    mel.connect_material_property(samples["arm"], "G", unreal.MaterialProperty.MP_ROUGHNESS)
    mel.connect_material_property(samples["arm"], "B", unreal.MaterialProperty.MP_METALLIC)
    if masked:
        mel.connect_material_property(samples["alpha"], "R", unreal.MaterialProperty.MP_OPACITY_MASK)
    mel.recompile_material(material)
    unreal.EditorAssetLibrary.set_metadata_tag(material, "Skyguard.SourceProvider", "Poly Haven")
    unreal.EditorAssetLibrary.set_metadata_tag(material, "Skyguard.SourceLicense", "CC0-1.0")
    unreal.EditorAssetLibrary.set_metadata_tag(material, "Skyguard.MaterialMethod", "EXPLICIT_SOURCE_BACKED_PBR")
    unreal.EditorAssetLibrary.set_metadata_tag(material, "Skyguard.RuntimePromotionAllowed", "false")
    ...
    return material, {
        ...
        "opacity_mask_clip_value": float(material.get_editor_property("opacity_mask_clip_value")) if masked else None,
        "channel_contract": {"arm_r": "ambient_occlusion", "arm_g": "roughness", "arm_b": "metallic", "normal": "OpenGL source with Unreal green-channel flip"},
...
def import_candidate(unreal: object, spec: dict[str, object], root_asset: str) -> tuple[object, dict[str, object]]:
    ...
    require(len(slots) == int(spec["expected_material_slots"]), f"Material-slot count changed for {asset_id}: {len(slots)}")
    materials = []
    for index, definition in enumerate(spec["materials"]):
        material, explicit_record = create_explicit_material(unreal, definition, destination)
        mesh.set_material(index, material)
        materials.append(explicit_record)
    try:
    ...
    return mesh, {
        ...
        "materials": materials,
        "material_import_method": "GEOMETRY_ONLY_GLTF_PLUS_EXPLICIT_SOURCE_BACKED_UNREAL_MATERIALS",
...
def run_unreal() -> None:
    ...
    result: dict[str, object] = {
        "schema": "skyguard.m01-polyhaven-vegetation-staging02.authoring-receipt.v1",
    ...
    try:
        ...
        require(not any(actor.get_actor_label().startswith("M01_PHV02_") for actor in before), "Vegetation staging labels already exist")
        ...
        for spec in contract["assets"]:
            ...
            for index, (x_cm, y_cm, yaw, scale) in enumerate(placements, start=1):
                ...
                actor.set_actor_label(f"M01_PHV02_{asset_id}_{index:02d}")
                actor.set_folder_path(f"M01/SourceBacked/VegetationStaging02/{asset_id}")
