"""Bind the frozen Stage02 proof executor to fresh Recovery01 evidence paths."""
...
def transform_source() -> str:
    ...
    for old, new in REPLACEMENTS:
        if old not in transformed:
            raise RuntimeError(f"Recovery01 executor token absent: {old}")
        transformed = transformed.replace(old, new)
    return transformed


if __name__ == "__main__":
    exec(compile(transform_source(), str(SOURCE) + "::stage02-proof01-recovery01", "exec"), globals(), globals())
