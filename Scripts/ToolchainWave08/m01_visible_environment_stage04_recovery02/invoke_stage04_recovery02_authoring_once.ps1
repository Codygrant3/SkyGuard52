...
$OutputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage04Recovery02.umap'
$Destination = 'D:\SG52T08_ENV01\Content\M01\VisibleEnvironmentStage04Recovery02'
...
$Author = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage04_recovery02\author_m01_visible_environment_stage04_recovery02.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage04_recovery02\stage04_recovery02_authoring_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_RECOVERY02\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_RECOVERY02_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_RECOVERY02_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1200
...
$Expected = @{
    ...
    Author = @{ Bytes = 26118; Sha256 = '4010c5879d1061d6d1436255c754c82b0949cb7471b502bbb70f65d38b8ccde2' }
    Contract = @{ Bytes = 6863; Sha256 = 'ec4253c842e7d97fa433d86ff183332236fb645ad7c7301bda350d8ca7d5c0dc' }
}
...
function Assert-Authorities {
    ...
    Assert-File $Author $Expected.Author.Bytes $Expected.Author.Sha256 'Recovery02 authoring source'
    Assert-File $Contract $Expected.Contract.Bytes $Expected.Contract.Sha256 'Recovery02 authoring contract'
...
if ($OfflineContractTest) {
    ...
    [ordered]@{
        classification = 'PASSED_OFFLINE_CONTRACT_READY_FOR_SINGLE_STAGE04_RECOVERY02_UNREAL_AUTHORING'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage04-recovery02.authoring01.supervisor.v1'
...
try {
    ...
    Copy-Item -LiteralPath $Author -Destination (Join-Path $AttemptRoot 'author_m01_visible_environment_stage04_recovery02.py')
    Copy-Item -LiteralPath $Contract -Destination (Join-Path $AttemptRoot 'stage04_recovery02_authoring_contract.json')
    ...
    if (-not $process.HasExited) {
        ...
        throw "Unreal Stage04 Recovery02 authoring exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit(); $process.Refresh()
    $state.exit_code = [int]$process.ExitCode
    $state.exit_code_type = $process.ExitCode.GetType().FullName
    if ($state.exit_code -ne 0) { throw "Unreal Stage04 Recovery02 authoring returned $($state.exit_code)" }
    if ($state.exit_code_type -ne 'System.Int32') { throw "Unexpected exit-code type: $($state.exit_code_type)" }
    if (-not [System.IO.File]::Exists($Receipt)) { throw "Recovery02 receipt missing: $Receipt" }
    ...
    if ($state.receipt_classification -ne 'PASSED_STAGE04_RECOVERY02_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF') { throw "Receipt failed: $($state.receipt_classification)" }
    if (-not [System.IO.File]::Exists($OutputMap)) { throw 'Stage04 Recovery02 output map missing' }
    Assert-File $InputMap $Expected.InputMap.Bytes $Expected.InputMap.Sha256 'Stage03 map after Recovery02 authoring'
    Assert-File $FailedTerminal $Expected.FailedTerminal.Bytes $Expected.FailedTerminal.Sha256 'Failed Stage04 terminal after Recovery02 authoring'
    ...
    $state.classification = 'PASSED_STAGE04_RECOVERY02_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF'
