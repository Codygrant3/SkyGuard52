...
$OutputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage07BMaterialTerrainDistrictVariety01.umap'
$Destination = 'D:\SG52T08_ENV01\Content\M01\VisibleEnvironmentStage07BMaterialTerrainDistrictVariety01'
$StandingAuthorization = Join-Path $Root 'Production\standing_heavy_process_authorization.json'
$Author = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage07b_material_terrain_district_variety01\author_m01_visible_environment_stage07b_material_terrain_district_variety01.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage07b_material_terrain_district_variety01\stage07b_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_AUTHORING01\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_AUTHORING01_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_AUTHORING01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 2400
...
if ($OfflineContractTest) {
    ...
    [ordered]@{
        classification = 'PASSED_OFFLINE_READY_FOR_SINGLE_STAGE07B_AUTHORING'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage07b-material-terrain-district-variety01.authoring01.supervisor.v1'
...
try {
    ...
    $arguments = @(
        $Project,
        '-Unattended', '-NoSplash', '-NoSound', '-d3d12', '-NoSaveOnExit',
    ...
    if (-not $process.HasExited) {
        ...
        throw "Stage7B authoring exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit()
    $process.Refresh()
    ...
    if ($state.exit_code -ne 0 -or $state.exit_code_type -ne 'System.Int32') {
        throw "Stage7B authoring returned $($state.exit_code) $($state.exit_code_type)"
    }

    if (-not [IO.File]::Exists($Receipt)) { throw 'Stage7B authoring receipt missing' }
    ...
    $expected = 'PASSED_STAGE07B_AUTHORING_AWAITING_CHECKPOINT_VISUAL'
    if ($state.receipt_classification -ne $expected) { throw "Stage7B authoring receipt failed: $($state.receipt_classification)" }
    if (-not [IO.File]::Exists($OutputMap)) { throw 'Stage7B output map missing' }
    $contractObject = Get-Content -LiteralPath $Contract -Raw | ConvertFrom-Json
