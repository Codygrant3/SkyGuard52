"""Bind the proven automatic postflight adjudicator to the Stage07A proof."""
...
SOURCE = Path(
    r"D:\Skyguard52\Scripts\ToolchainWave08\m01_visible_environment_stage06_district_correction01_visual_proof01\adjudicate_m01_visible_environment_stage06_district_correction01_visual_proof01_once.py"
)
...
def transform_source() -> str:
    ...
    for old, new in replacements:
        if old not in transformed and old not in optional:
            raise RuntimeError(f"Stage07A adjudicator token absent: {old}")
