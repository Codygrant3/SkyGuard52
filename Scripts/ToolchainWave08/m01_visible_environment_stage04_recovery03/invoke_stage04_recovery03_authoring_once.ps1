...
$OutputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage04Recovery03.umap'
$Destination = 'D:\SG52T08_ENV01\Content\M01\VisibleEnvironmentStage04Recovery03'
...
$Author = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage04_recovery03\author_m01_visible_environment_stage04_recovery03.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage04_recovery03\stage04_recovery03_authoring_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_RECOVERY03\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_RECOVERY03_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_RECOVERY03_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1200
...
$Expected = @{
    ...
    Author = @{ Bytes = 26118; Sha256 = 'df96240f6e16a6267a40b1e2f3449c82b0182d4fd9260d3a62bf794fb10ea1e3' }
    Contract = @{ Bytes = 7024; Sha256 = 'fe3282b612616cce532a6e4f87bfdab14e009f480bc78e7c1829c5e093f07499' }
}
...
function Assert-Authorities {
    ...
    Assert-File $Author $Expected.Author.Bytes $Expected.Author.Sha256 'Recovery03 authoring source'
    Assert-File $Contract $Expected.Contract.Bytes $Expected.Contract.Sha256 'Recovery03 authoring contract'
...
if ($OfflineContractTest) {
    ...
    [ordered]@{
        classification = 'PASSED_OFFLINE_CONTRACT_READY_FOR_SINGLE_STAGE04_RECOVERY03_UNREAL_AUTHORING'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage04-recovery03.authoring01.supervisor.v1'
...
try {
    ...
    Copy-Item -LiteralPath $Author -Destination (Join-Path $AttemptRoot 'author_m01_visible_environment_stage04_recovery03.py')
    Copy-Item -LiteralPath $Contract -Destination (Join-Path $AttemptRoot 'stage04_recovery03_authoring_contract.json')
    ...
    if (-not $process.HasExited) {
        ...
        throw "Unreal Stage04 Recovery03 authoring exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit(); $process.Refresh()
    $state.exit_code = [int]$process.ExitCode
    $state.exit_code_type = $process.ExitCode.GetType().FullName
    if ($state.exit_code -ne 0) { throw "Unreal Stage04 Recovery03 authoring returned $($state.exit_code)" }
    if ($state.exit_code_type -ne 'System.Int32') { throw "Unexpected exit-code type: $($state.exit_code_type)" }
    if (-not [System.IO.File]::Exists($Receipt)) { throw "Recovery03 receipt missing: $Receipt" }
    ...
    if ($state.receipt_classification -ne 'PASSED_STAGE04_RECOVERY03_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF') { throw "Receipt failed: $($state.receipt_classification)" }
    if (-not [System.IO.File]::Exists($OutputMap)) { throw 'Stage04 Recovery03 output map missing' }
    Assert-File $InputMap $Expected.InputMap.Bytes $Expected.InputMap.Sha256 'Stage03 map after Recovery03 authoring'
    Assert-File $FailedTerminal $Expected.FailedTerminal.Bytes $Expected.FailedTerminal.Sha256 'Failed Stage04 terminal after Recovery03 authoring'
    ...
    $state.classification = 'PASSED_STAGE04_RECOVERY03_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF'
