...
HERE = ROOT / "Scripts/ToolchainWave08/m01_visible_environment_stage04_recovery03"
CONTRACT_PATH = HERE / "stage04_recovery03_authoring_contract.json"
...
OUTPUT_ASSET = "/Game/M01/Lvl_M01_VisibleEnvironmentStage04Recovery03"
INPUT_FILE = ISOLATED / "Content/M01/Lvl_M01_VisibleEnvironmentStage03.umap"
OUTPUT_FILE = ISOLATED / "Content/M01/Lvl_M01_VisibleEnvironmentStage04Recovery03.umap"
DESTINATION = "/Game/M01/VisibleEnvironmentStage04Recovery03"
DESTINATION_DISK = ISOLATED / "Content/M01/VisibleEnvironmentStage04Recovery03"
ATTEMPT = ROOT / "Saved/BuildAttempts/M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_RECOVERY03/attempt_01"
...
def load_contract() -> dict[str, object]:
    ...
    require(contract["contract_id"] == "M01-VISIBLE-ENVIRONMENT-STAGE04-AUTHORING01-RECOVERY03", "Recovery03 contract identity changed")
    return contract
...
def assert_fresh() -> None:
    require(not DESTINATION_DISK.exists(), "Fresh Recovery03 Unreal destination exists")
    require(not OUTPUT_FILE.exists(), "Fresh Recovery03 output map exists")
    require(not ATTEMPT.exists(), "Fresh Recovery03 attempt exists")
...
def offline_contract_test() -> int:
    ...
    print("PASS_M01_VISIBLE_ENVIRONMENT_STAGE04_RECOVERY03_AUTHORING_CONTRACT")
    return 0
...
def spawn_group(actors_api: object, meshes: dict[str, object], prefix: str, location: list[float], rotation: list[float], unreal: object) -> list[dict[str, object]]:
    ...
    for name, mesh in sorted(meshes.items()):
        ...
        actor.set_folder_path("M01/VisibleEnvironmentStage04Recovery03")
        component = actor.get_component_by_class(unreal.StaticMeshComponent)
...
def run_unreal() -> None:
    ...
    result: dict[str, object] = {
        "schema": "skyguard.m01-visible-environment-stage04-recovery03.authoring01.v1",
    ...
    try:
        ...
        require(not DESTINATION_DISK.exists() and not unreal.EditorAssetLibrary.does_directory_exist(DESTINATION), "Fresh Recovery03 destination exists")
        require(not OUTPUT_FILE.exists() and not unreal.EditorAssetLibrary.does_asset_exist(OUTPUT_ASSET), "Fresh Recovery03 map exists")
        ...
        require(levels.new_level_from_template(OUTPUT_ASSET, INPUT_ASSET), "Failed to clone Stage03 into fresh Stage04 Recovery03")
        ...
        for column, x_cm in enumerate(facade["column_centers_x_cm"]):
            for floor in range(int(facade["floor_count"])):
                ...
                created += spawn_group(actors_api, facade_meshes, f"M01_STAGE04R03_Facade_C{column:02d}_F{floor:02d}", location, [0.0, 0.0, float(facade["yaw_degrees"])], unreal)
        lighthouse = contract["placement"]["lighthouse"]
        created += spawn_group(actors_api, lighthouse_meshes, "M01_STAGE04R03_LighthouseHero", lighthouse["asset_location_cm"], lighthouse["asset_rotation_degrees"], unreal)
        require(len(created) == 35, f"Expected 35 Recovery03 accepted-asset actors; found {len(created)}")
        ...
        require(len(result["vegetation_adjustments"]) == 28, "Expected 28 Recovery03 vegetation adjustments")
        ...
        require(sun is not None and fill is not None and sky is not None, "Recovery03 lighting components unavailable")
        ...
        require(len(actors_after) == int(contract["output"]["expected_actor_count"]), f"Recovery03 actor count changed: {len(actors_after)}")
        require(levels.save_current_level(), "Failed to save fresh Stage04 Recovery03 map")
        require(OUTPUT_FILE.is_file(), "Fresh Stage04 Recovery03 map was not created")
        ...
        result["classification"] = "PASSED_STAGE04_RECOVERY03_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF"
    ...
    if str(result["classification"]).startswith("PASSED_"):
        ...
        raise RuntimeError(result["error"] or "Stage04 Recovery03 authoring failed")
