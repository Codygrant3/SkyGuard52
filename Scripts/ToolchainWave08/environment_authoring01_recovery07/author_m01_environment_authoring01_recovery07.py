...
ATTEMPT_ROOT = r"D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07\attempt_01"
...
OUTPUT_FILE = r"D:\SG52T08_ENV01\Content\ToolchainWave08\Environment\Lvl_M01_T08_EnvironmentAuthoring01_Recovery07.umap"
...
AUTHORING_PREFIX = "/Game/ToolchainWave08/Environment/Authoring01_Recovery07"
...
result = {
    "schema": "skyguard.toolchain-wave08.m01-environment-authoring01-recovery07.receipt.v1",
...
try:
    ...
    require(len(directors) <= 1, f"Expected zero or one Mission 1 environment director; found {len(directors)}")
    if len(directors) == 0:
        ...
        director_action = "SPAWNED_DETERMINISTICALLY_IN_FRESH_OUTPUT"
    else:
        director = directors[0]
        director.set_actor_label("M01_A01_EnvironmentDirector")
        director_action = "REUSED_SINGLE_EXISTING_OUTPUT_DIRECTOR"
    require(director.get_class().get_path_name() == director_class_path, "Environment director class identity mismatch")
    result["director_acquisition"] = {
        "before_count": len(directors),
        "after_count": 1,
        "action": director_action,
        "class_path": director_class_path,
        "label": director.get_actor_label(),
        "location_cm": [director.get_actor_location().x, director.get_actor_location().y, director.get_actor_location().z],
    }
    ...
    before_save_allowed = {OUTPUT_ASSET, OUTPUT_ASSET + ".Lvl_M01_T08_EnvironmentAuthoring01_Recovery07"}
    ...
    result["classification"] = "PASSED_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_AUTOMATIC"
except Exception as exc:
...
if result["classification"] != "PASSED_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_AUTOMATIC":
