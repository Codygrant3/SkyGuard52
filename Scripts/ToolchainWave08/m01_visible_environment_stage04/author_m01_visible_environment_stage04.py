"""Import accepted Stage04 candidates and assemble a fresh Mission 1 map."""
...
CONTRACT_PATH = ROOT / "Scripts/ToolchainWave08/m01_visible_environment_stage04/stage04_authoring_contract.json"
...
OUTPUT_ASSET = "/Game/M01/Lvl_M01_VisibleEnvironmentStage04"
INPUT_FILE = ISOLATED / "Content/M01/Lvl_M01_VisibleEnvironmentStage03.umap"
OUTPUT_FILE = ISOLATED / "Content/M01/Lvl_M01_VisibleEnvironmentStage04.umap"
DESTINATION = "/Game/M01/VisibleEnvironmentStage04"
DESTINATION_DISK = ISOLATED / "Content/M01/VisibleEnvironmentStage04"
ATTEMPT = ROOT / "Saved/BuildAttempts/M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01/attempt_01"
...
def validate_glb(path: Path, authority: dict[str, object], expected_meshes: dict[str, int]) -> dict[str, object]:
    ...
    for mesh in document.get("meshes", []):
        ...
        if name.startswith("UCX_"):
            ...
            observed[name] = len(mesh.get("primitives", []))
    require(observed == expected_meshes, f"Renderable GLB contract changed: {path.name}: {observed}")
    require(collision, f"Collision contract missing: {path}")
    return {"source": record(path), "render_meshes": observed, "collision_meshes": sorted(collision)}


def load_contract() -> dict[str, object]:
    contract = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
    require(contract["contract_id"] == "M01-VISIBLE-ENVIRONMENT-STAGE04-AUTHORING01", "Stage04 contract identity changed")
    return contract
...
def validate_authorities(contract: dict[str, object]) -> dict[str, object]:
    ...
    for key in ("metadata_receipt", "scene_inventory"):
        spec = contract["accepted_sources"][key]
        path = Path(spec["file"])
        require(path.is_file() and path.stat().st_size == int(spec["bytes"]) and sha256(path) == str(spec["sha256"]), f"Accepted authority changed: {key}")
    return {
        "facade": validate_glb(FACADE_SOURCE, contract["accepted_sources"]["facade_glb"], FACADE_MESHES),
        "lighthouse": validate_glb(LIGHTHOUSE_SOURCE, contract["accepted_sources"]["lighthouse_glb"], LIGHTHOUSE_MESHES),
    }
...
def offline_contract_test() -> int:
    ...
    require(not DESTINATION_DISK.exists(), "Fresh Stage04 Unreal destination exists")
    require(not OUTPUT_FILE.exists(), "Fresh Stage04 output map exists")
    require(not ATTEMPT.exists(), "Fresh Stage04 attempt exists")
    compile(Path(__file__).read_text(encoding="utf-8"), __file__, "exec")
    print("PASS_M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING_CONTRACT")
    return 0
...
def slot_name(slot: object) -> str:
    material = slot.get_editor_property("material_interface")
    return "" if material is None else material.get_name()
...
def import_source(source: Path, destination: str, expected_meshes: dict[str, int], unreal: object) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    for asset_path in paths:
        ...
        if isinstance(asset, unreal.StaticMesh):
            ...
            row.update({"bounds_extent_cm": vector(extent), "material_slot_count": len(slots), "material_names": [slot_name(slot) for slot in slots]})
            if asset.get_name() in expected_meshes:
                require(len(slots) == expected_meshes[asset.get_name()], f"Material slot count changed: {asset.get_name()}")
                meshes[asset.get_name()] = asset
        rows.append(row)
    require(set(meshes) == set(expected_meshes), f"Imported semantic meshes changed: {sorted(meshes)}")
    try:
...
def spawn_group(actors_api: object, meshes: dict[str, object], prefix: str, location: list[float], rotation: list[float], unreal: object) -> list[dict[str, object]]:
    ...
    for name, mesh in sorted(meshes.items()):
        actor = actors_api.spawn_actor_from_class(
            ...
            unreal.Rotator(pitch=float(rotation[0]), yaw=float(rotation[2]), roll=float(rotation[1])),
            False,
        )
        require(actor is not None, f"Failed to spawn {name}")
        actor.set_actor_label(f"{prefix}_{name.removeprefix('SM_M01_')}")
        actor.set_folder_path("M01/VisibleEnvironmentStage04")
        component = actor.get_component_by_class(unreal.StaticMeshComponent)
...
def scale_and_ground_vegetation(actor: object, multiplier: float, unreal: object) -> dict[str, object]:
    ...
    return {
        ...
        "scale_before": vector(before_scale),
        "scale_after": vector(actor.get_actor_scale3d()),
...
def run_unreal() -> None:
    ...
    result: dict[str, object] = {
        "schema": "skyguard.m01-visible-environment-stage04.authoring01.v1",
    ...
    try:
        ...
        require(not DESTINATION_DISK.exists() and not unreal.EditorAssetLibrary.does_directory_exist(DESTINATION), "Fresh Stage04 destination exists")
        require(not OUTPUT_FILE.exists() and not unreal.EditorAssetLibrary.does_asset_exist(OUTPUT_ASSET), "Fresh Stage04 output map exists")

        facade_destination = DESTINATION + "/FacadeBayR02"
        lighthouse_destination = DESTINATION + "/LighthouseR04"
        facade_meshes, facade_rows = import_source(FACADE_SOURCE, facade_destination, FACADE_MESHES, unreal)
        lighthouse_meshes, lighthouse_rows = import_source(LIGHTHOUSE_SOURCE, lighthouse_destination, LIGHTHOUSE_MESHES, unreal)
        result["imported_assets"] = {"facade": facade_rows, "lighthouse": lighthouse_rows}

        # Calibrate the accepted facade trim away from the dark Blender review response.
        structure = facade_meshes["SM_M01_CoastalFacadeBay_A_StructureFrame"]
        old_limestone = unreal.load_asset("/Game/M01/EnvKit02/M01_LIGHTHOUSE_A/Materials/M_ENV_Plaster_Warm_2K")
        old_concrete = unreal.load_asset("/Game/M01/CoastalCorridorC06R01/M01_CoastalCorridor_C06R01_UNREAL_READY/Materials/M_M01_C06_WeatheredConcrete")
        require(old_limestone is not None and old_concrete is not None, "Accepted local calibration materials failed to load")
        calibration = []
        for mesh in facade_meshes.values():
            ...
            for index, slot in enumerate(slots):
                name = slot_name(slot)
                replacement = None
                if name == "M_M01_CoastalFacadeBay_R02_PaleLimestone":
                    replacement = old_limestone
                elif name == "M_M01_CoastalFacadeBay_R02_WeatheredPlinth":
                    replacement = old_concrete
                if replacement is not None:
                    mesh.set_material(index, replacement)
                    calibration.append({"mesh": mesh.get_name(), "slot": index, "source": name, "replacement": object_path(replacement)})
            mesh.post_edit_change()
        require(calibration, "Facade material calibration found no governed slots")
        result["facade_material_calibration"] = calibration
        try:
        ...
        require(levels.new_level_from_template(OUTPUT_ASSET, INPUT_ASSET), "Failed to clone Stage03 into fresh Stage04")
        ...
        facade_placement = contract["placement"]["facade"]
        lighthouse_placement = contract["placement"]["lighthouse"]
        created = []
        created += spawn_group(actors_api, facade_meshes, "M01_STAGE04_FacadeHero", facade_placement["asset_location_cm"], facade_placement["asset_rotation_degrees"], unreal)
        created += spawn_group(actors_api, lighthouse_meshes, "M01_STAGE04_LighthouseHero", lighthouse_placement["asset_location_cm"], lighthouse_placement["asset_rotation_degrees"], unreal)
        require(len(created) == 7, f"Expected seven Stage04 accepted-asset actors; found {len(created)}")
        ...
        require(len(result["vegetation_adjustments"]) == 28, "Expected 28 Stage04 vegetation adjustments")
        ...
        require(sun is not None and fill is not None and sky is not None, "Stage04 lighting components unavailable")
        ...
        require(len(actors_after) == int(contract["output"]["expected_actor_count"]), f"Stage04 actor count changed: {len(actors_after)}")
        require(levels.save_current_level(), "Failed to save fresh Stage04 map")
        require(OUTPUT_FILE.is_file(), "Fresh Stage04 output map was not created")
        ...
        result["classification"] = "PASSED_STAGE04_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF"
    ...
    if str(result["classification"]).startswith("PASSED_"):
        ...
        raise RuntimeError(result["error"] or "Stage04 authoring failed")
