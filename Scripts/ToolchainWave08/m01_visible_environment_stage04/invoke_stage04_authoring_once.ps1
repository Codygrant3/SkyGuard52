...
$OutputMap = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_VisibleEnvironmentStage04.umap'
$Destination = 'D:\SG52T08_ENV01\Content\M01\VisibleEnvironmentStage04'
$StandingAuthorization = Join-Path $Root 'Production\standing_heavy_process_authorization.json'
$Author = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage04\author_m01_visible_environment_stage04.py'
$Contract = Join-Path $Root 'Scripts\ToolchainWave08\m01_visible_environment_stage04\stage04_authoring_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_VISIBLE_ENVIRONMENT_STAGE04_AUTHORING01_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1200
...
$Expected = @{
    ...
    Author = @{ Bytes = 22326; Sha256 = '90618ead676418df7ce0decc38b7557fcd71347cd163d1fd9baf779157dcd20e' }
    Contract = @{ Bytes = 3302; Sha256 = '6342ab46bbb21d1b2ef96d0fc0a384f578c1d1b8be8f6f87a8a5f2880743d7cd' }
}
...
function Assert-Authorities {
    ...
    Assert-File $Author $Expected.Author.Bytes $Expected.Author.Sha256 'Stage04 authoring source'
    Assert-File $Contract $Expected.Contract.Bytes $Expected.Contract.Sha256 'Stage04 authoring contract'
...
if ($OfflineContractTest) {
    ...
    [ordered]@{
        classification = 'PASSED_OFFLINE_CONTRACT_READY_FOR_SINGLE_STAGE04_UNREAL_AUTHORING'
...
$state = [ordered]@{
    schema = 'skyguard.m01-visible-environment-stage04.authoring01.supervisor.v1'
...
try {
    ...
    Copy-Item -LiteralPath $Author -Destination (Join-Path $AttemptRoot 'author_m01_visible_environment_stage04.py')
    Copy-Item -LiteralPath $Contract -Destination (Join-Path $AttemptRoot 'stage04_authoring_contract.json')
    ...
    if (-not $process.HasExited) {
        ...
        throw "Unreal Stage04 authoring exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit(); $process.Refresh()
    $state.exit_code = [int]$process.ExitCode
    $state.exit_code_type = $process.ExitCode.GetType().FullName
    if ($state.exit_code -ne 0) { throw "Unreal Stage04 authoring returned $($state.exit_code)" }
    if ($state.exit_code_type -ne 'System.Int32') { throw "Unexpected exit-code type: $($state.exit_code_type)" }
    if (-not [System.IO.File]::Exists($Receipt)) { throw "Stage04 receipt missing: $Receipt" }
    ...
    if ($state.receipt_classification -ne 'PASSED_STAGE04_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF') { throw "Receipt failed: $($state.receipt_classification)" }
    if (-not [System.IO.File]::Exists($OutputMap)) { throw 'Stage04 output map missing' }
    Assert-File $InputMap $Expected.InputMap.Bytes $Expected.InputMap.Sha256 'Stage03 map after Stage04 authoring'
    ...
    $state.classification = 'PASSED_STAGE04_AUTHORING_AWAITING_GOVERNED_D3D12_VISUAL_PROOF'
