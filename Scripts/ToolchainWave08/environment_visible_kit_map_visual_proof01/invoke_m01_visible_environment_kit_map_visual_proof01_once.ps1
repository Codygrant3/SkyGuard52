...
$replacements = @(
    @('M01_ENVIRONMENT_REALISM_STACK_VISUAL_PROOF01', 'M01_VISIBLE_ENVIRONMENT_KIT_MAP_VISUAL_PROOF01'),
    @('M01-ENVIRONMENT-REALISM-STACK-VISUAL-PROOF01', 'M01-VISIBLE-ENVIRONMENT-KIT-MAP-VISUAL-PROOF01'),
    @('Lvl_M01_T08_EnvironmentRealismStack01_Recovery02', 'Lvl_M01_VisibleEnvironmentKit02_Recovery02'),
    @('M01EnvironmentRealismStackVisualProof01.csv', 'M01VisibleEnvironmentKitMapVisualProof01.csv'),
    @('capture_m01_environment_realism_stack_visual_proof01.py', 'capture_m01_visible_environment_kit_map_visual_proof01.py'),
    @('adjudicate_m01_environment_realism_stack_visual_proof01_once.py', 'adjudicate_m01_visible_environment_kit_map_visual_proof01_once.py'),
    @('m01_environment_realism_stack_visual_proof01', 'm01_visible_environment_kit_map_visual_proof01'),
    @('environment_realism_stack_visual_proof01', 'environment_visible_kit_map_visual_proof01'),
    @('visual-proof01', 'visible-environment-kit-map-visual-proof01'),
    @('PASSED_READY_FOR_STANDING_AUTHORIZED_SINGLE_M01_ENVIRONMENT_REALISM_STACK_VISUAL_PROOF01_EXECUTION', 'PASSED_READY_FOR_STANDING_AUTHORIZED_SINGLE_M01_VISIBLE_ENVIRONMENT_KIT_MAP_VISUAL_PROOF01_EXECUTION')
)
...
foreach ($pair in $replacements) {
    if ($pair[0] -ne 'visual-proof01' -and $transformed.Contains($pair[0])) { throw "Visible-kit supervisor retained stale token: $($pair[0])" }
}
foreach ($required in @('M01_VISIBLE_ENVIRONMENT_KIT_MAP_VISUAL_PROOF01', 'M01-VISIBLE-ENVIRONMENT-KIT-MAP-VISUAL-PROOF01', 'Lvl_M01_VisibleEnvironmentKit02_Recovery02', 'M01VisibleEnvironmentKitMapVisualProof01.csv', 'environment_visible_kit_map_visual_proof01', '/Game/M01/', '$timeoutSeconds = 1200')) {
    if (-not $transformed.Contains($required)) { throw "Visible-kit supervisor output binding is missing: $required" }
}
