"""Bind the proven automatic postflight adjudicator to VisibleEnvironmentKit02."""
...
SOURCE = Path(r"D:\Skyguard52\Scripts\ToolchainWave08\environment_realism_stack_visual_proof02\adjudicate_m01_environment_realism_stack_visual_proof02_once.py")
EXPECTED_BYTES = 2340
EXPECTED_SHA256 = "0300bdc2f55e42af4661d35cb6e9e5ad611afb6260dd30c94a77dc4f0a9f4be8"
...
def transform_source() -> str:
    if not SOURCE.is_file() or SOURCE.stat().st_size != EXPECTED_BYTES or sha256(SOURCE) != EXPECTED_SHA256:
        raise RuntimeError("Frozen Visual Proof02 adjudicator wrapper changed")
    namespace = {"__name__": "m01_visual_proof02_adjudicator_authority"}
    ...
    for stale, _ in replacements:
        if stale in transformed:
            raise RuntimeError(f"Visible-kit adjudicator retained stale token: {stale}")
    for token in (
        ...
        if token not in transformed:
            raise RuntimeError(f"Visible-kit adjudicator output binding is missing: {token}")
    return transformed
