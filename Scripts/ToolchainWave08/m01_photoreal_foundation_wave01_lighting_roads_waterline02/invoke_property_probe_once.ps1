...
$MapFile = 'D:\SG52T08_ENV01\Content\M01\Lvl_M01_PhotorealFoundation_NonVegetation01.umap'
$StandingAuthorization = 'D:\Skyguard52\Production\standing_heavy_process_authorization.json'
$Probe = 'D:\Skyguard52\Scripts\ToolchainWave08\m01_photoreal_foundation_wave01_lighting_roads_waterline02\probe_map_properties.py'
$AttemptRoot = 'D:\Skyguard52\Saved\BuildAttempts\M01_PHOTOREAL_FOUNDATION_WAVE01_LIGHTING_ROADS_WATERLINE02_PROPERTY_PROBE\attempt_01'
$Receipt = Join-Path $AttemptRoot 'property_probe_receipt.json'
$TerminalManifest = 'D:\Skyguard52\Saved\Reports\M01_PHOTOREAL_FOUNDATION_WAVE01_LIGHTING_ROADS_WATERLINE02_PROPERTY_PROBE_TERMINAL_MANIFEST.json'
$EmergencyReceipt = 'D:\Skyguard52\Saved\Reports\M01_PHOTOREAL_FOUNDATION_WAVE01_LIGHTING_ROADS_WATERLINE02_PROPERTY_PROBE_EMERGENCY_RECEIPT.jsonl'
$TimeoutSeconds = 1200
...
$Expected = @{
    ...
    Map = @{ Bytes = 736476; Sha256 = '618a260a905680cf5b17c1ac82a114a69f93f947334f45701cd1a8daa2b1f2a1' }
    StandingAuthorization = @{ Bytes = 2146; Sha256 = '48277d7edd869cc2b841c3993e8f7599bec5291ddf23e9c442b33e09b870c089' }
    Probe = @{ Bytes = 14373; Sha256 = 'a3e61eb8380fd974331c11ab14bca8f3978a027f35b1122c4c3cab2398221cbb' }
}
...
function Assert-Authorities {
    ...
    Assert-File $MapFile $Expected.Map.Bytes $Expected.Map.Sha256 'NonVegetation01 map'
    ...
    Assert-File $Probe $Expected.Probe.Bytes $Expected.Probe.Sha256 'Property probe source'
...
if ($OfflineContractTest) {
    ...
    foreach ($token in @('cross_streets', 'M01_RS01_CrossStreet_', 'M01_PR01_FillSun', 'post_process', 'water', 'candidate_materials', 'map_unchanged')) {
        if (-not $source.Contains($token)) { throw "Probe token missing: $token" }
    }
    [pscustomobject]@{
        classification = 'PASSED_OFFLINE_CONTRACT_READY_FOR_STANDING_AUTHORIZED_SINGLE_PROPERTY_PROBE'
...
$state = [ordered]@{
    schema = 'skyguard.m01-photoreal-foundation-wave01.lighting-roads-waterline02-property-probe-supervisor.v1'
...
try {
    ...
    if (-not $process.HasExited) {
        ...
        throw "Unreal property probe exceeded $TimeoutSeconds seconds"
    }
    $process.WaitForExit(); $process.Refresh()
    $state.exit_code = [int]$process.ExitCode
    $state.exit_code_type = $process.ExitCode.GetType().FullName
    if ($state.exit_code -ne 0) { throw "Unreal property probe returned $($state.exit_code)" }
    if ($state.exit_code_type -ne 'System.Int32') { throw "Unexpected exit-code type: $($state.exit_code_type)" }
    if (-not [System.IO.File]::Exists($Receipt)) { throw "Property probe receipt missing: $Receipt" }
    ...
    if ($state.receipt_classification -ne 'PASSED_READY_FOR_LIGHTING_ROADS_WATERLINE02_AUTHORING_DESIGN') { throw "Receipt failed: $($state.receipt_classification)" }
    if (-not $state.map_unchanged) { throw 'Property probe changed NonVegetation01 map' }
    Assert-File $MapFile $Expected.Map.Bytes $Expected.Map.Sha256 'NonVegetation01 map after probe'
    $state.classification = 'PASSED_READY_FOR_LIGHTING_ROADS_WATERLINE02_AUTHORING_DESIGN'
