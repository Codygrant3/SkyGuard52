...
$ScriptRoot = Join-Path $ProjectRoot 'Scripts\ToolchainWave08\environment_authoring01_recovery04'
...
$RecoveryOutput = Join-Path $IsolatedRoot 'Content\ToolchainWave08\Environment\Lvl_M01_T08_EnvironmentAuthoring01_Recovery04.umap'
...
function Get-TemporaryTestRoot([string]$Kind) {
    $base = Join-Path ([System.IO.Path]::GetTempPath()) 'Skyguard52\ToolchainWave08\EnvironmentAuthoring01Recovery04'
...
function Get-FutureGovernedPaths {
    return @(
        'D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04_DEPENDENCY_PROBE\attempt_01',
        'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04_DEPENDENCY_PROBE_TERMINAL_MANIFEST.json',
        'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04_DEPENDENCY_PROBE_EMERGENCY_RECEIPT.jsonl',
        'D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04\attempt_01',
        'D:\SG52T08_ENV01\Content\ToolchainWave08\Environment\Lvl_M01_T08_EnvironmentAuthoring01_Recovery04.umap'
    )
}
...
function Invoke-OfflineContractTest {
    ...
    try {
        Write-JsonAtomic $successTerminal ([ordered]@{schema='skyguard.recovery04.offline-success.v1';classification='PASS';exit_code=[int]0;exit_code_type=([int]0).GetType().FullName;unreal_launch_count=0;retry_count=0})
    ...
    try {
        ...
        Write-JsonAtomic $failureTerminal ([ordered]@{schema='skyguard.recovery04.offline-induced-failure.v1';classification='EXPECTED_FAILURE';failure=$_.Exception.Message;unreal_launch_count=0;retry_count=0})
    }
    try {
        ...
        Add-Content -LiteralPath $emergency -Value (([ordered]@{schema='skyguard.recovery04.offline-emergency.v1';classification='EXPECTED_EMERGENCY';failure=$_.Exception.Message;unreal_launch_count=0}) | ConvertTo-Json -Compress)
    }
    ...
    $result = [ordered]@{schema='skyguard.toolchain-wave08.m01-authoring01-recovery04.offline-test.v1';classification=if($failures.Count-eq0){'PASS'}else{'FAIL'};test_root=$testRoot;success_terminal=$successTerminal;failure_terminal=$failureTerminal;emergency_receipt=$emergency;unreal_launch_count=0;retry_count=0;governed_before=$governedBefore;governed_after=$governedAfter;failures=$failures}
    Write-JsonAtomic $summary $result
    Write-Host "OFFLINE_TEST_ROOT=$testRoot"
    Write-Host "OFFLINE_TEST_SUMMARY=$summary"
    if ($failures.Count -ne 0) { return [int]1 }
    Write-Host 'PASS_OFFLINE_CONTRACT_TEST'
    return [int]0
}
...
function Invoke-AuthorizationRefusal {
    ...
    Write-JsonAtomic $receipt ([ordered]@{schema='skyguard.recovery04.authorization-refusal.v1';classification='AUTHORIZATION_REQUIRED';unreal_launch_count=0;retry_count=0})
    Write-Host "AUTHORIZATION_REFUSAL_RECEIPT=$receipt"
    Write-Host 'AUTHORIZATION_REQUIRED'
    return [int]2
}
...
if ($OfflineContractTest) {
    ...
    if ($null -eq $testCode) { throw 'Offline test returned a null exit code' }
    if ($testCode -is [System.Array] -or $testCode -is [System.Collections.ICollection]) { throw 'Offline test returned a collection instead of a scalar exit code' }
    if (-not ($testCode -is [int])) { throw "Offline test exit-code type is $($testCode.GetType().FullName), expected System.Int32" }
    [Environment]::Exit($testCode)
}
if (-not $AuthorizeSingleDependencyProbe) {
    $refusalCode = Invoke-AuthorizationRefusal
    if ($null -eq $refusalCode) { throw 'Authorization refusal returned a null exit code' }
    if ($refusalCode -is [System.Array] -or $refusalCode -is [System.Collections.ICollection]) { throw 'Authorization refusal returned a collection instead of a scalar exit code' }
    if (-not ($refusalCode -is [int])) { throw "Authorization refusal exit-code type is $($refusalCode.GetType().FullName), expected System.Int32" }
    [Environment]::Exit($refusalCode)
}

# Governed paths and the outer terminal lifecycle exist only in authorized mode.
$Attempt = 'D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04_DEPENDENCY_PROBE\attempt_01'
$Terminal = 'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04_DEPENDENCY_PROBE_TERMINAL_MANIFEST.json'
$Emergency = 'D:\Skyguard52\Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04_DEPENDENCY_PROBE_EMERGENCY_RECEIPT.jsonl'
$FutureAuthoringAttempt = 'D:\Skyguard52\Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY04\attempt_01'
...
try {
    ...
    if(Test-Path -LiteralPath $RecoveryOutput){throw 'Recovery04 output appeared during read-only probe'}
    $classification='PASSED_DEPENDENCY_PROBE_READY_FOR_RECOVERY04_FREEZE'
} catch {
    ...
    try {
        ...
        $terminalValue=[ordered]@{schema='skyguard.toolchain-wave08.m01-authoring01-recovery04.dependency-probe-terminal.v1';classification=$classification;started_at_utc=$started.ToString('o');completed_at_utc=[DateTime]::UtcNow.ToString('o');preflight_passed=$preflightPassed;unreal_launch_count=if($null-eq$run){0}else{1};retry_count=0;exit_code=if($null-eq$run){$null}else{$run.ExitCode};exit_code_type=if($null-eq$run){$null}else{$run.ExitCodeType};timed_out=if($null-eq$run){$false}else{$run.TimedOut};failure=$failure;input_hash_after=if(Test-Path -LiteralPath $InputMap){Get-Sha256 $InputMap}else{$null};original_output_exists=(Test-Path -LiteralPath $OriginalOutput);recovery_output_exists=(Test-Path -LiteralPath $RecoveryOutput);artifacts=$artifacts}
...
if($classification-ne'PASSED_DEPENDENCY_PROBE_READY_FOR_RECOVERY04_FREEZE'){[Environment]::Exit([int]1)}
