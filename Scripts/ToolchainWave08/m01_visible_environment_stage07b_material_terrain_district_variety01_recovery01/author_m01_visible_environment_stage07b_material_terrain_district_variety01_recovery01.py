"""Author Stage 7B Recovery01: same corridor corrections, no Cmd screenshot capture."""
...
OUTPUT_ASSET = "/Game/M01/Lvl_M01_VisibleEnvironmentStage07BMaterialTerrainDistrictVariety01Recovery01"
INPUT_FILE = ISOLATED / "Content/M01/Lvl_M01_VisibleEnvironmentStage07AHeroCorridor01Correction01.umap"
OUTPUT_FILE = ISOLATED / "Content/M01/Lvl_M01_VisibleEnvironmentStage07BMaterialTerrainDistrictVariety01Recovery01.umap"
DESTINATION = "/Game/M01/VisibleEnvironmentStage07BMaterialTerrainDistrictVariety01Recovery01"
DESTINATION_DISK = ISOLATED / "Content/M01/VisibleEnvironmentStage07BMaterialTerrainDistrictVariety01Recovery01"
ATTEMPT = ROOT / "Saved/BuildAttempts/M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_RECOVERY01_AUTHORING01/attempt_01"
...
def offline_contract_test() -> int:
    ...
    print("PASS_M01_VISIBLE_ENVIRONMENT_STAGE07B_MATERIAL_TERRAIN_DISTRICT_VARIETY01_RECOVERY01_AUTHORING_CONTRACT")
    return 0
...
def run_unreal() -> None:
    ...
    result: dict[str, object] = {
        "schema": "skyguard.m01-visible-environment-stage07b-material-terrain-district-variety01.recovery01-authoring01.v1",
        ...
        "checkpoint_captures_deferred_to_d3d12_proof": True, "runtime_promotion_performed": False,
    ...
    try:
        ...
        require(not OUTPUT_FILE.exists() and not unreal.EditorAssetLibrary.does_asset_exist(OUTPUT_ASSET), "Fresh Stage7B Recovery01 map exists")
        require(not DESTINATION_DISK.exists() and not unreal.EditorAssetLibrary.does_directory_exist(DESTINATION), "Fresh Stage7B Recovery01 namespace exists")
        require(unreal.EditorAssetLibrary.make_directory(DESTINATION), "Failed to create Stage7B Recovery01 namespace")
        ...
        require(len(governed_after) == expected, f"Stage7B Recovery01 governed actor count changed: {len(governed_after)} != {expected}")
        require(levels.save_current_level(), "Failed to save Stage7B Recovery01 map")
        require(OUTPUT_FILE.is_file(), "Stage7B Recovery01 output map missing")
        ...
        result["classification"] = "PASSED_STAGE07B_RECOVERY01_AUTHORING_AWAITING_D3D12_PROOF"
    ...
    if str(result["classification"]).startswith("PASSED_"):
        ...
        raise RuntimeError(result["error"] or "Stage7B Recovery01 authoring failed")
