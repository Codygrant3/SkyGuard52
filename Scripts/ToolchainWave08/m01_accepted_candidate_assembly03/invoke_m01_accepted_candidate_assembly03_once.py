"""One-shot supervisor for Mission 1 AcceptedCandidateAssembly03 authoring."""
...
def offline_contract_test() -> int:
    ...
    if OUTPUT_MAP.exists() or ATTEMPT.exists() or TERMINAL.exists():
        raise RuntimeError("Fresh governed namespace is not absent")
    ...
    if author_test.returncode != 0 or "PASS_M01_ACCEPTED_CANDIDATE_ASSEMBLY03_OFFLINE_CONTRACT" not in author_test.stdout:
        raise RuntimeError(f"Author offline contract failed: {author_test.stdout} {author_test.stderr}")
    print("PASS_M01_ACCEPTED_CANDIDATE_ASSEMBLY03_SUPERVISOR_OFFLINE_CONTRACT")
    return 0
    ...
    try:
        ...
        while process.poll() is None:
            ...
            if now >= deadline:
                ...
                raise RuntimeError(f"Unreal authoring exceeded {TIMEOUT_SECONDS} seconds")
            time.sleep(1.0)
        process.wait()
        ...
        if not RECEIPT.is_file():
            raise RuntimeError("Authoring receipt is missing")
        receipt = json.loads(RECEIPT.read_text(encoding="utf-8"))
        expected = "PASSED_M01_ACCEPTED_CANDIDATE_ASSEMBLY03_READY_FOR_STRUCTURAL_ADJUDICATION"
        ...
        if receipt.get("actor_count_before") != 148 or receipt.get("actor_count_after") != 119:
            raise RuntimeError("Assembly03 actor-count contract failed")
        ...
        if not OUTPUT_MAP.is_file():
            raise RuntimeError("Assembly03 output map is absent")
        if sha256(INPUT_MAP) != INPUT_MAP_SHA256:
            raise RuntimeError("Accepted input map changed during authoring")
        classification = "PASSED_M01_ACCEPTED_CANDIDATE_ASSEMBLY03_READY_FOR_MAPPED_VISUAL_PROOF_DESIGN"
        failure_stage = None
        ...
        terminal = {
            "schema": "skyguard.m01-accepted-candidate-assembly03.supervisor.v1",
            ...
            "next_gate": (
                ...
                else "preserve this failed namespace; no automatic retry"
            ),
        }
        try:
