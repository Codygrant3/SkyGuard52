"""One-shot supervisor for Mission 1 Hero Street/Shore Cell03 authoring."""
...
INPUT_MAP = ISOLATED / "Content/M01/Lvl_M01_HeroStreetShoreCell02.umap"
INPUT_MAP_BYTES = 747_443
INPUT_MAP_SHA256 = "5847609807972cfdf9b8d8d87f4c2f52aabaebfbbf68c9304404b5abb99f6622"
AUTHOR = ROOT / "Scripts/ToolchainWave08/m01_hero_street_shore_cell03/author_m01_hero_street_shore_cell03.py"
...
OUTPUT_MAP = ISOLATED / "Content/M01/Lvl_M01_HeroStreetShoreCell03.umap"
MATERIAL_DIRECTORY = ISOLATED / "Content/M01/HeroStreetShoreCell03"
ATTEMPT = ROOT / "Saved/BuildAttempts/M01_HERO_STREET_SHORE_CELL03/attempt_01"
RECEIPT = ATTEMPT / "authoring_receipt.json"
TERMINAL = ROOT / "Saved/Reports/M01_HERO_STREET_SHORE_CELL03_TERMINAL_SUPERVISOR.json"
...
def author_authority() -> dict[str, object]:
    if not AUTHOR.is_file():
        raise RuntimeError(f"Cell03 author is missing: {AUTHOR}")
...
def offline_contract_test() -> int:
    ...
    if OUTPUT_MAP.exists() or MATERIAL_DIRECTORY.exists() or ATTEMPT.exists() or TERMINAL.exists():
        raise RuntimeError("Fresh Cell03 governed namespace is not absent")
    ...
    if author_test.returncode != 0 or "PASS_M01_HERO_STREET_SHORE_CELL03_AUTHORING_CONTRACT" not in author_test.stdout:
        raise RuntimeError(f"Cell03 author offline contract failed: {author_test.stdout} {author_test.stderr}")
    print("PASS_M01_HERO_STREET_SHORE_CELL03_SUPERVISOR_OFFLINE_CONTRACT")
    return 0
    ...
    try:
        ...
        while process.poll() is None:
            ...
            if now >= deadline:
                ...
                raise RuntimeError(f"Unreal Cell03 authoring exceeded {TIMEOUT_SECONDS} seconds")
            time.sleep(1.0)
        process.wait()
        ...
        if not RECEIPT.is_file():
            raise RuntimeError("Cell03 authoring receipt is missing")
        receipt = json.loads(RECEIPT.read_text(encoding="utf-8"))
        expected = "PASSED_M01_HERO_STREET_SHORE_CELL03_READY_FOR_D3D12_MAPPED_VISUAL_PROOF"
        ...
        if receipt.get("actor_count_before") != 122 or receipt.get("actor_count_after") != 164:
            raise RuntimeError("Cell03 actor-count contract failed")
        if len(receipt.get("uprighted_props", [])) < 15:
            raise RuntimeError("Cell03 prop-uprighting contract failed")
        if len(receipt.get("rear_window_actors", [])) != 36 or len(receipt.get("second_row_placements", [])) != 2:
            raise RuntimeError("Cell03 composition contract failed")
        if not OUTPUT_MAP.is_file() or not MATERIAL_DIRECTORY.is_dir():
            raise RuntimeError("Cell03 governed outputs are absent")
        if sha256(INPUT_MAP) != INPUT_MAP_SHA256:
            raise RuntimeError("Immutable Cell02 input map changed")
        ...
        terminal = {
            "schema": "skyguard.m01-hero-street-shore-cell03.supervisor.v1",
            ...
            "material_files": [record(path) for path in sorted(MATERIAL_DIRECTORY.rglob("*")) if path.is_file()] if MATERIAL_DIRECTORY.exists() else [],
            "attempt_artifacts": artifacts,
            "accepted_input_unchanged": bool(INPUT_MAP.is_file() and sha256(INPUT_MAP) == INPUT_MAP_SHA256),
            "runtime_promotion": False,
            "next_gate": "fresh Cell03 D3D12 mapped visual proof" if classification.startswith("PASSED_") else "preserve failed namespace; no automatic retry",
        }
        try:
