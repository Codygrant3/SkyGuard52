...
$contractId = 'T08-M01-ENV-AUTH01-RECOVERY07-MAPPED-VISUAL-PROOF01'
$scriptRoot = Join-Path $root 'Scripts\ToolchainWave08\environment_authoring01_recovery07_mapped_visual_proof01'
$executor = Join-Path $scriptRoot 'capture_recovery07_mapped_visual_proof01.py'
$adjudicator = Join-Path $scriptRoot 'adjudicate_recovery07_mapped_visual_proof01_once.py'
$contract = Join-Path $root 'Docs\AAA_Review\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_CONTRACT.json'
$offlineFreeze = Join-Path $root 'Docs\AAA_Review\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_OFFLINE_DESIGN_FREEZE.json'
$bindingFreeze = Join-Path $root 'Docs\AAA_Review\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_EXECUTION_PROMPT_BINDING_FREEZE.json'
$attemptRoot = Join-Path $root 'Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01\attempt_01'
$launcherRoot = Join-Path $root 'Saved\BuildAttempts\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01\launcher_attempt_01'
$preflightReceipt = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_EXECUTION_PREFLIGHT.json'
$terminalSupervisor = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_TERMINAL_SUPERVISOR.json'
$emergencyReceipt = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_EMERGENCY_RECEIPT.jsonl'
$postflightReport = Join-Path $root 'Saved\Reports\TOOLCHAIN_WAVE08_M01_ENVIRONMENT_AUTHORING01_RECOVERY07_MAPPED_VISUAL_PROOF01_POSTFLIGHT.json'
$profileFile = Join-Path $isolatedRoot 'Saved\Profiling\CSV\Recovery07MappedVisualProof01.csv'
$timeoutSeconds = 540
...
function Test-OfflineContract {
    ...
    if ($freeze.classification -ne 'PASSED_READY_FOR_EXPLICIT_SINGLE_RECOVERY07_MAPPED_VISUAL_PROOF_AUTHORIZATION') { throw 'Offline freeze classification mismatch' }
    if ($binding.classification -ne 'PASSED_READY_FOR_EXPLICIT_SINGLE_RECOVERY07_MAPPED_VISUAL_PROOF_AUTHORIZATION') { throw 'Binding freeze classification mismatch' }
...
try {
    ...
    elseif (-not $AuthorizeSingleUnrealProof) {
        ...
        $stdout = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01.stdout.log'
        $stderr = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01.stderr.log'
        $engineLog = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01.engine.log'
        $processTree = Join-Path $launcherRoot 'process_tree_samples.jsonl'
        $postflightLog = Join-Path $launcherRoot 'logs\recovery07_mapped_visual_proof01.postflight.log'
        $arguments = @(
            $uproject,
            $mapAsset,
            '-game',
