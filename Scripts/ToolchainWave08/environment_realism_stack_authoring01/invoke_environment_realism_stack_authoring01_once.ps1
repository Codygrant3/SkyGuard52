...
$AuthoringScript = Join-Path $Root 'Scripts\ToolchainWave08\environment_realism_stack_authoring01\author_environment_realism_stack01.py'
$Contract = Join-Path $Root 'Docs\Toolchain\ToolchainWave08\M01EnvironmentRealismStackAuthoring01\authoring_contract.json'
$AttemptRoot = Join-Path $Root 'Saved\BuildAttempts\M01_ENVIRONMENT_REALISM_STACK_AUTHORING01\attempt_01'
$Receipt = Join-Path $AttemptRoot 'authoring_receipt.json'
$TerminalManifest = Join-Path $Root 'Saved\Reports\M01_ENVIRONMENT_REALISM_STACK_AUTHORING01_TERMINAL_MANIFEST.json'
$EmergencyReceipt = Join-Path $Root 'Saved\Reports\M01_ENVIRONMENT_REALISM_STACK_AUTHORING01_EMERGENCY_RECEIPT.jsonl'
$InputMap = 'D:\SG52T08_ENV01\Content\ToolchainWave08\Environment\Lvl_M01_T08_EnvironmentAuthoring01_Recovery07.umap'
$OutputMap = 'D:\SG52T08_ENV01\Content\ToolchainWave08\Environment\Lvl_M01_T08_EnvironmentRealismStack01.umap'
$TimeoutSeconds = 900
...
function Assert-FrozenAuthorities {
    Assert-FileRecord $AuthoringScript 25390 '141da71f8ea2c26d56bded67cd1beda8bc4963a75c9ed543dc0b4277f44e6fa1' 'authoring script'
    Assert-FileRecord $Contract 2857 '7a0764c31b15b9493c66e348f524b78aae1cc3cefd93550583be1af90f66b900' 'authoring contract'
...
function Invoke-OfflineContractTest {
    ...
    try {
        ...
        if ($contractPayload.visible_density.buildings -ne 36 -or $contractPayload.visible_density.trees -ne 60) { $failures += 'Density contract failed.' }
        if ($contractPayload.execution_policy.automatic_retries -ne 0) { $failures += 'Zero-retry contract failed.' }
    }
...
$State = [ordered]@{
    schema = 'skyguard.m01-environment-realism-stack-authoring01-supervisor.v1'
...
try {
    ...
    if ($State.receipt_classification -ne 'PASSED_M01_ENVIRONMENT_REALISM_STACK_AUTHORING01_AUTOMATIC') { throw "Unexpected receipt classification: $($State.receipt_classification)" }
    ...
    if (@($r.saved_assets).Count -ne 1 -or $r.saved_assets[0] -ne '/Game/ToolchainWave08/Environment/Lvl_M01_T08_EnvironmentRealismStack01') { throw 'Saved-asset allowlist failed.' }
    ...
    $State.classification = 'PASSED_M01_ENVIRONMENT_REALISM_STACK_AUTHORING01_AUTOMATIC'
