...
$ViewRoot = 'D:\SG52M01R04'
$AttemptRoot = 'D:\Skyguard52\Saved\BuildAttempts\PHASE4_M01_RECOVERY05_ENVIRONMENT_NATIVE_BUILD_RECOVERY04\build_attempt_01'
$TerminalManifest = 'D:\Skyguard52\Saved\Reports\PHASE4_M01_RECOVERY05_ENVIRONMENT_NATIVE_BUILD_RECOVERY04_TERMINAL_SUPERVISOR_MANIFEST.json'
$EmergencyReceipt = 'D:\Skyguard52\Saved\Reports\PHASE4_M01_RECOVERY05_ENVIRONMENT_NATIVE_BUILD_RECOVERY04_EMERGENCY_RECEIPT.jsonl'
$OfflineTestRoot = [System.IO.Path]::Combine(
    [System.IO.Path]::GetTempPath(),
    'Skyguard52_Recovery04_OfflineContractTest'
)
$ActiveTerminalManifest = if ($OfflineContractTest) {
    [System.IO.Path]::Combine($OfflineTestRoot, 'terminal_manifest.json')
} else {
    $TerminalManifest
}
$ActiveEmergencyReceipt = if ($OfflineContractTest) {
    [System.IO.Path]::Combine($OfflineTestRoot, 'emergency_receipt.jsonl')
} else {
    $EmergencyReceipt
}
...
$Recovery02TerminalFreeze = 'D:\Skyguard52\Docs\AAA_Review\PHASE4_M01_RECOVERY05_ENVIRONMENT_NATIVE_BUILD_RECOVERY02_SUPERVISOR_ATTEMPT01_TERMINAL_FREEZE.json'
...
function Write-Emergency {
    ...
    try {
        ...
        $record = [ordered]@{
            ...
            gate = 'PHASE4_M01_RECOVERY05_ENVIRONMENT_NATIVE_BUILD_RECOVERY04'
            stage = $Stage
            message = $Message
        }
        $encoding = New-Object System.Text.UTF8Encoding($false)
...
function Invoke-OfflineContractTest {
    if ([System.IO.Directory]::Exists($OfflineTestRoot) -or
        [System.IO.File]::Exists($OfflineTestRoot)) {
        throw "Offline test namespace already exists: $OfflineTestRoot"
    }

    [void][System.IO.Directory]::CreateDirectory($OfflineTestRoot)

    [void](Assert-File `
        $Recovery02TerminalFreeze `
        4535 `
    ...
    $hash = Get-Sha256 $Recovery02TerminalFreeze
    if ($hash -cnotmatch '^[0-9a-f]{64}$') {
        throw 'Offline SHA-256 lowercase-format test failed.'
    }
    ...
    $stateProbePath = [System.IO.Path]::Combine($OfflineTestRoot, 'state_probe.json')
    Write-Json $stateProbePath $stateProbe 8
    if (-not [System.IO.File]::Exists($stateProbePath)) {
        throw 'Offline state serialization probe did not persist.'
    }

    $preflightReceipt = [System.IO.Path]::Combine(
        $OfflineTestRoot,
        'deliberate_preflight_failure_terminal.json'
    )
    try {
        throw 'DELIBERATE_OFFLINE_PREFLIGHT_FAILURE'
    } catch {
        Write-Json $preflightReceipt ([ordered]@{
            classification = 'EXPECTED_OFFLINE_FAILURE_CAPTURED'
            message = $_.Exception.Message
        }) 8
    }
    if (-not [System.IO.File]::Exists($preflightReceipt)) {
        throw 'Deliberate preflight failure did not produce a terminal receipt.'
    }

    $emergencyProbe = [System.IO.Path]::Combine(
        $OfflineTestRoot,
        'deliberate_manifest_failure_emergency.jsonl'
    )
    try {
        Write-Json $OfflineTestRoot ([ordered]@{ should_fail = $true }) 8
        throw 'Deliberate manifest-write failure did not fail.'
    } catch {
        $encoding = New-Object System.Text.UTF8Encoding($false)
        [System.IO.File]::AppendAllText(
            $emergencyProbe,
            (([ordered]@{
                classification = 'EXPECTED_EMERGENCY_RECEIPT'
                message = $_.Exception.Message
            }) | ConvertTo-Json -Depth 8 -Compress) + [Environment]::NewLine,
            $encoding
        )
    }
    if (-not [System.IO.File]::Exists($emergencyProbe)) {
        throw 'Deliberate manifest failure did not produce an emergency receipt.'
    }

    $distinctClasses = @(
        [pscustomobject][ordered]@{
            file = 'mock-distinct-a.Build.cs'
            class = 'SkyguardRecovery03'
        },
        [pscustomobject][ordered]@{
            file = 'mock-distinct-b.Build.cs'
            class = 'SkyguardRecovery03NativeRecovery05'
        }
    )
    $distinctDuplicates = @(
        $distinctClasses |
    ...
    if ($distinctDuplicates.Count -ne 0) {
        throw 'Distinct ModuleRules records produced a false duplicate.'
    }

    $duplicateClasses = @(
        [pscustomobject][ordered]@{
            file = 'mock-duplicate-a.Build.cs'
            class = 'ExpectedDuplicate'
        },
        [pscustomobject][ordered]@{
            file = 'mock-duplicate-b.Build.cs'
            class = 'ExpectedDuplicate'
        }
    )
    $expectedDuplicates = @(
    ...
    if ($expectedDuplicates.Count -ne 1 -or
        $expectedDuplicates[0].Name -cne 'ExpectedDuplicate') {
        throw 'Duplicate ModuleRules records did not produce exactly one named duplicate.'
    }

    $blankClasses = @(
        [pscustomobject][ordered]@{
            file = 'mock-blank.Build.cs'
            class = ''
        }
    )
    $blankCount = @(
        $blankClasses |
            Where-Object { [string]::IsNullOrWhiteSpace([string]$_.class) }
    ).Count
    if ($blankCount -ne 1) {
        throw 'Blank ModuleRules class rejection probe failed.'
    }

    $realBuildCsAuthorities = @(
        [ordered]@{
            path = 'D:\SG52M01R03\Plugins\SkyguardRecovery03\Source\SkyguardRecovery03\SkyguardRecovery03.Build.cs'
            bytes = 619
            sha256 = 'ae5236cdd7d7b80af194025fd65506f35924f9fa78b9b3e7d9fb584b7ba89088'
            expected_class = 'SkyguardRecovery03'
        },
        [ordered]@{
            path = 'D:\SG52M01R03\Plugins\SkyguardRecovery03NativeRecovery05\Source\SkyguardRecovery03NativeRecovery05\SkyguardRecovery03NativeRecovery05.Build.cs'
            bytes = 592
            sha256 = 'd5735c78adc4fcb24ca3760cec50426d9f039e1a08178ea00a309672efdaa7b6'
            expected_class = 'SkyguardRecovery03NativeRecovery05'
        }
    )
    $realClasses = @()
    foreach ($authority in $realBuildCsAuthorities) {
        [void](Assert-File `
            ([string]$authority.path) `
            ([int64]$authority.bytes) `
            ([string]$authority.sha256))
        $text = [System.IO.File]::ReadAllText([string]$authority.path)
        $match = [regex]::Match(
            $text,
            'public\s+(?:sealed\s+)?class\s+([A-Za-z_][A-Za-z0-9_]*)\s*:\s*ModuleRules'
        )
        if (-not $match.Success) {
            throw "Unable to determine real ModuleRules class: $($authority.path)"
        }
        $className = [string]$match.Groups[1].Value
        if ([string]::IsNullOrWhiteSpace($className)) {
            throw "Real ModuleRules class is blank: $($authority.path)"
        }
        if ($className -cne [string]$authority.expected_class) {
            throw "Real ModuleRules class mismatch: $($authority.path)"
        }
        $realClasses += [pscustomobject][ordered]@{
            file = [string]$authority.path
            class = $className
        }
    }
    $realDuplicates = @(
    ...
    if ($realDuplicates.Count -ne 0) {
        throw "Real ModuleRules classes produced duplicates: $($realDuplicates.Name -join ', ')"
    }
    ...
    foreach ($path in @(
        ...
        if ([System.IO.Directory]::Exists($path) -or
            [System.IO.File]::Exists($path)) {
            throw "Offline test created or found a governed namespace: $path"
        }
    }
    ...
    return [ordered]@{
        test_root = $OfflineTestRoot
        state_probe = Get-FileRecord $stateProbePath
        preflight_failure_receipt = Get-FileRecord $preflightReceipt
        emergency_receipt = Get-FileRecord $emergencyProbe
        grouping_tests = [ordered]@{
            distinct_duplicate_count = $distinctDuplicates.Count
            expected_duplicate_count = $expectedDuplicates.Count
            expected_duplicate_name = $expectedDuplicates[0].Name
            blank_rejection_count = $blankCount
            real_class_count = $realClasses.Count
            real_duplicate_count = $realDuplicates.Count
            real_classes = $realClasses
        }
        governed_namespaces_created = 0
        ubt_launch_count = 0
        dotnet_build_launch_count = 0
        unreal_launch_count = 0
        blender_launch_count = 0
        compiler_launch_count = 0
        linker_launch_count = 0
    }
}
...
try {
    $State = [ordered]@{
        schema = 'skyguard.phase4.m01-recovery05-environment-native-build-recovery04-terminal.v1'
    ...
    if ($OfflineContractTest -and $AuthorizeSingleBuild) {
        throw 'OfflineContractTest and AuthorizeSingleBuild are mutually exclusive.'
    }
    ...
    foreach ($excluded in @(
        'D:\SG52M01R04\Plugins\SkyguardRecovery03NativeRecovery01',
        'D:\SG52M01R04\Plugins\SkyguardRecovery03NativeRecovery04'
    )) {
    ...
    $outputs = @(
        ...
        'D:\SG52M01R04\Binaries\Win64\UnrealEditor-Skyguard52.pdb',
    ...
    $viewSource = 'D:\SG52M01R04\Source\Skyguard52\SkyguardMission01EnvironmentDirector.cpp'
    [void](Assert-File `
        $viewSource `
        15032 `
    ...
    if ($null -eq $State) {
        $State = [ordered]@{
            ...
            unreal_editor_launched = $false
            blender_launched = $false
            failure_stage = 'state_initialization'
            failure_message = $null
        }
    }
    $State.failure_stage = if ($State.preflight_passed) {
        'view_creation_build_or_postflight'
    } elseif ($State.failure_stage -eq 'state_initialization') {
    ...
    try {
        Write-Json $ActiveTerminalManifest $State 30
