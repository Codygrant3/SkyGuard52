...
PREFIX = "AAA_L43_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L43"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L43"
...
def densify():
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # WALL-PLANE densify only (x == bx+2 / bx+1 stripes). No deep stack, no mid-FOV blades.
        # Goal: lift Prop/PropHub/PropNose uniq+edge without regressing City/Cockpit.
        if name in ("Prop", "PropHub", "PropNose"):
            denser = 2.6 if name == "PropNose" else (3.0 if name == "Prop" else 3.2)
            yspan = 16 if name == "PropNose" else 13
            zspan = 12 if name == "PropNose" else 10
            for iy in range(-yspan, yspan + 1):
                for iz in range(-zspan, zspan + 1):
                    # checker parity for edge energy
                    mm = mats[(iy + iz * 3 + i * 5 + (0 if (iy + iz) % 2 == 0 else 1)) % len(mats)] if mats else m
                    scale = (0.22, 0.48, 0.48) if (iy + iz) % 2 == 0 else (0.35, 0.62, 0.62)
                    spawn_sm(cube, (bx + 2.0, cy + iy * denser, cz + iz * denser), scale, None, PREFIX + "WPlane_%s_%d_%d" % (name, iy, iz), mm)
            # extra fine stripes only on wall plane (bx+2) for edge recovery
            for iy in range(-18, 19):
                mm = mats[(iy + i) % len(mats)] if mats else m
                spawn_sm(cube, (bx + 2.0, cy + iy * 2.1, cz), (0.18, 0.22, 9.5), None, PREFIX + "WStripe_%s_%d" % (name, iy), mm)
            for iz in range(-14, 15):
                mm = mats[(iz + i * 2) % len(mats)] if mats else m
                spawn_sm(cube, (bx + 2.0, cy, cz + iz * 2.1), (0.18, 9.5, 0.22), None, PREFIX + "HStripe_%s_%d" % (name, iz), mm)
            # tiny hub marker on wall only
            spawn_sm(sphere, (bx + 2.0, cy, cz), (1.2, 1.2, 1.2), None, PREFIX + "WHub_%s" % name, mats[i % len(mats)] if mats else m)
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop43 wall-plane densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L43", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L43_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop43 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l38_freeze_wall_plane_prop_family\n")
...
def main():
    log("loop43 L38 freeze + wall-plane Prop family densify (no deep/blades) start")
    ...
    log("Loop43 complete stills=%d" % (len(saved) if saved else 0))
