...
"""Governed AH-64 CPG station-model06 worker.

Keeps the model05 layout, sockets, look-out, hoods, emit wells, TEDAC
readability, spun knobs, and HOCAS heads. Adds a filled canopy skin
between sill and rail so the greenhouse reads as an enclosure from
three-quarter, not a stick kit. Bucket seat keeps a visible dish.
...
ASSET_ID = "core-apache-cockpit-station-model06"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def build_seat(collection, material) -> None:
    ...
    for vert in pan.verts:
        ...
        dish = (1.0 - min(1.0, (abs(y_value) / 0.16) ** 2)) * 0.118
        fore = max(0.0, min(1.0, (x_value + 0.18) / 0.36))
        vert.co.z = z_value - dish * (0.65 + 0.80 * fore)
        if abs(y_value) > 0.055:
            vert.co.z += (abs(y_value) - 0.055) * 1.55
        if x_value > 0.10:
    ...
    for name, y_sign in (("GEO_SeatBolster_L", -1.0), ("GEO_SeatBolster_R", 1.0)):
        ...
        y1 = y_sign * 0.238
        verts = [
            bm.verts.new((-0.18, y0, 0.845)),
            bm.verts.new((0.16, y0, 0.812)),
