...
"""Governed AH-64 CPG station-model19 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8, and the model18 visual fail, not a
...
ASSET_ID = "core-apache-cockpit-station-model19"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark) -> None:
    ...
    Not a box with pillars plus two floating monitors. The surround is a
    single formed face from MPD_L (y=-0.205) to MPD_R (y=0.205). Wells
    recede so GEO_TEDAC emit and both MPD screens stay visible from the
    eye. Do not raise the dash through z<=0.86.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    ys = (
...
def build_mpd(name: str, center, toe: float, collection, dark, emit) -> None:
    ...
    GEO_MPD_L / GEO_MPD_R are recessed emit screens in the same housing.
...
def loft_canopy_fill(collection, material, name: str, y_sign: float, xs) -> None:
    """Dark plate between frames with a punched window matching bay z.

    Not a draped canopy skin. Not model17 plate-knob overhang/solidify.
    Skirt and header fill the bay so three-quarter reads as a formed
    enclosure with punched framed windows, not a beam skeleton.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    lookout_half = 0.20
    ...
    for x_value in xs:
        ...
        skirt_cols.append(
            [
                ...
                bm.verts.new((x_value, y_sign * y_outer, win_sill)),
                bm.verts.new((x_value, y_sign * inner_y, win_sill)),
            ]
        )
        header_cols.append(
            [
                bm.verts.new((x_value, y_sign * inner_y, win_rail)),
                bm.verts.new((x_value, y_sign * y_outer, win_rail)),
...
def loft_forward_windshield(collection, glass, rail) -> None:
    ...
    Dutch photo + TM Fig 2-8: a real CPG must look through framed glass. The pane
    fills roughly x 0.55-0.86, |y| <= 0.20, z 1.05-1.36. Thick black
    frame only on the edges, outboard of |y|>=0.20 or below z=1.05 /
    above z=1.36.
    """
    bmesh = bmesh_module()
    glass_bm = bmesh.new()
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    bmesh = bmesh_module()
    # Formed greenhouse from TM Fig 2-8 and the Dutch photo: thick black
    # frames, punched windows, not a beam cage and not overlapping milky
    # sheets. Rails read as a rail cap. Sills read as a sill plate.
    # A-pillars and side-bows read as a formed pillar. |y| >= 0.38. Aft
    # frame is behind the eye. No bow bar across the look-out. Members
    # stay formed rectangular sections, not a tube sweep. Do not thicken
