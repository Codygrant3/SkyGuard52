...
"""Yak-52 artist-grade Method02+: envelope correction + silhouette systems.

Independent lane after Method02 failed global_envelope_pass (height 2.375m vs
2.7m) with slab wing/canopy form. Does not cosmetically repair the rejected
Method02 or Recovery01 GLB. Fresh namespace only.
"""
...
ASSET_ID = "core-yak52-airframe-artist-grade-method02-plus"
SOURCE_BLEND = (
    PROJECT_ROOT
    / "Content"
    / "Skyguard"
...
CONTRACT = (
    ...
    / "Docs"
    / "AAA_Review"
    / "PHASE2_YAK52_AIRFRAME_ARTIST_GRADE_METHOD02_PLUS_CONTRACT.json"
)
SYSTEMS = (
    PROJECT_ROOT
    / "Docs"
    / "AAA_Review"
    / "PHASE2_YAK52_AIRFRAME_ARTIST_GRADE_METHOD02_PLUS_SYSTEMS.json"
)
STATION_CAGE = (
    PROJECT_ROOT
    / "Docs"
    / "AAA_Review"
    / "PHASE2_YAK52_AIRFRAME_ARTIST_GRADE_METHOD02_PLUS_SILHOUETTE_STATION_CAGE.json"
)
CAMERAS = (
    PROJECT_ROOT
    / "Docs"
    / "AAA_Review"
    / "PHASE2_YAK52_AIRFRAME_ARTIST_GRADE_METHOD02_PLUS_CAMERAS.json"
)
RUBRIC = (
    PROJECT_ROOT
    / "Docs"
    / "AAA_Review"
    / "PHASE2_YAK52_AIRFRAME_ARTIST_GRADE_METHOD02_PLUS_VISUAL_RUBRIC.json"
)
...
class Method02PlusError(RuntimeError):
    pass
...
def parse_args() -> argparse.Namespace:
    ...
    if args.asset_id != ASSET_ID:
        raise Method02PlusError(f"Unexpected asset id: {args.asset_id}")
    return args
...
def assert_fresh_namespace(output: Path) -> None:
    ...
    for forbidden in FAILED_ATTEMPT_ROOTS:
        try:
            ...
            raise Method02PlusError(
                f"Method02 must not write into failed namespace: {forbidden}"
            )
        except ValueError:
            continue
    if "core-yak52-airframe-artist-grade-method02-plus" not in str(resolved):
        raise Method02PlusError(
            "Output must remain under the Method02-plus fresh attempt namespace."
        )
...
def load_station_profiles(path: Path | None = None) -> list[dict[str, float | str]]:
    ...
    if not isinstance(stations, list) or len(stations) < 5:
        raise Method02PlusError("Station cage authority is incomplete.")
...
def fuselage_width_variation(profiles: list[dict[str, float | str]]) -> float:
    ...
    if mean <= 1e-9:
        raise Method02PlusError("Station cage mean width is zero.")
...
def evaluate_framing_gates(
    ...
    for view in views:
        ...
        if subject == "ALL_SYSTEMS":
            ...
            if missing:
                raise Method02PlusError(f"Framing subject missing bounds: {missing}")
...
def verify_authorities() -> list[dict[str, Any]]:
    ...
    for path in (CONTRACT, SYSTEMS, STATION_CAGE, CAMERAS, RUBRIC):
        if not path.is_file():
            raise Method02PlusError(f"Missing Method02-plus authority: {path}")
        inventory.append(
            {"path": str(path), "bytes": path.stat().st_size, "sha256": sha256(path)}
        )
    if not SOURCE_BLEND.is_file():
        raise Method02PlusError(f"Missing dimensional authority blend: {SOURCE_BLEND}")
    ...
    if source_bytes != 1526526 or source_hash != (
        ...
        raise Method02PlusError(f"Dimensional authority blend identity mismatch: {SOURCE_BLEND}")
    inventory.append(
        {
    ...
    if contract.get("asset_id") != ASSET_ID:
        raise Method02PlusError("Contract asset_id mismatch.")
    if "cosmetic repair" not in " ".join(contract.get("prohibited", [])).lower() and not any(
        ...
        raise Method02PlusError("Contract must prohibit cosmetic repair of rejected GLB.")
    return inventory
...
def load_dimensional_authority_record() -> dict[str, Any]:
    ...
    if SOURCE_BLEND.stat().st_size <= 0:
        raise Method02PlusError("Dimensional authority blend is empty.")
    after = sha256(SOURCE_BLEND)
    if before != after:
        raise Method02PlusError("Dimensional authority blend mutated during probe.")
    return {
...
def new_object(name: str, mesh: Any, collection: Any) -> Any:
    ...
    obj["SKG_Method"] = "artist_grade_method02_plus_envelope_silhouette"
    return obj
...
def mesh_from_loft(
    ...
    collection: Any,
    segments: int = 16,
    z_center: float = FUSELAGE_Z_CENTER_M,
) -> Any:
    import bpy
...
def join_objects(name: str, objects: list[Any], collection: Any) -> Any:
    ...
    if not objects:
        raise Method02PlusError(f"Cannot join empty system: {name}")
    if len(objects) == 1:
        objects[0].name = name
        return objects[0]
...
def mesh_from_wing_loft(name: str, collection: Any, sign: float) -> Any:
    """Spanwise lofted wing with tapering chord/thickness (not Method02 slabs)."""
    import bpy
    from mathutils import Vector

    # (span_y, chord, thickness, leading_edge_x, z)
    stations = (
        (0.72, 1.95, 0.26, 0.55, 1.18),
        (1.80, 1.70, 0.20, 0.40, 1.20),
        (3.10, 1.35, 0.14, 0.25, 1.24),
        (4.65, 1.00, 0.09, 0.12, 1.28),
    )
    segments = 14
    rings = []
    for span_y, chord, thickness, leading_x, z in stations:
        half_chord = chord * 0.5
        half_thick = thickness * 0.5
        center_x = leading_x - half_chord
        ring = []
        for index in range(segments):
            angle = (2.0 * math.pi * index) / segments
            # Flatten lower surface slightly for a more wing-like section.
            x = center_x + half_chord * math.cos(angle)
            local_z = half_thick * math.sin(angle)
            if local_z < 0.0:
                local_z *= 0.55
            ring.append(Vector((x, sign * span_y, z + local_z)))
        rings.append(ring)
    vertices: list[Any] = []
    faces: list[tuple[int, int, int, int]] = []
    for ring in rings:
    ...
    mesh.from_pydata([(v.x, v.y, v.z) for v in vertices], [], faces)
    mesh.update()
    return new_object(name, mesh, collection)


def mesh_from_canopy_loft(name: str, collection: Any) -> Any:
    """Two-seat bubble canopy loft (not Method02 cube glass)."""
    import bpy
    from mathutils import Vector

    # (x, half_width, height_above_rail, rail_z)
    stations = (
        (0.40, 0.34, 0.18, 1.72),
        (0.05, 0.42, 0.48, 1.74),
        (-0.35, 0.40, 0.52, 1.76),
    ...
    segments = 12
    rings = []
    for x, half_width, height, rail_z in stations:
        ring = []
        for index in range(segments + 1):
            # Upper semicircle from -1..+1 in Y, peak at Z.
            t = index / segments
            angle = math.pi * t  # 0..pi
            y = -half_width * math.cos(angle)
            z = rail_z + height * math.sin(angle)
...
def enforce_global_envelope(systems: dict[str, Any]) -> dict[str, Any]:
    """Scale Z about ground contact so overall height hits 2.7m authority."""
    dims = dimensions_from_systems(systems)
    current_height = float(dims["overall_height_m"])
    if current_height <= 1e-6:
        raise Method02PlusError("Cannot enforce envelope: zero height.")
    target = float(AUTHORITATIVE["overall_height_m"])
    min_z = float(dims["minimum_z_m"])
    scale_z = target / current_height
    for obj in systems.values():
        if obj.type != "MESH":
            continue
        for vertex in obj.data.vertices:
            vertex.co.z = min_z + (vertex.co.z - min_z) * scale_z
        obj.data.update()
    reconciled = dimensions_from_systems(systems)
    height_error = abs(reconciled["overall_height_m"] - target) / target
    if height_error > ENVELOPE_TOLERANCE:
        raise Method02PlusError(
            f"Height envelope still failing after scale: {reconciled['overall_height_m']:.4f}m"
        )
    return {
        "pre_scale_height_m": current_height,
        "scale_z": scale_z,
        "post_scale_height_m": reconciled["overall_height_m"],
        "minimum_z_m": reconciled["minimum_z_m"],
        "maximum_z_m": reconciled["maximum_z_m"],
    }
...
def build_systems(collection: Any, profiles: list[dict[str, float | str]]) -> dict[str, Any]:
    ...
    cowling = mesh_from_loft(
        "SYS_Cowling", cowling_profiles, collection, segments=20, z_center=FUSELAGE_Z_CENTER_M
    )
    assign_material(cowling, metal)
    fuselage = mesh_from_loft(
        "SYS_Fuselage",
        fuselage_profiles,
        collection,
        segments=20,
        z_center=FUSELAGE_Z_CENTER_M,
    )
    assign_material(fuselage, paint)

    # Lofted tapered wing (Method02-plus: not TMP slab cubes).
    wing_parts = [
        mesh_from_wing_loft("TMP_Wing_L", collection, sign=-1.0),
        mesh_from_wing_loft("TMP_Wing_R", collection, sign=1.0),
    ]
    wing = join_objects("SYS_Wing", wing_parts, collection)
    assign_material(wing, yellow)

    # Empennage sized toward 2.7m height envelope before final Z enforce.
    ht_root = add_box(
        "TMP_HT_Root",
        (-3.00, 0.0, 1.48),
        (1.05, AUTHORITATIVE["horizontal_tail_span_m"] * 0.55, 0.10),
        collection,
    )
    ht_tip_l = add_box(
        "TMP_HT_Tip_L", (-3.18, -1.45, 1.50), (0.70, 0.55, 0.06), collection
    )
    ht_tip_r = add_box(
        "TMP_HT_Tip_R", (-3.18, 1.45, 1.50), (0.70, 0.55, 0.06), collection
    )
    vt_lower = add_box("TMP_VT_Lower", (-3.20, 0.0, 1.85), (0.95, 0.12, 0.85), collection)
    vt_upper = add_box("TMP_VT_Upper", (-3.35, 0.0, 2.45), (0.70, 0.08, 0.70), collection)
    empennage = join_objects(
        "SYS_Empennage", [ht_root, ht_tip_l, ht_tip_r, vt_lower, vt_upper], collection
    )
    assign_material(empennage, paint)

    canopy_bubble = mesh_from_canopy_loft("TMP_CanopyBubble", collection)
    bow = add_box("TMP_CanopyBow", (-0.60, 0.0, 2.15), (0.07, 0.72, 0.38), collection)
    canopy = join_objects("SYS_Canopy", [canopy_bubble, bow], collection)
    ...
    spinner = add_cylinder(
        "TMP_Spinner",
        (3.70, 0.0, FUSELAGE_Z_CENTER_M),
        0.22,
        0.45,
        collection,
    ...
    for index in range(2):
        ...
        blade = add_box(
            ...
            (
                ...
                FUSELAGE_Z_CENTER_M + math.cos(angle) * radius * 0.45,
            ),
    ...
    aileron_l = add_box("TMP_Aileron_L", (0.05, -3.55, 1.22), (0.45, 1.4, 0.05), collection)
    aileron_r = add_box("TMP_Aileron_R", (0.05, 3.55, 1.22), (0.45, 1.4, 0.05), collection)
    elevator = add_box("TMP_Elevator", (-3.35, 0.0, 1.48), (0.35, 2.6, 0.05), collection)
    rudder = add_box("TMP_Rudder", (-3.55, 0.0, 2.20), (0.30, 0.06, 0.95), collection)
...
def add_datums(collection: Any, profiles: list[dict[str, float | str]]) -> list[str]:
    ...
    for index, profile in enumerate(profiles):
        ...
        datum = new_empty(
            name,
            (float(profile["x_m"]), 0.0, FUSELAGE_Z_CENTER_M),
            collection,
            "reference_station",
        )
...
def render_views(output: Path) -> tuple[list[Path], list[Path]]:
    ...
    if scene.world is None:
        scene.world = bpy.data.worlds.new("WORLD_Yak52Method02Plus")
...
def export_outputs(asset_collection: Any, output: Path) -> tuple[Path, Path]:
    ...
    blend_path = output / "SKG_Yak52_Airframe_ArtistGrade_Method02Plus.blend"
    glb_path = output / "SKG_Yak52_Airframe_ArtistGrade_Method02Plus.glb"
...
def main() -> int:
    ...
    if output.exists() and any(output.iterdir()):
        raise Method02PlusError(f"Output directory is not empty: {output}")
    ...
    if missing:
        raise Method02PlusError(f"Missing required systems: {missing}")
    envelope_correction = enforce_global_envelope(systems)
    ...
    if any(not item["pass"] for item in framing):
        raise Method02PlusError(f"Camera framing gate failed: {framing}")
    ...
    write_json(
        ...
        {
            "schema": "skyguard.phase2.yak52-airframe-artist-grade-method02-plus.dimension-receipt.v1",
            ...
            "envelope_correction": envelope_correction,
            "global_envelope_pass": all(
                abs(final_dimensions[key] - AUTHORITATIVE[key]) / AUTHORITATIVE[key]
                <= ENVELOPE_TOLERANCE
            ...
            "rejected_method02_glb_reused": False,
        },
    )
    write_json(
        output / "source_parity_receipt.json",
        {
            "schema": "skyguard.phase2.yak52-airframe-artist-grade-method02-plus.source-parity.v1",
            "authorities": authorities,
            "dimensional_authority_probe": donor,
            "failed_namespaces_not_used": [str(path) for path in FAILED_ATTEMPT_ROOTS],
            "immutable_failed_method02_attempt": str(
                PROJECT_ROOT
                / "Production"
                / "Attempts"
                / "core-yak52-airframe-artist-grade-method02"
                / "attempt_20260811T193939687938Z"
            ),
    ...
    write_json(
        ...
        {
            "schema": "skyguard.phase2.yak52-airframe-artist-grade-method02-plus.silhouette-station.v1",
            "station_count": len(profiles),
            "stations": profiles,
            "width_coefficient_of_variation": variation,
            "fuselage_z_center_m": FUSELAGE_Z_CENTER_M,
            "checkpoint_count": len(checkpoints),
            "checkpoint_names": [path.name for path in checkpoints],
            "construction": {
                "wing": "spanwise_loft_taper",
                "canopy": "bubble_semicircle_loft",
                "empennage": "tapered_fin_stack_for_height_envelope",
            },
        },
    )
    write_json(
        ...
        {
            "schema": "skyguard.phase2.yak52-airframe-artist-grade-method02-plus.systems.v1",
            "systems": sorted(systems.keys()),
            "system_count": len(systems),
            "separate_systems": True,
            "method02_slab_wing_canopy_reused": False,
        },
    )
    write_json(
        output / "camera_framing_receipt.json",
        {
            "schema": "skyguard.phase2.yak52-airframe-artist-grade-method02-plus.camera-framing.v1",
            "results": framing,
    ...
    write_json(
        ...
        {
            "schema": "skyguard.phase2.yak52-airframe-artist-grade-method02-plus.topology-material.v1",
    ...
    write_json(
        ...
        {
            "schema": "skyguard.phase2.yak52-airframe-artist-grade-method02-plus.artifact-receipt.v1",
            "asset_id": ASSET_ID,
...
if __name__ == "__main__":
    ...
    except Method02PlusError as exc:
        print(f"Method02PlusError: {exc}", file=sys.stderr)
