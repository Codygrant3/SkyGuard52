...
"""Governed AH-64 CPG station-model23 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model22 visual fail, not a
plate-knob pass. Not another named mesh on the same cage. Greenhouse
hull owns three-quarter as a closed outer volume (sill-in, belly,
rail-out, crown, back to sill-in) with punched window openings;
rails, sills, A-pillars, and side-bows are thin trim on that hull,
not a chunky cage. eye_forward looks through raked windshield / sky
in the upper two-thirds; TEDAC and dash occupy the lower third, not
a grey rectangular well and not a housing well. Canopy glass must
read as glass, not a dark grey wall. One continuous formed forward
panel with inset TDU and MPD wells at the same x. GEO_TEDAC is
well-only: emit well + crosshair + LHG/RHG + bezel buttons, no
formed_bezel body cube. Square TDU plus LHG/RHG stay as hardware on
the dash. TEDAC is not three stacked boxes. Destacked TDU numbers
are a ceiling: go smaller than width 0.118 / depth 0.018 / hood
0.010. Raked windshield stations stay different YZ trapezoids.
Glass may occupy the forward look-out band; frames may not. Keep
glass overhead brow, dash z<=0.86, TEDAC green emit, MPD emit-face
fix, and the model14-22 bucket seat. GEO_Dash look-out assert stays;
after finish_mesh bevel, clamp |y|<0.20 verts to z<=0.858 and drop
the housing grid top to 0.848 so bevel cannot recreate the model21
fail. Dark interior, dark rail, and olive stay. Do not restore a
draped canopy skin. Do not thicken section_along members. No tube
sweep in the greenhouse. Real bmesh only. Public layout only.
...
ASSET_ID = "core-apache-cockpit-station-model23"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def build_tedac(collection, dark, emit, grip, mark) -> None:
    ...
    Well-only GEO_TEDAC: emit well + crosshair + Left Hand Grip + Right
    Hand Grip + Fig 45 bezel (TAD/FCR/PNV/G/S, DAY-NT-OFF, LEV/GAIN,
    SYM/BRT/CON, AZ, four action buttons). Public shapes only. Not a
    0.188 cube sitting in front of the panel. No cube body
    extrusion that reads as a standalone housing.
...
def loft_canopy_shell(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    # Aft and forward end caps keep the hull a closed volume.
    for col in (0, -1):
        ring = columns[col]
...
def loft_forward_windshield(collection, glass, rail) -> None:
    ...
    z 1.05-1.36. Thin trim only on the edges, outboard of |y|>=0.20 or
    below z=1.05 / above z=1.36. Not a grey rectangular well.
    """
    bmesh = bmesh_module()
    glass_bm = bmesh.new()
    columns = []
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    ...
    # window openings. Rails, sills, A-pillars, and side-bows are thin trim
    # on that hull, not a chunky cage silhouette. Do not add
    ...
    for name, (points, width, height) in sections.items():
        ...
        for center, axis in joints.get(name, ()):
            spin_bulb(bm, center, axis, 0.008, 0.012, material_index=0)
        object_from_bmesh(name, bm, collection, [rail])
