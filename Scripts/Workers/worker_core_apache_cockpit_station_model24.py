...
"""Governed AH-64 CPG station-model24 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model23 cycle fail, not a
plate-knob pass on the greenhouse. Keep model23 architecture.
Greenhouse hull owns three-quarter as a closed outer volume
(sill-in, belly, rail-out, crown, back to sill-in) with punched
window openings; rails, sills, A-pillars, and side-bows are thin
trim on that hull, not a chunky cage. eye_forward looks through
raked windshield / sky in the upper two-thirds; TEDAC and dash
occupy the lower third, not a grey rectangular well and not a
housing well. Canopy glass must read as glass, not a dark grey
wall. One continuous formed forward panel with inset TDU and MPD
wells at the same x. GEO_TEDAC is well-only: emit well + crosshair
+ LHG/RHG + bezel buttons, no formed_bezel body cube. After
object_from_bmesh, GEO_TEDAC calls orient_emit_faces_to_eye then
assert_emit_faces_eye and assert_hood_geometry, same order as
build_mpd, so finish_mesh bevel / recalc_face_normals cannot leave
emit winding at alignment 0.325. Square TDU plus LHG/RHG stay as
hardware on the dash. TEDAC is not three stacked boxes. Destacked
TDU numbers are a ceiling: go smaller than width 0.118 / depth
0.018 / hood 0.010. Raked windshield stations stay different YZ
...
emit-face fix, and the model14-23 bucket seat. GEO_Dash look-out
...
ASSET_ID = "core-apache-cockpit-station-model24"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
