...
"""Governed AH-64 CPG station-model28 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model27 visual fail, not a
plate-knob pass. Keep the model27 assert-path. Not another named
...
to sill-in) with punched outer window openings — not a pane kit of
...
well, not a housing well, and not an opaque pane wall. model27
visual fail: a 3-sided tunnel plus a dark world.color left the
eye in an enclosed volume with no sky. Fill the look-out as one
raked pane and drive the Background shader to sky. Delete
GEO_WindshieldFrame / shell end-cap / brow faces that form a
rectangular tunnel. Canopy glass must read as glass, not a dark
grey wall. One continuous formed forward panel from
dash_panel_outline / dash_well_loops, not a housing-grid frame
in front of two flat cyan screens. TDU and both MPD wells are
recessed holes in that one face. GEO_TEDAC is grips
...
ASSET_ID = "core-apache-cockpit-station-model28"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def dash_panel_outline() -> tuple[tuple[float, float, float], ...]:
    """One formed forward panel outline in YZ at the TEDAC x."""
    return (
        (0.50, -0.278, 0.720),
        (0.50, 0.278, 0.720),
        (0.50, 0.278, 0.848),
...
def dash_well_loops() -> tuple[
    tuple[tuple[float, float, float], ...],
    tuple[tuple[float, float, float], ...],
    tuple[tuple[float, float, float], ...],
]:
    """TDU and both MPD wells as holes in that one face. Not a housing-grid."""
    tdu = (
        (0.50, -0.048, 0.804),
        (0.50, 0.048, 0.804),
        (0.50, 0.048, 0.844),
...
def formed_tedac_housing(collection, dark, tedac_emit, mpd_emit, tedac_mark) -> None:
    ...
    GEO_Dash is that one panel from dash_panel_outline / dash_well_loops.
    TDU and both MPD wells are recessed holes in that one face
    (inset + emit material), not a housing-grid frame in front of two
    flat cyan screens, not a grey bezel rectangle sitting in front of
    two flat cyan screens, and not sibling GEO_TEDAC / GEO_MPD meshes
    sitting in front of the panel. The surround is a single formed
    face from MPD_L (y=-0.205) to MPD_R (y=0.205) at the TEDAC x.
    Inset wells recede so the TDU and both MPD screens stay visible
    from the eye as holes, not as standalone cyan planes. Dash cap
    remains z<=0.86 so eye_forward is not a housing well. Panel top
    is 0.848 so finish_mesh bevel cannot recreate the model21 fail
    (0.86 ring lifted over 0.86). model25 fail: TDU well window
    missed housing face centers at y=±0.05, so GEO_Dash had no
    emit-material faces at index 1 after finish_mesh bevel. model27
    visual fail: the housing-grid still built a frame-in-front-of-
    screens dash; punch three wells in one face instead of retuning
    inset knobs on the same grid.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()

    def _vert(y_value: float, z_value: float):
        return bm.verts.new((0.50, y_value, min(z_value, 0.848)))

    def _quad(y0: float, y1: float, z0: float, z1: float):
        return bm.faces.new(
            (
                _vert(y0, z0),
                _vert(y1, z0),
    ...
    outline = dash_panel_outline()
    tdu_loop, mpd_l_loop, mpd_r_loop = dash_well_loops()
    _ = outline
    # One formed panel: surround bars + three punched well faces.
    _quad(-0.278, 0.278, 0.720, 0.804)
    _quad(-0.278, 0.278, 0.844, 0.848)
    _quad(-0.278, -0.237, 0.804, 0.844)
    _quad(-0.173, -0.048, 0.804, 0.844)
    _quad(0.048, 0.173, 0.804, 0.844)
    _quad(0.237, 0.278, 0.804, 0.844)
    tdu_faces = [
        bm.faces.new(tuple(bm.verts.new(point) for point in tdu_loop))
    ]
    mpd_faces = [
        bm.faces.new(tuple(bm.verts.new(point) for point in mpd_l_loop)),
        bm.faces.new(tuple(bm.verts.new(point) for point in mpd_r_loop)),
    ]
...
def canopy_shell_station_rings(xs, y_sign: float):
    ...
    Closed 6-point hull: sill-in, belly-in, belly, rail-out, crown-out,
    crown, back to sill-in. crown_y = inner_y so |y| >= 0.24.
...
def loft_canopy_shell(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    for col in range(len(columns) - 1):
        ...
        for row in range(ring_len):
            ...
            if (
                window_punch[0] <= mid_x <= window_punch[1]
...
def loft_forward_windshield(collection, glass, rail) -> None:
    ...
    four copies of one rectangle. The pane fills roughly x 0.55-0.86,
    |y| <= 0.20, z 1.05-1.36. Do not emit GEO_WindshieldFrame; those
...
def assert_not_sixty_mesh_kit(collection) -> None:
    """model28 must drop the 60-mesh cage family. Fail if it still emits 60 meshes."""
    meshes = [obj for obj in collection.all_objects if obj.type == "MESH"]
    if len(meshes) >= 60:
        raise WorkerError(
            f"model28 still emits 60 meshes of the same family "
    ...
    for forbidden in (
        ...
        if forbidden in names:
            raise WorkerError(f"model28 still emits {forbidden}.")
    if any(name.startswith("GEO_JointPlate") for name in names):
        raise WorkerError("model28 still emits GEO_JointPlate cage members.")
