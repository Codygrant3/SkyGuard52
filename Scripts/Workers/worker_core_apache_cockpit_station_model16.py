...
"""Governed AH-64 CPG station-model16 worker.

Keeps the model15 layout, sockets, look-out, inset canopy panes, glass
overhead brow, TEDAC green emit, spun knobs, HOCAS heads, dark formed
greenhouse rails, formed joint plates, dark interior, non-white glass,
and the model14-15 bucket seat. Side MPDs stay inset in the TEDAC
housing, but GEO_MPD_L / GEO_MPD_R emit faces must face the eye
(alignment >= 0.55). model15 attempt_20260816T223609855931Z failed
assert_emit_faces_eye on GEO_MPD_L (alignment=0.325). Real bmesh only.
...
ASSET_ID = "core-apache-cockpit-station-model16"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def loft_canopy_pane(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    window z (skirt_frac / header_frac). Plate stays the body. Glass stays
    MAT_CPG_CanopyGlass. Stays outboard so eye-forward does not see stacked
    inward fins. Bay-plate inner_y stays >= 0.24.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
...
def loft_canopy_bay(collection, material, name: str, y_sign: float, xs) -> None:
    """Formed-plate bay that fills the bay so the greenhouse shell owns the silhouette.

    Lower skirt, upper header, and end closers are wide enough that rails,
    sills, and A-pillars read as thin trim on the plate, not the structure
    you see first. Punched window openings stay. Front look-out stays open.
    ...
    # Plate sits outboard of the 0.052 rail so the shell, not the beam, is the
    # three-quarter silhouette. Members stay the same section size.
    outer_overhang = 0.034
    sill_underhang = 0.014
    rail_overhang = 0.018
    skirt_frac = 0.42
    header_frac = 0.36
    end_closer = 0.090
    ...
    def _ring(x_value, z0, z1, y_outer, y_inner):
        mid_z = z0 + (z1 - z0) * 0.5
        mid_y = max(y_outer, y_inner) + 0.010
        return [
    ...
    bmesh.ops.solidify(bm, geom=list(bm.faces), thickness=y_sign * 0.014)
    object_from_bmesh(name, bm, collection, [material])
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    ...
    # the eye. No bow bar across the look-out. The greenhouse shell owns
    # the silhouette: formed bay plates fill the bays so members read as
    # thin trim on punched window openings, not a beam cage. Members stay
    # formed rectangular sections, not a tube sweep.
    sections = {
        "GEO_Rail_L": (
            [
                (-0.30, -0.418, 1.06),
