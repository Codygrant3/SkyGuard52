...
"""Governed AH-64 CPG station-model11 worker.

Keeps the model10 layout, sockets, look-out, glass overhead brow,
side canopy skins, hoods, emit wells, TEDAC readability, spun knobs,
HOCAS heads, and the model10 bucket seat. Dark formed greenhouse:
rails, sills, and A-pillars use a dark canopy-rail value plus formed
joint plates so three-quarter reads as one dark enclosure — not a
white angular kit. Real bmesh only. Public layout only. Outputs go
only to --output.
"""
...
ASSET_ID = "core-apache-cockpit-station-model11"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_bezel(
    ...
    if corner_cut > 0.0:
        # Taper the housing so the bezel reads formed, not a raw box.
        for vert in back_verts:
            vert.co.y += (vert.co.y - center[1]) * 0.10
            vert.co.z += (vert.co.z - center[2]) * 0.10
...
def build_greenhouse(collection, rail, glass) -> None:
    ...
    # the eye. No bow bar across the look-out. Canopy skin enclosure fills
    # the bay. Members stay formed rectangular sections, not a tube sweep.
    sections = {
        "GEO_Rail_L": (
            [
                (-0.30, -0.418, 1.06),
