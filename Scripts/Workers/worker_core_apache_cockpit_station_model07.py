...
"""Governed AH-64 CPG station-model07 worker.

Keeps the model06 layout, sockets, look-out, side canopy skins, hoods,
emit wells, TEDAC readability, spun knobs, and HOCAS heads. Adds a
lofted overhead brow that connects the left and right upper rails
above the look-out so three-quarter reads as a cockpit enclosure, not
an open cage. Darker bucket seat keeps a visible dish. Real bmesh
only. Public layout only. Outputs go only to --output.
"""
...
ASSET_ID = "core-apache-cockpit-station-model07"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def build_seat(collection, material) -> None:
    """Bucket seat: visible dish and bolster walls from three-quarter, not a flat cushion."""
    bmesh = bmesh_module()
    ...
    for name, y_sign in (("GEO_SeatBolster_L", -1.0), ("GEO_SeatBolster_R", 1.0)):
        ...
        y1 = y_sign * 0.238
        verts = [
            bm.verts.new((-0.18, y0, 0.935)),
            bm.verts.new((0.16, y0, 0.898)),
    ...
    bmesh.ops.translate(back, verts=_bm_verts(extruded["geom"], bmesh), vec=(-0.052, 0.0, 0.012))
    bmesh.ops.inset_region(
        back,
        faces=[face],
        thickness=0.020,
...
def loft_canopy_skin(collection, material, name: str, y_sign: float) -> None:
    """Filled canopy skin between sill and rail. Enclosure sheet, not a tube sweep.

    Occupies x in [-0.20, 0.70]. Skin stays outboard of the look-out (|y| >= 0.20).
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    xs = (-0.20, -0.04, 0.12, 0.28, 0.44, 0.58, 0.70)
    lookout_half = 0.20
    ...
    for x_value in xs:
        ...
        mid_z = sill_z + (rail_z - sill_z) * 0.55
        mid_y = max(sill_y, rail_y) + 0.012
        ring = [
            (x_value, y_sign * inner_y, sill_z + 0.006),
            (x_value, y_sign * sill_y, sill_z + 0.006),
            (x_value, y_sign * mid_y, mid_z),
            (x_value, y_sign * rail_y, rail_z - 0.006),
            (x_value, y_sign * inner_y, rail_z - 0.006),
        ]
...
def loft_overhead_brow(collection, material) -> None:
    """Lofted overhead sheet and short forward brow connecting rail-L to rail-R.
...
def build_greenhouse(collection, rail, glass) -> None:
    bmesh = bmesh_module()
    # Formed greenhouse: extruded plate sills and lofted structural sections.
    # A-pillar and side-bow meet sill and upper rail with overlapping ends
    # and weld bulbs. |y| >= 0.38. Aft frame is behind the eye. No bow bar
    # across the look-out. Canopy skin enclosure fills the bay; members stay
    # formed rectangular sections, not a widened cage.
    sections = {
        "GEO_Rail_L": (
            [
                (-0.30, -0.418, 1.06),
    ...
    # Filled canopy skin enclosure between sill and rail. Not another sweep.
    loft_canopy_skin(collection, glass, "GEO_CanopyGlass_L", -1.0)
    loft_canopy_skin(collection, glass, "GEO_CanopyGlass_R", 1.0)
    # Overhead / forward brow connects the rails above the look-out.
    loft_overhead_brow(collection, rail)
...
def build_asset(asset_collection) -> None:
    ...
    glass = emit_material("MAT_CPG_CanopyGlass", (0.55, 0.62, 0.68), 0.08, 0.0, alpha=0.32)
    ...
    build_greenhouse(asset_collection, rail, glass)
