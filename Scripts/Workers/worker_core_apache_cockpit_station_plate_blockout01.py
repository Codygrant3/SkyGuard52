...
"""Governed AH-64 CPG station plate-blockout01 worker.

This is not model30. Do not copy the model29 greybox and retune
knobs. Build the blockout sketch by looking at the DCS / TM photos:
raked glass, one formed dash with screen wells, greenhouse hull.
Stop when eye_forward / eye_down_tedac / three_quarter look like
those photos, not a pile of boxes.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8, and the model29 visual fail. Keep the
model29 assert-path. This method does not supersede model29 until
accepted. Not another named mesh on the same 60-mesh cage.
Greenhouse hull owns three-quarter as a closed outer volume
(sill-in, belly, rail-out, crown, back to sill-in) with punched
discrete window bays — not a pane kit of floating canopy sheets
and shell slabs. Stop emitting the cage: do not create
GEO_CanopyBay_L/R or GEO_JointPlate_*. Rails are one thin trim
pair on that hull, not a chunky cage. eye_forward looks through
raked windshield / world/sky in the upper two-thirds; TEDAC and
dash occupy the lower third, not a dark rectangular well, not a
housing well, and not an opaque pane wall. model29 visual fail:
a 6-column cabin tunnel plus opaque side blocks boxed the
eyepoint so glass plus a sky Background never read as look-out.
Open the forward cabin — glass sits at the greenhouse nose, not
in the cabin — and flare the inner shell wall with cabin_inner_y
so that closed loop stays out of the eyepoint frustum. Delete
GEO_WindshieldFrame / shell faces that form a rectangular tunnel.
Canopy glass must read as glass, not a dark grey wall. One
continuous formed forward panel from dash_formed_front /
dash_well_cavities / dash_face_x, not a flat YZ plate and not
dash_panel_outline sibling faces in front of two flat cyan
screens. TDU and both MPD wells are recessed holes in that one
mesh. GEO_TEDAC is grips + Fig 45 buttons + hood only, sitting
on the dash well, no standalone emit rectangle in front of the
panel. After object_from_bmesh + finish_mesh bevel, GEO_Dash
...
then assert_emit_faces_eye (alignment >= 0.55). GEO_TEDAC has no
emit faces; do not orient or assert index 1 on it. Square TDU
plus LHG/RHG stay as hardware on the dash. TEDAC is not three stacked boxes.
Destacked TDU numbers are a ceiling: go smaller
than width 0.118 / depth 0.018 / hood 0.010. Raked windshield
stations stay different YZ trapezoids at the greenhouse nose.
Glass may occupy the forward look-out band; frames may not.
Keep glass overhead brow, dash z<=0.86, TEDAC green emit, MPD
...
ASSET_ID = "core-apache-cockpit-station-plate-blockout01"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def dash_face_x(y_value: float, z_value: float) -> float:
    """Plate-matched formed dash. Center closer to the eye; sides wrap back.

    Fig 43: one molded panel, not a flat rectangular block with a TEDAC
    frame standing on it. X varies with y (wrap) and z (brow step).
    Wells stay holes in this one mesh.
    """
    wrap = 0.030 * (y_value * y_value) / (0.278 * 0.278)
    step = 0.012 * max(0.0, (0.848 - z_value) / 0.128)
    return dash_front_x() + wrap + step
...
def formed_tedac_housing(collection, dark, tedac_emit, mpd_emit, tedac_mark) -> None:
    ...
    / dash_face_x. model29 visual fail: a flat YZ block plus a TEDAC
    frame read as a kit, not one formed dash. model28 visual fail:
    dash_panel_outline / dash_well_loops built
    ...
    # Formed front from dash_face_x(); wells from dash_well_cavities().
    ...
    for y_value in ys:
        for z_value in zs:
            front[(y_value, z_value)] = bm.verts.new(
                (dash_face_x(y_value, z_value), y_value, min(z_value, 0.848))
            )
    _ = front_x
    surround_faces = []
    tdu_faces = []
...
def cabin_inner_y(x_value: float) -> float:
    ...
    model29 visual fail: inner wall still read as two opaque side
    blocks in eye_forward. Keep the cabin open through the eyepoint
    frustum (|y| >= 0.50 near the eye). The wall only comes in to
    |y|=0.24 at the greenhouse nose, behind the raked pane. Crown
    stays |y| >= 0.24. Glass occupies the look-out. Structure may not.
    """
    floor = 0.24
    if x_value <= 0.50:
        return 0.50
    if x_value >= 0.70:
        return floor
    blend = (x_value - 0.50) / (0.70 - 0.50)
    return 0.50 + (floor - 0.50) * blend


def shell_window_bays() -> tuple[tuple[float, float], ...]:
    """Discrete greenhouse window bays. Not one giant punch that leaves C-slabs."""
    return (
        (-0.20, -0.04),
        (0.08, 0.28),
        (0.36, 0.58),
...
def canopy_shell_station_rings(xs, y_sign: float):
    ...
    for x_value in xs:
        ...
        y_outer = max(_sill_y, rail_y, inner_y) + 0.028
        y_belly = y_outer + 0.022
        crown_z = max(rail_z + 0.028, 1.38)
...
def loft_canopy_shell(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    closed loop, not a rectangular well. cabin_inner_y flares that
    wall out of the eyepoint frustum — that is the open-look-out
    move, not deleting the wall.
    ...
    of floating canopy sheets and shell slabs. model29 visual fail:
    aft end cap only plus a greybox pane/shell kit so
    GEO_CanopyShell did not own a closed greenhouse silhouette.
    Close aft and forward end caps. Do not solidify a thin ring
    into a C-slab. Loft an explicit inner/outer pair from
    ...
    # Aft and forward end caps close the greenhouse hull. Forward cap
    # verts stay |y| >= 0.24, outboard of the look-out, so the nose
    # closes three-quarter without boxing the eyepoint. Not aft-only
    # open tubes / floating C-slabs.
    bm.faces.new(tuple(outer_cols[0]))
    bm.faces.new(tuple(reversed(outer_cols[-1])))
    bm.faces.new(tuple(reversed(inner_cols[0])))
    bm.faces.new(tuple(inner_cols[-1]))
    object_from_bmesh(name, bm, collection, [material])


def windshield_lookout_stations():
    """Raked 4-point trapezoids of the forward pane at the greenhouse nose.
    ...
    rectangle. Dutch photo + Fig 43 + TM Fig 2-8: the windshield sits
    at the nose. The eye looks through it to world/sky. Not a
    6-column cabin tunnel from x=0.22. Glass still fills the look-out.
    """
    return (
        (
            (0.62, -0.198, 1.052),
...
def windshield_mid_station():
    """Inner face of the raked pane at the greenhouse nose.

    Not a cabin-filling near column at x=0.22-0.30. The eye looks
    through empty cabin, then this pane, then world/sky. Glass may
...
def windshield_pane_grid():
    """One raked sheet at the greenhouse nose. Not a 6-column cabin tunnel.

    model29 visual fail: a near column at x=0.22-0.30 plus a mid
    station at x=0.40-0.46 lofted with four far stations into a
    cabin tunnel / grey well. The cameras never saw a plate-matched
    windshield. Put glass at the nose: inner face plus the three
    raked stations. loft_forward_windshield emits these verts.
    """
    return (windshield_mid_station(),) + windshield_lookout_stations()
...
def loft_forward_windshield(collection, glass, rail) -> None:
    ...
    Dutch photo + Fig 43 + TM Fig 2-8: look through the raked
    windshield to world/sky. GEO_Windshield is the forward pane of that hull,
    not a separate rectangular box. Stations are raked YZ trapezoids at
    the greenhouse nose, not a 6-column cabin tunnel. The pane fills the
    look-out from the inner face through x 0.50-0.86, |y| <= 0.20,
    z 1.05-1.36. A shallow second surface from windshield_inset_grid sits
    toward +X and does not form a tunnel. Do not emit GEO_WindshieldFrame;
    those faces formed the rectangular tunnel around the look-out. Not a
    grey rectangular well. model29 visual fail: opaque side blocks plus a
    cabin tunnel boxed the eyepoint so glass never read as look-out.
    loft_forward_windshield emits windshield_pane_grid() so the live verts
    are the inner face / mid station at the nose plus the three raked
    stations. Not a grey rectangular well, not an opaque grey slab / opaque pane wall.
...
def assert_not_sixty_mesh_kit(collection) -> None:
    """plate-blockout01 must drop the 60-mesh cage family. Fail if it still emits 60 meshes."""
    meshes = [obj for obj in collection.all_objects if obj.type == "MESH"]
    if len(meshes) >= 60:
        raise WorkerError(
            f"plate-blockout01 still emits 60 meshes of the same family "
    ...
    for forbidden in (
        ...
        if forbidden in names:
            raise WorkerError(f"plate-blockout01 still emits {forbidden}.")
    if any(name.startswith("GEO_JointPlate") for name in names):
        raise WorkerError("plate-blockout01 still emits GEO_JointPlate cage members.")
