...
"""Governed AH-64 CPG station-model14 worker.

Keeps the model13 layout, sockets, look-out, glass overhead brow,
...
model13 bucket seat. The greenhouse shell owns the silhouette: formed
bay plates fill the bays so rails, sills, and A-pillars read as thin
trim on punched window openings, not a beam cage. Glass stays in the
window openings only. TEDAC plus left/right monitors wrap into one
formed housing while the green emit and public crosshair stay visible
from eye_down_tedac. Real bmesh only. Public layout only. Outputs go
only to --output.
"""
...
ASSET_ID = "core-apache-cockpit-station-model14"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark) -> None:
    ...
    crosshair stay visible from eye_down_tedac. Left/right MPD boxes share
    this bezel; they must not read as separate floating monitors.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
...
def loft_canopy_skin(collection, material, name: str, y_sign: float, xs=None) -> None:
    """Glass canopy skin in a punched window opening. Not a milky sheet over the cage.

    Enclosure sheet, not a tube sweep. Stays outboard of the look-out
    (|y| >= 0.20). Default xs is the forward side window opening only,
    matching the bay-plate punch so glass does not drape the whole shell.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    if xs is None:
        xs = (0.18, 0.30, 0.42, 0.54)
    lookout_half = 0.20
    ...
    for x_value in xs:
        ...
        win_sill_z = sill_z + span_z * 0.42
        win_rail_z = rail_z - span_z * 0.36
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    ...
    # the silhouette. Rails read as thin trim. Glass stays in the punched
    # window openings only — do not drape one milky sheet over the cage.
    loft_canopy_bay(
        collection,
        plate,
        "GEO_CanopyBay_L",
