...
"""Governed AH-64 CPG station-model01 worker.

Front-seat station modeled with bmesh (extrude, inset, solidify, bridge, spin).
...
ASSET_ID = "core-apache-cockpit-station-model01"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_bezel(
    ...
    half_h = height * 0.5
    front = [
        _rotate_z((center[0], center[1] - half_w, center[2] - half_h), center, toe),
        _rotate_z((center[0], center[1] + half_w, center[2] - half_h), center, toe),
        _rotate_z((center[0], center[1] + half_w, center[2] + half_h), center, toe),
        _rotate_z((center[0], center[1] - half_w, center[2] + half_h), center, toe),
    ]
    ...
    well_extrude = bmesh.ops.extrude_face_region(bm, geom=[front_face])
    ...
    for well_face in _bm_faces(well_extrude["geom"], bmesh):
        well_face.material_index = emit_index
    front_face.material_index = emit_index
    if hood <= 0.0:
        return
    bm.faces.ensure_lookup_table()
    rim = [
        ...
        if item.material_index == body_index
        and abs(item.normal.x * cosine + item.normal.y * sine) < 0.35
        and item.calc_center_median().x > center[0] - 0.02
    ]
    if not rim:
        return
    hood_extrude = bmesh.ops.extrude_face_region(bm, geom=rim)
    hood_verts = _bm_verts(hood_extrude["geom"], bmesh)
    bmesh.ops.translate(bm, verts=hood_verts, vec=(cosine * hood, sine * hood, 0.0))
    for vert in hood_verts:
        flare_y = 0.012 if vert.co.y >= center[1] else -0.012
        flare_z = 0.010 if vert.co.z >= center[2] else -0.010
        bmesh.ops.translate(bm, verts=[vert], vec=(0.0, flare_y, flare_z))
...
def build_seat(collection, material) -> None:
    ...
    bmesh.ops.create_grid(
        ...
        y_segments=6,
        size=0.46,
        matrix=matrix.Translation((-0.13, 0.0, 0.575)),
    )
    for vert in bm.verts:
        x_value, y_value, z_value = vert.co.x, vert.co.y, vert.co.z
        if x_value > -0.20:
            dish = (1.0 - min(1.0, (abs(y_value) / 0.23) ** 2)) * 0.024
            vert.co.z = z_value - dish
            if abs(y_value) > 0.13:
                vert.co.z += (abs(y_value) - 0.13) * 0.48
        else:
            wrap = min(1.0, (-0.20 - x_value) / 0.17)
            vert.co.z = z_value + wrap * 0.76
            vert.co.x = x_value - wrap * 0.03
            if wrap > 0.78:
                vert.co.y *= 0.52
            elif wrap > 0.42:
                vert.co.y *= 0.78
    bmesh.ops.solidify(bm, geom=list(bm.faces), thickness=0.030)
    object_from_bmesh("GEO_Seat", bm, collection, [material])
...
def build_dash_shelf(collection, olive, dark) -> None:
    ...
    for step in range(segments + 1):
        y_value = -0.40 + 0.80 * step / segments
        rings.append([bm.verts.new((x_value, y_value, z_value)) for x_value, z_value in profile])
    count = len(profile)
    for step in range(segments):
        for index in range(count):
            face = bm.faces.new(
                (
                    rings[step][index],
                    rings[step][(index + 1) % count],
                    rings[step + 1][(index + 1) % count],
                    rings[step + 1][index],
                )
            )
            center = face.calc_center_median()
            face.material_index = 1 if center.z > 0.80 else 0
    bm.faces.new(list(reversed(rings[0])))
    bm.faces.new(rings[-1])
    object_from_bmesh("GEO_DashShelf", bm, collection, [olive, dark])


def build_tedac(collection, dark, emit, grip) -> None:
    bmesh = bmesh_module()
    bm = bmesh.new()
    formed_bezel(
        bm,
    ...
    object_from_bmesh("GEO_TEDAC", bm, collection, [dark, emit, grip])
...
def build_mpd(name: str, center, toe: float, collection, dark, emit) -> None:
    ...
    object_from_bmesh(name, bm, collection, [dark, emit])
...
def build_console(name: str, y_center: float, collection, olive, dark) -> None:
    ...
    half_y = 0.055
    top = [
        bm.verts.new((-0.16, y_center - half_y, 0.695)),
        bm.verts.new((0.36, y_center - half_y, 0.695)),
        bm.verts.new((0.36, y_center + half_y, 0.695)),
        bm.verts.new((-0.16, y_center + half_y, 0.695)),
    ]
...
def add_knob(name: str, loc, material, collection) -> None:
    bpy = blender_module()
    ...
    obj.name = name
    obj.data.materials.append(material)
    finish_mesh(obj, collection)


def formed_head(name: str, center, size, collection, material) -> None:
    bmesh = bmesh_module()
    bm = bmesh.new()
    half_y = size[1] * 0.5
    half_z = size[2] * 0.5
    verts = [
        bm.verts.new((center[0], center[1] - half_y, center[2] - half_z)),
        bm.verts.new((center[0], center[1] + half_y, center[2] - half_z)),
        bm.verts.new((center[0], center[1] + half_y, center[2] + half_z)),
        bm.verts.new((center[0], center[1] - half_y, center[2] + half_z)),
    ]
    face = bm.faces.new(verts)
    extruded = bmesh.ops.extrude_face_region(bm, geom=[face])
    bmesh.ops.translate(bm, verts=_bm_verts(extruded["geom"], bmesh), vec=(size[0], 0.0, 0.0))
    bm.faces.ensure_lookup_table()
...
def build_hocas(collection, grip) -> None:
    ...
    formed_head("GEO_CollectiveHead", (0.185, -0.28, 0.86), (0.055, 0.038, 0.046), collection, grip)
    ...
    formed_head("GEO_CyclicHead", (0.195, -0.06, 0.875), (0.048, 0.032, 0.058), collection, grip)


def build_greenhouse(collection, rail) -> None:
    bmesh = bmesh_module()
    paths = {
        "GEO_Rail_L": [(-0.30, -0.41, 1.06), (0.08, -0.435, 1.30), (0.42, -0.42, 1.22), (0.76, -0.38, 1.07)],
        "GEO_Rail_R": [(-0.30, 0.41, 1.06), (0.08, 0.435, 1.30), (0.42, 0.42, 1.22), (0.76, 0.38, 1.07)],
        "GEO_Sill_L": [(-0.22, -0.395, 0.955), (0.28, -0.40, 0.948), (0.70, -0.38, 0.940)],
        "GEO_Sill_R": [(-0.22, 0.395, 0.955), (0.28, 0.40, 0.948), (0.70, 0.38, 0.940)],
        "GEO_BowFrame": [(0.80, -0.38, 1.055), (0.80, 0.0, 1.065), (0.80, 0.38, 1.055)],
...
def build_gloves(collection, material) -> None:
    ...
    for name, center, axis in (
        ("GEO_Glove_L", (0.458, -0.138, 0.718), (0.2, -0.4, -0.7)),
        ("GEO_Glove_R", (0.458, 0.138, 0.718), (0.2, 0.4, -0.7)),
    ):
        bm = bmesh.new()
...
def build_asset(asset_collection) -> None:
    ...
    build_tedac(asset_collection, dark, tedac, grip)
    ...
    for index, x_off in enumerate((-0.10, 0.04, 0.18, 0.30)):
        add_knob(f"GEO_Knob_L_{index + 1}", (0.08 + x_off, -0.36, 0.712), dark, asset_collection)
        add_knob(f"GEO_Knob_R_{index + 1}", (0.08 + x_off, 0.36, 0.712), dark, asset_collection)
