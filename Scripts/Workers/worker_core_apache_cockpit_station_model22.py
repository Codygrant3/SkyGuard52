...
"""Governed AH-64 CPG station-model22 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model21 cycle fail, not a
plate-knob pass. Keep model21 architecture. Not another fill plate
on the same cage. Greenhouse shell owns three-quarter as the visible
outer volume with punched window openings; rails, sills, and
A-pillars are thin trim on that shell. eye_forward looks through
raked windshield / sky in the upper two-thirds; TEDAC and dash
occupy the lower third, not a housing well. One continuous formed
forward panel with inset TDU and MPD wells at the same x. Square
TDU plus LHG/RHG stay as hardware on the dash, not a 0.188 cube.
TEDAC is not three stacked boxes. Raked windshield stations stay
different YZ trapezoids. Glass may occupy the forward look-out
band; frames may not. Keep glass overhead brow, dash z<=0.86,
TEDAC green emit, MPD emit-face fix, and the model14-21 bucket
seat. GEO_Dash look-out assert stays; after finish_mesh bevel,
clamp |y|<0.20 verts to z<=0.858 and drop the housing grid top
to 0.848 so bevel cannot recreate the model21 fail. Dark interior,
dark rail, olive, and non-white glass stay. Do not restore a draped
...
ASSET_ID = "core-apache-cockpit-station-model22"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def orient_emit_faces_to_eye(obj, emit_index: int, toe: float, body_index: int = 0) -> None:
    ...
    plus recalc_face_normals reversed the emit winding. Re-orient here so
...
def formed_tedac_housing(collection, dark) -> None:
    """One continuous formed forward panel. TDU and both MPDs are inset wells.

    GEO_Dash is that one panel. Not a TEDAC box plus two monitors at a
    different x. The surround is a single formed face from MPD_L
    (y=-0.205) to MPD_R (y=0.205) at the TEDAC x. Wells recede so
    GEO_TEDAC emit and both MPD screens stay visible from the eye.
    Dash cap remains z<=0.86 so eye_forward is not a housing well.
    Housing grid top is 0.848 so finish_mesh bevel cannot recreate
    the model21 fail (0.86 ring lifted over 0.86).
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    ys = (
    ...
    zs = (0.720, 0.752, 0.784, 0.816, 0.840, 0.848)

    def _well(y_value: float, z_value: float) -> float:
        tdu = max(0.0, 1.0 - (abs(y_value) / 0.094) ** 2) * max(
            0.0, 1.0 - (abs(z_value - 0.840) / 0.070) ** 2
        )
        mpd_l = max(0.0, 1.0 - (abs(y_value + 0.205) / 0.058) ** 2) * max(
            0.0, 1.0 - (abs(z_value - 0.840) / 0.050) ** 2
        )
        mpd_r = max(0.0, 1.0 - (abs(y_value - 0.205) / 0.058) ** 2) * max(
            0.0, 1.0 - (abs(z_value - 0.840) / 0.050) ** 2
        )
        return max(tdu, mpd_l, mpd_r)

    cols = []
    for y_value in ys:
        col = []
        for z_value in zs:
            well = _well(y_value, z_value)
            x_value = 0.50 + well * 0.022
            edge = max(0.0, (abs(y_value) - 0.220) / 0.058)
            x_value += edge * 0.010
            col.append(bm.verts.new((x_value, y_value, min(z_value, 0.848))))
        cols.append(col)
    ...
    bmesh.ops.solidify(bm, geom=list(bm.faces), thickness=0.016)
    for vert in bm.verts:
        if abs(vert.co.y) < 0.20 and vert.co.z > 0.848:
            vert.co.z = 0.848
    object_from_bmesh("GEO_Dash", bm, collection, [dark])
...
def build_tedac(collection, dark, emit, grip, mark) -> None:
    ...
    Single GEO_TEDAC: square TADS Display Unit, Left Hand Grip, Right Hand
    Grip, and Fig 45 bezel (TAD/FCR/PNV/G/S, DAY-NT-OFF, LEV/GAIN,
    SYM/BRT/CON, AZ, four action buttons). Public shapes only. Not a
    0.188 cube sitting in front of the panel.
...
def build_mpd(name: str, center, toe: float, collection, dark, emit) -> None:
    ...
    same x as the TDU. Not standalone extruded rectangles beside a
    TEDAC cube. The continuous panel owns the surround. Emit faces
    must pass assert_emit_faces_eye (alignment >= 0.55).
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    half_w = 0.048
    half_h = 0.040
    well_depth = 0.008
    # Front sits on the housing plane, then recesses into the dash well.
    emit_along = 0.0
    cosine = math.cos(toe)
    sine = math.sin(toe)
    # Wound so the front faces the eye (-X after toe).
    # model15 used the opposite winding and GEO_MPD_L failed at 0.325.
    corners = (
        _toed_point(center, emit_along, half_w, -half_h, toe),
        _toed_point(center, emit_along, -half_w, -half_h, toe),
        _toed_point(center, emit_along, -half_w, half_h, toe),
        _toed_point(center, emit_along, half_w, half_h, toe),
    )
    ...
    extruded = bmesh.ops.extrude_face_region(bm, geom=[face])
    back_verts = _bm_verts(extruded["geom"], bmesh)
    bmesh.ops.translate(bm, verts=back_verts, vec=(cosine * well_depth, sine * well_depth, 0.0))
    # Only the eye-facing cap keeps emit. Sides and the back wall are body
    # so bevel leftovers cannot fail assert_emit_faces_eye at 0.325.
...
def loft_canopy_pane(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    for x_value in xs:
        ...
        win_sill_z = sill_z + span_z * skirt_frac
        win_rail_z = rail_z - span_z * header_frac
        pane_sill = win_sill_z + pane_inset
        pane_rail = win_rail_z - pane_inset
        outer_y = max(sill_y, rail_y) + outer_overhang
        pane_outer = outer_y - pane_inset
        pane_inner = pane_outer - pane_depth
        ring = [
...
def loft_canopy_bay(collection, material, name: str, y_sign: float, xs) -> None:
    """Window frame around a punched opening. Thick black frame, not a beige plate wall.
...
def loft_canopy_shell(collection, material, name: str, y_sign: float, xs) -> None:
    """Formed L/R enclosure that owns the three-quarter silhouette.

    One punched shell is the visible outer volume. Rails, sills, and
    A-pillars stay thin trim on this shell. Not another fill plate on
    the same section_along cage. Not a draped canopy skin. Not
    model17 plate knobs. Inset GEO_CanopyPane_* sit in the punched holes.
    The U-profile is five points (sill-in, sill-out, belly, rail-out,
    rail-in) so the inboard edge stays open as the window. Raked
    GEO_Windshield stays the forward look-out.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    lookout_half = 0.20
    ...
    for x_value in xs:
        ...
        # Five-point U-profile. Do not close 4->0: that opening is the
        # punched window. Not a 4-ring plate hung on the cage.
        columns.append(
            [
                bm.verts.new((x_value, y_sign * inner_y, sill_z)),
                bm.verts.new((x_value, y_sign * y_outer, sill_z)),
                bm.verts.new((x_value, y_sign * y_belly, mid_z)),
                bm.verts.new((x_value, y_sign * y_outer, rail_z)),
                bm.verts.new((x_value, y_sign * inner_y, rail_z)),
            ]
        )
    for col in range(len(columns) - 1):
        for row in range(len(columns[0]) - 1):
    ...
    # Aft and forward end caps keep the shell a volume without filling
    # the punched inboard window.
    for col in (0, -1):
        ring = columns[col]
        bm.faces.new((ring[0], ring[1], ring[2]))
        bm.faces.new((ring[2], ring[3], ring[4]))
    bmesh.ops.solidify(bm, geom=list(bm.faces), thickness=y_sign * shell_depth)
    object_from_bmesh(name, bm, collection, [material])
...
def loft_forward_windshield(collection, glass, rail) -> None:
    ...
    Dutch photo + TM Fig 2-8: a real CPG must look through framed glass.
    Stations are raked YZ trapezoids, not four copies of one rectangle.
    The pane fills roughly x 0.55-0.86, |y| <= 0.20, z 1.05-1.36. Thick
    black frame only on the edges, outboard of |y|>=0.20 or below
    z=1.05 / above z=1.36.
    """
    bmesh = bmesh_module()
    glass_bm = bmesh.new()
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    ...
    # shell owns three-quarter as the visible outer volume with punched
    # window openings. Rails, sills, and A-pillars are thin trim on that
    # shell, not the silhouette. Do not add another loft_canopy_fill
    # plate on the same cage. |y| >= 0.38. Aft frame is behind the eye.
    # No bow bar across the look-out. Members stay formed rectangular
    # sections, not a tube sweep. Do not thicken section_along members.
    # No tube sweep inside this greenhouse builder.
    sections = {
        "GEO_Rail_L": (
            [
                (-0.30, -0.418, 1.06),
    ...
    # Window frames around punched openings. Rails read as frame trim.
    # Inset glass panes sit in those openings only — do not drape a milky sheet.
    loft_canopy_bay(
        collection,
        rail,
        "GEO_CanopyBay_L",
        -1.0,
        (-0.28, -0.10, 0.08, 0.28, 0.48, 0.64, 0.76),
    )
    loft_canopy_bay(
        collection,
        rail,
    ...
    loft_canopy_shell(collection, plate, "GEO_CanopyShell_L", -1.0, (-0.28, -0.10, 0.08, 0.28, 0.48, 0.64, 0.76))
    loft_canopy_shell(collection, plate, "GEO_CanopyShell_R", 1.0, (-0.28, -0.10, 0.08, 0.28, 0.48, 0.64, 0.76))
    loft_canopy_pane(
        collection,
        glass,
        "GEO_CanopyPane_L",
...
def render_eyepoint_views(output: Path) -> list[Path]:
    ...
    eye = Vector(EYE)
    # Upper two-thirds is raked windshield / sky, not a grey housing well.
