...
"""Governed AH-64 CPG station-model26 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model25 cycle fail, not a
plate-knob pass. Not another named mesh on the same 60-mesh cage.
Greenhouse hull owns three-quarter as a closed outer volume
(sill-in, belly, rail-out, crown, back to sill-in) with punched
window openings. Stop emitting the cage: do not create
...
occupy the lower third, not a grey rectangular well and not a
housing well. Delete GEO_WindshieldFrame / shell end-cap / brow
faces that form a rectangular tunnel. Canopy glass must read as
glass, not a dark grey wall. One continuous formed forward panel
with TDU and both MPD wells as holes in that one face. GEO_TEDAC
is grips + Fig 45 buttons + hood only, sitting on the dash
...
emit-face fix, and the model14-25 bucket seat. GEO_Dash look-out
...
ASSET_ID = "core-apache-cockpit-station-model26"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark, tedac_emit, mpd_emit, tedac_mark) -> None:
    ...
    GEO_Dash is that one panel. TDU and both MPD wells are holes in that one face
    (inset + emit material), not sibling GEO_TEDAC /
    GEO_MPD meshes sitting in front of the panel. The surround is a
    single formed face from MPD_L (y=-0.205) to MPD_R (y=0.205) at
    the TEDAC x. Inset wells recede so the TDU and both MPD screens
    stay visible from the eye. Dash cap remains z<=0.86 so
    ...
    at index 1 after finish_mesh bevel.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    ys = HOUSING_YS
...
def loft_canopy_shell(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    for x_value in xs:
        ...
        # back to sill-in. Punch the inboard wall for window openings.
        columns.append(
            [
                ...
                bm.verts.new((x_value, y_sign * inner_y, crown_z)),
            ]
        )
    for col in range(len(columns) - 1):
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    ...
    # window openings. Stop emitting the cage. Rails are one thin trim
...
def assert_not_sixty_mesh_kit(collection) -> None:
    """model26 must drop the 60-mesh cage family. Fail if it still emits 60 meshes."""
    meshes = [obj for obj in collection.all_objects if obj.type == "MESH"]
    if len(meshes) >= 60:
        raise WorkerError(
            f"model26 still emits 60 meshes of the same family "
    ...
    for forbidden in (
        ...
        if forbidden in names:
            raise WorkerError(f"model26 still emits {forbidden}.")
    if any(name.startswith("GEO_JointPlate") for name in names):
        raise WorkerError("model26 still emits GEO_JointPlate cage members.")
...
def build_asset(asset_collection) -> None:
    ...
    # Clearer glass so eye_forward reads as sky, not a dark grey wall.
