...
"""Governed AH-64 CPG station-model05 worker.

Keeps the model04 layout, sockets, look-out, hoods, emit wells, TEDAC
readability, spun knobs, and HOCAS heads. Replaces the pipe-kit canopy
with formed rectangular greenhouse members and a deeper bucket seat.
...
ASSET_ID = "core-apache-cockpit-station-model05"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def build_seat(collection, material) -> None:
    """Bucket seat: deeper dished pan, clearer bolster break, cupped headrest."""
    bmesh = bmesh_module()
    ...
    for vert in pan.verts:
        ...
        dish = (1.0 - min(1.0, (abs(y_value) / 0.18) ** 2)) * 0.058
        fore = max(0.0, min(1.0, (x_value + 0.18) / 0.36))
        vert.co.z = z_value - dish * (0.55 + 0.70 * fore)
        if abs(y_value) > 0.095:
            vert.co.z += (abs(y_value) - 0.095) * 0.95
    bmesh.ops.solidify(pan, geom=list(pan.faces), thickness=0.032)
    ...
    for name, y_sign in (("GEO_SeatBolster_L", -1.0), ("GEO_SeatBolster_R", 1.0)):
        ...
        y1 = y_sign * 0.232
        verts = [
            bm.verts.new((-0.18, y0, 0.758)),
            bm.verts.new((0.16, y0, 0.728)),
            bm.verts.new((0.16, y1, 0.652)),
            bm.verts.new((-0.18, y1, 0.678)),
        ]
        face = bm.faces.new(verts)
        extruded = bmesh.ops.extrude_face_region(bm, geom=[face])
        bmesh.ops.translate(bm, verts=_bm_verts(extruded["geom"], bmesh), vec=(0.0, 0.0, -0.112))
        bmesh.ops.inset_region(
            bm,
            faces=[face],
            thickness=0.012,
...
def build_greenhouse(collection, rail, glass) -> None:
    ...
    # across the look-out. Optional glass stays outboard of the look-out band.
    sections = {
        "GEO_Rail_L": (
            [
                (-0.30, -0.418, 1.06),
    ...
    # Thin canopy glass only outboard of the look-out band (|y| >= 0.20).
    for name, y_sign in (("GEO_CanopyGlass_L", -1.0), ("GEO_CanopyGlass_R", 1.0)):
        pane = bmesh.new()
        y_value = y_sign * 0.430
        xs = (-0.18, 0.04, 0.26, 0.48, 0.66)
        z_sill = (0.958, 0.956, 0.952, 0.948, 0.944)
        z_rail = (1.14, 1.22, 1.26, 1.18, 1.10)
        bottom = [pane.verts.new((x_value, y_value, z_value)) for x_value, z_value in zip(xs, z_sill)]
        top = [pane.verts.new((x_value, y_value, z_value)) for x_value, z_value in zip(xs, z_rail)]
        for index in range(len(xs) - 1):
            pane.faces.new((bottom[index], bottom[index + 1], top[index + 1], top[index]))
        bmesh.ops.solidify(pane, geom=list(pane.faces), thickness=y_sign * 0.004)
        object_from_bmesh(name, pane, collection, [glass])
...
def build_asset(asset_collection) -> None:
    ...
    glass = emit_material("MAT_CPG_CanopyGlass", (0.55, 0.62, 0.68), 0.04, 0.0, alpha=0.18)
