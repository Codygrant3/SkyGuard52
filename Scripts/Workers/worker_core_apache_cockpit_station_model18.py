...
"""Governed AH-64 CPG station-model18 worker.

Built from DCS AH-64D Fig 43-45 and TM-1-1520-238-10 reference plates,
not a plate-knob pass. One formed forward panel: one square TDU with
LHG/RHG, inset MPDs, EUFD strip, fire panel, BRU, armament strip. TEDAC
is not three stacked boxes. Greenhouse from TM Fig 2-8 and the Dutch
photo: thick black frames, punched windows, framed windshield panes,
thin wall-deck consoles. Keep model17 layout sockets, look-out, glass
overhead brow, dash z<=0.86, TEDAC green emit, MPD emit-face fix, and
the model14-17 bucket seat. Dark interior, dark rail, olive, and
non-white glass stay. Do not restore a draped canopy skin. Do not
thicken section_along members. No tube sweep in the greenhouse. Real
...
ASSET_ID = "core-apache-cockpit-station-model18"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark) -> None:
    """Single formed surround behind the one square TDU. Not three stacked boxes.
    ...
    inset in the same formed forward panel, not separate floating monitors.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    # One square surround behind the TDU only. Do not stack left/right boxes.
    _formed_plate(bm, 0.548, 0.582, -0.102, 0.102, 0.786, 0.974)
    _formed_plate(bm, 0.500, 0.548, -0.028, 0.028, 0.728, 0.786)
...
def loft_canopy_bay(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    def _frame_ring(x_value, sill_z, rail_z, y_outer):
        span_z = max(0.04, rail_z - sill_z)
        win_sill = sill_z + span_z * 0.18
        win_rail = rail_z - span_z * 0.16
        outer = [
...
def loft_forward_windshield(collection, glass, rail) -> None:
    """Large open look-out pane. Glass sits FORWARD of the fail band (x > 0.85).

    Framed windshield from TM Fig 2-8 and the Dutch photo. The look-out
    volume stays empty. Crown/overhead glass stays at z >= 1.36.
    """
    bmesh = bmesh_module()
    glass_bm = bmesh.new()
    # Pane just forward of x=0.85 so the fail band stays empty.
    xs = (0.858, 0.872, 0.886)
    columns = []
    for x_value in xs:
        ring = [
            (x_value, -0.22, 1.000),
    ...
    bmesh.ops.solidify(glass_bm, geom=list(glass_bm.faces), thickness=0.006)
    object_from_bmesh("GEO_Windshield", glass_bm, collection, [glass])

    frame = bmesh.new()
    _formed_plate(frame, 0.854, 0.890, -0.248, -0.210, 0.988, 1.362)
    _formed_plate(frame, 0.854, 0.890, 0.210, 0.248, 0.988, 1.362)
    _formed_plate(frame, 0.854, 0.890, -0.248, 0.248, 0.976, 1.000)
