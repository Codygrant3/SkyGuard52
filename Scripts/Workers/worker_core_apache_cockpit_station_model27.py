...
"""Governed AH-64 CPG station-model27 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model26 visual fail, not a
plate-knob pass. Keep the model26 assert-path. Not another named
...
to sill-in) with punched window openings — not a pane kit of
floating canopy sheets and shell slabs. Stop emitting the cage:
...
looks through raked windshield / sky in the upper two-thirds;
TEDAC and dash occupy the lower third, not a grey rectangular
well, not a housing well, and not an opaque pane wall. Delete or
punch the opaque grey slab that fills the glass. Delete
GEO_WindshieldFrame / shell end-cap / brow faces that form a
rectangular tunnel. Canopy glass must read as glass, not a dark
grey wall. One continuous formed forward panel with TDU and both
MPD wells as recessed holes in that one face, not a grey bezel
rectangle in front of two flat cyan screens. GEO_TEDAC is grips
+ Fig 45 buttons + hood only, sitting on the dash well, no
standalone emit rectangle in front of the panel. After
object_from_bmesh + finish_mesh bevel, GEO_Dash emit faces at
index 1 (TDU) and index 2 (MPD) are tagged so they survive the
model25 miss (housing-grid face centers at y=±0.05 missed
abs(y)<0.048) and then call orient_emit_faces_to_eye then
assert_emit_faces_eye (alignment >= 0.55). GEO_TEDAC has no emit
...
z<=0.858 and drop the housing grid top to 0.848 so bevel cannot
...
ASSET_ID = "core-apache-cockpit-station-model27"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
SCREEN_M = 0.127
# Housing Y samples include ±0.046 so a TDU face is centered near y=0.
# model25 used (..., -0.100, 0.0, 0.100, ...) so face centers landed at
# y=±0.05 and missed abs(y)<0.048.
HOUSING_YS = (
    -0.278,
    -0.240,
    -0.205,
...
HOUSING_ZS = (0.720, 0.752, 0.784, 0.816, 0.840, 0.848)
REQUIRED_SOCKETS = [
    "SOCKET_Origin",
    "SOCKET_CPG_Eye",
    "SOCKET_TEDAC",
...
def housing_grid_face_centers() -> list[tuple[float, float]]:
    """Median YZ of each housing-grid quad before inset."""
    centers: list[tuple[float, float]] = []
    for col in range(len(HOUSING_YS) - 1):
        for row in range(len(HOUSING_ZS) - 1):
            y_value = 0.5 * (HOUSING_YS[col] + HOUSING_YS[col + 1])
            z_value = 0.5 * (HOUSING_ZS[row] + HOUSING_ZS[row + 1])
            centers.append((y_value, z_value))
    return centers
...
def formed_tedac_housing(collection, dark, tedac_emit, mpd_emit, tedac_mark) -> None:
    """One continuous formed forward panel. TDU and both MPDs are holes.

    GEO_Dash is that one panel. TDU and both MPD wells are recessed
    holes in that one face (inset + emit material), not a grey bezel rectangle
    sitting in front of two flat cyan screens, and not
    sibling GEO_TEDAC / GEO_MPD meshes sitting in front of the panel.
    The surround is a single formed face from MPD_L (y=-0.205) to
    MPD_R (y=0.205) at the TEDAC x. Inset wells recede so the TDU
    and both MPD screens stay visible from the eye as holes, not
    as standalone cyan planes. Dash cap remains z<=0.86 so
    eye_forward is not a housing well. Housing grid top is 0.848 so
    finish_mesh bevel cannot recreate the model21 fail (0.86 ring
    lifted over 0.86). model25 fail: TDU well window missed housing
    face centers at y=±0.05, so GEO_Dash had no emit-material faces
    at index 1 after finish_mesh bevel. model26 visual fail: the
    shallow 0.008 inset still read as flat cyan MPD planes behind a
    TEDAC bezel; deepen the wells so they read as recessed holes.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    ys = HOUSING_YS
    ...
    for face in list(bm.faces):
        ...
        if dash_tdu_well(center.y, center.z):
            tdu_faces.append(face)
        elif dash_mpd_well(center.y, center.z):
            mpd_faces.append(face)
    ...
    if well_faces:
        ...
        for face in mpd_faces:
            ...
            _face_toward_eye(face, 0.0)
    add_screen_markings(bm, TEDAC, (0.096, 0.090), 0.018, 0.0, 3)
...
def loft_canopy_pane(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    for x_value in xs:
        ...
        pane_inner = max(inner_y, pane_outer - pane_depth)
        ring = [
...
def loft_canopy_shell(collection, material, name: str, y_sign: float, xs) -> None:
    ...
    for col in range(len(columns) - 1):
        ...
        for row in range(4):
            nxt = (row + 1) % 4
            if row == 3 and window_punch[0] <= mid_x <= window_punch[1]:
                continue
            bm.faces.new(
                (
                    columns[col][row],
...
def loft_forward_windshield(collection, glass, rail) -> None:
    ...
    Dutch photo + TM Fig 2-8: look through the raked windshield to sky.
    GEO_Windshield is the forward pane of that hull, not a separate
    rectangular box. Stations are raked YZ trapezoids, not four copies
    of one rectangle. The pane fills roughly x 0.55-0.86, |y| <= 0.20,
    z 1.05-1.36. Do not emit GEO_WindshieldFrame; those faces formed
    the rectangular tunnel around the look-out. Not a grey rectangular well.
    model26 visual fail: a solidified windshield volume read as an
    opaque grey slab / opaque pane wall. Delete that closed box —
    a single-sided raked pane, not a grey wall. Do not solidify.
    """
...
def loft_overhead_brow(collection, material) -> None:
    ...
    model26 visual fail: GEO_ForwardBrow overlaid the windshield as an
...
def assert_not_sixty_mesh_kit(collection) -> None:
    """model27 must drop the 60-mesh cage family. Fail if it still emits 60 meshes."""
    meshes = [obj for obj in collection.all_objects if obj.type == "MESH"]
    if len(meshes) >= 60:
        raise WorkerError(
            f"model27 still emits 60 meshes of the same family "
    ...
    for forbidden in (
        ...
        if forbidden in names:
            raise WorkerError(f"model27 still emits {forbidden}.")
    if any(name.startswith("GEO_JointPlate") for name in names):
        raise WorkerError("model27 still emits GEO_JointPlate cage members.")
...
def build_asset(asset_collection) -> None:
    ...
    # not an opaque pane wall. model26 visual fail: alpha 0.08 plus a
