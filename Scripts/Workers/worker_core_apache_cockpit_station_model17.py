...
"""Governed AH-64 CPG station-model17 worker.

Keeps the model16 layout, sockets, look-out, inset canopy panes, glass
overhead brow, TEDAC green emit, MPD emit-face fix, spun knobs, HOCAS
heads, dark formed greenhouse rails, formed joint plates, dark interior,
non-white glass, and the model14-16 bucket seat. Plate wall owns
three-quarter: loft_canopy_bay coverage grows so the station reads as a
formed plate wall with punched window holes, not faceted glass planes
on a frame. Do not restore a draped canopy skin. Do not thicken
section_along members. Real bmesh only. Public layout only. Outputs go
only to --output.
"""
...
ASSET_ID = "core-apache-cockpit-station-model17"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark) -> None:
    """One formed housing wrapping TEDAC + left/right monitors into one unit.

    Chassis sits behind the TEDAC emit well so the green screen and public
    crosshair stay visible from eye_down_tedac. Left/right MPD screens are
    inset in this housing; they must not read as separate floating monitors.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()

    def _plate(x0, x1, y0, y1, z0, z1) -> None:
        corners = (
            (x0, y0, z0),
            (x1, y0, z0),
            (x1, y1, z0),
    ...
    # Back chassis BEHIND the emit well (emit ~x=0.522). Do not hide the green screen.
    _plate(0.548, 0.588, -0.312, 0.312, 0.796, 0.974)
    # Continuous top cap — above the TEDAC emit sightline from the eye.
    _plate(0.478, 0.548, -0.312, 0.312, 0.970, 0.994)
    # Continuous bottom cap under all three screens. One housing, not three boxes.
    _plate(0.478, 0.548, -0.312, 0.312, 0.790, 0.816)
    # Gap pillars between TEDAC and each MPD. One bezel unit, not floating monitors.
    _plate(0.486, 0.548, -0.128, -0.090, 0.816, 0.970)
    _plate(0.486, 0.548, 0.090, 0.128, 0.816, 0.970)
    # Outboard end caps wrapping the MPD outer edges into the same formed bezel.
    _plate(0.486, 0.548, -0.312, -0.278, 0.816, 0.970)
    _plate(0.486, 0.548, 0.278, 0.312, 0.816, 0.970)
    # Inset MPD well recesses in the same housing. Screens sit here, not as boxes.
    _plate(0.490, 0.548, -0.262, -0.148, 0.836, 0.934)
    _plate(0.490, 0.548, 0.148, 0.262, 0.836, 0.934)
    # Formed spine under the housing. Not a tan tube.
    _plate(0.500, 0.548, -0.036, 0.036, 0.724, 0.792)
...
def build_mpd(name: str, center, toe: float, collection, dark, emit) -> None:
    """Inset MPD screen in the TEDAC housing. Not a standalone floating box.

    GEO_MPD_L / GEO_MPD_R stay as inset emit screens in the same housing.
    Emit faces must pass assert_emit_faces_eye (alignment >= 0.55).
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    half_w = 0.056
    half_h = 0.048
    well_depth = 0.006
    # Front sits on the housing plane, then recesses into the dash.
    emit_along = 0.0
    cosine = math.cos(toe)
    sine = math.sin(toe)
    ...
    face.material_index = 1
    _face_toward_eye(face, toe)
    add_explicit_hood(bm, center, 0.124, 0.108, (0.112, 0.096), toe, 0.012, 0)
...
def loft_canopy_bay(collection, material, name: str, y_sign: float, xs) -> None:
    """Formed-plate bay so the plate wall owns three-quarter.

    Larger outer_overhang, skirt/header, end closers, and solidify make a
    formed plate wall with punched window holes. Glass is only the inset
    pane in those holes. Rails, sills, and A-pillars stay the same
    section size and read as thin trim, not the first silhouette. Front
    look-out stays open. Stays outboard of the look-out (|y| >= 0.20).
    No pipe sweep. No draped canopy skin.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    lookout_half = 0.20
    inner_y = max(0.24, lookout_half)
    # Plate wall sits further outboard of the 0.052 rail so the wall, not
    # the beam cage or glass sheet, is the three-quarter silhouette.
    # Members stay the same section size.
    outer_overhang = 0.072
    sill_underhang = 0.022
    rail_overhang = 0.028
    skirt_frac = 0.48
    header_frac = 0.42
    end_closer = 0.155
    ...
    def _ring(x_value, z0, z1, y_outer, y_inner):
        mid_z = z0 + (z1 - z0) * 0.5
        mid_y = max(y_outer, y_inner) + 0.028
        return [
    ...
    def _window_zs(sill_z, rail_z):
        span_z = max(0.04, rail_z - sill_z)
        win_sill_z = sill_z + span_z * skirt_frac
        win_rail_z = rail_z - span_z * header_frac
        return win_sill_z, win_rail_z
    ...
    for x_value in xs:
        sill_y, sill_z = _lerp_poly(sill_samples, x_value)
        rail_y, rail_z = _lerp_poly(rail_samples, x_value)
        win_sill_z, win_rail_z = _window_zs(sill_z, rail_z)
        outer_y = max(sill_y, rail_y) + outer_overhang
        lower.append(
            [
                bm.verts.new(point)
    ...
    # Wide end closers swallow A-pillar / side-bow so the plate is the body.
    closer_spans = (
        (xs[0], end_closer),
        (0.08, 0.0),
        (xs[-1], -end_closer),
    )
    for closer_x, dx in closer_spans:
        sill_y, sill_z = _lerp_poly(sill_samples, closer_x)
        rail_y, rail_z = _lerp_poly(rail_samples, closer_x)
        win_sill_z, win_rail_z = _window_zs(sill_z, rail_z)
        outer_y = max(sill_y, rail_y) + outer_overhang
        if dx > 0.0:
            x0, x1 = closer_x, closer_x + dx
        elif dx < 0.0:
            x0, x1 = closer_x + dx, closer_x
        else:
            x0, x1 = closer_x - mid_closer, closer_x + mid_closer
        columns = []
    ...
    bmesh.ops.solidify(bm, geom=list(bm.faces), thickness=y_sign * 0.032)
    object_from_bmesh(name, bm, collection, [material])
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    ...
    # the eye. No bow bar across the look-out. The plate wall owns
    # three-quarter: formed bay plates fill the bays so members read as
    # thin trim on punched window openings, not a beam cage or faceted
    # glass planes on a frame. Members stay formed rectangular sections,
    # not a tube sweep.
    sections = {
        "GEO_Rail_L": (
            [
    ...
    # Formed-plate bay panels fill the bays so the greenhouse shell owns
    # the silhouette. Rails read as thin trim. Inset glass panes sit in
    # the punched window openings only — do not drape a milky sheet.
    loft_canopy_bay(
        collection,
        plate,
        "GEO_CanopyBay_L",
