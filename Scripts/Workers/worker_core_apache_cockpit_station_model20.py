...
"""Governed AH-64 CPG station-model20 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model19 visual fail, not a
plate-knob pass. Raked windshield stations are different YZ
trapezoids (sill wider than brow, x and z coupled). Greenhouse
enclosure owns three-quarter as a formed canopy with punched framed
windows. One continuous formed forward panel with inset TDU and MPD
wells at the same x. TEDAC is not three stacked boxes. Square TDU
plus LHG/RHG stay. Glass may occupy
the forward look-out band; frames may not. Keep glass overhead brow,
dash z<=0.86, TEDAC green emit, MPD emit-face fix, and the
model14-19 bucket seat. Dark interior, dark rail, olive, and
...
ASSET_ID = "core-apache-cockpit-station-model20"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark) -> None:
    ...
    GEO_TEDAC emit and both MPD screens stay visible from the eye. Do
    not raise the dash through z<=0.86.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    ys = (
    ...
    for y_value in ys:
        ...
        for z_value in zs:
            ...
            x_value += edge * 0.010
            col.append(bm.verts.new((x_value, y_value, z_value)))
        cols.append(col)
...
def build_tedac(collection, dark, emit, grip, mark) -> None:
    """One square TDU from DCS Fig 44-45.
    ...
    SYM/BRT/CON, AZ, four action buttons). Public shapes only.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    formed_bezel(
    ...
    # Fig 45 top video buttons: TAD, FCR, PNV, G/S.
    for y_value, _label in (
        ...
        _tdu_bezel_button(bm, (0.488, y_value, 0.968), 0.016, 0.010, 0.006)
    # DAY-NT-OFF display knob, top-right of the TDU bezel.
    spin_bulb(bm, (0.490, 0.082, 0.968), (1.0, 0.0, 0.0), 0.008, 0.012, 0)
    # Left bezel: LEV then GAIN.
    spin_bulb(bm, (0.490, -0.086, 0.918), (1.0, 0.0, 0.0), 0.007, 0.010, 0)
    spin_bulb(bm, (0.490, -0.086, 0.892), (1.0, 0.0, 0.0), 0.007, 0.010, 0)
    # Right bezel: SYM, BRT, CON rockers.
    for z_value, _label in ((0.926, "SYM"), (0.906, "BRT"), (0.886, "CON")):
        _tdu_bezel_button(bm, (0.488, 0.086, z_value), 0.010, 0.008, 0.005)
    # Bottom: AZ rocker and four action buttons.
    _tdu_bezel_button(bm, (0.488, -0.062, 0.792), 0.022, 0.008, 0.005)
    for y_value in (-0.028, -0.008, 0.012, 0.032):
        _tdu_bezel_button(bm, (0.488, y_value, 0.792), 0.012, 0.010, 0.005)
...
def build_mpd(name: str, center, toe: float, collection, dark, emit) -> None:
    """Inset MPD well in the formed forward panel. Not a floating hooded monitor.

    GEO_MPD_L / GEO_MPD_R are recessed emit wells of GEO_Dash at the
    same x as the TDU. Not standalone boxes in front of the housing.
    The continuous panel owns the surround. Emit faces must pass
    assert_emit_faces_eye (alignment >= 0.55).
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    half_w = 0.056
...
def loft_canopy_fill(collection, material, name: str, y_sign: float, xs) -> None:
    """Formed enclosure plate with a punched window matching bay z.

    Three-quarter must read as one formed canopy with punched framed
    windows, not a beam cage with glass sheets hung on it. Not a draped
    canopy skin. Not model17 plate-knob overhang/solidify. The plate
    owns the bay silhouette; the window hole matches loft_canopy_bay.
    """
    bmesh = bmesh_module()
    bm = bmesh.new()
    lookout_half = 0.20
    ...
    for x_value in xs:
        ...
        win_sill = sill_z + span_z * BAY_SKIRT_FRAC
        win_rail = rail_z - span_z * BAY_HEADER_FRAC
        y_outer = max(sill_y, rail_y) + 0.014
        outer_cols.append(
            [
        ...
        inner_cols.append(
            [
                bm.verts.new((x_value, y_sign * (inner_y + hole_lip), win_sill)),
                bm.verts.new((x_value, y_sign * (y_outer - hole_lip), win_sill)),
                bm.verts.new((x_value, y_sign * (y_outer - hole_lip), win_rail)),
                bm.verts.new((x_value, y_sign * (inner_y + hole_lip), win_rail)),
            ]
        )
    _loft(outer_cols)
    _loft(inner_cols)
    ...
    bmesh.ops.solidify(bm, geom=list(bm.faces), thickness=y_sign * plate_depth)
    object_from_bmesh(name, bm, collection, [material])
...
def build_greenhouse(collection, rail, glass, plate) -> None:
    ...
    # enclosure owns three-quarter as one formed canopy with punched
    # framed windows, not a beam cage with glass sheets hung on it.
    # Rails read as a rail cap. Sills read as a sill plate. A-pillars
    # and side-bows read as a formed pillar. |y| >= 0.38. Aft frame is
    # behind the eye. No bow bar across the look-out. Members stay
    # formed rectangular sections, not a tube sweep. Do not thicken
    # section_along members. No tube sweep inside this greenhouse builder.
    sections = {
        "GEO_Rail_L": (
            [
                (-0.30, -0.418, 1.06),
    ...
    loft_canopy_fill(collection, plate, "GEO_CanopyPlate_L", -1.0, (-0.28, -0.10, 0.08, 0.28, 0.48, 0.64, 0.76))
    loft_canopy_fill(collection, plate, "GEO_CanopyPlate_R", 1.0, (-0.28, -0.10, 0.08, 0.28, 0.48, 0.64, 0.76))
    loft_canopy_pane(
        collection,
        glass,
        "GEO_CanopyPane_L",
