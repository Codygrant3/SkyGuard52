...
"""Governed AH-64 CPG station-model02 worker.

Fixes model01 visual blockers: outboard-only greenhouse (no bow bar) and
explicit TEDAC/MPD hoods plus eye-facing emit wells. Public layout only.
...
ASSET_ID = "core-apache-cockpit-station-model02"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def build_greenhouse(collection, rail) -> None:
    bmesh = bmesh_module()
    # Left/right rails and sills only, far outboard (|y| >= 0.38). Aft frame is
    # behind the eye. No bow bar and no center transom in the forward cone.
    paths = {
        "GEO_Rail_L": [(-0.30, -0.41, 1.06), (0.08, -0.435, 1.30), (0.42, -0.42, 1.22), (0.76, -0.41, 1.07)],
        "GEO_Rail_R": [(-0.30, 0.41, 1.06), (0.08, 0.435, 1.30), (0.42, 0.42, 1.22), (0.76, 0.41, 1.07)],
        "GEO_Sill_L": [(-0.22, -0.405, 0.955), (0.28, -0.41, 0.948), (0.70, -0.405, 0.940)],
        "GEO_Sill_R": [(-0.22, 0.405, 0.955), (0.28, 0.41, 0.948), (0.70, 0.405, 0.940)],
...
def assert_greenhouse_outboard(collection) -> None:
    for obj in collection.all_objects:
        if obj.type != "MESH":
            continue
        if not (obj.name.startswith("GEO_Rail_") or obj.name.startswith("GEO_Sill_")):
            continue
        for vert in obj.data.vertices:
            world = obj.matrix_world @ vert.co
            if abs(world.y) < 0.38:
