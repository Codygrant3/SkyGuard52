...
"""Governed AH-64 CPG station-model21 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model20 visual fail, not a
plate-knob pass. Not another fill plate on the same cage. Greenhouse
...
shell. eye_forward looks through raked windshield / sky in the upper
two-thirds; TEDAC and dash occupy the lower third, not a housing
well. One continuous formed forward panel with inset TDU and MPD
wells at the same x. Square TDU plus LHG/RHG stay as hardware on
the dash, not a 0.188 cube. TEDAC is not three stacked boxes.
Raked windshield stations stay different
YZ trapezoids. Glass may occupy the forward look-out band; frames
may not. Keep glass overhead brow, dash z<=0.86, TEDAC green emit,
MPD emit-face fix, and the model14-20 bucket seat. Dark interior,
...
ASSET_ID = "core-apache-cockpit-station-model21"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark) -> None:
    ...
    zs = (0.720, 0.752, 0.784, 0.816, 0.840, 0.860)
    ...
    for y_value in ys:
        ...
        for z_value in zs:
            ...
            x_value += edge * 0.010
            col.append(bm.verts.new((x_value, y_value, min(z_value, 0.86))))
        cols.append(col)
    ...
    for vert in bm.verts:
        if abs(vert.co.y) < 0.20 and vert.co.z > 0.86:
            vert.co.z = 0.86
