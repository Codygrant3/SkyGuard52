...
"""Governed AH-64 CPG station-model03 worker.

Keeps the model02 layout, sockets, look-out, hoods, and emit wells.
Forms the seat, canopy frame, knobs, dash eyebrow, and HOCAS heads with
real bmesh (extrude, inset, solidify, spin, pipe, grid deform). Public
layout only. Outputs go only to --output.
"""
...
ASSET_ID = "core-apache-cockpit-station-model03"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def build_dash_shelf(collection, olive, dark) -> None:
    """Profiled CPG eyebrow shelf plus knee panels. Not a lofted slab."""
    bmesh = bmesh_module()
    matrix, _vector = mathutils_types()
    bm = bmesh.new()
    bmesh.ops.create_grid(
    ...
    for vert in bm.verts:
        ...
        z_value = 0.56 + 0.40 * max(0.18, peak)
        if abs(y_value) > 0.16:
            z_value += min(0.055, (abs(y_value) - 0.16) * 0.22)
        if abs(y_value) < 0.20:
            z_value = min(z_value, 1.03)
        if along < 0.18:
...
def build_greenhouse(collection, rail) -> None:
    bmesh = bmesh_module()
    # Continuous outboard rails and sills plus short A-pillars / side bows.
