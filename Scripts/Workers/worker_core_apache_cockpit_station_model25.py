...
"""Governed AH-64 CPG station-model25 worker.

Built from the Dutch MoD AH-64D CPG reference plate, DCS Fig 43-45,
TM-1-1520-238-10 Fig 2-8 / 2-14, and the model24 visual fail, not a
...
emit-face fix, and the model14-24 bucket seat. GEO_Dash look-out
...
ASSET_ID = "core-apache-cockpit-station-model25"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def formed_tedac_housing(collection, dark, tedac_emit, mpd_emit, tedac_mark) -> None:
    ...
    def _dash_tdu_well(y_value: float, z_value: float) -> bool:
        return abs(y_value) < 0.048 and z_value >= 0.800

    def _dash_mpd_well(y_value: float, z_value: float) -> bool:
    ...
    for face in list(bm.faces):
        ...
        if _dash_tdu_well(center.y, center.z):
            tdu_faces.append(face)
        elif _dash_mpd_well(center.y, center.z):
...
def assert_not_sixty_mesh_kit(collection) -> None:
    """model25 must drop the 60-mesh cage family. Fail if it still emits 60 meshes."""
    meshes = [obj for obj in collection.all_objects if obj.type == "MESH"]
    if len(meshes) >= 60:
        raise WorkerError(
            f"model25 still emits 60 meshes of the same family "
    ...
    for forbidden in (
        ...
        if forbidden in names:
            raise WorkerError(f"model25 still emits {forbidden}.")
    if any(name.startswith("GEO_JointPlate") for name in names):
        raise WorkerError("model25 still emits GEO_JointPlate cage members.")
