...
"""Governed AH-64 CPG station-model08 worker.

Keeps the model07 layout, sockets, look-out, side canopy skins, hoods,
emit wells, TEDAC readability, spun knobs, and HOCAS heads. The lofted
overhead brow stays at crown z >= 1.36 but uses canopy glass so
eye_forward is an open look-out, not a solid wall. Deeper bucket seat
with taller bolster inner walls and a darker pan well. Real bmesh
...
ASSET_ID = "core-apache-cockpit-station-model08"
EYE = (0.0, 0.0, 1.18)
TEDAC = (0.50, 0.0, 0.88)
...
def build_seat(collection, material, well) -> None:
    """Bucket seat: deeper dish and taller bolster walls from three-quarter, not a flat cushion."""
    bmesh = bmesh_module()
    ...
    for name, y_sign in (("GEO_SeatBolster_L", -1.0), ("GEO_SeatBolster_R", 1.0)):
        ...
        y1 = y_sign * 0.238
        verts = [
            bm.verts.new((-0.18, y0, 1.055)),
            bm.verts.new((0.16, y0, 1.018)),
