...
def evaluate_source(
    ...
    checks["supersedes_rejected_candidates"] = (
        ...
        and (
            ...
            or (
                ...
                and (
                    revision not in {"005", "006", "007", "008"}
    ...
    if revision == "008":
        checks["supersedes_007_visual_failure"] = (
            all(
                f"BLD_M01_HERO_GROUPED_TOPOLOGY_{item}"
                in contract["supersedes_candidates"]
                for item in ("003", "004", "005", "006", "007")
            )
            and contract.get("attempt_correction_basis")
            == "Docs/AAA_Review/M01_HERO_GROUPED_TOPOLOGY_BAKE_007_VISUAL_REVIEW_2026-08-02.md"
        )
        basis_path = resolve(root, contract["attempt_correction_basis"])
        checks["attempt_correction_basis_hash_bound"] = (
            basis_path.is_file()
            and contract.get("attempt_correction_basis_sha256")
    ...
    if revision == "008":
        classification_raw = contract.get("visual_failure_classification")
        classification_path = (
            resolve(root, classification_raw)
            if isinstance(classification_raw, str)
            else None
        )
        checks["visual_failure_classification_hash_bound"] = bool(
            classification_path
            and classification_path.is_file()
            and contract.get("visual_failure_classification_sha256")
            == sha256_file(classification_path)
        )
        try:
            classification = json.loads(
            ...
            classification = {}
        checks["visual_failure_classification_scope"] = (
            classification.get("source_build_id")
            == "BLD_M01_HERO_GROUPED_TOPOLOGY_007"
            and classification.get("target_build_id")
            == "BLD_M01_HERO_GROUPED_TOPOLOGY_008"
            ...
            and classification.get("failed_map_count") == 6
            and classification.get("reused_accepted_map_count") == 18
            and classification.get("gate") == "PASS"
        )
        corrective = contract.get("corrective_map_contract", {})
        source_manifest = resolve(root, corrective.get("source_manifest", ""))
        source_receipt = resolve(
            root,
            corrective.get("source_review_receipt", ""),
        )
        checks["corrective_map_contract"] = (
            corrective.get("analysis_mode")
            == "offline_json_and_hash_only_no_blender_no_unreal"
            and corrective.get("source_build_id")
            == "BLD_M01_HERO_GROUPED_TOPOLOGY_007"
            and corrective.get("rebake_map_count") == 6
            and corrective.get("reuse_map_count") == 18
            and len(set(corrective.get("rebake_targets", []))) == 6
            and corrective.get("production_geometry_translation_allowed")
            is False
            and corrective.get("accepted_map_copy_requires_hash_identity")
            is True
            and corrective.get("accepted_map_rebake_allowed") is False
            and source_manifest.is_file()
            and corrective.get("source_manifest_sha256")
            == sha256_file(source_manifest)
            and source_receipt.is_file()
            and corrective.get("source_review_receipt_sha256")
    ...
    expected_uv_method = (
        ...
        if revision in {"005", "006", "007", "008"}
    ...
    if revision in {"007", "008"}:
        ...
        checks["topology_repair_contract"] = (
            ...
            and len(policies) == 12
            and len(direct) == repair.get("direct_low_ao_count")
            and len(dedicated)
    ...
    required_tokens = {
        ...
        "explicit_cage": "bake.cage_object = bake_cage",
    ...
    if revision in {"005", "006", "007", "008"}:
        ...
        if revision == "008":
            required_tokens.update(
                {
                    "component_descriptors": "def component_descriptors(",
                    "component_matching": "def match_components(",
                    "component_explosion": "def make_component_exploded_bake_set(",
                    "production_mesh_preserved": '"production_geometry_translated": False',
                    "hash_verified_reuse": "def reuse_accepted_map(",
                    "six_map_partition": '"rebake_targets"',
                }
            )
    else:
...
def evaluate_artifacts(
    ...
    if build_id == "BLD_M01_HERO_GROUPED_TOPOLOGY_008":
        classification_path = resolve(
            root,
            contract["visual_failure_classification"],
        )
        checks["visual_failure_classification_integrity"] = (
            verify_file_evidence(
                manifest.get("visual_failure_classification"),
                root,
                "visual_failure_classification",
                errors,
            )
            and Path(
                manifest["visual_failure_classification"]["path"]
            ).resolve()
            == classification_path.resolve()
        )
        corrective = contract["corrective_map_contract"]
        source_manifest_path = resolve(
            root,
            corrective["source_manifest"],
        )
        source_receipt_path = resolve(
            root,
            corrective["source_review_receipt"],
        )
        checks["source_build_manifest_integrity"] = (
            verify_file_evidence(
                manifest.get("source_build_manifest"),
                root,
                "source_build_manifest",
                errors,
            )
            and Path(manifest["source_build_manifest"]["path"]).resolve()
            == source_manifest_path.resolve()
        )
        checks["source_direct_review_receipt_integrity"] = (
            verify_file_evidence(
                manifest.get("source_direct_review_receipt"),
                root,
                "source_direct_review_receipt",
                errors,
            )
            and Path(
                manifest["source_direct_review_receipt"]["path"]
            ).resolve()
            == source_receipt_path.resolve()
        )
        checks["corrective_map_contract_matches"] = (
            manifest.get("corrective_map_contract")
            == contract.get("corrective_map_contract")
        )
    ...
    for asset_id in sorted(REQUIRED_ASSETS):
        ...
        for group_id, group_spec in expected_groups.items():
            ...
            for map_spec in contract["bake_contract"]["maps"]:
                ...
                map_key = f"{asset_id}/{group_id}/{map_type}"
                ...
                provenance = map_item.get("provenance", {})
                ...
                if build_id == "BLD_M01_HERO_GROUPED_TOPOLOGY_008":
                    corrective = contract["corrective_map_contract"]
                    rebake_targets = set(corrective["rebake_targets"])
                    policy = (
                        contract.get("visual_failure_classification_data", {})
                        .get("group_policies", {})
                        .get(f"{asset_id}/{group_id}", {})
                    )
                    if not policy:
                        classification = json.loads(
                            resolve(
                        ...
                        policy = classification.get(
                            "group_policies",
                            {},
                        ).get(f"{asset_id}/{group_id}", {})
                    if provenance.get("mode") == "hash_verified_reuse":
                        reused_map_count += 1
                        source_path = Path(
                            str(provenance.get("source_path", ""))
                        )
                        provenance_valid = (
                            map_key not in rebake_targets
                            and provenance.get("source_build_id")
                            == "BLD_M01_HERO_GROUPED_TOPOLOGY_007"
                            and provenance.get("source_review_result") == "PASS"
                            and provenance.get("rebaked") is False
                        ...
                        expected_mode = projection.get("mode")
                    elif provenance.get("mode") == "corrective_rebake":
                        rebaked_map_count += 1
                        expected_mode = (
                            policy.get("ao_projection_policy")
                            if map_type == "AO"
                            else policy.get("normal_projection_policy")
                        )
                        provenance_valid = (
                            map_key in rebake_targets
                            and isinstance(expected_mode, str)
                            and provenance.get("source_build_id")
                            == "BLD_M01_HERO_GROUPED_TOPOLOGY_007"
                            and provenance.get("source_review_result") == "FAIL"
                            and provenance.get("rebaked") is True
                            and projection.get("component_isolated") is True
                            and isinstance(
                                projection.get("component_count"),
                                int,
                            )
                            and projection["component_count"] >= 1
                            and projection.get(
                                "component_spacing_multiplier"
                            )
                            == policy.get("component_spacing_multiplier")
                            and projection.get(
                                "production_geometry_translated"
                            )
                            is False
                        )
                        expected_projection = {
    ...
    if build_id == "BLD_M01_HERO_GROUPED_TOPOLOGY_008":
        corrective = contract["corrective_map_contract"]
        checks["corrective_map_partition"] = (
            rebaked_map_count == corrective["rebake_map_count"]
            and reused_map_count == corrective["reuse_map_count"]
            and manifest.get("rebaked_map_count") == rebaked_map_count
            and manifest.get("reused_map_count") == reused_map_count
            and rebaked_map_count + reused_map_count == 24
        )
