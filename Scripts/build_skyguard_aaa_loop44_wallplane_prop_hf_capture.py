...
PREFIX = "AAA_L44_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L44"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L44"
...
def densify():
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # WALL-PLANE densify for Prop family only. Maximize color uniqueness + edges.
        # No mid-FOV objects (nothing with x < bx+1 beyond baseline stripes/wall).
        if name in ("Prop", "PropHub", "PropNose"):
            denser = 2.2 if name in ("Prop", "PropNose") else 2.8
            yspan = 18 if name in ("Prop", "PropNose") else 14
            zspan = 14 if name in ("Prop", "PropNose") else 11
            for iy in range(-yspan, yspan + 1):
                for iz in range(-zspan, zspan + 1):
                    # high-frequency color parity for uniq score
                    mm = mats[(iy * 17 + iz * 13 + i * 7 + ((iy*iz) % 5)) % len(mats)] if mats else m
                    # alternate sizes for edge energy
                    if (iy + iz) % 3 == 0:
                        scale = (0.18, 0.40, 0.40)
                    elif (iy + iz) % 3 == 1:
                        scale = (0.30, 0.70, 0.70)
                    else:
                        scale = (0.24, 0.55, 0.55)
                    spawn_sm(cube, (bx + 2.0, cy + iy * denser, cz + iz * denser), scale, None, PREFIX + "WPlane_%s_%d_%d" % (name, iy, iz), mm)
            # dense multicolor vertical + horizontal wall stripes
            for iy in range(-22, 23):
                mm = mats[(iy * 3 + i) % len(mats)] if mats else m
                spawn_sm(cube, (bx + 2.0, cy + iy * 1.7, cz), (0.14, 0.18, 11.0), None, PREFIX + "WStripe_%s_%d" % (name, iy), mm)
            for iz in range(-18, 19):
                mm = mats[(iz * 5 + i * 2) % len(mats)] if mats else m
                spawn_sm(cube, (bx + 2.0, cy, cz + iz * 1.7), (0.14, 11.0, 0.18), None, PREFIX + "HStripe_%s_%d" % (name, iz), mm)
            # small hub + ring of cubes ON wall plane only
            spawn_sm(sphere, (bx + 2.0, cy, cz), (1.4, 1.4, 1.4), None, PREFIX + "WHub_%s" % name, mats[i % len(mats)] if mats else m)
            for k in range(12):
                ang = k * 0.52359877559
                ry = math.sin(ang) * 10.0
                rz = math.cos(ang) * 10.0
                mm = mats[(k * 2 + i) % len(mats)] if mats else m
                spawn_sm(cube, (bx + 2.0, cy + ry, cz + rz), (0.2, 0.9, 0.35), unreal.Rotator(0, 0, k * 15.0), PREFIX + "WRing_%s_%d" % (name, k), mm)
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop44 wall-plane HF densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L44", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L44_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop44 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l38_freeze_wall_plane_prop_hf_color\n")
...
def main():
    log("loop44 L38 freeze + high-frequency wall-plane Prop densify start")
    ...
    log("Loop44 complete stills=%d" % (len(saved) if saved else 0))
