...
PREFIX = "AAA_L46_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L46"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L46"
...
def densify():
    ...
    log("loop46 mat palette size=%d" % len(mats))
    nmat = max(len(mats), 1)
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # ---- HERO content ON/BEHIND wall plane only (x >= bx+1). No mid-FOV. ----
        # Materials: prefer authored airframe/prop/rifle/ocean over pure unlit boards.
        m_air = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Airframe") or load_mat("/Game/Skyguard/Materials/Generated/M_AirframeMetal") or load_mat("/Game/Skyguard/Materials/M_Metal")
        m_prop = load_mat("/Game/Skyguard/Materials/M_PropDisc") or load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightMetal") or m_air
        m_rifle = load_mat("/Game/Skyguard/Materials/M_RifleTan") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Boom") or m_air
        m_leather = load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Leather") or m_rifle
        m_panel = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Panel") or load_mat("/Game/Skyguard/Materials/M_CockpitInterior") or m_air
        m_ocean = load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightOcean") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Ocean") or load_mat("/Game/Skyguard/Materials/M_Ocean")
        m_sand = load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightSand") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Beach") or load_mat("/Game/Skyguard/Materials/M_Beach")
        m_brick = load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightBrick") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Brick")
        m_muzzle = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Muzzle") or load_mat("/Game/Skyguard/Materials/M_ExhaustGlow")
        m_foam = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Foam") or load_mat("/Game/Skyguard/Materials/M_L5_SeaFoam") or m_ocean
        m_glass = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Glass") or load_mat("/Game/Skyguard/Materials/M_CockpitGlass") or load_mat("/Game/Skyguard/Materials/M_CityGlass")
        ...
        if name in ("Prop", "PropHub", "PropNose"):
            # Hero propeller + motion disc shells ON wall (capture-safe)
            for k in range(6):
                ang = k * 30.0
                spawn_sm(sm_prop or cube, (bx + 2.2, cy, cz), (2.2, 2.2, 2.2), unreal.Rotator(0, 0, ang + i * 7), PREFIX + "HeroProp_%s_%d" % (name, k), m_prop or mats[k % nmat])
            # motion disc planes via thin scaled cubes with PropDisc mat
            for k in range(4):
                spawn_sm(cube, (bx + 2.4, cy, cz), (0.08, 8.0 + k, 8.0 + k), unreal.Rotator(0, 0, k * 18), PREFIX + "PropDisc_%s_%d" % (name, k), m_prop or mats[(k+3) % nmat])
            # spinner hub + dirt panels
            spawn_sm(sphere, (bx + 2.1, cy, cz), (1.8, 1.8, 1.8), None, PREFIX + "PropHubHero_%s" % name, m_air or mats[i % nmat])
            for k in range(8):
                ang = k * 0.78539816339
                ry = math.sin(ang) * 7.0
                rz = math.cos(ang) * 7.0
                spawn_sm(cube, (bx + 2.3, cy + ry, cz + rz), (0.2, 2.2, 0.35), unreal.Rotator(0, 0, k * 22.5), PREFIX + "BladeProxy_%s_%d" % (name, k), m_air or mats[(k+i) % nmat])

        if name == "YakBeauty":
            spawn_sm(sm_yak or cube, (bx + 2.0, cy, cz - 2.0), (8.0, 8.0, 8.0), unreal.Rotator(0, 90, 0), PREFIX + "HeroYak", m_air or mats[0])
            spawn_sm(sm_prop or cube, (bx + 2.2, cy + 18.0, cz), (3.0, 3.0, 3.0), unreal.Rotator(0, 90, 20), PREFIX + "HeroYakProp", m_prop or mats[1])
            # weathering / panel breakup on wall around airframe
            for iy in range(-6, 7):
                for iz in range(-4, 5):
                    mm = m_panel if ((iy + iz) % 2 == 0) else (m_air or mats[(iy+iz) % nmat])
                    spawn_sm(cube, (bx + 2.5, cy + iy * 4.0, cz + iz * 4.0), (0.2, 0.9, 0.9), None, PREFIX + "YakPanel_%d_%d" % (iy, iz), mm)

        if name == "Cockpit":
            spawn_sm(sm_cock or cube, (bx + 2.0, cy, cz - 1.0), (3.5, 3.5, 3.5), unreal.Rotator(0, 180, 0), PREFIX + "HeroCockpit", m_panel or mats[0])
            spawn_sm(sm_station or cube, (bx + 2.2, cy + 2.0, cz), (2.5, 2.5, 2.5), None, PREFIX + "HeroStation", m_panel or mats[1])
            spawn_sm(sm_instr or cube, (bx + 2.1, cy - 1.5, cz + 1.0), (1.8, 1.8, 1.8), None, PREFIX + "HeroInstr", m_panel or mats[2])
            spawn_sm(sm_glove or cube, (bx + 2.0, cy + 4.0, cz - 0.5), (1.6, 1.6, 1.6), unreal.Rotator(0, 20, 0), PREFIX + "HeroGlove", m_leather or mats[3])
            spawn_sm(sm_arm or cube, (bx + 2.0, cy + 5.5, cz - 1.0), (1.4, 1.4, 1.4), unreal.Rotator(0, 15, -10), PREFIX + "HeroArm", m_leather or mats[4])
            # canopy glass strip on wall
            for k in range(6):
                spawn_sm(cube, (bx + 1.8, cy - 4 + k * 1.5, cz + 3.0), (0.12, 1.2, 0.15), None, PREFIX + "CanopyGlass_%d" % k, m_glass or mats[k % nmat])

        if name == "ADS":
            spawn_sm(sm_rifle or cube, (bx + 2.0, cy, cz), (3.0, 3.0, 3.0), unreal.Rotator(0, 90, 0), PREFIX + "HeroRifle", m_rifle or mats[0])
            spawn_sm(sm_glove or cube, (bx + 2.1, cy - 1.2, cz - 0.6), (1.4, 1.4, 1.4), unreal.Rotator(0, 90, 0), PREFIX + "HeroADSGlove", m_leather or mats[1])
            spawn_sm(sm_igla or cube, (bx + 2.3, cy + 3.5, cz + 1.0), (2.2, 2.2, 2.2), unreal.Rotator(0, 80, 10), PREFIX + "HeroIgla", m_air or mats[2])
            # muzzle / VFX proxy bursts on wall (emissive-ish mats)
            for k in range(5):
                spawn_sm(sphere, (bx + 2.4, cy + 2.0 + k * 0.4, cz + 0.5), (0.35 + k * 0.08, 0.35 + k * 0.08, 0.35 + k * 0.08), None, PREFIX + "MuzzleProxy_%d" % k, m_muzzle or mats[(k+5) % nmat])

        if name == "Combat":
            for k in range(5):
                sm = sm_drone_h if k % 2 else sm_drone
                spawn_sm(sm or cube, (bx + 2.0, cy - 12 + k * 6.0, cz + (k % 3) * 3.0), (2.5, 2.5, 2.5), unreal.Rotator(0, k * 15, 0), PREFIX + "HeroDrone_%d" % k, m_air or mats[k % nmat])
            for k in range(6):
                spawn_sm(sphere, (bx + 2.5, cy - 8 + k * 3.0, cz + 2.0), (0.6, 0.6, 0.6), None, PREFIX + "ExplProxy_%d" % k, m_muzzle or mats[(k+2) % nmat])
                spawn_sm(cube, (bx + 2.6, cy - 8 + k * 3.0, cz + 4.0), (0.2, 1.8, 0.2), unreal.Rotator(0, 0, k * 25), PREFIX + "FlakProxy_%d" % k, m_muzzle or mats[(k+4) % nmat])

        if name == "City":
            for k in range(4):
                spawn_sm(sm_tower or cube, (bx + 2.0, cy - 20 + k * 12.0, cz - 4.0), (4.0, 4.0, 6.0 + k), None, PREFIX + "HeroTower_%d" % k, m_brick or mats[k % nmat])
            for k in range(3):
                spawn_sm(sm_apt or cube, (bx + 2.2, cy - 10 + k * 14.0, cz + 2.0), (5.0, 5.0, 5.0), None, PREFIX + "HeroApt_%d" % k, m_panel or mats[(k+3) % nmat])

        if name == "Harbor":
            spawn_sm(sm_crane or cube, (bx + 2.0, cy + 8.0, cz), (5.0, 5.0, 5.0), None, PREFIX + "HeroCrane", m_air or mats[0])
            spawn_sm(sm_ship or cube, (bx + 2.2, cy - 10.0, cz - 2.0), (6.0, 6.0, 3.0), None, PREFIX + "HeroShip", m_air or mats[1])
            spawn_sm(sm_pier or cube, (bx + 2.0, cy, cz - 6.0), (8.0, 2.0, 1.0), None, PREFIX + "HeroPier", m_sand or mats[2])
            spawn_sm(sm_seawall or cube, (bx + 2.1, cy + 16.0, cz - 5.0), (7.0, 1.5, 2.0), None, PREFIX + "HeroSeawall", m_panel or mats[3])

        if name == "Ocean":
            # Fix too-black Ocean: bright foam/ocean checker wall densify + hero water proxies
            nmat = max(len(mats), 1)
            for iy in range(-14, 15):
                for iz in range(-10, 11):
                    mm = m_ocean if ((iy + iz) % 2 == 0) else (m_foam or mats[(iy + iz) % nmat])
                    if mm is None:
                        mm = mats[(iy + iz) % nmat]
                    spawn_sm(cube, (bx + 2.0, cy + iy * 3.0, cz + iz * 3.0), (0.25, 0.7, 0.7), None, PREFIX + "OceanWall_%d_%d" % (iy, iz), mm)
            for iy in range(-16, 17):
                spawn_sm(cube, (bx + 1.5, cy + iy * 2.4, cz), (0.18, 0.25, 10.0), None, PREFIX + "OceanStripe_%d" % iy, m_foam or mats[iy % nmat])
            spawn_sm(sm_sub or cube, (bx + 2.2, cy - 6.0, cz - 2.0), (4.0, 4.0, 2.0), None, PREFIX + "HeroSub", m_air or mats[0])
            spawn_sm(sm_coast or cube, (bx + 2.0, cy + 10.0, cz - 4.0), (6.0, 3.0, 2.0), None, PREFIX + "HeroCoast", m_sand or mats[1])
            # bright point-like spheres for white-cap energy
            for k in range(10):
                spawn_sm(sphere, (bx + 2.3, cy - 12 + k * 2.5, cz + (k % 4) * 2.0), (0.8, 0.8, 0.8), None, PREFIX + "FoamCap_%d" % k, m_foam or unlit_w or mats[k % nmat])

        if name == "Wide":
            spawn_sm(sm_yak or cube, (bx + 2.0, cy - 8.0, cz), (5.0, 5.0, 5.0), unreal.Rotator(0, 70, 0), PREFIX + "WideYak", m_air or mats[0])
            spawn_sm(sm_tower or cube, (bx + 2.0, cy + 12.0, cz - 2.0), (4.0, 4.0, 7.0), None, PREFIX + "WideTower", m_brick or mats[1])
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        cx,cy,cz = cam
        bx = cx + dist
        intensity = 700000.0 if name == "Ocean" else 450000.0
        ...
        if pl:
            ...
            try:
                ...
                if c:
                    ...
                    c.set_editor_property("attenuation_radius", 7000.0 if name == "Ocean" else 6000.0)
            except Exception:
                pass
        if name == "Ocean":
            # second fill light to reduce pure-black Ocean captures
            pl2 = unreal.EditorLevelLibrary.spawn_actor_from_class(unreal.PointLight, unreal.Vector(bx + 2.0, cy, cz + 40), unreal.Rotator())
            if pl2:
                pl2.set_actor_label(PREFIX + "Pt_OceanFill")
                try:
                    pl2.set_actor_location(unreal.Vector(bx + 2.0, cy, cz + 40), False, True)
                except Exception:
                    pass
                try:
                    c = pl2.get_component_by_class(unreal.PointLightComponent)
                    if c:
                        c.set_intensity(900000.0)
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L46", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L46_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop46 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l45_freeze_hero_pbr_vfx_ocean_fix\n")
...
def main():
    log("loop46 L45 freeze + hero prop/airframe/ADS/ocean mats + VFX proxies start")
    ...
    log("Loop46 complete stills=%d" % (len(saved) if saved else 0))
