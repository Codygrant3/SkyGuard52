...
param(
    ...
    [switch]$AuthorizeSingleRecovery09Run,
    [Parameter(Mandatory = $true)]
...
if (-not $AuthorizeSingleRecovery09Run) {
    throw "Explicit -AuthorizeSingleRecovery09Run is required."
}

$ContractPath = Join-Path $ProjectRoot (
    "Docs\AAA_Review\" +
    "M01_HERO_GROUPED_TOPOLOGY_UNREAL_MAPPED_ATTEMPT03_" +
    "RECOVERY09_EXECUTION_CONTRACT.json"
)
...
foreach ($record in $execution.bound_files.PSObject.Properties.Value) {
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or
        ...
        throw "Recovery09 bound-file mismatch: $path"
    }
}
...
if (Test-Path -LiteralPath $AttemptRoot) {
    throw "Immutable Recovery09 output exists: $AttemptRoot"
}
$heavy = @(
...
if ($heavy.Count -ne 0) {
    throw "Recovery09 requires zero heavy processes."
}
...
if ($freeGiB -lt [double]$execution.resource_preflight.minimum_free_physical_gib) {
    throw "Recovery09 has only $freeGiB GiB free RAM."
}

New-Item -ItemType Directory -Path $AttemptRoot | Out-Null
$Preflight = Join-Path $AttemptRoot "preflight_snapshot.json"
[ordered]@{
    schema = "skyguard.m01.hero-grouped-topology-recovery09-preflight.v1"
...
try {
    $unrealProcess = Invoke-Bounded -FilePath $EditorExe -Arguments @(
        ...
        "-game",
        (
            "-SkyguardM01Recovery09ContractId=" +
            $execution.native_runtime.required_contract_id
        ),
        "-SkyguardM01Recovery09ExpectedMap=$($execution.review_map)",
        "-SkyguardM01Recovery09Output=$CaptureRoot",
    ...
    if ($unrealProcess.timed_out -or $unrealProcess.exit_code -ne 0 -or
        ...
        throw "Recovery09 high-resolution native capture failed closed."
    }
    ...
    if ($capture.gate -ne
            "PASS_RECOVERY09_HIGHRES_CAPTURE_AWAITING_OFFLINE_AUDIT" -or
        ...
        $combined -notmatch "\[RECOVERY09\]\[STATE\]" -or
        $combined -notmatch "\[RECOVERY09\]\[CAPTURE_COMPLETE\]") {
        throw "Recovery09 receipt or diagnostic acceptance failed."
    }
    $remaining = @(
        Get-Process -ErrorAction SilentlyContinue |
            Where-Object { $HeavyNames -contains $_.ProcessName }
    ...
    if ($remaining.Count -ne 0) {
        throw "Heavy process remained after Recovery09."
    }
    $receipt = [ordered]@{
        schema = (
            "skyguard.m01.hero-grouped-topology-" +
            "recovery09-supervisor.v1"
        )
        gate = (
            "PASS_RECOVERY09_EXECUTION_" +
    ...
    [ordered]@{
        ...
        gate = "FAIL_CLOSED_RECOVERY09_NOT_ACCEPTED"
