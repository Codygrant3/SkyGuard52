...
PREFIX = "AAA_L45_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L45"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L45"
...
def densify():
    ...
    log("loop45 mat palette size=%d" % len(mats))
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop45 multi-mat wall-plane densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L45", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L45_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop45 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l38_freeze_wall_plane_multimat_prop\n")
...
def main():
    log("loop45 L38 freeze + multi-mat wall-plane Prop densify for uniq>=80 start")
    ...
    log("Loop45 complete stills=%d" % (len(saved) if saved else 0))
