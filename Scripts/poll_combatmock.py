...
log=Path(r'D:\Skyguard52\Saved\Logs\skyguard-aaa-combatmock.log')
pid=int(Path(r'D:\Skyguard52\Saved\Logs\aaa-combatmock.pid').read_text().strip())
for i in range(20):
...
print('ndp', subprocess.run(['powershell','-NoProfile','-Command',"Get-Process NDP481* -EA 0 | Select Id,CPU | Out-String"],capture_output=True,text=True).stdout)
