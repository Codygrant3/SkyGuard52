...
PREFIX = "AAA_L76_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L76"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L76"
...
def densify():
    ...
    log("loop76 mat palette size=%d" % len(mats))
    ...
    log("loop76 true-art slice19 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L76", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L76_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop76 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l75_freeze_true_art_slice19_niagara_material_push")
...
def main():
    log("loop76 L75 freeze + true-art slice19 niagara + material push start")
    ...
    log("Loop76 complete stills=%d" % (len(saved) if saved else 0))
