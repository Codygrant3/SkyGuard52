"""Build Mission 01 hero high/low maps as isolated semantic bake groups.

Builds 001 and 002 joined every shell in an asset into one projection target.
Their direct 2048 px reviews exposed cross-shell rays, colored normal
excursions, and hard AO voids.  This immutable 003 build instead:

* partitions every source face exactly once by governed material semantics;
* creates an independent production-low mesh per semantic group;
* replaces inherited smart-projected UVs with explicit per-face authored
  islands, angle-based unwrap, scale equalization, and deterministic packing;
* authors angle-governed smoothing and sharp edges;
* creates a separate beveled high source and topology-matching normal-offset
  cage for every group; and
* bakes Normal/AO in complete group isolation while retaining neutral
  backgrounds.

Artifact checks remain candidate-only.  Direct original-resolution review is
required before any mapped-mesh or Unreal promotion.
"""
...
def bake_group_map(
    ...
    if map_type == "AO":
        if ao_policy == "direct_low_self_occlusion":
            ...
            isolate_render_meshes([ao_occluder, low, cage])
            select_only([ao_occluder, low], low)
    else:
        bake.use_selected_to_active = True
        bake.use_cage = True
        bake.cage_object = cage
        bake.cage_extrusion = float(group_spec["cage_extrusion_m"])
        bake.max_ray_distance = float(group_spec["max_ray_distance_m"])
        isolate_render_meshes([high, low, cage])
...
def main() -> None:
    ...
    low_collection = bpy.data.collections.new("GROUPED003_LOW_TARGETS")
    high_collection = bpy.data.collections.new("GROUPED003_HIGH_SOURCES")
    cage_collection = bpy.data.collections.new("GROUPED003_CAGES")
    ao_collection = bpy.data.collections.new("GROUPED007_AO_OCCLUDERS")
                ...
                projection = (
                    ...
                    and ao_policy == "direct_low_self_occlusion"
                        ...
                        "mode": (
                            "selected_to_active_from_dedicated_bounded_ao_occluder"
                            if map_type == "AO"
                            else "selected_to_active_tangent_normal"
                        ),
                    ...
                    if map_type == "AO"
                        ...
                        "isolated_group": True,
                        "mode": "selected_to_active_tangent_normal",
                        "selected_to_active": True,
                        "cage_object": cage.name,
                        "cage_method": "vertex_normal_offset",
                        "cage_extrusion_m": group_spec[
    ...
    manifest = {
        ...
        "classification_report": (
            file_evidence(resolve(contract["classification_report"]))
            if isinstance(contract.get("classification_report"), str)
            else None
        ),
