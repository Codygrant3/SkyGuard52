...
PREFIX = "AAA_L59_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L59"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L59"
...
def densify():
    ...
    log("loop59 mat palette size=%d" % len(mats))
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # ---- TRUE ART SLICE02 (L59 / L58 freeze deepen) ----
        # Capture-safe deepen: same count of hero anchors, better material separation + VFX readability.
        # Law: L52 HF densify remains FOV; opaque authored content x>=bx+3; no L53 multi-hero stacks.
        ...
        # MAT-01 aircraft metal anchor + readable secondary panel (still one airframe silhouette)
        if name in ("YakBeauty", "Prop", "PropNose", "Wide"):
            if sm_yak:
                spawn_sm(sm_yak, (bx + 3.55, cy - 0.2, cz - 0.4), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA2_MAT01_YakAnchor", m_air or m_panel or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 0.2, cz + 0.1), (0.12, 1.8, 0.7), None, PREFIX + "TA2_MAT01_AirframePanel", m_air or m_panel or mats[0])
            # secondary A/N/R separation strip (tiny, behind wall) — not a second aircraft
            spawn_sm(cube, (bx + 3.45, cy + 0.35, cz + 0.35), (0.06, 0.9, 0.18), unreal.Rotator(0, 0, 8), PREFIX + "TA2_MAT01_MetalStrip", m_air2 or m_bright_metal or mats[1])
            spawn_sm(cube, (bx + 3.42, cy - 0.55, cz + 0.05), (0.05, 0.55, 0.12), None, PREFIX + "TA2_MAT01_RustBreak", m_rust or mats[0])
            # MAT-04 exhaust emissive pin (restrained)
            spawn_sm(sphere, (bx + 3.25, cy - 0.9, cz + 0.15), (0.07, 0.07, 0.07), None, PREFIX + "TA2_MAT04_ExhaustPin", m_glow or unlit_y or mats[0])
            spawn_sm(sphere, (bx + 3.28, cy - 0.95, cz + 0.12), (0.04, 0.04, 0.04), None, PREFIX + "TA2_MAT04_ExhaustCore", unlit_y or m_muzzle or mats[1])
            # optional single prop disc accent only on Prop stage, still behind wall
            if name == "Prop" and sm_prop:
                spawn_sm(sm_prop, (bx + 3.7, cy + 0.0, cz + 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 0, 20), PREFIX + "TA2_MAT01_PropDisc", m_air2 or m_bright_metal or mats[0])

        # MAT-02 concrete + MAT-03 brick with micro-trim readability (no extra buildings)
        if name in ("City", "Wide", "Harbor"):
            if sm_tower:
                spawn_sm(sm_tower, (bx + 3.6, cy + 1.6, cz - 1.0), (0.45, 0.45, 0.55), unreal.Rotator(0, -20, 0), PREFIX + "TA2_MAT02_ConcreteFacade", m_conc or m_panel or mats[1])
            else:
                spawn_sm(cube, (bx + 3.35, cy + 1.6, cz + 0.2), (0.1, 1.4, 2.2), None, PREFIX + "TA2_MAT02_ConcreteSlab", m_conc or m_panel or mats[1])
            # concrete sill/trim for A/N/R edge read
            spawn_sm(cube, (bx + 3.38, cy + 1.6, cz - 0.35), (0.06, 1.2, 0.1), None, PREFIX + "TA2_MAT02_ConcreteSill", m_conc2 or m_bright_white or mats[1])
            if sm_apt:
                spawn_sm(sm_apt, (bx + 3.7, cy - 1.8, cz - 0.8), (0.4, 0.4, 0.5), unreal.Rotator(0, 15, 0), PREFIX + "TA2_MAT03_BrickFacade", m_brick or m_bright_brick or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 1.8, cz + 0.1), (0.1, 1.1, 1.8), None, PREFIX + "TA2_MAT03_BrickSlab", m_brick or m_bright_brick or mats[0])
            # brick header course for class separation vs concrete
            spawn_sm(cube, (bx + 3.42, cy - 1.8, cz + 0.75), (0.05, 1.0, 0.08), None, PREFIX + "TA2_MAT03_BrickHeader", m_brick2 or unlit_y or mats[0])

        # VFX-01 muzzle quality: NS + dual filament cores (tiny additive)
        if name in ("Combat", "ADS", "Cockpit"):
            spawn_niagara(PREFIX + "TA2_VFX01_Muzzle", (bx + 2.45, cy + 0.35, cz + 0.2), "NS_MuzzleFlash", (0.4, 0.4, 0.4))
            spawn_sm(sphere, (bx + 2.42, cy + 0.28, cz + 0.18), (0.1, 0.1, 0.1), None, PREFIX + "TA2_VFX01_CoreA", m_muzzle_hot or m_muzzle or unlit_y or mats[0])
            spawn_sm(sphere, (bx + 2.48, cy + 0.42, cz + 0.22), (0.07, 0.07, 0.07), None, PREFIX + "TA2_VFX01_CoreB", unlit_w or m_bright_white or mats[1])
            spawn_sm(cube, (bx + 2.4, cy + 0.35, cz + 0.2), (0.16, 0.03, 0.03), unreal.Rotator(0, -12, 0), PREFIX + "TA2_VFX01_Filament", m_muzzle or unlit_y or mats[0])

        # VFX-02 ocean contact quality: NS + dual foam tips
        if name in ("Ocean", "Harbor"):
            spawn_niagara(PREFIX + "TA2_VFX02_OceanSpray", (bx + 3.3, cy - 0.5, cz - 0.7), "NS_OceanSpray", (0.55, 0.55, 0.55))
            spawn_sm(sphere, (bx + 3.25, cy - 0.55, cz - 0.62), (0.09, 0.09, 0.09), None, PREFIX + "TA2_VFX02_FoamA", m_foam or unlit_w or mats[1])
            spawn_sm(sphere, (bx + 3.32, cy - 0.35, cz - 0.55), (0.06, 0.06, 0.06), None, PREFIX + "TA2_VFX02_FoamB", m_bright_white or unlit_w or mats[0])
            spawn_sm(sphere, (bx + 3.28, cy - 0.65, cz - 0.5), (0.05, 0.05, 0.05), None, PREFIX + "TA2_VFX02_Droplet", unlit_y or mats[1])

        # VFX-03 impact quality: NS + spark streak chain
        if name in ("City", "Combat", "Harbor"):
            spawn_niagara(PREFIX + "TA2_VFX03_HitSparks", (bx + 3.35, cy + 1.2, cz + 0.4), "NS_HitSparks", (0.45, 0.45, 0.45))
            spawn_sm(sphere, (bx + 3.3, cy + 1.2, cz + 0.4), (0.08, 0.08, 0.08), None, PREFIX + "TA2_VFX03_SparkCore", m_muzzle_hot or m_muzzle or unlit_y or mats[0])
            for k in range(3):
                spawn_sm(sphere, (bx + 3.28, cy + 1.05 + k * 0.12, cz + 0.45 + k * 0.05), (0.045, 0.045, 0.045), None, PREFIX + "TA2_VFX03_Spark_%d" % k, unlit_y or m_muzzle or mats[k % nmat])
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop59 true-art slice02 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L59", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L59_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop59 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l58_freeze_true_art_slice02_materials_niagara_deepen\n")
...
def main():
    log("loop59 L58 freeze + true-art slice02 materials+niagara deepen start")
    ...
    log("Loop59 complete stills=%d" % (len(saved) if saved else 0))
