...
PREFIX = "AAA_L60_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L60"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L60"
...
def densify():
    ...
    log("loop60 mat palette size=%d" % len(mats))
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # ---- TRUE ART SLICE03 (L60 / L59 freeze) ----
        # Multi-mat contrast panels + multi-Niagara event clusters.
        # Still capture-safe: L52 FOV densify retained; opaque x>=bx+3; no L53 multi-hero stacks.
        m_air = load_mat("/Game/Skyguard/Materials/M_Tex_airframe_metal") or load_mat("/Game/Skyguard/Materials/Generated/M_AirframeMetal") or load_mat("/Game/Skyguard/Materials/M_Metal")
        m_air2 = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Airframe") or load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightMetal") or m_air
        m_plate = load_mat("/Game/Skyguard/Materials/M_Tex_L3_plate") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Panel") or m_air2
        m_conc = load_mat("/Game/Skyguard/Materials/M_Tex_concrete") or load_mat("/Game/Skyguard/Materials/M_CityConcrete") or load_mat("/Game/Skyguard/Materials/Generated/M_ConcreteWall")
        m_conc2 = load_mat("/Game/Skyguard/Materials/M_Tex_L4_concrete8") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Plaster") or m_conc
        m_brick = load_mat("/Game/Skyguard/Materials/M_Tex_brick") or load_mat("/Game/Skyguard/Materials/Generated/M_BrickFacade") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Brick")
        m_brick2 = load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightBrick") or m_brick
        m_plaster = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Plaster") or load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightWhite") or unlit_w
        m_glass = load_mat("/Game/Skyguard/Materials/M_CityGlass") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Glass") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan")
        m_rust = load_mat("/Game/Skyguard/Materials/M_MetalRust") or load_mat("/Game/Skyguard/Materials/M_Tex_L4_rust") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Panel")
        m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Muzzle") or unlit_y
        m_muzzle_hot = load_mat("/Game/Skyguard/Materials/Generated/MI_MuzzleFlash_Hot") or load_mat("/Game/Skyguard/Materials/Generated/M_L23_Muzzle") or unlit_y
        m_foam = load_mat("/Game/Skyguard/Materials/Generated/M_L23_Foam") or load_mat("/Game/Skyguard/Materials/M_L5_SeaFoam") or unlit_w
        m_ocean = load_mat("/Game/Skyguard/Materials/Generated/M_L21_BrightOcean") or load_mat("/Game/Skyguard/Materials/M_Ocean") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan")
        ...
        # Aircraft multi-mat readability: one yak + panelization plates (not extra aircraft)
        if name in ("YakBeauty", "Prop", "PropNose", "Wide"):
            if sm_yak:
                spawn_sm(sm_yak, (bx + 3.55, cy - 0.2, cz - 0.4), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA3_MAT01_YakAnchor", m_air or m_panel or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 0.2, cz + 0.1), (0.12, 1.8, 0.7), None, PREFIX + "TA3_MAT01_AirframePanel", m_air or m_panel or mats[0])
            # plate stack / panelization (bright plate over textured airframe language)
            spawn_sm(cube, (bx + 3.45, cy + 0.35, cz + 0.35), (0.06, 0.95, 0.22), unreal.Rotator(0, 0, 8), PREFIX + "TA3_MAT01_MetalStrip", m_air2 or m_bright_metal or mats[1])
            spawn_sm(cube, (bx + 3.48, cy + 0.1, cz + 0.55), (0.05, 0.55, 0.35), None, PREFIX + "TA3_MAT01_PlateA", m_plate or m_bright_metal or mats[0])
            spawn_sm(cube, (bx + 3.48, cy - 0.35, cz + 0.15), (0.05, 0.45, 0.28), None, PREFIX + "TA3_MAT01_PlateB", m_plate or m_air2 or mats[1])
            spawn_sm(cube, (bx + 3.42, cy - 0.55, cz + 0.05), (0.05, 0.55, 0.12), None, PREFIX + "TA3_MAT01_RustBreak", m_rust or mats[0])
            # rivet row for edge density without stack collapse
            for k in range(4):
                spawn_sm(sphere, (bx + 3.5, cy - 0.2 + k * 0.22, cz + 0.7), (0.04, 0.04, 0.04), None, PREFIX + "TA3_MAT01_Rivet_%d" % k, m_bright_metal or unlit_w or mats[k % nmat])
            # exhaust dual pin
            spawn_sm(sphere, (bx + 3.25, cy - 0.9, cz + 0.15), (0.07, 0.07, 0.07), None, PREFIX + "TA3_MAT04_ExhaustPin", m_glow or unlit_y or mats[0])
            spawn_sm(sphere, (bx + 3.28, cy - 0.95, cz + 0.12), (0.045, 0.045, 0.045), None, PREFIX + "TA3_MAT04_ExhaustCore", unlit_y or m_muzzle or mats[1])
            if name == "Prop" and sm_prop:
                spawn_sm(sm_prop, (bx + 3.7, cy + 0.0, cz + 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 0, 20), PREFIX + "TA3_MAT01_PropDisc", m_air2 or m_bright_metal or mats[0])

        # City multi-mat facade language: concrete body + plaster frame + brick header + glass pin
        if name in ("City", "Wide", "Harbor"):
            if sm_tower:
                spawn_sm(sm_tower, (bx + 3.6, cy + 1.6, cz - 1.0), (0.45, 0.45, 0.55), unreal.Rotator(0, -20, 0), PREFIX + "TA3_MAT02_ConcreteFacade", m_conc or m_panel or mats[1])
            else:
                spawn_sm(cube, (bx + 3.35, cy + 1.6, cz + 0.2), (0.1, 1.4, 2.2), None, PREFIX + "TA3_MAT02_ConcreteSlab", m_conc or m_panel or mats[1])
            spawn_sm(cube, (bx + 3.38, cy + 1.6, cz - 0.35), (0.06, 1.25, 0.1), None, PREFIX + "TA3_MAT02_ConcreteSill", m_conc2 or m_bright_white or mats[1])
            spawn_sm(cube, (bx + 3.4, cy + 1.6, cz + 0.85), (0.05, 1.15, 0.08), None, PREFIX + "TA3_MAT02_PlasterLintel", m_plaster or unlit_w or mats[0])
            # window glass pin pair
            spawn_sm(cube, (bx + 3.36, cy + 1.35, cz + 0.25), (0.04, 0.28, 0.35), None, PREFIX + "TA3_MAT02_GlassA", m_glass or mats[2 % nmat])
            spawn_sm(cube, (bx + 3.36, cy + 1.85, cz + 0.25), (0.04, 0.28, 0.35), None, PREFIX + "TA3_MAT02_GlassB", m_glass or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan") or mats[3 % nmat])
            if sm_apt:
                spawn_sm(sm_apt, (bx + 3.7, cy - 1.8, cz - 0.8), (0.4, 0.4, 0.5), unreal.Rotator(0, 15, 0), PREFIX + "TA3_MAT03_BrickFacade", m_brick or m_bright_brick or mats[0])
            else:
                spawn_sm(cube, (bx + 3.4, cy - 1.8, cz + 0.1), (0.1, 1.1, 1.8), None, PREFIX + "TA3_MAT03_BrickSlab", m_brick or m_bright_brick or mats[0])
            spawn_sm(cube, (bx + 3.42, cy - 1.8, cz + 0.75), (0.05, 1.05, 0.08), None, PREFIX + "TA3_MAT03_BrickHeader", m_brick2 or unlit_y or mats[0])
            spawn_sm(cube, (bx + 3.43, cy - 1.8, cz - 0.2), (0.05, 0.9, 0.07), None, PREFIX + "TA3_MAT03_BrickCourse", m_brick or mats[1])

        # Combat/ADS weapon readability: optional rifle proxy tiny + multi Niagara
        if name in ("Combat", "ADS", "Cockpit"):
            if sm_rifle and name in ("ADS", "Cockpit"):
                spawn_sm(sm_rifle, (bx + 3.2, cy + 0.15, cz - 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA3_WPN_Rifle", load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate or mats[0])
            spawn_niagara(PREFIX + "TA3_VFX01_Muzzle", (bx + 2.45, cy + 0.35, cz + 0.2), "NS_MuzzleFlash", (0.45, 0.45, 0.45))
            spawn_niagara(PREFIX + "TA3_VFX01_GunSmoke", (bx + 2.5, cy + 0.25, cz + 0.15), "NS_GunSmoke", (0.3, 0.3, 0.3))
            spawn_sm(sphere, (bx + 2.42, cy + 0.28, cz + 0.18), (0.11, 0.11, 0.11), None, PREFIX + "TA3_VFX01_CoreA", m_muzzle_hot or m_muzzle or unlit_y or mats[0])
            spawn_sm(sphere, (bx + 2.48, cy + 0.42, cz + 0.22), (0.07, 0.07, 0.07), None, PREFIX + "TA3_VFX01_CoreB", unlit_w or m_bright_white or mats[1])
            spawn_sm(cube, (bx + 2.4, cy + 0.35, cz + 0.2), (0.18, 0.03, 0.03), unreal.Rotator(0, -12, 0), PREFIX + "TA3_VFX01_Filament", m_muzzle or unlit_y or mats[0])
            spawn_sm(cube, (bx + 2.43, cy + 0.3, cz + 0.25), (0.12, 0.02, 0.02), unreal.Rotator(0, 10, 0), PREFIX + "TA3_VFX01_FilamentB", unlit_y or mats[1])

        # Ocean multi-Niagara + foam crown
        if name in ("Ocean", "Harbor"):
            spawn_niagara(PREFIX + "TA3_VFX02_OceanSpray", (bx + 3.3, cy - 0.5, cz - 0.7), "NS_OceanSpray", (0.6, 0.6, 0.6))
            spawn_niagara(PREFIX + "TA3_VFX02_WaterSplash", (bx + 3.35, cy - 0.2, cz - 0.75), "NS_WaterSplash", (0.4, 0.4, 0.4))
            spawn_sm(sphere, (bx + 3.25, cy - 0.55, cz - 0.62), (0.09, 0.09, 0.09), None, PREFIX + "TA3_VFX02_FoamA", m_foam or unlit_w or mats[1])
            spawn_sm(sphere, (bx + 3.32, cy - 0.35, cz - 0.55), (0.06, 0.06, 0.06), None, PREFIX + "TA3_VFX02_FoamB", m_bright_white or unlit_w or mats[0])
            spawn_sm(sphere, (bx + 3.28, cy - 0.65, cz - 0.5), (0.05, 0.05, 0.05), None, PREFIX + "TA3_VFX02_Droplet", unlit_y or mats[1])
            spawn_sm(cube, (bx + 3.22, cy - 0.45, cz - 0.85), (0.08, 0.7, 0.06), None, PREFIX + "TA3_VFX02_Waterline", m_ocean or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan") or mats[2 % nmat])

        # Impact multi-Niagara + spark chain
        if name in ("City", "Combat", "Harbor"):
            spawn_niagara(PREFIX + "TA3_VFX03_HitSparks", (bx + 3.35, cy + 1.2, cz + 0.4), "NS_HitSparks", (0.5, 0.5, 0.5))
            spawn_niagara(PREFIX + "TA3_VFX03_Flak", (bx + 3.4, cy + 1.0, cz + 0.55), "NS_FlakBurst", (0.25, 0.25, 0.25))
            spawn_sm(sphere, (bx + 3.3, cy + 1.2, cz + 0.4), (0.09, 0.09, 0.09), None, PREFIX + "TA3_VFX03_SparkCore", m_muzzle_hot or m_muzzle or unlit_y or mats[0])
            for k in range(4):
                spawn_sm(sphere, (bx + 3.28, cy + 1.02 + k * 0.11, cz + 0.45 + k * 0.05), (0.04, 0.04, 0.04), None, PREFIX + "TA3_VFX03_Spark_%d" % k, unlit_y or m_muzzle or mats[k % nmat])
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop60 true-art slice03 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L60", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L60_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop60 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l59_freeze_true_art_slice03_multimat_multiniagara\n")
...
def main():
    log("loop60 L59 freeze + true-art slice03 multi-mat multi-niagara start")
    ...
    log("Loop60 complete stills=%d" % (len(saved) if saved else 0))
