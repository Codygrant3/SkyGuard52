...
PREFIX = "AAA_L34_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L34"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L34"
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L34", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = []
    for name, cam, dist, ny, nz in stages:
        cams.append(("AAA_Cam_L34_%s" % name, cam, (0.0, 0.0, 0.0)))
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop34 stills\n")
...
def main():
    log("loop34 additive yaw0 multi-stage HF lock start")
    ...
    log("Loop34 complete stills=%d" % (len(saved) if saved else 0))
