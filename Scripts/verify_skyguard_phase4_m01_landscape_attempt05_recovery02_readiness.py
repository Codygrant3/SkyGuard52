"""Offline fail-closed readiness for profiling-only recovery02."""
...
OUTPUT = (
    ROOT
    / "Saved/Reports"
    / "PHASE4_M01_LANDSCAPE_VISIBLE_ATTEMPT05_RECOVERY02_READINESS.json"
)
...
def main() -> int:
    ...
    failed_profile = json.loads(
        (
            ROOT
            / contract["immutable_predecessor_evidence"][
                "failed_baseline_profile_receipt"
            ]["file"]
        ).read_text(encoding="utf-8-sig")
    )
    capture_records = failure["captures"]["files"]
    capture_hashes_pass = (
    ...
    checks = {
        "contract_is_offline_pending_compile_and_authorization": (
            contract["status"]
            == "OFFLINE_RECOVERY02_DESIGN_READY_PENDING_CPP_RECOMPILE_AND_AUTHORIZATION"
        ...
        "cpp_fix_source_hashes_locked": cpp_sources_match,
        ...
        "recovery01_profile_fault_is_exact": (
            failed_profile["gate"] == "FAIL"
            and failed_profile["same_process_warmup_and_measurement"] is True
            and failed_profile["startup_frames_excluded"] is True
            and failed_profile["issue"] == "CSV profiler did not start."
        ),
        ...
        "supervisor_reuses_accepted_evidence_and_refuses_duplicate": (
            ...
            and "Recovery02 root already exists; refuse duplicate or overwrite"
            in supervisor
        ),
        ...
        "recovery02_root_absent": not recovery_root.exists(),
    }
    static_ready = all(checks.values())
    if static_ready and compiled:
        gate = "PASS_RECOVERY02_COMPILED_READY_PENDING_AUTHORIZATION"
    elif static_ready:
        gate = "PASS_RECOVERY02_STATIC_READY_PENDING_CPP_RECOMPILE"
    else:
        gate = "FAIL_RECOVERY02_OFFLINE_NOT_READY"
    report = {
        "schema": (
            "skyguard.phase4.m01-landscape-visible-attempt05-recovery02-readiness.v1"
        ),
        ...
        "remaining_before_execution": (
            [
                "compile the asynchronous CSV-start fix in an exclusive build lane",
                "rerun recovery02 readiness",
                "obtain explicit authorization for exactly one recovery02 run",
            ]
            if not compiled
            else [
                "obtain explicit authorization for exactly one recovery02 run"
