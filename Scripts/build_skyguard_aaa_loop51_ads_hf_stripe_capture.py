...
PREFIX = "AAA_L51_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L51"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L51"
...
def densify():
    ...
    log("loop51 mat palette size=%d" % len(mats))
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # Capture recovery densify for Cockpit/City (L50 style) - keep solid multi-mat walls
        if name in ("Cockpit", "City"):
            denser = 2.2 if name == "Cockpit" else 2.8
            yspan = 16 if name == "Cockpit" else 14
            zspan = 12 if name == "Cockpit" else 11
            nmat = max(len(mats), 1)
            for iy in range(-yspan, yspan + 1):
                for iz in range(-zspan, zspan + 1):
        ...
        # Combat partial recovery: light HF stripes only (avoid L50 over-fill)
        if name == "Combat":
            nmat = max(len(mats), 1)
            for iy in range(-14, 15):
                for iz in range(-10, 11):
                    if (iy + iz) % 2 != 0:
                        continue
                    mm = mats[(iy * 11 + iz * 9 + i) % nmat]
                    spawn_sm(cube, (bx + 2.0, cy + iy * 2.4, cz + iz * 2.4), (0.12, 0.35, 0.35), None, PREFIX + "CombatCheck_%d_%d" % (iy, iz), mm)
            for iy in range(-16, 17):
                spawn_sm(cube, (bx + 2.0, cy + iy * 1.8, cz), (0.10, 0.12, 9.0), None, PREFIX + "CombatV_%d" % iy, mats[(iy + i) % nmat])
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop51 ADS-HF densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L51", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L51_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop51 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l50_freeze_ads_only_hf_stripe_densify\n")
...
def main():
    log("loop51 L50 freeze + ADS-only high-frequency stripe densify (no hero) start")
    ...
    log("Loop51 complete stills=%d" % (len(saved) if saved else 0))
