...
$ContractPath = Join-Path $ScriptRoot 'production_refinement01_contract.json'
$BuilderPath = Join-Path $ScriptRoot 'build_m01_lighthouse_production_refinement01.py'
$AttemptPath = 'D:\Skyguard52\Saved\BuildAttempts\M01_LIGHTHOUSE_PRODUCTION_REFINEMENT01\attempt_01'
$TerminalPath = 'D:\Skyguard52\Saved\Reports\M01_LIGHTHOUSE_PRODUCTION_REFINEMENT01_TERMINAL_MANIFEST.json'
$EmergencyPath = 'D:\Skyguard52\Saved\Reports\M01_LIGHTHOUSE_PRODUCTION_REFINEMENT01_EMERGENCY_RECEIPT.jsonl'
$BlenderPath = 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe'
$state = [ordered]@{
    schema = 'skyguard.m01-lighthouse-production-refinement01.terminal-supervisor.v1'
...
function Write-FinalInventory {
    ...
    $value = [ordered]@{
        schema = 'skyguard.m01-lighthouse-production-refinement01.final-artifact-inventory.v1'
        created_at_utc = [DateTime]::UtcNow.ToString('o')
...
try {
    ...
    if ($contract.classification -ne 'PASSED_READY_FOR_STANDING_AUTHORIZED_SINGLE_M01_LIGHTHOUSE_PRODUCTION_REFINEMENT01_BLENDER_EXECUTION') {
        throw 'Contract classification changed'
    }
    ...
    if ($OfflineContractTest) {
        Write-Output 'PASS_M01_LIGHTHOUSE_PRODUCTION_REFINEMENT01_OFFLINE_CONTRACT'
