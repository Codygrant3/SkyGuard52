...
$RecoveryContractPath = Join-Path $ScriptRoot 'source_evaluation_recovery03_contract.json'
$SourceContractPath = 'D:\Skyguard52\Scripts\Production\m01_lighthouse_source_evaluation01\source_evaluation_contract.json'
$WrapperPath = Join-Path $ScriptRoot 'evaluate_m01_lighthouse_sources_recovery03.py'
$AttemptPath = 'D:\Skyguard52\Saved\BuildAttempts\M01_LIGHTHOUSE_SOURCE_EVALUATION01_RECOVERY03\attempt_01'
$TerminalPath = 'D:\Skyguard52\Saved\Reports\M01_LIGHTHOUSE_SOURCE_EVALUATION01_RECOVERY03_TERMINAL_MANIFEST.json'
$EmergencyPath = 'D:\Skyguard52\Saved\Reports\M01_LIGHTHOUSE_SOURCE_EVALUATION01_RECOVERY03_EMERGENCY_RECEIPT.jsonl'
$BlenderPath = 'C:\Program Files\Blender Foundation\Blender 5.2\blender.exe'
$state = [ordered]@{
    schema = 'skyguard.m01-lighthouse-source-evaluation01-recovery03.terminal-supervisor.v1'
...
function Write-FinalInventory {
    ...
    $value = [ordered]@{
        schema = 'skyguard.m01-lighthouse-source-evaluation01-recovery03.final-artifact-inventory.v1'
        created_at_utc = [DateTime]::UtcNow.ToString('o')
...
try {
    ...
    if ($contract.classification -ne 'PASSED_READY_FOR_STANDING_AUTHORIZED_SINGLE_READ_ONLY_BLENDER_SOURCE_EVALUATION_RECOVERY03') {
        throw 'Recovery03 classification changed'
    }
    Verify-Record $contract.recovery02_failure
    ...
    if ($OfflineContractTest) {
        Write-Output 'PASS_M01_LIGHTHOUSE_SOURCE_EVALUATION01_RECOVERY03_OFFLINE_CONTRACT'
    ...
    if (
        ...
        $receipt.classification -ne 'PASSED_SOURCE_EVALUATION_AWAITING_DIRECT_VISUAL_REVIEW' -or
        [int]$receipt.render_count -ne 8
    ) {
...
if ($state.classification -eq 'PASSED_SOURCE_EVALUATION_AWAITING_DIRECT_VISUAL_REVIEW') {
    [Environment]::Exit([int]0)
}
