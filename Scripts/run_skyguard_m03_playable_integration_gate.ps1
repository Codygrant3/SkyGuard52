...
$builder = Join-Path $ProjectRoot "Scripts\build_skyguard_m03_playable_integration.py"
$verifier = Join-Path $ProjectRoot "Scripts\verify_skyguard_m03_playable_integration.py"
$reports = Join-Path $ProjectRoot "Saved\Reports"
$stamp = [DateTime]::UtcNow.ToString("yyyyMMddTHHmmssfffZ")
$attempt = Join-Path $reports "M03_Playable\attempt_$stamp"
...
foreach ($required in @($project, $buildTool, $unrealCmd, $builder, $verifier)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required M03 playable input is missing: $required"
    }
}
...
function Invoke-GovernedStage {
    ...
    $semanticSuccess = switch ($Name) {
        ...
        "compose" {
            $text -match "\[SkyguardM03PlayableBuild\]"
            break
        }
        "fresh_audit" {
            $text -match "\[SkyguardM03PlayableAudit\]"
            break
        }
        "automation" {
    ...
    if ($nonZeroExit -or $markers.Count -gt 0 -or -not $semanticSuccess) {
        throw "$Name failed: exit=$observedExitCode; semantic_success=$semanticSuccess; markers=$($markers -join ', ')"
    }
    return [ordered]@{
        name = $Name
...
$stages += Invoke-GovernedStage -Name "automation" -FilePath $unrealCmd -Arguments @(
    $project,
    "-ExecCmds=Automation RunTests Skyguard52.Mission03",
...
$found = [regex]::Match(
    $automationText,
    "Found ([0-9]+) automation tests based on 'Skyguard52\.Mission03'"
)
...
$audit = Get-Content -LiteralPath (
    Join-Path $reports "M03_PLAYABLE_INTEGRATION_AUDIT.json"
...
$receipt = [ordered]@{
    schema = "skyguard.m03-playable-supervisor.v1"
    gate = $gate
...
$json | Set-Content -LiteralPath (
    Join-Path $reports "M03_PLAYABLE_INTEGRATION_GATE_LATEST.json"
) -Encoding utf8
if ($gate -ne "PASS") {
    throw "Mission 3 playable integration gate failed"
}
Write-Output "M03_PLAYABLE_INTEGRATION_GATE=PASS"
