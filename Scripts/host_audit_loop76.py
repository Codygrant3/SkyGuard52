...
BASE = r"D:\Skyguard52\Saved\Screenshots\AAA_L76"
THUMB = os.path.join(BASE, "thumbs")
OUT_MD = r"D:\Skyguard52\Docs\AAA_Review\CRITIC_FAIL_loop76.md"
...
files = [p for p in glob.glob(os.path.join(BASE, "AAA_Cam_L76_*_*.png"))]
cams = {}
for p in files:
...
for cam, src, a, status, reason, path in rows:
    key[cam.replace("AAA_Cam_L76_", "")] = (status, a)
...
lines = [
"# AAA Critic Report — Loop 76",
"",
"## Verdict: FAIL vs AAA (authoritative)",
"Loop76 uses yaw=0 (+X) cameras aimed into high-frequency checker boards and radial prop proxies with alternating unlit/PBR mats, plus triple-source capture (BASE/FINAL/SCENE when available).",
...
for cam, src, a, status, reason, path in rows:
    short = cam.replace("AAA_Cam_L76_", "")
...
## Latest
- Loop76 HF frustum boards + triple-source capture; map ~{map_size/1e6:.1f}MB
- Usable stills: {yes} | Partial: {partial} | Failed: {no}
- Prop recoverable: {prop_ok}; Yak strong: {yak_ok}; Cockpit strong: {cock_ok}
- Critic: Docs/AAA_Review/CRITIC_FAIL_loop76.md
