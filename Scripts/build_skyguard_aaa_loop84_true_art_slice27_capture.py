...
PREFIX = "AAA_L84_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L84"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L84"
...
def densify():
    ...
    log("loop84 mat palette size=%d" % len(mats))
    ...
    log("loop84 true-art slice27 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L84", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L84_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop84 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l83_freeze_true_art_slice27_niagara_material_push")
...
def main():
    log("loop84 L83 freeze + true-art slice27 niagara + material push start")
    ...
    log("Loop84 complete stills=%d" % (len(saved) if saved else 0))
