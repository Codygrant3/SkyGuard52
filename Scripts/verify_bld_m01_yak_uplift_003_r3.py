"""Strict source/artifact verifier for BLD-M01-YAK-UPLIFT-003-R3."""
...
R3_CLASSIFICATIONS = BASE_CLASSIFICATIONS | {"source_absent_hold"}
...
def resolve_ledger(overlay: dict, base: dict, runtime: dict) -> list[dict]:
    if overlay.get("build_id") != BUILD_ID:
        raise ValidationError("R3 ledger build id mismatch")
    if set(overlay.get("allowed_classifications", [])) != R3_CLASSIFICATIONS:
        raise ValidationError("R3 allowed classifications drifted")
    ...
    if (
        ...
        raise ValidationError("R3 source_absent_hold policy is unsafe")
    entries = runtime.get("entries", [])
    if len(entries) != 240 or len({item["name"] for item in entries}) != 240:
        raise ValidationError("R3 runtime inventory is not 240 unique components")
    ...
    if len(exceptions) != 8 or set(exceptions.values()) != {"source_absent_hold"}:
        raise ValidationError("R3 must have exactly eight source_absent_hold overrides")
    if set(exceptions) - runtime_names:
        raise ValidationError("R3 exception references unknown runtime name")
    observations = overlay.get("actual_source_observations", {})
    if set(observations) != set(exceptions) or len(set(observations.values())) != 8:
        raise ValidationError("R3 actual-source observations do not match exceptions")
    ...
    for entry in entries:
        ...
        if bundle not in base["bundle_defaults"]:
            raise ValidationError(f"R3 unknown bundle: {bundle}")
        classification = exceptions.get(
            name,
            base["component_overrides"].get(name, base["bundle_defaults"][bundle]),
        )
        if classification not in R3_CLASSIFICATIONS:
            raise ValidationError(f"R3 forbidden classification: {classification}")
    ...
    if counts["source_absent_hold"] != 8 or len(resolved) - counts["source_absent_hold"] != 232:
        raise ValidationError("R3 232 + 8 = 240 accounting failed")
    if set(counts) != R3_CLASSIFICATIONS:
        raise ValidationError("R3 must retain all component-by-component classes")
    donor_targets = {
        item["name"] for item in resolved if item["classification"] == "donor_from_002"
    }
    if donor_targets - set(base["donor_replacement_map"]):
        raise ValidationError("R3 donor component lacks replacement mapping")
    return resolved
...
def verify_source_audit(
    ...
    if audit.get("build_id") != "BLD-M01-YAK-UPLIFT-003-R2":
        raise ValidationError("R3 inherited R2 source-inventory audit id mismatch")
    if audit.get("status") != "source_inventory_reconciled_no_blender_run":
        raise ValidationError("R3 inherited source-inventory audit status mismatch")
    if audit.get("audit_method", {}).get("blender_launch_used") is not False:
        raise ValidationError("R3 inherited source-inventory audit must remain source-only")
    ...
    if (
        ...
        raise ValidationError("R3 inherited source-inventory exception policy is unsafe")
    ...
    if absent != set(overlay["classification_overrides"]):
        raise ValidationError("Actual governed-name absences differ from eight R3 exceptions")
    if extras != set(overlay["actual_source_observations"].values()):
        raise ValidationError("Actual dotted source names differ from R3 observations")
    exception_rows = audit.get("source_absent_hold_exceptions", [])
    if len(exception_rows) != 8:
        raise ValidationError("R3 inherited source audit must enumerate eight exception rows")
    ...
    for governed, observed in overlay["actual_source_observations"].items():
        ...
        if (
            ...
            raise ValidationError(f"R3 inherited source audit row mismatch: {governed}")
    return names


def verify_compatibility_audit(audit: dict) -> None:
    if audit.get("build_id") != BUILD_ID:
        raise ValidationError("R3 compatibility audit build id mismatch")
    if audit.get("status") != "source_only_compatibility_boundary_verified":
        raise ValidationError("R3 compatibility audit status mismatch")
    boundary = audit.get("verified_compatibility_boundary", {})
    if (
        boundary.get("rejected_value") != "BLENDER_EEVEE_NEXT"
        or boundary.get("r3_required_value") != "BLENDER_EEVEE"
        or "BLENDER_EEVEE" not in boundary.get("supported_values_reported", [])
    ):
        raise ValidationError("R3 Blender 5.2 compatibility boundary drifted")
    delta = audit.get("allowed_delta", {})
    if (
        delta.get("component_strategy_change_allowed") is not False
        or delta.get("component_accounting_change_allowed") is not False
        or delta.get("camera_or_clearance_change_allowed") is not False
        or delta.get("donor_geometry_change_allowed") is not False
        or delta.get("comparison_camera_change_allowed") is not False
    ):
        raise ValidationError("R3 compatibility-only delta boundary is too broad")
    claims = audit.get("claims", {})
    if any(claims.get(key) is not False for key in (
        "blender_run_performed_for_r3", "unreal_run_performed_for_r3",
        "final", "aaa", "accepted"
    )):
        raise ValidationError("R3 source-only compatibility audit makes a run or acceptance claim")
...
def verify_contract(root: Path) -> tuple[dict, dict, dict, list[dict]]:
    ...
    parent_r2_overlay = load_json(root / overlay["parent_r2_ledger"]["path"])
    base = load_json(root / overlay["base_ledger"]["path"])
    runtime = load_json(root / "Saved" / "Reports" / "L88_RUNTIME_ASSEMBLY_CONTRACT.json")
    if contract.get("schema") != "skyguard.blender-uplift-source-contract.v3":
        raise ValidationError("R3 source contract schema mismatch")
    if contract.get("build_id") != BUILD_ID:
        raise ValidationError("R3 source contract build id mismatch")
    if contract.get("status") != "source_only_not_run":
        raise ValidationError("R3 source contract must remain source_only_not_run")
    if contract.get("does_not_modify_or_invalidate_prior_evidence") is not True:
        raise ValidationError("R3 does not preserve prior attempt evidence")
    if contract.get("required_stage_order") != EXPECTED_STAGE_ORDER:
        raise ValidationError("R3 required stage order drifted")
    for key, record in contract.get("immutable_sources", {}).items():
        verify_record(root, record, key)
    verify_record(root, overlay["base_ledger"], "R3 base ledger")
    verify_record(root, overlay["source_inventory_audit"], "R3 inherited source audit")
    verify_record(root, overlay["parent_r2_ledger"], "R3 parent R2 ledger")
    verify_record(root, overlay["r3_compatibility_audit"], "R3 compatibility audit")
    verify_source_audit(root, inventory_audit, overlay, runtime)
    verify_compatibility_audit(compatibility_audit)
    resolved = resolve_ledger(overlay, base, runtime)
    for item in resolved:
        name = item["name"]
        parent_classification = parent_r2_overlay["classification_overrides"].get(
            name,
            base["component_overrides"].get(
                name, base["bundle_defaults"][item["bundle"]]
            ),
        )
        if item["classification"] != parent_classification:
            raise ValidationError(f"R3 changed R2 component classification: {name}")
    ...
    if (
        ...
        raise ValidationError("R3 contract component accounting is unsafe")
    outputs = contract["outputs"]
    for key in ("directory", "blend", "glb", "manifest", "comparison_directory"):
        path = outputs[key].replace("\\", "/")
        if "UPLIFT_003_R3" not in path.upper():
            raise ValidationError(f"R3 output not isolated: {path}")
        if (
            "Yak52_Uplift_003/" in path
            or "Yak52_Uplift_003_R2/" in path
            or "BLD_M01_YAK_UPLIFT_003_MANIFEST" in path
            or "BLD_M01_YAK_UPLIFT_003_R2_MANIFEST" in path
        ):
            raise ValidationError(f"R3 output overlaps R1/R2 namespace: {path}")
    if len(contract.get("matched_comparison_slots", [])) != 5:
        raise ValidationError("R3 requires five matched comparisons")
    if len({slot["candidate"] for slot in contract["matched_comparison_slots"]}) != 5:
        raise ValidationError("R3 comparison candidate paths are not unique")
    ...
    if (
        ...
        raise ValidationError("R3 inheritance/promotion guard drifted")
    compatibility = contract.get("render_compatibility", {})
    if (
        compatibility.get("required_engine") != "BLENDER_EEVEE"
        or compatibility.get("forbidden_engine") != "BLENDER_EEVEE_NEXT"
        or compatibility.get("allowed_behavioral_delta_from_r2")
        != "render_engine_enum_only"
    ):
        raise ValidationError("R3 render compatibility contract drifted")
    return contract, overlay, compatibility_audit, resolved


def verify_generator_source(root: Path, overlay: dict) -> None:
    path = root / "Scripts" / "blender_bld_m01_yak_uplift_003_r3.py"
    if not path.is_file():
        raise ValidationError("R3 generator missing")
    source = path.read_text(encoding="utf-8")
    try:
        tree = ast.parse(source, filename=str(path))
    except SyntaxError as exc:
        raise ValidationError(f"R3 generator syntax error: {exc}") from exc
    functions = {
    ...
    if required - set(functions):
        raise ValidationError(f"R3 generator functions missing: {sorted(required-set(functions))}")
    forbidden_calls = []
    open_calls = []
    ...
    if forbidden_calls:
        raise ValidationError(f"R3 generator has forbidden import calls: {forbidden_calls}")
    if len(open_calls) != 1 or "BLEND_PATH" not in ast.unparse(open_calls[0]):
        raise ValidationError("R3 generator must open exactly one isolated copied blend")
    if "shutil.copy2(L88_BLEND_PATH, BLEND_PATH)" not in source:
        raise ValidationError("R3 generator does not byte-copy L88 to R3 output")
    if ".main(" in source or "r1.main" in source:
        raise ValidationError("R3 generator must never invoke the R1 main function")
    if "source_absent_hold" not in source:
        raise ValidationError("R3 generator lacks source_absent_hold handling")
    if "BLENDER_EEVEE_NEXT" in source:
        raise ValidationError("R3 generator contains forbidden BLENDER_EEVEE_NEXT")
    if 'scene.render.engine = "BLENDER_EEVEE"' not in source:
        raise ValidationError("R3 generator does not select Blender 5.2 BLENDER_EEVEE")
    ...
    if skip_position < 0 or lookup_position < 0 or skip_position > lookup_position:
        raise ValidationError("R3 generator may require a source_absent_hold Blender object")
    if "continue" not in tag_source[skip_position:lookup_position]:
        raise ValidationError("R3 generator does not skip source_absent_hold object lookup")
    ...
    if "bpy.data.objects.new" in exception_source or "primitive_" in exception_source:
        raise ValidationError("R3 generator synthesizes source-absent objects")
    for governed_name in overlay["classification_overrides"]:
        if governed_name in source:
            raise ValidationError(
                f"R3 generator hard-codes absent governed object name: {governed_name}"
            )
    ...
    if any(position < 0 for position in positions) or positions != sorted(positions):
        raise ValidationError("R3 generator main stage call order drifted")
    if "refuse_overwrite()" not in main_source:
        raise ValidationError("R3 generator lacks overwrite refusal")

    r2_path = root / "Scripts" / "blender_bld_m01_yak_uplift_003_r2.py"
    r2_source = r2_path.read_text(encoding="utf-8")
    r2_tree = ast.parse(r2_source, filename=str(r2_path))
    r2_functions = {
        node.name: node for node in r2_tree.body if isinstance(node, ast.FunctionDef)
    }

    def normalized_function_text(text: str) -> str:
        return (
            text.replace("BLENDER_EEVEE_NEXT", "BLENDER_EEVEE")
            .replace("UPLIFT003R2", "UPLIFT003RX")
            .replace("UPLIFT003R3", "UPLIFT003RX")
            .replace("UPLIFT_003_R2", "UPLIFT_003_RX")
            .replace("UPLIFT_003_R3", "UPLIFT_003_RX")
            .replace("Uplift_003_R2", "Uplift_003_RX")
            .replace("Uplift_003_R3", "Uplift_003_RX")
            .replace("SKG_R2", "SKG_RX")
            .replace("SKG_R3", "SKG_RX")
            .replace('"R2 ', '"RX ')
            .replace('"R3 ', '"RX ')
        )

    compatibility_only_functions = (
        "stage_camera_and_clearance",
        "stage_component_ledger_tags",
        "stage_selective_002_donors",
        "stage_matched_comparison_setup",
    ...
    for name in compatibility_only_functions:
        r2_text = ast.get_source_segment(r2_source, r2_functions[name]) or ""
        r3_text = ast.get_source_segment(source, functions[name]) or ""
        if normalized_function_text(r2_text) != normalized_function_text(r3_text):
            raise ValidationError(
                f"R3 changed more than namespace/engine compatibility in {name}"
            )
...
def verify_artifacts(
    ...
    if manifest.get("build_id") != BUILD_ID:
        raise ValidationError("R3 artifact build id mismatch")
    if manifest.get("stage_order") != EXPECTED_STAGE_ORDER:
        raise ValidationError("R3 artifact stage order drifted")
    if manifest.get("promotion_allowed") is not False:
        raise ValidationError("R3 artifact silently promotes")
    ...
    if any(
        ...
        raise ValidationError("R3 artifact makes an acceptance/final/AAA claim")
    for key in ("blend", "glb"):
        verify_record(root, manifest["outputs"][key], f"R3 artifact {key}")
    ...
    if manifest.get("original_l88_unchanged") is not True:
        raise ValidationError("R3 manifest does not attest unchanged L88 source")
    if manifest.get("resolved_component_ledger") != expected_resolved:
        raise ValidationError("R3 artifact resolved ledger drifted")
    ...
    if manifest.get("classification_counts") != dict(sorted(counts.items())):
        raise ValidationError("R3 artifact classification counts drifted")
    ...
    if accounting != {
        ...
        raise ValidationError("R3 artifact 232 + 8 = 240 accounting failed")
    exception_records = manifest.get("source_absent_hold_records", [])
    if len(exception_records) != 8:
        raise ValidationError("R3 artifact exception record count is not eight")
    ...
    if {row.get("governed_name") for row in exception_records} != expected_exceptions:
        raise ValidationError("R3 artifact exception names drifted")
    for row in exception_records:
        if (
            ...
            raise ValidationError("R3 artifact source-absent exception is unsafe")
    object_records = {row["name"]: row for row in manifest.get("object_records", [])}
    if expected_exceptions & set(object_records):
        raise ValidationError("R3 source-absent governed name was synthesized as object")
    for item in expected_resolved:
        if item["classification"] == "source_absent_hold":
            continue
        row = object_records.get(item["name"])
        if not row:
            raise ValidationError(f"R3 exact object record missing: {item['name']}")
        if (
            row.get("uplift_class") != item["classification"]
            or row.get("promotion_allowed") is not False
            or row.get("inherited_from") != "L88"
        ):
            raise ValidationError(f"R3 exact object governance mismatch: {item['name']}")
    for name in contract["required_donor_objects"]:
        ...
        if (
            ...
            raise ValidationError(f"R3 selective donor governance mismatch: {name}")
    for name, spec in contract["required_safety_and_clearance_volumes"].items():
        ...
        if (
            ...
            raise ValidationError(f"R3 clearance/safety volume mismatch: {name}")
    ...
    if (
        ...
        raise ValidationError("R3 corrected rear-gunner camera record missing")
    comparisons = manifest.get("matched_comparisons", [])
    if len(comparisons) != 5:
        raise ValidationError("R3 artifact lacks five comparisons")
    by_slot = {item["slot"]: item for item in comparisons}
    for slot in contract["matched_comparison_slots"]:
        row = by_slot.get(slot["slot"])
        if not row or row.get("baseline") != slot["baseline"]:
            raise ValidationError(f"R3 matched baseline drifted: {slot['slot']}")
        candidate = row.get("candidate")
        if not candidate or candidate.get("path") != slot["candidate"]:
            raise ValidationError(f"R3 candidate record missing: {slot['slot']}")
        verify_record(root, candidate, f"R3 {slot['slot']} candidate")
