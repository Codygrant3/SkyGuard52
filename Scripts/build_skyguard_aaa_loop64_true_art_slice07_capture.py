...
PREFIX = "AAA_L64_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L64"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L64"
...
def create_textured_material(name, albedo_path, normal_path=None, rough_path=None, metallic_const=0.15, emissive_scale=0.0, brighten=1.12, uv_scale=1.0, rough_bias=0.0):
    ...
    try:
        ...
        try:
            ...
            pass
        log("slice07 mat ready " + name)
        return mat
    except Exception as e:
        log("slice07 mat fail %s %s" % (name, e))
        return None

def ensure_slice07_materials():
    global _SLICE07_MATS
    if _SLICE07_MATS is not None:
        return _SLICE07_MATS
    mats = {}
    ...
    _SLICE07_MATS = mats
    log("slice07 materials cached count=%d" % len([k for k,v in mats.items() if v]))
    return mats
...
def densify():
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # ---- TRUE ART SLICE07 (L64 / L63 freeze) ----
        # AO/detail contrast materials + denser multi-Niagara event language.
        # Capture-safe: L52 FOV densify retained; opaque x>=bx+3; AO only as seam/cavity accents, not sole FOV.
        s7 = ensure_slice07_materials()
        m_air = s7.get("air") or mats[0]
        m_plate = s7.get("plate") or m_air
        m_rust = s7.get("rust") or mats[1]
        m_conc = s7.get("concrete") or mats[1]
        m_brick = s7.get("brick") or mats[0]
        m_air_ao = s7.get("air_ao") or m_air
        m_plate_ao = s7.get("plate_ao") or m_plate
        m_conc_ao = s7.get("concrete_ao") or m_conc
        m_brick_d = s7.get("brick_detail") or m_brick
        m_rust_d = s7.get("rust_detail") or m_rust
        m_plaster = s7.get("plaster") or unlit_w or mats[0]
        m_glass = s7.get("glass") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan") or mats[2 % nmat]
        m_muzzle_hot = s7.get("muzzle") or unlit_y or mats[0]
        m_foam = s7.get("foam") or unlit_w or mats[1]
        m_waterline = s7.get("waterline") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan") or mats[2 % nmat]
        m_glow = s7.get("glow") or m_muzzle_hot
        m_expl = s7.get("explosion") or m_muzzle_hot
        ...
        # Aircraft HF + AO seam language (one silhouette)
        if name in ("YakBeauty", "Prop", "PropNose", "Wide"):
            if sm_yak:
                spawn_sm(sm_yak, (bx + 3.55, cy - 0.2, cz - 0.4), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA7_MAT01_YakAnchor", m_air)
            else:
                spawn_sm(cube, (bx + 3.4, cy - 0.2, cz + 0.1), (0.12, 1.8, 0.7), None, PREFIX + "TA7_MAT01_AirframePanel", m_air)
            for k in range(3):
                spawn_sm(cube, (bx + 3.46, cy - 0.55 + k * 0.45, cz + 0.15 + (k % 2) * 0.12), (0.05, 0.42, 0.26), None, PREFIX + "TA7_MAT01_Plate_%d" % k, m_plate)
            # AO cavity seams between plates (thin darker strips only)
            for k in range(4):
                spawn_sm(cube, (bx + 3.44, cy - 0.65 + k * 0.35, cz + 0.05), (0.04, 0.08, 0.35), None, PREFIX + "TA7_MAT01_AOSeam_%d" % k, m_air_ao)
            spawn_sm(cube, (bx + 3.45, cy + 0.35, cz + 0.35), (0.06, 0.95, 0.22), unreal.Rotator(0, 0, 8), PREFIX + "TA7_MAT01_MetalStrip", m_plate)
            spawn_sm(cube, (bx + 3.43, cy + 0.35, cz + 0.2), (0.04, 0.9, 0.05), None, PREFIX + "TA7_MAT01_AOStrip", m_plate_ao)
            spawn_sm(cube, (bx + 3.42, cy - 0.55, cz + 0.05), (0.05, 0.55, 0.12), None, PREFIX + "TA7_MAT01_RustBreak", m_rust_d)
            spawn_sm(cube, (bx + 3.43, cy + 0.6, cz - 0.05), (0.05, 0.35, 0.1), None, PREFIX + "TA7_MAT01_RustEdge", m_rust)
            for k in range(7):
                spawn_sm(sphere, (bx + 3.5, cy - 0.35 + k * 0.16, cz + 0.7), (0.03, 0.03, 0.03), None, PREFIX + "TA7_MAT01_Rivet_%d" % k, m_plate)
            for k in range(5):
                spawn_sm(sphere, (bx + 3.49, cy + 0.1 + k * 0.1, cz + 0.4), (0.025, 0.025, 0.025), None, PREFIX + "TA7_MAT01_Screw_%d" % k, m_plate_ao)
            spawn_sm(sphere, (bx + 3.25, cy - 0.9, cz + 0.15), (0.07, 0.07, 0.07), None, PREFIX + "TA7_MAT04_ExhaustPin", m_glow)
            spawn_sm(sphere, (bx + 3.28, cy - 0.95, cz + 0.12), (0.05, 0.05, 0.05), None, PREFIX + "TA7_MAT04_ExhaustCore", m_muzzle_hot)
            spawn_sm(sphere, (bx + 3.22, cy - 0.85, cz + 0.08), (0.04, 0.04, 0.04), None, PREFIX + "TA7_MAT04_ExhaustHalo", unlit_y or m_muzzle_hot)
            if name == "Prop" and sm_prop:
                spawn_sm(sm_prop, (bx + 3.7, cy + 0.0, cz + 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 0, 20), PREFIX + "TA7_MAT01_PropDisc", m_plate)
                spawn_niagara(PREFIX + "TA7_VFX_PropWash", (bx + 3.65, cy + 0.0, cz + 0.05), "NS_PropWash", (0.45, 0.45, 0.45))
                spawn_niagara(PREFIX + "TA7_VFX_Contrail", (bx + 3.8, cy + 0.1, cz + 0.0), "NS_ContrailRibbon", (0.3, 0.3, 0.3))

        # City AO/detail multi-mat facade language
        if name in ("City", "Wide", "Harbor"):
            if sm_tower:
                spawn_sm(sm_tower, (bx + 3.6, cy + 1.6, cz - 1.0), (0.45, 0.45, 0.55), unreal.Rotator(0, -20, 0), PREFIX + "TA7_MAT02_ConcreteFacade", m_conc)
            else:
                spawn_sm(cube, (bx + 3.35, cy + 1.6, cz + 0.2), (0.1, 1.4, 2.2), None, PREFIX + "TA7_MAT02_ConcreteSlab", m_conc)
            # AO recess bands under sills
            for k in range(3):
                spawn_sm(cube, (bx + 3.37, cy + 1.2 + k * 0.35, cz - 0.42 + k * 0.05), (0.04, 0.95, 0.04), None, PREFIX + "TA7_MAT02_AORecess_%d" % k, m_conc_ao)
                spawn_sm(cube, (bx + 3.38, cy + 1.2 + k * 0.35, cz - 0.35 + k * 0.05), (0.05, 0.9, 0.08), None, PREFIX + "TA7_MAT02_Sill_%d" % k, m_plaster)
            spawn_sm(cube, (bx + 3.4, cy + 1.6, cz + 0.85), (0.05, 1.15, 0.08), None, PREFIX + "TA7_MAT02_PlasterLintel", m_plaster)
            for iy in range(2):
                for ix in range(3):
                    spawn_sm(cube, (bx + 3.36, cy + 1.15 + ix * 0.4, cz + 0.0 + iy * 0.4), (0.04, 0.18, 0.26), None, PREFIX + "TA7_MAT02_Glass_%d_%d" % (ix, iy), m_glass)
                    # AO window reveal
                    spawn_sm(cube, (bx + 3.35, cy + 1.15 + ix * 0.4, cz - 0.05 + iy * 0.4), (0.03, 0.22, 0.04), None, PREFIX + "TA7_MAT02_WinAO_%d_%d" % (ix, iy), m_conc_ao)
            if sm_apt:
                spawn_sm(sm_apt, (bx + 3.7, cy - 1.8, cz - 0.8), (0.4, 0.4, 0.5), unreal.Rotator(0, 15, 0), PREFIX + "TA7_MAT03_BrickFacade", m_brick)
            else:
                spawn_sm(cube, (bx + 3.4, cy - 1.8, cz + 0.1), (0.1, 1.1, 1.8), None, PREFIX + "TA7_MAT03_BrickSlab", m_brick)
            for k in range(4):
                spawn_sm(cube, (bx + 3.42, cy - 1.8, cz + 0.65 - k * 0.28), (0.05, 1.0, 0.06), None, PREFIX + "TA7_MAT03_BrickCourse_%d" % k, m_brick_d if k % 2 == 0 else m_brick)
            spawn_sm(cube, (bx + 3.44, cy - 1.5, cz + 0.2), (0.04, 0.2, 0.5), None, PREFIX + "TA7_MAT03_BrickQuoin", m_plaster)
            spawn_sm(cube, (bx + 3.35, cy + 2.2, cz - 0.7), (0.08, 0.22, 0.16), None, PREFIX + "TA7_MAT02_UtilBox", m_plate)
            spawn_sm(cube, (bx + 3.34, cy + 2.2, cz - 0.55), (0.05, 0.1, 0.05), None, PREFIX + "TA7_MAT02_UtilLatch", m_plate_ao)
            # fire/smoke city event accent (tiny, behind wall)
            if name in ("City", "Wide"):
                spawn_niagara(PREFIX + "TA7_VFX_CityFire", (bx + 3.5, cy + 0.5, cz + 0.8), "NS_CityFire", (0.25, 0.25, 0.25))
        ...
        if name in ("Combat", "ADS", "Cockpit"):
            if sm_rifle and name in ("ADS", "Cockpit"):
                spawn_sm(sm_rifle, (bx + 3.2, cy + 0.15, cz - 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA7_WPN_Rifle", load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate)
            if sm_glove and name in ("ADS", "Cockpit"):
                spawn_sm(sm_glove, (bx + 3.15, cy + 0.0, cz - 0.25), (0.3, 0.3, 0.3), unreal.Rotator(0, 90, 0), PREFIX + "TA7_WPN_Glove", load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_rust)
            if sm_arm and name in ("ADS", "Cockpit"):
                spawn_sm(sm_arm, (bx + 3.18, cy - 0.15, cz - 0.35), (0.28, 0.28, 0.28), unreal.Rotator(0, 90, 0), PREFIX + "TA7_WPN_Arm", load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_rust)
            if sm_instr and name == "Cockpit":
                spawn_sm(sm_instr, (bx + 3.25, cy + 0.4, cz + 0.35), (0.25, 0.25, 0.25), None, PREFIX + "TA7_COCK_Instruments", m_plate)
            spawn_niagara(PREFIX + "TA7_VFX01_Muzzle", (bx + 2.45, cy + 0.35, cz + 0.2), "NS_MuzzleFlash", (0.6, 0.6, 0.6))
            spawn_niagara(PREFIX + "TA7_VFX01_GunSmoke", (bx + 2.5, cy + 0.25, cz + 0.15), "NS_GunSmoke", (0.45, 0.45, 0.45))
            spawn_niagara(PREFIX + "TA7_VFX01_Tracer", (bx + 2.55, cy + 0.4, cz + 0.25), "NS_TracerBurst", (0.4, 0.4, 0.4))
            spawn_niagara(PREFIX + "TA7_VFX01_Shells", (bx + 2.35, cy + 0.15, cz + 0.05), "NS_ShellCasings", (0.3, 0.3, 0.3))
            if name == "Combat":
                spawn_niagara(PREFIX + "TA7_VFX01_Missile", (bx + 2.7, cy - 0.2, cz + 0.4), "NS_MissileTrail", (0.3, 0.3, 0.3))
                spawn_niagara(PREFIX + "TA7_VFX01_Igla", (bx + 2.8, cy + 0.5, cz + 0.1), "NS_IglaLaunch", (0.25, 0.25, 0.25))
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.4, cy - 1.0, cz + 0.6), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 10), PREFIX + "TA7_ENEMY_Shahed", m_plate_ao)
                spawn_niagara(PREFIX + "TA7_VFX01_DroneExpl", (bx + 3.35, cy - 1.0, cz + 0.7), "NS_DroneExplosion", (0.35, 0.35, 0.35))
                spawn_sm(sphere, (bx + 3.35, cy - 1.0, cz + 0.7), (0.12, 0.12, 0.12), None, PREFIX + "TA7_VFX01_ExplCore", m_expl)
            spawn_sm(sphere, (bx + 2.42, cy + 0.28, cz + 0.18), (0.12, 0.12, 0.12), None, PREFIX + "TA7_VFX01_CoreA", m_muzzle_hot)
            spawn_sm(sphere, (bx + 2.48, cy + 0.42, cz + 0.22), (0.08, 0.08, 0.08), None, PREFIX + "TA7_VFX01_CoreB", unlit_w or m_bright_white or mats[1])
            spawn_sm(sphere, (bx + 2.52, cy + 0.33, cz + 0.28), (0.05, 0.05, 0.05), None, PREFIX + "TA7_VFX01_CoreC", unlit_y or m_muzzle_hot)
            for k in range(4):
                spawn_sm(cube, (bx + 2.36 + k * 0.03, cy + 0.28 + k * 0.05, cz + 0.16 + k * 0.03), (0.17 - k * 0.02, 0.022, 0.022), unreal.Rotator(0, -14 + k * 7, 0), PREFIX + "TA7_VFX01_Filament_%d" % k, m_muzzle_hot)
            for k in range(4):
                spawn_sm(sphere, (bx + 2.55, cy + 0.08 + k * 0.07, cz + 0.0 + k * 0.03), (0.03, 0.03, 0.03), None, PREFIX + "TA7_VFX01_Shell_%d" % k, m_plate)

        # Ocean multi-Niagara + denser foam crown
        if name in ("Ocean", "Harbor"):
            spawn_niagara(PREFIX + "TA7_VFX02_OceanSpray", (bx + 3.3, cy - 0.5, cz - 0.7), "NS_OceanSpray", (0.75, 0.75, 0.75))
            spawn_niagara(PREFIX + "TA7_VFX02_WaterSplash", (bx + 3.35, cy - 0.2, cz - 0.75), "NS_WaterSplash", (0.55, 0.55, 0.55))
            spawn_sm(cube, (bx + 3.22, cy - 0.45, cz - 0.85), (0.08, 0.85, 0.06), None, PREFIX + "TA7_VFX02_Waterline", m_waterline)
            for k in range(6):
                spawn_sm(sphere, (bx + 3.24, cy - 0.8 + k * 0.16, cz - 0.55 + (k % 2) * 0.08), (0.045 + (k % 2) * 0.015, 0.045 + (k % 2) * 0.015, 0.045 + (k % 2) * 0.015), None, PREFIX + "TA7_VFX02_Foam_%d" % k, m_foam)
            for k in range(5):
                spawn_sm(sphere, (bx + 3.3, cy - 0.4 + k * 0.1, cz - 0.4), (0.03, 0.03, 0.03), None, PREFIX + "TA7_VFX02_Mist_%d" % k, m_foam)

        # Impact multi-Niagara + denser spark/debris language
        if name in ("City", "Combat", "Harbor"):
            spawn_niagara(PREFIX + "TA7_VFX03_HitSparks", (bx + 3.35, cy + 1.2, cz + 0.4), "NS_HitSparks", (0.65, 0.65, 0.65))
            spawn_niagara(PREFIX + "TA7_VFX03_Flak", (bx + 3.4, cy + 1.0, cz + 0.55), "NS_FlakBurst", (0.4, 0.4, 0.4))
            spawn_sm(sphere, (bx + 3.3, cy + 1.2, cz + 0.4), (0.1, 0.1, 0.1), None, PREFIX + "TA7_VFX03_SparkCore", m_muzzle_hot)
            for k in range(7):
                spawn_sm(sphere, (bx + 3.28, cy + 0.9 + k * 0.08, cz + 0.45 + k * 0.03), (0.03, 0.03, 0.03), None, PREFIX + "TA7_VFX03_Spark_%d" % k, m_muzzle_hot)
            for k in range(4):
                spawn_sm(cube, (bx + 3.32, cy + 1.0 + k * 0.07, cz + 0.22), (0.03, 0.05, 0.03), unreal.Rotator(0, k * 18, 0), PREFIX + "TA7_VFX03_Debris_%d" % k, m_plate_ao)
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop64 true-art slice07 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L64", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L64_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop64 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l63_freeze_true_art_slice07_ao_detail_multiniagara\n")
...
def main():
    log("loop64 L63 freeze + true-art slice07 AO/detail mats + multi-niagara start")
    ...
    log("Loop64 complete stills=%d" % (len(saved) if saved else 0))
