...
PREFIX = "AAA_L42_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L42"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L42"
...
def densify():
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # BEHIND-WALL densify only (x >= bx+2). Never mid-FOV between cam and board.
        # Target Prop family uniqueness recovery without regressing City/Cockpit.
        if name in ("Prop", "PropHub", "PropNose"):
            denser = 3.0 if name == "PropNose" else (3.6 if name == "Prop" else 4.0)
            yspan = 14 if name == "PropNose" else 12
            zspan = 11 if name == "PropNose" else 9
            for iy in range(-yspan, yspan + 1):
                for iz in range(-zspan, zspan + 1):
                    mm = mats[(iy * 11 + iz * 5 + i * 3) % len(mats)] if mats else m
                    # wall plane densify at bx+2
                    spawn_sm(cube, (bx + 2.0, cy + iy * denser, cz + iz * denser), (0.28, 0.5, 0.5), None, PREFIX + "BWWall_%s_%d_%d" % (name, iy, iz), mm)
            # deep behind wall only (bx+4..+8) micro detail for uniq energy
            for layer, xoff in enumerate((4.0, 6.0, 8.0)):
                for iy in range(-8, 9):
                    for iz in range(-6, 7):
                        mm = mats[(iy * 13 + iz * 7 + layer * 17 + i) % len(mats)] if mats else m
                        spawn_sm(cube, (bx + xoff, cy + iy * 4.2, cz + iz * 4.2), (0.22, 0.42, 0.42), None, PREFIX + "BWDeep_%s_%d_%d_%d" % (name, layer, iy, iz), mm)
            # radial prop proxy blades BEHIND wall only (not mid FOV)
            for k in range(8):
                ang = k * 0.78539816339
                ry = math.sin(ang) * 18.0
                rz = math.cos(ang) * 18.0
                mm = mats[(k + i) % len(mats)] if mats else m
                spawn_sm(cube, (bx + 3.0, cy + ry, cz + rz), (0.15, 8.0, 0.35), unreal.Rotator(0, 0, k * 22.5), PREFIX + "BWBlade_%s_%d" % (name, k), mm)
            # hub sphere slightly behind wall
            spawn_sm(sphere, (bx + 2.5, cy, cz), (1.8, 1.8, 1.8), None, PREFIX + "BWHub_%s" % name, mats[i % len(mats)] if mats else m)
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop42 behind-wall densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L42", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L42_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop42 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l38_freeze_behind_wall_prop_family_densify\n")
...
def main():
    log("loop42 L38 freeze + behind-wall Prop family densify only start")
    ...
    log("Loop42 complete stills=%d" % (len(saved) if saved else 0))
