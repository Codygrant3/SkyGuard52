...
PREFIX = "AAA_L62_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L62"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L62"
...
def create_textured_material(name, albedo_path, normal_path=None, rough_path=None, metallic_const=0.15, emissive_scale=0.0, brighten=1.12):
    """Once-authored texture-sampled material under Generated. Brighten keeps capture from dark collapse."""
    path = "/Game/Skyguard/Materials/Generated/" + name
    try:
        if unreal.EditorAssetLibrary.does_asset_exist(path):
            # Prefer load existing rather than rebuild every densify (speed + stability)
        ...
        # optional brighten multiply for capture readability
        ...
        # specular constant (mid-high readability)
        ...
        try:
            ...
            pass
        log("slice05 mat ready " + name)
        return mat
    except Exception as e:
        log("slice05 mat fail %s %s" % (name, e))
        return None

def create_const_material(name, base_color, roughness=0.5, metallic=0.0, specular=0.5, emissive=None):
    path = "/Game/Skyguard/Materials/Generated/" + name
    try:
        if unreal.EditorAssetLibrary.does_asset_exist(path):
            return unreal.EditorAssetLibrary.load_asset(path)
        ...
        try:
            ...
            pass
        log("slice05 const mat ready " + name)
        return mat
    except Exception as e:
        log("slice05 const mat fail %s %s" % (name, e))
        return None

def ensure_slice05_materials():
    global _SLICE05_MATS
    if _SLICE05_MATS is not None:
        return _SLICE05_MATS
    mats = {}
    # Texture-sampled A/N/R masters (once)
    mats["air"] = create_textured_material(
        ...
        metallic_const=0.55, brighten=1.18,
    ) or load_mat("/Game/Skyguard/Materials/Generated/M_L61_AirframeBright") or load_mat("/Game/Skyguard/Materials/M_Tex_airframe_metal")
    mats["plate"] = create_textured_material(
        ...
        metallic_const=0.7, brighten=1.15,
    ) or load_mat("/Game/Skyguard/Materials/Generated/M_L61_PlateBright") or load_mat("/Game/Skyguard/Materials/M_Tex_L3_plate")
    mats["rust"] = create_textured_material(
        ...
        metallic_const=0.3, brighten=1.1,
    ) or load_mat("/Game/Skyguard/Materials/Generated/M_L61_RustWarm") or load_mat("/Game/Skyguard/Materials/M_MetalRust")
    mats["concrete"] = create_textured_material(
        ...
        metallic_const=0.02, brighten=1.2,
    ) or load_mat("/Game/Skyguard/Materials/Generated/M_L61_ConcreteLit") or load_mat("/Game/Skyguard/Materials/M_Tex_concrete")
    mats["brick"] = create_textured_material(
        ...
        metallic_const=0.0, brighten=1.15,
    ) or load_mat("/Game/Skyguard/Materials/Generated/M_L61_BrickWarm") or load_mat("/Game/Skyguard/Materials/M_Tex_brick")
    # Keep useful const emissives from L61 lineage
    mats["plaster"] = load_mat("/Game/Skyguard/Materials/Generated/M_L61_PlasterLit") or create_const_material("M_L62_PlasterLit", (0.88, 0.86, 0.8), roughness=0.7)
    mats["glass"] = load_mat("/Game/Skyguard/Materials/Generated/M_L61_GlassLit") or create_const_material("M_L62_GlassLit", (0.35, 0.55, 0.65), roughness=0.08, metallic=0.05, specular=0.95, emissive=(0.03, 0.05, 0.07))
    mats["muzzle"] = load_mat("/Game/Skyguard/Materials/Generated/M_L61_MuzzleHot") or create_const_material("M_L62_MuzzleHot", (1.0, 0.72, 0.22), roughness=0.25, emissive=(2.2, 1.1, 0.2))
    mats["foam"] = load_mat("/Game/Skyguard/Materials/Generated/M_L61_FoamLit") or create_const_material("M_L62_FoamLit", (0.92, 0.95, 0.98), roughness=0.45, emissive=(0.08, 0.1, 0.12))
    mats["waterline"] = load_mat("/Game/Skyguard/Materials/Generated/M_L61_Waterline") or create_const_material("M_L62_Waterline", (0.12, 0.38, 0.48), roughness=0.2, metallic=0.05, specular=0.8, emissive=(0.02, 0.05, 0.07))
    mats["glow"] = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or mats["muzzle"]
    _SLICE05_MATS = mats
    log("slice05 materials cached count=%d" % len([k for k,v in mats.items() if v]))
    return mats
...
def densify():
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # ---- TRUE ART SLICE05 (L62 / L61 freeze) ----
        # Texture-sampled A/N/R materials (once-cached) + multi-Niagara clusters.
        # Capture-safe: L52 FOV densify retained; opaque x>=bx+3; no L53 multi-hero stacks.
        s5 = ensure_slice05_materials()
        m_air = s5.get("air") or load_mat("/Game/Skyguard/Materials/M_Tex_airframe_metal") or mats[0]
        m_plate = s5.get("plate") or load_mat("/Game/Skyguard/Materials/M_Tex_L3_plate") or m_air
        m_rust = s5.get("rust") or load_mat("/Game/Skyguard/Materials/M_MetalRust") or mats[1]
        m_conc = s5.get("concrete") or load_mat("/Game/Skyguard/Materials/M_Tex_concrete") or mats[1]
        m_brick = s5.get("brick") or load_mat("/Game/Skyguard/Materials/M_Tex_brick") or mats[0]
        m_plaster = s5.get("plaster") or unlit_w or mats[0]
        m_glass = s5.get("glass") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan") or mats[2 % nmat]
        m_muzzle_hot = s5.get("muzzle") or unlit_y or mats[0]
        m_foam = s5.get("foam") or unlit_w or mats[1]
        m_waterline = s5.get("waterline") or load_mat("/Game/Skyguard/Materials/Generated/M_L18_UnlitCyan") or mats[2 % nmat]
        m_glow = s5.get("glow") or m_muzzle_hot
        ...
        # Aircraft textured multi-mat
        if name in ("YakBeauty", "Prop", "PropNose", "Wide"):
            if sm_yak:
                spawn_sm(sm_yak, (bx + 3.55, cy - 0.2, cz - 0.4), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA5_MAT01_YakAnchor", m_air)
            else:
                spawn_sm(cube, (bx + 3.4, cy - 0.2, cz + 0.1), (0.12, 1.8, 0.7), None, PREFIX + "TA5_MAT01_AirframePanel", m_air)
            spawn_sm(cube, (bx + 3.45, cy + 0.35, cz + 0.35), (0.06, 0.95, 0.22), unreal.Rotator(0, 0, 8), PREFIX + "TA5_MAT01_MetalStrip", m_plate)
            spawn_sm(cube, (bx + 3.48, cy + 0.1, cz + 0.55), (0.05, 0.55, 0.35), None, PREFIX + "TA5_MAT01_PlateA", m_plate)
            spawn_sm(cube, (bx + 3.48, cy - 0.35, cz + 0.15), (0.05, 0.45, 0.28), None, PREFIX + "TA5_MAT01_PlateB", m_plate)
            spawn_sm(cube, (bx + 3.42, cy - 0.55, cz + 0.05), (0.05, 0.55, 0.12), None, PREFIX + "TA5_MAT01_RustBreak", m_rust)
            for k in range(5):
                spawn_sm(sphere, (bx + 3.5, cy - 0.25 + k * 0.2, cz + 0.7), (0.04, 0.04, 0.04), None, PREFIX + "TA5_MAT01_Rivet_%d" % k, m_plate)
            spawn_sm(sphere, (bx + 3.25, cy - 0.9, cz + 0.15), (0.07, 0.07, 0.07), None, PREFIX + "TA5_MAT04_ExhaustPin", m_glow)
            spawn_sm(sphere, (bx + 3.28, cy - 0.95, cz + 0.12), (0.05, 0.05, 0.05), None, PREFIX + "TA5_MAT04_ExhaustCore", m_muzzle_hot)
            if name == "Prop" and sm_prop:
                spawn_sm(sm_prop, (bx + 3.7, cy + 0.0, cz + 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 0, 20), PREFIX + "TA5_MAT01_PropDisc", m_plate)
                spawn_niagara(PREFIX + "TA5_VFX_PropWash", (bx + 3.65, cy + 0.0, cz + 0.05), "NS_PropWash", (0.35, 0.35, 0.35))

        # City textured multi-mat facade language
        if name in ("City", "Wide", "Harbor"):
            if sm_tower:
                spawn_sm(sm_tower, (bx + 3.6, cy + 1.6, cz - 1.0), (0.45, 0.45, 0.55), unreal.Rotator(0, -20, 0), PREFIX + "TA5_MAT02_ConcreteFacade", m_conc)
            else:
                spawn_sm(cube, (bx + 3.35, cy + 1.6, cz + 0.2), (0.1, 1.4, 2.2), None, PREFIX + "TA5_MAT02_ConcreteSlab", m_conc)
            spawn_sm(cube, (bx + 3.38, cy + 1.6, cz - 0.35), (0.06, 1.25, 0.1), None, PREFIX + "TA5_MAT02_ConcreteSill", m_plaster)
            spawn_sm(cube, (bx + 3.4, cy + 1.6, cz + 0.85), (0.05, 1.15, 0.08), None, PREFIX + "TA5_MAT02_PlasterLintel", m_plaster)
            spawn_sm(cube, (bx + 3.36, cy + 1.35, cz + 0.25), (0.04, 0.28, 0.35), None, PREFIX + "TA5_MAT02_GlassA", m_glass)
            spawn_sm(cube, (bx + 3.36, cy + 1.85, cz + 0.25), (0.04, 0.28, 0.35), None, PREFIX + "TA5_MAT02_GlassB", m_glass)
            if sm_apt:
                spawn_sm(sm_apt, (bx + 3.7, cy - 1.8, cz - 0.8), (0.4, 0.4, 0.5), unreal.Rotator(0, 15, 0), PREFIX + "TA5_MAT03_BrickFacade", m_brick)
            else:
                spawn_sm(cube, (bx + 3.4, cy - 1.8, cz + 0.1), (0.1, 1.1, 1.8), None, PREFIX + "TA5_MAT03_BrickSlab", m_brick)
            spawn_sm(cube, (bx + 3.42, cy - 1.8, cz + 0.75), (0.05, 1.05, 0.08), None, PREFIX + "TA5_MAT03_BrickHeader", m_brick)
            spawn_sm(cube, (bx + 3.43, cy - 1.8, cz - 0.2), (0.05, 0.9, 0.07), None, PREFIX + "TA5_MAT03_BrickCourse", m_brick)
            spawn_sm(cube, (bx + 3.44, cy - 1.5, cz + 0.2), (0.04, 0.2, 0.5), None, PREFIX + "TA5_MAT03_BrickQuoin", m_plaster)

        # Combat/ADS multi-Niagara + weapon readability
        if name in ("Combat", "ADS", "Cockpit"):
            if sm_rifle and name in ("ADS", "Cockpit"):
                spawn_sm(sm_rifle, (bx + 3.2, cy + 0.15, cz - 0.1), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA5_WPN_Rifle", load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate)
            if sm_glove and name in ("ADS", "Cockpit"):
                spawn_sm(sm_glove, (bx + 3.15, cy + 0.0, cz - 0.25), (0.3, 0.3, 0.3), unreal.Rotator(0, 90, 0), PREFIX + "TA5_WPN_Glove", load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_rust)
            spawn_niagara(PREFIX + "TA5_VFX01_Muzzle", (bx + 2.45, cy + 0.35, cz + 0.2), "NS_MuzzleFlash", (0.5, 0.5, 0.5))
            spawn_niagara(PREFIX + "TA5_VFX01_GunSmoke", (bx + 2.5, cy + 0.25, cz + 0.15), "NS_GunSmoke", (0.35, 0.35, 0.35))
            spawn_niagara(PREFIX + "TA5_VFX01_Tracer", (bx + 2.55, cy + 0.4, cz + 0.25), "NS_TracerBurst", (0.3, 0.3, 0.3))
            spawn_sm(sphere, (bx + 2.42, cy + 0.28, cz + 0.18), (0.12, 0.12, 0.12), None, PREFIX + "TA5_VFX01_CoreA", m_muzzle_hot)
            spawn_sm(sphere, (bx + 2.48, cy + 0.42, cz + 0.22), (0.08, 0.08, 0.08), None, PREFIX + "TA5_VFX01_CoreB", unlit_w or m_bright_white or mats[1])
            spawn_sm(cube, (bx + 2.4, cy + 0.35, cz + 0.2), (0.2, 0.03, 0.03), unreal.Rotator(0, -12, 0), PREFIX + "TA5_VFX01_FilamentA", m_muzzle_hot)
            spawn_sm(cube, (bx + 2.43, cy + 0.3, cz + 0.25), (0.14, 0.025, 0.025), unreal.Rotator(0, 10, 0), PREFIX + "TA5_VFX01_FilamentB", unlit_y or mats[1])
            spawn_sm(sphere, (bx + 2.55, cy + 0.2, cz + 0.1), (0.05, 0.05, 0.05), None, PREFIX + "TA5_VFX01_ShellHint", m_plate)

        # Ocean multi-Niagara + foam/waterline
        if name in ("Ocean", "Harbor"):
            spawn_niagara(PREFIX + "TA5_VFX02_OceanSpray", (bx + 3.3, cy - 0.5, cz - 0.7), "NS_OceanSpray", (0.65, 0.65, 0.65))
            spawn_niagara(PREFIX + "TA5_VFX02_WaterSplash", (bx + 3.35, cy - 0.2, cz - 0.75), "NS_WaterSplash", (0.45, 0.45, 0.45))
            spawn_sm(sphere, (bx + 3.25, cy - 0.55, cz - 0.62), (0.1, 0.1, 0.1), None, PREFIX + "TA5_VFX02_FoamA", m_foam)
            spawn_sm(sphere, (bx + 3.32, cy - 0.35, cz - 0.55), (0.07, 0.07, 0.07), None, PREFIX + "TA5_VFX02_FoamB", m_foam)
            spawn_sm(sphere, (bx + 3.28, cy - 0.65, cz - 0.5), (0.055, 0.055, 0.055), None, PREFIX + "TA5_VFX02_Droplet", unlit_y or mats[1])
            spawn_sm(cube, (bx + 3.22, cy - 0.45, cz - 0.85), (0.08, 0.75, 0.06), None, PREFIX + "TA5_VFX02_Waterline", m_waterline)
            for k in range(3):
                spawn_sm(sphere, (bx + 3.27, cy - 0.7 + k * 0.2, cz - 0.45), (0.04, 0.04, 0.04), None, PREFIX + "TA5_VFX02_Mist_%d" % k, m_foam)

        # Impact multi-Niagara + spark chain
        if name in ("City", "Combat", "Harbor"):
            spawn_niagara(PREFIX + "TA5_VFX03_HitSparks", (bx + 3.35, cy + 1.2, cz + 0.4), "NS_HitSparks", (0.55, 0.55, 0.55))
            spawn_niagara(PREFIX + "TA5_VFX03_Flak", (bx + 3.4, cy + 1.0, cz + 0.55), "NS_FlakBurst", (0.3, 0.3, 0.3))
            spawn_sm(sphere, (bx + 3.3, cy + 1.2, cz + 0.4), (0.1, 0.1, 0.1), None, PREFIX + "TA5_VFX03_SparkCore", m_muzzle_hot)
            for k in range(5):
                spawn_sm(sphere, (bx + 3.28, cy + 1.0 + k * 0.1, cz + 0.45 + k * 0.04), (0.04, 0.04, 0.04), None, PREFIX + "TA5_VFX03_Spark_%d" % k, m_muzzle_hot)
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop62 true-art slice05 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L62", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L62_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop62 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l61_freeze_true_art_slice05_textured_anr_once_cached\n")
...
def main():
    log("loop62 L61 freeze + true-art slice05 textured A/N/R once-cached + multi-niagara start")
    ...
    log("Loop62 complete stills=%d" % (len(saved) if saved else 0))
