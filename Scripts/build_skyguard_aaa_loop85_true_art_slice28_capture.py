...
PREFIX = "AAA_L85_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L85"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L85"
...
def ensure_slice19_vfx_library():
    """Slice19: deepen VFX library with more capture-visible Auth systems on L75 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice18_vfx_library()
    names = [
        "NS_L76_MuzzleBloom", "NS_L76_SparkChain", "NS_L76_ExplCore",
        "NS_L76_PropWashCore", "NS_L76_FoamRibbon", "NS_L76_TracerBloom",
        "NS_L76_GunSmokeCore", "NS_L76_FlakBloom", "NS_L76_CitySpark",
        "NS_L76_ContrailBloom", "NS_L76_ShellBurst", "NS_L76_HitFlash",
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log("slice19 vfx library count=%d" % len([k for k, v in out.items() if v]))
    return out



def ensure_slice20_vfx_library():
    """Slice20: deepen VFX library with more capture-visible Auth systems on L76 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice19_vfx_library()
    names = [
        "NS_L77_MuzzleCore", "NS_L77_SparkBurst", "NS_L77_ExplPlume",
        "NS_L77_PropWashBloom", "NS_L77_FoamBurst", "NS_L77_TracerStreak",
        "NS_L77_GunSmokeBloom", "NS_L77_FlakCore", "NS_L77_CityEmber",
        "NS_L77_ContrailCore", "NS_L77_ShellCore", "NS_L77_HitBloom",
        "NS_L77_DebrisField", "NS_L77_ExhaustGlow",
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log("slice20 vfx library count=%d" % len([k for k, v in out.items() if v]))
    return out



def ensure_slice21_vfx_library():
    """Slice21: deepen VFX library with more capture-visible Auth systems on L77 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice20_vfx_library()
    names = [
        "NS_L78_MuzzleHot", "NS_L78_SparkRing", "NS_L78_ExplBloom",
        "NS_L78_PropWashSheet", "NS_L78_FoamCrest", "NS_L78_TracerCore",
        "NS_L78_GunSmokeCore", "NS_L78_FlakBloom", "NS_L78_CitySpark",
        "NS_L78_ContrailBloom", "NS_L78_ShellBurst", "NS_L78_HitCore",
        "NS_L78_DebrisCore", "NS_L78_ExhaustCore", "NS_L78_MuzzleSmoke",
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log("slice21 vfx library count=%d" % len([k for k, v in out.items() if v]))
    return out



def ensure_slice22_vfx_library():
    """Slice22: deepen VFX library with more capture-visible Auth systems on L78 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice21_vfx_library()
    names = [
        "NS_L79_MuzzleCore", "NS_L79_SparkBurst", "NS_L79_ExplPlume",
        "NS_L79_PropWashBloom", "NS_L79_FoamBurst", "NS_L79_TracerStreak",
        "NS_L79_GunSmokeBloom", "NS_L79_FlakCore", "NS_L79_CityEmber",
        "NS_L79_ContrailCore", "NS_L79_ShellCore", "NS_L79_HitBloom",
        "NS_L79_DebrisField", "NS_L79_ExhaustGlow", "NS_L79_MuzzleFlashSoft",
        "NS_L79_OceanSprayCore",
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log("slice22 vfx library count=%d" % len([k for k, v in out.items() if v]))
    return out



def ensure_slice23_vfx_library():
    """Slice23: deepen VFX library with more capture-visible Auth systems on L79 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice22_vfx_library()
    names = [
        "NS_L80_MuzzleHot", "NS_L80_SparkRing", "NS_L80_ExplBloom",
        "NS_L80_PropWashSheet", "NS_L80_FoamCrest", "NS_L80_TracerCore",
        "NS_L80_GunSmokeCore", "NS_L80_FlakBloom", "NS_L80_CitySpark",
        "NS_L80_ContrailBloom", "NS_L80_ShellBurst", "NS_L80_HitCore",
        "NS_L80_DebrisCore", "NS_L80_ExhaustCore", "NS_L80_MuzzleSmoke",
        "NS_L80_OceanFoam",
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log("slice23 vfx library count=%d" % len([k for k, v in out.items() if v]))
    return out



def ensure_slice24_vfx_library():
    """Slice24: deepen VFX library with more capture-visible Auth systems on L80 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice23_vfx_library()
    names = [
        "NS_L81_MuzzleCore", "NS_L81_SparkBurst", "NS_L81_ExplPlume",
        "NS_L81_PropWashBloom", "NS_L81_FoamBurst", "NS_L81_TracerStreak",
        "NS_L81_GunSmokeBloom", "NS_L81_FlakCore", "NS_L81_CityEmber",
        "NS_L81_ContrailCore", "NS_L81_ShellCore", "NS_L81_HitBloom",
        "NS_L81_DebrisField", "NS_L81_ExhaustGlow", "NS_L81_MuzzleFlashSoft",
        "NS_L81_OceanSprayCore",
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log("slice24 vfx library count=%d" % len([k for k, v in out.items() if v]))
    return out



def ensure_slice25_vfx_library():
    """Slice25: deepen VFX library with more capture-visible Auth systems on L81 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice24_vfx_library()
    names = [
        'NS_L82_MuzzleCone', 'NS_L82_SparkSheet', 'NS_L82_ExplCore',
        'NS_L82_PropDiscWash', 'NS_L82_FoamCrestLite', 'NS_L82_TracerRibbon',
        'NS_L82_GunSmokeSheet', 'NS_L82_FlakBurst', 'NS_L82_CityGlow',
        'NS_L82_ContrailRibbon', 'NS_L82_ShellEject', 'NS_L82_HitSpark',
        'NS_L82_DebrisLite', 'NS_L82_ExhaustRibbon', 'NS_L82_MuzzleSpark',
        'NS_L82_OceanCrest',
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log('slice25 vfx library count=%d' % len([k for k, v in out.items() if v]))
    return out
...
def ensure_slice27_vfx_library():
    """Slice27: deepen VFX library with more capture-visible Auth systems on L83 freeze."""
    ensure_dir("/Game/Skyguard/VFX")
    ensure_dir("/Game/Skyguard/VFX/Emitters")
    base = ensure_slice26_vfx_library()
    names = [
        "NS_L84_MuzzleBloomLite", "NS_L84_SparkFilament", "NS_L84_ExplHalo",
        "NS_L84_PropDiscGlow", "NS_L84_FoamSheetLite", "NS_L84_TracerBloom",
        "NS_L84_GunSmokeWisp", "NS_L84_FlakCoreLite", "NS_L84_CityEmberLite",
        "NS_L84_ContrailMist", "NS_L84_ShellFlash", "NS_L84_HitCoreLite",
        "NS_L84_DebrisMist", "NS_L84_ExhaustMist", "NS_L84_MuzzleCoreSoft",
        "NS_L84_OceanMist",
    ]
    out = dict(base)
    for n in names:
        out[n] = ensure_authored_ns(n, deepen=True)
    log("slice27 vfx library count=%d" % len([k for k, v in out.items() if v]))
    return out
...
def densify():
    ...
    log("loop85 mat palette size=%d" % len(mats))
    ...
    # Slice19 (thin / capture-safe on L75 KEEP): Niagara + material push
    # HARD RULES: no FOV PointLight stacks; keep L52 HF densify core; behind-wall only; no L53 stacks.
    vfx19 = ensure_slice19_vfx_library()
    s19 = ensure_slice16_materials()
    m_air = s19.get("M_L70_AirframeResp") or s19.get("M_L68_AirframeANR")
    m_plate = s19.get("M_L70_PlateResp") or s19.get("M_L68_PlateANR")
    m_rust = s19.get("M_L70_RustResp") or s19.get("M_L68_RustANR")
    m_brick = s19.get("M_L70_BrickResp") or s19.get("M_L68_BrickANR")
    m_conc = s19.get("M_L70_ConcreteResp") or s19.get("M_L68_ConcreteANR")
    m_hot = s19.get("M_L70_MuzzleResp") or s19.get("M_L61_MuzzleHot")
    m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or m_hot
    m_disc = s19.get("M_L72_PropDiscBright") or s19.get("M_L71_PropDiscMotion") or load_mat("/Game/Skyguard/Materials/M_PropDisc")
    m_blade = s19.get("M_L72_PropBladeEdge") or s19.get("M_L71_PropBlade") or m_air
    m_muzzle_emi = s19.get("M_L71_VFX_MuzzleEmi") or m_hot
    m_spark_emi = s19.get("M_L71_VFX_SparkEmi") or m_hot
    m_tracer_emi = s19.get("M_L71_VFX_TracerEmi") or m_hot
    m_foam_emi = s19.get("M_L71_VFX_FoamEmi") or m_plate
    m_expl_emi = s19.get("M_L71_VFX_ExplEmi") or m_hot
    m_card_hot = s19.get("M_L72_VFX_CardHot") or m_muzzle_emi
    m_card_soft = s19.get("M_L72_VFX_CardSoft") or m_spark_emi
    m_card_cool = s19.get("M_L72_VFX_CardCool") or m_foam_emi
    m_card_expl = s19.get("M_L72_VFX_CardExpl") or m_expl_emi
    m_rifle = load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate
    m_leather = load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_plate
    m_glass = load_mat("/Game/Skyguard/Materials/M_CityGlass") or m_plate
    m_ocean = load_mat("/Game/Skyguard/Materials/M_L23_Ocean") or s19.get("M_L21_BrightOcean")
    m_sand = load_mat("/Game/Skyguard/Materials/M_L23_Beach") or s19.get("M_L21_BrightSand")
    m_asphalt = load_mat("/Game/Skyguard/Materials/M_L23_Asphalt") or m_conc
    m_metal = load_mat("/Game/Skyguard/Materials/M_Metal") or m_plate
    m_metal_rust = load_mat("/Game/Skyguard/Materials/M_MetalRust") or m_rust
    cube = load_sm("/Engine/BasicShapes/Cube")
    sphere = load_sm("/Engine/BasicShapes/Sphere")
    sm_prop = load_sm("/Game/Skyguard/Meshes/Hero/propeller_proxy")
    sm_yak = load_sm("/Game/Skyguard/Meshes/Hero/yak52_hd_proxy") or load_sm("/Game/Skyguard/Meshes/Hero/yak52_proxy")
    ...
    for i, (name, cam, dist, mat) in enumerate(stages):
        ...
        cy, cz = cam[1], cam[2]
        if name in ("Prop", "PropHub", "PropNose", "YakBeauty"):
            spawn_sm(sphere, (bx + 3.92, cy + 0.0, cz + 0.15), (1.25, 0.026, 1.25), None, PREFIX + "TA19_PropDiscHi_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 3.93, cy + 0.0, cz + 0.15), (0.7, 0.02, 0.7), None, PREFIX + "TA19_PropDiscMid_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 3.94, cy + 0.0, cz + 0.15), (0.32, 0.045, 0.32), None, PREFIX + "TA19_PropHub_%s" % name, mat=m_plate)
            for k in range(12):
                ang = k * 15.0
                spawn_sm(cube, (bx + 3.95, cy + 0.06 * ((k % 2) * 2 - 1), cz + 0.15), (0.016, 1.12, 0.03), unreal.Rotator(0, ang, 1 + k), PREFIX + "TA19_Blade_%s_%d" % (name, k), mat=m_blade)
            if sm_prop:
                spawn_sm(sm_prop, (bx + 3.98, cy + 0.0, cz + 0.12), (0.45, 0.45, 0.45), unreal.Rotator(0, 24, 16), PREFIX + "TA19_PropHero_%s" % name, mats=[m_blade, m_air, m_plate, m_disc])
            if sm_yak and name == "YakBeauty":
                spawn_sm(sm_yak, (bx + 3.74, cy - 0.02, cz - 0.2), (0.6, 0.6, 0.6), unreal.Rotator(0, 90, 0), PREFIX + "TA19_YakHero", mats=[m_air, m_plate, m_rust, m_glow])
                for k in range(10):
                    spawn_sm(cube, (bx + 3.76, cy - 0.6 + k * 0.12, cz - 0.04), (0.018, 0.1, 0.5), None, PREFIX + "TA19_Panel_%d" % k, mat=m_air if k % 2 == 0 else m_plate)
                for k in range(6):
                    spawn_sm(cube, (bx + 3.78, cy - 0.35 + k * 0.14, cz + 0.28), (0.012, 0.08, 0.04), None, PREFIX + "TA19_Seam_%d" % k, mat=m_metal)
            spawn_niagara(PREFIX + "TA19_NS_WashCore_%s" % name, (bx + 3.9, cy + 0.02, cz + 0.03), "NS_L76_PropWashCore", (0.13, 0.13, 0.13), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_Wash_%s" % name, (bx + 3.91, cy + 0.03, cz + 0.02), "NS_PropWash", (0.1, 0.1, 0.1), bound=True)
            spawn_particle_field(PREFIX + "TA19_WashField_%s" % name, (bx + 3.91, cy + 0.0, cz + 0.08), m_card_soft, count=14, radius=0.52, size=0.024, label_base="Wash")
            if name in ("YakBeauty", "Prop"):
                spawn_niagara(PREFIX + "TA19_NS_Contrail_%s" % name, (bx + 4.0, cy + 0.05, cz + 0.0), "NS_L76_ContrailBloom", (0.11, 0.11, 0.11), bound=True)
            spawn_sm(sphere, (bx + 3.99, cy - 0.2, cz - 0.01), (0.04, 0.04, 0.04), None, PREFIX + "TA19_Exhaust_%s" % name, mat=m_muzzle_emi)
        if name in ("YakBeauty", "Cockpit", "ADS", "Prop"):
            spawn_sm(cube, (bx + 3.66, cy + 0.12, cz + 0.08), (0.032, 1.28, 0.7), None, PREFIX + "TA19_Air_%s" % name, mat=m_air)
            spawn_sm(cube, (bx + 3.7, cy - 0.38, cz + 0.0), (0.028, 0.92, 0.46), None, PREFIX + "TA19_Plate_%s" % name, mat=m_plate)
            spawn_sm(cube, (bx + 3.72, cy + 0.52, cz - 0.1), (0.022, 0.5, 0.32), None, PREFIX + "TA19_Rust_%s" % name, mat=m_metal_rust)
            for k in range(14):
                spawn_sm(sphere, (bx + 3.71, cy - 0.75 + k * 0.11, cz + 0.38), (0.016, 0.016, 0.016), None, PREFIX + "TA19_Rivet_%s_%d" % (name, k), mat=m_plate)
            if name in ("ADS", "Cockpit"):
                for k in range(10):
                    spawn_sm(cube, (bx + 1.2, cy - 0.35 + k * 0.08, cz + 0.03), (0.007, 0.06, 0.016), None, PREFIX + "TA19_HFStripe_%s_%d" % (name, k), mat=m_card_soft or m_plate)
                if name == "ADS" and sm_rifle_m:
                    spawn_sm(sm_rifle_m, (bx + 3.4, cy + 0.02, cz - 0.02), (0.38, 0.38, 0.38), unreal.Rotator(0, 90, 0), PREFIX + "TA19_Rifle", mats=[m_rifle, m_plate, m_rust, m_hot])
                    spawn_sm(cube, (bx + 3.42, cy - 0.08, cz - 0.08), (0.035, 0.07, 0.045), None, PREFIX + "TA19_Glove", mat=m_leather)
                    spawn_niagara(PREFIX + "TA19_NS_HitFlash", (bx + 3.55, cy + 0.2, cz + 0.1), "NS_L76_HitFlash", (0.1, 0.1, 0.1), bound=True)
        if name in ("Combat", "ADS", "Wide"):
            spawn_niagara(PREFIX + "TA19_NS_MuzzleBloom_%s" % name, (bx + 3.54, cy + 0.12, cz + 0.05), "NS_L76_MuzzleBloom", (0.11, 0.11, 0.11), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_Muzzle_%s" % name, (bx + 3.55, cy + 0.13, cz + 0.06), "NS_MuzzleFlash", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_SmokeCore_%s" % name, (bx + 3.56, cy + 0.08, cz + 0.02), "NS_L76_GunSmokeCore", (0.11, 0.11, 0.11), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_SparkChain_%s" % name, (bx + 3.64, cy + 0.94, cz + 0.25), "NS_L76_SparkChain", (0.12, 0.12, 0.12), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_Sparks_%s" % name, (bx + 3.65, cy + 0.95, cz + 0.26), "NS_HitSparks", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_TracerBloom_%s" % name, (bx + 3.59, cy + 0.16, cz + 0.1), "NS_L76_TracerBloom", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_ShellBurst_%s" % name, (bx + 3.5, cy + 0.0, cz - 0.02), "NS_L76_ShellBurst", (0.1, 0.1, 0.1), bound=True)
            spawn_particle_field(PREFIX + "TA19_MuzzleField_%s" % name, (bx + 3.55, cy + 0.14, cz + 0.07), m_card_hot, count=16, radius=0.2, size=0.026, label_base="Muz")
            spawn_burst_ring(PREFIX + "TA19_SparkRing_%s" % name, (bx + 3.64, cy + 0.88, cz + 0.24), m_card_soft, count=14, radius=0.34, label_base="Spark")
            for k in range(10):
                spawn_sm(cube, (bx + 3.48 + k * 0.025, cy + 0.08 + k * 0.035, cz + 0.02 + k * 0.02), (0.12 - k * 0.008, 0.01, 0.01), unreal.Rotator(0, -8 + k * 4, 0), PREFIX + "TA19_Filament_%s_%d" % (name, k), mat=m_tracer_emi)
            if name == "Combat":
                spawn_niagara(PREFIX + "TA19_NS_ExplCore", (bx + 3.7, cy - 0.82, cz + 0.49), "NS_L76_ExplCore", (0.14, 0.14, 0.14), bound=True)
                spawn_niagara(PREFIX + "TA19_NS_Expl", (bx + 3.71, cy - 0.84, cz + 0.5), "NS_DroneExplosion", (0.12, 0.12, 0.12), bound=True)
                spawn_niagara(PREFIX + "TA19_NS_FlakBloom", (bx + 3.68, cy + 1.04, cz + 0.44), "NS_L76_FlakBloom", (0.11, 0.11, 0.11), bound=True)
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.72, cy - 0.86, cz + 0.46), (0.36, 0.36, 0.36), unreal.Rotator(0, 90, 2), PREFIX + "TA19_Shahed", mats=[m_plate, m_air, m_rust, m_hot])
                spawn_particle_field(PREFIX + "TA19_ExplField", (bx + 3.7, cy - 0.82, cz + 0.52), m_card_expl, count=18, radius=0.48, size=0.03, label_base="Expl")
                spawn_burst_ring(PREFIX + "TA19_ExplRing", (bx + 3.7, cy - 0.82, cz + 0.48), m_card_hot, count=16, radius=0.55, label_base="ExplR")
                for k in range(14):
                    spawn_sm(sphere, (bx + 3.64, cy - 1.05 + k * 0.04, cz + 0.36 + (k % 3) * 0.028), (0.018, 0.018, 0.018), None, PREFIX + "TA19_Debris_%d" % k, mat=m_spark_emi if k % 2 == 0 else m_plate)
        if name in ("City", "Harbor", "Wide"):
            spawn_sm(cube, (bx + 3.68, cy + 0.2, cz - 0.22), (0.04, 1.1, 1.3), None, PREFIX + "TA19_Brick_%s" % name, mat=m_brick)
            spawn_sm(cube, (bx + 3.72, cy - 0.85, cz - 0.04), (0.035, 0.9, 1.05), None, PREFIX + "TA19_Conc_%s" % name, mat=m_conc)
            spawn_sm(cube, (bx + 3.74, cy - 0.08, cz - 0.52), (0.028, 1.35, 0.07), None, PREFIX + "TA19_Asphalt_%s" % name, mat=m_asphalt)
            for ix in range(4):
                for iy in range(5):
                    spawn_sm(cube, (bx + 3.69, cy - 0.35 + ix * 0.26, cz - 0.18 + iy * 0.24), (0.016, 0.1, 0.12), None, PREFIX + "TA19_Win_%s_%d_%d" % (name, ix, iy), mat=m_glass)
            if name == "City":
                spawn_niagara(PREFIX + "TA19_NS_CitySpark", (bx + 3.74, cy + 0.32, cz + 0.68), "NS_L76_CitySpark", (0.12, 0.12, 0.12), bound=True)
                spawn_particle_field(PREFIX + "TA19_Ember", (bx + 3.74, cy + 0.32, cz + 0.74), m_card_hot, count=14, radius=0.3, size=0.022, label_base="Ember")
        if name in ("Ocean", "Harbor"):
            spawn_sm(cube, (bx + 3.64, cy - 0.38, cz - 0.68), (0.035, 1.55, 0.1), None, PREFIX + "TA19_Beach_%s" % name, mat=m_sand)
            spawn_sm(cube, (bx + 3.66, cy + 0.18, cz - 0.76), (0.045, 1.75, 0.18), None, PREFIX + "TA19_Ocean_%s" % name, mat=m_ocean)
            spawn_niagara(PREFIX + "TA19_NS_FoamRibbon_%s" % name, (bx + 3.62, cy - 0.14, cz - 0.48), "NS_L76_FoamRibbon", (0.15, 0.15, 0.15), bound=True)
            spawn_niagara(PREFIX + "TA19_NS_Spray_%s" % name, (bx + 3.63, cy - 0.14, cz - 0.5), "NS_OceanSpray", (0.13, 0.13, 0.13), bound=True)
            spawn_particle_field(PREFIX + "TA19_FoamField_%s" % name, (bx + 3.62, cy - 0.28, cz - 0.4), m_card_cool, count=16, radius=0.52, size=0.03, label_base="Foam")
            for k in range(14):
                spawn_sm(sphere, (bx + 3.6, cy - 0.8 + k * 0.075, cz - 0.38 + (k % 2) * 0.03), (0.035, 0.035, 0.035), None, PREFIX + "TA19_Foam_%s_%d" % (name, k), mat=m_foam_emi)
    log("slice19 niagara + material push densify applied (no extra point lights)")

    # Slice20 (thin / capture-safe on L76 KEEP): Niagara + material push
    # HARD RULES: no FOV PointLight stacks; keep L52 HF densify core; behind-wall only; no L53 stacks.
    vfx20 = ensure_slice20_vfx_library()
    s20 = ensure_slice16_materials()
    m_air = s20.get("M_L70_AirframeResp") or s20.get("M_L68_AirframeANR")
    m_plate = s20.get("M_L70_PlateResp") or s20.get("M_L68_PlateANR")
    m_rust = s20.get("M_L70_RustResp") or s20.get("M_L68_RustANR")
    m_brick = s20.get("M_L70_BrickResp") or s20.get("M_L68_BrickANR")
    m_conc = s20.get("M_L70_ConcreteResp") or s20.get("M_L68_ConcreteANR")
    m_hot = s20.get("M_L70_MuzzleResp") or s20.get("M_L61_MuzzleHot")
    m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or m_hot
    m_disc = s20.get("M_L72_PropDiscBright") or s20.get("M_L71_PropDiscMotion") or load_mat("/Game/Skyguard/Materials/M_PropDisc")
    m_blade = s20.get("M_L72_PropBladeEdge") or s20.get("M_L71_PropBlade") or m_air
    m_muzzle_emi = s20.get("M_L71_VFX_MuzzleEmi") or m_hot
    m_spark_emi = s20.get("M_L71_VFX_SparkEmi") or m_hot
    m_tracer_emi = s20.get("M_L71_VFX_TracerEmi") or m_hot
    m_foam_emi = s20.get("M_L71_VFX_FoamEmi") or m_plate
    m_expl_emi = s20.get("M_L71_VFX_ExplEmi") or m_hot
    m_card_hot = s20.get("M_L72_VFX_CardHot") or m_muzzle_emi
    m_card_soft = s20.get("M_L72_VFX_CardSoft") or m_spark_emi
    m_card_cool = s20.get("M_L72_VFX_CardCool") or m_foam_emi
    m_card_expl = s20.get("M_L72_VFX_CardExpl") or m_expl_emi
    m_rifle = load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate
    m_leather = load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_plate
    m_glass = load_mat("/Game/Skyguard/Materials/M_CityGlass") or m_plate
    m_ocean = load_mat("/Game/Skyguard/Materials/M_L23_Ocean") or s20.get("M_L21_BrightOcean")
    m_sand = load_mat("/Game/Skyguard/Materials/M_L23_Beach") or s20.get("M_L21_BrightSand")
    m_asphalt = load_mat("/Game/Skyguard/Materials/M_L23_Asphalt") or m_conc
    m_metal = load_mat("/Game/Skyguard/Materials/M_Metal") or m_plate
    m_metal_rust = load_mat("/Game/Skyguard/Materials/M_MetalRust") or m_rust
    cube = load_sm("/Engine/BasicShapes/Cube")
    ...
    for i, (name, cam, dist, mat) in enumerate(stages):
        ...
        cy, cz = cam[1], cam[2]
        if name in ("Prop", "PropHub", "PropNose", "YakBeauty"):
            spawn_sm(sphere, (bx + 3.96, cy + 0.0, cz + 0.16), (1.28, 0.024, 1.28), None, PREFIX + "TA20_PropDiscHi_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 3.97, cy + 0.0, cz + 0.16), (0.75, 0.018, 0.75), None, PREFIX + "TA20_PropDiscMid_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 3.98, cy + 0.0, cz + 0.16), (0.34, 0.04, 0.34), None, PREFIX + "TA20_PropHub_%s" % name, mat=m_plate)
            for k in range(12):
                ang = k * 15.0
                spawn_sm(cube, (bx + 3.99, cy + 0.055 * ((k % 2) * 2 - 1), cz + 0.16), (0.015, 1.15, 0.028), unreal.Rotator(0, ang, 1 + k), PREFIX + "TA20_Blade_%s_%d" % (name, k), mat=m_blade)
            if sm_prop:
                spawn_sm(sm_prop, (bx + 4.02, cy + 0.0, cz + 0.13), (0.44, 0.44, 0.44), unreal.Rotator(0, 28, 12), PREFIX + "TA20_PropHero_%s" % name, mats=[m_blade, m_air, m_plate, m_disc])
            if sm_yak and name == "YakBeauty":
                spawn_sm(sm_yak, (bx + 3.76, cy - 0.02, cz - 0.18), (0.58, 0.58, 0.58), unreal.Rotator(0, 90, 0), PREFIX + "TA20_YakHero", mats=[m_air, m_plate, m_rust, m_glow])
                for k in range(10):
                    spawn_sm(cube, (bx + 3.78, cy - 0.58 + k * 0.12, cz - 0.03), (0.016, 0.1, 0.48), None, PREFIX + "TA20_Panel_%d" % k, mat=m_air if k % 2 == 0 else m_plate)
                for k in range(8):
                    spawn_sm(cube, (bx + 3.8, cy - 0.4 + k * 0.12, cz + 0.26), (0.01, 0.07, 0.035), None, PREFIX + "TA20_Seam_%d" % k, mat=m_metal)
            spawn_niagara(PREFIX + "TA20_NS_WashBloom_%s" % name, (bx + 3.94, cy + 0.02, cz + 0.03), "NS_L77_PropWashBloom", (0.12, 0.12, 0.12), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_Wash_%s" % name, (bx + 3.95, cy + 0.03, cz + 0.02), "NS_PropWash", (0.09, 0.09, 0.09), bound=True)
            spawn_particle_field(PREFIX + "TA20_WashField_%s" % name, (bx + 3.95, cy + 0.0, cz + 0.08), m_card_soft, count=14, radius=0.5, size=0.022, label_base="Wash")
            if name in ("YakBeauty", "Prop"):
                spawn_niagara(PREFIX + "TA20_NS_Contrail_%s" % name, (bx + 4.04, cy + 0.05, cz + 0.0), "NS_L77_ContrailCore", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_Exhaust_%s" % name, (bx + 4.01, cy - 0.18, cz - 0.01), "NS_L77_ExhaustGlow", (0.1, 0.1, 0.1), bound=True)
            spawn_sm(sphere, (bx + 4.01, cy - 0.18, cz - 0.01), (0.038, 0.038, 0.038), None, PREFIX + "TA20_Exhaust_%s" % name, mat=m_muzzle_emi)
        if name in ("YakBeauty", "Cockpit", "ADS", "Prop"):
            spawn_sm(cube, (bx + 3.7, cy + 0.12, cz + 0.08), (0.03, 1.26, 0.68), None, PREFIX + "TA20_Air_%s" % name, mat=m_air)
            spawn_sm(cube, (bx + 3.74, cy - 0.36, cz + 0.0), (0.026, 0.9, 0.44), None, PREFIX + "TA20_Plate_%s" % name, mat=m_plate)
            spawn_sm(cube, (bx + 3.76, cy + 0.5, cz - 0.1), (0.02, 0.48, 0.3), None, PREFIX + "TA20_Rust_%s" % name, mat=m_metal_rust)
            for k in range(14):
                spawn_sm(sphere, (bx + 3.75, cy - 0.72 + k * 0.105, cz + 0.36), (0.015, 0.015, 0.015), None, PREFIX + "TA20_Rivet_%s_%d" % (name, k), mat=m_plate)
            if name in ("ADS", "Cockpit"):
                for k in range(10):
                    spawn_sm(cube, (bx + 1.22, cy - 0.34 + k * 0.075, cz + 0.03), (0.006, 0.055, 0.014), None, PREFIX + "TA20_HFStripe_%s_%d" % (name, k), mat=m_card_soft or m_plate)
                if name == "ADS" and sm_rifle_m:
                    spawn_sm(sm_rifle_m, (bx + 3.42, cy + 0.02, cz - 0.02), (0.36, 0.36, 0.36), unreal.Rotator(0, 90, 0), PREFIX + "TA20_Rifle", mats=[m_rifle, m_plate, m_rust, m_hot])
                    spawn_sm(cube, (bx + 3.44, cy - 0.08, cz - 0.08), (0.032, 0.065, 0.04), None, PREFIX + "TA20_Glove", mat=m_leather)
                    spawn_niagara(PREFIX + "TA20_NS_HitBloom", (bx + 3.57, cy + 0.18, cz + 0.1), "NS_L77_HitBloom", (0.09, 0.09, 0.09), bound=True)
        if name in ("Combat", "ADS", "Wide"):
            spawn_niagara(PREFIX + "TA20_NS_MuzzleCore_%s" % name, (bx + 3.56, cy + 0.11, cz + 0.05), "NS_L77_MuzzleCore", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_Muzzle_%s" % name, (bx + 3.57, cy + 0.12, cz + 0.06), "NS_MuzzleFlash", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_SmokeBloom_%s" % name, (bx + 3.58, cy + 0.07, cz + 0.02), "NS_L77_GunSmokeBloom", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_SparkBurst_%s" % name, (bx + 3.66, cy + 0.92, cz + 0.24), "NS_L77_SparkBurst", (0.11, 0.11, 0.11), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_Sparks_%s" % name, (bx + 3.67, cy + 0.93, cz + 0.25), "NS_HitSparks", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_TracerStreak_%s" % name, (bx + 3.61, cy + 0.15, cz + 0.09), "NS_L77_TracerStreak", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_ShellCore_%s" % name, (bx + 3.52, cy + 0.0, cz - 0.02), "NS_L77_ShellCore", (0.09, 0.09, 0.09), bound=True)
            spawn_particle_field(PREFIX + "TA20_MuzzleField_%s" % name, (bx + 3.57, cy + 0.13, cz + 0.06), m_card_hot, count=16, radius=0.18, size=0.024, label_base="Muz")
            spawn_burst_ring(PREFIX + "TA20_SparkRing_%s" % name, (bx + 3.66, cy + 0.86, cz + 0.23), m_card_soft, count=14, radius=0.32, label_base="Spark")
            for k in range(10):
                spawn_sm(cube, (bx + 3.5 + k * 0.022, cy + 0.07 + k * 0.032, cz + 0.02 + k * 0.018), (0.11 - k * 0.007, 0.009, 0.009), unreal.Rotator(0, -7 + k * 4, 0), PREFIX + "TA20_Filament_%s_%d" % (name, k), mat=m_tracer_emi)
            if name == "Combat":
                spawn_niagara(PREFIX + "TA20_NS_ExplPlume", (bx + 3.72, cy - 0.8, cz + 0.48), "NS_L77_ExplPlume", (0.13, 0.13, 0.13), bound=True)
                spawn_niagara(PREFIX + "TA20_NS_Expl", (bx + 3.73, cy - 0.82, cz + 0.49), "NS_DroneExplosion", (0.11, 0.11, 0.11), bound=True)
                spawn_niagara(PREFIX + "TA20_NS_FlakCore", (bx + 3.7, cy + 1.02, cz + 0.43), "NS_L77_FlakCore", (0.1, 0.1, 0.1), bound=True)
                spawn_niagara(PREFIX + "TA20_NS_Debris", (bx + 3.71, cy - 0.9, cz + 0.4), "NS_L77_DebrisField", (0.12, 0.12, 0.12), bound=True)
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.74, cy - 0.84, cz + 0.45), (0.34, 0.34, 0.34), unreal.Rotator(0, 90, 2), PREFIX + "TA20_Shahed", mats=[m_plate, m_air, m_rust, m_hot])
                spawn_particle_field(PREFIX + "TA20_ExplField", (bx + 3.72, cy - 0.8, cz + 0.51), m_card_expl, count=18, radius=0.46, size=0.028, label_base="Expl")
                spawn_burst_ring(PREFIX + "TA20_ExplRing", (bx + 3.72, cy - 0.8, cz + 0.47), m_card_hot, count=16, radius=0.52, label_base="ExplR")
                for k in range(14):
                    spawn_sm(sphere, (bx + 3.66, cy - 1.0 + k * 0.038, cz + 0.34 + (k % 3) * 0.026), (0.016, 0.016, 0.016), None, PREFIX + "TA20_Debris_%d" % k, mat=m_spark_emi if k % 2 == 0 else m_plate)
        if name in ("City", "Harbor", "Wide"):
            spawn_sm(cube, (bx + 3.72, cy + 0.18, cz - 0.2), (0.038, 1.08, 1.25), None, PREFIX + "TA20_Brick_%s" % name, mat=m_brick)
            spawn_sm(cube, (bx + 3.76, cy - 0.82, cz - 0.03), (0.032, 0.88, 1.0), None, PREFIX + "TA20_Conc_%s" % name, mat=m_conc)
            spawn_sm(cube, (bx + 3.78, cy - 0.06, cz - 0.5), (0.026, 1.3, 0.065), None, PREFIX + "TA20_Asphalt_%s" % name, mat=m_asphalt)
            for ix in range(4):
                for iy in range(5):
                    spawn_sm(cube, (bx + 3.73, cy - 0.32 + ix * 0.25, cz - 0.16 + iy * 0.23), (0.014, 0.095, 0.11), None, PREFIX + "TA20_Win_%s_%d_%d" % (name, ix, iy), mat=m_glass)
            if name == "City":
                spawn_niagara(PREFIX + "TA20_NS_CityEmber", (bx + 3.78, cy + 0.3, cz + 0.66), "NS_L77_CityEmber", (0.11, 0.11, 0.11), bound=True)
                spawn_particle_field(PREFIX + "TA20_Ember", (bx + 3.78, cy + 0.3, cz + 0.72), m_card_hot, count=14, radius=0.28, size=0.02, label_base="Ember")
        if name in ("Ocean", "Harbor"):
            spawn_sm(cube, (bx + 3.68, cy - 0.36, cz - 0.66), (0.032, 1.5, 0.09), None, PREFIX + "TA20_Beach_%s" % name, mat=m_sand)
            spawn_sm(cube, (bx + 3.7, cy + 0.16, cz - 0.74), (0.042, 1.7, 0.16), None, PREFIX + "TA20_Ocean_%s" % name, mat=m_ocean)
            spawn_niagara(PREFIX + "TA20_NS_FoamBurst_%s" % name, (bx + 3.66, cy - 0.12, cz - 0.46), "NS_L77_FoamBurst", (0.14, 0.14, 0.14), bound=True)
            spawn_niagara(PREFIX + "TA20_NS_Spray_%s" % name, (bx + 3.67, cy - 0.12, cz - 0.48), "NS_OceanSpray", (0.12, 0.12, 0.12), bound=True)
            spawn_particle_field(PREFIX + "TA20_FoamField_%s" % name, (bx + 3.66, cy - 0.26, cz - 0.38), m_card_cool, count=16, radius=0.5, size=0.028, label_base="Foam")
            for k in range(14):
                spawn_sm(sphere, (bx + 3.64, cy - 0.76 + k * 0.07, cz - 0.36 + (k % 2) * 0.028), (0.032, 0.032, 0.032), None, PREFIX + "TA20_Foam_%s_%d" % (name, k), mat=m_foam_emi)
    log("slice20 niagara + material push densify applied (no extra point lights)")

    # Slice21 (thin / capture-safe on L77 KEEP): Niagara + material push
    # HARD RULES: no FOV PointLight stacks; keep L52 HF densify core; behind-wall only; no L53 stacks.
    vfx21 = ensure_slice21_vfx_library()
    s21 = ensure_slice16_materials()
    m_air = s21.get("M_L70_AirframeResp") or s21.get("M_L68_AirframeANR")
    m_plate = s21.get("M_L70_PlateResp") or s21.get("M_L68_PlateANR")
    m_rust = s21.get("M_L70_RustResp") or s21.get("M_L68_RustANR")
    m_brick = s21.get("M_L70_BrickResp") or s21.get("M_L68_BrickANR")
    m_conc = s21.get("M_L70_ConcreteResp") or s21.get("M_L68_ConcreteANR")
    m_hot = s21.get("M_L70_MuzzleResp") or s21.get("M_L61_MuzzleHot")
    m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or m_hot
    m_disc = s21.get("M_L72_PropDiscBright") or s21.get("M_L71_PropDiscMotion") or load_mat("/Game/Skyguard/Materials/M_PropDisc")
    m_blade = s21.get("M_L72_PropBladeEdge") or s21.get("M_L71_PropBlade") or m_air
    m_muzzle_emi = s21.get("M_L71_VFX_MuzzleEmi") or m_hot
    m_spark_emi = s21.get("M_L71_VFX_SparkEmi") or m_hot
    m_tracer_emi = s21.get("M_L71_VFX_TracerEmi") or m_hot
    m_foam_emi = s21.get("M_L71_VFX_FoamEmi") or m_plate
    m_expl_emi = s21.get("M_L71_VFX_ExplEmi") or m_hot
    m_card_hot = s21.get("M_L72_VFX_CardHot") or m_muzzle_emi
    m_card_soft = s21.get("M_L72_VFX_CardSoft") or m_spark_emi
    m_card_cool = s21.get("M_L72_VFX_CardCool") or m_foam_emi
    m_card_expl = s21.get("M_L72_VFX_CardExpl") or m_expl_emi
    m_rifle = load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate
    m_leather = load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_plate
    m_glass = load_mat("/Game/Skyguard/Materials/M_CityGlass") or m_plate
    m_ocean = load_mat("/Game/Skyguard/Materials/M_L23_Ocean") or s21.get("M_L21_BrightOcean")
    m_sand = load_mat("/Game/Skyguard/Materials/M_L23_Beach") or s21.get("M_L21_BrightSand")
    m_asphalt = load_mat("/Game/Skyguard/Materials/M_L23_Asphalt") or m_conc
    m_metal = load_mat("/Game/Skyguard/Materials/M_Metal") or m_plate
    m_metal_rust = load_mat("/Game/Skyguard/Materials/M_MetalRust") or m_rust
    cube = load_sm("/Engine/BasicShapes/Cube")
    ...
    for i, (name, cam, dist, mat) in enumerate(stages):
        ...
        cy, cz = cam[1], cam[2]
        if name in ("Prop", "PropHub", "PropNose", "YakBeauty"):
            spawn_sm(sphere, (bx + 4.0, cy + 0.0, cz + 0.17), (1.3, 0.022, 1.3), None, PREFIX + "TA21_PropDiscHi_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.01, cy + 0.0, cz + 0.17), (0.78, 0.016, 0.78), None, PREFIX + "TA21_PropDiscMid_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.02, cy + 0.0, cz + 0.17), (0.36, 0.038, 0.36), None, PREFIX + "TA21_PropHub_%s" % name, mat=m_plate)
            for k in range(12):
                ang = k * 15.0
                spawn_sm(cube, (bx + 4.03, cy + 0.05 * ((k % 2) * 2 - 1), cz + 0.17), (0.014, 1.18, 0.026), unreal.Rotator(0, ang, 1 + k), PREFIX + "TA21_Blade_%s_%d" % (name, k), mat=m_blade)
            if sm_prop:
                spawn_sm(sm_prop, (bx + 4.06, cy + 0.0, cz + 0.14), (0.43, 0.43, 0.43), unreal.Rotator(0, 32, 10), PREFIX + "TA21_PropHero_%s" % name, mats=[m_blade, m_air, m_plate, m_disc])
            if sm_yak and name == "YakBeauty":
                spawn_sm(sm_yak, (bx + 3.78, cy - 0.02, cz - 0.16), (0.56, 0.56, 0.56), unreal.Rotator(0, 90, 0), PREFIX + "TA21_YakHero", mats=[m_air, m_plate, m_rust, m_glow])
                for k in range(10):
                    spawn_sm(cube, (bx + 3.8, cy - 0.56 + k * 0.115, cz - 0.02), (0.015, 0.095, 0.46), None, PREFIX + "TA21_Panel_%d" % k, mat=m_air if k % 2 == 0 else m_plate)
                for k in range(8):
                    spawn_sm(cube, (bx + 3.82, cy - 0.38 + k * 0.115, cz + 0.25), (0.009, 0.065, 0.032), None, PREFIX + "TA21_Seam_%d" % k, mat=m_metal)
            spawn_niagara(PREFIX + "TA21_NS_WashSheet_%s" % name, (bx + 3.98, cy + 0.02, cz + 0.03), "NS_L78_PropWashSheet", (0.11, 0.11, 0.11), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_Wash_%s" % name, (bx + 3.99, cy + 0.03, cz + 0.02), "NS_PropWash", (0.08, 0.08, 0.08), bound=True)
            spawn_particle_field(PREFIX + "TA21_WashField_%s" % name, (bx + 3.99, cy + 0.0, cz + 0.08), m_card_soft, count=12, radius=0.48, size=0.02, label_base="Wash")
            if name in ("YakBeauty", "Prop"):
                spawn_niagara(PREFIX + "TA21_NS_Contrail_%s" % name, (bx + 4.08, cy + 0.05, cz + 0.0), "NS_L78_ContrailBloom", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_Exhaust_%s" % name, (bx + 4.05, cy - 0.16, cz - 0.01), "NS_L78_ExhaustCore", (0.09, 0.09, 0.09), bound=True)
            spawn_sm(sphere, (bx + 4.05, cy - 0.16, cz - 0.01), (0.035, 0.035, 0.035), None, PREFIX + "TA21_Exhaust_%s" % name, mat=m_muzzle_emi)
        if name in ("YakBeauty", "Cockpit", "ADS", "Prop"):
            spawn_sm(cube, (bx + 3.74, cy + 0.12, cz + 0.08), (0.028, 1.24, 0.66), None, PREFIX + "TA21_Air_%s" % name, mat=m_air)
            spawn_sm(cube, (bx + 3.78, cy - 0.34, cz + 0.0), (0.024, 0.88, 0.42), None, PREFIX + "TA21_Plate_%s" % name, mat=m_plate)
            spawn_sm(cube, (bx + 3.8, cy + 0.48, cz - 0.1), (0.018, 0.46, 0.28), None, PREFIX + "TA21_Rust_%s" % name, mat=m_metal_rust)
            for k in range(12):
                spawn_sm(sphere, (bx + 3.79, cy - 0.7 + k * 0.1, cz + 0.34), (0.014, 0.014, 0.014), None, PREFIX + "TA21_Rivet_%s_%d" % (name, k), mat=m_plate)
            if name in ("ADS", "Cockpit"):
                for k in range(10):
                    spawn_sm(cube, (bx + 1.24, cy - 0.33 + k * 0.07, cz + 0.03), (0.005, 0.05, 0.012), None, PREFIX + "TA21_HFStripe_%s_%d" % (name, k), mat=m_card_soft or m_plate)
                if name == "ADS" and sm_rifle_m:
                    spawn_sm(sm_rifle_m, (bx + 3.44, cy + 0.02, cz - 0.02), (0.35, 0.35, 0.35), unreal.Rotator(0, 90, 0), PREFIX + "TA21_Rifle", mats=[m_rifle, m_plate, m_rust, m_hot])
                    spawn_sm(cube, (bx + 3.46, cy - 0.08, cz - 0.08), (0.03, 0.06, 0.038), None, PREFIX + "TA21_Glove", mat=m_leather)
                    spawn_niagara(PREFIX + "TA21_NS_HitCore", (bx + 3.59, cy + 0.17, cz + 0.1), "NS_L78_HitCore", (0.08, 0.08, 0.08), bound=True)
        if name in ("Combat", "ADS", "Wide"):
            spawn_niagara(PREFIX + "TA21_NS_MuzzleHot_%s" % name, (bx + 3.58, cy + 0.1, cz + 0.05), "NS_L78_MuzzleHot", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_Muzzle_%s" % name, (bx + 3.59, cy + 0.11, cz + 0.06), "NS_MuzzleFlash", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_MuzzleSmoke_%s" % name, (bx + 3.6, cy + 0.08, cz + 0.03), "NS_L78_MuzzleSmoke", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_SmokeCore_%s" % name, (bx + 3.6, cy + 0.06, cz + 0.02), "NS_L78_GunSmokeCore", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_SparkRing_%s" % name, (bx + 3.68, cy + 0.9, cz + 0.23), "NS_L78_SparkRing", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_Sparks_%s" % name, (bx + 3.69, cy + 0.91, cz + 0.24), "NS_HitSparks", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_TracerCore_%s" % name, (bx + 3.63, cy + 0.14, cz + 0.08), "NS_L78_TracerCore", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_ShellBurst_%s" % name, (bx + 3.54, cy + 0.0, cz - 0.02), "NS_L78_ShellBurst", (0.08, 0.08, 0.08), bound=True)
            spawn_particle_field(PREFIX + "TA21_MuzzleField_%s" % name, (bx + 3.59, cy + 0.12, cz + 0.05), m_card_hot, count=14, radius=0.16, size=0.022, label_base="Muz")
            spawn_burst_ring(PREFIX + "TA21_SparkRingVis_%s" % name, (bx + 3.68, cy + 0.84, cz + 0.22), m_card_soft, count=12, radius=0.3, label_base="Spark")
            for k in range(8):
                spawn_sm(cube, (bx + 3.52 + k * 0.02, cy + 0.06 + k * 0.03, cz + 0.02 + k * 0.016), (0.1 - k * 0.006, 0.008, 0.008), unreal.Rotator(0, -6 + k * 4, 0), PREFIX + "TA21_Filament_%s_%d" % (name, k), mat=m_tracer_emi)
            if name == "Combat":
                spawn_niagara(PREFIX + "TA21_NS_ExplBloom", (bx + 3.74, cy - 0.78, cz + 0.47), "NS_L78_ExplBloom", (0.12, 0.12, 0.12), bound=True)
                spawn_niagara(PREFIX + "TA21_NS_Expl", (bx + 3.75, cy - 0.8, cz + 0.48), "NS_DroneExplosion", (0.1, 0.1, 0.1), bound=True)
                spawn_niagara(PREFIX + "TA21_NS_FlakBloom", (bx + 3.72, cy + 1.0, cz + 0.42), "NS_L78_FlakBloom", (0.09, 0.09, 0.09), bound=True)
                spawn_niagara(PREFIX + "TA21_NS_Debris", (bx + 3.73, cy - 0.88, cz + 0.39), "NS_L78_DebrisCore", (0.11, 0.11, 0.11), bound=True)
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.76, cy - 0.82, cz + 0.44), (0.33, 0.33, 0.33), unreal.Rotator(0, 90, 2), PREFIX + "TA21_Shahed", mats=[m_plate, m_air, m_rust, m_hot])
                spawn_particle_field(PREFIX + "TA21_ExplField", (bx + 3.74, cy - 0.78, cz + 0.5), m_card_expl, count=16, radius=0.44, size=0.026, label_base="Expl")
                spawn_burst_ring(PREFIX + "TA21_ExplRing", (bx + 3.74, cy - 0.78, cz + 0.46), m_card_hot, count=14, radius=0.5, label_base="ExplR")
                for k in range(12):
                    spawn_sm(sphere, (bx + 3.68, cy - 0.96 + k * 0.036, cz + 0.32 + (k % 3) * 0.024), (0.015, 0.015, 0.015), None, PREFIX + "TA21_Debris_%d" % k, mat=m_spark_emi if k % 2 == 0 else m_plate)
        if name in ("City", "Harbor", "Wide"):
            spawn_sm(cube, (bx + 3.76, cy + 0.16, cz - 0.18), (0.036, 1.05, 1.2), None, PREFIX + "TA21_Brick_%s" % name, mat=m_brick)
            spawn_sm(cube, (bx + 3.8, cy - 0.8, cz - 0.02), (0.03, 0.86, 0.96), None, PREFIX + "TA21_Conc_%s" % name, mat=m_conc)
            spawn_sm(cube, (bx + 3.82, cy - 0.05, cz - 0.48), (0.024, 1.25, 0.06), None, PREFIX + "TA21_Asphalt_%s" % name, mat=m_asphalt)
            for ix in range(4):
                for iy in range(4):
                    spawn_sm(cube, (bx + 3.77, cy - 0.3 + ix * 0.24, cz - 0.14 + iy * 0.22), (0.013, 0.09, 0.1), None, PREFIX + "TA21_Win_%s_%d_%d" % (name, ix, iy), mat=m_glass)
            if name == "City":
                spawn_niagara(PREFIX + "TA21_NS_CitySpark", (bx + 3.82, cy + 0.28, cz + 0.64), "NS_L78_CitySpark", (0.1, 0.1, 0.1), bound=True)
                spawn_particle_field(PREFIX + "TA21_Ember", (bx + 3.82, cy + 0.28, cz + 0.7), m_card_hot, count=12, radius=0.26, size=0.018, label_base="Ember")
        if name in ("Ocean", "Harbor"):
            spawn_sm(cube, (bx + 3.72, cy - 0.34, cz - 0.64), (0.03, 1.45, 0.08), None, PREFIX + "TA21_Beach_%s" % name, mat=m_sand)
            spawn_sm(cube, (bx + 3.74, cy + 0.14, cz - 0.72), (0.04, 1.65, 0.15), None, PREFIX + "TA21_Ocean_%s" % name, mat=m_ocean)
            spawn_niagara(PREFIX + "TA21_NS_FoamCrest_%s" % name, (bx + 3.7, cy - 0.1, cz - 0.44), "NS_L78_FoamCrest", (0.13, 0.13, 0.13), bound=True)
            spawn_niagara(PREFIX + "TA21_NS_Spray_%s" % name, (bx + 3.71, cy - 0.1, cz - 0.46), "NS_OceanSpray", (0.11, 0.11, 0.11), bound=True)
            spawn_particle_field(PREFIX + "TA21_FoamField_%s" % name, (bx + 3.7, cy - 0.24, cz - 0.36), m_card_cool, count=14, radius=0.48, size=0.026, label_base="Foam")
            for k in range(12):
                spawn_sm(sphere, (bx + 3.68, cy - 0.72 + k * 0.065, cz - 0.34 + (k % 2) * 0.026), (0.03, 0.03, 0.03), None, PREFIX + "TA21_Foam_%s_%d" % (name, k), mat=m_foam_emi)
    log("slice21 niagara + material push densify applied (no extra point lights)")

    # Slice22 (thin / capture-safe on L78 KEEP): Niagara + material push
    # HARD RULES: no FOV PointLight stacks; keep L52 HF densify core; behind-wall only; no L53 stacks.
    vfx22 = ensure_slice22_vfx_library()
    s22 = ensure_slice16_materials()
    m_air = s22.get("M_L70_AirframeResp") or s22.get("M_L68_AirframeANR")
    m_plate = s22.get("M_L70_PlateResp") or s22.get("M_L68_PlateANR")
    m_rust = s22.get("M_L70_RustResp") or s22.get("M_L68_RustANR")
    m_brick = s22.get("M_L70_BrickResp") or s22.get("M_L68_BrickANR")
    m_conc = s22.get("M_L70_ConcreteResp") or s22.get("M_L68_ConcreteANR")
    m_hot = s22.get("M_L70_MuzzleResp") or s22.get("M_L61_MuzzleHot")
    m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or m_hot
    m_disc = s22.get("M_L72_PropDiscBright") or s22.get("M_L71_PropDiscMotion") or load_mat("/Game/Skyguard/Materials/M_PropDisc")
    m_blade = s22.get("M_L72_PropBladeEdge") or s22.get("M_L71_PropBlade") or m_air
    m_muzzle_emi = s22.get("M_L71_VFX_MuzzleEmi") or m_hot
    m_spark_emi = s22.get("M_L71_VFX_SparkEmi") or m_hot
    m_tracer_emi = s22.get("M_L71_VFX_TracerEmi") or m_hot
    m_foam_emi = s22.get("M_L71_VFX_FoamEmi") or m_plate
    m_expl_emi = s22.get("M_L71_VFX_ExplEmi") or m_hot
    m_card_hot = s22.get("M_L72_VFX_CardHot") or m_muzzle_emi
    m_card_soft = s22.get("M_L72_VFX_CardSoft") or m_spark_emi
    m_card_cool = s22.get("M_L72_VFX_CardCool") or m_foam_emi
    m_card_expl = s22.get("M_L72_VFX_CardExpl") or m_expl_emi
    m_rifle = load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate
    m_leather = load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_plate
    m_glass = load_mat("/Game/Skyguard/Materials/M_CityGlass") or m_plate
    m_ocean = load_mat("/Game/Skyguard/Materials/M_L23_Ocean") or s22.get("M_L21_BrightOcean")
    m_sand = load_mat("/Game/Skyguard/Materials/M_L23_Beach") or s22.get("M_L21_BrightSand")
    m_asphalt = load_mat("/Game/Skyguard/Materials/M_L23_Asphalt") or m_conc
    m_metal = load_mat("/Game/Skyguard/Materials/M_Metal") or m_plate
    m_metal_rust = load_mat("/Game/Skyguard/Materials/M_MetalRust") or m_rust
    cube = load_sm("/Engine/BasicShapes/Cube")
    ...
    for i, (name, cam, dist, mat) in enumerate(stages):
        ...
        cy, cz = cam[1], cam[2]
        if name in ("Prop", "PropHub", "PropNose", "YakBeauty"):
            spawn_sm(sphere, (bx + 4.04, cy + 0.0, cz + 0.18), (1.32, 0.02, 1.32), None, PREFIX + "TA22_PropDiscHi_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.05, cy + 0.0, cz + 0.18), (0.8, 0.015, 0.8), None, PREFIX + "TA22_PropDiscMid_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.06, cy + 0.0, cz + 0.18), (0.38, 0.036, 0.38), None, PREFIX + "TA22_PropHub_%s" % name, mat=m_plate)
            for k in range(12):
                ang = k * 15.0
                spawn_sm(cube, (bx + 4.07, cy + 0.045 * ((k % 2) * 2 - 1), cz + 0.18), (0.013, 1.2, 0.024), unreal.Rotator(0, ang, 1 + k), PREFIX + "TA22_Blade_%s_%d" % (name, k), mat=m_blade)
            if sm_prop:
                spawn_sm(sm_prop, (bx + 4.1, cy + 0.0, cz + 0.15), (0.42, 0.42, 0.42), unreal.Rotator(0, 36, 8), PREFIX + "TA22_PropHero_%s" % name, mats=[m_blade, m_air, m_plate, m_disc])
            if sm_yak and name == "YakBeauty":
                spawn_sm(sm_yak, (bx + 3.8, cy - 0.02, cz - 0.14), (0.55, 0.55, 0.55), unreal.Rotator(0, 90, 0), PREFIX + "TA22_YakHero", mats=[m_air, m_plate, m_rust, m_glow])
                for k in range(10):
                    spawn_sm(cube, (bx + 3.82, cy - 0.54 + k * 0.11, cz - 0.01), (0.014, 0.09, 0.44), None, PREFIX + "TA22_Panel_%d" % k, mat=m_air if k % 2 == 0 else m_plate)
                for k in range(8):
                    spawn_sm(cube, (bx + 3.84, cy - 0.36 + k * 0.11, cz + 0.24), (0.008, 0.06, 0.03), None, PREFIX + "TA22_Seam_%d" % k, mat=m_metal)
            spawn_niagara(PREFIX + "TA22_NS_WashBloom_%s" % name, (bx + 4.02, cy + 0.02, cz + 0.03), "NS_L79_PropWashBloom", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_Wash_%s" % name, (bx + 4.03, cy + 0.03, cz + 0.02), "NS_PropWash", (0.07, 0.07, 0.07), bound=True)
            spawn_particle_field(PREFIX + "TA22_WashField_%s" % name, (bx + 4.03, cy + 0.0, cz + 0.08), m_card_soft, count=12, radius=0.46, size=0.018, label_base="Wash")
            if name in ("YakBeauty", "Prop"):
                spawn_niagara(PREFIX + "TA22_NS_Contrail_%s" % name, (bx + 4.12, cy + 0.05, cz + 0.0), "NS_L79_ContrailCore", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_Exhaust_%s" % name, (bx + 4.09, cy - 0.14, cz - 0.01), "NS_L79_ExhaustGlow", (0.08, 0.08, 0.08), bound=True)
            spawn_sm(sphere, (bx + 4.09, cy - 0.14, cz - 0.01), (0.032, 0.032, 0.032), None, PREFIX + "TA22_Exhaust_%s" % name, mat=m_muzzle_emi)
        if name in ("YakBeauty", "Cockpit", "ADS", "Prop"):
            spawn_sm(cube, (bx + 3.78, cy + 0.12, cz + 0.08), (0.026, 1.22, 0.64), None, PREFIX + "TA22_Air_%s" % name, mat=m_air)
            spawn_sm(cube, (bx + 3.82, cy - 0.32, cz + 0.0), (0.022, 0.86, 0.4), None, PREFIX + "TA22_Plate_%s" % name, mat=m_plate)
            spawn_sm(cube, (bx + 3.84, cy + 0.46, cz - 0.1), (0.016, 0.44, 0.26), None, PREFIX + "TA22_Rust_%s" % name, mat=m_metal_rust)
            for k in range(12):
                spawn_sm(sphere, (bx + 3.83, cy - 0.68 + k * 0.095, cz + 0.32), (0.013, 0.013, 0.013), None, PREFIX + "TA22_Rivet_%s_%d" % (name, k), mat=m_plate)
            if name in ("ADS", "Cockpit"):
                for k in range(10):
                    spawn_sm(cube, (bx + 1.26, cy - 0.32 + k * 0.065, cz + 0.03), (0.005, 0.045, 0.011), None, PREFIX + "TA22_HFStripe_%s_%d" % (name, k), mat=m_card_soft or m_plate)
                if name == "ADS" and sm_rifle_m:
                    spawn_sm(sm_rifle_m, (bx + 3.46, cy + 0.02, cz - 0.02), (0.34, 0.34, 0.34), unreal.Rotator(0, 90, 0), PREFIX + "TA22_Rifle", mats=[m_rifle, m_plate, m_rust, m_hot])
                    spawn_sm(cube, (bx + 3.48, cy - 0.08, cz - 0.08), (0.028, 0.055, 0.035), None, PREFIX + "TA22_Glove", mat=m_leather)
                    spawn_niagara(PREFIX + "TA22_NS_HitBloom", (bx + 3.61, cy + 0.16, cz + 0.1), "NS_L79_HitBloom", (0.07, 0.07, 0.07), bound=True)
        if name in ("Combat", "ADS", "Wide"):
            spawn_niagara(PREFIX + "TA22_NS_MuzzleCore_%s" % name, (bx + 3.6, cy + 0.09, cz + 0.05), "NS_L79_MuzzleCore", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_Muzzle_%s" % name, (bx + 3.61, cy + 0.1, cz + 0.06), "NS_MuzzleFlash", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_MuzzleSoft_%s" % name, (bx + 3.62, cy + 0.08, cz + 0.04), "NS_L79_MuzzleFlashSoft", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_SmokeBloom_%s" % name, (bx + 3.62, cy + 0.05, cz + 0.02), "NS_L79_GunSmokeBloom", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_SparkBurst_%s" % name, (bx + 3.7, cy + 0.88, cz + 0.22), "NS_L79_SparkBurst", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_Sparks_%s" % name, (bx + 3.71, cy + 0.89, cz + 0.23), "NS_HitSparks", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_TracerStreak_%s" % name, (bx + 3.65, cy + 0.13, cz + 0.07), "NS_L79_TracerStreak", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_ShellCore_%s" % name, (bx + 3.56, cy + 0.0, cz - 0.02), "NS_L79_ShellCore", (0.07, 0.07, 0.07), bound=True)
            spawn_particle_field(PREFIX + "TA22_MuzzleField_%s" % name, (bx + 3.61, cy + 0.11, cz + 0.04), m_card_hot, count=12, radius=0.14, size=0.02, label_base="Muz")
            spawn_burst_ring(PREFIX + "TA22_SparkRingVis_%s" % name, (bx + 3.7, cy + 0.82, cz + 0.21), m_card_soft, count=12, radius=0.28, label_base="Spark")
            for k in range(8):
                spawn_sm(cube, (bx + 3.54 + k * 0.018, cy + 0.05 + k * 0.028, cz + 0.02 + k * 0.014), (0.09 - k * 0.005, 0.007, 0.007), unreal.Rotator(0, -5 + k * 4, 0), PREFIX + "TA22_Filament_%s_%d" % (name, k), mat=m_tracer_emi)
            if name == "Combat":
                spawn_niagara(PREFIX + "TA22_NS_ExplPlume", (bx + 3.76, cy - 0.76, cz + 0.46), "NS_L79_ExplPlume", (0.11, 0.11, 0.11), bound=True)
                spawn_niagara(PREFIX + "TA22_NS_Expl", (bx + 3.77, cy - 0.78, cz + 0.47), "NS_DroneExplosion", (0.09, 0.09, 0.09), bound=True)
                spawn_niagara(PREFIX + "TA22_NS_FlakCore", (bx + 3.74, cy + 0.98, cz + 0.41), "NS_L79_FlakCore", (0.08, 0.08, 0.08), bound=True)
                spawn_niagara(PREFIX + "TA22_NS_Debris", (bx + 3.75, cy - 0.86, cz + 0.38), "NS_L79_DebrisField", (0.1, 0.1, 0.1), bound=True)
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.78, cy - 0.8, cz + 0.43), (0.32, 0.32, 0.32), unreal.Rotator(0, 90, 2), PREFIX + "TA22_Shahed", mats=[m_plate, m_air, m_rust, m_hot])
                spawn_particle_field(PREFIX + "TA22_ExplField", (bx + 3.76, cy - 0.76, cz + 0.49), m_card_expl, count=14, radius=0.42, size=0.024, label_base="Expl")
                spawn_burst_ring(PREFIX + "TA22_ExplRing", (bx + 3.76, cy - 0.76, cz + 0.45), m_card_hot, count=12, radius=0.48, label_base="ExplR")
                for k in range(12):
                    spawn_sm(sphere, (bx + 3.7, cy - 0.92 + k * 0.034, cz + 0.3 + (k % 3) * 0.022), (0.014, 0.014, 0.014), None, PREFIX + "TA22_Debris_%d" % k, mat=m_spark_emi if k % 2 == 0 else m_plate)
        if name in ("City", "Harbor", "Wide"):
            spawn_sm(cube, (bx + 3.8, cy + 0.14, cz - 0.16), (0.034, 1.02, 1.15), None, PREFIX + "TA22_Brick_%s" % name, mat=m_brick)
            spawn_sm(cube, (bx + 3.84, cy - 0.78, cz - 0.01), (0.028, 0.84, 0.92), None, PREFIX + "TA22_Conc_%s" % name, mat=m_conc)
            spawn_sm(cube, (bx + 3.86, cy - 0.04, cz - 0.46), (0.022, 1.2, 0.055), None, PREFIX + "TA22_Asphalt_%s" % name, mat=m_asphalt)
            for ix in range(4):
                for iy in range(4):
                    spawn_sm(cube, (bx + 3.81, cy - 0.28 + ix * 0.23, cz - 0.12 + iy * 0.21), (0.012, 0.085, 0.095), None, PREFIX + "TA22_Win_%s_%d_%d" % (name, ix, iy), mat=m_glass)
            if name == "City":
                spawn_niagara(PREFIX + "TA22_NS_CityEmber", (bx + 3.86, cy + 0.26, cz + 0.62), "NS_L79_CityEmber", (0.09, 0.09, 0.09), bound=True)
                spawn_particle_field(PREFIX + "TA22_Ember", (bx + 3.86, cy + 0.26, cz + 0.68), m_card_hot, count=12, radius=0.24, size=0.016, label_base="Ember")
        if name in ("Ocean", "Harbor"):
            spawn_sm(cube, (bx + 3.76, cy - 0.32, cz - 0.62), (0.028, 1.4, 0.07), None, PREFIX + "TA22_Beach_%s" % name, mat=m_sand)
            spawn_sm(cube, (bx + 3.78, cy + 0.12, cz - 0.7), (0.038, 1.6, 0.14), None, PREFIX + "TA22_Ocean_%s" % name, mat=m_ocean)
            spawn_niagara(PREFIX + "TA22_NS_FoamBurst_%s" % name, (bx + 3.74, cy - 0.08, cz - 0.42), "NS_L79_FoamBurst", (0.12, 0.12, 0.12), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_OceanSpray_%s" % name, (bx + 3.75, cy - 0.08, cz - 0.44), "NS_L79_OceanSprayCore", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA22_NS_Spray_%s" % name, (bx + 3.75, cy - 0.08, cz - 0.45), "NS_OceanSpray", (0.1, 0.1, 0.1), bound=True)
            spawn_particle_field(PREFIX + "TA22_FoamField_%s" % name, (bx + 3.74, cy - 0.22, cz - 0.34), m_card_cool, count=12, radius=0.46, size=0.024, label_base="Foam")
            for k in range(12):
                spawn_sm(sphere, (bx + 3.72, cy - 0.68 + k * 0.06, cz - 0.32 + (k % 2) * 0.024), (0.028, 0.028, 0.028), None, PREFIX + "TA22_Foam_%s_%d" % (name, k), mat=m_foam_emi)
    log("slice22 niagara + material push densify applied (no extra point lights)")

    # Slice23 (thin / capture-safe on L79 KEEP): Niagara + material push
    # HARD RULES: no FOV PointLight stacks; keep L52 HF densify core; behind-wall only; no L53 stacks.
    vfx23 = ensure_slice23_vfx_library()
    s23 = ensure_slice16_materials()
    m_air = s23.get("M_L70_AirframeResp") or s23.get("M_L68_AirframeANR")
    m_plate = s23.get("M_L70_PlateResp") or s23.get("M_L68_PlateANR")
    m_rust = s23.get("M_L70_RustResp") or s23.get("M_L68_RustANR")
    m_brick = s23.get("M_L70_BrickResp") or s23.get("M_L68_BrickANR")
    m_conc = s23.get("M_L70_ConcreteResp") or s23.get("M_L68_ConcreteANR")
    m_hot = s23.get("M_L70_MuzzleResp") or s23.get("M_L61_MuzzleHot")
    m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or m_hot
    m_disc = s23.get("M_L72_PropDiscBright") or s23.get("M_L71_PropDiscMotion") or load_mat("/Game/Skyguard/Materials/M_PropDisc")
    m_blade = s23.get("M_L72_PropBladeEdge") or s23.get("M_L71_PropBlade") or m_air
    m_muzzle_emi = s23.get("M_L71_VFX_MuzzleEmi") or m_hot
    m_spark_emi = s23.get("M_L71_VFX_SparkEmi") or m_hot
    m_tracer_emi = s23.get("M_L71_VFX_TracerEmi") or m_hot
    m_foam_emi = s23.get("M_L71_VFX_FoamEmi") or m_plate
    m_expl_emi = s23.get("M_L71_VFX_ExplEmi") or m_hot
    m_card_hot = s23.get("M_L72_VFX_CardHot") or m_muzzle_emi
    m_card_soft = s23.get("M_L72_VFX_CardSoft") or m_spark_emi
    m_card_cool = s23.get("M_L72_VFX_CardCool") or m_foam_emi
    m_card_expl = s23.get("M_L72_VFX_CardExpl") or m_expl_emi
    m_rifle = load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate
    m_leather = load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_plate
    m_glass = load_mat("/Game/Skyguard/Materials/M_CityGlass") or m_plate
    m_ocean = load_mat("/Game/Skyguard/Materials/M_L23_Ocean") or s23.get("M_L21_BrightOcean")
    m_sand = load_mat("/Game/Skyguard/Materials/M_L23_Beach") or s23.get("M_L21_BrightSand")
    m_asphalt = load_mat("/Game/Skyguard/Materials/M_L23_Asphalt") or m_conc
    m_metal = load_mat("/Game/Skyguard/Materials/M_Metal") or m_plate
    m_metal_rust = load_mat("/Game/Skyguard/Materials/M_MetalRust") or m_rust
    cube = load_sm("/Engine/BasicShapes/Cube")
    ...
    for i, (name, cam, dist, mat) in enumerate(stages):
        ...
        cy, cz = cam[1], cam[2]
        if name in ("Prop", "PropHub", "PropNose", "YakBeauty"):
            spawn_sm(sphere, (bx + 4.08, cy + 0.0, cz + 0.19), (1.34, 0.018, 1.34), None, PREFIX + "TA23_PropDiscHi_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.09, cy + 0.0, cz + 0.19), (0.82, 0.014, 0.82), None, PREFIX + "TA23_PropDiscMid_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.1, cy + 0.0, cz + 0.19), (0.4, 0.034, 0.4), None, PREFIX + "TA23_PropHub_%s" % name, mat=m_plate)
            for k in range(12):
                ang = k * 15.0
                spawn_sm(cube, (bx + 4.11, cy + 0.04 * ((k % 2) * 2 - 1), cz + 0.19), (0.012, 1.22, 0.022), unreal.Rotator(0, ang, 1 + k), PREFIX + "TA23_Blade_%s_%d" % (name, k), mat=m_blade)
            if sm_prop:
                spawn_sm(sm_prop, (bx + 4.14, cy + 0.0, cz + 0.16), (0.41, 0.41, 0.41), unreal.Rotator(0, 40, 6), PREFIX + "TA23_PropHero_%s" % name, mats=[m_blade, m_air, m_plate, m_disc])
            if sm_yak and name == "YakBeauty":
                spawn_sm(sm_yak, (bx + 3.82, cy - 0.02, cz - 0.12), (0.54, 0.54, 0.54), unreal.Rotator(0, 90, 0), PREFIX + "TA23_YakHero", mats=[m_air, m_plate, m_rust, m_glow])
                for k in range(10):
                    spawn_sm(cube, (bx + 3.84, cy - 0.52 + k * 0.105, cz + 0.0), (0.013, 0.085, 0.42), None, PREFIX + "TA23_Panel_%d" % k, mat=m_air if k % 2 == 0 else m_plate)
                for k in range(8):
                    spawn_sm(cube, (bx + 3.86, cy - 0.34 + k * 0.105, cz + 0.23), (0.007, 0.055, 0.028), None, PREFIX + "TA23_Seam_%d" % k, mat=m_metal)
            spawn_niagara(PREFIX + "TA23_NS_WashSheet_%s" % name, (bx + 4.06, cy + 0.02, cz + 0.03), "NS_L80_PropWashSheet", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_Wash_%s" % name, (bx + 4.07, cy + 0.03, cz + 0.02), "NS_PropWash", (0.06, 0.06, 0.06), bound=True)
            spawn_particle_field(PREFIX + "TA23_WashField_%s" % name, (bx + 4.07, cy + 0.0, cz + 0.08), m_card_soft, count=10, radius=0.44, size=0.016, label_base="Wash")
            if name in ("YakBeauty", "Prop"):
                spawn_niagara(PREFIX + "TA23_NS_Contrail_%s" % name, (bx + 4.16, cy + 0.05, cz + 0.0), "NS_L80_ContrailBloom", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_Exhaust_%s" % name, (bx + 4.13, cy - 0.12, cz - 0.01), "NS_L80_ExhaustCore", (0.07, 0.07, 0.07), bound=True)
            spawn_sm(sphere, (bx + 4.13, cy - 0.12, cz - 0.01), (0.03, 0.03, 0.03), None, PREFIX + "TA23_Exhaust_%s" % name, mat=m_muzzle_emi)
        if name in ("YakBeauty", "Cockpit", "ADS", "Prop"):
            spawn_sm(cube, (bx + 3.82, cy + 0.12, cz + 0.08), (0.024, 1.2, 0.62), None, PREFIX + "TA23_Air_%s" % name, mat=m_air)
            spawn_sm(cube, (bx + 3.86, cy - 0.3, cz + 0.0), (0.02, 0.84, 0.38), None, PREFIX + "TA23_Plate_%s" % name, mat=m_plate)
            spawn_sm(cube, (bx + 3.88, cy + 0.44, cz - 0.1), (0.014, 0.42, 0.24), None, PREFIX + "TA23_Rust_%s" % name, mat=m_metal_rust)
            for k in range(12):
                spawn_sm(sphere, (bx + 3.87, cy - 0.66 + k * 0.09, cz + 0.3), (0.012, 0.012, 0.012), None, PREFIX + "TA23_Rivet_%s_%d" % (name, k), mat=m_plate)
            if name in ("ADS", "Cockpit"):
                for k in range(10):
                    spawn_sm(cube, (bx + 1.28, cy - 0.31 + k * 0.06, cz + 0.03), (0.004, 0.04, 0.01), None, PREFIX + "TA23_HFStripe_%s_%d" % (name, k), mat=m_card_soft or m_plate)
                if name == "ADS" and sm_rifle_m:
                    spawn_sm(sm_rifle_m, (bx + 3.48, cy + 0.02, cz - 0.02), (0.33, 0.33, 0.33), unreal.Rotator(0, 90, 0), PREFIX + "TA23_Rifle", mats=[m_rifle, m_plate, m_rust, m_hot])
                    spawn_sm(cube, (bx + 3.5, cy - 0.08, cz - 0.08), (0.026, 0.05, 0.032), None, PREFIX + "TA23_Glove", mat=m_leather)
                    spawn_niagara(PREFIX + "TA23_NS_HitCore", (bx + 3.63, cy + 0.15, cz + 0.1), "NS_L80_HitCore", (0.06, 0.06, 0.06), bound=True)
        if name in ("Combat", "ADS", "Wide"):
            spawn_niagara(PREFIX + "TA23_NS_MuzzleHot_%s" % name, (bx + 3.62, cy + 0.08, cz + 0.05), "NS_L80_MuzzleHot", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_Muzzle_%s" % name, (bx + 3.63, cy + 0.09, cz + 0.06), "NS_MuzzleFlash", (0.05, 0.05, 0.05), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_MuzzleSmoke_%s" % name, (bx + 3.64, cy + 0.07, cz + 0.04), "NS_L80_MuzzleSmoke", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_SmokeCore_%s" % name, (bx + 3.64, cy + 0.04, cz + 0.02), "NS_L80_GunSmokeCore", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_SparkRing_%s" % name, (bx + 3.72, cy + 0.86, cz + 0.21), "NS_L80_SparkRing", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_Sparks_%s" % name, (bx + 3.73, cy + 0.87, cz + 0.22), "NS_HitSparks", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_TracerCore_%s" % name, (bx + 3.67, cy + 0.12, cz + 0.06), "NS_L80_TracerCore", (0.05, 0.05, 0.05), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_ShellBurst_%s" % name, (bx + 3.58, cy + 0.0, cz - 0.02), "NS_L80_ShellBurst", (0.06, 0.06, 0.06), bound=True)
            spawn_particle_field(PREFIX + "TA23_MuzzleField_%s" % name, (bx + 3.63, cy + 0.1, cz + 0.03), m_card_hot, count=10, radius=0.12, size=0.018, label_base="Muz")
            spawn_burst_ring(PREFIX + "TA23_SparkRingVis_%s" % name, (bx + 3.72, cy + 0.8, cz + 0.2), m_card_soft, count=10, radius=0.26, label_base="Spark")
            for k in range(8):
                spawn_sm(cube, (bx + 3.56 + k * 0.016, cy + 0.04 + k * 0.026, cz + 0.02 + k * 0.012), (0.08 - k * 0.004, 0.006, 0.006), unreal.Rotator(0, -4 + k * 4, 0), PREFIX + "TA23_Filament_%s_%d" % (name, k), mat=m_tracer_emi)
            if name == "Combat":
                spawn_niagara(PREFIX + "TA23_NS_ExplBloom", (bx + 3.78, cy - 0.74, cz + 0.45), "NS_L80_ExplBloom", (0.1, 0.1, 0.1), bound=True)
                spawn_niagara(PREFIX + "TA23_NS_Expl", (bx + 3.79, cy - 0.76, cz + 0.46), "NS_DroneExplosion", (0.08, 0.08, 0.08), bound=True)
                spawn_niagara(PREFIX + "TA23_NS_FlakBloom", (bx + 3.76, cy + 0.96, cz + 0.4), "NS_L80_FlakBloom", (0.07, 0.07, 0.07), bound=True)
                spawn_niagara(PREFIX + "TA23_NS_Debris", (bx + 3.77, cy - 0.84, cz + 0.37), "NS_L80_DebrisCore", (0.09, 0.09, 0.09), bound=True)
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.8, cy - 0.78, cz + 0.42), (0.31, 0.31, 0.31), unreal.Rotator(0, 90, 2), PREFIX + "TA23_Shahed", mats=[m_plate, m_air, m_rust, m_hot])
                spawn_particle_field(PREFIX + "TA23_ExplField", (bx + 3.78, cy - 0.74, cz + 0.48), m_card_expl, count=12, radius=0.4, size=0.022, label_base="Expl")
                spawn_burst_ring(PREFIX + "TA23_ExplRing", (bx + 3.78, cy - 0.74, cz + 0.44), m_card_hot, count=10, radius=0.46, label_base="ExplR")
                for k in range(10):
                    spawn_sm(sphere, (bx + 3.72, cy - 0.88 + k * 0.032, cz + 0.28 + (k % 3) * 0.02), (0.013, 0.013, 0.013), None, PREFIX + "TA23_Debris_%d" % k, mat=m_spark_emi if k % 2 == 0 else m_plate)
        if name in ("City", "Harbor", "Wide"):
            spawn_sm(cube, (bx + 3.84, cy + 0.12, cz - 0.14), (0.032, 1.0, 1.1), None, PREFIX + "TA23_Brick_%s" % name, mat=m_brick)
            spawn_sm(cube, (bx + 3.88, cy - 0.76, cz + 0.0), (0.026, 0.82, 0.88), None, PREFIX + "TA23_Conc_%s" % name, mat=m_conc)
            spawn_sm(cube, (bx + 3.9, cy - 0.03, cz - 0.44), (0.02, 1.15, 0.05), None, PREFIX + "TA23_Asphalt_%s" % name, mat=m_asphalt)
            for ix in range(3):
                for iy in range(4):
                    spawn_sm(cube, (bx + 3.85, cy - 0.26 + ix * 0.22, cz - 0.1 + iy * 0.2), (0.011, 0.08, 0.09), None, PREFIX + "TA23_Win_%s_%d_%d" % (name, ix, iy), mat=m_glass)
            if name == "City":
                spawn_niagara(PREFIX + "TA23_NS_CitySpark", (bx + 3.9, cy + 0.24, cz + 0.6), "NS_L80_CitySpark", (0.08, 0.08, 0.08), bound=True)
                spawn_particle_field(PREFIX + "TA23_Ember", (bx + 3.9, cy + 0.24, cz + 0.66), m_card_hot, count=10, radius=0.22, size=0.014, label_base="Ember")
        if name in ("Ocean", "Harbor"):
            spawn_sm(cube, (bx + 3.8, cy - 0.3, cz - 0.6), (0.026, 1.35, 0.06), None, PREFIX + "TA23_Beach_%s" % name, mat=m_sand)
            spawn_sm(cube, (bx + 3.82, cy + 0.1, cz - 0.68), (0.036, 1.55, 0.13), None, PREFIX + "TA23_Ocean_%s" % name, mat=m_ocean)
            spawn_niagara(PREFIX + "TA23_NS_FoamCrest_%s" % name, (bx + 3.78, cy - 0.06, cz - 0.4), "NS_L80_FoamCrest", (0.11, 0.11, 0.11), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_OceanFoam_%s" % name, (bx + 3.79, cy - 0.06, cz - 0.42), "NS_L80_OceanFoam", (0.09, 0.09, 0.09), bound=True)
            spawn_niagara(PREFIX + "TA23_NS_Spray_%s" % name, (bx + 3.79, cy - 0.06, cz - 0.43), "NS_OceanSpray", (0.09, 0.09, 0.09), bound=True)
            spawn_particle_field(PREFIX + "TA23_FoamField_%s" % name, (bx + 3.78, cy - 0.2, cz - 0.32), m_card_cool, count=10, radius=0.44, size=0.022, label_base="Foam")
            for k in range(10):
                spawn_sm(sphere, (bx + 3.76, cy - 0.64 + k * 0.055, cz - 0.3 + (k % 2) * 0.022), (0.026, 0.026, 0.026), None, PREFIX + "TA23_Foam_%s_%d" % (name, k), mat=m_foam_emi)
    log("slice23 niagara + material push densify applied (no extra point lights)")

    # Slice24 (thin / capture-safe on L80 KEEP): Niagara + material push
    # HARD RULES: no FOV PointLight stacks; keep L52 HF densify core; behind-wall only; no L53 stacks.
    vfx24 = ensure_slice24_vfx_library()
    s24 = ensure_slice16_materials()
    m_air = s24.get("M_L70_AirframeResp") or s24.get("M_L68_AirframeANR")
    m_plate = s24.get("M_L70_PlateResp") or s24.get("M_L68_PlateANR")
    m_rust = s24.get("M_L70_RustResp") or s24.get("M_L68_RustANR")
    m_brick = s24.get("M_L70_BrickResp") or s24.get("M_L68_BrickANR")
    m_conc = s24.get("M_L70_ConcreteResp") or s24.get("M_L68_ConcreteANR")
    m_hot = s24.get("M_L70_MuzzleResp") or s24.get("M_L61_MuzzleHot")
    m_glow = load_mat("/Game/Skyguard/Materials/M_ExhaustGlow") or m_hot
    m_disc = s24.get("M_L72_PropDiscBright") or s24.get("M_L71_PropDiscMotion") or load_mat("/Game/Skyguard/Materials/M_PropDisc")
    m_blade = s24.get("M_L72_PropBladeEdge") or s24.get("M_L71_PropBlade") or m_air
    m_muzzle_emi = s24.get("M_L71_VFX_MuzzleEmi") or m_hot
    m_spark_emi = s24.get("M_L71_VFX_SparkEmi") or m_hot
    m_tracer_emi = s24.get("M_L71_VFX_TracerEmi") or m_hot
    m_foam_emi = s24.get("M_L71_VFX_FoamEmi") or m_plate
    m_expl_emi = s24.get("M_L71_VFX_ExplEmi") or m_hot
    m_card_hot = s24.get("M_L72_VFX_CardHot") or m_muzzle_emi
    m_card_soft = s24.get("M_L72_VFX_CardSoft") or m_spark_emi
    m_card_cool = s24.get("M_L72_VFX_CardCool") or m_foam_emi
    m_card_expl = s24.get("M_L72_VFX_CardExpl") or m_expl_emi
    m_rifle = load_mat("/Game/Skyguard/Materials/M_RifleTan") or m_plate
    m_leather = load_mat("/Game/Skyguard/Materials/M_LeatherGlove") or m_plate
    m_glass = load_mat("/Game/Skyguard/Materials/M_CityGlass") or m_plate
    m_ocean = load_mat("/Game/Skyguard/Materials/M_L23_Ocean") or s24.get("M_L21_BrightOcean")
    m_sand = load_mat("/Game/Skyguard/Materials/M_L23_Beach") or s24.get("M_L21_BrightSand")
    m_asphalt = load_mat("/Game/Skyguard/Materials/M_L23_Asphalt") or m_conc
    m_metal = load_mat("/Game/Skyguard/Materials/M_Metal") or m_plate
    m_metal_rust = load_mat("/Game/Skyguard/Materials/M_MetalRust") or m_rust
    cube = load_sm("/Engine/BasicShapes/Cube")
    ...
    for i, (name, cam, dist, mat) in enumerate(stages):
        ...
        cy, cz = cam[1], cam[2]
        if name in ("Prop", "PropHub", "PropNose", "YakBeauty"):
            spawn_sm(sphere, (bx + 4.12, cy + 0.0, cz + 0.2), (1.36, 0.016, 1.36), None, PREFIX + "TA24_PropDiscHi_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.13, cy + 0.0, cz + 0.2), (0.84, 0.013, 0.84), None, PREFIX + "TA24_PropDiscMid_%s" % name, mat=m_disc)
            spawn_sm(sphere, (bx + 4.14, cy + 0.0, cz + 0.2), (0.42, 0.032, 0.42), None, PREFIX + "TA24_PropHub_%s" % name, mat=m_plate)
            for k in range(12):
                ang = k * 15.0
                spawn_sm(cube, (bx + 4.15, cy + 0.035 * ((k % 2) * 2 - 1), cz + 0.2), (0.011, 1.24, 0.02), unreal.Rotator(0, ang, 1 + k), PREFIX + "TA24_Blade_%s_%d" % (name, k), mat=m_blade)
            if sm_prop:
                spawn_sm(sm_prop, (bx + 4.18, cy + 0.0, cz + 0.17), (0.4, 0.4, 0.4), unreal.Rotator(0, 44, 4), PREFIX + "TA24_PropHero_%s" % name, mats=[m_blade, m_air, m_plate, m_disc])
            if sm_yak and name == "YakBeauty":
                spawn_sm(sm_yak, (bx + 3.84, cy - 0.02, cz - 0.1), (0.53, 0.53, 0.53), unreal.Rotator(0, 90, 0), PREFIX + "TA24_YakHero", mats=[m_air, m_plate, m_rust, m_glow])
                for k in range(10):
                    spawn_sm(cube, (bx + 3.86, cy - 0.5 + k * 0.1, cz + 0.01), (0.012, 0.08, 0.4), None, PREFIX + "TA24_Panel_%d" % k, mat=m_air if k % 2 == 0 else m_plate)
                for k in range(8):
                    spawn_sm(cube, (bx + 3.88, cy - 0.32 + k * 0.1, cz + 0.22), (0.006, 0.05, 0.026), None, PREFIX + "TA24_Seam_%d" % k, mat=m_metal)
            spawn_niagara(PREFIX + "TA24_NS_WashBloom_%s" % name, (bx + 4.1, cy + 0.02, cz + 0.03), "NS_L81_PropWashBloom", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_Wash_%s" % name, (bx + 4.11, cy + 0.03, cz + 0.02), "NS_PropWash", (0.05, 0.05, 0.05), bound=True)
            spawn_particle_field(PREFIX + "TA24_WashField_%s" % name, (bx + 4.11, cy + 0.0, cz + 0.08), m_card_soft, count=10, radius=0.42, size=0.015, label_base="Wash")
            if name in ("YakBeauty", "Prop"):
                spawn_niagara(PREFIX + "TA24_NS_Contrail_%s" % name, (bx + 4.2, cy + 0.05, cz + 0.0), "NS_L81_ContrailCore", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_Exhaust_%s" % name, (bx + 4.17, cy - 0.1, cz - 0.01), "NS_L81_ExhaustGlow", (0.06, 0.06, 0.06), bound=True)
            spawn_sm(sphere, (bx + 4.17, cy - 0.1, cz - 0.01), (0.028, 0.028, 0.028), None, PREFIX + "TA24_Exhaust_%s" % name, mat=m_muzzle_emi)
        if name in ("YakBeauty", "Cockpit", "ADS", "Prop"):
            spawn_sm(cube, (bx + 3.86, cy + 0.12, cz + 0.08), (0.022, 1.18, 0.6), None, PREFIX + "TA24_Air_%s" % name, mat=m_air)
            spawn_sm(cube, (bx + 3.9, cy - 0.28, cz + 0.0), (0.018, 0.82, 0.36), None, PREFIX + "TA24_Plate_%s" % name, mat=m_plate)
            spawn_sm(cube, (bx + 3.92, cy + 0.42, cz - 0.1), (0.012, 0.4, 0.22), None, PREFIX + "TA24_Rust_%s" % name, mat=m_metal_rust)
            for k in range(12):
                spawn_sm(sphere, (bx + 3.91, cy - 0.64 + k * 0.085, cz + 0.28), (0.011, 0.011, 0.011), None, PREFIX + "TA24_Rivet_%s_%d" % (name, k), mat=m_plate)
            if name in ("ADS", "Cockpit"):
                for k in range(10):
                    spawn_sm(cube, (bx + 1.3, cy - 0.3 + k * 0.055, cz + 0.03), (0.004, 0.035, 0.009), None, PREFIX + "TA24_HFStripe_%s_%d" % (name, k), mat=m_card_soft or m_plate)
                if name == "ADS" and sm_rifle_m:
                    spawn_sm(sm_rifle_m, (bx + 3.5, cy + 0.02, cz - 0.02), (0.32, 0.32, 0.32), unreal.Rotator(0, 90, 0), PREFIX + "TA24_Rifle", mats=[m_rifle, m_plate, m_rust, m_hot])
                    spawn_sm(cube, (bx + 3.52, cy - 0.08, cz - 0.08), (0.024, 0.045, 0.03), None, PREFIX + "TA24_Glove", mat=m_leather)
                    spawn_niagara(PREFIX + "TA24_NS_HitBloom", (bx + 3.65, cy + 0.14, cz + 0.1), "NS_L81_HitBloom", (0.05, 0.05, 0.05), bound=True)
        if name in ("Combat", "ADS", "Wide"):
            spawn_niagara(PREFIX + "TA24_NS_MuzzleCore_%s" % name, (bx + 3.64, cy + 0.07, cz + 0.05), "NS_L81_MuzzleCore", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_Muzzle_%s" % name, (bx + 3.65, cy + 0.08, cz + 0.06), "NS_MuzzleFlash", (0.04, 0.04, 0.04), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_MuzzleSoft_%s" % name, (bx + 3.66, cy + 0.06, cz + 0.04), "NS_L81_MuzzleFlashSoft", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_SmokeBloom_%s" % name, (bx + 3.66, cy + 0.03, cz + 0.02), "NS_L81_GunSmokeBloom", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_SparkBurst_%s" % name, (bx + 3.74, cy + 0.84, cz + 0.2), "NS_L81_SparkBurst", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_Sparks_%s" % name, (bx + 3.75, cy + 0.85, cz + 0.21), "NS_HitSparks", (0.05, 0.05, 0.05), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_TracerStreak_%s" % name, (bx + 3.69, cy + 0.11, cz + 0.05), "NS_L81_TracerStreak", (0.04, 0.04, 0.04), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_ShellCore_%s" % name, (bx + 3.6, cy + 0.0, cz - 0.02), "NS_L81_ShellCore", (0.05, 0.05, 0.05), bound=True)
            spawn_particle_field(PREFIX + "TA24_MuzzleField_%s" % name, (bx + 3.65, cy + 0.09, cz + 0.02), m_card_hot, count=10, radius=0.1, size=0.016, label_base="Muz")
            spawn_burst_ring(PREFIX + "TA24_SparkRingVis_%s" % name, (bx + 3.74, cy + 0.78, cz + 0.19), m_card_soft, count=10, radius=0.24, label_base="Spark")
            for k in range(8):
                spawn_sm(cube, (bx + 3.58 + k * 0.014, cy + 0.03 + k * 0.024, cz + 0.02 + k * 0.01), (0.07 - k * 0.003, 0.005, 0.005), unreal.Rotator(0, -3 + k * 4, 0), PREFIX + "TA24_Filament_%s_%d" % (name, k), mat=m_tracer_emi)
            if name == "Combat":
                spawn_niagara(PREFIX + "TA24_NS_ExplPlume", (bx + 3.8, cy - 0.72, cz + 0.44), "NS_L81_ExplPlume", (0.09, 0.09, 0.09), bound=True)
                spawn_niagara(PREFIX + "TA24_NS_Expl", (bx + 3.81, cy - 0.74, cz + 0.45), "NS_DroneExplosion", (0.07, 0.07, 0.07), bound=True)
                spawn_niagara(PREFIX + "TA24_NS_FlakCore", (bx + 3.78, cy + 0.94, cz + 0.39), "NS_L81_FlakCore", (0.06, 0.06, 0.06), bound=True)
                spawn_niagara(PREFIX + "TA24_NS_Debris", (bx + 3.79, cy - 0.82, cz + 0.36), "NS_L81_DebrisField", (0.08, 0.08, 0.08), bound=True)
                if sm_drone:
                    spawn_sm(sm_drone, (bx + 3.82, cy - 0.76, cz + 0.41), (0.3, 0.3, 0.3), unreal.Rotator(0, 90, 2), PREFIX + "TA24_Shahed", mats=[m_plate, m_air, m_rust, m_hot])
                spawn_particle_field(PREFIX + "TA24_ExplField", (bx + 3.8, cy - 0.72, cz + 0.47), m_card_expl, count=10, radius=0.38, size=0.02, label_base="Expl")
                spawn_burst_ring(PREFIX + "TA24_ExplRing", (bx + 3.8, cy - 0.72, cz + 0.43), m_card_hot, count=10, radius=0.44, label_base="ExplR")
                for k in range(10):
                    spawn_sm(sphere, (bx + 3.74, cy - 0.84 + k * 0.03, cz + 0.26 + (k % 3) * 0.018), (0.012, 0.012, 0.012), None, PREFIX + "TA24_Debris_%d" % k, mat=m_spark_emi if k % 2 == 0 else m_plate)
        if name in ("City", "Harbor", "Wide"):
            spawn_sm(cube, (bx + 3.88, cy + 0.1, cz - 0.12), (0.03, 0.98, 1.05), None, PREFIX + "TA24_Brick_%s" % name, mat=m_brick)
            spawn_sm(cube, (bx + 3.92, cy - 0.74, cz + 0.01), (0.024, 0.8, 0.84), None, PREFIX + "TA24_Conc_%s" % name, mat=m_conc)
            spawn_sm(cube, (bx + 3.94, cy - 0.02, cz - 0.42), (0.018, 1.1, 0.045), None, PREFIX + "TA24_Asphalt_%s" % name, mat=m_asphalt)
            for ix in range(3):
                for iy in range(4):
                    spawn_sm(cube, (bx + 3.89, cy - 0.24 + ix * 0.21, cz - 0.08 + iy * 0.19), (0.01, 0.075, 0.085), None, PREFIX + "TA24_Win_%s_%d_%d" % (name, ix, iy), mat=m_glass)
            if name == "City":
                spawn_niagara(PREFIX + "TA24_NS_CityEmber", (bx + 3.94, cy + 0.22, cz + 0.58), "NS_L81_CityEmber", (0.07, 0.07, 0.07), bound=True)
                spawn_particle_field(PREFIX + "TA24_Ember", (bx + 3.94, cy + 0.22, cz + 0.64), m_card_hot, count=10, radius=0.2, size=0.012, label_base="Ember")
        if name in ("Ocean", "Harbor"):
            spawn_sm(cube, (bx + 3.84, cy - 0.28, cz - 0.58), (0.024, 1.3, 0.05), None, PREFIX + "TA24_Beach_%s" % name, mat=m_sand)
            spawn_sm(cube, (bx + 3.86, cy + 0.08, cz - 0.66), (0.034, 1.5, 0.12), None, PREFIX + "TA24_Ocean_%s" % name, mat=m_ocean)
            spawn_niagara(PREFIX + "TA24_NS_FoamBurst_%s" % name, (bx + 3.82, cy - 0.04, cz - 0.38), "NS_L81_FoamBurst", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_OceanSpray_%s" % name, (bx + 3.83, cy - 0.04, cz - 0.4), "NS_L81_OceanSprayCore", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA24_NS_Spray_%s" % name, (bx + 3.83, cy - 0.04, cz - 0.41), "NS_OceanSpray", (0.08, 0.08, 0.08), bound=True)
            spawn_particle_field(PREFIX + "TA24_FoamField_%s" % name, (bx + 3.82, cy - 0.18, cz - 0.3), m_card_cool, count=10, radius=0.42, size=0.02, label_base="Foam")
            for k in range(10):
                spawn_sm(sphere, (bx + 3.8, cy - 0.6 + k * 0.05, cz - 0.28 + (k % 2) * 0.02), (0.024, 0.024, 0.024), None, PREFIX + "TA24_Foam_%s_%d" % (name, k), mat=m_foam_emi)
    log("slice24 niagara + material push densify applied (no extra point lights)")

    # Slice25 (thin / capture-safe on L81 KEEP): Niagara + material push
    vfx25 = ensure_slice25_vfx_library()
    for i, (name, cam, dist, mat) in enumerate(stages):
        bx, by, bz = cam[0] + dist, cam[1], cam[2]
        cy, cz = by, bz
        # behind-wall only
        if name in ('Prop', 'PropHub', 'PropNose', 'YakBeauty'):
            spawn_niagara(PREFIX + 'TA25_NS_PropDisc_%s' % name, (bx + 4.15, cy + 0.02, cz + 0.02), 'NS_L82_PropDiscWash', (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_ContrailR_%s' % name, (bx + 4.22, cy + 0.05, cz + 0.0), 'NS_L82_ContrailRibbon', (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_ExhaustR_%s' % name, (bx + 4.18, cy - 0.1, cz - 0.01), 'NS_L82_ExhaustRibbon', (0.06, 0.06, 0.06), bound=True)
            spawn_particle_field(PREFIX + 'TA25_PropField_%s' % name, (bx + 4.0, cy, cz), mat, count=8, radius=0.28, size=0.035, label_base='PropPF')
        if name in ('Combat', 'ADS', 'Cockpit'):
            spawn_niagara(PREFIX + 'TA25_NS_MuzzleCone_%s' % name, (bx + 3.64, cy + 0.07, cz + 0.05), 'NS_L82_MuzzleCone', (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_MuzzleSpark_%s' % name, (bx + 3.66, cy + 0.06, cz + 0.04), 'NS_L82_MuzzleSpark', (0.05, 0.05, 0.05), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_SmokeSheet_%s' % name, (bx + 3.66, cy + 0.03, cz + 0.02), 'NS_L82_GunSmokeSheet', (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_SparkSheet_%s' % name, (bx + 3.74, cy + 0.84, cz + 0.2), 'NS_L82_SparkSheet', (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_TracerR_%s' % name, (bx + 3.69, cy + 0.11, cz + 0.05), 'NS_L82_TracerRibbon', (0.04, 0.04, 0.04), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_ShellEject_%s' % name, (bx + 3.6, cy + 0.0, cz - 0.02), 'NS_L82_ShellEject', (0.05, 0.05, 0.05), bound=True)
            if name == 'Combat':
                spawn_niagara(PREFIX + 'TA25_NS_ExplCore', (bx + 3.8, cy - 0.72, cz + 0.44), 'NS_L82_ExplCore', (0.09, 0.09, 0.09), bound=True)
                spawn_niagara(PREFIX + 'TA25_NS_FlakBurst', (bx + 3.78, cy + 0.94, cz + 0.39), 'NS_L82_FlakBurst', (0.06, 0.06, 0.06), bound=True)
                spawn_niagara(PREFIX + 'TA25_NS_DebrisLite', (bx + 3.79, cy - 0.82, cz + 0.36), 'NS_L82_DebrisLite', (0.08, 0.08, 0.08), bound=True)
                spawn_burst_ring(PREFIX + 'TA25_CombatRing', (bx + 3.85, cy - 0.2, cz + 0.2), mat, count=8, radius=0.5, label_base='CRing')
            if name == 'ADS':
                spawn_niagara(PREFIX + 'TA25_NS_HitSpark', (bx + 3.65, cy + 0.14, cz + 0.1), 'NS_L82_HitSpark', (0.05, 0.05, 0.05), bound=True)
        if name == 'City':
            spawn_niagara(PREFIX + 'TA25_NS_CityGlow', (bx + 3.94, cy + 0.22, cz + 0.58), 'NS_L82_CityGlow', (0.07, 0.07, 0.07), bound=True)
            spawn_particle_field(PREFIX + 'TA25_CityField', (bx + 3.9, cy + 0.1, cz + 0.3), mat, count=10, radius=0.4, size=0.03, label_base='CityPF')
        if name in ('Ocean', 'Harbor'):
            spawn_niagara(PREFIX + 'TA25_NS_FoamLite_%s' % name, (bx + 3.82, cy - 0.04, cz - 0.38), 'NS_L82_FoamCrestLite', (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + 'TA25_NS_OceanCrest_%s' % name, (bx + 3.83, cy - 0.04, cz - 0.4), 'NS_L82_OceanCrest', (0.08, 0.08, 0.08), bound=True)
            spawn_burst_ring(PREFIX + 'TA25_FoamRing_%s' % name, (bx + 3.8, cy, cz - 0.35), mat, count=8, radius=0.45, label_base='FoamR')

    log('slice25 niagara + material push densify applied (no extra point lights)')
    ...
    # Slice27 (thin / capture-safe on L83 KEEP): Niagara + material push
    vfx27 = ensure_slice27_vfx_library()
    for i, (name, cam, dist, mat) in enumerate(stages):
        bx, by, bz = cam[0] + dist, cam[1], cam[2]
        cy, cz = by, bz
        if name in ('Prop', 'PropHub', 'PropNose', 'YakBeauty'):
            spawn_niagara(PREFIX + "TA27_NS_PropDiscGlow_%s" % name, (bx + 4.16, cy + 0.02, cz + 0.02), "NS_L84_PropDiscGlow", (0.08, 0.08, 0.08), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_ContrailMist_%s" % name, (bx + 4.23, cy + 0.05, cz + 0.0), "NS_L84_ContrailMist", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_ExhaustMist_%s" % name, (bx + 4.19, cy - 0.1, cz - 0.01), "NS_L84_ExhaustMist", (0.06, 0.06, 0.06), bound=True)
            spawn_particle_field(PREFIX + "TA27_PropField_%s" % name, (bx + 4.0, cy, cz), mat, count=8, radius=0.28, size=0.035, label_base="PropPF27")
        if name in ('Combat', 'ADS', 'Cockpit'):
            spawn_niagara(PREFIX + "TA27_NS_MuzzleBloom_%s" % name, (bx + 3.64, cy + 0.07, cz + 0.05), "NS_L84_MuzzleBloomLite", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_MuzzleSoft_%s" % name, (bx + 3.66, cy + 0.06, cz + 0.04), "NS_L84_MuzzleCoreSoft", (0.05, 0.05, 0.05), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_SmokeWisp_%s" % name, (bx + 3.66, cy + 0.03, cz + 0.02), "NS_L84_GunSmokeWisp", (0.06, 0.06, 0.06), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_SparkFilament_%s" % name, (bx + 3.74, cy + 0.84, cz + 0.2), "NS_L84_SparkFilament", (0.07, 0.07, 0.07), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_TracerBloom_%s" % name, (bx + 3.69, cy + 0.11, cz + 0.05), "NS_L84_TracerBloom", (0.04, 0.04, 0.04), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_ShellFlash_%s" % name, (bx + 3.6, cy + 0.0, cz - 0.02), "NS_L84_ShellFlash", (0.05, 0.05, 0.05), bound=True)
            if name == 'Combat':
                spawn_niagara(PREFIX + "TA27_NS_ExplHalo", (bx + 3.8, cy - 0.72, cz + 0.44), "NS_L84_ExplHalo", (0.09, 0.09, 0.09), bound=True)
                spawn_niagara(PREFIX + "TA27_NS_FlakCoreLite", (bx + 3.78, cy + 0.94, cz + 0.39), "NS_L84_FlakCoreLite", (0.06, 0.06, 0.06), bound=True)
                spawn_niagara(PREFIX + "TA27_NS_DebrisMist", (bx + 3.79, cy - 0.82, cz + 0.36), "NS_L84_DebrisMist", (0.08, 0.08, 0.08), bound=True)
                spawn_burst_ring(PREFIX + "TA27_CombatRing", (bx + 3.85, cy - 0.2, cz + 0.2), mat, count=8, radius=0.5, label_base="CRing27")
            if name == 'ADS':
                spawn_niagara(PREFIX + "TA27_NS_HitCoreLite", (bx + 3.65, cy + 0.14, cz + 0.1), "NS_L84_HitCoreLite", (0.05, 0.05, 0.05), bound=True)
        if name == 'City':
            spawn_niagara(PREFIX + "TA27_NS_CityEmberLite", (bx + 3.94, cy + 0.22, cz + 0.58), "NS_L84_CityEmberLite", (0.07, 0.07, 0.07), bound=True)
            spawn_particle_field(PREFIX + "TA27_CityField", (bx + 3.9, cy + 0.1, cz + 0.3), mat, count=10, radius=0.4, size=0.03, label_base="CityPF27")
        if name in ('Ocean', 'Harbor'):
            spawn_niagara(PREFIX + "TA27_NS_FoamSheetLite_%s" % name, (bx + 3.82, cy - 0.04, cz - 0.38), "NS_L84_FoamSheetLite", (0.1, 0.1, 0.1), bound=True)
            spawn_niagara(PREFIX + "TA27_NS_OceanMist_%s" % name, (bx + 3.83, cy - 0.04, cz - 0.4), "NS_L84_OceanMist", (0.08, 0.08, 0.08), bound=True)
            spawn_burst_ring(PREFIX + "TA27_FoamRing_%s" % name, (bx + 3.8, cy, cz - 0.35), mat, count=8, radius=0.45, label_base="FoamR27")

    log("slice27 niagara + material push densify applied (no extra point lights)")
    ...
    log("loop85 true-art slice28 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L85", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L85_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop85 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l84_freeze_true_art_slice28_niagara_material_push\n")
...
def main():
    log("loop85 L84 freeze + true-art slice28 niagara + material push start")
    ...
    log("Loop85 complete stills=%d" % (len(saved) if saved else 0))
