...
PREFIX = "AAA_L74_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L74"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L74"
...
def densify():
    ...
    log("loop74 mat palette size=%d" % len(mats))
    ...
    log("loop74 true-art slice17 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L74", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L74_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop74 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l73_freeze_true_art_slice17_visible_vfx_prop_airframe")
...
def main():
    log("loop74 L73 freeze + true-art slice17 deeper Niagara emitter pass + visible particle language start")
    ...
    log("Loop74 complete stills=%d" % (len(saved) if saved else 0))
