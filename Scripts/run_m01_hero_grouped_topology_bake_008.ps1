...
$contract = if ($ContractPath) {
    ...
    Join-Path $ProjectRoot "Docs\AAA_Review\M01_HERO_GROUPED_TOPOLOGY_BAKE_008_CONTRACT.json"
}
...
$generator = Join-Path $ProjectRoot "Scripts\blender_m01_hero_grouped_topology_bake_008.py"
$verifier = Join-Path $ProjectRoot "Scripts\verify_skyguard_m01_hero_grouped_topology_bake_008.py"
$analyzer = Join-Path $ProjectRoot "Scripts\analyze_m01_grouped_topology_007_visual_failures.py"
...
$inputFiles = @($contract, $generator, $verifier, $analyzer, $supervisorSource)
...
foreach ($field in @(
    ...
    if ($value) {
        $inputFiles += Resolve-ProjectPath ([string]$value)
    }
}
if ($contractData.corrective_map_contract) {
    $inputFiles += Resolve-ProjectPath (
        [string]$contractData.corrective_map_contract.source_manifest
    )
    $inputFiles += Resolve-ProjectPath (
        [string]$contractData.corrective_map_contract.source_review_receipt
    )
}
$inputFiles = @($inputFiles | Sort-Object -Unique)
...
try {
    ...
    foreach ($asset in $manifestData.assets) {
        foreach ($group in $asset.groups) {
            foreach ($map in $group.maps) {
                $isInheritedPass = $map.provenance.mode -eq "hash_verified_reuse"
                $reviewQueue += [ordered]@{
                    asset = $asset.id
                    group = $group.id
                    map_type = $map.type
                    ...
                    provenance_mode = $map.provenance.mode
                    review_status = if ($isInheritedPass) {
    ...
    [ordered]@{
        ...
        build_id = $buildId
        total_map_count = 24
        required_map_count = 6
        inherited_pass_map_count = 18
        reviewed_map_count = 18
