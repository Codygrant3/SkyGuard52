...
PREFIX = "AAA_L81_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L81"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L81"
...
def densify():
    ...
    log("loop81 mat palette size=%d" % len(mats))
    ...
    log("loop81 true-art slice24 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L81", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L81_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop81 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l80_freeze_true_art_slice24_niagara_material_push")
...
def main():
    log("loop81 L80 freeze + true-art slice24 niagara + material push start")
    ...
    log("Loop81 complete stills=%d" % (len(saved) if saved else 0))
