...
PREFIX = "AAA_L77_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L77"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L77"
...
def densify():
    ...
    log("loop77 mat palette size=%d" % len(mats))
    ...
    log("loop77 true-art slice20 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L77", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L77_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop77 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l76_freeze_true_art_slice20_niagara_material_push")
...
def main():
    log("loop77 L76 freeze + true-art slice20 niagara + material push start")
    ...
    log("Loop77 complete stills=%d" % (len(saved) if saved else 0))
