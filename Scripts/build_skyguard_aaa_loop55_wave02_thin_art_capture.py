...
PREFIX = "AAA_L55_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L55"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L55"
...
def densify():
    ...
    log("loop55 mat palette size=%d" % len(mats))
    ...
    for i,(name, cam, dist, mat) in enumerate(stages):
        ...
        # CO-001 City/Wide: bright brick tower vertical strip (behind wall, small)
        if name in ("City", "Wide"):
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop55 wave02 thin art densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L55", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L55_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop55 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l54_freeze_wave02_selected_thin_art_7\n")
...
def main():
    log("loop55 L54 freeze + wave02 selected thin art deltas (7 proposals) start")
    ...
    log("Loop55 complete stills=%d" % (len(saved) if saved else 0))
