"""Fresh-process persistence audit for M07 playable integration."""
...
REPORT = ROOT / "Saved/Reports/M07_PLAYABLE_INTEGRATION_AUDIT.json"
...
def main():
    if not unreal.EditorLevelLibrary.load_level(TARGET_MAP):
        raise RuntimeError("M07 playable map failed fresh load")
    ...
    bosses = [
        actor
        for actor in actors
        if actor.get_class().get_name() == "SkyguardRadarGhostBoss"
    ]
    governed = [
        actor
        for actor in actors
    ...
    checks = {
        ...
        "assembly_identity": mission_id == "M07_SearchIntercept",
        "assembly_valid": valid,
        "distinct_island_route": len(route_points) >= 4
        and route_x_span >= 70000.0
        and route_y_span >= 40000.0,
        "search_landmarks": all(
            role in landmark_roles
            for role in [
                "RadarObjective",
                "NavigationReference",
                "IdentificationTraffic",
            ]
        ),
        "single_integration": len(directors) == 1,
        "single_yak": names.count("SkyguardYak52Aircraft") == 1,
        "single_radar_ghost": len(bosses) == 1,
    ...
    report = {
        "schema": "skyguard.m07-playable-integration-audit.v1",
        ...
        "limitations": [
            ...
            "Island, fleet, radar and boss remain proxy art.",
        ],
    }
    REPORT.parent.mkdir(parents=True, exist_ok=True)
    REPORT.write_text(json.dumps(report, indent=2), encoding="utf-8")
    unreal.log("[SkyguardM07PlayableAudit] " + json.dumps(report))
    if report["gate"] != "PASS":
        raise RuntimeError("M07 playable persistence audit failed")
