...
PREFIX = "AAA_L38_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L38"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L38"
...
def densify():
    ...
    if pp:
        ...
        try:
            ...
            pass
    log("loop38 minimal marker densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L38", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L38_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop38 stills\n")
...
def main():
    log("loop38 minimal unique-marker capture smoke probe start")
    ...
    log("Loop38 complete stills=%d" % (len(saved) if saved else 0))
