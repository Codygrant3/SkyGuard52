...
PREFIX = "AAA_L73_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L73"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L73"
...
def densify():
    ...
    log("loop73 mat palette size=%d" % len(mats))
    ...
    log("loop73 true-art slice16 densify done")
    return stages
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L73", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = [("AAA_Cam_L73_%s" % name, cam, (0.0,0.0,0.0)) for name, cam, dist, mat in stages]
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop73 stills\n")
        f.write("time=%s\n" % time.strftime("%Y-%m-%dT%H:%M:%S"))
        f.write("note=l72_freeze_true_art_slice16_deeper_niagara_visible_particles")
...
def main():
    ...
    log("Loop73 complete stills=%d" % (len(saved) if saved else 0))
