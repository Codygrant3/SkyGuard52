...
PREFIX = "AAA_L32_"
OUT_DIR = r"D:\Skyguard52\Saved\Screenshots\AAA_L32"
RT_PATH = "/Game/Skyguard/Capture/RT_AAA_L32"
...
def capture(out_dir, stages):
    ...
    if unreal.EditorAssetLibrary.does_asset_exist(RT_PATH):
        ...
        rt = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            "RT_AAA_L32", "/Game/Skyguard/Capture", unreal.TextureRenderTarget2D, unreal.TextureRenderTargetFactoryNew()
        )
    ...
    cams = []
    for name, cam, dist, ny, nz in stages:
        cams.append(("AAA_Cam_L32_%s" % name, cam, (0.0, 0.0, 0.0)))
    # context cams folded into yaw0 stages with HF boards
    ...
    with open(man, "w", encoding="utf-8") as f:
        f.write("Skyguard AAA Loop32 stills\n")
...
def main():
    log("loop32 multi-cam HF contrast lock start")
    ...
    log("Loop32 complete stills=%d" % (len(saved) if saved else 0))
