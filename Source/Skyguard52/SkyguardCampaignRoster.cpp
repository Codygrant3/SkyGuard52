...
namespace
{
	...
	const FSkyguardCampaignMissionSpec GMissions[] = {
		Make(TEXT("M01_CoastalIntercept"), TEXT("First Contact"),
			TEXT("First hop. Helmet cannon on the inbound swarm, rockets the trucks. Switch stations - this is not a drone intercept."),
		...
		Make(TEXT("M02_HarborShield"), TEXT("Harbor Breaker"),
			TEXT("Fifteen-minute proof. 30 mm the boats, Hydra the shoreline armor, Hellfire the radar. Choice: kill the net or save the damaged ship. Strip the patrol ship by system, then helmet-sight extract."),
		...
		Make(TEXT("M03_ConvoyEscort"), TEXT("Broken Highway"),
			TEXT("Ridge tanks will kill the convoy. Save Hellfires for the armor - 30 mm the technicals, Hydra the clusters."),
		...
		Make(TEXT("M04_NightBlackout"), TEXT("Night Eyes"),
			TEXT("Blackout city. Sensor thermal. Hellfire the radar vans, 30 mm rooftop heat - do not hose dark blocks in helmet-sight."),
		...
		Make(TEXT("M06_AirfieldDefense"), TEXT("Dust Offensive"),
			TEXT("Sand. Hellfire ADA before it pins our armor. Hydra the dug-in line, 30 mm anything on the fence."),
		...
		Make(TEXT("M07_SearchIntercept"), TEXT("Downed Bird"),
			TEXT("Hold the wreck. Prioritize what can kill the rescue - cannon the swarm, rockets the armor, missile the rotor that tries to lift first."),
		...
		Make(TEXT("M08_RescueCover"), TEXT("Iron Rain"),
			TEXT("Artillery and SAMs at once. Hellfire the launchers first, Hydra the gun line - do not dump the cannon into the barrage."),
		...
		Make(TEXT("M09_SaturationAttack"), TEXT("Hunter-Killer"),
			TEXT("TEL hunt. Sensor-track the convoy. Hellfire the real launcher - 30 mm the decoys. Do not spend the last missile on a dummy."),
		...
		Make(TEXT("M10_EvacuationFinale"), TEXT("Fortress Dawn"),
			TEXT("Fortress. Hellfire the radar, Hydra the bunkers, strip the ship by system. Save a missile for the rival gunship on extract."),
