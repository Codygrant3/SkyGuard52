#include "SkyguardPhase4LandscapePerformanceCaptureAttempt06.h"
...
namespace
{
	const TCHAR* RequiredAttempt06ContractId =
		TEXT("P4.5-M01-LANDSCAPE-VISIBLE-006");
	constexpr double Attempt06CsvStartActivationTimeoutSeconds = 5.0;
	constexpr double Attempt06CsvFlushGraceSeconds = 2.0;
}

void USkyguardPhase4LandscapePerformanceCaptureAttempt06::Initialize(
	FSubsystemCollectionBase& Collection)
{
	...
	FParse::Value(
		...
		MeasuredSeconds);
	bRequested =
		ContractId == RequiredAttempt06ContractId
		&& !RunId.IsEmpty()
		&& !ExpectedMap.IsEmpty()
		&& !ReceiptPath.IsEmpty()
...
void USkyguardPhase4LandscapePerformanceCaptureAttempt06::Deinitialize()
{
...
TStatId
USkyguardPhase4LandscapePerformanceCaptureAttempt06::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(
		USkyguardPhase4LandscapePerformanceCaptureAttempt06,
		STATGROUP_Tickables);
}

bool USkyguardPhase4LandscapePerformanceCaptureAttempt06::IsTickable() const
{
	return bRequested && !bFinalized;
}

void USkyguardPhase4LandscapePerformanceCaptureAttempt06::Tick(
	float DeltaTime)
{
	if (!bWarmupStarted)
	{
	...
	if (!bMeasurementStarted)
	{
		...
		if (bMeasurementStartRequested
			&& Now - MeasurementStartRequestPlatformSeconds
				>= Attempt06CsvStartActivationTimeoutSeconds)
		{
	...
	if (bStopRequested
		&& Now - StopRequestPlatformSeconds
			>= Attempt06CsvFlushGraceSeconds)
	{
		FinishAfterCsvFlush();
	}
}

bool USkyguardPhase4LandscapePerformanceCaptureAttempt06::IsWorldReady(
	FString& OutIssue) const
{
	...
	if (GEngine->GameViewport->Viewport->GetSizeXY()
		!= FIntPoint(1920, 1080))
	{
...
void USkyguardPhase4LandscapePerformanceCaptureAttempt06::BeginWarmup()
{
	bWarmupStarted = true;
...
RequestMeasurementStart()
{
	...
	FCsvProfiler::SetMetadata(
		TEXT("startup_frames_excluded"),
		TEXT("true"));
	UE_LOG(
		LogTemp,
		Display,
...
void
USkyguardPhase4LandscapePerformanceCaptureAttempt06::
FailMeasurementStart(const FString& Issue)
{
	bFinalized = true;
	...
	FPlatformMisc::RequestExitWithStatus(
		false,
		1,
		TEXT("SkyguardP46CsvStartFailure"));
}
...
FinishAfterCsvFlush()
{
	...
	FPlatformMisc::RequestExitWithStatus(
		...
		TEXT("SkyguardP46LandscapeProfileComplete"));
}

bool
USkyguardPhase4LandscapePerformanceCaptureAttempt06::
WriteReceipt(const FString& Gate, const FString& Issue) const
{
	...
	Receipt->SetBoolField(
		TEXT("same_process_warmup_and_measurement"),
		true);
